...
#include "geometry/render_gltf_client/internal_http_service_curl.h"
...
void AddField(DataFieldsMap* data_map, const std::string& field_name, const std::string& field_data) {
...
template <typename T>
void AddField(DataFieldsMap* data_map, const std::string& field_name, T field_data) {
...
/* Overload required to prevent bad conversions between const char* and
 std::string. */
template <>
void AddField<const char*>(DataFieldsMap* data_map, const std::string& field_name, const char* field_data) {
    AddField(data_map, field_name, std::string(field_data));
}

// Overload for RenderImageType, to avoid bad conversion via std::to_string.
template <>
void AddField<RenderImageType>(DataFieldsMap* data_map, const std::string& field_name, RenderImageType field_data) {
    switch (field_data) {
...
void VerifyImageSize(const std::string& image_path, Image* image, int expected_width, int expected_height) {
    DRAKE_DEMAND(image != nullptr);
    const int actual_width = image->width();
    const int actual_height = image->height();
    if (actual_width != expected_width || actual_height != expected_height) {
        throw std::runtime_error(
                fmt::format("RenderClient: expected to import (width={},height={}) from the file "
                            "'{}', but got (width={},height={}).",
                            expected_width, expected_height, image_path, actual_width, actual_height));
    }
}
...
RenderClient::RenderClient(const RenderEngineGltfClientParams& params)
    : temp_directory_{drake::temp_directory()}, params_{params}, http_service_{std::make_unique<HttpServiceCurl>()} {}
...
RenderClient::~RenderClient() {
    ...
    if (params_.cleanup) {
        try {
            ...
            log()->debug("RenderClient: could not delete '{}'. {}", temp_directory_, e.what());
        }
    } else if (params_.verbose) {
        // NOTE: This gets printed twice because of cloning, cannot be avoided.
        log()->debug("RenderClient: temporary directory '{}' was *NOT* deleted.", temp_directory_);
    }
}

std::string RenderClient::RenderOnServer(const RenderCameraCore& camera_core,
    ...
    const HttpResponse response = http_service_->PostForm(temp_directory_, url, field_map,
    ...
    if (!response.Good()) {
        ...
        const std::string service_error_message = response.service_error_message.value_or("None.");
  ...
  Server Message:  {})""",
    ...
    if (!response.data_path.has_value()) {
        throw std::runtime_error(
                fmt::format("RenderClient: error from POST to {}, HTTP code={}: the server was "
    ...
    const std::optional<ImageIo::Metadata> header = ImageIo{}.LoadMetadata(bin_out_path);
    if (header.has_value()) {
    ...
    throw std::runtime_error(
            fmt::format("RenderClient: while trying to render the scene '{}' with a sha256 hash "
...
std::string RenderClient::ComputeSha256(const std::string& path) {
    ...
    if (!f_in.good()) {
        throw std::runtime_error(fmt::format("RenderClient: cannot open file '{}'.", path));
    }
    return Sha256::Checksum(&f_in).to_string();
}

std::string RenderClient::RenameHttpServiceResponse(const std::string& response_data_path,
    ...
    if (fs::exists(destination)) {
        throw std::runtime_error(fmt::format("RenderClient: refusing to rename '{}' to '{}', file already exists!",
...
void RenderClient::LoadColorImage(const std::string& path, ImageRgba8U* color_image_out) {
...
void RenderClient::LoadDepthImage(const std::string& path, ImageDepth32F* depth_image_out) {
            ...
            [depth_image_out]<typename SomeImage>(SomeImage& image) {
                if constexpr (std::is_same_v<SomeImage, ImageDepth16U>) {
                    ...
                    *depth_image_out = std::move(image);
                    ...
                    throw std::runtime_error("RenderClient: unsupported depth pixel type");
                }
            },
            image_any);
