...
public:
    ...
    std::string RenderOnServer(const render::RenderCameraCore& camera_core,
    ...
    /* Renames the specified file `response_data_path` to have the same name as
     ...
     @throws std::exception
       ...
       the file. */
    static std::string RenameHttpServiceResponse(const std::string& response_data_path,
    ...
    static void LoadColorImage(const std::string& path, systems::sensors::ImageRgba8U* color_image_out);
    ...
    static void LoadDepthImage(const std::string& path, systems::sensors::ImageDepth32F* depth_image_out);
