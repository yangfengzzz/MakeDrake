...
using DebugData = std::vector<std::pair<const curl_infotype, const std::string>>;
...
void LogIfTrimmedWhitespaceNonEmpty(curl_infotype type, const std::string& message) {
    const auto trimmed = std::regex_replace(message, std::regex(R"(^\s+|\s+$)"), "");
...
HttpServiceCurl::HttpServiceCurl() : HttpService() {
    ...
    static CURLcode ignored = curl_global_init(CURL_GLOBAL_ALL | CURL_GLOBAL_ACK_EINTR);
    unused(ignored);
}
...
HttpResponse HttpServiceCurl::DoPostForm(const std::string& temp_directory,
    ...
    if (fs::exists(temp_bin_out)) {
        ...
        throw std::runtime_error(
                fmt::format("RenderClient: refusing to overwrite temporary file '{}' that "
    ...
    if (!bin_out.good()) {
        cleanup_curl(curl, form, headerlist);
        throw std::runtime_error(fmt::format("RenderClient: unable to open temporary file '{}'.", bin_out_path));
    }
    ...
    if (!bin_out.good()) {
        cleanup_curl(curl, form, headerlist);
        throw std::runtime_error(fmt::format("RenderClient: unable to wtite temporary file '{}'.", bin_out_path));
    }
    if (verbose) {
    ...
    if (server_gave_data_response) {
        ...
        try {
            ...
            drake::log()->debug("HttpServiceCurl unable to delete '{}'. {}", bin_out_path, e.what());
        }
    }
