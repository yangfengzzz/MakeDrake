...
class DRAKE_NO_EXPORT RenderEngineGltfClient : public render_vtk::internal::RenderEngineVtk {
public:
    ...
    RenderEngineGltfClient(const RenderEngineGltfClientParams& parameters = RenderEngineGltfClientParams());

    const RenderEngineGltfClientParams& get_params() const { return render_client_->get_params(); }

    const std::string& temp_directory() const { return render_client_->temp_directory(); }
    ...
    Eigen::Matrix4d CameraModelViewTransformMatrix(render_vtk::internal::ImageType image_type) const;
...
private:
    ...
    void ExportScene(const std::string& export_path, render_vtk::internal::ImageType image_type) const;
    ...
    void ImplementMesh(const std::filesystem::path& mesh_path, double scale, void* user_data);
    ...
    bool ImplementGltf(const std::filesystem::path& gltf_path,
                       double scale,
