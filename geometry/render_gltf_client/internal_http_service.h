...
using FileFieldsMap = std::map<std::string, std::pair<std::string, std::optional<std::string>>>;
...
struct HttpResponse {
    ...
    /* Whether or not the HTTP transaction was successful.  This includes
     ...
     @note
       ...
       scope of Drake. */
    bool Good() const { return !service_error_message.has_value() && (http_code >= 200 && http_code < 400); }
};
