...
GTEST_TEST(HttpServiceCurlTest, PostForm) {
...

    {
        ...
        temp_file.close();
        DRAKE_EXPECT_THROWS_MESSAGE(service.PostForm(temp_dir, url, {}, {}, verbose),
...

    {
        ...
        const auto all_write = fs::perms::owner_write | fs::perms::group_write | fs::perms::others_write;
        fs::permissions(temp_dir, all_write, fs::perm_options::remove);
        DRAKE_EXPECT_THROWS_MESSAGE(service.PostForm(temp_dir, url, {}, {}, verbose),
                                    fmt::format(".*unable to open temporary file '{}.*\\.curl'.", temp_dir));
...

    {
        ...
        EXPECT_EQ(res_1.service_error_message.value(), "Couldn't resolve host name");
        ...
        const auto res_2 = service.PostForm(
                ...
                {{"json", {test_json_path, test_json_mime}}, {"binary", {test_binary_path, test_binary_mime}}},
                verbose);
        EXPECT_FALSE(res_2.Good());
        EXPECT_FALSE(res_2.data_path.has_value());
        EXPECT_TRUE(res_2.service_error_message.has_value());
        EXPECT_EQ(res_2.service_error_message.value(), "Couldn't resolve host name");
