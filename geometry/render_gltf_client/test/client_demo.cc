...
#include "systems/sensors/pixel_types.h"
...
DEFINE_string(server_base_url, RenderEngineGltfClientParams{}.base_url, "The base_url for the render server.");
...
DEFINE_bool(cleanup,
            RenderEngineGltfClientParams{}.cleanup,
...
static bool valid_render_engine(const char* flagname, const std::string& val) {
    if (val == "vtk")
        ...
        return true;
    drake::log()->error("Invalid value for {}: '{}'; options: 'client' or 'vtk'.", flagname, val);
    return false;
}
DEFINE_string(render_engine, "client", "Renderer choice, options: 'client', 'vtk'.");
...
RigidTransformd ParseCameraPose(const std::string& input_str) {
    ...
    const RigidTransformd X_WB(RollPitchYawd{xyzrpy_numeric[3], xyzrpy_numeric[4], xyzrpy_numeric[5]},
...
bool IsValidSaveDirectory(const std::string& save_dir) {
    ...
    if (!std::filesystem::exists(save_dir) || !std::filesystem::is_directory(save_dir)) {
        throw std::logic_error(fmt::format("Provided save_dir {} is invalid", save_dir));
    }
    return true;
}
...
int DoMain() {
    ...
    Parser parser{&plant};
    parser.AddModels(FindResourceOrThrow("drake/geometry/render_gltf_client/test/example_scene.sdf"));

    DrakeLcm lcm;
    DrakeVisualizerd::AddToBuilder(&builder, scene_graph, &lcm);
    const ColorRenderCamera color_camera{{renderer_name, {640, 480, M_PI_4}, {0.01, 10.0}, {}}, false};
    ...
    RgbdSensor* camera = builder.AddSystem<RgbdSensor>(world_id, X_WB, color_camera, depth_camera);
    ...
    ImageToLcmImageArrayT* image_to_lcm_image_array = builder.template AddSystem<ImageToLcmImageArrayT>();
    image_to_lcm_image_array->set_name("converter");

    LcmPublisherSystem* image_array_lcm_publisher{nullptr};
    image_array_lcm_publisher = builder.template AddSystem(
            LcmPublisherSystem::Make<lcmt_image_array>("DRAKE_RGBD_CAMERA_IMAGES", &lcm, image_publish_period));
    ...
    const std::string filename = (std::filesystem::path(FLAGS_save_dir) / "{image_type}_{count:03}").string();

    if (FLAGS_color) {
        const auto& port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kRgba8U>("color");
        builder.Connect(camera->color_image_output_port(), port);

        if (save_images) {
            const auto& writer_port = image_writer->DeclareImageInputPort<PixelType::kRgba8U>("color", filename,
                                                                                              image_publish_period, 0.);
            builder.Connect(camera->color_image_output_port(), writer_port);
        }
    }

    if (FLAGS_depth) {
        const auto& port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kDepth32F>("depth");
        builder.Connect(camera->depth_image_32F_output_port(), port);

        if (save_images) {
            const auto& writer_port = image_writer->DeclareImageInputPort<PixelType::kDepth32F>(
    ...
    if (FLAGS_label) {
        const auto& port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kLabel16I>("label");
        builder.Connect(camera->label_image_output_port(), port);

        if (save_images) {
            const auto& writer_port = image_writer->DeclareImageInputPort<PixelType::kLabel16I>(
    ...
    auto& context = static_cast<systems::DiagramContext<double>&>(simulator.get_mutable_context());
    ...
    const RigidBody<double>& mustard_body =
            plant.GetBodyByName("base_link_mustard", plant.GetModelInstanceByName("example_scene::mustard_bottle"));
    const RigidTransformd X_WMustardBottle(RollPitchYawd{-M_PI / 2, 0, -M_PI / 2}, Vector3d(0, 0, 0));
    plant.SetFreeBodyPose(&plant_context, mustard_body, X_WMustardBottle);
    const SpatialVelocity<double> V_WMustardBottle(Vector3d{0.6, 0, 0}, Vector3d{0, 0, 0.1});
    plant.SetFreeBodySpatialVelocity(&plant_context, mustard_body, V_WMustardBottle);
