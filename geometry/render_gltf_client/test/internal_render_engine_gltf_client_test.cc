...
public:
    ...
    static const std::map<GeometryId, GltfRecord>& gltfs(const RenderEngineGltfClient& engine) { return engine.gltfs_; }
...
public:
    RenderEngineGltfClientTest()
        : color_camera_{{"proxy_render", {kTestImageWidth, kTestImageHeight, M_PI_4}, {0.11, 111.111}, {}}, false},
...
TEST_F(RenderEngineGltfClientTest, Constructor) {
    ...
    EXPECT_EQ(engine.get_params().GetUrl(), "http://notarealserver:8192/no_render");
...
TEST_F(RenderEngineGltfClientTest, Clone) {
    ...
    RenderEngineGltfClient* clone_engine = dynamic_cast<RenderEngineGltfClient*>(clone.get());
...
public:
    ...
    HttpResponse DoPostForm(const std::string& temp_directory,
        ...
        if (image_type == "color") {
            test_image_path = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_rgba_8U.png");
        } else if (image_type == "depth") {
            test_image_path = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_depth_32F.tiff");
...
void CheckExpectedFiles(const std::string& directory, bool cleanup, const std::string& extension) {
    const std::vector<fs::path> files = FindRegularFiles(directory);
    if (cleanup) {
        EXPECT_EQ(files.size(), 0);
    } else {
        ASSERT_EQ(files.size(), 2);
        EXPECT_EQ(std::set<std::string>({files[0].extension(), files[1].extension()}),
...
public:
    RenderEngineGltfClientGltfTest()
        : X_WG_(Vector3d(4, 5, 6)), label_(render::RenderLabel(7)), temp_dir_(temp_directory()) {
...
protected:
    ...
    GeometryId AddObj() {
        ...
        EXPECT_TRUE(engine_.RegisterVisual(
                id, Mesh(FindResourceOrThrow("drake/geometry/render_gltf_client/test/tri.obj"), scale_), properties_,
                X_WG_));
    ...
    GeometryId AddGltf() {
        const GeometryId id = GeometryId::get_new_id();
        EXPECT_TRUE(engine_.RegisterVisual(
                id, Mesh(FindResourceOrThrow("drake/geometry/render_gltf_client/test/tri_tree.gltf"), scale_),
                properties_, X_WG_));
        EXPECT_TRUE(engine_.has_geometry(id));
        return id;
    }
...
TEST_F(RenderEngineGltfClientGltfTest, RegisteringMeshes) {
    ...
    EXPECT_FALSE(engine_.RegisterVisual(GeometryId::get_new_id(), Mesh("some.stl"), properties_, X_WG_));
    ...
    EXPECT_FALSE(engine_.RegisterVisual(GeometryId::get_new_id(), Convex("some.stl"), properties_, X_WG_));
    ...
    const std::map<GeometryId, Tester::GltfRecord>& gltfs = Tester::gltfs(engine_);
    ...
    for (const int index : {1, 2}) {
        EXPECT_TRUE(CompareMatrices(record.root_nodes.at(index), expected_roots.at(index))) << "Index: " << index;
    }
...
TEST_F(RenderEngineGltfClientGltfTest, PoseComputation) {
    ...
    auto extract_T_FN = [](const Matrix4<double> T_WN, const RigidTransformd& X_WF,
    ...
    for (const auto& [node_index, T_FN_expected] : record.root_nodes) {
        ...
        SCOPED_TRACE(fmt::format("Node {}({})", root["name"].get<std::string>(), node_index));
    ...
    const RigidTransformd X_WG_update(RollPitchYawd{M_PI * 0.5, 0, 3 * M_PI / 2}, Vector3d(-3, 1, 2));
    engine_.UpdatePoses(std::unordered_map<GeometryId, RigidTransformd>{{gltf_id, X_WG_update}});
    for (const auto& [node_index, T_FN_expected] : record.root_nodes) {
        const json& root = record.contents["nodes"][node_index];
        SCOPED_TRACE(fmt::format("Node {}({})", root["name"].get<std::string>(), node_index));
        // Confirm computation of X_WN during UpdatePoses.
        const Matrix4<double> T_WN_update = extract_T_WN(root);
        const Matrix4<double> T_FN_update = extract_T_FN(T_WN_update, X_WG_update, scale_);
...
TEST_F(RenderEngineGltfClientGltfTest, ExportScene) {
    ...
    AddObj();
    const json gltf_obj_only = ExportAndReadJson("only_obj.gltf", ImageType::kColor);
    ...
    auto find_mat_for_node = [](const json& gltf, std::string_view node_name) {
        ...
        const json& mat = gltf["materials"][mesh["primitives"][0]["material"].get<int>()];
        return mat;
    };
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ExportAndReadJson("no_access.gltf", ImageType::kColor),
