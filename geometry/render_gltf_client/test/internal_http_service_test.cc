...
TEST_F(HttpServicePostFormTest, NoSuchFile) {
    DRAKE_EXPECT_THROWS_MESSAGE(PostForm({{"foo", "/no/such/file"}}), ".*missing.*foo.*/no/such/file.*");
}

// A request to upload a multiple missing files reports all of the errors.
TEST_F(HttpServicePostFormTest, NoSuchFileMultiple) {
    DRAKE_EXPECT_THROWS_MESSAGE(PostForm({{"bar", "/no/such/other"}, {"foo", "/no/such/file"}}),
...
TEST_F(HttpServicePostFormTest, MixedFiles) {
    EXPECT_THAT(
            ...
            testing::ThrowsMessage<std::exception>(
                    testing::AllOf(testing::Not(testing::HasSubstr("image")), testing::HasSubstr("/no/such/file"))));
}
