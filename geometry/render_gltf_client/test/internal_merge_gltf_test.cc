#include "geometry/render_gltf_client/internal_merge_gltf.h"
...
struct MergeCase {
    ...
    friend std::ostream& operator<<(std::ostream& out, const MergeCase& merge_case) {
...
public:
     ...
     *concatenation*) with the source array's contents.
     ...
     @param merge         The function that merges the named arrays. This is the
                          device under test. */
    static void BumpIndex(vector<MergeCase>* cases,
                          const string& array,
                          const string& independent,
                          const string& index,
                          MergeFunction merge) {
        DRAKE_DEMAND(cases != nullptr);
        cases->push_back({.description = fmt::format("The {} {} indices get bumped - array in both.", array, index),
                          .target = fmt::format("{{'{}':[10, 20], '{}':[0, 1]}}", array, independent),
                          .source = fmt::format("{{'{0}':[{{'{1}':10}}, {{'{1}':20}}]}}", array, index),
                          .array_name = array,
                          .expected = fmt::format("[10, 20, {{'{0}':12}}, {{'{0}':22}}]", index),
                          .merge = merge});
        cases->push_back({.description = fmt::format("The {} {} indices get bumped - array in target.", array, index),
                          .target = fmt::format("{{'{}':[10, 20], '{}':[0, 1]}}", array, independent),
                          .source = "{}",
                          .array_name = array,
                          .expected = "[10, 20]",
                          .merge = merge});
        cases->push_back({.description = fmt::format("The {} {} indices get bumped - array in source.", array, index),
                          .target = fmt::format("{{'{}':[0, 1]}}", independent),
                          .source = fmt::format("{{'{0}':[{{'{1}':10}}, {{'{1}':20}}]}}", array, index),
                          .array_name = array,
                          .expected = fmt::format("[{{'{0}':12}}, {{'{0}':22}}]", index),
                          .merge = merge});
        cases->push_back({.description = fmt::format("The {} {} indices get bumped - array in neither.", array, index),
                          .target = fmt::format("{{'{}':[0, 1]}}", independent),
                          .source = "{}",
                          .array_name = array,
                          .expected = "null",
    ...
    /* Creates a test case where the elements of the array in question are copied
     ...
     @param merge         The function that merges the named arrays. This is the
                          device under test. */
    static void VerbatimCopy(vector<MergeCase>* cases, const string& array, MergeFunction merge) {
        DRAKE_DEMAND(cases != nullptr);
        cases->push_back(
                {.description = fmt::format("The {} array contents copied verbatim - array in both.", array),
                 .target = fmt::format("{{'{}':[0, 1]}}", array),
                 .source = fmt::format("{{'{}':[{{'one':1}}, {{'two':{{'arabic':2, 'roman':'II'}}}}]}}", array),
                 .array_name = array,
                 .expected = "[0, 1, {'one':1}, {'two':{'arabic':2, 'roman':'II'}}]",
                 .merge = merge});
        cases->push_back({.description = fmt::format("The {} array contents copied verbatim - array in target.", array),
                          .target = fmt::format("{{'{}':[0, 1]}}", array),
                          .source = "{}",
                          .array_name = array,
                          .expected = "[0, 1]",
                          .merge = merge});
        cases->push_back(
                {.description = fmt::format("The {} array contents copied verbatim - array in source.", array),
                 .target = "{}",
                 .source = fmt::format("{{'{}':[{{'one':1}}, {{'two':{{'arabic':2, 'roman':'II'}}}}]}}", array),
                 .array_name = array,
                 ...
                 .merge = merge});
        cases->push_back(
                {.description = fmt::format("The {} array contents copied verbatim - array in neither.", array),
                 .target = "{}",
                 .source = "{}",
                 .array_name = array,
                 .expected = "null",
    ...
    static vector<MergeCase> MergeExtensionsAndExtrasCases() {
        ...
        return vector<MergeCase>{
                ...
                {.description = "glTFs have deep, non-colliding duplications in assets[extras]",
                 .target = "{'asset':{'version':'2.0', 'extras':{'one':{'a':1}}}}",
                 .source = "{'asset':{'version':'2.0', 'extras':{'one':{'a':1}, 'b':2}}}",
                 .array_name = "asset",
                ...
                {.description = "glTFs have deep, non-colliding duplications in "
                 ...
                 .source = "{'asset':{'version':'2.0', 'extensions':{'one':{'a':1}, 'b':2}}}",
                 .array_name = "asset",
                 .expected = "{'generator':'Drake glTF merger', 'version':'2.0', "
    ...
    static vector<MergeCase> MergeMeshesCases() {
        ...
        vector<MergeCase> cases{
                ...
                {.description = "Primitives attributes get bumped.",
                 .target = "{'accessors':[0, 1]}",
                 .source = "{'meshes':[{'name':'A', 'primitives':[{'attributes':{'key1':1}}, "
                ...
                {.description = "Non-index primitive fields propagate.",
                 .target = "{'accessors':[0, 1]}",
                 .source = "{'meshes':[{'primitives':[{'mode':17, 'extras':{'v':2}}]}]}",
                 .array_name = "meshes",
                 .expected = "[{'primitives':[{'mode':17, 'extras':{'v':2}}]}]",
                 .merge = merge},
                {.description = "Targets get removed from primitives.",
                 .target = "{}",
                 .source = "{'meshes':[{'primitives':[{'mode':17, 'targets':{'v':2}}]}]}",
                 .array_name = "meshes",
    ...
    static vector<MergeCase> MergeMaterialsCases() {
        ...
        vector<MergeCase> cases{
                {.description = "Texture indices get bumped on all materials.",
                 .target = "{'textures':[0, 1]}",
                 .source = "{'materials':[{'normalTexture':{'index':2}, "
                 ...
                 .expected = "[{'normalTexture':{'index':4}, 'occlusionTexture':{'index':5}, "
...
INSTANTIATE_TEST_SUITE_P(ExtensionsAndExtras, MergeTest, testing::ValuesIn(MergeTest::MergeExtensionsAndExtrasCases()));
INSTANTIATE_TEST_SUITE_P(ExtensionDeclarations,
                         MergeTest,
                         testing::ValuesIn(MergeTest::MergeExtensionsDeclarationCases()));
INSTANTIATE_TEST_SUITE_P(Scenes, MergeTest, testing::ValuesIn(MergeTest::MergeScenesCases()));
INSTANTIATE_TEST_SUITE_P(Nodes, MergeTest, testing::ValuesIn(MergeTest::MergeNodesCases()));
INSTANTIATE_TEST_SUITE_P(Meshes, MergeTest, testing::ValuesIn(MergeTest::MergeMeshesCases()));
INSTANTIATE_TEST_SUITE_P(Materials, MergeTest, testing::ValuesIn(MergeTest::MergeMaterialsCases()));
INSTANTIATE_TEST_SUITE_P(Cameras, MergeTest, testing::ValuesIn(MergeTest::MergeCamerasCases()));
INSTANTIATE_TEST_SUITE_P(Accessors, MergeTest, testing::ValuesIn(MergeTest::MergeAccessorsCases()));
INSTANTIATE_TEST_SUITE_P(BufferViews, MergeTest, testing::ValuesIn(MergeTest::MergeBufferViewsCases()));
INSTANTIATE_TEST_SUITE_P(Buffers, MergeTest, testing::ValuesIn(MergeTest::MergeBuffersCases()));
INSTANTIATE_TEST_SUITE_P(Textures, MergeTest, testing::ValuesIn(MergeTest::MergeTexturesCases()));
INSTANTIATE_TEST_SUITE_P(Images, MergeTest, testing::ValuesIn(MergeTest::MergeImageCases()));
INSTANTIATE_TEST_SUITE_P(Samplers, MergeTest, testing::ValuesIn(MergeTest::MergeSamplersCases()));
...
struct MergeFailureCase {
    ...
    friend std::ostream& operator<<(std::ostream& out, const MergeFailureCase& merge_case) {
        out << merge_case.description;
        return out;
    }
};
    ...
    static vector<MergeFailureCase> MergeGltfCases() {
        ...
        return vector<MergeFailureCase>{
                ...
                {.description = "Target and source have non-object extensions",
                 .target = asset_prefix + ", 'extensions':1}",
                 .source = asset_prefix + ", 'extensions':2}",
                 .expected_error = ".*glTF.extensions.*has a primitive.*has a primitive.",
                 .merge = MergeGltf},
                ...
                {.description = "Shallow collision between extensions",
                 ...
                 .expected_error = ".*glTF.extensions.b.*defines it as 2.*defines it as 3.",
                 .merge = MergeGltf},
...
INSTANTIATE_TEST_SUITE_P(Gltf, MergeFailureTest, testing::ValuesIn(MergeFailureTest::MergeGltfCases()));
INSTANTIATE_TEST_SUITE_P(Asset, MergeFailureTest, testing::ValuesIn(MergeFailureTest::MergeAssetCases()));
INSTANTIATE_TEST_SUITE_P(Scene, MergeFailureTest, testing::ValuesIn(MergeFailureTest::MergeSceneCases()));
...
TEST_P(MergeFailureTest, Evaluate) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(test_case.merge(&merged_target, std::move(source), source_name, &record),
                                test_case.expected_error);
}

/* Check that a buffer with several relative URIs is converted to data URIs. */
GTEST_TEST(MergeGltf, BufferDataUri) {
    json target = ReadJsonFile(FindResourceOrThrow("drake/geometry/render_gltf_client/test/red_box.gltf"));
    const std::filesystem::path source_path = FindResourceOrThrow("drake/geometry/render/test/meshes/cube3.gltf");
    MergeRecord record("test_target");
    EXPECT_NO_THROW(MergeGltf(&target, ReadJsonFile(source_path), source_path, &record));
...
GTEST_TEST(MergeGltf, Smoke) {
    ...
    json source = ReadJsonFile(FindResourceOrThrow("drake/geometry/render_gltf_client/test/textured_green_box.gltf"));

    MergeRecord record("test_target");
    EXPECT_NO_THROW(MergeGltf(&target, std::move(source), "test_source", &record));
