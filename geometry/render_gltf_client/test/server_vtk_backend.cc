...
DEFINE_string(output_path,
              ...
              "are supported.");
DEFINE_string(image_type, "color,depth,label", "The type of image being rendered.");
...
DEFINE_double(min_depth, -1.0, "The minimum depth range.  Only required when rendering a depth image.");
DEFINE_double(max_depth, -1.0, "The maximum depth range.  Only required when rendering a depth image.");
...
static bool ValidateOutputPath(const char*, const std::string& path) {
    ...
    if (std::filesystem::exists(output_path) || (ext != ".png" && ext != ".tiff")) {
...
int DoMain() {
    ...
    if (FLAGS_image_type == "color") {
        ...
        for (int i = 0; i < actor_collection->GetNumberOfItems(); ++i) {
            ...
            const std::map<std::string, vtkTexture*>& texture_maps = actor->GetProperty()->GetAllTextures();
        ...
        for (int i = 0; i < actor_collection->GetNumberOfItems(); ++i) {
            vtkActor* actor = actor_collection->GetNextActor();
            vtkOpenGLPolyDataMapper* mapper = vtkOpenGLPolyDataMapper::SafeDownCast(actor->GetMapper());
            if (mapper && mapper->GetInput()) {
                ...
                mapper->AddObserver(vtkCommand::UpdateShaderEvent, uniform_setting_callback.Get());
            }
        }
    ...
    if (FLAGS_image_type == "depth") {
        ...
        for (int v = 0; v < FLAGS_height; ++v) {
            for (int u = 0; u < FLAGS_width; ++u) {
                if (rgb_image.at(u, v)[0] == 255u && rgb_image.at(u, v)[1] == 255u && rgb_image.at(u, v)[2] == 255u) {
                    ...
                    float shader_value = rgb_image.at(u, v)[0] + rgb_image.at(u, v)[1] / 255. +
