#include "geometry/render_gltf_client/internal_render_client.h"
...
#include "geometry/render_gltf_client/internal_http_service.h"
#include "geometry/render_gltf_client/test/internal_sample_image_data.h"
...
const auto kTestRgbImagePath = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_rgb_8U.png");
const auto kTestRgbaImagePath = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_rgba_8U.png");
const auto kTestDepthImage32FPath = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_depth_32F.tiff");
const auto kTestDepthImage16UTiffPath =
        FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_depth_16U.tiff");
const auto kTestDepthImage16UPngPath = FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_depth_16U.png");
const auto kTestColoredLabelImagePath =
        FindResourceOrThrow("drake/geometry/render_gltf_client/test/test_colored_label_rgba_8U.png");

class RenderClientTest : public ::testing::Test {
public:
    RenderClientTest()
        : color_camera_{{"proxy_render", {798, 247, M_PI_4}, {0.11, 111.111}, {}}, false},
...
protected:
    ...
    const Params params_{.base_url = "notarealserver:8192", .render_endpoint = "no_render"};
...
TEST_F(RenderClientTest, Constructor) {
    ...
    EXPECT_EQ(default_client.get_params().GetUrl(), "http://127.0.0.1:8000/render");
    ...
    const RenderClient client{
            Params{.base_url = base_url, .render_endpoint = render_endpoint, .verbose = verbose, .cleanup = cleanup}};
...
using DoPostFormCallback = typename std::function<HttpResponse(const DataFieldsMap&, const FileFieldsMap&)>;
...
public:
    explicit ProxyService(const DoPostFormCallback& callback) : do_post_form_callback_{callback} {}
...
TEST_F(RenderClientTest, RenderOnServerDepthRange) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            client.RenderOnServer(depth_camera_.core(), RenderImageType::kDepthDepth32F, fake_scene_path_),
            ".*is_depth_type.*");
    // Providing depth_range for non-depth (color or label) should raise.
    DRAKE_EXPECT_THROWS_MESSAGE(client.RenderOnServer(color_camera_.core(), RenderImageType::kColorRgba8U,
                                                      fake_scene_path_, std::nullopt, depth_camera_.depth_range()),
                                ".*is_depth_type.*");
    DRAKE_EXPECT_THROWS_MESSAGE(client.RenderOnServer(color_camera_.core(), RenderImageType::kLabel16I,
                                                      fake_scene_path_, std::nullopt, depth_camera_.depth_range()),
                                ".*is_depth_type.*");
}
...
TEST_F(RenderClientTest, RenderOnServerFieldsCheck) {
    ...
    DoPostFormCallback callback = [&](const DataFieldsMap& data_fields, const FileFieldsMap& file_fields) {
        switch (image_type) {
        ...
        EXPECT_EQ(data_fields.at("center_x"), std::to_string(intrinsics.center_x()));
        EXPECT_EQ(data_fields.at("center_y"), std::to_string(intrinsics.center_y()));
        // min_depth and max_depth should only be provided for depth renders.
        if (image_type == RenderImageType::kDepthDepth32F) {
            EXPECT_EQ(data_fields.at("min_depth"), std::to_string(depth_range.min_depth()));
            EXPECT_EQ(data_fields.at("max_depth"), std::to_string(depth_range.max_depth()));
        } else {
        ...
        std::set<std::string> data_fields_keys{"scene_sha256", "image_type", "width",   "height", "near",
        ...
        return HttpResponse{.http_code = 500, .service_error_message = "Always fails."};
    };
    client.SetHttpService(std::make_unique<ProxyService>(callback));

    // Check fields for a color render.
    image_type = RenderImageType::kColorRgba8U;
    DRAKE_EXPECT_THROWS_MESSAGE(client.RenderOnServer(color_camera_.core(), image_type, fake_scene_path_),
                                "[\\s\\S]*POST:[\\s\\S]*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            client.RenderOnServer(depth_camera_.core(), image_type, fake_scene_path_, mime_type, depth_range),
            "[\\s\\S]*POST:[\\s\\S]*");
}
...
TEST_F(RenderClientTest, RenderOnServerMessageFromServer) {
    ...
    client.SetHttpService(std::make_unique<ProxyService>(callback));
    DRAKE_EXPECT_THROWS_MESSAGE(
            client.RenderOnServer(color_camera_.core(), RenderImageType::kColorRgba8U, fake_scene_path_),
    ...
    client.SetHttpService(std::make_unique<ProxyService>(callback));
    DRAKE_EXPECT_THROWS_MESSAGE(
            client.RenderOnServer(color_camera_.core(), RenderImageType::kLabel16I, fake_scene_path_),
            "[\\s\\S]*Server Message:[\\s\\S]*None.[\\s\\S]*");
...
TEST_F(RenderClientTest, RenderOnServerNoFileReturn) {
    ...
    DoPostFormCallback callback = [&](const DataFieldsMap&, const FileFieldsMap&) {
...
TEST_F(RenderClientTest, RenderOnServerValidImageReturn) {
    ...
    client.SetHttpService(std::make_unique<ProxyService>(callback));
    const std::string expected_png_path = fs::path(fake_scene_path_).replace_extension(".png");
    const std::string response_png =
            client.RenderOnServer(color_camera_.core(), RenderImageType::kColorRgba8U, fake_scene_path_);
    ...
    client.SetHttpService(std::make_unique<ProxyService>(callback));
    const std::string expected_tiff_path = fs::path(fake_scene_path_).replace_extension(".tiff");
    const std::string response_tiff =
            client.RenderOnServer(depth_camera_.core(), RenderImageType::kDepthDepth32F, fake_scene_path_,
...
TEST_F(RenderClientTest, ComputeSha256Bad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::ComputeSha256("/no/such/file"), ".*cannot open.*/no/such/file.*");
}
...
TEST_F(RenderClientTest, RenameHttpServiceResponseGood) {
    ...
    const std::vector<RenameResult> renames{// All components have extensions.
    ...
    for (const auto& r : renames) {
        ...
        const std::string result =
                RenderClient::RenameHttpServiceResponse(r.response_data_path, r.reference_path, r.extension);
...
TEST_F(RenderClientTest, RenameHttpServiceResponseBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::RenameHttpServiceResponse("/no/such/file", fake_scene_path_, ".png"),
                                ".*[Nn]o such file.*/no/such/file.*");

    // Failure case 2: destination file already exists.
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::RenameHttpServiceResponse(fake_scene_path_, fake_scene_path_, ".gltf"),
...
TEST_F(RenderClientTest, LoadColorImageBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadColorImage("/no/such/file", &ignored), ".*open.*/no/such/file.*");

    // Failure case 2: not a valid image file.
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadColorImage(Touch(scratch_ / "fake.png"), &ignored),
                                ".*fake.*Not a PNG.*");
    ...
    for (const bool selector : {true, false}) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadColorImage(kTestRgbaImagePath, &wrong_size),
                                    ".*expected.*but got.*width=.*height=.*");
    }

    // Failure case 4: wrong number of channels (== 1) instead of 3 or 4.
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadColorImage(kTestDepthImage16UPngPath, &ignored),
...
TEST_F(RenderClientTest, LoadDepthImageBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadDepthImage(kTestRgbImagePath, &ignored),
                                ".*unsupported depth pixel.*");

    // Failure case 2: not a valid image file.
    DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadDepthImage(Touch(scratch_ / "fake.empty"), &ignored),
    ...
    for (const bool selector : {true, false}) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(RenderClient::LoadDepthImage(kTestDepthImage32FPath, &wrong_size),
                                    ".*expected.*but got.*width=.*height=.*");
    }
}
