...
void MaybeOffsetIndexArray(json* j_ptr, std::string_view array_name, int offset) {
    json& j = *j_ptr;
    if (j.contains(array_name)) {
...
void MaybeOffsetNamedIndexInTree(json* j_ptr, std::string_view name, int offset) {
    if (j_ptr->is_object()) {
...
void MergeExtensionNames(json* j1, json&& j2, const std::string& array_name) {
    if (j2.contains(array_name)) {
        ...
        for (auto& extension2 : j2[array_name]) {
            if (std::any_of(extensions1.begin(), extensions1.end(), [&extension2](const auto& value) {
        ...
        if (j1->contains(key)) {
            if ((*j1)[key] != value) {
                ...
                throw std::runtime_error(
                        fmt::format("Error in merging '{}.{}.{}'; two glTF files have different "
...
void MergeBlobs(json* j1,
    ...
    if (j2.contains(blob_name)) {
        ...
        if (blob1.is_object() && blob2.is_object()) {
            MergeTrees(&blob1, std::move(blob2), blob_name, container_type, j2_path, record);
...
// to lists.
void MergeExtrasAndExtensions(
        json* j1, json&& j2, ContainerType container_type, const std::filesystem::path& j2_path, MergeRecord* record) {
...
void MaybeEmbedDataUri(nlohmann::json* item_inout, const std::filesystem::path& base_path) {
    ...
    item["uri"] = fmt::format(
            ...
            common_robotics_utilities::base64_helpers::Encode(std::vector<uint8_t>(content.begin(), content.end())));
}
...
const std::filesystem::path& MergeRecord::FindSourcePath(const json& element) const {
    const auto iter = merged_trees_.find(&element);
    DRAKE_DEMAND(iter != merged_trees_.end());
    return source_paths_.at(iter->second);
}

void MergeRecord::AddElementTree(const json& root, const std::filesystem::path& source_path) {
...
void MergeDefaultScenes(json* j1, json&& j2, const std::filesystem::path& j2_path, MergeRecord* record) {
    ...
    MergeExtrasAndExtensions(&scene_1, std::move(scene_2), ContainerType::kDefaultScene, j2_path, record);
}
...
void MergeBuffers(json* j1, json&& j2, const std::filesystem::path& j2_base_path) {
...
void MergeImages(json* j1, json&& j2, const std::filesystem::path& j2_base_path) {
...
void MergeGltf(json* j1, json&& j2, const std::filesystem::path& j2_path, MergeRecord* record) {
    ...
    MergeExtrasAndExtensions(j1, std::move(j2), ContainerType::kGltf, j2_path, record);
    MergeExtrasAndExtensions(&asset1, std::move(asset2), ContainerType::kAsset, j2_path, record);
