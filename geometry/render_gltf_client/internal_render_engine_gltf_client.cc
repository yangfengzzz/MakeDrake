#include "geometry/render_gltf_client/internal_render_engine_gltf_client.h"
...
void LogFrameStart(ImageType image_type, int64_t scene_id) {
    log()->debug("RenderEngineGltfClient: rendering {} scene id {}.", ImageTypeToString(image_type), scene_id);
}

void LogFrameGltfExportPath(ImageType image_type, const std::string& path) {
    log()->debug("RenderEngineGltfClient: {} scene exported to '{}'.", ImageTypeToString(image_type), path);
}

void LogFrameServerResponsePath(ImageType image_type, const std::string& path) {
    log()->debug("RenderEngineGltfClient: {} server response image saved to '{}'.", ImageTypeToString(image_type),
                 path);
}
...
void CleanupFrame(const std::string& scene_path, const std::string& image_path, bool verbose) {
    fs::remove(scene_path);
    fs::remove(image_path);
    if (verbose) {
        log()->debug("RenderEngineGltfClient: deleted unused files {} and {}.", scene_path, image_path);
    }
}
...
std::string GetSceneFileName(ImageType image_type, int64_t scene_id) {
    return fmt::format("{:0>19}-{}.gltf", scene_id, ImageTypeToString(image_type));
}
        ...
        for (int n : indices) {
            ...
            if (node.contains("matrix")) {
                ...
                if (node.contains("translation")) {
                    ...
                    const Vector3<double> p_GN(t[0].get<double>(), t[1].get<double>(), t[2].get<double>());
                    T_FN.block<3, 1>(0, 3) = p_GN.transpose();
                }
                if (node.contains("rotation")) {
                    const auto& q = node["rotation"];
                    // glTF rotation is (x, y, z, w) quaternion. Eigen is (w, x, y, z).
                    const Quaternion<double> quat_GN(q[3].get<double>(), q[0].get<double>(), q[1].get<double>(),
                                                     q[2].get<double>());
...
void SetRootPoses(nlohmann::json* gltf,
    ...
    const math::RigidTransformd X_GF(math::RotationMatrixd::MakeXRotation(M_PI / 2));
...
RenderEngineGltfClient::RenderEngineGltfClient(const RenderEngineGltfClientParams& parameters)
    : render_client_{std::make_unique<RenderClient>(parameters)} {}

RenderEngineGltfClient::RenderEngineGltfClient(const RenderEngineGltfClient& other)
...
std::unique_ptr<RenderEngine> RenderEngineGltfClient::DoClone() const {
    return std::unique_ptr<RenderEngineGltfClient>(new RenderEngineGltfClient(*this));
}

void RenderEngineGltfClient::DoRenderColorImage(const ColorRenderCamera& camera, ImageRgba8U* color_image_out) const {
    const int64_t color_scene_id = GetNextSceneId();
    if (get_params().verbose) {
        LogFrameStart(ImageType::kColor, color_scene_id);
    }

    const RenderingPipeline& color_pipeline = get_mutable_pipeline(ImageType::kColor);
    // Update the VTK scene before exporting to glTF.
    UpdateWindow(camera.core(), camera.show_window(), color_pipeline, "Color Image");
    PerformVtkUpdate(color_pipeline);

    // Export and render the glTF scene.
    SetGltfCameraPerspective(camera.core(), color_pipeline.renderer->GetActiveCamera());
    const std::string scene_path = fs::path(temp_directory()) / GetSceneFileName(ImageType::kColor, color_scene_id);
    ...
    const std::string image_path = render_client_->RenderOnServer(
            camera.core(), VtkToRenderImageType(ImageType::kColor), scene_path, MimeType());
    if (get_params().verbose) {
...
void RenderEngineGltfClient::DoRenderDepthImage(const DepthRenderCamera& camera, ImageDepth32F* depth_image_out) const {
    ...
    const RenderingPipeline& depth_pipeline = get_mutable_pipeline(ImageType::kDepth);
    // Update the VTK scene before exporting to glTF.
    UpdateWindow(camera, depth_pipeline);
    PerformVtkUpdate(depth_pipeline);

    // Export and render the glTF scene.
    SetGltfCameraPerspective(camera.core(), depth_pipeline.renderer->GetActiveCamera());
    const std::string scene_path = fs::path(temp_directory()) / GetSceneFileName(ImageType::kDepth, depth_scene_id);
    ...
    const std::string image_path = render_client_->RenderOnServer(
            camera.core(), VtkToRenderImageType(ImageType::kDepth), scene_path, MimeType(), camera.depth_range());
    if (get_params().verbose) {
...
void RenderEngineGltfClient::DoRenderLabelImage(const ColorRenderCamera& camera, ImageLabel16I* label_image_out) const {
    ...
    const RenderingPipeline& label_pipeline = get_mutable_pipeline(ImageType::kLabel);
    // Update the VTK scene before exporting to glTF.
    UpdateWindow(camera.core(), camera.show_window(), label_pipeline, "Label Image");
    PerformVtkUpdate(label_pipeline);

    // Export and render the glTF scene.
    SetGltfCameraPerspective(camera.core(), label_pipeline.renderer->GetActiveCamera());
    const std::string scene_path = fs::path(temp_directory()) / GetSceneFileName(ImageType::kLabel, label_scene_id);
    ...
    const std::string image_path = render_client_->RenderOnServer(
            camera.core(), VtkToRenderImageType(ImageType::kLabel), scene_path, MimeType());
    if (get_params().verbose) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            label_image_out->at(x, y)[0] = (r == 255 && g == 255 && b == 255) ? render::RenderLabel::kDontCare
...
void RenderEngineGltfClient::ExportScene(const std::string& export_path, ImageType image_type) const {
    ...
    if (!f) {
        throw std::runtime_error("RenderEngineGltfClient: Error writing exported scene data to disk.");
    }
}

void RenderEngineGltfClient::DoUpdateVisualPose(GeometryId id, const math::RigidTransformd& X_WG) {
    auto iter = gltfs_.find(id);
    if (iter != gltfs_.end()) {
        GltfRecord& gltf = iter->second;
        SetRootPoses(&gltf.contents, gltf.root_nodes, X_WG, gltf.scale, false /* strip */);
        return;
    }
...
void RenderEngineGltfClient::ImplementGeometry(const Convex& convex, void* user_data) {
    ImplementMesh(convex.filename(), convex.scale(), user_data);
}

void RenderEngineGltfClient::ImplementGeometry(const Mesh& mesh, void* user_data) {
    ImplementMesh(mesh.filename(), mesh.scale(), user_data);
}

void RenderEngineGltfClient::ImplementMesh(const std::filesystem::path& mesh_path, double scale, void* user_data) {
...
bool RenderEngineGltfClient::ImplementGltf(const std::filesystem::path& gltf_path,
                                           double scale,
    ...
    gltfs_.insert(
            {data.id,
             {gltf_path, std::move(mesh_data), std::move(root_nodes), scale, GetRenderLabelOrThrow(data.properties)}});
    return true;
}

Eigen::Matrix4d RenderEngineGltfClient::CameraModelViewTransformMatrix(ImageType image_type) const {
...
void RenderEngineGltfClient::SetHttpService(std::unique_ptr<HttpService> service) {
