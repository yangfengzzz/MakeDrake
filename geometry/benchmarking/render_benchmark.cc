...
DEFINE_string(save_image_path, "", "Enables saving rendered images in the given location");
...
public:
    ...
    void ColorImage(::benchmark::State& state, const std::string& name) {
         ...
         */
        for (int i = 0; i < 2; ++i) {
            const ColorRenderCamera color_cam(depth_cameras_[0].core(), FLAGS_show_window);
        ...
        for (auto _ : state) {
            ...
            for (int i = 0; i < camera_count; ++i) {
                const ColorRenderCamera color_cam(depth_cameras_[i].core(), FLAGS_show_window);
                renderer->RenderColorImage(color_cam, &color_image);
            }
        }
    ...
    /* Parse arguments from the benchmark state.
     @return A tuple representing the sphere count, camera count, width, and
             height.  */
    static std::tuple<int, int, int, int> ReadState(const benchmark::State& state) {
        return std::make_tuple(state.range(0), state.range(1), state.range(2), state.range(3));
    }
    ...
    static std::string image_path_name(const std::string& test_name,
        ...
        return save_path.append(
                fmt::format("{}_{}_{}_{}_{}.{}", test_name, sphere_count, camera_count, width, height, format));
    }

    /* Computes a compact array of spheres which will remain in view. */
    void AddSphereArray(int sphere_count, const RenderCameraCore& core, RenderEngine* engine) {
        /* We assume the camera is located at (0, 0, c.z) pointing in the -Wz
         ...
         rectangle at s.z.
          ...
          cost. */
        const double aspect_ratio = core.intrinsics().width() / static_cast<double>(core.intrinsics().height());
        ...
        auto add_sphere = [this, engine, &sphere](const Vector3d& p_WS) {
            ...
            engine->RegisterVisual(geometry_id, sphere, material_, RigidTransformd::Identity(),
                                   true /* needs update */);
    ...
    void SetupScene(
            const int sphere_count, const int camera_count, const int width, const int height, RenderEngine* engine) {
        ...
        RigidTransformd X_WC{RotationMatrixd::MakeFromOrthonormalColumns(Cx_W, Cy_W, Cz_W)};
        engine->UpdateViewpoint(X_WC);

        // Add the cameras.
        for (int i = 0; i < camera_count; ++i) {
            depth_cameras_.emplace_back(
                    RenderCameraCore{"unused" + std::to_string(i), {width, height, kFovY}, {0.01, 100.0}, {}},
