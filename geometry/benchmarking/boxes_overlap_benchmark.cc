...
public:
    ...
    void SetupPosedCase(const benchmark::State& state) {
        const PosedCaseParameters acase{static_cast<bool>(state.range(0)), static_cast<int>(state.range(1)),
                                        static_cast<int>(state.range(2))};
        if (acase.b_axis >= 0) {
            X_AB = CalcEdgeTransform(a, b, acase.a_axis, acase.b_axis, acase.expect_overlap);
        } else {
...
BENCHMARK_REGISTER_F(BoxesOverlapBenchmark, ParallelContainedCase)->Unit(benchmark::kNanosecond);
