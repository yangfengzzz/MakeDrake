...
public:
    void SetupScene() {
        ...
        scene_graph_config_.default_proximity_properties.compliance_type = "compliant";
        ...
        auto frame_id = scene_graph_->RegisterFrame(source_id, GeometryFrame{"frame0"});
        ...
        for (int k = 0; k < num_geoms; ++k) {
            auto geom_id = scene_graph_->RegisterGeometry(
                    ...
                    std::make_unique<GeometryInstance>(math::RigidTransformd(), Box(1.0, 1.0, 1.0),
                                                       fmt::format("box{}", k)));
