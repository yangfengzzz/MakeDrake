...
template <typename T>
TriangleSurfaceMesh<double> ConvertVolumeToSurfaceMeshDouble(const VolumeMesh<T>& volume_mesh,
                                                             std::vector<int>* boundary_vertices) {
    TriangleSurfaceMesh<T> surface = ConvertVolumeToSurfaceMeshWithBoundaryVertices(volume_mesh, boundary_vertices);
...
VolumeMeshFieldLinear<T, T> MakeVolumeMeshPressureField(const VolumeMesh<T>* mesh_M, const T& hydroelastic_modulus) {
    ...
    TriangleSurfaceMesh<double> surface_d = ConvertVolumeToSurfaceMeshDouble(*mesh_M, &boundary_vertices);
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeVolumeMeshPressureField<T>));
