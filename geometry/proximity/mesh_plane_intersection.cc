...
void SliceTetrahedronWithPlane(int tet_index,
    ...
    const std::array<int, 4>& intersected_edges = kMarchingTetsTable[intersection_code];
...
int SliceTetWithPlane(int tet_index,
    ...
    for (int e = 0; e < 4; ++e) {
        ...
        if (iter != cut_edges->end()) {
            ...
            const int new_index = builder_W->AddVertex(X_WM * p_MC, e0 + t * (e1 - e0));
    ...
    const Vector3<T> grad_e_MN_W = X_WM.rotation() * field_M.EvaluateGradient(tet_index).cast<T>();
...
std::unique_ptr<ContactSurface<typename MeshBuilder::ScalarType>> ComputeContactSurface(
    ...
    for (const auto& tet_index : tet_indices) {
        const int num_new_faces = SliceTetWithPlane(tet_index, mesh_field_M, plane_M, X_WM, &builder_W, &cut_edges);
        ...
        const Vector3<T>& grad_eMi_W = X_WM.rotation() * mesh_field_M.EvaluateGradient(tet_index).cast<T>();
    ...
    return std::make_unique<ContactSurface<T>>(mesh_id, plane_id, std::move(mesh_W), std::move(field_W),
...
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
    ...
    Plane<double> plane_S_double{convert_to_double(Rz_S), convert_to_double(p_SRo), true /* already_normalized */};
    ...
    if (representation == HydroelasticContactRepresentation::kTriangle) {
        return ComputeContactSurface<TriMeshBuilder<T>>(id_S, field_S, id_R, plane_S, tet_indices, X_WS);
    } else {
        return ComputeContactSurface<PolyMeshBuilder<T>>(id_S, field_S, id_R, plane_S, tet_indices, X_WS);
    }
}

template int SliceTetWithPlane<TriMeshBuilder<double>>(int,
...
template int SliceTetWithPlane<TriMeshBuilder<AutoDiffXd>>(int,
                                                           const VolumeMeshFieldLinear<double, double>&,
                                                           const Plane<AutoDiffXd>&,
                                                           const math::RigidTransform<AutoDiffXd>&,
                                                           TriMeshBuilder<AutoDiffXd>*,
                                                           std::unordered_map<SortedPair<int>, int>*);
template int SliceTetWithPlane<PolyMeshBuilder<double>>(int,
                                                        const VolumeMeshFieldLinear<double, double>&,
                                                        const Plane<double>&,
                                                        const math::RigidTransform<double>&,
                                                        PolyMeshBuilder<double>*,
                                                        std::unordered_map<SortedPair<int>, int>*);
template int SliceTetWithPlane<PolyMeshBuilder<AutoDiffXd>>(int,
                                                            const VolumeMeshFieldLinear<double, double>&,
                                                            const Plane<AutoDiffXd>&,
                                                            const math::RigidTransform<AutoDiffXd>&,
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        (&ComputeContactSurfaceFromSoftVolumeRigidHalfSpace<T>, &SliceTetrahedronWithPlane<T>));
