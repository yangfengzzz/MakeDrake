...
/* Returns an approximation of the signed distance field inside the given
 ...
 @pre mesh != nullptr. */
std::unique_ptr<VolumeMeshFieldLinear<double, double>> ApproximateSignedDistanceField(const VolumeMesh<double>* mesh) {
    ...
    for (int v = 0; v < num_vertices; ++v) {
        ...
        signed_distance.emplace_back(-CalcDistanceToSurfaceMesh(mesh->vertex(v), surface_mesh, bvh_of_surface));
    }
    return make_unique<VolumeMeshFieldLinear<double, double>>(std::move(signed_distance), mesh);
}
...
DeformableGeometry& DeformableGeometry::operator=(const DeformableGeometry& other) {
    if (this == &other) return *this;

    deformable_mesh_ = std::make_unique<DeformableVolumeMeshWithBvh<double>>(*other.deformable_mesh_);
    // We can't simply copy the field; the copy must contain a pointer to
    // the new mesh. So, we use CloneAndSetMesh() instead.
    signed_distance_field_ = other.signed_distance_field_->CloneAndSetMesh(&deformable_mesh_->mesh());
    return *this;
}

DeformableGeometry::DeformableGeometry(VolumeMesh<double> mesh)
    : deformable_mesh_(std::make_unique<DeformableVolumeMeshWithBvh<double>>(std::move(mesh))),
      signed_distance_field_(ApproximateSignedDistanceField(&deformable_mesh_->mesh())) {}

const VolumeMeshFieldLinear<double, double>& DeformableGeometry::CalcSignedDistanceField() const {
    std::vector<double> values = signed_distance_field_->values();
    *signed_distance_field_ = VolumeMeshFieldLinear<double, double>(std::move(values), &deformable_mesh_->mesh());
