...
Vector3<T> CalcEdgePlaneIntersection(
        int a, int b, const T& s_a, const T& s_b, const std::vector<Vector3<double>>& vertices_F) {
    DRAKE_DEMAND(a != b);
...
int GetVertexAddIfNeeded(
        int a,
        int b,
        const typename MeshBuilder::ScalarType& s_a,
    ...
    auto edge_a_b_intersection_iter = edges_to_newly_created_vertices->find(edge_a_b);
    if (edge_a_b_intersection_iter == edges_to_newly_created_vertices->end()) {
        const Vector3<T> p_FV = CalcEdgePlaneIntersection(a, b, s_a, s_b, vertices_F);
        const int builder_index = builder_W->AddVertex(X_WF * p_FV, pressure_in_F(p_FV));
        bool inserted;
...
int GetVertexAddIfNeeded(const std::vector<Vector3<double>>& vertices_F,
                         int v_index,
    ...
    if (v_to_new_v_iter == vertices_to_newly_created_vertices->end()) {
        ...
        const int builder_index = builder_W->AddVertex(X_WF * p_FV, pressure_in_F(p_FV));
        bool inserted;
        std::tie(v_to_new_v_iter, inserted) = vertices_to_newly_created_vertices->insert({v_index, builder_index});
        DRAKE_DEMAND(inserted);
    }
...
void ConstructTriangleHalfspaceIntersectionPolygon(
    ...
    const Vector3<T> nhat_W = X_WF.rotation() * mesh_F.face_normal(tri_index).cast<T>();

    // Case 1: triangle lies completely within the half space. Preserve
    // the ordering of the triangle vertices.
    if (num_positive == 0) {
        const int v0_new_index = GetVertexAddIfNeeded(vertices_F, triangle.vertex(0), pressure_in_F, X_WF,
                                                      vertices_to_newly_created_vertices, builder_W);
        const int v1_new_index = GetVertexAddIfNeeded(vertices_F, triangle.vertex(1), pressure_in_F, X_WF,
                                                      vertices_to_newly_created_vertices, builder_W);
        const int v2_new_index = GetVertexAddIfNeeded(vertices_F, triangle.vertex(2), pressure_in_F, X_WF,
                                                      vertices_to_newly_created_vertices, builder_W);

        builder_W->AddPolygon({v0_new_index, v1_new_index, v2_new_index}, nhat_W, grad_pressure_in_W);
        return;
    }
    ...
    if (num_positive == 1) {
        for (int i0 = 0; i0 < 3; ++i0) {
            if (s[i0] >= 0) {
                ...
                // i0/i2.
                const int edge_i0_i1_intersection_index =
                        GetVertexAddIfNeeded(v0, v1, s[i0], s[i1], vertices_F, pressure_in_F, X_WF,
                                             edges_to_newly_created_vertices, builder_W);
                const int edge_i0_i2_intersection_index =
                        GetVertexAddIfNeeded(v0, v2, s[i0], s[i2], vertices_F, pressure_in_F, X_WF,
                                             edges_to_newly_created_vertices, builder_W);

                // Get the indices of the new vertices, adding them if needed.
                const int i1_new_index = GetVertexAddIfNeeded(vertices_F, v1, pressure_in_F, X_WF,
                                                              vertices_to_newly_created_vertices, builder_W);
                const int i2_new_index = GetVertexAddIfNeeded(vertices_F, v2, pressure_in_F, X_WF,
                                                              vertices_to_newly_created_vertices, builder_W);
                ...
                builder_W->AddPolygon(
                        {i1_new_index, i2_new_index, edge_i0_i2_intersection_index, edge_i0_i1_intersection_index},
    ...
    if (num_positive == 2) {
        for (int i0 = 0; i0 < 3; ++i0) {
            if (s[i0] <= 0) {
                ...
                const int i0_new_index = GetVertexAddIfNeeded(vertices_F, v0, pressure_in_F, X_WF,
                                                              vertices_to_newly_created_vertices, builder_W);
                ...
                builder_W->AddPolygon({i0_new_index, edge_i0_i1_intersection_index, edge_i0_i2_intersection_index},
                                      nhat_W, grad_pressure_in_W);
                return;
            }
        }
...
std::unique_ptr<ContactSurface<typename MeshBuilder::ScalarType>> ComputeContactSurface(
    ...
    for (const auto& tri_index : tri_indices) {
        ConstructTriangleHalfspaceIntersectionPolygon(input_mesh_F, tri_index, half_space_F, pressure_in_F, grad_p_W,
                                                      X_WF, &builder_W, &vertices_to_newly_created_vertices,
    ...
    auto grad_eS_W = std::make_unique<std::vector<Vector3<T>>>(mesh_W->num_elements(), grad_p_W);
    ...
    return std::make_unique<ContactSurface<T>>(half_space_id, mesh_id, std::move(mesh_W), std::move(field_W),
...
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
    ...
    auto bvh_callback = [&tri_indices, &mesh_R, R_WS = convert_to_double(X_WS).rotation(),
                         R_WR = convert_to_double(X_WR).rotation()](int tri_index) {
    ...
    if (representation == HydroelasticContactRepresentation::kTriangle) {
        return ComputeContactSurface<TriMeshBuilder<T>>(id_R, mesh_R, id_S, hs_R, calc_pressure_R, grad_p_W,
                                                        tri_indices, X_WR);
    } else {
        return ComputeContactSurface<PolyMeshBuilder<T>>(id_R, mesh_R, id_S, hs_R, calc_pressure_R, grad_p_W,
                                                         tri_indices, X_WR);
    }
}
...
template void ConstructTriangleHalfspaceIntersectionPolygon<TriMeshBuilder<double>>(
        const TriangleSurfaceMesh<double>&,
        int,
...
template void ConstructTriangleHalfspaceIntersectionPolygon<TriMeshBuilder<AutoDiffXd>>(
        const TriangleSurfaceMesh<double>&,
        int,
        ...
        std::unordered_map<int, int>*,
        std::unordered_map<SortedPair<int>, int>*);
template void ConstructTriangleHalfspaceIntersectionPolygon<PolyMeshBuilder<double>>(
        const TriangleSurfaceMesh<double>&,
        int,
        const PosedHalfSpace<double>&,
        const std::function<double(const Vector3<double>&)>&,
        ...
        std::unordered_map<int, int>*,
        std::unordered_map<SortedPair<int>, int>*);
template void ConstructTriangleHalfspaceIntersectionPolygon<PolyMeshBuilder<AutoDiffXd>>(
        const TriangleSurfaceMesh<double>&,
        int,
        const PosedHalfSpace<AutoDiffXd>&,
        const std::function<AutoDiffXd(const Vector3<AutoDiffXd>&)>&,
...
template std::unique_ptr<ContactSurface<double>> ComputeContactSurface<TriMeshBuilder<double>>(
        GeometryId,
        const TriangleSurfaceMesh<double>&,
        GeometryId,
        const PosedHalfSpace<double>&,
        ...
        const math::RigidTransform<double>&);
template std::unique_ptr<ContactSurface<AutoDiffXd>> ComputeContactSurface<TriMeshBuilder<AutoDiffXd>>(
        GeometryId,
        const TriangleSurfaceMesh<double>&,
        GeometryId,
        const PosedHalfSpace<AutoDiffXd>&,
        ...
        const std::vector<int>&,
        const math::RigidTransform<AutoDiffXd>&);
template std::unique_ptr<ContactSurface<double>> ComputeContactSurface<PolyMeshBuilder<double>>(
        GeometryId,
        const TriangleSurfaceMesh<double>&,
        GeometryId,
        const PosedHalfSpace<double>&,
        ...
        const std::vector<int>&,
        const math::RigidTransform<double>&);
template std::unique_ptr<ContactSurface<AutoDiffXd>> ComputeContactSurface<PolyMeshBuilder<AutoDiffXd>>(
        GeometryId,
        const TriangleSurfaceMesh<double>&,
        GeometryId,
        const PosedHalfSpace<AutoDiffXd>&,
