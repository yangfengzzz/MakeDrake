...
public:
    ...
    /** Constructs VolumeElement.
     ...
     @pre All indices are non-negative.
     */
    explicit VolumeElement(const int v[4]) : VolumeElement(v[0], v[1], v[2], v[3]) {}
    ...
    /** Checks to see whether the given VolumeElement use the same four indices in
     ...
     same space.
     */
    bool Equal(const VolumeElement& e) const { return this->vertex_ == e.vertex_; }
...
template <typename T>
std::optional<Vector3<T>> VolumeMesh<T>::MaybeCalcGradBarycentric(int e, int i) const {
