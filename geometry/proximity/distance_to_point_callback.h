...
struct CallbackData {
     ...
     */
    CallbackData(fcl::CollisionObjectd* query_in,
...
public:
    ...
    DistanceToPoint(const GeometryId id, const math::RigidTransform<T>& X_WG, const Vector3<T>& p_WQ)
    ...
    static int ExtremalAxis(const Vector<U, dim>& p, const Vector<double, dim>& bounds);
    ...
    template <int dim>
    static std::tuple<Vector<T, dim>, Vector<T, dim>, bool> ComputeDistanceToBox(const Vector<double, dim>& h,
