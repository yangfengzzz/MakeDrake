...
std::unique_ptr<SurfacePolygon> SurfacePolygon::copy_to_unique() const {
    return std::unique_ptr<SurfacePolygon>(new SurfacePolygon(&mesh_face_data_, index_));
}

template <typename T>
PolygonSurfaceMesh<T>::PolygonSurfaceMesh(vector<int> face_data, vector<Vector3<T>> vertices)
    : face_data_(std::move(face_data)), vertices_M_(std::move(vertices)), p_MSc_(Vector3<T>::Zero()) {
...
template <typename T>
std::pair<Vector3<T>, Vector3<T>> PolygonSurfaceMesh<T>::CalcBoundingBox() const {
...
template <class T>
void PolygonSurfaceMesh<T>::SetAllPositions(const Eigen::Ref<const VectorX<T>>& p_MVs) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class PolygonSurfaceMesh);
