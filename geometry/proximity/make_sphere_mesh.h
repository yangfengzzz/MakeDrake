...
std::pair<VolumeMesh<T>, std::vector<bool>> MakeSphereMeshLevel0() {
    ...
    return std::make_pair(VolumeMesh<T>(std::move(tetrahedra), std::move(vertices)), is_boundary);
}
...
void SplitOctohedron(const std::array<int, 6>& vertex_indices,
    ...
    for (int i = 0; i < 6; ++i) {
        for (int j = i + 1; j < 6; ++j) {
            ...
            square_edge_len[SortedPair<int>(a, b)] = ExtractDoubleOrThrow((p_MVs[a] - p_MVs[b]).squaredNorm());
        }
    }
    ...
    auto tet_quality = [&square_edge_len, &volume, &vertex_indices](const std::array<int, 4>& local_i) {
    ...
    auto split_quality = [&tet_quality](const std::array<std::array<int, 4>, 4>& tets_in) {
        double cost = 0;
  ...
  const std::array<std::array<int, 4>, 4> kEjTets{{{e, h, j, i},
    ...
    for (int t = 0; t < 4; ++t) {
        ...
        tets->emplace_back(vertex_indices[local_tets[t][0]], vertex_indices[local_tets[t][1]],
...
template <typename T>
std::vector<VolumeElement> SplitTetrahedron(const VolumeElement& t,
 ...
 */
template <typename T>
std::pair<VolumeMesh<T>, std::vector<bool>> RefineUnitSphereMesh(const VolumeMesh<T>& mesh,
    ...
    const std::vector<std::pair<int, int>> kEdges{{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}};
    for (const auto& t : mesh.tetrahedra()) {
        for (const auto& v_pair : kEdges) {
            const SortedPair<int> key{t.vertex(v_pair.first), t.vertex(v_pair.second)};
            if (!edge_vertex_map.contains(key)) {
                ...
                const bool split_is_boundary = is_boundary[key.first()] && is_boundary[key.second()];
    ...
    for (const auto& t : mesh.tetrahedra()) {
        std::vector<VolumeElement> split_tets = SplitTetrahedron(t, edge_vertex_map, all_vertices);
        all_tets.insert(all_tets.end(), split_tets.begin(), split_tets.end());
    }
    return {VolumeMesh<T>(std::move(all_tets), std::move(all_vertices)), all_boundary};
}
 ...
 */
template <typename T>
std::pair<VolumeMesh<T>, int> RefineUnitSphereMeshOnSurface(const VolumeMesh<T>& mesh, int center_index) {
    // Initialize the set of new vertices with all of the original vertices; we
    // will grow this set by splitting edges.
    std::vector<Vector3<T>> all_vertices(mesh.vertices());
    ...
    */
    ...
    return {VolumeMesh<T>(std::move(all_tets), std::move(all_vertices)), center_index};
}
 ...
 */
template <typename T>
VolumeMesh<T> MakeUnitSphereMesh(int refinement_level, TessellationStrategy strategy) {
    ...
    switch (strategy) {
        case TessellationStrategy::kSingleInteriorVertex: {
            ...
            for (int level = 1; level <= refinement_level; ++level) {
                std::tie(mesh, center_index) = RefineUnitSphereMeshOnSurface<T>(mesh, center_index);
                DRAKE_DEMAND(center_index == 0);
            }
            break;
        }
 ...
 */
template <typename T>
VolumeMesh<T> MakeSphereVolumeMesh(const Sphere& sphere, double resolution_hint, TessellationStrategy strategy) {
    /*
    ...
    const int L = std::max(0, static_cast<int>(std::ceil(std::log2(M_PI / std::asin(edge_length / (2.0 * r)))) - 2));
 ...
 */
template <typename T>
TriangleSurfaceMesh<T> MakeSphereSurfaceMesh(const Sphere& sphere, double resolution_hint) {
    DRAKE_DEMAND(resolution_hint > 0.0);
    return ConvertVolumeToSurfaceMesh<T>(
            MakeSphereVolumeMesh<T>(sphere, resolution_hint, TessellationStrategy::kSingleInteriorVertex));
}
