...
Obb::Obb(const RigidTransformd& X_HB, const Vector3<double>& half_width) : pose_(X_HB), half_width_(half_width) {
...
bool Obb::HasOverlap(const Obb& obb_G, const Aabb& aabb_H, const RigidTransformd& X_GH) {
...
bool Obb::HasOverlap(const Obb& bv, const Plane<double>& plane_P, const math::RigidTransformd& X_PH) {
...
bool Obb::HasOverlap(const Obb& bv, const HalfSpace&, const math::RigidTransformd& X_CH) {
    /*
                                                Hy           Hx
                                                  ╲        ╱
                          By  ╱╲       Bx          ╲      ╱
...
void Obb::PadBoundary() {
    ...
    const double incr = std::max(scale * std::numeric_limits<double>::epsilon(), kTolerance);
...
Obb ObbMaker<MeshType>::OptimizeObbVolume(const Obb& box0) const {
    ...
    for (int i = 0; i < 20; ++i) {
        step -= increment;
        const Obb try_box = CalcOrientedBox(R_MB0 * RotationMatrixd(RollPitchYawd(step * dV_dRPY)));
