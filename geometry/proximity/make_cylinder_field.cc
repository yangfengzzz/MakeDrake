...
VolumeMeshFieldLinear<T, T> MakeCylinderPressureField(const Cylinder& cylinder,
    ...
    for (const Vector3<T>& vertex : mesh_C->vertices()) {
        ...
        point_distance::DistanceToPoint<T> signed_distance_functor(unused_id, identity, r_CV);
        ...
        pressure_values.push_back(min(hydroelastic_modulus * extent, hydroelastic_modulus));
    }
    ...
    // of the cylinder.
    std::vector<int> boundary_vertices = CollectUniqueVertices(IdentifyBoundaryFaces(mesh_C->tetrahedra()));
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeCylinderPressureField<T>));
