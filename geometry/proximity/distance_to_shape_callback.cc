...
template <typename T>
void DistancePairGeometry<T>::operator()(const fcl::Sphered& sphere_A, const fcl::Sphered& sphere_B) {
    SphereShapeDistance(sphere_A, sphere_B);
}

template <typename T>
void DistancePairGeometry<T>::operator()(const fcl::Sphered& sphere_A, const fcl::Boxd& box_B) {
    SphereShapeDistance(sphere_A, box_B);
}

template <typename T>
void DistancePairGeometry<T>::operator()(const fcl::Sphered& sphere_A, const fcl::Cylinderd& cylinder_B) {
    SphereShapeDistance(sphere_A, cylinder_B);
}

template <typename T>
void DistancePairGeometry<T>::operator()(const fcl::Sphered& sphere_A, const fcl::Halfspaced& halfspace_B) {
    SphereShapeDistance(sphere_A, halfspace_B);
}

template <typename T>
void DistancePairGeometry<T>::operator()(const fcl::Sphered& sphere_A, const fcl::Capsuled& capsule_B) {
    SphereShapeDistance(sphere_A, capsule_B);
}

template <typename T>
template <typename FclShape>
void DistancePairGeometry<T>::SphereShapeDistance(const fcl::Sphered& sphere_A, const FclShape& shape_B) {
    const SignedDistanceToPoint<T> shape_B_to_point_Ao =
            point_distance::DistanceToPoint<T>(id_B_, X_WB_, X_WA_.translation())(shape_B);
    result_->id_A = id_A_;
    result_->id_B = id_B_;
...
void CalcDistanceFallback<double>(const fcl::CollisionObjectd& a,
    ...
    if (std::abs(result.min_distance) < kEps) {
        pair_data->nhat_BA_W = CalcGradientWhenTouching(a, X_WA, b, X_WB, pair_data->p_ACa, pair_data->p_BCb);
    } else {
        pair_data->nhat_BA_W = (p_WCa - p_WCb) / result.min_distance;
    }
}

bool RequiresFallback(const fcl::CollisionObjectd& a, const fcl::CollisionObjectd& b) {
    ...
    const fcl::CollisionGeometryd* other = a.collisionGeometry()->getNodeType() == fcl::GEOM_SPHERE
    ...
    return other->getNodeType() == fcl::GEOM_ELLIPSOID || other->getNodeType() == fcl::GEOM_CONVEX;
}
...
void ComputeNarrowPhaseDistance(const fcl::CollisionObjectd& a,
    ...
    const bool a_is_sphere = a.collisionGeometry()->getNodeType() == fcl::GEOM_SPHERE;
    DRAKE_ASSERT(a_is_sphere || b.collisionGeometry()->getNodeType() == fcl::GEOM_SPHERE);
    ...
    switch (o_geometry->getNodeType()) {
        ...
        case fcl::GEOM_HALFSPACE: {
            const auto& halfspace_O = *static_cast<const fcl::Halfspaced*>(o_geometry);
...
bool ScalarSupport<double>::is_supported(fcl::NODE_TYPE node1, fcl::NODE_TYPE node2) {
...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
    ...
    const bool can_collide =
            data.collision_filter == nullptr || data.collision_filter->CanCollideWith(encoding_a.id(), encoding_b.id());

    if (can_collide) {
        // Throw if the geometry-pair isn't supported.
        if (ScalarSupport<T>::is_supported(object_A_ptr->collisionGeometry()->getNodeType(),
            ...
            const fcl::CollisionObjectd& fcl_object_A = *(swap_AB ? object_B_ptr : object_A_ptr);
            const fcl::CollisionObjectd& fcl_object_B = *(swap_AB ? object_A_ptr : object_B_ptr);
            ...
            ComputeNarrowPhaseDistance(fcl_object_A, data.X_WGs.at(id_A), fcl_object_B, data.X_WGs.at(id_B),
            ...
            throw std::logic_error(fmt::format(
                    ...
                    GetGeometryName(*object_A_ptr), GetGeometryName(*object_B_ptr), NiceTypeName::Get<T>()));
        }
    }
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&ComputeNarrowPhaseDistance<T>, &Callback<T>));
