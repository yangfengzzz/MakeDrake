...
std::vector<std::array<int, 3>> IdentifyBoundaryFaces(const std::vector<VolumeElement>& tetrahedra) {
    ...
    for (const VolumeElement& tetrahedron : tetrahedra) {
        for (const auto& face_vertices : tetrahedron_faces) {
            insert_or_erase(tetrahedron.vertex(face_vertices[0]), tetrahedron.vertex(face_vertices[1]),
...
template <class T>
TriangleSurfaceMesh<T> ConvertVolumeToSurfaceMeshWithBoundaryVertices(const VolumeMesh<T>& volume,
                                                                      std::vector<int>* boundary_vertices_out) {
    const std::vector<std::array<int, 3>> boundary_faces = internal::IdentifyBoundaryFaces(volume.tetrahedra());

    std::vector<int> boundary_vertices = internal::CollectUniqueVertices(boundary_faces);
    ...
    for (const auto& face_vertices : boundary_faces) {
        surface_faces.emplace_back(volume_to_surface.at(face_vertices[0]), volume_to_surface.at(face_vertices[1]),
    ...
    return TriangleSurfaceMesh<T>(std::move(surface_faces), std::move(surface_vertices));
}
