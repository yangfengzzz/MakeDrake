    ...
    for (int index : indices) {
        if (index < 0 || index >= v_count) {
            throw std::logic_error(
                    fmt::format("{}: Adding a polygon to a ContactSurface mesh builder requires that "
...
void ThrowIfInvalidForCentroid(const char* prefix,
    ...
    if (n_F.norm() < 1e-10) {
        throw std::runtime_error(fmt::format("{}: given normal is too small; normal [{}] with length {}", prefix,
                                             fmt_eigen(n_F.transpose()), n_F.norm()));
    }
    ...
    for (int i = 0; i < v_count; ++i) {
        ...
        A.block(i, 0, 1, 4) << ExtractDoubleOrThrow(v(0)), ExtractDoubleOrThrow(v(1)), ExtractDoubleOrThrow(v(2)), 1.0;
    }
    ...
    if (lu.dimensionOfKernel() == 0) {
        ...
        throw std::runtime_error(fmt::format("{}: input polygon is not planar", prefix));
    }
...
int TriMeshBuilder<T>::AddPolygon(const std::vector<int>& polygon_vertices,
    ...
    DRAKE_ASSERT_VOID(ThrowForInvalidVertexIndex(polygon_vertices, vertices_B_, __func__));
...
template <typename T>
std::pair<std::unique_ptr<TriangleSurfaceMesh<T>>, std::unique_ptr<TriangleSurfaceMeshFieldLinear<T, T>>>
TriMeshBuilder<T>::MakeMeshAndField() {
    auto mesh = std::make_unique<TriangleSurfaceMesh<T>>(std::move(faces_), std::move(vertices_B_));
    auto* raw = mesh.get();
    return {std::move(mesh), std::make_unique<TriangleSurfaceMeshFieldLinear<T, T>>(std::move(pressures_), raw,
...
template <typename T>
std::pair<std::unique_ptr<PolygonSurfaceMesh<T>>, std::unique_ptr<PolygonSurfaceMeshFieldLinear<T, T>>>
PolyMeshBuilder<T>::MakeMeshAndField() {
    auto mesh = std::make_unique<PolygonSurfaceMesh<T>>(std::move(face_data_), std::move(vertices_B_));
    auto field = std::make_unique<PolygonSurfaceMeshFieldLinear<T, T>>(std::move(pressures_), mesh.get(),
                                                                       std::move(grad_e_MN_B_per_face_));
...
Vector3<T> CalcPolygonCentroid(const std::vector<int>& polygon,
    ...
    DRAKE_ASSERT_VOID(ThrowIfInvalidForCentroid("CalcPolygonCentroid", polygon, n_F, vertices_F));
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&AddPolygonToTriangleMeshData<T>,
