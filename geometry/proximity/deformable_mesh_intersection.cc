...
#include "geometry/deformable_mesh_with_bvh.h"
...
class DeformableSurfaceVolumeIntersector : public SurfaceVolumeIntersector<PolyMeshBuilder<double>, Aabb> {
public:
    /* Returns the indices of tetrahedra containing the contact polygons.
     @pre Call it after SampleVolumeFieldOnSurface() finishes.  */
    std::vector<int>& mutable_tetrahedron_index_of_polygons() { return tetrahedron_index_of_polygons_; }

    /* Returns barycentric coordinates of the centroids of the contact polygons.
     @pre Call it after SampleVolumeFieldOnSurface() finishes.  */
    std::vector<VolumeMesh<double>::Barycentric<double>>& mutable_barycentric_centroids() {
...
protected:
    ...
    void CalcContactPolygon(const VolumeMeshFieldLinear<double, double>& volume_field_D,
        ...
        SurfaceVolumeIntersector<PolyMeshBuilder<double>, Aabb>::CalcContactPolygon(
                volume_field_D, surface_R, X_DR, X_DR_d, builder_D, filter_face_normal_along_field_gradient, tet_index,
                tri_index);
        ...
        barycentric_centroids_.push_back(volume_field_D.mesh().CalcBarycentric(
                CalcPolygonCentroid(polygon, X_DR_d.rotation() * surface_R.face_normal(tri_index),
                                    builder_D->vertices()),
                tet_index));
    }
...
void AddDeformableRigidContactSurface(const VolumeMeshFieldLinear<double, double>& deformable_sdf,
    ...
    intersect.SampleVolumeFieldOnSurface(deformable_sdf, deformable_mesh.bvh(), rigid_mesh_R, rigid_bvh_R, X_DR,
                                         false /* don't filter face normal along field gradient */);

    if (intersect.has_intersection()) {
        std::unique_ptr<PolygonSurfaceMesh<double>> contact_mesh_W = intersect.release_mesh();
        const PolygonSurfaceMeshFieldLinear<double, double>& signed_distance_field = intersect.mutable_field();
        ...
        for (int i = 0; i < num_faces; ++i) {
            const Vector3<double>& contact_points_W = contact_mesh_W->element_centroid(i);
            /* `signed_distance_field` has a gradient, therefore `EvaluateCartesian()`
             should be cheap. */
            penetration_distances[i] = signed_distance_field.EvaluateCartesian(i, contact_points_W);
        }
        ...
        const std::vector<int>& participating_tetrahedra = intersect.mutable_tetrahedron_index_of_polygons();
        DRAKE_DEMAND(static_cast<int>(participating_tetrahedra.size()) == num_faces);
        ...
        deformable_contact->AddDeformableRigidContactSurface(
                deformable_id, rigid_id, participating_vertices, std::move(*contact_mesh_W),
