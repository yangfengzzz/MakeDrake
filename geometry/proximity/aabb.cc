...
bool Aabb::HasOverlap(const Aabb& a_G, const Aabb& b_H, const RigidTransformd& X_GH) {
    ...
    const RigidTransformd X_AB(X_GH.rotation(), X_GH * b_H.center() - a_G.center());
    return BoxesOverlap(a_G.half_width(), b_H.half_width(), X_AB);
}

bool Aabb::HasOverlap(const Aabb& aabb_G, const Obb& obb_H, const math::RigidTransformd& X_GH) {
    ...
    const RigidTransformd X_AO(X_GH.rotation() * obb_H.pose().rotation(),
