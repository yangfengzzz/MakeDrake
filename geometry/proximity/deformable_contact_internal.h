...
public:
    ...
    bool is_rigid(GeometryId id) const {
        return rigid_geometries_.contains(id) || rigid_geometries_pending_.contains(id);
    }

    /* Returns true if a deformable geometry representation with the given `id`
     exists. */
    bool is_deformable(GeometryId id) const { return deformable_geometries_.contains(id); }
    ...
    void MaybeAddRigidGeometry(const Shape& shape,
                               GeometryId id,
    ...
    /* Updates the world pose of the rigid geometry with the given id, if it
     exists, to `X_WG`. */
    void UpdateRigidWorldPose(GeometryId id, const math::RigidTransform<double>& X_WG);
    ...
    void UpdateDeformableVertexPositions(GeometryId id, const Eigen::Ref<const VectorX<double>>& q_WG);
    ...
    */
    DeformableContact<double> ComputeDeformableContact(const CollisionFilter& collision_filter) const;
