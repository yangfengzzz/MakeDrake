...
VolumeMeshFieldLinear<T, T> MakeBoxPressureField(const Box& box,
    ...
    for (const Vector3<T>& r_BV : mesh_B->vertices()) {
        ...
        const auto [r_BN, grad_B, is_V_on_edge_or_vertex] =
                point_distance::DistanceToPoint<T>::ComputeDistanceToBox(half_size, r_BV);
    ...
    return VolumeMeshFieldLinear<T, T>(std::move(pressure_values), mesh_B, MeshGradientMode::kOkOrMarkDegenerate);
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeBoxPressureField<T>));
