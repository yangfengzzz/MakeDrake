 ...
 */
std::vector<VolumeElement> SplitToTetrahedra(int v0, int v1, int v2, int v3, int v4, int v5, int v6, int v7) {
...
VolumeMesh<T> MakeBoxVolumeMeshWithMa(const Box& box) {
    ...
    // m[i][j][k].
    const Vector3d half_central_Ma_before_tolerancing = half_box - Vector3d::Constant(min_half_box);
    const Vector3d half_central_Ma =
            (half_central_Ma_before_tolerancing.array() > DistanceToPointRelativeTolerance(min_half_box))
    ...
    auto append = [&mesh_elements](const std::vector<VolumeElement>& new_elements) {
        mesh_elements.insert(mesh_elements.end(), new_elements.begin(), new_elements.end());
    };
    ...
    // front face (+X)
    append(SplitToTetrahedra(m[1][0][0], m[1][1][0], m[1][1][1], m[1][0][1], v[1][0][0], v[1][1][0], v[1][1][1],
                             v[1][0][1]));
    // back face (-X)
    append(SplitToTetrahedra(m[0][0][0], m[0][0][1], m[0][1][1], m[0][1][0], v[0][0][0], v[0][0][1], v[0][1][1],
                             v[0][1][0]));
    // right face (+Y)
    append(SplitToTetrahedra(m[0][1][0], m[0][1][1], m[1][1][1], m[1][1][0], v[0][1][0], v[0][1][1], v[1][1][1],
                             v[1][1][0]));
    // left face (-Y)
    append(SplitToTetrahedra(m[0][0][0], m[1][0][0], m[1][0][1], m[0][0][1], v[0][0][0], v[1][0][0], v[1][0][1],
                             v[0][0][1]));
    // top face (+Z)
    append(SplitToTetrahedra(m[0][0][1], m[1][0][1], m[1][1][1], m[0][1][1], v[0][0][1], v[1][0][1], v[1][1][1],
                             v[0][1][1]));
    // bottom face (-Z)
    append(SplitToTetrahedra(m[0][0][0], m[0][1][0], m[1][1][0], m[1][0][0], v[0][0][0], v[0][1][0], v[1][1][0],
                             v[1][0][0]));
...
std::vector<Vector3<T>> GenerateVertices(const Box& box, const Vector3<int>& num_vertices) {
    ...
    const auto x_coords = VectorX<T>::LinSpaced(num_vertices.x(), -half_x, half_x);
    const auto y_coords = VectorX<T>::LinSpaced(num_vertices.y(), -half_y, half_y);
    const auto z_coords = VectorX<T>::LinSpaced(num_vertices.z(), -half_z, half_z);
...
void AddSixTetrahedraOfCell(const Vector3<int>& lowest,
    ...
    for (int l = 0; l < 2; ++l)
        for (int m = 0; m < 2; ++m)
            for (int n = 0; n < 2; ++n) v[s++] = CalcSequentialIndex(i + l, j + m, k + n, num_vertices);
    ...
    const int tetrahedron[6][4]{// clang-format off
      {v[0], v[7], v[4], v[6]},
      {v[0], v[7], v[6], v[2]},
      {v[0], v[7], v[2], v[3]},
      {v[0], v[7], v[3], v[1]},
...
VolumeMesh<T> MakeBoxVolumeMesh(const Box& box, double resolution_hint) {
    ...
    const Vector3<int> num_vertices{1 + static_cast<int>(ceil(box.width() / resolution_hint)),
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeBoxVolumeMesh<T>,
