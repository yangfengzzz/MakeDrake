#include "geometry/proximity/detect_zero_simplex.h"
...
std::unordered_set<int> GetBoundaryVertexSet(const VolumeMesh<double>& mesh_M) {
    std::vector<int> boundary_vertex_vector = CollectUniqueVertices(IdentifyBoundaryFaces(mesh_M.tetrahedra()));
    return {boundary_vertex_vector.begin(), boundary_vertex_vector.end()};
}

std::set<SortedTriplet<int>> GetBoundaryFaceSet(const VolumeMesh<double>& mesh_M) {
    const std::vector<std::array<int, 3>> boundary_unordered_faces = IdentifyBoundaryFaces(mesh_M.tetrahedra());
    std::vector<SortedTriplet<int>> boundary_ordered_faces(boundary_unordered_faces.size());
    std::transform(boundary_unordered_faces.begin(), boundary_unordered_faces.end(), boundary_ordered_faces.begin(),
...
std::set<SortedTriplet<int>> GetInteriorFaceSet(const VolumeMesh<double>& mesh_M) {
    const std::set<SortedTriplet<int>> boundary_face_set = GetBoundaryFaceSet(mesh_M);
    // Four triangle faces of a tetrahedron.
    const int tetrahedron_faces[4][3] = {{1, 2, 3}, {3, 2, 0}, {2, 1, 0}, {1, 3, 0}};
    ...
    for (const VolumeElement& tetrahedron : mesh_M.tetrahedra()) {
        for (const auto& face_vertices : tetrahedron_faces) {
            SortedTriplet<int> face(tetrahedron.vertex(face_vertices[0]), tetrahedron.vertex(face_vertices[1]),
...
std::set<SortedPair<int>> GetBoundaryEdgeSet(const VolumeMesh<double>& mesh_M) {
    const std::vector<std::array<int, 3>> boundary_faces = IdentifyBoundaryFaces(mesh_M.tetrahedra());
...
std::set<SortedPair<int>> GetInteriorEdgeSet(const VolumeMesh<double>& mesh_M) {
    const std::set<SortedPair<int>> boundary_edge_set = GetBoundaryEdgeSet(mesh_M);
    std::set<SortedPair<int>> interior_edge_set;
    const int tetrahedron_edges[6][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}};
    for (const VolumeElement& tetrahedron : mesh_M.tetrahedra()) {
        for (const auto& edge_vertices : tetrahedron_edges) {
            SortedPair<int> edge(tetrahedron.vertex(edge_vertices[0]), tetrahedron.vertex(edge_vertices[1]));
...
std::vector<int> DetectTetrahedronWithAllBoundaryVertices(const VolumeMesh<double>& mesh_M) {
    const std::unordered_set<int> boundary_vertex_set = GetBoundaryVertexSet(mesh_M);
    auto is_all_vertices_on_boundary = [&boundary_vertex_set](const VolumeElement& tetrahedron) -> bool {
        for (int i = 0; i < 4; ++i) {
    ...
    for (int tetrahedron_index = 0; tetrahedron_index < mesh_M.num_elements(); ++tetrahedron_index) {
...
std::vector<SortedTriplet<int>> DetectInteriorTriangleWithAllBoundaryVertices(const VolumeMesh<double>& mesh_M) {
    ...
    for (const SortedTriplet<int>& interior_triangle : GetInteriorFaceSet(mesh_M)) {
