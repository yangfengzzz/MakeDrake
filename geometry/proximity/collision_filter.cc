...
void CollisionFilter::Apply(const CollisionFilterDeclaration& declaration,
    ...
    Apply(declaration, extract_ids, is_invariant, &filter_history_[0].filter_state);
}

FilterId CollisionFilter::ApplyTransient(const CollisionFilterDeclaration& declaration,
                                         const CollisionFilter::ExtractIds& extract_ids) {
    ...
    Apply(declaration, extract_ids, is_invariant, &filter_state_);
    filter_history_.emplace_back(InitializeTransientState(filter_state_, kUndefined), FilterId::get_new_id());
    Apply(declaration, extract_ids, is_invariant, &filter_history_.back().filter_state);
...
bool CollisionFilter::RemoveDeclaration(FilterId id) {
    /* We skip the first entry, that is always the persistent base and can't be
     removed. */
    for (auto it = filter_history_.begin() + 1; it != filter_history_.end(); ++it) {
...
void CollisionFilter::AddFiltersBetween(const GeometrySet& set_A,
    ...
    const std::unordered_set<GeometryId>& ids_B = &set_A == &set_B ? ids_A : extract_ids(set_B, scope);
...
void CollisionFilter::RemoveFiltersBetween(const GeometrySet& set_A,
                                           const GeometrySet& set_B,
                                           const CollisionFilter::ExtractIds& extract_ids,
                                           CollisionFilterScope scope,
                                           FilterState* state_out) {
...
void CollisionFilter::AddFilteredPair(GeometryId id_A, GeometryId id_B, bool is_invariant, FilterState* state_out) {
    FilterState& filter_state = *state_out;
    DRAKE_DEMAND(filter_state.contains(id_A) && filter_state.contains(id_B));

    if (id_A == id_B) return;
    PairRelationship& pair_relation = id_A < id_B ? filter_state[id_A][id_B] : filter_state[id_B][id_A];
    if (pair_relation == kInvariantFilter) return;
    pair_relation = is_invariant ? kInvariantFilter : kFiltered;
}

void CollisionFilter::RemoveFilteredPair(GeometryId id_A, GeometryId id_B, FilterState* state_out) {
    FilterState& filter_state = *state_out;
    DRAKE_DEMAND(filter_state.contains(id_A) && filter_state.contains(id_B));
    if (id_A == id_B) return;
    PairRelationship& pair_relation = id_A < id_B ? filter_state[id_A][id_B] : filter_state[id_B][id_A];
...
bool CollisionFilter::operator==(const CollisionFilter& other) const {
    ...
    for (const auto& [this_id, this_map] : filter_state_) {
        ...
        for (const auto& [this_pair_id, can_collide] : this_map) {
            ...
            if (CanCollideWith(this_id, this_pair_id) != other.CanCollideWith(this_id, this_pair_id)) {
                return false;
            }
        }
    }
...
CollisionFilter CollisionFilter::MakeClearCopy() const {
    auto clear_state = CollisionFilter::InitializeTransientState(filter_state_, CollisionFilter::kUnfiltered);
    ...
    for (const auto& statement : declaration.statements()) {
        switch (statement.operation) {
            case Operation::kAllowBetween:
                ...
                RemoveFiltersBetween(statement.set_A, statement.set_B, extract_ids, scope, filter_state);
                break;
            case Operation::kAllowWithin:
                DRAKE_DEMAND(!is_invariant);
                RemoveFiltersBetween(statement.set_A, statement.set_A, extract_ids, scope, filter_state);
                break;
            case Operation::kExcludeWithin:
                AddFiltersBetween(statement.set_A, statement.set_A, extract_ids, scope, is_invariant, filter_state);
                break;
            case Operation::kExcludeBetween:
                AddFiltersBetween(statement.set_A, statement.set_B, extract_ids, scope, is_invariant, filter_state);
                break;
        }
    }
}

void CollisionFilter::AddGeometry(GeometryId new_id, FilterState* filter_state_out, PairRelationship relationship) {
...
void CollisionFilter::RemoveGeometry(GeometryId remove_id, FilterState* filter_state_out) {
    FilterState& filter_state = *filter_state_out;
...
CollisionFilter::FilterState CollisionFilter::InitializeTransientState(const FilterState& reference,
                                                                       PairRelationship default_relationship) {
