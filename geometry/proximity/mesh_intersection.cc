...
#include "geometry/proximity/posed_half_space.h"
...
template <typename T>
Vector3<T> CalcIntersection(const Vector3<T>& p_FA, const Vector3<T>& p_FB, const PosedHalfSpace<T>& H_F) {
...
void ClipPolygonByHalfSpace(const std::vector<Vector3<T>>& input_vertices_F,
    ...
    for (int i = 0; i < size; ++i) {
        ...
        const bool current_contained = H_F.CalcSignedDistance(convert_to_double(current)) <= 0;
        const bool previous_contained = H_F.CalcSignedDistance(convert_to_double(previous)) <= 0;
...
SurfaceVolumeIntersector<MeshBuilder, BvType>::~SurfaceVolumeIntersector() = default;

template <typename MeshBuilder, typename BvType>
const std::vector<Vector3<typename SurfaceVolumeIntersector<MeshBuilder, BvType>::T>>&
SurfaceVolumeIntersector<MeshBuilder, BvType>::ClipTriangleByTetrahedron(int element,
...
template <typename MeshBuilder, typename BvType>
bool SurfaceVolumeIntersector<MeshBuilder, BvType>::IsFaceNormalAlongPressureGradient(
    ...
    return IsFaceNormalInNormalDirection(grad_p_M.normalized(), surface_N, tri_index, X_MN.rotation());
}
...
void SurfaceVolumeIntersector<MeshBuilder, BvType>::SampleVolumeFieldOnSurface(
    ...
    bvh_M.Collide(bvh_N, X_MN_d, [&candidate_tet_tri_pairs](int tet_index, int tri_index) -> BvttCallbackResult {
        candidate_tet_tri_pairs.emplace_back(tet_index, tri_index);
        return BvttCallbackResult::Continue;
    });

    for (const auto& [tet_index, tri_index] : candidate_tet_tri_pairs) {
        CalcContactPolygon(volume_field_M, surface_N, X_MN, X_MN_d, &builder_M, filter_face_normal_along_field_gradient,
                           tet_index, tri_index);
    }
...
void SurfaceVolumeIntersector<MeshBuilder, BvType>::CalcContactPolygon(
    ...
    if (filter_face_normal_along_field_gradient) {
        if (!this->IsFaceNormalAlongPressureGradient(volume_field_M, surface_N, X_MN_d, tet_index, tri_index)) {
            return;
        }
    }
    ...
    const std::vector<Vector3<T>>& polygon_vertices_M =
            this->ClipTriangleByTetrahedron(tet_index, vol_mesh_M, tri_index, surface_N, X_MN);
    ...
    for (const auto& p_MV : polygon_vertices_M) {
        polygon_vertex_indices_.push_back(
                builder_M->AddVertex(p_MV, volume_field_M.EvaluateCartesian(tet_index, p_MV)));
    }

    const Vector3<T> nhat_M = X_MN.rotation() * surface_N.face_normal(tri_index).template cast<T>();
    ...
    const int num_new_faces = builder_M->AddPolygon(polygon_vertex_indices_, nhat_M, grad_e_MN_M);
...
template <typename T>
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromSoftVolumeRigidSurface(
    ...
    auto process_intersection = [&X_WS, id_S, id_R](auto&& intersector_in) -> std::unique_ptr<ContactSurface<T>> {
        if (intersector_in.has_intersection()) {
            ...
            return std::make_unique<ContactSurface<T>>(
                    id_S, id_R, intersector_in.release_mesh(), intersector_in.release_field(),
                    std::make_unique<std::vector<Vector3<T>>>(std::move(grad_eS_S)), nullptr);
        }
        return nullptr;
    };
