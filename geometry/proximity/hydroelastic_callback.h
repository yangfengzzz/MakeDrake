...
template <typename T>
std::unique_ptr<ContactSurface<T>> DispatchRigidSoftCalculation(const SoftGeometry& soft,
                                                                const math::RigidTransform<T>& X_WS,
                                                                GeometryId id_S,
                                                                const RigidGeometry& rigid,
    ...
    if (soft.is_half_space() || rigid.is_half_space()) {
        if (soft.is_half_space()) {
            ...
            return ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(id_S, X_WS, soft.pressure_scale(), id_R, mesh_R,
                                                                   bvh_R, X_WR, representation);
        } else {
            // Soft volume vs rigid half space.
            const VolumeMeshFieldLinear<double, double>& field_S = soft.pressure_field();
            const Bvh<Obb, VolumeMesh<double>>& bvh_S = soft.bvh();
            return ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(id_S, field_S, bvh_S, X_WS, id_R, X_WR,
                                                                     representation);
        }
    } else {
        ...
        return ComputeContactSurfaceFromSoftVolumeRigidSurface(id_S, field_S, bvh_S, X_WS, id_R, mesh_R, bvh_R, X_WR,
                                                               representation);
    }
}
...
std::unique_ptr<ContactSurface<T>> DispatchCompliantCompliantCalculation(
        const SoftGeometry& compliant0_F,
        const math::RigidTransform<T>& X_WF,
        GeometryId id0,
        const SoftGeometry& compliant1_G,
        const math::RigidTransform<T>& X_WG,
        GeometryId id1,
        HydroelasticContactRepresentation representation) {
    DRAKE_DEMAND(!compliant0_F.is_half_space() && !compliant1_G.is_half_space());

    const VolumeMeshFieldLinear<double, double>& field0_F = compliant0_F.pressure_field();
    const Bvh<Obb, VolumeMesh<double>>& bvh0_F = compliant0_F.bvh();
    const VolumeMeshFieldLinear<double, double>& field1_G = compliant1_G.pressure_field();
    const Bvh<Obb, VolumeMesh<double>>& bvh1_G = compliant1_G.bvh();

    return ComputeContactSurfaceFromCompliantVolumes(id0, field0_F, bvh0_F, X_WF, id1, field1_G, bvh1_G, X_WG,
                                                     representation);
}
...
template <typename T>
CalcContactSurfaceResult MaybeCalcContactSurface(fcl::CollisionObjectd* object_A_ptr,
    ...
    if (data->geometries.is_vanished(encoding_a.id()) || data->geometries.is_vanished(encoding_b.id())) {
        return CalcContactSurfaceResult::kCalculated;
    }

    const HydroelasticType type_A = data->geometries.hydroelastic_type(encoding_a.id());
    const HydroelasticType type_B = data->geometries.hydroelastic_type(encoding_b.id());

    // One or two objects have no hydroelastic type.
    if (type_A == HydroelasticType::kUndefined || type_B == HydroelasticType::kUndefined) {
    ...
    if (type_A == HydroelasticType::kRigid && type_B == HydroelasticType::kRigid) {
    ...
    if (type_A == HydroelasticType::kSoft && type_B == HydroelasticType::kSoft) {
        ...
        std::unique_ptr<ContactSurface<T>> surface = DispatchCompliantCompliantCalculation(
                soft0, data->X_WGs.at(id0), id0, soft1, data->X_WGs.at(id1), id1, data->representation);
        if (surface != nullptr) {
    ...
    DRAKE_DEMAND((type_A == HydroelasticType::kRigid && type_B == HydroelasticType::kSoft) ||
                 (type_A == HydroelasticType::kSoft && type_B == HydroelasticType::kRigid));
    ...
    std::unique_ptr<ContactSurface<T>> surface =
            DispatchRigidSoftCalculation(soft, X_WS, id_S, rigid, X_WR, id_R, data->representation);
...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
    ...
    const bool can_collide = data.collision_filter.CanCollideWith(encoding_a.id(), encoding_b.id());

    if (can_collide) {
        CalcContactSurfaceResult result = MaybeCalcContactSurface(object_A_ptr, object_B_ptr, &data);
        ...
        switch (result) {
            case CalcContactSurfaceResult::kUnsupported:
                throw std::logic_error(
                        fmt::format("Requested a contact surface between a pair of geometries without "
            ...
            case CalcContactSurfaceResult::kRigidRigid:
                throw std::logic_error(
                        fmt::format("Requested contact between two rigid objects ({} with id "
                                    ...
                                    GetGeometryName(*object_A_ptr), encoding_a.id(), GetGeometryName(*object_B_ptr),
                                    encoding_b.id()));
            case CalcContactSurfaceResult::kCompliantHalfSpaceCompliantMesh:
                throw std::logic_error(
                        fmt::format("Requested hydroelastic contact between two compliant geometries, "
            ...
            case CalcContactSurfaceResult::kHalfSpaceHalfSpace:
                throw std::logic_error(
                        fmt::format("Requested contact between two half spaces with ids {} and {}; "
                                    "that is not allowed",
...
bool CallbackWithFallback(fcl::CollisionObjectd* object_A_ptr,
    ...
    const bool can_collide = data.data.collision_filter.CanCollideWith(encoding_a.id(), encoding_b.id());

    if (can_collide) {
        CalcContactSurfaceResult result = MaybeCalcContactSurface(object_A_ptr, object_B_ptr, &data.data);

        // Surface calculated; we're done.
        if (result == CalcContactSurfaceResult::kCalculated) return false;

        // Fall back to point pair.
        penetration_as_point_pair::CallbackData<T> point_data{&data.data.collision_filter, &(data.data.X_WGs),
                                                              data.point_pairs};
        penetration_as_point_pair::Callback<T>(object_A_ptr, object_B_ptr, &point_data);
    }
    // Tell the broadphase to keep searching.
    return false;
}
