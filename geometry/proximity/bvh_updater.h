...
public:
    ...
    BvhUpdater(const MeshType* mesh_M, Bvh<Aabb, MeshType>* bvh_M) : mesh_(*mesh_M), bvh_(*bvh_M) {
...
private:
    ...
    static const std::vector<Vector3<double>>& GetMeshVertices(const std::vector<Vector3<double>>& vertices) {
        return vertices;
    }

    // If the mesh type is AutoDiffXd-valued, return double-valued vertices.
    static std::vector<Vector3<double>> GetMeshVertices(const std::vector<Vector3<AutoDiffXd>>& vertices) {
    ...
    void UpdateRecursive(typename Bvh<Aabb, MeshType>::NodeType* node, const std::vector<Vector3<double>>& vertices) {
        ...
        if (node->is_leaf()) {
            ...
            for (int e = 0; e < num_elements; ++e) {
                ...
                for (int i = 0; i < kElementVertexCount; ++i) {
                    const Eigen::Vector3d& p_MV = convert_to_double(vertices[element.vertex(i)]);
