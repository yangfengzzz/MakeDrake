...
#include "geometry/proximity/mesh_traits.h"
...
public:
    ...
    SurfaceTriangle(int v0, int v1, int v2) : vertex_({v0, v1, v2}) { DRAKE_DEMAND(v0 >= 0 && v1 >= 0 && v2 >= 0); }

    /** Constructs SurfaceTriangle.
     @param v  array of three integer indices of the vertices of the triangle in
               TriangleSurfaceMesh.
     @pre index values are non-negative. */
    explicit SurfaceTriangle(const int v[3]) : SurfaceTriangle(v[0], v[1], v[2]) {}
...
public:
    ...
    Vector3<T> element_centroid(int t) const {
        ...
        return (vertices_M_[tri.vertex(0)] + vertices_M_[tri.vertex(1)] + vertices_M_[tri.vertex(2)]) / 3;
    }
    ...
    std::pair<Vector3<T>, Vector3<T>> CalcBoundingBox() const {
        Vector3<T> min_extent = Vector3<T>::Constant(std::numeric_limits<double>::max());
        Vector3<T> max_extent = Vector3<T>::Constant(std::numeric_limits<double>::lowest());
    ...
    Vector3<FieldValue> CalcGradientVectorOfLinearField(const std::array<FieldValue, 3>& field_value, int t) const {
        std::optional<Vector3<FieldValue>> grad = MaybeCalcGradientVectorOfLinearField(field_value, t);
        if (!grad.has_value()) {
...
template <typename T>
std::optional<Vector3<T>> TriangleSurfaceMesh<T>::MaybeCalcGradBarycentric(int t, int i) const {
    ...
    const Vector3<T> H_M = (ab2 <= kEps2) ? p_AV_M : p_AV_M - (p_AV_M.dot(p_AB_M)) * p_AB_M / ab2;
