...
public:
    explicit TestScene(ShapeType A_shape_type, ShapeType B_shape_type)
        : id_A_{GeometryId::get_new_id()},
          ...
          data_{&collision_filter_, &X_WGs_, &hydroelastic_geometries_, HydroelasticContactRepresentation::kTriangle,
                &surfaces_} {
    ...
    void ConfigureScene(const HydroelasticType type_A, const HydroelasticType type_B, bool are_colliding = true) {
        AddGeometry(type_A, type_B);
        PoseGeometry(are_colliding);
    }

    void AddGeometry(const HydroelasticType type_A, const HydroelasticType type_B) {
    ...
    // compliance `type`.
    void MakeHydroelastic(GeometryId id, const HydroelasticType type, const Shape& shape) {
        switch (type) {
            case HydroelasticType::kSoft:
                this->hydroelastic_geometries_.MaybeAddGeometry(shape, id, soft_properties());
                break;
            case HydroelasticType::kRigid:
                this->hydroelastic_geometries_.MaybeAddGeometry(shape, id, rigid_properties());
                break;
    ...
    // fcl representation.
    unique_ptr<CollisionObjectd> MakeShape(GeometryId id,
        ...
        switch (shape_type) {
            ...
            case ShapeType::kTinySphere:
                shape = make_unique<CollisionObjectd>(make_shared<Sphered>(kTinyRadius));
    ...
    void PoseGeometry(bool are_colliding) {
        ...
        if (this->shape_A_type_ == ShapeType::kHalfSpace && this->shape_B_type_ == ShapeType::kHalfSpace) return;
        ...
        X_WGs_[id_A_] = RigidTransform<T>(MakeRotation(rpy_WA), Vector3<T>(0, 0, 0));

        const double dist_WoB = distance_to_shape(this->shape_B_type_);
        const double z_offset = -(dist_WoA + dist_WoB) + (are_colliding ? 0.125 : -0.125);
    ...
    void FilterContact() {
        ...
        collision_filter_.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{data_A.id(), data_B.id()}),
    ...
    const Geometries& hydroelastic_geometries() const { return hydroelastic_geometries_; }
    GeometryId id_A() const { return id_A_; }
    GeometryId id_B() const { return id_B_; }
    const RigidTransform<T>& pose_in_world(GeometryId id) const { return X_WGs_.at(id); }
...
::testing::AssertionResult ValidateDerivatives(const ContactSurface<AutoDiffXd>& surface) {
    if (surface.representation() == HydroelasticContactRepresentation::kTriangle) {
        ...
        if (surface.tri_e_MN().EvaluateAtVertex(0).derivatives().size() != 3) {
            return ::testing::AssertionFailure() << "Pressure field at vertex 0 is missing derivatives";
        }
    } else {
    ...
    if (surface.HasGradE_M()) {
        if (surface.EvaluateGradE_M_W(0).x().derivatives().size() != 3) {
            return ::testing::AssertionFailure() << "Face 0's grad eM is missing derivatives";
        }
    }
    if (surface.HasGradE_N()) {
        if (surface.EvaluateGradE_N_W(0).x().derivatives().size() != 3) {
            return ::testing::AssertionFailure() << "Face 0's grad eN is missing derivatives";
        }
    }
...
TYPED_TEST(DispatchRigidSoftCalculationTests, SoftMeshRigidMesh) {
    ...
    for (const auto representation :
         {HydroelasticContactRepresentation::kTriangle, HydroelasticContactRepresentation::kPolygon}) {
        SCOPED_TRACE(fmt::format("representation = {}", static_cast<int>(representation)));
        {
            ...
            unique_ptr<ContactSurface<T>> surface =
                    DispatchRigidSoftCalculation(geometries.soft_geometry(id_A), X_WA, id_A,
...

    {
        ...
        unique_ptr<ContactSurface<T>> surface = DispatchRigidSoftCalculation(
                geometries.soft_geometry(id_A), X_WA, id_A, geometries.rigid_geometry(id_B), X_WB, id_B,
...
TYPED_TEST(DispatchRigidSoftCalculationTests, SoftHalfSpaceRigidMesh) {
    ...
    for (const auto representation :
        ...
        unique_ptr<ContactSurface<T>> surface =
                DispatchRigidSoftCalculation(geometries.soft_geometry(id_B), X_WB, id_A,
...

    {
        ...
        // we'll simply pick one.
        unique_ptr<ContactSurface<T>> surface = DispatchRigidSoftCalculation(
                geometries.soft_geometry(id_B), X_WB, id_A, geometries.rigid_geometry(id_A), X_WA, id_B,
                HydroelasticContactRepresentation::kTriangle);
        EXPECT_EQ(surface, nullptr);
    }
}
...
TYPED_TEST(DispatchCompliantCompliantCalculationTests, CompliantMeshCompliantMesh) {
    using T = TypeParam;
    ...
    for (const auto representation :
        ...
        SCOPED_TRACE(fmt::format("representation = {}", static_cast<int>(representation)));
        {
            ...
            unique_ptr<ContactSurface<T>> surface =
                    DispatchCompliantCompliantCalculation(geometries.soft_geometry(id_A), X_WA, id_A,
...

    {
        ...
        // When they are not in contact, mesh representation is irrelevant; so,
        // we'll simply pick one.
        unique_ptr<ContactSurface<T>> surface = DispatchCompliantCompliantCalculation(
                geometries.soft_geometry(id_A), X_WA, id_A, geometries.soft_geometry(id_B), X_WB, id_B,
                HydroelasticContactRepresentation::kTriangle);
        EXPECT_EQ(surface, nullptr);
    }
}
...
TYPED_TEST(MaybeCalcContactSurfaceTests, UndefinedGeometry) {
    ...
    CalcContactSurfaceResult result = MaybeCalcContactSurface<T>(&scene.shape_A(), &scene.shape_B(), &scene.data());
    ASSERT_EQ(result, CalcContactSurfaceResult::kUnsupported);
    ASSERT_EQ(scene.surfaces().size(), 0u);

    // Case: first is undefined.
    result = MaybeCalcContactSurface<T>(&scene.shape_B(), &scene.shape_A(), &scene.data());
    ASSERT_EQ(result, CalcContactSurfaceResult::kUnsupported);
    ASSERT_EQ(scene.surfaces().size(), 0u);

    // Case: both are undefined.
    result = MaybeCalcContactSurface<T>(&scene.shape_B(), &scene.shape_B(), &scene.data());
    ASSERT_EQ(result, CalcContactSurfaceResult::kUnsupported);
    ASSERT_EQ(scene.surfaces().size(), 0u);
}
...
TYPED_TEST(MaybeCalcContactSurfaceTests, BothHalfSpace) {
    ...
    for (const HydroelasticType first_type : {HydroelasticType::kRigid, HydroelasticType::kSoft}) {
        for (const HydroelasticType second_type : {HydroelasticType::kRigid, HydroelasticType::kSoft}) {
            SCOPED_TRACE(fmt::format("Use first_type = {}, second_type = {}.", static_cast<int>(first_type),
            ...
            CalcContactSurfaceResult result =
                    MaybeCalcContactSurface<T>(&scene.shape_A(), &scene.shape_B(), &scene.data());

            if (first_type == HydroelasticType::kRigid && second_type == HydroelasticType::kRigid) {
                EXPECT_EQ(result, CalcContactSurfaceResult::kRigidRigid);
                EXPECT_EQ(scene.surfaces().size(), 0u);
            } else {
...
TYPED_TEST(MaybeCalcContactSurfaceTests, NonColliding) {
    ...
    TestScene<T> scene{ShapeType::kSphere, ShapeType::kSphere};
    scene.ConfigureScene(HydroelasticType::kSoft, HydroelasticType::kRigid, false /* are_colliding */);
...
TYPED_TEST(MaybeCalcContactSurfaceTests, HandleSoftMeshRigidMesh) {
    ...
    TestScene<T> scene{ShapeType::kSphere, ShapeType::kSphere};
    scene.ConfigureScene(HydroelasticType::kRigid, HydroelasticType::kSoft, true /* are_colliding */);
...
TYPED_TEST(StrictHydroelasticCallbackTyped, ThrowForMissingHydroelasticRepresentation) {
    using T = TypeParam;
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Callback<T>(&scene.shape_A(), &scene.shape_B(), &scene.data()),
...
TYPED_TEST(StrictHydroelasticCallbackTyped, ThrowForCompliantHalfSpaceAndNonHalfSpace) {
    using T = TypeParam;
...
TYPED_TEST(StrictHydroelasticCallbackTyped, RespectsCollisionFilter) {
    ...
    scene.ConfigureScene(HydroelasticType::kRigid, HydroelasticType::kRigid);
    EXPECT_THROW(Callback<T>(&scene.shape_A(), &scene.shape_B(), &scene.data()), std::logic_error);

    scene.FilterContact();
    DRAKE_EXPECT_NO_THROW(Callback<T>(&scene.shape_A(), &scene.shape_B(), &scene.data()));
    EXPECT_EQ(scene.surfaces().size(), 0u);
}
...
// to guarantee that the body of the callback gets exercised in all cases.
TYPED_TEST(HydroelasticCallbackFallbackTyped, PointPairForMissingHydroelasticRepresentation) {
    using T = TypeParam;
    ...
    DRAKE_EXPECT_NO_THROW(CallbackWithFallback<T>(&scene.shape_A(), &scene.shape_B(), &data));
    EXPECT_EQ(scene.surfaces().size(), 0u);
