...
GTEST_TEST(MakeSphereMesh, VolumeConvergence) {
    ...
    for (auto strategy : {TessellationStrategy::kSingleInteriorVertex, TessellationStrategy::kDenseInteriorVertices}) {
        ...
        const int tet_growth = strategy == TessellationStrategy::kSingleInteriorVertex ? 4 : 8;
...
GTEST_TEST(MakeSphereMesh, NoDuplicateVertices) {
    ...
    for (int i = 0; i < 4; ++i) {
        VolumeMesh<double> mesh = MakeUnitSphereMesh<double>(i, TessellationStrategy::kDenseInteriorVertices);
...
GTEST_TEST(MakeSphereMesh, AutoDiffRefinement) {
    VolumeMesh<AutoDiffXd> mesh1 = MakeUnitSphereMesh<AutoDiffXd>(1, TessellationStrategy::kDenseInteriorVertices);
...
GTEST_TEST(MakeSphereMesh, SplitOctohedron) {
    ...
    const std::vector<Vector3d> tet_vertices{Vector3d(1, -1, -1), Vector3d(1, 1, 1), Vector3d(-1, 1, -1),
                                             Vector3d(-1, -1, 1)};
    ...
    const std::vector<Vector3d> unit_p_MVs{Vector3d(mid_point(a, b)),   // e = (a + b) / 2
    ...
    // and FI with the z.
    const std::vector<SortedPair<int>> split_edges{SortedPair<int>{e, j}, SortedPair<int>{g, h}, SortedPair<int>{f, i}};
    for (int axis = 0; axis < 3; ++axis) {
        Vector3d scale_factor{2, 2, 2};
        scale_factor(axis) = 1;

        std::vector<Vector3d> p_MVs;
        std::transform(unit_p_MVs.begin(), unit_p_MVs.end(), std::back_inserter(p_MVs),
...
GTEST_TEST(MakeSphereVolumeMesh, ConfirmEdgeLength) {
    ...
    Sphere sphere(r);
    auto test_equator = [r_squared](const VolumeMesh<double>& mesh, double edge_length) {
        ...
        std::sort(equator_vertices.begin(), equator_vertices.end(), [](const auto& v1, const auto& v2) {
        ...
        for (int i = 0; i < v_count; ++i) {
            const double chord_length = (equator_vertices[i] - equator_vertices[(i + 1) % v_count]).norm();
    ...
    for (auto strategy : {TessellationStrategy::kSingleInteriorVertex, TessellationStrategy::kDenseInteriorVertices}) {
        ...
        for (const double edge_length : {1.5 * r, r, r / 2, r / 4}) {
            VolumeMesh<double> mesh = MakeSphereVolumeMesh<double>(sphere, edge_length, strategy);
...
GTEST_TEST(MakeSphereVolumeMesh, MassiveEdgeLength) {
    ...
    VolumeMesh<double> mesh =
            MakeSphereVolumeMesh<double>(sphere, edge_length, TessellationStrategy::kDenseInteriorVertices);
...
GTEST_TEST(MakeSphereSurfaceMesh, GenerateSurface) {
    const Sphere sphere(1.5);
    const double edge_length = 3 * sphere.radius();
    TriangleSurfaceMesh<double> surface_mesh = MakeSphereSurfaceMesh<double>(sphere, edge_length);
...
GTEST_TEST(SparseSphereTest, Level0Identical) {
    VolumeMesh<double> dense_0 = MakeUnitSphereMesh<double>(0, TessellationStrategy::kDenseInteriorVertices);
    VolumeMesh<double> sparse_0 = MakeUnitSphereMesh<double>(0, TessellationStrategy::kSingleInteriorVertex);
...
GTEST_TEST(SparseSphereTest, SurfaceMatchesDenseMesh) {
    ...
    for (int level = 1; level < 3; ++level) {
        const VolumeMesh<double> dense_mesh =
                MakeUnitSphereMesh<double>(level, TessellationStrategy::kDenseInteriorVertices);
        const TriangleSurfaceMesh<double> dense_surface = ConvertVolumeToSurfaceMesh<double>(dense_mesh);
        const VolumeMesh<double> sparse_mesh =
                MakeUnitSphereMesh<double>(level, TessellationStrategy::kSingleInteriorVertex);
        TriangleSurfaceMesh<double> sparse_surface = ConvertVolumeToSurfaceMesh<double>(sparse_mesh);
...
GTEST_TEST(SparseSphereTest, RefinesWithSingleInteriorVertex) {
    const TessellationStrategy strategy = TessellationStrategy::kSingleInteriorVertex;
...
GTEST_TEST(SparseSphereTest, SparseSphereOfArbitraryRadius) {
    ...
    VolumeMesh<double> mesh = MakeSphereVolumeMesh<double>(sphere, r, TessellationStrategy::kSingleInteriorVertex);
