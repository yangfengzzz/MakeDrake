...
#include "geometry/proximity/aabb.h"
...
public:
    ObbMakerTester(const MeshType& mesh_M, const std::set<int>& vertices) : obb_maker_(mesh_M, vertices) {}

    RotationMatrixd CalcOrientationByPca() const { return obb_maker_.CalcOrientationByPca(); }

    Obb CalcOrientedBox(const math::RotationMatrixd& R_MB) const { return obb_maker_.CalcOrientedBox(R_MB); }

    // CalcVolumeGradient() is also tested as a part of OptimizeObbVolume().
    Obb OptimizeObbVolume(const Obb& box) const { return obb_maker_.OptimizeObbVolume(box); }
...
GTEST_TEST(ObbMakerTest, VerifyEigenvalues) {
    ...
    // because the points are co-planar.
    {
        ...
        for (const auto& p_MV : {Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()}) {
    ...
    // eigenvalue repeated three times: 2, 2, 2.
    {
        ...
        // types.
        for (const Vector3d& p_MV : std::vector<Vector3d>{Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ(),
    ...
    // eigenvalue: 2, 2, 2²*2.
    {
        ...
        for (const Vector3d& p_MV :
             std::vector<Vector3d>{2. * Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ(),
...
TEST_F(ObbMakerTestRectangularBox, CalcOrientationByPca) {
    ...
    const RotationMatrixd R_MB =
            ObbMakerTester<TriangleSurfaceMesh<double>>(mesh_M_, test_vertices_).CalcOrientationByPca();
    ...
    EXPECT_TRUE(CompareMatrices(Bx_M, Mz_M, kEps) || CompareMatrices(Bx_M, -Mz_M, kEps));
    EXPECT_TRUE(CompareMatrices(By_M, My_M, kEps) || CompareMatrices(By_M, -My_M, kEps));
    EXPECT_TRUE(CompareMatrices(Bz_M, Mx_M, kEps) || CompareMatrices(Bz_M, -Mx_M, kEps));
    ...
    const RotationMatrixd R_FM(RollPitchYawd(2. * M_PI / 3., M_PI / 4., M_PI / 5.));
    ...
    const RotationMatrixd R_FB =
            ObbMakerTester<TriangleSurfaceMesh<double>>(mesh_F, test_vertices_).CalcOrientationByPca();
    ...
    EXPECT_TRUE(CompareMatrices(Bx_F, R_FM * Bx_M, kEps) || CompareMatrices(Bx_F, -(R_FM * Bx_M), kEps));
    EXPECT_TRUE(CompareMatrices(By_F, R_FM * By_M, kEps) || CompareMatrices(By_F, -(R_FM * By_M), kEps));
    EXPECT_TRUE(CompareMatrices(Bz_F, R_FM * Bz_M, kEps) || CompareMatrices(Bz_F, -(R_FM * Bz_M), kEps));
    // Confirm the basis is right-handed.
...
public:
    ObbMakerTestTriangle()
        : ::testing::Test(),
          mesh_M_({{0, 1, 2}}, {Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()}),
...
TEST_F(ObbMakerTestTriangle, CalcOrientationByPca) {
    ...
    // equation AX = λX and rigid transform.
    double kEps = 100. * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(Bz_M, N_M, kEps) || CompareMatrices(Bz_M, -N_M, kEps));
    ...
    EXPECT_FALSE(CompareMatrices(Bx_F, R_FM * Bx_M, kEps) || CompareMatrices(Bx_F, -(R_FM * Bx_M), kEps));
    EXPECT_FALSE(CompareMatrices(By_F, R_FM * By_M, kEps) || CompareMatrices(By_F, -(R_FM * By_M), kEps));
}
...
TEST_F(ObbMakerTestTriangle, CalcOrientedBox) {
  ...
  const RotationMatrixd R_MB =
    ...
    const Obb obb = ObbMakerTester<TriangleSurfaceMesh<double>>(mesh_M_, test_vertices_).CalcOrientedBox(R_MB);
    ...
    EXPECT_TRUE(CompareMatrices(obb.center(), expect_p_MB, std::numeric_limits<double>::epsilon()));
    ...
    const Vector3d expect_half_width =
            Vector3d(std::sqrt(6.) / 4, 1. / std::sqrt(2.), 0.) + Vector3d::Constant(ObbTester::kTolerance);
    EXPECT_TRUE(CompareMatrices(obb.half_width(), expect_half_width, std::numeric_limits<double>::epsilon()));
}

// Returns true iff the oriented bounding box contains all the specified
// vertices in the mesh.
template <typename MeshType>
bool Contain(const Obb& obb_M, const MeshType& mesh_M, const std::set<int>& vertices) {
...
GTEST_TEST(ObbMakerTest, TestOptimizeObbVolume) {
    ...
    const RigidTransformd X_ME(RotationMatrixd(RollPitchYawd(2. * M_PI / 3., M_PI / 4., M_PI / 5.)),
                               Vector3d(-0.2, 2, 1.5));
    // The first line calls it mesh_M even though it's actually mesh_E. The
    // second line correctly makes it mesh_M.
    TriangleSurfaceMesh<double> mesh_M = MakeEllipsoidSurfaceMesh<double>(Ellipsoid(1., 2., 3.), 6);
    ...
    const Obb optimized_obb_M =
            ObbMakerTester<TriangleSurfaceMesh<double>>(mesh_M, test_vertices).OptimizeObbVolume(initial_obb_M);
    EXPECT_TRUE(Contain(optimized_obb_M, mesh_M, test_vertices));
    const double percent_improvement = 100. * (initial_volume - optimized_obb_M.CalcVolume()) / initial_volume;
    ...
    TriangleSurfaceMesh<double> mesh2_M = MakeEllipsoidSurfaceMesh<double>(Ellipsoid(1e-5, 2e-5, 3e-5), 6);
    mesh2_M.TransformVertices(X_ME);
    const Obb initial_obb2_M(X_ME, Vector3d(1e-5, 2e-5, 3e-5));
    const Obb optimized_obb2_M =
            ObbMakerTester<TriangleSurfaceMesh<double>>(mesh2_M, test_vertices).OptimizeObbVolume(initial_obb2_M);
    ...
    EXPECT_TRUE(CompareMatrices(initial_obb2_M.half_width(), optimized_obb2_M.half_width()));
    EXPECT_TRUE(CompareMatrices(initial_obb2_M.pose().GetAsMatrix34(), optimized_obb2_M.pose().GetAsMatrix34()));
}
...
TEST_F(ObbMakerTestOctahedron, CalcOrientationByPca) {
    ...
    EXPECT_FALSE(CompareMatrices(Bx_F, R_FM * Bx_M, kEps) || CompareMatrices(Bx_F, -(R_FM * Bx_M), kEps));
    EXPECT_FALSE(CompareMatrices(By_F, R_FM * By_M, kEps) || CompareMatrices(By_F, -(R_FM * By_M), kEps));
    EXPECT_FALSE(CompareMatrices(Bz_F, R_FM * Bz_M, kEps) || CompareMatrices(Bz_F, -(R_FM * Bz_M), kEps));
}
...
TEST_F(ObbMakerTestOctahedron, ObbMakerCompute) {
    ...
    const RigidTransformd X_MF(RotationMatrixd(RollPitchYawd(M_PI / 6., M_PI / 3., M_PI / 5.)), Vector3d(-0.2, 2, 1.5));
    ...
    // equation AX = λX and rigid transform.
    double kEps = 100. * std::numeric_limits<double>::epsilon();
    EXPECT_FALSE(CompareMatrices(obb_F.pose().GetAsMatrix4(), (X_MF * obb_M.pose()).GetAsMatrix4(), kEps));
}
...
GTEST_TEST(ObbMakerTest, TestTruncatedBox) {
    ...
    const RotationMatrixd R_MB =
            ObbMakerTester<TriangleSurfaceMesh<double>>(surface_mesh, test_vertices).CalcOrientationByPca();
    ...
    EXPECT_TRUE(CompareMatrices(obb.half_width(), Vector3d(3.552, 2.960, 0.977), 0.001));
}
...
GTEST_TEST(ObbMakerTest, TestVolumeMesh) {
    ...
    const VolumeMesh<double> volume_mesh =
            MakeEllipsoidVolumeMesh<double>(Ellipsoid(1., 2., 3.), 6, TessellationStrategy::kSingleInteriorVertex);
    std::set<int> test_vertices;
...
GTEST_TEST(ObbMakerTestAPI, ObbMakerCompute) {
    const TriangleSurfaceMesh<double> mesh(
            ...
            {{0, 1, 2}, {0, 3, 1}},
            {Vector3d::Zero(), Vector3d::UnitX(), 2. * Vector3d::UnitY(), 3. * Vector3d::UnitZ()});
...
TEST_F(ObbTester, TestVolume) {
    ...
    Obb zero_obb(RigidTransformd(RotationMatrixd(RollPitchYawd(M_PI / 6., M_PI / 3., M_PI / 5.)), Vector3d(3, -4, 1.3)),
                 Vector3d(0, 0, 0));
...
TEST_F(ObbTester, TestPadBoundary) {
    ...
    obb = Obb(RigidTransformd(RotationMatrixd(RollPitchYawd(M_PI / 6., M_PI / 3., M_PI / 5.)), Vector3d(-300, 50, 100)),
              Vector3d(1, 2, 0.5));
...
GTEST_TEST(ObbTest, AabbOverlap) {
    ...
    for (double distance : {-0.001, 0.001}) {
        ...
        EXPECT_EQ(Obb::HasOverlap(Obb(X_WA, half_sizeA), Obb(X_WB, half_sizeB), RigidTransformd::Identity()),
                  expect_overlap);
        ...
        const RigidTransformd X_GA{AngleAxisd{M_PI / 7, Vector3d{1, -2, 3}.normalized()}, Vector3d{-0.5, 3, 1.5}};
...
GTEST_TEST(ObbTest, PlaneOverlap) {
    ...
    const Obb obb_H{RigidTransformd{RollPitchYawd(2. * M_PI / 3., M_PI_4, -M_PI / 3.), p_HoBo_H}, Vector3d{1, 2, 3}};
    ...
    auto lowest_corner = [&obb_H](const RotationMatrixd& R_PH) {
        Vector3d p_HoCmin_P = Vector3d::Constant(std::numeric_limits<double>::infinity());
        for (const double x_sign : {-1.0, 1.0}) {
            for (const double y_sign : {-1.0, 1.0}) {
                for (const double z_sign : {-1.0, 1.0}) {
                    ...
                    const Vector3d p_HoC_H = obb_H.pose() * obb_H.half_width().cwiseProduct(signs);
    ...
    std::vector<AngleAxisd> R_PHs{AngleAxisd{0, Vector3d::UnitX()},
    ...
    std::vector<RigidTransformd> X_QPs{
            ...
            RigidTransformd{RotationMatrixd{AngleAxisd{M_PI / 4, Vector3d{1, 2, 3}.normalized()}}, Vector3d{1, 2, 3}},
            RigidTransformd{RotationMatrixd{AngleAxisd{12 * M_PI / 7, Vector3d{-1, -1, 3}.normalized()}},
                            Vector3d{-3, -1, 2}}};
...
GTEST_TEST(ObbTest, HalfSpaceOverlap) {
    ...
    auto lowest_corner = [&obb_H](const RotationMatrixd& R_CH) {
        Vector3d p_HoVmin_C = Vector3d::Constant(std::numeric_limits<double>::infinity());
        for (const double x_sign : {-1.0, 1.0}) {
            for (const double y_sign : {-1.0, 1.0}) {
                for (const double z_sign : {-1.0, 1.0}) {
                    const Vector3d signs{x_sign, y_sign, z_sign};
                    const Vector3d p_HoV_H = obb_H.pose() * obb_H.half_width().cwiseProduct(signs);
    ...
    std::vector<AngleAxisd> R_CHs{AngleAxisd{0, Vector3d::UnitX()},
                                  AngleAxisd{M_PI / 2, Vector3d::UnitX()},
                                  AngleAxisd{M_PI / 2, Vector3d::UnitY()},
                                  AngleAxisd{M_PI / 2, Vector3d::UnitZ()},
                                  AngleAxisd{M_PI / 4, Vector3d::UnitX()},
