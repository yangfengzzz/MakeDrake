    ...
    for (int axis = 0; axis < 3; ++axis) {
        for (const double face_bound : {-half_size[axis], half_size[axis]}) {
            ...
            for (int i = 0; i < mesh.kVertexPerElement && closest_to_this_face; ++i) {
                ...
                closest_to_this_face = tolerance >= distance_to_this_face - distance_to_boundary(vertex);
            }
...
bool VerifyBoxMeshWithMa(const VolumeMesh<double>& mesh, const Box& box) {
    ...
    EXPECT_EQ(1, euler_characteristic) << "The mesh's tetrahedra are not conforming because the mesh's Euler "
                                          "characteristic is "
        ...
        for (const double y : {-half_size.y(), half_size.y()}) {
            for (const double z : {-half_size.z(), half_size.z()}) {
                ...
                const bool corner_is_a_mesh_vertex =
                        mesh.vertices().end() !=
                        find_if(mesh.vertices().begin(), mesh.vertices().end(), [&corner](const Vector3d& v) -> bool {
                            return v == corner;
                        });
                EXPECT_TRUE(corner_is_a_mesh_vertex) << "A corner point of the box is missing from the mesh vertices.";
    ...
    for (int i = 0; i < num_vertices; ++i) {
        const bool vertex_is_inside_or_on_boundary = (mesh.vertex(i).array().abs() <= half_size.array()).all();
        EXPECT_TRUE(vertex_is_inside_or_on_boundary) << "A mesh vertex is outside the box.";
    ...
    const double volume_tolerance(mesh.num_elements() * 8.0 * std::numeric_limits<double>::epsilon());
    EXPECT_NEAR(mesh_volume, box_volume, volume_tolerance) << "The mesh's volume does not equal the box's volume.";
    ...
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        ...
        EXPECT_TRUE(tetrahedron_has_an_interior_vertex) << "A tetrahedron has all its vertices on the box's boundary.";
    ...
    const double distance_tolerance = DistanceToPointRelativeTolerance(half_size.maxCoeff());
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        ...
        EXPECT_TRUE(different_distance_from_v0) << "A tetrahedron has all vertices at the same distances to"
                                                   " the box's boundary.";
    ...
    // Assume the mesh already passes B2 (no mesh's vertex is outside the box).
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        bool tetrahedron_conform_to_MA = IsTetrahedronRespectingMa(tetrahedron, mesh, box, distance_tolerance);
        EXPECT_TRUE(tetrahedron_conform_to_MA) << "A tetrahedron does not conform to the medial axis of the box.";
    ...
    for (int i = 0; i < num_vertices.x(); ++i) {
        for (int j = 0; j < num_vertices.y(); ++j) {
            for (int k = 0; k < num_vertices.z(); ++k) {
                ...
                EXPECT_TRUE(CompareMatrices(expect_r_MV, r_MV)) << "Incorrect vertex position.";
            }
        }
    }
}
...
GTEST_TEST(MakeBoxVolumeMeshTest, AddSixTetrahedraOfCell) {
  ...
  const int expect_elements[6][4]{
    ...
    // clang-format on
    for (int e = 0; e < 6; ++e)
        for (int v = 0; v < 4; ++v) EXPECT_EQ(expect_elements[e][v], elements[e].vertex(v));
}

GTEST_TEST(MakeBoxVolumeMeshTest, GenerateElements) {
    const Vector3<int> num_vertices{3, 5, 7};
    const int expect_total_num_vertex = num_vertices.x() * num_vertices.y() * num_vertices.z();
...
GTEST_TEST(MakeBoxVolumeMeshTest, GenerateMesh) {
    ...
    EXPECT_NEAR(expect_volume, volume, 2.0 * std::numeric_limits<double>::epsilon());
}
...
GTEST_TEST(MakeBoxSurfaceMeshTest, GenerateSurface) {
    ...
    TriangleSurfaceMesh<double> surface_mesh = MakeBoxSurfaceMesh<double>(box, target_edge_length);
