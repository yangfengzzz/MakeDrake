...
GTEST_TEST(PolyToTriMeshTest, BasicSmokeTest) {
  ...
  const PolygonSurfaceMesh<double> poly_cube({
    ...
    const TriangleSurfaceMesh<double> tri_cube = MakeTriangleFromPolygonMesh(poly_cube);
    ...
    for (int p = 0; p < poly_cube.num_elements(); ++p) {
        ...
        for (int ti = 0; ti < 2; ++ti) {
            ...
            EXPECT_TRUE(CompareMatrices(poly_cube.face_normal(p), tri_cube.face_normal(t)));
            ++t;
        }
