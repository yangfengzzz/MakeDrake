...
GTEST_TEST(PosedHalfSpaceTest, Construction) {
    ...
    if (kDrakeAssertIsArmed) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(PosedHalfSpace<double>(nhat_F * 0.99999, p_FP, true),
        ...
        PosedHalfSpace<double> hs_F{0.5 * nhat_F, p_FP, true /* already_normalized */};
...
GTEST_TEST(PosedHalfSpaceTest, CalcSignedDistance) {
    ...
    EXPECT_NEAR(hs_F.CalcSignedDistance(Vector3d{p_FP + 0.5 * nhat_F}), 0.5, kEps);
    EXPECT_NEAR(hs_F.CalcSignedDistance(Vector3d{p_FP - 0.5 * nhat_F}), -0.5, kEps);
}
