...
public:
    void SetUp() override {
        std::string cube_file = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");
        std::string convex_file = FindResourceOrThrow("drake/geometry/test/convex.obj");

        cube_tri_mesh_ = std::make_unique<TriangleSurfaceMesh<double>>(ReadObjToTriangleSurfaceMesh(cube_file));
        convex_tri_mesh_ = std::make_unique<TriangleSurfaceMesh<double>>(ReadObjToTriangleSurfaceMesh(convex_file));

        cube_mesh_ = std::make_unique<VolumeMesh<double>>(MakeConvexVolumeMesh<double>(Convex(cube_file)));
        convex_mesh_ = std::make_unique<VolumeMesh<double>>(MakeConvexVolumeMesh<double>(Convex(convex_file)));
    }
...
void TestSurfaceMeshEquality(const TriangleSurfaceMesh<double>& mesh_A, const TriangleSurfaceMesh<double>& mesh_B) {
    ...
    ASSERT_TRUE(std::find(vertex_mapping.begin(), vertex_mapping.end(), -1) == vertex_mapping.end());
    ...
    auto unique_vertex_it = std::unique(sorted_vertex_mapping.begin(), sorted_vertex_mapping.end());
    int unique_vertex_size = static_cast<int>(unique_vertex_it - sorted_vertex_mapping.begin());
    ...
    auto find_element = [&mesh_A, &mesh_B, &vertex_mapping](int index) {
        ...
        SortedTriplet t_sorted(vertex_mapping[t.vertex(0)], vertex_mapping[t.vertex(1)], vertex_mapping[t.vertex(2)]);
        for (int i = 0; i < mesh_B.num_elements(); ++i) {
            ...
            if (t_sorted == u_sorted && mesh_A.face_normal(index).dot(mesh_B.face_normal(i)) > 0) {
                return i;
            }
        }
        return -1;
    ...
    ASSERT_TRUE(std::find(element_mapping.begin(), element_mapping.end(), -1) == element_mapping.end());

    // If the mapping is equivalent to its unique elements and doesn't contain
    // -1, then the mapping is a bijection.
    std::vector<int> sorted_element_mapping(element_mapping);
    std::sort(element_mapping.begin(), element_mapping.end());
    auto unique_element_it = std::unique(sorted_element_mapping.begin(), sorted_element_mapping.end());
    int unique_element_size = static_cast<int>(unique_element_it - sorted_element_mapping.begin());
...
void TestCoherentVertices(const TriangleSurfaceMesh<double>& tri_mesh, const VolumeMesh<double>& vol_mesh) {
    ...
    for (int i = 0; i < tri_mesh.num_elements(); ++i) {
        const Eigen::Vector3d p = tri_mesh.vertex(tri_mesh.element(i).vertex(0)) - internal_vertex;
...
TEST_F(MakeConvexMeshTest, SurfaceMeshEqualInput) {
    const TriangleSurfaceMesh<double> converted_cube_tri_mesh = ConvertVolumeToSurfaceMesh(*cube_mesh_);
    TestSurfaceMeshEquality(*cube_tri_mesh_, converted_cube_tri_mesh);

    const TriangleSurfaceMesh<double> converted_convex_tri_mesh = ConvertVolumeToSurfaceMesh(*convex_mesh_);
