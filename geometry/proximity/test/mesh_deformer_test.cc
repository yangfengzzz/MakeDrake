...
struct BoxMaker {
    static MeshType<T> make(const Box& box) { throw std::logic_error("Unimplemented call to MakeBox"); }
};

template <typename T>
struct BoxMaker<T, VolumeMesh> {
    static VolumeMesh<T> make(const Box& box) { return MakeBoxVolumeMeshWithMa<T>(box); }
};
...
struct BoxMaker<T, PolygonSurfaceMesh> {
    static TriangleSurfaceMesh<T> make(const Box& box) {
        ...
        TriangleSurfaceMesh<T> tri_mesh = ConvertVolumeToSurfaceMesh(MakeBoxVolumeMeshWithMa<T>(box));
...
public:
    MeshDeformerTest() : ::testing::Test(), mesh_(BoxMaker<T, MeshType>().make(Box(1, 2, 3))) {
    ...
    void TestSetAllPositions() {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(mesh_.SetAllPositions(too_small),
                                    ".*SetAllPositions.+ \\d+ vertices with data for \\d+ DoFs");

        const VectorX<T> too_large(q_size + 1);
        DRAKE_EXPECT_THROWS_MESSAGE(mesh_.SetAllPositions(too_large),
                                    ".*SetAllPositions.+ \\d+ vertices with data for \\d+ DoFs");
    }
    ...
    ::testing::AssertionResult MatchesQ() const {
        const VectorX<T> values =
                Eigen::Map<const VectorX<T>>(reinterpret_cast<const T*>(mesh_.vertices().data()), q_.size());
...
template <typename T>
using TriangleSurfaceMeshDeformerTest = MeshDeformerTest<T, TriangleSurfaceMesh>;
...
public:
    MeshDeformerDataTest() : ::testing::Test(), mesh_(MakeSingleTriangle<MeshType<T>>()) {
