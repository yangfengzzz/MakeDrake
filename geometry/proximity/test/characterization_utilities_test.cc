...
void ValidateBoxSample(const Vector3d& half_size, const ShapeTangentPlane<double>& tangent_plane) {
...
GTEST_TEST(SampleShapeSurfaceTest, Box) {
    ...
    EXPECT_THROW(ShapeConfigurations<double>(box, -half_size.minCoeff() * 1.01), std::exception);
    ...
    for (const double distance_factor : {-0.1, 0.1}) {
        ...
        for (const auto& tangent_plane : ShapeConfigurations<double>(box, distance).configs()) {
...
GTEST_TEST(SampleShapeSurfaceTest, Capsule) {
    ...
    /* We validate for a requested penetration that is too deep.  */
    EXPECT_THROW(ShapeConfigurations<double>(capsule, -capsule.radius() * 1.01), std::exception);
    /* Otherwise, any distance value larger is fine. */
    for (const auto& tangent_plane : ShapeConfigurations<double>(capsule, 0.0).configs()) {
        const Vector3d p_GP = tangent_plane.point;
        const Vector3d n_G = tangent_plane.normal;
...
GTEST_TEST(SampleShapeSurfaceTest, Convex) {
    ...
    const Vector3d half_size = box.size() / 2;
    /* We validate for a requested penetration that is too deep.  */
    EXPECT_THROW(ShapeConfigurations<double>(convex, -half_size.minCoeff() * 1.01), std::exception);
    ...
    for (const double distance_factor : {-0.1, 0.1}) {
        const double distance = min_axis * distance_factor;
        for (const auto& tangent_plane : ShapeConfigurations<double>(convex, distance).configs()) {
...
GTEST_TEST(SampleShapeSurfaceTest, Cylinder) {
    ...
    /* We validate for a requested penetration that is too deep.  */
    EXPECT_THROW(ShapeConfigurations<double>(cylinder, -cylinder.radius() * 1.01), std::exception);
    EXPECT_THROW(ShapeConfigurations<double>(cylinder, -cylinder.length() * 1.01), std::exception);

    /* Configuration can depend on distance. */
    for (const double distance : {-0.1, 0.1}) {
        for (const auto& tangent_plane : ShapeConfigurations<double>(cylinder, distance).configs()) {
            const Vector3d p_GP = tangent_plane.point;
            const Vector3d n_G = tangent_plane.normal;
            ...
            Vector3d p_GE =
                    cylinder.radius() * (std::abs(n_G.z()) < 1 - kEps ? n_xy_G.normalized() : Vector3d{1, 0, 0});
            ...
            if (p_GP.z() <= half_length + kEps && p_GP.z() >= -half_length - kEps) {
                if (p_GP.z() < half_length - kEps && p_GP.z() > -half_length + kEps) {
                    ...
                    EXPECT_NEAR(Vector3d(p_GP.x(), p_GP.y(), 0).norm(), cylinder.radius(), kEps);
                } else {
                    /* It lies on one of the caps and must lie within the cap's disk. */
                    EXPECT_LE(Vector3d(p_GP.x(), p_GP.y(), 0).norm(), cylinder.radius() + kEps);
                }
            } else {
...
GTEST_TEST(SampleShapeSurfaceTest, Ellipsoid) {
    ...
    /* We validate for a requested penetration that is too deep. And we're
     ...
     minimum coefficient. */
    EXPECT_THROW(ShapeConfigurations<double>(ellipsoid, -ellipsoid.c() * 1.01), std::exception);
    ...
    /* Otherwise, any distance value larger is fine. */
    for (const auto& tangent_plane : ShapeConfigurations<double>(ellipsoid, 0.0).configs()) {
        const Vector3d p_GP = tangent_plane.point;
        const Vector3d n_G = tangent_plane.normal;
        ...
        ASSERT_NEAR(p_GE.cwiseProduct(p_GE).cwiseQuotient(axes_squared).sum(), 1.0, kEps);
        /* Now confirm the extreme point lies "under" the plane. */
        ASSERT_LE((p_GE - p_GP).dot(n_G), kEps);

        /* Validate P lies on the ellipsoid: x²/a² + y²/b² + z²/c² = 1. */
        ASSERT_NEAR(p_GP.cwiseProduct(p_GP).cwiseQuotient(axes_squared).sum(), 1.0, kEps);
    }
}
...
GTEST_TEST(SampleShapeSurfaceTest, HalfSpace) {
    ...
    for (const auto& tangent_plane : ShapeConfigurations<double>(HalfSpace(), 0).configs()) {
        const Vector3d p_GP = tangent_plane.point;
        const Vector3d n_G = tangent_plane.normal;
...
GTEST_TEST(SampleShapeSurfaceTest, Sphere) {
    ...
    /* We validate for a requested penetration that is too deep.  */
    EXPECT_THROW(ShapeConfigurations<double>(sphere, -sphere.radius() * 1.01), std::exception);
    /* Otherwise, any distance value larger is fine. */
    for (const auto& tangent_plane : ShapeConfigurations<double>(sphere, 0.0).configs()) {
        const Vector3d p_GP = tangent_plane.point;
        const Vector3d n_G = tangent_plane.normal;
...
GTEST_TEST(AlignPlanes, Correctness) {
    ...
    /* We'll create a sequence of tangent planes and confirm that they get
     properly aligned. */
    vector<pair<Vector3d, Vector3d>> test_planes{{Vector3d{0, 0, 1}, Vector3d{0, -1, 0}},
...
public:
    explicit DummyCallbackData(vector<T>* results) : results_(results) { DRAKE_DEMAND(results != nullptr); }

    static void SetValueSequence(int invocation_count, double truth, double precision) {
...
public:
    RunCharacterizationCustomTest() : CharacterizeResultTest<double>(std::make_unique<DummyImplementation<double>>()) {}

    vector<double> TestDistances() const final { return {kTruth}; }

    vector<Configuration<double>> MakeConfigurations(const Shape&, const Shape&, const vector<double>&) const final {
...
TEST_F(RunCharacterizationCustomTest, CustomConfigurations) {
    ...
    for (const double target_precision : {1e-2, 1e-7, 1e-14}) {
        DummyCallbackData<double>::SetValueSequence(call_count, this->kTruth, target_precision);
...
public:
    RunCharacterizationTest() : CharacterizeResultTest<double>(std::make_unique<DummyImplementation<double>>()) {}

    vector<double> TestDistances() const final { return {this->sphere().radius() * 0.25}; }
};
...
TEST_F(RunCharacterizationTest, DefaultConfigurations) {
    ...
    const int sphere_config_count = static_cast<int>(ShapeConfigurations<double>(sphere, 0).configs().size());
    const int pose_count = static_cast<int>(X_WAs().size());
    const int call_count = 2 * sphere_config_count * sphere_config_count * pose_count;
