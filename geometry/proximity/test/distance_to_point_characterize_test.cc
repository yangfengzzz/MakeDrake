...
public:
    bool Invoke(fcl::CollisionObjectd* obj_A,
        ...
        CallbackData<T> data(obj_A, std::numeric_limits<double>::infinity(), p_WQ, X_WGs, &results_);
...
public:
    CharacterizePointDistanceResultTest() : CharacterizeResultTest<T>(std::make_unique<PointDistanceCallback<T>>()) {}

    vector<double> TestDistances() const final { return {-this->kDistance, this->kDistance}; }
    ...
    vector<Configuration<T>> MakeConfigurations(const Shape& shape_A,
                                                const Shape& shape_B,
        ...
        for (const double signed_distance : signed_distances) {
            ...
            for (const auto& sample_B : ShapeConfigurations<T>(shape_B, signed_distance).configs()) {
