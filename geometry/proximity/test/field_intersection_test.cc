#include "geometry/proximity/field_intersection.h"
...
public:
    FieldIntersectionLowLevelTest()
        : mesh0_M_(std::vector<VolumeElement>{{0, 1, 2, 3}},
                   std::vector<Vector3d>{{2.0, 0.0, 2.0}, {-2.0, 0.0, 2.0}, {0.0, 2.0, -2.0}, {0.0, -2.0, -2.0}}),
          field0_M_({8e4, 4e4, 4e4, 0}, &mesh0_M_),  // (4 + x + y + z)*10⁴
          mesh1_N_(std::vector<VolumeElement>{{0, 1, 2, 3}},
                   std::vector<Vector3d>{{1.5, 1.5, 2.5}, {-1.5, -1.5, 2.5}, {-1.5, 1.5, -2.5}, {1.5, -1.5, -2.5}}),
...
protected:
    ...
    // in frame G.
    static VolumeMesh<double> TransformVolumeMesh(const RigidTransformd& X_FG, const VolumeMesh<double>& mesh_G) {
...
TEST_F(FieldIntersectionLowLevelTest, CalcEquilibriumPlaneIdenticalFrames) {
    ...
    const bool success = CalcEquilibriumPlane(0, field0_M_, 0, field1_N_, X_MN, &plane_M);

    ASSERT_TRUE(success);
    EXPECT_TRUE(CompareMatrices(plane_M.normal(), expected_plane_M.normal(), tolerance()));
    ...
    EXPECT_NEAR(plane_M.CalcHeight<double>(p_MQ), expected_height, tolerance(expected_height));
}
...
TEST_F(FieldIntersectionLowLevelTest, CalcEquilibriumPlaneComplexTransform) {
    const auto X_FG = RigidTransformd(RollPitchYawd(M_PI_4, M_PI / 3, M_PI_2), Vector3d(-1., -2., -3.));
    const auto X_FM = RigidTransformd(RollPitchYawd(M_PI, M_PI_2, M_PI / 3), Vector3d(2, -6, 4));
    ...
    Plane<double> expected_plane_F(X_FM.rotation() * expected_normal_M, X_FM * expected_p_M);
    ...
    const VolumeMeshFieldLinear<double, double> field0_F(std::vector<double>(field0_M_.values()), &mesh0_F);

    const VolumeMesh<double> mesh1_G = TransformVolumeMesh(X_GN, mesh1_N_);
    const VolumeMeshFieldLinear<double, double> field1_G(std::vector<double>(field1_N_.values()), &mesh1_G);
    ...
    ASSERT_TRUE(success);
    EXPECT_TRUE(CompareMatrices(plane_F.normal(), expected_plane_F.normal(), tolerance()));
    // The choice of this query point is arbitrary.
    const Vector3d p_FQ(0.1, 0.2, 0.3);
    const double expected_height = expected_plane_F.CalcHeight<double>(p_FQ);
    EXPECT_NEAR(plane_F.CalcHeight<double>(p_FQ), expected_height, tolerance(expected_height));
}
...
TEST_F(FieldIntersectionLowLevelTest, CalcEquilibriumPlaneNone) {
    ...
    // equilibrium plane.)
    {
        ...
        const bool success = CalcEquilibriumPlane(0, field0_M_, 0, field0_M_, RigidTransformd::Identity(), &plane_M);
    ...
    // that are nowhere equal.
    {
        ...
        Plane<double> plane_M(init_plane_M);
        const bool success = CalcEquilibriumPlane(0, field0_M_, 0, field2_L, X_ML, &plane_M);
...
TEST_F(FieldIntersectionLowLevelTest, IntersectTetrahedra) {
    ...
    bool success = CalcEquilibriumPlane(first_element_in_field0, field0_M_, first_element_in_field1, field1_N_,
                                        identity_X_MN, &plane_M);
    ASSERT_TRUE(success);

    const std::vector<Vector3d> polygon_M =
            IntersectTetrahedra(first_element_in_field0, field0_M_.mesh(), first_element_in_field1, field1_N_.mesh(),
                                identity_X_MN, plane_M);
...
TEST_F(FieldIntersectionLowLevelTest, IntersectTetrahedra_NoIntersection) {
    ...
    const std::vector<Vector3d> polygon_M =
            IntersectTetrahedra(first_element_in_mesh0, field0_M_.mesh(), first_element_in_mesh1, field1_N_.mesh(),
                                RigidTransformd::Identity(), plane_M);
...
TEST_F(FieldIntersectionLowLevelTest, IsPlaneNormalAlongPressureGradient) {
    ...
    EXPECT_TRUE(IsPlaneNormalAlongPressureGradient(nhat_M_along_gradient, first_tetrahedron_in_field0, field0_M_));

    const Vector3d nhat_M_against_gradient = -nhat_M_along_gradient;
    EXPECT_FALSE(IsPlaneNormalAlongPressureGradient(nhat_M_against_gradient, first_tetrahedron_in_field0, field0_M_));
}
...
public:
    VolumeIntersectorTest()
        : box_mesh0_M_(MakeBoxVolumeMeshWithMa<double>(box_)),
          box_field0_M_(MakeBoxPressureField<double>(box_, &box_mesh0_M_, kBoxElasitcModulus_)),
          ...
          octahedron_mesh1_N_(MakeSphereVolumeMesh<double>(
                  sphere_, 10 * sphere_.radius(), TessellationStrategy::kSingleInteriorVertex)),
          octahedron_field1_N_(
                  MakeSpherePressureField<double>(sphere_, &octahedron_mesh1_N_, kOctahedronElasticModulus_)),
          octahedron_bvh1_N_(octahedron_mesh1_N_) {}

protected:
    void SetUp() override { DRAKE_DEMAND(octahedron_mesh1_N_.num_elements() == 8); }
...
TEST_F(VolumeIntersectorTest, IntersectFields) {
    const RigidTransformd X_MN = RigidTransformd(0.03 * Vector3d::UnitX());
    {
        ...
        VolumeIntersector<TriMeshBuilder<double>, Obb>().IntersectFields(
                box_field0_M_, box_bvh0_M_, octahedron_field1_N_, octahedron_bvh1_N_, X_MN, &surface_01_M, &e_MN_M);
    ...
    }
    {
        ...
        EXPECT_NEAR(e_MN_M->EvaluateAtVertex(0), kExpectPressure, kPressureTolerance);
    }
}
...
TEST_F(VolumeIntersectorTest, IntersectFieldsAutoDiffXd) {
    const math::RigidTransform<AutoDiffXd> X_MN(0.03 * Vector3<AutoDiffXd>::UnitX());
    {
        SCOPED_TRACE("Use TriMeshBuilder.");
        std::unique_ptr<TriangleSurfaceMesh<AutoDiffXd>> surface_01_M;
        std::unique_ptr<TriangleSurfaceMeshFieldLinear<AutoDiffXd, AutoDiffXd>> e_MN_M;
    ...
    }
    {
        ...
        std::unique_ptr<PolygonSurfaceMeshFieldLinear<AutoDiffXd, AutoDiffXd>> e_MN_M;
...
TEST_F(VolumeIntersectorTest, IntersectFieldsNoIntersection) {
    ...
    std::unique_ptr<PolygonSurfaceMesh<double>> surface_01_M;
    std::unique_ptr<PolygonSurfaceMeshFieldLinear<double, double>> e_MN_M;
    VolumeIntersector<PolyMeshBuilder<double>, Obb>().IntersectFields(box_field0_M_, box_bvh0_M_, octahedron_field1_N_,
                                                                      octahedron_bvh1_N_, X_MN, &surface_01_M, &e_MN_M);
...
TEST_F(VolumeIntersectorTest, IntersectCompliantVolumes) {
    ...
    const RigidTransformd X_WN(0.03 * Vector3d::UnitX());
    {
        ...
        HydroelasticVolumeIntersector<TriMeshBuilder<double>>().IntersectCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
                &contact_patch_W);
        ASSERT_NE(contact_patch_W.get(), nullptr);
        EXPECT_EQ(contact_patch_W->representation(), HydroelasticContactRepresentation::kTriangle);
    }
    {
        SCOPED_TRACE("Polygon contact surface.");
        std::unique_ptr<ContactSurface<double>> contact_patch_W;
        HydroelasticVolumeIntersector<PolyMeshBuilder<double>>().IntersectCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
                &contact_patch_W);
        ASSERT_NE(contact_patch_W.get(), nullptr);
        EXPECT_EQ(contact_patch_W->representation(), HydroelasticContactRepresentation::kPolygon);
        ...
        const double kRelativeTolerance = 1e-13;
        const double kGradientTolerance = kRelativeTolerance * kExpectGradientOrderOfMagnitude;
        const Vector3d expect_grad_e0_M = kExpectGradientOrderOfMagnitude * Vector3d(-1 / 3.0, 0, 0);
        EXPECT_TRUE(CompareMatrices(contact_patch_W->EvaluateGradE_M_W(0), expect_grad_e0_M, kGradientTolerance));
        const Vector3d expect_grad_e1_M = kExpectGradientOrderOfMagnitude * Vector3d(1 / 3.0, -1 / 3.0, -1 / 3.0);
        EXPECT_TRUE(CompareMatrices(contact_patch_W->EvaluateGradE_N_W(0), expect_grad_e1_M, kGradientTolerance));
    }
}
...
TEST_F(VolumeIntersectorTest, IntersectCompliantVolumesAutoDiffXd) {
    GeometryId first_id = GeometryId::get_new_id();
    GeometryId second_id = GeometryId::get_new_id();
    const math::RigidTransform<AutoDiffXd> X_WM = math::RigidTransform<AutoDiffXd>::Identity();
    const math::RigidTransform<AutoDiffXd> X_WN(0.03 * Vector3<AutoDiffXd>::UnitX());
    {
        SCOPED_TRACE("Triangle contact surface.");
        std::unique_ptr<ContactSurface<AutoDiffXd>> contact_patch_W;
        HydroelasticVolumeIntersector<TriMeshBuilder<AutoDiffXd>>().IntersectCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
                &contact_patch_W);
        ASSERT_NE(contact_patch_W.get(), nullptr);
        EXPECT_EQ(contact_patch_W->representation(), HydroelasticContactRepresentation::kTriangle);
    }
    {
        SCOPED_TRACE("Polygon contact surface.");
        std::unique_ptr<ContactSurface<AutoDiffXd>> contact_patch_W;
        HydroelasticVolumeIntersector<PolyMeshBuilder<AutoDiffXd>>().IntersectCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
                &contact_patch_W);
        ASSERT_NE(contact_patch_W.get(), nullptr);
        EXPECT_EQ(contact_patch_W->representation(), HydroelasticContactRepresentation::kPolygon);
...
TEST_F(VolumeIntersectorTest, ComputeContactSurfaceFromCompliantVolumes) {
    ...
    const RigidTransformd X_WN(0.03 * Vector3d::UnitX());
    {
        ...
        std::unique_ptr<ContactSurface<double>> contact_patch_W = ComputeContactSurfaceFromCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
...
// Smoke tests that AutoDiffXd can build. No checking on the values of
// derivatives.
TEST_F(VolumeIntersectorTest, ComputeContactSurfaceFromCompliantVolumesAutoDiffXd) {
    GeometryId first_id = GeometryId::get_new_id();
    GeometryId second_id = GeometryId::get_new_id();
    const math::RigidTransform<AutoDiffXd> X_WM = math::RigidTransform<AutoDiffXd>::Identity();
    const math::RigidTransform<AutoDiffXd> X_WN(0.03 * Vector3<AutoDiffXd>::UnitX());
    {
        SCOPED_TRACE("Request triangles.");
        std::unique_ptr<ContactSurface<AutoDiffXd>> contact_patch_W = ComputeContactSurfaceFromCompliantVolumes(
                first_id, box_field0_M_, box_bvh0_M_, X_WM, second_id, octahedron_field1_N_, octahedron_bvh1_N_, X_WN,
                HydroelasticContactRepresentation::kTriangle);
    }
    {
