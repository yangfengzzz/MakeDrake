...
void CheckMinMaxBoundaryValue(const VolumeMeshFieldLinear<double, double>& pressure_field,
    ...
    if (has_center_vertex) {
        ...
        EXPECT_NEAR(max_pressure, pressure_field.EvaluateAtVertex(center_vertex), tolerance);
    }
}
...
GTEST_TEST(MakeCylinderFieldTest, MakeCylinderPressureFieldWithMaLong) {
    ...
    const VolumeMesh<double> mesh = MakeCylinderVolumeMeshWithMa<double>(cylinder, 0.25);
    const double kElasticModulus = 1.0e5;
