...
GTEST_TEST(VolumeToSurfaceMeshTest, IdentifyBoundaryFaces) {
    ...
    const std::vector<VolumeElement> tetrahedra{VolumeElement(data[0]), VolumeElement(data[1])};
    ...
    const std::vector<std::array<int, 3>> expect_faces{{1, 3, 0}, {4, 1, 0}, {3, 2, 0},
                                                       {2, 4, 0}, {1, 2, 3}, {2, 1, 4}};
...
GTEST_TEST(VolumeToSurfaceMeshTest, CollectUniqueVertices) {
    ...
    const std::set<int> vertex_set(unique_vertices.begin(), unique_vertices.end());
...
void TestVolumeToSurfaceMesh() {
    ...
    for (const Vector3<T>& r_MV : volume.vertices()) {
        ...
        using std::abs;
        if (abs(r_MV.x()) == T(0.1) || abs(r_MV.y()) == T(0.2) || abs(r_MV.z()) == T(0.3)) {
