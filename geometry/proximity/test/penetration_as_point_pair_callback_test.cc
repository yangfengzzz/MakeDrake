...
public:
    PenetrationAsPointPairCallbackTest()
        : ::testing::Test(),
          ...
          cylinder_(make_shared<fcl::Cylinderd>(cylinder_size_[0], cylinder_size_[1])),
          halfspace_(make_shared<fcl::Halfspaced>(halfspace_normal_, halfspace_offset_)),
          capsule_(make_shared<fcl::Capsuled>(capsule_size_[0], capsule_size_[1])),
...
protected:
    ...
    void TestNoCollision() {
        ...
        const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{{id_A_, X_WA}, {id_B_, X_WB}}};
    ...
    void TestCollisionFilterRespected() {
        ...
        if constexpr (std::is_same_v<T, AutoDiffXd>) {
            p_WBo = math::InitializeAutoDiff((Vector3d{1, -2, 3}.normalized() * center_distance));
        } else {
            p_WBo = (Vector3d{1, -2, 3}.normalized() * center_distance);
        }
        const RigidTransform<T> X_WB = RigidTransform<T>{
                RotationMatrix<T>::MakeYRotation(M_PI / 3) * RotationMatrix<T>::MakeZRotation(-M_PI / 7), p_WBo};
        const RigidTransform<T> X_WA = RigidTransform<T>::Identity();
        const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{{id_A_, X_WA}, {id_B_, X_WB}}};
        ...
        const Eigen::Vector3d nhat_BA_W = ExtractDoubleOrThrow(first_result.nhat_BA_W);
        ...
        EXPECT_EQ(ExtractDoubleOrThrow(second_result.depth), ExtractDoubleOrThrow(first_result.depth));
        ASSERT_TRUE(CompareMatrices(ExtractDoubleOrThrow(first_result.nhat_BA_W),
                                    ExtractDoubleOrThrow(second_result.nhat_BA_W)));
        ASSERT_TRUE(
                CompareMatrices(ExtractDoubleOrThrow(first_result.p_WCa), ExtractDoubleOrThrow(second_result.p_WCa)));
        ASSERT_TRUE(
                CompareMatrices(ExtractDoubleOrThrow(first_result.p_WCb), ExtractDoubleOrThrow(second_result.p_WCb)));
        ...
        CollisionFilter::ExtractIds extract = [&ids](const GeometrySet&, CollisionFilterScope) {
            return ids;
        };
        collision_filter_.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{id_A_, id_B_}), extract,
                                false /* is_invariant */);
    ...
    template <typename T>
    void TestSphereShape(double target_depth,
                         const RigidTransform<T>& X_WB,
                         fcl::CollisionObjectd shape,
                         GeometryId shape_id) {
        ...
        const RigidTransform<T> X_WA = RigidTransform<T>::Identity();
        const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{{id_A_, X_WA}, {shape_id, X_WB}}};
        ...
        EXPECT_NEAR(ExtractDoubleOrThrow((first_result.p_WCb - first_result.p_WCa).dot(first_result.nhat_BA_W)),
                    target_depth, kEps);
        point_pairs.clear();
        ...
        if constexpr (std::is_same_v<T, AutoDiffXd>) {
            ...
            const RigidTransform<double> X_WA_double = RigidTransform<double>::Identity();
            ...
            CallbackData<double> callback_data_double(&collision_filter_, &X_WGs_double, &point_pairs_double);
            EXPECT_FALSE(Callback<double>(&sphere_A_, &shape, &callback_data_double));
            ASSERT_EQ(point_pairs_double.size(), 1u);
            EXPECT_NEAR(ExtractDoubleOrThrow(first_result.depth), point_pairs_double[0].depth, 2 * kEps);
            EXPECT_TRUE(CompareMatrices(math::ExtractValue(first_result.p_WCa), point_pairs_double[0].p_WCa));
            EXPECT_TRUE(CompareMatrices(math::ExtractValue(first_result.p_WCb), point_pairs_double[0].p_WCb, kEps));
            EXPECT_TRUE(CompareMatrices(math::ExtractValue(first_result.nhat_BA_W), point_pairs_double[0].nhat_BA_W));
        }
    }
    ...
    RigidTransform<T> CalcBoxPoseInSphereBox(double target_depth) {
        ...
        if constexpr (std::is_same_v<T, AutoDiffXd>) {
            p_WBo = R_WB * math::InitializeAutoDiff(Vector3d::UnitX() * center_distance);
        } else {
    ...
    RigidTransform<T> CalcCylinderPoseInSphereCylinder(double target_depth) {
        DRAKE_DEMAND(target_depth > 0 && target_depth < cylinder_size_[0] && target_depth < kRadius);
    ...
    std::pair<RigidTransform<T>, double> CalcHalfspacePoseInSphereHalfspace() {
        const RotationMatrix<T> R_WB = RotationMatrix<T>::MakeZRotation(0.1) * RotationMatrix<T>::MakeXRotation(0.5);
        Vector3<T> p_WBo;
        if constexpr (std::is_same_v<T, AutoDiffXd>) {
        ...
        const double target_depth =
                halfspace_offset_ + halfspace_normal_.dot(ExtractDoubleOrThrow(R_WB.inverse() * p_WBo)) + kRadius;
        const RigidTransform<T> X_WB = RigidTransform<T>{R_WB, p_WBo};
    ...
    RigidTransform<T> CalcCapsulePoseInSphereCapsule(double target_depth) {
        DRAKE_DEMAND(target_depth > 0 && target_depth < kRadius && target_depth < capsule_size_[0]);
    ...
    void UnsupportedGeometry(fcl::CollisionObjectd shape1,
        ...
        const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{
                {{id1, RigidTransform<T>::Identity()}, {id2, RigidTransform<T>::Identity()}}};
        vector<PenetrationAsPointPair<T>> point_pairs;
        CallbackData<T> callback_data(&collision_filter_, &X_WGs, &point_pairs);
        DRAKE_EXPECT_THROWS_MESSAGE(Callback<T>(&shape1, &shape2, &callback_data),
...
TEST_F(PenetrationAsPointPairCallbackTest, TestGradient) {
    ...
    const double target_depth = 0.1;
    const double center_distance = kRadius * 2 - target_depth;
    const Eigen::Vector3d p_WBo_val = Vector3d{1, -2, 3}.normalized() * center_distance;
    ...
    const RigidTransform<AutoDiffXd> X_WB = RigidTransform<AutoDiffXd>{
            RotationMatrix<AutoDiffXd>::MakeYRotation(M_PI / 3) * RotationMatrix<AutoDiffXd>::MakeZRotation(-M_PI / 7),
            p_WBo};
    const RigidTransform<AutoDiffXd> X_WA = RigidTransform<AutoDiffXd>::Identity();
    X_WGs.emplace(id_A_, X_WA);
    X_WGs.emplace(id_B_, X_WB);
    vector<PenetrationAsPointPair<AutoDiffXd>> point_pairs;
    CallbackData<AutoDiffXd> callback_data(&collision_filter_, &X_WGs, &point_pairs);
    ...
    const Eigen::Matrix3d dp_WBo_normalized_dp_WBo =
            (p_WBo_val.squaredNorm() * Eigen::Matrix3d::Identity() - p_WBo_val * p_WBo_val.transpose()) /
            std::pow(p_WBo_val.norm(), 3);
    // nhat_BA_W = -p_WBo / |p_WBo|, hence ∂nhat_BA_W /∂p_WBo = -∂(p_WBo/|p_WBo|)
    // / ∂p_WBo
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(result.nhat_BA_W), -dp_WBo_normalized_dp_WBo, kEps));
    // p_WCa = radius * p_WBo / |p_WBo|.
    const Eigen::Matrix3d dp_WCa_dp_WBo = dp_WBo_normalized_dp_WBo * kRadius;
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(result.p_WCa), dp_WCa_dp_WBo, kEps));
    // p_WCb = p_WBo - radius * p_WBo / |p_WBo|.
    const Eigen::Matrix3d dp_WCb_dp_WBo = Eigen::Matrix3d::Identity() - dp_WBo_normalized_dp_WBo * kRadius;
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(result.p_WCb), dp_WCb_dp_WBo, kEps));
}

TEST_F(PenetrationAsPointPairCallbackTest, SphereBoxDouble) {
    const double target_depth = 0.1;
    const RigidTransform<double> X_WB = CalcBoxPoseInSphereBox<double>(target_depth);
    TestSphereShape(target_depth, X_WB, box_, id_box_);
}

TEST_F(PenetrationAsPointPairCallbackTest, SphereBoxAutoDiffXd) {
    const double target_depth = 0.1;
    const RigidTransform<AutoDiffXd> X_WB = CalcBoxPoseInSphereBox<AutoDiffXd>(target_depth);
    TestSphereShape(target_depth, X_WB, box_, id_box_);
}

TEST_F(PenetrationAsPointPairCallbackTest, SphereCylinderDouble) {
    const double target_depth = 0.1;
    const RigidTransform<double> X_WB = CalcCylinderPoseInSphereCylinder<double>(target_depth);
...
TEST_F(PenetrationAsPointPairCallbackTest, SphereCylinderAutoDiffXd) {
    const double target_depth = 0.1;
    const RigidTransform<AutoDiffXd> X_WB = CalcCylinderPoseInSphereCylinder<AutoDiffXd>(target_depth);
    TestSphereShape(target_depth, X_WB, cylinder_, id_cylinder_);
}
...
TEST_F(PenetrationAsPointPairCallbackTest, SphereCapsuleDouble) {
    const double target_depth = 0.1;
    const RigidTransform<double> X_WB = CalcCapsulePoseInSphereCapsule<double>(target_depth);
    TestSphereShape(target_depth, X_WB, capsule_, id_capsule_);
}

TEST_F(PenetrationAsPointPairCallbackTest, SphereCapsuleAutoDiffXd) {
    const double target_depth = 0.1;
    const RigidTransform<AutoDiffXd> X_WB = CalcCapsulePoseInSphereCapsule<AutoDiffXd>(target_depth);
    TestSphereShape(target_depth, X_WB, capsule_, id_capsule_);
}
...
TEST_F(PenetrationAsPointPairCallbackTest, UnsupportedAutoDiffXd) {
    ...
    // AutoDiffXd yet.
    std::vector<std::pair<fcl::CollisionObjectd, GeometryId>> unsupported_geometries;
    ...
    for (int i = 0; i < static_cast<int>(unsupported_geometries.size()); ++i) {
        for (int j = 0; j < static_cast<int>(unsupported_geometries.size()); ++j) {
            if (i != j) {
                UnsupportedGeometry<AutoDiffXd>(unsupported_geometries[i].first, unsupported_geometries[j].first,
...
TEST_F(PenetrationAsPointPairCallbackTest, UnsupportedExpression) {
    ...
    for (int i = 0; i < static_cast<int>(unsupported_geometries.size()); ++i) {
        for (int j = 0; j < static_cast<int>(unsupported_geometries.size()); ++j) {
            if (i != j) {
                UnsupportedGeometry<Expression>(unsupported_geometries[i].first, unsupported_geometries[j].first,
                                                unsupported_geometries[i].second, unsupported_geometries[j].second);
            }
        }
    }
...
TEST_F(PenetrationAsPointPairCallbackTest, UnsupportedHalfSpaceHalfSpace) {
    ...
    CollisionObjectd halfspace2(make_shared<fcl::Halfspaced>(Vector3d{1, 0, 0}, 0));
    const GeometryId hs2_id = GeometryId::get_new_id();
    const EncodedData data(hs2_id, true);
    data.write_to(&halfspace2);
    this->collision_filter_.AddGeometry(data.id());
    UnsupportedGeometry<double>(this->halfspace_, halfspace2, this->id_halfspace_, hs2_id);
    UnsupportedGeometry<AutoDiffXd>(this->halfspace_, halfspace2, this->id_halfspace_, hs2_id);
}
