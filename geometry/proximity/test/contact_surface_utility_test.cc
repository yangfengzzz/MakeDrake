...
GTEST_TEST(TriMeshBuilderTest, AddingFeatures) {
    ...
    const RigidTransformd X_MN(AngleAxisd{M_PI / 7, Vector3d{1, 2, 3}.normalized()}, Vector3d{-0.5, 1.25, 0.75});
    // The triangle defined in frame N so it's easy to reason about.
    const std::vector<Vector3d> polygon_N{{0.25, 0, 0.25}, {0.5, 0, 0.25}, {0.25, 0, 0.5}, {0.25, 0, 0.35}};
    // clang-format off
  const std::vector<Vector3d> polygon_M{X_MN * polygon_N[0],
                                        ...
                                        X_MN * polygon_N[3]};
    // clang-format on
    const Vector3d p_MC_expected = (polygon_M[0] + polygon_M[1] + polygon_M[2]) / 3;
    ...
    EXPECT_THROW(builder_M.AddPolygon({0, 1, 2}, Vector3d::Zero(), Vector3d::Zero()), std::exception);
#endif
    ...
    for (int v = 0; v < static_cast<int>(polygon_M.size()); ++v) {
        ...
        polygon_indices.push_back(builder_M.AddVertex(p_MV, expected_pressures.back()));
    }
    ...
    ASSERT_EQ(mesh_M->num_vertices(), static_cast<int>(expected_pressures.size()));
...
public:
    ...
    double operator()(const Vector3d& p_FV) const { return grad_F_.dot(p_FV) + d_; }
...
GTEST_TEST(MeshBuilder, EquivalentForceIntegration) {
    vector<Polygon> polygons_M{
            {{Vector3d{0, 0, 0}, Vector3d{0, 1, 0}, Vector3d{0, 0, 1}}, LinearFunction{Vector3d{1, 2, 3}, 3}},
    ...
    for (const auto& polygon_M : polygons_M) {
        ...
        for (const auto& p_MV : polygon_M.vertices()) {
            tri_indices.push_back(tri_builder_M.AddVertex(p_MV, polygon_M.EvalPressure(p_MV)));
            poly_indices.push_back(poly_builder_M.AddVertex(p_MV, polygon_M.EvalPressure(p_MV)));
        }
        const Vector3d nhat_M = polygon_M.normal();
        tri_builder_M.AddPolygon(tri_indices, nhat_M, polygon_M.pressure_gradient());
        poly_builder_M.AddPolygon(poly_indices, nhat_M, polygon_M.pressure_gradient());
    }
    ...
    auto calc_normal_force = [](const auto& mesh, const auto& field) {
        ...
        for (int e = 0; e < mesh.num_elements(); ++e) {
            ...
            force += mesh.area(e) * field.EvaluateCartesian(e, centroid) * mesh.face_normal(e);
        }
        return force;
    };
...
protected:
    void SetUp() {
        ...
        X_FMs_.push_back(
                RigidTransformd{AngleAxisd{0.001, Vector3d{1, 2, 3}.normalized()}, Vector3d{-0.5, 1.25, 0.75}});
        X_FMs_.push_back(
                RigidTransformd{AngleAxisd{-9 * M_PI / 7, Vector3d{1, 2, 3}.normalized()}, Vector3d{0.5, 0.75, -1.25}});
    }

    // Compute the average vertex position of the given `polygon`.
    static Vector3d AverageVertex(const vector<int>& polygon, const vector<Vector3d>& vertices_F) {
...
TEST_F(ContactSurfaceUtilityTest, PolygonCentroidTest) {
    ...
    // on these vertices.
    const vector<Vector3d> vertices_M{Vector3d{0, 0, 0},      Vector3d{1, 0, 0},     Vector3d{0, 1, 0},
    ...
    for (const auto& X_FM : X_FMs_) {
        ...
        std::transform(vertices_M.begin(), vertices_M.end(), std::back_inserter(vertices_F),
...
TEST_F(ContactSurfaceUtilityTest, ZeroAreaPolygon) {
    ...
    // Triangle: (a, a, b)
    // Quad: (a, a, b, b)
    const vector<Vector3d> vertices{Vector3d{0.25, 1.5, -3}, Vector3d{-0.75, 0.25, 1.25}};
    const int a{0};
    const int b{1};
...
TEST_F(ContactSurfaceUtilityTest, NonPlanarPolygon) {
    ...
    if (kDrakeAssertIsArmed) {
        ...
        const vector<Vector3d> vertices_M{Vector3d{-1.5, -0.25, kEps}, Vector3d{1, 0, -kEps},
                                          Vector3d{0.75, 1.25, -kEps}, Vector3d{0, 1, -kEps}};
        const vector<int> quad{0, 1, 2, 3};
        DRAKE_EXPECT_THROWS_MESSAGE(CalcPolygonCentroid(quad, Mz, vertices_M),
...
TEST_F(ContactSurfaceUtilityTest, EvalNormalWithDegeneratePolygon) {
    if (kDrakeAssertIsArmed) {
        const vector<Vector3d> vertices_M{Vector3d{-1.5, -0.25, 0}, Vector3d{1, 0, 0}, Vector3d{0.75, 1.25, 0},
                                          Vector3d{0, 1, 0}};
...

        {
            ...
            const vector<int> quad{0, 1, 2, 3};
            DRAKE_EXPECT_THROWS_MESSAGE(CalcPolygonCentroid(quad, My, vertices_M),
...
TEST_F(ContactSurfaceUtilityTest, PolygonCentroidTest_NormalUse) {
    ...
    const Vector3d p_MC_expected = AverageVertex(vector<int>{0, 1, 2}, vertices_M);

    {
        ...
        if (kDrakeAssertIsArmed) {
            ...
            DRAKE_EXPECT_THROWS_MESSAGE(CalcPolygonCentroid(quad, kZeroVec, vertices_M),
            ...
            const Vector3d expected_centroid =
                    (vertices_M[quad[0]] + vertices_M[quad[1]] + vertices_M[quad[2]] + vertices_M[quad[3]]) / 4;
            EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid(quad, kZeroVec, vertices_M), expected_centroid));
        }
    }

    {
        ...
        if (kDrakeAssertIsArmed) {
            ...
            EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid(pseudo_triangle, kZeroVec, vertices_M), p_MC_expected));
        }
    }

    {
        ...
        if (kDrakeAssertIsArmed) {
            // With assertions armed; we throw.
            DRAKE_EXPECT_THROWS_MESSAGE(CalcPolygonCentroid(pseudo_triangle, Fy, vertices_M),
                                        "CalcPolygonCentroid: the given normal is not perpendicular to the "
                                        "polygon's plane.*");
        } else {
            // Without assertions, we compute the average vertex position.
            ...
            const Vector3d expected_centroid = (vertices_M[pseudo_triangle[0]] + vertices_M[pseudo_triangle[1]] +
                                                vertices_M[pseudo_triangle[2]] + vertices_M[pseudo_triangle[3]]) /
                                               4;
            EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid(pseudo_triangle, Fy, vertices_M), expected_centroid));
        }
    }
    ...
    for (const auto& X_FM : X_FMs_) {
        ...
        if (kDrakeAssertIsDisarmed) {
            ...
            EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid<double>(pseudo_triangle, 2 * Mz_F, vertices_F),
                                        p_FC_expected, kEps));
            EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid<double>(pseudo_triangle, 0.5 * Mz_F, vertices_F),
                                        p_FC_expected, kEps));
        }
        ...
        EXPECT_TRUE(CompareMatrices(CalcPolygonCentroid<double>(pseudo_triangle, not_normal_F, vertices_F),
...
TEST_F(ContactSurfaceUtilityTest, AddPolygonToTriangleMeshData) {
    ...
    //       v0    │
    //             │
    const vector<Vector3d> vertices_source{Vector3d{-1.5, -0.25, 0}, Vector3d{1, 0, 0}, Vector3d{0.75, 1.25, 0},
                                           Vector3d{0, 1, 0}};
    ...
    auto triangle_normal = [](int v0, int v1, int v2, const vector<Vector3d>& vertices_F) -> Vector3d {
    ...
    ASSERT_TRUE(CompareMatrices(vertices_M.back(), CalcPolygonCentroid(quad, nhat_M, vertices_M)));
    ...
    for (const auto& face : faces) {
        ...
        const Vector3d tri_normal_M = triangle_normal(face.vertex(0), face.vertex(1), face.vertex(2), vertices_M);
        EXPECT_NEAR(tri_normal_M.dot(poly_normal_M), 1, std::numeric_limits<double>::epsilon());
        for (int i = 0; i < 3; ++i) {
