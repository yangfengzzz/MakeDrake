...
GTEST_TEST(SplitTriangularPrismToTetrahedraTest, SimpleTest) {
    std::vector<VolumeElement> tetrahedra = SplitTriangularPrismToTetrahedra(0, 1, 2, 3, 4, 5);
    ...
    // need to change too.
    EXPECT_TRUE(tetrahedra.end() != std::find(tetrahedra.begin(), tetrahedra.end(), VolumeElement(3, 4, 0, 5)));
    EXPECT_TRUE(tetrahedra.end() != std::find(tetrahedra.begin(), tetrahedra.end(), VolumeElement(4, 1, 0, 5)));
    EXPECT_TRUE(tetrahedra.end() != std::find(tetrahedra.begin(), tetrahedra.end(), VolumeElement(1, 2, 0, 5)));
}

GTEST_TEST(SplitPyramidToTetrahedraTest, SimpleTest) {
    std::vector<VolumeElement> tetrahedra = SplitPyramidToTetrahedra(0, 1, 2, 3, 4);

    EXPECT_EQ(tetrahedra.size(), 2);
    EXPECT_TRUE(tetrahedra.end() != std::find(tetrahedra.begin(), tetrahedra.end(), VolumeElement(3, 4, 0, 2)));
    EXPECT_TRUE(tetrahedra.end() != std::find(tetrahedra.begin(), tetrahedra.end(), VolumeElement(4, 1, 0, 2)));
}

GTEST_TEST(CalcEnclosedVolumeTest, SimpleTest) {
    std::string obj = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");
    TriangleSurfaceMesh<double> surface_mesh = ReadObjToTriangleSurfaceMesh(obj, 1.0);
    ...
    // the volume.
    surface_mesh.TransformVertices(math::RigidTransform<double>(Eigen::Vector3d{4, 4, 4}));
    EXPECT_NEAR(CalcEnclosedVolume(surface_mesh), 8.0, 1e-14);
