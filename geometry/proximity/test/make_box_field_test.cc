...
GTEST_TEST(MakeBoxFieldTest, MakeBoxPressureField) {
    ...
    VolumeMeshFieldLinear<double, double> pressure_field = MakeBoxPressureField<double>(box, &mesh, kElasticModulus);
    ...
    std::vector<int> boundary_vertex_indices = CollectUniqueVertices(IdentifyBoundaryFaces(mesh.tetrahedra()));
...
GTEST_TEST(MakeBoxFieldTest, MakeBoxPressureFieldInMeshWithMedialAxis) {
    ...
    const double kPressureTolerance = kElasticModulus * std::numeric_limits<double>::epsilon();
    for (int e = 0; e < mesh.num_elements(); ++e) {
        const double pressure_v0 = pressure_field.EvaluateAtVertex(mesh.element(e).vertex(0));
        bool same_pressure = true;
        for (int i = 1; i < mesh.kVertexPerElement && same_pressure; ++i) {
            const double pressure_vi = pressure_field.EvaluateAtVertex(mesh.element(e).vertex(i));
