...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineTetrahedron) {
    ...
    EXPECT_EQ(DetectInteriorTriangleWithAllBoundaryVertices(refined_mesh).size(), 0);
    ...
    for (const VolumeElement& tetrahedron : refined_mesh.tetrahedra()) {
        ASSERT_TRUE(tetrahedron.vertex(0) == kNewVertexIndex || tetrahedron.vertex(1) == kNewVertexIndex ||
                    tetrahedron.vertex(2) == kNewVertexIndex || tetrahedron.vertex(3) == kNewVertexIndex);
    }
}
...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineTriangle) {
    ...
    EXPECT_EQ(refined_mesh.num_vertices(), kExpectedNumVerts);
    EXPECT_EQ(refined_mesh.num_elements(), kExpectedNumTets);
    EXPECT_EQ(refined_mesh.vertex(kNewVertexIndex), Vector3d(1.0 / 3, 1.0 / 3, 0));
    for (const VolumeElement& tetrahedron : refined_mesh.tetrahedra()) {
        ASSERT_TRUE(tetrahedron.vertex(0) == kNewVertexIndex || tetrahedron.vertex(1) == kNewVertexIndex ||
                    tetrahedron.vertex(2) == kNewVertexIndex || tetrahedron.vertex(3) == kNewVertexIndex);
    }
...
GTEST_TEST(VolumeMeshRefinerTest, TestRefineEdge) {
    ...
    const VolumeMesh<double> test_mesh(
            ...
            std::vector<VolumeElement>{{0, 1, 2, 3}, {1, 0, 2, 4}, {0, 2, 5, 3}, {2, 0, 5, 4}},
            std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ(),
