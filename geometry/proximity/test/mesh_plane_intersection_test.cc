#include "geometry/proximity/mesh_plane_intersection.h"
...
#include "geometry/proximity/contact_surface_utility.h"
...
*/
template <typename T>
VolumeMesh<T> TrivialVolumeMesh(const RigidTransform<T>& X_FM = RigidTransform<T>::Identity(),
...
protected:
    void SetUp() override {
        ...
        // error).
        X_WF_ = RigidTransformd{RotationMatrixd{AngleAxisd{M_PI / 3, Vector3d{-1, 1, 1}.normalized()}},
    ...
    pair<unique_ptr<PolygonSurfaceMesh<double>>, unique_ptr<PolygonSurfaceMeshFieldLinear<double, double>>>
    CallSliceFunction(const Plane<double>& plane_F, const RigidTransformd& X_FM, SliceFunctionType type) {
        switch (type) {
    ...
    pair<unique_ptr<PolygonSurfaceMesh<double>>, unique_ptr<PolygonSurfaceMeshFieldLinear<double, double>>>
    CallSliceTetWithPlane(const Plane<double>& plane_F, const RigidTransformd& X_FM) {
        ...
        field_F_ = make_unique<VolumeMeshFieldLinear<double, double>>(std::move(values), volume_mesh_F_.get());

        cut_edges_.clear();
        const int tet_index = 0;
        // Every invocation gets a new builder instance.
        builder_W_ = PolyMeshBuilder<double>();
        SliceTetWithPlane(tet_index, *field_F_, plane_F, X_WF_, &builder_W_, &cut_edges_);
    ...
    /* Invokes SliceTetrahedronWithPlane() on a trivial mesh in frame F. This
     ...
     @returns The resulting mesh and field expressed in the world frame. */
    pair<unique_ptr<PolygonSurfaceMesh<double>>, unique_ptr<PolygonSurfaceMeshFieldLinear<double, double>>>
    CallSliceTetrahedronWithPlane(const Plane<double>& plane_F, const RigidTransformd& X_FM) {
        volume_mesh_F_ = make_unique<VolumeMesh<double>>(TrivialVolumeMesh(X_FM));
        // Make an arbitrary mesh field with heterogeneous values.
        vector<double> values{0.25, 0.5, 0.75, 1, -1};
        field_F_ = make_unique<VolumeMeshFieldLinear<double, double>>(std::move(values), volume_mesh_F_.get());
        ...
        cut_edges_.clear();
        SliceTetrahedronWithPlane(tet_index, *volume_mesh_F_, plane_F, &polygon_vertices, &edges);
        ...
        if (polygon_vertices.size() != 0) {
            ...
            for (int i = 0; i < static_cast<int>(polygon_vertices.size()); ++i) {
                ...
                const int new_index =
                        builder_W_.AddVertex(p_WC, field_F_->EvaluateCartesian(tet_index, polygon_vertices[i]));
                face_verts.push_back(new_index);
                cut_edges_[edges[i]] = new_index;
            }

            const Vector3<double> nhat_W = X_WF_.rotation() * plane_F.normal();
            const Vector3<double> grad_e_FN_W = X_WF_.rotation() * field_F_->EvaluateGradient(tet_index);
    ...
    ::testing::AssertionResult HasNVertices(const PolygonSurfaceMesh<double>& mesh_W,
                                            const PolygonSurfaceMeshFieldLinear<double, double>& field_W,
                                            size_t num_vertices) const {
        bool error = false;
        ::testing::AssertionResult failure = ::testing::AssertionFailure();
        auto test_features = [&failure, &error](size_t test_count, size_t expected_count, const char* label) {
    ...
    pair<vector<EdgeVertex>, ::testing::AssertionResult> CharacterizeEdgeVertices(
            const PolygonSurfaceMesh<double>& slice_mesh_W,
            int tet_index,
        ...
        const int tet_edges[6][2] = {{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}};
        ...
        for (int i = 0; i < poly.num_vertices(); ++i) {
            ...
            for (int e = 0; e < 6; ++e) {
                ...
                if (w > 1 + kEps) {
                    return {{},
                            ::testing::AssertionFailure() << "Vertex " << v << " is co-linear with edge " << e
        ...
        if (static_cast<int>(edge_vertices.size()) != slice_mesh_W.num_vertices()) {
            return {{},
                    ::testing::AssertionFailure() << "Only " << edge_vertices.size() << " slice vertices were "
                                                  << "mapped to tet edges; expected " << slice_mesh_W.num_vertices()};
        }
        ...
        for (const auto& edge_vertex : edge_vertices) {
            ...
            if (!cut_edges_.contains(computed_edge)) {
                return {{},
                        ::testing::AssertionFailure()
                                << fmt::format("We matched surface vertex {} to edge {}, but that can't "
            ...
            if (cut_edges_.at(computed_edge) != edge_vertex.slice_vertex) {
                return {{},
                        ::testing::AssertionFailure()
                                << fmt::format("We matched surface vertex {} to edge {}, but the cut edge "
                                               " cache has surface vertex {}",
                                               edge_vertex.slice_vertex, computed_edge, cut_edges_.at(computed_edge))};
            }
        }
    ...
    ::testing::AssertionResult PressuresMatchVertices(const PolygonSurfaceMeshFieldLinear<double, double>& field_W,
                                                      int tet_index,
        ...
        for (const auto& edge_vertex : edge_vertices) {
            ...
            const double expected_pressure = p0 * edge_vertex.weight + (1 - edge_vertex.weight) * p1;
            ...
            if (std::abs(pressure - expected_pressure) > kEps) {
                return ::testing::AssertionFailure()
                       << "\nSurface vertex " << test_vertex_index << " has wrong pressure\n"
                       << " Found    " << pressure << "\n"
    ...
    ::testing::AssertionResult SliceIsConsistent(const PolygonSurfaceMesh<double>& mesh_W,
                                                 const PolygonSurfaceMeshFieldLinear<double, double>& field_W,
                                                 int tet_index,
                                                 const VolumeMeshFieldLinear<double, double>& field_F) const {
        // Confirms that each perimeter vertex lies on an edge of the tetrahedron.
        const auto [edge_vertices, result] = CharacterizeEdgeVertices(mesh_W, tet_index, field_F.mesh());
    ...
    static Vector3d ComputeTetCentroid(const VolumeMesh<double>& mesh_M, const VolumeElement& tet) {
    ...
    ::testing::AssertionResult FaceNormalMatches(const PolygonSurfaceMesh<double>& mesh_W,
                                                 int face_index,
...
TEST_P(SliceTest, NonIntersectingConfiguration) {
    ...
    vector<RigidTransformd> X_FMs{
            ...
            RigidTransformd{RotationMatrixd{AngleAxisd{8 * M_PI / 7, Vector3d{1, 2, 3}.normalized()}},
    ...
    for (const auto& X_FM : X_FMs) {
        ...
        {
            const auto [mesh_W, field_W] = CallSliceFunction(Plane<double>{Mz_F, p_FMax + offset_F}, X_FM, GetParam());
        ...
        {
            const auto [mesh_W, field_W] = CallSliceFunction(Plane<double>{Mz_F, p_FMin - offset_F}, X_FM, GetParam());
            EXPECT_TRUE(HasNVertices(*mesh_W, *field_W, 0));
        }
        ...
        // point _near_ V3, but just offset in the normal direction.
        {
            const auto [mesh_W, field_W] =
                    CallSliceFunction(Plane<double>{normal_F, p_FMax + kEps * normal_F}, X_FM, GetParam());
            EXPECT_TRUE(HasNVertices(*mesh_W, *field_W, 0));
        }
        ...
        {
            const auto [mesh_W, field_W] =
                    CallSliceFunction(Plane<double>{normal_F, p_FMax - 4 * kEps * normal_F}, X_FM, GetParam());
            EXPECT_TRUE(HasNVertices(*mesh_W, *field_W, 3));
        }
    }
}
...
TEST_P(SliceTest, TriangleIntersections) {
    ...
    vector<RigidTransformd> X_FMs{
            RigidTransformd::Identity(),
            RigidTransformd{RotationMatrixd{AngleAxisd{9 * M_PI / 7, Vector3d{1, 2, 3}.normalized()}},
                            Vector3d{-2.3, -4.2, 3.7}}};
    for (const auto& X_FM : X_FMs) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            for (const auto plane_sign : {1.0, -1.0}) {
                ...
                const auto [mesh_W, field_W] = CallSliceFunction(plane_F, X_FM, GetParam());
...
TEST_P(SliceTest, QuadIntersections) {
    ...
    for (const auto& X_FM : X_FMs) {
        ...
        auto nearest_point_to_edge = [](const Vector3d& p_FQ, const Vector3d& p_FA, const Vector3d& p_FB) -> Vector3d {
...
TEST_F(SliceTest, DuplicateOutputFromDuplicateInput) {
    ...
    VolumeMesh<double> min_mesh_F = TrivialVolumeMesh(X_FM, true /* min_vertices */);
    VolumeMeshFieldLinear<double, double> min_field_F{vector<double>{0, 0, 0, 1, -1}, &min_mesh_F};
    unordered_map<SortedPair<int>, int> min_cut_edges;

    // The infrastructure for the mesh with duplicate vertices.
    VolumeMesh<double> dupe_mesh_F = TrivialVolumeMesh(X_FM, false /* min_vertices */);
    VolumeMeshFieldLinear<double, double> dupe_field_F{vector<double>{0, 0, 0, 1, 0, 0, 0, -1}, &dupe_mesh_F};
    ...
    for (int i = 0; i < 2; ++i) {
        SliceTetWithPlane(i, min_field_F, plane_F, X_WF_, &min_builder_W, &min_cut_edges);
        SliceTetWithPlane(i, dupe_field_F, plane_F, X_WF_, &dupe_builder_W, &dupe_cut_edges);
    }
    ...
    ASSERT_EQ(static_cast<int>(vertex_indices.size()), dupe_mesh_W->num_vertices());
}
...
INSTANTIATE_TEST_SUITE_P(SliceFunctionCases,
                         SliceTest,
                         ::testing::Values(SliceFunctionType::SliceTet, SliceFunctionType::SliceTetrahedron));
...
protected:
    void SetUp() override {
        X_WF_ = RigidTransformd{RotationMatrixd{AngleAxisd{M_PI / 2, Vector3d::UnitX()}}, Vector3d{1.25, 2.5, -3.75}};
        X_FM_ = RigidTransformd{RotationMatrixd{AngleAxisd{9 * M_PI / 7, Vector3d{1, 2, 3}.normalized()}},
                                Vector3d{-2.3, -4.2, 3.7}};

        // NOTE: all tests assume the minimum number of vertices in the mesh; no
        // duplicates.
        mesh_F_ = make_unique<VolumeMesh<double>>(TrivialVolumeMesh(X_FM_, true /* minimum_vertices */));
        field_F_ = make_unique<VolumeMeshFieldLinear<double, double>>(vector<double>{0, 0, 0, 1, -1}, mesh_F_.get());
...
TEST_F(ComputeContactSurfaceTest, NoIntersectionReturnsNullPtr) {
    const Vector3d& Mz_F = X_FM_.rotation().col(2);
    {
        ...
        EXPECT_EQ(ComputeContactSurface<TriMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, both_tets_,
                                                                X_WF_),
                  nullptr);
    }

    {
        ...
        const Plane<double> plane_F{Mz_F, p_FB};
        EXPECT_EQ(ComputeContactSurface<TriMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, only_tet_1_,
                                                                X_WF_),
                  nullptr);
    }
}
...
TEST_F(ComputeContactSurfaceTest, AllTetsAreConsidered) {
    ...
    unique_ptr<ContactSurface<double>> contact_surface =
            ComputeContactSurface<TriMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, both_tets_, X_WF_);
    ...
    auto contact_surface_0 =
            ComputeContactSurface<TriMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, only_tet_0_, X_WF_);
    ...
    auto contact_surface_1 =
            ComputeContactSurface<TriMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, only_tet_1_, X_WF_);
    ...
    */
    {
        ...
        // Passing in all tet indices produces the full intersection: 6 triangles.
        unique_ptr<ContactSurface<double>> contact_surface = ComputeContactSurface<TriMeshBuilder<double>>(
                mesh_id_, *field_F_, plane_id_, plane_F, both_tets_, X_WF_);
        ASSERT_NE(contact_surface, nullptr);
        const TriangleSurfaceMesh<double>& contact_mesh_W = contact_surface->tri_mesh_W();
...
TEST_F(ComputeContactSurfaceTest, NormalsInPlaneDirection) {
    ...
    vector<pair<GeometryId, GeometryId>> ids{{mesh_id_, plane_id_}, {plane_id_, mesh_id_}};
    for (const auto& [id_A, id_B] : ids) {
        unique_ptr<ContactSurface<double>> contact =
                ComputeContactSurface<TriMeshBuilder<double>>(id_A, *field_F_, id_B, plane_F, both_tets_, X_WF_);
...
TEST_F(ComputeContactSurfaceTest, GradientConstituentPressure) {
    ...
    const Vector3d& grad_eMesh_W_expected0 = X_WF_.rotation() * field_F_->EvaluateGradient(0);
    const Vector3d& grad_eMesh_W_expected1 = X_WF_.rotation() * field_F_->EvaluateGradient(1);
    ...
    // 0, and for the last three should be tet 1.
    {
        ...
        for (int f = 0; f < 3; ++f) {
            EXPECT_TRUE(CompareMatrices(contact_surface->EvaluateGradE_M_W(f), grad_eMesh_W_expected0));
        }
        for (int f = 3; f < 6; ++f) {
            EXPECT_TRUE(CompareMatrices(contact_surface->EvaluateGradE_M_W(f), grad_eMesh_W_expected1));
        }
    }
    ...
    // be the same.
    {
        ...
        for (int f = 0; f < 3; ++f) {
            EXPECT_TRUE(CompareMatrices(contact_surface->EvaluateGradE_N_W(f), grad_eMesh_W_expected0));
        }
        for (int f = 3; f < 6; ++f) {
            EXPECT_TRUE(CompareMatrices(contact_surface->EvaluateGradE_N_W(f), grad_eMesh_W_expected1));
        }
    }
}
...
TEST_F(ComputeContactSurfaceTest, MeshRepresentation) {
    ...
    unique_ptr<ContactSurface<double>> poly_contact =
            ComputeContactSurface<PolyMeshBuilder<double>>(mesh_id_, *field_F_, plane_id_, plane_F, both_tets_, X_WF_);

    EXPECT_EQ(tri_contact->representation(), HydroelasticContactRepresentation::kTriangle);
    EXPECT_EQ(poly_contact->representation(), HydroelasticContactRepresentation::kPolygon);
}
...
GTEST_TEST(MeshPlaneIntersectionTest, SoftVolumeRigidHalfSpace) {
    ...
    const VolumeMeshFieldLinear<double, double> field_F{vector<double>{0.25, 0.5, 0.75, 1, -1}, &mesh_F};
    const Bvh<Obb, VolumeMesh<double>> bvh_F(mesh_F);

    // We'll pose the plane in the soft mesh's frame S and then transform the
    // whole system.
    const RigidTransformd X_WS{RotationMatrixd{AngleAxisd{M_PI / 3, Vector3d{-1, 1, 1}.normalized()}},
                               Vector3d{1.25, 2.5, -3.75}};
    ...
    const RigidTransformd X_SR{RotationMatrixd{AngleAxisd{M_PI / 2, Vector3d::UnitY()}}, Vector3d{0.5, 2.25, -1.25}};
...

    {
        ...
        auto contact_surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                id_A, field_F, bvh_F, X_WS, id_B, X_WR, HydroelasticContactRepresentation::kTriangle);
        EXPECT_TRUE(contact_surface->HasGradE_M());
        EXPECT_FALSE(contact_surface->HasGradE_N());
        ASSERT_NE(contact_surface, nullptr);
...

    {
        ...
        auto contact_surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                id_A, field_F, bvh_F, X_WS * X_SM, id_B, X_WR, HydroelasticContactRepresentation::kTriangle);
...

    {
        ...
        auto contact_surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                id_A, field_F, bvh_F, X_WS, id_B, X_WS * X_SR2, HydroelasticContactRepresentation::kTriangle);
        ASSERT_EQ(contact_surface, nullptr);
    }

    {
        ...
        auto contact_surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                id_B, field_F, bvh_F, X_WS, id_A, X_WR, HydroelasticContactRepresentation::kTriangle);
        ASSERT_NE(contact_surface, nullptr);
        EXPECT_EQ(contact_surface->id_M(), id_A);
        EXPECT_EQ(contact_surface->id_N(), id_B);
...

    {
        ...
        auto contact_surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                id_A, field_F, bvh_F, X_WS, id_B, X_WR, HydroelasticContactRepresentation::kPolygon);
        EXPECT_EQ(contact_surface->representation(), HydroelasticContactRepresentation::kPolygon);
    }
}
...
protected:
    void SetUp() override {
        ...
        vector<Vector3d> vertices_S({Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()});
        mesh_S_ = make_unique<VolumeMesh<double>>(std::move(elements), std::move(vertices_S));
        field_S_ =
                make_unique<VolumeMeshFieldLinear<double, double>>(vector<double>{0.25, 0.5, 0.75, 1}, mesh_S_.get());
        bvh_S_ = std::make_unique<Bvh<Obb, VolumeMesh<double>>>(*mesh_S_);

        /* Rigid plane; tilt and offset the plane so things are interesting. */
        X_WR_ = HalfSpace::MakePose(Vector3d{1, 2, 3}.normalized(), Vector3d{0.25, 0.1, -0.2}).cast<AutoDiffXd>();
    ...
    /* Creates a rotation to align two vectors: `s` and `t` such that t = Rs.
     ...
     @pre |s| = |t| = 1. */
    static RotationMatrixd OrientTetrahedron(const Vector3d& s_F, const Vector3d& t_F) {
        ...
        if (cos_theta < kAlmostOne) {
            ...
            if (cos_theta < -kAlmostOne) {
                ...
                const math::RotationMatrix<double> basis = math::RotationMatrix<double>::MakeFromOneVector(s_F, 2);
    ...
    void TestPositionDerivative(
            const std::function<void(const ContactSurface<AutoDiffXd>&, const RigidTransform<AutoDiffXd>&, TetPose)>
...

        {
            ...
            configurations.push_back({"Horizontal slice", p_RS_d, RotationMatrixd{}, 3, kHorizontalSlice});
        }

        {
            ...
            const RotationMatrixd R_RS_d = OrientTetrahedron(Vector3d{1, 1, 1}.normalized(), n_R);
            configurations.push_back({"Single vertex penetration", p_RS_d, R_RS_d, 3, kTriangleSlice});
        }

        {
            /* Vertices 0 and 3 (<0, 0, 1>) lie kDepth units on the negative side of
             ...
             plane. */
            const RotationMatrixd R_RS_d = OrientTetrahedron(Vector3d{1, 1, 0}.normalized(), n_R);
            ...
            configurations.push_back({"Two vertex penetration", p_RS_d, R_RS_d, 4, kQuadSlice});
        }
        ...
        for (const auto& config : configurations) {
            ...
            auto surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(
                    id_S_, *field_S_, *bvh_S_, X_WS, id_R_, X_WR_, HydroelasticContactRepresentation::kTriangle);
    ...
    Vector3d GetEdgeDirInS(const Vector3d& p_SE) const {
        ...
        const vector<pair<int, int>> edges{{0, 1}, {0, 2}, {0, 3}, {1, 2}, {1, 3}, {2, 3}};
        ...
        throw std::logic_error("Querying for point E that isn't actually on a tet edge");
    }
...
TEST_F(MeshPlaneDerivativesTest, Area) {
    ...
    auto evaluate_area = [X_WR = convert_to_double(this->X_WR_)](const ContactSurface<AutoDiffXd>& surface,
      ...
      result <<     0, -v(2),  v(1),
        ...
        EXPECT_TRUE(CompareMatrices(total_area.derivatives(), dArea_dSo_expected, 4 * kEps));
    };
...
TEST_F(MeshPlaneDerivativesTest, VertexPosition) {
    ...
    auto evalute_position = [this](const ContactSurface<AutoDiffXd>& surface, const RigidTransform<AutoDiffXd>& X_WS_ad,
                                   TetPose pose) {
      ...
      expected_J_R << 1,  0, -p.x(),
                      ...
                      0,  0,  0;
            // clang-format on
            const Matrix3<double> expected_J_W = R_WR * (expected_J_R * R_RW.matrix());
...
TEST_F(MeshPlaneDerivativesTest, FaceNormalsWrtPosition) {
    ...
    auto evaluate_normals = [X_WR = convert_to_double(this->X_WR_)](const ContactSurface<AutoDiffXd>& surface,
        ...
        for (int f = 0; f < mesh_W.num_elements(); ++f) {
            ...
            EXPECT_TRUE(CompareMatrices(math::ExtractValue(tri_n_W), plane_n_W, 2 * kEps));
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(tri_n_W), zeros, 16 * kEps));
        }
    };
...
TEST_F(MeshPlaneDerivativesTest, FaceNormalsWrtOrientation) {
    ...
    const Vector3<AutoDiffXd> p_WN = this->X_WR_ * Vector3<AutoDiffXd>{0.25, -0.3, 0};
    ...
    for (const double theta : {0.0, M_PI / 6, M_PI / 2 * 0.9, M_PI / 2 * 0.99}) {
        ...
        RigidTransform<AutoDiffXd> X_WS{RotationMatrix<AutoDiffXd>(AngleAxis<AutoDiffXd>(theta_ad, v_W)), p_WS};

        auto surface = ComputeContactSurfaceFromSoftVolumeRigidHalfSpace(this->id_S_, *this->field_S_, *this->bvh_S_,
                                                                         X_WS, this->id_R_, this->X_WR_,
        ...
        for (int f = 0; f < mesh_W.num_elements(); ++f) {
            ...
            EXPECT_TRUE(CompareMatrices(math::ExtractValue(tri_n_W), plane_n_W, 5 * kEps));
            /* Confirm the normal gradient w.r.t. theta. */
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(tri_n_W), Vector3d::Zero(), 10 * kEps));
        }
    }
}
...
TEST_F(MeshPlaneDerivativesTest, Pressure) {
    ...
    auto evaluate_pressure = [X_WR = convert_to_double(this->X_WR_), &grad_p_S](
                                     const ContactSurface<AutoDiffXd>& surface, const RigidTransform<AutoDiffXd>& X_WS,
                                     TetPose) {
        ...
        for (int v = 0; v < surface.tri_mesh_W().num_vertices(); ++v) {
            const Matrix3<double> dp_WQ_dp_WSo_W = math::ExtractGradient(surface.tri_mesh_W().vertex(v));
            const Vector3d dp_dp_WSo_W_expected = grad_p_W.transpose() * (dp_WQ_dp_WSo_W - Matrix3<double>::Identity());
