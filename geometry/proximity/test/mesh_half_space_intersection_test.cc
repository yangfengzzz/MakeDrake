#include "geometry/proximity/mesh_half_space_intersection.h"
...
TriangleSurfaceMesh<double> CreateBoxMesh(const RigidTransform<double>& X_FB) {
    ...
    std::vector<Vector3d> vertices_F{{X_FB * Vector3d(-1, -1, -1), X_FB * Vector3d(-1, -1, 1),
                                      X_FB * Vector3d(-1, 1, -1), X_FB * Vector3d(-1, 1, 1), X_FB * Vector3d(1, -1, -1),
                                      X_FB * Vector3d(1, -1, 1), X_FB * Vector3d(1, 1, -1), X_FB * Vector3d(1, 1, 1)}};
...
MeshType RexpressAsContactMesh(const TriangleSurfaceMesh<double>& mesh_F,
    ...
    if constexpr (std::is_same_v<MeshType, TriangleSurfaceMesh<T>>) {
        ...
        for (int t_index = 0; t_index < mesh_F.num_triangles(); ++t_index) {
            ...
            const Vector3<T>& nhat_W = X_WF.rotation() * mesh_F.face_normal(t_index).cast<T>();
...
// vertices.
TriangleSurfaceMesh<double> CreateOneTriangleMesh(const Vector3d& v0, const Vector3d& v1, const Vector3d& v2) {
...
public:
    ...
    std::unordered_map<int, int>& vertices_to_newly_created_vertices() { return vertices_to_newly_created_vertices_; }
    std::unordered_map<SortedPair<int>, int>& edges_to_newly_created_vertices() {
        return edges_to_newly_created_vertices_;
    }
    typename BuilderTypeFromMesh<MeshType>::Type& builder_W() { return builder_W_; }
    ...
    // `fa`.
    bool AreFacesEquivalent(int a, const MeshType& mesh_a, int b, const MeshType& mesh_b) {
        ...
        for (int rotation = 0; rotation < num_vertices; ++rotation) {
            ...
            for (int a_i = 0; a_i < num_vertices; ++a_i) {
                ...
                if ((mesh_a.vertex(fa.vertex(a_i)) - mesh_b.vertex(fb.vertex(b_i))).squaredNorm() > kEps2) {
                    match = false;
                    break;
                }
            }
    ...
    //         ordering).
    ::testing::AssertionResult MeshesEquivalent(const MeshType& test_mesh_W, const MeshType& expected_mesh_W) {
        // Simple checks first.
        if (test_mesh_W.num_elements() != expected_mesh_W.num_elements()) {
            return ::testing::AssertionFailure() << "Mismatch in face count. Test has " << test_mesh_W.num_elements()
                                                 << " faces; expected " << expected_mesh_W.num_elements();
        }
        if (test_mesh_W.num_vertices() != expected_mesh_W.num_vertices()) {
            return ::testing::AssertionFailure() << "Mismatch in vertex count. Test has " << test_mesh_W.num_vertices()
                                                 << " vertices; expected " << expected_mesh_W.num_vertices();
        }
        ...
        std::iota(unmatched_from_expected.begin(), unmatched_from_expected.end(), 0);
        for (int i = 0; i < face_count; ++i) {
            ...
            if (!found_match) {
                return ::testing::AssertionFailure() << "At least one face in the test mesh failed to match to a "
    ...
    ::testing::AssertionResult FieldConsistent(const MeshType& test_mesh_W,
        ...
        if (std::is_same_v<MeshType, PolygonSurfaceMesh<Scalar>>) {
            ...
            for (int f = 0; f < test_mesh_W.num_elements(); ++f) {
                auto result = CompareMatrices(test_field_W.EvaluateGradient(f), grad_p_W);
        ...
        for (int f = 0; f < test_mesh_W.num_elements(); ++f) {
            ...
            if (error > kPressureEps) {
                const std::string message = fmt::format(
                        ...
                        "  tolerance: {}\n"
                        "  error: {}",
                        f, fmt_eigen(p_WX.transpose()), p_X_expected, p_X_test, kPressureEps, error);
    ...
    // structures.
    void CallConstructTriangleHalfspaceIntersectionPolygon(const TriangleSurfaceMesh<double>& mesh_F,
                                                           const RigidTransform<Scalar> X_WF) {
        ...
        for (int f_index = 0; f_index < mesh_F.num_elements(); ++f_index) {
            ConstructTriangleHalfspaceIntersectionPolygon(
                    mesh_F, f_index, *this->half_space_F_, this->pressure_in_F_, grad_p_W, X_WF, &this->builder_W_,
                    &this->vertices_to_newly_created_vertices_, &this->edges_to_newly_created_vertices_);
        }
    }
...
private:
    void SetUp() {
        ...
        pressure_in_F_ = [grad_p_F = grad_p_F_, point_F](const Vector3<Scalar>& p_VF) {
...
TYPED_TEST_P(MeshHalfSpaceValueTest, NoIntersection) {
    ...
    const TriangleSurfaceMesh<double> mesh_F =
            CreateOneTriangleMesh(Vector3d(0, 0, 3), Vector3d(1, 0, 3), Vector3d(0, 1, 3));
...
TYPED_TEST_P(MeshHalfSpaceValueTest, InsideOrOnIntersection) {
    ...
    const RigidTransform<T> X_WF(RotationMatrix<T>(AngleAxis<T>{M_PI / 7, Vector3<T>{1, 2, 3}.normalized()}),
    ...
    // Case: The triangular mesh lies *completely* inside the half space.
    {
        ...
        // they'll lie inside.
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 1), Vector3d(4, 5, 1), Vector3d(3, 6, 1));
    ...
    // Case: triangle lies on the boundary plane.
    {
        ...
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 2), Vector3d(4, 5, 2), Vector3d(3, 6, 2));
    ...
    // space.
    {
        std::vector<Vector3d> vertices = {Vector3d(4, 5, 2), Vector3d(3, 5, 2), Vector3d(3, 5, 1), Vector3d(2, 5, 2)};
        std::vector<SurfaceTriangle> faces{{0, 1, 2}, {2, 1, 3}};
        const TriangleSurfaceMesh<double> tri_mesh_F(std::move(faces), std::move(vertices));
...
TYPED_TEST_P(MeshHalfSpaceValueTest, VertexOnHalfspaceIntersection) {
    ...
    {
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 2), Vector3d(4, 5, 3), Vector3d(3, 6, 3));
        ...
        for (int v = 0; v < mesh_W->num_vertices(); ++v) {
            EXPECT_NEAR(ExtractDoubleOrThrow((mesh_W->vertex(v) - p_WR).norm()), 0, 1e-15);
    ...
    {
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 2), Vector3d(4, 5, 1), Vector3d(3, 6, 1));
...
TYPED_TEST_P(MeshHalfSpaceValueTest, EdgeOnHalfspaceIntersection) {
    ...
    {
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 3), Vector3d(2, 5, 2), Vector3d(4, 5, 2));
        ...
        for (int v = 0; v < mesh_W->num_vertices() - 1; ++v) {
            const double min_dist =
                    ExtractDoubleOrThrow(min((mesh_W->vertex(v) - p_WV1).norm(), (mesh_W->vertex(v) - p_WV2).norm()));
    ...
    {
        const TriangleSurfaceMesh<double> tri_mesh_F =
                CreateOneTriangleMesh(Vector3d(3, 5, 1), Vector3d(4, 5, 2), Vector3d(3, 6, 2));
...
TYPED_TEST_P(MeshHalfSpaceValueTest, QuadrilateralResults) {
    ...
    const TriangleSurfaceMesh<double> tri_mesh_F =
            CreateOneTriangleMesh(Vector3d(3, 5, 3), Vector3d(2, 5, 1), Vector3d(4, 5, 1));
...

    {
        ...
        std::vector<Vector3<T>> expected_vertices_W = {X_WF * Vector3<T>(2, 5, 1),     // b
        ...
        if constexpr (std::is_same_v<MeshType, TriangleSurfaceMesh<T>>) {
            ...
            const Vector3<T> nhat_W = X_WF.rotation() * tri_mesh_F.face_normal(0).cast<T>();
            std::vector<SurfaceTriangle> expected_faces;
            AddPolygonToTriangleMeshData(polygon, nhat_W, &expected_faces, &expected_vertices_W);
            expected_mesh_W = make_unique<MeshType>(std::move(expected_faces), std::move(expected_vertices_W));
        } else {
            std::vector<int> face_data;
            AddPolygonToPolygonMeshData(polygon, &face_data);
            expected_mesh_W = make_unique<MeshType>(std::move(face_data), std::move(expected_vertices_W));
        }
...
TYPED_TEST_P(MeshHalfSpaceValueTest, OutsideInsideOn) {
    ...
    const TriangleSurfaceMesh<double> tri_mesh_F =
            CreateOneTriangleMesh(Vector3d(3, 5, 3), Vector3d(2, 5, 1), Vector3d(3.5, 5, 2));
...
TYPED_TEST_P(MeshHalfSpaceValueTest, OneInsideTwoOutside) {
    ...
    const TriangleSurfaceMesh<double> tri_mesh_F =
            CreateOneTriangleMesh(Vector3d(3, 5, 3), Vector3d(4, 5, 3), Vector3d(3, 6, 1));
    ...
    std::vector<Vector3<T>> expected_vertices_W = {X_WF * Vector3<T>(3, 6, 1), X_WF * Vector3<T>(3, 5.5, 2),
...
TYPED_TEST_P(MeshHalfSpaceValueTest, ComputeContactSurfaceInvocation) {
    ...
    static_assert(std::is_same_v<MeshType, TriangleSurfaceMesh<T>> || std::is_same_v<MeshType, PolygonSurfaceMesh<T>>,
    ...
    RigidTransform<T> X_FB(math::RollPitchYaw<T>(M_PI_4, M_PI_4, M_PI_4), Vector3<T>(1.0, 2.0, 3.0));
    ...
    auto pressure_in_F = [grad_p_F, p_F0 = X_FB.translation()](const Vector3<T>& p_FV) {
    ...
    // Case no intersections returns nullptr.
    {
        ...
        EXPECT_EQ(ComputeContactSurface<MeshBuilder>(mesh_id, tri_mesh_F, half_space_id, half_space_F, pressure_in_F,
                                                     grad_p_W, {8, 9}, X_WF),
                  nullptr);
    ...
    // Case: Plane intersects the mesh.
    {
        ...
        const auto& surface = ComputeContactSurface<MeshBuilder>(mesh_id, tri_mesh_F, half_space_id, half_space_F,
                                                                 pressure_in_F, grad_p_W, tri_indices, X_WF);
        ...
        if (surface->id_M() == half_space_id) {
            ...
            GTEST_FAIL() << "The half space id was not referenced in the contact surface.";
        }
    }
}

REGISTER_TYPED_TEST_SUITE_P(MeshHalfSpaceValueTest,
                            NoIntersection,
                            InsideOrOnIntersection,
                            VertexOnHalfspaceIntersection,
                            EdgeOnHalfspaceIntersection,
                            QuadrilateralResults,
                            OutsideInsideOn,
                            OneInsideTwoOutside,
...
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, DoubleValued) {
    ...
    const RigidTransform<double> X_WF(
            RotationMatrix<double>(AngleAxis<double>{M_PI / 7, Vector3<double>{1, 2, 3}.normalized()}),
            Vector3<double>{-0.25, 0.5, 0.75});

    // Orient and position the mesh arbitrarily in Frame F.
    const RigidTransform<double> X_FB(math::RollPitchYaw<double>(M_PI_4, M_PI_4, M_PI_4),
...

    {
        ...
        EXPECT_EQ(ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F,
    ...
    const RigidTransform<double> X_BH{AngleAxis<double>{M_PI * 0.027, Vector3d{1, 1, 0}.normalized()},
    ...
    // Case: Request triangle surface mesh.
    {
        const std::unique_ptr<ContactSurface<double>> contact_surface =
                ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F,
                                                                X_WF, HydroelasticContactRepresentation::kTriangle);
        ASSERT_NE(contact_surface, nullptr);
        ASSERT_EQ(contact_surface->representation(), HydroelasticContactRepresentation::kTriangle);
    }

    // Case: Request polygon surface mesh.
    {
        const std::unique_ptr<ContactSurface<double>> contact_surface = ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
                hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F, X_WF, HydroelasticContactRepresentation::kPolygon);
        ASSERT_NE(contact_surface, nullptr);
        ASSERT_EQ(contact_surface->representation(), HydroelasticContactRepresentation::kPolygon);
    }
}
...
GTEST_TEST(ComputeContactSurfaceFromSoftHalfSpaceRigidMeshTest, BackfaceCull) {
    ...
    const RigidTransform<double> X_WH = HalfSpace::MakePose(normal_W, Vector3<double>{0, 0, -30});
    const double pressure_scale{1.5};

    const std::unique_ptr<ContactSurface<double>> contact_surface = ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(
            hs_id, X_WH, pressure_scale, mesh_id, mesh_F, bvh_F, X_WF, HydroelasticContactRepresentation::kTriangle);
    ...
    for (int tri = 0; tri < mesh_W.num_elements(); ++tri) {
        ...
        ASSERT_TRUE(IsFaceNormalInNormalDirection(grad_p_W, mesh_W, tri, {})) << "Face " << tri;
    }
...
typedef ::testing::Types<TriangleSurfaceMesh<double>,
                         TriangleSurfaceMesh<AutoDiffXd>,
                         PolygonSurfaceMesh<double>,
...
protected:
    void SetUp() override {
        ...
        X_WS_ = HalfSpace::MakePose(Vector3d{1, 2, 3}.normalized(), Vector3d{0.25, 0.1, -0.2}).cast<AutoDiffXd>();

        /* Set up the *rigid* mesh. */
        vector<Vector3d> vertices{Vector3d{0, 0, 0}, Vector3d{1, 0, 0}, Vector3d{0, 0, -1}};
        using Face = SurfaceTriangle;
        vector<Face> faces{{Face{0, 1, 2}}};
        id_R_ = GeometryId::get_new_id();
        mesh_R_ = make_unique<TriangleSurfaceMesh<double>>(std::move(faces), std::move(vertices));
    ...
    void TestPositionDerivative(
            const std::function<void(const ContactSurface<AutoDiffXd>&, const RigidTransform<AutoDiffXd>&, TriPose)>&
                    evaluate_quantity) {
        ...
        const Vector3d p_SN{0.25, -0.3, 0};
        {
            ...
            configurations.push_back({"Horizontal slice", p_SR_d, RotationMatrixd{}, 3, kHorizontalSlice});
        }

        {
            ...
            const RotationMatrixd R_SR_d =
                    RotationMatrixd::MakeXRotation(-M_PI / 7) * RotationMatrixd::MakeYRotation(-M_PI / 6);
            ...
            configurations.push_back({"Skewed triangle", p_SR_d, R_SR_d, 3, kSkewTriangle});
        }

        {
             ...
             */
            const RotationMatrixd R_SR_d =
                    RotationMatrixd::MakeXRotation(-M_PI / 7) * RotationMatrixd::MakeYRotation(M_PI / 4.5);
        ...
        for (const auto& config : configurations) {
            ...
            ASSERT_EQ(surface->tri_mesh_W().num_triangles(), config.expected_num_faces);
    ...
    Vector3d GetEdgeDirInR(const Vector3d& p_RE) const {
        ...
        throw std::logic_error("Querying for point I that isn't actually on a triangle edge");
    }
...
TEST_F(MeshHalfSpaceDerivativesTest, Area) {
    ...
    auto evaluate_area = [X_WS = convert_to_double(this->X_WS_)](const ContactSurface<AutoDiffXd>& surface,
      ...
      result <<     0, -v(2),  v(1),
        ...
        /* The normal for the contact surface is simply Ry (here, expressed in
         world). */
        const Vector3d n_W = convert_to_double(Vector3<AutoDiffXd>(X_WR_ad.rotation().col(1)));
        ...
        EXPECT_TRUE(CompareMatrices(total_area.derivatives(), dArea_dRo_expected, 4 * kEps));
    };
...
TEST_F(MeshHalfSpaceDerivativesTest, VertexPosition) {
    ...
    auto evalute_position = [this](const ContactSurface<AutoDiffXd>& surface, const RigidTransform<AutoDiffXd>& X_WR_ad,
                                   TriPose pose) {
      ...
      expected_J_S << 1,  0, -e_S.x() / e_S.z(),
                      ...
                      0,  0,  0;
            // clang-format on
            const Matrix3<double> expected_J_W = R_WS * (expected_J_S * R_SW.matrix());
            const Matrix3<double> J_W = math::ExtractGradient(p_WV_ad);
...
TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtPosition) {
    /* None of the face normals depend on Ro. They should all report the zero
     matrix. */
    auto evaluate_normals = [](const ContactSurface<AutoDiffXd>& surface, const RigidTransform<AutoDiffXd>&, TriPose) {
        constexpr double kEps = std::numeric_limits<double>::epsilon();
        ...
        for (int t = 0; t < surface.tri_mesh_W().num_elements(); ++t) {
            ...
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(n_W), zero_matrix, 32 * kEps));
        }
    };
...
TEST_F(MeshHalfSpaceDerivativesTest, FaceNormalsWrtOrientation) {
    ...
    for (const double theta : {0.0, M_PI / 6, M_PI / 2 * 0.9, M_PI / 2 * 0.99}) {
        ...
        const Vector3<AutoDiffXd> p_WR = this->X_WS_ * Vector3<AutoDiffXd>{0.25, -0.7, 0.1};
        ...
        auto surface =
                ComputeContactSurfaceFromSoftHalfSpaceRigidMesh(id_S_, X_WS_, pressure_scale_, id_R_, *mesh_R_, *bvh_R_,
                                                                X_WR, HydroelasticContactRepresentation::kTriangle);
        ...
        for (int t = 0; t < surface->tri_mesh_W().num_elements(); ++t) {
            ...
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(n), expected_deriv, theta > 1.3 ? 1e-13 : 5e-15));
        }
    }
}
...
TEST_F(MeshHalfSpaceDerivativesTest, Pressure) {
    ...
    auto evaluate_pressures = [E = this->pressure_scale_, X_WS = convert_to_double(this->X_WS_)](
                                      const ContactSurface<AutoDiffXd>& surface, const RigidTransform<AutoDiffXd>&,
                                      double theta) {
        constexpr double kEps = std::numeric_limits<double>::epsilon();
        ...
        for (int v = 0; v < surface.tri_mesh_W().num_vertices(); ++v) {
            ...
            EXPECT_TRUE(CompareMatrices(p.derivatives(), expected_dp_dRo, 3 * E * kEps));
        }
    };
