...
private:
    ...
    vector<int> CanonicalizeVertices(const PolyMesh& mesh) {
        ...
        std::sort(canon_to_mesh.begin(), canon_to_mesh.end(), [&mesh](int a, int b) {
            const Vector3d& va = mesh.vertex(a);
            const Vector3d& vb = mesh.vertex(b);
            return std::lexicographical_compare(va.data(), va.data() + 3, vb.data(), vb.data() + 3);
        });
    ...
    void CanonicalizeFaces(const PolyMesh& mesh, const vector<int> vertex_mesh_to_canon) {
...
void MeshesAreEquivalent(const CanonicalMesh& dut, const CanonicalMesh& expected, double tolerance) {
    ...
    ASSERT_THAT(dut.vertices(), testing::Pointwise(NearVertex(tolerance), expected.vertices()));
...
GTEST_TEST(CanonicalMeshTest, Constructor) {
    ...
    const vector<vector<int>> canon_faces{{0, 1, 2}, {0, 2, 3}, {0, 3, 1}, {1, 3, 2}};
    ...
    /* A similar table for perturbed vertex indices. We don't have to worry about
     rotating or ordering the faces.
       ...
       | f2  | v0 v2 v1 | 3 1 0  |
       | f3  | v0 v3 v2 | 3 2 1  | */
    const vector<vector<int>> perturbed_faces{{0, 1, 2}, {3, 0, 2}, {3, 1, 0}, {3, 2, 1}};
    ...
    const CanonicalMesh dut(PolygonSurfaceMesh<double>(perturbed_face_data, perturbed_vertices));
...
void MeshesAreEquivalent(const PolyMesh& dut_mesh, const PolyMesh& expected_mesh, double tolerance) {
...
GTEST_TEST(MakeConvexHullMeshTest, MeshIsHull) {
    ...
    const PolyMesh dut = MakeConvexHull(FindResourceOrThrow("drake/geometry/render/test/meshes/box.obj"), scale);
...
GTEST_TEST(MakeConvexHullMeshTest, HullIsSubset) {
    ...
    const PolyMesh dut = MakeConvexHull(FindResourceOrThrow("drake/geometry/test/cube_with_hole.obj"), scale);
...
GTEST_TEST(MakeConvexHullMeshTest, DisjointMesh) {
    ...
    const PolyMesh dut = MakeConvexHull(FindResourceOrThrow("drake/geometry/test/cube_corners.obj"), scale);
...
GTEST_TEST(MakeConvexHullMeshTest, VolumeMesh) {
  ...
  const PolyMesh expected({
    ...
    const PolyMesh dut = MakeConvexHull(FindResourceOrThrow("drake/geometry/test/one_tetrahedron.vtk"), scale);
...
GTEST_TEST(MakeConvexHullMeshTest, GltfMesh) {
    ...
    const fs::path bin_source = FindResourceOrThrow("drake/geometry/test/cube_with_hole.bin");
    const fs::path bin_target = dir_path / bin_source.filename();
    fs::copy_file(bin_source, bin_target);

    const fs::path gltf_source = FindResourceOrThrow("drake/geometry/test/cube_with_hole.gltf");
...
GTEST_TEST(MakeConvexHullMeshTest, DegenerateMeshes) {
  ...
  f 1 1 2
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeConvexHull(too_few_obj, 1.0),
  ...
  f 1 2 3
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeConvexHull(coincident_obj, 1.0),
  ...
  f 1 2 3
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeConvexHull(colinear_obj, 1.0),
