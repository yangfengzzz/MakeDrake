#include "geometry/proximity/mesh_to_vtk.h"
...
GTEST_TEST(MeshToVtkTest, BoxTetrahedra) {
    ...
    WriteVolumeMeshToVtk(temp_directory() + "/" + "box_tet.vtk", mesh, "Tetrahedral Mesh of Box");
}
...
GTEST_TEST(MeshToVtkTest, BoxTriangles) {
    ...
    WriteSurfaceMeshToVtk(temp_directory() + "/" + "box_tri.vtk", mesh, "Triangular Mesh of Box");
}
...
GTEST_TEST(MeshToVtkTest, BoxTetrahedraPressureField) {
    ...
    const auto pressure = MakeBoxPressureField<double>(box, &mesh, kElasticModulus);
    WriteVolumeMeshFieldLinearToVtk(temp_directory() + "/" + "box_tet_pressure.vtk ", "Pressure[Pa]", pressure,
...
unique_ptr<ContactSurface<double>> BoxContactSurface() {
    ...
    // Very coarse resolution_hint 4.0 should give the coarsest mesh.
    const TriangleSurfaceMesh<double> surface_R = MakeBoxSurfaceMesh<double>(rigid_box, 4.0);
    ...
    return ComputeContactSurfaceFromSoftVolumeRigidSurface(GeometryId::get_new_id(), field_S, bvh_volume_S, X_WS,
...
GTEST_TEST(MeshToVtkTest, BoxContactSurfacePressure) {
    ...
    auto contact_pressure = dynamic_cast<const TriangleSurfaceMeshFieldLinear<double, double>*>(&contact->tri_e_MN());
    ASSERT_NE(contact_pressure, nullptr);
    WriteTriangleSurfaceMeshFieldLinearToVtk(temp_directory() + "/" + "box_rigid_soft_contact_pressure.vtk",
