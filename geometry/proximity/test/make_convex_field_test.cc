...
void CheckMinMaxBoundaryValue(const VolumeMeshFieldLinear<double, double>& pressure_field,
    ...
    std::vector<int> boundary_vertex_indices =
            CollectUniqueVertices(IdentifyBoundaryFaces(pressure_field.mesh().tetrahedra()));
...
INSTANTIATE_TEST_SUITE_P(ConvexField,
                         MakeConvexFieldTest,
                         testing::Values("drake/geometry/test/quad_cube.obj", "drake/geometry/test/convex.obj"));
...
TEST_P(MakeConvexFieldTest, CheckMinMaxBoundaryValue) {
    ...
    VolumeMesh<double> volume_mesh = MakeConvexVolumeMesh<double>(Convex(mesh_file));
