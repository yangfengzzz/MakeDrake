...
public:
    ...
    Vector3<T> CalcGradBarycentric(int e, int i) const { return mesh_.CalcGradBarycentric(e, i); }
...
template <typename T>
std::unique_ptr<VolumeMesh<T>> TestVolumeMesh(const RigidTransform<T> X_WM = RigidTransform<T>::Identity(),
                                              double scale = 1.0) {
    ...
    const Vector3<T> vertex_data[5] = {Vector3<T>::Zero(), Vector3<T>::UnitX(), Vector3<T>::UnitY(),
                                       Vector3<T>::UnitZ(), -Vector3<T>::UnitZ()};
    std::vector<Vector3<T>> vertices_W;
    for (int v = 0; v < 5; ++v) {
        vertices_W.emplace_back(X_WM * vertex_data[v] * scale);
    }
    auto volume_mesh_W = std::make_unique<VolumeMesh<T>>(std::move(elements), std::move(vertices_W));
    EXPECT_EQ(2, volume_mesh_W->num_elements());
    EXPECT_EQ(5, volume_mesh_W->num_vertices());
    for (int v = 0; v < 5; ++v) EXPECT_EQ(X_WM * vertex_data[v] * scale, volume_mesh_W->vertex(v));
    for (int e = 0; e < 2; ++e)
        for (int v = 0; v < 4; ++v) EXPECT_EQ(element_data[e][v], volume_mesh_W->element(e).vertex(v));
...
GTEST_TEST(VolumeMeshTest, TestDegenerateVolumeMeshDouble) {
    auto volume_mesh = TestVolumeMesh<double>(RigidTransform<double>::Identity(), 1e-8);
    std::array<double, 4> dummy_values{1, 2, 3, 4};
    DRAKE_EXPECT_THROWS_MESSAGE(volume_mesh->CalcGradientVectorOfLinearField(dummy_values, 0),
                                ".*not calculate gradient.*");
    auto result = volume_mesh->MaybeCalcGradientVectorOfLinearField(dummy_values, 0);
...
void TestVolumeMeshEqual() {
    ...
    {
        const auto mesh_translate = TestVolumeMesh<T>(RigidTransform<T>(Vector3<T>(1., 1., 1.)));
    ...
    tetrahedra[0] = VolumeElement(tetrahedra[0].vertex(1),
        ...
        // clang-format on
        VolumeMesh<T> mesh_different_tetrahedra(std::move(tetrahedra), std::move(vertices_copy));
...
void TestCalcTetrahedronVolume() {
    const RigidTransform<T> X_WM(RollPitchYaw<T>(M_PI / 6.0, 2.0 * M_PI / 3.0, 7.0 * M_PI / 4.0),
    ...
    for (int e = 0; e < 2; ++e) {
        const double tetrahedron_volume = ExtractDoubleOrThrow(volume_mesh->CalcTetrahedronVolume(e));
...
void TestCalcBarycentric() {
    ...
    // At the centroid of the tetrahedral element v0v1v2v3.
    {
        ...
        const Barycentric barycentric = volume_mesh->CalcBarycentric(X_WM * p_M, element);
...
void TestCalcGradBarycentric() {
    ...
    // test more realistic.
    const RigidTransform<T> X_WM(RollPitchYaw<T>(M_PI / 6.0, 2.0 * M_PI / 3.0, 5.0 * M_PI / 4.0),
                                 Vector3<T>(1.0, 2.0, 3.0));

    // Create the mesh with vertex coordinates expressed in World frame to make
    // the test more realistic.
    const VolumeMesh<T> mesh_W({VolumeElement(tetrahedron)}, {Vector3<T>(X_WM * v0_M), Vector3<T>(X_WM * v1_M),
    ...
    EXPECT_TRUE(CompareMatrices(gradb0_W + gradb1_W + gradb2_W + gradb3_W, Vector3<T>::Zero(), 1e-14));
}
...
void TestTransformVertices() {
    ...
    EXPECT_TRUE(volume_mesh_W->Equal(*volume_mesh_W_expected, 4.0 * std::numeric_limits<T>::epsilon()));
}
...
std::unique_ptr<VolumeMeshFieldLinear<T, T>> TestVolumeMeshFieldLinear() {
    ...
    auto volume_mesh_field = std::make_unique<VolumeMeshFieldLinear<T, T>>(std::move(f_values), volume_mesh.get());
    ...
    Vector3<T> expect_gradp_M = volume_mesh->CalcGradientVectorOfLinearField(std::array<T, 4>{f0, f1, f2, f3}, e0);
...
protected:
    void SetUp() override {
        ...
        mesh_ad_ = std::make_unique<VolumeMesh<AutoDiffXd>>(std::move(elements), std::move(vertices));
    }
...
TEST_F(ScalarMixingTest, CalcGradientVectorOfLinearField) {
...

    {
        ...
        const Vector3<double> grad_W = mesh_d_->CalcGradientVectorOfLinearField(field_d, e0_);
...

    {
        ...
        // result.
        const Vector3<AutoDiffXd> grad_W = mesh_d_->CalcGradientVectorOfLinearField(field_ad, e0_);
        EXPECT_EQ(grad_W(0).derivatives().size(), 3);
        EXPECT_EQ(grad_W(1).derivatives().size(), 3);
        EXPECT_EQ(grad_W(2).derivatives().size(), 3);
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(grad_W), grad_W_expected, kEps));
    }

    {
        // AutoDiffXd-valued mesh with double-valued field on triangle *with*
        // derivatives: AutodDiffXd-valued result *with* derivatives.
        const Vector3<AutoDiffXd> grad_W1 = mesh_ad_->CalcGradientVectorOfLinearField(field_d, e0_);
        EXPECT_EQ(grad_W1(0).derivatives().size(), 3);
        EXPECT_EQ(grad_W1(1).derivatives().size(), 3);
        EXPECT_EQ(grad_W1(2).derivatives().size(), 3);
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(grad_W1), grad_W_expected, kEps));

        // AutoDiffXd-valued mesh with double-valued field on triangle *without*
        // derivatives: AutodDiffXd-valued result *without* derivatives.
        const Vector3<AutoDiffXd> grad_W2 = mesh_ad_->CalcGradientVectorOfLinearField(field_d, e1_);
...

    {
        ...
        // AutodDiffXd-valued.
        const Vector3<AutoDiffXd> grad_W = mesh_ad_->CalcGradientVectorOfLinearField(field_ad, e0_);
        EXPECT_EQ(grad_W(0).derivatives().size(), 3);
        EXPECT_EQ(grad_W(1).derivatives().size(), 3);
        EXPECT_EQ(grad_W(2).derivatives().size(), 3);
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(grad_W), grad_W_expected, kEps));
