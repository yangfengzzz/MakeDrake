...
class BvhUpdater {
public:
    static void SetBounds(Aabb* aabb, const Vector3d& lower, const Vector3d& upper) { aabb->set_bounds(lower, upper); }
};
...
GTEST_TEST(AabbTest, Construction) {
    ...
    const double expected_volume = 2 * half_size.x() * 2 * half_size.y() * 2 * half_size.z();
...
GTEST_TEST(AabbTest, BoxOverlap) {
    ...
    for (double distance : {-0.001, 0.001}) {
        ...
        EXPECT_EQ(Aabb::HasOverlap(Aabb(X_WA.translation(), half_sizeA), Aabb(X_WB.translation(), half_sizeB),
...

        {
            ...
            const RigidTransformd X_GA{Vector3d{-0.5, 3, 1.5}};
            const RigidTransformd X_WG = X_WA * X_GA.inverse();
            const RigidTransformd X_HB{AngleAxisd{M_PI / 7, Vector3d{1, -2, 3}.normalized()}, Vector3d{-1, 1, -1}};
            const RigidTransformd X_WH = X_WB * X_HB.inverse();
...
GTEST_TEST(AabbMakerTest, Compute) {
...

    {
        ...
        const Aabb::Maker<TriangleSurfaceMesh<double>> maker(mesh, fit_vertices);
        const Aabb aabb = maker.Compute();
        EXPECT_TRUE(CompareMatrices(aabb.center(), Vector3d{0, 1, 1}.cwiseProduct(half_size)));
        EXPECT_TRUE(CompareMatrices(aabb.half_width(), Vector3d{1, 0, 0}.cwiseProduct(half_size)));
    }

    {
        ...
        const Aabb::Maker<TriangleSurfaceMesh<double>> maker(mesh, fit_vertices);
        const Aabb aabb = maker.Compute();
        EXPECT_TRUE(CompareMatrices(aabb.center(), Vector3d{1, 0, 0}.cwiseProduct(half_size)));
        EXPECT_TRUE(CompareMatrices(aabb.half_width(), Vector3d{0, 1, 1}.cwiseProduct(half_size)));
    }
