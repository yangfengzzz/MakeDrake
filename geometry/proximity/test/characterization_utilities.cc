#include "geometry/proximity/test/characterization_utilities.h"
...
QueryInstance::QueryInstance(GeometryType shape1_in, GeometryType shape2_in, double error_in)
    : shape1(shape1_in), shape2(shape2_in), error(error_in), outcome(kSupported) {
    DRAKE_DEMAND(error >= 0);
}

QueryInstance::QueryInstance(GeometryType shape1_in, GeometryType shape2_in, Outcome outcome_in)
...
std::string QueryInstanceName(const testing::TestParamInfo<QueryInstance>& info) {
    return fmt::format("{}Vs{}", info.param.shape1, info.param.shape2);
}

template <typename T>
ShapeConfigurations<T>::ShapeConfigurations(const Shape& shape, double distance) : ShapeReifier(), distance_(distance) {
    shape.Reify(this);
}
...
void ShapeConfigurations<T>::ImplementGeometry(const Box& box, void*) {
    ...
    configs_ = vector<ShapeTangentPlane<T>>{
            {half_size.cwiseProduct(Vector3d{1, -1, 1}), Vector3d{0.5, -2, 1}.normalized(), 0, "+x/-y/+z vertex"},
            {half_size.cwiseProduct(Vector3d{1, -1, 0.75}), Vector3d{2, -2, 0}.normalized(), 0,
             "+x/-y edge parallel to z axis"},
            {-half_size + Vector3d{edge_distance, edge_distance, 0}, Vector3d{0, 0, -1}, edge_distance,
             "-z face near the -x/-y vertex"}};
}
...
void ShapeConfigurations<T>::ImplementGeometry(const Capsule& capsule, void*) {
    ...
    const Vector3<T> p_CB{n_barrel_C[0] * capsule.radius(), n_barrel_C[1] * capsule.radius(), capsule.length() * 0.3};

    const Vector3<T> n_sphere_C = Vector3<T>{2, 0.3, 1.1}.normalized();
    const Vector3<T> p_CS = Vector3<T>{0, 0, capsule.length() / 2} + n_sphere_C * capsule.radius();
    configs_ = vector<ShapeTangentPlane<T>>{{p_CB, n_barrel_C, capsule.radius(), "Capsule barrel"},
...
template <typename T>
void ShapeConfigurations<T>::ImplementGeometry(const Cylinder& cylinder, void*) {
    ...
    const Vector3<T> p_CE =
            Vector3<T>{1, 1.5, 0}.normalized() * cylinder.radius() + Vector3<T>{0, 0, cylinder.length() / 2};
    configs_.push_back({p_CE, Vector3<T>{1, 1.5, 1}.normalized(), 0, "cylinder's +z edge"});
    ...
    const Vector3<T> p_AB = n_A * cylinder.radius() + Vector3<T>{0, 0, -cylinder.length() / 2 + distance_to_edge};
    configs_.push_back({p_AB, n_A, cylinder.radius(), "the bottom half of cylinder's barrel"});
}

template <typename T>
void ShapeConfigurations<T>::ImplementGeometry(const Ellipsoid& ellipsoid, void*) {
    const double min_axis = std::min({ellipsoid.a(), ellipsoid.b(), ellipsoid.c()});
    if (distance_ < 0) {
        const double depth = -distance_;
        if (depth > min_axis) {
            throw std::runtime_error(
    ...
    auto uv_sample = [&ellipsoid](double u, double v) {
        ...
        // scale factor of 2.
        return std::make_pair(Vector3d{x, y, z}, Vector3d{x / a / a, y / b / b, z / c / c}.normalized());
    };

    const auto& [p1, n1] = uv_sample(M_PI / 3, M_PI / 7);
    const auto& [p2, n2] = uv_sample(13 * M_PI / 7, 5 * M_PI / 6);
    configs_ = vector<ShapeTangentPlane<T>>{{p1, n1, min_axis, "ellipsoid's -x/+y/+z octant"},
                                            {p2, n2, min_axis, "ellipsoid's +x/-y/+z octant"}};
}

template <typename T>
void ShapeConfigurations<T>::ImplementGeometry(const HalfSpace&, void*) {
    configs_ = vector<ShapeTangentPlane<T>>{{Vector3<T>{0.3, 0.7, 0}, Vector3<T>{0, 0, 1},
...
void ShapeConfigurations<T>::ImplementGeometry(const Sphere& sphere, void*) {
    ...
    configs_ = vector<ShapeTangentPlane<T>>{{p1, n1, r, "sphere's -x/+y/+z octant"},
...
void MakeFclShape::ImplementGeometry(const Cylinder& cylinder, void*) {
    object_ = std::make_shared<fcl::Cylinderd>(cylinder.radius(), cylinder.length());
}

void MakeFclShape::ImplementGeometry(const Ellipsoid& ellipsoid, void*) {
    object_ = std::make_shared<fcl::Ellipsoidd>(ellipsoid.a(), ellipsoid.b(), ellipsoid.c());
}
...
::testing::AssertionResult MeshIsConvex() {
    ...
    const std::string obj_path = temp_directory() + "/tri.obj";
    {
        ...
        if (out.bad()) {
            throw std::runtime_error(fmt::format("Unable to write temporary obj file to: {}", obj_path));
        }
    ...
    return ::testing::AssertionFailure() << "A Mesh shape is no longer represented by an fcl::Convex in "
                                         << "ProximityEngine. We need to explicitly characterize Mesh-Shape "
                                         << "queries for all Shape types.";
}

template <typename T>
RigidTransform<T> AlignPlanes(const Vector3<T>& P, const Vector3<T>& m, const Vector3<T>& Q, const Vector3<T>& n) {
    ...
    if (cos_theta < kAlmostOne) {
        ...
        if (cos_theta < -kAlmostOne) {
            /* We need a normal perpendicular to nhat. Extract it from a valid
             basis. */
            const math::RotationMatrix<T> basis = math::RotationMatrix<T>::MakeFromOneVector(nhat, 2);
...
template <typename T>
void CharacterizeResultTest<T>::RunCallback(const QueryInstance& query,
    ...
    switch (query.outcome) {
        ...
        case kIgnores:
            ...
            ASSERT_EQ(callback_->GetNumResults(), query.outcome == kSupported ? 1 : 0);
            break;
        case kThrows:
            DRAKE_ASSERT_THROWS_MESSAGE(callback_->Invoke(obj_A, obj_B, collision_filter, X_WGs),
...
template <typename T>
std::optional<double> CharacterizeResultTest<T>::ComputeErrorMaybe(double expected_distance) const {
    if (callback_->GetNumResults() > 0) {
        const double distance = ExtractDoubleOrThrow(callback_->GetFirstSignedDistance());
        DRAKE_DEMAND(std::abs(expected_distance) > std::numeric_limits<double>::epsilon());
...
template <typename T>
vector<Configuration<T>> CharacterizeResultTest<T>::MakeConfigurations(const Shape& shape_A,
                                                                       const Shape& shape_B,
    ...
    for (const double signed_distance : signed_distances) {
        /* In order to pose the geometries, we start with X_WA = X_WB = I. That
         ...
         reflects this. */
        const vector<ShapeTangentPlane<T>> samples_A = ShapeConfigurations<T>(shape_A, signed_distance).configs();
        const vector<ShapeTangentPlane<T>> samples_B = ShapeConfigurations<T>(shape_B, signed_distance).configs();
        for (const auto& sample_A : samples_A) {
            ...
            for (const auto& sample_B : samples_B) {
                ...
                const std::string relates = signed_distance < 0
                                                    ? " penetrates into "
                                                    : (signed_distance > 0 ? " separated from " : " touching ");
                configs.push_back({AlignPlanes(p_WC, a_norm_W, p_WB, b_norm_W), signed_distance,
...
void CharacterizeResultTest<T>::RunCharacterization(const QueryInstance& query,
    ...
    auto evaluate_callback = [this, &query, &worst_error, &worst_config](
                                     char first, fcl::CollisionObjectd* obj_A, char second,
                                     fcl::CollisionObjectd* obj_B, auto test_config,
                                     const std::unordered_map<GeometryId, RigidTransform<T>>& world_poses) {
        SCOPED_TRACE(
                fmt::format("\n{}-{} (obj {}-obj {}) query:"
                            "\n  {}"
                            ...
                            "\n  X_AB"
                            "\n{}\n",
                            GetGeometryName(*obj_A), GetGeometryName(*obj_B), first, second, test_config.description,
                            test_config.signed_distance, fmt_eigen(test_config.X_AB.GetAsMatrix34())));
        RunCallback(query, obj_A, obj_B, &collision_filter_, &world_poses);
        const std::optional<double> error = ComputeErrorMaybe(test_config.signed_distance);
        if (error.has_value() && (!worst_error.has_value() || *error > *worst_error)) {
            worst_error = error;
    ...
    for (const auto& X_WA : X_WAs) {
        for (const auto& config : configs) {
            const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{{id_A, X_WA}, {id_B, X_WA * config.X_AB}}};
...
template <typename T>
void CharacterizeResultTest<T>::RunCharacterization(const QueryInstance& query, bool is_symmetric) {
    auto shape1 = MakeShape(query.shape1, false /* use_alt */);
    auto shape2 = MakeShape(query.shape2, query.shape1 == query.shape2);
    RunCharacterization(query, *shape1, *shape2, MakeConfigurations(*shape1, *shape2, TestDistances()), is_symmetric);
}
...
vector<RigidTransform<T>> CharacterizeResultTest<T>::X_WAs() {
    return vector<RigidTransform<T>>{
            RigidTransform<T>{math::RotationMatrix<T>(AngleAxis<T>(5 * M_PI / 9, Vector3<T>{-1, -1, 1}.normalized())),
                              Vector3<T>{-0.2, 0.15, -0.3}},
            RigidTransform<T>{math::RotationMatrix<T>(AngleAxis<T>(22 * M_PI / 9, Vector3<T>{-1, 1, -1}.normalized())),
                              Vector3<T>{-10.0, 20.2, -12.3}}};
}

template <typename T>
std::unique_ptr<Shape> CharacterizeResultTest<T>::MakeShape(GeometryType shape, bool use_alt) {
    switch (shape) {
        case kBox:
...
Mesh CharacterizeResultTest<T>::mesh(bool) {
    throw std::logic_error("Mesh will be supported when it is no longer represented as a Convex");
}
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&AlignPlanes<T>));
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::internal::ShapeConfigurations);
