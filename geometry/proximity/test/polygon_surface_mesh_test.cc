...
protected:
    ...
    void AddPolygon(const Polygon& polygon, vector<int>* face_data, vector<Vector3<T>>* vertices) {
    ...
    Polygon MakePentagon(const RigidTransformd& X_WM = {}) {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            vertices.emplace_back(X_WM * Vector3d(0, kR * std::cos(alpha), kR * std::sin(alpha)));
        }
...
TYPED_TEST(PolygonSurfaceMeshTest, DefaultConstructor) {
    ...
    EXPECT_TRUE(CompareMatrices(ExtractDoubleOrThrow(mesh.centroid()), Vector3d::Zero()));
...
TYPED_TEST(PolygonSurfaceMeshTest, FullConstructor) {
    ...
    vector<Polygon> polygons = {this->MakeIsoscelesTriangle(), this->MakeSquare(), this->MakePentagon()};
    ...
    for (int f = 0; f < expected_face_count; ++f) {
        ...
        for (int v = 0; v < v_count; ++v) {
            ASSERT_TRUE(CompareMatrices(mesh.vertex(face.vertex(v)), polygons[f].vertices[v]));
        }
    }
}
...
TYPED_TEST(PolygonSurfaceMeshTest, MeshCentroid) {
    ...
    const RigidTransformd X_WM{AngleAxisd{M_PI / 4, Vector3d(1, 2, 3).normalized()}, Vector3d{1, 2, 3}};
    vector<Polygon> polygons = {this->MakeIsoscelesTriangle(X_WM), this->MakeSquare(X_WM), this->MakePentagon(X_WM)};
    ...
    ASSERT_TRUE(CompareMatrices(mesh.centroid(), scaled_centroid_expected / total_area_expected, 1e-15));
    for (int e = 0; e < mesh.num_elements(); ++e) {
        ASSERT_TRUE(CompareMatrices(mesh.element_centroid(e), polygons[e].centroid, 1e-15));
    }
}
...
TYPED_TEST(PolygonSurfaceMeshTest, Area) {
    ...
    const RigidTransformd X_WM{AngleAxisd{M_PI / 4, Vector3d(1, 2, 3).normalized()}, Vector3d{1, 2, 3}};
    const vector<Polygon> polygons = {this->MakeIsoscelesTriangle(X_WM), this->MakeSquare(X_WM),
    ...
    ASSERT_NEAR(ExtractDoubleOrThrow(mesh.total_area()), expected_total_area, 1e-15);
    for (int e = 0; e < mesh.num_elements(); ++e) {
...
TYPED_TEST(PolygonSurfaceMeshTest, CalcBoundingBox) {
    ...
    Vector3d min_corner = Vector3d::Constant(std::numeric_limits<double>::infinity());
...
TYPED_TEST(PolygonSurfaceMeshTest, FaceNormals) {
    ...
    for (int p = 0; p < static_cast<int>(polygons.size()); ++p) {
        ASSERT_TRUE(CompareMatrices(ExtractDoubleOrThrow(mesh.face_normal(p)), polygons[p].normal, 1e-15));
    }
}
...
TYPED_TEST(PolygonSurfaceMeshTest, ZeroFaceNormal) {
    ...
    ASSERT_TRUE(CompareMatrices(ExtractDoubleOrThrow(mesh.face_normal(0)), poly.normal));
}
...
TYPED_TEST(PolygonSurfaceMeshTest, DegenerateFaceNormal) {
    ...
    vector<vector<int>> polygons{{0, 1, 2, 3}, {1, 2, 3, 0}, {2, 3, 0, 1}, {3, 0, 1, 2}};
    for (const auto& ordering : polygons) {
        ...
        ASSERT_TRUE(CompareMatrices(ExtractDoubleOrThrow(mesh.face_normal(0)), poly.normal));
        ASSERT_TRUE(CompareMatrices(ExtractDoubleOrThrow(mesh.element_centroid(0)), poly.centroid));
...
TYPED_TEST(PolygonSurfaceMeshTest, ReverseFaceWinding) {
    ...
    PolygonSurfaceMesh<T> mesh =
            this->MakeMesh({this->MakeIsoscelesTriangle(), this->MakeSquare(), this->MakePentagon()});
    ...
    for (int p = 0; p < mesh.num_elements(); ++p) {
        ...
        ASSERT_TRUE(CompareMatrices(mesh.face_normal(p), -mesh_copy.face_normal(p)));
    }
}
...
TYPED_TEST(PolygonSurfaceMeshTest, TransformVertices) {
    ...
    const PolygonSurfaceMesh<T> mesh_W =
            this->MakeMesh({this->MakeIsoscelesTriangle(X_WM), this->MakeSquare(X_WM), this->MakePentagon(X_WM)});
    ...
    ASSERT_NEAR(ExtractDoubleOrThrow(mesh_M.total_area()), ExtractDoubleOrThrow(mesh_W.total_area()), 1e-15)
    ...
    for (int p = 0; p < mesh_M.num_elements(); ++p) {
        ASSERT_TRUE(CompareMatrices(mesh_M.face_normal(p), mesh_W.face_normal(p), 1e-15));
        ASSERT_TRUE(CompareMatrices(mesh_M.element_centroid(p), mesh_W.element_centroid(p), 1e-15));
    }
...
TYPED_TEST(PolygonSurfaceMeshTest, CalcGradientVectorOfLinearField) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(mesh_M.CalcGradientVectorOfLinearField(dummy_values, 0),
    ...
    EXPECT_FALSE(mesh_M.MaybeCalcGradientVectorOfLinearField(dummy_values, 0).has_value());
}
...
TYPED_TEST(PolygonSurfaceMeshTest, ComputePositionDependentQuantities) {
    ...
    const std::vector<Vector3<T>> vertices = {// vertices for the triangle
    ...
    const std::vector<Vector3<T>> expected_face_normals = {-Vector3<T>::UnitY(), Vector3<T>::UnitZ()};
    const std::vector<Vector3<T>> expected_face_centroids = {Vector3<T>(2.0 / 3.0, 0.0, 2.0 / 3.0),
                                                             Vector3<T>(1.5, 1.5, 0.0)};
    const Vector3<T> expected_centroid =
            (expected_areas[0] * expected_face_centroids[0] + expected_areas[1] * expected_face_centroids[1]) /
            expected_total_area;

    // Returns true if all vertex position dependent quantities are as expected.
    auto verify_position_dependent_quantities =
            [&mesh](const std::vector<Vector3<T>>& positions, const std::vector<T>& areas, const T& total_area,
                    const std::vector<Vector3<T>>& face_normals, const std::vector<Vector3<T>>& face_centroids,
        ...
        for (int i = 0; i < ssize(face_normals); ++i) {
            if (!CompareMatrices(mesh.face_normal(i), face_normals[i], 4.0 * std::numeric_limits<T>::epsilon())) {
                return false;
            }
        ...
        if (!CompareMatrices(mesh.centroid(), centroid, 4.0 * std::numeric_limits<T>::epsilon())) {
            return false;
        }
        return true;
    };
    ...
    EXPECT_FALSE(verify_position_dependent_quantities(vertices, expected_areas, expected_total_area,
                                                      expected_face_normals, expected_face_centroids,
                                                      expected_centroid));

    mesh.SetAllPositions(ToVectorX(vertices));

    // Now, the vertex position dependent quantities should be updated.
    EXPECT_TRUE(verify_position_dependent_quantities(vertices, expected_areas, expected_total_area,
                                                     expected_face_normals, expected_face_centroids,
                                                     expected_centroid));
}
...
GTEST_TEST(PolygonSurfaceMeshTestCornerCases, ZeroAreaPolygon) {
    ...
    PolygonSurfaceMesh<double> zero_area_mesh_M({4, 0, 1, 2, 3},  // one polygon of 4 vertices
