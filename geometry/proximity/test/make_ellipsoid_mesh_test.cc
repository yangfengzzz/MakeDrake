...
GTEST_TEST(MakeEllipsoidMeshTest, MakeEllipsoidVolumeMesh) {
    ...
    // tetrahedra.
    {
        const auto coarse_mesh =
                MakeEllipsoidVolumeMesh<double>(ellipsoid, 0.16, TessellationStrategy::kDenseInteriorVertices);
    ...
    // the number of tetrahedra by 8X.
    {
        const auto medium_mesh =
                MakeEllipsoidVolumeMesh<double>(ellipsoid, 0.08, TessellationStrategy::kDenseInteriorVertices);
