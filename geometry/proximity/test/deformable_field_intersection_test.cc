...
void Translate(DeformableVolumeMeshWithBvh<double>* deform_W, const Vector3d& displacement_W) {
...
public:
    AddDeformableDeformableContactSurfaceTest()
        : id0_{GeometryId::get_new_id()},
          ...
          deform1_W_{VolumeMesh<double>({{1, 0, 2, 3}},
                                        {Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(), -Vector3d::UnitZ()})} {
        deformable_contact_W_.RegisterDeformableGeometry(id0_, deform0_W_.mesh().num_vertices());
        deformable_contact_W_.RegisterDeformableGeometry(id1_, deform1_W_.mesh().num_vertices());
    }
...
TEST_F(AddDeformableDeformableContactSurfaceTest, NoContact) {
    ...
    VolumeMeshFieldLinear<double, double> sdf1_W{{0, 0, 0, -1}, &deform1_W_.mesh()};

    AddDeformableDeformableContactSurface(sdf1_W, deform1_W_, id1_, signed_distance0_W_, deform0_W_, id0_,
...
TEST_F(AddDeformableDeformableContactSurfaceTest, HaveContact) {
    ...
    {
        EXPECT_EQ(deformable_contact_W_.contact_participation(id0_).CalcVertexPartialPermutation().permutation(),
                  std::vector<int>({0, 1, 2, 3}));
        EXPECT_EQ(deformable_contact_W_.contact_participation(id1_).CalcVertexPartialPermutation().permutation(),
                  std::vector<int>({0, 1, 2, 3}));
    }
    ...
    // Check DeformableContactSurface.
    {
        ...
        const DeformableContactSurface<double>& contact_surface = deformable_contact_W_.contact_surfaces().at(0);
        ...
        EXPECT_EQ(contact_surface.contact_points_W().at(0), contact_surface.contact_mesh_W().element_centroid(0));
        ...
        EXPECT_EQ(contact_surface.barycentric_coordinates_A().at(0),
                  deform0_W_.mesh().CalcBarycentric(contact_surface.contact_points_W().at(0), 0));
        EXPECT_EQ(contact_surface.contact_vertex_indexes_A().at(0), Vector4<int>(0, 1, 2, 3));
        EXPECT_EQ(contact_surface.barycentric_coordinates_B().at(0),
                  deform1_W_.mesh().CalcBarycentric(contact_surface.contact_points_W().at(0), 0));
        EXPECT_EQ(contact_surface.contact_vertex_indexes_B().at(0), Vector4<int>(1, 0, 2, 3));
