#include "geometry/proximity/mesh_intersection.h"
...
public:
    ...
    const std::vector<Vector3<T>>& ClipTriangleByTetrahedron(int element,
                                                             const VolumeMesh<double>& volume_M,
                                                             int face,
                                                             const TriangleSurfaceMesh<double>& surface_N,
                                                             const math::RigidTransform<T>& X_MN) {
        return intersect_.ClipTriangleByTetrahedron(element, volume_M, face, surface_N, X_MN);
    }
    bool IsFaceNormalAlongPressureGradient(const VolumeMeshFieldLinear<double, double>& volume_field_M,
                                           const TriangleSurfaceMesh<double>& surface_N,
                                           const math::RigidTransform<T>& X_MN,
                                           int tet_index,
                                           int tri_index) {
        return intersect_.IsFaceNormalAlongPressureGradient(volume_field_M, surface_N, X_MN, tet_index, tri_index);
    }
...
GTEST_TEST(MeshIntersectionTest, CalcIntersection) {
    ...
    EXPECT_TRUE(CompareMatrices(CalcIntersection(p_HP1, p_HP2, half_space_H), p_HI, kEps));
    // Tests two points are on the same negative side.
    EXPECT_TRUE(CompareMatrices(CalcIntersection(p_HN1, p_HN2, half_space_H), p_HI, kEps));
    // Tests two points are on different sides of the bounding plane.
    EXPECT_TRUE(CompareMatrices(CalcIntersection(p_HP2, p_HN2, half_space_H), p_HI, kEps));
    // Tests one point on the plane while the other point is not.
    EXPECT_TRUE(CompareMatrices(CalcIntersection(p_HI, p_HP2, half_space_H), p_HI, kEps));
}

// TODO(DamrongGuoy): Move the definition of this function here after 11612
//  landed. The definition is currently down below here.
template <typename T>
bool CompareConvexPolygon(const std::vector<Vector3<T>>& polygon0, const std::vector<Vector3<T>>& polygon1);
...
GTEST_TEST(MeshIntersectionTest, ClipPolygonByHalfSpace) {
    ...
    const PosedHalfSpace<double> half_space_H(unit_normal_H, offset * unit_normal_H);
...
unique_ptr<TriangleSurfaceMesh<T>> TrivialSurfaceMesh() {
    ...
    std::vector<Vector3<T>> vertices = {Vector3<T>::Zero(), Vector3<T>::UnitX(), Vector3<T>::UnitY()};
    return std::make_unique<TriangleSurfaceMesh<T>>(std::move(faces), std::move(vertices));
}
...
//
template <typename T>
unique_ptr<VolumeMesh<T>> TrivialVolumeMesh(const math::RigidTransform<T>& X_MN = math::RigidTransform<T>::Identity()) {
    const int element_data[2][4] = {{0, 1, 2, 3}, {0, 2, 1, 4}};
    std::vector<VolumeElement> elements;
    for (const auto& element : element_data) {
        elements.emplace_back(element);
    }
    std::vector<Vector3<T>> vertices = {X_MN * Vector3<T>::Zero(), X_MN * Vector3<T>::UnitX(),
                                        X_MN * Vector3<T>::UnitY(), X_MN * Vector3<T>::UnitZ(),
                                        X_MN * (-Vector3<T>::UnitZ())};
    return std::make_unique<VolumeMesh<T>>(std::move(elements), std::move(vertices));
}

template <typename T>
unique_ptr<VolumeMeshFieldLinear<T, T>> TrivialVolumeMeshField(const VolumeMesh<T>* volume_mesh) {
    ...
    auto volume_mesh_field = std::make_unique<VolumeMeshFieldLinear<T, T>>(std::move(p_values), volume_mesh);
...
bool CompareConvexPolygon(const std::vector<Vector3<T>>& polygon0, const std::vector<Vector3<T>>& polygon1) {
    ...
    auto it = std::find_if(polygon1.begin(), polygon1.end(), matches(polygon0[0]));
    if (it == polygon1.end()) {
        return false;
    }
...
GTEST_TEST(MeshIntersectionTest, ClipTriangleByTetrahedron) {
    ...
    // face of the tetrahedron. Expect the output polygon to be empty.
    {
        ...
        const std::vector<Vector3d> polygon = SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(
    ...
    // face of the tetrahedron. Expect the output polygon to be empty.
    {
        const auto X_MN = RigidTransformd(RollPitchYawd(0, 0, M_PI_2), Vector3d::Zero());
        const std::vector<Vector3d> polygon = SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(
                element0, *volume_M, face, *surface_N, X_MN);
    ...
    //  count" problem.
    {
        ...
        const std::vector<Vector3d> polygon0_M =
                SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(element0, *volume_M, face,
                                                                                        *surface_N, X_MN);
        const std::vector<Vector3d> polygon1_M =
                SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(element1, *volume_M, face,
                                                                                        *surface_N, X_MN);
        // clang-format off
    ...
    // a quad.
    {
        const auto X_MN = RigidTransformd(RollPitchYawd(0, 0, M_PI), Vector3d(0.5, 0.5, 0));
        const std::vector<Vector3d> polygon0_M =
                SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(element0, *volume_M, face,
                                                                                        *surface_N, X_MN);
        // clang-format off
...
GTEST_TEST(MeshIntersectionTest, ClipTriangleByTetrahedronIntoHeptagon) {
    ...
    std::vector<Vector3d> vertices = {
        ...
        // clang-format on
        volume_M = std::make_unique<VolumeMesh<double>>(std::move(elements), std::move(vertices));
    }
    ...
    std::vector<Vector3d> vertices = {
        ...
        {0.,   -1.5, 0.}};
        // clang-format on
        surface_N = std::make_unique<TriangleSurfaceMesh<double>>(std::move(faces), std::move(vertices));
    }
    const int tetrahedron = 0;
    const int triangle = 0;
    const auto X_MN = RigidTransformd::Identity();
    const std::vector<Vector3d> polygon_M = SurfaceVolumeIntersectorTester<MeshBuilder>().ClipTriangleByTetrahedron(
...
GTEST_TEST(MeshIntersectionTest, IsFaceNormalAlongPressureGradient) {
    ...
    RigidTransformd X_MF(RollPitchYawd(M_PI_4, 2. * M_PI / 3., M_PI / 6.), Vector3d(1.1, 2.5, 4.0));
    ...
    for (const TestData& t : test_data) {
        ...
        const auto X_FN = RigidTransformd(RollPitchYawd(t.angle, 0, 0), Vector3d::Zero());
        ...
        EXPECT_EQ(t.expect_result, SurfaceVolumeIntersectorTester<MeshBuilder>().IsFaceNormalAlongPressureGradient(
                                           *volume_field_M, *rigid_N, X_MN, 0 /*tet_index*/, 0 /*tri_index*/));
    }
}
...
int GetTetForFace(const MeshType& surface_S,
                  int f,
    ...
    const Vector3<T> p_SC =
            (surface_S.vertex(face.vertex(0)) + surface_S.vertex(face.vertex(1)) + surface_S.vertex(face.vertex(2))) /
            3;
    ...
    throw std::logic_error(fmt::format("Surface triangle was unable to place triangle {} in any tetrahedron", f));
}
...
protected:
    void SetUp() override {
        ...
        bvh_surface_R_ = make_unique<Bvh<Obb, TriangleSurfaceMesh<double>>>(*surface_R_);
    ...
    void VerifySampleVolumeFieldOnSurface(const typename MeshBuilder::MeshType& surface_S,
        ...
        ASSERT_TRUE(domain_checked[0]) << "Assumptions have been broken! In testing e_field, no "
                                          "vertex was on the z = 0 plane.";
        ASSERT_TRUE(domain_checked[1]) << "Assumptions have been broken! In testing e_field, no "
                                          "vertex was located at z = 0.25";
        ASSERT_TRUE(domain_checked[2]) << "Assumptions have been broken! In testing e_field, no "
                                          "vertex was located at z = -0.25";
        ...
        for (int f = 0; f < surface_S.num_elements(); ++f) {
            EXPECT_TRUE(CompareMatrices(surface_S.face_normal(f), X_SR_.rotation() * Vector3d::UnitZ(), 4 * kEps));
        }
    ...
    void VerifyComputeContactSurfaceFromSoftRigid(const ContactSurface<double>& contact_SR,
        ...
        const Vector3<double> p_WV = (mesh_SR_W.vertex(face.vertex(0)) + mesh_SR_W.vertex(face.vertex(1)) +
                                      mesh_SR_W.vertex(face.vertex(2))) /
                                     3;
        const int f_index = 0;
        EXPECT_EQ(field_SR_W.EvaluateCartesian(f_index, p_WV), field_RS_W.EvaluateCartesian(f_index, p_WV));
        ...
        EXPECT_TRUE(CompareMatrices(contact_SR.EvaluateGradE_M_W(f0), X_WS.rotation() * field_S_->EvaluateGradient(t),
                                    4. * kEps));
    }
...
using MeshBuilders = ::testing::Types<TriMeshBuilder<double>, PolyMeshBuilder<double>>;
...
TYPED_TEST(MeshIntersectionFixture, SampleVolumeFieldOnSurface) {
    ...
    intersector.SampleVolumeFieldOnSurface(*this->field_S_, *this->bvh_mesh_S_, *this->surface_R_,
    ...
    this->VerifySampleVolumeFieldOnSurface(surface_S, intersector.mutable_field(), intersector.mutable_grad_eM_M());
}
...
TYPED_TEST(MeshIntersectionFixture, TestComputeContactSurfaceSoftRigid) {
    ...
    using MeshBuilder = TypeParam;
    HydroelasticContactRepresentation representation = HydroelasticContactRepresentation::kTriangle;
    if (!std::is_same_v<typename MeshBuilder::MeshType, TriangleSurfaceMesh<typename MeshBuilder::ScalarType>>) {
    ...
    auto contact_SR = ComputeContactSurfaceFromSoftVolumeRigidSurface(id_A, *this->field_S_, *this->bvh_mesh_S_, X_WS,
                                                                      id_B, *this->surface_R_, *this->bvh_surface_R_,
                                                                      X_WR, representation);
    ...
    auto contact_RS = ComputeContactSurfaceFromSoftVolumeRigidSurface(id_B, *this->field_S_, *this->bvh_mesh_S_, X_WS,
                                                                      id_A, *this->surface_R_, *this->bvh_surface_R_,
                                                                      X_WR, representation);
    ...
    this->VerifyComputeContactSurfaceFromSoftRigid(*contact_SR, *contact_RS, X_WS);
}
...
protected:
    void SetUp() override {
        ...
        tet_mesh_S_ = make_unique<VolumeMesh<double>>(std::move(elements), std::move(vertices_S));
        field_S_ = make_unique<VolumeMeshFieldLinear<double, double>>(vector<double>{0.25, 0.5, 0.75, 1},
                                                                      tet_mesh_S_.get());
        ...
        /* Rigid triangle mesh; tilt and offset the triangle's plane so things are
         interesting. */
        vector<Vector3d> vertices{Vector3d{-5, -5, 0}, Vector3d{5, -5, 0}, Vector3d{0, 5, 0}};
        vector<SurfaceTriangle> faces{{0, 1, 2}};
        tri_mesh_R_ = make_unique<TriangleSurfaceMesh<double>>(std::move(faces), std::move(vertices));
...
TEST_F(MeshMeshDerivativesTest, FaceNormalsWrtPosition) {
    ...
    auto evaluate_normals = [X_WR = convert_to_double(this->X_WR_)](const ContactSurface<AutoDiffXd>& surface,
        ...
        for (int f = 0; f < mesh_W.num_elements(); ++f) {
            ...
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(tri_n_W), zeros, 10 * kEps));
        }
    };
...
TEST_F(MeshMeshDerivativesTest, FaceNormalsWrtOrientation) {
    ...
    for (const double theta : {0.0, M_PI / 6, M_PI / 2 * 0.9, M_PI / 2 * 0.99}) {
        ...
        auto surface = ComputeContactSurfaceFromSoftVolumeRigidSurface(
                this->id_S_, *this->field_S_, *this->bvh_S_, X_WS, this->id_R_, *this->tri_mesh_R_, *this->bvh_R_,
