...
VolumeMesh<double> MakeTransformedSphere(double radius, const math::RigidTransformd& X_FS) {
    const Sphere sphere = Sphere(radius);
    const VolumeMesh<double> mesh_S =
            MakeSphereVolumeMesh<double>(sphere, radius * 2, TessellationStrategy::kSingleInteriorVertex);
...
GTEST_TEST(DeformableGeometryTest, Constructor) {
    ...
    // be so taxing.
    math::RigidTransformd X_WS(math::RotationMatrixd::MakeXRotation(0.5) * math::RotationMatrixd::MakeYRotation(0.3) *
    ...
    auto verify_sdf = [num_vertices](const DeformableGeometry& geometry) {
        const VolumeMeshFieldLinear<double, double>& sdf = geometry.CalcSignedDistanceField();
        EXPECT_DOUBLE_EQ(sdf.EvaluateAtVertex(kCenterVertexIndex), -kRadius / std::sqrt(3));
        // Skipping center vertex = 0.
        for (int i = 1; i < num_vertices; ++i) {
            EXPECT_NEAR(sdf.EvaluateAtVertex(i), 0.0, std::numeric_limits<double>::epsilon());
        }
    };
...
GTEST_TEST(DeformableGeometryTest, TestCopyAndMoveSemantics) {
    ...
    std::vector<Vector3d> dummy_vertices = {Vector3d(0, 0, 0), Vector3d(1, 0, 0), Vector3d(0, 1, 0), Vector3d(0, 0, 1)};
    ...
    // Test copy-assignment operator.
    {
        ...
        EXPECT_FALSE(copy.deformable_mesh().mesh().Equal(original.deformable_mesh().mesh()));
        copy = original;

        // Test for uniqueness.
        EXPECT_NE(&original.deformable_mesh(), &copy.deformable_mesh());
        EXPECT_NE(&original.deformable_mesh().mesh(), &copy.deformable_mesh().mesh());
        EXPECT_NE(&copy.deformable_mesh().bvh(), &original.deformable_mesh().bvh());

        EXPECT_TRUE(copy.deformable_mesh().mesh().Equal(original.deformable_mesh().mesh()));
        EXPECT_TRUE(copy.deformable_mesh().bvh().Equal(original.deformable_mesh().bvh()));

        const VolumeMeshFieldLinear<double, double>& copy_sdf = copy.CalcSignedDistanceField();
        const VolumeMeshFieldLinear<double, double>& original_sdf = original.CalcSignedDistanceField();
    ...
    // ownership.
    {
        DeformableGeometry start(original);  // Assume the copy constructor is correct.

        // Grab raw pointers so we can determine that their ownership changes due to
        // move semantics.
        const DeformableVolumeMeshWithBvh<double>* const mesh_ptr = &start.deformable_mesh();
...
GTEST_TEST(DeformableGeometryTest, UpdateVertexPositions) {
    ...
    VolumeMesh<double> mesh = MakeSphereVolumeMesh<double>(sphere, 0.5, TessellationStrategy::kDenseInteriorVertices);
    ...
    deformable_geometry.UpdateVertexPositions(q);
    const VolumeMesh<double> deformed_mesh = deformable_geometry.deformable_mesh().mesh();
