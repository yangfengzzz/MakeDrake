...
public:
    explicit TriangleSurfaceMeshTester(const TriangleSurfaceMesh<T>& mesh) : mesh_(mesh) {}
    Vector3<T> CalcGradBarycentric(int f, int i) const { return mesh_.CalcGradBarycentric(f, i); }
...
std::unique_ptr<TriangleSurfaceMesh<T>> TestSurfaceMesh(
    ...
    const Vector3<T> vertex_data_M[4] = {{0., 0., 0.}, {15., 0., 0.}, {15., 15., 0.}, {0., 15., 0.}};
    std::vector<Vector3<T>> vertices_W;
    for (int v = 0; v < 4; ++v) vertices_W.emplace_back(X_WM * vertex_data_M[v]);
    auto surface_mesh_W = std::make_unique<TriangleSurfaceMesh<T>>(std::move(faces), std::move(vertices_W));

    EXPECT_EQ(2, surface_mesh_W->num_triangles());
    EXPECT_EQ(4, surface_mesh_W->num_vertices());
    for (int v = 0; v < 4; ++v) EXPECT_EQ(X_WM * vertex_data_M[v], surface_mesh_W->vertex(v));
    for (int f = 0; f < 2; ++f) {
        EXPECT_EQ(surface_mesh_W->element(f).num_vertices(), 3);
        for (int v = 0; v < 3; ++v) EXPECT_EQ(face_data[f][v], surface_mesh_W->element(f).vertex(v));
    }
...
GTEST_TEST(SurfaceMeshTest, TestFaceNormal) {
    const math::RigidTransform<double> X_WM(math::RollPitchYaw<double>(M_PI / 6.0, M_PI / 3.0, M_PI / 4.0),
    ...
    const Vector3<double> expect_normal = X_WM.rotation() * Vector3<double>::UnitZ();
...
GTEST_TEST(SurfaceMeshTest, TestCentroid) {
    ...
    EXPECT_TRUE(CompareMatrices(centroid, Vector3d(1.0 / 6, -1.0 / 6, -0.5), tol));
    EXPECT_TRUE(CompareMatrices(surface_mesh->element_centroid(0), Vector3d(-0.5 / 3, 0.5 / 3, -0.5), tol));
    EXPECT_TRUE(CompareMatrices(surface_mesh->element_centroid(1), Vector3d(1.0 / 3, -1.0 / 3, -0.5), tol));
...
GTEST_TEST(SurfaceMeshTest, TestDegenerateGradients) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(zero_area_mesh->CalcGradientVectorOfLinearField(dummy_values, 0),
                                ".*not calculate gradient.*");
    EXPECT_FALSE(zero_area_mesh->MaybeCalcGradientVectorOfLinearField(dummy_values, 0).has_value());
}
...
void TestCalcBarycentric() {
    const math::RigidTransform<T> X_WM(math::RollPitchYaw<T>(M_PI / 6.0, 2.0 * M_PI / 3.0, 7.0 * M_PI / 4.0),
    ...
    // Out of the plane of the triangle but still projected into the triangle.
    {
        ...
        auto barycentric = surface_mesh_W->CalcBarycentric(X_WM * above_midpoint, f0);
        const Barycentric expect_barycentric(1. / 3, 7. / 15., 1. / 5.);
        EXPECT_LE((barycentric - expect_barycentric).norm(), kTolerance);
    }
...
void TestCalcGradBarycentric() {
    ...
    // test more realistic.
    const math::RigidTransform<T> X_WM(math::RollPitchYaw<T>(M_PI / 6.0, 2.0 * M_PI / 3.0, 5.0 * M_PI / 4.0),
                                       Vector3<T>(1.0, 2.0, 3.0));

    // Create the mesh with vertex coordinates expressed in World frame to make
    // the test more realistic.
    const TriangleSurfaceMesh<T> mesh_W({SurfaceTriangle(triangle)},
                                        {Vector3<T>(X_WM * v0_M), Vector3<T>(X_WM * v1_M), Vector3<T>(X_WM * v2_M)});
    ...
    EXPECT_TRUE(CompareMatrices(gradb0_W + gradb1_W + gradb2_W, Vector3<T>::Zero(), 1e-14));
}
...
void TestCalcGradientVectorOfLinearField() {
    ...
    const TriangleSurfaceMesh<T> mesh_M({SurfaceTriangle(triangle)},
...
GTEST_TEST(SurfaceMeshTest, ReverseFaceWinding) {
    ...
    auto faces_match = [](const SurfaceTriangle& ref_face, const SurfaceTriangle& test_face) {
        for (int i = 0; i < 3; ++i) {
    ...
    auto winding_reversed = [](const SurfaceTriangle& ref_face, const SurfaceTriangle& test_face) {
        ...
        winding_is_valid &= ref_face.vertex((ref_first + 1) % 3) == test_face.vertex(2);
        winding_is_valid &= ref_face.vertex((ref_first + 2) % 3) == test_face.vertex(1);
...
GTEST_TEST(SurfaceMeshTest, TransformVertices) {
    ...
    RigidTransformd X_FM{AngleAxisd{M_PI / 4, Vector3d(1, 2, 3).normalized()}, Vector3d{1, 2, 3}};
...
protected:
    void SetUp() override {
        ...
        mesh_ad_ = std::make_unique<TriangleSurfaceMesh<AutoDiffXd>>(std::move(faces), std::move(vertices));
...
TEST_F(ScalarMixingTest, CalcCartesianFromBarycentric) {
    ...
    {
        // Double-valued mesh and double-valued point: double-valued result.
        const Vector3d p_WC = mesh_d_->CalcCartesianFromBarycentric(e0_, b_expected_d_);
...

    {
        // Double-valued mesh with AutoDiffXd-valued point: AutodDiffXd-valued
        // result.
        const Vector3<AutoDiffXd>& p_WC = mesh_d_->CalcCartesianFromBarycentric(e0_, b_expected_ad_);
...

    {
        // AutoDiffXd-valued mesh with double-valued point on triangle *with*
        // derivatives: AutodDiffXd-valued result *with* derivatives.
        const Vector3<AutoDiffXd>& p_WC1 = mesh_ad_->CalcCartesianFromBarycentric(e0_, b_expected_d_);
        ...
        // AutoDiffXd-valued mesh with double-valued point on triangle *without*
        // derivatives: AutodDiffXd-valued result *without* derivatives.
        const Vector3<AutoDiffXd>& p_WC2 = mesh_ad_->CalcCartesianFromBarycentric(e1_, b_expected_d_);
...

    {
        // AutoDiffXd-valued mesh with AutoDiffXd-valued point on triangle:
        // AutodDiffXd-valued.
        const Vector3<AutoDiffXd>& p_WC = mesh_ad_->CalcCartesianFromBarycentric(e0_, b_expected_ad_);
        EXPECT_EQ(p_WC(0).derivatives().size(), 3);
        EXPECT_EQ(p_WC(1).derivatives().size(), 3);
        EXPECT_EQ(p_WC(2).derivatives().size(), 3);
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(p_WC), p_WQ_d_, kEps));
