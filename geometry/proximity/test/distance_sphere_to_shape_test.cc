...
#include "geometry/query_results/signed_distance_to_point.h"
...
GTEST_TEST(SphereShapeDistance, FallbackSupport) {
    ...
    DRAKE_EXPECT_NO_THROW(CalcDistanceFallback<double>(obj_a, RigidTransformd(), obj_b, RigidTransformd(), request,
                                                       &distance_pair_d));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            CalcDistanceFallback<AutoDiffXd>(obj_a, RigidTransform<AutoDiffXd>(), obj_b, RigidTransform<AutoDiffXd>(),
...
protected:
    void SetUp() override {
        ...
        X_WShape_.set(RotationMatrixd(AngleAxis<double>(M_PI / 5, Vector3d{2, 4, 7}.normalized())), Vector3d::Zero());
    }
    ...
    ::testing::AssertionResult ResultsMatch(const Shape& test_shape) {
        ...
        DistancePairGeometry<T> distance_functor(sphere_id, shape_id, X_WSphere, X_WShape_.cast<T>(), &shape_result);

        // Computes sphere-shape results.
        distance_functor(query_sphere_, test_shape);
        point_distance::DistanceToPoint<T> distance_to_point(shape_id, X_WShape_.cast<T>(), p_WSphere<T>());
        ...
        result = CompareMatrices(shape_result.p_ACa, R_WSphere_.inverse().cast<T>() * (-kRadius * point_result.grad_W));
    ...
    ::testing::AssertionResult TestDerivatives(const SignedDistancePair<double>&,
                                               const SignedDistanceToPoint<double>&) {
        return ::testing::AssertionSuccess();
    }

    ::testing::AssertionResult TestDerivatives(const SignedDistancePair<AutoDiffXd>& shape_result,
        ...
        if (!result) {
            const std::string message = fmt::format("Distance derivatives don't match; expected: <{}>, got: <{}>",
        ...
        if (!result) {
            const std::string message = fmt::format("Normal derivatives don't match; expected:\n{}\ngot:\n{}",
        ...
        if (!result) {
            const std::string message = fmt::format("p_BCb derivatives don't match; expected:\n{}\ngot:\n{}",
        ...
        const Matrix3<double> dp_WCa_dAo =
                R_WB * dCb_dAo + grad_W * dd_dAo_shape.transpose() + shape_result.distance.value() * dgrad_dAo_shape;
        Matrix3<double> dCa_dAo_expected = R_AW * dp_WCa_dAo - Matrix3<double>::Identity();
        const auto dCa_dAo = math::ExtractGradient(shape_result.p_ACa);
        result = CompareMatrices(dCa_dAo, dCa_dAo_expected, 4 * kEps);
        if (!result) {
            const std::string message = fmt::format("p_ACa derivatives don't match; expected:\n{}\ngot:\n{}",
...
GTEST_TEST(ComputeNarrowPhaseDistance, NoFallbackInvocation) {
    ...
    const CollisionObjectd half_space(make_shared<Halfspaced>(Vector3d{0, 0, 1}, 0));
    const CollisionObjectd sphere(make_shared<Sphered>(1));

    // All pairs which have Drake implementations.
    for (const CollisionObjectd* other : {&box, &capsule, &cylinder, &half_space, &sphere}) {
...
GTEST_TEST(ComputeNarrowPhaseDistance, FallbackInvocation) {
    ...
    const CollisionObjectd convex(make_shared<Convexd>(make_shared<const std::vector<Vector3d>>(1, Vector3d{0, 0, 0}),
                                                       0, make_shared<const std::vector<int>>()));
    const CollisionObjectd cylinder(make_shared<Cylinderd>(1, 2));
    ...
    // All other cases.
    const std::vector<const CollisionObjectd*> others{&box, &capsule, &convex, &cylinder, &half_space};
...
GTEST_TEST(ComputeNarrowPhaseDistance, OrderInvariance) {
    ...
    const RigidTransformd X_WB(AngleAxis<double>(M_PI / 5, Vector3d{2, 4, 7}.normalized()), Vector3d::Zero());
    SignedDistancePair<double> result_BS;
    SignedDistancePair<double> result_SB;

    ComputeNarrowPhaseDistance<double>(sphere, X_WS, box, X_WB, request, &result_SB);

    ComputeNarrowPhaseDistance<double>(box, X_WB, sphere, X_WS, request, &result_BS);
...
public:
    CallbackScalarSupport()
        : collision_filter_(), X_WGs_(), results_(), data_(&collision_filter_, &X_WGs_, kInf, &results_), spheres_() {}
...
protected:
    ...
    std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> supported_pairs() {
        throw std::logic_error("No supported pairs implemented -- check scalar type");
    }

    std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> unsupported_pairs() {
        throw std::logic_error("No unsupported pairs implemented -- check scalar type");
    }
...
template <>
std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> CallbackScalarSupport<double>::supported_pairs() {
    ...
    return {{spheres_[0], spheres_[1]},    {spheres_[0], boxes_[1]},      {spheres_[0], capsules_[1]},
            {spheres_[0], cylinders_[1]},  {spheres_[0], halfspaces_[1]}, {boxes_[0], boxes_[1]},
            {boxes_[0], capsules_[1]},     {boxes_[0], cylinders_[1]},    {capsules_[0], capsules_[1]},
            {cylinders_[0], capsules_[1]}, {cylinders_[0], cylinders_[1]}};
}

template <>
std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> CallbackScalarSupport<double>::unsupported_pairs() {
    return {
...
template <>
std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> CallbackScalarSupport<AutoDiffXd>::supported_pairs() {
    return {{spheres_[0], spheres_[1]}, {spheres_[0], boxes_[1]}, {spheres_[0], halfspaces_[1]}};
}

template <>
std::vector<std::pair<CollisionObjectd&, CollisionObjectd&>> CallbackScalarSupport<AutoDiffXd>::unsupported_pairs() {
    return {
            {spheres_[0], cylinders_[1]},    {boxes_[0], boxes_[1]},           {spheres_[0], capsules_[1]},
            {boxes_[0], capsules_[1]},       {boxes_[0], cylinders_[1]},       {boxes_[0], halfspaces_[1]},
            {capsules_[0], capsules_[1]},    {cylinders_[0], capsules_[1]},    {cylinders_[0], cylinders_[1]},
...
GTEST_TEST(Callback, ScalarSupportWithFilters) {
    ...
    collision_filter.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{data_A.id(), data_B.id()}), extract,
                           false /* is_invariant */);

    const std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{id_A, RigidTransform<T>::Identity()},
...
GTEST_TEST(Callback, RespectCollisionFiltering) {
    ...
    const std::unordered_map<GeometryId, RigidTransformd> X_WGs{{id_A, RigidTransformd{Vector3d{10, 11, 12}}},
...
TYPED_TEST(CallbackMaxDistanceTest, MaxDistanceThreshold) {
    ...
    data_B.write_to(&sphere_B);
    const Vector3<T> p_WB = Vector3<T>(2, 3, 4).normalized() * (kMaxDistance + radius_A + radius_B - kEps);
    std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{id_A, RigidTransform<T>::Identity()},
    ...
    {
        X_WGs.at(id_B) = RigidTransform<T>(Eigen::Translation<T, 3>{Vector3<T>(2, 3, 4).normalized() *
