...
#include "geometry/proximity/make_sphere_field.h"
#include "geometry/proximity/make_sphere_mesh.h"
#include "geometry/proximity/proximity_utilities.h"
#include "geometry/proximity/tessellation_strategy.h"
...
GTEST_TEST(SoftMeshTest, TestCopyMoveAssignConstruct) {
    ...
    auto mesh = make_unique<VolumeMesh<double>>(
            MakeSphereVolumeMesh<double>(sphere, resolution_hint, TessellationStrategy::kSingleInteriorVertex));
    ...
    // Test copy-assignment operator.
    {
        ...
        const auto& copy_pressure = static_cast<const VolumeMeshFieldLinear<double, double>&>(copy.pressure());
        const auto& original_pressure = static_cast<const VolumeMeshFieldLinear<double, double>&>(original.pressure());
    ...
    // ownership.
    {
        ...
        const VolumeMeshFieldLinear<double, double>* const pressure_ptr = &start.pressure();
...
GTEST_TEST(SoftGeometryTest, TestCopyMoveAssignConstruct) {
    ...
    // Test copy-assignment operator.
    {
        ...
        const auto& copy_pressure = static_cast<const VolumeMeshFieldLinear<double, double>&>(dut.pressure_field());
        const auto& original_pressure =
                static_cast<const VolumeMeshFieldLinear<double, double>&>(original.pressure_field());
        EXPECT_TRUE(copy_pressure.Equal(original_pressure));
    ...
    // Test copy constructor.
    {
        ...
        EXPECT_TRUE(copy.mesh().Equal(original.mesh()));
        const auto& copy_pressure = static_cast<const VolumeMeshFieldLinear<double, double>&>(copy.pressure_field());
        const auto& original_pressure =
                static_cast<const VolumeMeshFieldLinear<double, double>&>(original.pressure_field());
        EXPECT_TRUE(copy_pressure.Equal(original_pressure));
        EXPECT_TRUE(copy.bvh().Equal(original.bvh()));
    ...
    // ownership.
    {
        ...
        // move semantics.
        const VolumeMesh<double>* const mesh_ptr = &start.mesh();
        const VolumeMeshFieldLinear<double, double>* const pressure_ptr = &start.pressure_field();
        const Bvh<Obb, VolumeMesh<double>>* const bvh_ptr = &start.bvh();
...
GTEST_TEST(RigidMeshTest, TestCopyMoveAssignConstruct) {
    const Sphere sphere(0.5);
    const double resolution_hint = 0.5;
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeSphereSurfaceMesh<double>(sphere, resolution_hint));
...
GTEST_TEST(RigidGeometryTest, TestCopyMoveAssignConstruct) {
    const RigidGeometry original(
            RigidMesh(make_unique<TriangleSurfaceMesh<double>>(MakeSphereSurfaceMesh<double>(Sphere(1.25), 2.0))));
...
GTEST_TEST(Hydroelastic, GeometriesPopulationAndQuery) {
    ...
    EXPECT_EQ(geometries.hydroelastic_type(rigid_id), HydroelasticType::kUndefined);
    EXPECT_EQ(geometries.hydroelastic_type(soft_id), HydroelasticType::kUndefined);
...
void DoTestVanished(const Shape& shape, bool expect_vanished) {
    SCOPED_TRACE(fmt::format("DoTestVanished: {}, expect_vanished: {}", shape.to_string(), expect_vanished));
    Geometries geometries;
...
GTEST_TEST(HydroelasticVanished, Mesh) {
    const Mesh normal_mesh_specification(FindResourceOrThrow("drake/geometry/test/non_convex_mesh.vtk"), 1e-4);
    DoTestVanished(normal_mesh_specification, false);
    const Mesh tiny_mesh_specification(FindResourceOrThrow("drake/geometry/test/non_convex_mesh.vtk"), 1e-8);
    DRAKE_EXPECT_THROWS_MESSAGE(DoTestVanished(tiny_mesh_specification, false), ".*cannot compute gradient.*");
}

GTEST_TEST(HydroelasticVanished, Convex) {
    const Convex normal_convex_specification(FindResourceOrThrow("drake/geometry/test/octahedron.obj"), 1e-4);
    DoTestVanished(normal_convex_specification, false);
    const Convex tiny_convex_specification(FindResourceOrThrow("drake/geometry/test/octahedron.obj"), 1e-8);
    DRAKE_EXPECT_THROWS_MESSAGE(DoTestVanished(tiny_convex_specification, false), ".*cannot compute gradient.*");
}
...
TEST_F(HydroelasticRigidGeometryTest, HalfSpace) {
    ...
    std::optional<RigidGeometry> half_space = MakeRigidRepresentation(HalfSpace(), props);
    ASSERT_NE(half_space, std::nullopt);
    EXPECT_TRUE(half_space->is_half_space());

    DRAKE_EXPECT_THROWS_MESSAGE(half_space->mesh(), "RigidGeometry::mesh.* cannot be invoked .* half space");
    DRAKE_EXPECT_THROWS_MESSAGE(half_space->bvh(), "RigidGeometry::bvh.* cannot be invoked .* half space");
}
...
TEST_F(HydroelasticRigidGeometryTest, Sphere) {
    ...
    std::optional<RigidGeometry> sphere = MakeRigidRepresentation(sphere_spec, props);
...
TEST_F(HydroelasticRigidGeometryTest, Box) {
    ...
    std::optional<RigidGeometry> box = MakeRigidRepresentation(Box(edge_len, edge_len, edge_len), props);
...
TEST_F(HydroelasticRigidGeometryTest, Cylinder) {
    ...
    std::optional<RigidGeometry> cylinder = MakeRigidRepresentation(Cylinder(radius, length), props);
...
TEST_F(HydroelasticRigidGeometryTest, Capsule) {
    ...
    std::optional<RigidGeometry> capsule = MakeRigidRepresentation(capsule_shape, props);
    ...
    std::optional<RigidGeometry> capsule_fine = MakeRigidRepresentation(capsule_shape, props_fine);
...
TEST_F(HydroelasticRigidGeometryTest, Ellipsoid) {
    ...
    std::optional<RigidGeometry> ellipsoid = MakeRigidRepresentation(Ellipsoid(a, b, c), props);
...
void TestRigidMeshCube(const std::string& file) {
    ...
    std::optional<RigidGeometry> geometry = MakeRigidRepresentation(Mesh(file, scale), props);
    ...
    for (int v = 0; v < surface_mesh.num_vertices(); ++v) {
        ...
        ASSERT_NEAR(dist, expected_dist, scale * kEps) << "for scale: " << scale << " at vertex " << v;
    }
}
...
TEST_F(HydroelasticRigidGeometryTest, Mesh) {
    {
        ...
        const std::string file_name = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");
        TestRigidMeshCube(file_name);
    }
    {
        SCOPED_TRACE("Rigid Mesh, vtk");
        const std::string file_name = FindResourceOrThrow("drake/geometry/test/cube_as_volume.vtk");
        TestRigidMeshCube(file_name);
    }
    {
        SCOPED_TRACE("Rigid Mesh, unsupported extension");
        DRAKE_EXPECT_THROWS_MESSAGE(TestRigidMeshCube("invalid.stl"), ".*Mesh shapes can only use .*invalid.stl");
    }
}
...
TEST_F(HydroelasticRigidGeometryTest, Convex) {
    auto expect_convex_cube = [](const std::string& file) {
        ...
        const std::optional<RigidGeometry> geometry = MakeRigidRepresentation(convex, ProximityProperties());
        ASSERT_NE(geometry, std::nullopt);
        ASSERT_FALSE(geometry->is_half_space());
...

    {
        ...
        const std::string file = FindResourceOrThrow("drake/geometry/test/cube_with_hole.obj");
        expect_convex_cube(file);
    }

    {
        SCOPED_TRACE("Rigid Convex from glTF file");
        const std::string file = FindResourceOrThrow("drake/geometry/test/cube_with_hole.gltf");
        expect_convex_cube(file);
    }
...
void TestPropertyErrors(const ShapeType& shape_spec,
                        const char* group_name,
                        const char* property_name,
                        const char* compliance,
                        function<void(const ShapeType&, const ProximityProperties&)> maker,
                        std::optional<ValueType> bad_value,
                        const ProximityProperties& props) {
    const std::string_view shape_name = shape_spec.type_name();

    // Error case: missing property value.
    {
        DRAKE_EXPECT_THROWS_MESSAGE(maker(shape_spec, props), fmt::format("Cannot create {} {}.+'{}'\\) property",
                                                                          compliance, shape_name, property_name));
    }
    ...
    // Error case: property value is wrong type.
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(maker(shape_spec, wrong_value),
    ...
    if (bad_value.has_value()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(
                ...
                fmt::format("Cannot create {} {}.+'{}'.+ positive", compliance, shape_name, property_name));
    }
}
...
TYPED_TEST_P(HydroelasticRigidGeometryErrorTests, BadResolutionHint) {
    ...
    TestPropertyErrors<ShapeType, double>(shape_spec, kHydroGroup, kRezHint, "rigid",
...
REGISTER_TYPED_TEST_SUITE_P(HydroelasticRigidGeometryErrorTests, BadResolutionHint);
typedef ::testing::Types<Sphere, Capsule, Cylinder, Ellipsoid> RigidErrorShapeTypes;
INSTANTIATE_TYPED_TEST_SUITE_P(My, HydroelasticRigidGeometryErrorTests, RigidErrorShapeTypes);
...
TEST_F(HydroelasticSoftGeometryTest, HalfSpace) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeSoftRepresentation(HalfSpace(), properties),
    ...
    std::optional<SoftGeometry> half_space = MakeSoftRepresentation(HalfSpace(), properties);
    ASSERT_NE(half_space, std::nullopt);
    EXPECT_TRUE(half_space->is_half_space());
    EXPECT_EQ(half_space->pressure_scale(), properties.GetProperty<double>(kHydroGroup, kElastic) / thickness);

    DRAKE_EXPECT_THROWS_MESSAGE(half_space->mesh(), "SoftGeometry::mesh.* cannot be invoked .* half space");
    DRAKE_EXPECT_THROWS_MESSAGE(half_space->pressure_field(),
                                "SoftGeometry::pressure.* cannot be invoked .* half space");
    DRAKE_EXPECT_THROWS_MESSAGE(half_space->bvh(), "SoftGeometry::bvh.* cannot be invoked .* half space");
}
...
TEST_F(HydroelasticSoftGeometryTest, Sphere) {
    ...
    std::optional<SoftGeometry> sphere1 = MakeSoftRepresentation(sphere_spec, properties1);
    std::optional<SoftGeometry> sphere2 = MakeSoftRepresentation(sphere_spec, properties2);
    ...
    for (int v = 0; v < mesh.num_vertices(); ++v) {
        ...
        EXPECT_NEAR(sphere1->pressure_field().EvaluateAtVertex(v), expected_p, kEps * E);
    }
    ...
    // property in the following ways:
    {
        ...
        EXPECT_EQ(sphere1->mesh().num_elements() * 4, sphere2->mesh().num_elements());
    }

    {
        ...
        std::optional<SoftGeometry> dense_sphere = MakeSoftRepresentation(sphere_spec, dense_properties);
        EXPECT_LT(sphere1->mesh().num_elements(), dense_sphere->mesh().num_elements());
    }

    {
        ...
        ProximityProperties dense_properties(properties1);
        dense_properties.AddProperty(kHydroGroup, "tessellation_strategy", TessellationStrategy::kSingleInteriorVertex);
        std::optional<SoftGeometry> dense_sphere = MakeSoftRepresentation(sphere_spec, dense_properties);
        EXPECT_EQ(sphere1->mesh().num_elements(), dense_sphere->mesh().num_elements());
    }

    {
        ...
        EXPECT_THROW(MakeSoftRepresentation(sphere_spec, dense_properties), std::logic_error);
    }
}
...
TEST_F(HydroelasticSoftGeometryTest, Box) {
    ...
    ProximityProperties properties = soft_properties();
    std::optional<SoftGeometry> box = MakeSoftRepresentation(box_spec, properties);
...
TEST_F(HydroelasticSoftGeometryTest, Cylinder) {
    ...
    std::optional<SoftGeometry> cylinder = MakeSoftRepresentation(cylinder_spec, properties);
...
TEST_F(HydroelasticSoftGeometryTest, Capsule) {
    ...
    ProximityProperties properties = soft_properties(1.5 * length);
    std::optional<SoftGeometry> capsule = MakeSoftRepresentation(capsule_spec, properties);
...
TEST_F(HydroelasticSoftGeometryTest, Ellipsoid) {
    ...
    std::optional<SoftGeometry> ellipsoid = MakeSoftRepresentation(ellipsoid_spec, properties);
    ...
    sparse_properties.AddProperty(kHydroGroup, "tessellation_strategy", TessellationStrategy::kSingleInteriorVertex);
    ProximityProperties dense_properties(basic_properties);
    dense_properties.AddProperty(kHydroGroup, "tessellation_strategy", TessellationStrategy::kDenseInteriorVertices);

    std::optional<SoftGeometry> implicit_sparse_ellipsoid = MakeSoftRepresentation(ellipsoid_spec, basic_properties);
    std::optional<SoftGeometry> sparse_ellipsoid = MakeSoftRepresentation(ellipsoid_spec, sparse_properties);
    std::optional<SoftGeometry> dense_ellipsoid = MakeSoftRepresentation(ellipsoid_spec, dense_properties);

    {
        ...
        EXPECT_EQ(implicit_sparse_ellipsoid->mesh().num_elements(), sparse_ellipsoid->mesh().num_elements());
    }

    {
        ...
        // tets.
        EXPECT_LT(sparse_ellipsoid->mesh().num_elements(), dense_ellipsoid->mesh().num_elements());
    }

    {
        ...
        EXPECT_THROW(MakeSoftRepresentation(ellipsoid_spec, bad_properties), std::logic_error);
    }
}
...
TEST_F(HydroelasticSoftGeometryTest, Convex) {
    ...
    ProximityProperties properties = soft_properties(0.16);
    std::optional<SoftGeometry> convex = MakeSoftRepresentation(convex_spec, properties);
...
TEST_F(HydroelasticSoftGeometryTest, Mesh) {
    ...
    ProximityProperties properties = soft_properties();
    std::optional<SoftGeometry> compliant_geometry = MakeSoftRepresentation(mesh_specification, properties);
    ...
    for (int v = 0; v < compliant_geometry->mesh().num_vertices(); ++v) {
        const double pressure = compliant_geometry->pressure_field().EvaluateAtVertex(v);
        EXPECT_GE(pressure, 0);
        EXPECT_LE(pressure, E);
    }
}
...
TYPED_TEST_P(HydroelasticSoftGeometryErrorTests, BadResolutionHint) {
    using ShapeType = TypeParam;
    ShapeType shape_spec = make_default_shape<ShapeType>();
    if (shape_spec.type_name() != "HalfSpace" && shape_spec.type_name() != "Box" &&
        shape_spec.type_name() != "Convex") {
        TestPropertyErrors<ShapeType, double>(shape_spec, kHydroGroup, kRezHint, "soft",
                                              [](const ShapeType& s, const ProximityProperties& p) {
...
TYPED_TEST_P(HydroelasticSoftGeometryErrorTests, BadSlabThickness) {
    ...
    if (shape_spec.type_name() == "HalfSpace") {
        TestPropertyErrors<ShapeType, double>(shape_spec, kHydroGroup, kSlabThickness, "soft",
                                              [](const ShapeType& s, const ProximityProperties& p) {
                                                  MakeSoftRepresentation(s, p);
                                              },
                                              -0.2, {});
...
REGISTER_TYPED_TEST_SUITE_P(HydroelasticSoftGeometryErrorTests, BadResolutionHint, BadElasticModulus, BadSlabThickness);
typedef ::testing::Types<Sphere, Box, Capsule, Cylinder, Ellipsoid, HalfSpace, Convex> SoftErrorShapeTypes;
INSTANTIATE_TYPED_TEST_SUITE_P(My, HydroelasticSoftGeometryErrorTests, SoftErrorShapeTypes);
