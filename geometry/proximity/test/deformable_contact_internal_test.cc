...
#include "geometry/proximity/deformable_field_intersection.h"
#include "geometry/proximity/deformable_mesh_intersection.h"
...
public:
    static std::unordered_set<GeometryId> geometries(const GeometrySet& s, CollisionFilterScope) {
        return s.geometries();
    }
};
...
public:
    ...
    */
    static const DeformableGeometry& get_deformable_geometry(const Geometries& geometries, GeometryId deformable_id) {
    ...
    static const RigidGeometry& get_rigid_geometry(const Geometries& geometries, GeometryId rigid_id) {
    ...
    static const std::unordered_map<GeometryId, DeformableGeometry>& deformable_geometries(
...
VolumeMesh<double> MakeVolumeMesh() {
    ...
    return MakeSphereVolumeMesh<double>(sphere, kRezHint, TessellationStrategy::kDenseInteriorVertices);
}
...
math::RigidTransformd default_pose() {
    return math::RigidTransformd(math::RollPitchYaw<double>(1, 2, 3), Vector3d(4, 5, 6));
}
...
GTEST_TEST(GeometriesTest, AddRigidGeometry) {
    ...
    geometries.MaybeAddRigidGeometry(Sphere(kRadius), rigid_id, props, default_pose());
    ...
    geometries.MaybeAddRigidGeometry(Sphere(kRadius), g_id, empty_props, default_pose());
    ...
    geometries.MaybeAddRigidGeometry(HalfSpace{}, half_space_id, props, default_pose());
...
GTEST_TEST(GeometriesTest, UnsupportedRigidShapes) {
    ...
    /* MeshcatCone */
    {
        ...
        const double a = 1.0;
        const double b = 1.0;
        EXPECT_NO_THROW(geometries.MaybeAddRigidGeometry(MeshcatCone(height, a, b), cone_id, props, default_pose()));
        EXPECT_FALSE(geometries.is_rigid(cone_id));
    }
    /* HalfSpace */
    {
        GeometryId hs_id = GeometryId::get_new_id();
        EXPECT_NO_THROW(geometries.MaybeAddRigidGeometry(HalfSpace(), hs_id, props, default_pose()));
...
GTEST_TEST(GeometriesTest, SupportedRigidShapes) {
    ...
    /* Box */
    {
        ...
        geometries.MaybeAddRigidGeometry(Box::MakeCube(1.0), box_id, props, default_pose());
    ...
    /* Sphere */
    {
        ...
        GeometryId box_id = GeometryId::get_new_id();
        geometries.MaybeAddRigidGeometry(Sphere(radius), box_id, props, default_pose());
        EXPECT_TRUE(geometries.is_rigid(box_id));
    }
    /* Cylinder */
    {
        ...
        geometries.MaybeAddRigidGeometry(Cylinder(radius, length), cylinder_id, props, default_pose());
    ...
    /* Capsule */
    {
        ...
        const double radius = 1.0;
        const double length = 2.0;
        geometries.MaybeAddRigidGeometry(Capsule(radius, length), capsule_id, props, default_pose());
    ...
    /* Ellipsoid */
    {
        ...
        const double b = 0.8;
        const double c = 0.3;
        geometries.MaybeAddRigidGeometry(Ellipsoid(a, b, c), ellipsoid_id, props, default_pose());
    ...
    /* Mesh */
    {
        ...
        geometries.MaybeAddRigidGeometry(Mesh(file, 1.0), mesh_id, props, default_pose());
    ...
    /* Convex */
    {
        ...
        std::string file = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");
        geometries.MaybeAddRigidGeometry(Convex(file, 1.0), convex_id, props, default_pose());
...
GTEST_TEST(GeometriesTest, UpdateRigidWorldPose) {
    ...
    {
        const RigidGeometry& rigid_geometry = GeometriesTester::get_rigid_geometry(geometries, rigid_id);
        EXPECT_TRUE(rigid_geometry.pose_in_world().IsExactlyEqualTo(default_pose()));
    }
    /* Update the pose to some arbitrary value. */
    const math::RigidTransform<double> X_WG(math::RollPitchYaw<double>(-1.57, 0, 3), Vector3d(-0.3, -0.55, 0.36));
...
GTEST_TEST(GeometriesTest, RemoveGeometry) {
    ...
    constexpr double kRezHint = 0.5;
    ProximityProperties props = MakeProximityPropsWithRezHint(kRezHint);
    geometries.MaybeAddRigidGeometry(Sphere(kRadius), rigid_id0, props, default_pose());
    geometries.MaybeAddRigidGeometry(Sphere(kRadius), rigid_id1, props, default_pose());
...
GTEST_TEST(GeometriesTest, UpdateDeformableVertexPositions) {
    ...
    /* Initially the vertex positions is the same as the registered mesh. */
    {
        ...
        const DeformableGeometry& geometry = GeometriesTester::get_deformable_geometry(geometries, deformable_id);
...
GTEST_TEST(GeometriesTest, ComputeDeformableContact_DeformableRigid) {
    ...
    DeformableContact<double> contact_data = geometries.ComputeDeformableContact(collision_filter);
    EXPECT_EQ(contact_data.contact_surfaces().size(), 0);

    /* Add a deformable unit cube. */
    GeometryId deformable_id = GeometryId::get_new_id();
    VolumeMesh<double> deformable_mesh = MakeBoxVolumeMesh<double>(Box::MakeCube(1.0), 1.0);
    ...
    geometries.MaybeAddRigidGeometry(Box::MakeCube(1.0), rigid_id, rigid_properties, X_WR);
    ...
    const auto& X_DR = X_WR;  // The deformable mesh frame is always the world frame.
    ...
    AddDeformableRigidContactSurface(deformable_geometry.CalcSignedDistanceField(),
                                     deformable_geometry.deformable_mesh(), deformable_id, rigid_id,
                                     rigid_geometry.rigid_mesh().mesh(), rigid_geometry.rigid_mesh().bvh(), X_DR,
                                     &expected_contact_data);

    /* Verify that the contact data is the same as expected by checking a subset
     of all data fields. */
    ASSERT_EQ(contact_data.contact_surfaces().size(), expected_contact_data.contact_surfaces().size());
    const DeformableContactSurface<double>& contact_surface = contact_data.contact_surfaces()[0];
    const DeformableContactSurface<double>& expected_contact_surface = expected_contact_data.contact_surfaces()[0];
    ...
    EXPECT_EQ(contact_surface.num_contact_points(), expected_contact_surface.num_contact_points());
    EXPECT_TRUE(contact_surface.contact_mesh_W().Equal(expected_contact_surface.contact_mesh_W()));
    ...
    collision_filter.Apply(
            CollisionFilterDeclaration().ExcludeBetween(GeometrySet(deformable_id), GeometrySet(rigid_id)),
...
protected:
    ...
    void SetUp() override {
        ...
        //                   | |
        //
        VolumeMesh<double> mesh0 = MakeBoxVolumeMeshWithMa<double>(Box(0.3, 0.01, 0.01));
        VolumeMesh<double> mesh1 = MakeBoxVolumeMeshWithMa<double>(Box(0.01, 0.01, 0.3));
...
TEST_F(DeformableDeformableContactTest, OneContactPair) {
    DeformableContact<double> contact_data = geometries_.ComputeDeformableContact(collision_filter_);
    ASSERT_EQ(contact_data.contact_surfaces().size(), 1);
    ...
    expected.RegisterDeformableGeometry(deformable0_id_, deformable0_geometry.deformable_mesh().mesh().num_vertices());
    expected.RegisterDeformableGeometry(deformable1_id_, deformable1_geometry.deformable_mesh().mesh().num_vertices());
    AddDeformableDeformableContactSurface(deformable1_geometry.CalcSignedDistanceField(),
    ...
    ASSERT_EQ(contact_data.contact_surfaces().size(), expected.contact_surfaces().size());
    EXPECT_EQ(contact_data.contact_surfaces().at(0).id_A(), expected.contact_surfaces().at(0).id_A());
    EXPECT_EQ(contact_data.contact_surfaces().at(0).id_B(), expected.contact_surfaces().at(0).id_B());
...
TEST_F(DeformableDeformableContactTest, MultipleContactPairs) {
    ...
    //
    {
        ...
        VolumeMesh<double> mesh2 = MakeBoxVolumeMeshWithMa<double>(Box(0.01, 0.3, 0.01));
    ...
    std::set<std::set<GeometryId>> expected_pairs{
            {deformable0_id, deformable1_id}, {deformable0_id, deformable2_id}, {deformable1_id, deformable2_id}};
    std::set<std::set<GeometryId>> pairs{
            {contact_data.contact_surfaces().at(0).id_A(), contact_data.contact_surfaces().at(0).id_B()},
            {contact_data.contact_surfaces().at(1).id_A(), contact_data.contact_surfaces().at(1).id_B()},
            {contact_data.contact_surfaces().at(2).id_A(), contact_data.contact_surfaces().at(2).id_B()}};
...
TEST_F(DeformableDeformableContactTest, RespectCollisionFilter) {
    ...
    const GeometryId deformable0_id = it->first;
    const GeometryId deformable1_id = (++it)->first;
    collision_filter_.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({deformable0_id, deformable1_id})),
                            get_extract_ids_functor());
