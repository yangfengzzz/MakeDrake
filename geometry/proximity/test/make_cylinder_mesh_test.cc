...
struct DistanceToCylinderBoundaryFromPointInside {
    explicit DistanceToCylinderBoundaryFromPointInside(const Cylinder& cylinder_in) : cylinder(cylinder_in) {}
...
bool IsTetrahedronRespectingMa(const VolumeElement& tetrahedron,
    ...
    auto distance_to_boundary_face = [&cylinder](int f, const Vector3d& point_in_cylinder) -> double {
        const double x = point_in_cylinder.x();
        const double y = point_in_cylinder.y();
        const double z = point_in_cylinder.z();
        const double top_z = cylinder.length() / 2;
    ...
    for (int f = 0; f < kNumCylinderFaces; ++f) {
        ...
        for (int v = 0; v < mesh.kVertexPerElement; ++v) {
            closest_to_this_face &=
                    closest_faces[v].end() != std::find(closest_faces[v].begin(), closest_faces[v].end(), f);
        }
...
void VerifyCylinderMeshWithMa(const VolumeMesh<double>& mesh,
    ...
    ASSERT_EQ(1, euler_characteristic) << "The mesh's tetrahedra are not conforming because the mesh's Euler "
                                          "characteristic is "
    ...
    const double distance_tolerance = DistanceToPointRelativeTolerance(std::max(half_length, cylinder.radius()));
    for (const Vector3d& v : mesh.vertices()) {
        const double x = v.x();
        const double y = v.y();
        const double z = v.z();
        bool is_inside_or_on_boundary =
                abs(z) < half_length + distance_tolerance && x * x + y * y < squared_radius + distance_tolerance;
        ASSERT_TRUE(is_inside_or_on_boundary) << "A mesh vertex is outside the cylinder.";
    }
    // B2. The volume of the mesh approximates the volume of the discretized
    // cylinder.
    const double mesh_volume = mesh.CalcVolume();
    const double expect_volume =
            half_length * squared_radius * num_vertex_per_circle * sin(2. * M_PI / num_vertex_per_circle);
    ...
    std::vector<int> boundary_vertices = CollectUniqueVertices(IdentifyBoundaryFaces(mesh.tetrahedra()));
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        bool tetrahedron_has_an_interior_vertex = false;
        for (int i = 0; i < mesh.kVertexPerElement && !tetrahedron_has_an_interior_vertex; ++i) {
            tetrahedron_has_an_interior_vertex =
                    boundary_vertices.end() ==
                    find(boundary_vertices.begin(), boundary_vertices.end(), tetrahedron.vertex(i));
        }
        ASSERT_TRUE(tetrahedron_has_an_interior_vertex) << "A tetrahedron has all its vertices on the boundary.";
    }
    ...
    DistanceToCylinderBoundaryFromPointInside distance_to_boundary(cylinder);
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        const double distance_v0 = distance_to_boundary(mesh.vertex(tetrahedron.vertex(0)));
        bool different_distance_from_v0 = false;
        for (int i = 1; i < mesh.kVertexPerElement && !different_distance_from_v0; ++i) {
            different_distance_from_v0 =
                    distance_tolerance < abs(distance_v0 - distance_to_boundary(mesh.vertex(tetrahedron.vertex(i))));
        }
        ASSERT_TRUE(different_distance_from_v0) << "A tetrahedron has all vertices at the same distances to"
                                                   " the cylinder's surface.";
    }
    // C3. Each tetrahedron conforms to MA. Assume every mesh vertex is in
    //     the cylinder (B1 test above).
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        bool tetrahedron_conform_to_MA = IsTetrahedronRespectingMa(tetrahedron, mesh, cylinder, distance_tolerance);
        ASSERT_TRUE(tetrahedron_conform_to_MA) << "A tetrahedron does not conform to the medial axis of the "
                                                  "cylinder.";
    }
}
...
GTEST_TEST(MakeCylinderVolumeMeshWithMaTest, Long) {
    ...
    VolumeMesh<double> mesh = MakeCylinderVolumeMeshWithMa<double>(cylinder, resolution_hint);

    const int num_vertex_per_circle =
            std::max(3, static_cast<int>(ceil(2. * M_PI * cylinder.radius() / resolution_hint)));
...
GTEST_TEST(MakeCylinderVolumeMesh, CoarsestMesh) {
    ...
    auto mesh_coarse = MakeCylinderVolumeMesh<double>(Cylinder(radius, length), resolution_hint_above);
    ...
    auto mesh_fine = MakeCylinderVolumeMesh<double>(Cylinder(radius, length), resolution_hint_below);
...
GTEST_TEST(MakeCylinderVolumeMesh, VolumeConvergence) {
    ...
    auto mesh0 = MakeCylinderVolumeMesh<double>(Cylinder(radius, height), resolution_hint);
    ...
    for (int level = 1; level < 6; ++level) {
        resolution_hint /= 2.0;
        auto mesh = MakeCylinderVolumeMesh<double>(drake::geometry::Cylinder(radius, height), resolution_hint);
...
GTEST_TEST(MakeCylinderSurfaceMesh, GenerateSurface) {
    ...
    const Cylinder cylinder(radius, length);
    TriangleSurfaceMesh<double> surface_mesh = MakeCylinderSurfaceMesh<double>(cylinder, resolution_hint);
