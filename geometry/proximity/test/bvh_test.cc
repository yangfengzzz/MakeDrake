#include "geometry/proximity/bvh.h"
...
public:
    ...
    static BvType ComputeBoundingVolume(
            const MeshType& mesh,
            const typename std::vector<typename Bvh<BvType, MeshType>::CentroidPair>::iterator start,
            const typename std::vector<typename Bvh<BvType, MeshType>::CentroidPair>::iterator end) {
    ...
    static bool Contain(const BvType& bv_M,
...
GTEST_TEST(BvNodeTest, TestEqualLeaf) {
    ...
    BvNode<Obb, TriangleSurfaceMesh<double>> leaf_of_two_elements(bv, {2, {0, 1}});
    EXPECT_FALSE(leaf_of_one_element.EqualLeaf(leaf_of_two_elements));

    // Second element is different.
    BvNode<Obb, TriangleSurfaceMesh<double>> leaf_with_a_different_element(bv, {2, {0, 2}});
    EXPECT_FALSE(leaf_of_two_elements.EqualLeaf(leaf_with_a_different_element));

    // All elements are the same.
    BvNode<Obb, TriangleSurfaceMesh<double>> leaf_with_same_two_elements(bv, {2, {0, 1}});
    ...
    BvNode<Obb, TriangleSurfaceMesh<AutoDiffXd>> leaf_of_one_ad_element(bv, {1, {0}});
    ...
    BvNode<Aabb, TriangleSurfaceMesh<double>> aabb_leaf_of_one_element(Aabb(Vector3d::Zero(), Vector3d::Zero()),
                                                                       {1, {0}});
...
TYPED_TEST(BvhTest, TestComputeBoundingVolume) {
    ...
    const TriangleSurfaceMesh<double> mesh({{0, 1, 2}, {3, 4, 5}},
                                           {Vector3d(0, 0, 1), Vector3d(1, 0, 1), Vector3d(0, 1, 1), Vector3d(0, 0, -1),
                                            Vector3d(1, 0, -1), Vector3d(0, 1, -1)});
    ...
    const BvType bv =
            BvhTester::ComputeBoundingVolume<BvType, TriangleSurfaceMesh<double>>(mesh, upper.begin(), upper.end());
...
TYPED_TEST(BvhTest, TestBuildBvTree) {
    ...
    std::function<void(const BvNode<BvType, TriangleSurfaceMesh<double>>&, int)> check_node;
    check_node = [&check_node, &element_indices, num_elements, tree_height](
                         const BvNode<BvType, TriangleSurfaceMesh<double>>& node, int depth) {
...
TYPED_TEST(BvhTest, TestCopy) {
    ...
    std::function<void(const BvNode<BvType, TriangleSurfaceMesh<double>>&,
            ...
            check_copy;
    check_copy = [&check_copy](const BvNode<BvType, TriangleSurfaceMesh<double>>& orig,
...
TYPED_TEST(BvhTest, TestCollide) {
    ...
    std::vector<std::pair<int, int>> pairs = this->bvh_.GetCollisionCandidates(separate, X_WV);
...
TYPED_TEST(BvhTest, TestCollideEarlyExit) {
    ...
    auto callback = [&count, &limit](int a, int b) -> BvttCallbackResult {
        ++count;
        return count < limit ? BvttCallbackResult::Continue : BvttCallbackResult::Terminate;
    };
...
TYPED_TEST(BvhTest, TestCollideSurfaceVolume) {
    ...
    const TriangleSurfaceMesh<double> mesh_S(std::move(faces_S), std::move(vertices_S));
...
TYPED_TEST(BvhTest, TestCollidePrimitive) {
    ...
    if constexpr (std::is_same_v<BvType, Obb>) {
        ...
        // All collision candidates.
        {
            ...
            this->bvh_.Collide(plane_P, RigidTransformd::Identity(), [&candidates](int f) -> BvttCallbackResult {
...
TYPED_TEST(BvhTest, TestComputeCentroid) {
    ...
    auto surface_mesh = MakeEllipsoidSurfaceMesh<double>(Ellipsoid(1., 2., 3.), 6);
    Vector3d centroid = BvhTester::ComputeCentroid<BvType, TriangleSurfaceMesh<double>>(surface_mesh, 0);
    ...
    centroid = BvhTester::ComputeCentroid<BvType, VolumeMesh<double>>(volume_mesh, 0 /* tet_index */);
...
TYPED_TEST(BvhTest, TestEqual) {
    ...
    // bounding volumes are not equal. Each tree has only one node.
    {
        const VolumeMesh<double> smaller_tetrahedron(
                ...
                std::vector<Vector3<double>>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(),
                                             Vector3d::UnitZ()});
        const Bvh<BvType, VolumeMesh<double>> smaller(smaller_tetrahedron);
        const VolumeMesh<double> bigger_tetrahedron(
                std::vector<VolumeElement>{VolumeElement(0, 1, 2, 3)},
                std::vector<Vector3<double>>{Vector3d::Zero(), 2. * Vector3d::UnitX(), 2. * Vector3d::UnitY(),
                                             2. * Vector3d::UnitZ()});
    ...
    {
        const std::vector<Vector3<double>> vertices{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(),
                                                    Vector3d::UnitZ()};
        const VolumeElement element(0, 1, 2, 3);
        const VolumeMesh<double> one_tetrahedron(std::vector<VolumeElement>{element}, std::vector<Vector3d>(vertices));
        ...
        const VolumeMesh<double> duplicated_tetrahedra(
                ...
                std::vector<VolumeElement>{element, element}, std::vector<Vector3d>(vertices));
    ...
    // enough depth.
    const TriangleSurfaceMesh<double> mesh_ellipsoid = MakeEllipsoidSurfaceMesh<double>(Ellipsoid(1., 2., 3.), 6.);
    ASSERT_EQ(8, mesh_ellipsoid.num_triangles());
    const Bvh<BvType, TriangleSurfaceMesh<double>> bvh_ellipsoid(mesh_ellipsoid);
    const Bvh<BvType, TriangleSurfaceMesh<double>> bvh_ellipsoid_too(mesh_ellipsoid);
    ...
    ASSERT_TRUE(bvh_ellipsoid.root_node().bv().Equal(bvh_ellipsoid_too.root_node().bv()));
...
TYPED_TEST(BvhTest, BvhFromAutodiffmesh) {
    using BvType = TypeParam;
    TriangleSurfaceMesh<AutoDiffXd> mesh_ad = MakeSphereSurfaceMesh<AutoDiffXd>(Sphere(1.5), 3);
    Bvh<BvType, TriangleSurfaceMesh<AutoDiffXd>> bvh_ad(mesh_ad);
    TriangleSurfaceMesh<double> mesh_d = MakeSphereSurfaceMesh<double>(Sphere(1.5), 3);
...
GTEST_TEST(BoundingVolumeHierarchyTest, CollideDifferentBvTypes) {
    TriangleSurfaceMesh<double> mesh = MakeSphereSurfaceMesh<double>(Sphere(1.5), 3);
...
GTEST_TEST(BoundingVolumeHierarchyTest, CandidatesDifferentBvTypes) {
    ...
    EXPECT_GT(aabb_bvh.GetCollisionCandidates(obb_bvh, RigidTransformd{}).size(), 0);
    EXPECT_EQ(aabb_bvh.GetCollisionCandidates(obb_bvh, RigidTransformd{Vector3d{100, 0, 0}}).size(), 0);

    EXPECT_GT(obb_bvh.GetCollisionCandidates(aabb_bvh, RigidTransformd{}).size(), 0);
    EXPECT_EQ(obb_bvh.GetCollisionCandidates(aabb_bvh, RigidTransformd{Vector3d{100, 0, 0}}).size(), 0);
}
