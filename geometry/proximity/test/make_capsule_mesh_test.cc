...
void VerifyCapsuleMeshWithMa(const VolumeMesh<double>& mesh, const Capsule& capsule) {
    // A. The mesh is conforming.
    ...
    // B. The mesh conforms to the capsule.
    const double tolerance = DistanceToPointRelativeTolerance(std::max(capsule.length() / 2., capsule.radius()));
    for (const Vector3d& v : mesh.vertices()) {
        ASSERT_TRUE(CalcDistanceToSurface(capsule, v) - tolerance <= 0) << "A mesh vertex is outside the capsule.";
    }

    // C. The mesh conforms to the capsule's medial axis.
    // C1. No tetrahedron has all four vertices on the capsule's boundary, i.e.,
    //     each tetrahedron has at least one interior vertex.
    const std::vector<int> boundary_vertices = CollectUniqueVertices(IdentifyBoundaryFaces(mesh.tetrahedra()));
    ...
    for (const VolumeElement& tetrahedron : mesh.tetrahedra()) {
        const bool tetrahedron_conform_to_MA = IsTetrahedronRespectingMa(tetrahedron, mesh, capsule, tolerance);
...
GTEST_TEST(MakeCapsuleVolumeMesTest, Long) {
    ...
    const VolumeMesh<double> mesh = MakeCapsuleVolumeMesh<double>(capsule, resolution_hint);
...
GTEST_TEST(MakeCapsuleVolumeMesh, CoarsestMesh) {
    ...
    const VolumeMesh<double> mesh_coarse =
            MakeCapsuleVolumeMesh<double>(Capsule(radius, length), resolution_hint_above);
    // V = 2n⌊n/2⌋ + 4
...
GTEST_TEST(MakeCapsuleVolumeMesh, SecondCoarsestMesh) {
    ...
    const VolumeMesh<double> mesh_finer = MakeCapsuleVolumeMesh<double>(Capsule(radius, length), resolution_hint_below);
    // V = 2n⌊n/2⌋ + 4
...
GTEST_TEST(MakeCapsuleVolumeMesh, FinestMesh) {
    ...
    const VolumeMesh<double> mesh_finest = MakeCapsuleVolumeMesh<double>(Capsule(radius, length), resolution_hint);
    // V = 2n⌊n/2⌋ + 4
...
GTEST_TEST(MakeCapsuleVolumeMesh, VolumeConvergence) {
    ...
    const VolumeMesh<double> mesh0 = MakeCapsuleVolumeMesh<double>(Capsule(radius, height), resolution_hint);

    const double volume0 = mesh0.CalcVolume();
    // Volume of the two hemispheres + volume of the cylinder
    const double capsule_volume = M_PI * radius * radius * ((4. / 3.) * radius + height);
    ...
    for (int level = 1; level < 6; ++level) {
        resolution_hint /= 2.0;
        const VolumeMesh<double> mesh =
                MakeCapsuleVolumeMesh<double>(drake::geometry::Capsule(radius, height), resolution_hint);
...
GTEST_TEST(MakeCapsuleSurfaceMesh, GenerateSurface) {
    ...
    const Capsule capsule(radius, length);
    const TriangleSurfaceMesh<double> surface_mesh = MakeCapsuleSurfaceMesh<double>(capsule, resolution_hint);
