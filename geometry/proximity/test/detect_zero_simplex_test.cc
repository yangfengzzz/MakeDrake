...
GTEST_TEST(DetectNullTetrahedronTest, TestOneNullTetrahedron) {
    ...
    const VolumeMesh<double> test_mesh(
            ...
            std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()});

    std::vector<int> null_tetrahedron = DetectTetrahedronWithAllBoundaryVertices(test_mesh);
...
GTEST_TEST(DetectNullTetrahedronTest, TestNoNullTetrahedron) {
    ...
    //
    const VolumeMesh<double> test_mesh(
            std::vector<VolumeElement>{{0, 1, 2, 4}, {0, 3, 1, 4}, {3, 2, 1, 4}, {3, 0, 2, 4}},
            std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ(),
...
GTEST_TEST(DetectNullInteriorTriangleTest, TestOneNullInteriorTriangleTest) {
    ...
    //      -Z
    //
    const VolumeMesh<double> test_mesh(std::vector<VolumeElement>{{0, 1, 2, 3}, {0, 2, 1, 4}},
                                       std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(),
                                                             Vector3d::UnitZ(), -Vector3d::UnitZ()});

    std::vector<SortedTriplet<int>> null_triangles = DetectInteriorTriangleWithAllBoundaryVertices(test_mesh);
...
GTEST_TEST(DetectNullInteriorEdgeTest, TestOneNullInteriorEdgeTest) {
    ...
    //    +X
    //
    const VolumeMesh<double> test_mesh(std::vector<VolumeElement>{{0, 1, 2, 3}, {0, 1, 3, 4}, {0, 1, 4, 2}},
                                       std::vector<Vector3d>{Vector3d::Zero(), Vector3d::UnitX(), Vector3d::UnitY(),
                                                             Vector3d::UnitZ(), Vector3d(0, -1, -1)});

    std::vector<SortedPair<int>> null_edges = DetectInteriorEdgeWithAllBoundaryVertices(test_mesh);
