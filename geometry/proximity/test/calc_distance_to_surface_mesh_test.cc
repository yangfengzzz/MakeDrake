#include "geometry/proximity/calc_distance_to_surface_mesh.h"
...
GTEST_TEST(CalcDistanceToSurfaceMeshTest, SingleTriangle) {
    ...
    const TriangleSurfaceMesh<double> mesh_W(std::move(triangles), std::move(vertices));
    ...
    // Region 0.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            EXPECT_NEAR(CalcDistanceToTxyPlane(p_TQ), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 1.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_TQ(0.25, -0.25, sign_z * kZScale);
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToLine(p_TQ, {p_TA, p_TB}), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 2.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_TQ(-0.25, -0.25, sign_z * kZScale);
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToPoint(p_TQ, p_TA), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 3.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToLine(p_TQ, {p_TA, p_TC}), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 4.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToPoint(p_TQ, p_TC), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 5.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToLine(p_TQ, {p_TB, p_TC}), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
    // Region 6.
    {
        for (int sign_z : {-1, 0, 1}) {
            ...
            const Vector3d p_WQ = X_WT * p_TQ;
            EXPECT_NEAR(CalcDistanceToPoint(p_TQ, p_TB), CalcDistanceToSurfaceMesh(p_WQ, mesh_W, bvh_W), kEps);
        }
    }
}
...
GTEST_TEST(CalcDistanceToSurfaceMeshTest, MultipleTriangles2) {
    ...
    vector<Vector3d> vertices = {X_WT * p_TA, X_WT * p_TB, X_WT * p_TC, X_WT * p_TD};
    ...
    const double d_Q_expected = CalcDistanceToLine(p_TQ, {Vector3d::UnitX(), Vector3d::UnitZ()});
    const Vector3d p_WQ = X_WT * p_TQ;
...
GTEST_TEST(CalcDistanceToSurfaceMeshTest, ManyTrianglesForRecursion) {
    ...
    const TriangleSurfaceMesh<double> surface_W =
            MakeBoxSurfaceMesh<double>(Box(kLongLength, kShortLength, kShortLength), kResolution);
    ...
    const double distance_from_center = CalcDistanceToSurfaceMesh(Vector3d::Zero(), surface_W, bvh_W);
