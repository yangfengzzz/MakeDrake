...
GTEST_TEST(ComputeContactSurfaceDeformableRigid, NoContact) {
    ...
    const deformable::DeformableGeometry deformable_W(MakeSphereVolumeMesh<double>(
            unit_sphere, 10.0 /* very coarse resolution */, TessellationStrategy::kDenseInteriorVertices));
    ...
    const TriangleSurfaceMesh<double> rigid_mesh_R =
            MakeBoxSurfaceMesh<double>(Box::MakeCube(2.0), 10.0 /* very coarse resolution */);
    ...
    math::RigidTransform<double> X_WR(Vector3<double>{4.0 / 3 + kTol, 4.0 / 3 + kTol, 4.0 / 3 + kTol});

    // Verify that the two BVHs have indeed collided.
    bool bvhs_collide = false;
    deformable_W.deformable_mesh().bvh().Collide(rigid_bvh_R, X_WR, [&bvhs_collide](int, int) {
        bvhs_collide = true;
    ...
    contact_data.RegisterDeformableGeometry(deformable_id, deformable_W.deformable_mesh().mesh().num_vertices());
    AddDeformableRigidContactSurface(deformable_W.CalcSignedDistanceField(), deformable_W.deformable_mesh(),
...
GTEST_TEST(ComputeContactSurfaceDeformableRigid, OnePolygon) {
    ...
    const VolumeMesh<double> single_tetrahedron_mesh_W(
            {{0, 1, 2, 3}},
            {Vector3<double>::Zero(), Vector3<double>::UnitX(), Vector3<double>::UnitY(), Vector3<double>::UnitZ()});
    const deformable::DeformableGeometry deformable_W(std::move(single_tetrahedron_mesh_W));

    const GeometryId rigid_id = GeometryId::get_new_id();
    // Single-triangle surface mesh with the triangle large enough to intersect
    // the tetrahedron well.
    const TriangleSurfaceMesh<double> rigid_mesh_R(
            {{0, 1, 2}}, {Vector3<double>(-1, -1, 0), Vector3<double>(3, -1, 0), Vector3<double>(-1, 3, 0)});
    const Bvh<Obb, TriangleSurfaceMesh<double>> rigid_bvh_R(rigid_mesh_R);
    ...
    ASSERT_EQ(contact_surface.signed_distances().size(), kExpectedNumContactPoints);
    const double signed_distance_at_contact_point = contact_surface.signed_distances()[0];
    EXPECT_NEAR(signed_distance_at_contact_point, 0.0, std::numeric_limits<double>::epsilon());
    // The centroid is (1/6, 1/6, 1/2).
    const Vector3<double> contact_point_W = contact_surface.contact_points_W()[0];
    constexpr double kTol = 1e-14;
    EXPECT_TRUE(CompareMatrices(contact_point_W, Vector3<double>(1.0 / 6, 1.0 / 6, 1.0 / 2), kTol));
    // The indexes of vertices incident to the only tetrahedron containing the
    // only contact point.
    const Vector4<int> vertex_indexes = contact_surface.contact_vertex_indexes_A()[0];
    ...
    // 1/6, 1/2).
    ASSERT_EQ(contact_surface.barycentric_coordinates_A().size(), kExpectedNumContactPoints);
    ...
    const ContactParticipation& contact_participation = contact_data.contact_participation(deformable_id);
...
GTEST_TEST(ComputeContactSurfaceDeformableRigid, OnlyRelativePoseMatters) {
    ...
    const math::RigidTransform<double> arbitrary_transform(math::RollPitchYaw<double>(0, 0, 0),
                                                           Vector3<double>(1.2, 3.4, 5.6));
    ...
    contact_data2.RegisterDeformableGeometry(deformable_id, mesh_W.num_vertices());
    AddDeformableRigidContactSurface(deformable_W.CalcSignedDistanceField(), deformable_W.deformable_mesh(),
                                     deformable_id, rigid_id, rigid_mesh_R, rigid_bvh_R, X_WR, &contact_data2);
    ASSERT_EQ(contact_data2.contact_surfaces().size(), 1);
    const DeformableContactSurface<double>& contact_surface2 = contact_data2.contact_surfaces()[0];

    EXPECT_EQ(contact_surface.num_contact_points(), contact_surface2.num_contact_points());
    /* Verify that penetration distances are not all equal simply because they are
     all zero -- some meaningful values do exist. */
    const auto has_negative_distance = [](const DeformableContactSurface<double>& surface) {
