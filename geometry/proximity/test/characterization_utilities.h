...
public:
    ...
    virtual bool Invoke(fcl::CollisionObjectd*,
...
/* The types of geometries that can be considered for characterization tests.
 ...
 drake::Shape). */
enum GeometryType { kBox, kCapsule, kConvex, kCylinder, kEllipsoid, kHalfSpace, kMesh, kPoint, kSphere };
...
public:
    ...
    std::shared_ptr<fcl::CollisionGeometry<double>> object() const { return object_; }
...
template <typename T>
math::RigidTransform<T> AlignPlanes(const Vector3<T>& P, const Vector3<T>& m, const Vector3<T>& Q, const Vector3<T>& n);
...
public:
     ...
     */
    explicit CharacterizeResultTest(std::unique_ptr<DistanceCallback<T>> callback) : callback_(std::move(callback)) {}
    ...
    void RunCallback(const QueryInstance& query,
    ...
    virtual std::vector<Configuration<T>> MakeConfigurations(const Shape& shape_A,
                                                             const Shape& shape_B,
    ...
    void RunCharacterization(const QueryInstance& query, bool is_symmetric = true);
...
namespace fmt {
template <>
struct formatter<drake::geometry::internal::GeometryType> : drake::ostream_formatter {};
}  // namespace fmt
