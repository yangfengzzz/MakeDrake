...
public:
    ...
    PointShapeAutoDiffSignedDistanceTester(const Shape* shape, const RigidTransformd& X_WG, double tolerance)
    ...
    ::testing::AssertionResult Test(const Vector3d& p_GN_G,
        ...
        DistanceToPoint<AutoDiffXd> distance_to_point(GeometryId::get_new_id(), X_WG_.cast<AutoDiffXd>(), p_WQ_ad);
        ...
        if (std::abs(result.distance.value() - expected_distance) > tolerance_) {
            ...
            failure << "The difference between expected distance and tested distance "
                    ...
                    << "  difference: " << (std::abs(result.distance.value() - expected_distance));
        }
        ...
        if (grad_W_val.array().isNaN().any()) {
            if (error) failure << "\n";
            error = true;
            failure << fmt::format("Analytical gradient contains NaN: {}", fmt_eigen(grad_W_val.transpose()));
        }
        auto gradient_compare = CompareMatrices(ddistance_dp_WQ, grad_W_val, tolerance_);
        if (!gradient_compare) {
            if (error) failure << "\n";
            error = true;
        ...
        if (grad_W_squared_norm.derivatives().size() > 0) {
            auto grad_W_unit_length_derivative_compare =
                    CompareMatrices(grad_W_squared_norm.derivatives(), Eigen::VectorXd::Zero(grad_size), tolerance_);
        ...
        const Vector3<AutoDiffXd> p_WQ_ad_expected = p_WN_ad + result.distance * result.grad_W;
        auto p_WQ_val_compare =
                CompareMatrices(math::ExtractValue(p_WQ_ad), math::ExtractValue(p_WQ_ad_expected), tolerance_);
        if (!p_WQ_val_compare) {
            if (error) failure << "\n";
            error = true;
        ...
        // unique.
        if (is_grad_W_unique) {
            auto p_WQ_derivative_compare = CompareMatrices(math::ExtractGradient(p_WQ_ad),
...
GTEST_TEST(DistanceToPoint, Box) {
    ...
    const RotationMatrix<double> R_WG(AngleAxis<double>(M_PI / 5, Vector3d{1, 2, 3}.normalized()));
        ...
        for (double x : {-1, 1}) {
            for (double y : {-1, 1}) {
                for (double z : {-1, 1}) {
                    ...
                    EXPECT_TRUE(tester.Test(p_GN_G, Vector3d::Zero(), false /* not inside the box */,
...
GTEST_TEST(DistanceToPoint, Capsule) {
    ...
    PointShapeAutoDiffSignedDistanceTester<fcl::Capsuled> tester(&capsule, X_WG, kEps);
    ...
    const Vector3d p_GN_Gs[3] = {capsule.radius * vhat_NQ_Gs[0] + Vector3d{0, 0, capsule.lz / 2},
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // Case: point lies on the capsule's spine.
        {
            ...
            EXPECT_TRUE(tester.Test(p_GN_G, p_NQ_G, true /* is inside */, false /* gradient ill defined */));
        }
    }
}
...
GTEST_TEST(DistanceToPoint, Ellipsoid) {
    ...
    for (const auto& coord : coords) {
        ...
        for (const double distance : {-0.125, 0.0, 0.2}) {
            ...
            SCOPED_TRACE(fmt::format("theta = {}, phi = {}, distance = {}\n  p_GQ: {} {} {}", coord.theta, coord.phi,
                                     distance, p_GQ.x(), p_GQ.y(), p_GQ.z()));
            ...
            EXPECT_TRUE(CompareMatrices(result.grad_W, X_WG.rotation() * n_G, kVectorTolerance));
        }
    }
    ...
    EXPECT_TRUE(CompareMatrices(result.p_GN.cwiseAbs(), min_axis_G * min_radius, kVectorTolerance));
    EXPECT_TRUE(CompareMatrices((X_WG.rotation().inverse() * result.grad_W).cwiseAbs(), min_axis_G, kVectorTolerance));
}
...
GTEST_TEST(DistanceToPoint, Halfspace) {
    ...
    PointShapeAutoDiffSignedDistanceTester<fcl::Halfspaced> tester(&hs, X_WG, kEps);
...
GTEST_TEST(DistanceToPoint, Sphere) {
    ...
    PointShapeAutoDiffSignedDistanceTester<fcl::Sphered> tester(&sphere, X_WG, kEps);
    ...
    EXPECT_TRUE(tester.Test(p_GN_G, -sphere.radius * vhat_NQ, true /* is inside */, false /* gradient ill defined */));
}
...
int ExpectedResult() {
    ...
    if constexpr (std::is_same_v<T, AutoDiffXd>) {
        if (std::is_same_v<S, fcl::Cylinderd> || std::is_same_v<S, fcl::Ellipsoidd>) {
            return 0;
        }
        return 1;
    }
...
void TestScalarShapeSupport() {
    ...
    std::unordered_map<GeometryId, RigidTransform<T>> X_WGs{{point_id, X_WQ},
                                                            {other_id, RigidTransform<T>::Identity()}};
    ...
    auto run_callback = [&query_point, &threshold, &distances, &data, other_id](auto geometry_shared_ptr) {
