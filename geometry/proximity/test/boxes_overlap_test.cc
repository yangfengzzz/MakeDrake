#include "geometry/proximity/boxes_overlap.h"
...
#include "geometry/test_utilities/boxes_overlap_transforms.h"
...
protected:
    ...
    static bool InvokeBoxesOverlap(const Vector3d& a_half,
                                   const Vector3d& b_half,
    ...
    static void DrawCase(const Vector3d& a_half,
                         const Vector3d& b_half,
                         const RigidTransformd& X_AB,
...
TEST_F(BoxesOverlapTest, AllCases) {
    ...
    for (int axis = 0; axis < 3; ++axis) {
        ...
        EXPECT_FALSE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("A{} separated", axes[axis])));
        X_AB = CalcCornerTransform(a, b, axis, true /* expect_overlap */);
        EXPECT_TRUE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("A{} colliding", axes[axis])));
    }
    ...
    for (int axis = 0; axis < 3; ++axis) {
        X_AB = CalcCornerTransform(b, a, axis, false /* expect_overlap */).inverse();
        EXPECT_FALSE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("B{} separated", axes[axis])));
        X_AB = CalcCornerTransform(b, a, axis, true /* expect_overlap */).inverse();
        EXPECT_TRUE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("B{} colliding", axes[axis])));
    }
    ...
    for (int a_axis = 0; a_axis < 3; ++a_axis) {
        for (int b_axis = 0; b_axis < 3; ++b_axis) {
            X_AB = CalcEdgeTransform(a, b, a_axis, b_axis, false /* expect_overlap */);
            // Separate along a's y-axis and b's x-axis.
            EXPECT_FALSE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("A{}-B{} separated", axes[a_axis], axes[b_axis])));
            X_AB = CalcEdgeTransform(a, b, a_axis, b_axis, true /* expect_overlap */);
            // Separate along a's y-axis and b's x-axis.
            EXPECT_TRUE(InvokeBoxesOverlap(a, b, X_AB, fmt::format("A{}-B{} colliding", axes[a_axis], axes[b_axis])));
        }
    }
}
