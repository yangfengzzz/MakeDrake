#include "geometry/proximity/bvh_updater.h"
...
using MeshTypes = ::testing::
        Types<TriangleSurfaceMesh<double>, TriangleSurfaceMesh<AutoDiffXd>, VolumeMesh<double>, VolumeMesh<AutoDiffXd>>;
...
TYPED_TEST(BvhUpdaterTest, Update) {
    ...
    const Aabb expected_root_bv(Vector3d{0, -0.5, 0.5}, Vector3d{dist + 1, 0.5, 0.5});
    const Aabb expected_left_bv(Vector3d{-dist - 0.5, -0.5, 0.5}, Vector3d{0.5, 0.5, 0.5});
    const Aabb expected_right_bv(Vector3d{dist + 0.5, -0.5, 0.5}, Vector3d{0.5, 0.5, 0.5});

    /* Test the domains of the bounding boxes. */
    ASSERT_TRUE(CompareMatrices(root.bv().center(), expected_root_bv.center()));
    ASSERT_TRUE(CompareMatrices(root.bv().half_width(), expected_root_bv.half_width()));

    ASSERT_TRUE(CompareMatrices(left.bv().center(), expected_left_bv.center()));
    ASSERT_TRUE(CompareMatrices(left.bv().half_width(), expected_left_bv.half_width()));

    ASSERT_TRUE(CompareMatrices(right.bv().center(), expected_right_bv.center()));
    ASSERT_TRUE(CompareMatrices(right.bv().half_width(), expected_right_bv.half_width()));
    ...
    EXPECT_TRUE(CompareMatrices(root.bv().center(), R * expected_root_bv.center().cast<T>(), kEps));
    EXPECT_TRUE(CompareMatrices(root.bv().half_width(), (R * expected_root_bv.half_width().cast<T>()).cwiseAbs(),
                                2 * kEps));
    EXPECT_TRUE(CompareMatrices(left.bv().center(), R * expected_left_bv.center().cast<T>(), kEps));
    EXPECT_TRUE(CompareMatrices(left.bv().half_width(), (R * expected_left_bv.half_width().cast<T>()).cwiseAbs(),
                                2 * kEps));
    EXPECT_TRUE(CompareMatrices(right.bv().center(), R * expected_right_bv.center().cast<T>(), kEps));
    EXPECT_TRUE(CompareMatrices(right.bv().half_width(), (R * expected_right_bv.half_width().cast<T>()).cwiseAbs(),
                                2 * kEps));
}
