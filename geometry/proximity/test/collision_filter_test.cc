...
::testing::AssertionResult ExpectCanCollide(const CollisionFilter& filter,
    ...
    if (result1 != result2) {
        return ::testing::AssertionFailure()
               << "The collision filter state for " << id_A << " and " << id_B << " changed based on ordering";
    }
    if (result1 != expect_collidable) {
        return ::testing::AssertionFailure()
               << "For pair (" << id_A << ", " << id_B << "), our expectation for can-collide was " << expect_collidable
...
protected:
    ...
    static std::tuple<GeometryId, GeometryId, GeometryId> InitIds(CollisionFilter* filter) {
        vector<GeometryId> ids{GeometryId::get_new_id(), GeometryId::get_new_id(), GeometryId::get_new_id()};
        for (auto id : ids) {
    ...
    static void FilterAllPairs(CollisionFilter* filter, std::initializer_list<GeometryId> ids, bool is_invariant) {
        filter->Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet(ids)), get_extract_ids_functor(),
                      is_invariant);
    ...
    static CollisionFilter::ExtractIds get_extract_ids_functor() { return &GeometrySetTester::geometries; }

    static CollisionFilter ClearCopy(const CollisionFilter& filter) { return filter.MakeClearCopy(); }
};
...
TEST_F(CollisionFilterTest, AllowBetween) {
    for (bool is_invariant : {true, false}) {
        ...
        filters.Apply(CollisionFilterDeclaration().AllowBetween(GeometrySet(id_A), GeometrySet({id_B, id_C})),
...
TEST_F(CollisionFilterTest, ExcludeBetween) {
    ...
    filters.Apply(CollisionFilterDeclaration().ExcludeBetween(GeometrySet(id_A), GeometrySet({id_B, id_C})),
                  this->get_extract_ids_functor());
...
TEST_F(CollisionFilterTest, TransientDeclarations) {
    ...
    auto is_expected = [this](const CollisionFilter& f, const std::set<Pair>& expected) -> ::testing::AssertionResult {
        CollisionFilter temp_filter = this->ClearCopy(f);
        for (const auto& pair : expected) {
            temp_filter.Apply(Decl().ExcludeWithin(GeometrySet({pair.first, pair.second})),
    ...
    filter.Apply(Decl().ExcludeBetween(GeometrySet{B}, GeometrySet{C, D, E}), extract, !invariant);
    ...
    /* Prop. H: Add a redundant declaration, so now we *know* we're testing a
     filter *with* history against one without. Filtered pairs are still:
      P(D, E), (B, C), (B, D), (B, E) */
    const FilterId id0 =
            filter.ApplyTransient(CollisionFilterDeclaration().ExcludeBetween(GeometrySet{B}, GeometrySet{{C, D, E}}),
                                  this->get_extract_ids_functor());
    ...
    /* Prop. A.2 and A.3: Add new filter (C, D), remove old, non-invariant
     filter (B, D). Filtered pairs become:
      P(D, E), (B, C), (B, E), (C, D) */
    const FilterId id1 =
            filter.ApplyTransient(Decl().ExcludeWithin(GeometrySet{C, D}).AllowWithin(GeometrySet{B, D}), extract);
    ...
    Decl add_2_clear_2;
    add_2_clear_2.ExcludeBetween(GeometrySet(A), GeometrySet{B, C}).AllowBetween(GeometrySet(B), GeometrySet{C, E});
    ...
    /* Add a further declaration that is *partially* redundant w.r.t. the previous
     declaration. We'll continue to filter (A, B) and continue to allow (B, E):
      P(D, E), (A, B), (A, C), (C, D) */
    const FilterId id4 =
            filter.ApplyTransient(Decl().ExcludeWithin(GeometrySet{A, B}).AllowWithin(GeometrySet{B, E}), extract);
    ...
    /* Prop. A.1: Attempting to remove invariant filter makes no difference:
      P(D, E), (A, B), (B, C), (C, D) */
    const FilterId id5 = filter.ApplyTransient(Decl().AllowWithin(GeometrySet{D, E}), extract);
...
TEST_F(CollisionFilterTest, Equality) {
    ...
    filters1.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_A, id_B})),
                   this->get_extract_ids_functor());
    EXPECT_FALSE(filters1 == filters2);
    EXPECT_TRUE(filters1 != filters2);
    filters2.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_A, id_B})),
                   this->get_extract_ids_functor());
    EXPECT_TRUE(filters1 == filters2);
    EXPECT_FALSE(filters1 != filters2);
    filters1.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_A, id_C})),
                   this->get_extract_ids_functor());
    EXPECT_FALSE(filters1 == filters2);
    EXPECT_TRUE(filters1 != filters2);
    filters2.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_B, id_C})),
                   this->get_extract_ids_functor());
    EXPECT_FALSE(filters1 == filters2);
    EXPECT_TRUE(filters1 != filters2);
    filters1.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_B, id_C})),
                   this->get_extract_ids_functor());
    filters2.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id_A, id_C})),
                   this->get_extract_ids_functor());
    EXPECT_TRUE(filters1 == filters2);
    EXPECT_FALSE(filters1 != filters2);
    filters1.Apply(CollisionFilterDeclaration().AllowWithin(GeometrySet({id_A, id_C})),
                   this->get_extract_ids_functor());
    EXPECT_FALSE(filters1 == filters2);
    EXPECT_TRUE(filters1 != filters2);
    ...
    filters2.Apply(CollisionFilterDeclaration().AllowBetween(GeometrySet(id_A), GeometrySet({id_B, id_C})),
                   this->get_extract_ids_functor());
    EXPECT_FALSE(filters1 == filters2);
    EXPECT_TRUE(filters1 != filters2);
}
