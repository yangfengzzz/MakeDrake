...
TYPED_TEST(MakeVolumeMeshPressureFieldTest, PressureOnNonConvexMesh) {
    ...
    VolumeMesh<T> non_convex_mesh =
            MakeVolumeMeshFromVtk<T>(Mesh(FindResourceOrThrow("drake/geometry/test/non_convex_mesh.vtk")));

    const T kHydroelasticModulus = 1e7;
    VolumeMeshFieldLinear<T, T> field = MakeVolumeMeshPressureField(&non_convex_mesh, kHydroelasticModulus);
...
GTEST_TEST(MakeVolumeMeshPressureFieldTest, NoInteriorVertex) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeVolumeMeshPressureField(&mesh_without_interior_vertex, kHydroelasticModulus),
