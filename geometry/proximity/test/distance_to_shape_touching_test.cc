#include "geometry/proximity/distance_to_shape_touching.h"
...
GTEST_TEST(CalcGradientWhenTouching, SphereTouch) {
    ...
    const RigidTransformd X_WF(RollPitchYawd(M_PI, M_PI_2, M_PI_4), Vector3d(0.5, 1, 2));
    ...
    const fcl::CollisionObjectd a(make_shared<fcl::Sphered>(radius), X_WA.rotation().matrix(), X_WA.translation());
    const double rx = 2;
    const RigidTransformd X_FB(Vector3d(rx, 0, 0));
    const RigidTransformd X_WB = X_WF * X_FB;
    const fcl::CollisionObjectd b(make_shared<fcl::Ellipsoidd>(rx, 3, 4), X_WB.rotation().matrix(), X_WB.translation());
    ...
    EXPECT_EQ(CalcGradientWhenTouching(a, X_WA, b, X_WB, p_ACa, p_BCb), X_WF.rotation() * expect_nhat_BA_F);
    // Switching `a` and `b` flips the gradient.
    const Vector3d expect_nhat_AB_F = Vector3d::UnitX();
    EXPECT_EQ(CalcGradientWhenTouching(b, X_WB, a, X_WA, p_BCb, p_ACa), X_WF.rotation() * expect_nhat_AB_F);
}
...
GTEST_TEST(CalcGradientWhenTouching, box_touches_box) {
    ...
    const fcl::CollisionObjectd box_A(make_shared<Boxd>(width, 3, 4), X_WA.rotation().matrix(), X_WA.translation());
    const RigidTransformd X_WB(Vector3d(width / 2, 0, 0));
    const fcl::CollisionObjectd box_B(make_shared<Boxd>(width, 5, 6), X_WB.rotation().matrix(), X_WB.translation());
    ...
    EXPECT_EQ(CalcGradientWhenTouching(box_A, X_WA, box_B, X_WB, p_ACa, p_BCb), -Vector3d::UnitX());
}

void PointOnBoxSurfaceHelperTester(const fcl::Boxd& box_B,
...
GTEST_TEST(PointInBox, PointOnBoxSurfaceHelper) {
    ...
    }
    {
        ...
        PointOnBoxSurfaceHelperTester(box_B, Vector3d(-1, 0.02, 0.03), 1, {}, -Vector3d::UnitX());
    }
    {
        SCOPED_TRACE("This point is strictly on +Y face of the box.");
        PointOnBoxSurfaceHelperTester(box_B, Vector3d(0.01, 2, -0.03), 1, {}, Vector3d::UnitY());
    }
    {
        SCOPED_TRACE("This point is strictly on -Z face of the box.");
        PointOnBoxSurfaceHelperTester(box_B, Vector3d(0.01, -0.02, -4), 1, {}, -Vector3d::UnitZ());
    }
    {
...
GTEST_TEST(SeparatingAxis, ProjectedMinMax) {
    ...
    // First we fix box_B's frame to World frame.
    {
        ...
        std::tie(min_value, max_value) = ProjectedMinMax(box_B, X_WB, unit_vector_W);
...
GTEST_TEST(SeparatingAxis, MaybeMakeSeparatingVector) {
    ...
    {
        const std::optional<Vector3d> nhat_BA_W =
                MaybeMakeSeparatingVector(box_A, box_B, X_WA, X_WB, {Vector3d(0.1, 0, 0)});
    ...
    {
        const std::optional<Vector3d> nhat_BA_W =
                MaybeMakeSeparatingVector(box_B, box_A, X_WB, X_WA, {Vector3d(0.1, 0, 0)});
        EXPECT_TRUE(nhat_BA_W.has_value());
        EXPECT_EQ(nhat_BA_W.value(), Vector3d::UnitX());
    }
    // Non-separating direction gets null.
    EXPECT_FALSE(MaybeMakeSeparatingVector(box_B, box_A, X_WB, X_WA, {Vector3d(0, 1, 1)}).has_value());
    // Zero vector gets null.
    EXPECT_FALSE(MaybeMakeSeparatingVector(box_B, box_A, X_WB, X_WA, {Vector3d::Zero()}).has_value());
    // An empty list of vectors gets null.
    EXPECT_FALSE(MaybeMakeSeparatingVector(box_B, box_A, X_WB, X_WA, {}).has_value());
}
...
GTEST_TEST(BoxBoxGradient, Face) {
    ...
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_A, box_B, X_WA, X_WB, p_ACa, p_BCb), -Vector3d::UnitX(), 1e-14));
    // Switching A and B flips the gradient.
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_B, box_A, X_WB, X_WA, p_BCb, p_ACa), Vector3d::UnitX(), 1e-14));
}
...
GTEST_TEST(BoxBoxGradient, EdgeCrossEdge) {
    ...
    // each box.
    const RigidTransformd X_WB(AngleAxisd(M_PI_2, Vector3d(1, 1, 0)), Vector3d(2, 2, 0));
    ...
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_A, box_B, X_WA, X_WB, p_ACa, p_BCb),
                                Vector3d(-1, -1, 0).normalized(), 1e-14));
    // Switching A and B flips the gradient.
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_B, box_A, X_WB, X_WA, p_BCb, p_ACa), Vector3d(1, 1, 0).normalized(),
                                1e-14));
}
...
GTEST_TEST(BoxBoxGradient, ParallelEdgeEdge) {
    ...
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_A, box_B, X_WA, X_WB, p_ACa, p_BCb), -Vector3d::UnitX()));
    // Switching A and B flips the gradient.
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_B, box_A, X_WB, X_WA, p_BCb, p_ACa), Vector3d::UnitX()));
}
...
GTEST_TEST(BoxBoxGradient, EdgeVertex) {
    ...
    const fcl::Boxd box_A(2, 2, 2);
    using std::sqrt;
    const RigidTransformd X_WB(AngleAxisd(M_PI_4, Vector3d::UnitZ()), Vector3d(1 + 1 / sqrt(2), 1 + 1 / sqrt(2), 2));
    // box_B spans [-1,1]x[-1,1]x[-1,1].
    const fcl::Boxd box_B(2, 2, 2);
    ...
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_A, box_B, X_WA, X_WB, p_ACa, p_BCb), -Vector3d::UnitZ(), 1e-14));
    // Switching A and B flips the gradient.
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_B, box_A, X_WB, X_WA, p_BCb, p_ACa), Vector3d::UnitZ(), 1e-14));
}
...
GTEST_TEST(BoxBoxGradient, VertexVertexNeedCrossProduct) {
    ...
    const RigidTransformd X_WB(AngleAxisd(M_PI_4, Vector3d::Ones()), Vector3d(2, 2, 2));
    // box_B spans [-1,1]x[-1,1]x[-1,1].
    const fcl::Boxd box_B(2, 2, 2);
    // Witness points are at vertices.
    const Vector3d p_ACa(1, 1, 1);
    ...
    const Vector3d expect_gradient = -X_WA.rotation().col(0).cross(X_WB.rotation().col(0)).normalized();
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_A, box_B, X_WA, X_WB, p_ACa, p_BCb), expect_gradient, 1e-14));
    // Switching A and B flips the gradient.
    EXPECT_TRUE(CompareMatrices(BoxBoxGradient(box_B, box_A, X_WB, X_WA, p_BCb, p_ACa), -expect_gradient, 1e-14));
}
