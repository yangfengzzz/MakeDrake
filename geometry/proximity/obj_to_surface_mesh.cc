 ...
 */
std::vector<Vector3d> TinyObjToSurfaceVertices(const std::vector<tinyobj::real_t>& tinyobj_vertices,
                                               const double scale) {
 ...
 */
void TinyObjToSurfaceFaces(const tinyobj::mesh_t& mesh, std::vector<SurfaceTriangle>* faces) {
    //
    ...
    for (int face = 0; face < num_faces; ++face) {
        ...
        const int vertex_indices[3] = {mesh.indices[3 * face].vertex_index, mesh.indices[3 * face + 1].vertex_index,
...
std::optional<TriangleSurfaceMesh<double>> DoReadObjToSurfaceMesh(std::istream* input_stream,
    ...
    bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, input_stream, readMatFn.get(), triangulate);
    if (!ret || !err.empty()) {
        const std::string err_message = fmt::format("Error parsing Wavefront obj data : {}", err);
    ...
    if (config.allowed_shape_count > 0 && static_cast<int>(shapes.size()) > config.allowed_shape_count) {
        const std::string err_message = fmt::format("The Wavefront obj data defines {} unique shapes; only {} allowed.",
                                                    shapes.size(), config.allowed_shape_count);
        config.diagnostic.Error(err_message);
        return std::nullopt;
    }

    std::vector<Vector3d> vertices = TinyObjToSurfaceVertices(attrib.vertices, scale);
    ...
    // TriangleSurfaceMesh.
    int total_num_faces = std::accumulate(shapes.begin(), shapes.end(), 0, [](int sum, const tinyobj::shape_t& shape) {
...
TriangleSurfaceMesh<double> ReadObjToTriangleSurfaceMesh(const std::string& filename,
    ...
    return *internal::DoReadObjToSurfaceMesh(&input_stream, scale, mtl_basedir, {.diagnostic = policy});
}
...
TriangleSurfaceMesh<double> ReadObjToTriangleSurfaceMesh(std::istream* input_stream,
    ...
    // We will either throw or return a mesh here.
    return *internal::DoReadObjToSurfaceMesh(input_stream, scale, std::nullopt /* mtl_basedir */,
                                             {.diagnostic = policy});
}
