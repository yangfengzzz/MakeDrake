#include "geometry/proximity/volume_mesh_refiner.h"
...
VolumeMesh<double> VolumeMeshRefiner::Refine() {
    ...
    for (const SortedPair<int>& edge : DetectInteriorEdgeWithAllBoundaryVertices(
                 {std::vector<VolumeElement>(tetrahedra_), std::vector<Vector3d>(vertices_)})) {
        RefineEdge(edge);
    }
    for (const SortedTriplet<int>& triangle : DetectInteriorTriangleWithAllBoundaryVertices(
                 {std::vector<VolumeElement>(tetrahedra_), std::vector<Vector3d>(vertices_)})) {
    ...
    return {std::vector<VolumeElement>(tetrahedra_), std::vector<Vector3d>(vertices_)};
}
...
void VolumeMeshRefiner::RefineTetrahedron(int tetrahedron) {
    ...
    vertices_.emplace_back((vertices_.at(v0) + vertices_.at(v1) + vertices_.at(v2) + vertices_.at(v3)) / 4);
...
void VolumeMeshRefiner::RefineTriangle(const SortedTriplet<int>& triangle) {
    ...
    vertices_.emplace_back((vertices_.at(v0) + vertices_.at(v1) + vertices_.at(v2)) / 3);
    const int new_vertex = vertices_.size() - 1;
...
void VolumeMeshRefiner::CutTetrahedron(const int tetrahedron,
    ...
    const std::array<int, 4> original{tetrahedra_[tetrahedron].vertex(0), tetrahedra_[tetrahedron].vertex(1),
    ...
    for (int i = 0; i < num_sub_simplex_vertices; ++i) {
        int local_index = std::distance(original.begin(), std::find(original.begin(), original.end(), sub_simplex[i]));
...
std::vector<int> VolumeMeshRefiner::GetTetrahedraOnTriangle(int v0, int v1, int v2) const {
    ...
    for (int tetrahedron = 0; tetrahedron < num_tetrahedra; ++tetrahedron) {
        const std::unordered_set<int> tetrahedron_vertices{
                ...
                tetrahedra_[tetrahedron].vertex(2), tetrahedra_[tetrahedron].vertex(3)};
        if (tetrahedron_vertices.contains(v0) && tetrahedron_vertices.contains(v1) &&
