    ...
    for (int e = 0; e < read_mesh.num_elements(); ++e) {
        if (read_mesh.CalcTetrahedronVolume(e) <= 0.) {
            throw std::runtime_error(fmt::format(
                    ...
                    filename.string(), scale, e, read_mesh.element(e).vertex(0), read_mesh.element(e).vertex(1),
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeVolumeMeshFromVtk<T>));
