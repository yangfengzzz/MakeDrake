...
Vector3d CalcGradientWhenTouching(const fcl::CollisionObjectd& a,
    ...
    if (a.collisionGeometry()->getNodeType() == fcl::GEOM_SPHERE &&
        static_cast<const fcl::Sphered*>(a.collisionGeometry().get())->radius > 0) {
    ...
    if (b.collisionGeometry()->getNodeType() == fcl::GEOM_SPHERE &&
        static_cast<const fcl::Sphered*>(b.collisionGeometry().get())->radius > 0) {
    ...
    if (a.collisionGeometry()->getNodeType() == fcl::GEOM_BOX &&
        ...
        const auto& box_A = *static_cast<const fcl::Boxd*>(a.collisionGeometry().get());
        const auto& box_B = *static_cast<const fcl::Boxd*>(b.collisionGeometry().get());
...
std::optional<Vector3d> MaybeMakeSeparatingVector(const fcl::Boxd& box_A,
...
Vector3d BoxBoxGradient(const fcl::Boxd& box_A,
    ...
    // Edge-to-edge.
    if (s_A == 2 && s_B == 2) {
        const Vector3d cross_product_W = X_WA.rotation().col(axis_index_A).cross(X_WB.rotation().col(axis_index_B));
        if (cross_product_W.norm() > kEps) {
            // The two edges are not parallel.
            const std::optional<Vector3d> nhat_BA_W =
                    MaybeMakeSeparatingVector(box_A, box_B, X_WA, X_WB, {cross_product_W});
            ...
            const std::optional<Vector3d> nhat_BA_W = MaybeMakeSeparatingVector(
                    ...
                    {X_WA.rotation().col((axis_index_A + 1) % 3), X_WA.rotation().col((axis_index_A + 2) % 3)});
            if (nhat_BA_W.has_value()) {
                return nhat_BA_W.value();
            }
        }
    ...
    if (s_A == 2 && s_B == 3) {
        ...
        const std::optional<Vector3d> nhat_BA_W = MaybeMakeSeparatingVector(
                box_A, box_B, X_WA, X_WB,
                {edge_vector_A_W.cross(X_WB.rotation().col(0)), edge_vector_A_W.cross(X_WB.rotation().col(1)),
    ...
    if (s_A == 3 && s_B == 2) {
        ...
        const std::optional<Vector3d> nhat_BA_W = MaybeMakeSeparatingVector(
                box_A, box_B, X_WA, X_WB,
                {edge_vector_B_W.cross(X_WA.rotation().col(0)), edge_vector_B_W.cross(X_WA.rotation().col(1)),
    ...
    // cross products.
    std::optional<Vector3d> nhat_BA_W =
            MaybeMakeSeparatingVector(box_A, box_B, X_WA, X_WB,
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            nhat_BA_W = MaybeMakeSeparatingVector(box_A, box_B, X_WA, X_WB,
