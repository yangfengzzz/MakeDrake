...
public:
     ...
     */
    Plane(const Vector3<T>& n_F, const Vector3<T>& p_FP, bool already_normalized = false) {
        if (!already_normalized) {
            ...
            if (magnitude < 1e-10) {
                throw std::runtime_error(
                        fmt::format("Cannot instantiate plane from normal n_F = [{}]; its magnitude is "
                                    "too small: {}",
...
private:
    ...
    static void ThrowIfInsufficientlyNormal(const Vector3<T>& n) {
        ...
        if (delta > 1e-13) {
            throw std::runtime_error(
                    fmt::format("Plane constructed with a normal vector that was declared normalized;"
