 ...
 */
template <typename T>
VolumeMeshFieldLinear<T, T> MakeEllipsoidPressureField(const Ellipsoid& ellipsoid,
                                                       const VolumeMesh<T>* mesh_E,
    ...
    return VolumeMeshFieldLinear<T, T>(std::move(pressure_values), mesh_E, MeshGradientMode::kOkOrMarkDegenerate);
}
