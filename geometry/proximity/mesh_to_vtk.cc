...
void WriteVtkUnstructuredGrid(std::ofstream& out, const Mesh& mesh) {
    ...
    const int cell_type = (Mesh::kVertexPerElement == 3) ? kVtkCellTypeTriangle : kVtkCellTypeTetrahedron;
 ...
 */
template <typename Mesh>
void WriteMeshToVtk(const std::string& file_name, const Mesh& mesh, const std::string& title) {
...
void WriteVtkScalarField(std::ofstream& out, std::string name, const Field& field) {
