...
VolumeMesh<double> ReadVtkToVolumeMesh(const std::string& filename, double scale) {
    if (scale <= 0.0) {
        throw std::runtime_error(fmt::format("ReadVtkToVolumeMesh('{}', {}): scale={} is not a positive number",
    ...
    auto iter = vtkSmartPointer<vtkCellIterator>::Take(vtk_mesh->NewCellIterator());
    for (iter->InitTraversal(); !iter->IsDoneWithTraversal(); iter->GoToNextCell()) {
        if (iter->GetCellType() != VTK_TETRA) {
            ...
            auto msg = fmt::format(
                    ...
                    filename, scale, static_cast<int>(VTK_TETRA), bad_cell->GetCellType(), bad_cell->GetCellDimension(),
                    bad_cell->GetNumberOfPoints());
