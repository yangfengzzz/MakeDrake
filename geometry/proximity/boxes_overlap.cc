...
bool BoxesOverlap(const Vector3d& half_size_a, const Vector3d& half_size_b, const RigidTransformd& X_AB) {
    ...
    for (int i = 0; i < 3; ++i) {
        if (abs(t.dot(r.block<3, 1>(0, i))) > half_size_b[i] + half_size_a.dot(abs_r.block<3, 1>(0, i))) {
            return false;
        }
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            if (abs(t[i2] * r(i1, j) - t[i1] * r(i2, j)) >
                half_size_a[i1] * abs_r(i2, j) + half_size_a[i2] * abs_r(i1, j) + half_size_b[j1] * abs_r(i, j2) +
                        half_size_b[j2] * abs_r(i, j1)) {
                return false;
            }
            j1 = j2;
        }
