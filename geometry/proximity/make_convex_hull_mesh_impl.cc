...
std::vector<int> OrderPolyVertices(const std::vector<Vector3d>& vertices,
    ...
    for (int vi : v_indices) {
        ...
        scored_vertices.push_back({std::atan2((v_C0.cross(v_CI).dot(n)), v_C0.dot(v_CI)), vi});
    }

    /* Sort by increasing angle. */
    std::sort(scored_vertices.begin(), scored_vertices.end(), [](const VertexScore& a, const VertexScore& b) {
...
void ReadObjVertices(const fs::path filename, double scale, std::vector<Vector3d>* vertices) {
    const auto [tinyobj_vertices, _1, _2] =
            geometry::internal::ReadObjFile(std::string(filename), scale, /* triangulate = */ false);
...
void ReadVtkVertices(const fs::path filename, double scale, std::vector<Vector3d>* vertices) {
    const VolumeMesh<double> volume_mesh = ReadVtkToVolumeMesh(std::string(filename), scale);
...
void ReadGltfVertices(const fs::path filename, double scale, std::vector<Vector3d>* vertices) {
    ...
    if (renderer->VisibleActorCount() == 0) {
        throw std::runtime_error(
                fmt::format("MakeConvexHull() found no vertices in the file '{}'.", filename.string()));
    }
    ...
    while (vtkActor* actor = actors->GetNextActor()) {
        ...
        auto* poly_data = dynamic_cast<vtkPolyData*>(actor->GetMapper()->GetInput());
...
Vector3d FindNormal(const std::vector<Vector3d>& vertices, const fs::path filename) {
    ...
    if (v == ssize(vertices)) {
        throw std::runtime_error(
                fmt::format("MakeConvexHull failed because all vertices in the mesh appear to be "
                            "co-linear for file: {}.",
                            filename.string()));
    }
...
PolygonSurfaceMesh<double> MakeConvexHull(const std::filesystem::path mesh_file, double scale) {
    VertexCloud cloud = ReadVertices(mesh_file, scale);

    // The default mapping from qhull to convex hull mesh is simply a copy.
    std::function<Vector3d(double, double, double)> map_hull_vertex = [](double x, double y, double z) {
    ...
    for (auto& facet : qhull.facetList()) {
        ...
        std::transform(qhull_vertices.cbegin(), qhull_vertices.cend(), std::back_inserter(mesh_indices),
        ...
        std::vector<int> ordered_vertices = OrderPolyVertices(vertices_M, mesh_indices, center, normal);

        // Now populate the face data.
        face_data.push_back(ssize(ordered_vertices));
        face_data.insert(face_data.end(), ordered_vertices.begin(), ordered_vertices.end());
    }
