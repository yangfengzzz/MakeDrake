...
*/
template <typename T>
std::vector<VolumeElement> CalcLongCylinderVolumeMeshWithMa(const Cylinder& cylinder,
    ...
    const double tolerance = DistanceToPointRelativeTolerance(std::min(top_z, cylinder.radius()));
    ...
    for (int j = 0; j < num_vertices_per_circle; ++j) {
        ...
        // above).
        Append(SplitTriangularPrismToTetrahedra(medial[0], bottom[i], bottom[j], medial[1], top[i], top[j]),
               &mesh_elements);
        i = j;
    }
...
*/
template <typename T>
std::vector<VolumeElement> CalcMediumCylinderVolumeMeshWithMa(const Cylinder& cylinder,
                                                              const int num_vertices_per_circle,
                                                              const int bottom_center,
                                                              const std::vector<int>& bottom,
                                                              const int top_center,
    ...
    for (int j = 0; j < num_vertices_per_circle; ++j) {
        ...
        // (m in the picture).
        Append(SplitPyramidToTetrahedra(top[i], top[j], bottom[j], bottom[i], medial), &mesh_elements);
        i = j;
    }
    return mesh_elements;
}
...
*/
template <typename T>
std::vector<VolumeElement> CalcShortCylinderVolumeMeshWithMa(const Cylinder& cylinder,
                                                             const int num_vertices_per_circle,
                                                             const int bottom_center,
                                                             const std::vector<int>& bottom,
                                                             const int top_center,
    ...
    const double scale_cylinder_radius_to_medial_circle = medial_radius / cylinder.radius();
    for (int i = 0; i < num_vertices_per_circle; ++i) {
        const double x =
                ExtractDoubleOrThrow(mesh_vertices->at(bottom[i]).x()) * scale_cylinder_radius_to_medial_circle;
        const double y =
                ExtractDoubleOrThrow(mesh_vertices->at(bottom[i]).y()) * scale_cylinder_radius_to_medial_circle;
    ...
    for (int j = 0; j < num_vertices_per_circle; ++j) {
        ...
        Append(SplitTriangularPrismToTetrahedra(bottom_center, bottom[i], bottom[j], center, medial[i], medial[j]),
               &mesh_elements);
        ...
        Append(SplitTriangularPrismToTetrahedra(center, medial[i], medial[j], top_center, top[i], top[j]),
               &mesh_elements);
        ...
        Append(SplitTriangularPrismToTetrahedra(bottom[i], medial[i], top[i], bottom[j], medial[j], top[j]),
               &mesh_elements);
        i = j;
    }
...
template <typename T>
VolumeMesh<T> MakeCylinderVolumeMeshWithMa(const Cylinder& cylinder, const double resolution_hint) {
    ...
    switch (cylinder_class) {
        case CylinderClass::kLong:
            mesh_elements = CalcLongCylinderVolumeMeshWithMa<T>(cylinder, num_vertices_per_circle, bottom_center,
                                                                bottom, top_center, top, &mesh_vertices);
            break;
        case CylinderClass::kMedium:
            mesh_elements = CalcMediumCylinderVolumeMeshWithMa<T>(cylinder, num_vertices_per_circle, bottom_center,
                                                                  bottom, top_center, top, &mesh_vertices);
            break;
        case CylinderClass::kShort:
            mesh_elements = CalcShortCylinderVolumeMeshWithMa<T>(cylinder, num_vertices_per_circle, bottom_center,
                                                                 bottom, top_center, top, &mesh_vertices);
            break;
    }
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeCylinderVolumeMeshWithMa<T>));
