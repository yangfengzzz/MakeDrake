...
private:
    void ThrowUnsupportedGeometry(const std::string& shape_name) final {
        throw std::runtime_error(
                fmt::format("MakeConvexHull only applies to Mesh and Convex types, given {}.", shape_name));
    }
    using ShapeReifier::ImplementGeometry;

    void ImplementGeometry(const Mesh& mesh, void*) final { hull_ = MakeConvexHull(mesh.filename(), mesh.scale()); }
