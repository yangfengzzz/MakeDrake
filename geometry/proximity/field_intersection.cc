...
template <typename T>
std::vector<Vector3<T>> IntersectTetrahedra(int element0,
                                            const VolumeMesh<double>& mesh0_M,
                                            int element1,
                                            const VolumeMesh<double>& mesh1_N,
                                            const math::RigidTransform<T>& X_MN,
    ...
    Vector3<T> p_MVs[4];
    for (int i = 0; i < 4; ++i) {
        p_MVs[i] = X_MN * mesh1_N.vertex(mesh1_N.element(element1).vertex(i)).cast<T>();
    }
    // Each tuple of three vertex indices are oriented so that their normal
    // vector points outward from the tetrahedron.
    constexpr int kFaceVertexLocalIndex[4][3] = {{1, 2, 3}, {0, 3, 2}, {0, 1, 3}, {0, 2, 1}};
    ...
    for (const auto& face_vertices : kFaceVertexLocalIndex) {
        ...
        const Vector3<T> triangle_outward_normal_M = (p_MB - p_MA).cross(p_MC - p_MA);
...
template <typename T>
bool IsPlaneNormalAlongPressureGradient(const Vector3<T>& nhat_M,
                                        int tetrahedron,
...
void VolumeIntersector<MeshBuilder, BvType>::IntersectFields(const VolumeMeshFieldLinear<double, double>& field0_M,
    ...
    auto callback = [&candidate_tetrahedra](int tet0, int tet1) -> BvttCallbackResult {
...
template <class MeshBuilder, class BvType>
void VolumeIntersector<MeshBuilder, BvType>::CalcContactPolygon(const VolumeMeshFieldLinear<double, double>& field0_M,
                                                                const VolumeMeshFieldLinear<double, double>& field1_N,
                                                                const math::RigidTransform<T>& X_MN,
    ...
    if (!CalcEquilibriumPlane(tet0, field0_M, tet1, field1_N, X_MN, &equilibrium_plane_M)) {
        return;
    }
    ...
    if (!IsPlaneNormalAlongPressureGradient(reverse_polygon_nhat_N, tet1, field1_N)) {
        return;
    }
    const std::vector<Vector3<T>> polygon_vertices_M =
            IntersectTetrahedra(tet0, field0_M.mesh(), tet1, field1_N.mesh(), X_MN, equilibrium_plane_M);
    ...
    for (const auto& p_MV : polygon_vertices_M) {
        polygon_vertex_indices.push_back(builder_M->AddVertex(p_MV, field0_M.EvaluateCartesian(tet0, p_MV)));
    }
    ...
    const int num_new_faces =
            builder_M->AddPolygon(polygon_vertex_indices, polygon_nhat_M, field0_M.EvaluateGradient(tet0));
...
void HydroelasticVolumeIntersector<MeshBuilder>::IntersectCompliantVolumes(
    ...
    volume_intersector.IntersectFields(field0_M, bvh0_M, field1_N, bvh1_N, X_MN, &surface01_M, &field01_M);
    ...
    for (int i = 0; i < num_contact_polygons; ++i) {
        const Vector3<T>& grad_field0_M = field0_M.EvaluateGradient(volume_intersector.tet0_of_polygon(i));
    ...
    for (int i = 0; i < num_contact_polygons; ++i) {
        const Vector3<T>& grad_field1_N = field1_N.EvaluateGradient(volume_intersector.tet1_of_polygon(i));
    ...
    *contact_surface_W = std::make_unique<ContactSurface<T>>(id0, id1, std::move(surface01_M), std::move(field01_M),
...
std::unique_ptr<ContactSurface<T>> ComputeContactSurfaceFromCompliantVolumes(
    ...
    if (representation == HydroelasticContactRepresentation::kTriangle) {
        HydroelasticVolumeIntersector<TriMeshBuilder<T>>().IntersectCompliantVolumes(
                id0, field0_M, bvh0_M, X_WM, id1, field1_N, bvh1_N, X_WN, &contact_surface_W);
    } else {
        HydroelasticVolumeIntersector<PolyMeshBuilder<T>>().IntersectCompliantVolumes(
                id0, field0_M, bvh0_M, X_WM, id1, field1_N, bvh1_N, X_WN, &contact_surface_W);
    }
