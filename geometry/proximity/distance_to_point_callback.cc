...
template <typename T>
void SphereDistanceInSphereFrame(
        const fcl::Sphered& sphere, const Vector3<T>& p_SQ, Vector3<T>* p_SN, T* distance, Vector3<T>* grad_S) {
    ...
    const bool non_zero_displacement = !scalar_predicate<T>::is_bool || (dist_SQ > tolerance);
...
void ComputeDistanceToPrimitive(const fcl::Capsuled& capsule,
    ...
    if (p_GQ.z() >= half_length || p_GQ.z() <= -half_length) {
        ...
        // in S.
        const Vector3<T> p_GS{0, 0, (p_GQ.z() >= half_length) ? half_length : -half_length};
...
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Boxd& box) {
    ...
    std::tie(p_GN_G, grad_G, is_Q_on_edge_or_vertex) = ComputeDistanceToBox(h, p_GQ_G);
...
template <typename T>
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Capsuled& capsule) {
...
template <typename T>
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Cylinderd& cylinder) {
    using std::sqrt;
    ...
    const bool near_center_line =
            scalar_predicate<T>::is_bool && (r_Q < DistanceToPointRelativeTolerance(cylinder.radius));
    const Vector2<T> v_GBr = near_center_line ? Vector2<T>(1., 0.) : Vector2<T>(p_GQ(0), p_GQ(1)) / r_Q;
    ...
    Vector2<T> grad_B;
    bool is_Q_on_edge_or_vertex{};
    std::tie(p_BN, grad_B, is_Q_on_edge_or_vertex) = ComputeDistanceToBox(h, p_BQ);
...
template <typename T>
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Ellipsoidd& ellipsoid) {
    if constexpr (std::is_same_v<T, double>) {
        ...
        fcl::distance(&ellipsoid, X_WG_.GetAsIsometry3(), &sphere_Q, math::RigidTransformd(p_WQ_).GetAsIsometry3(),
                      request, result_W);
        ...
        // query point Q is.
        const Vector3d grad_G = Vector3d(p_GN.x() / (radii.x() * radii.x()), p_GN.y() / (radii.y() * radii.y()),
                                         p_GN.z() / (radii.z() * radii.z()))
                                        .normalized();

        const Vector3d grad_W = X_WG_.rotation() * grad_G;
        return SignedDistanceToPoint<T>(geometry_id_, p_GN, result_W.min_distance, grad_W);
    } else {
...
template <typename T>
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Halfspaced& halfspace) {
    T distance{};
    Vector3<T> p_GN_G, grad_W;
    ComputeDistanceToPrimitive(halfspace, X_WG_, p_WQ_, &p_GN_G, &distance, &grad_W);

    return SignedDistanceToPoint<T>{geometry_id_, p_GN_G, distance, grad_W};
}

template <typename T>
SignedDistanceToPoint<T> DistanceToPoint<T>::operator()(const fcl::Sphered& sphere) {
    T distance{};
    Vector3<T> p_GN_G, grad_W;
...
int DistanceToPoint<T>::ExtremalAxis(const Vector<U, dim>& p, const Vector<double, dim>& bounds) {
...
template <typename T>
template <int dim>
std::tuple<Vector<T, dim>, Vector<T, dim>, bool> DistanceToPoint<T>::ComputeDistanceToBox(
...
bool Callback(fcl::CollisionObjectd* object_A_ptr,
    ...
    const fcl::CollisionObjectd* geometry_object = (&data.query_point == object_A_ptr) ? object_B_ptr : object_A_ptr;

    const EncodedData encoding(*geometry_object);
    GeometryId geometry_id = encoding.id();

    const fcl::CollisionGeometryd* collision_geometry = geometry_object->collisionGeometry().get();
    if (ScalarSupport<T>::is_supported(collision_geometry->getNodeType())) {
        ...
        switch (collision_geometry->getNodeType()) {
            case fcl::GEOM_BOX:
                distance = distance_to_point(*static_cast<const fcl::Boxd*>(collision_geometry));
                break;
            case fcl::GEOM_CAPSULE:
                distance = distance_to_point(*static_cast<const fcl::Capsuled*>(collision_geometry));
                break;
            case fcl::GEOM_CYLINDER:
                distance = distance_to_point(*static_cast<const fcl::Cylinderd*>(collision_geometry));
                break;
            case fcl::GEOM_ELLIPSOID:
                distance = distance_to_point(*static_cast<const fcl::Ellipsoidd*>(collision_geometry));
                break;
            case fcl::GEOM_HALFSPACE:
                distance = distance_to_point(*static_cast<const fcl::Halfspaced*>(collision_geometry));
                break;
            case fcl::GEOM_SPHERE:
                distance = distance_to_point(*static_cast<const fcl::Sphered*>(collision_geometry));
                break;
            default:
