...
public:
    ...
    PenetrationPairFunctor(const GeometryId& id_A,
    ...
    void operator()(const fcl::Sphered& sphere_A, const fcl::Sphered& sphere_B, PenetrationAsPointPair<T>* result) {
        SphereShapePenetration(sphere_A, sphere_B, result);
    }

    void operator()(const fcl::Sphered& sphere_A, const fcl::Boxd& box_B, PenetrationAsPointPair<T>* result) {
        SphereShapePenetration(sphere_A, box_B, result);
    }

    void operator()(const fcl::Sphered& sphere_A, const fcl::Cylinderd& cylinder_B, PenetrationAsPointPair<T>* result) {
    ...
    void operator()(const fcl::Sphered& sphere_A, const fcl::Capsuled& capsule_B, PenetrationAsPointPair<T>* result) {
...
private:
    ...
    void SphereShapePenetration(const fcl::Sphered& sphere_A,
        ...
        if (ExtractDoubleOrThrow(depth) >= 0) {
            ...
            result->p_WCa = X_WA_.translation() - sphere_A.radius * shape_B_to_point_Ao.grad_W;
        }
    }
...
void CalcDistanceFallback(const fcl::CollisionObjectd& a,
    ...
    throw std::logic_error(
            fmt::format("PenetrationAsPointPair() cannot be evaluated for shapes '{}' and '{}' "
...
void CalcDistanceFallback<double>(const fcl::CollisionObjectd& a,
    ...
    pair_data->p_WCa = contact.pos - 0.5 * pair_data->depth * pair_data->nhat_BA_W;
    pair_data->p_WCb = contact.pos + 0.5 * pair_data->depth * pair_data->nhat_BA_W;
...
struct ScalarSupport<AutoDiffXd> {
    static bool is_supported(fcl::NODE_TYPE node1, fcl::NODE_TYPE node2) {
        ...
        return (node1 == fcl::GEOM_SPHERE &&
                (node2 == fcl::GEOM_SPHERE || node2 == fcl::GEOM_BOX || node2 == fcl::GEOM_HALFSPACE ||
                 node2 == fcl::GEOM_CYLINDER || node2 == fcl::GEOM_CAPSULE)) ||
               (node2 == fcl::GEOM_SPHERE && (node1 == fcl::GEOM_BOX || node1 == fcl::GEOM_HALFSPACE ||
...
template <typename T>
bool Callback(fcl::CollisionObjectd* fcl_object_A_ptr, fcl::CollisionObjectd* fcl_object_B_ptr, void* callback_data) {
    ...
    if (ScalarSupport<T>::is_supported(fcl_object_A_ptr->collisionGeometry()->getNodeType(),
        ...
        ComputeNarrowPhasePenetration(*fcl_object_A_ptr, data.X_WGs.at(id_A), *fcl_object_B_ptr, data.X_WGs.at(id_B),
        ...
        throw std::logic_error(fmt::format(
                ...
                "QueryObject::ComputePointPairPenetration() for the full status of "
                "supported geometries.",
                GetGeometryName(*fcl_object_A_ptr), GetGeometryName(*fcl_object_B_ptr), NiceTypeName::Get<T>()));
    }
