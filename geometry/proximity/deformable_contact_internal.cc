...
void Geometries::MaybeAddRigidGeometry(const Shape& shape,
                                       GeometryId id,
...
void Geometries::UpdateRigidWorldPose(GeometryId id, const math::RigidTransform<double>& X_WG) {
    if (auto iter = rigid_geometries_pending_.find(id); iter != rigid_geometries_pending_.end()) {
...
void Geometries::UpdateDeformableVertexPositions(GeometryId id, const Eigen::Ref<const VectorX<double>>& q_WG) {
    if (is_deformable(id)) {
        deformable_geometries_.at(id).UpdateVertexPositions(q_WG);
    }
}

DeformableContact<double> Geometries::ComputeDeformableContact(const CollisionFilter& collision_filter) const {
    ...
    std::unordered_map<GeometryId, const VolumeMeshFieldLinear<double, double>*> signed_distance_fields;
    for (const auto& [deformable_id, deformable_geometry] : deformable_geometries_) {
        signed_distance_fields[deformable_id] = &deformable_geometry.CalcSignedDistanceField();
        result.RegisterDeformableGeometry(deformable_id, deformable_geometry.deformable_mesh().mesh().num_vertices());
    }
    ...
    for (const auto& [deformable_id, deformable_geometry] : deformable_geometries_) {
        ...
        for (const auto& [rigid_id, rigid_geometry] : rigid_geometries_) {
            ...
            if (collision_filter.CanCollideWith(deformable_id, rigid_id)) {
                ...
                AddDeformableRigidContactSurface(*signed_distance_fields.at(deformable_id),
    ...
    for (std::unordered_map<GeometryId, DeformableGeometry>::const_iterator it0 = deformable_geometries_.begin();
        ...
        for (std::unordered_map<GeometryId, DeformableGeometry>::const_iterator it1 = std::next(it0);
            ...
            if (collision_filter.CanCollideWith(deformable0_id, deformable1_id)) {
                if (deformable1_id < deformable0_id) {
                    AddDeformableDeformableContactSurface(*signed_distance_fields.at(deformable0_id),
                    ...
                    AddDeformableDeformableContactSurface(*signed_distance_fields.at(deformable1_id),
...
void Geometries::AddRigidGeometry(const ShapeType& shape, const ReifyData& data) {
    ...
    std::optional<RigidGeometry> rigid_geometry = internal::deformable::MakeRigidRepresentation(shape, data.properties);
    if (rigid_geometry) {
...
void Geometries::FlushPendingRigidGeometry() {
    ...
    for (const auto& [id, geometry_instance] : worklist) {
        ...
        MaybeAddRigidGeometry(geometry_instance.shape(), id, *props, geometry_instance.pose());
    }
}
