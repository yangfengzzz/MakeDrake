...
Vector3d CalcCentroidOfEnclosedVolume(const TriangleSurfaceMesh<double>& surface_mesh) {
...
VolumeMesh<T> MakeConvexVolumeMesh(const TriangleSurfaceMesh<double>& surface_mesh) {
    std::vector<Vector3<T>> volume_mesh_vertices(surface_mesh.vertices().begin(), surface_mesh.vertices().end());
...
VolumeMesh<T> MakeConvexVolumeMesh(const Convex& convex) {
    return MakeConvexVolumeMesh<T>(ReadObjToTriangleSurfaceMesh(convex.filename(), convex.scale()));
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        (static_cast<VolumeMesh<T> (*)(const Convex&)>(&MakeConvexVolumeMesh<T>),
         static_cast<VolumeMesh<T> (*)(const TriangleSurfaceMesh<double>&)>(&MakeConvexVolumeMesh<T>)));
