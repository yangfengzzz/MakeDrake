...
public:
    ...
    */
    ...
    MeshFieldLinear(std::vector<T>&& values,
        ...
        if (gradient_mode != MeshGradientMode::kNone) {
            ...
            if (!is_gradient_field_degenerate_) {
                ...
                DRAKE_DEMAND(mesh->num_elements() == static_cast<int>(values_at_Mo_.size()));
            }
        }
    }
    ...
    MeshFieldLinear(std::vector<T>&& values, const MeshType* mesh, std::vector<Vector3<T>>&& gradients)
        : mesh_(mesh), values_(std::move(values)), gradients_(std::move(gradients)) {
        DRAKE_DEMAND(mesh_ != nullptr);
     ...
     */
    bool is_gradient_field_degenerate() const { return is_gradient_field_degenerate_; }
     ...
     */
    template <typename B>
    promoted_numerical_t<B, T> Evaluate(int e,
    ...
    */
    ...
    [[nodiscard]] std::unique_ptr<MeshFieldLinear> CloneAndSetMesh(const MeshType* new_mesh) const {
...
private:
    ...
    // pointer is null.
    [[nodiscard]] std::unique_ptr<MeshFieldLinear<T, MeshType>> CloneWithNullMesh() const {
