...
public:
    ...
    using ExtractIds = std::function<std::unordered_set<GeometryId>(const GeometrySet&, CollisionFilterScope)>;
    ...
    /* Applies the collision filter declaration to the persistent state. The
     ...
     @pre If `is_invariant` is `true`, declaration contains no `Allow*()`
          statements. */
    void Apply(const CollisionFilterDeclaration& declaration, const ExtractIds& extract_ids, bool is_invariant = false);
    ...
    FilterId ApplyTransient(const CollisionFilterDeclaration& declaration, const ExtractIds& extract_ids);
    ...
    bool operator!=(const CollisionFilter& other) const { return !(*this == other); }
...
private:
    ...
    static void AddGeometry(GeometryId id, FilterState* filter_state_out, PairRelationship relationship);
    ...
    static void AddFilteredPair(GeometryId id_A, GeometryId id_B, bool is_invariant, FilterState* state_out);

    /* Atomic operation in support of RemoveFilterBetween().  */
    static void RemoveFilteredPair(GeometryId id_A, GeometryId id_B, FilterState* state_out);

    /* Instantiates a FilterState such that all known pairs have given
     default relationship. */
    static FilterState InitializeTransientState(const FilterState& reference, PairRelationship default_relationship);
    ...
    struct StateDelta {
        ...
        StateDelta(FilterState state, FilterId id_in) : filter_state(std::move(state)), id(id_in) {}
