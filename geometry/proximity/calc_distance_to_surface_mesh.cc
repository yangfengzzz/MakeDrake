...
bool IsInTriangle(const Vector2<double>& Q, const Vector2<double>& B, const Vector2<double>& C) {
...
double CalcSquaredDistanceToLineSegment(const Vector3<double>& Q, const Vector3<double>& A, const Vector3<double>& B) {
...
/* Computes the squared distance from the point p_WQ to a non-degenerate
 triangle t_W. */
double CalcSquaredDistanceToTriangle(const Vector3<double>& p_WQ, Triangle t_W) {
    ...
    const double d_AB_squared = CalcSquaredDistanceToLineSegment(p_TQ, p_TA, p_TB);
    const double d_AC_squared = CalcSquaredDistanceToLineSegment(p_TQ, p_TA, p_TC);
    const double d_BC_squared = CalcSquaredDistanceToLineSegment(p_TQ, p_TB, p_TC);
...
public:
    ...
    static double CalcMinDistance(const Vector3d& p_WQ,
...
private:
    BvhVisitor(const Vector3d& p_WQ, const TriangleSurfaceMesh<double>& mesh_W) : p_WQ_{p_WQ}, mesh_W_{mesh_W} {}
    ...
    void Visit(const Bvh<Obb, TriangleSurfaceMesh<double>>::NodeType& node_W) {
        if (node_W.is_leaf()) {
            for (int i = 0; i < node_W.num_element_indices(); ++i) {
                ...
                min_squared_ =
                        std::min(min_squared_, CalcSquaredDistanceToTriangle(p_WQ_, {mesh_W_.vertices()[t.vertex(0)],
        ...
        const auto [closest_point_on_box_boundary_B, grad_signed_distance_B, _] =
                point_distance::DistanceToPoint<double>::ComputeDistanceToBox<3>(box_half_width_B, p_BQ);
        ...
        const double signed_distance = grad_signed_distance_B.dot(p_BQ - closest_point_on_box_boundary_B);

        // Check for possible pruning.
        if (signed_distance > 0) {
            // The query point is outside, so we can get the lower bound.
            const double squared_distance_from_this_node_is_at_least = signed_distance * signed_distance;
