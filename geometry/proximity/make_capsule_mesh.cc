...
VolumeMesh<T> MakeCapsuleVolumeMesh(const Capsule& capsule, double resolution_hint) {
    ...
    const int num_vertices_per_circle =
            static_cast<int>(std::clamp(2. * M_PI * capsule.radius() / resolution_hint, 3.0, 706.0));
    ...
    for (int i = 0; i < num_circles_per_cap; ++i) {
        ...
        for (int j = 0; j < num_vertices_per_circle; ++j) {
            ...
            top_cap[num_vertices_per_circle * i + j] = static_cast<int>(mesh_vertices.size());
            mesh_vertices.emplace_back(x, y, top_circle_z);
            bottom_cap[num_vertices_per_circle * i + j] = static_cast<int>(mesh_vertices.size());
    ...
    for (int i = 0; i < num_circles_per_cap - 1; ++i) {
        for (int j = 0; j < num_vertices_per_circle; ++j) {
            ...
            // for the base.
            Append(SplitPyramidToTetrahedra(top_cap[(i + 1) * num_vertices_per_circle + j],
                                            top_cap[(i + 1) * num_vertices_per_circle + j1],
                                            top_cap[i * num_vertices_per_circle + j1],
                                            top_cap[i * num_vertices_per_circle + j], medial_top),
                   &mesh_elements);

            Append(SplitPyramidToTetrahedra(bottom_cap[i * num_vertices_per_circle + j],
    ...
    const int last_circle_offset = (num_circles_per_cap - 1) * num_vertices_per_circle;
    for (int j = 0; j < num_vertices_per_circle; ++j) {
        const int j1 = (j + 1) % num_vertices_per_circle;
        ...
        mesh_elements.emplace_back(top, top_cap[last_circle_offset + j1], top_cap[last_circle_offset + j], medial_top);

        mesh_elements.emplace_back(bottom, bottom_cap[last_circle_offset + j], bottom_cap[last_circle_offset + j1],
                                   medial_bottom);
        ...
        Append(SplitTriangularPrismToTetrahedra(medial_bottom, bottom_cap[j], bottom_cap[j1], medial_top, top_cap[j],
                                                top_cap[j1]),
               &mesh_elements);
    }
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&MakeCapsuleVolumeMesh<T>));
