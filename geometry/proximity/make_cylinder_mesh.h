 ...
 */
template <typename T>
VolumeMesh<T> MakeCylinderVolumeMeshWithMa(const Cylinder& cylinder, double resolution_hint);
...
template <typename T>
Vector3<T> ProjectMidpointToMiddleCylinder(const Vector3<T>& p, const Vector3<T>& q) {
...
// vertex.
template <typename T>
Vector3<T> ProjectMidPoint(const Vector3<T>& x, const Vector3<T>& y, CylinderVertexType v_type, const double radius) {
...
template <typename T>
int CreateNewVertex(int vertex_a_index,
                    int vertex_b_index,
    ...
    const CylinderVertexType p_vertex_type =
            std::min(split_vertex_type[vertex_a_index], split_vertex_type[vertex_b_index]);
    ...
    const SortedPair<int> p_parents = MakeSortedPair(vertex_a_index, vertex_b_index);
...
template <typename T>
void RefineCylinderTetrahdron(const VolumeElement& tet,
                              std::vector<Vector3<T>>* split_mesh_vertices_ptr,
    ...
    std::vector<Vector3<T>>& split_mesh_vertices = *split_mesh_vertices_ptr;
    std::vector<CylinderVertexType>& split_vertex_type = *split_vertex_type_ptr;
    std::vector<VolumeElement>& split_mesh_tetrahedra = *split_mesh_tetrahedra_ptr;
    std::unordered_map<SortedPair<int>, int>& vertex_map = *vertex_map_ptr;
    ...
    auto get_child_vertex = [&vertex_map, &split_mesh_vertices, &split_vertex_type, &radius](int p, int q) {
        SortedPair<int> parents{p, q};
        auto iter = vertex_map.find(parents);
        if (iter != vertex_map.end()) return iter->second;
        return CreateNewVertex(parents.first(), parents.second(), &split_mesh_vertices, &split_vertex_type, &vertex_map,
                               radius);
    };
...
std::pair<VolumeMesh<T>, std::vector<CylinderVertexType>> RefineCylinderMesh(
        const VolumeMesh<T>& mesh, const std::vector<CylinderVertexType>& vertex_type, const double radius) {
    ...
    for (const auto& t : mesh.tetrahedra()) {
        RefineCylinderTetrahdron<T>(t, &split_mesh_vertices, &split_mesh_tetrahedra, &split_vertex_type, &vertex_map,
                                    radius);
    }

    return std::make_pair(VolumeMesh<T>(std::move(split_mesh_tetrahedra), std::move(split_mesh_vertices)),
                          split_vertex_type);
}
...
template <typename T>
std::pair<VolumeMesh<T>, std::vector<CylinderVertexType>> MakeCylinderMeshLevel0(const double& height,
                                                                                 const double& radius) {
    ...
    int subdivisions = static_cast<int>(std::max(2.0, std::floor(height / radius)));
    ...
    std::vector<CylinderVertexType> vertex_type(5 * (subdivisions + 1), CylinderVertexType::kSide);
    ...
    return std::make_pair(VolumeMesh<T>(std::move(tetrahedra), std::move(vertices)), vertex_type);
}
...
template <typename T>
VolumeMesh<T> MakeCylinderVolumeMesh(const Cylinder& cylinder, double resolution_hint) {
    const double length = cylinder.length();
    const double radius = cylinder.radius();
    std::pair<VolumeMesh<T>, std::vector<CylinderVertexType>> pair = MakeCylinderMeshLevel0<T>(length, radius);
    ...
    const int L = std::max(0, static_cast<int>(std::ceil(std::log2(M_PI / std::asin(e / (2.0 * radius)))) - 2));
    ...
    const int refinement_level = std::min(L, static_cast<int>(std::floor(std::log2(1.e8 / mesh.num_elements()) / 3.)));
...
//    The Eigen-compatible scalar for representing the mesh vertex positions.
template <typename T>
TriangleSurfaceMesh<T> MakeCylinderSurfaceMesh(const Cylinder& cylinder, double resolution_hint) {
    DRAKE_DEMAND(resolution_hint > 0.0);
    return ConvertVolumeToSurfaceMesh<T>(MakeCylinderVolumeMeshWithMa<T>(cylinder, resolution_hint));
}
