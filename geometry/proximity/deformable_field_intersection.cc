...
void AddDeformableContactDataFromOneTetrahedron(const int tetrahedron_index,
                                                const Vector3d& contact_point_W,
    ...
    barycentric_coordinates->emplace_back(tetrahedral_mesh_W.CalcBarycentric(contact_point_W, tetrahedron_index));
...
void AddDeformableDeformableContactSurface(const VolumeMeshFieldLinear<double, double>& deformable0_sdf_W,
    ...
    std::unique_ptr<PolygonSurfaceMeshFieldLinear<double, double>> contact_field_W;
    VolumeIntersector<PolyMeshBuilder<double>, Aabb> intersector;
    intersector.IntersectFields(deformable0_sdf_W, deformable0_mesh_W.bvh(), deformable1_sdf_W,
                                deformable1_mesh_W.bvh(), math::RigidTransform<double>::Identity(), &contact_mesh_W,
                                &contact_field_W);
    ...
    for (int f = 0; f < num_faces; ++f) {
        signed_distances_at_contact_points.push_back(
                ...
                contact_field_W->EvaluateCartesian(f, contact_mesh_W->element_centroid(f)));
    }
    ...
    for (int f = 0; f < num_faces; ++f) {
        ...
        AddDeformableContactDataFromOneTetrahedron(intersector.tet0_of_polygon(f), contact_point_W,
                                                   deformable0_sdf_W.mesh(), &barycentric_coordinates[0],
                                                   &contact_vertex_indices[0], &participating_vertices[0]);
        AddDeformableContactDataFromOneTetrahedron(intersector.tet1_of_polygon(f), contact_point_W,
    ...
    deformable_contact->AddDeformableDeformableContactSurface(
            deformable1_id, deformable0_id, participating_vertices[1], participating_vertices[0],
            std::move(*contact_mesh_W), std::move(signed_distances_at_contact_points),
            std::move(contact_vertex_indices[1]), std::move(contact_vertex_indices[0]),
            std::move(barycentric_coordinates[1]), std::move(barycentric_coordinates[0]));
}
