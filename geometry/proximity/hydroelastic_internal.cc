...
#include "geometry/proximity/make_box_field.h"
#include "geometry/proximity/make_box_mesh.h"
#include "geometry/proximity/make_capsule_field.h"
#include "geometry/proximity/make_capsule_mesh.h"
#include "geometry/proximity/make_convex_field.h"
#include "geometry/proximity/make_convex_mesh.h"
#include "geometry/proximity/make_cylinder_field.h"
#include "geometry/proximity/make_cylinder_mesh.h"
#include "geometry/proximity/make_ellipsoid_field.h"
#include "geometry/proximity/make_ellipsoid_mesh.h"
#include "geometry/proximity/make_mesh_field.h"
...
private:
    ...
    void ImplementGeometry(const Convex&, void* user_data) final { *static_cast<bool*>(user_data) = false; }

    void ImplementGeometry(const HalfSpace&, void* user_data) final { *static_cast<bool*>(user_data) = false; }

    void ImplementGeometry(const Mesh&, void* user_data) final { *static_cast<bool*>(user_data) = false; }

    void ImplementGeometry(const MeshcatCone&, void*) final { DRAKE_UNREACHABLE(); }
};
...
void Geometries::MaybeAddGeometry(const Shape& shape, GeometryId id, const ProximityProperties& properties) {
    const HydroelasticType type =
            properties.GetPropertyOrDefault(kHydroGroup, kComplianceType, HydroelasticType::kUndefined);
...
void Geometries::ImplementGeometry(const Ellipsoid& ellipsoid, void* user_data) {
    MakeShape(ellipsoid, *static_cast<ReifyData*>(user_data));
}

void Geometries::ImplementGeometry(const HalfSpace& half_space, void* user_data) {
...
void Geometries::MakeShape(const ShapeType& shape, const ReifyData& data) {
    switch (data.type) {
        ...
        case HydroelasticType::kSoft: {
            ...
            if (hydro_geometry) {
                if (is_primitive(shape) && hydro_geometry->pressure_field().is_gradient_field_degenerate()) {
...
public:
    ...
    Validator(const char* shape_name, const char* compliance) : shape_name_(shape_name), compliance_(compliance) {}
    ...
    const ValueType& Extract(const ProximityProperties& props, const char* group_name, const char* property_name) {
        const std::string full_property_name = fmt::format("('{}', '{}')", group_name, property_name);
        if (!props.HasProperty(group_name, property_name)) {
            throw std::logic_error(fmt::format("Cannot create {} {}; missing the {} property", compliance(),
                                               shape_name(), full_property_name));
        }
...
protected:
    void ValidateValue(const double& value, const std::string& property) const override {
        if (value <= 0) {
            throw std::logic_error(fmt::format("Cannot create {} {}; the {} property must be positive", compliance(),
                                               shape_name(), property));
        }
    }
};

std::optional<RigidGeometry> MakeRigidRepresentation(const HalfSpace& hs, const ProximityProperties&) {
    return RigidGeometry(hs);
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Sphere& sphere, const ProximityProperties& props) {
    PositiveDouble validator("Sphere", "rigid");
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeSphereSurfaceMesh<double>(sphere, edge_length));

    return RigidGeometry(RigidMesh(std::move(mesh)));
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Box& box, const ProximityProperties&) {
    ...
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeBoxSurfaceMesh<double>(box, 1.1 * box.size().maxCoeff()));

    return RigidGeometry(RigidMesh(std::move(mesh)));
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Cylinder& cylinder, const ProximityProperties& props) {
    PositiveDouble validator("Cylinder", "rigid");
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeCylinderSurfaceMesh<double>(cylinder, edge_length));

    return RigidGeometry(RigidMesh(std::move(mesh)));
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Capsule& capsule, const ProximityProperties& props) {
    PositiveDouble validator("Capsule", "rigid");
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeCapsuleSurfaceMesh<double>(capsule, edge_length));

    return RigidGeometry(RigidMesh(std::move(mesh)));
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Ellipsoid& ellipsoid, const ProximityProperties& props) {
    PositiveDouble validator("Ellipsoid", "rigid");
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<TriangleSurfaceMesh<double>>(MakeEllipsoidSurfaceMesh<double>(ellipsoid, edge_length));

    return RigidGeometry(RigidMesh(std::move(mesh)));
}

std::optional<RigidGeometry> MakeRigidRepresentation(const Mesh& mesh_spec, const ProximityProperties&) {
    ...
    if (extension == ".obj") {
        ...
        throw(
                std::runtime_error(fmt::format("hydroelastic::MakeRigidRepresentation(): for rigid hydroelastic Mesh "
...
std::optional<RigidGeometry> MakeRigidRepresentation(const Convex& convex_spec, const ProximityProperties&) {
    // Simply use the Convex's GetConvexHull().
    return RigidGeometry(RigidMesh(
            make_unique<TriangleSurfaceMesh<double>>(MakeTriangleFromPolygonMesh(convex_spec.GetConvexHull()))));
}

std::optional<SoftGeometry> MakeSoftRepresentation(const Sphere& sphere, const ProximityProperties& props) {
    PositiveDouble validator("Sphere", "soft");
    // First, create the mesh.
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    // If nothing is said, let's go for the *cheap* tessellation strategy.
    const TessellationStrategy strategy = props.GetPropertyOrDefault(kHydroGroup, "tessellation_strategy",
                                                                     TessellationStrategy::kSingleInteriorVertex);
    auto mesh = make_unique<VolumeMesh<double>>(MakeSphereVolumeMesh<double>(sphere, edge_length, strategy));

    const double hydroelastic_modulus = validator.Extract(props, kHydroGroup, kElastic);
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Box& box, const ProximityProperties& props) {
    PositiveDouble validator("Box", "soft");
    // First, create the mesh.
    auto mesh = make_unique<VolumeMesh<double>>(MakeBoxVolumeMeshWithMa<double>(box));
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Cylinder& cylinder, const ProximityProperties& props) {
    PositiveDouble validator("Cylinder", "soft");
    // First, create the mesh.
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<VolumeMesh<double>>(MakeCylinderVolumeMeshWithMa<double>(cylinder, edge_length));
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Capsule& capsule, const ProximityProperties& props) {
    PositiveDouble validator("Capsule", "soft");
    // First, create the mesh.
    const double edge_length = validator.Extract(props, kHydroGroup, kRezHint);
    auto mesh = make_unique<VolumeMesh<double>>(MakeCapsuleVolumeMesh<double>(capsule, edge_length));
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Ellipsoid& ellipsoid, const ProximityProperties& props) {
    ...
    const TessellationStrategy strategy = props.GetPropertyOrDefault(kHydroGroup, "tessellation_strategy",
                                                                     TessellationStrategy::kSingleInteriorVertex);
    auto mesh = make_unique<VolumeMesh<double>>(MakeEllipsoidVolumeMesh<double>(ellipsoid, edge_length, strategy));
...
std::optional<SoftGeometry> MakeSoftRepresentation(const HalfSpace&, const ProximityProperties& props) {
    PositiveDouble validator("HalfSpace", "soft");

    const double thickness = validator.Extract(props, kHydroGroup, kSlabThickness);
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Convex& convex_spec, const ProximityProperties& props) {
    PositiveDouble validator("Convex", "soft");

    // Use the pre-computed convex hull for the shape.
    const TriangleSurfaceMesh<double> surface_mesh = MakeTriangleFromPolygonMesh(convex_spec.GetConvexHull());
    auto mesh = make_unique<VolumeMesh<double>>(MakeConvexVolumeMesh<double>(surface_mesh));
...
std::optional<SoftGeometry> MakeSoftRepresentation(const Mesh& mesh_specification, const ProximityProperties& props) {
    ...
    if (mesh_specification.extension() == ".vtk") {
        ...
        mesh = make_unique<VolumeMesh<double>>(MakeVolumeMeshFromVtk<double>(mesh_specification));
        ...
        mesh = make_unique<VolumeMesh<double>>(MakeConvexVolumeMesh<double>(surface_mesh));
