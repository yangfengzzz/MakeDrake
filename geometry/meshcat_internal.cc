...
std::string LoadResource(const std::string& resource_name) {
    ...
    if (!content) {
        throw std::runtime_error(fmt::format("Error opening resource: {}", resource_name));
    }
...
std::optional<std::string_view> GetMeshcatStaticResource(std::string_view url_path) {
    static const drake::never_destroyed<std::string> meshcat_js(LoadResource("drake/geometry/meshcat.js"));
    static const drake::never_destroyed<std::string> stats_js(LoadResource("drake/geometry/stats.min.js"));
    static const drake::never_destroyed<std::string> meshcat_ico(LoadResource("drake/geometry/meshcat.ico"));
    static const drake::never_destroyed<std::string> meshcat_html(LoadResource("drake/geometry/meshcat.html"));
    if ((url_path == "/") || (url_path == "/index.html") || (url_path == "/meshcat.html")) {
...
std::shared_ptr<const FileStorage::Handle> LoadGltfUri(const fs::path& gltf_filename,
                                                       std::string_view array_hint,
                                                       std::string_view uri,
                                                       FileStorage* storage) {
    ...
    if (uri.substr(0, 5) == "data:") {
        ...
        if (pos == std::string_view::npos) {
            log()->warn("Meshcat ignoring malformed data URI while loading {} from '{}'", array_hint,
        ...
        const std::vector<uint8_t> decoded_vec =
                common_robotics_utilities::base64_helpers::Decode(std::string{base64_content});
        std::string decoded(reinterpret_cast<const char*>(decoded_vec.data()), decoded_vec.size());
        std::string filename_hint = fmt::format("{} from {}", array_hint, gltf_filename.string());
        ...
        if (!asset_data) {
            log()->warn("Meshcat could not open '{}' while loading {} from '{}'", asset_filename.string(), array_hint,
                        gltf_filename.string());
            return nullptr;
        }
...
std::vector<std::shared_ptr<const FileStorage::Handle>> UnbundleGltfAssets(const fs::path& gltf_filename,
                                                                           std::string* gltf_contents,
                                                                           FileStorage* storage) {
    ...
    try {
        ...
        log()->warn("Meshcat could not unbundle '{}': glTF parse error: {}", gltf_filename.string(), e.what());
        return assets;
    }
    ...
    for (const auto& array_name : {"images", "buffers"}) {
        ...
        for (size_t i = 0; i < array.size(); ++i) {
            ...
            if (item.contains("uri") && item["uri"].is_string()) {
                ...
                std::shared_ptr<const FileStorage::Handle> asset = LoadGltfUri(gltf_filename, array_hint, uri, storage);
                if (asset != nullptr) {
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&TransformGeometryName<T>));
