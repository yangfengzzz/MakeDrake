...
public:
       ...
       - ᵃ The exact representation of a compliant mesh depends on the type of
           mesh file it references:
             ...
             - .vtk: the tetrahedral mesh will be used directly. This external
               working
               <a
     href="https://docs.google.com/document/d/1VZtVsxIjOLKvgQ8SNSrF6PtWuPW5z9PP7-dQuxfmqpc/edit?usp=sharing">
    ...
    */
    ...
    std::vector<SignedDistanceToPoint<T>> ComputeSignedDistanceToPoint(
            const Vector3<T>& p_WQ, const double threshold = std::numeric_limits<double>::infinity()) const;
    //@}
...
private:
    ...
    void set(const systems::Context<T>* context, const SceneGraph<T>* scene_graph) {
    ...
    bool is_default() const { return context_ == nullptr && scene_graph_ == nullptr && state_ == nullptr; }
    ...
    void ThrowIfNotCallable() const {
        if (!is_callable()) {
            throw std::runtime_error("Attempting to perform query on invalid QueryObject.");
        }
    }
