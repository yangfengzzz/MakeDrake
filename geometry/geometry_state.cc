...
#include "geometry/proximity/make_convex_hull_mesh.h"
#include "geometry/proximity/volume_to_surface_mesh.h"
#include "geometry/proximity_engine.h"
#include "geometry/proximity_properties.h"
#include "geometry/render/render_engine.h"
...
template <typename T>
void DrivenMeshData::SetControlMeshPositions(const std::unordered_map<GeometryId, VectorX<T>>& q_WGs) {
    for (auto& [id, meshes] : driven_meshes_) {
        ...
        const VectorX<double>& q_WG = geometry::internal::convert_to_double(q_WGs.at(id));
...
GeometryState<T>::GeometryState()
    : self_source_(SourceId::get_new_id()), geometry_engine_(make_unique<internal::ProximityEngine<T>>()) {
    ...
    frames_[world] = InternalFrame(self_source_, world, "world", InternalFrame::world_frame_group(), 0, world);
...
bool BackfillDefaults(ProximityProperties* properties, const DefaultProximityProperties& defaults) {
    auto backfill = [&](const std::string& group_name, const std::string& name, const auto& default_value) -> bool {
    ...
    result |= backfill(kMaterialGroup, kHcDissipation, defaults.hunt_crossley_dissipation);
    ...
    if (defaults.static_friction.has_value()) {
        ...
        const auto wrapped_friction = std::make_optional<multibody::CoulombFriction<double>>(
...
GeometryState<T>::GeometryState(const GeometryState<U>& source)
    : self_source_(source.self_source_),
      ...
      source_deformable_geometry_id_map_(source.source_deformable_geometry_id_map_),
      ...
      geometry_engine_(std::move(source.geometry_engine_->template ToScalarType<T>())),
      render_engines_(source.render_engines_),
      geometry_version_(source.geometry_version_) {
    auto convert_pose_vector = [](const std::vector<RigidTransform<U>>& s, std::vector<RigidTransform<T>>* d) {
    ...
    {
        std::unordered_map<GeometryId, RigidTransform<T>>& dest = kinematics_data_.X_WGs;
        const std::unordered_map<GeometryId, RigidTransform<U>>& s = source.kinematics_data_.X_WGs;
    ...
    // Now convert the id -> configuration map.
    {
        ...
        const std::unordered_map<GeometryId, VectorX<U>>& s = source.kinematics_data_.q_WGs;
...
template <typename T>
std::vector<GeometryId> GeometryState<T>::GetAllGeometryIds(std::optional<Role> role) const {
...
template <typename T>
unordered_set<GeometryId> GeometryState<T>::GetGeometryIds(const GeometrySet& geometry_set,
...
int GeometryState<T>::NumDeformableGeometries() const {
    int count = 0;
    for (const auto& [source_id, deformable_geometry_ids] : source_deformable_geometry_id_map_) {
        unused(source_id);
...
int GeometryState<T>::NumAnchoredGeometries() const {
    ...
    int count = 0;
    const std::unordered_set<GeometryId>& child_geometries = frame.child_geometries();
...
template <typename T>
std::set<std::pair<GeometryId, GeometryId>> GeometryState<T>::GetCollisionCandidates() const {
...
const std::string& GeometryState<T>::GetName(SourceId id) const {
    ...
    throw std::logic_error("Querying source name for an invalid source id: " + to_string(id) + ".");
}
...
template <typename T>
bool GeometryState<T>::BelongsToSource(FrameId frame_id, SourceId source_id) const {
...
const std::string& GeometryState<T>::GetName(FrameId frame_id) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
        return "No frame name available for invalid frame id: " + to_string(frame_id);
    });
...
int GeometryState<T>::GetFrameGroup(FrameId frame_id) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
        return "No frame group available for invalid frame id: " + to_string(frame_id);
    });
...
template <typename T>
int GeometryState<T>::NumGeometriesForFrameWithRole(FrameId frame_id, Role role) const {
    const InternalFrame& frame = GetValueOrThrow(frame_id, frames_);
    int count = 0;
...
template <typename T>
std::vector<GeometryId> GeometryState<T>::GetGeometries(FrameId frame_id, std::optional<Role> role) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
        return fmt::format("Cannot report geometries associated with invalid frame id: {}", frame_id);
    });
    const InternalFrame& frame = frames_.at(frame_id);
...
template <typename T>
GeometryId GeometryState<T>::GetGeometryIdByName(FrameId frame_id, Role role, const std::string& name) const {
    ...
    if (count < 1) {
        ...
        throw std::logic_error(
                fmt::format("The frame '{}' ({}) has no geometry with the role '{}' and the "
...
template <typename T>
bool GeometryState<T>::BelongsToSource(GeometryId geometry_id, SourceId source_id) const {
...
const std::string& GeometryState<T>::GetName(GeometryId geometry_id) const {
    ...
    throw std::logic_error("No geometry available for invalid geometry id: " + to_string(geometry_id));
}
...
const Shape& GeometryState<T>::GetShape(GeometryId id) const {
    ...
    throw std::logic_error("No geometry available for invalid geometry id: " + to_string(id));
}

template <typename T>
const math::RigidTransform<double>& GeometryState<T>::GetPoseInFrame(GeometryId geometry_id) const {
    const auto& geometry = GetValueOrThrow(geometry_id, geometries_);
    return geometry.X_FG();
}

template <typename T>
std::variant<std::monostate, const TriangleSurfaceMesh<double>*, const VolumeMesh<double>*>
...
template <typename T>
const ProximityProperties* GeometryState<T>::GetProximityProperties(GeometryId id) const {
    const InternalGeometry* geometry = GetGeometry(id);
    if (geometry) return geometry->proximity_properties();
    throw std::logic_error(fmt::format("Referenced geometry {} has not been registered", id));
}

template <typename T>
const IllustrationProperties* GeometryState<T>::GetIllustrationProperties(GeometryId id) const {
    const InternalGeometry* geometry = GetGeometry(id);
...
template <typename T>
const PerceptionProperties* GeometryState<T>::GetPerceptionProperties(GeometryId id) const {
    const InternalGeometry* geometry = GetGeometry(id);
...
template <typename T>
const VolumeMesh<double>* GeometryState<T>::GetReferenceMesh(GeometryId id) const {
    const InternalGeometry* geometry = GetGeometry(id);
    if (geometry == nullptr) {
        throw std::logic_error(fmt::format("Referenced geometry {} has not been registered", id));
    }
...
template <typename T>
const std::vector<RenderMesh>& GeometryState<T>::GetDrivenRenderMeshes(GeometryId id, Role role) const {
    const InternalGeometry* geometry = GetGeometry(id);
    DRAKE_THROW_UNLESS(role != Role::kUnassigned);
    if (geometry == nullptr || !geometry->has_role(role) || !geometry->is_deformable()) {
        throw std::logic_error(
...
private:
    ...
    void ImplementGeometry(const Mesh& mesh, void*) final { hull_ = &mesh.GetConvexHull(); }

    void ImplementGeometry(const Convex& convex, void*) final { hull_ = &convex.GetConvexHull(); }
...
template <typename T>
const PolygonSurfaceMesh<double>* GeometryState<T>::GetConvexHull(GeometryId id) const {
    const InternalGeometry& geometry = GetValueOrThrow(id, geometries_);
...
bool GeometryState<T>::CollisionFiltered(GeometryId id1, GeometryId id2) const {
    std::string base_message = "Can't report collision filter status between geometries " + to_string(id1) + " and " +
                               to_string(id2) + "; ";
    ...
    if (geometry1 != nullptr && geometry2 != nullptr) {
        if (geometry1->has_proximity_role() && geometry2->has_proximity_role()) {
            return !geometry_engine_->collision_filter().CanCollideWith(geometry1->id(), geometry2->id());
        }
        if (geometry1->has_proximity_role()) {
            throw std::logic_error(base_message + to_string(id2) + " does not have a proximity role");
        } else if (geometry2->has_proximity_role()) {
            throw std::logic_error(base_message + to_string(id1) + " does not have a proximity role");
        } else {
            throw std::logic_error(base_message + " neither id has a proximity role");
        }
    }
    if (geometry1 != nullptr) {
        throw std::logic_error(base_message + to_string(id2) + " is not a valid geometry");
    } else if (geometry2 != nullptr) {
        throw std::logic_error(base_message + to_string(id1) + " is not a valid geometry");
    } else {
...
template <typename T>
const math::RigidTransform<T>& GeometryState<T>::get_pose_in_world(FrameId frame_id) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
        return "No world pose available for invalid frame id: " + to_string(frame_id);
    });
    return kinematics_data_.X_WFs[frames_.at(frame_id).index()];
}

template <typename T>
const math::RigidTransform<T>& GeometryState<T>::get_pose_in_world(GeometryId geometry_id) const {
    FindOrThrow(geometry_id, geometries_, [geometry_id]() {
        return "No world pose available for invalid geometry id: " + to_string(geometry_id);
    });
    const auto& geometry = GetValueOrThrow(geometry_id, geometries_);
...
template <typename T>
const math::RigidTransform<T>& GeometryState<T>::get_pose_in_parent(FrameId frame_id) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
...
template <typename T>
const VectorX<T>& GeometryState<T>::get_configurations_in_world(GeometryId geometry_id) const {
    FindOrThrow(geometry_id, geometries_, [geometry_id]() {
        return "No world configurations available for invalid geometry id: " + to_string(geometry_id);
    });
    const auto& geometry = GetValueOrThrow(geometry_id, geometries_);
    if (!geometry.is_deformable()) {
        throw std::logic_error(
...
template <typename T>
std::vector<VectorX<T>> GeometryState<T>::GetDrivenMeshConfigurationsInWorld(GeometryId geometry_id, Role role) const {
    FindOrThrow(geometry_id, geometries_, [geometry_id]() {
        return "No mesh configurations available for invalid geometry id: " + to_string(geometry_id);
    });
    const auto& geometry = GetValueOrThrow(geometry_id, geometries_);
...
SourceId GeometryState<T>::RegisterNewSource(const std::string& name) {
    ...
    const std::string final_name = name != "" ? name : "Source_" + to_string(source_id);

    // The user can provide bad names, _always_ test.
    for (const auto& pair : source_names_) {
        if (pair.second == final_name) {
            throw std::logic_error("Registering new source with duplicate name: " + final_name + ".");
        }
    }
...
template <typename T>
FrameId GeometryState<T>::RegisterFrame(SourceId source_id, const GeometryFrame& frame) {
    return RegisterFrame(source_id, InternalFrame::world_frame_id(), frame);
}

template <typename T>
FrameId GeometryState<T>::RegisterFrame(SourceId source_id, FrameId parent_id, const GeometryFrame& frame) {
    FrameId frame_id = frame.id();

    if (frames_.contains(frame_id)) {
        throw std::logic_error("Registering frame with an id that has already been registered: " + to_string(frame_id));
    }
    ...
    frames_.emplace(frame_id, InternalFrame(source_id, frame_id, frame.name(), frame.frame_group(), index, parent_id));
    return frame_id;
}
...
template <typename T>
GeometryId GeometryState<T>::RegisterGeometry(SourceId source_id,
                                              FrameId frame_id,
                                              std::unique_ptr<GeometryInstance> geometry) {
    if (geometry == nullptr) {
        throw std::logic_error("Registering null geometry to frame " + to_string(frame_id) + ", on source " +
                               to_string(source_id) + ".");
    }
    ...
    kinematics_data_.X_WGs[geometry_id] = kinematics_data_.X_WFs[frame.index()] * geometry->pose().cast<T>();
    geometries_.emplace(geometry_id, InternalGeometry(source_id, geometry->release_shape(), frame_id, geometry_id,
...
template <typename T>
GeometryId GeometryState<T>::RegisterDeformableGeometry(SourceId source_id,
                                                        FrameId frame_id,
                                                        std::unique_ptr<GeometryInstance> geometry,
    ...
    if (frame_id != InternalFrame::world_frame_id()) {
        throw std::logic_error("Registering deformable geometry with id " + to_string(geometry_id) +
                               " to a non-world frame");
    }

    ValidateRegistrationAndSetTopology(source_id, frame_id, geometry_id);
    source_deformable_geometry_id_map_[source_id].insert(geometry_id);

    InternalGeometry internal_geometry(source_id, geometry->release_shape(), frame_id, geometry_id, geometry->name(),
    ...
    const InternalFrame& frame = frames_[frame_id];
    const RigidTransform<T> X_WG = kinematics_data_.X_WFs[frame.index()] * geometry->pose().cast<T>();
    VectorX<T> q_WG(reference_mesh->num_vertices() * 3);
    for (int v = 0; v < reference_mesh->num_vertices(); ++v) {
        q_WG.template segment<3>(3 * v) = X_WG * Vector3<T>(reference_mesh->vertex(v));
    }
...
template <typename T>
void GeometryState<T>::RenameGeometry(GeometryId geometry_id, const std::string& name) {
    InternalGeometry* geometry = GetMutableGeometry(geometry_id);
    if (geometry == nullptr) {
        throw std::logic_error("Cannot rename geometry with invalid geometry id: " + to_string(geometry_id));
    }
...
template <typename T>
void GeometryState<T>::ChangeShape(SourceId source_id,
    ...
    if (!BelongsToSource(geometry_id, source_id)) {
        throw std::logic_error("Given geometry id " + to_string(geometry_id) +
                               " does not belong to the given source id " + to_string(source_id));
    }
    InternalGeometry* geometry = GetMutableGeometry(geometry_id);
...
template <typename T>
GeometryId GeometryState<T>::RegisterAnchoredGeometry(SourceId source_id, std::unique_ptr<GeometryInstance> geometry) {
    return RegisterGeometry(source_id, InternalFrame::world_frame_id(), std::move(geometry));
}

template <typename T>
void GeometryState<T>::RemoveGeometry(SourceId source_id, GeometryId geometry_id) {
    if (!BelongsToSource(geometry_id, source_id)) {
        throw std::logic_error("Trying to remove geometry " + to_string(geometry_id) +
                               " from "
                               "source " +
                               to_string(source_id) +
...
template <typename T>
bool GeometryState<T>::IsValidGeometryName(FrameId frame_id, Role role, const std::string& candidate_name) const {
    FindOrThrow(frame_id, frames_, [frame_id]() {
...
void GeometryState<T>::AssignRole(SourceId source_id,
                                  ...
                                  RoleAssign assign) {
    InternalGeometry& geometry = ValidateRoleAssign(source_id, geometry_id, Role::kProximity, assign);
    ...
    switch (assign) {
        case RoleAssign::kNew: {
            ...
            if (geometry.is_deformable()) {
                ...
                geometry_engine_->AddDeformableGeometry(*geometry.reference_mesh(), geometry_id);
            } else if (geometry.is_dynamic()) {
                // Pass the geometry to the engine.
                const RigidTransformd& X_WG = convert_to_double(kinematics_data_.X_WGs.at(geometry_id));
                geometry_engine_->AddDynamicGeometry(geometry.shape(), X_WG, geometry_id,
                                                     *geometry.proximity_properties());
            } else {
                geometry_engine_->AddAnchoredGeometry(geometry.shape(), geometry.X_FG(), geometry_id,
                                                      *geometry.proximity_properties());
            }
        ...
        case RoleAssign::kReplace:
            ...
            geometry_engine_->UpdateRepresentationForNewProperties(geometry, properties);
            geometry.SetRole(std::move(properties));
            break;
        default:
            DRAKE_UNREACHABLE();
...
void GeometryState<T>::AssignRole(SourceId source_id,
    ...
    InternalGeometry& geometry = ValidateRoleAssign(source_id, geometry_id, Role::kPerception, assign);
    ...
    if (!added_to_renderer && render_engines_.size() > 0) {
        ...
        drake::log()->warn("Perception role assigned to geometry {}, but no renderer accepted it", geometry_id);
    }
}
...
void GeometryState<T>::AssignRole(SourceId source_id,
                                  ...
                                  RoleAssign assign) {
    InternalGeometry& geometry = ValidateRoleAssign(source_id, geometry_id, Role::kIllustration, assign);
...
template <typename T>
int GeometryState<T>::RemoveRole(SourceId source_id, FrameId frame_id, Role role) {
    int count = 0;
    ...
    for (GeometryId geometry_id : frame.child_geometries()) {
        ...
        if (frame_id != InternalFrame::world_frame_id() || BelongsToSource(geometry_id, source_id)) {
...
template <typename T>
int GeometryState<T>::RemoveRole(SourceId source_id, GeometryId geometry_id, Role role) {
    if (!BelongsToSource(geometry_id, source_id)) {
        throw std::logic_error("Trying to remove the role " + to_string(role) + " from the geometry " +
...
template <typename T>
int GeometryState<T>::RemoveFromRenderer(const std::string& renderer_name, SourceId source_id, FrameId frame_id) {
    int count = 0;
...
template <typename T>
int GeometryState<T>::RemoveFromRenderer(const std::string& renderer_name, SourceId source_id, GeometryId geometry_id) {
    if (!BelongsToSource(geometry_id, source_id)) {
        throw std::logic_error("Trying to remove geometry " + to_string(geometry_id) +
                               " from the "
                               "renderer '" +
...
void ThrowForNonProximity(const internal::InternalGeometry& g, const char* purpose) {
    if (!g.has_proximity_role()) {
        const char* role_description = g.has_illustration_role()
...
template <typename T>
SignedDistancePair<T> GeometryState<T>::ComputeSignedDistancePairClosestPoints(GeometryId id_A, GeometryId id_B) const {
    ...
    return geometry_engine_->ComputeSignedDistancePairClosestPoints(id_A, id_B, kinematics_data_.X_WGs);
}

template <typename T>
void GeometryState<T>::AddRenderer(std::string name, std::unique_ptr<render::RenderEngine> renderer) {
    if (render_engines_.contains(name)) {
        throw std::logic_error(fmt::format("AddRenderer(): A renderer with the name '{}' already exists", name));
    }
    ...
    for (auto& id_geo_pair : geometries_) {
        ...
        if (geometry.has_perception_role()) {
            ...
            auto accepting_renderers = properties->GetPropertyOrDefault("renderer", "accepting", set<string>{});
            if (accepting_renderers.empty() || accepting_renderers.contains(name)) {
                ...
                if (geometry.is_deformable()) {
                    accepted |= render_engine->RegisterDeformableVisual(
                            id, driven_mesh_data_.at(Role::kPerception).render_meshes(id), *properties);
                } else {
                    accepted |= render_engine->RegisterVisual(id, geometry.shape(), *properties,
...
void GeometryState<T>::RemoveRenderer(const std::string& name) {
    if (!render_engines_.contains(name)) {
        throw std::logic_error(fmt::format("RemoveRenderer(): A renderer with the name '{}' does not exist", name));
    }
...
void GeometryState<T>::RenderColorImage(const ColorRenderCamera& camera,
    ...
    const RigidTransformd X_WC = CalcCameraWorldPose(camera.core(), parent_frame, X_PC);
    const render::RenderEngine& engine = GetRenderEngineOrThrow(camera.core().renderer_name());
...
template <typename T>
std::unique_ptr<GeometryState<AutoDiffXd>> GeometryState<T>::ToAutoDiffXd() const {
    return std::unique_ptr<GeometryState<AutoDiffXd>>(new GeometryState<AutoDiffXd>(*this));
}

template <typename T>
void GeometryState<T>::ApplyProximityDefaults(const DefaultProximityProperties& defaults) {
...
template <typename T>
void GeometryState<T>::ApplyProximityDefaults(const DefaultProximityProperties& defaults, GeometryId geometry_id) {
    ...
    AssignRole(get_source_id(geometry_id), geometry_id, props, RoleAssign::kReplace);
}

template <typename T>
unordered_set<GeometryId> GeometryState<T>::CollectIds(const GeometrySet& geometry_set,
                                                       std::optional<Role> role,
                                                       CollisionFilterScope scope) const {
    auto must_include = [scope](const InternalGeometry& g, const std::optional<Role>& r) {
        // Must have compatible role and be part of the scope.
        return (!r.has_value() || g.has_role(*r)) && (scope == CollisionFilterScope::kAll || !g.is_deformable());
    };
...
template <typename T>
void GeometryState<T>::SetFramePoses(const SourceId source_id,
    ...
    for (auto frame_id : source_root_frame_map_.at(source_id)) {
        UpdatePosesRecursively(frames_.at(frame_id), world_pose, poses, kinematics_data);
    }
}

template <typename T>
void GeometryState<T>::SetGeometryConfiguration(SourceId source_id,
                                                const GeometryConfigurationVector<T>& configurations,
                                                internal::KinematicsData<T>* kinematics_data) const {
    const GeometryIdSet& g_ids = GetValueOrThrow(source_id, source_deformable_geometry_id_map_);
...
void GeometryState<T>::ValidateFrameIds(const SourceId source_id,
    ...
    if (ref_frame_count != kinematics_data.size()) {
        ...
        throw std::runtime_error("Disagreement in expected number of frames (" + to_string(frames.size()) +
                                 ") and the given number of frames (" + to_string(kinematics_data.size()) + ").");
    }
    for (auto id : frames) {
        if (!kinematics_data.has_id(id)) {
            throw std::runtime_error("Registered frame id (" + to_string(id) + ") belonging to source " +
                                     to_string(source_id) + " was not found in the provided kinematics data.");
        }
    }
}

template <typename T>
void GeometryState<T>::ValidateRegistrationAndSetTopology(SourceId source_id,
                                                          FrameId frame_id,
                                                          GeometryId geometry_id) {
    if (geometries_.contains(geometry_id)) {
        throw std::logic_error("Registering geometry with an id that has already been registered: " +
                               to_string(geometry_id));
    }
    ...
    FrameIdSet& set = GetMutableValueOrThrow(frame_source_id, &source_frame_id_map_);

    FindOrThrow(frame_id, set, [frame_id, frame_source_id]() {
        return "Referenced frame " + to_string(frame_id) + " for source " + to_string(frame_source_id) +
...
template <typename T>
void GeometryState<T>::FinalizePoseUpdate(const internal::KinematicsData<T>& kinematics_data,
...
template <typename T>
void GeometryState<T>::FinalizeConfigurationUpdate(const internal::KinematicsData<T>& kinematics_data,
...
SourceId GeometryState<T>::get_source_id(GeometryId id) const {
    const InternalGeometry* geometry = GetGeometry(id);
    if (geometry == nullptr) {
        throw std::logic_error("Geometry id " + to_string(id) + " does not map to a registered geometry");
    }
    return geometry->source_id();
}

template <typename T>
void GeometryState<T>::UpdatePosesRecursively(const internal::InternalFrame& frame,
                                              const RigidTransform<T>& X_WP,
                                              const FramePoseVector<T>& poses,
                                              internal::KinematicsData<T>* kinematics_data) const {
...
template <typename T>
bool GeometryState<T>::NameIsUnique(FrameId id, Role role, const std::string& name) const {
    bool unique = true;
...
template <typename T>
void GeometryState<T>::ThrowIfNameExistsInRole(FrameId id, Role role, const std::string& name) const {
...
template <typename T>
void GeometryState<T>::AssignAllDefinedRoles(SourceId source_id, std::unique_ptr<GeometryInstance> geometry) {
    DRAKE_DEMAND(geometry != nullptr);
    ...
    if (geometry->illustration_properties()) {
        AssignRole(source_id, geometry_id, std::move(*geometry->mutable_illustration_properties()));
    }

    if (geometry->proximity_properties()) {
        AssignRole(source_id, geometry_id, std::move(*geometry->mutable_proximity_properties()));
    }

    if (geometry->perception_properties()) {
        AssignRole(source_id, geometry_id, std::move(*geometry->mutable_perception_properties()));
    }
}
...
InternalGeometry& GeometryState<T>::ValidateRoleAssign(SourceId source_id,
    ...
    if (has_role && assign == RoleAssign::kNew) {
        throw std::logic_error("Trying to assign the '" + to_string(role) + "' role to geometry id " +
                               to_string(geometry_id) + " for the first time; it already has the role assigned");
    } else if (!has_role && assign == RoleAssign::kReplace) {
        throw std::logic_error("Trying to replace the properties on geometry id " + to_string(geometry_id) +
                               " for the '" + to_string(role) + "' role; it has not had the role initially assigned");
    }
...
template <typename T>
void GeometryState<T>::AddToProximityEngineUnchecked(const InternalGeometry& geometry) {
    const GeometryId geometry_id = geometry.id();
    if (geometry.is_deformable()) {
        DRAKE_DEMAND(geometry.reference_mesh() != nullptr);
        geometry_engine_->AddDeformableGeometry(*geometry.reference_mesh(), geometry_id);
        ...
        // Pass the geometry to the engine.
        const RigidTransformd& X_WG = convert_to_double(kinematics_data_.X_WGs.at(geometry_id));
        geometry_engine_->AddDynamicGeometry(geometry.shape(), X_WG, geometry_id, *geometry.proximity_properties());
    } else {
        geometry_engine_->AddAnchoredGeometry(geometry.shape(), geometry.X_FG(), geometry_id,
                                              *geometry.proximity_properties());
    }
...
template <typename T>
void GeometryState<T>::RemoveFromProximityEngineUnchecked(const InternalGeometry& geometry) {
    geometry_engine_->RemoveGeometry(geometry.id(), geometry.is_dynamic());
    geometry_version_.modify_proximity();
}

template <typename T>
bool GeometryState<T>::RemoveFromRendererUnchecked(const std::string& renderer_name, GeometryId id) {
...
template <typename T>
bool GeometryState<T>::AddToCompatibleRenderersUnchecked(const internal::InternalGeometry& geometry) {
    bool added_to_renderer = false;
    auto accepting_renderers =
            geometry.perception_properties()->GetPropertyOrDefault("renderer", "accepting", set<string>{});
    ...
    if (geometry.is_deformable()) {
        added_to_renderer = AddDeformableToCompatibleRenderersUnchecked(geometry, &candidate_renderers);
    } else {
        added_to_renderer = AddRigidToCompatibleRenderersUnchecked(geometry, &candidate_renderers);
    }
...
template <typename T>
bool GeometryState<T>::AddRigidToCompatibleRenderersUnchecked(const internal::InternalGeometry& geometry,
                                                              std::vector<render::RenderEngine*>* candidate_renderers) {
    const PerceptionProperties& properties = *geometry.perception_properties();

    const RigidTransformd& X_WG = convert_to_double(kinematics_data_.X_WGs.at(geometry.id()));

    bool added_to_renderer{false};
    for (auto& engine : *candidate_renderers) {
        added_to_renderer =
                engine->RegisterVisual(geometry.id(), geometry.shape(), properties, X_WG, geometry.is_dynamic()) ||
                added_to_renderer;
    }
    return added_to_renderer;
}
...
bool GeometryState<T>::AddDeformableToCompatibleRenderersUnchecked(
        const internal::InternalGeometry& geometry, std::vector<render::RenderEngine*>* candidate_renderers) {
...
void GeometryState<T>::RegisterDrivenMesh(GeometryId geometry_id, Role role) {
    ...
    if (role == Role::kPerception) {
        ...
        const string render_meshes_file = properties.GetPropertyOrDefault("deformable", "embedded_mesh", string{});
        if (!render_meshes_file.empty()) {
            render_meshes = internal::LoadRenderMeshesFromObj(render_meshes_file, properties, {});
            for (const internal::RenderMesh& render_mesh : render_meshes) {
                driven_meshes.emplace_back(MakeTriangleSurfaceMesh(render_mesh), control_mesh);
            }
            driven_mesh_data_[Role::kPerception].SetMeshes(geometry_id, std::move(driven_meshes),
    ...
    render_meshes.emplace_back(
            MakeRenderMeshFromTriangleSurfaceMesh(driven_meshes.back().triangle_surface_mesh(), properties));
    driven_mesh_data_[role].SetMeshes(geometry_id, std::move(driven_meshes), std::move(render_meshes));
}
...
template <typename T>
const InternalFrame& GeometryState<T>::ValidateAndGetFrame(SourceId source_id, FrameId frame_id) const {
    ...
    if (frame_id == InternalFrame::world_frame_id()) {
        ...
        FindOrThrow(frame_id, set, [frame_id, source_id]() {
            return "Referenced frame " + to_string(frame_id) + " for source " + to_string(source_id) +
                   ", but the frame doesn't belong to the source.";
        });
    }
    return frames_.at(frame_id);
...
template <typename T>
const render::RenderEngine& GeometryState<T>::GetRenderEngineOrThrow(const std::string& renderer_name) const {
    auto iter = render_engines_.find(renderer_name);
    if (iter != render_engines_.end()) {
        return *iter->second;
    }

    throw std::logic_error(fmt::format("No renderer exists with name: '{}'", renderer_name));
}

template <typename T>
RigidTransformd GeometryState<T>::CalcCameraWorldPose(const render::RenderCameraCore& core,
                                                      FrameId parent_frame,
                                                      const RigidTransformd& X_PC) const {
    return GetDoubleWorldPose(parent_frame) * X_PC * core.sensor_pose_in_camera_body();
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::GeometryState);
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
        (&drake::geometry::internal::DrivenMeshData::template SetControlMeshPositions<T>));
