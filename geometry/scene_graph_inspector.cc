...
template <typename T>
std::vector<GeometryId> SceneGraphInspector<T>::GetAllGeometryIds(std::optional<Role> role) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetAllGeometryIds(role);
}

template <typename T>
std::unordered_set<GeometryId> SceneGraphInspector<T>::GetGeometryIds(const GeometrySet& geometry_set,
...
template <typename T>
std::set<std::pair<GeometryId, GeometryId>> SceneGraphInspector<T>::GetCollisionCandidates() const {
    DRAKE_DEMAND(state_ != nullptr);
...
template <typename T>
const std::unordered_set<FrameId>& SceneGraphInspector<T>::FramesForSource(SourceId source_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->FramesForSource(source_id);
}

template <typename T>
bool SceneGraphInspector<T>::BelongsToSource(FrameId frame_id, SourceId source_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->BelongsToSource(frame_id, source_id);
}

template <typename T>
const std::string& SceneGraphInspector<T>::GetOwningSourceName(FrameId frame_id) const {
    DRAKE_DEMAND(state_ != nullptr);
...
template <typename T>
int SceneGraphInspector<T>::NumGeometriesForFrameWithRole(FrameId frame_id, Role role) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->NumGeometriesForFrameWithRole(frame_id, role);
}

template <typename T>
std::vector<GeometryId> SceneGraphInspector<T>::GetGeometries(FrameId frame_id, const std::optional<Role>& role) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetGeometries(frame_id, role);
}

template <typename T>
GeometryId SceneGraphInspector<T>::GetGeometryIdByName(FrameId frame_id, Role role, const std::string& name) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetGeometryIdByName(frame_id, role, name);
}

template <typename T>
bool SceneGraphInspector<T>::BelongsToSource(GeometryId geometry_id, SourceId source_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->BelongsToSource(geometry_id, source_id);
}

template <typename T>
const std::string& SceneGraphInspector<T>::GetOwningSourceName(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
...
template <typename T>
const std::string& SceneGraphInspector<T>::GetName(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
...
template <typename T>
const math::RigidTransform<double>& SceneGraphInspector<T>::GetPoseInFrame(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
...
SceneGraphInspector<T>::maybe_get_hydroelastic_mesh(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->maybe_get_hydroelastic_mesh(geometry_id);
}

template <typename T>
const GeometryProperties* SceneGraphInspector<T>::GetProperties(GeometryId geometry_id, Role role) const {
    DRAKE_DEMAND(state_ != nullptr);
    switch (role) {
...
template <typename T>
const ProximityProperties* SceneGraphInspector<T>::GetProximityProperties(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetProximityProperties(geometry_id);
}

template <typename T>
const IllustrationProperties* SceneGraphInspector<T>::GetIllustrationProperties(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetIllustrationProperties(geometry_id);
}

template <typename T>
const PerceptionProperties* SceneGraphInspector<T>::GetPerceptionProperties(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetPerceptionProperties(geometry_id);
}

template <typename T>
const VolumeMesh<double>* SceneGraphInspector<T>::GetReferenceMesh(GeometryId geometry_id) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->GetReferenceMesh(geometry_id);
}

template <typename T>
const std::vector<internal::RenderMesh>& SceneGraphInspector<T>::GetDrivenRenderMeshes(GeometryId geometry_id,
                                                                                       Role role) const {
    DRAKE_DEMAND(state_ != nullptr);
...
template <typename T>
std::vector<GeometryId> SceneGraphInspector<T>::GetAllDeformableGeometryIds() const {
    return state_->GetAllDeformableGeometryIds();
}

template <typename T>
const PolygonSurfaceMesh<double>* SceneGraphInspector<T>::GetConvexHull(GeometryId geometry_id) const {
    return state_->GetConvexHull(geometry_id);
}

template <typename T>
bool SceneGraphInspector<T>::CollisionFiltered(GeometryId geometry_id1, GeometryId geometry_id2) const {
    DRAKE_DEMAND(state_ != nullptr);
    return state_->CollisionFiltered(geometry_id1, geometry_id2);
}

template <typename T>
std::unique_ptr<GeometryInstance> SceneGraphInspector<T>::CloneGeometryInstance(GeometryId id) const {
    auto geometry_instance = std::make_unique<GeometryInstance>(GetPoseInFrame(id), GetShape(id), GetName(id));
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::SceneGraphInspector);
