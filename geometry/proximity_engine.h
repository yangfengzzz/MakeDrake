...
#include "geometry/query_results/signed_distance_pair.h"
...
public:
    ...
    */
    ...
    void UpdateWorldPoses(const std::unordered_map<GeometryId, math::RigidTransform<T>>& X_WGs);
    ...
    std::vector<SignedDistancePair<T>> ComputeSignedDistancePairwiseClosestPoints(
            const std::unordered_map<GeometryId, math::RigidTransform<T>>& X_WGs, const double max_distance) const;
...
private:
    ...
    const math::RigidTransform<double> GetX_WG(GeometryId id, bool is_dynamic) const;
