...
template <typename T>
MeshcatVisualizer<T>::MeshcatVisualizer(std::shared_ptr<Meshcat> meshcat, MeshcatVisualizerParams params)
    ...
    this->DeclarePeriodicPublishEvent(params_.publish_period, 0.0, &MeshcatVisualizer<T>::UpdateMeshcat);
    this->DeclareForcedPublishEvent(&MeshcatVisualizer<T>::UpdateMeshcat);

    if (params_.delete_on_initialization_event) {
        this->DeclareInitializationPublishEvent(&MeshcatVisualizer<T>::OnInitialization);
    }
...
template <typename T>
MeshcatAnimation* MeshcatVisualizer<T>::StartRecording(bool set_transforms_while_recording) {
    meshcat_->StartRecording(1.0 / params_.publish_period, set_transforms_while_recording);
...
template <typename T>
MeshcatVisualizer<T>& MeshcatVisualizer<T>::AddToBuilder(systems::DiagramBuilder<T>* builder,
                                                         const SceneGraph<T>& scene_graph,
                                                         std::shared_ptr<Meshcat> meshcat,
                                                         MeshcatVisualizerParams params) {
    return AddToBuilder(builder, scene_graph.get_query_output_port(), std::move(meshcat), std::move(params));
}
...
MeshcatVisualizer<T>& MeshcatVisualizer<T>::AddToBuilder(systems::DiagramBuilder<T>* builder,
                                                         ...
                                                         std::shared_ptr<Meshcat> meshcat,
                                                         MeshcatVisualizerParams params) {
    const std::string aspirational_name = fmt::format("meshcat_visualizer({})", params.prefix);
    auto& visualizer = *builder->template AddSystem<MeshcatVisualizer<T>>(std::move(meshcat), std::move(params));
...
template <typename T>
systems::EventStatus MeshcatVisualizer<T>::UpdateMeshcat(const systems::Context<T>& context) const {
    ...
    if (!version_.has_value() || !version_->IsSameAs(current_version, params_.role)) {
    ...
    std::optional<double> rate =
            realtime_rate_calculator_.UpdateAndRecalculate(ExtractDoubleOrThrow(context.get_time()));
    if (rate) {
...
template <typename T>
void MeshcatVisualizer<T>::SetObjects(const SceneGraphInspector<T>& inspector) const {
    ...
    for (FrameId frame_id : inspector.GetAllFrameIds()) {
        std::string frame_path = frame_id == inspector.world_frame_id()
                                         ? params_.prefix
        ...
        for (GeometryId geom_id : inspector.GetGeometries(frame_id, params_.role)) {
            const GeometryProperties& properties = *inspector.GetProperties(geom_id, params_.role);
            if (properties.HasProperty("meshcat", "accepting")) {
                if (properties.GetProperty<std::string>("meshcat", "accepting") != params_.prefix) {
                    continue;
                }
            ...
            const std::string geometry_name = internal::TransformGeometryName(geom_id, inspector);
            const std::string path = fmt::format("{}/{}", frame_path, geometry_name);
            const Rgba rgba = properties.GetPropertyOrDefault("phong", "diffuse", params_.default_color);
                ...
                if (params_.show_hydroelastic) {
                    ...
                    std::visit<void>(overloaded{[](std::monostate) {},
                                                ...
                                                [&](const VolumeMesh<double>* mesh) {
                                                    DRAKE_DEMAND(mesh != nullptr);
                                                    meshcat_->SetObject(path, ConvertVolumeToSurfaceMesh(*mesh), rgba);
                                                    geometry_already_set = true;
                                                }},
                                     maybe_mesh);
                }
            ...
            if (const PolygonSurfaceMesh<double>* hull = nullptr; (!geometry_already_set) &&
...
template <typename T>
void MeshcatVisualizer<T>::SetTransforms(const systems::Context<T>& context, const QueryObject<T>& query_object) const {
    for (const auto& [frame_id, path] : dynamic_frames_) {
        ...
        meshcat_->SetTransform(path, X_WF, ExtractDoubleOrThrow(context.get_time()));
    }
}
...
template <typename T>
systems::EventStatus MeshcatVisualizer<T>::OnInitialization(const systems::Context<T>&) const {
    Delete();
    return systems::EventStatus::Succeeded();
}

template <typename T>
typename systems::LeafSystem<T>::GraphvizFragment MeshcatVisualizer<T>::DoGetGraphvizFragment(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ::drake::geometry::MeshcatVisualizer);
