...
GLuint MakeGlTexture(void* pixel_data, int width, int height, GLint internal_format, GLenum type) {
    GLuint texture_id;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, width, height, 0, internal_format, type, pixel_data);
...
std::optional<GLuint> TextureLibrary::GetTextureId(const std::string& file_name_in) {
    ...
    const GLint internal_format = num_channels == 4 ? GL_RGBA : (num_channels == 3 ? GL_RGB : 0);
    if (internal_format == 0) {
        return std::nullopt;
    }

    unsigned char* pixel = static_cast<unsigned char*>(image->GetScalarPointer());
    GLuint texture_id = MakeGlTexture(pixel, width, height, internal_format, GL_UNSIGNED_BYTE);
...
void TextureLibrary::AddInMemoryImages(const std::map<std::string, std::vector<unsigned char>>& images) {
    for (const auto& [name, texture] : images) {
        ...
        ImageIo::ByteSpan texture_bytes{.data = texture.data(), .size = texture.size()};
        ImageIo image_io;
        ...
        std::visit(
                [this, &key]<typename SomeImage>(SomeImage& image) {
                    ...
                    const GLenum type = GetOpenGlTextureType(SomeImage::Traits::kPixelScalar);
                    const GLuint texture_id =
                            MakeGlTexture(image.at(0, 0), image.width(), image.height(), format, type);
...
std::string TextureLibrary::GetTextureKey(const std::string& file_uri) {
    ...
    const fs::path file_path = fs::is_symlink(path_in) ? fs::read_symlink(path_in) : path_in;
