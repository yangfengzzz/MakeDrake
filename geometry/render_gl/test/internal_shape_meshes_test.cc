...
Vector3d CalcTriCentroid(const RenderMesh& data, int tri_index) {
    const auto& vertices = data.positions;
    const auto& tris = data.indices;
    return (vertices.row(tris(tri_index, 0)) + vertices.row(tris(tri_index, 1)) + vertices.row(tris(tri_index, 2))) /
           3.f;
}
...
public:
    NormalCone(const RenderMesh& mesh_data, int t) {
        ...
        const Vector3d n_0 = mesh_data.normals.row(mesh_data.indices(t, 0)).normalized();
        const Vector3d n_1 = mesh_data.normals.row(mesh_data.indices(t, 1)).normalized();
        const Vector3d n_2 = mesh_data.normals.row(mesh_data.indices(t, 2)).normalized();
     ...
     */
    bool Contains(const Vector3d& dir) const { return ray_.dot(dir.normalized()) >= cos_theta_; }
...
GTEST_TEST(PrimitiveMeshTests, NormalCone) {
...

    {
        ...
        for (int axis = 0; axis < 3; ++axis) {
            ...
            const AngleAxisd R_outside(theta + kThetaEpsilon, Vector3d(-2, -1, 0).normalized());
...

    {
        ...
        const double theta = M_PI / 7;
        const double cos_theta_expected = std::cos(theta);
        const Vector3d n_base = AngleAxisd(theta, Vector3d(-2, 1, 0).normalized()) * expected_ray;
        for (int i = 0; i < 3; ++i) {
...
/* Testing utility to test various characteristics of the primitive shapes. As
 ...
 @param origin_is_inside  Reports if we have the expectation that the origin is
                          "inside" the mesh.
 */
void TestGenericPrimitiveTraits(const RenderMesh& mesh, bool origin_is_inside = true) {
...
GTEST_TEST(PrimitiveMeshTests, MakeLongLatUnitSphere) {
    ...
    for (int resolution : {3, 10, 20, 40}) {
        ...
        EXPECT_TRUE(CompareMatrices(mesh_data.uvs.row(0), Vector2d(0, 1).transpose()));
        const int row_count = mesh_data.uvs.rows();
        EXPECT_TRUE(CompareMatrices(mesh_data.uvs.row(row_count - 1), Vector2d(0, 0).transpose()));
    }
}
...
GTEST_TEST(PrimitiveMeshTests, MakeUnitCylinder) {
    ...
    for (int resolution : {3, 10, 20, 40}) {
        ...
        for (int v_i = mesh_data.positions.rows() - resolution - 1; v_i < mesh_data.positions.rows(); ++v_i) {
            EXPECT_EQ(mesh_data.positions(v_i, 2), -0.5);
        }
...
GTEST_TEST(PrimitiveMeshTests, MakeSquarePatch) {
    ...
    for (int resolution : {1, 4, 15}) {
        ...
        for (int t = 0; t < mesh_data.indices.rows(); ++t) {
            EXPECT_TRUE(CompareMatrices(CalcTriUnitNormal(mesh_data, t), Vector3d(0, 0, 1), kEps));
        }
        ...
        EXPECT_TRUE(CompareMatrices(mesh_data.uvs.row(0), Vector2d(0, 0).transpose()));
        const int row_count = mesh_data.uvs.rows();
        EXPECT_TRUE(CompareMatrices(mesh_data.uvs.row(row_count - 1), Vector2d(1, 1).transpose()));
    }
}
...
GTEST_TEST(PrimitiveMeshTests, MakeUnitBox) {
    ...
    for (int row = 0; row < mesh_data.uvs.rows(); row += 4) {
        ASSERT_TRUE(CompareMatrices(mesh_data.uvs.row(row), Vector2d(0, 0).transpose()));
        ASSERT_TRUE(CompareMatrices(mesh_data.uvs.row(row + 1), Vector2d(1, 0).transpose()));
        ASSERT_TRUE(CompareMatrices(mesh_data.uvs.row(row + 2), Vector2d(1, 1).transpose()));
        ASSERT_TRUE(CompareMatrices(mesh_data.uvs.row(row + 3), Vector2d(0, 1).transpose()));
    }
}
