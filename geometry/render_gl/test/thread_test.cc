...
TEST_P(ThreadTest, CallRenderEngineDirectly) {
    ...
    const GeometryInstance instance(RigidTransformd{},
                                    Mesh(FindResourceOrThrow("drake/geometry/render/test/meshes/box.obj"), 1), "box");
    ...
    const RigidTransformd X_WC(
            RotationMatrixd{AngleAxisd(M_PI, Vector3d::UnitY()) * AngleAxisd(-M_PI_2, Vector3d::UnitZ())}, {0, 0, 3.0});
    // The pose of the *sensor* frame S in the camera body frame.
    RigidTransformd X_CS;
    const ColorRenderCamera camera_params(
            RenderCameraCore("color", CameraInfo(640, 480, 2.0), ClippingRange(0.25, 10.0), X_CS));

    // The worker functor for the i'th camera.
    MatrixX<ImageRgba8U> images(num_workers, num_repeats);
    std::atomic<int> num_errors{0};
    auto work = [&images, &num_errors, num_repeats = num_repeats, &instance, &perception_properties, &X_WC,
                 &camera_params](int i) {
        try {
            ...
            renderer->RegisterVisual(instance.id(), instance.shape(), perception_properties, instance.pose(),
                                     false /* needs update */);
            for (int j = 0; j < num_repeats; ++j) {
                ...
                renderer->UpdateViewpoint(X_WC * camera_params.core().sensor_pose_in_camera_body());
    ...
    for (int x = 0; x < image_00.width(); ++x) {
        for (int y = 0; y < image_00.height(); ++y) {
            ...
            if (std::vector<uint8_t>(rgba, rgba + 4) == std::vector<uint8_t>{204, 229, 255, 255}) {
                ++num_clear_pixels;
            } else {
                ++num_box_pixels;
            }
