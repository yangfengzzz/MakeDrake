#include "geometry/render_gl/internal_render_engine_gl.h"
...
public:
    ...
    /* Constructs a tester on the given engine. The tester keeps a reference to
     ...
     tester.  */
    explicit RenderEngineGlTester(const RenderEngineGl* engine) : engine_(*engine) { DRAKE_DEMAND(engine != nullptr); }

    const internal::OpenGlContext& opengl_context() const { return *engine_.opengl_context_; }
    ...
    const TextureLibrary& texture_library() const { return *engine_.texture_library_; }

    const std::vector<internal::OpenGlGeometry>& opengl_geometries() const { return engine_.geometries_; }
...
struct RgbaColor {
    ...
    RgbaColor(int r_in, int g_in, int b_in, int a_in) : r(r_in), g(g_in), b(b_in), a(a_in) {}
    ...
    bool operator==(const RgbaColor& c) const { return r == c.r && g == c.g && b == c.b && a == c.a; }
...
bool IsColorNear(const RgbaColor& expected, const RgbaColor& tested, double tolerance = kColorPixelTolerance) {
    using std::abs;
...
::testing::AssertionResult CompareColor(const RgbaColor& expected,
    ...
    return ::testing::AssertionFailure() << "At pixel " << p << "\n  Expected: " << expected << "\n  Found: " << tested
...
public:
    RenderEngineGlTest()
        : color_(kWidth, kHeight),
          ...
          X_WR_(RotationMatrixd{AngleAxisd(M_PI, Vector3d::UnitY()) * AngleAxisd(-M_PI_2, Vector3d::UnitZ())},
...
protected:
    ...
    void VerifyUniformColor(const Rgba& rgba, const ImageRgba8U* color = nullptr) {
    ...
    void VerifyOutliers(const DepthRenderCamera& camera,
        ...
        for (const auto& screen_coord : GetOutliers(camera.core().intrinsics())) {
            ...
            EXPECT_EQ(label.at(screen_coord.x, screen_coord.y)[0], expected_outlier_label_)
    ...
    void Init(const RigidTransformd& X_WR, bool add_terrain = false) {
        ...
        EXPECT_FALSE(IsColorNear(kDefaultVisualColor, renderer_->parameters().default_diffuse));
    }

    // Tests that instantiate their own renderers can initialize their renderers
    // with this method.
    void InitializeRenderer(const RigidTransformd& X_WR, bool add_terrain, RenderEngineGl* engine) {
    ...
    void PopulateSphereTest(RenderEngineGl* renderer, bool use_texture = false) {
        ...
        renderer->RegisterVisual(sphere_id_, sphere, simple_material(use_texture), RigidTransformd::Identity(),
                                 true /* needs update */);
    ...
    void PopulateSimpleBoxTest(RenderEngineGl* renderer) {
        ...
        const double p_WVo_z = X_WR_.translation()(2) - length / 2 - expected_object_depth_;
    ...
    void PerformCenterShapeTest(RenderEngineGl* renderer, const DepthRenderCamera* camera = nullptr) {
    ...
    void VerifyCenterShapeTest(const DepthRenderCamera& camera,
        ...
        EXPECT_TRUE(CompareColor(expected_color_, color, inlier)) << "Color at: " << inlier;
        EXPECT_TRUE(IsExpectedDepth(depth, inlier, expected_object_depth_, kDepthTolerance)) << "Depth at: " << inlier;
        EXPECT_EQ(label.at(inlier.x, inlier.y)[0], static_cast<int>(expected_label_)) << "Label at: " << inlier;
    }
    ...
    void SetupGltfTest() {
        ...
        for (const char* resource : {"fully_textured_pyramid.bin", "fully_textured_pyramid_base_color.png"}) {
            const fs::path source_path =
                    FindResourceOrThrow(fmt::format("drake/geometry/render/test/meshes/{}", resource));
    ...
    static fs::path gltf_pyramid_path() {
        static never_destroyed<fs::path> path(
                FindResourceOrThrow("drake/geometry/render/test/meshes/fully_textured_pyramid.gltf"));
        return path.access();
    }
    ...
    void InitAndRegisterMesh(const fs::path& file_path) {
        ...
        const GeometryId id = GeometryId::get_new_id();
        renderer_->RegisterVisual(id, Mesh(file_path.string()), material, RigidTransformd::Identity(),
                                  false /* needs update */);
    }
    ...
    void RenderAndCompareAgainstRef(std::string_view image_file_name, RenderEngine* renderer_in = nullptr) {
        RenderEngine* renderer = renderer_in == nullptr ? renderer_.get() : renderer_in;
        ...
        if (const char* dir = std::getenv("TEST_UNDECLARED_OUTPUTS_DIR")) {
            ...
            systems::sensors::ImageIo{}.Save(image, out_dir / fmt::format("{}.png", image_file_name));
        }

        ImageRgba8U expected_image;
        const std::string ref_filename =
                FindResourceOrThrow("drake/geometry/render_gl/test/gl_fully_textured_pyramid_rendered.png");
...
TEST_F(RenderEngineGlTest, ControlBackgroundColor) {
    std::vector<Rgba> backgrounds{Rgba{0.1, 0.2, 0.3, 1.0}, Rgba{0.5, 0.6, 0.7, 1.0}, Rgba{1.0, 0.1, 0.4, 0.5}};
...
TEST_F(RenderEngineGlTest, HorizonTest) {
    ...
    RigidTransformd X_WR{
            RotationMatrixd{AngleAxisd(-M_PI_2, Vector3d::UnitX()) * AngleAxisd(M_PI_2, Vector3d::UnitY())}};
    Init(X_WR, true);
    ...
    auto CalcHorizon = [intrinsics](double z) {
        ...
        const double kFocalLength = intrinsics.height() * 0.5 / std::tan(0.5 * intrinsics.fov_y());
    ...
    for (const bool use_texture : {false, true}) {
        ...
        for (const double texture_scale : {1.0, 1.5, 0.5}) {
            ...
            if (use_texture) {
                ...
                if (texture_scaled) {
                    props.AddProperty("phong", "diffuse_scale", Vector2d{texture_scale, texture_scale});
                }
            }
            renderer_->RegisterVisual(id, box, props, RigidTransformd::Identity(), true /* needs update */);
            ...
            const double pixel_size = 4 * expected_object_depth_ * tan(intrinsics.fov_y() / 2.0) / intrinsics.height();
            RigidTransformd X_WV{RotationMatrixd{AngleAxisd(M_PI, Vector3d::UnitX())},
                                 Vector3d{(-box.width() + pixel_size) * 0.5, (-box.depth() + pixel_size) * 0.5, 0.625}};
            renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, X_WV}});
            ...
            SCOPED_TRACE(
                    fmt::format("Box test - {} - scale {}", use_texture ? "texture" : "diffuse color", texture_scale));
...
TEST_F(RenderEngineGlTest, SphereTest) {
    for (const bool use_texture : {false, true}) {
        ...
        SCOPED_TRACE(fmt::format("Sphere test {}", use_texture ? "textured" : "rgba"));
        PerformCenterShapeTest(renderer_.get());
    }
}
...
TEST_F(RenderEngineGlTest, TransparentSphereTest) {
    ...
    default_color_ = Rgba(kDefaultVisualColor.r(), kDefaultVisualColor.g(), kDefaultVisualColor.b(), int_alpha / 255.0);
    ...
    const RgbaColor expect_linear{blend(kDefaultVisualColor, kTerrainColor, linear_factor)};
    const double quad_factor = linear_factor * (-linear_factor + 2);
    const RgbaColor expect_quad{blend(kDefaultVisualColor, kTerrainColor, quad_factor)};

    const ScreenCoord inlier = GetInlier(intrinsics);
    const RgbaColor at_pixel(color.at(inlier.x, inlier.y));
    EXPECT_TRUE(CompareColor(expect_linear, color, inlier) || CompareColor(expect_quad, color, inlier))
...
TEST_F(RenderEngineGlTest, DeformableTest) {
    for (const bool use_texture : {false, true}) {
        ...
        std::vector<geometry::internal::RenderMesh> render_meshes =
                geometry::internal::LoadRenderMeshesFromObj(filename, material, unused_diffuse_color);
        ...
        SCOPED_TRACE(fmt::format("Deformable test -- has texture: {}", use_texture));

        PerformCenterShapeTest(renderer_.get());

        const Eigen::Matrix<double, Eigen::Dynamic, 3, Eigen::RowMajor>& initial_q_WG = render_meshes[0].positions;
        const Eigen::Matrix<double, Eigen::Dynamic, 3, Eigen::RowMajor>& initial_nhat_W = render_meshes[0].normals;
        ...
        auto flatten = [](const Eigen::Matrix<double, Eigen::Dynamic, 3, Eigen::RowMajor>& input) {
        ...
        renderer_->UpdateDeformableConfigurations(
                id, std::vector<VectorXd>{flatten(translate_all_vertices(Vector3d(0.99, 0, 0)))},
        ...
        renderer_->UpdateDeformableConfigurations(
                id, std::vector<VectorXd>{flatten(translate_all_vertices(Vector3d(1.01, 0, 0.0)))},
                std::vector<VectorXd>{flatten(initial_nhat_W)});
        PerformCenterShapeTest(renderer_.get());
        ...
        ResetExpectations();
        RgbaColor original_color = use_texture ? RgbaColor(kTextureColor) : default_color_;
        const Vector3d original_rgb(original_color.r, original_color.g, original_color.b);
        const Vector3d expected_rgb = original_rgb * std::cos(M_PI / 4.0);
        expected_color_ = RgbaColor(static_cast<int>(expected_rgb[0]), static_cast<int>(expected_rgb[1]),
                                    static_cast<int>(expected_rgb[2]), 255);
        expected_label_ = deformable_label;
        expected_object_depth_ += 0.5;

        renderer_->UpdateDeformableConfigurations(
                id, std::vector<VectorXd>{flatten(translate_all_vertices(Vector3d(0, 0, -0.5)))},
...
TEST_F(RenderEngineGlTest, CapsuleTest) {
    ...
    const GeometryId id = GeometryId::get_new_id();
    renderer_->RegisterVisual(id, capsule, simple_material(), RigidTransformd::Identity(), true /* needs update */);
...
TEST_F(RenderEngineGlTest, CapsuleRotatedTest) {
    ...
    // a total of 1.
    RigidTransformd X_WV{RotationMatrixd{AngleAxisd(M_PI / 2, Vector3d::UnitY())}, Vector3d{0, 0, 0.85}};
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, X_WV}});
    ...
    for (const int& offset : offsets) {
        ...
        EXPECT_TRUE(CompareColor(expected_color_, color_, offset_inlier)) << "Color at: " << offset_inlier;
        EXPECT_TRUE(IsExpectedDepth(depth_, offset_inlier, expected_object_depth_, kDepthTolerance))
                << "Depth at: " << offset_inlier;
        EXPECT_EQ(label_.at(x, y)[0], static_cast<int>(expected_label_)) << "Label at: " << offset_inlier;
    }
}
...
TEST_F(RenderEngineGlTest, CylinderTest) {
    for (const bool use_texture : {false, true}) {
        ...
        expected_label_ = RenderLabel(2);
        const GeometryId id = GeometryId::get_new_id();
        renderer_->RegisterVisual(id, cylinder, simple_material(use_texture), RigidTransformd::Identity(),
                                  true /* needs update */);
...
TEST_F(RenderEngineGlTest, EllipsoidTest) {
    ...
    expected_label_ = RenderLabel(2);
    const GeometryId id = GeometryId::get_new_id();
    renderer_->RegisterVisual(id, ellipsoid, simple_material(), RigidTransformd::Identity(), true /* needs update */);
    ...
    X_WV = RigidTransformd{RotationMatrixd{AngleAxisd(-M_PI / 2, Vector3d::UnitX())}, Vector3d{0, 0, target_z - b}};
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, X_WV}});
    {
    ...
    X_WV = RigidTransformd{RotationMatrixd{AngleAxisd(M_PI / 2, Vector3d::UnitY())}, Vector3d{0, 0, target_z - a}};
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, X_WV}});
    {
...
void TweakGltf(const fs::path& gltf_in, const fs::path& gltf_out, GltfTweaker tweaker) {
...
TEST_F(RenderEngineGlTest, GltfTinygltfErrors) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(malformed_gltf),
...
TEST_F(RenderEngineGlTest, GltfFindTargetRootNodesErrors) {
    ...
    TweakGltf(gltf_pyramid_path(), default_scene_oor, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        model["scene"] = 10;
    });
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(empty_scene), ".*scene 0 has no root nodes.*");

    // No nodes or scenes.
    const fs::path no_nodes_scenes = temp_dir_ / "no_nodes_scenes.gltf";
    TweakGltf(gltf_pyramid_path(), no_nodes_scenes, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        model.erase("nodes");
        model.erase("scenes");
        model.erase("scene");
    });
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(no_nodes_scenes), ".*no scenes and no nodes.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(node_cycle), ".*none of its 1 nodes are root nodes.*");
}
...
TEST_F(RenderEngineGlTest, GltfInstantiateMeshErrors) {
    ...
    TweakGltf(gltf_pyramid_path(), cross_buffer_attribs, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        DRAKE_DEMAND(model["bufferViews"][0]["buffer"].get<int>() == 0);
        model["bufferViews"][0]["buffer"] = 2;
    });
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(cross_buffer_attribs), ".*All attributes.*same buffer.*");
}
...
TEST_F(RenderEngineGlTest, GltfConfigureIndexBufferErrors) {
    ...
    TweakGltf(gltf_pyramid_path(), non_indexed_prim, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        model["meshes"][0]["primitives"][0].erase("indices");
    });
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(non_indexed_prim), ".*All meshes must be indexed.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(bad_index_type),
    ...
    TweakGltf(gltf_pyramid_path(), bad_index_stride, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        const int accessor_index = model["meshes"][0]["primitives"][0]["indices"].get<int>();
        const int bufferView_index = model["accessors"][accessor_index]["bufferView"].get<int>();
        model["bufferViews"][bufferView_index]["byteStride"] = 4;
    });
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(bad_index_stride), ".*indices must be compactly stored.*");
}
...
TEST_F(RenderEngineGlTest, GltfEmbeddedTextures) {
    ...
    TweakGltf(gltf_pyramid_path(), embedded_path, [](nlohmann::json* model_ptr) {
        ...
        model["buffers"].push_back(
                {{"byteLength", byte_length},
                 {"uri", fmt::format("data:application/octet-stream;base64,{}", std::move(data_contents).str())}});

        const int buffer_view_index = ssize(model["bufferViews"]);
        model["bufferViews"].push_back({{"buffer", buffer_index}, {"byteLength", byte_length}, {"byteOffset", 0}});
    ...
    ASSERT_TRUE(library.textures().begin()->first.starts_with(TextureLibrary::InMemoryPrefix()));
}
...
TEST_F(RenderEngineGlTest, GltfIncludedNodes) {
    ...
    TweakGltf(gltf_pyramid_path(), no_default_scene, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
...
TEST_F(RenderEngineGlTest, GltfNodeTransforms) {
    ...
    TweakGltf(gltf_pyramid_path(), from_xform_elements, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        ...
        pyramid["translation"] = std::vector<double>(p_10.data(), p_10.data() + 3);
    ...
    TweakGltf(gltf_pyramid_path(), non_uniform_scale, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        DRAKE_DEMAND(model["nodes"].size() == 1);
        model["nodes"].push_back({{"name", "injected"}, {"children", {0}}, {"scale", {4, 2, 5}}});
        auto& pyramid = model["nodes"][0];
    ...
    TweakGltf(gltf_pyramid_path(), transform_matrix, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
        RigidTransformd X_F1(RotationMatrixd(), Vector3d(1, 2, 3));
        const Matrix4d T_F1 = DiagonalMatrix<double, 4>(4, 2, 5, 1) * X_F1.GetAsMatrix4();
        const Matrix4d T_10 = X_F1.inverse().GetAsMatrix4() * DiagonalMatrix<double, 4>(0.25, 0.5, 0.2, 1);
        ...
        DRAKE_DEMAND(model["nodes"].size() == 1);
        model["nodes"].push_back({{"name", "injected"}, {"children", {0}}, {"matrix", matrix_vector(T_F1)}});
        auto& pyramid = model["nodes"][0];
...
TEST_F(RenderEngineGlTest, GltfVertexAttributes) {
    ...
    TweakGltf(gltf_pyramid_path(), missing_normals, [](nlohmann::json* model_ptr) {
        nlohmann::json& model = *model_ptr;
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(InitAndRegisterMesh(missing_normals), ".* missing the attribute 'NORMAL'.*");
    ...
    ASSERT_EQ(ssize(RenderEngineGlTester(renderer_.get()).texture_library().textures()), 0);
...
TEST_F(RenderEngineGlTest, GltfMaterialHeuristic) {
    ...
    const std::string diffuse_map = FindResourceOrThrow("drake/geometry/render/test/meshes/rainbow_stripes.png");
    Init(RigidTransformd::Identity());
    PerceptionProperties material;
    material.AddProperty("label", "id", RenderLabel(1));
    material.AddProperty("phong", "diffuse_map", diffuse_map);
    const GeometryId id = GeometryId::get_new_id();
    renderer_->RegisterVisual(id, Mesh(no_material.string()), material, RigidTransformd::Identity(),
                              false /* needs update */);
    ...
    // if we'd tried using them, we would've had a file access violation.
    ASSERT_EQ(ssize(library.textures()), 1);
    ASSERT_TRUE(library.textures().begin()->first.ends_with("rainbow_stripes.png"));
}
...
TEST_F(RenderEngineGlTest, MultiMaterialObj) {
    ...
    const std::string filename = FindResourceOrThrow("drake/geometry/render/test/meshes/rainbow_box.obj");
    ...
    const std::vector<Face> faces{
            {.rendered_color = Rgba(0.016, 0.945, 0.129), .rotation = RotationMatrixd(), .name = "green"},
    ...
    for (const auto& face : faces) {
        SCOPED_TRACE(fmt::format("multi-material test on {} face - original", face.name));
    ...
    for (const auto& face : faces) {
        SCOPED_TRACE(fmt::format("multi-material test on {} face - clone", face.name));
        expected_color_ = face.rendered_color;

        gl_clone->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd(face.rotation)}});
    ...
    expected_label_ = expected_outlier_label_;
    for (const auto& face : faces) {
        SCOPED_TRACE(fmt::format("multi-material test on {} face - after removal", face.name));
...
TEST_F(RenderEngineGlTest, ConvexTest) {
    ...
    auto filename = FindResourceOrThrow("drake/examples/scene_graph/cuboctahedron_with_hole.obj");
    ...
    PerceptionProperties material = simple_material();
    const GeometryId id = GeometryId::get_new_id();
    renderer_->RegisterVisual(id, convex, material, RigidTransformd::Identity(), true /* needs update */);
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd::Identity()}});
...
TEST_F(RenderEngineGlTest, RemoveVisual) {
    ...
    auto add_sphere = [this, kRadius](double z, GeometryId geometry_id) {
        ...
        renderer_->RegisterVisual(geometry_id, sphere, material, RigidTransformd::Identity(), true);
...
TEST_F(RenderEngineGlTest, CloneIndependence) {
    ...
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{sphere_id_, X_WT_new}});
...
TEST_F(RenderEngineGlTest, DefaultProperties) {
    ...
    PerceptionProperties material;
    material.AddProperty("label", "id", RenderLabel::kDontCare);
    renderer_->RegisterVisual(id, box, material, RigidTransformd::Identity(), true);
...
::testing::AssertionResult ImagesExactlyEqual(const ImageDepth32F& ref, const ImageDepth32F& test) {
    if (ref.size() != test.size()) {
        return ::testing::AssertionFailure() << "Image sizes don't match\n"
...
TEST_F(RenderEngineGlTest, RendererIndependence) {
    ...
    auto add_terrain = [ground](auto* renderer) {
        PerceptionProperties material;
        material.AddProperty("label", "id", RenderLabel::kDontCare);
        renderer->RegisterVisual(ground, HalfSpace(), material, RigidTransformd::Identity(), false /* needs update */);
    };
    ...
    engine2.RegisterVisual(box_id, box, simple_material(), RigidTransformd::Identity(), true);
    RigidTransformd X_WV{Translation3d(0.2, 0.2, 0.5)};
    engine2.UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{box_id, X_WV}});
    Render(&engine2);
...
TEST_F(RenderEngineGlTest, ShowRenderLabel) {
    ...
    const internal::OpenGlContext& context = RenderEngineGlTester(&engine).opengl_context();
    ...
    engine.RenderLabelImage(ColorRenderCamera{depth_camera_.core(), true}, &image);
    ASSERT_TRUE(context.IsWindowViewable());
    engine.RenderLabelImage(ColorRenderCamera{depth_camera_.core(), false}, &image);
    ASSERT_FALSE(context.IsWindowViewable());
...
TEST_F(RenderEngineGlTest, ConvexGeometryReuse) {
    ...
    auto add_convex = [&filename, &engine](double scale) {
        ...
        const bool accepted = engine.RegisterVisual(id, Convex(filename, scale), material, RigidTransformd());
...
TEST_F(RenderEngineGlTest, MeshGeometryReuse) {
    ...
    auto filename = FindResourceOrThrow("drake/geometry/render/test/meshes/box.obj");

    auto add_mesh = [&filename, &engine]() {
        const GeometryId id = GeometryId::get_new_id();
        PerceptionProperties material;
        material.AddProperty("label", "id", RenderLabel(17));
        const bool accepted = engine.RegisterVisual(id, Mesh(filename), material, RigidTransformd());
        DRAKE_DEMAND(accepted);
        return id;
    };
        ...
        for (int c = 0; c < image.width(); ++c) {
            for (int r : {0, mid_height}) {
                ...
                if (!IsColorNear(dut, config.expected_color)) {
                    EXPECT_EQ(dut, config.expected_color) << "image color: " << dut << "\n"
...
TEST_F(RenderEngineGlTest, SingleLight) {
    ...
    const Rgba low_intensity_color(kTerrainColor.r() * 0.1, kTerrainColor.g() * 0.1,
...
TEST_F(RenderEngineGlTest, MultiLights) {
    ...
    // Lights combine.
    {
        ...
        const RenderEngineGlParams params{.lights = {{.type = "point", .intensity = 0.25},
