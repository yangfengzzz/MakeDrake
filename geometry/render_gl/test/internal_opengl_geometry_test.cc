...
GTEST_TEST(OpenGlGeometryTest, IsDefined) {
    ...
    auto make_geo = [](GLuint a, GLuint b, GLuint c, int d, int e) {
        return OpenGlGeometry{.vertex_array = a, .vertex_buffer = b, .index_buffer = c, .v_count = d, .index_count = e};
    };
...
GTEST_TEST(OpenGlGeometryTest, ThrowIfUndefined) {
    const OpenGlGeometry valid{
            .vertex_array = 1, .vertex_buffer = 2, .index_buffer = 3, .v_count = 4, .index_count = 5};
    EXPECT_NO_THROW(valid.throw_if_undefined("test message"));
    DRAKE_EXPECT_THROWS_MESSAGE(OpenGlGeometry{.vertex_array = 1}.throw_if_undefined("default is undefined"),
...
GTEST_TEST(OpenGlInstanceTest, Validity) {
  ...
  const Matrix3f N_MN = (Matrix3f() << 21, 22, 23,
    ...
    // real meaning.
    const ShaderProgramData depth_data{ShaderId::get_new_id(), AbstractValue::Make(7.3)};
    const ShaderProgramData label_data{ShaderId::get_new_id(), AbstractValue::Make(RenderLabel(13))};
    const ShaderProgramData color_data(ShaderId::get_new_id(), AbstractValue::Make(33));

    const OpenGlInstance instance(geometry_index, scale.head<3>(), geometry, color_data, depth_data, label_data);
    ...
    EXPECT_EQ(instance.shader_data[RenderType::kDepth].value().get_value<double>(),
              depth_data.value().get_value<double>());
    EXPECT_EQ(instance.shader_data[RenderType::kDepth].shader_id(), depth_data.shader_id());
    EXPECT_EQ(instance.shader_data[RenderType::kLabel].value().get_value<RenderLabel>(),
              label_data.value().get_value<RenderLabel>());
    EXPECT_EQ(instance.shader_data[RenderType::kLabel].shader_id(), label_data.shader_id());
    EXPECT_EQ(instance.shader_data[RenderType::kColor].value().get_value<int>(), color_data.value().get_value<int>());
    EXPECT_EQ(instance.shader_data[RenderType::kColor].shader_id(), color_data.shader_id());
}
