...
GLXContext glXCreateContextAttribsARB(
    ...
    return GetGlXFunctionArb<GLXContext(Display*, GLXFBConfig, GLXContext, Bool, const int*)>(
            "glXCreateContextAttribsARB")(dpy, config, share_context, direct, attrib_list);
}
...
void GlDebugCallback(GLenum, GLenum type, GLuint, GLenum severity, GLsizei, const GLchar* message, const void*) {
    constexpr const char* output = "GL CALLBACK: {:s} type = 0x{:x}, severity = 0x{:x}, message = {:s}";
...
public:
    ...
    explicit Impl(bool debug, GLXContext source_context = NULL) : debug_(debug) {
        ...
        GLXFBConfig* fb_configs = glXChooseFBConfig(display(), screen_id, kVisualAttribs, &fb_count);
        ...
        window_attribs.colormap =
                XCreateColormap(display(), RootWindow(display(), screen_id), visual->visual, AllocNone);
        ScopeExit colormap_guard([colormap = window_attribs.colormap, &is_complete]() {
        ...
        window_ = XCreateWindow(display(), RootWindow(display(), screen_id), 0, 0, window_width_, window_height_, 0,
                                visual->depth, InputOutput, visual->visual, CWColormap | CWEventMask, &window_attribs);
        ScopeExit window_guard([window = window_, &is_complete]() {
            if (!is_complete) XDestroyWindow(display(), window);
        });

        // Create an OpenGL context.
        const int kContextAttribs[] = {GLX_CONTEXT_MAJOR_VERSION_ARB, 3, GLX_CONTEXT_MINOR_VERSION_ARB, 3, None};
        ...
        context_ = glXCreateContextAttribsARB(display(), fb_configs[0], source_context, True, kContextAttribs);
        if (context_ == nullptr) {
            throw std::runtime_error(
    ...
    void MakeCurrent() const {
        if (glXGetCurrentContext() != context_ && !glXMakeCurrent(display(), window_, context_)) {
...
OpenGlContext::OpenGlContext(bool debug) : impl_(new OpenGlContext::Impl(debug)) {}

OpenGlContext::OpenGlContext(const OpenGlContext& other) : impl_(std::make_unique<OpenGlContext::Impl>(*other.impl_)) {}
