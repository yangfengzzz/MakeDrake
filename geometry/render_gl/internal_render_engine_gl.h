...
#include "common/reset_on_copy.h"
#include "geometry/geometry_roles.h"
#include "geometry/render/render_engine.h"
#include "geometry/render/render_material.h"
#include "geometry/render/render_mesh.h"
#include "geometry/render_gl/internal_buffer_dim.h"
#include "geometry/render_gl/internal_opengl_context.h"
#include "geometry/render_gl/internal_opengl_geometry.h"
#include "geometry/render_gl/internal_shader_program.h"
#include "geometry/render_gl/internal_shape_meshes.h"
#include "geometry/render_gl/internal_texture_library.h"
...
private:
    ...
    // cache.
    void ImplementMeshesForFile(void* user_data,
    ...
    void DoUpdateVisualPose(GeometryId id, const math::RigidTransformd& X_WG) final;
    ...
    void DoRenderDepthImage(const render::DepthRenderCamera& render_camera,
    ...
    // render type.
    void RenderAt(const ShaderProgram& shader_program, RenderType render_type) const;
    ...
    //                         S_GM.
    void AddGeometryInstance(int geometry_index, void* user_data, const Vector3<double>& scale);
    ...
    void CacheFileMeshesMaybe(const std::string& filename, RegistrationData* data);
    ...
    static std::tuple<GLint, GLenum, GLenum> get_texture_format(RenderType render_type);
    ...
    static RenderTarget CreateRenderTarget(const render::RenderCameraCore& camera, RenderType render_type);
    ...
    void GetLabelImage(drake::systems::sensors::ImageLabel16I* label_image_out, const RenderTarget& target) const;
    ...
    // draw commands.
    RenderTarget GetRenderTarget(const render::RenderCameraCore& camera, RenderType render_type) const;
    ...
    int CreateGlGeometry(const geometry::internal::RenderMesh& mesh_data, bool is_deformable = false);
    ...
    // type.
    ShaderId AddShader(std::unique_ptr<ShaderProgram> program, RenderType render_type);
    ...
    ShaderProgramData GetShaderProgram(const PerceptionProperties& properties, RenderType render_type) const;
    ...
    const std::vector<render::LightParameter>& active_lights() const {
        if (active_lights_ == nullptr) {
            active_lights_ = parameters_.lights.size() > 0 ? &parameters_.lights : &fallback_lights_;
        }
    ...
    std::array<std::unordered_map<ShaderId, copyable_unique_ptr<ShaderProgram>>, RenderType::kTypeCount>
            shader_programs_;
    ...
    struct RenderGlMesh {
        ...
        std::optional<geometry::internal::RenderMaterial> mesh_material{std::nullopt};
    };
    ...
    mutable std::array<std::unordered_map<BufferDim, RenderTarget>, RenderType::kTypeCount> frame_buffers_;
    ...
    mutable reset_on_copy<const std::vector<render::LightParameter>*> active_lights_{};
