...
private:
    ...
    void DoSetModelViewMatrix(const Matrix4f& X_CW, const Matrix4f& T_WM, const Matrix3f& N_WM) const override {
    ...
    void SetLightParameters(int index, const LightParameter& light) const {
        glUniform1i(GetLightFieldLocation(index, "type"), static_cast<int>(render::light_type_from_string(light.type)));
        ...
        const render::LightFrame frame = render::light_frame_from_string(light.frame);
        ...
        glUniform3fv(GetLightFieldLocation(index, "atten_coeff"), 1, atten_coeff.data());
        glUniform1f(GetLightFieldLocation(index, "intensity"), static_cast<float>(light.intensity));
...
public:
    ...
    explicit DefaultRgbaColorShader(const Rgba& default_diffuse) : LightingShader(), default_diffuse_(default_diffuse) {
        ...
        LoadFromSources(fmt::format("{}{}", LightingShader::kVertexShader, kVertexShader),
    ...
    void SetInstanceParameters(const ShaderProgramData& data) const final {
        glUniform4fv(diffuse_color_loc_, 1, data.value().get_value<Vector4<float>>().data());
    }

private:
    void DoConfigureMoreUniforms() final { diffuse_color_loc_ = GetUniformLocation("diffuse_color"); }

    std::unique_ptr<ShaderProgram> DoClone() const final { return make_unique<DefaultRgbaColorShader>(*this); }

    std::optional<ShaderProgramData> DoCreateProgramData(const PerceptionProperties& properties) const final {
        const Rgba diffuse = properties.GetPropertyOrDefault("phong", "diffuse", default_diffuse_);
...
private:
    ...
    std::unique_ptr<ShaderProgram> DoClone() const final { return make_unique<DefaultTextureColorShader>(*this); }
    ...
    std::optional<ShaderProgramData> DoCreateProgramData(const PerceptionProperties& properties) const final {
        ...
        const string& file_name = properties.GetProperty<string>("phong", "diffuse_map");
        ...
        const auto& scale = properties.GetPropertyOrDefault("phong", "diffuse_scale", Vector2d(1, 1));
        const bool flip_y = properties.GetPropertyOrDefault("texture", "flip", false);
        return ShaderProgramData{shader_id(),
                                 AbstractValue::Make(InstanceData{*texture_id, scale.cast<float>(), flip_y})};
    }
...
public:
    ...
    DefaultDepthShader() : ShaderProgram() { LoadFromSources(kVertexShader, kFragmentShader); }
...
private:
    ...
    std::unique_ptr<ShaderProgram> DoClone() const final { return make_unique<DefaultDepthShader>(*this); }

    std::optional<ShaderProgramData> DoCreateProgramData(const PerceptionProperties&) const final {
...
public:
    ...
    */
    explicit DefaultLabelShader(std::function<Vector4<float>(const PerceptionProperties&)> label_encoder)
        : ShaderProgram(), label_encoder_(std::move(label_encoder)) {
        LoadFromSources(kVertexShader, kFragmentShader);
    }

    void SetInstanceParameters(const ShaderProgramData& data) const final {
        glUniform4fv(encoded_label_loc_, 1, data.value().get_value<Vector4<float>>().data());
    }

private:
    void DoConfigureUniforms() final { encoded_label_loc_ = GetUniformLocation("encoded_label"); }

    std::unique_ptr<ShaderProgram> DoClone() const final { return make_unique<DefaultLabelShader>(*this); }

    std::optional<ShaderProgramData> DoCreateProgramData(const PerceptionProperties& properties) const final {
        return ShaderProgramData{shader_id(), AbstractValue::Make(label_encoder_(properties))};
    }
...
std::string GetPathKey(const std::string& filename, bool is_convex) {
    ...
    if (path_error) {
        throw std::runtime_error(
                fmt::format("RenderEngineGl: unable to access the file {}; {}", filename, path_error.message()));
    }
...
RenderEngineGlParams CleanupLights(RenderEngineGlParams params) {
    if (ssize(params.lights) > LightingShader::kMaxNumLights) {
        throw std::runtime_error(
                fmt::format("RenderEngineGl supports up to five lights; {} specified.", ssize(params.lights)));
    }
...
RenderEngineGl::RenderEngineGl(RenderEngineGlParams params)
    ...
    AddShader(make_unique<DefaultRgbaColorShader>(params.default_diffuse), RenderType::kColor);
    AddShader(make_unique<DefaultTextureColorShader>(texture_library_), RenderType::kColor);
    ConfigureLights();
...
void RenderEngineGl::ImplementGeometry(const Box& box, void* user_data) {
    ...
    AddGeometryInstance(geometry, user_data, Vector3d(box.width(), box.depth(), box.height()));
}

void RenderEngineGl::ImplementGeometry(const Capsule& capsule, void* user_data) {
    const int resolution = 50;
    RenderMesh render_mesh = MakeCapsule(resolution, capsule.radius(), capsule.length());
...
void RenderEngineGl::ImplementGeometry(const Convex& convex, void* user_data) {
    ...
    ImplementMeshesForFile(user_data, kUnitScale * convex.scale(), convex.filename(), /* is_convex=*/true);
}

void RenderEngineGl::ImplementGeometry(const Cylinder& cylinder, void* user_data) {
...
void RenderEngineGl::ImplementGeometry(const Ellipsoid& ellipsoid, void* user_data) {
    const int geometry = GetSphere();
    AddGeometryInstance(geometry, user_data, Vector3d(ellipsoid.a(), ellipsoid.b(), ellipsoid.c()));
}
...
void RenderEngineGl::ImplementGeometry(const Mesh& mesh, void* user_data) {
    ...
    if (data->accepted) {
        ImplementMeshesForFile(user_data, kUnitScale * mesh.scale(), mesh.filename(), /* is_convex=*/false);
    }
}
...
void RenderEngineGl::ImplementMeshesForFile(void* user_data,
    ...
    for (const auto& gl_mesh : meshes_.at(file_key)) {
        ...
        if (gl_mesh.mesh_material.has_value()) {
            ...
            material = *MaybeMakeMeshFallbackMaterial(data.properties, filename, parameters_.default_diffuse,
        ...
        RegistrationData temp_data{data.id, data.X_WG, temp_props, data.default_diffuse};
        AddGeometryInstance(gl_mesh.mesh_index, &temp_data, scale);
    }
}

bool RenderEngineGl::DoRegisterVisual(GeometryId id,
                                      const Shape& shape,
...
bool RenderEngineGl::DoRegisterDeformableVisual(GeometryId id,
                                                const std::vector<RenderMesh>& render_meshes,
                                                const PerceptionProperties& properties) {
    opengl_context_->MakeCurrent();
    std::vector<int> gl_mesh_indices;
    for (const auto& render_mesh : render_meshes) {
        const int mesh_index = CreateGlGeometry(render_mesh, /* is_deformable */ true);
        DRAKE_DEMAND(mesh_index >= 0);
        gl_mesh_indices.emplace_back(mesh_index);
        const RenderMaterial& material = render_mesh.material.has_value()
                                                 ? *render_mesh.material
                                                 : MakeDiffuseMaterial(parameters_.default_diffuse);
        PerceptionProperties mesh_properties(properties);
        mesh_properties.UpdateProperty("phong", "diffuse_map", material.diffuse_map);
        mesh_properties.UpdateProperty("phong", "diffuse", material.diffuse);
        RegistrationData data{id, RigidTransformd::Identity(), mesh_properties, parameters_.default_diffuse};
...
void RenderEngineGl::DoUpdateVisualPose(GeometryId id, const RigidTransformd& X_WG) {
...
void RenderEngineGl::DoUpdateDeformableConfigurations(GeometryId id,
    ...
    for (int i = 0; i < ssize(q_WGs); ++i) {
        ...
        glNamedBufferSubData(geometry.vertex_buffer, positions_offset * sizeof(GLfloat), q_WG.size() * sizeof(GLfloat),
                             q_WG.data());
        // Update vertex normal data.
        std::size_t normals_offset = q_WG.size();
        glNamedBufferSubData(geometry.vertex_buffer, normals_offset * sizeof(GLfloat), nhat_W.size() * sizeof(GLfloat),
                             nhat_W.data());
    }
}
...
bool RenderEngineGl::DoRemoveGeometry(GeometryId id) {
    ...
    if (iter != visuals_.end()) {
        ...
        auto maybe_remove_from_family = [this, &visited_families](GeometryId g_id, const auto& shader_data,
...
void RenderEngineGl::RenderAt(const ShaderProgram& shader_program, RenderType render_type) const {
    ...
    for (const GeometryId& g_id : shader_families_.at(render_type).at(shader_program.shader_id())) {
        for (const auto& instance : visuals_.at(g_id).instances) {
            if (instance.shader_data.at(render_type).shader_id() != shader_program.shader_id()) {
                continue;
            }
...
void RenderEngineGl::DoRenderColorImage(const ColorRenderCamera& camera, ImageRgba8U* color_image_out) const {
    opengl_context_->MakeCurrent();
    ...
    const RenderTarget render_target = GetRenderTarget(camera.core(), RenderType::kColor);
    const Vector4<float> clear_color = parameters_.default_clear_color.rgba().cast<float>();
    glClearNamedFramebufferfv(render_target.frame_buffer, GL_COLOR, 0, clear_color.data());
    ...
    glGetTextureImage(render_target.value_texture, 0, GL_RGBA, GL_UNSIGNED_BYTE, color_image_out->size(),
                      color_image_out->at(0, 0));
}

void RenderEngineGl::DoRenderDepthImage(const DepthRenderCamera& camera, ImageDepth32F* depth_image_out) const {
    opengl_context_->MakeCurrent();

    const RenderTarget render_target = GetRenderTarget(camera.core(), RenderType::kDepth);
    ...
    glClearNamedFramebufferfv(render_target.frame_buffer, GL_COLOR, 0, &ImageTraits<PixelType::kDepth32F>::kTooFar);
    glClear(GL_DEPTH_BUFFER_BIT);
    ...
    glGetTextureImage(render_target.value_texture, 0, GL_RED, GL_FLOAT, depth_image_out->size() * sizeof(GLfloat),
                      depth_image_out->at(0, 0));
}

void RenderEngineGl::DoRenderLabelImage(const ColorRenderCamera& camera, ImageLabel16I* label_image_out) const {
    opengl_context_->MakeCurrent();

    const RenderTarget render_target = GetRenderTarget(camera.core(), RenderType::kLabel);
    ...
    float clear_color[4] = {static_cast<float>(empty_color.r()), static_cast<float>(empty_color.g()),
                            static_cast<float>(empty_color.b()), 1.0f};
    glClearNamedFramebufferfv(render_target.frame_buffer, GL_COLOR, 0, &clear_color[0]);
    glClear(GL_DEPTH_BUFFER_BIT);
...
void RenderEngineGl::AddGeometryInstance(int geometry_index, void* user_data, const Vector3d& scale) {
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    std::optional<ShaderProgramData> color_data = GetShaderProgram(data.properties, RenderType::kColor);
    std::optional<ShaderProgramData> depth_data = GetShaderProgram(data.properties, RenderType::kDepth);
    std::optional<ShaderProgramData> label_data = GetShaderProgram(data.properties, RenderType::kLabel);
    DRAKE_DEMAND(color_data.has_value() && depth_data.has_value() && label_data.has_value());

    visuals_[data.id].instances.push_back({geometry_index, scale.cast<float>(), geometries_.at(geometry_index),
...
int RenderEngineGl::GetSphere() {
    if (sphere_ < 0) {
        ...
        RenderMesh render_mesh = MakeLongLatUnitSphere(kLongitudeBands, kLatitudeBands);

        sphere_ = CreateGlGeometry(render_mesh);
    }

    geometries_[sphere_].throw_if_undefined("Built-in sphere has some invalid objects");
...
int RenderEngineGl::GetCylinder() {
    ...
    geometries_[cylinder_].throw_if_undefined("Built-in cylinder has some invalid objects");
...
int RenderEngineGl::GetHalfSpace() {
    ...
    geometries_[half_space_].throw_if_undefined("Built-in half space has some invalid objects");
...
void RenderEngineGl::CacheConvexHullMesh(const Convex& convex, const RegistrationData& data) {
    const std::string file_key = GetPathKey(convex.filename(), /*is_convex=*/true);

    if (!meshes_.contains(file_key)) {
        const Convex unit_convex(convex.filename(), 1.0);
        const PolygonSurfaceMesh<double>& hull =
                convex.scale() == 1.0 ? convex.GetConvexHull() : unit_convex.GetConvexHull();
        const TriangleSurfaceMesh<double> tri_hull = geometry::internal::MakeTriangleFromPolygonMesh(hull);
        RenderMesh render_mesh =
                geometry::internal::MakeFacetedRenderMeshFromTriangleSurfaceMesh(tri_hull, data.properties);
        ...
        meshes_[file_key] = vector<RenderGlMesh>{{.mesh_index = mesh_index, .uv_state = render_mesh.uv_state}};
    }
}
...
void CreateVertexArray(OpenGlGeometry* geometry) {
    ...
    auto init_array = [&g = *geometry](const VertexAttrib& props) {
        glVertexArrayVertexBuffer(g.vertex_array, props.attribute_index, g.vertex_buffer, props.byte_offset,
                                  props.stride);
        glVertexArrayAttribFormat(g.vertex_array, props.attribute_index, props.components_per_element,
                                  props.numeric_type, GL_FALSE, 0);
...
public:
    ...
    GltfMeshExtractor(std::vector<OpenGlGeometry>* geometries, TextureLibrary* texture_library)
    ...
    /* Extracts the geometry data (meshes and accompanying data) from the named
     ...
     instances returned. */
    vector<RenderGlMesh> ExtractMeshes(std::string_view file_name, const RegistrationData& data) {
        ...
        auto load_image_callback = [&all_embedded_images, this](
            ...
            all_embedded_images.insert({image->uri, vector<unsigned char>(bytes, bytes + size)});
            return true;
        };
        ...
        const bool valid_parse = loader.LoadASCIIFromFile(&model, &error, &warn, file_name_);

        if (!valid_parse) {
            policy_.Error(fmt::format("Failed parsing the glTF file: {}: {}", file_name_, error));
        }
        ...
        vector<RenderGlMesh> meshes = BuildGeometriesFromRootNodes(root_indices, model, data);
        ...
        for (const auto& mesh : meshes) {
            ...
            if (diffuse_map.starts_with(TextureLibrary::InMemoryPrefix())) {
                used_embedded_images[diffuse_map] = std::move(all_embedded_images[diffuse_map]);
...
private:
    ...
    string GetEmbeddedImageName(int image_index) const {
        /* Make sure we use TextureLibrary's required prefix. We also append a
         ...
         file path. */
        return fmt::format("{}{}?image={}", TextureLibrary::InMemoryPrefix(), file_name_, image_index);
    }
    ...
    vector<int> FindTargetRootNodes(const tinygltf::Model& model) const {
        ...
        if (model.scenes.size() > 0) {
            if (model.defaultScene >= ssize(model.scenes)) {
                policy_.Error(
                        fmt::format("Error parsing a glTF file; it defines {} scenes but has an "
            ...
            if (root_indices.empty()) {
                policy_.Error(fmt::format("Error parsing a glTF file; scene {} has no root nodes. '{}'.", scene_index,
                                          file_name_));
            }
        } else {
            if (model.nodes.size() == 0) {
                policy_.Error(
                        fmt::format("Error parsing a glTF file; it has no scenes and no nodes. '{}'.", file_name_));
            }
    ...
    vector<RenderGlMesh> BuildGeometriesFromRootNodes(const vector<int>& root_indices,
                                                      const tinygltf::Model& model,
                                                      const RegistrationData& data) {
        vector<RenderGlMesh> result;
        /* glTF is y-up, Drake's models are z-up. So, we'll start with a rotation
         around the x-axis. */
        const Matrix4d X_MGltf = RigidTransformd(RotationMatrixd::MakeFromOrthonormalColumns(
    ...
    /* Creates the description of the named attribute (and the glTF buffer in
     ...
     @throws std::exception if the requested attribute is missing (unless it is
                            "TEXCOORD_0"). */
    std::pair<VertexAttrib, int> GetAttribute(const std::string& attr_name,
        ...
        if (iter != prim.attributes.end()) {
            ...
            attribute.components_per_element = tinygltf::GetNumComponentsInType(accessor.type);
            ...
            policy_.Error(
                    fmt::format("RenderEngineGl has limited support for glTF files. Primitives must "
    ...
    void WalkNodeTree(int node_index,
                      const tinygltf::Model& model,
                      const Matrix4d& T_FP,
                      const RegistrationData& data,
                      vector<RenderGlMesh>* result) {
    ...
    void InstantiateMesh(int mesh_index,
                         const tinygltf::Model& model,
                         const Matrix4f& T_MN,
                         const RegistrationData& data,
                         vector<RenderGlMesh>* result) {
        ...
        for (int prim_i = 0; prim_i < ssize(mesh.primitives); ++prim_i) {
            ...
            if (p_buffer != n_buffer || (uv_buffer != -1 && uv_buffer != p_buffer)) {
                policy_.Error(
                        fmt::format("RenderEngineGl has limited support for glTF files. All attributes "
            ...
            const Vector3f inv_scale_squared(1.0 / geometry.N_MN.col(0).squaredNorm(),
            ...
            geometry.spec = VertexSpec{.positions = p_attr, .normals = n_attr, .uvs = uv_attr};
            CreateVertexArray(&geometry);

            const int g_index = ssize(geometries_);
            geometries_.push_back(geometry);

            result->push_back(MakeRenderGlMesh(g_index, mesh_index, prim, model, data));
        }
    }

    /* Creates the RenderGlMesh associated with the OpenGlGeometry. */
    RenderGlMesh MakeRenderGlMesh(int geometry_index,
                                  int mesh_index,
                                  const tinygltf::Primitive& prim,
                                  const tinygltf::Model& model,
                                  const RegistrationData& data) {
        ...
        gl_mesh.uv_state = geometry.spec.uvs.components_per_element == 0 ? UvState::kNone : UvState::kFull;

        gl_mesh.mesh_material = MakePrimitiveMaterial(prim, model, gl_mesh, mesh_index, data);
        return gl_mesh;
    }
    ...
    RenderMaterial MakePrimitiveMaterial(const tinygltf::Primitive& prim,
        ...
        if (prim.material < 0 || prim.material >= ssize(model.materials)) {
            if (prim.material >= ssize(model.materials)) {
                policy_.Warning(
                        fmt::format("A primitive from mesh {} has invalid material index {}; there are "
                                    "only {} materials in the glTF file '{}'.",
                                    gltf_mesh_index, prim.material, ssize(model.materials), file_name_));
            }
            return *MaybeMakeMeshFallbackMaterial(data.properties, file_name_, data.default_diffuse, policy_,
        ...
        const tinygltf::PbrMetallicRoughness& gltf_pbr = gltf_mat.pbrMetallicRoughness;

        /* Not required in the glTF, but defaults to [1, 1, 1, 1], as per spec. */
        const vector<double>& gltf_rgba = gltf_pbr.baseColorFactor;
        material.diffuse = Rgba(gltf_rgba[0], gltf_rgba[1], gltf_rgba[2], gltf_rgba[3]);

        if (gltf_pbr.baseColorTexture.index < 0 || render_mesh.uv_state == UvState::kNone) {
        ...
        if (gltf_pbr.baseColorTexture.texCoord > 0) {
            ...
            policy_.Warning(
                    fmt::format("Drake's support for glTF files only includes the \"TEXCOORD_0\" set "
        ...
        const tinygltf::Texture& texture = model.textures.at(gltf_pbr.baseColorTexture.index);
        ...
        if (image.uri.starts_with(TextureLibrary::InMemoryPrefix())) {
            ...
            material.diffuse_map = (path.parent_path() / image.uri).lexically_normal();
    ...
    void ConfigureIndexBuffer(const tinygltf::Primitive& prim,
        ...
        if (prim.indices == -1) {
            policy_.Error(
                    fmt::format("RenderEngineGl has limited support for glTF files. All meshes "
        ...
        if (!index_buffers_.contains(prim.indices)) {
            ...
            if (accessor.type != TINYGLTF_TYPE_SCALAR) {
                ...
                policy_.Error(
                        fmt::format("glTF requires the type of an accessor for mesh indices to be "
            ...
            if (buffer_view.byteStride != 0) {
                policy_.Error(
                        fmt::format("RenderEngineGl has limited support for glTF files. Primitive "
            ...
            GLuint buffer_id;
            glCreateBuffers(1, &buffer_id);
            glNamedBufferStorage(buffer_id, buffer_view.byteLength, buffer.data.data() + offset, 0);
    ...
    static Matrix4d EigenMatrixFromNode(const tinygltf::Node& node) {
        ...
        if (node.matrix.size() == 16) {
            ...
            if (node.translation.size() > 0) {
                ...
                const Vector3d p(node.translation.at(0), node.translation.at(1), node.translation.at(2));
                T.block<3, 1>(0, 3) = p.transpose();
            }
            if (node.rotation.size() > 0) {
                DRAKE_DEMAND(node.rotation.size() == 4);
                const Quaternion<double> quat(node.rotation[3], node.rotation[0], node.rotation[1], node.rotation[2]);
...
void RenderEngineGl::CacheFileMeshesMaybe(const std::string& filename, RegistrationData* data) {
    ...
    if (!meshes_.contains(file_key)) {
        ...
        if (extension == ".obj") {
            ...
            vector<RenderMesh> meshes = LoadRenderMeshesFromObj(
                    filename, PerceptionProperties(), parameters_.default_diffuse, drake::internal::DiagnosticPolicy());
            ...
            for (const auto& render_mesh : meshes) {
                ...
                file_meshes.push_back({.mesh_index = mesh_index, .uv_state = render_mesh.uv_state});
            ...
            file_meshes = GltfMeshExtractor(&geometries_, texture_library_.get()).ExtractMeshes(filename, *data);
        }
        meshes_[file_key] = std::move(file_meshes);
    }
}

std::tuple<GLint, GLenum, GLenum> RenderEngineGl::get_texture_format(RenderType render_type) {
    switch (render_type) {
...
RenderTarget RenderEngineGl::CreateRenderTarget(const RenderCameraCore& camera, RenderType render_type) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, internal_format, width, height, 0, format, pixel_type, 0);
    ...
    glNamedFramebufferTexture(target.frame_buffer, GL_COLOR_ATTACHMENT0, target.value_texture, 0);

    // Create the render buffer object (RBO), acting as the z buffer.
    glCreateRenderbuffers(1, &target.z_buffer);
    glNamedRenderbufferStorage(target.z_buffer, GL_DEPTH_COMPONENT, width, height);
    // Attach the RBO to FBO's depth attachment point.
    glNamedFramebufferRenderbuffer(target.frame_buffer, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, target.z_buffer);

    // Check FBO status.
    GLenum status = glCheckNamedFramebufferStatus(target.frame_buffer, GL_FRAMEBUFFER);
...
void RenderEngineGl::GetLabelImage(ImageLabel16I* label_image_out, const RenderTarget& target) const {
    ImageRgba8U image(label_image_out->width(), label_image_out->height());
    glGetTextureImage(target.value_texture, 0, GL_RGBA, GL_UNSIGNED_BYTE, image.size() * sizeof(GLubyte),
                      image.at(0, 0));
    for (int y = 0; y < image.height(); ++y) {
        for (int x = 0; x < image.width(); ++x) {
            *label_image_out->at(x, y) =
                    RenderEngine::MakeLabelFromRgb(image.at(x, y)[0], image.at(x, y)[1], image.at(x, y)[2]);
        }
    }
}

RenderTarget RenderEngineGl::GetRenderTarget(const RenderCameraCore& camera, RenderType render_type) const {
    const auto& intrinsics = camera.intrinsics();
    const BufferDim dim{intrinsics.width(), intrinsics.height()};
    RenderTarget target;
    std::unordered_map<BufferDim, RenderTarget>& frame_buffers = frame_buffers_[render_type];
...
int RenderEngineGl::CreateGlGeometry(const RenderMesh& render_mesh, bool is_deformable) {
    // Confirm that the context is allocated.
    DRAKE_ASSERT(opengl_context_->IsCurrent());

    const int v_count = render_mesh.positions.rows();
    OpenGlGeometry geometry{.v_count = v_count, .type = GL_UNSIGNED_INT, .mode = GL_TRIANGLES};
    ...
    vertex_data.reserve(v_count * (kFloatsPerPosition + kFloatsPerNormal + kFloatsPerUv));
    // N.B. we are implicitly converting from double to float by inserting them
    // into the vector.
    vertex_data.insert(vertex_data.end(), render_mesh.positions.data(),
    ...
    vertex_data.insert(vertex_data.end(), render_mesh.uvs.data(), render_mesh.uvs.data() + v_count * kFloatsPerUv);
    // For deformable meshes, we set the dynamic storage bit to allow modification
    // to the vertex position data.
    glNamedBufferStorage(geometry.vertex_buffer, vertex_data.size() * sizeof(GLfloat), vertex_data.data(),
                         is_deformable ? GL_DYNAMIC_STORAGE_BIT : 0);

    // Create the index buffer object (IBO).
    using indices_uint_t = decltype(render_mesh.indices)::Scalar;
    static_assert(sizeof(GLuint) == sizeof(indices_uint_t), "If this fails, cast from unsigned int to GLuint");
    glCreateBuffers(1, &geometry.index_buffer);
    // The connectivity is always NOT modifiable.
    glNamedBufferStorage(geometry.index_buffer, render_mesh.indices.size() * sizeof(GLuint), render_mesh.indices.data(),
                         0);
    ...
    const int uv_offset = v_count * (kFloatsPerPosition + kFloatsPerNormal) * sizeof(GLfloat);
    // clang-format off
...
void RenderEngineGl::SetWindowVisibility(const RenderCameraCore& camera,
    ...
    if (show_window) {
        ...
        glBlitNamedFramebuffer(target.frame_buffer, 0,
                               ...
                               // Dest bounds.
                               0, 0, intrinsics.width(), intrinsics.height(), GL_COLOR_BUFFER_BIT, GL_NEAREST);
...
ShaderId RenderEngineGl::AddShader(std::unique_ptr<ShaderProgram> program, RenderType render_type) {
...
ShaderProgramData RenderEngineGl::GetShaderProgram(const PerceptionProperties& properties,
    ...
    for (const auto& id_shader_pair : shader_programs_[render_type]) {
        ...
        std::optional<ShaderProgramData> candidate_data = program.CreateProgramData(properties);
...
void RenderEngineGl::ConfigureLights() {
    ...
    for (const auto& [_, shader_ptr] : shader_programs_[RenderType::kColor]) {
        const auto* lighting_program = dynamic_pointer_cast_or_throw<const LightingShader>(shader_ptr.get());
