...
#include "geometry/render/light_parameter.h"
...
public:
     ...
     */
    void LoadFromSources(const std::string& vertex_shader_source, const std::string& fragment_shader_source);
    ...
    /* Loads a %ShaderProgram from GLSL code contained in the named files.
     ...
     @throws std::exception if there is an error in reading the files, either
                            ...
                            program has errors.
     */
    void LoadFromFiles(const std::string& vertex_shader_file, const std::string& fragment_shader_file);
    ...
    std::optional<ShaderProgramData> CreateProgramData(const PerceptionProperties& properties) const {
    ...
    virtual void SetDepthCameraParameters(const render::DepthRenderCamera& /* camera */) const {}
...
protected:
    ...
    virtual std::optional<ShaderProgramData> DoCreateProgramData(const PerceptionProperties& /* properties */) const {
        return std::nullopt;
    }
