...
std::unique_ptr<RenderEngine> RenderEngine::Clone() const {
    ...
    if (typeid(*this) != typeid(clone_ref)) {
        throw std::logic_error(
                fmt::format("Error in cloning RenderEngine class of type {}; the clone returns "
                            "type {}. {}::DoClone() was probably not implemented",
                            NiceTypeName::Get(*this), NiceTypeName::Get(clone_ref), NiceTypeName::Get(*this)));
    }
    return clone;
}
...
bool RenderEngine::RegisterDeformableVisual(GeometryId id,
...
bool RenderEngine::RemoveGeometry(GeometryId id) {
    ...
    if (removed) {
        DRAKE_DEMAND(update_ids_.erase(id) > 0 || anchored_ids_.erase(id) > 0 || deformable_mesh_dofs_.erase(id) > 0);
    }
...
bool RenderEngine::has_geometry(GeometryId id) const {
    return update_ids_.contains(id) || anchored_ids_.contains(id) || deformable_mesh_dofs_.contains(id);
}

void RenderEngine::UpdateDeformableConfigurations(GeometryId id,
                                                  const std::vector<VectorX<double>>& q_WGs,
                                                  const std::vector<VectorX<double>>& nhats_W) {
    if (!deformable_mesh_dofs_.contains(id)) {
        throw std::runtime_error(fmt::format("No deformable geometry with id {} has been registered.", id));
    }
    const std::vector<int>& mesh_dofs = deformable_mesh_dofs_.at(id);
    if (mesh_dofs.size() != q_WGs.size() || mesh_dofs.size() != nhats_W.size()) {
        throw std::runtime_error(
                fmt::format("Vertex data for the wrong number of meshes. {} meshes are registered "
    ...
    for (int i = 0; i < ssize(mesh_dofs); ++i) {
        if (mesh_dofs[i] != q_WGs[i].size() || mesh_dofs[i] != nhats_W[i].size()) {
            throw std::runtime_error(
                    fmt::format("Wrong dofs in vertex positions and/or normals. There are {} dof "
...
RenderLabel RenderEngine::GetRenderLabelOrThrow(const PerceptionProperties& properties) const {
    RenderLabel label = properties.GetPropertyOrDefault("label", "id", default_render_label_);
...
void RenderEngine::DoUpdateDeformableConfigurations(GeometryId,
                                                    const std::vector<VectorX<double>>&,
                                                    const std::vector<VectorX<double>>&) {}

void RenderEngine::DoRenderColorImage(const ColorRenderCamera&, ImageRgba8U*) const {
    throw std::runtime_error(fmt::format("{}: has not implemented DoRenderColorImage().", NiceTypeName::Get(*this)));
}

void RenderEngine::DoRenderDepthImage(const DepthRenderCamera&, ImageDepth32F*) const {
    throw std::runtime_error(fmt::format("{}: has not implemented DoRenderDepthImage().", NiceTypeName::Get(*this)));
}

void RenderEngine::DoRenderLabelImage(const ColorRenderCamera&, ImageLabel16I*) const {
    throw std::runtime_error(fmt::format("{}: has not implemented DoRenderLabelImage().", NiceTypeName::Get(*this)));
}
