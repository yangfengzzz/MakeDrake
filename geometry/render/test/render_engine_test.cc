...
public:
    ...
    int num_geometries() const {
        return static_cast<int>(engine_.update_ids_.size() + engine_.anchored_ids_.size() +
                                engine_.deformable_mesh_dofs_.size());
    }

    bool has_id(GeometryId id) const {
        return engine_.update_ids_.contains(id) || engine_.anchored_ids_.contains(id) ||
...
public:
    ...
    bool DoRegisterVisual(GeometryId, const Shape&, const PerceptionProperties&, const RigidTransformd&) override {
        return false;
    }
...
GTEST_TEST(RenderEngine, DeformableGeometryRegistrationAndUpdate) {
    ...
    EXPECT_THROW(engine.RegisterDeformableVisual(id0, {mesh}, properties), std::exception);
    // Throw if no mesh is specified.
    EXPECT_THROW(engine.RegisterDeformableVisual(id1, {}, properties), std::exception);
    ...
    engine.UpdateDeformableConfigurations(id1, {new_q, another_q}, {new_normal, another_normal});
    {
        const std::vector<VectorXd>& q1 = engine.world_configurations(id1);
        ASSERT_EQ(q1.size(), 2);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateDeformableConfigurations(fake_id, {new_q}, {new_normal}),
                                "No deformable geometry with id.*");
    // Wrong number of vertex positions/normals.
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateDeformableConfigurations(id0, {new_q, new_q}, {new_normal}),
                                "Vertex data for the wrong number of meshes.*1 meshes are "
                                "registered.*vertex positions for 2 meshes and vertex normals for 1 "
                                "meshes are provided.*");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateDeformableConfigurations(id0, {new_q}, {new_normal, new_normal}),
                                "Vertex data for the wrong number of meshes.*1 meshes are "
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateDeformableConfigurations(id0, {incorrectly_sized_vector}, {new_normal}),
                                "Wrong dofs in vertex positions and/or normals.*");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateDeformableConfigurations(id0, {new_q}, {incorrectly_sized_vector}),
                                "Wrong dofs in vertex positions and/or normals.*");
}
...
GTEST_TEST(RenderEngine, RigidGeometryRegistrationAndUpdate) {
    ...
    // below).
    unordered_map<GeometryId, RigidTransformd> X_WG_all{{GeometryId::get_new_id(), RigidTransformd{Vector3d{1, 2, 3}}},
    ...
    for (const auto& label : {RenderLabel::kEmpty, RenderLabel::kUnspecified}) {
        DRAKE_EXPECT_THROWS_MESSAGE(
                engine.RegisterVisual(GeometryId::get_new_id(), sphere, make_properties(label), {}, false),
    ...
    const bool dynamic_accepted = engine.RegisterVisual(id1, sphere, skip_properties, {}, is_dynamic);
    EXPECT_FALSE(dynamic_accepted);
    const bool anchored_accepted = engine.RegisterVisual(id1, sphere, skip_properties, {}, !is_dynamic);
...

    {
        ...
        bool accepted = engine.RegisterVisual(id, sphere, add_properties, X_WG, !is_dynamic);
        EXPECT_TRUE(accepted);
        EXPECT_TRUE(CompareMatrices(engine.world_pose(id).GetAsMatrix34(), X_WG.GetAsMatrix34()));
        engine.UpdatePoses(X_WG_all);
        EXPECT_EQ(engine.updated_ids().size(), 0);
        EXPECT_TRUE(CompareMatrices(engine.world_pose(id).GetAsMatrix34(), X_WG.GetAsMatrix34()));
    }

    {
        ...
        EXPECT_TRUE(CompareMatrices(engine.updated_ids().at(id).translation(), p_WG));
        EXPECT_TRUE(CompareMatrices(engine.world_pose(id).GetAsMatrix34(), X_WG_all[id].GetAsMatrix34()));
    }
}
...
GTEST_TEST(RenderEngine, RemoveGeometry) {
    ...
    for (int i = 0; i < need_update_count + anchored_count; ++i) {
        ...
        const bool accepted = engine.RegisterVisual(id, sphere, add_properties, X_WG, is_dynamic);
        if (!accepted) {
    ...
    const bool accepted = engine.RegisterDeformableVisual(deformable_id, {mesh}, add_properties);
    if (!accepted) {
        throw std::logic_error("The geometry wasn't accepted for registration");
    }
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
        for (size_t j = 0; j < labels.size(); ++j) {
            if (i == j) {
                ...
                const Vector3<uint8_t> pixel = (color.rgba().head(3) * 255.0).template cast<uint8_t>();
                const RenderLabel readback = DummyRenderEngine::MakeLabelFromRgb(pixel[0], pixel[1], pixel[2]);
...
GTEST_TEST(RenderEngine, DefaultRenderLabel) {
    ...
    {
        for (auto label : {RenderLabel::kDoNotRender, RenderLabel::kEmpty, RenderLabel{10}}) {
...
GTEST_TEST(RenderEngine, ValidateIntrinsicsAndImage) {
    ...
    const ColorRenderCamera color_camera{{"n/a", intrinsics, {0.1, 10}, RigidTransformd{}}, false};
    const DepthRenderCamera depth_camera{{"n/a", intrinsics, {0.1, 10}, RigidTransformd{}}, {1.0, 5.0}};

    // Image is nullptr.
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderColorImage(color_camera, nullptr),
                                "Can't render a color image. The given output image is nullptr");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderDepthImage(depth_camera, nullptr),
                                "Can't render a depth image. The given output image is nullptr");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderLabelImage(color_camera, nullptr),
    ...
    auto test_bad_sizes = [&](const char* image_type, auto image_example, auto render) {
        using Image = decltype(image_example);
        std::vector<Image> images{Image{w - 1, h}, Image{w, h - 1}, Image{w - 1, h - 1}};
        for (Image& i : images) {
            DRAKE_EXPECT_THROWS_MESSAGE(render(&i), fmt::format(error_message, image_type, i.width(), i.height(),
...
public:
    ...
    std::unique_ptr<RenderEngine> DoClone() const override { return std::make_unique<CloneableEngine>(*this); }
};
...
GTEST_TEST(RenderEngine, DetectDoCloneFailure) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(not_cloneable.Clone(),
...
GTEST_TEST(RenderEngine, UnimplementedRenderErrors) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderColorImage(color_camera, &color),
                                ".*MinimumEngine.* has not implemented DoRenderColorImage.+");
    ImageDepth32F depth{w, h};
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderDepthImage(depth_camera, &depth),
                                ".*MinimumEngine.* has not implemented DoRenderDepthImage.+");
    ImageLabel16I label{w, h};
    DRAKE_EXPECT_THROWS_MESSAGE(engine.RenderLabelImage(color_camera, &label),
