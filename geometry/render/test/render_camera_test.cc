...
GTEST_TEST(ClippingRangeTest, Constructor) {
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(-0.1, 10), fmt::format(error_message, -0.1, 10.));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(0.1, -10), fmt::format(error_message, 0.1, -10.0));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(1.5, 1.0), fmt::format(error_message, 1.5, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(kInf, 1.0), fmt::format(error_message, kInf, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(kNaN, 1.0), fmt::format(error_message, kNaN, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(1.0, kInf), fmt::format(error_message, 1.0, kInf));
        DRAKE_EXPECT_THROWS_MESSAGE(ClippingRange(1.0, kNaN), fmt::format(error_message, 1.0, kNaN));
    }
}
...
GTEST_TEST(RenderCameraCoreTest, Constructor) {
    ...
    EXPECT_TRUE(CompareMatrices(core.intrinsics().intrinsic_matrix(), intrinsics.intrinsic_matrix()));
    EXPECT_EQ(core.clipping().near(), clipping.near());
    EXPECT_EQ(core.clipping().far(), clipping.far());
    EXPECT_TRUE(CompareMatrices(core.sensor_pose_in_camera_body().GetAsMatrix4(), X_BS.GetAsMatrix4()));
}
...
GTEST_TEST(ColorRenderCameraTest, Constructor) {
    ...
    EXPECT_TRUE(CompareMatrices(camera.core().intrinsics().intrinsic_matrix(), intrinsics.intrinsic_matrix()));
    EXPECT_EQ(camera.core().clipping().near(), clipping.near());
    EXPECT_EQ(camera.core().clipping().far(), clipping.far());
    EXPECT_TRUE(CompareMatrices(camera.core().sensor_pose_in_camera_body().GetAsMatrix4(), X_BS.GetAsMatrix4()));
...
GTEST_TEST(DepthRangeTest, Constructor) {
...

    {
        ...
        constexpr const char* error_message =
                ...
                "with min = {} and max = {}";
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(-0.1, 10), fmt::format(error_message, -0.1, 10.));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(0.1, -10), fmt::format(error_message, 0.1, -10.0));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(1.5, 1.0), fmt::format(error_message, 1.5, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(kInf, 1.0), fmt::format(error_message, kInf, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(kNaN, 1.0), fmt::format(error_message, kNaN, 1.0));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(1.0, kInf), fmt::format(error_message, 1.0, kInf));
        DRAKE_EXPECT_THROWS_MESSAGE(DepthRange(1.0, kNaN), fmt::format(error_message, 1.0, kNaN));
    }
}
...
GTEST_TEST(DepthRenderCameraTest, Constructor) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DepthRenderCamera(core, DepthRange(clipping.near() - 0.1, clipping.far())),
                                "Depth camera's depth range extends beyond the clipping planes; near = "
                                ".+, far = .+, min. depth = .+, max. depth = .+");
    DRAKE_EXPECT_THROWS_MESSAGE(DepthRenderCamera(core, DepthRange(clipping.near(), clipping.far() + 0.1)),
                                "Depth camera's depth range extends beyond the clipping planes; near = "
                                ".+, far = .+, min. depth = .+, max. depth = .+");
    DRAKE_EXPECT_THROWS_MESSAGE(DepthRenderCamera(core, DepthRange(clipping.near() - 0.1, clipping.far() + 0.1)),
                                "Depth camera's depth range extends beyond the clipping planes; near = "
                                ".+, far = .+, min. depth = .+, max. depth = .+");
}
