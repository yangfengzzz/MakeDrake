...
TEST_F(RenderLabelTests, Construction) {
    ...
    std::vector<ValueType> bad_values{-1, RenderLabel::kUnspecified, RenderLabel::kDoNotRender, RenderLabel::kDontCare,
                                      RenderLabel::kEmpty};
    for (ValueType value : bad_values) {
        DRAKE_EXPECT_THROWS_MESSAGE(RenderLabel{value}, "Invalid construction of RenderLabel with invalid value: .+");
    }
