...
RenderMaterial MakeMaterialFromMtl(const tinyobj::material_t& mat,
    ...
    result.diffuse.set(mat.diffuse[0], mat.diffuse[1], mat.diffuse[2], mat.dissolve);
    ...
    if (!result.diffuse_map.empty()) {
        ...
        if (!tex_file.is_open()) {
            policy.Warning(
                    fmt::format("The OBJ file's material requested an unavailable diffuse texture "
                                "image: {}. The image will be omitted.",
                                result.diffuse_map));
            result.diffuse_map.clear();
        } else if (uv_state != UvState::kFull) {
            policy.Warning(
                    fmt::format("The OBJ file's material requested a diffuse texture image: {}. "
                                "However the mesh doesn't define {} texture coordinates. The image "
                                "will be omitted.",
                                result.diffuse_map, uv_state == UvState::kNone ? "any" : "a complete set of"));
            result.diffuse_map.clear();
        }
    }
...
vector<RenderMesh> LoadRenderMeshesFromObj(const std::filesystem::path& obj_path,
                                           const GeometryProperties& properties,
    ...
    if (!valid_parse) {
        throw std::runtime_error(fmt::format("Failed parsing the obj file: {}: {}", obj_path.string(), reader.Error()));
    }
    ...
    if (!faces_found) {
        throw std::runtime_error(
                fmt::format("The OBJ data appears to have no faces; it could be missing faces or "
    ...
    if (attrib.normals.size() == 0) {
        throw std::runtime_error(fmt::format("OBJ has no normals: {}", obj_path.string()));
    }
    ...
    for (const auto& shape : shapes) {
        ...
        for (int f = 0; f < num_faces; ++f) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                if (norm_index < 0) {
                    throw std::runtime_error(fmt::format("Not all faces reference normals: {}", obj_path.string()));
                }
                const auto obj_indices = make_tuple(position_index, norm_index, uv_index);
                if (uv_index >= 0) {
                ...
                if (!obj_vertex_to_new_vertex.contains(obj_indices)) {
                    obj_vertex_to_new_vertex[obj_indices] = static_cast<int>(positions.size());
                    /* Guarantee that the positions.size() == normals.size() == uvs.size()
                     by always growing them in lock step.  */
                    positions.emplace_back(Vector3d{attrib.vertices[3 * position_index],
                                                    attrib.vertices[3 * position_index + 1],
                                                    attrib.vertices[3 * position_index + 2]});
                    normals.emplace_back(attrib.normals[3 * norm_index], attrib.normals[3 * norm_index + 1],
                                         attrib.normals[3 * norm_index + 2]);
                    if (uv_index >= 0) {
                        uvs.emplace_back(attrib.texcoords[2 * uv_index], attrib.texcoords[2 * uv_index + 1]);
                    } else {
    ...
    for (const auto& [mat_index, tri_indices] : material_triangles) {
        ...
        mesh_data.uv_state = material_uvs[mat_index] == 0                        ? UvState::kNone
                             : material_uvs[mat_index] == ssize(tri_indices) * 3 ? UvState::kFull
                                                                                 : UvState::kPartial;
        if (mat_index == -1) {
            /* This is the default material. No material was assigned to the faces.
             We'll apply the fallback logic. */
            mesh_data.material =
                    MaybeMakeMeshFallbackMaterial(properties, obj_path, default_diffuse, policy, mesh_data.uv_state);
        } else {
            mesh_data.material = MakeMaterialFromMtl(reader.GetMaterials().at(mat_index), obj_path, properties, policy,
                                                     mesh_data.uv_state);
        }
        ...
        for (const auto& t : tri_indices) {
            ...
            for (int i = 0; i < 3; ++i) {
                if (!vertex_index_full_to_part.contains(tri[i])) {
                    vertex_index_full_to_part[tri(i)] = static_cast<indices_uint_t>(vertex_index_full_to_part.size());
                }
            }
        }
...
RenderMesh MakeRenderMeshFromTriangleSurfaceMesh(const TriangleSurfaceMesh<double>& mesh,
                                                 ...
                                                 const DiagnosticPolicy& policy) {
    RenderMesh result;
    result.material = MaybeMakeMeshFallbackMaterial(properties, "", {}, policy, UvState::kNone);
...
RenderMesh MakeFacetedRenderMeshFromTriangleSurfaceMesh(const TriangleSurfaceMesh<double>& mesh,
    ...
    const TriangleSurfaceMesh<double> faceted(std::move(triangles), std::move(vertices));
...
TriangleSurfaceMesh<double> MakeTriangleSurfaceMesh(const RenderMesh& render_mesh) {
    ...
    for (int t = 0; t < num_triangles; ++t) {
        triangles.emplace_back(render_mesh.indices(t, 0), render_mesh.indices(t, 1), render_mesh.indices(t, 2));
    }
    return TriangleSurfaceMesh<double>(TriangleSurfaceMesh(std::move(triangles), std::move(vertices)));
}
