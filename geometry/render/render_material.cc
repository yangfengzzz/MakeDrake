...
void MaybeWarnForRedundantMaterial(const GeometryProperties& props,
    ...
    if (props.HasProperty("phong", "diffuse")) {
        ignored_props.push_back(fmt::format("('phong', 'diffuse') = {}",
                                            fmt_eigen(props.GetProperty<Rgba>("phong", "diffuse").rgba().transpose())));
    }
    if (props.HasProperty("phong", "diffuse_map")) {
        ignored_props.push_back(
                fmt::format("('phong', 'diffuse_map') = {}", props.GetProperty<std::string>("phong", "diffuse_map")));
    }
...
std::optional<RenderMaterial> MaybeMakeMeshFallbackMaterial(const GeometryProperties& props,
    ...
    if (props.HasProperty("phong", "diffuse") || props.HasProperty("phong", "diffuse_map")) {
    ...
    if (uv_state == UvState::kFull) {
        ...
        policy.Warning(
                fmt::format("A png file of the same name as the mesh has been found ('{}'), "
                            "but the mesh doesn't define {} texture coordinates. The map will "
                            "be omitted leaving a flat white color.",
                            alt_texture_path.string(), uv_state == UvState::kNone ? "any" : "a complete set of"));
    }
...
RenderMaterial DefineMaterial(const GeometryProperties& props,
    ...
    material.diffuse_map = props.GetPropertyOrDefault<std::string>("phong", "diffuse_map", "");
    ...
    material.diffuse =
            props.GetPropertyOrDefault<Rgba>("phong", "diffuse", has_diffuse_map ? Rgba(1, 1, 1) : default_diffuse);
    ...
    if (has_diffuse_map) {
        ...
        if (!std::ifstream(material.diffuse_map).is_open()) {
            ...
            policy.Warning(
                    fmt::format("The ('phong', 'diffuse_map') property referenced a map that "
                                "could not be found: '{}'",
                                material.diffuse_map));
            material.diffuse_map = "";
        } else if (uv_state != UvState::kFull) {
            policy.Warning(
                    fmt::format("The ('phong', 'diffuse_map') property referenced a map, but the "
                                "geometry doesn't define {} texture coordinates. The map will be "
                                "omitted: '{}'.",
                                uv_state == UvState::kNone ? "any" : "a complete set of", material.diffuse_map));
            material.diffuse_map = "";
        }
    }
    return material;
