...
ClippingRange::ClippingRange(double near, double far) : near_(near), far_(far) {
    if (near <= 0 || far <= 0 || far <= near || !std::isfinite(near + far)) {
        throw std::runtime_error(
                fmt::format("The clipping range values must both be positive and finite and far "
...
DepthRange::DepthRange(double min_in, double max_in) : min_depth_(min_in), max_depth_(max_in) {
    if (min_in <= 0 || max_in <= 0 || max_in <= min_in || !std::isfinite(min_in + max_in)) {
        throw std::runtime_error(
...
DepthRenderCamera::DepthRenderCamera(RenderCameraCore core, DepthRange depth_range)
    : core_(std::move(core)), depth_range_(std::move(depth_range)) {
    if (depth_range_.min_depth() < core_.clipping().near() || depth_range_.max_depth() > core_.clipping().far()) {
        throw std::runtime_error(fmt::format(
                "Depth camera's depth range extends beyond the clipping planes; near = "
                "{}, far = {}, min. depth = {}, max. depth = {}",
                core_.clipping().near(), core_.clipping().far(), depth_range_.min_depth(), depth_range_.max_depth()));
    }
}
