...
public:
    ...
    explicit GeometryStateValue(const GeometryState<T>& state) : Value<GeometryState<T>>(state) {}
...
private:
    ...
    bool do_double_assign(const AbstractValue& other) {
        const GeometryStateValue<double>* double_value = dynamic_cast<const GeometryStateValue<double>*>(&other);
        if (double_value) {
...
public:
    ...
    std::unique_ptr<GeometryState<T>> MakeAugmentedModel() const {
        ...
        if (augmented_model_cache_ != nullptr) {
            ...
            augmented_model_cache_ = std::make_unique<const GeometryState<T>>(*result);
            return result;
        }
    }
...
SceneGraph<T>::SceneGraph()
    : LeafSystem<T>(SystemTypeTag<SceneGraph>{}), owned_hub_(std::make_unique<Hub>()), hub_(*owned_hub_) {
    model_inspector_.set(&hub_.model());
    geometry_state_index_ = this->DeclareAbstractParameter(GeometryStateValue<T>());
    scene_graph_config_index_ = this->DeclareAbstractParameter(Value<SceneGraphConfig>());

    query_port_index_ =
            this->DeclareAbstractOutputPort(
                        "query", &SceneGraph::CalcQueryObject,
                        {this->all_input_ports_ticket(),
                         this->abstract_parameter_ticket(systems::AbstractParameterIndex(geometry_state_index_))})
                    .get_index();

    auto& pose_update_cache_entry = this->DeclareCacheEntry("Cache guard for pose updates", &SceneGraph::CalcPoseUpdate,
                                                            {this->all_input_ports_ticket()});
    pose_update_index_ = pose_update_cache_entry.cache_index();

    auto& configuration_update_cache_entry =
            this->DeclareCacheEntry("Cache guard for configuration updates", &SceneGraph::CalcConfigurationUpdate,
                                    {this->all_input_ports_ticket()});
...
template <typename T>
const InputPort<T>& SceneGraph<T>::get_source_configuration_port(SourceId id) const {
    ThrowUnlessRegistered(id, "Can't acquire configuration port for unknown source id: ");
    return this->get_input_port(input_source_ids_.at(id).configuration_port);
}

template <typename T>
FrameId SceneGraph<T>::RegisterFrame(SourceId source_id, const GeometryFrame& frame) {
    return hub_.mutable_model().RegisterFrame(source_id, frame);
}

template <typename T>
FrameId SceneGraph<T>::RegisterFrame(SourceId source_id, FrameId parent_id, const GeometryFrame& frame) {
...
template <typename T>
GeometryId SceneGraph<T>::RegisterGeometry(SourceId source_id,
                                           FrameId frame_id,
                                           std::unique_ptr<GeometryInstance> geometry) {
    return hub_.mutable_model().RegisterGeometry(source_id, frame_id, std::move(geometry));
}

template <typename T>
GeometryId SceneGraph<T>::RegisterGeometry(Context<T>* context,
                                           SourceId source_id,
                                           FrameId frame_id,
    ...
    auto geometry_id = g_state.RegisterGeometry(source_id, frame_id, std::move(geometry));
    if (has_proximity) {
        const auto& config = get_config(*context);
        g_state.ApplyProximityDefaults(config.default_proximity_properties, geometry_id);
    }
    return geometry_id;
}

template <typename T>
GeometryId SceneGraph<T>::RegisterAnchoredGeometry(SourceId source_id, std::unique_ptr<GeometryInstance> geometry) {
    return hub_.mutable_model().RegisterAnchoredGeometry(source_id, std::move(geometry));
}

template <typename T>
GeometryId SceneGraph<T>::RegisterDeformableGeometry(SourceId source_id,
                                                     FrameId frame_id,
                                                     std::unique_ptr<GeometryInstance> geometry,
                                                     double resolution_hint) {
    return hub_.mutable_model().RegisterDeformableGeometry(source_id, frame_id, std::move(geometry), resolution_hint);
}

template <typename T>
GeometryId SceneGraph<T>::RegisterDeformableGeometry(Context<T>* context,
                                                     SourceId source_id,
                                                     FrameId frame_id,
                                                     std::unique_ptr<GeometryInstance> geometry,
                                                     double resolution_hint) const {
    auto& g_state = mutable_geometry_state(context);
    return g_state.RegisterDeformableGeometry(source_id, frame_id, std::move(geometry), resolution_hint);
}

template <typename T>
void SceneGraph<T>::RenameGeometry(GeometryId geometry_id, const std::string& name) {
    return hub_.mutable_model().RenameGeometry(geometry_id, name);
}

template <typename T>
void SceneGraph<T>::ChangeShape(SourceId source_id,
...
template <typename T>
void SceneGraph<T>::ChangeShape(Context<T>* context,
                                SourceId source_id,
                                GeometryId geometry_id,
                                const Shape& shape,
                                std::optional<math::RigidTransform<double>> X_FG) {
...
template <typename T>
void SceneGraph<T>::RemoveGeometry(Context<T>* context, SourceId source_id, GeometryId geometry_id) const {
    auto& g_state = mutable_geometry_state(context);
    g_state.RemoveGeometry(source_id, geometry_id);
}

template <typename T>
void SceneGraph<T>::AddRenderer(std::string name, std::unique_ptr<render::RenderEngine> renderer) {
    return hub_.mutable_model().AddRenderer(std::move(name), std::move(renderer));
}

template <typename T>
void SceneGraph<T>::AddRenderer(Context<T>* context,
                                std::string name,
...
template <typename T>
void SceneGraph<T>::RemoveRenderer(Context<T>* context, const std::string& name) const {
    auto& g_state = mutable_geometry_state(context);
...
template <typename T>
bool SceneGraph<T>::HasRenderer(const Context<T>& context, const std::string& name) const {
...
template <typename T>
std::string SceneGraph<T>::GetRendererTypeName(const Context<T>& context, const std::string& name) const {
    const auto& g_state = geometry_state(context);
...
template <typename T>
vector<std::string> SceneGraph<T>::RegisteredRendererNames(const Context<T>& context) const {
    const auto& g_state = geometry_state(context);
...
void SceneGraph<T>::AssignRole(SourceId source_id,
                               ...
                               RoleAssign assign) {
    hub_.mutable_model().AssignRole(source_id, geometry_id, std::move(properties), assign);
}

template <typename T>
void SceneGraph<T>::AssignRole(Context<T>* context,
                               SourceId source_id,
                               GeometryId geometry_id,
                               ProximityProperties properties,
...
template <typename T>
int SceneGraph<T>::RemoveRole(Context<T>* context, SourceId source_id, FrameId frame_id, Role role) const {
    auto& g_state = mutable_geometry_state(context);
    return g_state.RemoveRole(source_id, frame_id, role);
}

template <typename T>
int SceneGraph<T>::RemoveRole(SourceId source_id, GeometryId geometry_id, Role role) {
    return hub_.mutable_model().RemoveRole(source_id, geometry_id, role);
}

template <typename T>
int SceneGraph<T>::RemoveRole(Context<T>* context, SourceId source_id, GeometryId geometry_id, Role role) const {
    auto& g_state = mutable_geometry_state(context);
...
template <typename T>
CollisionFilterManager SceneGraph<T>::collision_filter_manager(Context<T>* context) const {
    return mutable_geometry_state(context).collision_filter_manager();
}

template <typename T>
void SceneGraph<T>::SetDefaultParameters(const Context<T>& context, Parameters<T>* parameters) const {
    LeafSystem<T>::SetDefaultParameters(context, parameters);
    std::unique_ptr<GeometryState<T>> state = hub_.MakeAugmentedModel();
    parameters->template get_mutable_abstract_parameter<GeometryState<T>>(geometry_state_index_) = std::move(*state);
    parameters->template get_mutable_abstract_parameter<SceneGraphConfig>(scene_graph_config_index_) = hub_.config();
}
...
void SceneGraph<T>::MakeSourcePorts(SourceId source_id) {
    ...
    source_ports.pose_port =
            this->DeclareAbstractInputPort(hub_.model().GetName(source_id) + "_pose", Value<FramePoseVector<T>>())
                    .get_index();
    source_ports.configuration_port = this->DeclareAbstractInputPort(hub_.model().GetName(source_id) + "_configuration",
                                                                     Value<GeometryConfigurationVector<T>>())
                                              .get_index();
}

template <typename T>
void SceneGraph<T>::CalcQueryObject(const Context<T>& context, QueryObject<T>* output) const {
...
template <typename T>
std::vector<FrameId> SceneGraph<T>::GetDynamicFrames(const GeometryState<T>& g_state, Role role) const {
...
void SceneGraph<T>::CalcPoseUpdate(const Context<T>& context, int*) const {
    ...
    internal::KinematicsData<T>& kinematics_data = state.mutable_kinematics_data();
    ...
    for (const auto& pair : state.source_frame_id_map_) {
        if (pair.second.size() > 0) {
            ...
            if (itr != input_source_ids_.end()) {
                ...
                const auto& poses = pose_port.template Eval<FramePoseVector<T>>(context);
                state.SetFramePoses(source_id, poses, &kinematics_data);
            }
        }
    }

    state.FinalizePoseUpdate(kinematics_data, &state.mutable_proximity_engine(), state.GetMutableRenderEngines());
}

template <typename T>
void SceneGraph<T>::CalcConfigurationUpdate(const Context<T>& context, int*) const {
    const GeometryState<T>& state = geometry_state(context);
    // See KinematicsData class documentation for why this caching violation is
    // needed and is correct.
    internal::KinematicsData<T>& kinematics_data = state.mutable_kinematics_data();
    ...
    for (const auto& [source_id, geometry_id_set] : state.source_deformable_geometry_id_map_) {
        if (geometry_id_set.size() > 0) {
            const auto itr = input_source_ids_.find(source_id);
            if (itr != input_source_ids_.end()) {
                const auto& configuration_port = this->get_input_port(itr->second.configuration_port);
                if (!configuration_port.HasValue(context)) {
                    throw std::logic_error(
                            fmt::format("Source '{}' (id: {}) has registered deformable geometry "
                                        "but is not connected to the appropriate input port.",
                                        state.GetName(source_id), source_id));
                }
                const auto& configs = configuration_port.template Eval<GeometryConfigurationVector<T>>(context);
    ...
    for (const auto role : std::vector<Role>{Role::kIllustration, Role::kPerception, Role::kProximity}) {
        state.mutable_driven_mesh_data(role).SetControlMeshPositions(kinematics_data.q_WGs);
    }
    state.FinalizeConfigurationUpdate(kinematics_data, state.mutable_driven_mesh_data(Role::kPerception),
                                      &state.mutable_proximity_engine(), state.GetMutableRenderEngines());
}

template <typename T>
void SceneGraph<T>::ThrowUnlessRegistered(SourceId source_id, const char* message) const {
    using std::to_string;
...
template <typename T>
GeometryState<T>& SceneGraph<T>::mutable_geometry_state(Context<T>* context) const {
    this->ValidateContext(context);
    return context->get_mutable_parameters().template get_mutable_abstract_parameter<GeometryState<T>>(
            geometry_state_index_);
}

template <typename T>
const GeometryState<T>& SceneGraph<T>::geometry_state(const Context<T>& context) const {
    return context.get_parameters().template get_abstract_parameter<GeometryState<T>>(geometry_state_index_);
}

template <typename T>
const SceneGraphConfig& SceneGraph<T>::get_config(const systems::Context<T>& context) const {
    return context.get_parameters().template get_abstract_parameter<SceneGraphConfig>(scene_graph_config_index_);
}

}  // namespace geometry
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::SceneGraph);
