...
public:
    ...
    explicit GeometrySet(GeometryId geometry_id) { geometry_ids_.insert(geometry_id); }
    ...
    template <typename Container>
    explicit GeometrySet(std::initializer_list<GeometryId> geometry_ids, const Container& frame_ids) {
        Add(geometry_ids);
        Add(frame_ids);
    }

    template <typename Container>
    explicit GeometrySet(const Container& geometry_ids, std::initializer_list<FrameId> frame_ids) {
        Add(geometry_ids);
        Add(frame_ids);
    }

    explicit GeometrySet(std::initializer_list<GeometryId> geometry_ids, std::initializer_list<FrameId> frame_ids) {
        Add(geometry_ids);
        Add(frame_ids);
    }
    ...
    template <typename ContainerG,
              typename ContainerF,
              typename = typename std::enable_if_t<std::is_same_v<typename ContainerG::value_type, GeometryId> &&
    ...
    */
    ...
    template <typename Container>
    typename std::enable_if_t<std::is_same_v<typename Container::value_type, GeometryId>> Add(
            const Container& geometry_ids) {
        geometry_ids_.insert(geometry_ids.begin(), geometry_ids.end());
    }

    template <typename Container>
    typename std::enable_if_t<std::is_same_v<typename Container::value_type, FrameId>> Add(const Container& frame_ids) {
        frame_ids_.insert(frame_ids.begin(), frame_ids.end());
    }

    void Add(std::initializer_list<FrameId> frame_ids) { frame_ids_.insert(frame_ids.begin(), frame_ids.end()); }
    ...
    typename std::enable_if_t<std::is_same_v<typename ContainerG::value_type, GeometryId> &&
                              std::is_same_v<typename ContainerF::value_type, FrameId>>
    ...
    typename std::enable_if_t<std::is_same_v<typename ContainerF::value_type, FrameId>> Add(
            std::initializer_list<GeometryId> geometry_ids, const ContainerF& frame_ids) {
        Add(geometry_ids);
        Add(frame_ids);
    }

    template <typename ContainerG>
    typename std::enable_if_t<std::is_same_v<typename ContainerG::value_type, GeometryId>> Add(
            const ContainerG& geometry_ids, std::initializer_list<FrameId> frame_ids) {
        Add(geometry_ids);
        Add(frame_ids);
    }
    ...
    void Add(const GeometrySet& other) {
        ...
        geometry_ids_.insert(other.geometry_ids_.begin(), other.geometry_ids_.end());
    }
...
private:
    ...
    const std::unordered_set<GeometryId>& geometries() const { return geometry_ids_; }
    ...
    bool contains(FrameId frame_id) const { return frame_ids_.contains(frame_id); }
    ...
    bool contains(GeometryId geometry_id) const { return geometry_ids_.contains(geometry_id); }
