...
#include "geometry/proximity/collisions_exist_callback.h"
#include "geometry/proximity/deformable_contact_geometries.h"
#include "geometry/proximity/deformable_contact_internal.h"
#include "geometry/proximity/distance_to_point_callback.h"
#include "geometry/proximity/distance_to_shape_callback.h"
#include "geometry/proximity/find_collision_candidates_callback.h"
#include "geometry/proximity/hydroelastic_callback.h"
#include "geometry/proximity/hydroelastic_internal.h"
#include "geometry/proximity/make_mesh_from_vtk.h"
#include "geometry/proximity/obj_to_surface_mesh.h"
#include "geometry/proximity/penetration_as_point_pair_callback.h"
#include "geometry/proximity/volume_to_surface_mesh.h"
#include "geometry/proximity/vtk_to_volume_mesh.h"
#include "geometry/read_obj.h"
...
class FclDynamicAABBTreeCollisionManager : public fcl::DynamicAABBTreeCollisionManager<double> {};
class MapGeometryIdToFclCollisionObject : public unordered_map<GeometryId, unique_ptr<CollisionObjectd>> {};
...
shared_ptr<fcl::ShapeBased> CopyShapeOrThrow(const fcl::CollisionGeometryd& geometry) {
    ...
    switch (geometry.getNodeType()) {
        ...
        case fcl::GEOM_CONVEX: {
            ...
            return make_shared<fcl::Convexd>(make_shared<const std::vector<Vector3d>>(convex.getVertices()),
                                             convex.getFaceCount(),
        ...
        case fcl::GEOM_TRIANGLE:
            throw std::logic_error("Trying to copy fcl::CollisionGeometry of unsupported GEOM_* type");
        default:
            throw std::logic_error("Trying to copy fcl::CollisionGeometry of non GEOM_* type");
    }
}

// Helper function that creates a *deep* copy of the given collision object.
unique_ptr<CollisionObjectd> CopyFclObjectOrThrow(const CollisionObjectd& object) {
    shared_ptr<fcl::ShapeBased> geometry_copy = CopyShapeOrThrow(*object.collisionGeometry());
...
void CopyFclObjectsOrThrow(const unordered_map<GeometryId, unique_ptr<CollisionObjectd>>& source_objects,
...
void BuildTreeFromReference(const fcl::DynamicAABBTreeCollisionManager<double>& other,
                            const std::unordered_map<const CollisionObjectd*, CollisionObjectd*>& copy_map,
...
void FclCollide(const fcl::DynamicAABBTreeCollisionManager<double>& tree1,
                ...
                DataType* data,
                fcl::CollisionCallBack<T> callback) {
    tree1.collide(const_cast<fcl::DynamicAABBTreeCollisionManager<T>*>(&tree2), data, callback);
}
...
void FclDistance(const fcl::DynamicAABBTreeCollisionManager<double>& tree1,
                 const fcl::DynamicAABBTreeCollisionManager<double>& tree2,
                 DataType* data,
                 fcl::DistanceCallBack<T> callback) {
    tree1.distance(const_cast<fcl::DynamicAABBTreeCollisionManager<T>*>(&tree2), data, callback);
}

// Compare function to use with ordering PenetrationAsPointPairs.
template <typename T>
bool OrderPointPair(const PenetrationAsPointPair<T>& p1, const PenetrationAsPointPair<T>& p2) {
...
template <typename T>
bool OrderContactSurface(const ContactSurface<T>& s1, const ContactSurface<T>& s2) {
...
public:
    ...
    Impl(const Impl& other) : ShapeReifier(other) {
        ...
        geometries_for_deformable_contact_ = other.geometries_for_deformable_contact_;
        dynamic_tree_.clear();
        ...
        CopyFclObjectsOrThrow(other.anchored_objects_, &anchored_objects_, &object_map);
    ...
    std::unique_ptr<typename ProximityEngine<U>::Impl> ToScalarType() const {
        ...
        // Copy all of the geometry.
        std::unordered_map<const CollisionObjectd*, CollisionObjectd*> object_map;
        CopyFclObjectsOrThrow(anchored_objects_, &engine->anchored_objects_, &object_map);
        CopyFclObjectsOrThrow(dynamic_objects_, &engine->dynamic_objects_, &object_map);
        ...
        engine->geometries_for_deformable_contact_ = this->geometries_for_deformable_contact_;
    ...
    void AddDynamicGeometry(const Shape& shape,
                            const RigidTransformd& X_WG,
                            GeometryId id,
                            const ProximityProperties& props) {
        AddGeometry(shape, X_WG, id, props, true, &dynamic_tree_, &dynamic_objects_);
    }
    ...
    void AddAnchoredGeometry(const Shape& shape,
                             ...
                             GeometryId id,
                             const ProximityProperties& props) {
        AddGeometry(shape, X_WG, id, props, false, &anchored_tree_, &anchored_objects_);
    }
    ...
    bool NeedsConvexHull(const InternalGeometry& geometry) const {
        ...
        return !geometry.is_deformable() &&
               (geometry.shape().type_name() == "Mesh" || geometry.shape().type_name() == "Convex");
    }

    void UpdateRepresentationForNewProperties(const InternalGeometry& geometry,
        ...
        hydroelastic_geometries_.MaybeAddGeometry(geometry.shape(), id, new_properties);
        const RigidTransformd X_WG = GetX_WG(id, geometry.is_dynamic());
        geometries_for_deformable_contact_.RemoveGeometry(id);
        geometries_for_deformable_contact_.MaybeAddRigidGeometry(geometry.shape(), id, new_properties, X_WG);
    }
    ...
    bool IsRegisteredAsDeformable(GeometryId id) { return geometries_for_deformable_contact_.is_deformable(id); }
    ...
    bool IsRegisteredAsRigid(GeometryId id) { return dynamic_objects_.contains(id) || anchored_objects_.contains(id); }
    ...
    void RemoveDeformableGeometry(GeometryId id) {
        if (!geometries_for_deformable_contact_.is_deformable(id)) {
            throw std::logic_error(
                    fmt::format("The proximity engine does not contain a deformable geometry with "
    ...
    int num_anchored() const { return static_cast<int>(anchored_objects_.size()); }
    ...
    void UpdateWorldPoses(const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs) {
    ...
    void UpdateDeformableVertexPositions(const std::unordered_map<GeometryId, VectorX<T>>& q_WGs) {
        for (const auto& [id, q_WG] : q_WGs) {
            geometries_for_deformable_contact_.UpdateDeformableVertexPositions(id, ExtractDoubleOrThrow(q_WG));
        }
    }
    ...
    // deformable contact.
    template <typename Shape>
    void ProcessGeometriesForDeformableContact(const Shape& shape, void* user_data) {
        const ReifyData& data = *static_cast<ReifyData*>(user_data);

        geometries_for_deformable_contact_.MaybeAddRigidGeometry(shape, data.id, data.properties, data.X_WG);
    }
    ...
    void ImplementGeometry(const Capsule& capsule, void* user_data) override {
        ...
        auto fcl_capsule = make_shared<fcl::Capsuled>(capsule.radius(), capsule.length());
    ...
    void ImplementGeometry(const Cylinder& cylinder, void* user_data) override {
        // Note: Using `shared_ptr` because of FCL API requirements.
        auto fcl_cylinder = make_shared<fcl::Cylinderd>(cylinder.radius(), cylinder.length());
    ...
    void ImplementGeometry(const Ellipsoid& ellipsoid, void* user_data) override {
        // Note: Using `shared_ptr` because of FCL API requirements.
        auto fcl_ellipsoid = make_shared<fcl::Ellipsoidd>(ellipsoid.a(), ellipsoid.b(), ellipsoid.c());
    ...
    std::vector<SignedDistancePair<T>> ComputeSignedDistancePairwiseClosestPoints(
            const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs, const double max_distance) const {
        std::vector<SignedDistancePair<T>> witness_pairs;
        // All these quantities are aliased in the callback data.
        shape_distance::CallbackData<T> data{&collision_filter_, &X_WGs, max_distance, &witness_pairs};
        ...
        FclDistance(dynamic_tree_, anchored_tree_, &data, shape_distance::Callback<T>);
        return witness_pairs;
    }

    SignedDistancePair<T> ComputeSignedDistancePairClosestPoints(
            GeometryId id_A, GeometryId id_B, const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs) const {
        std::vector<SignedDistancePair<T>> witness_pairs;
        double max_distance = std::numeric_limits<double>::infinity();
        // All these quantities are aliased in the callback data.
        shape_distance::CallbackData<T> data{nullptr, &X_WGs, max_distance, &witness_pairs};
        data.request.enable_nearest_points = true;
        data.request.enable_signed_distance = true;
        data.request.gjk_solver_type = fcl::GJKSolverType::GST_LIBCCD;
        data.request.distance_tolerance = distance_tolerance_;
    ...
    std::vector<SignedDistanceToPoint<T>> ComputeSignedDistanceToPoint(
        ...
        point_distance::CallbackData<T> data{&query_point, threshold, p_WQ, &X_WGs, &distances};
    ...
    std::vector<PenetrationAsPointPair<T>> ComputePointPairPenetration(
        ...
        penetration_as_point_pair::CallbackData data{&collision_filter_, &X_WGs, &contacts};
        ...
        // Perform a query of the dynamic objects against the anchored. We don't do
        // anchored against anchored because those pairs are implicitly filtered.
        FclCollide(dynamic_tree_, anchored_tree_, &data, penetration_as_point_pair::Callback<T>);
    ...
    std::vector<SortedPair<GeometryId>> FindCollisionCandidates() const {
        ...
        // Perform a query of the dynamic objects against the anchored. We don't do
        // anchored against anchored because those pairs are implicitly filtered.
        FclCollide(dynamic_tree_, anchored_tree_, &data, find_collision_candidates::Callback);

        std::sort(pairs.begin(), pairs.end(), [](const SortedPair<GeometryId>& p1, const SortedPair<GeometryId>& p2) {
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool, std::vector<ContactSurface<T>>> ComputeContactSurfaces(
            HydroelasticContactRepresentation representation,
            const unordered_map<GeometryId, RigidTransform<T>>& X_WGs) const {
        vector<ContactSurface<T>> surfaces;
        // All these quantities are aliased in the callback data.
        hydroelastic::CallbackData<T> data{&collision_filter_, &X_WGs, &hydroelastic_geometries_, representation,
                                           &surfaces};
    ...
    template <typename T1 = T>
    typename std::enable_if_t<scalar_predicate<T1>::is_bool, void> ComputeContactSurfacesWithFallback(
            HydroelasticContactRepresentation representation,
            const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs,
        ...
        hydroelastic::CallbackWithFallbackData<T> data{
                hydroelastic::CallbackData<T>{&collision_filter_, &X_WGs, &hydroelastic_geometries_, representation,
                                              surfaces},
                point_pairs};
        ...
        FclCollide(dynamic_tree_, anchored_tree_, &data, hydroelastic::CallbackWithFallback<T>);
    ...
    void ComputeDeformableContact(DeformableContact<double>* deformable_contact) const {
        *deformable_contact = geometries_for_deformable_contact_.ComputeDeformableContact(collision_filter_);
    }
    ...
    bool IsDeepCopy(const Impl& other) const {
        if (this != &other) {
            ...
            auto are_maps_deep_copy =
                    [](const unordered_map<GeometryId, unique_ptr<CollisionObjectd>>& this_map,
                       const unordered_map<GeometryId, unique_ptr<CollisionObjectd>>& other_map) -> bool {
                ...
                for (const auto& id_object_pair : this_map) {
                    ...
                    bool objects_equal = test.getUserData() == ref.getUserData() &&
            ...
            if (!are_maps_deep_copy(this->anchored_objects_, other.anchored_objects_)) {
                return false;
            }
    ...
    const hydroelastic::Geometries& hydroelastic_geometries() const { return hydroelastic_geometries_; }

    const deformable::Geometries& deformable_contact_geometries() const { return geometries_for_deformable_contact_; }
    ...
    void* GetCollisionObject(GeometryId id) const {
        ...
        if (auto iter = anchored_objects_.find(id); iter != anchored_objects_.end()) {
            return iter->second.get();
        }
        return nullptr;
    }
...
private:
    ...
    void AddGeometry(const Shape& shape,
                     const RigidTransformd& X_WG,
                     GeometryId id,
                     const ProximityProperties& props,
                     bool is_dynamic,
    ...
    void RemoveGeometry(GeometryId id,
        ...
        unordered_map<GeometryId, unique_ptr<CollisionObjectd>>& typed_geometries = *geometries;
    ...
    void TakeShapeOwnership(const std::shared_ptr<fcl::ShapeBased>& shape, void* data) {
    ...
    void ImplementFromConvexHull(const MeshType& mesh, void* user_data) {
        ...
        auto fcl_convex =
                make_shared<fcl::Convexd>(std::move(shared_verts), hull.num_elements(), std::move(shared_faces));
...
template <typename T>
ProximityEngine<T>::ProximityEngine(ProximityEngine<T>::Impl* impl) : impl_(impl) {}
...
template <typename T>
ProximityEngine<T>& ProximityEngine<T>::operator=(const ProximityEngine<T>& other) {
...
template <typename T>
ProximityEngine<T>::ProximityEngine(ProximityEngine<T>&& other) noexcept : impl_(std::move(other.impl_)) {
    other.impl_ = new ProximityEngine<T>::Impl();
}

template <typename T>
ProximityEngine<T>& ProximityEngine<T>::operator=(ProximityEngine<T>&& other) noexcept {
    if (this == &other) return *this;
    if (impl_) delete impl_;
...
template <typename T>
void ProximityEngine<T>::AddDeformableGeometry(const VolumeMesh<double>& mesh, GeometryId id) {
...
template <typename T>
void ProximityEngine<T>::UpdateRepresentationForNewProperties(const InternalGeometry& geometry,
                                                              const ProximityProperties& new_properties) {
...
std::unique_ptr<ProximityEngine<U>> ProximityEngine<T>::ToScalarType() const {
    return std::unique_ptr<ProximityEngine<U>>(new ProximityEngine<U>(impl_->template ToScalarType<U>().release()));
}
...
template <typename T>
void ProximityEngine<T>::UpdateWorldPoses(const unordered_map<GeometryId, RigidTransform<T>>& X_WGs) {
    impl_->UpdateWorldPoses(X_WGs);
}

template <typename T>
void ProximityEngine<T>::UpdateDeformableVertexPositions(const std::unordered_map<GeometryId, VectorX<T>>& q_WGs) {
    impl_->UpdateDeformableVertexPositions(q_WGs);
}

template <typename T>
std::vector<SignedDistancePair<T>> ProximityEngine<T>::ComputeSignedDistancePairwiseClosestPoints(
        const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs, const double max_distance) const {
    return impl_->ComputeSignedDistancePairwiseClosestPoints(X_WGs, max_distance);
}

template <typename T>
SignedDistancePair<T> ProximityEngine<T>::ComputeSignedDistancePairClosestPoints(
        GeometryId id_A, GeometryId id_B, const std::unordered_map<GeometryId, math::RigidTransform<T>>& X_WGs) const {
    return impl_->ComputeSignedDistancePairClosestPoints(id_A, id_B, X_WGs);
}

template <typename T>
std::vector<SignedDistanceToPoint<T>> ProximityEngine<T>::ComputeSignedDistanceToPoint(
...
template <typename T>
std::vector<PenetrationAsPointPair<T>> ProximityEngine<T>::ComputePointPairPenetration(
        const std::unordered_map<GeometryId, math::RigidTransform<T>>& X_WGs) const {
    return impl_->ComputePointPairPenetration(X_WGs);
}

template <typename T>
template <typename T1>
typename std::enable_if_t<scalar_predicate<T1>::is_bool, std::vector<ContactSurface<T>>>
ProximityEngine<T>::ComputeContactSurfaces(HydroelasticContactRepresentation representation,
                                           const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs) const {
...
template <typename T>
template <typename T1>
typename std::enable_if_t<scalar_predicate<T1>::is_bool, void> ProximityEngine<T>::ComputeContactSurfacesWithFallback(
        HydroelasticContactRepresentation representation,
        const std::unordered_map<GeometryId, RigidTransform<T>>& X_WGs,
        std::vector<ContactSurface<T>>* surfaces,
        std::vector<PenetrationAsPointPair<T>>* point_pairs) const {
    return impl_->ComputeContactSurfacesWithFallback(representation, X_WGs, surfaces, point_pairs);
}

template <typename T>
template <typename T1>
typename std::enable_if_t<std::is_same_v<T1, double>, void> ProximityEngine<T>::ComputeDeformableContact(
        DeformableContact<T>* deformable_contact) const {
    impl_->ComputeDeformableContact(deformable_contact);
}

template <typename T>
std::vector<SortedPair<GeometryId>> ProximityEngine<T>::FindCollisionCandidates() const {
...
template <typename T>
const RigidTransformd ProximityEngine<T>::GetX_WG(GeometryId id, bool is_dynamic) const {
    return impl_->GetX_WG(id, is_dynamic);
}

template <typename T>
const hydroelastic::Geometries& ProximityEngine<T>::hydroelastic_geometries() const {
    return impl_->hydroelastic_geometries();
}

template <typename T>
const deformable::Geometries& ProximityEngine<T>::deformable_contact_geometries() const {
...
template void ProximityEngine<double>::ComputeDeformableContact<double>(DeformableContact<double>*) const;

}  // namespace internal
}  // namespace geometry
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::internal::ProximityEngine);
