...
BarycentricInterpolator::BarycentricInterpolator(const std::vector<Vector3<double>>& positions_M,
    ...
    for (int v = 0; v < ssize(positions_M); ++v) {
        ...
        for (int e = 0; e < control_mesh_M.num_elements(); ++e) {
            ...
            if ((bary.array() >= 0.0).all()) {
                ...
                vertex_indices_.emplace_back(control_mesh_M.element(e).vertex(0), control_mesh_M.element(e).vertex(1),
                                             control_mesh_M.element(e).vertex(2), control_mesh_M.element(e).vertex(3));
                matched = true;
                break;
            }
        }
        if (!matched) {
            throw std::runtime_error(fmt::format("A passively driven point lies outside the control mesh: ({}): {}", v,
...
VertexSampler::VertexSampler(std::vector<int> sampled_vertices, const VolumeMesh<double>& control_mesh)
    : sampled_vertices_(std::move(sampled_vertices)), num_control_vertices_(control_mesh.num_vertices()) {
    DRAKE_THROW_UNLESS(!sampled_vertices_.empty());
    DRAKE_THROW_UNLESS(sampled_vertices_[0] >= 0);
    // No duplicates.
    DRAKE_THROW_UNLESS(std::adjacent_find(sampled_vertices_.begin(), sampled_vertices_.end()) ==
                       sampled_vertices_.end());
    DRAKE_THROW_UNLESS(std::is_sorted(sampled_vertices_.begin(), sampled_vertices_.end()));
...
DrivenTriangleMesh::DrivenTriangleMesh(TriangleSurfaceMesh<double> triangle_mesh,
                                       const VolumeMesh<double>& control_mesh)
    : interpolator_(BarycentricInterpolator(triangle_mesh.vertices(), control_mesh)),
      triangle_mesh_(std::move(triangle_mesh)) {}

DrivenTriangleMesh::DrivenTriangleMesh(std::variant<BarycentricInterpolator, VertexSampler> interpolator,
                                       TriangleSurfaceMesh<double> triangle_surface_mesh)
    : interpolator_(std::move(interpolator)), triangle_mesh_(std::move(triangle_surface_mesh)) {}
...
VectorX<double> DrivenTriangleMesh::GetDrivenVertexNormals() const {
    VectorX<double> nhats_M = VectorX<double>::Zero(3 * triangle_mesh_.num_vertices());
    for (int f = 0; f < triangle_mesh_.num_triangles(); ++f) {
        const Vector3<double> area_weighted_normal = triangle_mesh_.area(f) * triangle_mesh_.face_normal(f);
        for (int v = 0; v < 3; ++v) {
...
DrivenTriangleMesh MakeDrivenSurfaceMesh(const VolumeMesh<double>& control_mesh) {
    std::vector<int> surface_to_volume_vertices;
    TriangleSurfaceMesh<double> triangle_mesh =
            ConvertVolumeToSurfaceMeshWithBoundaryVertices(control_mesh, &surface_to_volume_vertices);
    VertexSampler sampler{std::move(surface_to_volume_vertices), control_mesh};
    return DrivenTriangleMesh(std::variant<BarycentricInterpolator, VertexSampler>(std::move(sampler)),
