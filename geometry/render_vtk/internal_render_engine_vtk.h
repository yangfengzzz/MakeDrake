...
public:
    ...
    void Execute(vtkObject*, unsigned long, void* callback_object) override {
        vtkShaderProgram* program = reinterpret_cast<vtkShaderProgram*>(callback_object);
...
class DRAKE_NO_EXPORT RenderEngineVtk : public render::RenderEngine, private ShapeReifier {
public:
    ...
    */
    explicit RenderEngineVtk(const RenderEngineVtkParams& parameters = RenderEngineVtkParams());
...
protected:
    ...
    void UpdateWindow(const render::RenderCameraCore& camera,
                      bool show_window,
    ...
    void UpdateWindow(const render::DepthRenderCamera& camera, const RenderingPipeline& p) const;
    ...
    // polydata.
    void ImplementRenderMesh(geometry::internal::RenderMesh&& mesh, double scale, const RegistrationData& data);
    ...
    bool ImplementObj(const std::string& file_name, double scale, const RegistrationData& data);

    // Adds a .gltf to the scene for the id currently being reified (data->id).
    // Returns true if added, false if ignored (for whatever reason).
    bool ImplementGltf(const std::string& file_name, double scale, const RegistrationData& data);
