#include "geometry/render_vtk/internal_render_engine_vtk.h"
...
#include "systems/sensors/image_io.h"
#include "systems/sensors/test_utilities/image_compare.h"
#include "visualization/colorize_depth_image.h"
...
public:
    ...
    static vtkActor* GetColorActor(const RenderEngineVtk& renderer, GeometryId id) {
    ...
    static std::vector<vtkActor*> GetColorActors(const RenderEngineVtk& renderer, GeometryId id) {
...
struct TestColor {
    ...
    constexpr TestColor(int r_in, int g_in, int b_in, int a_in = 255) : r(r_in), g(g_in), b(b_in), a(a_in) {}
    ...
    Rgba ToRgba() const { return Rgba(r / 255.0, g / 255.0, b / 255.0, a / 255.0); }

    bool operator==(const TestColor& c) const { return r == c.r && g == c.g && b == c.b && a == c.a; }
...
bool IsColorNear(const TestColor& expected, const TestColor& tested, double tolerance = kColorPixelTolerance) {
    using std::abs;
    return (abs(expected.r - tested.r) < tolerance && abs(expected.g - tested.g) < tolerance &&
            abs(expected.b - tested.b) < tolerance && abs(expected.a - tested.a) < tolerance);
}
...
::testing::AssertionResult CompareColor(const TestColor& expected,
    ...
    return ::testing::AssertionFailure() << "Expected: " << expected << " at " << p << ", tested: " << tested
...
public:
    RenderEngineVtkTest()
        : color_(kWidth, kHeight),
          ...
          X_WC_(RotationMatrixd{AngleAxisd(M_PI, Vector3d::UnitY()) * AngleAxisd(-M_PI_2, Vector3d::UnitZ())},
...
protected:
    ...
    void Render(RenderEngineVtk* renderer = nullptr,
        ...
        const DepthRenderCamera& depth_camera = camera_in ? *camera_in : depth_camera_;
        const ColorRenderCamera color_camera(depth_camera.core(), FLAGS_show_window);
    ...
    void VerifyUniformColor(const TestColor& pixel, const ImageRgba8U* color = nullptr) {
    ...
    void VerifyUniformLabel(int16_t value, const ImageLabel16I* label = nullptr) {
        ...
        for (int y = 0; y < label->height(); ++y) {
            for (int x = 0; x < label->width(); ++x) {
                ASSERT_EQ(label->at(x, y)[0], value) << "At pixel (" << x << ", " << y << ")";
            }
        }
    }
    ...
    static std::vector<ScreenCoord> GetOutliers(const CameraInfo& intrinsics) {
        return std::vector<ScreenCoord>{{kInset, kInset},
        ...
        if (expected_depth == std::numeric_limits<float>::infinity()) {
            if (actual_depth == expected_depth) {
                ...
                return ::testing::AssertionFailure()
                       << "Expected depth at " << coord << " to be infinity. Found: " << actual_depth;
            }
        } else {
            ...
            if (delta <= tolerance) {
                ...
                return ::testing::AssertionFailure()
                       << "Expected depth at " << coord << " to be " << expected_depth << ". Found " << actual_depth
                       << ". Difference " << delta << " is greater than tolerance " << tolerance;
            }
        }
    }
    ...
    void VerifyOutliers(const RenderEngineVtk& renderer,
        ...
        for (const auto& screen_coord : GetOutliers(camera.core().intrinsics())) {
            ...
            EXPECT_TRUE(IsExpectedDepth(depth, screen_coord, expected_outlier_depth_, kDepthTolerance))
    ...
    void Init(const RigidTransformd& X_WR, bool add_terrain = false) {
        const Vector3d bg_rgb{kBgColor.r / 255., kBgColor.g / 255., kBgColor.b / 255.};
        ...
        EXPECT_FALSE(IsColorNear(kDefaultVisualColor, TestColor(renderer_->default_diffuse())));
    }

    // Tests that instantiate their own renderers can initialize their renderers
    // with this method.
    void InitializeRenderer(const RigidTransformd& X_WR, bool add_terrain, RenderEngineVtk* engine) {
        ...
        if (add_terrain) {
            ...
            engine->RegisterVisual(GeometryId::get_new_id(), HalfSpace(), material, RigidTransformd::Identity(),
                                   false /* needs update */);
        }
    }
    ...
    PerceptionProperties simple_material(bool use_texture = false) const {
        ...
        if (use_texture) {
            ...
            const Rgba default_color(default_color_.r / 255.0, default_color_.g / 255.0, default_color_.b / 255.0,
                                     default_color_.a / 255.0);
    ...
    void PopulateSphereTest(RenderEngineVtk* renderer, bool use_texture = false) {
        ...
        renderer->RegisterVisual(geometry_id_, sphere, simple_material(use_texture), RigidTransformd::Identity(),
                                 true /* needs update */);
    ...
    void PopulateSimpleBoxTest(RenderEngineVtk* renderer) {
        ...
        renderer->RegisterVisual(id, box, props, RigidTransformd::Identity(), true /* needs update */);
        ...
        const double p_WVo_z = X_WC_.translation()(2) - length / 2 - expected_object_depth_;
        RigidTransformd X_WV{Vector3d{0, 0, p_WVo_z}};
        renderer->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, X_WV}});
    ...
    void VerifyCenterShapeTest(const RenderEngineVtk& renderer,
        ...
        EXPECT_TRUE(CompareColor(expected_color_, color, inlier)) << "Color at: " << inlier << " for test: " << name;
        EXPECT_TRUE(IsExpectedDepth(depth, inlier, expected_object_depth_, kDepthTolerance))
    ...
    // FLAGS_show_window.
    const DepthRenderCamera depth_camera_{{"unused", {kWidth, kHeight, kFovY}, {kClipNear, kClipFar}, {}},
                                          {kZNear, kZFar}};
...
TEST_F(RenderEngineVtkTest, ControlBackgroundColor) {
    std::vector<TestColor> backgrounds{{10, 20, 30}, {128, 196, 255}, {255, 10, 40}};
    for (const auto& bg : backgrounds) {
        RenderEngineVtkParams params{{}, Vector3d{bg.r / 255., bg.g / 255., bg.b / 255.}};
...
TEST_F(RenderEngineVtkTest, MeshTest) {
    for (const bool use_texture : {false, true}) {
        ...
        auto filename = use_texture ? FindResourceOrThrow("drake/geometry/render/test/meshes/box.obj")
        ...
        const GeometryId id = GeometryId::get_new_id();
        renderer_->RegisterVisual(id, mesh, material, RigidTransformd::Identity(), true /* needs update */);
        renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd::Identity()}});

        expected_color_ = use_texture ? kTextureColor : default_color_;
        PerformCenterShapeTest(renderer_.get(), fmt::format("Mesh test {}", use_texture ? "textured" : "rgba").c_str());
    }
}
...
TEST_F(RenderEngineVtkTest, GltfTextureSupport) {
    ...
    const RigidTransformd X_WC(R_WC, R_WC * Vector3d(0, 0, -6) + Vector3d(0, 0, -0.15));
    Init(X_WC);

    PerceptionProperties material;
    material.AddProperty("label", "id", RenderLabel(1));
    const GeometryId id = GeometryId::get_new_id();
    const std::string filename = FindResourceOrThrow("drake/geometry/render/test/meshes/fully_textured_pyramid.gltf");
    renderer_->RegisterVisual(id, Mesh(filename), material, RigidTransformd::Identity(), false /* needs update */);
    ImageRgba8U image(64, 64);
    const ColorRenderCamera camera({"unused", {64, 64, kFovY / 2}, {0.01, 10}, {}}, FLAGS_show_window);
    renderer_->RenderColorImage(camera, &image);

    ImageRgba8U expected_image;
    const std::string ref_filename =
            FindResourceOrThrow("drake/geometry/render/test/fully_textured_pyramid_rendered.png");
    ...
    Eigen::Map<VectorX<uint8_t>> data_expected(expected_image.at(0, 0), expected_image.size());
    Eigen::Map<VectorX<uint8_t>> data2(image.at(0, 0), image.size());
    const auto differences = (data_expected.cast<float>() - data2.cast<float>()).array().abs();
...
TEST_F(RenderEngineVtkTest, GltfAssetFormats) {
    ...
    for (int i = 0; i < ssize(filenames); ++i) {
        ...
        const GeometryId id = GeometryId::get_new_id();
        renderer_->RegisterVisual(id, Mesh(filenames[i]), PerceptionProperties{}, RigidTransformd::Identity(), true);
        renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd::Identity()}});
    ...
    if (const char* dir = std::getenv("TEST_UNDECLARED_OUTPUTS_DIR")) {
        ...
        for (int i = 0; i < ssize(images); ++i) {
            ImageIo{}.Save(images[i], out_dir / fmt::format("gltf_asset_formats_{}.png", i));
        }
    }
}
...
TEST_F(RenderEngineVtkTest, GltfUnsupportedExtensionRequired) {
    ...
    material.AddProperty("label", "id", RenderLabel(1));
    const GeometryId id = GeometryId::get_new_id();
    const std::string filename = FindResourceOrThrow("drake/geometry/render/test/meshes/basisu_required.gltf");
    DRAKE_EXPECT_THROWS_MESSAGE(renderer_->RegisterVisual(id, Mesh(filename), material, RigidTransformd::Identity(),
                                                          false /* needs update */),
...
TEST_F(RenderEngineVtkTest, MultiMaterialObjects) {
    ...
    const std::vector<std::string> filenames{FindResourceOrThrow("drake/geometry/render/test/meshes/rainbow_box.gltf"),
    ...
    for (const auto& filename : filenames) {
        ...
        expected_outlier_color_ = mesh.extension() == ".gltf" ? TestColor(Rgba(0.4392, 0.4392, 0.4745)) : kTerrainColor;
        expected_label_ = RenderLabel(3);
        ...
        for (const auto& face : faces) {
            ...
            renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd(face.rotation)}});
            PerformCenterShapeTest(renderer_.get(),
                                   fmt::format("{} test on {} face - original", mesh.extension(), face.name).c_str());
        }
        ...
        for (const auto& face : faces) {
            ...
            vtk_clone->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{id, RigidTransformd(face.rotation)}});
            PerformCenterShapeTest(vtk_clone,
                                   fmt::format("{} test on {} face - clone", mesh.extension(), face.name).c_str());
        }
    }
}
...
bool TransformComponentsAreIdentity(vtkActor* a) {
    ...
    return (position.array() == 0).all() && (origin.array() == 0).all() && (orientation.array() == 0).all() &&
           (scale.array() == 1).all();
}
...
TEST_F(RenderEngineVtkTest, VtkGltfBehavior) {
    ...
    const std::string filename = FindResourceOrThrow("drake/geometry/render/test/meshes/rainbow_box.gltf");
    ...
    const GeometryId id = GeometryId::get_new_id();
    renderer_->RegisterVisual(id, mesh, material, RigidTransformd::Identity(), true /* needs update */);
    for (vtkActor* actor : RenderEngineVtkTester::GetColorActors(*renderer_, id)) {
...
TEST_F(RenderEngineVtkTest, UnsupportedMesh) {
    ...
    EXPECT_FALSE(renderer_->RegisterVisual(id, mesh, material, RigidTransformd::Identity(), false /* needs update */));
}
...
TEST_F(RenderEngineVtkTest, NonUcharChannelTextures) {
    ...
    const std::string file_path = FindResourceOrThrow("drake/geometry/render/test/meshes/box16u.png");
...
TEST_F(RenderEngineVtkTest, RemoveVisual) {
    ...
    auto add_sphere = [this](const TestColor& diffuse, double z, GeometryId geometry_id) {
        const double kRadius = 0.5;
        Sphere sphere{kRadius};
        const float depth = kDefaultDistance - kRadius - z;
        Vector4d norm_diffuse{diffuse.r / 255., diffuse.g / 255., diffuse.b / 255., diffuse.a / 255.};
        ...
        renderer_->RegisterVisual(geometry_id, sphere, material, RigidTransformd::Identity());
    ...
    auto set_expectations = [this](const TestColor& color, float depth, RenderLabel label)
...
// still, there is no agreement on attribute spelling.
    ...
    PerformCenterShapeTest(renderer_.get(), "Default image restored by removing extra geometries");
}
...
TEST_F(RenderEngineVtkTest, SimpleClone) {
    ...
    PerformCenterShapeTest(static_cast<RenderEngineVtk*>(clone.get()), "Simple clone");
}
...
TEST_F(RenderEngineVtkTest, ClonePersistence) {
    ...
    PerformCenterShapeTest(static_cast<RenderEngineVtk*>(clone.get()), "Clone persistence");
}
...
TEST_F(RenderEngineVtkTest, CloneIndependence) {
    ...
    renderer_->UpdatePoses(unordered_map<GeometryId, RigidTransformd>{{geometry_id_, X_WT_new}});
    PerformCenterShapeTest(static_cast<RenderEngineVtk*>(clone.get()), "Clone independence");
}
...
TEST_F(RenderEngineVtkTest, DifferentCameras) {
    ...
    PerformCenterShapeTest(renderer_.get(), "Camera change - baseline", &depth_camera_);

    // Test changes in sensor sizes.
    {
        // Now run it again with a camera with a smaller sensor (quarter area).
        const DepthRenderCamera small_camera{{ref_name, {ref_w / 2, ref_h / 2, ref_fov_y}, ref_clipping, ref_X_BS},
                                             depth_camera_.depth_range()};
        PerformCenterShapeTest(renderer_.get(), "Camera change - small camera", &small_camera);

        // Now run it again with a camera with a bigger sensor (4X area).
        const DepthRenderCamera big_camera{{ref_name, {ref_w * 2, ref_h * 2, ref_fov_y}, ref_clipping, ref_X_BS},
                                           depth_camera_.depth_range()};
        PerformCenterShapeTest(renderer_.get(), "Camera change - big camera", &big_camera);
    }

    // Test changes in fov (larger and smaller).
    {
        const DepthRenderCamera narrow_fov{{ref_name, {ref_w, ref_h, ref_fov_y / 2}, ref_clipping, ref_X_BS},
                                           depth_camera_.depth_range()};
        PerformCenterShapeTest(renderer_.get(), "Camera change - narrow fov", &narrow_fov);

        const DepthRenderCamera wide_fov{{ref_name, {ref_w, ref_h, ref_fov_y * 2}, ref_clipping, ref_X_BS},
                                         depth_camera_.depth_range()};
        PerformCenterShapeTest(renderer_.get(), "Camera change - wide fov", &wide_fov);
    }

    // Test changes to depth range.
    {
        const auto& depth_range = depth_camera_.depth_range();
        const DepthRenderCamera clipping_far_plane{depth_camera_.core(),
        ...
        PerformCenterShapeTest(renderer_.get(), "Camera change - z far clips terrain", &clipping_far_plane);
        // NOTE: Need to restored expected outlier depth for next test.
        expected_outlier_depth_ = old_outlier_depth;

        const DepthRenderCamera clipping_near_plane{depth_camera_.core(),
                                                    {expected_object_depth_ + 0.1, depth_range.max_depth()}};
        expected_object_depth_ = 0;
        PerformCenterShapeTest(renderer_.get(), "Camera change - z near clips mesh", &clipping_near_plane);
    }
}
...
TEST_F(RenderEngineVtkTest, DefaultProperties_RenderLabel) {
    ...
    auto populate_default_sphere = [](auto* engine) {
        Sphere sphere{0.5};
        const GeometryId id = GeometryId::get_new_id();
        engine->RegisterVisual(id, sphere, PerceptionProperties(), RigidTransformd::Identity(),
                               true /* needs update */);
        RigidTransformd X_WV{Vector3d{0, 0, 0.5}};
...
public:
    ...
    bool GeometryHasColorTexture(GeometryId id, const std::string& texture_name) const {
    ...
    // geometry.
    void ApplyColorTextureToGeometry(GeometryId id, const std::string& texture_name) {
        vtkActor* actor = RenderEngineVtkTester::GetColorActor(*this, id);
...
protected:
    std::unique_ptr<RenderEngine> DoClone() const override { return make_unique<TextureSetterEngine>(*this); }
};
...
TEST_F(RenderEngineVtkTest, PreservePropertyTexturesOverClone) {
    ...
    material.AddProperty("label", "id", label);
    engine.RegisterVisual(id, sphere, material, RigidTransformd(), true /* needs update */);
    ...
    const TextureSetterEngine* clone = dynamic_cast<TextureSetterEngine*>(clone_ptr.get());
...
TEST_F(RenderEngineVtkTest, FallbackLight) {
    ...
    renderer.RegisterVisual(GeometryId::get_new_id(), box, props, X_WB, false /* needs update */);
    renderer.RegisterVisual(GeometryId::get_new_id(), HalfSpace(), props, RigidTransformd::Identity(),
                            false /* needs update */);
    ...
    const Rgba reduced_color(test_color.r() * half_sqrt2, test_color.g() * half_sqrt2, test_color.b() * half_sqrt2);
    ...
    const std::vector<Config> configs{
            {.X_WR = RigidTransformd(RotationMatrixd::MakeXRotation(M_PI), X_WB.translation() + Vector3d(0, 0, 1.1)),
    ...
    for (RenderEngineVtk* renderer_ptr : {&renderer, clone_vtk}) {
        for (const auto& config : configs) {
            SCOPED_TRACE(fmt::format("{} - {}", config.description, renderer_ptr == clone_vtk ? "Cloned" : "Original"));
            ...
            for (int r : {0, mid_height}) {
                for (int c = 0; c < image.width(); ++c) {
                    ...
                    if (!IsColorNear(dut, config.expected_color)) {
                        EXPECT_EQ(dut, config.expected_color) << "at pixel (" << c << ", " << r << ")";
                        break;
                    }
                }
            }
...
TEST_F(RenderEngineVtkTest, SingleLight) {
    ...
    const RigidTransformd X_WR(RotationMatrixd::MakeXRotation(M_PI), Vector3d(0, 0, dist));
    ImageRgba8U image(camera.core().intrinsics().width(), camera.core().intrinsics().height());
    ...
    const std::vector<Config> configs{{.light = {.color = Rgba(1, 1, 1),
    ...
    for (const auto& config : configs) {
        for (const auto& l_type : {"point", "spot", "directional"}) {
            ...
            if (l_type == std::string("directional") && config.description.find("attenuation") != std::string::npos) {
                continue;
            }
            SCOPED_TRACE(fmt::format("{} - {}", fmt_streamed(l_type), config.description));
...
TEST_F(RenderEngineVtkTest, MultiLights) {
    const ColorRenderCamera camera(depth_camera_.core(), FLAGS_show_window);
    const RigidTransformd X_WR(RotationMatrixd::MakeXRotation(M_PI), Vector3d(0, 0, 3));
    ImageRgba8U image(camera.core().intrinsics().width(), camera.core().intrinsics().height());
    const int cx = image.width() / 2;
    const int cy = image.height() / 2;
    ...
    const RenderEngineVtkParams params{.lights = {{.type = "point", .intensity = 0.25 * 0.5},
    ...
    EXPECT_TRUE(IsColorNear(test_color, expected_color)) << "  test color: " << test_color << "\n"
...
TEST_F(RenderEngineVtkTest, EnvironmentMap) {
    ...
    const std::string hdr_path = FindResourceOrThrow("drake/geometry/test/env_256_six_color_room.hdr");
    const std::string ldr_path = FindResourceOrThrow("drake/geometry/test/env_256_six_color_room.png");
    ...
    for (const auto& config : configs) {
        ...
        const RenderEngineVtkParams params{
                .environment_map = EnvironmentMap{.skybox = config.show_map,
        ...
        renderer.RegisterVisual(geometry_id_, sphere, material, RigidTransformd::Identity(), false /* needs update */);
        ...
        EXPECT_TRUE(IsColorNear(edge_color, bg_color, tolerance)) << "  test bg color: " << edge_color << "\n"
...
TEST_F(RenderEngineVtkTest, PbrMaterialPromotion) {
    auto test_sphere_color = [this](const TestColor expected_color, RenderEngineVtk* renderer) {
        const ColorRenderCamera camera(depth_camera_.core(), FLAGS_show_window);
        ImageRgba8U image(camera.core().intrinsics().width(), camera.core().intrinsics().height());
        // Center pixel -- middle of the sphere.
        const int cx = image.width() / 2;
        ...
        EXPECT_TRUE(IsColorNear(sampled_color, expected_color)) << "  rendered color: " << sampled_color << "\n"
                                                                << "  expected color: " << expected_color;
    };
    ...
    // specified at all.
    {
        ...
        const Vector3d bg_rgb{kBgColor.r / 255., kBgColor.g / 255., kBgColor.b / 255.};
        const RenderEngineVtkParams params{.default_clear_color = bg_rgb, .environment_map = EnvironmentMap()};
    ...
    // Add a glTF file; material promoted to PBR no longer matches Phong color.
    {
        ...
        const Mesh mesh(FindResourceOrThrow("drake/geometry/render/test/meshes/rainbow_box.gltf"));
        PerceptionProperties material;
        material.AddProperty("label", "id", RenderLabel::kDontCare);
        renderer_->RegisterVisual(GeometryId::get_new_id(), mesh, material, RigidTransformd(Vector3d(30, 0, 0)),
                                  false /* needs update */);
...
AdjacentPixel Compare(const typename ImageDepth32F::T* curr_pixel, const typename ImageDepth32F::T* next_pixel) {
...
AdjacentPixel Compare(const typename ImageRgba8U::T* curr_pixel, const typename ImageRgba8U::T* next_pixel) {
    const TestColor ground(kTerrainColor);
    const TestColor curr(curr_pixel);
    const TestColor next(next_pixel);

    const bool curr_is_ground = curr.r == ground.r && curr.g == ground.g && curr.b == ground.b;
    const bool next_is_ground = next.r == ground.r && next.g == ground.g && next.b == ground.b;
    if (!curr_is_ground && next_is_ground) return BoxToGround;
    if (curr_is_ground && !next_is_ground) return GroundToBox;
    return Same;
}

AdjacentPixel Compare(const typename ImageLabel16I::T* curr_pixel, const typename ImageLabel16I::T* next_pixel) {
...
TEST_F(RenderEngineVtkTest, IntrinsicsAndRenderProperties) {
    ...
    const ColorRenderCamera ref_color_camera{{"n/a", ref_intrinsics, {clip_n, clip_f}, {}}, FLAGS_show_window};
    const DepthRenderCamera ref_depth_camera{{"n/a", ref_intrinsics, {clip_n, clip_f}, {}}, {min_depth, max_depth}};
    ...
    ASSERT_TRUE((ref_box_edges.array() > -1).all()) << fmt::to_string(fmt_eigen(ref_box_edges.transpose()));
...

    {
        ...
        ASSERT_EQ(color_edges, ref_box_edges) << fmt::to_string(fmt_eigen(color_edges));
        const Vector4<int> label_edges = FindBoxEdges(ref_label);
        ASSERT_EQ(label_edges, ref_box_edges) << fmt::to_string(fmt_eigen(label_edges));
    }

    {
        ...
        const ColorRenderCamera color_camera{{"n/a", intrinsics, {clip_n, clip_f}, {}}, FLAGS_show_window};
        const DepthRenderCamera depth_camera{{"n/a", intrinsics, {clip_n, clip_f}, {}}, {min_depth, max_depth}};
        ...
        ASSERT_TRUE((test_edges.array() > -1).all()) << fmt::to_string(fmt_eigen(test_edges.transpose()));
...

        {
            ...
            ASSERT_EQ(color_edges, test_edges) << fmt::to_string(fmt_eigen(color_edges.transpose()));
            const Vector4<int> label_edges = FindBoxEdges(label);
            ASSERT_EQ(label_edges, test_edges) << fmt::to_string(fmt_eigen(label_edges.transpose()));
        }
    }

    {
        ...
        const ColorRenderCamera color_camera{{"n/a", ref_intrinsics, {n_alt, f_alt}, {}}, FLAGS_show_window};
        // Set depth range to clipping range so we don't take a chance with the
        // depth range lying outside the clipping range.
        const DepthRenderCamera depth_camera{{"n/a", ref_intrinsics, {n_alt, f_alt}, {}}, {n_alt, f_alt}};
...

    {
        ...
        const DepthRenderCamera depth_camera{{"n/a", ref_intrinsics, {clip_n, clip_f}, {}}, {min_alt, max_alt}};
        ImageDepth32F depth(ref_intrinsics.width(), ref_intrinsics.height());
        renderer_->RenderDepthImage(depth_camera, &depth);

        // Confirm pixel in corner (ground) and pixel in center (box).
        EXPECT_TRUE(
                IsExpectedDepth(depth, ScreenCoord{w / 2, h / 2}, ImageTraits<PixelType::kDepth32F>::kTooClose, 0.0));
        EXPECT_TRUE(IsExpectedDepth(depth, ScreenCoord{0, 0}, ImageTraits<PixelType::kDepth32F>::kTooFar, 0.0));
    }

    {
        ...
        // Confirm pixel in corner (ground) and pixel in center (box).
        EXPECT_TRUE(IsExpectedDepth(depth, ScreenCoord{w / 2, h / 2}, ImageTraits<PixelType::kDepth32F>::kTooFar, 0.0));
        EXPECT_TRUE(IsExpectedDepth(depth, ScreenCoord{0, 0}, ImageTraits<PixelType::kDepth32F>::kTooFar, 0.0));
    }

    {
        ...
        EXPECT_TRUE(
                IsExpectedDepth(depth, ScreenCoord{w / 2, h / 2}, ImageTraits<PixelType::kDepth32F>::kTooClose, 0.0));
        EXPECT_TRUE(IsExpectedDepth(depth, ScreenCoord{0, 0}, ImageTraits<PixelType::kDepth32F>::kTooClose, 0.0));
    }
}
...
double AddShapeRows(RenderEngineVtk* render_engine, const WholeImageConfig& config = {}) {
    auto register_visual = [render_engine](GeometryId id, const Shape& shape, const PerceptionProperties& properties,
        ...
        if (render_engine != nullptr) {
            ...
            const RigidTransformd X_WG = std::holds_alternative<Vector3d>(pose)
    ...
    register_visual(GeometryId::get_new_id(), box, diffuse_material(Rgba(1, 0.25, 0.25)), Vector3d{x, row1, 0});
    register_visual(GeometryId::get_new_id(), box, texture_material(), Vector3d{x, row2, 0});
    PerceptionProperties scaled_texture_material(texture_material());
    scaled_texture_material.AddProperty("phong", "diffuse_scale", Vector2d{0.5, 0.5});
    register_visual(GeometryId::get_new_id(), box, scaled_texture_material, Vector3d{x, row3, 0});

    x -= 0.15;
    const Capsule capsule(0.05, 0.1);
    register_visual(GeometryId::get_new_id(), capsule, diffuse_material(Rgba(1, 1, 0.25)), Vector3d{x, row1, 0});
    register_visual(GeometryId::get_new_id(), capsule, texture_material(), Vector3d{x, row2, 0});

    // A non-convex shape should be replaced by its convex hull. It doesn't take
    // a texture, so we don't need a texture variant.
    x -= 0.15;
    const Convex convex(FindResourceOrThrow("drake/examples/scene_graph/cuboctahedron_with_hole.obj"), 0.05);
    register_visual(GeometryId::get_new_id(), convex, diffuse_material(Rgba(0.8, 0.25, 0.8)), Vector3d{x, row1, 0});

    x -= 0.15;
    const Cylinder cylinder(0.05, 0.1);
    register_visual(GeometryId::get_new_id(), cylinder, diffuse_material(Rgba(0.25, 1, 0.25)), Vector3d{x, row1, 0});
    register_visual(GeometryId::get_new_id(), cylinder, texture_material(), Vector3d{x, row2, 0});

    x -= 0.15;
    const Ellipsoid ellipsoid(0.05, 0.025, 0.0375);
    register_visual(GeometryId::get_new_id(), ellipsoid, diffuse_material(Rgba(0.25, 1, 1)), Vector3d{x, row1, 0});
    register_visual(GeometryId::get_new_id(), ellipsoid, texture_material(), Vector3d{x, row2, 0});
    ...
    const Mesh mesh1(
            FindResourceOrThrow(config.gltf_mesh ? "drake/geometry/render/test/meshes/fully_textured_pyramid.gltf"
                                                 : "drake/geometry/render/test/meshes/box_checkered.obj"),
            0.05);
    register_visual(GeometryId::get_new_id(), mesh1, diffuse_material(Rgba(0.25, 1, 0.25)), Vector3d{x, row1, 0});

    register_visual(GeometryId::get_new_id(), mesh1, minimum_material(), Vector3d{x, row2, 0});

    register_visual(GeometryId::get_new_id(), mesh1, texture_material(), Vector3d{x, row3, 0});

    x -= 0.15;
    const Mesh mesh2(FindResourceOrThrow("drake/geometry/render/test/meshes/box_no_mtl.obj"), 0.05);
    register_visual(GeometryId::get_new_id(), mesh2, diffuse_material(Rgba(0.25, 1, 0.25)), Vector3d{x, row1, 0});

    register_visual(GeometryId::get_new_id(), mesh2, minimum_material(), Vector3d{x, row2, 0});

    register_visual(GeometryId::get_new_id(), mesh2, texture_material(), Vector3d{x, row3, 0});

    x -= 0.15;
    const Sphere sphere(0.05);
    register_visual(GeometryId::get_new_id(), sphere, diffuse_material(Rgba(0.25, 0.25, 1)), Vector3d{x, row1, 0});
    register_visual(GeometryId::get_new_id(), sphere, texture_material(), Vector3d{x, row2, 0});
    ...
    // With shadows, it should receive but not cast shadows.
    {
        ...
        register_visual(GeometryId::get_new_id(), slab, diffuse_material(Rgba(0.8, 0.5, 0.5)),
                        RigidTransformd(Vector3d(mid_x + 0.05, row3, 0.1)));
        // Just a little transparent means it won't cast or receive shadows.
        register_visual(GeometryId::get_new_id(), slab, diffuse_material(Rgba(0.5, 0.8, 0.5, 0.5)),
                        RigidTransformd(Vector3d(mid_x + 0.1, row3 + 0.05, 0.2)));
        register_visual(GeometryId::get_new_id(), slab, diffuse_material(Rgba(0.5, 0.5, 0.8)),
    ...
    if (config.no_half_space) {
        ...
        register_visual(GeometryId::get_new_id(), ground, diffuse_material(ground_color),
        ...
        register_visual(GeometryId::get_new_id(), half_space, diffuse_material(ground_color), Vector3d{0, 0, -0.2});
    }
...
template <typename ImageType>
void CompareImages(const ImageType& test_image,
                   const std::string& ref_filename,
                   double tolerance,
                   std::string_view log_suffix = {}) {
    // The type we save to disk and compare the bytes of; may not be the same as
    // the input image type.
    using CompareType = std::conditional_t<std::is_same_v<ImageType, ImageDepth32F>, ImageRgba8U, ImageType>;
    ...
    if (const char* dir = std::getenv("TEST_UNDECLARED_OUTPUTS_DIR")) {
        const std::filesystem::path out_dir(dir);
        ImageIo{}.Save(compare_image, out_dir / fmt::format("{}{}_test.png", ref_path.stem().string(), log_suffix));
    }
    ...
    Eigen::Map<const VectorX<T>> data_expected(expected_image.at(0, 0), expected_image.size());
    Eigen::Map<const VectorX<T>> data_actual(compare_image.at(0, 0), compare_image.size());
    const Eigen::ArrayXd differences =
            (data_expected.template cast<double>() - data_actual.template cast<double>()).array().abs();
    const int num_acceptable = (differences <= tolerance).count();
    const double kConformity = 0.995;
    EXPECT_GE(num_acceptable / static_cast<float>(expected_image.size()), kConformity);
}

// Given lights measured and expressed in World, re-expresses them in Camera.
vector<LightParameter> TransformLightsToCamera(const vector<LightParameter>& lights_W, const RigidTransformd& X_WC) {
...
TEST_F(RenderEngineVtkTest, WholeImageDefaultParams) {
    ...
    const DepthRenderCamera camera(
            {"unused", intrinsics, depth_camera_.core().clipping(), depth_camera_.core().sensor_pose_in_camera_body()},
            depth_camera_.depth_range());
    ...
    const RigidTransformd X_WR(RotationMatrixd::MakeXRotation(-3.2 * M_PI / 4), Vector3d(last_x / 2, -0.75, 1.1));
    engine.UpdateViewpoint(X_WR);
...

    {
        ...
        CompareImages(color, "drake/geometry/render_vtk/test/whole_image_default_color.png",
                      /* tolerance = */ 2);
    }
    {
        SCOPED_TRACE("Depth image");
        CompareImages(depth, "drake/geometry/render_vtk/test/whole_image_default_depth.png",
                      /* tolerance = */ 2);
    }
    {
        SCOPED_TRACE("Label image");
        CompareImages(label, "drake/geometry/render_vtk/test/whole_image_default_label.png",
                      /* tolerance = */ 0);
    }
}
...
TEST_F(RenderEngineVtkTest, WholeImageCustomParams) {
    ...
    const double last_x = AddShapeRows(nullptr, {.gltf_mesh = true, .no_half_space = true});
    ...
    for (const vector<LightParameter>& lights : {lights_W, TransformLightsToCamera(lights_W, X_WR)}) {
        const RenderEngineVtkParams params{
                .default_diffuse = Eigen::Vector4d(0.1, 0.2, 0.4, 1.0),
                .default_clear_color = Vector3d(0.25, 0.25, 0.25),
                .lights = lights,
                .environment_map = {EnvironmentMap{// Note: The ground plane covers the full background, we won't be
        ...
        const CameraInfo& source_intrinsics = depth_camera_.core().intrinsics();
        const CameraInfo intrinsics(w, h, source_intrinsics.fov_y());
        const DepthRenderCamera camera({"unused", intrinsics, depth_camera_.core().clipping(),
                                        depth_camera_.core().sensor_pose_in_camera_body()},
                                       depth_camera_.depth_range());

        // The value we used to define the camera pose better match.
        DRAKE_DEMAND(AddShapeRows(&engine, {.gltf_mesh = true, .no_half_space = true}) == last_x);
...

        {
            ...
            CompareImages(color, "drake/geometry/render_vtk/test/whole_image_custom_color.png",
                          /* tolerance = */ 2, log_suffix);
        }
        {
            SCOPED_TRACE("Depth image");
            CompareImages(depth, "drake/geometry/render_vtk/test/whole_image_custom_depth.png",
                          /* tolerance = */ 2);
        }
        {
            SCOPED_TRACE("Label image");
            CompareImages(label, "drake/geometry/render_vtk/test/whole_image_custom_label.png",
                          /* tolerance = */ 0);
        }
    }
}
...
TEST_F(RenderEngineVtkTest, WholeImageVerticalAspectRatio) {
    ...
    const CameraInfo ref_intrinsics(480, 360, depth_camera_.core().intrinsics().fov_y());
    ...
    const CameraInfo intrinsics(w, h, ref_intrinsics.focal_x(), ref_intrinsics.focal_y(), w * 0.5 - 0.5, h * 0.5 - 0.5);
    const DepthRenderCamera camera(
            {"unused", intrinsics, depth_camera_.core().clipping(), depth_camera_.core().sensor_pose_in_camera_body()},
            depth_camera_.depth_range());

    // More boilerplate in common with the whole image custom parameters test.
    const double last_x = AddShapeRows(&engine, {.gltf_mesh = true, .no_half_space = true});
    // Now make the rendering.
    // The camera is above the Wz = 0 plane, looking generally down and in the
    // +Wy direction (across the rows of shapes).
    const RigidTransformd X_WR(RotationMatrixd::MakeXRotation(-3.2 * M_PI / 4), Vector3d(last_x / 2, -0.75, 1.1));
