...
void TryToRenderSomething() {
    ...
    const ColorRenderCamera color_camera({"unused", {kWidth, kHeight, kFovY}, {kClipNear, kClipFar}, {}});
...
GTEST_TEST(RenderEngineVtkNoDisplayTest, BadDisplay) {
    ...
    // Rendering should throw.
    DRAKE_EXPECT_THROWS_MESSAGE(TryToRenderSomething(), ".*bad X server.*DISPLAY=Hello.*");
}
