...
#include "geometry/render/shaders/depth_shaders.h"
#include "geometry/render_vtk/internal_render_engine_vtk_base.h"
#include "geometry/render_vtk/internal_vtk_util.h"
#include "math/rotation_matrix.h"
#include "systems/sensors/vtk_diagnostic_event_observer.h"
...
void SetModelTransformMatrixToVtkCamera(vtkCamera* camera, const vtkSmartPointer<vtkTransform>& X_WC) {
...
float CheckRangeAndConvertToMeters(float z_buffer_value, double z_near, double z_far) {
...
EnvironmentTexture ReadEquirectangularFile(std::string const& fileName) {
    ...
    std::transform(extension.cbegin(), extension.cend(), extension.begin(), [](char c) {
    ...
    if (std::string(".jpeg .jpg .png").find(extension, 0) != std::string::npos) {
        ...
        imgReader.TakeReference(readerFactory->CreateImageReader2(fileName.c_str()));
        ...
        if (std::string(extensions).find(extension, 0) != std::string::npos) {
            if (reader->CanReadFile(fileName.c_str())) {
                ...
                throw std::runtime_error(
                        fmt::format("Unable to instantiate environment map for RenderEngineVtk: '{}'.", fileName));
            }
        }
    }
...
RenderEngineVtk::RenderEngineVtk(const RenderEngineVtkParams& parameters)
    : RenderEngine(RenderLabel::kDontCare),
      ...
      pipelines_{
              {make_unique<RenderingPipeline>(), make_unique<RenderingPipeline>(), make_unique<RenderingPipeline>()}} {
    ...
    if (!parameters.environment_map.has_value() || parameters.environment_map->texture.index() == 0) {
...
void RenderEngineVtk::ImplementGeometry(const Capsule& capsule, void* user_data) {
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    ImplementPolyData(CreateVtkCapsule(capsule).GetPointer(), DefineMaterial(data.properties, default_diffuse_), data);
}
...
void RenderEngineVtk::ImplementGeometry(const Cylinder& cylinder, void* user_data) {
    vtkNew<vtkCylinderSource> vtk_cylinder;
    SetCylinderOptions(vtk_cylinder, cylinder.length(), cylinder.radius());
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    ImplementPolyData(TransformToDrakeCylinder(vtk_cylinder), DefineMaterial(data.properties, default_diffuse_), data);
}

void RenderEngineVtk::ImplementGeometry(const Ellipsoid& ellipsoid, void* user_data) {
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    ImplementPolyData(CreateVtkEllipsoid(ellipsoid).GetPointer(), DefineMaterial(data.properties, default_diffuse_),
                      data);
}
...
void RenderEngineVtk::ImplementGeometry(const HalfSpace&, void* user_data) {
    ...
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    ImplementPolyData(vtk_plane.GetPointer(), DefineMaterial(data.properties, default_diffuse_), data);
}
...
void RenderEngineVtk::ImplementGeometry(const Sphere& sphere, void* user_data) {
    ...
    const RegistrationData& data = *static_cast<RegistrationData*>(user_data);
    ImplementPolyData(vtk_sphere.GetPointer(), DefineMaterial(data.properties, default_diffuse_), data);
}

bool RenderEngineVtk::DoRegisterVisual(GeometryId id,
                                       const Shape& shape,
...
void RenderEngineVtk::DoUpdateVisualPose(GeometryId id, const RigidTransformd& X_WG) {
...
ColorRenderCamera MakeShadowCamera(const ColorRenderCamera& camera, bool active_shadow) {
    if (!active_shadow) {
        return camera;
    }
    ...
    return ColorRenderCamera{{core.renderer_name(),
                              {size, size, intrinsics.focal_x(), intrinsics.focal_y(), intrinsics.center_x(),
                               intrinsics.center_y() + y_offset},
                              core.clipping(),
...
void ExtractImage(const ColorRenderCamera& vtk_camera, vtkImageExport* exporter, ImageRgba8U* color_image_out) {
    ...
    // the output image.
    const int vtk_row_bytes = vtk_camera.core().intrinsics().width() * color_image_out->kPixelSize;
    auto* vtk_ptr = static_cast<ImageRgba8U::T*>(exporter->GetPointerToData());

    const int out_row_bytes = color_image_out->width() * color_image_out->kPixelSize;
    ...
    ImageRgba8U::T* out_ptr = color_image_out->at(0, 0) + start_out_row * out_row_bytes;
...
void RenderEngineVtk::DoRenderColorImage(const ColorRenderCamera& camera, ImageRgba8U* color_image_out) const {
    const ColorRenderCamera shadow_camera = MakeShadowCamera(camera, parameters_.cast_shadows);

    UpdateWindow(shadow_camera.core(), shadow_camera.show_window(), *pipelines_[ImageType::kColor], "Color Image");
    ...
    ExtractImage(shadow_camera, pipelines_[ImageType::kColor]->exporter, color_image_out);
}

void RenderEngineVtk::DoRenderDepthImage(const DepthRenderCamera& camera, ImageDepth32F* depth_image_out) const {
    ...
    for (int v = 0; v < intrinsics.height(); ++v) {
        for (int u = 0; u < intrinsics.width(); ++u) {
            if (image.at(u, v)[0] == 255u && image.at(u, v)[1] == 255u && image.at(u, v)[2] == 255u) {
                depth_image_out->at(u, v)[0] = ImageTraits<PixelType::kDepth32F>::kTooFar;
            } else {
                // Decoding three channel color values to a float value. For the detail,
                // see depth_shaders.h.
                float shader_value = image.at(u, v)[0] + image.at(u, v)[1] / 255. + image.at(u, v)[2] / (255. * 255.);

                // Dividing by 255 so that the range gets to be [0, 1].
                shader_value /= 255.f;
                // TODO(kunimatsu-tri) Calculate this in a vertex shader.
                depth_image_out->at(u, v)[0] = CheckRangeAndConvertToMeters(shader_value, min_depth, max_depth);
            }
        }
    }
}

void RenderEngineVtk::DoRenderLabelImage(const ColorRenderCamera& camera, ImageLabel16I* label_image_out) const {
    UpdateWindow(camera.core(), camera.show_window(), *pipelines_[ImageType::kLabel], "Label Image");
    ...
    for (int v = 0; v < intrinsics.height(); ++v) {
        for (int u = 0; u < intrinsics.width(); ++u) {
            label_image_out->at(u, v)[0] =
                    RenderEngine::MakeLabelFromRgb(image.at(u, v)[0], image.at(u, v)[1], image.at(u, v)[2]);
        }
    }
}
...
RenderEngineVtk::RenderEngineVtk(const RenderEngineVtk& other)
    ...
    for (const auto& [id, source_props] : other.props_) {
        ...
        for (int i = 0; i < kNumPipelines; ++i) {
            ...
            for (const auto& source_part : source_prop.parts) {
                ...
                target_prop.parts.push_back(Part{.actor = std::move(target_actor), .T_GA = source_part.T_GA});
            }
        }
    ...
    for (int p = 0; p < kNumPipelines; ++p) {
        copy_cameras(other.pipelines_.at(p)->renderer.Get(), pipelines_.at(p)->renderer.Get());
    }
}

void RenderEngineVtk::ImplementRenderMesh(RenderMesh&& mesh, double scale, const RegistrationData& data) {
    const RenderMaterial material = mesh.material.has_value() ? *mesh.material : MakeDiffuseMaterial(default_diffuse_);

    vtkSmartPointer<vtkPolyDataAlgorithm> mesh_source = CreateVtkMesh(std::move(mesh));
...
bool RenderEngineVtk::ImplementObj(const std::string& file_name, double scale, const RegistrationData& data) {
    std::vector<RenderMesh> meshes = LoadRenderMeshesFromObj(file_name, data.properties, default_diffuse_, diagnostic_);
...
bool RenderEngineVtk::ImplementGltf(const std::string& file_name, double scale, const RegistrationData& data) {
    ...
    vtkSmartPointer<vtkTransform> T_GF_transform = ConvertToVtkTransform(X_GF, scale);
    vtkMatrix4x4* T_GF = T_GF_transform->GetMatrix();

    // Color.
    if (data.properties.HasProperty("phong", "diffuse") || data.properties.HasProperty("phong", "diffuse_map")) {
        log()->warn(
        ...
        while (vtkActor* source_actor = actors->GetNextActor()) {
            ...
            if (i == ImageType::kColor) {
                ...
                if (i == ImageType::kLabel) {
                    ...
                    mapper->AddObserver(vtkCommand::UpdateShaderEvent, uniform_setting_callback_.Get());
                }
            }
            ...
            prop.parts.push_back({.actor = std::move(part_actor), .T_GA = std::move(T_GA)});
        }
    }
...
vtkSmartPointer<vtkLight> MakeVtkLight(const LightParameter& light_param) {
    ...
    if (light_param.frame == "camera") {
        ...
        const Vector3d p_WT = light_param.position + light_param.direction.normalized();
...
void RenderEngineVtk::HandleWarning(const DiagnosticDetail& detail) const {
    static const never_destroyed<std::regex> gltf_ext_regex{R"""(glTF extension ([^ ]*) .* will be ignored.)"""};
    std::smatch match;
...
void RenderEngineVtk::InitializePipelines() {
    const vtkSmartPointer<vtkTransform> vtk_identity = ConvertToVtkTransform(RigidTransformd::Identity());
    ...
    pipelines_[ImageType::kLabel]->renderer->SetBackground(empty_color.r(), empty_color.g(), empty_color.b());

    vtkOpenGLRenderer* renderer = vtkOpenGLRenderer::SafeDownCast(pipelines_[ImageType::kColor]->renderer);
    renderer->SetUseDepthPeeling(1);
    renderer->SetBackground(default_clear_color_.r(), default_clear_color_.g(), default_clear_color_.b());
    ...
    if (parameters_.environment_map.has_value()) {
        ...
        const std::string& path = std::get<EquirectangularMap>(parameters_.environment_map->texture).path;
    ...
    if (parameters_.cast_shadows) {
        ...
        shadows->GetShadowMapBakerPass()->SetResolution(parameters_.shadow_map_size);
    ...
    if (parameters_.exposure.has_value()) {
        ...
        tone_mapping_pass->SetExposure(static_cast<float>(parameters_.exposure.value()));
...
void RenderEngineVtk::ImplementPolyData(vtkPolyDataAlgorithm* source,
    ...
    std::array<vtkSmartPointer<vtkActor>, kNumPipelines> actors{
            vtkSmartPointer<vtkActor>::New(), vtkSmartPointer<vtkActor>::New(), vtkSmartPointer<vtkActor>::New()};
    ...
    vtkOpenGLShaderProperty* shader_prop =
            vtkOpenGLShaderProperty::SafeDownCast(actors[ImageType::kDepth]->GetShaderProperty());
    DRAKE_DEMAND(shader_prop != nullptr);
    shader_prop->SetVertexShaderCode(render::shaders::kDepthVS);
    shader_prop->SetFragmentShaderCode(render::shaders::kDepthFS);
    mappers[ImageType::kDepth]->AddObserver(vtkCommand::UpdateShaderEvent, uniform_setting_callback_.Get());
    ...
    PropArray props;
    auto connect_actor = [this, &actors, &mappers, &props, &vtk_X_WG](ImageType image_type) {
    ...
    if (!material.diffuse_map.empty()) {
        ...
        // *scale* the image.
        const Vector2d uv_scale = data.properties.GetPropertyOrDefault("phong", "diffuse_scale", Vector2d{1, 1});
...
RenderEngineVtk::RenderingPipeline& RenderEngineVtk::get_mutable_pipeline(internal::ImageType image_type) const {
    DRAKE_DEMAND(image_type == ImageType::kColor || image_type == ImageType::kLabel || image_type == ImageType::kDepth);
        ...
        for (auto& [_, prop_array] : props_) {
            for (auto& part : prop_array[ImageType::kColor].parts) {
                ...
                if (part.actor->GetTexture() != nullptr &&
                    ...
                    texture->SetUseSRGBColorSpace(true);
                    part.actor->GetProperty()->SetBaseColorTexture(part.actor->GetTexture());
                }
            }
        }
    }
...
void RenderEngineVtk::UpdateWindow(const DepthRenderCamera& camera, const RenderingPipeline& p) const {
    uniform_setting_callback_->set_z_near(static_cast<float>(camera.depth_range().min_depth()));
    uniform_setting_callback_->set_z_far(static_cast<float>(camera.depth_range().max_depth()));
