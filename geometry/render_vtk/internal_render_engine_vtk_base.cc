...
protected:
    DrakeCubeSource() : size_(1, 1, 1), uv_scale_(1, 1) { this->SetNumberOfInputPorts(0); }
        ...
        */
        ...
        for (int face = 0; face < 6; ++face) {
            ...
            for (const auto& [x, y] : vertices) {
                ...
                const double uv[] = {(x + 1) * 0.5 * uv_scale_[0], (y + 1) * 0.5 * uv_scale_[1]};
        ...
        vtkPolyData* output = vtkPolyData::SafeDownCast(out_info->Get(vtkDataObject::DATA_OBJECT()));
...
protected:
    ...
    int RequestData(vtkInformation*, vtkInformationVector**, vtkInformationVector* outputVector) override {
        ...
        for (int p = 0; p < num_points; ++p) {
            newPoints->InsertNextPoint(mesh_.positions(p, 0), mesh_.positions(p, 1), mesh_.positions(p, 2));
            const double n[] = {mesh_.normals(p, 0), mesh_.normals(p, 1), mesh_.normals(p, 2)};
...
vtkSmartPointer<vtkPolyDataAlgorithm> CreateVtkBox(const Box& box, const PerceptionProperties& properties) {
    vtkSmartPointer<DrakeCubeSource> vtk_box = vtkSmartPointer<DrakeCubeSource>::New();
    vtk_box->set_size({box.width(), box.depth(), box.height()});
    const Vector2d& uv_scale = properties.GetPropertyOrDefault("phong", "diffuse_scale", Vector2d{1, 1});
...
vtkSmartPointer<vtkPolyDataAlgorithm> CreateVtkMesh(RenderMesh mesh_data) {
    vtkSmartPointer<DrakeObjSource> mesh_source = vtkSmartPointer<DrakeObjSource>::New();
...
vtkSmartPointer<vtkPolyDataAlgorithm> TransformToDrakeCylinder(vtkCylinderSource* vtk_cylinder) {
