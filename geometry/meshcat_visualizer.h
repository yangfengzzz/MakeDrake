...
#include "systems/analysis/realtime_rate_calculator.h"
...
public:
    ...
    explicit MeshcatVisualizer(std::shared_ptr<Meshcat> meshcat, MeshcatVisualizerParams params = {});
    ...
    /** Adds a MeshcatVisualizer and connects it to the given SceneGraph's
     ...
     another system. */
    static MeshcatVisualizer<T>& AddToBuilder(systems::DiagramBuilder<T>* builder,
...
private:
     ...
     */
    void SetTransforms(const systems::Context<T>& context, const QueryObject<T>& query_object) const;
