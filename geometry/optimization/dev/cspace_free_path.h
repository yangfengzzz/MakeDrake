...
#include "geometry/optimization/cspace_separating_plane.h"
...
struct PlaneSeparatesGeometriesOnPath {
    PlaneSeparatesGeometriesOnPath(const PlaneSeparatesGeometries& plane_geometries,
    ...
    std::list<ParametrizedPolynomialPositiveOnUnitInterval> positive_side_conditions;
    std::list<ParametrizedPolynomialPositiveOnUnitInterval> negative_side_conditions;
    int plane_index;
};
...
public:
    ...
    struct SeparationCertificateProgram final : SeparationCertificateProgramBase {
        ...
        SeparationCertificateProgram(const std::unordered_map<symbolic::Variable, symbolic::Polynomial> m_path,
                                     int m_plane_index)
    ...
    [[nodiscard]] const Vector3<symbolic::Variable>& y_slack() const { return y_slack_; }
    ...
    /**
     ...
     geometry 2.
     */
    [[nodiscard]] const std::unordered_map<SortedPair<geometry::GeometryId>, int>& map_geometries_to_separating_planes()
            const {
        return map_geometries_to_separating_planes_;
    }

    [[nodiscard]] const std::vector<CSpaceSeparatingPlane<symbolic::Variable>>& separating_planes() const {
     ...
     */
    [[nodiscard]] SeparationCertificateProgram MakeIsGeometrySeparableOnPathProgram(
...
private:
    ...
    */
    void GeneratePathRationals(const std::vector<PlaneSeparatesGeometries>& plane_geometries);
    ...
    std::map<multibody::BodyIndex, std::vector<std::unique_ptr<CIrisCollisionGeometry>>> link_geometries_;
    ...
    std::unordered_map<SortedPair<geometry::GeometryId>, int> map_geometries_to_separating_planes_;
    ...
    // lambda.
    friend std::unordered_map<symbolic::Variable, symbolic::Polynomial> initialize_path_map(
