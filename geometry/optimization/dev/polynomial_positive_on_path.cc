...
ParametrizedPolynomialPositiveOnUnitInterval::ParametrizedPolynomialPositiveOnUnitInterval(
        const symbolic::Polynomial& poly,
        const symbolic::Variable& interval_variable,
        const symbolic::Variables& parameters)
    : mu_(interval_variable), poly_(poly), p_(poly), parameters_(parameters), psatz_variables_and_psd_constraints_() {
    ...
    for (const auto& var : poly.decision_variables()) {
        ...
        if (!parameters_.include(var)) {
            psatz_variables_and_psd_constraints_.AddDecisionVariables(solvers::VectorDecisionVariable<1>(var));
        }
    }
    ...
    if (poly.TotalDegree() == 0) {
        ...
        psatz_variables_and_psd_constraints_.AddBoundingBoxConstraint(0, std::numeric_limits<double>::infinity(),
                                                                      lambda);
        ...
        const int num_y_indets = poly.indeterminates().size() - (poly.Degree(mu_) > 0 ? 1 : 0);
        ...
        // to poly(μ,y).
        auto [lambda, Q_lambda] = psatz_variables_and_psd_constraints_.NewSosPolynomial(multiplier_basis_d, type, "Sl");
        ...
        if (deg == 0) {
            ...
            auto [nu, Q_nu] = psatz_variables_and_psd_constraints_.NewSosPolynomial(
                    multiplier_basis_d.tail(multiplier_basis_d.size() - 1),  // exclude μᵈ monomial
                    type, "Sv");
            nu_ = std::move(nu);
            p_ -= lambda_ + nu_ * symbolic::Polynomial(mu_, {mu_}) * (symbolic::Polynomial(1 - mu_, {mu_}));
        } else {
            auto [nu, Q_nu] = psatz_variables_and_psd_constraints_.NewSosPolynomial(multiplier_basis_d, type, "Sv");
            nu_ = std::move(nu);
...
void ParametrizedPolynomialPositiveOnUnitInterval::AddPositivityConstraintToProgram(
        const symbolic::Environment& env, solvers::MathematicalProgram* prog) const {
    DRAKE_DEMAND(env.size() == parameters_.size());
    for (const auto& parameter : parameters_) {
        DRAKE_DEMAND(env.find(parameter) != env.cend());
    }
    for (int i = 0; i < psatz_variables_and_psd_constraints_.indeterminates().size(); ++i) {
    ...
    prog->AddDecisionVariables(psatz_variables_and_psd_constraints_.decision_variables());
    for (const auto& binding : psatz_variables_and_psd_constraints_.GetAllConstraints()) {
