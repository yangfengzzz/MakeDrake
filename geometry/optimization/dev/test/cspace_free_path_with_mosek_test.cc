...
VectorX<Polynomiald> MakeBezierCurvePolynomialPath(
    ...
    MatrixX<symbolic::Expression> bezier_path_expr = path.GetExpression(symbolic::Variable("t"));
...
// aᵀx+b=0}
void CheckSeparationBySamples(const CspaceFreePathTester& tester,
    ...
    auto& scene_graph_context = scene_graph.GetMyMutableContextFromRoot(diagram_context.get());
    std::unique_ptr<AbstractValue> state_value = scene_graph.get_query_output_port().Allocate();
    ...
    for (int i = 0; i < mu_samples.rows(); ++i) {
        ...
        const Eigen::VectorXd q_val = tester.get_rational_forward_kin()->ComputeQValue(s, q_star);
        plant.SetPositions(&plant_context, q_val);
        const QueryObject<double>& query_object = state_value->get_value<QueryObject<double>>();
        scene_graph.get_query_output_port().Calc(scene_graph_context, state_value.get());
        EXPECT_FALSE(query_object.HasCollisions());
        for (int plane_index = 0; plane_index < static_cast<int>(tester.get_separating_planes().size());
             ++plane_index) {
...
void CheckForCollisionAlongPath(const CspaceFreePathTester& tester,
                                const systems::Diagram<double>& diagram,
                                const Eigen::Ref<const Eigen::VectorXd>& mu_samples,
                                const Eigen::Ref<const VectorX<Polynomiald>>& path,
                                const Eigen::Ref<const Eigen::VectorXd>& q_star) {
...
TEST_F(CIrisToyRobotTest, MakeAndSolveIsGeometrySeparableOnPathProgram) {
  ...
  C_good << 1, 1, 0,
    ...
    for (const int maximum_path_degree : {1, 4}) {
        CspaceFreePathTester tester(plant_, scene_graph_, q_star, maximum_path_degree, plane_order);
        ...
        for (int bezier_curve_order = 1; bezier_curve_order <= maximum_path_degree; ++bezier_curve_order) {
            ...
            VectorX<Polynomiald> bezier_poly_path_safe =
                    MakeBezierCurvePolynomialPath(s0_safe, s_end_safe, bezier_curve_order, c_free_polyhedron);
            auto separation_certificate_program = tester.cspace_free_path().MakeIsGeometrySeparableOnPathProgram(
                    geometry_pair, bezier_poly_path_safe);
            auto separation_certificate_result = tester.cspace_free_path().SolveSeparationCertificateProgram(
            ...
            VectorX<Polynomiald> bezier_poly_path_unsafe =
                    MakeBezierCurvePolynomialPath(s0_unsafe, s_end_unsafe, bezier_curve_order);
            auto separation_certificate_program2 = tester.cspace_free_path().MakeIsGeometrySeparableOnPathProgram(
                    geometry_pair, bezier_poly_path_unsafe);

            auto separation_certificate_result2 = tester.cspace_free_path().SolveSeparationCertificateProgram(
            ...
            if (!separation_certificate_result2.result.is_success()) {
                CheckForCollisionAlongPath(tester, *diagram_, mu_samples, bezier_poly_path_unsafe, q_star);
            }
        }
    }
}
