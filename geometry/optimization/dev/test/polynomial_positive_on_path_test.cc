#include "geometry/optimization/dev/polynomial_positive_on_path.h"
...
void validate_parametrized_polynomial_prog(const ParametrizedPolynomialPositiveOnUnitInterval& poly_prog,
                                           const Polynomial& poly,
                                           const int num_psd,
                                           const int num_bounding_box = 0) {
    ...
    const MathematicalProgram& psatz_variables_and_constraints{poly_prog.get_psatz_variables_and_psd_constraints()};

    for (const auto& var : poly.indeterminates()) {
        unused(var);
        // All the indeterminates of poly should be in the program.
        EXPECT_NO_THROW(unused(psatz_variables_and_constraints.FindIndeterminateIndex(var)));
    }
    ...
    for (const auto& var : p.decision_variables()) {
        if (poly_prog.get_parameters().include(var)) {
            ...
            EXPECT_THROW(unused(psatz_variables_and_constraints.FindDecisionVariableIndex(var)), std::runtime_error);

        } else {
            // All other decision variables of p_ should be in
            // psatz_variables_and_constraints.
            EXPECT_NO_THROW(unused(psatz_variables_and_constraints.FindDecisionVariableIndex(var)));
        }
    }

    EXPECT_EQ(psatz_variables_and_constraints.positive_semidefinite_constraints().size(), num_psd);
    EXPECT_EQ(psatz_variables_and_constraints.bounding_box_constraints().size(), num_bounding_box);
    EXPECT_EQ(psatz_variables_and_constraints.GetAllConstraints().size(), num_psd + num_bounding_box);
}
...
public:
    AddPositivityConstraintToProgramTest()
        : prog_{},
          ...
          // Add a cost
          cost_(prog_.AddLinearCost(initial_decision_variables_(0) + initial_decision_variables_(1) +
                                    initial_decision_variables_(2))),
          // Add a constraint
          initial_constraint_(prog_.AddLinearConstraint(
                  initial_decision_variables_(0) + initial_decision_variables_(1) + initial_decision_variables_(2) >=
                  2)) {}

    void CheckAddPositivityConstraintToProgram(const symbolic::Environment& env,
        ...
        const int expected_num_decision_variables =
                ...
                poly_prog->get_psatz_variables_and_psd_constraints().decision_variables().size();
        const int expected_num_constraints =
                ...
                poly_prog->get_p().monomial_to_coefficient_map().size();  // number of constraints for coefficient
                                                                          // matching condition
        ...
        EXPECT_EQ(test_prog->decision_variables().size(), expected_num_decision_variables);
        ...
        EXPECT_EQ(test_prog->positive_semidefinite_constraints().size(),
                  poly_prog->get_psatz_variables_and_psd_constraints().positive_semidefinite_constraints().size());
        ...
        const Polynomial p_sol{poly_prog->get_p().EvaluatePartial(env_with_solution)};
...
GTEST_TEST(ParametrizedPolyPositive, CtorTotalDegreeZero) {
    ...
    const ParametrizedPolynomialPositiveOnUnitInterval poly_prog{poly, Variable(), parameters};
    ...
    validate_parametrized_polynomial_prog(poly_prog, poly, 0,
...
GTEST_TEST(ParametrizedPolyPositive, CtorIntervalDegreeZero) {
    ...
    const ParametrizedPolynomialPositiveOnUnitInterval poly_prog{poly, interval_variable, parameters};
    ...
    validate_parametrized_polynomial_prog(poly_prog, poly,
...
GTEST_TEST(ParametrizedPolyPositive, CtorScalarDegreeOdd) {
    const int deg{3};
    const Variable interval_variable{"mu"};
    const VectorX<symbolic::Monomial> basis{symbolic::MonomialBasis({interval_variable}, deg)};
    ...
    const Polynomial nu{poly_prog.get_nu()};
    const Polynomial p{poly_prog.get_p()};
    const Polynomial rhs_expected{symbolic::Polynomial(interval_variable) * lambda +
...
GTEST_TEST(ParametrizedPolyPositive, CtorScalarDegreeEven) {
    ...
    const Polynomial nu{poly_prog.get_nu()};
    const Polynomial p{poly_prog.get_p()};
    const Polynomial rhs_expected{lambda + symbolic::Polynomial(interval_variable * (1 - interval_variable)) * nu};
...
GTEST_TEST(ParametrizedPolyPositive, CtorMatrix2DegreeOdd) {
    ...
    const Polynomial poly{poly_mat_upper(0) * y0 * y0 + 2 * poly_mat_upper(1) * y0 * y1 + poly_mat_upper(2) * y1 * y1,
...
TEST_F(AddPositivityConstraintToProgramTest, AddPositivityConstraintTotalDegreeZero) {
    const Variable coeff{"coeff"};
    const Variables parameters{coeff};
    // A polynomial with no indeterminates.
    const Polynomial poly{coeff * (*parameters.begin()), Variables()};
...
TEST_F(AddPositivityConstraintToProgramTest, AddPositivityConstraintScalarPolynomial) {
    const int deg{3};
    const Variable interval_variable{"mu"};
    const VectorX<symbolic::Monomial> basis{symbolic::MonomialBasis({interval_variable}, deg)};
    Polynomial::MapType poly_map;
...
TEST_F(AddPositivityConstraintToProgramTest, AddPositivityConstraintMatrixSOSPolynomial) {
    const int deg{4};
    const Variable interval_variable{"mu"};
    const Variable y0{"y0"};
    const Variable y1{"y1"};
