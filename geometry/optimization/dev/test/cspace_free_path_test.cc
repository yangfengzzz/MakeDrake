#include "geometry/optimization/dev/cspace_free_path.h"

#include <gtest/gtest.h>

#include "common/test_utilities/symbolic_test_util.h"
#include "geometry/optimization/cspace_free_internal.h"
#include "geometry/optimization/dev/test/c_iris_path_test_utilities.h"
...
TEST_F(CIrisToyRobotTest, CspaceFreePathConstructor) {
    ...
    for (int maximum_path_degree = 1; maximum_path_degree < 3; ++maximum_path_degree) {
        for (int plane_degree = 1; plane_degree < 3; ++plane_degree) {
            CspaceFreePathTester tester(plant_, scene_graph_, q_star, maximum_path_degree, plane_degree);
            ...
            VectorX<symbolic::Variable> s_vars{tester.get_rational_forward_kin()->s()};
            for (int i = 0; i < s_vars.size(); ++i) {
                EXPECT_TRUE(tester.get_path().contains(s_vars(i)));

                const symbolic::Polynomial& path_component{tester.get_path().at(s_vars(i))};
                EXPECT_EQ(path_component.indeterminates().size(), 1);
                EXPECT_EQ((*path_component.indeterminates().cbegin()), tester.get_mu());
                EXPECT_EQ(static_cast<unsigned int>(path_component.TotalDegree()), maximum_path_degree);
                EXPECT_EQ(path_component.monomial_to_coefficient_map().size(), maximum_path_degree + 1);
            }
            ...
            num_planes_expected +=
                    link_geometries.at(body_indices_[0]).size() * link_geometries.at(body_indices_[2]).size();
            num_planes_expected +=
                    link_geometries.at(body_indices_[1]).size() * link_geometries.at(body_indices_[3]).size();
            num_planes_expected +=
                    link_geometries.at(body_indices_[2]).size() * link_geometries.at(body_indices_[3]).size();
            ...
            for (const auto& [geometry_pair, plane_index] : dut.map_geometries_to_separating_planes()) {
                ...
                EXPECT_EQ(plane.expressed_body, multibody::internal::FindBodyInTheMiddleOfChain(
                ...
                for (int i = 0; i < 3; ++i) {
                    ...
                    EXPECT_EQ(plane.a(i).indeterminates(), symbolic::Variables({dut.mu()}));
                }
...
bool PolyEqualBySameRandomEval(symbolic::Polynomial p1, symbolic::Polynomial p2, double value, double tol) {
...
TEST_F(CIrisToyRobotTest, CspaceFreePathGeneratePathRationalsTest) {
    ...
    const CspaceFreePath& dut = tester.cspace_free_path();
    EXPECT_EQ(tester.get_path_plane_geometries().size(), dut.separating_planes().size());
    ...
    internal::GenerateRationals(separating_planes_ptrs, dut.y_slack(), q_star, *(tester.get_rational_forward_kin()),
                                &plane_geometries_in_s);

    EXPECT_EQ(plane_geometries_in_s.size(), tester.get_path_plane_geometries().size());
    ...
    for (const auto& plane_geometry : tester.get_path_plane_geometries()) {
        ...
        if (plane.positive_side_geometry->type() == CIrisGeometryType::kPolytope &&
            ...
            EXPECT_EQ(plane_geometry.positive_side_conditions.size(), plane.positive_side_geometry->num_rationals());
            EXPECT_EQ(plane_geometry.negative_side_conditions.size(), plane.negative_side_geometry->num_rationals());
        } else if (plane.positive_side_geometry->type() == CIrisGeometryType::kPolytope &&
                   plane.negative_side_geometry->type() != CIrisGeometryType::kPolytope) {
            EXPECT_EQ(plane_geometry.positive_side_conditions.size(), plane.positive_side_geometry->num_rationals());
        ...
        const auto& plane_geometry_s = plane_geometries_in_s.at(plane_geometry.plane_index);

        const auto& positive_side =
                std::tie(plane_geometry.positive_side_conditions, plane_geometry_s.positive_side_rationals);
        const auto& negative_side =
                std::tie(plane_geometry.negative_side_conditions, plane_geometry_s.negative_side_rationals);

        for (const auto& [path_conditions, polytope_rationals] : {positive_side, negative_side}) {
            ...
            for (const auto& path_rational : path_conditions) {
                ...
                const symbolic::Polynomial& polytope_condition{polytope_rationals.at(i).numerator()};
                ++i;

                // Gets the y_slacks needed to implement this matrix SOS condition.
                const symbolic::Variables path_y_slack = intersect(y_slack, path_condition.indeterminates());
                const symbolic::Variables polytope_y_slack = intersect(y_slack, polytope_condition.indeterminates());
                ...
                auto compute_total_s_degree = [&tester](const symbolic::Polynomial& poly) {
                    int degree{0};
                    for (const auto& [m, c] : poly.monomial_to_coefficient_map()) {
                        int cur_s_degree{0};
                        for (int k = 0; k < tester.get_rational_forward_kin()->s().size(); ++k) {
                            cur_s_degree += m.degree(tester.get_rational_forward_kin()->s()(k));
                        }
                ...
                EXPECT_EQ(static_cast<unsigned int>(path_condition.Degree(tester.get_mu())),
                          maximum_path_degree * compute_total_s_degree(polytope_condition) + plane_degree);
                ...
                for (int j = 0; j < mu_test_values.rows(); ++j) {
                    ...
                    for (int k = 0; k < 10; ++k) {
                        EXPECT_TRUE(PolyEqualBySameRandomEval(path_condition.EvaluatePartial(mu_env),
