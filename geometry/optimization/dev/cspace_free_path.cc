...
#include "geometry/optimization/cspace_free_structs.h"
#include "multibody/rational/rational_forward_kinematics_internal.h"

namespace drake {
namespace geometry {
namespace optimization {
std::unordered_map<symbolic::Variable, symbolic::Polynomial> initialize_path_map(
        CspaceFreePath* cspace_free_path,
    ...
    const VectorX<symbolic::Monomial> basis =
            symbolic::MonomialBasis(symbolic::Variables{cspace_free_path->mu_}, maximum_path_degree);
...
PlaneSeparatesGeometriesOnPath::PlaneSeparatesGeometriesOnPath(
        ...
        const std::unordered_map<symbolic::Variable, symbolic::Polynomial>& path_with_y_subs,
    ...
    auto substitute_and_create_condition = [this, &cached_substitutions, &path_with_y_subs, &indeterminates, &mu](
        ...
        symbolic::Polynomial path_numerator{
                rational.numerator().SubstituteAndExpand(path_with_y_subs, cached_substitutions)};

        // The current y_slacks along with mu.
        symbolic::Variables cur_indeterminates{intersect(indeterminates, rational.numerator().indeterminates())};
...
CspaceFreePath::CspaceFreePath(const multibody::MultibodyPlant<double>* plant,
    ...
    : rational_forward_kin_(plant),
      ...
      path_(initialize_path_map(this, maximum_path_degree, rational_forward_kin_.s())) {
    ...
    for (const auto& [link_pair, geometry_pairs] : collision_pairs) {
        for (const auto& geometry_pair : geometry_pairs) {
            ...
            CalcPlane<symbolic::Variable, symbolic::Variable, symbolic::Polynomial>(
                    plane_decision_vars, Vector1<symbolic::Variable>(mu_), plane_degree, &a, &b);
    ...
    internal::GenerateRationals(separating_planes_ptrs, y_slack_, q_star_, rational_forward_kin_, &plane_geometries);
    GeneratePathRationals(plane_geometries);
}

void CspaceFreePath::GeneratePathRationals(const std::vector<PlaneSeparatesGeometries>& plane_geometries) {
    ...
    // map.
    std::unordered_map<symbolic::Variable, symbolic::Polynomial> path_with_y_subs = path_;
    ...
    for (const auto& plane_geometry : plane_geometries) {
        plane_geometries_on_path_.emplace_back(plane_geometry, mu_, path_with_y_subs, indeterminates,
                                               &cached_substitutions);
    }
}

[[nodiscard]] CspaceFreePath::SeparationCertificateProgram CspaceFreePath::MakeIsGeometrySeparableOnPathProgram(
        const SortedPair<geometry::GeometryId>& geometry_pair, const VectorX<Polynomiald>& path) const {
    // Fail fast as building the program can be expensive.
    int plane_index{GetSeparatingPlaneIndex(geometry_pair)};
    if (plane_index < 0) {
        throw std::runtime_error(
                fmt::format("GetIsGeometrySeparableProgram(): geometry pair ({}, {}) does not need "
    ...
    for (int i = 0; i < path.rows(); ++i) {
        ...
        cspace_var_to_sym_path.emplace(rational_forward_kin_.s()(i), symbolic::Polynomial{sym_path_map});
    }

    return ConstructPlaneSearchProgramOnPath(plane_geometries_on_path_.at(plane_index), cspace_var_to_sym_path);
}

[[nodiscard]] CspaceFreePath::SeparationCertificateProgram CspaceFreePath::ConstructPlaneSearchProgramOnPath(
        const PlaneSeparatesGeometriesOnPath& plane_geometries_on_path,
        const std::unordered_map<symbolic::Variable, symbolic::Polynomial>& path) const {
    ...
    for (const auto& [config_space_var, eval_path] : path) {
        ...
        for (const auto& [mu_monom, mu_var_coeff] : symbolic_path.monomial_to_coefficient_map()) {
            ...
            param_eval_map.insert(*mu_var_coeff.GetVariables().begin(), mu_var_coeff_eval);
        }
    }
    ...
    for (const auto& condition : plane_geometries_on_path.positive_side_conditions) {
        condition.AddPositivityConstraintToProgram(param_eval_map, ret.prog.get_mutable());
    }
...
CspaceFreePath::SeparationCertificateResult CspaceFreePath::SolveSeparationCertificateProgram(
    ...
    internal::SolveSeparationCertificateProgramBase(certificate_program, options,
                                                    separating_planes_[certificate_program.plane_index], &result);
    return result;
}

int CspaceFreePath::GetSeparatingPlaneIndex(const SortedPair<geometry::GeometryId>& pair) const {
