...
#include "common/reset_after_move.h"
...
public:
    ...
    std::pair<VectorX<symbolic::Variable>, std::vector<solvers::Binding<solvers::Constraint>>> AddPointInSetConstraints(
            solvers::MathematicalProgram* prog, const Eigen::Ref<const solvers::VectorXDecisionVariable>& vars) const;
...
protected:
    ...
    virtual std::optional<bool> DoIsBoundedShortcut() const { return std::nullopt; }
    ...
    @pre ambient_dimension() > 0 */
    virtual std::pair<VectorX<symbolic::Variable>, std::vector<solvers::Binding<solvers::Constraint>>>
    ...
    @pre x.size() == ambient_dimension()
    @pre ambient_dimension() > 0 */
    virtual std::vector<solvers::Binding<solvers::Constraint>> DoAddPointInNonnegativeScalingConstraints(
            solvers::MathematicalProgram* prog,
            const Eigen::Ref<const solvers::VectorXDecisionVariable>& x,
...
private:
    ...
    @pre ambient_dimension() >= 0 */
    bool GenericDoPointInSet(const Eigen::Ref<const Eigen::VectorXd>& x, double tol) const;

    /** Generic implementation for Projection() -- applicable for all convex sets.
     @pre ambient_dimension() >= 0
     */
    std::optional<std::pair<std::vector<double>, Eigen::MatrixXd>> GenericDoProjection(
            const Eigen::Ref<const Eigen::MatrixXd>& point) const;
