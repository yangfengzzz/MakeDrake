...
std::pair<double, double> internal::GetMinimumAndMaximumValueAlongDimension(const ConvexSet& region, int dimension) {
    ...
    return internal::GetMinimumAndMaximumValueAlongDimension(region, std::vector<int>{dimension})[0];
}

std::vector<std::pair<double, double>> internal::GetMinimumAndMaximumValueAlongDimension(const ConvexSet& region,
                                                                                         std::vector<int> dimensions) {
    std::vector<std::pair<double, double>> bounds;
    MathematicalProgram prog;
    VectorXDecisionVariable y = prog.NewContinuousVariables(region.ambient_dimension());
...
VectorXd internal::ComputeOffsetContinuousRevoluteJoints(
    ...
    internal::ThrowsForInvalidContinuousJointsList(num_positions, continuous_revolute_joints);
    ...
    for (int i = 0; i < ssize(continuous_revolute_joints); ++i) {
        ...
        if (continuous_bbox_A.at(i).first < continuous_bbox_B.at(i).first) {
            ...
            offset(joint_index) =
                    2 * M_PI *
                    std::floor((continuous_bbox_B.at(i).second - continuous_bbox_A.at(i).first) / (2 * M_PI));
        } else {
            ...
            offset(joint_index) =
                    -2 * M_PI *
                    std::floor((continuous_bbox_A.at(i).second - continuous_bbox_B.at(i).first) / (2 * M_PI));
        }
    }
    return offset;
}

void internal::ThrowsForInvalidContinuousJointsList(int num_positions,
                                                    const std::vector<int>& continuous_revolute_joints) {
    for (int i = 0; i < ssize(continuous_revolute_joints); ++i) {
        // Make sure the unbounded revolute joints point to valid indices.
        if (continuous_revolute_joints[i] < 0 || continuous_revolute_joints[i] >= num_positions) {
            throw std::runtime_error(
                    fmt::format("Each joint index in continuous_revolute_joints must lie in the "
                                "interval [0, {}). Joint index {} (located at {}) violates this.",
                                num_positions, continuous_revolute_joints[i], i));
        }
    }
    std::unordered_set<int> comparison(continuous_revolute_joints.begin(), continuous_revolute_joints.end());
    if (comparison.size() != continuous_revolute_joints.size()) {
        throw std::runtime_error(fmt::format("continuous_revolute_joints must not contain duplicate entries."));
    }
}

bool CheckIfSatisfiesConvexityRadius(const ConvexSet& convex_set, const std::vector<int>& continuous_revolute_joints) {
    for (const int& j : continuous_revolute_joints) {
        auto [min_value, max_value] = internal::GetMinimumAndMaximumValueAlongDimension(convex_set, j);
...
ConvexSets PartitionConvexSet(const ConvexSet& convex_set,
                              const std::vector<int>& continuous_revolute_joints,
                              const double epsilon) {
    DRAKE_THROW_UNLESS(epsilon > 0);
    DRAKE_THROW_UNLESS(epsilon < M_PI);
    internal::ThrowsForInvalidContinuousJointsList(convex_set.ambient_dimension(), continuous_revolute_joints);
...
ConvexSets PartitionConvexSet(const ConvexSets& convex_sets,
                              const std::vector<int>& continuous_revolute_joints,
                              const double epsilon) {
    DRAKE_THROW_UNLESS(convex_sets.size() > 0);
    DRAKE_THROW_UNLESS(convex_sets[0] != nullptr);
    internal::ThrowsForInvalidContinuousJointsList(convex_sets[0]->ambient_dimension(), continuous_revolute_joints);
    ...
    for (int i = 1; i < ssize(convex_sets); ++i) {
        ...
        DRAKE_THROW_UNLESS(convex_sets[i]->ambient_dimension() == ambient_dimension);
    }

    ConvexSets sets;
    for (int i = 0; i < ssize(convex_sets); ++i) {
        ConvexSets new_sets = PartitionConvexSet(*convex_sets[i], continuous_revolute_joints, epsilon);
...
std::vector<std::tuple<int, int, Eigen::VectorXd>> CalcPairwiseIntersections(
    ...
    internal::ThrowsForInvalidContinuousJointsList(dimension, continuous_revolute_joints);
    ...
    std::vector<std::vector<std::pair<double, double>>> region_minimum_and_maximum_values_A;
    std::vector<std::vector<std::pair<double, double>>> region_minimum_and_maximum_values_B;

    // Compute bounding boxes for convex_sets_A.
    for (int i = 0; i < ssize(convex_sets_A); ++i) {
        region_minimum_and_maximum_values_A.emplace_back(std::vector<std::pair<double, double>>());
        for (const int k : continuous_revolute_joints) {
            region_minimum_and_maximum_values_A.at(i).emplace_back(
                    internal::GetMinimumAndMaximumValueAlongDimension(*convex_sets_A[i], k));
        }
    }
    ...
    if (!convex_sets_A_and_B_are_identical) {
        for (int i = 0; i < ssize(convex_sets_B); ++i) {
            region_minimum_and_maximum_values_B.emplace_back(std::vector<std::pair<double, double>>());
            for (const int k : continuous_revolute_joints) {
                region_minimum_and_maximum_values_B.at(i).emplace_back(
                        internal::GetMinimumAndMaximumValueAlongDimension(*convex_sets_B[i], k));
            }
        }
    }
    ...
    for (int i = 0; i < ssize(convex_sets_A); ++i) {
        for (int j = 0; j < ssize(convex_sets_B); ++j) {
            ...
            const auto& bbox_B = convex_sets_A_and_B_are_identical ? region_minimum_and_maximum_values_A.at(j)
            ...
            for (int k = 0; k < ssize(continuous_revolute_joints); ++k) {
                ...
                if (bbox_A.at(k).first < bbox_B.at(k).first) {
                    ...
                    // by taking that difference, dividing by 2π, and truncating.
                    offset(joint_index) =
                            2 * M_PI * std::floor((bbox_B.at(k).second - bbox_A.at(k).first) / (2 * M_PI));
                } else {
                    // In this case, the minimum value of convex_sets_B[j] along dimension
                    ...
                    // translation.
                    offset(joint_index) =
                            -2 * M_PI * std::floor((bbox_A.at(k).second - bbox_B.at(k).first) / (2 * M_PI));
                }
            }
...
std::vector<std::tuple<int, int, Eigen::VectorXd>> CalcPairwiseIntersections(
    ...
    return CalcPairwiseIntersections(convex_sets, convex_sets, continuous_revolute_joints);
}
