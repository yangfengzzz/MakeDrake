...
Hyperrectangle::Hyperrectangle(const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub)
    : ConvexSet(lb.size(), true), lb_(lb), ub_(ub) {
    CheckInvariants();
}

std::optional<Hyperrectangle> Hyperrectangle::MaybeCalcAxisAlignedBoundingBox(const ConvexSet& set) {
    if (!set.IsBounded()) {
        return std::nullopt;
    }
...
std::optional<bool> Hyperrectangle::DoPointInSetShortcut(const Eigen::Ref<const Eigen::VectorXd>& x, double tol) const {
    return (x.array() >= lb_.array() - tol).all() && (x.array() <= ub_.array() + tol).all();
}
...
std::vector<solvers::Binding<solvers::Constraint>> Hyperrectangle::DoAddPointInNonnegativeScalingConstraints(
        solvers::MathematicalProgram* prog,
    ...
    const auto infinity_vector = VectorXd::Constant(n_d, std::numeric_limits<double>::infinity());
    constraints.push_back(
            prog->AddLinearConstraint(A_con_lb, VectorXd::Zero(n_d), infinity_vector, {x, Vector1<Variable>(t)}));
    constraints.push_back(
            prog->AddLinearConstraint(A_con_ub, -infinity_vector, VectorXd::Zero(n_d), {x, Vector1<Variable>(t)}));
    return constraints;
}
...
std::vector<solvers::Binding<solvers::Constraint>> Hyperrectangle::DoAddPointInNonnegativeScalingConstraints(
    ...
    const auto infinity_vector = VectorXd::Constant(n_rows, std::numeric_limits<double>::infinity());
    constraints.push_back(prog->AddLinearConstraint(A_con_ub, -infinity_vector, -b + d * ub_, {x, t}));
    constraints.push_back(prog->AddLinearConstraint(A_con_lb, -b + d * lb_, infinity_vector, {x, t}));
    return constraints;
}

std::pair<VectorX<symbolic::Variable>, std::vector<solvers::Binding<solvers::Constraint>>>
Hyperrectangle::DoAddPointInSetConstraints(solvers::MathematicalProgram* prog,
...
std::optional<Hyperrectangle> Hyperrectangle::MaybeGetIntersection(const Hyperrectangle& other) const {
    DRAKE_THROW_UNLESS(this->ambient_dimension() == other.ambient_dimension());
    if ((lb_.array() > other.ub_.array()).any() || (ub_.array() < other.lb_.array()).any()) {
        return std::nullopt;
    }
    return Hyperrectangle(lb_.cwiseMax(other.lb_), ub_.cwiseMin(other.ub_));
}

std::pair<std::unique_ptr<Shape>, math::RigidTransformd> Hyperrectangle::DoToShapeWithPose() const {
    if (ambient_dimension() != 3) {
        throw std::runtime_error(
                "Hyperrectangle::DoToShapeWithPose() is only implemented for "
                "ambient_dimension() == 3");
    }
    return std::make_pair(std::make_unique<geometry::Box>(ub_ - lb_), math::RigidTransformd(Center()));
}
