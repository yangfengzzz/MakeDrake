...
void InitializeCoeffVects(const VectorX<T>& decision_variables,
                          ...
                          VectorX<T>* b_coeff) {
    static_assert(std::is_same_v<T, symbolic::Variable> || std::is_same_v<T, double>);
...
void MakeCalcPlanePolynomial(const VectorX<symbolic::Variable>& vars_for_plane,
    ...
    const Eigen::Matrix<symbolic::Monomial, Eigen::Dynamic, 1> basis =
            symbolic::MonomialBasis(symbolic::Variables{vars_for_plane}, plane_degree);
    for (int i = 0; i < 3; ++i) {
...
Eigen::VectorXd ComputeGradedRevLexEvaluatedPowers(const VectorX<double>& values, const int degree) {
...
void CalcPlaneImpl(const VectorX<T1>& decision_variables,
                   const VectorX<T2>& vars_for_plane,
                   int plane_degree,
                   Vector3<T3>* a_val,
                   T3* b_val) {
    const int num_vars = vars_for_plane.size();
    const int num_coeffs_per_poly = symbolic::NChooseK(num_vars + plane_degree, plane_degree);
    ...
    if constexpr (std::is_same_v<T3, symbolic::Polynomial>) {
        MakeCalcPlanePolynomial(vars_for_plane, plane_degree, a_coeff, b_coeff, a_val, b_val);
    } else {
        const Eigen::VectorXd evaluated_powers = ComputeGradedRevLexEvaluatedPowers(vars_for_plane, plane_degree);
...
SeparatingPlaneOrder ToPlaneOrder(int plane_degree) {
    if (plane_degree == 1) {
        ...
        throw std::runtime_error(
                fmt::format("ToPlaneOrder: plane_degree={}, only supports plane_degree = 1.", plane_degree));
    }
}
