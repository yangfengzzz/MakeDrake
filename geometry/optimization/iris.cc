#include "geometry/optimization/iris.h"
...
#include "geometry/optimization/affine_ball.h"
#include "geometry/optimization/cartesian_product.h"
#include "geometry/optimization/convex_set.h"
#include "geometry/optimization/iris_internal.h"
#include "geometry/optimization/minkowski_sum.h"
...
HPolyhedron Iris(const ConvexSets& obstacles,
                 const Ref<const VectorXd>& sample,
    ...
    Hyperellipsoid E = options.starting_ellipse.value_or(Hyperellipsoid::MakeHypersphere(kEpsilonEllipsoid, sample));
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> A(P.A().rows() + N, dim);
    ...
    while (true) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (HPolyhedron(A.topRows(num_constraints), b.head(num_constraints))
                ...
                A.row(num_constraints) = (tangent_matrix * (point - E.center())).normalized();
        ...
        if (options.require_sample_point_is_contained &&
            ((A.topRows(num_constraints) * sample).array() >= b.head(num_constraints).array()).any()) {
            break;
        }

        if (options.termination_func &&
            options.termination_func(HPolyhedron(A.topRows(num_constraints), b.head(num_constraints)))) {
            break;
        }
...
public:
    ...
    IrisConvexSetMaker(const QueryObject<double>& query, std::optional<FrameId> reference_frame)
...
ConvexSets MakeIrisObstacles(const QueryObject<double>& query_object, std::optional<FrameId> reference_frame) {
    const SceneGraphInspector<double>& inspector = query_object.inspector();
    const std::vector<GeometryId> geom_ids = inspector.GetAllGeometryIds(Role::kProximity);
...
public:
    ...
    void set(const Binding<Constraint>* binding_with_constraint_to_be_falsified, int index, bool falsify_lower_bound) {
        DRAKE_DEMAND(binding_with_constraint_to_be_falsified != nullptr);
        const int N = binding_with_constraint_to_be_falsified->evaluator()->num_constraints();
...
private:
    ...
    void DoEval(const Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        // MathematicalProgram::EvalBinding doesn't support symbolic, and we
        // shouldn't get here.
        throw std::logic_error("CounterExampleConstraint doesn't support DoEval for symbolic.");
    }
...
public:
    CounterExampleProgram(std::shared_ptr<CounterExampleConstraint> counter_example_constraint,
        ...
        P_constraint_ = prog_.AddLinearConstraint(
                A, VectorXd::Constant(b.size(), -std::numeric_limits<double>::infinity()), b, q_);
        ...
        const double scale = 1.0 / std::sqrt(es.eigenvalues().maxCoeff() * es.eigenvalues().minCoeff());
        prog_.AddQuadraticErrorCost(scale * Asq, E.center(), q_);

        prog_.AddConstraint(counter_example_constraint, q_);
    }

    void UpdatePolytope(const Eigen::Ref<const Eigen::MatrixXd>& A, const Eigen::Ref<const Eigen::VectorXd>& b) {
        P_constraint_->evaluator()->UpdateCoefficients(
                A, VectorXd::Constant(b.size(), -std::numeric_limits<double>::infinity()), b);
    }
...
// constraint.
void AddTangentToPolytope(const Hyperellipsoid& E,
    ...
    A->row(*num_constraints) = (E.A().transpose() * E.A() * (point - E.center())).normalized();
    (*b)[*num_constraints] = A->row(*num_constraints) * point - configuration_space_margin;
...
void MakeGuessFeasible(const HPolyhedron& P, Eigen::VectorXd* guess) {
    ...
    for (int i = M - 1; i >= 0; --i) {
        if (A.row(i) * *guess - b[i] > 0) {
            ...
            *guess -= A.row(i).normalized().transpose() * (A.row(i) * *guess - b[i] + kEps);
        }
    }
...
struct GeometryPairWithDistance {
    ...
    GeometryPairWithDistance(GeometryId gA, GeometryId gB, double dist) : geomA(gA), geomB(gB), distance(dist) {}

    bool operator<(const GeometryPairWithDistance& other) const { return distance < other.distance; }
};

bool CheckTerminate(const IrisOptions& options,
                    const HPolyhedron& P,
...
HPolyhedron IrisInConfigurationSpace(const MultibodyPlant<double>& plant,
    ...
    for (int i = 0; i < nc; ++i) {
        ...
        if (options.configuration_obstacles[i]->PointInSet(seed)) {
            throw std::runtime_error(fmt::format("The seed point is in configuration obstacle {}", i));
        }
    }
    ...
    HPolyhedron P = HPolyhedron::MakeBox(plant.GetPositionLowerLimits(), plant.GetPositionUpperLimits());
    ...
    const double kEpsilonEllipsoid = 1e-2;
    Hyperellipsoid E = options.starting_ellipse.value_or(Hyperellipsoid::MakeHypersphere(kEpsilonEllipsoid, seed));

    // Make all of the convex sets and supporting quantities.
    auto query_object = plant.get_geometry_query_input_port().Eval<QueryObject<double>>(context);
    const SceneGraphInspector<double>& inspector = query_object.inspector();
    ...
    auto same_point_constraint = std::make_shared<internal::SamePointConstraint>(&plant, context);
    std::map<std::pair<GeometryId, GeometryId>, std::vector<VectorXd>> counter_examples;
    ...
    for (const auto& [geomA, geomB] : pairs) {
        const double distance = query_object.ComputeSignedDistancePairClosestPoints(geomA, geomB).distance;
        if (distance < 0.0) {
            throw std::runtime_error(
    ...
    // size.
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> A(P.A().rows() + 2 * n + nc, nq);
    ...
    if (options.prog_with_additional_constraints) {
        counter_example_constraint =
                std::make_shared<CounterExampleConstraint>(options.prog_with_additional_constraints);
        additional_constraint_bindings = options.prog_with_additional_constraints->GetAllConstraints();
        // Fail fast if the seed point is infeasible.
        {
            if (!options.prog_with_additional_constraints->CheckSatisfied(additional_constraint_bindings, seed)) {
                throw std::runtime_error(
        ...
        auto AddConstraint = [&](const Eigen::MatrixXd& new_A, const Eigen::VectorXd& new_b,
            ...
            for (int i = 0; i < new_b.rows(); ++i) {
                if (!std::isinf(new_b[i])) {
                    ...
                    for (int j = 0; j < vars.rows(); ++j) {
                        const int index = options.prog_with_additional_constraints->FindDecisionVariableIndex(vars[j]);
        ...
        auto HandleLinearConstraints = [&](const auto& bindings) {
            for (const auto& binding : bindings) {
                AddConstraint(binding.evaluator()->get_sparse_A(), binding.evaluator()->upper_bound(),
                              binding.variables());
                AddConstraint(-binding.evaluator()->get_sparse_A(), -binding.evaluator()->lower_bound(),
                              binding.variables());
                auto pos = std::find(additional_constraint_bindings.begin(), additional_constraint_bindings.end(),
                                     binding);
                DRAKE_ASSERT(pos != additional_constraint_bindings.end());
                additional_constraint_bindings.erase(pos);
            }
        };
        HandleLinearConstraints(options.prog_with_additional_constraints->bounding_box_constraints());
        HandleLinearConstraints(options.prog_with_additional_constraints->linear_constraints());
        counter_example_prog = std::make_unique<CounterExampleProgram>(
                counter_example_constraint, E, A.topRows(num_initial_constraints), b.head(num_initial_constraints));

        P = HPolyhedron(A.topRows(num_initial_constraints), b.head(num_initial_constraints));
    }
    ...
    auto solver = solvers::MakeFirstAvailableSolver({solvers::SnoptSolver::id(), solvers::IpoptSolver::id()});
    ...
    while (true) {
        ...
        int num_constraints = num_initial_constraints;
        HPolyhedron P_candidate = HPolyhedron(A.topRows(num_initial_constraints), b.head(num_initial_constraints));
        DRAKE_ASSERT(best_volume > 0);
        // Find separating hyperplanes
        ...
        if (options.configuration_obstacles.size() > 0) {
            ...
            for (int i = 0; i < nc; ++i) {
                ...
                if (HPolyhedron(A.topRows(num_constraints), b.head(num_constraints))
                    ...
                    if (options.require_sample_point_is_contained) {
                        const bool seed_point_requirement = A.row(num_constraints - 1) * seed <= b(num_constraints - 1);
                    ...
                    if (CheckTerminate(options, HPolyhedron(A.topRows(num_constraints), b.head(num_constraints)),
                                       termination_error_msg, termination_msg, iteration == 0)) {
                        return P;
                    }
                }
            }

            P_candidate = HPolyhedron(A.topRows(num_constraints), b.head(num_constraints));
        ...
        for (const auto& pair_w_distance : sorted_pairs) {
            std::pair<GeometryId, GeometryId> geom_pair(pair_w_distance.geomA, pair_w_distance.geomB);
            int consecutive_failures = 0;
            internal::ClosestCollisionProgram prog(same_point_constraint, *frames.at(pair_w_distance.geomA),
            ...
            std::vector<VectorXd> prev_counter_examples = std::move(counter_examples[geom_pair]);
            ...
            std::sort(prev_counter_examples.begin(), prev_counter_examples.end(),
                      [&E](const VectorXd& x, const VectorXd& y) {
                          return (E.A() * x - E.center()).squaredNorm() < (E.A() * y - E.center()).squaredNorm();
                      });
            ...
            while (consecutive_failures < options.num_collision_infeasible_samples) {
                ...
                if (counter_example_searches_for_this_pair < ssize(prev_counter_examples)) {
                    guess = prev_counter_examples[counter_example_searches_for_this_pair];
                } else {
                    MakeGuessFeasible(P_candidate, &guess);
                    guess = P_candidate.UniformSample(&generator, guess, options.mixing_steps);
                }
                ++counter_example_searches_for_this_pair;
                if (do_debugging_visualization) {
                    ++num_points_drawn;
                    point_to_draw.head(nq) = guess;
                    std::string path = fmt::format("iteration{:02}/{:03}/guess", iteration, num_points_drawn);
                    options.meshcat->SetObject(path, Sphere(0.01), geometry::Rgba(0.1, 0.1, 0.1, 1.0));
                ...
                if (prog.Solve(*solver, guess, options.solver_options, &closest)) {
                    if (do_debugging_visualization) {
                        ...
                        std::string path = fmt::format("iteration{:02}/{:03}/found", iteration, num_points_drawn);
                        options.meshcat->SetObject(path, Sphere(0.01), geometry::Rgba(0.8, 0.1, 0.8, 1.0));
                        options.meshcat->SetTransform(path, RigidTransform<double>(point_to_draw));
                    }
                    consecutive_failures = 0;
                    new_counter_examples.emplace_back(closest);
                    AddTangentToPolytope(E, closest, options.configuration_space_margin, &A, &b, &num_constraints);
                    P_candidate = HPolyhedron(A.topRows(num_constraints), b.head(num_constraints));
                    MakeGuessFeasible(P_candidate, &guess);
                    if (options.require_sample_point_is_contained) {
                        const bool seed_point_requirement = A.row(num_constraints - 1) * seed <= b(num_constraints - 1);
                    ...
                    if (CheckTerminate(options, P_candidate, termination_error_msg, termination_msg, iteration == 0)) {
                        return P;
                    }
                    prog.UpdatePolytope(A.topRows(num_constraints), b.head(num_constraints));
                } else {
                    if (do_debugging_visualization) {
                        point_to_draw.head(nq) = closest;
                        std::string path = fmt::format("iteration{:02}/{:03}/closest", iteration, num_points_drawn);
                        options.meshcat->SetObject(path, Sphere(0.01), geometry::Rgba(0.1, 0.8, 0.8, 1.0));
                        options.meshcat->SetTransform(path, RigidTransform<double>(point_to_draw));
                    }
                    if (counter_example_searches_for_this_pair > ssize(counter_examples[geom_pair])) {
                        // Only count the failures once we start the random guesses.
                        ++consecutive_failures;
                    }
                }
                if (!warned_many_searches &&
                    counter_example_searches_for_this_pair - ssize(counter_examples[geom_pair]) >=
                    ...
                    log()->info(
                            ...
                            inspector.GetName(pair_w_distance.geomA), inspector.GetName(pair_w_distance.geomB),
                            counter_example_searches_for_this_pair);
                }
            }
        ...
        if (options.prog_with_additional_constraints) {
            counter_example_prog->UpdatePolytope(A.topRows(num_constraints), b.head(num_constraints));
            for (const auto& binding : additional_constraint_bindings) {
                for (int index = 0; index < binding.evaluator()->num_constraints(); ++index) {
                    for (bool falsify_lower_bound : {true, false}) {
                        int consecutive_failures = 0;
                        if (falsify_lower_bound && std::isinf(binding.evaluator()->lower_bound()[index])) {
                            continue;
                        }
                        if (!falsify_lower_bound && std::isinf(binding.evaluator()->upper_bound()[index])) {
                            continue;
                        }
                        counter_example_constraint->set(&binding, index, falsify_lower_bound);
                        while (consecutive_failures < options.num_additional_constraint_infeasible_samples) {
                            if (counter_example_prog->Solve(*solver, guess, options.solver_options, &closest)) {
                                consecutive_failures = 0;
                                AddTangentToPolytope(E, closest, options.configuration_space_margin, &A, &b,
                                                     &num_constraints);
                                P_candidate = HPolyhedron(A.topRows(num_constraints), b.head(num_constraints));
                                MakeGuessFeasible(P_candidate, &guess);
                                if (options.require_sample_point_is_contained) {
                                ...
                                if (CheckTerminate(options, P_candidate, termination_error_msg, termination_msg,
                                                   iteration == 0)) {
                                    return P;
                                }
        ...
        if (delta_volume <= options.termination_threshold) {
            ...
            break;
        } else if (delta_volume / best_volume <= options.relative_termination_threshold) {
            log()->info(
                    "IrisInConfigurationSpace: Terminating because the hyperellipsoid "
...
void SetEdgeContainmentTerminationCondition(IrisOptions* options,
