...
CspaceFreeBox::SeparationCertificateResult CspaceFreeBox::SeparationCertificate::GetSolution(
        int plane_index,
    ...
    auto set_lagrangians = [&result](const std::vector<CspaceFreeBox::SeparatingPlaneLagrangians>& lagrangians_vec,
...
CspaceFreeBox::CspaceFreeBox(const multibody::MultibodyPlant<double>* plant,
    ...
    : CspaceFreePolytopeBase(plant, scene_graph, plane_order, CspaceFreePolytopeBase::SForPlane::kOnChain, options) {}
...
bool CspaceFreeBox::FindSeparationCertificateGivenBox(
        ...
        std::unordered_map<SortedPair<geometry::GeometryId>, SeparationCertificateResult>* certificates) const {
    std::vector<std::optional<SeparationCertificateResult>> certificates_vec =
            this->FindSeparationCertificateGivenBox(ignored_collision_pairs, q_box_lower, q_box_upper, options);
...
CspaceFreeBox::SeparatingPlaneLagrangians CspaceFreeBox::SeparatingPlaneLagrangians::GetSolution(
        const solvers::MathematicalProgramResult& result) const {
...
void CspaceFreeBox::ComputeSBox(const Eigen::Ref<const Eigen::VectorXd>& q_box_lower,
                                const Eigen::Ref<const Eigen::VectorXd>& q_box_upper,
    ...
    if ((q_box_lower.array() > q_box_upper.array()).any()) {
        throw std::runtime_error(
                fmt::format("CspaceFreeBox: q_box_lower={} has some entries larger "
                            "than q_box_upper={}.",
                            fmt_eigen(q_box_lower.transpose()), fmt_eigen(q_box_upper.transpose())));
    }
    ...
    if ((q_box_lower.array() > q_position_upper.array()).any()) {
        throw std::runtime_error(
                fmt::format("CspaceFreeBox: q_box_lower={} has some entries larger the the robot "
                            "position upper limit={}.",
                            fmt_eigen(q_box_lower.transpose()), fmt_eigen(q_position_upper.transpose())));
    }
    if ((q_box_upper.array() < q_position_lower.array()).any()) {
        throw std::runtime_error(
                fmt::format("CspaceFreeBox: q_box_upper={} has some entries smaller than the robot "
                            "position lower limit={}.",
                            fmt_eigen(q_box_upper.transpose()), fmt_eigen(q_position_lower.transpose())));
    }
    const Eigen::VectorXd q_lower = q_box_lower.array().max(q_position_lower.array()).matrix();
    const Eigen::VectorXd q_upper = q_box_upper.array().min(q_position_upper.array()).matrix();
...
void CspaceFreeBox::GeneratePolynomialsToCertify(const Eigen::Ref<const Eigen::VectorXd>& s_box_lower,
                                                 const Eigen::Ref<const Eigen::VectorXd>& s_box_upper,
                                                 const Eigen::Ref<const Eigen::VectorXd>& q_star,
                                                 const IgnoredCollisionPairs& ignored_collision_pairs,
                                                 PolynomialsToCertify* certify_polynomials) const {
    this->CalcSBoundsPolynomial<double>(s_box_lower, s_box_upper, &(certify_polynomials->s_minus_s_box_lower),
    ...
    for (int i = 0; i < static_cast<int>(separating_planes().size()); ++i) {
        ...
        if (!ignored_collision_pairs.contains(SortedPair<geometry::GeometryId>(plane.positive_side_geometry->id(),
                                                                               plane.negative_side_geometry->id()))) {
            separating_planes_map.emplace(i, &plane);
        }
    }

    internal::GenerateRationals(separating_planes_map, y_slack(), q_star, rational_forward_kin(),
                                &(certify_polynomials->plane_geometries));
}

CspaceFreeBox::SeparationCertificateProgram CspaceFreeBox::ConstructPlaneSearchProgram(
    ...
    const SortedPair<multibody::BodyIndex> body_pair{plane.positive_side_geometry->body_index(),
                                                     plane.negative_side_geometry->body_index()};
    const std::vector<int> s_for_plane_indices = this->map_body_pair_to_s_on_chain().at(body_pair);
    const std::unordered_set<int> s_for_plane_indices_set{s_for_plane_indices.begin(), s_for_plane_indices.end()};
    ...
    auto add_rational_nonnegative =
            [this, &s_for_plane_indices_set, &s_minus_s_lower, &s_upper_minus_s, &gram_vars, &gram_var_count](
                    solvers::MathematicalProgram* prog, const symbolic::RationalFunction& rational,
                    const std::array<VectorX<symbolic::Monomial>, 4>& monomial_basis_array)
...
std::vector<std::optional<CspaceFreeBox::SeparationCertificateResult>> CspaceFreeBox::FindSeparationCertificateGivenBox(
        const IgnoredCollisionPairs& ignored_collision_pairs,
        const Eigen::Ref<const Eigen::VectorXd>& q_box_lower,
        const Eigen::Ref<const Eigen::VectorXd>& q_box_upper,
    ...
    this->ComputeSBox(q_box_lower, q_box_upper, &s_box_lower, &s_box_upper, &q_star);
    PolynomialsToCertify polynomials_to_certify;
    this->GeneratePolynomialsToCertify(s_box_lower, s_box_upper, q_star, ignored_collision_pairs,
    ...
    // pair of geometries.
    auto solve_small_sos = [this, &polynomials_to_certify, &active_plane_indices, &options,
        ...
        auto certificate_program = this->ConstructPlaneSearchProgram(
                polynomials_to_certify.plane_geometries[plane_count], polynomials_to_certify.s_minus_s_box_lower,
...
void CspaceFreeBox::AddCspaceBoxContainment(solvers::MathematicalProgram* prog,
    ...
    prog->AddBoundingBoxConstraint(Eigen::VectorXd::Constant(s_size, -kInf), s_inner_pts.rowwise().minCoeff(),
                                   s_box_lower);
    prog->AddBoundingBoxConstraint(s_inner_pts.rowwise().maxCoeff(), Eigen::VectorXd::Constant(s_size, kInf),
                                   s_box_upper);
}
}  // namespace optimization
}  // namespace geometry
