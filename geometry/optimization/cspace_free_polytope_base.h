...
public:
    ...
    [[nodiscard]] const multibody::RationalForwardKinematics& rational_forward_kin() const {
...
protected:
    ...
    */
    ...
    [[nodiscard]] const geometry::SceneGraph<double>& scene_graph() const { return *scene_graph_; }

    [[nodiscard]] const std::map<multibody::BodyIndex, std::vector<std::unique_ptr<CIrisCollisionGeometry>>>&
    link_geometries() const {
        return link_geometries_;
    }

    [[nodiscard]] SeparatingPlaneOrder plane_order() const { return plane_order_; }
    ...
    /** For a pair of bodies body_pair, returns the indices of all s on the
     ...
     and bodyB.
     */
    [[nodiscard]] const std::unordered_map<SortedPair<multibody::BodyIndex>, std::vector<int>>&
     ...
     */
    [[nodiscard]] VectorX<symbolic::Variable> GetSForPlane(const SortedPair<multibody::BodyIndex>& body_pair,
...
private:
    ...
    /* Set map_body_pair_to_s_on_chain.
     ...
     body_pair.second().
     */
    void SetIndicesOfSOnChainForBodyPair(const SortedPair<multibody::BodyIndex>& body_pair);
    ...
    // body_middle is the body in the middle of the kinematics chain between bodyA
    // and bodyB.
    std::unordered_map<SortedPair<multibody::BodyIndex>, std::vector<int>> map_body_pair_to_s_on_chain_;
};
