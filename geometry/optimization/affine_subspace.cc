...
AffineSubspace::AffineSubspace() : AffineSubspace(MatrixXd(0, 0), VectorXd(0)) {}

AffineSubspace::AffineSubspace(const Eigen::Ref<const MatrixXd>& basis, const Eigen::Ref<const VectorXd>& translation)
...
AffineSubspace::AffineSubspace(const ConvexSet& set, double tol) : ConvexSet(0, true) {
    ...
    if (singleton_maybe.has_value()) {
        ...
        *this = AffineSubspace(Eigen::MatrixXd::Zero(set.ambient_dimension(), 0), singleton_maybe.value());
        return;
    }
    ...
    if (!translation_maybe.has_value()) {
        throw std::runtime_error("AffineSubspace: Cannot take the affine hull of an empty set!");
    }
    ...
    const MatrixXd I = MatrixXd::Identity(set.ambient_dimension(), set.ambient_dimension());
    ...
    VectorXDecisionVariable x = prog.NewContinuousVariables(set.ambient_dimension(), "x");
    prog.AddBoundingBoxConstraint(-1, 1, x);

    // y and z are two feasible points in the set
    VectorXDecisionVariable y = prog.NewContinuousVariables(set.ambient_dimension(), "y");
    VectorXDecisionVariable z = prog.NewContinuousVariables(set.ambient_dimension(), "z");
    ...
    Binding<LinearCost> objective = prog.AddLinearCost(VectorXd::Zero(set.ambient_dimension()), x);
    ...
    while (affine_dimension + complement_dimension < set.ambient_dimension()) {
        ...
        const MatrixXd spanning_unknown =
                (I -
                 basis_orth.leftCols(complement_dimension) * basis_orth.leftCols(complement_dimension).transpose()) *
                (I - basis.leftCols(affine_dimension) * basis.leftCols(affine_dimension).transpose());
        int ii = 0;
        ...
        if (result.get_optimal_cost() < -tol) {
            ...
            basis.col(affine_dimension++) = new_basis_vector / new_basis_vector.norm();
...
std::optional<bool> AffineSubspace::DoPointInSetShortcut(const Eigen::Ref<const VectorXd>& x, double tol) const {
...
std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> AffineSubspace::DoAddPointInSetConstraints(
        solvers::MathematicalProgram* prog, const Eigen::Ref<const solvers::VectorXDecisionVariable>& x) const {
    ...
    new_constraints.push_back(prog->AddLinearEqualityConstraint(A, -translation_, {alpha, x}));
    return {std::move(alpha), std::move(new_constraints)};
}

std::vector<Binding<Constraint>> AffineSubspace::DoAddPointInNonnegativeScalingConstraints(
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> AffineSubspace::DoToShapeWithPose() const {
    throw std::runtime_error("ToShapeWithPose is not supported by AffineSubspace.");
}
...
Eigen::MatrixXd AffineSubspace::Project(const Eigen::Ref<const Eigen::MatrixXd>& x) const {
...
std::vector<std::optional<double>> AffineSubspace::DoProjectionShortcut(
        const Eigen::Ref<const Eigen::MatrixXd>& points, EigenPtr<Eigen::MatrixXd> projected_points) const {
    ...
    if (maybe_point) {
        const Eigen::VectorXd eigen_dists = (points - maybe_point.value()).colwise().norm();
        std::vector<std::optional<double>> distances(eigen_dists.data(), eigen_dists.data() + eigen_dists.size());
        if (points.cols() == 1) {
            ...
            // point.
            *projected_points = maybe_point.value() * Eigen::RowVectorXd::Ones(points.cols());
            return distances;
        }
    }
    const Eigen::MatrixXd least_squares = basis_decomp_->solve(points.colwise() - translation_);
    *projected_points = (basis_ * least_squares).colwise() + translation_;
    const Eigen::VectorXd eigen_dists = (points - *projected_points).colwise().norm();
    std::vector<std::optional<double>> distances(eigen_dists.data(), eigen_dists.data() + eigen_dists.size());
    return distances;
}

Eigen::MatrixXd AffineSubspace::ToLocalCoordinates(const Eigen::Ref<const Eigen::MatrixXd>& x) const {
    DRAKE_THROW_UNLESS(x.rows() == ambient_dimension());
...
Eigen::MatrixXd AffineSubspace::ToGlobalCoordinates(const Eigen::Ref<const Eigen::MatrixXd>& y) const {
    DRAKE_THROW_UNLESS(y.rows() == AffineDimension());
    return (basis_ * y).colwise() + translation_;
}

bool AffineSubspace::ContainedIn(const AffineSubspace& other, double tol) const {
...
bool AffineSubspace::IsNearlyEqualTo(const AffineSubspace& other, double tol) const {
...
Eigen::MatrixXd AffineSubspace::OrthogonalComplementBasis() const {
    ...
    MatrixXd Q = basis_decomp_.value().householderQ() * MatrixXd::Identity(ambient_dimension(), ambient_dimension());
