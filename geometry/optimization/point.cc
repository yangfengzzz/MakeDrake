...
Point::Point(const Eigen::Ref<const VectorXd>& x) : ConvexSet(x.size(), true), x_(x) {}

Point::Point(const QueryObject<double>& query_object,
    ...
    if (shape.type_name() != "Sphere") {
        throw std::logic_error(
                fmt::format("Point(geometry_id={}, ...) cannot convert a {}, only a Sphere", geometry_id, shape));
    }
...
std::optional<bool> Point::DoPointInSetShortcut(const Eigen::Ref<const VectorXd>& x, double tol) const {
    return is_approx_equal_abstol(x, x_, tol);
}

std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> Point::DoAddPointInSetConstraints(
...
std::vector<Binding<Constraint>> Point::DoAddPointInNonnegativeScalingConstraints(
    ...
    Aeq.col(n) = -x_;
    constraints.emplace_back(prog->AddLinearEqualityConstraint(Aeq, VectorXd::Zero(n), {x, Vector1<Variable>(t)}));
    return constraints;
}
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> Point::DoToShapeWithPose() const {
    return std::make_pair(std::make_unique<Sphere>(0.0), math::RigidTransformd(x_));
}

std::vector<std::optional<double>> Point::DoProjectionShortcut(const Eigen::Ref<const Eigen::MatrixXd>& points,
                                                               EigenPtr<Eigen::MatrixXd> projected_points) const {
    *projected_points = x_ * Eigen::MatrixXd::Ones(1, points.cols());
    Eigen::VectorXd distances_eigen = (points - *projected_points).colwise().norm();
    std::vector<std::optional<double>> distances(distances_eigen.data(),
                                                 distances_eigen.data() + distances_eigen.size());
    return distances;
}
