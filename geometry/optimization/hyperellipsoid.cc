...
Hyperellipsoid::Hyperellipsoid() : Hyperellipsoid(MatrixXd(0, 0), VectorXd(0)) {}

Hyperellipsoid::Hyperellipsoid(const Eigen::Ref<const MatrixXd>& A, const Eigen::Ref<const VectorXd>& center)
...
std::pair<double, VectorXd> Hyperellipsoid::MinimumUniformScalingToTouch(const ConvexSet& other) const {
    ...
    std::vector<solvers::SolverId> preferred_solvers{solvers::MosekSolver::id(), solvers::GurobiSolver::id(),
    ...
    if (!result.is_success()) {
        ...
        if (!result2.is_success()) {
            ...
            throw std::runtime_error(
                    fmt::format("Solver {} failed to solve the `minimum uniform scaling to touch' "
    ...
    return std::pair<double, VectorXd>(std::sqrt(result.get_optimal_cost()), result.GetSolution(x));
}

Hyperellipsoid Hyperellipsoid::MakeAxisAligned(const Eigen::Ref<const VectorXd>& radius,
...
Hyperellipsoid Hyperellipsoid::MakeHypersphere(double radius, const Eigen::Ref<const VectorXd>& center) {
...
Hyperellipsoid Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(const Eigen::Ref<const MatrixXd>& points,
                                                                   double rank_tol) {
    ...
    Eigen::VectorXd mean = points.rowwise().mean();
    {
        ...
        if (svd.singularValues()[0] < rank_tol) {
            throw std::runtime_error(
                    fmt::format("The numerical rank of the points appears to be zero. (The largest "
    ...
    MathematicalProgram prog;
    solvers::MatrixXDecisionVariable A = prog.NewSymmetricContinuousVariables(rank, "A");
...
std::optional<bool> Hyperellipsoid::DoPointInSetShortcut(const Eigen::Ref<const VectorXd>& x, double tol) const {
    DRAKE_DEMAND(A_.cols() == x.size());
    const VectorXd v = A_ * (x - center_);
    return v.dot(v) <= 1.0 + tol;
}

std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> Hyperellipsoid::DoAddPointInSetConstraints(
...
std::vector<Binding<Constraint>> Hyperellipsoid::DoAddPointInNonnegativeScalingConstraints(
    ...
    constraints.emplace_back(prog->AddLorentzConeConstraint(A_cone, VectorXd::Zero(m + 1), {x, Vector1<Variable>(t)}));
    return constraints;
}
...
std::vector<Binding<Constraint>> Hyperellipsoid::DoAddPointInNonnegativeScalingConstraints(
    ...
    constraints.emplace_back(prog->AddLorentzConeConstraint(A_cone, b_cone, {x, t}));
    return constraints;
}

std::pair<std::unique_ptr<Shape>, RigidTransformd> Hyperellipsoid::DoToShapeWithPose() const {
    ...
    auto shape = std::make_unique<Ellipsoid>(1.0 / sqrt(solver.eigenvalues()[0]), 1.0 / sqrt(solver.eigenvalues()[1]),
...
void Hyperellipsoid::ImplementGeometry(const Ellipsoid& ellipsoid, void* data) {
    ...
    *A = Eigen::DiagonalMatrix<double, 3>(1.0 / ellipsoid.a(), 1.0 / ellipsoid.b(), 1.0 / ellipsoid.c());
}
