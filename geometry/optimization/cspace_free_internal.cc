...
struct BodyPair {
    BodyPair(multibody::BodyIndex m_body1, multibody::BodyIndex m_body2) : body1{m_body1}, body2{m_body2} {}
    bool operator==(const BodyPair& other) const { return body1 == other.body1 && body2 == other.body2; }
...
[[nodiscard]] bool ChainIsWelded(const multibody::MultibodyPlant<double>& plant,
    ...
    const multibody::internal::MultibodyTree<double>& tree = multibody::internal::GetInternalTree(plant);
...
[[nodiscard]] std::vector<std::pair<const CIrisCollisionGeometry*, const CIrisCollisionGeometry*>>
GetLinkCollisionPairs(const multibody::MultibodyPlant<double>& plant,
                      const SceneGraph<double>& scene_graph,
                      multibody::BodyIndex link1,
                      multibody::BodyIndex link2,
                      const std::vector<std::unique_ptr<CIrisCollisionGeometry>>& link1_geometries,
                      const std::vector<std::unique_ptr<CIrisCollisionGeometry>>& link2_geometries) {
    std::vector<std::pair<const CIrisCollisionGeometry*, const CIrisCollisionGeometry*>> ret;
    ...
    for (const auto& geometry1 : link1_geometries) {
        for (const auto& geometry2 : link2_geometries) {
            if (!model_inspector.CollisionFiltered(geometry1->id(), geometry2->id())) {
...
template <typename T>
void SymmetricMatrixFromLowerTriangularPart(int rows,
                                            const Eigen::Ref<const VectorX<T>>& lower_triangle,
                                            MatrixX<T>* mat) {
...
void AddPsdConstraint(solvers::MathematicalProgram* prog, const MatrixX<symbolic::Variable>& X) {
    DRAKE_THROW_UNLESS(X.rows() == X.cols());
    if (X.rows() == 1) {
        prog->AddBoundingBoxConstraint(0, kInf, X(0, 0));
    } else if (X.rows() == 2) {
        prog->AddRotatedLorentzConeConstraint(Vector3<symbolic::Variable>(X(0, 0), X(1, 1), X(0, 1)));
    } else {
...
int GetNumYInRational(const symbolic::RationalFunction& rational, const Vector3<symbolic::Variable>& y_slack) {
    int count = 0;
    for (int i = 0; i < 3; ++i) {
        if (rational.numerator().indeterminates().find(y_slack(i)) != rational.numerator().indeterminates().end()) {
            ++count;
        }
    }
    return count;
...
std::map<multibody::BodyIndex, std::vector<std::unique_ptr<CIrisCollisionGeometry>>> GetCollisionGeometries(
        const multibody::MultibodyPlant<double>& plant, const geometry::SceneGraph<double>& scene_graph) {
    std::map<multibody::BodyIndex, std::vector<std::unique_ptr<CIrisCollisionGeometry>>> ret;
    const auto& inspector = scene_graph.model_inspector();

    for (multibody::BodyIndex body_index{0}; body_index < plant.num_bodies(); ++body_index) {
        const std::optional<geometry::FrameId> frame_id = plant.GetBodyFrameIdIfExists(body_index);
        if (frame_id.has_value()) {
            const auto geometry_ids = inspector.GetGeometries(frame_id.value(), geometry::Role::kProximity);
            for (const auto& geometry_id : geometry_ids) {
                auto collision_geometry =
                        std::make_unique<CIrisCollisionGeometry>(&(inspector.GetShape(geometry_id)), body_index,
                                                                 geometry_id, inspector.GetPoseInFrame(geometry_id));
        ...
        for (const auto& [link2, geometries2] : link_geometries) {
            if (link1 < link2) {
                auto it = collision_pairs->emplace_hint(
                        collision_pairs->end(), SortedPair<multibody::BodyIndex>(link1, link2),
                        GetLinkCollisionPairs(plant, scene_graph, link1, link2, geometries1, geometries2));
...
void GenerateRationals(const std::map<int, const CSpaceSeparatingPlane<symbolic::Variable>*>& separating_planes,
    ...
    // computation.
    std::unordered_map<BodyPair, multibody::RationalForwardKinematics::Pose<symbolic::Polynomial>, BodyPairHash>
    ...
    for (const auto& [plane_index, plane_ptr] : separating_planes) {
        ...
        for (const PlaneSide plane_side : {PlaneSide::kPositive, PlaneSide::kNegative}) {
            const CIrisCollisionGeometry* link_geometry = separating_plane.geometry(plane_side);

            const BodyPair expressed_to_link(separating_plane.expressed_body, link_geometry->body_index());
            ...
            if (it == body_pair_to_X_AB_multilinear.end()) {
                std::tie(it, std::ignore) = body_pair_to_X_AB_multilinear.emplace(
                        expressed_to_link,
                        rational_forward_kin.CalcBodyPoseAsMultilinearPolynomial(q_star, link_geometry->body_index(),
                                                                                 separating_plane.expressed_body));
            }
            const multibody::RationalForwardKinematics::Pose<symbolic::Polynomial>& X_AB_multilinear = it->second;
            auto& rationals = plane_side == PlaneSide::kPositive ? positive_side_rationals : negative_side_rationals;
            link_geometry->OnPlaneSide(separating_plane.a, separating_plane.b, X_AB_multilinear, rational_forward_kin,
        ...
        if (separating_plane.positive_side_geometry->type() == CIrisGeometryType::kPolytope &&
            separating_plane.negative_side_geometry->type() == CIrisGeometryType::kPolytope) {
            plane_geometries->emplace_back(positive_side_rationals, negative_side_rationals, plane_index);
        } else if (separating_plane.positive_side_geometry->type() == CIrisGeometryType::kPolytope &&
                   separating_plane.negative_side_geometry->type() != CIrisGeometryType::kPolytope) {
            ...
            for (int i = 0; i < static_cast<int>(negative_side_rationals.size()); ++i) {
                if (GetNumYInRational(negative_side_rationals[i], y_slack) > 0) {
                    negative_side_rationals_with_y.push_back(std::move(negative_side_rationals[i]));
                }
            }
            plane_geometries->emplace_back(positive_side_rationals, negative_side_rationals_with_y, plane_index);
        } else if (separating_plane.positive_side_geometry->type() != CIrisGeometryType::kPolytope &&
                   separating_plane.negative_side_geometry->type() == CIrisGeometryType::kPolytope) {
            ...
            for (int i = 0; i < static_cast<int>(positive_side_rationals.size()); ++i) {
                if (GetNumYInRational(positive_side_rationals[i], y_slack) > 0) {
                    positive_side_rationals_with_y.push_back(std::move(positive_side_rationals[i]));
                }
            }
            plane_geometries->emplace_back(positive_side_rationals_with_y, negative_side_rationals, plane_index);
        } else {
...
void GenerateRationals(const std::vector<std::unique_ptr<CSpaceSeparatingPlane<symbolic::Variable>>>& separating_planes,
                       ...
                       const multibody::RationalForwardKinematics& rational_forward_kin,
                       std::vector<PlaneSeparatesGeometries>* plane_geometries) {
    std::map<int, const CSpaceSeparatingPlane<symbolic::Variable>*> separating_planes_map;
    for (int i = 0; i < static_cast<int>(separating_planes.size()); ++i) {
        separating_planes_map.emplace(i, separating_planes.at(i).get());
    }
    GenerateRationals(separating_planes_map, y_slack, q_star, rational_forward_kin, plane_geometries);
}
...
void SolveSeparationCertificateProgramBase(const SeparationCertificateProgramBase& certificate_program,
    ...
    solvers::MakeSolver(options.solver_id)
            ->Solve(*certificate_program.prog, std::nullopt, options.solver_options, &(result->result));
    if (result->result.is_success()) {
        result->plane_decision_var_vals = result->result.GetSolution(separating_plane.decision_variables);
        for (int i = 0; i < 3; ++i) {
...
int GetGramVarSize(const std::array<VectorX<symbolic::Monomial>, 4>& monomial_basis_array,
    ...
    if (num_y == 0) {
        ...
        if (with_cross_y) {
            ...
            int ret = 0;
            for (int i = 0; i < num_y; ++i) {
                ret += gram_lower_size(monomial_basis_array[0].rows() + monomial_basis_array[i + 1].rows());
            }
            return ret;
        }
    }
...
GramAndMonomialBasis::GramAndMonomialBasis(const std::array<VectorX<symbolic::Monomial>, 4>& monomial_basis_array,
                                           bool with_cross_y,
                                           int num_y) {
    this->gram_var_size = internal::GetGramVarSize(monomial_basis_array, with_cross_y, num_y);
    if (num_y == 0) {
        // We only need to use monomial_basis_array[0].
        this->grams.emplace_back(monomial_basis_array[0].rows(), monomial_basis_array[0].rows());
        ...
        if (with_cross_y) {
            ...
            this->monomial_basis[0].topRows(monomial_basis_array[0].rows()) = monomial_basis_array[0];
            gram_rows = monomial_basis_array[0].rows();
            for (int i = 0; i < num_y; ++i) {
                this->monomial_basis[0].segment(gram_rows, monomial_basis_array[i + 1].rows()) =
            ...
            for (int i = 0; i < num_y; ++i) {
                const int gram_rows = monomial_basis_array[0].rows() + monomial_basis_array[i + 1].rows();
                this->grams.emplace_back(gram_rows, gram_rows);
                this->monomial_basis.emplace_back(gram_rows);
                this->monomial_basis.back().topRows(monomial_basis_array[0].rows()) = monomial_basis_array[0];
                this->monomial_basis.back().bottomRows(monomial_basis_array[i + 1].rows()) =
                        monomial_basis_array[i + 1];
            }
        }
    }
...
void GramAndMonomialBasis::AddSos(solvers::MathematicalProgram* prog,
    ...
    for (auto& gram : this->grams) {
        ...
        SymmetricMatrixFromLowerTriangularPart<symbolic::Variable>(
                gram.rows(), gram_lower.segment(gram_var_count, gram_lower_size), &gram);
    ...
    *poly = symbolic::Polynomial();
    ...
    for (int i = 0; i < static_cast<int>(this->grams.size()); ++i) {
        ...
        const int gram_lower_size = this->grams[i].rows() * (this->grams[i].rows() + 1) / 2;
        *poly += symbolic::CalcPolynomialWLowerTriangularPart(this->monomial_basis[i],
...
solvers::MathematicalProgramResult SolveWithBackoff(solvers::MathematicalProgram* prog,
    ...
    if (backoff_scale.has_value() && !(prog->linear_costs().empty())) {
        ...
        const double cost_upper_bound =
                cost_val > 0 ? (1 + backoff_scale.value()) * cost_val : (1 - backoff_scale.value()) * cost_val;
        prog->AddLinearConstraint(prog->linear_costs()[0].evaluator()->a(), -kInf,
