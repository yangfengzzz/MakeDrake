...
CartesianProduct::CartesianProduct(const ConvexSets& sets)
    : ConvexSet(SumAmbientDimensions(sets), HasExactVolume(sets)), sets_(sets) {}

CartesianProduct::CartesianProduct(const ConvexSet& setA, const ConvexSet& setB)
    : ConvexSet(setA.ambient_dimension() + setB.ambient_dimension(), HasExactVolume(std::vector{&setA, &setB})) {
...
CartesianProduct::CartesianProduct(const QueryObject<double>& query_object,
    ...
    if (shape.type_name() != "Cylinder") {
        throw std::logic_error(
                fmt::format("CartesianProduct(geometry_id={}, ...) cannot convert a {}, only a "
                            "Cylinder",
                            geometry_id, shape));
    }
    ...
    sets_.emplace_back(Hyperellipsoid::MakeHypersphere(cylinder.radius(), Vector2d::Zero()).Clone());
    // ToDo(@sadraddini) use MakeBox out of a future zonotope class instead so
    // that we can compute the volume exactly.
    sets_.emplace_back(
            HPolyhedron::MakeBox(Vector1d{-cylinder.length() / 2.0}, Vector1d{cylinder.length() / 2.0}).Clone());
...
std::optional<Eigen::VectorXd> CartesianProduct::DoMaybeGetFeasiblePoint() const {
    std::vector<VectorXd> points;
    for (const auto& s : sets_) {
...
VectorXd CartesianProduct::StackAndMaybeTransform(const std::vector<Eigen::VectorXd>& points) const {
    // Stack the points.
...
bool CartesianProduct::DoPointInSet(const Eigen::Ref<const VectorXd>& x, double tol) const {
    int index = 0;
...
std::pair<VectorX<symbolic::Variable>, std::vector<Binding<Constraint>>> CartesianProduct::DoAddPointInSetConstraints(
    ...
    if (A_) {
        ...
        new_constraints.push_back(prog->AddLinearEqualityConstraint(Aeq, (*b_), {y, x}));
    } else {
        y = x;
    }
    int index = 0;
    for (const auto& s : sets_) {
        if (s->ambient_dimension() == 0) {
            std::optional<Variable> new_var =
                    ConvexSet::HandleZeroAmbientDimensionConstraints(prog, *s, &new_constraints);
        ...
        new_constraints.insert(new_constraints.end(), new_constraints_in_s.begin(), new_constraints_in_s.end());
        index += s->ambient_dimension();
    }
    VectorX<Variable> new_vars_vec = Eigen::Map<VectorX<Variable>>(new_vars.data(), new_vars.size());
    return {std::move(new_vars_vec), std::move(new_constraints)};
}

std::vector<Binding<Constraint>> CartesianProduct::DoAddPointInNonnegativeScalingConstraints(
    ...
    if (A_) {
        ...
        constraints.emplace_back(
                prog->AddLinearEqualityConstraint(Aeq, VectorXd::Zero(A_->rows()), {y, Vector1<Variable>{t}, x}));
    } else {
        y = x;
    }
    int index = 0;
    for (const auto& s : sets_) {
        ...
        auto new_constraints =
                s->AddPointInNonnegativeScalingConstraints(prog, y.segment(index, s->ambient_dimension()), t);
        index += s->ambient_dimension();
        constraints.insert(constraints.end(), std::make_move_iterator(new_constraints.begin()),
...
std::vector<Binding<Constraint>> CartesianProduct::DoAddPointInNonnegativeScalingConstraints(
    ...
    if (A_) {
        ...
        MatrixXd Aeq = MatrixXd::Identity(A_->rows(), A_->rows() + x.size() + t.size());
        Aeq.middleCols(A_->rows(), x.size()) = -(*A_) * A_x;
        Aeq.rightCols(t.size()) = -(*b_) * c.transpose();
        VectorXd beq = (*A_) * b_x + (*b_) * d;
        constraints.emplace_back(prog->AddLinearEqualityConstraint(Aeq, beq, {y, x, t}));
        int index = 0;
        for (const auto& s : sets_) {
            if (s->ambient_dimension() == 0) {
                ConvexSet::HandleZeroAmbientDimensionConstraints(prog, *s, &constraints);
            ...
            auto new_constraints = s->AddPointInNonnegativeScalingConstraints(
                    prog, MatrixXd::Identity(set_dim, set_dim), VectorXd::Zero(set_dim), c, d,
                    y.segment(index, set_dim), t);
            index += set_dim;
            constraints.insert(constraints.end(), std::make_move_iterator(new_constraints.begin()),
                               std::make_move_iterator(new_constraints.end()));
        }
        ...
        for (const auto& s : sets_) {
            ...
            auto new_constraints =
                    s->AddPointInNonnegativeScalingConstraints(prog, A_x.middleRows(index, s->ambient_dimension()),
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> CartesianProduct::DoToShapeWithPose() const {
    // TODO(russt): Consider handling Cylinder as a (very) special case.
    throw std::runtime_error("ToShapeWithPose is not implemented yet for CartesianProduct.");
}
