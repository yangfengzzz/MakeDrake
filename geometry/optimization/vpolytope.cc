...
VPolytope::VPolytope(const HPolyhedron& hpoly, double tol) : ConvexSet(hpoly.ambient_dimension(), true) {
    ...
    if (affine_hull.AffineDimension() < affine_hull.ambient_dimension()) {
        ...
        if (affine_hull.AffineDimension() == 0) {
            ...
            *this = VPolytope(points_in);
            ...
            VectorXDecisionVariable x = prog.NewContinuousVariables(hpoly.ambient_dimension());
            hpoly.AddPointInSetConstraints(&prog, x);
            Binding<LinearCost> objective = prog.AddLinearCost(affine_hull.basis().col(0), x);
        ...
        HPolyhedron hpoly_subspace(A_filtered * affine_hull.basis(),
        ...
        Eigen::MatrixXd points_global = affine_hull.ToGlobalCoordinates(points_local);
    ...
    VectorXd::Map(&flat_coeffs[0], coeffs_t.size()) = VectorXd::Map(coeffs_t.data(), coeffs_t.size());
    ...
    qhull.runQhull("", hpoly.A().cols() + 1, hpoly.A().rows(), flat_coeffs.data(), "H");
    if (qhull.qhullStatus() != 0) {
        throw std::runtime_error(fmt::format("Qhull terminated with status {} and  message:\n{}", qhull.qhullStatus(),
                                             qhull.qhullMessage()));
    }
    ...
    for (const auto& facet : qhull.facetList()) {
        ...
        DRAKE_THROW_UNLESS(incident_hyperplanes.count() >= hpoly.ambient_dimension());
        MatrixXd vertex_A(incident_hyperplanes.count(), hpoly.ambient_dimension());
        for (int jj = 0; jj < incident_hyperplanes.count(); jj++) {
            std::vector<double> hyperplane = incident_hyperplanes.at(jj).point().toStdVector();
            vertex_A.row(jj) = Eigen::Map<Eigen::RowVectorXd, Eigen::Unaligned>(hyperplane.data(), hyperplane.size());
        }
        ...
        vertices_.col(ii) = QR.solve(VectorXd::Ones(incident_hyperplanes.count())) + eigen_center;
        ii++;
    }
}
...
VPolytope VPolytope::MakeBox(const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
...
void VPolytope::WriteObj(const std::filesystem::path& filename) const {
    ...
    qhull.runQhull("", vertices_.rows(), vertices_.cols(), vertices_.data(), qhull_options);
    if (qhull.qhullStatus() != 0) {
        throw std::runtime_error(fmt::format("Qhull terminated with status {} and message:\n{}", qhull.qhullStatus(),
                                             qhull.qhullMessage()));
    }
...
bool VPolytope::DoPointInSet(const Eigen::Ref<const VectorXd>& x, double tol) const {
    if (vertices_.cols() == 0) {
        return false;
    }
...
std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> VPolytope::DoAddPointInSetConstraints(
    ...
    new_constraints.push_back(prog->AddLinearEqualityConstraint(A, VectorXd::Zero(n), {alpha, x}));
    // ∑ αᵢ = 1.
    new_constraints.push_back(prog->AddLinearEqualityConstraint(RowVectorXd::Ones(m), 1.0, alpha));
    return {std::move(alpha), std::move(new_constraints)};
}

std::vector<Binding<Constraint>> VPolytope::DoAddPointInNonnegativeScalingConstraints(
    ...
    VectorXDecisionVariable alpha = prog->NewContinuousVariables(m, "a");
    // αᵢ ≥ 0.
    constraints.emplace_back(prog->AddBoundingBoxConstraint(0, std::numeric_limits<double>::infinity(), alpha));
    // v α = x.
    MatrixXd A(n, m + n);
    A.leftCols(m) = vertices_;
    A.rightCols(n) = -MatrixXd::Identity(n, n);
    constraints.emplace_back(prog->AddLinearEqualityConstraint(A, VectorXd::Zero(n), {alpha, x}));

    // ∑ αᵢ = t.
    RowVectorXd a = RowVectorXd::Ones(m + 1);
    a[m] = -1;
    constraints.emplace_back(prog->AddLinearEqualityConstraint(a, 0.0, {alpha, Vector1<Variable>(t)}));
    return constraints;
}
...
std::vector<Binding<Constraint>> VPolytope::DoAddPointInNonnegativeScalingConstraints(
    ...
    constraints.emplace_back(prog->AddLinearEqualityConstraint(A_combination, b, {alpha, x}));
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> VPolytope::DoToShapeWithPose() const {
