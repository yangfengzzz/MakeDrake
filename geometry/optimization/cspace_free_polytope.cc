...
#include "geometry/optimization/cspace_free_internal.h"
#include "multibody/rational/rational_forward_kinematics.h"
#include "multibody/rational/rational_forward_kinematics_internal.h"
...
CspaceFreePolytope::CspaceFreePolytope(const multibody::MultibodyPlant<double>* plant,
                                       const geometry::SceneGraph<double>* scene_graph,
                                       SeparatingPlaneOrder plane_order,
                                       const Eigen::Ref<const Eigen::VectorXd>& q_star,
                                       const CspaceFreePolytope::Options& options)
    : CspaceFreePolytopeBase(plant, scene_graph, plane_order, CspaceFreePolytopeBase::SForPlane::kAll, options),
      q_star_{q_star} {
    s_lower_ = rational_forward_kin().ComputeSValue(rational_forward_kin().plant().GetPositionLowerLimits(), q_star_);
    s_upper_ = rational_forward_kin().ComputeSValue(rational_forward_kin().plant().GetPositionUpperLimits(), q_star_);
    this->CalcSBoundsPolynomial(s_lower_, s_upper_, &s_minus_s_lower_, &s_upper_minus_s_);

    std::vector<std::unique_ptr<CSpaceSeparatingPlane<symbolic::Variable>>> separating_planes_ptrs;
    separating_planes_ptrs.reserve(separating_planes().size());
    for (const auto& plane : separating_planes()) {
        separating_planes_ptrs.push_back(std::make_unique<CSpaceSeparatingPlane<symbolic::Variable>>(plane));
    }

    internal::GenerateRationals(separating_planes_ptrs, y_slack(), q_star_, rational_forward_kin(), &plane_geometries_);
}

CspaceFreePolytope::SeparatingPlaneLagrangians CspaceFreePolytope::SeparatingPlaneLagrangians::GetSolution(
        const solvers::MathematicalProgramResult& result) const {
    CspaceFreePolytope::SeparatingPlaneLagrangians ret(this->polytope_.rows(), this->s_lower_.rows());
...
CspaceFreePolytope::SeparationCertificateProgram CspaceFreePolytope::ConstructPlaneSearchProgram(
    ...
    auto count_gram = [this, &d_minus_Cs, &C_redundant_indices, &s_lower_redundant_indices, &s_upper_redundant_indices](
        ...
        const int num_sos = 1 + d_minus_Cs.rows() + 2 * s_size - C_redundant_indices.size() -
        ...
        const int num_gram_vars_per_sos = internal::GetGramVarSize(monomial_basis_array, this->with_cross_y(), y_size);
        return num_gram_vars_per_sos * num_sos;
    };

    for (PlaneSide plane_side : {PlaneSide::kPositive, PlaneSide::kNegative}) {
        const SortedPair<multibody::BodyIndex> one_side_body_pair(plane.expressed_body,
    ...
    const auto gram_vars = ret.prog->NewContinuousVariables(gram_var_count, "Gram");

    gram_var_count = 0;
    auto add_rational_nonnegative = [this, &d_minus_Cs, &C_redundant_indices, &s_lower_redundant_indices,
        ...
        const int y_size = internal::GetNumYInRational(rational, this->y_slack());
        internal::GramAndMonomialBasis gram_and_monomial_basis(monomial_basis_array, this->with_cross_y(), y_size);
        ...
        for (int j = 0; j < d_minus_Cs.rows(); ++j) {
            if (!C_redundant_indices.contains(j)) {
                gram_and_monomial_basis.AddSos(prog, gram_vars.segment(gram_var_count, num_gram_vars_per_sos),
        ...
        const symbolic::Polynomial poly = rational.numerator() - lagrangians.polytope().dot(d_minus_Cs) -
                                          lagrangians.s_lower().dot(this->s_minus_s_lower_) -
                                          lagrangians.s_upper().dot(this->s_upper_minus_s_);
        symbolic::Polynomial poly_sos;
        gram_and_monomial_basis.AddSos(prog, gram_vars.segment(gram_var_count, num_gram_vars_per_sos), &poly_sos);
        gram_var_count += num_gram_vars_per_sos;
    ...
    for (PlaneSide plane_side : {PlaneSide::kPositive, PlaneSide::kNegative}) {
        ret.certificate.mutable_lagrangians(plane_side).reserve(plane_geometries.rationals(plane_side).size());
        const SortedPair<multibody::BodyIndex> one_side_body_pair(plane.expressed_body,
                                                                  plane.geometry(plane_side)->body_index());
        const std::array<VectorX<symbolic::Monomial>, 4>& monomial_basis_array =
                this->map_body_to_monomial_basis_array().at(one_side_body_pair);
        for (const auto& rational : plane_geometries.rationals(plane_side)) {
            ret.certificate.mutable_lagrangians(plane_side)
                    .push_back(add_rational_nonnegative(ret.prog.get_mutable(), rational, monomial_basis_array));
        }
    }
...
CspaceFreePolytope::SeparationCertificateResult CspaceFreePolytope::SeparationCertificate::GetSolution(
        int plane_index,
    ...
    auto set_lagrangians = [&result](const std::vector<CspaceFreePolytope::SeparatingPlaneLagrangians>& lagrangians_vec,
                                     std::vector<CspaceFreePolytope::SeparatingPlaneLagrangians>* lagrangians_result) {
    ...
    set_lagrangians(this->positive_side_rational_lagrangians, &ret.positive_side_rational_lagrangians);
    set_lagrangians(this->negative_side_rational_lagrangians, &ret.negative_side_rational_lagrangians);
    for (int i = 0; i < 3; ++i) {
...
CspaceFreePolytope::FindSeparationCertificateGivenPolytope(
    ...
    this->FindRedundantInequalities(C, d, this->s_lower_, this->s_upper_, 0., &C_redundant_indices,
    ...
    for (int i = 0; i < static_cast<int>(separating_planes().size()); ++i) {
        if (!ignored_collision_pairs.contains(
                    SortedPair<geometry::GeometryId>(separating_planes()[i].positive_side_geometry->id(),
                                                     separating_planes()[i].negative_side_geometry->id()))) {
            active_plane_indices.push_back(i);
        }
    }

    std::vector<std::optional<SeparationCertificateResult>> ret(active_plane_indices.size(), std::nullopt);

    // This lambda function formulates and solves a small SOS program for each
    // pair of geometries.
    auto solve_small_sos = [this, &d_minus_Cs, &C_redundant_indices, &s_lower_redundant_indices,
        ...
        auto certificate_program =
                this->ConstructPlaneSearchProgram(this->plane_geometries_[plane_index], d_minus_Cs, C_redundant_indices,
                                                  s_lower_redundant_indices, s_upper_redundant_indices);
        solvers::MathematicalProgramResult result;
        solvers::MakeSolver(options.solver_id)
                ->Solve(*certificate_program.prog, std::nullopt, options.solver_options, &result);
        if (result.is_success()) {
            ret[plane_count].emplace(certificate_program.certificate.GetSolution(
                    plane_index, separating_planes()[plane_index].a, separating_planes()[plane_index].b,
    ...
    this->SolveCertificationForEachPlaneInParallel(active_plane_indices, solve_small_sos, options.parallelism,
...
bool CspaceFreePolytope::FindSeparationCertificateGivenPolytope(
        ...
        const Eigen::Ref<const Eigen::VectorXd>& d,
        const IgnoredCollisionPairs& ignored_collision_pairs,
        const CspaceFreePolytope::FindSeparationCertificateGivenPolytopeOptions& options,
        std::unordered_map<SortedPair<geometry::GeometryId>, CspaceFreePolytope::SeparationCertificateResult>*
                certificates) const {
    ...
    std::unordered_set<int> s_lower_redundant_indices;
    std::unordered_set<int> s_upper_redundant_indices;
    this->FindRedundantInequalities(C, d, s_lower_, s_upper_, 0., &C_redundant_indices, &s_lower_redundant_indices,
                                    &s_upper_redundant_indices);

    const std::vector<std::optional<SeparationCertificateResult>> certificates_vec =
            this->FindSeparationCertificateGivenPolytope(ignored_collision_pairs, C, d, options);
    ...
    for (const auto& certificate : certificates_vec) {
        if (certificate.has_value()) {
            ...
            certificates->emplace(SortedPair<geometry::GeometryId>(plane.positive_side_geometry->id(),
...
int CspaceFreePolytope::GetGramVarSizeForPolytopeSearchProgram(
    ...
    auto count_gram_per_rational =
            [this, search_s_bounds_lagrangians](
                    const symbolic::RationalFunction& rational,
                    const std::array<VectorX<symbolic::Monomial>, 4>& monomial_basis_array) -> int {
        ...
        return num_sos * internal::GetGramVarSize(monomial_basis_array, this->with_cross_y(), num_y);
    };
    return CspaceFreePolytopeBase::GetGramVarSizeForPolytopeSearchProgram(plane_geometries_, ignored_collision_pairs,
                                                                          count_gram_per_rational);
}

std::unique_ptr<solvers::MathematicalProgram> CspaceFreePolytope::InitializePolytopeSearchProgram(
        const IgnoredCollisionPairs& ignored_collision_pairs,
    ...
    prog->AddDecisionVariables(Eigen::Map<const VectorX<symbolic::Variable>>(C.data(), C.rows() * C.cols()));
    ...
    int gram_var_count = 0;
    for (int plane_index = 0; plane_index < static_cast<int>(separating_planes().size()); ++plane_index) {
        const auto& plane = separating_planes()[plane_index];
        const SortedPair<geometry::GeometryId> geometry_pair = plane.geometry_pair();
        if (!ignored_collision_pairs.contains(geometry_pair)) {
            prog->AddDecisionVariables(plane.decision_variables);
            const auto& certificate = certificates_vec[plane_to_certificate_map.at(plane_index)];
            ...
            if (new_certificates != nullptr) {
                auto insertion_pair = new_certificates->emplace(plane_index, SeparationCertificate());
            ...
            for (PlaneSide plane_side : {PlaneSide::kPositive, PlaneSide::kNegative}) {
                const auto& monomial_basis_array_given_side =
                        this->map_body_to_monomial_basis_array().at(SortedPair<multibody::BodyIndex>(
                                plane.expressed_body, plane.geometry(plane_side)->body_index()));
                const auto& rationals = plane_geometries_[plane_index].rationals(plane_side);
                const auto& lagrangians_vec = certificate->lagrangians(plane_side);
                std::vector<SeparatingPlaneLagrangians>* new_lagrangians_vec =
                        new_certificate == nullptr ? nullptr : &(new_certificate->mutable_lagrangians(plane_side));

                DRAKE_THROW_UNLESS(rationals.size() == lagrangians_vec.size());
                for (int i = 0; i < ssize(rationals); ++i) {
                    const int num_y = internal::GetNumYInRational(rationals[i], this->y_slack());
                    const int num_gram_vars_per_sos =
                            internal::GetGramVarSize(monomial_basis_array_given_side, this->with_cross_y(), num_y);
                    internal::GramAndMonomialBasis gram_and_monomial_basis(monomial_basis_array_given_side,
                                                                           this->with_cross_y(), num_y);
                    // Add Lagrangian multipliers for joint limits.
                    if (search_s_bounds_lagrangians) {
                        for (int j = 0; j < s_size; ++j) {
                            gram_and_monomial_basis.AddSos(prog.get(),
                    ...
                    if (new_lagrangians_vec != nullptr) {
                        ...
                        new_lagrangians_vec->back().mutable_polytope() = lagrangians_vec[i].polytope();
                        new_lagrangians_vec->back().mutable_s_lower() = s_lower_lagrangians;
                        new_lagrangians_vec->back().mutable_s_upper() = s_upper_lagrangians;
                    }

                    const symbolic::Polynomial poly = rationals[i].numerator() -
                    ...
                    symbolic::Polynomial poly_sos;
                    gram_and_monomial_basis.AddSos(prog.get(), gram_vars.segment(gram_var_count, num_gram_vars_per_sos),
                                                   &poly_sos);
                    gram_var_count += num_gram_vars_per_sos;
                    prog->AddEqualityConstraintBetweenPolynomials(poly, poly_sos);
                }
...
std::unique_ptr<solvers::MathematicalProgram> CspaceFreePolytope::InitializePolytopeSearchProgram(
    ...
    const int s_size = rational_forward_kin().s().rows();
    const int C_rows = certificates.begin()->second.positive_side_rational_lagrangians[0].polytope().rows();
    *C = symbolic::MakeMatrixContinuousVariable(C_rows, s_size, "C");
    *d = symbolic::MakeVectorContinuousVariable(C_rows, "d");
    const VectorX<symbolic::Polynomial> d_minus_Cs = this->CalcDminusCs<symbolic::Variable>(*C, *d);
    ...
    for (const auto& plane : separating_planes()) {
        const SortedPair<geometry::GeometryId> geometry_pair(plane.positive_side_geometry->id(),
                                                             plane.negative_side_geometry->id());
        if (!ignored_collision_pairs.contains(geometry_pair)) {
            ...
            if (it == certificates.end()) {
                ...
                throw std::runtime_error(fmt::format(
                        "InitializePolytopeSearchProgram: certificates doesn't "
                        "contain result for the geometry pair ({}, {})",
                        inspector.GetName(geometry_pair.first()), inspector.GetName(geometry_pair.second())));
            }
            certificates_vec.emplace_back(it->second);
        }
    }
    const int gram_total_size =
            this->GetGramVarSizeForPolytopeSearchProgram(ignored_collision_pairs, search_s_bounds_lagrangians);
    return this->InitializePolytopeSearchProgram(ignored_collision_pairs, *C, *d, d_minus_Cs, certificates_vec,
                                                 search_s_bounds_lagrangians, gram_total_size, new_certificates);
}

void CspaceFreePolytope::AddEllipsoidContainmentConstraint(solvers::MathematicalProgram* prog,
    ...
    A_lorentz2.bottomRows(C.cols()) = Eigen::MatrixXd::Identity(C.cols(), C.cols());
    Eigen::VectorXd b_lorentz2 = Eigen::VectorXd::Zero(1 + C.cols());
    b_lorentz2(0) = 1;
    for (int i = 0; i < C.rows(); ++i) {
        prog->AddLorentzConeConstraint(A_lorentz2, b_lorentz2, C.row(i).transpose());
    }
}

void CspaceFreePolytope::AddCspacePolytopeContainment(solvers::MathematicalProgram* prog,
                                                      const MatrixX<symbolic::Variable>& C,
                                                      const VectorX<symbolic::Variable>& d,
                                                      const Eigen::MatrixXd& s_inner_pts) const {
    DRAKE_THROW_UNLESS(s_inner_pts.rows() == this->rational_forward_kin().s().rows());
    ...
    Eigen::MatrixXd A = Eigen::MatrixXd::Zero(s_inner_pts.cols() * C.rows(), (s_inner_pts.rows() + 1) * C.rows());
    VectorX<symbolic::Variable> vars(A.cols());
    for (int i = 0; i < C.rows(); ++i) {
        A.block(i * s_inner_pts.cols(), i * (s_inner_pts.rows() + 1), s_inner_pts.cols(), s_inner_pts.rows()) =
                s_inner_pts.transpose();
        A.block(i * s_inner_pts.cols(), i * (s_inner_pts.rows() + 1) + s_inner_pts.rows(), s_inner_pts.cols(), 1) =
                -Eigen::VectorXd::Ones(s_inner_pts.cols());
        vars.segment((s_inner_pts.rows() + 1) * i, s_inner_pts.rows()) = C.row(i).transpose();
        vars((s_inner_pts.rows() + 1) * i + s_inner_pts.rows()) = d(i);
    }
    prog->AddLinearConstraint(A, Eigen::VectorXd::Constant(A.rows(), -kInf), Eigen::VectorXd::Zero(A.rows()), vars);
}

void CspaceFreePolytope::SearchResult::SetPolytope(const Eigen::Ref<const Eigen::MatrixXd>& C,
                                                   const Eigen::Ref<const Eigen::VectorXd>& d,
...
void CspaceFreePolytope::SearchResult::SetSeparatingPlanes(std::unordered_map<int, Vector3<symbolic::Polynomial>> a,
...
void CspaceFreePolytope::SearchResult::SetSeparatingPlanes(
        const std::vector<std::optional<SeparationCertificateResult>>& certificates_result) {
    a_.clear();
    b_.clear();
...
std::vector<CspaceFreePolytope::SearchResult> CspaceFreePolytope::SearchWithBilinearAlternation(
        const IgnoredCollisionPairs& ignored_collision_pairs,
    ...
    const MatrixX<symbolic::Variable> C_var = symbolic::MakeMatrixContinuousVariable(C_init.rows(), C_init.cols(), "C");
    const VectorX<symbolic::Variable> d_var = symbolic::MakeVectorContinuousVariable(d_init.rows(), "d");
    const VectorX<symbolic::Variable> ellipsoid_margins =
            symbolic::MakeVectorContinuousVariable(C_init.rows(), "ellipsoid_margin");
    const VectorX<symbolic::Polynomial> d_minus_Cs = this->CalcDminusCs<symbolic::Variable>(C_var, d_var);
    const int gram_total_size_in_polytope_program = this->GetGramVarSizeForPolytopeSearchProgram(
            ignored_collision_pairs, options.find_polytope_options.search_s_bounds_lagrangians);
    ...
    Eigen::MatrixXd ellipsoid_Q = options.ellipsoid_scaling * (ellipsoid.A().inverse());
    double prev_cost = ellipsoid_Q.determinant();
    drake::log()->debug("det(Q) at the beginning is {}", prev_cost);
    while (iter < options.max_iter) {
        const std::vector<std::optional<SeparationCertificateResult>> certificates_result =
                this->FindSeparationCertificateGivenPolytope(ignored_collision_pairs, C, d,
                                                             options.find_lagrangian_options);
        if (std::any_of(certificates_result.begin(), certificates_result.end(),
        ...
        // planes.
        const std::optional<FindPolytopeGivenLagrangianResult> polytope_result = this->FindPolytopeGivenLagrangian(
                ignored_collision_pairs, C_var, d_var, d_minus_Cs, certificates_result, ellipsoid_Q, ellipsoid.center(),
                ellipsoid_margins, gram_total_size_in_polytope_program, options.find_polytope_options,
                nullptr /* certificates_result */);
        if (polytope_result.has_value()) {
            C = polytope_result->C;
            d = polytope_result->d;
            ret.back().SetPolytope(polytope_result->C, polytope_result->d, *this);
            ret.back().SetSeparatingPlanes(std::move(polytope_result->a), std::move(polytope_result->b));
            ret.back().num_iter_ = iter;
            // Now find the inscribed ellipsoid.
            ellipsoid = ret.back().certified_polytope().MaximumVolumeInscribedEllipsoid();
...
std::optional<CspaceFreePolytope::SearchResult> CspaceFreePolytope::BinarySearch(
        const IgnoredCollisionPairs& ignored_collision_pairs,
        const Eigen::Ref<const Eigen::MatrixXd>& C,
        const Eigen::Ref<const Eigen::VectorXd>& d_init,
    ...
    if ((C_row_norm == 0).any()) {
        throw std::runtime_error("C contains rows with all 0 entries. Please remove these rows.");
    }
    ...
    std::vector<double> geometry_pair_scale_lower_bounds(this->separating_planes().size(), 0);
    ...
    // is feasible.
    auto is_scale_feasible = [this, &ignored_collision_pairs, &C, &d_init, &s_center, &options,
                              &geometry_pair_scale_lower_bounds, &ret](double scale) {
        ...
        CspaceFreePolytope::IgnoredCollisionPairs ignored_collision_pairs_for_scale = ignored_collision_pairs;
        for (int i = 0; i < static_cast<int>(separating_planes().size()); ++i) {
            const auto& plane = separating_planes()[i];
            const SortedPair<geometry::GeometryId> geometry_pair(plane.positive_side_geometry->id(),
                                                                 plane.negative_side_geometry->id());
            if (!ignored_collision_pairs.contains(geometry_pair) && geometry_pair_scale_lower_bounds[i] >= scale) {
                ignored_collision_pairs_for_scale.insert(geometry_pair);
            }
        }
        const std::vector<std::optional<SeparationCertificateResult>> certificates_result =
                this->FindSeparationCertificateGivenPolytope(ignored_collision_pairs_for_scale, C, d,
                                                             options.find_lagrangian_options);
    ...
    if (!is_scale_feasible(options.scale_min)) {
        drake::log()->debug("CspaceFreePolytope::BinarySearch(): scale_min={} is infeasible.", options.scale_min);
        return std::nullopt;
    }
    if (is_scale_feasible(options.scale_max)) {
        drake::log()->debug("CspaceFreePolytope::BinarySearch(): scale_max={} is feasible.", options.scale_max);
        ret.num_iter_ = 0;
        return ret;
    }
    ...
    int iter = 0;
    while (scale_max - scale_min > options.convergence_tol && iter < options.max_iter) {
        const double scale = (scale_max + scale_min) / 2;
        if (is_scale_feasible(scale)) {
            drake::log()->debug("CspaceFreePolytope::BinarySearch(): scale={} is feasible", scale);
            scale_min = scale;
        } else {
            drake::log()->debug("CspaceFreePolytope::BinarySearch(): scale={} is infeasible", scale);
            scale_max = scale;
        }
        ++iter;
    }
...
CspaceFreePolytope::SeparationCertificateProgram CspaceFreePolytope::MakeIsGeometrySeparableProgram(
        const SortedPair<geometry::GeometryId>& geometry_pair,
        const Eigen::Ref<const Eigen::MatrixXd>& C,
        const Eigen::Ref<const Eigen::VectorXd>& d) const {
    const VectorX<symbolic::Polynomial> d_minus_Cs = this->CalcDminusCs<double>(C, d);
    auto geometry_pair_it = map_geometries_to_separating_planes().find(geometry_pair);
    if (geometry_pair_it == map_geometries_to_separating_planes().end()) {
        throw std::runtime_error(
                fmt::format("MakeIsGeometrySeparableProgram(): geometry pair ({}, {}) does not "
                            "need "
    ...
    std::unordered_set<int> s_lower_redundant_indices;
    std::unordered_set<int> s_upper_redundant_indices;
    this->FindRedundantInequalities(C, d, this->s_lower_, this->s_upper_, 0 /* tighten */, &C_redundant_indices,
                                    &s_lower_redundant_indices, &s_upper_redundant_indices);
    return this->ConstructPlaneSearchProgram(this->plane_geometries_[plane_index], d_minus_Cs, C_redundant_indices,
                                             s_lower_redundant_indices, s_upper_redundant_indices);
}

std::optional<CspaceFreePolytope::SeparationCertificateResult> CspaceFreePolytope::SolveSeparationCertificateProgram(
    ...
    DRAKE_THROW_UNLESS(certificate_program.plane_index >= 0 &&
                       certificate_program.plane_index < static_cast<int>(this->separating_planes().size()));

    internal::SolveSeparationCertificateProgramBase(
            certificate_program, options, separating_planes()[certificate_program.plane_index], &(ret.value()));
    std::optional<CspaceFreePolytope::SeparationCertificateResult> ret_optional{std::nullopt};
    if (ret->result.is_success()) {
        // Now set the Lagrangians of the result.
        auto set_lagrangians =
                [&ret](const std::vector<CspaceFreePolytope::SeparatingPlaneLagrangians>& lagrangians_vec,
                       std::vector<CspaceFreePolytope::SeparatingPlaneLagrangians>* lagrangians_result) {
                    lagrangians_result->reserve(lagrangians_vec.size());
                    for (const auto& lagrangians : lagrangians_vec) {
                        lagrangians_result->push_back(lagrangians.GetSolution(ret.value().result));
                    }
                };
        set_lagrangians(certificate_program.certificate.positive_side_rational_lagrangians,
                        &ret.value().positive_side_rational_lagrangians);
        set_lagrangians(certificate_program.certificate.negative_side_rational_lagrangians,
...
std::optional<CspaceFreePolytope::FindPolytopeGivenLagrangianResult> CspaceFreePolytope::FindPolytopeGivenLagrangian(
        const IgnoredCollisionPairs& ignored_collision_pairs,
        const MatrixX<symbolic::Variable>& C,
        const VectorX<symbolic::Variable>& d,
        const VectorX<symbolic::Polynomial>& d_minus_Cs,
        ...
        std::unordered_map<int, SeparationCertificateResult>* certificates_result) const {
    std::unordered_map<int, SeparationCertificate> new_certificates_map;
    auto prog = this->InitializePolytopeSearchProgram(ignored_collision_pairs, C, d, d_minus_Cs, certificates_vec,
    ...
    if (options.s_inner_pts.has_value()) {
        for (int i = 0; i < options.s_inner_pts->cols(); ++i) {
            DRAKE_THROW_UNLESS((options.s_inner_pts->col(i).array() <= s_upper_.array()).all());
            DRAKE_THROW_UNLESS((options.s_inner_pts->col(i).array() >= s_lower_.array()).all());
        }
    ...
    switch (options.ellipsoid_margin_cost) {
        case CspaceFreePolytope::EllipsoidMarginCost::kSum:
            // Maximize ∑ᵢ δᵢ
            prog->AddLinearCost(-Eigen::VectorXd::Ones(ellipsoid_margins.rows()), 0, ellipsoid_margins);
            break;
        case CspaceFreePolytope::EllipsoidMarginCost::kGeometricMean:
            // Maximize ∏ᵢ (δᵢ + ε)
            prog->AddMaximizeGeometricMeanCost(
                    Eigen::MatrixXd::Identity(ellipsoid_margins.rows(), ellipsoid_margins.rows()),
                    Eigen::VectorXd::Constant(ellipsoid_margins.rows(), options.ellipsoid_margin_epsilon),
                    ellipsoid_margins);
            break;
    }

    const solvers::MathematicalProgramResult result =
            internal::SolveWithBackoff(prog.get(), options.backoff_scale, options.solver_options, options.solver_id);
    if (result.is_success()) {
        CspaceFreePolytope::FindPolytopeGivenLagrangianResult ret;
        ret.C = result.GetSolution(C);
        ret.d = result.GetSolution(d);
        for (int plane_index = 0; plane_index < static_cast<int>(this->separating_planes().size()); ++plane_index) {
                ...
                if (!ignored_collision_pairs.contains(SortedPair<geometry::GeometryId>(
                    ...
                    certificates_result->emplace(
                            ...
                            new_certificates_map.at(plane_index)
                                    .GetSolution(plane_index, separating_planes()[plane_index].a,
...
void CspaceFreePolytope::FindRedundantInequalities(const Eigen::MatrixXd& C,
    ...
    C_bar << C, Eigen::MatrixXd::Identity(ns, ns), -Eigen::MatrixXd::Identity(ns, ns);
    ...
    for (const int index : redundant_indices) {
        if (index < C.rows()) {
            ...
            s_upper_redundant_indices->emplace_hint(s_upper_redundant_indices->end(), index - C.rows());
        } else {
            s_lower_redundant_indices->emplace_hint(s_lower_redundant_indices->end(), index - C.rows() - ns);
        }
    }
}

template <typename T>
VectorX<symbolic::Polynomial> CspaceFreePolytope::CalcDminusCs(const Eigen::Ref<const MatrixX<T>>& C,
                                                               const Eigen::Ref<const VectorX<T>>& d) const {
    // Now build the polynomials d(i) - C.row(i) * s
    const auto& s = rational_forward_kin().s();
    DRAKE_THROW_UNLESS(C.rows() == d.rows() && C.cols() == static_cast<int>(s.size()));
...
HPolyhedron CspaceFreePolytope::GetPolyhedronWithJointLimits(const Eigen::MatrixXd& C, const Eigen::VectorXd& d) const {
    const int s_size = rational_forward_kin().s().rows();
...
template VectorX<symbolic::Polynomial> CspaceFreePolytope::CalcDminusCs<double>(
        const Eigen::Ref<const Eigen::MatrixXd>&, const Eigen::Ref<const Eigen::VectorXd>&) const;
template VectorX<symbolic::Polynomial> CspaceFreePolytope::CalcDminusCs<symbolic::Variable>(
