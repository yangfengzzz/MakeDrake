...
#include "solvers/create_cost.h"
#include "solvers/get_program_type.h"
...
MathematicalProgramResult Solve(const MathematicalProgram& prog,
    ...
    if (preprocessing && options.preprocessing_solver && options.preprocessing_solver_options) {
        options.preprocessing_solver->Solve(prog, {}, options.preprocessing_solver_options, &result);
    } else if (preprocessing && options.preprocessing_solver && !options.preprocessing_solver_options) {
        options.preprocessing_solver->Solve(prog, {}, options.solver_options, &result);
...
struct EdgeIdComparator {
    bool operator()(const Edge* lhs, const Edge* rhs) const { return lhs->id() < rhs->id(); }
};
...
Vertex::Vertex(VertexId id, const ConvexSet& set, std::string name)
    : id_(id),
      set_(set.Clone()),
      name_(std::move(name)),
      placeholder_x_(symbolic::MakeVectorContinuousVariable(set_->ambient_dimension(), name_)) {}

Vertex::~Vertex() = default;

std::pair<Variable, Binding<Cost>> Vertex::AddCost(const symbolic::Expression& e,
...
std::pair<Variable, Binding<Cost>> Vertex::AddCost(const Binding<Cost>& binding,
                                                   const std::unordered_set<Transcription>& use_in_transcription) {
    DRAKE_THROW_UNLESS(Variables(binding.variables()).IsSubsetOf(Variables(placeholder_x_)));
...
Binding<Constraint> Vertex::AddConstraint(
        const symbolic::Formula& f, const std::unordered_set<GraphOfConvexSets::Transcription>& use_in_transcription) {
    return AddConstraint(solvers::internal::ParseConstraint(f), use_in_transcription);
}

Binding<Constraint> Vertex::AddConstraint(
        const Binding<Constraint>& binding,
        const std::unordered_set<GraphOfConvexSets::Transcription>& use_in_transcription) {
...
std::vector<solvers::Binding<solvers::Cost>> Vertex::GetCosts(
        const std::unordered_set<GraphOfConvexSets::Transcription>& used_in_transcription) const {
    ...
    for (const auto& [binding, transcriptions] : costs_) {
        if (std::any_of(transcriptions.begin(), transcriptions.end(), [&used_in_transcription](const auto& elem) {
...
void Vertex::RemoveIncomingEdge(Edge* e) {
    incoming_edges_.erase(std::remove(incoming_edges_.begin(), incoming_edges_.end(), e), incoming_edges_.end());
}

void Vertex::RemoveOutgoingEdge(Edge* e) {
    outgoing_edges_.erase(std::remove(outgoing_edges_.begin(), outgoing_edges_.end(), e), outgoing_edges_.end());
}
...
Edge::Edge(const EdgeId& id, Vertex* u, Vertex* v, std::string name)
    : id_{id},
      ...
      name_(std::move(name)),
      y_{symbolic::MakeVectorContinuousVariable(u_->ambient_dimension(), name_ + "y")},
      z_{symbolic::MakeVectorContinuousVariable(v_->ambient_dimension(), name_ + "z")},
...
std::pair<Variable, Binding<Cost>> Edge::AddCost(const symbolic::Expression& e,
                                                 const std::unordered_set<Transcription>& use_in_transcription) {
    return AddCost(solvers::internal::ParseCost(e), use_in_transcription);
}
...
std::pair<Variable, Binding<Cost>> Edge::AddCost(const Binding<Cost>& binding,
    ...
    const int n = ell_.size();
    ell_.conservativeResize(n + 1);
    ell_[n] = Variable(fmt::format("{}ell{}", name_, n), Variable::Type::CONTINUOUS);
    costs_.push_back({binding, use_in_transcription});
    return std::pair<Variable, Binding<Cost>>(ell_[n], binding);
}
...
Eigen::VectorXd Edge::GetSolutionPhiXu(const solvers::MathematicalProgramResult& result) const {
    return result.GetSolution(y_);
}

Eigen::VectorXd Edge::GetSolutionPhiXv(const solvers::MathematicalProgramResult& result) const {
...
std::string GraphOfConvexSets::GetGraphvizString(const std::optional<solvers::MathematicalProgramResult>& result,
                                                 bool show_slacks,
                                                 int precision,
    ...
    for (const auto& [e_id, e] : edges_) {
        ...
        if (result) {
            ...
            if (e->ell_.size() > 0) {
                ...
                if (result->get_decision_variable_index()->contains(e->ell_[0].get_id())) {
            ...
            if (show_slacks) {
                ...
                if (result->get_decision_variable_index()->contains(e->y_[0].get_id())) {
...
std::set<EdgeId> GraphOfConvexSets::PreprocessShortestPath(VertexId source_id,
                                                           VertexId target_id,
                                                           const GraphOfConvexSetsOptions& options) const {
    if (vertices_.find(source_id) == vertices_.end()) {
        throw std::runtime_error(fmt::format("Source vertex {} is not a vertex in this GraphOfConvexSets.", source_id));
    }
    if (vertices_.find(target_id) == vertices_.end()) {
        throw std::runtime_error(fmt::format("Target vertex {} is not a vertex in this GraphOfConvexSets.", target_id));
    }
    ...
    Binding<solvers::BoundingBoxConstraint> f_limits = prog.AddBoundingBoxConstraint(0, 1, f);
    VectorXDecisionVariable g = prog.NewContinuousVariables(nE, "flow_vt");
    Binding<solvers::BoundingBoxConstraint> g_limits = prog.AddBoundingBoxConstraint(0, 1, g);
    ...
    for (const auto& [vertex_id, v] : vertices_) {
        ...
        if (Ev.size() > 0) {
            ...
            A_flow << RowVectorXd::Ones(Ev_in.size()), -1 * RowVectorXd::Ones(Ev_out.size());
            ...
            if (vertex_id == source_id) {
                conservation_f.insert({vertex_id, prog.AddLinearEqualityConstraint(A_flow, -1, fv)});
            } else {
                conservation_f.insert({vertex_id, prog.AddLinearEqualityConstraint(A_flow, 0, fv)});
            }

            // Conservation of flow for g: ∑ g_in - ∑ g_out = δ(is_target).
            if (vertex_id == target_id) {
                conservation_g.insert({vertex_id, prog.AddLinearEqualityConstraint(A_flow, 1, gv)});
            } else {
                conservation_g.insert({vertex_id, prog.AddLinearEqualityConstraint(A_flow, 0, gv)});
            }
        }
    ...
    for (const auto& [edge_id, e] : edges_) {
        ...
        if (e->u().id() == source_id) {
            ...
            conservation_f.at(e->u().id()).evaluator()->set_bounds(Vector1d(0), Vector1d(0));
        } else {
            conservation_f.at(e->u().id()).evaluator()->set_bounds(Vector1d(1), Vector1d(1));
        }
        // ∑ g_in,v - ∑ f_out,v = δ(is_target) - 1.
        if (e->v().id() == target_id) {
            g_limits.evaluator()->set_bounds(VectorXd::Zero(nE), VectorXd::Zero(nE));
            conservation_g.at(e->v().id()).evaluator()->set_bounds(Vector1d(0), Vector1d(0));
        } else {
            conservation_g.at(e->v().id()).evaluator()->set_bounds(Vector1d(-1), Vector1d(-1));
        }
...
void GraphOfConvexSets::AddPerspectiveCost(MathematicalProgram* prog,
    ...
    if (LinearCost* lc = dynamic_cast<LinearCost*>(cost)) {
        ...
        if (lc->a().isZero() && lc->b() < 0.0) {
            throw std::runtime_error(fmt::format("Constant costs must be non-negative: {}", binding.to_string()));
        }
        ...
        Eigen::MatrixXd R = math::DecomposePSDmatrixIntoXtransposeTimesX(.5 * qc->Q(), tol);
        ...
        if (minimum[0] < -tol) {
            throw std::runtime_error(
                    fmt::format("In order to prevent negative edge lengths, quadratic "
        ...
        prog->AddRotatedLorentzConeConstraint(A_cone, VectorXd::Zero(A_cone.rows()), vars);
        ...
        A_linear.block(0, A_cols + 2, A_rows, l1c_slack.size()) = -MatrixXd::Identity(A_rows, A_rows);  // -δ.

        // -δᵢ ≤ Aᵢx+bᵢϕ
        A_linear.block(A_rows, 0, A_rows, 1) = -l1c->b();                                                    // -bϕ.
        A_linear.block(A_rows, 2, A_rows, A_cols) = -l1c->A();                                               // -Ax.
        A_linear.block(A_rows, A_cols + 2, A_rows, l1c_slack.size()) = -MatrixXd::Identity(A_rows, A_rows);  // -δ.

        // ℓ ≥ Σᵢ δᵢ
        A_linear(2 * A_rows, 1) = -1;
        A_linear.block(2 * A_rows, A_cols + 2, 1, l1c_slack.size()) = RowVectorXd::Ones(l1c_slack.size());
        prog->AddLinearConstraint(A_linear, VectorXd::Constant(A_linear.rows(), -inf), VectorXd::Zero(A_linear.rows()),
                                  cost_vars);
    } else if (L2NormCost* l2c = dynamic_cast<L2NormCost*>(cost)) {
        // |Ax + b|₂ becomes ℓ ≥ |Ax+bϕ|₂.
        MatrixXd A_cone = MatrixXd::Zero(l2c->get_sparse_A().rows() + 1, vars.size());
        A_cone(0, 1) = 1.0;                                            // z₀ = ℓ.
        A_cone.block(1, 0, l2c->get_sparse_A().rows(), 1) = l2c->b();  // bϕ.
        A_cone.block(1, 2, l2c->get_sparse_A().rows(), l2c->get_sparse_A().cols()) = l2c->get_sparse_A();  // Ax.
        ...
        prog->AddLinearConstraint(A_linear, VectorXd::Constant(A_linear.rows(), -inf), VectorXd::Zero(A_linear.rows()),
                                  vars);
    } else if (PerspectiveQuadraticCost* pqc = dynamic_cast<PerspectiveQuadraticCost*>(cost)) {
        ...
        prog->AddRotatedLorentzConeConstraint(A_cone, VectorXd::Zero(pqc->A().rows() + 1), vars);
    } else {
        throw std::runtime_error(
                fmt::format("GraphOfConvexSets::Edge does not support this binding type: {}", binding.to_string()));
    }
}

void GraphOfConvexSets::AddPerspectiveConstraint(MathematicalProgram* prog,
                                                 const Binding<Constraint>& binding,
                                                 const VectorXDecisionVariable& vars) const {
    const double inf = std::numeric_limits<double>::infinity();

    Constraint* constraint = binding.evaluator().get();
    if (LinearEqualityConstraint* lec = dynamic_cast<LinearEqualityConstraint*>(constraint)) {
        ...
        prog->AddConstraint(
                CreateBinding(std::make_shared<LinearEqualityConstraint>(Aeq, VectorXd::Zero(A.rows())), vars));
    ...
    } else if (LinearConstraint* lc = dynamic_cast<LinearConstraint*>(constraint)) {
        // lb <= A*x <= ub becomes
        ...
        if (lc->upper_bound().array().isFinite().all()) {
            ...
            prog->AddConstraint(
                    CreateBinding(std::make_shared<LinearConstraint>(Ac, VectorXd::Constant(Ac.rows(), -inf),
                                                                     VectorXd::Zero(Ac.rows())),
                                  vars));
        } else if (lc->upper_bound().array().isInf().all() && (lc->upper_bound().array() > 0).all()) {
        ...
        if (lc->lower_bound().array().isFinite().all()) {
            ...
            Ac.rightCols(A.cols()) = A;
            prog->AddConstraint(CreateBinding(std::make_shared<LinearConstraint>(Ac, VectorXd::Zero(Ac.rows()),
                                                                                 VectorXd::Constant(Ac.rows(), inf)),
                                              vars));
        } else if (lc->lower_bound().array().isInf().all() && (lc->lower_bound().array() < 0).all()) {
    ...
    } else if (LorentzConeConstraint* lcc = dynamic_cast<LorentzConeConstraint*>(constraint)) {
        ...
        prog->AddLorentzConeConstraint(A_cone, VectorXd::Zero(lcc->A().rows()), vars);
    } else if (RotatedLorentzConeConstraint* rc = dynamic_cast<RotatedLorentzConeConstraint*>(constraint)) {
        // z ∈ K for z = Ax + b becomes
        // z ∈ K for z = Ax + bϕ = [b A] [ϕ; x]
        ...
        prog->AddRotatedLorentzConeConstraint(A_cone, VectorXd::Zero(rc->A().rows()), vars);
    } else if (dynamic_cast<PositiveSemidefiniteConstraint*>(constraint) != nullptr) {
...
MathematicalProgramResult GraphOfConvexSets::SolveShortestPath(
        const Vertex& source, const Vertex& target, const GraphOfConvexSetsOptions& specified_options) const {
    ...
    auto IncludesCurrentTranscription = [&options](const std::unordered_set<Transcription>& transcriptions) -> bool {
        return ((*options.convex_relaxation && transcriptions.contains(Transcription::kRelaxation)) ||
                (!*options.convex_relaxation && transcriptions.contains(Transcription::kMIP)));
    };
    ...
    for (const auto& [edge_id, e] : edges_) {
        ...
        if (e->phi_value_.has_value()) {
            ...
            prog.AddLinearEqualityConstraint(Vector1d(1.0), phi_value, Vector1<Variable>(phi));
        }
    ...
    if (!has_edges_out_of_source) {
        MathematicalProgramResult result;
        log()->info("Source vertex {} ({}) has no outgoing edges.", source.name(), source_id);
        result.set_solution_result(SolutionResult::kInfeasibleConstraints);
        return result;
    }
    if (!has_edges_into_target) {
        MathematicalProgramResult result;
        log()->info("Target vertex {} ({}) has no incoming edges.", target.name(), target_id);
        result.set_solution_result(SolutionResult::kInfeasibleConstraints);
        return result;
    }

    for (const std::pair<const VertexId, std::unique_ptr<Vertex>>& vpair : vertices_) {
        ...
        if (incoming.size() + outgoing.size() > 0) {  // in degree + out degree
            ...
            a << RowVectorXd::Constant(incoming.size(), -1.0), RowVectorXd::Ones(outgoing.size());

            // Conservation of flow: ∑ ϕ_out - ∑ ϕ_in = δ(is_source) - δ(is_target).
            int count = 0;
            for (const Edge* e : incoming) {
                vars[count++] = *options.convex_relaxation ? relaxed_phi.at(e->id()) : e->phi_;
            }
            for (const Edge* e : outgoing) {
                vars[count++] = *options.convex_relaxation ? relaxed_phi.at(e->id()) : e->phi_;
            }
            prog.AddLinearEqualityConstraint(a, (is_source ? 1.0 : 0.0) - (is_target ? 1.0 : 0.0), vars);
        ...
        if (outgoing.size() > 0) {
            ...
            for (int i = 0; i < static_cast<int>(outgoing.size()); ++i) {
                phi_out[i] = *options.convex_relaxation ? relaxed_phi.at(outgoing[i]->id()) : outgoing[i]->phi_;
                yz_out.segment(i * n_v, n_v) = outgoing[i]->y_;
            }
            // Degree constraint: ∑ ϕ_out <= 1- δ(is_target).
            prog.AddLinearConstraint(RowVectorXd::Ones(outgoing.size()), 0.0, is_target ? 0.0 : 1.0, phi_out);
                ...
                for (int i = 0; i < static_cast<int>(outgoing.size()); ++i) {
                    ...
                    for (const Edge* e_in : incoming) {
                        if (e_in->u().id() == e_out->v().id()) {
                            a[i] = -1.0;
                            phi_out[i] = *options.convex_relaxation ? relaxed_phi.at(e_in->id()) : e_in->phi_;
                            ...
                            v->set().AddPointInNonnegativeScalingConstraints(&prog, A_yz, VectorXd::Zero(n_v), a, 0,
                                                                             yz_out, phi_out);

                            a[i] = 1.0;
                            phi_out[i] = *options.convex_relaxation ? relaxed_phi.at(e_out->id()) : e_out->phi_;
                            A_yz.block(0, i * n_v, n_v, n_v) = MatrixXd::Identity(n_v, n_v);
        ...
        if (v->ell_.size() > 0) {
            for (int ii = 0; ii < v->ell_.size(); ++ii) {
                ...
                if (IncludesCurrentTranscription(transcriptions)) {
                    VectorXDecisionVariable vertex_ell = prog.NewContinuousVariables(cost_edges.size());
    ...
    log()->info(
            ...
            "preprocessing={}{}.",
            result.get_solver_id().name(), *options.convex_relaxation, *options.preprocessing,
            *options.convex_relaxation && *options.max_rounded_paths > 0 ? " and rounding" : " and no rounding");
    ...
    {  // Push the placeholder variables and excluded edge variables into the
        ...
        std::unordered_map<symbolic::Variable::Id, int> decision_variable_index = prog.decision_variable_index();
        ...
        for (const std::pair<const VertexId, std::unique_ptr<Vertex>>& vpair : vertices_) {
            ...
            if (is_target) {
                ...
                for (const auto& e : outgoing_edges[v->id()]) {
                    ...
                    sum_phi += result.GetSolution(*options.convex_relaxation ? relaxed_phi.at(e->id()) : e->phi_);
                }
            }
            ...
            if (sum_phi < 100.0 * std::numeric_limits<double>::epsilon()) {
                x_v = VectorXd::Constant(v->ambient_dimension(), std::numeric_limits<double>::quiet_NaN());
            ...
            for (int i = 0; i < v->ell_.size(); ++i) {
                ...
                if (IncludesCurrentTranscription(transcriptions)) {
                    x_val[count++] = result.GetSolution(vertex_edge_ell.at(v->id())[active_ell++]).sum();
                } else {
                    x_val[count++] = 0.0;
                }
            }
    ...
    // "Motion Planning around Obstacles with Convex Optimization":
    // https://arxiv.org/abs/2205.04422
    if (*options.convex_relaxation && *options.max_rounded_paths > 0 && result.is_success()) {
        ...
        while (static_cast<int>(paths.size()) < *options.max_rounded_paths &&
            ...
            while (path_vertex_ids.back() != target_id) {
                ...
                for (const Edge* e : outgoing_edges[path_vertex_ids.back()]) {
                    if (std::find(visited_vertex_ids.begin(), visited_vertex_ids.end(), e->v().id()) ==
                                visited_vertex_ids.end() &&
            ...
            // Optimize path
            MathematicalProgramResult rounded_result = SolveConvexRestriction(new_path, options, &result);
            ++num_solves;

            // Check path quality.
            if (rounded_result.is_success() &&
                (!best_rounded_result.is_success() ||
                 rounded_result.get_optimal_cost() < best_rounded_result.get_optimal_cost())) {
...
std::vector<const Edge*> GraphOfConvexSets::GetSolutionPath(const Vertex& source,
                                                            const Vertex& target,
                                                            const solvers::MathematicalProgramResult& result,
                                                            double tolerance) const {
    if (!result.is_success()) {
        throw std::runtime_error("Cannot extract a solution path when result.is_success() is false.");
    }
    if (!vertices_.contains(source.id())) {
        throw std::invalid_argument(
                fmt::format("Source vertex {} is not a vertex in this GraphOfConvexSets.", source.name()));
    }
    if (!vertices_.contains(target.id())) {
        throw std::invalid_argument(
                fmt::format("Target vertex {} is not a vertex in this GraphOfConvexSets.", target.name()));
    }
    ...
    while (path_vertices.back() != &target) {
        ...
        for (const Edge* e : path_vertices.back()->outgoing_edges()) {
            ...
            if (flow >= 1 - tolerance && flow > maximum_flow && !visited_vertices.contains(&e->v())) {
                maximum_flow = flow;
                max_flow_vertex = &e->v();
                max_flow_edge = e;
            }
        ...
        if (max_flow_edge == nullptr) {
            ...
            if (path_vertices.empty()) {
                throw std::runtime_error(fmt::format("No path found from {} to {}.", source.name(), target.name()));
            }
            path_edges.pop_back();
            continue;
        } else {
...
void RewriteForConvexSolver(MathematicalProgram* prog) {
    ...
    for (const auto& binding : prog->generic_costs()) {
        ...
        if (const auto* l1c = dynamic_cast<const L1NormCost*>(cost)) {
            ...
            prog->AddLinearConstraint(A, VectorXd::Constant(m, -kInf), -l1c->b(), {binding.variables(), slack});
            // -(Ax + b) ≤ slack, written as [A,I][x;slack] ≥ -b.
            A.rightCols(m) = MatrixXd::Identity(m, m);
            prog->AddLinearConstraint(A, -l1c->b(), VectorXd::Constant(m, kInf), {binding.variables(), slack});
            to_remove.insert(binding);
            ...
            prog->AddLinearConstraint(A, VectorXd::Constant(m, -kInf), -linfc->b(), {binding.variables(), slack});
            // ∀i, -(aᵢᵀx + bᵢ) ≤ slack, written as [A,1][x;slack] ≥ -b.
            A.col(A.cols() - 1) = VectorXd::Ones(m);
            prog->AddLinearConstraint(A, -linfc->b(), VectorXd::Constant(m, kInf), {binding.variables(), slack});
            to_remove.insert(binding);
        } else if (const auto* pqc = dynamic_cast<const PerspectiveQuadraticCost*>(cost)) {
...
MathematicalProgramResult GraphOfConvexSets::SolveConvexRestriction(
    ...
    if (restriction_options.restriction_solver_options) {
        restriction_options.solver_options = *restriction_options.restriction_solver_options;
    }
    MathematicalProgram prog;
    ...
    for (const auto* e : active_edges) {
        if (!edges_.contains(e->id())) {
            throw std::runtime_error(fmt::format("Edge {} is not in the graph.", e->name()));
        }
    ...
    for (const auto& pair : edges_) {
        ...
        decision_variable_index.emplace(e->phi_.get_id(), count);
        x_val[count++] = std::find(active_edges.begin(), active_edges.end(), e) != active_edges.end() ? 1.0 : 0.0;
    }
