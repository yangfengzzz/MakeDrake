...
MinkowskiSum::MinkowskiSum(const ConvexSets& sets) : ConvexSet(GetAmbientDimension(sets), false), sets_(sets) {}

MinkowskiSum::MinkowskiSum(const ConvexSet& setA, const ConvexSet& setB) : ConvexSet(setA.ambient_dimension(), false) {
...
MinkowskiSum::MinkowskiSum(const QueryObject<double>& query_object,
    ...
    if (shape.type_name() != "Capsule") {
        throw std::logic_error(fmt::format("MinkowskiSum(geometry_id={}, ...) cannot convert a {}, only a Capsule",
                                           geometry_id, shape));
    }
    const Capsule& capsule = dynamic_cast<const Capsule&>(shape);

    // Sphere at zero.
    sets_.emplace_back(Hyperellipsoid::MakeHypersphere(capsule.radius(), Vector3d::Zero()).Clone());
    ...
    HPolyhedron H_G =
            HPolyhedron::MakeBox(Vector3d{0, 0, -capsule.length() / 2.0}, Vector3d{0, 0, capsule.length() / 2.0});
    // A_G*(p_GF + R_GF*p_FF_var) ≤ b_G
    sets_.emplace_back(
            std::make_unique<HPolyhedron>(H_G.A() * X_GF.rotation().matrix(), H_G.b() - H_G.A() * X_GF.translation()));
}
...
bool MinkowskiSum::DoPointInSet(const Eigen::Ref<const VectorXd>& x, double) const {
...
std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> MinkowskiSum::DoAddPointInSetConstraints(
    ...
    for (int i = 0; i < ambient_dimension(); ++i) {
        ...
        new_constraints.push_back(
                prog->AddLinearEqualityConstraint(a, 0.0, {Vector1<Variable>(x[i]), X.row(i).transpose()}));
    }
    for (int j = 0; j < num_terms(); ++j) {
        if (sets_[j]->ambient_dimension() == 0) {
            std::optional<Variable> new_var =
                    ConvexSet::HandleZeroAmbientDimensionConstraints(prog, *sets_[j], &new_constraints);
            if (new_var.has_value()) {
                new_vars.push_back(new_var.value());
            }
            continue;
        }
        const auto [new_vars_in_sets_j, new_constraints_in_sets_j] = sets_[j]->AddPointInSetConstraints(prog, X.col(j));
        for (int k = 0; k < new_vars_in_sets_j.rows(); ++k) {
            new_vars.push_back(new_vars_in_sets_j(k));
        }
        new_constraints.insert(new_constraints.end(), new_constraints_in_sets_j.begin(),
...
std::vector<Binding<Constraint>> MinkowskiSum::DoAddPointInNonnegativeScalingConstraints(
    ...
    for (int j = 0; j < num_terms(); ++j) {
        if (sets_[j]->ambient_dimension() == 0) {
            ConvexSet::HandleZeroAmbientDimensionConstraints(prog, *sets_[j], &constraints);
            continue;
        }
        auto new_constraints = sets_[j]->AddPointInNonnegativeScalingConstraints(prog, X.col(j), t);
...
std::vector<Binding<Constraint>> MinkowskiSum::DoAddPointInNonnegativeScalingConstraints(
    ...
    for (int j = 0; j < num_terms(); ++j) {
        ...
        auto new_constraints = sets_[j]->AddPointInNonnegativeScalingConstraints(prog, A, b, c, d, X.col(j), t);
        constraints.insert(constraints.end(), std::make_move_iterator(new_constraints.begin()),
                           std::make_move_iterator(new_constraints.end()));
    }
    return constraints;
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> MinkowskiSum::DoToShapeWithPose() const {
    // TODO(russt): Consider handling Capsule as a (very) special case.
    throw std::runtime_error("ToShapeWithPose is not implemented yet for MinkowskiSum.");
}
