...
AffineBall::AffineBall(const Eigen::Ref<const MatrixXd>& B, const Eigen::Ref<const VectorXd>& center)
...
AffineBall AffineBall::MinimumVolumeCircumscribedEllipsoid(const Eigen::Ref<const Eigen::MatrixXd>& points,
                                                           double rank_tol) {
    ...
    const Hyperellipsoid hyperellipsoid_local =
            Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(points_local, rank_tol);
...
AffineBall AffineBall::MakeAxisAligned(const Eigen::Ref<const VectorXd>& radius,
...
AffineBall AffineBall::MakeHypersphere(double radius, const Eigen::Ref<const VectorXd>& center) {
...
AffineBall AffineBall::MakeAffineBallFromLineSegment(const Eigen::Ref<const Eigen::VectorXd>& x_1,
    ...
    if (length < kTolerance) {
        throw std::runtime_error(
                fmt::format("AffineBall:MakeAffineBallFromLineSegment: x_1 and x_2 are the same "
...
std::optional<bool> AffineBall::DoPointInSetShortcut(const Eigen::Ref<const Eigen::VectorXd>& x, double tol) const {
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> AffineBall::DoToShapeWithPose() const {
    throw std::runtime_error("ToShapeWithPose is not yet supported by AffineBall.");
}

std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> AffineBall::DoAddPointInSetConstraints(
    ...
    new_constraints.push_back(
            prog->AddQuadraticAsRotatedLorentzConeConstraint(MatrixXd::Identity(n, n), VectorXd::Zero(n), -0.5, y));
    ...
    new_constraints.push_back(prog->AddLinearEqualityConstraint(equality_constraint_A, center_, {x, y}));
    return {std::move(y), std::move(new_constraints)};
}

std::vector<Binding<Constraint>> AffineBall::DoAddPointInNonnegativeScalingConstraints(
        MathematicalProgram*, const Eigen::Ref<const VectorXDecisionVariable>&, const Variable&) const {
    throw std::runtime_error(
