...
public:
    ...
    public:
        SeparatingPlaneLagrangians(int C_rows, int s_size) : polytope_(C_rows), s_lower_(s_size), s_upper_(s_size) {}

        /** Substitutes the decision variables in each Lagrangians with its value in
         * result, returns the substitution result.
         */
        [[nodiscard]] SeparatingPlaneLagrangians GetSolution(const solvers::MathematicalProgramResult& result) const;
    ...
    struct SeparationCertificateResult final : SeparationCertificateResultBase {
        ...
        const std::vector<SeparatingPlaneLagrangians>& lagrangians(PlaneSide plane_side) const {
            return plane_side == PlaneSide::kPositive ? positive_side_rational_lagrangians
    ...
    struct SeparationCertificate {
        ...
        [[nodiscard]] SeparationCertificateResult GetSolution(int plane_index,
    ...
    struct FindSeparationCertificateGivenPolytopeOptions final : public FindSeparationCertificateOptions {
    ...
    struct FindPolytopeGivenLagrangianOptions {
        ...
        EllipsoidMarginCost ellipsoid_margin_cost{EllipsoidMarginCost::kGeometricMean};
    };
    ...
    public:
        ...
        [[nodiscard]] const HPolyhedron& certified_polytope() const { return certified_polytope_; }

        /** Each plane index is mapped to a vector of polynomials. */
        [[nodiscard]] const std::unordered_map<int, Vector3<symbolic::Polynomial>>& a() const { return a_; }

        /** Each plane index is mapped to a polynomial, */
        [[nodiscard]] const std::unordered_map<int, symbolic::Polynomial>& b() const { return b_; }
    ...
    private:
        ...
        void SetSeparatingPlanes(std::unordered_map<int, Vector3<symbolic::Polynomial>> a,
        ...
        // nullopt).
        void SetSeparatingPlanes(const std::vector<std::optional<SeparationCertificateResult>>& certificates_result);

        // Update this->a and this->b with the values in certificates_result.
        void UpdateSeparatingPlanes(
                const std::vector<std::optional<SeparationCertificateResult>>& certificates_results);
     ...
     */
    [[nodiscard]] std::optional<SearchResult> BinarySearch(const IgnoredCollisionPairs& ignored_collision_pairs,
                                                           const Eigen::Ref<const Eigen::MatrixXd>& C,
                                                           const Eigen::Ref<const Eigen::VectorXd>& d_init,
    ...
    [[nodiscard]] std::unique_ptr<solvers::MathematicalProgram> InitializePolytopeSearchProgram(
            ...
            std::unordered_map<int, SeparationCertificate>* new_certificates = nullptr) const;
    ...
    /**
     ...
     empty.
     */
    [[nodiscard]] std::optional<SeparationCertificateResult> SolveSeparationCertificateProgram(
...
private:
    ...
    [[nodiscard]] std::vector<PlaneSeparatesGeometries>& get_mutable_plane_geometries() { return plane_geometries_; }
     ...
     */
    [[nodiscard]] std::vector<std::optional<SeparationCertificateResult>> FindSeparationCertificateGivenPolytope(
            const IgnoredCollisionPairs& ignored_collision_pairs,
            const Eigen::Ref<const Eigen::MatrixXd>& C,
            const Eigen::Ref<const Eigen::VectorXd>& d,
            const FindSeparationCertificateGivenPolytopeOptions& options) const;
    ...
    */
    [[nodiscard]] int GetGramVarSizeForPolytopeSearchProgram(const IgnoredCollisionPairs& ignored_collision_pairs,
     ...
     */
    [[nodiscard]] std::optional<FindPolytopeGivenLagrangianResult> FindPolytopeGivenLagrangian(
            const IgnoredCollisionPairs& ignored_collision_pairs,
            const MatrixX<symbolic::Variable>& C,
            const VectorX<symbolic::Variable>& d,
            const VectorX<symbolic::Polynomial>& d_minus_Cs,
