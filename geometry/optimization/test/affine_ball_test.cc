...
GTEST_TEST(AffineBallTest, UnitBall6DTest) {
    ...
    Vector6d in1_W{Vector6d::Constant(-0.99 * kScale)}, in2_W{Vector6d::Constant(0.99 * kScale)},
...
GTEST_TEST(AffineBallTest, MakeHypersphereTest) {
    ...
    EXPECT_THROW(AffineBall::MakeHypersphere(-1, VectorXd::Zero(2)), std::exception);
    EXPECT_THROW(AffineBall::MakeHypersphere(-1, VectorXd::Zero(0)), std::exception);
}
...
GTEST_TEST(AffineBallTest, MakeAxisAlignedTest) {
  ...
  B_expected << a, 0, 0,
    ...
    EXPECT_NO_THROW(AffineBall::MakeAxisAligned(VectorXd::Zero(0), VectorXd::Zero(0)));
    EXPECT_THROW(AffineBall::MakeAxisAligned(VectorXd::Zero(0), VectorXd::Zero(1)), std::exception);
    EXPECT_THROW(AffineBall::MakeAxisAligned(VectorXd::Zero(1), VectorXd::Zero(0)), std::exception);
    EXPECT_THROW(AffineBall::MakeAxisAligned(Vector2d(-1, 1), VectorXd::Zero(2)), std::exception);
}
...
GTEST_TEST(AffineBallTest, NotAxisAligned) {
  ...
  B1 << 2 * one_over_sqrt_two, -1 * one_over_sqrt_two,
    ...
    EXPECT_TRUE(ab1.PointInSet(center + Vector2d{2 * one_over_sqrt_two, 2 * one_over_sqrt_two}, kTol));
  ...
  B2 << 2 * one_over_sqrt_two, 0,
    ...
    EXPECT_TRUE(ab2.PointInSet(center + Vector2d{2 * one_over_sqrt_two, 2 * one_over_sqrt_two}, kTol));
    EXPECT_TRUE(ab2.PointInSet(center - Vector2d{2 * one_over_sqrt_two, 2 * one_over_sqrt_two}, kTol));
...
GTEST_TEST(AffineBallTest, MinimumVolumeCircumscribedEllipsoidPreconditions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AffineBall::MinimumVolumeCircumscribedEllipsoid(zero_rows), ".*points.*rows.*");
    DRAKE_EXPECT_THROWS_MESSAGE(AffineBall::MinimumVolumeCircumscribedEllipsoid(zero_cols), ".*points.*cols.*");
    DRAKE_EXPECT_THROWS_MESSAGE(AffineBall::MinimumVolumeCircumscribedEllipsoid(nan_entry), ".*points.*hasNaN.*");
    DRAKE_EXPECT_THROWS_MESSAGE(AffineBall::MinimumVolumeCircumscribedEllipsoid(inf_entry), ".*points.*allFinite.*");
}
...
GTEST_TEST(AffineBallTest, MinimumVolumeCircumscribedEllipsoidFullDimensional) {
  ...
  p_FA << 1, 0, 0, -1,  0,  0,
    ...
    if (solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) {
        tol = 1e-8;
        eps = 1e-6;
    } else if (solvers::GurobiSolver::is_available() && solvers::GurobiSolver::is_enabled()) {
        tol = 1e-4;
        eps = 1e-3;
    } else {
        tol = 1e-5;
    ...
    for (int i = 0; i < p_FA.cols(); ++i) {
        EXPECT_TRUE(E_F.PointInSet(p_FA.col(i), tol));
        Eigen::Vector3d point_outside = p_FA.col(i) + (p_FA.col(i) - translation) * (1. + eps);
        EXPECT_FALSE(E_F.PointInSet(point_outside, tol));
    }
}

GTEST_TEST(AffineBallTest, MinimumVolumeCircumscribedEllipsoidLowerDimensional) {
    // Axis-aligned example
  ...
  points << 0.1, -0.1,
    ...
    for (int i = 0; i < points.cols(); ++i) {
        ...
        Eigen::VectorXd point_outside = points.col(i) + (points.col(i) - center) * (1. + eps);
...
GTEST_TEST(AffineBallTest, MakeAffineBallFromLineSegment) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AffineBall::MakeAffineBallFromLineSegment(x_1, x_1), ".*same point.*");
