...
GTEST_TEST(HyperellipsoidTest, UnitSphereTest) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Sphere(1.0), RigidTransformd::Identity());
    ...
    // Test PointInSet.
    const Vector3d in1_W{.99, 0, 0}, in2_W{.5, .5, .5}, out1_W{1.01, 0, 0}, out2_W{1.0, 1.0, 1.0};
...
GTEST_TEST(HyperellipsoidTest, ScaledSphereTest) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Sphere(radius), RigidTransformd::Identity());

    Hyperellipsoid E(query, geom_id);
    EXPECT_TRUE(CompareMatrices(E.A(), Eigen::Matrix3d::Identity() / radius, 1e-16));
...
GTEST_TEST(HyperellipsoidTest, ArbitraryEllipsoidTest) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(
            Ellipsoid(1.0 / D(0, 0), 1.0 / D(1, 1), 1.0 / D(2, 2)), RigidTransformd{R.inverse(), center});
    ...
    // Test PointInSet.
    Vector3d in1_W{.99, 0, 0}, in2_W{.5, .5, .5}, out1_W{1.01, 0, 0}, out2_W{1.0, 1.0, 1.0};
...
GTEST_TEST(HyperellipsoidTest, UnitBall6DTest) {
    ...
    Vector6d in1_W{Vector6d::Constant(-.99 * kScale)}, in2_W{Vector6d::Constant(.99 * kScale)},
            out1_W{Vector6d::Constant(-1.01 * kScale)}, out2_W{Vector6d::Constant(1.01 * kScale)};
...
GTEST_TEST(HyperellipsoidTest, NonnegativeScalingTest) {
    ...
    std::vector<Binding<Constraint>> constraints = E.AddPointInNonnegativeScalingConstraints(&prog, x, t);
...
GTEST_TEST(HyperellipsoidTest, NonnegativeScalingTest2) {
  ...
  A << 1, 0,
    ...
    std::vector<Binding<Constraint>> constraints = E.AddPointInNonnegativeScalingConstraints(&prog, A, b, c, d, x, t);
    ...
    EXPECT_TRUE(PointInScaledSet(x, t, 0.99 * ub, Vector2d(1.0, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.99 * ub, Vector2d(0, -1.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 1.01 * ub, Vector2d(1.0, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 1.01 * ub, Vector2d(0, -1.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.99 * ub, Vector2d(-0.01, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.99 * ub, Vector2d(0, -0.01), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.49 * ub, Vector2d(0.5, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.49 * ub, Vector2d(0, -0.5), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.51 * ub, Vector2d(0.5, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.51 * ub, Vector2d(0, -0.5), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 1.99 * ub, Vector2d(2.0, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 1.99 * ub, Vector2d(0, -2.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 2.01 * ub, Vector2d(2.0, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 2.01 * ub, Vector2d(0, -2.0), &prog, constraints));
}
...
GTEST_TEST(HyperellipsoidTest, MakeAxisAlignedTest) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Ellipsoid(a, b, c), RigidTransformd{center});
...
GTEST_TEST(HyperellipsoidTest, MinimumVolumeCircumscribedEllipsoid) {
  ...
  p_FA << 1, 0, 0, -1,  0,  0,
          ...
          0, 0, 3,  0,  0, -3;
    // clang-format on
    Hyperellipsoid E_F = Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(p_FA);
  ...
  A_expected << 1,         0,         0,
    ...
    Hyperellipsoid E_G = Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(p_GA);
    ...
    for (int i = 0; i < p_FA.cols(); ++i) {
        EXPECT_NEAR((E_G.A() * (X_GF * p_FA.col(i) - E_G.center())).norm(), 1, kTol);
    }
}
...
GTEST_TEST(HyperellipsoidTest, MinimumVolumeCircumscribedEllipsoid2) {
  ...
  points << 2, -2,
            0,  0;
    // clang-format on
    EXPECT_THROW(Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(points), std::exception);
  ...
  points <<    2,      0,    -2,
            1e-3,  -1e-3,  1e-3;
    // clang-format on
    EXPECT_THROW(Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(points, 1e-2), std::exception);
  ...
  points <<    2,      0,    -2,
    ...
    if (solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(Hyperellipsoid::MinimumVolumeCircumscribedEllipsoid(points, 1e-14),
...
GTEST_TEST(HyperellipsoidTest, IsBoundedAndVolumeTest2) {
    ...
    Hyperellipsoid E(Matrix3d(Vector3d(1.0, 1.0, 0).asDiagonal()), Vector3d::Zero());
    EXPECT_FALSE(E.IsBounded());
    EXPECT_EQ(E.Volume(), std::numeric_limits<double>::infinity());
}
...
GTEST_TEST(HyperellipsoidTest, IsBoundedAndVolumeTest4) {
    ...
    Hyperellipsoid E = Hyperellipsoid::MakeHypersphere(kRadius, VectorXd::Ones(8));
    EXPECT_TRUE(E.IsBounded());
    EXPECT_NEAR(E.Volume(), std::pow(M_PI, 4) / 24.0 * std::pow(kRadius, 8), 1e-16);
}
...
GTEST_TEST(HyperellipsoidTest, MinimumUniformScaling) {
    ...
    Hyperellipsoid E = Hyperellipsoid::MakeAxisAligned(Vector2d{a, b}, Vector2d::Ones());
...
GTEST_TEST(HyperellipsoidTest, MinimumUniformScaling5) {
  ...
  A <<  1, 0,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(E.MinimumUniformScalingToTouch(H), ".*is empty.*");
}
...
GTEST_TEST(HyperellipsoidTest, FromAffineBall) {
    ...
    AffineBall ab_sub_dimensional = AffineBall::MakeAxisAligned(Vector3d{a, b, 0}, center);
