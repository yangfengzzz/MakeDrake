...
TEST_F(CIrisToyRobotTest, CspaceFreePolytopeGenerateRationals) {
    ...
    EXPECT_EQ(tester.plane_geometries().size(), tester.cspace_free_polytope().separating_planes().size());
    for (const auto& plane_geometries : tester.plane_geometries()) {
        ...
        if (plane.positive_side_geometry->type() == CIrisGeometryType::kPolytope &&
            ...
            EXPECT_EQ(plane_geometries.positive_side_rationals.size(), plane.positive_side_geometry->num_rationals());
            EXPECT_EQ(plane_geometries.negative_side_rationals.size(), plane.negative_side_geometry->num_rationals());
...
TEST_F(CIrisToyRobotTest, FindRedundantInequalities) {
  ...
  C << 1, 1, 0,
    ...
    tester.FindRedundantInequalities(C, d, s_lower, s_upper, 0., &C_redundant_indices, &s_lower_redundant_indices,
...
TEST_F(CIrisToyRobotTest, CalcDminusCs) {
    ...
    for (int i = 0; i < 2; ++i) {
        EXPECT_PRED2(symbolic::test::PolyEqual, ret(i),
                     symbolic::Polynomial(d(i) - C.row(i).dot(s.cast<symbolic::Expression>()), symbolic::Variables(s)));
    }
}
...
TEST_F(CIrisToyRobotTest, CalcSBoundsPolynomial) {
    ...
    const Eigen::Vector3d s_lower = tester.cspace_free_polytope().rational_forward_kin().ComputeSValue(

            plant_->GetPositionLowerLimits(), q_star);
    const Eigen::Vector3d s_upper = tester.cspace_free_polytope().rational_forward_kin().ComputeSValue(
            plant_->GetPositionUpperLimits(), q_star);
    const auto& s = tester.cspace_free_polytope().rational_forward_kin().s();
    for (int i = 0; i < 3; ++i) {
        EXPECT_PRED2(symbolic::test::PolyEqual, tester.s_minus_s_lower()(i), symbolic::Polynomial((s(i) - s_lower(i))));
        EXPECT_PRED2(symbolic::test::PolyEqual, tester.s_upper_minus_s()(i), symbolic::Polynomial((s_upper(i) - s(i))));
    }
}
...
TEST_F(CIrisToyRobotTest, CalcMonomialBasis) {
    ...
    for (bool with_cross_y : {false, true}) {
        ...
        const auto& map_body_to_monomial_basis_array = tester.map_body_to_monomial_basis_array();
        ...
        for (const auto& plane : tester.cspace_free_polytope().separating_planes()) {
            for (const auto collision_geometry : {plane.positive_side_geometry, plane.negative_side_geometry}) {
                ...
                for (int i = 0; i < monomial_basis_array[0].rows(); ++i) {
                    ...
                    for (const auto& [var, degree] : monomial_basis_array[0](i).get_powers()) {
                        EXPECT_LE(degree, 1);
                    }
                }
                for (int i = 0; i < 3; ++i) {
                    EXPECT_EQ(monomial_basis_array[i + 1].rows(), monomial_basis_array[0].rows());
                    for (int j = 0; j < monomial_basis_array[0].rows(); ++j) {
                        EXPECT_EQ(monomial_basis_array[i + 1](j),
...
TEST_F(CIrisToyRobotTest, AddEllipsoidContainmentConstraint) {
  ...
  Q << 1, 2, -1,
    ...
    tester.AddEllipsoidContainmentConstraint(&prog, Q, s0, C, d, ellipsoid_margins);
    ...
    for (int i = 0; i < C_sol.rows(); ++i) {
        ...
        EXPECT_LE((C_sol.row(i) * Q).norm(), d_sol(i) - C_sol.row(i).dot(s0) - margin_sol(i));
    }
}
...
TEST_F(CIrisToyRobotTest, AddCspacePolytopeContainment) {
  ...
  s_inner_pts << 1, 2, 0, 3,
    ...
    for (int i = 0; i < s_inner_pts.cols(); ++i) {
        s_inner_pts.col(i) = s_inner_pts.col(i).cwiseMin(tester.s_upper()).cwiseMax(tester.s_lower());
    }
    solvers::MathematicalProgram prog;
    ...
    const VectorX<symbolic::Expression> constraint_val =
            prog.linear_constraints()[0].evaluator()->get_sparse_A() * prog.linear_constraints()[0].variables();
    EXPECT_TRUE(CompareMatrices(
            prog.linear_constraints()[0].evaluator()->lower_bound(),
            Eigen::VectorXd::Constant(prog.linear_constraints()[0].evaluator()->num_constraints(), -kInf)));
    EXPECT_TRUE(
            CompareMatrices(prog.linear_constraints()[0].evaluator()->upper_bound(),
                            Eigen::VectorXd::Constant(prog.linear_constraints()[0].evaluator()->num_constraints(), 0)));
    for (int i = 0; i < C.rows(); ++i) {
        for (int j = 0; j < s_inner_pts.cols(); ++j) {
            EXPECT_PRED2(symbolic::test::ExprEqual, constraint_val(i * s_inner_pts.cols() + j),
...
TEST_F(CIrisToyRobotTest, GetPolyhedronWithJointLimits) {
  ...
  C << 0.5, 1, 2,
    ...
    const Eigen::VectorXd s_lower = tester.cspace_free_polytope().rational_forward_kin().ComputeSValue(
            plant_->GetPositionLowerLimits(), q_star);
    const Eigen::VectorXd s_upper = tester.cspace_free_polytope().rational_forward_kin().ComputeSValue(
            plant_->GetPositionUpperLimits(), q_star);
