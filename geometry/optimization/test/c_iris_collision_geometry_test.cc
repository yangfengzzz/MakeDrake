#include "geometry/optimization/c_iris_collision_geometry.h"
...
void SetupPlane(const Eigen::Ref<const VectorX<symbolic::Variable>>& s,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        (*a)(i) = symbolic::Polynomial(a_coeff.row(i).dot(s.cast<symbolic::Expression>()) + a_constant(i), s_set);
    }
    ...
    *b = symbolic::Polynomial(b_coeff.dot(s.cast<symbolic::Expression>()) + b_constant, s_set);
}

class CIrisCollisionGeometryTest : public CIrisToyRobotTest {
public:
    CIrisCollisionGeometryTest() : rational_forward_kin_{plant_}, diagram_context_{diagram_->CreateDefaultContext()} {
        SetupPlane(rational_forward_kin_.s(), &a_, &b_);
        plant_context_ = &(plant_->GetMyMutableContextFromRoot(diagram_context_.get()));
        scene_graph_context_ = &(scene_graph_->GetMyMutableContextFromRoot(diagram_context_.get()));
    ...
    Eigen::VectorXd SetQ(const Eigen::Vector3d& q_star, const Eigen::Vector3d& q_val) {
        DRAKE_DEMAND((q_val.array() <= plant_->GetPositionUpperLimits().array()).all());
        DRAKE_DEMAND((q_val.array() >= plant_->GetPositionLowerLimits().array()).all());
        const Eigen::VectorXd s_val = rational_forward_kin_.ComputeSValue(q_val, q_star);
...
void CheckRationalNoSlack(const symbolic::RationalFunction& rational,
    ...
    const symbolic::Expression expr =
            rational.numerator().EvaluatePartial(env).ToExpression() / rational.denominator().Evaluate(env);
    EXPECT_PRED3(symbolic::test::PolynomialEqual, symbolic::Polynomial(expr), symbolic::Polynomial(expr_expected),
                 1E-7);
}
...
void SplitRationals(const std::vector<symbolic::RationalFunction>& rationals,
    ...
    for (const auto& rational : rationals) {
        bool with_y = false;
        for (int i = 0; i < 3; ++i) {
            if (rational.numerator().indeterminates().find(y(i)) != rational.numerator().indeterminates().end()) {
...
void CheckRationalWSlack(const symbolic::RationalFunction& rational,
    ...
    const symbolic::Expression expr_expected =
            (lhs_expr + 2 * (P * a_expr).dot(y) + lhs_expr / (radius * radius) * y.cast<symbolic::Expression>().dot(y));

    EXPECT_PRED3(
            symbolic::test::PolynomialEqual,
            symbolic::Polynomial(rational.numerator().EvaluatePartial(env).ToExpression()).Expand() / denominator_val,
...
TEST_F(CIrisCollisionGeometryTest, Box) {
    ...
    CIrisCollisionGeometry box(&model_inspector.GetShape(body0_box_), geometry_body, body0_box_,
    ...
    const auto X_AB_multilinear =
            rational_forward_kin_.CalcBodyPoseAsMultilinearPolynomial(q_star, geometry_body, expressed_body);

    std::vector<symbolic::RationalFunction> rationals;
    // Positive side.
    box.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kPositive, y_slack_, &rationals);
  ...
  p_GV << 1, 1, 1, 1, -1, -1, -1, -1,
    ...
    plant_->CalcPointsPositions(*plant_context_, plant_->get_body(geometry_body).body_frame(), p_BV,
    ...
    for (int i = 0; i < 8; ++i) {
        const symbolic::Expression expr_expected = a_expr.dot(p_AV.col(i)) + b_expr - 1;
        CheckRationalNoSlack(rationals[i], env, expr_expected);
    }

    // Negative side.
    rationals.clear();
    box.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kNegative, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 8);
    EXPECT_EQ(box.num_rationals(), 8);
    for (int i = 0; i < 8; ++i) {
        const symbolic::Expression expr_expected = -1 - a_expr.dot(p_AV.col(i)) - b_expr;
        CheckRationalNoSlack(rationals[i], env, expr_expected);
    }
}
...
TEST_F(CIrisCollisionGeometryTest, Convex) {
    ...
    CIrisCollisionGeometry convex(&model_inspector.GetShape(body1_convex_), geometry_body, body1_convex_,
    ...
    auto query_object = scene_graph_->get_query_output_port().Eval<QueryObject<double>>(*scene_graph_context_);

    const VPolytope polytope(query_object, body1_convex_, model_inspector.GetFrameId(body1_convex_));
    // Compute the H-polyhedron in the geometry frame.
    const HPolyhedron h_poly(VPolytope(convex.X_BG().inverse() * polytope.vertices()));
    ...
    // Negative side.
    convex.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kNegative, y_slack_, &rationals);
    ...
    convex.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kPositive, y_slack_, &rationals);
    // The new rationals_for_points are appended to the existing ones.
    EXPECT_EQ(rationals.size(), 2 * polytope.vertices().cols());
    EXPECT_EQ(convex.num_rationals(), polytope.vertices().cols());
    for (int i = 0; i < polytope.vertices().cols(); ++i) {
        const symbolic::Expression expr_expected = a_expr.dot(p_AV.col(i)) + b_expr - 1;
        CheckRationalNoSlack(rationals[i + polytope.vertices().cols()], env, expr_expected);
    }
}
...
TEST_F(CIrisCollisionGeometryTest, Sphere) {
    ...
    CIrisCollisionGeometry sphere(&model_inspector.GetShape(body2_sphere_), geometry_body, body2_sphere_,
    ...
    // Negative side.
    std::vector<symbolic::RationalFunction> rationals;
    sphere.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kNegative, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 2);
    const Eigen::Vector3d p_BS = sphere.X_BG().translation();
    Eigen::Vector3d p_AS;
    plant_->CalcPointsPositions(*plant_context_, plant_->get_body(sphere.body_index()).body_frame(), p_BS,
                                plant_->get_body(expressed_body).body_frame(), &p_AS);
    const double radius = static_cast<const Sphere&>(model_inspector.GetShape(body2_sphere_)).radius();

    std::vector<symbolic::RationalFunction> rationals_no_y;
    std::vector<symbolic::RationalFunction> rationals_with_y;
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);

    CheckRationalWSlack(rationals_with_y[0], env, -a_expr.dot(p_AS) - b_expr, Eigen::Matrix3d::Identity(), a_expr,
                        y_slack_, radius);
    CheckRationalNoSlack(rationals_no_y[0], env, -1 - a_expr.dot(p_AS) - b_expr);

    // Positive side.
    rationals.clear();
    sphere.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kPositive, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 2);
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);
    CheckRationalWSlack(rationals_with_y[0], env, a_expr.dot(p_AS) + b_expr, Eigen::Matrix3d::Identity(), a_expr,
                        y_slack_, radius);
...
TEST_F(CIrisCollisionGeometryTest, Capsule) {
    ...
    CIrisCollisionGeometry capsule(&model_inspector.GetShape(body2_capsule_), geometry_body, body2_capsule_,
    ...
    // Negative side.
    std::vector<symbolic::RationalFunction> rationals;
    capsule.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kNegative, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 3);
    const Capsule& capsule_shape = static_cast<const Capsule&>(model_inspector.GetShape(body2_capsule_));
    ...
    Eigen::Vector3d p_AO;
    plant_->CalcPointsPositions(*plant_context_, plant_->get_body(geometry_body).body_frame(), p_BS,
                                plant_->get_body(expressed_body).body_frame(), &p_AS);
    plant_->CalcPointsPositions(*plant_context_, plant_->get_body(geometry_body).body_frame(), p_BO,
    ...
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);
    for (int i = 0; i < 2; ++i) {
        CheckRationalWSlack(rationals_with_y[i], env, -a_expr.dot(p_AS.col(i)) - b_expr, Eigen::Matrix3d::Identity(),
                            a_expr, y_slack_, capsule_shape.radius());
    }
    CheckRationalNoSlack(rationals_no_y[0], env, -a_expr.dot(p_AO) - b_expr - 1);

    // Positive side
    rationals.clear();
    capsule.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kPositive, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 3);
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);
    for (int i = 0; i < 2; ++i) {
        CheckRationalWSlack(rationals_with_y[i], env, a_expr.dot(p_AS.col(i)) + b_expr, Eigen::Matrix3d::Identity(),
...
TEST_F(CIrisCollisionGeometryTest, Cylinder) {
    ...
    CIrisCollisionGeometry cylinder(&model_inspector.GetShape(body3_cylinder_), geometry_body, body3_cylinder_,
    ...
    // Positive side.
    std::vector<symbolic::RationalFunction> rationals;
    cylinder.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kPositive, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 3);
    std::vector<symbolic::RationalFunction> rationals_no_y;
    std::vector<symbolic::RationalFunction> rationals_with_y;
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);

    const Cylinder& cylinder_shape = static_cast<const Cylinder&>(model_inspector.GetShape(body3_cylinder_));

    const math::RigidTransform X_BA =
            plant_->CalcRelativeTransform(*plant_context_, plant_->get_body(geometry_body).body_frame(),
                                          plant_->get_body(expressed_body).body_frame());
    const math::RigidTransform X_GA = cylinder.X_BG().InvertAndCompose(X_BA);
    const Vector3<symbolic::Expression> a_G_expr = X_GA.rotation().matrix() * a_expr;
    const symbolic::Expression b_G_expr = b_expr + a_expr.dot(X_GA.inverse().translation());
    CheckRationalNoSlack(rationals_no_y[0], env, b_G_expr - 1);

    const double radius = cylinder_shape.radius();
    const std::array<double, 2> height = {cylinder_shape.length() / 2, -cylinder_shape.length() / 2};
  ...
  P << 1, 0, 0,
    ...
    // clang-format on
    for (int i = 0; i < 2; ++i) {
        CheckRationalWSlack(rationals_with_y[i], env, a_G_expr(2) * height[i] + b_G_expr, P, a_G_expr,
                            y_slack_.head<2>(), radius);
    }

    // Negative side.
    rationals.clear();
    cylinder.OnPlaneSide(a_, b_, X_AB_multilinear, rational_forward_kin_, PlaneSide::kNegative, y_slack_, &rationals);
    EXPECT_EQ(rationals.size(), 3);
    SplitRationals(rationals, y_slack_, &rationals_no_y, &rationals_with_y);
...
GTEST_TEST(DistanceToHalfspace, Test) {
    ...
    std::tie(plant, scene_graph) = multibody::AddMultibodyPlantSceneGraph(&builder, 0.);

    ProximityProperties proximity_properties{};
    // C-IRIS doesn't care about robot dynamics. Use arbitrary material
    // properties.
    AddContactMaterial(0.1, 250.0, multibody::CoulombFriction<double>{0.9, 0.5}, &proximity_properties);

    std::vector<multibody::BodyIndex> body_indices;
    for (int i = 0; i < 5; ++i) {
        body_indices.push_back(plant->AddRigidBody("body" + std::to_string(i)).index());
    }
    const Eigen::Vector3d box_size(0.05, 0.02, 0.03);
    const geometry::GeometryId body0_box = plant->RegisterCollisionGeometry(
            plant->get_body(body_indices[0]), math::RigidTransform(Eigen::Vector3d(0.01, 0.02, 0.03)),
            Box(box_size(0), box_size(1), box_size(2)), "body0_box", proximity_properties);

    const geometry::GeometryId body1_sphere = plant->RegisterCollisionGeometry(
            plant->get_body(body_indices[1]), math::RigidTransform(Eigen::Vector3d(0.02, -0.01, 0.05)), Sphere(0.01),
    ...
    const geometry::GeometryId body2_capsule = plant->RegisterCollisionGeometry(
            ...
            math::RigidTransformd(math::RollPitchYawd(0.05, -0.02, 0.01), Eigen::Vector3d(0.01, 0.02, -0.03)),
            Capsule(capsule_radius, capsule_length), "body2_capsule", proximity_properties);

    const geometry::GeometryId body3_halfspace = plant->RegisterCollisionGeometry(
            plant->get_body(body_indices[3]),
            math::RigidTransformd(math::RollPitchYawd(0.02, -0.1, 0.05), Eigen::Vector3d(0.03, -0.01, 0.02)),
    ...
    const geometry::GeometryId body4_cylinder = plant->RegisterCollisionGeometry(
            ...
            math::RigidTransformd(math::RollPitchYawd(0.1, -0.2, 0.3), Eigen::Vector3d(0.02, 0.1, -0.05)),
            Cylinder(cylinder_radius, cylinder_length), "body4_cylinder", proximity_properties);
    ...
    const CIrisCollisionGeometry sphere(&(model_inspector.GetShape(body1_sphere)), body_indices[1], body1_sphere,
    ...
    const auto X_HalfspaceE = model_inspector.GetPoseInFrame(body3_halfspace).inverse();
    ...
    const auto& query_object = query_port.Eval<geometry::QueryObject<double>>(plant_context);
    const double kTol = 1E-10;

    {
        // Distance to sphere.
        const double distance =
                DistanceToHalfspace(sphere, a, b, expressed_body, PlaneSide::kPositive, *plant, plant_context);
        const double distance_expected =
                query_object.ComputeSignedDistancePairClosestPoints(body3_halfspace, body1_sphere).distance;
        EXPECT_NEAR(distance, distance_expected, kTol);
    }

    {
        // Distance to box.
        const CIrisCollisionGeometry box(&(model_inspector.GetShape(body0_box)), body_indices[0], body0_box,
                                         model_inspector.GetPoseInFrame(body0_box));
        const double distance =
                DistanceToHalfspace(box, a, b, expressed_body, PlaneSide::kNegative, *plant, plant_context);
    ...
    p_GV << -1, -1, -1, -1, 1, 1, 1, 1,
        ...
        plant->CalcPointsPositions(plant_context, plant->get_body(body_indices[0]).body_frame(),
    ...
    {
        // Distance to capsule.
        const CIrisCollisionGeometry capsule(&(model_inspector.GetShape(body2_capsule)), body_indices[2], body2_capsule,
                                             model_inspector.GetPoseInFrame(body2_capsule));
        const double distance =
                DistanceToHalfspace(capsule, a, b, expressed_body, PlaneSide::kNegative, *plant, plant_context);
        ...
        plant->CalcPointsPositions(plant_context, plant->get_body(body_indices[2]).body_frame(),
...

    {
        ...
        const CIrisCollisionGeometry cylinder(&(model_inspector.GetShape(body4_cylinder)), body_indices[4],
                                              body4_cylinder, model_inspector.GetPoseInFrame(body4_cylinder));
        const double distance =
                DistanceToHalfspace(cylinder, a, b, expressed_body, PlaneSide::kNegative, *plant, plant_context);
        ...
        Eigen::Matrix<double, 3, 2> p_ES;
        plant->CalcPointsPositions(plant_context, plant->get_body(body_indices[4]).body_frame(),
        ...
        // frame H.
        const Eigen::Vector3d axis_H = (p_HS.col(0) - p_HS.col(1)) / (cylinder_length);
        const double distance_expected = -std::max(p_HS(2, 0), p_HS(2, 1)) -
                                         cylinder_radius * std::sqrt(axis_H(0) * axis_H(0) + axis_H(1) * axis_H(1));
