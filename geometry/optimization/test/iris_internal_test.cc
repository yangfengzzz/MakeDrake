...
public:
    ...
    <worldbody>
      ...
      <body name="link1" pos="0 0.1 0"><joint name="joint0" type="hinge" axis="0 0 1"/>
        ...
        parser.AddModelsFromString(xml, "xml");
        for (int i = 0; i < 3; ++i) {
            body_indices_[i] = plant_->GetBodyByName("link" + std::to_string(i)).index();
        }
        plant_->Finalize();
        diagram_ = builder.Build();
    }
...
TEST_F(ThreeLinks, TestClosestCollisionProgram) {
    auto diagram_context = diagram_->CreateDefaultContext();
    auto& plant_context = plant_->GetMyMutableContextFromRoot(diagram_context.get());
    auto same_point_constraint = std::make_shared<SamePointConstraint>(plant_, plant_context);

    const double radius_A = 0.1;
    const double radius_B = 0.1;
    Hyperellipsoid setA(1.0 / radius_A * Eigen::Matrix3d::Identity(), Eigen::Vector3d(0, 0.5, 0));
    Hyperellipsoid setB(1.0 / radius_B * Eigen::Matrix3d::Identity(), Eigen::Vector3d(0, -0.5, 0));
    Hyperellipsoid E(0.2 * Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero());
    Eigen::MatrixXd A(0, 2);
    Eigen::VectorXd b(0);
    ClosestCollisionProgram dut(same_point_constraint, plant_->get_body(body_indices_[2]).body_frame(),
                                plant_->world_frame(), setA, setB, E, A, b);

    Eigen::Vector2d q_guess(0, 0.5);

    Eigen::VectorXd closest;
    bool found_collision = dut.Solve(solvers::IpoptSolver(), q_guess, std::nullopt, &closest);
    EXPECT_TRUE(found_collision);
    // Check the solution.
    Eigen::Vector3d p_WA;
    plant_->SetPositions(&plant_context, closest);
    plant_->CalcPointsPositions(plant_context, plant_->get_body(body_indices_[2]).body_frame(), setA.center(),
    ...
    found_collision = dut.Solve(solvers::IpoptSolver(), q_guess, solver_options, &closest);
