...
GTEST_TEST(MinkowskiSumTest, FromSceneGraph) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Capsule(kRadius, kLength), X_WG);
...
GTEST_TEST(MinkowskiSumTest, FromSceneGraphBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MinkowskiSum(query, geom_id), ".*MinkowskiSum.*cannot.*HalfSpace.*");
}
