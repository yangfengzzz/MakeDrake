...
GTEST_TEST(CartesianProductTest, FromSceneGraph) {
    const RigidTransformd X_WG{math::RollPitchYawd(.1, .2, 3), Vector3d{.5, .87, .1}};

    // Test SceneGraph constructor.
    const double kRadius = 0.2;
    const double kLength = 0.5;
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Cylinder(kRadius, kLength), X_WG);
  ...
  out_G << kRadius+.01, 0,     kRadius,          kRadius/std::sqrt(2.0),
    ...
    for (int i = 0; i < 4; ++i) {
        EXPECT_LE(query.ComputeSignedDistanceToPoint(in_W.col(i))[0].distance, kTol);
        EXPECT_GE(query.ComputeSignedDistanceToPoint(out_W.col(i))[0].distance, -kTol);
    ...
    const RigidTransformd X_WF{math::RollPitchYawd(.5, .26, -3), Vector3d{.9, -2., .12}};
...
GTEST_TEST(CartesianProductTest, FromSceneGraphBad) {
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(HalfSpace(), RigidTransformd());
    DRAKE_EXPECT_THROWS_MESSAGE(CartesianProduct(query, geom_id), ".*CartesianProduct.*cannot.*HalfSpace.*");
}
...
GTEST_TEST(CartesianProductTest, ScaledPointAddPointInSet) {
  ...
  A << -2, 0,
    ...
    EXPECT_TRUE(CompareMatrices(new_vars_val, Eigen::Vector2d(P1.x()(0), P2.x()(0)), 1E-6));
}
...
GTEST_TEST(CartesianProductTest, ScaledPointNotInjectiveFail) {
    ...
    A << 1, 0;
    Vector1d b{0};
    DRAKE_EXPECT_THROWS_MESSAGE(CartesianProduct(MakeConvexSets(P), A, b), ".*rank.*");
}
...
GTEST_TEST(CartesianProductTest, NonnegativeScalingTest2) {
  ...
  t_solutions << 0.5,  0,  0.25, 1,  0,  0.5, 2,  0,  1,
    ...
    for (int ii = 0; ii < t_solutions.cols(); ++ii) {
        EXPECT_TRUE(PointInScaledSet(x, t, scale[ii / 3] * x_solution, t_solutions.col(ii), &prog));
        EXPECT_FALSE(PointInScaledSet(x, t, 0.99 * scale[ii / 3] * x_solution, t_solutions.col(ii), &prog));
    }

    EXPECT_FALSE(PointInScaledSet(x, t, x_solution, Eigen::Vector2d(0, 1), &prog));
    EXPECT_FALSE(PointInScaledSet(x, t, x_solution, Eigen::Vector2d(-1, 0), &prog));
...
GTEST_TEST(CartesianProductTest, ZeroDimensionalInputNonempty) {
    ...
    auto new_constraints2 = S.AddPointInNonnegativeScalingConstraints(&prog2, x2, t2[0]);
...
GTEST_TEST(CartesianProductTest, Volume) {
  ...
  vertices << 0, 1, 0,
    ...
    Hyperellipsoid E = Hyperellipsoid(Eigen::MatrixXd::Identity(2, 2), Vector2d(1, 2));
