...
#include "geometry/optimization/cspace_free_box.h"
...
Eigen::MatrixXd CalcBoxGrid(const Eigen::Ref<const Eigen::VectorXd>& box_lower,
    ...
    for (int i = 0; i < x_dim; ++i) {
        x_grid.push_back(Eigen::VectorXd::LinSpaced(grid_size[i], box_lower(i), box_upper(i)));
    }
    ...
    while (!grid_pts.empty()) {
        ret.row(pt_count) = Eigen::Map<Eigen::RowVectorXd>(grid_pts.front().data(), x_dim);
        pt_count++;
        grid_pts.pop();
    }
    return ret;
...
// aᵀx+b=0}
// @param geometry_pair
void CheckSeparationBySamples(const CspaceFreeBoxTester& tester,
    ...
    for (int i = 0; i < q_samples.rows(); ++i) {
        const Eigen::Vector3d s_val =
                tester.cspace_free_box().rational_forward_kin().ComputeSValue(q_samples.row(i).transpose(), q_star);
        ...
        const int plane_index = tester.cspace_free_box().map_geometries_to_separating_planes().at(geometry_pair);
        const auto& plane = tester.cspace_free_box().separating_planes()[plane_index];
...
void CheckSosLagrangians(const std::vector<CspaceFreeBox::SeparatingPlaneLagrangians>& lagrangians_vec) {
...
std::optional<Eigen::VectorXd> FindInCollisionPosture(const multibody::MultibodyPlant<double>& plant,
                                                      const SortedPair<geometry::GeometryId>& geometry_pair,
    ...
    ik.get_mutable_prog()->AddBoundingBoxConstraint(q_box_lower, q_box_upper, ik.q());
...
TEST_F(CIrisToyRobotTest, ConstructPlaneSearchProgram) {
    CspaceFreeBoxTester tester(plant_, scene_graph_, SeparatingPlaneOrder::kAffine);
    ...
    const Eigen::VectorXd q_box_lower = 0.48 * q_position_lower + 0.52 * q_position_upper;
    const Eigen::VectorXd q_box_upper = 0.47 * q_position_lower + 0.53 * q_position_upper;
    Eigen::VectorXd s_box_lower;
    Eigen::VectorXd s_box_upper;
    Eigen::VectorXd q_star;
    tester.ComputeSBox(q_box_lower, q_box_upper, &s_box_lower, &s_box_upper, &q_star);
    ...
    tester.GeneratePolynomialsToCertify(s_box_lower, s_box_upper, q_star, ignored_collision_pairs,
                                        &polynomials_to_certify);
    auto get_lagrangian_result = [](const solvers::MathematicalProgramResult& prog_result,
                                    const std::vector<CspaceFreeBox::SeparatingPlaneLagrangians>& lagrangians_vec)
    ...
    auto check_certificate = [&get_lagrangian_result, &tester, this, &q_star](
                                     const CspaceFreeBox::SeparationCertificateProgram& separation_certificate_program,
                                     const solvers::MathematicalProgramResult& result,
                                     const Eigen::MatrixXd& q_samples) {
        const auto positive_side_rational_lagrangians_result = get_lagrangian_result(
                result, separation_certificate_program.certificate.positive_side_rational_lagrangians);
        const auto negative_side_rational_lagrangians_result = get_lagrangian_result(
                result, separation_certificate_program.certificate.negative_side_rational_lagrangians);
        ...
        const auto& plane = tester.cspace_free_box().separating_planes()[separation_certificate_program.plane_index];
        ...
        CheckSeparationBySamples(tester, *(this->diagram_), q_samples, a_result, b_result, q_star, geometry_pair);
    };

    {
        // Test the program for polytopic geometries.
        const int plane_index1 = tester.cspace_free_box().map_geometries_to_separating_planes().at(
                SortedPair<geometry::GeometryId>(world_box_, body3_box_));

        CspaceFreeBox::SeparationCertificateProgram separation_certificate_program = tester.ConstructPlaneSearchProgram(
                polynomials_to_certify.data.plane_geometries[plane_index1],
                polynomials_to_certify.data.s_minus_s_box_lower, polynomials_to_certify.data.s_box_upper_minus_s);
        ...
        for (const auto& lagrangians : separation_certificate_program.certificate.positive_side_rational_lagrangians) {
            for (int i : {0, 1}) {
                EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_lower()(i), symbolic::Polynomial());
                EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_upper()(i), symbolic::Polynomial());
            }
        }
        for (const auto& lagrangians : separation_certificate_program.certificate.negative_side_rational_lagrangians) {
            for (int i : {0, 1}) {
                EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_lower()(i), symbolic::Polynomial());
                EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_upper()(i), symbolic::Polynomial());
            }
        ...
        const auto result = mosek_solver.Solve(*separation_certificate_program.prog);
        EXPECT_TRUE(result.is_success());

        Eigen::MatrixXd q_samples(100, 3);
        q_samples.setZero();
        q_samples.col(2) = Eigen::VectorXd::LinSpaced(q_samples.rows(), q_box_lower(2), q_box_upper(2));
...

    {
        ...
        const int plane_index2 = tester.cspace_free_box().map_geometries_to_separating_planes().at(
        ...
        for (int i = 0; i < separation_certificate_program.prog->indeterminates().size(); ++i) {
            EXPECT_NE(separation_certificate_program.prog->indeterminate(i).get_id(),
                      tester.cspace_free_box().rational_forward_kin().s()(2).get_id());
        }
        // The Lagrangian polynomials for s(i) - s_lower(i) or s_upper(i) - s(i)
        // should be zero when i = 2
        for (const auto& lagrangians : separation_certificate_program.certificate.positive_side_rational_lagrangians) {
            EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_lower()(2), symbolic::Polynomial());
            EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_upper()(2), symbolic::Polynomial());
        }
        for (const auto& lagrangians : separation_certificate_program.certificate.negative_side_rational_lagrangians) {
            EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_lower()(2), symbolic::Polynomial());
            EXPECT_PRED2(symbolic::test::PolyEqual, lagrangians.s_box_upper()(2), symbolic::Polynomial());
        ...
        Eigen::MatrixXd q_samples(100, 3);
        q_samples.setZero();
        q_samples.col(0) = Eigen::VectorXd::LinSpaced(q_samples.rows(), q_box_lower(0), q_box_upper(0));
        q_samples.col(1) = Eigen::VectorXd::LinSpaced(q_samples.rows(), q_box_lower(1), q_box_upper(1));
...
TEST_F(CIrisToyRobotTest, FindSeparationCertificateGivenBoxSuccess) {
    ...
    const Eigen::VectorXd q_position_lower = plant_->GetPositionLowerLimits();
    const Eigen::VectorXd q_position_upper = plant_->GetPositionUpperLimits();
    const Eigen::VectorXd q_box_lower = 0.8 * q_position_lower + 0.2 * q_position_upper;
    const Eigen::VectorXd q_box_upper = 0.7 * q_position_lower + 0.3 * q_position_upper;
    ...
    for (int num_threads : {1, kTestConcurrency}) {
        ...
        const auto certificates_result =
                tester.FindSeparationCertificateGivenBox(ignored_collision_pairs, q_box_lower, q_box_upper, options);
        EXPECT_EQ(certificates_result.size(),
                  tester.cspace_free_box().separating_planes().size() - ignored_collision_pairs.size());
        // The C-space box is collision free.
        EXPECT_TRUE(std::all_of(certificates_result.begin(), certificates_result.end(),
                                [](const std::optional<CspaceFreeBox::SeparationCertificateResult>& result) {
                                    return result.has_value();
                                }));
        // Make sure we really find the separation plane.
        const Eigen::MatrixXd q_samples = CalcBoxGrid(q_box_lower, q_box_upper, {10, 10, 10});
        for (const auto& certificate_result : certificates_result) {
            const auto& separating_plane =
                    tester.cspace_free_box().separating_planes()[certificate_result->plane_index];
            CheckSeparationBySamples(tester, *(this->diagram_), q_samples, certificate_result->a, certificate_result->b,
                                     q_star,
                                     SortedPair<geometry::GeometryId>(separating_plane.positive_side_geometry->id(),
                                                                      separating_plane.negative_side_geometry->id()));
            CheckSosLagrangians(certificate_result->positive_side_rational_lagrangians);
            CheckSosLagrangians(certificate_result->negative_side_rational_lagrangians);
        }
        // Now test the public FindSeparationCertificateGivenBox function.
        std::unordered_map<SortedPair<geometry::GeometryId>, CspaceFreeBox::SeparationCertificateResult> certificates;
        const bool is_success = tester.cspace_free_box().FindSeparationCertificateGivenBox(
                q_box_lower, q_box_upper, ignored_collision_pairs, options, &certificates);
...
TEST_F(CIrisToyRobotTest, FindSeparationCertificateGivenBoxFailure) {
    ...
    const Eigen::VectorXd q_position_upper = plant_->GetPositionUpperLimits();
    const Eigen::VectorXd q_box_lower = 0.8 * q_position_lower + 0.2 * q_position_upper;
    const Eigen::VectorXd q_box_upper = 0.1 * q_position_lower + 0.9 * q_position_upper;
    FindSeparationCertificateOptions options;
    const CspaceFreeBox::IgnoredCollisionPairs ignored_collision_pairs{
            SortedPair<geometry::GeometryId>(world_box_, body2_sphere_)};
    Eigen::VectorXd s_box_lower;
    ...
    for (int num_threads : {1, kTestConcurrency}) {
        ...
        options.terminate_at_failure = false;
        std::unordered_map<SortedPair<geometry::GeometryId>, CspaceFreeBox::SeparationCertificateResult> certificates;
        const bool success = tester.cspace_free_box().FindSeparationCertificateGivenBox(
                q_box_lower, q_box_upper, ignored_collision_pairs, options, &certificates);
        EXPECT_FALSE(success);
        EXPECT_LT(certificates.size(),
                  tester.cspace_free_box().separating_planes().size() - ignored_collision_pairs.size());
        for (const auto& separating_plane : tester.cspace_free_box().separating_planes()) {
            const SortedPair<geometry::GeometryId> geometry_pair{separating_plane.positive_side_geometry->id(),
                                                                 separating_plane.negative_side_geometry->id()};
            if (!ignored_collision_pairs.contains(geometry_pair)) {
                ...
                if (it == certificates.end()) {
                    ...
                    auto plant_context = &(plant_->GetMyMutableContextFromRoot(diagram_context.get()));
                    EXPECT_TRUE(FindInCollisionPosture(*plant_, geometry_pair, plant_context, q_box_lower, q_box_upper,
                                                       std::nullopt)
                                        .has_value());
                } else {
                    const auto& certificate_result = it->second;
                    CheckSeparationBySamples(
                            tester, *(this->diagram_), q_samples, certificate_result.a, certificate_result.b, q_star,
                            SortedPair<geometry::GeometryId>(separating_plane.positive_side_geometry->id(),
                                                             separating_plane.negative_side_geometry->id()));
                }
            }
