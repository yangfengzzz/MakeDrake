...
#include "common/yaml/yaml_io.h"
#include "geometry/optimization/hpolyhedron.h"
#include "geometry/optimization/hyperellipsoid.h"
#include "geometry/optimization/point.h"
#include "geometry/optimization/vpolytope.h"
#include "solvers/choose_best_solver.h"
#include "solvers/clarabel_solver.h"
...
bool MixedIntegerSolverAvailable() {
    return (solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) ||
...
GTEST_TEST(GraphOfConvexSetsOptionsTest, Serialize) {
    ...
    const auto deserialized = yaml::LoadYamlString<GraphOfConvexSetsOptions>(serialized);
...
TEST_F(TwoPoints, AddCost) {
    ...
    EXPECT_THROW(e_->AddCost((e_->xv().head<2>() - e_->xu()).squaredNorm(), {}), std::exception);
    EXPECT_THROW(e_->AddCost(Binding(cost, e_->xu()), {}), std::exception);
    EXPECT_THROW(u_->AddCost((v_->x() + Vector3d::Ones()).squaredNorm(), {}), std::exception);
    ...
    // transcriptions.
    for (const auto& transcription : {Transcription::kMIP, Transcription::kRelaxation, Transcription::kRestriction}) {
...
TEST_F(TwoPoints, AddConstraint) {
    ...
    auto constraint = std::make_shared<LinearConstraint>(Matrix2d::Identity(), Vector2d::Zero(), Vector2d{1.2, 3.4});
    ...
    // If no transcription is specified, the constraint won't be added.
    EXPECT_THROW(e_->AddConstraint(e_->xv().head<2>() == e_->xu(), {}), std::exception);
    EXPECT_THROW(e_->AddConstraint(Binding(constraint, e_->xu()), {}), std::exception);
    EXPECT_THROW(u_->AddConstraint(u_->x() == pu_.x(), {}), std::exception);
    EXPECT_THROW(u_->AddConstraint(Binding(constraint, u_->x()), {}), std::exception);

    // Confirm that they are down-castable.
    auto linear_equality = dynamic_cast<LinearEqualityConstraint*>(b0.evaluator().get());
    EXPECT_TRUE(linear_equality != nullptr);
    auto linear = dynamic_cast<LinearConstraint*>(b1.evaluator().get());
    EXPECT_TRUE(linear != nullptr);
    linear_equality = dynamic_cast<LinearEqualityConstraint*>(u_b0.evaluator().get());
    EXPECT_TRUE(linear_equality != nullptr);
    ...
    symbolic::Variable other_var("x");
    DRAKE_EXPECT_THROWS_MESSAGE(e_->AddConstraint(other_var == 1), ".*IsSubsetOf.*");
    DRAKE_EXPECT_THROWS_MESSAGE(u_->AddConstraint(other_var == 1), ".*IsSubsetOf.*");
}
...
TEST_F(TwoPoints, ReportCorrectSolverId) {
    ...
    // solver.
    if (solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) {
...
TEST_F(TwoPoints, VerifyCostTranscriptionAssignment) {
    ...
    for (const auto& [transcription, cost] :
         {std::pair<Transcription, double>{Transcription::kMIP, kCostMIP},
          std::pair<Transcription, double>{Transcription::kRelaxation, kCostRelaxation},
          std::pair<Transcription, double>{Transcription::kRestriction, kCostRestriction}}) {
...
TEST_F(TwoPoints, VerifyTranscriptionAssignmentBaseline) {
    ...
    e_->AddConstraint(e_->xv()[0] == 3.0,
                      {Transcription::kMIP, Transcription::kRelaxation, Transcription::kRestriction});
...
TEST_F(TwoPoints, VerifyTranscriptionAssignmentkMIP) {
    ...
    e_->AddConstraint(e_->xv()[0] == 0.0, {Transcription::kRelaxation, Transcription::kRestriction});
    u_->AddConstraint(u_->x()[0] == 0.0, {Transcription::kRelaxation, Transcription::kRestriction});
    if (MixedIntegerSolverAvailable()) {
        options.convex_relaxation = false;
        EXPECT_TRUE(g_.SolveShortestPath(*u_, *v_, options).is_success());
    }
...
TEST_F(TwoPoints, VerifyTranscriptionAssignmentkRelaxation) {
    ...
    e_->AddConstraint(e_->xv()[0] == 0.0, {Transcription::kMIP, Transcription::kRestriction});
    u_->AddConstraint(u_->x()[0] == 0.0, {Transcription::kMIP, Transcription::kRestriction});
    options.convex_relaxation = true;
    options.max_rounded_paths = 0;
    EXPECT_TRUE(g_.SolveShortestPath(*u_, *v_, options).is_success());
...
TEST_F(TwoPoints, VerifyTranscriptionAssignmentkRestriction) {
    ...
    e_->AddConstraint(e_->xv()[0] == 0.0, {Transcription::kMIP, Transcription::kRelaxation});
    u_->AddConstraint(u_->x()[0] == 0.0, {Transcription::kMIP, Transcription::kRelaxation});
    EXPECT_TRUE(g_.SolveConvexRestriction({e_}, options).is_success());
...
GTEST_TEST(GraphOfConvexSetsTest, TwoNullPointsConstraint) {
    ...
    Vertex* v = g.AddVertex(pv, "v");
    Edge* e = g.AddEdge(u, v, "e");
    DRAKE_EXPECT_THROWS_MESSAGE(e->AddConstraint(symbolic::Formula::True()), ".*total.*ambient.*dimension.*");
}
...
protected:
    ThreePoints() : p_source_{Vector2d(3., 5.)}, p_target_{Vector2d(-2., 4.)}, p_sink_{Vector2d(5., -2.3)} {
    ...
    void CheckConvexRestriction(const MathematicalProgramResult& result) {
        ...
        log()->info("Solved convex restriction with {}", restriction_result.get_solver_id().name());
        // Confirm that we get a convex solver (not an NLP solver).
        if (MixedIntegerSolverAvailable()) {
            EXPECT_TRUE(restriction_result.get_solver_id() == solvers::MosekSolver::id() ||
                        restriction_result.get_solver_id() == solvers::GurobiSolver::id());
        }
        EXPECT_TRUE(restriction_result.is_success());
        EXPECT_NEAR(result.get_optimal_cost(), restriction_result.get_optimal_cost(), 1e-4);
        for (const auto* v : {source_, target_, sink_}) {
            EXPECT_TRUE(CompareMatrices(result.GetSolution(v->x()), restriction_result.GetSolution(v->x()), 1e-6));
        }
    }
...
TEST_F(ThreePoints, NoMixedIntegerSolverAvailable) {
    ...
    try {
        ...
        EXPECT_NE(std::string(err.what()).find("no solver available that can solve the mixed-integer version"),
                  std::string::npos);
    } catch (...) {
...
TEST_F(ThreePoints, LinearCost1) {
    ...
    EXPECT_TRUE(CompareMatrices(e_on_->GetSolutionPhiXu(result), p_source_.x(), 1e-6));
    EXPECT_TRUE(CompareMatrices(e_on_->GetSolutionPhiXv(result), p_target_.x(), 1e-6));
    EXPECT_TRUE(CompareMatrices(e_off_->GetSolutionPhiXu(result), 0 * p_source_.x(), 1e-6));
    EXPECT_TRUE(CompareMatrices(e_off_->GetSolutionPhiXv(result), 0 * p_sink_.x(), 1e-6));
    ...
    EXPECT_TRUE(CompareMatrices(source_->GetSolution(result4), p_source_.x(), 1e-6));
    EXPECT_TRUE(CompareMatrices(target_->GetSolution(result4), p_target_.x(), 1e-6));
...
TEST_F(ThreePoints, ConvexRelaxation) {
    ...
    EXPECT_NEAR(e_on_->GetSolutionCost(result), e_on_->GetSolutionCost(result2), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), e_off_->GetSolutionCost(result2), 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), source_->GetSolutionCost(result2), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), target_->GetSolutionCost(result2), 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), sink_->GetSolutionCost(result2), 1e-6);
}
...
TEST_F(ThreePoints, LinearCost4) {
    const double b = -1.23;
    e_on_->AddCost(b);
    DRAKE_EXPECT_THROWS_MESSAGE(g_.SolveShortestPath(*source_, *target_, options_),
...
TEST_F(ThreePoints, QuadraticCost) {
    ...
    source_->AddCost(static_cast<const VectorX<Expression>>(source_->x()).squaredNorm());
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result), (p_source_.x() - p_target_.x()).squaredNorm(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), p_source_.x().squaredNorm(), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
    CheckConvexRestriction(result);
}
...
TEST_F(ThreePoints, QuadraticCost2) {
    ...
    // point.
    Expression cost = (A * (e_on_->xu() - Vector2d{.54, -.23}) + A * (e_on_->xv() - Vector2d{7.2, -.73})).squaredNorm();
    e_on_->AddCost(cost);
    e_off_->AddCost(cost.Substitute(subs_on_off_));
    Expression vertex_cost = (A * (source_->x() - Vector2d{.54, -.23})).squaredNorm();
    ...
    EXPECT_NEAR(source_->GetSolutionCost(result), vertex_cost.Evaluate(env), 2e-5);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
    CheckConvexRestriction(result);
}
...
TEST_F(ThreePoints, QuadraticCost3) {
    ...
    Expression cost =
            (A * (e_on_->xu() - Vector2d{.54, -.23}) + A * (e_on_->xv() - Vector2d{7.2, -.73})).squaredNorm() + 4.2;
    e_on_->AddCost(cost);
    e_off_->AddCost(cost.Substitute(subs_on_off_));
    Expression vertex_cost = (A * (source_->x() - Vector2d{.54, -.23})).squaredNorm() + 4.2;
    source_->AddCost(vertex_cost);
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    ASSERT_TRUE(result.is_success());
    Environment env{};
    ...
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 4e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), vertex_cost.Evaluate(env), 2e-4);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
    CheckConvexRestriction(result);
}
...
TEST_F(ThreePoints, QuadraticCost4) {
    ...
    auto cost = std::make_shared<solvers::QuadraticCost>(2.0 * R.transpose() * R, 2.0 * R.transpose() * d, d.dot(d));
    ...
    auto vertex_cost = std::make_shared<solvers::QuadraticCost>(2.0 * R_v.transpose() * R_v,
                                                                2.0 * R_v.transpose() * d_v, d_v.dot(d_v));
    ...
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), (R_v * p_source_.x() + d_v).squaredNorm(), 1e-5);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
    CheckConvexRestriction(result);
}
...
TEST_F(ThreePoints, QuadraticCost6) {
    source_->AddCost(static_cast<const VectorX<Expression>>(source_->x()).squaredNorm() - 2.0);
...
TEST_F(ThreePoints, L1NormCost) {
    ...
    e_off_->AddCost(solvers::Binding(cost, {e_off_->xu(), e_off_->xv()}));
    RowVector2d A_v(1, -1);
    auto vertex_cost = std::make_shared<solvers::L1NormCost>(A_v, Vector1d::Zero());
    source_->AddCost(solvers::Binding(vertex_cost, source_->x()));
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    if (result.get_solver_id() == solvers::IpoptSolver::id()) {
        return;  // See IpoptTest for details.
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result), (p_source_.x() - p_target_.x()).cwiseAbs().sum(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
...
TEST_F(ThreePoints, L1NormCost2) {
  ...
  A << 4.3, .5, -.4, 1.2,
    ...
    EXPECT_NEAR(e_on_->GetSolutionCost(result),
                (A.leftCols(2) * p_source_.x() + A.rightCols(2) * p_target_.x() + b).cwiseAbs().sum(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), abs(A_v * p_source_.x()), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
...
TEST_F(ThreePoints, L2NormCost) {
    ...
    e_off_->AddCost(solvers::Binding(cost, {e_off_->xu(), e_off_->xv()}));
    RowVector2d A_v(1, -1);
    auto vertex_cost = std::make_shared<solvers::L2NormCost>(A_v, Vector1d::Zero());
    source_->AddCost(solvers::Binding(vertex_cost, source_->x()));
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    if (result.get_solver_id() == solvers::IpoptSolver::id()) {
        return;  // See IpoptTest for details.
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result), (p_source_.x() - p_target_.x()).norm(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), (A_v * p_source_.x()).norm(), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
    CheckConvexRestriction(result);
}
...
TEST_F(ThreePoints, L2NormCost2) {
  ...
  A << 4.3, .5, -.4, 1.2,
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result),
                (A.leftCols(2) * p_source_.x() + A.rightCols(2) * p_target_.x() + b).norm(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), (A_v * p_source_.x()).norm(), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
...
TEST_F(ThreePoints, LInfNormCost) {
    ...
    e_off_->AddCost(solvers::Binding(cost, {e_off_->xu(), e_off_->xv()}));
    RowVector2d A_v(1, -1);
    auto vertex_cost = std::make_shared<solvers::LInfNormCost>(A_v, Vector1d::Zero());
    source_->AddCost(solvers::Binding(vertex_cost, source_->x()));
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    if (result.get_solver_id() == solvers::IpoptSolver::id()) {
        return;  // See IpoptTest for details.
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result), (p_source_.x() - p_target_.x()).cwiseAbs().maxCoeff(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), abs(A_v * p_source_.x()), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
...
TEST_F(ThreePoints, LInfNormCost2) {
  ...
  A << 4.3, .5, -.4, 1.2,
    ...
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(e_on_->GetSolutionCost(result),
                (A.leftCols(2) * p_source_.x() + A.rightCols(2) * p_target_.x() + b).cwiseAbs().maxCoeff(), 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(source_->GetSolutionCost(result), abs(A_v * p_source_.x()), 1e-6);
    EXPECT_NEAR(target_->GetSolutionCost(result), 0.0, 1e-6);
    EXPECT_NEAR(sink_->GetSolutionCost(result), 0.0, 1e-6);
...
TEST_F(ThreePoints, PerspectiveQuadraticCost) {
  ...
  A << -1, 1, 0, 0,
    ...
    A_v << -1, 1, 1, 1;
    auto vertex_cost = std::make_shared<solvers::PerspectiveQuadraticCost>(A_v, b);
    source_->AddCost(solvers::Binding(vertex_cost, source_->x()));
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    if (result.get_solver_id() == solvers::IpoptSolver::id()) {
        return;  // See IpoptTest for details.
    ...
    const double expected_cost =
            std::pow(p_target_.x()(0) + p_target_.x()(1) + b(1), 2) / (p_source_.x()(1) - p_source_.x()(0) + b(0));
    EXPECT_NEAR(e_on_->GetSolutionCost(result), expected_cost, 1e-6);
    EXPECT_NEAR(e_off_->GetSolutionCost(result), 0.0, 1e-6);
    const double vertex_expected_cost =
            std::pow(p_source_.x()(0) + p_source_.x()(1) + b(1), 2) / (p_source_.x()(1) - p_source_.x()(0) + b(0));
...
TEST_F(ThreePoints, GetSolutionPath) {
    ...
    // Bad source.
    DRAKE_EXPECT_THROWS_MESSAGE(g_.GetSolutionPath(*v, *target_, result), ".*Source.*is not a vertex.*");
    // Bad target.
    DRAKE_EXPECT_THROWS_MESSAGE(g_.GetSolutionPath(*source_, *v, result), ".*Target.*is not a vertex.*");

    // is_success is false.
    result.set_solution_result(solvers::SolutionResult::kInfeasibleConstraints);
    DRAKE_EXPECT_THROWS_MESSAGE(g_.GetSolutionPath(*source_, *target_, result), ".*Cannot extract a solution.*");
}
...
protected:
    ...
    void CheckConvexRestriction(const MathematicalProgramResult& result) {
        ...
        EXPECT_TRUE(restriction_result.is_success());
        EXPECT_NEAR(result.get_optimal_cost(), restriction_result.get_optimal_cost(), 1e-6);
        for (const auto* v : {source_, target_, sink_}) {
            EXPECT_TRUE(CompareMatrices(result.GetSolution(v->x()), restriction_result.GetSolution(v->x()), 1e-6));
        }
    }
...
TEST_F(ThreeBoxes, NonConvexRounding) {
    ...
    */
    ...
    constraints_relaxation.push_back(source_->AddConstraint(source_->x()[0] <= -0.5, {Transcription::kRelaxation}));
    constraints_relaxation.push_back(source_->AddConstraint(source_->x()[1] >= 0.5, {Transcription::kRelaxation}));

    constraints_restriction.push_back(source_->AddConstraint(pow(source_->x()[0], 2) + pow(source_->x()[1], 2) >= 1.0,
                                                             {Transcription::kRestriction}));
    constraints_restriction.push_back(source_->AddConstraint(source_->x()[0] <= -0.1, {Transcription::kRestriction}));
    constraints_restriction.push_back(source_->AddConstraint(source_->x()[1] >= 0.1, {Transcription::kRestriction}));

    // Verify the constraints in the relaxation only contain the relaxed
    // constraints, but not the resctriction constraints.
    for (const auto& constraint : source_->GetConstraints({Transcription::kRelaxation})) {
        EXPECT_TRUE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                    constraints_relaxation.end());
        EXPECT_FALSE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
    ...
    for (const auto& constraint : source_->GetConstraints({Transcription::kRestriction})) {
        EXPECT_FALSE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                     constraints_relaxation.end());
        EXPECT_TRUE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
                    constraints_restriction.end());
    }
    ...
    └────────────┘   └────────────┘
    */
    constraints_relaxation.push_back(target_->AddConstraint(target_->x()[0] >= 0.5, {Transcription::kRelaxation}));
    constraints_relaxation.push_back(target_->AddConstraint(target_->x()[1] >= 0.5, {Transcription::kRelaxation}));

    constraints_restriction.push_back(target_->AddConstraint(pow(target_->x()[0], 2) + pow(target_->x()[1], 2) >= 1.0,
                                                             {Transcription::kRestriction}));
    constraints_restriction.push_back(target_->AddConstraint(target_->x()[0] >= 0.1, {Transcription::kRestriction}));
    constraints_restriction.push_back(target_->AddConstraint(target_->x()[1] >= 0.1, {Transcription::kRestriction}));

    // Verify the constraints in the relaxation only contain the relaxed
    // constraints, but not the restriction constraints.
    for (const auto& constraint : target_->GetConstraints({Transcription::kRelaxation})) {
        EXPECT_TRUE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                    constraints_relaxation.end());
        EXPECT_FALSE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
                     constraints_restriction.end());
    ...
    // Verify the constraints in the restriction only contain the appropiate
    // constraints, but not the relaxed constraints.
    for (const auto& constraint : target_->GetConstraints({Transcription::kRestriction})) {
        EXPECT_FALSE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                     constraints_relaxation.end());
        EXPECT_TRUE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
                    constraints_restriction.end());
    ...
    */
    ...
    constraints_relaxation.push_back(sink_->AddConstraint(sink_->x()[0] >= 0.5, {Transcription::kRelaxation}));
    constraints_relaxation.push_back(sink_->AddConstraint(sink_->x()[1] <= -0.5, {Transcription::kRelaxation}));

    constraints_restriction.push_back(
            sink_->AddConstraint(pow(sink_->x()[0], 2) + pow(sink_->x()[1], 2) >= 1.0, {Transcription::kRestriction}));
    constraints_restriction.push_back(sink_->AddConstraint(sink_->x()[0] >= 0.1, {Transcription::kRestriction}));
    constraints_restriction.push_back(sink_->AddConstraint(sink_->x()[1] <= -0.1, {Transcription::kRestriction}));

    // Verify the constraints in the relaxation only contain the relaxed
    // constraints, but not the restriction constraints.
    for (const auto& constraint : sink_->GetConstraints({Transcription::kRelaxation})) {
        EXPECT_TRUE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                    constraints_relaxation.end());
        EXPECT_FALSE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
                     constraints_restriction.end());
    ...
    // Verify the constraints in the restriction only contain the appropiate
    // constraints, but not the relaxed constraints.
    for (const auto& constraint : sink_->GetConstraints({Transcription::kRestriction})) {
        EXPECT_FALSE(std::find(constraints_relaxation.begin(), constraints_relaxation.end(), constraint) !=
                     constraints_relaxation.end());
        EXPECT_TRUE(std::find(constraints_restriction.begin(), constraints_restriction.end(), constraint) !=
                    constraints_restriction.end());
    ...
    const double distance = (source_->GetSolution(result) - target_->GetSolution(result)).norm();
...
TEST_F(ThreeBoxes, LinearEqualityConstraint2) {
  ...
  Aeq << 4.3, .5, -.4, 1.2,
    ...
    auto constraint = std::make_shared<solvers::LinearEqualityConstraint>(Aeq, beq);
    e_on_->AddConstraint(solvers::Binding(constraint, {e_on_->xu(), e_on_->xv()}));
    e_off_->AddConstraint(solvers::Binding(constraint, {e_off_->xu(), e_off_->xv()}));
    Matrix<double, 2, 2> Aeq_v;
    Aeq_v << 4.5, 0.7, 0.3, 0.6;
    auto vertex_constraint = std::make_shared<solvers::LinearEqualityConstraint>(Aeq_v, beq);
    ...
    EXPECT_TRUE(CompareMatrices(
            Aeq.leftCols(2) * source_->GetSolution(result) + Aeq.rightCols(2) * target_->GetSolution(result), beq,
            1e-6));
    EXPECT_TRUE(sink_->GetSolution(result).hasNaN());
    CheckConvexRestriction(result);
}
...
TEST_F(ThreeBoxes, LinearConstraint) {
    ...
    const Vector2d b{.5, .3};
    e_on_->AddConstraint(CreateBinding(std::make_shared<LinearConstraint>(A, b, Vector2d::Constant(kInf)),
                                       e_on_->xv()));  // b ≤ e_on_->xv() ≤ ∞.
    e_off_->AddConstraint(e_off_->xv() >= b);
    source_->AddConstraint(CreateBinding(std::make_shared<LinearConstraint>(A, Vector2d::Constant(-kInf), -b),
...
TEST_F(ThreeBoxes, LinearConstraint2) {
  ...
  A << 4.3, .5, -.4, 1.2,
    ...
    Matrix<double, 2, 2> A_v;
    A_v << 4.5, 0.7, 0.3, 0.6;
    auto vertex_constraint = std::make_shared<solvers::LinearConstraint>(A_v, lb, ub);
    source_->AddConstraint(solvers::Binding(vertex_constraint, source_->x()));
    auto result = g_.SolveShortestPath(*source_, *target_, options_);
    ASSERT_TRUE(result.is_success());
    EXPECT_TRUE(((A_v * source_->GetSolution(result)).array() <= ub.array() + 1e-6).all());
    EXPECT_TRUE(((A_v * source_->GetSolution(result)).array() >= lb.array() - 1e-6).all());
    EXPECT_TRUE(
            ((A.leftCols(2) * source_->GetSolution(result) + A.rightCols(2) * target_->GetSolution(result)).array() <=
             ub.array() + 1e-6)
                    .all());
    EXPECT_TRUE(
            ((A.leftCols(2) * source_->GetSolution(result) + A.rightCols(2) * target_->GetSolution(result)).array() >=
    ...
    EXPECT_FALSE(CompareMatrices(source_->GetSolution(result), Vector2d::Zero(), 1e-6));
    EXPECT_FALSE(CompareMatrices(target_->GetSolution(result), Vector2d::Zero(), 1e-6));
    CheckConvexRestriction(result);
}
...
TEST_F(ThreeBoxes, LinearConstraint3) {
    ...
    e_on_->AddConstraint(CreateBinding(std::make_shared<LinearConstraint>(A, b, Vector2d::Constant(kInf)),
                                       e_on_->xv()));  // b ≤ e_on_->xv() ≤ ∞.
    e_off_->AddConstraint(CreateBinding(std::make_shared<LinearConstraint>(A, b, Vector2d::Constant(kInf)),
...
TEST_F(ThreeBoxes, InvalidLinearConstraintUpper) {
    ...
    const Vector2d b{.5, .3};
    e_on_->AddConstraint(
            CreateBinding(std::make_shared<LinearConstraint>(A, b, Vector2d::Constant(-kInf)), e_on_->xv()));
    // b ≤ e_on_->xv() ≤ -∞. We can't take the perspective of such a constraint,
    // so solving should throw an error.
    DRAKE_EXPECT_THROWS_MESSAGE(g_.SolveShortestPath(*source_, *target_, options_), ".*inf.*");
}
...
TEST_F(ThreeBoxes, InvalidLinearConstraintLower) {
    ...
    const Vector2d b{.5, .3};
    e_on_->AddConstraint(
            CreateBinding(std::make_shared<LinearConstraint>(A, Vector2d::Constant(kInf), b), e_on_->xv()));
...
TEST_F(ThreeBoxes, LorentzConeConstraint) {
  ...
  A << 1, 2, 3, 4,
    ...
    auto z = A * res + b;
    EXPECT_GE(z[0], std::sqrt(std::pow(z[1], 2) + std::pow(z[2], 2) + std::pow(z[3], 2) + std::pow(z[4], 2)));
    EXPECT_GE(z[0], 0);
}
...
TEST_F(ThreeBoxes, PositiveSemidefiniteConstraint1) {
    ...
    psd_x_off << e_off_->xu()[0], e_off_->xu()[1], e_off_->xu()[1], e_off_->xv()[0];
    ...
    Eigen::Matrix2d mat;
    mat << source_->GetSolution(result)[0], source_->GetSolution(result)[1], source_->GetSolution(result)[1],
            target_->GetSolution(result)[0];
...
TEST_F(ThreeBoxes, RotatedLorentzConeConstraint) {
  ...
  A << 1, 2, 3, 4,
    ...
    auto constraint = std::make_shared<solvers::RotatedLorentzConeConstraint>(A, b);
    e_on_->AddConstraint(solvers::Binding(constraint, {e_on_->xu(), e_on_->xv()}));
    e_off_->AddConstraint(solvers::Binding(constraint, {e_off_->xu(), e_off_->xv()}));
    ...
    res << source_->GetSolution(result), target_->GetSolution(result);
    auto z = A * res + b;
    EXPECT_GE(z[0] * z[1], std::sqrt(std::pow(z[2], 2) + std::pow(z[3], 2) + std::pow(z[4], 2)));
    EXPECT_GE(z[0], 0);
    EXPECT_GE(z[1], 0);
}
...
TEST_F(ThreeBoxes, SolveConvexRestriction) {
    ...
    auto restriction_result = g_.SolveConvexRestriction(std::vector<const Edge*>{e_on_}, options_);
...
GTEST_TEST(ShortestPathTest, ClassicalShortestPath) {
    ...
    spp.AddEdge(v[1], v[4])->AddCost(2.5);  // Updated from original to break symmetry.
    ...
    EXPECT_EQ(spp.GetSolutionPath(*v[0], *v[4], result), std::vector<const Edge*>({v0_to_v3, v3_to_v2, v2_to_v4}));
...
GTEST_TEST(ShortestPathTest, InfeasibleProblem) {
    ...
    ASSERT_FALSE(result.is_success());
    EXPECT_EQ(result.get_solution_result(), SolutionResult::kInfeasibleConstraints);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(spp.GetSolutionPath(*source, *target, result), ".*is_success.*");
}
...
GTEST_TEST(ShortestPathTest, TwoStepLoopConstraint) {
    ...
    spp.AddEdge(v[0], v[1])->AddCost(solvers::Binding(cost, {v[0]->x(), v[1]->x()}));
    spp.AddEdge(v[1], v[0])->AddCost(solvers::Binding(cost, {v[1]->x(), v[0]->x()}));
    spp.AddEdge(v[1], v[2])->AddCost(solvers::Binding(cost, {v[1]->x(), v[2]->x()}));
    spp.AddEdge(v[2], v[1])->AddCost(solvers::Binding(cost, {v[2]->x(), v[1]->x()}));
    spp.AddEdge(v[2], v[5])->AddCost(solvers::Binding(cost, {v[2]->x(), v[5]->x()}));
    spp.AddEdge(v[5], v[2])->AddCost(solvers::Binding(cost, {v[5]->x(), v[2]->x()}));

    spp.AddEdge(v[0], v[3])->AddCost(solvers::Binding(cost, {v[0]->x(), v[3]->x()}));
    spp.AddEdge(v[3], v[0])->AddCost(solvers::Binding(cost, {v[3]->x(), v[0]->x()}));
    spp.AddEdge(v[3], v[4])->AddCost(solvers::Binding(cost, {v[3]->x(), v[4]->x()}));
    spp.AddEdge(v[4], v[3])->AddCost(solvers::Binding(cost, {v[4]->x(), v[3]->x()}));
    spp.AddEdge(v[4], v[5])->AddCost(solvers::Binding(cost, {v[4]->x(), v[5]->x()}));
    spp.AddEdge(v[5], v[4])->AddCost(solvers::Binding(cost, {v[5]->x(), v[4]->x()}));
...
GTEST_TEST(ShortestPathTest, PhiConstraint) {
    ...
    A.block(0, 2, 2, 2) = -Matrix2d::Identity();
    A.block(2, 0, 2, 2) = -Matrix2d::Identity();
    auto cost = std::make_shared<solvers::QuadraticCost>(2.0 * A, Vector4d::Zero());
    ...
    // imposed.
    {
        ...
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXu(result), 0.5 * Vector2d(1, -1), 1e-6));
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXv(result), 0.5 * Vector2d(2, 0), 1e-6));
        ...
        EXPECT_TRUE(CompareMatrices(v[1]->GetSolution(result), Vector2d(1, -1), 1e-6));
    }
    ...
    edge_13->AddPhiConstraint(false);
    {
        auto result = spp.SolveShortestPath(*v[0], *v[3], options);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXu(result), Vector2d::Zero(), 1e-6));
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXv(result), Vector2d::Zero(), 1e-6));
    ...
    edge_13->AddPhiConstraint(true);
    {
        ...
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXu(result), Vector2d(1, -1), 1e-6));
        EXPECT_TRUE(CompareMatrices(edge_13->GetSolutionPhiXv(result), Vector2d(2, 0), 1e-6));
...
protected:
    ...
    std::set<EdgeId> PreprocessShortestPath(VertexId source_id, VertexId target_id) {
...
TEST_F(PreprocessShortestPathTest, CheckEdges) {
    std::set<EdgeId> removed_edges = PreprocessShortestPath(v_[0]->id(), v_[5]->id());
...
TEST_F(PreprocessShortestPathTest, CheckResults) {
    ...
    for (Edge* e : edges_) {
        EXPECT_NEAR(result1.GetSolution(e->phi()), result2.GetSolution(e->phi()), 1e-10);
        EXPECT_TRUE(CompareMatrices(result1.GetSolution(e->xu()), result2.GetSolution(e->xu()), 1e-12));
        EXPECT_TRUE(CompareMatrices(result1.GetSolution(e->xv()), result2.GetSolution(e->xv()), 1e-12));
        EXPECT_NEAR(e->GetSolutionCost(result1), e->GetSolutionCost(result2), 1e-10);
    }
}
...
GTEST_TEST(ShortestPathTest, RoundedSolution) {
    ...
    Vertex* p1 = spp.AddVertex(HPolyhedron::MakeBox(Vector2d(-2, -2), Vector2d(2, -1)));
    Vertex* p2 = spp.AddVertex(HPolyhedron::MakeBox(Vector2d(-2, -2), Vector2d(-1, 2)));
    Vertex* p3 = spp.AddVertex(HPolyhedron::MakeBox(Vector2d(1, -2), Vector2d(2, 2)));
    Vertex* p4 = spp.AddVertex(HPolyhedron::MakeBox(Vector2d(-2, 1), Vector2d(2, 2)));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(spp.GetSolutionPath(*source, *target, relaxed_result, 0 /* tolerance*/), ".*No path.*");
    // However, relaxing the tolerance (significantly) will find a path.
    EXPECT_NO_THROW(spp.GetSolutionPath(*source, *target, relaxed_result, 0.9 /* tolerance*/));
    ...
    EXPECT_LT(relaxed_result.get_optimal_cost(), rounded_result.get_optimal_cost());
    ...
    for (size_t ii = 0; ii < edges.size(); ++ii) {
        if (ii < 6) {
            ...
            const double tol = (relaxed_result.get_solver_id() == solvers::GurobiSolver::id()) ? 1e-1
    ...
    EXPECT_NEAR(rounded_result.get_optimal_cost(), mip_result.get_optimal_cost(), 2e-6);
    ...
    if (solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) {
        ...
        options.restriction_solver_options->SetOption(solvers::MosekSolver::id(), "MSK_IPAR_INTPNT_MAX_ITERATIONS", 0);
        ...
        options.preprocessing_solver_options = solvers::SolverOptions();
        options.preprocessing_solver_options->SetOption(solvers::MosekSolver::id(), "MSK_DPAR_OPTIMIZER_MAX_TIME", 0.0);
...
GTEST_TEST(ShortestPathTest, RoundingBacktrack) {
    ...
    Vector3d b(3, 3, -1);
    Vertex* v0 =
            spp.AddVertex(HPolyhedron((Matrix<double, 3, 2>() << -1, 0, 0, -1, 1, 1).finished(), b).CartesianPower(2));
    Vertex* v1 =
            spp.AddVertex(HPolyhedron((Matrix<double, 3, 2>() << 1, 0, 0, -1, -1, 1).finished(), b).CartesianPower(2));
    Vertex* v2 =
            spp.AddVertex(HPolyhedron((Matrix<double, 3, 2>() << -1, 0, 0, 1, 1, -1).finished(), b).CartesianPower(2));
    ...
    auto continuity_con = std::make_shared<solvers::LinearEqualityConstraint>(A_diff, Vector2d::Zero());
    for (Edge* e : source_edges) {
        e->AddConstraint(solvers::Binding(continuity_con, {e->xu(), e->xv().head<2>()}));
    }
    for (Edge* e : source_edges) {
        e->AddConstraint(solvers::Binding(continuity_con, {e->xu().tail<2>(), e->xv().head<2>()}));
    }
    for (Edge* e : target_edges) {
        e->AddConstraint(solvers::Binding(continuity_con, {e->xu().tail<2>(), e->xv()}));
    }
...
GTEST_TEST(ShortestPathTest, TobiasToyExample) {
  ...
  vertices <<  2, 1, 2, 4, 4,
    ...
    Vertex* e1 = spp.AddVertex(Hyperellipsoid(Matrix2d::Identity(), Vector2d(4, 1)), "e1");
    Vertex* e2 = spp.AddVertex(Hyperellipsoid(Matrix2d(Vector2d(.25, 1).asDiagonal()), Vector2d(7, -2)), "e2");
    // clang-format off
  vertices.resize(2, 3);
  vertices <<  5, 7, 6,
               4, 4, 3;
  ...
  vertices <<  7, 8, 9, 8,
    ...
    EXPECT_EQ(spp.GetSolutionPath(*source, *target, result),
              std::vector<const Edge*>({source_to_p3, p3_to_e1, e1_to_p4, p4_to_p5, p5_to_target}));

    const std::forward_list<Vertex*> shortest_path{source, p3, e1, p4, p5, target};
    for (const auto& e : spp.Edges()) {
    ...
    EXPECT_NEAR(result.get_optimal_cost(), active_edges_result.get_optimal_cost(), 3e-5);
    // The vertex solutions should match on the shortest path.
    EXPECT_TRUE(CompareMatrices(result.GetSolution(source->x()), active_edges_result.GetSolution(source->x()), 2e-3));
    for (const auto* e : path) {
        EXPECT_TRUE(CompareMatrices(result.GetSolution(e->xv()), active_edges_result.GetSolution(e->xv()), 2e-3));
    }
    ...
    source_to_p2->AddPhiConstraint(true);
    {
        ...
        for (const auto& e : spp.Edges()) {
            auto iter = std::find(new_shortest_path.begin(), new_shortest_path.end(), &e->u());
    ...
    source_to_p3->AddPhiConstraint(false);
    {
        ...
        const std::forward_list<Vertex*> new_shortest_path{source, p1, e2, e1, p5, target};
        for (const auto& e : spp.Edges()) {
            auto iter = std::find(new_shortest_path.begin(), new_shortest_path.end(), &e->u());
...
GTEST_TEST(ShortestPathTest, Figure9) {
    ...
    Vertex* v3 = spp.AddVertex(HPolyhedron::MakeBox(Vector2d(2, -2), Vector2d(4, 2)));
    ...
    ASSERT_TRUE(relaxed_result.is_success());
    EXPECT_EQ(rounded_result.get_solution_result(), SolutionResult::kIterationLimit);
    for (const auto& e : spp.Edges()) {
        EXPECT_NEAR(relaxed_result.GetSolution(e->phi()), rounded_result.GetSolution(e->phi()), 1e-10);
        EXPECT_TRUE(CompareMatrices(relaxed_result.GetSolution(e->xu()), rounded_result.GetSolution(e->xu()), 1e-12));
        EXPECT_TRUE(CompareMatrices(relaxed_result.GetSolution(e->xv()), rounded_result.GetSolution(e->xv()), 1e-12));
        EXPECT_NEAR(e->GetSolutionCost(relaxed_result), e->GetSolutionCost(rounded_result), 1e-10);
    }
}
...
GTEST_TEST(ShortestPathTest, SavvaBoxExample) {
    auto add_edge_and_constraints = [](GraphOfConvexSets* gcs, Vertex* v_left, Vertex* v_right) {
        Edge* edge = gcs->AddEdge(v_left, v_right, v_left->name() + "-" + v_right->name());
    ...
    Vertex* v_left = gcs.AddVertex(HPolyhedron::MakeBox(Vector4d{0, 0, 0, 0}, Vector4d{2, 6, 2, 6}), "left");
    Vertex* v_above = gcs.AddVertex(HPolyhedron::MakeBox(Vector4d{0, 4, 0, 4}, Vector4d{6, 6, 6, 6}), "above");
    Vertex* v_right = gcs.AddVertex(HPolyhedron::MakeBox(Vector4d{4, 0, 4, 0}, Vector4d{6, 6, 6, 6}), "right");
    Vertex* v_below = gcs.AddVertex(HPolyhedron::MakeBox(Vector4d{0, 0, 0, 0}, Vector4d{6, 2, 6, 2}), "below");
    ...
    for (auto* e : gcs.Edges()) {
        if (e->name().find("start") == std::string::npos && e->name().find("target") == std::string::npos) {
            ...
            // not enforced).
            EXPECT_FALSE(CompareMatrices(result.GetSolution(e->xu()).tail<2>(), result.GetSolution(e->xv()).head<2>(),
                                         1e-4));
            // But the underlying edge decision variables *are* equal.
            EXPECT_TRUE(CompareMatrices(e->GetSolutionPhiXu(result).tail<2>(), e->GetSolutionPhiXv(result).head<2>(),
                                        1e-4));
        }
    }
}
...
GTEST_TEST(ShortestPathTest, Graphviz) {
    ...
    EXPECT_THAT(g.GetGraphvizString(), AllOf(HasSubstr("source"), HasSubstr("target"), HasSubstr("source_to_target")));
    auto result = g.SolveShortestPath(*source, *target, options);
    EXPECT_THAT(g.GetGraphvizString(result), AllOf(HasSubstr("x ="), HasSubstr("cost ="), HasSubstr("ϕ ="),
    ...
    // No slack variables.
    EXPECT_THAT(g.GetGraphvizString(result, false), AllOf(HasSubstr("x ="), HasSubstr("cost ="), Not(HasSubstr("ϕ =")),
    ...
    EXPECT_THAT(g.GetGraphvizString(result, false, 2, true), AllOf(HasSubstr("x = [1 2]"), HasSubstr("x = [1e-05]")));
}
