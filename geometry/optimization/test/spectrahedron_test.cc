...
GTEST_TEST(SpectrahedronTest, Attributes) {
    EXPECT_TRUE(
            Spectrahedron::supported_attributes().contains(solvers::ProgramAttribute::kPositiveSemidefiniteConstraint));
}
...
GTEST_TEST(SpectrahedronTest, TrivialSdp1) {
    ...
    const auto [new_vars, new_constraints] = spect.AddPointInSetConstraints(&prog2, x2);
    ...
    EXPECT_TRUE(CompareMatrices(prog.linear_equality_constraints()[0].evaluator()->GetDenseA(),
    ...
    EXPECT_TRUE(CompareMatrices(prog.linear_constraints()[0].evaluator()->GetDenseA(),
                                prog2.linear_constraints()[0].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(prog.linear_constraints()[0].evaluator()->lower_bound(),
                                prog2.linear_constraints()[0].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(prog.linear_constraints()[0].evaluator()->upper_bound(),
                                prog2.linear_constraints()[0].evaluator()->upper_bound()));
    EXPECT_EQ(prog2.bounding_box_constraints().size(), 1);
    EXPECT_TRUE(CompareMatrices(prog.bounding_box_constraints()[0].evaluator()->lower_bound(),
    ...
    MatrixXd A(1, 4);
    A << 1, 1, 1, -1;
    EXPECT_TRUE(CompareMatrices(prog3.linear_equality_constraints()[0].evaluator()->GetDenseA(), A));
    EXPECT_EQ(prog3.linear_constraints().size(),
              2 /* from the bounding box constraint */ + 2 /* from the linear constraint */);
    A.resize(1, 4);
    A << 1, -2, 1, 2;
    EXPECT_TRUE(CompareMatrices(prog3.linear_constraints()[0].evaluator()->GetDenseA(), A));
    A << 1, -2, 1, 0;
    EXPECT_TRUE(CompareMatrices(prog3.linear_constraints()[1].evaluator()->GetDenseA(), A));
    A.resize(1, 2);
    A << 1, 1;
    EXPECT_TRUE(CompareMatrices(prog3.linear_constraints()[2].evaluator()->GetDenseA(), A));
    A << 1, -1;
    EXPECT_TRUE(CompareMatrices(prog3.linear_constraints()[3].evaluator()->GetDenseA(), A));
    Vector<double, 7> xt_test;
    xt_test << x_star, 1;
    EXPECT_TRUE(prog3.CheckSatisfied(prog3.GetAllConstraints(), xt_test));
    EXPECT_TRUE(prog3.CheckSatisfied(prog3.GetAllConstraints(), VectorXd::Zero(7)));
    xt_test << x_bad, 1;
    ...
    spect.AddPointInNonnegativeScalingConstraints(&prog4, MatrixXd::Identity(6, 6), Vector6d::Zero(), Vector1d::Ones(),
                                                  0, x4, t4);

    // We expect prog4 to have one additional linear constraint (for t >= 0).
    EXPECT_EQ(prog3.linear_constraints().size() + 1, prog4.linear_constraints().size());
    ...
    for (int i = 0; i < static_cast<int>(prog3.linear_constraints().size()); ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(prog3.EvalBindingAtInitialGuess(b3), prog4.EvalBindingAtInitialGuess(b4), kTol));
        EXPECT_TRUE(CompareMatrices(b3.evaluator()->lower_bound(), b4.evaluator()->lower_bound(), kCoeffTol));
        EXPECT_TRUE(CompareMatrices(b3.evaluator()->upper_bound(), b4.evaluator()->upper_bound(), kCoeffTol));
    }
    ...
    spect.AddPointInNonnegativeScalingConstraints(&prog5, MatrixXd::Identity(6, 8), Vector6d::Zero(), Vector2d(1, 0), 0,
                                                  x5, t5);
    ...
    for (int i = 0; i < static_cast<int>(prog3.linear_constraints().size()); ++i) {
        ...
        // b4 depends on all of the variables, but b3 just depends on a subset, so
        // we eval instead of checking the coefficients.
        EXPECT_TRUE(CompareMatrices(prog3.EvalBindingAtInitialGuess(b3), prog5.EvalBindingAtInitialGuess(b5), kTol));
        EXPECT_TRUE(CompareMatrices(b3.evaluator()->lower_bound(), b5.evaluator()->lower_bound(), kCoeffTol));
        EXPECT_TRUE(CompareMatrices(b3.evaluator()->upper_bound(), b5.evaluator()->upper_bound(), kCoeffTol));
    }
    // We only expect the first linear equality constraint to match (prog4 has
    // additional equality constraints for the PSD slack variables).
    {
...
GTEST_TEST(SpectrahedronTest, Move) {
    auto make_sdp = []() {
        ...
        sdp->AddPositiveSemidefiniteConstraint(sdp->NewSymmetricContinuousVariables<3>());
        return sdp;
    };
    ...
    EXPECT_NO_THROW(dut.AddPointInSetConstraints(&constraints, constraints.NewContinuousVariables<6>()));
