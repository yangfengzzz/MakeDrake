...
GTEST_TEST(IrisTest, SmallBox) {
    ...
    obstacles.emplace_back(VPolytope::MakeBox(Vector2d(-.4, -1), Vector2d(.4, -.5)));
    const HPolyhedron domain = HPolyhedron::MakeUnitBox(2);
...
GTEST_TEST(IrisTest, BoundingRegion) {
    ConvexSets obstacles;
    obstacles.emplace_back(VPolytope::MakeBox(Vector2d(0.1, 0.5), Vector2d(1, 1)));
    obstacles.emplace_back(VPolytope::MakeBox(Vector2d(-1, -1), Vector2d(-0.1, -0.5)));
    obstacles.emplace_back(HPolyhedron::MakeBox(Vector2d(.1, -1), Vector2d(1, -0.5)));
    obstacles.emplace_back(HPolyhedron::MakeBox(Vector2d(-1, 0.5), Vector2d(-0.1, 1)));
    const HPolyhedron domain = HPolyhedron::MakeUnitBox(2);
    ...
    const HPolyhedron region_w_bounding = Iris(obstacles, sample, domain, options);
...
GTEST_TEST(IrisTest, TerminationConditions) {
    ...
    options.relative_termination_threshold = -1;
    const HPolyhedron abs_volume_region = Iris(obstacles, sample, domain, options);
    ...
    const HPolyhedron rel_volume_region = Iris(obstacles, sample, domain, options);
...
GTEST_TEST(IrisTest, TerminationFunc) {
    ...
    const HPolyhedron region_no_termination = Iris(obstacles, sample, domain, options);
    EXPECT_FALSE(region_no_termination.PointInSet(q1) && region_no_termination.PointInSet(q2));
...
GTEST_TEST(IrisTest, ClosestPointFailure) {
  ...
  centers << -4.42, -3.89, 0.42,
    ...
    for (int ii = 0; ii < radii.size(); ++ii) {
        obstacles.emplace_back(Hyperellipsoid::MakeHypersphere(radii[ii], centers.col(ii)));
        shrunk_obstacles.emplace_back(Hyperellipsoid::MakeHypersphere(0.99 * radii[ii], centers.col(ii)));
    }

    const HPolyhedron domain = HPolyhedron::MakeBox(Vector2d::Constant(-10.), Vector2d::Constant(10.));
...
GTEST_TEST(IrisOptionsTest, Serialize) {
    ...
    EXPECT_EQ(options.require_sample_point_is_contained, options2.require_sample_point_is_contained);
    ...
    EXPECT_EQ(options.relative_termination_threshold, options2.relative_termination_threshold);
    EXPECT_EQ(options.configuration_space_margin, options2.configuration_space_margin);
    EXPECT_EQ(options.num_collision_infeasible_samples, options2.num_collision_infeasible_samples);
...
protected:
    ...
    const QueryObject<double>& UpdateContextAndGetQueryObject() {
        ...
        return scene_graph_.get_query_output_port().Eval<QueryObject<double>>(*context_);
    }

    GeometryId RegisterAnchoredShape(const Shape& shape, const RigidTransformd& X_WS) {
        auto instance = std::make_unique<GeometryInstance>(X_WS, shape.Clone(), "S" + std::to_string(shape_count_++));
        instance->set_proximity_properties(ProximityProperties());
        return scene_graph_.RegisterAnchoredGeometry(source_id_, std::move(instance));
    }
...
TEST_F(SceneGraphTester, MakeCylinder) {
    RegisterAnchoredShape(Cylinder(1., 2.), RigidTransformd(Vector3d{1., 2., 3.}));
    auto query = UpdateContextAndGetQueryObject();
    auto obstacles = MakeIrisObstacles(query);
    EXPECT_EQ(obstacles.size(), 1);
...
TEST_F(SceneGraphTester, MakeEllipsoid) {
    RegisterAnchoredShape(Ellipsoid(1., 2., 3.), RigidTransformd(Vector3d{1., 2., 3.}));
    auto query = UpdateContextAndGetQueryObject();
    auto obstacles = MakeIrisObstacles(query);
    EXPECT_EQ(obstacles.size(), 1);
    EXPECT_NE(dynamic_cast<const Hyperellipsoid*>(obstacles[0].get()), nullptr);
