...
GTEST_TEST(HyperrectangleTest, BasicTests) {
    ...
    EXPECT_THROW(Hyperrectangle(Vector3d{1, 2, 3}, Vector3d{1, 2, 1}), std::exception);
    // It is ok to make a point hyperrectangle.
    const Hyperrectangle point_hyperrectangle(Vector3d{1, 2, 3}, Vector3d{1, 2, 3});
    ...
    EXPECT_TRUE(CompareMatrices(point_hyperrectangle.MaybeGetFeasiblePoint().value(), Vector3d{1, 2, 3}));
}

GTEST_TEST(HyperrectangleTest, Sampling) {
    const Hyperrectangle hyperrectangle(Eigen::Vector2d{-1, -2}, Eigen::Vector2d{3, 2});
    ...
    EXPECT_TRUE(CompareMatrices(sum_samples / n_samples, hyperrectangle.Center(), 1e-1));
    // The variance of the samples.
    const auto variance = sum_samples_squared / n_samples - (sum_samples / n_samples).array().square().matrix();
    EXPECT_TRUE(CompareMatrices(variance, Eigen::Vector2d{16.0 / 12, 16.0 / 12}, 1e-1));
}

GTEST_TEST(HyperrectangleTest, InvariantTest) {
    // Cannot make a hyperrectangle with nan.
    EXPECT_THROW(Hyperrectangle(Vector3d{1, 2, 3}, Vector3d{1, 2, std::numeric_limits<double>::quiet_NaN()}),
                 std::exception);
    EXPECT_THROW(Hyperrectangle(Vector3d{std::numeric_limits<double>::quiet_NaN(), 2, 3}, Vector3d{1, 2, 3}),
                 std::exception);
    // Cannot make an empty hyperrectangle
    EXPECT_THROW(Hyperrectangle(Vector3d{1, 2, 3}, Vector3d{1, 2, -1}), std::exception);

    // Cannot make an unbounded hyperrectangle
    EXPECT_THROW(Hyperrectangle(Vector3d::Constant(-std::numeric_limits<double>::infinity()), Vector3d::Constant(1)),
                 std::exception);
    EXPECT_THROW(Hyperrectangle(-Vector3d::Constant(1), Vector3d::Constant(std::numeric_limits<double>::infinity())),
                 std::exception);
}

GTEST_TEST(HyperrectangleTest, IsBoundedTest) {
    EXPECT_TRUE(Hyperrectangle(Vector3d{-1, -2, -3}, Vector3d{3, 2, 1}).IsBounded());
}

GTEST_TEST(HyperrectangleTest, IsEmptyTest) {
    EXPECT_FALSE(Hyperrectangle(Vector3d{-1, -2, -3}, Vector3d{3, 2, 1}).IsEmpty());
}
...
GTEST_TEST(HyperrectangleTest, MaybeGetIntersection) {
    ...
    const std::optional<Hyperrectangle> intersection12 = h1.MaybeGetIntersection(h2);
    const std::optional<Hyperrectangle> intersection21 = h2.MaybeGetIntersection(h1);
    ...
    const std::optional<Hyperrectangle> intersection13 = h1.MaybeGetIntersection(h3);
    const std::optional<Hyperrectangle> intersection31 = h3.MaybeGetIntersection(h1);
...
GTEST_TEST(HyperrectangleTest, AddPointInNonnegativeScalingConstraints) {
    ...
    const auto x = prog.NewContinuousVariables(3, "x");
    auto scaled_con = hyperrectangle.AddPointInNonnegativeScalingConstraints(&prog, x, t);
...
GTEST_TEST(HyperrectangleTest, AddPointInNonnegativeScalingConstraintsWithMatrices) {
    const Vector3d lb{-1, -2, -3};
    const Vector3d ub{3, 2, 1};
    const Hyperrectangle hyperrectangle(lb, ub);
    MathematicalProgram prog;
    ...
    auto scaled_matrix_con = hyperrectangle.AddPointInNonnegativeScalingConstraints(&prog, A, b, c, d, x, t);
    ...
    // d) \in hyperrectangle
    {
        ...
        const auto test_point = 1 / ((c.transpose() * t_value)[0] + d) * (A * x_value + b);
        EXPECT_TRUE(CompareMatrices(test_point, Eigen::Vector3d(2.5, 0.5, -1)));
        EXPECT_EQ(PointInScaledSet(x, t, x_value, t_value, &prog, scaled_matrix_con),
...
GTEST_TEST(HyperrectangleTest, AxisAlignedBoundingBox) {
    ...
    b << 1, 2, 1;
        // clang-format on
        HPolyhedron H(A, b);
        EXPECT_FALSE(Hyperrectangle::MaybeCalcAxisAlignedBoundingBox(H).has_value());
    }
    ...
    b << 1, 1, 2, 1;
        // clang-format on
        HPolyhedron H(A, b);
        std::optional<Hyperrectangle> aabb_opt = Hyperrectangle::MaybeCalcAxisAlignedBoundingBox(H);
...
GTEST_TEST(HyperrectangleTest, Serialize) {
    ...
    EXPECT_EQ(hyperrectangle.ambient_dimension(), hyperrectangle2.ambient_dimension());
