...
GTEST_TEST(GeodesicConvexityTest, PartitionConvexSetAPI) {
  ...
  vertices << 0.0, 4.0, 0.0, 4.0,
    ...
    ConvexSets sets = PartitionConvexSet(MakeConvexSets(v, v), continuous_revolute_joints, epsilon);
    EXPECT_EQ(sets.size(), 8);
    for (const auto& set : sets) {
        EXPECT_TRUE(CheckIfSatisfiesConvexityRadius(*set, continuous_revolute_joints));
    }

    // List of convex sets must have at least one entry.
    EXPECT_THROW(PartitionConvexSet(MakeConvexSets(), continuous_revolute_joints), std::exception);
    ...
    EXPECT_THROW(PartitionConvexSet(contains_nullptr, continuous_revolute_joints), std::exception);
    ...
    EXPECT_THROW(PartitionConvexSet(unbounded_hpolyhedron, continuous_revolute_joints), std::exception);
    EXPECT_THROW(PartitionConvexSet(MakeConvexSets(unbounded_hpolyhedron), continuous_revolute_joints), std::exception);

    // But unbounded dimensions are allowed on dimensions not corresponding to
    // continuous revolute joints.
    EXPECT_NO_THROW(PartitionConvexSet(unbounded_hpolyhedron, std::vector<int>{}));
    EXPECT_NO_THROW(PartitionConvexSet(MakeConvexSets(unbounded_hpolyhedron), std::vector<int>{}));

    // List of convex sets must have matching ambient dimension.
    VPolytope v_wrong_dimension(Eigen::MatrixXd::Zero(3, 1));
    DRAKE_EXPECT_THROWS_MESSAGE(PartitionConvexSet(MakeConvexSets(v, v_wrong_dimension), continuous_revolute_joints),
                                ".*convex_sets\\[i\\]->ambient_dimension.*");
    EXPECT_THROW(PartitionConvexSet(MakeConvexSets(v, v_wrong_dimension), continuous_revolute_joints), std::exception);
    ...
    EXPECT_THROW(PartitionConvexSet(v, continuous_revolute_joints, M_PI), std::exception);
}

bool PointInASet(const ConvexSets& sets, const Eigen::VectorXd& point, const double kTol) {
...
GTEST_TEST(GeodesicConvexityTest, PartitionConvexSet1) {
  ...
  vertices << 0.0, 4.0, 0.0, 4.0,
    ...
    for (int i = 0; i < ssize(sets); ++i) {
        EXPECT_TRUE(sets[i]->PointInSet(Eigen::Vector2d(M_PI - 2.0 * epsilon, M_PI - 2.0 * epsilon), kTol));
    }
}
...
GTEST_TEST(GeodesicConvexityTest, PartitionConvexSet2) {
  ...
  vertices2 << 0.0, 2.0, 0.0, 2.0,
    ...
    const auto maybe_intersection_bbox =
            Hyperrectangle::MaybeCalcAxisAlignedBoundingBox(Intersection(*sets[0], *sets[1]));
    ...
    for (int i = 0; i < intersection_bbox.ambient_dimension(); ++i) {
        ...
        EXPECT_TRUE(intersection_bbox.ub()[i] - intersection_bbox.lb()[i] >= epsilon - delta);
    }
}
...
GTEST_TEST(GeodesicConvexityTest, PartitionConvexSet4) {
  ...
  vertices << 0.0, 4.0, 0.0, 4.0,
    ...
    EXPECT_EQ(sets.size(), 2);
    for (const auto& set : sets) {
        EXPECT_TRUE(CheckIfSatisfiesConvexityRadius(*set, only_one_continuous_revolute_joint));
    }

    bool all_satisfy = true;
    for (const auto& set : sets) {
        all_satisfy = all_satisfy && CheckIfSatisfiesConvexityRadius(*set, continuous_revolute_joints);
    }
...
GTEST_TEST(GeodesicConvexityTest, ComputeOffset) {
    ...
    const auto v1_bbox = GetMinimumAndMaximumValueAlongDimension(v1, continuous_revolute_joints);
    const auto v2_bbox = GetMinimumAndMaximumValueAlongDimension(v2, continuous_revolute_joints);
    const auto v3_bbox = GetMinimumAndMaximumValueAlongDimension(v3, continuous_revolute_joints);

    const int num_positions = 1;
    const double kTol = 1e-9;

    Eigen::VectorXd offset;
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v1_bbox, v2_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(0.0), kTol));
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v2_bbox, v1_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(0.0), kTol));
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v1_bbox, v3_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(2 * M_PI), kTol));
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v3_bbox, v1_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(-2 * M_PI), kTol));
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v2_bbox, v3_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(0.0), kTol));
    offset = ComputeOffsetContinuousRevoluteJoints(num_positions, continuous_revolute_joints, v3_bbox, v2_bbox);
    EXPECT_TRUE(CompareMatrices(offset, Vector1d(0.0), kTol));
}
...
GTEST_TEST(GeodesicConvexityTest, CalcPairwiseIntersectionsAPI) {
    ...
    EXPECT_THROW(CalcPairwiseIntersections({}, std::vector<int>{}), std::exception);
    EXPECT_THROW(CalcPairwiseIntersections(sets, {}, std::vector<int>{}), std::exception);
    EXPECT_THROW(CalcPairwiseIntersections({}, sets, std::vector<int>{}), std::exception);
}
...
GTEST_TEST(GeodesicConvexityTest, CalcPairwiseIntersections1) {
    ...
    Hyperrectangle h2(Eigen::Vector2d(0.5, 0.5 + 2 * M_PI), Eigen::Vector2d(1.5, 1.5 + 2 * M_PI));
    Hyperrectangle h3(Eigen::Vector2d(8 * M_PI, 0.0), Eigen::Vector2d(1.0 + 8 * M_PI, 1.0));
    Hyperrectangle h4(Eigen::Vector2d(2 * M_PI, -4 * M_PI), Eigen::Vector2d(1.0 + 2 * M_PI, 1.0 - 4 * M_PI));
    ...
    EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{}).size(), 0);
    // With only first joint continuous, the only intersection is h1 <--> h3.
    EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{0}).size(), 1);
    // With only second joint continuous, the intersection is empty.
    EXPECT_EQ(CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{1}).size(), 0);
    // With all joints continuous, all possible pairs exist.
    const auto intersection_edges = CalcPairwiseIntersections(sets_A, sets_B, std::vector<int>{0, 1});
    ...
    for (const auto& [index_a, index_b, offset] : intersection_edges) {
        ...
        // rectangle.
        const auto* h_a = dynamic_cast<const Hyperrectangle*>(sets_A[index_a].get());
...
GTEST_TEST(GeodesicConvexityTest, CalcPairwiseIntersections2) {
    ...
    // With only first joint continuous, the only intersection is h1 <--> h3.
    auto intersections_zero = CalcPairwiseIntersections(sets, std::vector<int>{0});
    EXPECT_EQ(intersections_zero.size(), 2);
    EXPECT_EQ(std::get<0>(intersections_zero[0]), 0);
    EXPECT_EQ(std::get<1>(intersections_zero[0]), 2);
    EXPECT_TRUE(CompareMatrices(std::get<2>(intersections_zero[0]), Eigen::Vector2d(8 * M_PI, 0.0), 1e-9));
    // With all joints continuous, all pairs of intersections are non-empty.
    auto intersections_all = CalcPairwiseIntersections(sets, std::vector<int>{0, 1});
    ...
    for (const auto& [index_1, index_2, offset] : intersections_all) {
        ...
        // The center of the first rectangle + offset should be inside the second
        // rectangle.
        const auto* h_first = dynamic_cast<const Hyperrectangle*>(sets[index_1].get());
