...
public:
    ...
    CspaceFreePolytopeTester(const multibody::MultibodyPlant<double>* plant,
                             ...
                             const CspaceFreePolytope::Options& options = CspaceFreePolytope::Options())
        : cspace_free_polytope_{new CspaceFreePolytope(plant, scene_graph, plane_order, q_star, options)} {}

    const CspaceFreePolytope& cspace_free_polytope() const { return *cspace_free_polytope_; }

    void FindRedundantInequalities(const Eigen::MatrixXd& C,
    ...
    const Eigen::VectorXd& s_lower() const { return cspace_free_polytope_->s_lower_; }

    const Eigen::VectorXd& s_upper() const { return cspace_free_polytope_->s_upper_; }

    const VectorX<symbolic::Polynomial>& s_minus_s_lower() const { return cspace_free_polytope_->s_minus_s_lower_; }

    const VectorX<symbolic::Polynomial>& s_upper_minus_s() const { return cspace_free_polytope_->s_upper_minus_s_; }

    const std::vector<PlaneSeparatesGeometries>& plane_geometries() const {
        return cspace_free_polytope_->plane_geometries_;
    }

    [[nodiscard]] const symbolic::Variables& get_s_set() const { return cspace_free_polytope_->get_s_set(); }

    template <typename T>
    [[nodiscard]] VectorX<symbolic::Polynomial> CalcDminusCs(const Eigen::Ref<const MatrixX<T>>& C,
    ...
    [[nodiscard]] CspaceFreePolytope::SeparationCertificateProgram ConstructPlaneSearchProgram(
    ...
    [[nodiscard]] std::vector<std::optional<CspaceFreePolytope::SeparationCertificateResult>>
    ...
    [[nodiscard]] std::unique_ptr<solvers::MathematicalProgram> InitializePolytopeSearchProgram(
            const CspaceFreePolytope::IgnoredCollisionPairs& ignored_collision_pairs,
    ...
    void AddEllipsoidContainmentConstraint(solvers::MathematicalProgram* prog,
    ...
    [[nodiscard]] std::optional<CspaceFreePolytope::FindPolytopeGivenLagrangianResult> FindPolytopeGivenLagrangian(
            const CspaceFreePolytope::IgnoredCollisionPairs& ignored_collision_pairs,
            const MatrixX<symbolic::Variable>& C,
            const VectorX<symbolic::Variable>& d,
            const VectorX<symbolic::Polynomial>& d_minus_Cs,
    ...
    HPolyhedron GetPolyhedronWithJointLimits(const Eigen::MatrixXd& C, const Eigen::VectorXd& d) const;
...
public:
    CspaceFreeBoxTester(const multibody::MultibodyPlant<double>* plant,
                        const geometry::SceneGraph<double>* scene_graph,
                        SeparatingPlaneOrder plane_order,
    ...
    void ComputeSBox(const Eigen::Ref<const Eigen::VectorXd>& q_box_lower,
        ...
        return cspace_free_box_.ComputeSBox(q_box_lower, q_box_upper, s_box_lower, s_box_upper, q_star);
    }
    ...
    void GeneratePolynomialsToCertify(const Eigen::Ref<const Eigen::VectorXd>& s_box_lower,
        ...
        return cspace_free_box_.GeneratePolynomialsToCertify(s_box_lower, s_box_upper, q_star, ignored_collision_pairs,
                                                             &(certify_polynomials->data));
    }

    [[nodiscard]] CspaceFreeBox::SeparationCertificateProgram ConstructPlaneSearchProgram(
            const PlaneSeparatesGeometries& plane_geometries,
            const VectorX<symbolic::Polynomial>& s_minus_s_lower,
            const VectorX<symbolic::Polynomial>& s_upper_minus_s) const {
        return cspace_free_box_.ConstructPlaneSearchProgram(plane_geometries, s_minus_s_lower, s_upper_minus_s);
    }

    [[nodiscard]] std::vector<std::optional<CspaceFreeBox::SeparationCertificateResult>>
    FindSeparationCertificateGivenBox(const CspaceFreeBox::IgnoredCollisionPairs& ignored_collision_pairs,
                                      const Eigen::Ref<const Eigen::VectorXd>& q_box_lower,
                                      const Eigen::Ref<const Eigen::VectorXd>& q_box_upper,
                                      const FindSeparationCertificateOptions& options) const {
        return cspace_free_box_.FindSeparationCertificateGivenBox(ignored_collision_pairs, q_box_lower, q_box_upper,
                                                                  options);
    }
