...
GTEST_TEST(MakeConvexSetsTest, Basic) {
    ...
    ConvexSets sets = MakeConvexSets(box, box.Clone(), Point(Vector3d(1.0, 2.0, 3.0)));
...
public:
    explicit DummyVolumeSet(int dim, bool can_calc_volume) : ConvexSet(dim, can_calc_volume) {}

protected:
    std::unique_ptr<ConvexSet> DoClone() const override { return nullptr; }
    bool DoPointInSet(const Eigen::Ref<const Eigen::VectorXd>&, double) const override { return false; }
    std::pair<VectorX<symbolic::Variable>, std::vector<solvers::Binding<solvers::Constraint>>>
    DoAddPointInSetConstraints(solvers::MathematicalProgram*,
                               const Eigen::Ref<const solvers::VectorXDecisionVariable>&) const override {
        return {{}, {}};
    }
    ...
    std::pair<std::unique_ptr<Shape>, math::RigidTransformd> DoToShapeWithPose() const override {
...
public:
    explicit NoImplSet(int dim, bool can_calc_volume) : DummyVolumeSet(dim, can_calc_volume) {}
};
...
public:
    explicit HasImplSet(int dim, bool can_calc_volume)
        : DummyVolumeSet(dim, can_calc_volume), can_calc_volume_(can_calc_volume) {}
...
GTEST_TEST(ConvexSetTest, CalcVolume) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(HasImplSet(1, false).CalcVolume(),
    ...
    // has_exact_volume.
    DRAKE_EXPECT_THROWS_MESSAGE(NoImplSet(0, false).CalcVolume(), ".*an exact volume.*");
    DRAKE_EXPECT_THROWS_MESSAGE(NoImplSet(0, true).CalcVolume(), ".*NoImplSet is a zero-dimensional set.*");
    DRAKE_EXPECT_THROWS_MESSAGE(HasImplSet(0, false).CalcVolume(), ".*an exact volume.*");
    DRAKE_EXPECT_THROWS_MESSAGE(HasImplSet(0, true).CalcVolume(), ".*HasImplSet is a zero-dimensional set.*");
}
...
GTEST_TEST(ConvexSetTest, CalcVolumeViaSampling) {
    ...
    const SampledVolume bad_result =
            unit_circle.CalcVolumeViaSampling(&generator, desired_rel_accuracy, max_num_samples_low);
    const SampledVolume good_result =
            unit_circle.CalcVolumeViaSampling(&generator, desired_rel_accuracy, max_num_samples_high);
    ...
    EXPECT_FALSE(std::abs(bad_result.volume - M_PI) / M_PI < desired_rel_accuracy);
...
GTEST_TEST(ConvexSetTest, GenericProjection) {
  ...
  const Eigen::Matrix2Xd expected_projection{{0.5, 1, -1, 1},
    ...
    for (int i = 0; i < test_points.cols(); ++i) {
        EXPECT_TRUE(CompareMatrices(projections.col(i), expected_projection.col(i), kTol));
...
GTEST_TEST(ConvexSetTest, ShortcutProjection) {
  ...
  const Eigen::Matrix2Xd expected_projection{{1, 1.5, -1.55, 1},
    ...
    const std::vector<double> expected_distances{sqrt(2) / 2, sqrt(2) / 2, 9 / (10 * sqrt(2)), 0.0};
