...
GTEST_TEST(CalcPlane, TestAllSymbolic) {
    ...
    for (int i = 0; i < 3; ++i) {
        EXPECT_PRED2(
                ...
                symbolic::Polynomial(decision_vars(2 * i) * s + decision_vars(2 * i + 1), symbolic::Variables({s})));
    }
    EXPECT_PRED2(symbolic::test::PolyEqual, b,
                 symbolic::Polynomial(decision_vars(6) * s + decision_vars(7), symbolic::Variables({s})));
}
...
GTEST_TEST(CalcPlane, TestDoubleDecisionVariableSymbolicS) {
    ...
    Vector3<symbolic::Polynomial> a;
    symbolic::Polynomial b;
    CalcPlane<double, symbolic::Variable, symbolic::Polynomial>(decision_var_vals, Vector1<symbolic::Variable>(s),
                                                                plane_degree, &a, &b);
    for (int i = 0; i < 3; ++i) {
        EXPECT_PRED2(symbolic::test::PolyEqual, a(i),
                     symbolic::Polynomial(decision_var_vals(2 * i) * s + decision_var_vals(2 * i + 1)));
    }
    EXPECT_PRED2(symbolic::test::PolyEqual, b, symbolic::Polynomial(decision_var_vals(6) * s + decision_var_vals(7)));
}
...
GTEST_TEST(CalcPlane, TestDoubleDecisionVariableDoubleS) {
    ...
    Eigen::Vector3d a;
    double b;
    CalcPlane<double, double, double>(decision_var_vals, Vector1d(s), plane_degree, &a, &b);
    for (int i = 0; i < 3; ++i) {
        EXPECT_EQ(a(i), decision_var_vals(2 * i) * s + decision_var_vals(2 * i + 1));
    }
