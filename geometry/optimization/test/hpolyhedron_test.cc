...
#include "geometry/optimization/test_utilities.h"
...
GTEST_TEST(HPolyhedronTest, UnitBoxTest) {
    ...
    EXPECT_TRUE(H_scene_graph.PointInSet(H_scene_graph.MaybeGetFeasiblePoint().value()));
}
...
bool CheckHPolyhedronContainsVPolyhedron(const HPolyhedron& h, const VPolytope& v, double tol = 0) {
...
bool CheckBoundedHPolyhedronAreSame(const HPolyhedron& h1, const HPolyhedron& h2, double tol = 0) {
    VPolytope v1(h1);
    VPolytope v2(h2);
    return CheckHPolyhedronContainsVPolyhedron(h1, v2, tol) && CheckHPolyhedronContainsVPolyhedron(h2, v1, tol);
}
...
GTEST_TEST(HPolyhedronTest, ConstructorFromLinearProgramThrows) {
    ...
    VectorXDecisionVariable x_no_constraints = prog_no_constraints.NewContinuousVariables(5);
    ...
    prog_socp.AddRotatedLorentzConeConstraint(MatrixXd::Identity(3, 3), VectorXd::Zero(3), x_socp);
    EXPECT_THROW(HPolyhedron{prog_socp}, std::exception);

    MathematicalProgram prog_sdp;
    solvers::MatrixXDecisionVariable x_sdp = prog_sdp.NewSymmetricContinuousVariables(3);
...
GTEST_TEST(HPolyhedronTest, InfeasibleProgram) {
    ...
    prog1.AddLinearConstraint(MatrixXd::Identity(1, 1), Vector1d(0), Vector1d(-kInf), x1);
    ...
    prog2.AddLinearConstraint(MatrixXd::Identity(1, 1), Vector1d(kInf), Vector1d(0), x2);
...
GTEST_TEST(HPolyhedronTest, ConstructorFromLinearProgram) {
  ...
  A1_expected <<  1, -1,  0,
    ...
    MathematicalProgram prog2;
    VectorXDecisionVariable x2 = prog2.NewContinuousVariables(h2_expected.ambient_dimension());
...
GTEST_TEST(HPolyhedronTest, ArbitraryBoxTest) {
    RigidTransformd X_WG(RotationMatrixd::MakeZRotation(M_PI / 2.0), Vector3d(-4.0, -5.0, -6.0));
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Box(1.0, 2.0, 3.0), X_WG);
    HPolyhedron H(query, geom_id);

    EXPECT_EQ(H.ambient_dimension(), 3);
    // Rotated box should end up with lb=[-5,-5.5,-7.5], ub=[-3,-4.5,-4.5].
    Vector3d in1_W{-4.9, -5.4, -7.4}, in2_W{-3.1, -4.6, -4.6}, out1_W{-5.1, -5.6, -7.6}, out2_W{-2.9, -4.4, -4.4};
    ...
    const RigidTransformd X_WF{math::RollPitchYawd(.1, .2, 3), Vector3d{.5, .87, .1}};
    const FramePoseVector<double> pose_vector{{frame_id, X_WF}};
    scene_graph->get_source_pose_port(source_id).FixValue(context2.get(), pose_vector);
    auto query2 = scene_graph->get_query_output_port().Eval<QueryObject<double>>(*context2);
...
GTEST_TEST(HPolyhedronTest, HalfSpaceTest) {
    RigidTransformd X_WG(RotationMatrixd::MakeYRotation(M_PI / 2.0), Vector3d(-1.2, -2.1, -6.4));
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(HalfSpace(), X_WG);
    HPolyhedron H(query, geom_id);

    EXPECT_EQ(H.ambient_dimension(), 3);

    // Rotated HalfSpace should be x <= -1.2.
    Vector3d in1_W{-1.21, 0.0, 0.0}, in2_W{-1.21, 2., 3.}, out1_W{-1.19, 0, 0}, out2_W{-1.19, 2., 3.};
...
GTEST_TEST(HPolyhedronTest, UnitBox6DTest) {
    ...
    Vector6d in1_W{Vector6d::Constant(-.99)}, in2_W{Vector6d::Constant(.99)}, out1_W{Vector6d::Constant(-1.01)},
...
GTEST_TEST(HPolyhedronTest, InscribedEllipsoidTest) {
    ...
    EXPECT_TRUE(CompareMatrices(E.A().transpose() * E.A(), Matrix3d::Identity(3, 3), kTol));
  ...
  b << 2.1, 2.1, 2.1, 2.1, 2.1, 2.1, 1.3, 0.8;
    ...
    const VectorXd polytope_halfspace_residue = b - A * E2.center() - ((A * C).rowwise().lpNorm<2>());
...
GTEST_TEST(HPolyhedronTest, Scale3) {
  ...
  A <<  1,  0,  0,  // x <= 1
    ...
    const double kVertexValue = std::pow(16.0, 1.0 / 3.0) / 2.0;
    EXPECT_TRUE(H_scaled.PointInSet(Vector3d{kVertexValue - kOffset, 0, kVertexValue - kOffset}));
    EXPECT_FALSE(H_scaled.PointInSet(Vector3d{kVertexValue + kOffset, 0, kVertexValue + kOffset}));
...
GTEST_TEST(HPolyhedronTest, NonnegativeScalingTest) {
    ...
    std::vector<Binding<Constraint>> constraints = H.AddPointInNonnegativeScalingConstraints(&prog, x, t);
...
GTEST_TEST(HPolyhedronTest, NonnegativeScalingTest2) {
  ...
  A << 1, 0,
    ...
    std::vector<Binding<Constraint>> constraints = H.AddPointInNonnegativeScalingConstraints(&prog, A, b, c, d, x, t);

    EXPECT_EQ(constraints.size(), 2);

    EXPECT_TRUE(PointInScaledSet(x, t, 0.99 * ub.head(2), Vector2d(1.0, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.99 * ub.head(2), Vector2d(0, -1.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 1.01 * ub.head(2), Vector2d(1.0, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 1.01 * ub.head(2), Vector2d(0, -1.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.99 * ub.head(2), Vector2d(-0.01, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.99 * ub.head(2), Vector2d(0, -0.01), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.49 * ub.head(2), Vector2d(0.5, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 0.49 * ub.head(2), Vector2d(0, -0.5), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.51 * ub.head(2), Vector2d(0.5, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 0.51 * ub.head(2), Vector2d(0, -0.5), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 1.99 * ub.head(2), Vector2d(2.0, 0), &prog, constraints));
    EXPECT_TRUE(PointInScaledSet(x, t, 1.99 * ub.head(2), Vector2d(0, -2.0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 2.01 * ub.head(2), Vector2d(2.0, 0), &prog, constraints));
    EXPECT_FALSE(PointInScaledSet(x, t, 2.01 * ub.head(2), Vector2d(0, -2.0), &prog, constraints));
}
...
GTEST_TEST(HPolyhedronTest, CartesianPowerTest) {
    ...
    EXPECT_TRUE(H.CartesianPower(3).PointInSet((VectorXd(6) << x, x, x).finished()));
...
GTEST_TEST(HPolyhedronTest, OffsetIrredundantBoxes) {
    ...
    HPolyhedron intersection_right_into_left = left_box.Intersection(right_box, true);
    HPolyhedron intersection_left_into_right = right_box.Intersection(left_box, true);
...
GTEST_TEST(HPolyhedronTest, PontryaginDifferenceTestAxisAligned) {
    ...
    const HPolyhedron H_C_expected = HPolyhedron::MakeBox(Vector2d{-1, -1}, Vector2d{0, 0});
...
GTEST_TEST(HPolyhedronTest, PontryaginDifferenceTestNonAxisAligned) {
    ...
    const HPolyhedron H_C_expected = HPolyhedron::MakeBox(Vector3d::Constant(-0.5), Vector3d::Constant(0.5));
...
GTEST_TEST(HPolyhedronTest, UniformSampleTest1) {
  ...
  b << -4, 6, -2, 8;
    ...
    for (int i = 1; i < N; ++i) {
        samples.col(i) = H.UniformSample(&generator, samples.col(i - 1), mixing_steps);
    }
    ...
    EXPECT_NEAR(((2 * samples.row(0) + samples.row(1)).array() >= 5.0).count(), 0.5 * N, kTol);

    // Check that approximately half of them satisfy x - 2y ≥ 5.
    EXPECT_NEAR(((samples.row(0) - 2 * samples.row(1)).array() >= 5.0).count(), 0.5 * N, kTol);
    ...
    EXPECT_NEAR((samples.row(0).array() >= 3 && samples.row(0).array() <= 3.5 && samples.row(1).array() >= -1.5 &&
                 samples.row(1).array() <= -1)
                        .count(),
                N / 10, kTol);
}
  ...
  b << 1, 1, 1, 1, 0;
    ...
    for (int i = 0; i < 10; ++i) {
        try {
            ...
            ++num_throws;
            EXPECT_NE(std::string(err.what()).find("Hit and Run algorithm failed to find a feasible point"),
                      std::string::npos);
        }
    }
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation1) {
  ...
  A << 1, 0,  // x <= 2
    ...
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(min_volume_ratio, false);
  ...
  A_expected << 1, 0,  // x <= 2 * min_volume_ratio ^ (1 / dimension)
    ...
    b_expected << 2 * std::pow(min_volume_ratio, 0.5), 2 * std::pow(min_volume_ratio, 0.5), 2, 2;
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation2) {
    ...
    for (int row = 0; row < kNumFaces; ++row) {
        A.row(row) << std::cos(2 * M_PI * row / kNumFaces), std::sin(2 * M_PI * row / kNumFaces);
    }
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation3) {
  ...
  verts << 0, 1, -1,
    ...
    const std::vector<HPolyhedron> intersecting_polytopes = {intersecting_polytope};
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(
            min_volume_ratio, false, 10, Eigen::MatrixXd(), intersecting_polytopes, true, 0);
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume() / circumbody_V.CalcVolume(), min_volume_ratio);
    EXPECT_LE(inbody.b().rows(), circumbody.b().rows());
    // Check if intersection is still contained.
    EXPECT_TRUE((circumbody.Intersection(intersecting_polytope).ContainedIn(inbody, kConstraintTol)));
}
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation4) {
  ...
  verts << 0, 1, -1,
    ...
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(
            min_volume_ratio, false, 10, Eigen::MatrixXd(), intersecting_polytopes, true, 0.1);
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume() / circumbody_V.CalcVolume(), min_volume_ratio);
    EXPECT_LE(inbody.b().rows(), circumbody.b().rows());
    // Check if intersection is still contained.
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation5) {
  ...
  verts << 0, 1, -1,
    ...
    const std::vector<HPolyhedron> intersecting_polytopes = {intersecting_polytope};
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(
            min_volume_ratio, true, 10, Eigen::MatrixXd(), intersecting_polytopes, false);
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kAffineTransformationConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume() / circumbody_V.CalcVolume(), min_volume_ratio);
    EXPECT_LE(inbody.b().rows(), circumbody.b().rows());
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation6) {
  ...
  points << 0, 0.7, -0.7,
           -1, 0.7, 0.7;
    // clang-format on
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(min_volume_ratio, false, 10, points);
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume() / circumbody_V.CalcVolume(), min_volume_ratio);
    EXPECT_LE(inbody.b().rows(), circumbody.b().rows());
...
GTEST_TEST(HPolyhedronTest, SimplifyByIncrementalFaceTranslation7) {
  ...
  points << 0, 0.7, -0.7,
    ...
    const HPolyhedron inbody = circumbody.SimplifyByIncrementalFaceTranslation(min_volume_ratio, true, 10, points,
                                                                               intersecting_polytopes, false);
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kAffineTransformationConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume() / circumbody_V.CalcVolume(), min_volume_ratio);
    EXPECT_LE(inbody.b().rows(), circumbody.b().rows());
    EXPECT_TRUE(inbody.IntersectsWith(intersecting_polytope));
...
GTEST_TEST(HPolyhedronTest, MaximumVolumeInscribedAffineTransformationTest1) {
    ...
    for (int row = 0; row < kNumFaces; ++row) {
        circumbody_A.row(row) << std::cos(2 * M_PI * row / kNumFaces), std::sin(2 * M_PI * row / kNumFaces);
    }
  ...
  initial_polytope_verts << 1.4, 0.8, 0, 0.2,
    ...
    HPolyhedron inbody = initial_polytope.MaximumVolumeInscribedAffineTransformation(circumbody);
...
GTEST_TEST(HPolyhedronTest, MaximumVolumeInscribedAffineTransformationTest2) {
  ...
  initial_polytope_verts << 0.4, 0.8, 0, -0.5,
    ...
    // Check containment, and that volume increased.
    EXPECT_TRUE(inbody.ContainedIn(circumbody, kConstraintTol));
    EXPECT_GE(VPolytope(inbody).CalcVolume(), VPolytope(initial_polytope).CalcVolume());
}
}  // namespace optimization
}  // namespace geometry
}  // namespace drake
