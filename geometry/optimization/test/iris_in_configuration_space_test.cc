...
HPolyhedron IrisFromUrdf(const std::string urdf,
    ...
    multibody::MultibodyPlant<double>& plant = multibody::AddMultibodyPlantSceneGraph(&builder, 0.0);
    multibody::Parser parser(&plant);
    parser.package_map().AddPackageXml(FindResourceOrThrow("drake/multibody/parsing/test/box_package/package.xml"));
    ...
    return IrisInConfigurationSpace(plant, plant.GetMyContextFromRoot(*context), options);
}
...
GTEST_TEST(IrisInConfigurationSpaceTest, BoxesPrismatic) {
    ...
    const double qmin = -1.0 + options.configuration_space_margin, qmax = 1.0 - options.configuration_space_margin;
    EXPECT_TRUE(region.PointInSet(Vector1d{qmin + kTol}));
    EXPECT_TRUE(region.PointInSet(Vector1d{qmax - kTol}));
    EXPECT_FALSE(region.PointInSet(Vector1d{qmin - kTol}));
    EXPECT_FALSE(region.PointInSet(Vector1d{qmax + kTol}));

    DRAKE_EXPECT_THROWS_MESSAGE(IrisFromUrdf(boxes_urdf, Vector1d{1.1}, options), "The seed point is in collision.*");
}
...
GTEST_TEST(IrisInConfigurationSpaceTest, ConfigurationSpaceMargin) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(IrisFromUrdf(boxes_urdf, sample, options),
...
GTEST_TEST(IrisInConfigurationSpaceTest, ConfigurationObstacles) {
    ...
    // Configuration space obstacle more restrictive than task space obstacles.
    {
        ...
        options.solver_options->SetOption(solvers::IpoptSolver::id(), "max_iter", 0);
        options.solver_options->SetOption(solvers::SnoptSolver::id(), "Major Iterations Limit", 0);
        const HPolyhedron region_no_counterexample = IrisFromUrdf(boxes_urdf, sample, options);
    ...
    // Sample is in configuration space obstacles.
    {
        ...
        obstacles.emplace_back(HPolyhedron::MakeBox(Vector1d(0.5), Vector1d(3)));
        options.configuration_obstacles = obstacles;
        DRAKE_EXPECT_THROWS_MESSAGE(IrisFromUrdf(boxes_urdf, Vector1d(0.7), options),
...
GTEST_TEST(IrisInConfigurationSpaceTest, ConfigurationObstaclesMultipleBoxes) {
    ...
    obstacles.emplace_back(VPolytope::MakeBox(Vector2d(-1, -1), Vector2d(-.1, -.5)));
    obstacles.emplace_back(HPolyhedron::MakeBox(Vector2d(.1, -1), Vector2d(1, -.5)));
    obstacles.emplace_back(HPolyhedron::MakeBox(Vector2d(-1, .5), Vector2d(-.1, 1)));
    options.configuration_obstacles = obstacles;
...
GTEST_TEST(IrisInConfigurationSpaceTest, BadEllipseAndSample) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(IrisFromUrdf(boxes_in_2d_urdf, sample, options),
...
GTEST_TEST(IrisInConfigurationSpaceTest, TerminationFunc) {
    ...
    std::function<bool(const HPolyhedron&)> always_false = [&](const HPolyhedron&) {
        return false;
    };
    IrisOptions options;
    ...
    HPolyhedron without_termination = IrisFromUrdf(boxes_in_2d_urdf_no_collisions, sample, options);
    options.termination_func = always_false;
    HPolyhedron with_always_false = IrisFromUrdf(boxes_in_2d_urdf_no_collisions, sample, options);
    ...
    HPolyhedron region_with_termination = IrisFromUrdf(boxes_in_2d_urdf_no_collisions, sample, options);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(IrisFromUrdf(boxes_in_2d_urdf_no_collisions, sample, options),
...
GTEST_TEST(IrisInConfigurationSpaceTest, StartingEllipse) {
    ...
    HPolyhedron region_w_ellipse = IrisFromUrdf(boxes_in_2d_urdf, sample, options);
    ...
    EXPECT_NEAR(region_w_ellipse.A()(4, 0), 100 * region_w_ellipse.A()(4, 1), 1e-6);
}
...
GTEST_TEST(IrisInConfigurationSpaceTest, BoundingRegion) {
    ...
    options.num_collision_infeasible_samples = 0;
    ConvexSets obstacles;
    obstacles.emplace_back(VPolytope::MakeBox(Vector2d(0.2, 0.2), Vector2d(1, 1)));
    options.configuration_obstacles = obstacles;
    HPolyhedron region = IrisFromUrdf(boxes_in_2d_urdf, sample, options);

    // Add a bounding region that halves the plant's joint limits.
    options.bounding_region = HPolyhedron::MakeBox(Vector2d(-1, -0.5), Vector2d(1, 0.5));
    HPolyhedron region_w_bounding = IrisFromUrdf(boxes_in_2d_urdf, sample, options);
...
<robot name="pendulum">
  ...
  <joint name="pendulum" type="revolute">
    ...
    const double kTol = 1e-3;  // due to ibex's rel_eps_f.
    const double qmin = std::asin((-w + r) / l) + options.configuration_space_margin,
                 qmax = std::asin((w - r) / l) - options.configuration_space_margin;
    EXPECT_TRUE(region.PointInSet(Vector1d{qmin + kTol}));
    EXPECT_TRUE(region.PointInSet(Vector1d{qmax - kTol}));
    EXPECT_FALSE(region.PointInSet(Vector1d{qmin - kTol}));
    EXPECT_FALSE(region.PointInSet(Vector1d{qmax + kTol}));
...
<robot name="double_pendulum">
  ...
  <joint name="joint2" type="revolute">
    ...
    <child link="link2"/>
            ...
            fmt::arg("w_plus_one_half", w + .5), fmt::arg("l1", l1), fmt::arg("l2", l2), fmt::arg("r", r));
...

    {
        ...
        meshcat->Set2dRenderMode(math::RigidTransformd(Eigen::Vector3d{0, 0, 1}), -3.25, 3.25, -3.25, 3.25);
        ...
        for (int i = 0; i < theta2s.size(); ++i) {
            const double a = l1 + l2 * std::cos(theta2s[i]), b = l2 * std::sin(theta2s[i]);
            // wolfram solve a*sin(q) + b*cos(q) = c for q
            points(0, i) = 2 * std::atan((std::sqrt(a * a + b * b - c * c) + a) / (b + c)) + M_PI;
            points(1, i) = theta2s[i];
            points(0, points.cols() - i - 2) = 2 * std::atan((std::sqrt(a * a + b * b - c * c) + a) / (b - c)) - M_PI;
...
GTEST_TEST(IrisInConfigurationSpaceTest, BlockOnGround) {
    ...
    {
        std::shared_ptr<Meshcat> meshcat = geometry::GetTestEnvironmentMeshcat();
        meshcat->Set2dRenderMode(math::RigidTransformd(Eigen::Vector3d{0, 0, 1}), 0, 3.25, -3.25, 3.25);
        meshcat->SetProperty("/Grid", "visible", true);
...
<robot name="pendulum_on_vertical_track">
  ...
  <joint name="pendulum" type="revolute">
...

    {
        ...
        meshcat->SetTransform("Test point", math::RigidTransform(Eigen::Vector3d(z_test, theta_test, 0)));
...
<robot name="double_pendulum">
  ...
  <joint name="joint2" type="revolute">
    ...
    auto context = diagram->CreateDefaultContext();
    systems::Context<double>& plant_context = plant.GetMyMutableContextFromRoot(context.get());
    ...
    ik.AddPositionConstraint(plant.GetFrameByName("link2"), Eigen::Vector3d(0, 0, -1), plant.world_frame(),
                             Eigen::Vector3d(-kInf, -kInf, -kInf), Eigen::Vector3d(kInf, kInf, -1));
    ...
    HPolyhedron region = IrisInConfigurationSpace(plant, plant.GetMyContextFromRoot(*context), options);

    EXPECT_EQ(region.ambient_dimension(), 2);

    const double theta1 = 0.1;
    const double theta2_max = std::acos(1 - std::cos(theta1)) - theta1 - options.configuration_space_margin;
    const double theta2_min = -std::acos(1 - std::cos(theta1)) - theta1 + options.configuration_space_margin;
    ...
    EXPECT_TRUE(region.PointInSet(Eigen::Vector2d{theta1, theta2_min + kInnerTol}));
    EXPECT_TRUE(region.PointInSet(Eigen::Vector2d{theta1, theta2_max - kInnerTol}));
    EXPECT_FALSE(region.PointInSet(Eigen::Vector2d{theta1, theta2_min - kOuterTol}));
    EXPECT_FALSE(region.PointInSet(Eigen::Vector2d{theta1, theta2_max + kOuterTol}));

    {
        ...
        for (int i = 0; i < theta1s.size(); ++i) {
            ...
            points(1, points.cols() - i - 1) = -std::acos(1 - std::cos(theta1s[i])) - theta1s[i];
        }
        meshcat->SetLine("True C_free", points, 2.0, Rgba(0, 0, 1));
        VPolytope vregion = VPolytope(region).GetMinimalRepresentation();
        points.resize(3, vregion.vertices().cols() + 1);
