...
TEST_F(CIrisToyRobotTest, GetCollisionGeometries) {
    ...
    auto check_link_geometries = [&link_geometries](
                                         const multibody::BodyIndex body,
                                         const std::unordered_set<geometry::GeometryId>& geometry_ids_expected) {
    ...
    check_link_geometries(plant_->world_body().index(), {world_box_, world_cylinder_});
