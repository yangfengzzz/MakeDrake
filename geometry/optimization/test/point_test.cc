...
GTEST_TEST(PointTest, BasicTest) {
    ...
    EXPECT_FALSE(CheckAddPointInSetConstraints(P, p_W + Vector3d::Constant(0.01)));
...
GTEST_TEST(PointTest, FromSceneGraphTest) {
    ...
    auto [scene_graph, geom_id, context, query] = MakeSceneGraphWithShape(Sphere(kRadius), RigidTransformd(p_W));

    EXPECT_NEAR(query.ComputeSignedDistanceToPoint(p_W)[0].distance, -kRadius, 1e-16);
    ...
    EXPECT_THROW(Point(query, geom_id, std::nullopt, kRadius / 2.0), std::exception);
...
GTEST_TEST(PointTest, FromSceneGraphBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Point(query, geom_id), ".*Point.*cannot.*HalfSpace.*");
}
...
GTEST_TEST(PointTest, NonnegativeScalingTest) {
    ...
    std::vector<Binding<Constraint>> constraints = P.AddPointInNonnegativeScalingConstraints(&prog, x, t);
...
GTEST_TEST(PointTest, NonnegativeScalingTest2) {
  ...
  A << 1, 0, 0,
    ...
    std::vector<Binding<Constraint>> constraints = P.AddPointInNonnegativeScalingConstraints(&prog, A, b, c, d, x, t);
