...
#include "geometry/optimization/affine_subspace.h"
...
std::tuple<bool, solvers::MathematicalProgramResult> IsInfeasible(const MathematicalProgram& prog) {
    ...
    return {result.get_solution_result() == solvers::SolutionResult::kInfeasibleConstraints, result};
}
 ...
 */
bool IsRedundant(const Eigen::Ref<const MatrixXd>& c,
                 double d,
    ...
    DRAKE_DEMAND(prog->GetAllConstraints().size() == prog->GetAllLinearConstraints().size());
    DRAKE_DEMAND(prog->GetAllCosts().size() == 1 && prog->linear_costs()[0] == *program_cost_binding);
    ...
    new_constraint->evaluator()->UpdateCoefficients(c, VectorXd::Constant(1, -kInf), VectorXd::Constant(1, d + 1));
    ...
    if (!polyhedron_is_empty && !result.is_success()) {
        throw std::runtime_error(
                fmt::format("Solver {} failed to compute the set difference; it "
...
HPolyhedron::HPolyhedron(const Eigen::Ref<const MatrixXd>& A, const Eigen::Ref<const VectorXd>& b)
...
HPolyhedron::HPolyhedron(const QueryObject<double>& query_object,
    ...
    const RigidTransformd X_WE =
            reference_frame ? query_object.GetPoseInWorld(*reference_frame) : RigidTransformd::Identity();
...
HPolyhedron::HPolyhedron(const VPolytope& vpoly, double tol) : ConvexSet(vpoly.ambient_dimension(), false) {
    ...
    if (affine_hull.AffineDimension() < affine_hull.ambient_dimension()) {
        ...
        // affine subspace.
        Eigen::MatrixXd points_local = affine_hull.ToLocalCoordinates(vpoly.vertices());
        ...
        if (affine_hull.AffineDimension() == 0) {
            ...
            Eigen::MatrixXd P_in = Eigen::MatrixXd::Identity(ambient_dimension(), ambient_dimension());
            Eigen::MatrixXd P = affine_hull.ToLocalCoordinates(P_in.colwise() + affine_hull.translation());

            // Now, we construct the global versions of A and b.
            global_A = hpoly_subspace.A() * P;
            global_b = hpoly_subspace.b() + hpoly_subspace.A() * P * affine_hull.translation();
        }
        ...
        Eigen::MatrixXd orthogonal_complement_basis = affine_hull.OrthogonalComplementBasis();
        Eigen::MatrixXd orth_A(2 * orthogonal_complement_basis.cols(), ambient_dimension());
        orth_A << orthogonal_complement_basis.transpose(), -orthogonal_complement_basis.transpose();
        Eigen::VectorXd orth_b = orth_A * affine_hull.translation();

        Eigen::MatrixXd full_A(global_A.rows() + orth_A.rows(), ambient_dimension());
    ...
    qhull.runQhull("", vpoly.ambient_dimension(), vpoly.vertices().cols(), vpoly.vertices().data(), "");
    if (qhull.qhullStatus() != 0) {
        throw std::runtime_error(fmt::format("Qhull exited with status {} and  message:\n{}", qhull.qhullStatus(),
                                             qhull.qhullMessage()));
    }
    ...
    for (const auto& facet : qhull.facetList()) {
        A_.row(facet_count) = Eigen::Map<Eigen::RowVectorXd>(facet.outerplane().coordinates(), facet.dimension());
        b_(facet_count) = -facet.outerplane().offset();
        ++facet_count;
    }
}

HPolyhedron::HPolyhedron(const MathematicalProgram& prog) : ConvexSet(prog.num_vars(), false) {
    // Preconditions
    DRAKE_THROW_UNLESS(prog.num_vars() > 0);
    DRAKE_THROW_UNLESS(prog.GetAllConstraints().size() > 0);
    DRAKE_THROW_UNLESS(solvers::GetProgramType(prog) == solvers::ProgramType::kLP);
    ...
    solvers::internal::ParseLinearEqualityConstraints(prog, &A_triplets, &b, &A_row_count,
                                                      &unused_linear_eq_y_start_indices, &unused_num_rows_added);
    ...
    for (int i = 0; i < num_equality_triplets; ++i) {
        ...
        A_triplets.emplace_back(triplet.row() + num_equality_constraints, triplet.col(), -triplet.value());
    }
    // We also set b := [b; -b].
    ...
    std::vector<std::vector<std::pair<int, int>>> unused_linear_constraint_dual_indices;
    ...
    Eigen::SparseMatrix<double, Eigen::RowMajor> A_sparse(A_row_count, prog.num_vars());
    ...
    *this = HPolyhedron(A_sparse.toDense()(rows_to_keep, Eigen::all), b_eigen(rows_to_keep));
}
...
Hyperellipsoid HPolyhedron::MaximumVolumeInscribedEllipsoid() const {
    ...
    Eigen::MatrixXd A_lorentz = Eigen::MatrixXd::Zero(1 + C.cols(), (1 + C.cols()) * C.rows());
    Eigen::VectorXd b_lorentz = Eigen::VectorXd::Zero(1 + C.cols());
    for (int i = 0; i < b_.size(); ++i) {
        A_lorentz.setZero();
        A_lorentz.block(0, 0, 1, A_.cols()) = -A_.row(i) / row_norms(i);
        for (int j = 0; j < C.cols(); ++j) {
            A_lorentz.block(j + 1, (j + 1) * C.cols(), 1, A_.cols()) = A_.row(i) / row_norms(i);
        }
    ...
    if (!result.is_success()) {
        throw std::runtime_error(
                fmt::format("Solver {} failed to solve the maximum inscribed ellipse problem; it "
...
VectorXd HPolyhedron::ChebyshevCenter() const {
    ...
    if (!result.is_success()) {
        throw std::runtime_error(
                fmt::format("Solver {} failed to solve the Chebyshev center problem; it terminated "
...
HPolyhedron HPolyhedron::Scale(double scale, std::optional<Eigen::VectorXd> center) const {
    ...
    return HPolyhedron(A_, std::pow(scale, 1.0 / ambient_dimension()) * (b_ - A_ * *center) + A_ * *center);
}

HPolyhedron HPolyhedron::CartesianProduct(const HPolyhedron& other) const {
    MatrixXd A_product = MatrixXd::Zero(A_.rows() + other.A().rows(), A_.cols() + other.A().cols());
...
HPolyhedron HPolyhedron::Intersection(const HPolyhedron& other, bool check_for_redundancy, double tol) const {
    ...
    for (int step = 0; step < mixing_steps; ++step) {
        ...
        if (std::isinf(theta_max) || std::isinf(theta_min) || theta_max < theta_min) {
            throw std::invalid_argument(
                    fmt::format("The Hit and Run algorithm failed to find a feasible point in the "
...
HPolyhedron HPolyhedron::MakeBox(const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
...
std::optional<bool> HPolyhedron::DoIsBoundedShortcut() const {
    ...
    prog.AddBoundingBoxConstraint(1.0, std::numeric_limits<double>::infinity(), y);
    prog.AddLinearEqualityConstraint(A_.transpose(), VectorXd::Zero(A_.cols()), y);
    auto result = solvers::Solve(prog);
...
bool HPolyhedron::DoIsEmpty() const {
    ...
    solvers::MathematicalProgram prog;
    solvers::VectorXDecisionVariable x = prog.NewContinuousVariables(A_.cols(), "x");
...
bool HPolyhedron::ContainedIn(const HPolyhedron& other, double tol) const {
    ...
    Binding<solvers::LinearConstraint> redundant_constraint_binding =
            prog.AddLinearConstraint(other.A().row(0), VectorXd::Constant(1, -kInf), other.b().row(0), x);
    Binding<solvers::LinearCost> program_cost_binding = prog.AddLinearCost(-other.A().row(0), 0, x);

    for (int i = 0; i < other.A().rows(); ++i) {
        // If any of the constraints of `other` are irredundant then `this` is
        // not contained in `other`.
        if (!IsRedundant(other.A().row(i), other.b()(i), &prog, &redundant_constraint_binding, &program_cost_binding,
                         tol)) {
            return false;
        }
    }
...
HPolyhedron HPolyhedron::DoIntersectionNoChecks(const HPolyhedron& other) const {
    MatrixXd A_intersect = MatrixXd::Zero(A_.rows() + other.A().rows(), A_.cols());
...
HPolyhedron HPolyhedron::DoIntersectionWithChecks(const HPolyhedron& other, double tol) const {
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> A(A_.rows() + other.A().rows(), A_.cols());
...
std::set<int> HPolyhedron::FindRedundant(double tol) const {
    ...
    for (int i = 0; i < num_cons; ++i) {
        bindings_vec.push_back(prog.AddLinearConstraint(A_.row(i), -std::numeric_limits<double>::infinity(), b_[i], x));
    }
    ...
    for (int i = 0; i < num_cons; ++i) {
        ...
        if ((result.is_success() && -result.get_optimal_cost() > b_[i] + tol) || !result.is_success()) {
...
namespace {
HPolyhedron MoveFaceAndCull(const Eigen::MatrixXd& A,
...
HPolyhedron HPolyhedron::SimplifyByIncrementalFaceTranslation(const double min_volume_ratio,
                                                              const bool do_affine_transformation,
                                                              const int max_iterations,
                                                              const Eigen::MatrixXd& points_to_contain,
                                                              const std::vector<HPolyhedron>& intersecting_polytopes,
                                                              const bool keep_whole_intersection,
                                                              const double intersection_padding,
    ...
    const VectorXd circumbody_ellipsoid_center = circumbody.MaximumVolumeInscribedEllipsoid().center();
    VectorXd face_center_distance(circumbody_b.rows());
    for (int i = 0; i < circumbody_b.rows(); ++i) {
        face_center_distance(i) = circumbody_b(i) - circumbody_A.row(i) * circumbody_ellipsoid_center;
    }

    // Scaling factor for face distances being translated inward.
    const double face_scale_ratio = 1 - std::pow(min_volume_ratio, 1.0 / ambient_dimension());
    ...
    for (size_t i = 0; i < intersecting_polytopes.size(); ++i) {
        ...
        if (keep_whole_intersection || !scaled_circumbody.IntersectsWith(intersecting_polytopes[i])) {
    ...
    while (any_faces_moved && iterations < max_iterations) {
        ...
        for (int i_shuffle : shuffle_inds) {
            ...
            face_center_distance_shuffled(i_shuffle) = face_center_distance(shuffle_inds[i_shuffle]);
            face_moved_in_shuffled[i_shuffle] = face_moved_in[shuffle_inds[i_shuffle]];
        }
        ...
        while (i < inbody.b().rows()) {
            if (!face_moved_in[i]) {
                ...
                // intersections.
                double b_i_min_allowed = inbody.b()(i) - face_scale_ratio * face_center_distance(i);
                ...
                for (int intersection_ind = 0; intersection_ind < ssize(reduced_intersecting_polytopes);
                     ++intersection_ind) {
                    const HPolyhedron intersection =
                            inbody.Intersection(reduced_intersecting_polytopes[intersection_ind]);
                    ...
                    if (result.is_success()) {
                        if (cost_multiplier * result.get_optimal_cost() + intersection_padding > b_i_min_allowed) {
                            b_i_min_allowed = cost_multiplier * result.get_optimal_cost() + intersection_padding;
                        }
                    } else {
                        log()->warn(
                ...
                for (int i_point = 0; i_point < points_to_contain.cols(); ++i_point) {
                    b_i_min_allowed = std::max(b_i_min_allowed, inbody.A().row(i).dot(points_to_contain.col(i_point)));
                }
                ...
                const std::vector<int> i_redundant(i_redundant_set.begin(), i_redundant_set.end());
                if (i_redundant.size() > 0) {
                    any_faces_moved = true;
                    const MatrixXd A = inbody.A();
                    inbody = MoveFaceAndCull(A, b_proposed, &face_center_distance, &face_moved_in, &i, i_redundant);
                }
            }
            ++i;
        }
        log()->debug("{} faces saved so far", circumbody.b().size() - inbody.b().size());

        ++iterations;
    }

    const HPolyhedron inbody_before_affine_transformation = HPolyhedron(inbody.A(), inbody.b());
    ...
    for (int inter_ind = 0; inter_ind < ssize(reduced_intersecting_polytopes); ++inter_ind) {
        if (do_affine_transformation && !inbody.IntersectsWith(reduced_intersecting_polytopes[inter_ind])) {
...
HPolyhedron HPolyhedron::MaximumVolumeInscribedAffineTransformation(const HPolyhedron& circumbody) const {
    DRAKE_THROW_UNLESS(this->ambient_dimension() == circumbody.ambient_dimension());
    ...
    MathematicalProgram prog;
    solvers::VectorXDecisionVariable t = prog.NewContinuousVariables(this->ambient_dimension(), "t");
    solvers::MatrixXDecisionVariable T = prog.NewSymmetricContinuousVariables(this->ambient_dimension(), "T");
    ...
    // in HPolyhedron.
    solvers::MatrixXDecisionVariable Lambda = prog.NewContinuousVariables(n_y, n_x, "Lambda");
    ...
    solvers::VectorXDecisionVariable equality_variables(n_x + circumbody.ambient_dimension());
    MatrixXd left_hand_inequality_matrix(1, n_x + circumbody.ambient_dimension());
    solvers::VectorXDecisionVariable inequality_variables(n_x + circumbody.ambient_dimension());
    for (int i_row = 0; i_row < n_y; ++i_row) {
        ...
        left_hand_inequality_matrix << this->b().transpose(), circumbody.A().row(i_row);
    ...
    if (!result.is_success()) {
        throw std::runtime_error(
                fmt::format("Solver {} failed to solve the affine transformation problem; it "
...
std::optional<bool> HPolyhedron::DoPointInSetShortcut(const Eigen::Ref<const VectorXd>& x, double tol) const {
    DRAKE_DEMAND(A_.cols() == x.size());
    return ((A_ * x).array() <= b_.array() + tol).all();
}

std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> HPolyhedron::DoAddPointInSetConstraints(
        MathematicalProgram* prog, const Eigen::Ref<const VectorXDecisionVariable>& vars) const {
    VectorX<Variable> new_vars;
    std::vector<Binding<Constraint>> new_constraints;
    new_constraints.push_back(prog->AddLinearConstraint(
            A_, VectorXd::Constant(b_.size(), -std::numeric_limits<double>::infinity()), b_, vars));
    return {std::move(new_vars), std::move(new_constraints)};
}

std::vector<Binding<Constraint>> HPolyhedron::DoAddPointInNonnegativeScalingConstraints(
        MathematicalProgram* prog, const Eigen::Ref<const VectorXDecisionVariable>& x, const Variable& t) const {
    ...
    Abar.leftCols(n) = A_;
    Abar.col(n) = -b_;
    constraints.emplace_back(prog->AddLinearConstraint(Abar,
                                                       VectorXd::Constant(m, -std::numeric_limits<double>::infinity()),
...
std::vector<Binding<Constraint>> HPolyhedron::DoAddPointInNonnegativeScalingConstraints(
        MathematicalProgram* prog,
        const Eigen::Ref<const MatrixXd>& A_x,
        const Eigen::Ref<const VectorXd>& b_x,
        const Eigen::Ref<const VectorXd>& c,
        double d,
    ...
    constraints.emplace_back(prog->AddLinearConstraint(
            A_bar, VectorXd::Constant(m, -std::numeric_limits<double>::infinity()), b_bar, {x, t}));
    return constraints;
}

std::pair<std::unique_ptr<Shape>, math::RigidTransformd> HPolyhedron::DoToShapeWithPose() const {
    throw std::runtime_error(
...
HPolyhedron HPolyhedron::PontryaginDifference(const HPolyhedron& other) const {
    ...
    solvers::VectorXDecisionVariable x = prog.NewContinuousVariables(ambient_dimension(), "x");
    // -inf <= Ax <= b
    prog.AddLinearConstraint(other.A(), VectorXd::Constant(other.b().rows(), -kInf), other.b(), x);
    ...
    Binding<solvers::LinearCost> program_cost_binding = prog.AddLinearCost(A_.row(0), 0, x);
