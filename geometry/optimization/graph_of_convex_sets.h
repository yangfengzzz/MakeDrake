...
struct GraphOfConvexSetsOptions {
    ...
    std::optional<solvers::SolverOptions> restriction_solver_options{std::nullopt};
...
public:
        ...
        */
        std::vector<solvers::Binding<solvers::Cost>> GetCosts(
                const std::unordered_set<Transcription>& used_in_transcription = {
                        Transcription::kMIP, Transcription::kRelaxation, Transcription::kRestriction}) const;
    ...
    private:
        ...
        std::vector<std::pair<solvers::Binding<solvers::Cost>, std::unordered_set<Transcription>>> costs_{};
        std::vector<std::pair<solvers::Binding<solvers::Constraint>, std::unordered_set<Transcription>>> constraints_;
    ...
    */
    solvers::MathematicalProgramResult SolveShortestPath(
            const Vertex& source,
            const Vertex& target,
            const GraphOfConvexSetsOptions& options = GraphOfConvexSetsOptions()) const;
...
private: /* Facilitates testing. */
    ...
    void AddPerspectiveConstraint(solvers::MathematicalProgram* prog,
                                  const solvers::Binding<solvers::Constraint>& binding,
                                  const solvers::VectorXDecisionVariable& vars) const;
