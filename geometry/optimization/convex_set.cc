...
bool SolverReturnedWithoutError(const solvers::MathematicalProgramResult& result) {
    const SolutionResult status = result.get_solution_result();
    return status == SolutionResult::kSolutionFound || status == SolutionResult::kInfeasibleConstraints ||
           status == SolutionResult::kUnbounded || status == SolutionResult::kInfeasibleOrUnbounded ||
...
ConvexSet::ConvexSet(int ambient_dimension, bool has_exact_volume)
    : ambient_dimension_(ambient_dimension), has_exact_volume_(has_exact_volume) {
...
bool ConvexSet::GenericDoIsBounded() const {
    ...
    VectorXDecisionVariable x = prog.NewContinuousVariables(ambient_dimension(), "x");
    AddPointInSetConstraints(&prog, x);
    Binding<LinearCost> objective = prog.AddLinearCost(VectorXd::Zero(ambient_dimension()), x);
    ...
    for (int i = 0; i < ambient_dimension(); ++i) {
        ...
        if (result.get_solution_result() == solvers::SolutionResult::kUnbounded ||
            result.get_solution_result() == solvers::SolutionResult::kInfeasibleOrUnbounded ||
            result.get_solution_result() == solvers::SolutionResult::kDualInfeasible) {
            return false;
        }
        ...
        if (result2.get_solution_result() == solvers::SolutionResult::kUnbounded ||
            result2.get_solution_result() == solvers::SolutionResult::kInfeasibleOrUnbounded ||
            result2.get_solution_result() == solvers::SolutionResult::kDualInfeasible) {
            return false;
        }
    }
    return true;
...
std::optional<std::pair<std::vector<double>, Eigen::MatrixXd>> ConvexSet::Projection(
    ...
    std::vector<std::optional<double>> shortcut_distances = DoProjectionShortcut(points, &projected_points);
    int num_unprojected =
            std::count_if(shortcut_distances.begin(), shortcut_distances.end(), [](const std::optional<double>& opt) {
    ...
    const auto [remaining_distances, remaining_projected_points] = maybe_project.value();
    unprojected_ind = 0;
    for (int i = 0; i < points.cols(); ++i) {
        if (!shortcut_distances.at(i).has_value()) {
...
std::optional<std::pair<std::vector<double>, Eigen::MatrixXd>> ConvexSet::GenericDoProjection(
        const Eigen::Ref<const Eigen::MatrixXd>& points) const {
    MathematicalProgram prog;
    MatrixX<symbolic::Variable> projected_points_vars(points.rows(), points.cols());
    std::vector<solvers::Binding<solvers::Cost>> distances_bindings;
    for (int i = 0; i < points.cols(); ++i) {
        projected_points_vars.col(i) = prog.NewContinuousVariables(ambient_dimension(), fmt::format("x{}", i));
        AddPointInSetConstraints(&prog, projected_points_vars.col(i));
        distances_bindings.emplace_back(
                prog.AddQuadraticErrorCost(Eigen::MatrixXd::Identity(ambient_dimension(), ambient_dimension()),
    ...
    const Eigen::MatrixXd projected_points = result.GetSolution(projected_points_vars);
    std::vector<double> distances(points.cols(), 0.0);
    for (int i = 0; i < points.cols(); ++i) {
...
std::vector<std::optional<double>> ConvexSet::DoProjectionShortcut(const Eigen::Ref<const Eigen::MatrixXd>& points,
                                                                   EigenPtr<Eigen::MatrixXd> projected_points) const {
    DRAKE_DEMAND(projected_points != nullptr);
    DRAKE_DEMAND(projected_points->rows() == points.rows() && projected_points->cols() == points.cols());

    // If we have a fast point in set shortcut, use it first.
    const double kTol = 1e-12;  // This is below the tolerance of most convex solvers.
    std::vector<std::optional<double>> distances(points.cols(), std::nullopt);

    for (int i = 0; i < points.cols(); ++i) {
        const auto point_in_set_shortcut = DoPointInSetShortcut(points.col(i), kTol);
...
bool ConvexSet::DoPointInSet(const Eigen::Ref<const Eigen::VectorXd>& x, double tol) const {
...
bool ConvexSet::GenericDoPointInSet(const Eigen::Ref<const Eigen::VectorXd>& x, double tol) const {
    MathematicalProgram prog;
    VectorXDecisionVariable point = prog.NewContinuousVariables(ambient_dimension(), "x");
    AddPointInSetConstraints(&prog, point);
...
ConvexSet::AddPointInSetConstraints(solvers::MathematicalProgram* prog,
...
std::vector<solvers::Binding<solvers::Constraint>> ConvexSet::AddPointInNonnegativeScalingConstraints(
        solvers::MathematicalProgram* prog,
    ...
    constraints.emplace_back(prog->AddBoundingBoxConstraint(0, std::numeric_limits<double>::infinity(), t));
    return constraints;
}
...
std::vector<solvers::Binding<solvers::Constraint>> ConvexSet::AddPointInNonnegativeScalingConstraints(
    ...
    constraints.emplace_back(prog->AddLinearConstraint(c.transpose(), -d, std::numeric_limits<double>::infinity(), t));
    return constraints;
}

std::optional<symbolic::Variable> ConvexSet::HandleZeroAmbientDimensionConstraints(
        solvers::MathematicalProgram* prog,
        const ConvexSet& set,
...
SampledVolume ConvexSet::CalcVolumeViaSampling(RandomGenerator* generator,
    ...
    while (relative_accuracy_ub_squared > desired_rel_accuracy_squared && num_samples < max_num_samples) {
    ...
    const auto estimated_volume = aabb.CalcVolume() * static_cast<double>(num_hits) / static_cast<double>(num_samples);
    const auto relative_accuracy_ub = std::sqrt(relative_accuracy_ub_squared);
    return {.volume = estimated_volume, .rel_accuracy = relative_accuracy_ub, .num_samples = num_samples};
}
