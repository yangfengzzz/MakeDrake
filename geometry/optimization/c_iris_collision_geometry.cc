...
[[nodiscard]] symbolic::RationalFunction ComputePointOnPlaneSideRational(
    ...
    const Vector3<symbolic::Polynomial> p_AQ = X_AB_multilinear.position + X_AB_multilinear.rotation * p_BQ;
    // Compute lhs=aᵀ*p_AQ + b
    const symbolic::Polynomial lhs = a_A.dot(p_AQ) + b;
    if (plane_side == PlaneSide::kPositive) {
        return rational_forward_kin.ConvertMultilinearPolynomialToRationalFunction(lhs - offset);
    } else {
        return rational_forward_kin.ConvertMultilinearPolynomialToRationalFunction(-offset - lhs);
    }
}
...
void ImplementPointRationals(const Eigen::Ref<const Eigen::Matrix3Xd>& p_GV,
    ...
    reify_data->rationals->reserve(reify_data->rationals->size() + num_rationals_for_points);
    ...
    for (int i = 0; i < p_GV.cols(); ++i) {
        reify_data->rationals->push_back(ComputePointOnPlaneSideRational(
                *(reify_data->a), *(reify_data->b), p_GV.col(i), X_BG, *(reify_data->X_AB_multilinear), offset,
...
void ImplementSpherePsdMatRational(const Eigen::Vector3d& p_GS,
    ...
    const Vector3<symbolic::Polynomial> p_AS =
            reify_data->X_AB_multilinear->position + reify_data->X_AB_multilinear->rotation * p_BS;
    // Compute aᵀp_AS + b
    const symbolic::RationalFunction a_dot_x_plus_b =
            reify_data->rational_forward_kin->ConvertMultilinearPolynomialToRationalFunction(reify_data->a->dot(p_AS) +
                                                                                             *(reify_data->b));
    const Vector3<symbolic::Polynomial> y_poly(symbolic::Polynomial((*(reify_data->y_slack))(0)),
                                               symbolic::Polynomial((*(reify_data->y_slack))(1)),
                                               symbolic::Polynomial((*(reify_data->y_slack))(2)));
    // Compute yᵀy
    const symbolic::Polynomial y_squared{{{symbolic::Monomial((*(reify_data->y_slack))(0), 2), 1},
    ...
    reify_data->rationals->emplace_back(sign * a_dot_x_plus_b.numerator() +
...
public:
    OnPlaneSideReifier(const Shape* geometry, math::RigidTransformd X_BG, GeometryId geometry_id)
        : geometry_{geometry}, X_BG_{std::move(X_BG)}, geometry_id_{geometry_id} {}

    void ProcessData(const Vector3<symbolic::Polynomial>& a,
                     const symbolic::Polynomial& b,
                     const multibody::RationalForwardKinematics::Pose<symbolic::Polynomial>& X_AB_multilinear,
                     const multibody::RationalForwardKinematics& rational_forward_kin,
                     PlaneSide plane_side,
                     const VectorX<symbolic::Variable>& y_slack,
                     std::vector<symbolic::RationalFunction>* rationals) {
        ReifyData data{&a, &b, &X_AB_multilinear, &rational_forward_kin, plane_side, geometry_id_, &y_slack, rationals};
...
private:
    ...
    void ImplementGeometry(const Sphere& sphere, void* data) {
        ...
        ImplementSpherePsdMatRational(Eigen::Vector3d::Zero(), X_BG_, sphere.radius(), reify_data);
    ...
    void ImplementGeometry(const Capsule& capsule, void* data) {
        ...
        auto* reify_data = static_cast<ReifyData*>(data);
        ImplementSpherePsdMatRational(Eigen::Vector3d(0, 0, capsule.length() / 2), X_BG_, capsule.radius(), reify_data);
        ImplementSpherePsdMatRational(Eigen::Vector3d(0, 0, -capsule.length() / 2), X_BG_, capsule.radius(),
                                      reify_data);
    ...
    void ImplementGeometry(const Cylinder& cylinder, void* data) {
        ...
        const Vector3<symbolic::Polynomial> a_G =
                (reify_data->X_AB_multilinear->rotation * X_BG_.rotation().matrix()).transpose() * (*reify_data->a);
        // p_AG = R_AB * p_BG + p_AB
        const Vector3<symbolic::Polynomial> p_AG =
                reify_data->X_AB_multilinear->rotation * X_BG_.translation() + reify_data->X_AB_multilinear->position;
        ...
        switch (reify_data->plane_side) {
            case PlaneSide::kPositive: {
                ...
                reify_data->rationals->push_back(
                        reify_data->rational_forward_kin->ConvertMultilinearPolynomialToRationalFunction(b_G - 1));
                break;
            }
            case PlaneSide::kNegative: {
                // Impose the constraint b_G <= -1
                reify_data->rationals->push_back(
                        reify_data->rational_forward_kin->ConvertMultilinearPolynomialToRationalFunction(-1 - b_G));
                break;
            }
        }
        ...
        // ⌊y(1)⌋ ⌊a_G(1)            0      (a_G(2)*l/2+b_G)/r²⌋⌊y(1)⌋
        const Vector2<symbolic::Polynomial> y_poly(symbolic::Polynomial(symbolic::Monomial((*reify_data->y_slack)(0))),
                                                   symbolic::Polynomial(symbolic::Monomial((*reify_data->y_slack)(1))));
        // Compute y(0)*y(0) + y(1)*y(1)
        const symbolic::Polynomial y_squared{{{symbolic::Monomial((*reify_data->y_slack)(0), 2), 1},
                                              {symbolic::Monomial((*reify_data->y_slack)(1), 2), 1}}};
        const int plane_sign = reify_data->plane_side == PlaneSide::kPositive ? 1 : -1;
        for (double scalar : {1, -1}) {
            // scalar = 1 for the top rim, and scalar = -1 for the bottom rim.
            // Compute plane_sign * (±l/2 * a_G(2) + b_G)
            const symbolic::Polynomial a2_plus_b = plane_sign * (scalar * cylinder.length() / 2 * a_G(2) + b_G);
            reify_data->rationals->push_back(
                    reify_data->rational_forward_kin->ConvertMultilinearPolynomialToRationalFunction(
                            a2_plus_b + 2 * y_poly(0) * a_G(0) + 2 * y_poly(1) * a_G(1) +
                            a2_plus_b / (cylinder.radius() * cylinder.radius()) * y_squared));
        }
    }
...
private:
    using ShapeReifier::ImplementGeometry;
    void ImplementGeometry(const Box&, void*) { type_ = CIrisGeometryType::kPolytope; }

    void ImplementGeometry(const Convex&, void*) { type_ = CIrisGeometryType::kPolytope; }

    void ImplementGeometry(const Sphere&, void*) { type_ = CIrisGeometryType::kSphere; }

    void ImplementGeometry(const Capsule&, void*) { type_ = CIrisGeometryType::kCapsule; }

    void ImplementGeometry(const Cylinder&, void*) { type_ = CIrisGeometryType::kCylinder; }
...
public:
    explicit DistanceToHalfspaceReifier(const Shape& shape, Eigen::Vector3d a_G, double b_G)
...
private:
    ...
    void ImplementGeometry(const Convex& convex, void*) {
        distance_ = ((a_G_.transpose() * GetVertices(convex)).minCoeff() + b_G_) / (a_G_.norm());
    }

    void ImplementGeometry(const Sphere& sphere, void*) { distance_ = b_G_ / (a_G_.norm()) - sphere.radius(); }
    ...
    void ImplementGeometry(const Capsule& capsule, void*) {
        ...
        distance_ = ((a_G_.transpose() * p_GS).minCoeff() + b_G_) / (a_G_.norm()) - capsule.radius();
    }
    ...
    void ImplementGeometry(const Cylinder& cylinder, void*) {
        ...
        distance_ = (-cylinder.radius() * a_G_.head<2>().norm() +
...
void CIrisCollisionGeometry::OnPlaneSide(
    ...
    reifier.ProcessData(a, b, X_AB_multilinear, rational_forward_kin, plane_side, y_slack, rationals);
}
...
double DistanceToHalfspace(const CIrisCollisionGeometry& collision_geometry,
    ...
    const auto X_BE =
            plant.CalcRelativeTransform(plant_context, plant.get_body(collision_geometry.body_index()).body_frame(),
