#include "geometry/optimization/spectrahedron.h"
...
VectorXDecisionVariable GetVariablesByIndex(const Eigen::Ref<const VectorXDecisionVariable>& vars,
...
Spectrahedron::Spectrahedron(const MathematicalProgram& prog) : ConvexSet(prog.num_vars(), false) {
    for (const ProgramAttribute& attr : prog.required_capabilities()) {
        if (!supported_attributes().contains(attr)) {
            throw std::runtime_error(
                    fmt::format("Spectrahedron does not support MathematicalPrograms that require "
...
const ProgramAttributes& Spectrahedron::supported_attributes() {
    static const never_destroyed<ProgramAttributes> kSupportedAttributes{ProgramAttributes{
            ProgramAttribute::kLinearCost, ProgramAttribute::kLinearConstraint,
            ProgramAttribute::kLinearEqualityConstraint, ProgramAttribute::kPositiveSemidefiniteConstraint}};
...
std::optional<bool> Spectrahedron::DoPointInSetShortcut(const Eigen::Ref<const VectorXd>& x, double tol) const {
    return sdp_->CheckSatisfied(sdp_->GetAllConstraints(), x, tol);
}

std::pair<VectorX<Variable>, std::vector<Binding<Constraint>>> Spectrahedron::DoAddPointInSetConstraints(
        MathematicalProgram* prog, const Eigen::Ref<const VectorXDecisionVariable>& x) const {
    ...
    for (const auto& binding : sdp_->GetAllConstraints()) {
        new_constraints.push_back(prog->AddConstraint(
                binding.evaluator(), GetVariablesByIndex(x, sdp_->FindDecisionVariableIndices(binding.variables()))));
    }
...
std::vector<Binding<Constraint>> Spectrahedron::DoAddPointInNonnegativeScalingConstraints(
    ...
    auto stack_xt = [&x, &t, this](const VectorXDecisionVariable& bind_vars) {
        ...
        xt << GetVariablesByIndex(x, sdp_->FindDecisionVariableIndices(bind_vars)), t;
        return xt;
    };
    ...
    for (const auto& binding : sdp_->linear_equality_constraints()) {
        ...
        Ab.leftCols(binding.evaluator()->num_vars()) = binding.evaluator()->get_sparse_A();
        Ab.rightCols<1>() = -binding.evaluator()->lower_bound();

        VectorXd zeros = VectorXd::Zero(binding.evaluator()->num_constraints());
        constraints.emplace_back(prog->AddLinearEqualityConstraint(Ab, zeros, vars));
    }

    std::vector<Binding<LinearConstraint>> linear_inequality_constraints = sdp_->linear_constraints();
    // Treat bounding box constraints as general linear inequality constraints.
    linear_inequality_constraints.insert(linear_inequality_constraints.end(), sdp_->bounding_box_constraints().begin(),
    ...
    for (const auto& binding : linear_inequality_constraints) {
        ...
        if (binding.evaluator()->lower_bound().array().isFinite().any()) {
            ...
            VectorXd zeros = VectorXd::Zero(binding.evaluator()->num_constraints());
            VectorXd infs = VectorXd::Constant(binding.evaluator()->num_constraints(), kInf);

            constraints.emplace_back(prog->AddLinearConstraint(Ab, zeros, infs, vars));
        }
        if (binding.evaluator()->upper_bound().array().isFinite().any()) {
            ...
            constraints.emplace_back(prog->AddLinearConstraint(Ab, -infs, zeros, vars));
        }
    }
    ...
    for (const auto& binding : sdp_->positive_semidefinite_constraints()) {
        ...
        VectorXDecisionVariable vars = GetVariablesByIndex(x, sdp_->FindDecisionVariableIndices(binding.variables()));
        constraints.emplace_back(prog->AddConstraint(
                binding.evaluator(), Map<MatrixX<Variable>>(vars.data(), binding.evaluator()->matrix_rows(),
...
std::vector<Binding<Constraint>> Spectrahedron::DoAddPointInNonnegativeScalingConstraints(
    ...
    for (const auto& binding : sdp_->linear_equality_constraints()) {
        ...
        A_bar.rightCols(t.size()) = -binding.evaluator()->upper_bound() * c.transpose();
        VectorXd b_bar = d * binding.evaluator()->upper_bound() - binding.evaluator()->get_sparse_A() * this_b;
        constraints.emplace_back(prog->AddLinearEqualityConstraint(A_bar, b_bar, {x, t}));
    }
    ...
    for (const auto& binding : linear_inequality_constraints) {
        ...
        MatrixXd A_bar(num_constraints, x.size() + t.size());
        A_bar.leftCols(x.size()) = binding.evaluator()->get_sparse_A() * this_A;
        VectorXd b_bar = d * binding.evaluator()->lower_bound() - binding.evaluator()->get_sparse_A() * this_b;
        if (b_bar.array().isFinite().any()) {
            A_bar.rightCols(t.size()) = -binding.evaluator()->lower_bound() * c.transpose();
            constraints.emplace_back(
                    prog->AddLinearConstraint(A_bar, b_bar, VectorXd::Constant(num_constraints, kInf), {x, t}));
        }
        // Then again for the upper bound
        // A_c (A x + b) ≤ (c' t + d) b_c, written as
        // [A_c * A, -b_c * c'][x;t] ≤ d * b_c - A_c * b
        // A̅ [x;t] ≤ b̅,
        b_bar = d * binding.evaluator()->upper_bound() - binding.evaluator()->get_sparse_A() * this_b;
        if (b_bar.array().isFinite().any()) {
            A_bar.rightCols(t.size()) = -binding.evaluator()->upper_bound() * c.transpose();
            constraints.emplace_back(
                    prog->AddLinearConstraint(A_bar, VectorXd::Constant(num_constraints, -kInf), b_bar, {x, t}));
        }
    }
...
std::pair<std::unique_ptr<Shape>, math::RigidTransformd> Spectrahedron::DoToShapeWithPose() const {
    // I could potentially visualize the 2x2 case in three dimensions (as a mesh
    // if nothing else).
    throw std::runtime_error("ToShapeWithPose is not supported by Spectrahedron.");
}
