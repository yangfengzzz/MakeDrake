#include "geometry/optimization/cspace_free_polytope_base.h"
...
bool IsFutureReady(const std::future<T>& future) {
    ...
    const std::future_status status = future.wait_for(std::chrono::milliseconds(1));
...
void FindMonomialBasisArray(
        ...
        const std::unordered_map<SortedPair<multibody::BodyIndex>, std::vector<int>>& map_body_pair_to_s_on_chain,
        std::unordered_map<SortedPair<multibody::BodyIndex>, std::array<VectorX<symbolic::Monomial>, 4>>*
    ...
    if (body_pair_it == map_body_to_monomial_basis_array->end()) {
        ...
        const std::vector<int>& s_indices = map_body_pair_to_s_on_chain.at(body_pair);
        ...
        if (s_set.empty()) {
            ...
            (*monomial_basis_array)[0] = symbolic::CalcMonomialBasisOrderUpToOne(s_set);
        }
...
CspaceFreePolytopeBase::CspaceFreePolytopeBase(const multibody::MultibodyPlant<double>* plant,
    ...
    std::map<SortedPair<multibody::BodyIndex>,
            ...
            collision_pairs;
    int num_collision_pairs = internal::GenerateCollisionPairs(rational_forward_kin_.plant(), *scene_graph_,
                                                               link_geometries_, &collision_pairs);
        ...
        for (const auto& geometry_pair : geometry_pairs) {
            ...
            switch (plane_order_) {
                case SeparatingPlaneOrder::kAffine: {
                    const VectorX<symbolic::Variable> s_for_plane = GetSForPlane(link_pair, s_for_plane_enum);
                    plane_decision_vars.resize(4 * s_for_plane.rows() + 4);
                    for (int i = 0; i < plane_decision_vars.rows(); ++i) {
                        plane_decision_vars(i) = symbolic::Variable(fmt::format("plane_var{}", i));
                    }
                    CalcPlane<symbolic::Variable, symbolic::Variable, symbolic::Polynomial>(
                            plane_decision_vars, s_for_plane, ToPlaneDegree(plane_order_), &a, &b);
                    break;
                }
            }
            const multibody::BodyIndex expressed_body = multibody::internal::FindBodyInTheMiddleOfChain(
                    rational_forward_kin_.plant(), link_pair.first(), link_pair.second());
            separating_planes_.emplace_back(a, b, geometry_pair.first, geometry_pair.second, expressed_body,
                                            ToPlaneDegree(plane_order_), plane_decision_vars);

            map_geometries_to_separating_planes_.emplace(
                    SortedPair<geometry::GeometryId>(geometry_pair.first->id(), geometry_pair.second->id()),
            ...
            SetIndicesOfSOnChainForBodyPair(SortedPair<multibody::BodyIndex>(link_pair.first(), expressed_body));
            SetIndicesOfSOnChainForBodyPair(SortedPair<multibody::BodyIndex>(link_pair.second(), expressed_body));
        }
    }
...
void CspaceFreePolytopeBase::CalcMonomialBasis() {
    for (int plane_index = 0; plane_index < static_cast<int>(separating_planes_.size()); ++plane_index) {
        const auto& plane = separating_planes_[plane_index];
        for (const auto collision_geometry : {plane.positive_side_geometry, plane.negative_side_geometry}) {
            const SortedPair<multibody::BodyIndex> body_pair(plane.expressed_body, collision_geometry->body_index());
            std::array<VectorX<symbolic::Monomial>, 4> monomial_basis_array;
            FindMonomialBasisArray(rational_forward_kin_, y_slack_, body_pair, map_body_pair_to_s_on_chain_,
                                   &map_body_to_monomial_basis_array_, &monomial_basis_array);
        }
    }
}

template <typename T>
void CspaceFreePolytopeBase::CalcSBoundsPolynomial(const VectorX<T>& s_lower,
    ...
    for (int i = 0; i < s_size; ++i) {
        (*s_minus_s_lower)(i) = symbolic::Polynomial(
                symbolic::Polynomial::MapType({{symbolic::Monomial(s(i)), 1}, {monomial_one, -s_lower(i)}}));
        (*s_upper_minus_s)(i) = symbolic::Polynomial(
                symbolic::Polynomial::MapType({{monomial_one, s_upper(i)}, {symbolic::Monomial(s(i)), -1}}));
    }
}

void CspaceFreePolytopeBase::SetIndicesOfSOnChainForBodyPair(const SortedPair<multibody::BodyIndex>& body_pair) {
    if (!map_body_pair_to_s_on_chain_.contains(body_pair)) {
        const std::vector<multibody::internal::MobilizerIndex> mobilizer_indices =
                multibody::internal::FindMobilizersOnPath(rational_forward_kin_.plant(), body_pair.first(),
                                                          body_pair.second());
        const auto& tree = multibody::internal::GetInternalTree(rational_forward_kin_.plant());
        ...
        for (const auto& mobilizer_index : mobilizer_indices) {
            ...
            if ((mobilizer.num_positions() == 1 && mobilizer.num_velocities() == 1) &&
                ...
                s_indices.push_back(rational_forward_kin_.map_mobilizer_to_s_index()[mobilizer_index]);
...
void CspaceFreePolytopeBase::SolveCertificationForEachPlaneInParallel(
        ...
        bool verbose,
        bool terminate_at_failure) const {
    ...
    // not.
    std::vector<std::optional<bool>> is_success(active_plane_indices.size(), std::nullopt);
    ...
    while ((active_operations.size() > 0 ||
            (sos_dispatched < static_cast<int>(active_plane_indices.size()) && !stop_dispatching))) {
        // Check for completed operations.
        for (auto operation = active_operations.begin(); operation != active_operations.end();) {
            if (IsFutureReady(*operation)) {
                ...
                if (verbose) {
                    drake::log()->debug("SOS {}/{} completed, is_success {}", plane_count, active_plane_indices.size(),
                                        sos_success);
                }
        ...
        while (static_cast<int>(active_operations.size()) < num_threads &&
               sos_dispatched < static_cast<int>(active_plane_indices.size()) && !stop_dispatching) {
            active_operations.emplace_back(std::async(std::launch::async, std::move(solve_plane_sos), sos_dispatched));
            if (verbose) {
                drake::log()->debug("SOS {}/{} dispatched", sos_dispatched, active_plane_indices.size());
            }
            ++sos_dispatched;
        }
    ...
    if (std::all_of(is_success.begin(), is_success.end(), [](std::optional<bool> flag) {
        ...
        if (verbose) {
            ...
            for (int plane_count = 0; plane_count < static_cast<int>(active_plane_indices.size()); ++plane_count) {
                const int plane_index = active_plane_indices[plane_count];
                if (is_success[plane_count].has_value() && !(is_success[plane_count].value())) {
                    bad_pairs.append(fmt::format(
                            "({}, {})\n",
                            inspector.GetName(separating_planes()[plane_index].positive_side_geometry->id()),
                            inspector.GetName(separating_planes()[plane_index].negative_side_geometry->id())));
                }
            }

            drake::log()->debug("Cannot find Lagrangian multipliers and separating planes for \n{}", bad_pairs);
        }
    }
}

VectorX<symbolic::Variable> CspaceFreePolytopeBase::GetSForPlane(const SortedPair<multibody::BodyIndex>& body_pair,
    ...
    switch (s_for_plane_enum) {
        ...
        case SForPlane::kOnChain: {
            const std::vector<int>& s_indices = map_body_pair_to_s_on_chain_.at(body_pair);
...
int CspaceFreePolytopeBase::GetSeparatingPlaneIndex(const SortedPair<geometry::GeometryId>& pair) const {
    return (!map_geometries_to_separating_planes_.contains(pair)) ? -1 : map_geometries_to_separating_planes_.at(pair);
}
...
int CspaceFreePolytopeBase::GetGramVarSizeForPolytopeSearchProgram(
    ...
    for (const auto& plane_geometries : plane_geometries_vec) {
        ...
        if (!ignored_collision_pairs.contains(plane.geometry_pair())) {
            const auto& monomial_basis_array_positive_side = this->map_body_to_monomial_basis_array().at(
                    SortedPair<multibody::BodyIndex>(plane.expressed_body, plane.positive_side_geometry->body_index()));
            for (const auto& rational : plane_geometries.positive_side_rationals) {
                ret += count_gram_per_rational(rational, monomial_basis_array_positive_side);
            }
            const auto& monomial_basis_array_negative_side = this->map_body_to_monomial_basis_array().at(
                    SortedPair<multibody::BodyIndex>(plane.expressed_body, plane.negative_side_geometry->body_index()));
            for (const auto& rational : plane_geometries.negative_side_rationals) {
                ret += count_gram_per_rational(rational, monomial_basis_array_negative_side);
            }
        }
    }
    return ret;
