...
void Rgba::set(const Eigen::Ref<const Eigen::VectorXd>& rgba) {
    ...
    if (rgba.size() == 3) {
        ...
        throw std::runtime_error(fmt::format("Rgba must contain either 3 or 4 elements (given [{}])", rgba.size()));
    }
    ...
    for (int i = 0; i < 4; ++i) {
        if (!(new_value[i] >= 0 && new_value[i] <= 1.0)) {
            throw std::runtime_error(
                    fmt::format("Rgba values must be within the range [0, 1]. Values provided: "
                                "(r={}, g={}, b={}, a={})",
