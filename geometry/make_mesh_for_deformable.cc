...
std::unique_ptr<VolumeMesh<double>> MakeMeshForDeformable(const Shape& shape, double resolution_hint) {
    ...
    return shape.Visit(overloaded{[](const Mesh& mesh) {
                                      return std::make_unique<VolumeMesh<double>>(MakeVolumeMeshFromVtk<double>(mesh));
                                  },
                                  [resolution_hint](const Sphere& sphere) {
                                      return std::make_unique<VolumeMesh<double>>(MakeSphereVolumeMesh<double>(
