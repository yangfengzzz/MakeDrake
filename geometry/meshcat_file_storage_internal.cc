...
std::shared_ptr<const FileStorage::Handle> FileStorage::Insert(std::string&& content, std::string&& filename_hint) {
...
std::shared_ptr<const FileStorage::Handle> FileStorage::Find(const Sha256& sha256) const {
    std::lock_guard guard(impl_->mutex);
    return FindWhileLocked(sha256);
}

std::shared_ptr<const FileStorage::Handle> FileStorage::FindWhileLocked(const Sha256& sha256) const {
...
std::vector<std::shared_ptr<const FileStorage::Handle>> FileStorage::DumpEverything() const {
