...
#include "common/network_policy.h"
#include "common/overloaded.h"
#include "common/scope_exit.h"
#include "common/ssize.h"
#include "common/text_logging.h"
#include "geometry/meshcat_file_storage_internal.h"
#include "geometry/meshcat_internal.h"
#include "geometry/meshcat_recording_internal.h"
...
[[noreturn]] void ThrowThingNotFound(std::string_view thing, std::string_view name, Mapping thing_map) {
...
fetch("data:application/octet-binary;base64,{}")
    ...
    .then(buffer => viewer.handle_command_bytearray(new Uint8Array(buffer)));
            ...
            common_robotics_utilities::base64_helpers::Encode(std::vector<uint8_t>(data.begin(), data.end())));
}
...
public:
    ...
    SceneTreeElement& operator[](std::string_view path) {
        ...
        if (child == children_.end()) {
            child = children_.emplace(name, std::make_unique<SceneTreeElement>()).first;
        }
...
int ToMeshcatColor(const Rgba& rgba) {
    ...
    return (static_cast<int>(255 * rgba.r()) << 16) + (static_cast<int>(255 * rgba.g()) << 8) +
...
void SetLumpedObjectFromTriangleMesh(internal::LumpedObjectData* object,
                                     const Eigen::Ref<const Eigen::Matrix3Xd>& vertices,
                                     const Eigen::Ref<const Eigen::Matrix3Xi>& faces,
                                     const Rgba& rgba,
                                     bool wireframe,
                                     double wireframe_line_width,
                                     Meshcat::SideOfFaceToRender side,
...
public:
    ...
    MeshcatShapeReifier(internal::UuidGenerator* uuid_generator, FileStorage* file_storage, Rgba rgba)
        : uuid_generator_(*uuid_generator), file_storage_(*file_storage), rgba_(rgba) {
        DRAKE_DEMAND(uuid_generator != nullptr);
    ...
    void ImplementMesh(const std::string& filename, const std::string& extension, double scale, void* data) {
        ...
        size_t mtllib_pos;
        if (format == "obj" && (mtllib_pos = mesh_data.find("mtllib ")) != std::string::npos) {
            mtllib_pos += 7;  // Advance to after the actual "mtllib " string.
            std::string mtllib_string = mesh_data.substr(mtllib_pos, mesh_data.find('\n', mtllib_pos));
            std::smatch matches;
            ...
            auto& meshfile_object = lumped.object.emplace<internal::MeshfileObjectData>();
            ...
            if (std::optional<std::string> maybe_mtl_data = ReadFile(basedir / mtllib)) {
                ...
                for (std::sregex_iterator iter(meshfile_object.mtl_library.begin(), meshfile_object.mtl_library.end(),
                                               map_regex);
                     iter != std::sregex_iterator(); ++iter) {
                    std::string map = iter->str(1);
                    std::ifstream map_stream(basedir / map, std::ios::binary | std::ios::ate);
                    if (map_stream.is_open()) {
                        int map_size = map_stream.tellg();
                        map_stream.seekg(0, std::ios::beg);
                        std::vector<uint8_t> map_data;
                        map_data.reserve(map_size);
                        map_data.assign(std::istreambuf_iterator<char>(map_stream), std::istreambuf_iterator<char>());
            ...
            output.assets = internal::UnbundleGltfAssets(filename, &mesh_data, &file_storage_);
            auto& meshfile_object = lumped.object.emplace<internal::MeshfileObjectData>();
            meshfile_object.uuid = uuid_generator_.GenerateRandom();
            meshfile_object.format = std::move(format);
            meshfile_object.data = std::move(mesh_data);
        ...
        // Set the scale.
        std::visit<void>(overloaded{[](std::monostate) {},
    ...
    void ImplementGeometry(const Capsule& capsule, void* data) override {
        ...
        Eigen::Map<Eigen::Matrix4d>(mesh.matrix) =
                RigidTransformd(RotationMatrixd::MakeXRotation(M_PI / 2.0)).GetAsMatrix4();
    }
    ...
    void ImplementGeometry(const Convex& mesh, void* data) override {
        ...
        const TriangleSurfaceMesh<double> tri_hull = internal::MakeTriangleFromPolygonMesh(hull);
    ...
    void ImplementGeometry(const MeshcatCone& cone, void* data) override {
        ...
        Eigen::Map<Eigen::Matrix4d>(mesh.matrix) =
                ...
                RigidTransformd(RotationMatrixd::MakeXRotation(M_PI / 2.0), Eigen::Vector3d{0, 0, cone.height() / 2.0})
                        .GetAsMatrix4();
    }
...
RigidTransformd MakeDrakePoseFromMeshcatPoseForCamera(const std::vector<double>& values) {
...
public:
    ...
    explicit Impl(const MeshcatParams& params)
        : prefix_("/drake"), main_thread_id_(std::this_thread::get_id()), params_(params) {
        DRAKE_THROW_UNLESS(!params.port.has_value() || *params.port == 0 || *params.port >= 1024);
        ...
        const std::string url =
                fmt::format(fmt_runtime(params.web_url_pattern), fmt::arg("host", "foo"), fmt::arg("port", 1));
        ...
        websocket_thread_ =
                std::thread(&Impl::WrappedWebSocketMain, this, std::move(app_promise), params.host, params.port);
        bool connected;
    ...
    void ThrowIfWebsocketThreadExited() const {
        ...
        if (mode_.load() != kNominal) {
            mode_.store(kFinished);
            throw std::runtime_error("Meshcat's internal websocket thread exited unexpectedly");
        }
    }
    ...
    std::string web_url() const {
        ...
        return fmt::format(fmt_runtime(params_.web_url_pattern), fmt::arg("host", display_host),
                           fmt::arg("port", port_));
    }
    ...
    void SetObject(std::string_view path, const Shape& shape, const Rgba& rgba) {
        ...
        std::vector<std::shared_ptr<const FileStorage::Handle>> assets;
        MeshcatShapeReifier::Output reifier_output{.lumped = data.object, .assets = assets};
    ...
    // This function is public via the PIMPL.
    void SetObject(std::string_view path, const perception::PointCloud& cloud, double point_size, const Rgba& rgba) {
        DRAKE_DEMAND(IsThread(main_thread_id_));
    ...
    void SetObject(std::string_view path,
        ...
        SetTriangleMesh(path, vertices, faces, rgba, wireframe, wireframe_line_width, side);
    }
    ...
    void SetTriangleMesh(std::string_view path,
        ...
        SetLumpedObjectFromTriangleMesh(&data.object, vertices, faces, rgba, wireframe, wireframe_line_width, side,
                                        &uuid_generator_);
    ...
    // This function is public via the PIMPL.
    void SetTransform(std::string_view path, const RigidTransformd& X_ParentPath) {
        DRAKE_DEMAND(IsThread(main_thread_id_));
    ...
    template <typename T>
    void SetProperty(std::string_view path, std::string property, const T& value) {
        DRAKE_DEMAND(IsThread(main_thread_id_));
    ...
    //
    // This function is a file-internal helper, not public in the PIMPL.
    void SetPropertyToFile(std::string_view path, std::string property, const fs::path& file_path) {
        DRAKE_DEMAND(IsThread(main_thread_id_));

        // Read the file and insert it into the database.
        std::optional<std::string> content = ReadFile(file_path);
        if (!content) {
            throw std::runtime_error(fmt::format("Cannot open '{}' when attempting to set property '{}' on '{}'",
    ...
    // This function is public via the PIMPL.
    void Set2dRenderMode(const math::RigidTransformd& X_WC, double xmin, double xmax, double ymin, double ymax) {
        DRAKE_DEMAND(IsThread(main_thread_id_));
        ...
        // Lock orbit controls.
        SetProperty("/Cameras/default/rotated/<object>", "position", std::vector<double>{0.0, 0.0, 0.0});
    ...
    void ResetRenderMode() {
        ...
        // Lock orbit controls.
        SetProperty("/Cameras/default/rotated/<object>", "position", std::vector<double>{0.0, 1.0, 3.0});
    ...
    void SetCameraTarget(const Eigen::Vector3d& p_WT, bool only_perspective = true) {
        DRAKE_DEMAND(IsThread(main_thread_id_));
  ...
  'name': '{}'
...

        {
            ...
            if (sliders_.find(data.name) != sliders_.end()) {
                throw std::logic_error(fmt::format("Meshcat already has a slider named {}.", data.name));
            }
            ...
            if (iter == buttons_.end()) {
                ...
                if (iter->second.keycode1.empty()) {
                    ...
                    throw std::logic_error(
                            fmt::format("Meshcat already has a button named `{}`, but the previously "
  ...
  'value': value
...

        {
            ...
            if (buttons_.find(data.name) != buttons_.end()) {
                throw std::logic_error(fmt::format("Meshcat already has a button named {}.", data.name));
            }
            if (sliders_.find(data.name) != sliders_.end()) {
                throw std::logic_error(fmt::format("Meshcat already has a slider named {}.", data.name));
            }
    ...
    std::string CalcStandaloneHtml() const {
        ...
        for (const auto& [src_link, url] : js_paths) {
            const std::string_view url_data = internal::GetMeshcatStaticResource(url).value();
        ...
        std::vector<std::shared_ptr<const FileStorage::Handle>> assets = file_storage_.DumpEverything();
        for (const auto& asset : assets) {
            ...
            javascript += fmt::format(
                    ...
                    common_robotics_utilities::base64_helpers::Encode(
                            std::vector<uint8_t>(asset->content.begin(), asset->content.end())));
        }
        javascript += R"""(
    ...
    std::string GetPackedProperty(std::string_view path, std::string property) const {
        ...
        std::promise<std::string> p;
        std::future<std::string> f = p.get_future();
        Defer([this, path = FullPath(path), property = std::move(property), p = std::move(p)]() mutable {
            DRAKE_DEMAND(IsThread(websocket_thread_id_));
            const SceneTreeElement* e = scene_tree_root_.Find(path);
            std::string value;
            if (e != nullptr) {
...
private:
    bool IsThread(std::thread::id thread_id) const { return (std::this_thread::get_id() == thread_id); }
    ...
    void WrappedWebSocketMain(std::promise<std::tuple<int, bool>> app_promise,
        ...
        try {
            ...
            drake::log()->critical("Meshcat's internal websocket thread crashed via an exception: {}", e.what());
        }
    }
    ...
    void WebSocketMain(std::promise<std::tuple<int, bool>> app_promise,
        ...
        behavior.message = [this](WebSocket* ws, std::string_view message, uWS::OpCode) {
            // IsThread(websocket_thread_id_) is checked by the Handle... function.
        ...
        uWS::App app = uWS::App()
                               .get("/*",
                                    [this](uWS::HttpResponse<kSsl>* response, uWS::HttpRequest* request) {
                                        DRAKE_DEMAND(IsThread(websocket_thread_id_));
        ...
        for (int port = search_start; port <= search_end; ++port) {
            ...
            app.listen(bind_host, port, LIBUS_LISTEN_EXCLUSIVE_PORT, [this](us_listen_socket_t* socket) {
                DRAKE_DEMAND(IsThread(websocket_thread_id_));
                if (socket) {
        ...
        ScopeExit listen_guard([this]() {
            if (listen_socket_ != nullptr) {
                drake::log()->warn("Meshcat's internal websocket is stopping via an exception");
                Shutdown();
                ...
                us_internal_free_closed_sockets(reinterpret_cast<struct us_loop_t*>(uWS::Loop::get()));
            }
        });
    ...
    void HandleSocketOpen(WebSocket* ws) {
        DRAKE_DEMAND(IsThread(websocket_thread_id_));
        drake::log()->debug("Meshcat connection opened from {}", ws->getRemoteAddressAsText());
    ...
    void HandleSocketClose(WebSocket* ws) {
        DRAKE_DEMAND(IsThread(websocket_thread_id_));
        drake::log()->debug("Meshcat connection closed from {}", ws->getRemoteAddressAsText());
    ...
    void HandleHttpGet(std::string_view url_path, uWS::HttpResponse<kSsl>* response) const {
        DRAKE_DEMAND(IsThread(websocket_thread_id_));
        ...
        if (url_path == "/download") {
            ...
            response->writeHeader("Content-Disposition", "attachment; filename=\"meshcat.html\"");
        ...
        if (url_path.substr(0, 8) == "/cas-v1-") {
            ...
            std::shared_ptr<const FileStorage::Handle> handle = file_storage_.Find(*key);
            if (handle == nullptr) {
                drake::log()->warn("Meshcat: Unknown CAS key {} (there are {} assets in the cache)", suffix,
                                   file_storage_.size());
            ...
            response->writeHeader("Cache-Control", "public, max-age=604800, immutable");
            response->end(handle->content);
            return;
        }
        // Handle static (i.e., compiled-in) files.
        if (const std::optional<std::string_view> content = internal::GetMeshcatStaticResource(url_path)) {
    ...
    void HandleMessage(WebSocket* ws, std::string_view message) {
        ...
        try {
            msgpack::object_handle o_h = msgpack::unpack(message.data(), message.size());
        ...
        if (data.type == "camera_pose" && data.camera_pose.size() == 16 && data.is_perspective.has_value()) {
            ...
            if (*data.is_perspective) {
                camera_pose_ = MakeDrakePoseFromMeshcatPoseForCamera(std::move(data.camera_pose));
            } else {
                camera_pose_ = std::nullopt;
            }
            return;
        ...
        if (inject_message_fault_.load()) {
            throw std::runtime_error("InjectWebsocketThreadFault during message callback");
        }
    }
...
Meshcat::Meshcat(std::optional<int> port) : Meshcat(MeshcatParams{.port = port}) {}

Meshcat::Meshcat(const MeshcatParams& params)
    // The Impl constructor creates the server thread, binds to the port, etc.
    : impl_{new Impl(params)}, recording_{std::make_unique<internal::MeshcatRecording>()} {
...
void Meshcat::SetObject(std::string_view path, const Shape& shape, const Rgba& rgba) {
...
void Meshcat::SetTriangleMesh(std::string_view path,
                              const Eigen::Ref<const Eigen::Matrix3Xd>& vertices,
                              const Eigen::Ref<const Eigen::Matrix3Xi>& faces,
                              const Rgba& rgba,
                              bool wireframe,
                              double wireframe_line_width,
                              SideOfFaceToRender side) {
    impl().SetTriangleMesh(path, vertices, faces, rgba, wireframe, wireframe_line_width, side);
}

void Meshcat::SetTriangleColorMesh(std::string_view path,
                                   const Eigen::Ref<const Eigen::Matrix3Xd>& vertices,
                                   const Eigen::Ref<const Eigen::Matrix3Xi>& faces,
                                   const Eigen::Ref<const Eigen::Matrix3Xd>& colors,
                                   bool wireframe,
                                   double wireframe_line_width,
                                   SideOfFaceToRender side) {
    impl().SetTriangleColorMesh(path, vertices, faces, colors, wireframe, wireframe_line_width, side);
}
...
void Meshcat::PlotSurface(std::string_view path,
    ...
    if (wireframe) {
        ...
        impl().SetTriangleMesh(path, vertices, faces, rgba, wireframe, wireframe_line_width,
...
void Meshcat::SetTransform(std::string_view path,
    ...
    const bool show_live = recording_->SetTransform(path, X_ParentPath, time_in_recording);
    if (show_live) {
        impl().SetTransform(path, X_ParentPath);
    }
}

void Meshcat::SetTransform(std::string_view path, const Eigen::Ref<const Eigen::Matrix4d>& matrix) {
...
void Meshcat::SetProperty(std::string_view path,
                          std::string property,
                          bool value,
                          std::optional<double> time_in_recording) {
    const bool show_live = recording_->SetProperty(path, property, value, time_in_recording);
    if (show_live) {
...
void Meshcat::Set2dRenderMode(const math::RigidTransformd& X_WC, double xmin, double xmax, double ymin, double ymax) {
...
void Meshcat::SetCameraPose(const Eigen::Vector3d& camera_in_world, const Eigen::Vector3d& target_in_world) {
...
void Meshcat::AddSlider(std::string name,
                        double min,
                        double max,
                        double step,
                        double value,
                        std::string decrement_keycode,
                        std::string increment_keycode) {
    impl().AddSlider(std::move(name), min, max, step, value, std::move(decrement_keycode),
...
void Meshcat::StartRecording(double frames_per_second, bool set_visualizations_while_recording) {
    recording_->StartRecording(frames_per_second, set_visualizations_while_recording);
}
...
std::string Meshcat::GetPackedProperty(std::string_view path, std::string property) const {
