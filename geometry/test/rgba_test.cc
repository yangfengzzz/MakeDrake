...
GTEST_TEST(RgbaTest, Basic) {
    ...
    const Rgba color_delta(color.r() + kEps, color.g() - kEps, color.b() + kEps, color.a() - kEps);
...
GTEST_TEST(RgbaTest, Serialization) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(yaml::LoadYamlString<Rgba>("rgba: [1, 1, 1, 1, 1]"), ".*maximum size is 4.*");

    // Too-small YAML arrays are detected by our Serialize() function.
    DRAKE_EXPECT_THROWS_MESSAGE(yaml::LoadYamlString<Rgba>("rgba: [1, 1]"), "Rgba must contain either 3 or 4.+");

    // Out-of-range YAML values are detected by our Serialize() function.
    DRAKE_EXPECT_THROWS_MESSAGE(yaml::LoadYamlString<Rgba>("rgba: [1, 1, 2]"), ".*must be within the range.+");
}
