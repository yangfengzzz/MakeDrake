...
#include "geometry/meshcat_animation.h"
#include "geometry/meshcat_visualizer.h"
#include "geometry/rgba.h"
#include "geometry/shape_specification.h"
#include "math/rigid_transform.h"
#include "math/rotation_matrix.h"
#include "multibody/meshcat/contact_visualizer.h"
...
int do_main() {
    ...
    meshcat->SetProperty("sphere/scoped_name/<object>", "material.transparent", true);
    ...
    meshcat->SetObject("ellipsoid", Ellipsoid(0.25, 0.25, 0.5), Rgba(1.0, 0, 1, 0.5));
    ...
    const std::string polytope_with_hole =
            FindResourceOrThrow("drake/examples/scene_graph/cuboctahedron_with_hole.obj");
    meshcat->SetObject("obj_as_convex", Convex(polytope_with_hole, 0.25), Rgba(0.8, 0.4, 0.1, 1.0));
    meshcat->SetTransform("obj_as_convex", RigidTransformd(Vector3d{++x, 0, 0}));

    meshcat->SetObject("obj_as_mesh", Mesh(polytope_with_hole, 0.25), Rgba(0.8, 0.4, 0.1, 1.0));
    ...
    auto mustard_obj = FindRunfile("drake_models/ycb/meshes/006_mustard_bottle_textured.obj").abspath;
...

    {
        ...
        perception::PointCloud cloud(kPoints, perception::pc_flags::kXYZs | perception::pc_flags::kRGBs);
    ...
    start << -0.1, -0.1,  0.1,  0.1,
        ...
        meshcat->SetTransform("line_segments", RigidTransformd(Vector3d{++x, 0, -0.5}));
    }
    ...
    // visually inspecting the results of TriangleSurfaceMesh is sufficient here.
    {
        ...
        const Vector3d vertex_data[4] = {{0, 0, 0}, {0.5, 0, 0}, {0.5, 0.5, 0}, {0, 0.5, 0.5}};
        ...
        meshcat->SetTransform("triangle_mesh", RigidTransformd(Vector3d{++x, -0.25, 0}));

        meshcat->SetObject("triangle_mesh_wireframe", surface_mesh, Rgba(0.9, 0, 0.9, 1.0), true, 5.0);
        meshcat->SetTransform("triangle_mesh_wireframe", RigidTransformd(Vector3d{++x, -0.25, 0}));
    }
    ...
    colors <<
      ...
      0, 0, 1, 0;
        // clang-format on
        meshcat->SetTriangleColorMesh("triangle_color_mesh", vertices, faces, colors);
        meshcat->SetTransform("triangle_color_mesh", RigidTransformd(Vector3d{++x, -0.25, 0}));
    }

    // PlotSurface.
    {
        constexpr int nx = 15, ny = 11;
        Eigen::MatrixXd X = Eigen::RowVectorXd::LinSpaced(nx, 0, 1).replicate<ny, 1>();
        ...
        meshcat->SetTransform("plot_surface", RigidTransformd(Vector3d{++x, -0.25, 0}));
    }

    std::cout << "\nDo *not* open up your browser to the URL above. Instead use " << "the following URL\n\n"
    ...
    if (meshcat->GetTrackedCameraPose() == std::nullopt) {
        ...
        if (meshcat->GetTrackedCameraPose() == std::nullopt) {
            std::cout << "  !!! ERROR !!! It appears that camera tracking isn't " << "working!\n";
            return 1;
        } else {
...
- From left to right along the x axis, you should see:
  ...
  - a blue mesh plot of the function z = y*sin(5*x).
    ...
    std::cout << "\nAnimations:\n";
    meshcat->SetCameraPose(animation_target + Vector3d{0, -3, 1.5}, animation_target);
    ...
    animation.SetTransform(20, "sphere/scoped_name", RigidTransformd(sphere_home + Vector3d::UnitZ()));
    animation.SetTransform(40, "sphere/scoped_name", RigidTransformd(sphere_home));

    std::cout << "- the blue box should spin clockwise about the +z axis.\n";
    animation.SetTransform(0, "box", RigidTransformd(RotationMatrixd::MakeZRotation(0), box_home));
    animation.SetTransform(20, "box", RigidTransformd(RotationMatrixd::MakeZRotation(M_PI), box_home));
    animation.SetTransform(40, "box", RigidTransformd(RotationMatrixd::MakeZRotation(2 * M_PI), box_home));
    ...
    std::cout << "- the pink ellipsoid should get less and then more transparent.\n";
    ...
    meshcat->Set2dRenderMode(math::RigidTransform(Vector3d{0, -3, 0}), -4, 4, -2, 2);

    std::cout << "- The scene should have switched to 2D rendering mode.\n";
    MaybePauseForUser();

    meshcat->Set2dRenderMode(math::RigidTransform(math::RotationMatrixd::MakeZRotation(-M_PI / 2.0), sphere_home), -2,
                             2, -2, 2);
    ...
    std::cout << "- The blue box should have disappeared\n"
              << "- The lights should have dimmed.\n"
              << "- The background should have been disabled (it will appear white)" << std::endl;
    MaybePauseForUser();

    meshcat->SetCameraTarget(gltf_pose);
    meshcat->SetProperty("/Background", "visible", true);
    meshcat->SetEnvironmentMap(FindResourceOrThrow("drake/geometry/test/env_256_cornell_box.png"));

    std::cout << "- An environment map has been loaded from a png -- the Cornell " << "box.\n"
              << "  The dented green box should reflect it (the camera has moved "
              << "to focus on the box). This may not be apparent until after you " << "move the mouse.\n";
    MaybePauseForUser();

    meshcat->SetEnvironmentMap(FindResourceOrThrow("drake/geometry/test/env_256_brick_room.jpg"));

    std::cout << "- The Cornell box has been replaced by a room with brick walls "
              << "loaded from a jpg. Again, the change may not be apparent " << "until after you move the mouse.\n";
    MaybePauseForUser();
...
- Reloading the page should always succeed. Force a complete reload now using
  ...
  walls, etc.
...

    {
        ...
        const std::string hydro_sdf = FindResourceOrThrow("drake/multibody/meshcat/test/hydroelastic.sdf");
        parser.AddModels(hydro_sdf);
        const auto& body1 = plant.GetBodyByName("body1");
        plant.AddJoint<multibody::PrismaticJoint>("body1", plant.world_body(), std::nullopt, body1, std::nullopt,
                                                  Vector3d::UnitZ());
        const auto& body2 = plant.GetBodyByName("body2");
        plant.AddJoint<multibody::PrismaticJoint>("body2", plant.world_body(), std::nullopt, body2, std::nullopt,
                                                  Vector3d::UnitX());
        plant.Finalize();

        MeshcatVisualizerParams params;
        params.delete_on_initialization_event = false;
        auto& visualizer = MeshcatVisualizerd::AddToBuilder(&builder, scene_graph, meshcat, std::move(params));

        multibody::meshcat::ContactVisualizerParams cparams;
        cparams.newtons_per_meter = 60.0;
        auto& contact =
                multibody::meshcat::ContactVisualizerd::AddToBuilder(&builder, plant, meshcat, std::move(cparams));

        auto diagram = builder.Build();
        auto context = diagram->CreateDefaultContext();

        plant.SetPositions(&plant.GetMyMutableContextFromRoot(context.get()), Eigen::Vector2d{0.1, 0.3});
        diagram->ForcedPublish(*context);
        meshcat->SetCameraPose(Vector3d{0, -1.5, 1}, Vector3d{0.25, 0, 0});
        std::cout << "- Now you should see three colliding hydroelastic spheres." << std::endl;
        MaybePauseForUser();
...

    {
        ...
        plant.WeldFrames(plant.world_frame(), plant.GetFrameByName("base_link_mustard"),
        ...
        diagram->ForcedPublish(*context);
        std::cout << "- Now you should see a kuka model (from MultibodyPlant/SceneGraph) "
    ...
    std::cout << "Now we'll check the standalone HTML file capturing this scene.\n"
    ...
    std::cout << "Got " << meshcat->GetButtonClicks("ButtonTest") << " clicks on ButtonTest.\n"
              << "Got " << meshcat->GetButtonClicks("Press t Key") << " clicks on \"Press t Key\".\n"
              << "Got " << meshcat->GetSliderValue("SliderTest") << " value for SliderTest.\n\n"
              << std::endl;

    std::cout << "Next, we'll test gamepad (i.e., joystick) features.\n\n";
    std::cout << "While the Meshcat browser window has focus, click any button on "
              << "your gamepad to activate gamepad support in the browser.\n\n";
    std::cout << "Then(after you press RETURN), we'll print the gamepad stats for 5 "
              ...
              << "sufficient to consider the test passing; the exact values do not " << "matter.\n";
