...
GTEST_TEST(KinematicsVector, InitializerListCtor) {
    ...
    const RigidTransformd pose_1 = math::RigidTransformd{Eigen::Translation3d(0.1, 0.2, 0.3)};
    ...
    EXPECT_TRUE(CompareMatrices(dut.value(id_1).GetAsMatrix34(), pose_1.GetAsMatrix34()));
}
...
GTEST_TEST(KinematicsVector, WorkingWithValues) {
    ...
    for (int i = 0; i < kPoseCount; ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(pose.GetAsMatrix34(), recorded_poses[i].GetAsMatrix34()));
    }
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(poses.value(FrameId::get_new_id()), "No such FrameId.*\\d.*");
    // Tests the GeometryConfigurationVector counterpart throws a similar but
    // distinct message.
    GeometryConfigurationVector<double> configurations;
    DRAKE_EXPECT_THROWS_MESSAGE(configurations.value(GeometryId::get_new_id()), "No such GeometryId.*\\d.*");
}
...
GTEST_TEST(KinematicsVector, FramePoseVectorAutoDiffInstantiation) {
    ...
    poses.set_value(FrameId::get_new_id(), RigidTransform<AutoDiffXd>::Identity());
...
GTEST_TEST(KinematicsVector, FramePoseVectorSymbolicInstantiation) {
    ...
    const RigidTransform<Expression> pose = RigidTransform<Expression>(Eigen::Translation<Expression, 3>(x_, y_, z_));
...
GTEST_TEST(KinematicsVector, GeometryConfigurationVector) {
    ...
    configurations_double.set_value(GeometryId::get_new_id(), VectorX<double>::Zero(7));
    EXPECT_EQ(configurations_double.size(), 1);

    GeometryConfigurationVector<AutoDiffXd> configurations_autodiff;
    configurations_autodiff.set_value(GeometryId::get_new_id(), VectorX<AutoDiffXd>::Zero(7));
    ...
    configurations_expression.set_value(GeometryId::get_new_id(), VectorX<Expression>::Constant(7, x));
