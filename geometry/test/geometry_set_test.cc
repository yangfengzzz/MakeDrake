...
void ExpectContainsAll(const GeometrySetTester& set_tester, const IdContainer& ids, const char* failure_message) {
    for (auto id : ids) {
...
GTEST_TEST(GeometrySetTests, ConversionConstructor) {
    ...
    std::set<GeometryId> geometry_set{GeometryId::get_new_id(), GeometryId::get_new_id(), GeometryId::get_new_id()};
    std::unordered_set<GeometryId> geometry_hash{GeometryId::get_new_id(), GeometryId::get_new_id(),
    ...
    auto test_single_source_constructor = [](const GeometrySet& set, const auto& source, int expected_frame_count,
    ...
    test_single_source_constructor(set1, std::set<GeometryId>{g_id}, 0, 1, "Single geometry id constructor");

    GeometrySet set2(f_id);
    test_single_source_constructor(set2, std::set<FrameId>{f_id}, 1, 0, "Single frame id constructor");

    GeometrySet set3(geometry_vector);
    test_single_source_constructor(set3, geometry_vector, 0, static_cast<int>(geometry_vector.size()),
                                   "Vector constructor");

    GeometrySet set4(geometry_set);
    test_single_source_constructor(set4, geometry_set, 0, static_cast<int>(geometry_set.size()), "Set constructor");

    GeometrySet set5(geometry_hash);
    test_single_source_constructor(set5, geometry_hash, 0, static_cast<int>(geometry_hash.size()),
    ...
    test_single_source_constructor(set6, geometry_list, 0, static_cast<int>(geometry_vector.size()),
                                   "GeometryId initializer list lvalue constructor");

    GeometrySet set7({geometry_vector[0], geometry_vector[1]});
    test_single_source_constructor(set7, geometry_vector, 0, static_cast<int>(geometry_vector.size()),
                                   "GeometryId initializer list rvalue constructor");

    GeometrySet set8(frame_list);
    test_single_source_constructor(set8, frame_vector, static_cast<int>(frame_vector.size()), 0,
                                   "FrameId initializer list lvalue constructor");

    GeometrySet set9({frame_vector[0], frame_vector[1]});
    test_single_source_constructor(set9, frame_vector, static_cast<int>(frame_vector.size()), 0,
    ...
    EXPECT_EQ(tester10.num_geometries(), static_cast<int>(geometry_vector.size()));
    ExpectContainsAll(tester10, geometry_vector, "Geometry initializer list, frame vector constructor");
    ExpectContainsAll(tester10, frame_vector, "Geometry initializer list, frame vector constructor");
    ...
    EXPECT_EQ(tester11.num_geometries(), static_cast<int>(geometry_vector.size()));
    ExpectContainsAll(tester11, geometry_vector, "Geometry vector, frame initializer list constructor");
    ExpectContainsAll(tester11, frame_vector, "Geometry vector, frame initializer list constructor");
    ...
    EXPECT_EQ(tester12.num_geometries(), static_cast<int>(geometry_vector.size()));
...
GTEST_TEST(GeometrySetTests, IterableFrameAdd) {
    ...
    std::vector<FrameId> frames_vector{FrameId::get_new_id(), FrameId::get_new_id()};
    std::set<FrameId> frames_set{FrameId::get_new_id(), FrameId::get_new_id(), FrameId::get_new_id()};
    std::unordered_set<FrameId> frames_hash{FrameId::get_new_id(), FrameId::get_new_id(), FrameId::get_new_id(),
                                            FrameId::get_new_id()};
    ...
    ExpectContainsAll(tester2, frames_vector, "IterableFrameAdd - initializer_list");
}
...
GTEST_TEST(GeometrySetTests, IterableGeometryAdd) {
    ...
    std::vector<GeometryId> geometry_id_vector{GeometryId::get_new_id(), GeometryId::get_new_id()};
    std::set<GeometryId> geometry_id_set{GeometryId::get_new_id(), GeometryId::get_new_id(), GeometryId::get_new_id()};
    std::unordered_set<GeometryId> geometry_id_hash{GeometryId::get_new_id(), GeometryId::get_new_id(),
                                                    GeometryId::get_new_id(), GeometryId::get_new_id()};
    ...
    ExpectContainsAll(tester, geometry_id_vector, "IterableGeometryAdd - geometry_id_vector");

    geometry_set.Add(geometry_id_set);
    expected_num_geometries += static_cast<int>(geometry_id_set.size());
    EXPECT_EQ(tester.num_geometries(), expected_num_geometries);
    ExpectContainsAll(tester, geometry_id_set, "IterableGeometryAdd - geometry_id_set");

    geometry_set.Add(geometry_id_hash);
    expected_num_geometries += static_cast<int>(geometry_id_hash.size());
    EXPECT_EQ(tester.num_geometries(), expected_num_geometries);
    ExpectContainsAll(tester, geometry_id_hash, "IterableGeometryAdd - geometry_id_hash");

    GeometrySet geometry_set2;
    GeometrySetTester tester2(&geometry_set2);
    geometry_set2.Add({geometry_id_vector[0], geometry_id_vector[1]});
    EXPECT_EQ(tester2.num_geometries(), 2);
    ExpectContainsAll(tester2, geometry_id_vector, "IterableGeometryAdd - initializer_list");
}
...
GTEST_TEST(GeometrySetTests, IterableGeometryAndFrames) {
    ...
    EXPECT_EQ(tester.num_geometries(), static_cast<int>(geometry_id_vector.size()));
    ExpectContainsAll(tester, geometry_id_vector,
                      "IterableGeometryAndFrames - both containers - "
                      "geometry_id_vector");
    ExpectContainsAll(tester, frame_id_set, "IterableGeometryAndFrames - both containers - frame_id_set");

    GeometrySet geometry_set2;
    GeometrySetTester tester2(&geometry_set2);
    geometry_set2.Add({geometry_id_vector[0], geometry_id_vector[1]}, frame_id_set);
    EXPECT_EQ(tester2.num_frames(), static_cast<int>(frame_id_set.size()));
    EXPECT_EQ(tester2.num_geometries(), static_cast<int>(geometry_id_vector.size()));
    ExpectContainsAll(tester2, geometry_id_vector, "IterableGeometryAndFrames - geometry initializer list - geometry");
    ...
    geometry_set3.Add(geometry_id_vector, {*frame_id_set.begin(), *(++frame_id_set.begin())});
    EXPECT_EQ(tester3.num_frames(), static_cast<int>(frame_id_set.size()));
    EXPECT_EQ(tester3.num_geometries(), static_cast<int>(geometry_id_vector.size()));
...
GTEST_TEST(GeometrySetTests, GeometrySetAdd) {
    ...
    std::vector<GeometryId> geometry_ids{GeometryId::get_new_id(), GeometryId::get_new_id()};
    ...
    ExpectContainsAll(tester1, frame_ids, "GeometrySetAdd - set1 initial contents - frames");
    ExpectContainsAll(tester1, geometry_ids, "GeometrySetAdd - set1 initial contents - geometries");
    ...
    std::vector<GeometryId> geometry_ids_2{*geometry_ids.begin(), GeometryId::get_new_id()};
    ...
    ExpectContainsAll(tester2, frame_ids_2, "GeometrySetAdd - set2 initial contents - frames");
    ExpectContainsAll(tester2, geometry_ids_2, "GeometrySetAdd - set2 initial contents - geometries");
    ...
    ExpectContainsAll(tester1, frame_union, "GeometrySetAdd - set1 merged - frames");
    ExpectContainsAll(tester1, geometry_union, "GeometrySetAdd - set1 merged - geometries");
}
