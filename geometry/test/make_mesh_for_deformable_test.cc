...
GTEST_TEST(MakeMeshForDeformableTest, Sphere) {
    ...
    std::unique_ptr<VolumeMesh<double>> mesh = MakeMeshForDeformable(sphere, kRezHint);
...
GTEST_TEST(MakeMeshForDeformableTest, Box) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeMeshForDeformable(b, kRezHint), ".*don't yet generate deformable meshes.+ Box.*");
}
...
GTEST_TEST(MakeMeshForDeformableTest, Mesh) {
    const Mesh m(FindResourceOrThrow("drake/geometry/test/one_tetrahedron.vtk"), 1.5);
