...
public:
    ...
    static const vector<Statement>& statements(const CollisionFilterDeclaration& decl) { return decl.statements(); }
};

class GeometrySetTester {
public:
    static bool AreEqual(const GeometrySet& set_A, const GeometrySet& set_B) {
        return set_A.geometry_ids_ == set_B.geometry_ids_ && set_A.frame_ids_ == set_B.frame_ids_;
    }
};
...
GTEST_TEST(CollisionFilterDeclTest, AllowWithin) {
    ...
    for (const auto& geo_set : {set_A, set_B}) {
        ...
        EXPECT_TRUE(GeometrySetTester::AreEqual(statements[0].set_B, GeometrySet()));
    }
}
...
GTEST_TEST(CollisionFilterDeclTest, StatementOrder) {
    ...
    auto validate = [&validate_base](const Tester::Statement expected, const CollisionFilterDeclaration& declaration) {
