...
protected:
    ...
    void VerifySameRole(const GeometryVersion& v1, const GeometryVersion& v2, Role role) const {
        EXPECT_TRUE(v1.IsSameAs(v2, role));
        // Verify commutativity.
        EXPECT_TRUE(v2.IsSameAs(v1, role));
    }

    void VerifyDistinctRole(const GeometryVersion& v1, const GeometryVersion& v2, Role role) const {
        EXPECT_FALSE(v1.IsSameAs(v2, role));
        // Verify commutativity.
        EXPECT_FALSE(v2.IsSameAs(v1, role));
    }

    void VerifyIdenticalVersions(const GeometryVersion& v1, const GeometryVersion& v2) const {
        VerifySameRole(v1, v2, Role::kProximity);
        VerifySameRole(v1, v2, Role::kPerception);
        VerifySameRole(v1, v2, Role::kIllustration);
    }

    void VerifyDistinctVersions(const GeometryVersion& v1, const GeometryVersion& v2) const {
...
TEST_F(GeometryVersionTest, Unassigned) {
    GeometryVersion old_version = version_;
    const auto& new_version = version_;
    EXPECT_THROW(old_version.IsSameAs(new_version, Role::kUnassigned), std::logic_error);
}
