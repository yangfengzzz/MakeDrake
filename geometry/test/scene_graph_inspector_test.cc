...
GTEST_TEST(SceneGraphInspector, ExerciseEverything) {
    ...
    const FrameId frame_id = tester.mutable_state().RegisterFrame(source_id, GeometryFrame("frame"));
    ...
    const GeometryId deformable_geometry_id = tester.mutable_state().RegisterDeformableGeometry(
            source_id, internal::InternalFrame::world_frame_id(), std::move(deformable_instance), 0.5);
    ...
    const GeometryId geometry_id2 = tester.mutable_state().RegisterGeometry(
            source_id, frame_id,
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "sphere2"));
    tester.mutable_state().AssignRole(source_id, geometry_id, ProximityProperties());
    tester.mutable_state().AssignRole(source_id, geometry_id2, ProximityProperties());
...
GTEST_TEST(SceneGraphInspector, CloneGeometryInstance) {
    ...
    const GeometryInstance original(RigidTransformd(Eigen::Vector3d(1, 2, 3)), Sphere(1.5), "test_sphere");
    const GeometryId geometry_id =
            tester.mutable_state().RegisterGeometry(source_id, frame_id, make_unique<GeometryInstance>(original));

    // Confirm basic geometry parameters (name, id, etc.) and that if the source
    // doesn't have a role, the clone doesn't either.
    {
        std::unique_ptr<GeometryInstance> clone = inspector.CloneGeometryInstance(geometry_id);
    ...
    // Now confirm that assigned roles (via their properties) propagate.
    {
        ...
        EXPECT_TRUE(clone->illustration_properties()->HasProperty("illus", "value"));
        ...
        EXPECT_NE(clone->illustration_properties(), inspector.GetIllustrationProperties(geometry_id));
        EXPECT_NE(clone->perception_properties(), inspector.GetPerceptionProperties(geometry_id));
        EXPECT_NE(clone->proximity_properties(), inspector.GetProximityProperties(geometry_id));
    }
}
...
GTEST_TEST(SceneGraphInspector, AllFrameIds) {
    ...
    const FrameId frame_id_1 = tester.mutable_state().RegisterFrame(source_id, GeometryFrame("frame1"));
    const FrameId frame_id_2 = tester.mutable_state().RegisterFrame(source_id, GeometryFrame("frame2"));
    const FrameId frame_id_3 = tester.mutable_state().RegisterFrame(source_id, GeometryFrame("frame3"));

    // We expect the results to be nicely, consistently ordered.
    std::vector<FrameId> expected_ids{frame_id_1, frame_id_2, frame_id_3, world_id};
