...
::testing::AssertionResult ValidatePlanePose(const RigidTransformd& pose,
                                             const Vector3<double>& expected_z,
                                             const Vector3<double>& expected_translation,
                                             double tolerance = 1e-14) {
    using std::abs;

    // Test expected z-axis value.
    const Vector3<double>& z_axis = pose.rotation().col(2);
    if (!CompareMatrices(z_axis, expected_z, tolerance, MatrixCompareType::absolute)) {
        const std::string message = fmt::format("pose =\n{}\nExpected z-axis {} does not match pose's z-axis {}",
    ...
    if (!CompareMatrices(pose.translation(), expected_translation, tolerance, MatrixCompareType::absolute)) {
        const std::string message = fmt::format(
                "pose =\n{}\nExpected translation {} does not match pose's "
                "translation {}",
                fmt_eigen(pose.GetAsMatrix34()), fmt_eigen(expected_translation.transpose()),
    ...
    for (int i = 0; i < 3; ++i) {
        if (abs(pose.rotation().col(i).norm() - 1) > tolerance) {
            const std::string message =
                    fmt::format("pose =\n{}\ndoes not have unit length {}-axis {}", fmt_eigen(pose.GetAsMatrix34()),
                                axis_labels[i], fmt_eigen(pose.rotation().col(i).transpose()));
            return ::testing::AssertionFailure() << message;
        }
    }
...
GTEST_TEST(ShapeTest, NumericalValidation) {
    DRAKE_EXPECT_THROWS_MESSAGE(Box(2, 0, 2), "Box width, depth, and height should all be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Box(3, 1, -1), "Box width, depth, and height should all be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Box(Vector3<double>{3, 1, -1}), "Box width, depth, and height should all be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Box::MakeCube(0),
                                "Box width, depth, and height should "
                                "all be > 0.+");

    DRAKE_EXPECT_THROWS_MESSAGE(Capsule(0, 1), "Capsule radius and length should both be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Capsule(0.5, -1), "Capsule radius and length should both be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Capsule(Vector2<double>{0.5, -1}), "Capsule radius and length should both be > 0.+");

    DRAKE_EXPECT_THROWS_MESSAGE(Convex("bar", 0), "Convex .scale. cannot be < 1e-8.");
    DRAKE_EXPECT_NO_THROW(Convex("foo", -1));  // Special case for negative scale.

    DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(0, 1), "Cylinder radius and length should both be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(0.5, -1), "Cylinder radius and length should both be > 0.+");
    DRAKE_EXPECT_THROWS_MESSAGE(Cylinder(Vector2<double>{0.5, -1}), "Cylinder radius and length should both be > 0.+");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Mesh("foo", 1e-9), "Mesh .scale. cannot be < 1e-8.");
    DRAKE_EXPECT_NO_THROW(Mesh("foo", -1));  // Special case for negative scale.

    DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(0, 1, 1), "MeshcatCone parameters .+ should all be > 0.*");
    DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 0, 1), "MeshcatCone parameters .+ should all be > 0.*");
    DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(1, 1, 0), "MeshcatCone parameters .+ should all be > 0.*");
    DRAKE_EXPECT_THROWS_MESSAGE(MeshcatCone(Vector3<double>{1, 1, 0}), "MeshcatCone parameters .+ should all be > 0.*");
...
GTEST_TEST(ShapeTest, ConvexHull) {
    const std::string cube_path = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");

    auto expect_convex_hull = [](const auto& mesh_like) {
        SCOPED_TRACE(fmt::format("Testing convex hull for {}.", mesh_like.type_name()));
...
TEST_F(DefaultReifierTest, UnsupportedGeometry) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(this->ImplementGeometry(Ellipsoid(1, 1, 1), nullptr),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(this->ImplementGeometry(MeshcatCone(1, 1, 1), nullptr),
...
class OverrideDefaultGeometryTest : public ShapeReifier, public ::testing::Test {
public:
...
TEST_F(OverrideDefaultGeometryTest, UnsupportedGeometry) {
    ...
    // throwing method.
    DRAKE_EXPECT_THROWS_MESSAGE(this->ThrowUnsupportedGeometry("Foo"), "This class (.+) does not support Foo.");
...
GTEST_TEST(ShapeTest, Volume) {
    ...
    EXPECT_NEAR(CalcVolume(Capsule(1.23, 2.4)), CalcVolume(Cylinder(1.23, 2.4)) + CalcVolume(Sphere(1.23)), 1e-14);
    ...
    const std::string cube_obj = FindResourceOrThrow("drake/geometry/test/quad_cube.obj");
    EXPECT_NEAR(CalcVolume(Convex(cube_obj, 1.0)), 8.0, 1e-14);
    EXPECT_NEAR(CalcVolume(Mesh(cube_obj, 1.0)), 8.0, 1e-14);

    DRAKE_EXPECT_THROWS_MESSAGE(CalcVolume(Convex("fakename.obj", 1.0)), "Cannot open file.*");
    DRAKE_EXPECT_THROWS_MESSAGE(CalcVolume(Mesh("fakename.obj", 1.0)), "Cannot open file.*");

    // We only support obj but should eventually support vtk.
    const std::string non_obj = "only_extension_matters.not_obj";
    DRAKE_EXPECT_THROWS_MESSAGE(CalcVolume(Convex(non_obj, 1.0)), ".*only supports .obj files.*");
    DRAKE_EXPECT_THROWS_MESSAGE(CalcVolume(Mesh(non_obj, 1.0)), ".*only supports .obj files.*");
}
...
GTEST_TEST(ShapeTest, Pathname) {
    ...
    EXPECT_EQ(relpath_mesh.filename(), std::filesystem::current_path() / "relative_path.obj");

    const Convex relpath_convex("relative_path.obj");
    EXPECT_TRUE(std::filesystem::path(relpath_convex.filename()).is_absolute());
    EXPECT_EQ(relpath_convex.filename(), std::filesystem::current_path() / "relative_path.obj");
}
