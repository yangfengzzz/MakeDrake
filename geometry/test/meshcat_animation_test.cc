...
GTEST_TEST(MeshcatAnimationTest, SetTransformTest) {
    ...
    EXPECT_FALSE(animation.get_key_frame<std::vector<double>>(kFrame, "test", "position"));
    EXPECT_FALSE(animation.get_key_frame<std::vector<double>>(kFrame, "test", "quaternion"));
    ...
    std::optional<std::vector<double>> p = animation.get_key_frame<std::vector<double>>(kFrame, "test", "position");
    EXPECT_TRUE(p);
    EXPECT_TRUE(CompareMatrices(position, Vector3d::Map(&(*p)[0])));
    std::optional<std::vector<double>> q = animation.get_key_frame<std::vector<double>>(kFrame, "test", "quaternion");
    EXPECT_TRUE(q);
...
GTEST_TEST(MeshcatAnimationTest, SetPropertyTest) {
    ...
    std::optional<bool> visible = animation.get_key_frame<bool>(kFrame, "bool_test", "visible");
    EXPECT_TRUE(visible);
    EXPECT_TRUE(*visible);
    EXPECT_EQ(animation.get_javascript_type("bool_test", "visible"), "boolean");

    // Double
    EXPECT_FALSE(animation.get_key_frame<double>(kFrame, "double_test", "material.opacity"));
    animation.SetProperty(kFrame, "double_test", "material.opacity", 0.5);
    std::optional<double> opacity = animation.get_key_frame<double>(kFrame, "double_test", "material.opacity");
    EXPECT_TRUE(opacity);
    EXPECT_EQ(*opacity, 0.5);
    EXPECT_EQ(animation.get_javascript_type("double_test", "material.opacity"), "number");

    // Double vector
    EXPECT_FALSE(animation.get_key_frame<std::vector<double>>(kFrame, "vector_test", "position"));
    animation.SetProperty(kFrame, "vector_test", "position", {0.1, 0.2});
    std::optional<std::vector<double>> vec =
            animation.get_key_frame<std::vector<double>>(kFrame, "vector_test", "position");
    EXPECT_TRUE(vec);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(animation.SetProperty(kFrame, "bool_test", "visible", 32.0), ".*already has a track.*");
    // ... even at a different frame.
    DRAKE_EXPECT_THROWS_MESSAGE(animation.SetProperty(kFrame + 1, "bool_test", "visible", 32.0),
                                ".*already has a track.*");
}
