...
GTEST_TEST(GeometryProperties, AddProperty) {
    ...
    DRAKE_EXPECT_NO_THROW(properties.AddProperty(group_name, prop_name, int_value));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            ...
            fmt::format(".*Trying to add property \\('{}', '{}'\\).+ name already exists", group_name, prop_name));
    ASSERT_TRUE(properties.HasProperty(group_name, prop_name));
}
...
GTEST_TEST(GeometryProperties, UpdateProperty) {
    ...
    EXPECT_NO_THROW(properties.UpdateProperty(group_name, prop_name2, "from_update"));
    EXPECT_EQ(properties.GetProperty<string>(group_name, prop_name2), "from_update");

    // UpdateProperty alias works for changing value (with same type).
    EXPECT_NO_THROW(properties.UpdateProperty(group_name, prop_name, int_value + 10));
    EXPECT_EQ(properties.GetProperty<int>(group_name, prop_name), int_value + 10);

    // UpdateProperty alias fails for changing type.
    DRAKE_EXPECT_THROWS_MESSAGE(properties.UpdateProperty(group_name, prop_name, 17.0),
...
GTEST_TEST(GeometryProperties, AddPropertyStruct) {
    ...
    TestData data{1, 2., "3"};
    DRAKE_EXPECT_NO_THROW(properties.AddProperty(TestProperties::default_group_name(), prop_name, data));

    const TestData& read = properties.GetProperty<TestData>(TestProperties::default_group_name(), prop_name);
...
GTEST_TEST(GeometryProperties, GetPropertyOrDefault) {
    ...
    const string prop_name("some_property");
    DRAKE_EXPECT_NO_THROW(properties.AddProperty(group_name, prop_name, double_value));

    // Case: default value that can be *implicitly* converted to the desired
    // type requires explicit template declaration.
    DRAKE_EXPECT_THROWS_MESSAGE(properties.GetPropertyOrDefault(group_name, prop_name, 3),
                                fmt::format(".*The property \\('{}', '{}'\\) exists, but is of a different type. "
                                            "Requested 'int', but found 'double'",
                                            group_name, prop_name));
    DRAKE_EXPECT_NO_THROW(properties.GetPropertyOrDefault<double>(group_name, prop_name, 3));

    // Case: read an existing property.
    int read_value = properties.GetPropertyOrDefault(group_name, prop_name, default_value);
    EXPECT_EQ(double_value, read_value);

    // Case: read from valid group, but invalid property.
    read_value = properties.GetPropertyOrDefault(group_name, "invalid_prop", default_value);
    EXPECT_EQ(default_value, read_value);

    // Case: read from invalid group.
    read_value = properties.GetPropertyOrDefault("invalid_group", "invalid_prop", default_value);
    EXPECT_EQ(default_value, read_value);

    // Case: Property exists of different type.
    DRAKE_EXPECT_THROWS_MESSAGE(properties.GetPropertyOrDefault(group_name, prop_name, "test"),
    ...
    string valid_value = properties.GetPropertyOrDefault("strings", "valid_string", "missing");
    EXPECT_EQ("valid_string", valid_value);
    string default_value_return = properties.GetPropertyOrDefault("strings", "invalid_string", "rvalue_string");
...
GTEST_TEST(GeometryProperties, GetPropertyFailure) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(properties.GetProperty<int>(group_name, prop_name),
                                fmt::format(".*Trying to read property \\('{}', '{}'\\), but the group does not "
                                            "exist.",
                                            group_name, prop_name));
...
GTEST_TEST(GeometryProperties, PropertyIteration) {
    ...
    for (const auto& pair : properties.GetPropertiesInGroup(default_group)) {
        ...
        EXPECT_EQ(reference[name], properties.GetProperty<int>(default_group, name));
...
GTEST_TEST(GeometryProperties, CopyMoveSemantics) {
    ...
    auto properties_equal = [](const TestProperties& reference,
                               const TestProperties& test) -> ::testing::AssertionResult {
        if (reference.num_groups() != test.num_groups()) {
            return ::testing::AssertionFailure() << "Different number of groups. Expected " << reference.num_groups()
                                                 << " found " << test.num_groups();
        }
                    ...
                    if (test.HasProperty(group_name, name)) {
                        ...
                        if (expected_value != test_value) {
                            return ::testing::AssertionFailure()
                                   << "Expected value for '" << group_name << "':'" << name << "' to be "
                                   << expected_value << ". Found " << test_value;
                        }
                    } else {
                        return ::testing::AssertionFailure()
                               << "Expected group '" << group_name << "' to have property '" << name
                               << "'. It does not exist.";
                    }
                }
            } else {
                return ::testing::AssertionFailure()
                       << "Expected group '" << group_name << "' is missing from test properties";
            }
        }
    ...
    struct Stats {
        ...
        ::testing::AssertionResult Equal(Stats other) {
            if (num_copies != other.num_copies || num_moves != other.num_moves) {
                return ::testing::AssertionFailure()
                       << fmt::format("(num_copies, num_moves): ({}, {}) != ({}, {})", num_copies, num_moves,
                                      other.num_copies, other.num_moves);
            }
            return ::testing::AssertionSuccess();
        }
    };
...
GTEST_TEST(GeometryProperties, RgbaAndVector4) {
    ...
    EXPECT_EQ(vector, properties.GetPropertyOrDefault<Vector4d>(group_name, fake_name, vector));
