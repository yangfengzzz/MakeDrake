...
public:
    ...
    ::testing::AssertionResult MatchesQ(const VectorX<T>& q) const {
        const VectorX<T> values =
                Eigen::Map<const VectorX<T>>(reinterpret_cast<const T*>(mesh_.mesh().vertices().data()), q.size());
        return CompareMatrices(values, q);
    }

    /* The box specification we're meshing. */
    static Box box(double scale = 1) { return Box(1 * scale, 2 * scale, 3 * scale); }

    /* Creates an instance of the box we'll use for the test. */
    static VolumeMesh<T> MakeBox(double scale = 1) { return MakeBoxVolumeMeshWithMa<T>(box(scale)); }
