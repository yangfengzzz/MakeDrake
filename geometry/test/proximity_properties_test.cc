...
GTEST_TEST(ProximityPropertiesTest, AddContactMaterial) {
    ...
    // Case: Correct configuration.
    {
        ...
        const CoulombFrictiond& mu_stored = p.GetProperty<CoulombFrictiond>(kMaterialGroup, kFriction);
    ...
    // Error case: Already has dissipation.
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(AddContactMaterial(d, ps, mu, &p),
    ...
    {
        ProximityProperties p;
        DRAKE_EXPECT_THROWS_MESSAGE(AddContactMaterial(-1.2, ps, mu, &p), ".+dissipation can't be negative.+");
    }

    // Error case: negative stiffness.
    {
        ProximityProperties p;
        DRAKE_EXPECT_THROWS_MESSAGE(AddContactMaterial(d, -200, mu, &p), ".+stiffness must be strictly positive.+");
    }

    // Error case: zero-valued stiffness.
    {
        ProximityProperties p;
        DRAKE_EXPECT_THROWS_MESSAGE(AddContactMaterial(d, 0, mu, &p), ".+stiffness must be strictly positive.+");
    }
}
...
GTEST_TEST(ProximityPropertiesTest, AddRigidProperties) {
    for (double length : {1e-5, 1.25, 1e7}) {
        ...
        EXPECT_EQ(props.GetProperty<HydroelasticType>(kHydroGroup, kComplianceType), HydroelasticType::kRigid);
...
void CheckDisallowedModulusValues(const char* test_name,
    ...
    {
        ProximityProperties p;
        DRAKE_EXPECT_THROWS_MESSAGE(function_to_test(0., &p), ".+elastic modulus must be positive.+");
    }
    // Error case: negative hydroelastic modulus.
    {
        ProximityProperties p;
        DRAKE_EXPECT_THROWS_MESSAGE(function_to_test(-1.3, &p), ".+elastic modulus must be positive.+");
    }
}
...
GTEST_TEST(ProximityPropertiesTest, AddCompliantProperties) {
    ...
    for (double length : {1e-5, 1.25, 1e7}) {
        ...
        EXPECT_TRUE(props.HasProperty(kHydroGroup, kComplianceType));
        EXPECT_EQ(props.GetProperty<HydroelasticType>(kHydroGroup, kComplianceType), HydroelasticType::kSoft);
        EXPECT_TRUE(props.HasProperty(kHydroGroup, kRezHint));
        EXPECT_EQ(props.GetProperty<double>(kHydroGroup, kRezHint), length);
    ...
    CheckDisallowedModulusValues("AddCompliantHydroelasticProperties", [](double modulus, ProximityProperties* p) {
...
GTEST_TEST(ProximityPropertiesTest, AddHalfSpaceSoftProperties) {
    ...
    for (double thickness : {1e-5, 1.25, 1e7}) {
        ...
        EXPECT_EQ(props.GetProperty<double>(kHydroGroup, kSlabThickness), thickness);
        EXPECT_TRUE(props.HasProperty(kHydroGroup, kElastic));
        EXPECT_EQ(props.GetProperty<double>(kHydroGroup, kElastic), E);
        EXPECT_EQ(props.GetProperty<HydroelasticType>(kHydroGroup, kComplianceType), HydroelasticType::kSoft);
    }
...
GTEST_TEST(ProximityPropertiesTest, HydroelasticTypeTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(internal::GetHydroelasticTypeFromString("foobar"), ".*Unknown.*foobar.*");
}
