...
GTEST_TEST(MeshcatGraphvizTest, Publish) {
    ...
    const Params params_orig{.node_id = "node_bar", .header_lines = {"existing header"}};
    const Params params = dut.DecorateParams(params_orig);
    EXPECT_EQ(params.node_id, "node_bar");
    EXPECT_THAT(params.header_lines, testing::ElementsAre("existing header", "path=/drake/path_foo"));

    const Result result_orig{
            .input_ports = {"node_bar:u0"}, .output_ports = {"node_bar:y0"}, .fragments = {"existing fragment"}};
    ...
    EXPECT_THAT(result.fragments.back(), testing::Not(testing::HasSubstr("meshcat_out")));
}
...
GTEST_TEST(MeshcatGraphvizTest, Subscribe) {
    ...
    EXPECT_THAT(result.fragments.back(), testing::Not(testing::HasSubstr("meshcat_in")));
}
