...
public:
    ...
    template <typename T>
    static void FullPoseUpdate(const SceneGraph<T>& scene_graph, const Context<T>& context) {
        scene_graph.FullPoseUpdate(context);
    }

    template <typename T>
    static void FullConfigurationUpdate(const SceneGraph<T>& scene_graph, const Context<T>& context) {
    ...
    template <typename T>
    static const GeometryState<T>& GetGeometryState(const SceneGraph<T>& scene_graph,
                                                    const systems::Context<T>& context) {
        return scene_graph.geometry_state(context);
    }

    template <typename T>
    static GeometryState<T>& GetMutableGeometryState(const SceneGraph<T>& scene_graph, systems::Context<T>* context) {
...
std::unique_ptr<GeometryInstance> make_sphere_instance(double radius = 1.0) {
    return make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(radius), "sphere");
}
...
public:
    SceneGraphTest() : ::testing::Test(), query_object_(QueryObjectTest::MakeNullQueryObject<double>()) {}
...
protected:
    void CreateDefaultContext() {
        ...
        QueryObjectTest::set_query_object(&query_object_, &scene_graph_, context_.get());
    }

    const QueryObject<double>& query_object() const {
        // The `CreateDefaultContext()` method must have been called *prior* to this
        // method.
        if (!context_) throw std::runtime_error("Must call CreateDefaultContext() first.");
...
TEST_F(SceneGraphTest, RegisterSourcePostContext) {
    ...
    EXPECT_EQ(scene_graph_.model_inspector().GetName(new_source), new_source_name);
    // Not found in allocated context.
    DRAKE_EXPECT_THROWS_MESSAGE(query_object().inspector().GetName(new_source),
...
TEST_F(SceneGraphTest, InputPortsForInvalidSource) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph_.get_source_pose_port(fake_source),
                                "Can't acquire pose port for unknown source id: \\d+.");
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph_.get_source_configuration_port(fake_source),
...
TEST_F(SceneGraphTest, TopologyAfterAllocation) {
    SourceId id = scene_graph_.RegisterSource();
    FrameId old_frame_id = scene_graph_.RegisterFrame(id, GeometryFrame("old_frame"));
    // This geometry will be removed after allocation.
    GeometryId old_geometry_id = scene_graph_.RegisterGeometry(id, old_frame_id, make_sphere_instance());

    CreateDefaultContext();

    FrameId parent_frame_id = scene_graph_.RegisterFrame(id, GeometryFrame("parent"));
    FrameId child_frame_id = scene_graph_.RegisterFrame(id, parent_frame_id, GeometryFrame("child"));
    GeometryId geometry_id = scene_graph_.RegisterGeometry(id, parent_frame_id, make_sphere_instance());
    GeometryId anchored_id = scene_graph_.RegisterAnchoredGeometry(id, make_sphere_instance());
    scene_graph_.RemoveGeometry(id, old_geometry_id);

    const SceneGraphInspector<double>& model_inspector = scene_graph_.model_inspector();
    const SceneGraphInspector<double>& context_inspector = query_object().inspector();
    ...
    EXPECT_THROW(model_inspector.BelongsToSource(old_geometry_id, id), std::logic_error);

    EXPECT_THROW(context_inspector.BelongsToSource(parent_frame_id, id), std::logic_error);
    EXPECT_THROW(context_inspector.BelongsToSource(child_frame_id, id), std::logic_error);
    EXPECT_THROW(context_inspector.BelongsToSource(geometry_id, id), std::logic_error);
    EXPECT_THROW(context_inspector.BelongsToSource(anchored_id, id), std::logic_error);
...
TEST_F(SceneGraphTest, FullPoseUpdateEmpty) {
    CreateDefaultContext();
    DRAKE_EXPECT_NO_THROW(SceneGraphTester::FullPoseUpdate(scene_graph_, *context_));
}
...
TEST_F(SceneGraphTest, FullConfigurationUpdateEmpty) {
    CreateDefaultContext();
    DRAKE_EXPECT_NO_THROW(SceneGraphTester::FullConfigurationUpdate(scene_graph_, *context_));
}
...
TEST_F(SceneGraphTest, RegisterDeformableGeometry) {
    ...
    GeometryId rigid_id = scene_graph_.RegisterGeometry(s_id, scene_graph_.world_frame_id(), make_sphere_instance());
    constexpr double kRezHint = 0.5;
    std::unique_ptr<GeometryInstance> geometry_instance = make_sphere_instance();
    GeometryId deformable_id = scene_graph_.RegisterDeformableGeometry(s_id, scene_graph_.world_frame_id(),
    ...
    const VolumeMesh<double>* mesh_ptr = inspector.GetReferenceMesh(deformable_id);
    ASSERT_NE(mesh_ptr, nullptr);

    CreateDefaultContext();
    const QueryObject<double>& query_object = this->query_object();
    const VectorX<double> q_WG = VectorX<double>::Zero(mesh_ptr->num_vertices() * 3);
    GeometryConfigurationVector<double> configuration_vector;
    configuration_vector.set_value(deformable_id, q_WG);
    scene_graph_.get_source_configuration_port(s_id).FixValue(context_.get(), configuration_vector);
...
TEST_F(SceneGraphTest, RegisterUnsupportedDeformableGeometry) {
    ...
    auto geometry_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Cylinder>(1.0, 2.0), "cylinder");
    SourceId s_id = scene_graph_.RegisterSource();
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph_.RegisterDeformableGeometry(s_id, scene_graph_.world_frame_id(),
                                                                        std::move(geometry_instance), kRezHint),
...
TEST_F(SceneGraphTest, ApplyConfig) {
    EXPECT_EQ(scene_graph_.get_config().default_proximity_properties.compliance_type, "undefined");
    SourceId s_id = scene_graph_.RegisterSource();
    auto geometry_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Box>(1.0, 2.0, 3.0), "box");
    geometry_instance->set_proximity_properties(ProximityProperties());
    auto g_id = scene_graph_.RegisterGeometry(s_id, scene_graph_.world_frame_id(), std::move(geometry_instance));
    // Plain context.
    CreateDefaultContext();
    // No hydroelastic mesh available.
    EXPECT_EQ(query_object().inspector().maybe_get_hydroelastic_mesh(g_id).index(), 0);
    ...
    EXPECT_EQ(query_object().inspector().maybe_get_hydroelastic_mesh(g_id).index(), 2);
    ...
    geometry_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Box>(1.0, 3.0, 5.0), "box2");
    geometry_instance->set_proximity_properties(ProximityProperties());
    g_id = scene_graph_.RegisterGeometry(context_.get(), s_id, scene_graph_.world_frame_id(),
    ...
    geometry_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Box>(1.0, 1.0, 1.0), "box3");
    g_id = scene_graph_.RegisterGeometry(context_.get(), s_id, scene_graph_.world_frame_id(),
                                         std::move(geometry_instance));
    ...
    ASSERT_TRUE(scene_graph_.get_config(*context_).default_proximity_properties.resolution_hint.has_value());
    scene_graph_.AssignRole(context_.get(), s_id, g_id, ProximityProperties(), RoleAssign::kReplace);
    ...
    ASSERT_TRUE(scene_graph_.get_config(*context_).default_proximity_properties.resolution_hint.has_value());
    ProximityProperties edit_props(*query_object().inspector().GetProximityProperties(g_id));
    edit_props.RemoveProperty(kHydroGroup, kRezHint);
    scene_graph_.AssignRole(context_.get(), s_id, g_id, edit_props, RoleAssign::kReplace);
    ...
    ASSERT_FALSE(scene_graph_.get_config(*context_).default_proximity_properties.point_stiffness.has_value());
...
TYPED_TEST_P(TypedSceneGraphTest, TransmogrifyWithoutAllocation) {
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph_U->RegisterAnchoredGeometry(s_id, make_sphere_instance()));
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph_U2->RegisterAnchoredGeometry(s_id, make_sphere_instance()));
}
...
TYPED_TEST_P(TypedSceneGraphTest, TransmogrifyPorts) {
    ...
    this->CreateDefaultContext();
    auto scene_graph_U = System<double>::ToScalarType<U>(this->scene_graph_);
    EXPECT_EQ(scene_graph_U->num_input_ports(), this->scene_graph_.num_input_ports());
...
TYPED_TEST_P(TypedSceneGraphTest, TransmogrifyContext) {
    ...
    GeometryId g_id = this->scene_graph_.RegisterAnchoredGeometry(s_id, make_sphere_instance());
    this->CreateDefaultContext();
    ...
    const GeometryState<U>& geo_state_U = SceneGraphTester::GetGeometryState(*scene_graph_U, *context_U);
    // If the anchored geometry were not ported over, this would throw an
    // exception.
    EXPECT_TRUE(geo_state_U.BelongsToSource(g_id, s_id));
    EXPECT_THROW(geo_state_U.BelongsToSource(GeometryId::get_new_id(), s_id), std::logic_error);
...
TYPED_TEST_P(TypedSceneGraphTest, NonDoubleClone) {
    using U = TypeParam;
    std::unique_ptr<SceneGraph<U>> scene_graph_U = System<double>::ToScalarType<U>(this->scene_graph_);
...
TEST_F(SceneGraphTest, ModelInspector) {
    ...
    GeometryId deformable_id = scene_graph_.RegisterDeformableGeometry(
            ...
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "deformable_sphere"),
            kRezHint);
    ...
    GeometryId anchored_id = scene_graph_.RegisterAnchoredGeometry(
            source_id, make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "sphere"));
    GeometryId sphere_1 = scene_graph_.RegisterGeometry(
            source_id, frame_1,
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "sphere"));
    ...
    EXPECT_EQ(inspector.GetGeometryIdByName(frame_1, Role::kUnassigned, "sphere"), sphere_1);
    EXPECT_EQ(inspector.GetGeometryIdByName(frame_2, Role::kUnassigned, "sphere"), sphere_2);
    EXPECT_EQ(inspector.GetGeometryIdByName(scene_graph_.world_frame_id(), Role::kUnassigned, "sphere"), anchored_id);
    EXPECT_EQ(inspector.GetGeometryIdByName(scene_graph_.world_frame_id(), Role::kUnassigned, "deformable_sphere"),
              deformable_id);
}
...
TEST_F(SceneGraphTest, RendererInSceneGraphSmokeTest) {
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph_.AddRenderer(kRendererName, make_unique<DummyRenderEngine>()));
...
TEST_F(SceneGraphTest, RendererInContextSmokeTest) {
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph_.AddRenderer(context_.get(), kRendererName, make_unique<DummyRenderEngine>()));
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph_.RemoveRenderer(context_.get(), kRendererName));
    EXPECT_EQ(scene_graph_.RendererCount(*context_), 0);
    EXPECT_FALSE(scene_graph_.HasRenderer(*context_, kRendererName));
}
...
TEST_F(SceneGraphTest, GetRendererTypeName) {
    ...
    CreateDefaultContext();
    DRAKE_EXPECT_NO_THROW(scene_graph_.AddRenderer(kRendererName1, make_unique<DummyRenderEngine>()));
    DRAKE_EXPECT_NO_THROW(scene_graph_.AddRenderer(context_.get(), kRendererName2, make_unique<DummyRenderEngine>()));
    ...
    EXPECT_EQ(scene_graph_.GetRendererTypeName(kRendererName1), NiceTypeName::Get<DummyRenderEngine>());
    EXPECT_EQ(scene_graph_.GetRendererTypeName(*context_, kRendererName2), NiceTypeName::Get<DummyRenderEngine>());
}
...
TEST_F(SceneGraphTest, RemoveRenderer) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph_.RemoveRenderer("bad_name"),
...
TEST_F(SceneGraphTest, RoleManagementSmokeTest) {
    ...
    auto instance = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "sphere");
    ...
    GeometryId g_id = scene_graph_.RegisterGeometry(s_id, f_id, std::move(instance));
...
TEST_F(SceneGraphTest, ChangeShape) {
    ...
    GeometryId g_id = scene_graph_.RegisterAnchoredGeometry(
            source_id, make_unique<GeometryInstance>(X_WG_original, make_unique<Sphere>(sphere), "sphere"));
    CreateDefaultContext();
    ...
    EXPECT_TRUE(CompareMatrices(X_WG_original.GetAsMatrix34(), model_inspector.GetPoseInFrame(g_id).GetAsMatrix34()));
    ...
    EXPECT_TRUE(CompareMatrices(X_WG_original.GetAsMatrix34(), context_inspector.GetPoseInFrame(g_id).GetAsMatrix34()));
    ...
    EXPECT_TRUE(CompareMatrices(X_WG_new.GetAsMatrix34(), model_inspector.GetPoseInFrame(g_id).GetAsMatrix34()));
    ...
    EXPECT_TRUE(CompareMatrices(X_WG_new.GetAsMatrix34(), context_inspector.GetPoseInFrame(g_id).GetAsMatrix34()));
}
...
public:
    ...
    explicit GeometrySourceSystem(SceneGraph<double>* scene_graph, bool add_deformable = true)
        : systems::LeafSystem<double>(), scene_graph_(scene_graph) {
        // Register with SceneGraph.
        registered_source_name_ = add_deformable ? "mixed_source" : "non-deformable_only_source";
        source_id_ = scene_graph->RegisterSource(registered_source_name_);
        FrameId f_id = scene_graph->RegisterFrame(source_id_, GeometryFrame("frame"));
        frame_ids_.push_back(f_id);

        if (add_deformable) {
            constexpr double kRezHint = 0.5;
            GeometryId deformable_id = scene_graph->RegisterDeformableGeometry(
                    source_id_, scene_graph->world_frame_id(), make_sphere_instance(), kRezHint);

            const SceneGraphInspector<double>& inspector = scene_graph->model_inspector();
            const VolumeMesh<double>* mesh_ptr = inspector.GetReferenceMesh(deformable_id);
            DRAKE_DEMAND(mesh_ptr != nullptr);
            configurations_.set_value(deformable_id, VectorX<double>::Zero(mesh_ptr->num_vertices() * 3));
        }

        // Set up output pose port now that the frame is registered.
        pose_port_index_ =
                this->DeclareAbstractOutputPort("pose", &GeometrySourceSystem::CalcFramePoseOutput).get_index();

        // Set up output configuration port.
        configuration_port_index_ =
                this->DeclareAbstractOutputPort("configuration", &GeometrySourceSystem::CalcConfigurationOutput)
                        .get_index();
    }
    ...
    void add_extra_frame(bool add_to_output = true) {
        FrameId frame_id = scene_graph_->RegisterFrame(source_id_, GeometryFrame("frame"));
    ...
    const std::string& registered_source_name() const { return registered_source_name_; }
...
private:
    ...
    void CalcConfigurationOutput(const Context<double>& context,
...
GTEST_TEST(SceneGraphConnectionTest, FullPositionUpdateConnected) {
    ...
    auto mixed_source = builder.AddSystem<GeometrySourceSystem>(scene_graph, true);
    mixed_source->set_name("mixed source");
    auto nondeformable_source = builder.AddSystem<GeometrySourceSystem>(scene_graph, false);
    ...
    // ports.
    builder.Connect(mixed_source->get_pose_output_port(), scene_graph->get_source_pose_port(mixed_source_id));
    ...
    const auto& sg_context = diagram->GetMutableSubsystemContext(*scene_graph, diagram_context.get());
    DRAKE_EXPECT_NO_THROW(SceneGraphTester::FullPoseUpdate(*scene_graph, sg_context));
    DRAKE_EXPECT_NO_THROW(SceneGraphTester::FullConfigurationUpdate(*scene_graph, sg_context));

    // Also check that scene graph methods validate contexts.
    auto& ms_context = diagram->GetMutableSubsystemContext(*mixed_source, diagram_context.get());
    ...
    // its context.
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph->collision_filter_manager(diagram_context.get()),
                                ".*\n.*context-system-mismatch.*");
    DRAKE_EXPECT_THROWS_MESSAGE(scene_graph->collision_filter_manager(&ms_context), ".*\n.*context-system-mismatch.*");
}
...
GTEST_TEST(SceneGraphConnectionTest, FullPoseUpdateDisconnected) {
    ...
    diagram->SetDefaultContext(diagram_context.get());
    const auto& sg_context = diagram->GetMutableSubsystemContext(*scene_graph, diagram_context.get());
    DRAKE_EXPECT_THROWS_MESSAGE(SceneGraphTester::FullPoseUpdate(*scene_graph, sg_context),
                                fmt::format("Source '{}' \\(id: \\d+\\) has registered dynamic frames "
                                            "but is not connected .+",
                                            source_system->registered_source_name()));
    DRAKE_EXPECT_THROWS_MESSAGE(SceneGraphTester::FullConfigurationUpdate(*scene_graph, sg_context),
                                fmt::format("Source '{}' \\(id: \\d+\\) has registered deformable geometry "
                                            "but is not connected .+",
                                            source_system->registered_source_name()));
}
...
GTEST_TEST(SceneGraphAutoDiffTest, InstantiateAutoDiff) {
    ...
    QueryObject<AutoDiffXd> handle = QueryObjectTest::MakeNullQueryObject<AutoDiffXd>();
...
GTEST_TEST(SceneGraphExpressionTest, InstantiateExpression) {
    ...
    QueryObject<Expression> handle = QueryObjectTest::MakeNullQueryObject<Expression>();
    SceneGraphTester::GetQueryObjectPortValue(scene_graph, *context, &handle);
}
...
GTEST_TEST(SceneGraphContextModifier, RegisterGeometry) {
    ...
    FrameId frame_id = scene_graph.RegisterFrame(source_id, GeometryFrame("frame"));
    auto context = scene_graph.CreateDefaultContext();
    ...
    SceneGraphTester::GetQueryObjectPortValue(scene_graph, *context, &query_object);
    ...
    GeometryId sphere_id_1 = scene_graph.RegisterGeometry(context.get(), source_id, frame_id, make_sphere_instance());
    EXPECT_EQ(1, inspector.NumGeometriesForFrame(frame_id));
    EXPECT_EQ(frame_id, inspector.GetFrameId(sphere_id_1));

    // Remove the geometry.
    DRAKE_EXPECT_NO_THROW(scene_graph.RemoveGeometry(context.get(), source_id, sphere_id_1));
    EXPECT_EQ(0, inspector.NumGeometriesForFrame(frame_id));
    DRAKE_EXPECT_THROWS_MESSAGE(inspector.GetFrameId(sphere_id_1), "Referenced geometry \\d+ has not been registered.");
}
...
GTEST_TEST(SceneGraphRenderTest, AddRenderer) {
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph.AddRenderer("unique", make_unique<DummyRenderEngine>()));

    // Non-unique renderer name.
    // NOTE: The error message is tested in geometry_state_test.cc.
    EXPECT_THROW(scene_graph.AddRenderer("unique", make_unique<DummyRenderEngine>()), std::logic_error);
    ...
    DRAKE_EXPECT_NO_THROW(scene_graph.AddRenderer("different", make_unique<DummyRenderEngine>()));
}
