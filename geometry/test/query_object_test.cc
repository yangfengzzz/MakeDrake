...
protected:
    template <typename T>
    static std::unique_ptr<QueryObject<T>> MakeQueryObject(const systems::Context<T>* context,
    ...
    static ::testing::AssertionResult is_default(const QueryObject<T>& object) {
        if (object.scene_graph_ != nullptr || object.context_ != nullptr || object.state_ != nullptr) {
            return ::testing::AssertionFailure()
                   << "A default query object should have all null fields. Has "
                      "scene_graph: "
                   << object.scene_graph_ << ", context: " << object.context_ << ", state: " << object.state_.get();
        }
    ...
    template <typename T>
    static ::testing::AssertionResult shared_baked(const QueryObject<T>& o1, const QueryObject<T>& o2) {
        auto result = is_baked(o1);
        if (result) {
            result = is_baked(o2);
            if (result) {
                if (o1.state_ != o2.state_) {
                    result = ::testing::AssertionFailure() << "The two objects are baked but have different states";
                }
            }
        }
        return result;
...
TEST_F(QueryObjectTest, CopySemantics) {
    ...
    unique_ptr<Context<double>> live_context = scene_graph_.CreateDefaultContext();
    unique_ptr<QueryObject<double>> live_query_object = MakeQueryObject(live_context.get(), &scene_graph_);
...
#define EXPECT_DEFAULT_ERROR(expression) \
    DRAKE_EXPECT_THROWS_MESSAGE(expression, "Attempting to perform query on invalid QueryObject.+");
    ...
    EXPECT_DEFAULT_ERROR(default_object.GetConfigurationsInWorld(GeometryId::get_new_id()));
    EXPECT_DEFAULT_ERROR(
            default_object.GetDrivenMeshConfigurationsInWorld(GeometryId::get_new_id(), Role::kIllustration));

    // Penetration queries.
    EXPECT_DEFAULT_ERROR(default_object.ComputePointPairPenetration());
    const HydroelasticContactRepresentation representation = HydroelasticContactRepresentation::kTriangle;
    ...
    EXPECT_DEFAULT_ERROR(default_object.ComputeContactSurfacesWithFallback(representation, &surfaces, &point_pairs));
    internal::DeformableContact<double> deformable_contact;
    EXPECT_DEFAULT_ERROR(default_object.ComputeDeformableContact(&deformable_contact));

    // Signed distance queries.
    EXPECT_DEFAULT_ERROR(default_object.ComputeSignedDistancePairwiseClosestPoints());
    EXPECT_DEFAULT_ERROR(
            default_object.ComputeSignedDistancePairClosestPoints(GeometryId::get_new_id(), GeometryId::get_new_id()));
    EXPECT_DEFAULT_ERROR(default_object.ComputeSignedDistanceToPoint(Vector3<double>::Zero()));

    EXPECT_DEFAULT_ERROR(default_object.FindCollisionCandidates());
    EXPECT_DEFAULT_ERROR(default_object.HasCollisions());

    // Render queries.
    const ColorRenderCamera color_camera{{"n/a", {2, 2, M_PI}, {0.1, 10}, RigidTransformd{}}, false};
    const DepthRenderCamera depth_camera{{"n/a", {2, 2, M_PI}, {0.1, 10}, RigidTransformd{}}, {0.2, 0.9}};
    const RigidTransformd X_WC = RigidTransformd::Identity();

    ImageRgba8U color;
    EXPECT_DEFAULT_ERROR(default_object.RenderColorImage(color_camera, FrameId::get_new_id(), X_WC, &color));
    ImageDepth32F depth;
    EXPECT_DEFAULT_ERROR(default_object.RenderDepthImage(depth_camera, FrameId::get_new_id(), X_WC, &depth));
    ImageLabel16I label;
    EXPECT_DEFAULT_ERROR(default_object.RenderLabelImage(color_camera, FrameId::get_new_id(), X_WC, &label));
...
TEST_F(QueryObjectTest, CreateValidInspector) {
    ...
    FrameId frame_id = scene_graph_.RegisterFrame(source_id, GeometryFrame("frame"));
    GeometryId geometry_id = scene_graph_.RegisterGeometry(
            source_id, frame_id, make_unique<GeometryInstance>(identity, make_unique<Sphere>(1.0), "sphere"));
    unique_ptr<Context<double>> context = scene_graph_.CreateDefaultContext();
    unique_ptr<QueryObject<double>> query_object = MakeQueryObject(context.get(), &scene_graph_);
...
TEST_F(QueryObjectTest, BakedCopyHasFullUpdate) {
    ...
    auto deformable_geometry =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "deformable");
    // Make sure the resolution hint is large enough so that we know that the
    // volume mesh is a octahedron.
    GeometryId g_id = scene_graph_.RegisterDeformableGeometry(s_id, internal::InternalFrame::world_frame_id(),
    ...
    scene_graph_.get_source_configuration_port(s_id).FixValue(context.get(), configurations);

    const auto& query_object = scene_graph_.get_query_output_port().Eval<QueryObject<double>>(*context);
    ...
    EXPECT_FALSE(CompareMatrices(stale_pose.GetAsMatrix34(), X_WF.GetAsMatrix34()));
    ...
    const auto& baked_configuration = baked_state.get_configurations_in_world(g_id);
    EXPECT_TRUE(CompareMatrices(baked_pose.GetAsMatrix34(), X_WF.GetAsMatrix34()));
    ...
    // to the baked copy.
    EXPECT_TRUE(CompareMatrices(stale_pose.GetAsMatrix34(), X_WF.GetAsMatrix34()));
