...
VolumeMesh<double> MakeSingleTetVolumeMesh() {
    ...
    std::vector<Vector3d> vertices{Vector3d{0, 0, 0}, Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()};
...
GTEST_TEST(BarycentricInterpolatorTest, ConstructAndInterpolate) {
    ...
    positions[1] = Vector3d(0.25, 0.25, 0.25);  // barycentric coordinate (0.25, 0.25, 0.25, 0.25).
    const BarycentricInterpolator interpolator(positions, MakeSingleTetVolumeMesh());
    ...
    EXPECT_TRUE(CompareMatrices(interpolated_q.tail(3), expected_q_for_second_point));
...
GTEST_TEST(BarycentricInterpolatorTest, PassivePointOutOfBound) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(BarycentricInterpolator(positions, MakeSingleTetVolumeMesh()),
...
GTEST_TEST(VertexSampler, ConstructorFailures) {
    ...
    // Duplicated vertices.
    EXPECT_THROW(VertexSampler(std::vector<int>{1, 1}, control_mesh), std::exception);
    // Vertices out of bound.
    EXPECT_THROW(VertexSampler(std::vector<int>{-1}, control_mesh), std::exception);
    EXPECT_THROW(VertexSampler(std::vector<int>{7}, control_mesh), std::exception);
    // Vertices not sorted.
    EXPECT_THROW(VertexSampler(std::vector<int>{3, 2, 1}, control_mesh), std::exception);
}
...
GTEST_TEST(DrivenTriangleMesh, VertexPositionAndNormal) {
    ...
    TriangleSurfaceMesh<double> surface_mesh(std::move(elements), std::move(vertices));
  ...
  expected_q << 0, 0, 0,  // v0 doesn't move
    ...
    EXPECT_TRUE(CompareMatrices(expected_q, driven_mesh.GetDrivenVertexPositions()));
    ...
    EXPECT_TRUE(CompareMatrices(expected_normals, driven_mesh.GetDrivenVertexNormals()));
    ...
    DrivenTriangleMesh another_driven_mesh(
            std::variant<BarycentricInterpolator, VertexSampler>(std::move(interpolator)),
            TriangleSurfaceMesh<double>(std::move(surface_mesh)));
    another_driven_mesh.SetControlMeshPositions(q);
    EXPECT_TRUE(CompareMatrices(expected_q, another_driven_mesh.GetDrivenVertexPositions()));
    EXPECT_TRUE(CompareMatrices(expected_normals, another_driven_mesh.GetDrivenVertexNormals()));
}
...
GTEST_TEST(MakeDrivenSurfaceMesh, OctahedronMesh) {
    ...
    for (int i = 0; i < 6; ++i) {
        EXPECT_EQ(control_mesh.vertex(expected_mapping[i]), driven_q.segment<3>(3 * i));
    }
  ...
  expected_normals << 1,  0,  0,
    ...
    for (int i = 0; i < 6; ++i) {
        EXPECT_EQ(control_q.segment<3>(3 * expected_mapping[i]), driven_q.segment<3>(3 * i));
    }
