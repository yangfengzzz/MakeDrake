...
#include "geometry/proximity/deformable_contact_internal.h"
#include "geometry/proximity/hydroelastic_callback.h"
...
void CompareSignedDistancePair(const SignedDistancePair<T>& pair,
    ...
    EXPECT_TRUE(CompareMatrices(pair.p_ACa, pair_expected.p_ACa, tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(pair.p_BCb, pair_expected.p_BCb, tol, MatrixCompareType::absolute));
}
...
public:
    ...
    template <typename T>
    static bool IsDeepCopy(const ProximityEngine<T>& test_engine, const ProximityEngine<T>& ref_engine) {
        return ref_engine.IsDeepCopy(test_engine);
    }

    template <typename T>
    static math::RigidTransformd GetX_WG(GeometryId id, bool is_dynamic, const ProximityEngine<T>& engine) {
        return engine.GetX_WG(id, is_dynamic);
    }

    template <typename T>
    static HydroelasticType hydroelastic_type(GeometryId id, const ProximityEngine<T>& engine) {
    ...
    template <typename T>
    static const fcl::CollisionObjectd* GetCollisionObject(const ProximityEngine<T>& engine, GeometryId id) {
    ...
    template <typename T>
    static const internal::deformable::Geometries& get_deformable_contact_geometries(const ProximityEngine<T>& engine) {
...
public:
    ...
    static const DeformableGeometry& get_deformable_geometry(const Geometries& geometries, GeometryId id) {
        return geometries.deformable_geometries_.at(id);
    }

    static const RigidGeometry& get_rigid_geometry(const Geometries& geometries, GeometryId id) {
...
GTEST_TEST(ProximityEngineTests, ProcessHydroelasticProperties) {
    ...
    // Case: compliant sphere.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(sphere_id, engine), HydroelasticType::kCompliant);
    }
    ...
    // Case: rigid cylinder.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(cylinder_id, engine), HydroelasticType::kRigid);
    }
    ...
    // Case: rigid ellipsoid.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(ellipsoid_id, engine), HydroelasticType::kRigid);
    }
    ...
    // Case: rigid capsule.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(capsule_id, engine), HydroelasticType::kRigid);
    }
    ...
    // Case: rigid half_space.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(half_space_id, engine), HydroelasticType::kRigid);
    }
    ...
    // Case: rigid box.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(box_id, engine), HydroelasticType::kRigid);
    }

    // Case: rigid mesh.
    {
        Mesh mesh{drake::FindResourceOrThrow("drake/geometry/test/non_convex_mesh.obj"), 1.0 /* scale */};
        const GeometryId mesh_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(mesh, {}, mesh_id, rigid_properties);
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(mesh_id, engine), HydroelasticType::kRigid);
    }

    // Case: compliant mesh.
    {
        Mesh mesh{drake::FindResourceOrThrow("drake/geometry/test/non_convex_mesh.vtk"), 1.0 /* scale */};
        const GeometryId mesh_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(mesh, {}, mesh_id, soft_properties);
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(mesh_id, engine), HydroelasticType::kCompliant);
    }
    ...
    {
        Convex convex{drake::FindResourceOrThrow("drake/geometry/test/non_convex_mesh.vtk"), 1.0};
        const GeometryId convex_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(convex, {}, convex_id, soft_properties);
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine), HydroelasticType::kCompliant);
    }

    // Case: rigid convex.
    {
        Convex convex{drake::FindResourceOrThrow("drake/geometry/test/quad_cube.obj"), edge_length};
        const GeometryId convex_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(convex, {}, convex_id, rigid_properties);
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine), HydroelasticType::kRigid);
    }

    // Case: rigid convex vtk.
    {
        Convex convex{drake::FindResourceOrThrow("drake/geometry/test/one_tetrahedron.vtk"), edge_length};
        const GeometryId convex_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(convex, {}, convex_id, rigid_properties);
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine), HydroelasticType::kRigid);
    }
...
GTEST_TEST(ProximityEngineTests, ProcessVtkMeshUndefHydro) {
    ...
    // Case: mesh vtk, no hydro type annotation.
    {
        ...
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(mesh_id, engine), HydroelasticType::kUndefined);
    }
}
...
GTEST_TEST(ProximityEngineTests, ProcessVtkConvexUndefHydro) {
    ...
    {
        Convex convex{drake::FindResourceOrThrow("drake/geometry/test/one_tetrahedron.vtk"), 1.0 /* scale */};
        const GeometryId convex_id = GeometryId::get_new_id();
        engine.AddDynamicGeometry(convex, {}, convex_id, ProximityProperties());
        EXPECT_EQ(ProximityEngineTester::hydroelastic_type(convex_id, engine), HydroelasticType::kUndefined);
    }
}
...
std::pair<GeometryId, RigidTransformd> AddShape(
        ProximityEngine<double>* engine, const Shape& shape, bool is_anchored, bool is_soft, const Vector3d& p_S1S2_W) {
...
unordered_map<GeometryId, RigidTransformd> PopulateEngine(ProximityEngine<double>* engine,
                                                          const Shape& shape1,
                                                          bool anchored1,
                                                          bool soft1,
                                                          const Shape& shape2,
    ...
    GeometryId id1, id2;
    std::tie(id1, X_WG) = AddShape(engine, shape1, anchored1, soft1, Vector3d::Zero());
...
GTEST_TEST(ProximityEngineTest, ComputeContactSurfacesAutodiffSupport) {
    ...
    const Mesh mesh{drake::FindResourceOrThrow("drake/geometry/test/non_convex_mesh.obj"), 1.0 /* scale */};
    ...
    // contact surface has derivatives.
    {
        ...
        const auto X_WGs_d = PopulateEngine(&engine_d, sphere, anchored, soft, mesh, !anchored, !soft);
        ...
        for (const auto& [id, X_WG_d] : X_WGs_d) {
            if (!added_derivatives) {
                ...
                // we'll pass through.
                const Vector3<AutoDiffXd> p_WGo = math::InitializeAutoDiff(X_WG_d.translation());
                X_WGs_ad[id] = RigidTransform<AutoDiffXd>(X_WG_d.rotation().cast<AutoDiffXd>(), p_WGo);
        ...
        engine_ad->ComputeContactSurfacesWithFallback(HydroelasticContactRepresentation::kTriangle, X_WGs_ad, &surfaces,
                                                      &point_pairs);
    ...
    {
        ProximityEngine<double> engine_d;
        const auto X_WGs_d = PopulateEngine(&engine_d, sphere, false, !soft, sphere, false, !soft);
        const auto engine_ad = engine_d.ToScalarType<AutoDiffXd>();
        unordered_map<GeometryId, RigidTransform<AutoDiffXd>> X_WGs_ad;
        for (const auto& [id, X_WG_d] : X_WGs_d) {
            X_WGs_ad[id] = RigidTransform<AutoDiffXd>(X_WG_d.GetAsMatrix34());
...
GTEST_TEST(ProximityEngineTests, AddAnchoredGeometry) {
    ...
    EXPECT_TRUE(
            CompareMatrices(ProximityEngineTester::GetX_WG(id, false, engine).GetAsMatrix34(), X_WG.GetAsMatrix34()));
}
...
GTEST_TEST(ProximityEngineTests, AddMixedGeometry) {
    ...
    EXPECT_TRUE(
            CompareMatrices(ProximityEngineTester::GetX_WG(id_1, false, engine).GetAsMatrix34(), X_WA.GetAsMatrix34()));
    ...
    EXPECT_EQ(engine.num_dynamic(), 1);
    EXPECT_TRUE(
            CompareMatrices(ProximityEngineTester::GetX_WG(id_2, true, engine).GetAsMatrix34(), X_WD.GetAsMatrix34()));
}
...
GTEST_TEST(ProximityEngineTests, ReplaceProperties) {
    ...
    InternalGeometry sphere(SourceId::get_new_id(), make_unique<Sphere>(radius), FrameId::get_new_id(),
                            GeometryId::get_new_id(), "sphere", RigidTransformd());
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateRepresentationForNewProperties(sphere, {}),
    ...
    // it mindlessly attempts to update the hydroelastic representation.
    {
        ...
        DRAKE_EXPECT_NO_THROW(engine.UpdateRepresentationForNewProperties(sphere, {}));
        EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kUndefined);
    }
    ...
    // required.
    {
        ...
        EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kUndefined);
        props.AddProperty(kHydroGroup, kElastic, std::numeric_limits<double>::infinity());
        AddRigidHydroelasticProperties(3 * radius, &props);
        DRAKE_EXPECT_NO_THROW(engine.UpdateRepresentationForNewProperties(sphere, props));
    ...
    {
        EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kRigid);
        DRAKE_EXPECT_NO_THROW(engine.UpdateRepresentationForNewProperties(sphere, ProximityProperties()));
        EXPECT_EQ(PET::hydroelastic_type(sphere.id(), engine), kUndefined);
    }
    ...
    hydro_trigger.AddProperty(kHydroGroup, kComplianceType, HydroelasticType::kCompliant);
    ...
    // efforts to assign those properties throw.
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateRepresentationForNewProperties(sphere, bad_props_no_elasticity),
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(engine.UpdateRepresentationForNewProperties(sphere, bad_props_no_length),
                                    "Cannot create soft Sphere; missing the .+ property");
    }
}
...
GTEST_TEST(ProximityEngineTests, RemoveGeometry) {
    for (bool is_dynamic : {true, false}) {
        ...
        EXPECT_THROW(engine.RemoveGeometry(GeometryId::get_new_id(), true), std::logic_error);
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            EXPECT_EQ(ProximityEngineTester::hydroelastic_type(id, engine), HydroelasticType::kRigid);
        }
        ...
        for (int index : {1, 2, 0}) {
            ...
            EXPECT_EQ(engine.num_anchored(), is_dynamic ? 0 : expected_count);
            EXPECT_EQ(engine.num_dynamic(), is_dynamic ? expected_count : 0);
            EXPECT_THROW(engine.RemoveGeometry(remove_id, is_dynamic), std::logic_error);
            EXPECT_EQ(ProximityEngineTester::hydroelastic_type(remove_id, engine), HydroelasticType::kUndefined);
        }
    }
}
...
GTEST_TEST(ProximityEngineTests, FailedParsing) {
    ...
    {
        Convex convex{drake::FindResourceOrThrow("drake/geometry/test/forbidden_two_cubes.obj"), 1.0};
        DRAKE_EXPECT_THROWS_MESSAGE(engine.AddDynamicGeometry(convex, {}, GeometryId::get_new_id()),
...
GTEST_TEST(ProximityEngineTests, CopySemantics) {
    ...
    Convex convex{drake::FindResourceOrThrow("drake/geometry/test/quad_cube.obj"), 1.0};
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsOnEmptyScene) {
    ...
    const auto results = engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, kInf);
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsSingleAnchored) {
    ...
    Sphere sphere{0.5};
    const GeometryId id = GeometryId::get_new_id();
    const unordered_map<GeometryId, RigidTransformd> X_WGs{{id, RigidTransformd::Identity()}};
...
GTEST_TEST(ProximityEngineTests, SignedDistanceClosestPointsMaxDistance) {
    ...
    const GeometryId id_A = GeometryId::get_new_id();
    const GeometryId id_B = GeometryId::get_new_id();
    unordered_map<GeometryId, RigidTransformd> X_WGs{{id_A, RigidTransformd::Identity()},
    ...
    {
        const Vector3d p_WB = Vector3d(2, 3, 4).normalized() * (kCenterDistance - kEps);
        X_WGs[id_B].set_translation(p_WB);
        engine.UpdateWorldPoses(X_WGs);
        const auto results = engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, kMaxDistance);
...
GTEST_TEST(ProximityEngineTests, SignedDistancePairClosestPoint) {
    ...
    {
        const SignedDistancePair<double> result = engine.ComputeSignedDistancePairClosestPoints(id_A, id_B, X_WGs);
        EXPECT_EQ(result.id_A, id_A);
        EXPECT_EQ(result.id_B, id_B);
        EXPECT_NEAR(result.distance, kDistance, std::numeric_limits<double>::epsilon());
    ...
    {
        DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputeSignedDistancePairClosestPoints(bad_id, id_B, X_WGs),
    ...
    {
        DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputeSignedDistancePairClosestPoints(id_A, bad_id, X_WGs),
                                    fmt::format("The geometry given by id {} does not reference .+ used in "
                                                "a signed distance query",
                                                bad_id));
    }
    ...
    // Case: the distance is evaluated even though the pair is filtered.
    {
        ...
        engine.collision_filter().Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{id_A, id_B}),
                                        extract_ids, false /* is_invariant */);
        EXPECT_NO_THROW(engine.ComputeSignedDistancePairClosestPoints(id_A, id_B, X_WGs));
    }
}
...
GTEST_TEST(ProximityEngineTests, SignedDistanceToPointNonPositiveThreshold) {
    ...
    const GeometryId id_B = GeometryId::get_new_id();
    const unordered_map<GeometryId, RigidTransformd> X_WGs{
            {id_A, RigidTransformd{Translation3d{-kRadius + 0.5 * kPenetration, 0, 0}}},
            {id_B, RigidTransformd{Translation3d{kRadius - 0.5 * kPenetration, 0, 0}}}};
    ...
    std::vector<SignedDistanceToPoint<double>> results_all = engine.ComputeSignedDistanceToPoint(p_WQ, X_WGs, kInf);
    EXPECT_EQ(results_all.size(), 2u);

    std::vector<SignedDistanceToPoint<double>> results_zero = engine.ComputeSignedDistanceToPoint(p_WQ, X_WGs, 0);
    EXPECT_EQ(results_zero.size(), 2u);

    const double kEps = std::numeric_limits<double>::epsilon();

    std::vector<SignedDistanceToPoint<double>> results_barely_in =
            engine.ComputeSignedDistanceToPoint(p_WQ, X_WGs, -kPenetration * 0.5 + kEps);
...
struct SignedDistanceToPointTestData {
    SignedDistanceToPointTestData(shared_ptr<Shape> geometry_in,
        ...
        : geometry(geometry_in), X_WG(X_WG_in), p_WQ(p_WQ_in), expected_result(expect_in) {}
    ...
    static SignedDistanceToPointTestData Make(shared_ptr<Shape> shape,
        ...
        return SignedDistanceToPointTestData{shape, X_WG, p_WQ,
                                             SignedDistanceToPoint{GeometryId::get_new_id(), p_GN, distance, grad_W}};
    }

    // Google Test uses this operator to report the test data in the log file
    // when a test fails.
    friend std::ostream& operator<<(std::ostream& os, const SignedDistanceToPointTestData& obj) {
        fmt::print(os,
                   "{{\n"
                   ...
                   "}}",
                   fmt_eigen(obj.p_WQ.transpose()), fmt_eigen(obj.expected_result.p_GN.transpose()),
                   obj.expected_result.distance, fmt_eigen(obj.expected_result.grad_W.transpose()));
        return os;
    }
...
std::vector<SignedDistanceToPointTestData> GenDistanceTestDataSphere(
    ...
    std::vector<SignedDistanceToPointTestData> test_data{
            ...
            SignedDistanceToPointTestData::Make(sphere, X_WG, X_WG * Vector3d{2.0, 3.0, 6.0}, Vector3d(0.2, 0.3, 0.6),
                                                6.3, X_WG.rotation() * Vector3d(2.0, 3.0, 6.0) / 7.0),
            // p_GQ = (0.1,0.15,0.3) is inside G at the negative distance -0.35.
            SignedDistanceToPointTestData::Make(sphere, X_WG, X_WG * Vector3d{0.1, 0.15, 0.3}, Vector3d(0.2, 0.3, 0.6),
                                                -0.35, X_WG.rotation() * Vector3d(2.0, 3.0, 6.0) / 7.0),
            // Reports an arbitrary gradient vector (as defined in the
            // QueryObject::ComputeSignedDistanceToPoint() documentation) at the
            // center of the sphere.
            SignedDistanceToPointTestData::Make(sphere, X_WG, X_WG * Vector3d{0.0, 0.0, 0.0}, Vector3d(0.7, 0., 0.),
                                                -0.7, X_WG.rotation() * Vector3d(1.0, 0.0, 0.0))};
    return test_data;
}

std::vector<SignedDistanceToPointTestData> GenDistTestTransformSphere() {
    const RigidTransformd X_WG(RollPitchYawd(M_PI / 6., M_PI / 3., M_PI_2), Vector3d{10., 11., 12.});
...
std::vector<SignedDistanceToPointTestData> GenDistanceTestDataOutsideBox(
        const RigidTransformd& X_WG = RigidTransformd::Identity()) {
    const std::vector<SignedDistanceToPointTestData> test_data_box_boundary = GenDistanceTestDataBoxBoundary(X_WG);
    std::vector<SignedDistanceToPointTestData> test_data;
    for (const auto& data : test_data_box_boundary) {
        ...
        test_data.emplace_back(shape, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, distance, grad_W));
    }
    return test_data;
}

std::vector<SignedDistanceToPointTestData> GenDistTestTransformOutsideBox() {
    RigidTransformd X_WG(RollPitchYawd(M_PI / 3., M_PI / 6., M_PI_2), Vector3d{10., 11., 12.});
...
// the vector s.
std::vector<SignedDistanceToPointTestData> GenDistanceTestDataBoxBoundary(const RigidTransformd& X_WG) {
    ...
    for (const double sx : {-1., 0., 1.}) {
        for (const double sy : {-1., 0., 1.}) {
            for (const double sz : {-1., 0., 1.}) {
                ...
                test_data.emplace_back(box, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, distance, grad_W));
            }
        }
    }
    return test_data;
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataInsideBoxUnique(
    ...
    std::vector<SignedDistanceToPointTestData> test_data;
    for (const Vector3d unit_vector : {Vector3d::UnitX(), Vector3d::UnitY(), Vector3d::UnitZ()}) {
        for (const double sign : {-1., 1.}) {
            ...
            const Vector3d grad_W = R_WG * grad_G;
            test_data.emplace_back(box, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, -d, grad_W));
        }
    }
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestTransformInsideBoxUnique() {
    RigidTransformd X_WG(RollPitchYawd(M_PI / 3., M_PI / 6., M_PI_2), Vector3d{10., 11., 12.});
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataInsideBoxNonUnique() {
    ...
    std::vector<SignedDistanceToPointTestData> test_data{
            ...
            SignedDistanceToPointTestData::Make(box, X_WG, X_WG * Vector3d{6., 1., 2.}, Vector3d(6., 1., 5.), -3.,
                                                X_WG.rotation() * Vector3d(0., 0., 1.)),
            // Q is nearest to two faces {10}x[-5,5]x[-5,5] and [-10,10]x[-5,5]x{5}.
            SignedDistanceToPointTestData::Make(box, X_WG, X_WG * Vector3d{6., 0., 1.}, Vector3d(10., 0., 1.), -4.,
                                                X_WG.rotation() * Vector3d(1., 0., 0.)),
            // Q is nearest to three faces {10}x[-5,5]x[-5,5], [-10,10]x{5}x[-5,5],
            // and [-10,10]x[-5,5]x{5}.
            SignedDistanceToPointTestData::Make(box, X_WG, X_WG * Vector3d{6., 1., 1.}, Vector3d(10., 1., 1.), -4.,
                                                X_WG.rotation() * Vector3d(1., 0., 0.)),
            // Q at the center of the box is nearest to four faces
            // [-10,10]x{5}x[-5,5], [-10,10]x{-5}x[-5,5], [-10,10]x[5,-5]x{5},
            // and [-10,10]x[5,-5]x{-5}.
            SignedDistanceToPointTestData::Make(box, X_WG, X_WG * Vector3d{0., 0., 0.}, Vector3d(0., 5., 0.), -5.,
            ...
            SignedDistanceToPointTestData::Make(box, X_WG, X_WG * Vector3d{5., 0., 0.}, Vector3d(10., 0., 0.), -5.,
                                                X_WG.rotation() * Vector3d(1., 0., 0.))};
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistanceTestDataTranslateBox() {
    ...
    std::vector<SignedDistanceToPointTestData> test_data{
            ...
            SignedDistanceToPointTestData::Make(box, Translation3d(10., 20., 30.), Vector3d{23., 20., 30.},
...
// cylinder geometry G.
std::vector<SignedDistanceToPointTestData> GenDistTestDataCylinderBoundaryCircle(
        const RigidTransformd& X_WG = RigidTransformd::Identity()) {
    const RotationMatrixd& R_WG = X_WG.rotation();
    ...
    for (int c = 0; c < kNumVectors; ++c) {
        all_xy_vectors.push_back(RotationMatrixd(RollPitchYawd(0., 0., 2.0 * M_PI * c / kNumVectors)) * kXVector);
    }
    ...
    for (const auto& z_vector : {Vector3d(0., 0., 1.), Vector3d(0., 0., -1.)}) {
        for (const auto& xy_vector : all_xy_vectors) {
            ...
            const Vector3d grad_W = R_WG * grad_G;
            test_data.emplace_back(cylinder, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, distance, grad_W));
        }
    }
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataCylinderBoundarySurface(
        const RigidTransformd& X_WG = RigidTransformd::Identity()) {
    const RotationMatrixd& R_WG = X_WG.rotation();
    auto cylinder = make_shared<Cylinder>(3.0, 5.0);
    const double radius = cylinder->radius();
    ...
    struct LocalTestData {
        LocalTestData(const Vector3d& p_GQ_in, const Vector3d& grad_G_in) : p_GQ(p_GQ_in), grad_G(grad_G_in) {}
        Vector3d p_GQ;
        Vector3d grad_G;
    };
    ...
    for (const auto& z_vector : {Vector3d(0., 0., 1.), Vector3d(0., 0., -1.)}) {
        ...
        for (const auto& xy_vector : all_xy_vectors) {
            // Q on the barrel.
            {
                ...
                // xy_vector and z_vector are unit vectors in x-y plane and z-axis of G.
                const Vector3d p_GQ = radius * xy_vector + half_length * kZFactor * z_vector;
            ...
            // Q on the top and bottom caps.
            {
                ...
                // xy_vector and z_vector are unit vectors in x-y plane and z-axis of G.
                const Vector3d p_GQ = radius * kRFactor * xy_vector + half_length * z_vector;
    ...
    // test_data.
    auto convert = [&cylinder, &X_WG, &R_WG, &test_data](const LocalTestData& local) {
        ...
        test_data.push_back(SignedDistanceToPointTestData::Make(cylinder, X_WG, p_WQ, p_GN, distance, grad_W));
    };
    ...
    std::for_each(test_data_cap_centers.begin(), test_data_cap_centers.end(), convert);
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataCylinderBoundary(
    ...
    const auto test_data_boundary_surface = GenDistTestDataCylinderBoundarySurface(X_WG);
    for (const auto& data : test_data_boundary_surface) {
        test_data.emplace_back(data.geometry, data.X_WG, data.p_WQ, data.expected_result);
    }
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataOutsideCylinder(
        const RigidTransformd& X_WG = RigidTransformd::Identity()) {
    const auto test_data_cylinder_boundary = GenDistTestDataCylinderBoundary(X_WG);
    std::vector<SignedDistanceToPointTestData> test_data;
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataInsideCylinder(
        const RigidTransformd& X_WG = RigidTransformd::Identity()) {
    const auto test_data_cylinder_boundary_surface = GenDistTestDataCylinderBoundarySurface(X_WG);
    std::vector<SignedDistanceToPointTestData> test_data;
    ...
    for (const auto& data : test_data_cylinder_boundary_surface) {
        ...
        DRAKE_DEMAND(dynamic_cast<Cylinder*>(shape.get()) != nullptr);
        // The gradient grad_W has unit length by construction.
        const Vector3d p_WQ = data.p_WQ + kNegativeDistance * data.expected_result.grad_W;
        const GeometryId& id = data.expected_result.id_G;
        const Vector3d& p_GN = data.expected_result.p_GN;
        const Vector3d& grad_W = data.expected_result.grad_W;
        test_data.emplace_back(shape, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, kNegativeDistance, grad_W));
    }
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataCylinderCenter(
    ...
    const Vector3d grad_W = R_WG * grad_G;
    std::vector<SignedDistanceToPointTestData> test_data;
    test_data.emplace_back(long_cylinder, X_WG, p_WQ, SignedDistanceToPoint<double>(id, p_GN, distance, grad_W));
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataCylinderTransform() {
    RigidTransformd X_WG(RollPitchYawd(M_PI / 3., M_PI / 6., M_PI_2), Vector3d{10., 11., 12.});
    std::vector<SignedDistanceToPointTestData> test_data;
    for (const auto& record : GenDistTestDataCylinderBoundary(X_WG)) test_data.emplace_back(record);
    for (const auto& record : GenDistTestDataOutsideCylinder(X_WG)) test_data.emplace_back(record);
    for (const auto& record : GenDistTestDataInsideCylinder(X_WG)) test_data.emplace_back(record);
    for (const auto& record : GenDistTestDataCylinderCenter(X_WG)) test_data.emplace_back(record);
    return test_data;
}
...
std::vector<SignedDistanceToPointTestData> GenDistTestDataHalfSpace() {
    ...
    const RigidTransformd X_WG1(RollPitchYawd(M_PI / 3., M_PI / 6., M_PI_2), Vector3d{10., 11., 12.});
    ...
    for (const auto& X_WG : {RigidTransformd(), X_WG1}) {
        ...
        test_data.emplace_back(hs, X_WG, p_WQ1, SignedDistanceToPoint<double>(id, p_GN, distance, grad_W));

        // On the half space boundary.
        const Vector3d p_WQ2 = p_WN;
        test_data.emplace_back(hs, X_WG, p_WQ2, SignedDistanceToPoint<double>(id, p_GN, 0.0, grad_W));

        // Inside the half space.
        const Vector3d p_WQ3 = p_WN - distance * grad_W;
        test_data.emplace_back(hs, X_WG, p_WQ3, SignedDistanceToPoint<double>(id, p_GN, -distance, grad_W));
    }
...
struct SignedDistanceToPointTest : public testing::TestWithParam<SignedDistanceToPointTestData> {
    ProximityEngine<double> engine;
    unordered_map<GeometryId, RigidTransformd> X_WGs;
...
TEST_P(SignedDistanceToPointTest, SingleQueryPoint) {
    ...
    EXPECT_TRUE(CompareMatrices(results[0].p_GN, data.expected_result.p_GN, kTolerance)) << "Incorrect nearest point.";
    EXPECT_NEAR(results[0].distance, data.expected_result.distance, kTolerance) << "Incorrect signed distance.";
    EXPECT_TRUE(CompareMatrices(results[0].grad_W, data.expected_result.grad_W, kTolerance))
...
TEST_P(SignedDistanceToPointTest, SingleQueryPointWithThreshold) {
    ...
    auto results = engine.ComputeSignedDistanceToPoint(data.p_WQ, X_WGs, large_threshold);
    // The large threshold allows one object in the results.
    EXPECT_EQ(results.size(), 1);

    const double small_threshold = data.expected_result.distance - 0.01;
    results = engine.ComputeSignedDistanceToPoint(data.p_WQ, X_WGs, small_threshold);
...
TEST_P(SignedDistanceToPointTest, SingleQueryPointSymbolic) {
    ...
    std::unique_ptr<ProximityEngine<Expression>> sym_engine = engine.ToScalarType<Expression>();
    auto sym_results = sym_engine->ComputeSignedDistanceToPoint(
            data.p_WQ.cast<Expression>(), {{data.expected_result.id_G, data.X_WG.cast<Expression>()}}, large_threshold);
...
// TEST_P(SignedDistanceToPointTest, SingleQueryPoint).
// Sphere
INSTANTIATE_TEST_SUITE_P(Sphere, SignedDistanceToPointTest, testing::ValuesIn(GenDistanceTestDataSphere()));

INSTANTIATE_TEST_SUITE_P(TransformSphere, SignedDistanceToPointTest, testing::ValuesIn(GenDistTestTransformSphere()));

// Box
INSTANTIATE_TEST_SUITE_P(OutsideBox, SignedDistanceToPointTest, testing::ValuesIn(GenDistanceTestDataOutsideBox()));
INSTANTIATE_TEST_SUITE_P(BoxBoundary, SignedDistanceToPointTest, testing::ValuesIn(GenDistanceTestDataBoxBoundary()));
INSTANTIATE_TEST_SUITE_P(InsideBoxUnique,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestDataInsideBoxUnique()));
INSTANTIATE_TEST_SUITE_P(InsideBoxNonUnique,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestDataInsideBoxNonUnique()));
INSTANTIATE_TEST_SUITE_P(TranslateBox, SignedDistanceToPointTest, testing::ValuesIn(GenDistanceTestDataTranslateBox()));
INSTANTIATE_TEST_SUITE_P(TransformOutsideBox,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestTransformOutsideBox()));
INSTANTIATE_TEST_SUITE_P(TransformBoxBoundary,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestTransformBoxBoundary()));
INSTANTIATE_TEST_SUITE_P(TransformInsideBoxUnique,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestTransformInsideBoxUnique()));

// Cylinder
INSTANTIATE_TEST_SUITE_P(CylinderBoundary,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestDataCylinderBoundary()));
INSTANTIATE_TEST_SUITE_P(OutsideCylinder,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestDataOutsideCylinder()));
INSTANTIATE_TEST_SUITE_P(InsideCylinder, SignedDistanceToPointTest, testing::ValuesIn(GenDistTestDataInsideCylinder()));
INSTANTIATE_TEST_SUITE_P(CenterCylinder, SignedDistanceToPointTest, testing::ValuesIn(GenDistTestDataCylinderCenter()));
INSTANTIATE_TEST_SUITE_P(CylinderTransform,
                         SignedDistanceToPointTest,
                         testing::ValuesIn(GenDistTestDataCylinderTransform()));

// Half space
INSTANTIATE_TEST_SUITE_P(Halfspace, SignedDistanceToPointTest, testing::ValuesIn(GenDistTestDataHalfSpace()));
...
unordered_map<GeometryId, RigidTransformd> MakeCollidingRing(double radius, int N) {
    /*
                y
                │                   Example of N = 4.
                o
...
protected:
    void SetUp() override {
        ...
        for (const auto& id : ids) {
            engine_.AddDynamicGeometry(sphere, {}, id, (n % 2) ? soft_properties : rigid_properties);
            ++n;
        }
    }
...
TEST_F(ProximityEngineHydro, ComputeContactSurfacesResultOrdering) {
    ...
    const auto results1 = engine_.ComputeContactSurfaces(HydroelasticContactRepresentation::kTriangle, poses_);
    ASSERT_EQ(results1.size(), poses_.size());

    engine_.UpdateWorldPoses(poses_);
    const auto results2 = engine_.ComputeContactSurfaces(HydroelasticContactRepresentation::kTriangle, poses_);
...
protected:
    void SetUp() override {
        ...
        for (const auto& id : ids) {
            bool is_soft = (n % 4) < 2;
            engine_.AddDynamicGeometry(sphere, {}, id, is_soft ? soft_properties : rigid_properties);
            ++n;
        }
    }
...
TEST_F(ProximityEngineHydroWithFallback, ComputeContactSurfacesWithFallbackResultOrdering) {
    engine_.UpdateWorldPoses(poses_);
    vector<ContactSurface<double>> surfaces1;
    vector<PenetrationAsPointPair<double>> points1;
    engine_.ComputeContactSurfacesWithFallback(HydroelasticContactRepresentation::kTriangle, poses_, &surfaces1,
                                               &points1);
    ...
    engine_.ComputeContactSurfacesWithFallback(HydroelasticContactRepresentation::kTriangle, poses_, &surfaces2,
                                               &points2);
...
protected:
    ...
    void MoveDynamicSphere(GeometryId id, bool is_colliding, ProximityEngine<double>* engine = nullptr) {
    ...
    template <typename T>
    void ExpectPenetration(GeometryId origin_sphere, GeometryId colliding_sphere, ProximityEngine<T>* engine) {
        ...
        std::vector<SignedDistancePair<T>> distance_results =
                engine->ComputeSignedDistancePairwiseClosestPoints(GetTypedPoses<T>(), kInf);
        ...
        EXPECT_TRUE((penetration.id_A == origin_sphere && penetration.id_B == colliding_sphere) ||
                    (penetration.id_A == colliding_sphere && penetration.id_B == origin_sphere));

        EXPECT_TRUE((distance.id_A == origin_sphere && distance.id_B == colliding_sphere) ||
        ...
        EXPECT_TRUE(CompareMatrices(penetration.p_WCa, expected.p_WCa, 1e-13, MatrixCompareType::absolute));
        EXPECT_TRUE(CompareMatrices(penetration.p_WCb, expected.p_WCb, 1e-13, MatrixCompareType::absolute));
        EXPECT_TRUE(CompareMatrices(penetration.nhat_BA_W, expected.nhat_BA_W, 1e-13, MatrixCompareType::absolute));
        ...
        CompareSignedDistancePair(distance, expected_distance, std::numeric_limits<double>::epsilon());
    }

    // The two spheres collides, but are ignored due to the setting in the
    // collision filter.
    template <typename T>
    void ExpectIgnoredPenetration(GeometryId origin_sphere, GeometryId colliding_sphere, ProximityEngine<T>* engine) {
        std::vector<PenetrationAsPointPair<T>> penetration_results =
                engine->ComputePointPairPenetration(GetTypedPoses<T>());
    ...
    template <typename T>
    void ExpectNoPenetration(GeometryId origin_sphere, GeometryId colliding_sphere, ProximityEngine<T>* engine) {
...
TEST_F(SimplePenetrationTest, PenetrationDynamicAndAnchored) {
    ...
    std::unique_ptr<ProximityEngine<AutoDiffXd>> ad_engine = engine_.ToScalarType<AutoDiffXd>();
    ExpectPenetration(anchored_id, dynamic_id, ad_engine.get());
    std::unique_ptr<ProximityEngine<Expression>> sym_engine = engine_.ToScalarType<Expression>();
    DRAKE_EXPECT_THROWS_MESSAGE(ExpectPenetration(anchored_id, dynamic_id, sym_engine.get()),
...
TEST_F(SimplePenetrationTest, PenetrationDynamicAndDynamicSingleSource) {
    ...
    std::unique_ptr<ProximityEngine<Expression>> sym_engine = engine_.ToScalarType<Expression>();
    DRAKE_EXPECT_THROWS_MESSAGE(ExpectPenetration(origin_id, collide_id, sym_engine.get()),
                                ".*are not supported for scalar type drake::symbolic::Expression.*");
}
...
TEST_F(SimplePenetrationTest, WithCollisionFilters) {
    ...
    // I know the GeometrySet only has id_A and id_B, so I'll construct the
    // extracted set by hand.
    auto extract_ids = [origin_id, collide_id](const GeometrySet&, CollisionFilterScope) {
        return std::unordered_set<GeometryId>{origin_id, collide_id};
    };
    engine_.collision_filter().Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{origin_id, collide_id}),
                                     extract_ids, false /* is_invariant */);

    EXPECT_FALSE(engine_.collision_filter().CanCollideWith(origin_id, collide_id));
...
GTEST_TEST(ProximityEngineTests, PairwiseSignedDistanceNonPositiveThreshold) {
    ...
    const double kRadius = 0.5;
    const unordered_map<GeometryId, RigidTransformd> X_WGs{{id1, RigidTransformd{Translation3d{0, 2 * kRadius, 0}}},
    ...
    std::vector<SignedDistancePair<double>> results_zero = engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, 0);
    ...
    std::vector<SignedDistancePair<double>> results_barely_in =
            engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, penetration + kEps);
...
public:
    SignedDistancePairTestData(shared_ptr<const Shape> a,
        ...
        : a_(a), b_(b), X_WA_(X_WA), X_WB_(X_WB), expected_result_(expect), tolerance_(tolerance) {}
    ...
    static SignedDistancePairTestData Make(shared_ptr<const Shape> a,
                                           shared_ptr<const Shape> b,
                                           const RigidTransformd& X_WA,
                                           const RigidTransformd& X_WB,
                                           const Vector3d& p_ACa_A,
                                           const Vector3d& p_BCb_B,
                                           double distance,
        ...
        return {a,
                ...
                X_WA,
                X_WB,
                SignedDistancePair<double>(GeometryId::get_new_id(), GeometryId::get_new_id(), p_ACa_A, p_BCb_B,
                                           distance, nhat_BA_W),
                tolerance};
    }
    ...
    SignedDistancePairTestData GenSwapAB() const {
        ...
        return SignedDistancePairTestData(
                b_, a_, X_WB_, X_WA_,
                SignedDistancePair<double>(id_B, id_A, p_BCb, p_ACa, distance, -expected_result_.nhat_BA_W),
                tolerance_);
    }

    // Google Test uses this operator to report the test data in the log file
    // when a test fails.
    friend std::ostream& operator<<(std::ostream& os, const SignedDistancePairTestData& obj) {
        fmt::print(os,
                   "{{\n"
                   ...
                   "}}",
                   obj.expected_result_.id_A, obj.expected_result_.id_B, obj.expected_result_.distance,
...
std::vector<SignedDistancePairTestData> GenDistancePairTestSphereSphere(
    ...
    for (const auto& config : configurations) {
        ...
        test_data.emplace_back(SignedDistancePairTestData::Make(sphere_A, sphere_B, X_WA, X_WB, p_ACa_A, p_BCb_B,
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereTransform() {
    return GenDistancePairTestSphereSphere(
            RigidTransformd(RollPitchYawd(5. * M_PI / 8., M_PI / 6., M_PI / 12.), Vector3d(1., 2., 3.)),
            RotationMatrixd(RollPitchYawd(M_PI / 4., 5. * M_PI / 6., M_PI / 3)));
}

// Gimbal lock of the orientation of the sphere at pitch = pi/2.
std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereGimbalLock() {
    return GenDistancePairTestSphereSphere(RigidTransformd::Identity(),
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereNonAligned(
    ...
    std::vector<SignedDistancePairTestData> test_data{
            SignedDistancePairTestData::Make(sphere_A, sphere_B, X_WA, X_WB, p_ACa, p_BCb, -radius_A)};
    return test_data;
}

std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereNonAlignedTransform() {
    return GenDistPairTestSphereSphereNonAligned(
            RigidTransformd(RollPitchYawd(3. * M_PI / 8., 5 * M_PI / 6., M_PI / 12.), Vector3d(1., 2., 3.)),
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereBox(
    ...
    const std::vector<Configuration> configurations{// The sphere is outside the box.
                                                    {Vector3d(14., 0., 0.), 4.},
    ...
    for (auto config : configurations) {
        ...
        test_data.emplace_back(
                SignedDistancePairTestData::Make(sphere_A, box_B, X_WA, X_WB, p_ACa, p_BCb, config.pair_distance));
    }
    return test_data;
}
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereBoxGimbalLock() {
    return GenDistPairTestSphereBox(RotationMatrixd(RollPitchYawd(M_PI / 8., M_PI_2, M_PI / 8)),
    ...
    for (const double sign_x : {-1., 0., 1.}) {
        for (const double sign_y : {-1., 0., 1.}) {
            for (const double sign_z : {-1., 0., 1.}) {
                ...
                test_data.emplace_back(
                        SignedDistancePairTestData::Make(sphere_A, box_B, X_WA, X_WB, p_ACa, p_BCb, -radius_A));
            }
        }
    }
    return test_data;
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereBoxBoundaryTransform() {
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereCapsule(
    ...
    const std::vector<double> pair_distances{// The sphere is outside the capsule.
                                             radius_A * 2.,
    ...
    for (const auto& pair_distance : pair_distances) {
        const Vector3d p_BAo = Vector3d(0., 0., pair_distance + half_length + radius_A);
        // Set up the pose of A from the translation vector in the configuration
        // and the given rotation parameter R_WA.
        const Vector3d p_WAo = X_WB * p_BAo;
        const RigidTransformd X_WA(R_WA, p_WAo);
        ...
        const Vector3d p_ACa = X_AB * p_BCa;
        test_data.emplace_back(
                SignedDistancePairTestData::Make(sphere_A, capsule_B, X_WA, X_WB, p_ACa, p_BCb, pair_distance));
    }
    return test_data;
}

std::vector<SignedDistancePairTestData> GenDistPairTestSphereCapsuleTransform() {
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereCylinder(
    ...
    const std::vector<Configuration> configurations{// The sphere is outside the cylinder.
                                                    {Vector3d(0., 0., 14.), 4.},
    ...
    for (const auto& config : configurations) {
        ...
        const Vector3d p_ACa = X_AB * p_BCa;
        test_data.emplace_back(
                SignedDistancePairTestData::Make(sphere_A, cylinder_B, X_WA, X_WB, p_ACa, p_BCb, config.pair_distance));
    }
    return test_data;
}

std::vector<SignedDistancePairTestData> GenDistPairTestSphereCylinderTransform() {
    ...
    for (const double i : {-2., -1., 0., 1., 2.}) {
        for (const double j : {-2., -1., 0., 1., 2.}) {
            for (const double k : {-2., -1., 0., 1., 2.}) {
                ...
                const Vector3d p_AoCa_B = ((abs_u == 1. || abs_v == 1.) && abs_w == 1.)
                ...
                test_data.emplace_back(
                        SignedDistancePairTestData::Make(sphere_A, cylinder_B, X_WA, X_WB, p_ACa, p_BCb, -r_A));
            }
        }
    }
    return test_data;
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereCylinderBoundaryTransform() {
...
std::vector<SignedDistancePairTestData> GenDistPairTestHalfspaceSphere() {
    ...
    for (const auto& X_WH : {RigidTransformd(), X_WG1}) {
        ...
        // Outside the half space (with permuted ordering).
        {
            ...
            SignedDistancePair<double> expected(sphere_id, half_space_id, p_SCs, p_HCh, distance, nhat_HS_W);
        ...
        // On the half space boundary.
        {
            ...
            const RigidTransformd X_WS(p_WS);
            const Vector3d p_SCs = -radius * (R_WS.inverse() * nhat_HS_W);
            SignedDistancePair<double> expected(sphere_id, half_space_id, p_SCs, p_HCh, 0, nhat_HS_W);
            test_data.emplace_back(sphere, half_space, X_WS, X_WH, expected);
            expected.SwapAAndB();
            test_data.emplace_back(half_space, sphere, X_WH, X_WS, expected);
        }
...
class SignedDistancePairTest : public testing::TestWithParam<SignedDistancePairTestData> {
public:
    SignedDistancePairTest() {
        const auto& data = GetParam();
        engine_.AddAnchoredGeometry(*data.a_, data.X_WA_, data.expected_result_.id_A);
...
TEST_P(SignedDistancePairTest, SinglePair) {
    const auto& data = GetParam();
    const auto results = engine_.ComputeSignedDistancePairwiseClosestPoints(X_WGs_, kInf);
    ASSERT_EQ(results.size(), 1);
    const auto& result = results[0];
    const double tolerance = data.tolerance_ ? data.tolerance_ : kTolerance;

    EXPECT_NEAR(result.distance, data.expected_result_.distance, tolerance) << "Incorrect signed distance";

    const bool a_then_b = (result.id_A == data.expected_result_.id_A) && (result.id_B == data.expected_result_.id_B);
    const bool b_then_a = (result.id_B == data.expected_result_.id_A) && (result.id_A == data.expected_result_.id_B);
    ...
    EXPECT_TRUE(CompareMatrices(p_ACa, data.expected_result_.p_ACa, tolerance)) << "Incorrect witness point.";
    EXPECT_TRUE(CompareMatrices(p_BCb, data.expected_result_.p_BCb, tolerance)) << "Incorrect witness point.";
...
INSTANTIATE_TEST_SUITE_P(SphereSphere, SignedDistancePairTest, testing::ValuesIn(GenDistancePairTestSphereSphere()));
INSTANTIATE_TEST_SUITE_P(SphereSphereTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereSphereTransform()));
INSTANTIATE_TEST_SUITE_P(SphereSphereGimbalLock,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereSphereGimbalLock()));

INSTANTIATE_TEST_SUITE_P(SphereSphreNonAligned,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereSphereNonAligned()));
INSTANTIATE_TEST_SUITE_P(SphereSphreNonAlignedTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereSphereNonAlignedTransform()));

INSTANTIATE_TEST_SUITE_P(SphereBox, SignedDistancePairTest, testing::ValuesIn(GenDistPairTestSphereBox()));
INSTANTIATE_TEST_SUITE_P(SphereBoxTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereBoxTransform()));
INSTANTIATE_TEST_SUITE_P(SphereBoxGimbalLock,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereBoxGimbalLock()));

INSTANTIATE_TEST_SUITE_P(BoxSphere, SignedDistancePairTest, testing::ValuesIn(GenDistPairTestBoxSphere()));
INSTANTIATE_TEST_SUITE_P(BoxSphereTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestBoxSphereTransform()));

INSTANTIATE_TEST_SUITE_P(SphereBoxBoundary,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereBoxBoundary()));
INSTANTIATE_TEST_SUITE_P(SphereBoxBoundaryTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereBoxBoundaryTransform()));

INSTANTIATE_TEST_SUITE_P(SphereCapsule, SignedDistancePairTest, testing::ValuesIn(GenDistPairTestSphereCapsule()));
INSTANTIATE_TEST_SUITE_P(SphereCapsuleTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereCapsuleTransform()));

INSTANTIATE_TEST_SUITE_P(SphereCylinder, SignedDistancePairTest, testing::ValuesIn(GenDistPairTestSphereCylinder()));
INSTANTIATE_TEST_SUITE_P(SphereCylinderTransform,
                         SignedDistancePairTest,
                         testing::ValuesIn(GenDistPairTestSphereCylinderTransform()));
INSTANTIATE_TEST_SUITE_P(SphereCylinderBoundary,
                         SignedDistancePairTest,
...
INSTANTIATE_TEST_SUITE_P(HalfspaceSphere, SignedDistancePairTest, testing::ValuesIn(GenDistPairTestHalfspaceSphere()));
...
GTEST_TEST(SignedDistancePairError, HalfspaceException) {
    ...
    HalfSpace halfspace;
    engine.AddAnchoredGeometry(halfspace, RigidTransformd::Identity(), GeometryId::get_new_id());

    DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, kInf),
...
TEST_P(SignedDistancePairConcentricTest, DistanceInvariance) {
    ...
    EXPECT_NEAR(result.distance, data.expected_result_.distance, kTolerance) << "Incorrect signed distance";
...
SignedDistancePairTestData GenDistPairTestTwoSpheresConcentricBasic(
    ...
    return SignedDistancePairTestData(
            SignedDistancePairTestData::Make(sphere_A, sphere_B, X_WA, X_WB, p_ACa, p_BCb, -radius_A - radius_B));
}
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereConcentricTransform() {
...
// Gimbal lock of the orientation of the sphere at pitch = pi/2.
std::vector<SignedDistancePairTestData> GenDistPairTestSphereSphereConcentricGimbalLock() {
    return GenDistPairTestSphereSphereConcentric(RigidTransformd::Identity(),
...
std::vector<SignedDistancePairTestData> GenDistPairTestSphereBoxConcentric(
    ...
    std::vector<SignedDistancePairTestData> test_data{
            SignedDistancePairTestData::Make(sphere_A, box_B, X_WA, X_WB, p_ACa, p_BCb, -radius_A - half_B(0))};
    return test_data;
}

std::vector<SignedDistancePairTestData> GenDistPairTestSphereBoxConcentricTransform() {
...
GTEST_TEST(ProximityEngineCollisionTest, SpherePunchThroughBox) {
    ...
    unordered_map<GeometryId, RigidTransformd> poses{{box_id, RigidTransformd::Identity()},
  ...
  std::vector<SpherePunchData> test_data{
    ...
    for (const auto& test : test_data) {
        ...
        engine.UpdateWorldPoses(poses);
        std::vector<PenetrationAsPointPair<double>> results = engine.ComputePointPairPenetration(poses);
        ...
        if (test.contact_count == 1) {
            ...
            EXPECT_TRUE(CompareMatrices(penetration.nhat_BA_W, test.contact_normal, eps, MatrixCompareType::absolute))
                    << "Failed for the " << test.description << " case";
            // For this simple, axis-aligned test (where all the values are nicely
            // powers of 2), I should expect perfect answers.
            EXPECT_EQ(penetration.depth - test.depth, 0) << "Failed for the " << test.description << " case";
        }
    }
}
...
protected:
    void SetUp() {
        ...
        for (auto func : {X_WB_1, X_WB_2}) {
            ...
            EXPECT_TRUE(CompareMatrices(p_WC_test, p_WC_, 1e-15, MatrixCompareType::absolute));
...
private:
    ...
    void TestCollision(TangentShape shape_type, double tolerance, const math::RigidTransformd& X_WB) {
        ...
        unordered_map<GeometryId, RigidTransformd> poses{{tangent_id, shape_pose(shape_type)}, {box_id, X_WB}};
        engine_.UpdateWorldPoses(poses);
        std::vector<PenetrationAsPointPair<double>> results = engine_.ComputePointPairPenetration(poses);

        ASSERT_EQ(results.size(), 1u) << "Against tangent " << shape_name(shape_type);
        ...
        if (contact.id_A == tangent_id && contact.id_B == box_id) {
            ...
            GTEST_FAIL() << "Wrong geometry ids reported in contact for tangent " << shape_name(shape_type)
                         << ". Expected " << tangent_id << " and " << box_id << ". Got " << contact.id_A << " and "
                         << contact.id_B;
        }
        EXPECT_TRUE(CompareMatrices(contact.nhat_BA_W, normal, tolerance))
                << "Against tangent " << shape_name(shape_type);
        EXPECT_TRUE(CompareMatrices(contact.p_WCa, p_Ac, tolerance)) << "Against tangent " << shape_name(shape_type);
        EXPECT_TRUE(CompareMatrices(contact.p_WCb, p_Bc, tolerance)) << "Against tangent " << shape_name(shape_type);
    ...
    static math::RigidTransformd X_WB_2(const Vector3d& p_BoC_B, const Vector3d& p_WC) {
        ...
        const math::RotationMatrixd R_WB(AngleAxisd(std::atan(M_SQRT2), Vector3d(M_SQRT1_2, -M_SQRT1_2, 0)));
    ...
    static math::RigidTransformd X_WB_1(const Vector3d& p_BoC_B, const Vector3d& p_WC) {
        // Compute the pose of the colliding box.
        // a. Orient the box so that the corner p_BoC_B = (-0.5, -0.5, -0.5) lies in
        //    the most -z extent. With only rotation, p_BoC_B != p_WC.
    ...
    const Shape& shape(TangentShape shape) {
        ...
        throw std::logic_error("Trying to acquire shape for unknown shape enumerated value: " + std::to_string(shape));
    }
    ...
    RigidTransformd shape_pose(TangentShape shape) {
        ...
        switch (shape) {
            ...
            case TangentProneCylinder:
            case TangentProneCapsule:
                pose = RigidTransformd(AngleAxisd{M_PI_2, Vector3d::UnitX()}, Vector3d{0, 0, -kRadius});
                break;
        }
        return pose;
    }
    ...
    const Convex tangent_convex_{drake::FindResourceOrThrow("drake/geometry/test/quad_cube.obj"), 5.0};
...
GTEST_TEST(ProximityEngineTests, Issue10577Regression_Osculation) {
    ...
    RigidTransformd X_WA(Eigen::AngleAxisd{M_PI_2, Vector3d::UnitZ()}, Vector3d{-0.25, 0, 0});
    RigidTransformd X_WB(Vector3d{0, 0, 0.0085});
    const unordered_map<GeometryId, RigidTransformd> X_WG{{id_A, X_WA}, {id_B, X_WB}};
...
GTEST_TEST(ProximityEngineTests, AnchoredBroadPhaseInitialization) {
    ...
    std::unordered_map<GeometryId, math::RigidTransform<double>> X_WGs{{id_A, X_WA}, {id_D, X_WD}};
    engine.UpdateWorldPoses(X_WGs);
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffAnchored) {
    ...
    // Against an anchored sphere.
    {
        ...
        const unordered_map<GeometryId, RigidTransform<AutoDiffXd>> X_WGs{{anchored_id, X_WS_ad}};

        // Distance is just beyond the threshold.
        {
            std::vector<SignedDistanceToPoint<AutoDiffXd>> results =
                    engine.ComputeSignedDistanceToPoint(p_WQ_ad, X_WGs, expected_distance - 1e-14);
            EXPECT_EQ(results.size(), 0);
        }
...
GTEST_TEST(ProximityEngineTests, ComputePointSignedDistanceAutoDiffDynamic) {
    ...
    const unordered_map<GeometryId, RigidTransform<AutoDiffXd>> X_WGs{{id, X_WS_ad}};
    engine.UpdateWorldPoses(X_WGs);
...
GTEST_TEST(ProximityEngineTests, ComputePairwiseSignedDistanceAutoDiff) {
    ...
    const unordered_map<GeometryId, RigidTransform<AutoDiffXd>> X_WGs{{id1, X_WS1_ad}, {id2, X_WS2_ad}};
    engine.UpdateWorldPoses(X_WGs);
...
GTEST_TEST(ProximityEngineTests, ComputePairwiseSignedDistanceAutoDiffUnsupported) {
    ProximityEngine<AutoDiffXd> engine;
    ...
    const unordered_map<GeometryId, RigidTransform<AutoDiffXd>> X_WGs{{id1, RigidTransform<AutoDiffXd>::Identity()},
...
GTEST_TEST(ProximityEngineTests, ExpressionUnsupported) {
    ...
    const unordered_map<GeometryId, RigidTransform<Expression>> X_WGs{{id1, RigidTransform<Expression>::Identity()},
                                                                      {id2, RigidTransform<Expression>::Identity()}};
    DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputeSignedDistancePairwiseClosestPoints(X_WGs, kInf),
                                "Signed distance queries between shapes 'Box' and 'Box' are not "
                                "supported for scalar type drake::symbolic::Expression.*");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputeSignedDistancePairClosestPoints(id1, id2, X_WGs),
                                "Signed distance queries between shapes 'Box' and 'Box' are not "
                                "supported for scalar type drake::symbolic::Expression.*");
    DRAKE_EXPECT_THROWS_MESSAGE(engine.ComputePointPairPenetration(X_WGs),
...
protected:
    ...
    ProximityEngineDeformableContactTest() {
        const internal::deformable::Geometries& geometries = deformable_contact_geometries();
    ...
    const internal::deformable::DeformableGeometry& deformable_geometry(GeometryId id) {
        return internal::deformable::GeometriesTester::get_deformable_geometry(deformable_contact_geometries(), id);
    }
    ...
    const internal::deformable::RigidGeometry& rigid_geometry(GeometryId id) {
        return internal::deformable::GeometriesTester::get_rigid_geometry(deformable_contact_geometries(), id);
    }
    ...
    GeometryId AddDeformableSphere() {
        ...
        const VolumeMesh<double> input_mesh =
                MakeSphereVolumeMesh<double>(sphere, kResolutionHint, TessellationStrategy::kDenseInteriorVertices);
        const GeometryId id = GeometryId::get_new_id();
    ...
    void TestRigidRegistrationAndRemoval(const Shape& shape, const ProximityProperties& props, bool expected_status) {
...
TEST_F(ProximityEngineDeformableContactTest, AddSupportedRigidGeometries) {
    ...
    {
        Mesh mesh(drake::FindResourceOrThrow("drake/geometry/test/quad_cube.obj"), 1.0);
...
TEST_F(ProximityEngineDeformableContactTest, AddUnsupportedRigidGeometries) {
    ProximityProperties valid_props = MakeProximityPropsWithRezHint(1.0);
    HalfSpace half_space;
    EXPECT_NO_THROW(TestRigidRegistrationAndRemoval(half_space, valid_props, false));
}
...
TEST_F(ProximityEngineDeformableContactTest, ReplacePropertiesRigid) {
    ...
    InternalGeometry sphere(SourceId::get_new_id(), make_unique<Sphere>(kSphereRadius), FrameId::get_new_id(),
                            GeometryId::get_new_id(), "sphere", X_FG);

    // Note: The order of these tests matter; one builds on the next. Re-ordering
    // *may* break the test.

    // Case: throws when the id doesn't refer to a valid geometry.
    DRAKE_EXPECT_THROWS_MESSAGE(engine_.UpdateRepresentationForNewProperties(sphere, {}),
                                "The proximity engine does not contain a geometry with the id \\d+; its "
                                "properties cannot be updated");
    ...
    // declarations (i.e. no resolution hint).
    {
        ...
        DRAKE_EXPECT_NO_THROW(engine_.UpdateRepresentationForNewProperties(sphere, {}));
        EXPECT_FALSE(deformable_contact_geometries().is_rigid(sphere.id()));
    }
    ...
    // should add a new deformable contact rigid representation.
    {
        ...
        DRAKE_EXPECT_NO_THROW(engine_.UpdateRepresentationForNewProperties(sphere, props));
    ...
    // of the geometry won't change.
    {
        const deformable::RigidGeometry geometry_old =
                deformable::GeometriesTester::get_rigid_geometry(deformable_contact_geometries(), sphere.id());
        const RigidTransformd X_WG_old = geometry_old.pose_in_world();
        const int num_vertices_old = geometry_old.rigid_mesh().mesh().num_vertices();
        ...
        const int num_vertices_new = geometry_new.rigid_mesh().mesh().num_vertices();
    ...
    {
        EXPECT_TRUE(deformable_contact_geometries().is_rigid(sphere.id()));
        DRAKE_EXPECT_NO_THROW(engine_.UpdateRepresentationForNewProperties(sphere, ProximityProperties()));
        EXPECT_FALSE(deformable_contact_geometries().is_rigid(sphere.id()));
    }
}
...
TEST_F(ProximityEngineDeformableContactTest, ReplacePropertiesDeformable) {
    InternalGeometry sphere(SourceId::get_new_id(), make_unique<Sphere>(kSphereRadius), FrameId::get_new_id(),
                            GeometryId::get_new_id(), "sphere", RigidTransformd{}, 1.0);
    ...
    const internal::deformable::DeformableGeometry& old_geometry = deformable_geometry(sphere.id());
    DRAKE_EXPECT_NO_THROW(engine_.UpdateRepresentationForNewProperties(sphere, {}));
    EXPECT_TRUE(deformable_contact_geometries().is_deformable(sphere.id()));
    const internal::deformable::DeformableGeometry& new_geometry = deformable_geometry(sphere.id());
    // Verify that the content didn't change.
    EXPECT_TRUE(old_geometry.deformable_mesh().mesh().Equal(new_geometry.deformable_mesh().mesh()));
    EXPECT_TRUE(old_geometry.deformable_mesh().bvh().Equal(new_geometry.deformable_mesh().bvh()));
    EXPECT_TRUE(old_geometry.CalcSignedDistanceField().Equal(new_geometry.CalcSignedDistanceField()));
    // Verify that the address didn't change either; so it's indeed an no-op.
    EXPECT_EQ(&old_geometry.deformable_mesh().mesh(), &new_geometry.deformable_mesh().mesh());
    EXPECT_EQ(&old_geometry.deformable_mesh().bvh(), &new_geometry.deformable_mesh().bvh());
}
...
TEST_F(ProximityEngineDeformableContactTest, AddAndRemoveDeformableGeometry) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine_.RemoveDeformableGeometry(id), ".*does not contain a deformable geometry.*");
}
...
TEST_F(ProximityEngineDeformableContactTest, UpdateDeformablePositions) {
    ...
    Sphere sphere(kSphereRadius);
    const VolumeMesh<double> input_mesh =
            MakeSphereVolumeMesh<double>(sphere, kRezHint, TessellationStrategy::kDenseInteriorVertices);
    const GeometryId id = GeometryId::get_new_id();
    engine_.AddDeformableGeometry(input_mesh, id);
    /* Update the vertex positions to some arbitrary value. */
    const VectorX<double> q = VectorX<double>::LinSpaced(3 * input_mesh.num_vertices(), 0.0, 1.0);
    engine_.UpdateDeformableVertexPositions({{id, q}});
    const VolumeMesh<double>& mesh = deformable_geometry(id).deformable_mesh().mesh();
...
TEST_F(ProximityEngineDeformableContactTest, ComputeDeformableContact) {
    ...
    DeformableContactSurface<double> contact_surface = contact_data.contact_surfaces()[0];
    ...
    const VolumeMesh<double>& mesh = deformable_geometry(deformable_id).deformable_mesh().mesh();
...
GTEST_TEST(ProximityEngineTests, NeedsConvexHull) {
    ...
    EXPECT_TRUE(
            engine.NeedsConvexHull(InternalGeometry(s_id, make_unique<Mesh>("unimportant", 1.0), f_id, g_id, "n", {})));
    EXPECT_TRUE(engine.NeedsConvexHull(
            InternalGeometry(s_id, make_unique<Convex>("unimportant", 1.0), f_id, g_id, "n", {})));

    // Being deformable would eliminate the need for a convex hull.
    EXPECT_FALSE(engine.NeedsConvexHull(
            InternalGeometry(s_id, make_unique<Mesh>(FindResourceOrThrow("drake/geometry/test/one_tetrahedron.vtk"), 1),
                             f_id, g_id, "n", {}, 1.0)));
}
...
GTEST_TEST(ProximityEngineTests, ImplementedAsFclConvex) {
    ...
    const std::string obj_path = FindResourceOrThrow("drake/geometry/test/cube_corners.obj");

    auto expect_fcl_convex_is_cube = [&engine](GeometryId id) {
        const fcl::CollisionObjectd* object = ProximityEngineTester::GetCollisionObject(engine, id);
        DRAKE_DEMAND(object != nullptr);
        ASSERT_EQ(object->getNodeType(), fcl::GEOM_CONVEX);
        const fcl::Convexd* fcl_shape = dynamic_cast<const fcl::Convexd*>(object->collisionGeometry().get());
