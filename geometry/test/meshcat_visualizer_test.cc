...
protected:
    ...
    void SetUpDiagram(MeshcatVisualizerParams params = {}) {
        ...
        auto zero_torque = builder.template AddSystem<systems::ConstantVectorSource<double>>(Eigen::VectorXd::Zero(7));
        builder.Connect(zero_torque->get_output_port(), plant.get_actuation_input_port());

        visualizer_ = &MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat_, std::move(params));
...
TEST_F(MeshcatVisualizerWithIiwaTest, BasicTest) {
    ...
    EXPECT_EQ(meshcat_->GetPackedProperty("/drake/visualizer", "visible").size(), 0);
    ...
    for (int link = 0; link < 8; link++) {
        EXPECT_NE(meshcat_->GetPackedTransform(fmt::format("/drake/visualizer/iiwa14/iiwa_link_{}", link)), "");
    }
    CheckVisible("/drake/visualizer", true);

    // Confirm that the transforms change after running a simulation.
    const std::string packed_X_W7 = meshcat_->GetPackedTransform("visualizer/iiwa14/iiwa_link_7");
    systems::Simulator<double> simulator(*diagram_);
    simulator.AdvanceTo(0.1);
    EXPECT_NE(meshcat_->GetPackedTransform("visualizer/iiwa14/iiwa_link_7"), packed_X_W7);
}
...
TEST_F(MeshcatVisualizerWithIiwaTest, Roles) {
    ...
    for (Role role : {Role::kProximity, Role::kIllustration, Role::kPerception}) {
        ...
        FrameId iiwa_link_7 = plant_->GetBodyFrameIdOrThrow(plant_->GetBodyByName("iiwa_link_7").index());
        for (GeometryId geom_id : inspector.GetGeometries(iiwa_link_7, role)) {
            EXPECT_TRUE(meshcat_->HasPath(
                    fmt::format("visualizer/iiwa14/iiwa_link_7/{}", TransformGeometryName(geom_id, inspector))));
        }
        meshcat_->Delete();
    }
...
TEST_F(MeshcatVisualizerWithIiwaTest, DeletePrefixOnInitialization) {
    ...
    meshcat_->SetTransform("/drake/visualizer/my_random_path", math::RigidTransformd());
    ...
    {  // Send an initialization event.
        ...
        const systems::EventStatus status = diagram_->Publish(*context_, events->get_publish_events());
...
bool has_iiwa_frame(const MeshcatAnimation& animation, int frame) {
    return animation.get_key_frame<std::vector<double>>(0, "visualizer/iiwa14/iiwa_link_1", "position").has_value();
}
...
TEST_F(MeshcatVisualizerWithIiwaTest, Recording) {
    ...
    diagram_->ForcedPublish(*context_);
    EXPECT_TRUE(has_iiwa_frame(*animation, 0));
    EXPECT_TRUE(has_iiwa_frame(*animation, std::floor(1.0 / params.publish_period)));
...
TEST_F(MeshcatVisualizerWithIiwaTest, RecordingWithoutSetTransform) {
    SetUpDiagram();
    diagram_->ForcedPublish(*context_);
    std::string X_7_message = meshcat_->GetPackedTransform("/drake/visualizer/iiwa14/iiwa_link_7");

    // Now update the position of the iiwa.
    plant_->SetPositions(&plant_->GetMyMutableContextFromRoot(context_.get()), Eigen::VectorXd::Constant(7, .1));
    ...
    diagram_->ForcedPublish(*context_);
    EXPECT_EQ(meshcat_->GetPackedTransform("/drake/visualizer/iiwa14/iiwa_link_7"), X_7_message);

    set_transforms_while_recording = true;
    visualizer_->StartRecording(set_transforms_while_recording);
    // This publish *should* change the transform in the Meshcat scene tree.
    diagram_->ForcedPublish(*context_);
    EXPECT_NE(meshcat_->GetPackedTransform("/drake/visualizer/iiwa14/iiwa_link_7"), X_7_message);
}
...
GTEST_TEST(MeshcatVisualizerTest, HydroGeometry) {
    ...
    for (bool show_hydroelastic : {false, true}) {
        ...
        systems::DiagramBuilder<double> builder;
        auto [plant, scene_graph] = AddMultibodyPlantSceneGraph(&builder, 0.001);
        multibody::Parser(&plant).AddModelsFromUrl("package://drake/multibody/meshcat/test/hydroelastic.sdf");
        plant.Finalize();
        ...
        MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat, params);
        ...
        const std::string data = meshcat->GetPackedObject(
                fmt::format("/drake/{}/two_bodies/body1/{}", prefix, TransformGeometryName(sphere1, inspector)));
...
GTEST_TEST(MeshcatVisualizerTest, ConvexHull) {
    ...
    systems::DiagramBuilder<double> builder;
    auto [plant, scene_graph] = AddMultibodyPlantSceneGraph(&builder, 0.001);
    multibody::Parser(&plant).AddModelsFromUrl("package://drake/geometry/render/test/box.sdf");
    plant.Finalize();

    // Dig out a GeometryId that we just loaded.
    const auto& inspector = scene_graph.model_inspector();
    const GeometryId box_id = inspector.GetAllGeometryIds(Role::kProximity).front();
    ...
    ASSERT_TRUE(std::holds_alternative<std::monostate>(inspector.maybe_get_hydroelastic_mesh(box_id)));
    ...
    MeshcatVisualizerParams params{.role = Role::kProximity, .show_hydroelastic = true};
    MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat, params);
    ...
    const std::string data = meshcat->GetPackedObject(
            fmt::format("/drake/{}/box/box/{}", params.prefix, TransformGeometryName(box_id, inspector)));
...
GTEST_TEST(MeshcatVisualizerTest, MultipleModels) {
    ...
    plant.WeldFrames(plant.world_frame(), plant.GetBodyByName("base", iiwa0).body_frame());
    auto iiwa1 = multibody::Parser(&plant, "second").AddModelsFromUrl(urdf_url).at(0);
    plant.WeldFrames(plant.world_frame(), plant.GetBodyByName("base", iiwa1).body_frame());
    plant.Finalize();

    auto zero_torque = builder.AddSystem<systems::ConstantVectorSource>(Eigen::VectorXd::Zero(7));
    builder.Connect(zero_torque->get_output_port(), plant.get_actuation_input_port(iiwa0));
    builder.Connect(zero_torque->get_output_port(), plant.get_actuation_input_port(iiwa1));

    // Use the query_output_port version of AddToBuilder.
    MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph.get_query_output_port(), meshcat);
    ...
    for (int link = 0; link < 8; link++) {
        EXPECT_NE(meshcat->GetPackedTransform(fmt::format("/drake/visualizer/iiwa14/iiwa_link_{}", link)), "");
        EXPECT_NE(meshcat->GetPackedTransform(fmt::format("/drake/visualizer/second/iiwa14/iiwa_link_{}", link)), "");
    }
}
...
GTEST_TEST(MeshcatVisualizerTest, AcceptingProperty) {
    for (bool include_unspecified_accepting : {true, false}) {
        for (const std::string accepting : {"", "prefix", "no_match"}) {
            SCOPED_TRACE(fmt::format("include_unspecified = {}, accepting_str = {}", include_unspecified_accepting,
            ...
            const FrameId body_frame = plant.GetBodyFrameIdOrThrow(plant.GetBodyByName("box").index());
            const auto geom_ids = inspector.GetGeometries(body_frame, Role::kIllustration);
            DRAKE_DEMAND(geom_ids.size() == 1);
            const GeometryId geom_id = *geom_ids.begin();
            const IllustrationProperties* old_props = scene_graph.model_inspector().GetIllustrationProperties(geom_id);
            ...
            if (!accepting.empty()) {
                ...
                scene_graph.AssignRole(*plant.get_source_id(), geom_id, new_props, RoleAssign::kReplace);
            }
            ...
            const std::string geom_path = fmt::format("prefix/box/box/{}", TransformGeometryName(geom_id, inspector));
            const bool should_show = (accepting == "prefix") || (include_unspecified_accepting && accepting.empty());
            diagram->ForcedPublish(*context);
...
TEST_F(MeshcatVisualizerWithIiwaTest, AlphaSlidersSystemCheck) {
    ...
    const MeshcatVisualizerParams params{.enable_alpha_slider = true, .initial_alpha_slider_value = 0.5};
    SetUpDiagram(params);
    systems::Simulator<double> simulator(*diagram_);
...
std::optional<double> GetOpacityProperty(const Meshcat& meshcat, const std::string& path) {
    const std::string bytes = meshcat.GetPackedProperty(path, "modulated_opacity");
    if (bytes.empty()) {
        return {};
    }
...
GTEST_TEST(MeshcatVisualizerTest, AlphaSliderCheckResults) {
    ...
    DRAKE_DEMAND(geom_ids.size() == 1);
    const GeometryId geom_id = *geom_ids.begin();
    const std::string geom_path = fmt::format("visualizer/box/box/{}", TransformGeometryName(geom_id, inspector));
    ...
    diagram->ForcedPublish(*context);
    const std::optional<double> init_alpha = GetOpacityProperty(*meshcat, geom_path);
    ...
    for (const double slider_value : {0.2, 0.76, 1.0, 0.02}) {
        ...
        const std::optional<double> mod_opacity_value = GetOpacityProperty(*meshcat, params.prefix);
...
GTEST_TEST(MeshcatVisualizerTest, RealtimeRate) {
    ...
    auto* meshcat_visualizer = &MeshcatVisualizer<double>::AddToBuilder(&builder, scene_graph, meshcat, params);
...
TEST_F(MeshcatVisualizerWithIiwaTest, Graphviz) {
    SetUpDiagram();
    EXPECT_THAT(visualizer_->GetGraphvizString(), testing::HasSubstr("-> meshcat_in"));
}
