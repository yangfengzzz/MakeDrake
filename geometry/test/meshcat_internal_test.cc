...
GTEST_TEST(MeshcatInternalTest, GetMeshcatStaticResource) {
    ...
    const std::vector<const char*> urls{
            "/", "/favicon.ico", "/index.html", "/meshcat.html", "/meshcat.js", "/stats.min.js",
    };
    for (const auto& url : urls) {
        SCOPED_TRACE(fmt::format("url = {}", url));
        const std::optional<std::string_view> result = GetMeshcatStaticResource(url);
        ASSERT_TRUE(result);
...
GTEST_TEST(UnbundleGltfAssetsTest, DataUri) {
    ...
    const std::string gltf_filename = FindResourceOrThrow("drake/geometry/render/test/meshes/cube1.gltf");
    ...
    const Sha256 expected_sha256 =
            Sha256::Checksum(ReadFileOrThrow(FindResourceOrThrow("drake/geometry/render/test/meshes/cube2.bin")));
    ASSERT_EQ(assets.size(), 1);
    EXPECT_EQ(assets.front()->sha256, expected_sha256);

    // Make sure the new URI seems correct.
    EXPECT_THAT(json::parse(gltf_contents)["buffers"][0]["uri"], testing::EndsWith(expected_sha256.to_string()));
}
...
GTEST_TEST(UnbundleGltfAssetsTest, RelativeUri) {
    ...
    const std::string gltf_filename = FindResourceOrThrow("drake/geometry/render/test/meshes/cube2.gltf");
    std::string gltf_contents = ReadFileOrThrow(gltf_filename);
...
GTEST_TEST(TransformGeometryNameTest, SampledResults) {
    ...
    std::vector<std::pair<std::string, std::string>> test_names{
            {"unchanged", "unchanged"},      {"::prefixed", "/prefixed"},       {"single:colon", "single:colon"},
            {"simple::path", "simple/path"}, {"extra:::colon", "extra/:colon"}, {"empty::::slash", "empty//slash"},
            {"suffixed::", "suffixed/"},     {"final_trio:::", "final_trio/:"}};
    for (const auto& [name, transformed] : test_names) {
        const GeometryId id =
                scene_graph.RegisterAnchoredGeometry(s_id, std::make_unique<GeometryInstance>(I, Sphere(1.0), name));
        EXPECT_EQ(TransformGeometryName(id, scene_graph.model_inspector()), transformed);
    }
}
