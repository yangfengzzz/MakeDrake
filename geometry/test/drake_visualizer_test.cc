#include "geometry/drake_visualizer.h"
...
#include "geometry/kinematics_vector.h"
...
#include "lcmt_viewer_draw.hpp"
#include "lcmt_viewer_geometry_data.hpp"
#include "lcmt_viewer_load_robot.hpp"
...
public:
    ...
    template <typename T>
    static const DrakeLcmInterface* owned_lcm_interface(const DrakeVisualizer<T>& visualizer) {
        return visualizer.owned_lcm_.get();
    }

    template <typename T>
    static const DrakeLcmInterface* active_lcm_interface(const DrakeVisualizer<T>& visualizer) {
        return visualizer.lcm_;
    }

    template <typename T>
    static const DrakeVisualizerParams& get_params(const DrakeVisualizer<T>& visualizer) {
...
public:
    ...
    PoseSource() {
        this->DeclareAbstractOutputPort(systems::kUseDefaultName, FramePoseVector<T>(), &PoseSource<T>::ReadPoses);
    }

    void SetPoses(FramePoseVector<T> poses) { poses_ = std::move(poses); }

private:
    void ReadPoses(const Context<T>&, FramePoseVector<T>* poses) const { *poses = poses_; }
...
public:
    ...
    ConfigurationSource() {
        this->DeclareAbstractOutputPort(systems::kUseDefaultName, GeometryConfigurationVector<T>(),
...
private:
    void ReadConfigurations(const Context<T>&, GeometryConfigurationVector<T>* configurations) const {
...
protected:
    ...
    void ConfigureDiagram(DrakeVisualizerParams params = {}) {
        ...
        visualizer_ = builder.template AddSystem<DrakeVisualizer<T>>(&lcm_, std::move(params));
        builder.Connect(scene_graph_->get_query_output_port(), visualizer_->query_object_input_port());
        pose_source_id_ = scene_graph_->RegisterSource(kPoseSourceName);
        configuration_source_id_ = scene_graph_->RegisterSource(kConfigurationSourceName);
        pose_source_ = builder.template AddSystem<PoseSource<T>>();
        builder.Connect(pose_source_->get_output_port(0), scene_graph_->get_source_pose_port(pose_source_id_));
        configuration_source_ = builder.template AddSystem<ConfigurationSource<T>>();
    ...
    void PopulateScene() {
        ...
        const FrameId f0_id = scene_graph_->RegisterFrame(pose_source_id_, GeometryFrame("perception", 0));
        const GeometryId g0_id = scene_graph_->RegisterGeometry(
                pose_source_id_, f0_id,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Box>(1, 1, 1), "perception"));
        ...
        const FrameId f1_id = scene_graph_->RegisterFrame(pose_source_id_, GeometryFrame("illustration", 1));
        const GeometryId g1_id = scene_graph_->RegisterGeometry(
                pose_source_id_, f1_id,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Cylinder>(1, 1), "illustration"));
        ...
        proximity_frame_id_ = scene_graph_->RegisterFrame(pose_source_id_, GeometryFrame("proximity", 2));
        const GeometryId g2_id = scene_graph_->RegisterGeometry(
                pose_source_id_, proximity_frame_id_,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "proximity"));
    ...
    ::testing::AssertionResult ExpectedMessageCount(int num_load, int num_draw) {
        ...
        if (results.num_draw != num_draw || results.num_load != num_load || results.num_deformable != num_draw) {
            return ::testing::AssertionFailure()
                   << "Expected " << num_load << " load messages, " << num_draw << " draw messages, and " << num_draw
                   << " deformable messages" << "\nFound   " << results.num_load << " load messages, "
                   << results.num_draw << " draw messages, and " << results.num_deformable << " deformable messages.";
        }
    ...
    template <typename AddFunctor, typename AddFunctorNoParam, typename SourceFunctor>
    void TestAddToBuilder(const AddFunctor& add_to_builder,
        ...
        using Tester = DrakeVisualizerTester;
        {
            ...
            /* The fact that a visualizer is returned is proof that a DrakeVisualizer
             was created.  */
            const auto& visualizer = add_to_builder_no_param(&builder, port_source(scene_graph), &lcm_);
            ...
            const auto& sg_query_object = scene_graph.get_query_output_port().template Eval<QueryObject<T>>(sg_context);
            /* Not throwing is evidence of *some* connection.  */
            const auto& viz_query_object =
                    visualizer.query_object_input_port().template Eval<QueryObject<T>>(viz_context);
            ...
            EXPECT_TRUE(sg_query_object.inspector().geometry_version().IsSameAs(
                    viz_query_object.inspector().geometry_version(), Role::kIllustration));
...

        {
            ...
            DiagramBuilder<T> builder;
            const auto& scene_graph = *builder.template AddSystem<SceneGraph<T>>();
            const auto& visualizer = add_to_builder_no_param(&builder, port_source(scene_graph), nullptr);
            EXPECT_NE(Tester::owned_lcm_interface(visualizer), nullptr);
            EXPECT_EQ(Tester::owned_lcm_interface(visualizer), Tester::active_lcm_interface(visualizer));
...

        {
            ...
            DiagramBuilder<T> builder;
            const auto& scene_graph = *builder.template AddSystem<SceneGraph<T>>();
            const DrakeVisualizerParams params{1 / 13.0, Role::kPerception, Rgba{0.1, 0.2, 0.3, 0.4}};
            const auto& visualizer = add_to_builder(&builder, port_source(scene_graph), &lcm_, params);
    ...
    void ExpectMeshInMessage(Role role, bool expect_hull) {
        auto mesh =
                make_unique<MeshType>(FindResourceOrThrow("drake/geometry/render/test/meshes/"
                                                          "fully_textured_pyramid.gltf"));
        SCOPED_TRACE(fmt::format("{} shape with {} role", mesh->type_name(), role));
        this->ConfigureDiagram({.role = role});

        const FrameId f_id = this->scene_graph_->RegisterFrame(this->pose_source_id_, GeometryFrame("test", 0));

        const RigidTransformd X_PC(Vector3d(-1, 2, -3));
        const GeometryId g_id = this->scene_graph_->RegisterGeometry(
                this->pose_source_id_, f_id, make_unique<GeometryInstance>(X_PC, std::move(mesh), "test"));
        ...
        if (expect_hull) {
            ...
            const RotationMatrixd R_PG(Eigen::Quaternion<double>(q_PG[0], q_PG[1], q_PG[2], q_PG[3]));
            const RigidTransformd X_PG_test(R_PG, {p_PG[0], p_PG[1], p_PG[2]});
            /* Tolerance due to conversion to float. */
            EXPECT_TRUE(CompareMatrices(X_PC.GetAsMatrix34(), X_PG_test.GetAsMatrix34(), 1e-7));
        } else {
            EXPECT_THAT(geo_message.string_data, ::testing::HasSubstr("fully_textured_pyramid.gltf"));
        }
    ...
    static constexpr char kConfigurationSourceName[] = "DrakeVisualizerTestConfigurationSource";
    ...
    struct Subscribers {
        ...
        static std::string MakeRoleChannelName(const std::string& channel, std::optional<Role> role) {
...
TYPED_TEST(DrakeVisualizerTest, OwnedLcm) {
    ...
    EXPECT_EQ(Tester::active_lcm_interface(visualizer_owning), Tester::owned_lcm_interface(visualizer_owning));
}
...
TYPED_TEST(DrakeVisualizerTest, ConfigureDefaultDiffuse) {
    ...
    for (const auto& expected_rgba : {Rgba{0.75, 0.25, 0.125, 1.0}, Rgba{0.5, 0.75, 0.875, 0.5}}) {
        this->ConfigureDiagram({.publish_period = this->kPublishPeriod,
                                .role = Role::kProximity,
                                .default_color = expected_rgba,
                                .use_role_channel_suffix = true});
        const FrameId f_id = this->scene_graph_->RegisterFrame(this->pose_source_id_, GeometryFrame("frame", 0));
        const GeometryId g_id = this->scene_graph_->RegisterGeometry(
                this->pose_source_id_, f_id,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "proximity_sphere"));
        // ProximityProperties will make use of the default diffuse value.
        this->scene_graph_->AssignRole(this->pose_source_id_, g_id, ProximityProperties());
...
TYPED_TEST(DrakeVisualizerTest, AnchoredAndDynamicGeometry) {
    using T = TypeParam;
    this->ConfigureDiagram(
            {.publish_period = this->kPublishPeriod, .role = Role::kProximity, .use_role_channel_suffix = true});
    const FrameId f_id = this->scene_graph_->RegisterFrame(this->pose_source_id_, GeometryFrame("frame", 0));
    const GeometryId g0_id = this->scene_graph_->RegisterGeometry(
            this->pose_source_id_, f_id,
            make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "sphere0"));
    this->scene_graph_->AssignRole(this->pose_source_id_, g0_id, ProximityProperties());
    const GeometryId g1_id = this->scene_graph_->RegisterGeometry(
            this->pose_source_id_, f_id,
            make_unique<GeometryInstance>(RigidTransformd{Vector3d{10, 0, 0}}, make_unique<Sphere>(1), "sphere1"));
    this->scene_graph_->AssignRole(this->pose_source_id_, g1_id, ProximityProperties());

    const GeometryId g2_id = this->scene_graph_->RegisterAnchoredGeometry(
            this->pose_source_id_,
            make_unique<GeometryInstance>(RigidTransformd{Vector3d{5, 0, 0}}, make_unique<Sphere>(1), "sphere3"));
    this->scene_graph_->AssignRole(this->pose_source_id_, g2_id, ProximityProperties());
    ...
    ASSERT_EQ(results.load_message.link[1].name, string(this->kPoseSourceName) + "::frame");
    ...
    ASSERT_EQ(results.draw_message.link_name[0], string(this->kPoseSourceName) + "::frame");
...
TYPED_TEST(DrakeVisualizerTest, TargetRole) {
    ...
    const map<Role, string> expected{{Role::kIllustration, source_name + "::illustration"},
    ...
    for (const auto& [role, name] : expected) {
        for (const bool use_suffix : {false, true}) {
            SCOPED_TRACE(fmt::format("role '{}', name '{}', use_suffix '{}'", role, name, use_suffix));
            this->ConfigureDiagram(
                    {.publish_period = this->kPublishPeriod, .role = role, .use_role_channel_suffix = use_suffix});
            this->PopulateScene();
            Simulator<T> simulator(*(this->diagram_));
            simulator.AdvanceTo(0.0);
...
TYPED_TEST(DrakeVisualizerTest, ForcePublish) {
    this->ConfigureDiagram({.publish_period = this->kPublishPeriod, .role = Role::kIllustration});
    this->PopulateScene();
    auto context = this->diagram_->CreateDefaultContext();
    const auto& vis_context = this->visualizer_->GetMyContextFromRoot(*context);
    this->visualizer_->ForcedPublish(vis_context);
...
TYPED_TEST(DrakeVisualizerTest, GeometryWithIllustrationFallback) {
    ...
    const GeometryId g_id = this->scene_graph_->RegisterAnchoredGeometry(
            this->pose_source_id_, make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "sphere0"));
...
TYPED_TEST(DrakeVisualizerTest, AllRolesCanDefineDiffuse) {
    ...
    for (const Role role : {Role::kProximity, Role::kPerception}) {
        this->ConfigureDiagram({.publish_period = this->kPublishPeriod, .role = role, .use_role_channel_suffix = true});
        const GeometryId g_id = this->scene_graph_->RegisterAnchoredGeometry(
                this->pose_source_id_,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "sphere0"));
        const Rgba expected_rgba{0.25, 0.125, 0.75, 0.5};
...
TYPED_TEST(DrakeVisualizerTest, BadParameters) {
    using T = TypeParam;
    // Zero publish period.
    EXPECT_THROW(DrakeVisualizer<T>(&(this->lcm_), DrakeVisualizerParams{0, Role::kIllustration, Rgba{1, 1, 1, 1}}),
                 std::exception);

    // Negative publish period.
    EXPECT_THROW(DrakeVisualizer<T>(&(this->lcm_), DrakeVisualizerParams{-0.1, Role::kIllustration, Rgba{1, 1, 1, 1}}),
                 std::exception);

    // Unassigned role.
    EXPECT_THROW(DrakeVisualizer<T>(&(this->lcm_), DrakeVisualizerParams{0.1, Role::kUnassigned, Rgba{1, 1, 1, 1}}),
                 std::exception);
}
...
TYPED_TEST(DrakeVisualizerTest, ChangesInVersion) {
    using T = TypeParam;
    for (const Role role : {Role::kProximity, Role::kPerception, Role::kIllustration}) {
        this->ConfigureDiagram({.publish_period = this->kPublishPeriod, .role = role, .use_role_channel_suffix = true});
        const GeometryId g_id = this->scene_graph_->RegisterAnchoredGeometry(
                this->pose_source_id_,
                make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1), "sphere0"));
        ...
        Context<T>& sg_context = this->diagram_->GetMutableSubsystemContext(*(this->scene_graph_), &diagram_context);
        double t = 0.0;
        ...
        for (const Role modified_role : {Role::kProximity, Role::kIllustration}) {
            if (modified_role == Role::kProximity) {
                this->scene_graph_->AssignRole(&sg_context, this->pose_source_id_, g_id, ProximityProperties());
            } else if (modified_role == Role::kIllustration) {
                this->scene_graph_->AssignRole(&sg_context, this->pose_source_id_, g_id, IllustrationProperties());
            }
            t += this->kPublishPeriod;
            simulator.AdvanceTo(t);
            results = this->ProcessMessages(role);
...
TYPED_TEST(DrakeVisualizerTest, VisualizeDeformableGeometry) {
    for (const auto role : std::vector<Role>{Role::kProximity, Role::kIllustration}) {
        DrakeVisualizerParams params;
        ...
        auto geometry_instance =
                make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(kRadius), "sphere");
        geometry_instance->set_illustration_properties(IllustrationProperties{});
        ProximityProperties proximity_properties;
        proximity_properties.AddProperty("phong", "diffuse", expected_proximity_rgba);
        geometry_instance->set_proximity_properties(proximity_properties);
        GeometryId g_id = this->scene_graph_->RegisterDeformableGeometry(this->configuration_source_id_,
                                                                         this->scene_graph_->world_frame_id(),
        ...
        this->configuration_source_->SetConfigurations({{g_id, VectorX<double>::Zero(3 * mesh->num_vertices())}});
        ...
        auto is_visualizer_color = [](const lcmt_viewer_geometry_data& message, const Rgba expected) {
        ...
        EXPECT_EQ(geo_message.float_data.size(), 2 + geo_message.float_data[0] * 3 + geo_message.float_data[1] * 3);
        EXPECT_TRUE(is_visualizer_color(
                geo_message, role == Role::kProximity ? expected_proximity_rgba : expected_illustration_rgba));
    }
}
...
TYPED_TEST(DrakeVisualizerTest, VisualizeHydroGeometry) {
    ...
    FramePoseVector<T> poses;
    auto add_geometry = [this, &poses](auto shape_u_p, const std::string& name, const ProximityProperties& properties,
                                       const RigidTransformd& X_PG = {}) {
        const FrameId f_id = this->scene_graph_->RegisterFrame(this->pose_source_id_, GeometryFrame(name, 0));
        const GeometryId g_id = this->scene_graph_->RegisterGeometry(
                this->pose_source_id_, f_id, make_unique<GeometryInstance>(X_PG, std::move(shape_u_p), name));
    ...
    props.AddProperty(internal::kHydroGroup, internal::kComplianceType, HydroelasticType::kRigid);
    const RigidTransformd X_PBox{RotationMatrixd::MakeYRotation(0.25), Vector3d{1.25, 2.5, 3.75}};
    ...
    */
    ...
    props.UpdateProperty(internal::kHydroGroup, internal::kComplianceType, HydroelasticType::kSoft);
    const RigidTransformd X_PSphere{RotationMatrixd::MakeZRotation(0.3), Vector3d{2.5, 1.25, -3.75}};
    ...
    auto is_visualizer_color = [expected = params.default_color](const lcmt_viewer_geometry_data& message) {
        const auto& color = message.color;
        const Rgba test_rgba(color[0], color[1], color[2], color[3]);
        // The color values used in this test can all be perfectly represented by
        // 32-bit floats (e.g., 0.5, 0.75,  etc.). So, we can use exact equality.
    ...
    auto pose_matches = [](const RigidTransformd& X_PG_expected, const lcmt_viewer_geometry_data& message) {
        const auto& p_PG = message.position;
        const auto& q_PG = message.quaternion;
        const RotationMatrixd R_PG(Eigen::Quaternion<double>(q_PG[0], q_PG[1], q_PG[2], q_PG[3]));
        const RigidTransformd X_PG_test(R_PG, {p_PG[0], p_PG[1], p_PG[2]});
        /* Tolerance due to conversion to float. */
        return CompareMatrices(X_PG_expected.GetAsMatrix34(), X_PG_test.GetAsMatrix34(), 1e-7);
    };
    ...
    for (int i = 0; i < results.load_message.num_links; ++i) {
        ...
        } else if (link_message.name == fmt::format("{}::sphere", this->kPoseSourceName)) {
            EXPECT_EQ(geo_message.type, geo_message.MESH);
            EXPECT_TRUE(geo_message.string_data.empty());
            EXPECT_GT(geo_message.num_float_data, 2);
            EXPECT_EQ(geo_message.float_data.size(), 2 + geo_message.float_data[0] * 3 + geo_message.float_data[1] * 3);
        ...
        } else if (link_message.name == fmt::format("{}::soft_half_space", this->kPoseSourceName)) {
            /* In drake_visualizer, half spaces are big, flat boxes. */
            EXPECT_EQ(geo_message.type, geo_message.BOX);
        } else if (link_message.name == fmt::format("{}::rigid_half_space", this->kPoseSourceName)) {
            EXPECT_EQ(geo_message.type, geo_message.BOX);
        } else if (link_message.name == fmt::format("{}::ellipsoid", this->kPoseSourceName)) {
...
template <typename T>
const DrakeVisualizer<T>& add_to_builder_sg_no_params(DiagramBuilder<T>* builder,
                                                      const SceneGraph<T>& scene_graph,
                                                      DrakeLcmInterface* lcm) {
...
TYPED_TEST(DrakeVisualizerTest, AddToBuilderSceneGraph) {
    using T = TypeParam;
    this->TestAddToBuilder(add_to_builder_sg<T>, add_to_builder_sg_no_params<T>, pose_source_sg<T>);
}
...
template <typename T>
const DrakeVisualizer<T>& add_to_builder_port(DiagramBuilder<T>* builder,
                                              const systems::OutputPort<T>& port,
                                              DrakeLcmInterface* lcm,
                                              DrakeVisualizerParams params) {
...
template <typename T>
const DrakeVisualizer<T>& add_to_builder_port_no_params(DiagramBuilder<T>* builder,
                                                        const systems::OutputPort<T>& port,
                                                        DrakeLcmInterface* lcm) {
...
TYPED_TEST(DrakeVisualizerTest, AddToBuilderQueryObjectPort) {
    using T = TypeParam;
    this->TestAddToBuilder(add_to_builder_port<T>, add_to_builder_port_no_params<T>, pose_source_port<T>);
}
...
TYPED_TEST(DrakeVisualizerTest, DispatchLoadMessageFromModel) {
...

    {
        ...
        // "illustration".
        DrakeVisualizer<T>::DispatchLoadMessage(*(this->scene_graph_), &(this->lcm_));
        MessageResults results = this->ProcessMessages();
        ASSERT_EQ(results.num_load, 1);
        ASSERT_EQ(results.load_message.num_links, 1);
        ASSERT_EQ(results.load_message.link[0].name, string(this->kPoseSourceName) + "::illustration");
        ASSERT_EQ(results.load_message.link[0].num_geom, 1);
    ...
    }
    {
        ...
        params.role = Role::kProximity;
        params.use_role_channel_suffix = true;
        DrakeVisualizer<T>::DispatchLoadMessage(*(this->scene_graph_), &(this->lcm_), params);
        MessageResults results = this->ProcessMessages(params.role);
        ASSERT_EQ(results.num_load, 1);
        ASSERT_EQ(results.load_message.num_links, 1);
        ASSERT_EQ(results.load_message.link[0].name, string(this->kPoseSourceName) + "::proximity");
        ASSERT_EQ(results.load_message.link[0].num_geom, 1);
        ASSERT_EQ(results.num_draw, 0);
        ASSERT_EQ(results.num_deformable, 0);
    }
...
GTEST_TEST(DrakeVisualizerdTest, Transmogrify) {
    ...
    auto* vis_own_lcm_ad = dynamic_cast<DrakeVisualizer<AutoDiffXd>*>(sys_own_lcm_ad.get());
    ASSERT_NE(vis_own_lcm_ad, nullptr);
    ASSERT_NE(Tester::owned_lcm_interface(*vis_own_lcm_ad), nullptr);
    ASSERT_NE(Tester::active_lcm_interface(vis_own_lcm_d), Tester::owned_lcm_interface(*vis_own_lcm_ad));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(vis_not_own_lcm_d.ToAutoDiffXd(),
