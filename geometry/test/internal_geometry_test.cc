...
GTEST_TEST(InternalGeometryTest, PropertyAssignment) {
...

    {
        ...
        EXPECT_TRUE(geometry.proximity_properties()->HasProperty("group1", "value"));
        EXPECT_FALSE(geometry.proximity_properties()->HasProperty("group2", "value"));
        // Resetting proximity properties is not an error.
        DRAKE_EXPECT_NO_THROW(geometry.SetRole(p2));
        EXPECT_FALSE(geometry.proximity_properties()->HasProperty("group1", "value"));
        EXPECT_TRUE(geometry.proximity_properties()->HasProperty("group2", "value"));
        EXPECT_EQ(geometry.proximity_properties(), geometry.properties(Role::kProximity));
    }

    {
        ...
        EXPECT_TRUE(geometry.illustration_properties()->HasProperty("group1", "value"));
        EXPECT_FALSE(geometry.illustration_properties()->HasProperty("group2", "value"));
        // Resetting illustration properties is not an error.
        DRAKE_EXPECT_NO_THROW(geometry.SetRole(p2));
        EXPECT_FALSE(geometry.illustration_properties()->HasProperty("group1", "value"));
        EXPECT_TRUE(geometry.illustration_properties()->HasProperty("group2", "value"));
        EXPECT_EQ(geometry.illustration_properties(), geometry.properties(Role::kIllustration));
    }

    {
        ...
        EXPECT_TRUE(geometry.has_perception_role());
        EXPECT_EQ(geometry.perception_properties(), geometry.properties(Role::kPerception));
    }
}
...
GTEST_TEST(InternalGeometryTest, SetPose) {
    ...
    const RigidTransformd X_FGold(RotationMatrixd::MakeXRotation(M_PI / 3), Vector3d(1, 2, 3));
    const RigidTransformd X_FGNew(RotationMatrixd::MakeYRotation(M_PI / 7), Vector3d(4, 5, 6));

    InternalGeometry geometry(source_id, sphere.Clone(), frame_id, geometry_id, name, X_FGold);
    EXPECT_TRUE(CompareMatrices(geometry.X_FG().GetAsMatrix34(), X_FGold.GetAsMatrix34()));
    geometry.set_pose(X_FGNew);
    EXPECT_TRUE(CompareMatrices(geometry.X_FG().GetAsMatrix34(), X_FGNew.GetAsMatrix34()));
}
...
GTEST_TEST(InternalGeometryTest, DeformableGeometry) {
    ...
    const VolumeMesh<double> expected_mesh_G =
    ...
    InternalGeometry geometry(source_id, make_unique<Sphere>(sphere), frame_id, deformable_geometry_id, name, X_FG,
                              kRezHint);
    ...
    InternalGeometry rigid_geometry(source_id, make_unique<Sphere>(sphere), frame_id, rigid_geometry_id, name,
                                    RigidTransformd());
