#include "geometry/geometry_state.h"
...
#include "geometry/geometry_version.h"
#include "geometry/internal_frame.h"
#include "geometry/proximity/make_sphere_mesh.h"
#include "geometry/render/render_label.h"
...
public:
    ...
    const unordered_map<SourceId, string>& get_source_name_map() const { return state_->source_names_; }

    const unordered_map<SourceId, FrameIdSet>& get_source_frame_id_map() const { return state_->source_frame_id_map_; }
    ...
    const unordered_map<SourceId, unordered_set<GeometryId>>& get_source_anchored_geometry_map() const {
        return state_->source_anchored_geometry_map_;
    }

    const unordered_map<FrameId, InternalFrame>& get_frames() const { return state_->frames_; }

    const unordered_map<GeometryId, InternalGeometry>& get_geometries() const { return state_->geometries_; }

    const vector<FrameId>& get_frame_index_id_map() const { return state_->frame_index_to_id_map_; }

    const IdPoseMap<T>& get_geometry_world_poses() const { return state_->kinematics_data_.X_WGs; }

    const IdConfigurationMap<T>& get_geometry_world_configurations() const { return state_->kinematics_data_.q_WGs; }

    const vector<RigidTransform<T>>& get_frame_world_poses() const { return state_->kinematics_data_.X_WFs; }

    const vector<RigidTransform<T>>& get_frame_parent_poses() const { return state_->kinematics_data_.X_PFs; }

    internal::KinematicsData<T>& mutable_kinematics_data() const { return state_->mutable_kinematics_data(); }

    const internal::DrivenMeshData& driven_mesh_data(Role role) const { return state_->driven_mesh_data_.at(role); }

    void SetFramePoses(SourceId source_id,
    ...
    void SetGeometryConfiguration(SourceId source_id,
    ...
    void FinalizePoseUpdate() {
        state_->FinalizePoseUpdate(state_->kinematics_data_, &state_->mutable_proximity_engine(),
                                   state_->GetMutableRenderEngines());
    }

    void FinalizeConfigurationUpdate() {
        for (const auto role : std::vector<Role>{Role::kPerception, Role::kIllustration, Role::kProximity}) {
            state_->driven_mesh_data_[role].SetControlMeshPositions(state_->kinematics_data_.q_WGs);
        }
        state_->FinalizeConfigurationUpdate(state_->kinematics_data_, state_->driven_mesh_data_[Role::kPerception],
                                            &state_->mutable_proximity_engine(), state_->GetMutableRenderEngines());
    }

    template <typename ValueType>
    void ValidateFrameIds(SourceId source_id, const KinematicsVector<FrameId, ValueType>& data) const {
        state_->ValidateFrameIds(source_id, data);
    }

    const InternalGeometry* GetGeometry(GeometryId id) const { return state_->GetGeometry(id); }

    const render::RenderEngine& GetRenderEngineOrThrow(const std::string& name) const {
        return state_->GetRenderEngineOrThrow(name);
    }

    const ProximityEngine<T>& proximity_engine() const { return *state_->geometry_engine_; }

    const unordered_map<string, copyable_unique_ptr<render::RenderEngine>>& render_engines() const {
        return state_->render_engines_;
    }

    const GeometryVersion& geometry_version() const { return state_->geometry_version_; }

    // Provides acesss to the scalar-converting copy constructor.
    template <typename U>
    static std::unique_ptr<GeometryState<T>> CopyGeometryState(const GeometryState<U>& other) {
...
public:
    static const hydroelastic::Geometries& hydroelastic_geometries(const ProximityEngine<double>& engine) {
...
public:
    explicit ShapeMatcher(const ShapeType& expected) : expected_(expected), result_(::testing::AssertionFailure()) {}

    // Tests shape introspection.
    ::testing::AssertionResult ShapeIntrospects(GeometryState<double>* state, SourceId source_id, FrameId frame_id) {
        GeometryId g_id = state->RegisterGeometry(
                source_id, frame_id,
                make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<ShapeType>(expected_), "shape"));
...
private:
    ...
    void TestShapeParameters(const TestType& test) {
        error() << "Not implemented for " << NiceTypeName::Get<ShapeType>() << " vs" << NiceTypeName::Get<TestType>();
    }
    ...
    bool IsExpectedType(const TestShape& test) {
        if (typeid(ShapeType) == typeid(const TestShape)) {
            ...
            result_ << "Expected '" << NiceTypeName::Get<ShapeType>() << "', given '" << NiceTypeName::Get<TestShape>()
                    << "'";
            return false;
        }
    }
...
void ShapeMatcher<Sphere>::TestShapeParameters(const Sphere& test) {
    if (test.radius() != expected_.radius()) {
        error() << "\nExpected sphere radius " << expected_.radius() << ", " << "received sphere radius "
                << test.radius();
    }
}
...
void ShapeMatcher<Cylinder>::TestShapeParameters(const Cylinder& test) {
    if (test.radius() != expected_.radius()) {
        error() << "\nExpected cylinder radius " << expected_.radius() << ", " << "received cylinder radius "
                << test.radius();
    }
    if (test.length() != expected_.length()) {
        error() << "\nExpected cylinder length " << expected_.length() << ", received cylinder length "
                << test.length();
    }
}
...
void ShapeMatcher<Box>::TestShapeParameters(const Box& test) {
    if (test.width() != expected_.width()) {
        error() << "\nExpected box width " << expected_.width() << ", " << "received box width " << test.width();
    }
    if (test.height() != expected_.height()) {
        error() << "\nExpected box height " << expected_.height() << ", received box height " << test.height();
    }
    if (test.depth() != expected_.depth()) {
        error() << "\nExpected box depth " << expected_.depth() << ", received box depth " << test.depth();
    }
}
...
void ShapeMatcher<Capsule>::TestShapeParameters(const Capsule& test) {
    if (test.radius() != expected_.radius()) {
        error() << "\nExpected capsule radius " << expected_.radius() << ", " << "received capsule radius "
                << test.radius();
    }
    if (test.length() != expected_.length()) {
        error() << "\nExpected capsule length " << expected_.length() << ", received capsule length " << test.length();
    }
}
...
void ShapeMatcher<Mesh>::TestShapeParameters(const Mesh& test) {
    if (test.filename() != expected_.filename()) {
        error() << "\nExpected mesh filename " << expected_.filename() << ", " << "received mesh filename "
                << test.filename();
    }
    if (test.scale() != expected_.scale()) {
        error() << "\nExpected mesh scale " << expected_.scale() << ", received mesh scale " << test.scale();
    }
}
...
void ShapeMatcher<Convex>::TestShapeParameters(const Convex& test) {
    if (test.filename() != expected_.filename()) {
        error() << "\nExpected convex filename " << expected_.filename() << ", " << "received convex filename "
                << test.filename();
    }
    if (test.scale() != expected_.scale()) {
        error() << "\nExpected convex scale " << expected_.scale() << ", received convex scale " << test.scale();
    }
}

// A mask-style enumeration for indicating which of a set of roles should be
// assigned to the newly configured geometry.
enum class Assign { kNone = 0, kProximity = 1, kIllustration = 2, kPerception = 4 };
...
public:
    ...
    void TestInit(bool add_renderer = true) {
        ...
        instance_ = make_unique<GeometryInstance>(instance_pose_, make_unique<Sphere>(1.0), "instance");
    ...
    SourceId SetUpSingleSourceTree(Assign roles_to_assign = Assign::kNone) {
        ...
        // Create f0.
        // 90° around x-axis.
        RigidTransformd pose(AngleAxis<double>(M_PI_2, Vector3d::UnitX()), Vector3d{1, 2, 3});
        frames_.push_back(geometry_state_.RegisterFrame(source_id_, GeometryFrame("f0")));
        X_WFs_.push_back(pose);
        X_PFs_.push_back(pose);

        // Create f1.
        // 90° around y-axis.
        pose = RigidTransformd(AngleAxis<double>(M_PI_2, Vector3d::UnitY()), Vector3d{10, 20, 30});
        frames_.push_back(geometry_state_.RegisterFrame(source_id_, GeometryFrame("f1")));
        X_WFs_.push_back(pose);
        X_PFs_.push_back(pose);

        // Create f2.
        pose = pose.inverse();
        frames_.push_back(geometry_state_.RegisterFrame(source_id_, frames_[1], GeometryFrame("f2")));
        ...
        for (auto frame_id : frames_) {
            for (int i = 0; i < kGeometryCount; ++i) {
                pose = RigidTransformd(AngleAxis<double>(g_count * M_PI_2, x_axis), Vector3d{g_count + 1.0, 0, 0});
                ...
                geometries_[g_count] = geometry_state_.RegisterGeometry(
                        source_id_, frame_id, make_unique<GeometryInstance>(pose, make_unique<Sphere>(1), name));
        ...
        anchored_geometry_ = geometry_state_.RegisterAnchoredGeometry(
                source_id_, make_unique<GeometryInstance>(X_WA_, make_unique<Box>(100, 100, 2), anchored_name_));
    ...
    SourceId SetUpWithRigidAndDeformableGeometries(Assign roles_to_assign = Assign::kNone) {
        SourceId s_id = SetUpSingleSourceTree();
        const Sphere sphere(1.0);
        constexpr double kRezHint = 0.5;
        auto instance =
                make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(sphere), "sphere");
        GeometryId g_id = geometry_state_.RegisterDeformableGeometry(s_id, InternalFrame::world_frame_id(),
                                                                     std::move(instance), kRezHint);
    ...
    int single_tree_rigid_geometry_count() const {
        return single_tree_dynamic_rigid_geometry_count() + anchored_geometry_count();
    }

    // Total number of dynamic rigid geometries.
    int single_tree_dynamic_rigid_geometry_count() const { return kFrameCount * kGeometryCount; }

    int total_geometry_count() const { return single_tree_rigid_geometry_count() + ssize(deformable_geometries_); }
...
private:
    ...
    void AssignIllustrationToSingleSourceTree() {
        ...
        properties.AddProperty("phong", "diffuse", Vector4<double>{0.8, 0.8, 0.8, 1.0});
        // Add an arbitrary, meaningless property so we can detect if the properties
        // get thrown out.
        properties.AddProperty("test", "value", 17);
        AssignRoleToSingleSourceTree(properties);
    ...
    void AssignPerceptionToSingleSourceTree() {
        ...
        PerceptionProperties properties =
                render_engine_ ? render_engine_->accepting_properties() : DummyRenderEngine().accepting_properties();
        properties.AddProperty("phong", "diffuse", Vector4<double>{0.8, 0.8, 0.8, 1.0});
...
protected:
    ...
    template <typename ReturnType, typename... DeclaredArgs, typename... GivenArgs>
    std::tuple<GeometryVersion, GeometryVersion> ForwardAndGetRevisions(
            ReturnType (GeometryState<double>::*f)(DeclaredArgs...), GivenArgs&&... args) {
    ...
    template <typename ReturnType, typename... DeclaredArgs, typename... GivenArgs>
    void VerifyRoleVersionModified(Role role,
                                   ReturnType (GeometryState<double>::*f)(DeclaredArgs...),
                                   GivenArgs&&... args) {
        auto [old_version, new_version] = ForwardAndGetRevisions(f, std::forward<GivenArgs>(args)...);
        for (Role test_role : {Role::kProximity, Role::kPerception, Role::kIllustration}) {
            // We expect versions to match when the test_role is different from the
            // declared role.
            EXPECT_EQ(old_version.IsSameAs(new_version, test_role), role != test_role);
        }
    }
    ...
    template <typename ReturnType, typename... DeclaredArgs, typename... GivenArgs>
    ReturnType VerifyVersionUnchanged(ReturnType (GeometryState<double>::*f)(DeclaredArgs...), GivenArgs&&... args) {
        GeometryVersion old_version = geometry_state_.geometry_version();
    ...
    void VerifyVersionUnchanged(void (GeometryState<double>::*f)(DeclaredArgs...), GivenArgs&&... args) {
        auto [old_version, new_version] = ForwardAndGetRevisions(f, std::forward<GivenArgs>(args)...);
        EXPECT_TRUE(old_version.IsSameAs(new_version, Role::kProximity));
        EXPECT_TRUE(old_version.IsSameAs(new_version, Role::kPerception));
        EXPECT_TRUE(old_version.IsSameAs(new_version, Role::kIllustration));
    ...
    void VerifyIdenticalVersions(const GeometryState<double>& gs1, const GeometryState<double>& gs2) const {
...
TEST_F(GeometryStateTest, Constructor) {
    ...
    EXPECT_EQ(gs_tester_.get_source_frame_name_map().find(gs_tester_.get_self_source_id())->second,
...
TEST_F(GeometryStateTest, IntrospectShapes) {
    ...
    const FrameId frame_id = geometry_state_.RegisterFrame(source_id, GeometryFrame("frame"));

    // Test across all valid shapes.
    {
        ShapeMatcher<Sphere> matcher(Sphere(0.25));
        EXPECT_TRUE(matcher.ShapeIntrospects(&geometry_state_, source_id, frame_id));
    }
    {
    ...
    // Test invalid id.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetShape(GeometryId::get_new_id()),
...
TEST_F(GeometryStateTest, SourceRegistrationWithNames) {
    ...
    EXPECT_THAT(geometry_state_.FramesForSource(world_source), testing::ElementsAre(gs_tester_.get_world_frame()));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterNewSource(name),
                                "Registering new source with duplicate name: Unique.");

    // Case: query with invalid source id.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetName(SourceId::get_new_id()),
...
TEST_F(GeometryStateTest, GeometryStatistics) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.NumFramesForSource(SourceId::get_new_id()),
                                "Referenced geometry source .* is not registered.");
    EXPECT_EQ(geometry_state_.NumDynamicGeometries(), single_tree_dynamic_rigid_geometry_count());
    ...
    EXPECT_EQ(geometry_state_.get_num_geometries(), single_tree_rigid_geometry_count());
...
TEST_F(GeometryStateTest, GetOwningSourceName) {
    ...
    EXPECT_EQ(kSourceName, geometry_state_.GetOwningSourceName(anchored_geometry_));

    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetOwningSourceName(FrameId::get_new_id()),
                                "Referenced frame .* has not been registered.");

    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetOwningSourceName(GeometryId::get_new_id()),
                                "Geometry id .* does not map to a registered geometry");
}

TEST_F(GeometryStateTest, RenameFrame) {
    SetUpSingleSourceTree();
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RenameFrame(FrameId::get_new_id(), "invalid"),
                                ".*Cannot rename.*invalid frame id:.*");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RenameFrame(frames_[0], "f1"), ".*already existing.*");
...
TEST_F(GeometryStateTest, RenameGeometry) {
    SetUpSingleSourceTree();
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RenameGeometry(GeometryId::get_new_id(), "invalid"),
...
template <typename T>
void ExpectSuccessfulTransmogrification(const GeometryStateTester<T>& T_tester,
    ...
    EXPECT_EQ(T_tester.get_source_frame_id_map(), d_tester.get_source_frame_id_map());
    EXPECT_EQ(T_tester.get_source_frame_name_map(), d_tester.get_source_frame_name_map());
    EXPECT_EQ(T_tester.get_source_root_frame_map(), d_tester.get_source_root_frame_map());
    EXPECT_EQ(T_tester.get_source_anchored_geometry_map(), d_tester.get_source_anchored_geometry_map());
    ...
    EXPECT_EQ(T_tester.get_frame_index_id_map(), d_tester.get_frame_index_id_map());
    ...
    for (const auto& id_geom_pair : T_tester.get_geometries()) {
        ...
        EXPECT_TRUE(CompareMatrices(T_geometry.X_FG().GetAsMatrix34(),
                                    d_tester.get_geometries().at(id).X_FG().GetAsMatrix34()));
    }

    // 4. Compare RigidTransform<AutoDiffXd> with RigidTransformd.
    auto test_T_vs_double_pose = [](const RigidTransform<T>& test, const RigidTransformd& ref) {
        for (int row = 0; row < 3; ++row) {
            for (int col = 0; col < 4; ++col) {
                EXPECT_EQ(ExtractDoubleOrThrow(test.GetAsMatrix34()(row, col)), ref.GetAsMatrix34()(row, col));
            }
        }
    };

    auto test_T_vs_double = [test_T_vs_double_pose](const vector<RigidTransform<T>>& test,
    ...
    auto test_T_vs_double_pose_map = [test_T_vs_double_pose](const IdPoseMap<T>& test, const IdPoseMap<double>& ref) {
    ...
    auto test_T_vs_double_configuration_map = [](const IdConfigurationMap<T>& test,
        ...
        for (const auto& id_configuration_pair : ref) {
            ...
            const VectorX<double>& ref_configuration = id_configuration_pair.second;
            EXPECT_EQ(ExtractDoubleOrThrow(test.at(id)), ref_configuration);
        }
    };

    test_T_vs_double(T_tester.get_frame_parent_poses(), d_tester.get_frame_parent_poses());
    test_T_vs_double_pose_map(T_tester.get_geometry_world_poses(), d_tester.get_geometry_world_poses());
    test_T_vs_double(T_tester.get_frame_world_poses(), d_tester.get_frame_world_poses());
    test_T_vs_double_configuration_map(T_tester.get_geometry_world_configurations(),
...
TEST_F(GeometryStateTest, CopyAutoDiffIntoDouble) {
    ...
    // Convert.
    std::unique_ptr<GeometryState<double>> dut = GeometryStateTester<double>::CopyGeometryState(ad_state);
...
TEST_F(GeometryStateTest, CopySymbolicIntoDouble) {
    ...
    // Convert.
    std::unique_ptr<GeometryState<double>> dut = GeometryStateTester<double>::CopyGeometryState(sym_state);
...
TEST_F(GeometryStateTest, Transmogrify) {
    SetUpWithRigidAndDeformableGeometries();
    unique_ptr<GeometryState<AutoDiffXd>> ad_state = geometry_state_.ToAutoDiffXd();
    GeometryStateTester<AutoDiffXd> ad_tester;
...
TEST_F(GeometryStateTest, ValidateSingleSourceTree) {
    ...
    // The internal frames are what and where they should be.
    {
        ...
        auto test_frame = [internal_frames, this, s_id](int i, FrameId parent_id, int num_child_frames) {
            ...
            EXPECT_NE(child_geometries.find(geometries_[i * 2]), child_geometries.end());
            EXPECT_NE(child_geometries.find(geometries_[i * 2 + 1]), child_geometries.end());
            const auto& frame_in_parent = gs_tester_.get_frame_parent_poses();
            EXPECT_TRUE(CompareMatrices(frame_in_parent[frame.index()].GetAsMatrix34(), X_PFs_[i].GetAsMatrix34()));
        };
        ...
        for (FrameId frame_id : frames_) {
            ...
            EXPECT_TRUE(CompareMatrices(frame_in_parent[frame.index()].GetAsMatrix34(),
    ...
    // The internal geometries are what and where they should be.
    {
        ...
        for (int i = 0; i < single_tree_dynamic_rigid_geometry_count(); ++i) {
            ...
            EXPECT_TRUE(CompareMatrices(geometry_state_.GetPoseInFrame(geometry.id()).GetAsMatrix34(),
                                        X_FGs_[i].GetAsMatrix34()));
        }
    }
    EXPECT_EQ(static_cast<int>(gs_tester_.get_geometry_world_poses().size()), single_tree_rigid_geometry_count());
...
TEST_F(GeometryStateTest, GetAllGeometryIds) {
    ...
    const GeometryId local_anchored_id = geometry_state_.RegisterAnchoredGeometry(
            test_source, make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(1), "anchored"));
    geometry_state_.AssignRole(test_source, local_anchored_id, ProximityProperties{});
    SetUpSingleSourceTree();
    ...
    const vector<GeometryId> all_ids = geometry_state_.GetAllGeometryIds(/* role = */ std::nullopt);
    EXPECT_EQ(all_ids, expected_ids);

    expected_ids.clear();
    expected_ids.push_back(local_anchored_id);

    const vector<GeometryId> proximity_ids = geometry_state_.GetAllGeometryIds(Role::kProximity);
...
TEST_F(GeometryStateTest, GetGeometryIds) {
    ...
    unordered_set<GeometryId> all_geometries(geometries_.begin(), geometries_.end());
    all_geometries.insert(anchored_geometry_);
    const unordered_set<GeometryId> perception_geometries{geometries_[2], geometries_[3], geometries_[5]};
    const unordered_set<GeometryId> illustration_geometries{geometries_[4], geometries_[5]};

    {
        ...
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_frames), std::nullopt), all_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_frames), Role::kProximity), all_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_frames), Role::kPerception), perception_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_frames), Role::kIllustration),
...

    {
        ...
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_geometries), std::nullopt), all_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_geometries), Role::kProximity), all_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_geometries), Role::kPerception),
                  perception_geometries);
        EXPECT_EQ(geometry_state_.GetGeometryIds(GeometrySet(all_geometries), Role::kIllustration),
                  illustration_geometries);
    }

    {
        ...
        EXPECT_EQ(geometry_state_.GetGeometryIds(redundant_set, Role::kPerception),
                  unordered_set<GeometryId>({geometries_[2], geometries_[3]}));
        EXPECT_EQ(geometry_state_.GetGeometryIds(redundant_set, Role::kIllustration), unordered_set<GeometryId>());
    }
}
...
TEST_F(GeometryStateTest, GetNumGeometryTests) {
    ...
    EXPECT_EQ(total_geometry_count(), geometry_state_.NumGeometriesWithRole(Role::kProximity));
    ...
    for (int i = 0; i < kFrameCount; ++i) {
        EXPECT_EQ(kGeometryCount, geometry_state_.NumGeometriesForFrame(frames_[i]));
        EXPECT_EQ(kGeometryCount, geometry_state_.NumGeometriesForFrameWithRole(frames_[i], Role::kProximity));
        EXPECT_EQ(0, geometry_state_.NumGeometriesForFrameWithRole(frames_[i], Role::kPerception));
        EXPECT_EQ(0, geometry_state_.NumGeometriesForFrameWithRole(frames_[i], Role::kIllustration));
    }

    EXPECT_EQ(1, geometry_state_.NumDeformableGeometriesWithRole(Role::kProximity));
    EXPECT_EQ(0, geometry_state_.NumDeformableGeometriesWithRole(Role::kPerception));
    EXPECT_EQ(0, geometry_state_.NumDeformableGeometriesWithRole(Role::kIllustration));
}
...
TEST_F(GeometryStateTest, GetGeometryTest) {
    ...
    for (int i = 0; i < kFrameCount; ++i) {
        ...
        auto instance = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "shape");
        const GeometryId new_id = geometry_state_.RegisterGeometry(source_id_, f_id, std::move(instance));

        // Build the expected answers.
        const auto first_geometry_iter = geometries_.begin() + i * kGeometryCount;
        const vector<GeometryId> proximity_ids{first_geometry_iter, first_geometry_iter + kGeometryCount};
        ...
        EXPECT_EQ(geometry_state_.GetGeometries(f_id, Role::kPerception), empty_ids);
        EXPECT_EQ(geometry_state_.GetGeometries(f_id, Role::kProximity), proximity_ids);
        EXPECT_EQ(geometry_state_.GetGeometries(f_id, Role::kIllustration),
                  vector<GeometryId>{geometries_[i * kGeometryCount]});
        EXPECT_EQ(geometry_state_.GetGeometries(f_id, Role::kUnassigned), vector<GeometryId>{new_id});
...
TEST_F(GeometryStateTest, AddFrameToInvalidSource) {
    ...
    DRAKE_ASSERT_THROWS_MESSAGE(geometry_state_.RegisterFrame(s_id, *frame_),
...
TEST_F(GeometryStateTest, DuplicateFrameName) {
    ...
    DRAKE_ASSERT_THROWS_MESSAGE(geometry_state_.RegisterFrame(s_id, GeometryFrame(frame_->name())),
...
TEST_F(GeometryStateTest, ManyWorldsRefuted) {
    DRAKE_ASSERT_THROWS_MESSAGE(geometry_state_.RegisterFrame(gs_tester_.get_self_source_id(), GeometryFrame("world")),
...
TEST_F(GeometryStateTest, AddFrameWithDuplicateId) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterFrame(s_id, *frame_),
                                "Registering frame with an id that has already been registered: \\d+");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterFrame(s_id, f_id, *frame_),
                                "Registering frame with an id that has already been registered: \\d+");
}
...
TEST_F(GeometryStateTest, RegisterGeometryGoodSource) {
    ...
    const GeometryId g_id = geometry_state_.RegisterGeometry(s_id, f_id, std::move(instance_));
    ...
    EXPECT_TRUE(CompareMatrices(X_FG.GetAsMatrix34(), instance_pose_.GetAsMatrix34()));
...
TEST_F(GeometryStateTest, AddGeometryUpdatesX_WG) {
    ...
    vector<pair<FrameId, RigidTransformd>> parent_frames = {{InternalFrame::world_frame_id(), RigidTransformd{}},
                                                            {frames_[0], X_WFs_[0]},
                                                            {frames_[1], X_WFs_[1]},
                                                            {frames_[2], X_WFs_[2]}};
    for (const auto& [f_id, X_WF] : parent_frames) {
        auto instance = make_unique<GeometryInstance>(instance_pose_, make_unique<Sphere>(1.0), "instance");
        const GeometryId g_id = geometry_state_.RegisterGeometry(s_id, f_id, std::move(instance));
        const RigidTransformd X_WG_expected = X_WF * instance_->pose();
        EXPECT_TRUE(CompareMatrices(geometry_state_.get_pose_in_world(g_id).GetAsMatrix34(),
...
TEST_F(GeometryStateTest, RegisterDuplicateGeometry) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterGeometry(s_id, f_id, std::move(instance_copy)),
...
TEST_F(GeometryStateTest, RegisterGeometryMissingSource) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterGeometry(s_id, f_id, std::move(instance_)),
                                "Referenced geometry source \\d+ is not registered.");
}
...
TEST_F(GeometryStateTest, RegisterNullGeometry) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterGeometry(s_id, f_id, std::move(null_geometry)),
...
TEST_F(GeometryStateTest, RegisterAnchoredGeometry) {
    ...
    auto instance = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "sphere");
    const GeometryId expected_g_id = instance->id();
    const auto g_id = geometry_state_.RegisterAnchoredGeometry(s_id, std::move(instance));
    EXPECT_EQ(g_id, expected_g_id);
    EXPECT_TRUE(geometry_state_.BelongsToSource(g_id, s_id));
...
TEST_F(GeometryStateTest, RegisterDuplicateAnchoredGeometry) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterAnchoredGeometry(s_id, std::move(instance_copy)),
...
TEST_F(GeometryStateTest, RegisterAnchoredGeometryInvalidSource) {
    auto instance = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "sphere");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterAnchoredGeometry(SourceId::get_new_id(), std::move(instance)),
                                "Referenced geometry source \\d+ is not registered.");
}
...
TEST_F(GeometryStateTest, RegisterDeformableGeometry) {
    ...
    const Sphere sphere(1.0);
    constexpr double kRezHint = 0.5;
    const VolumeMesh<double> expected_mesh = internal::MakeSphereVolumeMesh<double>(
            sphere, kRezHint, internal::TessellationStrategy::kDenseInteriorVertices);

    /* Adding a deformable geometry to non-world frame throws. */
    auto instance1 = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(sphere), "sphere");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RegisterDeformableGeometry(s_id, f_id, std::move(instance1), kRezHint),
                                "Registering deformable geometry.*non-world frame.*");

    /* Successful registration of deformable geometry. */
    const RigidTransformd X_WG(AngleAxis<double>(M_PI_2, Vector3d::UnitX()), Vector3d{1, 2, 3});
    auto instance2 = make_unique<GeometryInstance>(X_WG, make_unique<Sphere>(sphere), "sphere");
    const GeometryId expected_g_id = instance2->id();
    const auto g_id = geometry_state_.RegisterDeformableGeometry(s_id, InternalFrame::world_frame_id(),
                                                                 std::move(instance2), kRezHint);
    EXPECT_EQ(g_id, expected_g_id);
    EXPECT_TRUE(geometry_state_.IsDeformableGeometry(g_id));

    /* Registers a non-deformable geometry. */
    auto instance3 = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(sphere), "sphere");
    const GeometryId non_deformable_g_id = geometry_state_.RegisterGeometry(s_id, f_id, std::move(instance3));
    ...
    const VolumeMesh<double>* reference_mesh = geometry_state_.GetReferenceMesh(g_id);
    ...
    const VectorX<double>& q_WG = geometry_state_.get_configurations_in_world(g_id);
    ...
    const std::vector<GeometryId> deformable_ids = geometry_state_.GetAllDeformableGeometryIds();
...
TEST_F(GeometryStateTest, GetAllDeformableGeometryIds) {
    ...
    auto instance1 = make_unique<GeometryInstance>(RigidTransformd::Identity(), sphere.Clone(), "deformable1");
    auto instance2 = make_unique<GeometryInstance>(RigidTransformd::Identity(), sphere.Clone(), "deformable2");
    ...
    geometry_state_.RegisterDeformableGeometry(s2_id, InternalFrame::world_frame_id(), std::move(instance2), kRezHint);
    geometry_state_.RegisterDeformableGeometry(s1_id, InternalFrame::world_frame_id(), std::move(instance1), kRezHint);

    /* Confirm that we get the expected ids in the expected order. */
    const vector<GeometryId> deformable_ids = geometry_state_.GetAllDeformableGeometryIds();
...
TEST_F(GeometryStateTest, ChangeShapeInternals) {
    const SourceId s_id = SetUpSingleSourceTree(Assign::kProximity | Assign::kPerception | Assign::kIllustration);
    ...
    EXPECT_TRUE(CompareMatrices(X_FG2.GetAsMatrix34(), geometry->X_FG().GetAsMatrix34()));
...
TEST_F(GeometryStateTest, ChangeShapeIllustration) {
    ...
    EXPECT_EQ(original_geo.illustration_properties()->GetProperty<int>("test", "value"),
...
ChangeShapeRenderEngine* AddRenderer(const char* name, GeometryState<double>* state) {
    state->AddRenderer(name, make_unique<ChangeShapeRenderEngine>());
    auto* engine = const_cast<render::RenderEngine*>(state->GetRenderEngineByName(name));
...
TEST_F(GeometryStateTest, ChangeShapePerception) {
    ...
    // processed.
    ChangeShapeRenderEngine* renderer1 = AddRenderer("renderer1", &geometry_state_);
    ChangeShapeRenderEngine* renderer2 = AddRenderer("renderer2", &geometry_state_);
    ...
    EXPECT_EQ(original_geo.perception_properties()->GetProperty<int>("test", "value"),
...
TEST_F(GeometryStateTest, ChangeShapeProximity) {
    ...
    const SignedDistancePair<double> old_distance =
            geometry_state_.ComputeSignedDistancePairClosestPoints(g_id, geometries_[2]);
    ...
    // this arbitrary (test, value) property; we'll confirm that it's still there
    // as *evidence* that the properties haven't changed.
    EXPECT_EQ(original_geo.proximity_properties()->GetProperty<int>("test", "value"),
...
TEST_F(GeometryStateTest, SetGeometryConfiguration) {
    ...
    auto deformable_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1.0), "deformable sphere");
    deformable_instance->set_proximity_properties(ProximityProperties());
    const auto deformable_id = geometry_state_.RegisterDeformableGeometry(s_id, InternalFrame::world_frame_id(),
                                                                          std::move(deformable_instance),
                                                                          /* resolution_hint */ 2.0);
    // Add a rigid geometry with a resolution hint (so that it can be in contact
    // with deformable geometries).
    auto rigid_instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Box>(1.5, 1.5, 1.5), "rigid_box");
    ...
    const FrameId f_id = geometry_state_.RegisterFrame(s_id, GeometryFrame("frame"));
    ...
    // the rigid geometry.
    const VectorX<double> q_WG_default = geometry_state_.get_configurations_in_world(deformable_id);
    ...
    gs_tester_.SetGeometryConfiguration(s_id, configurations, &gs_tester_.mutable_kinematics_data());
...
TEST_F(GeometryStateTest, DrivenMeshes) {
    ...
    illustration_properties.AddProperty("phong", "diffuse", Rgba(0.1, 0.2, 0.3, 0.4));
    ...
    // octahedron.
    for (const auto role : std::vector<Role>{Role::kIllustration, Role::kPerception, Role::kProximity}) {
    ...
    const std::vector<VectorX<double>> reference_proximity_configuration =
            geometry_state_.GetDrivenMeshConfigurationsInWorld(deformable_id, Role::kProximity);
    ...
    // are also scaled.
    const VectorX<double> reference_q_WG = geometry_state_.get_configurations_in_world(deformable_id);
    GeometryConfigurationVector<double> new_configurations;
    new_configurations.set_value(deformable_id, 2.0 * reference_q_WG);
    gs_tester_.SetGeometryConfiguration(s_id, new_configurations, &gs_tester_.mutable_kinematics_data());
    for (const auto role : std::vector<Role>{Role::kIllustration, Role::kPerception, Role::kProximity}) {
        const std::vector<VectorX<double>> configuration =
                geometry_state_.GetDrivenMeshConfigurationsInWorld(deformable_id, role);
        EXPECT_TRUE(CompareMatrices(configuration[0], reference_proximity_configuration[0]));
    }
    ...
    EXPECT_THROW(geometry_state_.GetDrivenRenderMeshes(deformable_id, Role::kIllustration), std::exception);
    EXPECT_THROW(geometry_state_.GetDrivenMeshConfigurationsInWorld(deformable_id, Role::kIllustration),
                 std::exception);
}
...
TEST_F(GeometryStateTest, RemoveGeometry) {
    ...
    const SourceId s_id = SetUpSingleSourceTree(Assign::kProximity | Assign::kPerception);
    ...
    EXPECT_EQ(geometry_state_.get_num_geometries(), single_tree_rigid_geometry_count() - 1);
    EXPECT_EQ(geometry_state_.NumDynamicGeometries(), single_tree_dynamic_rigid_geometry_count() - 1);
    ...
    DRAKE_EXPECT_NO_THROW(
            added_id = geometry_state_.RegisterGeometry(
                    ...
                    make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "newest")));
    ...
    DRAKE_EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, added_id, ProximityProperties()));
...
TEST_F(GeometryStateTest, RemoveGeometryInvalid) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveGeometry(SourceId::get_new_id(), geometries_[0]),
                                "Referenced geometry source \\d+ is not registered.");

    // Case: Invalid geometry id, valid source id.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveGeometry(s_id, GeometryId::get_new_id()),
    ...
    const GeometryId g_id = geometry_state_.RegisterGeometry(
            s_id2, frame_id,
            make_unique<GeometryInstance>(RigidTransformd::Identity(), unique_ptr<Shape>(new Sphere(1)), "new"));
    EXPECT_EQ(geometry_state_.get_num_geometries(), single_tree_rigid_geometry_count() + 1);
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveGeometry(s_id, g_id),
...
TEST_F(GeometryStateTest, RemoveAnchoredGeometry) {
    ...
    const auto anchored_id_1 = geometry_state_.RegisterAnchoredGeometry(
            s_id,
            make_unique<GeometryInstance>(HalfSpace::MakePose(normal_W, p_WB), make_unique<HalfSpace>(), "anchored1"));
...
TEST_F(GeometryStateTest, RemoveGeometryWithCollisionFilters) {
    ...
    auto is_filtered = [this](int index0, int index1) {
        return geometry_state_.CollisionFiltered(geometries_[index0], geometries_[index1]);
    };
...
TEST_F(GeometryStateTest, GetPoseForBadGeometryId) {
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetPoseInFrame(GeometryId::get_new_id()),
                                "Referenced geometry \\d+ has not been registered.");
}
...
TEST_F(GeometryStateTest, SourceOwnershipInvalidSource) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(FrameId::get_new_id(), source_id),
                                "Referenced geometry source \\d+ is not registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(GeometryId::get_new_id(), source_id),
                                "Referenced geometry source \\d+ is not registered.");
    SetUpSingleSourceTree();
    const GeometryId anchored_id = geometry_state_.RegisterAnchoredGeometry(
            source_id_, make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "sphere"));
    // Valid frame/geometry ids.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(frames_[0], source_id),
                                "Referenced geometry source \\d+ is not registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(geometries_[0], source_id),
                                "Referenced geometry source \\d+ is not registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(anchored_id, source_id),
                                "Referenced geometry source \\d+ is not registered.");
}
...
TEST_F(GeometryStateTest, SourceOwnershipFrameId) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(FrameId::get_new_id(), s_id),
...
TEST_F(GeometryStateTest, SourceOwnershipGeometryId) {
    const SourceId s_id = SetUpSingleSourceTree();
    const GeometryId anchored_id = geometry_state_.RegisterAnchoredGeometry(
            s_id, make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), "sphere"));
    // Test for invalid geometry.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.BelongsToSource(GeometryId::get_new_id(), s_id),
                                "Referenced geometry \\d+ has not been registered.");
...
TEST_F(GeometryStateTest, GetFrameIdFromBadId) {
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetFrameId(GeometryId::get_new_id()),
                                "Referenced geometry \\d+ has not been registered.");
}
...
TEST_F(GeometryStateTest, ValidateFrameIds) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gs_tester_.ValidateFrameIds(s_id, frame_set_2),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gs_tester_.ValidateFrameIds(s_id, frame_set_3),
...
TEST_F(GeometryStateTest, SetFramePoses) {
    ...
    for (int i = 0; i < total_geom; ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(world_poses.at(id).GetAsMatrix34(), X_FGs_[i].GetAsMatrix34()));
    }
    ...
    for (int i = 0; i < total_geom; ++i) {
        const GeometryId id = geometries_[i];
        EXPECT_TRUE(CompareMatrices(world_poses.at(id).GetAsMatrix34(), (offset * X_FGs_[i]).GetAsMatrix34()));
    }
    ...
    for (int i = 0; i < total_geom; ++i) {
        ...
        if (i < (kFrameCount - 1) * kGeometryCount) {
            ...
            EXPECT_TRUE(
                    CompareMatrices(world_poses.at(id).GetAsMatrix34(), (offset * offset * X_FGs_[i]).GetAsMatrix34()));
        }
    }
}
...
TEST_F(GeometryStateTest, QueryFrameProperties) {
    ...
    EXPECT_EQ(geometry_state_.GetFrameGroup(world), InternalFrame::world_frame_group());
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetFrameGroup(FrameId::get_new_id()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetName(FrameId::get_new_id()),
    ...
    EXPECT_TRUE(
            CompareMatrices(geometry_state_.get_pose_in_world(frames_[0]).GetAsMatrix34(), X_WFs_[0].GetAsMatrix34()));
    EXPECT_TRUE(CompareMatrices(geometry_state_.get_pose_in_world(world).GetAsMatrix34(),
                                RigidTransformd::Identity().GetAsMatrix34()));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.get_pose_in_world(FrameId::get_new_id()),
    ...
    EXPECT_TRUE(
            CompareMatrices(geometry_state_.get_pose_in_world(geometries_[0]).GetAsMatrix34(), X_WG.GetAsMatrix34()));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.get_pose_in_world(GeometryId::get_new_id()),
                                "No world pose available for invalid geometry id: \\d+");
    EXPECT_TRUE(CompareMatrices(geometry_state_.get_pose_in_world(anchored_geometry_).GetAsMatrix34(),
                                X_WA_.GetAsMatrix34()));

    EXPECT_TRUE(
            CompareMatrices(geometry_state_.get_pose_in_parent(frames_[0]).GetAsMatrix34(), X_PFs_[0].GetAsMatrix34()));
    EXPECT_TRUE(CompareMatrices(geometry_state_.get_pose_in_parent(world).GetAsMatrix34(),
                                RigidTransformd::Identity().GetAsMatrix34()));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.get_pose_in_parent(FrameId::get_new_id()),
...
TEST_F(GeometryStateTest, TestCollisionCandidates) {
    ...
    set<pair<GeometryId, GeometryId>> expected_candidates = {
            ...
            {geometries_[0], anchored_geometry_}, {geometries_[1], geometries_[2]},
            {geometries_[1], geometries_[3]},     {geometries_[1], geometries_[4]},
            {geometries_[1], geometries_[5]},     {geometries_[1], anchored_geometry_},
            {geometries_[2], geometries_[4]},     {geometries_[2], geometries_[5]},
            {geometries_[2], anchored_geometry_}, {geometries_[3], geometries_[4]},
            {geometries_[3], geometries_[5]},     {geometries_[3], anchored_geometry_},
            {geometries_[4], anchored_geometry_}, {geometries_[5], anchored_geometry_}};

    auto candidates_in_set = [](const set<pair<GeometryId, GeometryId>>& candidates,
                                const set<pair<GeometryId, GeometryId>>& expected) {
        ::testing::AssertionResult result = ::testing::AssertionSuccess();
        if (candidates != expected) {
            result = ::testing::AssertionFailure();
            auto print_difference = [&result](const auto& set1, const auto& set2, const char* msg) {
            ...
            print_difference(candidates, expected, "The following pairs were reported but not expected:");
            print_difference(expected, candidates, "The following pairs were expected but missing:");
        }
        return result;
    };

    // Confirm initial conditions.
    EXPECT_TRUE(candidates_in_set(geometry_state_.GetCollisionCandidates(), expected_candidates));
    ...
    while (!expected_candidates.empty()) {
        ...
        geometry_state_.collision_filter_manager().Apply(
                CollisionFilterDeclaration().ExcludeBetween(GeometrySet{pair->first}, GeometrySet{pair->second}));
...
TEST_F(GeometryStateTest, NonProximityRoleInCollisionFilter) {
    ...
    gs_tester_.SetFramePoses(source_id_, poses, &gs_tester_.mutable_kinematics_data());
    gs_tester_.FinalizePoseUpdate();
    ...
    GeometryId added_id = geometry_state_.RegisterGeometry(
            source_id_, frames_[2], make_unique<GeometryInstance>(pose, make_unique<Sphere>(1), name));
    gs_tester_.FinalizePoseUpdate();
    ...
    geometry_state_.collision_filter_manager().Apply(
            CollisionFilterDeclaration().ExcludeBetween(GeometrySet{added_id}, GeometrySet{anchored_geometry_}));
    pairs = geometry_state_.ComputePointPairPenetration();
    EXPECT_EQ(static_cast<int>(pairs.size()), expected_collisions);
...
TEST_F(GeometryStateTest, CollisionFilterRespectsScope) {
    ...
    auto instance =
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(sphere), "deformable");
    instance->set_proximity_properties(ProximityProperties());
    GeometryId deformable_id = geometry_state_.RegisterDeformableGeometry(s_id, InternalFrame::world_frame_id(),
                                                                          std::move(instance), kRezHint);
    internal::DeformableContact<double> contacts;
    geometry_state_.ComputeDeformableContact(&contacts);
    EXPECT_EQ(contacts.contact_surfaces().size(), single_tree_rigid_geometry_count());
    ...
    geometry_state_.collision_filter_manager().Apply(
            CollisionFilterDeclaration(CollisionFilterScope::kOmitDeformable)
                    .ExcludeBetween(GeometrySet{deformable_id}, GeometrySet(geometries_)));
    geometry_state_.ComputeDeformableContact(&contacts);
    EXPECT_EQ(contacts.contact_surfaces().size(), single_tree_rigid_geometry_count());
    ...
    geometry_state_.collision_filter_manager().Apply(
            CollisionFilterDeclaration(CollisionFilterScope::kAll)
                    .ExcludeBetween(GeometrySet{deformable_id}, GeometrySet({geometries_[0], anchored_geometry_})));
    geometry_state_.ComputeDeformableContact(&contacts);
    EXPECT_EQ(contacts.contact_surfaces().size(), single_tree_rigid_geometry_count() - 2);
}
...
TEST_F(GeometryStateTest, CollisionFilteredExceptions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.CollisionFiltered(geometries_[1], geometries_[0]),
                                ".* " + to_string(geometries_[1]) + " does not have a proximity role");

    // Case: Second argument does not have a proximity role.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.CollisionFiltered(geometries_[0], geometries_[1]),
                                ".* " + to_string(geometries_[1]) + " does not have a proximity role");

    // Case: Neither argument has a proximity role.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.CollisionFiltered(geometries_[1], geometries_[2]),
    ...
    EXPECT_TRUE(geometry_state_.CollisionFiltered(geometries_[0], geometries_[1]));

    const GeometryId bad_id = GeometryId::get_new_id();

    // Case: First argument is bad.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.CollisionFiltered(bad_id, geometries_[0]),
                                "Can't report collision filter status between geometries .* " + to_string(bad_id) +
                                        " is not a valid geometry");

    // Case: Second argument is bad.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.CollisionFiltered(geometries_[0], bad_id),
                                "Can't report collision filter status between geometries .* " + to_string(bad_id) +
                                        " is not a valid geometry");
...
TEST_F(GeometryStateTest, GetGeometryIdFromName) {
    ...
    for (int f = 0; f < kFrameCount; ++f) {
        for (int g = 0; g < kGeometryCount; ++g) {
            ...
            EXPECT_EQ(geometry_state_.GetGeometryIdByName(frames_[f], Role::kProximity, geometry_names_[g_index]),
                      expected_id);
    ...
    // Grab anchored.
    EXPECT_EQ(geometry_state_.GetGeometryIdByName(InternalFrame::world_frame_id(), Role::kProximity, anchored_name_),
              anchored_geometry_);

    // Failure cases.

    // Look up with the wrong name.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetGeometryIdByName(frames_[0], Role::kProximity, "bad"),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.GetGeometryIdByName(FrameId::get_new_id(), Role::kUnassigned, "irrelevant"),
            "Referenced frame \\d+ has not been registered.");

    // Bad *anchored* geometry name.
    const FrameId world_id = gs_tester_.get_world_frame();
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetGeometryIdByName(world_id, Role::kUnassigned, "bad"),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetGeometryIdByName(frames_[0], Role::kUnassigned, "bad_name"),
    ...
    for (int i = 0; i < 2; ++i) {
        geometry_state_.RegisterGeometry(
                source_id_, frames_[0],
                make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), dummy_name));
    }
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.GetGeometryIdByName(frames_[0], Role::kUnassigned, dummy_name),
...
TEST_F(GeometryStateTest, GeometryNameStorage) {
    ...
    // White space trimmed off and new string stored.
    {
        const GeometryId id = geometry_state_.RegisterGeometry(
                source_id_, frames_[0],
                make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), " " + name));
...
TEST_F(GeometryStateTest, GeometryAncestryStorage) {
    ...
    for (const FrameId frame_id : geometry_state_.get_frame_ids()) {
        ...
        const std::string& parent_frame_name = geometry_state_.GetName(parent_frame_id);
...
TEST_F(GeometryStateTest, GeometryNameValidation) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.IsValidGeometryName(FrameId::get_new_id(), Role::kProximity, ""),
                                "Given frame id is not valid: \\d+");

    auto expect_bad_name = [this](const string& name, const string& exception_message, const string& printable_name) {
        EXPECT_FALSE(geometry_state_.IsValidGeometryName(frames_[0], Role::kProximity, name))
                << "Failed on input name: " << printable_name;
    };

    // Unique in world frame.
    const FrameId world = InternalFrame::world_frame_id();
    EXPECT_FALSE(geometry_state_.IsValidGeometryName(world, Role::kProximity, anchored_name_));
    EXPECT_TRUE(geometry_state_.IsValidGeometryName(world, Role::kIllustration, anchored_name_));
    EXPECT_TRUE(geometry_state_.IsValidGeometryName(world, Role::kProximity, anchored_name_ + "2"));
    ...
    const string whitespace_message{"The proposed geometry name contains only whitespace"};
    ...
    EXPECT_TRUE(geometry_state_.IsValidGeometryName(frames_[0], Role::kProximity, unique));

    // Querying with non-canonical names test as the canonical name.
    const vector<string> names{" " + unique, unique + " ", " " + unique + " "};
    for (const auto& name : names) {
        EXPECT_TRUE(geometry_state_.IsValidGeometryName(frames_[0], Role::kProximity, name));
    }

    // Test potential duplicates.

    // A name with the same role will fail.
    EXPECT_FALSE(geometry_state_.IsValidGeometryName(frames_[0], Role::kProximity,
                                                     gs_tester_.get_geometries().at(geometries_[0]).name()));

    // A name with the *different* role will pass.
    EXPECT_TRUE(geometry_state_.IsValidGeometryName(frames_[0], Role::kIllustration,
                                                    gs_tester_.get_geometries().at(geometries_[0]).name()));
    ...
    for (const char* const s : {"\n", " \n\t", " \f", "\v", "\r", "\ntest"}) {
        EXPECT_TRUE(geometry_state_.IsValidGeometryName(frames_[0], Role::kProximity, s));
    }
}
...
TEST_F(GeometryStateTest, AssignRolesToGeometry) {
    ...
    for (int i = 0; i < 8 - single_tree_dynamic_rigid_geometry_count(); ++i) {
        ...
        geometries_.push_back(geometry_state_.RegisterGeometry(
                source_id_, frames_[0], make_unique<GeometryInstance>(pose, make_unique<Sphere>(1), name)));
    }

    auto set_roles = [this](GeometryId id, bool set_proximity, bool set_perception, bool set_illustration) {
        if (set_proximity) {
    ...
    auto has_expected_roles = [this](GeometryId id, bool has_proximity, bool has_perception,
        ...
        if (has_proximity != (geometry->proximity_properties() != nullptr)) {
            failure << "Proximity role: " << (has_proximity ? "expected, but not found" : "not expected, but found. ");
            passes = false;
        }
        if (has_perception != (geometry->perception_properties() != nullptr)) {
            failure << "Perception role: "
                    << (has_perception ? "expected, but not found" : "not expected, but found. ");
            passes = false;
        }
        if (has_perception != geometry->has_perception_role()) {
            failure << "Perception role: " << (has_perception ? "expected, but not found" : "not expected, but found");
            passes = false;
        }
        if (has_illustration != (geometry->illustration_properties() != nullptr)) {
            failure << "Illustration role: "
                    << (has_illustration ? "expected, but not found" : "not expected, but found. ");
            passes = false;
        }
        if (passes)
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        EXPECT_TRUE(has_expected_roles(id, false, false, false))
                << "Geometry " << id << " at index (" << i << ") didn't start without roles";
...
TEST_F(GeometryStateTest, ConvexHullForProximityGeometry) {
    ...
    DRAKE_DEMAND(geometry_state_.GetProximityProperties(geometries_[0]) != nullptr);
    EXPECT_EQ(geometry_state_.GetConvexHull(geometries_[0]), nullptr);

    // A shape that *does* require a convex hull has one.
    const std::string mesh_name = FindResourceOrThrow("drake/geometry/test/cube_with_hole.obj");
    const GeometryId mesh_id = geometry_state_.RegisterAnchoredGeometry(
            source_id_, make_unique<GeometryInstance>(RigidTransformd{}, Mesh(mesh_name, 1), "mesh_with_hull"));
    ...
    EXPECT_FALSE(geometry_state_.GetProximityProperties(mesh_id)->HasProperty("test", "value"));
    ProximityProperties alt_props;
    alt_props.AddProperty("test", "value", 17);
    geometry_state_.AssignRole(source_id_, mesh_id, alt_props, RoleAssign::kReplace);
    DRAKE_DEMAND(geometry_state_.GetProximityProperties(mesh_id)->HasProperty("test", "value"));
    EXPECT_NE(geometry_state_.GetConvexHull(mesh_id), nullptr);
...
TEST_F(GeometryStateTest, ModifyProximityProperties) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.AssignRole(source_id_, geometries_[0], empty_props, RoleAssign::kReplace),
    ...
    const auto& hydroelastic_geometries =
            internal::ProximityEngineTester::hydroelastic_geometries(gs_tester_.proximity_engine());
    ...
    const ProximityProperties* props = geometry_state_.GetProximityProperties(geometries_[0]);
    EXPECT_NE(props, nullptr);
    EXPECT_TRUE(props->HasGroup("prop1"));
    EXPECT_TRUE(props->HasProperty("prop1", "value"));
    EXPECT_EQ(props->GetProperty<int>("prop1", "value"), props1.GetProperty<int>("prop1", "value"));
    EXPECT_EQ(hydroelastic_geometries.hydroelastic_type(geometries_[0]), internal::HydroelasticType::kUndefined);

    DRAKE_EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometries_[0], props2, RoleAssign::kReplace));
    ...
    EXPECT_EQ(props->GetProperty<int>("prop2", "value"), props2.GetProperty<int>("prop2", "value"));
    EXPECT_EQ(hydroelastic_geometries.hydroelastic_type(geometries_[0]), internal::HydroelasticType::kRigid);
}
...
TEST_F(GeometryStateTest, ModifyIllustrationProperties) {
    ...
    EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometry_id, empty_props));
    const IllustrationProperties* props = geometry_state_.GetIllustrationProperties(geometry_id);
    ASSERT_NE(props, nullptr);
    ASSERT_EQ(props->num_groups(), 1);  // Just the default group.

    // Now re-assign.
    EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometry_id, props1, RoleAssign::kReplace));
    ...
    // Now re-assign again.
    EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometry_id, props2, RoleAssign::kReplace));
    props = geometry_state_.GetIllustrationProperties(geometry_id);
    ASSERT_NE(props, nullptr);
...
TEST_F(GeometryStateTest, ModifyPerceptionProperties) {
    ...
    PerceptionProperties perception_props = render_engine_->accepting_properties();
    perception_props.AddProperty("label", "id", RenderLabel(10));
    EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometries_[1], perception_props));
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.AssignRole(source_id_, geometries_[1], perception_props, RoleAssign::kReplace),
...
TEST_F(GeometryStateTest, RoleLookUp) {
    SetUpSingleSourceTree(Assign::kProximity | Assign::kIllustration | Assign::kPerception);
    GeometryId no_role_id = geometry_state_.RegisterGeometry(
            source_id_, frames_[0],
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(0.5), "no_roles"));
    GeometryId invalid_id = GeometryId::get_new_id();

    // Invalid id throws.
    EXPECT_THROW(geometry_state_.GetProximityProperties(invalid_id), std::logic_error);
    EXPECT_THROW(geometry_state_.GetIllustrationProperties(invalid_id), std::logic_error);
    EXPECT_THROW(geometry_state_.GetPerceptionProperties(invalid_id), std::logic_error);
...
TEST_F(GeometryStateTest, InstanceRoleAssignment) {
    ...
    auto make_instance = [this](const string& name) {
        return make_unique<GeometryInstance>(instance_pose_, make_unique<Sphere>(1.0), name);
    };
    ...
    {
        const GeometryId g_id = geometry_state_.RegisterGeometry(s_id, f_id, make_instance("instance1"));
        const InternalGeometry* geometry = gs_tester_.GetGeometry(g_id);
...
TEST_F(GeometryStateTest, RolePropertyValueAssignment) {
    ...
    const ProximityProperties* read = gs_tester_.GetGeometry(geometries_[0])->proximity_properties();
    ...
    auto num_group_properties = [](const ProximityProperties& properties, const string& group_name) {
    ...
    auto num_total_properties = [num_group_properties](const ProximityProperties& properties) {
        int count = 0;
    ...
    EXPECT_EQ(num_group_properties(source, default_group), num_group_properties(*read, default_group));
    EXPECT_EQ(source.GetProperty<int>(default_group, "prop1"), read->GetProperty<int>(default_group, "prop1"));
    EXPECT_EQ(source.GetProperty<int>(default_group, "prop2"), read->GetProperty<int>(default_group, "prop2"));

    EXPECT_EQ(num_group_properties(source, group1), num_group_properties(*read, group1));
    EXPECT_EQ(source.GetProperty<double>(group1, "propA"), read->GetProperty<double>(group1, "propA"));
    EXPECT_EQ(source.GetProperty<string>(group1, "propB"), read->GetProperty<string>(group1, "propB"));
}
...
TEST_F(GeometryStateTest, RoleAssignExceptions) {
    ...
    // Invalid source.
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.AssignRole(SourceId::get_new_id(), geometries_[0], ProximityProperties()),
            "Referenced geometry source \\d+ is not registered.");

    // Invalid geometry id.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, GeometryId::get_new_id(), ProximityProperties()),
                                "Referenced geometry \\d+ has not been registered.");

    // Geometry not owned by source.
    const SourceId other_source = geometry_state_.RegisterNewSource("alt_source");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(other_source, geometries_[0], ProximityProperties()),
    ...
    DRAKE_EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometries_[0], ProximityProperties()));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, geometries_[0], ProximityProperties()),
                                "Trying to assign the 'proximity' role to geometry id \\d+ for the first "
                                "time.*");

    DRAKE_EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometries_[0], perception_props));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, geometries_[0], perception_props),
                                "Trying to assign the 'perception' role to geometry id \\d+ for the "
                                "first time.*");

    DRAKE_EXPECT_NO_THROW(geometry_state_.AssignRole(source_id_, geometries_[0], IllustrationProperties()));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, geometries_[0], IllustrationProperties()),
    ...
    const GeometryId new_id = geometry_state_.RegisterGeometry(
            source_id_, frames_[0],
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(1), geometry_names_[0]));
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, new_id, ProximityProperties()),
                                "The name .* has already been used by a geometry with the 'proximity' "
                                "role.");

    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, new_id, IllustrationProperties()),
                                "The name .* has already been used by a geometry with the 'illustration' "
                                "role.");

    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AssignRole(source_id_, new_id, perception_props),
...
TEST_F(GeometryStateTest, ChildGeometryRoleCount) {
    ...
    auto expected_roles = [this](FrameId f_id, int num_proximity, int num_perception,
        ...
        for (const auto& pair : roles) {
            ...
            const int actual_count = geometry_state_.NumGeometriesWithRole(f_id, role);
            if (actual_count != expected_count) {
                success = false;
                failure << "\nExpected " << expected_count << " geometries with the " << role << " role. Found "
                        << actual_count;
            }
        }
        if (success)
    ...
    const FrameId f_id = frames_[0];
    ASSERT_TRUE(expected_roles(f_id, proximity_count, perception_count, illustration_count));
    ...
    geometry_state_.AssignRole(source_id_, anchored_geometry_, ProximityProperties());
    ...
    geometry_state_.AssignRole(source_id_, anchored_geometry_, IllustrationProperties());
...
TEST_F(GeometryStateTest, RemoveUnassignedRole) {
    ...
    EXPECT_EQ(geometry_state_.RemoveRole(source_id_, geometries_[0], Role::kUnassigned), 0);
    EXPECT_EQ(geometry_state_.RemoveRole(source_id_, frames_[0], Role::kUnassigned), 0);
...
TEST_F(GeometryStateTest, RemoveGeometryFromRenderer) {
    ...
    auto confirm_renderers = [this, &other_engine](set<GeometryId> removed_from_renderer) {
        ...
        for (GeometryId id : ids) {
            ...
            EXPECT_EQ(other_engine.has_geometry(id), !removed_from_renderer.contains(id));
        }
    };
    ...
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, remove_id), 1);
    ...
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, remove_id), 0);
    confirm_renderers(removed_ids);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.RemoveFromRenderer(other_renderer_name, SourceId::get_new_id(), remove_id),
            "Referenced geometry source .* is not registered.");

    // Case: GeometryId does not map to a registered geometry.
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, GeometryId::get_new_id()),
            "Referenced geometry .* has not been registered.");

    // Case: GeometryId does not belong to SourceId.
    const SourceId source_id_2 = geometry_state_.RegisterNewSource("second_source");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_2, remove_id),
...
TEST_F(GeometryStateTest, RemoveFrameFromRenderer) {
    ...
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, frames_[0]), 2);
    ...
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, frames_[0]), 0);
    confirm_renderers(removed_ids);
    ...
    geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, geometries_[2]);
    removed_ids.insert(geometries_[2]);
    confirm_renderers(removed_ids);
    // Now remove the parent frame.
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, frames_[1]), 1);
    ...
    // frame.
    const SourceId source_id_2 = geometry_state_.RegisterNewSource("second_source");
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_2, InternalFrame::world_frame_id()), 0);
    confirm_renderers(removed_ids);

    // Case: Source with registered anchored geometry removing from world frame.
    EXPECT_EQ(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, InternalFrame::world_frame_id()), 1);
    ...
    // Case: Source id does not map to a registered source.
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.RemoveFromRenderer(other_renderer_name, SourceId::get_new_id(), frames_[0]),
            "Referenced geometry source .* is not registered.");

    // Case: Frame does not map to a registered frame.
    DRAKE_EXPECT_THROWS_MESSAGE(
            geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_, FrameId::get_new_id()),
            "Referenced frame .* but the frame doesn't belong to the source.");

    // Case: FrameId does not belong to SourceId.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveFromRenderer(other_renderer_name, source_id_2, frames_[0]),
                                "Referenced frame .+ but the frame doesn't belong to the source.");
}
...
TEST_F(GeometryStateTest, AddRendererAfterGeometry) {
    ...
    const GeometryId id_no_perception = geometry_state_.RegisterGeometry(
            source_id_, frames_[0],
            make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(0.5), "shape"));
...
TEST_F(GeometryStateTest, AddRendererError) {
    ...
    DRAKE_EXPECT_NO_THROW(geometry_state_.AddRenderer(kName, make_unique<DummyRenderEngine>()));

    // Non-unique name.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.AddRenderer(kName, make_unique<DummyRenderEngine>()),
                                fmt::format("AddRenderer..: A renderer with the name '{}' already exists", kName));

    EXPECT_EQ(geometry_state_.GetRenderEngineByName("invliad_name"), nullptr);
    const render::RenderEngine* engine = geometry_state_.GetRenderEngineByName(kName);
...
TEST_F(GeometryStateTest, RespectAcceptingRendererProperty) {
    ...
    const DummyRenderEngine& first_renderer =
            dynamic_cast<const DummyRenderEngine&>(gs_tester_.GetRenderEngineOrThrow(kDummyRenderName));
    ...
    PerceptionProperties base_properties = DummyRenderEngine().accepting_properties();
    base_properties.AddProperty("phong", "diffuse", Vector4<double>{0.8, 0.8, 0.8, 1.0});
...

    {
        ...
        PerceptionProperties properties(base_properties);
        properties.AddProperty("renderer", "accepting", set<string>{kDummyRenderName});
...

    {
        ...
        PerceptionProperties properties(base_properties);
        properties.AddProperty("renderer", "accepting", set<string>{kDummyRenderName, second_name});
...
TEST_F(GeometryStateTest, PostHocRenderEngineRespectAcceptingRenderer) {
    ...
    geometry_state_.AssignRole(source_id_, geometries_[0], make_properties(kAccept));
    geometry_state_.AssignRole(source_id_, geometries_[1], make_properties(kReject));
    geometry_state_.AssignRole(source_id_, geometries_[2], make_properties(kEmpty));
    geometry_state_.AssignRole(source_id_, geometries_[3], make_properties(kNoStatement));
...
TEST_F(GeometryStateTest, GetRenderEngine) {
    const render::RenderEngine& engine = gs_tester_.GetRenderEngineOrThrow(kDummyRenderName);
    EXPECT_EQ(&engine, render_engine_);
    DRAKE_EXPECT_THROWS_MESSAGE(gs_tester_.GetRenderEngineOrThrow("bad name"), "No renderer exists with name.*");
}
...
TEST_F(GeometryStateTest, RendererPoseUpdate) {
    ...
    auto expect_poses = [](const auto& test, const auto& expected) {
        ...
        for (const auto& expected_id_pose_pair : expected) {
            ...
            EXPECT_TRUE(
                    CompareMatrices(test_iter->second.GetAsMatrix34(), expected_id_pose_pair.second.GetAsMatrix34()));
        }
    };
...
TEST_F(GeometryStateTest, DrivenMeshData) {
    ...
    auto instance = make_unique<GeometryInstance>(RigidTransformd::Identity(), make_unique<Sphere>(sphere), "sphere");
    GeometryId g_id = geometry_state_.RegisterDeformableGeometry(s_id, InternalFrame::world_frame_id(),
                                                                 std::move(instance), kRezHint);
...

    {
        ...
        // mesh.
        for (const auto role : std::vector<Role>{Role::kProximity, Role::kIllustration, Role::kPerception}) {
        ...
        const internal::DrivenMeshData& data = gs_tester_.driven_mesh_data(Role::kPerception);
...
TEST_F(GeometryStateTest, RendererConfigurationUpdate) {
    ...
    const std::vector<VectorXd> old_q_WG = render_engine_->world_configurations(g_id);
    ...
    gs_tester_.mutable_kinematics_data().q_WGs[g_id] = VectorXd::LinSpaced(3 * kNumControlMeshVertices, 0.0, 1.0);
    gs_tester_.FinalizeConfigurationUpdate();
    const std::vector<VectorXd> new_q_WG = render_engine_->world_configurations(g_id);
...
TEST_F(GeometryStateTest, GeometryVersionUpdate) {
    ...
    SourceId new_source = VerifyVersionUnchanged(&GeometryState<double>::RegisterNewSource, "my_new_source");
    // Registering a new frame does not modify the versions.
    FrameId new_frame_0 =
            VerifyVersionUnchanged(static_cast<FrameId (GeometryState<double>::*)(SourceId, const GeometryFrame&)>(
                                           &GeometryState<double>::RegisterFrame),
                                   new_source, GeometryFrame("new_f0"));
    VerifyVersionUnchanged(static_cast<FrameId (GeometryState<double>::*)(SourceId, FrameId, const GeometryFrame&)>(
                                   &GeometryState<double>::RegisterFrame),
    ...
    VerifyVersionUnchanged(
            ...
            std::make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(1), "new_geometry_0"));
    VerifyVersionUnchanged(
            &GeometryState<double>::RegisterAnchoredGeometry, new_source,
            std::make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(1), "new_geometry_2"));
    ...
    VerifyRoleVersionModified(
            Role::kProximity,
            static_cast<void (GeometryState<double>::*)(SourceId, GeometryId, ProximityProperties, RoleAssign)>(
    ...
    PerceptionProperties base_perception_properties = DummyRenderEngine().accepting_properties();
    base_perception_properties.AddProperty("phong", "diffuse", Vector4<double>{0.8, 0.8, 0.8, 1.0});
    base_perception_properties.AddProperty("label", "id", RenderLabel::kDontCare);
    {
        ...
        perception_properties.AddProperty("renderer", "accepting", set<string>{kDummyRenderName});
        VerifyRoleVersionModified(
                Role::kPerception,
                static_cast<void (GeometryState<double>::*)(SourceId, GeometryId, PerceptionProperties, RoleAssign)>(
                        &GeometryState<double>::AssignRole),
    ...
    {
        PerceptionProperties perception_properties(base_perception_properties);
        perception_properties.AddProperty("renderer", "accepting", set<string>{"junk"});
        VerifyVersionUnchanged(
                static_cast<void (GeometryState<double>::*)(SourceId, GeometryId, PerceptionProperties, RoleAssign)>(
                        &GeometryState<double>::AssignRole),
    ...
    // the other versions.
    IllustrationProperties illustration_properties;
    illustration_properties.AddProperty("phong", "diffuse", Vector4<double>{0.8, 0.8, 0.8, 1.0});
    VerifyRoleVersionModified(
            Role::kIllustration,
            static_cast<void (GeometryState<double>::*)(SourceId, GeometryId, IllustrationProperties, RoleAssign)>(
                    &GeometryState<double>::AssignRole),
    ...
    VerifyRoleVersionModified(
            Role::kProximity,
            static_cast<int (GeometryState<double>::*)(SourceId, GeometryId, Role)>(&GeometryState<double>::RemoveRole),
    ...
    // modify any version.
    VerifyVersionUnchanged(static_cast<int (GeometryState<double>::*)(const std::string&, SourceId, GeometryId)>(
    ...
    PerceptionProperties perception_properties(base_perception_properties);
    perception_properties.AddProperty("renderer", "accepting", set<string>{kDummyRenderName, "second"});
    geometry_state_.AssignRole(source_id_, geometries_[1], perception_properties, RoleAssign::kNew);
    ...
    VerifyRoleVersionModified(Role::kPerception,
                              static_cast<int (GeometryState<double>::*)(const std::string&, SourceId, GeometryId)>(
                                      &GeometryState<double>::RemoveFromRenderer),
                              kDummyRenderName, source_id_, geometries_[1]);
    ...
    // proximity filters.
    for (int i = 0; i < 3; ++i) {
        geometry_state_.AssignRole(source_id_, geometries_[i], ProximityProperties(), RoleAssign::kNew);
    }

    VerifyRoleVersionModified(Role::kProximity, &GeometryState<double>::collision_filter_manager);
    ...
    VerifyRoleVersionModified(Role::kPerception, &GeometryState<double>::AddRenderer, "second",
    ...
    // modify version.
    VerifyVersionUnchanged(&GeometryState<double>::AddRenderer, "third", make_unique<DummyRenderEngine>());
}
...
TEST_F(GeometryStateTest, ComputeSignedDistancePairClosestPointsError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(proximity1, bad_id),
                                ".*has not been registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(bad_id, proximity1),
                                ".*has not been registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(no_role, proximity1),
                                ".*has no role.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(proximity1, no_role),
                                ".*has no role.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(proximity1, illus),
                                ".*has the illustration role.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(illus, proximity1),
                                ".*has the illustration role.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(proximity1, percep),
                                ".*has the perception role.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.ComputeSignedDistancePairClosestPoints(percep, proximity1),
                                ".*has the perception role.");
}
...
protected:
    ...
    GeometryId AddSphere(const std::string name, const ProximityProperties* props) {
        auto instance = std::make_unique<GeometryInstance>(math::RigidTransformd{}, Sphere(0.1), name);
        if (props != nullptr) {
            instance->set_proximity_properties(*props);
        }
        return geometry_state_.RegisterGeometry(source_id_, frame_id_, std::move(instance));
    }

    DefaultProximityProperties empty_defaults_{"undefined", {}, {}, {}, {}, {}, {}, {}, {}};
    // Use stupid values distinct from those in MakeFullProperties().
    DefaultProximityProperties full_defaults_{"compliant", 0.1, 0.2, 0.3, 0.4, 0.4, 0.5, 0.6, 0.7};
    GeometryState<double> geometry_state_;
    SourceId source_id_{geometry_state_.RegisterNewSource("test")};
    FrameId frame_id_{geometry_state_.RegisterFrame(source_id_, GeometryFrame("frame"))};
};
...
TEST_F(ApplyProximityDefaultsTests, EmptyPropsEmptyDefaults) {
    ...
    ASSERT_NE(props, nullptr);
    EXPECT_EQ(props->GetProperty<HydroelasticType>(kHydroGroup, kComplianceType), HydroelasticType::kUndefined);
...
TEST_F(ApplyProximityDefaultsTests, EmptyPropsFullDefaults) {
    ...
    EXPECT_EQ(props->GetProperty<double>(kHydroGroup, kElastic), *full_defaults_.hydroelastic_modulus);
    EXPECT_EQ(props->GetProperty<double>(kHydroGroup, kRezHint), *full_defaults_.resolution_hint);
    EXPECT_EQ(props->GetProperty<double>(kHydroGroup, kSlabThickness), *full_defaults_.slab_thickness);

    auto friction = props->GetProperty<multibody::CoulombFriction<double>>(kMaterialGroup, kFriction);
    ...
    EXPECT_EQ(props->GetProperty<double>(kMaterialGroup, kHcDissipation), *full_defaults_.hunt_crossley_dissipation);
    EXPECT_EQ(props->GetProperty<double>(kMaterialGroup, kRelaxationTime), *full_defaults_.relaxation_time);
    EXPECT_EQ(props->GetProperty<double>(kMaterialGroup, kPointStiffness), *full_defaults_.point_stiffness);
}
...
TEST_F(ApplyProximityDefaultsTests, FullPropsFullDefaults) {
    ...
    auto friction = props->GetProperty<multibody::CoulombFriction<double>>(kMaterialGroup, kFriction);
    auto full_props_friction = full_props->GetProperty<multibody::CoulombFriction<double>>(kMaterialGroup, kFriction);
    EXPECT_EQ(friction.static_friction(), full_props_friction.static_friction());
    EXPECT_EQ(friction.dynamic_friction(), full_props_friction.dynamic_friction());
...
GTEST_TEST(GeometryStateHydroTest, GetHydroMesh) {
    ...
    // Case: id doesn't have proximity role.
    {
        const GeometryId id = geometry_state.RegisterAnchoredGeometry(
                source_id, make_unique<GeometryInstance>(X_WG, make_unique<Sphere>(1), "no_proximity"));
    ...
    {
        const GeometryId id = geometry_state.RegisterAnchoredGeometry(
                source_id, make_unique<GeometryInstance>(X_WG, make_unique<Sphere>(1), "no_hydro"));
    ...
    {
        const GeometryId id = geometry_state.RegisterAnchoredGeometry(
                source_id, make_unique<GeometryInstance>(X_WG, make_unique<Sphere>(1), "rigid_mesh"));
        geometry_state.AssignRole(source_id, id, rigid_hydro);

        const auto maybe_mesh = geometry_state.maybe_get_hydroelastic_mesh(id);
        EXPECT_TRUE(std::holds_alternative<const TriangleSurfaceMesh<double>*>(maybe_mesh));
        EXPECT_NE(std::get<const TriangleSurfaceMesh<double>*>(maybe_mesh), nullptr);
    }

    // Case: id has volume mesh.
    {
        const GeometryId id = geometry_state.RegisterAnchoredGeometry(
                source_id, make_unique<GeometryInstance>(X_WG, make_unique<Sphere>(1), "soft_mesh"));
    ...
    {
        const GeometryId id = geometry_state.RegisterAnchoredGeometry(
                source_id, make_unique<GeometryInstance>(X_WG, make_unique<HalfSpace>(), "no_hydro_mesh"));
        geometry_state.AssignRole(source_id, id, rigid_hydro);
...
// the role type.
class RemoveRoleTests : public GeometryStateTestBase, public testing::TestWithParam<Role> {
protected:
    void SetUp() {
        TestInit();
        ...
        SetUpSingleSourceTree(Assign::kPerception | Assign::kProximity | Assign::kIllustration);
    }
    ...
    void ExpectAllRolesExcept(const set<GeometryId>& ids_without_role, Role role_to_remove) const {
        for (GeometryId id : geometries_) {
            for (Role role : {Role::kProximity, Role::kIllustration, Role::kPerception}) {
                if (role == role_to_remove) {
                    ...
                    EXPECT_EQ(gs_tester_.GetGeometry(id)->has_role(role), !ids_without_role.contains(id));
                } else {
    ...
    void TestRemoveRoleFromGeometry(Role role_to_remove) {
        ...
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, geometries_[0], role_to_remove), 1);
        ...
        // no effect.
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, geometries_[0], role_to_remove), 0);
        EXPECT_FALSE(geometry->has_role(role_to_remove));
    ...
    void TestRemoveRoleFromFrame(Role role_to_remove) {
        ...
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, frames_[0], role_to_remove), 2);
        ExpectAllRolesExcept(ids_without_role, role_to_remove);

        // Case: attempting to remove role from the frame that has no geometries
        // with the role has no effect.
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, frames_[0], role_to_remove), 0);
        ExpectAllRolesExcept(ids_without_role, role_to_remove);
        ...
        // be affected.
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, frames_[1], role_to_remove), 1);
        ...
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_2, InternalFrame::world_frame_id(), role_to_remove), 0);
        ExpectAllRolesExcept(ids_without_role, role_to_remove);
        ...
        EXPECT_EQ(geometry_state_.RemoveRole(source_id_, InternalFrame::world_frame_id(), role_to_remove), 1);
        ids_without_role.insert(anchored_geometry_);
        ExpectAllRolesExcept(ids_without_role, role_to_remove);
    }
};

INSTANTIATE_TEST_SUITE_P(GeometryStateTest,
                         RemoveRoleTests,
                         ::testing::Values(Role::kProximity, Role::kIllustration, Role::kPerception));
...
TEST_F(RemoveRoleTests, RemoveRoleExceptions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(invalid_source_id, invalid_frame_id, Role::kUnassigned),
                                "Referenced geometry source \\d+ is not registered.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(invalid_source_id, invalid_geometry_id, Role::kUnassigned),
                                "Referenced geometry source \\d+ is not registered.");

    // Case: valid source id, but invalid frame/geometry id.
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(source_id_, invalid_frame_id, Role::kUnassigned),
                                "Referenced .* frame doesn't belong to the source.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(source_id_, invalid_geometry_id, Role::kUnassigned),

                                "Referenced geometry \\d+ has not been registered.");

    // Case: frame/geometry id belongs to a different source.
    const SourceId source_id_2 = geometry_state_.RegisterNewSource("second_source");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(source_id_2, frames_[0], Role::kUnassigned),
                                "Referenced .* frame doesn't belong to the source.");
    DRAKE_EXPECT_THROWS_MESSAGE(geometry_state_.RemoveRole(source_id_2, geometries_[0], Role::kUnassigned),
                                ".*the geometry doesn't belong to that source.");
}

// Special version of the class that does *not* default to having a renderer.
class GeometryStateNoRendererTest : public GeometryStateTestBase, public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(GeometryStateNoRendererTest, PerceptionRoleWithoutRenderer) {
    ...
    EXPECT_EQ(geometry_state_.RemoveRole(source_id_, geometries_[0], Role::kPerception), 1);
    EXPECT_FALSE(geometry.has_perception_role());
    EXPECT_EQ(geometry_state_.RemoveRole(source_id_, geometries_[0], Role::kPerception), 0);
}
...
protected:
    void SetUp() final {
        ...
        X_WP_ = RigidTransformd(AngleAxis<double>(M_PI_2, Vector3d::UnitX()), Vector3d{1, 2, 3});
        parent_id_ = geometry_state_.RegisterFrame(s_id, GeometryFrame("parent_frame"));

        // Equally arbitrary non-identity poses for the camera body in the parent
        // frame and camera sensor in the camera body frame.
        X_PC_ = RigidTransformd(AngleAxis<double>(M_PI_2, Vector3d::UnitY()), Vector3d{2, 3, 4});
        X_CS_ = RigidTransformd(AngleAxis<double>(M_PI_2, Vector3d::UnitZ()), Vector3d{3, 4, 5});
        ...
        geometry_state_.AddRenderer("engine2", std::make_unique<DummyRenderEngine>());
    ...
    render::RenderCameraCore camera_core(std::string_view engine_name) const {
        return render::RenderCameraCore(std::string(engine_name), {width(), height(), M_PI / 4}, {0.25, 10}, X_CS_);
    }
...
TEST_F(GeometryStateRenderTest, RenderColorImage) {
    ...
    //  apparent.
    geometry_state_.RenderColorImage(color_camera("engine1"), parent_id_, X_PC_, &image);
    EXPECT_TRUE(CompareMatrices(engine1_->last_updated_X_WC().GetAsMatrix4(), X_WS_.GetAsMatrix4(), 1e-15));
    ...
    EXPECT_NO_THROW(geometry_state_.RenderColorImage(color_camera("engine2"), parent_id_, X_PC_, &image));
    EXPECT_THROW(geometry_state_.RenderColorImage(color_camera("not_an_engine"), parent_id_, X_PC_, &image),
                 std::exception);
}
...
TEST_F(GeometryStateRenderTest, RenderDepthImage) {
    ...
    //  to confirm that the image is passed along, that would be screamingly
    //  apparent.
    geometry_state_.RenderDepthImage(depth_camera("engine1"), parent_id_, X_PC_, &image);
    EXPECT_TRUE(CompareMatrices(engine1_->last_updated_X_WC().GetAsMatrix4(), X_WS_.GetAsMatrix4(), 1e-15));
    ...
    // Passing another *valid* name doesn't throw (it finds the *other* engine).
    // But passing an invalid name throws.
    EXPECT_NO_THROW(geometry_state_.RenderDepthImage(depth_camera("engine2"), parent_id_, X_PC_, &image));
    EXPECT_THROW(geometry_state_.RenderDepthImage(depth_camera("not_an_engine"), parent_id_, X_PC_, &image),
                 std::exception);
}
...
TEST_F(GeometryStateRenderTest, RenderLabelImage) {
    ...
    //  to confirm that the image is passed along, that would be screamingly
    //  apparent.
    geometry_state_.RenderLabelImage(color_camera("engine1"), parent_id_, X_PC_, &image);
    EXPECT_TRUE(CompareMatrices(engine1_->last_updated_X_WC().GetAsMatrix4(), X_WS_.GetAsMatrix4(), 1e-15));
    EXPECT_EQ(engine1_->num_color_renders(), 0);
    EXPECT_EQ(engine1_->num_depth_renders(), 0);
    ...
    // Passing another *valid* name doesn't throw (it finds the *other* engine).
    // But passing an invalid name throws.
    EXPECT_NO_THROW(geometry_state_.RenderLabelImage(color_camera("engine2"), parent_id_, X_PC_, &image));
    EXPECT_THROW(geometry_state_.RenderLabelImage(color_camera("not_an_engine"), parent_id_, X_PC_, &image),
                 std::exception);
}
