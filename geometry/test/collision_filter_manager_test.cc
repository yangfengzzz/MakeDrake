...
array<GeometryId, 3> PopulateSceneGraph(SceneGraph<double>* scene_graph_ptr) {
    ...
    for (int i = 0; i < 3; ++i) {
        const FrameId f_id = scene_graph.RegisterFrame(source_id, GeometryFrame(fmt::format("frame_{}", i)));
        const GeometryId g_id = scene_graph.RegisterGeometry(source_id, f_id, make_sphere_instance());
    ...
    if (model_inspector.CollisionFiltered(ids[0], ids[1]) || model_inspector.CollisionFiltered(ids[0], ids[2]) ||
        model_inspector.CollisionFiltered(ids[1], ids[2])) {
        throw std::runtime_error("Initial conditions erroneously has filtered pairs");
    }
    return ids;
}
...
GTEST_TEST(CollisionFilterManagerTest, Apply) {
    ...
    const QueryObject<double>& query_object = scene_graph.get_query_output_port().Eval<QueryObject<double>>(*context);
    ...
    filter_manager.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({g_id1, g_id2})));
    ...
    const FilterId filter_id = filter_manager.ApplyTransient(
            CollisionFilterDeclaration().ExcludeBetween(GeometrySet({g_id1, g_id2}), GeometrySet({g_id3})));
    EXPECT_TRUE(inspector.CollisionFiltered(g_id1, g_id2));
...
GTEST_TEST(CollisionFilterManagerTest, CopySemantics) {
    ...
    copy_filter.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{g_id1, g_id2}));
    ...
    copy_filter.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet{g_id1, g_id3}));
    EXPECT_FALSE(model_inspector.CollisionFiltered(g_id1, g_id3));
...
GTEST_TEST(CollisionFilterManagerTest, MoveSemantics) {
    ...
    // be visible to the context inspector but not model inspector.
    CollisionFilterManager context_filter = scene_graph.collision_filter_manager(context.get());
