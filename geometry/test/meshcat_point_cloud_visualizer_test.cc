...
protected:
    ...
    void SetUpDiagram(bool connect_pose = true, double publish_period = 1 / 32.0) {
        systems::DiagramBuilder<double> builder;

        visualizer_ = builder.AddSystem<MeshcatPointCloudVisualizer>(meshcat_, "cloud", publish_period);
    ...
    cloud.mutable_xyzs().transpose() <<
      ...
      40, 50, 60;
        ...
        auto cloud_system =
                builder.template AddSystem<systems::ConstantValueSource>(Value<perception::PointCloud>(cloud));
        builder.Connect(cloud_system->get_output_port(), visualizer_->cloud_input_port());

        if (connect_pose) {
            auto pose_system = builder.template AddSystem<systems::ConstantValueSource>(Value<math::RigidTransformd>());
            builder.Connect(pose_system->get_output_port(), visualizer_->pose_input_port());
        }
