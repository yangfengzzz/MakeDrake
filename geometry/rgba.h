...
public:
    ...
    void set(double r, double g, double b, double a = 1.0) { set(Eigen::Vector4d{r, g, b, a}); }
    ...
    void update(std::optional<double> r = {},
        ...
        set(r.value_or(this->r()), g.value_or(this->g()), b.value_or(this->b()), a.value_or(this->a()));
    }
    ...
    bool AlmostEqual(const Rgba& other, double tolerance = 0.0) const {
        return std::abs(r() - other.r()) <= tolerance && std::abs(g() - other.g()) <= tolerance &&
               std::abs(b() - other.b()) <= tolerance && std::abs(a() - other.a()) <= tolerance;
    }
    ...
    Rgba scale_rgb(double scale) const {
        ...
        return {std::min(1.0, r() * scale), std::min(1.0, g() * scale), std::min(1.0, b() * scale), a()};
    }
