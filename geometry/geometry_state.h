...
#include "geometry/collision_filter_manager.h"
...
#include "geometry/mesh_deformation_interpolator.h"
#include "geometry/proximity_engine.h"
#include "geometry/render/render_camera.h"
#include "geometry/render/render_engine.h"
#include "geometry/scene_graph_config.h"
...
public:
    ...
    // @tparam_default_scalar
    template <typename T>
    void SetControlMeshPositions(const std::unordered_map<GeometryId, VectorX<T>>& q_WGs);

    const std::unordered_map<GeometryId, std::vector<DrivenTriangleMesh>>& driven_meshes() const {
        return driven_meshes_;
    }

    const std::vector<RenderMesh>& render_meshes(GeometryId id) const { return render_meshes_.at(id); }
    ...
    void SetMeshes(GeometryId id, std::vector<DrivenTriangleMesh> driven_meshes, std::vector<RenderMesh> render_meshes);
...
private:
    std::unordered_map<GeometryId, std::vector<DrivenTriangleMesh>> driven_meshes_;
...
public:
    ...
    template <class T1 = T>
    typename std::enable_if_t<!std::is_same_v<T1, double>, GeometryState<T>&> operator=(
            const GeometryState<double>& other) {
    ...
    int get_num_sources() const { return static_cast<int>(source_frame_id_map_.size()); }
    ...
    int get_num_geometries() const { return static_cast<int>(geometries_.size()); }
    ...
    */
    ...
    std::vector<PenetrationAsPointPair<T>> ComputePointPairPenetration() const {
        return geometry_engine_->ComputePointPairPenetration(kinematics_data_.X_WGs);
    }
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool, std::vector<ContactSurface<T>>> ComputeContactSurfaces(
        ...
        return geometry_engine_->ComputeContactSurfaces(representation, kinematics_data_.X_WGs);
    }
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool, void> ComputeContactSurfacesWithFallback(
        ...
        return geometry_engine_->ComputeContactSurfacesWithFallback(representation, kinematics_data_.X_WGs, surfaces,
                                                                    point_pairs);
    }

    /** Implementation of QueryObject::ComputeDeformableContact().  */
    template <typename T1 = T>
    typename std::enable_if_t<std::is_same_v<T1, double>, void> ComputeDeformableContact(
    ...
    CollisionFilterManager collision_filter_manager() {
        ...
        return CollisionFilterManager(&geometry_engine_->collision_filter(),
    ...
    std::vector<SignedDistancePair<T>> ComputeSignedDistancePairwiseClosestPoints(double max_distance) const {
        return geometry_engine_->ComputeSignedDistancePairwiseClosestPoints(kinematics_data_.X_WGs, max_distance);
    }
    ...
    std::vector<SignedDistanceToPoint<T>> ComputeSignedDistanceToPoint(const Vector3<T>& p_WQ, double threshold) const {
        return geometry_engine_->ComputeSignedDistanceToPoint(p_WQ, kinematics_data_.X_WGs, threshold);
    }
    ...
    bool HasRenderer(const std::string& name) const { return render_engines_.contains(name); }
...
private:
    ...
    void ValidateFrameIds(SourceId source_id, const KinematicsVector<FrameId, ValueType>& values) const;
    ...
    // the geometry "tree".
    void ValidateRegistrationAndSetTopology(SourceId source_id, FrameId frame_id, GeometryId geometry_id);

    // Method that updates the proximity engine and the render engines with the
    // up-to-date _pose_ data in `kinematics_data`.
    void FinalizePoseUpdate(const internal::KinematicsData<T>& kinematics_data,
    ...
    // `driven_mesh_data`.
    void FinalizeConfigurationUpdate(const internal::KinematicsData<T>& kinematics_data,
    ...
    bool is_dynamic(GeometryId id) const { return geometries_.at(id).is_dynamic(); }
    ...
    void ThrowIfNameExistsInRole(FrameId id, Role role, const std::string& name) const;

    // Propagates all roles defined in geometry instance to `this` geometry state.
    void AssignAllDefinedRoles(SourceId source_id, std::unique_ptr<GeometryInstance> geometry);
    ...
    void AddToProximityEngineUnchecked(const internal::InternalGeometry& geometry);

    // Handles removing the given geometry from the proximity engine. The only
    // GeometryState-level data structure modified is the proximity version. All
    // other changes to GeometryState data must happen elsewhere.
    void RemoveFromProximityEngineUnchecked(const internal::InternalGeometry& geometry);
    ...
    // @pre geometry_id maps to a registered geometry.
    bool RemoveFromRendererUnchecked(const std::string& renderer_name, GeometryId id);
    ...
    bool AddToCompatibleRenderersUnchecked(const internal::InternalGeometry& geometry);

    // Attempts to add the given `geometry` to all `candidate_renderers`
    // @pre `geometry` is not deformable.
    // @returns `true` if the geometry was added to *any* renderer.
    bool AddRigidToCompatibleRenderersUnchecked(const internal::InternalGeometry& geometry,
    ...
    // @returns `true` if the geometry was added to *any* renderer.
    bool AddDeformableToCompatibleRenderersUnchecked(const internal::InternalGeometry& geometry,
                                                     std::vector<render::RenderEngine*>* candidate_renderers);
    ...
    // requested.
    const internal::InternalFrame& ValidateAndGetFrame(SourceId source_id, FrameId frame_id) const;

    // Retrieves the requested renderer (if supported), throwing otherwise.
    const render::RenderEngine& GetRenderEngineOrThrow(const std::string& renderer_name) const;

    // Returns the world pose of the camera *sensor* based on camera properties,
    // parent frame, and pose in parent.
    math::RigidTransformd CalcCameraWorldPose(const render::RenderCameraCore& core,
                                              FrameId parent_frame,
    ...
    std::vector<render::RenderEngine*> GetMutableRenderEngines() const {
        ...
        for (auto& [name, render_engine] : render_engines_) {
            unused(name);
            results.emplace_back(const_cast<render::RenderEngine*>(render_engine.get()));
        }
        return results;
    }
    //@}
    ...
    std::unordered_map<SourceId, GeometryIdSet> source_deformable_geometry_id_map_;
    ...
    std::unordered_map<SourceId, std::unordered_set<GeometryId>> source_anchored_geometry_map_;
    ...
    std::unordered_map<std::string, copyable_unique_ptr<render::RenderEngine>> render_engines_;
