...
const GeometryProperties::Group& GeometryProperties::GetPropertiesInGroup(const string& group_name) const {
...
void GeometryProperties::AddPropertyAbstract(const string& group_name, const string& name, const AbstractValue& value) {
    WritePropertyAbstract(group_name, name, value, [&group_name, &name](const Group& group) {
...
void GeometryProperties::UpdatePropertyAbstract(const string& group_name,
    ...
    WritePropertyAbstract(group_name, name, value, [&group_name, &name, &value](const Group& group) {
        const auto value_iter = group.find(name);
        if (value_iter != group.end() && group.at(name)->type_info() != value.type_info()) {
            throw std::logic_error(
                    fmt::format("UpdateProperty(): Trying to update property ('{}', "
                                ...
                                group_name, name, value.GetNiceTypeName(), group.at(name)->GetNiceTypeName()));
        }
    });
}

bool GeometryProperties::HasProperty(const string& group_name, const string& name) const {
    return GetPropertyAbstractMaybe(group_name, name, false) != nullptr;
}

const AbstractValue& GeometryProperties::GetPropertyAbstract(const string& group_name, const string& name) const {
    const AbstractValue* abstract = GetPropertyAbstractMaybe(group_name, name, true);
    if (!abstract) {
        throw std::logic_error(fmt::format("GetProperty(): There is no property ('{}', '{}')", group_name, name));
    }
    return *abstract;
}

bool GeometryProperties::RemoveProperty(const string& group_name, const string& name) {
    const auto iter = values_.find(group_name);
...
void GeometryProperties::WritePropertyAbstract(const string& group_name,
                                               const string& name,
                                               const AbstractValue& value,
...
const AbstractValue* GeometryProperties::GetPropertyAbstractMaybe(const string& group_name,
                                                                  const string& name,
    ...
    if (iter == values_.end()) {
        if (throw_for_bad_group) {
            throw std::logic_error(
                    fmt::format("GetProperty(): Trying to read property ('{}', '{}'), but the group "
                                "does not exist.",
                                group_name, name));
        } else {
            return nullptr;
...
std::ostream& operator<<(std::ostream& out, const GeometryProperties& props) {
    ...
    for (const auto& group_pair : props.values_) {
        ...
        for (const auto& property_pair : group_properties) {
            ...
            out << "\n  " << property_name << ": " << property_pair.second->GetNiceTypeName();
