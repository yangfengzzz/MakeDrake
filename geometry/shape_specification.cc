...
#include "geometry/proximity/make_convex_hull_mesh_impl.h"
#include "geometry/proximity/meshing_utilities.h"
...
void ComputeConvexHullAsNecessary(std::shared_ptr<PolygonSurfaceMesh<double>>* hull_ptr,
                                  std::string_view filename,
                                  double scale) {
    std::shared_ptr<PolygonSurfaceMesh<double>> check = std::atomic_load(hull_ptr);
    if (check == nullptr) {
        ...
        auto new_hull = std::make_shared<PolygonSurfaceMesh<double>>(internal::MakeConvexHull(filename, scale));
...
Box::Box(double width, double depth, double height) : size_(width, depth, height) {
...
Box::Box(const Vector3<double>& measures) : Box(measures(0), measures(1), measures(2)) {}
...
std::string Box::do_to_string() const {
    return fmt::format("Box(width={}, depth={}, height={})", width(), depth(), height());
}

Capsule::Capsule(double radius, double length) : radius_(radius), length_(length) {
...
Capsule::Capsule(const Vector2<double>& measures) : Capsule(measures(0), measures(1)) {}
...
Convex::Convex(const std::string& filename, double scale)
    : filename_(std::filesystem::absolute(filename)), extension_(GetExtensionLower(filename_)), scale_(scale) {
...
Cylinder::Cylinder(double radius, double length) : radius_(radius), length_(length) {
    if (radius <= 0 || length <= 0) {
        throw std::logic_error(
...
Cylinder::Cylinder(const Vector2<double>& measures) : Cylinder(measures(0), measures(1)) {}
...
Ellipsoid::Ellipsoid(const Vector3<double>& measures) : Ellipsoid(measures(0), measures(1), measures(2)) {}
...
RigidTransform<double> HalfSpace::MakePose(const Vector3<double>& Hz_dir_F, const Vector3<double>& p_FB) {
    ...
    const auto R_FH = math::RotationMatrixd::MakeFromOrthonormalColumns(Hx_F, Hy_F, Hz_F);
...
MeshcatCone::MeshcatCone(double height, double a, double b) : height_(height), a_(a), b_(b) {
    if (height <= 0 || a <= 0 || b <= 0) {
        throw std::logic_error(
                fmt::format("MeshcatCone parameters height, a, and b should all be > 0 (they were "
                            "{}, {}, and {}, respectively).",
                            height, a, b));
    }
}

MeshcatCone::MeshcatCone(const Vector3<double>& measures) : MeshcatCone(measures(0), measures(1), measures(2)) {}
...
Sphere::Sphere(double radius) : radius_(radius) {
    if (radius < 0) {
        throw std::logic_error(fmt::format("Sphere radius should be >= 0 (was {}).", radius));
    }
}
...
void ShapeReifier::ThrowUnsupportedGeometry(const std::string& shape_name) {
    throw std::runtime_error(fmt::format("This class ({}) does not support {}.", NiceTypeName::Get(*this), shape_name));
}
...
double CalcMeshVolumeFromFile(const MeshType& mesh) {
    ...
    if (extension != ".obj") {
        throw std::runtime_error(
                fmt::format("CalcVolume currently only supports .obj files for mesh geometries; "
                            "but the volume of {} was requested.",
                            mesh.filename()));
    }
    TriangleSurfaceMesh<double> surface_mesh = ReadObjToTriangleSurfaceMesh(mesh.filename(), mesh.scale());
...
double CalcVolume(const Shape& shape) {
    return shape.Visit<double>(overloaded{[](const Box& box) {
...
#define DRAKE_DEFINE_SHAPE_SUBCLASS_BOILERPLATE(ShapeType)                        \
    ...
    void ShapeType::DoReify(ShapeReifier* shape_reifier, void* user_data) const { \
