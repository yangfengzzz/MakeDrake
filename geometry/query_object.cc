...
template <typename T>
const RigidTransform<T>& QueryObject<T>::GetPoseInWorld(FrameId frame_id) const {
    ThrowIfNotCallable();
...
template <typename T>
const RigidTransform<T>& QueryObject<T>::GetPoseInParent(FrameId frame_id) const {
    ThrowIfNotCallable();
...
template <typename T>
const RigidTransform<T>& QueryObject<T>::GetPoseInWorld(GeometryId geometry_id) const {
    ThrowIfNotCallable();
...
template <typename T>
const VectorX<T>& QueryObject<T>::GetConfigurationsInWorld(GeometryId geometry_id) const {
    ThrowIfNotCallable();
...
template <typename T>
std::vector<VectorX<T>> QueryObject<T>::GetDrivenMeshConfigurationsInWorld(GeometryId geometry_id, Role role) const {
    ThrowIfNotCallable();
    FullConfigurationUpdate();
    const GeometryState<T>& state = geometry_state();
...
template <typename T>
std::vector<PenetrationAsPointPair<T>> QueryObject<T>::ComputePointPairPenetration() const {
    ThrowIfNotCallable();
...
template <typename T>
std::vector<SortedPair<GeometryId>> QueryObject<T>::FindCollisionCandidates() const {
    ThrowIfNotCallable();
...
QueryObject<T>::ComputeContactSurfaces(HydroelasticContactRepresentation representation) const {
    ThrowIfNotCallable();
...
template <typename T>
template <typename T1>
typename std::enable_if_t<scalar_predicate<T1>::is_bool, void> QueryObject<T>::ComputeContactSurfacesWithFallback(
    ...
    FullPoseUpdate();
    const GeometryState<T>& state = geometry_state();
    state.ComputeContactSurfacesWithFallback(representation, surfaces, point_pairs);
}

template <typename T>
template <typename T1>
typename std::enable_if_t<std::is_same_v<T1, double>, void> QueryObject<T>::ComputeDeformableContact(
...
template <typename T>
std::vector<SignedDistancePair<T>> QueryObject<T>::ComputeSignedDistancePairwiseClosestPoints(
...
template <typename T>
SignedDistancePair<T> QueryObject<T>::ComputeSignedDistancePairClosestPoints(GeometryId geometry_id_A,
                                                                             GeometryId geometry_id_B) const {
    ThrowIfNotCallable();

    FullPoseUpdate();
    const GeometryState<T>& state = geometry_state();
    return state.ComputeSignedDistancePairClosestPoints(geometry_id_A, geometry_id_B);
}

template <typename T>
std::vector<SignedDistanceToPoint<T>> QueryObject<T>::ComputeSignedDistanceToPoint(const Vector3<T>& p_WQ,
...
template <typename T>
const render::RenderEngine* QueryObject<T>::GetRenderEngineByName(const std::string& name) const {
    ThrowIfNotCallable();
    FullPoseUpdate();
...
template void QueryObject<double>::ComputeDeformableContact<double>(internal::DeformableContact<double>*) const;

}  // namespace geometry
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::QueryObject);
