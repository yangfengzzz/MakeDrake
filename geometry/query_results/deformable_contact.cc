...
ContactParticipation::ContactParticipation(int num_vertices) : participation_(num_vertices, false) {}

void ContactParticipation::Participate(const std::unordered_set<int>& vertices) {
    for (int v : vertices) {
...
template <typename T>
DeformableContactSurface<T>::DeformableContactSurface(GeometryId id_A,
                                                      GeometryId id_B,
    ...
    DRAKE_DEMAND(num_contact_points == static_cast<int>(signed_distances_.size()));
    DRAKE_DEMAND(num_contact_points == static_cast<int>(barycentric_coordinates_A_.size()));
    DRAKE_DEMAND(num_contact_points == static_cast<int>(contact_vertex_indexes_A_.size()));
    DRAKE_DEMAND(contact_vertex_indexes_B_.has_value() == barycentric_coordinates_B_.has_value());
    if (contact_vertex_indexes_B_.has_value()) {
        DRAKE_DEMAND(num_contact_points == static_cast<int>(barycentric_coordinates_B_->size()));
        DRAKE_DEMAND(num_contact_points == static_cast<int>(contact_vertex_indexes_B_->size()));
    ...
    for (int i = 0; i < num_contact_points; ++i) {
        ...
        R_WCs_.emplace_back(math::RotationMatrix<T>::MakeFromOneUnitVector(-nhats_W_[i], kZAxis));
    }
}

template <typename T>
void DeformableContact<T>::AddDeformableRigidContactSurface(GeometryId deformable_id,
                                                            GeometryId rigid_id,
    ...
    DRAKE_DEMAND(static_cast<int>(signed_distances.size()) == contact_mesh_W.num_faces());
    DRAKE_DEMAND(static_cast<int>(contact_vertex_indexes.size()) == contact_mesh_W.num_faces());
    DRAKE_DEMAND(static_cast<int>(barycentric_coordinates.size()) == contact_mesh_W.num_faces());
    iter->second.Participate(participating_vertices);
    contact_surfaces_.emplace_back(deformable_id, rigid_id, std::move(contact_mesh_W), std::move(signed_distances),
                                   std::move(contact_vertex_indexes), std::move(barycentric_coordinates), std::nullopt,
                                   std::nullopt);
}

template <typename T>
void DeformableContact<T>::Participate(GeometryId id, const std::unordered_set<int>& vertices) {
...
template <typename T>
void DeformableContact<T>::AddDeformableDeformableContactSurface(GeometryId id0,
                                                                 GeometryId id1,
    ...
    contact_surfaces_.emplace_back(id0, id1, std::move(contact_mesh_W), std::move(signed_distances),
