...
template <typename T>
ContactSurface<T>::ContactSurface(GeometryId id_M,
                                  GeometryId id_N,
                                  MeshVariant mesh_W,
                                  FieldVariant e_MN,
    ...
    if (is_triangle()) {
        DRAKE_THROW_UNLESS(grad_eM_W_ == nullptr ||
                           static_cast<int>(grad_eM_W_->size()) == tri_mesh_W().num_elements());
        DRAKE_THROW_UNLESS(grad_eN_W_ == nullptr ||
                           static_cast<int>(grad_eN_W_->size()) == tri_mesh_W().num_elements());
    } else {
        DRAKE_THROW_UNLESS(grad_eM_W_ == nullptr ||
                           static_cast<int>(grad_eM_W_->size()) == poly_mesh_W().num_elements());
        DRAKE_THROW_UNLESS(grad_eN_W_ == nullptr ||
                           static_cast<int>(grad_eN_W_->size()) == poly_mesh_W().num_elements());
    }
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ContactSurface);
