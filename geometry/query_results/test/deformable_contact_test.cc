#include "geometry/query_results/deformable_contact.h"
...
GTEST_TEST(ContactParticipation, NoVertexIsParticipating) {
    ...
    multibody::contact_solvers::internal::PartialPermutation permutation = dut.CalcVertexPermutation();
...
GTEST_TEST(ContactParticipation, SomeVerticesAreParticipating) {
    ...
    const std::vector<int> kExpectedDofPermutation{0, 1, 2, 12, 13, 14, 3, 4, 5, 6, 7, 8, 15, 16, 17, 9, 10, 11};
    const std::vector<int> kExpectedDofPartialPermutation{0, 1, 2, -1, -1, -1, 3, 4, 5, 6, 7, 8, -1, -1, -1, 9, 10, 11};
    EXPECT_EQ(dut.CalcVertexPermutation().permutation(), kExpectedVertexPermutation);
    EXPECT_EQ(dut.CalcVertexPartialPermutation().permutation(), kExpectedVertexPartialPermutation);
    EXPECT_EQ(dut.CalcDofPermutation().permutation(), kExpectedDofPermutation);
    EXPECT_EQ(dut.CalcDofPartialPermutation().permutation(), kExpectedDofPartialPermutation);
}
...
GTEST_TEST(DeformableContactSurface, Getters) {
    PolygonSurfaceMesh<double> contact_mesh_W(std::vector<int>{3, 0, 1, 2},  // One polygon of three vertices
                                              std::vector<Vector3<double>>{
                                                      Vector3<double>::UnitX(),
                                                      Vector3<double>::UnitY(),
                                                      Vector3<double>::UnitZ(),
                                              });
    const std::vector<Vector3<double>> nhats_W = {Vector3<double>::Ones().normalized()};
    const std::vector<Vector3<double>> contact_points_W = {{1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0}};
    ...
    const std::vector<Vector4<double>> barycentric_centroids_A = {{0.1, 0.2, 0.3, 0.4}};
    const std::vector<Vector4<double>> barycentric_centroids_B = {{0.4, 0.3, 0.2, 0.1}};
    DeformableContactSurface<double> dut(kIdA, kIdB, contact_mesh_W, signed_distances, vertex_indexes_A,
...
GTEST_TEST(DeformableContact, AddDeformableRigidContactSurface) {
    ...
    const std::vector<Vector4<double>> barycentric_centroids = {{0.1, 0.2, 0.3, 0.4}};

    dut.AddDeformableRigidContactSurface(kIdA, kIdB, {0, 3, 2, 5}, contact_mesh_W, signed_distances,
                                         contact_vertex_indexes, barycentric_centroids);
    /* Verify that the contact surface is as expected. */
    const std::vector<DeformableContactSurface<double>>& surfaces = dut.contact_surfaces();
...
GTEST_TEST(DeformableContact, AddDeformableDeformableContactSurface) {
    ...
    const std::vector<Vector4<double>> barycentric_centroids_A = {{0.1, 0.2, 0.3, 0.4}};
    const std::vector<Vector4<double>> barycentric_centroids_B = {{0.11, 0.19, 0.29, 0.41}};

    dut.AddDeformableDeformableContactSurface(kIdA, kIdB, {0, 3, 2, 5}, {1, 4, 3, 0}, contact_mesh_W, signed_distances,
    ...
    // Verify that contact participation is as expected.
    EXPECT_EQ(dut.contact_participation(kIdA).num_vertices_in_contact(), 4);
    EXPECT_EQ(dut.contact_participation(kIdA).CalcVertexPermutation().permutation(),
    ...
    EXPECT_EQ(dut.contact_participation(kIdB).CalcVertexPermutation().permutation(),
