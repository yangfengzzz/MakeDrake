...
#include "geometry/proximity/sorted_triplet.h"
...
#include "systems/lcm/lcm_system_graphviz.h"
...
lcmt_viewer_geometry_data MakeFacetedMeshDataForLcm(const TriangleSurfaceMesh<double>& surface_mesh,
    ...
    EigenMapView(geometry_data.position) = X_PG.translation().template cast<float>();
    ...
    for (int f = 0; f < surface_mesh.num_triangles(); ++f) {
        ...
        for (int fv = 0; fv < 3; ++fv) {
            ...
            const Vector3<float> p_MV = surface_mesh.vertex(v_i).template cast<float>();
...
lcmt_viewer_geometry_data MakeHydroMesh(GeometryId geometry_id,
    ...
    const TriangleSurfaceMesh<double>& surface_mesh =
            std::holds_alternative<const TriangleSurfaceMesh<double>*>(hydro_mesh)
                    ...
                    : ConvertVolumeToSurfaceMesh(*std::get<const VolumeMesh<double>*>(hydro_mesh));
...
template <typename T>
lcmt_viewer_geometry_data MakeDeformableSurfaceMesh(const std::string& name,
                                                    const VectorX<T>& vertex_positions,
                                                    const internal::RenderMesh& render_mesh,
    ...
    // color.
    const Rgba& diffuse_color = render_mesh.material.has_value() ? render_mesh.material->diffuse : default_diffuse;
    ...
    //   M: 3T, the number of vertex indices for the T triangles.
    geometry_data.type = geometry_data.MESH;
    const Eigen::Matrix<unsigned int, Eigen::Dynamic, 3, Eigen::RowMajor>& triangles = render_mesh.indices;
...
private:
    ...
    lcmt_viewer_geometry_data MakeMeshDataForConvexHull(const PolygonSurfaceMesh<double>& hull) {
        // Convert polygonal surface mesh to triangle surface mesh.
        const TriangleSurfaceMesh<double> tri_mesh(internal::MakeTriangleFromPolygonMesh(hull));
...
template <typename T>
DrakeVisualizer<T>::DrakeVisualizer(lcm::DrakeLcmInterface* lcm, DrakeVisualizerParams params)
    : DrakeVisualizer(lcm, std::move(params), true) {}

template <typename T>
template <typename U>
DrakeVisualizer<T>::DrakeVisualizer(const DrakeVisualizer<U>& other) : DrakeVisualizer(nullptr, other.params_, false) {
    DRAKE_DEMAND(owned_lcm_ == nullptr);
    DRAKE_DEMAND(lcm_ == nullptr);
    const lcm::DrakeLcm* owned_lcm = dynamic_cast<const lcm::DrakeLcm*>(other.owned_lcm_.get());
...
template <typename T>
const DrakeVisualizer<T>& DrakeVisualizer<T>::AddToBuilder(systems::DiagramBuilder<T>* builder,
                                                           const SceneGraph<T>& scene_graph,
                                                           lcm::DrakeLcmInterface* lcm,
                                                           DrakeVisualizerParams params) {
    return AddToBuilder(builder, scene_graph.get_query_output_port(), lcm, std::move(params));
}
...
const DrakeVisualizer<T>& DrakeVisualizer<T>::AddToBuilder(systems::DiagramBuilder<T>* builder,
                                                           ...
                                                           lcm::DrakeLcmInterface* lcm,
                                                           DrakeVisualizerParams params) {
    const std::string aspirational_name = fmt::format("drake_visualizer({})", params.role);
    auto& visualizer = *builder->template AddSystem<DrakeVisualizer<T>>(lcm, std::move(params));
...
void DrakeVisualizer<T>::DispatchLoadMessage(const SceneGraph<T>& scene_graph,
    ...
    PopulateDynamicFrameData(scene_graph.model_inspector(), params, &dynamic_frames);
    SendLoadNonDeformableMessage(scene_graph.model_inspector(), params, dynamic_frames, 0, lcm);
}

template <typename T>
DrakeVisualizer<T>::DrakeVisualizer(lcm::DrakeLcmInterface* lcm, DrakeVisualizerParams params, bool use_lcm)
    ...
    if (params_.publish_period <= 0) {
        throw std::runtime_error(fmt::format("DrakeVisualizer requires a positive publish period; {} was given",
    ...
    this->DeclarePeriodicPublishEvent(params_.publish_period, 0.0, &DrakeVisualizer<T>::SendGeometryMessage);
    this->DeclareForcedPublishEvent(&DrakeVisualizer<T>::SendGeometryMessage);

    query_object_input_port_ = this->DeclareAbstractInputPort("query_object", Value<QueryObject<T>>()).get_index();

    // These cache entries depend on *nothing*.
    frame_data_cache_index_ = this->DeclareCacheEntry("dynamic_frames", &DrakeVisualizer<T>::CalcDynamicFrameData,
                                                      {this->nothing_ticket()})
                                      .cache_index();
}

template <typename T>
EventStatus DrakeVisualizer<T>::SendGeometryMessage(const Context<T>& context) const {
    const auto& query_object = query_object_input_port().template Eval<QueryObject<T>>(context);
    const GeometryVersion& current_version = query_object.inspector().geometry_version();
    ...
    if (send_load_message) {
        SendLoadNonDeformableMessage(query_object.inspector(), params_, RefreshDynamicFrameData(context),
                                     ExtractDoubleOrThrow(context.get_time()), lcm_);
    }

    SendDrawNonDeformableMessage(query_object, params_, EvalDynamicFrameData(context),
                                 ExtractDoubleOrThrow(context.get_time()), lcm_);
    SendDeformableGeometriesMessage(query_object, params_, ExtractDoubleOrThrow(context.get_time()), lcm_);

    return EventStatus::Succeeded();
}

template <typename T>
void DrakeVisualizer<T>::SendLoadNonDeformableMessage(const SceneGraphInspector<T>& inspector,
                                                      const DrakeVisualizerParams& params,
    ...
    const int anchored_count = inspector.NumGeometriesForFrameWithRole(inspector.world_frame_id(), params.role) -
                               inspector.NumDeformableGeometriesWithRole(params.role);
    const int frame_count = static_cast<int>(dynamic_frames.size()) + (anchored_count > 0 ? 1 : 0);
    ...
    auto make_geometry = [&params, &inspector](GeometryId g_id) {
        const GeometryProperties* props = inspector.GetProperties(g_id, params.role);
        ...
        if (params.role != Role::kIllustration) {
            const GeometryProperties* illus_props = inspector.GetIllustrationProperties(g_id);
            if (illus_props) {
                default_color = illus_props->GetPropertyOrDefault("phong", "diffuse", default_color);
            }
        }
        const Rgba& color = props->GetPropertyOrDefault("phong", "diffuse", default_color);
        if (params.role == Role::kProximity && params.show_hydroelastic) {
            ...
            if (!std::holds_alternative<std::monostate>(hydro_mesh)) {
                ...
                return MakeHydroMesh(g_id, inspector, inspector.GetPoseInFrame(g_id), color);
            }
        }
        return ShapeToLcm().Convert(shape, inspector.GetPoseInFrame(g_id), params, color);
    };
    ...
    if (anchored_count) {
        ...
        for (const GeometryId& g_id : inspector.GetGeometries(inspector.world_frame_id(), params.role)) {
    ...
    for (const auto& [frame_id, geometry_count, name] : dynamic_frames) {
        ...
        int geom_index = -1;  // We'll pre-increment before using.
        for (const GeometryId& g_id : inspector.GetGeometries(frame_id, params.role)) {
            message.link[link_index].geom[++geom_index] = make_geometry(g_id);
        }
        ++link_index;
    }

    std::string channel = MakeLcmChannelNameForRole("DRAKE_VIEWER_LOAD_ROBOT", params);
    lcm::Publish(lcm, channel, message, time);
}

template <typename T>
void DrakeVisualizer<T>::SendDrawNonDeformableMessage(const QueryObject<T>& query_object,
                                                      const DrakeVisualizerParams& params,
                                                      const vector<internal::DynamicFrameData>& dynamic_frames,
                                                      double time,
                                                      lcm::DrakeLcmInterface* lcm) {
    ...
    for (int i = 0; i < frame_count; ++i) {
        ...
        const math::RigidTransformd& X_WF = internal::convert_to_double(query_object.GetPoseInWorld(frame_id));
...
template <typename T>
void DrakeVisualizer<T>::SendDeformableGeometriesMessage(const QueryObject<T>& query_object,
                                                         const DrakeVisualizerParams& params,
                                                         double time,
                                                         lcm::DrakeLcmInterface* lcm) {
    ...
    const SceneGraphInspector<T>& inspector = query_object.inspector();
    const std::vector<GeometryId> deformable_geometries = inspector.GetAllDeformableGeometryIds();
    for (const auto& g_id : deformable_geometries) {
        ...
        const std::vector<internal::RenderMesh>& render_meshes = inspector.GetDrivenRenderMeshes(g_id, params.role);
        ...
        for (int j = 0; j < ssize(vertex_positions); ++j) {
            message.geom.emplace_back(MakeDeformableSurfaceMesh(inspector.GetName(g_id), vertex_positions[j],
                                                                render_meshes[j], params.default_color));
        }
    }
...
template <typename T>
void DrakeVisualizer<T>::CalcDynamicFrameData(const Context<T>& context,
...
template <typename T>
const vector<internal::DynamicFrameData>& DrakeVisualizer<T>::RefreshDynamicFrameData(const Context<T>& context) const {
    // We'll need to make sure our knowledge of dynamic frames can get updated.
    this->get_cache_entry(frame_data_cache_index_).get_mutable_cache_entry_value(context).mark_out_of_date();

    return EvalDynamicFrameData(context);
}

template <typename T>
const vector<internal::DynamicFrameData>& DrakeVisualizer<T>::EvalDynamicFrameData(const Context<T>& context) const {
    return this->get_cache_entry(frame_data_cache_index_).template Eval<vector<internal::DynamicFrameData>>(context);
}

template <typename T>
void DrakeVisualizer<T>::PopulateDynamicFrameData(const SceneGraphInspector<T>& inspector,
                                                  const DrakeVisualizerParams& params,
    ...
    for (const FrameId& frame_id : inspector.GetAllFrameIds()) {
        ...
        const int count = inspector.NumGeometriesForFrameWithRole(frame_id, params.role);
        if (count > 0) {
            dynamic_frames.push_back(
...
template <typename T>
typename LeafSystem<T>::GraphvizFragment DrakeVisualizer<T>::DoGetGraphvizFragment(
        const typename LeafSystem<T>::GraphvizFragmentParams& params) const {
    LcmSystemGraphviz lcm_system_graphviz(*lcm_, MakeLcmChannelNameForRole("DRAKE_VIEWER_DRAW", params_),
    ...
    return lcm_system_graphviz.DecorateResult(
            LeafSystem<T>::DoGetGraphvizFragment(lcm_system_graphviz.DecorateParams(params)));
}

}  // namespace geometry
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ::drake::geometry::DrakeVisualizer);
