...
public:
    ...
    /* Returns a list of ids of the frames that have *this* frame as a parent
     frame.  */
    const std::unordered_set<FrameId>& child_frames() const { return child_frames_; }
    ...
    const std::unordered_set<GeometryId>& child_geometries() const { return child_geometries_; }
    ...
    int num_child_geometries() const { return static_cast<int>(child_geometries_.size()); }

    /* Returns true if this frame considers the given `frame_id` to be a child.
     */
    bool has_child(FrameId frame_id) const { return child_frames_.contains(frame_id); }
    ...
    /* Returns true if this frame considers the given `geometry_id` to be rigidly
     affixed to it.  */
    bool has_child(GeometryId geometry_id) const { return child_geometries_.contains(geometry_id); }

    /* Adds the given `geometry_id` to the set of geometries that this frame
     considers to be children.  */
    void add_child(GeometryId geometry_id) { child_geometries_.insert(geometry_id); }
