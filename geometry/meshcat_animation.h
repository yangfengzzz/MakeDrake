...
public:
    ...
    */
    int frame(double time) const {
        ...
        return static_cast<int>(std::floor((time - start_time_) * frames_per_second_));
    }
...
private:
    ...
    struct TypedTrack {
        std::variant<std::monostate, Track<bool>, Track<double>, Track<std::vector<double>>> track;
        std::string js_type;
    };
    ...
    const TypedTrack* GetTypedTrack(std::string_view path, std::string_view property) const;
    TypedTrack& GetOrCreateTypedTrack(std::string_view path, std::string_view property);
