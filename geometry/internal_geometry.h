...
public:
     ...
     */
    bool operator==(const InternalGeometry& other) const { return id_ == other.id_; }

    /* Compares two %InternalGeometry instances for inequality. See operator==()
     for the definition of equality.  */
    bool operator!=(const InternalGeometry& other) const { return !(*this == other); }
    ...
    /* Returns true if the geometry is affixed to the frame with the given
     `frame_id`.  */
    bool is_child_of_frame(FrameId frame_id) const { return frame_id == frame_id_; }
    ...
    void SetRole(ProximityProperties properties) { proximity_props_ = std::move(properties); }

    /* Assigns an illustration role to this geometry, replacing any illustration
     properties that were previously assigned.  */
    void SetRole(IllustrationProperties properties) { illustration_props_ = std::move(properties); }
    ...
    bool has_illustration_role() const { return illustration_props_ != std::nullopt; }
    ...
    const VolumeMesh<double>* reference_mesh() const { return reference_mesh_.get(); }
