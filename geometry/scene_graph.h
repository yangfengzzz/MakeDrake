...
public:
    ...
    static FrameId world_frame_id() { return internal::InternalFrame::world_frame_id(); }
...
private:
    ...
    // parameter.
    void SetDefaultParameters(const systems::Context<T>& context, systems::Parameters<T>* parameters) const override;
    ...
    // perform queries.
    void CalcQueryObject(const systems::Context<T>& context, QueryObject<T>* output) const;

    // Collects all of the *dynamic* frames that have geometries with the given
    // role.
    std::vector<FrameId> GetDynamicFrames(const GeometryState<T>& g_state, Role role) const;
    ...
    void FullConfigurationUpdate(const systems::Context<T>& context) const {
        this->get_cache_entry(configuration_update_index_).template Eval<int>(context);
    }
    ...
    // given context.
    const GeometryState<T>& geometry_state(const systems::Context<T>& context) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::geometry::SceneGraph);
