...
// nice message.
void ThrowUnlessAbsentOr(std::string_view name, std::optional<double> property, Condition condition) {
    ...
    if (should_throw) {
        throw std::logic_error(fmt::format("Invalid scene graph configuration: '{}' ({}) must be a {} value.", name,
...
#undef DRAKE_ENFORCE
    ...
    if (static_friction.has_value() != dynamic_friction.has_value()) {
        ...
        throw std::logic_error(
                fmt::format("Invalid scene graph configuration: either both 'static_friction' ({})"
