...
BodyShapeDescription MakeBodyShapeDescription(const multibody::MultibodyPlant<double>& plant,
    ...
    const auto& query_object =
            plant.get_geometry_query_input_port().template Eval<geometry::QueryObject<double>>(plant_context);
    ...
    const std::string& model_instance_name = plant.GetModelInstanceName(body->model_instance());
    return BodyShapeDescription(inspector.GetShape(geometry_id), X_BS, model_instance_name, body->name());
}
