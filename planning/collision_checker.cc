...
#include "planning/linear_distance_and_interpolation_provider.h"
...
void SanityCheckConfigurationDistanceFunction(const ConfigurationDistanceFunction& distance_function,
                                              const Eigen::VectorXd& default_configuration) {
    const double test_distance = distance_function(default_configuration, default_configuration);
    DRAKE_THROW_UNLESS(test_distance == 0.0);
}

void SanityCheckConfigurationInterpolationFunction(const ConfigurationInterpolationFunction& interpolation_function,
                                                   const Eigen::VectorXd& default_configuration) {
    const Eigen::VectorXd test_interpolated_q =
            interpolation_function(default_configuration, default_configuration, 0.0);
    DRAKE_THROW_UNLESS(test_interpolated_q.size() == default_configuration.size());
    for (int index = 0; index < test_interpolated_q.size(); ++index) {
        DRAKE_THROW_UNLESS(test_interpolated_q(index) == default_configuration(index));
    }
}

class LegacyDistanceAndInterpolationProvider final : public DistanceAndInterpolationProvider {
public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(LegacyDistanceAndInterpolationProvider);

    LegacyDistanceAndInterpolationProvider(const ConfigurationDistanceFunction& distance_function,
                                           const ConfigurationInterpolationFunction& interpolation_function)
        : distance_function_(distance_function), interpolation_function_(interpolation_function) {
        DRAKE_THROW_UNLESS(distance_function_ != nullptr);
        DRAKE_THROW_UNLESS(interpolation_function_ != nullptr);
    }

    std::shared_ptr<const LegacyDistanceAndInterpolationProvider> WithConfigurationDistanceFunction(
            const ConfigurationDistanceFunction& distance_function) const {
        return std::make_shared<LegacyDistanceAndInterpolationProvider>(distance_function, interpolation_function_);
    }

    std::shared_ptr<const LegacyDistanceAndInterpolationProvider> WithConfigurationInterpolationFunction(
            const ConfigurationInterpolationFunction& interpolation_function) const {
        return std::make_shared<LegacyDistanceAndInterpolationProvider>(distance_function_, interpolation_function);
    }

private:
    double DoComputeConfigurationDistance(const Eigen::VectorXd& from, const Eigen::VectorXd& to) const final {
        return distance_function_(from, to);
    }

    Eigen::VectorXd DoInterpolateBetweenConfigurations(const Eigen::VectorXd& from,
...
// edge checking group.
ConfigurationInterpolationFunction MakeDefaultConfigurationInterpolationFunction(
    ...
    return [quaternion_dof_start_indices](const Eigen::VectorXd& q_1, const Eigen::VectorXd& q_2, double ratio) {
        ...
        Eigen::VectorXd interpolated_q = common_robotics_utilities::math::InterpolateXd(q_1, q_2, ratio);
        ...
        for (const int quat_dof_start_index : quaternion_dof_start_indices) {
            ...
            interpolated_q.segment<4>(quat_dof_start_index) = interpolated_quat.coeffs();
        }
        return interpolated_q;
    };
}

std::vector<int> GetQuaternionDofStartIndices(const MultibodyPlant<double>& plant) {
    return LinearDistanceAndInterpolationProvider(plant).quaternion_dof_start_indices();
}
...
std::vector<int> CalcUncontrolledDofsThatKinematicallyAffectTheRobot(const MultibodyPlant<double>& plant,
    ...
    for (JointIndex joint_i : plant.GetJointIndices()) {
        ...
        std::vector<BodyIndex> outboard_bodies = plant.GetBodiesKinematicallyAffectedBy({joint_i});
...
const CollisionCheckerContext& CollisionChecker::model_context(const std::optional<int> context_number) const {
    const int context_index = context_number.has_value() ? *context_number : GetContextOmpThreadNum();
    return owned_contexts_.get_model_context(context_index);
}

std::shared_ptr<CollisionCheckerContext> CollisionChecker::MakeStandaloneModelContext() const {
    // Make a shared clone of the special "prototype" context.
    std::shared_ptr<CollisionCheckerContext> standalone_context(owned_contexts_.prototype_context().Clone());
...
void CollisionChecker::PerformOperationAgainstAllModelContexts(
        const std::function<void(const RobotDiagram<double>&, CollisionCheckerContext*)>& operation) {
...
bool CollisionChecker::AddCollisionShape(const std::string& group_name, const BodyShapeDescription& description) {
    const RigidBody<double>& body = plant().GetBodyByName(
            description.body_name(), plant().GetModelInstanceByName(description.model_instance_name()));
    return AddCollisionShapeToBody(group_name, body, description.shape(), description.pose_in_body());
}

int CollisionChecker::AddCollisionShapes(const std::string& group_name,
...
std::map<std::string, int> CollisionChecker::AddCollisionShapes(
        const std::map<std::string, std::vector<BodyShapeDescription>>& geometry_groups) {
...
bool CollisionChecker::AddCollisionShapeToFrame(const std::string& group_name,
...
bool CollisionChecker::AddCollisionShapeToBody(const std::string& group_name,
                                               const RigidBody<double>& bodyA,
                                               const Shape& shape,
                                               const RigidTransform<double>& X_AG) {
    const std::optional<GeometryId> maybe_geometry = DoAddCollisionShapeToBody(group_name, bodyA, shape, X_AG);
    if (maybe_geometry.has_value()) {
        const std::string& model_instance_name = plant().GetModelInstanceName(bodyA.model_instance());
        geometry_groups_[group_name].push_back(AddedShape{
                *maybe_geometry, bodyA.index(), BodyShapeDescription(shape, X_AG, model_instance_name, bodyA.name())});
    }
    return maybe_geometry.has_value();
}

std::map<std::string, std::vector<BodyShapeDescription>> CollisionChecker::GetAllAddedCollisionShapes() const {
...
void CollisionChecker::RemoveAllAddedCollisionShapes(const std::string& group_name) {
...
std::optional<double> CollisionChecker::MaybeGetUniformRobotEnvironmentPadding() const {
    ...
    for (BodyIndex body_index(0); body_index < plant().num_bodies(); ++body_index) {
        for (BodyIndex other_body_index(body_index + 1); other_body_index < plant().num_bodies(); ++other_body_index) {
            if (IsPartOfRobot(get_body(body_index)) != IsPartOfRobot(get_body(other_body_index))) {
                const double this_padding = GetPaddingBetween(body_index, other_body_index);
...
std::optional<double> CollisionChecker::MaybeGetUniformRobotRobotPadding() const {
    std::optional<double> check_padding;
    for (BodyIndex body_index(0); body_index < plant().num_bodies(); ++body_index) {
        for (BodyIndex other_body_index(body_index + 1); other_body_index < plant().num_bodies(); ++other_body_index) {
            if (IsPartOfRobot(get_body(body_index)) && IsPartOfRobot(get_body(other_body_index))) {
...
void CollisionChecker::SetPaddingBetween(BodyIndex bodyA_index, BodyIndex bodyB_index, double padding) {
    DRAKE_THROW_UNLESS(bodyA_index >= 0 && bodyA_index < collision_padding_.rows());
    DRAKE_THROW_UNLESS(bodyB_index >= 0 && bodyB_index < collision_padding_.rows());
    DRAKE_THROW_UNLESS(bodyA_index != bodyB_index);
    DRAKE_THROW_UNLESS(std::isfinite(padding));
    DRAKE_THROW_UNLESS(IsPartOfRobot(get_body(bodyA_index)) || IsPartOfRobot(get_body(bodyB_index)));
...
void CollisionChecker::SetPaddingMatrix(const Eigen::MatrixXd& requested_padding) {
    ...
    if (requested_padding.rows() != collision_padding_.rows() ||
        ...
        throw std::logic_error(
                fmt::format("CollisionChecker::SetPaddingMatrix(): The padding"
                            ...
                            " wrong size: {}x{}.",
                            collision_padding_.rows(), collision_padding_.cols(), requested_padding.rows(),
                            requested_padding.cols()));
    }
...
void CollisionChecker::SetPaddingOneRobotBodyAllEnvironmentPairs(const BodyIndex body_index, const double padding) {
    DRAKE_THROW_UNLESS(std::isfinite(padding));
    DRAKE_THROW_UNLESS(IsPartOfRobot(get_body(body_index)));
    for (BodyIndex other_body_index(0); other_body_index < plant().num_bodies(); ++other_body_index) {
...
void CollisionChecker::SetPaddingAllRobotEnvironmentPairs(const double padding) {
    DRAKE_THROW_UNLESS(std::isfinite(padding));
    for (BodyIndex body_index(0); body_index < plant().num_bodies(); ++body_index) {
        for (BodyIndex other_body_index(body_index + 1); other_body_index < plant().num_bodies(); ++other_body_index) {
            if (IsPartOfRobot(get_body(body_index)) != IsPartOfRobot(get_body(other_body_index))) {
...
void CollisionChecker::SetCollisionFilterMatrix(const Eigen::MatrixXi& filter_matrix) {
    // First confirm it is of appropriate *size*.
    if (filter_matrix.rows() != filtered_collisions_.rows() || filter_matrix.cols() != filtered_collisions_.cols()) {
        throw std::logic_error(fmt::format(
                "CollisionChecker::SetCollisionFilterMatrix(): The filter "
                "matrix must be {}x{};. The given matrix is the wrong "
                "size: {}x{}.",
                filtered_collisions_.rows(), filtered_collisions_.cols(), filter_matrix.rows(), filter_matrix.cols()));
    }
...
bool CollisionChecker::IsCollisionFilteredBetween(BodyIndex bodyA_index, BodyIndex bodyB_index) const {
    DRAKE_THROW_UNLESS(bodyA_index >= 0 && bodyA_index < filtered_collisions_.rows());
    DRAKE_THROW_UNLESS(bodyB_index >= 0 && bodyB_index < filtered_collisions_.rows());
...
void CollisionChecker::SetCollisionFilteredBetween(BodyIndex bodyA_index,
    ...
    const int current_value = filtered_collisions_(int{bodyA_index}, int{bodyB_index});
...
void CollisionChecker::SetCollisionFilteredWithAllBodies(BodyIndex body_index) {
    DRAKE_THROW_UNLESS(body_index >= 0 && body_index < filtered_collisions_.rows());
...
bool CollisionChecker::CheckConfigCollisionFree(const Eigen::VectorXd& q,
                                                const std::optional<int> context_number) const {
    return CheckContextConfigCollisionFree(&mutable_model_context(context_number), q);
}

bool CollisionChecker::CheckContextConfigCollisionFree(CollisionCheckerContext* model_context,
...
std::vector<uint8_t> CollisionChecker::CheckConfigsCollisionFree(const std::vector<Eigen::VectorXd>& configs,
    ...
    drake::log()->debug("CheckConfigsCollisionFree uses {} thread(s)", number_of_threads);

    const auto config_work = [&](const int thread_num, const int64_t index) {
        collision_checks.at(index) = CheckConfigCollisionFree(configs.at(index), thread_num);
    };

    StaticParallelForIndexLoop(DegreeOfParallelism(number_of_threads), 0, configs.size(), config_work,
...
void CollisionChecker::SetDistanceAndInterpolationProvider(
    ...
    const double test_distance = provider->ComputeConfigurationDistance(default_q, default_q);
    DRAKE_THROW_UNLESS(test_distance == 0.0);

    const Eigen::VectorXd test_interpolated_q = provider->InterpolateBetweenConfigurations(default_q, default_q, 0.0);
...
void CollisionChecker::SetConfigurationDistanceFunction(const ConfigurationDistanceFunction& distance_function) {
    auto legacy = std::dynamic_pointer_cast<const LegacyDistanceAndInterpolationProvider>(
    ...
    SanityCheckConfigurationDistanceFunction(distance_function, GetDefaultConfiguration());
    distance_and_interpolation_provider_ = legacy->WithConfigurationDistanceFunction(distance_function);
}

ConfigurationDistanceFunction CollisionChecker::MakeStandaloneConfigurationDistanceFunction() const {
...
void CollisionChecker::SetConfigurationInterpolationFunction(
    ...
    if (interpolation_function == nullptr) {
        SetConfigurationInterpolationFunction(
                MakeDefaultConfigurationInterpolationFunction(GetQuaternionDofStartIndices(plant())));
        return;
    }
    SanityCheckConfigurationInterpolationFunction(interpolation_function, GetDefaultConfiguration());
    distance_and_interpolation_provider_ = legacy->WithConfigurationInterpolationFunction(interpolation_function);
}

ConfigurationInterpolationFunction CollisionChecker::MakeStandaloneConfigurationInterpolationFunction() const {
...
bool CollisionChecker::CheckEdgeCollisionFree(const Eigen::VectorXd& q1,
                                              ...
                                              const std::optional<int> context_number) const {
    return CheckContextEdgeCollisionFree(&mutable_model_context(context_number), q1, q2);
}

bool CollisionChecker::CheckContextEdgeCollisionFree(CollisionCheckerContext* model_context,
    ...
    const int num_steps = static_cast<int>(std::max(1.0, std::ceil(distance / edge_step_size())));
    for (int step = 0; step < num_steps; ++step) {
        const double ratio = static_cast<double>(step) / static_cast<double>(num_steps);
        const Eigen::VectorXd qinterp = InterpolateBetweenConfigurations(q1, q2, ratio);
...
bool CollisionChecker::CheckEdgeCollisionFreeParallel(const Eigen::VectorXd& q1,
                                                      const Eigen::VectorXd& q2,
                                                      const Parallelism parallelize) const {
    const int number_of_threads = GetNumberOfThreads(parallelize);
    drake::log()->debug("CheckEdgeCollisionFreeParallel uses {} thread(s)", number_of_threads);
    ...
    if (number_of_threads > 1) {
        ...
        StaticParallelForIndexLoop(DegreeOfParallelism(number_of_threads), 0, num_steps, step_work,
                                   ParallelForBackend::BEST_AVAILABLE);
...
std::vector<uint8_t> CollisionChecker::CheckEdgesCollisionFree(
        const std::vector<std::pair<Eigen::VectorXd, Eigen::VectorXd>>& edges, const Parallelism parallelize) const {
    // Note: vector<uint8_t> is used since vector<bool> is not thread safe.
    std::vector<uint8_t> collision_checks(edges.size(), 0);

    const int number_of_threads = GetNumberOfThreads(parallelize);
    drake::log()->debug("CheckEdgesCollisionFree uses {} thread(s)", number_of_threads);
    ...
    const auto edge_work = [&](const int thread_num, const int64_t index) {
        ...
        collision_checks.at(index) = CheckEdgeCollisionFree(edge.first, edge.second, thread_num);
    };

    StaticParallelForIndexLoop(DegreeOfParallelism(number_of_threads), 0, edges.size(), edge_work,
                               ParallelForBackend::BEST_AVAILABLE);

    return collision_checks;
}

EdgeMeasure CollisionChecker::MeasureEdgeCollisionFree(const Eigen::VectorXd& q1,
                                                       const Eigen::VectorXd& q2,
                                                       const std::optional<int> context_number) const {
    return MeasureContextEdgeCollisionFree(&mutable_model_context(context_number), q1, q2);
}

EdgeMeasure CollisionChecker::MeasureContextEdgeCollisionFree(CollisionCheckerContext* model_context,
                                                              const Eigen::VectorXd& q1,
                                                              const Eigen::VectorXd& q2) const {
    DRAKE_THROW_UNLESS(model_context != nullptr);
    const double distance = ComputeConfigurationDistance(q1, q2);
...
EdgeMeasure CollisionChecker::MeasureEdgeCollisionFreeParallel(const Eigen::VectorXd& q1,
                                                               const Eigen::VectorXd& q2,
                                                               const Parallelism parallelize) const {
    const int number_of_threads = GetNumberOfThreads(parallelize);
    drake::log()->debug("MeasureEdgeCollisionFreeParallel uses {} thread(s)", number_of_threads);
    ...
    if (number_of_threads > 1) {
        ...
        const auto step_work = [&](const int thread_num, const int64_t step) {
            ...
            const double possible_alpha = static_cast<double>(step - 1) / static_cast<double>(num_steps);
        ...
        StaticParallelForIndexLoop(DegreeOfParallelism(number_of_threads), 0, num_steps + 1, step_work,
                                   ParallelForBackend::BEST_AVAILABLE);
...
std::vector<EdgeMeasure> CollisionChecker::MeasureEdgesCollisionFree(
        const std::vector<std::pair<Eigen::VectorXd, Eigen::VectorXd>>& edges, const Parallelism parallelize) const {
    std::vector<EdgeMeasure> collision_checks(edges.size(), EdgeMeasure(0.0, -1.0));

    const int number_of_threads = GetNumberOfThreads(parallelize);
    drake::log()->debug("MeasureEdgesCollisionFree uses {} thread(s)", number_of_threads);

    const auto edge_work = [&](const int thread_num, const int64_t index) {
        const std::pair<Eigen::VectorXd, Eigen::VectorXd>& edge = edges.at(index);

        collision_checks.at(index) = MeasureEdgeCollisionFree(edge.first, edge.second, thread_num);
    };
...
RobotClearance CollisionChecker::CalcRobotClearance(const Eigen::VectorXd& q,
                                                    const double influence_distance,
                                                    const std::optional<int> context_number) const {
    return CalcContextRobotClearance(&mutable_model_context(context_number), q, influence_distance);
}

RobotClearance CollisionChecker::CalcContextRobotClearance(CollisionCheckerContext* model_context,
...
int CollisionChecker::MaxNumDistances(const std::optional<int> context_number) const {
    return MaxContextNumDistances(model_context(context_number));
}

int CollisionChecker::MaxContextNumDistances(const CollisionCheckerContext& model_context) const {
...
std::vector<RobotCollisionType> CollisionChecker::ClassifyBodyCollisions(
    ...
    return ClassifyContextBodyCollisions(&mutable_model_context(context_number), q);
}

std::vector<RobotCollisionType> CollisionChecker::ClassifyContextBodyCollisions(CollisionCheckerContext* model_context,
                                                                                const Eigen::VectorXd& q) const {
    DRAKE_THROW_UNLESS(model_context != nullptr);
    UpdateContextPositions(model_context, q);
...
CollisionChecker::CollisionChecker(CollisionCheckerParams params, bool supports_parallel_checking)
    : setup_model_(std::move(params.model)),
      robot_model_instances_([&params]() {
          ...
          const std::set<ModelInstanceIndex> sorted_set(params.robot_model_instances.begin(),
                                                        params.robot_model_instances.end());
          const std::vector<ModelInstanceIndex> sorted_vec(sorted_set.begin(), sorted_set.end());
      ...
      uncontrolled_dofs_that_kinematically_affect_the_robot_(
              CalcUncontrolledDofsThatKinematicallyAffectTheRobot(setup_model_->plant(), robot_model_instances_)),
    ...
    if (!SupportsParallelChecking() && implicit_context_parallelism_.num_threads() > 1) {
    ...
    default_configuration_ = plant().GetPositions(*plant().CreateDefaultContext());

    // Initialize the collision padding matrix.
    collision_padding_ = Eigen::MatrixXd::Zero(plant().num_bodies(), plant().num_bodies());
    ...
    const bool params_has_provider = params.distance_and_interpolation_provider != nullptr;
    const bool params_has_distance_function = params.configuration_distance_function != nullptr;
    ...
    if (params_has_provider) {
        SetDistanceAndInterpolationProvider(std::move(params.distance_and_interpolation_provider));
    } else if (params_has_distance_function) {
        SanityCheckConfigurationDistanceFunction(params.configuration_distance_function, GetDefaultConfiguration());
        // Generate the default interpolation function.
        const ConfigurationInterpolationFunction default_interpolation_fn =
                MakeDefaultConfigurationInterpolationFunction(GetQuaternionDofStartIndices(plant()));
        distance_and_interpolation_provider_ = std::make_unique<LegacyDistanceAndInterpolationProvider>(
                params.configuration_distance_function, default_interpolation_fn);
    } else {
        SetDistanceAndInterpolationProvider(std::make_unique<LinearDistanceAndInterpolationProvider>(plant()));
    }
...
void CollisionChecker::AllocateContexts() {
    ...
    const std::unique_ptr<CollisionCheckerContext> prototype_context = CreatePrototypeContext();
    DRAKE_THROW_UNLESS(prototype_context != nullptr);
    owned_contexts_.AllocateOwnedContexts(*prototype_context, implicit_context_parallelism_.num_threads());
}

void CollisionChecker::OwnedContextKeeper::AllocateOwnedContexts(const CollisionCheckerContext& prototype_context,
...
CollisionCheckerContext& CollisionChecker::mutable_model_context(const std::optional<int> context_number) const {
    const int context_index = context_number.has_value() ? *context_number : GetContextOmpThreadNum();
    return owned_contexts_.get_mutable_model_context(context_index);
}

void CollisionChecker::ValidateFilteredCollisionMatrix(const Eigen::MatrixXi& filtered, const char* func) const {
    ...
    for (int i = 0; i < count; ++i) {
        if (filtered(i, i) != -1) {
            throw std::logic_error(
                    fmt::format("CollisionChecker::{}(): The filtered collision matrix has invalid "
        ...
        for (int j = i + 1; j < count; ++j) {
            ...
            if (!pair_has_robot && filtered(i, j) != -1) {
                throw std::logic_error(
                        fmt::format("CollisionChecker::{}(): The filtered collision matrix must "
            ...
            if (filtered(i, j) != filtered(j, i)) {
                throw std::logic_error(
                        fmt::format("CollisionChecker::{}(): The filtered collision matrix must be "
            ...
            if (filtered(i, j) < 0 && pair_has_robot) {
                throw std::logic_error(
                        fmt::format("CollisionChecker::{}(): The filtered collision matrix can only be "
...
Eigen::MatrixXi CollisionChecker::GenerateFilteredCollisionMatrix() const {
    ...
    Eigen::MatrixXi filtered_collisions = Eigen::MatrixXi::Zero(num_bodies, num_bodies);
    ...
    const std::vector<std::set<BodyIndex>> subgraphs = plant().FindSubgraphsOfWeldedBodies();
    ...
    for (int i = 0; i < num_bodies; ++i) {
        ...
        const std::vector<GeometryId>& geometries_i = plant().GetCollisionGeometriesForBody(body_i);
        ...
        for (int j = i + 1; j < num_bodies; ++j) {
            ...
            const std::vector<GeometryId>& geometries_j = plant().GetCollisionGeometriesForBody(body_j);
            if (geometries_i.size() > 0 && geometries_j.size() > 0) {
                collisions_filtered = inspector.CollisionFiltered(geometries_i.at(0), geometries_j.at(0));
                ...
                for (const auto& id_i : geometries_i) {
                    for (const auto& id_j : geometries_j) {
                        const bool current_filtered = inspector.CollisionFiltered(id_i, id_j);
                        if (current_filtered != collisions_filtered) {
                            throw std::runtime_error(
                                    fmt::format("SceneGraph's collision filters on the geometries of bodies "
            ...
            const bool bodies_welded_together = body_i_subgraph_id == body_j_subgraph_id;
            ...
            if (collisions_filtered) {
                ...
                log()->debug("Collision between body {} [{}] and body {} [{}] not filtered", body_i.scoped_name(), i,
                             body_j.scoped_name(), j);
            }
        }
    }
    ...
    for (int r = 0; r < N - 1; ++r) {
        ...
        for (int c = r + 1; c < N; ++c) {
            ...
            if (r_is_robot || c_is_robot) {
                max_collision_padding_ = std::max(max_collision_padding_, collision_padding_(r, c));
            }
        }
    }
...
void CollisionChecker::ValidatePaddingMatrix(const Eigen::MatrixXd& padding, const char* func) const {
...
std::string CollisionChecker::CriticizePaddingMatrix(const Eigen::MatrixXd& padding, const char* func) const {
...
int CollisionChecker::GetNumberOfThreads(const Parallelism parallelize) const {
    const bool check_in_parallel = CanEvaluateInParallel() && parallelize.num_threads() > 1;
...
CollisionChecker::OwnedContextKeeper::OwnedContextKeeper(const CollisionChecker::OwnedContextKeeper& other) {
    AllocateOwnedContexts(other.prototype_context(), other.num_contexts());
}

void CollisionChecker::OwnedContextKeeper::PerformOperationAgainstAllOwnedContexts(
...
CollisionChecker::StandaloneContextReferenceKeeper::~StandaloneContextReferenceKeeper() = default;
...
void CollisionChecker::StandaloneContextReferenceKeeper::AddStandaloneContext(
    ...
    standalone_contexts_.push_back(std::weak_ptr<CollisionCheckerContext>(standalone_context));
}

void CollisionChecker::StandaloneContextReferenceKeeper::PerformOperationAgainstAllStandaloneContexts(
        const RobotDiagram<double>& model,
        const std::function<void(const RobotDiagram<double>&, CollisionCheckerContext*)>& operation) {
    DRAKE_DEMAND(operation != nullptr);
    std::lock_guard<std::mutex> lock(standalone_contexts_mutex_);
    for (auto standalone_context = standalone_contexts_.begin(); standalone_context != standalone_contexts_.end();) {
