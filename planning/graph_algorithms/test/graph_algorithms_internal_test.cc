#include "planning/graph_algorithms/graph_algorithms_internal.h"
...
GTEST_TEST(SymmetrizeTripletList, TestSymmetrization) {
    ...
    auto check_all_elts_in_v1_are_in_v2 = [](const std::vector<Eigen::Triplet<bool>>& v1,
                                             const std::vector<Eigen::Triplet<bool>>& v2) {
        for (const auto& elt1 : v1) {
            bool found_elt = false;
            for (const auto& elt2 : v2) {
                if (elt1.row() == elt2.row() && elt1.col() == elt2.col() && elt1.value() == elt2.value()) {
                    found_elt = true;
                    break;
                }
            }
...
GTEST_TEST(ComplementGraph, CompleteGraph) {
    for (const int n : {3, 8}) {
        ...
        Eigen::SparseMatrix<bool> complement_graph = internal::ComplementAdjacencyMatrix(graph);
        Eigen::SparseMatrix<bool> complement_graph_expected(n, n);
        complement_graph.finalize();
        EXPECT_TRUE(CompareMatrices(complement_graph.toDense().cast<double>(),
...
GTEST_TEST(ComplementGraph, BullGraph) {
    ...
    Eigen::SparseMatrix<bool> complement_graph_expected(graph.rows(), graph.cols());
    complement_graph_expected.setFromTriplets(expected_entries.begin(), expected_entries.end());
    EXPECT_TRUE(CompareMatrices(complement_graph.toDense().cast<double>(),
                                complement_graph_expected.toDense().cast<double>()));
}
