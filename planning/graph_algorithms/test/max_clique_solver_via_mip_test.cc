...
#include "planning/graph_algorithms/test/common_graphs.h"
...
// the graph.
void TestMaxCliqueViaMip(const Eigen::Ref<const Eigen::SparseMatrix<bool>>& adjacency_matrix,
    ...
    if ((solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) ||
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(solver.SolveMaxClique(adjacency_matrix), ".*There is no solver available.*");
    }
}
...
GTEST_TEST(MaxCliqueSolverViaMipTest, TestConstructorSettersAndGetters) {
    ...
    EXPECT_TRUE(CompareMatrices(solver2.GetInitialGuess().value(), initial_guess));
...
GTEST_TEST(MaxCliqueSolverViaMipTest, CompleteGraph) {
    for (const auto n : {3, 8}) {
        ...
        std::vector<VectorX<bool>> possible_solutions{VectorX<bool>::Constant(n, true)};
...
GTEST_TEST(MaxCliqueSolverViaMipTest, ButteryflyWithSelfLoops) {
    ...
    Eigen::SparseMatrix<bool> identity(graph_no_loops.rows(), graph_no_loops.rows());
    identity.setFromTriplets(triplets_identity.begin(), triplets_identity.end());

    Eigen::SparseMatrix<bool> graph = (graph_no_loops + identity).template cast<bool>();
    VectorX<bool> solution1(5);
    VectorX<bool> solution2(5);
    // The largest cliques are (0,1,2) and (2,3,4).
    solution1 << true, true, true, false, false;
...
GTEST_TEST(MaxCliqueSolverViaMipTest, FullyConnectedPlusFullBipartiteGraph) {
    ...
    // 3.
    Eigen::SparseMatrix<bool> graph = internal::FullyConnectedPlusFullBipartiteGraph();
