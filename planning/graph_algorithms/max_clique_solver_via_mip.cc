...
MaxCliqueSolverViaMip::MaxCliqueSolverViaMip(const std::optional<Eigen::VectorXd>& initial_guess,
                                             const solvers::SolverOptions& solver_options)
    : initial_guess_{initial_guess}, solver_options_{solver_options} {}

VectorX<bool> MaxCliqueSolverViaMip::DoSolveMaxClique(const SparseMatrix<bool>& adjacency_matrix) const {
    const int n = adjacency_matrix.rows();
    DRAKE_THROW_UNLESS(!initial_guess_.has_value() || initial_guess_.value().rows() == n);
    ...
    const int num_zero_in_adjacency = adjacency_matrix.rows() * adjacency_matrix.rows() - adjacency_matrix.nonZeros();
    const int num_zero_on_diagonal = adjacency_matrix.rows() - adjacency_matrix.diagonal().template cast<int>().sum();
    const int num_expected_constraints = num_zero_in_adjacency - num_zero_on_diagonal;
    ...
    prog.AddLinearConstraint(A, Eigen::VectorXd::Zero(A.rows()), Eigen::VectorXd::Ones(A.rows()), x);
