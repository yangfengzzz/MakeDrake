...
Eigen::VectorXi ComputeDegreeOfVertices(const Eigen::SparseMatrix<bool>& adjacency_matrix) {
    ...
    DRAKE_ASSERT(adjacency_matrix.toDense() == adjacency_matrix.transpose().toDense());
...
std::list<int> DecreasingArgsort(const Eigen::Ref<const Eigen::VectorXi>& values) {
...
VectorX<bool> MaxCliqueSolverViaGreedy::DoSolveMaxClique(const SparseMatrix<bool>& adjacency_matrix) const {
    ...
    // fill in solution
    Eigen::VectorX<bool> is_clique_member = Eigen::VectorX<bool>::Constant(adjacency_matrix.rows(), false);
