...
#include "planning/graph_algorithms/max_clique_solver_via_mip.h"
...
void Draw2dVPolytope(const VPolytope& polytope,
    ...
    Eigen::Matrix3Xd points = Eigen::Matrix3Xd::Zero(3, polytope.vertices().cols() + 1);
    ...
    meshcat->SetLine(meshcat_name, points, 2.0, Rgba(color(0), color(1), color(2)));
}

GTEST_TEST(IrisInConfigurationSpaceFromCliqueCover, TestIrisFromCliqueCoverOptions) {
    ...
    EXPECT_EQ(options.parallelism.num_threads(), Parallelism::Max().num_threads());
...
GTEST_TEST(IrisInConfigurationSpaceFromCliqueCover, BoxConfigurationSpaceTest) {
    ...
    params.robot_model_instances = builder.parser().AddModelsFromString(free_box, "urdf");
    ...
    // IrisOptions.
    options.iris_options.bounding_region = HPolyhedron::MakeBox(Eigen::Vector2d{-2, -2}, Eigen::Vector2d{2, 2});
    ...
    IrisInConfigurationSpaceFromCliqueCover(*checker, options, &generator, &sets, nullptr);
...
GTEST_TEST(IrisInConfigurationSpaceFromCliqueCover, NoJointLimits) {
    ...
    RobotDiagramBuilder<double> builder(0.0);
    params.robot_model_instances = builder.parser().AddModelsFromUrl("package://drake/examples/pendulum/Pendulum.urdf");
    params.edge_step_size = 0.01;
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(IrisInConfigurationSpaceFromCliqueCover(*checker, options, &generator, &sets, nullptr),
...
class IrisInConfigurationSpaceFromCliqueCoverTestFixture : public ::testing::Test {
protected:
    void SetUp() override {
    ...
    obs_points << -s, s,  s, -s, -s,
        ...
        RobotDiagramBuilder<double> builder(0.0);
        params.robot_model_instances = builder.parser().AddModelsFromString(boxes_in_corners, "urdf");
        params.edge_step_size = 0.01;
        ...
        manual_decomposition.push_back(Hyperrectangle(Vector2d{-2, -2}, Vector2d{-1.7, 2}));
        manual_decomposition.push_back(Hyperrectangle(Vector2d{-2, -2}, Vector2d{2, -1.7}));
        manual_decomposition.push_back(Hyperrectangle(Vector2d{1.7, -2}, Vector2d{2, 2}));
        manual_decomposition.push_back(Hyperrectangle(Vector2d{-2, 1.7}, Vector2d{2, 2}));
        manual_decomposition.push_back(Hyperrectangle(Vector2d{-0.3, -2}, Vector2d{0.3, 2}));
        manual_decomposition.push_back(Hyperrectangle(Vector2d{-2, -0.3}, Vector2d{2, 0.3}));
        ...
        for (int i = 0; i < ssize(manual_decomposition); ++i) {
            ...
            color.normalize();
            VPolytope vregion = VPolytope(manual_decomposition.at(i).MakeHPolyhedron()).GetMinimalRepresentation();
            Draw2dVPolytope(vregion, fmt::format("manual_decomposition_{}", i), color, meshcat);
        }
    }
...
TEST_F(IrisInConfigurationSpaceFromCliqueCoverTestFixture, BoxWithCornerObstaclesTestMip) {
    ...
    if ((solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled()) ||
        ...
        solver_options.SetOption(solvers::MosekSolver().id(), "MSK_IPAR_MIO_MAX_NUM_SOLUTIONS", kFeasibleSolutionLimit);
        solver_options.SetOption(solvers::MosekSolver().id(), "MSK_DPAR_MIO_TOL_REL_GAP", kRelOptGap);
        solver_options.SetOption(solvers::GurobiSolver().id(), "SolutionLimit", kFeasibleSolutionLimit);
        solver_options.SetOption(solvers::GurobiSolver().id(), "MIPGap", kRelOptGap);

        planning::graph_algorithms::MaxCliqueSolverViaMip solver{std::nullopt, solver_options};
        ...
        for (int i = 0; i < ssize(sets); ++i) {
            ...
            Draw2dVPolytope(vregion, fmt::format("iris_from_clique_cover_mip{}", i), color, meshcat);
        }
...
TEST_F(IrisInConfigurationSpaceFromCliqueCoverTestFixture, BoxWithCornerObstaclesTestGreedy) {
    ...
    for (int i = 0; i < ssize(sets); ++i) {
        ...
        color.normalize();
        VPolytope vregion = VPolytope(sets.at(i)).GetMinimalRepresentation();
        Draw2dVPolytope(vregion, fmt::format("iris_from_clique_cover_greedy{}", i), color, meshcat);
    }
