...
public:
    ...
    systems::Context<T>& mutable_plant_context(systems::Context<T>* root_context) const {
    ...
    understand `root_context`.
    @throws std::exception if the `root_context` is not a root context. */
    const systems::Context<T>& plant_context(const systems::Context<T>& root_context) const {
    ...
    understand `root_context`.
    @throws std::exception if the `root_context` is not a root context. */
    systems::Context<T>& mutable_scene_graph_context(systems::Context<T>* root_context) const {
    ...
    understand `root_context`.
    @throws std::exception if the `root_context` is not a root context. */
    const systems::Context<T>& scene_graph_context(const systems::Context<T>& root_context) const {
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::planning::RobotDiagram);
