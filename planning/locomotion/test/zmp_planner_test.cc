#include "planning/locomotion/zmp_planner.h"
...
#include "planning/locomotion/test_utilities/zmp_test_util.h"
...
protected:
    ...
    void SetUp() override {
        ...
        PiecewisePolynomial<double> zmp_d = PiecewisePolynomial<double>::FirstOrderHold(time, zmp_knots);

        PlanZMP(zmp_d, x0, height);
    }

    // Plan ZMP and set up internal matrices given a desired ZMP trajectory.
    void PlanZMP(const PiecewisePolynomial<double>& zmp_d, const Eigen::Vector4d& x0, double height) {
    ...
    Eigen::Vector4d get_x(double time) const {
        Eigen::Vector4d x;
        x << zmp_planner_.get_nominal_com(time), zmp_planner_.get_nominal_comd(time);
        return x;
    }
    ...
    Eigen::Vector2d get_ybar(double time) const {
        return zmp_planner_.get_desired_zmp(time) - zmp_planner_.get_final_desired_zmp();
    }
    ...
    Eigen::Matrix<double, 1, 1> ComputeS3dot(double time) const {
        ...
        Eigen::Vector2d rs = 0.5 * (r2 + B_.transpose() * zmp_planner_.get_Vx(time));
    ...
    Eigen::Vector4d ComputeS2dot(double time) const {
        ...
        Eigen::Vector2d r2 = -2 * D_ * Qy_ * y_bar;
        Eigen::Vector2d rs = 0.5 * (r2 + B_.transpose() * zmp_planner_.get_Vx(time));
        Eigen::Vector4d s2dot = -q2 + 2 * NB_.transpose() * R1i_ * rs - A_.transpose() * zmp_planner_.get_Vx(time);
    ...
    Eigen::Matrix4d ComputeS1dot() const {
        return -C_.transpose() * Qy_ * C_ + NB_.transpose() * R1i_ * NB_ - Vxx_ * A_ - A_.transpose() * Vxx_;
    }
    ...
    Eigen::Vector2d ComputeOptimalCoMddGivenValueFunction(double time, const Eigen::Vector4d& x) const {
        Eigen::Vector4d x_bar = x;
        x_bar.head<2>() -= zmp_planner_.get_final_desired_zmp();
        Eigen::Vector2d zmp_d = zmp_planner_.get_desired_zmp(time);

        Eigen::Vector2d lin = -(C_ * x - zmp_d).transpose() * Qy_ * D_ -
...
TEST_F(ZmpPlannerTest, TestS1dot) {
    EXPECT_TRUE(CompareMatrices(ComputeS1dot(), Eigen::Matrix4d::Zero(), 1e-8, MatrixCompareType::absolute));
}
...
TEST_F(ZmpPlannerTest, TestS2dot) {
    ...
    ExponentialPlusPiecewisePolynomial<double> Vxdot = zmp_planner_.get_Vx().derivative();

    for (double t = t0 + dt; t + dt < t1; t += dt) {
        EXPECT_TRUE(CompareMatrices(Vxdot.value(t), ComputeS2dot(t), 1e-8, MatrixCompareType::absolute));
    }
}
...
TEST_F(ZmpPlannerTest, TestCoMdd) {
    ...
    double t1 = zmp_planner_.get_desired_zmp().end_time();
    for (double t = t0 + dt; t + dt < t1; t += dt) {
        EXPECT_TRUE(CompareMatrices(zmp_planner_.get_nominal_comdd(t), zmp_planner_.ComputeOptimalCoMdd(t, get_x(t)),
...
TEST_F(ZmpPlannerTest, TestOptimalControl) {
    ...
    std::vector<Eigen::Vector2d> footsteps = {Eigen::Vector2d(0, 0),    Eigen::Vector2d(0.5, 0.1),
                                              Eigen::Vector2d(1, -0.1), Eigen::Vector2d(1.5, 0.1),
                                              Eigen::Vector2d(2, -0.1), Eigen::Vector2d(2.5, 0)};

    std::vector<PiecewisePolynomial<double>> zmp_trajs = GenerateDesiredZmpTrajs(footsteps, 0.5, 1);
    ...
    for (const auto& zmp_d : zmp_trajs) {
        ...
        x0 << 0.1, -0.05, 0.1, 0.1;
        ZmpTestTraj result = SimulateZmpPolicy(zmp_planner_, x0, sample_dt, simulation_past_end_time);

        int N = result.time.size();
        // Expect the trajectory converges to the desired at the end.
        EXPECT_TRUE(CompareMatrices(result.x.col(N - 1).head<4>(), result.nominal_com.col(N - 1).head<4>(), 1e-4,
