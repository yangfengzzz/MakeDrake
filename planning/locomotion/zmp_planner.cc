...
Eigen::Vector2d ZmpPlanner::ComputeOptimalCoMdd(double time, const Eigen::Vector4d& x) const {
...
bool ZmpPlanner::CheckStationaryEndPoint(const PiecewisePolynomial<double>& zmp_d) const {
    PiecewisePolynomial<double> last_segment = zmp_d.slice(zmp_d.get_number_of_segments() - 1, 1);
...
void ZmpPlanner::Plan(const PiecewisePolynomial<double>& zmp_d,
    ...
    std::vector<Eigen::MatrixXd> beta(n_segments, Eigen::MatrixXd::Zero(4, zmp_d_degree + 1));
    std::vector<Eigen::MatrixXd> gamma(n_segments, Eigen::MatrixXd::Zero(2, zmp_d_degree + 1));
    std::vector<Eigen::MatrixXd> c(n_segments, Eigen::MatrixXd::Zero(2, zmp_d_degree + 1));

    std::vector<Eigen::Matrix<Polynomial<double>, Eigen::Dynamic, Eigen::Dynamic>> beta_poly(n_segments);
    std::vector<Eigen::Matrix<Polynomial<double>, Eigen::Dynamic, Eigen::Dynamic>> gamma_poly(n_segments);
    ...
    for (int t = n_segments - 1; t >= 0; t--) {
        c[t].setZero();
        c[t].row(0).head(zmp_d_degree + 1) = zmp_d.getPolynomial(t, 0, 0).GetCoefficients();
        c[t].row(1).head(zmp_d_degree + 1) = zmp_d.getPolynomial(t, 1, 0).GetCoefficients();
        ...
        gamma[t].col(zmp_d_degree) =
                R1i * D_ * Qy_ * c[t].col(zmp_d_degree) - 0.5 * R1i * B_.transpose() * beta[t].col(zmp_d_degree);
        ...
        A2exp = A2exp.exp();
        for (int i = 0; i < zmp_d_degree + 1; i++) delta_time_vec[i] = std::pow(dt, i);
    ...
    s2_ = ExponentialPlusPiecewisePolynomial<double>(Eigen::Matrix4d::Identity(), A2, alpha, beta_traj);

    // Eq. 28 in [1].
    PiecewisePolynomial<double> gamma_traj(gamma_poly, zmp_d.get_segment_times());
    k2_ = ExponentialPlusPiecewisePolynomial<double>(-0.5 * R1i * B_.transpose(), A2, alpha, gamma_traj);
    ...
    std::vector<Eigen::MatrixXd> b(n_segments, Eigen::MatrixXd(4, zmp_d_degree + 1));
