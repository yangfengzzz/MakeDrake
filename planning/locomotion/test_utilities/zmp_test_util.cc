...
ZmpTestTraj SimulateZmpPolicy(const ZmpPlanner& zmp_planner,
    ...
    int N = static_cast<int>((zmp_d.end_time() + extra_time_at_the_end - zmp_d.start_time()) / dt);
    ...
    for (int i = 0; i < N; i++) {
        ...
        traj.u.col(i) = zmp_planner.ComputeOptimalCoMdd(traj.time[i], traj.x.col(i));
        ...
        traj.nominal_com.col(i).head<2>() = zmp_planner.get_nominal_com(traj.time[i]);
        traj.nominal_com.col(i).segment<2>(2) = zmp_planner.get_nominal_comd(traj.time[i]);
        traj.nominal_com.col(i).tail<2>() = zmp_planner.get_nominal_comdd(traj.time[i]);
    }
...
std::vector<PiecewisePolynomial<double>> GenerateDesiredZmpTrajs(const std::vector<Eigen::Vector2d>& footsteps,
    ...
    zmp_trajs.reserve(3);
    zmp_trajs.push_back(PiecewisePolynomial<double>::ZeroOrderHold(time_steps, zmp_d));
    zmp_trajs.push_back(PiecewisePolynomial<double>::FirstOrderHold(time_steps, zmp_d));
    zmp_trajs.push_back(PiecewisePolynomial<double>::CubicShapePreserving(time_steps, zmp_d));
...
const std::function<ZmpTestTraj(const ZmpPlanner&, const Eigen::Vector4d&, double, double)> SimulateZMPPolicy =
        SimulateZmpPolicy;
