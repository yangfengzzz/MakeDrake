...
CollisionCheckerContext::CollisionCheckerContext(const RobotDiagram<double>* model)
...
const QueryObject<double>& CollisionCheckerContext::GetQueryObject() const {
    return model_.plant().get_geometry_query_input_port().template Eval<QueryObject<double>>(plant_context());
}

CollisionCheckerContext::CollisionCheckerContext(const CollisionCheckerContext& other)
...
unique_ptr<CollisionCheckerContext> CollisionCheckerContext::DoClone() const {
    return unique_ptr<CollisionCheckerContext>(new CollisionCheckerContext(*this));
}
...
CollisionCheckerContext::CollisionCheckerContext(const RobotDiagram<double>* model,
    ...
    : model_(*NonNull(model)),
      ...
      scene_graph_context_(&model_.mutable_scene_graph_context(model_context_.get())) {}
