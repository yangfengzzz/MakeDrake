...
public:
    ...
    const systems::Context<double>& model_context() const { return *model_context_; }
    ...
    const systems::Context<double>& scene_graph_context() const { return *scene_graph_context_; }
    ...
    systems::Context<double>& mutable_scene_graph_context() { return *scene_graph_context_; }
...
private:
    /* The resulting object stores an alias to `model`; the passed model should
     have a lifetime greater than the constructed object. */
    CollisionCheckerContext(const RobotDiagram<double>* model, std::unique_ptr<systems::Context<double>> model_context);
