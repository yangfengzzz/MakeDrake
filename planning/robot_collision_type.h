...
inline RobotCollisionType SetInEnvironmentCollision(RobotCollisionType collision_type, bool in_environment_collision) {
    constexpr uint8_t mask = static_cast<uint8_t>(RobotCollisionType::kEnvironmentCollision);
    return static_cast<RobotCollisionType>((static_cast<uint8_t>(collision_type) & ~mask) |
...
/** @returns a RobotCollisionType where the self-collision value is that
 ...
 asserted by `collision_type`. */
inline RobotCollisionType SetInSelfCollision(RobotCollisionType collision_type, bool in_self_collision) {
    constexpr uint8_t mask = static_cast<uint8_t>(RobotCollisionType::kSelfCollision);
    return static_cast<RobotCollisionType>((static_cast<uint8_t>(collision_type) & ~mask) | (in_self_collision * mask));
}
