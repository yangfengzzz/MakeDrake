...
public:
    DummyContext(const RobotDiagram<double>* model, int value) : CollisionCheckerContext(model), value_(value) {}
...
private:
    ...
    DummyContext(const DummyContext& context) : CollisionCheckerContext(context), value_(context.value_) {}
...
GTEST_TEST(CollisionCheckerContextTest, Cloneable) {
    ...
    /* The clone is wired; the plant/scene graph contexts belong to the model
     context. */
    EXPECT_EQ(&diagram->plant_context(cloned->model_context()), &cloned->plant_context());
    EXPECT_EQ(&diagram->scene_graph_context(cloned->model_context()), &cloned->scene_graph_context());

    /* The cloned context were *copied* and not simply default allocated. */
    EXPECT_EQ(cloned->model_context().get_time(), source.model_context().get_time());
}
