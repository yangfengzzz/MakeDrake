...
ModelInstanceIndex AddEnvironmentModelInstance(MultibodyPlant<double>* plant, int num_geo = 1) {
    // We want to be able to call this without model instances to conflict, so
    // we'll simply track how many times we call it.
    static int count = 0;
    const ModelInstanceIndex instance = plant->AddModelInstance(fmt::format("env{}", ++count));
    ...
    for (int i = 0; i < num_geo; ++i) {
        plant->RegisterCollisionGeometry(body, RigidTransformd::Identity(), Sphere(0.01), fmt::format("g{}", i),
...
public:
    explicit CollisionCheckerTester(CollisionCheckerParams params, bool supports_parallel_checking = false)
        : UnimplementedCollisionChecker(MaybeNerfParamParallelism(std::move(params), supports_parallel_checking),
    ...
    bool DoCheckContextConfigCollisionFree(const CollisionCheckerContext& model_context) const override {
    ...
    optional<GeometryId> DoAddCollisionShapeToBody(const std::string&,
    ...
    RobotClearance DoCalcContextRobotClearance(const CollisionCheckerContext&, double influence_dist) const override {
        ...
        clearance.Append(BodyIndex(1), BodyIndex(0), RobotCollisionType::kEnvironmentCollision, 1.5, Jq_dist);
        return clearance;
    }
    ...
    std::vector<RobotCollisionType> DoClassifyContextBodyCollisions(
        ...
        // CollisionChecker promises that positions have been updated from public
        // method arguments.
        positions_for_classify_ = plant().GetPositions(model_context.plant_context());
        return {};
    }

    int32_t DoMaxContextNumDistances(const CollisionCheckerContext&) const override { return 13; }
    //@}
    ...
    std::string CriticizePaddingMatrix() const { return CollisionChecker::CriticizePaddingMatrix(); }
    //@}
    ...
    const VectorXd& positions_for_classify() const { return positions_for_classify_; }
    ...
    const Eigen::MatrixXi& updated_filter_matrix() const { return updated_filter_matrix_; }
    //@}

private:
    static CollisionCheckerParams MaybeNerfParamParallelism(CollisionCheckerParams params,
...
std::unique_ptr<CollisionCheckerTester> MakeUnallocatedChecker(std::unique_ptr<RobotDiagram<double>> robot,
                                                               const vector<ModelInstanceIndex>& robot_indices,
                                                               bool supports_parallel = true) {
    const ConfigurationDistanceFunction dist{[](const VectorXd& a, const VectorXd& b) {
        return (b - a).norm();
    }};
    return std::make_unique<CollisionCheckerTester>(CollisionCheckerParams{.model = std::move(robot),
...
pair<std::unique_ptr<RobotDiagram<double>>, ModelInstanceIndex> MakeModel(const ModelConfig& config = {}) {
    ...
    if (config.weld_robot) {
        ...
        const ModelInstanceIndex instance = builder_plant.AddModelInstance("floating");
        const RigidBody<double>& floater = builder_plant.AddRigidBody("floater", instance);
        // Connect the first chain body (1) to this floating base.
        builder_plant.AddJoint<RevoluteJoint>("floating", floater, {}, builder_plant.get_body(BodyIndex(1)), {},
...
GTEST_TEST(MakeModel, EnvironmentalFloatingBase) {
    auto [robot, robot_index] = MakeModel({.weld_robot = false, .on_env_base = true});
...
public:
    void ExpectConstructorThrow(const std::string& throw_message_pattern) {
        DRAKE_EXPECT_THROWS_MESSAGE(CollisionCheckerTester({.model = std::move(diagram_),
...
protected:
    ...
    std::vector<ModelInstanceIndex> robot_model_instances_{default_model_instance()};
    ConfigurationDistanceFunction distance_fn_{[](const VectorXd&, const VectorXd&) {
        return 0.0;
    }};
...
GTEST_TEST(CollisionCheckerTest, SortedRobots) {
    ...
    auto dut = std::make_unique<CollisionCheckerTester>(
            CollisionCheckerParams{.model = builder.Build(),
                                   .robot_model_instances = std::vector<ModelInstanceIndex>{robot2, robot2, robot1},
...
GTEST_TEST(CollisionCheckerTest, CollisionCheckerEmpty) {
    ...
    const ConfigurationDistanceFunction fn0 = [](const VectorXd&, const VectorXd&) {
        return 0.0;
    };
...
GTEST_TEST(CollisionCheckerTest, ModelIntrospection) {
    ...
    std::unique_ptr<CollisionCheckerTester> checker = MakeUnallocatedChecker(std::move(robot), {robot_index});
    ...
    EXPECT_THAT(checker->robot_model_instances(), testing::ElementsAre(robot_index));
...
public:
    ...
    vector<BodyIndex> environment_indices() const { return {BodyIndex(0), BodyIndex(4), BodyIndex(5)}; }
    ...
    // dut will be used.
    Eigen::MatrixXi MakeNothingFiltered(CollisionCheckerTester* checker = nullptr) {
        if (checker == nullptr) {
            checker = dut_.get();
        }
...
TEST_F(TrivialCollisionCheckerTest, ClonesAndContexts) {
    ...
    EXPECT_EQ(count_contexts(dut_.get()), dut_->num_allocated_contexts() + 1 /* prototype context */);

    // Add one standalone context.
    auto context = dut_->MakeStandaloneModelContext();
    EXPECT_EQ(count_contexts(dut_.get()),
              dut_->num_allocated_contexts() + 1 /* prototype context */ + 1 /* our standalone context */);

    auto cloned = dut_->Clone();
    EXPECT_NE(cloned, nullptr);

    // The clone's allocated context count matches.
    EXPECT_EQ(count_contexts(cloned.get()), dut_->num_allocated_contexts() + 1 /* prototype context */ +
...
TEST_F(TrivialCollisionCheckerTest, NonZeroDefaultConfiguration) {
    ...
    const VectorXd default_q = robot->plant().GetPositions(*robot->plant().CreateDefaultContext());
    ASSERT_TRUE((default_q.array() != 0).any());

    std::unique_ptr<CollisionChecker> dut = MakeUnallocatedChecker(std::move(robot), {robot_index});
    ...
    auto distance = [&distance_called, &default_q](const VectorXd& q0, const VectorXd& q1) {
    ...
    auto interp = [&interp_called, &default_q](const VectorXd& q0, const VectorXd& q1, double) {
        interp_called = true;
        EXPECT_EQ(q0, default_q);
        EXPECT_EQ(q1, default_q);
        return default_q;
...
TEST_F(TrivialCollisionCheckerTest, Padding) {
    ...
    EXPECT_NO_THROW(dut_->SetPaddingMatrix(Eigen::MatrixXd::Zero(nbodies, nbodies)));
    EXPECT_EQ(dut_->GetLargestPadding(), 0);
    EXPECT_EQ(dut_->CriticizePaddingMatrix(), "");
    EXPECT_EQ(dut_->MaybeGetUniformRobotEnvironmentPadding(), 0);
    EXPECT_EQ(dut_->MaybeGetUniformRobotRobotPadding(), 0);
    ...
    EXPECT_NO_THROW(dut_->SetPaddingOneRobotBodyAllEnvironmentPairs(BodyIndex(3), 0.01));
    EXPECT_EQ(dut_->GetLargestPadding(), 0.01);
    EXPECT_EQ(dut_->CriticizePaddingMatrix(), "");
    EXPECT_EQ(dut_->MaybeGetUniformRobotEnvironmentPadding(), std::nullopt);
    EXPECT_EQ(dut_->MaybeGetUniformRobotRobotPadding(), 0);
...
TEST_F(TrivialCollisionCheckerTest, SetPaddingMatrix) {
...

    {
        ...
        Eigen::MatrixXd wrong_size = Eigen::MatrixXd::Zero(num_bodies + 1, num_bodies + 1);

        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetPaddingMatrix(wrong_size), ".*6x6.*wrong size: 7x7.*");
        Eigen::MatrixXd too_tall = Eigen::MatrixXd::Zero(num_bodies + 1, num_bodies);

        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetPaddingMatrix(too_tall), ".*6x6.*wrong size: 7x6.*");
        Eigen::MatrixXd too_wide = Eigen::MatrixXd::Zero(num_bodies, num_bodies + 1);

        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetPaddingMatrix(too_wide), ".*6x6.*wrong size: 6x7.*");
    }
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetPaddingMatrix(bad_padding), ".* must be symmetric.*");
    }

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetPaddingMatrix(bad_padding), ".* must contain finite values.*");
...
TEST_F(TrivialCollisionCheckerTest, Shapes) {
    ...
    for (bool can_add : {false, true}) {
        ...
        auto added_shapes = dut_->AddCollisionShapes({{"test", {body_shape}}, {"test2", {body_shape}}});
        const int expected_count = can_add ? 1 : 0;
        EXPECT_EQ(added_shapes, (std::map<std::string, int>({{"test", expected_count}, {"test2", expected_count}})));

        EXPECT_EQ(dut_->AddCollisionShapeToFrame("test3", b1.body_frame(), sphere, {}), can_add);
        EXPECT_EQ(dut_->AddCollisionShapeToBody("test3", b1, sphere, {}), can_add);

        const std::map<std::string, std::vector<BodyShapeDescription>> all_shapes = dut_->GetAllAddedCollisionShapes();
        if (can_add) {
...
TEST_F(TrivialCollisionCheckerTest, ReportParallelChecking) {
    for (bool parallel_supported : {true, false}) {
        auto [robot, robot_index] = MakeModel();
        auto checker = MakeUnallocatedChecker(std::move(robot), {robot_index}, parallel_supported);
...
TEST_F(TrivialCollisionCheckerTest, UpdatePositions) {
    ...
    q[0] = 0.2;
    std::shared_ptr<CollisionCheckerContext> collision_context = dut_->MakeStandaloneModelContext();
    const Context<double>& standalone_plant_context = dut_->UpdateContextPositions(collision_context.get(), q);
    // Test for #3 and #4; it doesn't throw and the right positions were recorded.
...
TEST_F(TrivialCollisionCheckerTest, SetCollisionFilterMatrix) {
    ...
    EXPECT_TRUE(CompareMatrices(dut_->updated_filter_matrix(), dut_->GetFilteredCollisionMatrix()));

    // Now test the various ways we can throw.

    {
        // The filter matrix is the wrong size.
        Eigen::MatrixXi wrong_size = Eigen::MatrixXi::Zero(num_bodies + 1, num_bodies + 1);

        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetCollisionFilterMatrix(wrong_size), ".*wrong size.*");
        Eigen::MatrixXi not_square = Eigen::MatrixXi::Zero(num_bodies + 1, num_bodies);

        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetCollisionFilterMatrix(not_square), ".*wrong size.*");
    }
...

    {
        ...
        Eigen::MatrixXi bad_filters(no_filters);
        bad_filters(0, 3) = 1;
        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetCollisionFilterMatrix(bad_filters), ".* must be symmetric.*");
    }
...
TEST_F(TrivialCollisionCheckerTest, CollisionFiltersNominal) {
    {
        ...
        EXPECT_TRUE(CompareMatrices(dut_->GetNominalFilteredCollisionMatrix(), dut_->GetFilteredCollisionMatrix()));
    }

    {
        ...
        const Eigen::MatrixXi first_nominal = checker->GetNominalFilteredCollisionMatrix();
        ...
        // changed.
        EXPECT_TRUE(CompareMatrices(checker->GetFilteredCollisionMatrix(), no_filters));
        EXPECT_TRUE(CompareMatrices(first_nominal, checker->GetNominalFilteredCollisionMatrix()));
    }

    {
        ...
        collision_filters.Apply(
                CollisionFilterDeclaration().AllowWithin(GeometrySet(scene_graph.model_inspector().GetAllFrameIds())));
        auto checker = MakeUnallocatedChecker(std::move(robot), {robot_index});
...

    {
        ...
        collision_filters.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({id1, id3})));
        auto checker = MakeUnallocatedChecker(std::move(robot), {robot_index});
        const Eigen::MatrixXi filters = checker->GetFilteredCollisionMatrix();
        // Confirm it is valid.
        EXPECT_NO_THROW(checker->SetCollisionFilterMatrix(filters));
...

    {
        ...
        const GeometryId g_id1 = scene_graph.model_inspector().GetGeometries(f_id1, Role::kProximity)[0];
        const FrameId f_id3 = robot->plant().GetBodyFrameIdOrThrow(BodyIndex(3));
        const GeometryId g_id3 = scene_graph.model_inspector().GetGeometries(f_id3, Role::kProximity)[0];
        collision_filters.Apply(CollisionFilterDeclaration().ExcludeWithin(GeometrySet({g_id1, g_id3})));

        DRAKE_EXPECT_THROWS_MESSAGE(MakeUnallocatedChecker(std::move(robot), {robot_index}),
...
TEST_F(TrivialCollisionCheckerTest, SetCollisionFilteredBetween) {
    ...
    const vector<BodyIndex> b{BodyIndex(0), BodyIndex(1), BodyIndex(2), BodyIndex(3), BodyIndex(4), BodyIndex(5)};
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetCollisionFilteredBetween(b[0], b[4], true),
                                    ".* cannot be used on pairs of environment bodies.*");
        DRAKE_EXPECT_THROWS_MESSAGE(dut_->SetCollisionFilteredBetween(b[0], b[4], false),
                                    ".* cannot be used on pairs of environment bodies.*");

        // Out of range indices.
        EXPECT_THROW(dut_->SetCollisionFilteredBetween(b[0], BodyIndex(10), true), std::exception);
        EXPECT_THROW(dut_->SetCollisionFilteredBetween(BodyIndex(10), b[0], true), std::exception);

        // Trying to set filter on a robot body with itself.
        EXPECT_THROW(dut_->SetCollisionFilteredBetween(b[1], b[1], true), std::exception);
    }

    {
        ...
        EXPECT_NO_THROW(dut_->SetCollisionFilterMatrix(dut_->GetFilteredCollisionMatrix()));
    ...
    EXPECT_NO_THROW(dut_->SetCollisionFilteredBetween(dut_->get_body(b[2]), dut_->get_body(b[3]), true));
...
TEST_F(TrivialCollisionCheckerTest, SetCollisionFilteredWithAllBodies) {
...

    {
        ...
        // matrix and reports filters as expected.
        EXPECT_NO_THROW(dut_->SetCollisionFilteredWithAllBodies(dut_->get_body(b[2])));
        // Confirm that the collision filter matrix was updated before calling
        // UpdateCollisionFilters().
        EXPECT_TRUE(CompareMatrices(dut_->updated_filter_matrix(), dut_->GetFilteredCollisionMatrix()));
        EXPECT_NO_THROW(dut_->SetCollisionFilterMatrix(dut_->GetFilteredCollisionMatrix()));
...
TEST_F(TrivialCollisionCheckerTest, IsCollisionFilteredBetween) {
    ...
    EXPECT_TRUE(dut_->IsCollisionFilteredBetween(dut_->get_body(BodyIndex(1)), dut_->get_body(BodyIndex(3))));
}
...
GTEST_TEST(EdgeCheckTest, Configuration) {
    ...
    const ConfigurationDistanceFunction dist0 = [](const VectorXd& q1, const VectorXd& q2) {
...

    {
        ...
        const ConfigurationDistanceFunction dist1 = [](const VectorXd& a, const VectorXd& b) {
            return (b - a).norm();
        };
        ...
        ASSERT_NE(dut.MakeStandaloneConfigurationDistanceFunction(), nullptr);
        EXPECT_EQ(dut.MakeStandaloneConfigurationDistanceFunction()(q1, q2), q1.norm());
    }

    {
        ...
        // interpolation.
        EXPECT_EQ(dut.InterpolateBetweenConfigurations(q1, q2, 0.5), 0.5 * (q1 + q2));
        ASSERT_NE(dut.MakeStandaloneConfigurationInterpolationFunction(), nullptr);
        EXPECT_EQ(dut.MakeStandaloneConfigurationInterpolationFunction()(q1, q2, 0.5), 0.5 * (q1 + q2));

        // Change the function via (3).
        const ConfigurationInterpolationFunction interpolation_fn = [](const VectorXd& q, const VectorXd&,
                                                                       double alpha) {
        ...
        // Evaluate (1) and (2) again.
        EXPECT_EQ(dut.InterpolateBetweenConfigurations(q1, q2, 0.5), Eigen::VectorXd::Constant(q_size, 0.5));
        ASSERT_NE(dut.MakeStandaloneConfigurationInterpolationFunction(), nullptr);
        EXPECT_EQ(dut.MakeStandaloneConfigurationInterpolationFunction()(q1, q2, 0.5),
...
GTEST_TEST(EdgeCheckTest, DefaultInterpolation) {
    ...
    auto dut =
            MakeEdgeChecker<CollisionCheckerTester>(dist, 0.1, nullptr /* default interpolator */, false /* welded */);
    ...
    const RigidTransform X_WB0_init(RotationMatrixd(AngleAxisd(M_PI / 7, Vector3d{1, 2, 3}.normalized())),
    ...
    const RigidTransformd X_InitFinal(AngleAxisd(init_final_theta, rot_axis_W), p_InitFinal);
    ...
    auto get_interpolated = [&](double s) {
        ...
        const RigidTransformd X_InitS(RotationMatrixd(AngleAxisd(init_final_theta * s, rot_axis_W)), p_InitS);
    ...
    for (const double s : {0.0, 0.25, 0.6, 1.0}) {
        EXPECT_TRUE(
                CompareMatrices(dut.InterpolateBetweenConfigurations(q_init, q_final, s), get_interpolated(s), 1e-15));
    }

    // Definitely not simply LERP.
    for (const double s : {0.25, 0.6}) {
        EXPECT_FALSE(CompareMatrices(dut.InterpolateBetweenConfigurations(q_init, q_final, s),
...
std::ostream& operator<<(std::ostream& out, const EdgeTestConfig& c) {
    ...
    // name.
    out << "EdgeTestWith" << c.alpha << "AlphaIn" << (c.parallel ? "Parallel" : "Serial");
    return out;
}

class ParameterizedEdgeCheckTest : public testing::TestWithParam<EdgeTestConfig> {};
...
public:
    explicit MockEdgeChecker(CollisionCheckerParams params) : UnimplementedCollisionChecker(std::move(params), true) {
        DRAKE_DEMAND(plant().num_positions() >= kQSize);
        AllocateContexts();
        const int num_omp_threads = common_robotics_utilities::openmp_helpers::GetNumOmpThreads();
        const int max_num_omp_threads = common_robotics_utilities::openmp_helpers::GetMaxNumOmpThreads();
    ...
    int thread_count() const { return std::accumulate(thread_signals_.begin(), thread_signals_.end(), 0); }
    ...
    static VectorXd EncodeConfiguration(int size, double last_free, double last_colliding) {
...
protected:
    bool DoCheckContextConfigCollisionFree(const CollisionCheckerContext& model_context) const override {
        ...
        const int thread_index = common_robotics_utilities::openmp_helpers::GetContextOmpThreadNum();
...
std::vector<EdgeTestConfig> MakeEdgeTestCases() {
    ...
    for (const bool in_parallel : {true, false}) {
        ...
        // Edges are 100% valid.
        configs.push_back({.alpha = 1.0, .last_colliding_alpha = 2.0, .parallel = in_parallel});
        ...
        for (int step = 0; step < MockEdgeChecker::kNumSamples; ++step) {
            ...
            configs.push_back({.alpha = free, .last_colliding_alpha = colliding, .parallel = in_parallel});
        }
    }
...
INSTANTIATE_TEST_SUITE_P(EdgeCheckTest, ParameterizedEdgeCheckTest, testing::ValuesIn(MakeEdgeTestCases()));
...
TEST_P(ParameterizedEdgeCheckTest, MeasureEdgeCollisionFree) {
    ...
    auto dut = MakeEdgeChecker<MockEdgeChecker>(MockEdgeChecker::MakeEdgeDistance(step_size), step_size,
    ...
    const VectorXd q2 = dut.EncodeConfiguration(q_size, config.alpha, config.last_colliding_alpha);

    const EdgeMeasure result =
            config.parallel ? dut.MeasureEdgeCollisionFreeParallel(q1, q2) : dut.MeasureEdgeCollisionFree(q1, q2);
...
TEST_P(ParameterizedEdgeCheckTest, CheckEdgeCollisionFree) {
    ...
    const bool result =
            config.parallel ? dut.CheckEdgeCollisionFreeParallel(q1, q2) : dut.CheckEdgeCollisionFree(q1, q2);
...
GTEST_TEST(EdgeCheckTest, MeasureMultipleEdges) {
    ...
    // 100% clear.
    edges.emplace_back(q_start, MockEdgeChecker::EncodeConfiguration(q_size, 1.0, 1.5));
    // Quarter free (collision at 0.5 and 0.75).
    edges.emplace_back(q_start, MockEdgeChecker::EncodeConfiguration(q_size, 0.25, 0.75));
    // Three quarters free (collision at 1.0).
    edges.emplace_back(q_start, MockEdgeChecker::EncodeConfiguration(q_size, 0.75, 1.25));
    // The distance for all of the edges is the same.
    const double edge_dist = calc_dist(edges[0].first, edges[0].second);
    const vector<EdgeMeasure> expected_results{EdgeMeasure(edge_dist, edges[0].second(0)),
    ...
    for (const bool parallel : {false, true}) {
        ...
        auto dut = MakeEdgeChecker<MockEdgeChecker>(calc_dist, step_size, interp, true /* welded */, q_size + 1);
        ...
        vector<EdgeMeasure> results = dut.MeasureEdgesCollisionFree(edges, parallel);
...
GTEST_TEST(EdgeCheckTest, CheckMultipleEdgesFree) {
    ...
    // Later collision.
    edges.emplace_back(q_start, MockEdgeChecker::EncodeConfiguration(q_size, 0.75, 1.25));
    const vector<uint8_t> expected_results{edges[0].second(0) == 1.0, edges[1].second(0) == 1.0,
