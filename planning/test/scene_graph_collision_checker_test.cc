...
CollisionCheckerTestParams MakeSceneGraphCollisionCheckerParams(MakeCheckerOptions make_checker_option) {
    ...
    if (make_checker_option == MakeCheckerOptions::kMakeProvider) {
        provider = std::make_unique<LinearDistanceAndInterpolationProvider>(model->plant(), GetIiwaDistanceWeights());
    ...
    result.checker.reset(
            new SceneGraphCollisionChecker({.model = std::move(model),
...
Eigen::MatrixXi GenerateFilteredCollisionMatrixFromSceneGraphOnly(const SceneGraphCollisionChecker& checker,
    ...
    for (int i = 0; i < num_bodies; ++i) {
        ...
        const std::vector<GeometryId>& geometries_i = checker.plant().GetCollisionGeometriesForBody(body_i);
        ...
        for (int j = i; j < num_bodies; ++j) {
            ...
            const std::vector<GeometryId>& geometries_j = checker.plant().GetCollisionGeometriesForBody(body_j);
...
void EnforceCollisionFilterConsistency(const SceneGraphCollisionChecker& checker) {
    const Eigen::MatrixXi& current_collision_filter_matrix = checker.GetFilteredCollisionMatrix();
    ...
    using Operation = const std::function<void(const RobotDiagram<double>&, CollisionCheckerContext*)>;
    const Operation operation = [&](const RobotDiagram<double>& model, CollisionCheckerContext* model_context) {
        ...
        for (BodyIndex i(0); i < num_bodies; ++i) {
            ...
            for (BodyIndex j(0); j < num_bodies; ++j) {
                ...
                const bool current_filtered = current_collision_filter_matrix(int{i}, int{j}) != 0;
                const bool context_filtered = context_collision_filter_matrix(int{i}, int{j}) != 0;
    ...
    const_cast<SceneGraphCollisionChecker&>(checker).PerformOperationAgainstAllModelContexts(operation);
...
INSTANTIATE_TEST_SUITE_P(SceneGraphCollisionCheckerTestSuite,
                         CollisionCheckerAbstractTestSuite,
                         testing::Values(MakeSceneGraphCollisionCheckerParams(MakeCheckerOptions::kMakeNone),
                                         MakeSceneGraphCollisionCheckerParams(MakeCheckerOptions::kMakeProvider),
                                         MakeSceneGraphCollisionCheckerParams(MakeCheckerOptions::kMakeFunction)));
...
<world name='default'>
  ...
  <model name='environment'>
    ...
    params.configuration_distance_function = [](const VectorXd& q1, const VectorXd& q2) {
        return (q1 - q2).norm();
    };
    ...
    EXPECT_THAT(clearance.collision_types(),
                ElementsAre(RobotCollisionType::kSelfCollision, RobotCollisionType::kEnvironmentCollision,
...
- add_weld:
    ...
    for (int i = 0; i < clearance.size(); ++i) {
        ...
        ASSERT_TRUE(CompareMatrices(jacobian.head(floating_dofs), VectorXd::Zero(floating_dofs)));
...
- add_weld:
    ...
    // Note: this also tests setting the equivalent filtering via body indices.
    {
        ...
        dut.SetCollisionFilteredBetween(dut.plant().GetBodyByName("iiwa_link_0"),
                                        dut.plant().GetBodyByName("iiwa_link_6"), true);
        ASSERT_FALSE(CompareMatrices(pre_filter_matrix, dut.GetFilteredCollisionMatrix()));
        EXPECT_NO_THROW(EnforceCollisionFilterConsistency(dut));
    }
    ...
    {
        const Eigen::MatrixXi pre_filter_matrix = dut.GetFilteredCollisionMatrix();
        ASSERT_FALSE(CompareMatrices(pre_filter_matrix, dut.GetNominalFilteredCollisionMatrix()));
    ...
    {
        const Eigen::MatrixXi pre_filter_matrix = dut.GetFilteredCollisionMatrix();
        dut.SetCollisionFilteredWithAllBodies(dut.plant().GetBodyByName("iiwa_link_0"));
        ASSERT_FALSE(CompareMatrices(pre_filter_matrix, dut.GetFilteredCollisionMatrix()));
        EXPECT_NO_THROW(EnforceCollisionFilterConsistency(dut));
    }
}
