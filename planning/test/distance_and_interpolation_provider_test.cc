...
class BrokenDistanceAndInterpolationProvider final : public DistanceAndInterpolationProvider {
public:
...
class SimpleLinearDistanceAndInterpolationProvider final : public DistanceAndInterpolationProvider {
public:
...
GTEST_TEST(BrokenDistanceAndInterpolationProviderTest, Test) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.ComputeConfigurationDistance(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(3)),
            ".* condition 'from\\.size\\(\\) == to\\.size\\(\\)' failed.*");

    // Interpolation with different length qs throws.
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(3), 0.0),
            ".* condition 'from\\.size\\(\\) == to\\.size\\(\\)' failed.*");

    // Interpolation with ratios outside [0, 1] throws.
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2), -0.1),
            ".* condition 'ratio >= 0\\.0' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2), 1.1),
            ".* condition 'ratio <= 1\\.0' failed.*");

    // "Broken" provider causes all distance queries to throw.
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.ComputeConfigurationDistance(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2)),
            ".* condition 'distance >= 0\\.0' failed.*");

    // "Broken" provider causes all interpolation queries to throw.
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2), 0.0),
            ".* condition 'from\\.size\\(\\) == interpolated\\.size\\(\\)' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2), 1.0),
            ".* condition 'from\\.size\\(\\) == interpolated\\.size\\(\\)' failed.*");
}
...
GTEST_TEST(SimpleLinearDistanceAndInterpolationProviderTest, Test) {
    ...
    // Distance queries.
    EXPECT_EQ(provider.ComputeConfigurationDistance(Eigen::VectorXd::Zero(2), Eigen::VectorXd::Zero(2)), 0.0);
    EXPECT_EQ(provider.ComputeConfigurationDistance(Eigen::VectorXd::Ones(2), Eigen::VectorXd::Zero(2)),
              std::sqrt(2.0));

    // Interpolation queries.
    EXPECT_TRUE(CompareMatrices(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Ones(2), Eigen::VectorXd::Zero(2), 0.0),
            Eigen::VectorXd::Ones(2)));
    EXPECT_TRUE(CompareMatrices(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Ones(2), Eigen::VectorXd::Zero(2), 1.0),
            Eigen::VectorXd::Zero(2)));
    EXPECT_TRUE(CompareMatrices(
            provider.InterpolateBetweenConfigurations(Eigen::VectorXd::Ones(2), Eigen::VectorXd::Zero(2), 0.5),
