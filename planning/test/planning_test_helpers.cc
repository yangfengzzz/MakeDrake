...
std::unique_ptr<RobotDiagram<double>> MakePlanningTestModel(const ModelDirectives& directives) {
...
ConfigurationDistanceFunction MakeWeightedIiwaConfigurationDistanceFunction() {
    ...
    const ConfigurationDistanceFunction weighted_cspace_distance_fn = [weights](const Eigen::VectorXd& q1,
...
ModelInstanceIndex AddChain(MultibodyPlant<double>* plant, int n, int num_geo) {
    ...
    for (int k = 0; k < n; ++k) {
        ...
        for (int i = 0; i < num_geo; ++i) {
            plant->RegisterCollisionGeometry(*bodies.back(), RigidTransformd::Identity(), Sphere(0.01),
                                             fmt::format("g{}", i), CoulombFriction<double>());
        }
    }
    for (int k = 1; k < n; ++k) {
        plant->AddJoint<RevoluteJoint>(fmt::format("j{}", k), *bodies[k - 1], {}, *bodies[k], {},
