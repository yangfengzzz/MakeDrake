...
constexpr RobotCollisionType kEnvironmentCollision = RobotCollisionType::kEnvironmentCollision;
constexpr RobotCollisionType kSelfCollision = RobotCollisionType::kSelfCollision;
constexpr RobotCollisionType kEnvironmentAndSelfCollision = RobotCollisionType::kEnvironmentAndSelfCollision;
...
struct Case {
    ...
    std::string to_string() const {
        return fmt::format("input {}, in_collision {}, output {}", static_cast<int>(input), in_collision,
...
GTEST_TEST(CollisionCheckerTest, EnvCollisionBits) {
    ...
    for (const auto& a_case : cases) {
        ...
        EXPECT_EQ(SetInEnvironmentCollision(a_case.input, a_case.in_collision), a_case.output);
    }
}
...
GTEST_TEST(CollisionCheckerTest, SelfCollisionBits) {
    ...
    for (const auto& a_case : cases) {
        SCOPED_TRACE(a_case.to_string());
        EXPECT_EQ(SetInSelfCollision(a_case.input, a_case.in_collision), a_case.output);
    }
}
