...
std::unique_ptr<SceneGraphCollisionChecker> MakeSceneGraphCollisionCheckerFromString(const std::string& file_contents,
                                                                                     const std::string& file_type,
                                                                                     double edge_step_size = 0.01) {
    CollisionCheckerParams params;

    RobotDiagramBuilder<double> builder(0.0);
    params.robot_model_instances = builder.parser().AddModelsFromString(file_contents, file_type);
...
GTEST_TEST(VisibilityGraphTest, BoxesInCorners) {
  ...
  points <<     0,    0,
    ...
    for (const bool parallelize : {false, true}) {
        auto checker = MakeSceneGraphCollisionCheckerFromString(boxes_in_corners, "urdf");
        // Confirm that the midpoint between 1 and 2 is in collision.
        EXPECT_FALSE(checker->CheckConfigCollisionFree(
                checker->InterpolateBetweenConfigurations(points.col(1), points.col(2), 0.5)));
        ...
        checker = MakeSceneGraphCollisionCheckerFromString(boxes_in_corners, "urdf", kLargeEdgeStepSize);
