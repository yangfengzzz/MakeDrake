...
GTEST_TEST(RobotDiagramBuilderTest, AddSystem) {
    std::unique_ptr<RobotDiagramBuilder<double>> dut = MakeSampleDut();
    dut->builder().AddNamedSystem<SharedPointerSystem>("foo", std::make_shared<std::string>("bar"));
    auto diagram = dut->Build();
...
GTEST_TEST(RobotDiagramTest, ToAutoDiff) {
    std::unique_ptr<RobotDiagram<double>> robot_diagram_double = MakeSampleDut()->Build();
...
GTEST_TEST(RobotDiagramBuilderTest, AutoDiffFromBirthToDeath) {
    ...
    std::unique_ptr<RobotDiagram<AutoDiffXd>> robot_diagram_autodiff = builder_autodiff->Build();
    ASSERT_NE(robot_diagram_autodiff, nullptr);
    EXPECT_NE(robot_diagram_autodiff->CreateDefaultContext(), nullptr);
...
GTEST_TEST(RobotDiagramTest, ContextGetters) {
    ...
    const Context<double>& scene_graph_context = dut->scene_graph_context(*root_context);
    Context<double>& mutable_plant_context = dut->mutable_plant_context(root_context.get());
    Context<double>& mutable_scene_graph_context = dut->mutable_scene_graph_context(root_context.get());
...
GTEST_TEST(RobotDiagramTest, PortNamesExist) {
    ...
    for (int i = 0; i < dut->num_input_ports(); ++i) {
        EXPECT_THAT(dut->get_input_port(i).get_name(), ::testing::StartsWith("plant_"));
    }
}
