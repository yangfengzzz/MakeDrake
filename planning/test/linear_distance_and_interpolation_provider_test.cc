...
void DoFixedIiwaTest(const RobotDiagram<double>& model,
    ...
    for (int i = 0; i < model.plant().num_positions(); ++i) {
        ...
        EXPECT_EQ(provider.ComputeConfigurationDistance(zero_q, pos_test_q), expected_weight);
        EXPECT_EQ(provider.ComputeConfigurationDistance(zero_q, neg_test_q), expected_weight);
    }

    // Test full-q distance.
    {
        const Eigen::VectorXd weighted_deltas = (ones_q - zero_q).cwiseProduct(expected_weights);
        const double expected_distance = weighted_deltas.norm();
        EXPECT_EQ(provider.ComputeConfigurationDistance(zero_q, ones_q), expected_distance);
    }

    // Interpolation queries.
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(zero_q, ones_q, 0.0), zero_q));
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(zero_q, ones_q, 1.0), ones_q));
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(zero_q, ones_q, 0.5), half_q));
}
...
void DoFloatingIiwaTest(const RobotDiagram<double>& model,
    ...
    const auto make_full_q = [&](const Eigen::VectorXd& arm_q, const math::RigidTransformd& X_WBase) {
    ...
    const math::RigidTransformd X_WBase_test(math::RollPitchYawd(M_PI, -M_PI_2, M_PI_4),
                                             Eigen::Vector3d(1.0, 2.0, 3.0));
    const math::RigidTransformd X_WBase_half(
            Interpolate(X_WBase_identity.GetAsIsometry3(), X_WBase_test.GetAsIsometry3(), 0.5));
    ...
    for (int i = 0; i < num_arm_positions; ++i) {
        ...
        for (const auto& X_WBase : {X_WBase_identity, X_WBase_half, X_WBase_test}) {
            ...
            const Eigen::VectorXd full_pos_test_q = make_full_q(arm_pos_test_q, X_WBase);
            const Eigen::VectorXd full_neg_test_q = make_full_q(arm_neg_test_q, X_WBase);

            EXPECT_EQ(provider.ComputeConfigurationDistance(full_zero_test_q, full_pos_test_q), expected_weight);
            EXPECT_EQ(provider.ComputeConfigurationDistance(full_zero_test_q, full_neg_test_q), expected_weight);
        }
    }
    ...
    // Test full-q distance.
    {
        const double orientation_delta =
                Distance(X_WBase_identity.rotation().ToQuaternion(), X_WBase_test.rotation().ToQuaternion());

        const Eigen::Vector3d translation_deltas = X_WBase_test.translation() - X_WBase_identity.translation();
        ...
        const Eigen::VectorXd weighted_deltas = deltas.cwiseProduct(expected_weights);
        const double expected_distance = weighted_deltas.norm();
        EXPECT_EQ(provider.ComputeConfigurationDistance(full_zero_q, full_test_q), expected_distance);
    }

    // Interpolation queries.
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(full_zero_q, full_test_q, 0.0), full_zero_q));
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(full_zero_q, full_test_q, 1.0), full_test_q));
    EXPECT_TRUE(CompareMatrices(provider.InterpolateBetweenConfigurations(full_zero_q, full_test_q, 0.5), full_half_q,
                                1e-10));
}
...
directives:
  ...
  - add_weld:
    ...
    {
        const LinearDistanceAndInterpolationProvider provider(model->plant(), std::map<JointIndex, Eigen::VectorXd>{});
    ...
    // Non-default weights (map).
    {
        ...
        const LinearDistanceAndInterpolationProvider provider(model->plant(), joint_weights);
    ...
    // Non-default weights (vector).
    {
        ...
        const LinearDistanceAndInterpolationProvider provider(model->plant(), weights);
    ...
    // Invalid weights (map).
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), has_negative_weights),
                                    ".* \\[iiwa_joint_1\\] .* are not non-negative and finite");

        std::map<JointIndex, Eigen::VectorXd> has_non_finite_weights;
        has_non_finite_weights[get_index("iiwa_joint_1")] = Vector1d(std::numeric_limits<double>::infinity());
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), has_non_finite_weights),
                                    ".* \\[iiwa_joint_1\\] .* are not non-negative and finite");

        std::map<JointIndex, Eigen::VectorXd> has_too_many_values;
        has_too_many_values[get_index("iiwa_joint_1")] = Eigen::Vector2d(1.0, 2.0);
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), has_too_many_values),
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), has_no_such_joint),
                                    ".*Joint.*20.*bound.*");
    }
    ...
    // Invalid weights (vector).
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), too_few_weights),
                                    "Provided distance weights size 4 does not match num_positions 7");

        const auto too_many_weights = Eigen::VectorXd::Ones(10);
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), too_many_weights),
        ...
        has_non_finite_weights << 1, 2, 3, 4, 5, 6, std::numeric_limits<double>::infinity();
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), has_non_finite_weights),
...
directives:
  ...
  - add_weld:
    ...
    // Non-default weights (map).
    {
        ...
        joint_weights[get_index("iiwa_joint_6")] = Vector1d(6.0);
        joint_weights[get_index("iiwa_joint_7")] = Vector1d(7.0);
        joint_weights[get_index("flying_robot_base")] = (Eigen::VectorXd(7) << 11, 0, 0, 0, 12, 13, 14).finished();
    ...
    // Invalid weights (map).
    {
        ...
        invalid_quat_weights[get_index("flying_robot_base")] = (Eigen::VectorXd(7) << 1, 2, 3, 4, 5, 6, 7).finished();
        DRAKE_EXPECT_THROWS_MESSAGE(LinearDistanceAndInterpolationProvider(model->plant(), invalid_quat_weights),
...
- add_model:
    ...
    const auto& cracker_floating_joint = model->plant().GetJointByName("base_link_cracker");

    EXPECT_EQ(provider.quaternion_dof_start_indices().size(), 1);
    EXPECT_EQ(provider.quaternion_dof_start_indices().at(0), cracker_floating_joint.position_start());
}
