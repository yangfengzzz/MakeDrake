...
public:
    ...
    DummyCollisionChecker()
        : UnimplementedCollisionChecker({.model = MakeRobot(),
...
private:
    void DoUpdateContextPositions(CollisionCheckerContext* model_context) const final {
...
GTEST_TEST(ComputeCollisionAvoidanceDisplacementTest, ContextProvenance) {
    ...
    /* Implicit context. */
    EXPECT_FALSE(CompareMatrices(checker.plant().GetPositions(checker.plant_context()), q1));
    EXPECT_NO_THROW(ComputeCollisionAvoidanceDisplacement(checker, q1, 0, 1));
    EXPECT_TRUE(CompareMatrices(checker.plant().GetPositions(checker.plant_context()), q1));

    /* Implicit context using context number. */
    const int context_number = 0;
    EXPECT_FALSE(CompareMatrices(checker.plant().GetPositions(checker.plant_context(context_number)), q2));
    EXPECT_NO_THROW(ComputeCollisionAvoidanceDisplacement(checker, q2, 0, 1, context_number));
    EXPECT_TRUE(CompareMatrices(checker.plant().GetPositions(checker.plant_context(context_number)), q2));

    /* Explicit context. */
    auto context = checker.MakeStandaloneModelContext();
    EXPECT_FALSE(CompareMatrices(checker.plant().GetPositions(context->plant_context()), q2));
    EXPECT_NO_THROW(ComputeCollisionAvoidanceDisplacement(checker, q2, 0, 1, std::nullopt, context.get()));
    EXPECT_TRUE(CompareMatrices(checker.plant().GetPositions(context->plant_context()), q2));

    /* Combining implicit context number and explicit context throws. */
    EXPECT_THROW(ComputeCollisionAvoidanceDisplacement(checker, q2, 0, 1, context_number, context.get()),
                 std::exception);
}
...
GTEST_TEST(ComputeCollisionAvoidanceDisplacementTest, NoDistanceMeasurements) {
    ...
    EXPECT_TRUE(CompareMatrices(ComputeCollisionAvoidanceDisplacement(checker, q, 0, 1), VectorXd::Zero(kQSize)));
}
...
GTEST_TEST(ComputeCollisionAvoidanceDisplacementTest, WeightedCombinations) {
...

    {
        ...
        clearance.Append(robot_body, env_body, env_type, kMin - 0.1, q1.transpose());
        checker.set_robot_clearance(std::move(clearance));
        const VectorXd grad = ComputeCollisionAvoidanceDisplacement(checker, q0, kMin, kMax);
...

    {
        ...
        RobotClearance clearance(kQSize);
        clearance.Append(robot_body, env_body, env_type, kMax + 0.1, q1.transpose());
        checker.set_robot_clearance(std::move(clearance));
        const VectorXd grad = ComputeCollisionAvoidanceDisplacement(checker, q0, kMin, kMax);
...

    {
        ...
        RobotClearance clearance(kQSize);
        clearance.Append(robot_body, env_body, env_type, kMax - kRange * kWeight, q1.transpose());
        checker.set_robot_clearance(std::move(clearance));
        const VectorXd grad = ComputeCollisionAvoidanceDisplacement(checker, q0, kMin, kMax);
...

    {
        ...
        RobotClearance clearance(kQSize);
        clearance.Append(robot_body, env_body, env_type, kMax - kRange * kWeight1, q1.transpose());
        clearance.Append(robot_body, env_body, env_type, kMax - kRange * kWeight2, q2.transpose());
        checker.set_robot_clearance(std::move(clearance));
        const VectorXd grad = ComputeCollisionAvoidanceDisplacement(checker, q0, kMin, kMax);
        EXPECT_EQ(grad.size(), kQSize);
...
GTEST_TEST(ComputeCollisionAvoidanceDisplacementTest, Errors) {
    ...
    EXPECT_THROW(ComputeCollisionAvoidanceDisplacement(checker, q, 0.1, 1), std::exception);

    // The max_clearance cannot be negative.
    EXPECT_THROW(ComputeCollisionAvoidanceDisplacement(checker, q, -1, -0.1), std::exception);

    // They cannot be both zero.
    EXPECT_THROW(ComputeCollisionAvoidanceDisplacement(checker, q, 0, 0), std::exception);
}
