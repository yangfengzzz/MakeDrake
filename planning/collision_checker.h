...
#include "planning/collision_checker_params.h"
#include "planning/distance_and_interpolation_provider.h"
...
public:
    ...
    const multibody::MultibodyPlant<double>& plant() const { return model().plant(); }
    ...
    const std::vector<multibody::ModelInstanceIndex>& robot_model_instances() const { return robot_model_instances_; }
    ...
    const Eigen::VectorXd& GetZeroConfiguration() const { return zero_configuration_; }
    ...
    */
    ...
    const Eigen::MatrixXi& GetNominalFilteredCollisionMatrix() const { return nominal_filtered_collisions_; }
    ...
    */
    ...
    double ComputeConfigurationDistance(const Eigen::VectorXd& q1, const Eigen::VectorXd& q2) const {
        return distance_and_interpolation_provider_->ComputeConfigurationDistance(q1, q2);
    }
    ...
    Eigen::VectorXd InterpolateBetweenConfigurations(const Eigen::VectorXd& q1,
                                                     ...
                                                     double ratio) const {
        return distance_and_interpolation_provider_->InterpolateBetweenConfigurations(q1, q2, ratio);
    }
...
private:
    /* @param context_number Optional implicit context number.
     @see @ref ccb_implicit_contexts "Implicit Context Parallelism". */
    CollisionCheckerContext& mutable_model_context(std::optional<int> context_number) const;
    ...
    void ValidateFilteredCollisionMatrix(const Eigen::MatrixXi& filtered, const char* func) const;
    ...
    /* Tests the given collision padding matrix for several invariants, throwing
     if they are not satisfied:
        ...
        - It is symmetric.
        - All values are finite. */
    void ValidatePaddingMatrix(const Eigen::MatrixXd& padding, const char* func) const;
    ...
    /* Checks the same conditions as ValidatePaddingMatrix, but instead of
     ...
     found. */
    std::string CriticizePaddingMatrix(const Eigen::MatrixXd& padding, const char* func) const;
    ...
    const Eigen::VectorXd& GetDefaultConfiguration() const { return default_configuration_; }
    ...
    public:
        ...
        void AllocateOwnedContexts(const CollisionCheckerContext& prototype_context, int num_contexts);
        ...
        CollisionCheckerContext& get_mutable_model_context(int index) const { return *model_contexts_.at(index); }

        const CollisionCheckerContext& get_model_context(int index) const { return *model_contexts_.at(index); }
    ...
    public:
        ...
        /* The copy constructor is used to implement our outer class's Clone(). */
        explicit StandaloneContextReferenceKeeper(const StandaloneContextReferenceKeeper&) {
        ...
        void AddStandaloneContext(const std::shared_ptr<CollisionCheckerContext>& standalone_context) const;
    ...
    private:
        mutable std::list<std::weak_ptr<CollisionCheckerContext>> standalone_contexts_;
    ...
    std::shared_ptr<const DistanceAndInterpolationProvider> distance_and_interpolation_provider_;
