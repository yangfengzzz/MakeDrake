...
template <typename T>
void MidPointIntegrationConstraint::DoEvalGeneric(const Eigen::Ref<const VectorX<T>>& x, VectorX<T>* y) const {
...
void MidPointIntegrationConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    DoEvalGeneric<double>(x, y);
}

void MidPointIntegrationConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    DoEvalGeneric<AutoDiffXd>(x, y);
}

void MidPointIntegrationConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>& x,
