#include "planning/trajectory_optimization/sequential_expression_manager.h"
...
TEST_F(SequentialExpressionManagerTests, RegisterAndSubstituteExpressionTest) {
    MatrixX<Expression> x_sequential = MakeVariableMatrix(num_variables_, num_samples_);
    x_sequential.cwiseSqrt();
    VectorX<Variable> x_placeholder = dut_.RegisterSequentialExpressions(x_sequential.cast<Expression>(), "x");
    EXPECT_EQ(dut_.num_rows("x"), num_variables_);
    MatrixX<Expression> placeholder_expression = x_placeholder * x_placeholder.transpose();
    for (int j = 0; j < num_samples_; ++j) {
        MatrixX<Expression> expected_expression = x_sequential.col(j) * x_sequential.col(j).transpose();
        Substitution subs = dut_.ConstructPlaceholderVariableSubstitution(j);
        EXPECT_EQ(symbolic::Substitute(placeholder_expression, subs), expected_expression);
    }
}

TEST_F(SequentialExpressionManagerTests, RegisterAndSubstituteExpressionTestWithPlaceholders) {
    const MatrixX<Expression> x_sequential{MakeVariableMatrix(num_variables_, num_samples_)};
    x_sequential.cwiseSqrt();
    const VectorX<Variable> x_placeholder{symbolic::MakeVectorContinuousVariable(num_variables_, "x")};
    dut_.RegisterSequentialExpressions(x_placeholder, x_sequential.cast<Expression>(), "x");
    EXPECT_EQ(dut_.num_rows("x"), num_variables_);
    const MatrixX<Expression> placeholder_expression{x_placeholder * x_placeholder.transpose()};
    for (int j = 0; j < num_samples_; ++j) {
        const MatrixX<Expression> expected_expression{x_sequential.col(j) * x_sequential.col(j).transpose()};
...
TEST_F(SequentialExpressionManagerTests, RegisterAndSubstituteMapTest) {
    VectorX<Expression> x_sequential = MakeVariableMatrix(num_variables_ * num_samples_, 1);
    VectorX<Variable> x_placeholder = dut_.RegisterSequentialExpressions(
            Eigen::Map<MatrixX<Expression>>(x_sequential.data(), num_variables_, num_samples_), "x");
    MatrixX<Expression> placeholder_expression = x_placeholder * x_placeholder.transpose();
    for (int j = 0; j < num_samples_; ++j) {
        MatrixX<Expression> expected_expression = x_sequential.segment(j * num_variables_, num_variables_) *
...
TEST_F(SequentialExpressionManagerTests, RegisterAndSubstituteMapCastTest) {
    VectorX<Variable> x_sequential = MakeVariableMatrix(num_variables_ * num_samples_, 1);
    VectorX<Variable> x_placeholder = dut_.RegisterSequentialExpressions(
            Eigen::Map<MatrixX<Variable>>(x_sequential.data(), num_variables_, num_samples_).cast<Expression>(), "x");
    MatrixX<Expression> placeholder_expression = x_placeholder * x_placeholder.transpose();
    for (int j = 0; j < num_samples_; ++j) {
        MatrixX<Expression> expected_expression = x_sequential.segment(j * num_variables_, num_variables_) *
                                                  x_sequential.segment(j * num_variables_, num_variables_).transpose();
...
TEST_F(SequentialExpressionManagerTests, GetSequentialExpressionNamesTest) {
    MatrixX<Variable> x_sequential = MakeVariableMatrix(num_variables_, num_samples_);
    VectorX<Variable> x_placeholder = dut_.RegisterSequentialExpressions(
            Eigen::Map<MatrixX<Variable>>(x_sequential.data(), num_variables_, num_samples_).cast<Expression>(), "x");
...
TEST_F(SequentialExpressionManagerTests, GetVariables) {
    ...
    Variable z("z");
    DRAKE_EXPECT_THROWS_MESSAGE(dut_.GetVariables(Vector1<Variable>{z}, 0),
                                ".*does not appear to be a placeholder variable.*");

    // When the expressions are not variables, it throws.
    MatrixX<Expression> non_variable = Eigen::MatrixXd::Ones(num_variables_, num_samples_);
    VectorX<Variable> nv_placeholder = dut_.RegisterSequentialExpressions(non_variable, "nv");
    DRAKE_EXPECT_THROWS_MESSAGE(dut_.GetVariables(nv_placeholder, 0), ".*is not a variable.*");
}
