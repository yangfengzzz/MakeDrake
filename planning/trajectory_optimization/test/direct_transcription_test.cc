#include "planning/trajectory_optimization/direct_transcription.h"
...
#include "systems/primitives/trajectory_linear_system.h"
...
public:
    explicit CubicPolynomialSystem(double time_step)
        : systems::LeafSystem<T>(systems::SystemTypeTag<trajectory_optimization::CubicPolynomialSystem>{}),
          time_step_(time_step) {
        // Zero inputs, zero outputs.
        this->DeclareDiscreteState(1);  // One state variable.
        this->DeclarePeriodicDiscreteUpdateEvent(time_step, 0.0, &CubicPolynomialSystem<T>::CalcDiscreteUpdate);
    }
...
private:
    ...
    void CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
        using std::pow;
        (*discrete_state)[0] = pow(context.get_discrete_state(0).GetAtIndex(0), 3.0);
    }
...
public:
    LinearSystemWParams()
        : systems::LeafSystem<T>(systems::SystemTypeTag<trajectory_optimization::LinearSystemWParams>{}) {
        // Zero inputs, zero outputs.
        this->DeclareDiscreteState(1);                     // One state variable.
        this->DeclareNumericParameter(BasicVector<T>(1));  // One parameter.
        this->DeclarePeriodicDiscreteUpdateEvent(1.0, 0.0, &LinearSystemWParams<T>::CalcDiscreteUpdate);
    }

    // Scalar-converting copy constructor.
    template <typename U>
    explicit LinearSystemWParams(const LinearSystemWParams<U>& system) : LinearSystemWParams() {}

private:
    // x[n+1] = p0 * x[n]
    void CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
        (*discrete_state)[0] =
                context.get_numeric_parameter(0).GetAtIndex(0) * context.get_discrete_state(0).GetAtIndex(0);
    }
};
...
GTEST_TEST(DirectTranscriptionTest, DiscreteTimeConstraintTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DirectTranscription(&system, *context, kNumSampleTimes, TimeStep(kTimeStep)),
    ...
    prog.SetInitialGuessForAllVariables(Eigen::VectorXd::LinSpaced(prog.num_vars(), 1, prog.num_vars()));
    ...
    const std::vector<solvers::Binding<solvers::Constraint>>& dynamic_constraints = prog.generic_constraints();
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        EXPECT_EQ(prog.EvalBindingAtInitialGuess(dynamic_constraints[i])[0],
                  prog.GetInitialGuess(dirtran.state(i + 1)[0]) - pow(prog.GetInitialGuess(dirtran.state(i)[0]), 3.0));
    }
}
...
GTEST_TEST(DirectTranscriptionTest, ContinuousTimeConstraintTest) {
    ...
    using std::sin;
    Variable x{"x"};
    SymbolicVectorSystem<double> system({}, Vector1<Variable>{x}, Vector0<Variable>{}, Vector1<Expression>{sin(x)},
                                        Vector0<Expression>{});
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DirectTranscription(&system, *context, kNumSampleTimes),
                                ".*is for discrete-time systems.*");

    DirectTranscription dirtran(&system, *context, kNumSampleTimes, TimeStep{kTimeStep});
    auto& prog = dirtran.prog();
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        ...
        EXPECT_NEAR(prog.EvalBindingAtInitialGuess(dynamic_constraints[i])[0],
                    prog.GetInitialGuess(dirtran.state(i + 1)[0]) - state_i - kTimeStep * sin(state_i), 1e-14);
    }
}
...
GTEST_TEST(DirectTranscriptionTest, DiscreteTimeSymbolicConstraintTest) {
    ...
    const std::vector<solvers::Binding<solvers::LinearEqualityConstraint>>& dynamic_constraints =
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        const Vector1d dynamic_constraint_val = prog.EvalBindingAtInitialGuess(dynamic_constraints[i]) -
                                                dynamic_constraints[i].evaluator()->lower_bound();
        const Vector1d dynamic_constraint_expected =
                prog.GetInitialGuess(dirtran.state(i + 1)) - system->A() * prog.GetInitialGuess(dirtran.state(i)) -
        ...
        EXPECT_TRUE(CompareMatrices(dynamic_constraint_val, dynamic_constraint_expected) ||
...
GTEST_TEST(DirectTranscriptionTest, ContinuousTimeSymbolicConstraintTest) {
    ...
    int kNumSampleTimes = 3;
    const double kTimeStep = 0.1;
    DirectTranscription dirtran(system.get(), *context, kNumSampleTimes, TimeStep{kTimeStep});
    auto& prog = dirtran.prog();
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        const Vector1d dynamic_constraint_val = prog.EvalBindingAtInitialGuess(dynamic_constraints[i]) -
                                                dynamic_constraints[i].evaluator()->lower_bound();
        const Vector1d dynamic_constraint_expected =
                prog.GetInitialGuess(dirtran.state(i + 1)) - prog.GetInitialGuess(dirtran.state(i)) -
                kTimeStep * system->A() * prog.GetInitialGuess(dirtran.state(i)) -
                kTimeStep * system->B() * prog.GetInitialGuess(dirtran.input(i)) - kTimeStep * system->f0();

        // Check that the system's state equation still holds with equality,
        // regardless of the specific encoding MathematicalProgram chooses.
        EXPECT_TRUE(CompareMatrices(dynamic_constraint_val, dynamic_constraint_expected, 1e-14));
    }
}
...
void SolvePendulumTrajectory(bool continuous_time) {
    ...
    if (!(snopt_solver.is_available() && snopt_solver.is_enabled())) {
        drake::log()->warn("SNOPT is unavailable or disabled; this test shall be skipped.");
        return;
    }
    ...
    auto pendulum = std::make_unique<multibody::MultibodyPlant<double>>(kTimeStep);
    ...
    const InputPortIndex actuation_port_index = pendulum->get_actuation_input_port().get_index();
    const int kNumTimeSamples = 50;
    std::unique_ptr<DirectTranscription> dirtran{};
    if (continuous_time) {
        dirtran = std::make_unique<DirectTranscription>(pendulum.get(), *context, kNumTimeSamples,
                                                        TimeStep(kFixedTimeStep), actuation_port_index);
    } else {
        dirtran =
                std::make_unique<DirectTranscription>(pendulum.get(), *context, kNumTimeSamples, actuation_port_index);
    }
    auto& prog = dirtran->prog();
    ...
    prog.AddLinearConstraint(dirtran->initial_state() == initial_state.get_value());
...
GTEST_TEST(DirectTranscriptionTest, DiscreteTimeLinearSystemTest) {
  ...
  B << 5, 6,
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        EXPECT_TRUE(CompareMatrices(prog.EvalBindingAtInitialGuess(dynamic_constraints[i]),
...
GTEST_TEST(DirectTranscriptionTest, AddRunningCostTest) {
    ...
    for (int i{0}; i < kNumSamples - 1; i++) {
        expected_cost += kTimeStep * std::pow(result.GetSolution(dirtran.state(i))[0], 2.);
    }
...
GTEST_TEST(DirectTranscriptionTest, LinearSystemWParamsTest) {
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        ...
        EXPECT_EQ(prog.EvalBindingAtInitialGuess(dynamic_constraints[i])[0],
                  prog.GetInitialGuess(dirtran.state(i + 1)[0]) - kGain * prog.GetInitialGuess(dirtran.state(i)[0]));
    }
}
