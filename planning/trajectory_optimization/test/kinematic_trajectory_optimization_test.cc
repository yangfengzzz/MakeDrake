...
public:
    ...
    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const override { *y = x.transpose() * x; }
...
TEST_F(KinematicTrajectoryOptimizationTest, AddPathPositionConstraintGeneric) {
    ...
    auto binding = trajopt_.AddPathPositionConstraint(std::make_shared<SimplePositionConstraint>(), 0.2);
    EXPECT_THAT(binding.to_string(), HasSubstr("path position constraint"));
    ...
    trajopt_.SetInitialGuess(BsplineTrajectory(
            trajopt_.basis(), math::EigenToStdVector<double>(MatrixXd::Ones(3, trajopt_.num_control_points()))));
    auto result = Solve(trajopt_.prog());
    EXPECT_TRUE(result.is_success());
    double squared_norm = trajopt_.ReconstructTrajectory(result).value(0.2).squaredNorm();
...
TEST_F(KinematicTrajectoryOptimizationTest, AddVelocityConstraintAtNormalizedTime) {
    EXPECT_EQ(trajopt_.prog().generic_constraints().size(), 0);
    Vector6d x_desired;
    ...
    auto binding = trajopt_.AddVelocityConstraintAtNormalizedTime(
            std::make_shared<solvers::BoundingBoxConstraint>(x_desired, x_desired), 0.2);
...
TEST_F(KinematicTrajectoryOptimizationTest, AddPositionBounds) {
    ...
    for (int i = 0; i < num_control_points_; ++i) {
        trajopt_.get_mutable_prog().AddQuadraticErrorCost(
                Eigen::MatrixXd::Identity(trajopt_.num_positions(), trajopt_.num_positions()),
                Eigen::VectorXd::Constant(trajopt_.num_positions(), 2.0), trajopt_.control_points().col(i));
    }
    ...
    auto binding = trajopt_.AddPositionBounds(-VectorXd::Ones(num_positions_), VectorXd::Ones(num_positions_));
    EXPECT_THAT(binding[0].to_string(), HasSubstr("position bound"));

    EXPECT_EQ(trajopt_.prog().bounding_box_constraints().size(), trajopt_.num_control_points() + 2);
...
TEST_F(KinematicTrajectoryOptimizationTest, AddVelocityBounds) {
    ...
    trajopt_.get_mutable_prog().AddQuadraticErrorCost(
            Eigen::MatrixXd::Identity(trajopt_.num_positions(), trajopt_.num_positions()),
            Eigen::VectorXd::Constant(trajopt_.num_positions(), 10.0), trajopt_.control_points().col(0));
    for (int i = 1; i < trajopt_.num_control_points(); ++i) {
        trajopt_.get_mutable_prog().AddQuadraticErrorCost(
                Eigen::MatrixXd::Identity(trajopt_.num_positions(), trajopt_.num_positions()),
                Eigen::VectorXd::Constant(trajopt_.num_positions(), -10.0), trajopt_.control_points().col(i));
    }
    trajopt_.AddDurationConstraint(1, 1);
    ...
    EXPECT_EQ(trajopt_.prog().linear_constraints().size(), 0);
    auto binding = trajopt_.AddVelocityBounds(-VectorXd::Ones(num_positions_), VectorXd::Ones(num_positions_));
    EXPECT_THAT(binding[0].to_string(), HasSubstr("velocity bound"));
    EXPECT_EQ(trajopt_.prog().linear_constraints().size(), trajopt_.num_control_points() - 1);
...
TEST_F(KinematicTrajectoryOptimizationTest, AddAccelerationBounds) {
    ...
    trajopt_.AddPathPositionConstraint(Vector3d::Constant(10), Vector3d::Constant(10), 0);
    trajopt_.AddPathVelocityConstraint(Vector3d::Zero(), Vector3d::Zero(), 0);
    for (int i = 1; i < trajopt_.num_control_points(); ++i) {
        trajopt_.get_mutable_prog().AddQuadraticErrorCost(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Constant(-10.0),
    ...
    EXPECT_EQ(trajopt_.prog().generic_constraints().size(), 0);
    auto binding = trajopt_.AddAccelerationBounds(-Vector3d::Ones(), Vector3d::Ones());
...
TEST_F(KinematicTrajectoryOptimizationTest, AddJerkBounds) {
    ...
    trajopt_.get_mutable_prog().SetSolverOption(solvers::IpoptSolver::id(), "tol", 1e-6);
...
TEST_F(KinematicTrajectoryOptimizationTest, AddPathLengthCost) {
    ...
    EXPECT_EQ(trajopt_.prog().l2norm_costs().size(), trajopt_.num_control_points() - 1);

    MatrixXd guess(3, trajopt_.num_control_points());
    guess.row(0) = Eigen::RowVectorXd::LinSpaced(trajopt_.num_control_points(), 0, 1);
    guess.row(1) = guess.row(0);
    guess.row(2) = guess.row(0);

    trajopt_.SetInitialGuess(BsplineTrajectory(trajopt_.basis(), math::EigenToStdVector<double>(guess)));
    auto result = Solve(trajopt_.prog());
    EXPECT_TRUE(result.is_success());
...
TEST_F(KinematicTrajectoryOptimizationTest, AddPathLengthCostConic) {
    ...
    EXPECT_THAT(binding[0].to_string(), HasSubstr("path length cost"));
    EXPECT_EQ(trajopt_.prog().linear_costs().size(), trajopt_.num_control_points() - 1);
    EXPECT_EQ(trajopt_.prog().lorentz_cone_constraints().size(), trajopt_.num_control_points() - 1);
    EXPECT_EQ(trajopt_.prog().l2norm_costs().size(), 0);
...
TEST_F(KinematicTrajectoryOptimizationTest, AddPathEnergyCost) {
    ...
    EXPECT_EQ(trajopt_.prog().quadratic_costs().size(), trajopt_.num_control_points() - 1);
...
TEST_F(KinematicTrajectoryOptimizationTest, SolveWithAlmostEverything) {
    trajopt_.AddPathPositionConstraint(VectorXd::Constant(num_positions_, 0.4), VectorXd::Constant(num_positions_, 0.4),
                                       0.5);
    trajopt_.AddPositionBounds(-VectorXd::Ones(num_positions_), VectorXd::Ones(num_positions_));
    trajopt_.AddVelocityBounds(-VectorXd::Ones(num_positions_), VectorXd::Ones(num_positions_));
    trajopt_.AddDurationConstraint(0.1, 10);
    trajopt_.AddPathPositionConstraint(std::make_shared<SimplePositionConstraint>(), 0.2);
