#include "planning/trajectory_optimization/gcs_trajectory_optimization.h"
...
#include "geometry/optimization/geodesic_convexity.h"
#include "geometry/optimization/graph_of_convex_sets.h"
#include "geometry/optimization/hpolyhedron.h"
#include "geometry/optimization/hyperrectangle.h"
#include "geometry/optimization/intersection.h"
#include "geometry/optimization/point.h"
#include "geometry/optimization/vpolytope.h"
...
bool GurobiOrMosekSolverUnavailableDuringMemoryCheck() {
    return (std::getenv("VALGRIND_OPTS") != nullptr) &&
           !(solvers::MosekSolver::is_available() && solvers::MosekSolver::is_enabled() &&
             solvers::GurobiSolver::is_available() && solvers::GurobiSolver::is_enabled());
}

bool SnoptSolverUnavailable() {
    return !(solvers::SnoptSolver::is_available() && solvers::SnoptSolver::is_enabled());
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, Basic) {
    ...
    auto& regions = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 1, kMinimumDuration);
    ...
    EXPECT_NE(std::find(all_subgraphs.begin(), all_subgraphs.end(), &regions), all_subgraphs.end());
    EXPECT_NE(std::find(all_subgraphs.begin(), all_subgraphs.end(), &source), all_subgraphs.end());
    EXPECT_NE(std::find(all_subgraphs.begin(), all_subgraphs.end(), &target), all_subgraphs.end());
    ...
    {  // Confirm that the const accessor returns the same edges.
        ...
        EXPECT_THAT(const_edges.Edges(), testing::ElementsAreArray(source_to_regions.Edges()));
    }
    ...
    EXPECT_NE(std::find(all_subgraph_edges.begin(), all_subgraph_edges.end(), &source_to_regions),
              all_subgraph_edges.end());
    EXPECT_NE(std::find(all_subgraph_edges.begin(), all_subgraph_edges.end(), &regions_to_target),
              all_subgraph_edges.end());
    ...
    EXPECT_TRUE(CompareMatrices(traj.value(traj.start_time()), start, kTolerance));
    ...
    EXPECT_TRUE(CompareMatrices(new_traj.value(new_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(new_traj.value(new_traj.end_time()), new_goal, kTolerance));
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, PathLengthCost) {
    ...
    // Add the unit box.
    const int kOrder = 2;
    auto& regions = trajopt.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), kOrder);
    ...
    EXPECT_NEAR(prog.EvalBindingAtInitialGuess(v->GetCosts()[0])[0], (x.segment(2, 2) - x.segment(0, 2)).norm(), 1e-12);
    EXPECT_NEAR(prog.EvalBindingAtInitialGuess(v->GetCosts()[1])[0], (x.segment(4, 2) - x.segment(2, 2)).norm(), 1e-12);
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, MinimumTimeVsPathLength) {
    ...
    */
    ...
    auto& walking_regions = gcs.AddRegions(
            MakeConvexSets(HPolyhedron::MakeBox(Vector2d(-0.5, -0.5), Vector2d(0.5, 0.5))), 3, 0, 20, "walking");
    // Bob can walk at 1 m/s in x and y.
    walking_regions.AddVelocityBounds(Vector2d(-kWalkingSpeed, -kWalkingSpeed), Vector2d(kWalkingSpeed, kWalkingSpeed));

    auto& scooter_regions = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(-0.5, -0.5), Vector2d(-0.2, 1)),
    ...
    {  // Confirm that the const accessor returns the same vertices.
        ...
        EXPECT_THAT(const_scooter_regions.Vertices(), testing::ElementsAreArray(scooter_regions.Vertices()));
    }

    // Bob can ride an e-scooter at 10 m/s in x and y.
    scooter_regions.AddVelocityBounds(Vector2d(-kScooterSpeed, -kScooterSpeed), Vector2d(kScooterSpeed, kScooterSpeed));
    ...
    {  // Confirm that the const accessor returns the same vertices.
        ...
        EXPECT_THAT(const_edges.Edges(), testing::ElementsAreArray(source_scooter.Edges()));
    }
    ...
    auto [shortest_path_traj, shortest_path_result] = gcs.SolvePath(source, target);
    ASSERT_TRUE(shortest_path_result.is_success());
    EXPECT_EQ(shortest_path_traj.rows(), 2);
    EXPECT_EQ(shortest_path_traj.cols(), 1);
    EXPECT_TRUE(CompareMatrices(shortest_path_traj.value(shortest_path_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(shortest_path_traj.value(shortest_path_traj.end_time()), goal, kTolerance));
    ...
    for (double t = shortest_path_traj.start_time(); t < shortest_path_traj.end_time(); t += dt) {
        const double t_next = std::min(t + dt, shortest_path_traj.end_time());
        const double dx = (shortest_path_traj.value(t_next) - shortest_path_traj.value(t)).norm();
        shortest_path_length += dx;
    }
    EXPECT_NEAR(shortest_path_length, kExpectedShortestPathLength, kTolerance);

    const double shortest_path_duration = shortest_path_traj.end_time() - shortest_path_traj.start_time();
    ...
    std::vector<const geometry::optimization::GraphOfConvexSets::Vertex*> active_vertices;
    std::copy(source_vertices.begin(), source_vertices.end(), std::back_inserter(active_vertices));
    std::copy(scooter_vertices.begin(), scooter_vertices.end(), std::back_inserter(active_vertices));
    std::copy(target_vertices.begin(), target_vertices.end(), std::back_inserter(active_vertices));

    auto [detour_traj, detour_result] = gcs.SolveConvexRestriction(active_vertices);
    EXPECT_TRUE(detour_result.is_success());
    EXPECT_EQ(detour_traj.rows(), 2);
    EXPECT_EQ(detour_traj.cols(), 1);
    EXPECT_TRUE(CompareMatrices(detour_traj.value(detour_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(detour_traj.value(detour_traj.end_time()), goal, kTolerance));

    // The detour should be longer than the shortest path.
    double detour_path_length = 0.0;
    for (double t = detour_traj.start_time(); t < detour_traj.end_time(); t += dt) {
    ...
    EXPECT_TRUE(CompareMatrices(fastest_path_traj.value(fastest_path_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(fastest_path_traj.value(fastest_path_traj.end_time()), goal, kTolerance));
    ...
    for (double t = fastest_path_traj.start_time(); t < fastest_path_traj.end_time(); t += dt) {
        const double t_next = std::min(t + dt, fastest_path_traj.end_time());
        const double dx = (fastest_path_traj.value(t_next) - fastest_path_traj.value(t)).norm();

        // We also expect the fastest path to consistently hit either of the
        // velocity bounds.
        EXPECT_TRUE((fastest_path_velocity->value(t).cwiseAbs() - Vector2d(kScooterSpeed, kScooterSpeed))
                            .cwiseAbs()
                            .minCoeff() < kTolerance);
    ...
    const double fastest_path_duration = fastest_path_traj.end_time() - fastest_path_traj.start_time();
    ...
    EXPECT_TRUE(CompareMatrices(rain_path_traj.value(rain_path_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(rain_path_traj.value(rain_path_traj.end_time()), goal, kTolerance));

    // Since the scooter regions have been removed, Bob can only walk through the
    // through the gravel.
    std::vector<const geometry::optimization::GraphOfConvexSets::Edge*> expected_walking_path_edges = {
            walking_regions.Vertices()[0]->incoming_edges()[0], walking_regions.Vertices()[0]->outgoing_edges()[0]};

    std::vector<const geometry::optimization::GraphOfConvexSets::Edge*> rain_path_edges =
            gcs.graph_of_convex_sets().GetSolutionPath(*source.Vertices()[0], *target.Vertices()[0], rain_path_result,
                                                       1.0);
...
GTEST_TEST(GcsTrajectoryOptimizationTest, DerivativeBoundsOnEdges) {
  ...
  */
    ...
    auto& race_track_1 =
            gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(0, -5), Vector2d(305, 5))), 6, 0.0, 500.0);
    auto& race_track_2 =
            gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(0, -5), Vector2d(305, 5))), 6, 0.0, 500.0);
    ...
    // seconds.
    auto& ducks = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(75, -5), Vector2d(75, 5))), 0, kDuckDelay,
                                 kDuckDelay + 10);
    ...
    // maximum.
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(traj.start_time(), 1), Vector2d(0, 0), kTolerance));
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(traj.end_time(), 1), Vector2d(kMaxSpeed, 0), kTolerance));

    // We also said the car starts with zero acceleration.
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(traj.start_time(), 2), Vector2d(0, 0), kTolerance));
    ...
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(stopped_at_ducks_time, 1), Vector2d(0, 0), kTolerance));
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(stopped_at_ducks_time, 2), Vector2d(0, 0), kTolerance));

    EXPECT_TRUE(
            CompareMatrices(traj.EvalDerivative(stopped_at_ducks_time + kDuckDelay, 1), Vector2d(0, 0), kTolerance));
    EXPECT_TRUE(
            CompareMatrices(traj.EvalDerivative(stopped_at_ducks_time + kDuckDelay, 2), Vector2d(0, 0), kTolerance));
    ...
    race_track_1.AddNonlinearDerivativeBounds(Vector2d(-kMaxAcceleration, 0), Vector2d(kMaxAcceleration, 0), 2);
    race_track_2.AddNonlinearDerivativeBounds(Vector2d(-kMaxAcceleration, 0), Vector2d(kMaxAcceleration, 0), 2);
    ...
    EXPECT_TRUE(CompareMatrices(traj_truck.value(traj_truck.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(traj_truck.value(traj_truck.end_time()), goal, kTolerance));

    // Let's make sure the truck remained within the acceleration bounds.
    for (double t = traj_truck.start_time(); t < traj_truck.end_time(); t += kTimeStep) {
        EXPECT_TRUE(traj_truck.EvalDerivative(t, 2).cwiseAbs()(0) <= kMaxAcceleration + kTolerance);
    }

    // The truck should have taken longer to reach the finish line.
    EXPECT_GT(traj_truck.end_time() - traj_truck.start_time(), traj.end_time() - traj.start_time());

    // Ensure the race car was indeed accelerating faster than the truck.
    bool racecar_accelerating_faster = false;
    for (double t = traj.start_time(); t < traj.end_time(); t += kTimeStep) {
        if (traj.EvalDerivative(t, 2).cwiseAbs()(0) > kMaxAcceleration + kTolerance) {
...
GTEST_TEST(GcsTrajectoryOptimizationTest, RemoveSubgraph) {
    ...
    auto& graph1 = gcs.AddRegions(
            MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension), HPolyhedron::MakeUnitBox(kDimension)), 1);
    auto& graph2 = gcs.AddRegions(
            MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension), HPolyhedron::MakeUnitBox(kDimension)), 1);
    ...
    auto& graph3 = gcs2.AddRegions(
            MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension), HPolyhedron::MakeUnitBox(kDimension)), 1);
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.RemoveSubgraph(graph3), ".* is not registered with `this`.*");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, InvalidPositions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GcsTrajectoryOptimization(0), ".*num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(GcsTrajectoryOptimization(-1), ".*num_positions.*");
...
GTEST_TEST(GcsTrajectoryOptimizationTest, InvalidOrder) {
    ...
    GcsTrajectoryOptimization gcs(kDimension);
    EXPECT_EQ(gcs.num_positions(), kDimension);
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), -1), ".*order.*");

    DRAKE_EXPECT_NO_THROW(gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 0));
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, ZeroOrderPathLengthCost) {
    ...
    auto& regions = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 0);

    DRAKE_EXPECT_THROWS_MESSAGE(regions.AddPathLengthCost(), "Path length cost is not defined for a set of order 0.");
...
GTEST_TEST(GcsTrajectoryOptimizationTest, InvalidDerivativeBounds) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 1),
                                "Use AddVelocityBounds instead.*");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 0),
                                "Derivative order must be greater than 1.");

    auto& regions1 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 0);

    auto& regions2 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 0);

    auto& regions3 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 3);

    auto& regions1_to_regions2 = gcs.AddEdges(regions1, regions2);

    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddVelocityBounds(Vector2d::Zero(), Vector2d::Zero()),
    ...
    DRAKE_EXPECT_NO_THROW(gcs.AddVelocityBounds(Vector2d::Zero(), Vector2d::Zero()));

    // Lower and upper bound must have the same dimension as the graph.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddVelocityBounds(Vector3d::Zero(), Vector2d::Zero()),
                                ".*size.*.num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddVelocityBounds(Vector2d::Zero(), Vector3d::Zero()), ".*size.*.num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddVelocityBounds(Vector2d::Zero(), Vector3d::Zero()),
                                ".*size.*.num_positions.*");

    // Can't add velocity bounds to an edge if both regions have order 0.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddVelocityBounds(Vector2d::Zero(), Vector2d::Zero()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddZeroDerivativeConstraints(0),
                                "Derivative order must be greater than 1.");

    // Can't enforce zero derivatives to an edge if both regions have order 0.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddZeroDerivativeConstraints(1),
                                "Cannot add derivative bounds to subgraph edges where both subgraphs "
                                "have less than derivative order.\n From subgraph order: 0\n To subgraph "
                                "order: 0\n Derivative order: 1");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddZeroDerivativeConstraints(2),
                                "Cannot add derivative bounds to subgraph edges where both subgraphs "
                                "have less than derivative order.\n From subgraph order: 0\n To subgraph "
                                "order: 0\n Derivative order: 2");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 1),
                                "Use AddVelocityBounds instead.*");
    DRAKE_EXPECT_THROWS_MESSAGE(regions3.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 1),
                                "Use AddVelocityBounds instead.*");
    DRAKE_EXPECT_THROWS_MESSAGE(
            regions1_to_regions2.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 1),
            "Use AddVelocityBounds instead.*");

    // The zeroth order derivative is not a proper derivative.
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 0),
                                "Derivative order must be greater than 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions3.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 0),
                                "Derivative order must be greater than 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(
            regions1_to_regions2.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 0),
            "Derivative order must be greater than 1.");

    // Lower and upper bound must have the same dimension as the graph.
    DRAKE_EXPECT_THROWS_MESSAGE(regions3.AddNonlinearDerivativeBounds(Vector3d::Zero(), Vector2d::Zero(), 2),
                                ".*size.*.num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(
            regions1_to_regions2.AddNonlinearDerivativeBounds(Vector3d::Zero(), Vector2d::Zero(), 2),
            ".*size.*.num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector3d::Zero(), 2),
                                ".*size.*.num_positions.*");

    // Can't add derivative bounds to a subgraph with an order lower than the
    // derivative order.
    DRAKE_EXPECT_THROWS_MESSAGE(regions3.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 4),
                                "Derivative order must be less than or equal to the set order.");

    DRAKE_EXPECT_THROWS_MESSAGE(
            regions1_to_regions2.AddNonlinearDerivativeBounds(Vector2d::Zero(), Vector2d::Zero(), 2),
            "Cannot add derivative bounds to subgraph edges where both subgraphs "
            "have less than derivative order.\n From subgraph order: 0\n To subgraph "
            "order: 0\n Derivative order: 2");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, InvalidContinuityConstraints) {
    ...
    auto& regions1 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 1);

    auto& regions2 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 1);

    auto& regions1_to_regions2 = gcs.AddEdges(regions1, regions2);

    // Zero order path continuity should throw an error, since its support by
    // default.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddPathContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddPathContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddPathContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");

    // Zero order path continuity should throw an error, since it's enforced by
    // default.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddContinuityConstraints(0),
                                "Path continuity is enforced by default. Choose a higher order.");

    // Negative continuity should be rejected as well.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddPathContinuityConstraints(-1), "Order must be greater than or equal to 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddPathContinuityConstraints(-1),
                                "Order must be greater than or equal to 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddPathContinuityConstraints(-1), "Order must be greater than or equal to 1.");

    // Negative continuity should be rejected as well.
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddContinuityConstraints(-1), "Order must be greater than or equal to 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddContinuityConstraints(-1),
                                "Order must be greater than or equal to 1.");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddContinuityConstraints(-1), "Order must be greater than or equal to 1.");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddPathContinuityConstraints(2),
                                "Cannot add continuity constraint of order greater than the set "
                                "order.");
    DRAKE_EXPECT_THROWS_MESSAGE(regions1.AddContinuityConstraints(2),
                                "Cannot add continuity constraint of order greater than the set "
                                "order.");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddPathContinuityConstraints(2),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(regions1_to_regions2.AddContinuityConstraints(2),
                                "Cannot add continuity constraint to a subgraph edge where both "
                                "subgraphs order are not greater than or equal to the requested "
                                "continuity order.");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, DisjointGraph) {
    ...
    auto& source = gcs.AddRegions(MakeConvexSets(Point(start1), Point(start2)), 0);
...
GTEST_TEST(GcsTrajectoryOptimizationTest, DisconnectedPathInConvexRestriction) {
    ...
    auto& source = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(0, 0), Vector2d(1, 1)),
                                                 HPolyhedron::MakeBox(Vector2d(0.5, 0.5), Vector2d(2, 2))),
                                  0);

    auto& target = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeBox(Vector2d(3, 3), Vector2d(4, 4)),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.SolveConvexRestriction(active_vertices), ".*is not connected to vertex.*");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, MultipleEdgesInConvexRestriction) {
    ...
    */
    ...
    auto& graph1 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 1, kMinimumDuration);
    auto& graph2 = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(kDimension)), 1, kMinimumDuration);
    ...
    std::vector<const geometry::optimization::GraphOfConvexSets::Vertex*> active_vertices;
    std::copy(source_vertices.begin(), source_vertices.end(), std::back_inserter(active_vertices));
    std::copy(graph1_vertices.begin(), graph1_vertices.end(), std::back_inserter(active_vertices));
    std::copy(graph2_vertices.begin(), graph2_vertices.end(), std::back_inserter(active_vertices));
    std::copy(target_vertices.begin(), target_vertices.end(), std::back_inserter(active_vertices));

    // Since the vertex in graph1 and graph2 are connected by multiple edges, we
    // expect the convex restriction to throw an error.
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.SolveConvexRestriction(active_vertices), ".*through multiple edges.*");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, InvalidSubspace) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddEdges(regions1, regions2, &subspace_wrong_dimension),
...
GTEST_TEST(GcsTrajectoryOptimizationTest, UnwrapToContinousTrajectory) {
    ...
    Eigen::MatrixXd control_points_1(3, 3), control_points_2(3, 3), control_points_3(3, 3);
    // clang-format off
  ...
  control_points_3 << 4.0, -2.0, 0.0,
                      ...
                      2.0, 3.0, 4.0;
    // clang-format on
    segments.emplace_back(std::make_unique<trajectories::BezierCurve<double>>(0.0, 1.0, control_points_1));
    segments.emplace_back(std::make_unique<trajectories::BezierCurve<double>>(1.0, 4.0, control_points_2));
    segments.emplace_back(std::make_unique<trajectories::BezierCurve<double>>(4.0, 6.0, control_points_3));
    ...
    const auto unwrapped_traj =
            GcsTrajectoryOptimization::UnwrapToContinousTrajectory(traj, continuous_revolute_joints);
    ...
    EXPECT_TRUE(CompareMatrices(unwrapped_traj.value(middle_time_1 - time_eps),
                                unwrapped_traj.value(middle_time_1 + time_eps), pos_eps));
    EXPECT_TRUE(CompareMatrices(unwrapped_traj.value(middle_time_2 - time_eps),
                                unwrapped_traj.value(middle_time_2 + time_eps), pos_eps));
    std::vector<int> starting_rounds = {-1, 0};
    const auto unwrapped_traj_with_start = GcsTrajectoryOptimization::UnwrapToContinousTrajectory(
            traj, continuous_revolute_joints, starting_rounds, 1e-8);
    // Check if the start is unwrapped to the correct value.
    EXPECT_TRUE(
            CompareMatrices(unwrapped_traj_with_start.value(0.0), Eigen::Vector3d{0.0, 1.0 - 2 * M_PI, 2.0}, pos_eps));
    EXPECT_TRUE(CompareMatrices(unwrapped_traj_with_start.value(middle_time_1 - time_eps),
                                unwrapped_traj_with_start.value(middle_time_1 + time_eps), pos_eps));
    EXPECT_TRUE(CompareMatrices(unwrapped_traj_with_start.value(middle_time_2 - time_eps),
    ...
    EXPECT_THROW(GcsTrajectoryOptimization::UnwrapToContinousTrajectory(traj, continuous_revolute_joints,
                                                                        invalid_start_rounds, 1e-8),
                 std::runtime_error);
    // Check for discontinuity for continuous revolute joints
    control_points_2 << 2.5, 2.5, 4.0, 3.0 + 2 * M_PI, 1.0 + 2 * M_PI, 0.0 + 2 * M_PI, 4.0 - 6 * M_PI, 3.0 - 6 * M_PI,
            2.0 - 6 * M_PI;
    segments[1] = std::make_unique<trajectories::BezierCurve<double>>(1.0, 4.0, control_points_2);
    const auto traj_not_continous_on_revolute_manifold = trajectories::CompositeTrajectory<double>(segments);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GcsTrajectoryOptimization::UnwrapToContinousTrajectory(
    ...
    EXPECT_NO_THROW(GcsTrajectoryOptimization::UnwrapToContinousTrajectory(
            traj_not_continous_on_revolute_manifold, continuous_revolute_joints, std::nullopt, loose_tol));
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, NotBezierCurveError) {
    ...
    auto traj_1 = trajectories::PiecewisePolynomial<double>::FirstOrderHold(
            ...
            std::vector<Eigen::MatrixXd>{Eigen::MatrixXd::Zero(1, 2), Eigen::MatrixXd::Ones(1, 2)});
...
protected:
    ...
    vertices_12 << 5.0, 5.0, 4.4, 4.4,
        ...
        regions_ = MakeConvexSets(region_1, region_2, region_3, region_4, region_5, region_6, region_7, region_8,
...
TEST_F(SimpleEnv2D, GlobalPathContinuityConstraints) {
    ...
    EXPECT_TRUE(CompareMatrices(normalized_traj.value(normalized_traj.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(normalized_traj.value(normalized_traj.end_time()), goal, kTolerance));
    EXPECT_EQ(normalized_traj.get_number_of_segments(), traj.get_number_of_segments());
    ...
    EXPECT_EQ(normalized_traj.get_number_of_segments(), normalized_traj.end_time() - normalized_traj.start_time());
    ...
    for (int i = 0; i < normalized_traj.get_number_of_segments() - 1; ++i) {
        ...
        auto normalized_next_segment_vel = normalized_traj.segment(i + 1).MakeDerivative();
        EXPECT_TRUE(CompareMatrices(normalized_segment_vel->value(normalized_segment_vel->end_time()),
                                    normalized_next_segment_vel->value(normalized_next_segment_vel->start_time()),
                                    kTolerance));

        auto normalized_segment_acc = normalized_traj.segment(i).MakeDerivative(2);
        auto normalized_next_segment_acc = normalized_traj.segment(i + 1).MakeDerivative(2);
        EXPECT_TRUE(CompareMatrices(normalized_segment_acc->value(normalized_segment_acc->end_time()),
                                    normalized_next_segment_acc->value(normalized_next_segment_acc->start_time()),
                                    kTolerance));
    }
}
...
TEST_F(SimpleEnv2D, GlobalContinuityConstraints) {
    ...
    for (int i = 0; i < traj.get_number_of_segments() - 1; ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(segment_vel->value(segment_vel->end_time()),
        ...
        EXPECT_TRUE(CompareMatrices(segment_acc->value(segment_acc->end_time()),
...
TEST_F(SimpleEnv2D, DerivativeConstraints) {
    ...
    // Add velocity bounds.
    gcs.AddVelocityBounds(Vector2d(-kMaxSpeed, -kMaxSpeed), Vector2d(kMaxSpeed, kMaxSpeed));
    ...
    auto [traj_velocity_bounded, result_velocity_bounded] = gcs.SolvePath(source, target, options);

    EXPECT_TRUE(result_velocity_bounded.is_success());
    EXPECT_TRUE(CompareMatrices(traj_velocity_bounded.value(traj_velocity_bounded.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(traj_velocity_bounded.value(traj_velocity_bounded.end_time()), goal, kTolerance));

    bool exceeded_acceleration_bounds = false;
    bool exceeded_jerk_bounds = false;
    double dt = (traj_velocity_bounded.end_time() - traj_velocity_bounded.start_time()) / kNumSamples;
    for (double t = traj_velocity_bounded.start_time(); t < traj_velocity_bounded.end_time(); t += dt) {
        EXPECT_LT(traj_velocity_bounded.EvalDerivative(t, 1).cwiseAbs().maxCoeff(), kMaxSpeed + kTolerance);
        if (traj_velocity_bounded.EvalDerivative(t, 2).cwiseAbs().maxCoeff() > kMaxAcceleration + kTolerance) {
            exceeded_acceleration_bounds = true;
        }
        if (traj_velocity_bounded.EvalDerivative(t, 3).cwiseAbs().maxCoeff() > kMaxJerk + kTolerance) {
    ...
    gcs.AddNonlinearDerivativeBounds(Vector2d(-kMaxAcceleration, -kMaxAcceleration),
                                     Vector2d(kMaxAcceleration, kMaxAcceleration), 2);
    gcs.AddNonlinearDerivativeBounds(Vector2d(-kMaxJerk, -kMaxJerk), Vector2d(kMaxJerk, kMaxJerk), 3);
    ...
    for (double t = traj.start_time(); t < traj.end_time(); t += dt) {
        EXPECT_LT(traj.EvalDerivative(t, 1).cwiseAbs().maxCoeff(), kMaxSpeed + kTolerance);
        EXPECT_LT(traj.EvalDerivative(t, 2).cwiseAbs().maxCoeff(), kMaxAcceleration + kTolerance);
        EXPECT_LT(traj.EvalDerivative(t, 3).cwiseAbs().maxCoeff(), kMaxJerk + kTolerance);
    }
}
...
TEST_F(SimpleEnv2D, DurationDelay) {
    ...
    auto& source = gcs.AddRegions(MakeConvexSets(Point(start)), 0, kStartDelay, kStartDelay);
    // Similarly for the goal, but with a delay of kGoalDelay seconds.
    auto& target = gcs.AddRegions(MakeConvexSets(Point(goal)), 0, kGoalDelay, kGoalDelay);
    ...
    EXPECT_TRUE(CompareMatrices(traj.value(traj.end_time()), goal, kTolerance));
    EXPECT_TRUE(CompareMatrices(traj.value(traj.end_time() - kGoalDelay), goal, kTolerance));
...
TEST_F(SimpleEnv2D, MultiStartGoal) {
    ...
    */
    ...
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(traj.value(traj.start_time()), start2, kTolerance));
...
TEST_F(SimpleEnv2D, IntermediatePoint) {
  ...
  vertices << 5.0, 5.0, 4.8, 3.8, 3.8,
    ...
    main1_to_main2_pt.AddVelocityBounds(Vector2d(-kSpeed / 2, -kSpeed / 2), Vector2d(kSpeed / 2, kSpeed / 2));
    main1_to_main2_region.AddVelocityBounds(Vector2d(-kSpeed / 2, -kSpeed / 2), Vector2d(kSpeed / 2, kSpeed / 2));
    ...
    // region.
    main1_to_main2_pt.AddNonlinearDerivativeBounds(Vector2d(-kMaxAcceleration / 2, -kMaxAcceleration / 2),
                                                   Vector2d(kMaxAcceleration / 2, kMaxAcceleration / 2), 2);
    main1_to_main2_region.AddNonlinearDerivativeBounds(Vector2d(-kMaxAcceleration / 2, -kMaxAcceleration / 2),
                                                       Vector2d(kMaxAcceleration / 2, kMaxAcceleration / 2), 2);

    if (SnoptSolverUnavailable()) return;
    solvers::SnoptSolver snopt;
    options.restriction_solver = &snopt;
    auto [traj_nonlinear, result_nonlinear] = gcs.SolvePath(source, target, options);
    EXPECT_TRUE(result_nonlinear.is_success());
    EXPECT_TRUE(CompareMatrices(traj_nonlinear.value(traj_nonlinear.start_time()), start, kTolerance));
    EXPECT_TRUE(CompareMatrices(traj_nonlinear.value(traj_nonlinear.end_time()), goal, kTolerance));
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, EdgesFromWrappedJoints) {
    ...
    EXPECT_EQ(gcs_2.graph_of_convex_sets().Vertices().at(0)->outgoing_edges().size(), 0);
    EXPECT_EQ(gcs_2.graph_of_convex_sets().Vertices().at(1)->outgoing_edges().size(), 1);
    EXPECT_EQ(gcs_2.graph_of_convex_sets().Vertices().at(2)->outgoing_edges().size(), 1);
    ...
    EXPECT_EQ(gcs_4.graph_of_convex_sets().Vertices().at(0)->outgoing_edges().size(), 1);
    EXPECT_EQ(gcs_4.graph_of_convex_sets().Vertices().at(1)->outgoing_edges().size(), 1);
    EXPECT_EQ(gcs_4.graph_of_convex_sets().Vertices().at(2)->outgoing_edges().size(), 2);
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, WraparoundInOneDimension) {
    ...
    */
    ...
    // behavior.
    std::vector<ConvexSets> permutations = {MakeConvexSets(v1, v2, v3), MakeConvexSets(v3, v2, v1)};
    for (const auto& convexsets : permutations) {
        ...
        EXPECT_TRUE(CompareMatrices(traj1.value(traj1.start_time()), start, kTolerance));
        EXPECT_TRUE(CompareMatrices(traj1.value(traj1.end_time()), goal, kTolerance));
        ...
        auto& source1_alternate = gcs1.AddRegions(MakeConvexSets(Point(start_alternate)), 0);
        auto& target1_alternate = gcs1.AddRegions(MakeConvexSets(Point(goal_alternate)), 0);
        ...
        auto [traj1_alternate, result1_alternate] = gcs1.SolvePath(source1, target1);
        ASSERT_TRUE(result1_alternate.is_success());
        EXPECT_NEAR(result1_alternate.get_optimal_cost(), expected_cost_wraparound, tol);
    ...
    EXPECT_TRUE(CompareMatrices(traj2.value(traj2.start_time()), start, kTolerance));
...
GTEST_TEST(GcsTrajectoryOptimizationTest, WraparoundInTwoDimensions) {
  ...
  points << 0, 0,   2.4, 2.4,
    ...
    const double expected_cost_one_wraparound = sqrt((dist_long_way * dist_long_way) + (dist_shortcut * dist_shortcut));
    ...
    const std::vector<TestConfig> configs{{.name = "No wraparound",
    ...
    for (const TestConfig& config : configs) {
        ...
        auto& source2 = gcs.AddRegions(MakeConvexSets(Point(start + config.start_offset_works)), 0);
        ...
        if (config.continuous_joints.size() != 2) {
            ...
            auto& source3 = gcs.AddRegions(MakeConvexSets(Point(start + config.start_offset_fails)), 0);
...
GTEST_TEST(GcsTrajectoryOptimizationTest, WraparoundWithInvalidConvexityRadius) {
    // 1D example
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs.AddRegions(MakeConvexSets(v), 1), ".*doesn't respect the convexity radius.*");
  ...
  points_new << 0, 0, 2, 2,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(gcs2.AddRegions(MakeConvexSets(w), 1), ".*doesn't respect the convexity radius.*");
    DRAKE_EXPECT_THROWS_MESSAGE(gcs3.AddRegions(MakeConvexSets(w), 1), ".*doesn't respect the convexity radius.*");
}
...
GTEST_TEST(GcsTrajectoryOptimizationTest, WraparoundSubspace) {
    ...
    auto& subgraph2 = gcs.AddRegions(MakeConvexSets(HPolyhedron(v2), HPolyhedron(v3)), 1);
...
GTEST_TEST(GcsTrajectoryOptimizationTest, ContinuousJointsApi) {
    ...
    std::vector<int> continuous_revolute_joints = {0};
    GcsTrajectoryOptimization gcs(1, continuous_revolute_joints);
    EXPECT_EQ(gcs.continuous_revolute_joints().size(), continuous_revolute_joints.size());
    EXPECT_EQ(gcs.continuous_revolute_joints()[0], continuous_revolute_joints[0]);

    continuous_revolute_joints.emplace_back(0);
    DRAKE_EXPECT_THROWS_MESSAGE(GcsTrajectoryOptimization(1, continuous_revolute_joints),
...
GTEST_TEST(GcsTrajectoryOptimizationTest, GetContinuousJoints) {
    ...
    std::unique_ptr<PlanarJoint<double>> second_joint_ptr(new PlanarJoint<double>(
            "second_joint", first_body.body_frame(), second_body.body_frame(), Eigen::Matrix<double, 3, 1>::Zero()));
    second_joint_ptr->set_position_limits(Eigen::Vector3d{-1.0, -1.0, -1.0}, Eigen::Vector3d{1.0, 1.0, 1.0});
    ...
    plant.AddJoint<RevoluteJoint>("fourth_joint", third_body, {}, fourth_body, {},
                                  Eigen::Matrix<double, 3, 1>{1.0, 0.0, 0.0}, -1.0, 1.0);

    plant.Finalize();

    const std::vector<int> continuous_joint_indices = trajectory_optimization::GetContinuousRevoluteJointIndices(plant);
...
GTEST_TEST(GcsTrajectoryOptimizationTest, NonlinearDerivativeBoundsSymbolic) {
    ...
    GcsTrajectoryOptimization gcs(kDimension);
    auto& regions = gcs.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(2)), kOrder);
    gcs.AddNonlinearDerivativeBounds(Vector2d(-1, -1), Vector2d(1, 1), 2);

    ASSERT_EQ(regions.Vertices().size(), 1);
    std::vector<solvers::Binding<solvers::Constraint>> constraints =
            regions.Vertices()[0]->GetConstraints({GraphOfConvexSets::Transcription::kRestriction});
    ASSERT_EQ(constraints.size(), kDimension); /* we expect only NonlinearDerivativeBounds. */
...
GTEST_TEST(GcsTrajectoryOptimizationTest, ContinuityConstraintSymbolic) {
    ...
    GcsTrajectoryOptimization trajopt(kDimension);
    trajopt.AddRegions(MakeConvexSets(HPolyhedron::MakeUnitBox(2), HPolyhedron::MakeUnitBox(2)), kOrder);
    trajopt.AddContinuityConstraints(1);
    auto& gcs = trajopt.graph_of_convex_sets();

    ASSERT_EQ(gcs.Edges().size(), 2);
    std::vector<solvers::Binding<solvers::Constraint>> constraints =
            gcs.Edges()[0]->GetConstraints({GraphOfConvexSets::Transcription::kRestriction});
