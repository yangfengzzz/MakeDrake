#include "planning/trajectory_optimization/integration_constraint.h"
...
GTEST_TEST(MidPointIntegrationConstraintTest, Test) {
    ...
    const auto x_autodiff =
            math::InitializeAutoDiff(constraint_x, Eigen::MatrixXd::Identity(constraint_x.rows(), constraint_x.rows()));
    ...
    Eigen::MatrixXd y_grad_sparse(constraint.num_constraints(), constraint.num_vars());
    ...
    Eigen::MatrixXd y_grad_expected(constraint.num_constraints(), constraint.num_vars());
    y_grad_expected.block(0, 0, dim, dim) = Eigen::MatrixXd::Identity(dim, dim);
    y_grad_expected.block(0, dim, dim, dim) = -Eigen::MatrixXd::Identity(dim, dim);
    y_grad_expected.block(0, 2 * dim, dim, dim) = -dt / 2 * Eigen::MatrixXd::Identity(dim, dim);
    y_grad_expected.block(0, 3 * dim, dim, dim) = -dt / 2 * Eigen::MatrixXd::Identity(dim, dim);
