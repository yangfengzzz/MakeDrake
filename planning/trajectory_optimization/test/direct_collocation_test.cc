#include "planning/trajectory_optimization/direct_collocation.h"
...
GTEST_TEST(DirectCollocationConstraint, DoubleConstructor) {
    ...
    const std::unique_ptr<Context<double>> context = system->CreateDefaultContext();
...
GTEST_TEST(DirectCollocationConstraint, AutoDiffXdConstructor) {
    ...
    // Make sure that the constraint can be constructed and evaluated.
    DirectCollocationConstraint constraint(*system_ad, context_ad.get(), context_ad.get(), context_ad.get());
    const Eigen::VectorXd x = Eigen::VectorXd::Zero(constraint.num_vars());
    Eigen::VectorXd y = Eigen::VectorXd::Zero(constraint.num_constraints());
    constraint.Eval(x, &y);
}
...
GTEST_TEST(DirectCollocation, PassProgToConstructor) {
    ...
    DirectCollocation dircol(system.get(), *context, kNumSampleTimes, kTimeStep, kTimeStep,
    ...
    DirectCollocation dircol2(system.get(), *context, kNumSampleTimes, kTimeStep, kTimeStep,
                              systems::InputPortSelection::kUseFirstInputIfItExists, false, &prog);
...
GTEST_TEST(DirectCollocation, TestAddRunningCost) {
    ...
    dircol.AddRunningCost(dircol.state().cast<symbolic::Expression>().dot(dircol.state()) +
    ...
    const Eigen::Matrix<double, 1, kNumSampleTimes> g_val =
            (x.array() * x.array()).matrix().colwise().sum() + (u.array() * u.array()).matrix().colwise().sum();
    const double total_cost_expected =
            ((g_val.head<kNumSampleTimes - 1>() + g_val.tail<kNumSampleTimes - 1>()) / 2 * kTimeStep).sum();
...
GTEST_TEST(DirectCollocation, TestCollocationConstraint) {
    ...
    const std::vector<solvers::Binding<solvers::Constraint>>& collocation_constraints = prog.generic_constraints();
    ...
    const auto segment =
            PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives({0.0, kTimeStep}, {x0, x1}, xdot0, xdot1);
    const auto derivative = segment.derivative();
    const Eigen::Vector2d defect = derivative.value(kTimeStep / 2.0) - system->A() * segment.value(kTimeStep / 2.0) -
                                   system->B() * (u0 + u1) / 2.0;
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        ...
        EXPECT_TRUE(CompareMatrices(prog.EvalBindingAtInitialGuess(binding), defect, 1e-6));
    }
}
...
private:
    void DoCalcTimeDerivatives(const Context<AutoDiffXd>& context,
...
GTEST_TEST(DirectCollocation, TestCollocationConstraintCaching) {
    ...
    std::vector<std::unique_ptr<Context<AutoDiffXd>>> sample_context(kNumContexts);
    ...
    DirectCollocationConstraint constraint(system, sample_context[0].get(), sample_context[1].get(),
    ...
    DirectCollocationConstraint next_constraint(system, sample_context[1].get(), sample_context[2].get(),
                                                collocation_context.get());
...
GTEST_TEST(DirectCollocation, TestReconstruction) {
    ...
    result.set_x_val(Eigen::VectorXd::LinSpaced(prog.num_vars(), 1, prog.num_vars()));

    const PiecewisePolynomial<double> input_spline = dircol.ReconstructInputTrajectory(result);
    const PiecewisePolynomial<double> state_spline = dircol.ReconstructStateTrajectory(result);
    const auto derivative_spline = state_spline.derivative();

    double time = 0.0;
    for (int i = 0; i < kNumSampleTimes; i++) {
        EXPECT_TRUE(CompareMatrices(result.GetSolution(dircol.input(i)), input_spline.value(time), 1e-6));
        EXPECT_TRUE(CompareMatrices(result.GetSolution(dircol.state(i)), state_spline.value(time), 1e-6));

        EXPECT_TRUE(CompareMatrices(
                system->A() * result.GetSolution(dircol.state(i)) + system->B() * result.GetSolution(dircol.input(i)),
    ...
    for (int i = 0; i < (kNumSampleTimes - 1); i++) {
        ...
        const auto& binding = collocation_constraints[i];
        Eigen::Vector2d defect = derivative_spline.value(collocation_time) -
                                 system->A() * state_spline.value(collocation_time) -
                                 system->B() * input_spline.value(collocation_time);
        EXPECT_TRUE(CompareMatrices(defect, prog.EvalBinding(binding, result.get_x_val()), 1e-6));
        time += time_step;
    }
}
...
GTEST_TEST(DirectCollocation, DoubleIntegratorTest) {
    ...
    DirectCollocation dircol(double_integrator.get(), *context, kNumSampleTimes, 0.05, 2.0);
    auto& prog = dircol.prog();
    ...
    // x0 = [-1,0].
    prog.AddLinearConstraint(dircol.initial_state() == Eigen::Vector2d(-1.0, 0.0));
    ...
    int i = 0;
    while (i < kNumSampleTimes / 2.0) EXPECT_NEAR(result.GetSolution(dircol.input(i++))(0), 1.0, 1e-5);
    while (i < kNumSampleTimes) EXPECT_NEAR(result.GetSolution(dircol.input(i++))(0), -1.0, 1e-5);
}
...
GTEST_TEST(DirectCollocation, MinimumTimeTest) {
    ...
    DirectCollocation dircol(double_integrator.get(), *context, kNumSampleTimes, kMinTimeStep, kMaxTimeStep);
    auto& prog = dircol.prog();
    ...
    double total_time = 0;
    for (int i = 0; i < kNumSampleTimes - 1; i++) total_time += result.GetSolution(dircol.time_step(i))(0);
...
GTEST_TEST(DirectCollocation, NoInputs) {
    // xdot = -x.
    systems::LinearSystem<double> plant(Vector1d(-1.0),                        // A
    ...
    DirectCollocation dircol(&plant, *context, kNumSampleTimes, kFixedTimeStep, kFixedTimeStep);
    auto& prog = dircol.prog();
    ...
    EXPECT_NEAR(result.GetSolution(dircol.final_state())(0), x0 * std::exp(-duration), 1e-6);
...
GTEST_TEST(DirectCollocation, AddDirectCollocationConstraint) {
    const auto double_integrator = MakeDoubleIntegrator();
    auto context = double_integrator->CreateDefaultContext();
    auto constraint = std::make_shared<DirectCollocationConstraint>(*double_integrator, *context);
...
GTEST_TEST(DirectCollocation, InputPortSelection) {
    ...
    DirectCollocation dircol(plant.get(), *context, kNumSamples, kMinStep, kMaxStep,
...
GTEST_TEST(DirectCollocation, InputPortSelectionError) {
    ...
    const double kMinStep = 0.05;
    const double kMaxStep = 0.5;
    DRAKE_EXPECT_THROWS_MESSAGE(DirectCollocation(plant.get(), *context, kNumSamples, kMinStep, kMaxStep),
...
GTEST_TEST(DirectCollocation, IgnoreNonContinuousState) {
    ...
    DirectCollocation dircol(&plant, *context, kNumSamples, kMinStep, kMaxStep, InputPortSelection::kNoInput,
...
GTEST_TEST(DirectCollocation, DiscreteTimeSystemThrows) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DirectCollocationConstraint(plant, *context), ".*doesn't have any continuous states.*");

    const int kNumSamples = 15;
    const double kMinStep = 0.01;
    const double kMaxStep = 0.1;
    DRAKE_EXPECT_THROWS_MESSAGE(DirectCollocation(&plant, *context, kNumSamples, kMinStep, kMaxStep),
