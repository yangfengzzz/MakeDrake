#include "planning/trajectory_optimization/multiple_shooting.h"
...
public:
    MyDirectTrajOpt(const int num_inputs,
                    const int num_states,
                    const int num_time_samples,
                    const double fixed_time_step,
                    solvers::MathematicalProgram* prog = nullptr)
        : MultipleShooting(num_inputs, num_states, num_time_samples, fixed_time_step, prog) {}
    ...
    MyDirectTrajOpt(const int num_inputs,
                    ...
                    solvers::MathematicalProgram* prog = nullptr)
        : MultipleShooting(num_inputs, num_states, num_time_samples, min_time_step, max_time_step, prog) {}
    ...
    MyDirectTrajOpt(const solvers::VectorXDecisionVariable& input,
                    ...
                    const double fixed_time_step,
                    solvers::MathematicalProgram* prog = nullptr)
        : MultipleShooting(input, state, num_time_samples, fixed_time_step, prog) {}

    MyDirectTrajOpt(const solvers::VectorXDecisionVariable& input,
                    const solvers::VectorXDecisionVariable& state,
                    const symbolic::Variable& time,
                    const int num_time_samples,
                    const double min_time_step,
                    const double max_time_step,
                    solvers::MathematicalProgram* prog = nullptr)
        : MultipleShooting(input, state, time, num_time_samples, min_time_step, max_time_step, prog) {}

    PiecewisePolynomial<double> ReconstructInputTrajectory(const solvers::MathematicalProgramResult&) const override {
        return PiecewisePolynomial<double>();
    };

    PiecewisePolynomial<double> ReconstructStateTrajectory(const solvers::MathematicalProgramResult&) const override {
        return PiecewisePolynomial<double>();
    };
...
GTEST_TEST(MultipleShootingTest, FixedTimestepTest) {
    ...
    MyDirectTrajOpt trajopt(kNumInputs, kNumStates, kNumSampleTimes, kFixedTimeStep);
...
GTEST_TEST(MultipleShootingTest, FixedTimestepWithProgTest) {
    ...
    MyDirectTrajOpt trajopt(kNumInputs, kNumStates, kNumSampleTimes, kFixedTimeStep, &prog);
    EXPECT_EQ(&prog, &trajopt.prog());
    EXPECT_EQ(prog.num_vars(), 6);

    MyDirectTrajOpt trajopt2(kNumInputs, kNumStates, kNumSampleTimes, kFixedTimeStep, &prog);
...
GTEST_TEST(MultipleShootingTest, FixedTimestepTestWithPlaceholderVariables) {
    const VectorX<symbolic::Variable> input{symbolic::MakeVectorContinuousVariable(3, "my_input")};
    const VectorX<symbolic::Variable> state{symbolic::MakeVectorContinuousVariable(4, "my_state")};
    const int kNumSampleTimes{2};
    const double kFixedTimeStep{0.1};
    ...
    EXPECT_EQ(trajopt.prog().num_vars(), (input.size() + state.size()) * kNumSampleTimes);
    solvers::MathematicalProgramResult result;
    const Eigen::VectorXd times = trajopt.GetSampleTimes(result);
    EXPECT_EQ(times.size(), 2);
    EXPECT_EQ(times[0], 0.0);
    ...
    MyDirectTrajOpt trajopt2(input, state, kNumSampleTimes, kFixedTimeStep, &trajopt.prog());
    EXPECT_EQ(trajopt.prog().num_vars(), 2 * (input.size() + state.size()) * kNumSampleTimes);
}
...
GTEST_TEST(MultipleShootingTest, VariableTimestepTestWithPlaceholderVariables) {
    ...
    MyDirectTrajOpt trajopt(input, state, time, kNumSampleTimes, kMinTimeStep, kMaxTimeStep);
    ...
    EXPECT_EQ(trajopt.prog().num_vars(), (input.size() + state.size()) * kNumSampleTimes + 1 /* time variable */);
    ...
    // Now use the constructor which takes a prog.
    MyDirectTrajOpt trajopt2(input, state, time, kNumSampleTimes, kMinTimeStep, kMaxTimeStep, &trajopt.prog());
    EXPECT_EQ(trajopt.prog().num_vars(), 2 * (input.size() + state.size()) * kNumSampleTimes + 2 /* time variables */);
}
...
GTEST_TEST(MultipleShootingTest, VariableTimestepTest) {
    ...
    const double kMinTimeStep{0.1};
    const double kMaxTimeStep{0.1};
    MyDirectTrajOpt trajopt(kNumInputs, kNumStates, kNumSampleTimes, kMinTimeStep, kMaxTimeStep);
    ...
    // Now use the constructor which takes a prog.
    MyDirectTrajOpt trajopt2(kNumInputs, kNumStates, kNumSampleTimes, kMinTimeStep, kMaxTimeStep, &trajopt.prog());
...
GTEST_TEST(MultipleShootingTest, PlaceholderVariableTest) {
    ...
    EXPECT_THROW(prog.AddQuadraticErrorCost(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero(), x), std::exception);
    ...
    EXPECT_THROW(
            prog.AddLinearConstraint(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero(), Eigen::Vector2d::Zero(), x),
            std::exception);
...
GTEST_TEST(MultipleShootingTest, TimeIntervalBoundsTest) {
    ...
    solvers::Binding<solvers::BoundingBoxConstraint> interval_bounds = trajopt.AddTimeIntervalBounds(.5, .5);
    EXPECT_EQ(interval_bounds.evaluator()->lower_bound(), Eigen::Vector2d(0.5, 0.5));
    EXPECT_EQ(interval_bounds.evaluator()->upper_bound(), Eigen::Vector2d(0.5, 0.5));
    const solvers::MathematicalProgramResult result = Solve(trajopt.prog());
    ASSERT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(trajopt.h_vars()), Eigen::Vector2d(0.5, 0.5), 1e-6));
}
...
GTEST_TEST(MultipleShootingTest, EqualTimeIntervalsTest) {
    ...
    const solvers::MathematicalProgramResult result = Solve(prog, prog.initial_guess());
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(result.GetSolution(trajopt.time_step(0).coeff(0)), result.GetSolution(trajopt.time_step(1).coeff(0)),
                kSolverTolerance);
}
...
GTEST_TEST(MultipleShootingTest, DurationConstraintTest) {
    ...
    solvers::Binding<solvers::LinearConstraint> duration_bound = trajopt.AddDurationBounds(.5, .5);
...
GTEST_TEST(MultipleShootingTest, ConstraintAllKnotsTest) {
    ...
    for (int i = 0; i < kNumSampleTimes; i++) {
        ...
        // 1E-6.
        const double tol = result.get_solver_id() == solvers::OsqpSolver::id() ? 4E-6 : 1E-6;
        EXPECT_TRUE(CompareMatrices(result.GetSolution(trajopt.state(i)), state_value, tol));
    }
    ...
    // u(1) = h(0).
    EXPECT_NEAR(result.GetSolution(trajopt.input(1).coeff(0)), result.GetSolution(trajopt.time_step(0).coeff(0)), 1e-6);
    // u(2) = h(0)+h(1).
    EXPECT_NEAR(result.GetSolution(trajopt.input(2).coeff(0)), result.GetSolution(trajopt.h_vars()).sum(), 1e-6);

    // Add the same constraint again using the shared_ptr variant.
    std::vector<solvers::Binding<solvers::BoundingBoxConstraint>> c = trajopt.AddConstraintToAllKnotPoints(
            std::make_shared<solvers::BoundingBoxConstraint>(state_value, state_value), trajopt.state());
    EXPECT_EQ(c.size(), kNumSampleTimes);

    // Add the same constraint again using the vector of formulas variant.
    std::vector<solvers::Binding<solvers::Constraint>> c2 = trajopt.AddConstraintToAllKnotPoints(
            Vector2<symbolic::Formula>{trajopt.state()[0] == state_value[0], trajopt.state()[1] == state_value[1]});
...
GTEST_TEST(MultipleShootingTest, FinalCostTest) {
    ...
    solvers::Binding<solvers::Cost> final_cost = trajopt.AddFinalCost(error.dot(error));
    const solvers::MathematicalProgramResult result = Solve(trajopt.prog());
    ASSERT_TRUE(result.is_success());
    EXPECT_NEAR(result.get_optimal_cost(), 0.0, kSolverTolerance);
    EXPECT_TRUE(CompareMatrices(result.EvalBinding(final_cost), Vector1d(0.0), kSolverTolerance));
    EXPECT_TRUE(CompareMatrices(result.GetSolution(trajopt.state(1)), desired_state, kSolverTolerance));
}
...
GTEST_TEST(MultipleShootingTest, TrajectoryCallbackTest) {
    ...
    auto my_input_callback = [&input_callback_was_called](const Eigen::Ref<const Eigen::VectorXd>& t,
    ...
    auto my_state_callback = [&state_callback_was_called](const Eigen::Ref<const Eigen::VectorXd>& t,
    ...
    auto my_complete_callback = [&complete_callback_was_called](
    ...
    solvers::Binding<solvers::VisualizationCallback> b = trajopt.AddInputTrajectoryCallback(my_input_callback);
    ...
    prog.SetInitialGuess(trajopt.GetSequentialVariable(extra_variables[0]), Eigen::Vector3d::LinSpaced(3, 10., 12.));
    prog.SetInitialGuess(trajopt.GetSequentialVariable(extra_variables[1]), Eigen::Vector3d::LinSpaced(3, 13., 15.));
    b = trajopt.AddCompleteTrajectoryCallback(my_complete_callback, extra_variables);
    ...
    MyDirectTrajOpt trajopt2(kNumInputs, kNumStates, kNumSampleTimes, kFixedTimeStep, kFixedTimeStep);
    auto& prog2 = trajopt2.prog();
    prog2.SetInitialGuess(trajopt2.h_vars(), Eigen::Vector2d::Constant(kFixedTimeStep));
    prog2.SetInitialGuess(trajopt2.u_vars(), Eigen::Vector3d::LinSpaced(3, 1., 3.));
    prog2.SetInitialGuess(trajopt2.x_vars(), Eigen::VectorXd::LinSpaced(6, 4., 9.));
    ...
    prog2.SetInitialGuess(trajopt2.GetSequentialVariable(extra_variables[0]), Eigen::Vector3d::LinSpaced(3, 10., 12.));
    prog2.SetInitialGuess(trajopt2.GetSequentialVariable(extra_variables[1]), Eigen::Vector3d::LinSpaced(3, 13., 15.));
    b = trajopt2.AddCompleteTrajectoryCallback(my_complete_callback, extra_variables);
...
GTEST_TEST(MultipleShootingTest, InitialGuessTest) {
    ...
    const auto traj1 = PiecewisePolynomial<double>::FirstOrderHold({1, 2}, {Vector1d(3), Vector1d(4)});
    const auto traj2 = PiecewisePolynomial<double>::FirstOrderHold({1, 4}, {Vector1d(5), Vector1d(6)});

    // Throws if both are empty.
    EXPECT_THROW(trajopt.SetInitialTrajectory(PiecewisePolynomial<double>(), PiecewisePolynomial<double>()),
                 std::runtime_error);
...
GTEST_TEST(MultipleShootingTest, ResultSamplesTest) {
  ...
  state_trajectory << 3, 4,
    ...
    EXPECT_TRUE(CompareMatrices(trajopt.GetSampleTimes(result), Eigen::Vector2d(0.0, kFixedTimeStep), 0.0));
    EXPECT_TRUE(CompareMatrices(trajopt.GetInputSamples(result), input_trajectory, 0.0));
    EXPECT_TRUE(CompareMatrices(trajopt.GetStateSamples(result), state_trajectory, 0.0));
}
...
GTEST_TEST(MultipleShootingTest, NewSequentialVariableTest) {
    ...
    solvers::VectorXDecisionVariable new_sequential_variable = trajopt.NewSequentialVariable(kNumStates, "w");
    trajopt.AddConstraintToAllKnotPoints(-2.0 * new_sequential_variable == trajopt.state());
    ...
    for (int i = 0; i < kNumSampleTimes; i++) {
        ...
        EXPECT_TRUE(CompareMatrices(-2.0 * result.GetSolution(trajopt.GetSequentialVariableAtIndex("w", i)),
                                    state_value, tol));
    }

    EXPECT_TRUE(CompareMatrices(-2.0 * trajopt.GetSequentialVariableSamples(result, "w"),
