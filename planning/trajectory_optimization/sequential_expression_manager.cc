...
SequentialExpressionManager::SequentialExpressionManager(int num_samples) : num_samples_(num_samples) {
    DRAKE_THROW_UNLESS(num_samples_ > 0);
}

VectorX<Variable> SequentialExpressionManager::RegisterSequentialExpressions(
        const Eigen::Ref<const MatrixX<Expression>>& sequential_expressions, const string& name) {
    const int rows = sequential_expressions.rows();
    DRAKE_THROW_UNLESS(sequential_expressions.cols() == num_samples_);
    const VectorX<Variable> placeholders{symbolic::MakeVectorContinuousVariable(rows, name)};
...
void SequentialExpressionManager::RegisterSequentialExpressions(
    ...
    for (int i = 0; i < placeholders.size(); ++i) {
        placeholders_to_expressions_.emplace(std::make_pair(placeholders[i], sequential_expressions.row(i)));
    }
}

Substitution SequentialExpressionManager::ConstructPlaceholderVariableSubstitution(int index) const {
...
VectorX<symbolic::Variable> SequentialExpressionManager::GetVariables(
        const Eigen::Ref<const VectorX<symbolic::Variable>>& vars, int index) const {
    DRAKE_THROW_UNLESS(0 <= index && index < num_samples_);
    ...
    for (int i = 0; i < vars.size(); ++i) {
        ...
        if (it == placeholders_to_expressions_.end()) {
            throw std::runtime_error(vars[i].get_name() +
...
VectorX<Expression> SequentialExpressionManager::GetSequentialExpressionsByName(const string& name, int index) const {
    DRAKE_THROW_UNLESS(0 <= index && index < num_samples_);
...
vector<string> SequentialExpressionManager::GetSequentialExpressionNames() const {
    vector<string> ret;
    for (const auto& [p, e] : placeholders_to_expressions_) {
        unused(p);
