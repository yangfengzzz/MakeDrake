...
public:
    ...
    */
    ...
    explicit KinematicTrajectoryOptimization(const trajectories::BsplineTrajectory<double>& trajectory);
    ...
    const solvers::MatrixXDecisionVariable& control_points() const { return control_points_; }
    ...
    */
    ...
    ith control point.
    */
    std::vector<solvers::Binding<solvers::BoundingBoxConstraint>> AddPositionBounds(
            const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub);
    ...
    std::vector<std::vector<solvers::Binding<solvers::Constraint>>> AddAccelerationBounds(
            const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub);
...
private:
    ...
    // r(s) is the path.
    copyable_unique_ptr<trajectories::BsplineTrajectory<symbolic::Expression>> sym_r_{};
    copyable_unique_ptr<trajectories::BsplineTrajectory<symbolic::Expression>> sym_rdot_{};
    copyable_unique_ptr<trajectories::BsplineTrajectory<symbolic::Expression>> sym_rddot_{};
    copyable_unique_ptr<trajectories::BsplineTrajectory<symbolic::Expression>> sym_rdddot_{};
};
