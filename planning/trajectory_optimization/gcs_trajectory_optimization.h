...
#include "common/trajectories/composite_trajectory.h"
...
public:
    ...
    */
    ...
    public:
        ...
        const std::vector<geometry::optimization::GraphOfConvexSets::Vertex*>& Vertices() { return vertices_; }
        ...
        /** Returns constant reference to a vector of mutable pointers to the
        edges. */
        const std::vector<geometry::optimization::GraphOfConvexSets::Edge*>& Edges() { return edges_; }
        ...
        CartesianProduct. */
        const geometry::optimization::ConvexSets& regions() const { return regions_; }
    ...
    private:
        ...
        /* Convenience accessor, for brevity. */
        const std::vector<int>& continuous_revolute_joints() const { return traj_opt_.continuous_revolute_joints(); }
        ...
        symbolic::Variable GetTimeScaling(const geometry::optimization::GraphOfConvexSets::Vertex& v) const;
    ...
    */
    ...
    private:
        ...
        bool RegionsConnectThroughSubspace(const geometry::optimization::ConvexSet& A,
                                           const geometry::optimization::ConvexSet& B,
                                           const geometry::optimization::ConvexSet& subspace,
                                           std::optional<const Eigen::VectorXd> maybe_set_B_offset = std::nullopt,
                                           std::optional<const Eigen::VectorXd> maybe_subspace_offset = std::nullopt);
        ...
        symbolic::Variable GetTimeScalingU(const geometry::optimization::GraphOfConvexSets::Edge& e) const;

        /* Extracts the time scaling variable from a edge. */
        symbolic::Variable GetTimeScalingV(const geometry::optimization::GraphOfConvexSets::Edge& e) const;
    ...
    const std::vector<int>& continuous_revolute_joints() { return continuous_revolute_joints_; }
    ...
    */
    std::pair<trajectories::CompositeTrajectory<double>, solvers::MathematicalProgramResult> SolvePath(
            const Subgraph& source,
    ...
    std::pair<trajectories::CompositeTrajectory<double>, solvers::MathematicalProgramResult> SolveConvexRestriction(
            const std::vector<const geometry::optimization::GraphOfConvexSets::Vertex*>& active_vertices,
            const geometry::optimization::GraphOfConvexSetsOptions& options = {});
    ...
    const geometry::optimization::GraphOfConvexSets& graph_of_convex_sets() const { return gcs_; }
...
private:
    ...
    trajectories::CompositeTrajectory<double> ReconstructTrajectoryFromSolutionPath(
    ...
    geometry::optimization::GraphOfConvexSets::Edge* AddEdge(geometry::optimization::GraphOfConvexSets::Vertex* u,
    ...
    std::map<const geometry::optimization::GraphOfConvexSets::Vertex*, Subgraph*> vertex_to_subgraph_;
    std::vector<double> global_time_costs_;
    std::vector<Eigen::MatrixXd> global_path_length_costs_;
    std::vector<std::pair<Eigen::VectorXd, Eigen::VectorXd>> global_velocity_bounds_{};
    std::vector<std::tuple<Eigen::VectorXd, Eigen::VectorXd, int>> global_nonlinear_derivative_bounds_{};
