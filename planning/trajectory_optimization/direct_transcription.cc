...
public:
    ...
    DirectTranscriptionConstraint(IntegratorBase<AutoDiffXd>* integrator,
        ...
        DRAKE_DEMAND(context.has_only_discrete_state() || context.has_only_continuous_state());
        DRAKE_DEMAND(context.num_input_ports() == 0 || input_port_value_ != nullptr);
...
protected:
    ...
    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const override {
        ...
        if (context->has_only_continuous_state()) {
            ...
            DRAKE_THROW_UNLESS(integrator_->IntegrateWithSingleFixedStepToTime(evaluation_time_ + fixed_time_step_));
            *y = next_state - context->get_continuous_state_vector().CopyToVector();
        } else {
            context->SetDiscreteState(0, state);
            discrete_state_->SetFrom(integrator_->get_system().EvalUniquePeriodicDiscreteUpdate(*context));
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::logic_error("DirectTranscriptionConstraint does not support symbolic evaluation.");
    }
...
double get_period(const System<double>* system, std::string message) {
    std::optional<PeriodicEventData> periodic_data = system->GetUniquePeriodicDiscreteUpdateAttribute();
...
int get_input_port_size(const System<double>* system,
...
DirectTranscription::DirectTranscription(const TimeVaryingLinearSystem<double>* system,
                                         const Context<double>& context,
                                         int num_time_samples,
                                         const std::variant<InputPortSelection, InputPortIndex>& input_port_index)
    : MultipleShooting(get_input_port_size(system, input_port_index),
                       context.num_total_states(),
                       num_time_samples,
                       std::max(system->time_period(), std::numeric_limits<double>::epsilon())
    ...
    for (int i = 0; i < N() - 1; i++) {
        ...
        prog().AddLinearEqualityConstraint(state(i + 1).cast<symbolic::Expression>() ==
...
bool DirectTranscription::AddSymbolicDynamicConstraints(
    ...
    if (SystemSymbolicInspector::IsAbstract(*symbolic_system, *symbolic_context)) {
        return false;
    }
    symbolic_context->SetTimeStateAndParametersFrom(context);

    const InputPort<Expression>* input_port = symbolic_system->get_input_port_selection(input_port_index);

    ExplicitEulerIntegrator<Expression> integrator(*symbolic_system, fixed_time_step(), symbolic_context.get());
    ...
    for (int i = 0; i < N() - 1; i++) {
        ...
        if (discrete_time_system_) {
            ...
            DRAKE_THROW_UNLESS(integrator.IntegrateWithSingleFixedStepToTime((i + 1) * fixed_time_step()));
            next_state = symbolic_context->get_continuous_state_vector().CopyToVector();
        }
        if (i == 0 && !IsAffine(next_state, symbolic::Variables(prog().decision_variables()))) {
...
void DirectTranscription::AddAutodiffDynamicConstraints(
    ...
    if (input_port_) {
        ...
        input_port_value_ =
                &input_port_->FixValue(context_.get(), system_->AllocateInputVector(*input_port_)->get_value());
    }

    integrator_ = std::make_unique<ExplicitEulerIntegrator<AutoDiffXd>>(*system_, fixed_time_step(), context_.get());
    ...
    for (int i = 0; i < N() - 1; i++) {
        ...
        auto constraint = std::make_shared<DirectTranscriptionConstraint>(
                integrator_.get(), input_port_value_, num_states(), num_inputs(), i * fixed_time_step(),
                TimeStep{fixed_time_step()});
