#include "planning/trajectory_optimization/kinematic_trajectory_optimization.h"
...
public:
    PathConstraint(std::shared_ptr<Constraint> wrapped_constraint, std::vector<double> basis_function_values)
    ...
    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const override {
        AutoDiffVecXd x_sum = basis_function_values_[0] * x.segment(0, wrapped_constraint_->num_vars());
        const int num_terms = basis_function_values_.size();
        for (int i = 1; i < num_terms; ++i) {
            x_sum += basis_function_values_[i] *
                     x.segment(i * wrapped_constraint_->num_vars(), wrapped_constraint_->num_vars());
        }
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::runtime_error("PathConstraint does not support evaluation with Expression.");
    }
...
public:
    WrappedVelocityConstraint(std::shared_ptr<Constraint> wrapped_constraint,
        ...
        DRAKE_DEMAND(M_pos_.rows() + M_vel_.rows() == wrapped_constraint_->num_vars());
    }
    ...
    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const override {
        ...
        qv << M_pos_ * x.segment(1, M_pos_.cols()), M_vel_ * x.tail(M_vel_.cols()) / duration;
...
public:
    DerivativeConstraint(const Eigen::MatrixXd& M,
        ...
        : Constraint(M.rows(), M.cols() + 1, lb, ub), M_{M}, derivative_order_{derivative_order} {
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::runtime_error("DerivativeConstraint does not support evaluation with Expression.");
    }
...
KinematicTrajectoryOptimization::KinematicTrajectoryOptimization(int num_positions,
                                                                 int num_control_points,
                                                                 int spline_order,
                                                                 double duration)
    : KinematicTrajectoryOptimization(BsplineTrajectory(
              BsplineBasis<double>(
                      spline_order, num_control_points, math::KnotVectorType::kClampedUniform, 0, duration),
              std::vector<MatrixXd>(num_control_points, MatrixXd::Zero(num_positions, 1)))) {}

KinematicTrajectoryOptimization::KinematicTrajectoryOptimization(
        const trajectories::BsplineTrajectory<double>& trajectory)
    : num_positions_(trajectory.rows()), num_control_points_(trajectory.num_control_points()) {
    ...
    control_points_ = prog_.NewContinuousVariables(num_positions_, num_control_points_, "control_point");
    duration_ = prog_.NewContinuousVariables(1, "duration")[0];
    // duration >= 0.
    auto duration_bbox_binding = prog_.AddBoundingBoxConstraint(0, std::numeric_limits<double>::infinity(), duration_);
    duration_bbox_binding.evaluator()->set_description("positive duration constraint");
    ...
    sym_rdot_ = dynamic_pointer_cast_or_throw<BsplineTrajectory<symbolic::Expression>>(sym_r_->MakeDerivative());
    sym_rddot_ = dynamic_pointer_cast_or_throw<BsplineTrajectory<symbolic::Expression>>(sym_rdot_->MakeDerivative());
    sym_rdddot_ = dynamic_pointer_cast_or_throw<BsplineTrajectory<symbolic::Expression>>(sym_rddot_->MakeDerivative());
}

void KinematicTrajectoryOptimization::SetInitialGuess(const trajectories::BsplineTrajectory<double>& trajectory) {
    prog_.SetInitialGuess(control_points_, StdVectorToEigen<double>(trajectory.control_points()));
    prog_.SetInitialGuess(duration_, trajectory.end_time() - trajectory.start_time());
}

BsplineTrajectory<double> KinematicTrajectoryOptimization::ReconstructTrajectory(
...
Binding<LinearConstraint> KinematicTrajectoryOptimization::AddPathPositionConstraint(
        const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub, double s) {
    ...
    auto binding = prog_.AddLinearConstraint(lb <= sym_r_value && sym_r_value <= ub);
...
Binding<Constraint> KinematicTrajectoryOptimization::AddPathPositionConstraint(
    ...
    std::vector<int> active_control_point_indices = basis_.ComputeActiveBasisFunctionIndices(s);
    const int num_active_control_points = static_cast<int>(active_control_point_indices.size());
    VectorXDecisionVariable var_vector(num_active_control_points * num_positions());
    for (int i = 0; i < num_active_control_points; ++i) {
        const int control_point_index = active_control_point_indices[i];
        basis_function_values.push_back(basis_.EvaluateBasisFunctionI(control_point_index, s));
        var_vector.segment(i * num_positions(), num_positions()) = control_points_.col(control_point_index);
    }
    auto binding = prog_.AddConstraint(std::make_shared<PathConstraint>(constraint, basis_function_values), var_vector);
    binding.evaluator()->set_description("path position constraint");
    return binding;
}

Binding<LinearConstraint> KinematicTrajectoryOptimization::AddPathVelocityConstraint(
        const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub, double s) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    DRAKE_DEMAND(0 <= s && s <= 1);
    const VectorX<symbolic::Expression> sym_rdot_value = sym_rdot_->value(s);
    auto binding = prog_.AddLinearConstraint(lb <= sym_rdot_value && sym_rdot_value <= ub);
    binding.evaluator()->set_description("path velocity constraint");
    return binding;
}

Binding<Constraint> KinematicTrajectoryOptimization::AddVelocityConstraintAtNormalizedTime(
    ...
    std::tie(vars_vel, unused_map) = symbolic::ExtractVariablesFromExpression(rdot);
    ...
    VectorXDecisionVariable duration_and_vars(1 + vars_pos.size() + vars_vel.size());
    duration_and_vars << duration_, vars_pos, vars_vel;
    auto wrapped_constraint =
            std::make_shared<WrappedVelocityConstraint>(constraint, std::move(M_pos), std::move(M_vel));
...
Binding<LinearConstraint> KinematicTrajectoryOptimization::AddPathAccelerationConstraint(
        const Eigen::Ref<const Eigen::VectorXd>& lb, const Eigen::Ref<const Eigen::VectorXd>& ub, double s) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    DRAKE_DEMAND(0 <= s && s <= 1);
    const VectorX<symbolic::Expression> sym_rddot_value = sym_rddot_->value(s);
    auto binding = prog_.AddLinearConstraint(lb <= sym_rddot_value && sym_rddot_value <= ub);
    binding.evaluator()->set_description("path acceleration constraint");
    return binding;
}

Binding<BoundingBoxConstraint> KinematicTrajectoryOptimization::AddDurationConstraint(optional<double> lb,
                                                                                      optional<double> ub) {
    auto binding = prog_.AddBoundingBoxConstraint(lb.value_or(0), ub.value_or(std::numeric_limits<double>::infinity()),
                                                  duration_);
    binding.evaluator()->set_description("duration constraint");
    return binding;
}

std::vector<Binding<BoundingBoxConstraint>> KinematicTrajectoryOptimization::AddPositionBounds(
        const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    ...
    for (int i = 0; i < num_control_points(); ++i) {
        binding.emplace_back(prog_.AddBoundingBoxConstraint(lb, ub, control_points_.col(i)));
        binding[i].evaluator()->set_description(fmt::format("position bound {}", i));
    }
    return binding;
}

std::vector<Binding<LinearConstraint>> KinematicTrajectoryOptimization::AddVelocityBounds(
        const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    ...
    for (int i = 0; i < sym_rdot_->num_control_points(); ++i) {
        binding.emplace_back(prog_.AddLinearConstraint(sym_rdot_->control_points()[i] >= duration_ * lb &&
                                                       sym_rdot_->control_points()[i] <= duration_ * ub));
        binding[i].evaluator()->set_description(fmt::format("velocity bound {}", i));
    }
    return binding;
}

std::vector<std::vector<Binding<Constraint>>> KinematicTrajectoryOptimization::AddAccelerationBounds(
        const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    ...
    std::unordered_map<symbolic::Variable::Id, int> unused_map;
    std::vector<std::vector<Binding<Constraint>>> binding(sym_rddot_->num_control_points());
    for (int i = 0; i < sym_rddot_->num_control_points(); ++i) {
        for (int j = 0; j < num_positions(); ++j) {
            std::tie(vars, unused_map) = symbolic::ExtractVariablesFromExpression(sym_rddot_->control_points()[i](j));
            M.resize(vars.size());
            // TODO(russt): Avoid symbolic here and throughout.
            symbolic::DecomposeLinearExpressions(Vector1<Expression>(sym_rddot_->control_points()[i](j)), vars, &M);
            auto con = std::make_shared<DerivativeConstraint>(M, 2, lb.segment<1>(j), ub.segment<1>(j));
            ...
            binding[i][j].evaluator()->set_description(fmt::format("acceleration bound {}, {}", i, j));
        }
    }
    return binding;
}

std::vector<std::vector<Binding<Constraint>>> KinematicTrajectoryOptimization::AddJerkBounds(
        const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_DEMAND(lb.size() == num_positions());
    DRAKE_DEMAND(ub.size() == num_positions());
    ...
    for (int i = 0; i < sym_rdddot_->num_control_points(); ++i) {
        for (int j = 0; j < num_positions(); ++j) {
            std::tie(vars, map_var_to_index) =
                    symbolic::ExtractVariablesFromExpression(sym_rdddot_->control_points()[i](j));
            M.resize(vars.size());
            symbolic::DecomposeLinearExpressions(Vector1<Expression>(sym_rdddot_->control_points()[i](j)), vars, &M);
            auto con = std::make_shared<DerivativeConstraint>(M, 3, lb.segment<1>(j), ub.segment<1>(j));
            duration_and_vars.resize(vars.size() + 1);
            duration_and_vars << duration_, vars;
            binding[i].emplace_back(prog_.AddConstraint(con, duration_and_vars));
            binding[i][j].evaluator()->set_description(fmt::format("jerk bound {}, {}", i, j));
        }
    }
    return binding;
}

Binding<LinearCost> KinematicTrajectoryOptimization::AddDurationCost(double weight) {
    auto binding = prog_.AddLinearCost(weight * duration_);
    binding.evaluator()->set_description("duration cost");
    return binding;
}

std::vector<Binding<Cost>> KinematicTrajectoryOptimization::AddPathLengthCost(double weight,
                                                                              bool use_conic_constraint) {
    MatrixXd A(num_positions_, 2 * num_positions_);
    A.leftCols(num_positions_) = weight * MatrixXd::Identity(num_positions_, num_positions_);
    A.rightCols(num_positions_) = -weight * MatrixXd::Identity(num_positions_, num_positions_);
    ...
    for (int i = 1; i < num_control_points(); ++i) {
        ...
        if (use_conic_constraint) {
            auto slack_cost_and_constraint_tuple = prog_.AddL2NormCostUsingConicConstraint(A, b, vars);
            binding.emplace_back(std::get<1>(slack_cost_and_constraint_tuple));
            std::get<2>(slack_cost_and_constraint_tuple)
                    .evaluator()
                    ->set_description(fmt::format("path length cost {} conic constraint", i));
        } else {
            binding.emplace_back(prog_.AddL2NormCost(A, b, vars));
        }
        binding[i - 1].evaluator()->set_description(fmt::format("path length cost {}", i));
    }
    return binding;
}

std::vector<solvers::Binding<solvers::Cost>> KinematicTrajectoryOptimization::AddPathEnergyCost(double weight) {
    ...
    A.topLeftCorner(num_positions_, num_positions_) = 2.0 * weight * MatrixXd::Identity(num_positions_, num_positions_);
    ...
    for (int i = 1; i < num_control_points(); ++i) {
        ...
        binding[i - 1].evaluator()->set_description(fmt::format("path energy cost {}", i));
    }
    return binding;
}
