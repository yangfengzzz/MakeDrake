...
#include "geometry/optimization/cartesian_product.h"
...
public:
    NonlinearDerivativeConstraint(const Eigen::SparseMatrix<double>& M, double lb, double ub, int derivative_order)
        : Constraint(2 * M.rows(),
                     M.cols() + 1,
                     Eigen::VectorXd::Zero(2 * M.rows()),
                     Eigen::VectorXd::Constant(2 * M.rows(), std::numeric_limits<double>::infinity())),
          M_(M),
          lb_(lb),
          ub_(ub),
    ...
    void DoEvalGeneric(const Eigen::Ref<const VectorX<U>>& x, VectorX<T>* y) const {
    ...
    void DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const { DoEvalGeneric(x, y); }

    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const { DoEvalGeneric(x, y); }
...
public:
    NonlinearContinuityConstraint(const Eigen::SparseMatrix<double>& Mu,
        ...
        : Constraint(1, Mu.cols() + 1 + Mv.cols() + 1, Eigen::VectorXd::Zero(1), Eigen::VectorXd::Zero(1)),
          Mu_(Mu),
          Mv_(Mv),
    ...
    void DoEvalGeneric(const Eigen::Ref<const VectorX<U>>& x, VectorX<T>* y) const {
        ...
        T pow_h_v = pow(x[num_control_points_u_ + num_control_points_v_ + 1], continuity_order_);

        // Precompute Matrix Products.
        T Mu_x = Mu_.row(0) * x.head(num_control_points_u_);
        T Mv_x = Mv_.row(0) * x.segment(num_control_points_u_ + 1, num_control_points_v_);
...
Subgraph::Subgraph(const ConvexSets& regions,
    ...
    : regions_(regions), order_(order), h_min_(h_min), name_(std::move(name)), traj_opt_(*traj_opt) {
    ...
    // vertices to GCS.
    const HPolyhedron time_scaling_set = HPolyhedron::MakeBox(Vector1d(h_min), Vector1d(h_max));
    ...
    for (int i = 0; i < ssize(regions_); ++i) {
        ...
        vertex_set.insert(vertex_set.begin(), num_points, ConvexSets::value_type{*regions_[i]});
        // Add time scaling set.
        vertex_set.emplace_back(time_scaling_set);

        vertices_.emplace_back(
                traj_opt_.gcs_.AddVertex(CartesianProduct(vertex_set), fmt::format("{}: Region{}", name_, i)));
        traj_opt->vertex_to_subgraph_[vertices_.back()] = this;
    }

    r_trajectory_ = BezierCurve<double>(0, 1, MatrixXd::Zero(num_positions(), order + 1));
    ...
    for (int idx = 0; idx < ssize(edges_between_regions); ++idx) {
        ...
        const auto path_continuity_constraint = std::make_shared<LinearEqualityConstraint>(A, this_edge_offset);
...
void Subgraph::ThrowsForInvalidConvexityRadius() const {
    for (int i = 0; i < ssize(regions_); ++i) {
        for (const int& j : continuous_revolute_joints()) {
            auto [min_value, max_value] = GetMinimumAndMaximumValueAlongDimension(*regions_[i], j);
            if (max_value - min_value >= M_PI) {
                throw std::runtime_error(
                        fmt::format("GcsTrajectoryOptimization: Region at index {} is wider than π "
...
void Subgraph::AddTimeCost(double weight) {
    ...
    auto time_cost = std::make_shared<LinearCost>(weight * Eigen::VectorXd::Ones(1), 0.0);
...
void Subgraph::AddPathLengthCost(const MatrixXd& weight_matrix) {
    ...
    */
    ...
    if (order() == 0) {
        throw std::runtime_error("Path length cost is not defined for a set of order 0.");
    }

    MatrixXd A(num_positions(), 2 * num_positions());
    A << weight_matrix, -weight_matrix;
    const auto path_length_cost = std::make_shared<L2NormCost>(A, VectorXd::Zero(num_positions()));

    for (Vertex* v : vertices_) {
        auto control_points = GetControlPoints(*v);
        for (int i = 0; i < control_points.cols() - 1; ++i) {
            v->AddCost(Binding<L2NormCost>(path_length_cost, {control_points.col(i + 1), control_points.col(i)}));
        }
    }
}

void Subgraph::AddPathLengthCost(double weight) {
    const MatrixXd weight_matrix = weight * MatrixXd::Identity(num_positions(), num_positions());
    return Subgraph::AddPathLengthCost(weight_matrix);
}

void Subgraph::AddVelocityBounds(const Eigen::Ref<const VectorXd>& lb, const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_THROW_UNLESS(lb.size() == num_positions());
    DRAKE_THROW_UNLESS(ub.size() == num_positions());
    if (order() == 0) {
        throw std::runtime_error("Velocity Bounds are not defined for a set of order 0.");
    }
    ...
    SparseMatrix<double> H_lb(order_ /* number of control points for one row of ṙ(s)*/,
                              order_ + 2 /* number of control points for one row of r(s) + 1*/);
    H_lb.leftCols(order_ + 1) = r_trajectory_.AsLinearInControlPoints(1).transpose();
    SparseMatrix<double> H_ub = H_lb;
    for (int i = 0; i < num_positions(); ++i) {
        // Lower bound.  0 <= ṙ(s).row(i) - h * lb <= inf.
        H_lb.rightCols<1>() = VectorXd::Constant(order_, -lb[i]).sparseView();
        const auto lb_constraint = std::make_shared<LinearConstraint>(H_lb, kVecZero, kVecInf);
        // Upper bound. -inf <= ṙ(s).row(i) - h * ub <= 0.
        H_ub.rightCols<1>() = VectorXd::Constant(order_, -ub[i]).sparseView();
        const auto ub_constraint = std::make_shared<LinearConstraint>(H_ub, -kVecInf, kVecZero);
        for (Vertex* v : vertices_) {
...
void Subgraph::AddNonlinearDerivativeBounds(const Eigen::Ref<const Eigen::VectorXd>& lb,
    ...
    if (derivative_order > order()) {
        throw std::runtime_error("Derivative order must be less than or equal to the set order.");
    }
    ...
    // positions, rather than over the control points.
    solvers::VectorXDecisionVariable vars(order_ + 2);
    SparseMatrix<double> M_transpose = r_trajectory_.AsLinearInControlPoints(derivative_order).transpose();
    ...
    Eigen::MatrixXd H(2 * rdot_control_points,
                      order_ + 2);  // number of control points for one row of r(s) + 1
    H.block(0, 0, M_transpose.rows(), M_transpose.cols()) = M_transpose;
    H.block(M_transpose.rows(), 0, M_transpose.rows(), M_transpose.cols()) = -M_transpose;

    for (int i = 0; i < num_positions(); ++i) {
        // Update the bounds for each position.
        H.block(0, order_ + 1, rdot_control_points, 1) =
                VectorXd::Constant(rdot_control_points, -std::pow(h0, derivative_order - 1) * lb[i]);
        H.block(rdot_control_points, order_ + 1, rdot_control_points, 1) =
                VectorXd::Constant(rdot_control_points, std::pow(h0, derivative_order - 1) * ub[i]);
        ...
        const auto nonlinear_derivative_constraint =
                std::make_shared<NonlinearDerivativeConstraint>(M_transpose, lb[i], ub[i], derivative_order);
        for (Vertex* v : vertices_) {
            vars << GetControlPoints(*v).row(i).transpose(), GetTimeScaling(*v);
            // Add convex surrogate.
            v->AddConstraint(Binding<LinearConstraint>(normalized_path_derivative_constraint, vars),
                             {Transcription::kRelaxation});
            // Add nonlinear constraint.
            v->AddConstraint(Binding<NonlinearDerivativeConstraint>(nonlinear_derivative_constraint, vars),
...
void Subgraph::AddPathContinuityConstraints(int continuity_order) {
    if (continuity_order == 0) {
        throw std::runtime_error("Path continuity is enforced by default. Choose a higher order.");
    }
    ...
    SparseMatrix<double> Mu_transpose =
            r_trajectory_.AsLinearInControlPoints(continuity_order).col(order_ - continuity_order).transpose();

    SparseMatrix<double> Mv_transpose = r_trajectory_.AsLinearInControlPoints(continuity_order).col(0).transpose();
    ...
    const auto continuity_constraint = std::make_shared<LinearEqualityConstraint>(A, VectorXd::Zero(1));

    for (int i = 0; i < num_positions(); ++i) {
        for (Edge* edge : edges_) {
            // Add continuity constraints.
            edge->AddConstraint(Binding<LinearEqualityConstraint>(
                    continuity_constraint, {GetControlPoints(edge->u()).row(i), GetControlPoints(edge->v()).row(i)}));
        }
    }
}
...
void Subgraph::AddContinuityConstraints(int continuity_order) {
    ...
    const auto nonlinear_continuity_constraint =
            std::make_shared<NonlinearContinuityConstraint>(Mu_transpose, Mv_transpose, continuity_order);
    ...
    const auto path_continuity_constraint = std::make_shared<LinearEqualityConstraint>(A, VectorXd::Zero(1));

    for (int i = 0; i < num_positions(); ++i) {
        for (Edge* edge : edges_) {
            // Add convex surrogate.
            edge->AddConstraint(
                    Binding<LinearEqualityConstraint>(path_continuity_constraint, {GetControlPoints(edge->u()).row(i),
                                                                                   GetControlPoints(edge->v()).row(i)}),
                    {Transcription::kRelaxation});

            // Add nonlinear constraint.
            vars << GetControlPoints(edge->u()).row(i).transpose(), GetTimeScaling(edge->u()),
                    GetControlPoints(edge->v()).row(i).transpose(), GetTimeScaling(edge->v());
            edge->AddConstraint(Binding<NonlinearContinuityConstraint>(nonlinear_continuity_constraint, vars),
                                {Transcription::kMIP, Transcription::kRestriction});
        }
    }
}
...
Eigen::Map<const MatrixX<symbolic::Variable>> Subgraph::GetControlPoints(
    ...
    return Eigen::Map<const MatrixX<symbolic::Variable>>(v.x().data(), num_positions(), order_ + 1);
}

symbolic::Variable Subgraph::GetTimeScaling(const geometry::optimization::GraphOfConvexSets::Vertex& v) const {
    DRAKE_DEMAND(v.x().size() == num_positions() * (order_ + 1) + 1);
    return v.x()(v.x().size() - 1);
}

EdgesBetweenSubgraphs::EdgesBetweenSubgraphs(const Subgraph& from_subgraph,
                                             const Subgraph& to_subgraph,
                                             const ConvexSet* subspace,
                                             GcsTrajectoryOptimization* traj_opt)
    : traj_opt_(*traj_opt), from_subgraph_(from_subgraph), to_subgraph_(to_subgraph) {
    // Formulate edge costs and constraints.
    if (subspace != nullptr) {
        if (subspace->ambient_dimension() != num_positions()) {
            throw std::runtime_error("Subspace dimension must match the number of positions.");
        }
        if (typeid(*subspace) != typeid(Point) && typeid(*subspace) != typeid(HPolyhedron)) {
            throw std::runtime_error("Subspace must be a Point or HPolyhedron.");
        }
    }

    ur_trajectory_ = BezierCurve<double>(0, 1, MatrixXd::Zero(num_positions(), from_subgraph_.order() + 1));
    vr_trajectory_ = BezierCurve<double>(0, 1, MatrixXd::Zero(num_positions(), to_subgraph_.order() + 1));
    ...
    if (subspace != nullptr) {
        std::vector<std::pair<double, double>> subspace_revolute_joints_bbox =
                GetMinimumAndMaximumValueAlongDimension(*subspace, continuous_revolute_joints());
        for (const auto& region : from_subgraph.regions()) {
            std::vector<std::pair<double, double>> set_revolute_joints_bbox =
                    GetMinimumAndMaximumValueAlongDimension(*region, continuous_revolute_joints());
            sets_A_subspace_offset.push_back(
                    ComputeOffsetContinuousRevoluteJoints(num_positions(), continuous_revolute_joints(),
                                                          set_revolute_joints_bbox, subspace_revolute_joints_bbox));
        }
    }

    const std::vector<std::tuple<int, int, Eigen::VectorXd>> edge_data =
            CalcPairwiseIntersections(from_subgraph.regions(), to_subgraph.regions(), continuous_revolute_joints());
    for (const auto& edge : edge_data) {
        ...
        if (subspace != nullptr) {
            ...
            if (!RegionsConnectThroughSubspace(*from_subgraph.regions()[i], *to_subgraph.regions()[j], *subspace,
        ...
        // x = [u_controls.col(order); v_controls.col(0)].
        const auto path_continuity_constraint = std::make_shared<LinearEqualityConstraint>(A, -edge_offset);
        uv_edge->AddConstraint(
                Binding<Constraint>(path_continuity_constraint,
                                    ConcatenateVariableRefList({GetControlPointsU(*uv_edge).col(from_subgraph_.order()),
        ...
        if (subspace != nullptr) {
            ...
            if (subspace_point.has_value()) {
                ...
                const auto subgraph_constraint = std::make_shared<LinearEqualityConstraint>(
                ...
                const VectorX<symbolic::Variable> x = prog.NewContinuousVariables(num_positions(), "x");
                // To translate the HPolyhedron, rewrite A*(x+offset)<=b as
                // Ax<=b-A*offset.
                prog.AddLinearConstraint(
                        hpoly_A, Eigen::VectorXd::Constant(hpoly_b.size(), -std::numeric_limits<double>::infinity()),
...
bool EdgesBetweenSubgraphs::RegionsConnectThroughSubspace(const ConvexSet& A,
                                                          const ConvexSet& B,
                                                          const ConvexSet& subspace,
    ...
    DRAKE_THROW_UNLESS(maybe_set_B_offset.has_value() == maybe_subspace_offset.has_value());
    if (maybe_set_B_offset.has_value()) {
        DRAKE_THROW_UNLESS(maybe_set_B_offset.value().size() == A.ambient_dimension());
        DRAKE_THROW_UNLESS(maybe_subspace_offset.value().size() == A.ambient_dimension());
    }
    if (std::optional<VectorXd> subspace_point = subspace.MaybeGetPoint()) {
        ...
        if (maybe_set_B_offset.has_value()) {
            ...
            const VectorXd point_in_A_coords = *subspace_point - maybe_subspace_offset.value();
            // Compute the value of the point such that it can be directly checked for
            // containment in B. We take the value of the point that can be checked
            // for A, and add the offset form A to B.
            const VectorXd point_in_B_coords = point_in_A_coords + maybe_set_B_offset.value();
        ...
        equality_constraint_matrix.leftCols(dimension) = MatrixXd::Identity(dimension, dimension);
        equality_constraint_matrix.rightCols(dimension) = -MatrixXd::Identity(dimension, dimension);
        // y = x + B_offset as [I, -I][y; x] = [B_offset].
        prog.AddLinearEqualityConstraint(equality_constraint_matrix, maybe_set_B_offset.value(), {x, y});
        // z = x + subspace_offset as [I, -I][z; x] = [subspace_offset]
        prog.AddLinearEqualityConstraint(equality_constraint_matrix, maybe_subspace_offset.value(), {x, z});
...
void EdgesBetweenSubgraphs::AddVelocityBounds(const Eigen::Ref<const VectorXd>& lb,
    ...
    if (from_subgraph_.order() > 0) {
        ...
        SparseMatrix<double> m = ur_trajectory_.AsLinearInControlPoints(1).col(from_subgraph_.order() - 1).transpose();
        SparseMatrix<double> H_lb(1 /* we are only constraining the last point in the u set */,
                                  from_subgraph_.order() + 2 /* number of control points for one row of r(s) + 1*/);
        ...
        for (int i = 0; i < num_positions(); ++i) {
            ...
            for (Edge* edge : edges_) {
                ...
                vars << GetControlPointsU(*edge).row(i).transpose(), GetTimeScalingU(*edge);
    ...
    if (to_subgraph_.order() > 0) {
        ...
        SparseMatrix<double> m = vr_trajectory_.AsLinearInControlPoints(1).col(0).transpose();
        SparseMatrix<double> H_lb(1 /* we are only constraining the last point in the u set */,
                                  to_subgraph_.order() + 2 /* number of control points for one row of r(s) + 1*/);
        ...
        for (int i = 0; i < num_positions(); ++i) {
            ...
            for (Edge* edge : edges_) {
                // vars = [control_points.row(i).T; time_scaling]
                vars << GetControlPointsV(*edge).row(i).transpose(), GetTimeScalingV(*edge);
                edge->AddConstraint(Binding<LinearConstraint>(lb_constraint, vars));
                edge->AddConstraint(Binding<LinearConstraint>(ub_constraint, vars));
            }
        }
...
void EdgesBetweenSubgraphs::AddNonlinearDerivativeBounds(const Eigen::Ref<const Eigen::VectorXd>& lb,
                                                         const Eigen::Ref<const Eigen::VectorXd>& ub,
                                                         int derivative_order) {
    DRAKE_THROW_UNLESS(lb.size() == num_positions());
    DRAKE_THROW_UNLESS(ub.size() == num_positions());
    ...
    if (derivative_order > from_subgraph_.order() && derivative_order > to_subgraph_.order()) {
        throw std::runtime_error(
                fmt::format("Cannot add derivative bounds to subgraph edges where both subgraphs "
    ...
    if (from_subgraph_.order() >= derivative_order) {
        ...
        solvers::VectorXDecisionVariable vars(from_subgraph_.order() + 2);
        SparseMatrix<double> M_transpose = ur_trajectory_.AsLinearInControlPoints(derivative_order)
                                                   .col(from_subgraph_.order() - derivative_order)
                                                   .transpose();

        Eigen::MatrixXd H(2 /* we are only constraining the last point in the u set */,
                          from_subgraph_.order() + 2 /* number of control points for one row of r(s) + 1*/);
        ...
        for (int i = 0; i < num_positions(); ++i) {
            ...
            for (Edge* edge : edges_) {
                vars << GetControlPointsU(*edge).row(i).transpose(), GetTimeScalingU(*edge);
                // Add convex surrogate.
                edge->AddConstraint(Binding<LinearConstraint>(convex_derivative_constraint, vars),
                                    {Transcription::kRelaxation});
                // Add nonlinear constraint.
                edge->AddConstraint(Binding<NonlinearDerivativeConstraint>(nonlinear_derivative_constraint, vars),
                                    {Transcription::kMIP, Transcription::kRestriction});
            }
        }
    }
    ...
    if (to_subgraph_.order() >= derivative_order) {
        ...
        solvers::VectorXDecisionVariable vars(to_subgraph_.order() + 2);
        SparseMatrix<double> M_transpose = vr_trajectory_.AsLinearInControlPoints(derivative_order).col(0).transpose();

        Eigen::MatrixXd H(2 /* we are only constraining the last point in the v set */,
                          to_subgraph_.order() + 2 /* number of control points for one row of r(s) + 1*/);
        ...
        for (int i = 0; i < num_positions(); ++i) {
            ...
            const auto nonlinear_derivative_constraint =
                    std::make_shared<NonlinearDerivativeConstraint>(M_transpose, lb[i], ub[i], derivative_order);
            for (Edge* edge : edges_) {
                vars << GetControlPointsV(*edge).row(i).transpose(), GetTimeScalingU(*edge);
                // Add convex surrogate.
                edge->AddConstraint(Binding<LinearConstraint>(convex_derivative_constraint, vars),
                                    {Transcription::kRelaxation});
                // Add nonlinear constraint.
...
void EdgesBetweenSubgraphs::AddZeroDerivativeConstraints(int derivative_order) {
    ...
    if (from_subgraph_.order() >= derivative_order) {
        ...
        // Equality constraint.
        // (dᴺr(s) / dsᴺ).row(i) = 0.
        const auto zero_derivative_constraint = std::make_shared<LinearEqualityConstraint>(M_transpose, kVecZero);
        for (int i = 0; i < num_positions(); ++i) {
            for (Edge* edge : edges_) {
                edge->AddConstraint(Binding<LinearEqualityConstraint>(zero_derivative_constraint,
...
void EdgesBetweenSubgraphs::AddPathContinuityConstraints(int continuity_order) {
    ...
    if (continuity_order > from_subgraph_.order() || continuity_order > to_subgraph_.order()) {
        throw std::runtime_error(
    ...
    SparseMatrix<double> Mu_transpose = ur_trajectory_.AsLinearInControlPoints(continuity_order)
                                                .col(from_subgraph_.order() - continuity_order)
                                                .transpose();

    SparseMatrix<double> Mv_transpose = vr_trajectory_.AsLinearInControlPoints(continuity_order).col(0).transpose();
    ...
    // continuity in each dimension. The number of columns matches the number of
    // control points for one row of r_u(s) and r_v(s).
    SparseMatrix<double> A(1, (from_subgraph_.order() + 1) + (to_subgraph_.order() + 1));
    ...
    for (int i = 0; i < num_positions(); ++i) {
        for (Edge* edge : edges_) {
            // Add continuity constraints.
            edge->AddConstraint(Binding<LinearEqualityConstraint>(
                    continuity_constraint, {GetControlPointsU(*edge).row(i), GetControlPointsV(*edge).row(i)}));
        }
    }
}
...
void EdgesBetweenSubgraphs::AddContinuityConstraints(int continuity_order) {
    ...
    const auto nonlinear_continuity_constraint =
            std::make_shared<NonlinearContinuityConstraint>(Mu_transpose, Mv_transpose, continuity_order);
    solvers::VectorXDecisionVariable vars(from_subgraph_.order() + 2 + to_subgraph_.order() + 2);
    ...
    for (int i = 0; i < num_positions(); ++i) {
        for (Edge* edge : edges_) {
            // Add convex surrogate.
            edge->AddConstraint(
                    Binding<LinearEqualityConstraint>(path_continuity_constraint, {GetControlPointsU(*edge).row(i),
                                                                                   GetControlPointsV(*edge).row(i)}),
                    {Transcription::kRelaxation});

            // Add nonlinear constraint.
            vars << GetControlPointsU(*edge).row(i).transpose(), GetTimeScalingU(*edge),
                    GetControlPointsV(*edge).row(i).transpose(), GetTimeScalingV(*edge);
            edge->AddConstraint(Binding<NonlinearContinuityConstraint>(nonlinear_continuity_constraint, vars),
                                {Transcription::kMIP, Transcription::kRestriction});
        }
    }
...
std::vector<const GraphOfConvexSets::Edge*> EdgesBetweenSubgraphs::Edges() const {
    std::vector<const GraphOfConvexSets::Edge*> edges;
    edges.reserve(edges_.size());
    for (const auto& e : edges_) {
        edges.push_back(e);
...
Eigen::Map<const MatrixX<symbolic::Variable>> EdgesBetweenSubgraphs::GetControlPointsU(
        const geometry::optimization::GraphOfConvexSets::Edge& e) const {
    DRAKE_DEMAND(e.xu().size() == num_positions() * (from_subgraph_.order() + 1) + 1);
    return Eigen::Map<const MatrixX<symbolic::Variable>>(e.xu().data(), num_positions(), from_subgraph_.order() + 1);
}

Eigen::Map<const MatrixX<symbolic::Variable>> EdgesBetweenSubgraphs::GetControlPointsV(
        const geometry::optimization::GraphOfConvexSets::Edge& e) const {
    DRAKE_DEMAND(e.xv().size() == num_positions() * (to_subgraph_.order() + 1) + 1);
    return Eigen::Map<const MatrixX<symbolic::Variable>>(e.xv().data(), num_positions(), to_subgraph_.order() + 1);
}
...
GcsTrajectoryOptimization::GcsTrajectoryOptimization(int num_positions, std::vector<int> continuous_revolute_joints)
    : num_positions_(num_positions), continuous_revolute_joints_(std::move(continuous_revolute_joints)) {
    DRAKE_THROW_UNLESS(num_positions >= 1);
    ThrowsForInvalidContinuousJointsList(num_positions, continuous_revolute_joints_);
}
...
Subgraph& GcsTrajectoryOptimization::AddRegions(const ConvexSets& regions,
    ...
    Subgraph* subgraph =
            new Subgraph(regions, edges_between_regions, order, h_min, h_max, std::move(name), this, edge_offsets);
...
Subgraph& GcsTrajectoryOptimization::AddRegions(
        const ConvexSets& regions, int order, double h_min, double h_max, std::string name) {
    ...
    for (int i = 0; i < ssize(edge_data); ++i) {
        edges_between_regions.emplace_back(std::get<0>(edge_data[i]), std::get<1>(edge_data[i]));
        edge_offsets.emplace_back(std::get<2>(edge_data[i]));
    }

    return GcsTrajectoryOptimization::AddRegions(regions, edges_between_regions, order, h_min, h_max, std::move(name),
                                                 edge_offsets);
}

void GcsTrajectoryOptimization::RemoveSubgraph(const Subgraph& subgraph) {
    // Check if the subgraph is in the list of subgraphs.
    if (!std::any_of(subgraphs_.begin(), subgraphs_.end(), [&](const std::unique_ptr<Subgraph>& s) {
            return s.get() == &subgraph;
        })) {
        throw std::runtime_error(fmt::format("Subgraph {} is not registered with `this`", subgraph.name()));
    }

    // Remove the underlying edges between subgraphs from the gcs problem.
    for (const std::unique_ptr<EdgesBetweenSubgraphs>& subgraph_edge : subgraph_edges_) {
        if (&subgraph_edge->from_subgraph_ == &subgraph || &subgraph_edge->to_subgraph_ == &subgraph) {
    ...
    subgraph_edges_.erase(std::remove_if(subgraph_edges_.begin(), subgraph_edges_.end(),
                                         [&](const std::unique_ptr<EdgesBetweenSubgraphs>& e) {
                                             return &e->from_subgraph_ == &subgraph || &e->to_subgraph_ == &subgraph;
                                         }),
                          subgraph_edges_.end());
...
EdgesBetweenSubgraphs& GcsTrajectoryOptimization::AddEdges(const Subgraph& from_subgraph,
                                                           const Subgraph& to_subgraph,
                                                           const ConvexSet* subspace) {
    EdgesBetweenSubgraphs* subgraph_edge = new EdgesBetweenSubgraphs(from_subgraph, to_subgraph, subspace, this);
...
void GcsTrajectoryOptimization::AddVelocityBounds(const Eigen::Ref<const VectorXd>& lb,
                                                  const Eigen::Ref<const VectorXd>& ub) {
    DRAKE_THROW_UNLESS(lb.size() == num_positions());
    DRAKE_THROW_UNLESS(ub.size() == num_positions());
...
void GcsTrajectoryOptimization::AddNonlinearDerivativeBounds(const Eigen::Ref<const Eigen::VectorXd>& lb,
                                                             const Eigen::Ref<const Eigen::VectorXd>& ub,
                                                             int derivative_order) {
    DRAKE_THROW_UNLESS(lb.size() == num_positions());
    DRAKE_THROW_UNLESS(ub.size() == num_positions());
...
void GcsTrajectoryOptimization::AddPathContinuityConstraints(int continuity_order) {
    if (continuity_order == 0) {
        throw std::runtime_error("Path continuity is enforced by default. Choose a higher order.");
    }
    if (continuity_order < 1) {
    ...
    for (std::unique_ptr<EdgesBetweenSubgraphs>& subgraph_edge : subgraph_edges_) {
        if (subgraph_edge->from_subgraph_.order() >= continuity_order &&
            subgraph_edge->to_subgraph_.order() >= continuity_order) {
            subgraph_edge->AddPathContinuityConstraints(continuity_order);
        }
...
std::pair<CompositeTrajectory<double>, solvers::MathematicalProgramResult> GcsTrajectoryOptimization::SolvePath(
        const Subgraph& source, const Subgraph& target, const GraphOfConvexSetsOptions& specified_options) {
    ...
    solvers::MathematicalProgramResult result = gcs_.SolveShortestPath(*source_vertex, *target_vertex, options);
    if (!result.is_success()) {
        return {CompositeTrajectory<double>({}), result};
    }

    const double kTolerance = 1.0;  // take any path we can get.
    std::vector<const Edge*> path_edges = gcs_.GetSolutionPath(*source_vertex, *target_vertex, result, kTolerance);
...
GcsTrajectoryOptimization::SolveConvexRestriction(const std::vector<const Vertex*>& active_vertices,
    ...
    for (size_t i = 0; i < active_vertices.size() - 1; ++i) {
        ...
        for (const Edge* e : active_vertices[i]->outgoing_edges()) {
            if (e->v().id() == active_vertices[i + 1]->id()) {
                if (vertices_connected) {
                    throw std::runtime_error(
                            fmt::format("Vertex: {} is connected to vertex: {} through multiple edges.",
        ...
        if (!vertices_connected) {
            throw std::runtime_error(fmt::format("Vertex: {} is not connected to vertex: {}.",
                                                 active_vertices[i]->name(), active_vertices[i + 1]->name()));
        }
    }

    solvers::MathematicalProgramResult result = gcs_.SolveConvexRestriction(active_edges, options);
    if (!result.is_success()) {
        return {CompositeTrajectory<double>({}), result};
    }
...
CompositeTrajectory<double> GcsTrajectoryOptimization::ReconstructTrajectoryFromSolutionPath(
        std::vector<const Edge*> edges, const solvers::MathematicalProgramResult& result) {
    ...
    for (const Edge* e : edges) {
        ...
        const MatrixX<double> edge_path_points =
                phi_inv *
                Eigen::Map<MatrixX<double>>(result.GetSolution(e->xu()).data(), num_positions(), num_control_points);
        ...
        // derivatives.
        if (!(num_control_points == 1 && vertex_to_subgraph_[&e->u()]->h_min_ == 0)) {
            bezier_curves.emplace_back(
                    std::make_unique<BezierCurve<double>>(start_time, start_time + h, edge_path_points));
        }
    }
    ...
    const int num_control_points = vertex_to_subgraph_[&last_edge.v()]->order() + 1;
    const MatrixX<double> edge_path_points =
            phi_inv *
            Eigen::Map<MatrixX<double>>(result.GetSolution(last_edge.xv()).data(), num_positions(), num_control_points);
    ...
    if (!(num_control_points == 1 && vertex_to_subgraph_[&last_edge.v()]->h_min_ == 0)) {
        bezier_curves.emplace_back(std::make_unique<BezierCurve<double>>(start_time, start_time + h, edge_path_points));
    }
...
std::vector<EdgesBetweenSubgraphs*> GcsTrajectoryOptimization::GetEdgesBetweenSubgraphs() const {
...
trajectories::CompositeTrajectory<double> GcsTrajectoryOptimization::NormalizeSegmentTimes(
    ...
    for (int i = 0; i < trajectory.get_number_of_segments(); ++i) {
        ...
        if (const BezierCurve<double>* gcs_segment = dynamic_cast<const BezierCurve<double>*>(&trajectory.segment(i))) {
            normalized_bezier_curves.emplace_back(
                    std::make_unique<BezierCurve<double>>(start_time, start_time + 1.0, gcs_segment->control_points()));
            start_time += 1.0;
        } else {
            throw std::runtime_error(
...
double UnwrapAngle(const double angle, const int round) {
    const int twopi_factors_to_remove = static_cast<int>(std::floor(angle / (2 * M_PI))) - round;
    return angle - 2 * M_PI * twopi_factors_to_remove;
}
}  // namespace

trajectories::CompositeTrajectory<double> GcsTrajectoryOptimization::UnwrapToContinousTrajectory(
    ...
    if (starting_rounds.has_value()) {
        DRAKE_THROW_UNLESS(starting_rounds->size() == continuous_revolute_joints.size());
    }
    ...
    geometry::optimization::internal::ThrowsForInvalidContinuousJointsList(dim, continuous_revolute_joints);
    ...
    for (int i = 0; i < gcs_trajectory.get_number_of_segments(); ++i) {
        ...
        const auto* bezier_segment = dynamic_cast<const BezierCurve<double>*>(&traj_segment);
        ...
        const Eigen::MatrixXd& old_control_points = bezier_segment->control_points();
        ...
        if (i == 0) {
            ...
            if (starting_rounds.has_value()) {
                for (int j = 0; j < ssize(continuous_revolute_joints); ++j) {
                    ...
                    const double joint_shift =
                            old_start(joint_index) - UnwrapAngle(old_start(joint_index), start_round);
            ...
            for (const int joint_index : continuous_revolute_joints) {
                const double joint_shift = old_start(joint_index) - last_segment_finish(joint_index);
        ...
        for (int j = 0; j < ssize(continuous_revolute_joints); ++j) {
            ...
            new_control_points.row(joint_index) -= Eigen::VectorXd::Constant(old_control_points.cols(), shift.at(j));
        }
        last_segment_finish = new_control_points.rightCols(1);
        unwrapped_trajectories.emplace_back(std::make_unique<BezierCurve<double>>(
                bezier_segment->start_time(), bezier_segment->end_time(), new_control_points));
    }
...
std::vector<int> GetContinuousRevoluteJointIndices(const multibody::MultibodyPlant<double>& plant) {
    ...
    for (JointIndex i : plant.GetJointIndices()) {
        ...
        if (joint.type_name() == "revolute") {
            if (joint.position_lower_limits()[0] == -std::numeric_limits<float>::infinity() &&
                joint.position_upper_limits()[0] == std::numeric_limits<float>::infinity()) {
        ...
        if (joint.type_name() == "planar") {
            if (joint.position_lower_limits()[2] == -std::numeric_limits<float>::infinity() &&
                joint.position_upper_limits()[2] == std::numeric_limits<float>::infinity()) {
