...
typedef std::pair<std::unique_ptr<System<AutoDiffXd>>, std::unique_ptr<Context<AutoDiffXd>>> OwnedPair;

OwnedPair MakeAutoDiffXd(const systems::System<double>& system, const systems::Context<double>& context) {
...
DirectCollocationConstraint::DirectCollocationConstraint(
    ...
    : DirectCollocationConstraint(MakeAutoDiffXd(system, context),
                                  nullptr,  // System
                                  nullptr,
                                  nullptr,
                                  nullptr,  // Contexts
...
DirectCollocationConstraint::DirectCollocationConstraint(
        ...
        bool assume_non_continuous_states_are_fixed)
    : DirectCollocationConstraint(OwnedPair{nullptr, nullptr},
                                  &system,
                                  context_sample,
                                  context_next_sample,
                                  context_collocation,
...
DirectCollocationConstraint::DirectCollocationConstraint(
    ...
    : Constraint(CheckAndReturnStates(num_states),
      ...
      context_next_sample_(owned_context_ ? owned_context_.get() : context_next_sample),
      context_collocation_(owned_context_ ? owned_context_.get() : context_collocation),
...
void DirectCollocationConstraint::CalcDynamics(const AutoDiffVecXd& x_with_dvars,
                                               const AutoDiffVecXd& u_with_dvars,
                                               Context<AutoDiffXd>* context,
                                               AutoDiffVecXd* xdot_with_dvars) const {
    ...
    std::tie(x_with_dxu, u_with_dxu) = InitializeAutoDiffTuple(ExtractValue(x_with_dvars), ExtractValue(u_with_dvars));

    if (input_port_ &&
        (!input_port_->HasValue(*context) || !AreAutoDiffVecXdEqual(u_with_dxu, input_port_->Eval(*context)))) {
        input_port_->FixValue(context, u_with_dxu);
    }
    if (!AreAutoDiffVecXdEqual(x_with_dxu, context->get_continuous_state_vector().CopyToVector())) {
        context->SetContinuousState(x_with_dxu);
    }

    AutoDiffVecXd xdot_with_dxu = system_.EvalTimeDerivatives(*context).CopyToVector();
    ...
    InitializeAutoDiff(
            ...
            dxdot_dxu.leftCols(num_states_) * ExtractGradient(x_with_dvars) +
                    dxdot_dxu.rightCols(num_inputs_) * ExtractGradient(u_with_dvars,
                                                                       // pass in num_derivatives in case u is empty.
                                                                       x_with_dvars[0].derivatives().size()),
            xdot_with_dvars);
}

void DirectCollocationConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    AutoDiffVecXd y_t;
...
void DirectCollocationConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
...
void DirectCollocationConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
                                         VectorX<symbolic::Expression>*) const {
    throw std::logic_error("DirectCollocationConstraint does not support symbolic evaluation.");
}

Binding<Constraint> AddDirectCollocationConstraint(std::shared_ptr<DirectCollocationConstraint> constraint,
    ...
    return prog->AddConstraint(constraint, {time_step, state, next_state, input, next_input});
}
...
DirectCollocation::DirectCollocation(const System<double>* system,
    ...
    for (int i = 0; i < N() - 1; ++i) {
        auto constraint = std::make_shared<DirectCollocationConstraint>(
                *system_ad_, sample_contexts_[i].get(), sample_contexts_[i + 1].get(), context_ad_.get(),
        ...
        this->prog()
                ...
                .evaluator()
                ->set_description(fmt::format("collocation constraint for segment {}", i));
    }
}
...
void DirectCollocation::DoAddRunningCost(const symbolic::Expression& g) {
    ...
    for (int i = 1; i <= N() - 2; i++) {
        prog().AddCost(SubstitutePlaceholderVariables(g * (h_vars()(i - 1) + h_vars()(i)) / 2, i));
    }
    prog().AddCost(SubstitutePlaceholderVariables(g * h_vars()(N() - 2) / 2, N() - 1));
}

PiecewisePolynomial<double> DirectCollocation::ReconstructInputTrajectory(
        const solvers::MathematicalProgramResult& result) const {
    const InputPort<double>* input_port = system_->get_input_port_selection(input_port_index_);
    if (!input_port) {
...
PiecewisePolynomial<double> DirectCollocation::ReconstructStateTrajectory(
    ...
    if (input_port) {
        input_port_value =
                &input_port->FixValue(context_.get(), system_->AllocateInputVector(*input_port)->get_value());
    }
    ...
    for (int i = 0; i < N(); i++) {
        ...
        if (input_port) {
            input_port_value->GetMutableVectorData<double>()->SetFromVector(result.GetSolution(input(i)));
        }
