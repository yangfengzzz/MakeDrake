...
public:
    ...
    /// in an error.
    const solvers::VectorDecisionVariable<1>& time() const { return placeholder_t_var_; }
    ...
    const solvers::VectorXDecisionVariable& state() const { return placeholder_x_vars_; }
    ...
    /// AddRunningCost.  Passing these variables directly into
    /// objectives/constraints for the parent classes will result in an error.
    const solvers::VectorXDecisionVariable& input() const { return placeholder_u_vars_; }
    ...
    /// initial time index.
    Eigen::VectorBlock<const solvers::VectorXDecisionVariable> initial_state() const { return state(0); }

    /// Returns the decision variables associated with the state, x, at the final
    /// time index.
    Eigen::VectorBlock<const solvers::VectorXDecisionVariable> final_state() const { return state(N_ - 1); }
    ...
    std::vector<solvers::Binding<C>> AddConstraintToAllKnotPoints(
            std::shared_ptr<C> constraint, const Eigen::Ref<const VectorX<symbolic::Variable>>& vars) {
        std::vector<solvers::Binding<C>> bindings;
        for (int i = 0; i < N_; ++i) {
            bindings.push_back(prog_.AddConstraint(constraint, sequential_expression_manager_.GetVariables(vars, i)));
        }
        return bindings;
    }
    ...
    std::vector<solvers::Binding<solvers::Constraint>> AddConstraintToAllKnotPoints(const symbolic::Formula& f) {
        ...
        for (int i = 0; i < N_; i++) {
            bindings.push_back(prog_.AddConstraint(SubstitutePlaceholderVariables(f, i)));
        }
        return bindings;
    }
    ...
    std::vector<solvers::Binding<solvers::Constraint>> AddConstraintToAllKnotPoints(
    ...
    typedef std::function<void(const Eigen::Ref<const Eigen::VectorXd>& sample_times,
     ...
     * programming solver as the default solver.
     */
    solvers::Binding<solvers::VisualizationCallback> AddCompleteTrajectoryCallback(
            const CompleteTrajectoryCallback& callback, const std::vector<std::string>& names);
...
protected:
    ...
    bool time_steps_are_decision_variables() const { return time_steps_are_decision_variables_; }
...
private:
    ...
    symbolic::Substitution ConstructPlaceholderVariableSubstitution(int interval_index) const;
