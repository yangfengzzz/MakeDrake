...
std::vector<int> GetQuaternionDofStartIndices(const MultibodyPlant<double>& plant) {
    ...
    for (JointIndex joint_index(0); joint_index < plant.num_joints(); ++joint_index) {
...
Eigen::VectorXd GetDefaultDistanceWeights(int num_positions, const std::vector<int>& quaternion_dof_start_indices) {
    // The default weight for all joints is 1, except for quaternion DoF where
    // only the first weight may be non-zero.
    Eigen::VectorXd default_distance_weights = Eigen::VectorXd::Ones(num_positions);
...
Eigen::VectorXd GetDistanceWeights(const MultibodyPlant<double>& plant,
                                   const std::vector<int>& quaternion_dof_start_indices,
                                   const std::map<JointIndex, Eigen::VectorXd>& joint_distance_weights) {
    // The default weight for all joints is 1, except for quaternion DoF where
    // only the first weight may be non-zero.
    Eigen::VectorXd distance_weights = GetDefaultDistanceWeights(plant.num_positions(), quaternion_dof_start_indices);
    ...
    for (const auto& [joint_index, joint_weights] : joint_distance_weights) {
        ...
        if (joint.num_positions() != joint_weights.size()) {
            throw std::runtime_error(
                    fmt::format("Provided distance weights for joint {} [{}] with type [{}] are [{}] "
                                "which do not match that joint's num_positions {}",
                                joint_index, joint.name(), joint.type_name(), fmt_eigen(joint_weights.transpose()),
                                joint.num_positions()));
        }
        ...
        for (int i = 0; i < joint_weights.size(); ++i) {
            ...
            if (!std::isfinite(weight) || weight < 0.0) {
                throw std::runtime_error(
                        fmt::format("Provided distance weights for joint {} [{}] are [{}] which are not"
...
otherwise. */
Eigen::VectorXd SanityCheckDistanceWeights(int num_positions,
                                           const std::vector<int>& quaternion_dof_start_indices,
                                           const Eigen::VectorXd& distance_weights) {
    if (num_positions != distance_weights.size()) {
        throw std::runtime_error(fmt::format("Provided distance weights size {} does not match num_positions {}",
    ...
    for (int i = 0; i < distance_weights.size(); ++i) {
        ...
        if (!std::isfinite(weight)) {
            throw std::runtime_error(fmt::format("Provided distance weight {} with value {} is not finite", i, weight));
        }

        if (weight < 0.0) {
            throw std::runtime_error(
                    fmt::format("Provided distance weight {} with value {} is less than zero", i, weight));
        }
    }
    ...
    for (const int i : quaternion_dof_start_indices) {
        ...
        if (x_weight != 0.0 || y_weight != 0.0 || z_weight != 0.0) {
            throw std::runtime_error(
                    fmt::format("Provided distance weights for quaternion dof starting at index {} "
...
LinearDistanceAndInterpolationProvider::LinearDistanceAndInterpolationProvider(
        const MultibodyPlant<double>& plant, const std::map<JointIndex, Eigen::VectorXd>& joint_distance_weights)
    : quaternion_dof_start_indices_(GetQuaternionDofStartIndices(plant)),
      distance_weights_(SanityCheckDistanceWeights(
              ...
              GetDistanceWeights(plant, quaternion_dof_start_indices_, joint_distance_weights))) {}

LinearDistanceAndInterpolationProvider::LinearDistanceAndInterpolationProvider(const MultibodyPlant<double>& plant,
                                                                               const Eigen::VectorXd& distance_weights)
    : quaternion_dof_start_indices_(GetQuaternionDofStartIndices(plant)),
      distance_weights_(
              SanityCheckDistanceWeights(plant.num_positions(), quaternion_dof_start_indices_, distance_weights)) {}
...
LinearDistanceAndInterpolationProvider::LinearDistanceAndInterpolationProvider(const MultibodyPlant<double>& plant)
    : quaternion_dof_start_indices_(GetQuaternionDofStartIndices(plant)),
      distance_weights_(SanityCheckDistanceWeights(
              plant.num_positions(),
              quaternion_dof_start_indices_,
              GetDefaultDistanceWeights(plant.num_positions(), quaternion_dof_start_indices_))) {}

LinearDistanceAndInterpolationProvider::~LinearDistanceAndInterpolationProvider() = default;

double LinearDistanceAndInterpolationProvider::DoComputeConfigurationDistance(const Eigen::VectorXd& from,
...
Eigen::VectorXd LinearDistanceAndInterpolationProvider::DoInterpolateBetweenConfigurations(const Eigen::VectorXd& from,
