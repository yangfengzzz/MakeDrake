...
SceneGraphCollisionChecker::SceneGraphCollisionChecker(CollisionCheckerParams params)
...
SceneGraphCollisionChecker::SceneGraphCollisionChecker(const SceneGraphCollisionChecker&) = default;
...
std::unique_ptr<CollisionChecker> SceneGraphCollisionChecker::DoClone() const {
    ...
    return std::unique_ptr<SceneGraphCollisionChecker>(new SceneGraphCollisionChecker(*this));
}

void SceneGraphCollisionChecker::DoUpdateContextPositions(CollisionCheckerContext*) const {
    // No additional actions are required to update positions.
}

std::optional<GeometryId> SceneGraphCollisionChecker::DoAddCollisionShapeToBody(const std::string& group_name,
    ...
    const GeometrySet bodyA_geometries = plant().CollectRegisteredGeometries(plant().GetBodiesWeldedTo(bodyA));
    log()->debug("Adding shape (group: [{}]) to {} (FrameID {}) at X_AG =\n{}", group_name, bodyA.scoped_name(),
                 body_frame_id, fmt_eigen(X_AG.GetAsMatrix4()));
    ...
    const auto operation = [&](const RobotDiagram<double>& model, CollisionCheckerContext* model_context) {
        auto& sg_context = model_context->mutable_scene_graph_context();
        model.scene_graph().RegisterGeometry(&sg_context, model.plant().get_source_id().value(), body_frame_id,
...
void SceneGraphCollisionChecker::RemoveAddedGeometries(const std::vector<CollisionChecker::AddedShape>& shapes) {
    const auto operation = [&shapes](const RobotDiagram<double>& model, CollisionCheckerContext* model_context) {
        for (const auto& checker_shape : shapes) {
            ...
            model.scene_graph().RemoveGeometry(&model_context->mutable_scene_graph_context(),
...
bool SceneGraphCollisionChecker::DoCheckContextConfigCollisionFree(const CollisionCheckerContext& model_context) const {
    ...
    const std::vector<SignedDistancePair<double>>& distance_pairs =
            query_object.ComputeSignedDistancePairwiseClosestPoints(GetLargestPadding());
    ...
    for (const auto& distance_pair : distance_pairs) {
        ...
        if (IsCollisionFilteredBetween(*body_A, *body_B)) {
            throw std::runtime_error(
                    fmt::format("Drake internal error at {}:{} in {}(): Collision between bodies [{}]"
                                " and [{}] should already be filtered",
                                __FILE__, __LINE__, __func__, body_A->scoped_name(), body_B->scoped_name()));
        }
        ...
        if (distance_pair.distance <= padding) {
            if (body_A_part_of_robot && body_B_part_of_robot) {
                log()->trace("Self collision between bodies [{}] and [{}]", body_A->scoped_name(),
                             body_B->scoped_name());
            } else {
                log()->trace("Environment collision between bodies [{}] and [{}]", body_A->scoped_name(),
                             body_B->scoped_name());
            }
            return false;
        }
...
RobotClearance SceneGraphCollisionChecker::DoCalcContextRobotClearance(const CollisionCheckerContext& model_context,
    ...
    const double max_influence_distance = influence_distance + GetLargestPadding();
    std::vector<SignedDistancePair<double>> signed_distance_pairs =
            query_object.ComputeSignedDistancePairwiseClosestPoints(max_influence_distance);
    std::sort(signed_distance_pairs.begin(), signed_distance_pairs.end(), [](const auto& item0, const auto& item1) {
        return (item0.id_A < item1.id_A) || ((item0.id_A == item1.id_A) && (item0.id_B < item1.id_B));
    });
    ...
    for (const auto& signed_distance_pair : signed_distance_pairs) {
        ...
        const RigidBody<double>* body_A_maybe = plant().GetBodyFromFrameId(frame_id_A);
        const RigidBody<double>* body_B_maybe = plant().GetBodyFromFrameId(frame_id_B);
        ...
        const bool body_B_part_of_robot = IsPartOfRobot(body_B);
        DRAKE_ASSERT(body_A_part_of_robot || body_B_part_of_robot);
        const BodyIndex robot_index = body_A_part_of_robot ? body_A.index() : body_B.index();
        const BodyIndex other_index = body_A_part_of_robot ? body_B.index() : body_A.index();
        const RobotCollisionType collision_type = (body_A_part_of_robot && body_B_part_of_robot)
        ...
        const Vector3d p_ACa = inspector.GetPoseInFrame(geometry_id_A) * signed_distance_pair.p_ACa;
        const Vector3d p_BCb = inspector.GetPoseInFrame(geometry_id_B) * signed_distance_pair.p_BCb;
        ...
        const Vector3d ddist_dp_BA = (distance > 0 ? 1 : -1) * (p_WCa - p_WCb).stableNormalized();
...
std::vector<RobotCollisionType> SceneGraphCollisionChecker::DoClassifyContextBodyCollisions(
    ...
    std::vector<RobotCollisionType> robot_collision_types(plant().num_bodies(), RobotCollisionType::kNoCollision);
    ...
    for (const auto& distance_pair : distance_pairs) {
        ...
        if (distance_pair.distance <= padding) {
            if (body_A_part_of_robot && body_B_part_of_robot) {
                ...
                robot_collision_types.at(body_A->index()) =
                        SetInEnvironmentCollision(robot_collision_types.at(body_A->index()), true);
            } else if (body_B_part_of_robot) {
                robot_collision_types.at(body_B->index()) =
                        SetInEnvironmentCollision(robot_collision_types.at(body_B->index()), true);
            }
        }
    }
...
int SceneGraphCollisionChecker::DoMaxContextNumDistances(const CollisionCheckerContext& model_context) const {
    const QueryObject<double>& query_object = model_context.GetQueryObject();
    const SceneGraphInspector<double>& inspector = query_object.inspector();
    // Maximum number of SignedDistancePairs returned by calls to
    // ComputeSignedDistancePairwiseClosestPoints().
    const std::set<std::pair<GeometryId, GeometryId>>& collision_candidates = inspector.GetCollisionCandidates();
...
void SceneGraphCollisionChecker::ApplyCollisionFiltersToSceneGraph() {
    ...
    const auto operation = [&new_collision_filters](const RobotDiagram<double>& model,
                                                    CollisionCheckerContext* model_context) {
        auto& sg_context = model_context->mutable_scene_graph_context();
        model.scene_graph().collision_filter_manager(&sg_context).Apply(new_collision_filters);
    };
