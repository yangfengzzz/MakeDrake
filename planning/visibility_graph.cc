...
public:
    ...
    EdgesIterator(const std::vector<std::vector<int>>& edges,
                  int outer_idx = 0,
                  int inner_idx = 0,
                  bool upper_triangle = false)
        : edges_(edges), outer_idx_(outer_idx), inner_idx_(inner_idx), upper_triangle_(upper_triangle) {
        skip_empty_outer();
    }
    ...
    const Eigen::Triplet<bool>& operator*() const {
        ...
        if (upper_triangle_) {
            e = Eigen::Triplet<bool>(edges_[outer_idx_][inner_idx_], outer_idx_, true);
        } else {
            e = Eigen::Triplet<bool>(outer_idx_, edges_[outer_idx_][inner_idx_], true);
        }
        return e;
    }
...
Eigen::SparseMatrix<bool> VisibilityGraph(const CollisionChecker& checker,
    ...
    const int num_threads_to_use = checker.SupportsParallelChecking()
                                           ...
                                           : 1;
    drake::log()->debug("Generating VisibilityGraph using {} threads", num_threads_to_use);
    ...
    const auto point_check_work = [&](const int thread_num, const int64_t i) {
        points_free[i] = static_cast<uint8_t>(checker.CheckConfigCollisionFree(points.col(i), thread_num));
    };

    StaticParallelForIndexLoop(DegreeOfParallelism(num_threads_to_use), 0, num_points, point_check_work,
    ...
    const auto edge_check_work = [&](const int thread_num, const int64_t index) {
        ...
        if (points_free[i] > 0) {
            ...
            for (int j = i + 1; j < num_points; ++j) {
                if (points_free[j] > 0 && checker.CheckEdgeCollisionFree(points.col(i), points.col(j), thread_num)) {
                    edges[i].push_back(j);
                }
            }
        }
    ...
    DynamicParallelForIndexLoop(DegreeOfParallelism(num_threads_to_use), 0, num_points, edge_check_work,
                                ParallelForBackend::BEST_AVAILABLE);
