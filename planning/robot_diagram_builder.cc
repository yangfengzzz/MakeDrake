...
std::unique_ptr<RobotDiagram<T>> RobotDiagramBuilder<T>::Build() {
    ...
    return std::unique_ptr<RobotDiagram<T>>(new RobotDiagram<T>(std::move(builder_)));
}
...
void RobotDiagramBuilder<T>::ThrowIfAlreadyBuiltOrCorrupted() const {
    ...
    const bool is_good = systems.size() >= 2 && systems[0] == &plant_ && systems[1] == &scene_graph_;
    if (!is_good) {
        throw std::logic_error(
...
bool RobotDiagramBuilder<T>::ShouldExportDefaultPorts() const {
    return plant().get_name() == "plant" && scene_graph().get_name() == "scene_graph" &&
           builder_->GetSystems().size() == 2 && builder_->num_input_ports() == 0 && builder_->num_output_ports() == 0;
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::planning::RobotDiagramBuilder);
