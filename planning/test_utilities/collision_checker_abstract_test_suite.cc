#include "planning/test_utilities/collision_checker_abstract_test_suite.h"
...
multibody::parsing::ModelDirectives MakeCollisionCheckerTestScene() {
    ...
    add_env_model.add_model = multibody::parsing::AddModel{
            "package://drake/planning/test_utilities/collision_ground_plane.sdf", "ground_plane_box"};
    multibody::parsing::ModelDirective add_env_weld;
    add_env_weld.add_weld = multibody::parsing::AddWeld{"world", "ground_plane_box::ground_plane_box"};
...
std::ostream& operator<<(std::ostream& out, const CollisionCheckerTestParams& p) {
    out << "checker = " << p.checker;
    out << ", supports_added_world_obstacles = " << p.supports_added_world_obstacles;
...
TEST_P(CollisionCheckerAbstractTestSuite, Clone) {
    ...
    EXPECT_EQ(checker.GetFilteredCollisionMatrix(), cloned->GetFilteredCollisionMatrix());
    EXPECT_EQ(checker.ClassifyBodyCollisions(qs_.q1), cloned->ClassifyBodyCollisions(qs_.q1));
}
...
TEST_P(CollisionCheckerAbstractTestSuite, AddSpheres) {
    ...
    checker.AddCollisionShapeToFrame("test", dut_frame, geometry::Sphere(1.0), math::RigidTransform<double>());
    ...
    std::vector<RobotCollisionType> kinds = checker.ClassifyBodyCollisions(qs_.q1);
...
TEST_P(CollisionCheckerAbstractTestSuite, AddObstacles) {
    ...
    const bool added = checker.AddCollisionShapeToFrame("test", checker.plant().world_frame(),
                                                        geometry::Cylinder(1.0, 1.0), math::RigidTransform<double>());
...
void CollisionCheckerAbstractTestSuite::TestDiscreteQueries(const CollisionCheckerTestParams& params,
    ...
    EXPECT_TRUE(checker.CheckEdgeCollisionFreeParallel(qs_.q1, qs_.q2, parallelism));
    EXPECT_TRUE(checker.MeasureEdgeCollisionFree(qs_.q1, qs_.q2).completely_free());
    EXPECT_TRUE(checker.MeasureEdgeCollisionFreeParallel(qs_.q1, qs_.q2, parallelism).completely_free());
    ...
    EXPECT_FALSE(checker.MeasureEdgeCollisionFree(qs_.q3, qs_.q2).partially_free());

    // Now repeat for the same (q2, q3) and (q3, q2) edges, but in parallel.
    EXPECT_FALSE(checker.CheckEdgeCollisionFreeParallel(qs_.q2, qs_.q3, parallelism));
    EXPECT_FALSE(checker.CheckEdgeCollisionFreeParallel(qs_.q3, qs_.q2, parallelism));
    const EdgeMeasure forward_parallel = checker.MeasureEdgeCollisionFreeParallel(qs_.q2, qs_.q3, parallelism);
    ...
    EXPECT_FALSE(checker.MeasureEdgeCollisionFreeParallel(qs_.q3, qs_.q2, parallelism).partially_free());

    // Edge we know is entirely in collision.
    EXPECT_FALSE(checker.CheckEdgeCollisionFree(qs_.q3, qs_.q4));
    EXPECT_FALSE(checker.CheckEdgeCollisionFreeParallel(qs_.q3, qs_.q4, parallelism));
    EXPECT_FALSE(checker.MeasureEdgeCollisionFree(qs_.q3, qs_.q4).partially_free());
    EXPECT_FALSE(checker.MeasureEdgeCollisionFreeParallel(qs_.q3, qs_.q4, parallelism).partially_free());

    // Check parallel queries.
    const std::vector<uint8_t> checks = checker.CheckConfigsCollisionFree(qs_.configs, parallelism);
    ...
    const std::vector<uint8_t> edge_checks = checker.CheckEdgesCollisionFree(qs_.edges, parallelism);
    ...
    const std::vector<EdgeMeasure> results = checker.MeasureEdgesCollisionFree(qs_.edges, parallelism);
...
// TODO(calderpg-tri) Improve so that parallelism is exercised on all queries.
void CollisionCheckerAbstractTestSuite::TestDistanceQueries(const CollisionCheckerTestParams& params,
                                                            const Parallelism parallelism) {
    auto& checker = *params.checker;
...
#pragma omp parallel for num_threads(parallelism.num_threads()) schedule(static)
    ...
    // gradient.
    const Eigen::VectorXd grad_q1_free = ComputeCollisionAvoidanceDisplacement(checker, qs_.q1, 0.0, 1.0);
    EXPECT_GT(grad_q1_free.norm(), 0.0);
    // q2 is not in collision, so we should have a free-space only avoidance
    // gradient.
    const Eigen::VectorXd grad_q2_free = ComputeCollisionAvoidanceDisplacement(checker, qs_.q2, 0.0, 1.0);
    EXPECT_GT(grad_q2_free.norm(), 0.0);
    // q3 is in collision, so we should have a free-space only avoidance
    // gradient.
    const Eigen::VectorXd grad_q3_free = ComputeCollisionAvoidanceDisplacement(checker, qs_.q3, 0.0, 1.0);
    EXPECT_GT(grad_q3_free.norm(), 0.0);
    // q3 is in collision, so we should have a in-collision avoidance gradient.
    const Eigen::VectorXd grad_q3_penetrating = ComputeCollisionAvoidanceDisplacement(checker, qs_.q3, -1.0, 0.0);
    ...
    while (current_iteration < max_iterations && !checker.CheckConfigCollisionFree(qtest)) {
        ...
        // a new gradient.
        const Eigen::VectorXd grad_qtest = ComputeCollisionAvoidanceDisplacement(checker, qtest, -0.125, 0.0);
        const Eigen::VectorXd scaled_grad_qtest = grad_qtest.stableNormalized() * 0.05;
        qtest = qtest + scaled_grad_qtest;
        log()->info("Iteration {}; raw grad_q: {}; scaled_grad_q: {}; new q: {}", current_iteration,
    ...
    log()->info("Modified qtest {} to collision-free {} in {} iterations",
                common_robotics_utilities::print::Print(qtest_initial), common_robotics_utilities::print::Print(qtest),
                current_iteration);
}
