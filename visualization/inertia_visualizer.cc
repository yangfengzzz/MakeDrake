...
template <typename T>
InertiaVisualizer<T>::InertiaVisualizer(const multibody::MultibodyPlant<T>& plant, SceneGraph<T>* scene_graph) {
    DRAKE_THROW_UNLESS(scene_graph != nullptr);
    source_id_ = scene_graph->RegisterSource("inertia_visualizer");

    // For all MbP bodies, except for those welded to the world ...
    const std::vector<const multibody::RigidBody<T>*> world_bodies = plant.GetBodiesWeldedTo(plant.world_body());
    ...
    for (multibody::BodyIndex i{0}; i < num_bodies; ++i) {
        ...
        item.Bcm_frame = scene_graph->RegisterFrame(
                ...
                GeometryFrame{fmt::format("InertiaVisualizer::{}::{}",
        ...
        auto geom = std::make_unique<GeometryInstance>(RigidTransform<double>(), std::move(ellipsoid),
        ...
        item.geometry = scene_graph->RegisterGeometry(source_id_, item.Bcm_frame, std::move(geom));
    ...
    this->DeclareAbstractInputPort("plant_geometry_pose", Value<FramePoseVector<T>>());
    this->DeclareAbstractOutputPort("geometry_pose", &InertiaVisualizer<T>::CalcFramePoseOutput);
}
...
template <typename T>
const InertiaVisualizer<T>& InertiaVisualizer<T>::AddToBuilder(DiagramBuilder<T>* builder,
                                                               const multibody::MultibodyPlant<T>& plant,
                                                               SceneGraph<T>* scene_graph) {
    DRAKE_THROW_UNLESS(builder != nullptr);
    DRAKE_THROW_UNLESS(scene_graph != nullptr);
    auto result = builder->template AddSystem<InertiaVisualizer<T>>(plant, scene_graph);
    result->set_name("inertia_visualizer");
    builder->Connect(plant.get_geometry_pose_output_port(), result->get_input_port());
    builder->Connect(result->get_output_port(), scene_graph->get_source_pose_port(result->source_id_));
    return *result;
}
...
template <typename T>
void InertiaVisualizer<T>::UpdateItems(const multibody::MultibodyPlant<T>& plant,
                                       const Context<T>& plant_context,
                                       SceneGraph<T>* scene_graph) {
    for (auto& item : items_) {
        const multibody::RigidBody<T>& body = plant.get_body(item.body);

        auto [ellipsoid, X_BBcm] = internal::CalculateInertiaGeometry(body, plant_context);
        item.X_BBcm = X_BBcm;
...
template <typename T>
void InertiaVisualizer<T>::CalcFramePoseOutput(const Context<T>& context, FramePoseVector<T>* poses) const {
    const auto& plant_poses = this->get_input_port().template Eval<FramePoseVector<T>>(context);
...
template <typename T>
std::pair<Ellipsoid, RigidTransform<double>> CalculateInertiaGeometry(const multibody::RigidBody<T>& body,
                                                                      const Context<T>& plant_context) {
    // Interrogate the plant context for the spatial inertia of body B about its
    // origin Bo, expressed in body frame B.
    const SpatialInertia<T> M_BBo_B = body.CalcSpatialInertiaInBodyFrame(plant_context);
    ...
    if (mass == 0) {
        ...
        return std::make_pair(Ellipsoid(radius, radius, radius),
    ...
    std::tie(radii, X_BE) = M_BBo_B.CalcPrincipalSemiDiametersAndPoseForSolidEllipsoid();
    ...
    const double unit_inertia_ellipsoid_mass = density * (4.0 / 3.0) * M_PI * radii(0) * radii(1) * radii(2);
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&CalculateInertiaGeometry<T>));

}  // namespace internal
}  // namespace visualization
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::visualization::InertiaVisualizer);
