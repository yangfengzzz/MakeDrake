#include "visualization/meshcat_pose_sliders.h"
...
GTEST_TEST(MeshcatPoseSlidersTest, NarrowConstructor) {
    ...
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsExactlyEqualTo(RigidTransformd()));
    ...
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsExactlyEqualTo(
            RigidTransformd(RollPitchYawd(-kPiRounded, 0, kPiRounded), Vector3d::Zero())));
}
...
GTEST_TEST(MeshcatPoseSlidersTest, WideConstructor) {
    ...
    const RigidTransformd initial_pose(RollPitchYawd(0.1, 0.2, 0.3), Vector3d(0.4, 0.5, 0.6));
    const Vector6d lower_limit = Vector6d::Constant(-0.8);
    const Vector6d upper_limit = Vector6d::Constant(0.8);
    const Vector6d step = (Vector6d() << 0.1, 0.1, 0.1, 0.1, 0.01, 0.1).finished();
    const std::vector<std::string> decrement_keycodes = {"KeyA", "KeyB", "KeyC", "KeyD", "KeyE", "KeyF"};
    const std::vector<std::string> increment_keycodes = {"KeyG", "KeyH", "KeyI", "KeyJ", "KeyK", "KeyL"};
    const std::string prefix = "prefix";
    const Vector6<bool> visible = (Vector6<bool>() << false, false, true, true, true, false).finished();

    const MeshcatPoseSliders<double> dut(meshcat, initial_pose, lower_limit, upper_limit, step, decrement_keycodes,
    ...
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsNearlyEqualTo(initial_pose, 1e-14));
    ...
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsNearlyEqualTo(
            RigidTransformd(RollPitchYawd(0.1, 0.2, -0.8), Vector3d(0.8, 0.5, 0.6)), 1e-14));
...
GTEST_TEST(MeshcatPoseSlidersTest, ShiftedLimits) {
    ...
    const MeshcatPoseSliders<double> dut(meshcat, initial_pose, lower_limit, upper_limit);
    auto context = dut.CreateDefaultContext();
...
GTEST_TEST(MeshcatPoseSlidersTest, Run) {
    ...
    const RigidTransformd expected_pose(RollPitchYawd(0.25, 0.2, 0.3), Vector3d(0.4, 0.5, 0.6));
...
GTEST_TEST(MeshcatPoseSlidersTest, SetPose) {
    ...
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsExactlyIdentity());
    EXPECT_EQ(meshcat->GetSliderValue("roll"), 0);
    EXPECT_EQ(meshcat->GetSliderValue("pitch"), 0);

    /* Setting the pose should update both the output value and sliders. */
    const RigidTransformd X(RollPitchYawd(0.1, 0.2, 0.3), Vector3d(0.4, 0.5, 0.6));
    dut.SetPose(X);
    EXPECT_TRUE(dut.get_output_port().Eval<RigidTransformd>(*context).IsExactlyEqualTo(X));
