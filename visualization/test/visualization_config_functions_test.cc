...
GTEST_TEST(VisualizationConfigFunctionsTest, ParamConversionDefault) {
    ...
    const std::vector<DrakeVisualizerParams> drake_params = ConvertVisualizationConfigToDrakeParams(config);
    ASSERT_EQ(drake_params.size(), 2);

    EXPECT_EQ(drake_params.at(0).role, Role::kIllustration);
    EXPECT_EQ(drake_params.at(0).default_color, config.default_illustration_color);
    ...
    const std::vector<MeshcatVisualizerParams> meshcat_params = ConvertVisualizationConfigToMeshcatParams(config);
    ...
    EXPECT_EQ(meshcat_params.at(0).default_color, config.default_illustration_color);
    EXPECT_EQ(meshcat_params.at(0).delete_on_initialization_event, config.delete_on_initialization_event);
    EXPECT_EQ(meshcat_params.at(0).enable_alpha_slider, config.enable_alpha_sliders);
    ...
    EXPECT_EQ(meshcat_params.at(1).delete_on_initialization_event, config.delete_on_initialization_event);
    EXPECT_EQ(meshcat_params.at(1).enable_alpha_slider, config.enable_alpha_sliders);
    ...
    EXPECT_EQ(meshcat_params.at(2).delete_on_initialization_event, config.delete_on_initialization_event);
    EXPECT_EQ(meshcat_params.at(2).enable_alpha_slider, config.enable_alpha_sliders);
    EXPECT_EQ(meshcat_params.at(2).initial_alpha_slider_value, config.initial_proximity_alpha);
    ...
    const ContactVisualizerParams contact_params = ConvertVisualizationConfigToMeshcatContactParams(config);
    EXPECT_EQ(contact_params.publish_period, config.publish_period);
    EXPECT_EQ(contact_params.delete_on_initialization_event, config.delete_on_initialization_event);
}
...
GTEST_TEST(VisualizationConfigFunctionsTest, ParamConversionSpecial) {
    ...
    const VisualizationConfig proximity_alpha_config{.publish_proximity = true, .initial_proximity_alpha = 0.25};
...
GTEST_TEST(VisualizationConfigFunctionsTest, ApplyDefault) {
    ...
    drake_lcm.SubscribeAllChannels([&](std::string_view channel, const void* /* buffer */, int /* size */) {
    ...
    VisualizationConfig config;
    ApplyVisualizationConfig(config, &builder, &lcm_buses, &plant, &scene_graph, meshcat);
  ...
  const auto expected_channels = {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(meshcat->GetSliderValue("illustration α"),
                                ".*does not have any slider named illustration.*");
    DRAKE_EXPECT_THROWS_MESSAGE(meshcat->GetSliderValue("proximity α"), ".*does not have any slider named proximity.*");
}
...
GTEST_TEST(VisualizationConfigFunctionsTest, UpdatePeriod) {
    ...
    std::unique_ptr<CompositeEventCollection<double>> events = diagram->AllocateCompositeEventCollection();
...
GTEST_TEST(VisualizationConfigFunctionsTest, NoPlant) {
    DiagramBuilder<double> builder;
    DRAKE_EXPECT_THROWS_MESSAGE(AddDefaultVisualization(&builder), ".*does not contain.*plant.*");
}
...
GTEST_TEST(VisualizationConfigFunctionsTest, NoSceneGraph) {
    ...
    plant->Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(AddDefaultVisualization(&builder), ".*does not contain.*scene_graph.*");
}
...
GTEST_TEST(VisualizationConfigFunctionsTest, WrongSystemTypes) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AddDefaultVisualization(&builder), ".*not cast.*");
}
