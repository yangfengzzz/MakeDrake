#include "visualization/inertia_visualizer.h"
...
public:
    ...
    void SetUpDiagramWithConfig(const VisualizationConfig& config) {
        ...
        meshcat_->Delete();
        parser.AddModels(FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.sdf"));
        plant_.Finalize();

        ApplyVisualizationConfig(config, &builder_, nullptr, &plant_, &scene_graph_, meshcat_);
...
class InertiaVisualizerGeometryTest : public ::testing::TestWithParam<RowVector6d> {
public:
    ...
    std::pair<geometry::Ellipsoid, RigidTransform<double>> CalculateInertiaGeometryFor(const GeometryTestCase& input) {
        ...
        const std::string model =
                fmt::format(kModelTemplate, fmt::arg("mass", input.mass), fmt::arg("ixx", input.moment_ixx),
                            fmt::arg("iyy", input.moment_iyy), fmt::arg("izz", input.moment_izz),
                            fmt::arg("visual", input.visual_sdf), fmt::arg("x", xyz().x()), fmt::arg("y", xyz().y()),
                            fmt::arg("z", xyz().z()), fmt::arg("roll", rpy()(0)), fmt::arg("pitch", rpy()(1)),
                            fmt::arg("yaw", rpy()(2)));
        ...
        plant_.Finalize();
        const multibody::RigidBody<double>& body = plant_.GetBodyByName("test_body");
        ...
        for (double flip_r : {0.0, M_PI}) {
            for (double flip_p : {0.0, M_PI}) {
                for (double flip_y : {0.0, M_PI}) {
                    const RotationMatrix<double> flips(RollPitchYaw<double>(flip_r, flip_p, flip_y));
...
TEST_P(InertiaVisualizerGeometryTest, EllipsoidTest) {
    ...
    const auto M = SpatialInertia<double>::SolidEllipsoidWithDensity(kNominalDensity, a, b, c);
    const GeometryTestCase test_case{
            .visual_sdf = fmt::format("<ellipsoid><radii>{} {} {}</radii></ellipsoid>", a, b, c),
            .mass = M.get_mass(),
...
TEST_P(InertiaVisualizerGeometryTest, ZeroMassTest) {
    auto [ellipsoid, transform] = CalculateInertiaGeometryFor({.visual_sdf = "<sphere><radius>1.0</radius></sphere>"});
...
TEST_P(InertiaVisualizerGeometryTest, BoxTest) {
    ...
    const auto M = SpatialInertia<double>::SolidBoxWithDensity(kNominalDensity, x, y, z);
    auto box_inertia = M.CalcRotationalInertia();
    auto [ellipsoid, transform] =
            CalculateInertiaGeometryFor({.visual_sdf = fmt::format("<box><size>{} {} {}</size></box>", x, y, z),
                                         .mass = M.get_mass(),
    ...
    const auto ellipsoid_M = SpatialInertia<double>::SolidEllipsoidWithDensity(kNominalDensity, ellipsoid.a(),
    ...
    const Vector3d ratio = ellipsoid_inertia.get_moments().array() / box_inertia.get_moments().array();
    SCOPED_TRACE(fmt::format("box_inertia = {}; ellipsoid_inertia = {}; ratio = {}", box_inertia, ellipsoid_inertia,
                             fmt_eigen(ratio)));
...
TEST_P(InertiaVisualizerGeometryTest, ThinRodTest) {
    ...
    const auto M = SpatialInertia<double>::ThinRodWithMass(10, length, Vector3d(1, 0, 0));
    const GeometryTestCase test_case{
            .visual_sdf = fmt::format("<cylinder><radius>0.01</radius><length>{}</length></cylinder>", length),
            .mass = M.get_mass(),
            .moment_ixx = M.CalcRotationalInertia().get_moments()[0],
            .moment_iyy = M.CalcRotationalInertia().get_moments()[1],
            .moment_izz = M.CalcRotationalInertia().get_moments()[2],
    ...
    EXPECT_TRUE(CompareMatrices(GetCanonicalRpy(transform).tail(2), rpy().tail(2), kTolerance));
    ...
    // along the x-axis.
    SCOPED_TRACE(fmt::format("abc = {} {} {}", ellipsoid.a(), ellipsoid.b(), ellipsoid.c()));
