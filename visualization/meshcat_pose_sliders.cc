...
template <typename T>
MeshcatPoseSliders<T>::MeshcatPoseSliders(std::shared_ptr<geometry::Meshcat> meshcat,
    ...
    slider_names_ = {AddPrefix("roll", prefix), AddPrefix("pitch", prefix), AddPrefix("yaw", prefix),
                     AddPrefix("x", prefix),    AddPrefix("y", prefix),     AddPrefix("z", prefix)};

    // Add sliders.
    for (int i = 0; i < 6; ++i) {
        if (visible[i]) {
            meshcat_->AddSlider(slider_names_[i], lower_limit[i], upper_limit[i], step[i], rpy_xyz[i],
                                decrement_keycodes[i], increment_keycodes[i]);
        }
    }
    ...
    auto& output = this->DeclareAbstractOutputPort("pose", &MeshcatPoseSliders<T>::CalcOutput);
    ...
    // connected).
    this->DeclareInitializationPublishEvent(&MeshcatPoseSliders<T>::OnInitialization);
}
...
template <typename T>
void MeshcatPoseSliders<T>::CalcOutput(const Context<T>&, RigidTransformd* pose) const {
    if (is_registered_) {
...
template <typename T>
systems::EventStatus MeshcatPoseSliders<T>::OnInitialization(const Context<T>& context) const {
...
template <typename T>
typename systems::LeafSystem<T>::GraphvizFragment MeshcatPoseSliders<T>::DoGetGraphvizFragment(
...
template <typename T>
RigidTransformd MeshcatPoseSliders<T>::Run(const System<T>& system,
    ...
    while (meshcat_->GetButtonClicks(kButtonName) < 1) {
        ...
        const RigidTransformd& new_pose = this->get_output_port().template Eval<RigidTransformd>(pose_context);
