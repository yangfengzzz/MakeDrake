...
public:
    ...
    void set_invalid_color(const geometry::Rgba& invalid_color) { invalid_color_ = invalid_color; }

    /** Colorizes the `input` into `output`, without using any System port
    conections nor any Context. */
    void Calc(const systems::sensors::ImageDepth32F& input, systems::sensors::ImageRgba8U* output) const;

    /** Colorizes the `input` into `output`, without using any System port
    conections nor any Context. */
    void Calc(const systems::sensors::ImageDepth16U& input, systems::sensors::ImageRgba8U* output) const;

private:
    void CalcOutput(const systems::Context<T>& context, systems::sensors::ImageRgba8U* output) const;
