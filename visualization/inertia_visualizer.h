...
public:
    ...
    InertiaVisualizer(const multibody::MultibodyPlant<T>& plant, geometry::SceneGraph<T>* scene_graph);
    ...
    static const InertiaVisualizer<T>& AddToBuilder(systems::DiagramBuilder<T>* builder,
...
template <typename T>
std::pair<geometry::Ellipsoid, math::RigidTransform<double>> CalculateInertiaGeometry(
        const multibody::RigidBody<T>& body, const systems::Context<T>& plant_context);
}  // namespace internal
