...
void ApplyVisualizationConfigImpl(const VisualizationConfig& config,
    ...
    if (lcm->get_lcm_url() != LcmBuses::kLcmUrlMemqNull) {
        ...
        if (config.publish_contacts) {
            ConnectContactResultsToDrakeVisualizer(builder, plant, *scene_graph, lcm, config.publish_period);
        }
    }
    ...
    if (meshcat != nullptr) {
        ...
        if (config.publish_contacts) {
            ContactVisualizer<double>::AddToBuilder(builder, plant, meshcat,
...
void ApplyVisualizationConfig(const VisualizationConfig& config,
    ...
    lcm = FindOrCreateLcmBus(lcm, lcm_buses, builder, "ApplyVisualizationConfig", config.lcm_bus);
    DRAKE_DEMAND(lcm != nullptr);
    ...
    ApplyVisualizationConfigImpl(config, lcm, meshcat, *plant, scene_graph, builder);
}

void AddDefaultVisualization(DiagramBuilder<double>* builder, std::shared_ptr<geometry::Meshcat> meshcat) {
...
std::vector<DrakeVisualizerParams> ConvertVisualizationConfigToDrakeParams(const VisualizationConfig& config) {
...
std::vector<MeshcatVisualizerParams> ConvertVisualizationConfigToMeshcatParams(const VisualizationConfig& config) {
    ...
    if (config.publish_illustration) {
        ...
        illustration.delete_on_initialization_event = config.delete_on_initialization_event;
    ...
    if (config.publish_inertia) {
        ...
        inertia.delete_on_initialization_event = config.delete_on_initialization_event;
    ...
    if (config.publish_proximity) {
        ...
        proximity.delete_on_initialization_event = config.delete_on_initialization_event;
...
ContactVisualizerParams ConvertVisualizationConfigToMeshcatContactParams(const VisualizationConfig& config) {
