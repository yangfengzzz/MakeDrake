...
ColorizeDepthImage<T>::ColorizeDepthImage() {
    ...
    this->DeclareAbstractOutputPort("color_image", &ColorizeDepthImage::CalcOutput);
}
...
template <typename T>
void ColorizeDepthImage<T>::Calc(const ImageDepth32F& input, ImageRgba8U* output) const {
...
template <typename T>
void ColorizeDepthImage<T>::Calc(const ImageDepth16U& input, ImageRgba8U* output) const {
    ImageDepth32F temp;
...
template <typename T>
void ColorizeDepthImage<T>::CalcOutput(const Context<T>& context, ImageRgba8U* output) const {
    ...
    if (has_depth32f) {
        const auto& depth = this->get_input_port(0).template Eval<ImageDepth32F>(context);
        Calc(depth, output);
    } else {
        const auto& depth = this->get_input_port(1).template Eval<ImageDepth16U>(context);
        Calc(depth, output);
    }
}
