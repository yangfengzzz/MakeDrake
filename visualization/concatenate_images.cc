#include "visualization/concatenate_images.h"
...
// support ImageRgba8U.
template <typename T>
ConcatenateImages<T>::ConcatenateImages(int rows, int cols) : rows_(rows), cols_(cols) {
    ...
    for (int row = 0; row < rows; ++row) {
        for (int col = 0; col < cols; ++col) {
            inputs_(row, col) =
                    &this->DeclareAbstractInputPort(fmt::format("color_image_r{}_c{}", row, col), Value(ImageRgba8U()));
        }
    }
    this->DeclareAbstractOutputPort("color_image", &ConcatenateImages::CalcOutput);
}
...
template <typename T>
const InputPort<T>& ConcatenateImages<T>::get_input_port(int row, int col) const {
...
template <typename T>
void ConcatenateImages<T>::CalcOutput(const Context<T>& context, ImageRgba8U* output) const {
