#include "visualization/colorize_label_image.h"
...
ColorizeLabelImage<T>::ColorizeLabelImage() {
    ...
    this->DeclareAbstractOutputPort("color_image", &ColorizeLabelImage::CalcOutput);
}
...
template <typename T>
void ColorizeLabelImage<T>::Calc(const ImageLabel16I& input, ImageRgba8U* output) const {
    ...
    for (int v = 0; v < output->height(); ++v) {
        for (int u = 0; u < output->width(); ++u) {
            ...
            const std::array<uint8_t, 4>& color = is_reserved ? background : palette[label % palette.size()];
...
template <typename T>
void ColorizeLabelImage<T>::CalcOutput(const Context<T>& context, ImageRgba8U* output) const {
    const auto& input = this->get_input_port().template Eval<ImageLabel16I>(context);
    Calc(input, output);
}
