...
public:
    ...
    */
    MeshcatPoseSliders(
            ...
            const math::RigidTransform<double>& initial_pose = math::RigidTransform<double>(),
            const Eigen::Ref<const Vector6d>& lower_limit = (Vector6d() << -M_PI, -M_PI, -M_PI, -1, -1, -1).finished(),
            const Eigen::Ref<const Vector6d>& upper_limit = (Vector6d() << M_PI, M_PI, M_PI, 1, 1, 1).finished(),
            const Eigen::Ref<const Vector6d>& step = Vector6d::Constant(0.01),
            std::vector<std::string> decrement_keycodes = {"KeyQ", "KeyS", "KeyA", "KeyJ", "KeyK", "KeyU"},
            std::vector<std::string> increment_keycodes = {"KeyE", "KeyW", "KeyD", "KeyL", "KeyI", "KeyO"},
            std::string prefix = "",
            const Eigen::Ref<const Vector6<bool>>& visible = Vector6<bool>::Constant(true));
    ...
    */
    math::RigidTransform<double> Run(const systems::System<T>& system,
...
private:
    void CalcOutput(const systems::Context<T>&, math::RigidTransform<double>*) const;
