...
public:
    ...
    std::unique_ptr<::lcm_eventlog_event_t, decltype(&::lcm_eventlog_free_event)> next_event_{
            nullptr, &::lcm_eventlog_free_event};
};

DrakeLcmLog::DrakeLcmLog(const std::string& file_name, bool is_write, bool overwrite_publish_time_with_system_clock)
    : is_write_(is_write),
      overwrite_publish_time_with_system_clock_(overwrite_publish_time_with_system_clock),
...
void DrakeLcmLog::Publish(const std::string& channel, const void* data, int data_size, std::optional<double> time_sec) {
    if (!is_write_) {
    ...
    if (!overwrite_publish_time_with_system_clock_) {
        ...
        log_event.timestamp = std::chrono::steady_clock::now().time_since_epoch() / std::chrono::microseconds(1);
    }
...
std::shared_ptr<DrakeSubscriptionInterface> DrakeLcmLog::Subscribe(const std::string& channel,
...
std::shared_ptr<DrakeSubscriptionInterface> DrakeLcmLog::SubscribeMultichannel(
    ...
    throw std::logic_error("DrakeLcmLog::SubscribeMultichannel is not implemented.");
}

std::shared_ptr<DrakeSubscriptionInterface> DrakeLcmLog::SubscribeAllChannels(MultichannelHandlerFunction handler) {
    if (is_write_) {
        throw std::logic_error("Subscribe is only available for log playback.");
    }
    std::lock_guard<std::mutex> lock(mutex_);
...
int DrakeLcmLog::HandleSubscriptions(int) {
    if (is_write_) {
        throw std::logic_error("HandleSubscriptions is only available for log playback.");
    }
    return 0;
}
