...
public:
    ...
    static ::lcm::LCM get_native(DrakeLcm* dut) {
        ...
        return ::lcm::LCM(static_cast<::lcm_t*>(dut->get_native_lcm_handle_for_unit_testing()));
    }
};
namespace {
...
protected:
    ...
    void LoopUntilDone(lcmt_drake_signal* received, int retries, const std::function<void(void)>& step) {
        ...
        while (!message_was_received && count++ < retries) {
            step();
            message_was_received = CompareLcmtDrakeSignalMessages(*received, message_);
        }
...
TEST_F(DrakeLcmTest, EmptyChannelTest) {
    ...
    char data[1] = {};
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->Publish("", data, 1, {}), ".*channel.empty.*");
}
...
TEST_F(DrakeLcmTest, PublishTest) {
    ...
    ::lcm::LCM::HandlerFunction<lcmt_drake_signal> handler =
            [&received](const ::lcm::ReceiveBuffer*, const std::string&, const lcmt_drake_signal* new_value) {
...
TEST_F(DrakeLcmTest, SubscribeTest) {
    ...
    lcmt_drake_signal received{};
    auto subscription = dut_->Subscribe(channel_name, [&received](const void* data, int size) {
...
TEST_F(DrakeLcmTest, SubscribeAllTest) {
    ...
    auto subscription = dut_->SubscribeAllChannels(
            [&received, &channel_name](std::string_view channel, const void* data, int size) {
...
TEST_F(DrakeLcmTest, UnsubscribeTest) {
    ...
    lcmt_drake_signal received{};
    auto subscription = dut_->Subscribe(channel_name, [&](const void* data, int size) {
        received.decode(data, 0, size);
    });
...
TEST_F(DrakeLcmTest, UnsubscribeFromMultichannelTest) {
    const std::string channel_name = "DrakeLcmTest.UnsubscribeFromMultichannelTest";

    // First, confirm that the subscriber is active.
    lcmt_drake_signal received{};
    auto subscription = dut_->SubscribeAllChannels([&](std::string_view channel, const void* data, int size) {
        received.decode(data, 0, size);
    });
    subscription->set_unsubscribe_on_delete(true);
    LoopUntilDone(&received, 20 /* retries */, [&]() {
...
TEST_F(DrakeLcmTest, QueueCapacityTest) {
    ...
    int count = 0;
    auto subscription = dut_->Subscribe(channel_name, [&count](const void* data, int size) {
        ++count;
    });
...
TEST_F(DrakeLcmTest, Suffix) {
    ...
    ::lcm::LCM::HandlerFunction<lcmt_drake_signal> handler =
            [&received_native](const ::lcm::ReceiveBuffer*, const std::string&, const lcmt_drake_signal* new_value) {
                DRAKE_DEMAND(new_value != nullptr);
    ...
    auto subscription = dut_->Subscribe("SuffixDrakeLcmTest", [&received_drake](const void* data, int size) {
...
TEST_F(DrakeLcmTest, SuffixInSubscribeAllChannels) {
    ...
    lcmt_drake_signal received_drake{};
    auto subscription = dut_->SubscribeAllChannels(
            [&received_drake, unadorned](std::string_view channel_name, const void* data, int size) {
...
TEST_F(DrakeLcmTest, SubscribeMultiTest) {
    ...
    auto subscription = dut_->SubscribeMultichannel(
            "Drake.*MultiTest", [&received, &channel_name](std::string_view channel, const void* data, int size) {
                EXPECT_EQ(channel, channel_name);
                received.decode(data, 0, size);
            });
    subscription.reset();  // Deleting the subscription should be a no-op.
