...
protected:
    ...
    void LoopUntilDone(const MessageMailbox& mailbox, const std::function<void(void)>& publish) {
        ...
        while (!message_was_received && count++ < kMaxCount) {
            ...
            message_was_received = CompareLcmtDrakeSignalMessages(mailbox.GetMessage(), message_);
        }
