...
GTEST_TEST(LcmLogTest, LcmLogTestSaveAndRead) {
    ...
    for (int i = 0; i < 3; i++) {
        Subscribe(r_log.get(), channel_name, std::function{[i, &messages](const lcmt_drake_signal& message) {
                      messages[i] = message;
                  }});
    }
    ...
    r_log->SubscribeAllChannels([&multichannel_received](std::string_view channel, const void*, int) {
