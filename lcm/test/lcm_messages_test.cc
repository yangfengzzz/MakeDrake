...
GTEST_TEST(LcmMessagesTest, EncodeLcmMessageError) {
    BuggyLcmMessage bad;
    DRAKE_EXPECT_THROWS_MESSAGE(EncodeLcmMessage(bad), "Error encoding message of type.*BuggyLcmMessage.*");
}
...
GTEST_TEST(LcmMessagesTest, DecodeLcmMessageError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DecodeLcmMessage<lcmt_drake_signal>(corrupt_bytes),
