...
TEST_F(DrakeLcmInterfaceTest, DefaultErrorHandlingTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(lcm_.HandleSubscriptions(0), "Error decoding message on NAME");
...
TEST_F(DrakeLcmInterfaceTest, HandlerExceptionTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(lcm_.HandleSubscriptions(0), ".*callback.*dim > 0.*failed.*");
