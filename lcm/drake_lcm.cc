        ...
        if (lcm_url_.substr(0, 7) != "memq://") {
            if (!drake::internal::IsNetworkingAllowed("lcm")) {
                throw std::runtime_error(
                        fmt::format("LCM URL {} has been disabled via the DRAKE_ALLOW_NETWORK "
...
DrakeLcm::DrakeLcm(std::string lcm_url) : DrakeLcm(DrakeLcmParams{.lcm_url = std::move(lcm_url)}) {}

DrakeLcm::DrakeLcm(const DrakeLcmParams& params) : impl_(std::make_unique<Impl>(params)) {
...
void DrakeLcm::Publish(const std::string& channel, const void* data, int data_size, std::optional<double>) {
...
public:
    ...
    using MultichannelHandlerFunction = DrakeLcmInterface::MultichannelHandlerFunction;

    static std::shared_ptr<DrakeSubscription> CreateSingleChannel(::lcm_t* native_instance,
        ...
        return CreateMultichannel(
                ...
                [handler = std::move(single_channel_handler)](std::string_view, const void* data, int size) {
                    handler(data, size);
                });
    }

    static std::shared_ptr<DrakeSubscription> CreateMultichannel(::lcm_t* native_instance,
    ...
    void set_queue_capacity(int capacity) final {
        ...
        if (native_subscription_) {
            DRAKE_DEMAND(native_instance_ != nullptr);
            ::lcm_subscription_set_queue_capacity(native_subscription_, queue_capacity_);
        }
    }
    ...
    void AttachIfNeeded() {
        ...
        native_subscription_ =
                ::lcm_subscribe(native_instance_, channel_regex_.c_str(), &DrakeSubscription::NativeCallback, this);
        ::lcm_subscription_set_queue_capacity(native_subscription_, queue_capacity_);
    }
...
private:
    ...
    static void NativeCallback(const ::lcm_recv_buf_t* buffer, const char* channel, void* user_data) {
...
std::shared_ptr<DrakeSubscriptionInterface> DrakeLcm::Subscribe(const std::string& channel, HandlerFunction handler) {
    DRAKE_THROW_UNLESS(!channel.empty());
    ...
    const std::string actual_channel = channel + impl_->channel_suffix_;
    auto result = DrakeSubscription::CreateSingleChannel(impl_->lcm_, actual_channel, std::move(handler));
...
std::shared_ptr<DrakeSubscriptionInterface> DrakeLcm::SubscribeMultichannel(std::string_view regex,
                                                                            MultichannelHandlerFunction handler) {
    ...
    if (!suffix.empty()) {
        handler = [&suffix, handler](std::string_view channel, const void* data, int length) {
    ...
    auto result = DrakeSubscription::CreateMultichannel(
            impl_->lcm_, std::string(regex) + ConvertLiteralStringToLcmRegex(suffix), std::move(handler));
    if (!impl_->deferred_initialization_) {
        result->AttachIfNeeded();
    }
    impl_->subscriptions_.push_back(result);
...
std::shared_ptr<DrakeSubscriptionInterface> DrakeLcm::SubscribeAllChannels(MultichannelHandlerFunction handler) {
