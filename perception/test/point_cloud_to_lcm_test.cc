...
protected:
    lcmt_point_cloud Convert(const PointCloud& cloud,
        ...
        const lcmt_point_cloud output = dut.get_output_port().Eval<lcmt_point_cloud>(*context);
        return output;
    }
...
TEST_F(PointCloudToLcmTest, XyzRgb) {
    ...
    expected.data_size = data_size;
    const float data[8] = {
            1.0f, 2.0f, 3.0f, PackRgb(0, 127, 255), 4.0f, 5.0f, 6.0f, PackRgb(1, 128, 255),
    };
    static_assert(sizeof(data) == data_size);
    expected.data.resize(data_size);
    std::memcpy(expected.data.data(), data, data_size);
...
TEST_F(PointCloudToLcmTest, StorageReuse) {
    ...
    auto context = dut.CreateDefaultContext();
    context->SetTime(1.0);
    auto& fixed_value = dut.get_input_port().FixValue(context.get(), Value<PointCloud>(cloud));
