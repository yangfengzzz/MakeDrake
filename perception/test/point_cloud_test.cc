...
GTEST_TEST(PointCloudTest, Basic) {
    ...
    xyzs_expected.transpose() << 1, 2, 3, 10, 20, 30, 100, 200, 300, 4, 5, 6, 40, 50, 60;
    Matrix3Xf normals_expected(3, count);
    normals_expected.transpose() << 1, 2, 3, 10, 20, 30, 100, 200, 300, 4, 5, 6, 40, 50, 60;
    Matrix3X<uint8_t> rgbs_expected(3, count);
    rgbs_expected.transpose() << 1, 2, 3, 10, 20, 30, 100, 200, 255, 4, 5, 6, 40, 50, 60;
    ...
    auto CheckFields = [count](auto values_expected, pc_flags::Fields fields, auto get_mutable_values, auto get_values,
        ...
        EXPECT_TRUE(CompareMatrices(values_expected, get_values(cloud).middleCols(0, last)));

        // Resize to a size smaller.
        int small_size = 3;
        cloud.resize(small_size);
        EXPECT_EQ(small_size, cloud.size());
        EXPECT_TRUE(CompareMatrices(values_expected.middleCols(0, small_size), get_values(cloud)));
        ...
        EXPECT_TRUE(check_helper<T>::IsDefault(get_values(cloud).middleCols(small_size, large_size - small_size)));
    };
    ...
    {
        pc_flags::Fields xyz_rgb_normals = (pc_flags::kXYZs | pc_flags::kNormals | pc_flags::kRGBs);
        pc_flags::Fields all_fields = (xyz_rgb_normals | pc_flags::kDescriptorCurvature);

        const std::vector<std::pair<pc_flags::Fields, pc_flags::Fields>> fields_pairs{
    ...
    // Test `SetFields()` and its `skip_initialize` flag.
    {
        ...
        // Expand the fields.
        cloud.SetFields((pc_flags::kXYZs | pc_flags::kNormals | pc_flags::kRGBs | pc_flags::kDescriptorCurvature),
                        false);
        EXPECT_TRUE(cloud.HasFields(pc_flags::kXYZs));
    ...
    {
        PointCloud cloud = CreatePointCloud(pc_flags::kXYZs | pc_flags::kNormals | pc_flags::kRGBs |
        ...
        for (int i = 0; i < static_cast<int>(indices.size()); ++i) {
            EXPECT_TRUE(CompareMatrices(cropped.xyzs().col(i), xyzs_expected.col(indices[i])));
            EXPECT_TRUE((cropped.rgbs().col(i).array() == rgbs_expected.col(indices[i]).array()).all());
            EXPECT_TRUE(CompareMatrices(cropped.normals().col(i), normals_expected.col(indices[i])));
            EXPECT_TRUE(CompareMatrices(cropped.descriptors().col(i), descriptors_expected.col(indices[i])));
        }
    }
}
...
GTEST_TEST(PointCloudTest, Fields) {
    ...
    // Check basic requirements.
    {
        ...
        EXPECT_THROW(cloud.RequireFields(pc_flags::kDescriptorFPFH), std::runtime_error);
    }

    // Check with exact fields.
    {
        auto fields = pc_flags::kXYZs | pc_flags::kNormals | pc_flags::kRGBs | pc_flags::kDescriptorCurvature;
...
GTEST_TEST(PointCloud, Concatenate) {
    ...
    fields.push_back(pc_flags::kXYZs | pc_flags::kRGBs | pc_flags::kNormals | pc_flags::kDescriptorCurvature);
    ...
    for (const auto& f : fields) {
        ...
        if (f.contains(pc_flags::kNormals)) {
            ...
            normals_expected << clouds[0].normals(), clouds[1].normals(), clouds[2].normals();
            EXPECT_TRUE(CompareMatrices(merged.normals(), normals_expected));
        }

        if (f.has_descriptor()) {
            RowVectorXf descriptors_expected(6);
            descriptors_expected << clouds[0].descriptors(), clouds[1].descriptors(), clouds[2].descriptors();
...
GTEST_TEST(PointCloudTest, FlipNormals) {
  ...
  original_normals.transpose() <<
    ...
    auto CheckNormal = [&cloud, &original_normals](int index, bool expect_flipped) {
        if (expect_flipped) {
            EXPECT_TRUE(CompareMatrices(cloud.normal(index), -original_normals.col(index)));
        } else {
            EXPECT_TRUE(CompareMatrices(cloud.normal(index), original_normals.col(index)));
        }
    };
...
GTEST_TEST(PointCloudTest, VoxelizedDownSample) {
    const auto fields = pc_flags::kXYZs | pc_flags::kNormals | pc_flags::kRGBs | pc_flags::kDescriptorCurvature;
  ...
  cloud.mutable_xyzs().transpose() <<
    ...
    auto CheckHasPointAveragedFrom = [&cloud, &down_sampled](const std::vector<int>& indices) {
        ...
        EXPECT_TRUE(CompareMatrices(down_sampled.normal(i), normal.cast<float>(), kTol));
        EXPECT_NEAR(down_sampled.normal(i).norm(), 1.0, 1e-6);
        EXPECT_EQ(down_sampled.rgb(i), rgb.cast<u_int8_t>());
        EXPECT_TRUE(CompareMatrices(down_sampled.descriptor(i), descriptor.cast<float>(), kTol));
    };
...
void CheckNormal(const Eigen::Ref<const Vector3f>& normal,
                 const Eigen::Ref<const Vector3f>& expected,
                 double tolerance) {
    EXPECT_NEAR(normal.norm(), 1.0, tolerance)
            << fmt::format("Normal {} does not have unit length.", fmt_eigen(normal));
    EXPECT_NEAR(std::abs(normal.dot(expected)), 1.0, tolerance) << "normal.dot(expected) = " << normal.dot(expected);
}
...
GTEST_TEST(PointCloudTest, EstimateNormalsPlane) {
    ...
    cloud.EstimateNormals(10, 3, ENABLE_PARALLEL_OPS);
    for (int i = 0; i < 3; ++i) {
        CheckNormal(cloud.normal(i), Vector3f{0, 1.0 / std::sqrt(2.0), -1.0 / std::sqrt(2.0)}, kTol);
    }
}
...
GTEST_TEST(PointCloudTest, EstimateNormalsTwoPoints) {
  ...
  cloud.mutable_xyzs().transpose() <<
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_NEAR(cloud.normal(i).dot(cloud.xyz(1) - cloud.xyz(0)), 0.0, kTolerance);
    }
}
