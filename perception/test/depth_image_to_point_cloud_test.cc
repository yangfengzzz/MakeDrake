#include "perception/depth_image_to_point_cloud.h"
...
#include "systems/sensors/camera_info.h"
...
struct System16Color {
    static constexpr PixelType kPixelType = PixelType::kDepth16U;
    static constexpr bool kUseSystem = true;
    static constexpr pc_flags::BaseFieldT kFields = pc_flags::kXYZs | pc_flags::kRGBs;
};
...
using AllConfigs = ::testing::
        Types<System16, Static16, System32, Static32, System16Color, Static16Color, System32Color, Static32Color>;
...
protected:
    ...
    static ImageRgba8U MakeRgbaImage(
            const int width, const int height, const int red_value, const int green_value, const int blue_value) {
    ...
    static bool CompareClouds(const PointCloud& result_cloud,
        ...
        equal = equal && CompareMatrices(result_cloud.xyzs(), expected_cloud.xyzs(), xyz_tol);
    ...
    static PointCloud DoConvert(const CameraInfo& camera_info,
        ...
        if (kUseSystem) {
            ...
            const DepthImageToPointCloud dut(camera_info, kConfiguredPixelType, scale.value_or(1.0), kFields);
            ...
            PointCloud result(0, kFields);
            if (kFields & pc_flags::kRGBs) {
                DepthImageToPointCloud::Convert(camera_info, camera_pose, depth_image, color_image, scale, &result);
            } else {
                DepthImageToPointCloud::Convert(camera_info, camera_pose, depth_image, std::nullopt, scale, &result);
            }
            return result;
        }
    }

    static void DoConvert(const CameraInfo& camera_info,
                          const std::optional<RigidTransformd>& camera_pose,
                          const MatrixX<Pixel>& depth_image_matrix,
                          const std::optional<systems::sensors::ImageRgba8U>& color_image,
                          const std::optional<float>& scale,
        ...
        if (kUseSystem) {
            ...
            if (kFields & pc_flags::kRGBs) {
                DepthImageToPointCloud::Convert(camera_info, camera_pose, depth_image, color_image, scale,
                                                &(cloud->get_mutable_value<PointCloud>()));
            } else {
                DepthImageToPointCloud::Convert(camera_info, camera_pose, depth_image, std::nullopt, scale,
                                                &(cloud->get_mutable_value<PointCloud>()));
            }
        }
    }
    ...
    const RigidTransformd random_transform_{RollPitchYawd(0.1, -0.2, 0.3), Vector3d(1.1, -1.2, 1.3)};
...
TYPED_TEST(DepthImageToPointCloudTest, Basic) {
    ...
    // Camera settings.
    const CameraInfo camera(kImageWidth, kImageHeight, kFocal, kFocal, kImageWidth * 0.5, kImageHeight * 0.5);

    // A constant-value depth image.
    constexpr TestFixturePixel kDepthValue = 1;
    const auto& depth_image = MatrixX<TestFixturePixel>::Constant(kImageWidth, kImageHeight, kDepthValue).eval();

    // A constant-value RGB image.
    constexpr uint8_t kRedValue = 0;
    constexpr uint8_t kGreenValue = 127;
    constexpr uint8_t kBlueValue = 255;
    const auto& color_image = this->MakeRgbaImage(kImageWidth, kImageHeight, kRedValue, kGreenValue, kBlueValue);
    ...
    for (int v = 0; v < depth_image.cols(); ++v) {
        for (int u = 0; u < depth_image.rows(); ++u) {
            ...
            expected_cloud.mutable_xyzs().col(i) = Vector3f(expected_x, expected_y, kDepthValue);

            if (TestFixture::kFields & pc_flags::kRGBs) {
                expected_cloud.mutable_rgbs().col(i) = Vector3<uint8_t>(kRedValue, kGreenValue, kBlueValue);
            }
        }
    }
    ...
    result = this->DoConvert(camera, std::nullopt, depth_image, color_image, std::nullopt);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud, kDistanceTolerance));

    // Now with scale factor.
    result = this->DoConvert(camera, std::nullopt, depth_image, color_image, 0.001);
    if (TestFixture::kFields & pc_flags::kRGBs) {
        EXPECT_EQ(result.rgbs(), expected_cloud.rgbs());
    }
    EXPECT_TRUE(CompareMatrices(result.xyzs(), expected_cloud.xyzs() * 0.001, kDistanceTolerance));
    ...
    result = this->DoConvert(camera, pose, depth_image, color_image, std::nullopt);
    if (TestFixture::kFields & pc_flags::kRGBs) {
        EXPECT_EQ(result.rgbs(), expected_cloud.rgbs());
    }
    EXPECT_TRUE(CompareMatrices(result.xyzs().row(2), (expected_z_row + pose.translation().z()).matrix(),
                                kDistanceTolerance));
    ...
    EXPECT_TRUE(CompareMatrices(result.xyzs().row(2), (expected_z_row * 0.001 + pose.translation().z()).matrix(),
                                kDistanceTolerance));
}
...
TYPED_TEST(DepthImageToPointCloudTest, NanValue) {
    ...
    constexpr bool is_meaningful_nan = !std::is_same_v<TestFixturePixel, uint16_t>;
    ...
    const TestFixturePixel single_pixel = is_meaningful_nan ? kFloatNaN : throw std::runtime_error("");
    ...
    const auto& depth_image = Vector1<TestFixturePixel>::Constant(single_pixel).eval();

    // A single-pixel RGB image.
    const uint8_t kRedValue = 234;
    const uint8_t kGreenValue = 0;
    const uint8_t kBlueValue = 58;
    const auto& color_image = this->MakeRgbaImage(1, 1, kRedValue, kGreenValue, kBlueValue);

    PointCloud expected_cloud(1, TestFixture::kFields);
    expected_cloud.mutable_xyzs() = Vector3f::Constant(kFloatNaN).eval();
    if (TestFixture::kFields & pc_flags::kRGBs) {
        expected_cloud.mutable_rgbs() = Vector3<uint8_t>(kRedValue, kGreenValue, kBlueValue);
    }
...
TYPED_TEST(DepthImageToPointCloudTest, TooNearFar) {
    ...
    const auto& camera = this->single_pixel_camera_;
    const auto& pose = this->random_transform_;
    const auto& depth_image_near = Vector1<TestFixturePixel>::Constant(Traits::kTooClose).eval();
    const auto& depth_image_far = Vector1<TestFixturePixel>::Constant(Traits::kTooFar).eval();
    ...
    // {without scale, with scale}.
    result = this->DoConvert(camera, std::nullopt, depth_image_near, color_image, std::nullopt);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, std::nullopt, depth_image_near, color_image, 0.1);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, std::nullopt, depth_image_far, color_image, std::nullopt);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, std::nullopt, depth_image_far, color_image, 0.1);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, pose, depth_image_near, color_image, std::nullopt);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, pose, depth_image_near, color_image, 0.1);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));

    result = this->DoConvert(camera, pose, depth_image_far, color_image, std::nullopt);
    EXPECT_TRUE(TestFixture::CompareClouds(result, expected_cloud));
...
TYPED_TEST(DepthImageToPointCloudTest, ResetStorage) {
    ...
    this->DoConvert(camera, std::nullopt, depth_image, color_image, std::nullopt, abstract_value.get());
    ...
    if (TestFixture::kUseSystem) {
        ...
        abstract_value = std::make_unique<Value<PointCloud>>(1, pc_flags::kXYZs | pc_flags::kNormals);
        EXPECT_THROW(
                this->DoConvert(camera, std::nullopt, depth_image, color_image, std::nullopt, abstract_value.get()),
                std::exception);
    } else {
