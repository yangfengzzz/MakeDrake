...
std::ostream& join(std::ostream& os, const std::vector<std::string>& elements, const std::string& delim) {
    for (size_t i = 0; i < elements.size(); ++i) {
        os << elements[i];
        if (i + 1 < elements.size()) os << delim;
    }
    return os;
}
...
std::ostream& operator<<(std::ostream& os, const Fields& fields) {
    ...
    if (fields.contains(kXYZs)) values.push_back("kXYZs");
    if (fields.contains(kNormals)) values.push_back("kNormals");
    if (fields.contains(kRGBs)) values.push_back("kRGBs");
