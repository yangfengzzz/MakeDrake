...
public:
    ...
    constexpr DescriptorType(int size, const char* name) : size_(size), name_(name) {}

    int size() const { return size_; }
    std::string name() const { return name_; }
    bool operator==(const DescriptorType& other) const { return size_ == other.size_ && name() == other.name(); }
    bool operator!=(const DescriptorType& other) const { return !(*this == other); }
...
public:
    ...
    Fields(const DescriptorType& descriptor_type) : descriptor_type_(descriptor_type) {}
...
namespace fmt {
template <>
struct formatter<drake::perception::pc_flags::Fields> : drake::ostream_formatter {};
}  // namespace fmt
