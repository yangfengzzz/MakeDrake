#include "perception/point_cloud_to_lcm.h"
...
void Calc(double time, const std::string& frame_name, const PointCloud& cloud, lcmt_point_cloud* message) {
    ...
    const int num_fields = (has_xyzs ? 3 : 0) + (has_rgbs ? 1 : 0) + (has_normals ? 3 : 0);
    ...
    const Eigen::Ref<const Matrix3X<uint8_t>> rgbs =
            has_rgbs ? cloud.rgbs() : Eigen::Ref<const Matrix3X<uint8_t>>(empty_uint8);
    const Eigen::Ref<const Matrix3X<float>> normals =
            has_normals ? cloud.normals() : Eigen::Ref<const Matrix3X<float>>(empty_float);
...
PointCloudToLcm::PointCloudToLcm(std::string frame_name) : frame_name_(std::move(frame_name)) {
