...
public:
    ...
    explicit PointCloud(int new_size = 0, pc_flags::Fields fields = pc_flags::kXYZs, bool skip_initialize = false);
    ...
    /// @pre `has_normals()` must be true.
    Eigen::Ref<Vector3<T>> mutable_normal(int i) { return mutable_normals().col(i); }
    ...
    /// @pre `has_descriptors()` must be true.
    Eigen::Ref<VectorX<D>> mutable_descriptor(int i) { return mutable_descriptors().col(i); }
