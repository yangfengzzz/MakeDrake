...
int DoMain() {
    ...
    auto pc_maximal = PointCloud(
            FLAGS_size, pc_flags::kXYZs | pc_flags::kRGBs | pc_flags::kNormals | pc_flags::kDescriptorCurvature);
    ...
    double duration_xyz = static_cast<std::chrono::duration<double>>(post_xyz - start).count();
    double duration_max = static_cast<std::chrono::duration<double>>(post_max - start).count();

    std::cout << "xyz only time " << duration_xyz << std::endl << "maximal fields time " << duration_max << std::endl;
    return 0;
}
}  // namespace
}  // namespace perception
