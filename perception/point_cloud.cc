...
public:
    ...
    void resize(int new_size) {
        size_ = new_size;
        if (fields_.contains(pc_flags::kXYZs)) xyzs_.conservativeResize(NoChange, new_size);
        if (fields_.contains(pc_flags::kNormals)) normals_.conservativeResize(NoChange, new_size);
        if (fields_.contains(pc_flags::kRGBs)) rgbs_.conservativeResize(NoChange, new_size);
        if (fields_.has_descriptor()) descriptors_.conservativeResize(NoChange, new_size);
        CheckInvariants();
    }
    ...
    void UpdateFields(pc_flags::Fields f) {
        ...
        normals_.conservativeResize(NoChange, f.contains(pc_flags::kNormals) ? size_ : 0);
        ...
        descriptors_.conservativeResize(f.descriptor_type().size(), f.has_descriptor() ? size_ : 0);
        fields_ = f;
        CheckInvariants();
    }
...
pc_flags::Fields ResolveFields(const PointCloud& other, pc_flags::Fields fields) {
...
pc_flags::Fields FindAddedFields(pc_flags::Fields old_fields, pc_flags::Fields new_fields) {
    pc_flags::Fields added_fields(pc_flags::kNone);
    for (const auto field : {pc_flags::kXYZs, pc_flags::kNormals, pc_flags::kRGBs}) {
        if (!old_fields.contains(field) && new_fields.contains(field)) added_fields |= field;
    }

    if (new_fields.has_descriptor() && old_fields.descriptor_type() != new_fields.descriptor_type()) {
...
PointCloud::PointCloud(int new_size, pc_flags::Fields fields, bool skip_initialize) {
    if (fields == pc_flags::kNone) throw std::runtime_error("Cannot construct a PointCloud without fields");
    if (fields.contains(pc_flags::kInherit)) throw std::runtime_error("Cannot construct a PointCloud with kInherit");
...
PointCloud::PointCloud(PointCloud&& other) : PointCloud(0, other.storage_->fields(), true) {
...
void PointCloud::SetFields(pc_flags::Fields new_fields, bool skip_initialize) {
    ...
    if (old_fields == new_fields) return;
    storage_->UpdateFields(new_fields);

    if (!skip_initialize) {
        // Default-initialize containers for newly added fields.
        const pc_flags::Fields added_fields = FindAddedFields(old_fields, new_fields);
        if (added_fields.contains(pc_flags::kXYZs)) mutable_xyzs().setConstant(kDefaultValue);
        if (added_fields.contains(pc_flags::kNormals)) mutable_normals().setConstant(kDefaultValue);
        if (added_fields.contains(pc_flags::kRGBs)) mutable_rgbs().setConstant(kDefaultColor);
        if (added_fields.has_descriptor()) mutable_descriptors().setConstant(kDefaultValue);
    }
}
...
void PointCloud::SetFrom(const PointCloud& other, pc_flags::Fields fields_in, bool allow_resize) {
    ...
    if (allow_resize) {
        ...
        throw std::runtime_error(fmt::format("SetFrom: {} != {}", new_size, old_size));
    }
...
bool PointCloud::has_descriptors(const pc_flags::DescriptorType& descriptor_type) const {
...
PointCloud PointCloud::Crop(const Eigen::Ref<const Vector3<T>>& lower_xyz,
    ...
    for (int i = 0; i < size(); ++i) {
        if (((xyzs().col(i).array() >= lower_xyz.array()) && (xyzs().col(i).array() <= upper_xyz.array())).all()) {
...
void PointCloud::FlipNormalsTowardPoint(const Eigen::Ref<const Vector3<T>>& p_CP) {
...
PointCloud Concatenate(const std::vector<PointCloud>& clouds) {
    ...
    for (int i = 0; i < num_clouds; ++i) {
        ...
        if (new_cloud.has_descriptors()) {
            new_cloud.mutable_descriptors().middleCols(index, s) = clouds[i].descriptors();
        }
        index += s;
    }
    return new_cloud;
...
PointCloud PointCloud::VoxelizedDownSample(const double voxel_size, const Parallelism parallelize) const {
    DRAKE_THROW_UNLESS(has_xyzs());
    ...
    DynamicSpatialHashedVoxelGrid<std::vector<int>> dynamic_voxel_grid(chunk_sizes, default_chunk_value,
                                                                       num_expected_chunks);
    ...
    for (int i = 0; i < size(); ++i) {
        if (my_xyzs.col(i).array().isFinite().all()) {
            auto chunk_query = dynamic_voxel_grid.GetLocationMutable3d(my_xyzs.col(i).cast<double>());
            if (chunk_query) {
                ...
                dynamic_voxel_grid.SetLocation3d(my_xyzs.col(i).cast<double>(), DSHVGSetType::SET_CHUNK, {i});
            }
        }
    }

    // Initialize downsampled cloud.
    PointCloud down_sampled(dynamic_voxel_grid.GetImmutableInternalChunks().size(), storage_->fields());
    ...
    const auto process_voxel = [&storage, &down_sampled_storage, this_has_normals, this_has_rgbs, this_has_descriptors](
        ...
        Eigen::VectorXd descriptor{Eigen::VectorXd::Zero(this_has_descriptors ? storage.descriptors().rows() : 0)};
        int num_normals{0};
        int num_descriptors{0};

        for (int index_in_this : indices_in_this) {
            xyz += storage.xyzs().col(index_in_this).cast<double>();
            if (this_has_normals && storage.normals().col(index_in_this).array().isFinite().all()) {
            ...
            if (this_has_descriptors && storage.descriptors().col(index_in_this).array().isFinite().all()) {
                descriptor += storage.descriptors().col(index_in_this).cast<double>();
                ++num_descriptors;
            }
        }
        down_sampled_storage.xyzs().col(index_in_down_sampled) = (xyz / indices_in_this.size()).cast<T>();
        if (this_has_normals) {
            down_sampled_storage.normals().col(index_in_down_sampled) = (normal / num_normals).normalized().cast<T>();
        }
        if (this_has_rgbs) {
            down_sampled_storage.rgbs().col(index_in_down_sampled) = (rgb / indices_in_this.size()).cast<C>();
        }
        if (this_has_descriptors) {
            down_sampled_storage.descriptors().col(index_in_down_sampled) = (descriptor / num_descriptors).cast<D>();
        }
    };
    ...
    if (operate_in_parallel) {
        ...
        voxel_indices.reserve(dynamic_voxel_grid.GetImmutableInternalChunks().size());
        for (const auto& [chunk_region, chunk] : dynamic_voxel_grid.GetImmutableInternalChunks()) {
            unused(chunk_region);
            const std::vector<int>& indices_in_this = chunk.GetIndexImmutable(kSingleVoxel).Value();
...
#endif
        for (int index_in_down_sampled = 0; index_in_down_sampled < static_cast<int>(voxel_indices.size());
             ++index_in_down_sampled) {
            process_voxel(index_in_down_sampled, *voxel_indices[index_in_down_sampled]);
        }
    } else {
...
bool PointCloud::EstimateNormals(const double radius,
    ...
    nanoflann::KDTreeEigenMatrixAdaptor<Eigen::MatrixX3f, 3, nanoflann::metric_L2_Simple> kd_tree(3, data);
...
#pragma omp parallel for num_threads(parallelize.num_threads())
    ...
    for (int i = 0; i < size(); ++i) {
        ...
        const int num_neighbors =
                kd_tree.index_->knnSearch(xyz(i).data(), num_closest, indices.data(), distances.data());
        ...
        for (int j = 0; j < num_neighbors; ++j) {
            if (distances[j] <= squared_radius) {
                const Eigen::VectorXd x_minus_mean = xyz(indices[j]).cast<double>() - mean;
