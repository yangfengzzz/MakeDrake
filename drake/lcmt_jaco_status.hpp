...
int lcmt_jaco_status::_encodeNoHash(void* buf, int offset, int maxlen) const {
    ...
    if (this->num_fingers > 0) {
        tlen = __double_encode_array(buf, offset + pos, maxlen - pos, &this->finger_torque_external[0],
                                     this->num_fingers);
        if (tlen < 0)
            return tlen;
        else
...
int lcmt_jaco_status::_decodeNoHash(const void* buf, int offset, int maxlen) {
    ...
    if (this->num_fingers) {
        ...
        tlen = __double_decode_array(buf, offset + pos, maxlen - pos, &this->finger_torque_external[0],
                                     this->num_fingers);
        if (tlen < 0)
            return tlen;
        else
