...
GTEST_TEST(RollPitchYaw, ToRotationMatrix) {
    ...
    const Matrix3d m_eigen = (Eigen::AngleAxisd(y, Vector3d::UnitZ()) * Eigen::AngleAxisd(p, Vector3d::UnitY()) *
...
rDt;
    ...
    EXPECT_TRUE(CompareMatrices(RDt, MDt, 16 * kEpsilon, MatrixCompareType::absolute));
}
...
GTEST_TEST(RollPitchYaw, CalcAngularVelocityFromRpyDtAndViceVersa) {
    ...
    EXPECT_TRUE(CompareMatrices(w_AD_A, R_AD * w_AD_D, 4 * kEpsilon, MatrixCompareType::absolute));
    ...
    const Matrix3<double> Np = rpy.CalcMatrixRelatingRpyDtToAngularVelocityInParent();
    ...
    const Matrix3<double> Nc = rpy.CalcMatrixRelatingRpyDtToAngularVelocityInChild();
...
cos(p);
    // clang-format on
    EXPECT_TRUE(CompareMatrices(Np * Np_inv, Matrix3<double>::Identity(), 4 * kEpsilon, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(Nc * Nc_inv, Matrix3<double>::Identity(), 4 * kEpsilon, MatrixCompareType::absolute));
    const Vector3d w_AD_A_expected = Np_inv * rpyDt;
    const Vector3d w_AD_D_expected = Nc_inv * rpyDt;
    EXPECT_TRUE(CompareMatrices(w_AD_A, w_AD_A_expected, 16 * kEpsilon, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(w_AD_D, w_AD_D_expected, 16 * kEpsilon, MatrixCompareType::absolute));
    ...
    const Vector3d rpyDt_calculated_from_parent = rpy.CalcRpyDtFromAngularVelocityInParent(w_AD_A);
    const Vector3d rpyDt_calculated_from_child = rpy.CalcRpyDtFromAngularVelocityInChild(w_AD_D);
    EXPECT_TRUE(CompareMatrices(rpyDt_calculated_from_parent, rpyDt, 16 * kEpsilon, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(rpyDt_calculated_from_child, rpyDt, 16 * kEpsilon, MatrixCompareType::absolute));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(rpyA.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);

    const RollPitchYaw<double> rpyB(0.2, -M_PI / 2, 0.4);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyB.CalcRpyDtFromAngularVelocityInParent(w_AD_A), expected_message_parentB);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyB.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);

    const RollPitchYaw<double> rpyC(0.2, 3 * M_PI / 2, 0.4);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyC.CalcRpyDtFromAngularVelocityInParent(w_AD_A), expected_message_parentB);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyC.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);

    const RollPitchYaw<double> rpyD(0.2, -3 * M_PI / 2, 0.4);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyD.CalcRpyDtFromAngularVelocityInParent(w_AD_A), expected_message_parentB);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyD.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);

    const RollPitchYaw<double> rpyE(0.2, 3 * M_PI / 2 + 1E-8, 0.4);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyE.CalcRpyDtFromAngularVelocityInParent(w_AD_A), expected_message_parentB);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyE.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);

    const RollPitchYaw<double> rpyF(0.2, -3 * M_PI / 2 + 1E-8, 0.4);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyF.CalcRpyDtFromAngularVelocityInParent(w_AD_A), expected_message_parentB);
    DRAKE_EXPECT_THROWS_MESSAGE(rpyF.CalcRpyDtFromAngularVelocityInChild(w_AD_D), expected_message_childB);
}
...
GTEST_TEST(RollPitchYaw, PrecisionOfAngularVelocityFromRpyDtAndViceVersa) {
    ...
    const double tolerance = RollPitchYaw<double>::GimbalLockPitchAngleTolerance();
    ...
    for (double i = -2.00; i <= 2.001; i += (-0.9 <= i && i <= 0.8) ? 1 : 0.05) {
        ...
        if (is_imprecise) {
            ...
            expected_message =
                    ...
                    ".*gimbal-lock.*";
            DRAKE_EXPECT_THROWS_MESSAGE(rpyDDt = rpy.CalcRpyDDtFromRpyDtAndAngularAccelInParent(rpyDt, alphaA),
                                        expected_message);
        } else {
            ...
            EXPECT_TRUE(1E-3 / tolerance <= max_rpyDt && max_rpyDt <= 1E3 / tolerance);
            EXPECT_TRUE(1E-6 / (tolerance * tolerance) <= max_rpyDDt && max_rpyDDt <= 1E6 / (tolerance * tolerance));
            ...
            const Vector3d w_error(w_diff(0) / wA(0), w_diff(1) / wA(1), w_diff(2) / wA(2));
            ...
            EXPECT_LE(max_error,
                      256 * kEpsilon);  // Up to 8 bits lost.
        }
    }
            ...
            for (double yaw = -M_PI; yaw <= M_PI + tol; yaw += 10 * deg) {
                ...
                if (rpy.
                    ...
                    DRAKE_EXPECT_THROWS_MESSAGE(
                            rpyDDt = rpy.CalcRpyDDtFromRpyDtAndAngularAccelInParent(rpyDt, alpha_AD_A),
                            expected_message);
                    expected_message =
                            "RollPitchYaw::CalcRpyDDtFromAngularAccelInChild()"
                            ".*gimbal-lock.*";
                    DRAKE_EXPECT_THROWS_MESSAGE(
                            rpyDDt_verify = rpy.CalcRpyDDtFromAngularAccelInChild(rpyDt, alpha_AD_D), expected_message);
                    continue;
                }

                rpyDDt = rpy.CalcRpyDDtFromRpyDtAndAngularAccelInParent(rpyDt, alpha_AD_A);
                rpyDDt_verify = rpy.CalcRpyDDtFromAngularAccelInChild(rpyDt, alpha_AD_D);
                ...
                EXPECT_TRUE(CompareMatrices(rpyDDt, rpyDDt_verify, tolerance, MatrixCompareType::absolute));
                ...
                const double rDDt = (alfAx * cos(y) + alfAy * sin(y) + pDt * yDt + sin(p) * pDt * rDt) / cos(p);
                const double pDDt = alfAy * cos(y) - alfAx * sin(y) - cos(p) * rDt * yDt;
                const double yDDt = alfAz + cos(p) * pDt * rDt +
                                    tan(p) * (alfAx * cos(y) + alfAy * sin(y) + pDt * yDt + sin(p) * pDt * rDt);
                const Vector3d rpyDDt_expected(rDDt, pDDt, yDDt);
                EXPECT_TRUE(CompareMatrices(rpyDDt, rpyDDt_expected, tolerance, MatrixCompareType::absolute));
            }
        }
    }
}
...
GTEST_TEST(RollPitchYaw, SymbolicTest) {
    ...
    EXPECT_EQ(dut1.IsNearlyEqualTo(dut2, 1e-3)
                      .

              to_string(),
              fmt::format("(max({}, max({}, {})) <= 0.001)", "abs((r1 - r2))", "abs((p1 - p2))", "abs((y1 - y2))")

    );

    EXPECT_THAT(dut1.IsNearlySameOrientation(dut2, 1e-3)
                        .
    ...
    EXPECT_THAT(RollPitchYaw<Expression>::DoesCosPitchAngleViolateGimbalLockTolerance(cos(p1)).
    ...
    EXPECT_THAT(RollPitchYaw<Expression>::IsValid(vec).
...
GTEST_TEST(RollPitchYaw, StreamInsertionOperator) {
    ...
    EXPECT_EQ(rpy_expected_string, streamA.
    ...
    EXPECT_EQ(rpy_expected_string, streamB.
    ...
    EXPECT_EQ(rpy_expected_string, streamC.
    ...
    const RollPitchYaw<symbolic::Expression> rpy_symbolic_huge(big_expr, big_expr, big_expr);
    std::stringstream streamD;
    streamD << rpy_symbolic_huge;
    rpy_expected_string = "rpy = <symbolic> <symbolic> <symbolic>";
    EXPECT_EQ(rpy_expected_string, streamD.
