...
GTEST_TEST(CalculateQuaternionDtFromAngularVelocityExpressedInBTest, testA) {
    ...
    const drake::Vector4<AutoDiffXd> quatDt = CalculateQuaternionDtFromAngularVelocityExpressedInB(quat, w_B);

    // Verify Drake's quatDt versus MotionGenesis (MG) results.
    const Eigen::Vector4d quatDt_MG = GetQuaternionDtAssociatedWith30DegRotation();
...
<<  0.4330127018922194, -0.1620185174601965,  0.175;
e2Dt_partial_MG.

tail<3>()

<<  0.1620185174601965,  0.4330127018922194, -0.075;
e3Dt_partial_MG.
...
GTEST_TEST(CalculateAngularVelocityExpressedInBFromQuaternionDtTest, testA) {
    ...
    const drake::Vector3<AutoDiffXd> w = CalculateAngularVelocityExpressedInBFromQuaternionDt(quat, quatDt);
...
GTEST_TEST(IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB, testA) {
    ...
    const double epsilon = std::numeric_limits<double>::epsilon();
    const bool a = IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat, quatDt, w_B, 8 * epsilon);
    const Eigen::Vector3d w_bad = 0.99 * w_B;
    const bool b = IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat, quatDt, w_bad, 0.01);
    EXPECT_TRUE(a);
    EXPECT_FALSE(b);
}
...
void CheckQuaternionToAngleAxis(double w, double x, double y, double z) {
    ...
    const Eigen::AngleAxisd angle_axis = internal::QuaternionToAngleAxisLikeEigen(quaternion);
...
GTEST_TEST(TestQuaternionToAngleAxisLikeEigen, TestSymbolic) {
    ...
    const Eigen::AngleAxis<symbolic::Expression> angle_axis = internal::QuaternionToAngleAxisLikeEigen(quaternion);
    EXPECT_EQ(angle_axis
                      .
