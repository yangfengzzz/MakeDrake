...
GTEST_TEST(RigidTransform, ConstructorAndSet) {
    ...
    X.
            ...
            set(RotationMatrix<double>::Identity(), Vector3d(0, 0, 0)
    ...
    if (kDrakeAssertIsArmed) {
        EXPECT_THROW(RigidTransform<double>(RotationMatrix<double>(bad), p), std::exception);
    }
}
...
GTEST_TEST(RigidTransform, IsIdentity) {
    ...
    X2.
            ...
            set_rotation(RotationMatrix<double>::Identity());
...
GTEST_TEST(RigidTransform, OperatorMultiplyByRigidTransform) {
    ...
    EXPECT_TRUE(
            CompareMatrices(symbolic::Evaluate(X_CAx.GetAsMatrix34()), X_CA_expected.GetAsMatrix34(), 32 * kEpsilon));

    RigidTransform<Expression> will_be_X_CAx(X_CBx);
    will_be_X_CAx *= X_BAx;
    EXPECT_TRUE(CompareMatrices(symbolic::Evaluate(will_be_X_CAx.GetAsMatrix34()), X_CA_expected.GetAsMatrix34(),
                                32 * kEpsilon));
}
...
GTEST_TEST(RigidTransform, InvertAndCompose) {
    ...
    EXPECT_TRUE(
            CompareMatrices(symbolic::Evaluate(X_ACx.GetAsMatrix34()), X_AC_expected.GetAsMatrix34(), 32 * kEpsilon));
}
...
0, 1, 0,
0, 0,
x;  // Not necessarily an identity matrix.
    // clang-format on
...
GTEST_TEST(RigidTransform, SetRotationMethods) {
    ...
    EXPECT_TRUE(X_AB.
                ...
                rotation()
                        ...
                        .IsExactlyEqualTo(RotationMatrix<double>(rpy)));
    EXPECT_EQ(X_AB.
    ...
    EXPECT_TRUE(X_AB.
                ...
                rotation()
                        ...
                        .IsExactlyEqualTo(RotationMatrix<double>(quat)));
    EXPECT_EQ(X_AB.
    ...
    EXPECT_TRUE(X_AB.
                ...
                rotation()
                        ...
                        .IsExactlyEqualTo(RotationMatrix<double>(angle_axis)));
    EXPECT_EQ(X_AB.
...
GTEST_TEST(RigidTransform, OperatorMultiplyByTranslation3AndViceVersa) {
    ...
    EXPECT_TRUE(X_CA.
                ...
                translation()
                        ...
                        .isApprox(p_CoAo_C, 32 * kEpsilon));

    // Verify X_AC is the inverse of X_CA.
    EXPECT_TRUE(X_AC.IsNearlyEqualTo(X_CA.
...
GTEST_TEST(RigidTransform, OperatorMultiplyByVector4) {
    ...
    EXPECT_EQ(p4_AoQ_A(3),
              1.0);  // Ensure the 4ᵗʰ element is 1.
    ...
    EXPECT_EQ(vec4_A(3),
              0.0);  // Ensure the 4ᵗʰ element is 0.
...
GTEST_TEST(RigidTransform, OperatorMultiplyByMatrix3X) {
    ...
    EXPECT_EQ(decltype(p_AoQ_A)::RowsAtCompileTime,

              3);

    EXPECT_EQ(decltype(p_AoQ_A)::ColsAtCompileTime,
    ...
    EXPECT_EQ(decltype(p_AoP_A)::RowsAtCompileTime,

              3);
    EXPECT_EQ(p_AoP_A.

              cols(),
              number_of_position_vectors
    ...
    const Eigen::MatrixXd p_AoR_A = X_AB * (Eigen::Vector3d(1, 2, 3) * Eigen::RowVector4d(1, 2, 3, 4));
    EXPECT_EQ(p_AoR_A.
    ...
    const auto p_AoR_A_expected = X_AB * (Eigen::MatrixXd(3, 4) << Eigen::Vector3d(1, 2, 3), Eigen::Vector3d(2, 4, 6),
                                          Eigen::Vector3d(3, 6, 9), Eigen::Vector3d(4, 8, 12))
                                                 .finished();

    EXPECT_EQ(decltype(p_AoR_A_expected)::RowsAtCompileTime,

              3);
    EXPECT_EQ(p_AoR_A_expected.
...
void VerifyStreamInsertionOperator(const RigidTransform<T> X_AB,
    ...
    EXPECT_TRUE(CompareMatrices(Vector3<double>(roll, pitch, yaw), rpy_expected, 4 * kEpsilon));
...
GTEST_TEST(RigidTransform, StreamInsertionOperator) {
    ...
    VerifyStreamInsertionOperator(RigidTransform<AutoDiffXd>(rpy_autodiff, xyz_autodiff), rpy_values,
                                  xyz_expected_string);
    ...
    std::stringstream stream;
    stream << X_symbolic;
    const std::string expected_string = "rpy = <symbolic> <symbolic> <symbolic> xyz = x y z";
    EXPECT_EQ(expected_string, stream.
    ...
    VerifyStreamInsertionOperator(RigidTransform<Expression>(rpy_const_expr, xyz_const_expr), Vector3d{-0.1, 0.2, 0.3},
                                  "xyz = -10 20 30");
}
