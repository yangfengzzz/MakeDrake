...
void CheckHopfToQuaternion(double theta, double phi, double psi) {
    ...
    EXPECT_NEAR(std::pow(quat.w(), 2) + std::pow(quat.x(), 2) + std::pow(quat.y(), 2) + std::pow(quat.z(), 2), 1., tol);
    ...
    if (theta == 0.) {
        ...
        EXPECT_NEAR(std::fmod(std::abs(hopf_coordinate[1] + hopf_coordinate[2] / 2 - (phi + psi / 2)), 2 * M_PI), 0.,
                    tol);
    } else {
        EXPECT_NEAR(hopf_coordinate(0), theta, tol);
        EXPECT_NEAR(std::fmod(std::abs(hopf_coordinate(1) - phi), 2 * M_PI), 0, tol);
        EXPECT_NEAR(hopf_coordinate(2), psi, tol);
    }
}
...
GTEST_TEST(TestHopfCoordinate, TestQuaternion) {
    ...
    for (int i = 0; i < theta_all.

                        rows();

         ++i) {
        for (int j = 0; j < phi_all.

                            rows();

             ++j) {
            for (int k = 0; k < psi_all.
    ...
    CheckHopfRange(hopf);
    Eigen::Quaternion quat_reconstruct = HopfCoordinateToQuaternion(hopf(0), hopf(1), hopf(2));
