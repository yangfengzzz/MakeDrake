...
class FastPoseCompositionFunctions : public hwy::TestWithParamTargetAndT<Param> {
protected:
    void SetUp() override {
...
auto MakePermutations() {
    return ::testing::Values(Param{.args = "ABO", .inverse = false}, Param{.args = "ABA", .inverse = false},
                             Param{.args = "ABB", .inverse = false}, Param{.args = "AAA", .inverse = false},
                             Param{.args = "ABO", .inverse = true}, Param{.args = "ABA", .inverse = true},
                             Param{.args = "ABB", .inverse = true}, Param{.args = "AAA", .inverse = true});
}
...
TEST_P(FastPoseCompositionFunctions, ComposeRR) {
    ...
    const Matrix3d expected = (param.inverse ? MakeAinv() : MakeA()).rotation().matrix() * arg2->matrix();
...
TEST_P(FastPoseCompositionFunctions, ComposeXX) {
    ...
    // alias their input and output.
    DRAKE_DEMAND(param.args.at(0) == 'A');
    const Matrix4d expected = (param.inverse ? MakeAinv() : MakeA()).GetAsMatrix4() * arg2->GetAsMatrix4();
