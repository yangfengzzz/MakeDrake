...
#include "math/hopf_coordinate.h"
...
void CheckUniformHopfCoordinate(const std::vector<Eigen::Vector3d>& hopf_coordinates,
    ...
    const Eigen::VectorXd phi_all = Eigen::VectorXd::LinSpaced(num_intervals + 1, 0, 2 * M_PI);
    const Eigen::VectorXd psi_all = Eigen::VectorXd::LinSpaced(num_intervals + 1, -M_PI, M_PI);
    ...
    for (int i = 0; i < num_intervals; ++i) {
        for (int j = 0; j < num_intervals; ++j) {
            for (int k = 0; k < num_intervals; ++k) {
                const double cell_measure = (std::cos(theta_all[i]) - std::cos(theta_all[i + 1])) *
                                            (phi_all[j + 1] - phi_all[j]) * (psi_all[k + 1] - psi_all[k]);
                const double EY = cell_measure / (8 * std::pow(M_PI, 2));
                const double VarY = EY - EY * EY;
                EXPECT_NEAR(static_cast<double>(counts[i][j][k]) / N, EY, fudge_factor * 1.645 * std::sqrt(VarY / N));
            }
        }
    }
}
...
GTEST_TEST(RandomRotationTest, UniformlyRandomRotation) {
    ...
    for (int i = 0; i < num_samples; ++i) {
        ...
        rotation_matrices[i] = UniformlyRandomRotationMatrix<double>(&generator)
                                       .
