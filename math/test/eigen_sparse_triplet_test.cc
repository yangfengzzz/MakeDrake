...
void checkTriplet(const Eigen::SparseMatrix<T, options>& sp_mat) {
    ...
    for (int i = 0; i < static_cast<int>(row_indices.size()); ++i) {
        triplets_from_vectors.push_back(Eigen::Triplet<T>(row_indices[i], col_indices[i], val[i]));
    }
    Eigen::SparseMatrix<T, options> sp_mat_from_vectors(sp_mat.rows(), sp_mat.cols());
    sp_mat_from_vectors.setFromTriplets(triplets_from_vectors.begin(), triplets_from_vectors.end());
...
GTEST_TEST(testEigenSparseTriplet, sparseToTriplet) {
    ...
    sp_mat1.setFromTriplets(triplets.
    ...
    sp_mat3.setFromTriplets(triplets.
    ...
    sp_mat4.setFromTriplets(triplets.
