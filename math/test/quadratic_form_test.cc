...
void CheckDecomposePSDmatrixIntoXtransposeTimesX(const Eigen::Ref<const Eigen::MatrixXd> &Y,
    ...
    EXPECT_TRUE(CompareMatrices(X.transpose() * X, Y, check_tol, MatrixCompareType::absolute));
...
GTEST_TEST(TestDecomposePSDmatrixIntoXtransposeTimesX, Test0) {
    CheckDecomposePSDmatrixIntoXtransposeTimesX(Eigen::Matrix3d::Identity(), kDefaultZeroTol
...
GTEST_TEST(TestDecomposePSDmatrixIntoXtransposeTimesX, negativeY) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DecomposePSDmatrixIntoXtransposeTimesX(-Eigen::Matrix3d::Identity(), 0),
    ...
    // matrix.
    Eigen::MatrixXd X =
            DecomposePSDmatrixIntoXtransposeTimesX(-Eigen::Matrix3d::Identity(), 0, true /* return_empty_if_not_psd */);
    EXPECT_EQ(X.
...
1, 2, 1, 1;
    // clang-format on
    EXPECT_THROW(DecomposePSDmatrixIntoXtransposeTimesX(Y, 0), std::runtime_error);

    // If return_empty_if_not_psd is true, the function should return an empty
    // matrix.
    Eigen::MatrixXd X = DecomposePSDmatrixIntoXtransposeTimesX(Y, 0, true /* return_empty_if_not_psd */);
    EXPECT_EQ(X.
...
GTEST_TEST(TestDecomposePSDmatrixIntoXtransposeTimesX, almost_psd_Y) {
    ...
    const Eigen::Matrix3d Y = U.matrix() * Eigen::Vector3d(1, -1E-10, 2).asDiagonal() * U.transpose().matrix();
...
void CheckDecomposePositiveQuadraticForm(const Eigen::Ref<const Eigen::MatrixXd> &Q,
    ...
    EXPECT_TRUE(CompareMatrices(R.transpose() * R, Q, tol_check, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(R.transpose() * d, b / 2, tol_check, MatrixCompareType::absolute));
...
0, 0, 0;
    ...
    EXPECT_THROW(DecomposePositiveQuadraticForm(Q, b, c, -kDefaultZeroTol), std::runtime_error);
}
...
void CheckBalancing(const Eigen::Matrix3d &S, const Eigen::Matrix3d &P, const Eigen::MatrixXd &T) {
    ...
    EXPECT_TRUE(CompareMatrices(D, Eigen::MatrixXd(D.diagonal().asDiagonal()), 1e-11));
    EXPECT_TRUE(CompareMatrices(Dinv, Eigen::MatrixXd(Dinv.diagonal().asDiagonal()), 1e-11));
