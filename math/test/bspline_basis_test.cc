...
TYPED_TEST(BsplineBasisTests, ConstructorTest) {
    ...
    const std::vector<T> clamped_uniform_0_to_1{0, 0, 0, 0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1, 1, 1, 1};
    ...
    check_basis(BsplineBasis<T>(order, num_basis_functions, KnotVectorType::kClampedUniform,
                                0 /* initial_parameter_value */, 1 /* final_parameter_value */),
                order, num_basis_functions, clamped_uniform_0_to_1);

    check_basis(BsplineBasis<T>(order, num_basis_functions, KnotVectorType::kClampedUniform,
                                1 /* initial_parameter_value */, 9 /* final_parameter_value */),
    ...
    EXPECT_EQ(BsplineBasis<T>(order, num_basis_functions),
              BsplineBasis<T>(order, num_basis_functions, KnotVectorType::kClampedUniform,
                              0 /* initial_parameter_value */, 1 /* final_parameter_value */));

    // Check the order and num_basis_functions constructor with kUniform.
    check_basis(BsplineBasis<T>(order, num_basis_functions, KnotVectorType::kUniform), order, num_basis_functions,
                uniform_0_to_1);

    // Check the order and knots constructor.
    const std::vector<T> arbitrary_knots = {-5, 0, 0.1, 0.2, 0.3, 0.4, 3, 10, 17, 25, 50, 100, 200, 400, 401};
    check_basis(BsplineBasis<T>(order, arbitrary_knots), order, num_basis_functions, arbitrary_knots);
}
...
TYPED_TEST(BsplineBasisTests, ConstructFromDoubleTest) {
    ...
    EXPECT_EQ(BsplineBasis<T>(basis_double), BsplineBasis<T>(order, num_basis_functions));
}
...
TYPED_TEST(BsplineBasisTests, ComputeActiveBasisFunctionIndicesTest) {
    ...
    EXPECT_EQ(bspline_basis_0.ComputeActiveBasisFunctionIndices({{0.9, 1.0}}), expected_indices({9, 10, 11, 12, 13}));

    // Test that the active basis functions for part of the final interval are
    // the last five (and only the last five).
    EXPECT_EQ(bspline_basis_0.ComputeActiveBasisFunctionIndices({{0.95, 1.0}}), expected_indices({9, 10, 11, 12, 13}));
    ...
    EXPECT_EQ(bspline_basis_0.ComputeActiveBasisFunctionIndices(0.0), expected_indices({0, 1, 2, 3, 4}));

    // Test an intermediate point, t = 0.43 ∈ (0.1 * i, 0.1 * (i + 1)), for i = 4.
    EXPECT_EQ(bspline_basis_0.ComputeActiveBasisFunctionIndices(0.43), expected_indices({4, 5, 6, 7, 8}));

    // Test that the last five basis functions are active when
    // t = final_parameter_value().
    EXPECT_EQ(bspline_basis_0.ComputeActiveBasisFunctionIndices(1.0), expected_indices({9, 10, 11, 12, 13}));
    ...
    EXPECT_EQ(bspline_basis_1.ComputeActiveBasisFunctionIndices(0), expected_indices({0}));
}
...
TYPED_TEST(BsplineBasisTests, EvaluateBasisFunctionIScipyComparison) {
    ...
    parameter_values.push_back(std::vector<T>{
            0.00000000000000000, 0.04166666666666666, 0.08333333333333333, 0.12500000000000000, 0.16666666666666666,
            0.20833333333333331, 0.25000000000000000, 0.50000000000000000, 0.75000000000000000, 1.00000000000000000});
    expected_basis_function_values.push_back(std::vector<T>{
            1.00000000000000000, 0.57870370370370383, 0.29629629629629639, 0.12500000000000000, 0.03703703703703705,
            0.00462962962962964, 0.00000000000000000, 0.00000000000000000, 0.00000000000000000, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{0.00000000000000000, 0.08333333333333333, 0.16666666666666666,
                                              0.25000000000000000, 0.33333333333333331, 0.41666666666666663,
                                              0.50000000000000000, 0.75000000000000000, 1.00000000000000000});
    expected_basis_function_values.push_back(std::vector<T>{
            0.00000000000000000, 0.56481481481481488, 0.51851851851851860, 0.25000000000000000, 0.07407407407407410,
            0.00925925925925927, 0.00000000000000000, 0.00000000000000000, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{0.00000000000000000, 0.12500000000000000, 0.25000000000000000,
                                              0.37500000000000000, 0.50000000000000000, 0.62500000000000000,
                                              0.75000000000000000, 1.00000000000000000});
    expected_basis_function_values.push_back(
            std::vector<T>{0.00000000000000000, 0.26041666666666663, 0.58333333333333326, 0.46875000000000000,
                           0.16666666666666666, 0.02083333333333333, 0.00000000000000000, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{0.00000000000000000, 0.16666666666666666, 0.25000000000000000,
                                              0.33333333333333331, 0.50000000000000000, 0.66666666666666663,
                                              0.75000000000000000, 0.83333333333333326, 1.00000000000000000});
    expected_basis_function_values.push_back(std::vector<T>{
            0.00000000000000000, 0.04938271604938271, 0.16666666666666666, 0.37037037037037035, 0.66666666666666663,
            0.37037037037037052, 0.16666666666666666, 0.04938271604938278, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{0.00000000000000000, 0.25000000000000000, 0.37500000000000000,
                                              0.50000000000000000, 0.62500000000000000, 0.75000000000000000,
                                              0.87500000000000000, 1.00000000000000000});
    expected_basis_function_values.push_back(
            std::vector<T>{0.00000000000000000, 0.00000000000000000, 0.02083333333333333, 0.16666666666666666,
                           0.46875000000000000, 0.58333333333333326, 0.26041666666666663, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{0.00000000000000000, 0.25000000000000000, 0.50000000000000000,
                                              0.58333333333333337, 0.66666666666666663, 0.75000000000000000,
                                              0.83333333333333326, 0.91666666666666663, 1.00000000000000000});
    expected_basis_function_values.push_back(std::vector<T>{
            0.00000000000000000, 0.00000000000000000, 0.00000000000000000, 0.00925925925925927, 0.07407407407407403,
            0.25000000000000000, 0.51851851851851827, 0.56481481481481488, 0.00000000000000000});
    parameter_values.push_back(std::vector<T>{
            0.00000000000000000, 0.25000000000000000, 0.50000000000000000, 0.75000000000000000, 0.79166666666666663,
            0.83333333333333337, 0.87500000000000000, 0.91666666666666663, 0.95833333333333326, 1.00000000000000000});
    expected_basis_function_values.push_back(std::vector<T>{
            0.00000000000000000, 0.00000000000000000, 0.00000000000000000, 0.00000000000000000, 0.00462962962962962,
            0.03703703703703709, 0.12500000000000000, 0.29629629629629611, 0.57870370370370305, 1.00000000000000000});
    ASSERT_EQ(static_cast<int>(parameter_values.

                               size()

                                       ),
              num_basis_functions);
    ASSERT_EQ(static_cast<int>(expected_basis_function_values.
    ...
    for (int i = 0; i < num_basis_functions; ++i) {
        const int num_parameter_values = static_cast<int>(parameter_values[i].size());
        ASSERT_EQ(static_cast<int>(expected_basis_function_values[i].
        ...
        for (int j = 0; j < num_parameter_values; ++j) {
            EXPECT_NEAR(ExtractDoubleOrThrow(basis.EvaluateBasisFunctionI(i, parameter_values[i][j])),
...
GTEST_TEST(BsplineBasisSerializeTests, UnsortedKnotsTest) {
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlString<BsplineBasis<double>>(unsorted_knots), ".*CheckInvariants.*");
}
