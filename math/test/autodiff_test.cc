...
TEST_F(AutodiffTest, ExtractValue) {
    ...
    EXPECT_TRUE(CompareMatrices(expected, values, 1e-10, MatrixCompareType::absolute));
}
...
TEST_F(AutodiffTest, ExtractGradient) {
    ...
    expected(0, 0) = -sin(v0_) + (cos(v0_) * cos(v0_) - sin(v0_) * sin(v0_)) / v1_;
    ...
    EXPECT_TRUE(CompareMatrices(expected, gradients, 1e-10, MatrixCompareType::absolute));
    ...
    EXPECT_EQ(gradient_with_no_derivs.
    ...
    EXPECT_TRUE(CompareMatrices(gradient_with_three_derivs, Eigen::Matrix3d::Zero()));
    ...
    mixed_bad(1) = AutoDiffXd(2.0,

                              VectorXd()

    );
    mixed_bad(2) = AutoDiffXd(3.0, VectorXd::Zero(2));
    DRAKE_EXPECT_THROWS_MESSAGE(ExtractGradient(mixed_bad), "ExtractGradient..: Input matrix.*inconsistent.*3 and 2.*");
}
...
GTEST_TEST(AdditionalAutodiffTest, InitializeNoGradientMatrix) {
    ...
    EXPECT_TRUE(CompareMatrices(ExtractGradient(autodiff2), Eigen::Matrix4d::Identity()));
    ...
    EXPECT_TRUE(CompareMatrices(ExtractGradient(autodiff22), Eigen::Matrix4d::Identity()));
    ...
    EXPECT_EQ(decltype(ad4_return)::ColsAtCompileTime,

              2);

    EXPECT_EQ(decltype(ad4_return)::RowsAtCompileTime,

              2);

    EXPECT_EQ(decltype(ad4_return)::Scalar::DerType::RowsAtCompileTime,

              4);

    EXPECT_TRUE(CompareMatrices(ExtractValue(ad4_return), value));

    EXPECT_TRUE(CompareMatrices(ExtractGradient(ad4_return), Eigen::Matrix4d::Identity()));

    const auto adX_return = InitializeAutoDiff(value);  // Dynamic derivatives.
    // Since value was fixed size, adX_return should be also.
    EXPECT_EQ(decltype(adX_return)::ColsAtCompileTime,

              2);

    EXPECT_EQ(decltype(adX_return)::RowsAtCompileTime,
    ...
    EXPECT_TRUE(CompareMatrices(ExtractGradient(adX_return), Eigen::Matrix4d::Identity()));
...
4.0, 5.0, 6.0;
    ...
    EXPECT_EQ(decltype(adXT_return)::RowsAtCompileTime,

              2);

    EXPECT_EQ(decltype(adXT_return)::ColsAtCompileTime,

              3);
    EXPECT_TRUE(CompareMatrices(ExtractValue(adXT_return), value_rowmajor));
    EXPECT_TRUE(CompareMatrices(ExtractGradient(adXT_return), MatrixXd::Identity(6, 6)));
    ...
    EXPECT_TRUE(CompareMatrices(ExtractGradient(autodiffX), Eigen::MatrixXd::Identity(4, 4)));

    // Check that the general method obeys explicit size & start arguments
    // for the derivatives.
    InitializeAutoDiff(value, 7, 3, &autodiffX);
    EXPECT_TRUE(CompareMatrices(ExtractGradient(autodiffX).block<4, 3>(0, 0), Eigen::Matrix<double, 4, 3>::Zero()));
    EXPECT_TRUE(CompareMatrices(ExtractGradient(autodiffX).block<4, 4>(0, 3), Eigen::Matrix4d::Identity()));
    ...
    EXPECT_TRUE(CompareMatrices(ExtractGradient(ad_size).block<4, 2>(0, 0), Eigen::Matrix<double, 4, 2>::Zero()));
    EXPECT_TRUE(CompareMatrices(ExtractGradient(ad_size).block<4, 4>(0, 2), Eigen::Matrix4d::Identity()));
...
GTEST_TEST(AdditionalAutodiffTest, InitializeWithGradientMatrix) {
    const Eigen::Matrix2d value = (Eigen::Matrix2d() << 1.0, 2.0, 3.0, 4.0).finished();
    ...
    EXPECT_EQ(decltype(ad2_return)::ColsAtCompileTime,

              2);

    EXPECT_EQ(decltype(ad2_return)::RowsAtCompileTime,

              2);

    EXPECT_EQ(decltype(ad2_return)::Scalar::DerType::RowsAtCompileTime,
    ...
    const auto autodiff_return = InitializeAutoDiff(dynamic_value, dynamic_gradient);
    ...
    EXPECT_EQ(decltype(autodiff_return)::Scalar::DerType::RowsAtCompileTime, Eigen::Dynamic);

    EXPECT_EQ(autodiff_return.
...
GTEST_TEST(AdditionalAutodiffTest, CastToAutoDiff) {
    ...
    Vector2<AutoDiffUpTo16d> dynamic_max = Vector2d::Ones().cast<AutoDiffUpTo16d>();
    const auto dynamic_max_gradients = ExtractGradient(dynamic_max);
    EXPECT_EQ(dynamic_max_gradients.
    ...
    Vector2<AutoDiffScalar<Vector3d>> fixed = Vector2d::Ones().cast<AutoDiffScalar<Vector3d>>();
    const auto fixed_gradients = ExtractGradient(fixed);
    EXPECT_EQ(fixed_gradients.
...
GTEST_TEST(GetDerivativeSize, Test) {
    ...
    // Non-empty gradient.
    EXPECT_EQ(GetDerivativeSize(InitializeAutoDiff(Eigen::Vector2d(1, 2), Eigen::Matrix<double, 2, 3>::Ones())), 3);
    ...
    x(1).
            ...
            derivatives() = Eigen::Vector4d::Ones();
    ...
    x(2).
            ...
            derivatives() = Eigen::VectorXd::Ones(4);
    ...
    x(2).
            ...
            derivatives() = Eigen::VectorXd::Ones(3);

    DRAKE_EXPECT_THROWS_MESSAGE(GetDerivativeSize(x), ".* has size 3, while another entry has size 4");
}
...
GTEST_TEST(AutoDiffEqual, AreAutoDiffVecXdEqualTest) {
    ...
    a[0].
            ...
            derivatives() = Eigen::Vector2d(1, 2);

    b[0].

            derivatives() = Eigen::Vector3d(1, 2, 3);

    EXPECT_FALSE(AreAutoDiffVecXdEqual(a, b));
    b[0].

            derivatives() = Eigen::Vector2d(4, 5);

    EXPECT_FALSE(AreAutoDiffVecXdEqual(a, b));
    b[0].

            derivatives() = a[0].derivatives();
