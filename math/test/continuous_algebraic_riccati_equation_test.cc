...
void SolveCAREandVerify(const Eigen::Ref<const MatrixXd> &A,
    ...
    MatrixXd Y = (A.transpose() * X) + (X * A) - (X * B * R.inverse() * B.transpose() * X) + Q;

    EXPECT_TRUE(CompareMatrices(Y, MatrixXd::Zero(n, n), 1E-10, MatrixCompareType::absolute));
...
GTEST_TEST(CARE, TestUndetectable) {
    ...
    A.
            ...
            topRightCorner<6, 6>() = Eigen::MatrixXd::Identity(6, 6);
