...
GTEST_TEST(TestGrayCode, TestCalculateGrayCodes) {
    for (int i = 1; i < 4; i++) {
        ...
        for (int j = 1; j < test_code.

                            rows();

             j++) {
            EXPECT_EQ((test_code.row(j) - test_code.row(j - 1))
                              .
...
void TestGrayCode(const Eigen::Ref<const Eigen::MatrixXi> &gray_codes) {
    ...
    static_assert(std::is_same_v<decltype(gray_codes_static), Eigen::Matrix<int, NumCodes, NumDigits>>,
                  "Should be a static sized matrix");
    EXPECT_TRUE(CompareMatrices(gray_codes, gray_codes_dynamic, 1E-5, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(gray_codes, gray_codes_static, 1E-5, MatrixCompareType::absolute));
}
