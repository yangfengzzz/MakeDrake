...
GTEST_TEST(ComputeNumericalGradientTest, TestAffineFunction) {
    ...
    auto check_gradient = [&A, &calc_fun](const Eigen::Vector2d &x) {
        ...
        // backward difference
        J = ComputeNumericalGradient(calc_fun, x, NumericalGradientOption{NumericalGradientMethod::kBackward});
        EXPECT_TRUE(CompareMatrices(J, A, tol));
        // central difference
        J = ComputeNumericalGradient<Eigen::Vector2d, Eigen::Vector3d, Eigen::Vector2d>(
                calc_fun, x, NumericalGradientOption{NumericalGradientMethod::kCentral});
        EXPECT_TRUE(CompareMatrices(J, A, tol));
    };
...
GTEST_TEST(ComputeNumericalGradientTest, TestToyFunction) {
    auto check_gradient = [](const Eigen::Vector3d &x) {
        ...
        // backward difference
        J = ComputeNumericalGradient(ToyFunctionDouble, x, NumericalGradientOption{NumericalGradientMethod::kBackward});
        EXPECT_TRUE(CompareMatrices(J, ExtractGradient(y_autodiff), tol));
        ...
        J = ComputeNumericalGradient<Eigen::Vector3d, Eigen::Vector2d, Eigen::Vector3d>(
                ToyFunction<double>, x, NumericalGradientOption{NumericalGradientMethod::kCentral});
        EXPECT_TRUE(CompareMatrices(J, ExtractGradient(y_autodiff), tol));
    };
...
GTEST_TEST(ComputeNumericalGradientTest, TestParaboloid) {
    ...
    auto check_gradient = [](const Vector2<double> &x) {
        ...
        JAD3d = ComputeNumericalGradient(ParaboloidAD3d, x_autodiff,
        ...
        auto JExpr = ComputeNumericalGradient(ParaboloidExpr, x_symbolic,
                                              NumericalGradientOption{NumericalGradientMethod::kForward});
        const RowVector2<double> JExpr_to_double(ExtractDoubleOrThrow(JExpr(0)), ExtractDoubleOrThrow(JExpr(1)));
