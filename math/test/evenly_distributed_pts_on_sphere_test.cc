#include "math/evenly_distributed_pts_on_sphere.h"
...
void EvenlyDistributedPtsOnSphereFibonacciTest(int num_samples, double tol) {
    ...
    EXPECT_TRUE(CompareMatrices(pts.colwise().norm(), Eigen::RowVectorXd::Ones(num_samples), 1E-6,
    ...
    for (const auto &cone : cones) {
        ...
        EXPECT_NEAR(num_pts_in_cone / static_cast<double>(num_samples), (1 - cos_theta) / 2, tol);
    }
}
