...
GTEST_TEST(RotationMatrix, RotationMatrixConstructor) {
    ...
    if (kDrakeAssertIsArmed) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>{m}, "Error: Rotation matrix is not orthonormal[\\s\\S]*");
    }
    ...
    0, 0, -1;
        // clang-format on
        DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>{m}, "Error: Rotation matrix determinant is negative.*");
...
GTEST_TEST(RotationMatrix, MakeFromOrthonormalRowsOrColumns) {
    ...
    RotationMatrix<double> R = RotationMatrix<double>::MakeFromOrthonormalRows(Ax, Ay, Az);
    const Matrix3d zero_row_matrix = m_row - R.matrix();
    EXPECT_TRUE((zero_row_matrix.
    ...
    RotationMatrix<double> R2 = RotationMatrix<double>::MakeFromOrthonormalColumns(Ax, Ay, Az);
    const Matrix3d zero_column_matrix = m_column - R2.matrix();
    EXPECT_TRUE((zero_column_matrix.
    ...
    EXPECT_TRUE(R.IsExactlyEqualTo(R2.
    ...
    const double delta = 8 * RotationMatrix<double>::get_internal_tolerance_for_orthonormality();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(R = RotationMatrix<double>::MakeFromOrthonormalRows(Fx, Fy, Fz),
                                         "Error: Rotation matrix is not orthonormal[\\s\\S]*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(R = RotationMatrix<double>::MakeFromOrthonormalColumns(Fx, Fy, Fz),
                                         "Error: Rotation matrix is not orthonormal[\\s\\S]*");

    // Non-right handed matrix with determinant < 0 should throw an exception.
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(
            R = RotationMatrix<double>::MakeFromOrthonormalRows(Vector3d(-1, 0, 0), Fy, Fz),
            "Error: Rotation matrix determinant is negative.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(
            R = RotationMatrix<double>::MakeFromOrthonormalColumns(Vector3d(-1, 0, 0), Fy, Fz),
            "Error: Rotation matrix determinant is negative.*");

    // Matrix with a NaN should throw an exception.
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(R = RotationMatrix<double>::MakeFromOrthonormalRows(
                                                 Vector3d(std::numeric_limits<double>::quiet_NaN(), 0, 0), Fy, Fz),
                                         "Error: Rotation matrix contains an element that is infinity or NaN.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(R = RotationMatrix<double>::MakeFromOrthonormalColumns(
                                                 Vector3d(std::numeric_limits<double>::quiet_NaN(), 0, 0), Fy, Fz),
                                         "Error: Rotation matrix contains an element that is infinity or NaN.*");
    ...
    if (kDrakeAssertIsDisarmed) {
        ...
        DRAKE_EXPECT_NO_THROW(R = RotationMatrix<double>::MakeFromOrthonormalRows(Fx, Fy, Fz));
        EXPECT_FALSE(R.

                     IsValid()

        );
        DRAKE_EXPECT_NO_THROW(R = RotationMatrix<double>::MakeFromOrthonormalColumns(Fx, Fy, Fz));
        EXPECT_FALSE(R.
...
GTEST_TEST(RotationMatrix, MakeXRotationMakeYRotationMakeZRotation) {
    ...
    EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeXRotation(M_PI_4) * i, i, tolerance));
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeXRotation(M_PI_4) * j, Vector3d(0, M_SQRT1_2, M_SQRT1_2), tolerance));
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeXRotation(M_PI_4) * k, Vector3d(0, -M_SQRT1_2, M_SQRT1_2), tolerance));
    ...
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeYRotation(M_PI_4) * i, Vector3d(M_SQRT1_2, 0, -M_SQRT1_2), tolerance));
    EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeYRotation(M_PI_4) * j, j, tolerance));
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeYRotation(M_PI_4) * k, Vector3d(M_SQRT1_2, 0, M_SQRT1_2), tolerance));
    ...
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeZRotation(M_PI_4) * i, Vector3d(M_SQRT1_2, M_SQRT1_2, 0), tolerance));
    EXPECT_TRUE(
            CompareMatrices(RotationMatrixd::MakeZRotation(M_PI_4) * j, Vector3d(-M_SQRT1_2, M_SQRT1_2, 0), tolerance));
    EXPECT_TRUE(CompareMatrices(RotationMatrixd::MakeZRotation(M_PI_4) * k, k, tolerance));
    ...
    RB = RotationMatrixd(Eigen::DiagonalMatrix<double, 3>(1, -1, -1)) * RotationMatrixd::MakeXRotation(theta);
    EXPECT_TRUE(RA.IsNearlyEqualTo(RB, tolerance));

    RA = RotationMatrixd::MakeYRotation(M_PI + theta);
    RB = RotationMatrixd(Eigen::DiagonalMatrix<double, 3>(-1, 1, -1)) * RotationMatrixd::MakeYRotation(theta);
    EXPECT_TRUE(RA.IsNearlyEqualTo(RB, tolerance));

    RA = RotationMatrixd::MakeZRotation(M_PI + theta);
    RB = RotationMatrixd(Eigen::DiagonalMatrix<double, 3>(-1, -1, 1)) * RotationMatrixd::MakeZRotation(theta);
    EXPECT_TRUE(RA.IsNearlyEqualTo(RB, tolerance));
}
...
GTEST_TEST(RotationMatrix, ConstructorWithRollPitchYaw) {
    ...
    const Matrix3d m = (Eigen::AngleAxisd(y, Vector3d::UnitZ()) * Eigen::AngleAxisd(p, Vector3d::UnitY()) *
...
GTEST_TEST(RotationMatrix, OperatorMultiplyNonScalarType) {
    ...
    will_be_R_CA *= R_BA;
    EXPECT_TRUE(CompareMatrices(symbolic::Evaluate(will_be_R_CA.matrix()), R_CAd.matrix(), 10 * kEpsilon));
}
...
GTEST_TEST(RotationMatrix, InvertAndCompose) {
    ...
    EXPECT_TRUE(CompareMatrices(symbolic::Evaluate(R_AC.matrix()), R_ACd.matrix(), 10 * kEpsilon));
}
...
GTEST_TEST(RotationMatrix, ProjectToRotationMatrix) {
    ...
    RotationMatrix<double> R = RotationMatrix<double>::ProjectToRotationMatrix(m, &quality_factor);
    EXPECT_TRUE(R.IsNearlyEqualTo(RotationMatrix<double>(Matrix3d::Identity()), 10 * kEpsilon));
...
kEpsilon;
    ...
    EXPECT_TRUE(R.IsNearlyEqualTo(RotationMatrix<double>(Matrix3d::Identity()), 64 * kEpsilon));
...
kEpsilon;
    // clang-format on
    EXPECT_TRUE(-64 * kEpsilon * kEpsilon * kEpsilon < m.
                ...
                m.
                        ...
                        < 0);
    EXPECT_THROW(RotationMatrix<double>::ProjectToRotationMatrix(m, &quality_factor), std::exception);
...
7, 8, 7;
    ...
    EXPECT_TRUE(I.IsNearlyEqualTo(RotationMatrix<double>(Matrix3d::Identity()), 8 * kEpsilon));
    EXPECT_TRUE(R.
...
GTEST_TEST(RotationMatrix, SymbolicProjectionTest) {
    ...
    Expression quality;
    DRAKE_EXPECT_THROWS_MESSAGE(RotMatExpr::ProjectToRotationMatrix(m_symbolic, &quality),
...
GTEST_TEST(RotationMatrixTest, OperatorMultiplyByMatrix3X) {
    ...
    EXPECT_EQ(decltype(v_A)::RowsAtCompileTime,

              3);

    EXPECT_EQ(decltype(v_A)::ColsAtCompileTime,
    ...
    EXPECT_EQ(decltype(w_A)::RowsAtCompileTime,
    ...
    const Eigen::MatrixXd s_A = R_AB * (Eigen::Vector3d(1, 2, 3) * Eigen::RowVector4d(1, 2, 3, 4));
    EXPECT_EQ(s_A.
...
3, 6, 9, 12;
    ...
    const auto z_A_expected = R_AB * (Eigen::MatrixXd(3, 4) << Eigen::Vector3d(1, 2, 3), Eigen::Vector3d(2, 4, 6),
                                      Eigen::Vector3d(3, 6, 9), Eigen::Vector3d(4, 8, 12))
                                             .finished();

    EXPECT_EQ(decltype(z_A_expected)::RowsAtCompileTime,

              3);
    EXPECT_EQ(z_A_expected.
    ...
    if (kDrakeAssertIsArmed) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(bad_matrix_multiply = R_AB * m_7x8,
...
TEST_F(RotationMatrixConversionTests, RotationMatrixToQuaternionVariable) {
    ...
    const Matrix3<Variable> m_var = symbolic::MakeMatrixContinuousVariable<3, 3>("m");
...
TEST_F(RotationMatrixConversionTests, AngleAxisToRotationMatrix) {
    for (const Eigen::Quaterniond& qi : quaternion_test_cases_) {
        ...
        inverse_angle_axis.fromRotationMatrix(R_expected.
...
GTEST_TEST(RotationMatrixTest, MakeFromOneUnitVector) {
    ...
    for (int axis_index = 0; axis_index < 2; axis_index++) {
        for (const Vector3<double>& b_A : test_vectors) {
            ...
            const RotationMatrix<double> R_AB = RotationMatrix<double>::MakeFromOneUnitVector(u_A, axis_index);
            ...
            const RotationMatrix<double> R_AB_basic = RotationMatrix<double>::MakeFromOneVector(b_A, axis_index);
            ...
            // values).
            const RotationMatrix<Expression> R_AB_sym = RotationMatrix<Expression>::MakeFromOneVector(b_A, axis_index);
            EXPECT_TRUE(CompareMatrices(R_AB.matrix(), ExtractDoubleOrThrow(R_AB_sym.matrix())));
        }
    }
}
...
GTEST_TEST(RotationMatrixTest, MakeFromOneUnitVectorExceptions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>::MakeFromOneUnitVector(test_vector, axis_index),
    ...
    test_vector = Vector3<double>(1, 0, tol_sqrt);
    RotationMatrix<double> R;
    DRAKE_EXPECT_NO_THROW(R = RotationMatrix<double>::MakeFromOneUnitVector(test_vector, axis_index));
    EXPECT_TRUE(R.
...
GTEST_TEST(RotationMatrixTest, MakeFromOneVectorExceptions) {
    ...
    // Verify vector with NaN throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>::MakeFromOneVector(Vector3<double>(NAN, 1, NAN), axis_index),
    ...
    // Verify vector with infinity throws an exception.
    constexpr double kInfinity = std::numeric_limits<double>::infinity();
    DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>::MakeFromOneVector(Vector3<double>(kInfinity, 1, 0), axis_index),
                                "MakeFromOneVector.* cannot normalize the given vector v."
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>::MakeFromOneVector(too_small_vector, axis_index),
                                "MakeFromOneVector.* cannot normalize the given vector v."
    ...
    VerifyMakeFromOneUnitVector(R_AB,
                                ok_small_vector.
...
class MakeClosestRotationToIdentityFromUnitZTest : public ::testing::TestWithParam<Vector3d> {};

TEST_P(MakeClosestRotationToIdentityFromUnitZTest, TestDouble) {
    const Vector3d u_A = GetParam();
    const RotationMatrixd R_AB = RotationMatrixd::MakeClosestRotationToIdentityFromUnitZ(u_A);
...
GTEST_TEST(MakeClosestRotationToIdentityFromUnitZTest, SpecialCase) {
    {
        const RotationMatrixd R_AB = RotationMatrixd::MakeClosestRotationToIdentityFromUnitZ(Vector3d(0, 0, 1));
        EXPECT_TRUE(R_AB.
    ...
    }
    {
        ...
        const RotationMatrixd R_expected =
                RotationMatrixd::MakeFromOrthonormalColumns(Vector3d(1, 0, 0), Vector3d(0, -1, 0), Vector3d(0, 0, -1));
        EXPECT_TRUE(CompareMatrices(R_AB.matrix(), R_expected.matrix(), 4.0 * kEpsilon));
    }
}
...
GTEST_TEST(MakeClosestRotationToIdentityFromUnitZTest, Symbolic) {
    ...
    const RotationMatrix<symbolic::Expression> R_AB_sym =
            RotationMatrix<symbolic::Expression>::MakeClosestRotationToIdentityFromUnitZ(u_A);
    EXPECT_TRUE(CompareMatrices(R_AB.matrix(), ExtractDoubleOrThrow(R_AB_sym.matrix())));
}
...
GTEST_TEST(MakeClosestRotationToIdentityFromUnitZTest, ThrowCondition) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RotationMatrix<double>::MakeClosestRotationToIdentityFromUnitZ(nonunit_vector),
