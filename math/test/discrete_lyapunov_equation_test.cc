...
void SolveRealLyapunovEquationAndVerify(const Eigen::Ref<const MatrixXd> &A,
    ...
    EXPECT_TRUE(CompareMatrices(X, X.transpose(), symmetry_tol, MatrixCompareType::absolute));
    // Check that X is the solution to the discrete time ARE.
    EXPECT_TRUE(CompareMatrices(A.transpose() * X * A - X, -Q, 5 * kTolerance * Q.norm(), MatrixCompareType::absolute));
}
...
GTEST_TEST(RealDiscreteLyapunovEquation, Solve1by1Test) {
    ...
    EXPECT_TRUE(CompareMatrices(internal::Solve1By1RealDiscreteLyapunovEquation(A, Q), X, kTolerance,
...
GTEST_TEST(RealDiscreteLyapunovEquation, Solve2by2Test) {
    ...
    EXPECT_TRUE(CompareMatrices(internal::Solve2By2RealDiscreteLyapunovEquation(A, Q_internal), X, kTolerance,
                                MatrixCompareType::absolute));

    MatrixXd Q(n, n);
    Q << 3, 1, 1, 1;
    EXPECT_TRUE(CompareMatrices(RealDiscreteLyapunovEquation(A, Q), X, kTolerance, MatrixCompareType::absolute));
