...
typename std::enable_if<internal::is_autodiff_v<typename DerivedA::Scalar> ||
    ...
    if constexpr (std::is_same_v<typename DerivedA::Scalar, double> &&
        ...
        x_eigen_d = LinearSolverType<Eigen::Matrix<double, DerivedA::RowsAtCompileTime, DerivedA::ColsAtCompileTime>>(A)
                            .solve(b);
    }
    // TODO(jwnimmer-tri) Remove this extra unnecessary level of indentation.
    {
        ...
        if constexpr (std::is_same_v<typename decltype(Ax)::Scalar, double>) {
            ...
            for (int i = 0; i < Ax.cols(); ++i) {
                Ax_grad.push_back(Eigen::Matrix<double, DerivedA::RowsAtCompileTime, 0>::Zero(Ax.rows(), 0));
            }
        } else {
        ...
        if constexpr (std::is_same_v<typename DerivedB::Scalar, double>) {
            ...
            for (int i = 0; i < b.cols(); ++i) {
                b_grad.push_back(Eigen::Matrix<double, DerivedB::RowsAtCompileTime, 0>::Zero(b.rows(), 0));
            }
        } else {
        ...
        for (int i = 0; i < static_cast<int>(b_grad.size()); ++i) {
            if (b_grad[i].size() == 0 && Ax_grad[i].size() == 0) {
                ...
                EXPECT_TRUE(CompareMatrices(b_grad[i], Eigen::MatrixXd::Zero(b_grad[i].rows(), b_grad[i].cols()), tol));
            } else if (b_grad[i].size() == 0 && Ax_grad[i].size() != 0) {
                EXPECT_TRUE(
                        CompareMatrices(Ax_grad[i], Eigen::MatrixXd::Zero(Ax_grad[i].rows(), Ax_grad[i].cols()), tol));
            } else {
        ...
        static_assert(std::is_same_v<typename decltype(x_result)::Scalar, typename decltype(x)::Scalar>);
        if constexpr (std::is_same_v<typename decltype(x_result)::Scalar, double>) {
            ...
            EXPECT_TRUE(CompareMatrices(ExtractGradient(x_result), ExtractGradient(x)));
        }
        ...
        if constexpr (internal::is_autodiff_v<typename DerivedA::Scalar>) {
            const LinearSolverType<
                    Eigen::Matrix<typename DerivedA::Scalar, DerivedA::RowsAtCompileTime, DerivedA::ColsAtCompileTime>>
                    eigen_linear_solver(A);
            Eigen::Matrix<typename DerivedA::Scalar, DerivedA::RowsAtCompileTime, DerivedB::ColsAtCompileTime> x_eigen;
            if constexpr (internal::is_autodiff_v<typename DerivedB::Scalar>) {
                x_eigen = eigen_linear_solver.solve(b);
            } else {
                x_eigen = eigen_linear_solver.solve(b.template cast<typename DerivedA::Scalar>());
            }
            EXPECT_TRUE(CompareMatrices(ExtractValue(x_eigen), ExtractValue(x), 1.0e-14));
            for (int i = 0; i < b.cols(); ++i) {
                EXPECT_TRUE(CompareMatrices(ExtractGradient(x_eigen.col(i)), ExtractGradient(x.col(i)), tol));
            }
            // NOLINTNEXTLINE(readability/braces)
        } else if constexpr (std::is_same_v<typename DerivedA::Scalar, double> &&
                             internal::is_autodiff_v<typename DerivedB::Scalar>) {
            const LinearSolverType<
                    Eigen::Matrix<typename DerivedB::Scalar, DerivedA::RowsAtCompileTime, DerivedA::ColsAtCompileTime>>
...
public:
    LinearSolveTest() {
        ...
        A_sym_ << symbolic::Expression(1), symbolic::Expression(3), symbolic::Expression(3), symbolic::Expression(10);
        ...
        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < 3; ++j) {
                ...
                b_ad_fixed_der_size_(i, j).derivatives() = b_mat_ad_(i, j).derivatives();
            }
        }
    }
...
protected:
    ...
    Eigen::Matrix<Eigen::AutoDiffScalar<Eigen::Vector3d>, 2, 2> A_ad_fixed_der_size_;
    Eigen::Matrix<Eigen::AutoDiffScalar<Eigen::Vector3d>, 2, 3> b_ad_fixed_der_size_;
};
...
void TestSolveLinearSystemSymbolic(const Eigen::MatrixBase<DerivedA> &A, const Eigen::MatrixBase<DerivedB> &b) {
    ...
    {
        const auto x = SolveLinearSystem<LinearSolverType>(A, b);
        static_assert(std::is_same_v<typename decltype(x)::Scalar, symbolic::Expression>,
                      "The scalar type should be symbolic expression");
        const Eigen::Matrix<symbolic::Expression, DerivedA::RowsAtCompileTime, DerivedB::ColsAtCompileTime> Ax = A * x;
...
TEST_F(LinearSolveTest, TestNoGrad) {
    ...
    TestSolveLinearSystem<Eigen::LLT>(A_val_.
...
TEST_F(LinearSolveTest, TestBwithGrad) {
    ...
    TestSolveLinearSystem<Eigen::LDLT>(A_val_.
    ...
    TestSolveLinearSystem<Eigen::PartialPivLU>(A_val_.
...
TEST_F(LinearSolveTest, TestFixedDerivativeSize) {
    ...
    TestSolveLinearSystem<Eigen::LDLT>(A_ad_fixed_der_size_, b_ad_fixed_der_size_);
    TestSolveLinearSystem<Eigen::ColPivHouseholderQR>(A_ad_fixed_der_size_, b_ad_fixed_der_size_);
    TestSolveLinearSystem<Eigen::PartialPivLU>(A_ad_fixed_der_size_, b_ad_fixed_der_size_);
    ...
    TestSolveLinearSystem<Eigen::ColPivHouseholderQR>(A_val_, b_ad_fixed_der_size_);
    ...
    TestSolveLinearSystem<Eigen::ColPivHouseholderQR>(A_ad_fixed_der_size_, b_mat_val_);
...
TEST_F(LinearSolveTest, TestWrongGradientSize) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SolveLinearSystem<Eigen::LLT>(A_ad_error, b_vec_ad_),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SolveLinearSystem<Eigen::LLT>(A_ad_, b_vec_ad_error),
                                ".* has size 2, while another entry has size 3");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SolveLinearSystem<Eigen::LLT>(A_ad_, b_vec_ad_error2),
...
void CheckGetLinearSolver(const Eigen::MatrixBase<DerivedA> &A) {
    ...
    if constexpr (std::is_same_v<typename DerivedA::Scalar, double> ||
        ...
        static_assert(std::is_same_v<typename decltype(linear_solver)::MatrixType::Scalar, typename DerivedA::Scalar>,
                      "The scalar type don't match");
    } else {
        static_assert(std::is_same_v<typename decltype(linear_solver)::MatrixType::Scalar, double>,
                      "The scalar type should be double");
    }

    // Cast away the enum types of the matrix metrics so we can compare without
    // compiler warnings.
    constexpr int RowsSolver = static_cast<int>(decltype(linear_solver)::MatrixType::RowsAtCompileTime);
    constexpr int RowsA = static_cast<int>(DerivedA::RowsAtCompileTime);
    static_assert(RowsSolver == RowsA, "The matrix rows don't match");
    constexpr int ColsSolver = static_cast<int>(decltype(linear_solver)::MatrixType::ColsAtCompileTime);
