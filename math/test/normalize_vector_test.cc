...
void NormalizeVectorTestFun(const Eigen::Matrix<double, nx, 1>& x) {
    ...
    EXPECT_TRUE(CompareMatrices(x_normalized, ExtractValue(x_normalized_autodiff), 1E-10, MatrixCompareType::absolute));
    EXPECT_TRUE(
            CompareMatrices(dx_normalized, ExtractGradient(x_normalized_autodiff), 1E-10, MatrixCompareType::absolute));
}
