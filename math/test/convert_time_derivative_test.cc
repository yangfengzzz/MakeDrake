#include "math/convert_time_derivative.h"
...
GTEST_TEST(ConvertTimeDerivativeToOtherFrame, OnAngularVelocity) {
    ...
    ConvertTimeDerivativeOfAngularVelocity(Vector3d(0.0, 0.0, 0.0), Vector3d(0.0, 1.0, 0.0));
    ConvertTimeDerivativeOfAngularVelocity(Vector3d(-1.0, 0.0, 2.0), Vector3d(3.0, 1.0, 4.0));
}
...
void HorseOnCarousel(double horse_radius, double theta, double theta_dot, double swing_up_speed) {
    using std::cos;
    using std::sin;
    const double kAbsoluteTolerance = 2 * std::numeric_limits<double>::epsilon();
    ...
    const Vector3d v_WHo = ConvertTimeDerivativeToOtherFrame(p_CoHo_W, v_CHo /* DtC_p_CoHo */, w_WC);

    // Compute the expected value. Note that since rotation is only along the
    // z-axis we can directly add the contribution v_CHo_z.
    const Vector3d v_WHo_expected = horse_radius * theta_dot * Vector3d(-sin(theta), cos(theta), 0) + v_CHo;

    EXPECT_TRUE(CompareMatrices(v_WHo, v_WHo_expected, kAbsoluteTolerance, MatrixCompareType::absolute));
}
