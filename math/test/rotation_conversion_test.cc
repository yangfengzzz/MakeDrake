...
#include "math/normalize_vector.h"
...
GTEST_TEST(EigenEulerAngleTest, BodyXYZ) {
    ...
    const Matrix3d bodyXYZ_rotmat = (RotationMatrix<double>::MakeXRotation(input_angles(0)) *
...
GTEST_TEST(EigenEulerAngleTest, SpaceXYZ) {
    ...
    const Vector3d input_angles(0.5, 0.4, 0.3);
    const Matrix3d spaceXYZ_rotmat = (RotationMatrix<double>::MakeZRotation(input_angles(0)) *
                                      RotationMatrix<double>::MakeYRotation(input_angles(1)) *
...
GTEST_TEST(EigenEulerAngleTest, BodyZYZ) {
    ...
    const Vector3d input_angles(0.5, 0.4, 0.3);
    const Matrix3d bodyZYZ_angles = (RotationMatrix<double>::MakeZRotation(input_angles(0)) *
                                     RotationMatrix<double>::MakeYRotation(input_angles(1)) *
                                     RotationMatrix<double>::MakeZRotation(input_angles(2)))
                                            .matrix();
...
protected:
    ...
    void SetupRPYTestCases() {
        ...
        // pitch = 0.5*pi-eps
        const RollPitchYaw<double> pitch_near_half_piA(M_PI / 4, 0.5 * M_PI - kEpsilon, M_PI / 3);
        rpy_test_cases_.push_back(pitch_near_half_piA);

        // pitch = 0.5*pi-1.5*eps
        const RollPitchYaw<double> pitch_near_half_piB(M_PI / 4, 0.5 * M_PI - 1.5 * kEpsilon, M_PI / 3);
        rpy_test_cases_.push_back(pitch_near_half_piB);

        // pitch = 0.5*pi-2*eps
        const RollPitchYaw<double> pitch_near_half_piC(M_PI / 4, 0.5 * M_PI - 2 * kEpsilon, M_PI / 3);
        rpy_test_cases_.push_back(pitch_near_half_piC);

        // pitch = 0.5*pi - 1E-15
        const RollPitchYaw<double> pitch_near_half_piD(M_PI * 0.8, 0.5 * M_PI - 1E-15, 0.9 * M_PI);
        rpy_test_cases_.push_back(pitch_near_half_piD);

        // pitch = -0.5*pi+eps
        const RollPitchYaw<double> pitch_near_neg_half_piA(M_PI * -0.9, -0.5 * M_PI + kEpsilon, M_PI * 0.3);
        rpy_test_cases_.push_back(pitch_near_neg_half_piA);

        // pitch = -0.5*pi+1.5*eps
        const RollPitchYaw<double> pitch_near_neg_half_piB(M_PI * -0.6, -0.5 * M_PI + 1.5 * kEpsilon, M_PI * 0.3);
        rpy_test_cases_.push_back(pitch_near_neg_half_piB);

        // pitch = -0.5*pi+2*eps
        const RollPitchYaw<double> pitch_near_neg_half_piC(M_PI * -0.5, -0.5 * M_PI + 2 * kEpsilon, M_PI * 0.4);
        rpy_test_cases_.push_back(pitch_near_neg_half_piC);

        // pitch = -0.5*pi + 1E-15
        const RollPitchYaw<double> pitch_near_neg_half_piD(M_PI * 0.9, -0.5 * M_PI + 1E-15, 0.8 * M_PI);
        rpy_test_cases_.push_back(pitch_near_neg_half_piD);

        // non-singular cases
        auto roll = Eigen::VectorXd::LinSpaced(kSweepSize, -0.99 * M_PI, M_PI);
        auto pitch = Eigen::VectorXd::LinSpaced(kSweepSize, -0.49 * M_PI, 0.49 * M_PI);
    ...
    void SetupAngleAxisTestCases() {
        ...
        auto a_angle = Eigen::VectorXd::LinSpaced(kSweepSize, -0.95 * M_PI, 0.95 * M_PI);
...
TEST_F(RotationConversionTest, quaternionToRotationMatrixTest) {
    for (const Quaterniond& qi : quaternion_test_cases_) {
        ...
        const Eigen::Quaterniond quat_expected = RotationMatrix<double>::ToQuaternion(rotmat);
        EXPECT_TRUE(AreQuaternionsEqualForOrientation(qi, quat_expected, kTolerance));
    }
}
...
TEST_F(RotationConversionTest, rpy2rotmatTest) {
    for (const RollPitchYaw<double>& rpyi : rpy_test_cases_) {
        ...
        const Quaterniond q = Eigen::AngleAxisd(yaw, Vector3d::UnitZ()) * Eigen::AngleAxisd(pitch, Vector3d::UnitY()) *
