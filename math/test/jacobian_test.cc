...
#pragma GCC diagnostic pop
    ...
    static_assert(std::is_same_v<decltype(jac_chunk_size_default), decltype(jac_chunk_size_1)>,
                  "jacobian output type mismatch");
    static_assert(std::is_same_v<decltype(jac_chunk_size_default), decltype(jac_chunk_size_3)>,
                  "jacobian output type mismatch");
    static_assert(std::is_same_v<decltype(jac_chunk_size_default), decltype(jac_chunk_size_6)>,
                  "jacobian output type mismatch");
    ...
    EXPECT_TRUE(CompareMatrices(value_expected, value, 1e-12, MatrixCompareType::absolute));
    ...
    EXPECT_TRUE(CompareMatrices(jac_expected, jac, 1e-12, MatrixCompareType::absolute));
}
...
#pragma GCC diagnostic pop
    ...
    // Ensure that chunk size has no effect on output type.
    static_assert(std::is_same_v<decltype(hess_chunk_size_default), decltype(hess_chunk_size_2_4)>,
    ...
    auto jac_expected = ((A * x + b).transpose() * C * D + (D * x + e).transpose() * C.transpose() * A).eval();
    EXPECT_TRUE(CompareMatrices(jac_expected, jac1, 1e-12, MatrixCompareType::absolute));

    // Ensure that the Hessian is correct.
    auto hess_expected = (A.transpose() * C * D + D.transpose() * C.transpose() * A).eval();
    auto hess = ExtractGradient(jac_autodiff);
    EXPECT_TRUE(CompareMatrices(hess_expected, hess, 1e-12, MatrixCompareType::absolute));
}
