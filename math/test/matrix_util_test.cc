...
GTEST_TEST(TestMatrixUtil, TestToSymmetricMatrixFromLowerTriangularColumns) {
    ...
    // clang-format off
        ...
        3, 5, 6;
    // clang-format on
    EXPECT_TRUE(CompareMatrices(ToSymmetricMatrixFromLowerTriangularColumns(x2), X2));
}
...
GTEST_TEST(TestMatrixUtil, IsPositiveDefiniteTest) {
    ...
    EXPECT_FALSE(IsPositiveDefinite(A * Eigen::Vector3d(1., 0., 1.).asDiagonal() * A.transpose(), 1e-8));
}
...
GTEST_TEST(TestMatrixUtil, StdVector) {
    ...
    EXPECT_EQ(EigenToStdVector<double>(Eigen::RowVector3d::LinSpaced(0.0, 1.0))
                      .
...
GTEST_TEST(TestMatrixUtil, ExtractPrincipalSubmatrixSquareMatrix) {
    ...
    drake::MatrixX<symbolic::Variable> submatrix_manual(submatrix_indices.size(), submatrix_indices.size());
    // clang-format off
...
X(2, 1), X(2, 2);
    ...
    auto TestSubmatrix = [&submatrix_indices,
        ...
        for (int r = 0; r < submatrix_tested.rows(); ++r) {
            for (int c = 0; c < submatrix_tested.cols(); ++c) {
                EXPECT_TRUE(submatrix_tested(r, c).equal_to(submatrix_manual(r, c)));
            }
        }
    };
    ...
    submatrix_manual.resize(submatrix_indices.
