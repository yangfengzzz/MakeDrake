...
BarycentricMesh<T>::BarycentricMesh(MeshGrid input_grid)
    : input_grid_(std::move(input_grid)), stride_(input_grid_.size()), num_interpolants_{1} {
...
template <typename T>
void BarycentricMesh<T>::get_mesh_point(int index, EigenPtr<Eigen::VectorXd> point) const {
...
void BarycentricMesh<T>::EvalBarycentricWeights(const Eigen::Ref<const VectorX<T>>& input,
    ...
    for (int i = 1; i < num_interpolants_; i++) {
        ...
        if (i == (num_interpolants_ - 1)) {
            ...
            (*weights)[i] = relative_position[i].first - relative_position[i - 1].first;
        }
    }
}
