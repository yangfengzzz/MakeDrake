...
public:
     ...
     * about 1E-15.
     */
    explicit NumericalGradientOption(NumericalGradientMethod method, double function_accuracy = 1e-15)
        : method_{method} {
        perturbation_size_ = method == NumericalGradientMethod::kCentral ? std::cbrt(function_accuracy)
...
Eigen::Matrix<typename DerivedX::Scalar, DerivedY::RowsAtCompileTime, DerivedX::RowsAtCompileTime>
ComputeNumericalGradient(std::function<void(const DerivedCalcX&, DerivedY* y)> calc_func,
                         const DerivedX& x,
    ...
    static_assert(is_eigen_vector_of<DerivedY, T>::value,
                  "DerivedY must be templated on the same scalar type as DerivedX.");
    static_assert(is_eigen_vector_of<DerivedCalcX, T>::value,
    ...
    for (int i = 0; i < x.rows(); ++i) {
        // Perturbation size.
        const T h = max(option.perturbation_size(), abs(x(i)) * option.perturbation_size());
