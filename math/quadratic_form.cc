            ...
            for (int i = 0; i < es_Y.eigenvalues().rows(); ++i) {
                if (es_Y.eigenvalues()(i) < -zero_tol) {
                    ...
                    throw std::runtime_error(
                            fmt::format("Y is not positive semidefinite. It has an eigenvalue {} "
...
Eigen::MatrixXd BalanceQuadraticForms(const Eigen::Ref<const Eigen::MatrixXd>& S,
    ...
    const Eigen::MatrixXd R = S.llt().matrixL().solve(Eigen::MatrixXd::Identity(n, n));

    const Eigen::JacobiSVD<Eigen::MatrixXd> svd(R * P * R.transpose(), Eigen::ComputeThinU);
    ...
    const Eigen::VectorXd sigmaRootN4 = svd.singularValues().array().pow(-0.25).matrix();
