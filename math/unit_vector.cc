    ...
    if constexpr (scalar_predicate<T>::is_bool) {
        ...
        if (!(isfinite(norm) && norm >= kMinMagnitude)) {
            throw std::logic_error(
                    fmt::format("{}() cannot normalize the given vector v.\n"
                                "   v = {}\n"
                                " |v| = {}\n"
                                ...
                                function_name, fmt_eigen(DiscardGradient(v).transpose()), ExtractDoubleOrThrow(norm),
                                kMinMagnitude));
        }
    }
    return v / norm;
...
void ThrowIfNotUnitVector(const Vector3<T>& unit_vector,
    ...
    if constexpr (scalar_predicate<T>::is_bool) {
        ...
        const bool is_ok_unit_vector = std::isfinite(uvec_squared) && std::abs(uvec_squared - 1.0) <= tolerance2;
        if (!is_ok_unit_vector) {
            ...
            throw std::logic_error(fmt::format(
                    ...
                    function_name, fmt_eigen(unit_vector.transpose()), norm, norm_diff, tolerance_unit_vector_norm));
        }
    } else {
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&NormalizeOrThrow<T>, &ThrowIfNotUnitVector<T>));
