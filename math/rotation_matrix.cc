...
void ThrowIfAllElementsInQuaternionAreZero(const Eigen::Quaternion<T>& quaternion, const char* function_name) {
    if constexpr (scalar_predicate<T>::is_bool) {
        if (IsQuaternionZero(quaternion)) {
            std::string message = fmt::format("{}(): All the elements in a quaternion are zero.", function_name);
...
void ThrowIfAnyElementInQuaternionIsInfinityOrNaN(const Eigen::Quaternion<T>& quaternion, const char* function_name) {
    if constexpr (scalar_predicate<T>::is_bool) {
        if (!quaternion.coeffs().allFinite()) {
            std::string message =
                    fmt::format("{}(): Quaternion contains an element that is infinity or NaN.", function_name);
            throw std::logic_error(message);
        }
    } else {
        unused(quaternion, function_name);
...
template <typename T>
RotationMatrix<T> RotationMatrix<T>::MakeFromOneUnitVector(const Vector3<T>& u_A, int axis_index) {
...
template <typename T>
Matrix3<T> RotationMatrix<T>::QuaternionToRotationMatrix(const Eigen::Quaternion<T>& quaternion,
                                                         const T& two_over_norm_squared) {
    ThrowIfAllElementsInQuaternionAreZero(quaternion, __func__);
    DRAKE_ASSERT_VOID(ThrowIfAnyElementInQuaternionIsInfinityOrNaN(quaternion, __func__));
...
template <typename T>
Matrix3<T> RotationMatrix<T>::ProjectMatrix3ToOrthonormalMatrix3(const Matrix3<T>& M, T* quality_factor) {
...
template <typename T>
Eigen::Quaternion<T> RotationMatrix<T>::ToQuaternion(const Eigen::Ref<const Matrix3<T>>& M) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::math::RotationMatrix);
