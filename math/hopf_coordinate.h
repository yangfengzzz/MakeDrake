...
const Eigen::Quaternion<T> HopfCoordinateToQuaternion(const T& theta, const T& phi, const T& psi) {
    ...
    return Eigen::Quaternion(cos_half_theta * cos(psi / 2), cos_half_theta * sin(psi / 2),
                             sin_half_theta * cos(phi_plus_half_psi), sin_half_theta * sin(phi_plus_half_psi));
}
...
Vector3<T> QuaternionToHopfCoordinate(const Eigen::Quaternion<T>& quaternion) {
    if (quaternion.w() < 0) {
        return QuaternionToHopfCoordinate(
                Eigen::Quaternion<T>(-quaternion.w(), -quaternion.x(), -quaternion.y(), -quaternion.z()));
    }
    using std::atan2;
    ...
    using std::pow;
    using std::sqrt;
    const T theta = 2 * atan2(sqrt(pow(quaternion.y(), 2) + pow(quaternion.z(), 2)),
