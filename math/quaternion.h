...
boolean<T> AreQuaternionsEqualForOrientation(const Eigen::Quaternion<T>& quat1,
    ...
    return (quat1_canonical.coeffs() - quat2_canonical.coeffs()).template lpNorm<Eigen::Infinity>() <= tolerance;
}
...
bool IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(const Eigen::Quaternion<T>& quat,
    ...
    const Eigen::Vector3d w_from_quatDt = math::CalculateAngularVelocityExpressedInBFromQuaternionDt(quat, quatDt);
...
Eigen::AngleAxis<T> QuaternionToAngleAxisLikeEigen(const Eigen::Quaternion<T>& quaternion) {
    ...
    result.axis() =
            if_then_else(is_sin_angle_zero, unit_axis, (axis_sign * quaternion.vec() / sin_half_angle_abs).eval());
    return result;
}
}  // namespace internal
