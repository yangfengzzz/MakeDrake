...
public:
    ...
    static RotationMatrix<T> MakeClosestRotationToIdentityFromUnitZ(const Vector3<T>& u_A) {
        ...
        const Vector3<T> normalized_axis = axis_norm < 1e-10 ? Vector3<T>(1, 0, 0) : axis / axis_norm;
        using std::atan2;
    ...
    Vector3<T> operator*(const Vector3<T>& v_B) const { return Vector3<T>(matrix() * v_B); }
    ...
    /// @param[in] R an allegedly valid rotation matrix.
    /// @returns `true` if R is a valid rotation matrix.
    static boolean<T> IsValid(const Matrix3<T>& R) { return IsValid(R, get_internal_tolerance_for_orthonormality()); }
    ...
    boolean<T> IsExactlyIdentity() const { return matrix() == Matrix3<T>::Identity(); }
    ...
    /// @see IsNearlyEqualTo().
    boolean<T> IsExactlyEqualTo(const RotationMatrix<T>& other) const { return matrix() == other.matrix(); }
    ...
    static RotationMatrix<T> ProjectToRotationMatrix(const Matrix3<T>& M, T* quality_factor = nullptr) {
        const Matrix3<T> M_orthonormalized = ProjectMatrix3ToOrthonormalMatrix3(M, quality_factor);
    ...
    /// @see ToQuaternion().
    Vector4<T> ToQuaternionAsVector4() const { return ToQuaternionAsVector4(R_AB_); }
    ...
    friend void hash_append(HashAlgorithm& hasher, const RotationMatrix& R) noexcept {
...
private:
    ...
    void SetFromOrthonormalColumns(const Vector3<T>& Bx, const Vector3<T>& By, const Vector3<T>& Bz) {
        R_AB_.col(0) = Bx;
        R_AB_.col(1) = By;
        R_AB_.col(2) = Bz;
    ...
    void SetFromOrthonormalRows(const Vector3<T>& Ax, const Vector3<T>& Ay, const Vector3<T>& Az) {
        R_AB_.row(0) = Ax;
        R_AB_.row(1) = Ay;
        R_AB_.row(2) = Az;
        DRAKE_ASSERT_VOID(ThrowIfNotValid(R_AB_));
    ...
    // @returns `‖R - other‖∞`
    static T GetMaximumAbsoluteDifference(const Matrix3<T>& R, const Matrix3<T>& other) {
    ...
    static boolean<T> IsNearlyEqualTo(const Matrix3<T>& R, const Matrix3<T>& other, double tolerance) {
    ...
    // Engineering and Computer Science, Massachusetts Institute of Technology"
    // https://ocw.mit.edu/courses/electrical-engineering-and-computer-science/6-241j-dynamic-systems-and-control-spring-2011/readings/MIT6_241JS11_chap04.pdf
    static Matrix3<T> ProjectMatrix3ToOrthonormalMatrix3(const Matrix3<T>& M, T* quality_factor);
    ...
    static std::enable_if_t<scalar_predicate<S>::is_bool, Eigen::Quaternion<S>> RotationMatrixToUnnormalizedQuaternion(
    ...
    // are infinity or NaN.
    static Matrix3<T> QuaternionToRotationMatrix(const Eigen::Quaternion<T>& quaternion,
                                                 const T& two_over_norm_squared);
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::math::RotationMatrix);
