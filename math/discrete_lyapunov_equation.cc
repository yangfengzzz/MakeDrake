#include "math/discrete_lyapunov_equation.h"
...
MatrixXd RealDiscreteLyapunovEquation(const Eigen::Ref<const MatrixXd>& A, const Eigen::Ref<const MatrixXd>& Q) {
    ...
    for (int i = 0; i < eig_val.size(); ++i) {
        if (is_zero(eig_val(i).imag()) && (is_zero(eig_val(i).real() - 1) || is_zero(eig_val(i).real() + 1))) {
            throw std::runtime_error("RealDiscreteLyapunovEquation(): Solution is not unique!");
        }
    ...
    if (static_cast<int>(A.cols()) == 1) return internal::Solve1By1RealDiscreteLyapunovEquation(A, Q);
    ...
    if (schur.info() != Eigen::Success) {
        throw std::runtime_error("RealDiscreteLyapunovEquation(): Schur factorization failed.");
    }
    ...
    return (U * internal::SolveReducedRealDiscreteLyapunovEquation(S, Q_bar_upper) * U.transpose());
}
...
Vector1d Solve1By1RealDiscreteLyapunovEquation(const Eigen::Ref<const Vector1d>& A,
                                               const Eigen::Ref<const Vector1d>& Q) {
    return Vector1d(-Q(0) / (A(0) * A(0) - 1));
}

Matrix2d Solve2By2RealDiscreteLyapunovEquation(const Eigen::Ref<const Matrix2d>& A,
                                               const Eigen::Ref<const Matrix2d>& Q) {
    ...
    Matrix3d A_vec;
    A_vec << A(0, 0) * A(0, 0) - 1, 2 * A(0, 0) * A(1, 0), A(1, 0) * A(1, 0), A(0, 0) * A(0, 1),
            A(0, 0) * A(1, 1) + A(0, 1) * A(1, 0) - 1, A(1, 0) * A(1, 1), A(0, 1) * A(0, 1), 2 * A(0, 1) * A(1, 1),
            A(1, 1) * A(1, 1) - 1;
...
MatrixXd SolveReducedRealDiscreteLyapunovEquation(const Eigen::Ref<const MatrixXd>& S,
    ...
    if (n == 1) {
        ...
        const MatrixXd lhs{s_11(0) * S_1.transpose() - MatrixXd::Identity(S_1.cols(), S_1.rows())};
        ...
        lhs << s_11(0, 0) * S_1.transpose() - MatrixXd::Identity(m - 2, m - 2), s_11(1, 0) * S_1.transpose(),
                s_11(0, 1) * S_1.transpose(), s_11(1, 1) * S_1.transpose() - MatrixXd::Identity(m - 2, m - 2);

        VectorXd rhs(2 * (m - 2), 1);
        rhs << (-q_bar - s * x_bar_11 * s_11).col(0), (-q_bar - s * x_bar_11 * s_11).col(1);
    ...
    const Eigen::MatrixXd Q_new_upper{Q_1 + s * x_bar_11 * s.transpose() + temp_summand + temp_summand.transpose()};
    // The solution is found recursively.
    MatrixXd X_bar(S.cols(), S.rows());
    X_bar << x_bar_11, x_bar.transpose(), x_bar, SolveReducedRealDiscreteLyapunovEquation(S_1, Q_new_upper);
