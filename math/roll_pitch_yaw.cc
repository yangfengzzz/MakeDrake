...
// @author Paul Mitiguy
template <typename T>
Vector3<T> CalcRollPitchYawFromQuaternionAndRotationMatrix(const Eigen::Quaternion<T>& quaternion,
                                                           const Matrix3<T>& R) {
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcRotationMatrixDt(const Vector3<T>& rpyDt) const {
...
template <typename T>
Vector3<T> RollPitchYaw<T>::CalcRpyDDtFromRpyDtAndAngularAccelInParent(const Vector3<T>& rpyDt,
                                                                       const Vector3<T>& alpha_AD_A) const {
    ...
    const Matrix3<T> Minv = CalcMatrixRelatingRpyDtToAngularVelocityInParent(__func__);
    const Matrix3<T> MDt = CalcDtMatrixRelatingAngularVelocityInParentToRpyDt(rpyDt);
    return Minv * (alpha_AD_A - MDt * rpyDt);
}

template <typename T>
Vector3<T> RollPitchYaw<T>::CalcRpyDDtFromAngularAccelInChild(const Vector3<T>& rpyDt,
                                                              const Vector3<T>& alpha_AD_D) const {
  ...
  M << 1.0, sr_over_cp * sp, cr_over_cp * sp,
    ...
    const Vector3<T> remainder(tanp * rDt_pDt + one_over_cp * pDt_yDt, -cp * rDt * yDt,
...
template <typename T>
void RollPitchYaw<T>::CalcRotationMatrixDrDpDy(Matrix3<T>* R_r, Matrix3<T>* R_p, Matrix3<T>* R_y) const {
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcMatrixRelatingAngularVelocityInParentToRpyDt() const {
    using std::cos;
    using std::sin;
    const T& p = pitch_angle();
    const T& y = yaw_angle();
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcDtMatrixRelatingAngularVelocityInParentToRpyDt(const Vector3<T>& rpyDt) const {
    using std::cos;
    using std::sin;
    const T& p = pitch_angle();
    const T& y = yaw_angle();
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcMatrixRelatingAngularVelocityInChildToRpyDt() const {
    using std::cos;
    using std::sin;
    const T& r = roll_angle();
    const T& p = pitch_angle();
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcMatrixRelatingRpyDtToAngularVelocityInParent(const char* function_name) const {
    using std::cos;
    using std::sin;
    const T& p = pitch_angle();
    const T& y = yaw_angle();
    const T sp = sin(p), cp = cos(p);
    // TODO(Mitiguy) Improve accuracy when `cos(p) ≈ 0`.
    if (scalar_predicate<T>::is_bool && DoesCosPitchAngleViolateGimbalLockTolerance(cp)) {
...
template <typename T>
Matrix3<T> RollPitchYaw<T>::CalcMatrixRelatingRpyDtToAngularVelocityInChild(const char* function_name) const {
    using std::cos;
    using std::sin;
    const T& p = pitch_angle();
    const T& r = roll_angle();
...
void RollPitchYaw<T>::SetFromQuaternionAndRotationMatrix(const Eigen::Quaternion<T>& quaternion,
    ...
    const Vector3<T> rpy = CalcRollPitchYawFromQuaternionAndRotationMatrix(quaternion, R.matrix());
    ...
    if (scalar_predicate<T>::is_bool && !R_quaternion.IsNearlyEqualTo(R, tolerance)) {
...
template <typename T>
boolean<T> RollPitchYaw<T>::IsNearlySameOrientation(const RollPitchYaw<T>& other, double tolerance) const {
...
template <typename T>
void RollPitchYaw<T>::ThrowPitchAngleViolatesGimbalLockTolerance(const char* function_name, const T& pitch_angle) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::math::RollPitchYaw);
