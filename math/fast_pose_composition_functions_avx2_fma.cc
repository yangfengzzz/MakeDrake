...
#include "common/hwy_dynamic_impl.h"
...
void ComposeRR(const RotationMatrix<double>& R_AB, const RotationMatrix<double>& R_BC, RotationMatrix<double>* R_AC) {
    LateBoundFunction<ChooseBestComposeRR>::Call(GetRawData(R_AB), GetRawData(R_BC), GetRawData(R_AC));
}

void ComposeRinvR(const RotationMatrix<double>& R_BA,
                  const RotationMatrix<double>& R_BC,
                  RotationMatrix<double>* R_AC) {
    LateBoundFunction<ChooseBestComposeRinvR>::Call(GetRawData(R_BA), GetRawData(R_BC), GetRawData(R_AC));
}

void ComposeXX(const RigidTransform<double>& X_AB, const RigidTransform<double>& X_BC, RigidTransform<double>* X_AC) {
    LateBoundFunction<ChooseBestComposeXX>::Call(GetRawData(X_AB), GetRawData(X_BC), GetRawData(X_AC));
}

void ComposeXinvX(const RigidTransform<double>& X_BA,
                  const RigidTransform<double>& X_BC,
                  RigidTransform<double>* X_AC) {
    LateBoundFunction<ChooseBestComposeXinvX>::Call(GetRawData(X_BA), GetRawData(X_BC), GetRawData(X_AC));
}
