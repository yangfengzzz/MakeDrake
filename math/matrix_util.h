...
drake::MatrixX<typename Derived::Scalar> ToSymmetricMatrixFromLowerTriangularColumns(
    ...
    internal::to_symmetric_matrix_from_lower_triangular_columns_impl(rows, lower_triangular_columns, &symmetric_matrix);
...
drake::VectorX<typename Derived::Scalar> ToLowerTriangularColumnsFromMatrix(const Eigen::MatrixBase<Derived>& matrix) {
    ...
    for (int c = 0; c < matrix.cols(); ++c) {
        ...
        stacked_lower_triangular.segment(row_count, num_elts) = matrix.col(c).tail(num_elts);
        row_count += num_elts;
    }
...
bool IsPositiveDefinite(const Eigen::MatrixBase<Derived>& matrix,
    ...
    Eigen::SelfAdjointEigenSolver<typename Derived::PlainObject> eigensolver(matrix);
    ...
    return eigensolver.eigenvalues().minCoeff() >= eigenvalue_tolerance * std::max(1., max_abs_eigenvalue);
}
...
MatrixX<typename Derived::Scalar> ExtractPrincipalSubmatrix(const Eigen::MatrixBase<Derived>& mat,
    ...
    int interval_end{*it};
    DRAKE_THROW_UNLESS(interval_start >= 0 && *it < mat.rows() && *it < mat.cols());
    ...
    for (auto row_it = intervals.begin(); row_it != intervals.cend(); ++row_it) {
        ...
        for (auto col_it = intervals.begin(); col_it != intervals.cend(); ++col_it) {
            ...
            minor.block(minor_row_count, minor_col_count, cur_block_row_size, cur_block_col_size) =
                    mat.block(row_it->first, col_it->first, cur_block_row_size, cur_block_col_size);
