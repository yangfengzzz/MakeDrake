...
public:
    ...
    RollPitchYaw(const T& roll, const T& pitch, const T& yaw) { set(roll, pitch, yaw); }
    ...
    /// @param[in] rpy 3x1 array with roll, pitch, yaw angles (units of radians).
    /// @throws std::exception in debug builds if !IsValid(rpy).
    RollPitchYaw<T>& set(const Vector3<T>& rpy) { return SetOrThrowIfNotValidInDebugBuild(rpy); }
    ...
    /// @throws std::exception in debug builds if
    /// !IsValid(Vector3<T>(roll, pitch, yaw)).
    RollPitchYaw<T>& set(const T& roll, const T& pitch, const T& yaw) { return set(Vector3<T>(roll, pitch, yaw)); }
    ...
    Matrix3<T> ToMatrix3ViaRotationMatrix() const { return ToRotationMatrix().matrix(); }
    ...
    Vector3<T> CalcRpyDtFromAngularVelocityInParent(const Vector3<T>& w_AD_A) const {
        ...
        const Matrix3<T> M = CalcMatrixRelatingRpyDtToAngularVelocityInParent(__func__);
        return M * w_AD_A;
    }
    ...
    Vector3<T> CalcRpyDtFromAngularVelocityInChild(const Vector3<T>& w_AD_D) const {
        ...
        // TODO(Mitiguy) Improve speed -- first column of M is (1, 0, 0).
        // TODO(Mitiguy) Improve accuracy when `cos(p) ≈ 0`.
        const Matrix3<T> M = CalcMatrixRelatingRpyDtToAngularVelocityInChild(__func__);
        return M * w_AD_D;
    }
    ...
    friend void hash_append(HashAlgorithm& hasher, const RollPitchYaw& rpy) noexcept {
...
private:
    ...
    static typename std::enable_if_t<scalar_predicate<S>::is_bool> ThrowIfNotValid(const Vector3<T>& rpy) {
    ...
    template <typename S = T>
    static typename std::enable_if_t<!scalar_predicate<S>::is_bool> ThrowIfNotValid(const Vector3<S>&) {}
    ...
    static void ThrowPitchAngleViolatesGimbalLockTolerance(const char* function_name, const T& pitch_angle);
    ...
    void CalcRotationMatrixDrDpDy(Matrix3<T>* R_r, Matrix3<T>* R_p, Matrix3<T>* R_y) const;
    ...
    Matrix3<T> CalcDtMatrixRelatingAngularVelocityInParentToRpyDt(const Vector3<T>& rpyDt) const;
    ...
    Matrix3<T> CalcMatrixRelatingRpyDtToAngularVelocityInParent(const char* function_name) const;
    ...
    // method is called from a user-relevant API. If gimbal-lock is detected, the
    // issued error message includes the calling function's name.
    Matrix3<T> CalcMatrixRelatingRpyDtToAngularVelocityInChild(const char* function_name) const;
