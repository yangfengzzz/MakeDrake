...
typename Derived::Scalar DifferentiableNorm(const Eigen::MatrixBase<Derived>& x) {
    ...
    if constexpr (std::is_same_v<typename Derived::Scalar, AutoDiffXd>) {
        const Eigen::Matrix<double, Derived::RowsAtCompileTime, Derived::ColsAtCompileTime> x_val = ExtractValue(x);
        const double norm_val = x_val.norm();
        if (norm_val > 100 * kEps) {
            return x.norm();
        } else {
            return AutoDiffXd(norm_val, ExtractGradient(x).transpose() * x_val / (norm_val + 10 * kEps));
        }
    } else {
        return x.norm();
    }
