...
MatrixXd RealContinuousLyapunovEquation(const Eigen::Ref<const MatrixXd>& A, const Eigen::Ref<const MatrixXd>& Q) {
    ...
    if (static_cast<int>(A.cols()) == 1) {
        if (is_zero(A(0, 0))) {
            throw std::runtime_error("RealContinuousLyapunovEquation(): Solution is not unique!");
        }
    ...
    if (static_cast<int>(A.cols()) == 2) {
        ...
        if (is_zero(A(0, 0) + A(1, 1)) || is_zero(A(0, 0) * A(1, 1) - A(1, 0) * A(0, 1))) {
            throw std::runtime_error("RealContinuousLyapunovEquation(): Solution is not unique!");
        }
    ...
    if (schur.info() != Eigen::Success) {
        throw std::runtime_error("RealContinuousLyapunovEquation(): Schur factorization failed.");
    }
    ...
    return (U * internal::SolveReducedRealContinuousLyapunovEquation(S, Q_bar_upper) * U.transpose());
}
...
Vector1d Solve1By1RealContinuousLyapunovEquation(const Eigen::Ref<const Vector1d>& A,
                                                 const Eigen::Ref<const Vector1d>& Q) {
    return Vector1d(-Q(0) / (2 * A(0)));
}

Matrix2d Solve2By2RealContinuousLyapunovEquation(const Eigen::Ref<const Matrix2d>& A,
    ...
    // symmetric.
    Matrix3d A_vec;
    A_vec << 2 * A(0, 0), 2 * A(1, 0), 0, A(0, 1), A(0, 0) + A(1, 1), A(1, 0), 0, 2 * A(0, 1), 2 * A(1, 1);
...
MatrixXd SolveReducedRealContinuousLyapunovEquation(const Eigen::Ref<const MatrixXd>& S,
    ...
    if (n == 1) {
        ...
        const MatrixXd lhs{S_1.transpose() + MatrixXd::Identity(S_1.cols(), S_1.rows()) * s_11(0)};
        ...
        S_1_vec << S_1.transpose(), MatrixXd::Zero(m - 2, m - 2), MatrixXd::Zero(m - 2, m - 2), S_1.transpose();

        MatrixXd s_11_vec(2 * (m - 2), 2 * (m - 2));
        s_11_vec << s_11(0, 0) * MatrixXd::Identity(m - 2, m - 2), s_11(1, 0) * MatrixXd::Identity(m - 2, m - 2),
                s_11(0, 1) * MatrixXd::Identity(m - 2, m - 2), s_11(1, 1) * MatrixXd::Identity(m - 2, m - 2);
    ...
    const Eigen::MatrixXd Q_new_upper{(Q_1 + temp_summand + temp_summand.transpose())};
    // The solution is found recursively.
    MatrixXd X_bar(S.cols(), S.rows());
    X_bar << x_bar_11, x_bar.transpose(), x_bar, SolveReducedRealContinuousLyapunovEquation(S_1, Q_new_upper);
