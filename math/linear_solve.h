...
template <typename T>
inline constexpr bool is_double_or_symbolic_v = std::disjunction<std::is_same<T, double>, is_symbolic<T>>::value;
...
typename std::enable_if<internal::is_double_or_symbolic_v<typename LinearSolver::MatrixType::Scalar> &&
                                internal::is_double_or_symbolic_v<typename DerivedB::Scalar> &&
                                std::is_same_v<typename LinearSolver::MatrixType::Scalar, typename DerivedB::Scalar>,
...
SolveLinearSystem(const LinearSolver& linear_solver, const Eigen::MatrixBase<DerivedB>& b) {
    ...
    const Eigen::Matrix<double, DerivedB::RowsAtCompileTime, DerivedB::ColsAtCompileTime> x_val =
            linear_solver.solve(ExtractValue(b));
    Eigen::Matrix<double, DerivedB::RowsAtCompileTime, DerivedB::Scalar::DerType::RowsAtCompileTime> grad(b.rows(),
                                                                                                          num_z_b);
    Eigen::Matrix<typename DerivedB::Scalar, DerivedB::RowsAtCompileTime, DerivedB::ColsAtCompileTime> x_ad(b.rows(),
                                                                                                            b.cols());
...
SolveLinearSystem(const LinearSolver& linear_solver,
    ...
    if (num_z_A == 0 && num_z_b == 0) {
        if constexpr (std::is_same_v<typename DerivedB::Scalar, double>) {
            return SolveLinearSystem(linear_solver, ExtractValue(A), b).template cast<typename DerivedA::Scalar>();
        } else {
    ...
    Eigen::Matrix<double, DerivedA::RowsAtCompileTime, DerivedB::ColsAtCompileTime> x_val;
    if constexpr (std::is_same_v<typename DerivedB::Scalar, double>) {
    ...
    Eigen::Matrix<typename DerivedA::Scalar, DerivedA::RowsAtCompileTime, DerivedB::ColsAtCompileTime> x_ad(A.rows(),
                                                                                                            b.cols());
    ...
    for (int i = 0; i < A.rows(); ++i) {
        for (int j = 0; j < b.cols(); ++j) {
            ...
            x_ad(i, j).derivatives() =
                    Eigen::Matrix<double, DerivedA::Scalar::DerType::RowsAtCompileTime, 1>::Zero(num_z_A);
        }
    }

    Eigen::Matrix<double, DerivedA::RowsAtCompileTime, DerivedA::ColsAtCompileTime> dAdzi(A.rows(), A.cols());
    // This stores ∂b/∂zᵢ
    Eigen::Matrix<double, DerivedB::RowsAtCompileTime, DerivedB::ColsAtCompileTime> dbdzi(A.rows(), b.cols());
    // This stores ∂x/∂zᵢ
    Eigen::Matrix<double, DerivedA::RowsAtCompileTime, DerivedB::ColsAtCompileTime> dxdzi(A.rows(), b.cols());
...
using EigenLinearSolver =
        LinearSolverType<Eigen::Matrix<std::conditional_t<internal::is_double_or_symbolic_v<typename DerivedA::Scalar>,
...
using Promoted = typename std::conditional_t<
        ...
        ScalarA,
        std::conditional_t<std::is_same_v<double, ScalarA>,
                           ScalarB,
...
using Solution = typename Eigen::Matrix<Promoted<typename DerivedA::Scalar, typename DerivedB::Scalar>,
                                        DerivedA::RowsAtCompileTime,
...
internal::EigenLinearSolver<LinearSolverType, DerivedA> GetLinearSolver(const Eigen::MatrixBase<DerivedA>& A) {
    if constexpr (internal::is_double_or_symbolic_v<typename DerivedA::Scalar>) {
        ...
        const internal::EigenLinearSolver<LinearSolverType, DerivedA> linear_solver(A_val);
        return linear_solver;
    }
}
//@}
...
internal::Solution<DerivedA, DerivedB> SolveLinearSystem(const Eigen::MatrixBase<DerivedA>& A,
    ...
    static_assert(std::is_same_v<ScalarA, ScalarB> ||
                          (!internal::is_symbolic_v<ScalarA> && !internal::is_symbolic_v<ScalarB>),
...
public:
    ...
    using SolutionType = std::conditional_t<std::is_same_v<typename DerivedA::Scalar, typename DerivedB::Scalar> &&
                                                    internal::is_double_or_symbolic_v<typename DerivedA::Scalar>,
    ...
    SolutionType<DerivedB> Solve(const Eigen::MatrixBase<DerivedB>& b) const {
        ...
        if constexpr (std::is_same_v<ScalarA, ScalarB> && !internal::is_autodiff_v<ScalarA>) {
            return eigen_linear_solver_.solve(b);
            // NOLINTNEXTLINE(readability/braces)
        } else if constexpr (std::is_same_v<ScalarA, double> && internal::is_autodiff_v<ScalarB>) {
...
private:
    ...
    std::optional<Eigen::Matrix<typename DerivedA::Scalar,
                                DerivedA::RowsAtCompileTime,
                                DerivedA::ColsAtCompileTime,
                                Eigen::ColMajor,
                                DerivedA::MaxRowsAtCompileTime,
