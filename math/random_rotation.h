...
Eigen::Quaternion<T> UniformlyRandomQuaternion(Generator* generator) {
    ...
    using std::cos;
    using std::sin;
    return Eigen::Quaternion<T>(sqrt_one_minus_u1 * sin(2 * M_PI * u2), sqrt_one_minus_u1 * cos(2 * M_PI * u2),
                                sqrt_u1 * sin(2 * M_PI * u3), sqrt_u1 * cos(2 * M_PI * u3));
}
...
Eigen::AngleAxis<T> UniformlyRandomAngleAxis(Generator* generator) {
    DRAKE_DEMAND(generator != nullptr);
    const Eigen::Quaternion<T> quaternion = UniformlyRandomQuaternion<T>(generator);
