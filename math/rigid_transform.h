...
public:
    ...
    /// @param[in] p position vector from frame A's origin to frame B's origin,
    /// expressed in frame A.  In monogram notation p is denoted `p_AoBo_A`.
    RigidTransform(const RollPitchYaw<T>& rpy, const Vector3<T>& p) : RigidTransform(RotationMatrix<T>(rpy), p) {}
    ...
    /// position vector.
    /// @param[in] R rotation matrix relating frames A and B (e.g., `R_AB`).
    explicit RigidTransform(const RotationMatrix<T>& R) { set(R, Vector3<T>::Zero()); }
    ...
    static RigidTransform<T> MakeUnchecked(const Eigen::Matrix<T, 3, 4>& pose) {
        ...
        result.R_AB_ = RotationMatrix<T>::MakeUnchecked(pose.template block<3, 3>(0, 0));
    ...
    /// @note No attempt is made to orthogonalize the 3x3 rotation matrix part of
    /// `pose`.  As needed, use RotationMatrix::ProjectToRotationMatrix().
    void SetFromIsometry3(const Isometry3<T>& pose) { set(RotationMatrix<T>(pose.linear()), pose.translation()); }
    ...
    void set_rotation(const RollPitchYaw<T>& rpy) { set_rotation(RotationMatrix<T>(rpy)); }
    ...
    void set_rotation(const Eigen::Quaternion<T>& quaternion) { set_rotation(RotationMatrix<T>(quaternion)); }
    ...
    /// describes the parameter, preconditions, exception conditions, etc.
    void set_rotation(const Eigen::AngleAxis<T>& theta_lambda) { set_rotation(RotationMatrix<T>(theta_lambda)); }
    ...
    boolean<T> IsNearlyIdentity(double translation_tolerance) const {
        ...
        return max_component <= translation_tolerance && rotation().IsNearlyIdentity();
    }
    ...
    boolean<T> IsExactlyEqualTo(const RigidTransform<T>& other) const {
        return rotation().IsExactlyEqualTo(other.rotation()) && translation() == other.translation();
    }
    ...
    /// origin location.
    friend RigidTransform<T> operator*(const Eigen::Translation<T, 3>& X_AAq, const RigidTransform<T>& X_AqB) {
    ...
    Vector3<T> operator*(const Vector3<T>& p_BoQ_B) const { return p_AoBo_A_ + R_AB_ * p_BoQ_B; }
    ...
    Eigen::Matrix<typename Derived::Scalar, 3, Derived::ColsAtCompileTime> operator*(
            const Eigen::MatrixBase<Derived>& p_BoQ_B) const {
        if (p_BoQ_B.rows() != 3) {
        ...
        const Eigen::Matrix<typename Derived::Scalar, 3, Derived::ColsAtCompileTime> p_BoQ_A = R_AB * p_BoQ_B;

        // Reserve space (on stack or heap) to store the result.
        const int number_of_position_vectors = p_BoQ_B.cols();
        Eigen::Matrix<typename Derived::Scalar, 3, Derived::ColsAtCompileTime> p_AoQ_A(3, number_of_position_vectors);

        // Create each returned position vector as p_AoQi_A = p_AoBo_A + p_BoQi_A.
        for (int i = 0; i < number_of_position_vectors; ++i) p_AoQ_A.col(i) = translation() + p_BoQ_A.col(i);
    ...
    friend void hash_append(HashAlgorithm& hasher, const RigidTransform& X) noexcept {
...
private:
    ...
    static constexpr double kInternalToleranceForHomogeneousCheck{4 * std::numeric_limits<double>::epsilon()};

    // Constructs a RigidTransform without initializing the underlying 3x4 matrix.
    explicit RigidTransform(internal::DoNotInitializeMemberFields) : R_AB_{internal::DoNotInitializeMemberFields{}} {}
    ...
    static void ThrowIfInvalidBottomRow(const Eigen::MatrixBase<Derived>& pose) {
        ...
        if (pose(3, 0) != 0 || pose(3, 1) != 0 || pose(3, 2) != 0 || pose(3, 3) != 1) {
            throw std::logic_error(fmt::format("Error: Bottom row of 4x4 matrix differs from [0, 0, 0, 1]"));
        }
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::math::RigidTransform);
