...
Eigen::MatrixXd DiscreteAlgebraicRiccatiEquation(const Eigen::Ref<const Eigen::MatrixXd>& A,
    ...
    auto Q_ldlt = Q.ldlt();
    DRAKE_THROW_UNLESS(Q_ldlt.info() == Eigen::Success && (Q_ldlt.vectorD().array() >= 0.0).all());
    ...
    DRAKE_THROW_UNLESS(drake::systems::internal::IsStabilizable(A, B, false, std::nullopt));
    ...
    Eigen::MatrixXd C = (Q_ldlt.transpositionsP().transpose() *
                         ...
                         Eigen::MatrixXd{Q_ldlt.matrixL()} * Q_ldlt.vectorD().cwiseSqrt().asDiagonal())
                                .transpose();
    DRAKE_THROW_UNLESS(drake::systems::internal::IsDetectable(A, C, false, std::nullopt));
    ...
    do {
        ...
        // W = I + GₖHₖ
        Eigen::MatrixXd W = Eigen::MatrixXd::Identity(H_k.rows(), H_k.cols()) + G_k * H_k;
