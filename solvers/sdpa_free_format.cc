...
void SdpaFreeFormat::DeclareXforPositiveSemidefiniteConstraints(
    ...
    for (const auto& binding : prog.positive_semidefinite_constraints()) {
        ...
        for (int j = 0; j < matrix_rows; ++j) {
            for (int i = 0; i <= j; ++i) {
                const symbolic::Variable& psd_ij_var = binding.variables()(psd_matrix_variable_index++);
                const int psd_ij_var_index = prog.FindDecisionVariableIndex(psd_ij_var);
                const bool has_var_registered =
                        !(std::holds_alternative<std::nullptr_t>(prog_var_in_sdpa_[psd_ij_var_index]));
                ...
                if (!has_var_registered) {
                    ...
                    prog_var_in_sdpa_[psd_ij_var_index].emplace<DecisionVariableInSdpaX>(Sign::kPositive, 0,
                                                                                         psd_ij_entry_in_X);
                } else {
                    ...
                    // already.
                    const auto it2 = entries_in_X_for_same_decision_variable->find(psd_ij_var.get_id());
                    if (it2 == entries_in_X_for_same_decision_variable->end()) {
                        ...
                        entries_in_X_for_same_decision_variable->emplace_hint(
                                ...
                                std::vector<EntryInX>(
                                        {std::get<DecisionVariableInSdpaX>(prog_var_in_sdpa_[psd_ij_var_index])
                                                 .entry_in_X,
                                         psd_ij_entry_in_X}));
                    } else {
...
void AddTermToTriplets(const EntryInX& entry_in_X, double coeff, std::vector<Eigen::Triplet<double>>* triplets) {
    if (entry_in_X.row_index_in_block == entry_in_X.column_index_in_block) {
        triplets->emplace_back(entry_in_X.X_start_row + entry_in_X.row_index_in_block,
        ...
        triplets->emplace_back(entry_in_X.X_start_row + entry_in_X.column_index_in_block,
                               entry_in_X.X_start_row + entry_in_X.row_index_in_block, coeff / 2);
    }
}

void SdpaFreeFormat::AddLinearEqualityConstraint(const std::vector<double>& coeff_prog_vars,
    ...
    Ai_triplets.reserve(static_cast<int>(coeff_prog_vars.size() + coeff_X_entries.size()) * 2);
    ...
    for (int i = 0; i < static_cast<int>(coeff_prog_vars.size()); ++i) {
        if (coeff_prog_vars[i] != 0) {
            ...
            if (std::holds_alternative<DecisionVariableInSdpaX>(prog_var_in_sdpa_[prog_var_index])) {
                // This variable is an entry in X.
                const auto& decision_var_in_X = std::get<DecisionVariableInSdpaX>(prog_var_in_sdpa_[prog_var_index]);
                g_(constraint_index) -= coeff_prog_vars[i] * decision_var_in_X.offset;
                const double coeff =
                        decision_var_in_X.coeff_sign == Sign::kPositive ? coeff_prog_vars[i] : -coeff_prog_vars[i];
                AddTermToTriplets(decision_var_in_X.entry_in_X, coeff, &Ai_triplets);
            } else if (std::holds_alternative<double>(prog_var_in_sdpa_[prog_var_index])) {
                // This variable has a constant value.
                const double var_value = std::get<double>(prog_var_in_sdpa_[prog_var_index]);
                g_(constraint_index) -= coeff_prog_vars[i] * var_value;
            } else if (std::holds_alternative<FreeVariableIndex>(prog_var_in_sdpa_[prog_var_index])) {
                // This variable is a free variable (no lower nor upper bound).
                B_triplets_.emplace_back(constraint_index,
    ...
    if (!coeff_X_entries.empty()) {
        for (int i = 0; i < static_cast<int>(coeff_free_vars.size()); ++i) {
            B_triplets_.emplace_back(constraint_index, free_vars_indices[i], coeff_free_vars[i]);
        }
    }
}
...
void SdpaFreeFormat::AddEqualityConstraintOnXEntriesForSameDecisionVariable(
    ...
    for (const auto& item : entries_in_X_for_same_decision_variable) {
        ...
        for (int i = 1; i < static_cast<int>(Xentries.size()); ++i) {
            AddLinearEqualityConstraint({} /* coefficients for prog_vars */, {} /* empty prog_vars */, a,
    ...
    if (std::isinf(lower_bound) && std::isinf(upper_bound)) {
        ...
        if (!std::isinf(lower_bound) && std::isinf(upper_bound)) {
            ...
            prog_var_in_sdpa_[variable_index].emplace<DecisionVariableInSdpaX>(
                    Sign::kPositive, lower_bound, block_index, *new_X_var_count, *new_X_var_count, num_X_rows_);
            (*new_X_var_count)++;
        } else if (std::isinf(lower_bound) && !std::isinf(upper_bound)) {
            // x <= upper.
            prog_var_in_sdpa_[variable_index].emplace<DecisionVariableInSdpaX>(
                    Sign::kNegative, upper_bound, block_index, *new_X_var_count, *new_X_var_count, num_X_rows_);
            (*new_X_var_count)++;
            ...
            AddLinearEqualityConstraint(
                    {1}, {variable_index}, {1.0},
                    {EntryInX(block_index, *new_X_var_count + 1, *new_X_var_count + 1, num_X_rows_)}, {}, {},
                    upper_bound);
            (*new_X_var_count) += 2;
        }
    }
...
void SdpaFreeFormat::AddBoundsOnRegisteredDecisionVariable(
    ...
    if (std::holds_alternative<DecisionVariableInSdpaX>(prog_var_in_sdpa_[variable_index])) {
        if (std::isinf(lower_bound) && std::isinf(upper_bound)) {
            // no finite bounds.
            return;
        } else if (!std::isinf(lower_bound) && std::isinf(upper_bound)) {
            ...
            AddLinearEqualityConstraint({1}, {variable_index}, {-1.0},
                                        {EntryInX(block_index, *new_X_var_count, *new_X_var_count, num_X_rows_)}, {},
                                        {}, lower_bound);
            (*new_X_var_count)++;
        } else if (std::isinf(lower_bound) && !std::isinf(upper_bound)) {
            // x <= upper.
            ...
            AddLinearEqualityConstraint({1.0}, {variable_index}, {}, {}, {}, {}, lower_bound);
        } else {
            // lower <= x <= upper.
...
void SdpaFreeFormat::RegisterMathematicalProgramDecisionVariables(const MathematicalProgram& prog) {
    ...
    Eigen::VectorXd lower_bound = Eigen::VectorXd::Constant(prog.num_vars(), -kInf);
    Eigen::VectorXd upper_bound = Eigen::VectorXd::Constant(prog.num_vars(), kInf);
    for (const auto& bounding_box : prog.bounding_box_constraints()) {
        for (int i = 0; i < bounding_box.variables().rows(); ++i) {
            const int variable_index = prog.FindDecisionVariableIndex(bounding_box.variables()(i));
            lower_bound(variable_index) =
                    std::max(lower_bound(variable_index), bounding_box.evaluator()->lower_bound()(i));
            upper_bound(variable_index) =
                    std::min(upper_bound(variable_index), bounding_box.evaluator()->upper_bound()(i));
        }
    }
    ...
    for (int i = 0; i < prog.num_vars(); ++i) {
        ...
        bool has_var_registered = !std::holds_alternative<std::nullptr_t>(prog_var_in_sdpa_[i]);
        // First check if this variable has either finite lower or upper bound. If
        // not, then the variable is a free variable in SDPA.
        if (!has_var_registered) {
            ...
            AddBoundsOnRegisteredDecisionVariable(lower_bound(i), upper_bound(i), i, block_index, &new_X_var_count);
        }
    }
...
void SdpaFreeFormat::AddLinearCostsFromProgram(const MathematicalProgram& prog) {
    for (const auto& linear_cost : prog.linear_costs()) {
        for (int i = 0; i < linear_cost.variables().rows(); ++i) {
            ...
            if (coeff != 0) {
                ...
                const int var_index = prog.FindDecisionVariableIndex(linear_cost.variables()(i));
                if (std::holds_alternative<DecisionVariableInSdpaX>(prog_var_in_sdpa_[var_index])) {
                    const auto& decision_var_in_X = std::get<DecisionVariableInSdpaX>(prog_var_in_sdpa_[var_index]);
                    coeff = decision_var_in_X.coeff_sign == Sign::kPositive ? coeff : -coeff;
                    constant_min_cost_term_ += linear_cost.evaluator()->a()(i) * decision_var_in_X.offset;
                    AddTermToTriplets(decision_var_in_X.entry_in_X, coeff, &C_triplets_);
                } else if (std::holds_alternative<double>(prog_var_in_sdpa_[var_index])) {
                    const double val = std::get<double>(prog_var_in_sdpa_[var_index]);
                    constant_min_cost_term_ += linear_cost.evaluator()->a()(i) * val;
                } else if (std::holds_alternative<FreeVariableIndex>(prog_var_in_sdpa_[var_index])) {
                    const FreeVariableIndex& s_index = std::get<FreeVariableIndex>(prog_var_in_sdpa_[var_index]);
...
void SdpaFreeFormat::AddLinearConstraintsHelper(const MathematicalProgram& prog,
                                                const Binding<Constraint>& linear_constraint,
                                                bool is_equality_constraint,
                                                int* linear_constraint_slack_entry_in_X_count) {
    const std::vector<int> var_indices = prog.FindDecisionVariableIndices(linear_constraint.variables());
    ...
    for (int i = 0; i < linear_constraint.evaluator()->num_constraints(); ++i) {
        const bool does_lower_equal_upper_in_this_row =
                ...
                linear_constraint.evaluator()->lower_bound()(i) == linear_constraint.evaluator()->upper_bound()(i);
        ...
        if (a.size() == 0) {
            if (linear_constraint.evaluator()->lower_bound()(i) <= 0 &&
                ...
                throw std::runtime_error(
                        fmt::format("SdpaFreeFormat: the problem is infeasible as it contains "
        ...
        if (does_lower_equal_upper_in_this_row) {
            ...
            AddLinearEqualityConstraint(a, decision_var_indices_in_X, {}, {}, {}, {},
                                        linear_constraint.evaluator()->lower_bound()(i));
        } else {
            ...
            if (!std::isinf(linear_constraint.evaluator()->lower_bound()(i))) {
                AddLinearEqualityConstraint(
                        ...
                        {EntryInX(static_cast<int>(X_blocks_.size()), *linear_constraint_slack_entry_in_X_count,
...
void SdpaFreeFormat::AddLinearConstraintsFromProgram(const MathematicalProgram& prog) {
    int linear_constraint_slack_entry_in_X_count = 0;
    for (const auto& linear_eq_constraint : prog.linear_equality_constraints()) {
        AddLinearConstraintsHelper(prog, linear_eq_constraint, true, &linear_constraint_slack_entry_in_X_count);
    }
    for (const auto& linear_constraint : prog.linear_constraints()) {
        AddLinearConstraintsHelper(prog, linear_constraint, false, &linear_constraint_slack_entry_in_X_count);
    }
    if (linear_constraint_slack_entry_in_X_count > 0) {
        num_X_rows_ += linear_constraint_slack_entry_in_X_count;
        X_blocks_.emplace_back(BlockType::kDiagonal, linear_constraint_slack_entry_in_X_count);
    }
}

void SdpaFreeFormat::AddLinearMatrixInequalityConstraints(const MathematicalProgram& prog) {
    for (const auto& lmi_constraint : prog.linear_matrix_inequality_constraints()) {
        const std::vector<int> var_indices = prog.FindDecisionVariableIndices(lmi_constraint.variables());
        ...
        for (int j = 0; j < lmi_constraint.evaluator()->matrix_rows(); ++j) {
            for (int i = 0; i <= j; ++i) {
                ...
                AddLinearEqualityConstraint(a, var_indices, {-1},
                                            {EntryInX(static_cast<int>(X_blocks_.size()), i, j, num_X_rows_)}, {}, {},
                                            -F[0](i, j));
            }
        }

        X_blocks_.emplace_back(BlockType::kMatrix, lmi_constraint.evaluator()->matrix_rows());
...
// ⎢    ....      ⎥
// ⎣ zₙ 0   0  z₀ ⎦
void SdpaFreeFormat::AddLorentzConeConstraints(const MathematicalProgram& prog) {
    for (const auto& lorentz_cone_constraint : prog.lorentz_cone_constraints()) {
        ...
        for (int i = 0; i < num_block_rows; ++i) {
            AddLinearEqualityConstraint(a, prog_vars_indices, {-1.0},
        ...
        for (int i = 1; i < num_block_rows; ++i) {
            ...
            for (int j = 0; j < num_decision_vars; ++j) {
                const double coeff = lorentz_cone_constraint.evaluator()->A_dense()(i, j);
                a.push_back(coeff);
            }
            // Add the term -new_block(0, i)
            AddLinearEqualityConstraint(a, prog_vars_indices, {-1},
        ...
        for (int i = 1; i < num_block_rows; ++i) {
            for (int j = 1; j < i; ++j) {
                AddLinearEqualityConstraint(
                        {}, {}, {1.0}, {EntryInX(static_cast<int>(X_blocks_.size()), i, j, num_X_rows_)}, {}, {}, 0);
            }
        }
...
// ⎢ zₙ₋₁ ...   z₁ 0 ⎥
// ⎣ zₙ 0 ... 0    z₁⎦
void SdpaFreeFormat::AddRotatedLorentzConeConstraints(const MathematicalProgram& prog) {
    for (const auto& rotated_lorentz_constraint : prog.rotated_lorentz_cone_constraints()) {
        ...
        const std::vector<int> prog_vars_indices =
                prog.FindDecisionVariableIndices(rotated_lorentz_constraint.variables());
        ...
        for (int j = 2; j < num_block_rows; ++j) {
            for (int i = 1; i < j; ++i) {
                AddLinearEqualityConstraint(
                        {}, {}, {1}, {EntryInX(static_cast<int>(X_blocks_.size()), i, j, num_X_rows_)}, {}, {}, 0);
            }
        }
...
SdpaFreeFormat::SdpaFreeFormat(const MathematicalProgram& prog) {
    ...
    if (!AreRequiredAttributesSupported(prog.required_capabilities(), solver_capabilities)) {
    ...
    std::unordered_map<symbolic::Variable::Id, std::vector<EntryInX>> entries_in_X_for_same_decision_variable;
    DeclareXforPositiveSemidefiniteConstraints(prog, &entries_in_X_for_same_decision_variable);

    AddEqualityConstraintOnXEntriesForSameDecisionVariable(entries_in_X_for_same_decision_variable);
...
void SdpaFreeFormat::RemoveFreeVariableByNullspaceApproach(
    ...
    Q = QR_B->matrixQ();
    const Eigen::SparseMatrix<double> N = Q.rightCols(this->B().rows() - QR_B->rank());
    *rhs_hat = N.transpose() * this->g();
    ...
    Eigen::SparseQR<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int>> qr_B_t;
    qr_B_t.compute(B_t);
...
void SdpaFreeFormat::RemoveFreeVariableByTwoSlackVariablesApproach(std::vector<internal::BlockInX>* X_hat_blocks,
                                                                   std::vector<Eigen::SparseMatrix<double>>* A_hat,
                                                                   Eigen::SparseMatrix<double>* C_hat) const {
    *X_hat_blocks = this->X_blocks();
    X_hat_blocks->emplace_back(internal::BlockType::kDiagonal, 2 * this->num_free_variables());
    const int num_X_hat_rows = this->num_X_rows() + 2 * this->num_free_variables();
    std::vector<std::vector<Eigen::Triplet<double>>> A_hat_triplets = this->A_triplets();
    for (int j = 0; j < this->num_free_variables(); ++j) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(this->B(), j); it; ++it) {
            const int i = it.row();
            // Add the entry in Âᵢ that multiplies with sⱼ.
            A_hat_triplets[i].emplace_back(this->num_X_rows() + j, this->num_X_rows() + j, it.value());
            A_hat_triplets[i].emplace_back(this->num_X_rows() + this->num_free_variables() + j,
    ...
    for (int i = 0; i < static_cast<int>(this->A().size()); ++i) {
        ...
        A_hat->back().setFromTriplets(A_hat_triplets[i].begin(), A_hat_triplets[i].end());
    }
    ...
    for (Eigen::SparseMatrix<double>::InnerIterator it(this->d(), 0); it; ++it) {
        const int i = it.row();
        C_hat_triplets.emplace_back(this->num_X_rows() + i, this->num_X_rows() + i, it.value());
        C_hat_triplets.emplace_back(this->num_X_rows() + this->num_free_variables() + i,
...
void SdpaFreeFormat::RemoveFreeVariableByLorentzConeSlackApproach(std::vector<internal::BlockInX>* X_hat_blocks,
                                                                  std::vector<Eigen::SparseMatrix<double>>* A_hat,
                                                                  Eigen::VectorXd* rhs_hat,
                                                                  Eigen::SparseMatrix<double>* C_hat) const {
    *X_hat_blocks = this->X_blocks();
    X_hat_blocks->emplace_back(internal::BlockType::kMatrix, this->num_free_variables() + 1);
    const int num_X_hat_rows = this->num_X_rows() + this->num_free_variables() + 1;
    std::vector<std::vector<Eigen::Triplet<double>>> A_hat_triplets = this->A_triplets();
    // Âᵢ = diag(Aᵢ, B̂ᵢ)
    // where B̂ᵢ = ⎡0  bᵢᵀ/2⎤
    //            ⎣bᵢ/2   0⎦
    for (int j = 0; j < this->num_free_variables(); ++j) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(this->B(), j); it; ++it) {
            const int i = it.row();
            // Add the entry in Âᵢ that multiplies with sⱼ.
            A_hat_triplets[i].emplace_back(this->num_X_rows(), this->num_X_rows() + j + 1, it.value() / 2);
            A_hat_triplets[i].emplace_back(this->num_X_rows() + j + 1, this->num_X_rows(), it.value() / 2);
        }
    }
    const int num_linear_eq_constr = this->A().size() + (num_free_variables_ * (num_free_variables_ + 1)) / 2;
    A_hat->clear();
    ...
    *rhs_hat = Eigen::VectorXd::Zero(num_linear_eq_constr);
    ...
    for (Eigen::SparseMatrix<double>::InnerIterator it(this->d(), 0); it; ++it) {
        const int i = it.row();
        C_hat_triplets.emplace_back(this->num_X_rows(), this->num_X_rows() + i + 1, it.value() / 2);
        C_hat_triplets.emplace_back(this->num_X_rows() + i + 1, this->num_X_rows(), it.value() / 2);
    }
    *C_hat = Eigen::SparseMatrix<double>(num_X_hat_rows, num_X_hat_rows);
    C_hat->setFromTriplets(C_hat_triplets.begin(), C_hat_triplets.end());
}
...
bool GenerateSdpaImpl(const std::vector<BlockInX>& X_blocks,
    ...
    if (sdpa_file.is_open()) {
            ...
            for (Eigen::SparseMatrix<double>::InnerIterator it(C, i); it; ++it) {
                if (it.row() <= it.col()) {
                    ...
                    sdpa_file << 0 /* 0 for cost matrix C */ << " "
                              << row_to_block_indices[i] + 1           /* block number, starts from 1 */
                              << " " << it.row() - block_start_row + 1 /* block row index, starts from 1*/
                              << " " << i - block_start_row + 1        /* block column index, starts from 1*/
        ...
        for (int i = 0; i < static_cast<int>(A.size()); ++i) {
            for (int j = 0; j < num_X_rows; ++j) {
                for (Eigen::SparseMatrix<double>::InnerIterator it(A[i], j); it; ++it) {
                    if (it.row() <= it.col()) {
                        const int block_start_row = block_start_rows[row_to_block_indices[j]];
                        sdpa_file << i + 1 /* constraint number, starts from 1 */ << " "
                                  << row_to_block_indices[j] + 1 /* block number, starts from 1 */
                                  << " " << it.row() - block_start_row + 1 << " " << j - block_start_row + 1 << " "
                                  << fmt_floating_point(it.value()) << "\n";
                    }
                }
            }
        ...
        drake::log()->warn("GenerateSDPA(): Cannot open the file {}.dat-s", file_name);
        return false;
    }
    sdpa_file.close();
    return true;
...
bool GenerateSDPA(const MathematicalProgram& prog, const std::string& file_name, RemoveFreeVariableMethod method) {
    const internal::SdpaFreeFormat sdpa_free_format(prog);
    if (sdpa_free_format.num_free_variables() == 0) {
        return internal::GenerateSdpaImpl(sdpa_free_format.X_blocks(), sdpa_free_format.C(), sdpa_free_format.A(),
    ...
    switch (method) {
        case RemoveFreeVariableMethod::kNullspace: {
            ...
            sdpa_free_format.RemoveFreeVariableByNullspaceApproach(&C_hat, &A_hat, &rhs_hat, &y_hat, &QR_B);
            return internal::GenerateSdpaImpl(sdpa_free_format.X_blocks(), C_hat, A_hat, rhs_hat, file_name);
            break;
        }
        case RemoveFreeVariableMethod::kTwoSlackVariables: {
            ...
            std::vector<Eigen::SparseMatrix<double>> A_hat;
            Eigen::SparseMatrix<double> C_hat;
            sdpa_free_format.RemoveFreeVariableByTwoSlackVariablesApproach(&X_hat_blocks, &A_hat, &C_hat);
            return internal::GenerateSdpaImpl(X_hat_blocks, C_hat, A_hat, sdpa_free_format.g(), file_name);
        }
        case RemoveFreeVariableMethod::kLorentzConeSlack: {
            std::vector<internal::BlockInX> X_hat_blocks;
            std::vector<Eigen::SparseMatrix<double>> A_hat;
            Eigen::VectorXd rhs_hat;
            Eigen::SparseMatrix<double> C_hat;
            sdpa_free_format.RemoveFreeVariableByLorentzConeSlackApproach(&X_hat_blocks, &A_hat, &rhs_hat, &C_hat);
            return internal::GenerateSdpaImpl(X_hat_blocks, C_hat, A_hat, rhs_hat, file_name);
        }
    }
    return false;
}
