...
template <>
void UnrevisedLemkeSolver<AutoDiffXd>::DoSolve(const MathematicalProgram&,
...
void UnrevisedLemkeSolver<T>::DoSolve(const MathematicalProgram& prog,
    ...
    for (const auto& binding : bindings) {
        ...
        const std::shared_ptr<LinearComplementarityConstraint> constraint = binding.evaluator();
        bool solved = SolveLcpLemke(constraint->M(), constraint->q(), &constraint_solution, &num_pivots);
        if (!solved) {
            result->set_solution_result(SolutionResult::kSolverSpecificError);
            return;
        }
        for (int i = 0; i < binding.evaluator()->num_vars(); ++i) {
            const int variable_index = prog.FindDecisionVariableIndex(binding.variables()(i));
...
template <class T>
void UnrevisedLemkeSolver<T>::SelectSubColumnWithCovering(const MatrixX<T>& in,
...
template <class T>
void UnrevisedLemkeSolver<T>::SetSubVector(const VectorX<T>& v, const std::vector<int>& row_indices, VectorX<T>* out) {
...
template <class T>
bool UnrevisedLemkeSolver<T>::ValidateIndices(const std::vector<int>& row_indices, int vector_size) {
    ...
    return std::unique(sorted_row_indices.begin(), sorted_row_indices.end()) == sorted_row_indices.end();
}
...
template <class T>
bool UnrevisedLemkeSolver<T>::ValidateIndices(const std::vector<int>& row_indices,
                                              const std::vector<int>& col_indices,
                                              int num_rows,
                                              int num_cols) {
    return ValidateIndices(row_indices, num_rows) && ValidateIndices(col_indices, num_cols);
}
...
template <class T>
void UnrevisedLemkeSolver<T>::SelectSubMatrixWithCovering(const MatrixX<T>& in,
                                                          const std::vector<int>& row_indices,
                                                          const std::vector<int>& col_indices,
                                                          MatrixX<T>* out) {
    const int num_rows = row_indices.size();
    const int num_cols = col_indices.size();
    DRAKE_ASSERT(ValidateIndices(row_indices, col_indices, in.rows(), in.cols() + 1));
...
void UnrevisedLemkeSolver<T>::DetermineIndexSets() const {
    ...
    auto DetermineIndexSetsHelper = [this](const std::vector<LCPVariable>& variables, bool is_z,
                                           std::vector<int>* variable_set, std::vector<int>* variable_set_prime) {
        variable_and_array_indices_.clear();
        for (int i = 0; i < static_cast<int>(variables.size()); ++i) {
            if (variables[i].is_z() == is_z) variable_and_array_indices_.emplace_back(variables[i].index(), i);
        }
        std::sort(variable_and_array_indices_.begin(), variable_and_array_indices_.end());

        // Construct the set and the primed set.
        for (const auto& variable_and_array_index_pair : variable_and_array_indices_) {
    ...
    DetermineIndexSetsHelper(indep_variables_, false, &index_sets_.alpha, &index_sets_.alpha_prime);
    DetermineIndexSetsHelper(dep_variables_, false, &index_sets_.alpha_bar, &index_sets_.alpha_bar_prime);
    DetermineIndexSetsHelper(dep_variables_, true, &index_sets_.beta, &index_sets_.beta_prime);
    DetermineIndexSetsHelper(indep_variables_, true, &index_sets_.beta_bar, &index_sets_.beta_bar_prime);
}
...
template <class T>
bool UnrevisedLemkeSolver<T>::IsEachUnique(const std::vector<LCPVariable>& vars) {
    // Copy the set.
...
bool UnrevisedLemkeSolver<T>::LemkePivot(const MatrixX<T>& M,
    ...
    if (!IsArtificial(indep_variables_[driving_index])) DRAKE_DEMAND(M_prime_col != nullptr);
    ...
    SelectSubMatrixWithCovering(M, index_sets_.alpha, index_sets_.beta, &M_alpha_beta_);
    SelectSubMatrixWithCovering(M, index_sets_.alpha_bar, index_sets_.beta, &M_alpha_bar_beta_);
    ...
    //   identified.
    if ((M_alpha_beta_ * q_prime_beta_prime_ + q_alpha_).norm() > zero_tol) return false;

    // Equation (3) from [Dai 2018].
    q_prime_alpha_bar_prime_ = M_alpha_bar_beta_ * q_prime_beta_prime_ + q_alpha_bar_;
    ...
    if (!indep_variables_[driving_index].is_z()) {
        ...
        DRAKE_ASSERT(index_sets_.alpha[gamma] == indep_variables_[driving_index].index());
        ...
        // Equation (6).
        M_prime_driving_alpha_bar_prime_ = M_alpha_bar_beta_ * M_prime_driving_beta_prime_;
    } else {
        ...
        // Equation (8).
        M_prime_driving_alpha_bar_prime_ = g_alpha_bar_ + M_alpha_bar_beta_ * M_prime_driving_beta_prime_;
    }

    SetSubVector(M_prime_driving_beta_prime_, index_sets_.beta_prime, M_prime_col);
    SetSubVector(M_prime_driving_alpha_bar_prime_, index_sets_.alpha_bar_prime, M_prime_col);
...
template <class T>
int UnrevisedLemkeSolver<T>::FindComplementIndex(const LCPVariable& query) const {
...
bool UnrevisedLemkeSolver<T>::ConstructLemkeSolution(
        const MatrixX<T>& M, const VectorX<T>& q, int artificial_index, T zero_tol, VectorX<T>* z) const {
    ...
    VectorX<T> q_prime(n);
    if (!LemkePivot(M, q, artificial_index, zero_tol, nullptr, &q_prime)) return false;
...
template <typename T>
bool UnrevisedLemkeSolver<T>::IsSolution(const MatrixX<T>& M, const VectorX<T>& q, const VectorX<T>& z, T zero_tol) {
    using std::abs;
    ...
    const T dot = w.dot(z);
    const int n = q.size();
    return (min_z > -mod_zero_tol && min_w > -mod_zero_tol && abs(dot) < 10 * n * mod_zero_tol);
}

// Note: maintainers should read Section 4.4 - 4.4.5 of [Cottle 1992] to
// understand Lemke's Algorithm (and this function).
template <typename T>
bool UnrevisedLemkeSolver<T>::SolveLcpLemke(
        const MatrixX<T>& M, const VectorX<T>& q, VectorX<T>* z, int* num_pivots, const T& zero_tol) const {
    using std::abs;
    using std::max;
    ...
    if (M.rows() != n || M.cols() != n) throw std::logic_error("M's dimensions do not match that of q.");
    ...
    *num_pivots = 0;
    ...
    if (static_cast<size_t>(n) == dep_variables_.size()) {
        ...
        int zn_index = -1;
        for (int i = 0; i < static_cast<int>(indep_variables_.size()) && zn_index < 0; ++i) {
        ...
        if (zn_index >= 0) {
            ...
            if (ConstructLemkeSolution(M, q, zn_index, mod_zero_tol, z)) {
                ...
                DRAKE_LOGGER_DEBUG("Failed to solve linear system implied by last solution");
            }
        }
    }
    ...
    bool blocking_index_found = FindBlockingIndex(mod_zero_tol, q, q, &blocking_index);
    ...
    DRAKE_LOGGER_DEBUG("First blocking variable {}{}", ((blocking.is_z()) ? "z" : "w"), blocking.index());
    ...
    DRAKE_LOGGER_DEBUG("Independent set variables: {}", to_string(indep_variables_));
    ...
    while (++(*num_pivots) < max_pivots) {
        DRAKE_LOGGER_DEBUG("New driving variable {}{}", ((indep_variables_[driving_index].is_z()) ? "z" : "w"),
                           indep_variables_[driving_index].index());

        // Compute the permuted q and driving column of the permuted M matrix.
        if (!LemkePivot(M, q, driving_index, mod_zero_tol, &M_prime_col, &q_prime)) {
        ...
        if (!FindBlockingIndex(mod_zero_tol, M_prime_col, -(q_prime.array() / M_prime_col.array()).matrix(),
                               &blocking_index)) {
            z->setZero(n);
            return false;
        }
        blocking = dep_variables_[blocking_index];
        DRAKE_LOGGER_DEBUG("Blocking variable {}{}", ((blocking.is_z()) ? "z" : "w"), blocking.index());
        ...
        if (blocking.index() == kArtificial) {
            ...
            DRAKE_LOGGER_DEBUG("Linear system solver failed to construct Lemke solution");
            z->setZero(n);
            return false;
        }
        ...
        auto indep_variables_indices_iter = indep_variables_indices_.find(dep_variables_[blocking_index]);
...
template <typename T>
bool UnrevisedLemkeSolver<T>::ProgramAttributesSatisfied(const MathematicalProgram& prog) {
    ...
    // variables sit at 0.
    if (prog.required_capabilities() != ProgramAttributes({ProgramAttribute::kLinearComplementarityConstraint})) {
        return false;
    }
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ::drake::solvers::UnrevisedLemkeSolver);
