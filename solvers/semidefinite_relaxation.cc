...
std::unique_ptr<MathematicalProgram> InitializeRelaxation(const MathematicalProgram& prog, const Variable& one) {
...
MatrixXDecisionVariable DoMakeSemidefiniteRelaxation(const MathematicalProgram& sub_prog,
                                                     const Variable& one,
    ...
    internal::InitializeSemidefiniteRelaxationForProg(sub_prog, one, relaxation, &X, &variables_to_sorted_indices,
                                                      group_number);

    internal::DoLinearizeQuadraticCostsAndConstraints(sub_prog, X, variables_to_sorted_indices, relaxation,
                                                      options.preserve_convex_quadratic_constraints);

    if (options.add_implied_linear_constraints) {
        internal::DoAddImpliedLinearConstraints(sub_prog, X, variables_to_sorted_indices, relaxation);
    }
    if (options.add_implied_linear_equality_constraints) {
        internal::DoAddImpliedLinearEqualityConstraints(sub_prog, X, variables_to_sorted_indices, relaxation);
    }
    return X;
}
}  // namespace
...
std::unique_ptr<MathematicalProgram> MakeSemidefiniteRelaxation(const MathematicalProgram& prog,
    ...
    std::map<symbolic::Variables, solvers::MathematicalProgram> groups_to_container_programs;
    std::map<symbolic::Variables, MatrixXDecisionVariable> groups_to_psd_variables;
    ...
    for (const auto& [group, container_program] : groups_to_container_programs) {
        groups_to_psd_variables.emplace(
                group, DoMakeSemidefiniteRelaxation(container_program, one, options, relaxation.get(), group_number));
        ++group_number;
    }

    // Now constrain the semidefinite variables to agree where they overlap.
    for (auto it = groups_to_psd_variables.begin(); it != groups_to_psd_variables.end(); ++it) {
        for (auto it2 = std::next(it); it2 != groups_to_psd_variables.end(); ++it2) {
            const Variables common_variables = intersect(it->first, it2->first);
            if (!common_variables.empty()) {
                auto get_submatrix_of_variables = [&common_variables](const MatrixXDecisionVariable& X) {
                ...
                relaxation->AddLinearEqualityConstraint(get_submatrix_of_variables(it->second) ==
