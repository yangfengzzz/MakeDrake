...
double EvaluateCosts(const std::vector<double>& x, std::vector<double>& grad, void* f_data) {
    const MathematicalProgram* prog = reinterpret_cast<const MathematicalProgram*>(f_data);
        ...
        if (!grad.empty()) {
            if (ty(0).derivatives().size() > 0) {
                for (int j = 0; j < num_vars; ++j) {
                    const size_t vj_index = prog->FindDecisionVariableIndex(binding.variables()(j));
...
struct WrappedConstraint {
    WrappedConstraint(const Constraint* constraint_in,
        ...
        : constraint(constraint_in), vars(vars_in), prog(prog_in), force_bounds(false), force_upper(false) {}
...
double ApplyConstraintBounds(double result, double lb, double ub) {
    ...
    if ((ub != std::numeric_limits<double>::infinity()) && ((result >= lb) || (lb == ub))) {
        result -= ub;
    } else {
...
void EvaluateVectorConstraint(unsigned m, double* result, unsigned n, const double* x, double* grad, void* f_data) {
    const WrappedConstraint* wrapped = reinterpret_cast<WrappedConstraint*>(f_data);
    ...
    AutoDiffVecXd this_x = MakeInputAutoDiffVec(*(wrapped->prog), xvec, *(wrapped->vars));
    c->Eval(this_x, &ty);
    ...
    for (int i = 0; i < num_constraints; i++) {
        ...
        if (wrapped->force_bounds && wrapped->force_upper &&
            ...
            result[result_idx] =
                    ApplyConstraintBounds(ty(i).value(), -std::numeric_limits<double>::infinity(), upper_bound(i));
        } else if (wrapped->force_bounds && !wrapped->force_upper &&
                   (lower_bound(i) != -std::numeric_limits<double>::infinity())) {
            result[result_idx] =
                    ApplyConstraintBounds(ty(i).value(), lower_bound(i), std::numeric_limits<double>::infinity());
        } else {
            result[result_idx] = ApplyConstraintBounds(ty(i).value(), lower_bound(i), upper_bound(i));
        }
        result_idx++;
        ...
        for (int i = 0; i < num_constraints; i++) {
            ...
            if (ty(i).derivatives().size() > 0) {
                for (int j = 0; j < wrapped->vars->rows(); ++j) {
                    grad[(result_idx * n) + v_index[j]] = ty(i).derivatives()(v_index[j]) * grad_sign;
                }
            }
            result_idx++;
            DRAKE_ASSERT(result_idx <= m);
...
template <typename C>
void WrapConstraint(const MathematicalProgram& prog,
    ...
    WrappedConstraint wrapped_eq(binding.evaluator().get(), &binding.variables(), &prog);

    // Version of the wrapped constraint which refers only to inequality
    // constraints (if any), and will be used with
    // add_equality_mconstraint.
    WrappedConstraint wrapped_in(binding.evaluator().get(), &binding.variables(), &prog);
    ...
    if (wrapped_eq.active_constraints.size()) {
        ...
        std::vector<double> tol(wrapped_eq.active_constraints.size(), constraint_tol);
        opt->add_equality_mconstraint(EvaluateVectorConstraint, &wrapped_list->back(), tol);
    }

    if (wrapped_in.active_constraints.size()) {
        std::vector<double> tol(wrapped_in.active_constraints.size(), constraint_tol);
        wrapped_list->push_back(wrapped_in);
        if (is_pure_inequality) {
            opt->add_inequality_mconstraint(EvaluateVectorConstraint, &wrapped_list->back(), tol);
        } else {
...
bool IsVectorOfConstraintsSatisfiedAtSolution(const MathematicalProgram& prog,
                                              const std::vector<Binding>& bindings,
                                              const Eigen::Ref<const Eigen::VectorXd>& decision_variable_values,
                                              double tol) {
    for (const auto& binding : bindings) {
        const Eigen::VectorXd constraint_val = prog.EvalBinding(binding, decision_variable_values);
...
nlopt::algorithm GetNloptAlgorithm(const SolverOptions& merged_options) {
    ...
    if (it == options_str.end()) {
        ...
        for (int i = 0; i < nlopt::algorithm::NUM_ALGORITHMS; ++i) {
            if (requested_algorithm == nlopt_algorithm_to_string(static_cast<nlopt_algorithm>(i))) {
...
void NloptSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("NloptSolver doesn't support the feature of variable scaling.");
    }
    ...
    const double constraint_tol = GetOptionValueWithDefault(nlopt_options_double, ConstraintToleranceName(), 1e-6);
    const double xtol_rel = GetOptionValueWithDefault(nlopt_options_double, XRelativeToleranceName(), 1e-6);
    const double xtol_abs = GetOptionValueWithDefault(nlopt_options_double, XAbsoluteToleranceName(), 1e-6);
    const int max_eval = GetOptionValueWithDefault(nlopt_options_int, MaxEvalName(), 1000);
    ...
    NloptSolverDetails& solver_details = result->SetSolverDetailsType<NloptSolverDetails>();
    ...
    try {
        ...
        if (nlopt_result == nlopt::SUCCESS || nlopt_result == nlopt::STOPVAL_REACHED ||
            nlopt_result == nlopt::XTOL_REACHED || nlopt_result == nlopt::FTOL_REACHED ||
            nlopt_result == nlopt::MAXEVAL_REACHED || nlopt_result == nlopt::MAXTIME_REACHED) {
        ...
        switch (nlopt_result) {
            ...
            case nlopt::XTOL_REACHED: {
                ...
                auto constraint_test = [&prog, constraint_tol, &all_constraints_satisfied, result](auto constraints) {
