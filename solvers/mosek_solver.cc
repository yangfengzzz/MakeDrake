...
void MosekSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("MosekSolver doesn't support the feature of variable scaling.");
    }
    ...
    const int num_nonmatrix_vars_in_prog = impl.decision_variable_to_mosek_nonmatrix_variable().size();
    ...
    if (rescode == MSK_RES_OK) {
        rescode = impl.UpdateOptions(merged_options, id(), &print_to_console, &print_file_name, &msk_writedata);
    }
    ...
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddPositiveSemidefiniteConstraints(prog, &psd_barvar_indices);
    }
    // Add the constraint that Mosek matrix variable entries corresponding to the
    // same decision variables should all be equal.
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddEqualityConstraintBetweenMatrixVariablesForSameDecisionVariable();  // NOLINT
    }
    // Add costs
    if (rescode == MSK_RES_OK) {
    ...
    std::unordered_map<Binding<BoundingBoxConstraint>,
                       std::pair<internal::ConstraintDualIndices, internal::ConstraintDualIndices>>
            bb_con_dual_indices;
    ...
    std::unordered_map<Binding<LinearConstraint>, internal::ConstraintDualIndices> linear_con_dual_indices;
    std::unordered_map<Binding<LinearEqualityConstraint>, internal::ConstraintDualIndices> lin_eq_con_dual_indices;
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddLinearConstraints(prog, &linear_con_dual_indices, &lin_eq_con_dual_indices);
    }

    // Add Lorentz cone constraints.
    std::unordered_map<Binding<LorentzConeConstraint>, MSKint64t> lorentz_cone_acc_indices;
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddConeConstraints(prog, prog.lorentz_cone_constraints(), &lorentz_cone_acc_indices);
    }

    // Add rotated Lorentz cone constraints.
    std::unordered_map<Binding<RotatedLorentzConeConstraint>, MSKint64t> rotated_lorentz_cone_acc_indices;
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddConeConstraints(prog, prog.rotated_lorentz_cone_constraints(),
    ...
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddQuadraticConstraints(prog, &quadratic_constraint_dual_indices);
    }
    ...
    std::unordered_map<Binding<ExponentialConeConstraint>, MSKint64t> exp_cone_acc_indices;
    if (rescode == MSK_RES_OK) {
        rescode = impl.AddConeConstraints(prog, prog.exponential_cone_constraints(), &exp_cone_acc_indices);
    }
    ...
    } else if (prog.quadratic_costs().empty() && prog.l2norm_costs().empty() && prog.quadratic_constraints().empty() &&
               prog.lorentz_cone_constraints().empty() && prog.rotated_lorentz_cone_constraints().empty() &&
    ...
    if (initial_guess.array().isFinite().any()) {
        ...
        for (int i = 0; i < prog.num_vars(); ++i) {
            const auto& map_to_mosek = impl.decision_variable_to_mosek_nonmatrix_variable();
            if (auto it = map_to_mosek.find(i); it != map_to_mosek.end()) {
                if (rescode == MSK_RES_OK) {
                    ...
                    if (!std::isnan(initial_guess_i)) {
                        rescode = MSK_putxxslice(impl.task(), solution_type, it->second, it->second + 1,
                                                 &initial_guess_i);
                    }
                }
            }
    ...
    if (rescode == MSK_RES_OK) {
        ...
        for (int i = 0; i < num_bar_x; ++i) {
            ...
            rescode = MSK_getbarxj(impl.task(), solution_type, i, mosek_bar_x_sol[i].data());
        }
        DRAKE_ASSERT(rescode == MSK_RES_OK);
        ...
        for (int i = 0; i < num_decision_vars; ++i) {
            ...
            if (it1 != impl.decision_variable_to_mosek_nonmatrix_variable().end()) {
                ...
                sol_vector(i) = mosek_bar_x_sol[it2->second.bar_matrix_index()](it2->second.IndexInLowerTrianglePart());
            }
        }
        if (rescode == MSK_RES_OK) {
        ...
        rescode = impl.SetDualSolution(solution_type, prog, bb_con_dual_indices, linear_con_dual_indices,
                                       lin_eq_con_dual_indices, quadratic_constraint_dual_indices,
                                       lorentz_cone_acc_indices, rotated_lorentz_cone_acc_indices, exp_cone_acc_indices,
                                       psd_barvar_indices, result);
        DRAKE_ASSERT(rescode == MSK_RES_OK);
    }

    MosekSolverDetails& solver_details = result->SetSolverDetailsType<MosekSolverDetails>();
    solver_details.rescode = rescode;
    solver_details.solution_status = solution_status;
    if (rescode == MSK_RES_OK) {
        rescode = MSK_getdouinf(impl.task(), MSK_DINF_OPTIMIZER_TIME, &(solver_details.optimizer_time));
    }
