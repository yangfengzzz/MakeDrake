...
bool SatisfiesProgramType(const Requirements& requirements, const ProgramAttributes& program_attributes) {
...
const Requirements& GetRequirementsLP() {
    static const drake::never_destroyed<Requirements> requirements{[]() {
        ...
        return Requirements{.acceptable = attributes, .must_include = {}, .must_include_one_of = attributes};
    }()};
...
const Requirements& GetRequirementsGP() {
    static const drake::never_destroyed<Requirements> requirements{
            Requirements{.acceptable = {ProgramAttribute::kLinearCost, ProgramAttribute::kExponentialConeConstraint},
...
const Requirements& GetRequirementsQuadraticCostConicProgram() {
    static const drake::never_destroyed<Requirements> requirements{[]() {
        ...
        return Requirements{.acceptable = acceptable,
                            .must_include = {ProgramAttribute::kQuadraticCost},
                            .must_include_one_of = {// All nonlinear convex conic constraints.
...
const Requirements& GetRequirementsLCP() {
    static const drake::never_destroyed<Requirements> requirements{
            Requirements{.acceptable = {ProgramAttribute::kLinearComplementarityConstraint},
...
Requirements MixedIntegerRequirements(const Requirements& continuous_requirements) {
...
bool AllQuadraticCostsConvex(const std::vector<Binding<QuadraticCost>>& quadratic_costs) {
...
bool IsNLP(const MathematicalProgram& prog) {
    ...
    const bool has_generic_cost = prog.required_capabilities().contains(ProgramAttribute::kGenericCost);
    const bool has_nonconvex_quadratic_cost = !AllQuadraticCostsConvex(prog.quadratic_costs());
    const bool has_generic_constraint = prog.required_capabilities().contains(ProgramAttribute::kGenericConstraint);
    const bool has_quadratic_constraint =
            prog.required_capabilities().count(ProgramAttribute::kQuadraticConstraint) > 0;
    const bool no_binary_variable = prog.required_capabilities().count(ProgramAttribute::kBinaryVariable) == 0;
    const bool has_linear_complementarity_constraint =
            prog.required_capabilities().count(ProgramAttribute::kLinearComplementarityConstraint) > 0;
    const bool is_LCP = SatisfiesProgramType(GetRequirementsLCP(), prog.required_capabilities());
    return no_binary_variable && !is_LCP &&
           (has_generic_cost || has_nonconvex_quadratic_cost || has_generic_constraint || has_quadratic_constraint ||
...
ProgramType GetProgramType(const MathematicalProgram& prog) {
    ...
    } else if (SatisfiesProgramType(GetRequirementsQP(), prog.required_capabilities()) &&
               AllQuadraticCostsConvex(prog.quadratic_costs())) {
        return ProgramType::kQP;
    } else if (SatisfiesProgramType(GetRequirementsSOCP(), prog.required_capabilities())) {
        return ProgramType::kSOCP;
    } else if (SatisfiesProgramType(GetRequirementsSDP(), prog.required_capabilities())) {
        return ProgramType::kSDP;
    } else if (SatisfiesProgramType(GetRequirementsGP(), prog.required_capabilities())) {
        // TODO(hongkai.dai): support more general type of geometric programming,
        // with constraints on posynomials.
        return ProgramType::kGP;
    } else if (SatisfiesProgramType(GetRequirementsCGP(), prog.required_capabilities())) {
        return ProgramType::kCGP;
    } else if (SatisfiesProgramType(GetRequirementsMILP(), prog.required_capabilities())) {
        return ProgramType::kMILP;
    } else if (SatisfiesProgramType(GetRequirementsMIQP(), prog.required_capabilities()) &&
               AllQuadraticCostsConvex(prog.quadratic_costs())) {
        return ProgramType::kMIQP;
    } else if (SatisfiesProgramType(GetRequirementsMISOCP(), prog.required_capabilities())) {
        return ProgramType::kMISOCP;
    } else if (SatisfiesProgramType(GetRequirementsMISDP(), prog.required_capabilities())) {
        return ProgramType::kMISDP;
    } else if (SatisfiesProgramType(GetRequirementsQuadraticCostConicProgram(), prog.required_capabilities()) &&
               AllQuadraticCostsConvex(prog.quadratic_costs())) {
        return ProgramType::kQuadraticCostConicConstraint;
    } else if (SatisfiesProgramType(GetRequirementsLCP(), prog.required_capabilities())) {
