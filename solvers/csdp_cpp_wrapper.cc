...
#include "solvers/csdp_solver_error_handling.h"
...
#define DRAKE_CSDP_SETJMP()                                                                        \
    do {                                                                                           \
        ...
        if (setjmp(env) > 0) {                                                                     \
            throw std::runtime_error("CsdpSolver: the CSDP library exited via a fatal exception"); \
        }                                                                                          \
    } while (0)
...
int cpp_easy_sdp(const char* params_pathname,
                 int n,
                 int k,
                 ...
                 double* pdobj) {
    DRAKE_CSDP_SETJMP();
    return easy_sdp(params_pathname, n, k, C, a, constraints, constant_offset, pX, py, pZ, ppobj, pdobj);
}
