...
void AggregateQuadraticCostsHelper(const std::vector<Binding<QuadraticCost>>& quadratic_costs,
    ...
    for (const auto& quadratic_cost : quadratic_costs) {
        ...
        for (int i = 0; i < num_cost_var; ++i) {
            if (quadratic_cost.evaluator()->b()(i) != 0) {
                ...
                linear_coeff_triplets->emplace_back(linear_var_index, 0, quadratic_cost.evaluator()->b()(i));
            }
            for (int j = 0; j < num_cost_var; ++j) {
                if (quadratic_cost.evaluator()->Q()(i, j) != 0) {
                    if (cost_quadratic_var_indices[i] >= cost_quadratic_var_indices[j]) {
                        Q_lower_triplets.emplace_back(cost_quadratic_var_indices[i], cost_quadratic_var_indices[j],
    ...
    *Q_lower = Eigen::SparseMatrix<double>(quadratic_var_vec->size(), quadratic_var_vec->size());
...
void AggregateLinearCostsHelper(const std::vector<Binding<LinearCost>>& linear_costs,
                                std::vector<Eigen::Triplet<double>>* linear_coeff_triplets,
                                VariableVector* var_vec,
                                double* constant_cost) {
    for (const auto& cost : linear_costs) {
        const Eigen::SparseVector<double> cost_coeff = cost.evaluator()->a().sparseView();
...
void AggregateLinearCosts(const std::vector<Binding<LinearCost>>& linear_costs,
    ...
    VariableVector var_vec{};
    *constant_cost = 0;
    AggregateLinearCostsHelper(linear_costs, &linear_coeff_triplets, &var_vec, constant_cost);
...
void AggregateQuadraticAndLinearCosts(const std::vector<Binding<QuadraticCost>>& quadratic_costs,
    ...
    VariableVector linear_var_vec{};
    *constant_cost = 0;
    AggregateQuadraticCostsHelper(quadratic_costs, Q_lower, &quadratic_var_vec, &linear_coeff_triplets, &linear_var_vec,
                                  constant_cost);
    AggregateLinearCostsHelper(linear_costs, &linear_coeff_triplets, &linear_var_vec, constant_cost);
...
std::unordered_map<symbolic::Variable, Bound> AggregateBoundingBoxConstraints(
    ...
    for (const auto& constraint : bounding_box_constraints) {
        for (int i = 0; i < constraint.variables().rows(); ++i) {
            ...
            if (it == bounds.end()) {
                bounds.emplace_hint(it, var, Bound{.lower = var_lower, .upper = var_upper});
            } else {
...
namespace {
template <typename C>
void AggregateBoundingBoxConstraintsImpl(const MathematicalProgram& prog, C* lower, C* upper) {
    for (const auto& constraint : prog.bounding_box_constraints()) {
        for (int i = 0; i < constraint.variables().rows(); ++i) {
            const int var_index = prog.FindDecisionVariableIndex(constraint.variables()(i));
...
const Binding<QuadraticCost>* FindNonconvexQuadraticCost(const std::vector<Binding<QuadraticCost>>& quadratic_costs) {
...
bool CheckConvexSolverAttributes(const MathematicalProgram& prog,
    ...
    const bool capabilities_match =
            AreRequiredAttributesSupported(required_capabilities, solver_capabilities, explanation);
    if (!capabilities_match) {
        if (explanation) {
            *explanation = fmt::format("{} is unable to solve because {}.", solver_name, *explanation);
        }
        return false;
    }
...
void ParseLinearEqualityConstraints(const MathematicalProgram& prog,
    ...
    *num_linear_equality_constraints_rows = 0;
    ...
    for (const auto& linear_equality_constraint : prog.linear_equality_constraints()) {
        const Eigen::SparseMatrix<double>& Ai = linear_equality_constraint.evaluator()->get_sparse_A();
        ...
        const int num_Ai_rows = linear_equality_constraint.evaluator()->num_constraints();
...
void ParseLinearConstraints(const solvers::MathematicalProgram& prog,
    ...
    *num_linear_constraint_rows = 0;
    ...
    for (const auto& linear_constraint : prog.linear_constraints()) {
        linear_constraint_dual_indices->emplace_back(linear_constraint.evaluator()->num_constraints());
        ...
        const Eigen::SparseMatrix<double>& Ai = linear_constraint.evaluator()->get_sparse_A();
        ...
        std::vector<int> starting_row_indices(linear_constraint.evaluator()->num_constraints());
        ...
        for (int j = 0; j < cost.evaluator()->Q().cols(); ++j) {
            for (int i = 0; i <= j; ++i) {
                if (cost.evaluator()->Q()(i, j) != 0) {
                    ...
                    const double factor = (i != j && var_indices[i] == var_indices[j]) ? 2 : 1;
                    ...
                    P_upper_triplets->emplace_back(row_index, col_index, factor * cost.evaluator()->Q()(i, j));
                }
            }
    ...
    for (const auto& cost : prog.l2norm_costs()) {
        ...
        for (int k = 0; k < C.outerSize(); ++k) {
            ...
            for (Eigen::SparseMatrix<double>::InnerIterator it(C, k); it; ++it) {
                A_triplets->emplace_back(*A_row_count + 1 + it.row(), x_index, -it.value());
            }
        }
...
void ParseSecondOrderConeConstraints(const MathematicalProgram& prog,
    ...
    second_order_cone_length->reserve(prog.lorentz_cone_constraints().size() +
                                      prog.rotated_lorentz_cone_constraints().size());
    lorentz_cone_y_start_indices->reserve(prog.lorentz_cone_constraints().size());
    rotated_lorentz_cone_y_start_indices->reserve(prog.rotated_lorentz_cone_constraints().size());
    for (const auto& lorentz_cone_constraint : prog.lorentz_cone_constraints()) {
        ...
        const std::vector<int> x_indices = prog.FindDecisionVariableIndices(x);
        const Eigen::SparseMatrix<double>& Ai = lorentz_cone_constraint.evaluator()->A();
        const std::vector<Eigen::Triplet<double>> Ai_triplets = math::SparseMatrixToTriplets(Ai);
        for (const auto& Ai_triplet : Ai_triplets) {
            A_triplets->emplace_back(Ai_triplet.row() + *A_row_count, x_indices[Ai_triplet.col()], -Ai_triplet.value());
        }
    ...
    for (const auto& rotated_lorentz_cone : prog.rotated_lorentz_cone_constraints()) {
        ...
        const std::vector<int> x_indices = prog.FindDecisionVariableIndices(x);
        const Eigen::SparseMatrix<double> Ai = rotated_lorentz_cone.evaluator()->A();
        const Eigen::VectorXd& bi = rotated_lorentz_cone.evaluator()->b();
        const std::vector<Eigen::Triplet<double>> Ai_triplets = math::SparseMatrixToTriplets(Ai);
        ParseRotatedLorentzConeConstraint(Ai_triplets, bi, x_indices, A_triplets, b, A_row_count,
                                          second_order_cone_length, rotated_lorentz_cone_y_start_indices);
    }
}
...
void ParseExponentialConeConstraints(const MathematicalProgram& prog,
    ...
    for (const auto& binding : prog.exponential_cone_constraints()) {
        ...
        for (int i = 0; i < num_bound_variables; ++i) {
            A_triplets->reserve(A_triplets->size() + binding.evaluator()->A().nonZeros());
            const int decision_variable_index = prog.FindDecisionVariableIndex(binding.variables()(i));
            for (Eigen::SparseMatrix<double>::InnerIterator it(binding.evaluator()->A(), i); it; ++it) {
                // 2 - it.row() is used for reverse the row order, as mentioned in the
                // function documentation above.
                A_triplets->emplace_back(*A_row_count + 2 - it.row(), decision_variable_index, -it.value());
            }
        }
    ...
    for (const auto& psd_constraint : prog.positive_semidefinite_constraints()) {
        ...
        for (int j = 0; j < X_rows; ++j) {
            ...
            for (int i = i_start; i < i_end; ++i) {
                ...
                A_triplets->emplace_back(*A_row_count + x_index_count,
                                         prog.FindDecisionVariableIndex(flat_X(j * X_rows + i)), -scale_factor);
                b->push_back(0);
                ++x_index_count;
            }
        }
        ...
        for (int j = 0; j < F_rows; ++j) {
            ...
            for (int i = i_start; i < i_end; ++i) {
                ...
                for (int k = 1; k < static_cast<int>(F.size()); ++k) {
                    A_triplets->emplace_back(*A_row_count + A_cone_row_count, x_indices[k - 1],
