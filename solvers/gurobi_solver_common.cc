...
bool CheckAttributes(const MathematicalProgram& prog, std::string* explanation) {
    ...
    static const never_destroyed<ProgramAttributes> solver_capabilities(std::initializer_list<ProgramAttribute>{
            ...
            ProgramAttribute::kLinearConstraint, ProgramAttribute::kLinearEqualityConstraint,
            ProgramAttribute::kLorentzConeConstraint, ProgramAttribute::kRotatedLorentzConeConstraint,
            ProgramAttribute::kBinaryVariable});
    return internal::CheckConvexSolverAttributes(prog, solver_capabilities.access(), "GurobiSolver", explanation);
}
}  // namespace
...
std::string GurobiSolver::UnsatisfiedProgramAttributes(const MathematicalProgram& prog) {
