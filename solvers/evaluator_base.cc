...
std::ostream& EvaluatorBase::Display(std::ostream& os, const VectorX<symbolic::Variable>& vars) const {
...
std::ostream& EvaluatorBase::Display(std::ostream& os) const {
    ...
    return this->DoDisplay(os, symbolic::MakeVectorContinuousVariable(this->num_vars(), "$"));
}

std::ostream& EvaluatorBase::DoDisplay(std::ostream& os, const VectorX<symbolic::Variable>& vars) const {
...
std::string EvaluatorBase::ToLatex(const VectorX<symbolic::Variable>& vars, int precision) const {
    const int num_vars = this->num_vars();
    DRAKE_THROW_UNLESS(vars.rows() == num_vars || num_vars == Eigen::Dynamic);
    std::string tag = "";
...
std::string EvaluatorBase::DoToLatex(const VectorX<symbolic::Variable>& vars, int) const {
...
void CheckGradientSparsityPattern(const std::vector<std::pair<int, int>>& gradient_sparsity_pattern,
                                  int rows,
                                  int cols) {
    ...
    for (const auto& nonzero_entry : gradient_sparsity_pattern) {
        if (nonzero_entry.first < 0 || nonzero_entry.first >= rows) {
            throw std::invalid_argument("Constraint::SetSparsityPattern(): row index out of range.");
        }
        if (nonzero_entry.second < 0 || nonzero_entry.second >= cols) {
            throw std::invalid_argument("Constraint::SetSparsityPattern(): column index out of range.");
        }
...
void EvaluatorBase::SetGradientSparsityPattern(const std::vector<std::pair<int, int>>& gradient_sparsity_pattern) {
    if (kDrakeAssertIsArmed) {
        CheckGradientSparsityPattern(gradient_sparsity_pattern, num_outputs(), num_vars());
    }
...
void PolynomialEvaluator::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
...
void PolynomialEvaluator::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
