...
#include "solvers/integer_inequality_solver.h"
namespace drake {
namespace solvers {
namespace {
...
MonomialVector ExponentsToMonomials(const ExponentList& exponents, const drake::VectorX<Variable>& vars) {
...
ExponentList GetPolynomialExponents(const drake::symbolic::Polynomial& p) {
    ...
    ExponentList exponents(p.monomial_to_coefficient_map().size(), indeterminates.size());
    int row = 0;
...
void Intersection(const ExponentList& A, ExponentList* B) {
    ...
    for (int i = 0; i < B->rows(); i++) {
        if ((ContainsExponent(A, A.rows(), B->row(i))) && !(ContainsExponent(*B, index, B->row(i)))) {
            B->row(index++) = B->row(i);
        }
    }
 ...
 */
void RemoveDiagonallyInconsistentExponents(const ExponentList& exponents_of_p, ExponentList* exponents_of_basis) {
    while (1) {
        int num_exponents = exponents_of_basis->rows();

        ExponentList valid_squares = VerticalStack(PairwiseSums(*exponents_of_basis), exponents_of_p);
...
Hyperplanes RandomSupportingHyperplanes(const ExponentList& exponents_of_p, unsigned int seed) {
    ...
    std::uniform_int_distribution<int> distribution(normal_vector_component_min, normal_vector_component_max);
    auto get_random = [&]() {
...
ExponentList EnumerateInitialSet(const ExponentList& exponents_of_p) {
    ...
    ExponentList basis_exponents_1 = drake::solvers::EnumerateIntegerSolutions(
            hyperplanes.normal_vectors, hyperplanes.max_dot_product, lower_bounds, upper_bounds);

    ExponentList basis_exponents = drake::solvers::EnumerateIntegerSolutions(
            -hyperplanes.normal_vectors, -hyperplanes.min_dot_product, lower_bounds, upper_bounds);
...
void RemoveWithRandomSeparatingHyperplanes(const ExponentList& exponents_of_p, ExponentList* basis) {
    ...
    while (1) {
        ...
        for (int i = 0; i < current_basis_size; i++) {
            ...
            for (int j = 0; j < dot_products.cols(); j++) {
                if (dot_products(i, j) > H.max_dot_product(j) || dot_products(i, j) < H.min_dot_product(j)) {
                    keep_monomial = false;
                    break;
                }
            }
        ...
        if (next_basis_size > current_basis_size * (1.0 - kMinimumPercentReduction) || next_basis_size == 0) {
            break;
        }
    }
    return;
