...
void AddSos2Constraint(MathematicalProgram* prog,
    ...
    prog->AddLinearConstraint(lambda.tail<1>()(0) >= 0 && lambda.tail<1>()(0) <= y.tail<1>()(0));
...
void AddLogarithmicSos1Constraint(MathematicalProgram* prog,
    ...
    DRAKE_DEMAND(binary_encoding.rows() == num_lambda && binary_encoding.cols() == num_y);
    ...
    for (int j = 0; j < num_y; ++j) {
        ...
        for (int k = 0; k < num_lambda; ++k) {
            if (binary_encoding(k, j) == 1) {
                ...
                throw std::runtime_error("The binary_encoding entry can be only 0 or 1.");
            }
        }
        prog->AddLinearConstraint(lambda_sum1 <= y(j));
        prog->AddLinearConstraint(lambda_sum2 <= 1 - y(j));
...
std::pair<VectorX<symbolic::Variable>, VectorX<symbolic::Variable>> AddLogarithmicSos1Constraint(
    ...
    const int num_y = CeilLog2(num_lambda);
    const Eigen::MatrixXi binary_encoding = math::CalculateReflectedGrayCodes(num_y).topRows(num_lambda);
    auto lambda = prog->NewContinuousVariables(num_lambda);
    auto y = prog->NewBinaryVariables(num_y);
    AddLogarithmicSos1Constraint(prog, lambda.cast<symbolic::Expression>(), y, binary_encoding);
...
void AddBilinearProductMcCormickEnvelopeMultipleChoice(MathematicalProgram* prog,
    ...
    const auto Bxy = prog->NewContinuousVariables(m, n, x.get_name() + "_" + y.get_name() + "_Bxy");
    prog->AddLinearEqualityConstraint(Bxy.cast<Expression>().rowwise().sum().sum(), 1);

    for (int i = 0; i < m; ++i) {
        prog->AddLinearConstraint(y_bar(i) >= phi_y.head(n).dot(Bxy.row(i).transpose().cast<Expression>()));
        prog->AddLinearConstraint(y_bar(i) <= phi_y.tail(n).dot(Bxy.row(i).transpose().cast<Expression>()));
    }
    for (int j = 0; j < n; ++j) {
        prog->AddLinearConstraint(x_bar(j) >= phi_x.head(m).dot(Bxy.col(j).cast<Expression>()));
    ...
    w_constraint_rhs(0) = x_bar.cast<Expression>().dot(phi_y.head(n)) + y_bar.cast<Expression>().dot(phi_x.head(m));
    w_constraint_rhs(1) = x_bar.cast<Expression>().dot(phi_y.tail(n)) + y_bar.cast<Expression>().dot(phi_x.tail(m));
    w_constraint_rhs(2) = x_bar.cast<Expression>().dot(phi_y.head(n)) + y_bar.cast<Expression>().dot(phi_x.tail(m));
    w_constraint_rhs(3) = x_bar.cast<Expression>().dot(phi_y.tail(n)) + y_bar.cast<Expression>().dot(phi_x.head(m));
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            prog->AddConstraint(
