...
#include "solvers/sparse_and_dense_matrix.h"
...
public:
    ...
    Constraint(int num_constraints,
        ...
        : EvaluatorBase(num_constraints, num_vars, description), lower_bound_(lb), upper_bound_(ub) {
    ...
    Constraint(int num_constraints, int num_vars)
        : Constraint(num_constraints,
                     num_vars,
                     Eigen::VectorXd::Constant(num_constraints, -std::numeric_limits<double>::infinity()),
                     Eigen::VectorXd::Constant(num_constraints, std::numeric_limits<double>::infinity())) {}
...
protected:
    ...
    virtual bool DoCheckSatisfied(const Eigen::Ref<const Eigen::VectorXd>& x, const double tol) const {
        ...
        DoEval(x, &y);
        return (y.array() >= lower_bound_.array() - tol).all() && (y.array() <= upper_bound_.array() + tol).all();
    }
    ...
    virtual bool DoCheckSatisfied(const Eigen::Ref<const AutoDiffVecXd>& x, const double tol) const {
        ...
        return (y.array().unaryExpr(get_value) >= lower_bound_.array() - tol).all() &&
...
public:
    ...
    QuadraticConstraint(const Eigen::MatrixBase<DerivedQ>& Q0,
        ...
        : Constraint(kNumConstraints, Q0.rows(), drake::Vector1d::Constant(lb), drake::Vector1d::Constant(ub)),
...
public:
    ...
    EvaluatorConstraint(const std::shared_ptr<EvaluatorType>& evaluator, Args&&... args)
        : Constraint(evaluator->num_outputs(), evaluator->num_vars(), std::forward<Args>(args)...),
...
private:
    ...
    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const override { evaluator_->Eval(x, y); }
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>& x,
                VectorX<symbolic::Expression>* y) const override {
        evaluator_->Eval(x, y);
    }
...
public:
    ...
    PolynomialConstraint(const VectorXPoly& polynomials,
        ...
        : EvaluatorConstraint(std::make_shared<PolynomialEvaluator>(polynomials, poly_vars), lb, ub) {}
...
private:
    ...
    void UpdateCoefficients(const Eigen::MatrixBase<DerivedA>&,
        ...
        static_assert(!std::is_same_v<DerivedA, DerivedA>,
...
public:
    ...
    explicit PositiveSemidefiniteConstraint(int rows)
        : Constraint(rows,
                     ...
                     Eigen::VectorXd::Constant(rows, std::numeric_limits<double>::infinity())),
          matrix_rows_(rows) {}
...
public:
    ...
    const VectorX<symbolic::Expression>& expressions() const { return expressions_; }
