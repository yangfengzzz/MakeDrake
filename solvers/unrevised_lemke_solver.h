...
public:
    ...
    static U ComputeZeroTolerance(const MatrixX<U>& M) {
        return M.rows() * M.template lpNorm<Eigen::Infinity>() * (2 * std::numeric_limits<double>::epsilon());
    }
    ...
    bool SolveLcpLemke(
            const MatrixX<T>& M, const VectorX<T>& q, VectorX<T>* z, int* num_pivots, const T& zero_tol = T(-1)) const;
...
private:
    ...
    static void SelectSubColumnWithCovering(const MatrixX<T>& in,
                                            ...
                                            int column,
                                            VectorX<T>* out);
    static void SelectSubVector(const VectorX<T>& in, const std::vector<int>& rows, VectorX<T>* out);
    static void SetSubVector(const VectorX<T>& v_sub, const std::vector<int>& indices, VectorX<T>* v);
    static bool ValidateIndices(const std::vector<int>& row_indices,
                                const std::vector<int>& col_indices,
                                int num_rows,
                                int num_cols);
    static bool ValidateIndices(const std::vector<int>& row_indices, int vector_size);
    static bool IsEachUnique(const std::vector<LCPVariable>& vars);
    bool LemkePivot(const MatrixX<T>& M,
                    const VectorX<T>& q,
                    int driving_index,
                    T zero_tol,
                    VectorX<T>* M_bar_col,
                    VectorX<T>* q_bar) const;
    bool ConstructLemkeSolution(
    ...
    bool FindBlockingIndex(const T& zero_tol,
    ...
    public:
        ...
        bool operator()(const LCPVariableVector& v1, const LCPVariableVector& v2) const {
    ...
    mutable std::map<LCPVariableVector, int, LCPVariableVectorComparator> selections_;
    ...
    mutable VectorX<T> q_alpha_, q_alpha_bar_, q_prime_beta_prime_, q_prime_alpha_bar_prime_, e_,
            M_prime_driving_beta_prime_, M_prime_driving_alpha_bar_prime_, g_alpha_, g_alpha_bar_;
