...
void SetCsdpSolverDetails(int csdp_ret,
                          double pobj,
                          double dobj,
                          int y_size,
                          double* y,
                          const csdp::blockmatrix& Z,
...
void SetProgramSolution(const SdpaFreeFormat& sdpa_free_format,
    ...
    for (int i = 0; i < static_cast<int>(sdpa_free_format.prog_var_in_sdpa().size()); ++i) {
        if (std::holds_alternative<DecisionVariableInSdpaX>(sdpa_free_format.prog_var_in_sdpa()[i])) {
            const auto& decision_var_in_X = std::get<DecisionVariableInSdpaX>(sdpa_free_format.prog_var_in_sdpa()[i]);

            double X_entry_val{};
            switch (X.blocks[decision_var_in_X.entry_in_X.block_index + 1].blockcategory) {
                case csdp::MATRIX: {
                    X_entry_val = X.blocks[decision_var_in_X.entry_in_X.block_index + 1].data.mat[CsdpMatrixIndex(
                            decision_var_in_X.entry_in_X.row_index_in_block,
                            decision_var_in_X.entry_in_X.column_index_in_block,
                            X.blocks[decision_var_in_X.entry_in_X.block_index + 1].blocksize)];
                    break;
                }
                case csdp::DIAG: {
                    X_entry_val = X.blocks[decision_var_in_X.entry_in_X.block_index + 1]
                                          .data.vec[decision_var_in_X.entry_in_X.row_index_in_block + 1];
                    break;
                }
                default: {
                    throw std::runtime_error("SetProgramSolution(): unknown X block type.");
                }
            }
            (*prog_sol)(i) = decision_var_in_X.offset +
                             (decision_var_in_X.coeff_sign == internal::Sign::kPositive ? X_entry_val : -X_entry_val);
        } else if (std::holds_alternative<double>(sdpa_free_format.prog_var_in_sdpa()[i])) {
            (*prog_sol)(i) = std::get<double>(sdpa_free_format.prog_var_in_sdpa()[i]);
        } else if (std::holds_alternative<SdpaFreeFormat::FreeVariableIndex>(sdpa_free_format.prog_var_in_sdpa()[i])) {
            (*prog_sol)(i) =
                    s_val(int{std::get<SdpaFreeFormat::FreeVariableIndex>(sdpa_free_format.prog_var_in_sdpa()[i])});
        }
    }
}
...
void SolveProgramWithNoFreeVariables(const MathematicalProgram& prog,
    ...
    GenerateCsdpProblemDataWithoutFreeVariables(sdpa_free_format, &C, &rhs, &constraints);

    struct csdp::blockmatrix X, Z;
    double* y;
    csdp::cpp_initsoln(sdpa_free_format.num_X_rows(), sdpa_free_format.g().rows(), C, rhs, constraints, &X, &y, &Z);
    double pobj, dobj;
    const int ret = csdp::cpp_easy_sdp(csdp_params_pathname.c_str(), sdpa_free_format.num_X_rows(),
                                       sdpa_free_format.g().rows(), C, rhs, constraints,
                                       -sdpa_free_format.constant_min_cost_term(), &X, &y, &Z, &pobj, &dobj);

    // Set solver details.
    CsdpSolverDetails& solver_details = result->SetSolverDetailsType<CsdpSolverDetails>();
    SetCsdpSolverDetails(ret, pobj, dobj, sdpa_free_format.g().rows(), y, Z, &solver_details);
    ...
    result->set_optimal_cost(ret == 1 /* primal infeasible */ ? MathematicalProgram::kGlobalInfeasibleCost : -pobj);
    ...
    csdp::cpp_free_prob(sdpa_free_format.num_X_rows(), sdpa_free_format.g().rows(), C, rhs, constraints, X, y, Z);
}

void SolveProgramThroughNullspaceApproach(const MathematicalProgram& prog,
                                          const SdpaFreeFormat& sdpa_free_format,
                                          const std::string& csdp_params_pathname,
                                          MathematicalProgramResult* result) {
    ...
    ConvertSparseMatrixFormatToCsdpProblemData(sdpa_free_format.X_blocks(), C_hat, A_hat, rhs_hat, &C_csdp, &rhs_csdp,
                                               &constraints_csdp);
    struct csdp::blockmatrix X_csdp, Z;
    double* y{nullptr};
    csdp::cpp_initsoln(sdpa_free_format.num_X_rows(), rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, &X_csdp, &y,
                       &Z);
    double pobj{0};
    double dobj{0};
    const int ret = csdp::cpp_easy_sdp(csdp_params_pathname.c_str(), sdpa_free_format.num_X_rows(), rhs_hat.rows(),
                                       C_csdp, rhs_csdp, constraints_csdp,
                                       -sdpa_free_format.constant_min_cost_term() + sdpa_free_format.g().dot(y_hat),
                                       &X_csdp, &y, &Z, &pobj, &dobj);
    Eigen::SparseMatrix<double> X_hat(sdpa_free_format.num_X_rows(), sdpa_free_format.num_X_rows());
    ...
    csdp::cpp_free_prob(sdpa_free_format.num_X_rows(), rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, X_csdp, y,
                        Z);
}
 ...
 */
void SolveProgramThroughTwoSlackVariablesApproach(const MathematicalProgram& prog,
                                                  const SdpaFreeFormat& sdpa_free_format,
                                                  const std::string& csdp_params_pathname,
                                                  MathematicalProgramResult* result) {
    static const logging::Warn log_once(
    ...
    const int num_X_hat_rows = sdpa_free_format.num_X_rows() + 2 * sdpa_free_format.num_free_variables();

    // Now try to call CSDP to solve this problem.
    csdp::blockmatrix C_csdp;
    double* rhs_csdp{nullptr};
    csdp::constraintmatrix* constraints_csdp{nullptr};
    ConvertSparseMatrixFormatToCsdpProblemData(X_hat_blocks, C_hat, A_hat, sdpa_free_format.g(), &C_csdp, &rhs_csdp,
                                               &constraints_csdp);
    struct csdp::blockmatrix X_csdp, Z;
    double* y{nullptr};
    csdp::cpp_initsoln(num_X_hat_rows, sdpa_free_format.g().rows(), C_csdp, rhs_csdp, constraints_csdp, &X_csdp, &y,
                       &Z);
    double pobj{0};
    double dobj{0};
    const int ret = csdp::cpp_easy_sdp(csdp_params_pathname.c_str(), num_X_hat_rows, sdpa_free_format.g().rows(),
                                       C_csdp, rhs_csdp, constraints_csdp, -sdpa_free_format.constant_min_cost_term(),
                                       &X_csdp, &y, &Z, &pobj, &dobj);
    ...
    for (int i = 0; i < sdpa_free_format.num_free_variables(); ++i) {
        double y_plus_val{0};
        for (Eigen::SparseMatrix<double>::InnerIterator it(X_hat, sdpa_free_format.num_X_rows() + i); it; ++it) {
            y_plus_val = it.value();
        }
        double y_minus_val{0};
        for (Eigen::SparseMatrix<double>::InnerIterator it(
                     X_hat, sdpa_free_format.num_X_rows() + sdpa_free_format.num_free_variables() + i);
             it; ++it) {
    ...
    csdp::cpp_free_prob(num_X_hat_rows, sdpa_free_format.g().rows(), C_csdp, rhs_csdp, constraints_csdp, X_csdp, y, Z);
}
 ...
 * for more details.
 */
void SolveProgramThroughLorentzConeSlackApproach(const MathematicalProgram& prog,
                                                 const SdpaFreeFormat& sdpa_free_format,
                                                 const std::string& csdp_params_pathname,
                                                 MathematicalProgramResult* result) {
    static const logging::Warn log_once(
    ...
    const int num_X_hat_rows = sdpa_free_format.num_X_rows() + sdpa_free_format.num_free_variables() + 1;

    // Now try to call CSDP to solve this problem.
    csdp::blockmatrix C_csdp;
    double* rhs_csdp{nullptr};
    csdp::constraintmatrix* constraints_csdp{nullptr};
    ConvertSparseMatrixFormatToCsdpProblemData(X_hat_blocks, C_hat, A_hat, rhs_hat, &C_csdp, &rhs_csdp,
                                               &constraints_csdp);
    struct csdp::blockmatrix X_csdp, Z;
    double* y{nullptr};
    csdp::cpp_initsoln(num_X_hat_rows, rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, &X_csdp, &y, &Z);
    double pobj{0};
    double dobj{0};
    const int ret = csdp::cpp_easy_sdp(csdp_params_pathname.c_str(), num_X_hat_rows, rhs_hat.rows(), C_csdp, rhs_csdp,
                                       constraints_csdp, -sdpa_free_format.constant_min_cost_term(), &X_csdp, &y, &Z,
                                       &pobj, &dobj);
    Eigen::SparseMatrix<double> X_hat(num_X_hat_rows, num_X_hat_rows);
    ConvertCsdpBlockMatrixtoEigen(X_csdp, &X_hat);
    // Now retrieve the value for the free variable s from Y.
    Eigen::VectorXd s_val = Eigen::VectorXd::Zero(sdpa_free_format.num_free_variables());
    for (int i = 0; i < sdpa_free_format.num_free_variables(); ++i) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(X_hat, sdpa_free_format.num_X_rows() + i + 1); it; ++it) {
            s_val(i) = it.value();
    ...
    csdp::cpp_free_prob(num_X_hat_rows, rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, X_csdp, y, Z);
}
...
void CsdpSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("CsdpSolver doesn't support the feature of variable scaling.");
    }

    // If necessary, write the custom CSDP parameters to a temporary file, which
    // we should remove when this function returns.
    const std::string csdp_params_pathname = internal::MaybeWriteCsdpParams(merged_options);
    ...
    if (sdpa_free_format.num_free_variables() == 0) {
        internal::SolveProgramWithNoFreeVariables(prog, sdpa_free_format, csdp_params_pathname, result);
    } else {
        ...
        switch (method) {
            case RemoveFreeVariableMethod::kNullspace: {
                internal::SolveProgramThroughNullspaceApproach(prog, sdpa_free_format, csdp_params_pathname, result);
                break;
            }
            case RemoveFreeVariableMethod::kTwoSlackVariables: {
                internal::SolveProgramThroughTwoSlackVariablesApproach(prog, sdpa_free_format, csdp_params_pathname,
                                                                       result);
                break;
            }
            case RemoveFreeVariableMethod::kLorentzConeSlack: {
                internal::SolveProgramThroughLorentzConeSlackApproach(prog, sdpa_free_format, csdp_params_pathname,
                                                                      result);
                break;
            }
        }
