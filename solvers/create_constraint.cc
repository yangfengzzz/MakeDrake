...
Binding<Constraint> ParseConstraint(const Eigen::Ref<const VectorX<Expression>>& v,
    ...
    std::tie(vars, map_var_to_index) = symbolic::ExtractVariablesFromExpression(v);
    ...
    for (int i = 0; i < v.size(); ++i) {
        ...
        int num_vi_variables = symbolic::DecomposeAffineExpression(v(i), map_var_to_index, &Ai, &constant_term);
        A.row(i) = Ai;
    ...
    if (is_v_bounding_box) {
        ...
        return CreateBinding(make_shared<BoundingBoxConstraint>(new_lb, new_ub), bounding_box_x);
    }

    return CreateBinding(make_shared<LinearConstraint>(A, new_lb, new_ub), vars);
}

std::unique_ptr<Binding<Constraint>> MaybeParseLinearConstraint(const symbolic::Expression& e, double lb, double ub) {
    if (!e.is_polynomial()) {
    ...
    if (p.indeterminates().size() == 1) {
        ...
        return std::make_unique<Binding<Constraint>>(
                std::make_shared<BoundingBoxConstraint>(Vector1d(var_lower), Vector1d(var_upper)),
    ...
    for (const auto& term : p.monomial_to_coefficient_map()) {
        if (term.first.total_degree() == 0) {
            ...
            const int var_index = map_var_to_index.at(term.first.GetVariables().begin()->get_id());
            a(var_index) = get_constant_value(term.second);
        }
    }
    if (lower == upper) {
        return std::make_unique<Binding<Constraint>>(std::make_shared<LinearEqualityConstraint>(a, Vector1d(lower)),
                                                     bound_variables);
    } else {
        return std::make_unique<Binding<Constraint>>(
                std::make_shared<LinearConstraint>(a, Vector1d(lower), Vector1d(upper)), bound_variables);
    }
}
...
//     False.
//
void FindBound(const Expression& e1, const Expression& e2, Expression* const e, double* const c) {
    ...
    if (is_constant(e1_expanded)) {
        ...
        if (!isfinite(c1)) {
            ostringstream oss;
            oss << "FindBound() cannot handle the constraint: " << e1 << " <= " << e2 << " because " << e1
    ...
    if (is_constant(e2_expanded)) {
        ...
        if (!isfinite(c2)) {
            ostringstream oss;
            oss << "FindBound() cannot handle the constraint: " << e1 << " <= " << e2 << " because " << e2
                << " has infinity in the constant term after expansion.";
            throw runtime_error{oss.str()};
        }
    ...
    //    -∞ <= -∞     Trivially holds.
    ostringstream oss;
    if (c1 == numeric_limits<double>::infinity() && c2 == -numeric_limits<double>::infinity()) {
        oss << "FindBound() detects an infeasible constraint: " << e1 << " <= " << e2 << ".";
        throw runtime_error{oss.str()};
    } else {
...
std::unique_ptr<Binding<LorentzConeConstraint>> MaybeParseLorentzConeConstraint(
    ...
    if (is_greater_than_or_equal_to(f)) {
        ...
        throw std::runtime_error(
                fmt::format("ParseLorentzConeConstraint is called with a formula {} that is not a "
    ...
    if (!throw_on_failure) {
        ...
        if (!greater.is_polynomial() || symbolic::Polynomial(greater).TotalDegree() > 1) {
            return nullptr;
        }
    }
    symbolic::DecomposeAffineExpressions(Vector1<Expression>{greater}, &A, &b, &greater_vars);
    DRAKE_DEMAND(A.rows() == 1);
    DRAKE_DEMAND(b.size() == 1);
    auto [is_l2norm, C, d, lesser_vars] = DecomposeL2NormExpression(lesser, psd_tol, coefficient_tol);
    if (!is_l2norm) {
        if (throw_on_failure) {
            throw runtime_error(
                    fmt::format("Expression {} is not an L2 norm. ParseLorentzCone only supports "
    ...
    Eigen::MatrixXd A_lorentz = Eigen::MatrixXd::Zero(1 + C.rows(), A.cols() + C.cols());
    ...
    return std::make_unique<Binding<LorentzConeConstraint>>(
            make_shared<LorentzConeConstraint>(A_lorentz, b_lorentz, eval_type), vars);
}
}  // namespace

Binding<Constraint> ParseConstraint(const Eigen::Ref<const MatrixX<symbolic::Formula>>& formulas) {
    ...
    if (k == 0) {
        ...
        return internal::CreateBinding(std::make_shared<BoundingBoxConstraint>(Eigen::VectorXd(0), Eigen::VectorXd(0)),
...
Binding<Constraint> ParseConstraint(const Formula& f) {
    if (symbolic::is_false(f)) {
        throw std::runtime_error("ParseConstraint is called with a formula being always false.");
    } else if (symbolic::is_true(f)) {
        return internal::CreateBinding(std::make_shared<BoundingBoxConstraint>(Eigen::VectorXd(0), Eigen::VectorXd(0)),
                                       VectorXDecisionVariable(0));
    } else if (is_equal_to(f)) {
        ...
        auto lorentz_cone_binding = MaybeParseLorentzConeConstraint(f,
        ...
        const Eigen::Map<const VectorX<Formula>> map_operands(vec_operands.data(), vec_operands.size());
...
Binding<LinearEqualityConstraint> ParseLinearEqualityConstraint(const set<Formula>& formulas) {
    ...
    if (i == 0) {
        ...
        return internal::CreateBinding(std::make_shared<LinearEqualityConstraint>(Eigen::Matrix<double, 0, 0>(),
...
Binding<LinearEqualityConstraint> DoParseLinearEqualityConstraint(const Eigen::Ref<const VectorX<Expression>>& v,
    ...
    for (int i = 0; i < v.rows(); ++i) {
        double constant_term(0);
        symbolic::DecomposeAffineExpression(v(i), map_var_to_index, &Ai, &constant_term);
        A.row(i) = Ai;
...
Binding<QuadraticConstraint> ParseQuadraticConstraint(const symbolic::Expression& e,
    ...
    symbolic::DecomposeQuadraticPolynomial(poly, map_var_to_index, &Q, &b, &constant_term);
    // The constraint to be imposed is
    // lb - k ≤ 0.5 xᵀQx + bᵀx ≤ ub - k
    return CreateBinding(make_shared<QuadraticConstraint>(Q, b, lower_bound - constant_term,
                                                          upper_bound - constant_term, hessian_type),
                         vars_vec);
}

shared_ptr<Constraint> MakePolynomialConstraint(const VectorXPoly& polynomials,
    ...
    if (all_affine) {
        Eigen::MatrixXd linear_constraint_matrix = Eigen::MatrixXd::Zero(polynomials.rows(), poly_vars.size());
        ...
        for (int poly_num = 0; poly_num < polynomials.rows(); poly_num++) {
            for (const auto& monomial : polynomials[poly_num].GetMonomials()) {
                if (monomial.terms.size() == 0) {
                    ...
                    int var_num = (find(poly_vars.begin(), poly_vars.end(), term_var) - poly_vars.begin());
        ...
        if (ub == lb) {
            return make_shared<LinearEqualityConstraint>(linear_constraint_matrix, linear_constraint_ub);
        } else {
            return make_shared<LinearConstraint>(linear_constraint_matrix, linear_constraint_lb, linear_constraint_ub);
        }
    } else {
...
Binding<LorentzConeConstraint> ParseLorentzConeConstraint(const symbolic::Formula& f,
...
Binding<LorentzConeConstraint> ParseLorentzConeConstraint(const Eigen::Ref<const VectorX<Expression>>& v,
                                                          LorentzConeConstraint::EvalType eval_type) {
    ...
    return CreateBinding(make_shared<LorentzConeConstraint>(A, b, eval_type), vars);
}
...
Binding<LorentzConeConstraint> ParseLorentzConeConstraint(const Expression& linear_expr,
                                                          ...
                                                          double tol,
                                                          LorentzConeConstraint::EvalType eval_type) {
    const auto& quadratic_p = symbolic::ExtractVariablesFromExpression(quadratic_expr);
    ...
    double a;
    symbolic::DecomposeQuadraticPolynomial(poly, quadratic_var_to_index_map, &Q, &b, &a);
...
std::shared_ptr<RotatedLorentzConeConstraint> ParseQuadraticAsRotatedLorentzConeConstraint(
        const Eigen::Ref<const Eigen::MatrixXd>& Q,
        const Eigen::Ref<const Eigen::VectorXd>& b,
        double c,
        double zero_tol) {
    // [-bᵀx-c, 1, Fx] is in the rotated Lorentz cone, where FᵀF = 0.5 * Q
    const Eigen::MatrixXd F = math::DecomposePSDmatrixIntoXtransposeTimesX((Q + Q.transpose()) / 4, zero_tol);
