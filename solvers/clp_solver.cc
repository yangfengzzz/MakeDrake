...
namespace {
void ConstructClpModel(const std::vector<Eigen::Triplet<double>>& constraint_coeffs,
                       const std::vector<double>& constraint_lower,
                       const std::vector<double>& constraint_upper,
                       const Eigen::VectorXd& xlow,
                       const Eigen::VectorXd& xupp,
                       const Eigen::SparseMatrix<double>& quadratic_matrix,
                       const Eigen::VectorXd& objective_coeff,
                       double constant_cost,
                       ClpModel* model) {
    Eigen::SparseMatrix<double> constraint_mat(constraint_lower.size(), xlow.rows());
    constraint_mat.setFromTriplets(constraint_coeffs.begin(), constraint_coeffs.end());
    // Now convert the sparse matrix to Compressed Column Storage format.
    model->loadProblem(constraint_mat.cols(), constraint_mat.rows(), constraint_mat.outerIndexPtr(),
                       constraint_mat.innerIndexPtr(), constraint_mat.valuePtr(), xlow.data(), xupp.data(),
                       objective_coeff.data(), constraint_lower.data(), constraint_upper.data(),
    ...
    if (quadratic_matrix.nonZeros() > 0) {
        ...
        model->loadQuadraticObjective(quadratic_matrix.cols(), quadratic_matrix.outerIndexPtr(),
...
void AddLinearConstraint(const MathematicalProgram& prog,
    ...
    const Binding<Constraint> binding_cast = internal::BindingDynamicCast<Constraint>(linear_constraint);
    constraint_dual_start_index->emplace(binding_cast, *constraint_count);
    const Eigen::SparseMatrix<double>& A = linear_constraint.evaluator()->get_sparse_A();
    // First loop by column to set the coefficient since A is a column major
    // matrix.
    for (int j = 0; j < A.cols(); ++j) {
        const int var_index = prog.FindDecisionVariableIndex(linear_constraint.variables()(j));
        for (Eigen::SparseMatrix<double>::InnerIterator it(A, j); it; ++it) {
            const int row = it.row();
            constraint_coeffs->emplace_back(row + *constraint_count, var_index, it.value());
        }
    }
    // Now loop by rows of the constraints to set the bounds.
    for (int i = 0; i < linear_constraint.evaluator()->num_constraints(); ++i) {
        (*constraint_lower)[*constraint_count + i] = linear_constraint.evaluator()->lower_bound()(i);
        (*constraint_upper)[*constraint_count + i] = linear_constraint.evaluator()->upper_bound()(i);
    }
...
void AddLinearConstraints(const MathematicalProgram& prog,
    ...
    for (const auto& linear_constraint : prog.linear_constraints()) {
        ...
        num_nonzero_coeff_max +=
                linear_constraint.evaluator()->num_constraints() * linear_constraint.variables().rows();
    }
    for (const auto& linear_eq_constraint : prog.linear_equality_constraints()) {
        ...
        num_nonzero_coeff_max +=
                linear_eq_constraint.evaluator()->num_constraints() * linear_eq_constraint.variables().rows();
    }
    ...
    for (const auto& linear_constraint : prog.linear_constraints()) {
        AddLinearConstraint<LinearConstraint>(prog, linear_constraint, constraint_coeffs, constraint_lower,
                                              constraint_upper, &constraint_count, constraint_dual_start_index);
    }
    for (const auto& linear_equal_constraint : prog.linear_equality_constraints()) {
        AddLinearConstraint<LinearEqualityConstraint>(prog, linear_equal_constraint, constraint_coeffs,
                                                      constraint_lower, constraint_upper, &constraint_count,
...
void SetConstraintDualSolution(const Eigen::Map<const Eigen::VectorXd>& lambda,
                               const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_index,
                               MathematicalProgramResult* result) {
    for (const auto& [constraint, dual_start_index] : constraint_dual_start_index) {
        result->set_dual_solution(constraint,
                                  lambda.segment(dual_start_index, constraint.evaluator()->num_constraints()));
    }
}
...
void SetBoundingBoxConstraintDualSolution(
    ...
    for (const auto& [bb_con, dual_var_indices] : bb_con_dual_variable_indices) {
        Eigen::VectorXd bb_con_dual_sol = Eigen::VectorXd::Zero(bb_con.evaluator()->num_constraints());
        ...
        for (int k = 0; k < bb_con.evaluator()->num_constraints(); ++k) {
            ...
            if (lower_dual_indices[k] != -1 && column_dual_sol(lower_dual_indices[k]) > 0) {
            ...
            } else if (upper_dual_indices[k] != -1 && column_dual_sol(upper_dual_indices[k]) < 0) {
...
void SetSolution(const MathematicalProgram& prog,
                 const ClpModel& model,
                 const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_index,
                 const std::unordered_map<Binding<BoundingBoxConstraint>,
                                          std::pair<std::vector<int>, std::vector<int>>>& bb_con_dual_variable_indices,
                 MathematicalProgramResult* result) {
    ClpSolverDetails& solver_details = result->SetSolverDetailsType<ClpSolverDetails>();
    solver_details.clp_version = CLP_VERSION;
    result->set_x_val(Eigen::Map<const Eigen::VectorXd>(model.getColSolution(), prog.num_vars()));
    Eigen::Map<const Eigen::VectorXd> lambda(model.dualRowSolution(), model.getNumRows());
    SetConstraintDualSolution(lambda, constraint_dual_start_index, result);
    Eigen::Map<const Eigen::VectorXd> column_dual_sol(model.dualColumnSolution(), model.getNumCols());
    SetBoundingBoxConstraintDualSolution(column_dual_sol, bb_con_dual_variable_indices, result);
...
void SetBoundingBoxConstraintDualIndices(
        ...
        const Binding<BoundingBoxConstraint>& bb_con,
        const Eigen::VectorXd& xlow,
        const Eigen::VectorXd& xupp,
    ...
    bbcon_dual_indices->emplace(bb_con, std::make_pair(lower_dual_indices, upper_dual_indices));
}

void ParseModelExceptLinearConstraints(const MathematicalProgram& prog,
                                       Eigen::VectorXd* xlow,
                                       Eigen::VectorXd* xupp,
                                       Eigen::SparseMatrix<double>* quadratic_matrix,
    ...
    for (int i = 0; i < quadratic_vars.rows(); ++i) {
        quadratic_var_to_index.emplace(quadratic_vars(i).get_id(), prog.FindDecisionVariableIndex(quadratic_vars(i)));
    }
    ...
    for (int col = 0; col < Q_lower.outerSize(); ++col) {
        const int x_col_index = quadratic_var_to_index.at(quadratic_vars(col).get_id());
        for (Eigen::SparseMatrix<double>::InnerIterator it(Q_lower, col); it; ++it) {
            const int x_row_index = quadratic_var_to_index.at(quadratic_vars(it.row()).get_id());
            quadratic_matrix_triplets.emplace_back(x_row_index, x_col_index, it.value());
        }
    }
    quadratic_matrix->setFromTriplets(quadratic_matrix_triplets.begin(), quadratic_matrix_triplets.end());

    objective_coeff->resize(prog.num_vars());
    for (Eigen::SparseVector<double>::InnerIterator it(linear_coeff); it; ++it) {
        (*objective_coeff)(prog.FindDecisionVariableIndex(linear_vars(it.row()))) = it.value();
    }
}
...
void ClpSolver::DoSolve(const MathematicalProgram& prog,
    ...
    Eigen::SparseMatrix<double> quadratic_matrix(prog.num_vars(), prog.num_vars());
    double constant_cost{0.};
    ParseModelExceptLinearConstraints(prog, &xlow, &xupp, &quadratic_matrix, &objective_coeff, &constant_cost);

    std::vector<Eigen::Triplet<double>> constraint_coeffs;
    std::vector<double> constraint_lower, constraint_upper;
    std::unordered_map<Binding<Constraint>, int> constraint_dual_start_index;
    if (!prog.linear_constraints().empty() || !prog.linear_equality_constraints().empty()) {
        AddLinearConstraints(prog, &constraint_coeffs, &constraint_lower, &constraint_upper,
                             &constraint_dual_start_index);
    }
    ConstructClpModel(constraint_coeffs, constraint_lower, constraint_upper, xlow, xupp, quadratic_matrix,
                      objective_coeff, constant_cost, &model);
    std::unordered_map<Binding<BoundingBoxConstraint>, std::pair<std::vector<int>, std::vector<int>>>
            bb_con_dual_variable_indices;
    for (const auto& bb_con : prog.bounding_box_constraints()) {
        SetBoundingBoxConstraintDualIndices(prog, bb_con, xlow, xupp, &bb_con_dual_variable_indices);
    }
    ...
    // Set the solution
    SetSolution(prog, model, constraint_dual_start_index, bb_con_dual_variable_indices, result);
}
}  // namespace solvers
}  // namespace drake
