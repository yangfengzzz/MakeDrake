#include "solvers/mosek_solver_internal.h"
...
#include "math/quadratic_form.h"
#include "solvers/aggregate_costs_constraints.h"
...
void ConvertTripletsToVectors(const std::vector<Eigen::Triplet<double>>& triplets,
                              int matrix_rows,
                              int matrix_cols,
                              std::vector<MSKint32t>* row_indices,
                              std::vector<MSKint32t>* col_indices,
                              std::vector<MSKrealt>* values) {
...
MosekSolverProgram::MosekSolverProgram(const MathematicalProgram& prog, MSKenv_t env)
    ...
    task_ = nullptr;
    MSK_maketask(env, 0, map_decision_var_to_mosek_var_.decision_variable_to_mosek_nonmatrix_variable.size(), &task_);
}
...
MSKrescodee MosekSolverProgram::AddMatrixVariableEntryCoefficientMatrixIfNonExistent(
        const MatrixVariableEntry& matrix_variable_entry, MSKint64t* E_index) {
    MSKrescodee rescode{MSK_RES_OK};
    auto it = matrix_variable_entry_to_selection_matrix_id_.find(matrix_variable_entry.id());
    if (it != matrix_variable_entry_to_selection_matrix_id_.end()) {
        *E_index = it->second;
        ...
        rescode = MSK_appendsparsesymmat(task_, matrix_variable_entry.num_matrix_rows(), 1, &row, &col, &val, E_index);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
        matrix_variable_entry_to_selection_matrix_id_.emplace_hint(it, matrix_variable_entry.id(), *E_index);
    }
    return rescode;
}

MSKrescodee MosekSolverProgram::AddScalarTimesMatrixVariableEntryToMosek(
        MSKint32t constraint_index, const MatrixVariableEntry& matrix_variable_entry, MSKrealt scalar) {
    MSKrescodee rescode{MSK_RES_OK};
    MSKint64t E_index;
    rescode = AddMatrixVariableEntryCoefficientMatrixIfNonExistent(matrix_variable_entry, &E_index);
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    rescode = MSK_putbaraij(task_, constraint_index, matrix_variable_entry.bar_matrix_index(), 1, &E_index, &scalar);
    return rescode;
}
...
MSKrescodee MosekSolverProgram::SetMosekLinearConstraintBound(int linear_constraint_index,
                                                              double lower,
                                                              double upper,
    ...
    switch (bound_type) {
        case LinearConstraintBoundType::kEquality: {
            rescode = MSK_putconbound(task_, linear_constraint_index, MSK_BK_FX, lower, lower);
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
            break;
        ...
        case LinearConstraintBoundType::kInequality: {
            if (std::isinf(lower) && std::isinf(upper)) {
                ...
                rescode = MSK_putconbound(task_, linear_constraint_index, MSK_BK_FR, -MSK_INFINITY, MSK_INFINITY);
            } else if (std::isinf(lower) && !std::isinf(upper)) {
                rescode = MSK_putconbound(task_, linear_constraint_index, MSK_BK_UP, -MSK_INFINITY, upper);
            } else if (!std::isinf(lower) && std::isinf(upper)) {
                rescode = MSK_putconbound(task_, linear_constraint_index, MSK_BK_LO, lower, MSK_INFINITY);
            } else {
                rescode = MSK_putconbound(task_, linear_constraint_index, MSK_BK_RA, lower, upper);
            }
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
...
// upper.
MSKrescodee MosekSolverProgram::AddLinearConstraintToMosek(const MathematicalProgram& prog,
    ...
    DRAKE_ASSERT(B.rows() == lower.rows() && B.cols() == static_cast<int>(slack_vars_mosek_indices.size()));
    ...
    for (int i = 0; i < lower.rows(); ++i) {
        ...
        // on this order.
        rescode = SetMosekLinearConstraintBound(num_mosek_constraint + i, lower(i), upper(i), bound_type);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
    }
    ...
    std::vector<std::unordered_map<MSKint64t, std::pair<std::vector<MSKint64t>, std::vector<MSKrealt>>>> bar_A;

    rescode = ParseLinearExpression(prog, A, B, decision_vars, slack_vars_mosek_indices, &Ax_subi, &Ax_subj, &Ax_valij,
                                    &bar_A);
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    if (!bar_A.empty()) {
        for (int i = 0; i < lower.rows(); ++i) {
            for (const auto& [j, sub_weights] : bar_A[i]) {
                ...
                rescode = MSK_putbaraij(task_, num_mosek_constraint + i, j, sub.size(), sub.data(), weights.data());
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
            }
    ...
    rescode = MSK_putaijlist(task_, Ax_subi.size(), Ax_subi.data(), Ax_subj.data(), Ax_valij.data());
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    ...
    DRAKE_DEMAND(num_mosek_constraints_after == num_mosek_constraint + lower.rows());
    return rescode;
}
...
MSKrescodee MosekSolverProgram::ParseLinearExpression(
        ...
        std::vector<std::unordered_map<MSKint64t, std::pair<std::vector<MSKint64t>, std::vector<MSKrealt>>>>* bar_F) {
    ...
    if (static_cast<int>(decision_vars_set.size()) == decision_vars.rows()) {
        return this->ParseLinearExpressionNoDuplication(prog, A, B, decision_vars, slack_vars_mosek_indices, F_subi,
                                                        F_subj, F_valij, bar_F);
    } else {
        Eigen::SparseMatrix<double> A_unique;
        VectorX<symbolic::Variable> unique_decision_vars;
        AggregateDuplicateVariables(A, decision_vars, &A_unique, &unique_decision_vars);
        return this->ParseLinearExpressionNoDuplication(prog, A_unique, B, unique_decision_vars,
                                                        slack_vars_mosek_indices, F_subi, F_subj, F_valij, bar_F);
    }
}
...
MSKrescodee MosekSolverProgram::ParseLinearExpressionNoDuplication(
    ...
    // <F̅ᵢⱼA̅ᵢⱼ, X̅ⱼ>.
    std::unordered_map<MSKint64t, std::vector<MatrixVariableEntry>> mosek_matrix_variable_entries;
    if (A.nonZeros() != 0) {
        ...
        for (int i = 0; i < decision_vars.rows(); ++i) {
            decision_var_indices[i] = prog.FindDecisionVariableIndex(decision_vars(i));
            const auto it_mosek_matrix_variable =
                    decision_variable_to_mosek_matrix_variable().find(decision_var_indices[i]);
            if (it_mosek_matrix_variable != decision_variable_to_mosek_matrix_variable().end()) {
                ...
                const MatrixVariableEntry& matrix_variable_entry = it_mosek_matrix_variable->second;
                MSKint64t E_mn_index;
                rescode = AddMatrixVariableEntryCoefficientMatrixIfNonExistent(matrix_variable_entry, &E_mn_index);
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                mosek_matrix_variable_entries[matrix_variable_entry.bar_matrix_index()].push_back(
                        matrix_variable_entry);
            } else {
                const int mosek_nonmatrix_variable =
                        decision_variable_to_mosek_nonmatrix_variable().at(decision_var_indices[i]);
                for (Eigen::SparseMatrix<double>::InnerIterator it(A, i); it; ++it) {
        ...
        if (!mosek_matrix_variable_entries.empty()) {
            ...
            for (int col_index : mosek_matrix_variable_in_decision_var) {
                const MatrixVariableEntry& matrix_variable_entry =
                        decision_variable_to_mosek_matrix_variable().at(decision_var_indices[col_index]);
                for (Eigen::SparseMatrix<double>::InnerIterator it(A, col_index); it; ++it) {
                    ...
                    auto it_bar_F = (*bar_F)[it.row()].find(matrix_variable_entry.bar_matrix_index());
                    if (it_bar_F != (*bar_F)[it.row()].end()) {
                        it_bar_F->second.first.push_back(
                                matrix_variable_entry_to_selection_matrix_id_.at(matrix_variable_entry.id()));
...
MSKrescodee MosekSolverProgram::AddLinearConstraints(
        ...
        std::unordered_map<Binding<LinearConstraint>, ConstraintDualIndices>* linear_con_dual_indices,
        std::unordered_map<Binding<LinearEqualityConstraint>, ConstraintDualIndices>* lin_eq_con_dual_indices) {
    MSKrescodee rescode = AddLinearConstraintsFromBindings(
            prog.linear_equality_constraints(), LinearConstraintBoundType::kEquality, prog, lin_eq_con_dual_indices);
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    rescode = AddLinearConstraintsFromBindings(prog.linear_constraints(), LinearConstraintBoundType::kInequality, prog,
...
MSKrescodee MosekSolverProgram::AddBoundingBoxConstraints(
        const MathematicalProgram& prog,
        std::unordered_map<Binding<BoundingBoxConstraint>, std::pair<ConstraintDualIndices, ConstraintDualIndices>>*
                dual_indices) {
    ...
    auto add_variable_bound_in_mosek = [this](int mosek_var_index, double lower, double upper) {
        MSKrescodee rescode_bound{MSK_RES_OK};
        if (std::isinf(lower) && std::isinf(upper)) {
            rescode_bound = MSK_putvarbound(this->task_, mosek_var_index, MSK_BK_FR, -MSK_INFINITY, MSK_INFINITY);
        } else if (std::isinf(lower) && !std::isinf(upper)) {
            rescode_bound = MSK_putvarbound(this->task_, mosek_var_index, MSK_BK_UP, -MSK_INFINITY, upper);
        } else if (!std::isinf(lower) && std::isinf(upper)) {
            rescode_bound = MSK_putvarbound(this->task_, mosek_var_index, MSK_BK_LO, lower, MSK_INFINITY);
        } else {
            rescode_bound = MSK_putvarbound(this->task_, mosek_var_index, MSK_BK_RA, lower, upper);
        }
    ...
    const int bounded_matrix_var_count = static_cast<int>(bounded_matrix_var_indices.size());
    ...
    for (int i = 0; i < bounded_matrix_var_count; ++i) {
        ...
        bounded_matrix_vars(i) = prog.decision_variable(bounded_matrix_var_indices[i]);
        var_in_bounded_matrix_vars.emplace(prog.decision_variable(bounded_matrix_var_indices[i]).get_id(), i);
    }

    Eigen::SparseMatrix<double> A_eye(bounded_matrix_var_count, bounded_matrix_var_count);
    A_eye.setIdentity();
    ...
    rescode = AddLinearConstraintToMosek(prog, A_eye, B_zero, bounded_matrix_vars_lower, bounded_matrix_vars_upper,
    ...
    DRAKE_DEMAND(num_linear_constraints_after == num_linear_constraints_before + bounded_matrix_var_count);
    // Add dual variables.
    for (const auto& binding : prog.bounding_box_constraints()) {
        ...
        for (int i = 0; i < binding.variables().rows(); ++i) {
            const int var_index = prog.FindDecisionVariableIndex(binding.variables()(i));
            ...
            if (it1 != decision_variable_to_mosek_nonmatrix_variable().end()) {
                ...
                DRAKE_DEMAND(decision_variable_to_mosek_matrix_variable().contains(var_index));
                ...
                const int linear_constraint_index =
                        num_linear_constraints_before + var_in_bounded_matrix_vars.at(binding.variables()(i).get_id());
...
MSKrescodee MosekSolverProgram::AddQuadraticConstraints(
        const MathematicalProgram& prog, std::unordered_map<Binding<QuadraticConstraint>, MSKint64t>* dual_indices) {
    MSKrescodee rescode = MSK_RES_OK;
    ...
    for (int constraint_count = 0; constraint_count < static_cast<int>(prog.quadratic_constraints().size());
         ++constraint_count) {
        const Binding<QuadraticConstraint>& constraint = prog.quadratic_constraints()[constraint_count];
        // Find the variable index in mosek
        std::vector<int> decision_variable_in_mosek_index(constraint.variables().rows());
        ...
        // yet.
        for (int i = 0; i < constraint.variables().rows(); ++i) {
            const int decision_var_index = prog.FindDecisionVariableIndex(constraint.variables()(i));
            if (decision_variable_to_mosek_matrix_variable().count(decision_var_index) > 0) {
                throw std::runtime_error(
                        fmt::format("MosekSolverProgram::AddQuaraticConstraint(): variable {} shows up "
                ...
                decision_variable_in_mosek_index[i] =
                        decision_variable_to_mosek_nonmatrix_variable().at(decision_var_index);
            }
        }
            ...
            for (int i = j; i < constraint.variables().rows(); ++i) {
                if (constraint.evaluator()->Q()(i, j) != 0) {
                    ...
                    if (i == j) {
                        ...
                        if (decision_variable_in_mosek_index[i] != decision_variable_in_mosek_index[j]) {
                            qcval.push_back(constraint.evaluator()->Q()(i, j));
                        } else {
        ...
        for (int i = 0; i < constraint.variables().rows(); ++i) {
            if (constraint.evaluator()->b()(i) != 0) {
                A_triplets.emplace_back(constraint_count, decision_variable_in_mosek_index[i],
                                        constraint.evaluator()->b()(i));
            }
        }

        // Set dual variable index.
        dual_indices->emplace(constraint, num_existing_constraints + constraint_count);
        ...
        rescode = MSK_putconbound(task_, num_existing_constraints + constraint_count, bound_key, lb, ub);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
    }
    ...
    rescode = MSK_putqcon(task_, qcsubi.size(), qcsubk.data(), qcsubi.data(), qcsubj.data(), qcval.data());
    ...
    Eigen::SparseMatrix<double, Eigen::ColMajor> A(prog.quadratic_constraints().size(), num_mosek_vars);
    A.setFromTriplets(A_triplets.begin(), A_triplets.end());
    for (int i = 0; i < A.cols(); ++i) {
        for (Eigen::SparseMatrix<double, Eigen::ColMajor>::InnerIterator it(A, i); it; ++it) {
            rescode = MSK_putaij(task_, num_existing_constraints + it.row(), it.col(), it.value());
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
        }
...
MSKrescodee MosekSolverProgram::AddAffineConeConstraint(const MathematicalProgram& prog,
                                                        const Eigen::SparseMatrix<double>& A,
                                                        const Eigen::SparseMatrix<double>& B,
                                                        const VectorX<symbolic::Variable>& decision_vars,
                                                        const std::vector<MSKint32t>& slack_vars_mosek_indices,
    ...
    this->ParseLinearExpression(prog, A, B, decision_vars, slack_vars_mosek_indices, &F_subi, &F_subj, &F_valij,
                                &bar_F);
    ...
    rescode = MSK_putafefentrylist(task_, F_subi_increased.size(), F_subi_increased.data(), F_subj.data(),
                                   F_valij.data());
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    ...
    if (!bar_F.empty()) {
        for (int i = 0; i < afe_dim; ++i) {
            for (const auto& [j, sub_weights] : bar_F[i]) {
                const std::vector<MSKint64t>& sub = sub_weights.first;
                rescode = MSK_putafebarfentry(task_, num_afe + i, j, sub.size(), sub.data(), sub_weights.second.data());
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
            }
...
MSKrescodee MosekSolverProgram::AddPositiveSemidefiniteConstraints(
        const MathematicalProgram& prog,
        std::unordered_map<Binding<PositiveSemidefiniteConstraint>, MSKint32t>* psd_barvar_indices) {
    MSKrescodee rescode = MSK_RES_OK;
    ...
    rescode = MSK_appendbarvars(task_, bar_var_dimension.size(), bar_var_dimension.data());
    return rescode;
}

MSKrescodee MosekSolverProgram::AddLinearMatrixInequalityConstraint(
        const MathematicalProgram& prog,
        std::unordered_map<Binding<LinearMatrixInequalityConstraint>, MSKint64t>* acc_indices) {
    ...
    for (const auto& binding : prog.linear_matrix_inequality_constraints()) {
        ...
        A_triplets.reserve((binding.evaluator()->matrix_rows() + 1) * binding.evaluator()->matrix_rows() *
                           binding.variables().rows());
        int A_row_index = 0;
        Eigen::VectorXd c(binding.evaluator()->matrix_rows() * (binding.evaluator()->matrix_rows() + 1) / 2);
            ...
            for (int i = j; i < binding.evaluator()->matrix_rows(); ++i) {
                ...
                for (int k = 1; k < ssize(binding.evaluator()->F()); ++k) {
                    if (binding.evaluator()->F()[k](i, j) != 0) {
                        A_triplets.emplace_back(A_row_index, k - 1, binding.evaluator()->F()[k](i, j) * scaling_factor);
                    }
                }
                ++A_row_index;
            }
        }
        Eigen::SparseMatrix<double> A(c.rows(), binding.evaluator()->F().size() - 1);
        A.setFromTriplets(A_triplets.begin(), A_triplets.end());
        MSKint64t acc_index;
        rescode = this->AddAffineConeConstraint(prog, A, Eigen::SparseMatrix<double>(A.rows(), 0), binding.variables(),
...
MSKrescodee MosekSolverProgram::AddLinearCost(const Eigen::SparseVector<double>& linear_coeff,
    ...
    std::vector<std::vector<Eigen::Triplet<double>>> C_bar_lower_triplets(num_bar_var);
    for (Eigen::SparseVector<double>::InnerIterator it(linear_coeff); it; ++it) {
        ...
        if (it1 != decision_variable_to_mosek_nonmatrix_variable().end()) {
            ...
            rescode = MSK_putcj(task_, static_cast<MSKint32t>(it1->second), it.value());
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
        } else {
            ...
            C_bar_lower_triplets[it2->second.bar_matrix_index()].emplace_back(
                    ...
                    it2->second.row_index() == it2->second.col_index() ? it.value() : it.value() / 2);
        }
    }
    ...
    for (int i = 0; i < num_bar_var; ++i) {
        if (C_bar_lower_triplets[i].size() > 0) {
            ...
            ConvertTripletsToVectors(C_bar_lower_triplets[i], matrix_rows, matrix_rows, &Ci_bar_lower_rows,
                                     &Ci_bar_lower_cols, &Ci_bar_lower_values);
            MSKint64t Ci_bar_index{0};
            // Create the sparse matrix C̅ᵢ.
            rescode = MSK_appendsparsesymmat(task_, matrix_rows, Ci_bar_lower_rows.size(), Ci_bar_lower_rows.data(),
...
MSKrescodee MosekSolverProgram::AddQuadraticCostAsLinearCost(const Eigen::SparseMatrix<double>& Q_lower,
    ...
    const Eigen::MatrixXd L = math::DecomposePSDmatrixIntoXtransposeTimesX(Eigen::MatrixXd(Q_lower),
    ...
    rescode = this->AddAffineConeConstraint(prog, L_bar_sparse, s_coeff, quadratic_vars, slack_vars, g,
...
MSKrescodee MosekSolverProgram::AddQuadraticCost(const Eigen::SparseMatrix<double>& Q_quadratic_vars,
                                                 const VectorX<symbolic::Variable>& quadratic_vars,
                                                 const MathematicalProgram& prog) {
    MSKrescodee rescode = MSK_RES_OK;
    ...
    for (int i = 0; i < quadratic_vars.rows(); ++i) {
        var_indices[i] =
                decision_variable_to_mosek_nonmatrix_variable().at(prog.FindDecisionVariableIndex(quadratic_vars(i)));
    }
    std::vector<Eigen::Triplet<double>> Q_lower_triplets;
    for (int j = 0; j < Q_quadratic_vars.outerSize(); ++j) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(Q_quadratic_vars, j); it; ++it) {
...
MSKrescodee MosekSolverProgram::AddL2NormCost(const Eigen::SparseMatrix<double>& C,
    ...
    rescode = this->AddAffineConeConstraint(prog, A, B, x, std::vector<MSKint32t>{{t_index}}, offset,
...
MSKrescodee MosekSolverProgram::AddCosts(const MathematicalProgram& prog) {
    ...
    AggregateQuadraticAndLinearCosts(prog.quadratic_costs(), prog.linear_costs(), &Q_lower, &quadratic_vars,
                                     &linear_coeff, &linear_vars, &constant_cost);
    ...
    if (!prog.quadratic_costs().empty()) {
        if (prog.quadratic_constraints().empty() && prog.lorentz_cone_constraints().empty() &&
            prog.rotated_lorentz_cone_constraints().empty() && prog.linear_matrix_inequality_constraints().empty() &&
            prog.positive_semidefinite_constraints().empty() && prog.exponential_cone_constraints().empty() &&
    ...
    for (const auto& l2norm_cost : prog.l2norm_costs()) {
        rescode = this->AddL2NormCost(l2norm_cost.evaluator()->get_sparse_A(), l2norm_cost.evaluator()->b(),
...
MSKrescodee MosekSolverProgram::SpecifyVariableType(const MathematicalProgram& prog,
                                                    bool* with_integer_or_binary_variables) {
    MSKrescodee rescode = MSK_RES_OK;
    for (const auto& decision_variable_mosek_variable : decision_variable_to_mosek_nonmatrix_variable()) {
        const int mosek_variable_index = decision_variable_mosek_variable.second;
        switch (prog.decision_variable(decision_variable_mosek_variable.first).get_type()) {
            ...
            case MathematicalProgram::VarType::BINARY: {
                *with_integer_or_binary_variables = true;
                ...
                rescode = MSK_getvarbound(task_, mosek_variable_index, &bound_key, &xi_lb, &xi_ub);
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                xi_lb = std::max(0.0, xi_lb);
                xi_ub = std::min(1.0, xi_ub);
                rescode = MSK_putvarbound(task_, mosek_variable_index, MSK_BK_RA, xi_lb, xi_ub);
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                break;
            ...
            case MathematicalProgram::VarType::BOOLEAN: {
                throw std::runtime_error("Boolean variables should not be used with Mosek solver.");
            }
            ...
            case MathematicalProgram::VarType::RANDOM_EXPONENTIAL:
                throw std::runtime_error("Random variables should not be used with Mosek solver.");
        }
    }
    for (const auto& decision_variable_mosek_matrix_variable : decision_variable_to_mosek_matrix_variable()) {
        const auto& decision_variable = prog.decision_variable(decision_variable_mosek_matrix_variable.first);
        if (decision_variable.get_type() != MathematicalProgram::VarType::CONTINUOUS) {
            throw std::invalid_argument("The variable " + decision_variable.get_name() +
...
MapDecisionVariableToMosekVariable::MapDecisionVariableToMosekVariable(const MathematicalProgram& prog) {
    ...
    for (const auto& psd_constraint : prog.positive_semidefinite_constraints()) {
        ...
        for (int j = 0; j < matrix_rows; ++j) {
            for (int i = j; i < matrix_rows; ++i) {
                const MatrixVariableEntry matrix_variable_entry(psd_constraint_count, i, j, matrix_rows);
                const int decision_variable_index =
                        prog.FindDecisionVariableIndex(psd_constraint.variables()(j * matrix_rows + i));
                auto it = decision_variable_to_mosek_matrix_variable.find(decision_variable_index);
                if (it == decision_variable_to_mosek_matrix_variable.end()) {
                    // This variable has not been registered as a mosek matrix variable
                    // before.
                    decision_variable_to_mosek_matrix_variable.emplace_hint(it, decision_variable_index,
                                                                            matrix_variable_entry);
                } else {
                    ...
                    auto it_same_decision_variable =
                            matrix_variable_entries_for_same_decision_variable.find(decision_variable_index);
                    if (it_same_decision_variable != matrix_variable_entries_for_same_decision_variable.end()) {
                        ...
                        matrix_variable_entries_for_same_decision_variable.emplace_hint(
                                ...
                                std::vector<MatrixVariableEntry>({it->second, matrix_variable_entry}));
                    }
                }
            }
        }
    ...
    for (int i = 0; i < prog.num_vars(); ++i) {
        if (!decision_variable_to_mosek_matrix_variable.contains(i)) {
            decision_variable_to_mosek_nonmatrix_variable.emplace(i, nonmatrix_variable_count++);
        }
    }
    DRAKE_DEMAND(static_cast<int>(decision_variable_to_mosek_matrix_variable.size() +
                                  decision_variable_to_mosek_nonmatrix_variable.size()) == prog.num_vars());
}

MSKrescodee MosekSolverProgram::AddEqualityConstraintBetweenMatrixVariablesForSameDecisionVariable() {
    MSKrescodee rescode{MSK_RES_OK};
    for (const auto& pair : matrix_variable_entries_for_same_decision_variable()) {
        ...
        const int num_matrix_variable_entries = static_cast<int>(matrix_variable_entries.size());
        ...
        for (int i = 1; i < num_matrix_variable_entries; ++i) {
            ...
            if (matrix_variable_entries[0].bar_matrix_index() != matrix_variable_entries[i].bar_matrix_index()) {
                // We add the constraint X̅ᵢ(m, n) - X̅ⱼ(p, q) = 0 where the index of
                // the bar matrix is different (i ≠ j).
                rescode = AddScalarTimesMatrixVariableEntryToMosek(linear_constraint_index, matrix_variable_entries[0],
                                                                   1.0);
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                rescode = AddScalarTimesMatrixVariableEntryToMosek(linear_constraint_index, matrix_variable_entries[i],
                                                                   -1.0);
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                ...
                rescode = AddMatrixVariableEntryCoefficientMatrixIfNonExistent(matrix_variable_entries[0],
                                                                               &(E_indices[0]));
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                rescode = AddMatrixVariableEntryCoefficientMatrixIfNonExistent(matrix_variable_entries[i],
                                                                               &(E_indices[1]));
                if (rescode != MSK_RES_OK) {
                    return rescode;
                }
                ...
                weights[0] =
                        matrix_variable_entries[0].row_index() == matrix_variable_entries[0].col_index() ? 1.0 : 0.5;
                weights[1] =
                        matrix_variable_entries[i].row_index() == matrix_variable_entries[i].col_index() ? -1.0 : -0.5;

                rescode = MSK_putbaraij(task_, linear_constraint_index, matrix_variable_entries[0].bar_matrix_index(),
            ...
            rescode =
                    SetMosekLinearConstraintBound(linear_constraint_index, 0, 0, LinearConstraintBoundType::kEquality);
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
        }
...
MSKrescodee MosekSolverProgram::SetPositiveSemidefiniteConstraintDualSolution(
        const MathematicalProgram& prog,
        const std::unordered_map<Binding<PositiveSemidefiniteConstraint>, MSKint32t>& psd_barvar_indices,
    ...
    for (const auto& psd_constraint : prog.positive_semidefinite_constraints()) {
        ...
        // solution).
        std::vector<MSKrealt> barsj(psd_constraint.evaluator()->matrix_rows() *
...
template <typename T>
void ThrowForInvalidOption(MSKrescodee rescode, const std::string& option, const T& val) {
    if (rescode != MSK_RES_OK) {
        const std::string mosek_version = fmt::format("{}.{}", MSK_VERSION_MAJOR, MSK_VERSION_MINOR);
        throw std::runtime_error(
                fmt::format("MosekSolver(): cannot set Mosek option '{option}' to value '{value}', "
                            ...
                            fmt::arg("option", option), fmt::arg("value", val), fmt::arg("code", fmt_streamed(rescode)),
...
MSKrescodee MosekSolverProgram::UpdateOptions(const SolverOptions& merged_options,
                                              const SolverId mosek_id,
    ...
    for (const auto& double_options : merged_options.GetOptionsDouble(mosek_id)) {
        if (rescode == MSK_RES_OK) {
            rescode = MSK_putnadouparam(task_, double_options.first.c_str(), double_options.second);
            ThrowForInvalidOption(rescode, double_options.first, double_options.second);
        }
    }
    for (const auto& int_options : merged_options.GetOptionsInt(mosek_id)) {
        if (rescode == MSK_RES_OK) {
            rescode = MSK_putnaintparam(task_, int_options.first.c_str(), int_options.second);
    ...
    for (const auto& str_options : merged_options.GetOptionsStr(mosek_id)) {
        if (rescode == MSK_RES_OK) {
            if (str_options.first == "writedata") {
                ...
                rescode = MSK_putnastrparam(task_, str_options.first.c_str(), str_options.second.c_str());
    ...
    *print_file_name = merged_options.get_print_file_name();
    ...
    if (*print_to_console && !print_file_name->empty()) {
        ...
        if (rescode == MSK_RES_OK) {
            rescode = MSK_linkfunctotaskstream(task_, MSK_STREAM_LOG, nullptr, printstr);
        }
    } else if (!print_file_name->empty()) {
        if (rescode == MSK_RES_OK) {
            rescode = MSK_linkfiletotaskstream(task_, MSK_STREAM_LOG, print_file_name->c_str(), 0);
        }
    }
...
MSKrescodee MosekSolverProgram::SetDualSolution(
        MSKsoltypee which_sol,
        const MathematicalProgram& prog,
        const std::unordered_map<Binding<BoundingBoxConstraint>,
                                 std::pair<ConstraintDualIndices, ConstraintDualIndices>>& bb_con_dual_indices,
        const DualMap<LinearConstraint>& linear_con_dual_indices,
        const DualMap<LinearEqualityConstraint>& lin_eq_con_dual_indices,
        const std::unordered_map<Binding<QuadraticConstraint>, MSKint64t>& quadratic_constraint_dual_indices,
        const std::unordered_map<Binding<LorentzConeConstraint>, MSKint64t>& lorentz_cone_acc_indices,
        const std::unordered_map<Binding<RotatedLorentzConeConstraint>, MSKint64t>& rotated_lorentz_cone_acc_indices,
        const std::unordered_map<Binding<ExponentialConeConstraint>, MSKint64t>& exp_cone_acc_indices,
        const std::unordered_map<Binding<PositiveSemidefiniteConstraint>, MSKint32t>& psd_barvar_indices,
        MathematicalProgramResult* result) const {
    ...
    SetBoundingBoxDualSolution(prog.bounding_box_constraints(), slx, sux, slc, suc, bb_con_dual_indices, result);
    // Set the duals for the linear constraint.
    SetLinearConstraintDualSolution(prog.linear_constraints(), slc, suc, linear_con_dual_indices, result);
    // Set the duals for the linear equality constraint.
    SetLinearConstraintDualSolution(prog.linear_equality_constraints(), slc, suc, lin_eq_con_dual_indices, result);
    SetQuadraticConstraintDualSolution(prog.quadratic_constraints(), slc, suc, quadratic_constraint_dual_indices,
                                       result);
    ...
    if (which_sol == MSK_SOL_ITR) {
        ...
        rescode = SetAffineConeConstraintDualSolution(prog.lorentz_cone_constraints(), task_, which_sol,
                                                      lorentz_cone_acc_indices, result);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
        rescode = SetAffineConeConstraintDualSolution(prog.rotated_lorentz_cone_constraints(), task_, which_sol,
                                                      rotated_lorentz_cone_acc_indices, result);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
        rescode = SetAffineConeConstraintDualSolution(prog.exponential_cone_constraints(), task_, which_sol,
                                                      exp_cone_acc_indices, result);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
    }
    rescode = SetPositiveSemidefiniteConstraintDualSolution(prog, psd_barvar_indices, which_sol, result);
    if (rescode != MSK_RES_OK) {
        return rescode;
    }
    return rescode;
...
void SetBoundingBoxDualSolution(
        ...
        const std::vector<MSKrealt>& slx,
        const std::vector<MSKrealt>& sux,
        const std::vector<MSKrealt>& slc,
        const std::vector<MSKrealt>& suc,
        const std::unordered_map<Binding<BoundingBoxConstraint>,
                                 std::pair<ConstraintDualIndices, ConstraintDualIndices>>& bb_con_dual_indices,
        MathematicalProgramResult* result) {
    auto set_dual_sol = [](int mosek_dual_lower_index, int mosek_dual_upper_index,
                           const std::vector<MSKrealt>& mosek_dual_lower, const std::vector<MSKrealt>& mosek_dual_upper,
                           double* dual_sol) {
        const double dual_lower = mosek_dual_lower_index == -1 ? 0. : mosek_dual_lower[mosek_dual_lower_index];
        const double dual_upper = mosek_dual_upper_index == -1 ? 0. : mosek_dual_upper[mosek_dual_upper_index];
    ...
    for (const auto& binding : constraints) {
        ...
        std::tie(lower_bound_duals, upper_bound_duals) = bb_con_dual_indices.at(binding);
        ...
        for (int i = 0; i < binding.variables().rows(); ++i) {
            switch (lower_bound_duals[i].type) {
                case DualVarType::kVariableBound: {
                    set_dual_sol(lower_bound_duals[i].index, upper_bound_duals[i].index, slx, sux, &(dual_sol(i)));
                    break;
                }
                case DualVarType::kLinearConstraint: {
                    set_dual_sol(lower_bound_duals[i].index, upper_bound_duals[i].index, slc, suc, &(dual_sol(i)));
                    break;
                }
                default: {
                    throw std::runtime_error(
