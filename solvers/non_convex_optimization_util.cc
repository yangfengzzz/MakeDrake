...
std::pair<Eigen::MatrixXd, Eigen::MatrixXd> DecomposeNonConvexQuadraticForm(
    ...
    prog.AddLinearConstraint(symbolic::Expression(s) >= Q1.cast<symbolic::Expression>().trace());
...
AddRelaxNonConvexQuadraticConstraintInTrustRegion(MathematicalProgram* prog,
    ...
    if (Q1.rows() != Q2.rows() || Q1.rows() != x0.rows() || Q1.rows() != x.rows() || p.rows() != y.rows()) {
        throw std::runtime_error("The dimensions of the inputs are not consistent.");
    }
    if (trust_region_gap <= 0) {
        throw std::runtime_error("trust_region_gap should be positive.");
    }
    if (lower_bound > upper_bound) {
        throw std::runtime_error("lower_bound should be no larger than upper_bound");
    }
    ...
    if (Q1_is_zero || Q2_is_zero) {
        ...
        Binding<RotatedLorentzConeConstraint> lorentz_cone1 =
                prog->AddRotatedLorentzConeConstraint(z(0), 1, x.dot(nonzero_Q * x), psd_tol);
        Eigen::Vector2d linear_lb(x0.dot(nonzero_Q * x0) - trust_region_gap, lower_bound);
        Eigen::Vector2d linear_ub(std::numeric_limits<double>::infinity(), upper_bound);
        Vector2<symbolic::Expression> linear_expr(2 * x0.dot(nonzero_Q * x) - z(0), p.dot(y) + z_coeff * z(0));
        ...
        std::vector<Binding<RotatedLorentzConeConstraint>> lorentz_cones{{lorentz_cone1}};
    ...
    linear_expressions << z(0) - z(1) + p.dot(y), z(0) - 2 * x0.dot(Q1 * x), z(1) - 2 * x0.dot(Q2 * x);
    const Eigen::Vector3d linear_lb(lower_bound, -std::numeric_limits<double>::infinity(),
                                    -std::numeric_limits<double>::infinity());
    const Eigen::Vector3d linear_ub(upper_bound, -x0.dot(Q1 * x0) + trust_region_gap,
                                    -x0.dot(Q2 * x0) + trust_region_gap);
    Binding<LinearConstraint> linear_constraint = prog->AddConstraint(internal::BindingDynamicCast<LinearConstraint>(
            internal::ParseConstraint(linear_expressions, linear_lb, linear_ub)));

    const Eigen::MatrixXd A1 = math::DecomposePSDmatrixIntoXtransposeTimesX(Q1, psd_tol);
    const Eigen::MatrixXd A2 = math::DecomposePSDmatrixIntoXtransposeTimesX(Q2, psd_tol);
