...
bool CheckAttributes(const MathematicalProgram& prog, std::string* explanation) {
    ...
    if (!capabilities_match) {
        if (explanation) {
            *explanation = fmt::format("OsqpSolver is unable to solve because {}.", *explanation);
        }
        return false;
    }
    ...
    if (nonconvex_quadratic_cost != nullptr) {
        if (explanation) {
            *explanation = "OsqpSolver is unable to solve because the quadratic cost " +
...
std::string OsqpSolver::UnsatisfiedProgramAttributes(const MathematicalProgram& prog) {
