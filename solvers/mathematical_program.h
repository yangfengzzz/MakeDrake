...
#include "solvers/function.h"
#include "solvers/indeterminate.h"
...
struct NewVariableNames<Rows, Cols> : public NewVariableNames<Eigen::Matrix<double, Rows, Cols>::SizeAtCompileTime> {};

template <int Rows>
struct NewSymmetricVariableNames
    : public NewVariableNames<Rows == Eigen::Dynamic ? Eigen::Dynamic : Rows*(Rows + 1) / 2> {};
 ...
 */
template <int Size>
typename std::enable_if_t<Size >= 0, typename NewVariableNames<Size>::type> CreateNewVariableNames(int) {
 ...
 */
template <int Size>
typename std::enable_if_t<Size == Eigen::Dynamic, typename NewVariableNames<Size>::type> CreateNewVariableNames(
        int size) {
...
void SetVariableNames(const std::string& name, int rows, int cols, Derived* names) {
    ...
    if (cols == 1) {
        ...
        for (int j = 0; j < cols; ++j) {
            for (int i = 0; i < rows; ++i) {
                (*names)[j * rows + i] = name + "(" + std::to_string(i) + "," + std::to_string(j) + ")";
            }
        }
    }
}
...
public:
    ...
    static constexpr double kGlobalInfeasibleCost = std::numeric_limits<double>::infinity();
    /// The optimal cost is -∞ when the problem is unbounded.
    static constexpr double kUnboundedCost = -std::numeric_limits<double>::infinity();
    ...
    MatrixDecisionVariable<Rows, Cols> NewContinuousVariables(int rows, int cols, const std::string& name = "X") {
        ...
        auto names =
                internal::CreateNewVariableNames<Eigen::Matrix<double, Rows, Cols>::SizeAtCompileTime>(rows * cols);
     ...
     */
    std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable> NewEvenDegreeSosPolynomial(
     ...
     */
    std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable> NewEvenDegreeSdsosPolynomial(
            const symbolic::Variables& indeterminates, int degree);
     ...
     */
    std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable> NewEvenDegreeDsosPolynomial(
            const symbolic::Variables& indeterminates, int degree);
    //@}
     ...
     */
    [[nodiscard]] symbolic::Polynomial MakePolynomial(const symbolic::Expression& e) const;
    ...
    Binding<VisualizationCallback> AddVisualizationCallback(const VisualizationCallback::CallbackFunction& callback,
        ...
        return AddVisualizationCallback(callback, ConcatenateVariableRefList((vars)));
    }
     ...
     */
    template <typename F>
    typename std::enable_if_t<internal::is_cost_functor_candidate<F>::value, Binding<Cost>> AddCost(
            F&& f, const VariableRefList& vars) {
    ...
    template <typename F>
    typename std::enable_if_t<internal::is_cost_functor_candidate<F>::value, Binding<Cost>> AddCost(
            F&& f, const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    ...
    typename std::enable_if_t<internal::assert_if_is_constraint<F>::value, Binding<Cost>> AddCost(F&&, Vars&&) {
    ...
    AddL2NormCostUsingConicConstraint(const Eigen::Ref<const Eigen::MatrixXd>& A,
                                      const Eigen::Ref<const Eigen::VectorXd>& b,
                                      const Eigen::Ref<const VectorXDecisionVariable>& vars);
    ...
    AddMaximizeLogDeterminantCost(const Eigen::Ref<const MatrixX<symbolic::Expression>>& X);
    ...
    AddLogDeterminantLowerBoundConstraint(const Eigen::Ref<const MatrixX<symbolic::Expression>>& X, double lower);
    //@}
     ...
     */
    template <typename Derived>
    typename std::enable_if_t<is_eigen_scalar_same<Derived, symbolic::Formula>::value, Binding<Constraint>>
    ...
    typename std::enable_if_t<is_eigen_vector_expression_double_pair<DerivedV, DerivedB>::value,
                              Binding<LinearEqualityConstraint>>
    AddLinearEqualityConstraint(const Eigen::MatrixBase<DerivedV>& v, const Eigen::MatrixBase<DerivedB>& b) {
     ...
     */
    template <typename DerivedV, typename DerivedB>
    typename std::enable_if_t<is_eigen_nonvector_expression_double_pair<DerivedV, DerivedB>::value,
                              Binding<LinearEqualityConstraint>>
    AddLinearEqualityConstraint(const Eigen::MatrixBase<DerivedV>& V,
                                const Eigen::MatrixBase<DerivedB>& B,
                                bool lower_triangle = false) {
        return AddConstraint(internal::ParseLinearEqualityConstraint(V, B, lower_triangle));
    }
    ...
    Binding<LinearEqualityConstraint> AddLinearEqualityConstraint(const Eigen::Ref<const Eigen::MatrixXd>& Aeq,
                                                                  ...
                                                                  const VariableRefList& vars) {
        return AddLinearEqualityConstraint(Aeq, beq, ConcatenateVariableRefList(vars));
    }
    ...
    Binding<LinearEqualityConstraint> AddLinearEqualityConstraint(const Eigen::Ref<const Eigen::RowVectorXd>& a,
                                                                  double beq,
                                                                  const VariableRefList& vars) {
        return AddLinearEqualityConstraint(a, beq, ConcatenateVariableRefList(vars));
    }
     ...
     */
    template <typename Derived>
    typename std::enable_if_t<std::is_same_v<typename Derived::Scalar, symbolic::Variable> &&
    ...
    typename std::enable_if_t<std::is_same_v<typename Derived::Scalar, symbolic::Variable> &&
                              ...
                              Binding<BoundingBoxConstraint>>
    AddBoundingBoxConstraint(double lb, double ub, const Eigen::MatrixBase<Derived>& vars) {
        const int kSize = Derived::RowsAtCompileTime != Eigen::Dynamic && Derived::ColsAtCompileTime != Eigen::Dynamic
    ...
    Binding<LorentzConeConstraint> AddLorentzConeConstraint(
            ...
            const VariableRefList& vars,
            LorentzConeConstraint::EvalType eval_type = LorentzConeConstraint::EvalType::kConvexSmooth) {
        return AddLorentzConeConstraint(A, b, ConcatenateVariableRefList(vars), eval_type);
    }
    ...
    Binding<LorentzConeConstraint> AddLorentzConeConstraint(
            const VariableRefList& vars,
            LorentzConeConstraint::EvalType eval_type = LorentzConeConstraint::EvalType::kConvexSmooth) {
        return AddLorentzConeConstraint(ConcatenateVariableRefList(vars), eval_type);
    }
    ...
    Binding<RotatedLorentzConeConstraint> AddRotatedLorentzConeConstraint(const Eigen::Ref<const Eigen::MatrixXd>& A,
                                                                          const Eigen::Ref<const Eigen::VectorXd>& b,
                                                                          const VariableRefList& vars) {
        return AddRotatedLorentzConeConstraint(A, b, ConcatenateVariableRefList(vars));
    }
     ...
     */
    Binding<RotatedLorentzConeConstraint> AddQuadraticAsRotatedLorentzConeConstraint(
            const Eigen::Ref<const Eigen::MatrixXd>& Q,
            const Eigen::Ref<const Eigen::VectorXd>& b,
            double c,
    ...
    Binding<LinearComplementarityConstraint> AddLinearComplementarityConstraint(
            ...
            const VariableRefList& vars) {
        return AddLinearComplementarityConstraint(M, q, ConcatenateVariableRefList(vars));
    }
    ...
    Binding<Constraint> AddPolynomialConstraint(const Eigen::Ref<const MatrixX<Polynomiald>>& polynomials,
                                                ...
                                                const Eigen::Ref<const Eigen::MatrixXd>& ub,
                                                const VariableRefList& vars) {
        return AddPolynomialConstraint(polynomials, poly_vars, lb, ub, ConcatenateVariableRefList(vars));
    }
    ...
    Binding<PositiveSemidefiniteConstraint> AddPositiveSemidefiniteConstraint(
        ...
        AddLinearEqualityConstraint(e - M, Eigen::MatrixXd::Zero(e.rows(), e.rows()), true);
    ...
    Binding<LinearMatrixInequalityConstraint> AddLinearMatrixInequalityConstraint(std::vector<Eigen::MatrixXd> F,
                                                                                  const VariableRefList& vars) {
        return AddLinearMatrixInequalityConstraint(std::move(F), ConcatenateVariableRefList(vars));
    }
    ...
    Binding<LinearMatrixInequalityConstraint> AddLinearMatrixInequalityConstraint(
            std::vector<Eigen::MatrixXd> F, const Eigen::Ref<const VectorXDecisionVariable>& vars);
     ...
     * @pydrake_mkdoc_identifier{expression}
     */
    Binding<LinearConstraint> AddPositiveDiagonallyDominantDualConeMatrixConstraint(
            const Eigen::Ref<const MatrixX<symbolic::Expression>>& X);
     ...
     * @pydrake_mkdoc_identifier{expression}
     */
    std::vector<std::vector<Matrix2<symbolic::Expression>>> AddScaledDiagonallyDominantMatrixConstraint(
            const Eigen::Ref<const MatrixX<symbolic::Expression>>& X);
     ...
     * @pydrake_mkdoc_identifier{variable}
     */
    std::vector<std::vector<Matrix2<symbolic::Variable>>> AddScaledDiagonallyDominantMatrixConstraint(
            const Eigen::Ref<const MatrixX<symbolic::Variable>>& X);
    //@}
     ...
     * to the variables in @p constraint.
     */
    std::vector<std::vector<Matrix2<symbolic::Variable>>> TightenPsdConstraintToSdd(
     ...
     *  @pydrake_mkdoc_identifier{expression}
     */
    std::vector<Binding<RotatedLorentzConeConstraint>> AddScaledDiagonallyDominantDualConeMatrixConstraint(
            const Eigen::Ref<const MatrixX<symbolic::Expression>>& X);
     ...
     * applied to the variables in @p constraint.
     */
    std::vector<Binding<RotatedLorentzConeConstraint>> RelaxPsdConstraintToSddDualCone(
            const Binding<PositiveSemidefiniteConstraint>& constraint);
     ...
     * indeterminates if necessary.
     */
    std::pair<MatrixXDecisionVariable, VectorX<symbolic::Monomial>> AddSosConstraint(
     ...
     */
    std::vector<Binding<LinearEqualityConstraint>> AddEqualityConstraintBetweenPolynomials(
     ...
     */
    [[nodiscard]] double GetInitialGuess(const symbolic::Variable& decision_variable) const;
    ...
    GetInitialGuess(const Eigen::MatrixBase<Derived>& decision_variable_mat) const {
        MatrixLikewise<double, Derived> decision_variable_values(decision_variable_mat.rows(),
                                                                 decision_variable_mat.cols());
        for (int i = 0; i < decision_variable_mat.rows(); ++i) {
            for (int j = 0; j < decision_variable_mat.cols(); ++j) {
                decision_variable_values(i, j) = GetInitialGuess(decision_variable_mat(i, j));
            }
        }
     ...
     */
    void SetSolverOptions(const SolverOptions& solver_options) { solver_options_ = solver_options; }
    //@}
     ...
     */
    const std::vector<Binding<Cost>>& generic_costs() const { return generic_costs_; }  // e.g. for snopt_user_fun
    ...
    const std::vector<Binding<LinearCost>>& linear_costs() const { return linear_costs_; }

    /** Getter for quadratic costs. */
    const std::vector<Binding<QuadraticCost>>& quadratic_costs() const { return quadratic_costs_; }

    /** Getter for l2norm costs. */
    const std::vector<Binding<L2NormCost>>& l2norm_costs() const { return l2norm_costs_; }
    ...
    const std::vector<Binding<LinearConstraint>>& linear_constraints() const { return linear_constraints_; }

    /** Getter for quadratic constraints. */
    const std::vector<Binding<QuadraticConstraint>>& quadratic_constraints() const { return quadratic_constraints_; }
    ...
    const std::vector<Binding<BoundingBoxConstraint>>& bounding_box_constraints() const { return bbox_constraints_; }
     ...
     */
    [[nodiscard]] std::vector<Binding<LinearConstraint>> GetAllLinearConstraints() const;
     ...
     */
    [[nodiscard]] int FindDecisionVariableIndex(const symbolic::Variable& var) const;
     ...
     */
    [[nodiscard]] size_t FindIndeterminateIndex(const symbolic::Variable& var) const;
    ...
    typename std::enable_if_t<is_eigen_vector<DerivedX>::value, VectorX<typename DerivedX::Scalar>> EvalBinding(
            const Binding<C>& binding, const Eigen::MatrixBase<DerivedX>& prog_var_vals) const {
        ...
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            binding_x(i) = prog_var_vals(FindDecisionVariableIndex(binding.variables()(i)));
        }
     ...
     */
    template <typename C, typename DerivedX>
    typename std::enable_if_t<is_eigen_vector<DerivedX>::value, VectorX<typename DerivedX::Scalar>> EvalBindings(
            const std::vector<Binding<C>>& bindings, const Eigen::MatrixBase<DerivedX>& prog_var_vals) const {
     ...
     */
    template <typename C, typename DerivedX>
    typename std::enable_if_t<is_eigen_vector<DerivedX>::value, VectorX<typename DerivedX::Scalar>>
    GetBindingVariableValues(const Binding<C>& binding, const Eigen::MatrixBase<DerivedX>& prog_var_vals) const {
        DRAKE_DEMAND(prog_var_vals.rows() == num_vars());
        VectorX<typename DerivedX::Scalar> result(binding.GetNumElements());
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            result(i) = prog_var_vals(FindDecisionVariableIndex(binding.variables()(i)));
        }
        return result;
    }
     ...
     * @throws std::exception if the size of `prog_var_vals` is invalid.
     */
    [[nodiscard]] bool CheckSatisfied(const Binding<Constraint>& binding,
     ...
     */
    [[nodiscard]] bool CheckSatisfied(const std::vector<Binding<Constraint>>& bindings,
                                      const Eigen::Ref<const Eigen::VectorXd>& prog_var_vals,
                                      double tol = 1e-6) const;
     ...
     * guess.
     */
    [[nodiscard]] bool CheckSatisfiedAtInitialGuess(const Binding<Constraint>& binding, double tol = 1e-6) const;
     ...
     * @pydrake_mkdoc_identifier{vector}
     */
    [[nodiscard]] bool CheckSatisfiedAtInitialGuess(const std::vector<Binding<Constraint>>& bindings,
                                                    double tol = 1e-6) const;

    /** Getter for all decision variables in the program. */
    Eigen::Map<const VectorX<symbolic::Variable>> decision_variables() const {
        return Eigen::Map<const VectorX<symbolic::Variable>>(decision_variables_.data(), decision_variables_.size());
    }
    ...
    Eigen::Map<const VectorX<symbolic::Variable>> indeterminates() const {
        return Eigen::Map<const VectorX<symbolic::Variable>>(indeterminates_.data(), indeterminates_.size());
    }
    ...
    const ProgramAttributes& required_capabilities() const { return required_capabilities_; }
     ...
     */
    const std::unordered_map<int, double>& GetVariableScaling() const { return var_scaling_map_; }
...
private:
    ...
    template <typename T>
    void NewVariables_impl(VarType type,
                           const T& names,
                           bool is_symmetric,
        ...
        for (int i = 0; i < num_new_vars; ++i) {
            ...
            decision_variable_index_.insert(std::make_pair(decision_variables_[new_var_index].get_id(), new_var_index));
            decision_variable_matrix(row_index, col_index) = decision_variables_[new_var_index];
    ...
    MatrixXDecisionVariable NewVariables(
            VarType type, int rows, int cols, bool is_symmetric, const std::vector<std::string>& names);

    template <typename T>
    void NewIndeterminates_impl(const T& names, Eigen::Ref<MatrixXIndeterminate> indeterminates_matrix) {
        ...
        for (int i = 0; i < num_new_vars; ++i) {
            ...
            indeterminates_index_.insert(std::make_pair(indeterminates_[new_var_index].get_id(), new_var_index));
            indeterminates_matrix(row_index, col_index) = indeterminates_[new_var_index];
     ...
     */
    template <int Rows, int Cols>
    MatrixDecisionVariable<Rows, Cols> NewVariables(VarType type,
                                                    const typename NewVariableNames<Rows, Cols>::type& names,
                                                    int rows,
                                                    int cols) {
    ...
    MatrixDecisionVariable<Rows, Rows> NewSymmetricVariables(
            VarType type, const typename NewSymmetricVariableNames<Rows>::type& names, int rows = Rows) {
     ...
     * variables.
     */
    symbolic::Polynomial NewFreePolynomialImpl(const symbolic::Variables& indeterminates,
                                               int degree,
    ...
    std::vector<Binding<RotatedLorentzConeConstraint>> rotated_lorentz_cone_constraint_;
    ...
    std::vector<Binding<LinearComplementarityConstraint>> linear_complementarity_constraints_;
