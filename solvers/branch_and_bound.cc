...
bool MathProgHasBinaryVariables(const MathematicalProgram& prog) {
    for (int i = 0; i < prog.num_vars(); ++i) {
        if (prog.decision_variable(i).get_type() == symbolic::Variable::Type::BINARY) {
            return true;
        }
    }
    return false;
...
Binding<Constraint> ReplaceBoundVariables(
        ...
        const std::unordered_map<symbolic::Variable::Id, symbolic::Variable>& map_old_vars_to_new_vars) {
...
void AddVectorOfConstraintsToProgram(
    ...
    for (const auto& constraint : constraints) {
        prog->AddConstraint(ReplaceBoundVariables(constraint, map_old_vars_to_new_vars));
    }
}
...
MixedIntegerBranchAndBoundNode::ConstructRootNode(const MathematicalProgram& prog, const SolverId& solver_id) {
    ...
    for (int i = 0; i < prog_vars.rows(); ++i) {
        switch (prog_vars(i).get_type()) {
            case symbolic::Variable::Type::CONTINUOUS: {
                ...
                map_old_vars_to_new_vars.emplace_hint(map_old_vars_to_new_vars.end(), prog_vars(i).get_id(),
                                                      new_vars(i));
                break;
            }
    ...
    new_prog.AddBoundingBoxConstraint(Eigen::VectorXd::Zero(num_binary_variables),
                                      Eigen::VectorXd::Ones(num_binary_variables), binary_variables);

    // Add the indeterminates
    new_prog.AddIndeterminates(prog.indeterminates());

    // Now add all the costs in prog to new_prog.
    AddVectorOfCostsToProgram(prog.generic_costs(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfCostsToProgram(prog.linear_costs(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfCostsToProgram(prog.quadratic_costs(), map_old_vars_to_new_vars, &new_prog);
    ...
    AddVectorOfConstraintsToProgram(prog.generic_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.linear_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.linear_equality_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.bounding_box_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.lorentz_cone_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.rotated_lorentz_cone_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.positive_semidefinite_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.linear_matrix_inequality_constraints(), map_old_vars_to_new_vars, &new_prog);
    AddVectorOfConstraintsToProgram(prog.linear_complementarity_constraints(), map_old_vars_to_new_vars, &new_prog);
    ...
    MixedIntegerBranchAndBoundNode* node =
            new MixedIntegerBranchAndBoundNode(new_prog, binary_variables_list, solver_id);
    node->solution_result_ = SolveProgramWithSolver(*node->prog_, solver_id, node->prog_result_.get());
    if (node->solution_result_ == SolutionResult::kSolutionFound) {
        node->CheckOptimalSolutionIsIntegral();
    }
    return std::make_pair(std::unique_ptr<MixedIntegerBranchAndBoundNode>(node), map_old_vars_to_new_vars);
}
...
void MixedIntegerBranchAndBoundNode::FixBinaryVariable(const symbolic::Variable& binary_variable, bool binary_value) {
    // Add constraint y == 0 or y == 1.
    prog_->AddBoundingBoxConstraint(static_cast<double>(binary_value), static_cast<double>(binary_value),
                                    binary_variable);
    // Remove binary_variable from remaining_binary_variables_
    bool found_binary_variable = false;
    for (auto it = remaining_binary_variables_.begin(); it != remaining_binary_variables_.end(); ++it) {
    ...
    if (!found_binary_variable) {
        ...
        oss << binary_variable << " is not a remaining binary variable in this node.\n";
...
void MixedIntegerBranchAndBoundNode::Branch(const symbolic::Variable& binary_variable) {
    left_child_.reset(new MixedIntegerBranchAndBoundNode(*prog_, remaining_binary_variables_, solver_id_));
    right_child_.reset(new MixedIntegerBranchAndBoundNode(*prog_, remaining_binary_variables_, solver_id_));
    ...
    left_child_->solution_result_ =
            SolveProgramWithSolver(*left_child_->prog_, left_child_->solver_id_, left_child_->prog_result_.get());
    right_child_->solution_result_ =
            SolveProgramWithSolver(*right_child_->prog_, right_child_->solver_id_, right_child_->prog_result_.get());
...
MixedIntegerBranchAndBound::MixedIntegerBranchAndBound(const MathematicalProgram& prog,
    ...
    : root_{nullptr},
      ...
      solutions_{} {
    std::tie(root_, map_old_vars_to_new_vars_) = MixedIntegerBranchAndBoundNode::ConstructRootNode(prog, solver_id);
    if (root_->solution_result() == SolutionResult::kSolutionFound) {
        ...
        if (root_->optimal_solution_is_integral()) {
            UpdateIntegralSolution(root_->prog_result()->GetSolution(root_->prog()->decision_variables()),
...
SolutionResult MixedIntegerBranchAndBound::Solve() {
    ...
    if (root_->solution_result() == SolutionResult::kSolutionFound && !root_->optimal_solution_is_integral()) {
        SearchIntegralSolutionByRounding(*root_);
    }
    ...
    while (branching_node) {
        ...
        // any more.
        if (options_.max_explored_nodes >= 1 && root_->NumExploredNodesInSubtree() + 2 > options_.max_explored_nodes) {
            ...
            const symbolic::Variable* branching_variable = PickBranchingVariable(*branching_node);
    ...
    throw std::runtime_error("Unknown result. The problem is not optimal, infeasible, nor unbounded.");
}

void MixedIntegerBranchAndBound::NodeCallback(const MixedIntegerBranchAndBoundNode& node) {
...
double MixedIntegerBranchAndBound::GetOptimalCost() const {
    if (solutions_.empty()) {
        throw std::runtime_error("The branch-and-bound process did not find an optimal solution.");
    }
    return solutions_.begin()->first;
}

double MixedIntegerBranchAndBound::GetSubOptimalCost(int nth_suboptimal_cost) const {
    if (nth_suboptimal_cost < 0 || nth_suboptimal_cost >= static_cast<int>(solutions().size()) - 1) {
        throw std::runtime_error(
    ...
    ++it;
    for (int suboptimal_cost_count = 0; suboptimal_cost_count < nth_suboptimal_cost; ++suboptimal_cost_count) {
        ++it;
    }
    return it->first;
}

double MixedIntegerBranchAndBound::GetSolution(const symbolic::Variable& mip_var, int nth_best_solution) const {
    if (nth_best_solution < 0 || nth_best_solution >= static_cast<int>(solutions().size())) {
        throw std::runtime_error(
    ...
    const int variable_index = root_->prog()->FindDecisionVariableIndex(GetNewVariable(mip_var));

    auto it = solutions().begin();
    for (int best_solution_count = 0; best_solution_count < nth_best_solution; ++best_solution_count) {
        ++it;
    }
    return it->second(variable_index);
}

const symbolic::Variable& MixedIntegerBranchAndBound::GetNewVariable(const symbolic::Variable& old_variable) const {
        ...
        case NodeSelectionMethod::kUserDefined: {
            if (node_selection_userfun_ != nullptr) {
                ...
                if (!node->IsLeaf() || IsLeafNodeFathomed(*node)) {
                    throw std::runtime_error("The user should pick an un-fathomed leaf node for branching.");
                }
...
MixedIntegerBranchAndBoundNode* PickMinLowerBoundNodeInSubTree(const MixedIntegerBranchAndBound& bnb,
...
MixedIntegerBranchAndBoundNode* PickDepthFirstNodeInSubTree(const MixedIntegerBranchAndBound& bnb,
                                                            const MixedIntegerBranchAndBoundNode& sub_tree_root) {
    if (sub_tree_root.IsLeaf()) {
        if (bnb.IsLeafNodeFathomed(sub_tree_root)) {
            return nullptr;
...
double BestLowerBoundInSubTree(const MixedIntegerBranchAndBound& bnb,
    ...
    if (sub_tree_root.IsLeaf()) {
        ...
        const double left_best_lower_bound = BestLowerBoundInSubTree(bnb, *(sub_tree_root.left_child()));
        const double right_best_lower_bound = BestLowerBoundInSubTree(bnb, *(sub_tree_root.right_child()));
        return left_best_lower_bound < right_best_lower_bound ? left_best_lower_bound : right_best_lower_bound;
    }
}

const symbolic::Variable* PickMostOrLeastAmbivalentAsBranchingVariable(
        const MixedIntegerBranchAndBoundNode& node,
        MixedIntegerBranchAndBound::VariableSelectionMethod variable_selection_method) {
    DRAKE_ASSERT(variable_selection_method == MixedIntegerBranchAndBound::VariableSelectionMethod::kMostAmbivalent ||
                 variable_selection_method == MixedIntegerBranchAndBound::VariableSelectionMethod::kLeastAmbivalent);
    if (node.solution_result() == SolutionResult::kSolutionFound) {
        const double sign =
                variable_selection_method == MixedIntegerBranchAndBound::VariableSelectionMethod::kMostAmbivalent ? 1
                                                                                                                  : -1;
...
MixedIntegerBranchAndBoundNode* MixedIntegerBranchAndBound::PickMinLowerBoundNode() const {
...
const symbolic::Variable* MixedIntegerBranchAndBound::PickBranchingVariable(
    ...
    switch (variable_selection_method_) {
        ...
        case VariableSelectionMethod::kLeastAmbivalent:
            return PickMostOrLeastAmbivalentAsBranchingVariable(node, variable_selection_method_);
...
bool MixedIntegerBranchAndBound::IsLeafNodeFathomed(const MixedIntegerBranchAndBoundNode& leaf_node) const {
    if (!leaf_node.IsLeaf()) {
    ...
    if (leaf_node.solution_result() == SolutionResult::kSolutionFound && leaf_node.optimal_solution_is_integral()) {
        return true;
    }
...
void MixedIntegerBranchAndBound::BranchAndUpdate(MixedIntegerBranchAndBoundNode* node,
    ...
    for (auto& child : {node->left_child(), node->right_child()}) {
        if (child->solution_result() == SolutionResult::kSolutionFound && child->optimal_solution_is_integral()) {
            const double child_node_optimal_cost = child->prog_result()->get_optimal_cost();
            const Eigen::VectorXd x_sol = child->prog_result()->GetSolution(child->prog()->decision_variables());
...
void MixedIntegerBranchAndBound::UpdateIntegralSolution(const Eigen::Ref<const Eigen::VectorXd>& solution,
                                                        double cost) {
...
bool MixedIntegerBranchAndBound::HasConverged() const {
    ...
    if ((best_upper_bound_ - best_lower_bound_) / std::abs(best_lower_bound_) <= relative_gap_tol_) {
        return true;
    }
    return false;
}

void MixedIntegerBranchAndBound::SearchIntegralSolutionByRounding(const MixedIntegerBranchAndBoundNode& node) {
    ...
    if (node.solution_result() == SolutionResult::kSolutionFound && !node.optimal_solution_is_integral()) {
        ...
        for (const auto& remaining_binary_variable : node.remaining_binary_variables()) {
            ...
            const double roundoff_integer_val = std::round(node.prog_result()->GetSolution(remaining_binary_variable));
            new_prog->AddBoundingBoxConstraint(roundoff_integer_val, roundoff_integer_val, remaining_binary_variable);
        }
        MathematicalProgramResult result;
        SolveProgramWithSolver(*new_prog, node.solver_id(), &result);
        if (result.is_success()) {
            // Found integral solution.
            UpdateIntegralSolution(result.GetSolution(new_prog->decision_variables()), result.get_optimal_cost());
        }
    }
}
}  // namespace solvers
