        ...
        for (int i = 0; i < 10; ++i) {
            ...
            for (int j = 0; j < p_i.rows(); ++j) {
                ...
                for (int k = 0; k < monomial_basis.rows(); ++k) {
                    p_map.emplace(monomial_basis(k), static_cast<double>(i * j) / 100.0 + k / 10.0 + 1);
                }
...
static void BenchmarkSosProgram3(benchmark::State& state) {  // NOLINT
    for (auto _ : state) {
        ...
        using std::pow;
        const symbolic::Expression one_step_cost = (z - z0).dot(z - z0) + pow(u_opt, 2);
        const Vector3<symbolic::Expression> f(z(1) + z(2), -z(0) + z(2), (z(0) + u_opt - z(2)));
