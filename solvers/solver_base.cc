...
SolverBase::SolverBase(const SolverId& id,
...
MathematicalProgramResult SolverBase::Solve(const MathematicalProgram& prog,
    ...
    *result = {};
    if (!available()) {
        ...
        throw std::invalid_argument(
                fmt::format("{} cannot Solve because {}::available() is false, i.e.,"
    ...
    if (!enabled()) {
        const std::string name = ShortName(*this);
        throw std::invalid_argument(
                fmt::format("{} cannot Solve because {}::enabled() is false, i.e.,"
    ...
    const Eigen::VectorXd& x_init = initial_guess ? *initial_guess : prog.initial_guess();
    if (x_init.rows() != prog.num_vars()) {
        throw std::invalid_argument(
                fmt::format("Solve expects initial guess of size {}, got {}.", prog.num_vars(), x_init.rows()));
    }
    if (!solver_options) {
...
bool SolverBase::AreProgramAttributesSatisfied(const MathematicalProgram& prog) const {
    DRAKE_DEMAND(default_are_satisfied_ != nullptr);
    return default_are_satisfied_(prog);
}

std::string SolverBase::ExplainUnsatisfiedProgramAttributes(const MathematicalProgram& prog) const {
    ...
    return fmt::format("{} is unable to solve a MathematicalProgram with {}.", ShortName(*this),
