...
bool CheckAttributes(const MathematicalProgram& prog, std::string* explanation) {
    ...
    return internal::CheckConvexSolverAttributes(prog, solver_capabilities.access(), "ClpSolver", explanation);
}
}  // namespace
...
std::string ClpSolver::UnsatisfiedProgramAttributes(const MathematicalProgram& prog) {
