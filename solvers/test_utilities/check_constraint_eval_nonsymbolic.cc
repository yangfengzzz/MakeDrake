...
void CheckConstraintEvalNonsymbolic(const Constraint& constraint,
    ...
    std::function<void(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd*)> constraint_eval =
    ...
    const auto J = math::ComputeNumericalGradient(
            constraint_eval, x_double, math::NumericalGradientOption{math::NumericalGradientMethod::kCentral});
    EXPECT_TRUE(
            CompareMatrices(J * math::ExtractGradient(x_autodiff), math::ExtractGradient(y_autodiff), std::sqrt(tol)));
}
}  // namespace test
}  // namespace solvers
}  // namespace drake
