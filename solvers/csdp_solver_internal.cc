...
void ConvertSparseMatrixFormatToCsdpProblemData(const std::vector<BlockInX>& X_blocks,
    ...
    int row_count = 0;
    for (int block_index = 0; block_index < static_cast<int>(X_blocks.size()); ++block_index) {
        block_start_rows[block_index] = row_count;
        for (int row = row_count; row < row_count + X_blocks[block_index].num_rows; ++row) {
    ...
    C_csdp->blocks = static_cast<struct csdp::blockrec*>(malloc((C_csdp->nblocks + 1) * sizeof(struct csdp::blockrec)));
    for (int block_index = 0; block_index < C_csdp->nblocks; ++block_index) {
        ...
        C_block.blockcategory = X_block.block_type == BlockType::kMatrix ? csdp::MATRIX : csdp::DIAG;
        ...
        if (X_block.block_type == BlockType::kMatrix) {
            ...
            for (int j = 0; j < X_block.num_rows; ++j) {
                ...
                for (Eigen::SparseMatrix<double>::InnerIterator it(C, block_start_rows[block_index] + j); it; ++it) {
                    C_block.data.mat[CsdpMatrixIndex(it.row() - block_start_rows[block_index], j, X_block.num_rows)] =
                            it.value();
                }
            }
        } else if (X_block.block_type == BlockType::kDiagonal) {
            // CSDP uses Fortran 1-index array, so the 0'th entry is wasted.
            C_block.data.vec = static_cast<double*>(malloc((X_block.num_rows + 1) * sizeof(double)));
            for (int j = 0; j < X_block.num_rows; ++j) {
    ...
    *constraints = static_cast<struct csdp::constraintmatrix*>(
            malloc((static_cast<int>(A.size()) + 1) * sizeof(struct csdp::constraintmatrix)));
    for (int constraint_index = 0; constraint_index < static_cast<int>(A.size()); ++constraint_index) {
        ...
        for (int block_index = static_cast<int>(X_blocks.size() - 1); block_index >= 0; --block_index) {
            ...
            // A[constraint_index].
            A_block_triplets.reserve((X_blocks[block_index].num_rows + 1) * X_blocks[block_index].num_rows / 2);
            for (int col_index = block_start_rows[block_index];
                 col_index < block_start_rows[block_index] + X_blocks[block_index].num_rows; ++col_index) {
                for (Eigen::SparseMatrix<double>::InnerIterator it(A[constraint_index], col_index); it; ++it) {
                    if (it.row() > it.col()) {
                        break;
                    }
                    A_block_triplets.emplace_back(it.row() - block_start_rows[block_index] + 1,
                                                  it.col() - block_start_rows[block_index] + 1, it.value());
                }
            }
            if (!A_block_triplets.empty()) {
                struct csdp::sparseblock* blockptr =
                        static_cast<struct csdp::sparseblock*>(malloc(sizeof(struct csdp::sparseblock)));
                ...
                blockptr->entries =
                        static_cast<double*>(malloc((static_cast<int>(A_block_triplets.size()) + 1) * sizeof(double)));
                blockptr->iindices =
                        static_cast<int*>(malloc((static_cast<int>(A_block_triplets.size()) + 1) * sizeof(int)));
                blockptr->jindices =
                        static_cast<int*>(malloc((1 + static_cast<int>(A_block_triplets.size())) * sizeof(int)));
...
void GenerateCsdpProblemDataWithoutFreeVariables(const SdpaFreeFormat& sdpa_free_format,
    ...
    if (sdpa_free_format.num_free_variables() == 0) {
        Eigen::SparseMatrix<double> C(sdpa_free_format.num_X_rows(), sdpa_free_format.num_X_rows());
        C.setFromTriplets(sdpa_free_format.C_triplets().begin(), sdpa_free_format.C_triplets().end());
        std::vector<Eigen::SparseMatrix<double>> A;
        A.reserve(sdpa_free_format.A_triplets().size());
        for (int i = 0; i < static_cast<int>(sdpa_free_format.A_triplets().size()); ++i) {
            A.emplace_back(sdpa_free_format.num_X_rows(), sdpa_free_format.num_X_rows());
            A.back().setFromTriplets(sdpa_free_format.A_triplets()[i].begin(), sdpa_free_format.A_triplets()[i].end());
        }
        ConvertSparseMatrixFormatToCsdpProblemData(sdpa_free_format.X_blocks(), C, A, sdpa_free_format.g(), C_csdp,
...
void ConvertCsdpBlockMatrixtoEigen(const csdp::blockmatrix& X_csdp, Eigen::SparseMatrix<double>* X) {
    ...
    for (int i = 0; i < X_csdp.nblocks; ++i) {
        if (X_csdp.blocks[i + 1].blockcategory == csdp::MATRIX) {
            num_X_nonzero_entries += X_csdp.blocks[i + 1].blocksize * X_csdp.blocks[i + 1].blocksize;
        } else if (X_csdp.blocks[i + 1].blockcategory == csdp::DIAG) {
            num_X_nonzero_entries += X_csdp.blocks[i + 1].blocksize;
        } else {
            throw std::runtime_error("ConvertCsdpBlockMatrixtoEigen(): unknown block category.");
        }
    }
    ...
    for (int block_index = 0; block_index < X_csdp.nblocks; ++block_index) {
        if (X_csdp.blocks[block_index + 1].blockcategory == csdp::MATRIX) {
            for (int i = 0; i < X_csdp.blocks[block_index + 1].blocksize; ++i) {
                for (int j = 0; j < X_csdp.blocks[block_index + 1].blocksize; ++j) {
                    X_triplets.emplace_back(
                            ...
                            X_csdp.blocks[block_index + 1]
                                    .data.mat[CsdpMatrixIndex(i, j, X_csdp.blocks[block_index + 1].blocksize)]);
                }
            }
            ...
            throw std::runtime_error("ConvertCsdpBlockMatrixtoEigen(): unknown block matrix type.");
        }
