 ...
 */
IntegerVectorList BuildAlphabetFromBounds(const Eigen::VectorXi& lower_bound, const Eigen::VectorXi& upper_bound) {
...
SolutionList FeasiblePoints(const Eigen::MatrixXi& A,
    ...
    for (const auto& value : column_alphabets[last_free_var_pos]) {
        ...
        if (last_free_var_pos == 0) {
            ...
            new_feasible_points = CartesianProduct(FeasiblePoints(A, b - A.col(last_free_var_pos) * value,
...
SolutionList EnumerateIntegerSolutions(const Eigen::Ref<const Eigen::MatrixXi>& A,
