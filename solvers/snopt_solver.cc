...
struct SnoptImpl<false> {
    ...
    template <typename Int>
    static void sninit(const char* name, int len, int summOn, Int* iw, int leniw, double* rw, int lenrw) {
  ...
  static void snkera(
    ...
    template <typename Int>
    static void snseti(const char* buffer, int len, int iopt, int* errors, Int* iw, int leniw, double* rw, int lenrw) {
        ::f_snseti(buffer, &len, &iopt, errors, iw, &leniw, rw, &lenrw);
    }
    template <typename Int>
    static void snsetr(
            const char* buffer, int len, double rvalue, int* errors, Int* iw, int leniw, double* rw, int lenrw) {
        ::f_snsetr(buffer, &len, &rvalue, errors, iw, &leniw, rw, &lenrw);
    }
    template <typename Int>
    static void snset(const char* buffer, int len, int* errors, Int* iw, int leniw, double* rw, int lenrw) {
...
#pragma GCC diagnostic pop
const bool kIsSnopt76 = std::is_same_v<decltype(&f_sninit), decltype(&f_sninit_76_prototype)>;
...
public:
    ...
    std::set<int>& nonlinear_cost_gradient_indices() { return nonlinear_cost_gradient_indices_; }
    const std::set<int>& nonlinear_cost_gradient_indices() const { return nonlinear_cost_gradient_indices_; }

    std::vector<int>& duplicate_to_G_index_map() { return duplicate_to_G_index_map_; }

    const std::vector<int>& duplicate_to_G_index_map() const { return duplicate_to_G_index_map_; }
    ...
    std::optional<std::string>& userfun_error_message() { return userfun_error_message_; }

    int* iu() const { return const_cast<int*>(this_pointer_as_int_array_.data()); }
    ...
    static SnoptUserFunInfo& GetFrom(const int* iu, int leniu) {
        ...
        std::copy(reinterpret_cast<const char*>(iu), reinterpret_cast<const char*>(iu) + sizeof(result),
...
private:
    ...
    // remainder.
    static constexpr size_t kIntCount = (sizeof(SnoptUserFunInfo*) + sizeof(int) - 1) / sizeof(int);
    ...
    std::array<int, kIntCount> MakeThisAsInts() {
        ...
        std::copy(reinterpret_cast<const char*>(&value), reinterpret_cast<const char*>(&value) + sizeof(value),
...
public:
    ...
    explicit WorkspaceStorage(const SnoptUserFunInfo* user_info) : user_info_(user_info) {
...
template <>
int SingleNonlinearConstraintSize<LinearComplementarityConstraint>(const LinearComplementarityConstraint& constraint) {
    return 1;
}
...
void EvaluateNonlinearConstraints(const MathematicalProgram& prog,
                                  ...
                                  double F[],
                                  std::vector<double>* G_w_duplicate,
    ...
    for (const auto& binding : constraint_list) {
        ...
        for (int i = 0; i < num_variables; ++i) {
            binding_var_indices[i] = prog.FindDecisionVariableIndex(binding.variables()(i));
        ...
        const std::optional<std::vector<std::pair<int, int>>>& gradient_sparsity_pattern =
...
template <typename C>
void GetNonlinearCostNonzeroGradientIndices(const MathematicalProgram& prog,
                                            const std::vector<Binding<C>>& costs,
 ...
 */
std::set<int> GetAllNonlinearCostNonzeroGradientIndices(const MathematicalProgram& prog) {
    ...
    std::set<int> cost_gradient_indices;
    GetNonlinearCostNonzeroGradientIndices(prog, prog.quadratic_costs(), &cost_gradient_indices);
    GetNonlinearCostNonzeroGradientIndices(prog, prog.l2norm_costs(), &cost_gradient_indices);
    GetNonlinearCostNonzeroGradientIndices(prog, prog.generic_costs(), &cost_gradient_indices);
 ...
 */
template <typename C>
void EvaluateAndAddNonlinearCosts(const MathematicalProgram& prog,
                                  const std::vector<Binding<C>>& nonlinear_costs,
                                  const Eigen::VectorXd& x,
                                  double* total_cost,
                                  std::vector<double>* nonlinear_cost_gradients) {
    const auto& scale_map = prog.GetVariableScaling();
    for (const auto& binding : nonlinear_costs) {
        ...
        *total_cost += ty(0).value();
        if (ty(0).derivatives().size() > 0) {
            for (int i = 0; i < num_variables; ++i) {
                (*nonlinear_cost_gradients)[binding_var_indices[i]] += ty(0).derivatives()(i);
            }
        }
    }
}
...
void EvaluateAllNonlinearCosts(const MathematicalProgram& prog,
                               const Eigen::VectorXd& xvec,
    ...
    // Quadratic costs.
    EvaluateAndAddNonlinearCosts(prog, prog.quadratic_costs(), xvec, &(F[0]), &cost_gradients);
    // L2Norm costs.
    EvaluateAndAddNonlinearCosts(prog, prog.l2norm_costs(), xvec, &(F[0]), &cost_gradients);
    // Generic costs.
    EvaluateAndAddNonlinearCosts(prog, prog.generic_costs(), xvec, &(F[0]), &cost_gradients);
...
void EvaluateCostsConstraints(const SnoptUserFunInfo& info, int n, double x[], double F[], double G[]) {
    ...
    EvaluateAllNonlinearCosts(current_problem, xvec, info.nonlinear_cost_gradient_indices(), F, &G_w_duplicate,
                              &grad_index);
    ...
    EvaluateNonlinearConstraints(current_problem, current_problem.generic_constraints(), F, &G_w_duplicate,
                                 &constraint_index, &grad_index, xvec);
    EvaluateNonlinearConstraints(current_problem, current_problem.quadratic_constraints(), F, &G_w_duplicate,
                                 &constraint_index, &grad_index, xvec);
    EvaluateNonlinearConstraints(current_problem, current_problem.lorentz_cone_constraints(), F, &G_w_duplicate,
                                 &constraint_index, &grad_index, xvec);
    EvaluateNonlinearConstraints(current_problem, current_problem.rotated_lorentz_cone_constraints(), F, &G_w_duplicate,
                                 &constraint_index, &grad_index, xvec);
    EvaluateNonlinearConstraints(current_problem, current_problem.linear_complementarity_constraints(), F,
                                 &G_w_duplicate, &constraint_index, &grad_index, xvec);

    for (int i = 0; i < static_cast<int>(info.duplicate_to_G_index_map().size()); ++i) {
...
void snopt_userfun(int* Status,
    ...
    try {
        ...
        info.userfun_error_message() =
                fmt::format("Exception while evaluating SNOPT costs and constraints: '{}'", e.what());
...
void UpdateNumNonlinearConstraintsAndGradients(
    ...
    for (auto const& binding : constraint_list) {
        ...
        if (binding.evaluator()->gradient_sparsity_pattern().has_value()) {
            *max_num_gradients += static_cast<int>(binding.evaluator()->gradient_sparsity_pattern().value().size());
        } else {
            *max_num_gradients += n * binding.GetNumElements();
        }
        const Binding<Constraint> binding_cast = internal::BindingDynamicCast<Constraint>(binding);
        constraint_dual_start_index->emplace(binding_cast, 1 + *num_nonlinear_constraints);
...
void UpdateConstraintBoundsAndGradients(const MathematicalProgram& prog,
                                        ...
                                        std::vector<int>* iGfun_w_duplicate,
                                        std::vector<int>* jGvar_w_duplicate,
                                        size_t* constraint_index,
                                        size_t* grad_index) {
    for (auto const& binding : constraint_list) {
        auto const& c = binding.evaluator();
        ...
        const std::vector<int> bound_var_indices_in_prog = prog.FindDecisionVariableIndices(binding.variables());
        ...
        if (gradient_sparsity_pattern.has_value()) {
            for (const auto& nonzero_entry : gradient_sparsity_pattern.value()) {
                ...
                (*iGfun_w_duplicate)[*grad_index] = 1 + *constraint_index + nonzero_entry.first;
                (*jGvar_w_duplicate)[*grad_index] = 1 + bound_var_indices_in_prog[nonzero_entry.second];
                (*grad_index)++;
            }
        } else {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < static_cast<int>(binding.GetNumElements()); ++j) {
                    // Fortran is 1-indexed.
                    (*iGfun_w_duplicate)[*grad_index] = 1 + *constraint_index + i;  // row order
...
void UpdateConstraintBoundsAndGradients<LinearComplementarityConstraint>(
    ...
    for (const auto& binding : constraint_list) {
        ...
        for (int j = 0; j < binding.evaluator()->M().rows(); ++j) {
            ...
            (*jGvar_w_duplicate)[*grad_index] = 1 + prog.FindDecisionVariableIndex(binding.variables()(j));
            (*grad_index)++;
        }
        ++(*constraint_index);
    }
...
template <>
int LinearConstraintSize<LinearComplementarityConstraint>(const LinearComplementarityConstraint& constraint) {
...
template <>
std::pair<Eigen::VectorXd, Eigen::VectorXd> LinearConstraintBounds<LinearComplementarityConstraint>(
        const LinearComplementarityConstraint& constraint) {
    return std::make_pair(-constraint.q(),
                          Eigen::VectorXd::Constant(constraint.q().rows(), std::numeric_limits<double>::infinity()));
}

void UpdateLinearCost(const MathematicalProgram& prog,
    ...
    for (const auto& binding : prog.linear_costs()) {
        *linear_cost_constant_term += binding.evaluator()->b();
        for (int k = 0; k < static_cast<int>(binding.GetNumElements()); ++k) {
            const int variable_index = prog.FindDecisionVariableIndex(binding.variables()(k));
            ...
            (*variable_to_coefficient_map)[variable_index] += binding.evaluator()->a()(k) * scale;
        }
    }
}
...
void UpdateLinearConstraint(const MathematicalProgram& prog,
    ...
    for (auto const& binding : linear_constraints) {
        ...
        for (int k = 0; k < static_cast<int>(binding.GetNumElements()); ++k) {
            for (Eigen::SparseMatrix<double>::InnerIterator it(A_constraint, k); it; ++it) {
                tripletList->emplace_back(*linear_constraint_index + it.row(),
...
void SetVariableBounds(
        ...
        std::unordered_map<Binding<BoundingBoxConstraint>, BoundingBoxDualIndices>* bb_con_dual_variable_indices) {
    ...
    for (const auto& binding : prog.bounding_box_constraints()) {
        std::vector<int> lower_dual_indices(binding.evaluator()->num_constraints(), -1);
        std::vector<int> upper_dual_indices(binding.evaluator()->num_constraints(), -1);
        for (int k = 0; k < static_cast<int>(binding.GetNumElements()); ++k) {
        ...
        bb_con_dual_variable_indices->emplace(binding, std::make_pair(lower_dual_indices, upper_dual_indices));
    }
    const auto& scale_map = prog.GetVariableScaling();
...
void SetBoundingBoxConstraintDualSolution(
        const MathematicalProgram& prog,
        const Eigen::VectorXd& xmul,
        const std::unordered_map<Binding<BoundingBoxConstraint>, BoundingBoxDualIndices>& bb_con_dual_variable_indices,
    ...
    for (const auto& binding : prog.bounding_box_constraints()) {
        ...
        std::tie(lower_dual_indices, upper_dual_indices) = bb_con_dual_variable_indices.at(binding);
        Eigen::VectorXd dual_solution = Eigen::VectorXd::Zero(binding.GetNumElements());
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            ...
            } else if (upper_dual_indices[i] != -1 && xmul(upper_dual_indices[i]) <= 0) {
...
template <typename C>
void SetConstraintDualSolution(const std::vector<Binding<C>>& bindings,
                               const Eigen::VectorXd& Fmul,
                               const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_index,
                               MathematicalProgramResult* result) {
    for (const auto& binding : bindings) {
        const Binding<Constraint> binding_cast = internal::BindingDynamicCast<Constraint>(binding);

        result->set_dual_solution(
                binding, Fmul.segment(constraint_dual_start_index.at(binding), binding.evaluator()->num_constraints()));
    }
}

void SetConstraintDualSolutions(const MathematicalProgram& prog,
                                const Eigen::VectorXd& Fmul,
                                const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_index,
                                MathematicalProgramResult* result) {
    SetConstraintDualSolution(prog.generic_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.quadratic_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.lorentz_cone_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.rotated_lorentz_cone_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.rotated_lorentz_cone_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.linear_constraints(), Fmul, constraint_dual_start_index, result);
    SetConstraintDualSolution(prog.linear_equality_constraints(), Fmul, constraint_dual_start_index, result);
}
...
void SetMathematicalProgramResult(
        ...
        MathematicalProgramResult* result) {
    SnoptSolverDetails& solver_details = result->SetSolverDetailsType<SnoptSolverDetails>();
    // Populate our results structure.
    const SolutionResult solution_result = MapSnoptInfoToSolutionResult(snopt_status);
    result->set_solution_result(solution_result);
    result->set_x_val(x_val);
    SetBoundingBoxConstraintDualSolution(prog, solver_details.xmul, bb_con_dual_variable_indices, result);
    SetConstraintDualSolutions(prog, solver_details.Fmul, constraint_dual_start_index, result);
...
void UpdateNumConstraintsAndGradients(const MathematicalProgram& prog,
    ...
    UpdateNumNonlinearConstraintsAndGradients(prog.quadratic_constraints(), num_nonlinear_constraints,
                                              max_num_gradients, constraint_dual_start_index);
    UpdateNumNonlinearConstraintsAndGradients(prog.lorentz_cone_constraints(), num_nonlinear_constraints,
                                              max_num_gradients, constraint_dual_start_index);
    UpdateNumNonlinearConstraintsAndGradients(prog.rotated_lorentz_cone_constraints(), num_nonlinear_constraints,
                                              max_num_gradients, constraint_dual_start_index);
    UpdateNumNonlinearConstraintsAndGradients(prog.linear_complementarity_constraints(), num_nonlinear_constraints,
                                              max_num_gradients, constraint_dual_start_index);

    // Update linear constraints.
    for (auto const& binding : linear_constraints) {
        const Binding<Constraint> binding_cast = internal::BindingDynamicCast<Constraint>(binding);
        constraint_dual_start_index->emplace(binding_cast, 1 + *num_nonlinear_constraints + *num_linear_constraints);
...
void PruneGradientDuplication(int nx,
                              const std::vector<int>& iGfun_w_duplicate,
    ...
    for (int i = 0; i < static_cast<int>(iGfun_w_duplicate.size()); ++i) {
        const int index = gradient_index(iGfun_w_duplicate[i], jGvar_w_duplicate[i]);
...
void SolveWithGivenOptions(const MathematicalProgram& prog,
    ...
    Snopt::sninit(print_file_name.c_str(), print_file_name.length(), 0 /* no summary */, storage.iw(), storage.leniw(),
    ...
    user_info.nonlinear_cost_gradient_indices() = GetAllNonlinearCostNonzeroGradientIndices(prog);
    ...
    UpdateNumConstraintsAndGradients(prog, linear_constraints, &num_nonlinear_constraints, &num_linear_constraints,
                                     &max_num_gradients, &constraint_dual_start_index);
    ...
    size_t grad_index = 0;
    for (const auto cost_gradient_index : user_info.nonlinear_cost_gradient_indices()) {
        // Fortran is 1-indexed.
    ...
    size_t constraint_index = 1;
    UpdateConstraintBoundsAndGradients(prog, prog.generic_constraints(), &Flow, &Fupp, &iGfun_w_duplicate,
                                       &jGvar_w_duplicate, &constraint_index, &grad_index);
    UpdateConstraintBoundsAndGradients(prog, prog.quadratic_constraints(), &Flow, &Fupp, &iGfun_w_duplicate,
                                       &jGvar_w_duplicate, &constraint_index, &grad_index);
    UpdateConstraintBoundsAndGradients(prog, prog.lorentz_cone_constraints(), &Flow, &Fupp, &iGfun_w_duplicate,
                                       &jGvar_w_duplicate, &constraint_index, &grad_index);
    UpdateConstraintBoundsAndGradients(prog, prog.rotated_lorentz_cone_constraints(), &Flow, &Fupp, &iGfun_w_duplicate,
                                       &jGvar_w_duplicate, &constraint_index, &grad_index);
    UpdateConstraintBoundsAndGradients(prog, prog.linear_complementarity_constraints(), &Flow, &Fupp,
    ...
    UpdateLinearCost(prog, &variable_to_linear_cost_coefficient, &linear_cost_constant_term);
    ...
    UpdateLinearConstraint(prog, linear_constraints, &linear_constraints_triplets, &Flow, &Fupp, &constraint_index,
                           &linear_constraint_index);
    UpdateLinearConstraint(prog, prog.linear_complementarity_constraints(), &linear_constraints_triplets, &Flow, &Fupp,
    ...
    for (auto& triplet : linear_constraints_triplets) {
        ...
        if (it != scale_map.end()) {
            triplet = Eigen::Triplet<double>(triplet.row(), triplet.col(), triplet.value() * (it->second));
        }
    }

    Eigen::SparseMatrix<double> linear_constraints_A(nF - 1, prog.num_vars());
    // setFromTriplets sums up the duplicated entries.
    linear_constraints_A.setFromTriplets(linear_constraints_triplets.begin(), linear_constraints_triplets.end());
    int lenA = variable_to_linear_cost_coefficient.size() + linear_constraints_A.nonZeros();
    ...
    for (int i = 0; i < linear_constraints_A.outerSize(); ++i) {
        for (Eigen::SparseMatrix<double>::InnerIterator it(linear_constraints_A, i); it; ++it) {
            A[A_index] = it.value();
            // Fortran uses 1-index
    ...
    PruneGradientDuplication(nx, iGfun_w_duplicate, jGvar_w_duplicate, &duplicate_to_G_index_map, &iGfun, &jGvar);
    const int lenG = iGfun.size();
    user_info.set_lenG(lenG);

    for (const auto& it : snopt_options_double) {
        int errors = 0;
        Snopt::snsetr(it.first.c_str(), it.first.length(), it.second, &errors, storage.iw(), storage.leniw(),
                      storage.rw(), storage.lenrw());
        if (errors > 0) {
            throw std::runtime_error("Error setting Snopt double parameter " + it.first);
        }
    }

    for (const auto& it : snopt_options_int) {
        int errors = 0;
        Snopt::snseti(it.first.c_str(), it.first.length(), it.second, &errors, storage.iw(), storage.leniw(),
                      storage.rw(), storage.lenrw());
        if (errors > 0) {
            throw std::runtime_error("Error setting Snopt integer parameter " + it.first);
        }
    }
    ...
    for (const auto& it : snopt_options_string) {
        ...
        Snopt::snset(option_string.c_str(), option_string.length(), &errors, storage.iw(), storage.leniw(),
                     storage.rw(), storage.lenrw());
        if (errors > 0) {
            throw std::runtime_error("Error setting Snopt string parameter " + it.first);
        }
    }
    ...
    int miniw, minrw;
    int snopt_status{0};
    Snopt::snmema(&snopt_status, nF, nx, lenA, lenG, &miniw, &minrw, storage.iw(), storage.leniw(), storage.rw(),
                  storage.lenrw());
    ...
    if (miniw > storage.leniw()) {
        ...
        int errors;
        Snopt::snseti(option.c_str(), option.length(), storage.leniw(), &errors, storage.iw(), storage.leniw(),
                      storage.rw(), storage.lenrw());
    ...
    if (minrw > storage.lenrw()) {
        ...
        int errors;
        Snopt::snseti(option.c_str(), option.length(), storage.lenrw(), &errors, storage.iw(), storage.leniw(),
                      storage.rw(), storage.lenrw());
        // TODO(hongkai.dai): report the error in SnoptSolverDetails.
    }
    // Actual solve.
  ...
  Snopt::snkera(Cold, problem_name, nF, nx, objective_constant, ObjRow,
    ...
    SetMathematicalProgramResult(prog, snopt_status, x_val, bb_con_dual_variable_indices, constraint_dual_start_index,
                                 objective_constant, result);
}
...
void SnoptSolver::DoSolve(const MathematicalProgram& prog,
    ...
    // Call SNOPT.
    std::unordered_map<std::string, int> int_options = merged_options.GetOptionsInt(id());
    ...
    SolveWithGivenOptions(prog, initial_guess, merged_options.GetOptionsStr(id()), int_options,
                          merged_options.GetOptionsDouble(id()), merged_options.get_print_file_name(), result);
}
