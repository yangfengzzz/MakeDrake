#include "solvers/ipopt_solver_internal.h"
...
template <typename C>
void SetConstraintDualVariableIndex(const Binding<C>& binding,
...
void SetBoundingBoxConstraintDualSolution(
    ...
    for (const auto& binding : prog.bounding_box_constraints()) {
        ...
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            ...
            if (upper_dual_indices[i] != -1 && z_U[upper_dual_indices[i]] >= dual_solution(i)) {
...
template <typename C>
void SetConstraintDualSolution(const Binding<C>& binding,
    ...
    // to negate lambda.
    result->set_dual_solution(binding_cast, -lambda.segment(constraint_dual_start_index.at(binding_cast),
                                                            binding.evaluator()->num_constraints()));
}

void SetAllConstraintDualSolution(const MathematicalProgram& prog,
                                  const Eigen::VectorXd& lambda,
                                  const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_index,
                                  MathematicalProgramResult* result) {
    for (const auto& binding : prog.generic_constraints()) {
        SetConstraintDualSolution(binding, lambda, constraint_dual_start_index, result);
    }
...
size_t GetGradientMatrix(const MathematicalProgram& prog,
                         const Constraint& c,
    ...
    const std::optional<std::vector<std::pair<int, int>>>& sparsity_pattern = c.gradient_sparsity_pattern();
...
bool IpoptSolver_NLP::get_bounds_info(Index n, Number* x_l, Number* x_u, Index m, Number* g_l, Number* g_u) {
    unused(m);
    ...
    for (const auto& binding : problem_->bounding_box_constraints()) {
        ...
        bb_con_dual_variable_indices_.emplace(binding, std::make_pair(lower_dual_indices, upper_dual_indices));
    }

    size_t constraint_idx = 0;  // offset into g_l and g_u output arrays
    for (const auto& c : problem_->generic_constraints()) {
        SetConstraintDualVariableIndex(c, constraint_idx, &constraint_dual_start_index_);
        constraint_idx += GetConstraintBounds(*(c.evaluator()), g_l + constraint_idx, g_u + constraint_idx);
    }
    for (const auto& c : problem_->quadratic_constraints()) {
        SetConstraintDualVariableIndex(c, constraint_idx, &constraint_dual_start_index_);
        constraint_idx += GetConstraintBounds(*(c.evaluator()), g_l + constraint_idx, g_u + constraint_idx);
...
bool IpoptSolver_NLP::get_starting_point(Index n,
                                         bool init_x,
                                         Number* x,
                                         bool init_z,
                                         Number* z_L,
...
// NOLINTNEXTLINE(runtime/references); this is built into ipopt's API.
bool IpoptSolver_NLP::eval_f(Index n, const Number* x, bool new_x, Number& obj_value) {
...
bool IpoptSolver_NLP::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f) {
    if (new_x || !cost_cache_->is_x_equal(n, x)) {
        EvaluateCosts(n, x);
    }
...
bool IpoptSolver_NLP::eval_g(Index n, const Number* x, bool new_x, Index m, Number* g) {
...
bool IpoptSolver_NLP::eval_jac_g(
    ...
    if (values == nullptr) {
        ...
        int grad_idx = 0;        // Offset into iRow, jCol output variables.
                                 ...
                                 // GetGradientMatrix.
        for (const auto& c : problem_->generic_constraints()) {
            grad_idx += GetGradientMatrix(*problem_, *(c.evaluator()), c.variables(), constraint_idx, iRow + grad_idx,
                                          jCol + grad_idx);
    ...
    if (new_x || !constraint_cache_->grad_valid || !constraint_cache_->is_x_equal(n, x)) {
...
void IpoptSolver_NLP::finalize_solution(SolverReturn status,
                                        Index n,
                                        const Number* x,
                                        const Number* z_L,
                                        const Number* z_U,
    ...
    SetBoundingBoxConstraintDualSolution(*problem_, z_L, z_U, bb_con_dual_variable_indices_, result_);
    SetAllConstraintDualSolution(*problem_, lambda_, constraint_dual_start_index_, result_);
...
void IpoptSolver_NLP::EvaluateCosts(Index n, const Number* x) {
    ...
    for (auto const& binding : problem_->GetAllCosts()) {
        ...
        for (int i = 0; i < num_v_variables; ++i) {
            this_x(i) = xvec(problem_->FindDecisionVariableIndex(binding.variables()(i)));
        }
...
void IpoptSolver_NLP::EvaluateConstraints(Index n, const Number* x, bool eval_gradient) {
    const Eigen::VectorXd xvec = MakeEigenVector(n, x);
