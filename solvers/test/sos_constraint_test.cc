...
public:
    void CheckNewFreePolynomial(const symbolic::Polynomial& p,
        ...
        EXPECT_EQ(p.monomial_to_coefficient_map().size(), p.decision_variables().size());
        ...
        for (const auto& pair : p.monomial_to_coefficient_map()) {
            ...
            DRAKE_EXPECT_NO_THROW(unused(prog_.FindDecisionVariableIndex(decision_variable_i)));
            switch (degree_type) {
                case DegreeType::kAny: {
                    break;
                }
    ...
    void TestNewFreePolynomial(const Variables& indeterminates, int degree) {
        const symbolic::Polynomial poly1{prog_.NewFreePolynomial(indeterminates, degree)};
        CheckNewFreePolynomial(poly1, indeterminates, degree, DegreeType::kAny);
        const symbolic::Polynomial poly2{prog_.NewEvenDegreeFreePolynomial(indeterminates, degree)};
        CheckNewFreePolynomial(poly2, indeterminates, degree, DegreeType::kEven);
        const symbolic::Polynomial poly3{prog_.NewOddDegreeFreePolynomial(indeterminates, degree)};
    ...
    void CheckNewSosPolynomial(const symbolic::Polynomial& p,
        ...
        //       Q is p.s.d.
        const drake::VectorX<symbolic::Monomial> monomial_basis{symbolic::MonomialBasis(indeterminates, degree / 2)};
        EXPECT_EQ(p.TotalDegree(), degree);
        // Number of coefficients == number of distinct entries in a symmetric
        // matrix of size n x n where n = monomial_basis.size().
        EXPECT_EQ(p.decision_variables().size(), monomial_basis.size() * (monomial_basis.size() + 1) / 2);
    ...
    void CheckPositiveDefiniteMatrix(const MatrixXDecisionVariable& Q,
                                     ...
                                     const symbolic::Expression& sos_poly_expected,
        ...
        for (int i = 0; i < Q_val.rows(); ++i) {
            ...
            for (int j = i + 1; j < Q_val.cols(); ++j) {
                sos_poly.AddProduct(Q_val(i, j) + Q_val(j, i), monomial_basis(i) * monomial_basis(j));
            }
        }
        symbolic::Polynomial diff_poly = sos_poly - symbolic::Polynomial(result_.GetSolution(sos_poly_expected));
...
TEST_F(SosConstraintTest, NewSosPolynomialViaMonomialBasis) {
    ...
    const symbolic::Polynomial& poly{p.first};
    const MatrixXDecisionVariable& Q = p.second;
    const symbolic::Polynomial expected_poly{Q(0, 0) * x0 * x0 + 2 * Q(0, 1) * x0 * x1 + Q(1, 1) * x1 * x1,
...
TEST_F(SosConstraintTest, AddSosConstraintUnivariate2) {
    ...
    const symbolic::Expression e =
            pow(x, 6) - 10 * pow(x, 5) + 51 * pow(x, 4) - 166 * pow(x, 3) + 342 * pow(x, 2) - 400 * x + 200 - c;
    MatrixXDecisionVariable Q;
    VectorX<symbolic::Monomial> m;
    std::tie(Q, m) = prog_.AddSosConstraint(e);
    result_ = Solve(prog_);
...
TEST_F(SosConstraintTest, AddSosConstraintMultivariate1) {
    const auto& x0 = x_(0);
    const auto& x1 = x_(1);
    const symbolic::Expression e = 2 * pow(x0, 4) + 2 * pow(x0, 3) * x1 - pow(x0, 2) * pow(x1, 2) + 5 * pow(x1, 4);
...
TEST_F(SosConstraintTest, AddSosConstraintMultivariate2) {
    ...
    prog_.AddCost(-c);
    const symbolic::Expression e =
            4 * pow(x0, 2) - 2.1 * pow(x0, 4) + 1.0 / 3.0 * pow(x0, 6) + x0 * x1 - 4 * x1 * x1 + 4 * pow(x1, 4) - c;
    MatrixXDecisionVariable Q;
    VectorX<symbolic::Monomial> m;
    std::tie(Q, m) = prog_.AddSosConstraint(e);
