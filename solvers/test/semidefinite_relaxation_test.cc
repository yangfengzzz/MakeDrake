#include "solvers/semidefinite_relaxation.h"
...
namespace {
void SetRelaxationInitialGuess(const Eigen::Ref<const VectorXd>& y_expected, MathematicalProgram* relaxation) {
...
void SetRelaxationInitialGuess(const std::map<Variable, double>& expected_values, MathematicalProgram* relaxation) {
    ...
    for (const auto& constraint : relaxation->positive_semidefinite_constraints()) {
        ...
        VectorXd x_expected(n);
        const MatrixX<Variable> X_var = Eigen::Map<const MatrixX<Variable>>(constraint.variables().data(), n, n);
        for (int i = 0; i < n - 1; ++i) {
...
protected:
    ...
    A_eq <<  0.5,  0.7, -0.2,
        ...
        prog_.AddQuadraticConstraint(Q_non_convex, b_non_convex, lb_non_convex, ub_non_convex, y_.segment(2, 2));
    ...
    Q_convex << 1,   0.5,
        ...
        prog_.AddQuadraticConstraint(Q_convex, b_convex, -kInf, ub_convex, y_.segment(1, 2));
...
TEST_F(MakeSemidefiniteRelaxationTest, EnsureProgramsAreValidated) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeSemidefiniteRelaxation(prog_, options),
...
TEST_F(MakeSemidefiniteRelaxationTest, VerifyLinearCostsAndConstraintsCloned) {
    ...
    const std::vector<Binding<QuadraticCost>> quadratic_costs = prog_.quadratic_costs();
    for (const auto& cost : quadratic_costs) {
        prog_.RemoveCost(cost);
    }
    const std::vector<Binding<QuadraticConstraint>> quadratic_constraints = prog_.quadratic_constraints();
    ...
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_costs()[0])[0],
                a_cost.transpose() * y_test.head<2>() + b_cost, 1e-12);

    // The linear constraint is correct.
    const MatrixXd A_constraint = prog_.linear_constraints()[0].evaluator()->GetDenseA();
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[0]),
                                A_constraint * y_test.segment(2, 2), 1e-12));
    EXPECT_TRUE(CompareMatrices(relaxation->linear_constraints()[0].evaluator()->upper_bound(),
                                prog_.linear_constraints()[0].evaluator()->upper_bound()));
    EXPECT_TRUE(CompareMatrices(relaxation->linear_constraints()[0].evaluator()->lower_bound(),
                                prog_.linear_constraints()[0].evaluator()->lower_bound()));

    // The bounding box constraint is correct.
    const MatrixXd A_bb = prog_.bounding_box_constraints()[0].evaluator()->GetDenseA();
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(relaxation->bounding_box_constraints()[0]),
                                A_bb.transpose() * y_test.head<3>(), 1e-12));
    EXPECT_TRUE(CompareMatrices(relaxation->bounding_box_constraints()[0].evaluator()->upper_bound(),
                                prog_.bounding_box_constraints()[0].evaluator()->upper_bound()));
    EXPECT_TRUE(CompareMatrices(relaxation->bounding_box_constraints()[0].evaluator()->lower_bound(),
                                prog_.bounding_box_constraints()[0].evaluator()->lower_bound()));
    // The linear equality constraint is correct.
    const MatrixXd A_eq = prog_.linear_equality_constraints()[0].evaluator()->GetDenseA();
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[0]),
                                A_eq * y_test.tail<3>(), 1e-12));
    EXPECT_TRUE(CompareMatrices(relaxation->linear_equality_constraints()[0].evaluator()->upper_bound(),
                                prog_.linear_equality_constraints()[0].evaluator()->upper_bound()));
    EXPECT_TRUE(CompareMatrices(relaxation->linear_equality_constraints()[0].evaluator()->lower_bound(),
...
TEST_F(MakeSemidefiniteRelaxationTest, LinearizeQuadraticCostsAndConstraintsPreserveFalse) {
    SemidefiniteRelaxationOptions options;
    ...
    EXPECT_EQ(relaxation->linear_costs().size(), prog_.linear_costs().size() + prog_.quadratic_costs().size());
    // We should have the same total number of costs.
    EXPECT_EQ(relaxation->GetAllCosts().size(), prog_.linear_costs().size() + prog_.quadratic_costs().size());
    ...
    EXPECT_EQ(relaxation->GetAllConstraints().size(), prog_.GetAllConstraints().size() + 1 + 1);
}

TEST_F(MakeSemidefiniteRelaxationTest, LinearizeQuadraticCostsAndConstraintsPreserveTrue) {
    SemidefiniteRelaxationOptions options;
    options.set_to_weakest();
    ...
    EXPECT_EQ(relaxation->GetAllConstraints().size(), prog_.GetAllConstraints().size() + 1 + 1 + 1);
}
...
TEST_F(MakeSemidefiniteRelaxationTest, AddImpliedLinearConstraint) {
    ...
    // linear constraints.
    EXPECT_EQ(relaxation->linear_constraints().size(),
              prog_.linear_constraints().size() + prog_.quadratic_constraints().size() + 1);

    const auto implied_linear_constraint = relaxation->linear_constraints().back();
    ...
    EXPECT_EQ(implied_linear_constraint.evaluator()->GetDenseA().rows(), NChoose2(aggregated_A_row_size + 1));
}
...
TEST_F(MakeSemidefiniteRelaxationTest, AddImpliedLinearEqualityConstraint) {
    ...
    EXPECT_EQ(relaxation->linear_equality_constraints().size(),
              prog_.linear_equality_constraints().size() + 1 + prog_.linear_equality_constraints().size() * y_.size());
}
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, EmptyVariableGroup) {
    ...
    prog_.AddQuadraticCost(y_[0] * y_[0] + y_[1] * y_[1], true /* a convex cost*/);

    prog_.AddLinearConstraint(x_[0] >= x_[1]);
    prog_.AddLinearEqualityConstraint(x_[1] == x_[2]);
    prog_.AddQuadraticConstraint(x_[0] * x_[0] + x_[1] * x_[1], -kInf, 1,
    ...
    const auto relaxation_empty = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>());
    ...
    EXPECT_EQ(relaxation_empty->linear_costs().size(), prog_.linear_costs().size());
    EXPECT_EQ(relaxation_empty->quadratic_costs().size(), prog_.quadratic_costs().size());
    ...
    // constraint.
    EXPECT_EQ(relaxation_empty->linear_constraints().size(), prog_.linear_constraints().size());
    EXPECT_EQ(relaxation_empty->linear_equality_constraints().size(), prog_.linear_equality_constraints().size() + 1);
    EXPECT_EQ(relaxation_empty->quadratic_constraints().size(), prog_.quadratic_constraints().size());
    // This confirms that we don't have any PSD constraints.
    EXPECT_EQ(relaxation_empty->GetAllConstraints().size(), prog_.GetAllConstraints().size() + 1);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeSemidefiniteRelaxation(prog_, std::vector<Variables>()), ".*non-convex.*");
    ...
    auto non_convex_quadratic_constraint =
            prog_.AddQuadraticConstraint(x_[0] * y_[1], 0, 1, QuadraticConstraint::HessianType::kIndefinite);
    DRAKE_EXPECT_THROWS_MESSAGE(MakeSemidefiniteRelaxation(prog_, std::vector<Variables>()), ".*non-convex.*");
}
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, EmptyProgramWithVariableGroups) {
    ...
    EXPECT_EQ(relaxation->positive_semidefinite_constraints().size(), 1);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[0].evaluator()->matrix_rows(), 4);
    ...
    const Eigen::MatrixX<Variable> X = Eigen::Map<const MatrixX<Variable>>(
            relaxation->positive_semidefinite_constraints()[0].variables().data(), x_.size() + 1, x_.size() + 1);
    for (int i = 0; i < x_.size(); ++i) {
        EXPECT_TRUE(X(i, X.cols() - 1).equal_to(x_[i]));
    }

    relaxation = MakeSemidefiniteRelaxation(prog_, std::vector{x0_y0_y1_vars_, y_vars_});
    ...
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[0].evaluator()->matrix_rows(), 4);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[1].evaluator()->matrix_rows(), 3);
    // No costs are added.
    ...
    // equals 1.
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[0])[0], 1, 1e-12);
    ...
    const Eigen::MatrixX<Variable> X0 =
            Eigen::Map<const MatrixX<Variable>>(relaxation->positive_semidefinite_constraints()[0].variables().data(),
                                                x0_y0_y1_vars_.size() + 1, x0_y0_y1_vars_.size() + 1);
    const Eigen::MatrixX<Variable> X1 =
            Eigen::Map<const MatrixX<Variable>>(relaxation->positive_semidefinite_constraints()[1].variables().data(),
  ...
  X1_y0_y1 << relaxation->GetInitialGuess(X1(0, 0)),
    ...
    const Variables overlap_variables{X0(1, 1), X0(1, 2), X0(2, 2), X1(0, 0), X1(0, 1), X1(1, 1)};
    EXPECT_EQ(Variables{constraint.variables()}, overlap_variables);
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(constraint), Eigen::Vector3d::Zero()));
}
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, LinearCost) {
    ...
    // Only relaxes the x_ variables.
    auto relaxation = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>{x0_x2_y1_vars_});
    EXPECT_EQ(relaxation->linear_costs().size(), 1);
    EXPECT_EQ(relaxation->GetAllCosts().size(), 1);
    // The variables which get relaxed are [x(0), x(1), x(2), 1]. The remaining
    // variables that do not get relaxed are [y(0), y(1)]. So there are (5
    ...
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[0].evaluator()->matrix_rows(), 5);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[1].evaluator()->matrix_rows(), 4);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints().size(), 2);
    EXPECT_EQ(relaxation->linear_equality_constraints().size(), 2);
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, QuadraticCost) {
    ...
    // Only relaxes the x_ variables.
    auto relaxation = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>{x_vars_});
    EXPECT_EQ(relaxation->linear_costs().size(), 1);
    EXPECT_EQ(relaxation->GetAllCosts().size(), 1);
    // The variables which get relaxed are [x(0), x(1), x(2), 1]. The remaining
    // variables that do not get relaxed are [y(0), y(1)]. So there are (5
    ...
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[0].evaluator()->matrix_rows(), 4);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[1].evaluator()->matrix_rows(), 5);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints().size(), 2);
    EXPECT_EQ(relaxation->linear_equality_constraints().size(), 2);
    EXPECT_EQ(relaxation->GetAllConstraints().size(), 4);
    EXPECT_EQ(relaxation->linear_costs().size(), 1);
    ...
    relaxation = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>{x_vars_});
    ...
    EXPECT_EQ(relaxation->GetAllCosts().size(), 1);
    SetRelaxationInitialGuess(test_point, relaxation.get());
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->quadratic_costs()[0])[0],
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, QuadraticConstraint) {
  ...
  const Matrix2d Qy{{1.7, -0.8},
    ...
    prog_.AddQuadraticConstraint(Qy, by, lby, uby, y_, QuadraticConstraint::HessianType::kPositiveSemidefinite);

    // Relaxes the x_ and y_ variables separately.
    auto relaxation = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>{x_vars_, y_vars_});
    SetRelaxationInitialGuess(test_point, relaxation.get());
    ...
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[0])[0],
                (0.5 * x_test.transpose() * Qx * x_test + bx.transpose() * x_test)[0], 1e-12);
    EXPECT_EQ(relaxation->linear_constraints()[0].evaluator()->lower_bound()[0], lbx);
    EXPECT_EQ(relaxation->linear_constraints()[0].evaluator()->upper_bound()[0], ubx);

    const Vector2d y_test(test_point.at(y_(0)), test_point.at(y_(1)));
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[1])[0],
                (0.5 * y_test.transpose() * Qy * y_test + by.transpose() * y_test)[0], 1e-12);
    EXPECT_EQ(relaxation->linear_constraints()[1].evaluator()->lower_bound()[0], lby);
    EXPECT_EQ(relaxation->linear_constraints()[1].evaluator()->upper_bound()[0], uby);
    ...
    // Two psd constraints.
    EXPECT_EQ(relaxation->positive_semidefinite_constraints().size(), 2);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[0].evaluator()->matrix_rows(), 6);
    EXPECT_EQ(relaxation->positive_semidefinite_constraints()[1].evaluator()->matrix_rows(), 5);
    ...
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[2])[0],
                (0.5 * x_test.transpose() * Qx * x_test + bx.transpose() * x_test)[0], 1e-12);
    EXPECT_EQ(relaxation->linear_constraints()[2].evaluator()->lower_bound()[0], lbx);
    EXPECT_EQ(relaxation->linear_constraints()[2].evaluator()->upper_bound()[0], ubx);
    EXPECT_NEAR(relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[3])[0],
                (0.5 * y_test.transpose() * Qy * y_test + by.transpose() * y_test)[0], 1e-12);
    EXPECT_EQ(relaxation->linear_constraints()[3].evaluator()->lower_bound()[0], lby);
    EXPECT_EQ(relaxation->linear_constraints()[3].evaluator()->upper_bound()[0], uby);
    ...
    relaxation = MakeSemidefiniteRelaxation(prog_, std::vector<Variables>{x_vars_, x0_x2_y1_vars_});
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, LinearConstraint) {
  ...
  Ay << 1.1, 2.7,
    ...
    EXPECT_TRUE(CompareMatrices(Ax, relaxation->linear_constraints()[0].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lbx, relaxation->linear_constraints()[0].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(ubx, relaxation->linear_constraints()[0].evaluator()->upper_bound()));
    EXPECT_TRUE(CompareMatrices(Ay, relaxation->linear_constraints()[1].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lby, relaxation->linear_constraints()[1].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(uby, relaxation->linear_constraints()[1].evaluator()->upper_bound()));
    ...
    EXPECT_EQ(relaxation->linear_constraints()[2].evaluator()->num_constraints(), expected_size);
    const Vector3d x_test(test_point.at(x_(1)), test_point.at(x_(0)), test_point.at(x_(2)));
    VectorXd value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[2]);
    MatrixXd expected_mat = (A * x_test - b) * (A * x_test - b).transpose() - b * b.transpose();
    ...
    EXPECT_EQ(relaxation->linear_constraints()[3].evaluator()->num_constraints(), expected_size);
    const Vector2d y_test(test_point.at(y_(1)), test_point.at(y_(0)));
    value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[3]);
    expected_mat = (A * y_test - b) * (A * y_test - b).transpose() - b * b.transpose();
  ...
  A_overlap << -1.9, 2.7;
    ...
    std::vector<Variables> groups{Variables{x_(0), x_(1), x_(2), y_(1)}, Variables{x_(1), y_(0), y_(1)}};
    relaxation = MakeSemidefiniteRelaxation(prog_, groups);
    SetRelaxationInitialGuess(test_point, relaxation.get());
    ...
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(psd_agree_constraint),
                                Eigen::VectorXd::Zero(psd_agree_constraint.evaluator()->num_constraints())));
    ...
    EXPECT_TRUE(CompareMatrices(Ax, relaxation->linear_constraints()[constraint_ind].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lbx, relaxation->linear_constraints()[constraint_ind].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(ubx, relaxation->linear_constraints()[constraint_ind].evaluator()->upper_bound()));
    ++constraint_ind;
    EXPECT_TRUE(CompareMatrices(Ay, relaxation->linear_constraints()[constraint_ind].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lby, relaxation->linear_constraints()[constraint_ind].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(uby, relaxation->linear_constraints()[constraint_ind].evaluator()->upper_bound()));
    ++constraint_ind;
    EXPECT_TRUE(CompareMatrices(A_overlap, relaxation->linear_constraints()[constraint_ind].evaluator()->GetDenseA()));
    EXPECT_TRUE(
            CompareMatrices(lb_overlap, relaxation->linear_constraints()[constraint_ind].evaluator()->lower_bound()));
    EXPECT_TRUE(
            CompareMatrices(ub_overlap, relaxation->linear_constraints()[constraint_ind].evaluator()->upper_bound()));
    ++constraint_ind;
  ...
  A << -Ax(0,1), -Ax(0,0),         -Ax(0,2), 0, // NOLINT
    ...
    // clang-format on
    expected_size = (b.size() * (b.size() + 1)) / 2;
    EXPECT_EQ(relaxation->linear_constraints()[constraint_ind].evaluator()->num_constraints(), expected_size);
    VectorXd test_vec(4);
    test_vec << test_point.at(x_(0)), test_point.at(x_(1)), test_point.at(x_(2)), test_point.at(y_(1));
    value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_constraints()[constraint_ind]);
    expected_mat = (A * test_vec - b) * (A * test_vec - b).transpose() - b * b.transpose();
    expected = math::ToLowerTriangularColumnsFromMatrix(expected_mat);
    EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    value = relaxation->linear_constraints()[constraint_ind].evaluator()->lower_bound();
    expected = math::ToLowerTriangularColumnsFromMatrix(-b * b.transpose());
    EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    ++constraint_ind;
...
TEST_F(MakeSemidefiniteRelaxationVariableGroupTest, LinearEqualityConstraint) {
    const std::map<Variable, double> test_point{{x_(0), -0.6}, {x_(1), 3.8}, {x_(2), 4.7}, {y_(0), 9.9}, {y_(1), 3.4}};
  ...
  Ay << -0.08, 1.62,
    ...
    auto CompareLinearEqualityConstraintsAtIdx = [&](const int idx) {
        EXPECT_TRUE(CompareMatrices(relaxation->linear_equality_constraints()[idx].evaluator()->GetDenseA(),
                                    prog_.linear_equality_constraints()[idx].evaluator()->GetDenseA()));
        EXPECT_TRUE(CompareMatrices(relaxation->linear_equality_constraints()[idx].evaluator()->lower_bound(),
    ...
    EXPECT_TRUE(CompareMatrices(
            relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[cur_constraint++]),
            Eigen::VectorXd::Ones(1)));

    // This next set of equality constraints are due to the relaxation of the x_
    // variables.
    const Vector3d x_test(test_point.at(x_(0)), test_point.at(x_(1)), test_point.at(x_(2)));
    VectorXd expected;
    VectorXd value;
    ...
    for (; cur_constraint < start + 3; ++cur_constraint) {
        ...
        value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[cur_constraint]);
        EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    }
  ...
  A_overlap << -7.9, 2.4;
    ...
    const Vector2d overlap_test_vec_sorted(test_point.at(x_(2)), test_point.at(y_(0)));
    // Make sure the sorted and non-sorted version of this constraint are the
    // same.
    EXPECT_TRUE(CompareMatrices(A_overlap * overlap_test_vec - b_overlap,
                                A_overlap_sorted * overlap_test_vec_sorted - b_overlap));

    std::vector<Variables> groups{Variables{x_(0), x_(1), x_(2), y_(0)}, Variables{x_(2), y_(0), y_(1)}};
    VectorXd group_1_test_vec(4);
    group_1_test_vec << test_point.at(x_(0)), test_point.at(x_(1)), test_point.at(x_(2)), test_point.at(y_(0));
    VectorXd group_2_test_vec(3);
    group_2_test_vec << test_point.at(x_(2)), test_point.at(y_(0)), test_point.at(y_(1));
    ...
    EXPECT_EQ(relaxation->linear_equality_constraints().size(), 3 + 1 + 2 * 4 + 2 * 3 + 1);
    EXPECT_EQ(relaxation->GetAllConstraints().size(), relaxation->positive_semidefinite_constraints().size() +
    ...
    for (; cur_constraint < start + 4; ++cur_constraint) {
        ...
        // x(1), x(2), y(0)] = 0.
        expected = (A_overlap_sorted * overlap_test_vec_sorted - b_overlap) * group_1_test_vec[cur_constraint - start];
        value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[cur_constraint]);
        EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    }
    ...
    for (; cur_constraint < start + 3; ++cur_constraint) {
        ...
        expected = (A_overlap_sorted * overlap_test_vec_sorted - b_overlap) * group_2_test_vec[cur_constraint - start];
        value = relaxation->EvalBindingAtInitialGuess(relaxation->linear_equality_constraints()[cur_constraint]);
        EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    }
    ASSERT_EQ(cur_constraint, 18);

    auto psd_agree_constraint = relaxation->linear_equality_constraints()[cur_constraint++];
    EXPECT_TRUE(CompareMatrices(relaxation->EvalBindingAtInitialGuess(psd_agree_constraint),
                                Eigen::VectorXd::Zero(psd_agree_constraint.evaluator()->num_constraints())));
