...
void RunNonlinearProgram(const MathematicalProgram& prog,
    ...
    std::pair<const char*, SolverInterface*> solvers[] = {std::make_pair("SNOPT", &snopt_solver),
...
GTEST_TEST(testNonlinearProgram, BoundingBoxTest) {
    ...
    prog.AddBoundingBoxConstraint(Vector2d(-1, -2), Vector2d(-0.2, -1), variable_vec);
    prog.AddBoundingBoxConstraint(Vector3d(-1, -0.5, -3), Vector3d(2, 1, -0.1), {x.head<1>(), x.segment<2>(1)});
...
GTEST_TEST(testNonlinearProgram, trivialLinearEquality) {
    ...
    prog.AddLinearEqualityConstraint(Eigen::RowVector2d(0, 1), Vector1d::Constant(1), vars);
    MathematicalProgramResult result;
...
GTEST_TEST(testNonlinearProgram, QuadraticCost) {
    ...
    RunNonlinearProgram(
            ...
            [&]() {
                const auto& x_value = result.GetSolution(x);
                EXPECT_TRUE(CompareMatrices(x_value, expected, 1e-6, MatrixCompareType::absolute));
                EXPECT_TRUE(CompareMatrices(
                        prog.EvalBinding(prog.quadratic_costs().front(), result.GetSolution(prog.decision_variables())),
                        0.5 * x_value.transpose() * Q_symmetric * x_value + b.transpose() * x_value, 1E-14,
...
GTEST_TEST(testNonlinearProgram, testNonConvexQPproblem1) {
    for (const auto& cost_form : NonConvexQPproblem1::cost_forms()) {
        for (const auto& constraint_form : NonConvexQPproblem1::constraint_forms()) {
...
GTEST_TEST(testNonlinearProgram, testNonConvexQPproblem2) {
    for (const auto& cost_form : NonConvexQPproblem2::cost_forms()) {
        for (const auto& constraint_form : NonConvexQPproblem2::constraint_forms()) {
...
GTEST_TEST(testNonlinearProgram, testGloptiPolyConstrainedMinimization) {
    for (const auto& cost_form : GloptiPolyConstrainedMinimizationProblem::cost_forms()) {
        for (const auto& constraint_form : GloptiPolyConstrainedMinimizationProblem::constraint_forms()) {
...
GTEST_TEST(testNonlinearProgram, linearPolynomialConstraint) {
    ...
    std::shared_ptr<Constraint> resulting_constraint =
            problem.AddPolynomialConstraint(VectorXPoly::Constant(1, x), var_mapping, Vector1d::Constant(2),
...
GTEST_TEST(testNonlinearProgram, polynomialConstraint) {
    ...
    // Given a small univariate polynomial, find a low point.
    {
        ...
        const auto x_var = problem.NewContinuousVariables(1);
        const std::vector<Polynomiald::VarType> var_mapping = {x.GetSimpleVariable()};
        problem.AddPolynomialConstraint(VectorXPoly::Constant(1, poly), var_mapping, Eigen::VectorXd::Zero(1),
        ...
        RunNonlinearProgram(
                ...
                [&]() {
                    ...
                    EXPECT_LE(poly.EvaluateUnivariate(result.GetSolution(x_var(0))), kEpsilon);
                },
                &result);
    }
    ...
    // Given two polynomial constraints, satisfy both.
    {
        ...
        problem.AddPolynomialConstraint(polynomials_vec, var_mapping, Eigen::VectorXd::Constant(2, -kInf),
...
GTEST_TEST(testNonlinearProgram, MinDistanceFromPlaneToOrigin) {
    ...
    for (const auto& cost_form : MinDistanceFromPlaneToOrigin::cost_forms()) {
        for (const auto& constraint_form : MinDistanceFromPlaneToOrigin::constraint_forms()) {
            for (int k = 0; k < 2; ++k) {
                MinDistanceFromPlaneToOrigin prob(A[k], b[k], cost_form, constraint_form);
                MathematicalProgramResult result;
                RunNonlinearProgram(
