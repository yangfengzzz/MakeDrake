...
void ShortestDistanceToThreePoints::CheckSolution(const SolverInterface& solver,
    ...
    if (solver.available() && solver.enabled()) {
        ...
        if ((pts_[1] - pts_[0]).dot(pts_[2] - pts_[0]) / ((pts_[1] - pts_[0]).norm() * (pts_[2] - pts_[0]).norm()) >
            cos(M_PI / 3 * 2)) {
        ...
        EXPECT_NEAR(result.get_optimal_cost(),
                    (pts_[0] - x_sol_expected).norm() + (pts_[1] - x_sol_expected).norm() +
                            (pts_[2] - x_sol_expected).norm(),
                    tol);
    }
}

ShortestDistanceFromCylinderToPoint::ShortestDistanceFromCylinderToPoint() : prog_{} {
    x_ = prog_.NewContinuousVariables<3>();
    prog_.AddBoundingBoxConstraint(-1, 1, x_[2]);
    prog_.AddLorentzConeConstraint(Vector3<symbolic::Expression>(2, x_[0] * 1, x_[1] * 1));
    pt_ = Eigen::Vector3d(1, 2, 5);
    prog_.AddL2NormCost(Eigen::Matrix3d::Identity(), -pt_, x_);
}

void ShortestDistanceFromCylinderToPoint::CheckSolution(const SolverInterface& solver,
                                                        const std::optional<SolverOptions>& solver_options,
                                                        double tol) const {
    MathematicalProgramResult result;
    if (solver.available() && solver.enabled()) {
...
ShortestDistanceFromPlaneToTwoPoints::ShortestDistanceFromPlaneToTwoPoints() : prog_{} {
    x_ = prog_.NewContinuousVariables<3>();
  ...
  A_ << 1, 3, -2,
    ...
    prog_.AddLinearEqualityConstraint(plane_normal_.transpose() * A_, plane_normal_.dot(plane_pt_), x_);
}

void ShortestDistanceFromPlaneToTwoPoints::CheckSolution(const SolverInterface& solver,
                                                         const std::optional<SolverOptions>& solver_options,
                                                         double tol) const {
    MathematicalProgramResult result;
    if (solver.available() && solver.enabled()) {
        ...
        const Eigen::Vector3d pt1_mirror = pts_[1] - (pts_[1] - plane_pt_).dot(plane_normal_) * plane_normal_ * 2;
        const Eigen::Vector3d x_sol = result.GetSolution(x_);
        const Eigen::Vector3d y = A_ * x_sol;
        EXPECT_NEAR(plane_normal_.dot(y), plane_normal_.dot(plane_pt_), tol);
        EXPECT_TRUE(CompareMatrices((pt1_mirror - y).cross(y - pts_[0]), Eigen::Vector3d::Zero(), tol));
        EXPECT_NEAR(result.get_optimal_cost(), (y - pts_[0]).norm() + (y - pts_[1]).norm(), tol);
    }
}
}  // namespace test
}  // namespace solvers
