...
public:
    ChooseBestSolverTest()
        : prog_{},
          ...
          equality_constrained_qp_solver_{std::make_unique<EqualityConstrainedQPSolver>()},
...
void CheckGetAvailableSolvers(const MathematicalProgram& prog) {
    ...
    const std::unordered_set<SolverId> available_id_set(available_ids.begin(), available_ids.end());
    for (const auto& solver_id : known_solvers) {
        const auto solver = MakeSolver(solver_id);
        if (solver->available() && solver->enabled() && solver->AreProgramAttributesSatisfied(prog)) {
            ...
            if (solver_id == ClpSolver::id() && prog_type == ProgramType::kQP) {
                continue;
            } else if ((solver_id == SnoptSolver::id() || solver_id == IpoptSolver::id() ||
...
TEST_F(ChooseBestSolverTest, LPsolver) {
    ...
    CheckBestSolver({gurobi_solver_.get(), mosek_solver_.get(), clp_solver_.get(), clarabel_solver_.get(),
                     snopt_solver_.get(), ipopt_solver_.get(), nlopt_solver_.get(), csdp_solver_.get(),
                     scs_solver_.get()});
...
TEST_F(ChooseBestSolverTest, QPsolver) {
    ...
    prog_.AddQuadraticCost(x_(0) * x_(0));
    CheckBestSolver({mosek_solver_.get(), gurobi_solver_.get(), clarabel_solver_.get(), osqp_solver_.get(),
                     snopt_solver_.get(), ipopt_solver_.get(), nlopt_solver_.get(), scs_solver_.get()});
    CheckGetAvailableSolvers(prog_);
}

TEST_F(ChooseBestSolverTest, LorentzCone) {
    prog_.AddLorentzConeConstraint(x_.cast<symbolic::Expression>());
    CheckBestSolver({mosek_solver_.get(), gurobi_solver_.get(), clarabel_solver_.get(), csdp_solver_.get(),
                     scs_solver_.get(), snopt_solver_.get(), ipopt_solver_.get(), nlopt_solver_.get()});
    prog_.AddRotatedLorentzConeConstraint(x_.cast<symbolic::Expression>());
    CheckBestSolver({mosek_solver_.get(), gurobi_solver_.get(), clarabel_solver_.get(), csdp_solver_.get(),
                     scs_solver_.get(), snopt_solver_.get(), ipopt_solver_.get(), nlopt_solver_.get()});

    prog_.AddPolynomialCost(pow(x_(0), 3));
    CheckBestSolver({snopt_solver_.get(), ipopt_solver_.get(), nlopt_solver_.get()});
    CheckGetAvailableSolvers(prog_);
}

TEST_F(ChooseBestSolverTest, LinearComplementarityConstraint) {
    prog_.AddLinearComplementarityConstraint(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Ones(), x_);
...
TEST_F(ChooseBestSolverTest, PositiveSemidefiniteConstraint) {
    prog_.AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x_(0), x_(1), x_(1), x_(2)).finished());
    CheckBestSolver({mosek_solver_.get(), clarabel_solver_.get(), csdp_solver_.get(), scs_solver_.get()});
    CheckGetAvailableSolvers(prog_);
}
...
TEST_F(ChooseBestSolverTest, Nlp) {
    prog_.AddConstraint(std::make_shared<ExpressionConstraint>(
                                Vector1<symbolic::Expression>(x_(0) + symbolic::sin(x_(1))), Vector1d(0), Vector1d(2)),
                        x_.head<2>());
    CheckGetAvailableSolvers(prog_);
}
...
TEST_F(ChooseBestSolverTest, NoAvailableSolver) {
    ...
    prog_.AddLinearComplementarityConstraint(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Ones(), x_.tail<2>());
    prog_.NewBinaryVariables<2>();
    DRAKE_EXPECT_THROWS_MESSAGE(ChooseBestSolver(prog_), "There is no available solver for the optimization program");
}
...
TEST_F(ChooseBestSolverTest, MakeSolver) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeSolver(SolverId("foo")), "MakeSolver: no matching solver foo");
}
...
GTEST_TEST(MakeFirstAvailableSolver, Test) {
    const bool has_gurobi = GurobiSolver::is_available() && GurobiSolver::is_enabled();
    const bool has_scs = ScsSolver::is_available() && ScsSolver::is_enabled();

    if (has_gurobi || has_scs) {
        auto solver = MakeFirstAvailableSolver({GurobiSolver::id(), ScsSolver::id()});
        if (has_gurobi) {
    ...
    if (!has_gurobi) {
        DRAKE_EXPECT_THROWS_MESSAGE(MakeFirstAvailableSolver({GurobiSolver::id()}), ".* is available and enabled.");
    }
}
