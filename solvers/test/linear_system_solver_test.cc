...
GTEST_TEST(testLinearSystemSolver, EmptyProblem) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), ".*LinearEqualityConstraint is required.*");
}
...
GTEST_TEST(testLinearSystemSolver, QuadraticProblem) {
    ...
    EXPECT_FALSE(LinearSystemSolver::ProgramAttributesSatisfied(prog));
    EXPECT_THAT(LinearSystemSolver::UnsatisfiedProgramAttributes(prog), HasSubstr("QuadraticCost was declared"));

    LinearSystemSolver solver;
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), ".*QuadraticCost was declared.*");
}
...
GTEST_TEST(testLinearSystemSolver, InfeasibleProblem) {
    ...
    prog.AddLinearConstraint(3 * x(0) == 1 && 2 * x(0) + x(1) == 2 && x(0) - x(1) == 0);
    ...
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), x_expected, 1E-12, MatrixCompareType::absolute));
