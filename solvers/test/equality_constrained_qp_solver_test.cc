...
GTEST_TEST(testEqualityConstrainedQPSolver, testUnconstrainedQPDispatch) {
    ...
    EXPECT_TRUE(CompareMatrices(expected_answer, x_value, 1e-10, MatrixCompareType::absolute));
    ...
    EXPECT_TRUE(CompareMatrices(expected_answer, actual_answer, 1e-10, MatrixCompareType::absolute));
...
GTEST_TEST(testEqualityConstrainedQPSolver, testLinearlyConstrainedQPDispatch) {
    ...
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(-1), 1e-14));
    ...
    EXPECT_NEAR(0.5, result.get_optimal_cost(), 1e-10);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(-1), 1e-14));
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint2), Vector1d(0.), 1e-14));
}
...
GTEST_TEST(testEqualityConstrainedQPSolver, testPositiveHessianInfeasibleConstraint) {
    ...
    prog.AddLinearConstraint(x(0) + 2 * x(1) == 1 && x(0) - x(1) == 3 && 2 * x(0) + x(1) == 2);
    EqualityConstrainedQPSolver solver;
    auto result = solver.Solve(prog, {}, {});
...
GTEST_TEST(testEqualityConstrainedQPSolver, testIndefiniteHessian) {
    ...
    auto result = solver.Solve(prog, {}, {});
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(0, 1), 1E-12, MatrixCompareType::absolute));
    EXPECT_NEAR(result.get_optimal_cost(), -1, 1E-12);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Vector1d(-2), 1E-12));
}
...
GTEST_TEST(testEqualityConstrainedQPSolver, testNegativeHessianUniqueOptimum) {
    ...
    Eigen::Matrix2d Aeq;
    Aeq << 1, 1, 1, -1;
    auto constraint = prog.AddLinearEqualityConstraint(Aeq, Eigen::Vector2d(2, 3), x);
    EqualityConstrainedQPSolver solver;
    auto result = solver.Solve(prog, {}, {});
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(2.5, -0.5), 1E-12, MatrixCompareType::absolute));
    EXPECT_NEAR(result.get_optimal_cost(), -6.5, 1E-12);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Eigen::Vector2d(-2, -3), 1e-12));
}
...
GTEST_TEST(testEqualityConstrainedQPSolver, testPSDHessianUniqueOptimal) {
    ...
    auto result = solver.Solve(prog, {}, {});
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(-1.0 / 4, 5.0 / 8), 1E-12,
                                MatrixCompareType::absolute));
    EXPECT_NEAR(result.get_optimal_cost(), 23.0 / 16.0, 1E-12);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Vector1d(1.5), 1e-12));
}
...
GTEST_TEST(testEqualityConstrainedQPSolver, testIndefiniteHessianInfeasible) {
    ...
    prog.AddLinearConstraint(x(0) + 2 * x(1) == 1 && -x(0) + 3 * x(1) == 2 && 2 * x(0) - 3 * x(1) == 3);
    EqualityConstrainedQPSolver solver;
    auto result = solver.Solve(prog, {}, {});
    EXPECT_EQ(result.get_solution_result(), SolutionResult::kInfeasibleConstraints);
    EXPECT_EQ(result.get_optimal_cost(), MathematicalProgram::kGlobalInfeasibleCost);
...
GTEST_TEST(testEqualityConstrainedQPSolver, testFeasibilityTolerance) {
    ...
    auto x = prog.NewContinuousVariables<2>("x");
    prog.AddCost(x(0) * x(0) + 2 * x(1) * x(1));
    prog.AddLinearConstraint(x(0) + 2 * x(1) == 1 && x(0) - x(1) == -2 && x(0) + x(1) == 1E-6);
    EqualityConstrainedQPSolver solver;
    prog.SetSolverOption(EqualityConstrainedQPSolver::id(), "FeasibilityTol", 1E-7);
    auto result = solver.Solve(prog, {}, {});
    EXPECT_EQ(result.get_solution_result(), SolutionResult::kInfeasibleConstraints);
    EXPECT_EQ(result.get_optimal_cost(), MathematicalProgram::kGlobalInfeasibleCost);
    ...
    double tol = 1E-6;
    prog.SetSolverOption(EqualityConstrainedQPSolver::id(), EqualityConstrainedQPSolver::FeasibilityTolOptionName(),
                         tol);
    result = solver.Solve(prog, {}, {});
    EXPECT_TRUE(result.is_success());
    const Eigen::Vector2d x_val = result.GetSolution(x);
    const Eigen::Vector3d cnstr_val(x_val(0) + 2 * x_val(1), x_val(0) - x_val(1), x_val(0) + x_val(1));
    EXPECT_TRUE(CompareMatrices(cnstr_val, Eigen::Vector3d(1, -2, 1E-6), tol, MatrixCompareType::absolute));
    ...
    solver_options.SetOption(EqualityConstrainedQPSolver::id(), EqualityConstrainedQPSolver::FeasibilityTolOptionName(),
                             0.1 * tol);
...
GTEST_TEST(testEqualityConstrainedQPSolver, testLinearCost) {
    ...
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(-.5, -.5), 1e-6));
...
public:
    EqualityConstrainedQPSolverTest()
        : prog_{}, x_{prog_.NewContinuousVariables<2>()}, solver_{}, result_{}, solver_options_{} {
...
TEST_F(EqualityConstrainedQPSolverTest, WrongSolverOptions1) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(solver_.Solve(prog_, {}, solver_options_, &result_),
...
TEST_F(EqualityConstrainedQPSolverTest, WrongSolverOptions2) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver_.Solve(prog_, {}, solver_options_, &result_),
