...
GTEST_TEST(SolverOptionsTest, SetGetOption) {
    ...
    const std::unordered_map<CommonSolverOption, std::variant<double, int, std::string>> common_options_expected(
            {{CommonSolverOption::kPrintToConsole, 0}, {CommonSolverOption::kPrintFileName, "foo.txt"}});
...
GTEST_TEST(SolverOptionsTest, CheckOptionKeysForSolver) {
    ...
    DRAKE_EXPECT_NO_THROW(solver_options.CheckOptionKeysForSolver(id2, {"key1"}, {"key2"}, {"key3"}));
    // Check the solver id in solver_options.
    DRAKE_EXPECT_NO_THROW(solver_options.CheckOptionKeysForSolver(id1, {"key1"}, {"key2"}, {"key3"}));

    // Check an option not set for id1.
    solver_options.SetOption(id1, "key2", 1.3);
    DRAKE_EXPECT_THROWS_MESSAGE(solver_options.CheckOptionKeysForSolver(id1, {"key1"}, {"key2"}, {"key3"}),
                                "key2 is not allowed in the SolverOptions for id1.");

    DRAKE_EXPECT_NO_THROW(solver_options.CheckOptionKeysForSolver(id1, {"key1", "key2"}, {"key2"}, {"key3"}));
}

GTEST_TEST(SolverOptionsTest, SetOptionError) {
    SolverOptions solver_options;
    DRAKE_EXPECT_THROWS_MESSAGE(solver_options.SetOption(CommonSolverOption::kPrintFileName, 1),
                                "SolverOptions::SetOption support kPrintFileName only with std::string "
                                "value.");
    DRAKE_EXPECT_THROWS_MESSAGE(solver_options.SetOption(CommonSolverOption::kPrintToConsole, 2),
