...
using BarFType = std::vector<std::unordered_map<MSKint64t, std::pair<std::vector<MSKint64t>, std::vector<MSKrealt>>>>;
...
void AppendFreeVariable(MSKtask_t task, int num_vars) {
    ...
    for (int i = 0; i < num_vars; ++i) {
        MSK_putvarbound(task, num_existing_vars + i, MSK_BK_FR, -MSK_INFINITY, MSK_INFINITY);
    }
}

void CheckParseLinearExpression(const MosekSolverProgram& dut,
                                const MathematicalProgram& prog,
                                const Eigen::SparseMatrix<double>& A,
                                const Eigen::SparseMatrix<double>& B,
                                const VectorX<symbolic::Variable>& decision_vars,
                                const std::vector<MSKint32t>& slack_vars_mosek_indices,
                                const std::vector<MSKint32t>& F_subi,
                                const std::vector<MSKint32t>& F_subj,
                                const std::vector<MSKrealt>& F_valij,
                                const BarFType& bar_F) {
    ...
    VectorX<symbolic::Expression> linear_exprs_expected = A * decision_vars + B * slack_vars;
    ...
    for (int i = 0; i < prog.num_vars(); ++i) {
        ...
        if (it1 != dut.decision_variable_to_mosek_nonmatrix_variable().end()) {
            ...
            bar_X[it2->second.bar_matrix_index()].resize(it2->second.num_matrix_rows(), it2->second.num_matrix_rows());
            bar_X[it2->second.bar_matrix_index()](it2->second.row_index(), it2->second.col_index()) =
                    prog.decision_variable(i);
            bar_X[it2->second.bar_matrix_index()](it2->second.col_index(), it2->second.row_index()) =
                    prog.decision_variable(i);
        }
    }
    ...
    if (!bar_F.empty()) {
        ...
        for (int i = 0; i < A.rows(); ++i) {
            for (const auto& [j, sub_weights] : bar_F[i]) {
                ...
                Eigen::MatrixXd F_bar_ij = Eigen::MatrixXd::Zero(bar_X[j].rows(), bar_X[j].cols());
    ...
    for (int i = 0; i < linear_exprs.rows(); ++i) {
        EXPECT_PRED2(symbolic::test::ExprEqual, linear_exprs(i).Expand(), linear_exprs_expected(i).Expand());
    }
}
...
GTEST_TEST(ParseLinearExpression, Test1) {
    ...
    AppendFreeVariable(dut.task(), dut.decision_variable_to_mosek_nonmatrix_variable().size());
    ...
    BarFType bar_F;
    const auto rescode =
            dut.ParseLinearExpression(prog, A_sparse, B, x.tail<2>(), {}, &F_subi, &F_subj, &F_valij, &bar_F);
    ASSERT_EQ(rescode, MSK_RES_OK);
    ...
    CheckParseLinearExpression(dut, prog, A_sparse, B, x.tail<2>(), {}, F_subi, F_subj, F_valij, bar_F);
    MSK_deleteenv(&env);
}
...
GTEST_TEST(ParseLinearExpression, Test2) {
    ...
    BarFType bar_F;
    const auto rescode =
            dut.ParseLinearExpression(prog, A_sparse, B, decision_vars, {}, &F_subi, &F_subj, &F_valij, &bar_F);
    ASSERT_EQ(rescode, MSK_RES_OK);
    EXPECT_TRUE(F_subi.empty());
    EXPECT_TRUE(F_subj.empty());
    EXPECT_TRUE(F_valij.empty());
    EXPECT_FALSE(bar_F.empty());
    CheckParseLinearExpression(dut, prog, A_sparse, B, decision_vars, {}, F_subi, F_subj, F_valij, bar_F);
    MSK_deleteenv(&env);
}
...
GTEST_TEST(ParseLinearExpression, Test3) {
    ...
    AppendFreeVariable(dut.task(), dut.decision_variable_to_mosek_nonmatrix_variable().size() + num_slack_vars);
    std::vector<MSKint32t> slack_vars_mosek_indices = {
            static_cast<int>(dut.decision_variable_to_mosek_nonmatrix_variable().size()) + 1,
            static_cast<int>(dut.decision_variable_to_mosek_nonmatrix_variable().size()) + 2};
    ...
    std::vector<MSKrealt> F_valij;
    BarFType bar_F;
    const auto rescode = dut.ParseLinearExpression(prog, A_sparse, B_sparse, decision_vars, slack_vars_mosek_indices,
    ...
    EXPECT_FALSE(bar_F.empty());
    CheckParseLinearExpression(dut, prog, A_sparse, B_sparse, decision_vars, slack_vars_mosek_indices, F_subi, F_subj,
                               F_valij, bar_F);
    MSK_deleteenv(&env);
}
 ...
 */
VectorX<symbolic::Expression> GetAffineExpression(const MathematicalProgram& prog,
                                                  const MosekSolverProgram& dut,
    ...
    for (MSKint64t i = 0; i < num_afe; ++i) {
        ...
        MSK_getafebarfrow(dut.task(), i, barvar_idx.data(), ptr_term.data(), num_term.data(), term_idx.data(),
                          term_weight.data());
        for (int k = 0; k < num_entry; ++k) {
            const MSKint32t j = barvar_idx[k];
            Eigen::MatrixXd bar_F_ij = Eigen::MatrixXd::Zero(bar_X[j].rows(), bar_X[j].rows());
...
GTEST_TEST(AddConeConstraings, Test1) {
    ...
    auto rescode = dut.AddConeConstraints(prog, prog.lorentz_cone_constraints(), &acc_indices);
    ...
    MSKint64t afe_f_nnz;
    MSK_getafefnumnz(dut.task(), &afe_f_nnz);
    EXPECT_EQ(afe_f_nnz, constraint1.evaluator()->A().nonZeros() + constraint2.evaluator()->A().nonZeros());
    const VectorX<symbolic::Expression> affine_expressions = GetAffineExpression(prog, dut, {});
...
GTEST_TEST(AddConeConstraints, Test2) {
    ...
    A1 << 1, 2, -1, 3, 0, 2, 1, 0;
    Eigen::Vector4d b1(1, 0, -2, 3);
    auto constraint1 = prog.AddRotatedLorentzConeConstraint(A1, b1, Vector2<symbolic::Variable>(x(0), X1(1, 1)));
    Eigen::Matrix<double, 3, 3> A2;
    A2 << 1, 2, -1, 3, 0, 1, 0, 1, -4;
    Eigen::Vector3d b2(2, 1, 0);
    auto constraint2 =
            prog.AddRotatedLorentzConeConstraint(A2, b2, Vector3<symbolic::Variable>(x(2), X1(0, 1), X2(2, 1)));
    MSKenv_t env;
    MSK_makeenv(&env, nullptr);
    MosekSolverProgram dut(prog, env);
    AppendFreeVariable(dut.task(), dut.decision_variable_to_mosek_nonmatrix_variable().size());
    ...
    std::unordered_map<Binding<RotatedLorentzConeConstraint>, MSKint64t> acc_indices;
    auto rescode = dut.AddConeConstraints(prog, prog.rotated_lorentz_cone_constraints(), &acc_indices);
    ASSERT_EQ(rescode, MSK_RES_OK);
    ...
    const VectorX<symbolic::Expression> affine_expressions = GetAffineExpression(prog, dut, {});
    VectorX<symbolic::Expression> affine_expressions_expected(A1.rows() + A2.rows());
    affine_expressions_expected.head(A1.rows()) = A1 * constraint1.variables() + b1;
    affine_expressions_expected(0) *= 0.5;
    affine_expressions_expected.tail(A2.rows()) = A2 * constraint2.variables() + b2;
...
GTEST_TEST(AddQuadraticCostAsLinearCost, Test) {
    ...
    EXPECT_TRUE(CompareMatrices(c.head(x.rows()), Eigen::VectorXd::Zero(x.rows())));
    EXPECT_EQ(c(c.rows() - 1), 1);

    // Check the affine expression.
    const Eigen::MatrixXd L = math::DecomposePSDmatrixIntoXtransposeTimesX(Q, std::numeric_limits<double>::epsilon());
    std::unordered_map<MSKint32t, symbolic::Variable> slack_vars;
    symbolic::Variable s("s");
    slack_vars.emplace(x.rows(), s);
    VectorX<symbolic::Expression> affine_expressions = GetAffineExpression(prog, dut, slack_vars);
    EXPECT_PRED2(symbolic::test::ExprEqual, (2 * affine_expressions(0) * affine_expressions(1)).Expand(), 2 * s);
    // The sum-of-squares for affine_expressions(i), i> 1 is x'*Q*x.
    EXPECT_PRED3(symbolic::test::PolynomialEqual,
                 symbolic::Polynomial(affine_expressions.tail(affine_expressions.rows() - 2).array().square().sum()),
                 symbolic::Polynomial(x.cast<symbolic::Expression>().dot(Q * x)), 1E-10);

    // Add an arbitrary linear cost.
    const Eigen::SparseMatrix<double> linear_coeff = Eigen::Vector2d(1, 2).sparseView();
    ...
    EXPECT_TRUE(CompareMatrices(Q * x_sol + linear_coeff.toDense(), Eigen::VectorXd::Zero(x.rows()), 1E-10));
...
GTEST_TEST(AddQuadraticConstraint, Test) {
    ...
    std::unordered_map<Binding<LinearEqualityConstraint>, ConstraintDualIndices> linear_eq_con_dual_indices;
    MSKrescodee rescode = dut.AddLinearConstraints(prog, &linear_con_dual_indices, &linear_eq_con_dual_indices);
    ...
    const auto quadratic_con0 = prog.AddQuadraticConstraint(Q0, b0, -kInf, 10, Vector2<symbolic::Variable>(x(2), x(0)));
  ...
  Q1 << -1, -3,
    ...
    const auto quadratic_con1 = prog.AddQuadraticConstraint(Q1, b1, -2, kInf, Vector2<symbolic::Variable>(x(1), x(1)));
    std::unordered_map<Binding<QuadraticConstraint>, MSKint64t> quadratic_constraint_dual_indices;
    rescode = dut.AddQuadraticConstraints(prog, &quadratic_constraint_dual_indices);
    ...
    EXPECT_EQ(quadratic_constraint_dual_indices.at(quadratic_con1), 2);
    {
        ...
        MSK_getqconk(dut.task(), 1, maxnumqcnz, &numqcnz, qcsubi.data(), qcsubj.data(), qcval.data());
        EXPECT_EQ(numqcnz, 2);
    ...
    }
    {
        ...
        std::vector<MSKint32t> qcsubj(maxnumqcnz);
        std::vector<MSKrealt> qcval(maxnumqcnz);
        MSK_getqconk(dut.task(), 2, maxnumqcnz, &numqcnz, qcsubi.data(), qcsubj.data(), qcval.data());
        EXPECT_EQ(numqcnz, 1);
    ...
    EXPECT_TRUE(quadratic_con0.evaluator()->CheckSatisfied(Eigen::Vector2d(x_sol(2), x_sol(0))));
    EXPECT_TRUE(quadratic_con1.evaluator()->CheckSatisfied(Eigen::Vector2d(x_sol(1), x_sol(1))));
...
GTEST_TEST(AddLinearMatrixInequalityConstraint, LMIonly) {
    ...
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 0, 1, 0, 1, 2, 1, 0, 2, 1).finished()));
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 1, -1, 2, 0, 1, 1, 0, 1, -1).finished()));
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 2, 0, 1, 0, 2, -1, 0, 2, -1).finished()));
    auto lmi0 = prog.AddLinearMatrixInequalityConstraint(F, x.head<2>());
    F.clear();
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 1, 3, 1, 2, -4, 1, 0, 1, 0).finished()));
    F.push_back(ToSymmetric((Eigen::Matrix3d() << -1, 0, 1, 1, -2, 1, 0, 1, 0).finished()));
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = j; i < 3; ++i) {
            ...
            affine_expressions_expected(affine_expression_count++) =
                    scaling_factor *
                    (lmi0.evaluator()->F()[0](i, j) + lmi0.evaluator()->F()[1](i, j) * lmi0.variables()(0) +
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = j; i < 3; ++i) {
            ...
            affine_expressions_expected(affine_expression_count++) =
                    scaling_factor *
                    (lmi1.evaluator()->F()[0](i, j) + lmi1.evaluator()->F()[1](i, j) * lmi1.variables()(0));
        }
    }
    for (int i = 0; i < 12; ++i) {
        EXPECT_PRED2(symbolic::test::ExprEqual, affine_expressions_expected(i).Expand(),
...
GTEST_TEST(AddLinearMatrixInequalityConstraint, LMIandPSD) {
    ...
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 1, -1, 2, 0, 1, 1, 0, 1, -1).finished()));
    F.push_back(ToSymmetric((Eigen::Matrix3d() << 2, 0, 1, 0, 2, -1, 0, 2, -1).finished()));
    auto lmi0 = prog.AddLinearMatrixInequalityConstraint(F, Vector2<symbolic::Variable>(X(0, 1), x(0)));
