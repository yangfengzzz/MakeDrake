...
typedef std::set<Monomial, drake::symbolic::GradedReverseLexOrder<std::less<Variable>>> MonomialSet;
...
TEST_F(SosBasisGeneratorTest, MotzkinPoly) {
    const symbolic::Polynomial poly{pow(x_(0), 2) * pow(x_(1), 4) + pow(x_(0), 4) * pow(x_(1), 2) +
...
TEST_F(SosBasisGeneratorTest, Univariate) {
    const symbolic::Polynomial poly{1 + pow(x_(0), 1) + pow(x_(0), 2) + pow(x_(0), 8)};
    MonomialSet basis_ref;
    basis_ref.insert(Monomial());
...
TEST_F(SosBasisGeneratorTest, NewtonPolytopeEmptyInterior) {
    const symbolic::Polynomial poly{pow(x_(0), 2) * pow(x_(1), 2) + pow(x_(0), 3) * pow(x_(1), 3) +
