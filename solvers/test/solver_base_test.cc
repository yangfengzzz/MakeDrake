...
public:
    ...
    void DoSolve(const MathematicalProgram& prog,
...
GTEST_TEST(SolverBaseTest, ProgramAttributesDefault) {
    ...
    EXPECT_THAT(dut.ExplainUnsatisfiedProgramAttributes(prog), HasSubstr("StubSolverBase is unable to solve"));
...
GTEST_TEST(SolverBaseTest, ProgramAttributesCustom) {
    ...
    dut.satisfied_ = false;
    EXPECT_FALSE(dut.AreProgramAttributesSatisfied(prog));
    EXPECT_THAT(dut.ExplainUnsatisfiedProgramAttributes(prog), HasSubstr("affairs of wizards"));
...
GTEST_TEST(SolverBaseTest, AvailableError) {
    ...
    StubSolverBase dut;
    dut.available_ = false;
    DRAKE_EXPECT_THROWS_MESSAGE(dut.Solve(prog, {}, {}), ".*StubSolverBase has not been compiled.*");
}
...
GTEST_TEST(SolverBaseTest, ProgramAttributesError) {
    ...
    StubSolverBase dut;
    dut.satisfied_ = false;
    DRAKE_EXPECT_THROWS_MESSAGE(dut.Solve(prog, {}, {}), ".*StubSolverBase is unable to solve.*");
}
