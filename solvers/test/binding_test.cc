...
TEST_F(TestBinding, TestConstraintConstruction) {
    auto bb_con1 = std::make_shared<BoundingBoxConstraint>(Eigen::Vector3d::Zero(), Eigen::Vector3d::Ones());

    // Checks if the bound variables are stored in the right order.
    Binding<BoundingBoxConstraint> binding1(bb_con1,
    ...
    Binding<BoundingBoxConstraint> binding2(bb_con1, VectorDecisionVariable<3>(x3_, x1_, x2_));
...
TEST_F(TestBinding, TestPrinting) {
    ...
    Eigen::Matrix2d Aeq;
    Aeq << 1, 2, 3, 4;
    auto linear_eq_constraint = std::make_shared<LinearEqualityConstraint>(Aeq, Eigen::Vector2d(1, 2));
    Binding<LinearEqualityConstraint> linear_eq_binding(linear_eq_constraint, VectorDecisionVariable<2>(x1_, x2_));
    const std::string str_expected2 = "LinearEqualityConstraint\n(x1 + 2 * x2) == 1\n(3 * x1 + 4 * x2) == 2\n";
    ...
    auto linear_ineq_constraint = std::make_shared<LinearConstraint>(Ain, Eigen::Vector2d(1, 2), Eigen::Vector2d(2, 3));
    Binding<LinearConstraint> linear_binding(linear_ineq_constraint, Vector2<symbolic::Variable>(x1_, x2_));
...
TEST_F(TestBinding, TestHash) {
    ...
    const Binding<LinearConstraint> binding1_cast = internal::BindingDynamicCast<LinearConstraint>(binding1);
    const Binding<LinearConstraint> binding2_cast = internal::BindingDynamicCast<LinearConstraint>(binding2);
    EXPECT_EQ(binding1_cast, binding2_cast);
    // binding3 has different variables.
    Binding<BoundingBoxConstraint> binding3(bb_con1, VectorDecisionVariable<3>(x3_, x2_, x1_));
    EXPECT_NE(binding1, binding3);
    // bb_con2 has different address from bb_con1, although they have the same
    // bounds.
    auto bb_con2 = std::make_shared<BoundingBoxConstraint>(Eigen::Vector3d::Zero(), Eigen::Vector3d::Ones());
    ...
    Binding<BoundingBoxConstraint> binding4(bb_con2, VectorDecisionVariable<3>(x3_, x1_, x2_));
...
TEST_F(TestBinding, TestCost) {
    ...
    auto cost2 = std::make_shared<QuadraticCost>(Eigen::Matrix2d::Identity(), Eigen::Vector2d(2, 3), 1);
