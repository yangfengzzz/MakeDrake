...
void DrawCircle(double radius = 1.0) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        points.col(i) = Eigen::Vector2d(radius * std::cos(theta), radius * std::sin(theta));
    }

    common::CallPython("plot", points.row(0), points.row(1), "k", "linewidth", 4.0);
}

void PlotFeasiblePoints(const Eigen::Matrix2Xd& points, double radius = 1.0, int fig_num = 1) {
...
void PlotColumnVectorXYSlice(double z = 0.0, int fig_num = 1) {
    ...
    std::shared_ptr<LinearEqualityConstraint> feasibility_constraint =
            prog.AddLinearEqualityConstraint(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero(), R.col(0))
                    .evaluator();

    const int num_samples_per_axis = 50;
    Eigen::Matrix2Xd feasible_points(2, num_samples_per_axis * num_samples_per_axis);
    ...
    PlotFeasiblePoints(feasible_points, std::sqrt(1 - sample(2) * sample(2)), fig_num);
    using common::CallPython;
