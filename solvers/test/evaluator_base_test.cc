...
AssertionResult CompareAutodiff(const AutoDiffVecXd& tx_expected,
    ...
    AssertionResult value_result = CompareMatrices(x_expected, x_actual, tolerance);
    if (!value_result) {
        return value_result << "(value)";
    }
    const MatrixXd dx_expected = math::ExtractGradient(tx_expected);
    const MatrixXd dx_actual = math::ExtractGradient(tx_actual);
    AssertionResult grad_result = CompareMatrices(dx_expected, dx_actual, tolerance);
    if (!grad_result) {
...
void VerifyFunctionEvaluator(F&& f, const VectorXd& x) {
    ...
    shared_ptr<EvaluatorBase> evaluator = MakeFunctionEvaluator(std::forward<F>(f));
...
public:
    ...
    FunctionWrapper(CallableF&& callable, int num_outputs, int num_vars)
        : num_outputs_(num_outputs), num_vars_(num_vars), callable_(std::forward<CallableF>(callable)) {}
...
GTEST_TEST(EvaluatorBaseTest, SetGradientSparsityPattern) {
    ...
    EXPECT_EQ(fmt::format("{}", evaluator), "SimpleEvaluator with 3 decision variables $(0) $(1) $(2)\n");
    ...
    evaluator.SetGradientSparsityPattern({{0, 0}, {0, 1}, {0, 2}, {1, 0}, {1, 1}, {1, 2}});
    const auto& gradient_sparsity_pattern = evaluator.gradient_sparsity_pattern();
    int gradient_entry_count = 0;
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 3; ++j) {
            EXPECT_EQ(gradient_sparsity_pattern.value()[gradient_entry_count].first, i);
            EXPECT_EQ(gradient_sparsity_pattern.value()[gradient_entry_count].second, j);
    ...
    if (kDrakeAssertIsArmed) {
        ...
        EXPECT_THROW(evaluator.SetGradientSparsityPattern({{-1, 0}}), std::invalid_argument);
        // column index out of range.
        EXPECT_THROW(evaluator.SetGradientSparsityPattern({{0, -1}}), std::invalid_argument);
        // repeated entries.
        EXPECT_THROW(evaluator.SetGradientSparsityPattern({{0, 0}, {0, 0}}), std::invalid_argument);
    }
}
