#include "solvers/augmented_lagrangian.h"
...
void TestEmptyProg(const AL& dut) {
    ...
    if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
        al_val = dut.template Eval<double>(x_val, Eigen::VectorXd(0), 0.5, &constraint_residue, &cost);
    } else {
        al_val = dut.template Eval<double>(x_val, Eigen::VectorXd(0), Eigen::VectorXd(0), 0.5, &constraint_residue,
                                           &cost);
    }
    EXPECT_EQ(al_val, 0);
...
void TestObjective(const AL& dut) {
    ...
    if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
        al_ad = dut.template Eval<AutoDiffXd>(x_ad, Eigen::VectorXd(0), 0.5, &constraint_residue_ad, &cost_ad);
    } else {
        al_ad = dut.template Eval<AutoDiffXd>(x_ad, AutoDiffVecXd(0), Eigen::VectorXd(0), 0.5, &constraint_residue_ad,
                                              &cost_ad);
    }
    ...
    EXPECT_TRUE(CompareMatrices(cost_ad.derivatives(), al_expected.derivatives()));
...
public:
    DummyConstraint() : Constraint(4, 2, Eigen::Vector4d(1, 2, 3, 4), Eigen::Vector4d(1, 2, 3, 4)) {}
...
template <typename T>
void CompareAlResult(const T& al,
                     const T& al_expected,
                     const VectorX<T>& constraint_residue,
                     const VectorX<T>& constraint_residue_expected,
                     const T& cost,
                     const T& cost_expected,
                     double tol) {
    if constexpr (std::is_same_v<T, double>) {
        EXPECT_NEAR(al, al_expected, tol);
        EXPECT_TRUE(CompareMatrices(constraint_residue, constraint_residue_expected, tol));
        EXPECT_NEAR(cost, cost_expected, tol);
    } else {
        EXPECT_NEAR(al.value(), al_expected.value(), tol);
        EXPECT_TRUE(CompareMatrices(al.derivatives(), al_expected.derivatives(), tol));
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(constraint_residue),
                                    math::ExtractValue(constraint_residue_expected), tol));
        EXPECT_TRUE(CompareMatrices(math::ExtractGradient(constraint_residue),
                                    math::ExtractGradient(constraint_residue_expected), tol));
        EXPECT_NEAR(cost.value(), cost_expected.value(), tol);
        EXPECT_TRUE(CompareMatrices(cost.derivatives(), cost_expected.derivatives(), tol));
    }
}

template <typename T>
T al_equality(const Eigen::Ref<const VectorX<T>>& lhs, const Eigen::Ref<const Eigen::VectorXd>& lambda, double mu) {
...
template <typename T>
void CheckAugmentedLagrangianEqualityConstraint(const MathematicalProgram& prog,
                                                const Binding<Constraint>& constraint,
                                                const Vector3<T>& x_val,
                                                const Eigen::Vector4d& lambda_val,
                                                double mu_val,
                                                double tol_val,
                                                bool smooth_flag) {
    ...
    for (bool include_x_bounds : {false, true}) {
        ...
        if (smooth_flag) {
            ...
            al_val = dut.Eval<T>(x_val, VectorX<T>(0), lambda_val, mu_val, &constraint_residue, &cost);
        } else {
            const AugmentedLagrangianNonsmooth dut(&prog, include_x_bounds);
            EXPECT_EQ(dut.lagrangian_size(), 4);
            EXPECT_EQ(dut.is_equality(), std::vector<bool>({true, true, true, true}));
            al_val = dut.Eval<T>(x_val, lambda_val, mu_val, &constraint_residue, &cost);
        }
        const auto constraint_val = prog.EvalBinding(constraint, x_val);
        const auto& constraint_bound = constraint.evaluator()->lower_bound();
        const T al_expected = al_equality<T>(constraint_val - constraint_bound, lambda_val, mu_val);
        EXPECT_EQ(constraint_residue.rows(), lambda_val.rows());
        CompareAlResult<T>(al_val, al_expected, constraint_residue, constraint_val - constraint_bound, cost, T{0},
                           tol_val);
    }
}
...
GTEST_TEST(AugmentedLagrangian, EqualityConstraints) {
    ...
    auto constraint = prog.AddConstraint(std::make_shared<DummyConstraint>(), x.head<2>());

    const double tol = 1E-10;
    for (bool smooth_flag : {false, true}) {
        CheckAugmentedLagrangianEqualityConstraint(prog, constraint, Eigen::Vector3d(0.5, 1.5, 2),
                                                   Eigen::Vector4d(1, 3, -2, 4), 0.2, tol, smooth_flag);
        CheckAugmentedLagrangianEqualityConstraint(prog, constraint, Eigen::Vector3d(-0.5, 2.5, 2),
                                                   Eigen::Vector4d(-1, 2, 3, -2), 0.6, tol, smooth_flag);
        CheckAugmentedLagrangianEqualityConstraint(prog, constraint,
...
template <typename T>
void CheckAugmentedLagrangianNonsmoothInequalityConstraint(const MathematicalProgram& prog,
                                                           const Binding<Constraint>& constraint,
                                                           const Vector3<T>& x_val,
                                                           const Eigen::Matrix<double, 5, 1>& lambda_val,
                                                           double mu_val,
                                                           double tol_val) {
    VectorX<T> constraint_residue;
    T cost;
    for (const bool include_x_bounds : {false, true}) {
        ...
        EXPECT_EQ(dut.is_equality(), std::vector<bool>({true, false, false, false, false}));

        const T al = dut.Eval<T>(x_val, lambda_val, mu_val, &constraint_residue, &cost);
        const auto constraint_val = prog.EvalBinding(constraint, x_val);
        const auto& lb = constraint.evaluator()->lower_bound();
        const auto& ub = constraint.evaluator()->upper_bound();
        using std::pow;
        T al_expected = -lambda_val(0) * (constraint_val(0) - lb(0)) + mu_val / 2 * pow(constraint_val(0) - lb(0), 2) +
        ...
        constraint_residue_expected << constraint_val(0) - lb(0), constraint_val(1) - lb(1), ub(1) - constraint_val(1),
                ub(2) - constraint_val(2), constraint_val(3) - lb(3);
        CompareAlResult<T>(al, al_expected, constraint_residue, constraint_residue_expected, cost, T(0), tol_val);
    }
}

template <typename T>
void CheckAugmentedLagrangianSmoothInequalityConstraint(const MathematicalProgram& prog,
                                                        const Binding<Constraint>& constraint,
                                                        const Vector3<T>& x_val,
                                                        const Vector4<T>& s_val,
                                                        const Eigen::Matrix<double, 5, 1>& lambda_val,
                                                        double mu_val,
                                                        double tol_val) {
    VectorX<T> constraint_residue;
    ...
    for (const bool include_x_bounds : {false, true}) {
        ...
        const T al = dut.Eval<T>(x_val, s_val, lambda_val, mu_val, &constraint_residue, &cost);
        const auto constraint_val = prog.EvalBinding(constraint, x_val);
        const auto& lb = constraint.evaluator()->lower_bound();
        const auto& ub = constraint.evaluator()->upper_bound();
        using std::pow;
        Eigen::Matrix<T, 5, 1> lhs;
        lhs << constraint_val(0) - lb(0), constraint_val(1) - lb(1) - s_val(0), ub(1) - constraint_val(1) - s_val(1),
                ub(2) - constraint_val(2) - s_val(2), constraint_val(3) - lb(3) - s_val(3);
...
GTEST_TEST(AugmentedLagrangian, InequalityConstraint) {
    ...
    constraint_evaluator->set_bounds(Eigen::Vector4d(1, -1, -kInf, 2), Eigen::Vector4d(1, 2, 3, kInf));
    ...
    CheckAugmentedLagrangianNonsmoothInequalityConstraint(
            ...
            (Eigen::Matrix<double, 5, 1>() << 0.5, 0.4, 1.5, 0.2, 3).finished(), 0.5, tol);
    CheckAugmentedLagrangianNonsmoothInequalityConstraint(
            prog, constraint, Eigen::Vector3d(-1, 3, -5),
            (Eigen::Matrix<double, 5, 1>() << -.5, 1.4, 1.5, 0.5, 3).finished(), 0.2, tol);
    CheckAugmentedLagrangianNonsmoothInequalityConstraint(
    ...
    CheckAugmentedLagrangianSmoothInequalityConstraint(
            ...
            Vector4<AutoDiffXd>(AutoDiffXd(3), AutoDiffXd(-1), AutoDiffXd(-2), AutoDiffXd(1)),
            (Eigen::Matrix<double, 5, 1>() << 0.5, 0.4, 1.5, 0.2, 3).finished(), 0.5, tol);
}

template <typename T>
void CheckAugmentedLagrangianNonsmoothBoundingBoxConstraint(const MathematicalProgram& prog,
                                                            const Vector4<T>& x_val,
                                                            const Eigen::Matrix<double, 5, 1>& lambda,
                                                            double mu,
                                                            double tol_val) {
    ...
    const T al_expected = -lambda(0) * (x_val(0) - x_lb(0)) + mu / 2 * (x_val(0) - x_lb(0)) * (x_val(0) - x_lb(0)) +
                          psi(x_val(1) - x_lb(1), lambda(1), mu) + psi(x_ub(2) - x_val(2), lambda(2), mu) +
                          psi(x_val(3) - x_lb(3), lambda(3), mu) + psi(x_ub(3) - x_val(3), lambda(4), mu);
    Eigen::Matrix<T, 5, 1> constraint_residue_expected;
    constraint_residue_expected << x_val(0) - x_lb(0), x_val(1) - x_lb(1), x_ub(2) - x_val(2), x_val(3) - x_lb(3),
            x_ub(3) - x_val(3);
    CompareAlResult<T>(al, al_expected, constraint_residue, constraint_residue_expected, cost, T(0), tol_val);
...
template <typename T>
void CheckAugmentedLagrangianSmoothBoundingBoxConstraint(const MathematicalProgram& prog,
                                                         const Vector4<T>& x_val,
                                                         const Vector4<T>& s_val,
                                                         const Eigen::Matrix<double, 5, 1>& lambda,
                                                         double mu,
    ...
    VectorX<T> constraint_residue;
    T cost;
    const T al = dut.Eval<T>(x_val, s_val, lambda, mu, &constraint_residue, &cost);
    EXPECT_EQ(constraint_residue.rows(), lambda.rows());
    Eigen::VectorXd x_lb, x_ub;
    AggregateBoundingBoxConstraints(prog, &x_lb, &x_ub);
    Eigen::Matrix<T, 5, 1> lhs;
    lhs << x_val(0) - x_lb(0), x_val(1) - x_lb(1) - s_val(0), x_ub(2) - x_val(2) - s_val(1),
            x_val(3) - x_lb(3) - s_val(2), x_ub(3) - x_val(3) - s_val(3);
...
void CheckBoundingBoxConstraintEmpty(const AL& dut) {
    ...
    double al_val;
    if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
        al_val = dut.template Eval<double>(Eigen::Vector4d(1, 2, 3, 4), Eigen::VectorXd(0), 0.5, &constraint_residue,
                                           &cost);
    } else {
        al_val = dut.template Eval<double>(Eigen::Vector4d(1, 2, 3, 4), Eigen::VectorXd(0), Eigen::VectorXd(0), 0.5,
...
GTEST_TEST(EvalAugmentedLagrangian, BoundingBoxConstraint) {
    ...
    auto constraint1 =
            prog.AddBoundingBoxConstraint(Eigen::Vector4d(1, 3, -kInf, 0), Eigen::Vector4d(2, kInf, -1, 5), x);
    auto constraint2 =
            prog.AddBoundingBoxConstraint(Eigen::Vector4d(2, 1, -kInf, 1), Eigen::Vector4d(5, kInf, -1, 3), x);

    const double tol = 1E-10;
    CheckAugmentedLagrangianNonsmoothBoundingBoxConstraint(
            prog, Eigen::Vector4d(1, 3, 5, 2), (Eigen::Matrix<double, 5, 1>() << 0.5, 0.3, 1.5, 2, 1).finished(), 0.5,
            tol);
    CheckAugmentedLagrangianNonsmoothBoundingBoxConstraint(
            prog, math::InitializeAutoDiff(Eigen::Vector4d(1, 3, 5, 2)),
            (Eigen::Matrix<double, 5, 1>() << 0.5, 0.3, 1.5, 2, 1).finished(), 0.5, tol);
