...
#include "solvers/test/l2norm_cost_examples.h"
...
INSTANTIATE_TEST_SUITE_P(GurobiTest,
                         LinearProgramTest,
...
TEST_F(InfeasibleLinearProgramTest0, TestGurobiInfeasible) {
    ...
    if (solver.available()) {
        ...
        EXPECT_EQ(result.get_solution_result(), SolutionResult::kInfeasibleOrUnbounded);
...
TEST_F(UnboundedLinearProgramTest0, TestGurobiUnbounded) {
    ...
    if (solver.available()) {
        ...
        EXPECT_EQ(result.get_solver_details<GurobiSolver>().optimization_status, GRB_INF_OR_UNBD);
        ...
        EXPECT_EQ(result.get_solver_details<GurobiSolver>().optimization_status, GRB_UNBOUNDED);
    ...
    if (solver.available()) {
        ...
        for (const auto& x_expected : optimal_sols) {
            ...
            GurobiSolver::MipNodeCallbackFunction mip_node_callback_function_wrapper =
                    std::bind(MipNodeCallbackFunctionTest, std::placeholders::_1, std::placeholders::_2,
                              std::placeholders::_3, std::placeholders::_4, &cb_info);
            GurobiSolver::MipSolCallbackFunction mip_sol_callback_function_wrapper =
                    std::bind(MipSolCallbackFunctionTest, std::placeholders::_1, std::placeholders::_2, &cb_info);
...
INSTANTIATE_TEST_SUITE_P(GurobiTest, TestEllipsoidsSeparation, ::testing::ValuesIn(GetEllipsoidsSeparationProblems()));
...
INSTANTIATE_TEST_SUITE_P(GurobiTest, TestQPasSOCP, ::testing::ValuesIn(GetQPasSOCPProblems()));
...
GTEST_TEST(GurobiTest, MultipleThreadsSharingEnvironment) {
    ...
    auto solve_program = [](int i, int N) {
        ...
        for (int j = 0; j <= N; ++j) {
            ...
            line_segment(1) += j % 2 == 0 ? symbolic::Expression(lambda(j)) : symbolic::Expression(0);
        }
        ...
        if (i % 2 == 0) {
            ...
            EXPECT_TRUE(std::abs(x_val - (i - 0.5)) < tol || std::abs(x_val - (i + 0.5)) < tol);
        }
    };
...
GTEST_TEST(GurobiTest, GurobiErrorCode) {
    ...
    if (solver.available()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog, {}, solver_options4), ".*Quad.*integer.*not.*string.*");
    }
}
...
GTEST_TEST(GurobiTest, LogFile) {
    ...
    if (solver.available()) {
        ...
        // Set log file through CommonSolverOptions.
        {
            ...
            solver_options.SetOption(CommonSolverOption::kPrintFileName, log_file_common);
        ...
        {
            SolverOptions solver_options;
            const std::string log_file_common = temp_directory() + "/gurobi_common2.log";
            solver_options.SetOption(CommonSolverOption::kPrintFileName, log_file_common);
...
GTEST_TEST(GurobiTest, WriteModel) {
    ...
    if (solver.available()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog, {}, options), ".* setting GRBwrite to foo.wrong_extension.*");
    }
}
...
GTEST_TEST(GurobiTest, SolutionPool) {
    ...
    if (solver.is_available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(b), Eigen::Vector2d(0, 1), tol));
        EXPECT_TRUE(CompareMatrices(result.GetSuboptimalSolution(b, 0), Eigen::Vector2d(0, 1), tol));
        EXPECT_TRUE(CompareMatrices(result.GetSuboptimalSolution(b, 1), Eigen::Vector2d(1, 0), tol));
        EXPECT_NEAR(result.get_optimal_cost(), 0, tol);
...
GTEST_TEST(GurobiTest, SOCPDualSolution1) {
    ...
    if (solver.is_available()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(result.GetDualSolution(constraint1),
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(-1. / 12), 1e-7));
        ...
        result = solver.Solve(prog, std::nullopt, options);
        DRAKE_EXPECT_THROWS_MESSAGE(result.GetDualSolution(bb_con),
                                    "You used Gurobi to solve this optimization problem.*");
        ...
        // should be 1.
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(bb_con), Vector1d(1.), 1E-8));
    }
}
...
GTEST_TEST(GurobiTest, SOCPDualSolution2) {
    ...
    if (solver.is_available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(-1.0 / 4), 1e-8));
        // constraint 2 is not active at the optimal solution, hence the shadow
        // price is 0.
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint2), Vector1d(0), 1e-8));
    }
}
