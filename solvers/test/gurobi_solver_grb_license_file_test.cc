...
protected:
    GrbLicenseFileTest() : orig_grb_license_file_(GetEnvStr("GRB_LICENSE_FILE")) {}
    ...
    void TearDown() override {
        if (orig_grb_license_file_) {
            const int setenv_result = ::setenv("GRB_LICENSE_FILE", orig_grb_license_file_->c_str(), 1);
...
TEST_F(GrbLicenseFileTest, GrbLicenseFileUnset) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver_.Solve(prog_), ".*GurobiSolver has not been properly configured.*");
}
