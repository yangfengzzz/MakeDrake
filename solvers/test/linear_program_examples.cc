...
LinearFeasibilityProgram::LinearFeasibilityProgram(ConstraintForm constraint_form)
...
void LinearFeasibilityProgram::CheckSolution(const MathematicalProgramResult& result) const {
...
LinearProgram0::LinearProgram0(CostForm cost_form, ConstraintForm constraint_form)
    ...
    switch (constraint_form) {
      ...
      A << -1, 1,
          ...
          1, -2;
            // clang-format on
            prog()->AddLinearConstraint(A, b_lb, b_ub, x_);
            prog()->AddBoundingBoxConstraint(Vector2d(0, 2), Vector2d::Constant(numeric_limits<double>::infinity()),
                                             x_);
            break;
        }
        case ConstraintForm::kSymbolic: {
...
void LinearProgram0::CheckSolution(const MathematicalProgramResult& result) const {
    const double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
...
LinearProgram1::LinearProgram1(CostForm cost_form, ConstraintForm constraint_form)
...
void LinearProgram1::CheckSolution(const MathematicalProgramResult& result) const {
    const double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), x_expected_, tol, MatrixCompareType::absolute));
    ExpectSolutionCostAccurate(*prog(), result, tol);
}

LinearProgram2::LinearProgram2(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_(), x_expected_(0, 0, 15, 25.0 / 3.0) {
...
void LinearProgram2::CheckSolution(const MathematicalProgramResult& result) const {
    const double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), x_expected_, tol, MatrixCompareType::absolute));
    ExpectSolutionCostAccurate(*prog(), result, tol);
}

LinearProgram3::LinearProgram3(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_(), x_expected_(8, 3, 11) {
...
void LinearProgram3::CheckSolution(const MathematicalProgramResult& result) const {
...
std::vector<LinearProblems> linear_problems() {
    return std::vector<LinearProblems>{LinearProblems::kLinearFeasibilityProgram, LinearProblems::kLinearProgram0,
                                       LinearProblems::kLinearProgram1, LinearProblems::kLinearProgram2,
                                       LinearProblems::kLinearProgram3};
}

InfeasibleLinearProgramTest0::InfeasibleLinearProgramTest0() : prog_(std::make_unique<MathematicalProgram>()) {
...
UnboundedLinearProgramTest0::UnboundedLinearProgramTest0() : prog_(std::make_unique<MathematicalProgram>()) {
...
UnboundedLinearProgramTest1::UnboundedLinearProgramTest1() : prog_(std::make_unique<MathematicalProgram>()) {
...
DuplicatedVariableLinearProgramTest1::DuplicatedVariableLinearProgramTest1()
    ...
    prog_->AddLinearCost(Eigen::Vector4d(1, 2, 1, 4), Vector4<symbolic::Variable>(x_(0), x_(1), x_(1), x_(2)));
    prog_->AddLinearCost(Eigen::Vector2d(2, -2), 3, Vector2<symbolic::Variable>(x_(0), x_(0)));
    prog_->AddLinearEqualityConstraint(Eigen::RowVector3d(2, 2, -1), Vector1d(1),
  ...
  A << -1, 1, 3, 2, -1,
    ...
    prog_->AddLinearConstraint(A, Eigen::Vector2d(-kInf, 1), Eigen::Vector2d(3, 3), vars);
    prog_->AddLinearConstraint(Eigen::RowVector4d(2, 3, 2, -1), Vector1d(0), Vector1d(kInf),
                               Vector4<symbolic::Variable>(x_(1), x_(2), x_(0), x_(0)));
}

void DuplicatedVariableLinearProgramTest1::CheckSolution(const SolverInterface& solver,
    ...
    if (solver.available()) {
        ...
        EXPECT_NEAR(result.get_optimal_cost(), x_sol(0) + 3 * x_sol(1) + 4 * x_sol(2) + 3, tol);
...
void TestLPDualSolution1(const SolverInterface& solver, double tol) {
  ...
  A << 1, 2,
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result1.GetDualSolution(constraint), dual_solution_expected, tol));
        ...
        EXPECT_TRUE(CompareMatrices(result2.GetDualSolution(constraint), dual_solution_expected, tol));
    }
}
...
void TestLPDualSolution2(const SolverInterface& solver, double tol) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    auto constraint = prog.AddBoundingBoxConstraint(Eigen::Vector2d(-1, 2), Eigen::Vector2d(3, 5), x);
    prog.AddLinearCost(x[0] - x[1]);
    if (solver.available()) {
        MathematicalProgramResult result;
        solver.Solve(prog, {}, {}, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Eigen::Vector2d(1, -1), tol));
    }
}
...
void TestLPDualSolution3(const SolverInterface& solver, double tol) {
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Eigen::Vector2d(-1, 0), tol));
        // constraint2 has a lower bound x(1) >= -1 being active.
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint2), Vector1d(1), tol));
        // constraint 3 is not active.
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint3), Vector1d::Zero(), tol));
    }
}
...
void TestLPDualSolution4(const SolverInterface& solver, double tol) {
    ...
    auto constraint1 = prog.AddLinearEqualityConstraint(x(0) + 2 * x(1) + 3 * x(3) == 4);
    Eigen::Matrix2d A2;
    A2 << 1, 3, -2, 2;
    auto constraint2 = prog.AddLinearEqualityConstraint(A2, Eigen::Vector2d(2, 5), x.segment<2>(1));
...
void TestLPDualSolution5(const SolverInterface& solver, double tol) {
    ...
    // 1 <= x(1) <= 3
    // 2 <= x(2) <= 2
    auto bbcon1 = prog.AddBoundingBoxConstraint(Eigen::Vector2d(1, 2), Eigen::Vector2d(3, 2), x.tail<2>());
    // -1 <= x(0) <= -1
    // -2 <= x(1) <= 1
    //
    ...
    // later.
    auto bbcon2 = prog.AddBoundingBoxConstraint(Eigen::Vector2d(-1, -2), Eigen::Vector2d(-1, 1), x.head<2>());
    prog.AddLinearCost(x(0) + 2 * x(1) + 3 * x(2));
    MathematicalProgramResult result;
    if (solver.available()) {
        solver.Solve(prog, std::nullopt, std::nullopt, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector3d(-1, 1, 2), tol));
...
void TestLPPoorScaling2(const SolverInterface& solver,
  ...
  9.68599420e-01,  2.48626556e-01, -1.27003703e-12;
    ...
    b << 1.0, 0.5, 1.0, -0.0, 0.5, -0.0, 0.46783912, 0.39023174, 0.50647967, 0.50647967;
