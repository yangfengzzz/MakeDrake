...
#include "solvers/evaluator_base.h"
...
GTEST_TEST(testCost, testLinearCost) {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff), Vector1<double>(obj_expected), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), a.transpose() * x_grad, tol));
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff_identity), Vector1<double>(obj_expected), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff_identity), a.transpose(), tol));
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff_empty), Vector1<double>(obj_expected), tol));
    ...
    EXPECT_THROW(cost->UpdateCoefficients(Eigen::Vector3d::Ones(), b), runtime_error);
    ...
    EXPECT_EQ(fmt::format("{}", *new_cost), "LinearCost (100 + $(0) + 2 * $(1)) described as 'simple linear cost'");
}
...
GTEST_TEST(TestQuadraticCost, NonconvexCost) {
    ...
    EXPECT_TRUE(CompareMatrices(cost->Q(), (Q + Q.transpose()) / 2, 1E-10, MatrixCompareType::absolute));
    ...
    AutoDiffVecXd y_autodiff;
    cost->Eval(x_autodiff, &y_autodiff);
    const AutoDiffXd y_autodiff_expected = 0.5 * x_autodiff.dot(Q * x_autodiff) + b.dot(x_autodiff);
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff), Vector1<double>(obj_expected), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), y_autodiff_expected.derivatives(), tol));
    // Test Eval with identity gradient.
    const AutoDiffVecXd x_autodiff_identity = math::InitializeAutoDiff(x0);
    AutoDiffVecXd y_autodiff_identity;
    cost->Eval(x_autodiff_identity, &y_autodiff_identity);
    ...
    // Test Eval/CheckSatisfied using Expression.
    {
        ...
        EXPECT_PRED2(ExprEqual, y_sym[0].Expand(),
                     // 0.5 x'Qx + bx
                     (0.5 * (x_0 * x_0 + (2 + 3) * x_0 * x_1 + 4 * x_1 * x_1) + 5 * x_0 + 6 * x_1).Expand());
    }

    // Update with an asymmetric Q
    cost->UpdateCoefficients(2 * Q, b);
    EXPECT_TRUE(CompareMatrices(cost->Q(), (Q + Q.transpose()), 1E-10, MatrixCompareType::absolute));
    EXPECT_FALSE(cost->is_convex());
    ...
    EXPECT_THROW(cost->UpdateCoefficients(Eigen::Matrix3d::Identity(), b, c), runtime_error);
    EXPECT_THROW(cost->UpdateCoefficients(Q, Eigen::Vector3d::Ones(), c), runtime_error);
    ...
    cost->UpdateCoefficients(Q + 100 * Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero());
    ...
    cost->UpdateCoefficients(Q + 100 * Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero(), 0., true);
    EXPECT_TRUE(cost->is_convex());
    ...
    // occurs.
    cost->UpdateCoefficients(Q + 100 * Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero(), 0., false);
    EXPECT_FALSE(cost->is_convex());
}
...
GTEST_TEST(TestQuadraticCost, ConvexCost) {
    ...
    cost = Make2NormSquaredCost((Eigen::Matrix2d() << 1, 2, 3, 4).finished(), Eigen::Vector2d(2, 3));
    EXPECT_TRUE(cost->is_convex());
}
...
GTEST_TEST(testCost, testCostShim) {
    ...
    VerifyRelatedCost<QuadraticConstraint, double>(Vector1d(2), Vector1d(3), Vector1d(4));

    const Polynomiald x("x");
    const auto poly = (x - 1) * (x - 1);
    const auto var_mapping = make_vector({x.GetSimpleVariable()});
    VerifyRelatedCost<PolynomialConstraint, Vector1d>(Vector1d(2), VectorXPoly::Constant(1, poly), var_mapping);
}
...
GTEST_TEST(TestL1NormCost, Eval) {
  ...
  A << .32,  2.0, 1.3, -4.,
    ...
    // Test AutoDiffXd.
    {
        ...
        const Matrix<double, 1, 4> grad_expected = z.cwiseQuotient(z.cwiseAbs()).transpose() * A;
        EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y), grad_expected, 1e-15));
    }
...
GTEST_TEST(TestL1NormCost, UpdateCoefficients) {
    ...
    EXPECT_THROW(cost.UpdateCoefficients(Matrix3d::Identity(), Vector3d::Zero()), std::exception);

    // A and b must have the same number of rows.
    EXPECT_THROW(cost.UpdateCoefficients(Matrix3d::Identity(), Vector4d::Zero()), std::exception);
}
...
GTEST_TEST(TestL1NormCost, Display) {
    ...
    EXPECT_EQ(fmt::format("{}", os.str()), "L1NormCost (abs((1 + x(0))) + abs((1 + x(1))))");
}
...
GTEST_TEST(TestL2NormCost, Display) {
    ...
    std::ostringstream os;
    cost.Display(os, symbolic::MakeVectorContinuousVariable(2, "x"));
    EXPECT_EQ(fmt::format("{}", os.str()), "L2NormCost sqrt((pow((1 + x(0)), 2) + pow((1 + x(1)), 2)))");
}
...
GTEST_TEST(TestLInfNormCost, Eval) {
  ...
  A << .32,  2.0, 1.3, -4.,
    ...
    // Test AutoDiffXd.
    {
        ...
        cost.Eval(x, &y);
        int max_row;
        EXPECT_NEAR(z.cwiseAbs().maxCoeff(&max_row), math::ExtractValue(y)[0], 1e-15);
        const Matrix<double, 1, 4> grad_expected = (z.cwiseQuotient(z.cwiseAbs()))(max_row)*A.row(max_row);
        EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y), grad_expected, 1e-15));
    }
...
GTEST_TEST(TestLInfNormCost, Display) {
    ...
    std::ostringstream os;
    cost.Display(os, symbolic::MakeVectorContinuousVariable(2, "x"));
    EXPECT_EQ(fmt::format("{}", os.str()), "LInfNormCost max(abs((1 + x(0))), abs((1 + x(1))))");
}
...
GTEST_TEST(TestPerspectiveQuadraticCost, Eval) {
  ...
  A << .32,  2.0, 1.3, -4.,
    ...
    // Test AutoDiffXd.
    {
        ...
        const Matrix<double, 1, 4> grad_expected = RowVector2d(-(z(1) * z(1)) / (z(0) * z(0)), 2 * z(1) / z(0)) * A;
        EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y), grad_expected, 1e-13));
    }
...
GTEST_TEST(TestPerspectiveQuadraticCost, Display) {
    ...
    std::ostringstream os;
    cost.Display(os, symbolic::MakeVectorContinuousVariable(2, "x"));
    EXPECT_EQ(fmt::format("{}", os.str()), "PerspectiveQuadraticCost (pow((1 + x(1)), 2) / (1 + x(0)))");
}
...
private:
    ...
    void DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, VectorXd* y) const override { this->DoEvalGeneric(x, y); }
...
GTEST_TEST(ToLatex, GenericCost) {
    ...
    EXPECT_EQ(c.ToLatex(vars), "\\text{GenericTrivialCost1}(x_{0}, x_{1}, x_{2}) \\tag{test}");
}
...
GTEST_TEST(ToLatex, PerspectiveQuadraticCost) {
    ...
    c.set_description("test");
    Vector3<Variable> vars = symbolic::MakeVectorVariable<3>("x");
    EXPECT_EQ(c.ToLatex(vars), "\\frac{((2 + x_{1})^{2} + (3 + x_{2})^{2})}{(1 + x_{0})} \\tag{test}");
}
