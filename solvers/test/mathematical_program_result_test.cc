...
public:
    MathematicalProgramResultTest() : x0_{"x0"}, x1_{"x1"}, decision_variable_index_{} {
...
TEST_F(MathematicalProgramResultTest, DefaultConstructor) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(result.get_abstract_solver_details(), "The solver_details has not been set yet.");
}
...
TEST_F(MathematicalProgramResultTest, Setters) {
    ...
    EXPECT_TRUE(CompareMatrices(
            result.get_x_val(),
            Eigen::VectorXd::Constant(decision_variable_index_.size(), std::numeric_limits<double>::quiet_NaN())));
...
TEST_F(MathematicalProgramResultTest, GetSolution) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(result.GetSolution(y),
    ...
    EXPECT_TRUE(result.GetSolution(e).EqualTo(symbolic::Expression{x_val(0) + x_extra}));
...
TEST_F(MathematicalProgramResultTest, GetSolutionPolynomial) {
    ...
    const symbolic::Polynomial p2((1 + x0_ * x1_) * t1 * t1 + 2 * sin(x1_) * t1 * t2 + 3 * x0_, {t1, t2});
    EXPECT_PRED2(symbolic::test::PolyEqual,
                 symbolic::Polynomial({{symbolic::Monomial(t1, 2), 1 + x_val(0) * x_val(1)},
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(result.GetSolution(symbolic::Polynomial(x0_ * t1 + 1, {x0_, t1})),
                                ".*x0 is an indeterminate in the polynomial.*");
}

TEST_F(MathematicalProgramResultTest, DualSolution) {
    MathematicalProgramResult result;
    auto bb_con = std::make_shared<BoundingBoxConstraint>(Eigen::Vector2d(0, 1), Eigen::Vector2d(2, 4));
    Binding<BoundingBoxConstraint> binding1(bb_con, Vector2<symbolic::Variable>(x0_, x1_));
    const Eigen::Vector2d dual_solution1(5, 6);
    result.set_dual_solution(binding1, dual_solution1);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(binding1), dual_solution1));

    auto lin_con = std::make_shared<LinearConstraint>(Eigen::Matrix2d::Identity(), Eigen::Vector2d(-1, -3),
                                                      Eigen::Vector2d(2, 4));
    Binding<LinearConstraint> binding2(lin_con, Vector2<symbolic::Variable>(x0_, x1_));
    const Eigen::Vector2d dual_solution2(3, -2);
    result.set_dual_solution(binding2, dual_solution2);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(binding2), dual_solution2));

    auto lin_eq_con = std::make_shared<LinearEqualityConstraint>(Eigen::Matrix2d::Identity(), Eigen::Vector2d(2, 4));
    Binding<LinearEqualityConstraint> binding3(lin_eq_con, Vector2<symbolic::Variable>(x0_, x1_));
    const Eigen::Vector2d dual_solution3(4, -1);
    result.set_dual_solution(binding3, dual_solution3);
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(binding3), dual_solution3));

    // GetDualSolution for a binding whose dual solution has not been set yet.
    Binding<LinearEqualityConstraint> binding4(lin_eq_con, Vector2<symbolic::Variable>(x1_, x0_));
    DRAKE_EXPECT_THROWS_MESSAGE(result.GetDualSolution(binding4),
...
TEST_F(MathematicalProgramResultTest, SetSolverDetails) {
    ...
    result.set_decision_variable_index(decision_variable_index_);
    const int data = 1;
    DummySolverDetails& dummy_solver_details = result.SetSolverDetailsType<DummySolverDetails>();
...
TEST_F(MathematicalProgramResultTest, EvalBinding) {
    ...
    const Eigen::Vector2d x_val(0, 1);
    result.set_x_val(x_val);
    const Binding<LinearCost> cost{std::make_shared<LinearCost>(Vector1d(2), 0), Vector1<symbolic::Variable>(x1_)};
...
GTEST_TEST(TestMathematicalProgramResult, GetInfeasibleConstraintNames) {
    if (SnoptSolver::is_available() && SnoptSolver::is_enabled()) {
        ...
        std::vector<std::string> infeasible = result.GetInfeasibleConstraintNames(prog);
        ...
        EXPECT_PRED2(matcher, infeasible[0], "drake::solvers::BoundingBoxConstraint.*");
...
GTEST_TEST(TestMathematicalProgramResult, GetInfeasibleConstraintBindings) {
    ...
    if (solver.is_available() && solver.is_enabled()) {
        ...
        const Bindings infeasible_bindings = result.GetInfeasibleConstraints(prog, tol);
        const Eigen::Vector3d x_sol = result.GetSolution(x);
        const bool violates_constraint1 = !constraint1.evaluator()->CheckSatisfied(x_sol.head<2>(), tol);
        const bool violates_constraint2 = !constraint2.evaluator()->CheckSatisfied(x_sol, tol);
