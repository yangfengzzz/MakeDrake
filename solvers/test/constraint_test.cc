...
Environment BuildEnvironment(const VectorX<Variable>& vars, const VectorXd& values) {
    Environment env;
...
GTEST_TEST(TestConstraint, BoundSizeCheck) {
    DRAKE_EXPECT_THROWS_MESSAGE(
            LinearConstraint(Eigen::Matrix3d::Identity(), Eigen::Vector2d(1., 2), Eigen::Vector3d(2., 3, 4.)),
...
GTEST_TEST(TestConstraint, LinearConstraintSparse) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.get_sparse_A().toDense(), A_sparse.toDense()));
    ...
    EXPECT_TRUE(CompareMatrices(dut.get_sparse_A().toDense(), A_sparse_new.toDense()));
...
GTEST_TEST(TestConstraint, LinearConstraintInfiniteEntries) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LinearConstraint(A_sparse_bad, lb, ub), ".*IsFinite().*");
    DRAKE_EXPECT_THROWS_MESSAGE(LinearConstraint(A_sparse_bad.toDense(), lb, ub), ".*allFinite().*");
}
...
GTEST_TEST(TestConstraint, LinearEqualityConstraintInfiniteEntries) {
    ...
    EXPECT_THROW(LinearEqualityConstraint(A_sparse_bad.toDense(), bound), std::exception);
    EXPECT_THROW(LinearEqualityConstraint(A_sparse.toDense(), bound_bad), std::exception);
    DRAKE_EXPECT_THROWS_MESSAGE(LinearEqualityConstraint(A_sparse.toDense().row(0), kInf), ".*allFinite().*");
    DRAKE_EXPECT_THROWS_MESSAGE(LinearEqualityConstraint(A_sparse_bad.toDense().row(2), 0), ".*allFinite().*");
}
...
GTEST_TEST(TestConstraint, testLinearConstraintUpdate) {
    ...
    EXPECT_PRED2(FormulaEqual, constraint.CheckSatisfied(x_sym), 1 == x_sym[0] && 2 == x_sym[1]);
...
GTEST_TEST(TestConstraint, testLinearConstraintUpdateErrors) {
    ...
    EXPECT_THROW(constraint.UpdateCoefficients(A_bad.sparseView(), b), std::exception);
...
GTEST_TEST(testConstraint, testRemoveTinyCoefficient) {
  ...
  A_expected << 0, 0, 0,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.RemoveTinyCoefficient(-1), ".*tol should be non-negative");
}
...
GTEST_TEST(testConstraint, testQuadraticConstraintHessian) {
  ...
  Q << 1, 0,
    ...
    EXPECT_EQ(constraint1.hessian_type(), QuadraticConstraint::HessianType::kPositiveSemidefinite);
    ...
    EXPECT_PRED2(ExprEqual, y_sym[0], 0.5 * x0 * x0 + 0.5 * x1 * x1 + x0 + 2 * x1);
    EXPECT_PRED2(FormulaEqual, constraint1.CheckSatisfied(x_sym), 0 <= y_sym[0] && y_sym[0] <= 1);
  ...
  Q << -1, 1,
    ...
    EXPECT_TRUE(CompareMatrices(constraint1.b(), b));
    EXPECT_EQ(constraint1.hessian_type(), QuadraticConstraint::HessianType::kNegativeSemidefinite);
    EXPECT_FALSE(constraint1.is_convex());

    // Constructs a constraint with a non-symmetric Hessian.
    QuadraticConstraint constraint2(Q, b, 0, kInf, QuadraticConstraint::HessianType::kNegativeSemidefinite);
    EXPECT_TRUE(CompareMatrices(constraint2.Q(), (Q + Q.transpose()) / 2));
    EXPECT_TRUE(CompareMatrices(constraint2.b(), b));
    EXPECT_EQ(constraint2.hessian_type(), QuadraticConstraint::HessianType::kNegativeSemidefinite);
  ...
  Q << 1, 2,
    ...
    EXPECT_EQ(constraint2.hessian_type(), QuadraticConstraint::HessianType::kIndefinite);
    EXPECT_FALSE(constraint2.is_convex());

    // Updates constraint with a specified Hessian type.
    constraint2.UpdateCoefficients(Eigen::Matrix2d::Identity(), b,
                                   QuadraticConstraint::HessianType::kPositiveSemidefinite);
    EXPECT_EQ(constraint2.hessian_type(), QuadraticConstraint::HessianType::kPositiveSemidefinite);
    EXPECT_FALSE(constraint2.is_convex());
...
GTEST_TEST(testConstraint, QudraticConstraintLDLtFailute) {
  ...
  Q << 0, 0.5,
    ...
    EXPECT_EQ(constraint.hessian_type(), QuadraticConstraint::HessianType::kIndefinite);
}
...
void TestLorentzConeEvalConvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
    ...
    LorentzConeConstraint cnstr2(A, b, LorentzConeConstraint::EvalType::kConvexSmooth);
    ...
    std::ostringstream os;
    cnstr1.Display(os, symbolic::MakeVectorContinuousVariable(cnstr1.num_vars(), "x"));
    ...
    EXPECT_TRUE(CompareMatrices(y_expected, math::ExtractValue(y_autodiff1), 1e-12));
    EXPECT_TRUE(CompareMatrices(y_expected, math::ExtractValue(y_autodiff2), 1e-12));
    // With eval_type = kConvexSmooth, we approximate the gradient with some
    // smooth function, which introduces larger error (2e-12).
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff1), math::ExtractGradient(y_autodiff2), 2e-12));
}
...
void TestLorentzConeEvalNonconvex(const Eigen::Ref<const Eigen::MatrixXd>& A,
                                  ...
                                  const VectorXd& x_test,
                                  bool is_in_cone) {
    LorentzConeConstraint cnstr(A, b, LorentzConeConstraint::EvalType::kNonconvex);
    EXPECT_EQ(cnstr.num_constraints(), 2);
    EXPECT_TRUE(CompareMatrices(cnstr.lower_bound(), Eigen::Vector2d::Zero()));
    EXPECT_TRUE(CompareMatrices(cnstr.upper_bound(), Eigen::Vector2d::Constant(kInf)));
    VectorXd y;
    ...
    std::ostringstream os;
    cnstr.Display(os, symbolic::MakeVectorContinuousVariable(cnstr.num_vars(), "x"));
    EXPECT_THAT(os.str(), HasSubstr("LorentzConeConstraint\n"));
    EXPECT_THAT(os.str(), HasSubstr("pow"));
    ...
    // Test Eval/CheckSatisfied using Expression.
    const VectorX<Variable> x_sym{symbolic::MakeVectorContinuousVariable(x_test.size(), "x")};
    VectorX<Expression> y_sym;
    cnstr.Eval(x_sym, &y_sym);
    const Environment env{BuildEnvironment(x_sym, x_test)};
    EXPECT_TRUE(CompareMatrices(Evaluate(y_sym, env), y_expected, 1E-10, MatrixCompareType::absolute));
...
void TestRotatedLorentzConeEval(const Eigen::Ref<const Eigen::MatrixXd> A,
    ...
    Vector3d y_expected(z(0), z(1), z(0) * z(1) - z.tail(z.size() - 2).squaredNorm());
    EXPECT_TRUE(CompareMatrices(y, y_expected, 1E-10, MatrixCompareType::absolute));

    bool is_in_cone_expected = (z(0) >= 0) && (z(1) >= 0) && (z(0) * z(1) >= z.tail(z.size() - 2).norm());
    EXPECT_EQ(is_in_cone, is_in_cone_expected);
    EXPECT_EQ(cnstr.CheckSatisfied(x_test), is_in_cone_expected);
...
GTEST_TEST(testConstraint, testLorentzConeConstraint) {
    ...
    TestLorentzConeEvalConvex(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero(), x1);
    TestLorentzConeEvalNonconvex(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero(), x1, true);
...
GTEST_TEST(testConstraint, testLorentzConeConstraintAtZeroZ) {
    ...
    LorentzConeConstraint cnstr(A, b, LorentzConeConstraint::EvalType::kConvexSmooth);
...
GTEST_TEST(testConstraint, LorentzConeConstraintUpdateCoefficients) {
    ...
    LorentzConeConstraint constraint(A, b, LorentzConeConstraint::EvalType::kConvexSmooth);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(constraint.UpdateCoefficients(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero()),
...
GTEST_TEST(testConstraint, testPositiveSemidefiniteConstraint) {
  ...
  X2 << 1, 2, 0,
    ...
    // clang-format on
    cnstr.Eval(X2, &y);
    EXPECT_TRUE((y.array() < cnstr.lower_bound().array()).any() || (y.array() > cnstr.upper_bound().array()).any());
    EXPECT_EQ(cnstr.matrix_rows(), 3);
    EXPECT_FALSE(cnstr.CheckSatisfied(X2));

    // Test Eval/CheckSatisfied using Expression.
    const VectorX<Variable> x_sym{symbolic::MakeVectorContinuousVariable(X1.size(), "x")};
    VectorX<Expression> y_sym;
...
GTEST_TEST(testConstraint, testExpressionConstraint) {
  ...
  y_gradient_expected << .4, 0., 0.,
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), y_gradient_expected));
    ...
    EXPECT_PRED2(FormulaEqual, constraint.CheckSatisfied(vars), 0 <= e[0] && e[0] <= 2 && 0 <= e[1] && e[1] <= 2);
}
...
GTEST_TEST(testConstraint, testSimpleLCPConstraintEval) {
    ...
    Eigen::Vector2d x1(1, 1);
    c.Eval(x1, &w);
    EXPECT_TRUE(CompareMatrices(w, Vector2d(0, 0), 1e-4, MatrixCompareType::absolute));
    EXPECT_TRUE(c.CheckSatisfied(x1));

    Eigen::Vector2d x2(1, 2);
    c.Eval(x2, &w);
    EXPECT_TRUE(CompareMatrices(w, Vector2d(0, 1), 1e-4, MatrixCompareType::absolute));
...
protected:
    ...
    void DoEval(const Eigen::Ref<const VectorX<Variable>>& x, VectorX<Expression>* y) const override {
        DoEvalGeneric(x, y);
    }
...
GTEST_TEST(testConstraint, testExponentialConeConstraint) {
    ...
    y_autodiff_expected(0) = z_autodiff(0) - z_autodiff(1) * exp(z_autodiff(2) / z_autodiff(1));
    y_autodiff_expected(1) = z_autodiff(1);
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff), y_expected, tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), math::ExtractGradient(y_autodiff_expected), tol));
}
...
GTEST_TEST(ToLatex, GenericConstraint) {
    ...
    EXPECT_EQ(c.ToLatex(vars), "\\text{GenericTrivialConstraint1}(x_{0}, x_{1}, x_{2}) \\tag{test}");
}
...
GTEST_TEST(ToLatex, PolynomialConstraint) {
    ...
    const std::vector<Polynomiald::VarType> var_mapping = {x.GetSimpleVariable(), y.GetSimpleVariable()};
    PolynomialConstraint c(Vector1<Polynomiald>(poly), var_mapping, Vector1d{-1}, Vector1d{1});
    c.set_description("test");
    Vector2<Variable> vars = symbolic::MakeVectorVariable<2>("x");
    ...
    EXPECT_EQ(c.ToLatex(vars), "\\text{PolynomialConstraint}(x_{0}, x_{1}) \\tag{test}");
}
...
GTEST_TEST(ToLatex, ExpressionConstraint) {
    ...
    ExpressionConstraint c_scalar(Vector1<Expression>{1.2 + x(0) + 2 * x(1)}, Vector1d{-1.2}, Vector1d{0.3});
    c_scalar.set_description("test");
    EXPECT_EQ(c_scalar.ToLatex(c_scalar.vars(), 1), "-1.2 \\le (1.2 + x_{0} + 2x_{1}) \\le 0.3 \\tag{test}");

    ExpressionConstraint c_vector(Vector2<Expression>{0.1 + x(0), 3 * x(0) * x(1)}, Vector2d{-1, -2}, Vector2d{1, 2});
