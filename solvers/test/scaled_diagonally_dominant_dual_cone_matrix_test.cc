...
int TestInDualConeByGenerators(const Eigen::MatrixXd& X) {
    ...
    const std::vector<Eigen::MatrixX2d> generators = MakeSDDDualConeGenerator(X.rows());
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix2d> solver;

    for (int i = 0; i < ssize(generators); ++i) {
        const Eigen::Matrix2d product = generators.at(i).transpose() * X * generators.at(i);
...
GTEST_TEST(ScaledDiagonallyDominantMatrixDualConeConstraint, SizeOfReturnTest) {
    ...
    auto dual_cone_constraints =
            prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(X.cast<symbolic::Expression>());
    EXPECT_EQ(dual_cone_constraints.size(), 10);

    auto X2 = prog.NewSymmetricContinuousVariables<7>();
    auto dual_cone_constraints2 =
            prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(X2.cast<symbolic::Expression>());
    EXPECT_EQ(dual_cone_constraints2.size(), 21);
}

GTEST_TEST(ScaledDiagonallyDominantMatrixDualConeConstraint, FeasibilityCheck3by3Variable) {
    ...
    auto dual_cone_constraints = prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(X);
    VectorXDecisionVariable x_flat(6);
    x_flat << X(0, 0), X(0, 1), X(0, 2), X(1, 1), X(1, 2), X(2, 2);
    auto X_constraint = prog.AddBoundingBoxConstraint(Eigen::VectorXd::Zero(6), Eigen::VectorXd::Zero(6), x_flat);
    auto set_X_value = [&X_constraint](const Eigen::Matrix3d& X_val) {
        Eigen::VectorXd x_upper_triangle(6);
        x_upper_triangle << X_val(0, 0), X_val(0, 1), X_val(0, 2), X_val(1, 1), X_val(1, 2), X_val(2, 2);
  ...
  test3 << 32., 9., 0.,
    ...
    auto dd_dual_cone_constraints = prog_dd.AddPositiveDiagonallyDominantDualConeMatrixConstraint(X_dd);
    Eigen::VectorXd test3_flat(6);
    test3_flat << test3(0, 0), test3(0, 1), test3(0, 2), test3(1, 1), test3(1, 2), test3(2, 2);

    VectorXDecisionVariable x_dd_flat(6);
    x_dd_flat << X_dd(0, 0), X_dd(0, 1), X_dd(0, 2), X_dd(1, 1), X_dd(1, 2), X_dd(2, 2);
    auto X_dd_constraint = prog_dd.AddBoundingBoxConstraint(test3_flat, test3_flat, x_dd_flat);
    auto result_dd = Solve(prog_dd);
    EXPECT_TRUE(result_dd.is_success());
}

GTEST_TEST(ScaledDiagonallyDominantMatrixDualConeConstraint, FeasibilityCheck3by3Expression) {
    // SDD = PD = PD* = SDD* when n = 2, so the smallest meaningful test is 3.
    // Test that SDD* matrices are feasible.
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<3>();
    ...
    Z = 2. * X + 3. * Y;
    auto GetZResult = [&Z](const MathematicalProgramResult& result) -> Eigen::MatrixXd {
    ...
    auto dual_cone_constraints = prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(Z);

    auto Z_constraint = prog.AddLinearEqualityConstraint(z_upper, Eigen::VectorXd::Zero(6));

    auto set_Z_value = [&Z_constraint](const Eigen::Matrix3d& Z_val) {
        Eigen::VectorXd z_upper_triangle(6);
        z_upper_triangle << Z_val(0, 0), Z_val(0, 1), Z_val(0, 2), Z_val(1, 1), Z_val(1, 2), Z_val(2, 2);
  ...
  test3 << 32., 9., 0.,
    ...
    auto dd_dual_cone_constraints =
            prog_dd.AddPositiveDiagonallyDominantDualConeMatrixConstraint(X_dd.cast<symbolic::Expression>());
    Eigen::VectorXd test3_flat(6);
    test3_flat << test3(0, 0), test3(0, 1), test3(0, 2), test3(1, 1), test3(1, 2), test3(2, 2);
    VectorXDecisionVariable x_dd_flat(6);
    x_dd_flat << X_dd(0, 0), X_dd(0, 1), X_dd(0, 2), X_dd(1, 1), X_dd(1, 2), X_dd(2, 2);
...
GTEST_TEST(ScaledDiagonallyDominantMatrixDualConeConstraint, MaximizeOffDiagonalTest) {
    ...
    auto TestIdx = [](int i, int j) {
        ...
        prog.AddBoundingBoxConstraint(Eigen::Vector4d(1, 2, 3, 4), Eigen::Vector4d(1, 2, 3, 4), X.diagonal());
        // Y is constrained to be 0 so we can control the value of Z.
        prog.AddLinearEqualityConstraint(Y == Eigen::Matrix4d::Zero());

        auto dual_cone_constraints = prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(X);
        auto dual_cone_constraints_Z = prog.AddScaledDiagonallyDominantDualConeMatrixConstraint(Z);
        ...
        Eigen::Matrix4d Z_res = result.GetSolution(Z).unaryExpr([](const symbolic::Expression& x) {
            return x.Evaluate();
        });
