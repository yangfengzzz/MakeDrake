    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < nx; ++j) {
            ...
            if (i >= j) {
                ...
                EXPECT_PRED2(ExprEqual, symbolic::Expression(M[i][j](0, 1)), symbolic::Expression(X(i, j)));
                EXPECT_PRED2(ExprEqual, symbolic::Expression(M[i][j](1, 0)), symbolic::Expression(X(j, i)));
            }
        }
    }
}
...
template <typename T>
void CheckSDDMatrix(const Eigen::Ref<const Eigen::MatrixXd>& X_val, bool is_sdd) {
    ...
    if (is_sdd) {
        ...
        for (int i = 0; i < prog.num_vars(); ++i) {
            env.insert(prog.decision_variable(i), result.GetSolution(prog.decision_variable(i)));
        }
        ...
        for (int i = 0; i < nx; ++i) {
            ...
            for (int j = i + 1; j < nx; ++j) {
                ...
                EXPECT_GE(M_val[i][j](i, i) * M_val[i][j](j, j), std::pow(M_val[i][j](i, j), 2) - tol);
            }
        }
        EXPECT_TRUE(CompareMatrices(M_sum, X_val, tol));
    } else {
        EXPECT_FALSE(result.is_success());
        EXPECT_TRUE(result.get_solution_result() == SolutionResult::kInfeasibleConstraints ||
...
GTEST_TEST(ScaledDiagonallyDominantMatrixTest, TestSDDMatrix) {
  ...
  dd_X << 1, -0.2, 0.3, -0.45,
    ...
    not_dd_X << 1, -0.2, 0.3, -0.55, -0.2, 2, 0.5, 1, 0.3, 0.5, 3, 2, -0.55, 1, 2, 4;
...
GTEST_TEST(SdsosTest, SdsosPolynomial) {
    ...
    std::tie(p, std::ignore) = prog.NewSosPolynomial(m, MathematicalProgram::NonnegativePolynomial::kSdsos);
...
GTEST_TEST(SdsosTest, NotSdsosPolynomial) {
    ...
    // negative.
    symbolic::Polynomial non_sdsos_poly(1 + x(0) + 4 * x(1) + x(0) * x(0) + x(1) * x(1), symbolic::Variables(x));
    const auto Q = prog.AddSosConstraint(non_sdsos_poly, m, MathematicalProgram::NonnegativePolynomial::kSdsos);
