...
TEST_F(CsdpDocExample, Solve) {
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(y_), Eigen::Vector2d::Zero(), tol));
        const CsdpSolverDetails& solver_details = result.get_solver_details<CsdpSolver>();
        ...
        EXPECT_TRUE(CompareMatrices(solver_details.y_val, Eigen::Vector2d(0.75, 1), tol));
        ...
        Z_expected(5, 5) = 0.75;
        Z_expected(6, 6) = 1.0;
        EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(solver_details.Z_val), Z_expected, tol));
...
std::vector<RemoveFreeVariableMethod> GetRemoveFreeVariableMethods() {
    return {RemoveFreeVariableMethod::kNullspace, RemoveFreeVariableMethod::kTwoSlackVariables,
...
TEST_F(LinearProgramBoundingBox1, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            solver_options.SetOption(solver.id(), "drake::RemoveFreeVariableMethod", static_cast<int>(method));
            MathematicalProgramResult result;
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_).head<7>(), x_expected, tol));
        }
    }
}
...
TEST_F(CsdpLinearProgram2, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), Eigen::Vector3d(5.0 / 13, -2.0 / 13, 9.0 / 13), tol));
        }
    }
}
...
TEST_F(CsdpLinearProgram3, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), Eigen::Vector3d(10, -2.0 / 3, -17.0 / 9), tol));
        }
    }
}
...
TEST_F(TrivialSDP2, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(X1_), Eigen::Matrix2d::Zero(), tol));
...
TEST_F(TrivialSOCP1, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), Eigen::Vector3d(10, 0, 0), tol));
        }
    }
}
...
TEST_F(TrivialSOCP2, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), Eigen::Vector2d(0, 1), tol));
        }
    }
}
...
TEST_F(TrivialSOCP3, Solve) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            ...
            EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), Eigen::Vector2d(-0.1, 2 - std::sqrt(7.1)), tol));
        }
    }
}
...
INSTANTIATE_TEST_SUITE_P(CsdpTest, TestEllipsoidsSeparation, ::testing::ValuesIn(GetEllipsoidsSeparationProblems()));
...
INSTANTIATE_TEST_SUITE_P(CsdpTest, TestFindSpringEquilibrium, ::testing::ValuesIn(GetFindSpringEquilibriumProblems()));
...
GTEST_TEST(TestSOCP, SmallestEllipsoidCoveringProblem) {
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        SolverOptions solver_options;
        solver_options.SetOption(solver.id(), "drake::RemoveFreeVariableMethod", static_cast<int>(method));
        SolveAndCheckSmallestEllipsoidCoveringProblems(solver, solver_options, 1E-6);
    }
}
...
GTEST_TEST(TestSOS, UnivariateNonnegative1) {
    ...
    for (auto method : GetRemoveFreeVariableMethods()) {
        ...
        if (solver.available()) {
            SolverOptions solver_options;
            solver_options.SetOption(solver.id(), "drake::RemoveFreeVariableMethod", static_cast<int>(method));
            const auto result = solver.Solve(dut.prog(), std::nullopt, solver_options);
