...
GTEST_TEST(testMathematicalProgram, simpleLCP) {
  ...
  M << 1, 4,
    ...
    EXPECT_TRUE(CompareMatrices(x_value, Vector2d(16, 0), 1e-4, MatrixCompareType::absolute));
}
...
GTEST_TEST(testMathematicalProgram, multiLCP) {
  ...
  M << 1, 4,
    ...
    EXPECT_TRUE(CompareMatrices(y_value, Vector2d(16, 0), 1e-4, MatrixCompareType::absolute));
}
}  // namespace test
}  // namespace solvers
}  // namespace drake
