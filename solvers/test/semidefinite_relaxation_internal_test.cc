#include "solvers/semidefinite_relaxation_internal.h"
...
GTEST_TEST(MakeSemidefiniteRelaxationInternalTest, ValidateProgramIsSupportedTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ValidateProgramIsSupported(prog),
...
GTEST_TEST(MakeSemidefiniteRelaxationInternalTest, CheckProgramHasNonConvexQuadratics) {
    MathematicalProgram prog;
    const auto x = prog.NewContinuousVariables(3, "x");
...
GTEST_TEST(MakeSemidefiniteRelaxationInternalTest, InitializeSemidefiniteRelaxationForProg) {
    MathematicalProgram prog;
    Variable one("1");
    ...
    InitializeSemidefiniteRelaxationForProg(prog, one, &relaxation, &X, &variables_to_sorted_indices, 1);
...
public:
    MakeSemidefiniteRelaxationTestFixture() : prog_(), relaxation_(prog_.Clone()), one_("one") {
        relaxation_->AddDecisionVariables(Vector1<Variable>(one_));
        InitializeSemidefiniteRelaxationForProg(prog_, one_, relaxation_.get(), &X_, &variables_to_sorted_indices_);
    }
...
TEST_F(MakeSemidefiniteRelaxationTestFixture, DoLinearizeQuadraticCostsAndConstraintsQuadraticCost) {
    ...
    for (const auto& preserve_convex : {false, true}) {
        ...
        DoLinearizeQuadraticCostsAndConstraints(prog_, X_, variables_to_sorted_indices_, relaxation_.get(),
                                                preserve_convex);
        EXPECT_EQ(relaxation_->linear_costs().size(), 2);
        SetRelaxationInitialGuess(yd, relaxation_.get());
        EXPECT_NEAR(relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_costs()[0])[0], 0, 1e-12);
    }
}

TEST_F(MakeSemidefiniteRelaxationTestFixture, DoLinearizeQuadraticCostsAndConstraintsNonConvexQuadraticConstraint) {
    const auto y = prog_.NewContinuousVariables<2>("y");
    Matrix2d Q;
    Q << 1, 2, 3, 4;
    ...
    for (const auto& preserve_convex : {false, true}) {
        ...
        EXPECT_NEAR(relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_constraints()[0])[0],
                    (0.5 * y_test.transpose() * Q * y_test + b.transpose() * y_test)[0], 1e-12);

        EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->lower_bound()[0], lb);
        EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->upper_bound()[0], ub);
    }
}

// This test checks that repeated variables in a quadratic constraint are
// handled correctly.
TEST_F(MakeSemidefiniteRelaxationTestFixture, DoLinearizeQuadraticCostsAndConstraintsCasePositiveDefiniteDontPreserve) {
    const auto y = prog_.NewContinuousVariables<1>("y");
    // This is a convex quadratic.
    Eigen::Matrix2d Q{{2, 1}, {1, 1}};
    prog_.AddQuadraticConstraint(Q, Eigen::Vector2d::Zero(), -kInf, 1, Vector2<Variable>(y(0), y(0)));
    EXPECT_TRUE(prog_.quadratic_constraints()[0].evaluator()->is_convex());

    ReinitializeRelaxation();
    DoLinearizeQuadraticCostsAndConstraints(prog_, X_, variables_to_sorted_indices_, relaxation_.get(), false);
    ...
    EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->lower_bound()[0], -kInf);
    EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->upper_bound()[0], 1.0);
}

TEST_F(MakeSemidefiniteRelaxationTestFixture, DoLinearizeQuadraticCostsAndConstraintsCasePositiveDefinitePreserve) {
    const auto y = prog_.NewContinuousVariables<1>("y");
    // This is a convex quadratic.
    const Eigen::Matrix2d Q{{2, 1}, {1, 1}};
    prog_.AddQuadraticConstraint(Q, Eigen::Vector2d::Ones(), -kInf, 1, Vector2<Variable>(y(0), y(0)));
    EXPECT_TRUE(prog_.quadratic_constraints()[0].evaluator()->is_convex());

    ReinitializeRelaxation();
    DoLinearizeQuadraticCostsAndConstraints(prog_, X_, variables_to_sorted_indices_, relaxation_.get(), true);
    ...
    const VectorXd rotated_lorentz_eval =
            relaxation_->EvalBindingAtInitialGuess(relaxation_->rotated_lorentz_cone_constraints()[0]);
    ...
    EXPECT_NEAR(-rotated_lorentz_eval(2) + prog_.quadratic_constraints()[0].evaluator()->upper_bound()[0],
                prog_.EvalBindingAtInitialGuess(prog_.quadratic_constraints()[0])[0], 1e-12);
}

TEST_F(MakeSemidefiniteRelaxationTestFixture, DoLinearizeQuadraticCostsAndConstraintsCaseNegativeDefinitePreserve) {
    const auto y = prog_.NewContinuousVariables<2>("y");
    ...
    EXPECT_NEAR(relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_constraints()[0])[0],
                (0.5 * y_test.transpose() * Q * y_test + b.transpose() * y_test)(0), 1e-12);
    EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->lower_bound()[0], -1);
    EXPECT_EQ(relaxation_->linear_constraints()[0].evaluator()->upper_bound()[0], kInf);
    ...
    // 0.5xᵀQx + bᵀx. So we expect the following two expressions to be the same.
    EXPECT_NEAR(rotated_lorentz_eval(2) + prog_.quadratic_constraints()[0].evaluator()->lower_bound()[0],
                prog_.EvalBindingAtInitialGuess(prog_.quadratic_constraints()[0])[0], 1e-12);
}

TEST_F(MakeSemidefiniteRelaxationTestFixture, DoAddImpliedLinearConstraintsBoundingBoxConstraint) {
    const int n_vars = 2;
    const auto y = prog_.NewContinuousVariables<2>("y");
    ...
    ReinitializeRelaxation();
    DoAddImpliedLinearConstraints(prog_, X_, variables_to_sorted_indices_, relaxation_.get());
    ...
    MatrixXd expected_mat = (A * y_test - b) * (A * y_test - b).transpose() - b * b.transpose();
...
TEST_F(MakeSemidefiniteRelaxationTestFixture, DoAddImpliedLinearConstraintsLinearConstraint) {
    const auto y = prog_.NewContinuousVariables<2>("y");
    MatrixXd A0(3, 2);
    ...
    EXPECT_TRUE(CompareMatrices(A0, relaxation_->linear_constraints()[0].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lb0, relaxation_->linear_constraints()[0].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(ub0, relaxation_->linear_constraints()[0].evaluator()->upper_bound()));

    // Second linear constraint is lb1 ≤ A1 y ≤ ub1.
    EXPECT_TRUE(CompareMatrices(A1, relaxation_->linear_constraints()[1].evaluator()->GetDenseA()));
    EXPECT_TRUE(CompareMatrices(lb1, relaxation_->linear_constraints()[1].evaluator()->lower_bound()));
    EXPECT_TRUE(CompareMatrices(ub1, relaxation_->linear_constraints()[1].evaluator()->upper_bound()));
    ...
    MatrixXd A(8, 2);
    A << -A0.row(0), A0.row(0), A0.row(1), -A0.row(2), -A1_reordered.row(0), A1_reordered.row(0), -A1_reordered.row(1),
            A1_reordered.row(1);
    int expected_size = (b.size() * (b.size() + 1)) / 2;
    EXPECT_EQ(relaxation_->linear_constraints()[2].evaluator()->num_constraints(), expected_size);
    VectorXd value = relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_constraints()[2]);
    MatrixXd expected_mat = (A * y_test - b) * (A * y_test - b).transpose() - b * b.transpose();
    VectorXd expected = math::ToLowerTriangularColumnsFromMatrix(expected_mat);
    EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
...
TEST_F(MakeSemidefiniteRelaxationTestFixture, DoAddImpliedLinearEqualityConstraints) {
    const auto y = prog_.NewContinuousVariables<2>("y");
    MatrixXd A(3, 2);
    ...
    ReinitializeRelaxation();
    DoAddImpliedLinearEqualityConstraints(prog_, X_, variables_to_sorted_indices_, relaxation_.get());
    ...
    VectorXd value = relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_equality_constraints()[0]) -
    ...
    value = relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_equality_constraints()[1]);
    EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));

    for (int i = 2; i < 4; ++i) {
        // Linear constraints are (Ay - b)*y_i = 0.
        expected = (A * y_test - b) * y_test[i - 2];
        value = relaxation_->EvalBindingAtInitialGuess(relaxation_->linear_equality_constraints()[i]);
        EXPECT_TRUE(CompareMatrices(value, expected, 1e-12));
    }
}
...
GTEST_TEST(MakeSemidefiniteRelaxationInternalTest, TestSparseKron) {
  ...
  B << 7.16, 1.8,
    ...
    auto TestKron = [](const Eigen::SparseMatrix<double>& A_test, const Eigen::SparseMatrix<double>& B_test) {
        ...
        Eigen::VectorXd kron_vec = kron * Eigen::Map<const Eigen::VectorXd>(testMatrix.data(), testMatrix.size());
        EXPECT_TRUE(CompareMatrices(Eigen::Map<const Eigen::VectorXd>(AXB.data(), AXB.size()), kron_vec, 1e-10,
...
GTEST_TEST(MakeSemidefiniteRelaxationInternalTest, TestToSymmetricMatrixFromTensorVector) {
    // Check the 2x2 ⊗  3x3 case.
    int num_elts = 3 * 6;  // There are 3 elements in 2x2 basis and 6 in the 3x3 basis.
    VectorX<symbolic::Variable> y(num_elts);
    for (int i = 0; i < num_elts; ++i) {
        y(i) = symbolic::Variable("y_" + std::to_string(i));
    }
    Eigen::MatrixX<symbolic::Variable> Y = ToSymmetricMatrixFromTensorVector(y, 2, 3);
  ...
  Y_expected << y(0), y(1),  y(2),  y(6),  y(7),  y(8),
    ...
    Eigen::MatrixX<symbolic::Variable> X = ToSymmetricMatrixFromTensorVector(x, 4, 3);
...
Eigen::VectorXd MakeRandomPositiveOrthantNonLorentzVector(const int r, const double scale) {
    Eigen::VectorXd ret = scale * (Eigen::VectorXd::Random(r) + Eigen::VectorXd::Ones(r));
    while (r > 1 && ret(0) > ret.tail(r - 1).norm()) {
        // This ensures we are not in the Lorentz cone by scaling the first entry to
        // be too small.
        ret(0) = 1 / (0.5 * ret.tail(r - 1).norm()) * (Eigen::VectorXd::Random(1)(0) + 1);
    }
    return ret;
}
...
Eigen::MatrixXd MakeRandomPositiveOrthantPositiveMap(const int r, const int m, const double scale) {
...
// vectors.
Eigen::VectorXd MakeRandomLorentzNonPositiveOrthantVector(const int r, const double scale) {
...
Eigen::MatrixXd MakeRandomLorentzPositiveMap(const int r, const int m, const double scale) {
    ...
    if (r > 1 && m > 1) {
        ...
        const Eigen::VectorXd regularized_singular_values = svd_scaling * svd.singularValues();

        A.bottomRightCorner(r - 1, m - 1) =
                svd.matrixU() * regularized_singular_values.asDiagonal() * svd.matrixV().transpose();
    } else if (m == 1) {
        // Make the last r entries of the column matrix have norm A(0,0) / 2
        A.col(0).tail(r - 1) = Eigen::VectorXd::Random(r - 1);
        A.col(0).tail(r - 1) = A.col(0).tail(r - 1) / A.col(0).tail(r - 1).norm() * (A(0, 0) / 2);
    } else if (r == 1) {
...
bool ProgramSolvedWithoutError(const SolutionResult& result) {
    return result == kSolutionFound || result == kInfeasibleConstraints || result == kUnbounded ||
...
class Cone1ByCone2SeparabilityTest : public ::testing::TestWithParam<std::tuple<int, int>> {
public:
    Cone1ByCone2SeparabilityTest(const SupportedTestCone cone1, const SupportedTestCone cone2)
    ...
    void DoTest(const std::optional<Eigen::MatrixXd>& A_opt = std::nullopt,
        ...
        Eigen::MatrixXd A = A_opt.value_or(Eigen::MatrixXd::Identity(X_.rows(), X_.rows()));
        Eigen::MatrixXd B = B_opt.value_or(Eigen::MatrixXd::Identity(X_.cols(), X_.cols()));
        ...
        test_matrix = (2 * xr_cone1_ * yc_cone2_.transpose() + 3 * yr_cone1_ * xc_cone2_.transpose());
        EXPECT_EQ(DoTestCase(Y, test_matrix), true);
        ...
        EXPECT_EQ(DoTestCase(Y, test_matrix), cone1_ == cone2_ && B.cols() <= B.rows());
        ...
        EXPECT_EQ(DoTestCase(Y, test_matrix), cone1_ == cone2_ && A.rows() <= A.cols());

        // A tensor that is non-conic combination of cone1-cone2 separable tensors
        // separable tensors.
        test_matrix = 2 * yr_cone1_ * xc_cone2_.transpose() - 1000 * xr_cone1_ * yc_cone2_.transpose();
        EXPECT_EQ(DoTestCase(Y, test_matrix), false);
...
protected:
    ...
    // to prog_.
    virtual void AddCone1ByCone2SeparableConstraint(const MatrixX<Variable>& Z) = 0;

    // Adds the constraint that Z is Cone 1 by Cone 2 separable to prog_ and
    // ensures that the right number of constraints and variables have been added
    // to prog_.
    virtual void AddCone1ByCone2SeparableConstraint(const MatrixX<Expression>& Z) = 0;
    ...
    virtual void CheckCone1ByCone2SeparableConstraintsAdded(const Eigen::MatrixXd& A, const Eigen::MatrixXd& B) = 0;
...
private:
    ...
    void MakeTestVectors(const Eigen::Ref<const Eigen::MatrixXd>& A, const Eigen::Ref<const Eigen::MatrixXd>& B) {
        const double scale = 2.0;
        const auto initialize_vectors_in_cone_and_image = [&scale, &A, &B](const Eigen::MatrixXd& C,
            ...
            switch (cone) {
                case SupportedTestCone::kPositiveOrthant:
                    *x = C * MakeRandomPositiveOrthantNonLorentzVector(C.cols(), scale);
                    *y = C * MakeRandomPositiveOrthantNonLorentzVector(C.cols(), scale);
                    if (!CheckIsPositiveOrthantVector(*x) || !CheckIsPositiveOrthantVector(*y)) {
                        throw std::logic_error(
                                fmt::format("C=\n{}\n is not a positive-orthant positive map.", fmt_eigen(C)));
                    }
                ...
                case SupportedTestCone::kLorentzCone:
                    *x = C * MakeRandomLorentzNonPositiveOrthantVector(C.cols(), scale);
                    *y = C * MakeRandomLorentzNonPositiveOrthantVector(C.cols(), scale);
                    if (!CheckIsLorentzVector(*x) || !CheckIsLorentzVector(*y)) {
                        throw std::logic_error(fmt::format("C=\n{}\n is not a Lorentz positive map.", fmt_eigen(C)));
                    }
        ...
        initialize_vectors_in_cone_and_image(A, cone1_, &xr_cone1_, &yr_cone1_, &wr_cone1_, &wc_cone1_);

        initialize_vectors_in_cone_and_image(B.transpose(), cone2_, &xc_cone2_, &yc_cone2_, &wr_cone2_, &wc_cone2_);

        const auto is_conic_vector = [](const Eigen::VectorXd& z, const SupportedTestCone cone) {
            switch (cone) {
                case SupportedTestCone::kPositiveOrthant:
        ...
        // to take very long.
        while (is_conic_vector(zr_not_conic_, cone1_) || is_conic_vector(zr_not_conic_, cone2_)) {
            zr_not_conic_ = Eigen::VectorXd::Random(A.rows());
        }
        ...
        // 1/2 the volume of the ambient space and therefore this loop is unlikely
        // to take very long.
        while (is_conic_vector(zc_not_conic_, cone1_) || is_conic_vector(zc_not_conic_, cone2_)) {
            zc_not_conic_ = Eigen::VectorXd::Random(B.cols());
        }
    }

    bool DoTestCase(const Eigen::Ref<const MatrixX<Expression>>& Y, const Eigen::Ref<const Eigen::MatrixXd>& test_mat) {
...
class PositiveOrthantByLorentzSeparabilityTest : public Cone1ByCone2SeparabilityTest {
public:
    PositiveOrthantByLorentzSeparabilityTest()
        : Cone1ByCone2SeparabilityTest(SupportedTestCone::kPositiveOrthant, SupportedTestCone::kLorentzCone) {}
...
private:
    ...
    void CheckCone1ByCone2SeparableConstraintsAdded(const Eigen::MatrixXd& A, const Eigen::MatrixXd& B) {
...
TEST_P(PositiveOrthantByLorentzSeparabilityTest, AddMatrixIsPositiveOrthantByLorentzSeparableConstraintVariable) {
    this->DoTest();
}
...
TEST_P(PositiveOrthantByLorentzSeparabilityTest, AddMatrixIsPositiveOrthantByLorentzSeparableConstraintChangeSize) {
...
class LorentzByPositiveOrthantSeparabilityTest : public Cone1ByCone2SeparabilityTest {
public:
    LorentzByPositiveOrthantSeparabilityTest()
        : Cone1ByCone2SeparabilityTest(SupportedTestCone::kLorentzCone, SupportedTestCone::kPositiveOrthant) {}
...
TEST_P(LorentzByPositiveOrthantSeparabilityTest, AddMatrixIsLorentzByPositiveOrthantSeparableConstraintVariable) {
    this->DoTest();
}
...
TEST_P(LorentzByPositiveOrthantSeparabilityTest, AddMatrixIsLorentzByPositiveOrthantSeparableConstraintChangeSize) {
    // The subspaces in some of these tests have small dimension, making them
    // numerically difficult for SDP solvers which do no preprocessing. Therefore,
    // we only solve these with Mosek as it is the only one which currently gives
    // reliable results.
...
public:
    LorentzByLorentzSeparabilityTest()
        : Cone1ByCone2SeparabilityTest(SupportedTestCone::kLorentzCone, SupportedTestCone::kLorentzCone) {}
...
private:
    ...
    void CheckCone1ByCone2SeparableConstraintsAdded(const Eigen::MatrixXd& A, const Eigen::MatrixXd& B) {
        ...
        if (r >= 3 && c >= 3) {
            ...
            const Binding<PositiveSemidefiniteConstraint> psd_constraint = prog_.positive_semidefinite_constraints()[0];
            ...
            EXPECT_EQ(ssize(prog_.bounding_box_constraints()) + ssize(prog_.linear_constraints()), 1);
...
TEST_P(LorentzByLorentzSeparabilityTest, AddMatrixIsLorentzByLorentzSeparableConstraintVariable) {
    int m, n;
    std::tie(m, n) = GetParam();
    this->DoTest();
}

TEST_P(LorentzByLorentzSeparabilityTest, AddMatrixIsLorentzByLorentzSeparableConstraintExpressionKeepSize) {
    int m, n;
    std::tie(m, n) = GetParam();
    double scale = 3;
...
TEST_P(LorentzByLorentzSeparabilityTest, AddMatrixIsLorentzByLorentzSeparableConstraintExpressionChangeSize) {
    // The subspaces in some of these tests have small dimension, making them
    // numerically difficult for SDP solvers which do no preprocessing. Therefore,
    // we only solve these with Mosek as it is the only one which currently gives
    // reliable results.
