#include "solvers/test/quadratic_program_examples.h"
...
#include "solvers/mathematical_program_result.h"
...
std::vector<QuadraticProblems> quadratic_problems() {
    return std::vector<QuadraticProblems>{QuadraticProblems::kQuadraticProgram0, QuadraticProblems::kQuadraticProgram1,
                                          QuadraticProblems::kQuadraticProgram2, QuadraticProblems::kQuadraticProgram3,
                                          QuadraticProblems::kQuadraticProgram4};
}

QuadraticProgram0::QuadraticProgram0(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_(), x_expected_(0.25, 0.75) {
    ...
    switch (cost_form) {
      ...
      Q << 4, 2,
        ...
        case CostForm::kSymbolic: {
            prog()->AddQuadraticCost(2 * x_(0) * x_(0) + x_(0) * x_(1) + x_(1) * x_(1) + x_(0) + x_(1) + 3);
            break;
        }
        default: {
...
void QuadraticProgram0::CheckSolution(const MathematicalProgramResult& result) const {
    ...
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x_), x_expected_, tol, MatrixCompareType::absolute));
    ExpectSolutionCostAccurate(*prog(), result, tol);
}

QuadraticProgram1::QuadraticProgram1(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_{}, x_expected_(0, 1, 2.0 / 3) {
    ...
    switch (cost_form) {
        ...
        case CostForm::kSymbolic: {
            prog()->AddQuadraticCost(x_(0) * x_(0) + x_(0) * x_(1) + x_(1) * x_(1) + x_(1) * x_(2) + x_(2) * x_(2) +
                                     2 * x_(0));
            break;
        }
        default:
    ...
    Vector4d b_lb(4, -numeric_limits<double>::infinity(), -20, -numeric_limits<double>::infinity());
    Vector4d b_ub(numeric_limits<double>::infinity(), -1, 100, numeric_limits<double>::infinity());
    switch (constraint_form) {
        case ConstraintForm::kNonSymbolic: {
            prog()->AddBoundingBoxConstraint(0, numeric_limits<double>::infinity(), x_);
...
void QuadraticProgram1::CheckSolution(const MathematicalProgramResult& result) const {
    double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
    if (result.get_solver_id() == GurobiSolver::id()) {
        tol = 1E-8;
    } else if (result.get_solver_id() == MosekSolver::id()) {
...
QuadraticProgram2::QuadraticProgram2(CostForm cost_form, ConstraintForm constraint_form)
...
void QuadraticProgram2::CheckSolution(const MathematicalProgramResult& result) const {
    double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
...
QuadraticProgram3::QuadraticProgram3(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_{}, x_expected_{} {
    ...
    switch (cost_form) {
        ...
        case CostForm::kSymbolic: {
            prog()->AddQuadraticCost(0.5 * x_.head<4>().dot(Q1 * x_.head<4>()) + b1.dot(x_.head<4>()) +
...
void QuadraticProgram3::CheckSolution(const MathematicalProgramResult& result) const {
    double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
    if (result.get_solver_id() == MosekSolver::id()) {
        tol = 1E-8;
    }
...
QuadraticProgram4::QuadraticProgram4(CostForm cost_form, ConstraintForm constraint_form)
    : OptimizationProgram(cost_form, constraint_form), x_{}, x_expected_(0.8, 0.2, 0.6) {
    x_ = prog()->NewContinuousVariables<3>();
    switch (cost_form) {
        case CostForm::kNonSymbolic: {
...
void QuadraticProgram4::CheckSolution(const MathematicalProgramResult& result) const {
    double tol = GetSolverSolutionDefaultCompareTolerance(result.get_solver_id());
    if (result.get_solver_id() == MosekSolver::id()) {
        tol = 1E-8;
    }
...
void TestQPonUnitBallExample(const SolverInterface& solver) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (theta <= M_PI_2) {
            ...
            x_expected << (x_desired(0) - x_desired(1) + 1.0) / 2.0, (x_desired(1) - x_desired(0) + 1.0) / 2.0;
        } else if (theta <= M_PI) {
            // min (x-x_d)^2 + (y-y_d)^2 s.t. x-y=1
            x_expected << (x_desired(0) + x_desired(1) + 1.0) / 2.0, (x_desired(0) + x_desired(1) - 1.0) / 2.0;
        ...
        const MathematicalProgramResult result = RunSolver(prog, solver, initial_guess);
        ...
        EXPECT_TRUE(CompareMatrices(x_value, x_expected, tol, MatrixCompareType::absolute));
    }
    ...
    //
    {
        ...
        const auto& x_value = result.GetSolution(x);
        EXPECT_TRUE(CompareMatrices(x_value, x_expected, 1e-5, MatrixCompareType::absolute));
        ExpectSolutionCostAccurate(prog, result, 1E-5);
    }
}

void TestQPDualSolution1(const SolverInterface& solver, const SolverOptions& solver_options, double tol) {
    MathematicalProgram prog;
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector3d(0, 1. / 11, 8. / 11.), tol));
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(0.), tol));
        const double dual_solution_expected = 0.363636;
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint2), Vector1d(dual_solution_expected), tol));
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint3), Eigen::Vector3d::Zero(), tol));
        ...
        constraint2.evaluator()->set_bounds(Vector1d(3 + delta), Vector1d(3 + delta));
        MathematicalProgramResult result_updated;
        solver.Solve(prog, {}, solver_options, &result_updated);
        EXPECT_NEAR((result_updated.get_optimal_cost() - result.get_optimal_cost()) / delta, dual_solution_expected,
                    tol);
    }
}
...
void TestQPDualSolution2(const SolverInterface& solver) {
    ...
    prog.AddQuadraticCost(x[0] * x[0] + 2 * x[1] * x[1] + x[2] * x[2] + 2 * x[1] * x[2] + 4 * x[2]);
    if (solver.available()) {
        MathematicalProgramResult result;
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint1), Vector1d(dual_solution_expected), 1e-6));
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint2), Vector1d(-1.14285714), 1e-6));
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint3), Eigen::Vector3d::Zero(), 1e-6));
...
void TestQPDualSolution3(const SolverInterface& solver, double tol, double sensitivity_tol) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    auto constraint = prog.AddBoundingBoxConstraint(-1, 2, x);
    prog.AddQuadraticCost(x[0] * x[0] + 2 * x[1] * x[1] + 2 * x[0] * x[1] - 8 * x[0] + 6 * x[1]);
    if (solver.available()) {
        MathematicalProgramResult result;
        solver.Solve(prog, {}, {}, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(2, -1), tol));
        // At the optimal solution, the active constraints are
        // x[0] <= 2
        // x[1] >= -1
        // Solving the KKT condition, we get the dual solution as
        ...
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Eigen::Vector2d(-6, 6), tol));
        ...
        EXPECT_NEAR((result1.get_optimal_cost() - result.get_optimal_cost()) / delta, -6, sensitivity_tol);
        ...
        EXPECT_NEAR(result2.get_optimal_cost(), result.get_optimal_cost(), sensitivity_tol);

        constraint.evaluator()->set_bounds(Eigen::Vector2d(-1 + delta, -1), Eigen::Vector2d(2, 2));
        MathematicalProgramResult result3;
        solver.Solve(prog, {}, {}, &result3);
        // The dual solution for x[0] >= -1 is 0.
        EXPECT_NEAR(result3.get_optimal_cost(), result.get_optimal_cost(), sensitivity_tol);

        constraint.evaluator()->UpdateLowerBound(Eigen::Vector2d(-1, -1 + delta));
        MathematicalProgramResult result4;
        solver.Solve(prog, {}, {}, &result4);
        EXPECT_NEAR((result4.get_optimal_cost() - result.get_optimal_cost()) / delta, 6, sensitivity_tol);
        ...
        // should be zero.
        constraint.evaluator()->set_bounds(Eigen::Vector2d(-1, -1), Eigen::Vector2d(2, 2));
        ...
        EXPECT_TRUE(CompareMatrices(result5.GetDualSolution(constraint), Eigen::Vector2d(-6, 6), tol));
        EXPECT_TRUE(CompareMatrices(result5.GetDualSolution(constraint1), Vector1d(0)));
        EXPECT_TRUE(CompareMatrices(result5.GetDualSolution(constraint2), Vector1d(0)));
    }
}
...
void TestEqualityConstrainedQPDualSolution1(const SolverInterface& solver) {
    ...
    if (solver.available()) {
        ...
        solver.Solve(prog, {}, {}, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(constraint), Vector1d(1), 1e-5));
    }
}
...
void TestEqualityConstrainedQPDualSolution2(const SolverInterface& solver) {
    ...
    A << 1, 0, 3, 0, 2, 1;
    auto constraint = prog.AddLinearEqualityConstraint(A, Eigen::Vector2d(1, 2), x);
    prog.AddQuadraticCost(x[0] * x[0] + 2 * x[1] * x[1] + x[2] * x[2] + 4 * x[1]);
    if (solver.available()) {
        MathematicalProgramResult result;
        solver.Solve(prog, {}, {}, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector3d(-0.42857143, 0.76190476, 0.47619048), 1e-5));
        EXPECT_TRUE(
                CompareMatrices(result.GetDualSolution(constraint), Eigen::Vector2d(-0.85714286, 3.52380952), 1e-5));
    }
}

void TestNonconvexQP(const SolverInterface& solver, bool convex_solver, double tol) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    ...
    if (convex_solver) {
        ...
        EXPECT_THAT(solver.ExplainUnsatisfiedProgramAttributes(prog), HasSubstr("is non-convex"));
        EXPECT_THAT(solver.ExplainUnsatisfiedProgramAttributes(prog), HasSubstr(description));
    } else {
        MathematicalProgramResult result;
        // Use a non-zero initial guess, since at x = [0, 0], the gradient is 0.
        solver.Solve(prog, Eigen::Vector2d(0.1, 0.1), std::nullopt, &result);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(1, 0), tol));
        EXPECT_NEAR(result.get_optimal_cost(), 1., tol);
    }
}

void TestDuplicatedVariableQuadraticProgram(const SolverInterface& solver, double tol) {
    ...
    prog.AddLinearEqualityConstraint(Eigen::RowVector3d(2, 2, -1), Vector1d(1), x_0_2_0);
  ...
  A << -1, 1, 3, 2, -1,
    ...
    prog.AddLinearConstraint(A, Eigen::Vector2d(-kInf, 1), Eigen::Vector2d(3, 3), x_2_0_2_1_2);
    const Vector4<symbolic::Variable> x_1_2_0_0(x(1), x(2), x(0), x(0));

    prog.AddLinearConstraint(Eigen::RowVector4d(2, 3, 2, -1), Vector1d(0), Vector1d(kInf), x_1_2_0_0);
    ...
    if (solver.available()) {
        ...
        EXPECT_NEAR(result.get_optimal_cost(),
                    x_sol(0) * x_sol(0) + 5 * x_sol(1) * x_sol(1) + 2 * x_sol(2) * x_sol(2) + x_sol(0) * x_sol(1), tol);
...
void TestEqualityConstrainedQP1(const SolverInterface& solver, double tol) {
    ...
    prog.AddLinearEqualityConstraint(
            x(0, 0) + 3 * x(0, 1) + 9 * x(0, 2) + 27 * x(0, 3) + 81 * x(0, 4) + 243 * x(0, 5) - x(1, 0), 0);
    prog.AddLinearEqualityConstraint(x(0, 1) + 6 * x(0, 2) + 27 * x(0, 3) + 108 * x(0, 4) + 405 * x(0, 5) - x(1, 1), 0);
...
void TestQuadraticCostVariableOrder(const SolverInterface& solver, double tol) {
  ...
  Q << 3, 2, 0.5, 0.3,
    ...
    const auto cost1 = prog.AddQuadraticCost(Q, b, Vector4<symbolic::Variable>(x(2), x(0), x(1), x(0)));
