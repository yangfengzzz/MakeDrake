...
GTEST_TEST(ProgramAttributeTest, ToString) {
    const ProgramAttributes attrs{ProgramAttribute::kGenericCost, ProgramAttribute::kGenericConstraint};

    EXPECT_EQ(to_string(ProgramAttribute::kGenericCost), "GenericCost");
    EXPECT_EQ(to_string(attrs), "{ProgramAttributes: GenericCost, GenericConstraint}");
...
GTEST_TEST(ProgramAttributeTest, Supported) {
    ...
    const ProgramAttributes quad_cost_cons{ProgramAttribute::kQuadraticCost, ProgramAttribute::kQuadraticConstraint};
    const ProgramAttributes lin_cons_quad_cost{ProgramAttribute::kLinearConstraint, ProgramAttribute::kQuadraticCost};
    const ProgramAttributes quad_cost_cons_generic_cost{
            ProgramAttribute::kQuadraticCost, ProgramAttribute::kQuadraticConstraint, ProgramAttribute::kGenericCost};

    // List out all of our test cases, formatted as tuples of:
    // - required attributes,
    // - supported attributes,
    // - expected unsupported message (if any).
    std::vector<std::tuple<ProgramAttributes, ProgramAttributes, std::string>> cases{
            {quad_cost, quad_cost, ""},
            ...
            {lin_cons, quad_cost, "a LinearConstraint was declared but is not supported"},
            {lin_cons_quad_cost, quad_cost_cons, "a LinearConstraint was declared but is not supported"},
    ...
    for (const auto& one_case : cases) {
        ...
        const bool is_supported_1 = AreRequiredAttributesSupported(required, supported);
        EXPECT_EQ(is_supported_1, expected_is_supported);

        std::string message{"this should be clobbered"};
        const bool is_supported_2 = AreRequiredAttributesSupported(required, supported, &message);
...
GTEST_TEST(ProgramTypeTest, tostring) {
    ...
    EXPECT_EQ(to_string(ProgramType::kMIQP), "mixed-integer quadratic programming");
    EXPECT_EQ(to_string(ProgramType::kMISOCP), "mixed-integer second order cone programming");
    EXPECT_EQ(to_string(ProgramType::kMISDP), "mixed-integer semidefinite programming");
    EXPECT_EQ(to_string(ProgramType::kQuadraticCostConicConstraint), "conic-constrained quadratic programming");
    ...
    EXPECT_EQ(to_string(ProgramType::kUnknown), "uncategorized mathematical programming type");
}
