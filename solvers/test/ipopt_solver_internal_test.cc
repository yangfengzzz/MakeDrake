...
void GetConstraintJacobian(
        IpoptSolver_NLP* dut, Index n, Index m, Index nnz_jac_g, const Number* x, Eigen::SparseMatrix<double>* jac) {
    ...
    bool status = dut->eval_jac_g(n, x, true /* new_x */, m, nnz_jac_g, iRow.data(), jCol.data(), nullptr);
    ASSERT_TRUE(status);
    status = dut->eval_jac_g(n, x, false /* new_x */, m, nnz_jac_g, nullptr, nullptr, values.data());
...
GTEST_TEST(TestIpoptSolverNlp, LinearConstraintsDuplicatedVariable) {
  ...
  A << 1, 0, 3,
    ...
    prog.AddLinearConstraint(A, lb, ub, Vector3<symbolic::Variable>(x(0), x(1), x(0)));
    Eigen::Vector4d x_init(0, 1, 2, 3);
    MathematicalProgramResult result;
    IpoptSolver_NLP dut(prog, x_init, &result);
    Index n;
...
public:
    ...
    explicit ToyConstraint(bool set_sparsity_pattern) : Constraint(2, 4, Eigen::Vector2d(1, 2), Eigen::Vector2d(3, 4)) {
...
private:
    ...
    void DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const { DoEvalGeneric<double>(x, y); }

    void DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const { DoEvalGeneric<AutoDiffXd>(x, y); }
...
GTEST_TEST(TestIpoptSolverNlp, NonlinearConstraint) {
    for (bool set_sparsity_pattern : {true, false}) {
        MathematicalProgram prog;
        auto x = prog.NewContinuousVariables<4>();
        prog.AddConstraint(std::make_shared<ToyConstraint>(set_sparsity_pattern), x);
