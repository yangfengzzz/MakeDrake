...
public:
    ...
    explicit MixedIntegerBranchAndBoundTester(
            ...
            MixedIntegerBranchAndBound::Options options = MixedIntegerBranchAndBound::Options{})
        : bnb_{new MixedIntegerBranchAndBound(prog, solver_id, options)} {}

    MixedIntegerBranchAndBound* bnb() const { return bnb_.get(); }

    MixedIntegerBranchAndBoundNode* PickBranchingNode() const { return bnb_->PickBranchingNode(); }
    ...
    void UpdateIntegralSolution(const Eigen::Ref<const Eigen::VectorXd>& solution, double cost) {
        return bnb_->UpdateIntegralSolution(solution, cost);
    }

    void BranchAndUpdate(MixedIntegerBranchAndBoundNode* node, const symbolic::Variable& branching_variable) {
        return bnb_->BranchAndUpdate(node, branching_variable);
    }

    MixedIntegerBranchAndBoundNode* mutable_root() { return bnb_->root_.get(); }

    void SearchIntegralSolutionByRounding(const MixedIntegerBranchAndBoundNode& node) {
...
std::unique_ptr<MathematicalProgram> ConstructMathematicalProgram4() {
    ...
    AddSos2Constraint(prog.get(), x.cast<symbolic::Expression>(), y.cast<symbolic::Expression>());
...
void CheckNewRootNode(const MixedIntegerBranchAndBoundNode& root,
    ...
    EXPECT_EQ(root.remaining_binary_variables().size(), binary_vars_expected.size());
    EXPECT_TRUE(std::equal(root.remaining_binary_variables().begin(), root.remaining_binary_variables().end(),
                           binary_vars_expected.begin(),
...
void CheckNodeSolution(const MixedIntegerBranchAndBoundNode& node,
    ...
    EXPECT_TRUE(CompareMatrices(node.prog_result()->GetSolution(x), x_expected, tol, MatrixCompareType::absolute));
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestConstructRoot1) {
    ...
    std::tie(root, map_old_vars_to_new_vars) =
            MixedIntegerBranchAndBoundNode::ConstructRootNode(*prog, GurobiSolver::id());
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestConstructRootError) {
    ...
    EXPECT_THROW(MixedIntegerBranchAndBoundNode::ConstructRootNode(prog, GurobiSolver::id()), std::runtime_error);
}
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestBranch1) {
    ...
    std::unique_ptr<MixedIntegerBranchAndBoundNode> root;
    std::tie(root, std::ignore) = MixedIntegerBranchAndBoundNode::ConstructRootNode(*prog, GurobiSolver::id());
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestBranch2) {
    ...
    EXPECT_EQ(root->left_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_EQ(root->right_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_EQ(root->left_child()->solution_result(), SolutionResult::kInfeasibleConstraints);
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestBranch3) {
    ...
    for (const auto& x_binary : {x(0), x(2)}) {
        ...
        EXPECT_EQ(root->left_child()->NumExploredNodesInSubtree(), 1);
        EXPECT_EQ(root->right_child()->NumExploredNodesInSubtree(), 1);
        EXPECT_EQ(root->left_child()->solution_result(), SolutionResult::kUnbounded);
        EXPECT_EQ(root->right_child()->solution_result(), SolutionResult::kUnbounded);
    }
...
GTEST_TEST(MixedIntegerBranchAndBoundNodeTest, TestBranch4) {
    ...
    EXPECT_EQ(root->right_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_EQ(root->left_child()->solution_result(), SolutionResult::kInfeasibleConstraints);
    EXPECT_EQ(root->right_child()->solution_result(), SolutionResult::kInfeasibleConstraints);
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestNewVariable) {
    ...
    static_assert(std::is_same_v<decltype(bnb.GetNewVariables(prog_x.head<2>())), VectorDecisionVariable<2>>,
                  "Should return VectorDecisionVariable<2> object.\n");
    static_assert(std::is_same_v<decltype(bnb.GetNewVariables(prog_x.head(2))), VectorUpTo4<symbolic::Variable>>,
                  "Should return VectorUpTo4<Variable> object.\n");
    MatrixDecisionVariable<2, 2> X;
    X << prog_x(0), prog_x(1), prog_x(2), prog_x(3);
    static_assert(std::is_same_v<decltype(bnb.GetNewVariables(X)), MatrixDecisionVariable<2, 2>>,
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestConstructor1) {
    ...
    const Eigen::Vector4d x_expected(0, 1, 0, 0.5);
    EXPECT_TRUE(CompareMatrices(best_solutions.begin()->second, x_expected, tol, MatrixCompareType::absolute));
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestLeafNodeFathomed1) {
    ...
    EXPECT_THROW(unused(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()))), std::runtime_error);
}
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestLeafNodeFathomed3) {
    ...
    const VectorDecisionVariable<4> x = dut.bnb()->root()->prog()->decision_variables();
    ...
    EXPECT_EQ(dut.mutable_root()->right_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_EQ(dut.mutable_root()->left_child()->left_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_EQ(dut.mutable_root()->left_child()->right_child()->NumExploredNodesInSubtree(), 1);
    EXPECT_FALSE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->right_child())));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->left_child()->left_child())));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->left_child()->right_child())));

    dut.mutable_root()->mutable_right_child()->Branch(x(2));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->right_child()->left_child())));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->right_child()->right_child())));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->left_child()->left_child())));
    EXPECT_TRUE(dut.bnb()->IsLeafNodeFathomed(*(dut.bnb()->root()->left_child()->right_child())));
}
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestLeafNodeFathomed4) {
    ...
    MixedIntegerBranchAndBoundTester dut(*prog, GurobiSolver::id());
    const VectorDecisionVariable<5> x = dut.bnb()->root()->prog()->decision_variables();
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestPickBranchingNode1) {
    ...
    dut.bnb()->SetNodeSelectionMethod(MixedIntegerBranchAndBound::NodeSelectionMethod::kMinLowerBound);
    // Pick the root node.
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestPickBranchingNode2) {
    ...
    dut.bnb()->SetNodeSelectionMethod(MixedIntegerBranchAndBound::NodeSelectionMethod::kDepthFirst);
    EXPECT_EQ(dut.PickBranchingNode(), dut.bnb()->root());
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestPickBranchingVariable1) {
    ...
    dut.bnb()->SetVariableSelectionMethod(MixedIntegerBranchAndBound::VariableSelectionMethod::kMostAmbivalent);
    EXPECT_TRUE(dut.PickBranchingVariable(*(dut.bnb()->root()))->equal_to(x(0)));
    dut.bnb()->SetVariableSelectionMethod(MixedIntegerBranchAndBound::VariableSelectionMethod::kLeastAmbivalent);
    ...
    dut.bnb()->SetVariableSelectionMethod(MixedIntegerBranchAndBound::VariableSelectionMethod::kMostAmbivalent);
    EXPECT_THROW(dut.PickBranchingVariable(*(dut.bnb()->root()->left_child())), std::runtime_error);
    dut.bnb()->SetVariableSelectionMethod(MixedIntegerBranchAndBound::VariableSelectionMethod::kLeastAmbivalent);
    EXPECT_THROW(dut.PickBranchingVariable(*(dut.bnb()->root()->left_child())), std::runtime_error);
}
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestBranchAndUpdate2) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.bnb()->solutions().begin()->second, x_expected, tol, MatrixCompareType::absolute));
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestSolve1) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.bnb()->GetSolution(x, 0), x_expected, tol, MatrixCompareType::absolute));
}

std::vector<MixedIntegerBranchAndBound::NodeSelectionMethod> NonUserDefinedPickNodeMethods() {
...
std::vector<MixedIntegerBranchAndBound::VariableSelectionMethod> NonUserDefinedPickVariableMethods() {
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestSolve2) {
    ...
    for (auto pick_variable : NonUserDefinedPickVariableMethods()) {
        for (auto pick_node : NonUserDefinedPickNodeMethods()) {
            ...
            EXPECT_TRUE(CompareMatrices(dut.bnb()->GetSolution(x, 0), x_expected0, tol, MatrixCompareType::absolute));
            // The costs are in the ascending order.
            double previous_cost = dut.bnb()->GetOptimalCost();
            for (int i = 1; i < static_cast<int>(dut.bnb()->solutions().size()); ++i) {
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestSteelBlendingProblem) {
    ...
    prog.AddLinearCost(1750 * b(0) + 990 * b(1) + 1240 * b(2) + 1680 * b(3) + 500 * x(0) + 450 * x(1) + 400 * x(2) +
                       100 * x(3));

    prog.AddLinearConstraint(5 * b(0) + 3 * b(1) + 4 * b(2) + 6 * b(3) + x(0) + x(1) + x(2) + x(3) == 25);
    prog.AddLinearConstraint(0.25 * b(0) + 0.12 * b(1) + 0.2 * b(2) + 0.18 * b(3) + 0.08 * x(0) + 0.07 * x(1) +
                                     0.06 * x(2) + 0.03 * x(3) ==
                             1.25);
    prog.AddLinearConstraint(0.15 * b(0) + 0.09 * b(1) + 0.16 * b(2) + 0.24 * b(3) + 0.06 * x(0) + 0.07 * x(1) +
                                     0.08 * x(2) + 0.09 * x(3) ==
                             1.25);
    ...
    for (auto pick_variable : NonUserDefinedPickVariableMethods()) {
        for (auto pick_node : NonUserDefinedPickNodeMethods()) {
            ...
            EXPECT_TRUE(CompareMatrices(bnb.GetSolution(x), x_expected0, tol, MatrixCompareType::absolute));
            EXPECT_TRUE(CompareMatrices(bnb.GetSolution(b), b_expected0, tol, MatrixCompareType::absolute));
...
void CheckAllIntegralSolution(const MixedIntegerBranchAndBound& bnb,
                              const Eigen::Ref<const VectorXDecisionVariable>& x,
                              const std::vector<std::pair<double, Eigen::VectorXd>>& all_integral_solutions,
                              double tol) {
    ...
    EXPECT_TRUE(CompareMatrices(bnb.GetSolution(x), all_integral_solutions[0].second, tol));
    double previous_cost = bnb.GetOptimalCost();
    for (int i = 1; i < static_cast<int>(bnb.solutions().size()); ++i) {
        ...
        for (int j = 1; j < static_cast<int>(all_integral_solutions.size()); ++j) {
            if (std::abs(cost_i - all_integral_solutions[j].first) < tol &&
                (bnb.GetSolution(x, i) - all_integral_solutions[j].second).norm() < tol) {
                found_match = true;
                break;
            }
        }
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, TestMultipleIntegralSolution1) {
    ...
    AddSos2Constraint(&prog, x.cast<symbolic::Expression>(), y.cast<symbolic::Expression>());
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, SearchIntegralSolutionByRounding2) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.bnb()->GetSolution(x), x_expected, tol, MatrixCompareType::absolute));
...
MixedIntegerBranchAndBoundNode* LeftMostNodeInSubTree(const MixedIntegerBranchAndBound& branch_and_bound,
    ...
    if (subtree_root.IsLeaf()) {
        ...
        auto left_most_node_left_tree = LeftMostNodeInSubTree(branch_and_bound, *(subtree_root.left_child()));
        if (!left_most_node_left_tree) {
            return LeftMostNodeInSubTree(branch_and_bound, *(subtree_root.right_child()));
        }
        return left_most_node_left_tree;
    }
}

GTEST_TEST(MixedIntegerBranchAndBoundTest, TestSetUserDefinedNodeSelectionFunction) {
    ...
    dut.bnb()->SetNodeSelectionMethod(MixedIntegerBranchAndBound::NodeSelectionMethod::kUserDefined);
    dut.bnb()->SetUserDefinedNodeSelectionFunction([](const MixedIntegerBranchAndBound& branch_and_bound) {
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, NodeCallbackTest) {
    ...
    auto call_back_fun = [&num_visited_nodes](const MixedIntegerBranchAndBoundNode& node,
...
GTEST_TEST(MixedIntegerBranchAndBoundTest, MaxNodesInTree) {
    ...
    EXPECT_LE(dut2.bnb()->root()->NumExploredNodesInSubtree(), options.max_explored_nodes);
