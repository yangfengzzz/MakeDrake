...
GTEST_TEST(DiagonallyDominantMatrixConstraint, ReturnYTest) {
    ...
    auto Y = prog.AddPositiveDiagonallyDominantMatrixConstraint(X.cast<symbolic::Expression>());
    EXPECT_EQ(Y.rows(), 5);
    EXPECT_EQ(Y.cols(), 5);
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 5; ++j) {
...
GTEST_TEST(DiagonallyDominantMatrixConstraint, three_by_three_vertices) {
    ...
    prog.AddPositiveDiagonallyDominantMatrixConstraint(X.cast<symbolic::Expression>());

    auto cost = prog.AddLinearCost(Eigen::Vector3d::Zero(), 0, VectorDecisionVariable<3>(X(0, 1), X(0, 2), X(1, 2)));
    ...
    auto solve_and_check = [&prog, &X](const Eigen::Vector3d& sol_expected,
        ...
        if (result.get_solver_id() != SnoptSolver::id()) {
            ...
            EXPECT_NEAR(result.get_optimal_cost(), result_expected.get_optimal_cost(), cost_equality_tol);

            // The matrix should be positive semidefinite.
            const Eigen::Matrix3d X_sol_expected = result_expected.GetSolution(X);
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> eigen_solver_expected(X_sol_expected);
            EXPECT_TRUE((eigen_solver_expected.eigenvalues().array() >= -psd_tol).all());
