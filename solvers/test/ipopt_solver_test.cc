...
INSTANTIATE_TEST_SUITE_P(IpoptTest,
                         LinearProgramTest,
...
TEST_F(InfeasibleLinearProgramTest0, TestIpopt) {
    ...
    if (solver.available()) {
        ...
        EXPECT_EQ(result.get_solver_details<IpoptSolver>().status, kIpoptLocalInfeasibility);
...
public:
    ...
    void eval(internal::VecIn<T> const& x, internal::VecOut<T>* y) const {
        ...
        double noise = std::pow(-1., noise_counter_) * max_noise_ * noise_counter_ / noise_counter_limit_;
        if (noise_counter_ >= 0) {
...
GTEST_TEST(IpoptSolverTest, AcceptableResult) {
    ...
    if (solver.available()) {
        double max_noise = 1e-2;
        {
            ...
            EXPECT_EQ(result.get_solver_details<IpoptSolver>().status, kIpoptMaxiterExceeded);
        }
        ...
        options.SetOption(IpoptSolver::id(), "acceptable_iter", 3);
        {
            ...
            prog.AddCost(NoisyQuadraticCost(max_noise), x);
            auto result = solver.Solve(prog, x_initial_guess, options);
            EXPECT_EQ(result.get_solver_details<IpoptSolver>().status, kIpoptStopAtAcceptablePoint);
    ...
    if (solver.is_available()) {
        ...
        for (int common_print_to_console : {0, 1}) {
            for (int solver_print_to_console : {0, 2}) {
                ...
                options.SetOption(IpoptSolver::id(), "print_level", solver_print_to_console);
                solver.Solve(prog, {}, options);
            }
        }
    }
...
INSTANTIATE_TEST_SUITE_P(IpoptSolverTest,
...
INSTANTIATE_TEST_SUITE_P(IpoptSolverTest, TestQPasSOCP, ::testing::ValuesIn(GetQPasSOCPProblems()));
