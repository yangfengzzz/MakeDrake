...
#include "solvers/csdp_solver_internal.h"
#include "solvers/test/csdp_test_examples.h"
...
void CompareProgVarInSdpa(const SdpaFreeFormat& sdpa_free_format, int variable_index, double val_expected) {
    const double val = std::get<double>(sdpa_free_format.prog_var_in_sdpa()[variable_index]);
...
void CompareProgVarInSdpa(const SdpaFreeFormat& sdpa_free_format,
    ...
    const SdpaFreeFormat::FreeVariableIndex s_index =
            std::get<SdpaFreeFormat::FreeVariableIndex>(sdpa_free_format.prog_var_in_sdpa()[variable_index]);
...
void CompareProgVarInSdpa(const SdpaFreeFormat& sdpa_free_format,
    ...
    const auto val = std::get<DecisionVariableInSdpaX>(sdpa_free_format.prog_var_in_sdpa()[variable_index]);
    ...
    EXPECT_EQ(val.entry_in_X.row_index_in_block, val_expected.entry_in_X.row_index_in_block);
    EXPECT_EQ(val.entry_in_X.column_index_in_block, val_expected.entry_in_X.column_index_in_block);
...
void CompareTriplets(const std::vector<Eigen::Triplet<double>>& triplets1,
    ...
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(mat1), Eigen::MatrixXd(mat2), 1E-12));
}
...
TEST_F(SDPwithOverlappingVariables1, TestSdpaFreeFormatConstructor) {
    ...
    CompareTriplets(dut.A_triplets()[0], {Eigen::Triplet<double>(0, 0, 1.0), Eigen::Triplet<double>(1, 1, -1.0)},
                    dut.num_X_rows(), dut.num_X_rows());
    CompareTriplets(dut.A_triplets()[1], {Eigen::Triplet<double>(0, 0, 1.0), Eigen::Triplet<double>(2, 2, -1.0)},
                    dut.num_X_rows(), dut.num_X_rows());
    CompareTriplets(dut.A_triplets()[2], {Eigen::Triplet<double>(0, 0, 1.0), Eigen::Triplet<double>(3, 3, -1.0)},
                    dut.num_X_rows(), dut.num_X_rows());
    // The constraint x0 >= 0.5, we add a slack variable y0 >= 0 with constraint
    // x0 - y0 = 0.5
    CompareTriplets(dut.A_triplets()[3], {Eigen::Triplet<double>(0, 0, 1.), Eigen::Triplet(4, 4, -1.)},
                    dut.num_X_rows(), dut.num_X_rows());
    // The constraint that x1 = 1
    CompareTriplets(dut.A_triplets()[4], {Eigen::Triplet<double>(0, 1, 0.5), Eigen::Triplet<double>(1, 0, 0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    // The constraint x2 <= 2. We add a slack variable y2 >= 0 with the constraint
    // x2 + y2 = 2
    CompareTriplets(
            dut.A_triplets()[5],
            {Eigen::Triplet<double>(2, 3, 0.5), Eigen::Triplet<double>(3, 2, 0.5), Eigen::Triplet<double>(5, 5, 1)},
            dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(
            dut.C_triplets(),
            {Eigen::Triplet<double>(0, 0, -2), Eigen::Triplet<double>(2, 3, -0.5), Eigen::Triplet<double>(3, 2, -0.5)},
            dut.num_X_rows(), dut.num_X_rows());
...
TEST_F(SDPwithOverlappingVariables2, TestSdpaFreeFormatConstructor) {
    ...
    // = 3
    CompareTriplets(dut.A_triplets()[1], {Eigen::Triplet<double>(0, 0, 1.), Eigen::Triplet<double>(2, 2, -1.)},
                    dut.num_X_rows(), dut.num_X_rows());
    CompareTriplets(dut.A_triplets()[2], {Eigen::Triplet<double>(0, 0, 1.), Eigen::Triplet<double>(3, 3, 1.)},
                    dut.num_X_rows(), dut.num_X_rows());
    // The constraint that x1 >= 1
    CompareTriplets(
            dut.A_triplets()[3],
            {Eigen::Triplet<double>(0, 1, 0.5), Eigen::Triplet<double>(1, 0, 0.5), Eigen::Triplet<double>(4, 4, -1)},
            dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(
            dut.C_triplets(),
            {Eigen::Triplet<double>(0, 0, -2), Eigen::Triplet<double>(0, 1, -0.5), Eigen::Triplet<double>(1, 0, -0.5)},
            dut.num_X_rows(), dut.num_X_rows());
    EXPECT_EQ(dut.d_triplets().size(), 0);
    EXPECT_EQ(dut.constant_min_cost_term(), 0);
}
...
TEST_F(CsdpDocExample, TestSdpaFreeFormatConstructor) {
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = 0; i <= j; ++i) {
            CompareProgVarInSdpa(dut, prog_->FindDecisionVariableIndex(X2_(i, j)),
                                 DecisionVariableInSdpaX(Sign::kPositive, 0, 1, i, j, 2));
        }
    }
    for (int i = 0; i < 2; ++i) {
        CompareProgVarInSdpa(dut, prog_->FindDecisionVariableIndex(y_(i)),
                             DecisionVariableInSdpaX(Sign::kPositive, 0, 2, i, i, 5));
    }

    // Check the cost.
    CompareTriplets(dut.C_triplets(),
                    {Eigen::Triplet<double>(0, 0, 2), Eigen::Triplet<double>(0, 1, 1), Eigen::Triplet<double>(1, 0, 1),
                     Eigen::Triplet<double>(1, 1, 2), Eigen::Triplet<double>(2, 2, 3), Eigen::Triplet<double>(3, 3, 2),
                     Eigen::Triplet<double>(2, 4, 1), Eigen::Triplet<double>(4, 2, 1), Eigen::Triplet<double>(4, 4, 3)},
                    dut.num_X_rows(), dut.num_X_rows());
    EXPECT_EQ(dut.d_triplets().size(), 0);
    EXPECT_EQ(dut.constant_min_cost_term(), 0);
    ...
    CompareTriplets(dut.A_triplets()[0],
                    {Eigen::Triplet<double>(0, 0, 3), Eigen::Triplet<double>(0, 1, 1), Eigen::Triplet<double>(1, 0, 1),
                     Eigen::Triplet<double>(1, 1, 3), Eigen::Triplet<double>(5, 5, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    CompareTriplets(dut.A_triplets()[1],
                    {Eigen::Triplet<double>(2, 2, 3), Eigen::Triplet<double>(3, 3, 4), Eigen::Triplet<double>(2, 4, 1),
                     Eigen::Triplet<double>(4, 2, 1), Eigen::Triplet<double>(4, 4, 5), Eigen::Triplet<double>(6, 6, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    EXPECT_EQ(dut.B_triplets().size(), 0);
...
TEST_F(LinearProgramBoundingBox1, TestSdpaFreeFormatConstructor) {
    ...
    EXPECT_EQ(dut.prog_var_in_sdpa().size(), prog_->num_vars());
    CompareProgVarInSdpa(dut, 0, DecisionVariableInSdpaX(Sign::kPositive, 0, 0, 0, 0, 0));
    CompareProgVarInSdpa(dut, 1, DecisionVariableInSdpaX(Sign::kPositive, 0, 0, 1, 1, 0));
    CompareProgVarInSdpa(dut, 2, DecisionVariableInSdpaX(Sign::kPositive, -1, 0, 3, 3, 0));
    CompareProgVarInSdpa(dut, 3, DecisionVariableInSdpaX(Sign::kNegative, 10, 0, 4, 4, 0));
    CompareProgVarInSdpa(dut, 4, DecisionVariableInSdpaX(Sign::kPositive, -2, 0, 5, 5, 0));
    ...
    CompareTriplets(dut.A_triplets()[0], {Eigen::Triplet<double>(1, 1, 1), Eigen::Triplet<double>(2, 2, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    g_expected(0) = 5;
    // Constraining -2 <= x(4) <= 5
    CompareTriplets(dut.A_triplets()[1], {Eigen::Triplet<double>(5, 5, 1), Eigen::Triplet<double>(6, 6, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    g_expected(1) = 7;
    EXPECT_EQ(dut.B_triplets().size(), 0);
    EXPECT_TRUE(CompareMatrices(dut.g(), g_expected));
    ...
    CompareTriplets(
            dut.C_triplets(),
            {Eigen::Triplet<double>(0, 0, -1), Eigen::Triplet<double>(1, 1, 1), Eigen::Triplet<double>(3, 3, -2),
             Eigen::Triplet<double>(4, 4, -3), Eigen::Triplet<double>(5, 5, 1)},
            dut.num_X_rows(), dut.num_X_rows());
...
TEST_F(CsdpLinearProgram2, TestSdpaFreeFormatConstructor) {
    ...
    CompareTriplets(dut.A_triplets()[1], A1_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[2], A2_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[3], A3_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[4], A4_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.B_triplets(), B_triplets_expected, 6, dut.num_free_variables());
    EXPECT_TRUE(CompareMatrices(dut.g(), g_expected));
...
TEST_F(CsdpLinearProgram3, TestSdpaFreeFormatConstructor) {
    ...
    EXPECT_EQ(dut.num_free_variables(), 1);
    EXPECT_EQ(dut.prog_var_in_sdpa().size(), 3);
    CompareProgVarInSdpa(dut, 0, DecisionVariableInSdpaX(Sign::kPositive, -1, 0, 0, 0, 0));
    CompareProgVarInSdpa(dut, 1, DecisionVariableInSdpaX(Sign::kNegative, 8, 0, 2, 2, 0));
    ...
    CompareTriplets(dut.C_triplets(), {Eigen::Triplet<double>(0, 0, 2), Eigen::Triplet<double>(2, 2, -3)},
                    dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[0], {Eigen::Triplet<double>(0, 0, 1), Eigen::Triplet<double>(1, 1, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    g_expected(0) = 11;

    // Check the constraint x(0) + 2x(1) + 3x(2) = 3
    CompareTriplets(dut.A_triplets()[1], {Eigen::Triplet<double>(0, 0, 1), Eigen::Triplet<double>(2, 2, -2)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(1, 0, 3);
    g_expected(1) = -12;

    // Check the constraint 2x(0) - x(2)>= -1
    CompareTriplets(dut.A_triplets()[2], {Eigen::Triplet<double>(0, 0, 2), Eigen::Triplet<double>(3, 3, -1)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(2, 0, -1);
    g_expected(2) = 1;

    // Check the constraint x(1) - 3x(2) <= 5
    CompareTriplets(dut.A_triplets()[3], {Eigen::Triplet<double>(2, 2, -1), Eigen::Triplet<double>(4, 4, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(3, 0, -3);
    g_expected(3) = -3;

    // Check the constraint -4 <= x(0) + x(2) <= 9
    CompareTriplets(dut.A_triplets()[4], {Eigen::Triplet<double>(0, 0, 1), Eigen::Triplet<double>(5, 5, -1)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(4, 0, 1);
    g_expected(4) = -3;

    CompareTriplets(dut.A_triplets()[5], {Eigen::Triplet<double>(0, 0, 1), Eigen::Triplet<double>(6, 6, 1)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(5, 0, 1);
    g_expected(5) = 10;
    CompareTriplets(dut.B_triplets(), B_triplets_expected, dut.g().rows(), dut.num_free_variables());
    EXPECT_TRUE(CompareMatrices(dut.g(), g_expected));
}
...
TEST_F(TrivialSDP1, TestSdpaFreeFormatConstructor) {
    ...
    for (int j = 0; j < 3; ++j) {
        for (int i = 0; i <= j; ++i) {
            CompareProgVarInSdpa(dut, prog_->FindDecisionVariableIndex(X1_(i, j)),
    ...
    CompareTriplets(dut.A_triplets()[0], A0_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.C_triplets(), C_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    EXPECT_EQ(dut.d_triplets().size(), 0);
}
...
TEST_F(TrivialSDP2, TestSdpaFreeFormatConstructor) {
    ...
    CompareProgVarInSdpa(dut, prog_->FindDecisionVariableIndex(y_), SdpaFreeFormat::FreeVariableIndex(0));
...
TEST_F(TrivialSOCP1, TestSdpaFreeFormatConstructor) {
    ...
    for (int i = 0; i < 2; ++i) {
        CompareProgVarInSdpa(dut, i + 1, DecisionVariableInSdpaX(Sign::kPositive, 0, 0, i, i, 0));
    }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        CompareTriplets(dut.A_triplets()[1 + i], A_diagonal_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[5], A5_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(5, 0, 1);
    g_expected(5) = -3;
    ...
    CompareTriplets(dut.A_triplets()[6], A6_triplets_expected, dut.num_X_rows(), dut.num_X_rows());
    g_expected(6) = 0;
...
TEST_F(TrivialSOCP2, TestSdpaFreeFormatConstructor) {
    ...
    // x(0) + 2 for i = 0, 1, 2.
    for (int i = 0; i < 3; ++i) {
        CompareTriplets(dut.A_triplets()[i], {Eigen::Triplet<double>(i, i, -1)}, dut.num_X_rows(), dut.num_X_rows());
        B_triplets_expected.emplace_back(i, 0, 1);
        g_expected(i) = -2;
    }
    // Check the linear equality constraint arising from Lorentz cone, X(0, 1) =
    // x(0) + x(1) + 1.
    CompareTriplets(dut.A_triplets()[3], {Eigen::Triplet<double>(0, 1, -0.5), Eigen::Triplet<double>(1, 0, -0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(3, 0, 1);
    ...
    // x(0) - x(1) + 1.
    CompareTriplets(dut.A_triplets()[4], {Eigen::Triplet<double>(0, 2, -0.5), Eigen::Triplet<double>(2, 0, -0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(4, 0, 1);
    ...
    // 0.
    CompareTriplets(dut.A_triplets()[5], {Eigen::Triplet<double>(1, 2, 0.5), Eigen::Triplet<double>(2, 1, 0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    g_expected(5) = 0;
...
TEST_F(TrivialSOCP3, TestSdpaFreeFormatConstructor) {
    ...
    // 2x(0) + 2 = X(0, 0).
    CompareTriplets(dut.A_triplets()[0], {Eigen::Triplet<double>(0, 0, -1)}, dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(0, 0, 2);
    g_expected(0) = -2;

    // Check the equality constraint arising from rotated Lorentz cone
    // x(0) + 2 = X(0, 1).
    CompareTriplets(dut.A_triplets()[1], {Eigen::Triplet<double>(0, 1, -0.5), Eigen::Triplet<double>(1, 0, -0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    B_triplets_expected.emplace_back(1, 0, 1);
    g_expected(1) = -2;
    ...
    // Check the equality constraint arising from rotated Lorentz cone
    // 3x(0) + x(1) + 1 = X(0, 2).
    CompareTriplets(dut.A_triplets()[2], {Eigen::Triplet<double>(0, 2, -0.5), Eigen::Triplet<double>(2, 0, -0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    ...
    for (int i = 0; i < 2; ++i) {
        CompareTriplets(dut.A_triplets()[3 + i], {Eigen::Triplet<double>(i + 1, i + 1, -1)}, dut.num_X_rows(),
                        dut.num_X_rows());
    ...
    CompareTriplets(dut.A_triplets()[5], {Eigen::Triplet<double>(1, 2, 0.5), Eigen::Triplet<double>(2, 1, 0.5)},
                    dut.num_X_rows(), dut.num_X_rows());
    g_expected(5) = 0;
    CompareTriplets(dut.B_triplets(), B_triplets_expected, dut.g().rows(), dut.num_X_rows());
    EXPECT_TRUE(CompareMatrices(dut.g(), g_expected));
}

void CheckRemoveFreeVariableByNullspaceApproach(
        const SdpaFreeFormat& dut,
        const Eigen::SparseMatrix<double>& C_hat,
        const std::vector<Eigen::SparseMatrix<double>>& A_hat,
        const Eigen::VectorXd& rhs_hat,
        const Eigen::VectorXd& y_hat,
        const Eigen::SparseQR<Eigen::SparseMatrix<double>, Eigen::COLAMDOrdering<int>>& QR_B,
        double tol) {
    EXPECT_EQ(y_hat.rows(), static_cast<int>(dut.A().size()));
    // Check Bᵀ * ŷ = d
    EXPECT_TRUE(CompareMatrices(Eigen::VectorXd(dut.B().transpose() * y_hat), Eigen::VectorXd(dut.d()), tol));
    // Check Ĉ = C -∑ᵢ ŷᵢAᵢ
    Eigen::SparseMatrix<double> C_hat_expected = dut.C();
    for (int i = 0; i < y_hat.rows(); ++i) {
        C_hat_expected -= y_hat(i) * dut.A()[i];
    }
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(C_hat), Eigen::MatrixXd(C_hat_expected), tol));
    // N is the null space of Bᵀ. Namely if we do a QR decomposition on B, then
    // N = Q₂.
    Eigen::SparseMatrix<double> Q;
    Q = QR_B.matrixQ();
    const Eigen::SparseMatrix<double> N = Q.rightCols(dut.B().rows() - QR_B.rank());
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(dut.B().transpose() * N),
                                Eigen::MatrixXd::Zero(dut.B().cols(), N.cols()), tol));
    // Check rhs_hat = Nᵀ * rhs
    EXPECT_TRUE(CompareMatrices(rhs_hat, Eigen::VectorXd(N.transpose() * dut.g()), tol));
    // Check Âᵢ = ∑ⱼNⱼᵢAⱼ
    EXPECT_EQ(static_cast<int>(A_hat.size()), N.cols());
    for (int i = 0; i < N.cols(); ++i) {
        Eigen::SparseMatrix<double> A_hat_expected(dut.num_X_rows(), dut.num_X_rows());
        A_hat_expected.setZero();
        for (int j = 0; j < static_cast<int>(dut.A().size()); ++j) {
            A_hat_expected += N.coeff(j, i) * dut.A()[j];
        }
        EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(A_hat[i]), Eigen::MatrixXd(A_hat_expected), tol));
    }
}

void TestRemoveFreeVariableByNullspaceApproach(const MathematicalProgram& prog) {
    ...
    dut.RemoveFreeVariableByNullspaceApproach(&C_hat, &A_hat, &rhs_hat, &y_hat, &QR_B);
    CheckRemoveFreeVariableByNullspaceApproach(dut, C_hat, A_hat, rhs_hat, y_hat, QR_B, 1E-10);
}
...
TEST_F(LinearProgramBoundingBox1, RemoveFreeVariableByNullspaceApproach) {
    ...
    ConvertSparseMatrixFormatToCsdpProblemData(dut.X_blocks(), C_hat, A_hat, rhs_hat, &C_csdp, &rhs_csdp,
                                               &constraints_csdp);
    struct csdp::blockmatrix X_csdp, Z;
    double* y{nullptr};
    csdp::cpp_initsoln(dut.num_X_rows(), rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, &X_csdp, &y, &Z);
    double pobj{0};
    double dobj{0};
    const int ret =
            csdp::cpp_easy_sdp(nullptr, dut.num_X_rows(), rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp,
                               -dut.constant_min_cost_term() + dut.g().dot(y_hat), &X_csdp, &y, &Z, &pobj, &dobj);
    ...
    csdp::cpp_free_prob(dut.num_X_rows(), rhs_hat.rows(), C_csdp, rhs_csdp, constraints_csdp, X_csdp, y, Z);
}
...
void TestRemoveFreeVariableByTwoSlackVariablesApproach(const MathematicalProgram& prog) {
    const SdpaFreeFormat dut(prog);
    std::vector<internal::BlockInX> X_hat_blocks;
    std::vector<Eigen::SparseMatrix<double>> A_hat;
    Eigen::SparseMatrix<double> C_hat;
    dut.RemoveFreeVariableByTwoSlackVariablesApproach(&X_hat_blocks, &A_hat, &C_hat);
    ...
    EXPECT_EQ(X_hat_blocks[X_hat_blocks.size() - 1].block_type, BlockType::kDiagonal);
    EXPECT_EQ(X_hat_blocks[X_hat_blocks.size() - 1].num_rows, dut.num_free_variables() * 2);
    ...
    for (int i = 0; i < static_cast<int>(A_hat.size()); ++i) {
        Eigen::MatrixXd A_hat_expected = Eigen::MatrixXd::Zero(num_X_hat_rows, num_X_hat_rows);
        A_hat_expected.block(0, 0, dut.A()[i].rows(), dut.A()[i].cols()) = dut.A()[i];
        Eigen::VectorXd bi = dut.B().row(i).transpose();
        A_hat_expected.block(dut.A()[i].rows(), dut.A()[i].cols(), dut.B().cols(), dut.B().cols()) = bi.asDiagonal();
    ...
    Eigen::MatrixXd C_hat_expected = Eigen::MatrixXd::Zero(num_X_hat_rows, num_X_hat_rows);
    C_hat_expected.topLeftCorner(dut.num_X_rows(), dut.num_X_rows()) = dut.C();
    C_hat_expected.block(dut.num_X_rows(), dut.num_X_rows(), dut.num_free_variables(), dut.num_free_variables()) =
            Eigen::VectorXd(dut.d()).asDiagonal();
    C_hat_expected.bottomRightCorner(dut.num_free_variables(), dut.num_free_variables()) =
            Eigen::VectorXd(-dut.d()).asDiagonal();
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(C_hat), C_hat_expected));
}

TEST_F(LinearProgramBoundingBox1, RemoveFreeVariableByTwoSlackVariablesApproach) {
...
void TestRemoveFreeVariableByLorentzConeSlackApproach(const MathematicalProgram& prog) {
    const SdpaFreeFormat dut(prog);
    std::vector<internal::BlockInX> X_hat_blocks;
    std::vector<Eigen::SparseMatrix<double>> A_hat;
    Eigen::VectorXd rhs_hat;
    Eigen::SparseMatrix<double> C_hat;
    dut.RemoveFreeVariableByLorentzConeSlackApproach(&X_hat_blocks, &A_hat, &rhs_hat, &C_hat);
    EXPECT_EQ(X_hat_blocks.size(), dut.X_blocks().size() + 1);
    for (int i = 0; i < static_cast<int>(dut.X_blocks().size()); ++i) {
        EXPECT_EQ(X_hat_blocks[i].block_type, dut.X_blocks()[i].block_type);
        EXPECT_EQ(X_hat_blocks[i].num_rows, dut.X_blocks()[i].num_rows);
    }
    EXPECT_EQ(X_hat_blocks[X_hat_blocks.size() - 1].block_type, BlockType::kMatrix);
    EXPECT_EQ(X_hat_blocks[X_hat_blocks.size() - 1].num_rows, dut.num_free_variables() + 1);
    EXPECT_EQ(A_hat.size(), dut.A().size() + dut.num_free_variables() * (dut.num_free_variables() + 1) / 2);
    ...
    for (int i = 0; i < static_cast<int>(dut.A().size()); ++i) {
        ...
        A_hat_expected.block(dut.A()[i].rows(), dut.A()[i].cols() + 1, 1, dut.B().cols()) = bi.transpose() / 2;
        A_hat_expected.block(dut.A()[i].rows() + 1, dut.A()[i].cols(), dut.B().cols(), 1) = bi / 2;
        EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(A_hat[i]), A_hat_expected));
    }
    ...
    for (int i = 1; i < dut.num_free_variables() + 1; ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(A_hat[A_hat_count++]), A_hat_expected));
    ...
    Eigen::MatrixXd C_hat_expected = Eigen::MatrixXd::Zero(num_X_hat_rows, num_X_hat_rows);
    C_hat_expected.topLeftCorner(dut.num_X_rows(), dut.num_X_rows()) = dut.C();
    C_hat_expected.block(dut.num_X_rows(), dut.num_X_rows() + 1, 1, dut.num_free_variables()) =
            Eigen::VectorXd(dut.d()).transpose() / 2;
    C_hat_expected.block(dut.num_X_rows() + 1, dut.num_X_rows(), dut.num_free_variables(), 1) =
            Eigen::VectorXd(dut.d()) / 2;
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(C_hat), C_hat_expected));
}

TEST_F(LinearProgramBoundingBox1, RemoveFreeVariableByLorentzConeSlackApproach) {
...
GTEST_TEST(SdpaFreeFormatTest, EmptyConstraint) {
    ...
    EXPECT_EQ(dut_empty_constraint.num_X_rows(), dut_no_empty_constraint.num_X_rows());
    EXPECT_EQ(dut_empty_constraint.num_free_variables(), dut_no_empty_constraint.num_free_variables());
    EXPECT_EQ(dut_empty_constraint.constant_min_cost_term(), dut_no_empty_constraint.constant_min_cost_term());
    EXPECT_EQ(dut_empty_constraint.A().size(), dut_no_empty_constraint.A().size());
    for (int i = 0; i < static_cast<int>(dut_empty_constraint.A().size()); ++i) {
        EXPECT_TRUE(CompareMatrices(dut_empty_constraint.A()[i].toDense(), dut_no_empty_constraint.A()[i].toDense()));
    }
    EXPECT_TRUE(CompareMatrices(dut_empty_constraint.C().toDense(), dut_no_empty_constraint.C().toDense()));
    EXPECT_TRUE(CompareMatrices(dut_empty_constraint.B().toDense(), dut_no_empty_constraint.B().toDense()));
    EXPECT_TRUE(CompareMatrices(dut_empty_constraint.d().toDense(), dut_no_empty_constraint.d().toDense()));

    // Now add an infeasible empty constraint, expect an exception.
    prog.AddLinearConstraint(Eigen::RowVector2d(0, 0), 1, kInf, Vector2<symbolic::Variable>(X(0, 0), X(1, 1)));
    DRAKE_EXPECT_THROWS_MESSAGE(SdpaFreeFormat(prog), ".* the problem is infeasible as it contains.*");
}
...
GTEST_TEST(SdpaFreeFormatTest, GenerateSDPA1) {
    ...
    prog.AddLinearEqualityConstraint(x(1) + 5 * X(0, 0) + 4 * X(0, 1) + 6 * X(1, 1), 20);
...
GTEST_TEST(SdpaFreeFormatTest, GenerateSDPA_remove_free_variables_two_slack) {
    ...
    EXPECT_TRUE(GenerateSDPA(prog, file_name, RemoveFreeVariableMethod::kTwoSlackVariables));
    EXPECT_TRUE(std::filesystem::exists({file_name + ".dat-s"}));
...
GTEST_TEST(SdpaFreeFormatTest, GenerateSDPA_remove_free_variables_null_space) {
    ...
    EXPECT_TRUE(GenerateSDPA(prog, file_name, RemoveFreeVariableMethod::kNullspace));
    EXPECT_TRUE(std::filesystem::exists({file_name + ".dat-s"}));
    std::ifstream infile(file_name + ".dat-s");
    ASSERT_TRUE(infile.is_open());
    std::string line;
...
GTEST_TEST(SdpaFreeFormatTest, GenerateSDPA_remove_free_variables_lorentz_slack) {
    // Test GenerateSDPA with prog that has free variables.
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    auto X = prog.NewSymmetricContinuousVariables<2>();
    ...
    EXPECT_TRUE(GenerateSDPA(prog, file_name, RemoveFreeVariableMethod::kLorentzConeSlack));
    EXPECT_TRUE(std::filesystem::exists({file_name + ".dat-s"}));
...
GTEST_TEST(SdpaFreeFormatTest, EnumToString) {
    ...
    // single item.
    EXPECT_EQ(fmt::to_string(RemoveFreeVariableMethod::kTwoSlackVariables), "kTwoSlackVariables");
}
