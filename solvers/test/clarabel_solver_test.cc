...
GTEST_TEST(LinearProgramTest, TestGeneralLP) {
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), x_expected, kTol, MatrixCompareType::absolute));
    }
    ...
    prog.AddBoundingBoxConstraint(2, 6 /* this upper bound = 6 is intentionally redundant.*/, x(0));
    if (solver.available()) {
        auto result = solver.Solve(prog, {}, {});
        EXPECT_TRUE(result.is_success());
...
GTEST_TEST(LinearProgramTest, TestAllLPConstraintTypes) {
  ...
  A << 1, 4, 0,
       ...
       -1, 0, 2;
    // clang-format on
    prog.AddLinearConstraint(A, Eigen::Vector3d(-2, -5, -std::numeric_limits<double>::infinity()),
    ...
    if (clarabel_solver.available()) {
        ...
        EXPECT_TRUE(
                CompareMatrices(result.GetSolution(x), Eigen::Vector3d(1, 1, 1), kTol, MatrixCompareType::absolute));
    }
}
...
INSTANTIATE_TEST_SUITE_P(ClarabelTest,
                         LinearProgramTest,
...
TEST_F(UnboundedLinearProgramTest0, TestUnbounded) {
    ...
    if (solver.available()) {
        ...
        EXPECT_TRUE(result.GetSolution(prog_->decision_variables()).array().isFinite().all());
    }
}
...
INSTANTIATE_TEST_SUITE_P(ClarabelTest, TestQPasSOCP, ::testing::ValuesIn(GetQPasSOCPProblems()));
...
GTEST_TEST(TestOptions, StandaloneReproduction) {
    ...
    prog.AddExponentialConeConstraint(Vector3<symbolic::Expression>(x(2), x(0), x(1)));
    ...
    if (solver.available()) {
        ...
        solver_options.SetOption(CommonSolverOption::kStandaloneReproductionFileName, repro_file_name);
...
GTEST_TEST(TestOptions, unrecognized) {
    ...
    if (solver.available()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(dut.prog(), std::nullopt, solver_options),
...
GTEST_TEST(TestZeroStepSize, ZeroStepSize) {
    ...
    if (solver.available()) {
        ...
        EXPECT_EQ(result.get_solver_details<ClarabelSolver>().status, "InsufficientProgress");
    }
}
