...
namespace solvers {
namespace {
bool IsFeasibleCheck(const MathematicalProgram& prog,
...
public:
    ...
    TestMixedIntegerRotationConstraint(MixedIntegerRotationConstraintGenerator::Approach approach,
                                       int num_intervals_per_half_axis)
        : prog_(),
          R_(NewRotationMatrixVars(&prog_)),
          approach_(approach),
          num_intervals_per_half_axis_(num_intervals_per_half_axis),
          feasibility_constraint_{prog_.AddLinearEqualityConstraint(Eigen::Matrix<double, 9, 9>::Identity(),
    ...
    bool IsFeasible(const RotationMatrixd& R_to_check) { return IsFeasible(R_to_check.matrix()); }
    ...
    void TestExactRotationMatrix() {
        ...
        Matrix3d R_check;
        R_check << 0.17082017792981191, 0.65144498431260445, -0.73921573253413542, -0.82327804434149443,
                -0.31781600529013027, -0.47032568342231595, -0.54132589862048197, 0.68892119955432829,
                0.48203096610835455;
    ...
    void TestInexactRotationMatrix() {
        ...
        Eigen::Matrix3d R_test = Matrix3d::Constant(1.0 / sqrt(3.0));  // All rows and columns are
        // on the unit sphere.
        ...
        if (num_intervals_per_half_axis_ == 1 &&
            approach_ == MixedIntegerRotationConstraintGenerator::Approach::kBoxSphereIntersection)
            EXPECT_TRUE(IsFeasible(R_test));
        else
            EXPECT_FALSE(IsFeasible(R_test));
...
class TestMixedIntegerRotationConstraintGenerator
    : public TestMixedIntegerRotationConstraint,
      public ::testing::TestWithParam<
              std::tuple<MixedIntegerRotationConstraintGenerator::Approach, int, IntervalBinning>> {
public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(TestMixedIntegerRotationConstraintGenerator);

    TestMixedIntegerRotationConstraintGenerator()
        : TestMixedIntegerRotationConstraint(std::get<0>(GetParam()), std::get<1>(GetParam())),
          interval_binning_(std::get<2>(GetParam())),
          rotation_generator_(approach_, num_intervals_per_half_axis_, interval_binning_),
...
TEST_P(TestMixedIntegerRotationConstraintGenerator, TestConstructor) {
    EXPECT_TRUE(CompareMatrices(rotation_generator_.phi(),
                                Eigen::VectorXd::LinSpaced(2 * num_intervals_per_half_axis_ + 1, -1, 1), 1E-12));
    EXPECT_TRUE(CompareMatrices(rotation_generator_.phi_nonnegative(),
                                Eigen::VectorXd::LinSpaced(num_intervals_per_half_axis_ + 1, 0, 1), 1E-12));
}
...
TEST_P(TestMixedIntegerRotationConstraintGenerator, TestBinaryAssignment) {
    ...
    auto UpdateBConstraint = [&b_constraint](const Eigen::Ref<const Eigen::VectorXd>& b_val) {
...
INSTANTIATE_TEST_SUITE_P(
        RotationTest,
        TestMixedIntegerRotationConstraintGenerator,
        ::testing::Combine(::testing::ValuesIn<std::vector<MixedIntegerRotationConstraintGenerator::Approach>>(
                                   {MixedIntegerRotationConstraintGenerator::Approach::kBilinearMcCormick,
                                    MixedIntegerRotationConstraintGenerator::Approach::kBoxSphereIntersection,
                                    MixedIntegerRotationConstraintGenerator::Approach::kBoth}),
                           ::testing::ValuesIn<std::vector<int>>({1, 2}),
                           ::testing::ValuesIn<std::vector<IntervalBinning>>({IntervalBinning::kLinear,
                                                                              IntervalBinning::kLogarithmic})));

class TestRotationMatrixBoxSphereIntersection : public TestMixedIntegerRotationConstraint,
                                                public ::testing::TestWithParam<int> {
public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(TestRotationMatrixBoxSphereIntersection);

    TestRotationMatrixBoxSphereIntersection()
        : TestMixedIntegerRotationConstraint(MixedIntegerRotationConstraintGenerator::Approach::kBoxSphereIntersection,
                                             GetParam()) {
        AddRotationMatrixBoxSphereIntersectionMilpConstraints(R_, num_intervals_per_half_axis_, &prog_);
    }
...
INSTANTIATE_TEST_SUITE_P(RotationTest,
                         TestRotationMatrixBoxSphereIntersection,
                         ::testing::ValuesIn<std::vector<int>>({1, 2}));
...
class TestOrthant : public ::testing::TestWithParam<std::tuple<int,
                                                               int,
                                                               bool,
                                                               std::pair<int, int>,
                                                               bool,
...
public:
    ...
    TestOrthant() : prog_(), R_(NewRotationMatrixVars(&prog_)) {
        ...
        MixedIntegerRotationConstraintGenerator rotation_generator(approach, num_bin, IntervalBinning::kLinear);
        ...
        Eigen::Vector3d vec0_lb = Eigen::Vector3d::Constant(-std::numeric_limits<double>::infinity());
        Eigen::Vector3d vec0_ub = Eigen::Vector3d::Constant(std::numeric_limits<double>::infinity());
...
INSTANTIATE_TEST_SUITE_P(
        RotationTest,
        TestOrthant,
        ::testing::Combine(::testing::ValuesIn<std::vector<int>>({1}),  // # of intervals per half axis
                           ::testing::ValuesIn<std::vector<int>>({0, 1, 2, 3, 4, 5, 6, 7}),  // orthant index
                           ::testing::ValuesIn<std::vector<bool>>({false, true}),  // row vector or column vector
                           ::testing::ValuesIn<std::array<std::pair<int, int>, 3>>(vector_indices()),  // vector indices
                           ::testing::ValuesIn<std::vector<bool>>({false, true}),  // same or opposite orthant
                           ::testing::ValuesIn<std::vector<MixedIntegerRotationConstraintGenerator::Approach>>(
                                   {MixedIntegerRotationConstraintGenerator::Approach::kBoxSphereIntersection,
                                    MixedIntegerRotationConstraintGenerator::Approach::
