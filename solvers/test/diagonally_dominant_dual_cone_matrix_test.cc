...
int TestIn2by2DiagonallyDominantDualCone(const Eigen::Matrix2d& X) {
    ...
    extreme_rays << Eigen::Matrix2d::Identity(2, 2), Eigen::Vector2d::Ones(), Eigen::Vector2d(1, -1);
...
int TestIn3by3DiagonallyDominantDualCone(const Eigen::Matrix3d& X) {
    ...
    extreme_rays << Eigen::Matrix3d::Identity(), Eigen::Vector3d(1, 1, 0), Eigen::Vector3d(1, -1, 0),
            Eigen::Vector3d(0, 1, 1), Eigen::Vector3d(0, 1, -1), Eigen::Vector3d(1, 0, 1), Eigen::Vector3d(1, 0, -1);
    for (int c = 0; c < extreme_rays.cols(); ++c) {
        if (extreme_rays.col(c).transpose() * X * extreme_rays.col(c) < -1e-10) {
            return c;
        }
...
GTEST_TEST(DiagonallyDominantMatrixDualConeConstraint, FeasibilityVariableCheck2by2) {
    // Test that DD* matrices are feasible.
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<2>();
    auto dual_cone_constraint = prog.AddPositiveDiagonallyDominantDualConeMatrixConstraint(X);
    // The dual cone constraint should be a single linear constraint with n²
    // inequalities where n is the number of rows in X.
    EXPECT_EQ(dual_cone_constraint.evaluator()->get_sparse_A().rows(), X.rows() * X.rows());
    ...
    // entries.
    EXPECT_EQ(dual_cone_constraint.evaluator()->get_sparse_A().nonZeros(), 4 * X.rows() * X.rows() - 3 * X.rows());

    // Ensure that a sparse constraint is constructed.
    EXPECT_FALSE(dual_cone_constraint.evaluator()->is_dense_A_constructed());

    auto X_constraint = prog.AddBoundingBoxConstraint(Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
    ...
    auto set_X_value = [&X_constraint](const Eigen::Vector3d& x_upper_triangle) {
        ...
        Eigen::Matrix2d ret;
        ret << x_upper_triangle(0), x_upper_triangle(1), x_upper_triangle(1), x_upper_triangle(2);
        return ret;
    };
...
GTEST_TEST(DiagonallyDominantMatrixDualConeConstraint, FeasibilityExpressionCheck2by2) {
    // Test that DD* matrices are feasible.
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<2>();
    ...
    auto dual_cone_constraint = prog.AddPositiveDiagonallyDominantDualConeMatrixConstraint(Z);
    // The dual cone constraint should be a single linear constraint with n²
    // inequalities where n is the number of rows in Z.
    EXPECT_EQ(dual_cone_constraint.evaluator()->get_sparse_A().rows(), Z.rows() * Z.rows());
    // Ensure that a sparse constraint is constructed.
    EXPECT_FALSE(dual_cone_constraint.evaluator()->is_dense_A_constructed());

    VectorX<symbolic::Expression> z_upper(3);
    z_upper << Z(0, 0), Z(0, 1), Z(1, 1);
    auto Z_constraint = prog.AddLinearEqualityConstraint(z_upper, Eigen::Vector3d::Zero());
    ...
    auto set_Z_value = [&Z_constraint](const Eigen::Vector3d& z_upper_triangle) {
        ...
        Eigen::Matrix2d ret;
        ret << z_upper_triangle(0), z_upper_triangle(1), z_upper_triangle(1), z_upper_triangle(2);
        return ret;
    };
...
GTEST_TEST(DiagonallyDominantMatrixDualConeConstraint, ThreeByThreeVerticesVariable) {
    ...
    prog.AddBoundingBoxConstraint(Eigen::Vector3d(1, 2, 3), Eigen::Vector3d(1, 2, 3), X.diagonal());
    auto dual_cone_constraint = prog.AddPositiveDiagonallyDominantDualConeMatrixConstraint(X);
    // The dual cone constraint should be a single linear constraint with n²
    // inequalities where n is the number of rows in X.
    EXPECT_EQ(dual_cone_constraint.evaluator()->get_sparse_A().rows(), X.rows() * X.rows());
    ...
    auto cost = prog.AddLinearCost(Eigen::Vector3d::Zero(), 0, VectorDecisionVariable<3>(a, b, c));
    auto solve_and_check = [&prog, &X, &a, &b, &c](const Eigen::Vector3d& sol_expected, double tol) {
        const auto result = Solve(prog);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(VectorDecisionVariable<3>(a, b, c)), sol_expected, tol));
...
GTEST_TEST(DiagonallyDominantMatrixDualConeConstraint, ThreeByThreeVerticesExpression) {
    // For a matrix to be in DD*, we have to have that Xₖₖ + Xⱼⱼ ≥ 2|Xₖⱼ+ Xⱼₖ|
    // I can manually compute the vertices of the polytope of (a, b, c)
    // to make
    //     [1 a+b b+c]
    ...
    auto cost = prog.AddLinearCost(Eigen::Vector3d::Zero(), 0, VectorDecisionVariable<3>(a, b, c));
    auto solve_and_check = [&prog, &GetZResult, &a, &b, &c](const Eigen::Vector3d& sol_expected, double tol) {
        const auto result = Solve(prog);
        EXPECT_TRUE(result.is_success());
        EXPECT_TRUE(CompareMatrices(result.GetSolution(VectorDecisionVariable<3>(a, b, c)), sol_expected, tol));
        // The matrix should be positive in DD*.
