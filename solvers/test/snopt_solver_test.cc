...
INSTANTIATE_TEST_SUITE_P(SnoptTest,
                         LinearProgramTest,
...
TEST_F(UnboundedLinearProgramTest0, TestSnopt) {
    ...
    if (solver.available() && !solver.is_bounded_lp_broken()) {
        ...
        EXPECT_EQ(result.get_optimal_cost(), -std::numeric_limits<double>::infinity());
    }
}
...
GTEST_TEST(SnoptTest, TestSetOption) {
    ...
    prog.AddConstraint(
            std::make_shared<QuadraticConstraint>(2 * Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero(), 1, 1), x);
    ...
    EXPECT_TRUE(CompareMatrices(solver_details.F, Eigen::Vector2d(-std::sqrt(3), 1), 1E-6));
...
GTEST_TEST(SnoptTest, TestPrintFile) {
    ...
    // This is to verify we can set the print out file through CommonSolverOption.
    {
        ...
        SolverOptions solver_options;
        solver_options.SetOption(CommonSolverOption::kPrintFileName, print_file_common);
        const auto result = solver.Solve(prog, {}, solver_options);
        EXPECT_TRUE(result.is_success());
    ...
    {
        const std::string print_file_common = temp_directory() + "/snopt_common2.out";
...
GTEST_TEST(SnoptTest, TestSparseCost) {
    ...
    solver.Solve(prog, {}, {}, &result);
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector3d(1, -1, 1), tol));
...
GTEST_TEST(SnoptTest, DistanceToTetrahedron) {
    ...
    x0 << 0, 0, 0, 0.7, 0.8, 0.9, 0.1, 0.2, 0.3, 1, 1, 1, 1, 0.4, 0.5, 0.6, 1.1, 1.2;
    ...
    const Eigen::Quaterniond quaternion_WB_sol(quat_WB_sol(0), quat_WB_sol(1), quat_WB_sol(2), quat_WB_sol(3));
    const math::RotationMatrixd R_WB(quaternion_WB_sol);
    const Eigen::Vector3d p_BQ = R_WB.transpose() * (p_WQ_sol - p_WB_sol);
    EXPECT_TRUE(((prog.A_tetrahedron() * p_BQ).array() <= prog.b_tetrahedron().array()).all());
}
...
GTEST_TEST(SnoptTest, MultiThreadTest) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        single_threaded[i].print_file = fmt::format("{}/snopt_single_thread_{}.out", temp_dir, i);
        multi_threaded[i].print_file = fmt::format("{}/snopt_multi_thread_{}.out", temp_dir, i);
    }
    ...
    auto run_solver = [&snopt_solver, &const_prog](PerThreadData* thread_data) {
        ...
        snopt_solver.Solve(const_prog, {thread_data->x_init}, options, &thread_data->result);
    };
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        for (const auto& per_thread_data_vec : {single_threaded, multi_threaded}) {
            ...
            EXPECT_TRUE(result.is_success());
            EXPECT_TRUE(CompareMatrices(result.get_x_val(), Eigen::Vector2d(0, 1), 1E-6));
        ...
        if (!kUsingAsan) {
            ...
            std::string contents_single = ReadFileOrThrow(single_threaded[i].print_file);
            ...
            for (auto* contents : {&contents_single, &contents_multi}) {
                ...
                *contents = std::regex_replace(*contents, std::regex("..... seconds"), "##### seconds");
                *contents = std::regex_replace(*contents, std::regex(".Printer........................\\d"),
                                               "(Printer)..............      ####");
            }
...
GTEST_TEST(SnoptTest, VariableScaling1) {
    ...
    if (solver.available() && solver.enabled()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(-1000000000000, -0.0001), tol));
    }
}
...
GTEST_TEST(SnoptTest, VariableScaling2) {
    ...
    Q(0, 0) /= (s * s);
    prog.AddConstraint(std::make_shared<QuadraticConstraint>(2 * Q, Eigen::Vector2d::Zero(), 25, 25), x);
    ...
    if (solver.available() && solver.enabled()) {
        ...
        EXPECT_TRUE(CompareMatrices(result.GetSolution(x), Eigen::Vector2d(-3 * s, -4), tol));
    }
}
...
GTEST_TEST(SnoptSolverTest, BadIntegerParameter) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), "Error setting Snopt integer parameter not an option");
}
...
GTEST_TEST(SnoptSolverTest, BadDoubleParameter) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), "Error setting Snopt double parameter not an option");
}
...
GTEST_TEST(SnoptSolverTest, BadStringParameter) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), "Error setting Snopt string parameter not an option");
}
...
INSTANTIATE_TEST_SUITE_P(SnoptTest, TestEllipsoidsSeparation, ::testing::ValuesIn(GetEllipsoidsSeparationProblems()));
...
INSTANTIATE_TEST_SUITE_P(SnoptTest, TestQPasSOCP, ::testing::ValuesIn(GetQPasSOCPProblems()));
...
INSTANTIATE_TEST_SUITE_P(SnoptTest, TestFindSpringEquilibrium, ::testing::ValuesIn(GetFindSpringEquilibriumProblems()));
...
private:
    void DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const final { EvalGeneric(); }
    ...
    void EvalGeneric() const { throw std::runtime_error("ThrowCost::EvalGeneric"); }
};
...
GTEST_TEST(SnoptTest, TestCostExceptionHandling) {
    ...
    SnoptSolver solver;
    if (solver.available() && solver.enabled()) {
        DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog), "Exception.*SNOPT.*ThrowCost.*");
    }
}
...
protected:
    ...
    void TearDown() override { EXPECT_EQ(::unsetenv("DRAKE_SNOPT_SOLVER_ENABLED"), 0); }
...
TEST_F(SnoptSolverEnabledTest, ExplicitlyDisabled) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver_.Solve(prog_), ".*SnoptSolver has not been properly configured.*");
}
