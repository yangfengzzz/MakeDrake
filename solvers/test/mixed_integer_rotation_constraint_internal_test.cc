...
namespace solvers {
namespace {
void CompareIntersectionResults(const std::vector<Vector3d>& desired, const std::vector<Vector3d>& actual) {
...
GTEST_TEST(RotationTest, TestAreAllVerticesCoPlanar) {
    ...
    for (const auto& bmin_bmax : bmin_bmax_coplanar) {
        auto pts = internal::ComputeBoxEdgesAndSphereIntersection(bmin_bmax.first, bmin_bmax.second);
    ...
    std::array<std::pair<Eigen::Vector3d, Eigen::Vector3d>, 3> bmin_bmax_non_coplanar{
    ...
    EXPECT_TRUE(internal::AreAllVerticesCoPlanar(pts, &n, &d));
    EXPECT_TRUE(CompareMatrices(n, Eigen::Vector3d::Constant(1.0 / std::sqrt(3)), 1E-10, MatrixCompareType::absolute));
...
void CompareHalfspaceRelaxation(const std::vector<Vector3d>& pts) {
    ...
    double d_expected;
    internal::ComputeHalfSpaceRelaxationForBoxSphereIntersection(pts, &n_expected, &d_expected);
    if (pts.size() == 3) {
...
GTEST_TEST(RotationTest, TestHalfSpaceRelaxation) {
    ...
    EXPECT_TRUE(CompareMatrices(n, Eigen::Vector3d::Constant(1 / std::sqrt(3)),
                                10 * std::numeric_limits<double>::epsilon(), MatrixCompareType::absolute));
...
GTEST_TEST(RotationTest, TestInnerFacetsAndHalfSpace) {
    ...
    Eigen::Vector3d n;
    double d;
    internal::ComputeHalfSpaceRelaxationForBoxSphereIntersection(intersection_pts, &n, &d);
    Eigen::Matrix<double, Eigen::Dynamic, 3> A;
    Eigen::VectorXd b;
    internal::ComputeInnerFacetsForBoxSphereIntersection(intersection_pts, &A, &b);
