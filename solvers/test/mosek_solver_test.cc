...
#include "solvers/test/exponential_cone_program_examples.h"
#include "solvers/test/l2norm_cost_examples.h"
#include "solvers/test/linear_program_examples.h"
#include "solvers/test/quadratic_constrained_program_examples.h"
...
INSTANTIATE_TEST_SUITE_P(MosekTest,
                         LinearProgramTest,
...
TEST_F(UnboundedLinearProgramTest0, Test) {
    ...
    if (solver.available()) {
        ...
        const MosekSolverDetails& mosek_solver_details = result.get_solver_details<MosekSolver>();
...
INSTANTIATE_TEST_SUITE_P(MosekTest, TestEllipsoidsSeparation, ::testing::ValuesIn(GetEllipsoidsSeparationProblems()));
...
INSTANTIATE_TEST_SUITE_P(MosekTest, TestQPasSOCP, ::testing::ValuesIn(GetQPasSOCPProblems()));
...
INSTANTIATE_TEST_SUITE_P(MosekTest, TestFindSpringEquilibrium, ::testing::ValuesIn(GetFindSpringEquilibriumProblems()));
...
GTEST_TEST(MosekTest, TestLogging) {
    ...
    solver_options.SetOption(CommonSolverOption::kPrintToConsole, 1);
    DRAKE_EXPECT_THROWS_MESSAGE(solver.Solve(prog, {}, solver_options, &result),
...
GTEST_TEST(MosekTest, SolverOptionsTest) {
    ...
    EXPECT_EQ(result.get_solver_details<MosekSolver>().rescode, MSK_RES_ERR_HUGE_C);
...
GTEST_TEST(MosekSolver, SolverOptionsErrorTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(mosek_solver.Solve(prog, {}, solver_options, &result),
...
GTEST_TEST(MosekTest, MinimalDistanceFromSphereProblem) {
    for (bool with_linear_cost : {true, false}) {
        MinimalDistanceFromSphereProblem<3> dut1(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0, 0, 0), 1,
                                                 with_linear_cost);
        MinimalDistanceFromSphereProblem<3> dut2(Eigen::Vector3d(0, 0, 1), Eigen::Vector3d(0, 0, 0), 1,
                                                 with_linear_cost);
        MinimalDistanceFromSphereProblem<3> dut3(Eigen::Vector3d(0, 1, 1), Eigen::Vector3d(0, 0, 0), 1,
                                                 with_linear_cost);
        MinimalDistanceFromSphereProblem<3> dut4(Eigen::Vector3d(0, 1, 1), Eigen::Vector3d(-1, -1, 0), 1,
                                                 with_linear_cost);
        MosekSolver solver;
        if (solver.available()) {
...
GTEST_TEST(MosekTest, QPDualSolution1) {
    ...
    if (solver.available()) {
        ...
        solver_options.SetOption(solver.id(), "MSK_DPAR_INTPNT_QO_TOL_REL_GAP", 1e-12);
...
template <typename C>
void CheckDualSolutionNotNan(const MathematicalProgramResult& result, const Binding<C>& constraint) {
...
GTEST_TEST(MosekTest, InfeasibleSemidefiniteProgramTest) {
    ...
    auto constraint2 = prog.AddLinearConstraint(X(0, 0) + X(1, 1) + X(2, 2) <= -1);
