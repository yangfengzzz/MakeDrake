...
protected:
    MoseklmLicenseFileTest() : orig_moseklm_license_file_(GetEnvStr("MOSEKLM_LICENSE_FILE")) {}
    ...
    void TearDown() override {
        if (orig_moseklm_license_file_) {
            const int setenv_result = ::setenv("MOSEKLM_LICENSE_FILE", orig_moseklm_license_file_->c_str(), 1);
...
TEST_F(MoseklmLicenseFileTest, MoseklmLicenseFileUnset) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(solver_.Solve(prog_), ".*MosekSolver has not been properly configured.*");
}
