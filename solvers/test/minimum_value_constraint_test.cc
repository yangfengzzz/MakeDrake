...
template <typename T>
VectorX<T> SquareAndReturnLessThanInfluenceValue(const Eigen::Ref<const VectorX<T>>& x, double influence_value) {
...
GTEST_TEST(MinimumValueLowerBoundConstraintTests, ConstructorTest) {
    ...
    MinimumValueLowerBoundConstraint dut(expected_num_vars, expected_minimum_value,
                                         expected_influence_value - expected_minimum_value, expected_max_num_values,
...
GTEST_TEST(MinimumValueUpperBoundConstraintTest, ConstructorTest) {
    ...
    MinimumValueUpperBoundConstraint dut(expected_num_vars, expected_minimum_value_upper,
...
GTEST_TEST(MinimumValueLowerBoundConstraintTests, EvalNonsymbolicTest) {
    ...
    MinimumValueLowerBoundConstraint dut_return_all(num_vars, minimum_value_lower,
                                                    influence_value - minimum_value_lower, max_num_values,
    ...
    MinimumValueLowerBoundConstraint dut_return_less_than_influence_value(
            num_vars, minimum_value_lower, influence_value - minimum_value_lower, max_num_values,
    ...
    AutoDiffVecXd x_sqrt_min_value_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, std::sqrt(minimum_value_lower), 2 * std::sqrt(influence_value));
    auto check_constraints = [&](std::vector<AutoDiffVecXd> inputs, bool should_constraints_be_satisfied) {
        for (const AutoDiffVecXd& x : inputs) {
            test::CheckConstraintEvalNonsymbolic(dut_return_all, x, tol);
            test::CheckConstraintEvalNonsymbolic(dut_return_less_than_influence_value, x, tol);
            AutoDiffVecXd y_return_all, y_return_less_than_influence_value;
            dut_return_all.Eval(x, &y_return_all);
            dut_return_less_than_influence_value.Eval(x, &y_return_less_than_influence_value);
            ...
            EXPECT_EQ(dut_return_all.CheckSatisfied(x, kEps), should_constraints_be_satisfied);
            EXPECT_EQ(dut_return_less_than_influence_value.CheckSatisfied(x, kEps), should_constraints_be_satisfied);
        }
    };
    // Check with inputs that should violate the constraints.
    check_constraints({x_0, x_0_to_twice_sqrt_influence_value,
                       x_sqrt_min_value_to_twice_sqrt_influence_value - AutoDiffVecXd::Constant(num_vars, kEps)},
                      false);
    ...
    AutoDiffVecXd x_sqrt_influence_value_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, 1.1 * std::sqrt(influence_value), 2 * std::sqrt(influence_value));
    check_constraints({x_sqrt_influence_value_to_twice_sqrt_influence_value}, true);
}
...
GTEST_TEST(MinimumValueUpperBoundConstraintTests, EvalNonsymbolicTest) {
    ...
    double influence_value{0.2};
    MinimumValueUpperBoundConstraint dut_return_all(num_vars, minimum_value_upper,
                                                    influence_value - minimum_value_upper, max_num_values,
                                                    &SquareAndReturnAll<AutoDiffXd>, &SquareAndReturnAll<double>);
    MinimumValueUpperBoundConstraint dut_return_less_than_influence_value(
            num_vars, minimum_value_upper, influence_value - minimum_value_upper, max_num_values,
            &SquareAndReturnLessThanInfluenceValue<AutoDiffXd>);
    double tol = kEps;
    AutoDiffVecXd x_0 = AutoDiffVecXd::Zero(num_vars);
    AutoDiffVecXd x_0_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, 0, 2 * std::sqrt(influence_value));
    AutoDiffVecXd x_sqrt_min_value_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, std::sqrt(minimum_value_upper), 2 * std::sqrt(influence_value));
    AutoDiffVecXd x_sqrt_min_value_plus_eps_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, std::sqrt(minimum_value_upper + 0.01), 2 * std::sqrt(influence_value));
    auto check_constraints = [&](std::vector<AutoDiffVecXd> inputs, bool should_constraints_be_satisfied) {
        for (const AutoDiffVecXd& x : inputs) {
            test::CheckConstraintEvalNonsymbolic(dut_return_all, x, tol);
            test::CheckConstraintEvalNonsymbolic(dut_return_less_than_influence_value, x, tol);
    ...
    check_constraints({x_sqrt_min_value_to_twice_sqrt_influence_value}, true);
    // Check with inputs that should violate the constraints.
    check_constraints({x_sqrt_min_value_plus_eps_to_twice_sqrt_influence_value}, false);
    // All value are larger than influence_value.
    AutoDiffVecXd x_sqrt_influence_value_to_twice_sqrt_influence_value =
            AutoDiffVecXd::LinSpaced(num_vars, 1.1 * std::sqrt(influence_value), 2 * std::sqrt(influence_value));
    check_constraints({x_sqrt_influence_value_to_twice_sqrt_influence_value}, false);

    // Now make sure the gradient is continuous across influence_value.
    const AutoDiffVecXd x_below_influence = math::InitializeAutoDiff(Eigen::VectorXd::LinSpaced(
            num_vars, std::sqrt(minimum_value_upper * 0.99), std::sqrt(influence_value - 1E-5)));
    ...
    for (const auto* constraint : {&dut_return_all, &dut_return_less_than_influence_value}) {
        constraint->Eval(x_below_influence, &y_below_influence);
        constraint->Eval(x_above_influence, &y_above_influence);
        EXPECT_NEAR(y_below_influence(0).value(), y_above_influence(0).value(), 1E-10);
        EXPECT_TRUE(CompareMatrices(y_below_influence(0).derivatives(), y_above_influence(0).derivatives(), 1E-10));
    }
}
...
GTEST_TEST(MinimumValueLowerBoundConstraintTests, EvalNoValuesTest) {
    ...
    double minimum_value_lower{0.1};
    double influence_value{0.2};
    MinimumValueLowerBoundConstraint dut_no_values(num_vars, minimum_value_lower, influence_value - minimum_value_lower,
                                                   max_num_values, &ReturnNoValues<AutoDiffXd>);
    test::CheckConstraintEvalNonsymbolic(dut_no_values, AutoDiffVecXd::Zero(num_vars), kEps);
}
...
GTEST_TEST(MinimumValueUpperBoundConstraintTests, EvalNoValuesTest) {
    ...
    double minimum_value_upper{0.1};
    double influence_value{0.2};
    MinimumValueUpperBoundConstraint dut_no_values(num_vars, minimum_value_upper, influence_value - minimum_value_upper,
                                                   max_num_values, &ReturnNoValues<AutoDiffXd>);
    test::CheckConstraintEvalNonsymbolic(dut_no_values, AutoDiffVecXd::Zero(num_vars), kEps);
}
...
GTEST_TEST(MinimumValueLowerBoundConstraintTests, EvalSymbolicTest) {
    ...
    double minimum_value_lower{0.1};
    double influence_value{0.2};
    MinimumValueLowerBoundConstraint dut(num_vars, minimum_value_lower, influence_value - minimum_value_lower,
...
GTEST_TEST(MinimumValueUpperBoundConstraintTests, EvalSymbolicTest) {
    ...
    double minimum_value_upper{0.1};
    double influence_value{0.2};
    MinimumValueUpperBoundConstraint dut(num_vars, minimum_value_upper, influence_value - minimum_value_upper,
                                         max_num_values, &SquareAndReturnAll<AutoDiffXd>);
    VectorX<symbolic::Variable> x{num_vars};
    VectorX<symbolic::Expression> y;
    EXPECT_THROW(dut.Eval(x, &y), std::logic_error);
