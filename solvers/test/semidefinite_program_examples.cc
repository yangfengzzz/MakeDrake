#include "solvers/test/semidefinite_program_examples.h"
...
void FindCommonLyapunov(const SolverInterface& solver, const std::optional<SolverOptions>& solver_options, double tol) {
    MathematicalProgram prog;
  ...
  A1 << -1, -1, -2,
    ...
    // clang-format on
    auto binding1 =
            prog.AddPositiveSemidefiniteConstraint(-A1.transpose() * P - P * A1 - psd_epsilon * Matrix3d::Identity());
  ...
  A2 << -1, -1.2, -1.8,
    ...
    // clang-format on
    auto binding2 =
            prog.AddPositiveSemidefiniteConstraint(-A2.transpose() * P - P * A2 - psd_epsilon * Matrix3d::Identity());

    const MathematicalProgramResult result = RunSolver(prog, solver, {}, solver_options);
    ...
    EXPECT_TRUE(CompareMatrices(P_value, P_value.transpose(), std::numeric_limits<double>::epsilon(),
    ...
    EXPECT_TRUE(CompareMatrices(A1.transpose() * P_value + P_value * A1 + psd_epsilon * Matrix3d::Identity(), -Q1_value,
                                tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(A2.transpose() * P_value + P_value * A2 + psd_epsilon * Matrix3d::Identity(), -Q2_value,
                                tol, MatrixCompareType::absolute));
}

void FindOuterEllipsoid(const SolverInterface& solver, const std::optional<SolverOptions>& solver_options, double tol) {
...
void SolveEigenvalueProblem(const SolverInterface& solver,
  ...
  F2 << 2, 0.4, 0.7,
    ...
    prog.AddLinearMatrixInequalityConstraint({Matrix3d::Zero(), Matrix3d::Identity(), -F1, -F2}, {z, x});
...
void SolveSDPwithSecondOrderConeExample1(const SolverInterface& solver, double tol) {
    MathematicalProgram prog;
  ...
  C0 << 2, 1, 0,
    ...
    prog.AddLinearConstraint((Matrix3d::Identity() * X.cast<Expression>()).trace() + x(0) == 1);
    prog.AddLinearConstraint((Matrix3d::Ones() * X.cast<Expression>()).trace() + x(1) + x(2) == 0.5);
...
void SolveSDPwithSecondOrderConeExample2(const SolverInterface& solver, double tol) {
    MathematicalProgram prog;
    ...
    const auto X_val = result.GetSolution(X);
    const auto x_val = result.GetSolution(x);
    EXPECT_NEAR(result.get_optimal_cost(), X_val(0, 0) + X_val(1, 1) + x_val, tol);
    ...
    EXPECT_GE(X_val(0, 0), std::sqrt(std::pow(X_val(1, 1) + x_val, 2) + std::pow(X_val(1, 1) + X_val(2, 2), 2)) - tol);
    EXPECT_NEAR(X_val(1, 0) + X_val(2, 1), 1, tol);
    EXPECT_GE(x_val, -tol);
}

void SolveSDPwithOverlappingVariables(const SolverInterface& solver, double tol) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<3>();
    prog.AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x(0), x(1), x(1), x(0)).finished());
    prog.AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x(0), x(2), x(2), x(0)).finished());
...
void SolveSDPwithQuadraticCosts(const SolverInterface& solver, double tol) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<3>();
    const Matrix2<symbolic::Variable> X1 = (Matrix2<symbolic::Variable>() << x(0), x(1), x(1), x(0)).finished();
    auto psd_constraint1 = prog.AddPositiveSemidefiniteConstraint(X1);
    const Matrix2<symbolic::Variable> X2 = (Matrix2<symbolic::Variable>() << x(0), x(2), x(2), x(0)).finished();
    ...
    const auto psd_dual1 = math::ToSymmetricMatrixFromLowerTriangularColumns(result.GetDualSolution(psd_constraint1));
    const auto psd_dual2 = math::ToSymmetricMatrixFromLowerTriangularColumns(result.GetDualSolution(psd_constraint2));
...
void TestSDPDualSolution1(const SolverInterface& solver, double tol) {
    ...
    auto bb_con = prog.AddBoundingBoxConstraint(Eigen::Vector2d(kInf, kInf), Eigen::Vector2d(4, 1),
    ...
    EXPECT_TRUE(CompareMatrices(X_sol, (Eigen::Matrix2d() << 4, -2, -2, 1).finished(), tol));
    ...
    EXPECT_TRUE(CompareMatrices(result.GetDualSolution(bb_con), bb_con_dual_expected, tol));
    const auto psd_dual = math::ToSymmetricMatrixFromLowerTriangularColumns(result.GetDualSolution(psd_con));
