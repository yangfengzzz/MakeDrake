#include "solvers/test/rotation_constraint_visualization.h"
...
void DrawArcBoundaryOfBoxSphereIntersection(const Eigen::Vector3d& arc_end0,
    ...
    bool positive_free_axis1 = arc_end0(free_axis1) > 0 || arc_end1(free_axis1) > 0;
    for (int i = 1; i < kNumViaPoints - 1; ++i) {
        // A point `x` on the arc satisfies
        // x(free_axis0)^2 + x(free_axis1)^2 = 1 - x(fixed_axis)^2
        via_pts(free_axis1, i) =
                std::sqrt(1 - std::pow(via_pts(fixed_axis, i), 2) - std::pow(via_pts(free_axis0, i), 2));
    ...
    CallPython("plot3", via_pts.row(0).transpose(), via_pts.row(1).transpose(), via_pts.row(2).transpose(),
...
void DrawSphere(const Eigen::RowVector3d& color) {
    using common::CallPython;
    using common::ToPythonKwargs;
    CallPython("sphere", 40, ToPythonKwargs("alpha", 0.2, "color", color.transpose(), "linestyle", "None"));
}
...
void DrawBox(const Eigen::Vector3d& bmin, const Eigen::Vector3d& bmax, const Eigen::RowVector3d& color) {
    ...
    if (bmin.norm() <= 1 && bmax.norm() >= 1) {
        ...
        CallPython("box", bmin, bmax, ToPythonKwargs("alpha", 0.2, "color", color.transpose(), "linestyle", "None"));
    }
}
...
void DrawBoxSphereIntersection(const Eigen::Vector3d& bmin,
    ...
    auto intersection_pts = internal::ComputeBoxEdgesAndSphereIntersection(orthant_bmin, orthant_bmax);
