...
void CheckGetDense(const SparseAndDenseMatrix* dut, const Eigen::Ref<const Eigen::MatrixXd>& dense_expected) {
    EXPECT_TRUE(CompareMatrices(dut->GetAsDense(), dense_expected));
}

void CheckGetDenseThread(const SparseAndDenseMatrix& dut, const Eigen::Ref<const Eigen::MatrixXd>& dense_expected) {
