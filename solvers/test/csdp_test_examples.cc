...
SDPwithOverlappingVariables1::SDPwithOverlappingVariables1()
    ...
    prog_->AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x_(0), x_(1), x_(1), x_(0)).finished());
    prog_->AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x_(0), x_(2), x_(2), x_(0)).finished());
}
...
SDPwithOverlappingVariables2::SDPwithOverlappingVariables2()
    ...
    prog_->AddBoundingBoxConstraint(Eigen::Vector2d(2, 1), Eigen::Vector2d(3, kInf), x_);
    prog_->AddPositiveSemidefiniteConstraint((Matrix2<symbolic::Variable>() << x_(0), x_(1), x_(1), x_(0)).finished());
}
...
CsdpDocExample::CsdpDocExample()
    ...
    prog_->AddBoundingBoxConstraint(Eigen::Vector2d(0, 0), Eigen::Vector2d(kInf, kInf), y_);
    prog_->AddLinearCost(-(2 * X1_(0, 0) + 2 * X1_(0, 1) + 2 * X1_(1, 1) + 3 * X2_(0, 0) + 2 * X2_(1, 1) +
                           2 * X2_(0, 2) + 3 * X2_(2, 2)));
    prog_->AddLinearEqualityConstraint(3 * X1_(0, 0) + 2 * X1_(0, 1) + 3 * X1_(1, 1) + y_(0), 1);
    prog_->AddLinearEqualityConstraint(3 * X2_(0, 0) + 4 * X2_(1, 1) + 2 * X2_(0, 2) + 5 * X2_(2, 2) + y_(1), 2);
}
...
CsdpLinearProgram2::CsdpLinearProgram2() : prog_{new MathematicalProgram()}, x_{prog_->NewContinuousVariables<3>()} {
...
CsdpLinearProgram3::CsdpLinearProgram3() : prog_{new MathematicalProgram()}, x_{prog_->NewContinuousVariables<3>()} {
    prog_->AddBoundingBoxConstraint(Eigen::Vector2d(-1, -kInf), Eigen::Vector2d(10, 8), x_.head<2>());
    Eigen::Matrix<double, 4, 3> A;
    // clang-format off
  A << 1, 2, 3,
       2, 0, -1,
       ...
       1, 0, 1;
    // clang-format on
    prog_->AddLinearConstraint(A, Eigen::Vector4d(3, -1, -kInf, -4), Eigen::Vector4d(3, kInf, 5, 9), x_);
    prog_->AddLinearCost(-(2 * x_(0) + 3 * x_(1) + 4 * x_(2) + 3));
}

TrivialSDP1::TrivialSDP1() : prog_{new MathematicalProgram()}, X1_{prog_->NewSymmetricContinuousVariables<3>()} {
    prog_->AddPositiveSemidefiniteConstraint(X1_);
...
TrivialSDP2::TrivialSDP2()
    ...
    Eigen::Matrix2d F2;
    F2 << 2, 0, 0, 4;
    prog_->AddConstraint(std::make_shared<LinearMatrixInequalityConstraint>(std::vector<Eigen::MatrixXd>{F0, F1, F2}),
                         Vector2<symbolic::Variable>(y_, X1_(0, 0)));
    prog_->AddLinearEqualityConstraint(X1_(0, 0) + 2 * X1_(1, 1) + 3 * y_, 1);
    prog_->AddLinearCost(-y_);
}

TrivialSOCP1::TrivialSOCP1() : prog_{new MathematicalProgram()}, x_{prog_->NewContinuousVariables<3>()} {
    prog_->AddBoundingBoxConstraint(Eigen::Vector2d::Zero(), Eigen::Vector2d(kInf, kInf), x_.tail<2>());
...
TrivialSOCP2::TrivialSOCP2() : prog_{new MathematicalProgram()}, x_{prog_->NewContinuousVariables<2>()} {
...
TrivialSOCP3::TrivialSOCP3() : prog_{new MathematicalProgram()}, x_{prog_->NewContinuousVariables<2>()} {
    prog_->AddLinearCost(x_(1));
