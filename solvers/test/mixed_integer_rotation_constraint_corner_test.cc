...
class TestBoxSphereCorner : public ::testing::TestWithParam<std::tuple<int, bool, int, RotationMatrixIntervalBinning>> {
public:
    ...
    TestBoxSphereCorner()
        ...
        if (r_interval_binning_ == RotationMatrixIntervalBinning::kLinear ||
            ...
            const IntervalBinning interval_binning = r_interval_binning_ == RotationMatrixIntervalBinning::kLinear
                                                             ? IntervalBinning::kLinear
                                                             : IntervalBinning::kLogarithmic;
            const MixedIntegerRotationConstraintGenerator rotation_generator(
                    MixedIntegerRotationConstraintGenerator::Approach::kBoxSphereIntersection, N, interval_binning);
                ...
                for (int i = 0; i < 3; ++i) {
                    for (int j = 0; j < 3; ++j) {
                        if (interval_binning == IntervalBinning::kLinear) {
                            ...
                            // logarithmic binning.
                            prog_.AddConstraint(CreateBinaryCodeMatchConstraint(ret.B_[i][j].cast<Expression>(),
                                                                                gray_codes.row(N + k).transpose(),
                                                                                Cpos_[k](i, j)));
                            prog_.AddConstraint(CreateBinaryCodeMatchConstraint(ret.B_[i][j].cast<Expression>(),
                                                                                gray_codes.row(N - k - 1).transpose(),
                                                                                Cneg_[k](i, j)));
                        }
                    }
                }
            ...
            const auto ret = AddRotationMatrixBoxSphereIntersectionMilpConstraints(R_, N, &prog_);
            Cpos_ = ret.CRpos;
            Cneg_ = ret.CRneg;
        }
    }
...
TEST_P(TestBoxSphereCorner, TestOrthogonal) {
    ...
    if (is_bmin_) {
        ...
        prog_.AddLinearConstraint(Eigen::Vector2d::Ones() == orthant_C[2].block<2, 1>(1, 0));
    } else {
        prog_.AddLinearConstraint(1 == orthant_C[0](0));
        prog_.AddLinearConstraint(Eigen::Vector2d::Ones() == orthant_C[1].block<2, 1>(1, 0));
    }
    ...
    EXPECT_TRUE(CompareMatrices(box_pt.cross(R_val.col(free_axis0)), R_val.col(free_axis1), 1E-4,
...
INSTANTIATE_TEST_SUITE_P(RotationTest,
                         TestBoxSphereCorner,
                         ::testing::Combine(::testing::ValuesIn<std::vector<int>>({0}),      // Orthant
...
INSTANTIATE_TEST_SUITE_P(RotationTest,
                         TestBoxSphereCorner,
                         ::testing::Combine(::testing::ValuesIn<std::vector<int>>({0, 1, 2, 3, 4, 5, 6, 7}),  // Orthant
