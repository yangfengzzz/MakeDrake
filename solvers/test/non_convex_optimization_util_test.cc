#include "solvers/non_convex_optimization_util.h"
...
GTEST_TEST(DecomposeNonConvexQuadraticForm, Test0) {
    ...
    EXPECT_TRUE(CompareMatrices(Q2, Eigen::Matrix3d::Zero(), 1E-5, MatrixCompareType::absolute));
}
...
GTEST_TEST(DecomposeNonConvexQuadraticForm, Test1) {
    ...
    Eigen::Matrix3d Q = -Eigen::Matrix3d::Identity();
    std::tie(Q1, Q2) = DecomposeNonConvexQuadraticForm(Q);
    EXPECT_TRUE(CompareMatrices(Q1, Eigen::Matrix3d::Zero(), 1E-5, MatrixCompareType::absolute));
...
GTEST_TEST(DecomposeNonConvexQuadraticForm, Test2) {
    ...
    EXPECT_TRUE(CompareMatrices(Q1, Q1_expected, 1E-5, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(Q2, Q2_expected, 1E-5, MatrixCompareType::absolute));
...
void CheckRelaxNonConvexQuadraticConstraintInTrustRegion(MathematicalProgram* prog,
    ...
    const auto result = AddRelaxNonConvexQuadraticConstraintInTrustRegion(
            prog, x, Q1, Q2, y, p, lower_bound, upper_bound, linearization_point, trust_region_gap);
    Binding<LinearConstraint> linear_constraint = std::get<0>(result);
    EXPECT_EQ(std::get<1>(result).size(), 2);
    Binding<RotatedLorentzConeConstraint> rotated_lorentz_cone_constraint1 = std::get<1>(result)[0];
    Binding<RotatedLorentzConeConstraint> rotated_lorentz_cone_constraint2 = std::get<1>(result)[1];
    VectorDecisionVariable<2> z = std::get<2>(result);
    Vector3<symbolic::Expression> linear_constraint_expr =
            linear_constraint.evaluator()->GetDenseA() * linear_constraint.variables();
    Vector3<symbolic::Expression> linear_constraint_expr_expected;
    linear_constraint_expr_expected << z(0) - z(1) + p.dot(y), z(0) - 2 * x0.dot(Q1 * x), z(1) - 2 * x0.dot(Q2 * x);
    const double poly_tol{1E-10};
    for (int i = 0; i < 3; ++i) {
        EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(linear_constraint_expr(i)),
                                    symbolic::Polynomial(linear_constraint_expr_expected(i)), poly_tol) ||
                    PolynomialEqual(symbolic::Polynomial(linear_constraint_expr(i)),
                                    symbolic::Polynomial(-linear_constraint_expr_expected(i)), poly_tol));
    }
    const VectorX<symbolic::Expression> y_lorentz1 =
            rotated_lorentz_cone_constraint1.evaluator()->A() * rotated_lorentz_cone_constraint1.variables() +
            rotated_lorentz_cone_constraint1.evaluator()->b();
    EXPECT_TRUE(
            PolynomialEqual(symbolic::Polynomial(y_lorentz1(0) * y_lorentz1(1)), symbolic::Polynomial(z(0)), poly_tol));
    EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(y_lorentz1.tail(y_lorentz1.rows() - 2).squaredNorm()),
                                symbolic::Polynomial(x.dot(Q1 * x)), poly_tol));

    const VectorX<symbolic::Expression> y_lorentz2 =
            rotated_lorentz_cone_constraint2.evaluator()->A() * rotated_lorentz_cone_constraint2.variables() +
            rotated_lorentz_cone_constraint2.evaluator()->b();
    EXPECT_TRUE(
            PolynomialEqual(symbolic::Polynomial(y_lorentz2(0) * y_lorentz2(1)), symbolic::Polynomial(z(1)), poly_tol));
    EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(y_lorentz2.tail(y_lorentz2.rows() - 2).squaredNorm()),
...
class TestRelaxNonConvexQuadraticConstraintInTrustRegion : public ::testing::Test {
public:
    TestRelaxNonConvexQuadraticConstraintInTrustRegion() : prog_{}, x_{prog_.NewContinuousVariables<2>()} {}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, Test0) {
    ...
    CheckRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, p, x_, lower_bound, upper_bound,
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, Test1) {
    ...
    CheckRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, p_prime, y, lower_bound, upper_bound,
                                                        linearization_point, trust_region_gap);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, TestRuntimeError) {
    ...
    EXPECT_THROW(
            AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, non_positive_Q, positive_Q, x_,
                                                              Eigen::Vector2d(1, 0), -1, 0.1, Eigen::Vector2d(1, 2), 1),
            std::runtime_error);
    // Q2 not being positive semidefinite.
    EXPECT_THROW(
            AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, positive_Q, non_positive_Q, x_,
                                                              Eigen::Vector2d(1, 0), -1, 0.1, Eigen::Vector2d(1, 2), 1),
            std::runtime_error);
    // Negative trust region gap.
    EXPECT_THROW(AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, positive_Q, positive_Q, x_,
                                                                   Eigen::Vector2d(1, 0), -1, 0.1,
                                                                   Eigen::Vector2d(1, 2), -0.1),
                 std::runtime_error);
}

void SolveRelaxNonConvexQuadraticConstraintInTrustRegion(MathematicalProgram* prog,
                                                         const Eigen::Ref<const VectorXDecisionVariable>& x,
                                                         const Eigen::Ref<const Eigen::MatrixXd>& Q1,
                                                         const Eigen::Ref<const Eigen::MatrixXd>& Q2,
                                                         const Eigen::Ref<const VectorXDecisionVariable>& y,
    ...
    const auto relaxed_constraints = AddRelaxNonConvexQuadraticConstraintInTrustRegion(
            prog, x, Q1, Q2, y, p, lower_bound, upper_bound, x0, trust_region_gap);
    ...
    for (int i = 0; i < c.cols(); ++i) {
        ...
        EXPECT_LE(z_sol(0), 2 * x0.dot(Q1 * x_sol) - x0.dot(Q1 * x0) + trust_region_gap + check_tol);
        EXPECT_LE(z_sol(1), 2 * x0.dot(Q2 * x_sol) - x0.dot(Q2 * x0) + trust_region_gap + check_tol);
        EXPECT_GE(z_sol(0), x_sol.dot(Q1 * x_sol) - check_tol);
        EXPECT_GE(z_sol(1), x_sol.dot(Q2 * x_sol) - check_tol);
        const double original_constraint_value{x_sol.dot((Q1 - Q2) * x_sol) + p.dot(y_sol)};
        EXPECT_LE(original_constraint_value, upper_bound + trust_region_gap + check_tol);
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, SolveProblem0) {
    ...
    SolveRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, x_, Eigen::Vector2d::Zero(), lb, ub,
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, SolveProblem1) {
    ...
    const double ub{2};
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, x_, Eigen::Vector2d(1, 3), lb, ub,
                                                        Eigen::Vector2d(2, 1), trust_region_gap, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, SolveProblem2) {
    ...
    SolveRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, y, Eigen::Vector3d(1, 2, 3), lb, ub,
                                                        Eigen::Vector2d(2, 1), trust_region_gap, c);
}

TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, SolveInfeasibleProblem0) {
    ...
    AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Q2, x_, Eigen::Vector2d::Zero(), 1, 1,
                                                      Eigen::Vector2d(1, 0), 0.1);

    auto result = Solve(prog_).get_solution_result();
    EXPECT_TRUE(result == SolutionResult::kInfeasibleOrUnbounded || result == SolutionResult::kInfeasibleConstraints);
}

GTEST_TEST(TestRelaxNonConvexQuadraticConstraintInTrustRegionInfeasible, SolveInfeasibleProblem1) {
    ...
    const auto relaxed_constraint = AddRelaxNonConvexQuadraticConstraintInTrustRegion(
            &prog1, x1, Q1, Q2, x1, Eigen::Vector2d::Zero(), 1, 1, Eigen::Vector2d(-5, -3), 0.1);

    auto result = Solve(prog1).get_solution_result();
    EXPECT_TRUE(result == SolutionResult::kInfeasibleConstraints || result == SolutionResult::kInfeasibleOrUnbounded);
    ...
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegion(&prog2, x2, Q1, Q2, x2, Eigen::Vector2d::Zero(), 1, 1,
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ1Q2) {
    ...
    EXPECT_THROW(AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Eigen::Matrix2d::Zero(),
                                                                   Eigen::Matrix2d::Zero(), x_, Eigen::Vector2d(1, 2),
                                                                   1, 2, Eigen::Vector2d(2, 1), 1),
                 std::runtime_error);
    // Q1 is zero and upper_bound is infinity.
    EXPECT_THROW(AddRelaxNonConvexQuadraticConstraintInTrustRegion(
                         &prog_, x_, Eigen::Matrix2d::Zero(), Eigen::Matrix2d::Identity(), x_, Eigen::Vector2d(1, 2), 1,
                         std::numeric_limits<double>::infinity(), Eigen::Vector2d(2, 1), 1),
                 std::runtime_error);
    // Q2 is zero and lower_bound is -infinity.
    EXPECT_THROW(AddRelaxNonConvexQuadraticConstraintInTrustRegion(
                         &prog_, x_, Eigen::Matrix2d::Identity(), Eigen::Matrix2d::Zero(), x_, Eigen::Vector2d(1, 2),
                         -std::numeric_limits<double>::infinity(), 1, Eigen::Vector2d(2, 1), 1),
                 std::runtime_error);
    // lower_bound is larger than upper_bound
    EXPECT_THROW(AddRelaxNonConvexQuadraticConstraintInTrustRegion(
                         &prog_, x_, Eigen::Matrix2d::Identity(), 0.1 * Eigen::Matrix2d::Identity(), x_,
                         Eigen::Vector2d(1, 2), 2, 1, Eigen::Vector2d(2, 1), 1),
                 std::runtime_error);
}
...
void SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(
    ...
    const auto relaxed_constraints =
            AddRelaxNonConvexQuadraticConstraintInTrustRegion(prog, x, Q1, Q2, y, p, lb, ub, x0, trust_region_gap);
    const Binding<LinearConstraint> linear_constraint = std::get<0>(relaxed_constraints);
    EXPECT_EQ(std::get<1>(relaxed_constraints).size(), 1);
    const Binding<RotatedLorentzConeConstraint> lorentz_cone1 = std::get<1>(relaxed_constraints)[0];
    ...
    linear_expr = linear_constraint.evaluator()->GetDenseA() * linear_constraint.variables();
    ...
    EXPECT_TRUE(CompareMatrices(linear_constraint.evaluator()->lower_bound(), linear_lb_expected, 1E-15,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(linear_constraint.evaluator()->upper_bound(), linear_ub_expected, 1E-15,
                                MatrixCompareType::absolute));
    const double poly_tol{1E-10};
    for (int i = 0; i < 2; ++i) {
        EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(linear_expr(i)), symbolic::Polynomial(linear_expr_expected(i)),
                                    poly_tol));
    }
    VectorX<symbolic::Expression> y1 =
            lorentz_cone1.evaluator()->A() * lorentz_cone1.variables() + lorentz_cone1.evaluator()->b();
    EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(y1(0) * y1(1)), symbolic::Polynomial(z(0)), poly_tol));
    EXPECT_TRUE(PolynomialEqual(symbolic::Polynomial(y1.tail(y1.rows() - 2).squaredNorm()),
    ...
    for (int i = 0; i < c.cols(); ++i) {
        ...
        EXPECT_LE(z_sol, 2 * x0.dot(Q * x_sol) - x0.dot(Q * x0) + trust_region_gap + tol);
        EXPECT_GE(z_sol, x_sol.dot(Q * x_sol) - tol);
        const double original_constraint_val{x_sol.dot(Q_sign * Q * x_sol) + p.dot(y_sol)};
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ1Test0) {
    ...
    Q << 1, 0.5, 0.5, 1;
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, x_, Eigen::Vector2d(2, 0),
                                                                      -std::numeric_limits<double>::infinity(), 1,
                                                                      Eigen::Vector2d(1, 2), 1, true, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ1Test1) {
    ...
    Q << 1, 1, 1, 4;
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, x_, Eigen::Vector2d(3, 2), -1, 4,
                                                                      Eigen::Vector2d(1, 0), 1, true, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ1Test2) {
    ...
    AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Eigen::Matrix2d::Zero(), Q2, x_,
                                                      Eigen::Vector2d::Zero(), -4, -1, Eigen::Vector2d(0, 1), 1);
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ1Test3) {
    ...
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, y, Eigen::Vector3d(2, 3, 2), -1, 4,
                                                                      Eigen::Vector2d(1, 0), 1, true, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ2Test0) {
    ...
    Q << 1, 0.5, 0.5, 1;
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, x_, Eigen::Vector2d(1, 0), 1, 4,
                                                                      Eigen::Vector2d(1, -1), 0.5, false, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ2Test1) {
    ...
    Q << 1, 0.5, 0.5, 3;
    const auto c = GenerateCostDirection();
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, x_, Eigen::Vector2d(2, 4), 1, 4,
                                                                      Eigen::Vector2d(1, -1), 0.5, false, c);
}
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ2Test2) {
    ...
    AddRelaxNonConvexQuadraticConstraintInTrustRegion(&prog_, x_, Q1, Eigen::Matrix2d::Zero(), x_,
                                                      Eigen::Vector2d::Zero(), 1, 4, Eigen::Vector2d(0, 1), 1);
...
TEST_F(TestRelaxNonConvexQuadraticConstraintInTrustRegion, ZeroQ2Test3) {
    ...
    SolveRelaxNonConvexQuadraticConstraintInTrustRegionWithZeroQ1orQ2(&prog_, x_, Q, y, Eigen::Vector3d(2, 4, 3), 1, 4,
                                                                      Eigen::Vector2d(1, -1), 0.5, false, c);
}
}  // namespace
}  // namespace solvers
