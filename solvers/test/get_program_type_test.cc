...
GTEST_TEST(GetProgramTypeTest, QP) {
    ...
    auto nonconvex_cost = prog.AddQuadraticCost(-x[1] * x[1], false /* non-convex */);
    EXPECT_NE(GetProgramType(prog), ProgramType::kQP);
    prog.RemoveCost(nonconvex_cost);
    EXPECT_EQ(GetProgramType(prog), ProgramType::kQP);

    prog.AddLorentzConeConstraint(Vector3<symbolic::Expression>(x[0] + 2, 2 * x[0] + 1, x[0] + x[1]));
    EXPECT_NE(GetProgramType(prog), ProgramType::kQP);
}
...
GTEST_TEST(GetProgramTypeTest, SOCP) {
    ...
    EXPECT_EQ(GetProgramType(prog), ProgramType::kSOCP);
    prog.AddPositiveSemidefiniteConstraint(x[0] * Eigen::Matrix2d::Identity() + x[1] * Eigen::Matrix2d::Ones());
    EXPECT_NE(GetProgramType(prog), ProgramType::kSOCP);
}
...
GTEST_TEST(GetProgramTypeTest, SDP) {
    ...
    prog.AddLorentzConeConstraint(Vector3<symbolic::Expression>(X(0, 0) + 1, X(1, 1), X(1, 2) + 2));
    EXPECT_EQ(GetProgramType(prog), ProgramType::kSDP);
    auto x = prog.NewContinuousVariables<2>();
...
GTEST_TEST(GetProgramTypeTest, NLP) {
    {
        MathematicalProgram prog;
        auto x = prog.NewContinuousVariables<2>();
        auto quadratic_cost = prog.AddQuadraticCost(x(0) * x(0) - x(1) * x(1), false);
    ...
    }
    {
        ...
        auto x = prog.NewContinuousVariables<2>();
        prog.AddConstraint(
                std::make_shared<QuadraticConstraint>(Eigen::Matrix2d::Identity(), Eigen::Vector2d(1, 0), 0, 1), x);
        EXPECT_EQ(GetProgramType(prog), ProgramType::kNLP);
    ...
    }
    {
        ...
        auto x = prog.NewContinuousVariables<2>();
        prog.AddConstraint(
                std::make_shared<LinearComplementarityConstraint>(Eigen::Matrix2d::Identity(), Eigen::Vector2d(1, 1)),
                x);
        prog.AddLinearCost(x(0) + x(1));
        EXPECT_EQ(GetProgramType(prog), ProgramType::kNLP);
    }
    {
        ...
        MathematicalProgram prog;
        auto x = prog.NewContinuousVariables<3>();
        prog.AddConstraint(std::make_shared<LinearComplementarityConstraint>(Eigen::Matrix3d::Identity(),
...
GTEST_TEST(GetProgramTypeTest, LCP) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    prog.AddLinearComplementarityConstraint(Eigen::Matrix2d::Identity(), Eigen::Vector2d(1, 2), x);
...
GTEST_TEST(GetProgramTypeTest, MISOCP) {
    ...
    prog.AddPositiveSemidefiniteConstraint(x[0] * Eigen::Matrix2d::Identity() + b[0] * Eigen::Matrix2d::Ones());
...
GTEST_TEST(GetProgramTypeTest, Unknown) {
    ...
    prog.AddLinearComplementarityConstraint(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Ones(), x);
