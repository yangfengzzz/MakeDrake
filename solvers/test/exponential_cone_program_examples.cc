...
void ExponentialConeTrivialExample(const SolverInterface& solver, double tol, bool check_dual) {
    ...
    auto exp_constr = prog.AddExponentialConeConstraint(Vector3<symbolic::Expression>(x, y, z));
    ...
    if (check_dual) {
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(exp_constr), Eigen::Vector3d(1, -1, -1), tol));
        EXPECT_TRUE(CompareMatrices(result.GetDualSolution(lin_eq_constr), Vector1d(1), tol));
    }
}
...
void MinimizeKLDivergence(const SolverInterface& solver, double tol) {
    ...
    for (int i = 0; i < 4; ++i) {
        prog.AddExponentialConeConstraint(Vector3<symbolic::Expression>(q(i), p(i), -t(i)));
    }
    ...
    EXPECT_TRUE(CompareMatrices(result.GetSolution(t), Eigen::Vector4d::Zero(), tol));
}
...
void MinimalEllipsoidCoveringPoints(const SolverInterface& solver, double tol) {
    ...
    ellipsoid_psd << S, b.cast<symbolic::Expression>() / 2, b.cast<symbolic::Expression>().transpose() / 2, c;
    prog.AddPositiveSemidefiniteConstraint(ellipsoid_psd);
    const auto [linear_cost, log_det_t, log_det_Z] = prog.AddMaximizeLogDeterminantCost(S.cast<symbolic::Expression>());
    for (int i = 0; i < 4; ++i) {
        prog.AddLinearConstraint(pts.col(i).dot(S.cast<symbolic::Expression>() * pts.col(i)) +
    ...
    for (int i = 0; i < 4; ++i) {
        EXPECT_NEAR(pts.col(i).dot(S_sol * pts.col(i)) + b_sol.dot(pts.col(i)) + c_sol, 1, tol);
    }
    ...
    const double expected_cost = -std::log(0.25 / std::pow(scaling_factor(0) * scaling_factor(1), 2));
    ...
    Eigen::MatrixXd log_det_Z_sol = ExtractDoubleOrThrow(result.GetSolution(log_det_Z));
    EXPECT_TRUE((log_det_Z_sol.diagonal().array().log() >= log_det_t_sol.array() - tol).all());
    EXPECT_TRUE(
            CompareMatrices(Eigen::TriangularView<Eigen::MatrixXd, Eigen::StrictlyUpper>(log_det_Z_sol).toDenseMatrix(),
...
void MatrixLogDeterminantLower(const SolverInterface& solver, double tol) {
    ...
    const auto ret = prog.AddLogDeterminantLowerBoundConstraint(X.cast<symbolic::Expression>(), lower);
    if (solver.available()) {
        MathematicalProgramResult result;
        ...
        prog.AddBoundingBoxConstraint(0.01 * Eigen::Matrix3d::Identity(), 0.01 * Eigen::Matrix3d::Identity(), X);
        solver.Solve(prog, std::nullopt, std::nullopt, &result);
        EXPECT_TRUE(result.is_success());
    }
}
