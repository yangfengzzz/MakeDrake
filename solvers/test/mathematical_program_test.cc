...
#include "common/test_utilities/is_dynamic_castable.h"
#include "common/test_utilities/symbolic_test_util.h"
#include "math/matrix_util.h"
#include "solvers/constraint.h"
#include "solvers/decision_variable.h"
#include "solvers/program_attribute.h"
#include "solvers/snopt_solver.h"
#include "solvers/solve.h"
#include "solvers/test/generic_trivial_constraints.h"
#include "solvers/test/generic_trivial_costs.h"
#include "solvers/test/mathematical_program_test_util.h"
...
void CheckAddedIndeterminates(const MathematicalProgram& prog,
    ...
    for (int i = 0; i < indeterminates.rows(); ++i) {
        for (int j = 0; j < indeterminates.cols(); ++j) {
            EXPECT_EQ(prog.FindIndeterminateIndex(indeterminates(i, j)), j * indeterminates.rows() + i);
        }
    }
    ...
    for (int i = 0; i < indeterminates.rows(); ++i) {
        for (int j = 0; j < indeterminates.cols(); ++j) {
            EXPECT_EQ(indeterminates(i, j).get_type(), MathematicalProgram::VarType::CONTINUOUS);
        }
    }
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariables1) {
    ...
    CheckAddedVariable<MatrixXDecisionVariable>(prog, X, 2, 3, "X(0,0) X(0,1) X(0,2)\nX(1,0) X(1,1) X(1,2)", false,
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariable2) {
    ...
    MathematicalProgram prog;
    auto X = prog.NewContinuousVariables<2, 3>("X");
    CheckAddedVariable<MatrixDecisionVariable<2, 3>>(prog, X, 2, 3, "X(0,0) X(0,1) X(0,2)\nX(1,0) X(1,1) X(1,2)", false,
                                                     MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariable3) {
    ...
    CheckAddedVariable<VectorXDecisionVariable>(prog, x, 4, 1, "x(0)\nx(1)\nx(2)\nx(3)", false,
                                                MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariable4) {
    ...
    CheckAddedVariable<VectorDecisionVariable<4>>(prog, x, 4, 1, "y(0)\ny(1)\ny(2)\ny(3)", false,
                                                  MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariable5) {
    ...
    CheckAddedVariable<MatrixDecisionVariable<2, 3>>(prog, X, 2, 3, "Y(0,0) Y(0,1) Y(0,2)\nY(1,0) Y(1,1) Y(1,2)", false,
                                                     MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariables6) {
    // Adds a dynamic-sized matrix of continuous variables.
    MathematicalProgram prog;
    auto X = prog.NewContinuousVariables<Eigen::Dynamic, Eigen::Dynamic>(2, 3, "Y");
    CheckAddedVariable<MatrixXDecisionVariable>(prog, X, 2, 3, "Y(0,0) Y(0,1) Y(0,2)\nY(1,0) Y(1,1) Y(1,2)", false,
                                                MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariables7) {
    ...
    CheckAddedVariable<MatrixDecisionVariable<2, Eigen::Dynamic>>(prog, X, 2, 3,
                                                                  "Y(0,0) Y(0,1) Y(0,2)\nY(1,0) Y(1,1) Y(1,2)", false,
                                                                  MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariables8) {
    ...
    CheckAddedVariable<MatrixDecisionVariable<Eigen::Dynamic, 3>>(prog, X, 2, 3,
                                                                  "Y(0,0) Y(0,1) Y(0,2)\nY(1,0) Y(1,1) Y(1,2)", false,
                                                                  MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddContinuousVariables9) {
    ...
    CheckAddedVariable<MatrixXDecisionVariable>(prog1, X1, 2, 3, X_names, false,
                                                MathematicalProgram::VarType::CONTINUOUS);

    MathematicalProgram prog2;
    auto X2 = prog2.NewContinuousVariables<Eigen::Dynamic, 3>(2, 3);
    CheckAddedVariable<MatrixDecisionVariable<Eigen::Dynamic, 3>>(prog2, X2, 2, 3, X_names, false,
                                                                  MathematicalProgram::VarType::CONTINUOUS);

    MathematicalProgram prog3;
    auto X3 = prog3.NewContinuousVariables<2, 3>(2, 3);
    CheckAddedVariable<MatrixDecisionVariable<2, 3>>(prog3, X3, 2, 3, X_names, false,
                                                     MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddSymmetricVariable1) {
    ...
    CheckAddedVariable<MatrixDecisionVariable<3, 3>>(prog, X, 3, 3,
                                                     "X(0,0) X(1,0) X(2,0)\nX(1,0) X(1,1) X(2,1)\nX(2,0) X(2,1) X(2,2)",
...
GTEST_TEST(TestAddVariable, TestAddSymmetricVariable2) {
    ...
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables(3, "X");
    CheckAddedVariable<MatrixXDecisionVariable>(prog, X, 3, 3,
                                                "X(0,0) X(1,0) X(2,0)\nX(1,0) X(1,1) X(2,1)\nX(2,0) X(2,1) X(2,2)",
                                                true, MathematicalProgram::VarType::CONTINUOUS);
}
...
GTEST_TEST(TestAddVariable, TestAddBinaryVariable1) {
    ...
    CheckAddedVariable<MatrixXDecisionVariable>(prog, X, 2, 3, "B(0,0) B(0,1) B(0,2)\nB(1,0) B(1,1) B(1,2)", false,
...
GTEST_TEST(TestAddVariable, TestAddBinaryVariable3) {
    ...
    CheckAddedVariable<VectorXDecisionVariable>(prog, X, 4, 1, "B(0)\nB(1)\nB(2)\nB(3)", false,
                                                MathematicalProgram::VarType::BINARY);
}
...
GTEST_TEST(TestAddVariable, TestAddBinaryVariable4) {
    ...
    MathematicalProgram prog;
    auto X = prog.NewBinaryVariables<4>("B");
    CheckAddedVariable<VectorDecisionVariable<4>>(prog, X, 4, 1, "B(0)\nB(1)\nB(2)\nB(3)", false,
                                                  MathematicalProgram::VarType::BINARY);
}
...
GTEST_TEST(TestAddVariable, TestAddBinaryVariable5) {
    ...
    MathematicalProgram prog;
    auto X = prog.NewBinaryVariables<2, 3>("B");
    CheckAddedVariable<MatrixDecisionVariable<2, 3>>(prog, X, 2, 3, "B(0,0) B(0,1) B(0,2)\nB(1,0) B(1,1) B(1,2)", false,
                                                     MathematicalProgram::VarType::BINARY);
}
...
GTEST_TEST(TestAddVariable, TestAddBinaryVariable7) {
    ...
    CheckAddedVariable<MatrixDecisionVariable<2, Eigen::Dynamic>>(
            prog, X, 2, 3, "B(0,0) B(0,1) B(0,2)\nB(1,0) B(1,1) B(1,2)", false, MathematicalProgram::VarType::BINARY);
}
...
GTEST_TEST(TestAddDecisionVariables, AddDecisionVariables1) {
    ...
    EXPECT_TRUE(prog.required_capabilities().contains(ProgramAttribute::kBinaryVariable));
...
GTEST_TEST(TestAddDecisionVariables, AddVariableError) {
    ...
    const Variable x0("x0", Variable::Type::CONTINUOUS);
    EXPECT_THROW(prog.AddDecisionVariables(VectorDecisionVariable<2>(x0, z(0))), std::runtime_error);

    // Call AddDecisionVariables with unsupported variable type.
    for (symbolic::Variable::Type unsupported_type :
         {symbolic::Variable::Type::BOOLEAN, symbolic::Variable::Type::RANDOM_UNIFORM,
          symbolic::Variable::Type::RANDOM_GAUSSIAN, symbolic::Variable::Type::RANDOM_EXPONENTIAL}) {
        const symbolic::Variable unsupported_var("b", unsupported_type);
        DRAKE_EXPECT_THROWS_MESSAGE(prog.AddDecisionVariables(VectorDecisionVariable<1>(unsupported_var)),
...
GTEST_TEST(NewIndeterminates, DynamicSizeMatrix) {
    ...
    static_assert(is_same_v<decltype(X), MatrixXIndeterminate>, "should be a dynamic sized matrix");
    EXPECT_EQ(X.rows(), 2);
    EXPECT_EQ(X.cols(), 3);
    CheckAddedIndeterminates(prog, X, "X(0,0) X(0,1) X(0,2)\nX(1,0) X(1,1) X(1,2)");
}
...
GTEST_TEST(NewIndeterminates, StaticSizeMatrix) {
    ...
    MathematicalProgram prog;
    auto X = prog.NewIndeterminates<2, 3>("X");
    static_assert(is_same_v<decltype(X), MatrixIndeterminate<2, 3>>, "should be a static sized matrix");
    CheckAddedIndeterminates(prog, X, "X(0,0) X(0,1) X(0,2)\nX(1,0) X(1,1) X(1,2)");
}
...
GTEST_TEST(NewIndeterminates, DynamicSizeVector) {
    ...
    static_assert(is_same_v<decltype(x), VectorXIndeterminate>, "Should be a VectorXDecisionVariable object.");
    EXPECT_EQ(x.rows(), 4);
...
GTEST_TEST(NewIndeterminates, StaticSizeVector) {
    ...
    MathematicalProgram prog;
    auto x = prog.NewIndeterminates<4>("x");
    static_assert(is_same_v<decltype(x), VectorIndeterminate<4>>, "Should be a VectorXDecisionVariable object.");
    CheckAddedIndeterminates(prog, x, "x(0)\nx(1)\nx(2)\nx(3)");
}
...
GTEST_TEST(TestAddIndeterminate, AddIndeterminateError) {
    ...
    // variables.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminate(x(0)), ".*is a decision variable.*");
    // Call AddIndeterminate with an input of type BINARY.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminate(z), ".*should be of type CONTINUOUS.*");
}
...
GTEST_TEST(TestAddIndeterminates, AddIndeterminatesVec1) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        const auto it = indeterminates_index.find(indeterminates_expected(i).get_id());
        ASSERT_TRUE(it != indeterminates_index.end());
        EXPECT_EQ(it->second, prog.FindIndeterminateIndex(indeterminates_expected(i)));
    }
}
...
GTEST_TEST(TestAddIndeterminates, AddIndeterminatesVecError) {
    ...
    // variables.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminates(VectorIndeterminate<2>(x(0), x0)), ".*is a decision variable.*");
    // Call AddIndeterminates with an input of type BINARY.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminates(VectorIndeterminate<2>(x1, x0)),
...
GTEST_TEST(TestAddIndeterminates, AddIndeterminatesVarsError) {
    ...
    // Call AddIndeterminates with an input that intersects with old decision
    // variables.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminates(symbolic::Variables({x0, x(0)})), ".*is a decision variable.*");
    // Call AddIndeterminates with an input of type BINARY.
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddIndeterminates(symbolic::Variables({x0, x1})),
                                ".*should be of type CONTINUOUS.*");
}
...
void ExpectBadVar(MathematicalProgram* prog, int num_var, Args&&... args) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AddItem(prog, CreateBinding(c, x)), ".*is not a decision variable.*");
}

// Returns True if all the elements of the matrix m1 and m2 are EqualTo as
// Expressions.
bool MatrixExprAllEqual(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    return m1
            .binaryExpr(m2,
...
GTEST_TEST(TestMathematicalProgram, TestBadBindingVariable) {
    ...
    ExpectBadVar<PositiveSemidefiniteConstraint>(&prog, num_var * num_var, num_var);
    ExpectBadVar<LinearMatrixInequalityConstraint>(&prog, F.size() - 1, F);
    ExpectBadVar<ExponentialConeConstraint>(&prog, num_var, Eigen::MatrixXd::Ones(3, num_var).sparseView(),
                                            Eigen::Vector3d::Zero());
...
GTEST_TEST(TestMathematicalProgram, BoundingBoxTest2) {
    ...
    auto constraint2 = prog.AddBoundingBoxConstraint(0, 1, {x1.col(0), x1.col(1)}).evaluator();
    ...
    auto constraint6 = prog.AddBoundingBoxConstraint(Eigen::Vector4d::Zero(), Eigen::Vector4d::Ones(), x3).evaluator();
    ...
    EXPECT_TRUE(CompareMatrices(constraint1->lower_bound(), constraint2->lower_bound()));
    EXPECT_TRUE(CompareMatrices(constraint2->lower_bound(), constraint3->lower_bound()));
    EXPECT_TRUE(CompareMatrices(constraint3->lower_bound(), constraint4->lower_bound()));
    EXPECT_TRUE(CompareMatrices(constraint4->lower_bound(), constraint5->lower_bound()));
    EXPECT_TRUE(CompareMatrices(constraint5->lower_bound(), constraint6->lower_bound()));

    EXPECT_TRUE(CompareMatrices(constraint1->upper_bound(), constraint2->upper_bound()));
    EXPECT_TRUE(CompareMatrices(constraint2->upper_bound(), constraint3->upper_bound()));
    EXPECT_TRUE(CompareMatrices(constraint3->upper_bound(), constraint4->upper_bound()));
    EXPECT_TRUE(CompareMatrices(constraint4->upper_bound(), constraint5->upper_bound()));
    EXPECT_TRUE(CompareMatrices(constraint5->upper_bound(), constraint6->upper_bound()));
}
...
GTEST_TEST(TestMathematicalProgram, BoundingBoxTest3) {
    ...
    // of matrix.
    Eigen::Matrix<double, 3, 2, Eigen::RowMajor> X_up = (X_lo.array() + 1).matrix();
    ...
    for (int i = 0; i < 6; ++i) {
        EXPECT_EQ(cnstr.evaluator()->lower_bound()(i), X_bounds.at(cnstr.variables()(i)).first);
        EXPECT_EQ(cnstr.evaluator()->upper_bound()(i), X_bounds.at(cnstr.variables()(i)).second);
    }

    // Now add constraint on X.topRows<2>(). It doesn't occupy contiguous memory.
    auto cnstr2 = prog.AddBoundingBoxConstraint(X_lo.topRows<2>(), X_up.topRows<2>(), X.topRows<2>());
...
void VerifyAddedCost1(const MathematicalProgram& prog,
    ...
    EXPECT_EQ(static_cast<int>(prog.generic_costs().size()), num_generic_costs_expected);
...
GTEST_TEST(TestMathematicalProgram, AddCostTest) {
    ...
    prog.AddCost(Binding<Cost>(generic_trivial_cost1, VectorDecisionVariable<3>(x(0), x(1), y(1))));
    ++num_generic_costs;
    VerifyAddedCost1(prog, generic_trivial_cost1, Eigen::Vector3d(1, 3, 5), num_generic_costs);

    // Adds a std::shared_ptr<Constraint> on a VectorDecisionVariable object.
    prog.AddCost(generic_trivial_cost1, VectorDecisionVariable<3>(x(0), x(1), y(1)));
    ++num_generic_costs;
    VerifyAddedCost1(prog, generic_trivial_cost1, Eigen::Vector3d(1, 2, 3), num_generic_costs);

    // Adds a std::shared_ptr<Constraint> on a VariableRefList object.
    prog.AddCost(generic_trivial_cost1, {x, y.tail<1>()});
    ++num_generic_costs;
    VerifyAddedCost1(prog, generic_trivial_cost1, Eigen::Vector3d(2, 3, 4), num_generic_costs);
    ...
    auto returned_cost3 = prog.AddCost(generic_trivial_cost2, VectorDecisionVariable<2>(x(0), y(1))).evaluator();
    ++num_generic_costs;
    VerifyAddedCost2(prog, generic_trivial_cost2, returned_cost3, Eigen::Vector2d(1, 2), num_generic_costs);

    // Add an object that can be converted to a ConstraintImpl object on a
    // VariableRefList object.
    auto returned_cost4 = prog.AddCost(generic_trivial_cost2, {x.head<1>(), y.tail<1>()}).evaluator();
    ++num_generic_costs;
    VerifyAddedCost2(prog, generic_trivial_cost2, returned_cost4, Eigen::Vector2d(1, 2), num_generic_costs);
}

class EmptyConstraint final : public Constraint {
public:
    EmptyConstraint() : Constraint(0, 2, Eigen::VectorXd(0), Eigen::VectorXd(0), "empty_constraint") {}
...
private:
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const {}
};
...
GTEST_TEST(TestMathematicalProgram, AddEmptyConstraint) {
    ...
    auto binding2 = prog.AddConstraint(
            std::make_shared<LinearConstraint>(Eigen::MatrixXd(0, 2), Eigen::VectorXd(0), Eigen::VectorXd(0)), x);
    EXPECT_TRUE(prog.GetAllConstraints().empty());
    EXPECT_EQ(binding2.evaluator()->num_constraints(), 0);

    auto binding3 = prog.AddConstraint(
            std::make_shared<LinearEqualityConstraint>(Eigen::MatrixXd(0, 2), Eigen::VectorXd(0)), x);
    EXPECT_TRUE(prog.GetAllConstraints().empty());
    EXPECT_EQ(binding3.evaluator()->num_constraints(), 0);

    auto binding4 = prog.AddConstraint(std::make_shared<BoundingBoxConstraint>(Eigen::VectorXd(0), Eigen::VectorXd(0)),
                                       VectorX<symbolic::Variable>(0));
    EXPECT_TRUE(prog.GetAllConstraints().empty());
    EXPECT_EQ(binding4.evaluator()->num_constraints(), 0);

    auto binding5 = prog.AddConstraint(
            std::make_shared<LinearComplementarityConstraint>(Eigen::MatrixXd(0, 0), Eigen::VectorXd(0)),
            VectorX<symbolic::Variable>(0));
    EXPECT_TRUE(prog.GetAllConstraints().empty());

    auto binding6 = prog.AddConstraint(
            internal::CreateBinding(std::make_shared<PositiveSemidefiniteConstraint>(0), VectorXDecisionVariable(0)));
    EXPECT_TRUE(prog.GetAllConstraints().empty());

    auto binding7 = prog.AddConstraint(std::make_shared<LinearMatrixInequalityConstraint>(std::vector<Eigen::MatrixXd>(
                                               {Eigen::MatrixXd(0, 0), Eigen::MatrixXd(0, 0), Eigen::MatrixXd(0, 0)})),
                                       x);
    EXPECT_TRUE(prog.GetAllConstraints().empty());
}
...
void CheckAddedSymbolicLinearCostUserFun(const MathematicalProgram& prog,
    ...
    EXPECT_TRUE(CheckStructuralEquality(prog.linear_costs().back().variables(), binding.variables()));
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintMatrixVectorVariablesTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(4, "x");
    Matrix4d A;
    // clang-format off
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintMatrixVariableRefListTest) {
    MathematicalProgram prog;
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSparseMatrixVectorVariablesTest) {
    MathematicalProgram prog;
    const int n = 4;
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSparseMatrixVariableRefListTest) {
    MathematicalProgram prog;
    const int n = 4;
    auto x = prog.NewContinuousVariables(1, "x");
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintRowVectorVectorVariablesTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(4, "x");
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintRowVectorVariableRefListTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(1, "x");
    auto y = prog.NewContinuousVariables(3, "y");
    VariableRefList vars{x, y};
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolic4) {
    ...
    EXPECT_EQ(prog.bounding_box_constraints().back().evaluator(), binding.evaluator());
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolic5) {
    ...
    EXPECT_EQ(prog.bounding_box_constraints().back().variables(), binding.variables());
    EXPECT_EQ(binding.variables(), VectorDecisionVariable<1>(x(1)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(), Vector1d(-2)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Vector1d(-1)));
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolic6) {
    ...
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(), Vector1d(-3)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Vector1d(-1)));
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolic7) {
  ...
  e << -2 * x(0),
    ...
    -4 * x(1) + 3;
    // clang-format on
    prog.AddLinearConstraint(e, Vector4d(1, 3, 2, 3), Vector4d(3, 5, 4, numeric_limits<double>::infinity()));

    EXPECT_EQ(prog.bounding_box_constraints().size(), 1);
    const auto& binding = prog.bounding_box_constraints().back();
    EXPECT_EQ(binding.variables(), VectorDecisionVariable<4>(x(0), x(0), x(2), x(1)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(),
                                Vector4d(-1.5, 0.25, -0.5, -numeric_limits<double>::infinity())));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Vector4d(-0.5, 0.75, 0.5, 0)));
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolic9) {
    ...
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->GetDenseA(), Eigen::Vector2d(0, 1)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(), Eigen::Vector2d(-1, 2)));
    EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Eigen::Vector2d(1, 4)));
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormula1) {
    ...
    for (const auto& fi : f) {
        ...
        EXPECT_EQ(++num_bounding_box_constraint, prog.bounding_box_constraints().size());
        ...
        EXPECT_EQ(binding.variables(), VectorDecisionVariable<1>(x(0)));
        EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Vector1d(3)));
        EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(), Vector1d(-numeric_limits<double>::infinity())));
    }
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormula2) {
    ...
    for (const auto& fi : f) {
        ...
        EXPECT_EQ(binding.variables(), VectorDecisionVariable<1>(x(0)));
        EXPECT_TRUE(CompareMatrices(binding.evaluator()->lower_bound(), Vector1d(2)));
        EXPECT_TRUE(CompareMatrices(binding.evaluator()->upper_bound(), Vector1d(numeric_limits<double>::infinity())));
    }
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormula3) {
    ...
    for (const auto& fi : f) {
        prog.AddLinearConstraint(fi);
        EXPECT_EQ(++num_linear_equality_constraint, prog.linear_equality_constraints().size());
        EXPECT_EQ(prog.linear_constraints().size(), 0);
        ...
        VectorX<Expression> expr =
                binding.evaluator()->GetDenseA() * binding.variables() - binding.evaluator()->lower_bound();
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormula4) {
    ...
    for (const auto& fi : f) {
        ...
        const VectorX<Expression> expr =
                binding.evaluator()->upper_bound() - binding.evaluator()->GetDenseA() * binding.variables();
        EXPECT_EQ(expr.size(), 1);
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormula5) {
  ...
  const vector<Formula> f{x(0) <= inf,
    ...
    for (const auto& f_i : f) {
        ...
        const VectorX<Expression> expr = binding.evaluator()->GetDenseA() * binding.variables();
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicFormulaException2) {
    ...
    const Expression zero_after_expansion0 = pow(x(0) + 1, 2) - pow(x(0), 2) - 2 * x(0) - 1;
    // (x₁ + 1)² - x₁² -2x₁ -1 => 0 (by Expand()).
    const Expression zero_after_expansion1 = pow(x(1) + 1, 2) - pow(x(1), 2) - 2 * x(1) - 1;
    ...
    // -∞ <= +∞   -->  Exception.
    // -∞ <= -∞   -->  Exception.
    EXPECT_THROW(prog.AddLinearConstraint(zero_after_expansion0 + inf <= zero_after_expansion1 + -inf), runtime_error);
    EXPECT_THROW(prog.AddLinearConstraint(zero_after_expansion0 + inf <= zero_after_expansion1 + inf), runtime_error);
    EXPECT_THROW(prog.AddLinearConstraint(zero_after_expansion0 - inf <= zero_after_expansion1 + inf), runtime_error);
    EXPECT_THROW(prog.AddLinearConstraint(zero_after_expansion0 - inf <= zero_after_expansion1 - inf), runtime_error);
}
...
GTEST_TEST(TestMathematicalProgram, AddLinearConstraintSymbolicArrayFormula1) {
  ...
  M_f << (3 - 5 * x(0) + 10 * x(2) - 7 * y(1) >= 3),
    ...
    for (int i{0}; i < M_f.size(); ++i) {
        if (!std::isinf(lb_in_constraint(i))) {
            EXPECT_PRED2(ExprEqual, get_lhs_expression(M_f(i)) - get_rhs_expression(M_f(i)),
  ...
  M_lb << 3, -10,
    ...
    for (int j{0}; j < M_e.cols(); ++j) {
        for (int i{0}; i < M_e.rows(); ++i) {
            EXPECT_PRED2(ExprEqual, M_e(i, j) - M_lb(i, j), Ax(k) - lb_in_constraint(k));
            ++k;
        }
    }
}
...
namespace {
void CheckAddedLinearEqualityConstraintCommon(const Binding<LinearEqualityConstraint>& binding,
                                              const MathematicalProgram& prog,
                                              int num_linear_eq_cnstr) {
    ...
    EXPECT_EQ(prog.linear_equality_constraints().back().evaluator(), binding.evaluator());
...
void CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(MathematicalProgram* prog, const Expression& e, double b) {
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(prog, Vector1<Expression>(e), Vector1d(b));
}
}  // namespace

GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintMatrixVectorVariablesTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(4, "x");
    Matrix4d A;
    // clang-format off
...
GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintMatrixVariableRefListTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(1, "x");
    auto y = prog.NewContinuousVariables(3, "y");
    VariableRefList vars{x, y};
...
GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintSparseMatrixVariableRefListTest) {
    MathematicalProgram prog;
    const int n = 4;
    auto x = prog.NewContinuousVariables(1, "x");
    auto y = prog.NewContinuousVariables(3, "y");
...
GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintSparseMatrixVectorVariablesTest) {
    MathematicalProgram prog;
    const int n = 4;
    auto x = prog.NewContinuousVariables(n, "x");
    std::vector<Eigen::Triplet<double>> A_triplet_list;
...
GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintRowVectorVectorVariablesTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(4, "x");
    const Eigen::RowVector4d a{1, 2, 3, 4};
    const double b = -10;
...
GTEST_TEST(TestMathematicalProgram, AddLinearEqualityConstraintRowVectorVariableRefListTest) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables(1, "x");
    auto y = prog.NewContinuousVariables(3, "y");
    VariableRefList vars{x, y};
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicLinearEqualityConstraint1) {
    ...
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, 3 * x(0) + x(1) + 4 * x(2) + 1, 2);
    // Checks -x(1) = 3
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, -x(1), 3);
    // Checks -(x(0) + 2 * x(1)) = 2
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, -(x(0) + 2 * x(1)), 2);
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicLinearEqualityConstraint2) {
    ...
    // Checks x(1) = 2
    //        x(0) = 1
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, Vector2<Expression>(+x(1), +x(0)),
                                                           Eigen::Vector2d(2, 1));
    ...
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, v, Eigen::Vector3d(3, 4, 1));

    // Checks x(0) = 4
    //          1  = 1
    //       -x(1) = 2
    CheckAddedNonSymmetricSymbolicLinearEqualityConstraint(&prog, Vector3<Expression>(+x(0), 1, -x(1)),
                                                           Eigen::Vector3d(4, 1, 2));
}
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicLinearEqualityConstraint5) {
    ...
    const Binding<LinearEqualityConstraint> binding{prog.AddLinearEqualityConstraint(f)};
    EXPECT_EQ(prog.linear_equality_constraints().size(), 1u);

    const Expression expr_in_added_constraint{
            (binding.evaluator()->GetDenseA() * binding.variables() - binding.evaluator()->lower_bound())(0)};
    // expr_in_added_constraint should be:
    //    lhs(f) - rhs(f)
    //  = (3x₀ + 2x₁ + 3) - 5
    //  = 3x₀ + 2x₁ - 2.
    EXPECT_PRED2(ExprEqual, expr_in_added_constraint, 3 * x(0) + 2 * x(1) - 2);
    EXPECT_PRED2(ExprEqual, expr_in_added_constraint, get_lhs_expression(f) - get_rhs_expression(f));
}
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicLinearEqualityConstraint6) {
  ...
  b << 9.0,
    ...
    EXPECT_TRUE(is_permutation(exprs_in_added_constraint.data(), exprs_in_added_constraint.data() + 3,
...
bool AreTwoPolynomialsNear(const symbolic::Polynomial& p1,
    ...
    return all_of(monomial_to_coeff_map.begin(), monomial_to_coeff_map.end(), [tol](const auto& p) {
        return std::abs(symbolic::get_constant_value(p.second)) <= tol;
    });
}  // namespace

void CheckParsedSymbolicLorentzConeConstraint(MathematicalProgram* prog,
                                              const Formula& f,
    ...
    EXPECT_TRUE(symbolic::test::PolynomialEqual(symbolic::Polynomial(z(0)), symbolic::Polynomial(greater), 1E-10));
    ASSERT_TRUE(is_sqrt(lesser));
    EXPECT_TRUE(symbolic::test::PolynomialEqual(symbolic::Polynomial(z.tail(z.rows() - 1).squaredNorm()),
...
void CheckParsedSymbolicLorentzConeConstraint(MathematicalProgram* prog,
                                              ...
                                              LorentzConeConstraint::EvalType eval_type) {
    const auto& binding1 = prog->AddLorentzConeConstraint(linear_expr, quadratic_expr, 0., eval_type);
    EXPECT_EQ(binding1.evaluator()->eval_type(), eval_type);
    const auto& binding2 = prog->lorentz_cone_constraints().back();
    EXPECT_EQ(binding1.evaluator(), binding2.evaluator());
    EXPECT_EQ(binding1.variables(), binding2.variables());
    // Now check if the linear and quadratic constraints are parsed correctly.
    const VectorX<Expression> e_parsed = binding1.evaluator()->A() * binding1.variables() + binding1.evaluator()->b();
    EXPECT_PRED2(ExprEqual, (e_parsed(0) * e_parsed(0)).Expand(), (linear_expr * linear_expr).Expand());
    Expression quadratic_expr_parsed = e_parsed.tail(e_parsed.rows() - 1).squaredNorm();
...
void CheckParsedSymbolicLorentzConeConstraint(MathematicalProgram* prog,
    ...
    for (const auto eval_type :
         {LorentzConeConstraint::EvalType::kConvex, LorentzConeConstraint::EvalType::kConvexSmooth,
        ...
        EXPECT_EQ(binding1.evaluator(), binding2.evaluator());
        EXPECT_EQ(binding1.evaluator()->A() * binding1.variables() + binding1.evaluator()->b(), e);
        EXPECT_EQ(binding2.evaluator()->A() * binding2.variables() + binding2.evaluator()->b(), e);

        CheckParsedSymbolicLorentzConeConstraint(prog, e(0), e.tail(e.rows() - 1).squaredNorm(), eval_type);
    }
}

void CheckParsedSymbolicRotatedLorentzConeConstraint(MathematicalProgram* prog,
...
public:
    SymbolicLorentzConeTest() : prog_(), x_() { x_ = prog_.NewContinuousVariables<3>("x"); }
...
TEST_F(SymbolicLorentzConeTest, Formula) {
    CheckParsedSymbolicLorentzConeConstraint(&prog_, 2.0 * x_(2) >= x_.head<2>().cast<Expression>().norm(),
                                             LorentzConeConstraint::EvalType::kConvex);

    CheckParsedSymbolicLorentzConeConstraint(&prog_, x_.head<2>().cast<Expression>().norm() <= 2.0 * x_(2),
                                             LorentzConeConstraint::EvalType::kConvex);

    DRAKE_EXPECT_THROWS_MESSAGE(
            prog_.AddLorentzConeConstraint(2.0 * x_(2) * x_(1) >= x_.head<2>().cast<Expression>().norm(),
                                           LorentzConeConstraint::EvalType::kConvex),
            ".*non-linear.*");
}
...
TEST_F(SymbolicLorentzConeTest, Test7) {
    CheckParsedSymbolicLorentzConeConstraint(&prog_, x_(0) + 2, pow(x_(0), 2) + 4 * x_(0) * x_(1) + 4 * pow(x_(1), 2),
                                             LorentzConeConstraint::EvalType::kConvex);
    CheckParsedSymbolicLorentzConeConstraint(&prog_, x_(0) + 2, pow(x_(0), 2) + 4 * x_(0) * x_(1) + 4 * pow(x_(1), 2),
...
TEST_F(SymbolicLorentzConeTest, Test9) {
    CheckParsedSymbolicLorentzConeConstraint(&prog_, 2, pow(x_(0), 2) + pow(x_(1), 2),
                                             LorentzConeConstraint::EvalType::kConvex);
    CheckParsedSymbolicLorentzConeConstraint(&prog_, 2, pow(x_(0), 2) + pow(x_(1), 2),
                                             LorentzConeConstraint::EvalType::kConvexSmooth);
}
...
TEST_F(SymbolicLorentzConeTest, TestLinearConstraint) {
    ...
    // actually a constant.
    CheckParsedSymbolicLorentzConeConstraint(&prog_, x_(0) + 2, 1, LorentzConeConstraint::EvalType::kConvexSmooth);
    CheckParsedSymbolicLorentzConeConstraint(&prog_, x_(0) + 2, x_(0) - 2 * (0.5 * x_(0) + 1) + 3,
                                             LorentzConeConstraint::EvalType::kConvexSmooth);
}
...
TEST_F(SymbolicLorentzConeTest, TestError) {
    ...
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, pow(x_(1), 3)), runtime_error);

    // The quadratic expression is actually affine.
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0), 3 * x_(1) + 2), runtime_error);
    EXPECT_THROW(prog_.AddLorentzConeConstraint(
                         2 * x_(0), x_(1) * x_(1) - (x_(1) - x_(0)) * (x_(1) + x_(0)) - x_(0) * x_(0) + 2 * x_(1) + 3),
                 runtime_error);

    // The Hessian matrix is not positive semidefinite.
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, x_(1) * x_(1) - x_(2) * x_(2)), runtime_error);
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, x_(1) * x_(1) + x_(2) * x_(2) + 3 * x_(1) * x_(2)),
                 runtime_error);
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, x_(1) * x_(1) + x_(2) * x_(2) + 3 * x_(0) * x_(2)),
                 runtime_error);

    // The quadratic expression is not always non-negative.
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, x_(1) * x_(1) + x_(2) * x_(2) - 1), runtime_error);
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3, pow(2 * x_(0) + 3 * x_(1) + 2, 2) - 1), runtime_error);

    // The quadratic expression is a negative constant.
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) + 3,
                                                pow(x_(0), 2) - pow(x_(1), 2) - (x_(0) + x_(1)) * (x_(0) - x_(1)) - 1),
                 runtime_error);

    // The first expression is not actually linear.
    EXPECT_THROW(prog_.AddLorentzConeConstraint(2 * x_(0) * x_(1), pow(x_(0), 2)), runtime_error);
}
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicRotatedLorentzConeConstraint5) {
    ...
    Q << 1, 0.5, 0.5, 1;
    b << 0, 0.1;
    const Expression quadratic_expression = x.head<2>().cast<Expression>().dot(Q * x.head<2>() + b) + c;
    const auto binding =
            prog.AddRotatedLorentzConeConstraint(linear_expression1, linear_expression2, quadratic_expression);
    EXPECT_EQ(binding.evaluator(), prog.rotated_lorentz_cone_constraints().back().evaluator());
    const VectorX<Expression> z = binding.evaluator()->A() * binding.variables() + binding.evaluator()->b();
    const double tol{1E-10};
    EXPECT_TRUE(
            symbolic::test::PolynomialEqual(symbolic::Polynomial(linear_expression1), symbolic::Polynomial(z(0)), tol));
    EXPECT_TRUE(
            symbolic::test::PolynomialEqual(symbolic::Polynomial(linear_expression2), symbolic::Polynomial(z(1)), tol));
    EXPECT_TRUE(symbolic::test::PolynomialEqual(symbolic::Polynomial(quadratic_expression),
                                                symbolic::Polynomial(z.tail(z.rows() - 2).squaredNorm()), tol));
}

GTEST_TEST(TestMathematicalProgram, TestAddQuadraticAsRotatedLorentzConeConstraint) {
    solvers::MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();

    auto check = [&prog, &x](const Eigen::Matrix2d& Q, const Eigen::Vector2d& b, double c) {
        ...
        EXPECT_TRUE(symbolic::test::PolynomialEqual(
                ...
                symbolic::Polynomial(0.5 * x.cast<symbolic::Expression>().dot(Q * x) + b.dot(x) + c), tol));
        EXPECT_EQ(prog.rotated_lorentz_cone_constraints().back().evaluator().get(), dut.evaluator().get());
    };
    Eigen::Matrix2d Q;
    Q << 2, 1, 3, 2;
...
CheckAddedSymbolicPositiveSemidefiniteConstraint(MathematicalProgram* prog, const Eigen::MatrixBase<Derived>& V) {
    ...
    EXPECT_EQ(binding.evaluator().get(), prog->positive_semidefinite_constraints().back().evaluator().get());
    ...
    auto V_minus_M = math::ToSymmetricMatrixFromLowerTriangularColumns(new_lin_eq_cnstr.evaluator()->GetDenseA() *
...
GTEST_TEST(TestMathematicalProgram, AddPositiveSemidefiniteConstraint) {
    ...
    // Adds X is psd.
    CheckAddedSymbolicPositiveSemidefiniteConstraint(&prog, Matrix4d::Identity() * X);

    // Adds 2 * X + Identity() is psd.
    CheckAddedSymbolicPositiveSemidefiniteConstraint(&prog, 2.0 * X + Matrix4d::Identity());
...
GTEST_TEST(TestMathematicalProgram, AddPrincipalSubmatrixIsPsdConstraintVariable) {
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<4>("X");
  ...
  minor_manual << X(0, 0), X(0, 1), X(0, 3),
    ...
    auto psd_cnstr = prog.AddPrincipalSubmatrixIsPsdConstraint(X, minor_indices).evaluator();
    EXPECT_EQ(prog.positive_semidefinite_constraints().size(), 1);
    EXPECT_EQ(prog.GetAllConstraints().size(), 1);
    const auto& new_psd_cnstr = prog.positive_semidefinite_constraints().back();
    EXPECT_EQ(psd_cnstr.get(), new_psd_cnstr.evaluator().get());
...
GTEST_TEST(TestMathematicalProgram, AddPrincipalSubmatrixIsPsdConstraintExpression) {
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<4>("X");
    std::set<int> minor_indices{0, 1, 3};
    MatrixXDecisionVariable minor_manual(3, 3);
  ...
  minor_manual << X(0, 0), X(0, 1), X(0, 3),
    ...
    auto psd_cnstr = prog.AddPrincipalSubmatrixIsPsdConstraint(2 * Matrix4d::Identity() * X, minor_indices).evaluator();
    ...
    auto psd_cnstr_manual =
            prog_manual.AddPositiveSemidefiniteConstraint(2 * Matrix3d::Identity() * minor_manual).evaluator();
    const auto& new_psd_cnstr_manual = prog.positive_semidefinite_constraints().back();

    EXPECT_TRUE(CheckStructuralEquality(new_psd_cnstr_manual.variables(), new_psd_cnstr.variables()));
    EXPECT_EQ(prog.positive_semidefinite_constraints().size(), prog_manual.positive_semidefinite_constraints().size());
    EXPECT_EQ(prog.linear_equality_constraints().size(), prog_manual.linear_equality_constraints().size());
    EXPECT_EQ(prog.linear_equality_constraints().size(), 1);
    EXPECT_EQ(prog.GetAllConstraints().size(), prog_manual.GetAllConstraints().size());

    EXPECT_TRUE(CompareMatrices(prog.linear_equality_constraints()[0].evaluator()->upper_bound(),
                                prog_manual.linear_equality_constraints()[0].evaluator()->upper_bound()));
    EXPECT_TRUE(CompareMatrices(prog.linear_equality_constraints()[0].evaluator()->lower_bound(),
...
GTEST_TEST(TestMathematicalProgram, TestExponentialConeConstraint) {
    MathematicalProgram prog;
    EXPECT_EQ(prog.required_capabilities().count(ProgramAttribute::kExponentialConeConstraint), 0);
    auto x = prog.NewContinuousVariables<4>();
    const Vector3<symbolic::Expression> expr(2 * x(0) + x(1) + 2, 1, -2 * x(0) + 3);
    auto binding = prog.AddExponentialConeConstraint(expr);
    EXPECT_GT(prog.required_capabilities().count(ProgramAttribute::kExponentialConeConstraint), 0);
    EXPECT_EQ(prog.GetAllConstraints().size(), 1);
...
GTEST_TEST(TestMathematicalProgram, AddQuadraticCost) {
    ...
    CheckAddedQuadraticCost(&prog, Matrix3d::Identity(), Vector3d(1, 2, 3), x, true);

    CheckAddedQuadraticCost(&prog, -Matrix3d::Identity(), Vector3d(1, 2, 3), x, false);

    // Now check AddQuadraticCost(Q, b, x) without passing is_convex flag.
    auto cost1 = prog.AddQuadraticCost(Matrix3d::Identity(), Vector3d(1, 2, 3), x);
    ...
    cost1 = prog.AddQuadraticCost(Matrix3d::Identity(), Vector3d(1, 2, 3), x, false);
    EXPECT_FALSE(cost1.evaluator()->is_convex());

    auto cost2 = prog.AddQuadraticCost(-Matrix3d::Identity(), Vector3d(1, 2, 3), x);
    ...
    auto cost3 = prog.AddQuadraticCost(Matrix3d::Identity(), Vector3d(1, 2, 3), {x.tail<2>(), x.head<1>()});
    ...
    EXPECT_PRED2(ExprEqual, cost_eval_sym(0).Expand(),
                 (0.5 * (x(0) * x(0) + x(1) * x(1) + x(2) * x(2)) + x(1) + 2 * x(2) + 3 * x(0)).Expand());
    auto cost4 = prog.AddQuadraticCost(Matrix3d(Vector3d(-1, -2, -3).asDiagonal()), Vector3d(1, 2, 3),
                                       {x.tail<2>(), x.head<1>()});
    EXPECT_FALSE(cost4.evaluator()->is_convex());
    cost4.evaluator()->Eval(cost4.variables(), &cost_eval_sym);
    EXPECT_PRED2(ExprEqual, cost_eval_sym(0).Expand(),
                 (-0.5 * (x(1) * x(1) + 2 * x(2) * x(2) + 3 * x(0) * x(0)) + x(1) + 2 * x(2) + 3 * x(0)).Expand());
    // Intentionlly pass the wrong is_convex flag. MathematicalProgram will
    // use this wrong flag in the added cost.
    cost4 = prog.AddQuadraticCost(Matrix3d(Vector3d(-1, -2, -3).asDiagonal()), Vector3d(1, 2, 3),
                                  {x.tail<2>(), x.head<1>()}, true);
    EXPECT_TRUE(cost4.evaluator()->is_convex());

    // Now check AddQuadraticCost(Q, b, c, x) without passing is_convex flag.
    auto cost5 = prog.AddQuadraticCost(Eigen::Matrix3d::Identity(), Vector3d(1, 2, 3), 1.5, x);
    EXPECT_TRUE(cost5.evaluator()->is_convex());
    EXPECT_EQ(cost5.evaluator()->c(), 1.5);

    auto cost6 = prog.AddQuadraticCost(-Eigen::Matrix3d::Identity(), Vector3d(1, 2, 3), 2.5, x);
    EXPECT_FALSE(cost6.evaluator()->is_convex());
    EXPECT_EQ(cost6.evaluator()->c(), 2.5);
    // Intentionlly pass the wrong is_convex flag. MathematicalProgram will
    // use this wrong flag in the added cost.
    cost6 = prog.AddQuadraticCost(-Eigen::Matrix3d::Identity(), Vector3d(1, 2, 3), 2.5, x, true);
...
void CheckAddedSymbolicQuadraticCostUserFun(const MathematicalProgram& prog,
    ...
    const Expression e_added = 0.5 * x_bound.dot(cnstr->Q() * x_bound) + cnstr->b().dot(x_bound) + cnstr->c();
    EXPECT_PRED2(ExprEqual, e_added.Expand(), e.Expand());
}

void CheckAddedSymbolicQuadraticCost(MathematicalProgram* prog, const Expression& e, bool is_convex_expected) {
    int num_quadratic_cost = prog->quadratic_costs().size();
    auto binding1 = prog->AddQuadraticCost(e);
    EXPECT_EQ(binding1.evaluator()->is_convex(), is_convex_expected);
    CheckAddedSymbolicQuadraticCostUserFun(*prog, e, binding1, ++num_quadratic_cost);
    auto binding2 = prog->AddCost(e);
    CheckAddedSymbolicQuadraticCostUserFun(*prog, e, binding2, ++num_quadratic_cost);
}
...
GTEST_TEST(TestMathematicalProgram, AddSymbolicQuadraticCost) {
    ...
    Expression e7 = (x(0) + 2 * x(1) + 3) * (x(0) + x(1) + 4) + 3 * x(0) * x(0) + 6 * pow(x(1) + 1, 2);
...
GTEST_TEST(TestMathematicalProgram, AddL2NormCost) {
    ...
    auto obj1 = prog.AddCost(Binding<L2NormCost>(std::make_shared<L2NormCost>(A, b), x));
    EXPECT_TRUE(prog.required_capabilities().contains(ProgramAttribute::kL2NormCost));
    ...
    EXPECT_FALSE(prog.required_capabilities().contains(ProgramAttribute::kL2NormCost));
...
GTEST_TEST(TestMathematicalProgram, AddQuadraticConstraint) {
    ...
    auto constraint1 = prog.AddConstraint(Binding<QuadraticConstraint>(
            std::make_shared<QuadraticConstraint>(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Ones(), 0, 1),
            x.head<2>()));
    EXPECT_GT(prog.required_capabilities().count(ProgramAttribute::kQuadraticConstraint), 0);
    ASSERT_EQ(prog.quadratic_constraints().size(), 1);
    EXPECT_EQ(prog.quadratic_constraints()[0].evaluator().get(), constraint1.evaluator().get());
    EXPECT_EQ(prog.quadratic_constraints()[0].variables(), x.head<2>());

    auto constraint2 =
            prog.AddQuadraticConstraint(-Eigen::Matrix2d::Identity(), Eigen::Vector2d::Ones(), 0, 1, x.tail<2>(),
                                        QuadraticConstraint::HessianType::kNegativeSemidefinite);
    EXPECT_GT(prog.required_capabilities().count(ProgramAttribute::kQuadraticConstraint), 0);
    EXPECT_EQ(prog.quadratic_constraints().size(), 2);

    auto constraint3 = prog.AddQuadraticConstraint(x(0) * x(0) - x(1) * x(1), 0, 1);
    EXPECT_GT(prog.required_capabilities().count(ProgramAttribute::kQuadraticConstraint), 0);
    EXPECT_EQ(prog.quadratic_constraints().size(), 3);
    EXPECT_EQ(constraint3.evaluator()->hessian_type(), QuadraticConstraint::HessianType::kIndefinite);
    ...
    EXPECT_EQ(prog.required_capabilities().count(ProgramAttribute::kQuadraticConstraint), 0);
}
...
GTEST_TEST(TestMathematicalProgram, AddL2NormCostUsingConicConstraint) {
    ...
    const Binding<LorentzConeConstraint>& lorentz_cone_constraint = std::get<2>(ret);
    ...
    EXPECT_EQ(lorentz_cone_constraint.variables(), Vector3<symbolic::Variable>(s, x(0), x(1)));
    Vector3<symbolic::Expression> lorentz_eval_expected;
    lorentz_eval_expected << s, A * x + b;
    EXPECT_EQ(lorentz_cone_constraint.evaluator()->A() * lorentz_cone_constraint.variables() +
...
symbolic::Polynomial transform(const symbolic::Polynomial& poly, const map<Variable::Id, Variable>& var_id_to_var) {
    ...
    for (const pair<const symbolic::Monomial, symbolic::Expression>& p : poly.monomial_to_coefficient_map()) {
...
GTEST_TEST(TestMathematicalProgram, TestAddCostThrowError) {
    ...
    Variable y("y");
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddCost(x(0) + y), ".*is not a decision variable.*");
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddCost(x(0) * x(0) + y), ".*is not a decision variable.*");
}
...
GTEST_TEST(TestMathematicalProgram, TestClone) {
    ...
    // Add costs
    shared_ptr<Cost> generic_trivial_cost1 = make_shared<GenericTrivialCost1>();
    prog.AddCost(Binding<Cost>(generic_trivial_cost1, VectorDecisionVariable<3>(x(0), x(1), x(2))));
    GenericTrivialCost2 generic_trivial_cost2;
    ...
    // Add constraints
    shared_ptr<Constraint> generic_trivial_constraint1 = make_shared<GenericTrivialConstraint1>();
    prog.AddConstraint(Binding<Constraint>(generic_trivial_constraint1, VectorDecisionVariable<3>(x(0), x(1), x(2))));
    prog.AddConstraint(Binding<Constraint>(generic_trivial_constraint1, VectorDecisionVariable<3>(x(2), x(1), x(0))));
    ...
    prog.AddLorentzConeConstraint(Vector3<symbolic::Expression>(+x(0), +x(1), x(2) - 0.5 * x(1)),
                                  LorentzConeConstraint::EvalType::kConvexSmooth);
    prog.AddLorentzConeConstraint(Vector3<symbolic::Expression>(x(0) + x(1), +x(0), x(2) - x(1)),
                                  LorentzConeConstraint::EvalType::kConvexSmooth);
    prog.AddRotatedLorentzConeConstraint(Vector4<symbolic::Expression>(+x(0), +x(1), 0.5 * (x(0) + x(1)), 0.5 * x(2)));
    prog.AddRotatedLorentzConeConstraint(Vector4<symbolic::Expression>(x(0) + x(1), x(1) + x(2), +x(0), +x(1)));
    ...
    prog.AddLinearMatrixInequalityConstraint(
            {Eigen::Matrix2d::Identity(), Eigen::Matrix2d::Ones(), 2 * Eigen::Matrix2d::Ones()}, x.head<2>());
    EXPECT_EQ(prog.GetAllConstraints().size(), num_all_constraints + 1);
    num_all_constraints++;
    prog.AddLinearComplementarityConstraint(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Ones(), x.head<2>());
    EXPECT_EQ(prog.GetAllConstraints().size(), num_all_constraints + 1);
    num_all_constraints++;
    prog.AddLinearComplementarityConstraint(2 * Eigen::Matrix2d::Identity(), Eigen::Vector2d::Ones(), x.tail<2>());
    EXPECT_EQ(prog.GetAllConstraints().size(), num_all_constraints + 1);
    num_all_constraints++;
    prog.AddExponentialConeConstraint(Eigen::Matrix3d::Identity().sparseView(), Eigen::Vector3d::Ones(), x.head<3>());
    EXPECT_EQ(prog.GetAllConstraints().size(), num_all_constraints + 1);
    num_all_constraints++;
    ...
    for (int i = 0; i < prog.num_vars(); ++i) {
        EXPECT_TRUE(prog.decision_variable(i).equal_to(new_prog->decision_variable(i)));
    ...
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.generic_costs(), new_prog->generic_costs()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_costs(), new_prog->linear_costs()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.quadratic_costs(), new_prog->quadratic_costs()));

    // Cloned program should have the same constraints.
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.generic_constraints(), new_prog->generic_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_constraints(), new_prog->linear_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_equality_constraints(), new_prog->linear_equality_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.bounding_box_constraints(), new_prog->bounding_box_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.lorentz_cone_constraints(), new_prog->lorentz_cone_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.rotated_lorentz_cone_constraints(),
    ...
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_matrix_inequality_constraints(),
                                       new_prog->linear_matrix_inequality_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_matrix_inequality_constraints(),
                                       new_prog->linear_matrix_inequality_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.exponential_cone_constraints(), new_prog->exponential_cone_constraints()));
    EXPECT_TRUE(IsVectorOfBindingEqual(prog.linear_complementarity_constraints(),
...
GTEST_TEST(TestMathematicalProgram, TestEvalBinding) {
    ...
    auto linear_constraint = prog.AddLinearEqualityConstraint(Eigen::RowVector2d(1, 2), Vector1d(2), x.head<2>());
    // Add constraint 0 ≤ x(1)² + 2x(2)² + x(1) + 2x(2) ≤ 1
    auto quadratic_constraint =
            prog.AddConstraint(std::make_shared<QuadraticConstraint>((Eigen::Matrix2d() << 2, 0, 0, 4).finished(),
    ...
    EXPECT_TRUE(CompareMatrices(prog.EvalBinding(linear_constraint, x_val), Vector1d(5), 1E-15,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(prog.EvalBinding(quadratic_constraint, x_val), Vector1d(30), 1E-15,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(
            CompareMatrices(prog.EvalBinding(quadratic_cost, x_val), Vector1d(7), 1E-15, MatrixCompareType::absolute));

    EXPECT_TRUE(CompareMatrices(prog.EvalBindings(prog.GetAllConstraints(), x_val), Vector2d(30, 5), 1E-15,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(prog.EvalBindings(prog.GetAllCosts(), x_val), Vector1d(7), 1E-15,
                                MatrixCompareType::absolute));

    // Pass in an incorrect size input.
    EXPECT_THROW(prog.EvalBinding(linear_constraint, Eigen::Vector2d::Zero()), std::logic_error);

    // Pass in some variable not registered in the program.
    symbolic::Variable y("y");
    Binding<QuadraticCost> quadratic_cost_y(std::make_shared<QuadraticCost>(Vector1d(1), Vector1d(0)),
...
GTEST_TEST(TestMathematicalProgram, TestGetBindingVariableValues) {
    ...
    EXPECT_TRUE(CompareMatrices(prog.GetBindingVariableValues(binding1, x_val), Vector1d(-2)));
    EXPECT_TRUE(CompareMatrices(prog.GetBindingVariableValues(binding2, x_val), Vector2d(-2, 2)));
}
...
GTEST_TEST(TestMathematicalProgram, TestCheckSatisfied) {
    ...
    bindings.emplace_back(prog.AddLinearEqualityConstraint(y[0] == 3 * x[0] + 2 * x[1]));
...
GTEST_TEST(TestMathematicalProgram, TestSetAndGetInitialGuess) {
    ...
    EXPECT_TRUE(CompareMatrices(prog.GetInitialGuess(x.tail<2>()), Eigen::Vector2d(3, 4)));
    ...
    prog.SetDecisionVariableValueInVector(x.head<2>(), Eigen::Vector2d(0.0, 1.0), &guess);
    EXPECT_EQ(guess[0], 0.0);
    EXPECT_EQ(guess[1], 1.0);
    EXPECT_EQ(guess[2], 2.0);
    EXPECT_THROW(prog.SetDecisionVariableValueInVector(y, 0.0, &guess), std::exception);
}
...
GTEST_TEST(TestMathematicalProgram, TestNonlinearExpressionConstraints) {
    ...
    const MathematicalProgramResult result = Solve(prog, Eigen::Vector2d(-0.5, -0.5));
    EXPECT_TRUE(result.is_success());
    EXPECT_TRUE(CompareMatrices(result.get_x_val(), Vector2d::Constant(-std::sqrt(2.) / 2.), 1e-6));
}
...
GTEST_TEST(TestMathematicalProgram, TestAddVisualizationCallback) {
    ...
    Binding<VisualizationCallback> b = prog.AddVisualizationCallback(my_callback, x);
    ...
    const VectorX<AutoDiffXd> test_x_autodiff = math::InitializeAutoDiff(VectorXd{test_x});
...
void CheckNewSosPolynomial(MathematicalProgram::NonnegativePolynomial type) {
    ...
    for (int i = 0; i < Q_poly.rows(); ++i) {
        for (int j = 0; j < Q_poly.cols(); ++j) {
            Q_poly(i, j) = symbolic::Polynomial({{monomial_one, Q(j * Q_poly.rows() + i)}});
        }
    }
...
GTEST_TEST(TestMathematicalProgram, NewSosPolynomial) {
    ...
    EXPECT_THROW(CheckNewSosPolynomial(static_cast<MathematicalProgram::NonnegativePolynomial>(0)), std::runtime_error);

    // Check NewSosPolynomial with degree = 0
    for (const auto type :
         {MathematicalProgram::NonnegativePolynomial::kSos, MathematicalProgram::NonnegativePolynomial::kSdsos,
        ...
        EXPECT_EQ(prog.bounding_box_constraints().size(), 1u);
        EXPECT_TRUE(
                CompareMatrices(prog.bounding_box_constraints()[0].evaluator()->lower_bound(), Vector1d::Constant(0)));
        EXPECT_TRUE(CompareMatrices(prog.bounding_box_constraints()[0].evaluator()->upper_bound(),
        ...
        EXPECT_EQ(p.monomial_to_coefficient_map().at(symbolic::Monomial()), gram(0, 0));
    }
}

void CheckNewEvenDegreeNonnegativePolynomial(MathematicalProgram::NonnegativePolynomial type) {
    ...
    std::tie(p, Q_oo, Q_ee) = prog.NewEvenDegreeNonnegativePolynomial(t_vars, degree, type);
...
GTEST_TEST(TestMathematicalProgram, NewEvenDegreeNonnegativePolynomial) {
    CheckNewEvenDegreeNonnegativePolynomial(MathematicalProgram::NonnegativePolynomial::kSos);
    CheckNewEvenDegreeNonnegativePolynomial(MathematicalProgram::NonnegativePolynomial::kSdsos);
    CheckNewEvenDegreeNonnegativePolynomial(MathematicalProgram::NonnegativePolynomial::kDsos);
}
...
GTEST_TEST(TestMathematicalProgram, AddEqualityConstraintBetweenPolynomials) {
    ...
    auto is_satisfied = [&prog](const Eigen::Vector4d& a_val) {
        for (const auto& linear_eq_constraint : prog.linear_equality_constraints()) {
            ...
            if (std::abs(constraint_val(0) - linear_eq_constraint.evaluator()->lower_bound()(0)) > 1E-12) {
                return false;
            }
        }
        return true;
    ...
    // decision variables.
    EXPECT_THROW(prog.AddEqualityConstraintBetweenPolynomials(p1, symbolic::Polynomial(a(0) * a(1) * x, {x})),
                 std::runtime_error);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog.AddEqualityConstraintBetweenPolynomials(p1, symbolic::Polynomial(b * x, {x})),
    ...
    DRAKE_EXPECT_NO_THROW(prog.AddEqualityConstraintBetweenPolynomials(p1, symbolic::Polynomial(b * x, {x})));
}
...
GTEST_TEST(TestMathematicalProgram, AddConstraintMatrix2) {
  ...
  exprs << x(0), x(0) + 2*x(1),
    ...
    auto check_binding = [&lb, &ub, &coeff](const Binding<LinearConstraint>& binding) {
            ...
            for (int j = 0; j < 2; ++j) {
                ...
                for (int k = 0; k < 4; ++k) {
                    if (binding.evaluator()->lower_bound()(k) == lb(i, j) &&
                        ...
                        EXPECT_TRUE(CompareMatrices(binding.evaluator()->GetDenseA().row(k), coeff[i][j]));
                        find_match = true;
                    }
                }
...
GTEST_TEST(TestMathematicalProgram, AddSosConstraint) {
    ...
    const Vector2<symbolic::Monomial> monomial_basis(symbolic::Monomial{}, symbolic::Monomial(x, 1));

    const Matrix2<symbolic::Variable> Q_psd =
            prog.AddSosConstraint(p1, monomial_basis, MathematicalProgram::NonnegativePolynomial::kSos, "Q");
    ...
    const int num_lin_con = prog.linear_constraints().size() + prog.linear_equality_constraints().size();
    // Now call AddSosConstraint with type=kDsos.
    prog.AddSosConstraint(p1, monomial_basis, MathematicalProgram::NonnegativePolynomial::kDsos);
    // With dsos, the mathematical program adds more linear constraints than it
    // did with sos.
    EXPECT_GT(prog.linear_constraints().size() + prog.linear_equality_constraints().size(), 2 * num_lin_con);
    EXPECT_EQ(prog.positive_semidefinite_constraints().size(), 1u);

    // Now call AddSosConstraint with type=kSdsos.
    prog.AddSosConstraint(p1, monomial_basis, MathematicalProgram::NonnegativePolynomial::kSdsos);
    EXPECT_GT(prog.lorentz_cone_constraints().size() + prog.rotated_lorentz_cone_constraints().size(), 0u);
    EXPECT_EQ(prog.positive_semidefinite_constraints().size(), 1u);
    ...
    // contain [x].
    const symbolic::Polynomial p2{{{symbolic::Monomial(), 0}, {symbolic::Monomial(x, 2), a + 1}}};
...
GTEST_TEST(TestMathematicalProgram, RemoveLinearCost) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    RemoveCostTest<LinearCost>(&prog, x[0] + 2 * x[1], &(prog.linear_costs()), ProgramAttribute::kLinearCost);
}

GTEST_TEST(TestMathematicalProgram, RemoveQuadraticCost) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    RemoveCostTest(&prog, x[0] * x[0] + 2 * x[1] * x[1], &(prog.quadratic_costs()), ProgramAttribute::kQuadraticCost);
}

GTEST_TEST(TestMathematicalProgram, RemoveGenericCost) {
    MathematicalProgram prog;
    auto x = prog.NewContinuousVariables<2>();
    RemoveCostTest(&prog, x[0] * x[0] * x[1], &(prog.generic_costs()), ProgramAttribute::kGenericCost);
}
...
GTEST_TEST(TestMathematicalProgram, RemoveDecisionVariable) {
    ...
    EXPECT_THAT(prog.decision_variables(), testing::Pointwise(testing::Truly(TupleVarEqual), {x(0), x(2)}));
    EXPECT_TRUE(CompareMatrices(prog.initial_guess(), Eigen::Vector2d(0, 2)));
    EXPECT_THAT(prog.GetVariableScaling(),
                testing::WhenSorted(testing::ElementsAre(std::make_pair(0, 0.5), std::make_pair(1, 4.0))));
}
...
GTEST_TEST(TestMathematicalProgram, RemoveDecisionVariableError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog.RemoveDecisionVariable(dummy),
                                ".*is not a decision variable of this MathematicalProgram.");

    // Remove a variable associated with a cost.
    prog.AddLinearCost(x(0));
    DRAKE_EXPECT_THROWS_MESSAGE(prog.RemoveDecisionVariable(x(0)), ".* is associated with a LinearCost.*");

    // Remove a variable associated with a constraint.
    prog.AddLinearConstraint(x(0) + x(1) <= 1);
    DRAKE_EXPECT_THROWS_MESSAGE(prog.RemoveDecisionVariable(x(1)), ".* is associated with a LinearConstraint[^]*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog.RemoveDecisionVariable(x(2)), ".* is associated with a VisualizationCallback[^]*");
}

GTEST_TEST(TestMathematicalProgram, TestToLatex) {
    MathematicalProgram prog;
    std::string empty_prog = prog.ToLatex();
    EXPECT_EQ(empty_prog, "\\text{This MathematicalProgram has no decision variables.}");
...
GTEST_TEST(TestMathematicalProgram, RemoveLinearConstraint) {
    ...
    EXPECT_TRUE(prog.required_capabilities().contains(ProgramAttribute::kLinearConstraint));
    ...
    EXPECT_FALSE(prog.required_capabilities().contains(ProgramAttribute::kLinearConstraint));
...
GTEST_TEST(TestMathematicalProgram, RemoveConstraintPSD) {
    ...
    EXPECT_GT(prog.required_capabilities().count(ProgramAttribute::kPositiveSemidefiniteConstraint), 0);
    EXPECT_EQ(prog.RemoveConstraint(lmi_con), 1);
    EXPECT_EQ(prog.linear_matrix_inequality_constraints().size(), 0u);
    EXPECT_EQ(prog.required_capabilities().count(ProgramAttribute::kPositiveSemidefiniteConstraint), 0);
}
...
GTEST_TEST(TestMathematicalProgram, RemoveConstraint) {
    ...
    auto lorentz_con = prog.AddLorentzConeConstraint(x.cast<symbolic::Expression>(),
                                                     LorentzConeConstraint::EvalType::kConvexSmooth);
    auto rotated_lorentz_con = prog.AddRotatedLorentzConeConstraint(x.cast<symbolic::Expression>());
    Eigen::SparseMatrix<double> A(3, 3);
    A.setIdentity();
    auto exponential_con = prog.AddExponentialConeConstraint(A, Eigen::Vector3d(1, 2, 3), x);
    ...
    TestRemoveConstraint(&prog, rotated_lorentz_con, &(prog.rotated_lorentz_cone_constraints()),
                         ProgramAttribute::kRotatedLorentzConeConstraint);
    TestRemoveConstraint(&prog, exponential_con, &(prog.exponential_cone_constraints()),
                         ProgramAttribute::kExponentialConeConstraint);
    TestRemoveConstraint(&prog, generic_con, &(prog.generic_constraints()), ProgramAttribute::kGenericConstraint);

    auto lcp_con = prog.AddLinearComplementarityConstraint(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Ones(), x);
    TestRemoveConstraint(&prog, lcp_con, &(prog.linear_complementarity_constraints()),
...
GTEST_TEST(TestMathematicalProgram, RemoveVisualizationCallback) {
    ...
    EXPECT_TRUE(prog.required_capabilities().contains(ProgramAttribute::kCallback));
    int count = prog.RemoveVisualizationCallback(callback);
    EXPECT_EQ(count, 1);
    EXPECT_TRUE(prog.visualization_callbacks().empty());
    EXPECT_FALSE(prog.required_capabilities().contains(ProgramAttribute::kCallback));
}
...
TEST_F(ApproximatePSDConstraint, TightenPsdConstraintToDd) {
    ...
    EXPECT_EQ(ssize(prog_.linear_constraints()), Y_.rows() * Y_.rows() + X_.rows() * X_.rows() + 2);
    EXPECT_EQ(ssize(prog_.linear_equality_constraints()), 1);
}
...
GTEST_TEST(TightenPsdConstraintToDd, UnregisteredVariableError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog1.TightenPsdConstraintToDd(psd_constraint2), ".*is not a decision variable.*");
}
...
GTEST_TEST(TightenPsdConstraintToDd, NoConstraintToReplace) {
    ...
    auto constraint = internal::CreateBinding(std::make_shared<PositiveSemidefiniteConstraint>(X.rows()),
...
GTEST_TEST(RelaxPsdConstraintToDdDualCone, UnregisteredVariableError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog1.RelaxPsdConstraintToDdDualCone(psd_constraint2),
                                ".*is not a decision variable.*");
}
...
GTEST_TEST(RelaxPsdConstraintToSddDualCone, UnregisteredVariableError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(prog1.RelaxPsdConstraintToSddDualCone(psd_constraint2),
                                ".*is not a decision variable.*");
}
...
GTEST_TEST(MathematicalProgramTest, AddLogDeterminantLowerBoundConstraint) {
    MathematicalProgram prog;
    auto X = prog.NewSymmetricContinuousVariables<3>("X");
    auto ret = prog.AddLogDeterminantLowerBoundConstraint(X.cast<symbolic::Expression>(), 1);
    const auto& constraint = std::get<0>(ret);
    const auto& t = std::get<1>(ret);
    EXPECT_TRUE(CompareMatrices(constraint.evaluator()->GetDenseA(), Eigen::RowVector3d::Ones()));
    EXPECT_TRUE(CompareMatrices(constraint.evaluator()->lower_bound(), Vector1d(1)));
    EXPECT_TRUE(CompareMatrices(constraint.evaluator()->upper_bound(), Vector1d(kInf)));
