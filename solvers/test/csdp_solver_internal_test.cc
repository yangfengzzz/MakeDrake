...
void CompareBlockrec(const csdp::blockrec& block,
    ...
    if (blockcategory == csdp::blockcat::MATRIX) {
        for (int j = 0; j < blocksize; ++j) {
            for (int i = 0; i < blocksize; ++i) {
                EXPECT_NEAR(block.data.mat[ijtok(i + 1, j + 1, blocksize)], value[j * blocksize + i], tol);
            }
        }
...
TEST_F(SDPwithOverlappingVariables1, GenerateCsdpProblemDataWithoutFreeVariables) {
    const SdpaFreeFormat dut(*prog_);
    struct csdp::blockmatrix C_csdp;
    double* rhs_csdp;
    struct csdp::constraintmatrix* constraints{nullptr};
    GenerateCsdpProblemDataWithoutFreeVariables(dut, &C_csdp, &rhs_csdp, &constraints);
...
TEST_F(CsdpDocExample, GenerateCsdpProblemDataWithoutFreeVariables) {
    ...
    CompareBlockrec(C_csdp.blocks[2], csdp::MATRIX, 3, {3, 0, 1, 0, 2, 0, 1, 0, 3}, 0);
    CompareBlockrec(C_csdp.blocks[3], csdp::DIAG, 2, {0, 0}, 0);
...
TEST_F(TrivialSDP1, GenerateCsdpProblemDataWithoutFreeVariables) {
    ...
    CompareBlockrec(C_csdp.blocks[1], csdp::MATRIX, 3, {0, 0.5, 0, 0.5, 0, 0.5, 0, 0.5, 0}, 0);
...
TEST_F(SDPwithOverlappingVariables1, Solve) {
    ...
    double* y;
    csdp::cpp_initsoln(dut.num_X_rows(), dut.g().rows(), C, rhs, constraints, &X, &y, &Z);
    double pobj, dobj;
    const int ret = csdp::cpp_easy_sdp(nullptr, dut.num_X_rows(), dut.g().rows(), C, rhs, constraints,
    ...
    csdp::cpp_free_prob(dut.num_X_rows(), dut.g().rows(), C, rhs, constraints, X, y, Z);
}
...
TEST_F(CsdpDocExample, Solve) {
    ...
    EXPECT_EQ(X.nblocks, 3);
    CompareBlockrec(X.blocks[1], csdp::MATRIX, 2, {0.125, 0.125, 0.125, 0.125}, tol);
    CompareBlockrec(X.blocks[2], csdp::MATRIX, 3, {2.0 / 3, 0, 0, 0, 0, 0, 0, 0, 0}, tol);
    CompareBlockrec(X.blocks[3], csdp::DIAG, 2, {0, 0}, tol);

    EXPECT_EQ(Z.nblocks, 3);
    CompareBlockrec(Z.blocks[1], csdp::MATRIX, 2, {0.25, -0.25, -0.25, 0.25}, tol);
    CompareBlockrec(Z.blocks[2], csdp::MATRIX, 3, {0, 0, 0, 0, 2, 0, 0, 0, 2}, tol);
