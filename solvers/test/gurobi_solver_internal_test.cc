...
GTEST_TEST(AddL2NormCostVariables, Test) {
    ...
    AddL2NormCostVariables(prog.l2norm_costs(), &is_new_variable, &num_gurobi_vars, &lorentz_cone_variable_indices,
                           &gurobi_var_type, &xlow, &xupp);
    ExpectVectorEqual(is_new_variable,
                      std::vector<bool>{{false, false, false, true, true, true, true, true, true, true}});
    EXPECT_EQ(num_gurobi_vars, prog.num_vars() + (1 + cost0.evaluator()->get_sparse_A().rows()) +
                                       (1 + cost1.evaluator()->get_sparse_A().rows()));
    EXPECT_EQ(lorentz_cone_variable_indices.size(), 2);
    ExpectVectorEqual(lorentz_cone_variable_indices[0], std::vector<int>{{3, 4, 5, 6}});
    ExpectVectorEqual(lorentz_cone_variable_indices[1], std::vector<int>{{7, 8, 9}});
    ExpectVectorEqual(gurobi_var_type, std::vector<char>(num_gurobi_vars, GRB_CONTINUOUS));
    ExpectVectorEqual(xlow, std::vector<double>{{-kInf, -kInf, -kInf, 0, -kInf, -kInf, -kInf, 0, -kInf, -kInf}});
...
GTEST_TEST(AddL2NormCosts, Test) {
    ...
    GRBemptyenv(&env);
    GRBstartenv(env);
    GRBnewmodel(env, &model, "model", num_gurobi_vars, nullptr, &xlow[0], &xupp[0], gurobi_var_type.data(), nullptr);
    int num_gurobi_linear_constraints{0};
    int error = AddL2NormCosts(prog, lorentz_cone_variable_indices, model, &num_gurobi_linear_constraints);
    GRBupdatemodel(model);
    EXPECT_EQ(error, 0);
    EXPECT_EQ(num_gurobi_linear_constraints,
              cost0.evaluator()->get_sparse_A().rows() + cost1.evaluator()->get_sparse_A().rows());
    double cost_coeff{0};
    GRBgetdblattrelement(model, "Obj", lorentz_cone_variable_indices[0][0], &cost_coeff);
    EXPECT_EQ(cost_coeff, 1);
    GRBgetdblattrelement(model, "Obj", lorentz_cone_variable_indices[1][0], &cost_coeff);
    EXPECT_EQ(cost_coeff, 1);
    int num_q_constrs;
