...
public:
    ...
    IntegerOptimizationUtilTest()
        : prog_{},
          ...
          b_vals_{{Eigen::Vector2i(0, 0), Eigen::Vector2i(0, 1), Eigen::Vector2i(1, 0), Eigen::Vector2i(1, 1)}},
    ...
    void CheckLogicalOperand(LogicalOperand operand) {
        ...
        switch (operand) {
            case LogicalOperand::kAnd:
                prog_.AddConstraint(CreateLogicalAndConstraint(symbolic::Expression{b_(0)}, symbolic::Expression{b_(1)},
                                                               symbolic::Expression{operand_result}));
                break;
            case LogicalOperand::kOr:
                prog_.AddConstraint(CreateLogicalOrConstraint(symbolic::Expression{b_(0)}, symbolic::Expression{b_(1)},
                                                              symbolic::Expression{operand_result}));
                break;
            case LogicalOperand::kXor:
                prog_.AddConstraint(CreateLogicalXorConstraint(symbolic::Expression{b_(0)}, symbolic::Expression{b_(1)},
                                                               symbolic::Expression{operand_result}));
                break;
        }
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            switch (operand) {
                case LogicalOperand::kAnd:
                    operand_result_expected = static_cast<double>(b_vals_[i](0) & b_vals_[i](1));
                    break;
                case LogicalOperand::kOr:
                    operand_result_expected = static_cast<double>(b_vals_[i](0) | b_vals_[i](1));
                    break;
            ...
            EXPECT_NEAR(result.GetSolution(operand_result), operand_result_expected, tol);
