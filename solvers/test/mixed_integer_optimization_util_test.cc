...
GTEST_TEST(TestLogarithmicSos2, TestAddSos2) {
    ...
    auto y1 = AddLogarithmicSos2Constraint(&prog, lambda1.cast<symbolic::Expression>());
    static_assert(std::is_same_v<decltype(y1), VectorXDecisionVariable>, "y1 should be a dynamic-sized vector.");
    auto lambda2 = prog.NewContinuousVariables<3>("lambda2");
    auto y2 = AddLogarithmicSos2Constraint(&prog, lambda2.cast<symbolic::Expression>());
    static_assert(std::is_same_v<decltype(y2), VectorDecisionVariable<1>>, "y2 should be a static-sized vector.");
}
...
void LogarithmicSos2Test(int num_lambda, bool logarithmic_binning) {
    ...
    if (logarithmic_binning) {
        y = AddLogarithmicSos2Constraint(&prog, lambda.cast<symbolic::Expression>());
    } else {
        y = prog.NewBinaryVariables(num_lambda - 1);
        AddSos2Constraint(&prog, lambda.cast<symbolic::Expression>(), y.cast<symbolic::Expression>());
    }
...
GTEST_TEST(TestSos2, TestClosestPointOnLineSegments) {
  ...
  A << 0, 1, 2, 4, 6, 7, 8,
    ...
    AddSos2Constraint(&prog, lambda.cast<symbolic::Expression>(), z.cast<symbolic::Expression>());
    ...
    Binding<QuadraticCost> cost =
            prog.AddQuadraticCost(Eigen::Matrix2d::Zero(), Eigen::Vector2d::Zero(), 0, VectorDecisionVariable<2>(x, y));
    ...
    Q_and_P.push_back(std::make_pair(Eigen::Vector2d(1, 1), Eigen::Vector2d(1, 1)));
    Q_and_P.push_back(std::make_pair(Eigen::Vector2d(1.9, 1), Eigen::Vector2d(1.45, 0.55)));
    Q_and_P.push_back(std::make_pair(Eigen::Vector2d(3, 1), Eigen::Vector2d(3, 1)));
    Q_and_P.push_back(std::make_pair(Eigen::Vector2d(5, 1.2), Eigen::Vector2d(4.9, 1.1)));
    Q_and_P.push_back(std::make_pair(Eigen::Vector2d(7.5, 1.2), Eigen::Vector2d(7.15, 0.85)));
    for (const auto& QP_pair : Q_and_P) {
        const Eigen::Vector2d& Q = QP_pair.first;
        // The cost is |P-Q|²
        cost.evaluator()->UpdateCoefficients(2 * Eigen::Matrix2d::Identity(), -2 * Q, Q.squaredNorm());
        ...
        if (gurobi_solver.available()) {
            auto result = gurobi_solver.Solve(prog, {}, {});
            EXPECT_TRUE(result.is_success());
            const Eigen::Vector2d P(result.GetSolution(VectorDecisionVariable<2>(x, y)));
            const Eigen::Vector2d P_expected = QP_pair.second;
            EXPECT_TRUE(CompareMatrices(P, P_expected, 1E-6));
            EXPECT_NEAR(result.get_optimal_cost(), (Q - P_expected).squaredNorm(), 1E-12);
        }
    }
}
...
void LogarithmicSos1Test(int num_lambda, const Eigen::Ref<const Eigen::MatrixXi>& codes) {
    ...
    AddLogarithmicSos1Constraint(&prog, lambda.cast<symbolic::Expression>(), y, codes);
    ...
    for (int i = 0; i < num_lambda; ++i) {
        ...
        if (gurobi_solver.available()) {
            ...
            lambda_expected(i) = 1;
            EXPECT_TRUE(
                    CompareMatrices(result.GetSolution(lambda), lambda_expected, 1E-6, MatrixCompareType::absolute));
        }
    }
}
...
GTEST_TEST(TestLogarithmicSos1, Test) {
    ...
    auto check = [&prog, &y, &lambda](const Eigen::VectorXd& lambda_val, const Eigen::VectorXd& y_val,
        ...
        for (const auto& binding : prog.GetAllConstraints()) {
            satisfied = satisfied && binding.evaluator()->CheckSatisfied(prog.GetBindingVariableValues(binding, x_val));
        }
...
GTEST_TEST(TestBilinearProductMcCormickEnvelopeSos2, AddConstraint) {
    ...
    auto lambda1 = AddBilinearProductMcCormickEnvelopeSos2(&prog, x, y, w, phi_x_static, phi_y_static, Bx, By,
                                                           IntervalBinning::kLogarithmic);
    static_assert(std::is_same_v<decltype(lambda1), MatrixDecisionVariable<3, 4>>, "lambda should be a static matrix");

    auto lambda2 = AddBilinearProductMcCormickEnvelopeSos2(&prog, x, y, w, phi_x_dynamic, phi_y_static, Bx, By,
                                                           IntervalBinning::kLogarithmic);
    static_assert(std::is_same_v<decltype(lambda2), MatrixDecisionVariable<Eigen::Dynamic, 4>>,
                  "lambda's type is incorrect");

    auto lambda3 = AddBilinearProductMcCormickEnvelopeSos2(&prog, x, y, w, phi_x_static, phi_y_dynamic, Bx, By,
                                                           IntervalBinning::kLogarithmic);
    static_assert(std::is_same_v<decltype(lambda3), MatrixDecisionVariable<3, Eigen::Dynamic>>,
                  "lambda's type is incorrect");

    auto lambda4 = AddBilinearProductMcCormickEnvelopeSos2(&prog, x, y, w, phi_x_dynamic, phi_y_dynamic, Bx, By,
                                                           IntervalBinning::kLogarithmic);
    static_assert(std::is_same_v<decltype(lambda4), MatrixDecisionVariable<Eigen::Dynamic, Eigen::Dynamic>>,
                  "lambda's type is incorrect");
}
...
public:
    ...
    virtual Eigen::VectorXd SetBinaryValue(int active_interval, int num_interval) const = 0;

    Eigen::VectorXd SetBinaryValueLinearBinning(int active_interval, int num_interval) const {
    ...
    void TestFeasiblePoint() {
        ...
        auto CheckFeasibility = [&x_constraint, &y_constraint, &w_constraint](MathematicalProgram* prog, double x_val,
                                                                              double y_val, double w_val,
                                                                              bool is_feasible) {
            auto UpdateBound = [](Binding<BoundingBoxConstraint>* constraint, double val) {
            ...
            if (solver.available()) {
                ...
                EXPECT_EQ(result.get_solution_result(),
                          is_feasible ? SolutionResult::kSolutionFound : SolutionResult::kInfeasibleConstraints);
            }
        };
    ...
    void TestLinearObjective() {
        ...
        auto Bx_constraint = prog_.AddBoundingBoxConstraint(Eigen::VectorXd::Zero(Bx_.rows()),
                                                            Eigen::VectorXd::Zero(Bx_.rows()), Bx_);
        auto By_constraint = prog_.AddBoundingBoxConstraint(Eigen::VectorXd::Zero(By_.rows()),
            ...
            for (int j = 0; j < num_interval_y_; ++j) {
                ...
                for (int k = 0; k < a.cols(); ++k) {
                    ...
                    if (gurobi_solver.available()) {
                        ...
                        EXPECT_NEAR(prog_result_.get_optimal_cost(), cost_at_vertices.minCoeff(), 1E-4);
...
class BilinearProductMcCormickEnvelopeSos2Test : public ::testing::TestWithParam<std::tuple<int, int, IntervalBinning>>,
...
public:
    ...
    BilinearProductMcCormickEnvelopeSos2Test()
        : BilinearProductMcCormickEnvelopeTest(std::get<0>(GetParam()), std::get<1>(GetParam())),
          binning_{std::get<2>(GetParam())},
          Bx_size_{binning_ == IntervalBinning::kLogarithmic ? CeilLog2(num_interval_x_) : num_interval_x_},
          By_size_{binning_ == IntervalBinning::kLogarithmic ? CeilLog2(num_interval_y_) : num_interval_y_} {
        Bx_ = prog_.NewBinaryVariables(Bx_size_);
        By_ = prog_.NewBinaryVariables(By_size_);
        lambda_ = AddBilinearProductMcCormickEnvelopeSos2(&prog_, x_, y_, w_, phi_x_, phi_y_,
                                                          Bx_.cast<symbolic::Expression>(),
                                                          By_.cast<symbolic::Expression>(), binning_);
    }

    Eigen::VectorXd SetBinaryValueLogarithmicBinning(int active_interval, int num_interval) const {
        const Eigen::MatrixXi gray_codes = math::CalculateReflectedGrayCodes(solvers::CeilLog2(num_interval));
        return gray_codes.row(active_interval).cast<double>().transpose();
    }

    Eigen::VectorXd SetBinaryValue(int active_interval, int num_interval) const override {
        switch (binning_) {
    ...
    void TestLinearObjectiveCheck(int i, int j, int k) const override {
        ...
        for (int m = 0; m <= num_interval_x_; ++m) {
            for (int n = 0; n <= num_interval_y_; ++n) {
                if (!((m == i && n == j) || (m == i && n == (j + 1)) || (m == (i + 1) && n == j) ||
                      (m == (i + 1) && n == (j + 1)))) {
...
INSTANTIATE_TEST_SUITE_P(TestMixedIntegerUtil,
                         BilinearProductMcCormickEnvelopeSos2Test,
                         ::testing::Combine(::testing::ValuesIn(std::vector<int>{2, 3}),
                                            ...
                                            ::testing::ValuesIn(std::vector<IntervalBinning>{
                                                    IntervalBinning::kLogarithmic, IntervalBinning::kLinear})));

class BilinearProductMcCormickEnvelopeMultipleChoiceTest : public ::testing::TestWithParam<std::tuple<int, int>>,
                                                           public BilinearProductMcCormickEnvelopeTest {
public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(BilinearProductMcCormickEnvelopeMultipleChoiceTest);
    ...
    BilinearProductMcCormickEnvelopeMultipleChoiceTest()
        ...
        AddBilinearProductMcCormickEnvelopeMultipleChoice(
                &prog_, x_, y_, w_, phi_x_, phi_y_, Bx_.cast<symbolic::Expression>(), By_.cast<symbolic::Expression>());
    }
...
TEST_P(BilinearProductMcCormickEnvelopeMultipleChoiceTest, LinearObjectiveTest) {
    TestLinearObjective();
}
