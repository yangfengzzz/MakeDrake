...
const AbstractValue& MathematicalProgramResult::get_abstract_solver_details() const {
    if (!solver_details_) {
...
void MathematicalProgramResult::set_x_val(const Eigen::VectorXd& x_val) {
    ...
    if (x_val.size() != static_cast<int>(decision_variable_index_->size())) {
        ...
        oss << "MathematicalProgramResult::set_x_val, the dimension of x_val is " << x_val.size() << ", expected "
            << decision_variable_index_->size();
...
double GetVariableValue(const symbolic::Variable& var,
    ...
    DRAKE_ASSERT(variable_values.rows() == static_cast<int>(variable_index->size()));
    auto it = variable_index->find(var.get_id());
    if (it == variable_index->end()) {
        throw std::invalid_argument(
                fmt::format("GetVariableValue: {} is not captured by the variable_index map.", var.get_name()));
    }
    return variable_values(it->second);
}

double MathematicalProgramResult::GetSolution(const symbolic::Variable& var) const {
    return GetVariableValue(var, decision_variable_index_, x_val_);
}

void MathematicalProgramResult::SetSolution(const symbolic::Variable& var, double value) {
    x_val_(decision_variable_index_->at(var.get_id())) = value;
}

symbolic::Expression MathematicalProgramResult::GetSolution(const symbolic::Expression& e) const {
...
symbolic::Polynomial MathematicalProgramResult::GetSolution(const symbolic::Polynomial& p) const {
    DRAKE_ASSERT(decision_variable_index_.has_value());
    ...
    for (const auto& [monomial, coefficient] : p.monomial_to_coefficient_map()) {
        ...
        monomial_to_coefficient_result_map.emplace_hint(monomial_to_coefficient_result_map.end(), monomial,
                                                        coefficient.EvaluatePartial(env));
    }
    return symbolic::Polynomial(monomial_to_coefficient_result_map);
}

double MathematicalProgramResult::GetSuboptimalSolution(const symbolic::Variable& var, int solution_number) const {
    return GetVariableValue(var, decision_variable_index_, suboptimal_x_val_[solution_number]);
}

void MathematicalProgramResult::AddSuboptimalSolution(double suboptimal_objective,
...
std::vector<std::string> MathematicalProgramResult::GetInfeasibleConstraintNames(
    ...
    for (const auto& binding : prog.GetAllConstraints()) {
        ...
        for (int i = 0; i < val.rows(); i++) {
            if (std::isnan(val(i)) || val[i] < constraint->lower_bound()[i] - *tolerance ||
                val[i] > constraint->upper_bound()[i] + *tolerance) {
                descriptions.push_back(
                        d + "[" + std::to_string(i) + "]: " + std::to_string(constraint->lower_bound()[i]) +
                        " <= " + std::to_string(val[i]) + " <= " + std::to_string(constraint->upper_bound()[i]));
            }
        }
    }
    return descriptions;
...
std::vector<Binding<Constraint>> MathematicalProgramResult::GetInfeasibleConstraints(
        const MathematicalProgram& prog, std::optional<double> tolerance) const {
    ...
    for (const auto& binding : prog.GetAllConstraints()) {
        ...
        for (int i = 0; i < constraint->num_constraints(); ++i) {
            if (std::isnan(val(i)) || val(i) > constraint->upper_bound()(i) + *tolerance ||
