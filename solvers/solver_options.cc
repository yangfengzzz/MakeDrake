...
using CommonMap = std::unordered_map<CommonSolverOption, SolverOptions::OptionValue>;

void SolverOptions::SetOption(const SolverId& solver_id, const std::string& solver_option, double option_value) {
    solver_options_double_[solver_id][solver_option] = option_value;
}

void SolverOptions::SetOption(const SolverId& solver_id, const std::string& solver_option, int option_value) {
    ...
    switch (key) {
        case CommonSolverOption::kPrintToConsole: {
            if (!std::holds_alternative<int>(value)) {
                throw std::runtime_error(fmt::format("SolverOptions::SetOption support {} only with int value.", key));
            }
            const int int_value = std::get<int>(value);
            if (int_value != 0 && int_value != 1) {
                throw std::runtime_error(fmt::format("{} expects value either 0 or 1", key));
            }
        ...
        case CommonSolverOption::kPrintFileName: {
            if (!std::holds_alternative<std::string>(value)) {
                throw std::runtime_error(
                        fmt::format("SolverOptions::SetOption support {} only with std::string value.", key));
            }
            common_solver_options_[key] = std::move(value);
            return;
        }
...
template <typename T>
const std::unordered_map<std::string, T>& GetOptionsHelper(const SolverId& solver_id, const MapMap<T>& options) {
...
const std::unordered_map<std::string, double>& SolverOptions::GetOptionsDouble(const SolverId& solver_id) const {
    return GetOptionsHelper(solver_id, solver_options_double_);
}

const std::unordered_map<std::string, int>& SolverOptions::GetOptionsInt(const SolverId& solver_id) const {
    return GetOptionsHelper(solver_id, solver_options_int_);
}

const std::unordered_map<std::string, std::string>& SolverOptions::GetOptionsStr(const SolverId& solver_id) const {
...
std::string SolverOptions::get_standalone_reproduction_file_name() const {
    // N.B. SetOption sanity checks the value; we don't need to re-check here.
    std::string result;
    auto iter = common_solver_options_.find(CommonSolverOption::kStandaloneReproductionFileName);
    if (iter != common_solver_options_.end()) {
        result = std::get<std::string>(iter->second);
    }
    return result;
...
bool SolverOptions::operator==(const SolverOptions& other) const {
    return solver_options_double_ == other.solver_options_double_ && solver_options_int_ == other.solver_options_int_ &&
           solver_options_str_ == other.solver_options_str_ && common_solver_options_ == other.common_solver_options_;
}
...
void Summarize(const SolverId& id,
    ...
    for (const auto& keyval : keyvals) {
        (*pairs)[fmt::format("{}:{}", id.name(), keyval.first)] = fmt::format("{}", keyval.second);
    }
}
        ...
        for (const auto& keyval : x.common_solver_options()) {
            ...
            std::visit(
                    [key, &pairs](auto& val_x) {
                        pairs[fmt::format("CommonSolverOption::{}", key)] = fmt::format("{}", val_x);
                    },
                    val);
        }
...
template <typename T>
void CheckOptionKeysForSolverHelper(const std::unordered_map<std::string, T>& key_vals,
    ...
    for (const auto& key_val : key_vals) {
        if (!allowable_keys.contains(key_val.first)) {
            throw std::invalid_argument(key_val.first + " is not allowed in the SolverOptions for " + solver_name +
                                        ".");
        }
    }
}
...
void SolverOptions::CheckOptionKeysForSolver(const SolverId& solver_id,
    ...
    CheckOptionKeysForSolverHelper(GetOptionsDouble(solver_id), double_keys, solver_id.name());
    CheckOptionKeysForSolverHelper(GetOptionsInt(solver_id), int_keys, solver_id.name());
    CheckOptionKeysForSolverHelper(GetOptionsStr(solver_id), str_keys, solver_id.name());
}
