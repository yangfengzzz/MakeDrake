...
#include "common/scoped_singleton.h"
#include "common/text_logging.h"
#include "math/eigen_sparse_triplet.h"
#include "solvers/aggregate_costs_constraints.h"
#include "solvers/gurobi_solver_internal.h"
...
std::string refman() {
    return fmt::format("https://www.gurobi.com/documentation/{}.{}/refman", GRB_VERSION_MAJOR, GRB_VERSION_MINOR);
}
...
void SetBoundingBoxDualSolution(
        ...
        const std::unordered_map<Binding<BoundingBoxConstraint>, std::pair<std::vector<int>, std::vector<int>>>&
                bb_con_dual_indices,
        MathematicalProgramResult* result) {
    for (const auto& binding : prog.bounding_box_constraints()) {
        ...
        std::tie(lower_dual_indices, upper_dual_indices) = bb_con_dual_indices.at(binding);
        for (int i = 0; i < binding.evaluator()->num_vars(); ++i) {
            if (lower_dual_indices[i] != -1 && gurobi_dual_solutions[lower_dual_indices[i]] >= 0) {
                // This lower bound is active since the reduced cost is non-negative.
                dual_sol(i) = gurobi_dual_solutions[lower_dual_indices[i]];
            } else if (upper_dual_indices[i] != -1 && gurobi_dual_solutions[upper_dual_indices[i]] <= 0) {
 ...
 */
void SetLinearConstraintDualSolutions(const MathematicalProgram& prog,
                                      const Eigen::VectorXd& gurobi_dual_solutions,
                                      const std::unordered_map<Binding<Constraint>, int>& constraint_dual_start_row,
                                      MathematicalProgramResult* result) {
    for (const auto& binding : prog.linear_equality_constraints()) {
        result->set_dual_solution(binding, gurobi_dual_solutions.segment(constraint_dual_start_row.at(binding),
                                                                         binding.evaluator()->num_constraints()));
    }

    for (const auto& binding : prog.linear_constraints()) {
        Eigen::VectorXd dual_solution = Eigen::VectorXd::Zero(binding.evaluator()->num_constraints());
        ...
        for (int i = 0; i < binding.evaluator()->num_constraints(); ++i) {
            if (!std::isinf(ub(i)) || !std::isinf(lb(i))) {
                if (!std::isinf(lb(i)) && std::isinf(ub(i))) {
                    ...
                    const double lower_bound_dual = gurobi_dual_solutions(gurobi_constraint_index);
                    const double upper_bound_dual = gurobi_dual_solutions(gurobi_constraint_index + 1);
                    ...
                    dual_solution(i) = std::abs(lower_bound_dual) > std::abs(upper_bound_dual) ? lower_bound_dual
                                                                                               : upper_bound_dual;
...
template <typename C>
void SetSecondOrderConeDualSolution(const std::vector<Binding<C>>& constraints,
...
void SetAllSecondOrderConeDualSolution(const MathematicalProgram& prog,
                                       GRBmodel* model,
                                       MathematicalProgramResult* result) {
    const int num_soc = prog.lorentz_cone_constraints().size() + prog.rotated_lorentz_cone_constraints().size();
    Eigen::VectorXd gurobi_qcp_dual_solutions(num_soc);
    GRBgetdblattrarray(model, GRB_DBL_ATTR_QCPI, 0, num_soc, gurobi_qcp_dual_solutions.data());

    int soc_count = 0;
    SetSecondOrderConeDualSolution(prog.lorentz_cone_constraints(), gurobi_qcp_dual_solutions, result, &soc_count);
    SetSecondOrderConeDualSolution(prog.rotated_lorentz_cone_constraints(), gurobi_qcp_dual_solutions, result,
                                   &soc_count);
}
...
int gurobi_callback(GRBmodel* model, void* cbdata, int where, void* usrdata) {
    GurobiCallbackInformation* callback_info = reinterpret_cast<GurobiCallbackInformation*>(usrdata);

    if (where == GRB_CB_POLLING) {
    } else if (where == GRB_CB_PRESOLVE) {
    } else if (where == GRB_CB_SIMPLEX) {
    } else if (where == GRB_CB_MIP) {
    } else if (where == GRB_CB_MIPSOL && callback_info->mip_sol_callback != nullptr) {
        // Extract variable values from Gurobi, and set the current
        // solution of the MathematicalProgram to these values.
        int error = GRBcbget(cbdata, where, GRB_CB_MIPSOL_SOL, callback_info->solver_sol_vector.data());
        if (error) {
            drake::log()->error("GRB error {} in MIPSol callback cbget: {}\n", error, GRBgeterrormsg(GRBgetenv(model)));
            return 0;
        }
        SetProgramSolutionVector(callback_info->is_new_variable, callback_info->solver_sol_vector,
                                 &(callback_info->prog_sol_vector));
        callback_info->result->set_x_val(callback_info->prog_sol_vector);

        GurobiSolver::SolveStatusInfo solve_status = GetGurobiSolveStatus(cbdata, where);

        callback_info->mip_sol_callback(*(callback_info->prog), solve_status);

    } else if (where == GRB_CB_MIPNODE && callback_info->mip_node_callback != nullptr) {
        int sol_status;
        int error = GRBcbget(cbdata, where, GRB_CB_MIPNODE_STATUS, &sol_status);
        if (error) {
            drake::log()->error("GRB error {} in MIPNode callback getting sol status: {}\n", error,
                                GRBgeterrormsg(GRBgetenv(model)));
            return 0;
        } else if (sol_status == GRB_OPTIMAL) {
            // Extract variable values from Gurobi, and set the current
            // solution of the MathematicalProgram to these values.
            error = GRBcbget(cbdata, where, GRB_CB_MIPSOL_SOL, callback_info->solver_sol_vector.data());
            if (error) {
                drake::log()->error("GRB error {} in MIPSol callback cbget: {}\n", error,
                                    GRBgeterrormsg(GRBgetenv(model)));
                return 0;
            ...
            callback_info->mip_node_callback(*(callback_info->prog), solve_status, &vals, &vars);
            ...
            if (vals.size() > 0) {
                std::vector<double> new_sol(callback_info->prog->num_vars(), GRB_UNDEFINED);
...
__attribute__((unused)) bool HasCorrectNumberOfVariables(GRBmodel* model, int num_vars_expected) {
 ...
 */
int AddLinearAndQuadraticCosts(GRBmodel* model, double* pconstant_cost, const MathematicalProgram& prog) {
    ...
    for (const auto& binding : prog.quadratic_costs()) {
        ...
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            constraint_variable_index[i] = prog.FindDecisionVariableIndex(binding.variables()(i));
        }
        ...
        for (int i = 0; i < Q.rows(); i++) {
            ...
            if (Qii != 0) {
                Q_nonzero_coefs.push_back(
                        Eigen::Triplet<double>(constraint_variable_index[i], constraint_variable_index[i], Qii));
            }
            for (int j = i + 1; j < Q.cols(); j++) {
                const double Qij = 0.5 * (Q(i, j) + Q(j, i));
                if (Qij != 0) {
                    Q_nonzero_coefs.push_back(
                            Eigen::Triplet<double>(constraint_variable_index[i], constraint_variable_index[j], Qij));
                }
            }
        }
        ...
        for (int i = 0; i < b.size(); i++) {
            if (b(i) != 0) {
                b_nonzero_coefs.push_back(Eigen::Triplet<double>(constraint_variable_index[i], 0, b(i)));
            }
        }
    }
    ...
    for (const auto& binding : prog.linear_costs()) {
        ...
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            b_nonzero_coefs.push_back(
                    Eigen::Triplet<double>(prog.FindDecisionVariableIndex(binding.variables()(i)), 0, a(i)));
        }
    }
    ...
    const int QPtermsError = GRBaddqpterms(model, static_cast<int>(Q_all_row.size()), Q_all_row_indices_int.data(),
                                           Q_all_col_indices_int.data(), Q_all_val.data());
    if (QPtermsError) {
        return QPtermsError;
    }
    for (int i = 0; i < static_cast<int>(linear_row.size()); i++) {
        const int LinearTermError =
                GRBsetdblattrarray(model, "Obj", linear_row_indices_int[i], 1, linear_val.data() + i);
        if (LinearTermError) {
...
int ProcessLinearConstraints(GRBmodel* model,
                             const MathematicalProgram& prog,
    ...
    for (const auto& binding : prog.linear_equality_constraints()) {
        ...
        const int error = internal::AddLinearConstraint(prog, model, constraint->get_sparse_A(),
                                                        constraint->lower_bound(), constraint->upper_bound(),
                                                        binding.variables(), true, num_gurobi_linear_constraints);
        if (error) {
            return error;
        }
    }
    ...
    for (const auto& binding : prog.linear_constraints()) {
        ...
        const int error = internal::AddLinearConstraint(prog, model, constraint->get_sparse_A(),
                                                        constraint->lower_bound(), constraint->upper_bound(),
                                                        binding.variables(), false, num_gurobi_linear_constraints);
        if (error) {
            return error;
        }
    }
...
template <typename T>
void SetOptionOrThrow(GRBenv* model_env, const std::string& option, const T& val) {
    ...
    if (error == GRB_ERROR_VALUE_OUT_OF_RANGE) {
        throw std::runtime_error(
                fmt::format("GurobiSolver(): '{}' is outside the parameter {}'s valid range", val, option));
    }
    ...
    if (error == GRB_ERROR_UNKNOWN_PARAMETER) {
        ...
        if (expected_type != nullptr) {
            throw std::runtime_error(fmt::format("GurobiSolver(): parameter {} should be a {} not a {}", option,
                                                 expected_type, actual_type));
        }

        // Otherwise, it was truly unknown not just wrongly-typed.
        throw std::runtime_error(
                fmt::format("GurobiSolver(): '{}' is an unknown parameter in Gurobi, check "
    ...
    throw std::runtime_error(
            fmt::format("GurobiSolver(): error code {}, cannot set option '{}' to value '{}', "
                        "check {}/parameters.html for all allowable options and values.",
                        error, option, val, refman()));
}

void SetSolution(GRBmodel* model,
                 GRBenv* model_env,
                 const MathematicalProgram& prog,
                 ...
                 const std::unordered_map<Binding<BoundingBoxConstraint>,
                                          std::pair<std::vector<int>, std::vector<int>>>& bb_con_dual_indices,
                 MathematicalProgramResult* result,
    ...
    SetProgramSolutionVector(is_new_variable, solver_sol_vector, &prog_sol_vector);
    ...
    if (!is_mip && compute_dual) {
        ...
        Eigen::VectorXd gurobi_dual_solutions = Eigen::VectorXd::Zero(num_gurobi_linear_constraints);
        GRBgetdblattrarray(model, GRB_DBL_ATTR_PI, 0, num_gurobi_linear_constraints, gurobi_dual_solutions.data());
        SetLinearConstraintDualSolutions(prog, gurobi_dual_solutions, constraint_dual_start_row, result);
    ...
    if (is_mip) {
        ...
        for (int solution_number = 0; solution_number < sol_count; ++solution_number) {
            ...
            error = GRBgetdblattrarray(model, "Xn", 0, num_total_variables, solver_sol_vector.data());
            DRAKE_DEMAND(!error);
...
public:
    License() {
        ...
        if (const char* filename = std::getenv("GRB_LICENSE_FILE")) {
            ...
            if (std::string_view{filename}.find("DRAKE_UNIT_TEST_NO_LICENSE") != std::string_view::npos) {
                return;
            }
        }
...
std::shared_ptr<GurobiSolver::License> GurobiSolver::AcquireLicense() {
    ...
    static never_destroyed<std::shared_ptr<void>> local_host_holder{
            IsGrbLicenseFileLocalHost() ? GetScopedSingleton<GurobiSolver::License>() : nullptr};
...
void GurobiSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("GurobiSolver doesn't support the feature of variable scaling.");
    }
    ...
    for (int i = 0; i < num_prog_vars; ++i) {
        switch (prog.decision_variable(i).get_type()) {
            ...
            case MathematicalProgram::VarType::BOOLEAN:
                throw std::runtime_error("Boolean variables should not be used with Gurobi solver.");
            ...
            case MathematicalProgram::VarType::RANDOM_EXPONENTIAL:
                throw std::runtime_error("Random variables should not be used with Gurobi solver.");
        }
    }
    ...
    std::unordered_map<Binding<BoundingBoxConstraint>, std::pair<std::vector<int>, std::vector<int>>>
            bb_con_dual_indices;
    ...
    for (const auto& binding : prog.bounding_box_constraints()) {
        ...
        bb_con_dual_indices.emplace(binding, std::make_pair(lower_dual_indices, upper_dual_indices));
    }
    ...
    internal::AddSecondOrderConeVariables(prog.lorentz_cone_constraints(), &is_new_variable, &num_gurobi_vars,
    ...
    internal::AddSecondOrderConeVariables(prog.rotated_lorentz_cone_constraints(), &is_new_variable, &num_gurobi_vars,
                                          &rotated_lorentz_cone_new_variable_indices, &gurobi_var_type, &xlow, &xupp);

    std::vector<std::vector<int>> l2norm_costs_lorentz_cone_variable_indices;
    internal::AddL2NormCostVariables(prog.l2norm_costs(), &is_new_variable, &num_gurobi_vars,
                                     &l2norm_costs_lorentz_cone_variable_indices, &gurobi_var_type, &xlow, &xupp);

    GRBmodel* model = nullptr;
    GRBnewmodel(env, &model, "gurobi_model", num_gurobi_vars, nullptr, &xlow[0], &xupp[0], gurobi_var_type.data(),
                nullptr);
    ...
    if (!error) {
        error = internal::AddL2NormCosts(prog, l2norm_costs_lorentz_cone_variable_indices, model,
                                         &num_gurobi_linear_constraints);
    }

    if (!error) {
        error = ProcessLinearConstraints(model, prog, &num_gurobi_linear_constraints, &constraint_dual_start_row);
    }

    // Add Lorentz cone constraints.
    if (!error) {
        error = internal::AddSecondOrderConeConstraints(prog, prog.lorentz_cone_constraints(),
    ...
    if (!error) {
        error = internal::AddSecondOrderConeConstraints(prog, prog.rotated_lorentz_cone_constraints(),
    ...
    if (!error) {
        SetOptionOrThrow(model_env, "LogToConsole", static_cast<int>(merged_options.get_print_to_console()));
    }
    ...
    if (!merged_options.GetOptionsInt(id()).contains("Threads")) {
        if (char* num_threads_str = std::getenv("GUROBI_NUM_THREADS")) {
            ...
            if (num_threads.has_value()) {
                ...
                static const logging::Warn log_once("Ignoring unparseable value '{}' for GUROBI_NUM_THREADS",
                                                    num_threads_str);
            }
        }
    }
    ...
    GRBgetintattr(model, GRB_INT_ATTR_NUMCONSTRS, &num_gurobi_linear_constraints_expected);
    DRAKE_DEMAND(num_gurobi_linear_constraints == num_gurobi_linear_constraints_expected);
    ...
    if (!error) {
        if (grb_write.has_value()) {
            ...
            if (error) {
                const std::string gurobi_version = fmt::format("{}.{}", GRB_VERSION_MAJOR, GRB_VERSION_MINOR);
                throw std::runtime_error(
                        fmt::format("GurobiSolver(): setting GRBwrite to {}, this is not supported. "
    ...
    GurobiSolverDetails& solver_details = result->SetSolverDetailsType<GurobiSolverDetails>();
    ...
    if (error) {
        ...
        SetSolution(model, model_env, prog, is_new_variable, num_prog_vars, is_mip, num_gurobi_linear_constraints,
                    constant_cost, constraint_dual_start_row, bb_con_dual_indices, result, &solver_details);
        int optimstatus = 0;
        GRBgetintattr(model, GRB_INT_ATTR_STATUS, &optimstatus);
    ...
    error = GRBgetdblattr(model, GRB_DBL_ATTR_RUNTIME, &(solver_details.optimizer_time));
