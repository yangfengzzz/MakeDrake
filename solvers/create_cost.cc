...
Binding<QuadraticCost> DoParseQuadraticCost(const symbolic::Polynomial& poly,
    ...
    return CreateBinding(make_shared<QuadraticCost>(Q, b, constant_term, is_convex), vars_vec);
}

Binding<LinearCost> DoParseLinearCost(const Expression& e,
                                      const VectorXDecisionVariable& vars_vec,
                                      const unordered_map<Variable::Id, int>& map_var_to_index) {
    Eigen::RowVectorXd c(vars_vec.size());
    double constant_term{};
    symbolic::DecomposeAffineExpression(e, map_var_to_index, &c, &constant_term);
    return CreateBinding(make_shared<LinearCost>(c.transpose(), constant_term), vars_vec);
}
...
Binding<QuadraticCost> ParseQuadraticCost(const Expression& e, std::optional<bool> is_convex) {
...
Binding<PolynomialCost> ParsePolynomialCost(const symbolic::Expression& e) {
    ...
    return CreateBinding(make_shared<PolynomialCost>(Vector1<Polynomiald>(polynomial), polynomial_vars), var_vec);
}

Binding<L2NormCost> ParseL2NormCost(const symbolic::Expression& e, double psd_tol, double coefficient_tol) {
    auto [is_l2norm, A, b, vars] = DecomposeL2NormExpression(e, psd_tol, coefficient_tol);
    if (!is_l2norm) {
        throw runtime_error(
                fmt::format("Expression {} is not an L2 norm. ParseL2NormCost only supports "
