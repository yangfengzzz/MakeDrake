...
void GetEqualityConstrainedQPSolverOptions(const SolverOptions& solver_options,
                                           EqualityConstrainedQPSolverOptions* equality_qp_solver_options) {
    DRAKE_ASSERT_VOID(solver_options.CheckOptionKeysForSolver(
            EqualityConstrainedQPSolver::id(), {EqualityConstrainedQPSolver::FeasibilityTolOptionName()}, {}, {}));

    const auto& options_double = solver_options.GetOptionsDouble(EqualityConstrainedQPSolver::id());
    auto it = options_double.find(EqualityConstrainedQPSolver::FeasibilityTolOptionName());
    if (it != options_double.end()) {
        if (it->second >= 0) {
            equality_qp_solver_options->feasibility_tol = it->second;
        } else {
            throw std::invalid_argument("FeasibilityTol should be a non-negative number.");
        }
    }
}
...
void SetDualSolutions(const MathematicalProgram& prog,
    ...
    for (const auto& binding : prog.linear_equality_constraints()) {
        result->set_dual_solution(binding,
                                  dual_solutions.segment(num_constraints, binding.evaluator()->num_constraints()));
...
void EqualityConstrainedQPSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (num_constraints > 0) {
        ...
        for (auto const& binding : prog.linear_equality_constraints()) {
            ...
            for (int i = 0; i < num_v_variables; ++i) {
                ...
                for (Eigen::SparseMatrix<double>::InnerIterator it(bc->get_sparse_A(), i); it; ++it) {
                    A(constraint_index + it.row(), variable_index) = it.value();
                }
            }

            b.segment(constraint_index, n) = bc->lower_bound().segment(0, n);  // = c->upper_bound() since it's
        ...
        if (llt.info() == Eigen::Success) {
            ...
            solution_result = rhs.isApprox(A_iG_A_T * lambda, solver_options_struct.feasibility_tol)
            ...
            Eigen::JacobiSVD<Eigen::MatrixXd> svd_A_thin(A, Eigen::ComputeThinU | Eigen::ComputeThinV);
            ...
            if (!b.isApprox(A * x0, solver_options_struct.feasibility_tol)) {
                ...
                if (svd_A_thin.rank() == A.cols()) {
                    ...
                    // for the null space.
                    Eigen::ColPivHouseholderQR<Eigen::MatrixXd> qr_A(A.transpose());
                    const Eigen::MatrixXd Q = qr_A.householderQ().setLength(qr_A.nonzeroPivots());
                    const Eigen::MatrixXd N = Q.rightCols(A.cols() - qr_A.rank());
                    Eigen::VectorXd y(N.cols());
                    solution_result =
                            SolveUnconstrainedQP(N.transpose() * G * N, x0.transpose() * G * N + c.transpose() * N,
        ...
        // num_constraints = 0
        solution_result = SolveUnconstrainedQP(G, c, solver_options_struct.feasibility_tol, &x);
    }
...
bool EqualityConstrainedQPSolver::ProgramAttributesSatisfied(const MathematicalProgram& prog) {
    static const never_destroyed<ProgramAttributes> solver_capabilities(
            std::initializer_list<ProgramAttribute>{ProgramAttribute::kQuadraticCost, ProgramAttribute::kLinearCost,
