...
#include "solvers/scs_clarabel_common.h"

namespace drake {
namespace solvers {
namespace {

void ParseQuadraticCostWithRotatedLorentzCone(const MathematicalProgram& prog,
                                              std::vector<double>* c,
    ...
    for (const auto& cost : prog.quadratic_costs()) {
        ...
        const Eigen::MatrixXd C = math::DecomposePSDmatrixIntoXtransposeTimesX(Q, 1E-10);
        ...
        internal::ParseRotatedLorentzConeConstraint(Ai_triplets, b_cone, Ai_var_indices, A_triplets, b, A_row_count,
    ...
    for (const auto& bounding_box_constraint : prog.bounding_box_constraints()) {
        ...
        for (int i = 0; i < num_xi_rows; ++i) {
            ...
            if (!std::isinf(bounding_box_constraint.evaluator()->upper_bound()(i))) {
                ...
                // cone.
                A_triplets->emplace_back(num_scs_new_constraint + *A_row_count, prog.FindDecisionVariableIndex(xi(i)),
                                         1);
...
void SetScsProblemData(int A_row_count,
                       int num_vars,
    ...
    scs_problem_data->A->x = static_cast<scs_float*>(scs_calloc(A.nonZeros(), sizeof(scs_float)));

    // This scs_calloc doesn't need to accompany a ScopeExit since
    // scs_problem_data->A->i will be cleaned up recursively by freeing up
    // scs_problem_data in scs_free_data()
    scs_problem_data->A->i = static_cast<scs_int*>(scs_calloc(A.nonZeros(), sizeof(scs_int)));

    // This scs_calloc doesn't need to accompany a ScopeExit since
    // scs_problem_data->A->p will be cleaned up recursively by freeing up
    // scs_problem_data in scs_free_data()
    scs_problem_data->A->p = static_cast<scs_int*>(scs_calloc(scs_problem_data->n + 1, sizeof(scs_int)));
    ...
    // scs_problem_data in scs_free_data()
    scs_problem_data->b = static_cast<scs_float*>(scs_calloc(b.size(), sizeof(scs_float)));
    ...
    if (P_upper_triplets.empty()) {
        ...
        // scs_problem_data in scs_free_data()
        scs_problem_data->P->x = static_cast<scs_float*>(scs_calloc(P_upper.nonZeros(), sizeof(scs_float)));

        // This scs_calloc doesn't need to accompany a ScopeExit since
        // scs_problem_data->P->i will be cleaned up recursively by freeing up
        // scs_problem_data in scs_free_data()
        scs_problem_data->P->i = static_cast<scs_int*>(scs_calloc(P_upper.nonZeros(), sizeof(scs_int)));

        // This scs_calloc doesn't need to accompany a ScopeExit since
        // scs_problem_data->P->p will be cleaned up recursively by freeing up
        // scs_problem_data in scs_free_data()
        scs_problem_data->P->p = static_cast<scs_int*>(scs_calloc(scs_problem_data->n + 1, sizeof(scs_int)));
    ...
    // scs_problem_data in scs_free_data()
    scs_problem_data->c = static_cast<scs_float*>(scs_calloc(num_vars, sizeof(scs_float)));
...
// This should be invoked only once on each unique instance of ScsSettings.
// Namely, only call this function for once in DoSolve.
void SetScsSettings(std::unordered_map<std::string, double>* solver_options_double, ScsSettings* scs_settings) {
...
void SetBoundingBoxDualSolution(const MathematicalProgram& prog,
                                const Eigen::Ref<const Eigen::VectorXd>& y,
                                const std::vector<std::vector<std::pair<int, int>>>& bbcon_dual_indices,
                                MathematicalProgramResult* result) {
    for (int i = 0; i < static_cast<int>(prog.bounding_box_constraints().size()); ++i) {
        Eigen::VectorXd bbcon_dual = Eigen::VectorXd::Zero(prog.bounding_box_constraints()[i].variables().rows());
        for (int j = 0; j < prog.bounding_box_constraints()[i].variables().rows(); ++j) {
...
void ScsSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("ScsSolver doesn't support the feature of variable scaling.");
    }
    ...
    ScsData* scs_problem_data = static_cast<ScsData*>(scs_calloc(1, sizeof(ScsData)));
    ScsSettings* scs_stgs = static_cast<ScsSettings*>(scs_calloc(1, sizeof(ScsSettings)));
    ...
    internal::ParseLinearEqualityConstraints(prog, &A_triplets, &b, &A_row_count, &linear_eq_y_start_indices,
    ...
    std::vector<std::vector<std::pair<int, int>>> bbcon_dual_indices;
    ParseBoundingBoxConstraint(prog, &A_triplets, &b, &A_row_count, cone, &bbcon_dual_indices);
    ...
    internal::ParseLinearConstraints(prog, &A_triplets, &b, &A_row_count, &linear_constraint_dual_indices,
    ...
    internal::ParseSecondOrderConeConstraints(prog, &A_triplets, &b, &A_row_count, &second_order_cone_length,
    ...
    internal::ParseL2NormCosts(prog, &num_x, &A_triplets, &b, &A_row_count, &second_order_cone_length,
                               &l2norm_costs_lorentz_cone_y_start_indices, &c, &l2norm_costs_t_slack_indices);
    ...
    if (A_triplets.empty() && prog.positive_semidefinite_constraints().empty() &&
        prog.linear_matrix_inequality_constraints().empty() && prog.exponential_cone_constraints().empty()) {
        ...
        ParseQuadraticCostWithRotatedLorentzCone(prog, &c, &A_triplets, &b, &A_row_count, &second_order_cone_length,
                                                 &num_x);
    } else {
    ...
    internal::ParsePositiveSemidefiniteConstraints(prog, /* upper_triangular = */ false, &A_triplets, &b, &A_row_count,
                                                   &psd_cone_length);
    ...
    internal::ParseExponentialConeConstraints(prog, &A_triplets, &b, &A_row_count);
    ...
    SetScsProblemData(A_row_count, num_x, A, b, P_upper_triplets, c, scs_problem_data);
    std::unordered_map<std::string, int> input_solver_options_int = merged_options.GetOptionsInt(id());
    std::unordered_map<std::string, double> input_solver_options_double = merged_options.GetOptionsDouble(id());
    SetScsSettings(&input_solver_options_int, merged_options.get_print_to_console(), scs_stgs);
    SetScsSettings(&input_solver_options_double, scs_stgs);

    ScsInfo scs_info{0};

    ScsSolution* scs_sol = static_cast<ScsSolution*>(scs_calloc(1, sizeof(ScsSolution)));
    ScopeExit sol_guard([&scs_sol]() {
        SCS(free_sol)(scs_sol);
    });

    ScsSolverDetails& solver_details = result->SetSolverDetailsType<ScsSolverDetails>();
    solver_details.scs_status = scs(scs_problem_data, cone, scs_stgs, scs_sol, &scs_info);
    ...
    result->set_x_val((Eigen::Map<VectorX<scs_float>>(scs_sol->x, prog.num_vars())).cast<double>());
    SetBoundingBoxDualSolution(prog, solver_details.y, bbcon_dual_indices, result);
    internal::SetDualSolution(prog, solver_details.y, linear_constraint_dual_indices, linear_eq_y_start_indices,
                              lorentz_cone_y_start_indices, rotated_lorentz_cone_y_start_indices, result);
    // Set the solution_result enum and the optimal cost based on SCS status.
    if (solver_details.scs_status == SCS_SOLVED || solver_details.scs_status == SCS_SOLVED_INACCURATE) {
        solution_result = SolutionResult::kSolutionFound;
        result->set_optimal_cost(scs_info.pobj + cost_constant);
    } else if (solver_details.scs_status == SCS_UNBOUNDED || solver_details.scs_status == SCS_UNBOUNDED_INACCURATE) {
        solution_result = SolutionResult::kUnbounded;
        result->set_optimal_cost(MathematicalProgram::kUnboundedCost);
    } else if (solver_details.scs_status == SCS_INFEASIBLE || solver_details.scs_status == SCS_INFEASIBLE_INACCURATE) {
