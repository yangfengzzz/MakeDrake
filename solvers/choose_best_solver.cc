...
public:
    ...
    static constexpr StaticSolverInterface Make() {
        ...
        result.are_program_attributes_satisfied_ = &SomeSolver::ProgramAttributesSatisfied;
...
private:
    ...
    bool (*are_program_attributes_satisfied_)(const MathematicalProgram& prog) = nullptr;
...
constexpr std::array<StaticSolverInterface, 13> kKnownSolvers{
        StaticSolverInterface::Make<ClarabelSolver>(),     StaticSolverInterface::Make<ClpSolver>(),
        StaticSolverInterface::Make<CsdpSolver>(),         StaticSolverInterface::Make<EqualityConstrainedQPSolver>(),
        StaticSolverInterface::Make<GurobiSolver>(),       StaticSolverInterface::Make<IpoptSolver>(),
        StaticSolverInterface::Make<LinearSystemSolver>(), StaticSolverInterface::Make<MobyLCPSolver<double>>(),
        StaticSolverInterface::Make<MosekSolver>(),        StaticSolverInterface::Make<NloptSolver>(),
        StaticSolverInterface::Make<OsqpSolver>(),         StaticSolverInterface::Make<ScsSolver>(),
...
using KnownSolversMap = std::unordered_map<SolverId, const StaticSolverInterface*>;
...
SolverId ChooseFirstMatchingSolver(const MathematicalProgram& prog,
    ...
    throw std::invalid_argument(
            fmt::format("There is no available solver for the optimization program{}", additional_error_message));
}

template <typename SomeSolver>
void AddSolverIfAvailable(absl::InlinedVector<SolverId, kNumberOfSolvers>* solver_ids) {
...
void AddSolversIfAvailable(absl::InlinedVector<SolverId, kNumberOfSolvers>* solver_ids) {
...
void GetAvailableSolversHelper(ProgramType prog_type,
                               bool conservative,
    ...
    switch (prog_type) {
        ...
        case ProgramType::kUnknown: {
            if (!conservative) {
                ...
                AddSolversIfAvailable<LinearSystemSolver, EqualityConstrainedQPSolver, MosekSolver, GurobiSolver,
                                      ClarabelSolver, OsqpSolver, ClpSolver, MobyLCPSolver<double>, SnoptSolver,
                                      IpoptSolver, NloptSolver, CsdpSolver, ScsSolver>(result);
            }
            return;
        }
    }
...
SolverId ChooseBestSolver(const MathematicalProgram& prog) {
    ...
    GetAvailableSolversHelper(program_type, false /* conservative=false*/, &solver_ids);
    switch (program_type) {
        case ProgramType::kLP:
        case ProgramType::kQP:
        case ProgramType::kSOCP:
...
std::unique_ptr<SolverInterface> MakeFirstAvailableSolver(const std::vector<SolverId>& solver_ids) {
    ...
    throw std::runtime_error("MakeFirstAvailableSolver(): none of the solvers " + solver_names +
                             "is available and enabled.");
}

std::vector<SolverId> GetAvailableSolvers(ProgramType prog_type) {
    absl::InlinedVector<SolverId, kNumberOfSolvers> solver_ids;
    GetAvailableSolversHelper(prog_type, true /* conservative=true */, &solver_ids);
