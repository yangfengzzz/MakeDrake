...
void ParseProgram(const MathematicalProgram* prog,
...
AugmentedLagrangianNonsmooth::AugmentedLagrangianNonsmooth(const MathematicalProgram* prog, bool include_x_bounds)
    : prog_{prog}, include_x_bounds_{include_x_bounds} {
    ParseProgram(prog_, include_x_bounds, &lagrangian_size_, &is_equality_, &x_lo_, &x_up_);
}

namespace {
template <typename AL, typename T>
T EvalAugmentedLagrangian(const AL& al,
        ...
        if (!dynamic_cast<BoundingBoxConstraint*>(constraint.evaluator().get())) {
            ...
            for (int i = 0; i < constraint.evaluator()->num_constraints(); ++i) {
                ...
                if ((std::isinf(lb) && lb > 0) || (std::isinf(ub) && ub < 0)) {
                    throw std::invalid_argument(fmt::format("constraint lower bound is {}, upper bound is {}", lb, ub));
                }
                if (lb == ub) {
                    // We have one Lagrangian multiplier for the equality constraint. Add
                    // −λ*h(x) + μ/2*h(x)² to the augmented Lagrangian.
                    al_val += al_for_equality(constraint_val(i) - lb, lambda_val(lagrangian_count), mu);
                    ...
                    if (!std::isinf(lb)) {
                        ...
                        if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
                            al_val += psi(constraint_val(i) - lb, lambda_val(lagrangian_count), mu);
                            (*constraint_residue)(lagrangian_count) = constraint_val(i) - lb;
                        } else {
                            al_val += al_for_equality(constraint_val(i) - s(s_count) - lb, lambda_val(lagrangian_count),
                                                      mu);
                            (*constraint_residue)(lagrangian_count) = constraint_val(i) - s(s_count) - lb;
                            s_count++;
                        }
                        lagrangian_count++;
                    }
                    if (!std::isinf(ub)) {
                        ...
                        if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
                            al_val += psi(ub - constraint_val(i), lambda_val(lagrangian_count), mu);
                            (*constraint_residue)(lagrangian_count) = ub - constraint_val(i);
                        } else {
                            al_val += al_for_equality(ub - constraint_val(i) - s(s_count), lambda_val(lagrangian_count),
                                                      mu);
                            (*constraint_residue)(lagrangian_count) = ub - constraint_val(i) - s(s_count);
                            s_count++;
                        }
                        lagrangian_count++;
                    }
    ...
    if (al.include_x_bounds()) {
        for (int i = 0; i < al.prog().num_vars(); ++i) {
            if (al.x_lo()(i) == al.x_up()(i)) {
                al_val += al_for_equality(x(i) - al.x_lo()(i), lambda_val(lagrangian_count), mu);
                (*constraint_residue)(lagrangian_count) = x(i) - al.x_lo()(i);
                lagrangian_count++;
            } else {
                if (!std::isinf(al.x_lo()(i))) {
                    if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
                        al_val += psi(x(i) - al.x_lo()(i), lambda_val(lagrangian_count), mu);
                        (*constraint_residue)(lagrangian_count) = x(i) - al.x_lo()(i);
                    } else {
                        al_val += al_for_equality(x(i) - al.x_lo()(i) - s(s_count), lambda_val(lagrangian_count), mu);
                        (*constraint_residue)(lagrangian_count) = x(i) - al.x_lo()(i) - s(s_count);
                    }
                    s_count++;
                    lagrangian_count++;
                }
                if (!std::isinf(al.x_up()(i))) {
                    if constexpr (std::is_same_v<AL, AugmentedLagrangianNonsmooth>) {
                        al_val += psi(al.x_up()(i) - x(i), lambda_val(lagrangian_count), mu);
                        (*constraint_residue)(lagrangian_count) = al.x_up()(i) - x(i);
                    } else {
                        al_val += al_for_equality(al.x_up()(i) - x(i) - s(s_count), lambda_val(lagrangian_count), mu);
                        (*constraint_residue)(lagrangian_count) = al.x_up()(i) - x(i) - s(s_count);
                        s_count++;
                    }
                    lagrangian_count++;
                }
...
T AugmentedLagrangianNonsmooth::Eval(const Eigen::Ref<const VectorX<T>>& x,
    ...
    return EvalAugmentedLagrangian<AugmentedLagrangianNonsmooth, T>(*this, x, s_dummy, lambda_val, mu,
                                                                    constraint_residue, cost);
}

AugmentedLagrangianSmooth::AugmentedLagrangianSmooth(const MathematicalProgram* prog, bool include_x_bounds)
    : prog_{prog}, include_x_bounds_{include_x_bounds} {
    ParseProgram(prog_, include_x_bounds, &lagrangian_size_, &is_equality_, &x_lo_, &x_up_);
...
T AugmentedLagrangianSmooth::Eval(const Eigen::Ref<const VectorX<T>>& x,
                                  ...
                                  VectorX<T>* constraint_residue,
                                  T* cost) const {
    return EvalAugmentedLagrangian<AugmentedLagrangianSmooth, T>(*this, x, s, lambda_val, mu, constraint_residue, cost);
}

// Explicit instantiation.
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&AugmentedLagrangianNonsmooth::Eval<T>,
