...
public:
    ...
    Binding(const std::shared_ptr<C>& c, const Eigen::Ref<const VectorXDecisionVariable>& v) : evaluator_(c), vars_(v) {
     ...
     * the constraint @p c.
     */
    Binding(const std::shared_ptr<C>& c, const VariableRefList& v) : evaluator_(c) {
    ...
    [[nodiscard]] const std::shared_ptr<C>& evaluator() const { return evaluator_; }

    [[nodiscard]] const VectorXDecisionVariable& variables() const { return vars_; }
     ...
     * evaluator.ToLatex() + "$$". */
    std::string ToLatex(int precision = 3) const { return evaluator()->ToLatex(variables(), precision); }
    ...
    [[nodiscard]] bool operator!=(const Binding<C>& other) const { return !((*this) == (other)); }

    /** Implements the @ref hash_append concept. */
    template <class HashAlgorithm>
    friend void hash_append(HashAlgorithm& hasher, const Binding<C>& item) noexcept {
...
[[nodiscard]] Binding<C> CreateBinding(const std::shared_ptr<C>& c, Args&&... args) {
