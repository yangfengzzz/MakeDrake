...
public:
    typedef size_t Id;
    MatrixVariableEntry(MSKint64t bar_matrix_index, MSKint32t row_index, MSKint32t col_index, int num_matrix_rows)
    ...
    int IndexInLowerTrianglePart() const {
        return (2 * num_matrix_rows_ - col_index_ + 1) * col_index_ / 2 + row_index_ - col_index_;
    }
...
using DualMap = std::unordered_map<Binding<ConstraintType>, ConstraintDualIndices>;
...
public:
    ...
    std::unordered_map<int, MatrixVariableEntry> decision_variable_to_mosek_matrix_variable{};
    ...
    std::unordered_map<int, std::vector<MatrixVariableEntry>> matrix_variable_entries_for_same_decision_variable{};
};
...
public:
    ...
    MSKrescodee AddMatrixVariableEntryCoefficientMatrixIfNonExistent(const MatrixVariableEntry& matrix_variable_entry,
                                                                     MSKint64t* E_index);
    ...
    MSKrescodee AddScalarTimesMatrixVariableEntryToMosek(MSKint32t constraint_index,
    ...
    MSKrescodee SetMosekLinearConstraintBound(int linear_constraint_index,
                                              double lower,
                                              double upper,
    ...
    // upper.
    MSKrescodee AddLinearConstraintToMosek(const MathematicalProgram& prog,
    ...
    MSKrescodee ParseLinearExpression(
            ...
            std::vector<std::unordered_map<MSKint64t, std::pair<std::vector<MSKint64t>, std::vector<MSKrealt>>>>*
                    bar_F);
    ...
    // indices.
    template <typename C>
    MSKrescodee AddLinearConstraintsFromBindings(const std::vector<Binding<C>>& constraint_list,
                                                 LinearConstraintBoundType bound_type,
    ...
    // index.
    MSKrescodee AddQuadraticConstraints(const MathematicalProgram& prog,
     ...
     */
    MSKrescodee AddAffineConeConstraint(const MathematicalProgram& prog,
                                        const Eigen::SparseMatrix<double>& A,
                                        const Eigen::SparseMatrix<double>& B,
                                        const VectorX<symbolic::Variable>& decision_vars,
                                        const std::vector<MSKint32t>& slack_vars_mosek_indices,
     ...
     */
    template <typename C>
    MSKrescodee AddConeConstraints(const MathematicalProgram& prog,
    ...
    MSKrescodee AddLinearMatrixInequalityConstraint(
            const MathematicalProgram& prog,
            std::unordered_map<Binding<LinearMatrixInequalityConstraint>, MSKint64t>* lmi_acc_indices);

    MSKrescodee AddLinearCost(const Eigen::SparseVector<double>& linear_coeff,
                              const VectorX<symbolic::Variable>& linear_vars,
                              const MathematicalProgram& prog);

    MSKrescodee AddQuadraticCostAsLinearCost(const Eigen::SparseMatrix<double>& Q_lower,
                                             const VectorX<symbolic::Variable>& quadratic_vars,
                                             const MathematicalProgram& prog);

    MSKrescodee AddQuadraticCost(const Eigen::SparseMatrix<double>& Q_quadratic_vars,
                                 const VectorX<symbolic::Variable>& quadratic_vars,
                                 const MathematicalProgram& prog);
    ...
    MSKrescodee SpecifyVariableType(const MathematicalProgram& prog, bool* with_integer_or_binary_variables);
    ...
    const std::unordered_map<MatrixVariableEntry::Id, MSKint64t>& matrix_variable_entry_to_selection_matrix_id() const {
        return matrix_variable_entry_to_selection_matrix_id_;
    }

    const std::unordered_map<int, MatrixVariableEntry>& decision_variable_to_mosek_matrix_variable() const {
        return map_decision_var_to_mosek_var_.decision_variable_to_mosek_matrix_variable;
    }

    const std::unordered_map<int, int>& decision_variable_to_mosek_nonmatrix_variable() const {
        return map_decision_var_to_mosek_var_.decision_variable_to_mosek_nonmatrix_variable;
    }

    const std::unordered_map<int, std::vector<MatrixVariableEntry>>&
    matrix_variable_entries_for_same_decision_variable() const {
        return map_decision_var_to_mosek_var_.matrix_variable_entries_for_same_decision_variable;
    }

private:
    MSKtask_t task_{nullptr};
    std::unordered_map<MatrixVariableEntry::Id, MSKint64t> matrix_variable_entry_to_selection_matrix_id_{};
...
MSKrescodee MosekSolverProgram::AddLinearConstraintsFromBindings(
    ...
    for (const auto& binding : constraint_list) {
        ...
        rescode = AddLinearConstraintToMosek(prog, A, B_zero, lb, ub, binding.variables(), {}, bound_type);
        if (rescode != MSK_RES_OK) {
            return rescode;
        }
...
template <typename C>
MSKrescodee MosekSolverProgram::AddConeConstraints(const MathematicalProgram& prog,
                                                   const std::vector<Binding<C>>& cone_constraints,
                                                   std::unordered_map<Binding<C>, MSKint64t>* acc_indices) {
    static_assert(std::is_same_v<C, LorentzConeConstraint> || std::is_same_v<C, RotatedLorentzConeConstraint> ||
    ...
    for (auto const& binding : cone_constraints) {
        ...
        if (is_rotated_cone) {
            ...
            b(0) *= 0.5;
            rescode = this->AddAffineConeConstraint(prog, A, Eigen::SparseMatrix<double>(A.rows(), 0),
            ...
            rescode = this->AddAffineConeConstraint(
                    prog, binding.evaluator()->A(), Eigen::SparseMatrix<double>(binding.evaluator()->A().rows(), 0),
                    binding.variables(), {}, binding.evaluator()->b(), cone_type, &acc_index);
            if (rescode != MSK_RES_OK) {
                return rescode;
            }
        }
...
void SetLinearConstraintDualSolution(
        ...
        const std::vector<MSKrealt>& slc,
        const std::vector<MSKrealt>& suc,
        const std::unordered_map<Binding<C>, ConstraintDualIndices>& linear_con_dual_indices,
        MathematicalProgramResult* result) {
    for (const auto& binding : bindings) {
        const ConstraintDualIndices duals = linear_con_dual_indices.at(binding);
        Eigen::VectorXd dual_sol = Eigen::VectorXd::Zero(binding.evaluator()->num_constraints());
 ...
 */
template <typename C>
MSKrescodee SetAffineConeConstraintDualSolution(const std::vector<Binding<C>>& bindings,
                                                MSKtask_t task,
                                                MSKsoltypee whichsol,
