...
#include "tools/workspace/clarabel_cpp_internal/serialize.h"
...
void ParseBoundingBoxConstraints(const MathematicalProgram& prog,
    ...
    for (int i = 0; i < ssize(prog.bounding_box_constraints()); ++i) {
        bbcon_dual_indices->emplace_back(prog.bounding_box_constraints()[i].variables().rows(),
                                         std::pair<int, int>(-1, -1));
        for (int j = 0; j < prog.bounding_box_constraints()[i].variables().rows(); ++j) {
            const int var_index = prog.FindDecisionVariableIndex(prog.bounding_box_constraints()[i].variables()(j));
            const Bound& var_bound = variable_bounds.at(prog.bounding_box_constraints()[i].variables()(j));
            // We use the lower bound of this constraint in the optimization
            // program.
            const bool use_lb = var_bound.lower == prog.bounding_box_constraints()[i].evaluator()->lower_bound()(j) &&
                                std::isfinite(var_bound.lower);
            const bool use_ub = var_bound.upper == prog.bounding_box_constraints()[i].evaluator()->upper_bound()(j) &&
...
void SetSolverDetails(const clarabel::DefaultSolution<double>& clarabel_solution,
...
public:
    ...
    explicit SettingsConverter(const SolverOptions& solver_options) {
        ...
        pending_options_double_ = solver_options.GetOptionsDouble(ClarabelSolver::id());
        ...
        if (unknown_names.size() > 0) {
            throw std::logic_error(
                    fmt::format("ClarabelSolver: unrecognized solver options {}. Please check "
    ...
    const clarabel::DefaultSettings<double>& settings() const { return settings_; }

    void Visit(const NameValue<double>& x) { this->SetFromDoubleMap(x.name(), x.value()); }
    ...
    void Visit(const NameValue<clarabel::ClarabelCliqueMergeMethods>& x) {
        DRAKE_THROW_UNLESS(x.name() == std::string{"chordal_decomposition_merge_method"});
        // TODO(jwnimmer-tri) Add support for this option.
        // For now it is unsupported and will throw (as an unknown name, below).
    }
...
private:
    ...
    clarabel::DefaultSettings<double> settings_ = clarabel::DefaultSettingsBuilder<double>::default_settings().build();
};
...
void WriteClarabelReproduction(std::string filename,
    ...
    auto write_csc = [&](const Eigen::SparseMatrix<double>& mat, const std::string& name) {
        if (mat.nonZeros() == 0) {
            out_file << fmt::format("{} = sparse.csc_matrix(({}, {}))", name, P.rows(), P.cols()) << std::endl;
            return;
        }
...
{} = sparse.csc_matrix((data, (rows, cols)))
                                ...
                                fmt::join(data, ", "), fmt::join(rows, ", "), fmt::join(cols, ", "), name);
    };
    ...
    for (const clarabel::SupportedConeT<double>& cone : cones) {
        switch (cone.tag) {
            case clarabel::SupportedConeT<double>::Tag::ZeroConeT:
                out_file << "  clarabel.ZeroConeT(" << cone.nvars() << ")," << std::endl;
                break;
            case clarabel::SupportedConeT<double>::Tag::NonnegativeConeT:
                out_file << "  clarabel.NonnegativeConeT(" << cone.nvars() << ")," << std::endl;
                break;
            case clarabel::SupportedConeT<double>::Tag::SecondOrderConeT:
                out_file << "  clarabel.SecondOrderConeT(" << cone.nvars() << ")," << std::endl;
                break;
            case clarabel::SupportedConeT<double>::Tag::PSDTriangleConeT: {
                ...
                out_file << "  clarabel.PSDTriangleConeT(" << psd_cone->dimension() << ")," << std::endl;
            } break;
...
void ClarabelSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("ClarabelSolver doesn't support the feature of variable scaling.");
    }
    ...
    internal::ParseL2NormCosts(prog, &num_x, &A_triplets, &b, &A_row_count, &l2norm_costs_second_order_cone_length,
                               &l2norm_costs_lorentz_cone_y_start_indices, &q, &l2norm_costs_t_slack_indices);
    ...
    if (num_linear_equality_constraints_rows > 0) {
        cones.push_back(clarabel::ZeroConeT<double>(num_linear_equality_constraints_rows));
    }
    ...
    std::vector<std::vector<std::pair<int, int>>> bbcon_dual_indices;
    ParseBoundingBoxConstraints(prog, &A_triplets, &b, &A_row_count, &cones, &bbcon_dual_indices);
    ...
    if (num_linear_constraint_rows > 0) {
        cones.push_back(clarabel::NonnegativeConeT<double>(num_linear_constraint_rows));
    }
    ...
    internal::ParsePositiveSemidefiniteConstraints(prog, /* upper triangular = */ true, &A_triplets, &b, &A_row_count,
                                                   &psd_cone_length);
    ...
    std::string repro_file_name = merged_options.get_standalone_reproduction_file_name();
    ...
    ClarabelSolverDetails& solver_details = result->SetSolverDetailsType<ClarabelSolverDetails>();
    SetSolverDetails(solution, &solver_details);

    SolutionResult solution_result{SolutionResult::kSolutionResultNotSet};

    result->set_x_val(Eigen::Map<Eigen::VectorXd>(solution.x.data(), prog.num_vars()));

    SetBoundingBoxDualSolution(prog, solution.z, bbcon_dual_indices, result);
    internal::SetDualSolution(prog, solution.z, linear_constraint_dual_indices, linear_eq_y_start_indices,
    ...
    if (solution.status == clarabel::SolverStatus::Solved || solution.status == clarabel::SolverStatus::AlmostSolved) {
        ...
        result->set_optimal_cost(solution.obj_val + cost_constant);
               ...
               solution.status == clarabel::SolverStatus::AlmostPrimalInfeasible) {
        ...
        drake::log()->info("Clarabel returns {}", SolverStatusToString(solution.status));
