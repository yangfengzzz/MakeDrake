...
using ValueFunctionDoubleType = std::function<VectorX<double>(const VectorX<double>&, double)>;
using ValueFunctionAutodiffType = std::function<AutoDiffVecXd(const AutoDiffVecXd&, double)>;
...
void InitializeY(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd* y, int y_index, double y_value) {
    (*y)(y_index) = y_value;
}

void InitializeY(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y, int y_index, double y_value) {
    (*y)(y_index).value() = y_value;
    (*y)(y_index).derivatives() = Eigen::RowVectorXd::Zero(x(0).derivatives().size());
}

void Penalty(const double& value,
...
void Penalty(const AutoDiffXd& value,
             double minimum_value,
             double influence_value,
             const MinimumValuePenaltyFunction& penalty_function,
             AutoDiffXd* y) {
    const AutoDiffXd scaled_value_autodiff = ScaleValue(value, minimum_value, influence_value);

    double penalty, dpenalty_dscaled_value;
    penalty_function(scaled_value_autodiff.value(), &penalty, &dpenalty_dscaled_value);

    const Vector1<AutoDiffXd> penalty_autodiff = math::InitializeAutoDiff(
            Vector1d(penalty),
            dpenalty_dscaled_value * math::ExtractGradient(Vector1<AutoDiffXd>{scaled_value_autodiff}));
...
VectorX<double> Values(const Eigen::Ref<const VectorX<double>>& x,
    ...
    return value_function_double ? value_function_double(x, influence_value)
                                 : math::ExtractValue(value_function_ad(x.cast<AutoDiffXd>(), influence_value));
}
...
template <typename T>
void EvalPenalty(double bound_value,
                 const VectorX<T>& values,
                 int max_num_values,
                 double influence_value,
                 MinimumValuePenaltyFunction penalty_function,
                 double penalty_output_scaling,
    ...
    for (int i = 0; i < num_values; ++i) {
        ...
        if (value < influence_value) {
            ...
            Penalty(value, bound_value, influence_value, penalty_function, &(penalties.back()));
...
template <typename T>
void EvalGeneric(double bound,
                 int max_num_values,
                 double y_for_empty_value,
                 double influence_value,
                 const ValueFunctionDoubleType& value_function_double,
                 const ValueFunctionAutodiffType& value_function_ad,
    ...
    InitializeY(x, y, 0, smooth_func_double(std::vector<double>(max_num_values, 0.0)));

    VectorX<T> values = Values(x, influence_value, value_function_double, value_function_ad);
    const int num_values = static_cast<int>(values.size());
    DRAKE_ASSERT(num_values <= max_num_values);
    EvalPenalty<T>(bound, values, max_num_values, influence_value, penalty_function, penalty_output_scaling,
                   smooth_func, y->data());
}
}  // namespace
...
MinimumValueLowerBoundConstraint::MinimumValueLowerBoundConstraint(
        int num_vars,
        double minimum_value_lower,
        double influence_value_offset,
        int max_num_values,
        std::function<AutoDiffVecXd(const Eigen::Ref<const AutoDiffVecXd>&, double)> value_function,
        std::function<VectorX<double>(const Eigen::Ref<const VectorX<double>>&, double)> value_function_double)
...
void MinimumValueLowerBoundConstraint::set_penalty_function(MinimumValuePenaltyFunction new_penalty_function) {
    set_penalty_function_impl(new_penalty_function, &penalty_function_, &penalty_output_scaling_);
}

template <typename T>
void MinimumValueLowerBoundConstraint::DoEvalGeneric(const Eigen::Ref<const VectorX<T>>& x, VectorX<T>* y) const {
    // If we know that Values() will return at most zero values, then this
    // is a non-constraint. Set y=0 which trivially satisfies the constraint y
    // <= 1.
    const double y_for_empty_value = 0;
    EvalGeneric<T>(minimum_value_lower_, max_num_values_, y_for_empty_value, influence_value_, value_function_double_,
                   value_function_, SmoothOverMax<double>, SmoothOverMax<T>, penalty_function_, penalty_output_scaling_,
                   x, y);
}

void MinimumValueLowerBoundConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    DoEvalGeneric<double>(x, y);
}

void MinimumValueLowerBoundConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
...
void MinimumValueUpperBoundConstraint::set_penalty_function(MinimumValuePenaltyFunction new_penalty_function) {
    set_penalty_function_impl(new_penalty_function, &penalty_function_, &penalty_output_scaling_);
}

template <typename T>
void MinimumValueUpperBoundConstraint::DoEvalGeneric(const Eigen::Ref<const VectorX<T>>& x, VectorX<T>* y) const {
    // If we know that Values() will return at most zero values, then this
    // is a non-constraint. Set y=2 which always satisfies the constraint is y
    // >= 1.
    const double y_for_empty_value = 2;
    EvalGeneric<T>(minimum_value_upper_, max_num_values_, y_for_empty_value, influence_value_, value_function_double_,
                   value_function_, SmoothUnderMax<double>, SmoothUnderMax<T>, penalty_function_,
                   penalty_output_scaling_, x, y);
}

void MinimumValueUpperBoundConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    DoEvalGeneric<double>(x, y);
}

void MinimumValueUpperBoundConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    DoEvalGeneric<AutoDiffXd>(x, y);
}
}  // namespace solvers
}  // namespace drake
