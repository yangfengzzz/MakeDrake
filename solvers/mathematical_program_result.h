 ...
 */
double GetVariableValue(const symbolic::Variable& var,
...
typename std::enable_if_t<std::is_same_v<typename Derived::Scalar, symbolic::Variable>, MatrixLikewise<double, Derived>>
GetVariableValue(const Eigen::MatrixBase<Derived>& var,
                 const std::optional<std::unordered_map<symbolic::Variable::Id, int>>& variable_index,
                 const Eigen::Ref<const Eigen::VectorXd>& variable_values) {
    MatrixLikewise<double, Derived> value(var.rows(), var.cols());
    for (int i = 0; i < var.rows(); ++i) {
        for (int j = 0; j < var.cols(); ++j) {
            value(i, j) = GetVariableValue(var(i, j), variable_index, variable_values);
        }
    }
    return value;
}
...
public:
    ...
    void set_decision_variable_index(std::unordered_map<symbolic::Variable::Id, int> decision_variable_index) {
        ...
        x_val_ = Eigen::VectorXd::Constant(decision_variable_index_->size(), std::numeric_limits<double>::quiet_NaN());
    }
    ...
    /** Sets SolutionResult. */
    void set_solution_result(SolutionResult solution_result) { solution_result_ = solution_result; }
    ...
    const typename Solver::Details& get_solver_details() const {
        return get_abstract_solver_details().template get_value<typename Solver::Details>();
    }
    ...
    T& SetSolverDetailsType() {
        ...
        if (!solver_details_ || (solver_details_->static_type_info() != typeid(T))) {
     ...
     */
    [[nodiscard]] symbolic::Expression GetSolution(const symbolic::Expression& e) const;
     ...
     */
    [[nodiscard]] symbolic::Polynomial GetSolution(const symbolic::Polynomial& p) const;
     ...
     */
    template <typename Derived>
    [[nodiscard]] typename std::enable_if_t<std::is_same_v<typename Derived::Scalar, symbolic::Expression>,
     ...
     */
    template <typename C>
    [[nodiscard]] Eigen::VectorXd GetDualSolution(const Binding<C>& constraint) const {
        const Binding<Constraint> constraint_cast = internal::BindingDynamicCast<Constraint>(constraint);
        ...
        if (it == dual_solutions_.end()) {
            ...
            if (solver_id_.name() == "Gurobi") {
                throw std::invalid_argument(
                        fmt::format("You used {} to solve this optimization problem. If the problem is "
    ...
    Eigen::VectorXd EvalBinding(const Binding<Evaluator>& binding) const {
        ...
        for (int i = 0; i < binding_x.rows(); ++i) {
            binding_x(i) = x_val_(decision_variable_index_->at(binding.variables()(i).get_id()));
        }
    ...
    GetSuboptimalSolution(const Eigen::MatrixBase<Derived>& var, int solution_number) const {
        return GetVariableValue(var, decision_variable_index_, suboptimal_x_val_[solution_number]);
    }
     ...
     * problem.
     */
    [[nodiscard]] double GetSuboptimalSolution(const symbolic::Variable& var, int solution_number) const;

    /**
     * Number of suboptimal solutions stored inside MathematicalProgramResult.
     * See @ref solution_pools "solution pools".
     */
    [[nodiscard]] int num_suboptimal_solution() const { return static_cast<int>(suboptimal_x_val_.size()); }
    ...
    [[nodiscard]] std::vector<std::string> GetInfeasibleConstraintNames(
            const MathematicalProgram& prog, std::optional<double> tolerance = std::nullopt) const;
...
private:
    std::optional<std::unordered_map<symbolic::Variable::Id, int>> decision_variable_index_{};
