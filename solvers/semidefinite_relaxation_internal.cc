...
std::vector<int> FindDecisionVariableIndices(const std::map<Variable, int>& variables_to_sorted_indices,
...
void ValidateProgramIsSupported(const MathematicalProgram& prog) {
    ...
    const ProgramAttributes supported_attributes(std::initializer_list<ProgramAttribute>{
            ProgramAttribute::kLinearCost, ProgramAttribute::kQuadraticCost, ProgramAttribute::kLinearConstraint,
            ProgramAttribute::kLinearEqualityConstraint, ProgramAttribute::kQuadraticConstraint});
    if (!AreRequiredAttributesSupported(prog.required_capabilities(), supported_attributes, &unsupported_message)) {
        throw std::runtime_error(fmt::format("MakeSemidefiniteRelaxation() does not (yet) support this program: {}.",
                                             unsupported_message));
    }
}

bool CheckProgramHasNonConvexQuadratics(const MathematicalProgram& prog) {
    return std::any_of(prog.quadratic_costs().begin(), prog.quadratic_costs().end(),
                       [](const auto& cost) {
                           return !cost.evaluator()->is_convex();
                       }) ||
           std::any_of(prog.quadratic_constraints().begin(), prog.quadratic_constraints().end(),
                       [](const auto& constraint) {
                           return !constraint.evaluator()->is_convex();
                       });
}

void InitializeSemidefiniteRelaxationForProg(const MathematicalProgram& prog,
                                             const Variable& one,
    ...
    // variables).
    std::string name = group_number.has_value() ? fmt::format("Y{}", group_number.value()) : "Y";
    ...
    std::sort(sorted_variables.data(), sorted_variables.data() + sorted_variables.size(), std::less<Variable>{});
...
void DoLinearizeQuadraticCostsAndConstraints(const MathematicalProgram& prog,
    ...
    auto half_trace_QY =
            [&X, &variables_to_sorted_indices](
                    const Eigen::MatrixXd& Q,
                    const VectorXDecisionVariable& binding_vars) -> std::pair<VectorXd, VectorX<Variable>> {
        const int N = binding_vars.size();
        const int num_vars = N * (N + 1) / 2;
        const std::vector<int> indices = FindDecisionVariableIndices(variables_to_sorted_indices, binding_vars);
    ...
    for (const auto& binding : prog.quadratic_constraints()) {
        ...
        if (preserve_convex_quadratic_constraints && binding.evaluator()->is_convex()) {
            switch (binding.evaluator()->hessian_type()) {
                case QuadraticConstraint::HessianType::kPositiveSemidefinite: {
                    relaxation->AddQuadraticAsRotatedLorentzConeConstraint(
                            binding.evaluator()->Q(), binding.evaluator()->b(), -binding.evaluator()->upper_bound()[0],
                            binding.variables(),
                ...
                case QuadraticConstraint::HessianType::kNegativeSemidefinite: {
                    relaxation->AddQuadraticAsRotatedLorentzConeConstraint(
                            -binding.evaluator()->Q(), -binding.evaluator()->b(), binding.evaluator()->lower_bound()[0],
                            binding.variables(),
                            // Set the PSD tolerance check to infinity since -Q is already
                            // known to be PSD
                            kInf);
...
void DoAddImpliedLinearConstraints(const MathematicalProgram& prog,
                                   const MatrixXDecisionVariable& X,
                                   const std::map<Variable, int>& variables_to_sorted_indices,
                                   MathematicalProgram* relaxation) {
    DRAKE_DEMAND(relaxation != nullptr);
    ...
    for (const auto& binding : prog.bounding_box_constraints()) {
        const std::vector<int> indices = FindDecisionVariableIndices(variables_to_sorted_indices, binding.variables());
        for (int i = 0; i < binding.evaluator()->num_constraints(); ++i) {
            if (std::isfinite(binding.evaluator()->lower_bound()[i])) {
            ...
            if (std::isfinite(binding.evaluator()->lower_bound()[i])) {
                for (int j = 0; j < binding.evaluator()->num_vars(); ++j) {
                    if (binding.evaluator()->get_sparse_A().coeff(i, j) != 0) {
                        A_triplets.push_back(Triplet<double>(constraint_idx, indices[j],
    ...
    const MatrixX<Expression> AYAT = A * X.topLeftCorner(prog.num_vars(), prog.num_vars()) * A.transpose();
    const VectorX<Variable> y = X.col(prog.num_vars()).head(prog.num_vars());

    const VectorX<Expression> rhs_flat_tril =
            math::ToLowerTriangularColumnsFromMatrix(AYAT - b * (A * y).transpose() - A * y * b.transpose());
    const VectorXd bbT_flat_tril = math::ToLowerTriangularColumnsFromMatrix(-b * b.transpose());

    relaxation->AddLinearConstraint(rhs_flat_tril, bbT_flat_tril, VectorXd::Constant(bbT_flat_tril.size(), kInf));
}

void DoAddImpliedLinearEqualityConstraints(const MathematicalProgram& prog,
                                           const MatrixXDecisionVariable& X,
                                           const std::map<Variable, int>& variables_to_sorted_indices,
                                           MathematicalProgram* relaxation) {
    DRAKE_DEMAND(relaxation != nullptr);
    ...
    for (const auto& binding : prog.linear_equality_constraints()) {
        ...
        for (int j = 0; j < static_cast<int>(X.cols()) - 1; ++j) {
            ...
            relaxation->AddLinearEqualityConstraint(Ab, VectorXd::Zero(binding.evaluator()->num_constraints()), vars);
        }
    }
}

Eigen::SparseMatrix<double> SparseKroneckerProduct(const Eigen::SparseMatrix<double>& A,
        ...
        for (SparseMatrix<double>::InnerIterator itA(A, iA); itA; ++itA) {
            for (int iB = 0; iB < B.outerSize(); ++iB) {
                for (SparseMatrix<double>::InnerIterator itB(B, iB); itB; ++itB) {
                    C_triplets.emplace_back(itA.row() * B.rows() + itB.row(), itA.col() * B.cols() + itB.col(),
                                            itA.value() * itB.value());
                }
            }
        }
...
namespace {
template <typename T, typename = std::enable_if_t<std::is_same_v<T, Expression> || std::is_same_v<T, Variable>>>
void DoAddMatrixIsLorentzByPositiveOrthantSeparableConstraint(const Eigen::Ref<const MatrixX<T>>& X,
...
void AddMatrixIsLorentzByPositiveOrthantSeparableConstraint(const Eigen::Ref<const MatrixX<Variable>>& X,
                                                            MathematicalProgram* prog) {
    DoAddMatrixIsLorentzByPositiveOrthantSeparableConstraint<Variable>(X, prog);
}

void AddMatrixIsLorentzByPositiveOrthantSeparableConstraint(const Eigen::Ref<const MatrixX<Expression>>& X,
                                                            MathematicalProgram* prog) {
    DoAddMatrixIsLorentzByPositiveOrthantSeparableConstraint<Expression>(X, prog);
}

void AddMatrixIsPositiveOrthantByLorentzSeparableConstraint(const Eigen::Ref<const MatrixX<Variable>>& X,
                                                            MathematicalProgram* prog) {
    DoAddMatrixIsLorentzByPositiveOrthantSeparableConstraint<Variable>(X.transpose(), prog);
}

void AddMatrixIsPositiveOrthantByLorentzSeparableConstraint(const Eigen::Ref<const MatrixX<Expression>>& X,
                                                            MathematicalProgram* prog) {
    DoAddMatrixIsLorentzByPositiveOrthantSeparableConstraint<Expression>(X.transpose(), prog);
}
...
// encoded as such.
template <typename T, typename = std::enable_if_t<std::is_same_v<T, Expression> || std::is_same_v<T, Variable>>>
void DoAddMatrixIsLorentzByLorentzSeparableConstraintSimplicialCase(const Eigen::Ref<const Eigen::MatrixX<T>>& X,
                                                                    MathematicalProgram* prog) {
    ...
    const Eigen::Matrix2d R{{1 / std::sqrt(2), -1 / std::sqrt(2)}, {1 / std::sqrt(2), 1 / std::sqrt(2)}};
    if (X.rows() == 2) {
        X_expr = R * X_expr;
    }
    if (X.cols() == 2) {
...
void DoAddMatrixIsLorentzByLorentzSeparableConstraint(const SparseMatrix<double>& A,
    ...
    // [W_adj_n ⊗ W_adj_m; -A]
    SparseMatrix<double> CoefficientMat(W_adj_m.rows() * W_adj_n.rows(), W_adj_m.cols() * W_adj_n.cols() + A.cols());
    std::vector<Triplet<double>> CoefficientMat_triplets;
    CoefficientMat_triplets.reserve(W_adj_n.nonZeros() * W_adj_m.nonZeros() + A.nonZeros());
    // Set the left columns of CoefficientMat to W_adj_n ⊗ W_adj_m.
    for (int idx_n = 0; idx_n < W_adj_n.outerSize(); ++idx_n) {
        for (SparseMatrix<double>::InnerIterator it_n(W_adj_n, idx_n); it_n; ++it_n) {
            for (int idx_m = 0; idx_m < W_adj_m.outerSize(); ++idx_m) {
                for (SparseMatrix<double>::InnerIterator it_m(W_adj_m, idx_m); it_m; ++it_m) {
                    CoefficientMat_triplets.emplace_back(it_n.row() * W_adj_m.rows() + it_m.row(),
    ...
    for (int i = 0; i < A.outerSize(); ++i) {
        for (SparseMatrix<double>::InnerIterator it(A, i); it; ++it) {
            CoefficientMat_triplets.emplace_back(it.row(), it.col() + col, -it.value());
        }
    }
    CoefficientMat.setFromTriplets(CoefficientMat_triplets.begin(), CoefficientMat_triplets.end());
...
void AddMatrixIsLorentzByLorentzSeparableConstraint(const Eigen::Ref<const MatrixX<Variable>>& X,
                                                    MathematicalProgram* prog) {
    if (std::min(X.rows(), X.cols()) <= 2) {
        ...
        I.setIdentity();
        const Eigen::VectorX<Variable> x = Eigen::Map<const Eigen::VectorX<Variable>>(X.data(), X.size());
        DoAddMatrixIsLorentzByLorentzSeparableConstraint(I, Eigen::VectorXd::Zero(I.rows()),
                                                         Eigen::Map<const Eigen::VectorX<Variable>>(X.data(), X.size()),
                                                         m, n, prog);
    }
}

void AddMatrixIsLorentzByLorentzSeparableConstraint(const Eigen::Ref<const MatrixX<Expression>>& X,
                                                    MathematicalProgram* prog) {
    if (std::min(X.rows(), X.cols()) <= 2) {
        DoAddMatrixIsLorentzByLorentzSeparableConstraintSimplicialCase(X, prog);
    } else {
        ...
        Eigen::VectorXd b;
        VectorX<Variable> x;
        symbolic::DecomposeAffineExpressions(Eigen::Map<const VectorX<Expression>>(X.data(), X.size()), &A, &b, &x);
        DoAddMatrixIsLorentzByLorentzSeparableConstraint(A.sparseView(), b, x, m, n, prog);
    }
}
