...
protected:
    ...
    EvaluatorBase(int num_outputs, int num_vars, const std::string& description = "")
        : num_vars_(num_vars), num_outputs_(num_outputs), description_(description) {}
...
public:
    ...
    PolynomialEvaluator(const VectorXPoly& polynomials, const std::vector<Polynomiald::VarType>& poly_vars)
        : EvaluatorBase(polynomials.rows(), poly_vars.size()), polynomials_(polynomials), poly_vars_(poly_vars) {}
...
private:
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::logic_error("PolynomialEvaluator does not support symbolic evaluation.");
    }
    ...
    mutable std::map<Polynomiald::VarType, AutoDiffXd> taylor_evaluation_point_temp_;
};
...
private:
    void DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const override {
        ...
        DRAKE_ASSERT(static_cast<size_t>(x.rows()) == internal::FunctionTraits<F>::numInputs(f_));
        DRAKE_ASSERT(static_cast<size_t>(y->rows()) == internal::FunctionTraits<F>::numOutputs(f_));
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::logic_error("FunctionEvaluator does not support symbolic evaluation.");
    }
...
private:
    ...
    void DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const override {
        throw std::logic_error("VisualizationCallback does not support symbolic evaluation.");
    }
