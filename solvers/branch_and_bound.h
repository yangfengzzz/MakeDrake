...
public:
     ...
     */
    static std::pair<std::unique_ptr<MixedIntegerBranchAndBoundNode>,
     ...
     */
    const MathematicalProgramResult* prog_result() const { return prog_result_.get(); }

    /** Getter for the left child. */
    const MixedIntegerBranchAndBoundNode* left_child() const { return left_child_.get(); }

    /** Getter for the mutable left child. */
    MixedIntegerBranchAndBoundNode* mutable_left_child() { return left_child_.get(); }

    /** Getter for the right child. */
    const MixedIntegerBranchAndBoundNode* right_child() const { return right_child_.get(); }

    /** Getter for the mutable right child. */
    MixedIntegerBranchAndBoundNode* mutable_right_child() { return right_child_.get(); }
     ...
     */
    const symbolic::Variable& fixed_binary_variable() const { return fixed_binary_variable_; }
     ...
     */
    const std::list<symbolic::Variable>& remaining_binary_variables() const { return remaining_binary_variables_; }
...
private:
    ...
    void set_integral_tolerance(double integral_tol) { integral_tol_ = integral_tol; }
    ...
    // binary_var_value_.
    void FixBinaryVariable(const symbolic::Variable& binary_variable, bool binary_value);
...
public:
     ...
     */
    [[nodiscard]] double GetSolution(const symbolic::Variable& mip_var, int nth_best_solution = 0) const;
    ...
    [[nodiscard]] typename std::enable_if_t<std::is_same_v<typename Derived::Scalar, symbolic::Variable>,
                                            MatrixLikewise<double, Derived>>
    GetSolution(const Eigen::MatrixBase<Derived>& mip_vars, int nth_best_solution = 0) const {
     ...
     */
    [[nodiscard]] const symbolic::Variable& GetNewVariable(const symbolic::Variable& old_variable) const;
     ...
     */
    template <typename Derived>
    typename std::enable_if_t<is_eigen_scalar_same<Derived, symbolic::Variable>::value,
     ...
     * fathomed.
     */
    void SetUserDefinedNodeSelectionFunction(NodeSelectFun fun) { node_selection_userfun_ = fun; }
     ...
     * \endcode
     */
    void SetUserDefinedVariableSelectionFunction(VariableSelectFun fun) { variable_selection_userfun_ = fun; }
     ...
     */
    void SetSearchIntegralSolutionByRounding(bool flag) { search_integral_solution_by_rounding_ = flag; }
     ...
     */
    void SetUserDefinedNodeCallbackFunction(NodeCallbackFun fun) { node_callback_userfun_ = fun; }
     ...
     * @throws std::exception if the precondition is not satisfied.
     */
    [[nodiscard]] bool IsLeafNodeFathomed(const MixedIntegerBranchAndBoundNode& leaf_node) const;

    /**
     * Getter for the root node. Note that this is aliased for the lifetime of
     * this object.
     */
    [[nodiscard]] const MixedIntegerBranchAndBoundNode* root() const { return root_.get(); }
     ...
     * cost.
     */
    [[nodiscard]] const std::multimap<double, Eigen::VectorXd>& solutions() const { return solutions_; }
...
private:
     ...
     */
    [[nodiscard]] const symbolic::Variable* PickBranchingVariable(const MixedIntegerBranchAndBoundNode& node) const;
    ...
    VariableSelectionMethod variable_selection_method_ = VariableSelectionMethod::kMostAmbivalent;

    NodeSelectionMethod node_selection_method_ = NodeSelectionMethod::kMinLowerBound;
