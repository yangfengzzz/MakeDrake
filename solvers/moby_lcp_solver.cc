...
bool CheckLemkeTrivial(int n, const Scalar& zero_tol, const VectorX<Scalar>& q, VectorX<Scalar>* z) {
...
template <>
VectorX<double> LinearSolve(const MatrixX<double>& M, const VectorX<double>& b) {
    // Special case necessary because Eigen doesn't always handle empty matrices
    // properly.
    if (M.rows() == 0) {
        DRAKE_ASSERT(b.size() == 0);
...
template <typename T>
void selectSubVec(const VectorX<T>& in, const std::vector<unsigned>& rows, VectorX<T>* out) {
    const int num_rows = rows.size();
    out->resize(num_rows);
    for (int i = 0; i < num_rows; i++) {
        (*out)(i) = in(rows[i]);
...
template <>
void MobyLCPSolver<Eigen::AutoDiffScalar<Vector1d>>::DoSolve(const MathematicalProgram&,
...
void MobyLCPSolver<T>::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("MobyLCPSolver doesn't support the feature of variable scaling.");
    }
    ...
    for (const auto& binding : bindings) {
        ...
        bool solved = SolveLcpLemkeRegularized(constraint->M(), constraint->q(), &constraint_solution);
        if (!solved) {
...
template <typename T>
bool MobyLCPSolver<T>::SolveLcpFast(const MatrixX<T>& M, const VectorX<T>& q, VectorX<T>* z, const T& zero_tol) const {
    using std::abs;
    ...
    for (pivots_ = 1; pivots_ <= MAX_PIV; pivots_++) {
        ...
        if (minw == UINF || w[minw] > -mod_zero_tol) {
            ...
            if (log_enabled_ && zz.rows() > 0) {
                Log() << "MobyLCPSolver::SolveLcpFast() - minimum z after pivot: " << zz[minz] << std::endl;
            }
    ...
    if (result) {
        ...
        if (z->minCoeff() >= -mod_zero_tol) {
            ...
            if (wx.minCoeff() >= -mod_zero_tol) {
                ...
                Log() << "  MobyLCPSolver::SolveLcpFastRegularized() - " << "'w' not solved to desired tolerance"
                      << std::endl;
                Log() << "  minimum w: " << wx.minCoeff() << std::endl;
            }
        } else {
            Log() << "  MobyLCPSolver::SolveLcpFastRegularized() - " << "'z' not solved to desired tolerance"
                  << std::endl;
            Log() << "  minimum z: " << z->minCoeff() << std::endl;
        }
    } else {
        Log() << "  MobyLCPSolver::SolveLcpFastRegularized() " << "- solver failed with zero regularization"
              << std::endl;
    }
...
template <typename T>
bool MobyLCPSolver<T>::SolveLcpLemke(
        const MatrixX<T>& M, const VectorX<T>& q, VectorX<T>* z, const T& piv_tol, const T& zero_tol) const {
    using std::max;
    ...
    const T naive_piv_tol =
            n * max(T(1), M.template lpNorm<Eigen::Infinity>()) * std::numeric_limits<double>::epsilon();
    ...
    leaving = *iiter;
    Log() << " -- x: " << x << std::endl;
    Log() << " -- first pivot: leaving index=" << lvindex << "  entering index=" << entering
    ...
    *iiter = t;  // replace w var with _z0 in basic indices
    ...
    Log() << " -- maximum number of iterations exceeded (n=" << n << ", max=" << max_iter << ")" << std::endl;
    Log() << "MobyLCPSolver::SolveLcpLemke() exited" << std::endl;
    z->setZero(n);
    return false;
}
        ...
        if (z->minCoeff() >= -mod_zero_tol) {
            ...
            if (wx.minCoeff() >= -mod_zero_tol) {
                ...
                if (wx_min >= -mod_zero_tol && wx_max < mod_zero_tol) {
                    ...
                    Log() << "MobyLCPSolver::SolveLcpLemke() - " << "'<w, z> not within tolerance(min value: " << wx_min
...
template <typename T>
MobyLCPSolver<T>::MobyLCPSolver() : SolverBase(id(), &is_available, &is_enabled, &ProgramAttributesSatisfied) {}
...
template <typename T>
bool MobyLCPSolver<T>::ProgramAttributesSatisfied(const MathematicalProgram& prog) {
