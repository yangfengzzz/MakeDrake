...
void LinearSystemSolver::DoSolve(const MathematicalProgram& prog,
    ...
    if (!prog.GetVariableScaling().empty()) {
        static const logging::Warn log_once("LinearSystemSolver doesn't support the feature of variable scaling.");
    }
    ...
    for (auto const& binding : prog.linear_equality_constraints()) {
        ...
        for (int i = 0; i < static_cast<int>(binding.GetNumElements()); ++i) {
            ...
            for (Eigen::SparseMatrix<double>::InnerIterator it(c->get_sparse_A(), i); it; ++it) {
                Aeq(constraint_index + it.row(), variable_index) = it.value();
            }
        }
        beq.segment(constraint_index, n) = c->lower_bound();  // = c->upper_bound() since it's an equality
        // constraint
        constraint_index += n;
    }

    // least-squares solution
    const Eigen::VectorXd least_square_sol = Aeq.jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(beq);
...
bool CheckAttributes(const MathematicalProgram& prog, std::string* explanation) {
    static const never_destroyed<ProgramAttributes> solver_capabilities(
            std::initializer_list<ProgramAttribute>{ProgramAttribute::kLinearEqualityConstraint});
    const ProgramAttributes& required_capabilities = prog.required_capabilities();
    const bool capabilities_match =
            AreRequiredAttributesSupported(required_capabilities, solver_capabilities.access(), explanation);
    if (!capabilities_match || required_capabilities.empty()) {
        if (explanation) {
            if (required_capabilities.empty()) {
                *explanation = " a LinearEqualityConstraint is required but has not been declared";
            }
            *explanation = fmt::format("LinearSystemSolver is unable to solve because {}.", *explanation);
        }
        return false;
    }
    if (explanation) {
...
bool LinearSystemSolver::ProgramAttributesSatisfied(const MathematicalProgram& prog) {
    return CheckAttributes(prog, nullptr);
}

std::string LinearSystemSolver::UnsatisfiedProgramAttributes(const MathematicalProgram& prog) {
