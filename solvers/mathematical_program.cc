...
#include "common/fmt_eigen.h"
#include "common/ssize.h"
#include "common/symbolic/decompose.h"
#include "common/symbolic/latex.h"
#include "common/symbolic/monomial_util.h"
#include "math/matrix_util.h"
#include "solvers/binding.h"
#include "solvers/decision_variable.h"
#include "solvers/sos_basis_generator.h"
...
std::string MathematicalProgram::ToLatex(int precision) {
    ...
    for (int i = 0; i < num_vars(); ++i) {
        ...
        first = false;
        ss << symbolic::ToLatex(decision_variables()[i]);  // precision is not needed.
    }
    ss << "} \\quad & ";
...
MatrixXDecisionVariable MathematicalProgram::NewVariables(
        VarType type, int rows, int cols, bool is_symmetric, const vector<string>& names) {
...
MatrixXDecisionVariable MathematicalProgram::NewSymmetricContinuousVariables(int rows, const string& name) {
    vector<string> names(rows * (rows + 1) / 2);
    int count = 0;
    for (int j = 0; j < static_cast<int>(rows); ++j) {
        for (int i = j; i < static_cast<int>(rows); ++i) {
            names[count] = name + "(" + std::to_string(i) + "," + std::to_string(j) + ")";
            ++count;
        }
    }
...
void MathematicalProgram::AddDecisionVariables(const Eigen::Ref<const MatrixXDecisionVariable>& decision_variables) {
    int new_var_count = 0;
    for (int i = 0; i < decision_variables.rows(); ++i) {
        for (int j = 0; j < decision_variables.cols(); ++j) {
            const auto& var = decision_variables(i, j);
            if (decision_variable_index_.find(var.get_id()) != decision_variable_index_.end()) {
                continue;
            }
            if (indeterminates_index_.find(var.get_id()) != indeterminates_index_.end()) {
                throw std::runtime_error(fmt::format("{} is already an indeterminate.", var));
            }
...
symbolic::Polynomial MathematicalProgram::NewFreePolynomialImpl(const Variables& indeterminates,
                                                                const int degree,
                                                                const string& coeff_name,
                                                                symbolic::internal::DegreeType degree_type) {
    const drake::VectorX<symbolic::Monomial> m =
            symbolic::internal::ComputeMonomialBasis<Eigen::Dynamic>(indeterminates, degree, degree_type);
    const VectorXDecisionVariable coeffs{this->NewContinuousVariables(m.size(), coeff_name)};
...
symbolic::Polynomial MathematicalProgram::NewFreePolynomial(const Variables& indeterminates,
                                                            const int degree,
                                                            const string& coeff_name) {
    return NewFreePolynomialImpl(indeterminates, degree, coeff_name, symbolic::internal::DegreeType::kAny);
}

symbolic::Polynomial MathematicalProgram::NewEvenDegreeFreePolynomial(const symbolic::Variables& indeterminates,
                                                                      int degree,
                                                                      const std::string& coeff_name) {
    return NewFreePolynomialImpl(indeterminates, degree, coeff_name, symbolic::internal::DegreeType::kEven);
}

symbolic::Polynomial MathematicalProgram::NewOddDegreeFreePolynomial(const symbolic::Variables& indeterminates,
                                                                     int degree,
                                                                     const std::string& coeff_name) {
    return NewFreePolynomialImpl(indeterminates, degree, coeff_name, symbolic::internal::DegreeType::kOdd);
}
...
pair<symbolic::Polynomial, MatrixXDecisionVariable> MathematicalProgram::NewSosPolynomial(
        const Eigen::Ref<const VectorX<symbolic::Monomial>>& monomial_basis,
        NonnegativePolynomial type,
        const std::string& gram_name) {
    const MatrixXDecisionVariable Q = NewSymmetricContinuousVariables(monomial_basis.size(), gram_name);
...
symbolic::Polynomial MathematicalProgram::NewSosPolynomial(
    ...
    const symbolic::Polynomial p = ComputePolynomialFromMonomialBasisAndGramMatrix(monomial_basis, gramian);
    switch (type) {
        ...
        case MathematicalProgram::NonnegativePolynomial::kDsos: {
            AddPositiveDiagonallyDominantMatrixConstraint(gramian.cast<symbolic::Expression>());
            return p;
        }
    }
    throw std::runtime_error("NewSosPolynomial() was passed an invalid NonnegativePolynomial type");
}
...
pair<symbolic::Polynomial, MatrixXDecisionVariable> MathematicalProgram::NewSosPolynomial(
    ...
    if (degree == 0) {
        ...
        const symbolic::Variable poly_constant = NewContinuousVariables<1>(gram_name)(0);
        AddBoundingBoxConstraint(0, kInf, poly_constant);
        MatrixXDecisionVariable gram(1, 1);
        gram(0, 0) = poly_constant;
        return std::make_pair(symbolic::Polynomial({{symbolic::Monomial(), poly_constant}}), gram);
    } else {
        const drake::VectorX<symbolic::Monomial> x{MonomialBasis(indeterminates, degree / 2)};
        return NewSosPolynomial(x, type, gram_name);
    }
}

std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable>
MathematicalProgram::NewEvenDegreeNonnegativePolynomial(const symbolic::Variables& indeterminates,
                                                        int degree,
                                                        MathematicalProgram::NonnegativePolynomial type) {
    DRAKE_DEMAND(degree % 2 == 0);
    const VectorX<symbolic::Monomial> m_e = EvenDegreeMonomialBasis(indeterminates, degree / 2);
    const VectorX<symbolic::Monomial> m_o = OddDegreeMonomialBasis(indeterminates, degree / 2);
...
std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable>
MathematicalProgram::NewEvenDegreeSosPolynomial(const symbolic::Variables& indeterminates, int degree) {
    return NewEvenDegreeNonnegativePolynomial(indeterminates, degree, MathematicalProgram::NonnegativePolynomial::kSos);
}

std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable>
MathematicalProgram::NewEvenDegreeSdsosPolynomial(const symbolic::Variables& indeterminates, int degree) {
    return NewEvenDegreeNonnegativePolynomial(indeterminates, degree,
                                              MathematicalProgram::NonnegativePolynomial::kSdsos);
}

std::tuple<symbolic::Polynomial, MatrixXDecisionVariable, MatrixXDecisionVariable>
MathematicalProgram::NewEvenDegreeDsosPolynomial(const symbolic::Variables& indeterminates, int degree) {
    return NewEvenDegreeNonnegativePolynomial(indeterminates, degree,
                                              MathematicalProgram::NonnegativePolynomial::kDsos);
}

symbolic::Polynomial MathematicalProgram::MakePolynomial(const symbolic::Expression& e) const {
...
MatrixXIndeterminate MathematicalProgram::NewIndeterminates(int rows, int cols, const vector<string>& names) {
...
VectorXIndeterminate MathematicalProgram::NewIndeterminates(int rows, const std::vector<std::string>& names) {
    return NewIndeterminates(rows, 1, names);
}

VectorXIndeterminate MathematicalProgram::NewIndeterminates(int rows, const string& name) {
...
MatrixXIndeterminate MathematicalProgram::NewIndeterminates(int rows, int cols, const string& name) {
...
int MathematicalProgram::AddIndeterminate(const symbolic::Variable& new_indeterminate) {
    if (decision_variable_index_.find(new_indeterminate.get_id()) != decision_variable_index_.end()) {
        throw std::runtime_error(
                fmt::format("{} is a decision variable in the optimization program.", new_indeterminate));
    }
    if (new_indeterminate.get_type() != symbolic::Variable::Type::CONTINUOUS) {
        throw std::runtime_error(fmt::format("{} should be of type CONTINUOUS.", new_indeterminate));
    }
    ...
    if (it == indeterminates_index_.end()) {
        ...
        indeterminates_index_.insert(std::make_pair(new_indeterminate.get_id(), var_index));
...
void MathematicalProgram::AddIndeterminates(const Eigen::Ref<const MatrixXDecisionVariable>& new_indeterminates) {
...
void MathematicalProgram::AddIndeterminates(const symbolic::Variables& new_indeterminates) {
...
Binding<VisualizationCallback> MathematicalProgram::AddVisualizationCallback(
    ...
    visualization_callbacks_.push_back(internal::CreateBinding<VisualizationCallback>(
...
Binding<LinearCost> MathematicalProgram::AddCost(const Binding<LinearCost>& binding) {
    DRAKE_DEMAND(CheckBinding(binding));
...
Binding<LinearCost> MathematicalProgram::AddLinearCost(const Eigen::Ref<const Eigen::VectorXd>& a,
                                                       double b,
                                                       const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    return AddCost(make_shared<LinearCost>(a, b), vars);
}

Binding<QuadraticCost> MathematicalProgram::AddCost(const Binding<QuadraticCost>& binding) {
    DRAKE_DEMAND(CheckBinding(binding));
    required_capabilities_.insert(ProgramAttribute::kQuadraticCost);
    DRAKE_ASSERT(binding.evaluator()->Q().rows() == static_cast<int>(binding.GetNumElements()) &&
                 binding.evaluator()->b().rows() == static_cast<int>(binding.GetNumElements()));
    quadratic_costs_.push_back(binding);
    return quadratic_costs_.back();
}

Binding<QuadraticCost> MathematicalProgram::AddQuadraticCost(const Expression& e, std::optional<bool> is_convex) {
...
Binding<QuadraticCost> MathematicalProgram::AddQuadraticCost(const Eigen::Ref<const Eigen::MatrixXd>& Q,
...
Binding<L2NormCost> MathematicalProgram::AddCost(const Binding<L2NormCost>& binding) {
    DRAKE_DEMAND(CheckBinding(binding));
...
Binding<L2NormCost> MathematicalProgram::AddL2NormCost(const Eigen::Ref<const Eigen::MatrixXd>& A,
                                                       const Eigen::Ref<const Eigen::VectorXd>& b,
                                                       const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    return AddCost(std::make_shared<L2NormCost>(A, b), vars);
}

Binding<L2NormCost> MathematicalProgram::AddL2NormCost(const symbolic::Expression& e,
                                                       double psd_tol,
                                                       double coefficient_tol) {
    return AddCost(internal::ParseL2NormCost(e, psd_tol, coefficient_tol));
}

std::tuple<symbolic::Variable, Binding<LinearCost>, Binding<LorentzConeConstraint>>
MathematicalProgram::AddL2NormCostUsingConicConstraint(const Eigen::Ref<const Eigen::MatrixXd>& A,
                                                       const Eigen::Ref<const Eigen::VectorXd>& b,
                                                       const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    auto s = this->NewContinuousVariables<1>("slack")(0);
    auto linear_cost = this->AddLinearCost(Vector1d(1), 0, Vector1<symbolic::Variable>(s));
    // A_full = [1 0]
    //          [0 A]
    // b_full = [0 b]
    ...
    auto lorentz_cone_constraint =
            this->AddLorentzConeConstraint(A_full, b_full, {Vector1<symbolic::Variable>(s), vars});
    return std::make_tuple(s, linear_cost, lorentz_cone_constraint);
}

Binding<PolynomialCost> MathematicalProgram::AddPolynomialCost(const Expression& e) {
...
namespace {
void CreateLogDetermiant(MathematicalProgram* prog,
  ...
  psd_mat << X,             *Z,
    ...
    *t = prog->NewContinuousVariables(X_rows);
    ...
    for (int i = 0; i < X_rows; ++i) {
        prog->AddExponentialConeConstraint(Vector3<symbolic::Expression>((*Z)(i, i), 1, (*t)(i)));
    }
}
}  // namespace
std::tuple<Binding<LinearCost>, VectorX<symbolic::Variable>, MatrixX<symbolic::Expression>>
MathematicalProgram::AddMaximizeLogDeterminantCost(const Eigen::Ref<const MatrixX<symbolic::Expression>>& X) {
    VectorX<symbolic::Variable> t;
    MatrixX<symbolic::Expression> Z;
...
std::tuple<Binding<LinearConstraint>, VectorX<symbolic::Variable>, MatrixX<symbolic::Expression>>
MathematicalProgram::AddLogDeterminantLowerBoundConstraint(const Eigen::Ref<const MatrixX<symbolic::Expression>>& X,
                                                           double lower) {
    VectorX<symbolic::Variable> t;
    MatrixX<symbolic::Expression> Z;
    CreateLogDetermiant(this, X, &t, &Z);
    const auto constraint = AddLinearConstraint(Eigen::RowVectorXd::Ones(t.rows()), lower, kInf, t);
...
Binding<LinearCost> MathematicalProgram::AddMaximizeGeometricMeanCost(
    ...
    for (int i = 0; i < w.rows() - 1; ++i) {
        AddRotatedLorentzConeConstraint(Vector3<symbolic::Variable>(x(2 * i), x(2 * i + 1), w(i)));
    }
    if (2 * w.rows() == x.rows()) {
        // x has even number of rows.
        AddRotatedLorentzConeConstraint(Vector3<symbolic::Variable>(x(x.rows() - 2), x(x.rows() - 1), w(w.rows() - 1)));
    } else {
        ...
        AddRotatedLorentzConeConstraint(C, d, Vector2<symbolic::Variable>(x(x.rows() - 1), w(w.rows() - 1)));
    }
    if (x.rows() == 2) {
...
Binding<Constraint> MathematicalProgram::AddConstraint(const Binding<Constraint>& binding) {
    ...
    if (dynamic_cast<LinearMatrixInequalityConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<LinearMatrixInequalityConstraint>(binding));
    } else if (dynamic_cast<PositiveSemidefiniteConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<PositiveSemidefiniteConstraint>(binding));
    } else if (dynamic_cast<RotatedLorentzConeConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<RotatedLorentzConeConstraint>(binding));
    } else if (dynamic_cast<LorentzConeConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<LorentzConeConstraint>(binding));
    } else if (dynamic_cast<QuadraticConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<QuadraticConstraint>(binding));
    } else if (dynamic_cast<LinearConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<LinearConstraint>(binding));
    } else {
...
Binding<Constraint> MathematicalProgram::AddConstraint(const Expression& e, const double lb, const double ub) {
    return AddConstraint(internal::ParseConstraint(e, lb, ub));
}

Binding<Constraint> MathematicalProgram::AddConstraint(const Eigen::Ref<const MatrixX<Expression>>& v,
    ...
    return AddConstraint(internal::ParseConstraint(Flatten(v), Flatten(lb), Flatten(ub)));
}
...
Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(const Expression& e,
                                                                   const double lb,
                                                                   const double ub) {
...
Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(const Eigen::Ref<const MatrixX<Expression>>& v,
                                                                   const Eigen::Ref<const Eigen::MatrixXd>& lb,
                                                                   const Eigen::Ref<const Eigen::MatrixXd>& ub) {
    DRAKE_DEMAND(v.rows() == lb.rows());
    DRAKE_DEMAND(v.rows() == ub.rows());
    DRAKE_DEMAND(v.cols() == lb.cols());
    DRAKE_DEMAND(v.cols() == ub.cols());
    Binding<Constraint> binding = internal::ParseConstraint(Flatten(v), Flatten(lb), Flatten(ub));
    Constraint* constraint = binding.evaluator().get();
    if (dynamic_cast<LinearConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<LinearConstraint>(binding));
    } else {
        throw std::runtime_error(fmt::format("Expression {} is non-linear.", fmt_eigen(v)));
    }
}

Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(const Formula& f) {
...
Binding<LinearConstraint> MathematicalProgram::AddLinearConstraint(
        const Eigen::Ref<const Eigen::Array<symbolic::Formula, Eigen::Dynamic, Eigen::Dynamic>>& formulas) {
    ...
    if (dynamic_cast<LinearConstraint*>(constraint)) {
        ...
        throw std::runtime_error("AddLinearConstraint called but formulas are non-linear");
    }
}

Binding<LinearConstraint> MathematicalProgram::AddConstraint(const Binding<LinearConstraint>& binding) {
    ...
    if (dynamic_cast<BoundingBoxConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<BoundingBoxConstraint>(binding));
    } else if (dynamic_cast<LinearEqualityConstraint*>(constraint)) {
        return AddConstraint(internal::BindingDynamicCast<LinearEqualityConstraint>(binding));
    } else {
        // TODO(eric.cousineau): This is a good assertion... But seems out of place,
        // possibly redundant w.r.t. the binding infrastructure.
        DRAKE_ASSERT(binding.evaluator()->get_sparse_A().cols() == static_cast<int>(binding.GetNumElements()));
        if (!CheckBinding(binding)) {
            return binding;
        }
...
Binding<LinearEqualityConstraint> MathematicalProgram::AddConstraint(const Binding<LinearEqualityConstraint>& binding) {
    DRAKE_ASSERT(binding.evaluator()->get_sparse_A().cols() == static_cast<int>(binding.GetNumElements()));
    if (!CheckBinding(binding)) {
        return binding;
    }
...
Binding<LinearEqualityConstraint> MathematicalProgram::AddLinearEqualityConstraint(const Expression& e, double b) {
    return AddConstraint(internal::ParseLinearEqualityConstraint(e, b));
}

Binding<LinearEqualityConstraint> MathematicalProgram::AddLinearEqualityConstraint(const Formula& f) {
...
Binding<BoundingBoxConstraint> MathematicalProgram::AddConstraint(const Binding<BoundingBoxConstraint>& binding) {
    if (!CheckBinding(binding)) {
        return binding;
    }
    DRAKE_ASSERT(binding.evaluator()->num_outputs() == static_cast<int>(binding.GetNumElements()));
    required_capabilities_.insert(ProgramAttribute::kLinearConstraint);
    bbox_constraints_.push_back(binding);
    return bbox_constraints_.back();
}

Binding<LorentzConeConstraint> MathematicalProgram::AddConstraint(const Binding<LorentzConeConstraint>& binding) {
    DRAKE_DEMAND(CheckBinding(binding));
...
Binding<LorentzConeConstraint> MathematicalProgram::AddLorentzConeConstraint(const symbolic::Formula& f,
                                                                             LorentzConeConstraint::EvalType eval_type,
                                                                             double psd_tol,
                                                                             double coefficient_tol) {
    return AddConstraint(internal::ParseLorentzConeConstraint(f, eval_type, psd_tol, coefficient_tol));
}

Binding<LorentzConeConstraint> MathematicalProgram::AddLorentzConeConstraint(
        const Eigen::Ref<const VectorX<Expression>>& v, LorentzConeConstraint::EvalType eval_type) {
    return AddConstraint(internal::ParseLorentzConeConstraint(v, eval_type));
}

Binding<LorentzConeConstraint> MathematicalProgram::AddLorentzConeConstraint(
        const Expression& linear_expression,
        const Expression& quadratic_expression,
        double tol,
        LorentzConeConstraint::EvalType eval_type) {
    return AddConstraint(internal::ParseLorentzConeConstraint(linear_expression, quadratic_expression, tol, eval_type));
}
...
Binding<LorentzConeConstraint> MathematicalProgram::AddLorentzConeConstraint(
        ...
        const Eigen::Ref<const VectorXDecisionVariable>& vars,
        LorentzConeConstraint::EvalType eval_type) {
    shared_ptr<LorentzConeConstraint> constraint = make_shared<LorentzConeConstraint>(A, b, eval_type);
...
Binding<RotatedLorentzConeConstraint> MathematicalProgram::AddConstraint(
    ...
    DRAKE_DEMAND(CheckBinding(binding));
    required_capabilities_.insert(ProgramAttribute::kRotatedLorentzConeConstraint);
...
Binding<RotatedLorentzConeConstraint> MathematicalProgram::AddRotatedLorentzConeConstraint(
        ...
        double tol) {
    auto binding = internal::ParseRotatedLorentzConeConstraint(linear_expression1, linear_expression2,
                                                               quadratic_expression, tol);
...
Binding<RotatedLorentzConeConstraint> MathematicalProgram::AddRotatedLorentzConeConstraint(
        ...
        const Eigen::Ref<const Eigen::VectorXd>& b,
        const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    shared_ptr<RotatedLorentzConeConstraint> constraint = make_shared<RotatedLorentzConeConstraint>(A, b);
    return AddConstraint(constraint, vars);
}

Binding<RotatedLorentzConeConstraint> MathematicalProgram::AddQuadraticAsRotatedLorentzConeConstraint(
        const Eigen::Ref<const Eigen::MatrixXd>& Q,
        const Eigen::Ref<const Eigen::VectorXd>& b,
        double c,
        const Eigen::Ref<const VectorXDecisionVariable>& vars,
        double psd_tol) {
    auto constraint = internal::ParseQuadraticAsRotatedLorentzConeConstraint(Q, b, c, psd_tol);
    return AddConstraint(constraint, vars);
}
...
Binding<BoundingBoxConstraint> MathematicalProgram::AddBoundingBoxConstraint(
    ...
    shared_ptr<BoundingBoxConstraint> constraint = make_shared<BoundingBoxConstraint>(Flatten(lb), Flatten(ub));
    return AddConstraint(Binding<BoundingBoxConstraint>(constraint, Flatten(vars)));
}

Binding<QuadraticConstraint> MathematicalProgram::AddConstraint(const Binding<QuadraticConstraint>& binding) {
    DRAKE_DEMAND(CheckBinding(binding));
...
Binding<QuadraticConstraint> MathematicalProgram::AddQuadraticConstraint(
    ...
    auto constraint = std::make_shared<QuadraticConstraint>(Q, b, lb, ub, hessian_type);
...
Binding<QuadraticConstraint> MathematicalProgram::AddQuadraticConstraint(
        ...
        double ub,
        std::optional<QuadraticConstraint::HessianType> hessian_type) {
    return AddConstraint(internal::ParseQuadraticConstraint(e, lb, ub, hessian_type));
}
...
Binding<LinearComplementarityConstraint> MathematicalProgram::AddConstraint(
    ...
    required_capabilities_.insert(ProgramAttribute::kLinearComplementarityConstraint);

    linear_complementarity_constraints_.push_back(binding);
    return linear_complementarity_constraints_.back();
}

Binding<LinearComplementarityConstraint> MathematicalProgram::AddLinearComplementarityConstraint(
        const Eigen::Ref<const Eigen::MatrixXd>& M,
        const Eigen::Ref<const Eigen::VectorXd>& q,
        const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    shared_ptr<LinearComplementarityConstraint> constraint = make_shared<LinearComplementarityConstraint>(M, q);
    return AddConstraint(constraint, vars);
}
...
Binding<Constraint> MathematicalProgram::AddPolynomialConstraint(
    ...
    auto constraint = internal::MakePolynomialConstraint(Flatten(polynomials), poly_vars, Flatten(lb), Flatten(ub));
    return AddConstraint(constraint, vars);
}
...
Binding<PositiveSemidefiniteConstraint> MathematicalProgram::AddConstraint(
    ...
    DRAKE_ASSERT(math::IsSymmetric(Eigen::Map<const MatrixXDecisionVariable>(
            binding.variables().data(), binding.evaluator()->matrix_rows(), binding.evaluator()->matrix_rows())));
    required_capabilities_.insert(ProgramAttribute::kPositiveSemidefiniteConstraint);
...
Binding<PositiveSemidefiniteConstraint> MathematicalProgram::AddPositiveSemidefiniteConstraint(
        const Eigen::Ref<const MatrixXDecisionVariable>& symmetric_matrix_var) {
    auto constraint = make_shared<PositiveSemidefiniteConstraint>(symmetric_matrix_var.rows());
    return AddConstraint(constraint, symmetric_matrix_var);
}

Binding<PositiveSemidefiniteConstraint> MathematicalProgram::AddPrincipalSubmatrixIsPsdConstraint(
        const Eigen::Ref<const MatrixXDecisionVariable>& symmetric_matrix_var, const std::set<int>& minor_indices) {
    // This function relies on AddPositiveSemidefiniteConstraint to validate the
    // documented symmetry prerequisite.
    return AddPositiveSemidefiniteConstraint(math::ExtractPrincipalSubmatrix(symmetric_matrix_var, minor_indices));
}

Binding<PositiveSemidefiniteConstraint> MathematicalProgram::AddPrincipalSubmatrixIsPsdConstraint(
        const Eigen::Ref<const MatrixX<symbolic::Expression>>& e, const std::set<int>& minor_indices) {
    // This function relies on AddPositiveSemidefiniteConstraint to validate the
    // documented symmetry prerequisite.
    return AddPositiveSemidefiniteConstraint(math::ExtractPrincipalSubmatrix(e, minor_indices));
}
...
Binding<LinearMatrixInequalityConstraint> MathematicalProgram::AddConstraint(
    ...
    DRAKE_ASSERT(static_cast<int>(binding.evaluator()->F().size()) == static_cast<int>(binding.GetNumElements()) + 1);
    required_capabilities_.insert(ProgramAttribute::kPositiveSemidefiniteConstraint);
    linear_matrix_inequality_constraint_.push_back(binding);
    return linear_matrix_inequality_constraint_.back();
}

Binding<LinearMatrixInequalityConstraint> MathematicalProgram::AddLinearMatrixInequalityConstraint(
        vector<Eigen::MatrixXd> F, const Eigen::Ref<const VectorXDecisionVariable>& vars) {
    auto constraint = make_shared<LinearMatrixInequalityConstraint>(std::move(F));
    return AddConstraint(constraint, vars);
}

MatrixX<symbolic::Expression> MathematicalProgram::AddPositiveDiagonallyDominantMatrixConstraint(
        const Eigen::Ref<const MatrixX<symbolic::Expression>>& X) {
    ...
    auto Y_upper = NewContinuousVariables((num_X_rows - 1) * num_X_rows / 2, "diagonally_dominant_slack");
...
MatrixX<symbolic::Expression> MathematicalProgram::TightenPsdConstraintToDd(
    ...
    const MatrixXDecisionVariable mat_vars =
            Eigen::Map<const MatrixXDecisionVariable>(constraint.variables().data(), n, n);
    return AddPositiveDiagonallyDominantMatrixConstraint(mat_vars.cast<Expression>());
}
...
Binding<LinearConstraint> MathematicalProgram::AddPositiveDiagonallyDominantDualConeMatrixConstraint(
        const Eigen::Ref<const MatrixX<symbolic::Expression>>& X) {
    const int n = X.rows();
    ...
    symbolic::DecomposeAffineExpressions(Eigen::Map<const VectorX<symbolic::Expression>>(X.data(), X.size()), &A_expr,
                                         &b_expr, &variables);
    const std::tuple<Eigen::SparseMatrix<double>, Eigen::VectorXd, Eigen::VectorXd> constraint_mats{
            ConstructPositiveDiagonallyDominantDualConeConstraintMatricesForN(n)};
    return AddLinearConstraint((std::get<0>(constraint_mats) * A_expr).sparseView(),                  // A * A_expr
                               std::get<1>(constraint_mats) - std::get<0>(constraint_mats) * b_expr,  // lb - A * b_expr
...
Binding<LinearConstraint> MathematicalProgram::AddPositiveDiagonallyDominantDualConeMatrixConstraint(
    ...
    const std::tuple<Eigen::SparseMatrix<double>, Eigen::VectorXd, Eigen::VectorXd> constraint_mats{
            ConstructPositiveDiagonallyDominantDualConeConstraintMatricesForN(n)};
    return AddLinearConstraint(std::get<0>(constraint_mats), std::get<1>(constraint_mats), std::get<2>(constraint_mats),
...
void AddSlackVariableForScaledDiagonallyDominantMatrixConstraint(
    ...
    *M_ij_diagonal = prog->NewContinuousVariables<2, Eigen::Dynamic>(2, (n - 1) * n / 2, "sdd_slack_M");
    int k = 0;
    M->resize(n);
    for (int i = 0; i < n; ++i) {
        (*M)[i].resize(n);
...
MathematicalProgram::AddScaledDiagonallyDominantMatrixConstraint(
    ...
    AddSlackVariableForScaledDiagonallyDominantMatrixConstraint<symbolic::Expression>(X, this, &M_ij_diagonal, &M);
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            ...
            AddRotatedLorentzConeConstraint(Vector3<symbolic::Expression>(M[i][j](0, 0), M[i][j](1, 1), M[i][j](0, 1)));
        }
...
std::vector<std::vector<Matrix2<symbolic::Variable>>> MathematicalProgram::AddScaledDiagonallyDominantMatrixConstraint(
        const Eigen::Ref<const MatrixX<symbolic::Variable>>& X) {
    const int n = X.rows();
    std::vector<std::vector<Matrix2<symbolic::Variable>>> M(n);
    Matrix2X<symbolic::Variable> M_ij_diagonal;
    AddSlackVariableForScaledDiagonallyDominantMatrixConstraint<symbolic::Variable>(X, this, &M_ij_diagonal, &M);
    ...
    auto rotated_lorentz_cone_constraint =
            std::make_shared<RotatedLorentzConeConstraint>(Eigen::Matrix3d::Identity(), Eigen::Vector3d::Zero());
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            ...
            AddConstraint(rotated_lorentz_cone_constraint,
                          Vector3<symbolic::Variable>(M[i][j](0, 0), M[i][j](1, 1), M[i][j](0, 1)));
        }
    }
    AddLinearEqualityConstraint(A_diagonal_sum, Eigen::VectorXd::Zero(n), diagonal_sum_var);
    return M;
}

std::vector<std::vector<Matrix2<symbolic::Variable>>> MathematicalProgram::TightenPsdConstraintToSdd(
        const Binding<PositiveSemidefiniteConstraint>& constraint) {
    RemoveConstraint(constraint);
    // Variables are flattened by the Flatten method, which flattens in
    // column-major order. This is the same convention as Eigen, so we can use the
...
MathematicalProgram::AddScaledDiagonallyDominantDualConeMatrixConstraint(
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = i + 1; j < n; ++j) {
            ...
            // can be
            ret.push_back(AddRotatedLorentzConeConstraint(
                    Vector3<symbolic::Expression>(X(i, i), X(j, j), 0.5 * (X(i, j) + X(j, i)))));
        }
    }
    return ret;
}
...
MathematicalProgram::AddScaledDiagonallyDominantDualConeMatrixConstraint(
        const Eigen::Ref<const MatrixX<symbolic::Variable>>& X) {
    return AddScaledDiagonallyDominantDualConeMatrixConstraint(X.cast<Expression>());
}

std::vector<Binding<RotatedLorentzConeConstraint>> MathematicalProgram::RelaxPsdConstraintToSddDualCone(
        const Binding<PositiveSemidefiniteConstraint>& constraint) {
    RemoveConstraint(constraint);
    // Variables are flattened by the Flatten method, which flattens in
    // column-major order. This is the same convention as Eigen, so we can use the
...
Binding<ExponentialConeConstraint> MathematicalProgram::AddExponentialConeConstraint(
...
std::vector<Binding<Cost>> MathematicalProgram::GetAllCosts() const {
    ...
    costlist.insert(costlist.end(), quadratic_costs_.begin(), quadratic_costs_.end());
    costlist.insert(costlist.end(), l2norm_costs_.begin(), l2norm_costs_.end());
    return costlist;
}

std::vector<Binding<LinearConstraint>> MathematicalProgram::GetAllLinearConstraints() const {
    std::vector<Binding<LinearConstraint>> conlist = linear_constraints_;
    conlist.insert(conlist.end(), linear_equality_constraints_.begin(), linear_equality_constraints_.end());
    return conlist;
}

std::vector<Binding<Constraint>> MathematicalProgram::GetAllConstraints() const {
...
bool MathematicalProgram::CheckSatisfied(const Binding<Constraint>& binding,
...
bool MathematicalProgram::CheckSatisfied(const std::vector<Binding<Constraint>>& bindings,
                                         const Eigen::Ref<const Eigen::VectorXd>& prog_var_vals,
                                         double tol) const {
...
bool MathematicalProgram::CheckSatisfiedAtInitialGuess(const Binding<Constraint>& binding, double tol) const {
    return CheckSatisfied(binding, x_initial_guess_, tol);
}

bool MathematicalProgram::CheckSatisfiedAtInitialGuess(const std::vector<Binding<Constraint>>& bindings,
                                                       double tol) const {
...
MatrixXDecisionVariable DoAddSosConstraint(MathematicalProgram* const prog,
...
MatrixXDecisionVariable MathematicalProgram::AddSosConstraint(
    ...
    if (Variables(p.indeterminates()).IsSubsetOf(indeterminates_vars) &&
        intersect(indeterminates_vars, Variables(p.decision_variables())).empty()) {
        ...
        Reparse(&p_reparsed);
        return DoAddSosConstraint(this, p_reparsed, monomial_basis, type, gram_name);
    }
}

pair<MatrixXDecisionVariable, VectorX<symbolic::Monomial>> MathematicalProgram::AddSosConstraint(
        const symbolic::Polynomial& p, MathematicalProgram::NonnegativePolynomial type, const std::string& gram_name) {
    const Variables indeterminates_vars{indeterminates()};
    if (Variables(p.indeterminates()).IsSubsetOf(indeterminates_vars) &&
        intersect(indeterminates_vars, Variables(p.decision_variables())).empty()) {
...
MatrixXDecisionVariable MathematicalProgram::AddSosConstraint(
        ...
        MathematicalProgram::NonnegativePolynomial type,
        const std::string& gram_name) {
    return AddSosConstraint(symbolic::Polynomial{e, symbolic::Variables{this->indeterminates()}}, monomial_basis, type,
                            gram_name);
}

pair<MatrixXDecisionVariable, VectorX<symbolic::Monomial>> MathematicalProgram::AddSosConstraint(
        const symbolic::Expression& e, MathematicalProgram::NonnegativePolynomial type, const std::string& gram_name) {
    return AddSosConstraint(symbolic::Polynomial{e, symbolic::Variables{this->indeterminates()}}, type, gram_name);
}

std::vector<Binding<LinearEqualityConstraint>> MathematicalProgram::AddEqualityConstraintBetweenPolynomials(
...
double MathematicalProgram::GetInitialGuess(const symbolic::Variable& decision_variable) const {
    return x_initial_guess_[FindDecisionVariableIndex(decision_variable)];
}

void MathematicalProgram::SetInitialGuess(const symbolic::Variable& decision_variable, double variable_guess_value) {
    x_initial_guess_(FindDecisionVariableIndex(decision_variable)) = variable_guess_value;
}

void MathematicalProgram::SetDecisionVariableValueInVector(const symbolic::Variable& decision_variable,
...
void MathematicalProgram::SetDecisionVariableValueInVector(
    ...
    DRAKE_THROW_UNLESS(values != nullptr);
    DRAKE_THROW_UNLESS(values->size() == num_vars());
    DRAKE_THROW_UNLESS(decision_variables.rows() == decision_variables_new_values.rows());
    DRAKE_THROW_UNLESS(decision_variables.cols() == decision_variables_new_values.cols());
    for (int i = 0; i < decision_variables.rows(); ++i) {
        for (int j = 0; j < decision_variables.cols(); ++j) {
...
void MathematicalProgram::EvalVisualizationCallbacks(const Eigen::Ref<const Eigen::VectorXd>& prog_var_vals) const {
    if (prog_var_vals.rows() != num_vars()) {
        std::ostringstream oss;
        oss << "The input binding variable is not in the right size. Expects " << num_vars()
            << " rows, but it actually has " << prog_var_vals.rows() << " rows.\n";
    ...
    for (auto const& binding : visualization_callbacks_) {
        ...
        for (int j = 0; j < num_v_variables; ++j) {
            this_x(j) = prog_var_vals(FindDecisionVariableIndex(binding.variables()(j)));
        }
...
void MathematicalProgram::SetVariableScaling(const symbolic::Variable& var, double s) {
    DRAKE_DEMAND(0 < s);
...
[[nodiscard]] bool IsVariableBound(const symbolic::Variable& var,
    ...
    if (IsVariableBound(var, prog.visualization_callbacks(), binding_description)) {
        return true;
    }
    return false;
}
...
int MathematicalProgram::RemoveDecisionVariable(const symbolic::Variable& var) {
    ...
    if (IsVariableBound(var, *this, &binding_description)) {
        throw std::invalid_argument(fmt::format("RemoveDecisionVariable: {} is associated with a {}.", var.get_name(),
                                                binding_description));
    }
...
template <typename C>
int MathematicalProgram::RemoveCostOrConstraintImpl(const Binding<C>& removal,
                                                    ProgramAttribute affected_capability,
                                                    std::vector<Binding<C>>* existings) {
    const int num_existing = static_cast<int>(existings->size());
    existings->erase(std::remove(existings->begin(), existings->end(), removal), existings->end());
...
void MathematicalProgram::UpdateRequiredCapability(ProgramAttribute query_capability) {
    switch (query_capability) {
        case ProgramAttribute::kLinearCost: {
            UpdateRequiredCapabilityImpl(query_capability, this->linear_costs(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kQuadraticCost: {
            UpdateRequiredCapabilityImpl(query_capability, this->quadratic_costs(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kGenericCost: {
            UpdateRequiredCapabilityImpl(query_capability, this->generic_costs(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kGenericConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->generic_constraints(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kLinearConstraint: {
            if (this->linear_constraints().empty() && this->bounding_box_constraints().empty()) {
        ...
        case ProgramAttribute::kLinearEqualityConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->linear_equality_constraints(),
                                         &required_capabilities_);
            break;
        }
        case ProgramAttribute::kLinearComplementarityConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->linear_complementarity_constraints(),
                                         &required_capabilities_);
            break;
        }
        case ProgramAttribute::kLorentzConeConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->lorentz_cone_constraints(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kRotatedLorentzConeConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->rotated_lorentz_cone_constraints(),
                                         &required_capabilities_);
            break;
        }
        case ProgramAttribute::kPositiveSemidefiniteConstraint: {
            if (positive_semidefinite_constraint_.empty() && linear_matrix_inequality_constraint_.empty()) {
                required_capabilities_.erase(ProgramAttribute::kPositiveSemidefiniteConstraint);
            } else {
                required_capabilities_.emplace(ProgramAttribute::kPositiveSemidefiniteConstraint);
            }
            break;
        }
        case ProgramAttribute::kExponentialConeConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->exponential_cone_constraints(),
                                         &required_capabilities_);
            break;
        }
        case ProgramAttribute::kQuadraticConstraint: {
            UpdateRequiredCapabilityImpl(query_capability, this->quadratic_constraints(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kL2NormCost: {
            UpdateRequiredCapabilityImpl(query_capability, this->l2norm_costs(), &required_capabilities_);
            break;
        }
        case ProgramAttribute::kBinaryVariable: {
            bool has_binary_var = false;
            for (int i = 0; i < num_vars(); ++i) {
                if (decision_variables_[i].get_type() == symbolic::Variable::Type::BINARY) {
                    has_binary_var = true;
                    break;
                }
            }
        ...
        case ProgramAttribute::kCallback: {
            UpdateRequiredCapabilityImpl(query_capability, visualization_callbacks_, &required_capabilities_);
            break;
        }
    }
}
...
int MathematicalProgram::RemoveCost(const Binding<Cost>& cost) {
    ...
    if (dynamic_cast<QuadraticCost*>(cost_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<QuadraticCost>(cost),
                                          ProgramAttribute::kQuadraticCost, &(this->quadratic_costs_));
    } else if (dynamic_cast<LinearCost*>(cost_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LinearCost>(cost), ProgramAttribute::kLinearCost,
                                          &(this->linear_costs_));
    } else if (dynamic_cast<L2NormCost*>(cost_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<L2NormCost>(cost), ProgramAttribute::kL2NormCost,
                                          &(this->l2norm_costs_));
    } else {
        return RemoveCostOrConstraintImpl(cost, ProgramAttribute::kGenericCost, &(this->generic_costs_));
    }
    DRAKE_UNREACHABLE();
}

int MathematicalProgram::RemoveConstraint(const Binding<Constraint>& constraint) {
    ...
    if (dynamic_cast<ExponentialConeConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<ExponentialConeConstraint>(constraint),
                                          ProgramAttribute::kExponentialConeConstraint, &exponential_cone_constraints_);
    } else if (dynamic_cast<LinearMatrixInequalityConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LinearMatrixInequalityConstraint>(constraint),
                                          ProgramAttribute::kPositiveSemidefiniteConstraint,
                                          &linear_matrix_inequality_constraint_);
    } else if (dynamic_cast<PositiveSemidefiniteConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<PositiveSemidefiniteConstraint>(constraint),
                                          ProgramAttribute::kPositiveSemidefiniteConstraint,
                                          &positive_semidefinite_constraint_);
    } else if (dynamic_cast<QuadraticConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<QuadraticConstraint>(constraint),
                                          ProgramAttribute::kQuadraticConstraint, &quadratic_constraints_);
    } else if (dynamic_cast<RotatedLorentzConeConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<RotatedLorentzConeConstraint>(constraint),
                                          ProgramAttribute::kRotatedLorentzConeConstraint,
                                          &rotated_lorentz_cone_constraint_);
    } else if (dynamic_cast<LorentzConeConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LorentzConeConstraint>(constraint),
                                          ProgramAttribute::kLorentzConeConstraint, &lorentz_cone_constraint_);
    } else if (dynamic_cast<LinearComplementarityConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LinearComplementarityConstraint>(constraint),
        ...
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LinearEqualityConstraint>(constraint),
                                          ProgramAttribute::kLinearEqualityConstraint, &linear_equality_constraints_);
        ...
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<BoundingBoxConstraint>(constraint),
                                          ProgramAttribute::kLinearConstraint, &bbox_constraints_);
    } else if (dynamic_cast<LinearConstraint*>(constraint_evaluator)) {
        return RemoveCostOrConstraintImpl(internal::BindingDynamicCast<LinearConstraint>(constraint),
        ...
        return RemoveCostOrConstraintImpl(constraint, ProgramAttribute::kGenericConstraint, &generic_constraints_);
    }
    DRAKE_UNREACHABLE();
}

int MathematicalProgram::RemoveVisualizationCallback(const Binding<VisualizationCallback>& callback) {
    return RemoveCostOrConstraintImpl(callback, ProgramAttribute::kCallback, &visualization_callbacks_);
}
...
void MathematicalProgram::CheckVariableType(VarType var_type) {
    switch (var_type) {
        ...
        case VarType::INTEGER:
            throw std::runtime_error("MathematicalProgram does not support integer variables yet.");
        case VarType::BOOLEAN:
            throw std::runtime_error("MathematicalProgram does not support Boolean variables.");
        case VarType::RANDOM_UNIFORM:
            throw std::runtime_error("MathematicalProgram does not support random uniform variables.");
        case VarType::RANDOM_GAUSSIAN:
            throw std::runtime_error("MathematicalProgram does not support random Gaussian variables.");
...
void MathematicalProgram::CheckIsDecisionVariable(const VectorXDecisionVariable& vars) const {
    for (int i = 0; i < vars.rows(); ++i) {
        for (int j = 0; j < vars.cols(); ++j) {
            if (!decision_variable_index_.contains(vars(i, j).get_id())) {
                throw std::logic_error(
                        fmt::format("{} is not a decision variable of the mathematical program.", vars(i, j)));
            }
        }
    }
}
