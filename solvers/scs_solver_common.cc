...
bool CheckAttributes(const MathematicalProgram& prog, std::string* explanation) {
    ...
    return internal::CheckConvexSolverAttributes(prog, solver_capabilities.access(), "ScsSolver", explanation);
}
}  // namespace
...
std::string ScsSolver::UnsatisfiedProgramAttributes(const MathematicalProgram& prog) {
