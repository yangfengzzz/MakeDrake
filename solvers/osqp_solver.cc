    ...
    if (!scale_map.empty()) {
        for (auto& triplet : P_upper_triplets) {
            ...
            if (column != scale_map.end()) {
                triplet = Eigen::Triplet<double>(triplet.row(), triplet.col(), triplet.value() * (column->second));
            }
            // Row
            const auto row = scale_map.find(triplet.row());
            if (row != scale_map.end()) {
                triplet = Eigen::Triplet<double>(triplet.row(), triplet.col(), triplet.value() * (row->second));
            }
        }
    }
...
void ParseLinearCosts(const MathematicalProgram& prog, std::vector<c_float>* q, double* constant_cost_term) {
...
void ParseLinearConstraints(const MathematicalProgram& prog,
    ...
    for (const auto& constraint : linear_constraints) {
        const std::vector<int> x_indices = prog.FindDecisionVariableIndices(constraint.variables());
        const std::vector<Eigen::Triplet<double>> Ai_triplets =
                math::SparseMatrixToTriplets(constraint.evaluator()->get_sparse_A());
        const Binding<Constraint> constraint_cast = internal::BindingDynamicCast<Constraint>(constraint);
        ...
        for (const auto& Ai_triplet : Ai_triplets) {
            A_triplets->emplace_back(*num_A_rows + Ai_triplet.row(), x_indices[Ai_triplet.col()],
...
void ParseBoundingBoxConstraints(const MathematicalProgram& prog,
    ...
    for (const auto& constraint : prog.bounding_box_constraints()) {
        ...
        for (int i = 0; i < static_cast<int>(constraint.GetNumElements()); ++i) {
            A_triplets->emplace_back(*num_A_rows + i, prog.FindDecisionVariableIndex(constraint.variables()(i)),
...
void ParseAllLinearConstraints(const MathematicalProgram& prog,
    ...
    u->clear();
    int num_A_rows = 0;
    ParseLinearConstraints(prog, prog.linear_constraints(), &A_triplets, l, u, &num_A_rows, constraint_start_row);
    ParseLinearConstraints(prog, prog.linear_equality_constraints(), &A_triplets, l, u, &num_A_rows,
                           constraint_start_row);
    ParseBoundingBoxConstraints(prog, &A_triplets, l, u, &num_A_rows, constraint_start_row);
...
csc* EigenSparseToCSC(const Eigen::SparseMatrix<c_float>& mat) {
    ...
    c_float* values = static_cast<c_float*>(c_malloc(sizeof(c_float) * mat.nonZeros()));
    c_int* inner_indices = static_cast<c_int*>(c_malloc(sizeof(c_int) * mat.nonZeros()));
    c_int* outer_indices = static_cast<c_int*>(c_malloc(sizeof(c_int) * (mat.cols() + 1)));
    ...
    return csc_matrix(mat.rows(), mat.cols(), mat.nonZeros(), values, inner_indices, outer_indices);
}
...
template <typename T1, typename T2>
void SetOsqpSolverSettingWithDefaultValue(const std::unordered_map<std::string, T1>& options,
                                          const std::string& option_name,
                                          T2* osqp_setting_field,
...
void SetOsqpSolverSettings(const SolverOptions& solver_options, OSQPSettings* settings) {
    const std::unordered_map<std::string, double>& options_double = solver_options.GetOptionsDouble(OsqpSolver::id());
    const std::unordered_map<std::string, int>& options_int = solver_options.GetOptionsInt(OsqpSolver::id());
    ...
    SetOsqpSolverSetting(options_double, "eps_prim_inf", &(settings->eps_prim_inf));
    SetOsqpSolverSetting(options_double, "eps_dual_inf", &(settings->eps_dual_inf));
    ...
    SetOsqpSolverSettingWithDefaultValue(options_int, "polish", &(settings->polish), 1);
    ...
    SetOsqpSolverSettingWithDefaultValue(options_int, "verbose", &(settings->verbose), verbose_console);
    SetOsqpSolverSetting(options_int, "scaled_termination", &(settings->scaled_termination));
    SetOsqpSolverSetting(options_int, "check_termination", &(settings->check_termination));
    ...
    SetOsqpSolverSettingWithDefaultValue(options_int, "adaptive_rho_interval", &(settings->adaptive_rho_interval),
                                         ADAPTIVE_RHO_FIXED);
    SetOsqpSolverSetting(options_double, "adaptive_rho_tolerance", &(settings->adaptive_rho_tolerance));
    SetOsqpSolverSetting(options_double, "adaptive_rho_fraction", &(settings->adaptive_rho_fraction));
    SetOsqpSolverSetting(options_double, "time_limit", &(settings->time_limit));
}

template <typename C>
void SetDualSolution(const std::vector<Binding<C>>& constraints,
    ...
    for (const auto& constraint : constraints) {
        ...
        // as the shadow price.
        const Binding<Constraint> constraint_cast = internal::BindingDynamicCast<Constraint>(constraint);
        result->set_dual_solution(constraint, -all_dual_solution.segment(constraint_start_row.at(constraint_cast),
    ...
    if (!solution_result) {
        ...
        switch (work->info->status_val) {
            ...
            case OSQP_SOLVED_INACCURATE: {
                const Eigen::Map<Eigen::Matrix<c_float, Eigen::Dynamic, 1>> osqp_sol(work->solution->x,
                                                                                     prog.num_vars());
                ...
                solver_details.y = Eigen::Map<Eigen::VectorXd>(work->solution->y, work->data->m);
                solution_result = SolutionResult::kSolutionFound;
                SetDualSolution(prog.linear_constraints(), solver_details.y, constraint_start_row, result);
                SetDualSolution(prog.linear_equality_constraints(), solver_details.y, constraint_start_row, result);
                SetDualSolution(prog.bounding_box_constraints(), solver_details.y, constraint_start_row, result);
