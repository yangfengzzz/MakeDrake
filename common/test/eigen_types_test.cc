...
void CheckTraits(bool is_vector) {
    ...
    EXPECT_EQ(!is_vector, (is_eigen_nonvector_of<Derived, double>::value)) << info;
}
...
GTEST_TEST(EigenTypesTest, LikewiseStorageOrder) {
    ...
    EXPECT_EQ(static_cast<int>(LikewiseCol::Options) & Eigen::ColMajor, Eigen::ColMajor);
    EXPECT_EQ(static_cast<int>(LikewiseRow::Options) & Eigen::RowMajor, Eigen::RowMajor);
}
