...
#include "common/test/hwy_dynamic_test_array_mul.h"
...
TEST_P(Fixture, ArrayMulTest) {
    ...
    const std::string expected_impl = fmt::format("ArrayMulImpl.{}", hwy::TargetName(GetParam()));
    using Pair = std::pair<std::string, int>;
    string_map<int>& counters = GetMutableArrayMulCounters();
    EXPECT_THAT(counters, testing::UnorderedElementsAre(Pair("ChooseBestArrayMul", 1), Pair(expected_impl, 2)));
}
