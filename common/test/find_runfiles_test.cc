...
GTEST_TEST(FindRunfilesTest, AcceptanceTest) {
    ...
    const auto result = FindRunfile("drake/common/test/find_resource_test_data.txt");
...
GTEST_TEST(FindRunfilesTest, NotDeclaredTest) {
    ...
    EXPECT_THAT(result.error, testing::MatchesRegex("Sought 'foo' in runfiles directory '.*' but "
...
GTEST_TEST(FindRunfilesTest, AbsolutePathTest) {
    ...
    EXPECT_THAT(result.error, testing::MatchesRegex(".*must not be an absolute path.*"));
};
