...
#define BINARY_TEST(OP, OP_NAME)                                                                   \
    template <typename T, typename U, typename = decltype(std::declval<T>() OP std::declval<U>())> \
...
TEST_F(IdentifierTests, Convertible) {
    static_assert(!std::is_convertible_v<AId, BId>, "Identifiers of different types should not be convertible.");
    static_assert(!std::is_convertible_v<AId, int>, "Identifiers should not be convertible to ints.");
    static_assert(!std::is_convertible_v<int, AId>, "Identifiers should not be convertible from ints");
}
...
TEST_F(IdentifierTests, BadInvalidOrSameComparison) {
    ...
    AId invalid;
    DRAKE_EXPECT_THROWS_MESSAGE(a1_.is_same_as_valid_id(invalid), ".*is_valid.*failed.*");
}
