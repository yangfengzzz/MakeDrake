...
GTEST_TEST(StaticPointerCastTest, DowncastConst) {
    ...
    std::unique_ptr<const Derived> t = static_pointer_cast<const Derived>(std::move(u));
    EXPECT_EQ(!!u, false);
    EXPECT_EQ(!!t, true);
}
...
GTEST_TEST(DynamicPointerCastTest, DowncastConst) {
    std::unique_ptr<const Base> u = std::make_unique<const Derived>();
    std::unique_ptr<const Derived> t = dynamic_pointer_cast<const Derived>(std::move(u));
    EXPECT_EQ(!!u, false);
    EXPECT_EQ(!!t, true);
}
...
GTEST_TEST(DynamicPointerCastOrThrowTest, Downcast) {
    std::unique_ptr<Base> u = std::make_unique<Derived>();
    std::unique_ptr<Derived> t = dynamic_pointer_cast_or_throw<Derived>(std::move(u));
    EXPECT_EQ(!!u, false);
    EXPECT_EQ(!!t, true);
}
...
GTEST_TEST(DynamicPointerCastOrThrowTest, FailedCastFromNullptr) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dynamic_pointer_cast_or_throw<MoreDerived>(std::move(u)),
...
GTEST_TEST(DynamicPointerCastOrThrowTest, DowncastConst) {
    std::unique_ptr<const Base> u = std::make_unique<const Derived>();
    std::unique_ptr<const Derived> t = dynamic_pointer_cast_or_throw<const Derived>(std::move(u));
    EXPECT_EQ(!!u, false);
    EXPECT_EQ(!!t, true);
}
