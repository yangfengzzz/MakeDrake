...
class SetEnv {
public:
    SetEnv(const std::string& var_name, std::optional<std::string> var_value) : var_name_(var_name) {
...
GTEST_TEST(FindCacheTest, TestTmpdir) {
    ...
    const fs::path expected = fs::path(std::getenv("TEST_TMPDIR")) / ".cache" / "drake" / "foo";
