...
GTEST_TEST(TempDirectoryTest, TestTmpdirSet) {
    ...
    EXPECT_THAT(temp_path.filename().string(), testing::MatchesRegex("robotlocomotion_drake_[^/]{6}$"));
}
...
GTEST_TEST(TempDirectoryTest, TestTmpdirUnset) {
    ...
    EXPECT_THAT(temp_directory_result, ::testing::StartsWith(expected_prefix.string()));
}
