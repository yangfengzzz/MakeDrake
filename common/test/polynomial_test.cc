...
void testIntegralAndDerivative() {
    ...
    EXPECT_TRUE(CompareMatrices(poly.GetCoefficients(), poly.Derivative(0).GetCoefficients(), 1e-14,
                                MatrixCompareType::absolute));

    const T x{1.32};
    Polynomial<T> first_derivative = poly.Derivative(1);
    EXPECT_NEAR(poly.EvaluateUnivariate(x, 1), first_derivative.EvaluateUnivariate(x), 1e-14);

    Polynomial<T> third_derivative = poly.Derivative(3);
    EXPECT_NEAR(poly.EvaluateUnivariate(x, 3), third_derivative.EvaluateUnivariate(x), 1e-14);

    Polynomial<T> third_derivative_check = poly.Derivative().Derivative().Derivative();

    EXPECT_TRUE(CompareMatrices(third_derivative.GetCoefficients(), third_derivative_check.GetCoefficients(), 1e-14,
                                MatrixCompareType::absolute));

    Polynomial<T> tenth_derivative = poly.Derivative(10);

    EXPECT_TRUE(
            CompareMatrices(tenth_derivative.GetCoefficients(), VectorXd::Zero(1), 1e-14, MatrixCompareType::absolute));

    Polynomial<T> integral = poly.Integral(0.0);
    Polynomial<T> poly_back = integral.Derivative();

    EXPECT_TRUE(
            CompareMatrices(poly_back.GetCoefficients(), poly.GetCoefficients(), 1e-14, MatrixCompareType::absolute));
}
...
void testOperators() {
    ...
    for (int i = 0; i < num_tests; ++i) {
        ...
        EXPECT_NEAR(sum.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) + poly2.EvaluateUnivariate(t), 1e-8);
        EXPECT_NEAR(difference.EvaluateUnivariate(t), poly2.EvaluateUnivariate(t) - poly1.EvaluateUnivariate(t), 1e-8);
        EXPECT_NEAR(product.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) * poly2.EvaluateUnivariate(t), 1e-8);
        EXPECT_NEAR(poly1_plus_scalar.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) + scalar, 1e-8);
        EXPECT_NEAR(poly1_minus_scalar.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) - scalar, 1e-8);
        EXPECT_NEAR(poly1_scaled.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) * scalar, 1e-8);
        EXPECT_NEAR(poly1_div.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) / scalar, 1e-8);
        EXPECT_NEAR(poly1_times_poly1.EvaluateUnivariate(t), poly1.EvaluateUnivariate(t) * poly1.EvaluateUnivariate(t),
                    1e-8);
        EXPECT_NEAR(pow_poly1_3.EvaluateUnivariate(t), pow(poly1.EvaluateUnivariate(t), 3), 1e-8);
        EXPECT_NEAR(pow_poly1_4.EvaluateUnivariate(t), pow(poly1.EvaluateUnivariate(t), 4), 1e-8);
        EXPECT_NEAR(pow_poly1_10.EvaluateUnivariate(t), pow(poly1.EvaluateUnivariate(t), 10), 1e-8);
        EXPECT_NEAR(pow_poly1_3.EvaluateUnivariate(t) * pow_poly1_4.EvaluateUnivariate(t) *
    ...
    for (int row = 0; row < A.rows(); ++row) {
        for (int col = 0; col < A.cols(); ++col) {
            ...
            EXPECT_NEAR(sum(row, col).evaluateUnivariate(t),
                        A(row, col).evaluateUnivariate(t) + C(row, col).evaluateUnivariate(t), 1e-8);

            double expected_product = 0.0;
            for (int i = 0; i < A.cols(); ++i) {
                expected_product += A(row, i).evaluateUnivariate(t) * B(i, col).evaluateUnivariate(t);
            }
            EXPECT_NEAR(product(row, col).evaluateUnivariate(t), expected_product, 1e-8);
        }
    }
...
GTEST_TEST(PolynomialTest, MultivariateValue) {
    ...
    const std::map<Polynomiald::VarType, double> eval_point = {{x.GetSimpleVariable(), 1}, {y.GetSimpleVariable(), 2}};
    ...
    EXPECT_NEAR(pow_p_2.EvaluateMultivariate(eval_point), pow(p.EvaluateMultivariate(eval_point), 2), 1e-8);
    EXPECT_NEAR(pow_p_3.EvaluateMultivariate(eval_point), pow(p.EvaluateMultivariate(eval_point), 3), 1e-8);
    EXPECT_NEAR(pow_p_7.EvaluateMultivariate(eval_point), pow(p.EvaluateMultivariate(eval_point), 7), 1e-8);
...
GTEST_TEST(PolynomialTest, EvaluatePartial) {
    ...
    const std::map<Polynomiald::VarType, double> eval_point_x = {{x.GetSimpleVariable(), 7}};
    const std::map<Polynomiald::VarType, double> eval_point_y = {{y.GetSimpleVariable(), 11}};
    const std::map<Polynomiald::VarType, double> eval_point_xy = {{x.GetSimpleVariable(), 7},
                                                                  {y.GetSimpleVariable(), 11}};

    // Test a couple of straightforward explicit cases.
    EXPECT_EQ(dut.EvaluatePartial(eval_point_null).GetMonomials(), dut.GetMonomials());
  ...
  EXPECT_EQ(dut.evaluatePartial(eval_point_y).getMonomials(),
    ...
    EXPECT_EQ(dut.EvaluatePartial(eval_point_null).EvaluateMultivariate(eval_point_xy), expected_result);
    EXPECT_EQ(dut.EvaluatePartial(eval_point_xy).EvaluateMultivariate(eval_point_null), expected_result);
    EXPECT_EQ(dut.EvaluatePartial(eval_point_x).EvaluateMultivariate(eval_point_y), expected_result);
    EXPECT_EQ(dut.EvaluatePartial(eval_point_y).EvaluateMultivariate(eval_point_x), expected_result);

    // Test that zeroing out one term gives a sensible result.
    EXPECT_EQ(dut.EvaluatePartial(std::map<Polynomiald::VarType, double>{{x.GetSimpleVariable(), 0}}), (2 * y) + 1);
    EXPECT_EQ(dut.EvaluatePartial(std::map<Polynomiald::VarType, double>{{y.GetSimpleVariable(), 0}}),
              (5 * x * x * x) + 1);
}
...
GTEST_TEST(PolynomialTest, FromExpression) {
    ...
    EXPECT_NEAR(e0.Evaluate(env), Polynomial<double>::FromExpression(e0).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e1.Evaluate(env), Polynomial<double>::FromExpression(e1).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e2.Evaluate(env), Polynomial<double>::FromExpression(e2).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e3.Evaluate(env), Polynomial<double>::FromExpression(e3).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e4.Evaluate(env), Polynomial<double>::FromExpression(e4).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e5.Evaluate(env), Polynomial<double>::FromExpression(e5).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e6.Evaluate(env), Polynomial<double>::FromExpression(e6).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e7.Evaluate(env), Polynomial<double>::FromExpression(e7).EvaluateMultivariate(eval_point), 1e-8);
    EXPECT_NEAR(e8.Evaluate(env), Polynomial<double>::FromExpression(e8).EvaluateMultivariate(eval_point), 1e-8);

    const vector<Expression> test_vec{log(x),
                                      abs(x),
                                      exp(x),
                                      sqrt(x),
                                      sin(x),
...
void TestScalarType() {
    ...
    EXPECT_NEAR(ExtractDoubleOrThrow(p.EvaluateUnivariate(0.0)), coeffs(0), 1e-14);
    ...
    Polynomial<T> y("y", 1);
    const std::map<Polynomiald::VarType, double> eval_point = {{x.GetSimpleVariable(), 1}, {y.GetSimpleVariable(), 2}};
    EXPECT_NEAR(ExtractDoubleOrThrow((x * x + y).EvaluateMultivariate(eval_point)), 3, 1e-14);
}
...
GTEST_TEST(PolynomialTest, CoefficientsAlmostEqualTest) {
    ...
    EXPECT_TRUE((x + x * x + 3 * pow(x, 3)).CoefficientsAlmostEqual(3 * pow(x, 3) + x + x * x));

    // Test relative and absolute tolerance.
    EXPECT_TRUE((100 * x).CoefficientsAlmostEqual(99 * x, 0.1, ToleranceType::kRelative));
    EXPECT_FALSE((100 * x).CoefficientsAlmostEqual(99 * x, 0.1, ToleranceType::kAbsolute));
    EXPECT_FALSE((0.01 * x).CoefficientsAlmostEqual(0.02 * x, 0.1, ToleranceType::kRelative));
    EXPECT_TRUE((0.01 * x).CoefficientsAlmostEqual(0.02 * x, 0.1, ToleranceType::kAbsolute));
    ...
    EXPECT_TRUE((x + y + 0.01 * x * x).CoefficientsAlmostEqual(x + y, 0.02, ToleranceType::kAbsolute));
    EXPECT_TRUE((x + y).CoefficientsAlmostEqual(x + y + 0.01 * x * x, 0.02, ToleranceType::kAbsolute));
    EXPECT_FALSE((x + y + 0.01 * x * x).CoefficientsAlmostEqual(x + y, 0.02, ToleranceType::kRelative));
    EXPECT_FALSE((x + y).CoefficientsAlmostEqual(x + y + 0.01 * x * x, 0.02, ToleranceType::kRelative));
}
...
GTEST_TEST(PolynomialTest, SubsitutionTest) {
    ...
    EXPECT_TRUE(p1.Substitute(xvar, 1 + x).CoefficientsAlmostEqual(1 + x + 3 * (1 + x) * (1 + x) + 3 * y));
    EXPECT_TRUE(p1.Substitute(yvar, 1 + x).CoefficientsAlmostEqual(3 + 4 * x + 3 * x * x));
    EXPECT_TRUE(p1.Substitute(xvar, x * x).CoefficientsAlmostEqual(x * x + 3 * pow(x, 4) + 3 * y));
}
