...
GTEST_TEST(IsApproxEqualAbstolPermutationTest, PermutationTest) {
  ...
  test << 1, 2, 3,
    ...
    EXPECT_FALSE(IsApproxEqualAbsTolWithPermutedColumns(test, test.leftCols<2>(), tol));
    EXPECT_FALSE(IsApproxEqualAbsTolWithPermutedColumns(test.leftCols<2>(), test, tol));
