...
GTEST_TEST(TypeSafeIndex, Constructor) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(AIndex(-1), "Explicitly constructing an invalid index.+");
...
GTEST_TEST(TypeSafeIndex, ConversionToInt) {
    ...
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(static_cast<int>(invalid), "Converting to an int.+");
}
...
GTEST_TEST(TypeSafeIndex, InvalidIndexComparisonOperators) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid == valid, "Testing == with invalid LHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(valid == invalid, "Testing == with invalid RHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid != valid, "Testing != with invalid LHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(valid != invalid, "Testing != with invalid RHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid < valid, "Testing < with invalid LHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(valid < invalid, "Testing < with invalid RHS.+");
...
GTEST_TEST(TypeSafeIndex, PrefixIncrement) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(++max_index, "Pre-incrementing produced an invalid index.+");
    // Increment invalid index.
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(++invalid, "Pre-incrementing an invalid index.+");
}
...
GTEST_TEST(TypeSafeIndex, PostfixIncrement) {
    ...
    // Overflow produces an invalid index.
    AIndex max_index(std::numeric_limits<int>::max());
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(max_index++, "Post-incrementing produced an invalid index.+");
    // Increment invalid index.
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid++, "Post-incrementing an invalid index.+");
}
...
GTEST_TEST(TypeSafeIndex, PrefixDecrement) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(--about_to_be_negative_index, "Pre-decrementing produced an invalid index.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(--AIndex(), "Pre-decrementing an invalid index.+");
}
...
GTEST_TEST(TypeSafeIndex, PostfixDecrement) {
    ...
    // In Debug builds decrements leading to a negative result will throw.
    AIndex about_to_be_negative_index(0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(about_to_be_negative_index--, "Post-decrementing produced an invalid index.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(AIndex()--, "Post-decrementing an invalid index.+");
}
...
GTEST_TEST(TypeSafeIndex, InPlaceAddition) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(about_to_be_negative_index += (-9),
                                         "In-place addition with an int produced an invalid index.+");
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid += 1, "In-place addition with an int on an invalid index.+");

    // In-place with an index.
    AIndex max_index(std::numeric_limits<int>::max());
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(max_index += AIndex(1),
                                         "In-place addition with another index produced an invalid index.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid += AIndex(1), "In-place addition with another index invalid LHS.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(index += invalid, "In-place addition with another index invalid RHS.+");
}
...
GTEST_TEST(TypeSafeIndex, InPlaceSubtract) {
    ...
    // In-place with an int.
    AIndex about_to_be_negative_index(0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(about_to_be_negative_index -= 7,
                                         "In-place subtraction with an int produced an invalid index.+");
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid -= -3, "In-place subtraction with an int on an invalid index.+");

    // In-place with an index.
    about_to_be_negative_index = AIndex(0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(about_to_be_negative_index -= AIndex(1),
                                         "In-place subtraction with another index produced an invalid index.+");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(invalid -= AIndex(1), "In-place subtraction with another index invalid LHS.+");
    about_to_be_negative_index = AIndex(0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(about_to_be_negative_index -= invalid,
...
GTEST_TEST(TypeSafeIndex, StreamInsertion) {
    ...
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(stream << invalid, "Converting to an int.+");
}
...
GTEST_TEST(TypeSafeIndex, ToString) {
    ...
    AIndex invalid;
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(std::to_string(invalid), "Converting to an int.+");
}
...
GTEST_TEST(IntegralComparisons, CompareInt64) {
    ...
    auto indirect_call = [](const int64_t local_value) {
        AIndex a;
        DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(a = AIndex(local_value),
...
GTEST_TEST(IntegralComparisons, CompareSizeT) {
    ...
    EXPECT_LT(static_cast<int>(small_overflow_value), 0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(AIndex(small_overflow_value),
                                         "Explicitly constructing an invalid index. Type .* has an invalid "
                                         "value; it must lie in the range .*");
    ...
    EXPECT_GT(static_cast<int>(big_overflow_value), 0);
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(AIndex(big_overflow_value),
                                         "Explicitly constructing an invalid index. Type .* has an invalid "
                                         "value; it must lie in the range .*");
