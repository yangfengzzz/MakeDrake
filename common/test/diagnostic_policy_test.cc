...
GTEST_TEST(DiagnosticPolicyTest, ErrorDefaultAction) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.ErrorDefaultAction(detail), "a.txt:22: error: well");
}
...
GTEST_TEST(DiagnosticPolicyTest, SetActionForWarnings) {
    ...
    auto warning_action = [&details](const DiagnosticDetail& d) { details.push_back(d); };
...
GTEST_TEST(DiagnosticPolicyTest, SetActionForErrors) {
    ...
    std::vector<DiagnosticDetail> details;
    auto error_action = [&details](const DiagnosticDetail& d) { details.push_back(d); };
