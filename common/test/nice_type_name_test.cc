...
GTEST_TEST(NiceTypeNameTest, Canonicalize) {
    ...
    EXPECT_EQ(NiceTypeName::Canonicalize("class std :: vector < double   >"), "std::vector<double>");
    ...
    EXPECT_EQ(NiceTypeName::Canonicalize("std::my__1::__23x::resigned char"), "std::my__1::resigned char");
}
...
GTEST_TEST(NiceTypeNameTest, BuiltIns) {
    ...
    EXPECT_EQ(NiceTypeName::Get<std::complex<long double>>(), "std::complex<long double>");
}
...
GTEST_TEST(NiceTypeNameTest, StdClasses) {
    ...
    EXPECT_EQ(NiceTypeName::Get<std::vector<int>>(), "std::vector<int,std::allocator<int>>");
    EXPECT_EQ(NiceTypeName::Get<std::vector<std::string>>(), "std::vector<std::string,std::allocator<std::string>>");

    // Try non-standard allocator.
    using NonStdAlloc = std::vector<unsigned, std::allocator<unsigned>>;
    EXPECT_EQ(NiceTypeName::Get<NonStdAlloc>(), "std::vector<unsigned int,std::allocator<unsigned int>>");
}
...
GTEST_TEST(NiceTypeNameTest, Eigen) {
    ...
    EXPECT_EQ(NiceTypeName::Get<PairType>(), "std::pair<Eigen::Vector2i,Eigen::Vector3d>");
}
...
GTEST_TEST(NiceTypeNameTest, Enum) {
    ...
    EXPECT_EQ(NiceTypeName::Get<ForTesting::MyEnum>(), "drake::(anonymous)::ForTesting::MyEnum");
    EXPECT_EQ(NiceTypeName::Get<ForTesting::MyEnumClass>(), "drake::(anonymous)::ForTesting::MyEnumClass");

    EXPECT_EQ(NiceTypeName::Get<decltype(ForTesting::One)>(), "drake::(anonymous)::ForTesting::MyEnum");
...
GTEST_TEST(NiceTypeNameTest, Expressions) {
    ...
    EXPECT_NE(NiceTypeName::Get<decltype(*base)>(), NiceTypeName::Get<decltype(derived)>());
    ...
    EXPECT_EQ(NiceTypeName::Get<decltype(base_uptr)>(), NiceTypeName::Get(base_uptr));
}

GTEST_TEST(NiceTypeNameTest, RemoveNamespaces) {
    EXPECT_EQ(NiceTypeName::RemoveNamespaces("JustAPlainType"), "JustAPlainType");
    EXPECT_EQ(NiceTypeName::RemoveNamespaces("drake::(anonymous)::Derived"), "Derived");
    // Should ignore nested namespaces.
    EXPECT_EQ(NiceTypeName::RemoveNamespaces("std::vector<std::string,std::allocator<std::string>>"),
              "vector<std::string,std::allocator<std::string>>");
    // Should stop at the first templatized segment.
    EXPECT_EQ(NiceTypeName::RemoveNamespaces("drake::systems::sensors::RgbdRenderer<T>::Impl"),
              "RgbdRenderer<T>::Impl");
...
GTEST_TEST(NiceTypeNameTest, Override) {
    internal::SetNiceTypeNamePtrOverride([](const internal::type_erased_ptr& ptr) -> std::string {
    ...
    EXPECT_EQ(NiceTypeName::Get<OverrideName>(), "drake::(anonymous)::OverrideName");
