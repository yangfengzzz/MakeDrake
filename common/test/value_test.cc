...
struct CloneableInt {
    ...
    std::unique_ptr<CloneableInt> Clone() const { return std::make_unique<CloneableInt>(data); }
...
struct MoveOrCloneInt {
    ...
    std::unique_ptr<MoveOrCloneInt> Clone() const { return std::make_unique<MoveOrCloneInt>(data); }
...
typedef ::testing::Types<int, CopyableInt, CloneableInt, MoveOrCloneInt> Implementations;
...
GTEST_TEST(ValueTest, DefaultConstructor) {
    ...
    static_assert(!std::is_default_constructible_v<Value<CopyableInt>>, "Value<CopyableInt>() should not work.");

    static_assert(!std::is_default_constructible_v<Value<CloneableInt>>, "Value<CloneableInt>() should not work.");
...
GTEST_TEST(ValueTest, NiceTypeName) {
    ...
    auto base_value = std::make_unique<Value<BasicVector<double>>>(MyVector2d::Make(1., 2.));
    ...
    EXPECT_EQ(base_value->GetNiceTypeName(), "drake::systems::MyVector<double,2>");
}
...
GTEST_TEST(ValueTest, MaybeGetValue) {
    ...
    const std::string* const string_pointer = string_value->maybe_get_value<std::string>();
...
GTEST_TEST(ValueTest, MaybeGetMutableValue) {
    ...
    double* const double_pointer = double_value->maybe_get_mutable_value<double>();
    std::string* const string_pointer = string_value->maybe_get_mutable_value<std::string>();
...
GTEST_TEST(ValueTest, SubclassOfValueSurvivesClone) {
    ...
    std::unique_ptr<AbstractValue> cloned = erased.Clone();
    PrintInterface* printable_erased = dynamic_cast<PrintInterface*>(cloned.get());
    ASSERT_NE(nullptr, printable_erased);
...
void CheckHash(const std::string& name) {
    ...
    EXPECT_EQ(internal::TypeHash<T>::value, size_t(hasher)) << "  for name\n"
                                                            << "    Which is: " << name << "\n"
...
GTEST_TEST(TypeHashTest, WellKnownValues) {
    ...
    CheckHash<std::shared_ptr<double>>(fmt::format("{std}::shared_ptr<double>", fmt::arg("std", stdcc)));
    ...
    CheckHash<std::vector<double>>(
            fmt::format("{std}::vector<double,{std}::allocator<double>>", fmt::arg("std", stdcc)));
    ...
    CheckHash<std::shared_ptr<const double>>(fmt::format("{std}::shared_ptr<const double>", fmt::arg("std", stdcc)));

    // Eigen classes.
    CheckHash<Eigen::VectorXd>("Eigen::Matrix<double,int=-1,int=1,int=0,int=-1,int=1>");
    CheckHash<Eigen::MatrixXd>("Eigen::Matrix<double,int=-1,int=-1,int=0,int=-1,int=-1>");
    CheckHash<Eigen::Vector3d>("Eigen::Matrix<double,int=3,int=1,int=0,int=3,int=1>");
    CheckHash<Eigen::Matrix3d>("Eigen::Matrix<double,int=3,int=3,int=0,int=3,int=3>");

    // Vectors of Eigens.
    CheckHash<std::vector<Eigen::VectorXd>>(
            fmt::format("{std}::vector<{eigen},{std}::allocator<{eigen}>>", fmt::arg("std", stdcc),
                        fmt::arg("eigen", "Eigen::Matrix<double,int=-1,int=1,int=0,int=-1,int=1>")));

    // Everything together at once works.
    using BigType = std::vector<std::pair<const double, std::shared_ptr<Eigen::Matrix3d>>>;
    CheckHash<BigType>(fmt::format(
            ...
            "const double,"
            "{std}::shared_ptr<{eigen}>>>>",
            fmt::arg("std", stdcc), fmt::arg("eigen", "Eigen::Matrix<double,int=3,int=3,int=0,int=3,int=3>")));

    // Templated on a value, but with the 'using NonTypeTemplateParameter'
    // decoration so that the hash works.
    const std::string kfoo = kClang || kGcc9 ? "drake::test::{anonymous}::AnonEnum::kFoo" : "0";
...
GTEST_TEST(ValueTest, TypesInBadCastMessage) {
...

    {
        ...
        // above.
        DRAKE_EXPECT_THROWS_MESSAGE(abstract.get_value<double>(), ".*request.*double.*static.*int'\\.$");
    }
}
