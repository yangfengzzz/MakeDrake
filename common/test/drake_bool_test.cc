...
TEST_F(BoolTestDouble, TypeCheck) {
    static_assert(std::is_same_v<boolean<double>, bool>, "boolean<double> should be bool");
    static_assert(std::is_same_v<scalar_predicate<double>::type, bool>,
                  "scalar_predicate<double>::type should be bool");
    static_assert(scalar_predicate<double>::is_bool, "scalar_predicate<double>::is_bool should be true");
}
...
TEST_F(BoolTestAutoDiffXd, TypeCheck) {
    static_assert(std::is_same_v<boolean<AutoDiffXd>, bool>, "boolean<AutoDiffXd> should be bool");
    static_assert(std::is_same_v<scalar_predicate<AutoDiffXd>::type, bool>,
                  "scalar_predicate<AutoDiffXd>::type should be bool");
    static_assert(scalar_predicate<AutoDiffXd>::is_bool, "scalar_predicate<AutoDiffXd>::is_bool should be true");
}

TEST_F(BoolTestAutoDiffXd, AllOf) {
    EXPECT_TRUE(all_of(m_, [](const AutoDiffXd& v) { return v.derivatives()[0] == 1.0 || v.derivatives()[1] == 1.0; }));
...
TEST_F(BoolTestAutoDiffXd, AnyOf) {
    EXPECT_TRUE(any_of(m_, [](const AutoDiffXd& v) { return v.derivatives()[0] == 1.0 && v.derivatives()[1] == 0.0; }));
    EXPECT_FALSE(
            any_of(m_, [](const AutoDiffXd& v) { return v.derivatives()[0] == 1.0 && v.derivatives()[1] == 1.0; }));
...
TEST_F(BoolTestAutoDiffXd, NoneOf) {
    EXPECT_TRUE(
            none_of(m_, [](const AutoDiffXd& v) { return v.derivatives()[0] == 1.0 && v.derivatives()[1] == 1.0; }));
    EXPECT_FALSE(
            none_of(m_, [](const AutoDiffXd& v) { return v.derivatives()[0] == 1.0 && v.derivatives()[1] == 0.0; }));
...
TEST_F(BoolTestSymbolic, TypeCheck) {
    static_assert(std::is_same_v<boolean<Expression>, Formula>, "boolean<Expression> should be Formula");
    static_assert(std::is_same_v<scalar_predicate<Expression>::type, Formula>,
                  "scalar_predicate<Expression>::type should be Formula");
    static_assert(!scalar_predicate<Expression>::is_bool, "scalar_predicate<Expression>::is_bool should be false");
}

TEST_F(BoolTestSymbolic, All) {
    const MatrixX<Formula> formulae = m_.unaryExpr(&symbolic::isnan);
    EXPECT_PRED2(FormulaEqual, all(formulae), isnan(x_) && isnan(y_) && isnan(z_) && isnan(w_));
...
TEST_F(BoolTestSymbolic, AllOf) {
    EXPECT_PRED2(FormulaEqual, all_of(m_, [](const Expression& v) { return !isnan(v); }),
...
TEST_F(BoolTestSymbolic, Any) {
    const MatrixX<Formula> formulae = m_.unaryExpr(&symbolic::isnan);
    EXPECT_PRED2(FormulaEqual, any(formulae), isnan(x_) || isnan(y_) || isnan(z_) || isnan(w_));
...
TEST_F(BoolTestSymbolic, AnyOf) {
    EXPECT_PRED2(FormulaEqual, any_of(m_, [](const Expression& v) { return isnan(v); }),
                 isnan(x_) || isnan(y_) || isnan(z_) || isnan(w_));
...
TEST_F(BoolTestSymbolic, None) {
    const MatrixX<Formula> formulae = m_.unaryExpr(&symbolic::isnan);
    EXPECT_PRED2(FormulaEqual, none(formulae), !isnan(x_) && !isnan(y_) && !isnan(z_) && !isnan(w_));
...
TEST_F(BoolTestSymbolic, NoneOf) {
    EXPECT_PRED2(FormulaEqual, none_of(m_, [](const Expression& v) { return isnan(v); }),
                 !isnan(x_) && !isnan(y_) && !isnan(z_) && !isnan(w_));
