...
GTEST_TEST(FindResourceTest, NonRelativeRequest) {
    ...
    EXPECT_EQ(*error_message, "Drake resource_path '/dev/null' is not a relative path.");
}
...
GTEST_TEST(FindResourceTest, NotFound) {
    ...
    EXPECT_THAT(*error_message,
                testing::ContainsRegex("Sought '" + relpath + "' in runfiles.*not exist.*on the manifest"));
...
GTEST_TEST(GetDrakePathTest, PathIncludesDrake) {
    ...
    const std::filesystem::path expected =
            std::filesystem::path(*result) / std::filesystem::path("common/test/find_resource_test_data.txt");
    EXPECT_TRUE(std::filesystem::exists(expected));
}

GTEST_TEST(ReadFileTest, NoSuchPath) {
    EXPECT_FALSE(ReadFile("/foo/bar/missing"));
    DRAKE_EXPECT_THROWS_MESSAGE(ReadFileOrThrow("/foo/bar/missing"), "Error reading.*/foo/bar/missing.*");
}
