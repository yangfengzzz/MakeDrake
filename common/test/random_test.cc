...
void CheckCalcProbabilityDensityUniform() {
    ...
    for (const auto& x : {Vector2<T>(0.1, 0.5), Vector2<T>(0., 0.5), Vector2<T>(0.1, 1.)}) {
    ...
    for (const auto& x_zero_prob : {Vector2<T>(-0.1, 0.5), Vector2<T>(0.2, 1.5), Vector2<T>(-0.1, 1.2)}) {
        T density = CalcProbabilityDensity<T>(RandomDistribution::kUniform, x_zero_prob);
...
void CheckCalcProbabilityDensityGaussian() {
    for (const auto& x : {Vector3<T>(0.5, 1.2, 3.5), Vector3<T>(-0.2, -1., 2.1)}) {
        ...
        const T density = CalcProbabilityDensity<T>(RandomDistribution::kGaussian, x);
...
void CheckCalcProbabilityDensityExponential() {
    ...
    for (const auto& x : {Vector3<T>(0.1, 0.5, 0.), Vector3<T>(0.1, 0.9, 1.5)}) {
        ...
        EXPECT_NEAR(ExtractDoubleOrThrow(CalcProbabilityDensity<T>(RandomDistribution::kExponential, x)),
                    ExtractDoubleOrThrow(density_expected), 10 * kEps);
    }

    // Check samples with zero probability.
    for (const Vector3<T>& x_zero_prob : {Vector3<T>(-0.5, 1., 2.), Vector3<T>(-0.01, 0.01, 1)}) {
        EXPECT_EQ(ExtractDoubleOrThrow(CalcProbabilityDensity<T>(RandomDistribution::kExponential, x_zero_prob)), 0.);
    }
}
