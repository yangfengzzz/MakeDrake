...
#include "common/proto/rpc_pipe_temp_directory.h"
...
PythonRemoteVariable::PythonRemoteVariable() : unique_id_(py_globally_unique_id++) {}

namespace internal {

// Copies `size` bytes at `data` into `message->data`.
void CopyBytes(const void* bytes, const int size, lcmt_call_python_data* message) {
    message->num_bytes = size;
    message->data.resize(size);
    std::memcpy(message->data.data(), bytes, size);
}

void ToPythonRemoteData(const PythonRemoteVariable& variable, lcmt_call_python_data* message) {
...
void ToPythonRemoteData(const std::string& str, lcmt_call_python_data* message) {
...
void ToPythonRemoteDataMatrix(const Eigen::Ref<const MatrixX<bool>>& mat,
    ...
    message->shape_type = is_vector ? lcmt_call_python_data::VECTOR : lcmt_call_python_data::MATRIX;
...
std::ofstream* InitOutput(const std::optional<std::string>& filename) {
    ...
    if (!raw_output.access()) {
        ...
        const std::string filename_default = GetRpcPipeTempDirectory() + "/python_rpc";
        ...
        // `CallPythonInit`.
        if (filename) {
            throw std::runtime_error("`CallPython` or `CallPythonInit` has already been called");
        }
    }
