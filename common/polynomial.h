...
public:
    ...
    explicit Polynomial(const Eigen::MatrixBase<Derived>& coefficients) {
        DRAKE_THROW_UNLESS((coefficients.cols() == 1) || (coefficients.rows() == 1) || (coefficients.size() == 0));
    ...
    typename Product<T, U>::type EvaluateUnivariate(const U& x, int derivative_order = 0) const {
        ...
        ProductType value = 0;
        using std::pow;
        for (typename std::vector<Monomial>::const_iterator iter = monomials_.begin(); iter != monomials_.end();
             iter++) {
    ...
    friend std::ostream& operator<<(std::ostream& os, const Polynomial& poly) {
        ...
        for (typename std::vector<Monomial>::const_iterator iter = poly.monomials_.begin();
...
template <>
struct formatter<drake::Polynomial<double>::Monomial> : drake::ostream_formatter {};
}  // namespace fmt
