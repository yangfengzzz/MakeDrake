...
constexpr bool hash_template_argument_from_pretty_func(
        const char* pretty, int which_argument, bool discard_nested, bool discard_cast, FNV1aHasher* result) {
...
template <typename T>
struct TypeHasherHasNonTypeTemplateParameter<T, std::void_t<typename T::NonTypeTemplateParameter>> {
...
struct TypeHasher {
    ...
    static constexpr bool calc(FNV1aHasher* result) {
        ...
        return hash_template_argument_from_pretty_func(__PRETTY_FUNCTION__, which_argument, discard_nested,
                                                       discard_cast, result);
    }
};
...
struct TypeHasher<T, true> {
    static constexpr bool calc(FNV1aHasher* result) {
        ...
        const bool discard_nested = true;
        const bool discard_cast = false;
        hash_template_argument_from_pretty_func(__PRETTY_FUNCTION__, which_argument, discard_nested, discard_cast,
                                                result);
        // Then, hash the "<U=u>".
...
struct ValueTraitsImpl<T, false> {
    static_assert(drake::is_cloneable<T>::value, "Types placed into a Value<T> must either be copyable or cloneable");
