...
namespace internal {
[[noreturn]] void ThrowTypeSafeIndexAssertValidFailed(const std::type_info& type, const char* source);
[[noreturn]] void ThrowTypeSafeIndexAssertNoOverflowFailed(const std::type_info& type, const char* source);
}  // namespace internal
...
public:
    ...
    explicit TypeSafeIndex(int64_t index) : index_(static_cast<int>(index)) {
        ...
        DRAKE_ASSERT_VOID(AssertValid(index, "Explicitly constructing an invalid index."));
    }
    ...
    const TypeSafeIndex& operator++() {
        DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-incrementing an invalid index."));
        DRAKE_ASSERT_VOID(AssertNoOverflow(1, "Pre-incrementing produced an invalid index."));
        ++index_;
        return *this;
    }
    ...
    TypeSafeIndex operator++(int) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "Post-incrementing an invalid index."));
        DRAKE_ASSERT_VOID(AssertNoOverflow(1, "Post-incrementing produced an invalid index."));
        ++index_;
    ...
    /// non-negative.
    const TypeSafeIndex& operator--() {
        DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-decrementing an invalid index."));
        --index_;
        DRAKE_ASSERT_VOID(AssertValid(index_, "Pre-decrementing produced an invalid index."));
        return *this;
    }
    ...
    /// non-negative.
    TypeSafeIndex operator--(int) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "Post-decrementing an invalid index."));
        --index_;
        DRAKE_ASSERT_VOID(AssertValid(index_, "Post-decrementing produced an invalid index."));
        return TypeSafeIndex(index_ + 1);
    }
    ///@}
    ...
    TypeSafeIndex& operator+=(int i) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with an int on an invalid index."));
        DRAKE_ASSERT_VOID(AssertNoOverflow(i, "In-place addition with an int produced an invalid index."));
        index_ += i;
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with an int produced an invalid index."));
        return *this;
    }

    /// Allow addition for indices with the same tag.
    TypeSafeIndex<Tag>& operator+=(const TypeSafeIndex<Tag>& other) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with another index invalid LHS."));
        DRAKE_ASSERT_VOID(AssertValid(other.index_, "In-place addition with another index invalid RHS."));
        DRAKE_ASSERT_VOID(
                AssertNoOverflow(other.index_, "In-place addition with another index produced an invalid index."));
        index_ += other.index_;
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place addition with another index produced an invalid index."));
        return *this;
    }
    ...
    /// non-negative.
    TypeSafeIndex& operator-=(int i) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with an int on an invalid index."));
        DRAKE_ASSERT_VOID(AssertNoOverflow(-i, "In-place subtraction with an int produced an invalid index."));
        index_ -= i;
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with an int produced an invalid index."));
        return *this;
    }

    /// Allow subtraction for indices with the same tag.
    TypeSafeIndex<Tag>& operator-=(const TypeSafeIndex<Tag>& other) {
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with another index invalid LHS."));
        DRAKE_ASSERT_VOID(AssertValid(other.index_, "In-place subtraction with another index invalid RHS."));
        ...
        index_ -= other.index_;
        DRAKE_ASSERT_VOID(AssertValid(index_, "In-place subtraction with another index produced an invalid index."));
        return *this;
    }
...
private:
    ...
    static void AssertValid(int64_t index, const char* source) {
        if (index < 0 || index > kMaxIndex) {
            internal::ThrowTypeSafeIndexAssertValidFailed(typeid(TypeSafeIndex), source);
        }
    }
    ...
    void AssertNoOverflow(int delta, const char* source) const {
        if (delta > 0 && index_ > kMaxIndex - delta) {
            internal::ThrowTypeSafeIndexAssertNoOverflowFailed(typeid(TypeSafeIndex), source);
        }
    }
