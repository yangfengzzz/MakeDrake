...
#include "common/trajectories/test/random_piecewise_polynomial.h"
...
void testIntegralAndDerivative() {
    ...
    vector<double> segment_times = PiecewiseTrajectory<double>::RandomSegmentTimes(num_segments, generator);
    PiecewisePolynomial<T> piecewise =
            test::MakeRandomPiecewisePolynomial<T>(rows, cols, num_coefficients, segment_times);

    // derivative(0) should be same as original piecewise.
    EXPECT_TRUE(CompareMatrices(piecewise.value(piecewise.start_time()),
                                piecewise.derivative(0).value(piecewise.start_time()), 1e-10,
    ...
    MatrixX<T> desired_value_at_t0 = MatrixX<T>::Random(piecewise.rows(), piecewise.cols());
    ...
    for (int i = 0; i < piecewise.get_number_of_segments() - 1; ++i) {
        EXPECT_EQ(integral.getPolynomial(i).EvaluateUnivariate(integral.duration(i)),
...
void testBasicFunctionality() {
    ...
    for (int i = 0; i < num_tests; ++i) {
        ...
        vector<double> segment_times = PiecewiseTrajectory<double>::RandomSegmentTimes(num_segments, generator);
        PiecewisePolynomial<T> piecewise1 =
                test::MakeRandomPiecewisePolynomial<T>(rows, cols, num_coefficients, segment_times);
        PiecewisePolynomial<T> piecewise2 =
                test::MakeRandomPiecewisePolynomial<T>(rows, cols, num_coefficients, segment_times);
        PiecewisePolynomial<T> piecewise3_not_matching_rows =
                test::MakeRandomPiecewisePolynomial<T>(rows + 1, cols, num_coefficients, segment_times);
        PiecewisePolynomial<T> piecewise4_not_matching_cols =
                test::MakeRandomPiecewisePolynomial<T>(rows, cols + 1, num_coefficients, segment_times);
        PiecewisePolynomial<T> piecewise5 =
                test::MakeRandomPiecewisePolynomial<T>(cols, rows, num_coefficients, segment_times);

        normal_distribution<double> normal;
        double shift = normal(generator);
        MatrixX<T> offset = MatrixX<T>::Random(piecewise1.rows(), piecewise1.cols());
        ...
        EXPECT_THROW(piecewise2_twice.ConcatenateInTime(piecewise2_shifted_twice), std::runtime_error);

        // Checks that concatenation of trajectories that have different
        // row counts is a failure.
        PiecewisePolynomial<T> piecewise3_not_matching_rows_shifted = piecewise3_not_matching_rows;
        piecewise3_not_matching_rows_shifted.shiftRight(total_time);
        EXPECT_THROW(piecewise2_twice.ConcatenateInTime(piecewise3_not_matching_rows_shifted), std::runtime_error);

        // Checks that concatenation of trajectories that have different
        // col counts is a failure.
        PiecewisePolynomial<T> piecewise4_not_matching_cols_shifted = piecewise4_not_matching_cols;
        piecewise4_not_matching_cols_shifted.shiftRight(total_time);
        EXPECT_THROW(piecewise2_twice.ConcatenateInTime(piecewise4_not_matching_cols_shifted), std::runtime_error);

        piecewise2_twice.ConcatenateInTime(piecewise2_shifted);

        uniform_real_distribution<double> uniform(piecewise1.start_time(), piecewise1.end_time());
        double t = uniform(generator);

        EXPECT_TRUE(CompareMatrices(sum.value(t), piecewise1.value(t) + piecewise2.value(t), 1e-8,
                                    MatrixCompareType::absolute));

        EXPECT_TRUE(CompareMatrices(difference.value(t), piecewise2.value(t) - piecewise1.value(t), 1e-8,
                                    MatrixCompareType::absolute));

        EXPECT_TRUE(CompareMatrices(piecewise1_plus_offset.value(t), piecewise1.value(t) + offset, 1e-8,
                                    MatrixCompareType::absolute));

        EXPECT_TRUE(CompareMatrices(piecewise1_minus_offset.value(t), piecewise1.value(t) - offset, 1e-8,
                                    MatrixCompareType::absolute));

        EXPECT_TRUE(CompareMatrices(piecewise1_shifted.value(t), piecewise1.value(t - shift), 1e-8,
                                    MatrixCompareType::absolute));

        EXPECT_TRUE(CompareMatrices(product.value(t), piecewise1.value(t) * piecewise5.value(t), 1e-8,
                                    MatrixCompareType::absolute));
        ...
        EXPECT_TRUE(CompareMatrices(piecewise2_twice.value(t), piecewise2_twice.value(t + total_time), 1e-8,
                                    MatrixCompareType::absolute));
    }
}
...
void testValueOutsideOfRange() {
    default_random_engine generator;
    vector<double> segment_times = PiecewiseTrajectory<double>::RandomSegmentTimes(6, generator);
    PiecewisePolynomial<T> piecewise = test::MakeRandomPiecewisePolynomial<T>(3, 4, 5, segment_times);

    EXPECT_TRUE(CompareMatrices(piecewise.value(piecewise.start_time()), piecewise.value(piecewise.start_time() - 1.0),
                                1e-10, MatrixCompareType::absolute));

    EXPECT_TRUE(CompareMatrices(piecewise.value(piecewise.end_time()), piecewise.value(piecewise.end_time() + 1.0),
                                1e-10, MatrixCompareType::absolute));
}
...
GTEST_TEST(testPiecewisePolynomial, CubicSplinePeriodicBoundaryConditionTest) {
  ...
  samples << 1, 1, 1,
    ...
    PiecewisePolynomial<double> periodic_spline =
            PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples, periodic_endpoint);

    std::unique_ptr<Trajectory<double>> spline_dt = periodic_spline.MakeDerivative(1);
    std::unique_ptr<Trajectory<double>> spline_ddt = periodic_spline.MakeDerivative(2);
    ...
    EXPECT_TRUE(CompareMatrices(periodic_spline.EvalDerivative(t, 1), spline_dt->value(t), 1e-14));
    EXPECT_TRUE(CompareMatrices(periodic_spline.EvalDerivative(t, 2), spline_ddt->value(t), 1e-14));
}
...
GTEST_TEST(testPiecewisePolynomial, ExceptionsTest) {
  ...
  samples << 1, 1, 1,
    ...
    PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples, true);
...
GTEST_TEST(testPiecewisePolynomial, VectorValueTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(mat.vector_values(times), "This method only supports vector-valued trajectories.");
}
...
GTEST_TEST(testPiecewisePolynomial, RemoveFinalSegmentTest) {
  ...
  samples << 1, 1, 2,
    ...
    PiecewisePolynomial<double> pp = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples);
...
std::unique_ptr<Trajectory<double>> TestReverseTime(const PiecewisePolynomial<double>& pp_orig) {
    std::unique_ptr<Trajectory<double>> pp_ptr = pp_orig.Clone();
    PiecewisePolynomial<double>* pp = dynamic_cast<PiecewisePolynomial<double>*>(pp_ptr.get());
...
void TestScaling(const PiecewisePolynomial<double>& pp_orig, const double scale) {
    std::unique_ptr<Trajectory<double>> pp_ptr = pp_orig.Clone();
    PiecewisePolynomial<double>* pp = dynamic_cast<PiecewisePolynomial<double>*>(pp_ptr.get());
    ...
    for (const double trel : {0.1, .2, .52, .77}) {
        const double t = pp_orig.start_time() + trel * (pp_orig.end_time() - pp_orig.start_time());
...
GTEST_TEST(testPiecewisePolynomial, ReverseAndScaleTimeTest) {
  ...
  samples << 1, 1, 2,
    ...
    const PiecewisePolynomial<double> zoh = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
    ...
    EXPECT_FALSE(CompareMatrices(reversed_zoh->value(-breaks(1)), zoh.value(breaks(1))));
    EXPECT_TRUE(CompareMatrices(reversed_zoh->value(-breaks(1)), zoh.value(breaks(1) - 1e-14)));
    TestScaling(zoh, 2.3);

    const PiecewisePolynomial<double> foh = PiecewisePolynomial<double>::FirstOrderHold(breaks, samples);
    TestReverseTime(foh);
    TestScaling(foh, 1.2);
    TestScaling(foh, 3.6);

    const PiecewisePolynomial<double> spline =
            PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples);
...
GTEST_TEST(testPiecewisePolynomial, ReshapeBlockAndTranspose) {
    ...
    PiecewisePolynomial<double> zoh = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
...
GTEST_TEST(testPiecewisePolynomial, IsApproxTest) {
    ...
    const PiecewisePolynomial<double> pp1 = PiecewisePolynomial<double>::FirstOrderHold(breaks, samples);
...
void TestScalarType() {
    ...
    const PiecewisePolynomial<T> spline = PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(breaks, samples);

    const MatrixX<T> value = spline.value(0.5);
    EXPECT_NEAR(ExtractDoubleOrThrow(value(0)), ExtractDoubleOrThrow(samples(0, 1)), 1e-14);
    EXPECT_NEAR(ExtractDoubleOrThrow(value(1)), ExtractDoubleOrThrow(samples(1, 1)), 1e-14);
}
...
GTEST_TEST(PiecewiseTrajectoryTest, SymbolicValues) {
    ...
    const PiecewisePolynomial<Expression> foh = PiecewisePolynomial<Expression>::FirstOrderHold(breaks, samples);
    ...
    const Vector3<Expression> symbolic_breaks(Variable("t0"), Variable("t1"), Variable("t2"));
    EXPECT_THROW(PiecewisePolynomial<Expression>::FirstOrderHold(symbolic_breaks, samples), std::runtime_error);
    ...
    EXPECT_TRUE(foh_w_symbolic_coeffs.value(0.25)(0).Expand().EqualTo(0.5 * x0 + 0.5 * x1));
}
...
GTEST_TEST(PiecewiseTrajectoryTest, AutoDiffDerivativesTest) {
    ...
    const PiecewisePolynomial<AutoDiffXd> trajectory =
            PiecewisePolynomial<AutoDiffXd>::CubicWithContinuousSecondDerivatives(breaks, samples);
    std::unique_ptr<Trajectory<AutoDiffXd>> derivative_trajectory = trajectory.MakeDerivative();
    const int num_times = 100;
    VectorX<double> t = VectorX<double>::LinSpaced(num_times, ExtractDoubleOrThrow(trajectory.start_time()),
    ...
    for (int k = 0; k < num_times; ++k) {
        ...
        MatrixX<double> expected_derivative_value = DiscardGradient(derivative_trajectory->value(t(k)));
        EXPECT_TRUE(CompareMatrices(derivative_value, expected_derivative_value, tolerance));
    }
}
...
GTEST_TEST(PiecesewisePolynomialTest, YamlIoComplicated) {
  ...
  samples << 1, 1, 1,
            ...
            -2, 2, 2,
             1, 1, 1;
    // clang-format on
    const auto dut = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples);
