...
public:
    explicit TrajectoryTester(bool has_derivative) : has_derivative_(has_derivative) {}
...
GTEST_TEST(TrajectoryTest, EvalDerivativesTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(traj_yes_deriv.EvalDerivative(traj_yes_deriv.start_time()), ".* must implement .*");
    DRAKE_EXPECT_THROWS_MESSAGE(traj_yes_deriv.EvalDerivative(traj_yes_deriv.start_time(), -1),
                                ".*derivative_order >= 0.*");

    TrajectoryTester traj_no_deriv(false);
    EXPECT_FALSE(traj_no_deriv.has_derivative());
    DRAKE_EXPECT_THROWS_MESSAGE(traj_no_deriv.EvalDerivative(traj_no_deriv.start_time()), ".* does not support .*");
}

GTEST_TEST(TrajectoryTest, MakeDerivativesTest) {
    TrajectoryTester traj_yes_deriv(true);
    EXPECT_TRUE(traj_yes_deriv.has_derivative());
    DRAKE_EXPECT_THROWS_MESSAGE(traj_yes_deriv.MakeDerivative(), ".* must implement .*");

    TrajectoryTester traj_no_deriv(false);
    EXPECT_FALSE(traj_no_deriv.has_derivative());
    DRAKE_EXPECT_THROWS_MESSAGE(traj_no_deriv.MakeDerivative(), ".* does not support .*");
}
