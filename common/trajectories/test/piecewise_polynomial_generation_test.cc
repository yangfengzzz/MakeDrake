...
bool CheckInterpolatedValuesAtBreakTime(const PiecewisePolynomial<T>& traj,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (!CompareMatrices(traj.value(breaks[i]), values[i], tol, MatrixCompareType::absolute)) {
            return false;
        }
    }
    return true;
...
GTEST_TEST(SplineTests, PchipAndCubicSplineCompareWithMatlabTest) {
    ...
    PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::CubicShapePreserving(T, Y);
    ...
    for (int t = 0; t < spline.get_number_of_segments(); ++t) {
        ...
        EXPECT_TRUE(
                CompareMatrices(poly_matrix(0, 0).GetCoefficients(), coeffs[t], 1e-12, MatrixCompareType::absolute));
    }
    ...
    spline = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y);
    EXPECT_EQ(spline.get_number_of_segments(), static_cast<int>(T.size()) - 1);
    for (int t = 0; t < spline.get_number_of_segments(); ++t) {
        const PiecewisePolynomial<double>::PolynomialMatrix& poly_matrix = spline.getPolynomialMatrix(t);
        EXPECT_TRUE(
    ...
    EXPECT_TRUE(CompareMatrices(spline.getPolynomialMatrix(0)(0, 0).GetCoefficients(), Vector4<double>(1, 3, 0, -1),
                                1e-12, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(spline.getPolynomialMatrix(1)(0, 0).GetCoefficients(), Vector4<double>(3, 0, 0, 0),
                                1e-12, MatrixCompareType::absolute));
}
...
GTEST_TEST(SplineTests, RandomizedLinearSplineTest) {
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        std::vector<double> T = PiecewiseTrajectory<double>::RandomSegmentTimes(N - 1, generator);
        std::vector<MatrixX<double>> Y(N);
        for (int i = 0; i < N; ++i) Y[i] = MatrixX<double>::Random(rows, cols);

        PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::FirstOrderHold(T, Y);
...
GTEST_TEST(SplineTests, RandomizedConstantSplineTest) {
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        ...
        PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::ZeroOrderHold(T, Y);
...
GTEST_TEST(SplineTests, RandomizedPchipSplineTest) {
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        ...
        spline = PiecewisePolynomial<double>::CubicShapePreserving(T, Y, true /* Uses zero end point derivative. */);
        PchipTest(T, Y, spline, 1e-8);
...
GTEST_TEST(SplineTests, PchipLength2Test) {
    ...
    PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::CubicShapePreserving(T, Y, true);
    PiecewisePolynomial<double> spline_dot = spline.derivative();
    ...
    double v_min = ComputeExtremeVel(spline_dot.getPolynomial(0), T.back(), false);
    EXPECT_GE(v_min, 0);
...
GTEST_TEST(SplineTests, RandomizedCubicSplineTest) {
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        ...
        PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y);
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        ...
        PiecewisePolynomial<double> spline =
                PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y, Ydot0, Ydot1);
        EXPECT_TRUE(CheckContinuity(spline, 1e-8, 2));
        EXPECT_TRUE(CheckValues(spline, {Y}, 1e-8));
        EXPECT_TRUE(CheckInterpolatedValuesAtBreakTime(spline, T, Y, 1e-8));
    ...
    for (int ctr = 0; ctr < num_tests; ++ctr) {
        ...
        const PiecewisePolynomial<double> spline = PiecewisePolynomial<double>::CubicHermite(T, Y, Ydot);
        ...
        // incrementally.
        PiecewisePolynomial<double> incremental =
                PiecewisePolynomial<double>::CubicHermite({T[0], T[1]}, {Y[0], Y[1]}, {Ydot[0], Ydot[1]});
        for (int i = 2; i < N; i++) {
...
GTEST_TEST(SplineTests, CubicSplineSize2) {
    ...
    EXPECT_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y), std::runtime_error);
}
...
GTEST_TEST(SplineTests, EigenTest) {
    ...
    std::vector<MatrixXd> samples_vec = {Vector3d{1., 0., 0.}, Vector3d{0., 1., 0.}, Vector3d{0., 0., 1.}};

    // Keep the code cleaner below.
    using PP = PiecewisePolynomial<double>;

    EXPECT_TRUE(PP::ZeroOrderHold(breaks_mat, samples_mat).isApprox(PP::ZeroOrderHold(breaks_vec, samples_vec), tol));

    EXPECT_TRUE(PP::FirstOrderHold(breaks_mat, samples_mat).isApprox(PP::FirstOrderHold(breaks_vec, samples_vec), tol));

    EXPECT_TRUE(PP::CubicShapePreserving(breaks_mat, samples_mat, false)
                        .isApprox(PP::CubicShapePreserving(breaks_vec, samples_vec, false), tol));

    EXPECT_TRUE(PP::CubicShapePreserving(breaks_mat, samples_mat, true)
                        .isApprox(PP::CubicShapePreserving(breaks_vec, samples_vec, true), tol));

    EXPECT_TRUE(PP::CubicWithContinuousSecondDerivatives(breaks_mat, samples_mat)
                        .isApprox(PP::CubicWithContinuousSecondDerivatives(breaks_vec, samples_vec), tol));

    Matrix3d samples_dot_mat = 2. * Matrix3d::Identity();
    std::vector<MatrixXd> samples_dot_vec = {Vector3d{2., 0., 0.}, Vector3d{0., 2., 0.}, Vector3d{0., 0., 2.}};

    EXPECT_TRUE(PP::CubicHermite(breaks_mat, samples_mat, samples_dot_mat)
                        .isApprox(PP::CubicHermite(breaks_vec, samples_vec, samples_dot_vec), tol));

    EXPECT_TRUE(
            PP::CubicWithContinuousSecondDerivatives(breaks_mat, samples_mat, samples_dot_vec[0], samples_dot_vec[2])
                    .isApprox(PP::CubicWithContinuousSecondDerivatives(breaks_vec, samples_vec, samples_dot_vec[0],
                                                                       samples_dot_vec[2]),
                              tol));
}

template <typename T>
void TestThrows(const std::vector<double>& breaks, const std::vector<MatrixX<T>>& samples) {
    EXPECT_THROW(PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples), std::runtime_error);
    EXPECT_THROW(PiecewisePolynomial<double>::FirstOrderHold(breaks, samples), std::runtime_error);
    EXPECT_THROW(PiecewisePolynomial<double>::CubicShapePreserving(breaks, samples), std::runtime_error);
    EXPECT_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples),
                 std::runtime_error);
}

template <typename T>
void TestNoThrows(const std::vector<double>& breaks, const std::vector<MatrixX<T>>& samples) {
    DRAKE_EXPECT_NO_THROW(PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples));
    DRAKE_EXPECT_NO_THROW(PiecewisePolynomial<double>::FirstOrderHold(breaks, samples));
    DRAKE_EXPECT_NO_THROW(PiecewisePolynomial<double>::CubicShapePreserving(breaks, samples));
    DRAKE_EXPECT_NO_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(breaks, samples));
}
...
GTEST_TEST(SplineTests, TestException) {
    ...
    EXPECT_THROW(PiecewisePolynomial<double>::CubicShapePreserving(T, Y), std::runtime_error);
    EXPECT_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y), std::runtime_error);
    ...
    Ydot0.setZero();
    Ydot1.setZero();
    DRAKE_EXPECT_NO_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y, Ydot0, Ydot1));

    Ydot0.resize(rows, cols);
    Ydot1.resize(rows, cols + 1);
    EXPECT_THROW(PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(T, Y, Ydot0, Ydot1),
                 std::runtime_error);
    ...
    EXPECT_THROW(PiecewisePolynomial<double>::CubicHermite(T, Y, Ydot), std::runtime_error);
...
GTEST_TEST(SplineTests, LagrangeInterpolatingPolynomialTest) {
    ...
    PiecewisePolynomial<double> scalar_trajectory =
            PiecewisePolynomial<double>::LagrangeInterpolatingPolynomial(times, samples);
    ...
    EXPECT_EQ(scalar_trajectory.getPolynomial(0, 0, 0).GetDegree(), times.size() - 1);
    EXPECT_TRUE(
            CompareMatrices(scalar_trajectory.vector_values({times[0], times[1], times[2], times[3]}), samples, 1e-12));
    ...
    PiecewisePolynomial<double> matrix_trajectory =
            PiecewisePolynomial<double>::LagrangeInterpolatingPolynomial(mtimes, msamples);
    ...
    for (size_t i = 0; i < mtimes.size(); ++i) {
        EXPECT_TRUE(CompareMatrices(matrix_trajectory.value(mtimes[i]), msamples[i], 1e-12));
    }
}
