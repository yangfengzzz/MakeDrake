#include "common/trajectories/path_parameterized_trajectory.h"
...
protected:
    void SetUp() override {
        ...
        time_scaling_ = PiecewisePolynomial<double>::CubicShapePreserving(times, s_samples, true);
        path_ = PiecewisePolynomial<double>::CubicShapePreserving(Vector2<double>(s_samples(0), s_samples(2)),
                                                                  x_samples, true);
        dut_ = std::make_unique<PathParameterizedTrajectory<double>>(path_, time_scaling_);
    }
...
TEST_F(PathParameterizedTrajectoryTest, TestValue) {
    for (double t = 0.88; t < 2.2; t += 0.12) {
        const double time = std::clamp(t, time_scaling_.start_time(), time_scaling_.end_time());
        EXPECT_TRUE(CompareMatrices(dut_->value(t), path_.value(time_scaling_.scalarValue(time)), 1e-14));
    }
}
...
TEST_F(PathParameterizedTrajectoryTest, TestDerivatives) {
    for (double t = 0.88; t < 2.2; t += 0.12) {
        ...
        EXPECT_TRUE(CompareMatrices(dut_->EvalDerivative(t, 0), dut_->value(t), 1e-14));
        ...
        VectorX<double> x_ddot = path_.EvalDerivative(s, 1) * s_ddot + path_.EvalDerivative(s, 2) * s_dot * s_dot;
        ...
        VectorX<double> x_3dot = path_.EvalDerivative(s, 1) * s_3dot + 3 * path_.EvalDerivative(s, 2) * s_ddot * s_dot +
...
TEST_F(PathParameterizedTrajectoryTest, TestGetTrajectory) {
    EXPECT_TRUE(dynamic_cast<const PiecewisePolynomial<double>&>(dut_->path()).isApprox(path_, 0));
    EXPECT_TRUE(dynamic_cast<const PiecewisePolynomial<double>&>(dut_->time_scaling()).isApprox(time_scaling_, 0));
}
...
void TestScalarType() {
    ...
    PiecewisePolynomial<T> time_scaling = PiecewisePolynomial<T>::FirstOrderHold(times, s_samples.transpose());
    PiecewisePolynomial<T> path = PiecewisePolynomial<T>::FirstOrderHold(s_samples, x_samples);
