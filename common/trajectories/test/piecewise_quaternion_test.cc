#include "common/trajectories/piecewise_quaternion.h"
...
Quaternion<double> EulerIntegrateQuaternion(const Quaternion<Scalar>& q0, const Vector3<Scalar>& omega, double dt) {
    ...
    if (delta.norm() < Eigen::NumTraits<Scalar>::epsilon()) {
        ...
        Quaternion<Scalar> q_delta = Quaternion<Scalar>(AngleAxis<Scalar>(delta.norm(), delta.normalized()));
        q1 = q_delta * q0;
    }
    q1.normalize();
    return q1;
...
template <typename Scalar>
bool CheckSlerpInterpolation(const PiecewiseQuaternionSlerp<Scalar>& spline, double t) {
...
template <typename Scalar>
std::vector<Quaternion<Scalar>> GenerateRandomQuaternions(int size, std::default_random_engine* generator) {
    DRAKE_DEMAND(size >= 0);
    ...
    for (int i = 0; i < size; ++i) {
        ret[i] = Quaternion<Scalar>(AngleAxis<Scalar>(
                uniform(*generator), Vector3<Scalar>(uniform(*generator), uniform(*generator), uniform(*generator))));
    }
    return ret;
}
...
GTEST_TEST(TestPiecewiseQuaternionSlerp, TestRandomizedPiecewiseQuaternionSlerp) {
    std::default_random_engine generator(123);
    int N = 10000;
    std::vector<double> time = PiecewiseTrajectory<double>::RandomSegmentTimes(N - 1, generator);
    std::vector<Quaternion<double>> quat = GenerateRandomQuaternions<double>(N, &generator);
    ...
    PiecewiseQuaternionSlerp<double> incremental_rotation_matrices, incremental_angle_axes;
    for (int i = 0; i < N; i++) {
        incremental_rotation_matrices.Append(time[i], math::RotationMatrix<double>(quat[i]));
    ...
    for (double t = time.front(); t < time.back(); t += 0.1) {
        EXPECT_TRUE(
                CompareMatrices(incremental_rotation_matrices.EvalDerivative(t), rot_spline.EvalDerivative(t), 1e-12));
        EXPECT_TRUE(CompareMatrices(incremental_angle_axes.EvalDerivative(t), rot_spline.EvalDerivative(t), 1e-12));
    }
}
...
GTEST_TEST(TestPiecewiseQuaternionSlerp, TestShortestQuaternionPiecewiseQuaternionSlerp) {
    ...
    PiecewiseQuaternionSlerp<double> rot_spline(time, quat);
    const std::vector<Quaternion<double>>& internal_quat = rot_spline.get_quaternion_samples();

    EXPECT_TRUE(CheckClosest(internal_quat));
    EXPECT_FALSE(CheckClosest(quat));

    for (size_t i = 0; i < time.size() - 1; ++i) {
        EXPECT_TRUE(CompareMatrices(rot_spline.orientation(time[i]).coeffs(), internal_quat[i].coeffs(), 1e-10,
                                    MatrixCompareType::absolute));
        double ang_diff = ang[i + 1] - ang[i];
        double time_diff = time[i + 1] - time[i];
        double omega = math::wrap_to(ang_diff, -M_PI, M_PI) / time_diff;
        EXPECT_TRUE(CompareMatrices(rot_spline.angular_velocity(time[i]), omega * axis, 1e-10,
                                    MatrixCompareType::absolute));
    }
...
GTEST_TEST(TestPiecewiseQuaternionSlerp, TestIdenticalPiecewiseQuaternionSlerp) {
    ...
    const std::vector<Quaternion<double>>& internal_quats = rot_spline.get_quaternion_samples();
    EXPECT_TRUE(CompareMatrices(rot_spline.orientation(t).coeffs(), internal_quats[0].coeffs(), 1e-10,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(rot_spline.orientation(t).coeffs(), internal_quats[1].coeffs(), 1e-10,
                                MatrixCompareType::absolute));
}
...
GTEST_TEST(TestPiecewiseQuaternionSlerp, TestDerivatives) {
    ...
    for (double t = -1.0; t < 4.0; t += 0.234) {
        ...
        EXPECT_TRUE(CompareMatrices(rot_spline.value(t), zeroth_derivative->value(t), 1e-14));
        EXPECT_TRUE(CompareMatrices(rot_spline.value(t), rot_spline.EvalDerivative(t, 0), 1e-14));

        // Test first derivative.
        EXPECT_TRUE(CompareMatrices(rot_spline.angular_velocity(t), first_derivative->value(t), 1e-14));
        EXPECT_TRUE(CompareMatrices(rot_spline.angular_velocity(t), rot_spline.EvalDerivative(t, 1), 1e-14));

        // Test second derivative.
        EXPECT_TRUE(CompareMatrices(rot_spline.angular_acceleration(t), second_derivative->value(t), 1e-14));
        EXPECT_TRUE(CompareMatrices(rot_spline.angular_acceleration(t), rot_spline.EvalDerivative(t, 2), 1e-14));
    }
}
