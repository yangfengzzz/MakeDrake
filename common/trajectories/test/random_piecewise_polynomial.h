...
PiecewisePolynomial<T> MakeRandomPiecewisePolynomial(Eigen::Index rows,
                                                     Eigen::Index cols,
    ...
    for (Eigen::Index segment_index = 0; segment_index < num_segments; ++segment_index) {
        polynomials.push_back(drake::test::RandomPolynomialMatrix<T>(num_coefficients_per_polynomial, rows, cols));
    }
