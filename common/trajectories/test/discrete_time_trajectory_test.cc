...
GTEST_TEST(DiscreteTimeTrajectoryTest, ValueThrowsTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(traj.value(-1), "Value requested at time -1(\\.0)? does not match .*");
    DRAKE_EXPECT_THROWS_MESSAGE(traj.value(0.4), "Value requested at time 0.4 does not match .*");
    DRAKE_EXPECT_THROWS_MESSAGE(traj.value(1), "Value requested at time 1(\\.0)? does not match .*");

    const double kLooseTolerance = 0.1;
    DiscreteTimeTrajectory<double> traj_w_loose_tol(times, values, kLooseTolerance);
...
GTEST_TEST(DicreteTimeTrajectoryTest, SymbolicTimes) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DiscreteTimeTrajectory<Expression>(times, values),
                                ".*environment does not have an entry for the variable t.*\n*");

    times[1] = 0.4;
    DiscreteTimeTrajectory<Expression> traj(times, values);
    DRAKE_EXPECT_THROWS_MESSAGE(traj.value(t), ".*environment does not have an entry for the variable t.*\n*");
}
