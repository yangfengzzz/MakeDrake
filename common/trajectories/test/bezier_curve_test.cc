#include "common/trajectories/bezier_curve.h"
...
void CheckConvexHullProperty(const BezierCurve<double>& curve, int num_samples) {
    VPolytope control_polytope{curve.control_points()};
    const double tol_sol = 1e-8;
    Eigen::VectorXd samples = Eigen::VectorXd::LinSpaced(num_samples, curve.start_time(), curve.end_time());
...
GTEST_TEST(BezierCurveTest, Linear) {
  ...
  points << 1, 2,
    ...
    EXPECT_TRUE(CompareMatrices(curve.value(2.5), Eigen::Vector2d(1.5, 5), 1e-14));
    EXPECT_TRUE(CompareMatrices(curve.value(0), Eigen::Vector2d(1, 3), 1e-14));

    auto deriv = curve.MakeDerivative();
    BezierCurve<double>& deriv_bezier = dynamic_cast<BezierCurve<double>&>(*deriv);
    ...
    EXPECT_TRUE(CompareMatrices(curve.EvalDerivative(2.5), Eigen::Vector2d(1, 4), 1e-14));
    ...
    EXPECT_EQ(M.rows(), 2);
    EXPECT_EQ(M.cols(), 1);
    EXPECT_TRUE(CompareMatrices(deriv_bezier.control_points(), curve.control_points() * M, 1e-14));
...
GTEST_TEST(BezierCurveTest, Quadratic) {
  ...
  points << 1, 0, 0,
    ...
    BezierCurve<double>& second_deriv_bezier = dynamic_cast<BezierCurve<double>&>(*second_deriv);
    ...
    BezierCurve<double>& second_deriv_via_deriv_bezier = dynamic_cast<BezierCurve<double>&>(*second_deriv_via_deriv);
    // second derivative matches, if obtained by differentiating twice.
    EXPECT_TRUE(CompareMatrices(second_deriv_bezier.control_points(), second_deriv_via_deriv_bezier.control_points()));
    ...
    for (double sample_time = -0.2; sample_time <= 1.2; sample_time += 0.1) {
        ...
        EXPECT_TRUE(CompareMatrices(curve.value(sample_time), Eigen::Vector2d((1 - t) * (1 - t), t * t), 1e-14));
        EXPECT_TRUE(CompareMatrices(deriv->value(sample_time), Eigen::Vector2d(-2 * (1 - t), 2 * t), 1e-14));
        EXPECT_TRUE(CompareMatrices(curve.EvalDerivative(sample_time), Eigen::Vector2d(-2 * (1 - t), 2 * t), 1e-14));
        EXPECT_TRUE(CompareMatrices(second_deriv->value(sample_time), Eigen::Vector2d(2, 2), 1e-14));
        EXPECT_TRUE(CompareMatrices(curve.EvalDerivative(sample_time, 2), Eigen::Vector2d(2, 2), 1e-14));
    }

    M = curve.AsLinearInControlPoints(2);
    EXPECT_EQ(M.rows(), 3);
    EXPECT_EQ(M.cols(), 1);
    EXPECT_TRUE(CompareMatrices(second_deriv_bezier.control_points(), curve.control_points() * M, 1e-14));

    // Extract the symbolic expression for the bezier curve.
    VectorX<symbolic::Expression> curve_expression{curve.GetExpression(symbolic::Variable("t"))};
...
GTEST_TEST(BezierCurve, ConstraintDerivativeControlPoint) {
  ...
  points << 1, 0, 0,
    ...
    for (int i = 0; i < curve.rows(); ++i) {
        auto constraint =
                std::make_shared<solvers::LinearConstraint>(M.col(k).transpose(), Vector1d(lb(i)), Vector1d(ub(i)));
        auto b = prog.AddConstraint(constraint, p.row(i).transpose());
        EXPECT_NEAR(prog.EvalBindingAtInitialGuess(b)[0], deriv_bezier.control_points()(i, k), 1e-12);
    }
    ...
    */
    ...
    Map<const VectorXd> vec_curve_points(curve.control_points().transpose().data(), curve.rows() * (curve.order() + 1));
    Map<const VectorXd> vec_deriv_points(deriv_bezier.control_points().transpose().data(),
                                         curve.rows() * (deriv_bezier.order() + 1));
    EXPECT_TRUE(CompareMatrices(vec_deriv_points, blockMT * vec_curve_points, 1e-12));
}
...
GTEST_TEST(BezierCurve, Clone) {
  ...
  points << 1, 0, 0,
    ...
    BezierCurve<double>& clone_bezier = dynamic_cast<BezierCurve<double>&>(*clone);
...
GTEST_TEST(BezierCurve, ScalarTypes) {
  ...
  points << 1, 0, 0,
    ...
    BezierCurve<symbolic::Expression> curve_sym(0, 1, points.cast<symbolic::Expression>());
...
GTEST_TEST(BezierCurve, GetExpressionLinear) {
    ...
    auto test_expression_from_points = [&t](double start_time, double end_time,
                                            const Eigen::Ref<const Eigen::MatrixXd>& points) {
        BezierCurve<double> curve_double(start_time, end_time, points);
        BezierCurve<AutoDiffXd> curve_ad(start_time, end_time, points.cast<AutoDiffXd>());
        BezierCurve<symbolic::Expression> curve_sym(start_time, end_time, points.cast<symbolic::Expression>());

        const VectorX<symbolic::Expression> curve_expression{curve_sym.GetExpression(t)};
        const VectorX<symbolic::Expression> curve_expression_double{curve_double.GetExpression(t)};
        const VectorX<symbolic::Expression> curve_expression_ad{curve_ad.GetExpression(t)};
        for (int i = 0; i < curve_expression.rows(); i++) {
            EXPECT_TRUE(curve_expression(i).is_polynomial());
            EXPECT_EQ(symbolic::Polynomial(curve_expression(i)).TotalDegree(), points.cols() - 1);
  ...
  points_linear << 1, 2,
    ...
    const VectorX<symbolic::Expression> linear_curve_expression =
            test_expression_from_points(start_time_linear, end_time_linear, points_linear);
    for (int i = 0; i < linear_curve_expression.rows(); i++) {
        symbolic::Expression expr_expected =
                (end_time_linear - t) * points_linear(i, 0) + (t - start_time_linear) * points_linear(i, 1);
