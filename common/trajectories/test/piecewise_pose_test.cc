...
protected:
    void SetUp() override {
        ...
        dut_ = PiecewisePose<double>::MakeCubicLinearWithEndLinearVelocity(times, samples, start_vel, end_vel);

        test_times_ = {times.front() - 0.2, times.front(), (times.front() + times.back()) / 2., times.back(),
                       times.back() + 0.3};

        position_ = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(times, pos_samples, start_vel,
                                                                                      end_vel);
...
TEST_F(PiecewisePoseTest, TestEndLinearVelocity) {
    ...
    EXPECT_TRUE(drake::CompareMatrices(dut_.GetVelocity(t0).tail<3>(), Vector3<double>::Zero(), 1e-12,
                                       drake::MatrixCompareType::absolute));

    EXPECT_TRUE(drake::CompareMatrices(dut_.GetVelocity(t1).tail<3>(), Vector3<double>::Zero(), 1e-12,
                                       drake::MatrixCompareType::absolute));
}
...
TEST_F(PiecewisePoseTest, TestPose) {
    for (double time : test_times_) {
        math::RigidTransform<double> expected(orientation_.orientation(time), position_.value(time));
        EXPECT_TRUE(drake::CompareMatrices(dut_.GetPose(time).GetAsMatrix4(), expected.GetAsMatrix4(), 1e-12,
                                           drake::MatrixCompareType::absolute));
        EXPECT_TRUE(drake::CompareMatrices(dut_.value(time), expected.GetAsMatrix4(), 1e-12,
                                           drake::MatrixCompareType::absolute));
    }
}
...
TEST_F(PiecewisePoseTest, TestVelocity) {
    for (double time : test_times_) {
        ...
        EXPECT_TRUE(
                drake::CompareMatrices(dut_.GetVelocity(time), expected, 1e-12, drake::MatrixCompareType::absolute));
    }
}
...
TEST_F(PiecewisePoseTest, TestAcceleration) {
    for (double time : test_times_) {
        ...
        EXPECT_TRUE(drake::CompareMatrices(dut_.GetAcceleration(time), expected, 1e-12,
                                           drake::MatrixCompareType::absolute));
    }
}
...
TEST_F(PiecewisePoseTest, TestIsApprox) {
    ...
    PiecewisePolynomial<double> new_pos_traj =
            PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(times, pos_samples, start_vel, end_vel);
...
TEST_F(PiecewisePoseTest, TestTrajectoryOverrides) {
    ...
    std::vector<double> truncated_test_times_ = std::vector<double>(test_times_.begin() + 1, test_times_.end() - 1);
    ...
    for (double time : truncated_test_times_) {
        EXPECT_TRUE(drake::CompareMatrices(dut_.value(time), zeroth_derivative->value(time), 1e-12,
                                           drake::MatrixCompareType::absolute));
        EXPECT_TRUE(drake::CompareMatrices(dut_.value(time), dut_.EvalDerivative(time, 0), 1e-12,
                                           drake::MatrixCompareType::absolute));

        EXPECT_TRUE(drake::CompareMatrices(dut_.GetVelocity(time), first_derivative->value(time), 1e-12,
                                           drake::MatrixCompareType::absolute));
        EXPECT_TRUE(drake::CompareMatrices(dut_.GetVelocity(time), dut_.EvalDerivative(time, 1), 1e-12,
                                           drake::MatrixCompareType::absolute));

        EXPECT_TRUE(drake::CompareMatrices(dut_.GetAcceleration(time), second_derivative->value(time), 1e-12,
                                           drake::MatrixCompareType::absolute));
        EXPECT_TRUE(drake::CompareMatrices(dut_.GetAcceleration(time), dut_.EvalDerivative(time, 2), 1e-12,
                                           drake::MatrixCompareType::absolute));
    }
}
...
GTEST_TEST(PiecewisePoseMakeTest, MakeLinear) {
    ...
    const std::vector<RigidTransformd> poses{
            RigidTransformd(), RigidTransformd(RotationMatrixd::MakeXRotation(M_PI / 2), Vector3d{.1, .2, .3}),
            RigidTransformd(Vector3d{.4, .5, .6})};

    PiecewisePose<double> pose_traj = PiecewisePose<double>::MakeLinear(times, poses);
    ...
    EXPECT_TRUE(pose_traj.GetPose(1.5).IsNearlyEqualTo(
            RigidTransformd(RotationMatrixd::MakeXRotation(M_PI / 4), Vector3d{.05, .1, .15}), 1e-14));
    EXPECT_TRUE(pose_traj.GetPose(2.5).IsNearlyEqualTo(
            RigidTransformd(RotationMatrixd::MakeXRotation(M_PI / 4), Vector3d{.25, .35, .45}), 1e-14));
}
...
void TestScalarType() {
    ...
    PiecewisePolynomial<T> position_traj =
            PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(times, pos_samples, start_vel, end_vel);
