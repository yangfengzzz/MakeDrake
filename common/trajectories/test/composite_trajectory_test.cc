...
GTEST_TEST(CompositeTrajectoryTest, Basic) {
  ...
  points << 1, 2,
    ...
    EXPECT_TRUE(CompareMatrices(traj.value(3.5), Eigen::Vector2d(2.5, 8.5), 1e-14));
    ...
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(2.5), Eigen::Vector2d(1, 4), 1e-14));
    EXPECT_TRUE(CompareMatrices(traj.EvalDerivative(3.5), Eigen::Vector2d(1, 3), 1e-14));

    auto clone = traj.Clone();
    EXPECT_EQ(clone->start_time(), 2.0);
    EXPECT_EQ(clone->end_time(), 4.0);
    EXPECT_TRUE(CompareMatrices(clone->value(2.5), Eigen::Vector2d(1.5, 5), 1e-14));
    EXPECT_TRUE(CompareMatrices(clone->value(3.5), Eigen::Vector2d(2.5, 8.5), 1e-14));
...
void CheckScalarType() {
  ...
  points << 1, 0, 0,
    ...
    EXPECT_TRUE(CompareMatrices(traj.value(0.5), Eigen::Vector2d(0.25, 0.25), 1e-14));
}
