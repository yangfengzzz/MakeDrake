...
public:
    explicit PiecewiseTrajectoryTester(const std::vector<T>& times) : PiecewiseTrajectory<T>(times) {}
...
void TestPiecewiseTrajectoryTimeRelatedGetters(const PiecewiseTrajectoryTester<double>& traj,
...
void TestScalarType() {
    std::default_random_engine generator(123);
    std::vector<T> time = PiecewiseTrajectory<T>::RandomSegmentTimes(5, generator);
    ...
    EXPECT_TRUE(static_cast<bool>(traj.is_time_in_range((traj.start_time() + traj.end_time()) / 2.0)));
}
