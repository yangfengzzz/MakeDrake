...
DiscreteTimeTrajectory<double> MakeDiscrete(double t0, const MatrixXd& value0, double t1, const MatrixXd& value1) {
...
GTEST_TEST(StackedTrajectoryTest, AppendMismatchedSizesRowwise) {
    ...
    StackedTrajectory<double> dut;
    dut.Append(MakeDiscrete(0, x, 1, -x));
    DRAKE_EXPECT_THROWS_MESSAGE(dut.Append(MakeDiscrete(0, y, 1, -y)), ".*cols.*");
}
...
GTEST_TEST(StackedTrajectoryTest, AppendMismatchedSizesColwise) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.Append(MakeDiscrete(0, x, 1, -x)), ".*rows.*");
}
