#include "common/trajectories/bspline_trajectory.h"
...
#include "math/knot_vector_type.h"

DEFINE_bool(visualize,
            false,
...
BsplineTrajectory<T> MakeCircleTrajectory() {
    ...
    for (int i = 0; i < num_control_points; ++i) {
        control_points.push_back((MatrixX<T>(2, 1) << sin(t_dummy(i)), cos(t_dummy(i))).finished());
    }
...
TYPED_TEST(BsplineTrajectoryTests, ConstructorTest) {
    ...
    const std::vector<T> expected_knots{0, 0, 0, 0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1, 1, 1, 1};
    ...
    for (int i = 0; i < expected_num_control_points; ++i) {
        expected_control_points.push_back(i * MatrixX<T>::Ones(expected_rows, expected_cols));
    }
    ...
    EXPECT_TRUE(std::equal(traj_control_points.begin(), traj_control_points.end(), expected_control_points.begin(),
                           expected_control_points.end(), [](const auto& traj_matrix, const auto& expected_matrix) {
    ...
    BsplineTrajectory<T> trajectory_from_double{bspline_basis_2, expected_control_points};
...
TYPED_TEST(BsplineTrajectoryTests, ValueTest) {
    ...
    VectorX<T> t = VectorX<T>::LinSpaced(num_times, trajectory.start_time() - 0.1, trajectory.end_time() + 0.1);
    for (int k = 0; k < num_times; ++k) {
        MatrixX<T> value = trajectory.value(t(k));
        using std::clamp;
        const T t_clamped = clamp(t(k), trajectory.start_time(), trajectory.end_time());
        MatrixX<T> expected_value = trajectory.basis().EvaluateCurve(trajectory.control_points(), t_clamped);
        EXPECT_TRUE(CompareMatrices(value, expected_value, std::numeric_limits<T>::epsilon()));
    }
}
...
TYPED_TEST(BsplineTrajectoryTests, MakeDerivativeTest) {
    ...
    for (int k = 0; k < num_times; ++k) {
        ...
        MatrixX<T> expected_derivative =
                ComputeNumericalGradient(calc_value, Vector1<T>{t(k)}, NumericalGradientOption{method});
    ...
    MatrixX<T> expected_derivative = MatrixX<T>::Zero(trajectory.rows(), trajectory.cols());
    for (int k = 0; k < num_times; ++k) {
        MatrixX<T> derivative = derivative_trajectory->value(t(k));
...
TYPED_TEST(BsplineTrajectoryTests, EvalDerivativeTest) {
    ...
    for (int o = 0; o < trajectory.basis().order(); ++o) {
        std::unique_ptr<Trajectory<T>> derivative_trajectory = trajectory.MakeDerivative(o);
        for (int k = 0; k < num_times; ++k) {
    ...
    for (int o = 0; o < trajectory.basis().order(); ++o) {
        EXPECT_TRUE(CompareMatrices(trajectory.EvalDerivative(trajectory.start_time() - 1, o),
                                    trajectory.EvalDerivative(trajectory.start_time(), o)));
        EXPECT_TRUE(CompareMatrices(trajectory.EvalDerivative(trajectory.end_time() + 1, o),
...
TYPED_TEST(BsplineTrajectoryTests, CopyBlockTest) {
    ...
    for (const auto& knot_vector_type : {KnotVectorType::kUniform, KnotVectorType::kClampedUniform}) {
        BsplineTrajectory<T> trajectory =
                BsplineTrajectory<T>{BsplineBasis<T>{order, num_control_points, knot_vector_type}, control_points}
                        .CopyBlock(0, 0, 2, 3);
        BsplineTrajectory<T> expected_trajectory = BsplineTrajectory<T>{
                BsplineBasis<T>{order, num_control_points, knot_vector_type}, expected_control_points};
...
TYPED_TEST(BsplineTrajectoryTests, InsertKnotsTest) {
    ...
    // multiplicity of 2.
    std::vector<T> new_knots{original_trajectory.start_time(), M_PI_4, 0.25, 0.25, original_trajectory.end_time()};
    // Add the new knots.
    ...
    EXPECT_EQ(trajectory_with_new_knots.start_time(), original_trajectory.start_time());
    EXPECT_EQ(trajectory_with_new_knots.end_time(), original_trajectory.end_time());
    ...
    const int num_times = 100;
    VectorX<T> t = VectorX<T>::LinSpaced(num_times, original_trajectory.start_time(), original_trajectory.end_time());
    ...
    for (int k = 0; k < num_times; ++k) {
        ...
        if constexpr (std::is_same_v<T, double>) {
            if (FLAGS_visualize) {
                CallPython("plot", t(k), value.transpose(),
...
GTEST_TEST(BsplineTrajectoryDerivativeTests, AutoDiffTest) {
    ...
    std::unique_ptr<Trajectory<double>> derivative_trajectory = MakeCircleTrajectory<double>().MakeDerivative();
    const int num_times = 100;
    ...
    for (int k = 0; k < num_times; ++k) {
        ...
        MatrixX<double> expected_derivative_value = derivative_trajectory->value(t(k));
        EXPECT_TRUE(CompareMatrices(derivative_value, expected_derivative_value, kTolerance));
    }
}
...
GTEST_TEST(BsplineTrajectorySerializeTests, GoodTest) {
  ...
  const std::vector<MatrixX<double>> control_points{
    ...
    EXPECT_EQ(dut, BsplineTrajectory<double>(BsplineBasis<double>(kOrder, knots), control_points));
}
...
GTEST_TEST(BsplineTrajectorySerializeTests, NotEnoughControlPointsTest) {
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlString<BsplineTrajectory<double>>(not_enough_control_points),
                                ".*num_basis_functions.*");
}
