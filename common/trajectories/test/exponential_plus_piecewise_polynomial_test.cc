...
void testSimpleCase() {
    ...
    auto polynomial_part = test::MakeRandomPiecewisePolynomial<T>(1, 1, num_coefficients, breaks);
    ...
    uniform_real_distribution<T> uniform(expPlusPp.start_time(), expPlusPp.end_time());
    double t = uniform(generator);
    auto check = K(0) * std::exp(A(0) * (t - expPlusPp.start_time())) * alpha(0) + polynomial_part.scalarValue(t);
    auto derivative_check = K(0) * A(0) * std::exp(A(0) * (t - expPlusPp.start_time())) * alpha(0) +
