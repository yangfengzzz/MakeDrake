...
#include "common/trajectories/discrete_time_trajectory.h"
...
PiecewisePolynomial<T> MakePP() {
    return PiecewisePolynomial<T>::CubicShapePreserving(
            {0.2, 0.3, 0.7, 0.8}, {Vector2d(10, -100), Vector2d(20, 0), Vector2d(90, 10), Vector2d(100, 200)});
}
...
GTEST_TEST(DerivativeTrajectoryTest, BasicTest) {
    ...
    for (int derivative_order = 0; derivative_order < 3; ++derivative_order) {
        ...
        const double t = 0.45;
        EXPECT_TRUE(CompareMatrices(dut.value(t), nominal.EvalDerivative(t, derivative_order)));
        auto clone = dut.Clone();
        EXPECT_TRUE(CompareMatrices(clone->value(t), nominal.EvalDerivative(t, derivative_order)));
        for (int i = 0; i < 2; ++i) {
            EXPECT_TRUE(CompareMatrices(dut.EvalDerivative(t, i), nominal.EvalDerivative(t, derivative_order + i)));
            auto deriv = dut.MakeDerivative(i);
            EXPECT_TRUE(CompareMatrices(deriv->value(t), nominal.EvalDerivative(t, derivative_order + i)));
        }
    }
}

GTEST_TEST(DerivativeTrajectoryTest, ThrowsIfNoDerivative) {
    DiscreteTimeTrajectory<double> nominal({0.1, 0.4}, {Vector1d{0.3}, Vector1d{0.5}});
    DRAKE_EXPECT_THROWS_MESSAGE(DerivativeTrajectory(nominal), ".*has_derivative.*failed.*");
}

GTEST_TEST(DerivativeTrajectoryTest, ThrowsIfNegativeOrder) {
    const PiecewisePolynomial<double> nominal = MakePP<double>();
    DRAKE_EXPECT_THROWS_MESSAGE(DerivativeTrajectory(nominal, -1), ".*derivative_order.*failed.*");
}
