...
template <typename T>
bool PiecewiseQuaternionSlerp<T>::is_approx(const PiecewiseQuaternionSlerp<T>& other, double tol) const {
    ...
    for (size_t i = 0; i < quaternions_.size(); ++i) {
        ...
        double dot = abs(ExtractDoubleOrThrow(quaternions_[i].dot(other.quaternions_[i])));
        if (dot < cos(tol / 2)) {
            return false;
        }
    }
...
template <typename T>
Vector3<T> ComputeAngularVelocity(const T& duration, const Quaternion<T>& q, const Quaternion<T>& qnext) {
...
template <typename T>
void PiecewiseQuaternionSlerp<T>::Initialize(const std::vector<T>& breaks,
    ...
    for (size_t i = 0; i < quaternions.size(); ++i) {
        if (i == 0) {
            ...
            quaternions_[i] = math::ClosestQuaternion(quaternions_[i - 1], quaternions[i]);
            angular_velocities_[i - 1] =
                    internal::ComputeAngularVelocity(this->duration(i - 1), quaternions_[i - 1], quaternions[i]);
        }
    }
}

template <typename T>
PiecewiseQuaternionSlerp<T>::PiecewiseQuaternionSlerp(const std::vector<T>& breaks,
                                                      const std::vector<Quaternion<T>>& quaternions)
...
template <typename T>
T PiecewiseQuaternionSlerp<T>::ComputeInterpTime(int segment_index, const T& time) const {
    T interp_time = (time - this->start_time(segment_index)) / this->duration(segment_index);
...
Quaternion<T> PiecewiseQuaternionSlerp<T>::orientation(const T& t) const {
    ...
    Quaternion<T> q1 = quaternions_.at(segment_index).slerp(interp_t, quaternions_.at(segment_index + 1));
...
template <typename T>
void PiecewiseQuaternionSlerp<T>::Append(const T& time, const Quaternion<T>& quaternion) {
    DRAKE_DEMAND(this->breaks().empty() || time > this->breaks().back());
    if (quaternions_.empty()) {
        quaternions_.push_back(quaternion.normalized());
    } else {
        angular_velocities_.push_back(
                internal::ComputeAngularVelocity(time - this->breaks().back(), quaternions_.back(), quaternion));
        quaternions_.push_back(math::ClosestQuaternion(quaternions_.back(), quaternion));
    }
    this->get_mutable_breaks().push_back(time);
}

template <typename T>
void PiecewiseQuaternionSlerp<T>::Append(const T& time, const math::RotationMatrix<T>& rotation_matrix) {
    Append(time, rotation_matrix.ToQuaternion());
}

template <typename T>
void PiecewiseQuaternionSlerp<T>::Append(const T& time, const AngleAxis<T>& angle_axis) {
...
template <typename T>
MatrixX<T> PiecewiseQuaternionSlerp<T>::DoEvalDerivative(const T& t, int derivative_order) const {
...
template <typename T>
std::unique_ptr<Trajectory<T>> PiecewiseQuaternionSlerp<T>::DoMakeDerivative(int derivative_order) const {
    if (derivative_order == 0) {
        return this->Clone();
    } else if (derivative_order == 1) {
        std::vector<MatrixX<T>> m(angular_velocities_.begin(), angular_velocities_.end());
        m.push_back(Vector3<T>::Zero());
        return PiecewisePolynomial<T>::ZeroOrderHold(this->get_segment_times(), m).Clone();
    }
    // All higher derivatives are zero.
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PiecewiseQuaternionSlerp);
