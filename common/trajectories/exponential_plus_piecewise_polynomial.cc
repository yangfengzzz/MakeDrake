...
ExponentialPlusPiecewisePolynomial<T>::ExponentialPlusPiecewisePolynomial(
    ...
    : PiecewiseTrajectory<T>(piecewise_polynomial_part),
      ...
      alpha_(MatrixX<T>::Zero(1, piecewise_polynomial_part.get_number_of_segments())),
...
template <typename T>
std::unique_ptr<Trajectory<T>> ExponentialPlusPiecewisePolynomial<T>::Clone() const {
...
template <typename T>
ExponentialPlusPiecewisePolynomial<T> ExponentialPlusPiecewisePolynomial<T>::derivative(int derivative_order) const {
    ...
    return ExponentialPlusPiecewisePolynomial<T>(K_new, A_, alpha_,
