...
template <typename T>
BsplineTrajectory<T>::BsplineTrajectory(BsplineBasis<T> basis, std::vector<MatrixX<T>> control_points)
...
MatrixX<T> BsplineTrajectory<T>::value(const T& time) const {
    using std::clamp;
    return basis().EvaluateCurve(control_points(), clamp(time, start_time(), end_time()));
}
...
template <typename T>
MatrixX<T> BsplineTrajectory<T>::DoEvalDerivative(const T& time, int derivative_order) const {
    if (derivative_order == 0) {
        ...
        BsplineBasis<T> lower_order_basis = BsplineBasis<T>(basis_.order() - derivative_order, derivative_knots);
        std::vector<MatrixX<T>> coefficients(control_points());
        std::vector<int> base_indices = basis_.ComputeActiveBasisFunctionIndices(clamped_time);
        ...
        std::vector<MatrixX<T>> derivative_control_points(num_control_points() - derivative_order,
                                                          MatrixX<T>::Zero(rows(), cols()));
        for (int i : lower_order_basis.ComputeActiveBasisFunctionIndices(clamped_time)) {
            derivative_control_points.at(i) = coefficients.at(i);
        }
        return lower_order_basis.EvaluateCurve(derivative_control_points, clamped_time);
    } else {
...
template <typename T>
std::unique_ptr<Trajectory<T>> BsplineTrajectory<T>::DoMakeDerivative(int derivative_order) const {
    if (derivative_order == 0) {
        return this->Clone();
        ...
        return std::make_unique<BsplineTrajectory<T>>(BsplineBasis<T>(1, derivative_knots), control_points);
    } else if (derivative_order > 1) {
        ...
        for (int i = 0; i < num_control_points() - 1; ++i) {
            derivative_control_points.push_back(basis_.degree() /
                                                (basis_.knots()[i + basis_.order()] - basis_.knots()[i + 1]) *
                                                (control_points()[i + 1] - control_points()[i]));
        }
        return std::make_unique<BsplineTrajectory<T>>(BsplineBasis<T>(basis_.order() - 1, derivative_knots),
...
BsplineTrajectory<T> BsplineTrajectory<T>::CopyBlock(int start_row,
    ...
    return CopyWithSelector([&start_row, &start_col, &block_rows, &block_cols](const MatrixX<T>& full) {
...
template <typename T>
boolean<T> BsplineTrajectory<T>::operator==(const BsplineTrajectory<T>& other) const {
    if (this->basis() == other.basis() && this->rows() == other.rows() && this->cols() == other.cols()) {
        boolean<T> result{true};
        for (int i = 0; i < this->num_control_points(); ++i) {
            result = result && drake::all(this->control_points()[i].array() == other.control_points()[i].array());
...
void BsplineTrajectory<T>::CheckInvariants() const {
    DRAKE_THROW_UNLESS(static_cast<int>(control_points_.size()) == basis_.num_basis_functions());
}

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class BsplineTrajectory);
}  // namespace trajectories
}  // namespace drake
