...
public:
    ...
    ExponentialPlusPiecewisePolynomial(const Eigen::MatrixBase<DerivedK>& K,
        ...
        DRAKE_ASSERT(alpha.cols() == piecewise_polynomial_part.get_number_of_segments());
...
private:
    std::unique_ptr<Trajectory<T>> DoMakeDerivative(int derivative_order = 1) const override {
