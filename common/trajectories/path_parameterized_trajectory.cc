...
#include "common/trajectories/derivative_trajectory.h"

namespace drake {
namespace trajectories {

template <typename T>
PathParameterizedTrajectory<T>::PathParameterizedTrajectory(const Trajectory<T>& path,
...
MatrixX<T> PathParameterizedTrajectory<T>::value(const T& t) const {
    using std::clamp;
    const T time = clamp(t, time_scaling_->start_time(), time_scaling_->end_time());
    return path_->value(time_scaling_->value(time)(0, 0));
}

template <typename T>
T PathParameterizedTrajectory<T>::BellPolynomial(int n, int k, const VectorX<T>& x) const {
    if (n == 0 && k == 0) {
        return 1;
    } else if (n == 0 || k == 0) {
        return 0;
...
template <typename T>
MatrixX<T> PathParameterizedTrajectory<T>::DoEvalDerivative(const T& t, int derivative_order) const {
    using std::clamp;
    const T time = clamp(t, time_scaling_->start_time(), time_scaling_->end_time());
    if (derivative_order == 0) {
        ...
        for (int order = 0; order < derivative_order; ++order) {
            s_derivatives(order) = time_scaling_->EvalDerivative(time, order + 1)(0, 0);
        }
        ...
        for (int order = 1; order <= derivative_order; ++order) {
            MatrixX<T> path_partial = path_->EvalDerivative(time_scaling_->value(time)(0, 0), order);
            derivative += path_partial * BellPolynomial(derivative_order, order, s_derivatives);
        }
        return derivative;
    } else {
...
template <typename T>
std::unique_ptr<Trajectory<T>> PathParameterizedTrajectory<T>::DoMakeDerivative(int derivative_order) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PathParameterizedTrajectory);
