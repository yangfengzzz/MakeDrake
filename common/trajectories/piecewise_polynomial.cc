...
template <typename T>
std::tuple<std::vector<double>, std::vector<MatrixX<VectorXd>>> PiecewisePolynomial<T>::GetSerialized() const {
...
template <typename T>
void PiecewisePolynomial<T>::SetSerialized(const std::vector<double>& breaks,
    ...
    if constexpr (!std::is_same_v<T, double>) {
        ...
        if (breaks.size() != polynomials.size() + 1) {
            throw std::logic_error(
                    fmt::format("PiecewisePolynomial deserialization must provide "
        ...
        for (int n = 1; n < static_cast<int>(polynomials.size()); ++n) {
            if ((polynomials[n].rows() != polynomials[0].rows()) || (polynomials[n].cols() != polynomials[0].cols())) {
                throw std::logic_error(fmt::format(
                        ...
                        n, polynomials[n].rows(), polynomials[n].cols(), polynomials[0].rows(), polynomials[0].cols()));
            }
        }
        ...
        for (Eigen::Index row = 0; row < rows(); row++) {
            for (Eigen::Index col = 0; col < cols(); col++) {
                if (segment_index == 0) {
                    ...
                    matrix(row, col) = matrix(row, col).Integral(ret.EvaluateSegmentAbsoluteTime(
                            segment_index - 1, this->start_time(segment_index), row, col));
                }
            }
        }
    }
...
template <typename T>
const Polynomial<T>& PiecewisePolynomial<T>::getPolynomial(int segment_index,
                                                           Eigen::Index row,
...
void PiecewisePolynomial<T>::ConcatenateInTime(const PiecewisePolynomial<T>& other) {
    if (!empty()) {
        ...
        const T absolute_tolerance = max(abs(this->end_time()), 1.0) * std::numeric_limits<double>::epsilon();
...
template <typename T>
void PiecewisePolynomial<T>::AppendCubicHermiteSegment(const T& time,
    ...
    for (int row = 0; row < rows(); ++row) {
        for (int col = 0; col < cols(); ++col) {
            ...
            const T start_dot =
                    EvaluateSegmentAbsoluteTime(segment_index, this->end_time(), row, col, derivative_order);
...
T PiecewisePolynomial<T>::EvaluateSegmentAbsoluteTime(
        int segment_index, const T& t, Eigen::Index row, Eigen::Index col, int derivative_order) const {
...
void PiecewisePolynomial<T>::CheckSplineGenerationInputValidityOrThrow(const std::vector<T>& breaks,
    ...
    if (times.size() != Y.size()) {
        throw std::runtime_error(
                fmt::format("Number of break points {} does not match number of samples {}.", times.size(), Y.size()));
    }
    if (static_cast<int>(times.size()) < min_length) {
        throw std::runtime_error(fmt::format("{} samples is not enough samples (this method requires at least {}).",
    ...
    for (size_t i = 0; i < times.size() - 1; i++) {
        ...
        if (times[i + 1] - times[i] < PiecewiseTrajectory<T>::kEpsilonTime) {
            throw std::runtime_error(
                    fmt::format("Times must be at least {} apart.", PiecewiseTrajectory<T>::kEpsilonTime));
        }
    }
}
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::CubicShapePreserving(const std::vector<T>& breaks,
    ...
    if (!zero_end_point_derivatives) {
        Ydot_start =
                ComputePchipEndSlope<T>(times[1] - times[0], times[2] - times[1], (Y[1] - Y[0]) / (times[1] - times[0]),
            ...
            for (size_t t = 0; t < dt.size() - 1; ++t) {
                ...
                if (slope[t](j, k) * slope[t + 1](j, k) <= 0) {
                    ...
                    Ydot[t + 1](j, k) = 3 * common /
                                        ((common + dt[t + 1]) / slope[t](j, k) + (common + dt[t]) / slope[t + 1](j, k));
                }
            }
    ...
    for (int j = 0; j < rows; ++j) {
        for (int k = 0; k < cols; ++k) {
            ...
            triplet_list.push_back(Eigen::Triplet<T>(
                    row_idx, 3 * (N - 2) + 2, 3 * (times[N - 1] - times[N - 2]) * (times[N - 1] - times[N - 2])));
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::LagrangeInterpolatingPolynomial(const std::vector<T>& times,
    ...
    for (size_t i = 1; i < times.size(); ++i) {
        DRAKE_DEMAND(times[i] - times[i - 1] > PiecewiseTrajectory<T>::kEpsilonTime);
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::ZeroOrderHold(const Eigen::Ref<const VectorX<T>>& breaks,
    ...
    return PiecewisePolynomial<T>::ZeroOrderHold(my_breaks, EigenToStdVector(samples));
}
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::FirstOrderHold(const Eigen::Ref<const VectorX<T>>& breaks,
    ...
    DRAKE_DEMAND(samples.cols() == breaks.size());
    std::vector<T> my_breaks(breaks.data(), breaks.data() + breaks.size());
    return PiecewisePolynomial<T>::FirstOrderHold(my_breaks, EigenToStdVector(samples));
}
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::CubicShapePreserving(const Eigen::Ref<const VectorX<T>>& breaks,
    ...
    DRAKE_DEMAND(samples.cols() == breaks.size());
    std::vector<T> my_breaks(breaks.data(), breaks.data() + breaks.size());
    return PiecewisePolynomial<T>::CubicShapePreserving(my_breaks, EigenToStdVector(samples),
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(
    ...
    return PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(
            my_breaks, EigenToStdVector(samples), samples_dot_start.eval(), samples_dot_end.eval());
}
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::CubicHermite(const Eigen::Ref<const VectorX<T>>& breaks,
    ...
    DRAKE_DEMAND(samples.cols() == breaks.size());
    std::vector<T> my_breaks(breaks.data(), breaks.data() + breaks.size());
    return PiecewisePolynomial<T>::CubicHermite(my_breaks, EigenToStdVector(samples), EigenToStdVector(samples_dot));
}
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(
    ...
    DRAKE_DEMAND(samples.cols() == breaks.size());
    std::vector<T> my_breaks(breaks.data(), breaks.data() + breaks.size());
    return PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(my_breaks, EigenToStdVector(samples),
...
PiecewisePolynomial<T> PiecewisePolynomial<T>::LagrangeInterpolatingPolynomial(
    ...
    return PiecewisePolynomial<T>::LagrangeInterpolatingPolynomial(my_times, EigenToStdVector(samples));
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PiecewisePolynomial);
