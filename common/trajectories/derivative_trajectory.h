...
public:
    ...
    explicit DerivativeTrajectory(const Trajectory<T>& nominal, int derivative_order = 1);
...
private:
    ...
    std::unique_ptr<Trajectory<T>> DoMakeDerivative(int derivative_order) const final;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::DerivativeTrajectory);
