...
template <typename T>
MatrixX<T> StackedTrajectory<T>::DoEvalDerivative(const T& t, int derivative_order) const {
    MatrixX<T> result(rows(), cols());
    Index row = 0;
    Index col = 0;
    for (const auto& child : children_) {
...
template <typename T>
std::unique_ptr<Trajectory<T>> StackedTrajectory<T>::DoMakeDerivative(int derivative_order) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::StackedTrajectory);
