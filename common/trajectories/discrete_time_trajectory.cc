...
template <typename T>
DiscreteTimeTrajectory<T>::DiscreteTimeTrajectory(const Eigen::Ref<const VectorX<T>>& times,
                                                  const Eigen::Ref<const MatrixX<T>>& values,
                                                  const double time_comparison_tolerance)
    : DiscreteTimeTrajectory(std::vector<T>(times.data(), times.data() + times.size()),
                             EigenToStdVector(values),
                             time_comparison_tolerance) {}

template <typename T>
DiscreteTimeTrajectory<T>::DiscreteTimeTrajectory(const std::vector<T>& times,
                                                  const std::vector<MatrixX<T>>& values,
                                                  const double time_comparison_tolerance)
    : times_(times), values_(values), time_comparison_tolerance_(time_comparison_tolerance) {
...
std::unique_ptr<Trajectory<T>> DiscreteTimeTrajectory<T>::Clone() const {
    return std::make_unique<DiscreteTimeTrajectory<T>>(times_, values_, time_comparison_tolerance_);
}
...
MatrixX<T> DiscreteTimeTrajectory<T>::value(const T& t) const {
    ...
    for (int i = 0; i < static_cast<int>(times_.size()); ++i) {
        if (time < times_[i] - time_comparison_tolerance_) {
            throw std::runtime_error(fmt::format(kNoMatchingTimeStr, time, time_comparison_tolerance_));
        }
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::DiscreteTimeTrajectory);
