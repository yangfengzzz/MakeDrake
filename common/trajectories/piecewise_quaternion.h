...
public:
     ...
     */
    const std::vector<Quaternion<T>>& get_quaternion_samples() const { return quaternions_; }
...
private:
    ...
    void Initialize(const std::vector<T>& breaks, const std::vector<Quaternion<T>>& quaternions);
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PiecewiseQuaternionSlerp);
