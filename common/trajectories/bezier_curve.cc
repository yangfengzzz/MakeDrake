...
#include "math/binomial_coefficient.h"
...
template <typename T>
BezierCurve<T>::BezierCurve(double start_time, double end_time, const Eigen::Ref<const MatrixX<T>>& control_points)
    : start_time_{start_time}, end_time_{end_time}, control_points_{control_points} {
    DRAKE_DEMAND(end_time >= start_time);
}

template <typename T>
T BezierCurve<T>::BernsteinBasis(int i, const T& time, std::optional<int> order) const {
    using std::pow;
...
std::unique_ptr<Trajectory<T>> BezierCurve<T>::Clone() const {
    return std::make_unique<BezierCurve<T>>(start_time_, end_time_, control_points_);
}
...
template <typename T>
VectorX<symbolic::Expression> BezierCurve<T>::GetExpression(symbolic::Variable time) const {
    if constexpr (scalar_predicate<T>::is_bool) {
        MatrixX<symbolic::Expression> control_points{control_points_.rows(), control_points_.cols()};
        ...
        return BezierCurve<symbolic::Expression>(start_time_, end_time_, control_points).GetExpression(time);
    } else {
...
template <typename T>
SparseMatrix<double> BezierCurve<T>::AsLinearInControlPoints(int derivative_order) const {
...
MatrixX<T> BezierCurve<T>::CalcDerivativePoints(int derivative_order) const {
    ...
    int n = order();
    MatrixX<T> points = (control_points_.rightCols(order()) - control_points_.leftCols(order())) * order() /
                        (end_time_ - start_time_);
    for (int i = 1; i < derivative_order; ++i) {
        n -= 1;
        points = (points.rightCols(n) - points.leftCols(n)).eval() * n / (end_time_ - start_time_);
    }
    return points;
}

template <typename T>
MatrixX<T> BezierCurve<T>::DoEvalDerivative(const T& time, int derivative_order) const {
...
template <typename T>
std::unique_ptr<Trajectory<T>> BezierCurve<T>::DoMakeDerivative(int derivative_order) const {
    DRAKE_DEMAND(derivative_order >= 0);
    if (derivative_order == 0) {
        return this->Clone();
    }
    if (derivative_order > order()) {
        ...
        return std::make_unique<BezierCurve<T>>(start_time_, end_time_, VectorX<T>::Zero(rows()));
    }

    return std::make_unique<BezierCurve<T>>(start_time_, end_time_, CalcDerivativePoints(derivative_order));
}

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class BezierCurve);
