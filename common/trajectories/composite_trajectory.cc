...
#include "common/trajectories/piecewise_trajectory.h"
...
template <typename T>
std::vector<T> ExtractBreaks(const std::vector<copyable_unique_ptr<Trajectory<T>>>& segments) {
...
template <typename T>
CompositeTrajectory<T>::CompositeTrajectory(std::vector<copyable_unique_ptr<Trajectory<T>>> segments)
    : PiecewiseTrajectory<T>(ExtractBreaks(segments)), segments_(std::move(segments)) {
...
Eigen::Index CompositeTrajectory<T>::rows() const {
    if (segments_.size() > 0) {
        ...
        throw std::runtime_error("CompositeTrajectory has no segments. Number of rows is undefined.");
    }
}
...
Eigen::Index CompositeTrajectory<T>::cols() const {
    if (segments_.size() > 0) {
        ...
        throw std::runtime_error("CompositeTrajectory has no segments. Number of cols is undefined.");
    }
}
...
template <typename T>
MatrixX<T> CompositeTrajectory<T>::DoEvalDerivative(const T& time, int derivative_order) const {
    const int segment_index = this->get_segment_index(time);
    DRAKE_DEMAND(static_cast<int>(segments_.size()) > segment_index);
...
template <typename T>
std::unique_ptr<Trajectory<T>> CompositeTrajectory<T>::DoMakeDerivative(int derivative_order) const {
    DRAKE_DEMAND(derivative_order >= 0);
    if (derivative_order == 0) {
        return this->Clone();
    }
    std::vector<copyable_unique_ptr<Trajectory<T>>> derivative_curves(segments_.size());
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class CompositeTrajectory);
