...
template <typename T>
DerivativeTrajectory<T>::DerivativeTrajectory(const Trajectory<T>& nominal, int derivative_order)
...
template <typename T>
MatrixX<T> DerivativeTrajectory<T>::DoEvalDerivative(const T& t, int derivative_order) const {
    return nominal_->EvalDerivative(t, derivative_order_ + derivative_order);
}

template <typename T>
std::unique_ptr<Trajectory<T>> DerivativeTrajectory<T>::DoMakeDerivative(int derivative_order) const {
    return std::make_unique<DerivativeTrajectory<T>>(*nominal_, derivative_order_ + derivative_order);
}

}  // namespace trajectories
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::DerivativeTrajectory);
