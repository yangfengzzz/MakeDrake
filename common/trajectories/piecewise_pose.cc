...
template <typename T>
PiecewisePose<T>::PiecewisePose(const PiecewisePolynomial<T>& position_trajectory,
...
template <typename T>
PiecewisePose<T> PiecewisePose<T>::MakeLinear(const std::vector<T>& times,
    ...
    return PiecewisePose<T>(PiecewisePolynomial<T>::FirstOrderHold(times, pos_knots),
...
PiecewisePose<T> PiecewisePose<T>::MakeCubicLinearWithEndLinearVelocity(
        ...
        const std::vector<math::RigidTransform<T>>& poses,
        const Vector3<T>& start_vel,
        const Vector3<T>& end_vel) {
    std::vector<MatrixX<T>> pos_knots(poses.size());
    std::vector<math::RotationMatrix<T>> rot_knots(poses.size());
    for (size_t i = 0; i < poses.size(); ++i) {
        pos_knots[i] = poses[i].translation();
    ...
    return PiecewisePose<T>(
            PiecewisePolynomial<T>::CubicWithContinuousSecondDerivatives(times, pos_knots, start_vel, end_vel),
            PiecewiseQuaternionSlerp<T>(times, rot_knots));
}
...
math::RigidTransform<T> PiecewisePose<T>::GetPose(const T& time) const {
    return math::RigidTransform<T>(orientation_.orientation(time), position_.value(time));
}
...
template <typename T>
bool PiecewisePose<T>::IsApprox(const PiecewisePose<T>& other, double tol) const {
...
template <typename T>
MatrixX<T> PiecewisePose<T>::DoEvalDerivative(const T& t, int derivative_order) const {
    if (derivative_order == 0) {
        return value(t);
    }
    Vector6<T> derivative;
    derivative.template head<3>() = orientation_.EvalDerivative(t, derivative_order);
    derivative.template tail<3>() = position_.EvalDerivative(t, derivative_order);
    return derivative;
}

template <typename T>
std::unique_ptr<Trajectory<T>> PiecewisePose<T>::DoMakeDerivative(int derivative_order) const {
    if (derivative_order == 0) {
        return this->Clone();
    }
    std::unique_ptr<PiecewisePolynomial<T>> orientation_deriv =
            dynamic_pointer_cast<PiecewisePolynomial<T>>(orientation_.MakeDerivative(derivative_order));
    PiecewisePolynomial<T> position_deriv = position_.derivative(derivative_order);
    ...
    for (size_t ii = 0; ii < breaks.size() - 1; ii++) {
        ...
        segment_derivative.template topRows<3>() = orientation_deriv->getPolynomialMatrix(ii);
        segment_derivative.template bottomRows<3>() = position_deriv.getPolynomialMatrix(ii);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PiecewisePose);
