...
public:
     ...
     *
     */
    bool isApprox(const PiecewisePolynomial& other,
                  double tol,
...
private:
    ...
    T EvaluateSegmentAbsoluteTime(
            int segment_index, const T& t, Eigen::Index row, Eigen::Index col, int derivative_order = 0) const;
    ...
    variable power. */
    std::tuple<std::vector<double>, std::vector<MatrixX<Eigen::VectorXd>>> GetSerialized() const;

    /* Resets `this` to the data returned by a previous call to GetSerialized. */
    void SetSerialized(const std::vector<double>& breaks, const std::vector<MatrixX<Eigen::VectorXd>>& polynomials);
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::trajectories::PiecewisePolynomial);
