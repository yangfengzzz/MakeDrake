...
DEFINE_string(print_resource_path,
              "",
...
DEFINE_bool(print_resource_root_environment_variable_name,
            false,
...
int main(int argc, char* argv[]) {
    ...
    const int num_commands =
            (FLAGS_print_resource_path.empty() ? 0 : 1) + (FLAGS_print_resource_root_environment_variable_name ? 1 : 0);
    if (num_commands != 1) {
