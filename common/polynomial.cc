...
Polynomial<T> Polynomial<T>::Integral(const T& integration_constant) const {
    ...
    for (typename vector<Monomial>::iterator iter = ret.monomials_.begin(); iter != ret.monomials_.end(); iter++) {
        if (iter->terms.empty()) {
            Term t;
            t.var = 0;
            for (typename vector<Monomial>::iterator iterB = ret.monomials_.begin(); iterB != ret.monomials_.end();
                 iterB++) {
...
template <typename T>
boolean<T> Polynomial<T>::CoefficientsAlmostEqual(const Polynomial<T>& other,
...
string Polynomial<T>::IdToVariableName(const VarType id) {
    ...
    unsigned int multiplier = static_cast<unsigned int>(
            std::pow(static_cast<double>(kNumNameChars + 1), static_cast<int>(kNameLength) - 1));
...
public:
    Polynomial<T> Visit(const Expression& e) { return drake::symbolic::VisitExpression<Polynomial<T>>(this, e); }
...
private:
    static Polynomial<T> VisitAddition(const Expression& e) {
        ...
        return accumulate(expr_to_coeff_map.begin(), expr_to_coeff_map.end(), Polynomial<T>{constant},
                          [](const Polynomial<T>& polynomial, const pair<const Expression, double>& p) {
    ...
    static Polynomial<T> VisitMultiplication(const Expression& e) {
        ...
        const auto& base_to_exponent_map = drake::symbolic::get_base_to_exponent_map_in_multiplication(e);
        return accumulate(base_to_exponent_map.begin(), base_to_exponent_map.end(), Polynomial<T>{constant},
                          [](const Polynomial<T>& polynomial, const pair<const Expression, Expression>& p) {
                              ...
                              return polynomial * pow(Polynomial<T>::FromExpression(base),
    ...
    static Polynomial<T> VisitDivision(const Expression& e) {
        ...
        return Polynomial<T>::FromExpression(first_arg) / get_constant_value(second_arg);
    }

    static Polynomial<T> VisitVariable(const Expression& e) {
        return Polynomial<T>{1.0, static_cast<Polynomial<double>::VarType>(get_variable(e).get_id())};
    }

    static Polynomial<T> VisitConstant(const Expression& e) { return Polynomial<T>{get_constant_value(e)}; }
    ...
    static Polynomial<T> VisitPow(const Expression& e) {
        DRAKE_ASSERT(e.is_polynomial());
        const int exponent{static_cast<int>(get_constant_value(get_second_argument(e)))};
    ...
    static Polynomial<T> VisitUninterpretedFunction(const Expression&) {
        throw runtime_error("Uninterpreted-function expression is not polynomial-convertible.");
    }

    // Makes VisitExpression a friend of this class so that VisitExpression can
    // use its private methods.
    friend Polynomial<T> drake::symbolic::VisitExpression<Polynomial<T>>(FromExpressionVisitor*, const Expression&);
};
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::Polynomial);
