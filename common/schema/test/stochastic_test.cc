...
void CheckUniformDiscreteSymbolic(const Expression& e, std::vector<double> values) {
    const Variables vars{e.GetVariables()};
    ASSERT_EQ(vars.size(), 1);
    const Variable& v{*(vars.begin())};
    EXPECT_EQ(v.get_type(), Variable::Type::RANDOM_UNIFORM);
...
GTEST_TEST(StochasticTest, ScalarTest) {
    ...
    EXPECT_NE(std::find(ub.values.begin(), ub.values.end(), ub.Sample(&generator)), ub.values.end());
    ...
    EXPECT_NE(std::find(ub.values.begin(), ub.values.end(), vec(3)), ub.values.end());
    EXPECT_EQ(vec(4), 3.2);
...
GTEST_TEST(StochasticTest, VectorTest) {
    const auto variants = LoadYamlString<DistributionVectorStruct>(vector_variants);

    RandomGenerator generator;

    EXPECT_TRUE(
            CompareMatrices(ToDistributionVector(variants.vector)->Sample(&generator), Eigen::Vector3d(1., 2., 3.)));
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.vector)->Mean(), Eigen::Vector3d(1., 2., 3.)));
    ...
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.deterministic)->Sample(&generator),
                                Eigen::Vector3d(3., 4., 5.)));
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.deterministic)->Mean(), Eigen::Vector3d(3., 4., 5.)));
    ...
    EXPECT_EQ(ToDistributionVector(variants.gaussian1)->Sample(&generator).size(), 3);
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.gaussian1)->Mean(), Eigen::Vector3d(1.1, 1.2, 1.3)));
    ...
    EXPECT_EQ(ToDistributionVector(variants.gaussian2)->Sample(&generator).size(), 4);
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.gaussian2)->Mean(), Eigen::Vector4d(2.1, 2.2, 2.3, 2.4)));
    ...
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(variants.vector), Eigen::Vector3d(1., 2., 3.)));

    EXPECT_TRUE(IsDeterministic(variants.deterministic));
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(variants.deterministic), Eigen::Vector3d(3., 4., 5.)));
    ...
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(variants.deterministic_scalar), Vector1d(19.5)));

    EXPECT_FALSE(IsDeterministic(variants.gaussian_scalar));
    EXPECT_THROW(GetDeterministicValue(variants.gaussian_scalar), std::logic_error);

    EXPECT_FALSE(IsDeterministic(variants.uniform_scalar));
    EXPECT_THROW(GetDeterministicValue(variants.uniform_scalar), std::logic_error);

    Eigen::VectorXd uniform = ToDistributionVector(variants.uniform)->Sample(&generator);
    ASSERT_EQ(uniform.size(), 2);
    EXPECT_LE(10, uniform(0));
    EXPECT_GE(11, uniform(0));
    EXPECT_LE(20, uniform(1));
    EXPECT_GE(22, uniform(1));
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.uniform)->Mean(), Eigen::Vector2d(10.5, 21.0)));
    ...
    Eigen::VectorXd deterministic_scalar = ToDistributionVector(variants.deterministic_scalar)->Sample(&generator);
    ASSERT_EQ(deterministic_scalar.size(), 1);
    EXPECT_EQ(deterministic_scalar(0), 19.5);
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.deterministic_scalar)->Mean(), Vector1d(19.5)));

    Eigen::VectorXd gaussian_scalar = ToDistributionVector(variants.gaussian_scalar)->Sample(&generator);
    ASSERT_EQ(gaussian_scalar.size(), 1);
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.gaussian_scalar)->Mean(), Vector1d(5.0)));

    Eigen::VectorXd uniform_scalar = ToDistributionVector(variants.uniform_scalar)->Sample(&generator);
    ASSERT_EQ(uniform_scalar.size(), 1);
    EXPECT_LE(1, uniform_scalar(0));
    EXPECT_GE(2, uniform_scalar(0));
    EXPECT_TRUE(CompareMatrices(ToDistributionVector(variants.uniform_scalar)->Mean(), Vector1d(1.5)));
...
GTEST_TEST(StochasticTest, ZeroSizeRandomRanges) {
    DistributionVectorVariantX gaussian =
            GaussianVector<Eigen::Dynamic>(Eigen::VectorXd::Constant(3, 4.5), Eigen::VectorXd::Constant(3, 0.0));
    DistributionVectorVariantX uniform =
            UniformVector<Eigen::Dynamic>(Eigen::VectorXd::Constant(2, 1.5), Eigen::VectorXd::Constant(2, 1.5));
    ...
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(gaussian), Eigen::VectorXd::Constant(3, 4.5)));

    EXPECT_TRUE(IsDeterministic(uniform));
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(uniform), Eigen::VectorXd::Constant(2, 1.5)));

    EXPECT_TRUE(IsDeterministic(gaussian_scalar));
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(gaussian_scalar), Eigen::VectorXd::Constant(1, 4.5)));

    EXPECT_TRUE(IsDeterministic(uniform_scalar));
    EXPECT_TRUE(CompareMatrices(GetDeterministicValue(uniform_scalar), Eigen::VectorXd::Constant(1, 1.5)));
}
...
value: !Deterministic { value: 2.0 }
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlString<FixedSize2>(input),
