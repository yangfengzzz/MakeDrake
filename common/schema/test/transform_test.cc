...
GTEST_TEST(DeterministicTest, TransformTest) {
    ...
    drake::math::RigidTransformd expected(drake::math::RollPitchYawd(Eigen::Vector3d(10., 20., 30.) * (M_PI / 180.0)),
                                          Eigen::Vector3d(1., 2., 3.));
    constexpr double kTol = std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(
            drake::CompareMatrices(transform.GetDeterministicValue().GetAsMatrix34(), expected.GetAsMatrix34(), kTol));
    EXPECT_TRUE(drake::CompareMatrices(transform.Mean().GetAsMatrix34(), expected.GetAsMatrix34(), kTol));
}
...
GTEST_TEST(StochasticTest, TransformTest) {
    ...
    EXPECT_TRUE(std::holds_alternative<Rotation::Uniform>(transform.rotation.value));
    EXPECT_TRUE(drake::CompareMatrices(transform.Mean().translation(), Eigen::Vector3d(2.5, 3.5, 4.5)));
}
...
GTEST_TEST(StochasticSampleTest, TransformTest) {
    ...
    const auto& translation_domain = std::get<schema::UniformVector<3>>(transform.translation);
    for (int i = 0; i < 3; i++) {
    ...
    const auto& rotation_domain =
            std::get<schema::UniformVector<3>>(std::get<schema::Rotation::Rpy>(transform.rotation.value).deg);
    const Eigen::Vector3d rpy = drake::math::RollPitchYawd(sampled_transform.rotation()).vector() * (180 / M_PI);
    ...
    const drake::math::RigidTransformd sampled_transform_2 = transform.Sample(&generator);
    EXPECT_FALSE(sampled_transform.IsExactlyEqualTo(sampled_transform_2));

    // Check the mean.
    drake::math::RigidTransformd expected_mean(
            drake::math::RollPitchYawd(Eigen::Vector3d(390., 0., 0.) * (M_PI / 180.0)), Eigen::Vector3d(2.5, 3.5, 4.5));
...
GTEST_TEST(StochasticSampleTest, TransformTestWithBaseFrame) {
    ...
    drake::RandomGenerator generator(0);
    const math::RigidTransformd sampled_rigidtransformd = transform.Sample(&generator);
    ...
    const Transform sampled_transform = transform_with_base_frame.SampleAsTransform(&generator);

    EXPECT_EQ(sampled_transform.GetDeterministicValue().translation(), sampled_rigidtransformd.translation());
    EXPECT_TRUE(sampled_transform.GetDeterministicValue().rotation().IsNearlyEqualTo(sampled_rigidtransformd.rotation(),
                                                                                     1e-14));
}
