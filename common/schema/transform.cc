...
namespace {
math::RigidTransformd SampleInternal(const Transform& stochastic_transform, RandomGenerator* generator) {
    ...
    const math::RigidTransform<Expression> symbolic_transform = stochastic_transform.ToSymbolic();

    const Vector3<Expression> symbolic_translation = symbolic_transform.translation();
    const Eigen::Vector3d concrete_translation = symbolic::Evaluate(symbolic_translation, {}, generator);

    const math::RotationMatrix<Expression> symbolic_rotation = symbolic_transform.rotation();
    const math::RotationMatrixd concrete_rotation(symbolic::Evaluate(symbolic_rotation.matrix(), {}, generator));
...
math::RigidTransformd Transform::Sample(RandomGenerator* generator) const {
    if (base_frame.has_value() && (*base_frame != "world")) {
        throw std::logic_error(
                fmt::format("Transform::Sample() would discard non-trivial base frame \"{}\"; "
