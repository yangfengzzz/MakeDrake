...
Gaussian::Gaussian(double mean_in, double stddev_in) : mean(mean_in), stddev(stddev_in) {}
...
Uniform::Uniform(double min_in, double max_in) : min(min_in), max(max_in) {}  // NOLINT(build/include_what_you_use)
...
UniformDiscrete::UniformDiscrete(std::vector<double> values_in) : values(std::move(values_in)) {}

UniformDiscrete::~UniformDiscrete() {}

double UniformDiscrete::Sample(drake::RandomGenerator* generator) const {
    if (values.empty()) {
        throw std::logic_error("Cannot Sample() empty UniformDiscrete distribution.");
    }
    const std::vector<double> weights(values.size(), 1.);
    const int index = std::discrete_distribution<int>(weights.begin(), weights.end())(*generator);
...
Expression UniformDiscrete::ToSymbolic() const {
    if (values.empty()) {
        throw std::logic_error("Cannot ToSymbolic() empty UniformDiscrete distribution.");
    }
...
drake::VectorX<Expression> ToSymbolic(const std::vector<DistributionVariant>& vec) {
        ...
        std::visit(
                [](auto&& arg) {
                    ...
                    throw std::logic_error(
                            fmt::format("Attempt to GetDeterministicValue() on a variant that contains a {}",
...
template <int Size>
DeterministicVector<Size>::DeterministicVector(const drake::Vector<double, Size>& value_in) : value(value_in) {}

template <int Size>
DeterministicVector<Size>::~DeterministicVector() {}

template <int Size>
Eigen::VectorXd DeterministicVector<Size>::Sample(drake::RandomGenerator* generator) const {
    unused(generator);
    return value;
}
...
template <int Size>
Eigen::VectorXd GaussianVector<Size>::Sample(drake::RandomGenerator* generator) const {
    if (!(stddev.size() == mean.size() || stddev.size() == 1)) {
        throw std::logic_error(
                fmt::format("Cannot Sample() a GaussianVector distribution with "
                            "size {} mean but size {} dev",
...
drake::VectorX<Expression> GaussianVector<Size>::ToSymbolic() const {
    if (!(stddev.size() == mean.size() || stddev.size() == 1)) {
        throw std::logic_error(
                fmt::format("Cannot ToSymbolic() a GaussianVector distribution with "
                            "size {} mean but size {} dev",
                            mean.size(), stddev.size()));
    }
...
template <int Size>
UniformVector<Size>::UniformVector(const drake::Vector<double, Size>& min_in, const drake::Vector<double, Size>& max_in)
...
template <int Size>
Eigen::VectorXd UniformVector<Size>::Sample(drake::RandomGenerator* generator) const {
    if (min.size() != max.size()) {
        throw std::logic_error(
                fmt::format("Cannot Sample() a UniformVector distribution with "
                            "size {} min but size {} max",
                            min.size(), max.size()));
    }
...
Eigen::VectorXd UniformVector<Size>::Mean() const {
    if (min.size() != max.size()) {
        throw std::logic_error(
                fmt::format("Cannot Mean() a UniformVector distribution with "
                            "size {} min but size {} max",
                            min.size(), max.size()));
    }
    return (min + max) / 2.0;
...
drake::VectorX<Expression> UniformVector<Size>::ToSymbolic() const {
    if (min.size() != max.size()) {
        throw std::logic_error(
                fmt::format("Cannot ToSymbolic() a UniformVector distribution with "
                            "size {} min but size {} max",
                            min.size(), max.size()));
    }
...
template <int Size>
unique_ptr<DistributionVector> ToDistributionVector(const DistributionVectorVariant<Size>& vec) {
    return std::visit<unique_ptr<DistributionVector>>(
            overloaded{
                    ...
                    [](const Gaussian& arg) {
                        return std::make_unique<GaussianVector<Size>>(Eigen::VectorXd::Constant(1, arg.mean),
                                                                      Eigen::VectorXd::Constant(1, arg.stddev));
                    },
                    [](const Uniform& arg) {
                        return std::make_unique<UniformVector<Size>>(Eigen::VectorXd::Constant(1, arg.min),
                                                                     Eigen::VectorXd::Constant(1, arg.max));
                    },
                    []<typename T>(const internal::InvalidVariantSelection<T>&) -> std::nullptr_t {
                        DRAKE_UNREACHABLE();
                    },
            },
            vec);
...
DRAKE_DEFINE_TEMPLATE_INSTANTIATIONS_ON_ALL_SIZES(unique_ptr<DistributionVector> ToDistributionVector)
DRAKE_DEFINE_TEMPLATE_INSTANTIATIONS_ON_ALL_SIZES(bool IsDeterministic)
DRAKE_DEFINE_TEMPLATE_INSTANTIATIONS_ON_ALL_SIZES(Eigen::VectorXd GetDeterministicValue)
