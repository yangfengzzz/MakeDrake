...
private:
    ...
    static constexpr std::enable_if_t<
            std::is_same_v<decltype(std::declval<const U>().Clone()), std::unique_ptr<std::remove_const_t<U>>>,
            bool>
    can_clone(int) {
        return true;
    }
    ...
    static T* CopyOrNull(const T* raw) {
        ...
        static_assert(check_can_clone || check_can_copy,
...
DRAKE_FORMATTER_AS(typename T, drake, copyable_unique_ptr<T>, x, static_cast<const void*>(x.get()))
