...
RunfilesSingleton Create() {
    ...
    } else if ((std::getenv("RUNFILES_MANIFEST_FILE") != nullptr) || (std::getenv("RUNFILES_DIR") != nullptr)) {
    ...
    if (result.runfiles) {
        ...
        if (result.runfiles_dir.empty()) {
            ...
            bazel_error = fmt::format("RUNFILES_DIR '{}' does not exist", result.runfiles_dir);
            result.runfiles.reset();
        }
    }
...
RlocationOrError FindRunfile(const std::string& resource_path) {
    ...
    if (resource_path[0] == '/') {
        result.error = fmt::format("Resource path '{}' must not be an absolute path", resource_path);
        return result;
    }
    ...
    drake::log()->debug("FindRunfile found by-manifest '{}' ({}) and by-directory '{}' ({})", by_man,
                        by_man_ok ? "good" : "bad", by_dir, by_dir_ok ? "good" : "bad");
    ...
    const char* detail{};
    if (!by_man_ok && !by_dir_ok) {
        detail = "but the file does not exist at that location "
                 "nor is it on the manifest";
    } else if (!by_man_ok && by_dir_ok) {
        detail = "and the file exists at that location "
                 "but it is not on the manifest";
    } else {
        DRAKE_DEMAND(by_man_ok && !by_dir_ok);
        detail = "and it is on the manifest"
