...
struct PowCase {
    ...
    PowCase(double base_in, double exp_in) : base(base_in), exp(exp_in) {
        ...
        ill_defined_base_grad = (base == 0.0) || (!std::isfinite(base)) || (!std::isfinite(exp));
        ...
        // ill-defined (i.e., NaN) corresponding element in the grad(result).
        ill_defined_exp_grad = (base <= 0.0) || (!std::isfinite(base)) || (!std::isfinite(exp));
    }
...
std::vector<PowCase> SweepAllCases() {
    ...
    std::initializer_list<double> sweep = {
            -kInf, -2.5, -2.0, -1.5, -1.0, -0.5, -0.0, +0.0, 0.5, 1.0, 1.5, 2.0, 2.5, kInf, kNaN,
    };
...
std::string CalcTestName(const testing::TestParamInfo<PowCase>& info) {
    return fmt::format("{}__{}", ToAlphaNumeric(info.param.base), ToAlphaNumeric(info.param.exp));
}
...
TEST_P(PowSpecial, AdsAds) {
    ...
    EXPECT_THAT(result.value(), testing::NanSensitiveDoubleEq(pow_case.expected_value));
    ...
    if (pow_case.ignore_base_grad) {
        ...
        const double expected = pow_case.exp * std::pow(pow_case.base, pow_case.exp - 1);
...
INSTANTIATE_TEST_SUITE_P(StandardOperationsTest, PowSpecial, testing::ValuesIn(SweepAllCases()), &CalcTestName);
