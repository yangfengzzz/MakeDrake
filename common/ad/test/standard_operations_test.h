...
protected:
    ...
    ::testing::AssertionResult Check(const F& f) {
        ...
        return CompareMatrices(value_and_der_x, value_and_der_3, 10 * std::numeric_limits<double>::epsilon(),
...
#define CHECK_EXPR(expr)                                                                                   \
    EXPECT_TRUE(Check([](const auto& x,                                                                    \
                         const auto& y) -> typename Eigen::internal::remove_reference<decltype(x)>::type { \
        return expr;                                                                                       \
