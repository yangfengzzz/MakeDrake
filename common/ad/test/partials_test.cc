...
TEST_F(PartialsTest, FullCtor) {
    ...
    EXPECT_EQ(dut.size(), 4);
    EXPECT_TRUE(CompareMatrices(dut.make_const_xpr(), Vector4d::LinSpaced(10.0, 13.0)));
}
...
TEST_F(PartialsTest, DivZero) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.make_const_xpr(), Vector4d(kNaN, kNaN, kInf, kNaN)));
}
...
TEST_F(PartialsTest, AddScaled) {
    ...
    EXPECT_TRUE(CompareMatrices(dut.make_const_xpr(), Vector4d::LinSpaced(1.0, 4.0)));
}
...
TEST_F(PartialsTest, AddScaledDifferentSizes) {
    Partials dut{4, 2};
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddScaled(-1.0, Partials(2, 0)), ".*different sizes.*");
}
