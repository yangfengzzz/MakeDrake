...
PathOrError CreateDirectory(fs::path path) {
    ...
    if (ec) {
        return {.error = fmt::format("Could not create {}: {}", path.string(), ec.message())};
    }
...
PathOrError FindOrCreateCache(std::string_view subdir) {
    ...
    if (test_tmpdir.has_value()) {
        ...
        cache_dir = CreateDirectory(fs::path("/private/var/tmp") / fmt::format(".cache_{}", *user));
    } else if (home.has_value()) {
        cache_dir = CreateDirectory(*home / ".cache");
    } else {
        return {.error = "Could not determine an appropriate cache_dir to use. "
