...
[[nodiscard]] bool IsMemcpyMovable(const T& value, const InvariantPred& invariant_pred = InvariantPred()) {
    // 1. Create ptr_to_original via placement-new.
    auto original_storage = std::make_unique<typename std::aligned_storage<sizeof(T), alignof(T)>::type>();
    T* const ptr_to_original{new (original_storage.get()) T{value}};

    // 2. Create ptr_to_moved from ptr_to_original via memcpy.
    auto moved_storage = std::make_unique<typename std::aligned_storage<sizeof(T), alignof(T)>::type>();
