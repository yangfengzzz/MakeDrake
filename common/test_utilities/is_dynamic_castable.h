...
[[nodiscard]] ::testing::AssertionResult is_dynamic_castable(const FromType* ptr) {
    if (ptr == nullptr) {
...
/// fails.
template <typename ToType, typename FromType>
[[nodiscard]] ::testing::AssertionResult is_dynamic_castable(std::shared_ptr<FromType> ptr) {
...
/// fails.
template <typename ToType, typename FromType>
[[nodiscard]] ::testing::AssertionResult is_dynamic_castable(const std::unique_ptr<FromType>& ptr) {
    return is_dynamic_castable<ToType, FromType>(ptr.get());
}
