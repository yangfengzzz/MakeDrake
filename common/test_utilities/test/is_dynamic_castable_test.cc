...
string ExpectedMessage(const string& derived, const string& base, const string& dynamic) {
    return "is_dynamic_castable<" + derived + ">(" + base + "* ptr) failed because ptr is of dynamic type " + dynamic +
           ".";
}
...
TEST_F(IsDynamicCastableTest, DiagnosticMessage) {
    ...
    EXPECT_EQ(assertion_result1.failure_message(), ExpectedMessage(b_typename, a_typename, a_typename));

    const auto assertion_result2 = is_dynamic_castable<C>(a_a_);
    ASSERT_FALSE(assertion_result2);
    EXPECT_EQ(assertion_result2.failure_message(), ExpectedMessage(c_typename, a_typename, a_typename));

    const auto assertion_result3 = is_dynamic_castable<A>(b_b_);
    ASSERT_FALSE(assertion_result3);
    EXPECT_EQ(assertion_result3.failure_message(), ExpectedMessage(a_typename, b_typename, b_typename));

    const auto assertion_result4 = is_dynamic_castable<C>(b_b_);
    ASSERT_FALSE(assertion_result4);
    EXPECT_EQ(assertion_result4.failure_message(), ExpectedMessage(c_typename, b_typename, b_typename));
}
