...
bool IsSupportedConfiguration() {
    static const bool is_supported{DRAKE_INSTALL_HEAP_HOOKS && !std::getenv("LSAN_OPTIONS") &&
...
class Monitor {
public:
    Monitor(const LimitMalloc* owner, LimitMallocParams args) : owner_(owner), args_(std::move(args)) {}
...
void Monitor::ObserveAllocation() {
    ...
    if ((args_.max_num_allocations >= 0) && (observed > args_.max_num_allocations)) {
        failure = true;
    }
    ...
    ActiveMonitor::reset();
    std::cerr << "abort due to malloc #" << observed << " while max_num_allocations = " << args_.max_num_allocations
              << " in effect";
    std::cerr << std::endl;
...
static void EvaluateMinNumAllocations(int observed, int min_num_allocations) {
    ...
    if ((min_num_allocations >= 0) && (observed < min_num_allocations)) {
        std::cerr << "abort due to scope end with " << observed
                  << " mallocs while min_num_allocations = " << min_num_allocations << " in effect";
        std::cerr << std::endl;
        std::abort();
    }
}
