...
[[nodiscard]] ::testing::AssertionResult CompareMatrices(const Eigen::MatrixBase<DerivedA>& m1,
                                                         const Eigen::MatrixBase<DerivedB>& m2,
                                                         double tolerance = 0.0,
                                                         MatrixCompareType compare_type = MatrixCompareType::absolute) {
    if (m1.rows() != m2.rows() || m1.cols() != m2.cols()) {
        const std::string message =
                fmt::format("Matrix size mismatch: ({} x {} vs. {} x {})", m1.rows(), m1.cols(), m2.rows(), m2.cols());
    ...
    for (int ii = 0; ii < m1.rows(); ii++) {
        for (int jj = 0; jj < m1.cols(); jj++) {
            ...
            if ((isnan(m1(ii, jj)) && !isnan(m2(ii, jj))) || (!isnan(m1(ii, jj)) && isnan(m2(ii, jj)))) {
                const std::string message = fmt::format("NaN mismatch at ({}, {}):\nm1 =\n{}\nm2 =\n{}", ii, jj,
            ...
            if (compare_type == MatrixCompareType::absolute) {
                ...
                if (delta > tolerance) {
                    const std::string message = fmt::format(
                            ...
                            "tolerance = {}\nm1 =\n{}\nm2 =\n{}\ndelta=\n{}",
                            ii, jj, m1(ii, jj), m2(ii, jj), delta, tolerance, fmt_eigen(m1), fmt_eigen(m2),
                            fmt_eigen(m1 - m2));
                    return ::testing::AssertionFailure() << message;
                }
            } else {
                ...
                if (delta > relative_tolerance) {
                    const std::string message = fmt::format(
                            ...
                            "=\n{}\ndelta=\n{}",
                            ii, jj, m1(ii, jj), m2(ii, jj), delta, tolerance, relative_tolerance, fmt_eigen(m1),
