...
protected:
    ...
    std::string FormatFirstError() {
        ...
        if (error_records_.empty()) {
            ...
            EXPECT_GT(error_records_.size(), 0) << "FormatFirstError did not get any errors";
            return {};
        }
    ...
    std::string FormatFirstWarning() {
        ...
        if (warning_records_.empty()) {
            EXPECT_TRUE(warning_records_.size() > 0) << "FormatFirstWarning did not get any warnings";
            return {};
        }
    ...
    private:
        const testing::TestInfo* const test_info_{testing::UnitTest::GetInstance()->current_test_info()};
        ::testing::ScopedTrace trace_{test_info_->file(), test_info_->line(),
                                      fmt::format("{}.{}", test_info_->test_suite_name(), test_info_->name())};
    };
