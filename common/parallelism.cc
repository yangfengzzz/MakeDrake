...
int ConfigureMaxNumThreads(const char* const drake_num_threads, const char* const omp_num_threads) {
    ...
    if (drake_num_threads != nullptr) {
        ...
        if (!parsed) {
            ...
            drake::log()->warn(
                    ...
                    kEnvDrakeNumThreads, drake_num_threads, hardware_concurrency, hardware_concurrency);
            return hardware_concurrency;
        } else {
            drake::log()->debug("Initializing max threads to {} from environment variable {}", *parsed,
                                kEnvDrakeNumThreads);
            return *parsed;
        }
...
Parallelism Parallelism::Max() {
    static const int latched_max = internal::ConfigureMaxNumThreads(std::getenv(internal::kEnvDrakeNumThreads),
