...
namespace symbolic {
namespace {
std::map<Variable, int> ToVarToDegreeMap(const Eigen::Ref<const VectorX<Variable>>& vars,
    ...
    for (int i = 0; i < vars.size(); ++i) {
        if (powers.contains(vars[i])) {
            throw std::invalid_argument(fmt::format("PolynomialBasisElement: {} is repeated", vars[i].get_name()));
        }
        if (exponents[i] > 0) {
...
PolynomialBasisElement::PolynomialBasisElement(const std::map<Variable, int>& var_to_degree_map) {
    ...
    for (const auto& p : var_to_degree_map) {
        if (p.second > 0) {
            ...
            throw std::logic_error(fmt::format("The degree for {} is negative.", p.first.get_name()));
        }
        // Ignore the entry if the degree == 0.
    }
}

PolynomialBasisElement::PolynomialBasisElement(const Eigen::Ref<const VectorX<Variable>>& vars,
...
double PolynomialBasisElement::Evaluate(const Environment& env) const {
    return accumulate(var_to_degree_map().begin(), var_to_degree_map().end(), 1.0,
                      [this, &env](const double v, const std::pair<const Variable, int>& p) {
                          const Variable& var{p.first};
                          const auto it = env.find(var);
                          if (it == env.end()) {
                              throw std::invalid_argument(fmt::format("Evaluate: {} is not in env", var.get_name()));
                          } else {
...
bool PolynomialBasisElement::EqualTo(const PolynomialBasisElement& other) const {
    if (var_to_degree_map_.size() != other.var_to_degree_map_.size()) {
        return false;
    }
    for (auto it1 = var_to_degree_map_.begin(), it2 = other.var_to_degree_map_.begin(); it1 != var_to_degree_map_.end();
         ++it1, ++it2) {
...
bool PolynomialBasisElement::lexicographical_compare(const PolynomialBasisElement& other) const {
    ...
    return std::lexicographical_compare(
            var_to_degree_map_.begin(), var_to_degree_map_.end(), other.var_to_degree_map_.begin(),
            other.var_to_degree_map_.end(),
...
void PolynomialBasisElement::DoMergeBasisElementInPlace(const PolynomialBasisElement& other) {
    DRAKE_ASSERT(typeid(*this) == typeid(other));
    auto it1 = this->var_to_degree_map_.begin();
    auto it2 = other.var_to_degree_map_.begin();
    while (it1 != this->var_to_degree_map_.end() && it2 != other.var_to_degree_map_.end()) {
    ...
    while (it2 != other.var_to_degree_map_.end()) {
        this->var_to_degree_map_.insert(this->var_to_degree_map_.end(), std::make_pair(it2->first, it2->second));
        it2++;
    }
