...
namespace drake {
namespace symbolic {
ChebyshevPolynomial::ChebyshevPolynomial(Variable var, int degree) : var_{std::move(var)}, degree_{degree} {
...
void ToPolynomialImpl(Eigen::Matrix<int, Eigen::Dynamic, 3>* monomial_coeffs,
    ...
    monomial_coeffs->block(1, *prev_col_index, monomial_coeffs->rows() - 1, 1) =
            2 * monomial_coeffs->block(0, *next_col_index, monomial_coeffs->rows() - 1, 1);
    (*monomial_coeffs)(0, *prev_col_index) = 0;
    monomial_coeffs->col(*prev_col_index) -= monomial_coeffs->col(*current_col_index);
...
Polynomial ChebyshevPolynomial::ToPolynomial() const {
    if (degree_ == 0) {
        ...
        for (int i = 0; i < degree_ - 2; ++i) {
            ToPolynomialImpl(&monomial_coeffs, &next_col_index, &current_col_index, &prev_col_index);
        }
        Polynomial::MapType monomial_to_coefficient_map;
        for (int i = 0; i <= degree_; ++i) {
            if (monomial_coeffs(i, next_col_index) != 0) {
                monomial_to_coefficient_map.emplace(Monomial(var_, i), monomial_coeffs(i, next_col_index));
            }
        }
...
std::vector<std::pair<ChebyshevPolynomial, double>> ChebyshevPolynomial::Differentiate() const {
    ...
    if (degree_ % 2 == 0) {
        ...
        for (int j = 1; j <= degree_ / 2; ++j) {
            derivative.emplace_back(ChebyshevPolynomial(var_, 2 * j - 1), 2 * degree_);
        }
        return derivative;
    } else {
