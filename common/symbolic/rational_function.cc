...
RationalFunction::RationalFunction() : numerator_{} /* zero polynomial */, denominator_{1} {}

RationalFunction::RationalFunction(Polynomial numerator, Polynomial denominator)
    : numerator_{std::move(numerator)}, denominator_{std::move(denominator)} {
    DRAKE_DEMAND(!denominator_.monomial_to_coefficient_map().empty() /* zero polynomial */);
    DRAKE_ASSERT_VOID(CheckIndeterminates());
}

RationalFunction::RationalFunction(const Polynomial& p) : RationalFunction(p, Polynomial(1)) {}

RationalFunction::RationalFunction(const Monomial& m) : RationalFunction(Polynomial(m)) {}

RationalFunction::RationalFunction(double c) : RationalFunction(Polynomial(c), Polynomial(1)) {}

bool RationalFunction::EqualTo(const RationalFunction& f) const {
    return numerator_.EqualTo(f.numerator()) && denominator_.EqualTo(f.denominator());
}
...
void RationalFunction::CheckIndeterminates() const {
    const Variables vars1{intersect(numerator_.indeterminates(), denominator_.decision_variables())};
    const Variables vars2{intersect(numerator_.decision_variables(), denominator_.indeterminates())};
