...
#include "common/symbolic/polynomial_basis_element.h"
...
public:
    static_assert(std::is_base_of_v<PolynomialBasisElement, BasisElement>,
     ...
     */
    explicit GenericPolynomial(std::nullptr_t) : GenericPolynomial<BasisElement>() {}
    ...
    [[nodiscard]] const Variables& indeterminates() const { return indeterminates_; }

    /** Returns the decision variables of this generic polynomial. */
    [[nodiscard]] const Variables& decision_variables() const { return decision_variables_; }
    ...
    [[nodiscard]] const MapType& basis_element_to_coefficient_map() const { return basis_element_to_coefficient_map_; }
     ...
     * indeterminate.
     */
    [[nodiscard]] GenericPolynomial<BasisElement> Differentiate(const Variable& x) const;
    ...
    Eigen::Matrix<GenericPolynomial<BasisElement>, 1, Derived::RowsAtCompileTime> Jacobian(
        ...
        Eigen::Matrix<GenericPolynomial<BasisElement>, 1, Derived::RowsAtCompileTime> J{n};
     ...
     */
    [[nodiscard]] GenericPolynomial<BasisElement> EvaluatePartial(const Environment& env) const;
     ...
     * evaluation.
     */
    [[nodiscard]] GenericPolynomial<BasisElement> EvaluatePartial(const Variable& var, double c) const;
     ...
     */
    [[nodiscard]] GenericPolynomial<BasisElement> RemoveTermsWithSmallCoefficients(double coefficient_tol) const;
     ...
     * the coefficients. */
    [[nodiscard]] bool EqualToAfterExpansion(const GenericPolynomial<BasisElement>& p) const;
    ...
    friend void hash_append(HashAlgorithm& hasher, const GenericPolynomial<BasisElement>& item) noexcept {
        using drake::hash_append;
        for (const auto& [basis_element, coeff] : item.basis_element_to_coefficient_map_) {
...
using GenericPolynomialEnable =
        std::enable_if_t<std::is_base_of_v<PolynomialBasisElement, BasisElement>, GenericPolynomial<BasisElement>>;
...
GenericPolynomialEnable<BasisElement> pow(const GenericPolynomial<BasisElement>& p, int n) {
    if (n < 0) {
        throw std::runtime_error(fmt::format("pow(): the degree should be non-negative, got {}.", n));
    } else if (n == 0) {
...
std::ostream& operator<<(std::ostream& os, const GenericPolynomial<BasisElement>& p) {
    const typename GenericPolynomial<BasisElement>::MapType& map{p.basis_element_to_coefficient_map()};
    if (map.empty()) {
        return os << 0;
    }
    auto it = map.begin();
...
template <typename BasisElement>
struct hash<drake::symbolic::GenericPolynomial<BasisElement>> : public drake::DefaultHash {};
...
// for details.
template <typename BasisElement>
struct __is_fast_hash<hash<drake::symbolic::GenericPolynomial<BasisElement>>> : std::false_type {};
#endif
}  // namespace std
...
template <>
struct NumTraits<drake::symbolic::GenericPolynomial<drake::symbolic::MonomialBasisElement>>
    : GenericNumTraits<drake::symbolic::GenericPolynomial<drake::symbolic::MonomialBasisElement>> {
    static inline int digits10() { return 0; }
};

template <>
struct NumTraits<drake::symbolic::GenericPolynomial<drake::symbolic::ChebyshevBasisElement>>
    : GenericNumTraits<drake::symbolic::GenericPolynomial<drake::symbolic::ChebyshevBasisElement>> {
    static inline int digits10() { return 0; }
};
}  // namespace Eigen
...
namespace fmt {
template <typename BasisElement>
struct formatter<drake::symbolic::GenericPolynomial<BasisElement>> : drake::ostream_formatter {};
}  // namespace fmt
