...
void AddMonomialsOfDegreeN(const Variables& vars,
                           int degree,
                           const Monomial& b,
...
Eigen::Matrix<Monomial, rows, 1> ComputeMonomialBasis(const Variables& vars,
    ...
    DRAKE_DEMAND((rows == Eigen::Dynamic) || (static_cast<size_t>(rows) == monomials.size()));
...
Eigen::Matrix<Monomial, NChooseK(n + degree, degree), 1> MonomialBasis(const Variables& vars) {
    ...
    return internal::ComputeMonomialBasis<NChooseK(n + degree, degree)>(vars, degree);
}
 ...
 */
[[nodiscard]] VectorX<Monomial> MonomialBasis(const std::unordered_map<Variables, int>& variables_degree);
 ...
 */
[[nodiscard]] VectorX<Monomial> CalcMonomialBasisOrderUpToOne(const Variables& x, bool sort_monomial = false);
}  // namespace symbolic
}  // namespace drake
