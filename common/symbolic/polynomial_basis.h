...
void AddPolynomialBasisElementsOfDegreeN(const Variables& vars,
                                         int degree,
                                         const BasisElement& b,
    ...
    static_assert(std::is_base_of_v<PolynomialBasisElement, BasisElement> || std::is_same_v<BasisElement, Monomial>,
    ...
    for (int var_degree = degree; var_degree >= 0; --var_degree) {
        ...
        AddPolynomialBasisElementsOfDegreeN(vars - var, degree - var_degree, new_b, bin);
    }
}
}  // namespace internal
...
Eigen::Matrix<BasisElement, rows, 1> ComputePolynomialBasisUpToDegree(const Variables& vars,
    ...
    // 1. Collect elements.
    std::set<BasisElement, BasisElementGradedReverseLexOrder<std::less<Variable>, BasisElement>> basis_elements_set;
    int start_degree = 0;
    int degree_stride = 1;
    switch (degree_type) {
    ...
    for (int i = start_degree; i <= degree; i += degree_stride) {
        internal::AddPolynomialBasisElementsOfDegreeN(vars, i, BasisElement{}, &basis_elements_set);
    }
    // 2. Prepare the return value, basis.
    DRAKE_DEMAND((rows == Eigen::Dynamic) || (static_cast<size_t>(rows) == basis_elements_set.size()));
