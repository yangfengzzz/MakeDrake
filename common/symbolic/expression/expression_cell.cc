...
BinaryExpressionCell::BinaryExpressionCell(
        const ExpressionKind k, Expression e1, Expression e2, const bool is_poly, const bool is_expanded)
...
ExpressionVar::ExpressionVar(Variable v) : ExpressionCell{ExpressionKind::Var, true, true}, var_{std::move(v)} {
...
bool ExpressionAdd::EqualTo(const ExpressionCell& e) const {
    ...
    return equal(expr_to_coeff_map_.cbegin(), expr_to_coeff_map_.cend(), add_e.expr_to_coeff_map_.cbegin(),
                 ...
                 [](const pair<const Expression, double>& p1, const pair<const Expression, double>& p2) {
...
ostream& ExpressionAdd::DisplayTerm(ostream& os,
...
ExpressionAddFactory::ExpressionAddFactory(const double constant, map<Expression, double> expr_to_coeff_map)
...
void ExpressionAddFactory::AddMap(const map<Expression, double>& expr_to_coeff_map) {
    for (const auto& p : expr_to_coeff_map) {
        AddTerm(p.second, p.first);
    }
}

ExpressionMul::ExpressionMul(const double constant, map<Expression, Expression> base_to_exponent_map)
...
Expression ExpressionMul::EvaluatePartial(const Environment& env) const {
    return accumulate(base_to_exponent_map_.begin(), base_to_exponent_map_.end(), Expression{constant_},
                      [&env](const Expression& init, const pair<const Expression, Expression>& p) {
...
Expression ExpressionMul::Substitute(const Substitution& s) const {
    return accumulate(base_to_exponent_map_.begin(), base_to_exponent_map_.end(), Expression{constant_},
                      [&s](const Expression& init, const pair<const Expression, Expression>& p) {
...
ostream& ExpressionMul::DisplayTerm(ostream& os,
                                    const bool print_mul,
...
ExpressionMulFactory::ExpressionMulFactory(const double constant, map<Expression, Expression> base_to_exponent_map)
    : is_expanded_{false}, constant_{constant}, base_to_exponent_map_{std::move(base_to_exponent_map)} {}

ExpressionMulFactory::ExpressionMulFactory(const map<Variable, int>& base_to_exponent_map)
...
void ExpressionMulFactory::AddMap(const map<Expression, Expression>& base_to_exponent_map) {
...
private:
    [[nodiscard]] Expression VisitAddition(const Expression& e, const double n) const {
        // e =  (c₀ + ∑ᵢ (cᵢ * eᵢ)) / n
        //   => c₀/n + ∑ᵢ (cᵢ / n * eᵢ)
    ...
    [[nodiscard]] Expression VisitMultiplication(const Expression& e, const double n) const {
        // e =  (c₀ * ∏ᵢ (bᵢ * eᵢ)) / n
        //   => c₀ / n * ∏ᵢ (bᵢ * eᵢ)
    ...
    [[nodiscard]] Expression VisitDivision(const Expression& e, const double n) const {
    ...
    [[nodiscard]] Expression VisitVariable(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitConstant(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitLog(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitPow(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitAbs(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitExp(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitSqrt(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitSin(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitCos(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitTan(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitAsin(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitAcos(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitAtan(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitAtan2(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitSinh(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitCosh(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitTanh(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitMin(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitMax(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitCeil(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitFloor(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitIfThenElse(const Expression& e, const double n) const { return (1.0 / n) * e; }
    [[nodiscard]] Expression VisitUninterpretedFunction(const Expression& e, const double n) const {
        return (1.0 / n) * e;
    }

    // Makes VisitExpression a friend of this class so that VisitExpression can
    // use its private methods.
    friend Expression drake::symbolic::VisitExpression<Expression>(const DivExpandVisitor*,
                                                                   const Expression&,
                                                                   const double&);
};
}  // namespace
...
void ExpressionPow::check_domain(const double v1, const double v2) {
    if (std::isfinite(v1) && (v1 < 0.0) && std::isfinite(v2) && !is_integer(v2)) {
        ostringstream oss;
        oss << "pow(" << v1 << ", " << v2 << ") : numerical argument out of domain. " << v1
            << " is finite negative and " << v2 << " is finite non-integer." << endl;
        throw domain_error(oss.str());
    }
}
...
// --------------------
ExpressionUninterpretedFunction::ExpressionUninterpretedFunction(string name, vector<Expression> arguments)
...
void ExpressionUninterpretedFunction::HashAppendDetail(DelegatingHasher* hasher) const {
    using drake::hash_append;
