...
double Expression::Evaluate(const Environment& env, RandomGenerator* const random_generator) const {
    if (is_constant(*this)) {
    ...
    if (random_generator == nullptr) {
        ...
        return cell().Evaluate(PopulateRandomVariables(env, GetVariables(), random_generator));
    }
}
...
RowVectorX<Expression> Expression::Jacobian(const Eigen::Ref<const VectorX<Variable>>& vars) const {
...
void DoEnumerateMultiIndex(const int order,
                           const int num_vars,
                           const int start_dim,
                           const MultiIndex& base,
...
void DoTaylorExpand(const Expression& f,
                    const Environment& a,
                    const vector<Expression>& terms,
                    const int order,
                    const int num_vars,
                    ExpressionAddFactory* const factory) {
    DRAKE_ASSERT(order > 0);
...
struct GetDistinctVariablesVisitor {
    ...
    void init(const Expression& value, Eigen::Index, Eigen::Index) { variables += value.GetVariables(); }
    // called for all other coefficients
    void operator()(const Expression& value, Eigen::Index, Eigen::Index) { variables += value.GetVariables(); }
    Variables variables;
};
}  // namespace
...
void Gemm<reverse>::CalcDV(const MatrixRef<double>& D,
    ...
    for (int i = 0; i < max_i; ++i) {
        ...
        for (int k = 0; k < max_k; ++k) {
            const Variable& var = !reverse ? V(k, i) : V(i, k);
            auto iter = std::lower_bound(inner_vars.begin(), inner_vars.end(), var, std::less<Variable>{});
