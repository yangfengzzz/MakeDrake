...
public:
     ...
     * @p vars.
     */
    [[nodiscard]] RowVectorX<Expression> Jacobian(const Eigen::Ref<const VectorX<Variable>>& vars) const;
    ...
    friend Expression& operator+=(Expression& lhs, const Expression& rhs) {
        ...
        const double speculative_value = lhs.boxed_.constant_or_nan() + rhs.boxed_.constant_or_nan();
    ...
    friend void swap(Expression& a, Expression& b) { std::swap(a.boxed_, b.boxed_); }
...
template <>
struct numeric_limits<drake::symbolic::Expression> : public std::numeric_limits<double> {};
...
public:
    ...
    explicit uniform_real_distribution(RealType a, RealType b = 1.0)
        : a_{std::move(a)}, b_{std::move(b)}, random_variables_{std::make_shared<std::vector<Variable>>()} {
        if (is_constant(a_) && is_constant(b_) && get_constant_value(a_) > get_constant_value(b_)) {
            throw std::runtime_error(
                    "In constructing a uniform_real_distribution<Expression>, we "
                    "detected that the minimum distribution parameter " +
                    a_.to_string() + " is greater than the maximum distribution parameter " + b_.to_string() + ".");
        }
    }
    ...
    result_type operator()() {
        if (random_variables_->size() == index_) {
            random_variables_->emplace_back("random_uniform_" + std::to_string(index_),
...
private:
    ...
    friend bool operator==(const uniform_real_distribution<drake::symbolic::Expression>& lhs,
                           const uniform_real_distribution<drake::symbolic::Expression>& rhs) {
        return lhs.a().EqualTo(rhs.a()) && lhs.b().EqualTo(rhs.b()) && (lhs.index_ == rhs.index_) &&
               (lhs.random_variables_ == rhs.random_variables_);
    }
};

inline bool operator!=(const uniform_real_distribution<drake::symbolic::Expression>& lhs,
                       const uniform_real_distribution<drake::symbolic::Expression>& rhs) {
    return !(lhs == rhs);
}

// TODO(jwnimmer-tri) Rewrite this as a fmt::formatter specialization.
inline std::ostream& operator<<(std::ostream& os, const uniform_real_distribution<drake::symbolic::Expression>& d) {
    return os << d.a() << " " << d.b();
}
...
public:
    ...
    result_type operator()() {
        if (random_variables_->size() == index_) {
            random_variables_->emplace_back("random_gaussian_" + std::to_string(index_),
    ...
    [[nodiscard]] result_type min() const { return -std::numeric_limits<double>::infinity(); }
    /// Returns the maximum potentially generated value.o
    [[nodiscard]] result_type max() const { return std::numeric_limits<double>::infinity(); }
...
private:
    ...
    friend bool operator==(const normal_distribution<drake::symbolic::Expression>& lhs,
                           const normal_distribution<drake::symbolic::Expression>& rhs) {
        return lhs.mean().EqualTo(rhs.mean()) && lhs.stddev().EqualTo(rhs.stddev()) && (lhs.index_ == rhs.index_) &&
               (lhs.random_variables_ == rhs.random_variables_);
    }
};

inline bool operator!=(const normal_distribution<drake::symbolic::Expression>& lhs,
                       const normal_distribution<drake::symbolic::Expression>& rhs) {
    return !(lhs == rhs);
}

// TODO(jwnimmer-tri) Rewrite this as a fmt::formatter specialization.
inline std::ostream& operator<<(std::ostream& os, const normal_distribution<drake::symbolic::Expression>& d) {
...
public:
    ...
    explicit exponential_distribution(RealType lambda)
        : lambda_{std::move(lambda)}, random_variables_{std::make_shared<std::vector<Variable>>()} {
    ...
    result_type operator()() {
        if (random_variables_->size() == index_) {
            random_variables_->emplace_back("random_exponential_" + std::to_string(index_),
...
private:
    ...
    friend bool operator==(const exponential_distribution<drake::symbolic::Expression>& lhs,
...
inline bool operator!=(const exponential_distribution<drake::symbolic::Expression>& lhs,
                       const exponential_distribution<drake::symbolic::Expression>& rhs) {
    return !(lhs == rhs);
}

// TODO(jwnimmer-tri) Rewrite this as a fmt::formatter specialization.
inline std::ostream& operator<<(std::ostream& os, const exponential_distribution<drake::symbolic::Expression>& d) {
    return os << d.lambda();
}
...
template <>
struct NumTraits<drake::symbolic::Expression> : GenericNumTraits<drake::symbolic::Expression> {
    static inline int digits10() { return 0; }
};

// Informs Eigen that Variable op Variable gets Expression.
template <typename BinaryOp>
struct ScalarBinaryOpTraits<drake::symbolic::Variable, drake::symbolic::Variable, BinaryOp> {
    enum { Defined = 1 };
...
template <typename BinaryOp>
struct ScalarBinaryOpTraits<drake::symbolic::Variable, drake::symbolic::Expression, BinaryOp> {
    enum { Defined = 1 };
    typedef drake::symbolic::Expression ReturnType;
};
...
template <typename BinaryOp>
struct ScalarBinaryOpTraits<drake::symbolic::Expression, drake::symbolic::Variable, BinaryOp> {
    enum { Defined = 1 };
    typedef drake::symbolic::Expression ReturnType;
};
...
struct Gemm {
    ...
    static void CalcDV(const MatrixRef<double>& D, const MatrixRef<Variable>& V, EigenPtr<MatrixX<Expression>> result);
    // Matrix product for Variable, Variable.
    // When reverse == false, sets result to A * B.
    // When reverse == true, sets result to B * A.
    static void CalcVV(const MatrixRef<Variable>& A,
    ...
    static void CalcDE(const MatrixRef<double>& D,
...
class ExpressionInnerProduct : public Expression, public Eigen::Map<Eigen::Matrix<Expression, 1, 1>> {
public:
...
using ExpressionMatMulResult =
        std::conditional_t<MatrixL::RowsAtCompileTime == 1 && MatrixR::ColsAtCompileTime == 1,
                           ExpressionInnerProduct,
                           Eigen::Matrix<Expression, MatrixL::RowsAtCompileTime, MatrixR::ColsAtCompileTime>>;

}  // namespace internal

// Matrix<Expression> * Matrix<double> => Matrix<Expression>
template <typename MatrixL, typename MatrixR>
typename std::enable_if_t<std::is_base_of_v<Eigen::MatrixBase<MatrixL>, MatrixL> &&
...
std::enable_if_t<std::is_same_v<typename Derived::Scalar, Expression>, MatrixLikewise<double, Derived>> Evaluate(
...
typename std::enable_if_t<std::is_base_of_v<Eigen::MatrixBase<DerivedA>, DerivedA> &&
...
struct is_eigen_nonvector_expression_double_pair
    : std::bool_constant<is_eigen_nonvector_of<DerivedV, symbolic::Expression>::value &&
...
struct is_eigen_vector_expression_double_pair
    : std::bool_constant<is_eigen_vector_of<DerivedV, symbolic::Expression>::value &&
