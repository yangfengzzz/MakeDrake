...
//
void CheckHomomorphism(const function<Expression(const Expression&)>& f, const Variable& var, const Expression& expr) {
...
void CheckHomomorphism(const function<Expression(const vector<Expression>&)>& f, const Substitution& s) {
...
void CheckHomomorphism(const function<Formula(const Expression&)>& f, const Variable& var, const Expression& expr) {
...
// Note that the above assertion holds only if f is a quantifier-free
// formula. We have a separate tests which covers the quantified case.
void CheckHomomorphism(const function<Formula(const vector<Expression>&)>& f, const Substitution& s) {
    vector<Expression> args1;  // {x_1, ..., x_n}
    vector<Expression> args2;  // {e_1, ..., e_n}
    for (const pair<const Variable, Expression>& p : s) {
        args1.emplace_back(p.first);
...
TEST_F(SymbolicSubstitutionTest, CheckHomomorphismExpressionSubstitution) {
  ...
  fns.push_back([](const vector<Expression>& v) {
    ...
    substs.push_back({{var_x_, pow(x_, y_)}, {var_y_, sin(y_) + cos(z_)}, {var_z_, sqrt(x_ * y_ * z_)}});
    substs.push_back({{var_x_, pow(x_, y_)}, {var_y_, sin(y_) + cos(z_)}, {var_z_, log(pow(x_, y_) * z_)}});
...
TEST_F(SymbolicSubstitutionTest, UninterpretedFunction) {
    ...
    // = uf2(1.0, x + y)
    EXPECT_PRED2(ExprEqual, uf2.Substitute(s1), uninterpreted_function("uf2", {1.0, x_ + y_}));

    //   (uf2(x, y)).Substitute(x ↦ y, y ↦ z)
    // = uf2(y, z)
    EXPECT_PRED2(ExprEqual, uf2.Substitute(s2), uninterpreted_function("uf2", {y_, z_}));

    //   (uf2(x, y)).Substitute(x ↦ 3.0, y ↦ 4.0)
    // = uf2(3.0, 4.0)
    EXPECT_PRED2(ExprEqual, uf2.Substitute(s3), uninterpreted_function("uf2", {3.0, 4.0}));

    //   (uf2(x, y)).EvaluatePartial(x ↦ 5.0)
    // = uf2(5.0, y)
    EXPECT_PRED2(ExprEqual, uf2.EvaluatePartial({{var_x_, 5.0}}), uninterpreted_function("uf2", {5.0, var_y_}));
}
...
protected:
    ...
    const vector<Formula> formulas_{f1_, f2_, f3_, f4_, f5_, f6_, f7_, f8_, f9_, f10_, f11_, f12_, f13_, f14_};
    ...
    const vector<Formula> forall_formulas_{forall_x_1_,  forall_x_2_,  forall_x_3_,  forall_x_4_, forall_x_5_,
