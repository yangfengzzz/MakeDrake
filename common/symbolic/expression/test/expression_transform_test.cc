...
protected:
    void SetUp() override {
        R1_ = RotationMatrixd::MakeXRotation(.25 * M_PI) * RotationMatrixd::MakeYRotation(.5 * M_PI) *
              RotationMatrixd::MakeZRotation(.33 * M_PI);
        R2_ = RotationMatrixd::MakeXRotation(M_PI / 2) * RotationMatrixd::MakeYRotation(-M_PI / 2) *
...
::testing::AssertionResult CheckMultiplication(const LhsTransform& lhs, const RhsTransform& rhs) {
    ...
    EXPECT_TRUE(CompareMatrices(expected.matrix(), lhs_cast.matrix(), absolute_tolerance));
    EXPECT_TRUE(CompareMatrices(expected.matrix(), rhs_cast.matrix(), absolute_tolerance));
