...
// method numerically.
Expression DifferenceQuotient(const Expression& f, const Variable& x, const Environment& env) {
...
TEST_F(SymbolicDifferentiationTest, SymbolicCompoundCases) {
    ...
    check_list.emplace_back(5 + x + 2 * x * x + 3 * pow(x, 3), var_x_, 1 + 4 * x + 9 * x * x);
    // ∂/∂x (-2 - 3 * x + 6 * x^2 - 4 * x^5) = -3 + 12 x - 20 x^4
    check_list.emplace_back(-2 - 3 * x + 6 * pow(x, 2) - 4 * pow(x, 5), var_x_, -3 + 12 * x - 20 * pow(x, 4));
    // ∂/∂x (3 * (x^3 -x)^3) = 9 * (x^3 -x)^2 * (3x^2 -1)
    check_list.emplace_back(3 * pow(pow(x, 3) - x, 3), var_x_, 9 * pow(pow(x, 3) - x, 2) * (3 * pow(x, 2) - 1));
    // ∂/∂x [(x^3 + 2) / (x^2 - 3)]
    // = [(x^2 - 3) * 3x^2 - (x^3 + 2) * 2x] / (x^2 - 3)^2
    check_list.emplace_back((pow(x, 3) + 2) / (pow(x, 2) - 3), var_x_,
                            ((pow(x, 2) - 3) * 3 * pow(x, 2) - (pow(x, 3) + 2) * 2 * x) / pow(pow(x, 2) - 3, 2));
    // ∂/∂x x^3yz + xy + z + 3 = 3x^2yz + y
    check_list.emplace_back(pow(x, 3) * y * z + x * y + z + 3, var_x_, 3 * pow(x, 2) * y * z + y);
    // ∂/∂y x^3yz + xy + z + 3 = x^3z + x
    check_list.emplace_back(pow(x, 3) * y * z + x * y + z + 3, var_y_, pow(x, 3) * z + x);
    // ∂/∂z x^3yz + xy + z + 3 = 3x^2y + 1
    check_list.emplace_back(pow(x, 3) * y * z + x * y + z + 3, var_z_, pow(x, 3) * y + 1);
    ...
    check_list.emplace_back(x * exp(x * y), var_x_, exp(x * y) + (x * y) * exp(x * y));
    // ∂/∂y x * exp(xy) = x^2 exp(xy)
    check_list.emplace_back(x * exp(x * y), var_y_, pow(x, 2) * exp(x * y));
    // ∂/∂x log(x^2 + 2y) = 2x / (x^2 + y)
    check_list.emplace_back(log(pow(x, 2) + 2 * y), var_x_, 2 * x / (pow(x, 2) + 2 * y));
    // ∂/∂x log(x^2 + 2y) = 2 / (x^2 + 2 * y)
    check_list.emplace_back(log(pow(x, 2) + 2 * y), var_y_, 2 / (pow(x, 2) + 2 * y));
    ...
    check_list.emplace_back(sin(y * cos(x)), var_x_, -y * sin(x) * cos(y * cos(x)));
    // ∂/∂y sin(y * cos(x)) = cos(x) cos(y cos(x))
    check_list.emplace_back(sin(y * cos(x)), var_y_, cos(x) * cos(y * cos(x)));
    // ∂/∂x tan(x + y^2) = 1 / cos^2(x + y^2)
    check_list.emplace_back(tan(x + pow(y, 2)), var_x_, 1 / (pow(cos(x + pow(y, 2)), 2)));
    // ∂/∂y tan(x + y^2) = 2y / cos^2(x + y^2)
    check_list.emplace_back(tan(x + pow(y, 2)), var_y_, 2 * y / (pow(cos(x + pow(y, 2)), 2)));
    // ∂/∂x asin(x + 2y) = 1 / sqrt(1 - (x + y)^2)
    check_list.emplace_back(asin(x + 2 * y), var_x_, 1 / sqrt(1 - pow(x + 2 * y, 2)));
    // ∂/∂y asin(x + 2y) = 2 / sqrt(1 - (x + y)^2)
    check_list.emplace_back(asin(x + 2 * y), var_y_, 2 / sqrt(1 - pow(x + 2 * y, 2)));
    // ∂/∂x atan(3x + 2y) = 3 / ((3x + 2y)^2 + 1)
    check_list.emplace_back(atan(3 * x + 2 * y), var_x_, 3 / (pow(3 * x + 2 * y, 2) + 1));
    // ∂/∂y atan(3x + 2y) = 2 / ((3x + 2y)^2 + 1)
    check_list.emplace_back(atan(3 * x + 2 * y), var_y_, 2 / (pow(3 * x + 2 * y, 2) + 1));
    // ∂/∂x atan2(2x, 3y) = 6y / (4x^2 + 9y^2)
    check_list.emplace_back(atan2(2 * x, 3 * y), var_x_, 6 * y / (pow(2 * x, 2) + pow(3 * y, 2)));
    // ∂/∂y atan2(2x, 3y) = -6x / (4x^2 + 9y^2)
    check_list.emplace_back(atan2(2 * x, 3 * y), var_y_, -6 * x / (pow(2 * x, 2) + pow(3 * y, 2)));
    ...
    check_list.emplace_back(tanh(2 * x + 3 * y), var_x_, 2 / (pow(cosh(2 * x + 3 * y), 2)));
    // ∂/∂y tanh(2x + 3y) = 3 / (cosh^2(2x + 3y))
    check_list.emplace_back(tanh(2 * x + 3 * y), var_y_, 3 / (pow(cosh(2 * x + 3 * y), 2)));

    for (const tuple<Expression, Variable, Expression>& item : check_list) {
        const Expression& expr{get<0>(item)};
        const Variable& var{get<1>(item)};
        const Expression& diff_result{get<2>(item)};
        EXPECT_PRED2(ExprEqual, expr.Differentiate(var).Expand(), diff_result.Expand());
    }
}
...
TEST_F(SymbolicDifferentiationTest, NotDifferentiable) {
    ...
    EXPECT_ANY_THROW(if_then_else(x == 0, y + 1, y).Differentiate(var_x_).Evaluate(env));
    EXPECT_NO_THROW(if_then_else(x == 1, y + 1, y).Differentiate(var_x_).Evaluate(env));
