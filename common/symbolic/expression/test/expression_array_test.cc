...
typename std::enable_if_t<std::is_same_v<typename Eigen::internal::traits<DerivedA>::XprKind, Eigen::ArrayXpr> &&
...
CheckArrayOperatorEq(const ScalarType& v, const Derived& a) {
    const Eigen::Array<Formula, Derived::RowsAtCompileTime, Derived::ColsAtCompileTime> arr = (v == a);
    for (int i = 0; i < arr.rows(); ++i) {
        for (int j = 0; j < arr.cols(); ++j) {
...
CheckArrayOperatorEq(const Derived& a, const ScalarType& v) {
    const Eigen::Array<Formula, Derived::RowsAtCompileTime, Derived::ColsAtCompileTime> arr = (a == v);
    for (int i = 0; i < arr.rows(); ++i) {
        for (int j = 0; j < arr.cols(); ++j) {
...
template <typename DerivedA, typename DerivedB>
typename std::enable_if_t<std::is_same_v<typename Eigen::internal::traits<DerivedA>::XprKind, Eigen::MatrixXpr> &&
...
// whose (i, j) element is a formula a1(i, j) <= a2(i, j).
template <typename DerivedA, typename DerivedB>
typename std::enable_if_t<std::is_base_of_v<Eigen::ArrayBase<DerivedA>, DerivedA> &&
