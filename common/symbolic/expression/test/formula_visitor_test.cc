...
private:
    ...
    Formula Visit(const Formula& f, const bool polarity) const { return VisitFormula<Formula>(this, f, polarity); }
    ...
    Formula VisitGreaterThanOrEqualTo(const Formula& f, const bool polarity) const {
        // NNF(e1 >= e2)    = (e1 >= e2)
        // NNF(¬(e1 >= e2)) = (e1 < e2)
        return polarity ? f : get_lhs_expression(f) < get_rhs_expression(f);
    }
    ...
    Formula VisitPositiveSemidefinite(const Formula& f, const bool polarity) const {
        // NNF(psd(M))  =  psd(M)
        // NNF(¬psd(M)) = ¬psd(M)
        return polarity ? f : !f;
    }
    ...
    // methods.
    friend Formula drake::symbolic::VisitFormula<Formula>(const NegationNormalFormConverter*,
                                                          const Formula&,
                                                          const bool&);
};
...
TEST_F(SymbolicFormulaVisitorTest, NNFNoChanges) {
    // No Changes: True
    EXPECT_PRED2(FormulaEqual, converter_.Visit(Formula::True()), Formula::True());

    // No Changes: False
    EXPECT_PRED2(FormulaEqual, converter_.Visit(Formula::False()), Formula::False());
...
TEST_F(SymbolicFormulaVisitorTest, NNFRelational) {
    // ¬(x ≥ y) ∧ ¬(y ≤ z)  ==>  x < y ∧ y > z
    EXPECT_PRED2(FormulaEqual, converter_.Visit(!(x_ >= y_) && !(y_ <= z_)), x_ < y_ && y_ > z_);
...
TEST_F(SymbolicFormulaVisitorTest, NNFBoolean) {
    // ¬(b₁ ∨ ¬(b₂ ∧ b₃))  ==>  ¬b₁ ∧ b₂ ∧ b₃
    EXPECT_PRED2(FormulaEqual, converter_.Visit(!(b1_ || !(b2_ && b3_))), !b1_ && b2_ && b3_);
