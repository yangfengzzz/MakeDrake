...
TEST_F(SymbolicExpressionCellTest, CastFunctionsConst) {
    ...
    EXPECT_EQ(to_addition(e_add_).get_constant(), get_constant_in_addition(e_add_));
    EXPECT_EQ(to_addition(e_add_).get_expr_to_coeff_map(), get_expr_to_coeff_map_in_addition(e_add_));

    EXPECT_EQ(to_multiplication(e_mul_).get_constant(), get_constant_in_multiplication(e_mul_));
    ...
    EXPECT_PRED2(ExprEqual, to_multiplication(e_mul_).get_base_to_exponent_map().at(var_x_),
                 get_base_to_exponent_map_in_multiplication(e_mul_).at(var_x_));
    EXPECT_PRED2(ExprEqual, to_multiplication(e_mul_).get_base_to_exponent_map().at(var_y_),
                 get_base_to_exponent_map_in_multiplication(e_mul_).at(var_y_));

    EXPECT_EQ(to_division(e_div_).get_second_argument(), get_second_argument(e_div_));
    EXPECT_EQ(to_division(e_div_).get_second_argument(), get_second_argument(e_div_));
    ...
    EXPECT_EQ(to_atan2(e_atan2_).get_first_argument(), get_first_argument(e_atan2_));
    EXPECT_EQ(to_atan2(e_atan2_).get_second_argument(), get_second_argument(e_atan2_));
    ...
    EXPECT_PRED2(FormulaEqual, to_if_then_else(e_ite_).get_conditional_formula(), get_conditional_formula(e_ite_));
    EXPECT_PRED2(ExprEqual, to_if_then_else(e_ite_).get_then_expression(), get_then_expression(e_ite_));
    EXPECT_PRED2(ExprEqual, to_if_then_else(e_ite_).get_else_expression(), get_else_expression(e_ite_));
    EXPECT_EQ(to_uninterpreted_function(e_uf_).get_name(), get_uninterpreted_function_name(e_uf_));
    ...
    EXPECT_EQ(to_uninterpreted_function(e_uf_).get_arguments()[0], get_uninterpreted_function_arguments(e_uf_)[0]);
    EXPECT_EQ(to_uninterpreted_function(e_uf_).get_arguments()[1], get_uninterpreted_function_arguments(e_uf_)[1]);
}

TEST_F(SymbolicExpressionCellTest, CastFunctionsNonConst) {
    EXPECT_EQ(to_variable(Expression{e_var_}).get_variable(), get_variable(e_var_));

    EXPECT_EQ(to_addition(Expression{e_add_}).get_constant(), get_constant_in_addition(e_add_));
    EXPECT_EQ(to_addition(Expression{e_add_}).get_expr_to_coeff_map(), get_expr_to_coeff_map_in_addition(e_add_));

    EXPECT_EQ(to_multiplication(Expression{e_mul_}).get_constant(), get_constant_in_multiplication(e_mul_));
    EXPECT_EQ(to_multiplication(Expression{e_mul_}).get_base_to_exponent_map().size(),
              get_base_to_exponent_map_in_multiplication(e_mul_).size());
    EXPECT_EQ(to_multiplication(Expression{e_mul_}).get_base_to_exponent_map().size(), 2);
    EXPECT_PRED2(ExprEqual, to_multiplication(Expression{e_mul_}).get_base_to_exponent_map().at(var_x_),
                 get_base_to_exponent_map_in_multiplication(e_mul_).at(var_x_));
    EXPECT_PRED2(ExprEqual, to_multiplication(Expression{e_mul_}).get_base_to_exponent_map().at(var_y_),
                 get_base_to_exponent_map_in_multiplication(e_mul_).at(var_y_));

    EXPECT_EQ(to_division(Expression{e_div_}).get_second_argument(), get_second_argument(e_div_));
    EXPECT_EQ(to_division(Expression{e_div_}).get_second_argument(), get_second_argument(e_div_));
    ...
    EXPECT_EQ(to_pow(Expression{e_pow_}).get_first_argument(), get_first_argument(e_pow_));
    EXPECT_EQ(to_pow(Expression{e_pow_}).get_second_argument(), get_second_argument(e_pow_));
    ...
    EXPECT_EQ(to_atan2(Expression{e_atan2_}).get_first_argument(), get_first_argument(e_atan2_));
    EXPECT_EQ(to_atan2(Expression{e_atan2_}).get_second_argument(), get_second_argument(e_atan2_));
    ...
    EXPECT_EQ(to_min(Expression{e_min_}).get_first_argument(), get_first_argument(e_min_));
    EXPECT_EQ(to_min(Expression{e_min_}).get_second_argument(), get_second_argument(e_min_));
    EXPECT_EQ(to_max(Expression{e_max_}).get_first_argument(), get_first_argument(e_max_));
    EXPECT_EQ(to_max(Expression{e_max_}).get_second_argument(), get_second_argument(e_max_));
    EXPECT_EQ(to_ceil(Expression{e_ceil_}).get_argument(), get_argument(e_ceil_));
    EXPECT_EQ(to_floor(Expression{e_floor_}).get_argument(), get_argument(e_floor_));
    EXPECT_PRED2(FormulaEqual, to_if_then_else(Expression{e_ite_}).get_conditional_formula(),
                 get_conditional_formula(e_ite_));
    EXPECT_PRED2(ExprEqual, to_if_then_else(Expression{e_ite_}).get_then_expression(), get_then_expression(e_ite_));
    EXPECT_PRED2(ExprEqual, to_if_then_else(Expression{e_ite_}).get_else_expression(), get_else_expression(e_ite_));
    EXPECT_EQ(to_uninterpreted_function(Expression{e_uf_}).get_name(), get_uninterpreted_function_name(e_uf_));
    EXPECT_EQ(to_uninterpreted_function(Expression{e_uf_}).get_arguments().size(), 2);
