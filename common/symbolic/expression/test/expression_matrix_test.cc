...
TEST_F(SymbolicExpressionMatrixTest, GetVariableVector) {
    ...
    EXPECT_THROW(GetVariableVector(Vector3<Expression>(x_, y_, x_ + y_)), std::logic_error);
}
...
TEST_F(SymbolicExpressionMatrixTest, EigenMul5) {
  ...
  M_expected << x_ * y_ + x_ * y_, x_ * y_ + x_ * z_,
    ...
    // clang-format on
    EXPECT_TRUE(CheckStructuralEquality(M, M_expected)) << fmt::to_string(fmt_eigen(M));
}
...
bool CheckMatrixOperatorEq(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
...
//   - m1 != m2
//   - ⋁ᵢⱼ (m1.array() != m2.array())
bool CheckMatrixOperatorNeq(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    const Formula f1{m1 != m2};
...
//   - m1 < m2
//   - ⋀ᵢⱼ (m1.array() < m2.array())
bool CheckMatrixOperatorLt(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    const Formula f1{m1 < m2};
    const Formula f2{(m1.array() < m2.array()).redux(internal::logic_and)};
    return f1.EqualTo(f2);
}
...
//   - m1 <= m2
//   - ⋀ᵢⱼ (m1.array() <= m2.array())
bool CheckMatrixOperatorLte(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    const Formula f1{m1 <= m2};
    const Formula f2{(m1.array() <= m2.array()).redux(internal::logic_and)};
    return f1.EqualTo(f2);
}
...
//   - m1 > m2
//   - ⋀ᵢⱼ (m1.array() > m2.array())
bool CheckMatrixOperatorGt(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    const Formula f1{m1 > m2};
    const Formula f2{(m1.array() > m2.array()).redux(internal::logic_and)};
    return f1.EqualTo(f2);
}
...
//   - m1 >= m2
//   - ⋀ᵢⱼ (m1.array() >= m2.array())
bool CheckMatrixOperatorGte(const MatrixX<Expression>& m1, const MatrixX<Expression>& m2) {
    const Formula f1{m1 >= m2};
    const Formula f2{(m1.array() >= m2.array()).redux(internal::logic_and)};
    return f1.EqualTo(f2);
}
...
TEST_F(SymbolicExpressionMatrixTest, MatrixOperatorVarEqdouble) {
    ...
    const Formula f1{m1 == m2};
    const Formula f2{m2 == m1};
    EXPECT_PRED2(FormulaEqual, f1, (var_x_ == 1.0) && (var_y_ == 2.0) && (var_z_ == 3.0));
    EXPECT_PRED2(FormulaEqual, f2, (1.0 == var_x_) && (2.0 == var_y_) && (3.0 == var_z_));
}
...
TEST_F(SymbolicExpressionMatrixTest, MatrixOperatorVarEqVar) {
    ...
    const Formula f1{m1 == m2};
    const Formula f2{m2 == m1};
    EXPECT_PRED2(FormulaEqual, f1, (var_x_ == var_y_) && (var_y_ == var_z_) && (var_z_ == var_x_));
    EXPECT_PRED2(FormulaEqual, f2, (var_y_ == var_x_) && (var_z_ == var_y_) && (var_x_ == var_z_));
}
...
TEST_F(SymbolicExpressionMatrixTest, EvaluateDenseMatrix) {
  ...
  C << x_, Expression::NaN(),
       y_, z_;
    // clang-format on
    DRAKE_EXPECT_THROWS_MESSAGE(Evaluate(C, env), "NaN is detected during Symbolic computation.");
}
...
const char* kBadMatrixInversion = R"""(.*does not have an entry for the variable[\s\S]*)""";
...
void CheckSymbolicMatrixInversion() {
    ...
    EXPECT_TRUE(CompareMatrices(Substitute(Minv, subst), Substitute(M, subst).inverse(), 1e-10));
...
TEST_F(SymbolicExpressionMatrixTest, Inverse) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(CheckSymbolicMatrixInversion<5>(), kBadMatrixInversion);
}
...
void CheckNumericExpressionMatrixInversion() {
    ...
    // Dynamically sized.
    EXPECT_TRUE(CompareMatrices(Eigen::MatrixXd(M_f).inverse(), MatrixX<Expression>(M_sym).inverse(), 1e-9));
}
