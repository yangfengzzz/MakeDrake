        ...
        for (size_t j{0}; j < formulas.size(); ++j) {
            if (i < j) {
                EXPECT_PRED2(FormulaLess, formulas[i], formulas[j])
                        << "(Formulas[" << i << "] = " << formulas[i] << ")" << " is not less than " << "(Formulas["
                        << j << "] = " << formulas[j] << ")";
                EXPECT_PRED2(FormulaNotLess, formulas[j], formulas[i])
                        << "(Formulas[" << j << "] = " << formulas[j] << ")" << " is less than " << "(Formulas[" << i
                        << "] = " << formulas[i] << ")";
            } else if (i > j) {
                EXPECT_PRED2(FormulaLess, formulas[j], formulas[i])
                        << "(Formulas[" << j << "] = " << formulas[j] << ")" << " is not less than " << "(Formulas["
                        << i << "] = " << formulas[i] << ")";
                EXPECT_PRED2(FormulaNotLess, formulas[i], formulas[j])
                        << "(Formulas[" << i << "] = " << formulas[i] << ")" << " is less than " << "(Formulas[" << j
                        << "] = " << formulas[j] << ")";
            } else {
                // i == j
                EXPECT_PRED2(FormulaNotLess, formulas[i], formulas[j])
...
TEST_F(SymbolicFormulaTest, And1) {
    ...
    EXPECT_PRED2(FormulaEqual, make_conjunction({f1_ && f2_, f3_ && f4_}), make_conjunction({f1_, f2_, f3_, f4_}));
...
TEST_F(SymbolicFormulaTest, And3) {
    // Flattening
    EXPECT_PRED2(FormulaEqual, f1_ && f2_ && f3_ && f4_, f1_ && f2_ && f3_ && f4_);
    EXPECT_PRED2(FormulaEqual, (f1_ && f2_) && (f3_ && f4_), f1_ && f2_ && f3_ && f4_);
    EXPECT_PRED2(FormulaEqual, f1_ && (f2_ && f3_) && f4_, f1_ && f2_ && f3_ && f4_);
    EXPECT_PRED2(FormulaEqual, f1_ && ((f2_ && f3_) && f4_), f1_ && f2_ && f3_ && f4_);
    // Remove duplicate
...
TEST_F(SymbolicFormulaTest, And4) {
    ...
    for (const Formula& f : {b1_, b2_, tt_, ff_, f1_, f2_, f3_, f4_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_,
...
TEST_F(SymbolicFormulaTest, Or1) {
    ...
    // Checks if flattening works: (f₁ ∨ f₂) ∨ (f₃ ∨ f₄) => f₁ ∨ f₂ ∨ f₃ ∨ f₄.
    EXPECT_PRED2(FormulaEqual, make_disjunction({f1_ || f2_, f3_ || f4_}), make_disjunction({f1_, f2_, f3_, f4_}));
...
TEST_F(SymbolicFormulaTest, ToString) {
    ...
    EXPECT_EQ(f_forall_.to_string(), "forall({x, y}. (((x + y) > 0) or ((x * y) < 5)))");
    EXPECT_EQ(f_isnan_.to_string(), "isnan(NaN)");
}

TEST_F(SymbolicFormulaTest, IsTrue) {
    EXPECT_TRUE(is_true(tt_));
    EXPECT_FALSE(any_of({ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_,
                         f_isnan_, f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_true));
}

TEST_F(SymbolicFormulaTest, IsFalse) {
    EXPECT_TRUE(is_false(ff_));
    EXPECT_FALSE(any_of({tt_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_,
                         f_isnan_, f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_false));
}
...
TEST_F(SymbolicFormulaTest, IsEqualTo) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
...
TEST_F(SymbolicFormulaTest, IsNotEqualTo) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_not_equal_to));
}
...
TEST_F(SymbolicFormulaTest, IsLessThan) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_less_than));
}
...
TEST_F(SymbolicFormulaTest, IsLessThanOrEqualTo) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
...
TEST_F(SymbolicFormulaTest, IsGreaterThan) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_greater_than));
}
...
TEST_F(SymbolicFormulaTest, IsGreaterThanOrEqualTo) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_greater_than_or_equal_to));
}

TEST_F(SymbolicFormulaTest, IsRelational) {
    EXPECT_TRUE(all_of({f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_}, is_relational));
    EXPECT_FALSE(any_of({tt_, ff_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_, f_psd_static_2x2_, f_psd_dynamic_2x2_,
                         f_psd_static_3x3_},
                        is_relational));
}
...
TEST_F(SymbolicFormulaTest, IsConjunction) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_or_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_conjunction));
}
...
TEST_F(SymbolicFormulaTest, IsDisjunction) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_disjunction));
}
...
TEST_F(SymbolicFormulaTest, IsNary) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, not_f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_nary));
}
...
TEST_F(SymbolicFormulaTest, IsNegation) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, f_forall_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_negation));
}
...
TEST_F(SymbolicFormulaTest, IsForall) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_isnan_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_forall));
}
...
TEST_F(SymbolicFormulaTest, IsIsnan) {
    ...
    EXPECT_FALSE(any_of({tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_,
                         f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_},
                        is_isnan));
...
TEST_F(SymbolicFormulaTest, IsPositiveSemidefinite) {
    EXPECT_TRUE(all_of({f_psd_static_2x2_, f_psd_dynamic_2x2_, f_psd_static_3x3_}, is_positive_semidefinite));
    EXPECT_FALSE(any_of(
            {tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_, f_forall_, f_isnan_},
...
TEST_F(SymbolicFormulaTest, GetMatrixInPsdNonSymmetricStatic) {
  ...
  sym_from_upper << 1.0, 2.0, 3.0,
    ...
    EXPECT_TRUE(CheckStructuralEquality(get_matrix_in_positive_semidefinite(positive_semidefinite(m, Eigen::Lower)),
                                        sym_from_lower));

    EXPECT_TRUE(CheckStructuralEquality(
            get_matrix_in_positive_semidefinite(positive_semidefinite(m.triangularView<Eigen::Lower>())),
            sym_from_lower));

    EXPECT_TRUE(CheckStructuralEquality(get_matrix_in_positive_semidefinite(positive_semidefinite(m, Eigen::Upper)),
                                        sym_from_upper));

    EXPECT_TRUE(CheckStructuralEquality(
            get_matrix_in_positive_semidefinite(positive_semidefinite(m.triangularView<Eigen::Upper>())),
            sym_from_upper));
}
...
TEST_F(SymbolicFormulaTest, MemcpyKeepsFomrulaIntact) {
    for (const Formula& formula : {tt_, ff_, f_eq_, f_neq_, f_lt_, f_lte_, f_gt_, f_gte_, f_and_, f_or_, not_f_or_,
...
// +---------------------+                             +--------------------+
::testing::AssertionResult CheckFormulaWithRandomVariables(const Formula& f,
    ...
    const Environment env_extended{PopulateRandomVariables(env, f.GetFreeVariables(), &random_generator_copy)};
