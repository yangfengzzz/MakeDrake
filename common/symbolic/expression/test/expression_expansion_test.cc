...
protected:
    ...
    // invariant holds.
    bool CheckAlreadyExpanded(const Expression& e) { return e.is_expanded() && e.EqualTo(e.Expand()); }
    ...
    bool CheckUnchangedExpand(const Expression& e) { return !e.is_expanded() && e.EqualTo(e.Expand()); }

    // Checks if e.Expand() == e.Expand().Expand().
    bool CheckExpandIsFixpoint(const Expression& e) { return e.Expand().EqualTo(e.Expand().Expand()); }
};
...
TEST_F(SymbolicExpansionTest, ExpressionExpansion) {
    ...
    //   3x^2 + 3xy^2 + 6xy + 3x +
    //   y^3 + 3y^2 + 3y + 1
    test_exprs.emplace_back(pow(x_ + y_ + 1, 3), pow(x_, 3) + 3 * pow(x_, 2) * y_ + 3 * pow(x_, 2) +
                                                         3 * x_ * pow(y_, 2) + 6 * x_ * y_ + 3 * x_ + pow(y_, 3) +
                                                         3 * pow(y_, 2) + 3 * y_ + 1);
    ...
    test_exprs.emplace_back(pow(x_ + y_ + 1, 4), 1 + 4 * x_ + 4 * y_ + 12 * x_ * y_ + 12 * x_ * pow(y_, 2) +
                                                         ...
                                                         6 * pow(x_, 2) * pow(y_, 2) + 4 * pow(x_, 3) * y_ +
                                                         6 * pow(x_, 2) + 4 * pow(x_, 3) + pow(x_, 4) + 6 * pow(y_, 2) +
                                                         4 * pow(y_, 3) + pow(y_, 4));
