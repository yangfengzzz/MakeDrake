...
public:
    ...
    [[nodiscard]] bool equal_to(const Variable& v) const { return get_id() == v.get_id(); }

    /// Compares two variables based on their ID values.
    [[nodiscard]] bool less(const Variable& v) const { return get_id() < v.get_id(); }

    /** Implements the @ref hash_append concept. */
    template <class HashAlgorithm>
    friend void hash_append(HashAlgorithm& hasher, const Variable& item) noexcept {
...
Eigen::Matrix<Variable, rows, cols> MakeMatrixVariable(const std::string& name,
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            m(i, j) = Variable{name + "(" + std::to_string(i) + ", " + std::to_string(j) + ")", type};
        }
    }
    return m;
}
...
struct less<drake::symbolic::Variable> {
    bool operator()(const drake::symbolic::Variable& lhs, const drake::symbolic::Variable& rhs) const {
        return lhs.less(rhs);
    }
};
...
template <>
struct NumTraits<drake::symbolic::Variable> : GenericNumTraits<drake::symbolic::Variable> {
