...
void DoCompute(const Ref<const MatrixXE>& a,
               MatrixXE* matrix,
               E* l1_norm,
...
#define DRAKE_DEFINE_SPECIALIZE_LDLT(SomeMatrix)                                        \
    template <>                                                                         \
    template <>                                                                         \
    Eigen::LDLT<SomeMatrix>& Eigen::LDLT<SomeMatrix>::compute<Ref<const SomeMatrix>>(   \
            const EigenBase<Ref<const SomeMatrix>>& a) {                                \
        MatrixXE matrix;                                                                \
        Transpositions<Dynamic> transpositions;                                         \
        DoCompute(a.derived(), &matrix, &m_l1_norm, &transpositions, &m_sign, &m_info); \
        m_matrix = matrix;                                                              \
