...
public:
    ...
    explicit BoxedCell(double constant) : value_(constant) { DRAKE_ASSERT(!std::isnan(constant)); }
    ...
    friend void swap(BoxedCell& a, BoxedCell& b) { std::swap(a.value_, b.value_); }
    ...
    [[nodiscard]] bool trivially_equals(const BoxedCell& other) const { return value_bits() == other.value_bits(); }
...
private:
    ...
    std::uintptr_t pointer_bits() const { return value_bits() & ~(0xFFFFull << 48); }
