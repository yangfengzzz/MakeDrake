...
Formula::Formula(const Variable& var) : ptr_{make_shared<const FormulaVar>(var)} {}
...
bool Formula::Evaluate(const Environment& env, RandomGenerator* const random_generator) const {
    DRAKE_ASSERT(ptr_ != nullptr);
    if (random_generator == nullptr) {
        return ptr_->Evaluate(env);
    } else {
        return ptr_->Evaluate(PopulateRandomVariables(env, GetFreeVariables(), random_generator));
    }
}
...
Formula positive_semidefinite(const MatrixX<Expression>& m, const Eigen::UpLoType mode) {
    switch (mode) {
        case Eigen::Lower:
            return Formula{make_shared<const FormulaPositiveSemidefinite>(m.triangularView<Eigen::Lower>())};
        case Eigen::Upper:
            return Formula{make_shared<const FormulaPositiveSemidefinite>(m.triangularView<Eigen::Upper>())};
        default:
