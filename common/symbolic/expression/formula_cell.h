...
public:
    ...
    [[nodiscard]] const std::set<Formula>& get_operands() const { return formulas_; }
...
public:
    ...
    [[nodiscard]] const Variables& get_quantified_variables() const { return vars_; }
...
public:
     ...
     */
    explicit FormulaPositiveSemidefinite(const Eigen::Ref<const MatrixX<Expression>>& m);

    /** Constructs a symbolic positive-semidefinite formula from a
     * lower triangular-view @p l.
     */
    template <typename Derived>
    explicit FormulaPositiveSemidefinite(const Eigen::TriangularView<Derived, Eigen::Lower>& l)
        : FormulaCell{FormulaKind::PositiveSemidefinite}, m_{BuildSymmetricMatrixFromLowerTringularView(l)} {}

    /** Constructs a symbolic positive-semidefinite formula from an
     * upper triangular-view @p u.
     */
    template <typename Derived>
    explicit FormulaPositiveSemidefinite(const Eigen::TriangularView<Derived, Eigen::Upper>& u)
        : FormulaCell{FormulaKind::PositiveSemidefinite}, m_{BuildSymmetricMatrixFromUpperTriangularView(u)} {}
    ...
    [[nodiscard]] const MatrixX<symbolic::Expression>& get_matrix() const { return m_; }
...
private:
    ...
    static MatrixX<Expression> BuildSymmetricMatrixFromLowerTringularView(
        ...
        m.triangularView<Eigen::StrictlyUpper>() = m.transpose().triangularView<Eigen::StrictlyUpper>();
        return m;
    }
