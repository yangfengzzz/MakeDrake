...
ChebyshevBasisElement::ChebyshevBasisElement(const std::map<Variable, int>& var_to_degree_map)
    : PolynomialBasisElement(var_to_degree_map) {}

ChebyshevBasisElement::ChebyshevBasisElement(const Variable& var) : ChebyshevBasisElement({{var, 1}}) {}

ChebyshevBasisElement::ChebyshevBasisElement(const Variable& var, int degree)
    : ChebyshevBasisElement({{var, degree}}) {}

ChebyshevBasisElement::ChebyshevBasisElement(std::nullptr_t) : ChebyshevBasisElement() {}

ChebyshevBasisElement::ChebyshevBasisElement(const Eigen::Ref<const VectorX<Variable>>& vars,
                                             const Eigen::Ref<const Eigen::VectorXi>& degrees)
    : PolynomialBasisElement(vars, degrees) {}

bool ChebyshevBasisElement::operator<(const ChebyshevBasisElement& other) const {
    return this->lexicographical_compare(other);
}

std::map<ChebyshevBasisElement, double> ChebyshevBasisElement::Differentiate(const Variable& var) const {
...
std::map<ChebyshevBasisElement, double> ChebyshevBasisElement::Integrate(const Variable& var) const {
    ...
    std::map<ChebyshevBasisElement, double> result;
    it->second = degree + 1;
    result.emplace(ChebyshevBasisElement(var_to_degree_map), 1.0 / (2 * degree + 2));
    it->second = degree - 1;
    result.emplace(ChebyshevBasisElement(var_to_degree_map), -1.0 / (2 * degree - 2));
    return result;
}

void ChebyshevBasisElement::MergeBasisElementInPlace(const ChebyshevBasisElement& other) {
    this->DoMergeBasisElementInPlace(other);
}

std::pair<double, ChebyshevBasisElement> ChebyshevBasisElement::EvaluatePartial(const Environment& env) const {
    double coeff;
...
double ChebyshevBasisElement::DoEvaluate(double variable_val, int degree) const {
...
Expression ChebyshevBasisElement::DoToExpression() const {
    ...
    for (const auto& [var, degree] : var_to_degree_map()) {
        base_to_exponent_map.emplace(ChebyshevPolynomial(var, degree).ToPolynomial().ToExpression(), 1);
    }
...
void AppendVariableAndDegree(const Variable& var,
                             int degree,
...
std::map<ChebyshevBasisElement, double> operator*(const ChebyshevBasisElement& a, const ChebyshevBasisElement& b) {
    ...
    // variables.
    while (it_a != a.var_to_degree_map().end() && it_b != b.var_to_degree_map().end()) {
    ...
    std::vector<std::map<Variable, int>> chebyshev_basis_all(power_of_2(num_common_variables));
    ...
    while (it_a != a.var_to_degree_map().end() && it_b != b.var_to_degree_map().end()) {
        ...
        if (var_a.less(var_b)) {
            ...
            for (int j = 0; j < power_of_2(num_common_variables - common_variables_count - 1); ++j) {
