#include "common/symbolic/simplification.h"
...
public:
    ...
    [[nodiscard]] std::optional<Substitution> Unify(const Pattern& p, const Expression& e) const {
        Substitution subst;
        if (Unify(p, e, &subst)) {
            return subst;
        } else {
...
private:
    ...
    bool Unify(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitExpression<bool>(this, p, e, subst);
    }

    bool VisitVariable(const Pattern& p, const Expression& e, Substitution* const subst) const {
    ...
    bool VisitConstant(const Pattern& p, const Expression& e, Substitution* const) const {
    ...
    // approach.
    bool VisitAddition(const Pattern& p, const Expression& e, Substitution* const subst) const {
        if (!is_addition(e)) {
            return false;
        }
    ...
    //
    bool VisitMultiplication(const Pattern& p, const Expression& e, Substitution* const subst) const {
    ...
    bool VisitMultiplicationAligned(const double c_p,
        ...
        return VisitMultiplicationCheckPairs(map_p.begin(), map_e.begin(), n, m, subst);
    }
    ...
    //  - Unify the last element of p with the rest of elements in e. That is,
    //    Unify(pow(bₙ, tₙ), pow(b'ₙ, t'ₙ) * ... * pow(b'ₘ, t'ₘ)).
    bool VisitMultiplicationCheckPairs(map<Expression, Expression>::const_iterator it_p,
                                       map<Expression, Expression>::const_iterator it_e,
                                       const int n,
                                       const int m,
                                       Substitution* const subst) const {
    ...
    bool VisitBinary(const function<bool(const Expression&)>& pred,
                     ...
                     const Expression& e,
                     Substitution* const subst) const {
        return pred(e) && Unify(get_first_argument(p), get_first_argument(e), subst) &&
               Unify(get_second_argument(p), get_second_argument(e), subst);
    }

    bool VisitPow(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitBinary(&is_pow, p, e, subst);
    }

    bool VisitDivision(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitBinary(&is_division, p, e, subst);
    }

    bool VisitAbs(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_abs, p, e, subst);
    }

    bool VisitLog(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_log, p, e, subst);
    }

    bool VisitExp(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_exp, p, e, subst);
    }

    bool VisitSqrt(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_sqrt, p, e, subst);
    }

    bool VisitSin(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_sin, p, e, subst);
    }

    bool VisitCos(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_cos, p, e, subst);
    }

    bool VisitTan(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_tan, p, e, subst);
    }

    bool VisitAsin(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_asin, p, e, subst);
    }

    bool VisitAcos(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_acos, p, e, subst);
    }

    bool VisitAtan(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_atan, p, e, subst);
    }

    bool VisitAtan2(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitBinary(&is_atan2, p, e, subst);
    }

    bool VisitSinh(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_sinh, p, e, subst);
    }

    bool VisitCosh(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_cosh, p, e, subst);
    }

    bool VisitTanh(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_tanh, p, e, subst);
    }

    bool VisitMin(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitBinary(&is_min, p, e, subst);
    }

    bool VisitMax(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitBinary(&is_max, p, e, subst);
    }

    bool VisitCeil(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_ceil, p, e, subst);
    }

    bool VisitFloor(const Pattern& p, const Expression& e, Substitution* const subst) const {
        return VisitUnary(&is_floor, p, e, subst);
    }

    bool VisitIfThenElse(const Pattern&, const Expression&, Substitution* const) const {
        // TODO(soonho): Support this.
        throw runtime_error("Unification algorithm does not support if-then-else-expressions, yet");
    }

    bool VisitUninterpretedFunction(const Pattern&, const Expression&, Substitution* const) const {
        // TODO(soonho): Support this.
        throw runtime_error("Unification algorithm does not support uninterpreted functions, yet");
    }

    // Makes VisitExpression a friend of this class so that it can use private
    // methods.
    friend bool VisitExpression<bool>(const UnificationVisitor*,
                                      const Pattern&,
                                      const Expression&,
                                      Substitution* const&);
};
