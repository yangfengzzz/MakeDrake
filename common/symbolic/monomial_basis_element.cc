...
std::map<Variable, int> ToMonomialPower(const Expression& e) {
    ...
    if (is_one(e)) {  // This block is deliberately left empty.
        ...
        throw std::runtime_error("A constant not equal to 1, this is not a monomial.");
        ...
        throw std::runtime_error("This expression cannot be converted to a monomial.");
    }
    return powers;
}
...
MonomialBasisElement::MonomialBasisElement(const std::map<Variable, int>& var_to_degree_map)
    : PolynomialBasisElement(var_to_degree_map) {}

MonomialBasisElement::MonomialBasisElement(const Eigen::Ref<const VectorX<Variable>>& vars,
                                           const Eigen::Ref<const Eigen::VectorXi>& degrees)
    : PolynomialBasisElement(vars, degrees) {}

MonomialBasisElement::MonomialBasisElement(const Variable& var) : MonomialBasisElement({{var, 1}}) {}

MonomialBasisElement::MonomialBasisElement(const Variable& var, int degree) : MonomialBasisElement({{var, degree}}) {}

MonomialBasisElement::MonomialBasisElement() : PolynomialBasisElement() {}

MonomialBasisElement::MonomialBasisElement(const Expression& e) : MonomialBasisElement(ToMonomialPower(e.Expand())) {}

bool MonomialBasisElement::operator<(const MonomialBasisElement& other) const {
    return this->lexicographical_compare(other);
}

std::pair<double, MonomialBasisElement> MonomialBasisElement::EvaluatePartial(const Environment& env) const {
    double coeff{};
...
MonomialBasisElement& MonomialBasisElement::pow_in_place(const int p) {
    if (p < 0) {
        ...
        oss << "MonomialBasisElement::pow(int p) is called with a negative p = " << p;
...
std::map<MonomialBasisElement, double> MonomialBasisElement::Differentiate(const Variable& var) const {
...
std::map<MonomialBasisElement, double> MonomialBasisElement::Integrate(const Variable& var) const {
...
std::vector<std::pair<int, double>> UnivariateMonomialToChebyshevBasis(int degree) {
    ...
    for (int k = 1; k < half_n + 1; ++k) {
        ...
        double new_coeff = result[k - 1].second * static_cast<double>(degree - k + 1) / static_cast<double>(k);
        if (2 * k == degree) {
            new_coeff /= 2;
        }
...
std::map<ChebyshevBasisElement, double> MonomialToChebyshevBasisRecursive(std::map<Variable, int> var_to_degree_map) {
    ...
    for (const auto& [degree_x, coeff_x] : first_univariate_in_chebyshev) {
        for (const auto& [remaining_vars, coeff_remaining_vars] : remaining_chebyshevs) {
            std::map<Variable, int> new_chebyshev_var_to_degree_map = remaining_vars.var_to_degree_map();
            // Multiply Tᵢ(x) to each term Tⱼ(y)Tₖ(z) to get the new
            // ChebyshevBasisElement.
            new_chebyshev_var_to_degree_map.emplace_hint(new_chebyshev_var_to_degree_map.end(), var_first, degree_x);
            // Multiply cᵢ to dⱼₖ to get the new coefficient.
            const double new_coeff = coeff_remaining_vars * coeff_x;
            result.emplace(ChebyshevBasisElement(new_chebyshev_var_to_degree_map), new_coeff);
        }
    }
    return result;
}
...
std::map<ChebyshevBasisElement, double> MonomialBasisElement::ToChebyshevBasis() const {
    return MonomialToChebyshevBasisRecursive(var_to_degree_map());
}

void MonomialBasisElement::MergeBasisElementInPlace(const MonomialBasisElement& other) {
