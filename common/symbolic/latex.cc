...
std::optional<string> multiple_of_famous_constant(double value,
                                                  double famous_constant_value,
...
public:
    ...
    [[nodiscard]] std::string Latex(const Expression& e) const { return VisitExpression<string>(this, e); }
    [[nodiscard]] std::string Latex(const Formula& f) const { return VisitFormula(f, true); }
...
private:
    ...
    [[nodiscard]] string VisitBinary(const string& f, const Expression& e) const {
        return "\\" + f + "\\{" + Latex(get_first_argument(e)) + ", " + Latex(get_second_argument(e)) + "\\}";
    }
    ...
    [[nodiscard]] string VisitDivision(const Expression& e) const {
        return "\\frac{" + Latex(get_first_argument(e)) + "}{" + Latex(get_second_argument(e)) + "}";
    }

    [[nodiscard]] string VisitAbs(const Expression& e) const { return "|" + Latex(get_argument(e)) + "|"; }

    [[nodiscard]] string VisitLog(const Expression& e) const { return VisitUnary("log", e); }

    [[nodiscard]] string VisitExp(const Expression& e) const { return "e^{" + Latex(get_argument(e)) + "}"; }

    [[nodiscard]] string VisitSqrt(const Expression& e) const { return VisitUnary("sqrt", e); }

    [[nodiscard]] string VisitSin(const Expression& e) const { return VisitUnary("sin", e); }

    [[nodiscard]] string VisitCos(const Expression& e) const { return VisitUnary("cos", e); }

    [[nodiscard]] string VisitTan(const Expression& e) const { return VisitUnary("tan", e); }

    [[nodiscard]] string VisitAsin(const Expression& e) const { return VisitUnary("asin", e); }

    [[nodiscard]] string VisitAcos(const Expression& e) const { return VisitUnary("acos", e); }

    [[nodiscard]] string VisitAtan(const Expression& e) const { return VisitUnary("atan", e); }

    [[nodiscard]] string VisitAtan2(const Expression& e) const {
        return "\\atan{\\frac{" + Latex(get_first_argument(e)) + "}{" + Latex(get_second_argument(e)) + "}}";
    }

    [[nodiscard]] string VisitSinh(const Expression& e) const { return VisitUnary("sinh", e); }

    [[nodiscard]] string VisitCosh(const Expression& e) const { return VisitUnary("cosh", e); }

    [[nodiscard]] string VisitTanh(const Expression& e) const { return VisitUnary("tanh", e); }

    [[nodiscard]] string VisitMin(const Expression& e) const { return VisitBinary("min", e); }

    [[nodiscard]] string VisitMax(const Expression& e) const { return VisitBinary("max", e); }
    ...
    [[nodiscard]] std::string VisitFormula(const Formula& f, const bool polarity = true) const {
        return symbolic::VisitFormula<std::string>(this, f, polarity);
    }
    [[nodiscard]] std::string VisitFalse(const Formula&, const bool polarity) const {
        return polarity ? "\\text{false}" : "\\text{true}";
    }
    [[nodiscard]] std::string VisitTrue(const Formula&, const bool polarity) const {
        return polarity ? "\\text{true}" : "\\text{false}";
    }
    [[nodiscard]] std::string VisitVariable(const Formula& f, const bool polarity) const {
        return (polarity ? "" : "\\neg") + get_variable(f).to_string();
    }
    [[nodiscard]] std::string VisitEqualTo(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " = " : " \\neq ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitNotEqualTo(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " \\neq " : " = ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitGreaterThan(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " > " : " \\le ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitGreaterThanOrEqualTo(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " \\ge " : " < ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitLessThan(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " < " : " \\ge ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitLessThanOrEqualTo(const Formula& f, const bool polarity) const {
        return Latex(get_lhs_expression(f)) + (polarity ? " \\le " : " > ") + Latex(get_rhs_expression(f));
    }
    [[nodiscard]] std::string VisitConjunction(const Formula& f, const bool polarity) const {
        ostringstream oss;
    ...
    [[nodiscard]] std::string VisitDisjunction(const Formula& f, const bool polarity) const {
        ostringstream oss;
        bool print_symbol = false;
        for (const auto& o : get_operands(f)) {
            if (print_symbol) {
    ...
    [[nodiscard]] std::string VisitNegation(const Formula& f, const bool polarity) const {
        return VisitFormula(get_operand(f), !polarity);
    }

    [[nodiscard]] std::string VisitForall(const Formula& f, const bool polarity) const {
        ...
        oss << "\\forall " << VisitVariables(get_quantified_variables(f)) << ": " << get_quantified_formula(f);
        return oss.str();
    }

    [[nodiscard]] std::string VisitIsnan(const Formula& f, const bool polarity) const {
        ostringstream oss;
        if (!polarity) {
            oss << "\\neg ";
        }
    ...
    [[nodiscard]] std::string VisitPositiveSemidefinite(const Formula& f, const bool polarity) const {
        DRAKE_ASSERT(polarity);  // "Not PSD" could be an arbitrary matrix.
        return ToLatex(get_matrix_in_positive_semidefinite(f), precision_) + " \\succeq 0";
    }
    ...
    // methods.
    friend std::string VisitExpression<std::string>(const LatexVisitor*, const Expression&);

    // Makes VisitFormula a friend of this class so that it can use private
    // methods.
    friend std::string symbolic::VisitFormula<std::string>(const LatexVisitor*, const Formula&, const bool&);
...
string ToLatex(double val, int precision) {
    ...
    if (optional<string> result = multiple_of_famous_constant(val, M_PI, "\\pi")) {
        return *result;
    }
