...
Eigen::Matrix<Monomial, Eigen::Dynamic, 1> MonomialBasis(const Variables& vars, const int degree) {
    return internal::ComputeMonomialBasis<Eigen::Dynamic>(vars, degree, internal::DegreeType::kAny);
}

VectorX<Monomial> MonomialBasis(const std::unordered_map<Variables, int>& vars_degree) {
        ...
        for (const auto& var : vars) {
            if (all_vars.include(var)) {
                throw std::runtime_error(
                        fmt::format("MonomialBasis(): {} shows up more than once in vars_degree", var.get_name()));
            }
            all_vars.insert(var);
        }
    }
    ...
    for (const auto& monomial_vec : monomial_basis_each_vars) {
        ...
        basis.insert(basis.end(), monomial_vec.data(), monomial_vec.data() + monomial_vec.rows() - 1);
    ...
    std::sort(basis.begin(), basis.end(), GradedReverseLexOrder<std::less<Variable>>());
...
Eigen::Matrix<Monomial, Eigen::Dynamic, 1> EvenDegreeMonomialBasis(const Variables& vars, int degree) {
    return internal::ComputeMonomialBasis<Eigen::Dynamic>(vars, degree, internal::DegreeType::kEven);
}

Eigen::Matrix<Monomial, Eigen::Dynamic, 1> OddDegreeMonomialBasis(const Variables& vars, int degree) {
    return internal::ComputeMonomialBasis<Eigen::Dynamic>(vars, degree, internal::DegreeType::kOdd);
}
...
void CalcMonomialBasisOrderUpToOneHelper(const std::vector<Variable>& x,
                                         int start,
...
VectorX<Monomial> CalcMonomialBasisOrderUpToOne(const Variables& x, bool sort_monomial) {
    ...
    if (sort_monomial) {
        std::sort(monomial_basis.begin(), monomial_basis.end(), GradedReverseLexOrder<std::less<Variable>>());
    }
