 ...
 */
MapVarToIndex ConstructVarToIndexMap(const Eigen::Ref<const VectorX<symbolic::Variable>>& x) {
    MapVarToIndex map;
    map.reserve(x.rows());
    for (int i = 0; i < x.rows(); ++i) {
        const auto it = map.find(x(i).get_id());
        if (it != map.end()) {
            throw runtime_error("Input vector contains duplicate variable " + x(i).get_name());
        }
...
Expression ReplaceBilinearTerms(const Expression& e,
    ...
    for (const auto& p : map_bilinear) {
        ...
        if (monomial_degree > 2) {
            ...
            // y.
            const symbolic::Expression& w_xy_expr{W(it_x_idx->second, it_y_idx->second)};
            if (is_variable(w_xy_expr)) {
                ...
                if (intersect(w_xy_expr.GetVariables(), variables_in_x_y).size() != 0) {
                    ...
                    ostringstream oss;
                    oss << "W(" + std::to_string(it_x_idx->second) + "," + std::to_string(it_y_idx->second) + ")="
