...
#include "common/symbolic/expression/expression_cell.h"
...
// Polynomial::Add.
void DoAddProduct(const Expression& coeff, const Monomial& m, Polynomial::MapType* const map) {
    if (is_zero(coeff)) {
        return;
    }
    auto it = map->find(m);
...
public:
    Polynomial::MapType Decompose(const Expression& e, const Variables& indeterminates) const {
        // Note that it calls `Expression::Expand()` here.
        return Visit(e.Expand(), indeterminates);
    }

private:
    Polynomial::MapType Visit(const Expression& e, const Variables& indeterminates) const {
        return VisitExpression<Polynomial::MapType>(this, e, indeterminates);
    }

    Polynomial::MapType VisitVariable(const Expression& e, const Variables& indeterminates) const {
    ...
    Polynomial::MapType VisitConstant(const Expression& e, const Variables&) const {
    ...
    Polynomial::MapType VisitAddition(const Expression& e, const Variables& indeterminates) const {
        // e = c₀ + ∑ᵢ (cᵢ * eᵢ)
        ...
        for (const pair<const Expression, double>& p : get_expr_to_coeff_map_in_addition(e)) {
    ...
    Polynomial::MapType VisitMultiplication(const Expression& e, const Variables& indeterminates) const {
        // e = c * ∏ᵢ pow(baseᵢ, exponentᵢ).
        const double c = get_constant_in_multiplication(e);
        DRAKE_DEMAND(c != 0);
        Expression coeff{c};
        Monomial m{};
        for (const pair<const Expression, Expression>& p : get_base_to_exponent_map_in_multiplication(e)) {
            const Expression& base_i{p.first};
            const Expression& exponent_i{p.second};
            const pair<Monomial, Expression> result_i{VisitPow(base_i, exponent_i, indeterminates)};
    ...
    pair<Monomial, Expression> VisitPow(const Expression& base,
        ...
        if (intersect(base.GetVariables(), indeterminates).empty()) {
            ...
            if (!intersect(exponent.GetVariables(), indeterminates).empty()) {
                ...
                // hold.
                ostringstream oss;
                oss << "Exponent " << exponent << " includes an indeterminates " << indeterminates << ".";
            ...
            if (!is_constant(exponent) || !is_positive_integer(get_constant_value(exponent))) {
                ostringstream oss;
            ...
            if (!is_variable(base) && !is_multiplication(base)) {
                ostringstream oss;
                oss << "Base " << base << " is not a product of indeterminates, " << indeterminates;
                throw runtime_error(oss.str());
            }
    ...
    Polynomial::MapType VisitPow(const Expression& e, const Variables& indeterminates) const {
        const Expression& base{get_first_argument(e)};
        const Expression& exponent{get_second_argument(e)};
        const pair<Monomial, Expression> result{VisitPow(base, exponent, indeterminates)};
        return Polynomial::MapType{{{result.first, result.second}}};
    }

    Polynomial::MapType VisitDivision(const Expression& e, const Variables& indeterminates) const {
        // e = e₁ / e₂
        ...
        if (!intersect(e2.GetVariables(), indeterminates).empty()) {
            ostringstream oss;
            oss << "In " << e1 << " / " << e2 << ", the denominator " << e2 << " should be free of the indeterminates, "
                << indeterminates << ".";
            throw runtime_error(oss.str());
        }
    ...
    // indeterminate `x`.
    Polynomial::MapType VisitNonPolynomialTerm(const Expression& e, const Variables& indeterminates) const {
        // vars(e) ∩ indeterminates = ∅.
        if (!intersect(e.GetVariables(), indeterminates).empty()) {
            ostringstream oss;
            oss << "The non-polynomial term " << e << " should be free of the indeterminates " << indeterminates << ".";
            throw runtime_error(oss.str());
        }
        return {{Monomial{}, e}};  // = {1 ↦ e}.
    }

    Polynomial::MapType VisitAbs(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitLog(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitExp(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitSqrt(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitSin(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitCos(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitTan(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitAsin(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitAcos(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitAtan(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitAtan2(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitSinh(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitCosh(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitTanh(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitMin(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitMax(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitCeil(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitFloor(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitIfThenElse(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }
    Polynomial::MapType VisitUninterpretedFunction(const Expression& e, const Variables& indeterminates) const {
        return VisitNonPolynomialTerm(e, indeterminates);
    }

    // Makes VisitExpression a friend of this class so that it can use private
    // methods.
    friend Polynomial::MapType drake::symbolic::VisitExpression<Polynomial::MapType>(const DecomposePolynomialVisitor*,
                                                                                     const Expression&,
                                                                                     const Variables&);
};
...
Polynomial::Polynomial(MapType map)
    ...
    for (auto it = monomial_to_coefficient_map_.begin(); it != monomial_to_coefficient_map_.end();) {
...
Polynomial::Polynomial(const Monomial& m)
    : monomial_to_coefficient_map_{{m, 1}}, indeterminates_{m.GetVariables()}, decision_variables_{} {
...
Polynomial::Polynomial(const Expression& e, Variables indeterminates)
    : monomial_to_coefficient_map_{DecomposePolynomialVisitor{}.Decompose(e, indeterminates)},
...
int Polynomial::Degree(const Variable& v) const {
    int degree{0};
    for (const pair<const Monomial, Expression>& p : monomial_to_coefficient_map_) {
...
Expression Polynomial::ToExpression() const {
    // Returns ∑ᵢ (cᵢ * mᵢ).
    return accumulate(monomial_to_coefficient_map_.begin(), monomial_to_coefficient_map_.end(), Expression{0.0},
...
// returns `(2, xy)`.
pair<int, Monomial> DifferentiateMonomial(const Monomial& m, const Variable& x) {
...
Polynomial Polynomial::Differentiate(const Variable& x) const {
    if (indeterminates().include(x)) {
        ...
        for (const pair<const Monomial, Expression>& term : monomial_to_coefficient_map_) {
            const Monomial& m{term.first};
            const Expression& coeff{term.second};
            const pair<int, Monomial> m_prime{DifferentiateMonomial(m, x)};  // = d/dx m.
...
double Polynomial::Evaluate(const Environment& env) const {
    return accumulate(monomial_to_coefficient_map_.begin(), monomial_to_coefficient_map_.end(), 0.0,
...
Polynomial Polynomial::EvaluatePartial(const Environment& env) const {
    ...
    for (const auto& product_i : monomial_to_coefficient_map_) {
        ...
        const pair<double, Monomial> partial_eval_result{monomial_i.EvaluatePartial(env)};
...
Polynomial Polynomial::EvaluatePartial(const Variable& var, const double c) const {
...
Eigen::VectorXd Polynomial::EvaluateIndeterminates(
    ...
    Eigen::VectorXd polynomial_values = Eigen::VectorXd::Zero(indeterminates_values.cols());
    for (const auto& [monomial, coeff] : monomial_to_coefficient_map_) {
        const symbolic::Expression& coeff_expanded = coeff.is_expanded() ? coeff : coeff.Expand();
        ...
        polynomial_values += coeff_val * monomial.Evaluate(indeterminates, indeterminates_values);
    }
    return polynomial_values;
}

void Polynomial::EvaluateWithAffineCoefficients(const Eigen::Ref<const VectorX<symbolic::Variable>>& indeterminates,
    ...
    for (const auto& [monomial, monomial_coeff] : monomial_to_coefficient_map_) {
        ...
        DecomposeAffineExpression(monomial_coeff_expand, map_var_to_index, &a_coeff, &b_coeff);
        const Eigen::VectorXd monomial_vals = monomial.Evaluate(indeterminates, indeterminates_values);
...
Polynomial& Polynomial::operator+=(const Polynomial& p) {
    for (const pair<const Monomial, Expression>& item : p.monomial_to_coefficient_map_) {
...
bool Polynomial::CoefficientsAlmostEqual(const Polynomial& p, double tolerance) const {
    return (*this - p).Expand().RemoveTermsWithSmallCoefficients(tolerance).EqualTo(Polynomial());
}
...
Formula Polynomial::operator==(const Polynomial& p) const {
    ...
    for (const pair<const Monomial, Expression>& item : diff.monomial_to_coefficient_map_) {
        const Expression& coeff{item.second};
        ret = ret && (coeff == 0.0);
    }
    return ret;
...
Polynomial Polynomial::SubstituteAndExpand(const std::unordered_map<Variable, Polynomial>& indeterminate_substitution,
    ...
    SubstituteAndExpandCacheData* cached_data_ptr =
            substitutions_cached_data == nullptr ? &substitutions_default_obj : substitutions_cached_data;
    std::map<Monomial, Polynomial, internal::CompareMonomial>* substitutions = cached_data_ptr->get_data();

    for (const auto& var : indeterminates_) {
        DRAKE_DEMAND(indeterminate_substitution.find(var) != indeterminate_substitution.cend());
    ...
    auto find_nearest_cached_monomial = [&substitutions](const Monomial& monomial) {
        auto nearest_cached_monomial_iter = std::prev(substitutions->lower_bound(monomial));
        while (!((*nearest_cached_monomial_iter).first).GetVariables().IsSubsetOf(monomial.GetVariables())) {
    ...
    auto compute_substituted_monomial_expansion =
            [&indeterminate_substitution, &substitutions, &find_nearest_cached_monomial](
                    const Monomial& monomial,
                    auto&& compute_substituted_monomial_expansion_recursion_handle) -> const Polynomial& {
        ...
        if (nearest_cached_monomial == Monomial()) {
            ...
            const Polynomial& halved_power_substitution{compute_substituted_monomial_expansion_recursion_handle(
                    halved_monomials, compute_substituted_monomial_expansion_recursion_handle)};
            ...
            if (substitutions->find(halved_monomials_squared) == substitutions->cend()) {
                substitutions->emplace(halved_monomials_squared, pow(halved_power_substitution, 2).Expand());
            }
            ...
            const std::map<Variable, int>& cached_powers{nearest_cached_monomial.get_powers()};
            for (const auto& [var, power] : monomial.get_powers()) {
            ...
            const Polynomial& remaining_substitution{compute_substituted_monomial_expansion_recursion_handle(
                    remaining_monomials, compute_substituted_monomial_expansion_recursion_handle)};
            substitutions->emplace(monomial,
                                   (substitutions->at(nearest_cached_monomial) * remaining_substitution).Expand());
        }
        return substitutions->at(monomial);
    };
    ...
    for (const auto& [old_monomial, old_coeff] : monomial_to_coefficient_map_) {
        ...
        // now.
        if (old_monomial.total_degree() != 1 && substitutions->find(old_monomial) == substitutions->cend()) {
            compute_substituted_monomial_expansion(old_monomial, compute_substituted_monomial_expansion);
        }
        ...
        const Polynomial& substitution_map =
                (old_monomial.total_degree() == 1 ? indeterminate_substitution.at(*old_monomial.GetVariables().begin())
                                                  : substitutions->at(old_monomial));
        for (const auto& [new_monomial, new_coeff] : substitution_map.monomial_to_coefficient_map()) {
            if (new_polynomial_coeff_map.find(new_monomial) == new_polynomial_coeff_map.cend()) {
                new_polynomial_coeff_map.insert({new_monomial, Expression()});
            }
            new_polynomial_coeff_map.at(new_monomial) += (new_coeff * old_coeff).Expand();
        }
    }
...
Polynomial Polynomial::RemoveTermsWithSmallCoefficients(double coefficient_tol) const {
    DRAKE_DEMAND(coefficient_tol > 0);
    MapType cleaned_polynomial{};
    for (const auto& term : monomial_to_coefficient_map_) {
        if (is_constant(term.second) && std::abs(get_constant_value(term.second)) <= coefficient_tol) {
            // The coefficients are small.
            continue;
        } else {
            cleaned_polynomial.emplace_hint(cleaned_polynomial.end(), term.first, term.second);
        }
    }
...
bool IsEvenOrOdd(const Polynomial& p, bool check_even) {
    for (const auto& [monomial, coeff] : p.monomial_to_coefficient_map()) {
        ...
        if (monomial.total_degree() % 2 == static_cast<int>(check_even)) {
            const symbolic::Expression& coeff_expanded = coeff.is_expanded() ? coeff : coeff.Expand();
            if (!(is_constant(coeff_expanded) && get_constant_value(coeff_expanded) == 0)) {
                return false;
            }
        }
    }
...
Eigen::VectorXcd Polynomial::Roots() const {
    if (indeterminates().size() != 1) {
        throw runtime_error(
                fmt::format("{} is not a univariate polynomial; it has indeterminates {}.", *this, indeterminates()));
    }
    ...
    for (const auto& [monomial, coeff] : monomial_to_coefficient_map()) {
        if (!is_constant(coeff)) {
            throw runtime_error(
                    fmt::format("Polynomial::Roots() only supports polynomials with constant "
...
namespace numext {
template <>
bool equal_strict(const drake::symbolic::Polynomial& x, const drake::symbolic::Polynomial& y) {
