...
public:
     ...
     */
    explicit MonomialBasisElement(const std::map<Variable, int>& var_to_degree_map);
     ...
     *   = (2³ = 8, y²).
     */
    [[nodiscard]] std::pair<double, MonomialBasisElement> EvaluatePartial(const Environment& env) const;
     ...
     * map.
     */
    [[nodiscard]] std::map<MonomialBasisElement, double> Differentiate(const Variable& var) const;
     ...
     */
    [[nodiscard]] std::map<MonomialBasisElement, double> Integrate(const Variable& var) const;
    ...
    friend void hash_append(HashAlgorithm& hasher, const MonomialBasisElement& item) noexcept {
     ...
     */
    [[nodiscard]] std::map<ChebyshevBasisElement, double> ToChebyshevBasis() const;
...
private:
    [[nodiscard]] double DoEvaluate(double variable_val, int degree) const override;
...
template <>
struct NumTraits<drake::symbolic::MonomialBasisElement> : GenericNumTraits<drake::symbolic::MonomialBasisElement> {
...
template <>
EIGEN_DEVICE_FUNC inline drake::symbolic::Expression cast(const drake::symbolic::MonomialBasisElement& m) {
...
namespace fmt {
template <>
struct formatter<drake::symbolic::MonomialBasisElement> : drake::ostream_formatter {};
}  // namespace fmt
