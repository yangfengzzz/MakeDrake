...
#include "common/symbolic/monomial.h"
...
struct CompareMonomial {
    bool operator()(const Monomial& m1, const Monomial& m2) const {
        ...
        return std::lexicographical_compare(
                ...
                [](const std::pair<const Variable, int>& p1, const std::pair<const Variable, int>& p2) {
...
public:
    ...
    [[nodiscard]] Eigen::Matrix<Polynomial, 1, Derived::RowsAtCompileTime> Jacobian(
            const Eigen::MatrixBase<Derived>& vars) const {
        static_assert(std::is_same_v<typename Derived::Scalar, Variable> && (Derived::ColsAtCompileTime == 1),
    ...
    @throws std::exception if `x` is a decision variable. */
    [[nodiscard]] Polynomial Integrate(const Variable& x, double a, double b) const;
    ...
    public:
        std::map<Monomial, Polynomial, internal::CompareMonomial>* get_data() { return &data_; }
    ...
    are removed. */
    [[nodiscard]] Polynomial RemoveTermsWithSmallCoefficients(double coefficient_tol) const;
    ...
    friend void hash_append(HashAlgorithm& hasher, const Polynomial& item) noexcept {
...
template <>
struct NumTraits<drake::symbolic::Polynomial> : GenericNumTraits<drake::symbolic::Polynomial> {
    static inline int digits10() { return 0; }
};

/* Informs Eigen that BinaryOp(LhsType, RhsType) gets ResultType. */
#define DRAKE_SYMBOLIC_SCALAR_BINARY_OP_TRAITS(LhsType, RhsType, BinaryOp, ResultType) \
    template <>                                                                        \
...
conj_product}. */
#define DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(LhsType, RhsType, ResultType)         \
    DRAKE_SYMBOLIC_SCALAR_BINARY_OP_TRAITS(LhsType, RhsType, internal::scalar_sum_op, ResultType)        \
    DRAKE_SYMBOLIC_SCALAR_BINARY_OP_TRAITS(LhsType, RhsType, internal::scalar_difference_op, ResultType) \
    DRAKE_SYMBOLIC_SCALAR_BINARY_OP_TRAITS(LhsType, RhsType, internal::scalar_product_op, ResultType)    \
    DRAKE_SYMBOLIC_SCALAR_BINARY_OP_TRAITS(LhsType, RhsType, internal::scalar_conj_product_op, ResultType)

/* Informs Eigen that Polynomial op Monomial gets Polynomial
where op ∈ {+, -, *, conj_product}. */
DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(drake::symbolic::Polynomial,
...
where op ∈ {+, -, *, conj_product}. */
DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(drake::symbolic::Monomial,
                                                           drake::symbolic::Polynomial,
                                                           drake::symbolic::Polynomial)

/* Informs Eigen that Variable op Polynomial gets Polynomial
where op ∈ {+, -, *, conj_product}. */
DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(drake::symbolic::Variable,
                                                           drake::symbolic::Polynomial,
                                                           drake::symbolic::Polynomial)
...
where op ∈ {+, -, *, conj_product}. */
DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(double,
                                                           drake::symbolic::Polynomial,
                                                           drake::symbolic::Polynomial)
...
where op ∈ {+, -, *, conj_product}. */
DRAKE_SYMBOLIC_SCALAR_SUM_DIFF_PRODUCT_CONJ_PRODUCT_TRAITS(drake::symbolic::Expression,
                                                           drake::symbolic::Polynomial,
                                                           drake::symbolic::Expression)
...
drake::symbolic::Expression. */
template <>
EIGEN_DEVICE_FUNC inline drake::symbolic::Expression cast(const drake::symbolic::Polynomial& p) {
...
template <>
EIGEN_STRONG_INLINE bool not_equal_strict(const drake::symbolic::Polynomial& x, const drake::symbolic::Polynomial& y) {
...
@pydrake_mkdoc_identifier{polynomial} */
[[nodiscard]] MatrixX<Polynomial> Jacobian(const Eigen::Ref<const VectorX<Polynomial>>& f,
...
[[nodiscard]] typename std::enable_if<is_eigen_vector_of<Derived1, symbolic::Monomial>::value &&
...
CalcPolynomialWLowerTriangularPart(const Eigen::MatrixBase<Derived1>& monomial_basis,
                                   const Eigen::MatrixBase<Derived2>& gram_lower) {
    DRAKE_DEMAND(monomial_basis.rows() * (monomial_basis.rows() + 1) / 2 == gram_lower.rows());
    ...
    for (int j = 0; j < monomial_basis.rows(); ++j) {
        for (int i = j; i < monomial_basis.rows(); ++i) {
            ...
            const auto& Qij = gram_lower(monomial_basis.rows() * j + i - (j + 1) * j / 2);
