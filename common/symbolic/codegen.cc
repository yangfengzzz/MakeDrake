...
string CodeGen(const string& function_name, const vector<Variable>& parameters, const Expression& e) {
    ostringstream oss;
    ...
    // <function_name>_meta().
    oss << function_name << "_meta_t " << function_name << "_meta() { return {{" << parameters.size() << "}}; }\n";
    return oss.str();
}
...
void CodeGenDenseMeta(
        const string& function_name, const int parameter_size, const int rows, const int cols, ostream* const os) {
    // <function_name>_meta_t type.
    (*os) << "typedef struct {\n"
             "    /* p: input, vector */\n"
             "    struct { int size; } p;\n"
          ...
          << function_name << "_meta_t;\n";
    // <function_name>_meta().
    (*os) << function_name << "_meta_t " << function_name << "_meta() { return {{" << parameter_size << "}, {" << rows
          << ", " << cols << "}}; }\n";
}
...
void CodeGenSparseData(const string& function_name,
    ...
    const CodeGenVisitor visitor{parameters};
    for (int i = 0; i < non_zeros; ++i) {
        (*os) << fmt::format("    values[{0}] = {1};\n", i, visitor.CodeGen(value_ptr[i]));
    }
    // Print footer.
    (*os) << "}\n";
}

void CodeGenSparseMeta(const string& function_name,
    ...
    (*os) << fmt::format(
            ...
            "{7}}}}}; }}\n",
            function_name, function_name, parameter_size, rows, cols, non_zeros, outer_indices, inner_indices);
}
...
std::string CodeGen(const std::string& function_name,
    ...
    ostringstream oss;
    internal::CodeGenSparseData(function_name, parameters, M.cols() + 1, M.nonZeros(), M.outerIndexPtr(),
                                M.innerIndexPtr(), M.valuePtr(), &oss);
    internal::CodeGenSparseMeta(function_name, parameters.size(), M.rows(), M.cols(), M.nonZeros(), M.cols() + 1,
                                M.nonZeros(), &oss);
    return oss.str();
}
