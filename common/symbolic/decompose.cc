...
public:
    ...
    explicit IsAffineVisitor(const Variables& variables) : variables_{&variables} {}

    // Called for the first coefficient. Needed for Eigen::MatrixBase::visit()
    // function.
    void init(const Expression& e, const Eigen::Index i, const Eigen::Index j) { (*this)(e, i, j); }
...
private:
    ...
    [[nodiscard]] bool IsNotAffine(const Expression& e) const {
        ...
        const Polynomial p{(variables_ != nullptr) ? Polynomial{e, *variables_} : Polynomial{e}};
...
namespace {
void ThrowError(const string& type, const string& expression, const string& additional_msg) {
    throw runtime_error("While decomposing an expression, we detected a " + type + " expression: " + expression +
                        additional_msg + ".");
}
...
void FindCoefficientAndFill(const Polynomial::MapType& map,
                            const Monomial& m,
                            const int i,
...
std::pair<VectorX<Variable>, std::unordered_map<Variable::Id, int>> ExtractVariablesFromExpression(
    ...
    VectorX<Variable> vars = Eigen::Map<VectorX<Variable>>(var_vec.data(), var_vec.size());
    ...
    *constant_term = 0;
    ...
    for (const auto& p : poly.monomial_to_coefficient_map()) {
        ...
        if (p_monomial.total_degree() > 1) {
            ...
            const symbolic::Variable::Id var_id = p_monomial_powers.begin()->first.get_id();
...
std::tuple<bool, Eigen::MatrixXd, Eigen::VectorXd, VectorX<Variable>> DecomposeL2NormExpression(
    ...
    A = math::DecomposePSDmatrixIntoXtransposeTimesX(Q, psd_tol, true /* return empty if not psd */);
    if (A.rows() == 0) {
        return {false, A, b, vars};
    }
    b = A.transpose().colPivHouseholderQr().solve(0.5 * r);
    if ((A.transpose() * b - 0.5 * r).array().abs().maxCoeff() > coefficient_tol) {
        return {false, A, b, vars};
    }
...
typedef std::tuple<VectorX<Expression>, VectorX<Expression>, Expression> LumpedFactorization;

// Visitor class to implement DecomposeLumpedParameters.
class DecomposeLumpedParametersVisitor {
public:
    LumpedFactorization Decompose(const Expression& e, const Variables& parameters) const {
        // Note that it calls `Expression::Expand()` here.
        return Visit(e.Expand(), parameters);
    }

private:
    LumpedFactorization Visit(const Expression& e, const Variables& parameters) const {
        return VisitExpression<LumpedFactorization>(this, e, parameters);
    }

    LumpedFactorization VisitVariable(const Expression& e, const Variables& parameters) const {
        const Variable& var{get_variable(e)};
        if (parameters.include(var)) {
            // W = [1], alpha = [e], w0 = [0]
            return LumpedFactorization{Vector1<Expression>{1}, Vector1<Expression>{e}, 0};
        } else {
            // W = [], alpha = [], w0 = [e]
    ...
    LumpedFactorization VisitConstant(const Expression& e, const Variables&) const {
        return LumpedFactorization{Vector0<Expression>{}, Vector0<Expression>{}, e};
    }

    LumpedFactorization VisitAddition(const Expression& e, const Variables& parameters) const {
        ...
        for (const std::pair<const Expression, double>& p : get_expr_to_coeff_map_in_addition(e)) {
    ...
    LumpedFactorization SimpleMultiplication(const LumpedFactorization& a, const LumpedFactorization& b) const {
        ...
        const auto N = w_a.size() * w_b.size() + (nonzero_w0a ? w_b.size() : 0) + (nonzero_w0b ? w_a.size() : 0);
    ...
    LumpedFactorization VisitMultiplication(const Expression& e, const Variables& parameters) const {
        const double c = get_constant_in_multiplication(e);
        LumpedFactorization f({}, {}, {c});

        // e = c * ∏ᵢ pow(baseᵢ, exponentᵢ).
        for (const std::pair<const Expression, Expression>& p : get_base_to_exponent_map_in_multiplication(e)) {
            const Expression& base_i{p.first};
            const Expression& exponent_i{p.second};
            const auto [w, alpha, w0] = SimpleMultiplication(
                    f, is_one(exponent_i) ? Visit(base_i, parameters) : VisitPow(pow(base_i, exponent_i), parameters));
    ...
    LumpedFactorization VisitPow(const Expression& e, const Variables& parameters) const {
    ...
    LumpedFactorization VisitNonPolynomialTerm(const Expression& e, const Variables& parameters) const {
    ...
    LumpedFactorization VisitDivision(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }

    LumpedFactorization VisitAbs(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitLog(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitExp(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitSqrt(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitSin(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitCos(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitTan(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitAsin(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitAcos(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitAtan(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitAtan2(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitSinh(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitCosh(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitTanh(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitMin(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitMax(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitCeil(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitFloor(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitIfThenElse(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }
    LumpedFactorization VisitUninterpretedFunction(const Expression& e, const Variables& parameters) const {
        return VisitNonPolynomialTerm(e, parameters);
    }

    // Makes VisitExpression a friend of this class so that it can use private
    // methods.
    friend LumpedFactorization drake::symbolic::VisitExpression<LumpedFactorization>(
            const DecomposeLumpedParametersVisitor*, const Expression&, const Variables&);
};

}  // namespace

std::tuple<MatrixX<Expression>, VectorX<Expression>, VectorX<Expression>> DecomposeLumpedParameters(
        const Eigen::Ref<const VectorX<Expression>>& f, const Eigen::Ref<const VectorX<Variable>>& parameters) {
    ...
    for (int i = 0; i < f.size(); i++) {
        const auto [w, alpha, this_w0] = visitor.Decompose(f[i], Variables(parameters));
        w0[i] = this_w0;
        for (int j = 0; j < alpha.size(); j++) {
            auto it = alpha_map.emplace(alpha[j], VectorX<Expression>::Zero(f.size())).first;
