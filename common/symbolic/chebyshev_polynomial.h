...
public:
     ...
     */
    [[nodiscard]] std::vector<std::pair<ChebyshevPolynomial, double>> Differentiate() const;

    /** Implements the @ref hash_append concept. */
    template <class HashAlgorithm>
    friend void hash_append(HashAlgorithm& hasher, const ChebyshevPolynomial& item) noexcept {
        if (item.degree() == 0) {
...
namespace fmt {
template <>
struct formatter<drake::symbolic::ChebyshevPolynomial> : drake::ostream_formatter {};
}  // namespace fmt
