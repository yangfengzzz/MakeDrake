...
private:
    ...
    [[nodiscard]] std::string VisitUnary(const std::string& f, const Expression& e) const;
    // Helper method to handle binary cases.
    [[nodiscard]] std::string VisitBinary(const std::string& f, const Expression& e) const;
    ...
    [[nodiscard]] std::string VisitUninterpretedFunction(const Expression& e) const;
    // Makes VisitExpression a friend of this class so that it can use private
    // methods.
    friend std::string VisitExpression<std::string>(const CodeGenVisitor*, const Expression&);
...
// drake::symbolic::CodeGen(const std::string &, const std::vector<Variable>&,
// const Eigen::PlainObjectBase<Derived>&).
void CodeGenDenseMeta(const std::string& function_name, int parameter_size, int rows, int cols, std::ostream* os);
}  // namespace internal
...
std::string CodeGen(const std::string& function_name,
    ...
    static_assert(std::is_same_v<typename Derived::Scalar, Expression>, "CodeGen should take a symbolic matrix.");
    std::ostringstream oss;
    internal::CodeGenDenseData(function_name, parameters, M.data(), M.cols() * M.rows(), &oss);
    internal::CodeGenDenseMeta(function_name, parameters.size(), M.rows(), M.cols(), &oss);
    return oss.str();
}
