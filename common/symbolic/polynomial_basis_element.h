...
public:
     ...
     */
    explicit PolynomialBasisElement(const std::map<Variable, int>& var_to_degree_map);
    ...
    [[nodiscard]] const std::map<Variable, int>& var_to_degree_map() const { return var_to_degree_map_; }
     ...
     */
    [[nodiscard]] const std::map<Variable, int>& get_powers() const { return var_to_degree_map_; }
...
protected:
     ...
     */
    [[nodiscard]] bool lexicographical_compare(const PolynomialBasisElement& other) const;
    ...
    std::map<Variable, int>* get_mutable_var_to_degree_map() { return &var_to_degree_map_; }
...
private:
    ...
    [[nodiscard]] virtual double DoEvaluate(double variable_val, int degree) const = 0;
...
template <>
EIGEN_DEVICE_FUNC inline drake::symbolic::Expression cast(const drake::symbolic::PolynomialBasisElement& m) {
