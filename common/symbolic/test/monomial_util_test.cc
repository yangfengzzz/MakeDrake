...
void CheckCalcMonomialBasisOrderUpToOne(const drake::symbolic::Variables& t) {
    ...
    const int basis_size_expected = static_cast<int>(std::pow(2, static_cast<int>(t.size())));
    ...
    for (int i = 0; i < basis_sorted.rows() - 1; ++i) {
        EXPECT_TRUE(GradedReverseLexOrder<std::less<Variable>>()(basis_sorted(i), basis_sorted(i + 1)));
    }
}
...
TEST_F(CalcMonomialBasisTest, CalcMonomialBasisOrderUpToOne) {
    ...
    const Vector4<symbolic::Monomial> monomial12 = CalcMonomialBasisOrderUpToOne(Variables({t1_, t2_}), true);
...
GTEST_TEST(MonomialBasis, TestMultipleVariables) {
    ...
    const VectorX<Monomial> monomials = MonomialBasis({{x_set, 2}, {y_set, 1}, {z_set, 3}, {w_set, 0}});
    ...
    for (int i = 0; i < monomials.rows(); ++i) {
        if (i != 0) {
            ...
            EXPECT_TRUE(GradedReverseLexOrder<std::less<Variable>>()(monomials(i - 1), monomials(i)));
        }
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MonomialBasis({{Variables({x0, x1}), 2}, {Variables({x0, y0}), 1}}),
