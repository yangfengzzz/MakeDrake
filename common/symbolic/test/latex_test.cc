...
GTEST_TEST(SymbolicLatex, BasicTest) {
    ...
    EXPECT_EQ(ToLatex(if_then_else(x > y, 2 * x, 3)), R"""(\begin{cases} 2 x & \text{if } x > y, \\)"""
                                                      R"""( 3 & \text{otherwise}.\end{cases})""");
    DRAKE_EXPECT_THROWS_MESSAGE(ToLatex(uninterpreted_function("myfunc", std::vector<Expression>())),
    ...
    M << 1.2, 3, 4.56, 7;
    EXPECT_EQ(ToLatex(M, 1), R"""(\begin{bmatrix} 1.2 & 3 \\ 4.6 & 7 \end{bmatrix})""");

    // Matrix<Expression>
    Eigen::Matrix<Expression, 2, 2> Me;
    Me << x, 2.3 * y, 2.3 * y, x + y;
    EXPECT_EQ(ToLatex(Me, 1), R"""(\begin{bmatrix} x & 2.3 y \\ 2.3 y & (x + y) \end{bmatrix})""");

    // Formula with a PSD Matrix.
    EXPECT_EQ(ToLatex(positive_semidefinite(Me), 1), R"""(\begin{bmatrix} x & 2.3 y \\ 2.3 y & (x + y) \end{bmatrix})"""
                                                     R"""( \succeq 0)""");
}
