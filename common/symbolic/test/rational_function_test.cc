...
TEST_F(SymbolicRationalFunctionTest, Subtraction) {
    ...
    EXPECT_PRED2(RationalFunctionEqual, monom_x_ - f1, -f1_minus_monom_x_expected);
    RationalFunction f1_monom_x_minus(p1_, p2_);
    f1_monom_x_minus -= monom_x_;
    EXPECT_PRED2(RationalFunctionEqual, f1_monom_x_minus, f1_minus_monom_x_expected);
...
TEST_F(SymbolicRationalFunctionTest, Product) {
    ...
    EXPECT_PRED2(RationalFunctionEqual, f1_times_monom_x, f1_times_monom_x_expected);
...
TEST_F(SymbolicRationalFunctionTest, Division) {
    ...
    EXPECT_PRED2(RationalFunctionEqual, f1 / monom_x_, monom_x_divides_f1_expected);
    EXPECT_PRED2(RationalFunctionEqual, monom_x_ / f1, 1 / monom_x_divides_f1_expected);
    RationalFunction monom_x_divides_f1 = f1;
    monom_x_divides_f1 /= monom_x_;
    EXPECT_PRED2(RationalFunctionEqual, monom_x_divides_f1, monom_x_divides_f1_expected);
    ...
    const std::string zero_divider_error{"RationalFunction: operator/=: The divider is 0."};
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(f1 / polynomial_fraction_zero, zero_divider_error);
    DRAKE_EXPECT_THROWS_MESSAGE(p1_ / polynomial_fraction_zero, zero_divider_error);
    DRAKE_EXPECT_THROWS_MESSAGE(monom_x_ / polynomial_fraction_zero, zero_divider_error);
...
TEST_F(SymbolicRationalFunctionTest, Exponentiation) {
    ...
    EXPECT_PRED2(RationalFunctionEqual, pow(f, 0), RationalFunction(polynomial_one_, polynomial_one_));
    EXPECT_PRED2(RationalFunctionEqual, pow(f, 1), f);
    EXPECT_PRED2(RationalFunctionEqual, pow(f, 2), RationalFunction(pow(p1_, 2), pow(p2_, 2)));
    EXPECT_PRED2(RationalFunctionEqual, pow(f, -1), RationalFunction(p2_, p1_));
    EXPECT_PRED2(RationalFunctionEqual, pow(f, -2), RationalFunction(pow(p2_, 2), pow(p1_, 2)));
}
...
TEST_F(SymbolicRationalFunctionTest, ProductAndAddition) {
    ...
    const RationalFunction result_expected(p1_ * p1_ * p3_ * p4_ + p2_ * p2_ * p3_ * p4_, p1_ * p2_ * p4_ * p4_);
    EXPECT_PRED2(test::PolyEqualAfterExpansion, result.numerator(), result_expected.numerator());
    EXPECT_PRED2(test::PolyEqualAfterExpansion, result.denominator(), result_expected.denominator());
}
...
TEST_F(SymbolicRationalFunctionTest, Evaluate) {
    ...
    const double expected_numerator{4.0 * -7.0 * -7.0 * -5.0 + 1.0 * -7.0 * -5.0 + 2.0 * -2.0};
    const double expected_denominator{4.0 * -7.0 * -2.0 + 1.0 * -5.0 * -2.0 + 2.0 * -7.0 * -5.0};
