...
public:
    ...
    bool operator<(const DerivedBasisA& other) const { return this->lexicographical_compare(other); }

    std::pair<double, DerivedBasisA> EvaluatePartial(const Environment& env) const {
        double coeff;
    ...
    void MergeBasisElementInPlace(const DerivedBasisA& other) { this->DoMergeBasisElementInPlace(other); }

private:
    double DoEvaluate(double variable_val, int degree) const override { return std::pow(variable_val, degree); }
...
public:
    ...
    bool operator<(const DerivedBasisB& other) const { return this->lexicographical_compare(other); }

    void MergeBasisElementInPlace(const DerivedBasisB& other) { this->DoMergeBasisElementInPlace(other); }

private:
    double DoEvaluate(double variable_val, int degree) const override { return 1.; }
...
TEST_F(SymbolicPolynomialBasisElementTest, Constructor) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DerivedBasisA({{x_, -1}}), "The degree for x is negative.");
}
...
TEST_F(SymbolicPolynomialBasisElementTest, EigenMatrix) {
    ...
    M << DerivedBasisA(std::map<Variable, int>({})), DerivedBasisA({{x_, 1}}), DerivedBasisA({{x_, 1}, {y_, 2}}),
            DerivedBasisA({{y_, 2}});
}
...
TEST_F(SymbolicPolynomialBasisElementTest, MergeBasisElementInPlace) {
    ...
    basis_element4.MergeBasisElementInPlace(DerivedBasisA({{x_, 1}, {y_, 3}, {z_, 2}}));
