#include "common/symbolic/monomial_basis_element.h"
...
protected:
    ...
    void SetUp() override {
        monomials_ = {
                ...
                MonomialBasisElement{{{var_x_, 1}, {var_y_, 2}, {var_z_, 3}}},  // xy^2z^3
                MonomialBasisElement{{{var_x_, 2}, {var_y_, 4}, {var_z_, 3}}},  // x^2y^4z^3
        };
    ...
    // equal.
    bool CheckEvaluatePartial(const MonomialBasisElement& m, const Environment& env) {
        const Expression e1{m.ToExpression().EvaluatePartial(env)};
        const pair<double, MonomialBasisElement> subst_result{m.EvaluatePartial(env)};
        const Expression e2{subst_result.first * subst_result.second.ToExpression()};
...
TEST_F(MonomialBasisElementTest, Constructor) {
    ...
    const MonomialBasisElement m6{Vector2<Variable>(var_x_, var_y_), Eigen::Vector2i(1, 3)};
...
TEST_F(MonomialBasisElementTest, ConstructFromVariablesAndExponents) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MonomialBasisElement(vars_xyz, Eigen::Vector3i(2, 0, -1)), "The exponent is negative.");
}
...
TEST_F(MonomialBasisElementTest, EigenMatrixOfMonomials) {
    ...
    // M = | 1   x    |
    //     | y²  x²z³ |
    M << MonomialBasisElement{}, MonomialBasisElement{var_x_}, MonomialBasisElement{{{var_y_, 2}}},
...
TEST_F(MonomialBasisElementTest, ToMonomialException1) { EXPECT_THROW(MonomialBasisElement{2 * x_}, runtime_error); }

// `x + y` is not a monomial.
TEST_F(MonomialBasisElementTest, ToMonomialException2) { EXPECT_THROW(MonomialBasisElement{x_ + y_}, runtime_error); }

// `x / 2.0` is not a monomial.
TEST_F(MonomialBasisElementTest, ToMonomialException3) { EXPECT_THROW(MonomialBasisElement{x_ / 2.0}, runtime_error); }
...
TEST_F(MonomialBasisElementTest, ToMonomialException4) {
    ...
    // initializer list.
    DRAKE_EXPECT_THROWS_MESSAGE((MonomialBasisElement{{{var_x_, -1}}}), "The degree for x is negative.");
}
...
TEST_F(MonomialBasisElementTest, Multiplication) {
    ...
    EXPECT_EQ(result2.at(MonomialBasisElement({{var_x_, 1}, {var_y_, 2}, {var_z_, 3}})), 1);
...
TEST_F(MonomialBasisElementTest, Differentiate) {
    // d1/dx = 0
    const std::map<MonomialBasisElement, double> result1 = MonomialBasisElement().Differentiate(var_x_);
    ...
    // dxy / dy = x
    const auto result4 = MonomialBasisElement({{var_x_, 1}, {var_y_, 1}}).Differentiate(var_y_);
    EXPECT_EQ(result4.size(), 1);
    EXPECT_EQ(result4.at(MonomialBasisElement(var_x_)), 1);

    // dx²y²z / dx = 2xy²z
    const auto result5 = MonomialBasisElement({{var_x_, 2}, {var_y_, 2}, {var_z_, 1}}).Differentiate(var_x_);
    EXPECT_EQ(result5.size(), 1);
    EXPECT_EQ(result5.at(MonomialBasisElement({{var_x_, 1}, {var_y_, 2}, {var_z_, 1}})), 2);
    // x³y⁴z² / dy = 4x³y³z²
    const auto result6 = MonomialBasisElement({{var_x_, 3}, {var_y_, 4}, {var_z_, 2}}).Differentiate(var_y_);
    EXPECT_EQ(result6.size(), 1);
    EXPECT_EQ(result6.at(MonomialBasisElement({{var_x_, 3}, {var_y_, 3}, {var_z_, 2}})), 4);
}

TEST_F(MonomialBasisElementTest, Integrate) {
    // ∫1 dx = x
    const std::map<MonomialBasisElement, double> result1 = MonomialBasisElement().Integrate(var_x_);
    EXPECT_EQ(result1.size(), 1);
    EXPECT_EQ(result1.at(MonomialBasisElement(var_x_)), 1);

    // ∫ x dy = xy
    const std::map<MonomialBasisElement, double> result2 = MonomialBasisElement(var_x_).Integrate(var_y_);
    EXPECT_EQ(result2.size(), 1);
    ...
    // ∫ xy² dy = 1/3 xy³
    const auto result4 = MonomialBasisElement({{var_x_, 1}, {var_y_, 2}}).Integrate(var_y_);
    EXPECT_EQ(result4.size(), 1);
    EXPECT_EQ(result4.at(MonomialBasisElement({{var_x_, 1}, {var_y_, 3}})), 1. / 3);

    // ∫ x²y³z dy = 1/4 x²y⁴z
    const auto result5 = MonomialBasisElement({{var_x_, 2}, {var_y_, 3}, {var_z_, 1}}).Integrate(var_y_);
    EXPECT_EQ(result5.size(), 1);
    EXPECT_EQ(result5.at(MonomialBasisElement({{var_x_, 2}, {var_y_, 4}, {var_z_, 1}})), 1. / 4);
}

TEST_F(MonomialBasisElementTest, ToChebyshevBasisUnivariate) {
    // Test ToChebyshevBasis for univariate monomials.
    // 1 = T0()
    const std::map<ChebyshevBasisElement, double> result1 = MonomialBasisElement().ToChebyshevBasis();
    EXPECT_EQ(result1.size(), 1);
    EXPECT_EQ(result1.at(ChebyshevBasisElement()), 1.);

    // x = T1(x)
    const std::map<ChebyshevBasisElement, double> result2 = MonomialBasisElement(var_x_).ToChebyshevBasis();
    EXPECT_EQ(result2.size(), 1);
    EXPECT_EQ(result2.at(ChebyshevBasisElement(var_x_)), 1.);

    // x² = 0.5T₂(x)+0.5T₀(x)
    const std::map<ChebyshevBasisElement, double> result3 = MonomialBasisElement(var_x_, 2).ToChebyshevBasis();
    EXPECT_EQ(result3.size(), 2);
    EXPECT_EQ(result3.at(ChebyshevBasisElement(var_x_, 2)), 0.5);
    EXPECT_EQ(result3.at(ChebyshevBasisElement()), 0.5);

    // x³ = 0.25T₃(x)+0.75T₁(x)
    const std::map<ChebyshevBasisElement, double> result4 = MonomialBasisElement(var_x_, 3).ToChebyshevBasis();
    ...
    const std::map<ChebyshevBasisElement, double> result5 = MonomialBasisElement(var_x_, 4).ToChebyshevBasis();
    ...
    const std::map<ChebyshevBasisElement, double> result6 = MonomialBasisElement(var_x_, 20).ToChebyshevBasis();
...
TEST_F(MonomialBasisElementTest, ToChebyshevBasisMultivariate) {
    ...
    const std::map<ChebyshevBasisElement, double> result2 =
            MonomialBasisElement({{var_x_, 1}, {var_y_, 1}, {var_z_, 1}}).ToChebyshevBasis();
    EXPECT_EQ(result2.size(), 1);
    EXPECT_EQ(result2.at(ChebyshevBasisElement({{var_x_, 1}, {var_y_, 1}, {var_z_, 1}})), 1.);
    ...
    const std::map<ChebyshevBasisElement, double> result4 =
            MonomialBasisElement({{var_x_, 3}, {var_y_, 2}, {var_z_, 1}}).ToChebyshevBasis();
    EXPECT_EQ(result4.size(), 4);
    EXPECT_EQ(result4.at(ChebyshevBasisElement({{var_x_, 3}, {var_y_, 2}, {var_z_, 1}})), 1.0 / 8);
    EXPECT_EQ(result4.at(ChebyshevBasisElement({{var_x_, 1}, {var_y_, 2}, {var_z_, 1}})), 3.0 / 8);
    EXPECT_EQ(result4.at(ChebyshevBasisElement({{var_x_, 3}, {var_z_, 1}})), 1.0 / 8);
    EXPECT_EQ(result4.at(ChebyshevBasisElement({{var_x_, 1}, {var_z_, 1}})), 3.0 / 8);
}

TEST_F(MonomialBasisElementTest, MergeBasisElementInPlace) {
    // x²y³ * xyz² = x³y⁴z²
    MonomialBasisElement basis_element1({{var_x_, 2}, {var_y_, 3}});
    basis_element1.MergeBasisElementInPlace(MonomialBasisElement({{var_x_, 1}, {var_y_, 1}, {var_z_, 2}}));
