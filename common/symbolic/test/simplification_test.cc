...
TEST_F(SymbolicUnificationTest, MultiplicationSuccess2) {
    ...
    //    => x * pow(sin(y), 3) * tan(z).
    const RewritingRule rule{x_ * pow(sin(y_), 2) * cos(z_), x_ * pow(sin(y_), 3) * tan(z_)};
    const Rewriter rewriter = MakeRuleRewriter(rule);
...
TEST_F(SymbolicUnificationTest, MultiplicationFailure1) {
    ...
    //    => 4pow(x, y)pow(2, z)
    const RewritingRule rule{3 * pow(sin(x_), cos(y_)) * tan(z_), 4 * pow(x_, y_) * pow(2, z_)};
    const Rewriter rewriter = MakeRuleRewriter(rule);
...
enum UnaryTestOp { Abs, Log, Exp, Sqrt, Sin, Cos, Tan, Asin, Acos, Atan, Sinh, Cosh, Tanh, Ceil, Floor };
...
class SymbolicUnificationTestUnary : public ::testing::TestWithParam<UnaryTestOp> {
protected:
    const Variable x_{"x"};
    const Variable a_{"a"};
};
...
INSTANTIATE_TEST_SUITE_P(
        UnaryCases,
        SymbolicUnificationTestUnary,
        ::testing::Values(Abs, Log, Exp, Sqrt, Sin, Cos, Tan, Asin, Acos, Atan, Sinh, Cosh, Tanh, Ceil, Floor));
...
class SymbolicUnificationTestBinary : public ::testing::TestWithParam<BinaryTestOp> {
protected:
    const Variable x_{"x"};
    const Variable y_{"y"};
    const Variable a_{"a"};
...
INSTANTIATE_TEST_SUITE_P(BinaryCases, SymbolicUnificationTestBinary, ::testing::Values(Pow, Div, Min, Max, Atan2));
