...
::testing::AssertionResult CompareMatricesWithExpansion(const Eigen::Ref<const MatrixX<Expression>>& m1,
    ...
    if (m1_expanded == m2_expanded) {
        ...
        return ::testing::AssertionFailure() << fmt::format(
                       ...
                       "m1_expanded = \n{}\n"
                       "m2_expanded = \n{}\n",
                       fmt_eigen(m1), fmt_eigen(m2), fmt_eigen(m1_expanded), fmt_eigen(m2_expanded));
    }
}

template <typename Matrix1, typename Matrix2>
::testing::AssertionResult CheckAddition(const Matrix1& M1, const Matrix2& M2) {
    return CompareMatricesWithExpansion((M1 + M2).template cast<Expression>(),
                                        M1.template cast<Expression>() + M2.template cast<Expression>());
}

template <typename Matrix1, typename Matrix2>
::testing::AssertionResult CheckSubtraction(const Matrix1& M1, const Matrix2& M2) {
    return CompareMatricesWithExpansion((M1 - M2).template cast<Expression>(),
                                        M1.template cast<Expression>() - M2.template cast<Expression>());
}

template <typename Matrix1, typename Matrix2>
::testing::AssertionResult CheckMultiplication(const Matrix1& M1, const Matrix2& M2) {
    return CompareMatricesWithExpansion((M1 * M2).template cast<Expression>(),
                                        M1.template cast<Expression>() * M2.template cast<Expression>());
}
...
bool CheckDotProductResPoly(const Vector1& v1, const Vector2& v2) {
    ...
    const Expression e2{v1.template cast<Expression>().dot(v2.template cast<Expression>()).Expand()};
...
TEST_F(SymbolicPolynomialMatrixTest, ExpressionMonomial) {
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_expression_dynamic_, v_monomial_dynamic_));
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_expression_static_, v_monomial_dynamic_));
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_expression_dynamic_, v_monomial_static_));
...
TEST_F(SymbolicPolynomialMatrixTest, MonomialExpression) {
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_monomial_dynamic_, v_expression_dynamic_));
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_monomial_static_, v_expression_dynamic_));
    ...
    EXPECT_TRUE(CheckDotProductResExpr(v_monomial_dynamic_, v_expression_static_));
...
TEST_F(SymbolicPolynomialMatrixTest, EvaluateMatrix) {
    ...
    EXPECT_EQ(Evaluate(M_poly_dynamic_, env), Evaluate(M_poly_dynamic_.cast<Expression>(), env));

    EXPECT_EQ(Evaluate(M_poly_static_, env), Evaluate(M_poly_static_.cast<Expression>(), env));

    EXPECT_EQ(Evaluate(v_poly_dynamic_, env), Evaluate(v_poly_dynamic_.cast<Expression>(), env));

    EXPECT_EQ(Evaluate(v_poly_static_, env), Evaluate(v_poly_static_.cast<Expression>(), env));
}
