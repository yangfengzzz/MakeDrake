...
TEST_F(SymbolicDecomposeTest, DecomposeLinearExpressionsExceptionNonLinear) {
  ...
  extra_terms_ << x0_ * x0_,
                 ...
                 x2_ * x2_;
    // clang-format on
    DRAKE_EXPECT_THROWS_MESSAGE(DecomposeLinearExpressions(M_ * x_ + extra_terms_, x_, &M_expected_static_),
                                ".*we detected a non-linear expression.*");
}

// Adds nonlinear terms to check if we have an exception.
TEST_F(SymbolicDecomposeTest, DecomposeLinearExpressionsExceptionNonPolynomial) {
    // clang-format off
...
TEST_F(SymbolicDecomposeTest, DecomposeLinearExpressionsExceptionNonConstCoefficient) {
    // clang-format off
  extra_terms_ << a_ * x0_,
                  b_ * x1_,
                  c_ * x2_;
...
TEST_F(SymbolicDecomposeTest, DecomposeLinearExpressionsExceptionAffine) {
  ...
  extra_terms_ << -1,
                  ...
                  M_PI;
    // clang-format on
    EXPECT_THROW(DecomposeLinearExpressions(M_ * x_ + extra_terms_, x_, &M_expected_static_), runtime_error);
}

TEST_F(SymbolicDecomposeTest, DecomposeAffineExpressionsDynamic) {
    DecomposeAffineExpressions(M_ * x_ + v_, x_, &M_expected_dynamic_, &v_expected_dynamic_);
    EXPECT_EQ(M_expected_dynamic_, M_);
    EXPECT_EQ(v_expected_dynamic_, v_);
}

TEST_F(SymbolicDecomposeTest, DecomposeAffineExpressionsStatic) {
    DecomposeAffineExpressions(M_ * x_ + v_, x_, &M_expected_static_, &v_expected_static_);
    EXPECT_EQ(M_expected_static_, M_);
...
TEST_F(SymbolicDecomposeTest, DecomposeAffineExpressionsExceptionNonlinear) {
  ...
  extra_terms_ << x0_ * x0_,
                 ...
                 x2_ * x2_;
    // clang-format on
    EXPECT_THROW(DecomposeAffineExpressions(M_ * x_ + v_ + extra_terms_, x_, &M_expected_static_, &v_expected_static_),
                 runtime_error);

    // Check variant with vars as an output.
    EXPECT_THROW(
            DecomposeAffineExpressions(M_ * x_ + v_ + extra_terms_, &M_expected_dynamic_, &v_expected_dynamic_, &x_),
            runtime_error);
}

// Adds terms with non-const coefficients to check if we have an exception.
TEST_F(SymbolicDecomposeTest, DecomposeAffineExpressionsExceptionNonConstCoefficient) {
    // clang-format off
  extra_terms_ << a_ * x0_,
                  b_ * x1_,
                  c_ * x2_;
...
// Adds nonlinear terms to check if we have an exception.
TEST_F(SymbolicDecomposeTest, DecomposeAffineExpressionsExceptionNonPolynomial) {
    // clang-format off
  extra_terms_ << sin(x0_),
                 cos(x1_),
                 log(x2_);
...
void ExpectValidMapVarToIndex(const VectorX<Variable>& vars, const MapVarToIndex& map_var_to_index) {
...
GTEST_TEST(SymbolicExtraction, ExtractVariables2) {
    ...
    VectorX<Variable> vars;
    MapVarToIndex map_var_to_index;
    std::tie(vars, map_var_to_index) = ExtractVariablesFromExpression(expressions);
    EXPECT_EQ(symbolic::Variables(vars), symbolic::Variables({x, y, z}));
    ExpectValidMapVarToIndex(vars, map_var_to_index);

    std::tie(vars, map_var_to_index) = ExtractVariablesFromExpression(expressions.tail<2>());
    EXPECT_EQ(symbolic::Variables(vars), symbolic::Variables({x, y, z}));
    ExpectValidMapVarToIndex(vars, map_var_to_index);

    std::tie(vars, map_var_to_index) = ExtractVariablesFromExpression(expressions.head<2>());
...
void AverageOffDiagonalTerms(const Eigen::MatrixBase<Derived>& Q_asym, Eigen::MatrixBase<Derived>* pQ_sym) {
...
GTEST_TEST(SymbolicExtraction, DecomposeQuadraticExpression) {
    ...
    for (const Eigen::Matrix3d& Q_in : {Q_diagonal, Q_symmetric, Q_asymmetric}) {
        const Expression e = vars_expected.dot(0.5 * Q_in * vars_expected + b_expected) + c_expected;
...
GTEST_TEST(SymbolicExtraction, DecomposeAffineExpressionReturnValue) {
    ...
    std::tie(vars, map_var_to_index) = ExtractVariablesFromExpression(e);
    coeffs.resize(1);
    DRAKE_EXPECT_THROWS_MESSAGE(DecomposeAffineExpression(e, map_var_to_index, &coeffs, &c), ".*non-linear.*");

    // Not even polynomial.
    e = sin(x);
    std::tie(vars, map_var_to_index) = ExtractVariablesFromExpression(e);
    coeffs.resize(1);
    DRAKE_EXPECT_THROWS_MESSAGE(DecomposeAffineExpression(e, map_var_to_index, &coeffs, &c), ".*not a polynomial.\n");
}
...
GTEST_TEST(SymbolicExtraction, DecomposeLumpedParameters) {
...

    {
        ...
        const auto [W, alpha, w0] = DecomposeLumpedParameters(Vector1<Expression>(e), Vector3<Variable>{a, b, c});
...

    {
        ...
        const auto [W, alpha, w0] = DecomposeLumpedParameters(f, Vector3<Variable>{a, b, c});
...

    {
        ...
        EXPECT_THROW(DecomposeLumpedParameters(Vector1<Expression>(e), Vector3<Variable>{a, b, c}), std::exception);
    }

    {
        // Test additional symbolic elements.
        const Expression e = x / y + abs(x) + log(x) + exp(x) + sqrt(x) + sin(x) + cos(x) + tan(x) + asin(x) + acos(x) +
                             atan(x) + atan2(x, y) + sinh(x) + cosh(x) + tanh(x) + min(x, y) + max(x, y) + ceil(x) +
                             floor(x);
...
GTEST_TEST(SymbolicExtraction, DecomposeL2Norm2) {
    ...
    EXPECT_TRUE(CompareMatrices(A.transpose() * A, A_expected.transpose() * A_expected, 1e-12));
    EXPECT_TRUE(CompareMatrices(A.transpose() * b, A_expected.transpose() * b_expected, 1e-12));
    EXPECT_EQ(vars, x);
}
