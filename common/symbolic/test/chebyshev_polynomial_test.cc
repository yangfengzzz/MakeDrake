...
GTEST_TEST(ChebyshevPolynomialTest, ToPolynomial) {
    ...
    EXPECT_PRED2(test::PolyEqual, p3.ToPolynomial(), Polynomial(4 * x * x * x - 3 * x));

    const ChebyshevPolynomial p4(x, 4);
    EXPECT_PRED2(test::PolyEqual, p4.ToPolynomial(), Polynomial(8 * pow(x, 4) - 8 * x * x + 1));
...
GTEST_TEST(ChebyshevPolynomialTest, Eval) {
    ...
    for (int degree = 0; degree < 20; ++degree) {
        EXPECT_NEAR(ChebyshevPolynomial(x, degree).Evaluate(cos_theta), std::cos(degree * theta), tol);
    }
}
...
GTEST_TEST(ChebyshevPolynomialTest, Differentiate) {
    ...
    const std::vector<std::pair<ChebyshevPolynomial, double>> p1_derivative = p1.Differentiate();
    ...
    const std::vector<std::pair<ChebyshevPolynomial, double>> p2_derivative = p2.Differentiate();
    ...
    const std::vector<std::pair<ChebyshevPolynomial, double>> p3_derivative = p3.Differentiate();
    ...
    std::vector<std::pair<ChebyshevPolynomial, double>> p4_derivative = p4.Differentiate();
    ...
    for (int degree = 2; degree < 20; ++degree) {
        ...
        const std::vector<std::pair<ChebyshevPolynomial, double>> T_derivative = T_n.Differentiate();
        if (degree % 2 == 0) {
        ...
        EXPECT_PRED2(test::PolyEqual, lhs, degree * (x * T_n.ToPolynomial() - T_n_plus_1.ToPolynomial()));
        EXPECT_PRED2(test::PolyEqual, lhs, degree * (T_n_minus_1.ToPolynomial() - x * T_n.ToPolynomial()));
    }
}
...
GTEST_TEST(ChebyshevPolynomialTest, UnorderedMapOfChebyshevPolynomial) {
    ...
    EXPECT_EQ(chebyshev_to_coeff_map.find(ChebyshevPolynomial(x, 3)), chebyshev_to_coeff_map.end());
