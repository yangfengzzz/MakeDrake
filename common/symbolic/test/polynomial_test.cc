...
protected:
    ...
    const drake::VectorX<symbolic::Monomial> monomials_{MonomialBasis(var_xyz_, 3)};
...
TEST_F(SymbolicPolynomialTest, ConstructFromMonomial) {
    for (int i = 0; i < monomials_.size(); ++i) {
        ...
        for (const std::pair<const Monomial, Expression>& map : p.monomial_to_coefficient_map()) {
...
TEST_F(SymbolicPolynomialTest, ConstructorFromExpressionAndIndeterminates1) {
    ...
    EXPECT_EQ(p1.monomial_to_coefficient_map(), Polynomial::MapType({{Monomial{}, Expression(1.0)}}));
    // p₂ = ax + by + cz + 10
    const Polynomial p2{a_ * x_ + b_ * y_ + c_ * z_ + 10, var_xyz_};
    EXPECT_EQ(p2.monomial_to_coefficient_map(),
              Polynomial::MapType(
                      {{Monomial{var_x_}, a_}, {Monomial{var_y_}, b_}, {Monomial{var_z_}, c_}, {Monomial{}, 10}}));
    // p₃ = 3ab²*x²y -bc*z³
    const Polynomial p3{3 * a_ * pow(b_, 2) * pow(x_, 2) * y_ - b_ * c_ * pow(z_, 3), var_xyz_};
    EXPECT_EQ(p3.monomial_to_coefficient_map(), Polynomial::MapType(
                                                        // x²y ↦ 3ab²
    ...
    // p₄ = 3ab²*x²y - bc*x³
    const Polynomial p4{3 * a_ * pow(b_, 2) * pow(x_, 2) * y_ - b_ * c_ * pow(x_, 3), var_xyz_};
    EXPECT_EQ(p4.monomial_to_coefficient_map(),
              Polynomial::MapType({{Monomial{{{var_x_, 2}, {var_y_, 1}}}, 3 * a_ * pow(b_, 2)},
...
TEST_F(SymbolicPolynomialTest, IndeterminatesAndDecisionVariables) {
    // p = 3ab²*x²y -bc*z³
    const Polynomial p{3 * a_ * pow(b_, 2) * pow(x_, 2) * y_ - b_ * c_ * pow(z_, 3), var_xyz_};
...
TEST_F(SymbolicPolynomialTest, AdditionPolynomialPolynomial) {
    ...
    for (const Expression& e1 : exprs_) {
        for (const Expression& e2 : exprs_) {
            EXPECT_PRED2(ExprEqual, (Polynomial{e1} + Polynomial{e2}).ToExpression(), (e1 + e2).Expand());
        }
    }
...
TEST_F(SymbolicPolynomialTest, AdditionPolynomialMonomial) {
    ...
    for (const Expression& e : exprs_) {
        ...
        for (int i = 0; i < monomials_.size(); ++i) {
            ...
            EXPECT_PRED2(ExprEqual, sum1.ToExpression(), (e + m.ToExpression()).Expand());
            EXPECT_PRED2(ExprEqual, sum2.ToExpression(), (m.ToExpression() + e).Expand());
...
TEST_F(SymbolicPolynomialTest, AdditionPolynomialDouble) {
    ...
    for (const Expression& e : exprs_) {
        for (const double c : doubles_) {
            EXPECT_PRED2(ExprEqual, (Polynomial(e) + c).ToExpression(), (e + c).Expand());
            EXPECT_PRED2(ExprEqual, (c + Polynomial(e)).ToExpression(), (c + e).Expand());
        }
    }
...
TEST_F(SymbolicPolynomialTest, AdditionMonomialMonomial) {
    ...
    for (int i = 0; i < monomials_.size(); ++i) {
        ...
        for (int j = 0; j < monomials_.size(); ++j) {
            ...
            EXPECT_PRED2(ExprEqual, (m_i + m_j).ToExpression(), m_i.ToExpression() + m_j.ToExpression());
        }
    }
}
...
TEST_F(SymbolicPolynomialTest, SubtractionPolynomialMonomial) {
    ...
    for (const Expression& e : exprs_) {
        for (int i = 0; i < monomials_.size(); ++i) {
            const Monomial& m{monomials_[i]};
            EXPECT_PRED2(ExprEqual, (Polynomial(e) - m).ToExpression(), (e - m.ToExpression()).Expand());
            EXPECT_PRED2(ExprEqual, (m - Polynomial(e)).ToExpression(), (m.ToExpression() - e).Expand());
        }
    }
    // Test Polynomial& operator-=(const Monomial& m);
    for (const Expression& e : exprs_) {
...
TEST_F(SymbolicPolynomialTest, MultiplicationPolynomialPolynomial1) {
    ...
    for (const Expression& e1 : exprs_) {
        for (const Expression& e2 : exprs_) {
            ...
            EXPECT_PRED2(ExprEqual, product.ToExpression(), (e1.Expand() * e2.Expand()).Expand());
            ...
            // variables.
            EXPECT_EQ(product.indeterminates(), p1.indeterminates() + p2.indeterminates());
            EXPECT_EQ(product.decision_variables(), p1.decision_variables() + p2.decision_variables());
        }
    }
    // Test Polynomial& operator*=(Polynomial& c);
    for (const Expression& e1 : exprs_) {
        for (const Expression& e2 : exprs_) {
            ...
            const symbolic::Polynomial p2{e2};
            p *= p2;
            EXPECT_PRED2(ExprEqual, p.ToExpression(), (e1.Expand() * e2.Expand()).Expand());
            EXPECT_EQ(p.indeterminates(), e1_indeterminates + p2.indeterminates());
            EXPECT_EQ(p.decision_variables(), e1_decision_variables + p2.decision_variables());
        }
    }
}
...
TEST_F(SymbolicPolynomialTest, MultiplicationPolynomialMonomial) {
    ...
    for (const Expression& e : exprs_) {
        for (int i = 0; i < monomials_.size(); ++i) {
            ...
            EXPECT_PRED2(ExprEqual, product1.ToExpression(), (e * m.ToExpression()).Expand());
            EXPECT_PRED2(ExprEqual, product2.ToExpression(), (m.ToExpression() * e).Expand());
            EXPECT_EQ(product1.indeterminates(), p.indeterminates() + m.GetVariables());
            EXPECT_EQ(product2.indeterminates(), p.indeterminates() + m.GetVariables());
...
TEST_F(SymbolicPolynomialTest, BinaryOperationBetweenPolynomialAndVariable) {
    ...
    // Checks addition.
    {
        ...
        EXPECT_PRED2(ExprEqual, result1.monomial_to_coefficient_map().at(m_one), 7 + a_);
        ...
        EXPECT_PRED2(ExprEqual, result3.monomial_to_coefficient_map().at(m_x), 3 * a_ + 1);
    }
    ...
    // Checks multiplication.
    {
        ...
        EXPECT_EQ(result1.indeterminates(), p.indeterminates());
        EXPECT_EQ(result1.monomial_to_coefficient_map().size(), 3);
        EXPECT_PRED2(ExprEqual, result1.monomial_to_coefficient_map().at(m_x_sq), 2 * pow(a_, 3));
        EXPECT_PRED2(ExprEqual, result1.monomial_to_coefficient_map().at(m_x), 3 * pow(a_, 2));
        EXPECT_PRED2(ExprEqual, result1.monomial_to_coefficient_map().at(m_one), 7 * a_);
        ...
        EXPECT_EQ(result3.indeterminates(), p.indeterminates());
        EXPECT_EQ(result3.monomial_to_coefficient_map().size(), 3);
        EXPECT_PRED2(ExprEqual, result3.monomial_to_coefficient_map().at(m_x_cube), 2 * pow(a_, 2));
        EXPECT_PRED2(ExprEqual, result3.monomial_to_coefficient_map().at(m_x_sq), 3 * a_);
        ...
        EXPECT_EQ(result5.decision_variables(), symbolic::Variables({var_a_, var_b_}));
        EXPECT_EQ(result5.monomial_to_coefficient_map().size(), 3);
        EXPECT_PRED2(ExprEqual, result5.monomial_to_coefficient_map().at(m_x_sq), 2 * pow(a_, 2) * b_);
        EXPECT_PRED2(ExprEqual, result5.monomial_to_coefficient_map().at(m_x), 3 * a_ * b_);
        EXPECT_PRED2(ExprEqual, result5.monomial_to_coefficient_map().at(m_one), 7 * b_);
    }
}
...
TEST_F(SymbolicPolynomialTest, DivideByConstant) {
    for (double v = -5.5; v <= 5.5; v += 1.0) {
        for (const Expression& e : exprs_) {
            EXPECT_PRED2(ExprEqual, (Polynomial(e) / v).ToExpression(), Polynomial(e / v).ToExpression());
        }
    }
}
...
TEST_F(SymbolicPolynomialTest, DivPolynomialExpression) {
    ...
    for (const Expression& e1 : exprs_) {
        for (const Expression& e2 : exprs_) {
            ...
            EXPECT_PRED2(ExprEqual, (Polynomial(e1) / e2).Expand(), (e1 / e2).Expand());
            EXPECT_PRED2(ExprEqual, (e1 / Polynomial(e2)).Expand(), (e1 / e2).Expand());
        }
    }
}
...
TEST_F(SymbolicPolynomialTest, DifferentiateJacobian) {
    // p = 2a²bx² + 3bc²x + 7ac.
    const Polynomial p{2 * pow(a_, 2) * b_ * pow(x_, 2) + 3 * b_ * pow(c_, 2) * x_ + 7 * a_ * c_,
                       {var_a_, var_b_, var_c_}};

    // d/dx p = 4a²bx + 3bc²
    const Polynomial p_x{4 * pow(a_, 2) * b_ * x_ + 3 * b_ * pow(c_, 2), {var_a_, var_b_, var_c_}};
    ...
    // d/da p = 4abx² + 7c
    const Polynomial p_a{4 * a_ * b_ * pow(x_, 2) + 7 * c_, {var_a_, var_b_, var_c_}};
    EXPECT_PRED2(PolyEqual, p.Differentiate(var_a_), p_a);

    // d/db p = 2a²x² + 3c²x
    const Polynomial p_b{2 * pow(a_, 2) * pow(x_, 2) + 3 * pow(c_, 2) * x_, {var_a_, var_b_, var_c_}};
...
TEST_F(SymbolicPolynomialTest, Integrate) {
    // p = 2a²x²y + 3axy³.
    const Polynomial p{2 * pow(a_, 2) * pow(x_, 2) * y_ + 3 * a_ * x_ * pow(y_, 3), {var_x_, var_y_}};

    // ∫ p dx = 2/3 a²x³y + 3/2 ax²y³.
    const Polynomial int_p_dx{2 * pow(a_, 2) * pow(x_, 3) * y_ / 3 + 3 * a_ * pow(x_, 2) * pow(y_, 3) / 2,
                              {var_x_, var_y_}};
    EXPECT_PRED2(PolyEqual, p.Integrate(var_x_), int_p_dx);

    // ∫ p dx from 1 to 3 = 52/3 a²y + 12 ay³.
    const Polynomial def_int_p_dx{52 * pow(a_, 2) * y_ / 3 + 12 * a_ * pow(y_, 3), {var_y_}};
    EXPECT_PRED2(PolyEqual, p.Integrate(var_x_, 1, 3), def_int_p_dx);
    // ∫ from [a,b] = -∫ from [b,a]
    EXPECT_PRED2(PolyEqual, p.Integrate(var_x_, 3, 1), -def_int_p_dx);

    // ∫ p dy = a²x²y² + 3/4 axy⁴.
    const Polynomial int_p_dy{pow(a_, 2) * pow(x_, 2) * pow(y_, 2) + 3 * a_ * x_ * pow(y_, 4) / 4, {var_x_, var_y_}};
    EXPECT_PRED2(PolyEqual, p.Integrate(var_y_), int_p_dy);

    // ∫ p dz = 2a²x²yz + 3axy³z.
    const Polynomial int_p_dz{2 * pow(a_, 2) * pow(x_, 2) * y_ * z_ + 3 * a_ * x_ * pow(y_, 3) * z_,
    ...
    const Polynomial def_int_p_dz{4 * pow(a_, 2) * pow(x_, 2) * y_ + 6 * a_ * x_ * pow(y_, 3),
                                  {var_x_, var_y_, var_z_}};
    EXPECT_TRUE(p.Integrate(var_z_).indeterminates().include(var_z_));
...
TEST_F(SymbolicPolynomialTest, ConstructNonPolynomialCoefficients) {
    ...
    // sin(a)x = sin(a) * x
    testcases.emplace_back(sin(a_) * x_, Polynomial::MapType{{{Monomial{x_}, sin(a_)}}});

    // cos(a)(x + 1)² = cos(a) * x² + 2cos(a) * x + cos(a) * 1
    testcases.emplace_back(cos(a_) * pow(x_ + 1, 2), Polynomial::MapType{{{Monomial{{{var_x_, 2}}}, cos(a_)},
                                                                          {Monomial{x_}, 2 * cos(a_)},
                                                                          {Monomial{}, cos(a_)}}});

    //   log(a)(x + 1)² / sqrt(b)
    // = log(a)/sqrt(b) * x² + 2log(a)/sqrt(b) * x + log(a)/sqrt(b) * 1
    testcases.emplace_back(log(a_) * pow(x_ + 1, 2) / sqrt(b_),
    ...
    //   (tan(a)x + 1)²
    // = (tan(a))² * x² + 2tan(a) * x + 1
    testcases.emplace_back(pow(tan(a_) * x_ + 1, 2), Polynomial::MapType{{{Monomial{{{var_x_, 2}}}, pow(tan(a_), 2)},
    ...
    testcases.emplace_back(
            ...
            Polynomial::MapType{{{Monomial{x_}, abs(b_ + 1) - atan(c_)}, {Monomial{}, asin(a_) + acos(a_)}}});

    //   atan(b)x * atan2(a, c)y
    // = (atan(b) * atan2(a, c)) * xy
    testcases.emplace_back(abs(b_) * x_ * atan2(a_, c_) * y_,
    ...
    testcases.emplace_back((sinh(a_) * x_ + cosh(b_) * y_ + tanh(c_) * z_) / (5 * min(a_, b_) * max(b_, c_)),
                           Polynomial::MapType{{{
                                                        Monomial{x_},
    ...
    testcases.emplace_back(pow(ceil(a_) * x_ + floor(b_) * y_, 2),
                           Polynomial::MapType{{{Monomial{{{var_x_, 2}}}, ceil(a_) * ceil(a_)},
    ...
    testcases.emplace_back(uf1 * x_ * uf2 * x_, Polynomial::MapType{{{Monomial{{{var_x_, 2}}}, uf1 * uf2}}});

    //   (x + y)² = x² + 2xy + y²
    testcases.emplace_back(pow(x_ + y_, 2), Polynomial::MapType{{
                                                    {Monomial{{{var_x_, 2}}}, 1},
    ...
    // pow(pow(x, 2.5), 2) = x⁵
    testcases.emplace_back(pow(pow(x_, 2.5), 2), Polynomial::MapType{{{Monomial{{{var_x_, 5}}}, 1}}});

    // pow(pow(x * y, 2.5), 2) = (xy)⁵
    testcases.emplace_back(pow(pow(x_ * y_, 2.5), 2), Polynomial::MapType{{{Monomial{{{var_x_, 5}, {var_y_, 5}}}, 1}}});
...
TEST_F(SymbolicPolynomialTest, PartialEvaluate4) {
    ...
    const Polynomial p{((a_ + c_) / (b_ + c_)) * x_ * x_ + b_ * x_ + c_, var_xyz_};
...
TEST_F(SymbolicPolynomialTest, EvaluateIndeterminates) {
  ...
  indeterminates_values << 1, 2, 3, 4,
                           ...
                           2, 3, 4, 5;
    // clang-format on
    Eigen::VectorXd polynomial_values = p.EvaluateIndeterminates(indeterminates, indeterminates_values);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(p_exception.EvaluateIndeterminates(indeterminates, indeterminates_values),
...
TEST_F(SymbolicPolynomialTest, EvaluateWithAffineCoefficients) {
  ...
  indeterminates_values << 1, 2, 3,
    ...
    Eigen::VectorXd b;
    {
        const Polynomial p((a_ + 1) * var_x_ * var_x_ + b_ * var_x_ * var_y_ + c_, var_xy_);
        p.EvaluateWithAffineCoefficients(indeterminates, indeterminates_values, &A, &decision_variables, &b);
...
TEST_F(SymbolicPolynomialTest, SubstituteAndExpandTest) {
    ...
    indeterminate_substitution.emplace(var_y_, Polynomial(var_a_ * var_b_ + var_a_));
    // A substitution with powers of a variable
    indeterminate_substitution.emplace(var_z_, Polynomial(pow(var_a_, 3) + 2 * var_a_ + 1));

    // A polynomial with only linear monomials.
    const Polynomial poly0{x_ + y_};
    const Polynomial sub0 = poly0.SubstituteAndExpand(indeterminate_substitution, &substitutions_cached_data);
    ...
    EXPECT_TRUE(substitutions_cached_data.get_data()->at(Monomial()).EqualTo(Polynomial(1)));

    const Polynomial poly1{2 * x_ * x_};
    const Polynomial sub1 = poly1.SubstituteAndExpand(indeterminate_substitution, &substitutions_cached_data);
    ...
    EXPECT_TRUE(substitutions_cached_data.get_data()->at(Monomial(x_ * x_)).EqualTo(Polynomial(4 * a_ * a_)));

    const Polynomial poly2{x_ * x_ * y_};
    const Polynomial sub2 = poly2.SubstituteAndExpand(indeterminate_substitution, &substitutions_cached_data);
    ...
    const Polynomial sub3 = poly3.SubstituteAndExpand(indeterminate_substitution, &substitutions_cached_data);
    const Polynomial sub3_expected{pow(2 * a_, 3) * (pow(var_a_, 3) + 2 * var_a_ + 1)};
    ...
    EXPECT_TRUE(substitutions_cached_data.get_data()->at(Monomial(pow(x_, 3) * z_)).EqualTo(sub3_expected));
    ...
    const Polynomial::MapType sub4_expected_map{{Monomial(a_), pow(cos(z_), 2) + 2 * cos(z_) + 1}};
...
TEST_F(SymbolicPolynomialTest, CoefficientsAlmostEqual) {
    ...
    EXPECT_TRUE(p1.CoefficientsAlmostEqual(Polynomial{(1 + 1e-7) * x_ * x_}, 1e-6));
    ...
    EXPECT_TRUE(p1.CoefficientsAlmostEqual(Polynomial{(1. - 1e-7) * x_ * x_ + 1e-7}, 1e-6));
    EXPECT_FALSE(p1.CoefficientsAlmostEqual(Polynomial{(1. + 2e-6) * x_ * x_ + 1e-7}, 1e-6));

    // Another polynomial with decision variables in the coefficient.
    const symbolic::Polynomial p2(a_ * x_ * x_, {indeterminates_});
    EXPECT_TRUE(p2.CoefficientsAlmostEqual(Polynomial{(a_ + 1e-7) * x_ * x_, {indeterminates_}}, 1e-6));
    EXPECT_FALSE(p2.CoefficientsAlmostEqual(Polynomial{(a_ + 1e-7) * x_ * x_, {indeterminates_}}, 1e-8));
}

TEST_F(SymbolicPolynomialTest, RemoveTermsWithSmallCoefficients) {
    // Single term.
    Polynomial p1{1e-5 * x_ * x_};
    EXPECT_PRED2(PolyEqual, p1.RemoveTermsWithSmallCoefficients(1E-4), Polynomial(0));
    EXPECT_PRED2(PolyEqual, p1.RemoveTermsWithSmallCoefficients(1E-5), Polynomial(0));
    EXPECT_PRED2(PolyEqual, p1.RemoveTermsWithSmallCoefficients(1E-6), p1);

    // Multiple terms.
    Polynomial p2(2 * x_ * x_ + 3 * x_ * y_ + 1E-4 * x_ - 1E-4);
    EXPECT_PRED2(PolyEqual, p2.RemoveTermsWithSmallCoefficients(1E-4), Polynomial(2 * x_ * x_ + 3 * x_ * y_));
    ...
    EXPECT_PRED2(PolyEqual, Polynomial(p3_map).RemoveTermsWithSmallCoefficients(1E-3), Polynomial(p3_expected_map));
}

TEST_F(SymbolicPolynomialTest, EqualTo) {
    const Polynomial p1{var_x_};
    EXPECT_PRED2(PolyEqual, p1, Polynomial(var_x_));
    EXPECT_PRED2(PolyEqual, Polynomial(var_a_ * var_x_, var_xy_), Polynomial(var_x_ * var_a_, var_xy_));
    EXPECT_PRED2(test::PolyNotEqual, Polynomial(var_a_ * var_x_, var_xy_), Polynomial(var_b_ * var_x_, var_xy_));
}
...
TEST_F(SymbolicPolynomialTest, IsEvenOdd) {
    ...
    // p = 1 + (a+1)*x*y
    p = symbolic::Polynomial{{{Monomial(), 1},
    ...
    // p = a * x  + 2 * x²y
    p = symbolic::Polynomial{{{Monomial(), pow(a_, 2) - 1 - (a_ + 1) * (a_ - 1)},
                              {Monomial(var_x_), a_},
...
TEST_F(SymbolicPolynomialTest, Roots) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(p.Roots(), ".* only supports polynomials with constant coefficients.*");
    ...
    EXPECT_TRUE(CompareMatrices(roots.real(), Eigen::Vector3d{1.23, 4.56, 4.56}, 1e-7));
    ...
    roots = sorted(p.Roots());
    EXPECT_TRUE(CompareMatrices(roots.real(), Eigen::Vector4d{-1, 0, 0, 1}, 1e-7));
    EXPECT_TRUE(CompareMatrices(roots.imag(), Eigen::Vector4d{0, -1, 1, 0}, 1e-7));

    // Leading coefficient is not 1.
    p = symbolic::Polynomial{(2.1 * x_ - 1.23) * (x_ - 4.56)};
    roots = sorted(p.Roots());
    EXPECT_TRUE(CompareMatrices(roots.real(), Eigen::Vector2d{1.23 / 2.1, 4.56}, 1e-7));
...
TEST_F(SymbolicPolynomialTest, Expand) {
    ...
    const symbolic::Polynomial p1{{{symbolic::Monomial(), a_}, {symbolic::Monomial(var_x_, 2), a_ + 1}}};
    ...
    const symbolic::Polynomial p2{
            {{symbolic::Monomial(), (a_ + 2) * (b_ + 3)}, {symbolic::Monomial(var_x_, 2), pow(a_ + 1, 2)}}};
    const symbolic::Polynomial p2_expanded = p2.Expand();
    EXPECT_EQ(p2_expanded, symbolic::Polynomial(p2.ToExpression().Expand(), symbolic::Variables({var_x_})));

    // p3 is 0 after expansion.
    const symbolic::Polynomial p3{{{symbolic::Monomial(), (a_ + 1) * (a_ - 1) - pow(a_, 2) + 1}}};
...
TEST_F(SymbolicPolynomialTest, CalcPolynomialWLowerTriangularPart) {
    ...
    const Vector2<symbolic::Monomial> monomial_basis1(symbolic::Monomial({{var_x_, 2}, {var_y_, 1}}),
                                                      symbolic::Monomial({{var_x_, 1}}));
    Eigen::Vector3d Q1_lower(1, 2, 3);
    const auto poly1 = CalcPolynomialWLowerTriangularPart(monomial_basis1, Q1_lower);
    EXPECT_PRED2(
            test::PolyEqual, poly1,
            symbolic::Polynomial(pow(var_x_, 4) * pow(var_y_, 2) + 4 * pow(var_x_, 3) * var_y_ + 3 * var_x_ * var_x_));

    // Q is a matrix of symbolic variable.
    const Vector3<symbolic::Monomial> monomial_basis2(symbolic::Monomial({{var_x_, 2}}),
    ...
    const auto poly2 = CalcPolynomialWLowerTriangularPart(monomial_basis2, Q2_lower);
    const symbolic::Polynomial poly2_expected =
            symbolic::Polynomial(Q2_lower(0) * pow(var_x_, 4) + 2 * Q2_lower(1) * pow(var_x_, 3) * var_y_ +
                                         (2 * Q2_lower(2) + Q2_lower(3)) * pow(var_x_, 2) * pow(var_y_, 2) +
                                         2 * Q2_lower(4) * var_x_ * pow(var_y_, 3) + Q2_lower(5) * pow(var_y_, 4),
                                 var_xy_);
    ...
    const auto poly3 = CalcPolynomialWLowerTriangularPart(monomial_basis2, Q3_lower);
    const symbolic::Polynomial poly3_expected = symbolic::Polynomial(
            var_a_ * pow(var_x_, 4) + 2 * pow(var_x_, 3) * var_y_ +
                    (2 * var_a_ + 2 * var_b_ + 2) * pow(var_x_, 2) * pow(var_y_, 2) + var_a_ * var_b_ * pow(var_y_, 4),
            var_xy_);
