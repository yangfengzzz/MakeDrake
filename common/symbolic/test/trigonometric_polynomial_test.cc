#include "common/symbolic/trigonometric_polynomial.h"
...
GTEST_TEST(SymbolicLatex, BasicTest) {
    ...
    EXPECT_PRED2(ExprEqual, Substitute(sin(x + z), subs), sx * cos(z) + cx * sin(z));
    EXPECT_PRED2(ExprEqual, Substitute(sin(z + x), subs), sx * cos(z) + cx * sin(z));
    EXPECT_PRED2(ExprEqual, Substitute(sin(x + 0.15), subs), sx * cos(0.15) + cx * sin(0.15));
    EXPECT_PRED2(ExprEqual, Substitute(cos(x + y), subs), cx * cy - sx * sy);
    EXPECT_PRED2(ExprEqual, Substitute(cos(x + z), subs), cx * cos(z) - sx * sin(z));
    EXPECT_PRED2(ExprEqual, Substitute(cos(z + x), subs), cx * cos(z) - sx * sin(z));
    EXPECT_PRED2(ExprEqual, Substitute(cos(x + 0.15), subs), cx * cos(0.15) - sx * sin(0.15));
    EXPECT_PRED2(ExprEqual, Substitute(2 + 3 * x + 4 * y, subs), 2 + 3 * x + 4 * y);
    // Multiplication.
    EXPECT_PRED2(ExprEqual, Substitute(sin(2 * x), subs), 2 * sx * cx);
    EXPECT_PRED2(ExprEqual, Substitute(sin(-2 * x), subs), -2 * sx * cx);
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(2.4 * x), subs), ".*only support sin.* where c is an integer.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(x * x), subs), ".*'status == kNotSinCos' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(x * y), subs), ".*only support sin.* where c is an integer.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(x * z), subs), ".*only support sin.* where c is an integer.*");
    EXPECT_PRED2(ExprEqual, Substitute(cos(2 * x), subs), cx * cx - sx * sx);
    EXPECT_PRED2(ExprEqual, Substitute(cos(-2 * x), subs), cx * cx - sx * sx);
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(2.4 * x), subs), ".*only support cos.* where c is an integer.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(x * x), subs), ".*'status == kNotSinCos' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(x * y), subs), ".*only support cos.* where c is an integer.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(x * z), subs), ".*only support cos.* where c is an integer.*");
    EXPECT_PRED2(ExprEqual, Substitute(x * sin(x) * cos(x), subs), x * sx * cx);
    EXPECT_PRED2(ExprEqual, Substitute(sin(2 * x + y), subs), 2 * sx * cx * cy + (cx * cx - sx * sx) * sy);
    // Other Expressions.
    const auto TestUnary = [&x, &sx, &cx, &subs](const std::function<Expression(const Expression&)>& pred) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(pred(x)), subs), ".*'status == kNotSinCos' failed.*");
        DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(pred(x)), subs), ".*'status == kNotSinCos' failed.*");
    };
    const auto TestBinary = [&x, &y, &sx, &cx, &sy, &cy,
                             &subs](const std::function<Expression(const Expression&, const Expression&)>& pred) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(pred(x, y)), subs), ".*'status == kNotSinCos' failed.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(x / y), subs), ".*'status == kNotSinCos' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(x / y), subs), ".*'status == kNotSinCos' failed.*");
    TestUnary(abs);
    TestUnary(log);
    TestUnary(exp);
    TestUnary(sqrt);
    EXPECT_PRED2(ExprEqual, Substitute(tan(x), subs), sx / cx);
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(tan(x)), subs), ".*'status == kNotSinCos' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(tan(x)), subs), ".*'status == kNotSinCos' failed.*");
    TestUnary(asin);
    TestUnary(acos);
    TestUnary(atan);
    TestBinary(atan2);
    ...
    TestUnary(ceil);
    TestUnary(floor);
    EXPECT_PRED2(ExprEqual, Substitute(if_then_else(z > 1, z, 1), subs), if_then_else(z > 1, z, 1));
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(if_then_else(x > y, x, y), subs),
                                "Substituting sin/cos into formulas is not supported yet");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(if_then_else(x > y, x, y)), subs), ".*'status == kNotSinCos' failed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(if_then_else(x > y, x, y)), subs), ".*'status == kNotSinCos' failed.*");

    // Limited half-angle support.
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(sin(0.5 * x), subs), ".*only support sin.* where c is an integer.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Substitute(cos(0.5 * x), subs), ".*only support cos.* where c is an integer.*");
...
GTEST_TEST(SymbolicLatex, HalfAngleTest) {
    ...
    EXPECT_PRED2(ExprEqual, Substitute(sin(2 * x), subs), 4 * sx * cx * (1 - 2 * sx * sx));
    ...
    EXPECT_PRED2(ExprEqual, m2(1, 1), (2 * sx * cx) * (2 * cy * cy - 1) + (2 * sy * cy) * (1 - 2 * sx * sx));
}

void CheckSubstituteStereographicProjection(const symbolic::Polynomial& e_poly,
    ...
    const symbolic::RationalFunction e_rational2 = SubstituteStereographicProjection(e_poly, sin_cos, t_angle);
    EXPECT_PRED2(symbolic::test::PolyEqualAfterExpansion, e_rational2.numerator(), e_rational_expected.numerator());
    EXPECT_PRED2(symbolic::test::PolyEqualAfterExpansion, e_rational2.denominator(), e_rational_expected.denominator());
}

class SubstituteStereographicProjectionTest : public testing::Test {
public:
    SubstituteStereographicProjectionTest()
        : cos_vars_(3), sin_vars_(3), t_angle_(3), theta_(3), a_("a"), b_("b"), x_("x"), y_("y") {
        for (int i = 0; i < 3; ++i) {
            theta_(i) = symbolic::Variable("theta" + std::to_string(i));
            cos_vars_(i) = symbolic::Variable("cos(theta(" + std::to_string(i) + "))");
            sin_vars_(i) = symbolic::Variable("sin(theta(" + std::to_string(i) + "))");
...
TEST_F(SubstituteStereographicProjectionTest, SingleTrigonometricPoly) {
    ...
    CheckSubstituteStereographicProjection(
            ...
            symbolic::RationalFunction(symbolic::Polynomial(1 - t_angle_(0) * t_angle_(0)),
                                       symbolic::Polynomial(1 + t_angle_(0) * t_angle_(0))));
    // test sin(theta(0))
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(sin_vars_(0)), sin_cos_, t_angle_,
            symbolic::RationalFunction(symbolic::Polynomial(2 * t_angle_(0)),
                                       symbolic::Polynomial(1 + t_angle_(0) * t_angle_(0))));
}

TEST_F(SubstituteStereographicProjectionTest, One) {
    // test constant polynomial 1.
    CheckSubstituteStereographicProjection(symbolic::Polynomial(1), sin_cos_, t_angle_, symbolic::RationalFunction(1));
}

TEST_F(SubstituteStereographicProjectionTest, ConstantPoly) {
    // test a + b as a constant polynomial.
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial({{symbolic::Monomial(), a_ + b_}}), sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(a_ + b_, t_) /* A constant polynomial with a+b being the constant term */));
}
...
TEST_F(SubstituteStereographicProjectionTest, OnePlusCos) {
    ...
    CheckSubstituteStereographicProjection(
            ...
            symbolic::RationalFunction(symbolic::Polynomial(2), symbolic::Polynomial(1 + t_angle_(0) * t_angle_(0))));
}

TEST_F(SubstituteStereographicProjectionTest, LinearTrigPoly) {
    // test a + b*cos(delta_q(0)) + sin(delta_q(1))
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(a_ + b_ * cos_vars_(0) + sin_vars_(1), cos_sin_vars_), sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(
                            a_ * (1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)) +
                                    b_ * (1 - t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)) +
                                    2 * t_angle_(1) * (1 + t_angle_(0) * t_angle_(0)),
                            t_),
                    symbolic::Polynomial((1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)))));
}

TEST_F(SubstituteStereographicProjectionTest, NonlinearTrigPoly1) {
    // test a + b * cos(theta(0) * sin(theta(1)) + sin(theta(0))
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(a_ + b_ * cos_vars_(0) * sin_vars_(1) + sin_vars_(0), cos_sin_vars_), sin_cos_,
            t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(a_ * (1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)) +
...
TEST_F(SubstituteStereographicProjectionTest, NonlinearTrigPoly2) {
    ...
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(a_ + b_ * cos_vars_(0) * sin_vars_(1) + sin_vars_(0) * cos_vars_(2), cos_sin_vars_),
            sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(
                            a_ * (1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)) *
                                    ...
                                    b_ * (1 - t_angle_(0) * t_angle_(0)) * 2 * t_angle_(1) *
                                            (1 + t_angle_(2) * t_angle_(2)) +
                                    2 * t_angle_(0) * (1 + t_angle_(1) * t_angle_(1)) * (1 - t_angle_(2) * t_angle_(2)),
                            t_),
                    symbolic::Polynomial((1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)) *
                                         (1 + t_angle_(2) * t_angle_(2)))));
}
...
TEST_F(SubstituteStereographicProjectionTest, TAngle) {
    // test t_angle(0)
    CheckSubstituteStereographicProjection(symbolic::Polynomial({{symbolic::Monomial(), t_angle_(0)}}), sin_cos_,
                                           t_angle_, symbolic::RationalFunction(symbolic::Polynomial(t_angle_(0), t_)));
}

TEST_F(SubstituteStereographicProjectionTest, TAngleTimesCos) {
    // test t_angle(0) * cos(theta(0))
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(t_angle_(0) * cos_vars_(0), cos_sin_vars_), sin_cos_, t_angle_,
            symbolic::RationalFunction(symbolic::Polynomial(t_angle_(0) - t_angle_(0) * t_angle_(0) * t_angle_(0), t_),
...
TEST_F(SubstituteStereographicProjectionTest, TAngleTimesSin) {
    ...
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(t_angle_(0) * sin_vars_(0), cos_sin_vars_), sin_cos_, t_angle_,
            symbolic::RationalFunction(symbolic::Polynomial(2 * t_angle_(0) * t_angle_(0), t_),
                                       symbolic::Polynomial(1 + t_angle_(0) * t_angle_(0), t_)));
}
...
TEST_F(SubstituteStereographicProjectionTest, NonlinearTrigPolyWithT) {
    ...
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(
                    (t_angle_(0) * a_ + t_angle_(1) * b_) * sin_vars_(0) * cos_vars_(1) + 2 * t_angle_(0) * b_,
                    cos_sin_vars_),
            sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(
                            (a_ * t_angle_(0) + b_ * t_angle_(1)) * 2 * t_angle_(0) * (1 - t_angle_(1) * t_angle_(1)) +
                                    2 * t_angle_(0) * b_ * (1 + t_angle_(0) * t_angle_(0)) *
                                            (1 + t_angle_(1) * t_angle_(1)),
                            t_),
                    symbolic::Polynomial((1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)), t_)));
}
...
TEST_F(SubstituteStereographicProjectionTest, XTimesSin) {
    ...
    CheckSubstituteStereographicProjection(
            ...
            symbolic::RationalFunction(symbolic::Polynomial(2 * x_ * t_angle_(0)),
                                       symbolic::Polynomial(1 + t_angle_(0) * t_angle_(0))));
}
...
TEST_F(SubstituteStereographicProjectionTest, NonlinearTrigPoly3) {
    ...
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(a_ * x_ * sin_vars_(0) + b_ * x_ * y_ * sin_vars_(0) * cos_vars_(1), xy_sin_cos_),
            sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(a_ * x_ * 2 * t_angle_(0) * (1 + t_angle_(1) * t_angle_(1)) +
                                                 b_ * x_ * y_ * 2 * t_angle_(0) * (1 - t_angle_(1) * t_angle_(1)),
                                         xyt_),
                    symbolic::Polynomial((1 + t_angle_(0) * t_angle_(0)) * (1 + t_angle_(1) * t_angle_(1)))));
}
...
TEST_F(SubstituteStereographicProjectionTest, HighDegreeTrigPoly1) {
    ...
    CheckSubstituteStereographicProjection(
            ...
            symbolic::RationalFunction(symbolic::Polynomial(4 * t_angle_(0) * t_angle_(0)),
                                       symbolic::Polynomial(pow(1 + t_angle_(0) * t_angle_(0), 2))));

    // (cos(theta(0)))²
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(cos_vars_(0) * cos_vars_(0)), sin_cos_, t_angle_,
            symbolic::RationalFunction(symbolic::Polynomial(pow(1 - t_angle_(0) * t_angle_(0), 2)),
                                       symbolic::Polynomial(pow(1 + t_angle_(0) * t_angle_(0), 2))));

    // sin(theta(0))*cos(theta(0))
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(sin_vars_(0) * cos_vars_(0)), sin_cos_, t_angle_,
            symbolic::RationalFunction(symbolic::Polynomial(2 * t_angle_(0) * (1 - t_angle_(0) * t_angle_(0))),
                                       symbolic::Polynomial(pow(1 + t_angle_(0) * t_angle_(0), 2))));
}
...
TEST_F(SubstituteStereographicProjectionTest, HighDegreeTrigPoly2) {
    ...
    const symbolic::Polynomial numerator(4 * t_angle_(0) * t_angle_(0) * (1 - t_angle_(1) * t_angle_(1)) *
                                                 (1 + t_angle_(1) * t_angle_(1)) * (1 + t_angle_(2) * t_angle_(2)) +
                                         3 * (1 - t_angle_(0) * t_angle_(0)) * (1 + t_angle_(0) * t_angle_(0)) * 2 *
                                                 t_angle_(1) * (1 - t_angle_(1) * t_angle_(1)) *
                                                 (1 - t_angle_(2) * t_angle_(2)));
    const symbolic::Polynomial denominator(pow(1 + t_angle_(0) * t_angle_(0), 2) *
                                           pow(1 + t_angle_(1) * t_angle_(1), 2) * (1 + t_angle_(2) * t_angle_(2)));
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(sin_vars_(0) * sin_vars_(0) * cos_vars_(1) +
                                 3 * cos_vars_(0) * sin_vars_(1) * cos_vars_(1) * cos_vars_(2)),
...
TEST_F(SubstituteStereographicProjectionTest, NonlinearTrigPoly4) {
    // a * x * t0 * cos(theta1)
    CheckSubstituteStereographicProjection(
            symbolic::Polynomial(a_ * x_ * t_angle_(0) * cos_vars_(1), xy_sin_cos_), sin_cos_, t_angle_,
            symbolic::RationalFunction(
                    symbolic::Polynomial(a_ * x_ * t_angle_(0) * (1 - t_angle_(1) * t_angle_(1)), xyt_),
