...
public:
    BilinearProductTest()
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                xy_(i, j) = Expression(Variable("xy(" + std::to_string(i) + "," + std::to_string(j) + ")"));
            }
        }

        for (int i = 0; i < 3; ++i) {
            for (int j = i; j < 3; ++j) {
                xx_(i, j) = Expression(Variable("xx(" + std::to_string(i) + "," + std::to_string(j) + ")"));
            }
        }
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < i; ++j) {
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                Z1_(i, j) = Expression(Variable("Z1(" + std::to_string(i) + "," + std::to_string(j) + ")"));
                Z2_(i, j) = Expression(Variable("Z2(" + std::to_string(i) + "," + std::to_string(j) + ")"));
            }
        }
    }
...
TEST_F(BilinearProductTest, ConstantTerm) {
    const std::vector<Expression> expressions{0, 1, z_(0) + z_(1), pow(z_(0), 3) + z_(0) * z_(1)};
...
TEST_F(BilinearProductTest, LinearTerm) {
    const std::vector<Expression> expressions{x_(0), y_(0), x_(0) + y_(1) + z_(0) + 2,
...
TEST_F(BilinearProductTest, WIsExpression0) {
    EXPECT_PRED2(ExprEqual, ReplaceBilinearTerms(x_(0) * y_(0), x_.head<3>(), y_.head<3>(), Z1_ - Z2_),
                 Z1_(0, 0) - Z2_(0, 0));
}
...
TEST_F(BilinearProductTest, WIsExpression2) {
    EXPECT_PRED2(
            ExprEqual,
            (ReplaceBilinearTerms(x_(0) * y_(1) + 2 * x_(0) * y_(0) + Z1_(0, 0), x_.head<3>(), y_.head<3>(), Z1_ - Z2_))
                    .Expand(),
...
TEST_F(BilinearProductTest, WIsExpression3) {
    ...
    W(0, 1) += x_(0);
    Expression dummy;
    EXPECT_THROW(dummy = ReplaceBilinearTerms(x_(0) + x_(0) * y_(1), x_, y_, W).Expand(), std::runtime_error);
}

TEST_F(BilinearProductTest, DuplicateEntry) {
    // x_duplicate contains duplicate entries.
    Vector2<Variable> x_duplicate(x_(0), x_(0));
    EXPECT_THROW(ReplaceBilinearTerms(x_(0) * x_(0), x_duplicate, x_duplicate, xx_.block<2, 2>(0, 0)),
                 std::runtime_error);
}
}  // namespace
}  // namespace symbolic
