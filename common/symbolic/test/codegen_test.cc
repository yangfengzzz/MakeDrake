#include "common/symbolic/codegen.h"
...
string MakeScalarFunctionCode(const string& function_name, const int n, const string& e) {
...
string MakeDenseMatrixFunctionCode(
        const string& function_name, const int in, const int rows, const int cols, const vector<string>& expressions) {
    ostringstream oss;
    // Main function f.
...
TEST_F(SymbolicCodeGenTest, Variable) {
    ...
    EXPECT_EQ(CodeGen("f", {x_, y_, z_}, z_), MakeScalarFunctionCode("f", 3, "p[2]"));
}

TEST_F(SymbolicCodeGenTest, Constant) {
    EXPECT_EQ(CodeGen("f", {}, 3.141592), MakeScalarFunctionCode("f", 0, "3.141592"));
}
...
TEST_F(SymbolicCodeGenTest, Multiplication) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, 2.0 * 3.0 * x_ * x_ * -7.0 * y_ * y_ * y_),
              MakeScalarFunctionCode("f", 2, "(-42 * pow(p[0], 2.000000) * pow(p[1], 3.000000))"));
}
...
TEST_F(SymbolicCodeGenTest, Division) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, (2 + x_) / (3 * y_)), MakeScalarFunctionCode("f", 2, "((2 + p[0]) / (3 * p[1]))"));
}

TEST_F(SymbolicCodeGenTest, Abs) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, abs(2 + x_)), MakeScalarFunctionCode("f", 2, "fabs((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Log) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, log(2 + x_)), MakeScalarFunctionCode("f", 2, "log((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Exp) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, exp(2 + x_)), MakeScalarFunctionCode("f", 2, "exp((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Sqrt) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, sqrt(2 + x_)), MakeScalarFunctionCode("f", 2, "sqrt((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Sin) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, sin(2 + x_)), MakeScalarFunctionCode("f", 2, "sin((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Cos) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, cos(2 + x_)), MakeScalarFunctionCode("f", 2, "cos((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Tan) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, tan(2 + x_)), MakeScalarFunctionCode("f", 2, "tan((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Asin) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, asin(2 + x_)), MakeScalarFunctionCode("f", 2, "asin((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Acos) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, acos(2 + x_)), MakeScalarFunctionCode("f", 2, "acos((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Atan) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, atan(2 + x_)), MakeScalarFunctionCode("f", 2, "atan((2 + p[0]))"));
}
...
TEST_F(SymbolicCodeGenTest, Sinh) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, sinh(2 + x_)), MakeScalarFunctionCode("f", 2, "sinh((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Cosh) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, cosh(2 + x_)), MakeScalarFunctionCode("f", 2, "cosh((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Tanh) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, tanh(2 + x_)), MakeScalarFunctionCode("f", 2, "tanh((2 + p[0]))"));
}
...
TEST_F(SymbolicCodeGenTest, Ceil) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, ceil(2 + x_)), MakeScalarFunctionCode("f", 2, "ceil((2 + p[0]))"));
}

TEST_F(SymbolicCodeGenTest, Floor) {
    EXPECT_EQ(CodeGen("f", {x_, y_}, floor(2 + x_)), MakeScalarFunctionCode("f", 2, "floor((2 + p[0]))"));
}
...
TEST_F(SymbolicCodeGenTest, DenseMatrixRowMajor) {
    ...
    EXPECT_EQ(CodeGen("f", {x_, y_, z_, w_}, M),
              MakeDenseMatrixFunctionCode("f", 4 /* number of input parameters */, 3 /* number of rows */,
...
TEST_F(SymbolicCodeGenTest, DenseMatrixExampleInDocumentation) {
    ...
    EXPECT_EQ(CodeGen("f", {x_, y_}, M),
              MakeDenseMatrixFunctionCode("f", 2 /* number of input parameters */, 2 /* number of rows */,
                                          2 /* number of columns */, expected));
}
...
void f(const double* p, int* outer_indices, int* inner_indices, double* values) {
    outer_indices[0] = 0;
    outer_indices[1] = 1;
    outer_indices[2] = 1;
    outer_indices[3] = 2;
...
TEST_F(SymbolicCodeGenTest, SparseMatrixColMajorExampleUsingEigenMap) {
    ...
    Eigen::Map<Eigen::SparseMatrix<double, Eigen::ColMajor>> map_sp(
            meta.m.rows, meta.m.cols, meta.m.non_zeros, outer_indices.data(), inner_indices.data(), values.data());
