#include "common/symbolic/chebyshev_basis_element.h"

#include <fmt/format.h>
#include <gtest/gtest.h>

#include "common/symbolic/chebyshev_polynomial.h"
...
TEST_F(SymbolicChebyshevBasisElementTest, Constructor) {
    ...
    const ChebyshevBasisElement b5{Vector2<Variable>(x_, y_), Eigen::Vector2i(2, 1)};
    ...
    const ChebyshevBasisElement b6{Vector2<Variable>(x_, y_), Eigen::Vector2i(2, 0)};
    EXPECT_EQ(b6.var_to_degree_map().size(), 1);
    EXPECT_EQ(b6.var_to_degree_map().at(x_), 2);
    EXPECT_EQ(b6.total_degree(), 2);

    DRAKE_EXPECT_THROWS_MESSAGE(ChebyshevBasisElement(Vector2<Variable>(x_, x_), Eigen::Vector2i(2, 1)),
                                ".*x is repeated");

    DRAKE_EXPECT_THROWS_MESSAGE(ChebyshevBasisElement(Vector2<Variable>(x_, y_), Eigen::Vector2i(2, -1)),
...
TEST_F(SymbolicChebyshevBasisElementTest, Evaluate) {
    ...
    EXPECT_EQ(ChebyshevBasisElement({{x_, 1}, {y_, 2}, {z_, 3}}).Evaluate(env), 2 * 17 * 244);
}

TEST_F(SymbolicChebyshevBasisElementTest, multiply) {
    const auto result1 = ChebyshevBasisElement({{x_, 1}}) * ChebyshevBasisElement({{y_, 2}});
    EXPECT_EQ(result1.size(), 1);
    EXPECT_EQ(result1.at(ChebyshevBasisElement({{x_, 1}, {y_, 2}})), 1.);

    const auto result2 = ChebyshevBasisElement({{x_, 1}}) * ChebyshevBasisElement({{x_, 2}});
    EXPECT_EQ(result2.size(), 2);
    EXPECT_EQ(result2.at(ChebyshevBasisElement({{x_, 3}})), 0.5);
    EXPECT_EQ(result2.at(ChebyshevBasisElement({{x_, 1}})), 0.5);

    const auto result3 = ChebyshevBasisElement({{x_, 1}, {y_, 2}}) * ChebyshevBasisElement({{x_, 2}});
    EXPECT_EQ(result3.size(), 2);
    EXPECT_EQ(result3.at(ChebyshevBasisElement({{x_, 3}, {y_, 2}})), 0.5);
    EXPECT_EQ(result3.at(ChebyshevBasisElement({{x_, 1}, {y_, 2}})), 0.5);

    const auto result4 = ChebyshevBasisElement({{y_, 2}}) * ChebyshevBasisElement({{x_, 1}});
    EXPECT_EQ(result4.size(), 1);
    EXPECT_EQ(result4.at(ChebyshevBasisElement({{x_, 1}, {y_, 2}})), 1.);

    const auto result5 = ChebyshevBasisElement({{x_, 1}, {y_, 3}}) * ChebyshevBasisElement({{x_, 2}, {y_, 1}});
    ...
    const auto result6 = ChebyshevBasisElement({{y_, 2}, {z_, 3}}) * ChebyshevBasisElement({{x_, 3}, {y_, 1}});
    EXPECT_EQ(result6.size(), 2);
    EXPECT_EQ(result6.at(ChebyshevBasisElement({{x_, 3}, {y_, 3}, {z_, 3}})), 0.5);
    EXPECT_EQ(result6.at(ChebyshevBasisElement({{x_, 3}, {y_, 1}, {z_, 3}})), 0.5);

    const auto result7 =
            ChebyshevBasisElement({{x_, 1}, {y_, 2}, {z_, 3}}) * ChebyshevBasisElement({{x_, 2}, {y_, 3}, {z_, 1}});
    EXPECT_EQ(result7.size(), 8);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 3}, {y_, 5}, {z_, 4}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 3}, {y_, 5}, {z_, 2}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 3}, {y_, 1}, {z_, 4}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 3}, {y_, 1}, {z_, 2}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 1}, {y_, 5}, {z_, 4}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 1}, {y_, 5}, {z_, 2}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 1}, {y_, 1}, {z_, 4}})), 0.125);
    EXPECT_EQ(result7.at(ChebyshevBasisElement({{x_, 1}, {y_, 1}, {z_, 2}})), 0.125);
}
...
TEST_F(SymbolicChebyshevBasisElementTest, Differentiate) {
    ...
    const auto result4 = ChebyshevBasisElement({{x_, 2}, {y_, 3}}).Differentiate(x_);
    EXPECT_EQ(result4.size(), 1);
    ...
    const auto result6 = ChebyshevBasisElement({{x_, 3}, {y_, 4}}).Differentiate(x_);
    EXPECT_EQ(result6.size(), 2);
    ...
    const auto result8 = ChebyshevBasisElement({{x_, 4}, {y_, 3}}).Differentiate(x_);
...
TEST_F(SymbolicChebyshevBasisElementTest, ToExpression) {
    EXPECT_PRED2(test::ExprEqual, ChebyshevBasisElement().ToExpression(), Expression(1.));

    EXPECT_PRED2(test::ExprEqual, ChebyshevBasisElement({{x_, 0}}).ToExpression(), Expression(1.));

    EXPECT_PRED2(test::ExprEqual, ChebyshevBasisElement({{x_, 2}}).ToExpression(),
                 ChebyshevPolynomial(x_, 2).ToPolynomial().ToExpression());

    EXPECT_PRED2(test::ExprEqual, ChebyshevBasisElement({{x_, 2}, {y_, 3}}).ToExpression(),
                 ChebyshevPolynomial(x_, 2).ToPolynomial().ToExpression() *
...
TEST_F(SymbolicChebyshevBasisElementTest, EigenMatrix) {
    ...
    M << ChebyshevBasisElement(), ChebyshevBasisElement({{x_, 1}}), ChebyshevBasisElement({{x_, 1}, {y_, 2}}),
...
TEST_F(SymbolicChebyshevBasisElementTest, MergeBasisElementInPlace) {
    ...
    basis_element1.MergeBasisElementInPlace(ChebyshevBasisElement({{x_, 1}, {z_, 2}}));
