...
// types of arguments.
map<Variable, int> ToMonomialPower(const Eigen::Ref<const VectorX<Variable>>& vars,
...
Monomial::Monomial(const Eigen::Ref<const VectorX<Variable>>& vars, const Eigen::Ref<const Eigen::VectorXi>& exponents)
    : total_degree_{exponents.sum()}, powers_{ToMonomialPower(vars, exponents)} {}

Monomial::Monomial(const Variable& var) : total_degree_{1}, powers_{{var, 1}} {}

Monomial::Monomial(const Variable& var, const int exponent) : total_degree_{exponent} {
...
Monomial::Monomial(const map<Variable, int>& powers) : total_degree_{TotalDegree(powers)} {
...
Monomial::Monomial(const Expression& e) : Monomial(ToMonomialPower(e.Expand())) {}
...
bool Monomial::operator==(const Monomial& m) const {
    ...
    for (auto it1 = powers_.begin(), it2 = m.powers_.begin(); it1 != powers_.end(); ++it1, ++it2) {
...
Eigen::VectorXd Monomial::Evaluate(const Eigen::Ref<const VectorX<symbolic::Variable>>& vars,
    ...
    if (static_cast<int>(vars_set.size()) != vars.rows()) {
        throw std::invalid_argument("Monomial::Evaluate(): vars contains repeated variables.");
    }
    for (const auto& [var, degree] : powers_) {
        if (vars_set.find(var) == vars_set.end()) {
            throw std::invalid_argument(fmt::format("Monomial::Evaluate(): {} is not present in vars", var.get_name()));
        }
    }
