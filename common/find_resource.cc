...
#include "common/drake_marker.h"
#include "common/drake_throw.h"
#include "common/find_loaded_library.h"
...
void Result::CheckInvariants() {
    if (resource_path_.empty()) {
        ...
        DRAKE_DEMAND((absolute_path_ == std::nullopt) != (error_message_ == std::nullopt));
    }
...
Result CheckAndMakeResult(const string& root_description, const string& root, const string& resource_path) {
    ...
    if (!fs::is_regular_file({root + "/" + kSentinelRelpath})) {
        return Result::make_error(resource_path,
                                  fmt::format("Could not find Drake resource_path '{}' because {} specified a "
...
std::optional<string> MaybeGetEnvironmentResourceRoot() {
    ...
    if (!fs::is_directory({root})) {
        static const logging::Warn log_once("FindResource ignoring {}='{}' because it does not exist.", env_name,
                                            env_value);
        return std::nullopt;
    }
...
std::optional<string> MaybeGetInstallResourceRoot() {
    ...
    std::optional<string> maybe_libdrake_dir = LoadedLibraryPath("libdrake_marker.so");
    if (maybe_libdrake_dir) {
        ...
        const fs::path canonical_root =
                fs::canonical(libdrake_dir / "libdrake_marker.so").parent_path().parent_path() / "share";
...
Result MakeResultFrom(const string& resource_path, RlocationOrError other) {
    return other.error.empty() ? Result::make_success(resource_path, std::move(other.abspath))
                               : Result::make_error(resource_path, std::move(other.error));
}

}  // namespace

const char* const kDrakeResourceRootEnvironmentVariableName = "DRAKE_RESOURCE_ROOT";
...
Result FindResource(const string& resource_path) {
    ...
    if (!IsRelativePath(resource_path)) {
        return Result::make_error(resource_path,
                                  fmt::format("Drake resource_path '{}' is not a relative path.", resource_path));
    }
    const string prefix("drake/");
    if (!resource_path.starts_with(prefix)) {
        return Result::make_error(
                resource_path, fmt::format("Drake resource_path '{}' does not start with {}.", resource_path, prefix));
    }
    ...
    if (auto guess = MaybeGetEnvironmentResourceRoot()) {
        return CheckAndMakeResult(fmt::format("{} environment variable", kDrakeResourceRootEnvironmentVariableName),
                                  *guess, resource_path);
    }
    ...
    if (HasRunfiles()) {
        if (FindRunfile(kSentinelRelpath).error.empty()) {
            ...
            log()->debug("FindResource ignoring Bazel runfiles with no sentinel file {}.", kSentinelRelpath);
        }
    }

    // (3) Check the `libdrake_marker.so` location in the install tree.
    if (auto guess = MaybeGetInstallResourceRoot()) {
        return CheckAndMakeResult("Drake CMake install marker", *guess, resource_path);
    }

    // No resource roots were found.
    return Result::make_error(resource_path,
                              fmt::format("Could not find Drake resource_path '{}' because no resource roots "
...
std::string ReadFileOrThrow(const std::filesystem::path& path) {
    ...
    if (!result) {
        throw std::runtime_error(fmt::format("Error reading from '{}'", path.string()));
    }
