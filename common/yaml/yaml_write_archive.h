...
private:
    ...
    auto DoVisit(const NVP& nvp,
                 const T&,
                 int32_t) -> decltype(nvp.value()->Serialize(static_cast<YamlWriteArchive*>(nullptr))) {
        return this->VisitSerializable(nvp);
    }

    // This version applies when the type has an ADL Serialize function.
    template <typename NVP, typename T>
    auto DoVisit(const NVP& nvp, const T&, int32_t) -> decltype(Serialize(static_cast<YamlWriteArchive*>(nullptr),
                                                                          nvp.value())) {
        return this->VisitSerializable(nvp);
    }
    ...
    void DoVisit(const NVP& nvp, const std::vector<T>&, int32_t) {
        ...
        this->VisitArrayLike<T>(nvp.name(), data.size(), data.empty() ? nullptr : &data.at(0));
    }
    ...
    template <typename NVP, typename K, typename V, typename Hash, typename KeyEqual, typename Allocator>
    void DoVisit(const NVP& nvp, const std::unordered_map<K, V, Hash, KeyEqual, Allocator>&, int32_t) {
        this->VisitMap<K, V>(nvp);
    }
    ...
    void DoVisit(const NVP& nvp, const Eigen::Matrix<T, Rows, Cols, Options, MaxRows, MaxCols>&, int32_t) {
        if constexpr (Cols == 1) {
            ...
            this->VisitArrayLike<T>(nvp.name(), value.size(), empty ? nullptr : &value.coeffRef(0));
        } else {
    ...
    void VisitMatrix(const char* name, const Eigen::Matrix<T, Rows, Cols, Options, MaxRows, MaxCols>* matrix) {
        auto sub_node = internal::Node::MakeSequence();
