...
GTEST_TEST(YamlJsonTest, WriteNested) {
    OuterStruct data;
    EXPECT_EQ(SaveJsonString(data), R"""({"inner_struct":{"inner_value":1.2345},"outer_value":1.2345})""");
}
...
GTEST_TEST(YamlJsonTest, WriteVariant) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SaveJsonString(data), ".*SaveJsonString.*mapping.*tag.*");
}
