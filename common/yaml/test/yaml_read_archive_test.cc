#include "common/yaml/yaml_read_archive.h"
...
#include "common/yaml/test/example_structs.h"
...
public:
    ...
    static internal::Node Load(std::string contents) {
        ...
        const internal::Node loaded = YamlReadArchive::LoadStringAsNode(contents, std::nullopt);
        if (!loaded.IsMapping()) {
    ...
    // nor Null itself, etc.)
    static internal::Node LoadSingleValue(const std::string& value) { return Load("doc:\n  value: " + value + "\n"); }
    ...
    static Serializable AcceptEmptyDoc() {
        ...
        if (GetParam().allow_cpp_with_no_yaml) {
            ...
            EXPECT_TRUE(raised);
            EXPECT_THAT(what, testing::MatchesRegex(".*missing entry for [^ ]* value.*"));
        }
        return result;
    }
};
...
TEST_P(YamlReadArchiveTest, StdArray) {
    const auto test = [](const std::string& value, const std::array<double, 3>& expected) {
...
TEST_P(YamlReadArchiveTest, StdVector) {
    const auto test = [](const std::string& value, const std::vector<double>& expected) {
...
TEST_P(YamlReadArchiveTest, StdMap) {
    const auto test = [](const std::string& doc, const std::map<std::string, double>& expected) {
...
TEST_P(YamlReadArchiveTest, StdMapWithBadMergeKey) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<MapStruct>(Load(R"""(
...
doc:
  ...
  key_with_defaulted_value: {}
    ...
    if (GetParam().allow_cpp_with_no_yaml) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE((AcceptIntoDummy<std::map<std::string, DoubleStruct>>(node)),
                                    ".*missing entry for double value.*");
    }
}

TEST_P(YamlReadArchiveTest, Optional) {
    const auto test_with_default = [](const std::string& doc, const std::optional<double>& expected) {
    ...
    const auto test_no_default = [](const std::string& doc, const std::optional<double>& expected) {
...
TEST_P(YamlReadArchiveTest, PrimitiveVariant) {
    const auto test = [](const std::string& doc, const PrimitiveVariant& expected) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<PrimitiveVariantStruct>(Load("doc:\n  value: 1.0")),
                                ".*vector.*double.*");
}
...
TEST_P(YamlReadArchiveTest, EigenVector) {
    const auto test = [](const std::string& value, const Eigen::VectorXd& expected) {
        ...
        const auto& upto3 = AcceptNoThrow<EigenVecUpTo3Struct>(LoadSingleValue(value));
...
TEST_P(YamlReadArchiveTest, EigenMatrix) {
    ...
    const auto test = [](const std::string& doc, const Eigen::MatrixXd& expected) {
...
TEST_P(YamlReadArchiveTest, NestedWithMergeKeys) {
    const auto test = [](const std::string& orig_doc) {
        ...
        if (!GetParam().allow_yaml_with_no_cpp) {
            doc = std::regex_replace(orig_doc, std::regex(" *ignored_key: ignored_value"), "");
        }
...
TEST_P(YamlReadArchiveTest, NestedWithBadMergeKey) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<OuterStruct>(Load(R"""(
_template: &template 99.0
...
doc:
  ...
  inner_struct:
    ...
    if (GetParam().allow_cpp_with_no_yaml && GetParam().allow_yaml_with_no_cpp) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<OuterStruct>(node),
                                    "<string>:4:5:"
                                    " YAML node of type Mapping"
...
TEST_P(YamlReadArchiveTest, VisitArrayFoundNothing) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<ArrayStruct>(LoadSingleValue("")),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
TEST_P(YamlReadArchiveTest, VisitArrayFoundScalar) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<ArrayStruct>(LoadSingleValue("1.0")),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
TEST_P(YamlReadArchiveTest, VisitArrayFoundStruct) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<ArrayStruct>(Load(R"""(
doc:
  value:
    inner_value: 1.0
)""")),
...
TEST_P(YamlReadArchiveTest, VisitVectorFoundNothing) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<VectorStruct>(LoadSingleValue("")),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
TEST_P(YamlReadArchiveTest, VisitVectorFoundScalar) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<VectorStruct>(LoadSingleValue("1.0")),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
TEST_P(YamlReadArchiveTest, VisitVectorFoundStruct) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<VectorStruct>(Load(R"""(
doc:
  value:
    inner_value: 1.0
)""")),
...
TEST_P(YamlReadArchiveTest, VisitOptionalScalarFoundSequence) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<OptionalStruct>(LoadSingleValue("[1.0]")),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
TEST_P(YamlReadArchiveTest, VisitVariantFoundNoTag) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<VariantWrappingStruct>(Load("doc:\n  inner:\n    value:")),
                                "<string>:3:5:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
    ...
    const auto& str = AcceptNoThrow<VariantWrappingStruct>(Load("doc:\n  inner:\n    value: foo"));
...
TEST_P(YamlReadArchiveTest, VisitVariantFoundUnknownTag) {
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<VariantStruct>(Load("doc:\n  value: !UnknownTag foo")),
                                "<string>:2:3: "
                                "YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\) "
...
TEST_P(YamlReadArchiveTest, VisitEigenFoundNothing) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenVecStruct>(LoadSingleValue(value)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
                                " has non-Sequence \\(Null\\) entry for Eigen::VectorXd value\\.");
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenVec3Struct>(LoadSingleValue(value)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
                                " has non-Sequence \\(Null\\) entry for Eigen::Vector3d value\\.");
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenMatrixStruct>(LoadSingleValue(value)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
                                " has non-Sequence \\(Null\\) entry for Eigen::MatrixXd value\\.");
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenMatrix34Struct>(LoadSingleValue(value)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
doc:
  ...
  - [8.0, 9.0, 0.0]
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenMatrixStruct>(Load(doc)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
                                " has inconsistent cols dimensions entry for Eigen::MatrixXd value\\.");
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<EigenMatrix34Struct>(Load(doc)),
                                "<string>:2:3:"
                                " YAML node of type Mapping \\(with size 1 and keys \\{value\\}\\)"
...
- bar
    ...
    const internal::Node root = YamlReadArchive::LoadStringAsNode(doc, std::nullopt);
    DRAKE_EXPECT_THROWS_MESSAGE(AcceptIntoDummy<OuterStruct>(root),
...
INSTANTIATE_TEST_SUITE_P(AllOptions, YamlReadArchiveTest, ::testing::ValuesIn(MakeAllPossibleOptions()));
