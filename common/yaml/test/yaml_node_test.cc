...
protected:
    ...
    std::string GetExpectedCannot(std::string_view operation) {
        return fmt::format(".*Cannot.*{}.*on a {}.*", operation, GetExpectedTypeString());
    }
};
...
TEST_P(YamlNodeParamaterizedTest, ScalarOps) {
    ...
    if (dut.IsScalar()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut.GetScalar(), GetExpectedCannot("GetScalar"));
    }
}
...
TEST_P(YamlNodeParamaterizedTest, SequenceOps) {
    ...
    if (dut.IsSequence()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut.GetSequence(), GetExpectedCannot("GetSequence"));
        DRAKE_EXPECT_THROWS_MESSAGE(dut.Add(Node::MakeNull()), GetExpectedCannot("Add"));
    }
}
...
TEST_P(YamlNodeParamaterizedTest, MappingOps) {
    ...
    if (dut.IsMapping()) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(dut2.Add("key", Node::MakeScalar()), "Cannot .*Add.* duplicate key 'key'");
        dut2.Remove("key");
        EXPECT_TRUE(dut.GetMapping().empty());
    } else {
        DRAKE_EXPECT_THROWS_MESSAGE(dut.GetMapping(), GetExpectedCannot("GetMapping"));
        DRAKE_EXPECT_THROWS_MESSAGE(dut.Add("key", Node::MakeNull()), GetExpectedCannot("Add"));
