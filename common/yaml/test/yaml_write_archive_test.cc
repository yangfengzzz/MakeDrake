#include "common/yaml/yaml_write_archive.h"
...
public:
    ...
    static std::string Save(const Serializable& data, const std::string& root_name) {
    ...
    static std::string WrapDoc(const std::string& value) { return "doc:\n  value: " + value + "\n"; }
};
...
TEST_F(YamlWriteArchiveTest, StdArray) {
    const auto test = [](const std::array<double, 3>& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, StdVector) {
    const auto test = [](const std::vector<double>& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, StdMap) {
    const auto test = [](const std::map<std::string, double>& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, StdUnorderedMap) {
    const auto test = [](const std::unordered_map<std::string, double>& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, Optional) {
    const auto test = [](const std::optional<double>& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, Variant) {
    ...
    test(Variant4(EigenVecStruct{Eigen::Vector2d(1.0, 2.0)}), "!EigenStruct\n    value: [1.0, 2.0]");
}

TEST_F(YamlWriteArchiveTest, PrimitiveVariant) {
    const auto test = [](const PrimitiveVariant& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, EigenVector) {
    const auto test = [](const Eigen::VectorXd& value, const std::string& expected) {
...
TEST_F(YamlWriteArchiveTest, EmitterWithProvidedSerializable) {
    const auto test = [](const std::string& root_name, const std::string& expected) {
