...
some_child_name:
  value:
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlString<StringStruct>(data, "wrong_child_name"),
...
extra_junk: will_be_ignored
    ...
    const auto result = LoadYamlString<StringStruct>(data, no_child_name, no_defaults, options);
    EXPECT_EQ(result.value, "some_value");
    // Cross-check that the options actually were important.
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlString<StringStruct>(data, no_child_name, no_defaults, {/* no options */}),
                                ".*extra_junk.*");
}
...
extra_junk:
  will_be_ignored: 2.0
    ...
    LoadYamlOptions options;
    options.allow_yaml_with_no_cpp = true;
    const auto result = LoadYamlString<MapStruct>(data, no_child_name, defaults, options);
...
GTEST_TEST(YamlIoTest, LoadFile) {
    const std::string filename = FindResourceOrThrow("drake/common/yaml/test/yaml_io_test_input_1.yaml");
...
GTEST_TEST(YamlIoTest, LoadFileChildName) {
    const std::string filename = FindResourceOrThrow("drake/common/yaml/test/yaml_io_test_input_2.yaml");
...
GTEST_TEST(YamlIoTest, LoadFileChildNameBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlFile<StringStruct>(filename, child_name),
...
GTEST_TEST(YamlIoTest, LoadFileSchemaBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadYamlFile<WrongSchema>(filename),
...
GTEST_TEST(YamlIoTest, SaveFileBad) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SaveYamlFile(filename, StringStruct{}), ".* could not open .*/no_such_dir/.*");
}
