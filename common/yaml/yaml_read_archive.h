...
#include "common/yaml/yaml_io_options.h"
#include "common/yaml/yaml_node.h"
...
public:
    ...
    static internal::Node LoadFileAsNode(const std::string& filename, const std::optional<std::string>& child_name);

    static internal::Node LoadStringAsNode(const std::string& data, const std::optional<std::string>& child_name);
...
private:
    ...
    YamlReadArchive(const char* mapish_item_key, const internal::Node* mapish_item_value, const YamlReadArchive* parent)
        : owned_root_(),
          root_(nullptr),
    ...
    template <typename Serializable>
    auto DoAccept(Serializable* serializable, int32_t) -> decltype(serializable->Serialize(this)) {
    ...
    void DoAccept(std::map<std::string, Serializable, Compare, Allocator>* value, int32_t) {
    ...
    auto DoVisit(const NVP& nvp,
                 const T&,
                 int32_t) -> decltype(nvp.value()->Serialize(static_cast<YamlReadArchive*>(nullptr))) {
        this->VisitSerializable(nvp);
    }

    // This version applies when the type has an ADL Serialize function.
    template <typename NVP, typename T>
    auto DoVisit(const NVP& nvp, const T&, int32_t) -> decltype(Serialize(static_cast<YamlReadArchive*>(nullptr),
                                                                          nvp.value())) {
        this->VisitSerializable(nvp);
    }
    ...
    template <typename NVP, typename T, int Rows, int Cols, int Options = 0, int MaxRows = Rows, int MaxCols = Cols>
    void DoVisit(const NVP& nvp, const Eigen::Matrix<T, Rows, Cols, Options, MaxRows, MaxCols>&, int32_t) {
        if constexpr (Cols == 1) {
            if constexpr (Rows >= 0) {
    ...
    void VisitOptional(const NVP& nvp) {
        ...
        this->Visit(drake::MakeNameValue(nvp.name(), &storage.value()), VisitShouldMemorizeType::kNo);
    }
    ...
    void VariantHelper(std::string_view tag, const char* name, Variant<Types...>* storage) {
    ...
    void VariantHelperImpl(std::string_view tag, const char* name, Variant* storage) {
        ...
        if (((I == 0) && (tag.empty() || (tag == "?") || (tag == "!"))) || IsTagMatch<T>(tag)) {
            T& typed_storage = storage->index() == I ? std::get<I>(*storage) : storage->template emplace<I>();
            this->Visit(drake::MakeNameValue(name, &typed_storage));
            return;
        }
    ...
    void VariantHelperImpl(std::string_view tag, const char*, Variant*) {
        ReportError(fmt::format("has unsupported type tag {} while selecting a variant<>", tag));
    }
    ...
    static bool IsTagMatch(std::string_view name, std::string_view tag) {
        ...
        const auto start = name.begin() + (start_offset == std::string::npos ? 0 : start_offset + 1);
    ...
    void VisitArray(const char* name, size_t size, T* data) {
        ...
        if (elements.size() != size) {
            ReportError(fmt::format("has {}-size entry (wanted {}-size)", elements.size(), size));
        }
    ...
    void VisitVector(const NVP& nvp, std::optional<size_t> max_size = {}) {
        ...
        if (max_size.has_value() && size > *max_size) {
            ...
            ReportError(
                    fmt::format("has too many array elements ({}); the maximum size is {} in the", size, *max_size));
            return;
        }
    ...
    void VisitMatrix(const char* name, Eigen::Matrix<T, Rows, Cols, Options, MaxRows, MaxCols>* matrix) {
        const internal::Node* sub_node = GetSubNodeSequence(name);
        if (sub_node == nullptr) {
            return;
        }
        ...
        for (size_t i = 0; i < pending_rows; ++i) {
            ...
            if (!one_row.IsSequence()) {
                ReportError(fmt::format("is Sequence-of-{} (not Sequence-of-Sequence)", one_row.GetTypeString()));
                return;
            }
        ...
        if (((Rows != Eigen::Dynamic) && (static_cast<int>(rows) != Rows)) ||
            ...
            ReportError(fmt::format("has dimension {}x{} (wanted {}x{})", rows, cols, Rows, Cols));
            return;
        }
        auto&& storage = *matrix;
    ...
    void VisitMap(const NVP& nvp) {
        ...
        static_assert(std::is_same_v<Key, std::string>, "std::map keys must be strings");
        const internal::Node* sub_node = GetSubNodeMapping(nvp.name());
        if (sub_node == nullptr) {
            return;
        }
