...
bool operator==(const Node& a, const Node& b) {
    ...
    auto a_tag = a.GetTag();
    auto b_tag = b.GetTag();
    return std::tie(a_tag, a.data_, a.filename_, a.mark_) == std::tie(b_tag, b.data_, b.filename_, b.mark_);
}
...
void Node::Add(std::string key, Node value) {
    return std::visit<void>(overloaded{
                                    [&key, &value](MappingData& data) {
                                        const auto result = data.mapping.insert({std::move(key), std::move(value)});
                                    ...
                                    [](auto&& data) {
                                        throw std::logic_error(fmt::format("Cannot Node::Add(key, value) on a {}",
                                                                           GetNiceVariantName(data)));
                                    },
                            },
                            data_);
    ...
    return std::visit<void>(
            overloaded{
                    [key](MappingData& data) {
                        ...
                        if (!erased) {
                            throw std::logic_error(fmt::format("No such key '{}' during Node::Remove(key)", key));
                        }
                    },
                    [](auto&& data) {
                        throw std::logic_error(
                                fmt::format("Cannot Node::Remove(key) on a {}", GetNiceVariantName(data)));
                    },
            },
            data_);
}
