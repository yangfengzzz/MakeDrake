...
void CopyWithMergeKeySemantics(const YAML::Node& source, YAML::Node* destination) {
...
void RewriteMergeKeys(const YAML::Node& parent, YAML::Node* node) {
        ...
        case YAML::NodeType::Sequence: {
            ...
            for (const YAML::Node& merge_key_item : merge_key) {
                if (merge_key_item.Type() != YAML::NodeType::Map) {
                    error_message = "has invalid merge key type (Sequence-of-non-Mapping).";
                    error_locus = *node;
                    break;
                }
    ...
    throw std::runtime_error(fmt::format("YAML node of type Mapping (with size {} and keys {{{}}}) {}", keys.size(),
                                         fmt::join(keys, ", "), error_message));
}
...
// The `parent` is only used to provide context during error reporting.
internal::Node ConvertJbederYamlNodeToDrakeYamlNode(const YAML::Node& parent, const YAML::Node& node) {
    ...
    if (node.Mark().line >= 0 && node.Mark().column >= 0) {
        ...
        mark = Node::Mark{.line = node.Mark().line + 1, .column = node.Mark().column + 1};
    }
    switch (node.Type()) {
        case YAML::NodeType::Undefined: {
        ...
        case YAML::NodeType::Map: {
            ...
            for (const auto& key_value : merged_node) {
                const YAML::Node& key = key_value.first;
                const YAML::Node& value = key_value.second;
                result.Add(key.Scalar(), ConvertJbederYamlNodeToDrakeYamlNode(node, value));
            }
            return result;
        }
    }
...
YamlReadArchive::YamlReadArchive(internal::Node root, const LoadYamlOptions& options)
    ...
    if (!root_->IsMapping()) {
        throw std::runtime_error(
                fmt::format("{}: invalid document: the top level element should be a Mapping "
                            "(not a {})",
...
internal::Node YamlReadArchive::LoadFileAsNode(const std::string& filename,
    ...
    if (child_name.has_value()) {
        ...
        if (!child_node) {
            throw std::runtime_error(fmt::format("When loading '{}', there was no such top-level map entry '{}'",
                                                 filename, *child_name));
        }
...
// a separate file with more specific testing, but for the moment our use of
// yaml-cpp in our public API makes that difficult.
internal::Node YamlReadArchive::LoadStringAsNode(const std::string& data,
                                                 const std::optional<std::string>& child_name) {
    ...
    if (child_name.has_value()) {
        ...
        if (!child_node) {
            throw std::runtime_error(
                    fmt::format("When loading YAML, there was no such top-level map entry '{}'", *child_name));
        }
...
void YamlReadArchive::ParseScalarImpl(const std::string& value, T* result) {
    ...
    if (!success) {
        ReportError(fmt::format("could not parse {} value", drake::NiceTypeName::Get<T>()));
    }
}
...
void YamlReadArchive::ParseScalar(const std::string& value, std::string* result) {
    DRAKE_DEMAND(result != nullptr);
    *result = value;
}
...
const internal::Node* YamlReadArchive::GetSubNodeScalar(const char* name) const {
    const internal::Node* result = GetSubNodeAny(name, internal::NodeType::kScalar);
...
const internal::Node* YamlReadArchive::GetSubNodeSequence(const char* name) const {
    return GetSubNodeAny(name, internal::NodeType::kSequence);
}

const internal::Node* YamlReadArchive::GetSubNodeMapping(const char* name) const {
    return GetSubNodeAny(name, internal::NodeType::kMapping);
}

const internal::Node* YamlReadArchive::GetSubNodeAny(const char* name, internal::NodeType expected_type) const {
    ...
    if (actual_type != expected_type) {
        std::string_view expected_type_string = internal::Node::GetTypeString(expected_type);
        std::string_view actual_type_string = result->GetTypeString();
        if (result->GetTag() == internal::Node::kTagNull) {
            actual_type_string = "Null";
        }
        ReportError(fmt::format("has non-{} ({})", expected_type_string, actual_type_string));
        result = nullptr;
    }
    return result;
}
...
void YamlReadArchive::ReportError(const std::string& note) const {
    ...
    for (auto* archive = this; archive != nullptr; archive = archive->parent_) {
        if ((archive->root_ != nullptr) && (archive->root_->GetFilename().has_value())) {
...
void YamlReadArchive::PrintNodeSummary(std::ostream& s) const {
    ...
    fmt::print(s, " (with size {} and keys {{{}}})", keys.size(), fmt::join(keys, ", "));
}
...
void YamlReadArchive::PrintVisitNameType(std::ostream& s) const {
    ...
    fmt::print(s, "{} {}", drake::NiceTypeName::Get(*debug_visit_type_), debug_visit_name_);
}
