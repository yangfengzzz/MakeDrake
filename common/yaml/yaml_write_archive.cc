...
void RecursiveEmit(const internal::Node& node, YAML::EmitFromEvents* sink) {
    ...
    if ((tag == internal::Node::kTagNull) || (tag == internal::Node::kTagBool) || (tag == internal::Node::kTagInt) ||
        (tag == internal::Node::kTagFloat) || (tag == internal::Node::kTagStr)) {
...
std::string YamlWriteArchive::YamlDumpWithSortedMaps(const internal::Node& document) {
    ...
    switch (node.GetType()) {
        case internal::NodeType::kScalar: {
            ...
            throw std::logic_error(
                    fmt::format("SaveJsonString: Cannot save a YAML scalar '{}' with tag '{}' "
                                "to JSON",
                                scalar, tag));
        }
        case internal::NodeType::kSequence: {
            if (!tag.empty()) {
                ...
                throw std::logic_error(
                        fmt::format("SaveJsonString: Cannot save a YAML sequence with tag '{}' to JSON", tag));
            }
            os << "[";
        ...
        case internal::NodeType::kMapping: {
            if (!tag.empty()) {
                throw std::logic_error(
                        fmt::format("SaveJsonString: Cannot save a YAML mapping with tag '{}' to JSON", tag));
            }
            os << "{";
            bool first = true;
