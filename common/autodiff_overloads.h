...
Eigen::AutoDiffScalar<typename internal::remove_all<DerTypeA>::type::PlainObject> pow(
        const Eigen::AutoDiffScalar<DerTypeA>& base, const Eigen::AutoDiffScalar<DerTypeB>& exponent) {
    ...
    // the same dimension.
    static_assert(std::is_same_v<typename internal::remove_all<DerTypeA>::type::PlainObject,
    ...
    if (ygrad.isZero(std::numeric_limits<double>::epsilon()) || ygrad.size() == 0) {
...
auto ExtractDoubleOrThrow(const Eigen::MatrixBase<Eigen::Matrix<Eigen::AutoDiffScalar<DerType>,
                                                                RowsAtCompileTime,
                                                                ColsAtCompileTime,
                                                                Options,
...
Eigen::AutoDiffScalar<typename Eigen::internal::remove_all<DerType1>::type::PlainObject> if_then_else(
        bool f_cond, const Eigen::AutoDiffScalar<DerType1>& x, const Eigen::AutoDiffScalar<DerType2>& y) {
    typedef Eigen::AutoDiffScalar<typename Eigen::internal::remove_all<DerType1>::type::PlainObject> ADS1;
    typedef Eigen::AutoDiffScalar<typename Eigen::internal::remove_all<DerType2>::type::PlainObject> ADS2;
...
Eigen::AutoDiffScalar<typename Eigen::internal::remove_all<DerType>::type::PlainObject> cond(
        bool f_cond, const Eigen::AutoDiffScalar<DerType>& e_then, Rest... rest) {
