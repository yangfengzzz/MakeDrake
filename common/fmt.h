...
#define DRAKE_FORMATTER_AS(TEMPLATE_ARGS, NAMESPACE, TYPE, ARG, EXPR)                                  \
    ...
    template <TEMPLATE_ARGS>                                                                           \
    struct Formatter<NAMESPACE::TYPE> : fmt::formatter<typename Traits<NAMESPACE::TYPE>::OutputType> { \
        using MyTraits = Traits<NAMESPACE::TYPE>;                                                      \
        /* Shadow our base class member function template of the same name. */                         \
        template <typename FormatContext>                                                              \
        auto format(const typename MyTraits::InputType& x, FormatContext& ctx) DRAKE_FMT8_CONST {      \
    ...
    namespace fmt {                                                                                    \
    template <TEMPLATE_ARGS>                                                                           \
    struct formatter<NAMESPACE::TYPE> : drake::internal::formatter_as::Formatter<NAMESPACE::TYPE> {};  \
    } /* namespace fmt */
