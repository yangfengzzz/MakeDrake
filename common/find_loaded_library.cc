...
std::optional<string> LoadedLibraryPath(const string& library_name) {
    ...
    if (task_info(mach_task_self(), TASK_DYLD_INFO, reinterpret_cast<task_info_t>(&dyld_info), &count) ==
        KERN_SUCCESS) {
        // Recover list of dynamic libraries.
        mach_msg_type_number_t size = sizeof(dyld_all_image_infos);
        unsigned char* data = ReadProcessMemory(dyld_info.all_image_info_addr, &size);
        if (!data) {
            return std::nullopt;
        }
        ...
        unsigned char* info_addr = ReadProcessMemory(reinterpret_cast<mach_vm_address_t>(infos->infoArray), &size2);
        if (!info_addr) {
            return std::nullopt;
        }
...
std::optional<string> LoadedLibraryPath(const std::string& library_name) {
    ...
    while (map) {
        ...
        if (pos_slash && !strcmp(pos_slash + 1, library_name.c_str())) {
            ...
            if (map->l_name[0] != '/') {
                std::string argv0 = std::filesystem::read_symlink({"/proc/self/exe"}).string();
                return string(dirname(&argv0[0])) + "/" + string(map->l_name, pos_slash - map->l_name);
            } else {
