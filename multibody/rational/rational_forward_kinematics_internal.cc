...
std::vector<MobilizerIndex> FindMobilizersOnPath(const MultibodyPlant<double>& plant, BodyIndex start, BodyIndex end) {
    ...
    for (int i = 0; i < static_cast<int>(path.size()) - 1; ++i) {
        const RigidBodyTopology& body_topology = tree.get_topology().get_rigid_body(path[i]);
        if (path[i] != world_index() && body_topology.parent_body == path[i + 1]) {
            ...
            mobilizers_on_path.push_back(tree.get_topology().get_rigid_body(path[i + 1]).inboard_mobilizer);
        }
    }
...
BodyIndex FindBodyInTheMiddleOfChain(const MultibodyPlant<double>& plant, BodyIndex start, BodyIndex end) {
    ...
    const MultibodyTree<double>& tree = GetInternalTree(plant);
    const std::vector<MobilizerIndex> mobilizer_indices = FindMobilizersOnPath(plant, path[0], path.back());
