...
#include "multibody/tree/weld_mobilizer.h"
...
RationalForwardKinematics::Pose<Scalar2> CalcChildPose(const Matrix3<Scalar2>& R_AP,
                                                       const Vector3<Scalar2>& p_WP,
                                                       const math::RigidTransform<double>& X_PF,
                                                       const math::RigidTransform<double>& X_MC,
                                                       const Matrix3<Scalar1>& R_FM,
...
RationalForwardKinematics::RationalForwardKinematics(const MultibodyPlant<double>* plant) : plant_(*plant) {
    ...
    for (BodyIndex body_index(1); body_index < plant_.num_bodies(); ++body_index) {
        const internal::RigidBodyTopology& body_topology = tree.get_topology().get_rigid_body(body_index);
        const internal::Mobilizer<double>* mobilizer = &(tree.get_mobilizer(body_topology.inboard_mobilizer));
    ...
    for (int i = 0; i < static_cast<int>(s_angles_.size()); ++i) {
        one_minus_s_angles_squared_(i) =
                symbolic::Polynomial({{monomial_one, 1}, {symbolic::Monomial(s_angles_[i], 2), -1}});
        two_s_angles_(i) = symbolic::Polynomial({{symbolic::Monomial(s_angles_[i], 1), 2}});
        one_plus_s_angles_squared_(i) =
                symbolic::Polynomial({{monomial_one, 1}, {symbolic::Monomial(s_angles_[i], 2), 1}});
    }
...
RationalForwardKinematics::Pose<symbolic::Polynomial> RationalForwardKinematics::CalcBodyPoseAsMultilinearPolynomial(
        const Eigen::Ref<const Eigen::VectorXd>& q_star, BodyIndex body_index, BodyIndex expressed_body_index) const {
    // First find the path from expressed_body_index to body_index.
    const std::vector<BodyIndex> path = internal::FindPath(plant_, expressed_body_index, body_index);
    std::vector<Pose<symbolic::Polynomial>> poses;
    poses.reserve(path.size());
    poses.push_back(GetIdentityPose());
    for (int i = 1; i < static_cast<int>(path.size()); ++i) {
        poses.push_back(CalcChildBodyPoseAsMultilinearPolynomial(q_star, path[i - 1], path[i], poses[i - 1]));
    }
    return std::move(poses.back());
}

symbolic::RationalFunction RationalForwardKinematics::ConvertMultilinearPolynomialToRationalFunction(
        const symbolic::Polynomial& e) const {
    const symbolic::RationalFunction e_rational = symbolic::internal::SubstituteStereographicProjectionImpl(
            e, sin_cos_, sin_cos_set_,
            Eigen::Map<const VectorX<symbolic::Variable>>(s_angles_.data(), s_angles_.size()), s_variables_,
            one_plus_s_angles_squared_, two_s_angles_, one_minus_s_angles_squared_);
    return e_rational;
}

template <typename T>
RationalForwardKinematics::Pose<T> RationalForwardKinematics::CalcRevoluteJointChildBodyPoseAsMultilinearPolynomial(
    ...
    const symbolic::Polynomial cos_angle({{symbolic::Monomial(cos_delta_theta, 1), cos_theta_star},
                                          {symbolic::Monomial(sin_delta_theta, 1), -sin_theta_star}});
    const symbolic::Polynomial sin_angle({{symbolic::Monomial(cos_delta_theta, 1), sin_theta_star},
                                          {symbolic::Monomial(sin_delta_theta, 1), cos_theta_star}});
    // Frame F is the inboard frame (attached to the parent body), and frame
    // M is the outboard frame (attached to the child body).
    const Matrix3<symbolic::Polynomial> R_FM =
            Eigen::Matrix3d::Identity() + sin_angle * A_F + (1 - cos_angle) * A_F * A_F;
...
template <typename T>
RationalForwardKinematics::Pose<T> RationalForwardKinematics::CalcWeldJointChildBodyPose(
...
template <typename T>
RationalForwardKinematics::Pose<T> RationalForwardKinematics::CalcPrismaticJointChildLinkPose(
        const Eigen::Ref<const Eigen::Vector3d>& axis_F,
        const math::RigidTransformd& X_PF,
        const math::RigidTransformd& X_MC,
        const Pose<T>& X_AP,
        double d_star,
        const symbolic::Variable& d) const {
    const symbolic::Polynomial poly_one(1);
    const symbolic::Polynomial poly_zero(0);
...
RationalForwardKinematics::CalcChildBodyPoseAsMultilinearPolynomial(
    ...
    // X_M'C' = X_PF.inverse()
    const internal::MultibodyTree<double>& tree = GetInternalTree(plant_);
    const internal::RigidBodyTopology& parent_topology = tree.get_topology().get_rigid_body(parent);
    const internal::RigidBodyTopology& child_topology = tree.get_topology().get_rigid_body(child);
    internal::MobilizerIndex mobilizer_index;
    bool is_order_reversed;
    if (parent_topology.parent_body.is_valid() && parent_topology.parent_body == child) {
        is_order_reversed = true;
        mobilizer_index = parent_topology.inboard_mobilizer;
    } else if (child_topology.parent_body.is_valid() && child_topology.parent_body == parent) {
        is_order_reversed = false;
        mobilizer_index = child_topology.inboard_mobilizer;
    } else {
        throw std::invalid_argument(
                fmt::format("CalcChildBodyPoseAsMultilinearPolynomial: the pair of body indices "
                            "({}, {}) do not have a parent-child relationship.",
                            parent, child));
    }
    const internal::Mobilizer<double>* mobilizer = &(tree.get_mobilizer(mobilizer_index));
    ...
    if (IsRevolute(*mobilizer)) {
        ...
        return CalcRevoluteJointChildBodyPoseAsMultilinearPolynomial(
                axis_F, X_PF, X_MC, X_AP, q_star(q_index), cos_delta_[s_angle_index], sin_delta_[s_angle_index]);
    } else if (IsPrismatic(*mobilizer)) {
        // A prismatic joint.
        ...
        return CalcPrismaticJointChildLinkPose(axis_F, X_PF, X_MC, X_AP, q_star(s_index), s_[s_index]);
    } else if (IsWeld(*mobilizer)) {
...
bool RationalForwardKinematics::IsRevolute(const internal::Mobilizer<double>& mobilizer) const {
    const bool is_revolute = (mobilizer.num_positions() == 1 && mobilizer.num_velocities() == 1 &&
                              mobilizer.can_rotate() && !mobilizer.can_translate());
    if (is_revolute) {
        DRAKE_THROW_UNLESS(dynamic_cast<const internal::RevoluteMobilizer<double>*>(&mobilizer) != nullptr);
    }
    return is_revolute;
}

// TODO(hongkai.dai): determine the joint type through a Reifier.
bool RationalForwardKinematics::IsWeld(const internal::Mobilizer<double>& mobilizer) const {
    const bool is_weld = (mobilizer.num_positions() == 0 && mobilizer.num_velocities() == 0 &&
                          !mobilizer.can_rotate() && !mobilizer.can_translate());
    if (is_weld) {
        DRAKE_THROW_UNLESS(dynamic_cast<const internal::WeldMobilizer<double>*>(&mobilizer) != nullptr);
    }
    return is_weld;
}

// TODO(hongkai.dai): determine the joint type through a Reifier.
bool RationalForwardKinematics::IsPrismatic(const internal::Mobilizer<double>& mobilizer) const {
    const bool is_prismatic = (mobilizer.num_positions() == 1 && mobilizer.num_velocities() == 1 &&
                               !mobilizer.can_rotate() && mobilizer.can_translate());
    if (is_prismatic) {
        DRAKE_THROW_UNLESS(dynamic_cast<const internal::PrismaticMobilizer<double>*>(&mobilizer) != nullptr);
    }
    return is_prismatic;
}
