...
public:
     ...
     */
    [[nodiscard]] symbolic::RationalFunction ConvertMultilinearPolynomialToRationalFunction(
    ...
    [[nodiscard]] std::enable_if_t<is_eigen_vector<Derived>::value, VectorX<typename Derived::Scalar>> ComputeSValue(
        ...
        for (int i = 0; i < s_val.size(); ++i) {
            const internal::Mobilizer<double>& mobilizer =
                    GetInternalTree(plant_).get_mobilizer(map_s_to_mobilizer_.at(s_[i].get_id()));
     ...
     */
    template <typename Derived>
    [[nodiscard]] std::enable_if_t<is_eigen_vector<Derived>::value, VectorX<typename Derived::Scalar>> ComputeQValue(
            const Eigen::MatrixBase<Derived>& s_val, const Eigen::Ref<const Eigen::VectorXd>& q_star_val) const {
        ...
        for (int i = 0; i < s_val.size(); ++i) {
            ...
            using std::pow;
            if (IsRevolute(mobilizer)) {
                q_val(q_index) = atan2(2 * s_val(i), 1 - pow(s_val(i), 2)) + q_star_val(q_index);
            } else if (IsPrismatic(mobilizer)) {
    ...
    Eigen::Map<const VectorX<symbolic::Variable>> s() const { return EigenMapView(s_); }
     ...
     */
    [[nodiscard]] const std::vector<int> map_mobilizer_to_s_index() const { return map_mobilizer_to_s_index_; }
...
private:
    ...
    */
    template <typename T>
    Pose<T> CalcRevoluteJointChildBodyPoseAsMultilinearPolynomial(const Eigen::Ref<const Eigen::Vector3d>& axis_F,
    ...
    // along `axis_F`.
    template <typename T>
    Pose<T> CalcPrismaticJointChildLinkPose(const Eigen::Ref<const Eigen::Vector3d>& axis_F,
                                            const math::RigidTransformd& X_PF,
                                            const math::RigidTransformd& X_MC,
                                            const Pose<T>& X_AP,
                                            double d_star,
    ...
    /* Given the pose of the parent frame X_AP measured in a frame A, calculates
     ...
     body in the world).
     */
    Pose<symbolic::Polynomial> CalcChildBodyPoseAsMultilinearPolynomial(const Eigen::Ref<const Eigen::VectorXd>& q_star,
                                                                        BodyIndex parent,
                                                                        BodyIndex child,
                                                                        const Pose<symbolic::Polynomial>& X_AP) const;
    ...
    std::unordered_map<symbolic::Variable::Id, internal::MobilizerIndex> map_s_to_mobilizer_;
    // map_mobilizer_to_s_index_[mobilizer_index] returns the starting index of
    // the mobilizer's variable in s_ (the variable will be contiguous in s for
    // the same mobilizer). If this mobilizer doesn't have a variable in s_ (like
    // the weld joint), then the s index is set to -1.
