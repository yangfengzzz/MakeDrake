 ...
 */
Eigen::Matrix<double, 3, 8> GenerateBoxVertices(const Eigen::Vector3d& size, const drake::math::RigidTransformd& pose);
...
std::unique_ptr<MultibodyPlant<double>> ConstructDualArmIiwaPlant(const std::string& iiwa_sdf_name,
                                                                  const drake::math::RigidTransformd& X_WL,
 ...
 */
void AddIiwaWithSchunk(const math::RigidTransformd& X_7S, MultibodyPlant<double>* plant);
