...
#include "multibody/rational/test/rational_forward_kinematics_test_utilities.h"
...
void CheckBodyKinematics(const RationalForwardKinematics& dut,
    ...
    const internal::MultibodyTree<double>& tree = internal::GetInternalTree(dut.plant());
    ...
    for (int i = 0; i < dut.plant().num_bodies(); ++i) {
        const math::RigidTransformd X_AB_expected = X_WB_expected[expressed_body_index].inverse() * X_WB_expected[i];
        ...
        const RationalForwardKinematics::Pose<symbolic::Polynomial> pose_poly_i =
                dut.CalcBodyPoseAsMultilinearPolynomial(q_star_val, BodyIndex(i), expressed_body_index);
        Eigen::Matrix3d R_AB_i;
        Eigen::Vector3d p_AB_i;
        for (int m = 0; m < 3; ++m) {
            for (int n = 0; n < 3; ++n) {
                const symbolic::RationalFunction R_AB_i_mn_rational =
                        dut.ConvertMultilinearPolynomialToRationalFunction(pose_poly_i.rotation(m, n));
                R_AB_i(m, n) = R_AB_i_mn_rational.Evaluate(env);
            }
            const symbolic::RationalFunction p_AB_i_m_rational =
                    dut.ConvertMultilinearPolynomialToRationalFunction(pose_poly_i.position(m));
            p_AB_i(m) = p_AB_i_m_rational.Evaluate(env);
        }
        EXPECT_TRUE(CompareMatrices(R_AB_i, X_AB_expected.rotation().matrix(), kTol));
...
TEST_F(FinalizedIiwaTest, CalcBodyPose1) {
    ...
    CheckBodyKinematics(dut, Eigen::VectorXd::Zero(kNumJoints), Eigen::VectorXd::Zero(kNumJoints),
...
TEST_F(FinalizedIiwaTest, CalcBodyPose2) {
    ...
    CheckBodyKinematics(dut, q_val, Eigen::VectorXd::Zero(kNumJoints), s_val, world_);
    // Compute the pose in the iiwa_link[i]'s frame.
    for (int i = 0; i < 8; ++i) {
        CheckBodyKinematics(dut, q_val, Eigen::VectorXd::Zero(kNumJoints), s_val, iiwa_link_[i]);
    }
}
...
TEST_F(FinalizedIiwaTest, CalcBodyPose3) {
    ...
    Eigen::VectorXd s_val_overflow = dut.ComputeSValue(q_val, q_star_val, true /* angles_wrap_to_inf */);
    ...
    s_val_overflow = dut.ComputeSValue(q_val, q_star_val, true /* angles_wrap_to_inf */);
...
GTEST_TEST(RationalForwardKinematicsTest, CalcBodyPosesForDualArmIiwa) {
    ...
    RigidTransformd X_WR{RotationMatrixd(Eigen::AngleAxisd(0.2 * M_PI, Eigen::Vector3d::UnitZ())), {0.2, 0.8, 0.1}};
    ModelInstanceIndex left_iiwa_instance, right_iiwa_instance;
    auto iiwa_plant =
            ConstructDualArmIiwaPlant("iiwa14_no_collision.sdf", X_WL, X_WR, &left_iiwa_instance, &right_iiwa_instance);
    ...
    auto calc_s_val = [&](const Eigen::VectorXd& q_left, const Eigen::VectorXd& q_right,
...
public:
    KinematicTreeTest() : plant_{new MultibodyPlant<double>{0.}} {
        AddBodyWithJoint<RevoluteJoint>(
                ...
                math::RigidTransformd(math::RollPitchYawd(0.2, 0.1, 0.5), Eigen::Vector3d(0.2, 0.5, 0.1)),
                math::RigidTransformd(math::RollPitchYawd(-0.5, 0.1, 0.4), Eigen::Vector3d(0.1, 0.3, 0.2)),
                Eigen::Vector3d(0, 1 / std::sqrt(2), 1 / std::sqrt(2)));
        AddBodyWithJoint<PrismaticJoint>(
                "body1", body_indices_[0], "joint1",
                math::RigidTransformd(math::RollPitchYawd(0.1, 0.2, 0.3), Eigen::Vector3d(0.2, 0.4, -0.1)),
                math::RigidTransformd(math::RollPitchYawd(-0.1, 0.5, 0.3), Eigen::Vector3d(0.2, 0.5, -0.1)),
                Eigen::Vector3d(1.0 / 3, 2.0 / 3, -2. / 3));
        AddBodyWithJoint<WeldJoint>(
                "body2", body_indices_[1], "joint2",
                math::RigidTransformd(math::RollPitchYawd(0.2, -0.1, 0.5), Eigen::Vector3d(0.4, -0.1, 0.2)),
                math::RigidTransformd(math::RollPitchYawd(-0.1, 0.5, 0.3), Eigen::Vector3d(0.2, 0.4, 0.3)),
                math::RigidTransformd(math::RollPitchYawd(0.5, 0.3, -0.2), Eigen::Vector3d(0.2, 0.1, 0.4)));
        AddBodyWithJoint<RevoluteJoint>(
                "body3", body_indices_[0], "joint3",
                math::RigidTransformd(math::RollPitchYawd(0.2, -0.5, 0.3), Eigen::Vector3d(0.2, -0.1, 1.5)),
                math::RigidTransformd(math::RollPitchYawd(0.4, 0.2, 0.5), Eigen::Vector3d(0.4, -0.2, 1.1)),
                Eigen::Vector3d::UnitX());
        AddBodyWithJoint<PrismaticJoint>(
                "body4", body_indices_[3], "joint4",
                math::RigidTransformd(math::RollPitchYawd(0.2, 0.1, 0.5), Eigen::Vector3d(0.5, -0.3, 1.1)),
                math::RigidTransformd(math::RollPitchYawd(0.2, -0.1, 0.5), Eigen::Vector3d(0, 0.1, -0.2)),
                Eigen::Vector3d::UnitY());
        AddBodyWithJoint<RevoluteJoint>(
                "body5", body_indices_[3], "joint5",
                math::RigidTransformd(math::RollPitchYawd(0.1, 0.4, 0.5), Eigen::Vector3d(0.2, 0.4, 0.1)),
                math::RigidTransformd(math::RollPitchYawd(0.3, 0.1, -0.5), Eigen::Vector3d(0.2, 0.5, -0.1)),
                Eigen::Vector3d::UnitX());
        AddBodyWithJoint<RevoluteJoint>(
                "body6", body_indices_[5], "joint6",
                math::RigidTransformd(math::RollPitchYawd(1.1, 0.4, -0.5), Eigen::Vector3d(0.4, -0.4, 0.1)),
                math::RigidTransformd(math::RollPitchYawd(0.7, -0.1, -0.5), Eigen::Vector3d(0.2, 0.5, -0.1)),
                Eigen::Vector3d::UnitY());
        AddBodyWithJoint<RevoluteJoint>(
                "body7", world_index(), "joint7",
                math::RigidTransformd(math::RollPitchYawd(0.1, -0.4, -0.5), Eigen::Vector3d(0.2, -0.5, 0.1)),
                math::RigidTransformd(math::RollPitchYawd(0.1, -0.3, -0.5), Eigen::Vector3d(0.2, 0.5, -1.1)),
                Eigen::Vector3d::UnitY());
        AddBodyWithJoint<WeldJoint>(
                "body8", body_indices_[7], "joint8",
                math::RigidTransformd(math::RollPitchYawd(0.2, -0.4, -0.5), Eigen::Vector3d(0.2, 0.5, 0.1)),
                math::RigidTransformd(math::RollPitchYawd(0.3, -0.1, -0.5), Eigen::Vector3d(0.2, 0.5, -1.1)),
                math::RigidTransformd(math::RollPitchYawd(0.1, -1.1, -0.3), Eigen::Vector3d(0.2, 0.3, -1.2)));
        plant_->Finalize();
    }
...
protected:
    ...
    void AddBodyWithJoint(const std::string& body_name,
                          ...
                          Args&&... args) {
        const SpatialInertia<double> spatial_inertia(1, Eigen::Vector3d::Zero(),
                                                     UnitInertia<double>(0.01, 0.01, 0.01, 0, 0, 0));
        body_indices_.push_back(plant_->AddRigidBody(body_name, spatial_inertia).index());
        plant_->AddJoint<JointType>(joint_name, plant_->get_body(parent_index), X_PF,
                                    plant_->get_body(body_indices_.back()), X_BM, std::forward<Args>(args)...);
    }
...
TEST_F(KinematicTreeTest, CalcBodyPoses) {
    ...
    CheckBodyKinematics(dut, Eigen::VectorXd::Zero(plant_->num_positions()),
                        Eigen::VectorXd::Zero(plant_->num_positions()), Eigen::VectorXd::Zero(plant_->num_positions()),
                        world_index());
...
TEST_F(KinematicTreeTest, TestSAndQConversionDouble) {
    ...
    auto check_s_val = [&dut, tol](const Eigen::VectorXd& q_val, const Eigen::VectorXd& q_star) {
    ...
    check_s_val(Eigen::VectorXd::Zero(plant_->num_positions()), Eigen::VectorXd::Zero(plant_->num_positions()));
...
TEST_F(KinematicTreeTest, TestSAndQConversionSymbolic) {
    ...
    auto check_s_val = [&dut](const Eigen::Ref<const VectorX<symbolic::Variable>>& q_vars,
       ...
       -0.87, -0.25,  0.81, -0.88,  1.01, -0.94, -0.55;
        ...
        auto build_env = [&q_vars](const Eigen::Ref<const Eigen::VectorXd> vals) {
            symbolic::Environment env;
