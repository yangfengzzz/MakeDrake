...
struct DifferentialInverseKinematicsResult {
    ...
    DifferentialInverseKinematicsStatus status{DifferentialInverseKinematicsStatus::kNoSolutionFound};
};
...
public:
    DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(DifferentialInverseKinematicsParameters);
    ...
    const VectorX<double>& get_nominal_joint_position() const { return nominal_joint_position_; }

    const MatrixX<double>& get_joint_centering_gain() const { return joint_centering_gain_; }

    const Vector6<bool>& get_end_effector_velocity_flag() const { return flag_E_; }
    ...
    double get_maximum_scaling_to_report_stuck() const { return max_scaling_to_report_stuck_; }

    double get_end_effector_angular_speed_limit() const { return angular_speed_limit_; }
     ...
     * formulation.
     */
    void set_end_effector_velocity_flag(const Vector6<bool>& flag_E) { flag_E_ = flag_E; }
    ...
    void set_joint_position_limits(const std::pair<VectorX<double>, VectorX<double>>& q_bounds) {
        ...
        DRAKE_THROW_UNLESS((q_bounds.second.array() >= q_bounds.first.array()).all());
        q_bounds_ = q_bounds;
    }
    ...
    void set_joint_velocity_limits(const std::pair<VectorX<double>, VectorX<double>>& v_bounds) {
        ...
        DRAKE_THROW_UNLESS((v_bounds.second.array() >= v_bounds.first.array()).all());
        v_bounds_ = v_bounds;
    }
    ...
    void set_joint_acceleration_limits(const std::pair<VectorX<double>, VectorX<double>>& vd_bounds) {
        ...
        DRAKE_THROW_UNLESS((vd_bounds.second.array() >= vd_bounds.first.array()).all());
    ...
    @default 0.01. */
    void set_maximum_scaling_to_report_stuck(double scaling) { max_scaling_to_report_stuck_ = scaling; }
     ...
     */
    const solvers::SolverOptions& get_solver_options() const { return solver_options_; }

    /**
     * Provides mutable access to change the solver options, e.g., to tune for
     * speed vs accuracy.
     */
    solvers::SolverOptions& get_mutable_solver_options() { return solver_options_; }
...
private:
    ...
    std::optional<std::pair<Eigen::Vector3d, Eigen::Vector3d>> translational_velocity_bounds_{};
    std::vector<std::shared_ptr<solvers::LinearConstraint>> linear_velocity_constraints_;
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
        ...
        const std::optional<Eigen::Ref<const MatrixX<double>>>& Nplus = std::nullopt);
}  // namespace internal
#endif
...
namespace fmt {
template <>
struct formatter<drake::multibody::DifferentialInverseKinematicsStatus> : drake::ostream_formatter {};
}  // namespace fmt
