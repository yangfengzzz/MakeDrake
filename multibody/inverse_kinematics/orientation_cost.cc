...
OrientationCost::OrientationCost(const MultibodyPlant<double>* const plant,
    ...
    : solvers::Cost(RefFromPtrOrThrow(plant).num_positions()),
      constraint_(plant,
                  ...
                  R_BbarB,
                  0,
                  PtrOrThrow(plant_context, "OrientationCost(): plant_context is nullptr")),
      c_{c} {}
...
void OrientationCost::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
...
void OrientationCost::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    y->resize(1);
...
void OrientationCost::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
                             VectorX<symbolic::Expression>*) const {
    throw std::logic_error("OrientationCost::DoEval() does not work for symbolic variables.");
}
