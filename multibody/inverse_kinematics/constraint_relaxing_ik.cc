#include "multibody/inverse_kinematics/constraint_relaxing_ik.h"
...
ConstraintRelaxingIk::ConstraintRelaxingIk(const std::string& model_path, const std::string& end_effector_link_name)
    ...
    if (plant_.GetBodiesWeldedTo(plant_.world_body()).size() <= 1) {
        ...
        plant_.WeldFrames(plant_.world_frame(), plant_.get_body(bodies[0]).body_frame());
    }
    plant_.Finalize();
...
bool ConstraintRelaxingIk::PlanSequentialTrajectory(const std::vector<IkCartesianWaypoint>& waypoints,
    ...
    for (const auto& waypoint : waypoints) {
        ...
        while (true) {
            if (!waypoint.constrain_orientation) DRAKE_DEMAND(mode == RelaxMode::kRelaxPosTol);
            ...
            if (res) {
                ...
                if ((rot_tol <= waypoint.rot_tol) && (pos_tol.array() <= waypoint.pos_tol.array()).all()) {
                    break;
                }
            ...
            if (relaxed_ctr > kMaxNumConstraintRelax) {
                ...
                plant_.SetRandomState(*context, &context->get_mutable_state(), &rand_generator_);
                ...
                if (!waypoint.constrain_orientation) rot_tol = 0;
                mode = RelaxMode::kRelaxPosTol;
                drake::log()->warn("IK FAILED at max constraint relaxing iter: " + std::to_string(relaxed_ctr));
                relaxed_ctr = 0;
                random_ctr++;
            }
            ...
            if (random_ctr > kMaxNumInitialGuess) {
                drake::log()->error("IK FAILED at max random starts: " + std::to_string(random_ctr));
...
bool ConstraintRelaxingIk::SolveIk(const IkCartesianWaypoint& waypoint,
    ...
    ik.AddPositionConstraint(plant_.get_body(end_effector_body_idx_).body_frame(), Vector3<double>::Zero(),
                             plant_.world_frame(), pos_lb, pos_ub);

    if (waypoint.constrain_orientation) {
        ik.AddOrientationConstraint(plant_.world_frame(), waypoint.pose.rotation(),
                                    plant_.get_body(end_effector_body_idx_).body_frame(), math::RotationMatrixd(),
                                    rot_tol);
    }
