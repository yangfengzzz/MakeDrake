...
namespace drake {
namespace multibody {
GazeTargetConstraint::GazeTargetConstraint(const MultibodyPlant<double>* const plant,
    ...
    if (cone_half_angle < 0 || cone_half_angle > M_PI_2) {
        throw std::invalid_argument("GazeTargetConstraint: cone_half_angle should be within [0, pi/2]");
    }
}

GazeTargetConstraint::GazeTargetConstraint(const MultibodyPlant<AutoDiffXd>* const plant,
...
void EvalConstraintGradient(const systems::Context<double>& context,
    ...
    plant.CalcJacobianSpatialVelocity(context, JacobianWrtVariable::kQDot, frameB, p_BT, frameA, frameA, &Jq_V_ABt);
    // Jq_p = Jq_v_ABt = ∂p/∂q.
    const Matrix3X<double> Jq_p = Jq_V_ABt.bottomRows<3>();
    // J_g_p = ∂g/∂p.
    const Eigen::Matrix<double, 2, 3> Jp_g = (Eigen::Matrix<double, 2, 3>() << n_A.transpose(),
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    const Vector3<T> cos_cone_half_angle_squared_times_p = std::pow(cos_cone_half_angle, 2) * p_ST_A;
    // Constraint values, g.
    const Vector2<T> g{p_dot_n, p_dot_n * p_dot_n - cos_cone_half_angle_squared_times_p.dot(p_ST_A)};
    if constexpr (std::is_same_v<T, S>) {
        *y = g;
    } else {
        EvalConstraintGradient(*context, plant, frameA, frameB, p_BT, n_A, cos_cone_half_angle_squared_times_p, p_dot_n,
                               g, x, y);
    }
}

void GazeTargetConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
        Eval(x.cast<AutoDiffXd>(), &y_t);
        *y = math::ExtractValue(y_t);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameA_index_, frameB_index_, p_AS_, n_A_, p_BT_,
                      cos_cone_half_angle_, x, y);
    }
}

void GazeTargetConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, frameA_index_, frameB_index_, p_AS_, n_A_, p_BT_,
                      cos_cone_half_angle_, x, y);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameA_index_, frameB_index_, p_AS_, n_A_, p_BT_,
                      cos_cone_half_angle_, x, y);
