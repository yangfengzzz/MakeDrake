#include "multibody/inverse_kinematics/global_inverse_kinematics.h"
...
namespace multibody {
namespace {
std::map<BodyIndex, JointIndex> GetBodyToJointMap(const MultibodyPlant<double>& plant) {
    ...
    for (JointIndex joint_index : plant.GetJointIndices()) {
        body_to_joint_map.emplace(plant.get_joint(joint_index).child_body().index(), joint_index);
    }
    return body_to_joint_map;
}

std::unordered_set<BodyIndex> GetWeldToWorldBodyIndexSet(const MultibodyPlant<double>& plant) {
    const std::vector<const RigidBody<double>*> weld_to_world_bodies = plant.GetBodiesWeldedTo(plant.world_body());
...
bool IsWeld(const Joint<double>& joint) {
    const bool is_weld =
            !joint.can_rotate() && !joint.can_translate() && joint.num_positions() == 0 && joint.num_velocities() == 0;
    if (is_weld) {
        DRAKE_THROW_UNLESS(dynamic_cast<const WeldJoint<double>*>(&joint) != nullptr);
    }
    return is_weld;
}
...
bool IsRevolute(const Joint<double>& joint) {
    const bool is_revolute =
            joint.can_rotate() && !joint.can_translate() && joint.num_positions() == 1 && joint.num_velocities() == 1;
    if (is_revolute) {
        DRAKE_THROW_UNLESS(dynamic_cast<const RevoluteJoint<double>*>(&joint) != nullptr);
    }
    return is_revolute;
}
}  // namespace

GlobalInverseKinematics::GlobalInverseKinematics(const MultibodyPlant<double>& plant,
                                                 const GlobalInverseKinematics::Options& options)
    : prog_{},
      plant_(plant),
      joint_lower_bounds_{Eigen::VectorXd::Constant(plant_.num_positions(), -std::numeric_limits<double>::infinity())},
      joint_upper_bounds_{Eigen::VectorXd::Constant(plant_.num_positions(), std::numeric_limits<double>::infinity())} {
    ...
    solvers::MixedIntegerRotationConstraintGenerator rotation_generator(
            options.approach, options.num_intervals_per_half_axis, options.interval_binning);

    const std::map<BodyIndex, JointIndex> body_to_joint_map = GetBodyToJointMap(plant_);
    // Loop through each body in the robot, to add the constraint that the bodies
    // are welded by joints.
    const std::unordered_set<BodyIndex> weld_to_world_body_index_set = GetWeldToWorldBodyIndexSet(plant_);
    ...
    for (BodyIndex body_idx{1}; body_idx < num_bodies; ++body_idx) {
        ...
        if (weld_to_world_body_index_set.contains(body_idx)) {
            ...
            const math::RigidTransformd X_WB =
                    plant_.CalcRelativeTransform(*dummy_plant_context, plant_.world_frame(), body.body_frame());
            // TODO(hongkai.dai): clean up this for loop using
            // elementwise matrix constraint when it is ready.
            for (int i = 0; i < 3; ++i) {
                prog_.AddBoundingBoxConstraint(X_WB.rotation().matrix().col(i), X_WB.rotation().matrix().col(i),
                                               R_WB_[body_idx].col(i));
            }
            prog_.AddBoundingBoxConstraint(X_WB.translation(), X_WB.translation(), p_WBo_[body_idx]);
        } else {
            // This body is not rigidly fixed to the world.
            if (!options.linear_constraint_only) {
                solvers::AddRotationMatrixOrthonormalSocpConstraint(&prog_, R_WB_[body_idx]);
            }
            if (body.is_floating()) {
            ...
            if (joint->parent_body().index().is_valid()) {
                ...
                const RigidTransformd X_PJp = joint->frame_on_parent().GetFixedPoseInBodyFrame();
                const RigidTransformd X_CJc = joint->frame_on_child().GetFixedPoseInBodyFrame();
                if (IsWeld(*joint)) {
                    const WeldJoint<double>* weld_joint = static_cast<const WeldJoint<double>*>(joint);
                    ...
                    prog_.AddLinearEqualityConstraint(
                            p_WBo_[parent_idx] + R_WB_[parent_idx] * X_PC.translation() - p_WBo_[body_idx],
                            Vector3d::Zero());
                    ...
                    for (int i = 0; i < 3; ++i) {
                        prog_.AddLinearEqualityConstraint(orient_invariance.col(i), Vector3d::Zero());
                    }
                } else if (IsRevolute(*joint)) {
                    const RevoluteJoint<double>* revolute_joint = static_cast<const RevoluteJoint<double>*>(joint);
                    ...
                    prog_.AddLinearEqualityConstraint(R_WB_[body_idx] * X_CJc.rotation().matrix() * axis -
                                                              R_WB_[parent_idx] * X_PJp.rotation().matrix() * axis,
                                                      Vector3d::Zero());

                    // The position of the rotation axis is the same on both child and
                    // parent bodies.
                    prog_.AddLinearEqualityConstraint(p_WBo_[parent_idx] + R_WB_[parent_idx] * X_PJp.translation() -
                    ...
                    AddJointLimitConstraint(body_idx, joint_lb, joint_ub, options.linear_constraint_only);
                } else {
...
const solvers::MatrixDecisionVariable<3, 3>& GlobalInverseKinematics::body_rotation_matrix(BodyIndex body_index) const {
...
const solvers::VectorDecisionVariable<3>& GlobalInverseKinematics::body_position(BodyIndex body_index) const {
    if (body_index >= plant_.num_bodies() || body_index <= 0) {
        throw std::runtime_error("body index out of range.");
    }
...
void GlobalInverseKinematics::ReconstructGeneralizedPositionSolutionForBody(
    ...
    if (body.is_floating()) {
        ...
        if (body.has_quaternion_dofs()) {
            ...
            q.segment<4>(body.floating_positions_start() + 3) = normalized_rotmat.ToQuaternionAsVector4();
        } else {
            // The position order is x-y-z-roll-pitch-yaw.
            q.segment<3>(body.floating_positions_start() + 3) = math::RollPitchYaw<double>(normalized_rotmat).vector();
        }
    ...
    if (!weld_to_world_body_index_set.contains(body_idx)) {
        ...
        if (joint.num_positions() == 1) {
            ...
            if (dynamic_cast<const RevoluteJoint<double>*>(&joint) != nullptr) {
                const RevoluteJoint<double>* revolute_joint = dynamic_cast<const RevoluteJoint<double>*>(&joint);
                const Matrix3d R_JpJc =
                        X_PJp.rotation().matrix().transpose() * R_WP.transpose() * R_WC * X_CJc.rotation().matrix();
                ...
                const double revolute_joint_angle =
                        math::ProjectMatToRotMatWithAxis(R_JpJc, axis_F, joint_lb, joint_ub);
                q(position_idx) = revolute_joint_angle;
                reconstruct_R_WB->at(body_idx) = R_WP * X_PJp.rotation().matrix() *
    ...
    while (num_link_visited < plant_.num_bodies()) {
        if (!is_link_visited[body_idx]) {
            ...
            if (plant_.get_body(body_idx).is_floating()) {
                ...
                parent_idx = plant_.get_joint(body_to_joint_map.at(body_idx)).parent_body().index();
            }
            while (!is_link_visited[parent_idx]) {
                ...
                if (plant_.get_body(BodyIndex{parent_idx}).is_floating()) {
                    ...
                    parent_idx = plant_.get_joint(body_to_joint_map.at(BodyIndex{parent_idx})).parent_body().index();
                }
            }
            ...
            while (!unvisited_links.empty()) {
                ...
                unvisited_links.pop();
                ReconstructGeneralizedPositionSolutionForBody(result, unvisited_link_idx, body_to_joint_map,
...
solvers::Binding<solvers::LinearConstraint> GlobalInverseKinematics::AddWorldPositionConstraint(
        BodyIndex body_idx,
    ...
    return prog_.AddLinearConstraint(X_WF.inverse().cast<Expression>() * p_WQ, box_lb_F, box_ub_F);
}

solvers::Binding<solvers::LinearConstraint> GlobalInverseKinematics::AddWorldRelativePositionConstraint(
        BodyIndex body_idx_B,
        const Eigen::Vector3d& p_BQ,
        BodyIndex body_idx_A,
    ...
    const Vector3<Expression> p_WQ = p_WBo_[body_idx_B] + R_WB_[body_idx_B] * p_BQ;
    const Vector3<Expression> p_WP = p_WBo_[body_idx_A] + R_WB_[body_idx_A] * p_AP;
    return prog_.AddLinearConstraint(X_WF.rotation().inverse().cast<Expression>() * (p_WQ - p_WP), box_lb_F, box_ub_F);
}

solvers::Binding<solvers::LinearConstraint> GlobalInverseKinematics::AddWorldOrientationConstraint(
        BodyIndex body_idx, const Eigen::Quaterniond& desired_orientation, double angle_tol) {
    if (body_idx >= plant_.num_bodies() || body_idx <= 0) {
        throw std::runtime_error("body index out of range.");
    }
    ...
    Matrix3<Expression> rotation_matrix_err = desired_orientation.toRotationMatrix() * R_WB_[body_idx].transpose();
    double lb = angle_tol < M_PI ? 2 * cos(angle_tol) + 1 : -1;
    return prog_.AddLinearConstraint(rotation_matrix_err.trace(), lb, 3);
}

void GlobalInverseKinematics::AddPostureCost(const Eigen::Ref<const Eigen::VectorXd>& q_desired,
    ...
    solvers::VectorXDecisionVariable p_WBo_err = prog_.NewContinuousVariables(num_bodies - 1, "p_WBo_error");
    for (int i = 1; i < num_bodies; ++i) {
        // body 0 is the world. There is no position or orientation error on the
        // world, so we just skip i = 0 and start from i = 1.
        const auto& X_WB_desired = plant_.CalcRelativeTransform(*context, plant_.world_frame(),
        ...
        pos_error_expr << p_WBo_err(i - 1), body_position_cost(i) * (p_WBo_[i] - X_WB_desired.translation());
        ...
        orient_err_sum += body_orientation_cost(i) *
                          (1 - ((X_WB_desired.rotation().matrix() * R_WB_[i].transpose()).trace() - 1) / 2);
    }
...
solvers::VectorXDecisionVariable GlobalInverseKinematics::BodyPointInOneOfRegions(
    ...
    solvers::VectorXDecisionVariable z = prog_.NewBinaryVariables(num_regions, "z_" + body_name);
    ...
    for (int i = 0; i < num_regions; ++i) {
        ...
        w[i] = prog_.NewContinuousVariables(num_vertices_i, "w_" + body_name + "_region_" + std::to_string(i));
        prog_.AddLinearConstraint(w[i].cast<symbolic::Expression>().sum() - z(i) == 0);
        prog_.AddBoundingBoxConstraint(Eigen::VectorXd::Zero(num_vertices_i), Eigen::VectorXd::Ones(num_vertices_i),
                                       w[i]);
    ...
    prog_.AddLinearEqualityConstraint(p_WBo + R_WB * p_BQ - p_WQ, Eigen::Vector3d::Zero());

    return z;
}

solvers::VectorXDecisionVariable GlobalInverseKinematics::BodySphereInOneOfPolytopes(
        BodyIndex body_index,
        const Eigen::Ref<const Eigen::Vector3d>& p_BQ,
        double radius,
    ...
    // z1 + ... + zn = 1
    prog_.AddLinearEqualityConstraint(Eigen::RowVectorXd::Ones(num_polytopes), 1, z);

    const auto y = prog_.NewContinuousVariables<3, Eigen::Dynamic>(3, num_polytopes, "y");
    const Vector3<symbolic::Expression> p_WQ = p_WBo_[body_index] + R_WB_[body_index] * p_BQ;
    // p_WQ = y.col(0) + ... + y.col(n)
    prog_.AddLinearEqualityConstraint(p_WQ - y.cast<symbolic::Expression>().rowwise().sum(), Eigen::Vector3d::Zero());

    for (int i = 0; i < num_polytopes; ++i) {
        DRAKE_DEMAND(polytopes[i].A.rows() == polytopes[i].b.rows());
        prog_.AddLinearConstraint(polytopes[i].A * y.col(i) <=
...
void ApproximateBoundedNormByLinearConstraints(const Eigen::Ref<const Vector3<symbolic::Expression>>& x,
                                               double c,
                                               solvers::MathematicalProgram* prog) {
    DRAKE_DEMAND(c >= 0);
    // -c ≤ xᵢ ≤ c
    prog->AddLinearConstraint(x, Eigen::Vector3d::Constant(-c), Eigen::Vector3d::Constant(c));
...
void GlobalInverseKinematics::AddJointLimitConstraint(BodyIndex body_index,
    ...
    if (joint == nullptr) {
        throw std::runtime_error(fmt::format("The body {} is not the child of any joint in the plant.",
    ...
    switch (joint->num_velocities()) {
        ...
        case 1: {
            ...
            if (is_limits_tightened) {
                ...
                // not have a method to tell if a joint is revolute or not.
                if (dynamic_cast<const RevoluteJoint<double>*>(joint) != nullptr) {
                    const auto* revolute_joint = dynamic_cast<const RevoluteJoint<double>*>(joint);
                    // axis_F is the vector of the rotation axis in the joint
                    // inboard/outboard frame.
                    const Vector3d axis_F = revolute_joint->revolute_axis();

                    // Now we process the joint limits constraint.
                    const double joint_bound =
                            (joint_upper_bounds_[position_idx] - joint_lower_bounds_[position_idx]) / 2;
                    ...
                    if (joint_bound < M_PI) {
                        ...
                        const Matrix3d rotmat_joint_offset =
                                Eigen::AngleAxisd(
                                        (joint_lower_bounds_[position_idx] + joint_upper_bounds_[position_idx]) / 2,
                                        axis_F)
                                        .toRotationMatrix();
                        ...
                        for (const auto& v : v_samples) {
                            ...
                            joint_limit_expr.tail<3>() =
                                    ...
                                    R_WB_[parent_idx] * X_PJp.rotation().matrix() * rotmat_joint_offset * v;
                            if (linear_constraint_approximation) {
                                ApproximateBoundedNormByLinearConstraints(joint_limit_expr.tail<3>(),
                                                                          joint_limit_lorentz_rhs, &prog_);
                        ...
                        if (weld_to_world_body_index_set.contains(BodyIndex{parent_idx})) {
                            const RigidTransformd X_WP =
                                    plant_.CalcRelativeTransform(*dummy_plant_context, plant_.world_frame(),
                            ...
                            Eigen::Matrix<symbolic::Expression, 3, 3> R_joint_beta =
                                    (X_WP.rotation().matrix() * X_PJp.rotation().matrix() * rotmat_joint_offset)
                                            .transpose() *
                            ...
                            if (!linear_constraint_approximation) {
                                ...
                                const Eigen::Matrix<symbolic::Expression, 2, 2> M =
                                        V.transpose() * (R_joint_beta + R_joint_beta.transpose()) / 2 * V -
                            ...
                            const symbolic::Expression R_joint_beta_trace{R_joint_beta.trace()};
                            prog_.AddLinearConstraint(R_joint_beta_trace >= 1 + 2 * joint_bound_cos);
                        }
                    }
                } else {
...
void GlobalInverseKinematics::SetInitialGuess(const Eigen::Ref<const Eigen::VectorXd>& q) {
    auto context = plant_.CreateDefaultContext();
    ...
    for (int i = 1; i < plant_.num_bodies(); ++i) {
        const auto& X_WB = plant_.CalcRelativeTransform(*context, plant_.world_frame(),
                                                        plant_.get_body(BodyIndex{i}).body_frame());

        bindings.push_back(prog_.AddBoundingBoxConstraint(X_WB.translation(), X_WB.translation(), p_WBo_[i]));
        bindings.push_back(
                prog_.AddBoundingBoxConstraint(X_WB.rotation().matrix(), X_WB.rotation().matrix(), R_WB_[i]));
    }
