...
void CalcDistanceDerivatives(const MultibodyPlant<double>& plant,
    ...
    plant.CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kQDot, frameA, p_ACa, frameB,
...
VectorX<S> Distances(const MultibodyPlant<T>& plant,
    ...
    const auto& query_object = query_port.template Eval<geometry::QueryObject<T>>(*context);

    const std::vector<geometry::SignedDistancePair<T>> signed_distance_pairs =
            query_object.ComputeSignedDistancePairwiseClosestPoints(influence_distance);
    VectorX<S> distances(signed_distance_pairs.size());
    for (int i = 0; i < static_cast<int>(signed_distance_pairs.size()); ++i) {
        const geometry::SceneGraphInspector<T>& inspector = query_object.inspector();
        const geometry::FrameId frame_A_id = inspector.GetFrameId(signed_distance_pairs[i].id_A);
        const geometry::FrameId frame_B_id = inspector.GetFrameId(signed_distance_pairs[i].id_B);
        ...
        internal::CalcDistanceDerivatives(plant, *context, frameA, frameB,
                                          // GetPoseInFrame() returns RigidTransform<double> -- we can't
                                          // multiply across heterogeneous scalar types; so we cast the double
                                          // to T.
                                          inspector.GetPoseInFrame(signed_distance_pairs[i].id_A).template cast<T>() *
                                                  signed_distance_pairs[i].p_ACa,
                                          signed_distance_pairs[i].distance, signed_distance_pairs[i].nhat_BA_W, q,
                                          &distances(i));
    }
    return distances;
}
...
Eigen::VectorXd Distances(const planning::CollisionChecker& collision_checker,
    ...
    return collision_checker.CalcContextRobotClearance(collision_checker_context, x, influence_distance_val)
            .distances();
}
...
AutoDiffVecXd Distances(const planning::CollisionChecker& collision_checker,
                        ...
                        double influence_distance_val) {
    const planning::RobotClearance robot_clearance = collision_checker.CalcContextRobotClearance(
            collision_checker_context, math::ExtractValue(x), influence_distance_val);
    return math::InitializeAutoDiff(robot_clearance.distances(),
                                    robot_clearance.jacobians() * math::ExtractGradient(x));
}

// Explicit instantiation
template VectorX<double> Distances<double, double>(const MultibodyPlant<double>&,
                                                   systems::Context<double>*,
                                                   const Eigen::Ref<const VectorX<double>>&,
                                                   double);
template VectorX<AutoDiffXd> Distances<double, AutoDiffXd>(const MultibodyPlant<double>&,
                                                           systems::Context<double>*,
                                                           const Eigen::Ref<const VectorX<AutoDiffXd>>&,
                                                           double);
template VectorX<double> Distances<AutoDiffXd, double>(const MultibodyPlant<AutoDiffXd>&,
                                                       systems::Context<AutoDiffXd>*,
                                                       const Eigen::Ref<const VectorX<double>>&,
                                                       double);
template VectorX<AutoDiffXd> Distances<AutoDiffXd, AutoDiffXd>(const MultibodyPlant<AutoDiffXd>&,
                                                               systems::Context<AutoDiffXd>*,
                                                               const Eigen::Ref<const VectorX<AutoDiffXd>>&,
                                                               double);
}  // namespace internal
