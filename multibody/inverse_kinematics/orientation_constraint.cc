...
namespace drake {
namespace multibody {
OrientationConstraint::OrientationConstraint(const MultibodyPlant<double>* const plant,
    ...
    : solvers::Constraint(
              1, RefFromPtrOrThrow(plant).num_positions(), Vector1d(2 * std::cos(theta_bound) + 1), Vector1d(3)),
      plant_double_{plant},
    ...
    if (theta_bound < 0) {
        throw std::invalid_argument("OrientationConstraint: theta_bound should be non-negative.\n");
    }
}

OrientationConstraint::OrientationConstraint(const MultibodyPlant<AutoDiffXd>* const plant,
...
void EvalConstraintGradient(const systems::Context<double>& context,
    ...
    const Eigen::Vector3d r_AB{m(1, 2) - m(2, 1), m(2, 0) - m(0, 2), m(0, 1) - m(1, 0)};
    Eigen::MatrixXd Jq_w_AbarBbar(3, plant.num_positions());
    plant.CalcJacobianAngularVelocity(context, JacobianWrtVariable::kQDot, frameBbar, frameAbar, frameAbar,
                                      &Jq_w_AbarBbar);
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    const math::RotationMatrix<T> R_AbarBbar = plant.CalcRelativeRotationMatrix(*context, frameAbar, frameBbar);
    ...
    const math::RotationMatrix<T> R_AB = R_AAbar.cast<T>() * R_AbarBbar * R_BbarB.cast<T>();
    if constexpr (std::is_same_v<T, S>) {
        (*y)(0) = R_AB.matrix().trace();
    } else {
        EvalConstraintGradient(*context, plant, frameAbar, frameBbar, R_AAbar, R_AB, x, y);
    }
}

}  // namespace

void OrientationConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
        Eval(x.cast<AutoDiffXd>(), &y_t);
        *y = math::ExtractValue(y_t);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameAbar_index_, frameBbar_index_, R_AAbar_, R_BbarB_, x, y);
    }
}

void OrientationConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, frameAbar_index_, frameBbar_index_, R_AAbar_, R_BbarB_, x,
                      y);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameAbar_index_, frameBbar_index_, R_AAbar_, R_BbarB_, x, y);
    }
