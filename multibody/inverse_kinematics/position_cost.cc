...
PositionCost::PositionCost(const MultibodyPlant<double>* const plant,
    ...
    : solvers::Cost(RefFromPtrOrThrow(plant).num_positions()),
      constraint_(plant,
                  ...
                  frameB,
                  p_BQ,
                  PtrOrThrow(plant_context, "PositionCost(): plant_context is nullptr")),
      C_{C} {}
...
void PositionCost::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    y->resize(1);
    Eigen::VectorXd p_AQ;
...
void PositionCost::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    y->resize(1);
    AutoDiffVecXd p_AQ;
    constraint_.Eval(x, &p_AQ);
    const Eigen::VectorXd& p_AP = constraint_.lower_bound();
...
void PositionCost::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&, VectorX<symbolic::Expression>*) const {
    throw std::logic_error("PositionCost::DoEval() does not work for symbolic variables.");
}
