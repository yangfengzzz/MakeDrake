#include "multibody/inverse_kinematics/differential_inverse_kinematics.h"
...
#include "manipulation/kuka_iiwa/iiwa_constants.h"
...
protected:
    void SetUpRobot(bool floating = false) {
        ...
        parser.AddModelsFromUrl("package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf");
        if (!floating) {
            math::RigidTransform<double> X_WA = math::RigidTransform<double>(
                    math::RotationMatrix<double>::MakeZRotation(M_PI / 10.0), Vector3d(-0.1, -0.2, 0.3));
            plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0"), X_WA);
        }
        // Add the EE frame.
        const math::RigidTransformd X_7E(AngleAxis<double>(M_PI, Vector3d::UnitZ()), Vector3d(0.1, 0, 0));
        ...
        params_ = std::make_unique<DifferentialInverseKinematicsParameters>(plant_->num_positions(),
        ...
        VectorXd vel_limits = VectorXd::Constant(plant_->num_velocities(), std::numeric_limits<double>::infinity());
        ...
        if (floating) {
            plant_->SetFreeBodyPoseInWorldFrame(
                    ...
                    math::RigidTransformd(math::RollPitchYawd(0.12, 0.24, 0.35), Vector3d{-1.2, 2.5, 5.3}));
        }
        // Set the revolute joints. These values were randomly generated.
        q.tail<7>() << 0.155184061989378285773000, 1.401571412639109226461187, 2.230260142950035717746004,
                0.902511993417089097846428, 1.416899102195978255025465, -1.76907066662500445097805,
    ...
    DifferentialInverseKinematicsResult DoDiffIKForRigidTransform(const math::RigidTransform<double>& X_WE_desired) {
        return DoDifferentialInverseKinematics(*plant_, *context_, X_WE_desired, *frame_E_, *params_);
    }

    DifferentialInverseKinematicsResult DoDiffIKForSpatialVelocity(
            const multibody::SpatialVelocity<double>& V_WE_W_desired) {
        return DoDifferentialInverseKinematics(*plant_, *context_, V_WE_W_desired.get_coeffs(), *frame_E_, *params_);
    }
    ...
    void CheckPositiveResult(const multibody::SpatialVelocity<double>& V_AB,
        ...
        drake::log()->info("result.joint_velocities = {}", fmt_eigen(result.joint_velocities->transpose()));
        ...
        drake::log()->info("V_AB_actual = {}", fmt_eigen(V_AB_actual.get_coeffs().transpose()));
        drake::log()->info("V_AB = {}", fmt_eigen(V_AB.get_coeffs().transpose()));
        EXPECT_TRUE(CompareMatrices(V_AB_actual.get_coeffs().normalized(), V_AB.get_coeffs().normalized(), 1e-6));
...
TEST_F(DifferentialInverseKinematicsTest, PositiveTest) {
    SetUpRobot();
    const multibody::SpatialVelocity<double> V_WE_W(Vector3d(1.0, 2.0, 3.0), Vector3d(4.0, 5.0, 6.0));
    // Test without additional linear constraints.
    DifferentialInverseKinematicsResult result = DoDiffIKForSpatialVelocity(V_WE_W);
    ...
    params_->AddLinearVelocityConstraint(std::make_shared<LinearConstraint>(A, Vector1d{-1}, Vector1d{1}));
    ...
    params_->get_mutable_solver_options().SetOption(solvers::OsqpSolver::id(), "max_iter", 0);
    result = DoDiffIKForSpatialVelocity(V_WE_W);
    EXPECT_EQ(result.status, DifferentialInverseKinematicsStatus::kNoSolutionFound);
}
...
TEST_F(DifferentialInverseKinematicsTest, OverConstrainedTest) {
  ...
  const auto A = (MatrixX<double>(2, 7) <<
    ...
    params_->AddLinearVelocityConstraint(std::make_shared<LinearConstraint>(A, b, b));
    const DifferentialInverseKinematicsResult result = DoDiffIKForSpatialVelocity(V_WE_W);
...
TEST_F(DifferentialInverseKinematicsTest, GainTest) {
    ...
    const math::RotationMatrix<double> R_WE = frame_E_->CalcRotationMatrixInWorld(*context_);
    const math::RotationMatrix<double> R_EW = R_WE.inverse();

    const multibody::SpatialVelocity<double> V_WE_W_desired(Vector3d(0.1, -0.2, 0.3) / 2,
                                                            Vector3d(-0.3, 0.2, -0.1) / 2);
    // Transform the desired end effector velocity into the body frame.
    const multibody::SpatialVelocity<double> V_WE_E_desired = R_EW * V_WE_W_desired;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        const DifferentialInverseKinematicsResult result = DoDiffIKForSpatialVelocity(V_WE_W_desired);
        ASSERT_TRUE(result.status == DifferentialInverseKinematicsStatus::kSolutionFound);
        ...
        const multibody::SpatialVelocity<double> V_WE_E = R_EW * frame_E_->CalcSpatialVelocityInWorld(*context_);
    ...
    const auto& v_bounds = *(params_->get_joint_velocity_limits());
    VectorXd v_desired = params_->get_joint_centering_gain() * (params_->get_nominal_joint_position() - q);
    ...
    EXPECT_TRUE(CompareMatrices(plant_->GetVelocities(*context_), v_desired, 5e-5, MatrixCompareType::absolute));
}
...
TEST_F(DifferentialInverseKinematicsTest, SimpleTracker) {
    ...
    math::RigidTransform<double> X_WE_desired = math::RigidTransform<double>(Vector3d(-0.02, -0.01, -0.03)) * X_WE;
    for (int iteration = 0; iteration < 900; ++iteration) {
        const DifferentialInverseKinematicsResult result = DoDiffIKForRigidTransform(X_WE_desired);
        ASSERT_EQ(result.status, DifferentialInverseKinematicsStatus::kSolutionFound);
    ...
    EXPECT_TRUE(CompareMatrices(X_WE.GetAsMatrix4(), X_WE_desired.GetAsMatrix4(), 1e-5, MatrixCompareType::absolute));
}
...
TEST_F(DifferentialInverseKinematicsTest, EndEffectorVelocityLimits) {
    ...
    math::RigidTransform<double> X_WE_desired =
            math::RigidTransform<double>(math::RotationMatrix<double>::MakeXRotation(M_PI / 10.0),
    ...
    params_->set_end_effector_translational_velocity_limits(Vector3d::Constant(-0.1), Vector3d::Constant(0.1));
    const DifferentialInverseKinematicsResult result2 = DoDiffIKForRigidTransform(X_WE_desired);
    EXPECT_EQ(result2.status, DifferentialInverseKinematicsStatus::kSolutionFound);
}

TEST_F(DifferentialInverseKinematicsTest, FloatingBase) {
    SetUpRobot(true);
    const multibody::SpatialVelocity<double> V_WE_W(Vector3d(1.0, 2.0, 3.0), Vector3d(4.0, 5.0, 6.0));
    params_->set_joint_centering_gain(Eigen::MatrixXd::Identity(plant_->num_positions(), plant_->num_positions()));
    DifferentialInverseKinematicsResult result = DoDiffIKForSpatialVelocity(V_WE_W);
    CheckPositiveResult(V_WE_W, result);
...
TEST_F(DifferentialInverseKinematicsTest, PositiveTestIiwaFrame) {
    SetUpRobot();
    const multibody::Frame<double>& frame_A = plant_->GetFrameByName("iiwa_link_0");
    const multibody::SpatialVelocity<double> V_AE_A(Vector3d(1.0, 2.0, 3.0), Vector3d(4.0, 5.0, 6.0));
    DifferentialInverseKinematicsResult result =
            DoDifferentialInverseKinematics(*plant_, *context_, V_AE_A.get_coeffs(), frame_A, *frame_E_, *params_);
...
TEST_F(DifferentialInverseKinematicsTest, SimpleTrackerIiwaFrame) {
    ...
    for (int iteration = 0; iteration < 60; ++iteration) {
        const DifferentialInverseKinematicsResult result =
                DoDifferentialInverseKinematics(*plant_, *context_, X_AE_desired, frame_A, *frame_E_, *params_);
        ASSERT_EQ(result.status, DifferentialInverseKinematicsStatus::kSolutionFound);
    ...
    EXPECT_TRUE(CompareMatrices(X_AE.GetAsMatrix4(), X_AE_desired.GetAsMatrix4(), 1e-5, MatrixCompareType::absolute));
}
...
GTEST_TEST(DifferentialInverseKinematicsParametersTest, TestSetter) {
    ...
    EXPECT_THROW(dut.set_joint_acceleration_limits({VectorXd(2), h}), std::exception);
    ...
    EXPECT_EQ(dut.get_end_effector_translational_velocity_limits()->first[0], -4.56);
    EXPECT_EQ(dut.get_end_effector_translational_velocity_limits()->second[0], 7.89);

    EXPECT_THROW(dut.set_end_effector_translational_velocity_limits(Vector3d::Constant(1), Vector3d::Constant(-1)),
                 std::exception);
}
...
GTEST_TEST(DifferentialInverseKinematicsParametersTest, TestMutators) {
    ...
    dut.AddLinearVelocityConstraint(
            std::make_shared<LinearConstraint>(Eigen::RowVector3d(1, 1, 1), VectorXd::Zero(1), VectorXd::Zero(1)));
    EXPECT_EQ(dut.get_linear_velocity_constraints().size(), 1);
    dut.AddLinearVelocityConstraint(
            std::make_shared<LinearConstraint>(Eigen::RowVector3d(1, 0, 1), VectorXd::Zero(1), VectorXd::Zero(1)));
    EXPECT_EQ(dut.get_linear_velocity_constraints().size(), 2);

    // Test with wrong number of variables.
    EXPECT_THROW(dut.AddLinearVelocityConstraint(std::make_shared<LinearConstraint>(
...
GTEST_TEST(AdditionalDifferentialInverseKinematicsTests, TestLinearObjective) {
    ...
    // Configure the Diff IK.
    DifferentialInverseKinematicsParameters params(plant.num_positions(), plant.num_velocities());
    ...
    // y-axis.
    params.set_end_effector_velocity_flag((Vector6<bool>() << false, true, false, true, false, true).finished());
    ...
    multibody::SpatialVelocity<double> V_WE_W_desired(Vector3d(0, 1, 0), Vector3d(0.1, 0, 0.3));
    auto result = DoDifferentialInverseKinematics(plant, *context, V_WE_W_desired.get_coeffs(), frame_7, params);
    ...
    multibody::SpatialVelocity<double> V_WE_W_actual = frame_7.CalcSpatialVelocityInWorld(*context);
    EXPECT_TRUE(CompareMatrices(V_WE_W_actual.get_coeffs(), V_WE_W_desired.get_coeffs(), kTol));

    const Vector3d joint_velocity_limits{0.5, 0.5, 0.5};
    // Check that these joint limits were violated with the unconstrained
    // solution.
    EXPECT_GT((result.joint_velocities.value().cwiseAbs() - joint_velocity_limits).maxCoeff(), 0);

    params.set_joint_velocity_limits(std::pair(-joint_velocity_limits, joint_velocity_limits));
    result = DoDifferentialInverseKinematics(plant, *context, V_WE_W_desired.get_coeffs(), frame_7, params);
    EXPECT_EQ(result.status, DifferentialInverseKinematicsStatus::kSolutionFound);
    plant.SetVelocities(context.get(), result.joint_velocities.value());
    // Check that these joint limits are enforced.
    EXPECT_LE((result.joint_velocities.value().cwiseAbs() - joint_velocity_limits).maxCoeff(), 0);
    V_WE_W_actual = frame_7.CalcSpatialVelocityInWorld(*context);
    // We no longer match the desired spatial velocity exactly...
    EXPECT_FALSE(CompareMatrices(V_WE_W_actual.get_coeffs(), V_WE_W_desired.get_coeffs(), kTol));
    // but we still match the direction.
    EXPECT_TRUE(
            CompareMatrices(V_WE_W_actual.get_coeffs().normalized(), V_WE_W_desired.get_coeffs().normalized(), kTol));
}
...
GTEST_TEST(AdditionalDifferentialInverseKinematicsTests, QdotNeV) {
    ...
    EXPECT_TRUE(result.status == DifferentialInverseKinematicsStatus::kSolutionFound);
...
GTEST_TEST(AdditionalDifferentialInverseKinematicsTests, QdotNeV2) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(DoDifferentialInverseKinematics(q, v, V, J, parameters),
