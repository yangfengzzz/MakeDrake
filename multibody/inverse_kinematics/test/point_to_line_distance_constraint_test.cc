...
void TestIiwa(const MultibodyPlant<T>* plant,
    ...
    PointToLineDistanceConstraint constraint(plant, frame_point, p_B1P, frame_line, p_B2Q, n_B2, distance_lower,
    ...
    plant->CalcPointsPositions(*plant_context, frame_point, p_B1P.cast<T>(), plant->world_frame(), &p_WP);
    const auto X_WB2 = plant->CalcRelativeTransform(*plant_context, plant->world_frame(), frame_line);
    ...
    const Vector3<T> p_WP_perp = (p_WP - p_WQ).dot(n_W_normalized) * n_W_normalized + p_WQ;
    ...
    plant_autodiff->CalcPointsPositions(*plant_context_autodiff, plant_autodiff->get_frame(frame_point_index),
    ...
    const Vector3<AutoDiffXd> n_B2_normalized_ad = n_B2.normalized().cast<AutoDiffXd>();
    const AutoDiffXd distance_squared_ad =
            (p_QP_B2_ad.dot(n_B2_normalized_ad) * n_B2_normalized_ad + p_B2Q.cast<AutoDiffXd>() - p_B2P_ad)
                    .squaredNorm();
    CompareAutoDiffVectors(y_autodiff, Vector1<AutoDiffXd>(distance_squared_ad), 1E-12);
}
