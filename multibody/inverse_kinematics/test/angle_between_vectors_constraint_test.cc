...
namespace multibody {
namespace {
AutoDiffVecXd EvalAngleBetweenVectorsConstraint(const Context<AutoDiffXd>& context,
                                                const MultibodyPlant<AutoDiffXd>& plant,
                                                const Frame<AutoDiffXd>& frameA,
                                                const Vector3<double>& n_A,
                                                const Frame<AutoDiffXd>& frameB,
                                                const Vector3<double>& n_B) {
    Vector1<AutoDiffXd> y_autodiff;
    const math::RotationMatrix<AutoDiffXd> R_AB = plant.CalcRelativeRotationMatrix(context, frameA, frameB);
...
TEST_F(IiwaKinematicConstraintTest, AngleBetweenVectorsConstraint) {
    ...
    AngleBetweenVectorsConstraint constraint(plant_, frameA, n_A, frameB, n_B, angle_lower, angle_upper,
                                             plant_context_);

    EXPECT_EQ(constraint.num_constraints(), 1);
    EXPECT_EQ(constraint.num_vars(), plant_autodiff_->num_positions());
    EXPECT_TRUE(CompareMatrices(constraint.lower_bound(), Vector1d(std::cos(angle_upper))));
    EXPECT_TRUE(CompareMatrices(constraint.lower_bound(), Vector1d(std::cos(angle_upper))));
    ...
    Vector1<AutoDiffXd> y_autodiff_expected = EvalAngleBetweenVectorsConstraint(
            *plant_context_autodiff_, *plant_autodiff_, plant_autodiff_->GetFrameByName(frameA.name()), n_A,
    ...
    plant_autodiff_->SetPositions(plant_context_autodiff_.get(), q_autodiff);
    constraint.Eval(q_autodiff, &y_autodiff);
    y_autodiff_expected = EvalAngleBetweenVectorsConstraint(*plant_context_autodiff_, *plant_autodiff_,
    ...
    const AngleBetweenVectorsConstraint constraint_from_autodiff(
            ...
            plant_autodiff_->get_frame(frameB_index), n_B, angle_lower, angle_upper, plant_context_autodiff_.get());
...
TEST_F(TwoFreeBodiesConstraintTest, AngleBetweenVectorsConstraint) {
    ...
    q << QuaternionToVectorWxyz(body1_quaternion), body1_position, QuaternionToVectorWxyz(body2_quaternion),
            body2_position;
    {
        AngleBetweenVectorsConstraint good_constraint(plant_, plant_->get_frame(body1_index_), n_A,
    ...
    {
        AngleBetweenVectorsConstraint bad_constraint(plant_, plant_->get_frame(body1_index_), n_A,
...
TEST_F(IiwaKinematicConstraintTest, AngleBetweenVectorsConstraintConstructorError) {
    ...
    EXPECT_THROW(AngleBetweenVectorsConstraint(plant_, frameA, Eigen::Vector3d::Zero(), frameB, Eigen::Vector3d::Ones(),
                                               0.1, 0.2, plant_context_),
                 std::invalid_argument);
    // n_B being zero vector.
    EXPECT_THROW(AngleBetweenVectorsConstraint(plant_, frameA, Eigen::Vector3d::Ones(), frameB, Eigen::Vector3d::Zero(),
                                               0.1, 0.2, plant_context_),
                 std::invalid_argument);
    // angle_lower < 0
    EXPECT_THROW(AngleBetweenVectorsConstraint(plant_, frameA, Eigen::Vector3d::Ones(), frameB, Eigen::Vector3d::Ones(),
                                               -0.1, 0.2, plant_context_),
                 std::invalid_argument);
    // angle_upper < angle_lower
    EXPECT_THROW(AngleBetweenVectorsConstraint(plant_, frameA, Eigen::Vector3d::Ones(), frameB, Eigen::Vector3d::Zero(),
                                               0.1, 0.09, plant_context_),
                 std::invalid_argument);
    // angle_upper > pi
    EXPECT_THROW(AngleBetweenVectorsConstraint(plant_, frameA, Eigen::Vector3d::Ones(), frameB, Eigen::Vector3d::Zero(),
                                               0.1, 1.1 * M_PI, plant_context_),
                 std::invalid_argument);
}
}  // namespace
}  // namespace multibody
