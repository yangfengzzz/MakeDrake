...
GTEST_TEST(ConstraintRelaxingIkTest, SolveIkFromFk) {
    ...
    const RigidBody<double>& end_effector = iiwa.GetBodyByName(kEndEffectorLinkName);
    ...
    std::unique_ptr<systems::Context<double>> context = iiwa.CreateDefaultContext();

    const VectorX<double> kQcurrent = iiwa.GetPositions(*context);

    const double kEpsilon = 1e-8;
    const Vector3<double> kUpperBound = wp.pos_tol + kEpsilon * Vector3<double>::Ones();
    const Vector3<double> kLowerBound = -wp.pos_tol - kEpsilon * Vector3<double>::Ones();
    RandomGenerator rand_generator(1234);

    for (int i = 0; i < 100; ++i) {
        iiwa.SetRandomState(*context, &context->get_mutable_state(), &rand_generator);

        math::RigidTransformd fk_pose = iiwa.EvalBodyPoseInWorld(*context, end_effector);
        waypoints[0].pose = fk_pose;

        std::vector<Eigen::VectorXd> q_sol;
        const bool ret = ik_planner.PlanSequentialTrajectory(waypoints, kQcurrent, &q_sol);
        ASSERT_TRUE(ret);

        iiwa.SetPositions(context.get(), q_sol.front());
        const math::RigidTransformd ik_pose = iiwa.EvalBodyPoseInWorld(*context, end_effector);

        const Vector3<double> pos_diff = ik_pose.translation() - fk_pose.translation();
        const math::RotationMatrix R_diff = ik_pose.rotation().transpose() * fk_pose.rotation();
    ...
    EXPECT_FALSE(ik_planner.PlanSequentialTrajectory(waypoints, kQcurrent, &q_sol, give_up_early));
    EXPECT_EQ(prior_index, 2);
    // Bash on regardless.
    auto never_stop = [](int) {
        return true;
    };
    EXPECT_TRUE(ik_planner.PlanSequentialTrajectory(waypoints, kQcurrent, &q_sol, never_stop));
}
