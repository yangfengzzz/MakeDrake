#include "multibody/inverse_kinematics/com_position_constraint.h"
...
namespace multibody {
namespace {
AutoDiffVecXd EvalComPositionConstraintAutoDiff(const Context<AutoDiffXd>& context,
                                                const MultibodyPlant<AutoDiffXd>& plant,
                                                const std::vector<ModelInstanceIndex>& model_instances,
                                                const Frame<AutoDiffXd>& expressed_frame,
                                                const Eigen::Ref<const Vector3<AutoDiffXd>>& p_EC) {
    const Vector3<AutoDiffXd> p_WC = plant.CalcCenterOfMassPositionInWorld(context, model_instances);
    const math::RigidTransform<AutoDiffXd> X_EW =
            plant.CalcRelativeTransform(context, expressed_frame, plant.world_frame());
    return X_EW * p_WC - p_EC;
}

template <typename T>
void TestComPositionConstraint(const MultibodyPlant<T>* const plant,
                               const MultibodyPlant<AutoDiffXd>* const plant_autodiff,
                               const std::optional<std::vector<ModelInstanceIndex>>& model_instances,
                               const FrameIndex& expressed_frame_index,
    ...
    ComPositionConstraint constraint(plant, model_instances, plant->get_frame(expressed_frame_index), plant_context);
    EXPECT_EQ(constraint.num_vars(), plant->num_positions() + 3);
    EXPECT_EQ(constraint.num_constraints(), 3);
    EXPECT_TRUE(CompareMatrices(constraint.lower_bound(), Eigen::Vector3d::Zero()));
    EXPECT_TRUE(CompareMatrices(constraint.upper_bound(), Eigen::Vector3d::Zero()));
    ...
    if (model_instances.has_value()) {
        p_WC = plant->CalcCenterOfMassPositionInWorld(*plant_context, model_instances.value());
    } else {
        p_WC = plant->CalcCenterOfMassPositionInWorld(*plant_context);
    }
    const Vector3<T> y_expected = plant->CalcRelativeTransform(*plant_context, plant->get_frame(expressed_frame_index),
                                                               plant->world_frame()) *
                                          p_WC -
                                  p_EC_val;
    ...
    plant_autodiff->SetPositions(plant_context_autodiff, x_autodiff.head(plant->num_positions()));
    std::vector<ModelInstanceIndex> model_instances_val;
    if (model_instances.has_value()) {
        model_instances_val = model_instances.value();
    } else {
        for (ModelInstanceIndex model_instance_index{1}; model_instance_index < plant->num_model_instances();
             ++model_instance_index) {
            model_instances_val.push_back(model_instance_index);
        }
    }
    Vector3<AutoDiffXd> y_autodiff_expected =
            EvalComPositionConstraintAutoDiff(*plant_context_autodiff, *plant_autodiff, model_instances_val,
...
TEST_F(IiwaKinematicConstraintTest, ComPositionConstraint_empty_model_instance) {
    ...
    TestComPositionConstraint(plant_, plant_autodiff_.get(), std::nullopt, link2_frame.index(), plant_context_,
                              plant_context_autodiff_.get(), q, p_EC);
    // Test constraint constructed with MBP<AutoDiffXd>
    TestComPositionConstraint(plant_autodiff_.get(), plant_autodiff_.get(), std::nullopt, link2_frame.index(),
                              plant_context_autodiff_.get(), plant_context_autodiff_.get(), q, p_EC);
}
...
GTEST_TEST(DualIiwaTest, ComPositionConstraint_model_instance) {
    ...
    auto plant = ConstructIiwaPlant(iiwa_url, 0.1 /* time step */, 2 /* num_iiwa */);
    ...
    Eigen::Vector3d p_EC(0.9, 1.2, 0.8);
    std::vector<ModelInstanceIndex> model_instances{{plant->GetModelInstanceByName("iiwa14")}};
    TestComPositionConstraint(plant_autodiff.get(), plant_autodiff.get(), model_instances,
