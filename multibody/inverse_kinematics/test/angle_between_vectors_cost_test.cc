...
auto ConstructTwoFreeBodiesCost(const Eigen::Ref<const Vector3d>& a_A,
    ...
    v.cost = std::make_unique<AngleBetweenVectorsCost>(v.plant.get(), v.body1->body_frame(), a_A, v.body2->body_frame(),
                                                       b_B, c, v.context.get());
...
GTEST_TEST(AngleBetweenVectorsCostTest, TwoFreeBodies) {
    ...
    auto [plant, context, body1, body2, cost] = ConstructTwoFreeBodiesCost<double>(a_A, b_B, c);
    auto [plant_ad, context_ad, body1_ad, body2_ad, cost_ad] = ConstructTwoFreeBodiesCost<AutoDiffXd>(a_A, b_B, c);
    ...
    auto CheckCases = [](const AngleBetweenVectorsCost& angle_cost, const AngleBetweenVectorsCost& angle_cost_ad,
        ...
        EXPECT_TRUE(CompareMatrices(y_d_ad[0].derivatives(), y_ad_ad[0].derivatives(), 1e-11));
    };
    ...
    double cos_theta = a_A.dot(b_B) / (a_A.norm() * b_B.norm());  // because A == B.
    CheckCases(*cost, *cost_ad, q, c * (1 - cos_theta));

    // X_WA, X_WB are arbitrary non-indentity transforms.
    RigidTransform<double> X_WA(RollPitchYaw<double>(0.32, -0.24, -0.51), Vector3d(0.1, 0.3, 0.72));
    RigidTransform<double> X_WB(RollPitchYaw<double>(8.1, 0.42, -0.2), Vector3d(-0.84, 0.2, 1.4));
