...
GTEST_TEST(GlobalInverseKinematicsTest, TestConstructor) {
    ...
    EXPECT_FALSE(global_ik_default.prog().rotated_lorentz_cone_constraints().empty());
...
TEST_F(KukaTest, SetInitialGuess) {
    const Eigen::VectorXd initial_guess0 = Eigen::VectorXd::Zero(global_ik_.prog().num_vars());
    global_ik_.get_mutable_prog()->SetInitialGuessForAllVariables(initial_guess0);

    const int num_bb_constraints = global_ik_.prog().bounding_box_constraints().size();
    Eigen::VectorXd q(7);
    q << 0, 0.6, 0, -1.75, 0, 1.0, 0;
    global_ik_.SetInitialGuess(q);
    // Check that SetInitialGuess does not leave any new constraints.
    EXPECT_EQ(num_bb_constraints, global_ik_.prog().bounding_box_constraints().size());

    EXPECT_FALSE(CompareMatrices(global_ik_.prog().initial_guess(), initial_guess0));

    Eigen::Map<const VectorX<symbolic::Variable>> variables = global_ik_.prog().decision_variables();
    ...
    EXPECT_TRUE(CompareMatrices(global_ik_.prog().GetInitialGuess(global_ik_.body_position(link7.index())),
                                X_W7.translation(), kTol));
    EXPECT_TRUE(CompareMatrices(global_ik_.prog().GetInitialGuess(global_ik_.body_rotation_matrix(link7.index())),
...
TEST_F(KukaTest, UnreachableTest) {
    ...
    global_ik_.AddWorldPositionConstraint(ee_idx_, Vector3d::Zero(), ee_pos_lb, ee_pos_ub);

    Eigen::Quaterniond ee_desired_orient(Eigen::AngleAxisd(-M_PI, Vector3d(0, 1, 0)));
    global_ik_.AddWorldOrientationConstraint(ee_idx_, ee_desired_orient, 0.0 * M_PI);

    solvers::GurobiSolver gurobi_solver;
    if (gurobi_solver.available()) {
        global_ik_.get_mutable_prog()->SetSolverOption(solvers::GurobiSolver::id(), "OutputFlag", 1);

        const solvers::MathematicalProgramResult result = gurobi_solver.Solve(global_ik_.prog(), {}, {});
        EXPECT_TRUE(result.get_solution_result() == SolutionResult::kInfeasibleOrUnbounded ||
    ...
    q_nom.setZero();
    q_guess.setZero();
    CheckNonlinearIK(ee_pos_lb, ee_pos_ub, ee_desired_orient, 0, {q_guess}, q_nom, false);
}
...
TEST_F(KukaTest, ReachableWithCost) {
    ...
    math::RigidTransformd X_WEe = plant_->CalcRelativeTransform(*context, plant_->world_frame(),
                                                                plant_->get_body(BodyIndex{ee_idx_}).body_frame());
    math::RigidTransformd X_W0 =
            plant_->CalcRelativeTransform(*context, plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0"));
    ...
    global_ik_.AddWorldPositionConstraint(ee_idx_,              // body index
                                          ...
                                          RigidTransformd());
    global_ik_.AddWorldRelativePositionConstraint(ee_idx_, Vector3d::Zero(),
                                                  plant_->GetBodyByName("iiwa_link_0").index(), Vector3d::Zero(),
                                                  X_WEe.translation() - X_W0.translation(),  // lower bound
                                                  X_WEe.translation() - X_W0.translation(),  // upper bound
                                                  RigidTransformd());
    global_ik_.AddWorldOrientationConstraint(ee_idx_,                                        // body index
    ...
    if (gurobi_solver.available()) {
        ...
        const Eigen::VectorXd q_no_cost = global_ik_.ReconstructGeneralizedPositionSolution(result);

        // Now add the cost on the posture error.
        // Any positive cost should be able to achieve the optimal solution
        // being equal to q.
        global_ik_.AddPostureCost(q, Eigen::VectorXd::Constant(plant_->num_bodies(), 1),
        ...
        const Eigen::VectorXd q_w_cost = global_ik_.ReconstructGeneralizedPositionSolution(result);
...
TEST_F(ToyTest, Test) {
    ...
    for (BodyIndex body_index{1}; body_index < plant_->num_bodies(); ++body_index) {
        ...
        global_ik.get_mutable_prog()->AddBoundingBoxConstraint(X_WB.rotation().matrix(), X_WB.rotation().matrix(),
                                                               global_ik.body_rotation_matrix(body_index));
        global_ik.get_mutable_prog()->AddBoundingBoxConstraint(X_WB.translation(), X_WB.translation(),
