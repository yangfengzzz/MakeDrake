...
namespace multibody {
namespace {
AutoDiffVecXd EvalOrientationConstraintAutoDiff(const Context<AutoDiffXd>& context,
                                                const MultibodyPlant<AutoDiffXd>& plant,
                                                const Frame<AutoDiffXd>& frameAbar,
                                                const RotationMatrixd& R_AbarA,
                                                const Frame<AutoDiffXd>& frameBbar,
                                                const RotationMatrixd& R_BbarB) {
    Vector1<AutoDiffXd> y_autodiff(1);
    const math::RotationMatrix<AutoDiffXd> R_AbarBbar = plant.CalcRelativeRotationMatrix(context, frameAbar, frameBbar);
    const math::RotationMatrix<AutoDiffXd> R_AB =
            R_AbarA.cast<AutoDiffXd>().transpose() * R_AbarBbar * R_BbarB.cast<AutoDiffXd>();
...
TEST_F(IiwaKinematicConstraintTest, OrientationConstraint) {
    ...
    const math::RotationMatrix<double> R_AbarA(
            Eigen::AngleAxisd(0.2 * M_PI, Eigen::Vector3d(0.2, 0.4, -0.5).normalized()));
    const math::RotationMatrix<double> R_BbarB(
            Eigen::AngleAxisd(-0.4 * M_PI, Eigen::Vector3d(0.1, 1.2, -0.7).normalized()));
    std::unique_ptr<OrientationConstraint> constraint{};
    {
        ...
        constraint = std::make_unique<OrientationConstraint>(plant_, frameAbar, R_AbarA_local, frameBbar, R_BbarB_local,
                                                             angle_bound, plant_context_);
    }

    EXPECT_EQ(constraint->num_constraints(), 1);
    EXPECT_EQ(constraint->num_vars(), plant_autodiff_->num_positions());
    EXPECT_TRUE(CompareMatrices(constraint->lower_bound(), Vector1d(2 * std::cos(angle_bound) + 1)));
    ...
    AutoDiffVecXd y_autodiff_expected = EvalOrientationConstraintAutoDiff(
            *plant_context_autodiff_, *plant_autodiff_, plant_autodiff_->GetFrameByName(frameAbar.name()), R_AbarA,
    ...
    plant_autodiff_->SetPositions(plant_context_autodiff_.get(), q_autodiff);
    constraint->Eval(q_autodiff, &y_autodiff);
    y_autodiff_expected = EvalOrientationConstraintAutoDiff(*plant_context_autodiff_, *plant_autodiff_,
    ...
    const OrientationConstraint constraint_from_autodiff(
            plant_autodiff_.get(), plant_autodiff_->get_frame(frameAbar_index), R_AbarA,
            plant_autodiff_->get_frame(frameBbar_index), R_BbarB, angle_bound, plant_context_autodiff_.get());
...
TEST_F(TwoFreeBodiesConstraintTest, OrientationConstraint) {
    ...
    const Eigen::Quaterniond body1_quaternion(
            Eigen::AngleAxisd(0.3 * M_PI, Eigen::Vector3d(0.1, 0.3, 0.2).normalized()));
    const Eigen::Quaterniond body2_quaternion(
            Eigen::AngleAxisd(-0.2 * M_PI, Eigen::Vector3d(0.4, 1.5, -0.2).normalized()));
    ...
    const math::RotationMatrix<double> R_BbarB(
            Eigen::AngleAxisd(-0.4 * M_PI, Eigen::Vector3d(0.1, 1.2, -0.7).normalized()));
    const math::RotationMatrixd R_AbarBbar(body1_quaternion.inverse() * body2_quaternion);
    const math::RotationMatrixd R_AB = R_AbarA.transpose() * R_AbarBbar * R_BbarB;
    const double theta = R_AB.ToAngleAxis().angle();

    OrientationConstraint good_constraint(plant_, plant_->get_frame(body1_index_), R_AbarA,
                                          plant_->get_frame(body2_index_), R_BbarB, theta * 1.01, plant_context_);
    EXPECT_TRUE(good_constraint.CheckSatisfied(q));

    OrientationConstraint bad_constraint(plant_, plant_->get_frame(body1_index_), R_AbarA,
                                         plant_->get_frame(body2_index_), R_BbarB, theta * 0.99, plant_context_);
...
TEST_F(IiwaKinematicConstraintTest, OrientationConstraintConstructionError) {
    ...
    EXPECT_THROW(OrientationConstraint(plant_, plant_->GetFrameByName("iiwa_link_7"),
                                       math::RotationMatrix<double>::Identity(), plant_->GetFrameByName("iiwa_link_3"),
