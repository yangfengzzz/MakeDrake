...
auto ConstructTwoFreeBodiesCost(const math::RotationMatrix<double>& R_AbarA,
    ...
    v.cost = std::make_unique<OrientationCost>(v.plant.get(), v.body1->body_frame(), R_AbarA, v.body2->body_frame(),
...
GTEST_TEST(OrientationCostTest, TwoFreeBodies) {
    ...
    auto [plant, context, body1, body2, cost] = ConstructTwoFreeBodiesCost<double>(R_AbarA, R_BbarB, c);
    ...
    auto CheckCases = [](const OrientationCost& orientation_cost, const OrientationCost& orientation_cost_ad,
    ...
    RigidTransform<double> X_WAbar(RollPitchYaw<double>(0.32, -0.24, -0.51), Vector3d(0.1, 0.3, 0.72));
    RigidTransform<double> X_WBbar(RollPitchYaw<double>(8.1, 0.42, -0.2), Vector3d(-0.84, 0.2, 1.4));
    plant->SetFreeBodyPose(context.get(), *body1, X_WAbar);
    plant->SetFreeBodyPose(context.get(), *body2, X_WBbar);
    q = plant->GetPositions(*context);

    R_AB = (X_WAbar.rotation() * R_AbarA).inverse() * X_WBbar.rotation() * R_BbarB;
