...
auto ConstructTwoFreeBodiesCost(const Eigen::Ref<const Vector3d>& p_AP,
    ...
    v.cost = std::make_unique<PositionCost>(v.plant.get(), v.body1->body_frame(), p_AP, v.body2->body_frame(), p_BQ, C,
                                            v.context.get());
...
GTEST_TEST(PositionCostTest, TwoFreeBodies) {
    ...
    auto [plant, context, body1, body2, cost] = ConstructTwoFreeBodiesCost<double>(p_AP, p_BQ, C);
    auto [plant_ad, context_ad, body1_ad, body2_ad, cost_ad] = ConstructTwoFreeBodiesCost<AutoDiffXd>(p_AP, p_BQ, C);

    // Note: We are passing PositionCost here instead of capturing them due to a
    // known issue in C++:
    // https://stackoverflow.com/questions/46114214/lambda-implicit-capture-fails-with-variable-declared-from-structured-binding
    auto CheckCases = [](const PositionCost& position_cost, const PositionCost& position_cost_ad, const VectorXd& q,
                         double y_expected) {
