...
TEST_F(KukaTest, FeasiblePostureTest) {
    ...
    for (int i = 0; i < 7; ++i) {
        q_grid(i, 0) = plant_->GetPositionLowerLimits()(i) * 0.95 + plant_->GetPositionUpperLimits()(i) * 0.05;
        q_grid(i, 1) = plant_->GetPositionLowerLimits()(i) * 0.5 + plant_->GetPositionUpperLimits()(i) * 0.5;
        q_grid(i, 2) = plant_->GetPositionLowerLimits()(i) * 0.05 + plant_->GetPositionUpperLimits()(i) * 0.95;
    }
    auto context = plant_->CreateDefaultContext();

    std::vector<solvers::Binding<solvers::BoundingBoxConstraint>> body_position_constraint;
    std::vector<solvers::Binding<solvers::BoundingBoxConstraint>> body_orientation_constraint;
    for (BodyIndex body{1}; body < plant_->num_bodies(); ++body) {
        ...
        body_position_constraint.push_back(
                global_ik_.get_mutable_prog()->AddBoundingBoxConstraint(0, 0, global_ik_.body_position(body)));
        ...
        body_orientation_constraint.push_back(
                global_ik_.get_mutable_prog()->AddBoundingBoxConstraint(0, 0, body_rotmat_flat));
    }
    ...
    while (q_count < num_q) {
        ...
        for (int body = 1; body < plant_->num_bodies(); ++body) {
            const math::RigidTransformd body_pose = plant_->CalcRelativeTransform(
                    *context, plant_->world_frame(), plant_->get_body(BodyIndex{body}).body_frame());
            ...
            rotmat_lb_flat << body_pose.rotation().matrix().col(0), body_pose.rotation().matrix().col(1),
                    body_pose.rotation().matrix().col(2);
            rotmat_ub_flat << body_pose.rotation().matrix().col(0), body_pose.rotation().matrix().col(1),
                    body_pose.rotation().matrix().col(2);
            body_orientation_constraint[body - 1].evaluator()->UpdateLowerBound(rotmat_lb_flat);
            body_orientation_constraint[body - 1].evaluator()->UpdateUpperBound(rotmat_ub_flat);
        }
