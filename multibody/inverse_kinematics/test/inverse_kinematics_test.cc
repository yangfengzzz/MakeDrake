#include "multibody/inverse_kinematics/inverse_kinematics.h"
...
public:
    ...
    std::unique_ptr<systems::Context<double>> RetrieveSolution(const solvers::MathematicalProgramResult& result) {
...
GTEST_TEST(InverseKinematicsTest, ConstructorWithJointLimits) {
    ...
    auto plant = ConstructIiwaPlant("package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf", 0.01);
    ...
    // IK constructor.
    auto q_test_with_joint_limits = ik_with_joint_limits.get_mutable_prog()->AddBoundingBoxConstraint(
            Eigen::VectorXd::Zero(7), Eigen::VectorXd::Zero(7), ik_with_joint_limits.q());
    auto q_test_without_joint_limits = ik_without_joint_limits.get_mutable_prog()->AddBoundingBoxConstraint(
            Eigen::VectorXd::Zero(7), Eigen::VectorXd::Zero(7), ik_without_joint_limits.q());
    auto check_q_with_joint_limits = [&ik_with_joint_limits, &q_test_with_joint_limits](const Eigen::VectorXd& q_test) {
    ...
    auto check_q_without_joint_limits = [&ik_without_joint_limits,
...
TEST_F(TwoFreeBodiesTest, ConstructorAddsUnitQuaterionConstraints) {
    ...
    EXPECT_TRUE(CompareMatrices(ik_.prog().GetInitialGuess(ik_.q().head<4>()), Eigen::Vector4d(1, 0, 0, 0)));

    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().head<4>(), Eigen::Vector4d(1, 2, 3, 4));
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().segment<4>(7), Eigen::Vector4d(.1, .2, .3, .4));
    ...
    ik2.get_mutable_prog()->SetInitialGuess(ik2.q().head<4>(), Eigen::Vector4d(1, 2, 3, 4));
    ik2.get_mutable_prog()->SetInitialGuess(ik2.q().segment<4>(7), Eigen::Vector4d(.1, .2, .3, .4));
...
GTEST_TEST(InverseKinematicsTest, ConstructorLockedJoints) {
    ...
    const auto& joint1 = plant.AddJoint<QuaternionFloatingJoint>("joint1", world, I, body1, I);
    const auto& joint2 = plant.AddJoint<QuaternionFloatingJoint>("joint2", world, I, body2, I);
    const auto& joint3 = plant.AddJoint<RevoluteJoint>("joint3", world, I, body3, I, X);
    const auto& joint4 = plant.AddJoint<RevoluteJoint>("joint4", world, I, body4, I, X);
    plant.Finalize();
    ...
    EXPECT_TRUE(CompareMatrices(ik.prog().GetInitialGuess(ik.q().segment(joint1.position_start(), 4)),
                                Eigen::Vector4d(1, 0, 0, 0)));
    EXPECT_TRUE(CompareMatrices(ik.prog().GetInitialGuess(ik.q().segment(joint2.position_start(), 4)),
                                Eigen::Vector4d(0, 1, 0, 0)));

    // The unit quaternion constraint is only added to joint1.
    ASSERT_EQ(prog.generic_constraints().size(), 1);
    const solvers::Binding<solvers::Constraint>& unit_quat = prog.generic_constraints().front();
    ...
    lower.segment(j2_start, 7) = upper.segment(j2_start, 7) = (Vector<double, 7>() << 0, 1, 0, 0, 0, 0, 0).finished();
    ...
    const solvers::Binding<solvers::BoundingBoxConstraint>& limits = prog.bounding_box_constraints().front();
...
TEST_F(TwoFreeBodiesTest, PositionConstraint) {
    ...
    ik_.AddPositionConstraint(body1_frame_, p_BQ, body2_frame_, p_AQ_lower, p_AQ_upper);

    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().head<4>(), Eigen::Vector4d(1, 0, 0, 0));
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().segment<4>(7), Eigen::Vector4d(1, 0, 0, 0));
    const auto result = Solve(ik_.prog());
    EXPECT_TRUE(result.is_success());
    RetrieveSolution(result);
    const Eigen::Vector3d p_AQ = body2_quaternion_sol_.inverse() *
                                 (body1_quaternion_sol_ * p_BQ + body1_position_sol_ - body2_position_sol_);
    const double tol = 1E-6;
    EXPECT_TRUE((p_AQ.array() <= p_AQ_upper.array() + Eigen::Array3d::Constant(tol)).all());
    EXPECT_TRUE((p_AQ.array() >= p_AQ_lower.array() - Eigen::Array3d::Constant(tol)).all());
}

TEST_F(TwoFreeBodiesTest, PositionCost) {
    const Eigen::Vector3d p_AP(-0.1, -0.2, -0.3);
    const Eigen::Vector3d p_BQ(0.2, 0.3, 0.5);
    auto binding = ik_.AddPositionCost(body1_frame_, p_AP, body2_frame_, p_BQ, Eigen::Matrix3d::Identity());
    ...
    // cost.
    math::RigidTransform<double> X_WA(math::RollPitchYaw<double>(0.32, -0.24, -0.51), Eigen::Vector3d(0.1, 0.3, 0.72));
    math::RigidTransform<double> X_WB(math::RollPitchYaw<double>(8.1, 0.42, -0.2), Eigen::Vector3d(-0.84, 0.2, 1.4));
    auto context = two_bodies_plant_->CreateDefaultContext();
    two_bodies_plant_->SetFreeBodyPose(context.get(), two_bodies_plant_->GetBodyByName("body1"), X_WA);
    two_bodies_plant_->SetFreeBodyPose(context.get(), two_bodies_plant_->GetBodyByName("body2"), X_WB);
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q(), two_bodies_plant_->GetPositions(*context));

    const Eigen::Vector3d p_AQ = X_WA.inverse() * X_WB * p_BQ;
    const Eigen::Vector3d err = p_AQ - p_AP;
    const double expected_cost = err.dot(err);

    EXPECT_NEAR(ik_.prog().EvalBindingAtInitialGuess(binding)[0], expected_cost, 1e-12);
}
...
TEST_F(TwoFreeBodiesTest, OrientationConstraint) {
    ...
    const math::RotationMatrix<double> R_AbarA(
            Eigen::AngleAxisd(0.1 * M_PI, Eigen::Vector3d(0.2, 0.4, 1.2).normalized()));
    const math::RotationMatrix<double> R_BbarB(
            Eigen::AngleAxisd(0.2 * M_PI, Eigen::Vector3d(1.2, 2.1, -0.2).normalized()));

    ik_.AddOrientationConstraint(body1_frame_, R_AbarA, body2_frame_, R_BbarB, angle_bound);
    ...
    const auto q_sol = result.GetSolution(ik_.q());
    RetrieveSolution(result);
    const math::RotationMatrix<double> R_AbarBbar(body1_quaternion_sol_.inverse() * body2_quaternion_sol_);
    const math::RotationMatrix<double> R_AB = R_AbarA.transpose() * R_AbarBbar * R_BbarB;
    const double angle = R_AB.ToAngleAxis().angle();
    EXPECT_LE(angle, angle_bound + 1E-6);
}

TEST_F(TwoFreeBodiesTest, OrientationCost) {
    const math::RotationMatrix<double> R_AbarA(math::RollPitchYaw<double>(1, 2, 3));
    const math::RotationMatrix<double> R_BbarB(math::RollPitchYaw<double>(4, 5, 6));
    const double c{2.4};
    auto binding = ik_.AddOrientationCost(body1_frame_, R_AbarA, body2_frame_, R_BbarB, c);

    // We don't need to test the cost implementation, only that the arguments are
    // passed correctly.  Just set an arbitrary (but valid) q and evaluate the
    // cost.
    math::RigidTransform<double> X_WAbar(math::RollPitchYaw<double>(0.32, -0.24, -0.51),
                                         Eigen::Vector3d(0.1, 0.3, 0.72));
    math::RigidTransform<double> X_WBbar(math::RollPitchYaw<double>(8.1, 0.42, -0.2), Eigen::Vector3d(-0.84, 0.2, 1.4));
    auto context = two_bodies_plant_->CreateDefaultContext();
    two_bodies_plant_->SetFreeBodyPose(context.get(), two_bodies_plant_->GetBodyByName("body1"), X_WAbar);
    two_bodies_plant_->SetFreeBodyPose(context.get(), two_bodies_plant_->GetBodyByName("body2"), X_WBbar);
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q(), two_bodies_plant_->GetPositions(*context));

    const math::RotationMatrix<double> R_AB = (X_WAbar.rotation() * R_AbarA).inverse() * X_WBbar.rotation() * R_BbarB;
    const double theta = math::wrap_to(R_AB.ToAngleAxis().angle(), -M_PI / 2.0, M_PI / 2.0);
    EXPECT_NEAR(ik_.prog().EvalBindingAtInitialGuess(binding)[0], c * (1.0 - cos(theta)), 1e-12);
}
...
TEST_F(TwoFreeBodiesTest, GazeTargetConstraint) {
    ...
    ik_.AddGazeTargetConstraint(body1_frame_, p_AS, n_A, body2_frame_, p_BT, cone_half_angle);
    ...
    RetrieveSolution(result);
    const Eigen::Vector3d p_WS = body1_quaternion_sol_ * p_AS + body1_position_sol_;
    const Eigen::Vector3d p_WT = body2_quaternion_sol_ * p_BT + body2_position_sol_;
    const Eigen::Vector3d p_ST_A = body1_quaternion_sol_.inverse() * (p_WT - p_WS);
    EXPECT_GE(p_ST_A.dot(n_A), std::cos(cone_half_angle) * p_ST_A.norm() * n_A.norm() - 1E-3);
}
...
TEST_F(TwoFreeBodiesTest, AngleBetweenVectorsConstraint) {
    ...
    ik_.AddAngleBetweenVectorsConstraint(body1_frame_, n_A, body2_frame_, n_B, angle_lower, angle_upper);
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().head<4>(), Eigen::Vector4d(1, 0, 0, 0));
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().segment<4>(7), Eigen::Vector4d(1, 0, 0, 0));
    const auto result = Solve(ik_.prog());
    EXPECT_TRUE(result.is_success());
    ...
    const double angle = std::acos(n_A_W.dot(n_B_W) / (n_A_W.norm() * n_B_W.norm()));
...
TEST_F(TwoFreeBodiesTest, AngleBetweenVectorsCost) {
    ...
    const double c = 10;
    auto binding = ik_.AddAngleBetweenVectorsCost(body1_frame_, na_A, body2_frame_, nb_B, c);
...
TEST_F(TwoFreeBodiesTest, PointToPointDistanceConstraint) {
    ...
    ik_.AddPointToPointDistanceConstraint(body1_frame_, p_B1P1, body2_frame_, p_B2P2, distance_lower, distance_upper);
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().head<4>(), Eigen::Vector4d(1, 0, 0, 0));
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().segment<4>(7), Eigen::Vector4d(1, 0, 0, 0));
    const auto result = Solve(ik_.prog());
    EXPECT_TRUE(result.is_success());
    ...
    const Eigen::Vector3d p_WP1 = body1_position_sol_ + body1_quaternion_sol_ * p_B1P1;
    const Eigen::Vector3d p_WP2 = body2_position_sol_ + body2_quaternion_sol_ * p_B2P2;
...
TEST_F(TwoFreeBodiesTest, PointToLineDistanceConstraint) {
    ...
    ik_.AddPointToLineDistanceConstraint(body1_frame_, p_B1P, body2_frame_, p_B2Q, n_B2, distance_lower,
                                         distance_upper);
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().head<4>(), Eigen::Vector4d(1, 0, 0, 0));
    ik_.get_mutable_prog()->SetInitialGuess(ik_.q().segment<4>(7), Eigen::Vector4d(1, 0, 0, 0));
    const auto result = Solve(ik_.prog());
    ...
    const Eigen::Vector3d p_WP = body1_position_sol_ + body1_quaternion_sol_ * p_B1P;
    const Eigen::Vector3d p_WQ = body2_position_sol_ + body2_quaternion_sol_ * p_B2Q;
    const Eigen::Vector3d n_W = body2_quaternion_sol_ * n_B2;
    const Eigen::Vector3d n_W_normalized = n_W.normalized();

    const double distance_sol = (p_WQ + (p_WP - p_WQ).dot(n_W_normalized) * n_W_normalized - p_WP).norm();
...
TEST_F(TwoFreeSpheresTest, MinimumDistanceLowerBoundConstraintTest) {
    ...
    auto constraint = ik.AddMinimumDistanceLowerBoundConstraint(min_distance_lower);

    // The two spheres are colliding in the initial guess.
    ik.get_mutable_prog()->SetInitialGuess(ik.q().head<4>(), Eigen::Vector4d(1, 0, 0, 0));
    ik.get_mutable_prog()->SetInitialGuess(ik.q().segment<3>(4), Eigen::Vector3d(0, 0, 0.01));
    ik.get_mutable_prog()->SetInitialGuess(ik.q().segment<4>(7), Eigen::Vector4d(1, 0, 0, 0));
    ik.get_mutable_prog()->SetInitialGuess(ik.q().tail<3>(), Eigen::Vector3d(0, 0, -0.01));
    ...
    auto solve_and_check = [&]() {
        ...
        const Eigen::Quaterniond quat_WB1(result.GetSolution(ik.q()(0)), result.GetSolution(ik.q()(1)),
                                          result.GetSolution(ik.q()(2)), result.GetSolution(ik.q()(3)));
        const Eigen::Vector3d p_WB2 = result.GetSolution(ik.q().tail<3>());
        const Eigen::Quaterniond quat_WB2(result.GetSolution(ik.q()(7)), result.GetSolution(ik.q()(8)),
                                          result.GetSolution(ik.q()(9)), result.GetSolution(ik.q()(10)));
        const Eigen::Vector3d p_WS1 = p_WB1 + quat_WB1.toRotationMatrix() * X_B1S1_.translation();
        const Eigen::Vector3d p_WS2 = p_WB2 + quat_WB2.toRotationMatrix() * X_B2S2_.translation();
        ...
        EXPECT_GE((p_WS1 - p_WS2).norm() - radius1_ - radius2_, min_distance_lower - tol);
    };

    solve_and_check();

    // Now set the two spheres to coincide at the initial guess, and solve again.
    ik.get_mutable_prog()->SetInitialGuess(ik.q().tail<3>(), ik.prog().initial_guess().segment<3>(4));
    solve_and_check();
}
...
TEST_F(TwoFreeSpheresTest, MinimumDistanceUpperBoundConstraintTest) {
    ...
    auto constraint = ik.AddMinimumDistanceUpperBoundConstraint(min_distance_upper, influence_distance_offset);
    ...
    auto solve_and_check = [&]() {
        ...
        // minimum_distance.
        const double tol = 2e-4;
        EXPECT_LE((p_WS1 - p_WS2).norm() - radius1_ - radius2_, min_distance_upper + tol);
    };
