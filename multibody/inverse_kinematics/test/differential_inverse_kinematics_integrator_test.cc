...
GTEST_TEST(DifferentialInverseKinematicsIntegratorTest, BasicTest) {
    ...
    const multibody::Frame<double>& frame_E = robot->GetFrameByName("iiwa_link_7");

    DifferentialInverseKinematicsParameters params(robot->num_positions(), robot->num_velocities());

    const double time_step = 0.1;
    DifferentialInverseKinematicsIntegrator diff_ik(*robot, frame_E, time_step, params);
    ...
    diff_ik_context->get_mutable_discrete_state(1).SetAtIndex(
            0, static_cast<double>(DifferentialInverseKinematicsStatus::kNoSolutionFound));
    ...
    EXPECT_TRUE(diff_ik.ForwardKinematics(*diff_ik_context)
                        .IsExactlyEqualTo(robot->EvalBodyPoseInWorld(*robot_context, frame_E.body())));
    ...
    for (const auto& [data, events] : diff_ik.MapPeriodicEventsByTiming()) {
        dynamic_cast<const systems::DiscreteUpdateEvent<double>*>(events[0])->handle(diff_ik, *diff_ik_context,
                                                                                     discrete_state.get());
    }
    ...
    DifferentialInverseKinematicsResult result =
            DoDifferentialInverseKinematics(*robot, *robot_context, X_WE_desired, frame_E, params);
  ...
  const auto A = (MatrixX<double>(2, 7) <<
    ...
    diff_ik.get_mutable_parameters().AddLinearVelocityConstraint(std::make_shared<solvers::LinearConstraint>(A, b, b));
...
GTEST_TEST(DifferentialInverseKinematicsIntegratorTest, FrameATest) {
    ...
    auto robot_context = robot->CreateDefaultContext();
    const multibody::Frame<double>& frame_A = robot->GetFrameByName("iiwa_link_0");
    const multibody::Frame<double>& frame_E = robot->GetFrameByName("iiwa_link_7");

    DifferentialInverseKinematicsParameters params(robot->num_positions(), robot->num_velocities());

    const double time_step = 0.1;
    DifferentialInverseKinematicsIntegrator diff_ik(*robot, frame_A, frame_E, time_step, params);
    auto diff_ik_context = diff_ik.CreateDefaultContext();
    ...
    Eigen::VectorXd q(14);
    q << 0.1, 0.2, -.15, 0.43, -0.32, -0.21, 0.11, 0.4, 0.21, -.25, 0.67, -0.21, -0.53, 0.21;

    // Test SetPosition and ForwardKinematics.
    robot->SetPositions(robot_context.get(), q);
    diff_ik.SetPositions(diff_ik_context.get(), q);
    EXPECT_TRUE(
            diff_ik.ForwardKinematics(*diff_ik_context).IsExactlyEqualTo(frame_E.CalcPose(*robot_context, frame_A)));

    // Test that discrete update is equivalent to calling diff IK directly.
    math::RigidTransformd X_AE_desired(Eigen::Vector3d(0.2, 0.0, 0.2));
    diff_ik.GetInputPort("X_AE_desired").FixValue(diff_ik_context.get(), X_AE_desired);
    auto discrete_state = diff_ik.AllocateDiscreteVariables();
    for (const auto& [data, events] : diff_ik.MapPeriodicEventsByTiming()) {
        dynamic_cast<const systems::DiscreteUpdateEvent<double>*>(events[0])->handle(diff_ik, *diff_ik_context,
                                                                                     discrete_state.get());
    ...
    params.set_time_step(time_step);  // intentionally set this after diff_ik call
    DifferentialInverseKinematicsResult result =
            DoDifferentialInverseKinematics(*robot, *robot_context, X_AE_desired, frame_A, frame_E, params);

    EXPECT_EQ(result.status, DifferentialInverseKinematicsStatus::kSolutionFound);
    Eigen::VectorXd qdot(robot->num_positions());
    robot->MapVelocityToQDot(*robot_context, result.joint_velocities.value(), &qdot);
    EXPECT_TRUE(CompareMatrices(q + time_step * qdot, discrete_state->get_vector(0).get_value()));
}
...
GTEST_TEST(DeprecatedTest, BasicTest) {
    ...
    // Test that discrete update is equivalent to calling diff IK directly.
    math::RigidTransformd X_WE_desired(Eigen::Vector3d(0.2, 0.0, 0.2));
    diff_ik.GetInputPort("X_WE_desired").FixValue(diff_ik_context.get(), X_WE_desired);
    auto discrete_state = diff_ik.AllocateDiscreteVariables();
    for (const auto& [data, events] : diff_ik.MapPeriodicEventsByTiming()) {
        dynamic_cast<const systems::DiscreteUpdateEvent<double>*>(events[0])->handle(diff_ik, *diff_ik_context,
                                                                                     discrete_state.get());
...
// logic is disabled.
GTEST_TEST(DifferentialInverseKinematicsIntegratorTest, DifferenceEquationSystemTest) {
    auto robot = MakeIiwa();
    auto robot_context = robot->CreateDefaultContext();
    const multibody::Frame<double>& frame_E = robot->GetFrameByName("iiwa_link_7");
    ...
    DifferentialInverseKinematicsIntegrator diff_ik_with_status_state(*robot, frame_E, time_step, params);
    EXPECT_EQ(diff_ik_with_status_state.num_discrete_state_groups(), 2);

    DifferentialInverseKinematicsIntegrator diff_ik_without_status_state(*robot, frame_E, time_step, params, nullptr,
                                                                         false);
...
GTEST_TEST(DifferentialInverseKinematicsIntegratorTest, InitializationEvent) {
    ...
    DifferentialInverseKinematicsParameters params(robot->num_positions(), robot->num_velocities());
    const double time_step = 0.1;
    auto diff_ik = builder.AddSystem<DifferentialInverseKinematicsIntegrator>(*robot, frame_E, time_step, params);
    builder.Connect(robot->get_state_output_port(), diff_ik->GetInputPort("robot_state"));
    auto diagram = builder.Build();
    systems::Simulator<double> simulator(*diagram);

    systems::Context<double>& context = simulator.get_mutable_context();
    systems::Context<double>& robot_context = robot->GetMyMutableContextFromRoot(&context);
    robot->SetPositions(&robot_context, Eigen::VectorXd::Constant(7, 0.1));
    systems::Context<double>& diff_ik_context = diff_ik->GetMyMutableContextFromRoot(&context);
    ...
    // robot positions.
    EXPECT_FALSE(
            CompareMatrices(diff_ik_context.get_discrete_state(0).get_value(), robot->GetPositions(robot_context)));
    simulator.Initialize();
    EXPECT_TRUE(CompareMatrices(diff_ik_context.get_discrete_state(0).get_value(), robot->GetPositions(robot_context)));
}
...
GTEST_TEST(DifferentialInverseKinematicsIntegratorTest, UseRobotStatePort) {
    ...
    robot->SetPositions(&robot_context, Eigen::VectorXd::Constant(7, 0.1));
    systems::Context<double>& diff_ik_context = diff_ik->GetMyMutableContextFromRoot(&context);
    const math::RigidTransform<double> X_G = robot->GetBodyByName("iiwa_link_7").EvalPoseInWorld(robot_context);
    ...
    EXPECT_FALSE(CompareMatrices(discrete_state->get_value(0), robot->GetPositions(robot_context)));

    // use_robot_state port is set to false.
    diff_ik_context.FixInputPort(diff_ik->GetInputPort("use_robot_state").get_index(), Value<bool>{false});
    for (const auto& [data, events] : diff_ik->MapPeriodicEventsByTiming()) {
        dynamic_cast<const systems::DiscreteUpdateEvent<double>*>(events[0])->handle(*diff_ik, diff_ik_context,
                                                                                     discrete_state.get());
    }
    ...
    diff_ik_context.FixInputPort(diff_ik->GetInputPort("use_robot_state").get_index(), Value<bool>{true});
    for (const auto& [data, events] : diff_ik->MapPeriodicEventsByTiming()) {
        dynamic_cast<const systems::DiscreteUpdateEvent<double>*>(events[0])->handle(*diff_ik, diff_ik_context,
                                                                                     discrete_state.get());
    }
    ...
    EXPECT_TRUE(CompareMatrices(discrete_state->get_value(0), robot->GetPositions(robot_context), 1e-12));
}
