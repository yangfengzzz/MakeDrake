...
void TestIiwa(const MultibodyPlant<T>* plant,
    ...
    PointToPointDistanceConstraint constraint(plant, frame1, p_B1P1, frame2, p_B2P2, distance_lower, distance_upper,
                                              plant_context);
    ...
    plant->CalcPointsPositions(*plant_context, frame1, p_B1P1.cast<T>(), plant->world_frame(), &p_WP1);
    plant->CalcPointsPositions(*plant_context, frame2, p_B2P2.cast<T>(), plant->world_frame(), &p_WP2);
    ...
    plant_autodiff->CalcPointsPositions(*plant_context_autodiff, plant_autodiff->get_frame(frame2_index),
...
TEST_F(IiwaKinematicConstraintTest, PointToPointDistanceConstraint) {
    ...
    TestIiwa(plant_, plant_context_, plant_autodiff_.get(), plant_context_autodiff_.get());
    // Test PointToPointDistanceConstraint constructed with MBP<AutoDiffXd>
    TestIiwa(plant_autodiff_.get(), plant_context_autodiff_.get(), plant_autodiff_.get(),
...
TEST_F(TwoFreeBodiesConstraintTest, PointToPointDistanceConstraint) {
    ...
    plant_->CalcPointsPositions(*plant_context_, plant_->get_frame(body1_index_), p_B1P1, plant_->world_frame(),
                                &p_WP1);
    plant_->CalcPointsPositions(*plant_context_, plant_->get_frame(body2_index_), p_B2P2, plant_->world_frame(),
                                &p_WP2);
    const double distance = (p_WP1 - p_WP2).norm();
    PointToPointDistanceConstraint good_constraint(plant_, plant_->get_frame(body1_index_), p_B1P1,
                                                   plant_->get_frame(body2_index_), p_B2P2, 0.5 * distance,
                                                   2 * distance, plant_context_);
    EXPECT_TRUE(good_constraint.CheckSatisfied(q));
    PointToPointDistanceConstraint bad_constraint1(plant_, plant_->get_frame(body1_index_), p_B1P1,
                                                   plant_->get_frame(body2_index_), p_B2P2, 1.01 * distance,
                                                   2 * distance, plant_context_);
    EXPECT_FALSE(bad_constraint1.CheckSatisfied(q));
    PointToPointDistanceConstraint bad_constraint2(plant_, plant_->get_frame(body1_index_), p_B1P1,
                                                   plant_->get_frame(body2_index_), p_B2P2, 0.5 * distance,
                                                   0.99 * distance, plant_context_);
