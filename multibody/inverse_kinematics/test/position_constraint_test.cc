...
AutoDiffVecXd EvalPositionConstraintAutoDiff(const Context<AutoDiffXd>& context,
    ...
    plant.CalcPointsPositions(context, frameB, p_BQ.cast<AutoDiffXd>(), frameAbar, &p_AbarQ);
...
TEST_F(IiwaKinematicConstraintTest, PositionConstraint) {
    ...
    const math::RigidTransformd X_AAbar(
            math::RotationMatrixd(Eigen::AngleAxisd(0.2 * M_PI, Eigen::Vector3d(1.0 / 3, 2.0 / 3, -2.0 / 3))),
            Eigen::Vector3d(0.5, 0.2, -0.1));
    const Frame<double>& frameB = plant_->get_frame(frameB_index);

    auto check_constraint = [this, p_AQ_lower, p_AQ_upper, &frameB, p_BQ, &frameAbar](
                                    const PositionConstraint& dut, const math::RigidTransformd& X_AAbar_val) {
        ...
        this->plant_->CalcPointsPositions(*(this->plant_context_), frameB, p_BQ, frameAbar, &p_AbarQ);
        ...
        this->plant_autodiff_->SetPositions(this->plant_context_autodiff_.get(), q_autodiff);
        Vector3<AutoDiffXd> y_autodiff_expected =
                EvalPositionConstraintAutoDiff(*(this->plant_context_autodiff_), *(this->plant_autodiff_),
    ...
    PositionConstraint constraint(plant_, frameAbar, X_AAbar.inverse(), p_AQ_lower, p_AQ_upper, frameB, p_BQ,
                                  plant_context_);
    check_constraint(constraint, X_AAbar);

    // Constructs constraint with empty X_AAbar.
    PositionConstraint constraint_empty_X_AAbar(plant_, frameAbar, p_AQ_lower, p_AQ_upper, frameB, p_BQ,
                                                plant_context_);
    ...
    PositionConstraint constraint_from_autodiff(
            plant_autodiff_.get(), plant_autodiff_->get_frame(frameAbar_index), X_AAbar.inverse(), p_AQ_lower,
            p_AQ_upper, plant_autodiff_->get_frame(frameB_index), p_BQ, plant_context_autodiff_.get());
    Eigen::VectorXd q(7);
    q << 0.1, 0.2, 0.3, 0.4, -0.1, -0.2, -0.3;
...
TEST_F(TwoFreeBodiesConstraintTest, PositionConstraint) {
    ...
    Eigen::Vector3d p_AQ;
    plant_->CalcPointsPositions(*plant_context_, plant_->get_frame(body1_index_), p_BQ, plant_->get_frame(body2_index_),
                                &p_AQ);

    {
        PositionConstraint good_constraint(
                plant_, plant_->get_frame(body2_index_), p_AQ - Eigen::Vector3d::Constant(0.001),
                p_AQ + Eigen::Vector3d::Constant(0.001), plant_->get_frame(body1_index_), p_BQ, plant_context_);
        EXPECT_TRUE(good_constraint.CheckSatisfied(q));
    }
    {
        PositionConstraint bad_constraint(
                plant_, plant_->get_frame(body2_index_), p_AQ - Eigen::Vector3d::Constant(0.002),
                p_AQ - Eigen::Vector3d::Constant(0.001), plant_->get_frame(body1_index_), p_BQ, plant_context_);
