...
TEST_F(TwoFreeBodiesConstraintTest, PolyhedronConstraint) {
    ...
    const Eigen::Quaterniond body1_quaternion(Eigen::Vector4d(0.2, 0.5, -1, 2).normalized());
    const Eigen::Quaterniond body2_quaternion(Eigen::Vector4d(0.5, -1.5, -0.1, 2.3).normalized());
  ...
  A << 0.2, -0.5, 0.1, 1.2, 3.1, -1,
    ...
    plant_->CalcPointsPositions(*plant_context_, plant_->get_frame(frameG_index), p_GP, plant_->get_frame(frameF_index),
                                &p_FP);
    Vector6d p_FP_stack;
    p_FP_stack << p_FP.col(0), p_FP.col(1);
    const Eigen::Vector2d y_expected = A * p_FP_stack;
    PolyhedronConstraint dut(plant_, plant_->get_frame(frameF_index), plant_->get_frame(frameG_index), p_GP, A, b,
                             plant_context_);
    EXPECT_TRUE(CompareMatrices(dut.lower_bound(), Eigen::VectorXd::Constant(b.rows(), -kInf)));
    EXPECT_TRUE(CompareMatrices(dut.upper_bound(), b));
    PolyhedronConstraint dut_ad(plant_autodiff_.get(), plant_autodiff_->get_frame(frameF_index),
                                plant_autodiff_->get_frame(frameG_index), p_GP, A, b, plant_context_autodiff_.get());
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_ad), math::ExtractValue(y_ad_expected), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_ad), math::ExtractGradient(y_ad_expected), tol));
}
...
TEST_F(TwoFreeBodiesConstraintTest, SquareGazeCone) {
  ...
  A << -0.2, 0, -0.1,
       ...
       0, 0.3, -0.1;
    // clang-format on
    PolyhedronConstraint dut(plant_, plant_->get_frame(camera_frame), plant_->get_frame(target_frame), p_TP, A,
...
TEST_F(TwoFreeBodiesConstraintTest, AlignVector) {
    ...
    PolyhedronConstraint dut(plant_, plant_->world_frame(), plant_->get_frame(body1_index_), p_GP, A,
    ...
    const math::RotationMatrixd R_WG =
            math::RotationMatrixd::MakeFromOneUnitVector(Eigen::Vector3d(1.0 / 3, 2. / 3, 2. / 3), 2);
