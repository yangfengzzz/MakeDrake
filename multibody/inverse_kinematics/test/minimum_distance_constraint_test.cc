...
#include "math/compute_numerical_gradient.h"
#include "math/rigid_transform.h"
#include "multibody/inverse_kinematics/minimum_distance_lower_bound_constraint.h"
#include "multibody/inverse_kinematics/minimum_distance_upper_bound_constraint.h"
#include "multibody/inverse_kinematics/test/inverse_kinematics_test_utilities.h"
#include "planning/scene_graph_collision_checker.h"
#include "solvers/minimum_value_constraint.h"
#include "solvers/test_utilities/check_constraint_eval_nonsymbolic.h"
...
public:
    void CheckConstraintBounds(const MinimumDistanceLowerBoundConstraint& constraint) const {
    ...
    void CheckConstraintBounds(const MinimumDistanceUpperBoundConstraint& constraint) const {
        EXPECT_EQ(constraint.num_constraints(), 1);
    ...
    void ConstructQ(const Eigen::Vector3d& p_WB1,
                    const Eigen::Vector3d& p_WB2,
                    Eigen::Matrix<double, kNumPositionsForTwoFreeBodies, 1>* q,
                    Eigen::Matrix<AutoDiffXd, kNumPositionsForTwoFreeBodies, 1>* q_autodiff) const {
        ...
        *q << QuaternionToVectorWxyz(sphere1_quaternion), p_WB1, QuaternionToVectorWxyz(sphere2_quaternion), p_WB2;
        *q_autodiff = math::InitializeAutoDiff(*q);
    }

    template <typename C>
    void CheckConstraintEvalLargerThanInfluenceDistance(const C& constraint,
                                                        const Eigen::Vector3d& p_WB1,
                                                        const Eigen::Vector3d& p_WB2,
                                                        bool expect_satisfied) const {
        ...
        for (int i = 0; i < constraint.num_constraints(); ++i) {
            EXPECT_TRUE(CompareMatrices(y_autodiff(i).derivatives(),
    ...
    void CheckConstraintEvalSmallerThanMinimumLowerDistance(const MinimumDistanceLowerBoundConstraint& constraint,
                                                            const Eigen::Vector3d& p_WB1,
                                                            const Eigen::Vector3d& p_WB2) const {
        Eigen::Matrix<double, kNumPositionsForTwoFreeBodies, 1> q;
        Eigen::Matrix<AutoDiffXd, kNumPositionsForTwoFreeBodies, 1> q_autodiff;
    ...
    void CheckConstraintEvalSmallerThanMinimumUpperDistance(const MinimumDistanceUpperBoundConstraint& constraint,
                                                            const Eigen::Vector3d& p_WB1,
                                                            const Eigen::Vector3d& p_WB2) const {
        Eigen::Matrix<double, kNumPositionsForTwoFreeBodies, 1> q;
        Eigen::Matrix<AutoDiffXd, kNumPositionsForTwoFreeBodies, 1> q_autodiff;
    ...
    void CheckConstraintEval(const MinimumDistanceLowerBoundConstraint& constraint) const {
        ...
        CheckConstraintEvalLargerThanInfluenceDistance(constraint, p_WB1, p_WB2, true /* expect satisfied */);
        ...
        p_WB2 = p_WB1 +
                Eigen::Vector3d(1.0 / 3, 2.0 / 3, 2.0 / 3) *
                        (radius1_ + radius2_ + 0.5 * (constraint.distance_bound() + constraint.influence_distance()));
        CheckConstraintEvalBetweenMinimumLowerAndInfluenceDistance(constraint, p_WB1, p_WB2);

        // Two spheres are colliding.
        p_WB1 << 0.1, 0.2, 0.3;
        p_WB2 << 0.11, 0.21, 0.31;
        CheckConstraintEvalSmallerThanMinimumLowerDistance(constraint, p_WB1, p_WB2);

        // Two spheres are separated, but their distance is smaller than
        // minimum_distance_lower.
        p_WB1 << 0.1, 0.2, 0.3;
        p_WB2 = p_WB1 +
                Eigen::Vector3d(1.0 / 3, 2.0 / 3, 2.0 / 3) * (radius1_ + radius2_ + 0.5 * constraint.distance_bound());

        CheckConstraintEvalSmallerThanMinimumLowerDistance(constraint, p_WB1, p_WB2);
    }

    void CheckConstraintEval(const MinimumDistanceUpperBoundConstraint& constraint) const {
        // Distance between the spheres is larger than influence_distance
        Eigen::Vector3d p_WB1(0.2, 1.2, 0.3);
        Eigen::Vector3d p_WB2(1.2, -0.4, 2.3);
        CheckConstraintEvalLargerThanInfluenceDistance(constraint, p_WB1, p_WB2, false /* expect satisfied */);
        ...
        p_WB2 = p_WB1 +
                Eigen::Vector3d(1.0 / 3, 2.0 / 3, 2.0 / 3) *
                        (radius1_ + radius2_ + 0.5 * (constraint.distance_bound() + constraint.influence_distance()));
        CheckConstraintEvalBetweenMinimumUpperAndInfluenceDistance(constraint, p_WB1, p_WB2);

        // Two spheres are colliding.
        p_WB1 << 0.1, 0.2, 0.3;
        p_WB2 << 0.11, 0.21, 0.31;
        DRAKE_DEMAND((p_WB1 - p_WB2).norm() - radius1_ - radius2_ < constraint.distance_bound());
        CheckConstraintEvalSmallerThanMinimumUpperDistance(constraint, p_WB1, p_WB2);
...
public:
    explicit NFreeSpheresModel(int num_spheres) : num_spheres_{num_spheres} {
        ...
        const RotationalInertia<double> I_AAcm_A = UnitInertia<double>::SolidSphere(radius_);
...
TEST_F(TwoFreeSpheresMinimumDistanceTest, ExponentialPenaltyLowerBound) {
    ...
    const MinimumDistanceLowerBoundConstraint constraint(plant_double_, minimum_distance_lower, plant_context_double_,
...
TEST_F(TwoFreeSpheresMinimumDistanceTest, ExponentialPenaltyUpperBound) {
    ...
    const double influence_distance_offset{1.0};
    const MinimumDistanceUpperBoundConstraint constraint(plant_double_, minimum_distance_upper, plant_context_double_,
                                                         influence_distance_offset,
                                                         solvers::ExponentiallySmoothedHingeLoss);
    CheckConstraintBounds(constraint);
...
TEST_F(TwoFreeSpheresMinimumDistanceTest, QuadraticallySmoothedHingePenaltyLowerBound) {
    const double minimum_distance_lower{0.1};
    const double influence_distance_offset{1.0};
...
TEST_F(TwoFreeSpheresMinimumDistanceTest, QuadraticallySmoothedHingePenaltyUpperBound) {
    const double minimum_distance_upper{0.2};
    const double influence_distance_offset{1.0};
    // The default penalty type is smoothed hinge.
    const MinimumDistanceUpperBoundConstraint constraint(plant_double_, minimum_distance_upper, plant_context_double_,
...
GTEST_TEST(MinimumDistanceLowerBoundConstraintTest, MultibodyPlantWithouthGeometrySource) {
    auto plant = ConstructTwoFreeBodiesPlant<double>();
    auto context = plant->CreateDefaultContext();
    DRAKE_EXPECT_THROWS_MESSAGE(MinimumDistanceLowerBoundConstraint(plant.get(), 0.1, context.get()),
...
GTEST_TEST(MinimumDistanceUpperBoundConstraintTest, MultibodyPlantWithouthGeometrySource) {
    auto plant = ConstructTwoFreeBodiesPlant<double>();
    auto context = plant->CreateDefaultContext();
    DRAKE_EXPECT_THROWS_MESSAGE(MinimumDistanceUpperBoundConstraint(plant.get(), 0.1, context.get(), 1),
                                "Kinematic constraint: MultibodyPlant has not registered "
                                "with a SceneGraph yet. Please refer to "
                                "AddMultibodyPlantSceneGraph on how to connect MultibodyPlant to "
                                "SceneGraph.");
...
GTEST_TEST(MinimumDistanceLowerUpperBoundConstraintTest, MultibodyPlantWithoutCollisionPairs) {
    ...
    {
        const double minimum_distance_lower(0.1);
        const MinimumDistanceLowerBoundConstraint lower_bound_constraint(&plant, minimum_distance_lower, plant_context);

        Eigen::Matrix<AutoDiffXd, kNumPositionsForTwoFreeBodies, 1> q_autodiff =
                math::InitializeAutoDiff(Eigen::VectorXd::Zero(kNumPositionsForTwoFreeBodies));
    ...
    }
    {
        ...
        const MinimumDistanceUpperBoundConstraint upper_bound_constraint(&plant, minimum_distance_upper, plant_context,
                                                                         influence_distance_offset);
...
TEST_F(TwoFreeSpheresTest, NonpositiveInfluenceDistanceOffsetLowerBound) {
    DRAKE_EXPECT_THROWS_MESSAGE(
            MinimumDistanceLowerBoundConstraint(plant_double_, 0.1, plant_context_double_, {}, -0.05),
    ...
    EXPECT_THROW(MinimumDistanceLowerBoundConstraint(plant_double_, 0.1, plant_context_double_, {}, 0.0),
                 std::exception);
}

TEST_F(TwoFreeSpheresTest, NonpositiveInfluenceDistanceOffsetUpperBound) {
    DRAKE_EXPECT_THROWS_MESSAGE(
            MinimumDistanceUpperBoundConstraint(plant_double_, 0.1, plant_context_double_, -0.05, {}),
    ...
    // distance. We'll assume the message is the same for an equal distance and
    // just confirm the throw.
    DRAKE_EXPECT_THROWS_MESSAGE(MinimumDistanceLowerBoundConstraint(plant_double_, 0.1, plant_context_double_, {},
                                                                    std::numeric_limits<double>::infinity()),
                                "MinimumDistanceLowerBoundConstraint: "
                                "influence_distance must be finite.");
    EXPECT_THROW(MinimumDistanceLowerBoundConstraint(plant_double_, 0.1, plant_context_double_, {}, 0.0),
                 std::exception);
    DRAKE_EXPECT_THROWS_MESSAGE(MinimumDistanceUpperBoundConstraint(plant_double_, 0.1, plant_context_double_,
                                                                    std::numeric_limits<double>::infinity(), {}),
                                "MinimumDistanceUpperBoundConstraint: "
                                "influence_distance must be finite.");
    EXPECT_THROW(MinimumDistanceUpperBoundConstraint(plant_double_, 0.1, plant_context_double_, 0.0, {}),
                 std::exception);
}
...
TEST_F(BoxSphereTest, Test) {
    ...
    for (solvers::MinimumValuePenaltyFunction penalty_function :
        ...
        MinimumDistanceLowerBoundConstraint lower_bound_constraint(plant_double_, minimum_distance_lower,
                                                                   plant_context_double_, penalty_function,
                                                                   influence_distance_offset1);
        MinimumDistanceUpperBoundConstraint upper_bound_constraint(plant_double_, minimum_distance_upper,
                                                                   plant_context_double_, influence_distance_offset2,
                                                                   penalty_function);

        auto check_eval_autodiff = [&lower_bound_constraint, &upper_bound_constraint](
                                           const Eigen::VectorXd& q_val, const Eigen::MatrixXd& q_gradient, double tol,
        ...
        q.tail<3>() << 0, 0, 0;
        check_eval_autodiff(q, Eigen::MatrixXd::Identity(kNumPositionsForTwoFreeBodies, kNumPositionsForTwoFreeBodies),
        ...
        const MinimumDistanceLowerBoundConstraint lower_bound_constraint_from_autodiff(
                plant_autodiff_, minimum_distance_lower, plant_context_autodiff_, penalty_function,
                influence_distance_offset1);
        const MinimumDistanceUpperBoundConstraint upper_bound_constraint_from_autodiff(
                plant_autodiff_, minimum_distance_upper, plant_context_autodiff_, influence_distance_offset2,
                penalty_function);
        ...
        TestKinematicConstraintEval(lower_bound_constraint, lower_bound_constraint_from_autodiff, q, dq, gradient_tol);
        TestKinematicConstraintEval(upper_bound_constraint, upper_bound_constraint_from_autodiff, q, dq, gradient_tol,
                                    1E-11 /* tol */);
    }
}
...
GTEST_TEST(ThreeSpheresTest, SomeLargerThanInfluenceSomeSmallerThanLowerBound) {
    ...
    Eigen::VectorXd q = three_spheres.plant().GetPositions(three_spheres.plant_context());
    ...
    ASSERT_LT((q.segment<3>(kSpheres[0]) - q.segment<3>(kSpheres[1])).norm() - 2 * three_spheres.radius(),
              minimum_distance_lower);
    // Make sure distance(sphere0, sphere2) > d_influence.
    ASSERT_GT((q.segment<3>(kSpheres[2]) - q.segment<3>(kSpheres[0])).norm() - 2 * three_spheres.radius(), d_influence);
    for (solvers::MinimumValuePenaltyFunction penalty_function :
         {QuadraticallySmoothedHingeLoss, ExponentiallySmoothedHingeLoss}) {
        MinimumDistanceLowerBoundConstraint dut(&(three_spheres.plant()), minimum_distance_lower,
...
GTEST_TEST(ThreeSpheresTest, SomeLargerThanInfluenceSomeSmallerThanUpperBound) {
    ...
    for (solvers::MinimumValuePenaltyFunction penalty_function :
         {QuadraticallySmoothedHingeLoss, ExponentiallySmoothedHingeLoss}) {
        MinimumDistanceUpperBoundConstraint dut(&(three_spheres.plant()), minimum_distance_upper,
...
TEST_F(SpheresAndWallsTest, TestWithCollisionCheckerLowerUpperBounds) {
    ...
    params.configuration_distance_function = [quaternion_difference](const Eigen::VectorXd& q1,
                                                                     const Eigen::VectorXd& q2) {
        return quaternion_difference(q1.head<4>(), q2.head<4>()) +
               quaternion_difference(q1.segment<4>(7), q2.segment<4>(7)) +
               (q1.segment<3>(4) - q2.segment<3>(4)).norm() + (q1.segment<3>(11) - q2.segment<3>(11)).norm();
    };
    params.edge_step_size = 0.05;
    planning::SceneGraphCollisionChecker collision_checker(std::move(params));
    auto collision_checker_context = collision_checker.MakeStandaloneModelContext();
    ...
    MinimumDistanceLowerBoundConstraint dut_lower_bound(
            &collision_checker, minimum_distance_lower, collision_checker_context.get(),
            solvers::QuadraticallySmoothedHingeLoss, influence_distance - minimum_distance_lower);
    MinimumDistanceUpperBoundConstraint dut_upper_bound(
            &collision_checker, minimum_distance_upper, collision_checker_context.get(),
            influence_distance - minimum_distance_upper, solvers::QuadraticallySmoothedHingeLoss);
    EXPECT_EQ(dut_lower_bound.num_constraints(), 1);
    EXPECT_EQ(dut_lower_bound.num_vars(), collision_checker.plant().num_positions());
    EXPECT_EQ(dut_upper_bound.num_constraints(), 1);
    EXPECT_EQ(dut_upper_bound.num_vars(), collision_checker.plant().num_positions());

    auto check_constraint = [&collision_checker, &dut_lower_bound, &dut_upper_bound](
                                    const Eigen::Vector3d& p_WS1, const Eigen::Vector3d& p_WS2, bool is_lower_satisfied,
                                    bool is_upper_satisfied) {
        ...
        auto check_eval = [&q, &q_ad](const solvers::Constraint& dut) {
            ...
            math::NumericalGradientOption options(math::NumericalGradientMethod::kCentral);
            const Eigen::MatrixXd grad_numeric =
                    math::ComputeNumericalGradient<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd>(
                            [&dut](const Eigen::VectorXd& x, Eigen::VectorXd* y) {
                                dut.Eval(x, y);
                            },
                            q, options);
            EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_ad), grad_numeric, 1E-7));
        };
    ...
    check_constraint(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(1E-4, 0, 0), false, true);

    // The two spheres are separated, but the distance is smaller than
    // minimum_distance_lower
    check_constraint(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(0.9 * minimum_distance_lower + 2 * radius_, 0, 0), false,
                     true);
    // The distance between the two spheres is between minimum_distance_lower
    // and minimum_distance_upper.
    check_constraint(Eigen::Vector3d(0, 0, 0),
                     Eigen::Vector3d((minimum_distance_lower + minimum_distance_upper) / 2 + 2 * radius_, 0, 0), true,
                     true);
    // The distance between the two spheres is above minimum_distance_upper.
    check_constraint(Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(minimum_distance_upper + 2 * radius_ + 1E-3, 0, 0), true,
                     false);
    ...
    check_constraint(
            ...
            Eigen::Vector3d(wall_length_ / 2 - (minimum_distance_lower + minimum_distance_upper) / 2 - radius_, 0, 0),
            true, true);
}
}  // namespace multibody
}  // namespace drake
