...
void KukaTest::CheckGlobalIKSolution(const solvers::MathematicalProgramResult& result,
                                     double pos_tol,
                                     double orient_tol) const {
    Eigen::VectorXd q_global_ik = global_ik_.ReconstructGeneralizedPositionSolution(result);
    ...
    for (BodyIndex i{1}; i < plant_->num_bodies(); ++i) {
        ...
        const auto& body_pose_fk = plant_->CalcRelativeTransform(*context, plant_->world_frame(),
                                                                 plant_->get_body(BodyIndex{i}).body_frame());

        const Eigen::Matrix3d body_Ri = result.GetSolution(global_ik_.body_rotation_matrix(i));
        // Tolerance from Gurobi is about 1E-6. I increase it to 3e-6 to pass on Mac
        // CI.
        const double tol = 3e-6;
        EXPECT_TRUE((body_Ri.array().abs() <= 1 + tol).all()) << fmt::format("body_Ri:\n{}\n", fmt_eigen(body_Ri));
        EXPECT_LE(body_Ri.trace(), 3 + tol);
        EXPECT_GE(body_Ri.trace(), -1 - tol);
        Vector3d body_pos_global_ik = result.GetSolution(global_ik_.body_position(i));
        EXPECT_TRUE(
                CompareMatrices(body_pose_fk.translation(), body_pos_global_ik, pos_tol, MatrixCompareType::absolute));
        EXPECT_TRUE(
                CompareMatrices(body_pose_fk.rotation().matrix(), body_Ri, orient_tol, MatrixCompareType::absolute));
    }
}

Eigen::VectorXd KukaTest::CheckNonlinearIK(const Eigen::Vector3d& ee_pos_lb_W,
    ...
    ik.AddPositionConstraint(plant_->get_body(BodyIndex{ee_idx_}).body_frame(), Eigen::Vector3d::Zero(),
                             plant_->world_frame(), ee_pos_lb_W, ee_pos_ub_W);
    ik.AddOrientationConstraint(plant_->get_body(BodyIndex{ee_idx_}).body_frame(),
    ...
    ik.get_mutable_prog()->AddQuadraticErrorCost(
            Eigen::MatrixXd::Identity(plant_->num_positions(), plant_->num_positions()), q_nom_ik, ik.q());
    ...
    for (const auto& q_guess : q_guesses) {
        ...
        if (!result.is_success()) {
            ...
            drake::log()->info("Nonlinear IK use solver {}", result.get_solver_id().name());
            drake::log()->info("Solution result is {}", result.get_solution_result());
            if (result.get_solver_id() == solvers::SnoptSolver::id()) {
                drake::log()->info("SNOPT info {}", result.get_solver_details<solvers::SnoptSolver>().info);
            }
        } else {
            found_solution = true;
            break;
...
ToyTest::ToyTest() : plant_{std::make_unique<MultibodyPlant<double>>(0.0)} {
    const SpatialInertia<double> spatial_inertia(1, Eigen::Vector3d::Zero(), UnitInertia<double>(0.01, 0.01, 0.01));
    body_indices_.push_back(plant_->AddRigidBody("body0", spatial_inertia).index());
    // weld body0.
    plant_->AddJoint<WeldJoint>(
            "joint0", plant_->get_body(world_index()),
            math::RigidTransform<double>(math::RollPitchYaw<double>(0.2, 0.1, 0.3), Eigen::Vector3d(0.2, -0.1, 0.1)),
            plant_->get_body(body_indices_[0]),
            math::RigidTransform<double>(math::RollPitchYaw<double>(0.3, 0.1, -0.5), Eigen::Vector3d(0.3, 0.2, 0.5)),
            math::RigidTransform<double>(math::RollPitchYaw(0.4, -0.3, 0.2), Eigen::Vector3d(0.5, 0.3, -0.2)));
    ...
    plant_->AddJoint<RevoluteJoint>(
            ...
            math::RigidTransform<double>(math::RollPitchYawd(0.1, 0.5, 0.3), Eigen::Vector3d(0.2, 0.1, 0.4)), body1,
            math::RigidTransformd(math::RollPitchYawd(0.5, -0.2, 0.1), Eigen::Vector3d(0.1, 0.2, 0.3)),
            Eigen::Vector3d::UnitX());
    plant_->AddJoint<RevoluteJoint>(
            "joint2", body1,
            math::RigidTransform<double>(math::RollPitchYawd(0.3, -0.5, 0.3), Eigen::Vector3d(-0.2, 0.2, 0.4)), body2,
            math::RigidTransformd(math::RollPitchYawd(0.2, -0.5, 0.1), Eigen::Vector3d(0.1, 0.2, 0.1)),
