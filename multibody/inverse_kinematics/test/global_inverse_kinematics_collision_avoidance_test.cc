...
#include "multibody/inverse_kinematics/test/global_inverse_kinematics_test_util.h"
...
void CheckPtCollisionFreeBinary(const Eigen::Ref<const Eigen::VectorXd>& collision_avoidance_binary_val) {
...
struct Box {
    Box(const Eigen::Vector3d& m_center, const Eigen::Vector3d& m_size) : center{m_center}, size{m_size} {}
...
GTEST_TEST(GlobalInverseKinematicsTest, BodySphereInOneOfPolytopesTest) {
    ...
    boxes.emplace_back(Eigen::Vector3d(0.2, 0.4, 1.2), Eigen::Vector3d(1.1, 1.2, 1.3));
    // This box is not large enough to contain the sphere.
    boxes.emplace_back(Eigen::Vector3d(0.1, -0.5, 1.3), Eigen::Vector3d(0.9, 1.5, 1.4));
    ...
    auto z = global_ik.BodySphereInOneOfPolytopes(link_idx, p_BQ, radius, polytopes);
    ...
    const auto R_WB_sol = result.GetSolution(global_ik.body_rotation_matrix(link_idx));

    const Eigen::Vector3d p_WQ_sol = p_WB_sol + R_WB_sol * p_BQ;
    const double tol = 1E-6;
    for (int i = 0; i < 3; ++i) {
        EXPECT_LE(p_WQ_sol(i) + radius, boxes[0].center(i) + boxes[0].size(i) / 2 + tol);
        EXPECT_GE(p_WQ_sol(i) - radius, boxes[0].center(i) - boxes[0].size(i) / 2 - tol);
    }
    EXPECT_TRUE(CompareMatrices(result.GetSolution(z), Eigen::Vector2d(1, 0), tol));
}
...
TEST_F(KukaTest, CollisionAvoidanceTest) {
    ...
    region_vertices[0].row(2) << Eigen::RowVector4d::Constant(box_pos(2) + box_size(2) / 2 + margin),
    ...
    region_vertices[1].row(2) << Eigen::RowVector4d::Constant(box_pos(2) - box_size(2) / 2 - margin),
    ...
    region_vertices[2].row(0) << Eigen::RowVector4d::Constant(box_pos(0) - box_size(0) / 2 - margin - 0.4),
    ...
    region_vertices[3].row(0) << Eigen::RowVector4d::Constant(box_pos(0) + box_size(0) / 2 + margin),
    ...
    region_vertices[4].row(1) << Eigen::RowVector4d::Constant(box_pos(1) + box_size(1) / 2 + margin),
    ...
    region_vertices[5].row(1) << Eigen::RowVector4d::Constant(box_pos(1) - box_size(1) / 2 - margin),
            Eigen::RowVector4d::Constant(-1);
    region_vertices[5].row(2) = region_vertices[4].row(2);

    Eigen::Vector3d ee_pos(-0.7, 0, 0.45);
    global_ik_.AddWorldPositionConstraint(ee_idx_, Vector3d::Zero(), ee_pos, ee_pos);

    // First run the global IK without collision avoidance.
    solvers::GurobiSolver gurobi_solver;
    global_ik_.get_mutable_prog()->SetSolverOption(solvers::GurobiSolver::id(), "OutputFlag", 1);
    solvers::MathematicalProgramResult result = gurobi_solver.Solve(global_ik_.prog(), {}, {});
    EXPECT_TRUE(result.is_success());
    const auto& q_without_collision_avoidance = global_ik_.ReconstructGeneralizedPositionSolution(result);
    auto context = plant_->CreateDefaultContext();
    plant_->SetPositions(context.get(), q_without_collision_avoidance);
    const auto ee_pose_ik_without_collision_avoidance = plant_->CalcRelativeTransform(
            *context, plant_->world_frame(), plant_->get_body(BodyIndex{ee_idx_}).body_frame());
    EXPECT_LE((ee_pose_ik_without_collision_avoidance.translation() - ee_pos).norm(), 0.11);
    ...
    std::vector<solvers::VectorXDecisionVariable> link5_collision_avoidance_binary(link5_pts.size());
    std::vector<solvers::VectorXDecisionVariable> link6_collision_avoidance_binary(link6_pts.size());
    for (int i = 0; i < static_cast<int>(link5_pts.size()); ++i) {
        link5_collision_avoidance_binary[i] =
                global_ik_.BodyPointInOneOfRegions(link5_idx, link5_pts[i], region_vertices);
    }
    for (int i = 0; i < static_cast<int>(link6_pts.size()); ++i) {
        link6_collision_avoidance_binary[i] =
                global_ik_.BodyPointInOneOfRegions(link6_idx, link6_pts[i], region_vertices);
    }

    result = gurobi_solver.Solve(global_ik_.prog(), {}, {});
    EXPECT_TRUE(result.is_success());
    const auto& q_with_collision_avoidance = global_ik_.ReconstructGeneralizedPositionSolution(result);
    plant_->SetPositions(context.get(), q_with_collision_avoidance);
    const auto ee_pose_ik_with_collision_avoidance = plant_->CalcRelativeTransform(
            *context, plant_->world_frame(), plant_->get_body(BodyIndex{ee_idx_}).body_frame());
    // TODO(eric.cousineau): Revisit loose tolerance (PR #6162).
    EXPECT_LE((ee_pose_ik_with_collision_avoidance.translation() - ee_pos).norm(), 0.1);

    // Now check to make sure the points are collision free.
    const auto& link5_pose_ik_with_collision_avoidance = plant_->CalcRelativeTransform(
            *context, plant_->world_frame(), plant_->get_body(BodyIndex{link5_idx}).body_frame());
    const auto& link6_pose_ik_with_collision_avoidance = plant_->CalcRelativeTransform(
            *context, plant_->world_frame(), plant_->get_body(BodyIndex{link6_idx}).body_frame());
    for (int i = 0; i < static_cast<int>(link5_pts.size()); ++i) {
        CheckPtCollisionFree(link5_pose_ik_with_collision_avoidance.rotation().matrix() * link5_pts[i] +
                                     link5_pose_ik_with_collision_avoidance.translation(),
                             box_pos, box_size);
        const auto& link5_collision_avoidance_binary_val = result.GetSolution(link5_collision_avoidance_binary[i]);
        CheckPtCollisionFreeBinary(link5_collision_avoidance_binary_val);
    }
    for (int i = 0; i < static_cast<int>(link6_pts.size()); ++i) {
        CheckPtCollisionFree(link6_pose_ik_with_collision_avoidance.rotation().matrix() * link6_pts[i] +
                                     link6_pose_ik_with_collision_avoidance.translation(),
                             box_pos, box_size);
        const auto& link6_collision_avoidance_binary_val = result.GetSolution(link6_collision_avoidance_binary[i]);
