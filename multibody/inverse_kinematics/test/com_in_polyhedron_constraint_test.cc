#include "multibody/inverse_kinematics/com_in_polyhedron_constraint.h"
...
namespace multibody {
namespace {
AutoDiffVecXd EvalComInPolyhedronConstraintAutoDiff(const Context<AutoDiffXd>& context,
        ...
        for (auto body_index : plant.GetBodyIndices(model_instance)) {
            if (body_index != 0) {
                const math::RigidTransform<AutoDiffXd> X_EB =
                        plant.CalcRelativeTransform(context, expressed_frame, plant.get_body(body_index).body_frame());
                const Vector3<AutoDiffXd> p_BBcm = plant.get_body(body_index).CalcCenterOfMassInBodyFrame(context);
                const Vector3<AutoDiffXd> p_EBcm = X_EB * p_BBcm;
                const AutoDiffXd body_mass = plant.get_body(body_index).get_mass(context);
...
template <typename T>
void TestComInPolyhedronConstraint(const MultibodyPlant<T>* const plant,
    ...
    ComInPolyhedronConstraint constraint(plant, model_instances, plant->get_frame(expressed_frame_index), A, lb, ub,
                                         plant_context);
    ...
    VectorX<AutoDiffXd> y_autodiff_expected =
            EvalComInPolyhedronConstraintAutoDiff(*plant_context_autodiff, *plant_autodiff, model_instances_val,
...
TEST_F(IiwaKinematicConstraintTest, ComInPolyhedronConstraintEmptyModelInstance) {
    ...
    TestComInPolyhedronConstraint(plant_, plant_autodiff_.get(), std::nullopt, link2_frame.index(), plant_context_,
                                  plant_context_autodiff_.get(), A, lb, ub, q);
    // Test constraint constructed with MBP<AutoDiffXd>
    TestComInPolyhedronConstraint(plant_autodiff_.get(), plant_autodiff_.get(), std::nullopt, link2_frame.index(),
                                  plant_context_autodiff_.get(), plant_context_autodiff_.get(), A, lb, ub, q);
}
...
GTEST_TEST(DualIiwaTest, ComInPolyhedronConstraintModelInstance) {
    ...
    TestComInPolyhedronConstraint(plant_autodiff.get(), plant_autodiff.get(), model_instances,
                                  plant_autodiff->world_frame().index(), plant_context_autodiff.get(),
                                  plant_context_autodiff.get(), A, lb, ub, q);
    TestComInPolyhedronConstraint(plant.get(), plant_autodiff.get(), model_instances, plant->world_frame().index(),
                                  plant_context.get(), plant_context_autodiff.get(), A, lb, ub, q);
    // Test model_instances being an empty vector.
    model_instances.clear();
    DRAKE_EXPECT_THROWS_MESSAGE(
            ComInPolyhedronConstraint(plant_autodiff.get(), model_instances, plant_autodiff->world_frame(), A, lb, ub,
