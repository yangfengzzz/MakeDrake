...
TEST_F(KukaTest, ReachableTest) {
    ...
    auto ee_pos_cnstr = global_ik_.AddWorldPositionConstraint(ee_idx_, Vector3d::Zero(), ee_pos_lb_W, ee_pos_ub_W);

    Eigen::Quaterniond ee_desired_orient(Eigen::AngleAxisd(-M_PI / 2, Vector3d(0, 1, 0)));
    double angle_tol = 0.2 * M_PI;
    auto ee_orient_cnstr = global_ik_.AddWorldOrientationConstraint(ee_idx_, ee_desired_orient, angle_tol);
    ...
    if (gurobi_solver.available()) {
        ...
        Eigen::Matrix<double, 7, 1> q_global_ik = global_ik_.ReconstructGeneralizedPositionSolution(result);
        ...
        ee_orient_cnstr.evaluator()->UpdateLowerBound(Vector1d(2 * cos(angle_tol) + 1));
        result = gurobi_solver.Solve(global_ik_.prog());
        EXPECT_TRUE(result.is_success());
        const auto& ee_pos_sol = result.GetSolution(global_ik_.body_position(ee_idx_));
        EXPECT_TRUE((ee_pos_sol.array() >= ee_pos_lb_W.array() - 1E-6).all());
        EXPECT_TRUE((ee_pos_sol.array() <= ee_pos_ub_W.array() + 1E-6).all());
        const auto& ee_rotmat_sol = result.GetSolution(global_ik_.body_rotation_matrix(ee_idx_));
        ...
        EXPECT_NEAR(q_global_ik(int{plant_->GetJointByName("iiwa_joint_2").index()}), 0.5, 1e-3);
        const JointIndex iiwa_joint_3_index = plant_->GetJointByName("iiwa_joint_3").index();
        ...
        global_ik_.AddJointLimitConstraint(plant_->GetBodyByName("iiwa_link_4").index(), 0.5, 0.55);
        result = gurobi_solver.Solve(global_ik_.prog());
        EXPECT_TRUE(result.get_solution_result() == solvers::SolutionResult::kInfeasibleConstraints ||
