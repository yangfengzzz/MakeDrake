...
IiwaKinematicConstraintTest::IiwaKinematicConstraintTest() {
    const std::string iiwa_url = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
    systems::DiagramBuilder<double> builder{};
    plant_ = builder.AddSystem<MultibodyPlant<double>>(0.1);
    plant_->RegisterAsSourceForSceneGraph(builder.AddSystem<SceneGraph<double>>());
    ...
    plant_autodiff_ = systems::System<double>::ToAutoDiffXd(*ConstructIiwaPlant(iiwa_url, 0.1));
...
std::unique_ptr<MultibodyPlant<double>> ConstructIiwaPlant(const std::string& url, double time_step, int num_iiwa) {
    ...
    for (int i = 0; i < num_iiwa; ++i) {
        ...
        plant->WeldFrames(plant->world_frame(), plant->GetFrameByName("iiwa_link_0", iiwa_instance));
    }
    plant->Finalize();
    return plant;
}
...
namespace {
template <typename T>
std::unique_ptr<systems::Diagram<T>> BuildTwoFreeSpheresDiagram(double radius1,
                                                                double radius2,
                                                                const RigidTransformd& X_B1S1,
                                                                const RigidTransformd& X_B2S2,
    ...
    (*plant)->RegisterCollisionGeometry(sphere1, RigidTransformd(X_B1S1), geometry::Sphere(radius1),
                                        "sphere1_collision", multibody::CoulombFriction<double>());
    (*plant)->RegisterCollisionGeometry(sphere2, RigidTransformd(X_B2S2), geometry::Sphere(radius2),
...
TwoFreeSpheresTest::TwoFreeSpheresTest() {
    ...
    diagram_double_ = BuildTwoFreeSpheresDiagram(radius1_, radius2_, X_B1S1_, X_B2S2_, &plant_double_,
    ...
    plant_context_double_ =
            &(diagram_double_->GetMutableSubsystemContext(*plant_double_, diagram_context_double_.get()));

    diagram_autodiff_ = BuildTwoFreeSpheresDiagram(radius1_, radius2_, X_B1S1_, X_B2S2_, &plant_autodiff_,
    ...
    plant_context_autodiff_ =
            &(diagram_autodiff_->GetMutableSubsystemContext(*plant_autodiff_, diagram_context_autodiff_.get()));
}
...
SpheresAndWallsTest::SpheresAndWallsTest() : builder_{} {
    ...
    // properties.
    AddContactMaterial(0.1, 250, multibody::CoulombFriction<double>{0.9, 0.5}, &proximity_properties);
    left_wall_ = builder_.plant().RegisterCollisionGeometry(
            builder_.plant().world_body(), math::RigidTransformd(Eigen::Vector3d(-(wall_length_ / 2 + 0.05), 0, 0)),
    ...
    top_wall_ = builder_.plant().RegisterCollisionGeometry(
            builder_.plant().world_body(), math::RigidTransformd(Eigen::Vector3d(0, wall_length_ / 2 + 0.05, 0)),
    ...
    const multibody::SpatialInertia<double> spatial_inertia(1, Eigen::Vector3d::Zero(),
                                                            multibody::UnitInertia<double>(0.001, 0.001, 0.001));

    for (int i = 0; i < static_cast<int>(body_indices_.size()); ++i) {
        body_indices_[i] = builder_.plant().AddRigidBody(fmt::format("body{}", i), spatial_inertia).index();
        spheres_[i] = builder_.plant().RegisterCollisionGeometry(
                builder_.plant().get_body(body_indices_[i]), math::RigidTransformd(Eigen::Vector3d::Zero()),
                geometry::Sphere(radius_), fmt::format("sphere{}", i), proximity_properties);
    }
    builder_.plant().Finalize();
}

template <typename T>
std::unique_ptr<systems::Diagram<T>> ConstructBoxSphereDiagram(const Eigen::Vector3d& box_size,
                                                               double radius,
                                                               const math::RigidTransformd& X_BGb,
                                                               MultibodyPlant<T>** plant,
                                                               geometry::SceneGraph<T>** scene_graph,
                                                               FrameIndex* sphere_frame_index,
                                                               FrameIndex* box_frame_index) {
    systems::DiagramBuilder<T> builder;
    std::tie(*plant, *scene_graph) = AddMultibodyPlantSceneGraph(&builder, 0.0);
    const auto& box = (*plant)->AddRigidBody(
            "box", SpatialInertia<double>(1, Eigen::Vector3d(0, 0, 0), UnitInertia<double>(1, 1, 1)));
    (*plant)->RegisterCollisionGeometry(box, X_BGb, geometry::Box(box_size(0), box_size(1), box_size(2)), "box",
                                        CoulombFriction<double>(0.9, 0.8));

    const auto& sphere = (*plant)->AddRigidBody(
            "sphere", SpatialInertia<double>(1, Eigen::Vector3d::Zero(), UnitInertia<double>(1, 1, 1)));
    (*plant)->RegisterCollisionGeometry(sphere, RigidTransformd::Identity(), geometry::Sphere(radius), "sphere",
                                        CoulombFriction<double>(0.9, 0.8));
...
BoxSphereTest::BoxSphereTest() : box_size_(10, 10, 10), radius_(1), X_BGb_(Eigen::Vector3d(0.1, 0, 0)) {
    diagram_double_ = ConstructBoxSphereDiagram(box_size_, radius_, X_BGb_, &plant_double_, &scene_graph_double_,
                                                &sphere_frame_index_, &box_frame_index_);
    diagram_context_double_ = diagram_double_->CreateDefaultContext();
    plant_context_double_ =
            &(diagram_double_->GetMutableSubsystemContext(*plant_double_, diagram_context_double_.get()));
    owned_diagram_autodiff_ = diagram_double_->ToAutoDiffXd();
    diagram_autodiff_ = static_cast<systems::Diagram<AutoDiffXd>*>(owned_diagram_autodiff_.get());
    ...
    scene_graph_autodiff_ = static_cast<const geometry::SceneGraph<AutoDiffXd>*>(
            &(diagram_autodiff_->GetSubsystemByName(scene_graph_double_->get_name())));
    ...
    sphere_geometry_id_ =
            plant_double_->GetCollisionGeometriesForBody(plant_double_->get_frame(sphere_frame_index_).body())[0];
    box_geometry_id_ =
            plant_double_->GetCollisionGeometriesForBody(plant_double_->get_frame(box_frame_index_).body())[0];
}
...
T BoxSphereSignedDistance(const Eigen::Ref<const Eigen::Vector3d>& box_size,
    ...
    const bool is_sphere_center_inside_box = (p_BS.array().abs() <= half_size).all();
    using std::max;
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&ConstructTwoFreeBodiesPlant<T>,
