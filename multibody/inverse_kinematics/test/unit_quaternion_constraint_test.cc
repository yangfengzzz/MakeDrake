...
GTEST_TEST(UnitQuaternionConstraintTest, Test) {
    ...
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), 2 * x_val.transpose() * x_grad));
    ...
    EXPECT_PRED2(symbolic::test::ExprEqual, y_symbolic(0), z0 * z0 + z1 * z1 + z2 * z2 + z3 * z3);
}
...
TEST_F(TwoFreeBodiesConstraintTest, AddUnitQuaternionConstraintOnPlant) {
    ...
    prog.SetInitialGuess(q.segment<4>(plant_->GetBodyByName("body1").floating_positions_start()), body1_guess);
    ...
    EXPECT_TRUE(CompareMatrices(
            prog.GetInitialGuess(q.segment<4>(plant_->GetBodyByName("body1").floating_positions_start())),
            body1_guess));
    // Confirm that body 2's default (singular) initial guess _was_ overwritten.
    EXPECT_TRUE(CompareMatrices(
            prog.GetInitialGuess(q.segment<4>(plant_->GetBodyByName("body2").floating_positions_start())),
