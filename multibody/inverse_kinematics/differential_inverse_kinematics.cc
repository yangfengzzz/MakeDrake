...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
    ...
    return DoDifferentialInverseKinematics(q_current, v_current, V_WE_E_with_flags.head(num_cart_constraints),
...
DifferentialInverseKinematicsParameters::get_linear_velocity_constraints() const {
...
void DifferentialInverseKinematicsParameters::AddLinearVelocityConstraint(
    ...
    if (constraint->num_vars() != get_num_velocities()) {
        throw std::invalid_argument(fmt::format("Number of variables, {}, does not match number of velocities, {}.",
...
Vector6<double> ComputePoseDiffInCommonFrame(const math::RigidTransform<double>& X_C0,
    ...
    // Angular.
    AngleAxis<double> rot_err = (X_C1.rotation() * X_C0.rotation().transpose()).ToAngleAxis();
    diff.head<3>() = rot_err.axis() * rot_err.angle();

    return diff;
}

DifferentialInverseKinematicsParameters::DifferentialInverseKinematicsParameters(int num_positions,
    ...
    : num_positions_(num_positions),
      ...
      joint_centering_gain_(MatrixX<double>::Zero(num_positions_, num_positions)) {
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
    ...
    const auto identity_num_positions = MatrixX<double>::Identity(num_positions, num_positions);

    solvers::MathematicalProgram prog;
    bool quadratic_cost = false;
    solvers::VectorXDecisionVariable v_next = prog.NewContinuousVariables(num_velocities, "v_next");
    solvers::VectorDecisionVariable<1> alpha = prog.NewContinuousVariables<1>("alpha");
    ...
    if (lu.rank() < num_velocities) {
        ...
        if (Nplus) {
            ...
            prog.Add2NormSquaredCost(
                    P,
                    P * parameters.get_joint_centering_gain() * (parameters.get_nominal_joint_position() - q_current),
                    v_next);
        }
        quadratic_cost = true;
    }
    ...
    if (V.size() > 0) {
        ...
        A.rightCols(1) = -V;
        prog.AddLinearEqualityConstraint(A, VectorX<double>::Zero(num_cart_constraints), {v_next, alpha});
    }
    ...
    if (parameters.get_joint_position_limits()) {
        const Eigen::VectorXd lb = (parameters.get_joint_position_limits()->first - q_current) / dt;
        const Eigen::VectorXd ub = (parameters.get_joint_position_limits()->second - q_current) / dt;
        if (N) {
            ...
            prog.AddConstraint(std::make_shared<solvers::LinearConstraint>(*N, lb, ub), v_next);
        } else if (num_positions != num_velocities) {
            throw std::runtime_error(
    ...
    if (parameters.get_joint_velocity_limits()) {
        prog.AddBoundingBoxConstraint(parameters.get_joint_velocity_limits()->first,
    ...
    if (parameters.get_joint_acceleration_limits()) {
        prog.AddLinearConstraint(identity_num_positions,
                                 ...
                                 parameters.get_joint_acceleration_limits()->second * dt + v_current, v_next);
    }

    // additional linear velocity constraints
    for (const auto& constraint : parameters.get_linear_velocity_constraints()) {
        prog.AddConstraint(solvers::Binding<solvers::LinearConstraint>(constraint, v_next));
    }
    ...
    if (!result.is_success()) {
        return {std::nullopt, DifferentialInverseKinematicsStatus::kNoSolutionFound};
    }
    ...
    if (alpha_sol < parameters.get_maximum_scaling_to_report_stuck()) {
        ...
        return {result.GetSolution(v_next), DifferentialInverseKinematicsStatus::kStuck};
    }

    return {result.GetSolution(v_next), DifferentialInverseKinematicsStatus::kSolutionFound};
}
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
    ...
    return DoDifferentialInverseKinematics(plant, context, V_WE_desired, frame_W, frame_E, parameters);
}
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
        ...
        const Frame<double>& frame_E,
        const DifferentialInverseKinematicsParameters& parameters) {
    const math::RigidTransform<double> X_AE = plant.CalcRelativeTransform(context, frame_A, frame_E);
    MatrixX<double> J_AE(6, plant.num_velocities());
    plant.CalcJacobianSpatialVelocity(context, JacobianWrtVariable::kV, frame_E, Vector3<double>::Zero(), frame_A,
                                      frame_A, &J_AE);
    ...
    return internal::DoDifferentialInverseKinematics(plant.GetPositions(context), plant.GetVelocities(context), X_AE,
                                                     J_AE, SpatialVelocity<double>(V_AE_desired), parameters, N, Nplus);
}
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
        ...
        const DifferentialInverseKinematicsParameters& parameters) {
    const Frame<double>& frame_W = plant.world_frame();
    return DoDifferentialInverseKinematics(plant, context, X_WE_desired, frame_W, frame_E, parameters);
}
...
DifferentialInverseKinematicsResult DoDifferentialInverseKinematics(
        ...
        const DifferentialInverseKinematicsParameters& parameters) {
    const math::RigidTransform<double> X_AE = plant.CalcRelativeTransform(context, frame_A, frame_E);
    Vector6<double> V_AE_desired = ComputePoseDiffInCommonFrame(X_AE, X_AE_desired) / parameters.get_time_step();
    // Saturate the velocity command at the limits:
    if (V_AE_desired.head<3>().norm() > parameters.get_end_effector_angular_speed_limit()) {
    ...
    if (parameters.get_end_effector_translational_velocity_limits()) {
        V_AE_desired.tail<3>() = V_AE_desired.tail<3>()
                                         .cwiseMax(parameters.get_end_effector_translational_velocity_limits()->first)
                                         .cwiseMin(parameters.get_end_effector_translational_velocity_limits()->second);
    }
    return DoDifferentialInverseKinematics(plant, context, V_AE_desired, frame_A, frame_E, parameters);
}
