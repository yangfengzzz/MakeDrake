...
PointToLineDistanceConstraint::PointToLineDistanceConstraint(const MultibodyPlant<double>* const plant,
    ...
    : solvers::Constraint(1,
      ...
      project_matrix_(Eigen::Matrix3d::Identity() - n_B2_normalized_ * n_B2_normalized_.transpose()),
...
PointToLineDistanceConstraint::PointToLineDistanceConstraint(const MultibodyPlant<AutoDiffXd>* plant,
...
template <typename T>
void EvalPointToLineDistanceConstraintGradient(const systems::Context<T>&,
                                               const MultibodyPlant<T>&,
                                               const Frame<T>&,
                                               const Frame<T>&,
                                               const Eigen::Vector3d&,
                                               const Vector3<T>& p_QP_B2,
...
void EvalPointToLineDistanceConstraintGradient(const systems::Context<double>& context,
                                               const MultibodyPlant<double>& plant,
                                               const Frame<double>& frame_point,
                                               const Frame<double>& frame_line,
                                               const Eigen::Vector3d& p_B1P,
                                               const Eigen::Vector3d& p_QP_B2,
                                               const Eigen::Matrix3d& project_matrix,
                                               const Eigen::Ref<const AutoDiffVecXd>& x,
                                               AutoDiffVecXd* y) {
    Eigen::MatrixXd Jq_V_B2P(3, plant.num_positions());
    plant.CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kQDot, frame_point, p_B1P, frame_line,
                                            frame_line, &Jq_V_B2P);

    *y = math::InitializeAutoDiff(Vector1d(p_QP_B2.dot(project_matrix * p_QP_B2)),
                                  2 * p_QP_B2.transpose() * project_matrix * Jq_V_B2P * math::ExtractGradient(x));
}
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    // Compute p_B2P.
    Vector3<T> p_B2P;
    plant.CalcPointsPositions(*context, frame_point, p_B1P.cast<T>(), frame_line, &p_B2P);
    const Vector3<T> p_QP_B2 = p_B2P - p_B2Q;
    EvalPointToLineDistanceConstraintGradient(*context, plant, frame_point, frame_line, p_B1P, p_QP_B2, project_matrix,
                                              x, y);
}

void PointToLineDistanceConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
        Eval(x.cast<AutoDiffXd>(), &y_t);
        *y = math::ExtractValue(y_t);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frame_point_index_, p_B1P_, frame_line_index_, p_B2Q_,
                      project_matrix_, x, y);
    }
}

void PointToLineDistanceConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, frame_point_index_, p_B1P_, frame_line_index_, p_B2Q_,
                      project_matrix_, x, y);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frame_point_index_, p_B1P_, frame_line_index_, p_B2Q_,
                      project_matrix_, x, y);
