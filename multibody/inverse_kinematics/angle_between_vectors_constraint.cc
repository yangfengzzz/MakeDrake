...
namespace drake {
namespace multibody {
AngleBetweenVectorsConstraint::AngleBetweenVectorsConstraint(const MultibodyPlant<double>* const plant,
    ...
    if (!(angle_lower >= 0 && angle_upper >= angle_lower && angle_upper <= M_PI)) {
...
AngleBetweenVectorsConstraint::AngleBetweenVectorsConstraint(const MultibodyPlant<AutoDiffXd>* const plant,
...
void EvalConstraintGradient(const systems::Context<double>& context,
    ...
    plant.CalcJacobianSpatialVelocity(context, JacobianWrtVariable::kQDot, frameB, Eigen::Vector3d::Zero() /* p_BQ */,
                                      frameA, frameA, &Jq_V_AB);
    const Eigen::Vector3d b_unit_A = R_AB * b_unit_B;
    *y = math::InitializeAutoDiff(
            a_unit_A.transpose() * b_unit_A,
            b_unit_A.cross(a_unit_A).transpose() * Jq_V_AB.topRows<3>() * math::ExtractGradient(x));
}
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    const math::RotationMatrix<T> R_AB = plant.CalcRelativeRotationMatrix(*context, frameA, frameB);
    ...
    *y = a_unit_A.transpose() * b_unit_A;
    if constexpr (!std::is_same_v<T, S>) {
        EvalConstraintGradient(*context, plant, frameA, frameB, a_unit_A, b_unit_B, R_AB, x, y);
    }
}

void AngleBetweenVectorsConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
        Eval(x.cast<AutoDiffXd>(), &y_t);
        *y = math::ExtractValue(y_t);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameA_index_, frameB_index_, a_unit_A_, b_unit_B_, x, y);
    }
}

void AngleBetweenVectorsConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, frameA_index_, frameB_index_, a_unit_A_, b_unit_B_, x, y);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, frameA_index_, frameB_index_, a_unit_A_, b_unit_B_, x, y);
    }
}
