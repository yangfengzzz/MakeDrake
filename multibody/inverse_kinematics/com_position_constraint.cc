...
namespace drake {
namespace multibody {
ComPositionConstraint::ComPositionConstraint(const MultibodyPlant<double>* const plant,
                                             std::optional<std::vector<ModelInstanceIndex>> model_instances,
                                             const Frame<double>& expressed_frame,
                                             systems::Context<double>* plant_context)
    : solvers::Constraint(
              3, RefFromPtrOrThrow(plant).num_positions() + 3, Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero()),
      plant_double_{plant},
...
ComPositionConstraint::ComPositionConstraint(const MultibodyPlant<AutoDiffXd>* const plant,
                                             std::optional<std::vector<ModelInstanceIndex>> model_instances,
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    if constexpr (std::is_same_v<T, S>) {
        ...
        *y = p_EC - x.template tail<3>();
        ...
        EvalConstraintGradient(*context, plant, model_instances, plant.get_frame(expressed_frame_index), p_EC, x, y);
    }
}

void ComPositionConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
        Eval(x.cast<AutoDiffXd>(), &y_t);
        *y = math::ExtractValue(y_t);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, model_instances_, expressed_frame_index_, x, y);
    }
}

void ComPositionConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, model_instances_, expressed_frame_index_, x, y);
    } else {
        DoEvalGeneric(*plant_double_, context_double_, model_instances_, expressed_frame_index_, x, y);
    }
}
