...
void UpdateContextPositionsAndVelocities(systems::Context<double>* context,
    ...
    return UpdateContextPositionsAndVelocities(context, plant, math::ExtractValue(q_v));
}
...
void UpdateContextPositionsAndVelocities(systems::Context<AutoDiffXd>* context,
                                         ...
                                         const Eigen::Ref<const AutoDiffVecXd>& q_v) {
    DRAKE_ASSERT(context != nullptr);
    if (!math::AreAutoDiffVecXdEqual(q_v, plant.GetPositionsAndVelocities(*context))) {
        plant.SetPositionsAndVelocities(context, q_v);
    }
}
}  // namespace internal
