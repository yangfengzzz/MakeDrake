#include "multibody/meshcat/joint_sliders.h"
...
#include "common/autodiff_overloads.h"
...
// "true".
template <typename T>
std::map<int, std::string> GetPositionNames(const MultibodyPlant<T>* plant, bool use_model_instance_name = false) {
    DRAKE_THROW_UNLESS(plant != nullptr);
    ...
    for (JointIndex i : plant->GetJointIndices()) {
        ...
        for (int j = 0; j < joint.num_positions(); ++j) {
            ...
            if (joint.num_positions() > 1) {
                description = fmt::format("{}_{}", joint.name(), joint.position_suffix(j));
            } else {
                description = joint.name();
            }
            if (use_model_instance_name) {
                description += fmt::format("/{}", plant->GetModelInstanceName(joint.model_instance()));
            }
            const bool inserted = result.insert({position_index, std::move(description)}).second;
...
VectorXd Broadcast(const char* diagnostic_name,
            ...
            overloaded{
                    ...
                    [num_positions, diagnostic_name](VectorXd&& arg) {
                        if (arg.size() != num_positions) {
                            throw std::logic_error(fmt::format("Expected {} of size {}, but got size {} instead",
...
template <typename T>
JointSliders<T>::JointSliders(std::shared_ptr<geometry::Meshcat> meshcat,
    ...
    : meshcat_(std::move(meshcat)),
      ...
      nominal_value_(std::move(initial_value).value_or(GetDefaultPositions(plant))),
      is_registered_{true} {
    ...
    if (nominal_value_.size() != nq) {
        throw std::logic_error(
                fmt::format("Expected initial_value of size {}, but got size {} instead", nq, nominal_value_.size()));
    }

    // Default any missing arguments; check (or widen) them to be of size == nq.
    const VectorXd lower_broadcast = Broadcast("lower_limit", -10.0, nq, std::move(lower_limit));
    const VectorXd upper_broadcast = Broadcast("upper_limit", 10.0, nq, std::move(upper_limit));
    const VectorXd step_broadcast = Broadcast("step", 0.01, nq, std::move(step));

    if (decrement_keycodes.size() && static_cast<int>(decrement_keycodes.size()) != nq) {
        throw std::logic_error(
    ...
    if (increment_keycodes.size() && static_cast<int>(increment_keycodes.size()) != nq) {
        throw std::logic_error(
    ...
    for (const auto& [position_index, slider_name] : position_names_) {
        ...
        const double one_min = std::max(lower_broadcast[position_index], lower_plant[position_index]);
        const double one_max = std::min(upper_broadcast[position_index], upper_plant[position_index]);
        ...
        const std::string one_decrement_keycode =
                decrement_keycodes.size() ? std::move(decrement_keycodes[position_index]) : "";
        const std::string one_increment_keycode =
                increment_keycodes.size() ? std::move(increment_keycodes[position_index]) : "";
        meshcat_->AddSlider(slider_name, one_min, one_max, one_step, one_value, one_decrement_keycode,
                            one_increment_keycode);
    }

    // Declare the output port.
    auto& output = this->DeclareVectorOutputPort("positions", nq, &JointSliders<T>::CalcOutput);
...
template <typename T>
void JointSliders<T>::CalcOutput(const Context<T>&, BasicVector<T>* output) const {
...
template <typename T>
typename systems::LeafSystem<T>::GraphvizFragment JointSliders<T>::DoGetGraphvizFragment(
...
Eigen::VectorXd JointSliders<T>::Run(const Diagram<T>& diagram,
    ...
    log()->info("Press the '{}' button in Meshcat{} to continue.", kButtonName,
                stop_button_keycode.empty() ? "" : fmt::format(" or press '{}'", stop_button_keycode));
    ...
    plant_->SetPositions(&plant_context, this->get_output_port().Eval(sliders_context));
...
void JointSliders<T>::SetPositions(const Eigen::VectorXd& q) {
    const int nq = plant_->num_positions();
    if (q.size() != nq) {
        throw std::logic_error(fmt::format("Expected q of size {}, but got size {} instead", nq, q.size()));
    }
