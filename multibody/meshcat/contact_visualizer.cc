...
#include "geometry/meshcat_graphviz.h"
#include "math/rigid_transform.h"
#include "multibody/meshcat/hydroelastic_contact_visualizer.h"
#include "multibody/meshcat/point_contact_visualizer.h"
...
template <typename T>
ContactVisualizer<T>::ContactVisualizer(std::shared_ptr<Meshcat> meshcat, ContactVisualizerParams params)
    ...
    point_visualizer_ = std::make_unique<PointContactVisualizer>(meshcat_, std::move(point_params));

    ContactVisualizerParams hydro_params = params_;
    hydro_params.prefix += "/hydroelastic";
    hydroelastic_visualizer_ = std::make_unique<HydroelasticContactVisualizer>(meshcat_, std::move(hydro_params));

    this->DeclarePeriodicPublishEvent(params_.publish_period, 0.0, &ContactVisualizer::UpdateMeshcat);
    this->DeclareForcedPublishEvent(&ContactVisualizer::UpdateMeshcat);

    if (params_.delete_on_initialization_event) {
        this->DeclareInitializationPublishEvent(&ContactVisualizer::OnInitialization);
    }

    const InputPort<T>& contact_results = this->DeclareAbstractInputPort("contact_results", Value<ContactResults<T>>());
    contact_results_input_port_ = contact_results.get_index();

    const InputPort<T>& query_object = this->DeclareAbstractInputPort("query_object", Value<QueryObject<T>>());
    query_object_input_port_ = query_object.get_index();

    const CacheEntry& geometry_names = this->DeclareCacheEntry(
            "geometry_names", ValueProducer(GeometryNames(), &ValueProducer::NoopCalc), {this->nothing_ticket()});
    geometry_names_scratch_ = geometry_names.cache_index();

    const CacheEntry& point_contacts = this->DeclareCacheEntry("point_contacts", &ContactVisualizer::CalcPointContacts,
    ...
    const CacheEntry& hydroelastic_contacts = this->DeclareCacheEntry(
            "hydroelastic_contacts", &ContactVisualizer::CalcHydroelasticContacts, {contact_results.ticket()});
...
template <typename T>
const ContactVisualizer<T>& ContactVisualizer<T>::AddToBuilder(DiagramBuilder<T>* builder,
    ...
    const auto& result =
            AddToBuilder(builder, plant.get_contact_results_output_port(), std::move(meshcat), std::move(params));

    // Add the query connection (if the plant has a SceneGraph).
    builder->ConnectToSame(plant.get_geometry_query_input_port(), result.query_object_input_port());
...
template <typename T>
const ContactVisualizer<T>& ContactVisualizer<T>::AddToBuilder(DiagramBuilder<T>* builder,
                                                               const OutputPort<T>& contact_results_port,
                                                               const OutputPort<T>& query_object_port,
                                                               std::shared_ptr<Meshcat> meshcat,
                                                               ContactVisualizerParams params) {
    DRAKE_THROW_UNLESS(builder != nullptr);
    auto& result = *builder->template AddSystem<ContactVisualizer<T>>(std::move(meshcat), std::move(params));
...
template <typename T>
EventStatus ContactVisualizer<T>::UpdateMeshcat(const Context<T>& context) const {
    double time = ExtractDoubleOrThrow(context.get_time());
    const auto& point_contacts = this->get_cache_entry(point_contacts_cache_)
                                         .template Eval<std::vector<PointContactVisualizerItem>>(context);
    point_visualizer_->Update(time, point_contacts);

    const auto& hydroelastic_contacts = this->get_cache_entry(hydroelastic_contacts_cache_)
                                                .template Eval<std::vector<HydroelasticContactVisualizerItem>>(context);
    hydroelastic_visualizer_->Update(time, hydroelastic_contacts);
    return EventStatus::Succeeded();
}

template <typename T>
const GeometryNames& ContactVisualizer<T>::GetGeometryNames(const Context<T>& context,
    ...
    GeometryNames& geometry_names = this->get_cache_entry(geometry_names_scratch_)
    ...
    if (geometry_names.entries().empty()) {
        if (query_object_input_port().HasValue(context)) {
            const QueryObject<T>& query_object = query_object_input_port().template Eval<QueryObject<T>>(context);
...
template <typename T>
void ContactVisualizer<T>::CalcHydroelasticContacts(const Context<T>& context,
                                                    std::vector<HydroelasticContactVisualizerItem>* result) const {
    result->clear();

    // Obtain the list of contacts.
    const ContactResults<T>& contact_results = contact_results_input_port().template Eval<ContactResults<T>>(context);
    ...
    for (int ci = 0; ci < contact_results.num_hydroelastic_contacts(); ++ci) {
        const HydroelasticContactInfo<T>& info = contact_results.hydroelastic_contact_info(ci);

        const geometry::ContactSurface<T>& contact_surface = info.contact_surface();

        const SortedPair<GeometryId> sorted_ids(contact_surface.id_M(), contact_surface.id_N());
        ...
        if (contact_surface.is_triangle()) {
            ...
            const std::vector<T>& field_pressures = contact_surface.tri_e_MN().values();
            const VectorX<T> pressure_T = EigenMapView(field_pressures);
            const Eigen::VectorXd pressure = ExtractDoubleOrThrow(pressure_T);
            result->emplace_back(std::move(body_A), std::move(body_B), centroid_W, force_C_W, moment_C_W, vertices,
                                 faces, pressure);
        } else {
            ...
            const std::vector<T>& field_pressures = contact_surface.poly_e_MN().values();
            const VectorX<T> pressure_T = EigenMapView(field_pressures);
            const Eigen::VectorXd pressure = ExtractDoubleOrThrow(pressure_T);

            result->emplace_back(std::move(body_A), std::move(body_B), centroid_W, force_C_W, moment_C_W,
                                 std::move(vertices), std::move(faces), std::move(pressure));
        }
    }
}

template <typename T>
void ContactVisualizer<T>::CalcPointContacts(const Context<T>& context,
    ...
    for (int i = 0; i < contact_results.num_point_pair_contacts(); ++i) {
        const PointPairContactInfo<T>& info = contact_results.point_pair_contact_info(i);
...
template <typename T>
typename systems::LeafSystem<T>::GraphvizFragment ContactVisualizer<T>::DoGetGraphvizFragment(
        const typename systems::LeafSystem<T>::GraphvizFragmentParams& params) const {
    geometry::internal::MeshcatGraphviz meshcat_graphviz(params_.prefix,
                                                         /* subscribe = */ false);
    return meshcat_graphviz.DecorateResult(
            systems::LeafSystem<T>::DoGetGraphvizFragment(meshcat_graphviz.DecorateParams(params)));
}
