...
#include "multibody/meshcat/contact_visualizer_params.h"
...
public:
    ...
    explicit ContactVisualizer(std::shared_ptr<geometry::Meshcat> meshcat, ContactVisualizerParams params = {});
    ...
    another system. */
    static const ContactVisualizer<T>& AddToBuilder(systems::DiagramBuilder<T>* builder,
...
private:
    ...
    const multibody::internal::GeometryNames& GetGeometryNames(const systems::Context<T>&,
                                                               const MultibodyPlant<T>*) const;

    /* Calc function for the cache entry of visualized point contacts. */
    void CalcPointContacts(const systems::Context<T>&, std::vector<internal::PointContactVisualizerItem>*) const;

    /* Calc function for the cache entry of visualized hydroelastic contacts. */
    void CalcHydroelasticContacts(const systems::Context<T>&,
    ...
    */
    ...
    /* This helper class is mutable because we must update it (a non-const
    operation) from a const System (e.g., during simulation). */
    std::unique_ptr<internal::HydroelasticContactVisualizer> hydroelastic_visualizer_;
...
template <>
struct Traits<drake::multibody::meshcat::ContactVisualizer> : public NonSymbolicTraits {};
}  // namespace scalar_conversion
}  // namespace systems
