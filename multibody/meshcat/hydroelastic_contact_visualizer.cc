...
HydroelasticContactVisualizer::HydroelasticContactVisualizer(std::shared_ptr<Meshcat> meshcat,
...
void HydroelasticContactVisualizer::Update(double time, const std::vector<HydroelasticContactVisualizerItem>& items) {
    ...
    for (const HydroelasticContactVisualizerItem& item : items) {
        ...
        const std::string path = fmt::format("{}/{}+{}", params_.prefix, item.body_A, item.body_B);
        ...
        status.active = (force_norm >= params_.force_threshold) || (moment_norm >= params_.moment_threshold);
        if (!status.active) {
            continue;
        }
        ...
        if (force_norm >= params_.force_threshold) {
            meshcat_->SetTransform(path + "/force_C_W",
                                   RigidTransformd(RotationMatrixd::MakeFromOneVector(item.force_C_W, 2)), time);

            // Stretch the cylinder in z.
            const double height = force_norm / params_.newtons_per_meter;
            meshcat_->SetProperty(path + "/force_C_W/cylinder", "position", {0, 0, 0.5 * height}, time);
            ...
            meshcat_->SetProperty(path + "/force_C_W/cylinder", "scale", {1, 1, height}, time);
            // Translate the arrowheads.
            const double arrowhead_height = params_.radius * 2.0;
            meshcat_->SetTransform(
                    path + "/force_C_W/head",
                    RigidTransformd(RotationMatrixd::MakeXRotation(M_PI), Vector3d{0, 0, height + arrowhead_height}),
                    time);
        ...
        if (moment_norm >= params_.moment_threshold) {
            meshcat_->SetTransform(path + "/moment_C_W",
                                   RigidTransformd(RotationMatrixd::MakeFromOneVector(item.moment_C_W, 2)), time);

            // Stretch the cylinder in z.
            const double height = moment_norm / params_.newton_meters_per_meter;
            meshcat_->SetProperty(path + "/moment_C_W/cylinder", "position", {0, 0, 0.5 * height}, time);
            // Note: Meshcat does not fully support non-uniform scaling (see #18095).
            // We get away with it here since there is no rotation on this frame and
            // no children in the kinematic tree.
            meshcat_->SetProperty(path + "/moment_C_W/cylinder", "scale", {1, 1, height}, time);
            // Translate the arrowheads.
            const double arrowhead_height = params_.radius * 2.0;
            meshcat_->SetTransform(
        ...
        // Contact surface
        {
            ...
            for (int i = 0; i < item.pressure.size(); ++i) {
                const double norm_pressure = (item.pressure[i] - min_pressure) / (max_pressure - min_pressure);
                ...
                if (norm_pressure < 0.25) {
                    ...
                    colors(2, i) = std::clamp(1.0 - (norm_pressure - 0.25) * 4.0, 0.0, 1.0);
                }
            }
            ...
            meshcat_->SetTriangleColorMesh(path + "/contact_surface", item.p_WV, item.faces, colors, false);
            meshcat_->SetTransform(path + "/contact_surface", RigidTransformd(-item.centroid_W));
        }
    }
...
HydroelasticContactVisualizer::VisibilityStatus& HydroelasticContactVisualizer::FindOrAdd(const std::string& path) {
    ...
    const MeshcatCone arrowhead(arrowhead_height, arrowhead_width, arrowhead_width);

    meshcat_->SetObject(path + "/force_C_W/cylinder", cylinder, params_.hydro_force_color);
    meshcat_->SetObject(path + "/force_C_W/head", arrowhead, params_.hydro_force_color);
    meshcat_->SetObject(path + "/moment_C_W/cylinder", cylinder, params_.hydro_moment_color);
    meshcat_->SetObject(path + "/moment_C_W/head", arrowhead, params_.hydro_moment_color);
    return iter->second;
}
