...
TEST_F(JointSlidersTest, NarrowConstructor) {
    ...
    EXPECT_EQ(dut.get_output_port().Eval(*context), Vector2d(default_angle_1, default_angle_2));
    ...
    EXPECT_EQ(dut.get_output_port().Eval(*context), Vector2d(max_angle_1, default_angle_2));
...
TEST_F(JointSlidersTest, WideConstructorWithVectors) {
    ...
    const JointSliders dut(meshcat_, &plant_, initial_value, lower_limit, upper_limit, step, decrement_keycodes,
                           increment_keycodes);
    auto context = dut.CreateDefaultContext();
...
TEST_F(JointSlidersTest, WideConstructorWithScalars) {
    ...
    EXPECT_EQ(dut.get_output_port().Eval(*context), Vector2d::Constant(max_angle));
...
TEST_F(JointSlidersTest, WrongNumPositions) {
    AddAcrobot();
    DRAKE_EXPECT_THROWS_MESSAGE(JointSliders(meshcat_, &plant_, Vector1d::Zero(), {}, {}, {}),
...
TEST_F(JointSlidersTest, Run) {
    ...
    auto* dut = builder_.AddSystem<JointSliders<double>>(meshcat_, &plant_, initial_value);
...
TEST_F(JointSlidersTest, SetPositionsWrongNumPositions) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.SetPositions(Vector1d::Zero()), "Expected q of size 2, but got size 1 instead");
}
...
TEST_F(JointSlidersTest, SetPositionsKukaIiwaRobot) {
    ...
    slider_values << meshcat_->GetSliderValue(kKukaIiwaJoint1), meshcat_->GetSliderValue(kKukaIiwaJoint2),
            meshcat_->GetSliderValue(kKukaIiwaJoint3), meshcat_->GetSliderValue(kKukaIiwaJoint4),
            meshcat_->GetSliderValue(kKukaIiwaJoint5), meshcat_->GetSliderValue(kKukaIiwaJoint6),
