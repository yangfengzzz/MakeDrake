...
protected:
    ...
    void SetUpDiagram(int add_to_builder_overload = 0,
        ...
        const std::string sphere_url = "package://drake_models/manipulation_station/sphere.sdf";
        auto sphere1_model = Parser(&plant, "sphere1").AddModelsFromUrl(sphere_url).at(0);
        const auto& sphere1 = plant.GetBodyByName("base_link", sphere1_model);
        plant.AddJoint<multibody::PrismaticJoint>("sphere1_x", plant.world_body(), std::nullopt, sphere1, std::nullopt,
                                                  Eigen::Vector3d::UnitX());
        auto sphere2_model = Parser(&plant, "sphere2").AddModelsFromUrl(sphere_url).at(0);
        const auto& sphere2 = plant.GetBodyByName("base_link", sphere2_model);
        plant.AddJoint<multibody::PrismaticJoint>("sphere2_x", plant.world_body(), std::nullopt, sphere2, std::nullopt,
                                                  Eigen::Vector3d::UnitX());
        if (add_second_geometry) {
            ...
            plant.RegisterCollisionGeometry(sphere2, X, *shape, "bonus", multibody::CoulombFriction<double>());
        }

        // Add the hydroelastic spheres and joints between them.
        const std::string hydro_url = "package://drake/multibody/meshcat/test/hydroelastic.sdf";
        ...
        switch (add_to_builder_overload) {
            case 0:
                visualizer_ = &ContactVisualizerd::AddToBuilder(&builder, plant, meshcat_, params);
                break;
            case 1:
                visualizer_ = &ContactVisualizerd::AddToBuilder(&builder, plant.get_contact_results_output_port(),
        ...
        plant.SetPositions(&plant.GetMyMutableContextFromRoot(context_.get()), Eigen::Vector4d{-0.03, 0.03, 0.1, 0.3});
    }
    ...
    void PublishAndCheck(bool expect_geometry_names = false) {
        ...
        if (visualizer_->query_object_input_port().HasValue(visualizer_->GetMyContextFromRoot(*context_))) {
            EXPECT_TRUE(meshcat_->HasPath("contact_forces/hydroelastic/body1.body1_collision+body2"));
            EXPECT_TRUE(meshcat_->HasPath("contact_forces/hydroelastic/body1.body1_collision2+body2"));
        } else {
            EXPECT_TRUE(meshcat_->HasPath(fmt::format("contact_forces/hydroelastic/body1.Id({})+body2", body1_id1)));
            EXPECT_TRUE(meshcat_->HasPath(fmt::format("contact_forces/hydroelastic/body1.Id({})+body2", body1_id2)));
        }
    }
