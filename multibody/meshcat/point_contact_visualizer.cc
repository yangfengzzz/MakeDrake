...
PointContactVisualizer::PointContactVisualizer(std::shared_ptr<Meshcat> meshcat, ContactVisualizerParams params)
...
void PointContactVisualizer::Update(double time, const std::vector<PointContactVisualizerItem>& items) {
    ...
    for (const PointContactVisualizerItem& item : items) {
        ...
        meshcat_->SetTransform(
                path, RigidTransformd(RotationMatrixd::MakeFromOneVector(item.contact_force, 2), item.contact_point),
                time);
        ...
        meshcat_->SetTransform(path + "/head", RigidTransformd(Vector3d{0, 0, -height - arrowhead_height}), time);
        meshcat_->SetTransform(
                path + "/tail",
                RigidTransformd(RotationMatrixd::MakeXRotation(M_PI), Vector3d{0, 0, height + arrowhead_height}), time);
    }
...
PointContactVisualizer::VisibilityStatus& PointContactVisualizer::FindOrAdd(const std::string& path) {
