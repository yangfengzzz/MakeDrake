...
#include "multibody/test_utilities/spatial_kinematics.h"
...
public:
    ...
    static std::unique_ptr<MultibodyPlant<T>> CreateMultibodyPlantFromTree(
        ...
        return std::unique_ptr<MultibodyPlant<T>>(new MultibodyPlant<T>(std::move(tree), time_step));
    }
};
...
public:
    ...
    explicit DrakeKukaIIwaRobot(double gravity) {
        plant_ = MultibodyPlantTester::CreateMultibodyPlantFromTree(
                MakeKukaIiwaModel<T>(false /* finalized model */, gravity /* acceleration of gravity */));
    ...
    KukaRobotJointReactionForces<T> CalcJointReactionForces(const Eigen::Ref<const VectorX<T>>& q,
        ...
        multibody::internal::PositionKinematicsCache<T> pc(tree().get_topology());
        multibody::internal::VelocityKinematicsCache<T> vc(tree().get_topology());
        multibody::internal::AccelerationKinematicsCache<T> ac(tree().get_topology());
        tree().CalcPositionKinematicsCache(*context_, &pc);
        tree().CalcVelocityKinematicsCache(*context_, pc, &vc);
        ...
        std::vector<SpatialForce<T>>& Fapplied_Bo_W_array = forces.mutable_body_forces();
        VectorX<T>& generalized_force_applied = forces.mutable_generalized_forces();

        // Calculate inverse dynamics on this robot.
        tree().CalcInverseDynamics(*context_, qDDt, Fapplied_Bo_W_array, generalized_force_applied, &A_WB_array,
    ...
    const multibody::internal::MultibodyTree<T>& tree() const { return multibody::internal::GetInternalTree(*plant_); }
