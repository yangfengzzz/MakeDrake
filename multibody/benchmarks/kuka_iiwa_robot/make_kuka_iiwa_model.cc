...
template <typename T>
const RevoluteJoint<T>& KukaIiwaModelBuilder<T>::AddRevoluteJointFromSpaceXYZAnglesAndXYZ(
        const std::string& joint_name,
        const RigidBody<T>& A,
        const Vector3<double>& q123A,
        const Vector3<double>& xyzA,
        const RigidBody<T>& B,
    ...
    return model->template AddJoint<RevoluteJoint>(joint_name, A, X_AAb, B, X_BBa, revolute_unit_vector);
}

template <typename T>
unique_ptr<multibody::internal::MultibodyTree<T>> KukaIiwaModelBuilder<T>::Build() const {
    ...
    const SpatialInertia<double> M_AAo_A =
            SpatialInertia<double>::MakeFromCentralInertia(massA_, p_AoAcm_A_, I_AAcm_A_);
    const SpatialInertia<double> M_BBo_B =
            SpatialInertia<double>::MakeFromCentralInertia(massB_, p_BoBcm_B_, I_BBcm_B_);
    const SpatialInertia<double> M_CCo_C =
            SpatialInertia<double>::MakeFromCentralInertia(massC_, p_CoCcm_C_, I_CCcm_C_);
    const SpatialInertia<double> M_DDo_D =
            SpatialInertia<double>::MakeFromCentralInertia(massD_, p_DoDcm_D_, I_DDcm_D_);
    const SpatialInertia<double> M_EEo_E =
            SpatialInertia<double>::MakeFromCentralInertia(massE_, p_EoEcm_E_, I_EEcm_E_);
    const SpatialInertia<double> M_FFo_F =
            SpatialInertia<double>::MakeFromCentralInertia(massF_, p_FoFcm_F_, I_FFcm_F_);
    const SpatialInertia<double> M_GGo_G =
            SpatialInertia<double>::MakeFromCentralInertia(massG_, p_GoGcm_G_, I_GGcm_G_);
    ...
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_1", linkN, joint_1_rpy_, joint_1_xyz_, linkA,
    ...
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_2", linkA, joint_2_rpy_, joint_2_xyz_, linkB,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_2", *joint);

    // Create a revolute joint between linkB and linkC.
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_3", linkB, joint_3_rpy_, joint_3_xyz_, linkC,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_3", *joint);

    // Create a revolute joint between linkB and linkC.
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_4", linkC, joint_4_rpy_, joint_4_xyz_, linkD,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_4", *joint);

    // Create a revolute joint between linkD and linkE.
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_5", linkD, joint_5_rpy_, joint_5_xyz_, linkE,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_5", *joint);

    // Create a revolute joint between linkE and linkF.
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_6", linkE, joint_6_rpy_, joint_6_xyz_, linkF,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_6", *joint);

    // Create a revolute joint between linkE and linkF.
    joint = &AddRevoluteJointFromSpaceXYZAnglesAndXYZ("iiwa_joint_7", linkF, joint_7_rpy_, joint_7_xyz_, linkG,
                                                      Eigen::Vector3d::UnitZ(), model.get());
    model->AddJointActuator("iiwa_actuator_7", *joint);

    // Add arbitrary tool frame.
    model->template AddFrame<FixedOffsetFrame>("tool_arbitrary", model->GetFrameByName("iiwa_link_7"),
