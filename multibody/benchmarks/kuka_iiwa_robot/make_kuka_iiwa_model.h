...
public:
    ...
    //   downwards).
    KukaIiwaModelBuilder(bool finalize_model, double gravity) : gravity_(gravity), finalize_model_(finalize_model) {}
...
private:
    ...
    static const RevoluteJoint<T>& AddRevoluteJointFromSpaceXYZAnglesAndXYZ(const std::string& joint_name,
                                                                            const RigidBody<T>& A,
...
template <typename T>
std::unique_ptr<multibody::internal::MultibodyTree<T>> MakeKukaIiwaModel(bool finalize_model = true,
                                                                         double gravity = 9.81) {
