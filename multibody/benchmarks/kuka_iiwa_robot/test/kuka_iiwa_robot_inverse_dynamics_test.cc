...
void TestKukaArmInverseDynamics(const Eigen::Ref<const VectorX<double>>& q,
    ...
    const SpatialKinematicsPVA<double> drake_kinematics = drake_kuka_robot.CalcEndEffectorKinematics(q, qDt, qDDt);

    // Kinematics: Get corresponding MotionGenesis information.
    MG::MGKukaIIwaRobot<double> MG_kuka_robot(gravity);
    const SpatialKinematicsPVA<double> MG_kinematics = MG_kuka_robot.CalcEndEffectorKinematics(q, qDt, qDDt);
    ...
    EXPECT_TRUE(
            drake_kinematics.rotation_matrix().matrix().isApprox(MG_kinematics.rotation_matrix().matrix(), kEpsilon));
    EXPECT_TRUE(drake_kinematics.position_vector().isApprox(MG_kinematics.position_vector(), kEpsilon));
    EXPECT_TRUE(drake_kinematics.angular_velocity().isApprox(MG_kinematics.angular_velocity(), kEpsilon));
    EXPECT_TRUE(drake_kinematics.translational_velocity().isApprox(MG_kinematics.translational_velocity(), kEpsilon));
    EXPECT_TRUE(drake_kinematics.angular_acceleration().isApprox(MG_kinematics.angular_acceleration(), kEpsilon));
    EXPECT_TRUE(drake_kinematics.translational_acceleration().isApprox(MG_kinematics.translational_acceleration(),
                                                                       kEpsilon));
    ...
    const KukaRobotJointReactionForces<double> forces = drake_kuka_robot.CalcJointReactionForces(q, qDt, qDDt);
