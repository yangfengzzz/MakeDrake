...
void CompareEndEffectorPositionVelocityVsExpectedSolution(const Vector7d& q,
                                                          const Vector7d& qDt,
                                                          const Vector7d& qDDt,
    ...
    EXPECT_TRUE(kinematics.rotation_matrix().matrix().isApprox(R_NG_expected, kEpsilon));
    EXPECT_TRUE(kinematics.position_vector().isApprox(p_No_Go_N_expected, kEpsilon));
    EXPECT_TRUE(kinematics.angular_velocity().isApprox(w_NG_N_expected, kEpsilon));
    EXPECT_TRUE(kinematics.translational_velocity().isApprox(v_NGo_N_expected, kEpsilon));
    EXPECT_TRUE(kinematics.angular_acceleration().isApprox(alpha_NG_N_expected, kEpsilon));
    EXPECT_TRUE(kinematics.translational_acceleration().isApprox(a_NGo_N_expected, kEpsilon));
}
...
GTEST_TEST(KukaIIwaRobot, ForwardKinematicsA) {
    ...
    CompareEndEffectorPositionVelocityVsExpectedSolution(q, q_Dt, q_DDt, R_NG_expected, p_No_Go_N_expected,
                                                         w_NG_N_expected, v_NGo_N_expected, alpha_NG_N_expected,
                                                         a_NGo_N_expected);
}
...
GTEST_TEST(KukaIIwaRobot, ForwardKinematicsB) {
    ...
    Eigen::Vector3d p_No_Go_N_expected, w_NG_N_expected, v_NGo_N_expected;
    Eigen::Vector3d alpha_NG_N_expected, a_NGo_N_expected;
    R_NG_expected << -0.5939002959880204, 0.8043869080239565, -0.015625, -0.8043869080239565, -0.5932991120359317,
            0.03094940802395521, 0.015625, 0.03094940802395527, 0.9993988160479114;
    p_No_Go_N_expected << 0.2970356451892219, 0.1727696964662288, 1.154681973689345;
    w_NG_N_expected << 0, 0, 0;
    v_NGo_N_expected << 0, 0, 0;
    alpha_NG_N_expected << 0, 0, 0;
    a_NGo_N_expected << 0, 0, 0;
