...
template <typename T>
void MGKukaIIwaRobot<T>::PrepareMGOutput(const Eigen::Ref<const VectorX<T>>& q,
    ...
    MG_kuka_auto_generated_.SetVariablesFromArray(joint_angles_and_1st_2nd_derivatives.data());
    MG_kuka_auto_generated_.CalculateOutput();
}

template <typename T>
SpatialKinematicsPVA<T> MGKukaIIwaRobot<T>::CalcEndEffectorKinematics(const Eigen::Ref<const VectorX<T>>& q,
                                                                      const Eigen::Ref<const VectorX<T>>& qDt,
                                                                      const Eigen::Ref<const VectorX<T>>& qDDt) const {
    ...
    return SpatialKinematicsPVA<T>(R_NG, p_NoGo_N, w_NG_N, v_NGo_N, alpha_NG_N, a_NGo_N);
}

template <typename T>
std::tuple<SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced>
MGKukaIIwaRobot<T>::CalcJointReactionForcesExpressedInMobilizer(const Eigen::Ref<const VectorX<T>>& q,
                                                                const Eigen::Ref<const VectorX<T>>& qDt,
                                                                const Eigen::Ref<const VectorX<T>>& qDDt) const {
    ...
    const SpatialForced F_A_Na(Vector3d(MG_kuka_auto_generated_.tA), Vector3d(MG_kuka_auto_generated_.fA));
    const SpatialForced F_B_Ab(Vector3d(MG_kuka_auto_generated_.tB), Vector3d(MG_kuka_auto_generated_.fB));
    const SpatialForced F_C_Bc(Vector3d(MG_kuka_auto_generated_.tC), Vector3d(MG_kuka_auto_generated_.fC));
    const SpatialForced F_D_Cd(Vector3d(MG_kuka_auto_generated_.tD), Vector3d(MG_kuka_auto_generated_.fD));
    const SpatialForced F_E_De(Vector3d(MG_kuka_auto_generated_.tE), Vector3d(MG_kuka_auto_generated_.fE));
    const SpatialForced F_F_Ef(Vector3d(MG_kuka_auto_generated_.tF), Vector3d(MG_kuka_auto_generated_.fF));
    const SpatialForced F_G_Fg(Vector3d(MG_kuka_auto_generated_.tG), Vector3d(MG_kuka_auto_generated_.fG));

    return std::make_tuple(F_A_Na, F_B_Ab, F_C_Bc, F_D_Cd, F_E_De, F_F_Ef, F_G_Fg);
}

template <typename T>
std::tuple<SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced, SpatialForced>
MGKukaIIwaRobot<T>::CalcJointReactionForcesExpressedInWorld(const Eigen::Ref<const VectorX<T>>& q,
                                                            const Eigen::Ref<const VectorX<T>>& qDt,
                                                            const Eigen::Ref<const VectorX<T>>& qDDt) const {
    // Calculate joint reaction torque/forces with MotionGenesis.
    PrepareMGOutput(q, qDt, qDDt);
...
template <typename T>
Vector7d MGKukaIIwaRobot<T>::CalcRevoluteMotorZTorques(const Eigen::Ref<const VectorX<T>>& q,
                                                       const Eigen::Ref<const VectorX<T>>& qDt,
                                                       const Eigen::Ref<const VectorX<T>>& qDDt) const {
