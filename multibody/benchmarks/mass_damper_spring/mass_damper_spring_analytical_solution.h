...
public:
    ...
    MassDamperSpringAnalyticalSolution(const T& mass, const T& b, const T& k) : m_(mass), b_(b), k_(k) {}
...
private:
    ...
    //
    // @returns Three-element matrix consisting of x, ẋ, ẍ, respectively.
    static Vector3<T> CalculateOutputImpl(const T& zeta, const T& wn, const T& x0, const T& xDt0, const T& t);
};
