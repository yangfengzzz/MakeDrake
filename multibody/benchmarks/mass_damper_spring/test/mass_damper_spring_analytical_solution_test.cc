...
void CompareMassDamperSpringSolutionVsExpectedSolution(const double m,
                                                       const double b,
                                                       const double k,
                                                       const double x0,
                                                       const double xDt0,
...
GTEST_TEST(MassDamperSpringUndamped, UndampedVibrationA) {
    ...
    x_xDt_xDtDt << x0 * std::cos(wn * t), -x0 * std::sin(wn * t) * wn, -x0 * std::cos(wn * t) * wn * wn;

    CompareMassDamperSpringSolutionVsExpectedSolution(m, b, k, x0, xDt0, t, x_xDt_xDtDt, 4 * kEpsilon);
}
...
GTEST_TEST(MassDamperSpringUndamped, UnderdampedVibrationB) {
    ...
    Eigen::Vector3d x_xDt_xDtDt;
    x_xDt_xDtDt << x0 * std::exp(-zeta * wn * t), 0, -x0 * wn * wn * std::exp(-zeta * wn * t);
...
GTEST_TEST(MassDamperSpringUndamped, CriticallyDampedC) {
    ...
    const double wn = std::sqrt(k / m);
    Eigen::Vector3d x_xDt_xDtDt;
    x_xDt_xDtDt << xDt0 * t * std::exp(-wn * t), xDt0 * (1 - wn * t) * std::exp(-wn * t),
...
GTEST_TEST(MassDamperSpringUndamped, OverDampedD) {
    ...
    Eigen::Vector3d x_xDt_xDtDt;
    x_xDt_xDtDt << 0.2730433627750083, 0.1082082477370706, -3.43126449460871;
...
GTEST_TEST(MassDamperSpringUndamped, CriticallyDampedAlmostE) {
    ...
    for (int i = -100; i <= 100; ++i) {
        ...
        CompareMassDamperSpringSolutionVsExpectedSolution(m, b, k, x0, xDt0, t, x_xDt_xDtDt, tolerance);
    }
}
...
GTEST_TEST(MassDamperSpringUndamped, UndampedVibrationWithAutoDiffF) {
    ...
    x_xDt_xDtDt_expected << cos(wn * t), -sin(wn * t) * wn, -cos(wn * t) * wn * wn;
