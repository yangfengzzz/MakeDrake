...
std::tuple<Quaterniond, Vector3d, Vector3d> FreeBody::CalculateExactRotationalSolutionABInitiallyAligned(
...
std::tuple<Quaterniond, Vector4d, Vector3d, Vector3d> FreeBody::CalculateExactRotationalSolutionNB(
        const double t) const {
    // Kane's analytical solution is for quaternion quat_AB that relates A to B,
    ...
    std::tie(quat_AB, w_NB_B, wDt_NB_B) = CalculateExactRotationalSolutionABInitiallyAligned(t);
    ...
    const Vector4d quatDt = math::CalculateQuaternionDtFromAngularVelocityExpressedInB(quat_NB, w_NB_B);
...
std::tuple<Vector3d, Vector3d, Vector3d> FreeBody::CalculateExactTranslationalSolution(const double t) const {
