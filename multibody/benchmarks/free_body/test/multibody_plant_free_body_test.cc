...
void TestDrakeSolutionVsExactSolutionForTorqueFreeCylinder(
    ...
    const systems::VectorBase<double>& state_drake = context.get_continuous_state_vector();
    ...
    const Eigen::Quaterniond quat_NB_drake(quat4_NB_drake(0), quat4_NB_drake(1), quat4_NB_drake(2), quat4_NB_drake(3));
    ...
    std::tie(xyz_exact, xyzDt_exact, xyzDDt_exact) = torque_free_cylinder_exact.CalculateExactTranslationalSolution(t);
    ...
    const bool is_ok_quat = math::AreQuaternionsEqualForOrientation(quat_NB_exact, quat_NB_drake, tolerance);
    ...
    EXPECT_TRUE(math::IsBothQuaternionAndQuaternionDtOK(quat_NB_drake, quatDt_NB_drake, 16 * tolerance));

    EXPECT_TRUE(math::IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat_NB_exact, quatDt_NB_exact,
                                                                                  w_NB_B_exact, 16 * tolerance));

    EXPECT_TRUE(math::IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat_NB_drake, quatDt_NB_drake,
                                                                                  w_NB_B_drake, 16 * tolerance));
}
...
void IntegrateForwardWithVariableStepRungeKutta3(const FreeBody& torque_free_cylinder_exact,
                                                 const AxiallySymmetricFreeBodyPlant<double>& axisymmetric_plant,
    ...
    DRAKE_DEMAND(context != NULL && stateDt_drake != NULL && dt_max > 0.0 && t_final > 0.0);
    ...
    while (true) {
        ...
        const double tolerance = (1 + scale) * maximum_absolute_error_per_integration_step;
        TestDrakeSolutionVsExactSolutionForTorqueFreeCylinder(torque_free_cylinder_exact, axisymmetric_plant, *context,
                                                              stateDt_drake, tolerance);
...
void TestDrakeSolutionForSpecificInitialValue(const FreeBody& torque_free_cylinder_exact,
                                              const AxiallySymmetricFreeBodyPlant<double>& axisymmetric_plant,
                                              drake::systems::Context<double>* context,
                                              drake::systems::ContinuousState<double>* stateDt_drake,
    ...
    state_initial << quat_NB.w(), quat_NB.x(), quat_NB.y(), quat_NB.z(), p_NoBcm_N, w_NB_N, v_NBcm_N;
    systems::VectorBase<double>& state_drake = context->get_mutable_continuous_state_vector();
    ...
    if (should_numerically_integrate) {
        ...
        IntegrateForwardWithVariableStepRungeKutta3(torque_free_cylinder_exact, axisymmetric_plant, context,
                                                    stateDt_drake, dt_max, t_final, number_of_periods);
    }
}
...
void TestDrakeSolutionForVariousInitialValues(FreeBody* torque_free_cylinder_exact,
                                              const AxiallySymmetricFreeBodyPlant<double>& axisymmetric_plant) {
    ...
    std::unique_ptr<systems::ContinuousState<double>> stateDt = axisymmetric_plant.AllocateTimeDerivatives();
    ...
    for (double thetaX = 0; thetaX <= 2 * M_PI; thetaX += 0.1 * M_PI) {
        for (double thetaY = 0; thetaY <= 0.5 * M_PI; thetaY += 0.1 * M_PI) {
            ...
            TestDrakeSolutionForSpecificInitialValue(*torque_free_cylinder_exact, axisymmetric_plant, &context,
...
GTEST_TEST(uniformSolidCylinderTorqueFree, testNumericalIntegration) {
    ...
    FreeBody torque_free_cylinder_exact(quat_NB, w_NB_B, p_NoBcm_N, v_NBcm_B, gravity);
    ...
    AxiallySymmetricFreeBodyPlant<double> axisymmetric_plant(mass, inertia_transverse, inertia_axial, -gravity(2));

    TestDrakeSolutionForVariousInitialValues(&torque_free_cylinder_exact, axisymmetric_plant);
}
...
void TestKaneExactSolution(FreeBody torque_free_cylinder_kane, const double thetaDt, const Vector3d& lambda) {
    ...
    const bool is_ok_quat = math::AreQuaternionsEqualForOrientation(quat_kane, quat_expected, 8 * kEpsilon);
    EXPECT_TRUE(is_ok_quat);

    // Ensure Kane's quaternion time-derivative matches expected angular velocity.
    EXPECT_TRUE(math::IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat_kane, quatDt_kane, w_expected,
                                                                                  12 * kEpsilon));

    // Ensure expected quaternion time-derivative matches Kane's angular velocity.
    EXPECT_TRUE(math::IsQuaternionAndQuaternionDtEqualAngularVelocityExpressedInB(quat_expected, quatDt_expected,
                                                                                  w_kane, 8 * kEpsilon));
}
...
GTEST_TEST(uniformSolidCylinderTorqueFree, testKaneExactSolution) {
    ...
    FreeBody torque_free_cylinder_kane(quat_NB, w_NB_B, p_NoBcm_N, v_NBcm_B, gravity);
