#include "multibody/benchmarks/inclined_plane/inclined_plane_plant.h"
...
void AddInclinedPlaneAndGravityToPlant(double gravity,
    ...
    const RotationMatrix<double> R_WA = RotationMatrix<double>::MakeYRotation(inclined_plane_angle);
    ...
    if (inclined_plane_dimensions == std::nullopt) {
        ...
        plant->RegisterVisualGeometry(plant->world_body(), X_WA, geometry::HalfSpace(), "InclinedPlaneVisualGeometry",
                                      green);
        plant->RegisterCollisionGeometry(plant->world_body(), X_WA, geometry::HalfSpace(),
        ...
        const RigidTransform<double> X_WA(R_WA, p_WoAo_W);
        plant->RegisterVisualGeometry(plant->world_body(), X_WA, geometry::Box(LAx, LAy, LAz),
                                      "InclinedPlaneVisualGeometry", green);
        plant->RegisterCollisionGeometry(plant->world_body(), X_WA, geometry::Box(LAx, LAy, LAz),
                                         "InclinedPlaneCollisionGeometry", coefficient_friction_inclined_plane);
    }
...
void AddInclinedPlaneWithBlockToPlant(double gravity,
    ...
    AddInclinedPlaneAndGravityToPlant(gravity, inclined_plane_angle, inclined_plane_dimensions,
    ...
    const SpatialInertia<double> M_BBcm_B = SpatialInertia<double>::SolidBoxWithMass(massB, LBx, LBy, LBz);
    ...
    plant->RegisterVisualGeometry(blockB, X_BG, geometry::Box(LBx, LBy, LBz), "BlockB_VisualGeometry", lightBlue);
    ...
    if (is_block_with_4Spheres) {
        ...
        for (double x_sign : {-1.0, 1.0}) {
            for (double y_sign : {-1.0, 1.0}) {
                const std::string name_spherei = "Sphere" + std::to_string(++i) + "_Geometry";
                ...
                plant->RegisterCollisionGeometry(blockB, X_BSpherei, geometry::Sphere(radius), name_spherei,
                                                 coefficient_friction_bodyB);
                plant->RegisterVisualGeometry(blockB, X_BSpherei, geometry::Sphere(radius), name_spherei, red);
            }
        }
    } else {
        // Block B's collision geometry is a solid box.
        plant->RegisterCollisionGeometry(blockB, X_BG, geometry::Box(LBx, LBy, LBz), "BlockB_VisualGeometry",
                                         coefficient_friction_bodyB);
    }
}
...
void AddInclinedPlaneWithSphereToPlant(double gravity,
    ...
    // Describe body B's mass, center of mass, and inertia properties.
    const SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(massB, radiusB);
    ...
    const RigidTransform<double> X_BG;  // Identity transform.
    const Vector4<double> lightBlue(0.5, 0.8, 1.0, 1.0);
    plant->RegisterVisualGeometry(sphereB, X_BG, geometry::Sphere(radiusB), "SphereB_VisualGeometry", lightBlue);
    plant->RegisterCollisionGeometry(sphereB, X_BG, geometry::Sphere(radiusB), "SphereB_CollisionGeometry",
                                     coefficient_friction_bodyB);
    ...
    plant->RegisterVisualGeometry(sphereB, X_B_Spoke1, geometry::Sphere(radius_spoke), "Spoke1_VisualGeometry", red);
    plant->RegisterVisualGeometry(sphereB, X_B_Spoke2, geometry::Sphere(radius_spoke), "Spoke2_VisualGeometry", red);
    plant->RegisterVisualGeometry(sphereB, X_B_Spoke3, geometry::Sphere(radius_spoke), "Spoke3_VisualGeometry", red);
    plant->RegisterVisualGeometry(sphereB, X_B_Spoke4, geometry::Sphere(radius_spoke), "Spoke4_VisualGeometry", red);
}
