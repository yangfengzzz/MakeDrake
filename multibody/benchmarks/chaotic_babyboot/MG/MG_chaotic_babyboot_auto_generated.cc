...
// -----------------------------------------------------------------------------
const char* MGChaoticBabyboot::MGeqns(double t,
...
double MGIntegrator::KMIntegrator(double y[], double tStart, double* hEntry) {
    ...
    while (tStart + h != tStart && !errorMessage)  // Avoid round-off.
    {
        ...
        if (errorRatioMax >= 1) {
            ...
            snprintf(stepsizeCutMessage, 80, "Error: Stepsize has been cut too many times for variable %d.",
                     errorVarIndex);
            errorMessage = stepsizeCutMessage;
            break;
        }
        ...
        *hEntry = h;
        return errorRatioMax < 1.0 / 64.0 ? h + h : h;
    }
    ...
    if (errorMessage)
        ...
        AddErrorMessage("Error: Numerical round-off makes stepsize h too small so (tStart + h == tStart).");
...
bool MGIntegrator::IntegratorStep(double y[], double* t, double tStep, bool isFirstCall) {
    ...
    while (*t + h != *t)  // Check round-off problems.
    {
        ...
        *t += h;  // Prepare for next step.
        ...
        if (!isIntegrationFinished || fabs(hNext) < fabs(hToIntegrator) || fabs(h) < fabs(hToIntegrator))
            h = myPreviousStepsize = hNext;
...
bool MGIntegrator::IntegrateForwardOrBackward(double varArrayToIntegrate[], double& t) {
    ...
    while (!isIntegrationFinished) {
        const bool isFullStep =
                (isIntegrateForward && t + tStepMax <= tFinal) || (!isIntegrateForward && t + tStepMax >= tFinal);
