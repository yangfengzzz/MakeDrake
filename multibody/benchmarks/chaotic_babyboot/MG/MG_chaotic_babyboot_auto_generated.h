...
protected:
    ...
    virtual const char* MGeqns(double t,
                               double VAR[myNumberOfODEs],
                               double VARp[myNumberOfODEs],
                               bool isIntegratorBoundary) = 0;
...
private:
    ...
    void SetErrorMessagesToNull() {
        for (int i = 0; i < myErrorSize; i++) myErrorMessages[i] = NULL;
    }
    void AddErrorMessage(const char* msg) {
        for (int i = 0; i < myErrorSize; i++)
            if (!myErrorMessages[i]) {
                myErrorMessages[i] = msg;
                break;
            }
    }
