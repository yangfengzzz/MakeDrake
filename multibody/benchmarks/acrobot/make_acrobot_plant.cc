...
std::unique_ptr<MultibodyPlant<double>> MakeAcrobotPlant(const AcrobotParameters& params,
                                                         bool finalize,
    ...
    UnitInertia<double> G1_Bcm = UnitInertia<double>::StraightLine(params.Gc1(), Vector3d::UnitZ());
    SpatialInertia<double> M1_L1o =
            SpatialInertia<double>::MakeFromCentralInertia(params.m1(), p_L1L1cm, G1_Bcm * params.m1());

    UnitInertia<double> G2_Bcm = UnitInertia<double>::StraightLine(params.Gc2(), Vector3d::UnitZ());
    SpatialInertia<double> M2_L2o =
            SpatialInertia<double>::MakeFromCentralInertia(params.m2(), p_L2L2cm, G2_Bcm * params.m2());

    // Add a rigid body to model each link.
    const RigidBody<double>& link1 = plant->AddRigidBody(params.link1_name(), M1_L1o);
    const RigidBody<double>& link2 = plant->AddRigidBody(params.link2_name(), M2_L2o);
    ...
    if (scene_graph != nullptr) {
        ...
        plant->RegisterVisualGeometry(link1, X_L1G1, Cylinder(params.r1(), params.l1()), "visual");

        // Pose of the geometry for link 2 in the link's frame.
        const RigidTransformd X_L2G2(-params.l2() / 2.0 * Vector3d::UnitZ());
        plant->RegisterVisualGeometry(link2, X_L2G2, Cylinder(params.r2(), params.l2()), "visual");
