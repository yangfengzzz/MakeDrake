...
template <typename T>
Acrobot<T>::Acrobot(const Vector3<T>& normal,
                    const Vector3<T>& up,
                    double m1,
                    double m2,
                    double l1,
                    ...
                    double b2,
                    double g)
    : m1_(m1), m2_(m2), l1_(l1), l2_(l2), lc1_(lc1), lc2_(lc2), Ic1_(Ic1), Ic2_(Ic2), b1_(b1), b2_(b2), g_(g) {
    ...
    DRAKE_ASSERT(std::abs(std::abs(ExtractDoubleOrThrow(normal.dot(up))) - 1.0) > Eigen::NumTraits<double>::epsilon());
    ...
    const math::RotationMatrix<T> R_WD = math::RotationMatrix<T>::MakeFromOrthonormalColumns(x_W, y_W, z_W);
...
template <typename T>
Vector2<T> Acrobot<T>::CalcCoriolisVector(const T&, const T& theta2, const T& theta1dot, const T& theta2dot) const {
    using std::cos;
    using std::sin;
    ...
    Vector2<T> C;
    C << -2 * m2l1lc2 * s2 * theta2dot * theta1dot + -m2l1lc2 * s2 * theta2dot * theta2dot,
...
template <typename T>
Vector2<T> Acrobot<T>::CalcGravityVector(const T& theta1, const T& theta2) const {
    using std::cos;
    using std::sin;
...
template <typename T>
math::RigidTransform<T> Acrobot<T>::CalcLink1PoseInWorldFrame(const T& theta1) const {
    using std::cos;
    using std::sin;
    ...
    const math::RigidTransform<T> X_DL1(math::RotationMatrix<T>::MakeZRotation(theta1), xcm1_D);
...
template <typename T>
RigidTransform<T> Acrobot<T>::CalcLink2PoseInWorldFrame(const T& theta1, const T& theta2) const {
    using std::cos;
    using std::sin;

    // Center of mass position of link 2 in the model frame D.
    const Vector3<T> xcm2_D = lc2_ * Vector3<T>(sin(theta1 + theta2), -cos(theta1 + theta2), 0.0) +
                              l1_ * Vector3<T>(sin(theta1), -cos(theta1), 0.0);

    // Pose of link 2 frame measured and expressed in D.
    const RigidTransform<T> X_DL2(math::RotationMatrix<T>::MakeZRotation(theta1 + theta2), xcm2_D);
...
template <typename T>
RigidTransform<T> Acrobot<T>::CalcElbowOutboardFramePoseInWorldFrame(const T& theta1, const T& theta2) const {
...
template <typename T>
Vector6<T> Acrobot<T>::CalcLink1SpatialVelocityInWorldFrame(const T& theta1, const T& theta1dot) const {
    using std::cos;
    using std::sin;

    // Linear velocity of link1's center of mass expressed in model frame D.
    const Vector3<T> vcm1_D = lc1_ * Vector3<T>(cos(theta1), sin(theta1), 0.0) * theta1dot;
...
template <typename T>
Vector6<T> Acrobot<T>::CalcLink2SpatialVelocityInWorldFrame(const T& theta1,
                                                            const T& theta2,
                                                            const T& theta1dot,
    ...
    const Vector3<T> vcm2_D =
            lc2_ * Vector3<T>(cos(theta1 + theta2), sin(theta1 + theta2), 0.0) * (theta1dot + theta2dot) +
...
template <typename T>
Vector6<T> Acrobot<T>::CalcLink1SpatialAccelerationInWorldFrame(const T& theta1,
                                                                const T& theta1dot,
                                                                const T& theta1dotdot) const {
    using std::cos;
    using std::sin;

    // Linear acceleration of link1's center of mass expressed in model frame D.
    const Vector3<T> acm1_D = lc1_ * Vector3<T>(-sin(theta1), cos(theta1), 0.0) * theta1dot * theta1dot +
...
template <typename T>
Vector6<T> Acrobot<T>::CalcLink2SpatialAccelerationInWorldFrame(const T& theta1,
                                                                const T& theta2,
                                                                const T& theta1dot,
                                                                const T& theta2dot,
                                                                const T& theta1dotdot,
                                                                const T& theta2dotdot) const {
    using std::cos;
    using std::sin;

    // Linear acceleration of link2's center of mass expressed in model frame D.
    const Vector3<T> acm2_D =
            lc2_ * Vector3<T>(-sin(theta1 + theta2), cos(theta1 + theta2), 0.0) * (theta1dot + theta2dot) *
                    (theta1dot + theta2dot) +
            lc2_ * Vector3<T>(cos(theta1 + theta2), sin(theta1 + theta2), 0.0) * (theta1dotdot + theta2dotdot) +
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class drake::multibody::benchmarks::Acrobot);
