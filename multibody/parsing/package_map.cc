...
bool operator==(const PackageMap::RemoteParams& left, const PackageMap::RemoteParams& right) {
...
public:
    ...
    PathWithMutex(const PathWithMutex& other) : needs_fetch_(other.needs_fetch_.load()), path_(other.path_) {}
    ...
    [[nodiscard]] auto lock() const {
        return std::pair<std::lock_guard<std::mutex>, PathWithMutex*>(mutex_, const_cast<PathWithMutex*>(this));
    }
...
class PackageData {
public:
    static PackageData MakeLocal(std::string path) { return PackageData(PathWithMutex::MakeLocal(std::move(path))); }
    ...
    const PackageMap::RemoteParams& remote_params() const { return remote_params_.value(); }
    ...
    const std::optional<std::string> get_deprecation() const { return deprecation_; }
    ...
    const std::string& display_path() const {
        return remote_params_.has_value() ? remote_params_.value().urls.front() : local_path();
    }
    ...
    The `package_name` is non-functional (only used when reporting errors). */
    const std::string& GetPathWithAutomaticFetching(std::string_view package_name) const;
...
std::vector<std::string> GetAllowedUrls(const std::vector<std::string>& urls) {
    ...
    result.erase(std::remove_if(result.begin(), result.end(), should_deny), result.end());
    return result;
}

fs::path PackageData::GetCacheRelativePath() const {
    DRAKE_DEMAND(is_remote());
    const std::string hashed_strip_prefix = Sha256::Checksum(remote_params_->strip_prefix.value_or("")).to_string();
...
bool PackageData::FindInCache() {
    ...
    drake::internal::PathOrError try_cache = drake::internal::FindOrCreateCache("package_map");
...
const std::string& PackageData::GetPathWithAutomaticFetching(std::string_view package_name) const {
    ...
    if (!try_cache.error.empty()) {
        throw std::runtime_error(
                fmt::format("PackageMap: when downloading '{}', could not create temporary cache "
                            "directory: {}",
    ...
    const std::string downloader = FindResourceOrThrow("drake/multibody/parsing/package_downloader.py");
    const std::string command = fmt::format("/usr/bin/python3 {} {} {} {}", downloader, json_filename, error_filename,
    ...
    if (returncode != 0) {
        ...
        throw std::runtime_error(
                fmt::format("PackageMap: when downloading '{}', the downloader experienced an "
                            "error: {}",
                            package_name, *error));
    }
    ...
    if (!fs::is_directory(package_dir, ec)) {
        throw std::runtime_error(
                fmt::format("PackageMap: when downloading '{}', the downloader claimed success but "
...
public:
    ...
    void Remove(const std::string& package_name) {
        if (map_.erase(package_name) == 0) {
            throw std::runtime_error(
                    fmt::format("Could not find and remove package://{} from the search path.", package_name));
        }
    }

    /* The Impl flavor of the outer class's PackageMap::SetDeprecated().
    @pre The package_name exists in the map. */
    void SetDeprecated(const std::string& package_name, std::optional<std::string> new_value) {
    ...
    void Emplace(std::string package_name, PackageData data) {
        ...
        if (iter != map_.end()) {
            ...
            if (!iter->second.CanMerge(data, &error)) {
                throw std::runtime_error(fmt::format("PackageMap::Add{}() cannot add '{}' {}",
...
PackageMap::PackageMap(const PackageMap& other) : impl_{std::make_unique<Impl>(*other.impl_)} {}

PackageMap::PackageMap(PackageMap&& other) : impl_{std::exchange(other.impl_, std::make_unique<Impl>())} {}
...
PackageMap::RemoteParams GetDrakeModelsRemoteParams() {
    const std::string json_filename = FindResourceOrThrow("drake/multibody/parsing/drake_models.json");
    ...
    auto json_contents = yaml::LoadYamlFile<RepositoryMetadataSchema>(json_filename, {}, {}, options);
...
PackageMap::PackageMap() : PackageMap{std::nullopt} {
    ...
    static const never_destroyed<RemoteParams> memoized_params(internal::GetDrakeModelsRemoteParams());
    ...
    const RlocationOrError maybe_models_package_xml = FindRunfile("drake_models/package.xml");
...
void PackageMap::Add(const std::string& package_name, const std::string& package_path) {
    drake::log()->trace("PackageMap.Add('{}', '{}')", package_name, package_path);

    // Reject missing directories.
    if (!fs::is_directory(package_path)) {
        throw std::runtime_error(fmt::format("PackageMap::Add cannot add '{}' because directory '{}' does not exist",
...
bool PackageData::CanMerge(const PackageData& other, std::string* error) const {
    ...
    if (this->is_local() && other.is_local()) {
        ...
        if (!is_equivalent_canonical(this_path, other_path) &&
            !is_equivalent_canonical(this_path / "package.xml", other_path / "package.xml")) {
            *error = fmt::format("because the local paths are not equivalent ('{}' vs '{}')", this->display_path(),
                                 other.display_path());
            return false;
        }
        return true;
    ...
    if (this->is_remote() && other.is_remote()) {
        if (!(this->remote_params() == other.remote_params())) {
            *error = fmt::format("because the remote package parameters differ ({} vs {})",
    ...
    *error = fmt::format(
            ...
            this->is_local() ? "local" : "remote", this->display_path(), other.is_local() ? "local" : "remote",
            other.display_path());
    return false;
}

void PackageData::Merge(std::string_view package_name, const PackageData& other) {
    // Check whether `this` and `other` specify equivalent paths.
    std::string error;
    if (!CanMerge(other, &error)) {
        throw std::runtime_error(
                fmt::format("PackageMap::AddMap cannot merge the package definition for '{}' {}", package_name, error));
    }
...
void PackageMap::AddRemote(std::string package_name, RemoteParams params) {
    ...
    if (params.urls.empty()) {
        throw std::logic_error(fmt::format("PackageMap::AddRemote on '{}' requires at least one URL", package_name));
    }
    for (const std::string_view url : params.urls) {
        if (!((url.substr(0, 8) == "https://") || (url.substr(0, 7) == "http://") || (url.substr(0, 7) == "file://"))) {
            throw std::logic_error(
                    fmt::format("PackageMap::AddRemote on '{}' has unsupported URL '{}'", package_name, url));
        }
    }
    if (!((params.sha256.size() == 64) && (std::all_of(params.sha256.begin(), params.sha256.end(), [](char ch) {
              return std::isxdigit(ch);
          })))) {
        throw std::logic_error(
                fmt::format("PackageMap::AddRemote on '{}' has invalid sha256 '{}'", package_name, params.sha256));
    }
    if (params.archive_type.has_value()) {
        ...
        const std::initializer_list<const char*> known_types = {"zip", "tar", "gztar", "bztar", "xztar"};
        if (std::count(known_types.begin(), known_types.end(), *params.archive_type) == 0) {
            throw std::logic_error(fmt::format("PackageMap::AddRemote on '{}' has unsupported archive type '{}'",
    ...
    if (GetAllowedUrls(package_data.remote_params().urls).empty()) {
        if (!package_data.FindInCache()) {
            throw std::runtime_error(
                    fmt::format("PackageMap::AddRemote on '{}' only provides network URLs (i.e., no "
...
void PackageMap::SetDeprecated(const std::string& package_name, std::optional<std::string> deprecated_message) {
...
std::optional<std::string> PackageMap::GetDeprecated(const std::string& package_name) const {
    DRAKE_THROW_UNLESS(Contains(package_name));
...
const std::string& PackageMap::GetPath(const std::string& package_name,
    ...
    if (data.is_deprecated()) {
        warning = fmt::format("Package '{}' is deprecated: {}", package_name, *data.get_deprecation());
    }
...
void PackageMap::PopulateFromEnvironment(const std::string& environment_variable) {
...
std::tuple<std::string, std::optional<std::string>> ParsePackageManifest(const std::string& package_xml_file) {
    DRAKE_DEMAND(!package_xml_file.empty());
    XMLDocument xml_doc;
    xml_doc.LoadFile(package_xml_file.data());
    if (xml_doc.ErrorID()) {
        throw std::runtime_error(
                fmt::format("PackageMap::GetPackageName(): Failed to parse XML in file \"{}\"."
                            "\n{}",
    ...
    if (!package_node) {
        throw std::runtime_error(
                fmt::format("PackageMap::GetPackageName(): ERROR: XML file \"{}\" does not "
    ...
    if (!name_node) {
        throw std::runtime_error(
                fmt::format("PackageMap::GetPackageName(): ERROR: <package> element does not "
    ...
    if (export_node) {
        ...
        if (deprecated_node) {
            if (deprecated_node->NoChildren()) {
                ...
                deprecated_message = RemoveBreaksAndIndentation(deprecated_node->FirstChild()->Value());
            }
        }
    }
...
void PackageMap::CrawlForPackages(const std::string& path,
                                  bool stop_at_package,
                                  const std::vector<std::string_view>& stop_markers) {
    DRAKE_THROW_UNLESS(!path.empty());
    fs::path dir = fs::path(path).lexically_normal();
    if (std::any_of(stop_markers.begin(), stop_markers.end(), [dir](std::string_view name) {
    ...
    if (fs::exists(manifest)) {
        const auto [package_name, deprecated_message] = ParsePackageManifest(manifest.string());
        ...
        if (!Contains(package_name)) {
            ...
            if (!existing_data.CanMerge(local, &error)) {
                drake::log()->warn("PackageMap is ignoring newly-found path for package '{}' {}", package_name, error);
            }
        }
        if (stop_at_package) {
            return;
...
void PackageMap::AddPackageXml(const std::string& filename) {
    ...
    const auto [package_name, deprecated_message] = ParsePackageManifest(filename);
