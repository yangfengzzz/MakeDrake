...
NamedMesh DoGetObjMesh(const DiagnosticPolicy& diagnostic, std::istream* input_stream, const std::string& mtl_basedir) {
    std::optional<TriangleSurfaceMesh<double>> mesh = geometry::internal::DoReadObjToSurfaceMesh(
            input_stream, 1.0, mtl_basedir, {.diagnostic = diagnostic, .allowed_shape_count = 1});
    ...
    const bool result = tinyobj::LoadObjWithCallback(*input_stream, callback, &object_name);
    ...
    return {object_name, std::make_unique<TriangleSurfaceMesh<double>>(std::move(*mesh))};
}
...
// an in-memory source.
NamedMesh GetMeshFromFile(const DiagnosticPolicy& diagnostic, const std::string filename) {
    ...
    const std::string mtl_basedir = std::filesystem::path(filename).parent_path().string() + "/";
...
std::optional<ModelInstanceIndex> AddModelFromMesh(const DataSource& data_source,
    ...
    std::string model_instance_name = MakeModelName(candidate_name, parent_model_name, workspace);
    const ModelInstanceIndex model_instance = plant.AddModelInstance(model_instance_name);

    const SpatialInertia<double> M_BBo_B = CalcSpatialInertia(*named_mesh.mesh, 1000.0 /* kg/m3 */);
    const RigidBody<double>& body = plant.AddRigidBody(candidate_name, model_instance, M_BBo_B);
    ...
    if (plant.geometry_source_is_registered()) {
        ...
        plant.RegisterCollisionGeometry(body, X_BG, mesh, "collision", CoulombFriction<double>());
...
std::optional<ModelInstanceIndex> MeshParserWrapper::AddModel(const DataSource& data_source,
                                                              const std::string& model_name,
                                                              const std::optional<std::string>& parent_model_name,
                                                              const ParsingWorkspace& workspace) {
    return AddModelFromMesh(data_source, model_name, parent_model_name, workspace);
}

std::vector<ModelInstanceIndex> MeshParserWrapper::AddAllModels(const DataSource& data_source,
                                                                const std::optional<std::string>& parent_model_name,
                                                                const ParsingWorkspace& workspace) {
    std::optional<ModelInstanceIndex> maybe_index = AddModelFromMesh(data_source, "", parent_model_name, workspace);
