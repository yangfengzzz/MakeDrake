...
FileType DetermineFileType(const DiagnosticPolicy& policy, const std::string& filename) {
    ...
    if (EndsWithCaseInsensitive(filename, ".usda") || EndsWithCaseInsensitive(filename, ".usd")) {
        return FileType::kUsd;
    }
    policy.Error(
            fmt::format("The file '{}' is not a recognized type."
...
public:
    ...
    std::vector<ModelInstanceIndex> AddAllModels(const DataSource&,
                                                 const std::optional<std::string>&,
                                                 const ParsingWorkspace&) final {
        return {};
    }
...
ParserInterface& SelectParser(const DiagnosticPolicy& policy, const std::string& filename) {
