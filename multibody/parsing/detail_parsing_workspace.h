...
public:
    ...
    virtual std::optional<ModelInstanceIndex> AddModel(const DataSource& data_source,
    ...
    virtual std::string MergeModel(const DataSource& /* data_source */,
        ...
        throw std::runtime_error(fmt::format("MergeModel is not implemented for this input type"));
    }
    ...
    virtual std::vector<ModelInstanceIndex> AddAllModels(const DataSource& data_source,
                                                         const std::optional<std::string>& parent_model_name,
                                                         const ParsingWorkspace& workspace) = 0;
};
...
using ParserSelector =
        std::function<ParserInterface&(const drake::internal::DiagnosticPolicy& policy, const std::string& filename)>;
...
struct ParsingWorkspace {
    ...
    // this struct.
    ParsingWorkspace(const ParsingOptions& options_in,
