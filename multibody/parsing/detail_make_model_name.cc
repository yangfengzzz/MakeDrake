...
std::string MakeModelName(std::string_view candidate_name,
    ...
    std::string model_name = ScopedName::Join(parent_model_name.value_or(""), candidate_name).to_string();

    if (workspace.options.enable_auto_renaming && workspace.plant->HasModelInstanceNamed(model_name)) {
