...
#include "geometry/proximity/obb.h"
#include "geometry/proximity/obj_to_surface_mesh.h"
...
#include "multibody/parsing/detail_tinyxml.h"
#include "multibody/parsing/detail_tinyxml2_diagnostic.h"
...
void ApplyDefaultAttributes(const XMLElement& default_node, XMLElement* node) {
    for (const XMLAttribute* default_attr = default_node.FirstAttribute(); default_attr != nullptr;
...
class MujocoParser {
public:
    explicit MujocoParser(const ParsingWorkspace& workspace, const DataSource& data_source)
        : workspace_(workspace), diagnostic_(&workspace.diagnostic, &data_source), plant_(workspace.plant) {
        // Clang complains that the workspace_ field is unused. Nerf the warning
        // for now; it will be used soon.
        unused(workspace_);
    }

    RigidTransformd ParseTransform(XMLElement* node, const RigidTransformd& X_default = RigidTransformd{}) {
        ...
        const int num_orientation_attrs =
                (node->Attribute("quat") != nullptr ? 1 : 0) + (node->Attribute("axisangle") != nullptr ? 1 : 0) +
                (node->Attribute("euler") != nullptr ? 1 : 0) + (node->Attribute("xyaxes") != nullptr ? 1 : 0) +
                (node->Attribute("zaxis") != nullptr ? 1 : 0);
        if (num_orientation_attrs > 1) {
            Error(*node, fmt::format("Element {} has more than one orientation attribute specified "
...

        {
            ...
            if (ParseVectorAttribute(node, "quat", &quat)) {
                return RigidTransformd(Eigen::Quaternion<double>(quat[0], quat[1], quat[2], quat[3]), pos);
            }
        }
        ...
        if (ParseVectorAttribute(node, "axisangle", &axisangle)) {
            ...
            return RigidTransformd(Eigen::AngleAxis<double>(axisangle[3], axisangle.head<3>()), pos);
        }
        ...
        if (ParseVectorAttribute(node, "zaxis", &zaxis)) {
            ...
            // here.
            return RigidTransformd(Eigen::Quaternion<double>::FromTwoVectors(Vector3d{0, 0, 1}, zaxis), pos);
        }
    ...
    bool ParseLimits(XMLElement* node, const char* range_attr_name, const char* limited_attr_name, Vector2d* limits) {
        ...
        if (limited_attr == "true") {
            ...
            Error(*node, fmt::format("The '{}' attribute must be one of 'true', 'false', "
                                     "or 'auto'.",
                                     limited_attr_name));
        }
        return limited;
    ...
    void ParseMotor(XMLElement* node) {
        ...
        if (!ParseStringAttribute(node, "joint", &joint_name)) {
            Warning(*node, fmt::format("The motor '{}' does not use the 'joint' transmission "
        ...
        bool ctrl_limited = ParseLimits(node, "ctrlrange", "ctrllimited", &ctrl_range);
        bool force_limited = ParseLimits(node, "forcerange", "forcelimited", &force_range);

        if (ctrl_limited) {
            if (ctrl_range[0] > ctrl_range[1]) {
                Warning(*node, fmt::format("The motor '{}' specified a ctrlrange attribute where lower "
                                           "limit > upper limit; these limits will be ignored.",
                                           name));
            } else {
                effort_limit = std::max(ctrl_range[1], -ctrl_range[0]);
                if (-ctrl_range[0] != ctrl_range[1]) {
                    Warning(*node, fmt::format("The motor '{}' specified a ctrlrange attribute "
        ...
        if (force_limited) {
            ...
            if (force_range[0] > force_range[1]) {
                Warning(*node, fmt::format("The motor '{}' specified a forcerange attribute where lower "
                                           "limit > upper limit; these limits will be ignored.",
                                           name));
            } else {
                effort_limit = std::min(effort_limit, std::max(force_range[1], -force_range[0]));
                if (-force_range[0] != force_range[1]) {
                    Warning(*node, fmt::format("The motor '{}' specified a forcerange attribute "
                                               "where lower limit != -upper limit. Asymmetrical "
                                               "effort limits are not supported yet, so the "
                                               "larger of the values {} will be used.",
        ...
        plant_->AddJointActuator(name, plant_->GetJointByName(joint_name, model_instance_), effort_limit);
    }
    ...
    void ParseJoint(XMLElement* node,
        ...
        if (type == "free") {
            if (damping != 0.0) {
                Warning(*node, fmt::format("Damping was specified for the 'free' joint {}, but is not "
            ...
            JointIndex index = plant_->AddJoint<PrismaticJoint>(name, parent, X_PJ, child, X_CJ, axis,
                                                                -std::numeric_limits<double>::infinity(),
                                                                std::numeric_limits<double>::infinity(), damping)
                                       .index();
            if (limited) {
                plant_->get_mutable_joint(index).set_position_limits(Vector1d{range[0]}, Vector1d{range[1]});
            }
        } else if (type == "hinge") {
            JointIndex index = plant_->AddJoint<RevoluteJoint>(name, parent, X_PJ, child, X_CJ, axis, damping).index();
            if (limited) {
                if (angle_ == kDegree) {
                    range *= (M_PI / 180.0);
                }
    ...
    public:
        ...
        // mesh filename.
        InertiaCalculator(std::string name, std::map<std::string, SpatialInertia<double>>* mesh_inertia)
        ...
        void ImplementGeometry(const geometry::Mesh& mesh, void*) final {
            if (mesh_inertia_->contains(name_)) {
                ...
                try {  // TODO(21666), remove the try-catch.
                    ...
                    const geometry::internal::Obb obb = geometry::internal::ObbMaker(surface_mesh, v).Compute();
                    UnitInertia<double> unit_M_GBox_Box = multibody::UnitInertia<double>::SolidBox(
                            obb.half_width()[0] * 2.0, obb.half_width()[1] * 2.0, obb.half_width()[2] * 2.0);
                    UnitInertia<double> unit_M_GG_G = unit_M_GBox_Box.ReExpress(obb.pose().rotation())
                                                              .ShiftFromCenterOfMass(-obb.pose().translation());
                    M_GG_G_ = SpatialInertia<double>(obb.CalcVolume() /* since density = 1.0 */, obb.center(),
                                                     unit_M_GG_G);
                }
    ...
    SpatialInertia<double> ParseInertial(XMLElement* node) {
        ...
        if (ParseVectorAttribute(node, "diaginertia", &diag)) {
            I_BFo_B = RotationalInertia<double>(diag[0], diag[1], diag[2]).ReExpress(X_BF.rotation());
        } else {
            // fullinertia is required if diaginertia is not specified.
            Vector6d full;
            if (ParseVectorAttribute(node, "fullinertia", &full)) {
                I_BFo_B = RotationalInertia<double>(full[0], full[1], full[2], full[3], full[4], full[5]);
            } else {
                Error(*node,
        ...
        SpatialInertia<double> M_BBo_B =
                SpatialInertia<double>::MakeFromCentralInertia(mass, X_BF.translation(), I_BFo_B);
        return M_BBo_B;
    }
    ...
    MujocoGeometry ParseGeometry(XMLElement* node,
                                 int num_geom,
        ...
        if (has_fromto) {
            ...
            geom.X_BG = RigidTransformd(Eigen::Quaternion<double>::FromTwoVectors(Vector3d{0, 0, 1}, to - from),
                                        (from + to) / 2);
        } else {
            geom.X_BG = ParseTransform(node);
        }
        ...
        if (type == "plane") {
            ...
            if (has_fromto) {
                ...
                geom.shape = std::make_unique<geometry::Ellipsoid>(size[0], size[1], size[2]);
            }
            ...
            if (has_fromto) {
                ...
                geom.shape = std::make_unique<geometry::Box>(size[0] * 2.0, size[1] * 2.0, size[2] * 2.0);
            }
        } else if (type == "mesh") {
            ...
            if (mesh_.contains(mesh)) {
                ...
                Warning(*node, fmt::format("geom {} specified unknown mesh {} and will be ignored", geom.name, mesh));
                return geom;
            }
        } else if (type == "hfield") {
        ...
        if (contype == 0 && conaffinity == 0) {
            ...
            Warning(*node, fmt::format("geom {} specified contype={} and conaffinity={}; but collision "
        ...
        if (geom.register_collision && type == "sphere" && size.size() < 1) {
            Warning(*node, fmt::format("Using zero-radius spheres (MuJoCo's default geometry) for "
        ...
        if (ParseStringAttribute(node, "material", &material)) {
            if (material_.contains(material)) {
                ...
                Warning(*node, fmt::format("geom {} specified an unrecognized material {}", geom.name, material));
            }
        }
        ...
        if (!friction.empty()) {
            ...
            if ((friction.size() > 1 && friction[1] != 0.0) || (friction.size() > 2 && friction[2] != 0.0)) {
                Warning(*node, fmt::format("The torsional and rolling friction specified in the friction "
        ...
        if (compute_inertia) {
            ...
            if (calculator.used_obb_fallback()) {
                Warning(*node, fmt::format("CalcSpatialInertia() failed to compute a physically "
            ...
            SpatialInertia<double> M_GG_G(mass, M_GG_G_one.get_com(), M_GG_G_one.get_unit_inertia());
            geom.M_GBo_B = M_GG_G.ReExpress(geom.X_BG.rotation()).Shift(-geom.X_BG.translation());
        }
        return geom;
    }
    ...
    void ParseBody(XMLElement* node,
        ...
        if (inertial_node && (inertia_from_geom_ != kTrue)) {
            ...
            Error(*node, fmt::format("{} has no inertial tag and inertiafromgeom=false. You "
        ...
        for (XMLElement* link_node = node->FirstChildElement("geom"); link_node;
            ...
            auto geom = ParseGeometry(link_node, geometries.size(), compute_inertia, child_class);
        ...
        const RigidBody<double>& body = plant_->AddRigidBody(body_name, model_instance_, M_BBo_B);

        if (plant_->geometry_source_is_registered()) {
            for (auto& geom : geometries) {
                if (geom.register_visual) {
                    plant_->RegisterVisualGeometry(body, geom.X_BG, *geom.shape, geom.name, geom.rgba);
                }
                if (geom.register_collision) {
                    plant_->RegisterCollisionGeometry(body, geom.X_BG, *geom.shape, geom.name, geom.friction);
                }
            }
        }
        ...
        if (joint_node) {
            ...
            for (; joint_node; joint_node = joint_node->NextSiblingElement("joint")) {
                if (joint_node->NextSiblingElement("joint")) {
                    const RigidBody<double>& dummy_body =
                            plant_->AddRigidBody(fmt::format("{}{}", body_name, dummy_bodies++), model_instance_,
                                                 SpatialInertia<double>::Zero());
                    ParseJoint(joint_node, *last_body, dummy_body, X_WP, RigidTransformd(), child_class);
                ...
                if (type == "free") {
                    if (dummy_bodies > 0) {
                        Error(*node, fmt::format("No other joints can be defined in the body {} if "
    ...
    void ParseWorldBody(XMLElement* node) {
        if (plant_->geometry_source_is_registered()) {
            ...
            for (XMLElement* link_node = node->FirstChildElement("geom"); link_node;
                ...
                if (!geom.shape) continue;
                if (geom.register_visual) {
                    plant_->RegisterVisualGeometry(plant_->world_body(), geom.X_BG, *geom.shape, geom.name, geom.rgba);
                }
                if (geom.register_collision) {
                    plant_->RegisterCollisionGeometry(plant_->world_body(), geom.X_BG, *geom.shape, geom.name,
                                                      geom.friction);
                }
                ++num_geometries;
            }
    ...
    void ParseClassDefaults(XMLElement* node,
        ...
        for (XMLElement* e = node->FirstChildElement(elt_name); e; e = e->NextSiblingElement(elt_name)) {
    ...
    void ParseDefault(XMLElement* node, const std::string& parent_default = "") {
        ...
        auto parse_class_defaults = [&](const std::string& element_name,
                                        std::map<std::string, XMLElement*>* default_map) {
            ParseClassDefaults(node, class_name, parent_default, element_name, default_map);
        };
        ...
        for (XMLElement* default_node = node->FirstChildElement("default"); default_node;
    ...
    void ParseAsset(XMLElement* node) {
        for (XMLElement* material_node = node->FirstChildElement("material"); material_node;
            ...
            if (ParseStringAttribute(mesh_node, "file", &file)) {
                ...
                if (ParseVectorAttribute(mesh_node, "scale", &scale)) {
                    if (scale[0] != scale[1] || scale[1] != scale[2]) {
                        Error(*node, fmt::format("mesh {} was defined with a non-uniform scale; but "
                ...
                if (std::filesystem::exists(filename)) {
                    ...
                    Warning(*node, fmt::format("Drake's MuJoCo parser currently only supports mesh files in "
                    ...
                    std::transform(original_extension.begin(), original_extension.end(), original_extension.begin(),
                                   [](unsigned char c) {
                                       return std::tolower(c);
                                   });
                    if (original_extension == ".stl") {
                        Warning(*node, fmt::format("If you have built Drake from source, "
                                                   "running\n\n bazel run "
                    ...
                    Warning(*node, fmt::format("The mesh asset \"{}\" could not be found, nor "
    ...
    void ParseCompiler(XMLElement* node) {
        ...
        if (ParseStringAttribute(node, "coordinate", &coordinate)) {
            if (coordinate != "local") {
                Error(*node, fmt::format("Compiler attribute coordinate={} is not supported yet.", coordinate));
        ...
        if (ParseStringAttribute(node, "angle", &angle)) {
            if (angle == "degree") {
                ...
                Warning(*node, fmt::format("Unknown value {} for the compiler `angle` attribute will be "
        ...
        if (ParseStringAttribute(node, "eulerseq", &eulerseq)) {
            if (eulerseq.size() != 3 || eulerseq.find_first_not_of("xyzXYZ") != std::string::npos) {
                Error(*node, fmt::format("Illegal value '{}' for the eulerseq in {}. Valid eulerseq are "
        ...
        switch (node->QueryBoolAttribute("inertiafromgeom", &flag)) {
            ...
            case tinyxml2::XML_WRONG_ATTRIBUTE_TYPE:
                ...
                Warning(*node, fmt::format("The attribute 'inertiafromgeom' found in a '{}' tag cannot be "
    ...
    void ParseContact(XMLElement* node) {
        ...
        geometry::SceneGraph<double>* scene_graph = plant_->GetMutableSceneGraphPreFinalize();
        geometry::CollisionFilterManager manager = scene_graph->collision_filter_manager();
        const geometry::SceneGraphInspector<double>& inspector = scene_graph->model_inspector();
        const auto geom_ids = inspector.GetGeometryIds(geometry::GeometrySet(inspector.GetAllGeometryIds()),
        ...
        auto geom_id_from_name = [&inspector, &geom_ids, frame_group](const std::string& name) {
            for (GeometryId id : geom_ids) {
                // Only match geometry loaded from the current model instance.
                int candidate_frame_group = inspector.GetFrameGroup(inspector.GetFrameId(id));
        ...
        for (XMLElement* pair_node = node->FirstChildElement("pair"); pair_node;
            ...
            GeometryId geom1_id = geom_id_from_name(geom1), geom2_id = geom_id_from_name(geom2);
            if (!geom1_id.is_valid()) {
                Warning(*node, fmt::format("contact pair specified unknown geom1 {} and will be ignored.", geom1));
                continue;
            }
            if (!geom2_id.is_valid()) {
                Warning(*node, fmt::format("contact pair specified unknown geom2 {} and will be ignored.", geom2));
                continue;
            }
            ...
            if (plant_->get_adjacent_bodies_collision_filters()) {
                ...
                const BodyIndex body1_index = plant_->GetBodyFromFrameId(inspector.GetFrameId(geom1_id))->index();
                const BodyIndex body2_index = plant_->GetBodyFromFrameId(inspector.GetFrameId(geom2_id))->index();
                for (const auto& joint_index : plant_->GetJointIndices(model_instance_)) {
                    const Joint<double>& joint = plant_->get_joint(joint_index);
                    if ((joint.parent_body().index() == body1_index && joint.child_body().index() == body2_index) ||
                        (joint.parent_body().index() == body2_index && (joint.child_body().index() == body1_index))) {
                        Warning(*node, fmt::format("This mjcf specified a contact pair with geom1 {} and "
            ...
            manager.Apply(geometry::CollisionFilterDeclaration().AllowBetween(geometry::GeometrySet({geom1_id}),
                                                                              geometry::GeometrySet({geom2_id})));
        }

        for (XMLElement* exclude_node = node->FirstChildElement("exclude"); exclude_node;
            ...
            geometry::FrameId fid1 =
                    plant_->GetBodyFrameIdOrThrow(plant_->GetBodyByName(body1, model_instance_).index());
            geometry::FrameId fid2 =
                    plant_->GetBodyFrameIdOrThrow(plant_->GetBodyByName(body2, model_instance_).index());
            manager.Apply(geometry::CollisionFilterDeclaration().ExcludeBetween(geometry::GeometrySet(fid1),
                                                                                geometry::GeometrySet(fid2)));
        }
    }

    // Updates node by recursively replacing any <include> elements under it with
    // the children of the named file's root element.
    void ExpandIncludeTags(XMLElement* node, const std::filesystem::path& parent_mjcf_path) {
        ...
        if (std::string(node->Value()) == "include") {
            ...
            if (include_doc.LoadFile(filename.c_str()) != tinyxml2::XML_SUCCESS) {
                Error(*node, fmt::format("Failed to load <include> file at path {}.", filename.string()));
                return;
            }

            XMLElement* include_root = include_doc.RootElement();
            if (!include_root) {
                Error(*node, fmt::format("Included file {} has no root element.", filename.string()));
                return;
            }
        ...
        while (child) {
            ...
            XMLElement* to_delete = (std::string(child->Value()) == "include") ? child : nullptr;
            child = child->NextSiblingElement();
    ...
    std::pair<std::optional<ModelInstanceIndex>, std::string> Parse(
        ...
        if (model_name.empty() && !ParseStringAttribute(node, "model", &model_name)) {
            Error(*node,
        ...
        for (XMLElement* compiler_node = node->FirstChildElement("compiler"); compiler_node;
             compiler_node = compiler_node->NextSiblingElement("compiler")) {
            ParseCompiler(compiler_node);
        }

        // Parse the options.
        for (XMLElement* option_node = node->FirstChildElement("option"); option_node;
        ...
        for (XMLElement* link_node = node->FirstChildElement("worldbody"); link_node;
             link_node = link_node->NextSiblingElement("worldbody")) {
        ...
        for (XMLElement* actuator_node = node->FirstChildElement("actuator"); actuator_node;
             actuator_node = actuator_node->NextSiblingElement("actuator")) {
            ParseActuator(actuator_node);
        }

        // Parses the model's contact elements.
        for (XMLElement* contact_node = node->FirstChildElement("contact"); contact_node;
    ...
    void Error(const XMLNode& location, std::string message) const { diagnostic_.Error(location, std::move(message)); }
    ...
    void WarnUnsupportedAttribute(const XMLElement& node, const std::string& attribute) {
...
std::pair<std::optional<ModelInstanceIndex>, std::string> AddOrMergeModelFromMujocoXml(
    ...
    if (data_source.IsFilename()) {
        ...
        if (xml_doc.ErrorID()) {
            diag.Error(xml_doc, fmt::format("Failed to parse XML string: {}", xml_doc.ErrorName()));
            return {};
        }
    ...
    return parser.Parse(model_name_in, parent_model_name, merge_into_model_instance, &xml_doc, path);
}
}  // namespace
...
std::optional<ModelInstanceIndex> AddModelFromMujocoXml(const DataSource& data_source,
                                                        ...
                                                        const std::optional<std::string>& parent_model_name,
                                                        const ParsingWorkspace& workspace) {
    return AddOrMergeModelFromMujocoXml(data_source, model_name_in, parent_model_name, workspace, std::nullopt).first;
}
...
std::optional<ModelInstanceIndex> MujocoParserWrapper::AddModel(const DataSource& data_source,
                                                                const std::string& model_name,
                                                                const std::optional<std::string>& parent_model_name,
                                                                const ParsingWorkspace& workspace) {
    return AddModelFromMujocoXml(data_source, model_name, parent_model_name, workspace);
}

std::string MujocoParserWrapper::MergeModel(const DataSource& data_source,
                                            const std::string& model_name,
                                            ModelInstanceIndex merge_into_model_instance,
                                            const ParsingWorkspace& workspace) {
    return AddOrMergeModelFromMujocoXml(data_source, model_name, std::nullopt, workspace, merge_into_model_instance)
            .second;
}

std::vector<ModelInstanceIndex> MujocoParserWrapper::AddAllModels(const DataSource& data_source,
                                                                  const std::optional<std::string>& parent_model_name,
                                                                  const ParsingWorkspace& workspace) {
