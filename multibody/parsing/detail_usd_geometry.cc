#include "multibody/parsing/detail_usd_geometry.h"
...
Eigen::Quaterniond UsdQuatdToEigen(const pxr::GfQuatd& q) {
    return Eigen::Quaterniond(q.GetReal(), q.GetImaginary()[0], q.GetImaginary()[1], q.GetImaginary()[2]);
}

// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector3d> GetPrimScale(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
    pxr::UsdGeomXformable xformable = pxr::UsdGeomXformable(prim);
    if (!xformable) {
        diagnostic.Error(
                fmt::format("Failed to cast the Prim at {} into an UsdGeomXformable.", prim.GetPath().GetString()));
        return std::nullopt;
    }

    pxr::GfMatrix4d transform_matrix = xformable.ComputeLocalToWorldTransform(pxr::UsdTimeCode::Default());
...
// TODO(hong-nvidia): Add test case for this function.
double GetPrimMass(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector4d> GetGeomPrimColor(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
...
void RaiseFailedToReadAttributeError(const std::string& attr_name,
    ...
    diagnostic.Error(
            fmt::format("Failed to read the '{}' attribute of the Prim at {}.", attr_name, prim.GetPath().GetString()));
}

// TODO(hong-nvidia): Add test case for this function.
std::optional<math::RigidTransform<double>> GetPrimRigidTransform(const pxr::UsdPrim& prim,
    ...
    math::RotationMatrix<double> rotation_matrix(UsdQuatdToEigen(rotation.GetQuat()));

    return math::RigidTransform<double>(rotation_matrix, UsdVec3dToEigen(translation));
}
...
bool WriteMeshToObjFile(const std::string& file_path,
    ...
    for (const auto& vertex : vertices) {
        obj_file_contents.append(fmt::format("v {} {} {}\n", vertex[0], vertex[1], vertex[2]));
    }
    for (int i = 0; i < num_triangles; ++i) {
        ...
        obj_file_contents.append(fmt::format("f {} {} {}\n", index0, index1, index2));
    }

    std::ofstream out_file(file_path);
    if (!out_file.is_open()) {
        diagnostic.Error(fmt::format("Failed to create file {} for obj mesh.", file_path));
        return false;
    }
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector3d> GetBoxDimension(const pxr::UsdPrim& prim,
                                               double meters_per_unit,
                                               const DiagnosticPolicy& diagnostic) {
    pxr::UsdGeomCube cube = pxr::UsdGeomCube(prim);
    if (!cube) {
        diagnostic.Error(fmt::format("Failed to cast the Prim at {} into an UsdGeomCube.", prim.GetPath().GetString()));
        return std::nullopt;
    }
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector3d> GetEllipsoidDimension(const pxr::UsdPrim& prim,
                                                     double meters_per_unit,
                                                     const DiagnosticPolicy& diagnostic) {
    pxr::UsdGeomSphere sphere = pxr::UsdGeomSphere(prim);
    if (!sphere) {
        diagnostic.Error(
                fmt::format("Failed to cast the Prim at {} into an UsdGeomSphere.", prim.GetPath().GetString()));
        return std::nullopt;
    }
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector2d> GetCylinderDimension(const pxr::UsdPrim& prim,
                                                    double meters_per_unit,
    ...
    if (!cylinder) {
        diagnostic.Error(
                fmt::format("Failed to cast the Prim at {} into an UsdGeomCylinder.", prim.GetPath().GetString()));
        return std::nullopt;
    }
    ...
    if (cylinder_axis.value() != stage_up_axis) {
        diagnostic.Error(
                fmt::format("Only upright cylinders are supported at the moment. The cylinder at {} "
                            "is not upright because its axis '{}' differs from the axis of the "
                            "stage '{}'.",
                            prim.GetPath().GetString(), cylinder_axis.value().GetString(), stage_up_axis.GetString()));
        return std::nullopt;
    }

    std::optional<Eigen::Vector3d> prim_scale_opt = GetPrimScale(prim, diagnostic);
    ...
    if (prim_scale[0] != prim_scale[1]) {
        diagnostic.Error(
                fmt::format("The cylinder at {} has different scaling in X and Y axis, and that is "
                            "not supported.",
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector2d> GetCapsuleDimension(const pxr::UsdPrim& prim,
                                                   double meters_per_unit,
                                                   const pxr::TfToken& stage_up_axis,
                                                   const DiagnosticPolicy& diagnostic) {
    ...
    if (!capsule) {
        diagnostic.Error(
                fmt::format("Failed to cast the Prim at {} into an UsdGeomCapsule.", prim.GetPath().GetString()));
        return std::nullopt;
    }
    ...
    if (capsule_axis.value() != stage_up_axis) {
        diagnostic.Error(
                fmt::format("Only upright capsules are supported at the moment. The capsule at {} "
                            "is not upright because its axis '{}' differs from the axis of the "
                            "stage '{}'.",
                            prim.GetPath().GetString(), capsule_axis.value().GetString(), stage_up_axis.GetString()));
        return std::nullopt;
    }
    ...
    if (prim_scale[0] != prim_scale[1]) {
        diagnostic.Error(
                fmt::format("The capsule at {} has different scaling in X and Y axis, and that is "
                            "not supported.",
                            prim.GetPath().GetString()));
        return std::nullopt;
    }
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<double> GetMeshScale(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
    std::optional<Eigen::Vector3d> prim_scale_opt = GetPrimScale(prim, diagnostic);
    if (!prim_scale_opt.has_value()) {
        return std::nullopt;
    }
    ...
    if (prim_scale[0] != prim_scale[1] || prim_scale[1] != prim_scale[2]) {
        diagnostic.Error(
                fmt::format("The scaling of the mesh at {} is not isotropic. Non-isotropic scaling "
...
// TODO(hong-nvidia): Add test case for this function.
std::unique_ptr<geometry::Shape> CreateGeometryBox(const pxr::UsdPrim& prim,
                                                   double meters_per_unit,
                                                   const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector3d> dimension = GetBoxDimension(prim, meters_per_unit, diagnostic);
...
// TODO(hong-nvidia): Add test case for this function.
std::unique_ptr<geometry::Shape> CreateGeometryEllipsoid(const pxr::UsdPrim& prim,
                                                         double meters_per_unit,
                                                         const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector3d> dimension = GetEllipsoidDimension(prim, meters_per_unit, diagnostic);
    if (dimension.has_value()) {
        if (dimension.value()[0] == dimension.value()[1] && dimension.value()[1] == dimension.value()[2]) {
            return std::make_unique<geometry::Sphere>(dimension.value()[0]);
        } else {
            return std::make_unique<geometry::Ellipsoid>(dimension.value()[0], dimension.value()[1],
                                                         dimension.value()[2]);
        }
    } else {
        return nullptr;
...
// TODO(hong-nvidia): Add test case for this function.
std::unique_ptr<geometry::Shape> CreateGeometryCylinder(const pxr::UsdPrim& prim,
                                                        double meters_per_unit,
                                                        const pxr::TfToken& stage_up_axis,
                                                        const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector2d> dimension =
...
// TODO(hong-nvidia): Add test case for this function.
std::unique_ptr<geometry::Shape> CreateGeometryMesh(const std::string& obj_file_path,
                                                    const pxr::UsdPrim& prim,
                                                    double meters_per_unit,
                                                    const DiagnosticPolicy& diagnostic) {
    pxr::UsdGeomMesh mesh = pxr::UsdGeomMesh(prim);
    if (!mesh) {
        diagnostic.Error(fmt::format("Failed to cast the Prim at {} into an UsdGeomMesh.", prim.GetPath().GetString()));
        return nullptr;
    }
    ...
    for (int count : face_vertex_counts) {
        if (count != 3) {
            diagnostic.Error(
                    fmt::format("The mesh at {} is not a triangle mesh. Only triangle mesh are "
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<SpatialInertia<double>> CreateSpatialInertiaForBox(const pxr::UsdPrim& prim,
                                                                 double meters_per_unit,
                                                                 const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector3d> dimension = GetBoxDimension(prim, meters_per_unit, diagnostic);
    if (dimension.has_value()) {
        return SpatialInertia<double>::SolidBoxWithMass(GetPrimMass(prim, diagnostic), dimension.value()[0],
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<SpatialInertia<double>> CreateSpatialInertiaForEllipsoid(const pxr::UsdPrim& prim,
                                                                       double meters_per_unit,
                                                                       const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector3d> dimension = GetEllipsoidDimension(prim, meters_per_unit, diagnostic);
    if (dimension.has_value()) {
        return SpatialInertia<double>::SolidEllipsoidWithMass(GetPrimMass(prim, diagnostic), dimension.value()[0],
                                                              dimension.value()[1], dimension.value()[2]);
    } else {
        return std::nullopt;
    }
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<SpatialInertia<double>> CreateSpatialInertiaForCylinder(const pxr::UsdPrim& prim,
                                                                      double meters_per_unit,
                                                                      const pxr::TfToken& stage_up_axis,
                                                                      const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector2d> dimension =
    ...
    if (dimension.has_value() && cylinder_axis.has_value()) {
        return SpatialInertia<double>::SolidCylinderWithMass(GetPrimMass(prim, diagnostic), dimension.value()[0],
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<SpatialInertia<double>> CreateSpatialInertiaForCapsule(const pxr::UsdPrim& prim,
                                                                     double meters_per_unit,
                                                                     const pxr::TfToken& stage_up_axis,
                                                                     const DiagnosticPolicy& diagnostic) {
    const std::optional<Eigen::Vector2d> dimension =
    ...
    if (dimension.has_value() && capsule_axis.has_value()) {
        return SpatialInertia<double>::SolidCapsuleWithMass(GetPrimMass(prim, diagnostic), dimension.value()[0],
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<pxr::TfToken> GetUsdGeomAxis(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
    pxr::TfToken axis;
    bool success = false;
...
// TODO(hong-nvidia): Add test case for this function.
std::optional<Eigen::Vector3d> GetUsdGeomAxisUnitVector(const pxr::UsdPrim& prim, const DiagnosticPolicy& diagnostic) {
    ...
    if (axis.value() == "X") {
        ...
        diagnostic.Error(fmt::format("The axis of the geometry at {} is invalid.", prim.GetPath().GetString()));
        return std::nullopt;
    }
}
