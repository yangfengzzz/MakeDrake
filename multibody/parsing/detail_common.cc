...
bool EndsWithCaseInsensitive(std::string_view str, std::string_view ext) {
    ...
    return std::equal(str.end() - ext.size(), str.end(), ext.begin(), [](char a, char b) {
        return tolower(a) == tolower(b);
    });
}

DataSource::DataSource(DataSourceType type, const std::string* data) : type_(type), data_(data) {
...
geometry::ProximityProperties ParseProximityProperties(
    ...
    std::optional<double> hydroelastic_modulus = read_double("drake:hydroelastic_modulus");
    if (hydroelastic_modulus) {
        if (is_rigid) {
            diagnostic.Warning(
                    fmt::format("Rigid geometries defined with the tag drake:rigid_hydroelastic"
    ...
    std::optional<double> dissipation = read_double("drake:hunt_crossley_dissipation");

    std::optional<double> relaxation_time = read_double("drake:relaxation_time");

    std::optional<double> stiffness = read_double("drake:point_contact_stiffness");
    ...
    if (relaxation_time.has_value()) {
        if (*relaxation_time < 0) {
            throw std::logic_error(fmt::format("The dissipation time scale can't be negative; given {}", *dissipation));
        }
        properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kRelaxationTime,
                               *relaxation_time);
    }
...
const LinearBushingRollPitchYaw<double>* ParseLinearBushingRollPitchYaw(
    ...
    const Eigen::Vector3d bushing_torque_stiffness = read_vector("drake:bushing_torque_stiffness");
    const Eigen::Vector3d bushing_torque_damping = read_vector("drake:bushing_torque_damping");
    const Eigen::Vector3d bushing_force_stiffness = read_vector("drake:bushing_force_stiffness");
    const Eigen::Vector3d bushing_force_damping = read_vector("drake:bushing_force_damping");

    return &plant->AddForceElement<LinearBushingRollPitchYaw>(*frame_A, *frame_C, bushing_torque_stiffness,
                                                              bushing_torque_damping, bushing_force_stiffness,
                                                              bushing_force_damping);
}
...
void CollectCollisionFilterGroup(
    ...
    for (auto member_node = next_child_element(group_node, "drake:member");
         std::holds_alternative<sdf::ElementPtr>(member_node) ? std::get<sdf::ElementPtr>(member_node) != nullptr
    ...
    resolver->AddGroup(diagnostic, group_name, bodies, member_groups, model_instance);

    for (auto ignore_node = next_child_element(group_node, "drake:ignored_collision_filter_group");
         std::holds_alternative<sdf::ElementPtr>(ignore_node) ? std::get<sdf::ElementPtr>(ignore_node) != nullptr
                                                              : std::get<tinyxml2::XMLElement*>(ignore_node) != nullptr;
         ignore_node = next_sibling_element(ignore_node, "drake:ignored_collision_filter_group")) {
...
void ParseCollisionFilterGroupCommon(
    ...
    for (auto group_node = next_child_element(model_node, "drake:collision_filter_group");
         std::holds_alternative<sdf::ElementPtr>(group_node) ? std::get<sdf::ElementPtr>(group_node) != nullptr
                                                             : std::get<tinyxml2::XMLElement*>(group_node) != nullptr;
         group_node = next_sibling_element(group_node, "drake:collision_filter_group")) {
        CollectCollisionFilterGroup(diagnostic, model_instance, *plant, group_node, resolver, next_child_element,
                                    next_sibling_element, has_attribute, read_string_attribute, read_bool_attribute,
                                    read_tag_string);
    }
}
...
SpatialInertia<double> ParseSpatialInertia(const DiagnosticPolicy& diagnostic,
    ...
    const double plausible_dummy_mass = (std::isfinite(mass) && mass > 0.0) ? mass : 1.0;
    ...
    const double plausible_radius = std::cbrt((3.0 / (4.0 * M_PI)) * plausible_volume);
    ...
    const SpatialInertia<double> Mdum_BBcm =
            SpatialInertia<double>::SolidSphereWithDensity(kPlausibleDensity, plausible_radius);
    ...
    try {
        ...
        I_BBcm_Bi = RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(
                inertia_Bi_Bi.ixx, inertia_Bi_Bi.iyy, inertia_Bi_Bi.izz, inertia_Bi_Bi.ixy, inertia_Bi_Bi.ixz,
                inertia_Bi_Bi.iyz);
    } catch (const std::exception& e) {
        diagnostic.Warning(fmt::format("While parsing inertia matrix: {}", e.what()));
        return Mdum_BBo_B;
    }

    // If this is a massless body, return a zero SpatialInertia.
    if (mass == 0.0 && I_BBcm_Bi.get_moments().isZero() && I_BBcm_Bi.get_products().isZero()) {
        return SpatialInertia<double>(0.0, Vector3d::Zero(), UnitInertia<double>{0.0, 0.0, 0.0});
    }
    ...
    try {
        return SpatialInertia<double>::MakeFromCentralInertia(mass, p_BoBcm_B, I_BBcm_B);
    } catch (const std::exception& e) {
        diagnostic.Warning(fmt::format("While re-expressing as central inertia: {}", e.what()));
        return Mdum_BBo_B;
    }
    DRAKE_UNREACHABLE();
}
