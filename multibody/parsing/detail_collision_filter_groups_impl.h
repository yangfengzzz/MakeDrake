...
public:
    ...
    template <typename U>
    auto Convert(const std::function<U(const T&)>& name_converter) const -> CollisionFilterGroupsImpl<U>;
...
CollisionFilterGroupsImpl<std::string> ConvertInstancedNamesToStrings(
        const CollisionFilterGroupsImpl<InstancedName>& other, const MultibodyPlantDeferred& plant) {
    auto name_converter = [&plant](const InstancedName& instanced_name) {
        return instanced_name.index.has_value()
                       ? std::string{ScopedName::Join(plant.GetModelInstanceName(*instanced_name.index),
                                                      instanced_name.name)
                                             .get_full()}
                       : instanced_name.name;
    };
...
DRAKE_FORMATTER_AS(typename T, drake::multibody::internal, CollisionFilterGroupsImpl<T>, x, x.to_string())
