#include "multibody/parsing/detail_usd_parser.h"
...
private:
    ...
    std::unique_ptr<geometry::Shape> CreateCollisionGeometry(const pxr::UsdPrim& prim);
    std::unique_ptr<geometry::Shape> CreateVisualGeometry(const pxr::UsdPrim& prim);
...
std::optional<ModelInstanceIndex> UsdParserWrapper::AddModel(const DataSource& data_source,
...
std::vector<ModelInstanceIndex> UsdParserWrapper::AddAllModels(const DataSource& data_source,
                                                               const std::optional<std::string>& parent_model_name,
                                                               const ParsingWorkspace& workspace) {
...
std::unique_ptr<geometry::Shape> UsdParser::CreateVisualGeometry(const pxr::UsdPrim& prim) {
    if (prim.IsA<pxr::UsdGeomCube>()) {
        ...
        return CreateGeometryEllipsoid(prim, metadata_.meters_per_unit, w_.diagnostic);
    } else if (prim.IsA<pxr::UsdGeomCapsule>()) {
        return CreateGeometryCapsule(prim, metadata_.meters_per_unit, metadata_.up_axis, w_.diagnostic);
    } else if (prim.IsA<pxr::UsdGeomCylinder>()) {
        return CreateGeometryCylinder(prim, metadata_.meters_per_unit, metadata_.up_axis, w_.diagnostic);
        ...
        std::string obj_file_path = fmt::format("{}/{}.obj", temp_directory_, mesh_files_.size());
        mesh_files_.push_back(obj_file_path);
        return CreateGeometryMesh(obj_file_path, prim, metadata_.meters_per_unit, w_.diagnostic);
    } else {
...
std::unique_ptr<geometry::Shape> UsdParser::CreateCollisionGeometry(const pxr::UsdPrim& prim) {
...
const RigidBody<double>* UsdParser::CreateRigidBody(const pxr::UsdPrim& prim) {
    ...
    if (prim.IsA<pxr::UsdGeomCube>()) {
        inertia = CreateSpatialInertiaForBox(prim, metadata_.meters_per_unit, w_.diagnostic);
    } else if (prim.IsA<pxr::UsdGeomSphere>()) {
        inertia = CreateSpatialInertiaForEllipsoid(prim, metadata_.meters_per_unit, w_.diagnostic);
    } else if (prim.IsA<pxr::UsdGeomCapsule>()) {
        inertia = CreateSpatialInertiaForCapsule(prim, metadata_.meters_per_unit, metadata_.up_axis, w_.diagnostic);
    } else if (prim.IsA<pxr::UsdGeomCylinder>()) {
        inertia = CreateSpatialInertiaForCylinder(prim, metadata_.meters_per_unit, metadata_.up_axis, w_.diagnostic);
    } else {
        RaiseUnsupportedPrimTypeError(prim);
    }
    if (inertia.has_value()) {
        return &w_.plant->AddRigidBody(fmt::format("{}-RigidBody", prim.GetPath().GetString()), model_instance_,
                                       inertia.value());
    } else {
        return nullptr;
    }
...
void UsdParser::ProcessRigidBody(const pxr::UsdPrim& prim, bool is_static) {
    ...
    w_.plant->RegisterCollisionGeometry(*rigid_body, X_BG, *collision_geometry,
                                        fmt::format("{}-CollisionGeometry", prim.GetPath().GetString()),
                                        GetPrimFriction(prim));

    std::optional<Eigen::Vector4d> prim_color = GetGeomPrimColor(prim, w_.diagnostic);

    w_.plant->RegisterVisualGeometry(*rigid_body, X_BG, *visual_geometry,
...
UsdStageMetadata UsdParser::GetStageMetadata(const pxr::UsdStageRefPtr stage) {
    ...
    if (stage->HasAuthoredMetadata(pxr::TfToken("metersPerUnit"))) {
        success = stage->GetMetadata(pxr::TfToken("metersPerUnit"), &metadata.meters_per_unit);
    }
    if (!success) {
...
std::vector<ModelInstanceIndex> UsdParser::AddAllModels(const DataSource& data_source,
                                                        const std::optional<std::string>& parent_model_name) {
    ...
    if (data_source.IsFilename()) {
        ...
        if (!std::filesystem::exists(file_absolute_path)) {
            w_.diagnostic.Error(fmt::format("File does not exist: {}.", file_absolute_path));
            return std::vector<ModelInstanceIndex>();
        }
        stage = pxr::UsdStage::Open(file_absolute_path);
        if (!stage) {
            w_.diagnostic.Error(fmt::format("Failed to open USD stage: {}.", data_source.filename()));
            return std::vector<ModelInstanceIndex>();
        }
    } else {
    ...
    std::string model_name = MakeModelName(data_source.GetStem(), parent_model_name, w_);
