...
#include "multibody/parsing/detail_urdf_geometry.h"
#include "multibody/parsing/package_map.h"
...
public:
    ...
    // object.
    UrdfParser(const DataSource* data_source,
    ...
    void ParseTransmission(const JointEffortLimits& joint_effort_limits, XMLElement* node);
    void ParseJoint(JointEffortLimits* joint_effort_limits, XMLElement* node);
    void ParseMimicTag(XMLElement* node);
    const RigidBody<double>* GetBodyForElement(const std::string& element_name, const std::string& link_name);
    void ParseJointDynamics(XMLElement* node, double* damping);
    void ParseJointLimits(
            XMLElement* node, double* lower, double* upper, double* velocity, double* acceleration, double* effort);
    ...
    SpatialInertia<double> ExtractSpatialInertiaAboutBoExpressedInB(XMLElement* node);
    ...
    bool ParseScalarAttribute(const tinyxml2::XMLElement* node, const char* attribute_name, double* val) {
        return internal::ParseScalarAttribute(node, attribute_name, val, diagnostic_.MakePolicyForNode(node));
    }
...
SpatialInertia<double> UrdfParser::ExtractSpatialInertiaAboutBoExpressedInB(XMLElement* node) {
    ...
    return ParseSpatialInertia(diagnostic_.MakePolicyForNode(node), X_BBi, body_mass, inputs);
}
...
void UrdfParser::ParseBody(XMLElement* node, MaterialMap* materials) {
    ...
    if (ParseStringAttribute(node, "drake_ignore", &drake_ignore) && drake_ignore == std::string("true")) {
        return;
    }
    ...
    if (w_.plant->geometry_source_is_registered()) {
        ...
        for (XMLElement* visual_node = node->FirstChildElement("visual"); visual_node;
            ...
            std::optional<geometry::GeometryInstance> geometry_instance = ParseVisual(
                    diagnostic_, body_name, w_.package_map, root_dir_, visual_node, materials, &geometry_names);
            ...
            w_.plant->RegisterVisualGeometry(body, geometry_instance->pose(), geometry_instance->shape(),
                                             geometry_instance->name(), *geometry_instance->illustration_properties());
        }

        geometry_names.clear();  // See ParseCollision API; the names are per-role.
        for (XMLElement* collision_node = node->FirstChildElement("collision"); collision_node;
             collision_node = collision_node->NextSiblingElement("collision")) {
            std::optional<geometry::GeometryInstance> geometry_instance =
                    ParseCollision(diagnostic_, body_name, w_.package_map, root_dir_, collision_node, &geometry_names);
            if (!geometry_instance) {
                continue;
            }
            DRAKE_DEMAND(geometry_instance->proximity_properties() != nullptr);
            w_.plant->RegisterCollisionGeometry(body, geometry_instance->pose(), geometry_instance->shape(),
...
void UrdfParser::ParseCollisionFilterGroup(XMLElement* node) {
    ...
    auto next_sibling_element = [](const ElementNode& data_element, const char* element_name) {
        return std::get<XMLElement*>(data_element)->NextSiblingElement(element_name);
    };
    auto has_attribute = [](const ElementNode& data_element, const char* attribute_name) {
        std::string attribute_value;
        return ParseStringAttribute(std::get<XMLElement*>(data_element), attribute_name, &attribute_value);
    };
    auto get_string_attribute = [this](const ElementNode& data_element, const char* attribute_name) {
        std::string attribute_value;
    ...
    auto get_bool_attribute = [](const ElementNode& data_element, const char* attribute_name) {
        std::string attribute_value;
        ParseStringAttribute(std::get<XMLElement*>(data_element), attribute_name, &attribute_value);
        return attribute_value == std::string("true") ? true : false;
    };
    ParseCollisionFilterGroupCommon(diagnostic_.MakePolicyForNode(node), model_instance_, node, w_.plant,
                                    w_.collision_resolver, next_child_element, next_sibling_element, has_attribute,
...
void UrdfParser::ParseJointDynamics(XMLElement* node, double* damping) {
    *damping = 0.0;
    ...
    if (dynamics_node) {
        ...
        if (ParseScalarAttribute(dynamics_node, "friction", &coulomb_friction) && coulomb_friction != 0.0) {
        ...
        if (ParseScalarAttribute(dynamics_node, "coulomb_window", &coulomb_window)) {
            Warning(*dynamics_node,
...
void UrdfParser::ParseScrewJointThreadPitch(XMLElement* node, double* screw_thread_pitch) {
    // Always set a value for the output-only argument, even if parsing fails.
    *screw_thread_pitch = 0.0;
    XMLElement* screw_thread_pitch_node = node->FirstChildElement("drake:screw_thread_pitch");
    if (screw_thread_pitch_node) {
        if (!ParseScalarAttribute(screw_thread_pitch_node, "value", screw_thread_pitch)) {
...
const RigidBody<double>* UrdfParser::GetBodyForElement(const std::string& element_name, const std::string& link_name) {
    auto plant = w_.plant;
...
void UrdfParser::ParseJoint(JointEffortLimits* joint_effort_limits, XMLElement* node) {
    // TODO(rpoyner-tri): legacy undocumented tag: remove, fix?
    std::string drake_ignore;
    if (ParseStringAttribute(node, "drake_ignore", &drake_ignore) && drake_ignore == std::string("true")) {
        return;
    ...
    if (name.empty() || type.empty() || parent_name.empty() || child_name.empty()) {
        return;
    }
    ...
    if (axis_node && type.compare("fixed") != 0 && type.compare("floating") != 0 && type.compare("ball") != 0) {
        ParseVectorAttribute(axis_node, "xyz", &axis);
        if (axis.norm() < 1e-8) {
            Error(*axis_node, fmt::format("Joint '{}' axis is zero.  Don't do that.", name));
            return;
        }
        axis.normalize();
    }
    ...
    auto throw_on_custom_joint = [node, name, type, this](bool want_custom_joint) {
    ...
    if (type.compare("revolute") == 0 || type.compare("continuous") == 0) {
        ...
        index = plant->AddJoint<RevoluteJoint>(name, *parent_body, X_PF, *child_body, std::nullopt, axis, lower, upper,
                                               damping)
                        .index();
        Joint<double>& joint = plant->get_mutable_joint(*index);
        joint.set_velocity_limits(Vector1d(-velocity), Vector1d(velocity));
        joint.set_acceleration_limits(Vector1d(-acceleration), Vector1d(acceleration));
    } else if (type.compare("fixed") == 0) {
        throw_on_custom_joint(false);
        // Frame M is Frame B. Frame F and Frame M are coincident at the zero state
        // of the joint. See Joint class documentation.
        const RigidTransformd& X_PF = X_PB;
        index = plant->AddJoint<WeldJoint>(name, *parent_body, X_PF, *child_body, std::nullopt,
        ...
        // of the joint. See Joint class documentation.
        const RigidTransformd& X_PF = X_PB;
        index = plant->AddJoint<PrismaticJoint>(name, *parent_body, X_PF, *child_body, std::nullopt, axis, lower, upper,
                                                damping)
                        .index();
        Joint<double>& joint = plant->get_mutable_joint(*index);
        joint.set_velocity_limits(Vector1d(-velocity), Vector1d(velocity));
        ...
        // of the joint. See Joint class documentation.
        const RigidTransformd& X_PF = X_PB;
        index = plant->AddJoint<BallRpyJoint>(name, *parent_body, X_PF, *child_body, std::nullopt, damping).index();
        ...
        const RotationMatrixd R_BM =
                RotationMatrixd::MakeClosestRotationToIdentityFromUnitZ(axis);  // axis is normalized above.
        ...
        index = plant->AddJoint<PlanarJoint>(name, *parent_body, X_PF, *child_body, X_BM, damping_vec).index();
        ...
        // of the joint. See Joint class documentation.
        const RigidTransformd& X_PF = X_PB;
        index = plant->AddJoint<ScrewJoint>(name, *parent_body, X_PF, *child_body, std::nullopt, axis,
        ...
        // of the joint. See Joint class documentation.
        const RigidTransformd& X_PF = X_PB;
        index = plant->AddJoint<UniversalJoint>(name, *parent_body, X_PF, *child_body, std::nullopt, damping).index();
    } else {
        Error(*node, fmt::format("Joint '{}' has unrecognized type: '{}'", name, type));
        return;
    }
...
void UrdfParser::ParseMimicTag(XMLElement* node) {
    ...
    if (!plant->is_discrete() || plant->get_discrete_contact_solver() != DiscreteContactSolver::kSap) {
        Warning(*mimic_node, fmt::format("Joint '{}' specifies a mimic element that will be "
    ...
    if (!plant->HasJointNamed(joint_to_mimic, model_instance_)) {
        Error(*mimic_node, fmt::format("Joint '{}' mimic element specifies joint '{}' which"
                                       " does not exist.",
                                       name, joint_to_mimic));
        return;
    }
    ...
    if (joint_to_mimic == name) {
        Error(*mimic_node, fmt::format("Joint '{}' mimic element specifies "
    ...
    if (!plant->HasJointNamed(name, model_instance_)) {
        ...
        Warning(*mimic_node, fmt::format("Drake only supports the mimic element for "
    ...
    const Joint<double>& joint1 = plant->GetJointByName(joint_to_mimic, model_instance_);

    if (joint1.num_velocities() != joint0.num_velocities()) {
        Error(*mimic_node, fmt::format("Joint '{}' which has {} DOF cannot mimic "
                                       "joint '{}' which has {} DOF.",
                                       name, joint0.num_velocities(), joint_to_mimic, joint1.num_velocities()));
        return;
    }
...
void UrdfParser::ParseMechanicalReduction(const XMLElement& node) {
    ...
    Warning(*child, fmt::format("A '{}' element contains a mechanicalReduction element with a"
                                " value '{}' other than the default of 1. MultibodyPlant does"
                                " not currently support non-default mechanical reductions.",
                                node.Name(), text));
}

void UrdfParser::ParseTransmission(const JointEffortLimits& joint_effort_limits, XMLElement* node) {
    ...
    const Joint<double>& joint = plant->GetJointByName(joint_name, model_instance_);
    ...
    const JointActuator<double>& actuator = plant->AddJointActuator(actuator_name, joint, effort_iter->second);

    // Parse and add the optional drake:rotor_inertia parameter.
    XMLElement* rotor_inertia_node = actuator_node->FirstChildElement("drake:rotor_inertia");
    if (rotor_inertia_node) {
        double rotor_inertia;
        if (!ParseScalarAttribute(rotor_inertia_node, "value", &rotor_inertia)) {
            Error(*rotor_inertia_node, fmt::format("joint actuator {}'s drake:rotor_inertia does not have"
                                                   " a \"value\" attribute!",
                                                   actuator_name));
            return;
        }
        plant->get_mutable_joint_actuator(actuator.index()).set_default_rotor_inertia(rotor_inertia);
    }

    // Parse and add the optional drake:gear_ratio parameter.
    XMLElement* gear_ratio_node = actuator_node->FirstChildElement("drake:gear_ratio");
    if (gear_ratio_node) {
        double gear_ratio;
        if (!ParseScalarAttribute(gear_ratio_node, "value", &gear_ratio)) {
            Error(*gear_ratio_node, fmt::format("joint actuator {}'s drake:gear_ratio does not have a"
                                                " \"value\" attribute!",
                                                actuator_name));
            return;
        }
        plant->get_mutable_joint_actuator(actuator.index()).set_default_gear_ratio(gear_ratio);
    }

    // Parse and add the optional drake:controller_gains parameter.
    XMLElement* controller_gains_node = actuator_node->FirstChildElement("drake:controller_gains");
    if (controller_gains_node) {
        double p = 0.0;
        if (!ParseScalarAttribute(controller_gains_node, "p", &p)) {
            Error(*controller_gains_node, fmt::format("joint actuator {}'s drake:controller_gains does not have a"
                                                      " 'p' attribute!",
                                                      actuator_name));
            return;
        }
...
void UrdfParser::ParseFrame(XMLElement* node) {
    ...
    w_.plant->AddFrame(std::make_unique<FixedOffsetFrame<double>>(name, body->body_frame(), X_BF));
}
...
void UrdfParser::ParseBushing(XMLElement* node) {
    ...
    // model.
    auto read_frame = [node, this](const char* element_name) -> const Frame<double>* {
...
void UrdfParser::ParseBallConstraint(XMLElement* node) {
    ...
    // improperly formed, or if it does not refer to a frame already in the
    // model.
    auto read_body = [node, this](const char* element_name) -> const RigidBody<double>* {
        XMLElement* value_node = node->FirstChildElement(element_name);
...
std::pair<std::optional<ModelInstanceIndex>, std::string> UrdfParser::Parse() {
    ...
    for (XMLElement* material_node = node->FirstChildElement("material"); material_node;
        ...
        ParseMaterial(diagnostic_, material_node, true /* name_required */, w_.package_map, root_dir_, &materials);
    }
    ...
    for (XMLElement* transmission_node = node->FirstChildElement("transmission"); transmission_node;
         transmission_node = transmission_node->NextSiblingElement("transmission")) {
    ...
    for (XMLElement* bushing_node = node->FirstChildElement("drake:linear_bushing_rpy"); bushing_node;
         bushing_node = bushing_node->NextSiblingElement("drake:linear_bushing_rpy")) {
        ParseBushing(bushing_node);
    }

    // Parses the model's custom Drake ball constraint tags.
    for (XMLElement* ball_constraint_node = node->FirstChildElement("drake:ball_constraint"); ball_constraint_node;
         ball_constraint_node = ball_constraint_node->NextSiblingElement("drake:ball_constraint")) {
...
std::pair<std::optional<ModelInstanceIndex>, std::string> AddOrMergeModelFromUrdf(
    ...
    if (data_source.IsFilename()) {
        ...
        if (xml_doc.ErrorID()) {
            diag.Error(xml_doc, fmt::format("Failed to parse XML file: {}", xml_doc.ErrorName()));
    ...
    UrdfParser parser(&data_source, model_name_in, parent_model_name, merge_into_model_instance,
...
std::optional<ModelInstanceIndex> AddModelFromUrdf(const DataSource& data_source,
                                                   const std::string& model_name_in,
                                                   const std::optional<std::string>& parent_model_name,
                                                   const ParsingWorkspace& workspace) {
    return AddOrMergeModelFromUrdf(data_source, model_name_in, parent_model_name, workspace, std::nullopt).first;
}
...
std::optional<ModelInstanceIndex> UrdfParserWrapper::AddModel(const DataSource& data_source,
                                                              const std::string& model_name,
                                                              const std::optional<std::string>& parent_model_name,
                                                              const ParsingWorkspace& workspace) {
    return AddModelFromUrdf(data_source, model_name, parent_model_name, workspace);
}
...
std::string UrdfParserWrapper::MergeModel(const DataSource& data_source,
                                          ...
                                          const ParsingWorkspace& workspace) {
    return AddOrMergeModelFromUrdf(data_source, model_name, std::nullopt, workspace, merge_into_model_instance).second;
}

std::vector<ModelInstanceIndex> UrdfParserWrapper::AddAllModels(const DataSource& data_source,
                                                                const std::optional<std::string>& parent_model_name,
                                                                const ParsingWorkspace& workspace) {
