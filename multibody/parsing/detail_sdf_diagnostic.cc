...
SDFormatDiagnostic::SDFormatDiagnostic(const drake::internal::DiagnosticPolicy* diagnostic,
                                       const drake::multibody::internal::DataSource* data_source,
                                       const std::string& file_extension)
    : diagnostic_(diagnostic), data_source_(data_source), file_extension_(file_extension) {
    DRAKE_DEMAND(diagnostic != nullptr);
    DRAKE_DEMAND(data_source != nullptr);
}

DiagnosticDetail SDFormatDiagnostic::MakeDetail(const sdf::Element& element, const std::string& message) const {
...
void SDFormatDiagnostic::Warning(sdf::ElementConstPtr element, const std::string& message) const {
    diagnostic_->Warning(MakeDetail(*element, message));
}

void SDFormatDiagnostic::Error(sdf::ElementConstPtr element, const std::string& message) const {
    diagnostic_->Error(MakeDetail(*element, message));
}

DiagnosticPolicy SDFormatDiagnostic::MakePolicyForNode(const sdf::Element& element) const {
...
bool SDFormatDiagnostic::PropagateErrors(const sdf::Errors& errors) const {
    ...
    for (const auto& e : errors) {
        ...
        if (e.XmlPath().has_value()) {
            detail.message = fmt::format("At XML path {}: {}", e.XmlPath().value(), e.Message());
        } else {
...
void CheckSupportedElements(const SDFormatDiagnostic& diagnostic,
    ...
    while (element) {
        ...
        if ((supported_elements.find(element_name) == supported_elements.end()) && element->GetExplicitlySetInFile()) {
            // Unsupported elements in the drake namespace are errors.
            if (element_name.find("drake:") == 0) {
                std::string message = std::string("Unsupported SDFormat element in ") + root_element->GetName() +
                                      std::string(": ") + element_name;
                diagnostic.Error(element, std::move(message));
            } else {
                std::string message = std::string("Ignoring unsupported SDFormat element in ") +
...
void CheckSupportedElementValue(const SDFormatDiagnostic& diagnostic,
    ...
    if (value->GetAsString() != expected) {
        std::string message = std::string("Unsupported value for SDFormat element ") + element->GetName() +
