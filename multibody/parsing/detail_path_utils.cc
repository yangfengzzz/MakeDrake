...
string ResolveUri(const DiagnosticPolicy& diagnostic,
                  const string& uri,
    ...
    static const never_destroyed<std::regex> uri_matcher{"^([a-z0-9+.-]+)://([^/]*)/+(.*)"};
    std::smatch match;
    if (std::regex_match(uri, match, uri_matcher.access())) {
        ...
        if (uri_scheme == "file") {
            ...
            if (!package_map.Contains(uri_package)) {
                diagnostic.Error(fmt::format("URI '{}' refers to unknown package '{}'", uri, uri_package));
                return {};
            }
            std::optional<string> deprecation;
            const std::string& package_path = package_map.GetPath(uri_package, &deprecation);
            ...
            diagnostic.Error(
                    fmt::format("URI '{}' specifies an unsupported scheme; supported schemes are "
        ...
        if (root_dir.empty()) {
            diagnostic.Error(fmt::format("URI '{}' is invalid when parsing a string instead of a filename.", uri));
            return {};
        }
    ...
    if (!fs::exists(result)) {
        diagnostic.Error(fmt::format("URI '{}' resolved to '{}' which does not exist.", uri, result.string()));
        return {};
    }
