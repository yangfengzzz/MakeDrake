...
std::optional<T> GetChildElementValue(const SDFormatDiagnostic& diagnostic,
    ...
    if (!element->HasElement(child_name)) {
        ...
        std::string message = "Element <" + child_name + "> is required within element <" + element->GetName() + ">.";
...
std::optional<std::unique_ptr<geometry::Shape>> MakeShapeFromSdfGeometry(const SDFormatDiagnostic& diagnostic,
    ...
    const std::set<std::string> supported_geometry_elements{
            "box",   "capsule",   "cylinder", "drake:capsule", "drake:ellipsoid", "ellipsoid",
            "empty", "heightmap", "mesh",     "plane",         "polyline",        "sphere"};
    CheckSupportedElements(diagnostic, sdf_geometry.Element(), supported_geometry_elements);
    ...
    switch (sdf_geometry.Type()) {
        case sdf::GeometryType::EMPTY: {
            ...
            if (sdf_geometry.Element()->HasElement("drake:capsule")) {
                const sdf::ElementPtr capsule_element = sdf_geometry.Element()->GetElement("drake:capsule");
                CheckSupportedElements(diagnostic, capsule_element, {"radius", "length"});
                ...
                const sdf::ElementPtr ellipsoid_element = sdf_geometry.Element()->GetElement("drake:ellipsoid");
                CheckSupportedElements(diagnostic, ellipsoid_element, {"a", "b", "c"});
                std::optional<const double> a = GetChildElementValue<double>(diagnostic, ellipsoid_element, "a");
                if (!a.has_value()) return std::nullopt;
                std::optional<const double> b = GetChildElementValue<double>(diagnostic, ellipsoid_element, "b");
                if (!b.has_value()) return std::nullopt;
                std::optional<const double> c = GetChildElementValue<double>(diagnostic, ellipsoid_element, "c");
        ...
        case sdf::GeometryType::MESH: {
            ...
            const sdf::ElementConstPtr mesh_element = geometry_element->FindElement("mesh");
            DRAKE_DEMAND(mesh_element != nullptr);
            std::optional<std::string> mesh_uri = GetChildElementValue<std::string>(diagnostic, mesh_element, "uri");
            ...
            if (mesh_element->HasElement("scale")) {
                std::optional<gz::math::Vector3d> scale_vector =
                        GetChildElementValue<gz::math::Vector3d>(diagnostic, mesh_element, "scale");
                if (!scale_vector.has_value()) return std::nullopt;
                // geometry::Mesh only supports isotropic scaling and therefore we
                // enforce it.
                if (!(scale_vector->X() == scale_vector->Y() && scale_vector->X() == scale_vector->Z())) {
                    std::string message =
...
std::optional<std::unique_ptr<GeometryInstance>> MakeGeometryInstanceFromSdfVisual(const SDFormatDiagnostic& diagnostic,
                                                                                   const sdf::Visual& sdf_visual,
                                                                                   ResolveFilename resolve_filename,
                                                                                   const math::RigidTransformd& X_LG) {
    const std::set<std::string> supported_visual_elements{"geometry", "material", "pose", kAcceptingTag};
    CheckSupportedElements(diagnostic, sdf_visual.Element(), supported_visual_elements);
    ...
    switch (sdf_geometry.Type()) {
        ...
        case sdf::GeometryType::PLANE: {
            ...
            // direction.
            const RigidTransformd X_GC(geometry::HalfSpace::MakePose(normal_G, Vector3d::Zero()));
    ...
    auto instance = make_unique<GeometryInstance>(X_LC, std::move(*shape), sdf_visual.Name());
...
std::optional<IllustrationProperties> MakeVisualPropertiesFromSdfVisual(const SDFormatDiagnostic& diagnostic,
                                                                        const sdf::Visual& sdf_visual,
                                                                        ResolveFilename resolve_filename) {
    ...
    const sdf::ElementConstPtr material_element = visual_element->FindElement("material");

    if (material_element.get() != nullptr) {
        const std::set<std::string> supported_material_elements{"ambient", "diffuse", "drake:diffuse_map", "emissive",
                                                                "specular"};
        CheckSupportedElements(diagnostic, material_element, supported_material_elements);

        if (material_element->HasElement("drake:diffuse_map")) {
            auto [texture_name, has_value] = material_element->Get<std::string>("drake:diffuse_map", {});
            if (has_value) {
                const std::string resolved_path = resolve_filename(diagnostic, texture_name);
                if (resolved_path.empty()) {
                    std::string message =
                            std::string(fmt::format("Unable to locate the texture file: {}", texture_name));
        ...
        auto add_property = [material_element](const char* property, IllustrationProperties* props) {
            if (!material_element->HasElement(property)) return;
            using gz::math::Color;
            const std::pair<Color, bool> value_pair = material_element->Get<Color>(property, Color());
            if (value_pair.second == false) return;
            const Color& sdf_color = value_pair.first;

            Vector4<double> color{sdf_color.R(), sdf_color.G(), sdf_color.B(), sdf_color.A()};
    ...
    if (visual_element->HasElement(kAcceptingTag)) {
        ...
        while (accepting != nullptr) {
            ...
            if (name.empty()) {
                std::string message = fmt::format("<{}> tag given without any name", kAcceptingTag);
...
RigidTransformd MakeGeometryPoseFromSdfCollision(const sdf::Collision& sdf_collision, const RigidTransformd& X_LG) {
...
std::optional<ProximityProperties> MakeProximityPropertiesForCollision(const SDFormatDiagnostic& diagnostic,
                                                                       const sdf::Collision& sdf_collision) {
    const sdf::ElementPtr collision_element = sdf_collision.Element();
    DRAKE_DEMAND(collision_element != nullptr);

    const std::set<std::string> supported_collision_elements{"drake:proximity_properties", "geometry", "laser_retro",
                                                             "pose", "surface"};
    CheckSupportedElements(diagnostic, collision_element, supported_collision_elements);
    CheckSupportedElementValue(diagnostic, collision_element, "laser_retro", "0");

    const sdf::ElementConstPtr drake_element = collision_element->FindElement("drake:proximity_properties");

    geometry::ProximityProperties properties;
    if (drake_element != nullptr) {
        const std::set<std::string> supported_proximity_elements{
                "drake:rigid_hydroelastic",   "drake:compliant_hydroelastic",
                "drake:soft_hydroelastic",    "drake:mesh_resolution_hint",
                "drake:hydroelastic_modulus", "drake:hunt_crossley_dissipation",
                "drake:relaxation_time",      "drake:point_contact_stiffness",
                "drake:mu_dynamic",           "drake:mu_static"};
        CheckSupportedElements(diagnostic, drake_element, supported_proximity_elements);
        ...
        auto read_double = [drake_element, &diagnostic](const char* element_name) -> std::optional<double> {
            ...
            if (drake_element->FindElement(element_name) != nullptr) {
                result = GetChildElementValue<double>(diagnostic, drake_element, element_name);
            }
            return result;
        };

        const bool is_rigid = drake_element->HasElement("drake:rigid_hydroelastic");
        const bool is_compliant = drake_element->HasElement("drake:compliant_hydroelastic");
        ...
        const bool is_unsupported_soft = drake_element->HasElement("drake:soft_hydroelastic");
        ...
        properties = ParseProximityProperties(diagnostic.MakePolicyForNode(*drake_element), read_double, is_rigid,
                                              is_compliant);
    }

    // TODO(SeanCurtis-TRI): Remove all of this legacy parsing code based on
    //  issue #12598.
    if (!properties.HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction)) {
        ...
        properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, *coulomb_friction);
    } else {
        ...
        const sdf::ElementConstPtr surface_element = collision_element->FindElement("surface");
        if (surface_element.get()) {
            CheckSupportedElements(diagnostic, surface_element, {"friction"});
            const sdf::ElementConstPtr friction_element = surface_element->FindElement("friction");
            if (friction_element.get()) {
                ...
                if (ode_element->FindElement("mu").get() || ode_element->FindElement("mu2").get()) {
...
std::optional<CoulombFriction<double>> MakeCoulombFrictionFromSdfCollisionOde(const SDFormatDiagnostic& diagnostic,
    ...
    std::optional<const double> static_friction =
            GetChildElementValue<double>(diagnostic, ode_element, "mu", default_friction().static_friction());
    if (!static_friction.has_value()) return std::nullopt;
    std::optional<const double> dynamic_friction =
            GetChildElementValue<double>(diagnostic, ode_element, "mu2", default_friction().dynamic_friction());
