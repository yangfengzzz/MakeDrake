...
const drake::multibody::Frame<double>* GetScopedFrameByNameMaybe(const drake::multibody::MultibodyPlant<double>& plant,
...
const drake::multibody::Frame<double>& GetScopedFrameByName(const drake::multibody::MultibodyPlant<double>& plant,
                                                            const std::string& full_name) {
    if (full_name == "world") return plant.world_frame();
    auto scoped_name = multibody::ScopedName::Parse(full_name);
    if (!scoped_name.get_namespace().empty()) {
