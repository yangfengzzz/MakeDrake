...
#include "multibody/parsing/detail_composite_parse.h"
...
AddWeld ApplyDirectiveNamespace(const AddWeld& orig, const std::string& model_namespace) {
...
AddModel ApplyDirectiveNamespace(const AddModel& orig, const std::string& model_namespace) {
    if (model_namespace.empty()) return orig;
...
AddModelInstance ApplyDirectiveNamespace(const AddModelInstance& orig, const std::string& model_namespace) {
    if (model_namespace.empty()) return orig;
...
AddFrame ApplyDirectiveNamespace(const AddFrame& orig, const std::string& model_namespace) {
    ...
    if (!model_namespace.empty()) {
        result.name = DmdScopedNameJoin(model_namespace, orig.name).to_string();
        result.X_PF.base_frame = DmdScopedNameJoin(model_namespace, orig.X_PF.base_frame.value()).to_string();
    }
    ...
    if (orig_scoped_name.get_namespace().empty()) {
        ...
        const std::string base_frame_namespace(ScopedName::Parse(orig.X_PF.base_frame.value()).get_namespace());
        const std::string orig_element(orig_scoped_name.get_element());
        result.name =
                DmdScopedNameJoin(model_namespace, DmdScopedNameJoin(base_frame_namespace, orig_element).to_string())
                        .to_string();
    }
    return result;
}

AddCollisionFilterGroup ApplyDirectiveNamespace(const AddCollisionFilterGroup& orig,
                                                const std::string& model_namespace) {
    if (model_namespace.empty()) return orig;
    AddCollisionFilterGroup result;
    ScopedName orig_name = DmdScopedNameJoin(orig.model_namespace.value_or(""), orig.name);
    ScopedName full_name = DmdScopedNameJoin(model_namespace, orig_name.to_string());
...
void FlattenModelDirectivesInternal(const ModelDirectives& directives,
    ...
    for (auto& directive : directives.directives) {
        if (directive.add_directives) {
            ...
            const std::string sub_file = ResolveModelDirectiveUri(sub.file, package_map);
            FlattenModelDirectivesInternal(
                    LoadModelDirectives(sub_file), package_map, out,
                    (sub.model_namespace.has_value()
                             ? DmdScopedNameJoin(model_namespace, *sub.model_namespace).to_string()
                             : model_namespace));
        } else if (directive.add_weld) {
            ModelDirective result;
            result.add_weld = ApplyDirectiveNamespace(*directive.add_weld, model_namespace);
            out->directives.push_back(result);
        } else if (directive.add_model) {
            ModelDirective result;
            result.add_model = ApplyDirectiveNamespace(*directive.add_model, model_namespace);
            out->directives.push_back(result);
        } else if (directive.add_model_instance) {
            ModelDirective result;
            result.add_model_instance = ApplyDirectiveNamespace(*directive.add_model_instance, model_namespace);
            out->directives.push_back(result);
        } else if (directive.add_frame) {
            ModelDirective result;
            result.add_frame = ApplyDirectiveNamespace(*directive.add_frame, model_namespace);
            out->directives.push_back(result);
        } else if (directive.add_collision_filter_group) {
            ModelDirective result;
            result.add_collision_filter_group =
                    ApplyDirectiveNamespace(*directive.add_collision_filter_group, model_namespace);
            out->directives.push_back(result);
        } else {
            DRAKE_UNREACHABLE();
        }
...
std::string ResolveModelDirectiveUri(const std::string& uri, const PackageMap& package_map) {
...
void ProcessModelDirectives(const ModelDirectives& directives,
    ...
    std::vector<ModelInstanceInfo> models =
            multibody::internal::ParseModelDirectives(directives, "", composite->workspace());
    if (added_models) {
...
std::vector<ModelInstanceInfo> ProcessModelDirectives(const ModelDirectives& directives, Parser* parser) {
...
ModelDirectives LoadModelDirectives(const std::string& filename) {
    return multibody::internal::LoadModelDirectives({DataSource::kFilename, &filename});
}

ModelDirectives LoadModelDirectivesFromString(const std::string& model_directives) {
    return multibody::internal::LoadModelDirectives({DataSource::kContents, &model_directives});
}

void FlattenModelDirectives(const ModelDirectives& directives, const PackageMap& package_map, ModelDirectives* out) {
