#include "multibody/parsing/detail_collision_filter_group_resolver.h"

#include <unordered_set>
#include <vector>

#include "common/unused.h"
#include "multibody/parsing/detail_strongly_connected_components.h"
...
CollisionFilterGroupResolver::CollisionFilterGroupResolver(MultibodyPlant<double>* plant) : plant_(plant) {
    DRAKE_DEMAND(plant != nullptr);
    minimum_model_instance_index_ = ModelInstanceIndex(plant->num_model_instances());
...
void CollisionFilterGroupResolver::AddGroup(const DiagnosticPolicy& diagnostic,
    ...
    if (!ScopedName::Parse(group_name).get_namespace().empty()) {
        diagnostic.Error(fmt::format("group name '{}' cannot be a scoped name", full_group_name));
        return;
    }
    if (groups_.contains(full_group_name)) {
        diagnostic.Error(fmt::format("group '{}' has already been defined", full_group_name));
        return;
    }
    ...
    for (const auto& body_name : body_names) {
        ...
        const ScopedName scoped_body_name = ScopedName::Parse(FullyQualify(body_name, model_instance));

        const RigidBody<double>* body{};
        if (plant_->HasModelInstanceNamed(scoped_body_name.get_namespace())) {
            ModelInstanceIndex body_model = plant_->GetModelInstanceByName(scoped_body_name.get_namespace());
            if (body_model < minimum_model_instance_index_) {
                diagnostic.Error(
                        fmt::format("body name '{}' refers to a model outside"
                                    " the current parse",
                                    scoped_body_name));
                continue;
            }
        ...
        if (!body) {
            diagnostic.Error(fmt::format("body with name '{}' not found", scoped_body_name));
            continue;
        }
    ...
    for (const auto& insertion_group : member_group_names) {
        const auto full_insertion_group{FullyQualify(insertion_group, model_instance)};
...
void CollisionFilterGroupResolver::AddPair(const DiagnosticPolicy& diagnostic,
...
CollisionFilterGroupsImpl<InstancedName> CollisionFilterGroupResolver::Resolve(const DiagnosticPolicy& diagnostic) {
    ...
    // insertion order.
    StronglyConnectedComponents<std::string> sccs = FindStronglyConnectedComponents(group_insertion_graph_);
    ...
    for (const auto& pair : pairs_) {
        ...
        plant_->ExcludeCollisionGeometriesWithCollisionFilterGroupPair({name_a, set_a->geometries},
                                                                       {name_b, set_b->geometries});
    }
    ...
    return result.template Convert<InstancedName>([this](const std::string& input) {
        InstancedName instanced_name;
        auto scoped = ScopedName::Parse(input);
        if (plant_->HasModelInstanceNamed(scoped.get_namespace())) {
            instanced_name.index = plant_->GetModelInstanceByName(scoped.get_namespace());
        } else {
...
std::string CollisionFilterGroupResolver::FullyQualify(const std::string& name,
...
const CollisionFilterGroupResolver::GroupData* CollisionFilterGroupResolver::FindGroup(
        const DiagnosticPolicy& diagnostic, const std::string& group_name) const {
    auto iter = groups_.find(group_name);
    if (iter == groups_.end()) {
        diagnostic.Error(fmt::format("collision filter group with name '{}' not found", group_name));
        return nullptr;
    }
    return &iter->second;
}

const RigidBody<double>* CollisionFilterGroupResolver::FindBody(std::string_view name,
