...
#include "multibody/parsing/detail_ignition.h"
#include "multibody/parsing/detail_make_model_name.h"
#include "multibody/parsing/detail_path_utils.h"
#include "multibody/parsing/detail_sdf_diagnostic.h"
#include "multibody/parsing/detail_sdf_geometry.h"
#include "multibody/parsing/detail_urdf_parser.h"
#include "multibody/parsing/scoped_names.h"
#include "multibody/tree/ball_rpy_joint.h"
#include "multibody/tree/fixed_offset_frame.h"
...
#include "multibody/tree/scoped_name.h"
...
#include "multibody/tree/weld_joint.h"
...
using ModelInstanceIndexRange = std::pair<ModelInstanceIndex, ModelInstanceIndex>;
...
std::string GetInstanceScopeNameIgnoringWorld(const MultibodyPlant<double>& plant, ModelInstanceIndex instance) {
...
std::pair<ModelInstanceIndex, std::string> GetResolvedModelInstanceAndLocalName(const std::string& relative_nested_name,
                                                                                ModelInstanceIndex model_instance,
                                                                                const MultibodyPlant<double>& plant) {
    auto [parent_name, unscoped_local_name] = sdf::SplitName(relative_nested_name);
    ...
    if (!parent_name.empty()) {
        ...
        if (world_model_instance() == model_instance) {
            ...
            const std::string parent_model_absolute_name =
                    sdf::JoinName(plant.GetModelInstanceName(model_instance), parent_name);

            resolved_model_instance = plant.GetModelInstanceByName(parent_model_absolute_name);
        }
    }
...
std::string GetRelativeBodyName(const RigidBody<double>& body,
                                ...
                                const MultibodyPlant<double>& plant) {
    const std::string& relative_to_model_absolute_name = plant.GetModelInstanceName(relative_to_model_instance);
    ...
    if (relative_to_model_instance == world_model_instance()) {
        const std::string& model_absolute_name = plant.GetModelInstanceName(body.model_instance());
        ...
        const std::string& nested_model_absolute_name = plant.GetModelInstanceName(body.model_instance());
        ...
        const std::string required_prefix = relative_to_model_absolute_name + std::string(sdf::kSdfScopeDelimiter);
        DRAKE_DEMAND(nested_model_absolute_name.starts_with(required_prefix));

        const std::string nested_model_relative_name = nested_model_absolute_name.substr(required_prefix.size());
...
math::RigidTransformd ResolveRigidTransform(const SDFormatDiagnostic& diagnostic,
...
Eigen::Vector3d ResolveAxisXyz(const SDFormatDiagnostic& diagnostic, const sdf::JointAxis& axis) {
...
std::string ResolveJointParentLinkName(const SDFormatDiagnostic& diagnostic, const sdf::Joint& joint) {
    std::string link;
...
std::string ResolveJointChildLinkName(const SDFormatDiagnostic& diagnostic, const sdf::Joint& joint) {
    std::string link;
...
// object.
SpatialInertia<double> ExtractSpatialInertiaAboutBoExpressedInB(const SDFormatDiagnostic& diagnostic,
    ...
    return ParseSpatialInertia(
            diagnostic.MakePolicyForNode(*link_element), X_BBi, mass,
            {.ixx = I(0, 0), .iyy = I(1, 1), .izz = I(2, 2), .ixy = I(1, 0), .ixz = I(2, 0), .iyz = I(2, 1)});
}

// Helper method to retrieve a Body given the name of the link specification.
const RigidBody<double>& GetBodyByLinkSpecificationName(const std::string& link_name,
                                                        ModelInstanceIndex model_instance,
                                                        const MultibodyPlant<double>& plant) {
...
Vector3d ExtractJointAxis(const SDFormatDiagnostic& diagnostic, const sdf::Joint& joint_spec) {
    DRAKE_DEMAND(joint_spec.Type() == sdf::JointType::REVOLUTE || joint_spec.Type() == sdf::JointType::SCREW ||
                 joint_spec.Type() == sdf::JointType::PRISMATIC || joint_spec.Type() == sdf::JointType::CONTINUOUS);

    // Axis specification.
    const sdf::JointAxis* axis = joint_spec.Axis();
    if (axis == nullptr) {
        std::string message = fmt::format("An axis must be specified for joint '{}'", joint_spec.Name());
...
std::pair<Vector3d, Vector3d> ExtractJointAxisAndAxis2(const SDFormatDiagnostic& diagnostic,
                                                       const sdf::Joint& joint_spec) {
    DRAKE_DEMAND(joint_spec.Type() == sdf::JointType::REVOLUTE2 || joint_spec.Type() == sdf::JointType::UNIVERSAL);

    // Axis specification.
    const sdf::JointAxis* axis = joint_spec.Axis(0);
    const sdf::JointAxis* axis2 = joint_spec.Axis(1);
    if (axis == nullptr || axis2 == nullptr) {
        std::string message = fmt::format("Both axis and axis2 must be specified for joint '{}'", joint_spec.Name());
        diagnostic.Error(joint_spec.Element(), std::move(message));
...
double ParseJointDamping(const SDFormatDiagnostic& diagnostic, const sdf::Joint& joint_spec) {
    DRAKE_DEMAND(joint_spec.Type() == sdf::JointType::REVOLUTE || joint_spec.Type() == sdf::JointType::PRISMATIC ||
                 joint_spec.Type() == sdf::JointType::SCREW || joint_spec.Type() == sdf::JointType::UNIVERSAL ||
                 joint_spec.Type() == sdf::JointType::BALL || joint_spec.Type() == sdf::JointType::CONTINUOUS);
...
double GetEffortLimit(const SDFormatDiagnostic& diagnostic, const sdf::Joint& joint_spec, int axis_index) {
    DRAKE_DEMAND(axis_index == 0 || axis_index == 1);
    const sdf::JointAxis* axis = joint_spec.Axis(axis_index);
    if (axis == nullptr) {
        std::string message =
                fmt::format("An axis{} must be specified for joint '{}'", axis_index > 0 ? "2" : "", joint_spec.Name());
        diagnostic.Warning(joint_spec.Element(), std::move(message));
        return 0.0;
    }
    return axis->Effort();
...
void AddJointActuatorFromSpecification(const SDFormatDiagnostic& diagnostic,
    ...
    DRAKE_DEMAND(joint_spec.Type() == sdf::JointType::BALL || joint_spec.Type() == sdf::JointType::SCREW ||
                 joint_spec.Type() == sdf::JointType::UNIVERSAL || joint_spec.Type() == sdf::JointType::PRISMATIC ||
                 joint_spec.Type() == sdf::JointType::REVOLUTE || joint_spec.Type() == sdf::JointType::CONTINUOUS);
    ...
    if (joint_spec.Type() == sdf::JointType::UNIVERSAL) {
        if (GetEffortLimit(diagnostic, joint_spec, 0) != 0 || GetEffortLimit(diagnostic, joint_spec, 1) != 0) {
            std::string message = fmt::format(
    ...
    if (effort_limit != 0) {
        const JointActuator<double>& actuator = plant->AddJointActuator(joint_spec.Name(), joint, effort_limit);
        ...
        if (joint_spec.Element()->HasElement("drake:rotor_inertia")) {
            plant->get_mutable_joint_actuator(actuator.index())
                    .set_default_rotor_inertia(joint_spec.Element()->Get<double>("drake:rotor_inertia"));
        }

        // Parse and add the optional drake:gear_ratio parameter.
        if (joint_spec.Element()->HasElement("drake:gear_ratio")) {
            plant->get_mutable_joint_actuator(actuator.index())
                    .set_default_gear_ratio(joint_spec.Element()->Get<double>("drake:gear_ratio"));
        }

        // Parse and add the optional drake:controller_gains parameter.
        if (joint_spec.Element()->HasElement("drake:controller_gains")) {
            sdf::ElementPtr controller_gains = joint_spec.Element()->GetElement("drake:controller_gains");

            const bool has_p = controller_gains->HasAttribute("p");
            const bool has_d = controller_gains->HasAttribute("d");

            if (!has_p) {
                std::string message = "<drake:controller_gains>: Unable to find the 'p' attribute.";
                diagnostic.Error(controller_gains, std::move(message));
            }
            if (!has_d) {
                std::string message = "<drake:controller_gains>: Unable to find the 'd' attribute.";
                diagnostic.Error(controller_gains, std::move(message));
            }
            ...
            if (has_p && has_d) {
                ...
                plant->get_mutable_joint_actuator(actuator.index()).set_controller_gains({p, d});
            }
        }
    }
    if (joint_spec.Axis(1) != nullptr) {
...
void AddPrismaticSpringFromSpecification(const SDFormatDiagnostic& diagnostic,
    ...
    if (spring_stiffness > 0) {
        plant->AddForceElement<PrismaticSpring>(joint, spring_reference, spring_stiffness);
    } else if (spring_stiffness < 0) {
        std::string message =
                fmt::format("The stiffness specified for joint '{}' must be non-negative.", joint_spec.Name());
        diagnostic.Error(joint_spec.Element(), std::move(message));
    }
}
...
bool AddRevoluteSpringFromSpecification(const SDFormatDiagnostic& diagnostic,
    ...
    const sdf::JointAxis* axis = joint_spec.Axis();
    if (axis == nullptr) {
        std::string message = "An axis must be specified for joint '" + joint_spec.Name() + "'";
        diagnostic.Error(joint_spec.Element(), std::move(message));
        return false;
    }
    ...
    // throw an error.
    if (spring_stiffness != 0) {
        plant->AddForceElement<RevoluteSpring>(joint, spring_reference, spring_stiffness);
    }
...
std::optional<std::tuple<double, double, double, double>> ParseJointLimits(const SDFormatDiagnostic& diagnostic,
                                                                           const sdf::Joint& joint_spec) {
    DRAKE_THROW_UNLESS(joint_spec.Type() == sdf::JointType::REVOLUTE ||
    ...
    if (axis->Element()->HasElement("limit")) {
        ...
        const std::set<std::string> supported_limit_elements{"drake:acceleration", "effort", "lower",
        ...
        if (limit_element->HasElement("drake:acceleration")) {
            ...
            if (acceleration_limit < 0) {
                std::string message = "Acceleration limit is negative for joint '" + joint_spec.Name() +
    ...
    return std::make_tuple(lower_limit, upper_limit, velocity_limit, acceleration_limit);
}
...
bool AddJointFromSpecification(const SDFormatDiagnostic& diagnostic,
    ...
    const std::set<std::string> supported_joint_elements{
            "axis",        "axis2",  "child", "drake:rotor_inertia", "drake:gear_ratio", "drake:controller_gains",
            "drake:mimic", "parent", "pose",  "screw_thread_pitch"};
    CheckSupportedElements(diagnostic, joint_spec.Element(), supported_joint_elements);

    // Axis elements should be fully supported, let sdformat validate those.

    const RigidBody<double>& parent_body =
            GetBodyByLinkSpecificationName(ResolveJointParentLinkName(diagnostic, joint_spec), model_instance, *plant);
    const RigidBody<double>& child_body =
            GetBodyByLinkSpecificationName(ResolveJointChildLinkName(diagnostic, joint_spec), model_instance, *plant);
    ...
    const RigidTransformd X_CJ = ResolveRigidTransform(diagnostic, joint_spec.SemanticPose(),
    ...
    if (parent_body.index() == world_index()) {
        const RigidTransformd X_MJ = ResolveRigidTransform(diagnostic, joint_spec.SemanticPose(), relative_to);
    ...
    switch (joint_spec.Type()) {
        case sdf::JointType::FIXED: {
            plant->AddJoint<WeldJoint>(joint_spec.Name(), parent_body, X_PJ, child_body, X_CJ,
        ...
        case sdf::JointType::PRISMATIC: {
            ...
            std::tie(lower_limit, upper_limit, velocity_limit, acceleration_limit) = *joint_limits;
            const auto& joint = plant->AddJoint<PrismaticJoint>(joint_spec.Name(), parent_body, X_PJ, child_body, X_CJ,
                                                                axis_J, lower_limit, upper_limit, damping);
            plant->get_mutable_joint(joint.index())
                    .set_velocity_limits(Vector1d(-velocity_limit), Vector1d(velocity_limit));
            plant->get_mutable_joint(joint.index())
                    .set_acceleration_limits(Vector1d(-acceleration_limit), Vector1d(acceleration_limit));
        ...
        case sdf::JointType::REVOLUTE: {
            ...
            if (!joint_limits.has_value()) return false;
            std::tie(lower_limit, upper_limit, velocity_limit, acceleration_limit) = *joint_limits;
            const auto& joint = plant->AddJoint<RevoluteJoint>(joint_spec.Name(), parent_body, X_PJ, child_body, X_CJ,
                                                               axis_J, lower_limit, upper_limit, damping);
            plant->get_mutable_joint(joint.index())
                    .set_velocity_limits(Vector1d(-velocity_limit), Vector1d(velocity_limit));
            plant->get_mutable_joint(joint.index())
                    .set_acceleration_limits(Vector1d(-acceleration_limit), Vector1d(acceleration_limit));
            AddJointActuatorFromSpecification(diagnostic, joint_spec, joint, plant);
            if (!AddRevoluteSpringFromSpecification(diagnostic, joint_spec, joint, plant)) {
                return false;
            }
            break;
        }
        case sdf::JointType::UNIVERSAL: {
            ...
            if (!math::RotationMatrixd::IsValid(R_JI)) {
                std::string message =
                        fmt::format("axis and axis2 must be orthogonal for joint '{}'", joint_spec.Name());
                diagnostic.Error(joint_spec.Element(), std::move(message));
            } else {
                ...
                const auto& joint = plant->AddJoint<UniversalJoint>(joint_spec.Name(), parent_body, X_PF, child_body,
                                                                    X_CM, damping);
        ...
        case sdf::JointType::BALL: {
            const double damping = ParseJointDamping(diagnostic, joint_spec);
            const auto& joint =
                    plant->AddJoint<BallRpyJoint>(joint_spec.Name(), parent_body, X_PJ, child_body, X_CJ, damping);
            // At most, this prints a warning (it does not add an actuator).
            AddJointActuatorFromSpecification(diagnostic, joint_spec, joint, plant);
            break;
        }
        ...
        case sdf::JointType::SCREW: {
            ...
            Vector3d axis_J = ExtractJointAxis(diagnostic, joint_spec);
            const auto& joint = plant->AddJoint<ScrewJoint>(joint_spec.Name(), parent_body, X_PJ, child_body, X_CJ,
                                                            axis_J, screw_thread_pitch, damping);
            AddJointActuatorFromSpecification(diagnostic, joint_spec, joint, plant);
            break;
        }
        case sdf::JointType::GEARBOX: {
            ...
            std::string message = fmt::format("Joint type (gearbox) not supported for joint '{}'.", joint_spec.Name());
            diagnostic.Error(joint_spec.Element(), std::move(message));
            break;
        }
        case sdf::JointType::REVOLUTE2: {
            ...
            std::string message =
                    fmt::format("Joint type (revolute2) not supported for joint '{}'.", joint_spec.Name());
            diagnostic.Error(joint_spec.Element(), std::move(message));
            break;
        }
...
bool ParseMimicTag(const SDFormatDiagnostic& diagnostic,
    ...
    if (!plant->is_discrete() || plant->get_discrete_contact_solver() != DiscreteContactSolver::kSap) {
        diagnostic.Warning(joint_spec.Element(), fmt::format("Joint '{}' specifies a drake:mimic element that will be "
    ...
    if (!mimic_node->HasAttribute("joint")) {
        diagnostic.Error(mimic_node, fmt::format("Joint '{}' drake:mimic element is missing the "
    ...
    if (!plant->HasJointNamed(joint_to_mimic, model_instance)) {
        diagnostic.Error(mimic_node, fmt::format("Joint '{}' drake:mimic element specifies joint '{}' which"
                                                 " does not exist.",
    ...
    if (joint_to_mimic == joint_spec.Name()) {
        diagnostic.Error(mimic_node, fmt::format("Joint '{}' drake:mimic element specifies "
    ...
    const Joint<double>& joint0 = plant->GetJointByName(joint_spec.Name(), model_instance);
    const Joint<double>& joint1 = plant->GetJointByName(joint_to_mimic, model_instance);
    ...
    if (joint0.num_velocities() != 1 || joint1.num_velocities() != 1) {
        ...
        diagnostic.Warning(mimic_node, fmt::format("Drake only supports the drake:mimic element for "
...
std::optional<std::vector<LinkInfo>> AddLinksFromSpecification(const SDFormatDiagnostic& diagnostic,
    ...
    const std::set<std::string> supported_link_elements{"collision", "gravity", "inertial",
    ...
    for (uint64_t link_index = 0; link_index < model.LinkCount(); ++link_index) {
        ...
        const SpatialInertia<double> M_BBo_B =
                ExtractSpatialInertiaAboutBoExpressedInB(diagnostic, link_element, Inertial_Bcm_Bi);

        // Add a rigid body to model each link.
        const RigidBody<double>& body = plant->AddRigidBody(link.Name(), model_instance, M_BBo_B);

        // Register information.
        const RigidTransformd X_ML = ResolveRigidTransform(diagnostic, link.SemanticPose());
        ...
        if (plant->geometry_source_is_registered()) {
            ResolveFilename resolve_filename = [&package_map, &root_dir, &link_element](
                                                       const SDFormatDiagnostic& inner_diagnostic, std::string uri) {
                return ResolveUri(inner_diagnostic.MakePolicyForNode(*link_element), uri, package_map, root_dir);
            };

            for (uint64_t visual_index = 0; visual_index < link.VisualCount(); ++visual_index) {
                ...
                const RigidTransformd X_LG = ResolveRigidTransform(diagnostic, sdf_visual.SemanticPose());
                std::optional<unique_ptr<GeometryInstance>> geometry_instance =
                        MakeGeometryInstanceFromSdfVisual(diagnostic, sdf_visual, resolve_filename, X_LG);
                ...
                if (*geometry_instance) {
                    ...
                    plant->RegisterVisualGeometry(body, (*geometry_instance)->pose(), (*geometry_instance)->shape(),
                                                  (*geometry_instance)->name(),
                                                  *(*geometry_instance)->illustration_properties());
                }
            }

            for (uint64_t collision_index = 0; collision_index < link.CollisionCount(); ++collision_index) {
                const sdf::Collision& sdf_collision = *link.CollisionByIndex(collision_index);
                ...
                if (*shape != nullptr) {
                    const RigidTransformd X_LG = ResolveRigidTransform(diagnostic, sdf_collision.SemanticPose());
                    const RigidTransformd X_LC = MakeGeometryPoseFromSdfCollision(sdf_collision, X_LG);
                    ...
                    plant->RegisterCollisionGeometry(body, X_LC, **shape, sdf_collision.Name(), std::move(*props));
                }
            }
        }
    }
...
const Frame<double>& AddFrameFromSpecification(const SDFormatDiagnostic& diagnostic,
                                               const sdf::Frame& frame_spec,
                                               ModelInstanceIndex model_instance,
                                               const Frame<double>& default_frame,
                                               MultibodyPlant<double>* plant) {
    const Frame<double>* parent_frame{};
    const RigidTransformd X_PF = ResolveRigidTransform(diagnostic, frame_spec.SemanticPose(), frame_spec.AttachedTo());
    if (frame_spec.AttachedTo().empty()) {
        parent_frame = &default_frame;
    } else {
        const std::string attached_to_absolute_name =
                ScopedName::Join(GetInstanceScopeNameIgnoringWorld(*plant, model_instance), frame_spec.AttachedTo())
                        .to_string();
        ...
        if (plant->HasModelInstanceNamed(attached_to_absolute_name)) {
            const auto attached_to_model_instance = plant->GetModelInstanceByName(attached_to_absolute_name);
            parent_frame = &plant->GetFrameByName("__model__", attached_to_model_instance);
        } else {
            const auto [parent_model_instance, local_name] =
                    GetResolvedModelInstanceAndLocalName(frame_spec.AttachedTo(), model_instance, *plant);
            if (plant->HasFrameNamed(local_name, parent_model_instance)) {
                ...
                sdf::Errors errors = frame_spec.ResolveAttachedToBody(resolved_attached_to_body_name);
                diagnostic.PropagateErrors(errors);
                const std::string resolved_attached_to_body_absolute_name =
                        ScopedName::Join(GetInstanceScopeNameIgnoringWorld(*plant, model_instance),
                                         resolved_attached_to_body_name)
                                .to_string();
                parent_frame = parsing::GetScopedFrameByNameMaybe(*plant, resolved_attached_to_body_absolute_name);
            }
        }
    }
    ...
    const Frame<double>& frame =
            plant->AddFrame(std::make_unique<FixedOffsetFrame<double>>(frame_spec.Name(), *parent_frame, X_PF));
    return frame;
}
...
Eigen::Vector3d ParseVector3(const SDFormatDiagnostic& diagnostic,
    ...
    if (!node->HasElement(element_name)) {
        std::string message = fmt::format("<{}>: Unable to find the <{}> child tag.", node->GetName(), element_name);
...
const Frame<double>* ParseFrame(const SDFormatDiagnostic& diagnostic,
    ...
    const std::string current_model_name = plant->GetModelInstanceName(model_instance);
    ...
    if (!plant->HasModelInstanceNamed(search_model_name)) {
        std::string message = fmt::format(
                ...
                node->GetName(), search_model_name, frame_name, element_name, current_model_name);
        diagnostic.Error(node, std::move(message));
        return nullptr;
    }

    ModelInstanceIndex search_model_instance = plant->GetModelInstanceByName(search_model_name);
    ...
    if (!plant->HasFrameNamed(search_frame_name, search_model_instance)) {
        std::string message = fmt::format("<{}>: Frame '{}' specified for <{}> does not exist in the model.",
...
const RigidBody<double>* ParseBody(const SDFormatDiagnostic& diagnostic,
    ...
    if (!plant->HasBodyNamed(body_name, model_instance)) {
        std::string message = fmt::format("<{}>: Body '{}' specified for <{}> does not exist in the model.",
...
bool AddDrakeJointFromSpecification(const SDFormatDiagnostic& diagnostic,
                                    ...
                                    ModelInstanceIndex model_instance,
                                    MultibodyPlant<double>* plant) {
    const std::set<std::string> supported_joint_elements{"drake:parent", "drake:child", "drake:damping", "pose"};
    ...
    if (node->HasElement("pose")) {
        std::string message = "<drake:joint> does not yet support the <pose> child tag.";
        diagnostic.Error(node, std::move(message));
        return false;
    }

    const Frame<double>* parent_frame = ParseFrame(diagnostic, node, model_instance, plant, "drake:parent");
    if (parent_frame == nullptr) {
        return false;
    }
    const Frame<double>* child_frame = ParseFrame(diagnostic, node, model_instance, plant, "drake:child");
    ...
    if (joint_type == "planar") {
        ...
        plant->AddJoint(std::make_unique<PlanarJoint<double>>(joint_name, *parent_frame, *child_frame, damping));
    } else {
        std::string message =
                "ERROR: <drake:joint> '" + joint_name + "' has unrecognized value for 'type' attribute: " + joint_type;
        diagnostic.Error(node, std::move(message));
        return false;
    }
    return true;
...
const LinearBushingRollPitchYaw<double>* AddBushingFromSpecification(const SDFormatDiagnostic& diagnostic,
                                                                     const sdf::ElementPtr node,
                                                                     ModelInstanceIndex model_instance,
                                                                     MultibodyPlant<double>* plant) {
    const std::set<std::string> supported_bushing_elements{
            "drake:bushing_frameA",          "drake:bushing_frameC",         "drake:bushing_force_damping",
            "drake:bushing_force_stiffness", "drake:bushing_torque_damping", "drake:bushing_torque_stiffness"};
    ...
    auto read_vector = [&diagnostic, node](const char* element_name) -> Eigen::Vector3d {
    ...
    // the plant.
    auto read_frame = [&diagnostic, node, model_instance, plant](const char* element_name) -> const Frame<double>* {
...
std::optional<MultibodyConstraintId> AddBallConstraintFromSpecification(const SDFormatDiagnostic& diagnostic,
                                                                        const sdf::ElementPtr node,
                                                                        ModelInstanceIndex model_instance,
                                                                        MultibodyPlant<double>* plant) {
    ...
    // the plant.
    auto read_body = [&diagnostic, node, model_instance, plant](const char* element_name) -> const RigidBody<double>* {
...
bool AreWelded(const MultibodyPlant<double>& plant, const RigidBody<double>& a, const RigidBody<double>& b) {
...
void ParseCollisionFilterGroup(const SDFormatDiagnostic& diagnostic,
    ...
    auto next_child_element = [](const ElementNode& data_element, const char* element_name) {
        return std::get<sdf::ElementPtr>(data_element)->GetElementImpl(std::string(element_name));
    };
    auto next_sibling_element = [](const ElementNode& data_element, const char* element_name) {
        return std::get<sdf::ElementPtr>(data_element)->GetNextElement(std::string(element_name));
    };
    auto has_attribute = [](const ElementNode& data_element, const char* attribute_name) {
        return std::get<sdf::ElementPtr>(data_element)->HasAttribute(std::string(attribute_name));
    };
    auto get_string_attribute = [&diagnostic](const ElementNode& data_element,
                                              const char* attribute_name) -> std::string {
        auto element = std::get<sdf::ElementPtr>(data_element);
        if (!element->HasAttribute(attribute_name)) {
            std::string message = fmt::format("The tag <{}> is missing the required attribute \"{}\"",
                                              element->GetName(), attribute_name);
            diagnostic.Error(element, std::move(message));
            return {};
        }
        return std::get<sdf::ElementPtr>(data_element)->Get<std::string>(attribute_name);
    };
    auto get_bool_attribute = [](const ElementNode& data_element, const char* attribute_name) {
        return std::get<sdf::ElementPtr>(data_element)->Get<bool>(attribute_name);
    };
    auto read_tag_string = [&diagnostic](const ElementNode& data_element, const char*) -> std::string {
        auto element = std::get<sdf::ElementPtr>(data_element);
        sdf::ParamPtr param = element->GetValue();
        if (param == nullptr) {
            std::string message = fmt::format("The tag <{}> is missing a required string value.", element->GetName());
            diagnostic.Error(element, std::move(message));
            return {};
        }
        return param->GetAsString();
    };
    ParseCollisionFilterGroupCommon(diagnostic.MakePolicyForNode(*(model.Element())), model_instance, model.Element(),
                                    plant, resolver, next_child_element, next_sibling_element, has_attribute,
                                    get_string_attribute, get_bool_attribute, read_tag_string);
}

bool CanReuseModelInstance(const ModelInstanceIndex& model_instance,
...
ModelInstanceIndex AddModelInstanceIfReusable(MultibodyPlant<double>* plant,
    ...
    if (plant->HasModelInstanceNamed(model_name)) {
        ...
        if (!CanReuseModelInstance(model_instance, reusable_model_instance_range)) {
            throw std::logic_error("This model already contains a model instance named '" + model_name +
...
public:
    explicit InterfaceModelHelper(const MultibodyPlant<double>& pt) : plant_(pt) {}

    std::vector<ModelInstanceIndex> GetChildModelInstanceIndices() const {
        std::vector<ModelInstanceIndex> output;

        for (ModelInstanceIndex mi(model_instance_ + 1); mi < plant_.num_model_instances(); ++mi) {
            if (multibody::ScopedName::Parse(plant_.GetModelInstanceName(mi)).get_element() == name_) {
                output.push_back(mi);
            }
        }
        return output;
    ...
    template <typename T>
    static std::vector<T> GetVectorDiff(const std::vector<T>& current, const std::vector<T>& previous) {
        std::vector<T> output;
    ...
    void ComputeDiffFromSnapshot() {
        ...
        body_indices_ = GetVectorDiff(plant_.GetBodyIndices(model_instance_), body_indices_);
        frame_indices_ = GetVectorDiff(plant_.GetFrameIndices(model_instance_), frame_indices_);
        joint_indices_ = GetVectorDiff(plant_.GetJointIndices(model_instance_), joint_indices_);
        model_instance_indices_ = GetVectorDiff(GetChildModelInstanceIndices(), model_instance_indices_);
    }

    ModelInstanceIndex model_instance() const { return model_instance_; }

    const std::vector<JointIndex>& joint_indices() const { return joint_indices_; }
    const std::vector<FrameIndex>& frame_indices() const { return frame_indices_; }
    ...
    void set_model_frame_name(const std::string& model_frame_name) { model_frame_name_ = model_frame_name; }
...
std::vector<ModelInstanceIndex> AddModelsFromSpecification(
        ...
        const ModelInstanceIndexRange& reusable_model_instance_range) {
    const ModelInstanceIndex model_instance =
            AddModelInstanceIfReusable(plant, model_name, reusable_model_instance_range);

    const std::set<std::string> supported_model_elements{"drake:joint",
    ...
    // frame.
    const RigidTransformd X_PM = ResolveRigidTransform(diagnostic, model.SemanticPose());
    ...
    for (uint64_t model_index = 0; model_index < model.ModelCount(); ++model_index) {
        const sdf::Model& nested_model = *model.ModelByIndex(model_index);
        std::vector<ModelInstanceIndex> nested_model_instances =
                AddModelsFromSpecification(diagnostic, nested_model, sdf::JoinName(model_name, nested_model.Name()),
                                           X_WM, plant, resolver, package_map, root_dir, reusable_model_instance_range);

        added_model_instances.insert(added_model_instances.end(), nested_model_instances.begin(),
                                     nested_model_instances.end());
    }
    drake::log()->trace("sdf_parser: Add links");
    std::optional<std::vector<LinkInfo>> added_link_infos =
            AddLinksFromSpecification(diagnostic, model_instance, model, X_WM, plant, package_map, root_dir);
    ...
    const Frame<double>& model_frame = [&]() -> const Frame<double>& {
        const auto [canonical_link, canonical_link_name] = model.CanonicalLinkAndRelativeName();

        if (canonical_link != nullptr) {
            const auto [parent_model_instance, local_name] =
                    GetResolvedModelInstanceAndLocalName(canonical_link_name, model_instance, *plant);
            const Frame<double>& canonical_link_frame = plant->GetFrameByName(local_name, parent_model_instance);
            const RigidTransformd X_LcM = ResolveRigidTransform(diagnostic, model.SemanticPose(),
    ...
    for (uint64_t joint_index = 0; joint_index < model.JointCount(); ++joint_index) {
        // Get a pointer to the SDF joint, and the joint axis information.
        const sdf::Joint& joint = *model.JointByIndex(joint_index);
        if (!AddJointFromSpecification(diagnostic, X_WM, joint, model_instance, plant, &joint_types)) {
            return {};
        }
    }
    ...
    for (uint64_t frame_index = 0; frame_index < model.FrameCount(); ++frame_index) {
        const sdf::Frame& frame = *model.FrameByIndex(frame_index);
        AddFrameFromSpecification(diagnostic, frame, model_instance, model_frame, plant);
    }

    drake::log()->trace("sdf_parser: Add drake custom joints");
    if (model.Element()->HasElement("drake:joint")) {
        for (sdf::ElementPtr joint_node = model.Element()->GetElement("drake:joint"); joint_node;
             joint_node = joint_node->GetNextElement("drake:joint")) {
            if (!AddDrakeJointFromSpecification(diagnostic, joint_node, model_instance, plant)) {
                return {};
            }
        }
    }
    ...
    if (model.Element()->HasElement("drake:linear_bushing_rpy")) {
        for (sdf::ElementPtr bushing_node = model.Element()->GetElement("drake:linear_bushing_rpy"); bushing_node;
             bushing_node = bushing_node->GetNextElement("drake:linear_bushing_rpy")) {
            if (AddBushingFromSpecification(diagnostic, bushing_node, model_instance, plant) == nullptr) {
                return {};
            }
        }
    }
    ...
    if (model.Element()->HasElement("drake:ball_constraint")) {
        for (sdf::ElementPtr constraint_node = model.Element()->GetElement("drake:ball_constraint"); constraint_node;
             constraint_node = constraint_node->GetNextElement("drake:ball_constraint")) {
            AddBallConstraintFromSpecification(diagnostic, constraint_node, model_instance, plant);
        }
    }
    ...
    if (model.Static()) {
        ...
        for (sdf::JointType joint_type : joint_types) {
            if (joint_type != sdf::JointType::FIXED) {
                std::string message = "Only fixed joints are permitted in static models.";
        ...
        for (const LinkInfo& link_info : *added_link_infos) {
            if (!AreWelded(*plant, plant->world_body(), *link_info.body)) {
                ...
                const std::string joint_name = "sdformat_model_static_" + A.name() + "_welds_to_" + B.name();
                plant->AddJoint(std::make_unique<WeldJoint<double>>(joint_name, A, B, link_info.X_WL));
            }
        }
    }
    ...
    if (plant->geometry_source_is_registered()) {
        ...
        ParseCollisionFilterGroup(diagnostic, model_instance, model, plant, resolver);
    }
...
RigidTransformd GetDefaultFramePose(const MultibodyPlant<double>& plant, const Frame<double>& frame) {
...
void AddFramesToInterfaceModel(const MultibodyPlant<double>& plant,
    ...
    for (auto index : frame_indices) {
        ...
        interface_model->AddFrame({frame.name(), GetRelativeBodyName(frame.body(), model_instance, plant),
...
void AddJointsToInterfaceModel(const MultibodyPlant<double>& plant,
    ...
    for (auto index : joint_indices) {
        ...
        const RigidTransformd X_CJ = joint.frame_on_child().GetFixedPoseInBodyFrame();
        interface_model->AddJoint({joint.name(), child_name, ToIgnitionPose3d(X_CJ)});
    }
}
...
ModelInstanceIndex GetOrCreateModelInstanceByName(MultibodyPlant<double>* plant, const std::string& model_name) {
    if (plant->HasModelInstanceNamed(model_name)) {
...
sdf::InterfaceModelPtr ConvertToInterfaceModel(MultibodyPlant<double>* plant,
    ...
    sdf::RepostureFunction reposture_model = [plant, model_instance,
        ...
        for (auto interface_link_ind : plant->GetBodyIndices(model_instance)) {
            ...
            gz::math::Pose3d X_WL;
            sdf::Errors inner_errors = graph.ResolveNestedFramePose(X_WL, interface_link.name());
    ...
    const std::string canonical_link_name = GetRelativeBodyName(model_frame.body(), model_instance, *plant);
    const RigidTransformd X_WM = GetDefaultFramePose(*plant, model_frame);
    const RigidTransformd X_PM = X_WP.inverse() * X_WM;

    auto interface_model = std::make_shared<sdf::InterfaceModel>(model_name, reposture_model, false,
                                                                 canonical_link_name, ToIgnitionPose3d(X_PM));

    AddBodiesToInterfaceModel(*plant, interface_model, interface_model_helper.body_indices(), X_WM.inverse());

    AddFramesToInterfaceModel(*plant, model_instance, interface_model, interface_model_helper.frame_indices(),
                              interface_model_helper.model_frame_name());

    AddJointsToInterfaceModel(*plant, interface_model, interface_model_helper.joint_indices());
...
sdf::InterfaceModelPtr ParseNestedInterfaceModel(const ParsingWorkspace& workspace,
                                                 const sdf::NestedInclude& include,
                                                 sdf::Errors* errors) {
    const sdf::ParserConfig parser_config = MakeSdfParserConfig(workspace);
    auto& [options, package_map, diagnostic, plant, collision_resolver, parser_selector] = workspace;
    ...
    ParsingWorkspace subworkspace{options, package_map, subdiagnostic, plant, collision_resolver, parser_selector};
    ...
    if (is_merge_include) {
        ...
        const auto parent_model_instance = GetOrCreateModelInstanceByName(plant, include.AbsoluteParentName());

        interface_model_helper.TakeSnapShot(parent_model_instance);
        auto& parser = parser_selector(diagnostic, resolved_filename);
        model_name = parser.MergeModel(data_source, include.LocalModelName().value_or(""), parent_model_instance,
                                       subworkspace);
        ...
        const std::optional<ModelInstanceIndex> maybe_model =
                parser_selector(diagnostic, resolved_filename)
                        .AddModel(data_source, include.LocalModelName().value_or(""), include.AbsoluteParentName(),
                                  subworkspace);
        ...
        model_name = ScopedName::Parse(plant->GetModelInstanceName(main_model_instance)).get_element();
    ...
    if (body_indices.empty()) {
        errors->emplace_back(sdf::ErrorCode::ELEMENT_INVALID, "Model must have at least one link.");
        return nullptr;
    }
    const auto& canonical_link = workspace.plant->get_body(body_indices[0]);

    const Frame<double>& canonical_link_frame = plant->GetFrameByName(canonical_link.name(), main_model_instance);
    plant->AddFrame(std::make_unique<FixedOffsetFrame<double>>(model_frame_name, canonical_link_frame,
                                                               RigidTransformd::Identity(), main_model_instance));
    ...
    main_interface_model = ConvertToInterfaceModel(plant, model_name, interface_model_helper, errors);
...
sdf::ParserConfig MakeSdfParserConfig(const ParsingWorkspace& workspace) {
    ...
    parser_config.RegisterCustomModelParser([&workspace](const sdf::NestedInclude& include, sdf::Errors& errors) {
...
std::optional<ModelInstanceIndex> AddModelFromSdf(const DataSource& data_source,
    ...
    const auto model_index_begin = static_cast<ModelInstanceIndex>(workspace.plant->num_model_instances());
    sdf::Errors errors = LoadSdf(diagnostic, &root, data_source, parser_config);
    if (diagnostic.PropagateErrors(errors)) {
        return std::nullopt;
    }
    const auto model_index_end = static_cast<ModelInstanceIndex>(workspace.plant->num_model_instances());

    ModelInstanceIndexRange reusable_model_instance_range = std::make_pair(model_index_begin, model_index_end);
    ...
    const std::string local_model_name = model_name_in.empty() ? model.Name() : model_name_in;

    std::string model_name = MakeModelName(local_model_name, parent_model_name, workspace);

    std::vector<ModelInstanceIndex> added_model_instances =
            AddModelsFromSpecification(diagnostic, model, model_name, {}, workspace.plant, workspace.collision_resolver,
                                       workspace.package_map, data_source.GetRootDir(), reusable_model_instance_range);
...
#pragma GCC diagnostic pop
    ...
    if (model_count > 0) {
        ...
        std::string model_name = MakeModelName(model.Name(), parent_model_name, workspace);

        std::vector<ModelInstanceIndex> added_model_instances = AddModelsFromSpecification(
                diagnostic, model, model_name, {}, workspace.plant, workspace.collision_resolver, workspace.package_map,
        ...
        const std::set<std::string> supported_world_elements{"frame", "include", "joint", "model"};
        CheckSupportedElements(diagnostic, world.Element(), supported_world_elements);
        ...
        for (uint64_t model_index = 0; model_index < world.ModelCount(); ++model_index) {
            // Get the model.
            const sdf::Model& model = *world.ModelByIndex(model_index);
            std::string model_name = MakeModelName(model.Name(), parent_model_name, workspace);

            std::vector<ModelInstanceIndex> added_model_instances = AddModelsFromSpecification(
                    diagnostic, model, model_name, {}, workspace.plant, workspace.collision_resolver,
                    workspace.package_map, data_source.GetRootDir(), reusable_model_instance_range);
            model_instances.insert(model_instances.end(), added_model_instances.begin(), added_model_instances.end());
        }

        for (uint64_t frame_index = 0; frame_index < world.FrameCount(); ++frame_index) {
            const sdf::Frame& frame = *world.FrameByIndex(frame_index);
            AddFrameFromSpecification(diagnostic, frame, world_model_instance(), workspace.plant->world_frame(),
                                      workspace.plant);
        }

        // Add all the joints
        std::set<sdf::JointType> joint_types;
        for (uint64_t joint_index = 0; joint_index < world.JointCount(); ++joint_index) {
            const sdf::Joint& joint = *world.JointByIndex(joint_index);
            if (!AddJointFromSpecification(diagnostic, {}, joint, world_model_instance(), workspace.plant, &joint_types,
                                           false)) {
                return {};
            }
        }
        ...
        for (uint64_t joint_index = 0; joint_index < world.JointCount(); ++joint_index) {
            // Get a pointer to the SDF joint, and the joint axis information.
            const sdf::Joint& joint = *world.JointByIndex(joint_index);
            if (!ParseMimicTag(diagnostic, joint, world_model_instance(), workspace.plant)) {
                return {};
            }
        }
        ...
        for (sdf::JointType joint_type : joint_types) {
            if (joint_type != sdf::JointType::FIXED) {
                std::string message = "Only fixed joints are permitted in world joints.";
...
std::optional<ModelInstanceIndex> SdfParserWrapper::AddModel(const DataSource& data_source,
                                                             const std::string& model_name,
                                                             const std::optional<std::string>& parent_model_name,
                                                             const ParsingWorkspace& workspace) {
...
std::vector<ModelInstanceIndex> SdfParserWrapper::AddAllModels(const DataSource& data_source,
                                                               const std::optional<std::string>& parent_model_name,
                                                               const ParsingWorkspace& workspace) {
