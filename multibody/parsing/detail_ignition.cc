...
RigidTransformd ToRigidTransform(const gz::math::Pose3d& pose) {
    const Quaternion<double> rotation(pose.Rot().W(), pose.Rot().X(), pose.Rot().Y(), pose.Rot().Z());
...
gz::math::Pose3d ToIgnitionPose3d(const RigidTransformd& pose) {
    ...
    return gz::math::Pose3d(gz::math::Vector3d(pose.translation().x(), pose.translation().y(), pose.translation().z()),
