...
public:
    ...
    std::optional<ModelInstanceIndex> AddModelFromUrdfFile(const std::string& file_name,
                                                           const std::string& model_name) {
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = AddModelFromUrdf({DataSource::kFilename, &file_name}, model_name, {}, w);
        last_parsed_groups_ = ConvertInstancedNamesToStrings(resolver.Resolve(diagnostic_policy_), plant_);
        return result;
    }

    std::optional<ModelInstanceIndex> AddModelFromUrdfString(const std::string& file_contents,
                                                             const std::string& model_name) {
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = AddModelFromUrdf({DataSource::kContents, &file_contents}, model_name, {}, w);
        last_parsed_groups_ = ConvertInstancedNamesToStrings(resolver.Resolve(diagnostic_policy_), plant_);
        return result;
    }
...
TEST_F(UrdfParserTest, BadFilename) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex("/.*/nonexistent.urdf:0: error: "
...
TEST_F(UrdfParserTest, NoRobot) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*URDF does not contain a robot tag."));
}

TEST_F(UrdfParserTest, NoName) {
    EXPECT_EQ(AddModelFromUrdfString("<robot/>", ""), std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Your robot must have a name attribute or a model name must"
                                          " be specified."));
}
...
TEST_F(UrdfParserTest, ObsoleteLoopJoint) {
    EXPECT_NE(AddModelFromUrdfString("<robot name='a'><loop_joint/></robot>", ""), std::nullopt);
...
TEST_F(UrdfParserTest, BodyNameBroken) {
    ...
    <robot name='a'>
      <link naQQQme='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*link tag is missing name attribute."));
}
...
TEST_F(UrdfParserTest, JointNameBroken) {
    ...
    <robot name='a'>
      <joint naQQQme='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint tag is missing name attribute"));
}
...
TEST_F(UrdfParserTest, JointTypeBroken) {
    ...
    <robot name='a'>
      <joint name='a' tyQQQpe='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint 'a' is missing type attribute"));
}
    ...
    <robot name='a'>
      <joint name='a' type='revolute'>
        ...
        <child link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint 'a' doesn't have a parent node!"));
}
    ...
    <robot name='a'>
      ...
      <joint name='a' type='revolute'>
        ...
        <child link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint a's parent does not have a link attribute!"));
}
    ...
    <robot name='a'>
      <joint name='a' type='revolute'>
        ...
        <chiQQQld link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint 'a' doesn't have a child node!"));
}
    ...
    <robot name='a'>
      ...
      <joint name='a' type='revolute'>
        ...
        <child liQQQnk='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*joint a's child does not have a link attribute!"));
}
...
TEST_F(UrdfParserTest, JointBadDynamicsAttributes) {
    ...
    const auto attrs = std::array<std::string, 3>{"damping", "friction", "coulomb_window"};
    for (const auto& attr : attrs) {
        EXPECT_NE(AddModelFromUrdfString(fmt::format(base, attr + "='1 2'"), attr), std::nullopt);
        EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*" + attr + ".*"));
    }
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='revolute'>
        ...
        <child link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Could not find link named 'parent' with model instance"
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='revolute'>
        ...
        <axis xyz="0 0 0"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint 'joint' axis is zero.  Don't do that."));
}
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='floating'>
        ...
        <child link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*Joint 'joint' specified as type floating which is not"
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='who'>
        ...
        <child link='child'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint 'joint' has unrecognized type: 'who'"));
}
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='revolute'>
        ...
        <mimic/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint 'joint' mimic element is missing the "
    ...
    <robot name='a'>
      ...
      <joint name='joint' type='revolute'>
        ...
        <mimic joint='nonexistent'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint 'joint' mimic element specifies joint "
    ...
    <robot name='a'>
      ...
      <joint name='joint1' type='revolute'>
        ...
        <mimic joint='joint0'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint 'joint1' which has 1 DOF cannot mimic "
    ...
    <robot name='a'>
      ...
      <joint name='joint1' type='fixed'>
        ...
        <mimic joint='joint0'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*Drake only supports the mimic element for "
                                            "single-dof joints.*"));
}
...
TEST_F(UrdfParserTest, FrameLinkBroken) {
    ...
    <robot name="a">
      <frame name="frameA" liQQQnk="broken" rpy="0 0 0" xyz="0 0 0"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*missing link name for frame frameA."));
}
...
TEST_F(UrdfParserTest, TransmissionTypeBroken) {
    ...
    <robot name="a">
      <transmission tyQQQpe='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission element is missing a type."));
}
...
TEST_F(UrdfParserTest, TransmissionActuatorMissing) {
    ...
    <robot name="a">
      <transmission type='SimpleTransmission'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission is missing an actuator element."));
}
    ...
    <robot name="a">
      <transmission type='SimpleTransmission'>
        <actuator naQQQme='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission is missing an actuator name."));
}
    ...
    <robot name="a">
      <transmission type='SimpleTransmission'>
        <actuator name='a'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission is missing a joint element."));
}
    ...
    <robot name="a">
      <transmission type='SimpleTransmission'>
        ...
        <joint naQQQme='broken'/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission is missing a joint name."));
}
...
TEST_F(UrdfParserTest, TransmissionJointBadLimits) {
    ...
    EXPECT_NE(AddModelFromUrdfString(fmt::format(base, "effort='0'"), ""), std::nullopt);
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*Skipping transmission since it's attached to joint \"a\""
                                            " which has a zero effort limit 0.*"));

    EXPECT_NE(AddModelFromUrdfString(fmt::format(base, "effort='-3'"), "b"), std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Transmission specifies joint 'a' which has a negative"
                                          " effort limit."));

    const auto attrs = std::array<std::string, 5>{"lower", "upper", "velocity", "drake:acceleration", "effort"};
    for (const auto& attr : attrs) {
        EXPECT_NE(AddModelFromUrdfString(fmt::format(base, attr + "='1 2'"), attr), std::nullopt);
        EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*" + attr + ".*"));
    }
...
TEST_F(UrdfParserTest, PackageMapSpecified) {
    ...
    const std::string full_urdf_filename =
            FindResourceOrThrow("drake/multibody/parsing/test/box_package/urdfs/box.urdf");
...
TEST_F(UrdfParserTest, DoublePendulum) {
    std::string full_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/double_pendulum.urdf");
    ...
    EXPECT_EQ(frame_on_link1.body().index(), plant_.GetBodyByName("link1").index());

    math::RollPitchYaw<double> rpy_expected(-1, 0.1, 0.2);
    Vector3d xyz_expected(0.8, -0.2, 0.3);
    math::RigidTransform<double> X_BF_expected(rpy_expected.ToRotationMatrix(), xyz_expected);

    EXPECT_TRUE(CompareMatrices(frame_on_link1.GetFixedPoseInBodyFrame().GetAsMatrix4(), X_BF_expected.GetAsMatrix4(),
                                1e-10));
...
TEST_F(UrdfParserTest, TestAtlasMinimalContact) {
    const std::string full_name = FindRunfile("drake_models/atlas/atlas_minimal_contact.urdf").abspath;
    AddModelFromUrdfFile(full_name, "");
    for (int k = 0; k < 30; k++) {
    ...
    const JointActuator<double>& actuator = plant_.GetJointActuatorByName("back_bkz_motor");
    EXPECT_EQ(actuator.effort_limit(), 106);
}

TEST_F(UrdfParserTest, TestAddWithQuaternionFloatingDof) {
    const std::string resource_dir{"drake/multibody/parsing/test/urdf_parser_test/"};
    const std::string model_file = FindResourceOrThrow(resource_dir + "zero_dof_robot.urdf");
...
TEST_F(UrdfParserTest, JointParsingTest) {
    ...
    const RevoluteJoint<double>& revolute_joint = plant_.GetJointByName<RevoluteJoint>("revolute_joint");
    ...
    // Revolute actuator
    const JointActuator<double>& revolute_actuator = plant_.GetJointActuatorByName("revolute_actuator");
    EXPECT_EQ(revolute_actuator.effort_limit(), 100);

    // Prismatic joint
    DRAKE_EXPECT_NO_THROW(plant_.GetJointByName<PrismaticJoint>("prismatic_joint"));
    const PrismaticJoint<double>& prismatic_joint = plant_.GetJointByName<PrismaticJoint>("prismatic_joint");
    ...
    const BallRpyJoint<double>& ball_joint = plant_.GetJointByName<BallRpyJoint>("ball_joint");
    ...
    const Joint<double>& no_limit_joint = plant_.GetJointByName("revolute_joint_no_limits");
    ...
    // Universal joint
    DRAKE_EXPECT_NO_THROW(plant_.GetJointByName<UniversalJoint>("universal_joint"));
    const UniversalJoint<double>& universal_joint = plant_.GetJointByName<UniversalJoint>("universal_joint");
    ...
    const PlanarJoint<double>& planar_joint = plant_.GetJointByName<PlanarJoint>("planar_joint");
    ...
    // Continuous joint
    DRAKE_EXPECT_NO_THROW(plant_.GetJointByName<RevoluteJoint>("continuous_joint"));
    const RevoluteJoint<double>& continuous_joint = plant_.GetJointByName<RevoluteJoint>("continuous_joint");
    ...
    const ScrewJoint<double>& screw_joint = plant_.GetJointByName<ScrewJoint>("screw_joint");
    ...
    DRAKE_EXPECT_NO_THROW(plant_.GetJointByName("revolute_joint_with_mimic_forward_reference"));
    DRAKE_EXPECT_NO_THROW(plant_.GetJointByName("revolute_joint_with_mimic_backward_reference"));
    ...
    const Joint<double>& joint_to_mimic = plant_.GetJointByName("revolute_joint_to_mimic");
    ...
    std::sort(specs.begin(), specs.end(), [](const auto& specA, const auto& specB) {
...
TEST_F(UrdfParserTest, JointParsingTagMismatchTest) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint fixed_joint of type fixed is a standard joint type,"
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Joint ball_joint of type ball is a custom joint"
...
TEST_F(UrdfParserTest, JointParsingTagMissingScrewParametersTest) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*A screw joint is missing the <drake:screw_thread_pitch>"
                                          " tag."));
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*A screw joint has a <drake:screw_thread_pitch> tag"
...
<robot xmlns:xacro="http://ros.org/wiki/xacro" name="joint_parsing_test">
  <link name="world">
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*<inertial> tag is being ignored.*"));

    const auto& inspector = scene_graph_.model_inspector();
    EXPECT_EQ(inspector.num_geometries(), 2);
    EXPECT_EQ(inspector.NumGeometriesForFrame(scene_graph_.world_frame_id()), 2);
    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(scene_graph_.world_frame_id(), geometry::Role::kProximity), 1);
    // This does not total three geometries; the sphere has two roles,
    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(scene_graph_.world_frame_id(), geometry::Role::kIllustration), 1);
    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(scene_graph_.world_frame_id(), geometry::Role::kPerception), 1);
}
...
TEST_F(UrdfParserTest, EntireInertialTagOmitted) {
    ...
    const auto& body = dynamic_cast<const RigidBody<double>&>(plant_.GetBodyByName("entire_inertial_tag_omitted"));
      ...
      <link name='inertia_tag_omitted'>
        <inertial>
          <mass value="2"/>
                                     ...
                                     ""),
              std::nullopt);
    const auto& body = dynamic_cast<const RigidBody<double>&>(plant_.GetBodyByName("inertia_tag_omitted"));
      ...
      <link name='mass_tag_omitted'>
        <inertial>
          <inertia ixx="0" ixy="0" ixz="0" iyy="0" iyz="0" izz="0"/>
                                     ...
                                     ""),
              std::nullopt);
    const auto& body = dynamic_cast<const RigidBody<double>&>(plant_.GetBodyByName("mass_tag_omitted"));
    EXPECT_EQ(body.default_mass(), 0.);
    EXPECT_TRUE(body.default_rotational_inertia().get_moments().isZero());
    EXPECT_TRUE(body.default_rotational_inertia().get_products().isZero());
}
      ...
      <link name='massless_link'>
        <inertial>
          ...
          <inertia ixx="0" ixy="0" ixz="0" iyy="0" iyz="0" izz="0"/>
                                     ...
                                     ""),
              std::nullopt);
    const auto& body = dynamic_cast<const RigidBody<double>&>(plant_.GetBodyByName("massless_link"));
    EXPECT_EQ(body.default_mass(), 0.);
    EXPECT_TRUE(body.default_rotational_inertia().get_moments().isZero());
    EXPECT_TRUE(body.default_rotational_inertia().get_products().isZero());
}
      ...
      <link name='point_mass'>
        <inertial>
          ...
          <inertia ixx="0" ixy="0" ixz="0" iyy="0" iyz="0" izz="0"/>
                                     ...
                                     ""),
              std::nullopt);
    const auto& body = dynamic_cast<const RigidBody<double>&>(plant_.GetBodyByName("point_mass"));
...
TEST_F(UrdfParserTest, BadInertiaFormats) {
    ...
    AddModelFromUrdfString(fmt::format(base, "value='1 2 3'", "ixx='0' ixy='0' ixz='0' iyy='0' iyz='0' izz='0'"), "");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*value.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0 2 3' ixy='0' ixz='0' iyy='0' iyz='0' izz='0'"), "a");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*ixx.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0' ixy='0 2 3' ixz='0' iyy='0' iyz='0' izz='0'"), "b");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*ixy.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0' ixy='0' ixz='0 2 3' iyy='0' iyz='0' izz='0'"), "c");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*ixz.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0' ixy='0' ixz='0' iyy='0 2 3' iyz='0' izz='0'"), "d");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*iyy.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0' ixy='0' ixz='0' iyy='0' iyz='0 2 3' izz='0'"), "e");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*iyz.*"));
    AddModelFromUrdfString(fmt::format(base, "value='1'", "ixx='0' ixy='0' ixz='0' iyy='0' iyz='0' izz='0 2 3'"), "f");
    ...
    <robot name="bushing_test">
      ...
      <drake:linear_bushing_rpy>
        ...
        <drake:bushing_force_damping    value="10 11 12"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to find the <drake:bushing_frameC> tag"));
}
    ...
    <robot name="bushing_test">
      ...
      <drake:linear_bushing_rpy>
        ...
        <drake:bushing_force_damping    value="10 11 12"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to read the 'name' attribute for the"
    ...
    <robot name="bushing_test">
      ...
      <drake:linear_bushing_rpy>
        ...
        <drake:bushing_force_damping    value="10 11 12"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Frame: frameZ specified for <drake:bushing_frameC> does"
    ...
    <robot name="bushing_test">
      ...
      <drake:linear_bushing_rpy>
        ...
        <drake:bushing_force_damping    value="10 11 12"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to find the <drake:bushing_torque_damping> tag"));
}
    ...
    <robot name="bushing_test">
      ...
      <drake:linear_bushing_rpy>
        ...
        <drake:bushing_force_damping    value="10 11 12"/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to read the 'value' attribute for the"
...
public:
    ...
    void VerifyParameters(const std::string& body_A,
                          const std::string& body_B,
                          const Vector3d& p_AP,
                          const Vector3d& p_BQ) {
        std::string text = fmt::format(
                kTestString, fmt::format("<drake:ball_constraint_body_A name=\"{}\"/>", body_A),
                fmt::format("<drake:ball_constraint_body_B name=\"{}\"/>", body_B),
                fmt::format("<drake:ball_constraint_p_AP value=\"{} {} {}\"/>", p_AP.x(), p_AP.y(), p_AP.z()),
                fmt::format("<drake:ball_constraint_p_BQ value=\"{} {} {}\"/>", p_BQ.x(), p_BQ.y(), p_BQ.z()));
    ...
    void ProvokeError(const std::optional<const std::string>& body_A,
        ...
        std::string text = fmt::format(
                kTestString,
                body_A.has_value() ? fmt::format("<drake:ball_constraint_body_A name=\"{}\"/>", body_A.value()) : "",
                body_B.has_value() ? fmt::format("<drake:ball_constraint_body_B name=\"{}\"/>", body_B.value()) : "",
                p_AP.has_value() ? fmt::format("<drake:ball_constraint_p_AP value=\"{} {} {}\"/>", p_AP.value().x(),
                                               p_AP.value().y(), p_AP.value().z())
                                 : "",
                p_BQ.has_value() ? fmt::format("<drake:ball_constraint_p_BQ value=\"{} {} {}\"/>", p_BQ.value().x(),
                                               p_BQ.value().y(), p_BQ.value().z())
                                 : "");
        EXPECT_NE(AddModelFromUrdfString(text, ""), std::nullopt);
...
TEST_F(BallConstraintTest, Missing_p_AP) {
    ProvokeError("A", "B", {}, Vector3d(4, 5, 6), ".*Unable to find the <drake:ball_constraint_p_AP> tag");
}

TEST_F(BallConstraintTest, Missing_p_BQ) {
    ProvokeError("A", "B", Vector3d(1, 2, 3), {}, ".*Unable to find the <drake:ball_constraint_p_BQ> tag");
}

TEST_F(BallConstraintTest, InvalidBody) {
    ProvokeError("INVALID", "B", Vector3d(1, 2, 3), Vector3d(4, 5, 6),
...
public:
    void VerifyParameters(const std::string& rotor_inertia_text,
                          ...
                          double gear_ratio) {
        std::string text = fmt::format(kTestString, rotor_inertia_text, gear_ratio_text);
        EXPECT_NE(AddModelFromUrdfString(text, ""), std::nullopt);

        const JointActuator<double>& actuator = plant_.GetJointActuatorByName("revolute_AB");
...
TEST_F(ReflectedInertiaTest, Both) {
    ...
    VerifyParameters("<drake:rotor_inertia value='1.5' />", "<drake:gear_ratio value='300.0' />", 1.5, 300);
}
...
TEST_F(ReflectedInertiaTest, RotorInertiaManyValues) {
    ProvokeError("<drake:rotor_inertia value='1 2 3'/>", "", ".*Expected single value.*value.*");
}
...
TEST_F(ReflectedInertiaTest, GearRatioManyValues) {
    ProvokeError("<drake:gear_ratio value='1 2 3'/>", "", ".*Expected single value.*value.*");
}

class ControllerGainsTest : public UrdfParserTest {
public:
    void VerifyParameters(const std::string& controller_gains_text, double expected_p, double expected_d) {
    ...
    void ProvokeError(const std::string& controller_gains_text, const std::string& error_pattern) {
        std::string text = fmt::format(kTestString, controller_gains_text);
        EXPECT_NE(AddModelFromUrdfString(text, ""), std::nullopt);
        EXPECT_THAT(TakeError(), MatchesRegex(error_pattern));
    }
...
TEST_F(ControllerGainsTest, MissingP) {
    ProvokeError("<drake:controller_gains d='100'/>",
                 ".*joint actuator revolute_AB's drake:controller_gains does not have a"
                 " \'p\' attribute!");
}

TEST_F(ControllerGainsTest, MissingD) {
    ProvokeError("<drake:controller_gains p='10000'/>",
                 ".*joint actuator revolute_AB's drake:controller_gains does not have a"
                 " \'d\' attribute!");
}

TEST_F(ControllerGainsTest, PTooManyValues) {
    ProvokeError("<drake:controller_gains p='1 2 3' d='100'/>", ".*Expected single value for attribute 'p'.*");
}

TEST_F(ControllerGainsTest, DTooManyValues) {
    ProvokeError("<drake:controller_gains p='10000' d='1 2 3'/>", ".*Expected single value for attribute 'd'.*");
}
...
TEST_F(UrdfParserTest, CollisionFilterGroupParsingTest) {
    ...
    const geometry::SceneGraphInspector<double>& inspector = scene_graph_.model_inspector();
    ...
    for (int k = 1; k <= kNumLinks; ++k) {
        const auto geometry_id = inspector.GetGeometryIdByName(
                plant_.GetBodyFrameIdOrThrow(plant_.GetBodyByName(fmt::format("link{}", k)).index()),
                geometry::Role::kProximity, fmt::format("collision_filter_group_parsing_test::link{}_sphere", k));
        ids[k] = geometry_id;
    }
    ...
    <robot name='robot'>
      <link name='a'/>
        <drake:collision_filter_group/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*The tag <drake:collision_filter_group> does not specify"
    ...
    <robot name='robot'>
      ...
      <drake:collision_filter_group name="group_a">
        <drake:member/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*The tag <drake:member> does not specify the required "
                                          "attribute \"link\"."));
    ...
    <robot name='robot'>
      ...
      <drake:collision_filter_group name="group_a">
        <drake:member_group/>
                                     ...
                                     ""),
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*The tag <drake:member_group> does not specify the"
    ...
    <robot name='robot'>
      ...
      <drake:collision_filter_group name="group_a">
        <drake:ignored_collision_filter_group/>
              ...
              std::nullopt);
    EXPECT_THAT(TakeError(), MatchesRegex(".*'robot::group_a'.*no members"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The tag <drake:ignored_collision_filter_group> does not"
    ...
    <robot>
      ...
      <joint name='a' type='revolute'>
        ...
        <{}/>
                                         ...
                                         tag),
                  std::nullopt);
        EXPECT_THAT(TakeWarning(), MatchesRegex(fmt::format(".*{}.*ignored.*", tag)));
    }
}
...
TEST_F(UrdfParserTest, UnsupportedMechanicalReductionIgnoredMaybe) {
    ...
    for (const Case& acase : cases) {
        // Within actuator.
        EXPECT_NE(AddModelFromUrdfString(fmt::format(robot_template, acase.input, ""), acase.input + "_actuator"),
                  std::nullopt);
        if (acase.provokes_warning) {
            EXPECT_THAT(TakeWarning(), MatchesRegex(pattern));
        }
        // Within transmission.
        EXPECT_NE(AddModelFromUrdfString(fmt::format(robot_template, "", acase.input), acase.input + "_transmission"),
                  std::nullopt);
        if (acase.provokes_warning) {
            EXPECT_THAT(TakeWarning(), MatchesRegex(pattern));
        }
...
TEST_F(UrdfParserTest, PlanarJointAxisRespected) {
    ...
    EXPECT_NE(AddModelFromUrdfString(model, ""), std::nullopt);
    plant_.Finalize();
    plant_.GetMutableJointByName<PlanarJoint>("planar_joint").set_default_translation(Vector2<double>(1.2, 3.4));
    auto context = plant_.CreateDefaultContext();
    const math::RigidTransform<double>& X_WB = plant_.EvalBodyPoseInWorld(*context, plant_.GetBodyByName("link2"));
    const Vector3d p_WB_F(1.2, 3.4, 0.0);
    // The rotation that aligns +Mz with +By is the right-handed rotation around
    // Bx by 270 degrees.
    const math::RotationMatrixd R_BM = math::RotationMatrixd::MakeXRotation(3 * M_PI_2);
    // R_WB and R_MF are both identities.
    const math::RotationMatrixd R_WF = R_BM;
    const Vector3d p_WB = R_WF * p_WB_F;
    EXPECT_TRUE(CompareMatrices(X_WB.translation(), p_WB, 4.0 * std::numeric_limits<double>::epsilon()));
}
