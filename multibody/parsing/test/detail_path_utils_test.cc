...
std::string ResolveGoodUri(const std::string& uri, const PackageMap& package_map, const std::string& root_dir) {
...
// for bad URIs.
std::string ResolveBadUri(const std::string& uri, const PackageMap& package_map, const std::string& root_dir) {
    DiagnosticPolicy diagnostic;
...
GTEST_TEST(ResolveUriUncheckedTest, NormalizedPath) {
    ...
    const string target_file = FindResourceOrThrow(
            ...
            "sdf/test_model.sdf");
    const string root_dir = std::filesystem::path(target_file).parent_path().string();

    // Case: Simple concatenation would produce /fake/root/./file.txt.
    EXPECT_EQ(ResolveGoodUri("./test_model.sdf", package_map, root_dir), target_file);

    // Case: Moving up one directory.
    const std::string fake_root = root_dir + "/fake";
    EXPECT_EQ(ResolveGoodUri("../test_model.sdf", package_map, fake_root), target_file);
...
GTEST_TEST(ResolveUriTest, TestNoRoot) {
    ...
    const std::string root_dir;
    EXPECT_THAT(ResolveBadUri(rel_path, package_map, root_dir),
                ::testing::MatchesRegex(".*URI 'multibody/parsing/.*invalid when parsing a string.*"));
}
...
GTEST_TEST(ResolveUriTest, TestModel) {
    ...
    // Create the URI.
    const string uri_model = "model://package_map_test_package_a/sdf/test_model.sdf";
    EXPECT_EQ(ResolveGoodUri(uri_model, package_map, root_dir), sdf_file_name);

    // Create another URI using "package":
    const string uri_package = "package://package_map_test_package_a/sdf/test_model.sdf";
...
GTEST_TEST(ResolveUriTest, TestUnsupported) {
    ...
    EXPECT_THAT(ResolveBadUri(uri, package_map, root_dir), ::testing::MatchesRegex(".*unsupported scheme.*"));
}
...
GTEST_TEST(ResolveUriTest, DeprecatedPackage) {
    ...
    std::string result = ResolveUri(diagnostic, "package://foo/multibody", package_map, "");
    EXPECT_EQ(result, "multibody");
    EXPECT_THAT(warning.message, ::testing::MatchesRegex(".*package://foo/multibody.*is deprecated.*Stop using foo.*"));

    // Check that we get a basic warning.
    warning = {};
    result = ResolveUri(diagnostic, "package://bar/multibody", package_map, "");
    EXPECT_EQ(result, "multibody");
    EXPECT_THAT(warning.message, ::testing::MatchesRegex(".*package://bar/multibody.*is deprecated.*"));
}
