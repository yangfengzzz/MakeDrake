...
TEST_F(TinyXml2DiagnosticFilenameTest, Error) {
    diagnostic_.Error(GetFirstChildNamed("error"), "badness");
    EXPECT_THAT(TakeError(), testing::MatchesRegex("/.*/test_data.stuff:4: error: badness"));
}

TEST_F(TinyXml2DiagnosticFilenameTest, Warning) {
    diagnostic_.Warning(GetFirstChildNamed("warning"), "regret");
    EXPECT_THAT(TakeWarning(), testing::MatchesRegex("/.*/test_data.stuff:5: warning: regret"));
}
...
TEST_F(TinyXml2DiagnosticContentsTest, PolicyForNode) {
    ...
    auto error_policy = diagnostic_.MakePolicyForNode(&GetFirstChildNamed("error"));
    error_policy.Error("bad");
    EXPECT_EQ(TakeError(), "<literal-string>.stuff:4: error: bad");

    auto warning_policy = diagnostic_.MakePolicyForNode(&GetFirstChildNamed("warning"));
...
TEST_F(TinyXml2DiagnosticContentsTest, Unsuppported) {
    diagnostic_.WarnUnsupportedElement(GetFirstChildNamed("warning"), "unsupported_element");
    ...
    diagnostic_.WarnUnsupportedAttribute(GetFirstChildNamed("warning"), "unsupported_attribute");
    EXPECT_EQ(TakeWarning(),
