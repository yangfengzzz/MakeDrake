...
protected:
    ...
    fs::path PrepopulateCache(std::string_view package_name, std::string_view sha256, std::string_view strip_prefix) {
        const auto try_cache = drake::internal::FindOrCreateCache("package_map");
        DRAKE_DEMAND(try_cache.error.empty());
        const fs::path package_dir =
                try_cache.abspath / fmt::format("{}-{}", sha256, Sha256::Checksum(strip_prefix).to_string());
...
RemoteParams MakeGoodParams() {
    RemoteParams params;
    params.urls.push_back(fmt::format("file://{}", FindResourceOrThrow("drake/multibody/parsing/test/"
                                                                       "package_map_test_packages/compressed.zip")));
    params.sha256 = "b4bdbad313293ca61fe8f4ed1b5579dadadb3a5c08f0a6d06a8e39e5f97f1bd1";
...
TEST_F(PackageMapRemoteTest, GetPathEnvironmentDenied) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddRemote("bad", params), ".*DRAKE_ALLOW_NETWORK.*");
}
...
TEST_F(PackageMapRemoteTest, RejectDuplicates) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddRemote("foo", MakeGoodParams()), ".*existing path is local.*");
}
...
TEST_F(PackageMapRemoteTest, InputSanitization) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddRemote("foo", RemoteParams{}), ".*at least one URL.*");

    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddRemote("foo",
                                              RemoteParams{
...
TEST_F(PackageMapRemoteTest, CouldNotFetch) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.GetPath(package_name), ".*downloader.*error[^]*Checksum[^]*");
...
TEST_F(PackageMapRemoteTest, AddMapMergeFetched) {
    ...
    EXPECT_THAT(fmt::to_string(fmt_streamed(dut)), testing::HasSubstr("package_map_test_packages/compressed.zip"));
}
...
TEST_F(PackageMapRemoteTest, DrakeModelsDefaults) {
    ...
    EXPECT_THAT(params.strip_prefix, testing::Optional(testing::StartsWith("models-")));
}
