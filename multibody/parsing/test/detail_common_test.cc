...
TEST_F(DataSourceTest, GetAbsolutePath) {
    EXPECT_THAT(relative_.GetAbsolutePath(), ::testing::MatchesRegex("/.*/relative.txt"));
...
template <typename T>
::testing::AssertionResult ExpectScalar(const char* group,
                                        const char* property,
                                        T expected,
    ...
    if (!has_value) {
        return failure << "Expected (" << group << ", " << property << "); not found";
    }
...
public:
    ...
    geometry::ProximityProperties ParseProximityProperties(
            const std::function<std::optional<double>(const char*)>& read_double, bool is_rigid, bool is_compliant) {
        return internal::ParseProximityProperties(diagnostic_, read_double, is_rigid, is_compliant);
    }
...
TEST_F(ParseProximityPropertiesTest, NoProperties) {
    ProximityProperties properties = ParseProximityProperties(empty_read_double, !rigid, !compliant);
    ...
    ASSERT_EQ(properties.GetPropertiesInGroup(GeometryProperties::default_group_name()).size(), 0u);
}
...
TEST_F(ParseProximityPropertiesTest, HydroelasticProperties) {
    ...
    auto expect_compliance = [](bool is_rigid, bool is_compliant,
                                const ProximityProperties& p) -> ::testing::AssertionResult {
        DRAKE_DEMAND(!(is_rigid && is_compliant));
        ::testing::AssertionResult failure = ::testing::AssertionFailure();
        const bool has_compliance_type = p.HasProperty(kHydroGroup, kComplianceType);
        if (is_rigid || is_compliant) {
            if (!has_compliance_type) {
                return failure << "Expected compliance; found none";
            }
            auto compliance = p.GetProperty<HydroelasticType>(kHydroGroup, kComplianceType);
    ...
    // Case: Declared rigid with a resolution hint.
    {
        ProximityProperties properties =
                ParseProximityProperties(param_read_double(kTag, kRezHintValue), rigid, !compliant);
        EXPECT_TRUE(expect_compliance(rigid, !compliant, properties));
...
TEST_F(ParseProximityPropertiesTest, HydroelasticModulus) {
    ...
    ProximityProperties properties =
            ParseProximityProperties(param_read_double("drake:hydroelastic_modulus", kValue), !rigid, !compliant);
...
TEST_F(ParseProximityPropertiesTest, RigidHydroelasticModulusIgnored) {
    ...
    ProximityProperties properties =
            ParseProximityProperties(param_read_double("drake:hydroelastic_modulus", kValue), rigid, !compliant);
    EXPECT_THAT(warning.message, ::testing::MatchesRegex(".*hydroelastic_modulus.*value.*1.75.*ignored.*"));
    EXPECT_FALSE(ExpectScalar(kHydroGroup, kElastic, kValue, properties));
    EXPECT_TRUE(ExpectScalar(kHydroGroup, kComplianceType, geometry::internal::HydroelasticType::kRigid, properties));
...
TEST_F(ParseProximityPropertiesTest, Dissipation) {
    ...
    ProximityProperties properties =
            ParseProximityProperties(param_read_double("drake:hunt_crossley_dissipation", kValue), !rigid, !compliant);
...
TEST_F(ParseProximityPropertiesTest, LinearDissipation) {
    const double kValue = 1.25;
    ProximityProperties properties =
            ParseProximityProperties(param_read_double("drake:relaxation_time", kValue), !rigid, !compliant);
    EXPECT_TRUE(ExpectScalar(kMaterialGroup, kRelaxationTime, kValue, properties));
    // Dissipation is the only property.
    EXPECT_EQ(properties.GetPropertiesInGroup(kMaterialGroup).size(), 1u);
    EXPECT_EQ(properties.num_groups(), 2);  // Material and default groups.
}
...
TEST_F(ParseProximityPropertiesTest, Stiffness) {
    ...
    ProximityProperties properties =
            ParseProximityProperties(param_read_double("drake:point_contact_stiffness", kValue), !rigid, !compliant);
    EXPECT_TRUE(ExpectScalar(kMaterialGroup, kPointStiffness, kValue, properties));
...
TEST_F(ParseProximityPropertiesTest, Friction) {
    ...
    auto friction_read_double = [](optional<double> mu_d, optional<double> mu_s) -> ReadDoubleFunc {
    ...
    auto expect_friction = [](double mu_d, double mu_s, const ProximityProperties& p) -> ::testing::AssertionResult {
        ::testing::AssertionResult failure = ::testing::AssertionFailure();
        const bool has_value = p.HasProperty(kMaterialGroup, kFriction);
        if (!has_value) {
            return failure << "Expected (" << kMaterialGroup << ", " << kFriction << "); not found";
        }
        const auto& friction = p.GetProperty<CoulombFriction<double>>(kMaterialGroup, kFriction);
        if (friction.dynamic_friction() != mu_d || friction.static_friction() != mu_s) {
            return failure << "Wrong value for (" << kMaterialGroup << ", " << kFriction
                           << "):" << "\n  Expected mu_d: " << mu_d << ", mu_s: " << mu_s << "\n  Found mu_d "
                           << friction.dynamic_friction() << ", mu_s: " << friction.static_friction();
        }
        return ::testing::AssertionSuccess();
    };
    ...
    {
        const double kValue = 1.25;
        ProximityProperties properties = ParseProximityProperties(friction_read_double(kValue, {}), !rigid, !compliant);
    ...
    // Case: Both defined -- resulting coefficients match.
    {
        ...
        ProximityProperties properties = ParseProximityProperties(friction_read_double(kMuD, kMuS), !rigid, !compliant);
...
TEST_F(ParseSpatialInertiaTest, BadInertiaValues) {
    ...
    results.push_back(ParseSpatialInertia(diagnostic_policy_, X_BBi, 1,
    ...
    results.push_back(ParseSpatialInertia(diagnostic_policy_, X_BBi, 0,
                                          {.ixx = 1, .iyy = 1, .izz = 1, .ixy = 0, .ixz = 0, .iyz = 0}));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*mass > 0.*"));
