#include "multibody/parsing/detail_mesh_parser.h"
...
public:
    ...
    static ParserInterface& TestingSelect(const DiagnosticPolicy&, const std::string&) {
        static never_destroyed<MeshParserWrapper> mesh;
        return mesh.access();
    }

    ModelInstanceIndex AddModelFromMeshFile(const std::string& file_name,
        ...
        std::optional<ModelInstanceIndex> result = wrapper.AddModel(data_source, model_name, parent_model_name, w);
        return result.value_or(ModelInstanceIndex{});
    }

    ModelInstanceIndex AddModelsFromMeshFile(const std::string& file_name,
                                             const std::optional<std::string>& parent_model_name = {}) {
        const DataSource data_source{DataSource::kFilename, &file_name};
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, TestingSelect};
        ...
        MeshParserWrapper wrapper;
        std::vector<ModelInstanceIndex> results = wrapper.AddAllModels(data_source, parent_model_name, w);
...
TEST_F(MeshParserTest, ModelInstanceNameTest) {
    ...
    // library.
    const std::string obj_path = FindResourceOrThrow("drake/multibody/parsing/test/box_package/meshes/box.obj");
    const std::string obj_with_object_path = FindResourceOrThrow("drake/geometry/render/test/meshes/box.obj");

    // We'll add boxes with a various combination of permutations.
    const ModelInstanceIndex filename_model_instance = AddModelFromMeshFile(obj_path, "");

    DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromMeshFile(obj_path, ""),
                                ".+already contains a model instance named 'box'.+");

    const ModelInstanceIndex user_named_model_instance = AddModelFromMeshFile(obj_path, "box1");

    const ModelInstanceIndex prefix_user_model_instance = AddModelFromMeshFile(obj_path, "box2", "prefix");

    const ModelInstanceIndex prefix_file_model_instance = AddModelFromMeshFile(obj_path, "", "prefix");

    const ModelInstanceIndex geom_box_model_instance = AddModelFromMeshFile(obj_with_object_path, "");

    const ModelInstanceIndex user_geom_box_model_instance = AddModelFromMeshFile(obj_with_object_path, "geom_box");

    // This is the token call to the plural form: AddModelsFromMesh.
    const ModelInstanceIndex prefix_geom_box_model_instance = AddModelsFromMeshFile(obj_with_object_path, "prefix");
    ...
    EXPECT_EQ(plant_.GetModelInstanceByName("prefix::box2"), prefix_user_model_instance);
    EXPECT_EQ(plant_.GetModelInstanceByName("prefix::box"), prefix_file_model_instance);
    EXPECT_EQ(plant_.GetModelInstanceByName("test_box"), geom_box_model_instance);
    EXPECT_EQ(plant_.GetModelInstanceByName("geom_box"), user_geom_box_model_instance);
    EXPECT_EQ(plant_.GetModelInstanceByName("prefix::test_box"), prefix_geom_box_model_instance);
...
TEST_F(MeshParserTest, ErrorModes) {
    ...
    // 1. No geometry.
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromMeshFile(file.string(), ""), ".* has no faces.*");
    }
    // 2. Two objects.
    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromMeshFile(file.string(), ""), ".* 2 unique shapes; only 1 allowed.*");
    }
    // 3. Not an OBJ.
    {
        const std::filesystem::path file = temp_dir / "empty.obj";
    ...
    // 4. Called with obj data in a string.
    {
        ...
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, TestingSelect};
        DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromMesh(data_source, "", std::nullopt, w), ".*must be .+ file.*");
    }
...
TEST_F(MeshParserTest, CorrectMass) {
    ...
    const std::string obj_path = FindResourceOrThrow("drake/multibody/parsing/test/box_package/meshes/box.obj");
    const ModelInstanceIndex model_instance = AddModelFromMeshFile(obj_path, "body");
    ...
    const auto I_BBo_B_expected = SpatialInertia<double>::SolidBoxWithDensity(density, 2, 2, 2);
    const SpatialInertia<double> I_BBo_B = body.CalcSpatialInertiaInBodyFrame(*context);
    ...
    EXPECT_TRUE(CompareMatrices(I_BBo_B.CopyToFullMatrix6(), I_BBo_B_expected.CopyToFullMatrix6(), 1e-12));
}
...
TEST_F(MeshParserTest, RegisteredGeometry) {
    ...
    const RigidBody<double>& body = plant_.GetBodyByName("body", model_instance);
    std::optional<geometry::FrameId> frame_id = plant_.GetBodyFrameIdIfExists(body.index());
    ASSERT_TRUE(frame_id.has_value());

    const geometry::SceneGraphInspector<double>& inspector = scene_graph_.model_inspector();

    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(*frame_id, geometry::Role::kIllustration), 1);
    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(*frame_id, geometry::Role::kPerception), 1);
    EXPECT_EQ(inspector.NumGeometriesForFrameWithRole(*frame_id, geometry::Role::kProximity), 1);
}
