...
template <typename T>
std::unordered_set<T> ComponentsNodes(const StronglyConnectedComponents<T>& components) {
    std::unordered_set<T> unique_nodes;
...
template <typename T>
void CheckStronglyConnectedComponentsInvariants(const DirectedGraph<T>& graph,
