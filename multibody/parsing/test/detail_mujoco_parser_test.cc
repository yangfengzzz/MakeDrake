...
#include "common/find_runfiles.h"
...
#include "common/test_utilities/maybe_pause_for_user.h"
#include "common/text_logging.h"
#include "geometry/test_utilities/meshcat_environment.h"
#include "multibody/tree/ball_rpy_joint.h"
#include "multibody/tree/geometry_spatial_inertia.h"
#include "multibody/tree/prismatic_joint.h"
#include "multibody/tree/revolute_joint.h"
#include "multibody/tree/weld_joint.h"
#include "visualization/visualization_config_functions.h"
...
public:
    ...
    std::optional<ModelInstanceIndex> AddModelFromFile(const std::string& file_name, const std::string& model_name) {
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = wrapper_.AddModel({DataSource::kFilename, &file_name}, model_name, {}, w);
        resolver.Resolve(diagnostic_policy_);
        return result;
    }

    std::optional<ModelInstanceIndex> AddModelFromString(const std::string& file_contents,
                                                         const std::string& model_name) {
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = wrapper_.AddModel({DataSource::kContents, &file_contents}, model_name, {}, w);
        resolver.Resolve(diagnostic_policy_);
        return result;
    }
    ...
    std::vector<ModelInstanceIndex> AddAllModelsFromFile(const std::string& file_name,
        ...
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = wrapper_.AddAllModels({DataSource::kFilename, &file_name}, parent_model_name, w);
        resolver.Resolve(diagnostic_policy_);
        return result;
    }

    std::vector<ModelInstanceIndex> AddAllModelsFromString(const std::string& file_contents,
                                                           const std::optional<std::string>& parent_model_name) {
        internal::CollisionFilterGroupResolver resolver{&plant_};
        ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, NoSelect};
        auto result = wrapper_.AddAllModels({DataSource::kContents, &file_contents}, parent_model_name, w);
        resolver.Resolve(diagnostic_policy_);
        return result;
    }

    // Mujoco cannot delegate to any other parsers.
    static ParserInterface& NoSelect(const drake::internal::DiagnosticPolicy&, const std::string&) {
        DRAKE_UNREACHABLE();
    }
...
protected:
    ...
    std::string box_obj_{
            std::filesystem::canonical(FindResourceOrThrow("drake/multibody/parsing/test/box_package/meshes/box.obj"))};
    std::string non_convex_obj_{
            std::filesystem::canonical(FindResourceOrThrow("drake/geometry/test/non_convex_mesh.obj"))};
    std::string box_urdf_{
            std::filesystem::canonical(FindResourceOrThrow("drake/multibody/parsing/test/box_package/urdfs/box.urdf"))};
};

class DeepMindControlTest : public MujocoParserTest, public testing::WithParamInterface<const char*> {};
...
TEST_P(DeepMindControlTest, DeepMindControl) {
    ...
    const std::string filename =
            FindResourceOrThrow(fmt::format("drake/multibody/parsing/dm_control/suite/{}.xml", model));
...
const char* dm_control_models[] = {"acrobot",   "cartpole",     "cheetah", "finger",      "fish",     "hopper",
                                   "humanoid",  "humanoid_CMU", "lqr",     "manipulator", "pendulum", "point_mass",
                                   "quadruped", "reacher",      "stacker", "swimmer",     "walker"};
INSTANTIATE_TEST_SUITE_P(DeepMindControl, DeepMindControlTest, testing::ValuesIn(dm_control_models));

class MujocoMenagerieTest : public MujocoParserTest, public testing::WithParamInterface<const char*> {};

TEST_P(MujocoMenagerieTest, MujocoMenagerie) {
    // Confirm successful parsing of the MuJoCo models in the DeepMind control
    // suite.
    std::string model{GetParam()};
    const RlocationOrError rlocation = FindRunfile(fmt::format("mujoco_menagerie_internal/{}.xml", model));
...
const char* mujoco_menagerie_models[] = {"google_robot/robot", "kuka_iiwa_14/iiwa14", "rethink_robotics_sawyer/sawyer"};
// TODO(russt): Add the remaining models, once they can be parsed correctly, as
// tracked in #20444.

INSTANTIATE_TEST_SUITE_P(MujocoMenagerie, MujocoMenagerieTest, testing::ValuesIn(mujoco_menagerie_models));
...
GTEST_TEST(MujocoParserExtraTest, Visualize) {
    ...
    std::shared_ptr<geometry::Meshcat> meshcat = geometry::GetTestEnvironmentMeshcat();
    ...
    const std::string filename =
            FindResourceOrThrow(fmt::format("drake/multibody/parsing/dm_control/suite/{}.xml", model_name));
...
TEST_F(MujocoParserTest, CartPole) {
    const std::string filename = FindResourceOrThrow("drake/multibody/parsing/dm_control/suite/cartpole.xml");
    ...
    Vector3d p_WP;
    plant_.CalcPointsPositions(*context, plant_.GetFrameByName("pole_1"), Vector3d{0, 0, l}, plant_.world_frame(),
                               &p_WP);
    EXPECT_TRUE(CompareMatrices(p_WP, Vector3d{x + l * sin(theta), 0, z_offset_from_model + l * cos(theta)}, 1e-14));
}

TEST_F(MujocoParserTest, Acrobot) {
    const std::string filename = FindResourceOrThrow("drake/multibody/parsing/dm_control/suite/acrobot.xml");
    ...
    Vector3d p_WP;
    plant_.CalcPointsPositions(*context, plant_.GetFrameByName("lower_arm"), Vector3d{0, 0, l2}, plant_.world_frame(),
                               &p_WP);
    EXPECT_TRUE(CompareMatrices(p_WP,
...
<mujoco model="test">
  <option gravity="0 -9.81 0"/>
    ...
    EXPECT_TRUE(CompareMatrices(plant_.gravity_field().gravity_vector(), Vector3d{0, -9.81, 0}));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    auto CheckShape = [&inspector](const std::string& geometry_name, std::string_view shape_type) {
        GeometryId geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, geometry_name);
        EXPECT_EQ(inspector.GetShape(geom_id).type_name(), shape_type);
        geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kPerception, geometry_name);
        EXPECT_EQ(inspector.GetShape(geom_id).type_name(), shape_type);
        geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kIllustration, geometry_name);
        EXPECT_EQ(inspector.GetShape(geom_id).type_name(), shape_type);
    };
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    AddModelFromString(xml, "test");
    const SceneGraphInspector<double>& inspector = scene_graph_.model_inspector();
    EXPECT_NO_THROW(inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, "geom0"));
    EXPECT_NO_THROW(inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, "geom1"));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    auto CheckPose = [&inspector](const std::string& geometry_name, const RigidTransformd& X_FG) {
        GeometryId geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, geometry_name);
        EXPECT_TRUE(inspector.GetPoseInFrame(geom_id).IsNearlyEqualTo(X_FG, 1e-14)) << geometry_name;
        geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kPerception, geometry_name);
        EXPECT_TRUE(inspector.GetPoseInFrame(geom_id).IsNearlyEqualTo(X_FG, 1e-14)) << geometry_name;
        geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kIllustration, geometry_name);
        EXPECT_TRUE(inspector.GetPoseInFrame(geom_id).IsNearlyEqualTo(X_FG, 1e-14)) << geometry_name;
    ...
    CheckPose("axisangle", RigidTransformd(Eigen::AngleAxis<double>(M_PI / 6.0, Vector3d{4, 5, 6}), p));
    CheckPose("euler", RigidTransformd(RollPitchYawd{M_PI / 6.0, M_PI / 4.0, M_PI / 3.0}, p));
    CheckPose("xyaxes", RigidTransformd(RotationMatrixd::MakeZRotation(M_PI / 2.0), p));
    CheckPose("zaxis", RigidTransformd(RotationMatrixd::MakeXRotation(-M_PI / 2.0), p));
    CheckPose("fromto_capsule", RigidTransformd(RotationMatrixd::MakeXRotation(-M_PI / 2.0), -p));
    CheckPose("fromto_cylinder", RigidTransformd(RotationMatrixd::MakeXRotation(-M_PI / 2.0), -p));
    CheckPose("from_default", RigidTransformd(Eigen::Quaternion<double>{0, 1, 0, 0}, p));

    CheckPose("axisangle_rad", RigidTransformd(Eigen::AngleAxis<double>(0.5, Vector3d{4, 5, 6}), p));
    CheckPose("euler_rad", RigidTransformd(RotationMatrixd::MakeXRotation(0.5) * RotationMatrixd::MakeYRotation(0.7) *
                                                   RotationMatrixd::MakeZRotation(1.05),
                                           p));
    CheckPose("axisangle_deg", RigidTransformd(Eigen::AngleAxis<double>(M_PI / 6.0, Vector3d{4, 5, 6}), p));
    CheckPose("euler_deg",
              RigidTransformd(RotationMatrixd::MakeZRotation(M_PI / 3.0) * RotationMatrixd::MakeYRotation(M_PI / 4.0) *
...
<mujoco model="test">
  <worldbody>
    ...
    AddModelFromString(xml, "test");
    EXPECT_THAT(TakeError(), MatchesRegex(".*has more than one orientation attribute specified.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*has more than one orientation attribute specified.*"));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    auto CheckProperties = [&inspector](const std::string& geometry_name, double mu, const Vector4d& rgba,
                                        bool has_collision = true, bool has_visual = true) {
        if (has_collision) {
            GeometryId geom_id =
                    inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, geometry_name);
            const geometry::ProximityProperties* proximity_prop = inspector.GetProximityProperties(geom_id);
            EXPECT_TRUE(proximity_prop);
            EXPECT_TRUE(proximity_prop->HasProperty("material", "coulomb_friction"));
            const auto& friction = proximity_prop->GetProperty<CoulombFriction<double>>("material", "coulomb_friction");
        ...
        if (has_visual) {
            GeometryId geom_id =
                    inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kPerception, geometry_name);
            const geometry::PerceptionProperties* perception_prop = inspector.GetPerceptionProperties(geom_id);
            EXPECT_TRUE(perception_prop);
            EXPECT_TRUE(perception_prop->HasProperty("phong", "diffuse"));
            EXPECT_TRUE(CompareMatrices(perception_prop->GetProperty<Vector4d>("phong", "diffuse"), rgba));
        } else {
            DRAKE_EXPECT_THROWS_MESSAGE(
                    inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kPerception, geometry_name),
                    ".*has no geometry.*");
        ...
        if (has_visual) {
            GeometryId geom_id =
                    inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kIllustration, geometry_name);
            const geometry::IllustrationProperties* illustration_prop = inspector.GetIllustrationProperties(geom_id);
            EXPECT_TRUE(illustration_prop);
            EXPECT_TRUE(illustration_prop->HasProperty("phong", "diffuse"));
            EXPECT_TRUE(CompareMatrices(illustration_prop->GetProperty<Vector4d>("phong", "diffuse"), rgba));
        } else {
            DRAKE_EXPECT_THROWS_MESSAGE(
                    inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kIllustration, geometry_name),
                    ".*has no geometry.*");
    ...
    CheckProperties("no_collision", 1.0, Vector4d{.5, .5, .5, 1}, false /* has collision */, true /* has visual */);
    CheckProperties("no_visual", 1.0, Vector4d{.5, .5, .5, 1}, true /* has collision */, false /* has visual */);

    // Confirm that the default geometry is a zero-radius sphere.
    GeometryId no_collision_id =
            inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kIllustration, "no_collision");
    const geometry::Sphere* no_collision_shape =
            dynamic_cast<const geometry::Sphere*>(&inspector.GetShape(no_collision_id));
...
TEST_F(MujocoParserTest, Include) {
    ...
    AddAllModelsFromFile(FindResourceOrThrow("drake/multibody/parsing/test/mujoco_parser_test/scene.xml"), {});
    FlushDiagnostics();
    plant_.Finalize();
...
<mujoco model="test">
  ...
  <worldbody>
    <geom name="box_geom" type="mesh" mesh="box"/>
        ...
        const SceneGraphInspector<double>& inspector = scene_graph_.model_inspector();
        GeometryId geom_id = inspector.GetGeometryIdByName(inspector.world_frame_id(), Role::kProximity, "box_geom");
        auto* mesh = dynamic_cast<const geometry::Mesh*>(&inspector.GetShape(geom_id));
...
TEST_F(BoxMeshTest, MeshFileDirect) {
    ...
    std::string mesh_asset = fmt::format(R"""(<mesh name="box" file="{}"/>)""", box_obj_);
    TestBoxMesh(box_obj_, mesh_asset);
}

TEST_F(BoxMeshTest, MeshFileStlOrObj) {
    // Absolute path, referencing an stl with an obj replacement available.
    std::string quad_cube_stl = FindResourceOrThrow("drake/geometry/test/quad_cube.stl");
    std::string quad_cube_obj = std::filesystem::canonical(FindResourceOrThrow("drake/geometry/test/quad_cube.obj"));
    std::string mesh_asset = fmt::format(R"""(<mesh name="box" file="{}"/>)""", quad_cube_stl);
...
TEST_F(BoxMeshTest, MeshFileAbsolutePathCompiler) {
    ...
    std::string compiler = fmt::format(R"""(<compiler assetdir="invalid_name" meshdir="{}"/>)""",
...
TEST_F(BoxMeshTest, MeshFileAbsolutePathCompilerUsingAssetDir) {
    ...
    std::string mesh_asset = R"""(<mesh name="box" file="box.obj"/>)""";
    std::string compiler =
            fmt::format(R"""(<compiler assetdir="{}"/>)""", std::filesystem::path(box_obj_).parent_path().string());
    TestBoxMesh(box_obj_, mesh_asset, compiler);
}
...
TEST_F(BoxMeshTest, MeshFileRelativePathCompiler) {
    ...
    std::string mesh_asset = R"""(<mesh name="box" file="box.obj"/>)""";
    std::string compiler = R"""(<compiler meshdir="multibody/parsing/test/box_package/meshes"/>)""";
    TestBoxMesh(box_obj_, mesh_asset, compiler);
}

TEST_F(BoxMeshTest, MeshFileScale) {
    // Test the scale attribute.
    std::string mesh_asset = fmt::format(R"""(<mesh name="box" file="{}" scale="2 2 2"/>)""", box_obj_);
...
TEST_F(MujocoParserTest, BadMeshSpatialInertiaFallback) {
    ...
    const RlocationOrError rlocation =
            FindRunfile("mujoco_menagerie_internal/hello_robot_stretch/assets/base_link_0.obj");
    ASSERT_EQ(rlocation.error, "");
    ...
    // truly is "bad".
    DRAKE_EXPECT_THROWS_MESSAGE(CalcSpatialInertia(bad_mesh, 1.0), ".*IsPhysicallyValid[\\s\\S]*");
...
TEST_F(MujocoParserTest, MeshFileRelativePathFromFile) {
    const std::string file = FindResourceOrThrow("drake/multibody/parsing/test/box_package/mjcfs/box.xml");
...
<mujoco model="test_false">
  ...
  <worldbody>
    ...
    auto check_body = [this, &context](const std::string& body_name, const UnitInertia<double>& unit_M_BBo_B) {
        SCOPED_TRACE(fmt::format("checking body {}", body_name));
        const RigidBody<double>& body = plant_.GetBodyByName(body_name);
        EXPECT_TRUE(CompareMatrices(body.CalcSpatialInertiaInBodyFrame(*context).CopyToFullMatrix6(),
                                    SpatialInertia<double>(2.53, Vector3d::Zero(), unit_M_BBo_B).CopyToFullMatrix6(),
                                    1e-14));
    };

    auto check_body_spatial = [this, &context](const std::string& body_name, const SpatialInertia<double>& M_BBo_B,
                                               double tol = 1e-14) {
    ...
    const SpatialInertia<double> inertia_from_inertial_tag =
            SpatialInertia<double>::MakeFromCentralInertia(524, Vector3d::Zero(), RotationalInertia<double>(1, 2, 3));

    check_body("default", UnitInertia<double>::SolidSphere(0.1));
    check_body_spatial("sphere", inertia_from_inertial_tag);
    check_body("capsule", UnitInertia<double>::SolidCapsule(0.1, 4.0, Vector3d::UnitZ()));
    check_body("ellipsoid", UnitInertia<double>::SolidEllipsoid(0.1, 0.2, 0.3));
    check_body("cylinder", UnitInertia<double>::SolidCylinder(0.1, 4.0, Vector3d::UnitZ()));
    ...
    check_body("ellipsoid_from_default", UnitInertia<double>::SolidEllipsoid(0.1, 0.2, 0.3));
    check_body("box_from_sub", UnitInertia<double>::SolidBox(0.8, 1.0, 1.2));
    SpatialInertia<double> M_BBo_B = SpatialInertia<double>::SolidSphereWithMass(2.53, 0.1);
    ...
    M_BBo_B = SpatialInertia<double>(2.53,                                    // mass
    ...
    check_body_spatial("box_from_mesh", SpatialInertia<double>::SolidCubeWithMass(1.0, 2.0), 1e-13);
    ...
    const UnitInertia<double> non_convex_unit_inertia{0.168, 0.168, 0.168, -0.034, -0.034, -0.034};
    const Vector3d non_convex_com = Vector3d::Constant(0.2166666666666666);
    check_body_spatial("non_convex_body", SpatialInertia<double>{1.0, non_convex_com, non_convex_unit_inertia}, 1e-13);
    ...
    check_body_spatial("box_w_density", SpatialInertia<double>::SolidBoxWithDensity(123, 0.8, 1.0, 1.2));
    check_body_spatial("box_default_density", SpatialInertia<double>::SolidBoxWithDensity(1000, 0.8, 1.0, 1.2));
    check_body_spatial("box_from_mesh_w_density", SpatialInertia<double>::SolidCubeWithMass(8.0, 2.0), 1e-12);
    check_body_spatial("non_convex_body_w_density",
                       SpatialInertia<double>{0.1, non_convex_com, non_convex_unit_inertia}, 1e-13);

    // A cube rotating about its corner.
    RotationalInertia<double> I_BFo_B(2, 2, 2, -.75, -.75, -.75);
    check_body_spatial("full_inertia", SpatialInertia<double>::MakeFromCentralInertia(1.23, Vector3d::Zero(), I_BFo_B));
    ...
    check_body_spatial("full_inertia_w_pose",
                       SpatialInertia<double>::MakeFromCentralInertia(1.23, Vector3d{1, 2, 3}, I_BFo_B));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*inertiafromgeom.*interpret.*ignored.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*coordinate.*not supported.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Illegal value 'abc' for the eulerseq in compiler.*"));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    AddModelFromString(xml, "test");
    EXPECT_THAT(TakeError(), MatchesRegex(".*Illegal value 'a' for the eulerseq in compiler.*"));
}
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    AddModelFromString(xml, "test");
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*mesh asset.*could not be found.*"));
...
<mujoco model="test">
  ...
  <worldbody>
    ...
    RigidTransformd X_WB(RollPitchYawd{M_PI / 6.0, M_PI / 4.0, M_PI / 3.0}, Vector3d{1.0, 2.0, 3.0});
    Vector3d pos{.1, .2, .3};
    ...
    EXPECT_TRUE(plant_.GetFreeBodyPose(*context, freejoint_body).IsNearlyEqualTo(X_WB, 1e-14));
    ...
    EXPECT_TRUE(plant_.GetFreeBodyPose(*context, free_body).IsNearlyEqualTo(X_WB, 1e-14));

    const BallRpyJoint<double>& ball_joint = plant_.GetJointByName<BallRpyJoint>("ball");
    EXPECT_EQ(ball_joint.default_damping(), 0.1);
    EXPECT_TRUE(ball_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(RigidTransformd(pos), 1e-14));
    EXPECT_TRUE(plant_.GetBodyByName("ball").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));

    const PrismaticJoint<double>& slide_joint = plant_.GetJointByName<PrismaticJoint>("slide");
    EXPECT_EQ(slide_joint.default_damping(), 0.2);
    EXPECT_TRUE(
            slide_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(RigidTransformd(pos), 1e-14));
    EXPECT_TRUE(CompareMatrices(slide_joint.translation_axis(), Vector3d{1, 0, 0}));
    EXPECT_TRUE(plant_.GetBodyByName("slide").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("slide").position_lower_limits(), Vector1d{-2.0}));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("slide").position_upper_limits(), Vector1d{1.5}));

    const RevoluteJoint<double>& hinge_joint = plant_.GetJointByName<RevoluteJoint>("hinge");
    EXPECT_EQ(hinge_joint.default_damping(), 0.3);
    EXPECT_TRUE(
            hinge_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(RigidTransformd(pos), 1e-14));
    EXPECT_TRUE(CompareMatrices(hinge_joint.revolute_axis(), Vector3d{0, 1, 0}));
    EXPECT_TRUE(plant_.GetBodyByName("hinge").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("hinge").position_lower_limits(), Vector1d{-M_PI / 6.0}, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("hinge").position_upper_limits(), Vector1d{M_PI / 3.0}, 1e-14));
    ...
    EXPECT_TRUE(hinge_w_joint_defaults_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(
            RigidTransformd(Vector3d{-0.1, -0.2, -0.3}), 1e-14));
    EXPECT_TRUE(CompareMatrices(hinge_w_joint_defaults_joint.revolute_axis(), Vector3d{1, 0, 0}));
    EXPECT_TRUE(plant_.GetBodyByName("hinge_w_joint_defaults").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("hinge_w_joint_defaults").position_lower_limits(),
                                Vector1d{-M_PI / 6.0}, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("hinge_w_joint_defaults").position_upper_limits(),
                                Vector1d{M_PI / 3.0}, 1e-14));

    const RevoluteJoint<double>& default_joint = plant_.GetJointByName<RevoluteJoint>("default");
    EXPECT_EQ(default_joint.default_damping(), 0.4);
    EXPECT_TRUE(default_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyIdentity(1e-14));
    EXPECT_TRUE(CompareMatrices(default_joint.revolute_axis(), Vector3d{0, 0, 1}));
    EXPECT_TRUE(plant_.GetBodyByName("default").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("default").position_lower_limits(), Vector1d{-M_PI / 9.0}));
    EXPECT_TRUE(CompareMatrices(plant_.GetJointByName("default").position_upper_limits(), Vector1d{M_PI / 12.0}));

    const RevoluteJoint<double>& hinge1_joint = plant_.GetJointByName<RevoluteJoint>("hinge1");
    EXPECT_EQ(hinge1_joint.default_damping(), 0.5);
    EXPECT_TRUE(CompareMatrices(hinge1_joint.revolute_axis(), Vector3d{1, 0, 0}));
    EXPECT_TRUE(
            hinge1_joint.frame_on_parent().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(RigidTransformd(pos), 1e-14));
    const RevoluteJoint<double>& hinge2_joint = plant_.GetJointByName<RevoluteJoint>("hinge2");
    EXPECT_EQ(hinge2_joint.default_damping(), 0.6);
    EXPECT_TRUE(CompareMatrices(hinge2_joint.revolute_axis(), Vector3d{0, 1, 0}));
    EXPECT_TRUE(
            hinge2_joint.frame_on_child().CalcPoseInBodyFrame(*context).IsNearlyEqualTo(RigidTransformd(pos), 1e-14));
    EXPECT_TRUE(plant_.GetBodyByName("two_hinges").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));

    EXPECT_TRUE(plant_.GetBodyByName("weld").EvalPoseInWorld(*context).IsNearlyEqualTo(X_WB, 1e-14));
    const WeldJoint<double>& weld_joint = plant_.GetJointByName<WeldJoint>("world_welds_to_weld");
...
<mujoco model="test">
  <worldbody>
    ...
    AddModelFromString(xml, "test");
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*Damping.*not supported for free.*"));
...
std::string MakeAutoLimitsXML(bool auto_limits, const std::string& motor_limit_prefix) {
    return fmt::format(R"""(
  <mujoco model="test">
...
TEST_F(MujocoParserTest, AutoLimitsTrue) {
    ...
    EXPECT_TRUE(CompareMatrices(plant_.GetPositionLowerLimits(), -expected_limits));
    EXPECT_TRUE(CompareMatrices(plant_.GetPositionUpperLimits(), expected_limits));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'limited' attribute must be one of.*"));
    EXPECT_TRUE(CompareMatrices(plant_.GetEffortLowerLimits(), -expected_limits.tail<5>()));
    EXPECT_TRUE(CompareMatrices(plant_.GetEffortUpperLimits(), expected_limits.tail<5>()));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'ctrllimited' attribute must be one of.*"));
}
...
TEST_F(MujocoParserTest, AutoLimitsFalse) {
    ...
    EXPECT_TRUE(CompareMatrices(plant_.GetPositionLowerLimits(), -expected_limits));
    EXPECT_TRUE(CompareMatrices(plant_.GetPositionUpperLimits(), expected_limits));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'range' attribute was specified.*but "
    ...
    EXPECT_TRUE(CompareMatrices(plant_.GetEffortLowerLimits(), -expected_limits.tail<5>()));
    EXPECT_TRUE(CompareMatrices(plant_.GetEffortUpperLimits(), expected_limits.tail<5>()));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'forcerange' attribute was specified.*but "
                                          "'autolimits' is disabled."));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'forcerange' attribute was specified.*but "
                                          "'autolimits' is disabled."));
    EXPECT_THAT(TakeError(), MatchesRegex(".*The 'forcelimited' attribute must be one of.*"));
}
...
<mujoco model="test">
  <worldbody>
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*inertial.*must include.*diaginertia or fullinertia.*"));
}
...
<mujoco model="test">
  <worldbody>
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*fromto.*ellipsoid.*unsupported.*ignored.*"));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*fromto.*box.*unsupported.*ignored.*"));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*unknown mesh.*ignored.*"));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*hfield.*unsupported.*ignored.*"));

    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*capsule.*must have.*two elements.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*capsule.*fromto.*must have.*one element.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*ellipsoid.*must have.*three elements.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*cylinder.*must have.*two elements.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*cylinder.*fromto.*must have.*one element.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*size.*box.*must have.*three elements.*"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*mesh.*did not set the mesh attribute.*"));
}
...
<mujoco model="test">
  ...
  <actuator>
    ...
    const JointActuator<double>& motor0 = plant_.get_joint_actuator(JointActuatorIndex(0));
    ...
    const JointActuator<double>& motor1 = plant_.get_joint_actuator(JointActuatorIndex(1));
    EXPECT_EQ(motor1.name(), "motor1");
    EXPECT_EQ(motor1.joint().name(), "hinge1");
    EXPECT_EQ(motor1.effort_limit(), 2);

    const JointActuator<double>& motor2 = plant_.get_joint_actuator(JointActuatorIndex(2));
    EXPECT_EQ(motor2.name(), "motor2");
    EXPECT_EQ(motor2.joint().name(), "hinge2");
    EXPECT_EQ(motor2.effort_limit(), .5);

    const JointActuator<double>& motor3 = plant_.get_joint_actuator(JointActuatorIndex(3));
...
class ContactTest : public MujocoParserTest, public testing::WithParamInterface<std::tuple<bool, bool>> {};
...
TEST_P(ContactTest, Contact) {
  ...
  <mujoco model="test">
    ...
    plant_.set_adjacent_bodies_collision_filters(adjacent_bodies_collision_filters);
    AddModelFromString(xml, "test");
    plant_.Finalize();

    const SceneGraphInspector<double>& inspector = scene_graph_.model_inspector();
    GeometryId base_geom = inspector.GetGeometries(plant_.GetBodyFrameIdOrThrow(plant_.GetBodyByName("base").index()),
                                                   geometry::Role::kProximity)[0];
    GeometryId body1_geom = inspector.GetGeometries(plant_.GetBodyFrameIdOrThrow(plant_.GetBodyByName("body1").index()),
                                                    geometry::Role::kProximity)[0];
    GeometryId body2_geom = inspector.GetGeometries(plant_.GetBodyFrameIdOrThrow(plant_.GetBodyByName("body2").index()),
                                                    geometry::Role::kProximity)[0];
    if (include_contact) {
...
INSTANTIATE_TEST_SUITE_P(ContactTests, ContactTest, testing::Combine(testing::Bool(), testing::Bool()));
...
<mujoco model="test">
  ...
  <contact>
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*pair.*not have.*geom1.*geom2.*ignored.*"));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*pair.*not have.*geom1.*geom2.*ignored.*"));
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*exclude.*not have.*body1.*body2.*ignored.*"));
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*exclude.*not have.*body1.*body2.*ignored.*"));
}
...
<mujoco model="test">
  ...
  <contact>
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromString(fmt::format(xml_base, "QQQ", "body2"), "bad_body1"),
                                ".*no RigidBody named 'QQQ' anywhere.*");
    DRAKE_EXPECT_THROWS_MESSAGE(AddModelFromString(fmt::format(xml_base, "body1", "QQQ"), "bad_body2"),
                                ".*no RigidBody named 'QQQ' anywhere.*");
}
