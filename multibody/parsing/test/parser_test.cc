...
GTEST_TEST(FileParserTest, BasicTest) {
    const std::string sdf_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.sdf");
    const std::string urdf_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.urdf");
    const std::string xml_name = FindResourceOrThrow("drake/multibody/parsing/dm_control/suite/acrobot.xml");
    const std::string dmd_name = FindResourceOrThrow(
            "drake/multibody/parsing/test/process_model_directives_test/"
            "acrobot.dmd.yaml");
    const std::string obj_name = FindResourceOrThrow("drake/multibody/parsing/test/box_package/meshes/box.obj");
...
GTEST_TEST(FileParserTest, UrlTest) {
    MultibodyPlant<double> plant(0.0);
    Parser dut(&plant);
    auto models = dut.AddModelsFromUrl("package://drake/multibody/benchmarks/acrobot/acrobot.sdf");
    EXPECT_EQ(models.size(), 1);
    EXPECT_EQ(plant.GetModelInstanceName(models.at(0)), "acrobot");

    // Check the error message for unsupported schemes.
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AddModelsFromUrl("ftp://l33t.w4r3z/acrobotz.sdf"), ".*unsupported scheme.*");
}

GTEST_TEST(FileParserTest, BasicStringTest) {
    const std::string sdf_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.sdf");
    const std::string urdf_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.urdf");
    const std::string xml_name = FindResourceOrThrow("drake/multibody/benchmarks/acrobot/acrobot.xml");
    const std::string dmd_name = FindResourceOrThrow(
            "drake/multibody/parsing/test/process_model_directives_test/"
            "acrobot.dmd.yaml");
    ...
    // Load an SDF via string using plural method.
    {
        ...
        MultibodyPlant<double> plant(0.0);
        Parser dut(&plant);
        const std::vector<ModelInstanceIndex> ids = dut.AddModelsFromString(sdf_contents, "sdf");
        EXPECT_EQ(ids.size(), 1);
        EXPECT_EQ(plant.GetModelInstanceName(ids[0]), "acrobot");
    }
    ...
    // Load an URDF via string using plural method.
    {
        ...
        MultibodyPlant<double> plant(0.0);
        Parser dut(&plant);
        const std::vector<ModelInstanceIndex> ids = dut.AddModelsFromString(urdf_contents, "urdf");
        EXPECT_EQ(ids.size(), 1);
        EXPECT_EQ(plant.GetModelInstanceName(ids[0]), "acrobot");
    }
    ...
    // Load an MJCF via string using plural method.
    {
        ...
        MultibodyPlant<double> plant(0.0);
        Parser dut(&plant);
        const std::vector<ModelInstanceIndex> ids = dut.AddModelsFromString(xml_contents, "xml");
        EXPECT_EQ(ids.size(), 1);
        EXPECT_EQ(plant.GetModelInstanceName(ids[0]), "acrobot");
    }
    ...
    // Load a DMD.YAML via string using plural method.
    {
        ...
        MultibodyPlant<double> plant(0.0);
        Parser dut(&plant);
        const std::vector<ModelInstanceIndex> ids = dut.AddModelsFromString(dmd_contents, "dmd.yaml");
        EXPECT_EQ(ids.size(), 1);
        EXPECT_EQ(plant.GetModelInstanceName(ids[0]), "acrobot");
    }
}
...
GTEST_TEST(FileParserTest, MultiModelErrorsTest) {
    const std::string sdf_name = FindResourceOrThrow("drake/multibody/parsing/test/sdf_parser_test/two_models.sdf");
    MultibodyPlant<double> plant(0.0);
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModels(sdf_name),
                                R"([\s\S]*Root object can only contain one model.*)");
}

std::vector<std::string> GetModelInstanceNames(const MultibodyPlant<double>& plant,
...
GTEST_TEST(FileParserTest, MultiModelViaWorldIncludesTest) {
    ...
    MultibodyPlant<double> plant(0.0);
    const std::vector<ModelInstanceIndex> models = Parser(&plant).AddModels(sdf_name);
    const std::vector<std::string> model_names_actual = GetModelInstanceNames(plant, models);
...
GTEST_TEST(FileParserTest, ExtensionMatchTest) {
    ...
    MultibodyPlant<double> plant(0.0);
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModels("acrobot.foo"), ".*file.*\\.foo.* is not.*recognized.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModels("acrobot.foo"), ".*file.*\\.foo.* is not.*recognized.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModels("acrobot.SDF"), "error: Error finding file.*");
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModels("acrobot.URDF"),
...
GTEST_TEST(FileParserTest, BadStringTest) {
    ...
    MultibodyPlant<double> plant(0.0);
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString("bad", "sdf"),
                                ".*Error parsing XML from string: Error=XML_ERROR_PARSING_TEXT.*");

    // Malformed URDF string is an error.
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString("bad", "urdf"),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString("bad", "xml"),
                                ".*Failed to parse XML string: XML_ERROR_PARSING_TEXT");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString("bad:", "dmd.yaml"), ".*YAML.*bad.*");
    ...
    // Syntactically well-formed DMD data, but semantically invalid.
    {
        ...
        constexpr char yaml[] =
                "directives:\n"
                "- add_model_instance:\n";
        DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString(yaml, "dmd.yaml"), ".*IsValid.*failed.*");
    }

    // Unknown extension is an error.
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant).AddModelsFromString("<bad/>", "weird-ext"),
...
GTEST_TEST(FileParserTest, PrefixConstructors) {
    ...
    // collision.
    DRAKE_EXPECT_THROWS_MESSAGE(Parser(&plant, &scene_graph).AddModelsFromString(model, "urdf"),
    ...
    EXPECT_NO_THROW(Parser(&plant, &scene_graph, "prefix2").AddModelsFromString(model, "urdf"));
}
...
GTEST_TEST(FileParserTest, PackageMapTest) {
    ...
    const std::string full_package_filename =
            FindResourceOrThrow("drake/multibody/parsing/test/box_package/package.xml");
    ...
    const std::string full_obj_filename =
            FindResourceOrThrow("drake/multibody/parsing/test/box_package/meshes/box.obj");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(parser.AddModels(new_sdf_filename), ".*error.*unknown package.*box_model.*");
...
GTEST_TEST(FileParserTest, StrictParsing) {
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(parser.AddModelsFromString(model_provokes_warning, "urdf"), warning_pattern);
    }
}
...
GTEST_TEST(FileParserTest, AutoRenaming) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(parser.AddModelsFromString(model, "urdf"), ".*names must be unique.*");
...
GTEST_TEST(FileParserTest, InterleavedRenaming) {
    ...
    auto [plant, scene_graph] = drake::multibody::AddMultibodyPlant(plant_config, &builder);
    ...
    expected.AddGroup("my_kuka_iiwa0::iiwa_wrist",
                      {"my_kuka_iiwa0::iiwa_link_5", "my_kuka_iiwa0::iiwa_link_6", "my_kuka_iiwa0::iiwa_link_7"});
    expected.AddGroup("my_kuka_iiwa1::iiwa_wrist",
                      {"my_kuka_iiwa1::iiwa_link_5", "my_kuka_iiwa1::iiwa_link_6", "my_kuka_iiwa1::iiwa_link_7"});
    expected.AddExclusionPair({"my_kuka_iiwa0::iiwa_wrist", "my_kuka_iiwa0::iiwa_wrist"});
    expected.AddExclusionPair({"my_kuka_iiwa1::iiwa_wrist", "my_kuka_iiwa1::iiwa_wrist"});
