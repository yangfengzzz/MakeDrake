...
protected:
    ...
    ParsingWorkspace w_{options_, {}, diagnostic_policy_, &plant_, &resolver_, SelectParser};
};
...
TEST_F(SelectParserTest, UnknownFileType) {
    ...
    // does not crash.
    std::string empty;
    EXPECT_EQ(parser.AddModel({DataSource::kContents, &empty}, "", {}, w_), std::nullopt);
    EXPECT_EQ(parser.AddAllModels({DataSource::kContents, &empty}, "", w_).size(), 0);
    FlushDiagnostics();
}
