...
public:
    CollisionFilterGroupResolverTest() { plant_.RegisterAsSourceForSceneGraph(&scene_graph_); }

    // Add a body to a model, with collision geometry and return the geometry id.
    GeometryId AddBody(const std::string& name, std::optional<ModelInstanceIndex> model_instance) {
        auto model = model_instance.value_or(default_model_instance());
        return plant_.RegisterCollisionGeometry(plant_.AddRigidBody(name, model), RigidTransformd::Identity(),
...
protected:
    ...
    const geometry::SceneGraphInspector<double>& inspector_{scene_graph_.model_inspector()};
...
TEST_F(CollisionFilterGroupResolverTest, MissingBodyGlobal) {
    ...
    resolver_.AddGroup(diagnostic_policy_, "a", {"abody", "r1::abody", "zzz::abody"}, {}, {});
...
TEST_F(CollisionFilterGroupResolverTest, MissingMemberGroupGlobal) {
    plant_.AddModelInstance("r1");
    resolver_.AddGroup(diagnostic_policy_, "a", {}, {"agroup", "r1::agroup", "zzz::agroup"}, {});
...
TEST_F(CollisionFilterGroupResolverTest, OutOfParseBodyGlobal) {
    ...
    // out of bounds.
    ASSERT_LT(plant_.GetModelInstanceByName("DefaultModelInstance"), resolver_.minimum_model_instance_index());
    ASSERT_LT(plant_.GetModelInstanceByName("WorldModelInstance"), resolver_.minimum_model_instance_index());
...
TEST_F(CollisionFilterGroupResolverTest, MissingBodyScoped) {
    ...
    resolver_.AddGroup(diagnostic_policy_, "a", {"abody", "sub::abody", "zzz::abody"}, {}, r1);
    EXPECT_THAT(TakeError(), MatchesRegex(".*'r1::abody'.*not found"));
...
TEST_F(CollisionFilterGroupResolverTest, MissingMemberGroupScoped) {
    ModelInstanceIndex r1 = plant_.AddModelInstance("r1");
    plant_.AddModelInstance("r1::sub");
    resolver_.AddGroup(diagnostic_policy_, "a", {}, {"agroup", "sub::agroup", "zzz::agroup"}, r1);
    // Group insertions get evaluated at resolution time.
    resolver_.Resolve(diagnostic_policy_);
...
TEST_F(CollisionFilterGroupResolverTest, GroupGlobalBodies) {
    ...
    const CollisionFilterGroupsImpl<std::string> actual_groups =
            ConvertInstancedNamesToStrings(resolver_.Resolve(diagnostic_policy_), plant_);
...
TEST_F(CollisionFilterGroupResolverTest, MemberGroupCycle) {
    ...
    for (int k = 0; k < length; ++k) {
        ...
        resolver_.AddGroup(diagnostic_policy_, body_k, {body_k}, {body_of((k + 1) % length)}, r1);
    }
    ...
    EXPECT_TRUE(inspector_.CollisionFiltered(geoms.front(), geoms[geoms.size() / 2]));
}
...
TEST_F(CollisionFilterGroupResolverTest, MemberGroupDeepNest) {
    ...
    for (int k = 0; k < depth; ++k) {
        ...
        if (k + 1 >= depth) {
            ...
            resolver_.AddGroup(diagnostic_policy_, body_k, {body_k}, {body_of(k + 1)}, r1);
        }
    }
