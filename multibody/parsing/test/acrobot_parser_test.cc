...
#include "multibody/parsing/test/test_loaders.h"
...
class AcrobotModelTests : public testing::TestWithParam<test::ModelLoadFunction> {
public:
    void SetUp() override {
        ...
        benchmark_shoulder_ = &benchmark_plant_->GetJointByName<RevoluteJoint>("ShoulderJoint");
        benchmark_elbow_ = &benchmark_plant_->GetJointByName<RevoluteJoint>("ElbowJoint");
    ...
    void VerifyModelMassMatrix(double theta1, double theta2) {
        ...
        benchmark_plant_->CalcMassMatrixViaInverseDynamics(*benchmark_context_.get(), &M_benchmark);
        ...
        EXPECT_TRUE(CompareMatrices(M, M_benchmark, kTolerance, MatrixCompareType::relative));
    }
...
TEST_P(AcrobotModelTests, ModelBasics) {
    ...
    EXPECT_EQ(plant_->GetJointActuatorByName("ElbowJoint").joint().index(), elbow_->index());
    ...
    EXPECT_EQ(plant_->num_multibody_states(), benchmark_plant_->num_multibody_states());

    // Get links by name.
    const RigidBody<double>& link1 = plant_->GetBodyByName(parameters_.link1_name());
    EXPECT_EQ(link1.name(), parameters_.link1_name());
    const RigidBody<double>& link2 = plant_->GetBodyByName(parameters_.link2_name());
    ...
    const JointActuator<double>& elbow_actuator = plant_->GetJointActuatorByName("ElbowJoint");
...
INSTANTIATE_TEST_SUITE_P(SdfAcrobatModelTests, AcrobotModelTests, ::testing::Values(test::LoadFromSdf));

INSTANTIATE_TEST_SUITE_P(UrdfAcrobatModelTests, AcrobotModelTests, ::testing::Values(test::LoadFromUrdf));
