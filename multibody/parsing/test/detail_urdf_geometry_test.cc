...
public:
    ...
    UrdfMaterial AddMaterial(const std::string& material_name, UrdfMaterial material, bool error_if_name_clash) {
        return AddMaterialToMaterialMap(policy_, material_name, material, error_if_name_clash, &materials_);
    }
...
TEST_F(UrdfMaterialTest, UniqueNoData) {
    ...
    AddMaterialGood(empty_mat_name_, empty_mat_, error_if_name_clash_, default_mat_);
    AddMaterialGood(empty_mat_name_, empty_mat_, !error_if_name_clash_, default_mat_);
}
...
TEST_F(UrdfMaterialTest, RedundantDifferentColor) {
    ...
    AddMaterialGood(rgba_mat_name_, UrdfMaterial{rgba_color2, {}}, !error_if_name_clash_, rgba_mat_);

    // Deviation between colors too big.
    const Vector4d rgba_color3 = rgba_color_ + Vector4d::Constant(5e-11);
    AddMaterialBad(rgba_mat_name_, UrdfMaterial{rgba_color3, {}}, !error_if_name_clash_,
...
TEST_F(UrdfMaterialTest, UniqueDiffuse) {
    ...
    AddMaterialGood(diffuse_mat_name, diffuse_mat, error_if_name_clash_, expected);
    AddMaterialGood(diffuse_mat_name, diffuse_mat, !error_if_name_clash_, expected);
}
...
TEST_F(UrdfMaterialTest, NameClashParameter) {
    ...
    AddMaterialBad(empty_mat_name_, empty_mat_, error_if_name_clash_, "Material '.+' was previously defined.+");
}
...
TEST_F(UrdfMaterialTest, RgbaMismatch) {
    ...
    AddMaterialGood(rgba_mat_name_, rgba_mat_, error_if_name_clash_, rgba_mat_);
    AddMaterialBad(rgba_mat_name_, UrdfMaterial{Vector4d{0.1, 0.1, 0.1, 0.1}, {}}, !error_if_name_clash_,
                   "Material '.+' was previously defined.+");
}
...
TEST_F(UrdfMaterialTest, DiffuseMismatch) {
    ...
    AddMaterialGood(rgba_mat_name_, rgba_mat_, error_if_name_clash_, rgba_mat_);
    AddMaterialBad(rgba_mat_name_, UrdfMaterial{{}, "bad_name"}, !error_if_name_clash_,
                   "Material '.+' was previously defined.+");
}
...
TEST_F(UrdfMaterialTest, ValueMismatch) {
    ...
    AddMaterialGood(full_mat_name_, full_mat_, error_if_name_clash_, full_mat_);
    AddMaterialBad(full_mat_name_, UrdfMaterial{full_color_, "bad_name"}, !error_if_name_clash_,
                   "Material '.+' was previously defined.+");
}
...
<?xml version="1.0"?>
  <collision>
    ...
    std::optional<GeometryInstance> ParseCollisionDoc(const std::string& collision_spec) {
        ...
        return ParseCollision(*diag_, "link_name", package_map_, root_dir_, collision_node, &geometry_names_,
    ...
    const ProximityProperties& ParseCollisionDocGood(const std::string& collision_spec) {
        std::optional<GeometryInstance> maybe_instance = ParseCollisionDoc(collision_spec);
    ...
    void VerifySingleProperty(const ProximityProperties& properties,
        ...
        EXPECT_TRUE(has_property) << fmt::format("  for property: ('{}', '{}')", group, property);
        if (!has_property) {
            return;
        }
    ...
    void VerifyFriction(const ProximityProperties& properties, const CoulombFriction<double>& expected_friction) {
        VerifySingleProperty(properties, "material", "coulomb_friction", expected_friction);
    }
...
protected:
    void UpdateSource(DataSource::DataSourceType type, std::string* data) {
        ...
        diag_ = std::make_unique<TinyXml2Diagnostic>(&diagnostic_policy_, &*source_);
    }
    ...
    void DoParseUrdfGeometry() {
        ...
        for (const XMLElement* material_node = node->FirstChildElement("material"); material_node;
             material_node = material_node->NextSiblingElement("material")) {
            ParseMaterial(*diag_, material_node, true, package_map_, root_dir_, &materials_);
        }

        // Parses geometry out of the model's link elements.
        for (const XMLElement* link_node = node->FirstChildElement("link"); link_node;
             link_node = link_node->NextSiblingElement("link")) {
            const char* attr = node->Attribute("name");
            ASSERT_TRUE(attr);
            const std::string body_name = attr;

            for (const XMLElement* visual_node = link_node->FirstChildElement("visual"); visual_node;
                 visual_node = visual_node->NextSiblingElement("visual")) {
                std::optional<geometry::GeometryInstance> geometry_instance =
                        ParseVisual(*diag_, body_name, package_map_, root_dir_, visual_node, &materials_,
                                    &geometry_names_, numeric_name_suffix_limit_);
                if (geometry_instance.has_value()) {
                    visual_instances_.push_back(*geometry_instance);
                }
            }

            for (const XMLElement* collision_node = link_node->FirstChildElement("collision"); collision_node;
                 collision_node = collision_node->NextSiblingElement("collision")) {
                std::optional<geometry::GeometryInstance> geometry_instance =
                        ParseCollision(*diag_, body_name, package_map_, root_dir_, collision_node, &geometry_names_,
                                       numeric_name_suffix_limit_);
                if (geometry_instance.has_value()) {
...
TEST_F(UrdfGeometryTest, TestParseMaterial1) {
    const std::string file_no_conflict_1 = FindUrdfTestResourceOrThrow("non_conflicting_materials_1.urdf");
    ...
    const std::string& file_name2 = *(materials_.at("texture_and_color").diffuse_map);
    EXPECT_EQ("empty.png", file_name2.substr(file_name2.size() - 9));
    EXPECT_TRUE(CompareMatrices(*(materials_.at("texture_and_color").rgba), green));
    ...
    EXPECT_TRUE(CompareMatrices(visual.pose().GetAsMatrix34(), RigidTransformd().GetAsMatrix34()));

    const geometry::IllustrationProperties* properties = visual.illustration_properties();
    ASSERT_NE(properties, nullptr);
    EXPECT_TRUE(properties->HasProperty("phong", "diffuse"));
    EXPECT_TRUE(CompareMatrices(properties->GetProperty<Vector4d>("phong", "diffuse"), *(materials_.at("green").rgba)));

    const geometry::Box* box = dynamic_cast<const geometry::Box*>(&visual.shape());
    ASSERT_TRUE(box);
    EXPECT_TRUE(CompareMatrices(box->size(), Eigen::Vector3d(0.2, 0.2, 0.2)));

    const auto& capsule_visual = visual_instances_[1];
    const geometry::Capsule* capsule = dynamic_cast<const geometry::Capsule*>(&capsule_visual.shape());
    ASSERT_TRUE(capsule);
    EXPECT_EQ(capsule->radius(), 0.2);
    EXPECT_EQ(capsule->length(), 0.5);

    const auto& sphere_visual = visual_instances_[2];
    const geometry::Sphere* sphere = dynamic_cast<const geometry::Sphere*>(&sphere_visual.shape());
    ASSERT_TRUE(sphere);
    EXPECT_EQ(sphere->radius(), 0.25);
}

TEST_F(UrdfGeometryTest, TestParseMaterial2) {
    const std::string file_no_conflict_2 = FindUrdfTestResourceOrThrow("non_conflicting_materials_2.urdf");
    ...
    const auto& visual = visual_instances_[0];
    EXPECT_TRUE(visual.illustration_properties()->HasProperty("phong", "diffuse"));
    EXPECT_TRUE(
            CompareMatrices(visual.illustration_properties()->GetProperty<Vector4d>("phong", "diffuse"), brown, 1e-14));
    const RigidTransformd expected_pose(Eigen::Vector3d(0, 0, 0.3));
    EXPECT_TRUE(CompareMatrices(visual.pose().GetAsMatrix34(), expected_pose.GetAsMatrix34()));

    const geometry::Cylinder* cylinder = dynamic_cast<const geometry::Cylinder*>(&visual.shape());
    ...
    const geometry::Mesh* mesh = dynamic_cast<const geometry::Mesh*>(&mesh_visual.shape());
    ASSERT_TRUE(mesh);

    const std::string& mesh_filename = mesh->filename();
    std::string obj_name = "tri_cube.obj";
    EXPECT_EQ(mesh_filename.rfind(obj_name), mesh_filename.size() - obj_name.size());

    const auto& sphere_visual = visual_instances_[2];
    EXPECT_TRUE(sphere_visual.illustration_properties()->HasProperty("phong", "diffuse"));
    EXPECT_TRUE(CompareMatrices(sphere_visual.illustration_properties()->GetProperty<Vector4d>("phong", "diffuse"),
                                brown, 1e-14));

    ASSERT_EQ(collision_instances_.size(), 1);
    const auto& collision = collision_instances_.front();

    const geometry::Sphere* sphere = dynamic_cast<const geometry::Sphere*>(&collision.shape());
    ASSERT_TRUE(sphere);
    EXPECT_EQ(sphere->radius(), 0.2);
}

TEST_F(UrdfGeometryTest, TestParseMaterial3) {
    const std::string file_no_conflict_3 = FindUrdfTestResourceOrThrow("non_conflicting_materials_3.urdf");
...
TEST_F(UrdfGeometryTest, TestParseMaterialEmpty) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Material 'b'.*no color or texture.*"));
}
...
TEST_F(UrdfGeometryTest, TestParseMaterialDuplicateButSame) {
    ...
    const std::string file_same_color_diff_links =
            FindUrdfTestResourceOrThrow("duplicate_but_same_materials_with_texture.urdf");
    ...
    math::RigidTransform<double> expected_pose(math::RotationMatrix<double>(expected_rpy), expected_xyz);
...
TEST_F(UrdfGeometryTest, TestDuplicateMaterials) {
    const std::string file_duplicate = FindUrdfTestResourceOrThrow("duplicate_materials.urdf");
...
TEST_F(UrdfGeometryTest, TestConflictingMaterials) {
    const std::string file_conflict = FindUrdfTestResourceOrThrow("conflicting_materials.urdf");
...
TEST_F(UrdfGeometryTest, TestWrongElementType) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Expected material element, got <robot>"));

    const XMLElement* material_node = node->FirstChildElement("material");
    ASSERT_TRUE(material_node);

    ParseVisual(*diag_, "fake_name", package_map_, root_dir_, material_node, &materials_, &geometry_names_);
    EXPECT_THAT(TakeError(), MatchesRegex(".*In link fake_name expected visual"
                                          " element, got material"));

    ParseCollision(*diag_, "fake_name", package_map_, root_dir_, material_node, &geometry_names_);
...
TEST_F(UrdfGeometryTest, TestParseConvexMesh) {
    const std::string convex_and_nonconvex_test = FindUrdfTestResourceOrThrow("convex_and_nonconvex_test.urdf");
...

    {
        ...
        const geometry::Convex* convex = dynamic_cast<const geometry::Convex*>(&instance.shape());
        ASSERT_TRUE(convex);
    }

    {
        const auto& instance = collision_instances_[1];
        const geometry::Mesh* mesh = dynamic_cast<const geometry::Mesh*>(&instance.shape());
        ASSERT_TRUE(mesh);
    }
}
...
TEST_F(UrdfGeometryTest, CollisionSmokeTest) {
  ...
  <drake:proximity_properties>
    ...
    VerifySingleProperty(properties, geometry::internal::kHydroGroup, geometry::internal::kRezHint, 2.5);
    VerifySingleProperty(properties, geometry::internal::kHydroGroup, geometry::internal::kElastic, 3.5);
    VerifySingleProperty(properties, geometry::internal::kMaterialGroup, geometry::internal::kHcDissipation, 3.5);
    VerifySingleProperty(properties, geometry::internal::kMaterialGroup, geometry::internal::kRelaxationTime, 3.1);
...
TEST_F(UrdfGeometryTest, TestDrakeDiffuseMapError) {
    ...
    DRAKE_EXPECT_NO_THROW(ParseUrdfGeometryString(robot));
    EXPECT_THAT(TakeError(), ContainsRegex("<drake:diffuse_map> is not supported.*"));
}
...
TEST_F(UrdfGeometryTest, TestBadCylinder) {
    ...
    ParseUrdfGeometryString(fmt::format(base, ""));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Missing cylinder attribute.*radius.*"));

    ParseUrdfGeometryString(fmt::format(base, "radius='1'"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Missing cylinder attribute.*length.*"));
...
TEST_F(UrdfGeometryTest, TestBadCapsule) {
    ...
    for (const auto& prefix : std::array<std::string, 2>{"", "drake:"}) {
        ...
        EXPECT_THAT(TakeError(), MatchesRegex(".*Missing capsule attribute.*radius.*"));
        ParseUrdfGeometryString(fmt::format(base, prefix, "radius='1'"));
        EXPECT_THAT(TakeError(), MatchesRegex(".*Missing capsule attribute.*length.*"));
        ParseUrdfGeometryString(fmt::format(base, prefix, "radius='1 2 3' length='1'"));
        EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*radius.*"));
        ParseUrdfGeometryString(fmt::format(base, prefix, "radius='1' length='1 2 3'"));
        EXPECT_THAT(TakeError(), MatchesRegex(".*Expected single value.*length.*"));
    }
}
...
TEST_F(UrdfGeometryTest, TestBadEllipsoid) {
    ...
    ParseUrdfGeometryString(fmt::format(base, ""));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Missing ellipsoid attribute.*'a'.*"));

    ParseUrdfGeometryString(fmt::format(base, "a='1'"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Missing ellipsoid attribute.*'b'.*"));

    ParseUrdfGeometryString(fmt::format(base, "a='1' b='2'"));
    EXPECT_THAT(TakeError(), MatchesRegex(".*Missing ellipsoid attribute.*'c'.*"));
...
TEST_F(UrdfGeometryTest, TestLegacyDrakeCompliance) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to parse dynamic_friction.*"));
    ...
    ParseUrdfGeometryString(fmt::format(base, "<static_friction>10</static_friction>"));
...
TEST_F(UrdfGeometryTest, TestBadProperty) {
  ...
  <drake:proximity_properties>
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Unable to read.*attribute.*mu_dynamic.*"));
...
TEST_F(UrdfGeometryTest, LegacySoftHydroelastic) {
  ...
  <drake:proximity_properties>
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Collision geometry uses the tag"
...
TEST_F(UrdfGeometryTest, BothHydroelastic) {
  ...
  <drake:proximity_properties>
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*Collision geometry has defined mutually-exclusive tags"
                                          " .*rigid.* and .*compliant.*"));
}
...
TEST_F(UrdfGeometryTest, LegacyDrakeComplianceVsProxProperties) {
  ...
  <drake:proximity_properties>
    ...
    EXPECT_THAT(TakeWarning(), MatchesRegex(".*both <drake:proximity_properties> and"
...
TEST_F(UrdfGeometryTest, AcceptingRenderers) {
    const std::string file_no_conflict_1 = FindUrdfTestResourceOrThrow("accepting_renderer.urdf");
    ...
    for (const auto& instance : visual_instances_) {
        ...
        if (instance.name() == "all_renderers") {
            ...
            const auto& names = props.GetProperty<std::set<std::string>>(group, property);
            ...
            GTEST_FAIL() << "Encountered visual geometry not expected: " << instance.name();
        }
    }
}
...
TEST_F(UrdfGeometryTest, BadAcceptingRenderer) {
    ...
    EXPECT_THAT(TakeError(), MatchesRegex(".*<drake:accepting_renderer>.*without.* name.*"));
}
...
TEST_F(UrdfGeometryTest, TwoUnnamedBoxes) {
    const std::string two_unnamed_boxes = FindUrdfTestResourceOrThrow("two_unnamed_boxes.urdf");
