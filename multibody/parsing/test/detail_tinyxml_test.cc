...
GTEST_TEST(TinyxmlUtilTest, ParseAttributeTest) {
    ...
    EXPECT_THROW(ParseVectorAttribute(element, "vec4", &vec3), std::invalid_argument);
    EXPECT_THROW(ParseVectorAttribute(element, "scalar", &vec3), std::invalid_argument);
    ...
    EXPECT_THROW(ParseVectorAttribute(element, "vec3", &vec4), std::invalid_argument);
    EXPECT_THROW(ParseVectorAttribute(element, "scalar", &vec4), std::invalid_argument);
}
...
public:
    ...
    bool ParseScalarAttribute(const tinyxml2::XMLElement* node, const char* attribute_name, double* val) {
        return internal::ParseScalarAttribute(node, attribute_name, val, diagnostic_policy_);
    }
};

TEST_F(TinyxmlUtilDiagnosticTest, ParseScalarAttributeDiagnosticTest) {
    const std::string test_xml = "<element scalar='1' empty='' bad='one' vec3='2.1 3.2 4.3'/>";
    ...
    EXPECT_THAT(TakeError(), testing::MatchesRegex(".*Expected single value.*vec3.*"));
    EXPECT_FALSE(ParseScalarAttribute(element, "empty", &scalar));
    EXPECT_THAT(TakeError(), testing::MatchesRegex(".*Expected single value.*empty.*"));
    EXPECT_FALSE(ParseScalarAttribute(element, "bad", &scalar));
    EXPECT_THAT(TakeError(), testing::MatchesRegex(".*Expected single value.*bad.*"));
}
...
GTEST_TEST(TinyxmlUtilTest, MalformedRpyOriginAttributesTest) {
    ...
    EXPECT_THROW(internal::OriginAttributesToTransform(element), std::invalid_argument);
}
...
GTEST_TEST(TinyxmlUtilTest, LocaleAttributeTest) {
    ...
    std::locale original_global_locale =
            std::locale::global(std::locale(std::locale::classic(), new CommaDecimalPointFacet));
