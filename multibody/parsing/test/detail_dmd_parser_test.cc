...
public:
    ...
    std::vector<ModelInstanceInfo> ParseModelDirectives(const ModelDirectives& directives) {
        ...
        const ParsingWorkspace w{options_, package_map_, diagnostic_policy_, &plant_, &resolver, TestingSelect};
        auto result = multibody::internal::ParseModelDirectives(directives, std::nullopt, w);
  ...
  <model name="sphere">
    ...
    <frame name="dummy_offset" attached_to="dummy">
      <pose>{x} {y} {z} 0 0 0</pose>
                     ...
                     fmt::arg("x", p_BF.x()), fmt::arg("y", p_BF.y()), fmt::arg("z", p_BF.z()));
    return sdf_path;
}
...
- add_model:
    ...
    default_free_body_pose:
        ball:
            ...
            translation: {offset_str}
                    ...
                    fmt::arg("offset_str", offset_str), fmt::arg("sphere_sdf", sphere_sdf)),
            {}, ModelDirectives());
    ...
    auto get_body = [this, &results](int expected_index,
                                     std::string_view body_name = "ball") -> const RigidBody<double>& {
    ...
    std::vector<BallExpectations> balls{{.body = &ball_2, .parent = &world, .body_floats = true},
    ...
    for (const auto& ball : balls) {
        SCOPED_TRACE(fmt::format("Ball {}", plant_.GetModelInstanceName(ball.body->model_instance())));
        ...
        if (ball.body_floats) {
            ...
            RigidTransformd X_PC_plant_default = plant_.GetDefaultFreeBodyPose(*ball.body);
            EXPECT_TRUE(CompareMatrices(X_PC_plant_default.GetAsMatrix34(),
        ...
        const auto& joint = plant_.GetJointByName(joint_name, ball.body->model_instance());

        /* The default pose can *always* be checked from the joint. */
        EXPECT_TRUE(CompareMatrices(joint.GetDefaultPose().GetAsMatrix34(), RigidTransformd(p_PC).GetAsMatrix34()));
        EXPECT_EQ(&joint.parent_body(), ball.parent);
        EXPECT_EQ(&joint.child_body(), ball.body);

        EXPECT_TRUE(CompareMatrices(ball.body->EvalPoseInWorld(*context).GetAsMatrix34(),
                                    RigidTransformd(ball.num_offset * p_PC).GetAsMatrix34()));
    }
}
