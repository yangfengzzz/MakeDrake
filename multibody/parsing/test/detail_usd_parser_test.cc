...
std::string FindUsdTestResourceOrThrow(const std::string& filename) {
    const std::string resource_dir{"drake/multibody/parsing/test/usd_parser_test/"};
...
TEST_F(UsdParserTest, InvalidFileTest) {
    ...
    const DataSource source{DataSource::kFilename, &filename};
    ParseFile(source);
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*Failed to open USD stage:.*"));
}
...
TEST_F(UsdParserTest, InvalidInMemoryStageTest) {
    ...
    ParseFile(source);
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*Failed to load in-memory USD stage."));
}
...
TEST_F(UsdParserTest, MissingStageMetadataTest) {
    ...
    const DataSource source{DataSource::kContents, &file_content};
    ParseFile(source);
    EXPECT_THAT(TakeWarning(), ::testing::MatchesRegex(".*Failed to read metersPerUnit in stage metadata.*"));
    EXPECT_THAT(TakeWarning(), ::testing::MatchesRegex(".*Failed to read upAxis in stage metadata.*"));
}
...
TEST_F(UsdParserTest, UnsupportedPrimTypesTest) {
    ...
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*The type of the Prim at /World/Box is not specified.*"));
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*Failed to create collision geometry.*"));
    // Errors from the `/World/Cone` Prim.
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*Unsupported Prim type 'Cone'.*"));
    EXPECT_THAT(TakeError(), ::testing::MatchesRegex(".*Failed to create collision geometry.*"));
}
