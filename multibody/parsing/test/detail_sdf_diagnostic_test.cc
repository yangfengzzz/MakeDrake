...
public:
    SDFormatDiagnosticContentsTest() {
        sdf::Errors errors = root_.LoadSdfString(data_source_.contents(), parser_config_);
        EXPECT_FALSE(sdf_diagnostic_.PropagateErrors(errors));
    }

protected:
    DataSource data_source_{DataSource::kContents, &data_};
    SDFormatDiagnostic sdf_diagnostic_{&diagnostic_policy_, &data_source_, "stuff"};
};
...
TEST_F(SDFormatDiagnosticContentsTest, Error) {
    sdf_diagnostic_.Error(root_.Element()->FindElement("model"), std::move("badness"));
...
TEST_F(SDFormatDiagnosticFilenameTest, Error) {
    ...
    EXPECT_THAT(TakeError(), testing::MatchesRegex("/.*/test_data.stuff:2: error: badness"));
}

TEST_F(SDFormatDiagnosticFilenameTest, Warning) {
    sdf_diagnostic_.Warning(root_.Element()->FindElement("model"), "regret");
    EXPECT_THAT(TakeWarning(), testing::MatchesRegex("/.*/test_data.stuff:2: warning: regret"));
}
...
TEST_F(SDFormatDiagnosticContentsTest, PolicyForNode) {
    ...
    auto error_policy = sdf_diagnostic_.MakePolicyForNode(*(root_.Element()->FindElement("model")));
    error_policy.Error("bad");
    EXPECT_EQ(TakeError(), "<literal-string>.stuff:2: error: bad");

    auto warning_policy = sdf_diagnostic_.MakePolicyForNode(*(root_.Element()->FindElement("model")));
