#include "multibody/parsing/process_model_directives.h"
...
const char* const kTestDir = "drake/multibody/parsing/test/process_model_directives_test";

void add_test_package(PackageMap* package_map) {
    const std::filesystem::path abspath_xml = FindResourceOrThrow(std::string(kTestDir) + "/package.xml");
...
void VerifyCollisionFilters(const geometry::SceneGraph<double>& scene_graph,
    ...
    auto collision_id_set = inspector.GetGeometryIds(id_set, geometry::Role::kProximity);
...
GTEST_TEST(ProcessModelDirectivesTest, BasicSmokeTest) {
    ModelDirectives station_directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/add_scoped_sub.dmd.yaml"));
    const MultibodyPlant<double> empty_plant(0.0);

    MultibodyPlant<double> plant(0.0);
    ProcessModelDirectives(station_directives, &plant, nullptr, make_parser(&plant).get());
    plant.Finalize();
...
GTEST_TEST(ProcessModelDirectivesTest, FromString) {
    ModelDirectives station_directives = LoadModelDirectivesFromString(
            ReadFileOrThrow(FindResourceOrThrow(std::string(kTestDir) + "/add_scoped_sub.dmd.yaml")));
...
GTEST_TEST(ProcessModelDirectivesTest, SugarSmokeTest) {
    const ModelDirectives station_directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/add_scoped_sub.dmd.yaml"));

    MultibodyPlant<double> plant(0.0);
    std::vector<ModelInstanceInfo> added_models = ProcessModelDirectives(station_directives, make_parser(&plant).get());
    plant.Finalize();
...
GTEST_TEST(ProcessModelDirectivesTest, AddScopedSmokeTest) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/add_scoped_top.dmd.yaml"));
    ...
    ProcessModelDirectives(directives, &plant, nullptr, make_parser(&plant).get());
    plant.Finalize();
    auto diagram = builder.Build();

    // Helper lambda for checking existence of frame in model scope.
    auto check_frame = [&plant](const std::string instance, const std::string frame) {
        const std::string scoped_frame = instance + "::" + frame;
        drake::log()->debug("Check: {}", scoped_frame);
        EXPECT_TRUE(plant.HasFrameNamed(frame, plant.GetModelInstanceByName(instance)));
    };
...
GTEST_TEST(ProcessModelDirectivesTest, AddFrameWithoutScope) {
    ModelDirectives directives = LoadModelDirectives(
            FindResourceOrThrow(std::string(kTestDir) + "/add_frame_without_model_namespace.dmd.yaml"));
    ...
    EXPECT_TRUE(plant.HasFrameNamed("world_as_base_frame", world_model_instance()));

    // Frame added with included model as base frame.
    auto simple_model_instance = plant.GetModelInstanceByName("simple_model");
    EXPECT_TRUE(plant.HasFrameNamed("included_as_base_frame", simple_model_instance));
}
...
- add_frame:
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadModelDirectivesFromString(yaml), ".*IsValid.*");
}

// Test backreference behavior in ModelDirectives.
GTEST_TEST(ProcessModelDirectivesTest, TestBackreferences) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/test_backreferences.dmd.yaml"));
    ...
    // instead of world.
    EXPECT_TRUE(plant.HasJointNamed("simple_model_origin_welds_to_base", plant.GetModelInstanceByName("simple_model")));

    // Weld joint for the nested model.
    EXPECT_TRUE(plant.HasJointNamed("simple_model_origin_welds_to_base",
...
GTEST_TEST(ProcessModelDirectivesTest, InjectFrames) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/inject_frames.dmd.yaml"));
    ...
    EXPECT_TRUE(plant.HasFrameNamed("top_injected_frame", plant.GetModelInstanceByName("top_level_model")));
    EXPECT_TRUE(plant.HasFrameNamed("base", plant.GetModelInstanceByName("mid_level_model")));

    // Check for pose of welded models' base.
    EXPECT_TRUE(plant.GetFrameByName("base", plant.GetModelInstanceByName("mid_level_model"))
    ...
    EXPECT_TRUE(plant.GetFrameByName("base", plant.GetModelInstanceByName("bottom_level_model"))
                        .CalcPoseInWorld(*context)
                        .translation()
...
GTEST_TEST(ProcessModelDirectivesTest, CollisionFilterGroupSmokeTest) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/collision_filter_group.dmd.yaml"));
    ...
    expected_report.AddGroup("group_4567", {"model4::base", "model5::base", "model6::base", "model7::base"});
    expected_report.AddGroup("group_67", {"model6::base", "model7::base"});
    expected_report.AddGroup("nested::across_sub_models", {"nested::sub_model1::base", "nested::sub_model2::base"});
    expected_report.AddGroup("nested_group", {"model3::base"});
    expected_report.AddGroup("nested_members", {"model1::base", "nested::sub_model2::base"});
    expected_report.AddExclusionPair({"across_models", "across_models"});
    expected_report.AddExclusionPair({"group_4567", "group_4567"});
    expected_report.AddExclusionPair({"nested::across_sub_models", "nested::across_sub_models"});
    expected_report.AddExclusionPair({"nested::across_sub_models", "nested_group"});
...
GTEST_TEST(ProcessModelDirectivesTest, CollisionFilterGroupNoSceneGraph) {
    ...
    DRAKE_EXPECT_NO_THROW(ProcessModelDirectives(directives, &plant, nullptr, make_parser(&plant).get()));
}
...
- add_collision_filter_group:
    ...
    DiagramBuilder<double> builder;
    auto [plant, scene_graph] = AddMultibodyPlantSceneGraph(&builder, 0.);
    DRAKE_EXPECT_THROWS_MESSAGE(ProcessModelDirectives(directives, &plant, nullptr, make_parser(&plant).get()),
...
GTEST_TEST(ProcessModelDirectivesTest, DefaultPositions) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/default_positions.dmd.yaml"));
    ...
    auto context = plant.CreateDefaultContext();
    const math::RigidTransformd X_WB(math::RollPitchYawd(5 * M_PI / 180, 6 * M_PI / 180, 7 * M_PI / 180),
                                     Eigen::Vector3d(1, 2, 3));
    EXPECT_TRUE(plant.GetFreeBodyPose(*context, plant.GetBodyByName("base")).IsNearlyEqualTo(X_WB, 1e-14));
    EXPECT_EQ(plant.GetJointByName<RevoluteJoint>("ShoulderJoint").get_angle(*context), 0.1);
    EXPECT_EQ(plant.GetJointByName<RevoluteJoint>("ElbowJoint").get_angle(*context), 0.2);
}
...
GTEST_TEST(ProcessModelDirectivesTest, ErrorMessages) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(LoadModelDirectives("no-such-file.yaml"), ".*no-such-file.yaml.*");

    // User specifies a package that hasn't been properly registered.
    {
        MultibodyPlant<double> plant(0.0);
        ModelDirectives directives =
                LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/bad_package_uri.dmd.yaml"));
        DRAKE_EXPECT_THROWS_MESSAGE(ProcessModelDirectives(directives, &plant, nullptr, make_parser(&plant).get()),
...
GTEST_TEST(ProcessModelDirectivesTest, DeepNestedChildWelds) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/deep_child_weld.dmd.yaml"));
    MultibodyPlant<double> plant(0.0);
...
GTEST_TEST(ProcessModelDirectivesTest, DeepNestedChildFrameWelds) {
    ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/deep_child_frame_weld.dmd.yaml"));
    MultibodyPlant<double> plant(0.0);
...
GTEST_TEST(ProcessModelDirectivesTest, Flatten) {
    ...
    std::vector<ModelInstanceInfo> reflattened_models;
    {
        ...
        FlattenModelDirectives(directives, parser->package_map(), &flat_directives);
        FlattenModelDirectives(flat_directives, parser->package_map(), &reflattened_directives);
        reflattened_models = ProcessModelDirectives(reflattened_directives, parser.get());
    }
...
GTEST_TEST(ProcessModelDirectivesTest, FlattenCollisionGroups) {
    ...
    const ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/unflattened_top.dmd.yaml"));
    ...
    const ModelDirectives expected_directives =
            LoadModelDirectives(FindResourceOrThrow(std::string(kTestDir) + "/flattened.dmd.yaml"));
    ...
    // Check that both flattened and expected can be processed.
    {
        ...
        std::vector<ModelInstanceInfo> added_models = ProcessModelDirectives(flat_directives, parser.get());
    ...
    }
    {
        ...
        geometry::SceneGraph<double> scene_graph;
        std::unique_ptr<Parser> parser = make_parser(&plant, &scene_graph);
        std::vector<ModelInstanceInfo> added_models = ProcessModelDirectives(expected_directives, parser.get());
        ASSERT_EQ(added_models.size(), 4);
    }
...
directives:
  - add_directives:
    ...
    ModelDirectives flat_directives;
    FlattenModelDirectives(directives, parser->package_map(), &flat_directives);
    EXPECT_EQ(flat_directives.directives[1].add_frame.value().X_PF.base_frame.value(),
...
GTEST_TEST(ProcessModelDirectivesTest, WeldOffset) {
    const ModelDirectives directives =
            LoadModelDirectives(FindResourceOrThrow("drake/manipulation/util/test/panda_arm_and_hand.dmd.yaml"));
    MultibodyPlant<double> plant(0.0);
    std::vector<ModelInstanceInfo> added_models = ProcessModelDirectives(directives, make_parser(&plant).get());
    plant.Finalize();
    ...
    const Frame<double>& frame_P = plant.GetFrameByName(added_models[1].parent_frame_name);
    const Frame<double>& frame_C = plant.GetFrameByName(added_models[1].child_frame_name);
    const math::RigidTransformd panda_hand_X_PC_expected = plant.CalcRelativeTransform(*context, frame_P, frame_C);
