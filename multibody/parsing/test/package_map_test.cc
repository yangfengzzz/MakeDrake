...
string GetTestDataRoot() {
    const string desired_dir = "drake/multibody/parsing/test/package_map_test_packages/";
    const string contained_file = "package_map_test_package_a/package.xml";
    const string absolute_file_path = FindResourceOrThrow(desired_dir + contained_file);
    return absolute_file_path.substr(0, absolute_file_path.size() - contained_file.size());
}

void VerifyMatch(const PackageMap& package_map, const map<string, string>& expected_packages) {
    ...
    for (const auto& [package_name, package_path] : expected_packages) {
        ...
        const bool should_be_deprecated =
                (package_name == "package_map_test_package_b") || (package_name == "package_map_test_package_d");
        EXPECT_EQ(deprecation.has_value(), should_be_deprecated) << "for " << package_name;
        EXPECT_EQ(!deprecation.value_or("").empty(), should_be_deprecated) << "for " << package_name;
    }
...
void VerifyMatchWithTestDataRoot(const PackageMap& package_map) {
    ...
    map<string, string> expected_packages = {
            ...
            {"package_map_test_package_aa", root_path + "package_map_test_package_a/package_map_test_package_aa/"},
            {"package_map_test_package_b", root_path + "package_map_test_package_b/"},
            {"package_map_test_package_c", root_path + "package_map_test_package_set/package_map_test_package_c/"},
            {"package_map_test_package_d", root_path + "package_map_test_package_set/package_map_test_package_d/"},
            {"package_map_test_package_e", root_path + "package_map_test_package_e/"},
    };
...
GTEST_TEST(PackageMapTest, TestManualPopulation) {
    ...
    map<string, string> expected_packages = {{"package_foo", "package_foo"}, {"my_package", "package_bar"}};
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(package_map.Add("package_foo", "package_baz"), ".*paths are not eq.*");
    // Adding a package with a nonexistent path throws.
    DRAKE_EXPECT_THROWS_MESSAGE(package_map.Add("garbage", "garbage"), ".*does not exist.*");
...
GTEST_TEST(PackageMapTest, TestAddMap) {
    ...
    fs::create_directory("package_bar");
    fs::create_directory("package_baz");
    map<string, string> expected_packages_1 = {{"package_foo", "package_foo"}, {"package_bar", "package_bar"}};
    map<string, string> expected_packages_2 = {{"package_foo", "package_foo"}, {"package_baz", "package_baz"}};
    map<string, string> expected_packages_combined = {
            {"package_foo", "package_foo"}, {"package_bar", "package_bar"}, {"package_baz", "package_baz"}};
    map<string, string> expected_packages_conflicting = {{"package_foo", "package_foo"},
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(package_map_1_copy.AddMap(package_map_conflicting), ".*paths are not eq.*");
}
...
GTEST_TEST(PackageMapTest, TestPopulateFromXml) {
    ...
    const string conflicting_xml_filename = FindResourceOrThrow(
            ...
            "package.xml");
    DRAKE_EXPECT_THROWS_MESSAGE(package_map.AddPackageXml(conflicting_xml_filename), ".*paths are not eq.*");
...
GTEST_TEST(PackageMapTest, TestPopulateFromRosPackagePath) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(package_map.PopulateFromEnvironment("ROS_PACKAGE_PATH"),
...
GTEST_TEST(PackageMapTest, TestStreamingToString) {
    ...
    package_map.AddRemote("remote", {.urls = {url}, .sha256 = std::string(64u, '0')});
...
GTEST_TEST(PackageMapTest, TestResolveUrl) {
    ...
    const string filename = package_map.ResolveUrl("package://package_map_test_package_a/sdf/test_model.sdf");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(package_map.ResolveUrl("package://bad_package_name/sdf/test_model.sdf"),
                                ".*unknown package.*");

    DRAKE_EXPECT_THROWS_MESSAGE(package_map.ResolveUrl("package://package_map_test_package_a/bad_filename.sdf"),
                                ".*does not exist.*");
}
...
GTEST_TEST(PackageMapTest, TestDeprecation) {
    ...
    for (const auto& package_name : package_map.GetPackageNames()) {
        ...
        if (expected_message != expected_deprecations.end()) {
            EXPECT_EQ(package_map.GetDeprecated(package_name), expected_message->second);
        } else {
