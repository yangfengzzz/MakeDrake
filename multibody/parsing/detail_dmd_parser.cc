#include "multibody/parsing/detail_dmd_parser.h"
...
void ParseModelDirectivesImpl(const ModelDirectives& directives,
    ...
    auto get_scoped_frame = [plant = plant, &model_namespace](const std::string& name) -> const Frame<double>& {
        return GetScopedFrameByName(*plant, DmdScopedNameJoin(model_namespace, name).to_string());
    };
    ...
    for (auto& directive : directives.directives) {
        if (directive.add_model) {
            ...
            const std::string name = ScopedName::Join(model_namespace, model.name).to_string();
            drake::log()->debug("  add_model: {}\n    {}", name, model.file);
            const std::string file = ResolveUri(diagnostic, model.file, package_map, {});
            std::optional<ModelInstanceIndex> child_model_instance_id =
                    parser_selector(diagnostic, file)
                            .AddModel({DataSource::kFilename, &file}, model.name, model_namespace, workspace);
            if (!child_model_instance_id.has_value()) {
                // Error should have already been emitted.
                continue;
            }
            for (const auto& [joint_name, positions] : directive.add_model->default_joint_positions) {
                plant->GetMutableJointByName(joint_name, child_model_instance_id).set_default_positions(positions);
            }
            for (const auto& [element_name, pose] : directive.add_model->default_free_body_pose) {
                const Frame<double>& child_frame = plant->GetFrameByName(element_name, *child_model_instance_id);
                if (pose.base_frame.has_value() && *pose.base_frame != "world") {
                    ...
                    const Frame<double>& parent_frame = get_scoped_frame(*pose.base_frame);
                    plant->AddJoint(
                            std::make_unique<QuaternionFloatingJoint<double>>(joint_name, parent_frame, child_frame));
                    auto& joint = plant->GetMutableJointByName(joint_name, child_model_instance_id);
                    joint.SetDefaultPose(pose.GetDeterministicValue());
                } else {
                    // We can only call this if we haven't injected the floating joint.
                    plant->SetDefaultFreeBodyPose(child_frame.body(), pose.GetDeterministicValue());
                }
            }
            ...
            const std::string name = MakeModelName(instance.name, model_namespace, workspace);
            ...
            if (!frame.X_PF.base_frame) {
                ...
                throw std::logic_error("add_frame directive with empty base frame is ambiguous");
            }
            ...
            if (!parsed.get_namespace().empty()) {
                ...
                parsed.set_namespace(ScopedName::Join(model_namespace, parsed.get_namespace()).get_full());
            ...
            auto& added = plant->AddFrame(std::make_unique<FixedOffsetFrame<double>>(
                    std::string{parsed.get_element()}, get_scoped_frame(*frame.X_PF.base_frame),
                    frame.X_PF.GetDeterministicValue(), instance));
            const std::string resolved_name =
                    ScopedName::Join(plant->GetModelInstanceName(added.model_instance()), added.name()).to_string();
            ...
            AddWeld(get_scoped_frame(directive.add_weld->parent), get_scoped_frame(directive.add_weld->child), X_PC,
                    plant, added_models);
            ...
            std::set<std::string> member_set(group.members.begin(), group.members.end());
            std::set<std::string> member_groups_set(group.member_groups.begin(), group.member_groups.end());
            collision_resolver->AddGroup(diagnostic, group.name, member_set, member_groups_set, model_instance);
            for (const auto& ignored_group : group.ignored_collision_filter_groups) {
                collision_resolver->AddPair(diagnostic, group.name, ignored_group, model_instance);
            }
            ...
            std::string new_model_namespace =
                    ScopedName::Join(model_namespace, sub.model_namespace.value_or("")).to_string();
            ...
            if (!new_model_namespace.empty() && !plant->HasModelInstanceNamed(new_model_namespace)) {
                throw std::runtime_error(
                        fmt::format("Namespace '{}' does not exist as model instance", new_model_namespace));
            }
            std::string filename = ResolveUri(diagnostic, sub.file, package_map, {});
            auto sub_directives = LoadModelDirectives({DataSource::kFilename, &filename});
            ParseModelDirectivesImpl(sub_directives, new_model_namespace, workspace, added_models);
        }
    }
}
...
ModelDirectives LoadModelDirectives(const DataSource& data_source) {
    ...
    if (data_source.IsFilename()) {
        ...
        if (!std::filesystem::exists({filename})) {
            throw std::runtime_error(fmt::format("No such file {} during LoadModelDirectives", filename));
        }

        directives = yaml::LoadYamlFile<ModelDirectives>(filename, std::nullopt /* child_name */, defaults);
    } else {
        DRAKE_DEMAND(data_source.IsContents());
        directives =
                yaml::LoadYamlString<ModelDirectives>(data_source.contents(), std::nullopt /* child_name */, defaults);
    }
    DRAKE_THROW_UNLESS(directives.IsValid());
    return directives;
}

std::vector<ModelInstanceInfo> ParseModelDirectives(const ModelDirectives& directives,
                                                    const std::optional<std::string>& parent_model_name,
                                                    const ParsingWorkspace& workspace) {
    std::vector<ModelInstanceInfo> added_models;
    ParseModelDirectivesImpl(directives, parent_model_name.value_or(""), workspace, &added_models);
    return added_models;
}
...
std::optional<ModelInstanceIndex> DmdParserWrapper::AddModel(const DataSource& data_source,
                                                             const std::string&,
                                                             const std::optional<std::string>&,
                                                             const ParsingWorkspace& workspace) {
    std::string display_source =
            data_source.IsFilename() ? data_source.GetAbsolutePath() : data_source.GetStem() + ".dmd.yaml";
    workspace.diagnostic.Error(
            fmt::format("'{}' is a model directives data source; it is always an error to pass"
...
std::vector<ModelInstanceIndex> DmdParserWrapper::AddAllModels(const DataSource& data_source,
                                                               const std::optional<std::string>& parent_model_name,
                                                               const ParsingWorkspace& workspace) {
    // TODO(#18052): diagnostic policy?
    ModelDirectives directives = LoadModelDirectives(data_source);
    // TODO(#18052): diagnostic policy?
    const std::vector<ModelInstanceInfo> infos = ParseModelDirectives(directives, parent_model_name, workspace);
