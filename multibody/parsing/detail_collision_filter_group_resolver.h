...
public:
    ...
    ModelInstanceIndex minimum_model_instance_index() const { return minimum_model_instance_index_; }
    ...
    CollisionFilterGroupsImpl<InstancedName> Resolve(const drake::internal::DiagnosticPolicy& diagnostic);
...
private:
    ...
    std::string FullyQualify(const std::string& name, std::optional<ModelInstanceIndex> model_instance) const;

    const GroupData* FindGroup(const drake::internal::DiagnosticPolicy& diagnostic,
                               const std::string& group_name) const;

    const RigidBody<double>* FindBody(std::string_view name, ModelInstanceIndex model_instance) const;
