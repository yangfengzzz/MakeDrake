        ...
        if (error) {
            auto mat_descrip = [](const UrdfMaterial& mat) {
                std::string rgb_string =
                        mat.rgba.has_value() ? fmt::format("RGBA: {}", fmt_eigen(mat.rgba->transpose())) : "RGBA: None";
                std::string map_string = mat.diffuse_map.has_value()
...
UrdfMaterial ParseMaterial(const TinyXml2Diagnostic& diagnostic,
    ...
    if (std::string(node->Name()) != "material") {
        diagnostic.Error(*node, fmt::format("Expected material element, got <{}>", node->Name()));
        return {};
    }
    ...
    const XMLElement* drake_diffuse_map_node = node->FirstChildElement("drake:diffuse_map");
    ...
    if (texture_node) {
        ...
        if (ParseStringAttribute(texture_node, "filename", &texture_name) && !texture_name.empty()) {
            texture_path = ResolveUri(diagnostic.MakePolicyForNode(texture_node), texture_name, package_map, root_dir);
    ...
    if (color_node) {
        ...
        if (!ParseVectorAttribute(color_node, "rgba", &rgba_value)) {
            diagnostic.Error(*color_node, "Failed to parse 'rgba' attribute of <color>}");
            return {};
        }
        rgba = rgba_value;
    }
    ...
    if (!rgba && !texture_path) {
        if (!name.empty() && materials->find(name) == materials->end()) {
            ...
            // defined.
            diagnostic.Error(*node, fmt::format("Material '{}' not previously defined, but has no"
    ...
    if (!name.empty()) {
        ...
        // error.
        material = AddMaterialToMaterialMap(diagnostic.MakePolicyForNode(node), name, material,
...
std::unique_ptr<geometry::Shape> ParseBox(const TinyXml2Diagnostic& diagnostic, const XMLElement* shape_node) {
...
std::unique_ptr<geometry::Shape> ParseSphere(const TinyXml2Diagnostic& diagnostic, const XMLElement* shape_node) {
    double r = 0;
    if (!ParseScalarAttribute(shape_node, "radius", &r, diagnostic.MakePolicyForNode(shape_node))) {
...
std::unique_ptr<geometry::Shape> ParseCylinder(const TinyXml2Diagnostic& diagnostic, const XMLElement* shape_node) {
    double r = 0;
    if (!ParseScalarAttribute(shape_node, "radius", &r, diagnostic.MakePolicyForNode(shape_node))) {
    ...
    double l = 0;
    if (!ParseScalarAttribute(shape_node, "length", &l, diagnostic.MakePolicyForNode(shape_node))) {
...
std::unique_ptr<geometry::Shape> ParseCapsule(const TinyXml2Diagnostic& diagnostic, const XMLElement* shape_node) {
    double r = 0;
    if (!ParseScalarAttribute(shape_node, "radius", &r, diagnostic.MakePolicyForNode(shape_node))) {
...
std::unique_ptr<geometry::Shape> ParseEllipsoid(const TinyXml2Diagnostic& diagnostic, const XMLElement* shape_node) {
    double axes[3];
    const char* names[] = {"a", "b", "c"};
    for (int i = 0; i < 3; ++i) {
        if (!ParseScalarAttribute(shape_node, names[i], &axes[i], diagnostic.MakePolicyForNode(shape_node))) {
            diagnostic.Error(*shape_node, fmt::format("Missing ellipsoid attribute: '{}'", names[i]));
            return {};
        }
    }
...
std::unique_ptr<geometry::Shape> ParseMesh(const TinyXml2Diagnostic& diagnostic,
    ...
    const std::string resolved_filename =
            ResolveUri(diagnostic.MakePolicyForNode(shape_node), filename, package_map, root_dir);
    ...
    if (shape_node->Attribute("scale") != nullptr) {
        ...
        // enforce it.
        if (!(scale_vector(0) == scale_vector(1) && scale_vector(0) == scale_vector(2))) {
...
std::unique_ptr<geometry::Shape> ParseGeometry(const TinyXml2Diagnostic& diagnostic,
    ...
    } else if (child_node = node->FirstChildElement("drake:capsule"); child_node) {
    ...
    } else if (child_node = node->FirstChildElement("drake:ellipsoid"); child_node) {
...
std::optional<std::string> ParseGeometryName(const char* visual_or_collision,
    ...
    if (geometry_names->contains(result)) {
        for (int i = 1;; ++i) {
            if (i >= numeric_name_suffix_limit) {
                policy.Error(fmt::format("Too many geometries with identical name '{}'", result));
                return {};
            }
            std::string guess = fmt::format("{}_{}", result, i);
            if (!geometry_names->contains(guess)) {
                if (explicitly_named) {
                    policy.Warning(fmt::format("{} name '{}' has already been used, renaming to '{}' instead",
...
std::optional<geometry::GeometryInstance> ParseVisual(const TinyXml2Diagnostic& diagnostic,
    ...
    if (std::string(node->Name()) != "visual") {
        diagnostic.Error(*node,
                         fmt::format("In link {} expected visual element, got {}", parent_element_name, node->Name()));
        return {};
    }
    ...
    if (!geometry_node) {
        diagnostic.Error(*node, fmt::format("Link {} has a visual element without geometry.", parent_element_name));
        return {};
    }
    ...
    std::unique_ptr<geometry::Shape> shape = ParseGeometry(diagnostic, geometry_node, package_map, root_dir);
    if (!shape) {
    ...
    if (material_node) {
        UrdfMaterial material =
                ParseMaterial(diagnostic, material_node, false /* name required */, package_map, root_dir, materials);
        if (material.rgba) {
    ...
    if (accepting_node) {
        ...
        while (accepting_node) {
            ...
            if (!ParseStringAttribute(accepting_node, "name", &name)) {
                diagnostic.Error(*accepting_node, fmt::format("<{}> tag given without any name", kAcceptingTag));
                return {};
            }
    ...
    std::optional<std::string> geometry_name =
            ParseGeometryName("visual", diagnostic, node, *shape, geometry_names, numeric_name_suffix_limit);
    if (!geometry_name) {
        return {};
    }

    auto instance = geometry::GeometryInstance(T_element_to_link, std::move(shape), *geometry_name);
...
// body tree days.
CoulombFriction<double> ParseCoulombFrictionFromDrakeCompliance(const TinyXml2Diagnostic& diagnostic,
                                                                const std::string& parent_element_name,
                                                                const XMLElement* node) {
    const XMLElement* compliant_node = node->FirstChildElement("drake_compliance");
    if (compliant_node) {
        ...
        if (compliant_node->FirstChildElement("youngs_modulus")) {
            diagnostic.Warning(*compliant_node,
        ...
        const XMLElement* friction_node = compliant_node->FirstChildElement("static_friction");
        if (friction_node) {
            static_friction_present = true;
            if (friction_node->QueryDoubleText(&static_friction)) {
                diagnostic.Error(*friction_node,
                                 fmt::format("Unable to parse static_friction for link {}", parent_element_name));
        ...
        if (friction_node) {
            ...
            if (friction_node->QueryDoubleText(&dynamic_friction)) {
                diagnostic.Error(*friction_node,
                                 fmt::format("Unable to parse dynamic_friction for link {}", parent_element_name));
                return default_friction();
            }
        }

        if (static_friction_present != dynamic_friction_present) {
            diagnostic.Error(*compliant_node, fmt::format("Link '{}': When specifying coefficient of friction,"
...
std::optional<geometry::GeometryInstance> ParseCollision(const TinyXml2Diagnostic& diagnostic,
    ...
    if (std::string(node->Name()) != "collision") {
        diagnostic.Error(*node, fmt::format("In link '{}' expected collision element, got {}", parent_element_name,
                                            node->Name()));
        return {};
    }
    ...
    const XMLElement* geometry_node = node->FirstChildElement("geometry");
    if (!geometry_node) {
        diagnostic.Error(*node, fmt::format("Link '{}' has a collision element without geometry", parent_element_name));
        return {};
    }
    ...
    const XMLElement* drake_element = node->FirstChildElement("drake:proximity_properties");
    if (drake_element) {
        auto read_double = [drake_element, &diagnostic](const char* element_name) -> std::optional<double> {
            const XMLElement* value_node = drake_element->FirstChildElement(element_name);
            if (value_node != nullptr) {
                double value{};
                if (ParseScalarAttribute(value_node, "value", &value, diagnostic.MakePolicyForNode(value_node))) {
                    return value;
                } else {
                    diagnostic.Error(*value_node, fmt::format("Unable to read the 'value' attribute for the"
                                                              " <{}> tag",
                                                              element_name));
        ...
        const XMLElement* const rigid_element = drake_element->FirstChildElement("drake:rigid_hydroelastic");
        const XMLElement* const compliant_element = drake_element->FirstChildElement("drake:compliant_hydroelastic");
        ...
        const XMLElement* const no_longer_supported = drake_element->FirstChildElement("drake:soft_hydroelastic");
        if (no_longer_supported) {
            diagnostic.Error(*no_longer_supported,
        ...
        props = ParseProximityProperties(diagnostic.MakePolicyForNode(drake_element), read_double,
    ...
    if (!props.HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction)) {
        // We have no friction from <drake:proximity_properties> so we need the old
        // tag.
        CoulombFriction<double> friction =
                ParseCoulombFrictionFromDrakeCompliance(diagnostic, parent_element_name, node);
        props.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, friction);
    } else {
    ...
    std::optional<std::string> geometry_name =
            ParseGeometryName("collision", diagnostic, node, *shape, geometry_names, numeric_name_suffix_limit);
    if (!geometry_name) {
        return {};
    }

    geometry::GeometryInstance instance(T_element_to_link, std::move(shape), *geometry_name);
