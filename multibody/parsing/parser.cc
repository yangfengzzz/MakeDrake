...
#include "multibody/parsing/detail_collision_filter_groups_impl.h"
#include "multibody/parsing/detail_common.h"
#include "multibody/parsing/detail_composite_parse.h"
#include "multibody/parsing/detail_instanced_name.h"
#include "multibody/parsing/detail_parsing_workspace.h"
#include "multibody/parsing/detail_path_utils.h"
#include "multibody/parsing/detail_select_parser.h"
...
Parser::Parser(MultibodyPlant<double>* plant, geometry::SceneGraph<double>* scene_graph)
    : Parser(plant, scene_graph, {}) {}

Parser::Parser(MultibodyPlant<double>* plant, std::string_view model_name_prefix)
...
CollisionFilterGroups Parser::GetCollisionFilterGroups() const {
    return CollisionFilterGroups(ConvertInstancedNamesToStrings(data_->collision_filter_groups_storage_, *plant_));
}

std::vector<ModelInstanceIndex> Parser::AddModels(const std::filesystem::path& file_name) {
    ...
    auto result = parser.AddAllModels(data_source, model_name_prefix_, composite->workspace());
    composite->Finish();
    return result;
}

std::vector<ModelInstanceIndex> Parser::AddModelsFromUrl(const std::string& url) {
    const std::string file_name = internal::ResolveUri(diagnostic_policy_, url, package_map_, {});
    if (file_name.empty()) {
        return {};
    }
...
std::vector<ModelInstanceIndex> Parser::AddModelsFromString(const std::string& file_contents,
...
void Parser::ResolveCollisionFilterGroupsFromCompositeParse(internal::CollisionFilterGroupResolver* resolver) {
    DRAKE_DEMAND(resolver != nullptr);

    const CollisionFilterGroupsImpl<internal::InstancedName> groups = resolver->Resolve(diagnostic_policy_);
