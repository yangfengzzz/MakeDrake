...
std::unique_ptr<CompositeParse> CompositeParse::MakeCompositeParse(Parser* parser) {
...
CompositeParse::CompositeParse(Parser* parser)
    : parser_(parser),
      ...
      workspace_(
              options_, parser->package_map(), parser->diagnostic_policy_, &parser->plant(), &resolver_, SelectParser) {
}
