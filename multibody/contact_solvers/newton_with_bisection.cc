...
std::pair<double, int> DoNewtonWithBisectionFallback(const std::function<std::pair<double, double>(double)>& function,
    ...
    if (abs(bracket.f_lower()) < f_tolerance) return std::make_pair(bracket.x_lower(), 0);

    if (abs(bracket.f_upper()) < f_tolerance) return std::make_pair(bracket.x_upper(), 0);
    ...
    for (int num_evaluations = 1; num_evaluations <= max_iterations; ++num_evaluations) {
        ...
        if (abs(minus_dx) < x_tolerance) return std::make_pair(root, num_evaluations);
    }
