...
public:
    ...
    Block3x3SparseMatrix(int block_rows, int block_cols)
        : row_data_(block_rows), block_rows_(block_rows), block_cols_(block_cols), col_to_indices_(block_cols) {
    ...
    */
    ...
    /* Performs y += A * M.
     @pre y != nullptr and the sizes of A and y are compatible with this matrix.
    */
    void LeftMultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A, EigenPtr<MatrixX<T>> y) const;
    ...
    */
    ...
    /* Performs y += Mᵀ * A, where A is also 3x3 block sparse.
     @pre y != nullptr and the sizes of A and y are compatible with this matrix.
    */
    void TransposeAndMultiplyAndAddTo(const Block3x3SparseMatrix<T>& A, EigenPtr<MatrixX<T>> y) const;
    ...
    /* Returns the non-zero blocks in the matrix in a row-major fashion. Within
     ...
     block row. */
    const std::vector<std::vector<Triplet>>& get_triplets() const { return row_data_; }
