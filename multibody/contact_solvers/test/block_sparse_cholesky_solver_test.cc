...
BlockSparseSymmetricMatrix MakeSparseSpdMatrix(double scale = 1.0) {
    ...
    std::vector<std::pair<int, int>> nonzero_lower_triangular_blocks{{0, 0}, {1, 1}, {2, 1}, {2, 2}, {3, 1}, {3, 3}};
    for (const auto& [a, b] : nonzero_lower_triangular_blocks) {
        A.AddToBlock(a, b, dense_A.block(starting_cols[a], starting_cols[b], block_sizes[a], block_sizes[b]));
    }
    return A;
}
...
GTEST_TEST(BlockSparseCholeskySolverTest, Solve) {
    ...
    EXPECT_EQ(solver.solver_mode(), BlockSparseCholeskySolver<MatrixXd>::SolverMode::kEmpty);
    solver.SetMatrix(A);
    EXPECT_EQ(solver.solver_mode(), BlockSparseCholeskySolver<MatrixXd>::SolverMode::kAnalyzed);
    bool success = solver.Factor();
    EXPECT_TRUE(success);
    EXPECT_EQ(solver.solver_mode(), BlockSparseCholeskySolver<MatrixXd>::SolverMode::kFactored);
...
GTEST_TEST(BlockSparseCholeskySolverTest, FailureDueToNonSpdness) {
    ...
    {
        for (const std::unordered_set<int>& eliminated_blocks : std::vector<std::unordered_set<int>>{
                     std::unordered_set<int>{}, std::unordered_set<int>{0}, std::unordered_set<int>{0, 1}}) {
            const std::optional<MatrixXd> schur_complement = solver.FactorAndCalcSchurComplement(A, eliminated_blocks);
...
GTEST_TEST(BlockSparseCholeskySolverTest, PermutationMatrix) {
    ...
    const Eigen::PermutationMatrix<Eigen::Dynamic> P = solver.CalcPermutationMatrix();
...
GTEST_TEST(BlockSparseCholeskySolverTest, PermutationMatrixPrecondition) {
    ...
    solver.SetMatrix(A);
    const Eigen::PermutationMatrix<Eigen::Dynamic> P0 = solver.CalcPermutationMatrix();
    const bool success = solver.Factor();
    EXPECT_TRUE(success);
    const Eigen::PermutationMatrix<Eigen::Dynamic> P1 = solver.CalcPermutationMatrix();
...
GTEST_TEST(BlockSparseCholeskySolverTest, SolverModeAfterMove) {
    ...
    EXPECT_EQ(new_solver.solver_mode(), BlockSparseCholeskySolver<MatrixXd>::SolverMode::kAnalyzed);
...
GTEST_TEST(BlockSparseCholeskySolverTest, FactorAndCalcSchurComplement) {
    ...
    /* All blocks are eliminated. */
    {
        ...
        const std::optional<MatrixXd> schur_complement = solver.FactorAndCalcSchurComplement(
                M, std::unordered_set<int>(eliminated_blocks.begin(), eliminated_blocks.end()));
    ...
    /* Some of the blocks are eliminated. */
    {
        ...
        const std::optional<MatrixXd> schur_complement = solver.FactorAndCalcSchurComplement(M, eliminated_blocks);
        EXPECT_TRUE(schur_complement.has_value());
        ...
        EXPECT_TRUE(CompareMatrices(schur_complement.value(), expected_schur_complement, 1e-14));
        EXPECT_EQ(solver.solver_mode(), BlockSparseCholeskySolver<MatrixXd>::SolverMode::kFactored);
    }
}
