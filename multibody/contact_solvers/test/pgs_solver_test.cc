...
#include "multibody/contact_solvers/sparse_linear_operator.h"
...
protected:
    ...
    void SetZeroPointContactData() {
        ...
        mu_.resize(0);
        point_data_ = std::make_unique<PointContactData<double>>(&penetration_depth_, Jc_.get(), &stiffness_,
                                                                 &dissipation_, &mu_);
    }
    ...
    void SetSystemDynamicsData() {
        ...
        dynamics_data_ = std::make_unique<SystemDynamicsData<double>>(Ainv_.get(), &v_star_);
    }
...
TEST_F(PgsTest, Stiction) {
    ...
    EXPECT_EQ(pgs_.SolveWithGuess(dt, *dynamics_data_, *point_data_, v_guess, &result), ContactSolverStatus::kSuccess);
    EXPECT_TRUE(CompareMatrices(result.ft, Vector2<double>(1. / 12., 1. / 4.), kTol));
    ...
    EXPECT_TRUE(CompareMatrices(result.tau_contact, Vector3<double>(1. / 12., 1. / 4., 1. / 3.), kTol));
    VerifySolverStats();
}
...
TEST_F(PgsTest, Sliding) {
    ...
    EXPECT_TRUE(CompareMatrices(result.fn, Vector1<double>(1. / 3.), kTol));
    EXPECT_TRUE(CompareMatrices(result.v_next, Vector3<double>(-1.0, -1.0, 0), kTol));
    EXPECT_TRUE(CompareMatrices(result.tau_contact, Vector3<double>(0, 0, 1. / 3.), kTol));
    VerifySolverStats();
}
...
TEST_F(PgsTest, NoContact) {
    ...
    EXPECT_TRUE(CompareMatrices(result.v_next, Vector3<double>(-1.0, -1.0, -1.0), kTol));
    EXPECT_TRUE(CompareMatrices(result.tau_contact, Vector3<double>::Zero(), kTol));
    VerifySolverStats();
}
}  // namespace
}  // namespace internal
