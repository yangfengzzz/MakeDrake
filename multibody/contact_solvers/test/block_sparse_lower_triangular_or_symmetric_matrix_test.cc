...
GTEST_TEST(TriangularBlockSparseMatrixTest, Construction) {
    const BlockSparseLowerTriangularMatrix A_triangular = MakeLowerTriangularMatrix();
...
GTEST_TEST(TriangularBlockSparseMatrixTest, ZeroRowsAndColumns) {
    ...
    BlockSparseLowerTriangularMatrix A_triangular = MakeLowerTriangularMatrix();
    DRAKE_EXPECT_THROWS_MESSAGE(A_triangular.ZeroRowsAndColumns({0, 1}), ".*is_symmetric.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(A_symmetric.ZeroRowsAndColumns({42}), ".* out of range.*42 is given.*");
}
...
GTEST_TEST(TriangularBlockSparseMatrixTest, InvalidOperations) {
    if (kDrakeAssertIsArmed) {
        ...
        BlockSparseSymmetricMatrix A_symmetric = MakeSymmetricMatrix();
        /* i <= block_rows() fails. */
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_triangular.AddToBlock(200, 100, A00), ".*out of bound.*");
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_symmetric.AddToBlock(200, 100, A00), ".*out of bound.*");
        /* j <= i fails. */
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_triangular.AddToBlock(0, 1, MakeArbitraryMatrix(2, 3)),
                                             ".*out of bound.*");
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_symmetric.AddToBlock(0, 1, MakeArbitraryMatrix(2, 3)),
                                             ".*out of bound.*");
        /* 0 <= j fails. */
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_triangular.AddToBlock(0, -1, MakeArbitraryMatrix(2, 3)),
                                             ".*out of bound.*");
        ...
        /* Non-existing block. */
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_triangular.AddToBlock(2, 0, MakeArbitraryMatrix(2, 4)),
                                             ".*doesn't exist.*");
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_symmetric.AddToBlock(2, 0, MakeArbitraryMatrix(2, 4)),
                                             ".*doesn't exist.*");

        MatrixXd non_symmetric_matrix = MatrixXd::Zero(4, 4);
        non_symmetric_matrix(1, 0) = 0.1;
        /* Non-symmetric diagonal block. */
        DRAKE_ASSERT_THROWS_MESSAGE_IF_ARMED(A_symmetric.AddToBlock(2, 2, non_symmetric_matrix),
                                             ".*must be symmetric[^]*");
        ...
        ASSERT_THROW(A_triangular.SetBlock(0, 1, MakeArbitraryMatrix(2, 3)), std::exception);
        ASSERT_THROW(A_symmetric.SetBlockFlat(0, 2, non_symmetric_matrix), std::exception);
    }
}
