...
GTEST_TEST(MatrixBlockTest, Constructors) {
    ...
    EXPECT_TRUE(CompareMatrices(sparse_block.MakeDenseMatrix(), dense_block.MakeDenseMatrix()));
}
...
GTEST_TEST(MatrixBlockTest, LeftMultiplyByBlockDiagonal) {
    ...
    const int start = 2;
    const int end = 5;
    const MatrixBlock<double> sparse_result = sparse_block.LeftMultiplyByBlockDiagonal(Gs, start, end);
    const MatrixBlock<double> dense_result = dense_block.LeftMultiplyByBlockDiagonal(Gs, start, end);
...
GTEST_TEST(MatrixBlockTest, LeftMultiplyByBlockDiagonalWithNon3x3GBlocks) {
    ...
    const int start = 3;
    const int end = 5;
    const MatrixBlock<double> result = sparse_block.LeftMultiplyByBlockDiagonal(Gs, start, end);
    ...
    EXPECT_TRUE(CompareMatrices(result.MakeDenseMatrix(), expected, 16 * std::numeric_limits<double>::epsilon()));
}
...
GTEST_TEST(MatrixBlockTest, StackMatrixBlock) {
    ...
    std::vector<MatrixBlock<double>> sparse_blocks = {sparse_block1, sparse_block2};

    MatrixXd expected(dense_matrix1.rows() + dense_matrix2.rows(), dense_matrix1.cols());
