...
SparseMatrixd MakeMeshInContactJacobian(int num_vertices, const std::vector<int>& vertices_in_contact) {
...
TEST_F(ContactJacobianTest, AssembleMatrix) {
    ...
    Eigen::Map<VectorX<int>> Jcopy_inner(Jcopy.innerIndexPtr(), Jcopy.innerSize());
    Eigen::Map<VectorX<int>> J_inner(J_.innerIndexPtr(), J_.innerSize());
    EXPECT_EQ(Jcopy_inner, J_inner);

    Eigen::Map<VectorX<int>> Jcopy_outer(Jcopy.outerIndexPtr(), Jcopy.outerSize());
