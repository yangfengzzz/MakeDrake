...
struct RootFindingTestData {
    ...
    // when a test fails.
    friend std::ostream& operator<<(std::ostream& os, const RootFindingTestData& data) {
        return os << "{\n"
                  << "  Function: " << data.description << std::endl
                  << "  [a , b ] = [" << data.a << ", " << data.b << "]" << std::endl
                  << "  guess = " << data.guess << std::endl
                  << "  [fa, fb] = [" << data.function(data.a).first << ", " << data.function(data.b).second << "]"
                  << std::endl
...
TEST_P(RootFindingTest, VerifyExpectedResults) {
    ...
    // Create bracket.
    const Bracket bracket(data.a, data.function(data.a).first, data.b, data.function(data.b).first);

    // Find root in the interval [data.a, data.b]
    const auto [x, num_iterations] = DoNewtonWithBisectionFallback(data.function, bracket, data.guess, data.x_tolerance,
                                                                   data.f_tolerance, data.max_iterations);

    // Verify DoNewtonWithBisectionFallback converged.
    const bool root_convergence = std::abs(x - data.root) < data.x_tolerance;
    const bool function_convergence = std::abs(data.function(x).first) < data.f_tolerance;
...
//   --test_output=all
INSTANTIATE_TEST_SUITE_P(AllCases, RootFindingTest, testing::ValuesIn(GenerateTestCases()));
