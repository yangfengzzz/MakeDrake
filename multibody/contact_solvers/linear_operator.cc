...
template <typename T>
void LinearOperator<T>::DoMultiplyByTranspose(const Eigen::Ref<const Eigen::SparseVector<T>>&,
                                              Eigen::SparseVector<T>*) const {
    ThrowIfNotImplemented(__func__);
}

template <typename T>
void LinearOperator<T>::DoMultiplyByTranspose(const Eigen::Ref<const VectorX<T>>&, VectorX<T>*) const {
    ThrowIfNotImplemented(__func__);
}
