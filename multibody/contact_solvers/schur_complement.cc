...
SchurComplement::SchurComplement(const Block3x3SparseSymmetricMatrix& A, const std::unordered_set<int>& D_indices)
    ...
    std::optional<MatrixX<double>> S = A_solver_.FactorAndCalcSchurComplement(A, D_indices);
    if (!S) {
    ...
    DRAKE_DEMAND(A_solver_.solver_mode() == BlockSparseCholeskySolver<Matrix3<double>>::SolverMode::kFactored);
    S_ = std::move(*S);
}
...
VectorX<double> SchurComplement::SolveForX(const Eigen::Ref<const VectorX<double>>& y) const {
...
VectorX<double> SchurComplement::Solve(const Eigen::Ref<const VectorX<double>>& c) const {
