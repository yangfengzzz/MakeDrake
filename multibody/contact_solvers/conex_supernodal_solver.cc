...
const conex::SupernodalKKTSolver& solver_cast(const std::shared_ptr<void>& solver) {
    DRAKE_DEMAND(solver.get() != nullptr);
    return *static_cast<conex::SupernodalKKTSolver*>(solver.get());
}
...
void ComputeWeightMatrixTerms(const vector<MatrixBlock<double>>& J_blocks,
                              const vector<MatrixXd>& G_blocks,
                              int G_start,
                              int G_end,
                              MatrixType* result) {
    ...
    for (size_t k = 0; k < J_blocks.size(); ++k) {
        ...
        for (size_t l = 0; l < J_blocks.size(); ++l) {
            ...
            Eigen::Ref<MatrixXd> block = result->block(row_offset, col_offset, J.cols(), GJ.cols());
...
void VerifyMassMatrixPartitionRefinesJacobianPartition(const std::vector<int>& jacobian_column_block_size,
...
std::vector<int> GetMassMatrixStartingColumn(const std::vector<Eigen::MatrixXd>& mass_matrices) {
    vector<int> y;
    int col_start = 0;
    for (size_t i = 0; i < mass_matrices.size(); ++i) {
...
class DRAKE_NO_EXPORT ConexSuperNodalSolver::CliqueAssembler final : public ::conex::SupernodalAssemblerBase {
public:
    ...
    void SetWeightMatrixPointer(const std::vector<Eigen::MatrixXd>* weight_matrix) { weight_matrix_ = weight_matrix; }
...
void ConexSuperNodalSolver::CliqueAssembler::SetDenseData() {
    ...
    ComputeWeightMatrixTerms(jacobian_row_data_, *weight_matrix_, weight_start_, weight_end_, &submatrix_data_.G);
    int i = 0;
    for (const auto& pos : mass_matrix_position_) {
        submatrix_data_.G.block(pos, pos, mass_matrix_[i].rows(), mass_matrix_[i].cols()) += mass_matrix_[i];
        ++i;
    }
}

std::pair<int, int> FindPositionInClique(int element, const vector<vector<int>>& clique) {
    int i = 0;
...
SparsityData GetEliminationOrdering(int num_jacobian_row_blocks, const std::vector<BlockTriplet>& jacobian_blocks) {
...
void ConexSuperNodalSolver::Initialize(const vector<vector<int>>& cliques,
                                       int num_jacobian_row_blocks,
                                       const std::vector<BlockTriplet>& jacobian_blocks,
                                       const std::vector<Eigen::MatrixXd>& mass_matrices) {
    std::vector<int> jacobian_column_block_size = GetJacobianBlockSizesVerifyTriplets(jacobian_blocks);
    // Will throw an exception if verification fails.
    VerifyMassMatrixPartitionRefinesJacobianPartition(jacobian_column_block_size, mass_matrices);
    ...
    const std::vector<int> mass_matrix_starting_columns = GetMassMatrixStartingColumn(mass_matrices);
    int cnt = 0;
    for (const auto& c : mass_matrix_starting_columns) {
        const std::pair<int, int> position = FindPositionInClique(c, cliques);
        owned_clique_assemblers_[position.first]->AssignMassMatrix(position.second, mass_matrices[cnt]);
        ++cnt;
    }
...
ConexSuperNodalSolver::ConexSuperNodalSolver(int num_jacobian_row_blocks,
                                             const std::vector<BlockTriplet>& jacobian_blocks,
                                             const std::vector<Eigen::MatrixXd>& mass_matrices)
    : owned_clique_assemblers_(num_jacobian_row_blocks) {
    SparsityData clique_data = GetEliminationOrdering(num_jacobian_row_blocks, jacobian_blocks);

    solver_ = std::make_shared<::conex::SupernodalKKTSolver>(clique_data.variable_cliques, clique_data.data.num_vars,
                                                             clique_data.data.order, clique_data.data.supernodes,
                                                             clique_data.data.separators);

    Initialize(clique_data.variable_cliques, num_jacobian_row_blocks, jacobian_blocks, mass_matrices);
}

bool ConexSuperNodalSolver::DoSetWeightMatrix(const std::vector<Eigen::MatrixXd>& weight_matrix) {
...
void ConexSuperNodalSolver::CliqueAssembler::Initialize(std::vector<MatrixBlock<double>>&& r) {
    int num_vars = 0;
    ...
    SupernodalAssemblerBase::submatrix_data_.InitializeWorkspace(workspace_memory_.data());
}
