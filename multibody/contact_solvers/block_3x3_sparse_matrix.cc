#include "multibody/contact_solvers/block_3x3_sparse_matrix.h"
...
template <class T>
void Block3x3SparseMatrix<T>::SetFromTriplets(const std::vector<Triplet>& triplets) {
    ...
    for (int r = 0; r < block_rows_; ++r) {
        std::sort(row_data_[r].begin(), row_data_[r].end(), [](const Triplet& t1, const Triplet& t2) {
    ...
    for (int block_row = 0; block_row < static_cast<int>(row_data_.size()); ++block_row) {
        for (int flat_index = 0; flat_index < static_cast<int>(row_data_[block_row].size()); ++flat_index) {
...
template <class T>
void Block3x3SparseMatrix<T>::MultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A, EigenPtr<MatrixX<T>> y) const {
    ...
    for (const std::vector<Triplet>& row : row_data_) {
        for (const Triplet& triplet : row) {
            ...
            y->template middleRows<3>(3 * block_row) += m * A.template middleRows<3>(3 * block_col);
        }
    }
}

template <class T>
void Block3x3SparseMatrix<T>::LeftMultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A,
    ...
    for (const std::vector<Triplet>& row : row_data_) {
        for (const Triplet& triplet : row) {
            ...
            const int block_col = std::get<1>(triplet);
            const Matrix3<T>& m = std::get<2>(triplet);
            y->template middleCols<3>(3 * block_col) += A.template middleCols<3>(3 * block_row) * m;
        }
    }
}

template <class T>
void Block3x3SparseMatrix<T>::TransposeAndMultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A,
                                                           EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
    ...
    for (const std::vector<Triplet>& row : row_data_) {
        for (const Triplet& triplet : row) {
            ...
            const int block_col = std::get<1>(triplet);
            const Matrix3<T>& m = std::get<2>(triplet);
            y->template middleRows<3>(3 * block_col) += m.transpose() * A.template middleRows<3>(3 * block_row);
        }
    }
}

template <class T>
void Block3x3SparseMatrix<T>::TransposeAndMultiplyAndAddTo(const Block3x3SparseMatrix<T>& A,
                                                           EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
    DRAKE_DEMAND(rows() == A.rows());
...
template <class T>
void Block3x3SparseMatrix<T>::MultiplyWithScaledTransposeAndAddTo(const VectorX<T>& scale,
                                                                  EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
    ...
    for (int k = 0; k < block_cols_; ++k) {
        ...
        for (int a = 0; a < static_cast<int>(indices.size()); ++a) {
            ...
            for (int b = a; b < static_cast<int>(indices.size()); ++b) {
                ...
                const Matrix3<T> y_ij = M_ik * scale_block.asDiagonal() * M_kj.transpose();
