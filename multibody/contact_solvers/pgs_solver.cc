#include "multibody/contact_solvers/pgs_solver.h"
...
template <typename T>
ContactSolverStatus PgsSolver<T>::SolveWithGuess(const T& time_step,
    ...
    for (int k = 0; k < max_iters; ++k) {
        ...
        for (int i = 0; i < nc; ++i) {
            ...
            gammai_kp = gammai - omega * Dinv.template segment<3>(i3).asDiagonal() * vci_kp;
        ...
        const bool converged =
                VerifyConvergenceCriteria(nc, vc_, vc_kp, gamma, gamma_kp, &stats_.vc_err, &stats_.gamma_err);
        ++stats_.iterations;
...
template <typename T>
void PgsSolver<T>::PreProcessData(const SystemDynamicsData<T>& dynamics_data, const PointContactData<T>& contact_data) {
    const int nc = contact_data.num_contacts();
    ...
    if (nc != 0) {
        ...
        this->FormDelassusOperatorMatrix(contact_data.get_Jc(), dynamics_data.get_Ainv(), contact_data.get_Jc(), &W);
        ...
        for (int i = 0; i < nc; ++i) {
            ...
            // division by zero.
            Dinv.template segment<3>(3 * i) = Vector3<T>(1.0 / max((Wii(0, 0) + Wii(1, 1)) / 2, Wii_norm(i)),
...
template <typename T>
bool PgsSolver<T>::VerifyConvergenceCriteria(int num_contacts,
                                             const VectorX<T>& vc,
    ...
    *gamma_err = 0;
    for (int ic = 0; ic < num_contacts; ++ic) {
        ...
        const double vc_norm = ExtractDoubleOrThrow(vci.template lpNorm<Eigen::Infinity>());
        const double vci_err = ExtractDoubleOrThrow((vci_kp - vci).template lpNorm<Eigen::Infinity>());
        *vc_err = max(*vc_err, vci_err);
        ...
        const double g_norm = ExtractDoubleOrThrow(gi.template lpNorm<Eigen::Infinity>() * Wii_norm(ic));
        const double g_err = ExtractDoubleOrThrow((gi_kp - gi).template lpNorm<Eigen::Infinity>() * Wii_norm(ic));
...
template <typename T>
Vector3<T> PgsSolver<T>::ProjectImpulse(const Eigen::Ref<const Vector3<T>>& vc,
