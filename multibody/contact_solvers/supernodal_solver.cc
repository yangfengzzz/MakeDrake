...
void SuperNodalSolver::SetWeightMatrix(const std::vector<Eigen::MatrixXd>& weight_matrix) {
...
Eigen::VectorXd SuperNodalSolver::Solve(const Eigen::VectorXd& b) const {
    ...
    if (!factorization_ready_) {
        throw std::runtime_error("Call to Solve() failed: factorization not ready.");
    }
    Eigen::VectorXd y = b;
    DoSolveInPlace(&y);
    return y;
...
void SuperNodalSolver::SolveInPlace(Eigen::VectorXd* b) const {
    ...
    if (!factorization_ready_) {
        throw std::runtime_error("Call to SolveInPlace() failed: factorization not ready.");
    }
    DoSolveInPlace(b);
}

std::vector<std::vector<int>> GetRowToTripletMapping(int num_row_blocks,
    ...
    auto sort_row_data_by_column = [](std::vector<int>* col_index, std::vector<int>* triplet_position) {
    ...
    for (const auto& j : jacobian_blocks) {
        ...
        if (index >= num_row_blocks) {
            throw std::runtime_error(
                    fmt::format("Jacobian block with block row index {} is greater than or equal to "
        ...
        if (column[index].size() > 2) {
            throw std::runtime_error("Jacobian can only be nonzero on at most two column blocks.");
        }
        ++cnt;
    }
    for (int i = 0; i < num_row_blocks; ++i) {
        if (y[i].empty()) {
            throw std::runtime_error(fmt::format("The {}-th block row in the Jacobian is empty.", i));
        }
    }
    return y;
}

std::vector<int> GetJacobianBlockSizesVerifyTriplets(const std::vector<BlockTriplet>& jacobian_blocks) {
    int max_index = -1;
    for (const auto& j : jacobian_blocks) {
        int col_index = j.col;
    ...
    for (const auto& j : jacobian_blocks) {
        ...
        if ((j.value.cols() == 0) || (j.value.rows() == 0)) {
            throw std::runtime_error("Invalid Jacobian triplets: a triplet contains an empty matrix");
        }
        ...
        if (block_column_size[col_index] == -1) {
            ...
            if (block_column_size[col_index] != j.value.cols()) {
                throw std::runtime_error(
                        fmt::format("Invalid Jacobian triplets: conflicting block sizes specified "
                                    "for column {}.",
                                    col_index));
            }
        }
    ...
    for (int i = 0; i < max_index; ++i) {
        ...
        if (block_column_size[i] < 0) {
            throw std::runtime_error(fmt::format("Invalid Jacobian triplets: no triplet provided for column {}.", i));
        }
    }
