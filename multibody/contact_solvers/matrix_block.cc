#include "multibody/contact_solvers/matrix_block.h"
...
template <class T>
MatrixBlock<T>::MatrixBlock(Block3x3SparseMatrix<T> data) : data_(std::move(data)), is_dense_(false) {}

template <class T>
MatrixBlock<T>::MatrixBlock(MatrixX<T> data) : data_(std::move(data)), is_dense_(true) {}
...
template <class T>
void MatrixBlock<T>::MultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A, EigenPtr<MatrixX<T>> y) const {
    ...
    const Block3x3SparseMatrix<T>& M_sparse = std::get<Block3x3SparseMatrix<T>>(data_);
    M_sparse.MultiplyAndAddTo(A, y);
}

template <class T>
void MatrixBlock<T>::TransposeAndMultiplyAndAddTo(const Eigen::Ref<const MatrixX<T>>& A, EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
...
template <class T>
void MatrixBlock<T>::TransposeAndMultiplyAndAddTo(const MatrixBlock<T>& A, EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
    DRAKE_DEMAND(cols() == y->rows());
    DRAKE_DEMAND(rows() == A.rows());
    DRAKE_DEMAND(A.cols() == y->cols());
    ...
    /* A is sparse. */
    const Block3x3SparseMatrix<T>& A_sparse = std::get<Block3x3SparseMatrix<T>>(A.data_);
    if (this->is_dense_) {
    ...
    const Block3x3SparseMatrix<T>& M_sparse = std::get<Block3x3SparseMatrix<T>>(this->data_);
...
// type provides an API to operate on every other block type.
template <class T>
MatrixBlock<T> MatrixBlock<T>::LeftMultiplyByBlockDiagonal(const std::vector<MatrixX<T>>& Gs,
                                                           int start,
                                                           int end) const {
    ...
    std::vector<int> row_starts;  // starting row index for each diagonal block in G
    ...
    if (is_dense_) {
        ...
        for (int i = start; i <= end; ++i) {
            ...
            GM.middleRows(row_start, rows).noalias() = Gs[i] * M_dense.middleRows(row_start, rows);
        }
    ...
    /* At this point, we know that M is block 3x3 sparse and G is block diagonal,
     ...
     block row indices. */
    std::vector<int> G_indices(M_sparse.block_rows());  // `G_indices[k]` gives the relevant index h into
                                                        // `Gs` for block column k.
    std::vector<int> i_start(M_sparse.block_rows());    // `i_start[k]` gives the starting block row
                                                        // index from (2) above.
    {
        int i = 0;
        int h = start;
    ...
    for (const auto& row_data : M_triplets) {
        for (const Triplet& t : row_data) {
            ...
            for (int l = 0; 3 * l < Gs[h].cols(); ++l) {
                ...
                A_triplets.emplace_back(i, j, Gs[h].template block<3, 3>(3 * l, 3 * local_block_cols[k]) * M_block);
            }
        }
    }
...
template <class T>
void MatrixBlock<T>::MultiplyWithScaledTransposeAndAddTo(const VectorX<T>& scale, EigenPtr<MatrixX<T>> y) const {
    DRAKE_DEMAND(y != nullptr);
...
MatrixBlock<T> StackMatrixBlocks(const std::vector<MatrixBlock<T>>& blocks) {
    ...
    for (const auto& b : blocks) {
        const Block3x3SparseMatrix<T>& entry = std::get<Block3x3SparseMatrix<T>>(b.data_);
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&StackMatrixBlocks<T>));
