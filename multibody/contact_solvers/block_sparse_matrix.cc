...
template <typename T>
void BlockSparseMatrix<T>::Multiply(const Eigen::Ref<const VectorX<T>>& x, EigenPtr<VectorX<T>> y) const {
...
template <typename T>
void BlockSparseMatrix<T>::MultiplyByTranspose(const Eigen::Ref<const VectorX<T>>& x, EigenPtr<VectorX<T>> y) const {
...
void BlockSparseMatrixBuilder<T>::PushBlock(int i, int j, MatrixBlock<T> Bij) {
    ...
    if (Bij.size() > 0) {
        ...
        unused(it);
        if (!success) {
            throw std::runtime_error(fmt::format("Block ({}, {}) already added.", i, j));
        }
...
BlockSparseMatrix<T> BlockSparseMatrixBuilder<T>::Build() {
    VerifyInvariants();
    return BlockSparseMatrix<T>(std::move(blocks_), std::move(block_row_size_), std::move(block_col_size_));
}
...
void BlockSparseMatrixBuilder<T>::VerifyInvariants() const {
    ...
    for (size_t ib = 0; ib < block_row_size_.size(); ++ib) {
        if (block_row_size_[ib] < 0) {
            throw std::runtime_error(fmt::format("No block was specified for row {:d}.", ib));
        }
    }
    for (size_t jb = 0; jb < block_col_size_.size(); ++jb) {
        if (block_col_size_[jb] < 0) {
            throw std::runtime_error(fmt::format("No block was specified for column {:d}.", jb));
        }
    }
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        class ::drake::multibody::contact_solvers::internal::BlockSparseMatrixBuilder);
