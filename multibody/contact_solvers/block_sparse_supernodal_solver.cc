...
bool MassMatrixPartitionEqualsJacobianPartition(const std::vector<int>& jacobian_column_block_size,
...
BlockSparseSuperNodalSolver::BlockSparseSuperNodalSolver(int num_jacobian_row_blocks,
                                                         std::vector<BlockTriplet> jacobian_blocks,
                                                         std::vector<Eigen::MatrixXd> mass_matrices)
    : jacobian_blocks_(std::move(jacobian_blocks)), mass_matrices_(std::move(mass_matrices)) {
    const std::vector<int> jacobian_column_block_size = GetJacobianBlockSizesVerifyTriplets(jacobian_blocks_);
    /* Throw an exception if verification fails. */
    if (!MassMatrixPartitionEqualsJacobianPartition(jacobian_column_block_size, mass_matrices_)) {
        throw std::runtime_error("Mass matrices and constraint Jacobians are incompatible.");
    }
    row_to_triplet_index_ = GetRowToTripletMapping(num_jacobian_row_blocks, jacobian_blocks_);
...
bool BlockSparseSuperNodalSolver::DoSetWeightMatrix(const std::vector<Eigen::MatrixXd>& weight_matrix) {
    H_->SetZero();
    /* Add mass matrices. */
    ...
    for (int k = 0; k < num_constraints; ++k) {
        ...
        const int num_constraint_equations = jacobian_blocks_[triplet_indices[0]].value.rows();
        int G_rows = 0;
        while (G_rows < num_constraint_equations && weight_end < ssize(weight_matrix)) {
        ...
        if (triplet_indices.size() == 1) {
            ...
            const MatrixBlock<double> GJ = J.LeftMultiplyByBlockDiagonal(weight_matrix, weight_start, weight_end - 1);
            ...
            const int i = jacobian_blocks_[triplet_indices[1]].col;
            DRAKE_DEMAND(j < i);
            const MatrixBlock<double>& Jj = jacobian_blocks_[triplet_indices[0]].value;
            const MatrixBlock<double>& Ji = jacobian_blocks_[triplet_indices[1]].value;

            // TODO(xuchenhan-tri): Consider adding a more specialized routine for
            // computing JᵀGJ to further exploit sparsity. */
            const MatrixBlock<double> GJj = Jj.LeftMultiplyByBlockDiagonal(weight_matrix, weight_start, weight_end - 1);
            const MatrixBlock<double> GJi = Ji.LeftMultiplyByBlockDiagonal(weight_matrix, weight_start, weight_end - 1);
