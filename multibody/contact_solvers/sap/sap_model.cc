#include "multibody/contact_solvers/sap/sap_model.h"
...
#include "multibody/contact_solvers/block_sparse_supernodal_solver.h"
#include "multibody/contact_solvers/conex_supernodal_solver.h"
#include "multibody/contact_solvers/sap/contact_problem_graph.h"
#include "multibody/contact_solvers/sap/dense_supernodal_solver.h"
...
HessianFactorizationCache::HessianFactorizationCache(SapHessianFactorizationType type,
    ...
    switch (type) {
        case SapHessianFactorizationType::kConex:
            factorization_ = std::make_unique<ConexSuperNodalSolver>(J->block_rows(), J->get_blocks(), *A);
            break;
        case SapHessianFactorizationType::kBlockSparseCholesky:
            factorization_ = std::make_unique<BlockSparseSuperNodalSolver>(J->block_rows(), J->get_blocks(), *A);
            break;
...
std::unique_ptr<HessianFactorizationCache> HessianFactorizationCache::Clone() const {
    throw std::runtime_error("Attempting to clone an expensive Hessian factorization.");
}

void HessianFactorizationCache::UpdateWeightMatrixAndFactor(const std::vector<MatrixX<double>>& G) {
    DRAKE_DEMAND(!is_empty());
    mutable_factorization()->SetWeightMatrix(G);
    mutable_factorization()->Factor();
}

void HessianFactorizationCache::SolveInPlace(EigenPtr<MatrixX<double>> rhs) const {
    DRAKE_DEMAND(!is_empty());
...
template <typename T>
SapModel<T>::SapModel(const SapContactProblem<T>* problem_ptr, SapHessianFactorizationType hessian_type)
    ...
    PartialPermutation velocities_permutation = MakeParticipatingVelocitiesPermutation(problem());
    PartialPermutation impulses_permutation = MakeImpulsesPermutation(graph);

    // Extract momentum matrix for participating DOFs only.
    const int num_participating_cliques = cliques_permutation.permuted_domain_size();
    ...
    for (const auto& Ac : dynamics_matrix) {
        ...
        inv_sqrt_A.segment(clique_offset, clique_nv) = Ac.diagonal().cwiseInverse().cwiseSqrt();
...
void SapModel<T>::DeclareCacheEntries() {
    ...
    const auto& constraint_velocities_cache_entry = system_->DeclareCacheEntry(
            "Constraint velocities, vc = J⋅v.", systems::ValueProducer(this, &SapModel<T>::CalcConstraintVelocities),
            {systems::System<T>::xd_ticket()});
    system_->mutable_cache_indexes().constraint_velocities = constraint_velocities_cache_entry.cache_index();
    ...
    const auto& bundle_data_cache_entry = system_->DeclareCacheEntry(
            ...
            systems::ValueProducer(this, bundle_cache_model, &SapModel<T>::CalcConstraintBundleDataCache),
            {system_->cache_entry_ticket(system_->cache_indexes().constraint_velocities)});
    system_->mutable_cache_indexes().bundle_data = bundle_data_cache_entry.cache_index();

    const auto& impulses_cache_entry = system_->DeclareCacheEntry(
            "Impulses, γ = P(−R⁻¹(vc − v̂)).", systems::ValueProducer(this, &SapModel<T>::CalcImpulsesCache),
            {system_->cache_entry_ticket(system_->cache_indexes().constraint_velocities)});
    system_->mutable_cache_indexes().impulses = impulses_cache_entry.cache_index();

    const auto& momentum_gain_cache_entry = system_->DeclareCacheEntry(
            "Momentum gain. Δv = v - v*, Δp = A⋅Δv.", systems::ValueProducer(this, &SapModel<T>::CalcMomentumGainCache),
            {systems::System<T>::xd_ticket()});
    system_->mutable_cache_indexes().momentum_gain = momentum_gain_cache_entry.cache_index();

    const auto& cost_cache_entry =
            system_->DeclareCacheEntry("Cost cache.", systems::ValueProducer(this, &SapModel<T>::CalcCostCache),
    ...
    const auto& gradients_cache_entry = system_->DeclareCacheEntry(
            "Gradients cache.", systems::ValueProducer(this, &SapModel<T>::CalcGradientsCache),
            {system_->cache_entry_ticket(system_->cache_indexes().momentum_gain),
             system_->cache_entry_ticket(system_->cache_indexes().impulses)});
    system_->mutable_cache_indexes().gradients = gradients_cache_entry.cache_index();

    const auto& hessian_cache_entry =
            system_->DeclareCacheEntry("Hessian cache.", systems::ValueProducer(this, &SapModel<T>::CalcHessianCache),
                                       {system_->cache_entry_ticket(system_->cache_indexes().constraint_velocities)});
    ...
    const auto& hessian_factorization_cache_entry = system_->DeclareCacheEntry(
            "Hessian factorization cache.", systems::ValueProducer(this, &SapModel<T>::CalcHessianFactorizationCache),
            {system_->cache_entry_ticket(system_->cache_indexes().constraint_velocities)});
    system_->mutable_cache_indexes().hessian_factorization = hessian_factorization_cache_entry.cache_index();
}
...
template <typename T>
Eigen::VectorBlock<VectorX<T>> SapModel<T>::GetMutableVelocities(Context<T>* context) const {
    DRAKE_DEMAND(context != nullptr);
    system_->ValidateContext(*context);
    return context->get_mutable_discrete_state(system_->velocities_index()).get_mutable_value();
}
...
template <typename T>
void SapModel<T>::CalcConstraintVelocities(const Context<T>& context, VectorX<T>* vc) const {
    system_->ValidateContext(context);
...
template <typename T>
void SapModel<T>::CalcConstraintBundleDataCache(const systems::Context<T>& context,
...
template <typename T>
void SapModel<T>::CalcImpulsesCache(const Context<T>& context, ImpulsesCache<T>* cache) const {
    ...
    const systems::CacheEntry& hessian_cache_entry = system_->get_cache_entry(system_->cache_indexes().hessian);
    const systems::CacheEntryValue& hessian_value = hessian_cache_entry.get_cache_entry_value(context);
    if (!hessian_value.is_out_of_date()) {
        const auto& hessian_cache = hessian_value.template get_value<HessianCache<T>>();
    ...
    const SapConstraintBundleData& bundle_data = EvalSapConstraintBundleData(context);
    constraints_bundle().CalcImpulses(bundle_data, &cache->gamma);
}

template <typename T>
void SapModel<T>::CalcMomentumGainCache(const Context<T>& context, MomentumGainCache<T>* cache) const {
    system_->ValidateContext(context);
...
template <typename T>
void SapModel<T>::CalcCostCache(const Context<T>& context, CostCache<T>* cache) const {
    system_->ValidateContext(context);
...
template <typename T>
void SapModel<T>::CalcGradientsCache(const systems::Context<T>& context, GradientsCache<T>* cache) const {
    cache->Resize(num_velocities());
...
template <typename T>
void SapModel<T>::CalcHessianCache(const systems::Context<T>& context, HessianCache<T>* cache) const {
    system_->ValidateContext(context);
    cache->Resize(num_constraints(), num_constraint_equations());
    const SapConstraintBundleData& bundle_data = EvalSapConstraintBundleData(context);
    constraints_bundle().CalcImpulsesAndConstraintsHessian(bundle_data, &cache->impulses.gamma, &cache->G);
}

template <typename T>
void SapModel<T>::CalcHessianFactorizationCache(const systems::Context<T>&, HessianFactorizationCache*) const {
    throw std::runtime_error("Hessian computation is only supported for T = double");
}

template <>
void SapModel<double>::CalcHessianFactorizationCache(const systems::Context<double>& context,
    ...
    if (hessian->is_empty()) {
        *hessian = HessianFactorizationCache(hessian_type_, &dynamics_matrix(), &constraints_bundle().J());
    }
...
template <typename T>
PartialPermutation SapModel<T>::MakeParticipatingVelocitiesPermutation(const SapContactProblem<T>& problem) {
    const PartialPermutation& cliques_permutation = problem.graph().participating_cliques();
    ...
    for (int participating_clique = 1; participating_clique < num_participating_cliques; ++participating_clique) {
        const int previous_participating_clique = cliques_permutation.domain_index(participating_clique - 1);
        const int previous_clique_nv = problem.num_velocities(previous_participating_clique);
    ...
    for (int clique = 0; clique < problem.num_cliques(); ++clique) {
        ...
        if (cliques_permutation.participates(clique)) {
            const int clique_participating = cliques_permutation.permuted_index(clique);
            // Add participating dofs to the list.
            for (int i = 0; i < nv; ++i) {
                const int v = v_start + i;
                const int v_participating = v_participating_start[clique_participating] + i;
...
template <typename T>
PartialPermutation SapModel<T>::MakeImpulsesPermutation(const ContactProblemGraph& graph) const {
    if (problem().num_constraints() == 0) return PartialPermutation();  // empty permutation.

    std::vector<int> constraint_start(problem().num_constraints());
    constraint_start[0] = 0;
    for (int i = 1; i < problem().num_constraints(); ++i) {
        const int previous_constraint_size = problem().get_constraint(i - 1).num_constraint_equations();
...
template <typename T>
void SapModel<T>::MultiplyByDynamicsMatrix(const VectorX<T>& v, VectorX<T>* p) const {
    DRAKE_DEMAND(v.size() == num_velocities());
    DRAKE_DEMAND(p->size() == num_velocities());
    int clique_start = 0;
    for (const auto& Ab : dynamics_matrix()) {
        const int clique_size = Ab.rows();
        p->segment(clique_start, clique_size) = Ab * v.segment(clique_start, clique_size);
        clique_start += clique_size;
    }
}

template <typename T>
void SapModel<T>::CalcDelassusDiagonalApproximation(const std::vector<MatrixX<T>>& A,
    ...
    for (const ContactProblemGraph::ConstraintCluster& e : problem().graph().clusters()) {
    ...
    for (int i = 0; i < num_constraints; ++i) {
        ...
        delassus_diagonal->segment(constraint_start, ni).setConstant(W[i].norm() / ni);
        constraint_start += ni;
    }
}
