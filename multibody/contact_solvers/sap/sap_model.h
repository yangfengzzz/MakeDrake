...
#include "multibody/contact_solvers/sap/partial_permutation.h"
#include "multibody/contact_solvers/sap/sap_constraint_bundle.h"
#include "multibody/contact_solvers/sap/sap_contact_problem.h"
#include "multibody/contact_solvers/supernodal_solver.h"
...
struct SapConstraintBundleDataCache {
    ...
    SapConstraintBundleDataCache& operator=(SapConstraintBundleDataCache&&) = default;
...
struct ImpulsesCache {
    void Resize(int num_constraint_equations) { gamma.resize(num_constraint_equations); }
...
public:
    ...
    HessianFactorizationCache& operator=(const HessianFactorizationCache&) = delete;
...
public:
    ...
    explicit SapModel(const SapContactProblem<T>* problem,
                      SapHessianFactorizationType hessian_type = SapHessianFactorizationType::kBlockSparseCholesky);
    ...
    const PartialPermutation& velocities_permutation() const { return const_model_data_.velocities_permutation; }
    ...
    /* Returns permutation to go back and forth between constraint equations in
     ...
     indexes. */
    const PartialPermutation& impulses_permutation() const { return const_model_data_.impulses_permutation; }
    ...
    /* Evaluates the constraint velocities vc = J⋅v.
     @pre `context` is created with a call to MakeContext(). */
    const VectorX<T>& EvalConstraintVelocities(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().constraint_velocities)
                .template Eval<VectorX<T>>(context);
    }

    const SapConstraintBundleData& EvalSapConstraintBundleData(const systems::Context<T>& context) const {
    ...
    /* Evaluates the generalized impulses j = Jᵀ⋅γ, where J is the constraints'
     ...
     @pre `context` is created with a call to MakeContext(). */
    const VectorX<T>& EvalGeneralizedImpulses(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().gradients).template Eval<GradientsCache<T>>(context).j;
    }
    ...
    const T& EvalCost(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().cost).template Eval<CostCache<T>>(context).cost;
    }
    ...
    /* Evaluates the constraints's Hessian G, a vector of size num_constraints().
     ...
     @pre `context` is created with a call to MakeContext(). */
    const std::vector<MatrixX<T>>& EvalConstraintsHessian(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().hessian).template Eval<HessianCache<T>>(context).G;
    }
    ...
    /* This method evaluates the Hessian H of the primal cost ℓₚ(v).
     ...
     != double. */
    const HessianFactorizationCache& EvalHessianFactorizationCache(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().hessian_factorization)
...
private:
    ...
    public:
        ...
        /* Constructs a system that declares a discrete state of size
         num_velocities. */
        explicit SapModelSystem(int num_velocities) { velocities_index_ = this->DeclareDiscreteState(num_velocities); }
        ...
        systems::DiscreteStateIndex velocities_index() const { return velocities_index_; }
    ...
    static PartialPermutation MakeParticipatingVelocitiesPermutation(const SapContactProblem<T>& problem);
     ...
     */
    PartialPermutation MakeImpulsesPermutation(const ContactProblemGraph& graph) const;
    ...
    void CalcDelassusDiagonalApproximation(const std::vector<MatrixX<T>>& A, VectorX<T>* delassus_diagonal) const;

    /* Calc methods to update cache entries. See the documentation for the
    corresponding Eval methods for details. */
    void CalcConstraintVelocities(const systems::Context<T>& context, VectorX<T>* vc) const;
    void CalcConstraintBundleDataCache(const systems::Context<T>& context,
                                       SapConstraintBundleDataCache* bundle_data) const;
    void CalcImpulsesCache(const systems::Context<T>& context, ImpulsesCache<T>* cache) const;
    void CalcMomentumGainCache(const systems::Context<T>& context, MomentumGainCache<T>* cache) const;
    void CalcCostCache(const systems::Context<T>& context, CostCache<T>* cache) const;
    void CalcGradientsCache(const systems::Context<T>& context, GradientsCache<T>* cache) const;
    void CalcHessianCache(const systems::Context<T>& context, HessianCache<T>* cache) const;
    void CalcHessianFactorizationCache(const systems::Context<T>& context, HessianFactorizationCache* hessian) const;
    ...
    const MomentumGainCache<T>& EvalMomentumGainCache(const systems::Context<T>& context) const {
    ...
    const ImpulsesCache<T>& EvalImpulsesCache(const systems::Context<T>& context) const {
        return system_->get_cache_entry(system_->cache_indexes().impulses).template Eval<ImpulsesCache<T>>(context);
    }

    const SapContactProblem<T>* problem_{nullptr};
    SapHessianFactorizationType hessian_type_{SapHessianFactorizationType::kBlockSparseCholesky};
