...
SapWeldConstraint<T>::Kinematics::Kinematics(int objectA,
    ...
    const AngleAxis<T> a_PQ = X_WP_.rotation().InvertAndCompose(X_WQ_.rotation()).ToAngleAxis();
...
template <typename T>
bool SapWeldConstraint<T>::Kinematics::operator==(const Kinematics& other) const {
...
template <typename T>
typename SapHolonomicConstraint<T>::Parameters SapWeldConstraint<T>::MakeSapHolonomicConstraintParameters() {
    ...
    return typename SapHolonomicConstraint<T>::Parameters{-infinity, infinity, infinity, Vector6<T>::Zero(), kBeta};
}

template <typename T>
typename SapHolonomicConstraint<T>::Kinematics SapWeldConstraint<T>::MakeSapHolonomicConstraintKinematics(
        const Kinematics& kinematics) {
    // See class documentation for SapWeldConstraint for definitions of all
    // quantites referenced below.

    // Constraint function g.
    Vector6<T> g = (Vector6<T>() << kinematics.a_PQ_W(), kinematics.p_PoQo_W()).finished();
    ...
    return typename SapHolonomicConstraint<T>::Kinematics(std::move(g), std::move(kinematics.jacobian()), std::move(b));
}

template <typename T>
void SapWeldConstraint<T>::DoAccumulateSpatialImpulses(int i,
    ...
    if (i == 0) {
        ...
        const SpatialForce<T> gamma_AAm_W(-gamma.template head<3>(), -gamma.template tail<3>());
        const Vector3<T> p_AoAm_W = kinematics().p_AP_W() + 0.5 * kinematics().p_PoQo_W();
        *F += gamma_AAm_W.Shift(-p_AoAm_W);
    } else {
        // Object B.
        // Spatial impulse on B, applied at point Bm, expressed in the world frame.
        const SpatialForce<T> gamma_BBm_W(gamma.template head<3>(), gamma.template tail<3>());
        const Vector3<T> p_BoBm_W = kinematics().p_BQ_W() - 0.5 * kinematics().p_PoQo_W();
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapWeldConstraint<T>::DoToDouble() const {
    const typename SapWeldConstraint<T>::Kinematics& k = kinematics_;
    auto discard_gradient = [](const RigidTransform<T>& X) {
        return RigidTransform<double>(RotationMatrix<double>(math::DiscardGradient(X.rotation().matrix())),
                                      math::DiscardGradient(X.translation()));
    };
    SapWeldConstraint<double>::Kinematics k_to_double(
            k.objectA(), discard_gradient(k.X_WP()), math::DiscardGradient(k.p_AP_W()), k.objectB(),
            discard_gradient(k.X_WQ()), math::DiscardGradient(k.p_BQ_W()), k.jacobian().ToDouble());
