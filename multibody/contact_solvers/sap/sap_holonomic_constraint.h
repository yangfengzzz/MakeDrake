...
public:
    ...
    public:
        ...
        /* Constructs a valid set of parameters.
         ...
         @pre beta > 0 */
        Parameters(VectorX<T> impulse_lower_limits,
                   VectorX<T> impulse_upper_limits,
                   VectorX<T> stiffnesses,
                   VectorX<T> relaxation_times,
                   double beta = 0.1);

        const VectorX<T>& impulse_lower_limits() const { return impulse_lower_limits_; }
        const VectorX<T>& impulse_upper_limits() const { return impulse_upper_limits_; }
        ...
        int num_constraint_equations() const { return impulse_lower_limits_.size(); }
    ...
    /* Constructor for a holonomic constraint given its kinematics and
     parameters. */
    SapHolonomicConstraint(Kinematics kinematics, Parameters parameters, std::vector<int> objects);
    ...
    SapHolonomicConstraint(VectorX<T> g, SapConstraintJacobian<T> J, Parameters parameters);
    ...
    SapHolonomicConstraint(VectorX<T> g, SapConstraintJacobian<T> J, VectorX<T> b, Parameters parameters);
...
private:
    ...
    std::unique_ptr<SapConstraint<T>> DoClone() const override {
        return std::unique_ptr<SapHolonomicConstraint<T>>(new SapHolonomicConstraint<T>(*this));
    }
