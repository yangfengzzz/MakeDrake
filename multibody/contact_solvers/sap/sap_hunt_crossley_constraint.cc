...
template <typename T>
SapHuntCrossleyConstraint<T>::SapHuntCrossleyConstraint(ContactConfiguration<T> configuration,
                                                        SapConstraintJacobian<T> J,
                                                        Parameters parameters)
    : SapConstraint<T>(std::move(J), {configuration.objectA, configuration.objectB}),
...
std::unique_ptr<AbstractValue> SapHuntCrossleyConstraint<T>::DoMakeData(
        const T& time_step, const Eigen::Ref<const VectorX<T>>& delassus_estimation) const {
    using std::max;
    ...
    const T wt = max(0.5 * (delassus_estimation(0) + delassus_estimation(1)), w_rms);
    const T Rt = sigma * wt;  // SAP's regularization.

    SapHuntCrossleyConstraintData<T> data;
    typename SapHuntCrossleyConstraintData<T>::InvariantData& p = data.invariant_data;
    p.dt = time_step;
...
template <typename T>
T SapHuntCrossleyConstraint<T>::CalcDiscreteHuntCrossleyAntiderivative(const T& dt, const T& vn) const {
    using std::min;
    ...
    const T df = -dt * k * v;
    const T N = dt * (v * (fe0 + 1.0 / 2.0 * df) - d * v * v / 2.0 * (fe0 + 2.0 / 3.0 * df));

    return N;
}

template <typename T>
T SapHuntCrossleyConstraint<T>::CalcDiscreteHuntCrossleyImpulse(const T& dt, const T& vn) const {
    // Parameters:
    const T& k = parameters_.stiffness;
    const T& d = parameters_.dissipation;
    const T& fe0 = configuration_.fe;
...
template <typename T>
T SapHuntCrossleyConstraint<T>::CalcDiscreteHuntCrossleyImpulseGradient(const T& dt, const T& vn) const {
    // Parameters:
    const T& k = parameters_.stiffness;
    const T& d = parameters_.dissipation;
    const T& fe0 = configuration_.fe;
...
template <typename T>
void SapHuntCrossleyConstraint<T>::DoCalcData(const Eigen::Ref<const VectorX<T>>& vc,
                                              AbstractValue* abstract_data) const {
    auto& data = abstract_data->get_mutable_value<SapHuntCrossleyConstraintData<T>>();
...
template <typename T>
T SapHuntCrossleyConstraint<T>::DoCalcCost(const AbstractValue& abstract_data) const {
    const auto& data = abstract_data.get_value<SapHuntCrossleyConstraintData<T>>();
    switch (parameters_.model) {
        case SapHuntCrossleyApproximation::kSimilar:
            return -data.Nz;
        case SapHuntCrossleyApproximation::kLagged:
...
template <typename T>
void SapHuntCrossleyConstraint<T>::DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const {
    const auto& data = abstract_data.get_value<SapHuntCrossleyConstraintData<T>>();
    const T& mu = parameters_.friction;
    const T& n0 = data.invariant_data.n0;
    const T& n = data.nz;
    ...
    const T n_friction = parameters_.model == SapHuntCrossleyApproximation::kSimilar ? n : n0;
    const Vector2<T> gt = -mu * n_friction * t_soft;
    *gamma << gt, n;
}

template <typename T>
void SapHuntCrossleyConstraint<T>::DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const {
    const auto& data = abstract_data.get_value<SapHuntCrossleyConstraintData<T>>();
...
template <typename T>
void SapHuntCrossleyConstraint<T>::DoAccumulateSpatialImpulses(int i,
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapHuntCrossleyConstraint<T>::DoToDouble() const {
    SapConstraintJacobian<double> J = this->jacobian().ToDouble();
    const auto& p = parameters();
    SapHuntCrossleyConstraint<double>::Parameters parameters{p.model,
    ...
    return std::make_unique<SapHuntCrossleyConstraint<double>>(std::move(configuration), std::move(J),
                                                               std::move(parameters));
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        class ::drake::multibody::contact_solvers::internal::SapHuntCrossleyConstraint);
