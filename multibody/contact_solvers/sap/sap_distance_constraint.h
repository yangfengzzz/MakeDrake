...
public:
    ...
    const ComplianceParameters& compliance_parameters() const { return parameters_; }
...
private:
    ...
    static typename SapHolonomicConstraint<T>::Kinematics MakeSapHolonomicConstraintKinematics(
            const Kinematics& kinematics);

    std::unique_ptr<SapConstraint<T>> DoClone() const final {
        return std::unique_ptr<SapDistanceConstraint<T>>(new SapDistanceConstraint<T>(*this));
    }
