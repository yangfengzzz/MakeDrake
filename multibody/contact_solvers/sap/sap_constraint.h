...
#include "multibody/contact_solvers/sap/sap_constraint_jacobian.h"
...
public:
    ...
    int second_clique() const {
        if (num_cliques() == 1) throw std::logic_error("This constraint only involves a single clique.");
        return J_.clique(1);
    }
    ...
    const MatrixBlock<T>& first_clique_jacobian() const { return J_.clique_jacobian(0); }
    ...
    std::unique_ptr<AbstractValue> MakeData(const T& time_step,
    ...
    */
    void CalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* abstract_data) const;
    ...
    */
    ...
    /* Computes the impulse according to:
     ...
     the type wrapped by AbstractValue is consistent with the concrete subclass.
    */
    void CalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const;
    ...
    */
    ...
    void AccumulateSpatialImpulses(int o, const Eigen::Ref<const VectorX<T>>& gamma, SpatialForce<T>* F) const;
    ...
    std::unique_ptr<SapConstraint<double>> ToDouble() const { return DoToDouble(); }
    ...
    */
    std::unique_ptr<SapConstraint<T>> MakeReduced(const PartialPermutation& clique_permutation,
...
protected:
    ...
    virtual std::unique_ptr<AbstractValue> DoMakeData(
            const T& time_step, const Eigen::Ref<const VectorX<T>>& delassus_estimation) const = 0;
    virtual void DoCalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* data) const = 0;
    virtual T DoCalcCost(const AbstractValue& data) const = 0;
    virtual void DoCalcImpulse(const AbstractValue& data, EigenPtr<VectorX<T>> gamma) const = 0;
    virtual void DoCalcCostHessian(const AbstractValue& data, MatrixX<T>* G) const = 0;
    virtual void DoAccumulateGeneralizedImpulses(int, const Eigen::Ref<const VectorX<T>>&, EigenPtr<VectorX<T>>) const {
        ...
        throw std::logic_error("Constraints must implement this function. See #19435. ");
    }
    virtual void DoAccumulateSpatialImpulses(int, const Eigen::Ref<const VectorX<T>>&, SpatialForce<T>*) const {
        // TODO(amcastro-tri): Temporarily, the default implementation throws until
        // the full resolution of #19435. Once all constraints report forces, this
        // function will be pure virtual.
        throw std::logic_error("Constraints must implement this function. See #19435. ");
