...
public:
    ...
    SapPdControllerConstraintData(T Kp_eff, T Kd_eff, T time_step)
        : parameters_{std::move(Kp_eff), std::move(Kd_eff), std::move(time_step)} {}
...
public:
    ...
    //@{
    SapPdControllerConstraint& operator=(const SapPdControllerConstraint&) = delete;
...
private:
    ...
    std::unique_ptr<SapConstraint<T>> DoClone() const final {
        return std::unique_ptr<SapPdControllerConstraint<T>>(new SapPdControllerConstraint<T>(*this));
    }
