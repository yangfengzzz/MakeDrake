...
// outside of the body.
template <typename T>
SapFixedConstraint<T>::SapFixedConstraint(FixedConstraintKinematics<T> kinematics)
    : SapHolonomicConstraint<T>(typename SapHolonomicConstraint<T>::Kinematics(
...
template <typename T>
typename SapHolonomicConstraint<T>::Parameters SapFixedConstraint<T>::MakeSapHolonomicConstraintParameters(
    ...
    VectorX<T> gamma_lower = VectorX<T>::Constant(num_constraint_equations, -kInfinity);
    VectorX<T> gamma_upper = VectorX<T>::Constant(num_constraint_equations, kInfinity);

    VectorX<T> stiffness = VectorX<T>::Constant(num_constraint_equations, kInfinity);
    VectorX<T> relaxation_time = VectorX<T>::Zero(num_constraint_equations);

    return typename SapHolonomicConstraint<T>::Parameters{std::move(gamma_lower), std::move(gamma_upper),
                                                          std::move(stiffness), std::move(relaxation_time), kBeta};
}

template <typename T>
std::vector<int> SapFixedConstraint<T>::MakeObjectIndices(const FixedConstraintKinematics<T>& kinematics) {
...
template <typename T>
void SapFixedConstraint<T>::DoAccumulateSpatialImpulses(int i,
    ...
    if (i == 0) {
        ...
        for (int c = 0; c < num_constrained_point_pairs_; ++c) {
            const SpatialForce<T> gamma_APi_W(Vector3<T>::Zero(), -gamma.template segment<3>(3 * c));
        ...
        for (int c = 0; c < num_constrained_point_pairs_; ++c) {
            const SpatialForce<T> gamma_BQi_W(Vector3<T>::Zero(), gamma.template segment<3>(3 * c));
