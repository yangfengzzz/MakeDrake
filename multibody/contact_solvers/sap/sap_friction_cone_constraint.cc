...
template <typename T>
SapFrictionConeConstraint<T>::SapFrictionConeConstraint(ContactConfiguration<T> configuration,
...
template <typename T>
SapFrictionConeConstraintData<T>::SapFrictionConeConstraintData(const T& mu,
                                                                const T& Rt,
                                                                const T& Rn,
                                                                const T& vn_hat) {
    using std::sqrt;
...
std::unique_ptr<AbstractValue> SapFrictionConeConstraint<T>::DoMakeData(
    ...
    const double beta_factor = parameters_.beta * parameters_.beta / (4.0 * M_PI * M_PI);

    const T& k = parameters_.stiffness;
    const T& taud = parameters_.dissipation_time_scale;

    // Regularization parameters.
    const T Rn = max(beta_factor * wn, 1.0 / (time_step * k * (time_step + taud)));
...
template <typename T>
ContactMode SapFrictionConeConstraint<T>::CalcContactMode(const T& mu, const T& mu_hat, const T& yr, const T& yn) {
...
template <typename T>
void SapFrictionConeConstraint<T>::DoCalcData(const Eigen::Ref<const VectorX<T>>& vc,
                                              AbstractValue* abstract_data) const {
    auto& data = abstract_data->get_mutable_value<SapFrictionConeConstraintData<T>>();
    ...
    data.mutable_mode() = CalcContactMode(data.mu(), data.mu_hat(), data.yr(), data.yn());

    ProjectImpulse(data, &data.mutable_gamma());
}

template <typename T>
void SapFrictionConeConstraint<T>::ProjectImpulse(const SapFrictionConeConstraintData<T>& data,
...
template <typename T>
T SapFrictionConeConstraint<T>::DoCalcCost(const AbstractValue& abstract_data) const {
    const auto& data = abstract_data.get_value<SapFrictionConeConstraintData<T>>();
...
template <typename T>
void SapFrictionConeConstraint<T>::DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const {
    const auto& data = abstract_data.get_value<SapFrictionConeConstraintData<T>>();
    *gamma = data.gamma();
}

template <typename T>
void SapFrictionConeConstraint<T>::DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const {
    const auto& data = abstract_data.get_value<SapFrictionConeConstraintData<T>>();
...
template <typename T>
void SapFrictionConeConstraint<T>::DoAccumulateSpatialImpulses(int i,
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapFrictionConeConstraint<T>::DoToDouble() const {
    ...
    return std::make_unique<SapFrictionConeConstraint<double>>(configuration().ToDouble(), this->jacobian().ToDouble(),
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        class ::drake::multibody::contact_solvers::internal::SapFrictionConeConstraint);
