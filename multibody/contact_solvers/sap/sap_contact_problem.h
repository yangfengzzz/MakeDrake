...
public:
    ...
    SapContactProblem(const T& time_step, std::vector<MatrixX<T>> A, VectorX<T> v_star);
    ...
    */
    ...
    int num_constraint_equations() const { return graph_.num_constraint_equations(); }
    ...
    int constraint_equations_start(int constraint_index) const {
        DRAKE_THROW_UNLESS(0 <= constraint_index && constraint_index < num_constraints());
    ...
    void CalcConstraintMultibodyForces(const VectorX<T>& gamma,
