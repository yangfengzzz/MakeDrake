...
template <typename T>
SapConstraintBundle<T>::SapConstraintBundle(const SapContactProblem<T>* problem, const VectorX<T>& delassus_diagonal) {
    DRAKE_THROW_UNLESS(problem != nullptr);
    DRAKE_THROW_UNLESS(delassus_diagonal.size() == problem->num_constraint_equations());
...
template <typename T>
void SapConstraintBundle<T>::MakeConstraintBundleJacobian(const SapContactProblem<T>& problem) {
    ...
    BlockSparseMatrixBuilder<T> builder(graph.num_clusters(), cliques_permutation.permuted_domain_size(),
    ...
    for (int block_row = 0; block_row < graph.num_clusters(); ++block_row) {
        const ContactProblemGraph::ConstraintCluster& cluster = graph.get_cluster(block_row);
        ...
        for (int i : cluster.constraint_index()) {
            ...
            J0_blocks.emplace_back(c0 == c.first_clique() ? c.first_clique_jacobian() : c.second_clique_jacobian());
            if (c1 != c0) {
                J1_blocks.emplace_back(c1 == c.first_clique() ? c.first_clique_jacobian() : c.second_clique_jacobian());
            }
        }
...
template <typename T>
SapConstraintBundleData SapConstraintBundle<T>::MakeData(const T& time_step,
...
template <typename T>
void SapConstraintBundle<T>::CalcData(const VectorX<T>& vc, SapConstraintBundleData* bundle_data) const {
...
template <typename T>
T SapConstraintBundle<T>::CalcCost(const SapConstraintBundleData& bundle_data) const {
    DRAKE_DEMAND(ssize(bundle_data) == num_constraints());
    T cost = 0.0;
    for (int i = 0; i < num_constraints(); ++i) {
        const SapConstraint<T>& c = *constraints_[i];
...
template <typename T>
void SapConstraintBundle<T>::CalcImpulses(const SapConstraintBundleData& bundle_data, VectorX<T>* gamma) const {
    DRAKE_DEMAND(ssize(bundle_data) == num_constraints());
...
template <typename T>
void SapConstraintBundle<T>::CalcImpulsesAndConstraintsHessian(const SapConstraintBundleData& bundle_data,
                                                               VectorX<T>* gamma,
