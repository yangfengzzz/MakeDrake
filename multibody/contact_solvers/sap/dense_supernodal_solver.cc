...
const T& SafeDeference(std::string_view variable_name, const T* ptr) {
    if (ptr == nullptr) {
        throw std::runtime_error(fmt::format("Condition '{} != nullptr' failed.", variable_name));
    }
    return *ptr;
}

DenseSuperNodalSolver::DenseSuperNodalSolver(const std::vector<MatrixX<double>>* A, const BlockSparseMatrix<double>* J)
...
bool DenseSuperNodalSolver::DoSetWeightMatrix(const std::vector<Eigen::MatrixXd>& G) {
