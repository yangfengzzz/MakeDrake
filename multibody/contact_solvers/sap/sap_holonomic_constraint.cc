...
template <typename T>
SapHolonomicConstraint<T>::Parameters::Parameters(VectorX<T> impulse_lower_limits,
                                                  VectorX<T> impulse_upper_limits,
                                                  VectorX<T> stiffnesses,
                                                  VectorX<T> relaxation_times,
                                                  double beta)
...
template <typename T>
SapHolonomicConstraint<T>::SapHolonomicConstraint(VectorX<T> g, SapConstraintJacobian<T> J, Parameters parameters)
    : SapConstraint<T>(std::move(J), {}), g_(std::move(g)), parameters_(std::move(parameters)) {
...
SapHolonomicConstraint<T>::SapHolonomicConstraint(VectorX<T> g,
                                                  ...
                                                  VectorX<T> b,
                                                  Parameters parameters)
    : SapConstraint<T>(std::move(J), {}), g_(std::move(g)), bias_(std::move(b)), parameters_(std::move(parameters)) {
    DRAKE_THROW_UNLESS(g_.size() == this->jacobian().rows());
...
std::unique_ptr<AbstractValue> SapHolonomicConstraint<T>::DoMakeData(
    ...
    const VectorX<T> R_compliant = 1.0 / (time_step * (time_step + tau.array()) * k.array());
    ...
    VectorX<T> v_hat = -VectorX<T>(this->constraint_function().array() / (time_step + tau.array())) - bias_;
...
template <typename T>
void SapHolonomicConstraint<T>::DoCalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* abstract_data) const {
    auto& data = abstract_data->get_mutable_value<SapHolonomicConstraintData<T>>();
...
template <typename T>
T SapHolonomicConstraint<T>::DoCalcCost(const AbstractValue& abstract_data) const {
...
template <typename T>
void SapHolonomicConstraint<T>::DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const {
    const auto& data = abstract_data.get_value<SapHolonomicConstraintData<T>>();
    *gamma = data.gamma();
}

template <typename T>
void SapHolonomicConstraint<T>::DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const {
    const auto& data = abstract_data.get_value<SapHolonomicConstraintData<T>>();
    const VectorX<T>& gl = parameters_.impulse_lower_limits();
    const VectorX<T>& gu = parameters_.impulse_upper_limits();
    const VectorX<T>& R_inv = data.R_inv();
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapHolonomicConstraint<T>::DoToDouble() const {
    const typename SapHolonomicConstraint<T>::Parameters& p = parameters_;
    SapHolonomicConstraint<double>::Parameters p_to_double(
            math::DiscardGradient(p.impulse_lower_limits()), math::DiscardGradient(p.impulse_upper_limits()),
            math::DiscardGradient(p.stiffnesses()), math::DiscardGradient(p.relaxation_times()), p.beta());
    return std::make_unique<SapHolonomicConstraint<double>>(math::DiscardGradient(constraint_function()),
                                                            this->jacobian().ToDouble(), math::DiscardGradient(bias()),
