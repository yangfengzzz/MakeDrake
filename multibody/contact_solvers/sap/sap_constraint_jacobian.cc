...
SapConstraintJacobian<T>::SapConstraintJacobian(int first_clique,
    ...
    : SapConstraintJacobian(first_clique,
...
SapConstraintJacobian<T> SapConstraintJacobian<T>::LeftMultiplyByTranspose(
    ...
    return SapConstraintJacobian<T>(clique(0), std::move(ATJ_first_clique), clique(1), std::move(ATJ_second_clique));
}
...
SapConstraintJacobian<double> SapConstraintJacobian<T>::ToDouble() const {
    ...
    MatrixX<double> J_first_clique = math::DiscardGradient(first_block.MakeDenseMatrix());
    if (num_cliques() == 1) {
    ...
    MatrixX<double> J_second_clique = math::DiscardGradient(second_block.MakeDenseMatrix());
    return SapConstraintJacobian<double>(clique(0), std::move(J_first_clique), clique(1), std::move(J_second_clique));
}
