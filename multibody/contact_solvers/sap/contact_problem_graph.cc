...
ContactProblemGraph::ConstraintCluster::ConstraintCluster(SortedPair<int> cliques) : cliques_(std::move(cliques)) {
    DRAKE_THROW_UNLESS(cliques_.first() >= 0 && cliques_.second() >= 0);
}

ContactProblemGraph::ConstraintCluster& ContactProblemGraph::ConstraintCluster::AddConstraint(
...
int ContactProblemGraph::AddConstraint(SortedPair<int> cliques, int num_constraint_equations) {
    participating_cliques_.push(cliques.first());
    participating_cliques_.push(cliques.second());
    auto [iterator, new_cluster] = pair_to_cluster_index_.insert(std::make_pair(cliques, clusters_.size()));
...
int ContactProblemGraph::AddConstraint(int clique, int num_constraint_equations) {
    DRAKE_THROW_UNLESS(0 <= clique && clique < num_cliques());
    DRAKE_THROW_UNLESS(num_constraint_equations >= 0);
    return AddConstraint(MakeSortedPair(clique, clique), num_constraint_equations);
}

int ContactProblemGraph::AddConstraint(int first_clique, int second_clique, int num_constraint_equations) {
    DRAKE_THROW_UNLESS(0 <= first_clique && first_clique < num_cliques());
    DRAKE_THROW_UNLESS(0 <= second_clique && second_clique < num_cliques());
    DRAKE_THROW_UNLESS(num_constraint_equations >= 0);
    return AddConstraint(MakeSortedPair(first_clique, second_clique), num_constraint_equations);
}
