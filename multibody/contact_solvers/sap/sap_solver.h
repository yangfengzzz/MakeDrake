...
struct SapSolverParameters {
    ...
    SapHessianFactorizationType linear_solver_type{SapHessianFactorizationType::kBlockSparseCholesky};
};
...
private:
    ...
    void SetProblemVelocitiesIntoModelContext(const SapModel<T>& model,
                                              const VectorX<T>& v_problem,
                                              systems::Context<T>* context) const {
        Eigen::VectorBlock<VectorX<T>> v_model = model.GetMutableVelocities(context);
    ...
    SapSolverStatus SolveWithGuessImpl(const SapModel<T>& model, systems::Context<T>* context)
    ...
    std::pair<T, int> PerformBackTrackingLineSearch(const SapModel<T>& model,
                                                    const systems::Context<T>& context,
                                                    const SearchDirectionData& search_direction_data,
    ...
    std::pair<T, int> PerformExactLineSearch(const SapModel<T>& model,
                                             const systems::Context<T>& context,
                                             const SearchDirectionData& search_direction_data,
                                             systems::Context<T>* scratch_workspace) const
        requires std::is_same_v<T, double>;
    ...
    MatrixX<T> CalcDenseHessian(const SapModel<T>& model, const systems::Context<T>& context) const;

    // Makes a new SuperNodalSolver compatible with the underlying SapModel.
    std::unique_ptr<SuperNodalSolver> MakeSuperNodalSolver(const SapModel<T>& model) const;
    ...
    void CallDenseSolver(const SapModel<T>& model, const systems::Context<T>& context, VectorX<T>* dv) const;
...
template <>
SapSolverStatus SapSolver<double>::SolveWithGuess(const SapContactProblem<double>&,
                                                  const VectorX<double>&,
                                                  SapSolverResults<double>*);
template <>
SapSolverStatus SapSolver<AutoDiffXd>::SolveWithGuess(const SapContactProblem<AutoDiffXd>&,
