...
public:
    TestConstraint(int clique, int size, T param)
        : SapConstraint<T>({clique, MakeJacobian(size, clique + 1)}, {}), param_(param) {}

    TestConstraint(int clique0, int clique1, int size, T param)
        : SapConstraint<T>({clique0, MakeJacobian(size, clique0 + 1), clique1, MakeJacobian(size, clique1 + 1)}, {}),
          param_(param) {}
...
public:
    void SetUp() override {
        ...
        std::vector<MatrixX<AutoDiffXd>> A = {MatrixX<AutoDiffXd>::Ones(1, 1), MatrixX<AutoDiffXd>::Ones(2, 2),
                                              MatrixX<AutoDiffXd>::Ones(3, 3)};
        VectorX<AutoDiffXd> v_star = VectorX<AutoDiffXd>::LinSpaced(6, 1., 6.);
        problem_ = std::make_unique<SapContactProblem<AutoDiffXd>>(time_step, std::move(A), std::move(v_star));
        // First cluster of constraints between cliques 0 and 2.
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(0, 2, 1, 1.0));
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(2, 0, 2, 2.0));
        // A second cluster of constraints between cliques 0 and 1.
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(0, 1, 3, 3.0));
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(0, 1, 2, 4.0));
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(0, 1, 3, 5.0));
        // A third cluster only involving clique 1.
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(1, 4, 6.0));
        problem_->AddConstraint(std::make_unique<TestConstraint<AutoDiffXd>>(1, 2, 7.0));

        delassus_diagonal_.resize(17);
        delassus_diagonal_ = VectorX<AutoDiffXd>::LinSpaced(17, 1., 7.0);

        bundle_ = std::make_unique<SapConstraintBundle<AutoDiffXd>>(problem_.get(), delassus_diagonal_);
    }
...
TEST_F(SapConstraintBundleTest, VerifyJacobian) {
    ...
    EXPECT_EQ(bundle_->num_constraint_equations(), problem_->num_constraint_equations());
    ...
    J_cluster0_clique0 << problem_->get_constraint(0).first_clique_jacobian().MakeDenseMatrix(),
    ...
    J_cluster0_clique2 << problem_->get_constraint(0).second_clique_jacobian().MakeDenseMatrix(),
    ...
    J_cluster1_clique0 << problem_->get_constraint(2).first_clique_jacobian().MakeDenseMatrix(),
    ...
    J_cluster1_clique1 << problem_->get_constraint(2).second_clique_jacobian().MakeDenseMatrix(),
    ...
    J_cluster2_clique1 << problem_->get_constraint(5).first_clique_jacobian().MakeDenseMatrix(),
...
TEST_F(SapConstraintBundleTest, VerifyGradients) {
    ...
    SapConstraintBundleData data = bundle_->MakeData(time_step, delassus_diagonal_);
    ...
    EXPECT_TRUE(CompareMatrices(gamma, minus_cost_gradient, 20 * std::numeric_limits<double>::epsilon(),
    ...
    for (int k = 0; k < problem_->num_constraints(); ++k) {
        ...
        EXPECT_TRUE(CompareMatrices(Gk, minus_gamma_gradient[k], 20 * std::numeric_limits<double>::epsilon(),
                                    MatrixCompareType::relative));
    }
}
