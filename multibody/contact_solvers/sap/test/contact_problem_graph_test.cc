...
public:
    static bool CompareClusters(const ContactProblemGraph::ConstraintCluster& a,
        ...
        return a.cliques() == b.cliques() && a.num_constraints() == b.num_constraints() &&
               a.num_total_constraint_equations() == b.num_total_constraint_equations() &&
    ...
    static void VerifyForExpectedGraph(const ContactProblemGraph& graph) {
        ...
        //    2    |    (0, 3)     |   2,4        | 11
        //    3    |    (1, 3)     |   3          |  2
        const auto cluster0 = ContactProblemGraph::ConstraintCluster(MakeSortedPair(3, 3)).AddConstraint(0, 1);
        const auto cluster1 = ContactProblemGraph::ConstraintCluster(MakeSortedPair(0, 1)).AddConstraint(1, 3);
        const auto cluster2 =
                ContactProblemGraph::ConstraintCluster(MakeSortedPair(0, 3)).AddConstraint(2, 6).AddConstraint(4, 5);
        const auto cluster3 = ContactProblemGraph::ConstraintCluster(MakeSortedPair(1, 3)).AddConstraint(3, 2);
