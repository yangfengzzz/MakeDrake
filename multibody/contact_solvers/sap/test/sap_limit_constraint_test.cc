...
void ExpectEqual(const SapLimitConstraint<double>& c1, const SapLimitConstraint<double>& c2) {
...
public:
    ...
    void SetUp() override {
        ...
        dut_ = std::make_unique<SapLimitConstraint<double>>(clique_, clique_dof_, clique_nv_, q0_, p);

        SapLimitConstraint<AutoDiffXd>::Parameters p_ad(p.lower_limit(), p.upper_limit(), p.stiffness(),
                                                        p.dissipation_time_scale(), p.beta());
        dut_ad_ = std::make_unique<SapLimitConstraint<AutoDiffXd>>(clique_, clique_dof_, clique_nv_, q0_, p_ad);
    }
...
TEST_P(SapLimitConstraintTest, Construction) {
    ...
    EXPECT_EQ(dut_->first_clique_jacobian().MakeDenseMatrix(), MakeExpectedJacobian());
    ...
    EXPECT_EQ(dut_->parameters().dissipation_time_scale(), p.dissipation_time_scale());
...
TEST_P(SapLimitConstraintTest, CalcBias) {
    ...
    const VectorXd delassus_approximation =
            VectorXd::Constant(dut_->num_constraint_equations(), NAN);  // Does not participate.
    ...
    const VectorXd v_hat_expected = -MakeExpectedConstraintFunction() / (time_step + dissipation_time_scale);
    EXPECT_TRUE(CompareMatrices(data.v_hat(), v_hat_expected, std::numeric_limits<double>::epsilon(),
...
TEST_P(SapLimitConstraintTest, CalcRegularizationSoft) {
    ...
    SapLimitConstraint<double>::Parameters p(p0.lower_limit(), p0.upper_limit(), soft_stiffness,
                                             p0.dissipation_time_scale(), p0.beta());
    const SapLimitConstraint<double> c(clique_, clique_dof_, clique_nv_, q0_, p);

    const double time_step = 5e-3;
    const VectorXd delassus_approximation = VectorXd::Constant(c.num_constraint_equations(), 1.5);
    std::unique_ptr<AbstractValue> abstract_data = c.MakeData(time_step, delassus_approximation);
    const auto& data = abstract_data->get_value<SapLimitConstraintData<double>>();

    const double Rvalue = 1. / (time_step * (time_step + p.dissipation_time_scale()) * p.stiffness());

    const VectorXd R_expected = VectorXd::Constant(expected_num_equations(), Rvalue);
    EXPECT_TRUE(
            CompareMatrices(data.R(), R_expected, std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
...
TEST_P(SapLimitConstraintTest, CalcRegularizationNearRigid) {
    ...
    SapLimitConstraint<double>::Parameters p(p0.lower_limit(), p0.upper_limit(), near_rigid_stiffness,
                                             p0.dissipation_time_scale(), p0.beta());
    const SapLimitConstraint<double> c(clique_, clique_dof_, clique_nv_, q0_, p);
    ...
    const VectorXd R_expected = p.beta() * p.beta() / (4 * M_PI * M_PI) * delassus_approximation;
...
TEST_P(SapLimitConstraintTest, ValidateConstraintGradients) {
    ...
    std::unique_ptr<AbstractValue> abstract_data = dut_ad_->MakeData(time_step_, delassus_estimation);
    const auto& data = abstract_data->get_value<SapLimitConstraintData<AutoDiffXd>>();
...
TEST_P(SapLimitConstraintTest, Clone) {
    ...
    // Test ToDouble.
    auto clone_from_ad = dynamic_pointer_cast<SapLimitConstraint<double>>(dut_ad_->ToDouble());
...
INSTANTIATE_TEST_SUITE_P(SapLimitConstraintTest,
                         SapLimitConstraintTest,
