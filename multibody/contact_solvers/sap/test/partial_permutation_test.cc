...
GTEST_TEST(PartialPermutation, Construction) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(p.permuted_index(0), "Index .* does not participate in this permutation.");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(p.permuted_index(4), "Index .* does not participate in this permutation.");
...
GTEST_TEST(PartialPermutation, PermutedDomainIsLargerThanOriginalDomain) {
    ...
    // domain of size 4.
    DRAKE_EXPECT_THROWS_MESSAGE(PartialPermutation({0, -1, 4, 1}),
