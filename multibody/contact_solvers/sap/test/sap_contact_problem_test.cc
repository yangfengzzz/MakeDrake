...
public:
    ...
    TestConstraint(int num_constraint_equations, int clique, int clique_nv)
        : SapConstraint<T>({clique, MatrixX<T>::Ones(num_constraint_equations, clique_nv)}, {}) {}

    // Constructor for a constraint between two cliques.
    // Registers objects with index first_clique and second_clique, for testing.
    TestConstraint(int num_constraint_equations,
                   int first_clique,
                   int first_clique_nv,
                   int second_clique,
                   int second_clique_nv)
        : SapConstraint<T>({first_clique, MatrixX<T>::Ones(num_constraint_equations, first_clique_nv), second_clique,
    ...
    void DoCalcData(const Eigen::Ref<const VectorX<T>>&, AbstractValue*) const final {}
...
private:
    ...
    void DoAccumulateSpatialImpulses(int o, const Eigen::Ref<const VectorX<T>>& gamma, SpatialForce<T>* F) const final {
    ...
    std::unique_ptr<SapConstraint<double>> DoToDouble() const final {
        if (this->num_cliques() == 1) {
            return std::make_unique<TestConstraint<double>>(this->num_constraint_equations(), this->first_clique(),
...
GTEST_TEST(ContactProblem, AddConstraintWithWrongArguments) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(problem.AddConstraint(std::make_unique<TestConstraint<double>>(
                                        3 /* num_equations */, 5 /* (wrong) first_clique */, 2 /* first_clique_nv */,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(problem.AddConstraint(std::make_unique<TestConstraint<double>>(
                                        3 /* num_equations */, 0 /* first_clique */, 3 /* (wrong) first_clique_nv */,
                                        1 /* second_clique */, 3 /* second_clique_nv */)),
...
void AddConstraints(SapContactProblem<T>* problem) {
    auto add_and_verify_access = [problem](std::unique_ptr<TestConstraint<T>> owned_constraint) {
        const int expected_index = problem->num_constraints();
        const TestConstraint<T>& constraint = *owned_constraint;
        EXPECT_EQ(problem->AddConstraint(std::move(owned_constraint)), expected_index);
    ...
    add_and_verify_access(
            std::make_unique<TestConstraint<T>>(1 /* num_equations */, 3 /* clique */, 2 /* clique_nv */));
    add_and_verify_access(std::make_unique<TestConstraint<T>>(3 /* num_equations */, 0 /* first_clique */,
                                                              2 /* first_clique_nv */, 1 /* second_clique */,
                                                              3 /* second_clique_nv */));
    add_and_verify_access(std::make_unique<TestConstraint<T>>(6 /* num_equations */, 0 /* first_clique */,
                                                              2 /* first_clique_nv */, 3 /* second_clique */,
                                                              2 /* second_clique_nv */));
    add_and_verify_access(std::make_unique<TestConstraint<T>>(2 /* num_equations */, 1 /* first_clique */,
                                                              3 /* first_clique_nv */, 3 /* second_clique */,
                                                              2 /* second_clique_nv */));
    add_and_verify_access(std::make_unique<TestConstraint<T>>(5 /* num_equations */, 0 /* first_clique */,
                                                              2 /* first_clique_nv */, 3 /* second_clique */,
                                                              2 /* second_clique_nv */));
}
...
GTEST_TEST(ContactProblem, AddConstraints) {
    ...
    for (int i = 0; i < problem.num_constraints(); ++i) {
        EXPECT_EQ(problem.constraint_equations_start(i), constraint_equations_start_expected[i]);
    }
...
GTEST_TEST(ContactProblem, Clone) {
    ...
    auto verify_clone = [&problem](const auto& clone) {
        ...
        EXPECT_EQ(clone.num_constraint_equations(), problem.num_constraint_equations());
    ...
    SapContactProblem<AutoDiffXd> problem_ad(time_step, std::move(A_ad), std::move(v_star_ad));
    AddConstraints(&problem_ad);

    // Unit test ToDouble().
    std::unique_ptr<SapContactProblem<double>> clone_from_ad = problem_ad.ToDouble();
...
GTEST_TEST(ContactProblem, MakeReduced) {
    ...
    const std::vector<std::vector<int>> clique_known_indices{{0, 1}, {0, 1, 2}, {}, {0}};
    ...
    EXPECT_EQ(mapping.velocity_permutation.domain_size(), problem.num_velocities());
    ...
    for (int i = 0; i < ssize(unknown_indices); ++i) {
        EXPECT_EQ(mapping.velocity_permutation.permuted_index(unknown_indices[i]), i);
    }
    ...
    EXPECT_EQ(mapping.constraint_equation_permutation.domain_size(), problem.num_constraint_equations());
    ...
    for (int i = 0; i < problem.num_constraints(); ++i) {
        ...
        for (int j = 0; j < c.num_constraint_equations(); ++j) {
            if (i == 1) {
                EXPECT_FALSE(mapping.constraint_equation_permutation.participates(constraint_equation_index + j));
            } else {
                EXPECT_TRUE(mapping.constraint_equation_permutation.participates(constraint_equation_index + j));
                EXPECT_EQ(mapping.constraint_equation_permutation.permuted_index(constraint_equation_index + j),
    ...
    */
    {
        ...
        EXPECT_EQ(c.first_clique_jacobian().cols(), reduced_problem->num_velocities(c.first_clique()));
    }
...
GTEST_TEST(ContactProblem, ExpandContactSolverResults) {
    ...
    reduced_results.Resize(reduced_problem->num_velocities(), reduced_problem->num_constraint_equations());
    reduced_results.v = VectorXd::LinSpaced(reduced_problem->num_velocities(), 1.0, reduced_problem->num_velocities());
    reduced_results.gamma = VectorXd::LinSpaced(reduced_problem->num_constraint_equations(), 1.0,
                                                reduced_problem->num_constraint_equations());
    reduced_results.vc = VectorXd::LinSpaced(reduced_problem->num_constraint_equations(), 1.0,
                                             reduced_problem->num_constraint_equations());
    reduced_results.j = VectorXd::LinSpaced(reduced_problem->num_velocities(), 1.0, reduced_problem->num_velocities());
    ...
    VectorX<double> gamma_expected = VectorX<double>::Zero(problem.num_constraint_equations());
    VectorX<double> vc_expected = VectorX<double>::Zero(problem.num_constraint_equations());
    ...
    for (int i = 0; i < problem.num_constraints(); ++i) {
        const SapConstraint<double>& c = problem.get_constraint(i);
        double sum =
                v_star.segment(problem.velocities_start(c.first_clique()), problem.num_velocities(c.first_clique()))
                        .sum();
        if (c.num_cliques() > 1) {
            sum += v_star.segment(problem.velocities_start(c.second_clique()),
                                  problem.num_velocities(c.second_clique()))
                           .sum();
        }
        vc_expected.segment(problem.constraint_equations_start(i), c.num_constraint_equations()) =
...
GTEST_TEST(ContactProblem, CalcConstraintMultibodyForces) {
    ...
    const VectorXd gamma = VectorXd::LinSpaced(problem.num_constraint_equations(), -12.3, 24.5);
    ...
    std::vector<SpatialForce<double>> spatial_forces(problem.num_objects(), SpatialForce<double>::Zero());
    ...
    for (int i = 0; i < problem.num_constraints(); ++i) {
        ...
        for (int c = 0; c < constraint.num_cliques(); ++c) {
            ...
            tau_expected.segment(problem.velocities_start(clique), problem.num_velocities(clique)).array() +=
                    gamma_c.sum();
            if (start <= i && i <= end) {
                tau_partial_expected.segment(problem.velocities_start(clique), problem.num_velocities(clique))
                        .array() += gamma_c.sum();
            }
        }
        offset += ne;
    ...
    EXPECT_TRUE(
            CompareMatrices(tau, tau_expected, std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(tau, tau_partial_expected, std::numeric_limits<double>::epsilon(),
                                MatrixCompareType::relative));

    // TestConstraint applies spatial force:
    // F = (time_step + local_object) * gamma.sum() * Vector6d::Ones().
    std::vector<SpatialForce<double>> spatial_forces_expected(problem.num_objects(), SpatialForce<double>::Zero());
    offset = 0;
    for (int i = 0; i < problem.num_constraints(); ++i) {
        const auto& constraint = problem.get_constraint(i);
        const int ne = constraint.num_constraint_equations();
        ...
        for (int o = 0; o < constraint.num_objects(); ++o) {
            ...
            spatial_forces_expected[object].get_coeffs().array() += (o * gamma_c.sum());
        }
        offset += ne;
    }
    ...
    for (int object = 0; object < problem.num_objects(); ++object) {
        EXPECT_TRUE(CompareMatrices(spatial_forces[object].get_coeffs(), spatial_forces_expected[object].get_coeffs(),
