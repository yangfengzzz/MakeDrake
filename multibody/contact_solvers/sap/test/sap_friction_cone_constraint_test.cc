...
typename SapFrictionConeConstraint<T>::Parameters MakeArbitraryParameters() {
    ...
    return typename SapFrictionConeConstraint<T>::Parameters{mu, stiffness, dissipation_time_scale, beta, sigma};
}

void ExpectEqual(const SapFrictionConeConstraint<double>& c1, const SapFrictionConeConstraint<double>& c2) {
...
GTEST_TEST(SapFrictionConeConstraint, SingleCliqueConstraint) {
    ...
    const SapFrictionConeConstraint<double>::Parameters parameters = MakeArbitraryParameters();
    ...
    EXPECT_EQ(c.parameters().dissipation_time_scale, parameters.dissipation_time_scale);
...
GTEST_TEST(SapFrictionConeConstraint, CalcBias) {
    ...
    const ContactConfiguration<double> configuration = MakeArbitraryConfiguration();
    SapFrictionConeConstraint<double>::Parameters parameters = MakeArbitraryParameters();
    ...
    const Vector3d delassus_approximation = Vector3d::Constant(bad_number);  // Does not participate.
    std::unique_ptr<AbstractValue> abstract_data = c.MakeData(time_step, delassus_approximation);
    const auto& data = abstract_data->get_value<SapFrictionConeConstraintData<double>>();
    const Vector3d& vhat = data.v_hat();
    const double vn_hat = -configuration.phi / (time_step + parameters.dissipation_time_scale);
    const Vector3d vhat_expected(0., 0., vn_hat);
    EXPECT_TRUE(
            CompareMatrices(vhat, vhat_expected, std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
...
GTEST_TEST(SapFrictionConeConstraint, CalcRegularization) {
    ...
    const double time_step = 5e-3;
    const Vector3d delassus_approximation = Vector3d::Constant(3.0);  // Does not participate.
    std::unique_ptr<AbstractValue> abstract_data = c.MakeData(time_step, delassus_approximation);
    const auto& data = abstract_data->get_value<SapFrictionConeConstraintData<double>>();
    ...
    const double Rn =
            std::max(parameters.beta * parameters.beta / (4 * M_PI * M_PI) * delassus_approximation(2),
                     1. / (time_step * (time_step + parameters.dissipation_time_scale) * parameters.stiffness));

    const Vector3d R_expected(Rt, Rt, Rn);
    EXPECT_TRUE(CompareMatrices(R, R_expected, std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
...
Vector3d SolveProjectionWithScs(double mu, const Vector3d& R, const Vector3d& y) {
    MathematicalProgram prog;
    Eigen::Matrix<symbolic::Variable, 3, 1> gamma = prog.NewContinuousVariables<3>();
    ...
    const Matrix3d A = (Matrix3d() << 0., 0., mu, 1., 0., 0., 0., 1., 0.).finished();
...
void ValidateAgainstNumericalReference(const SapFrictionConeConstraint<double>::Parameters& p, const Vector3d& vc) {
    ...
    std::unique_ptr<AbstractValue> data = c.MakeData(time_step, delassus_estimation);
    ...
    const auto& cone_constraint_data = data->get_value<SapFrictionConeConstraintData<double>>();
    ...
    EXPECT_TRUE(CompareMatrices(gamma, gamma_numerical, 10.0 * kTolerance, MatrixCompareType::relative));
}
...
void ValidateProjection(const SapFrictionConeConstraint<double>::Parameters& p, const Vector3d& vc) {
    // Instantiate constraint on AutoDiffXd for automatic differentiation.
    SapFrictionConeConstraint<AutoDiffXd>::Parameters p_ad{p.mu, p.stiffness, p.dissipation_time_scale, p.beta,
                                                           p.sigma};
...
GTEST_TEST(SapFrictionConeConstraint, SingleCliqueConstraintClone) {
    ...
    auto clone = dynamic_pointer_cast<SapFrictionConeConstraint<double>>(c.Clone());
    ASSERT_NE(clone, nullptr);
    ExpectEqual(c, *clone);

    // Test ToDouble.
    SapFrictionConeConstraint<AutoDiffXd> c_ad(MakeArbitraryConfiguration<AutoDiffXd>(),
                                               SapConstraintJacobian<AutoDiffXd>(clique, J32),
                                               MakeArbitraryParameters<AutoDiffXd>());
    auto clone_from_ad = dynamic_pointer_cast<SapFrictionConeConstraint<double>>(c_ad.ToDouble());
