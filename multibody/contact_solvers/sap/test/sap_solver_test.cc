...
#include "multibody/contact_solvers/block_sparse_matrix.h"
#include "multibody/contact_solvers/conex_supernodal_solver.h"
...
public:
    ...
    PizzaSaverProblem(double dt, double mass, double radius, double mu, double k, double taud)
        : time_step_(dt), m_(mass), radius_(radius), mu_(mu), stiffness_(k), taud_(taud) {
    ...
    PizzaSaverProblem(double dt, double mu, double k, double taud) : PizzaSaverProblem(dt, 3.0, 1.5, mu, k, taud) {}
    ...
    *M << m_,  0,  0,  0,
    ...
    std::unique_ptr<SapContactProblem<double>> MakeContactProblem(
            const VectorXd& q0, const VectorXd& v0, const VectorXd& tau, double beta, double sigma) const {
        std::unique_ptr<SapContactProblem<double>> problem = MakeContactProblemWithoutConstraints(q0, v0, tau);
        ...
        const SapFrictionConeConstraint<double>::Parameters parameters{mu_, stiffness_, taud_, beta, 1.0e-3};
        ...
        const ContactConfiguration<double> configuration{
                .objectA = 0 /* valid, though not used */, .objectB = 0 /* valid, though not used */, .phi = phi0};
        ...
        problem->AddConstraint(std::make_unique<SapFrictionConeConstraint<double>>(
                configuration, SapConstraintJacobian<double>{0, J.middleRows(0, 3)}, parameters));
        problem->AddConstraint(std::make_unique<SapFrictionConeConstraint<double>>(
                configuration, SapConstraintJacobian<double>{0, J.middleRows(3, 3)}, parameters));
        problem->AddConstraint(std::make_unique<SapFrictionConeConstraint<double>>(
                configuration, SapConstraintJacobian<double>{0, J.middleRows(6, 3)}, parameters));

        return problem;
    }

    std::unique_ptr<SapContactProblem<double>> MakeContactProblemWithoutConstraints(const VectorXd& q0,
                                                                                    const VectorXd& v0,
        ...
        auto problem = std::make_unique<SapContactProblem<double>>(time_step_, std::move(A), std::move(v_star));
...
class PizzaSaverTest : public testing::TestWithParam<SapSolverParameters::LineSearchType> {
public:
    static SapSolverResults<double> AdvanceNumSteps(const PizzaSaverProblem& problem,
                                                    const VectorXd& tau,
                                                    int num_steps,
        ...
        for (int i = 0; i < num_steps; ++i) {
            const auto contact_problem = problem.MakeContactProblem(q, v, tau, beta, kDefaultSigma);
            const SapSolverStatus status = sap.SolveWithGuess(*contact_problem, v_guess, &result);
    ...
    static void VerifyStictionSolution(const SapSolverParameters& params,
        ...
        const SapSolverResults<double> result = AdvanceNumSteps(problem, tau, 40, params, beta, cost_criterion_reached);

        // Expected generalized impulse.
        const Vector4d j_expected = problem.time_step() * Vector4d(0.0, 0.0, problem.mass() * problem.g(), -Mz);

        // Maximum expected slip. See Castro et al. 2022 for details (Eq. 31).
        const double max_slip_expected = kDefaultSigma * problem.mu() * problem.time_step() * problem.g();

        EXPECT_TRUE(CompareMatrices(result.v.head<3>(), Vector3d::Zero(), 1.0e-10));
        EXPECT_TRUE((result.j - j_expected).norm() / j_expected.norm() < relative_tolerance);
        VectorXd gamma_normal(problem.kNumContacts);
        ExtractNormal(result.gamma, &gamma_normal);
        EXPECT_TRUE(CompareMatrices(gamma_normal, VectorXd::Constant(problem.kNumContacts, j_expected(2) / 3.0),
                                    relative_tolerance, MatrixCompareType::relative));
        VectorXd vc_normal(problem.kNumContacts);
        ExtractNormal(result.vc, &vc_normal);
        EXPECT_TRUE(CompareMatrices(vc_normal, VectorXd::Zero(problem.kNumContacts), 1.0e-10));
        const double friction_impulse_expected = problem.time_step() * Mz / problem.radius() / 3.0;
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            EXPECT_NEAR(friction_impulse, friction_impulse_expected, relative_tolerance * friction_impulse_expected);
...
TEST_P(PizzaSaverTest, NoAppliedTorque) {
    ...
    const SapSolverResults<double> result = AdvanceNumSteps(problem, tau, 30, params, beta);
    ...
    // Expected generalized impulse.
    const Vector3d per_contact_impulse = dt * problem.mass() * problem.g() * Vector3d::UnitZ() / 3.0;
    const Vector4d j_expected(0.0, 0.0, dt * problem.mass() * problem.g(), 0.0);

    EXPECT_TRUE(CompareMatrices(result.v, VectorXd::Zero(problem.kNumVelocities), params.rel_tolerance));
    EXPECT_TRUE((result.j - j_expected).norm() / j_expected.norm() < params.rel_tolerance);
    const VectorXd gamma_expected = per_contact_impulse.replicate(problem.kNumContacts, 1);
    EXPECT_TRUE(CompareMatrices(result.gamma, gamma_expected, 2.0 * params.rel_tolerance));
    EXPECT_TRUE(CompareMatrices(result.vc, VectorXd::Zero(3 * problem.kNumContacts), params.rel_tolerance));
}
...
TEST_P(PizzaSaverTest, ConvergenceWithExactGuess) {
    ...
    sap.set_parameters(params);
    SapSolverResults<double> result;
    const SapSolverStatus status = sap.SolveWithGuess(*contact_problem, v, &result);
    EXPECT_EQ(status, SapSolverStatus::kSuccess);
...
TEST_P(PizzaSaverTest, NoFriction) {
    ...
    const SapSolverResults<double> result = AdvanceNumSteps(problem, tau, num_steps, params, beta);
    ...
    EXPECT_TRUE(CompareMatrices(result.v, Vector4d(vx_expected, 0.0, 0.0, 0.0), 2.0 * params.rel_tolerance));
    EXPECT_TRUE(CompareMatrices(result.j, j_expected, 2.0 * params.rel_tolerance, MatrixCompareType::relative));

    const Vector3d per_contact_impulse = dt * problem.mass() * problem.g() * Vector3d::UnitZ() / 3.0;
    const VectorXd gamma_expected = per_contact_impulse.replicate(problem.kNumContacts, 1);
    EXPECT_TRUE(CompareMatrices(result.gamma, gamma_expected, 2.0 * params.rel_tolerance, MatrixCompareType::relative));
    const Vector3d per_contact_velocity = Vector3d::UnitX() * vx_expected;
    const VectorXd vc_expected = per_contact_velocity.replicate(problem.kNumContacts, 1);
    EXPECT_TRUE(CompareMatrices(result.vc, vc_expected, 2.0 * params.rel_tolerance, MatrixCompareType::relative));
}
...
TEST_P(PizzaSaverTest, Sliding) {
    ...
    const SapSolverResults<double> result = AdvanceNumSteps(problem, tau, 10, params, beta);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        EXPECT_NEAR(friction_impulse, mu * normal_impulse, std::numeric_limits<double>::epsilon() * normal_impulse);
    }
    ...
    if (params.line_search_type != SapSolverParameters::LineSearchType::kExact) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(AdvanceNumSteps(problem, tau, 1, params),
...
TEST_P(PizzaSaverTest, NearRigidStiction) {
    ...
    const Vector4d j_expected(0.0, 0.0, dt * problem.mass() * problem.g(), -dt * Mz);

    // Maximum expected slip. See Castro et al. 2022 for details (Eq. 31).
    const double max_slip_expected = kDefaultSigma * problem.mu() * dt * problem.g();
    ...
    VectorXd vc_normal(problem.kNumContacts);
    ExtractNormal(result.vc, &vc_normal);
    EXPECT_TRUE(CompareMatrices(vc_normal, VectorXd::Zero(problem.kNumContacts), 1.0e-9));
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        EXPECT_NEAR(friction_impulse, friction_impulse_expected, params.rel_tolerance * friction_impulse_expected);
        EXPECT_LE(slip, max_slip_expected);
    }
}
...
TEST_P(PizzaSaverTest, NoConstraints) {
    ...
    const auto contact_problem = problem.MakeContactProblemWithoutConstraints(q, v, tau);
    SapSolver<double> sap;
    SapSolverParameters params;  // Default set of parameters.
    params.line_search_type = GetParam();
    sap.set_parameters(params);
...
INSTANTIATE_TEST_SUITE_P(TestLineSearchMethods,
                         PizzaSaverTest,
...
public:
    ...
    LimitConstraint(int clique, const VectorX<T>& vl, const VectorX<T>& vu, VectorX<T> R)
...
private:
    static VectorX<T> ConcatenateVectors(const VectorX<T>& v1, const VectorX<T>& v2) {
        VectorX<T> v(v1.size() + v2.size());
        v << v1, v2;
        return v;
    }

    static VectorX<T> CalcConstraintFunction(const VectorX<T>& vl, const VectorX<T>& vu, const VectorX<T>& v) {
    ...
    std::unique_ptr<AbstractValue> DoMakeData(const T&, const Eigen::Ref<const VectorX<T>>&) const final {
    ...
    void DoCalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* abstract_data) const final {
    ...
    void DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const final {
        const auto& vc = abstract_data.get_value<VectorX<T>>();
        const VectorX<T> y = R_.asDiagonal() * (vhat_ - vc);
        Project(y, gamma);
    }
    void DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const final {
        const auto& vc = abstract_data.get_value<VectorX<T>>();
        const VectorX<T> y = R_.asDiagonal() * (vhat_ - vc);
...
class SapNewtonIterationTest : public testing::TestWithParam<SapSolverParameters::LineSearchType> {
public:
    void SetUp() override {
    ...
    const Eigen::Matrix4d S44 =
      (Eigen::Matrix4d() << 7, 1, 2, 3,
        ...
        VectorXd v_star = VectorXd::LinSpaced(num_velocities, 1., 1. * num_velocities);
        ...
        // below:
        auto v_star_clique1 = v_star.segment<clique1_nv>(2);  // Velocities for clique 1.
        v_star_clique1 = 0.5 * (vl_ + vu_);
        v_star_.resize(num_velocities);
        v_star_ = v_star;

        std::vector<MatrixXd> A = {S22, S33, S44};
        sap_problem_ = std::make_unique<SapContactProblem<double>>(time_step, std::move(A), std::move(v_star));
        ...
        sap_problem_->AddConstraint(std::make_unique<LimitConstraint<double>>(1, vl_, vu_, std::move(R)));
        // Model and context to test solver internals.
        model_ = std::make_unique<SapModel<double>>(sap_problem_.get(),
    ...
    void VerifySupernodalHessian(const SapSolver<double>& sap, const VectorXd& v_guess) const {
        ...
        const HessianFactorizationCache& factorization = model_->EvalHessianFactorizationCache(*context_);
        ...
        HessianFactorizationCache dense_factorization(SapHessianFactorizationType::kDense, &model_->dynamics_matrix(),
                                                      &model_->constraints_bundle().J());
        dense_factorization.UpdateWeightMatrixAndFactor(model_->EvalConstraintsHessian(*context_));
        ...
        EXPECT_TRUE(CompareMatrices(H, H_expected, 3.0 * kEps, MatrixCompareType::relative));
    }

    // Compute next step velocity using the provide set of `params` and the given
    // guess `v_guess`.
    VectorXd SolveWithGuess(const SapSolverParameters& params, const VectorXd& v_guess) const {
        SapSolver<double> sap;
        sap.set_parameters(params);
        SapSolverResults<double> result;
        const SapSolverStatus status = sap.SolveWithGuess(*sap_problem_, v_guess, &result);
        EXPECT_EQ(status, SapSolverStatus::kSuccess);
        return result.v;
    }
    ...
    void CompareDenseAgainstSupernodal(const VectorXd& v_guess) const {
        ...
        EXPECT_TRUE(CompareMatrices(v_supernodal, v_dense, 5.0 * relative_tolerance, MatrixCompareType::relative));
    }
...
TEST_P(SapNewtonIterationTest, GuessIsTheSolution) {
    ...
    EXPECT_EQ(result.gamma, VectorXd::Zero(sap_problem_->num_constraint_equations()));
}
...
TEST_P(SapNewtonIterationTest, GuessWithinLimits) {
    ...
    params.line_search_type = GetParam();
    if (params.line_search_type == SapSolverParameters::LineSearchType::kBackTracking) {
        ...
        // precision.
        params.backtracking_line_search.alpha_max = 1.0 / params.backtracking_line_search.rho;
    }
    sap.set_parameters(params);
    ...
    const int expected_line_search_iterations = params.line_search_type == SapSolverParameters::kBackTracking ? 2 : 1;
    ...
    EXPECT_TRUE(CompareMatrices(result.v, v_star_, 10.0 * kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(result.j, VectorXd::Zero(sap_problem_->num_velocities()), 3.0 * kEps,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(result.gamma, VectorXd::Zero(sap_problem_->num_constraint_equations()), 3.0 * kEps,
                                MatrixCompareType::absolute));
...
TEST_P(SapNewtonIterationTest, GuessOutsideLimits) {
    ...
    // Newton iterations.
    VectorXd v_guess = v_star_;
    v_guess.segment<3>(2) = Vector3d(1.2 * vl_(0) /* below lower limit */, v_star_(1) /* within limits */,
    ...
    EXPECT_TRUE(CompareMatrices(result.v, v_star_, 3.0 * kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(result.j, VectorXd::Zero(sap_problem_->num_velocities()), 3.0 * kEps,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(result.gamma, VectorXd::Zero(sap_problem_->num_constraint_equations()), 3.0 * kEps,
                                MatrixCompareType::absolute));
...
private:
    ...
    std::unique_ptr<AbstractValue> DoMakeData(const double&, const Eigen::Ref<const VectorXd>&) const final {
        // We'll store the constraint velocity in our data.
        VectorXd vc(this->num_constraint_equations());
        return SapConstraint<double>::MoveAndMakeAbstractValue(std::move(vc));
    }
    void DoCalcData(const Eigen::Ref<const VectorXd>& vc, AbstractValue* abstract_data) const final {
    ...
    void DoCalcImpulse(const AbstractValue&, EigenPtr<VectorXd> gamma) const final { *gamma = g_; }
    ...
    std::unique_ptr<SapConstraint<double>> DoClone() const final {
        return std::unique_ptr<ConstantForceConstraint>(new ConstantForceConstraint(*this));
    }
    std::unique_ptr<SapConstraint<double>> DoToDouble() const final {
        throw std::runtime_error("DoToDouble() not used in these unit tests.");
    }
...
GTEST_TEST(SapSolver, ConstraintWithNegativeCost) {
    ...
    problem.AddConstraint(std::make_unique<ConstantForceConstraint>(std::move(J), g));
    ...
    SapSolverResults<double> result;
    const SapSolverStatus status = solver.SolveWithGuess(problem, v_guess, &result);
    EXPECT_EQ(status, SapSolverStatus::kSuccess);
    ...
    EXPECT_TRUE(CompareMatrices(result.v, v_expected, 4 * kEps, MatrixCompareType::relative));
}
