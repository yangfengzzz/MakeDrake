...
public:
    ...
    TestConstraint(int clique, MatrixXd J) : SapConstraint<double>({clique, std::move(J)}, {}) {}

    TestConstraint(int first_clique, int second_clique, MatrixXd J_first_clique, MatrixXd J_second_clique)
        : SapConstraint<double>({first_clique, std::move(J_first_clique), second_clique, std::move(J_second_clique)},
                                {}) {}
...
private:
    ...
    void DoCalcData(const Eigen::Ref<const VectorXd>&, AbstractValue*) const final {}
    ...
    std::unique_ptr<SapConstraint<double>> DoToDouble() const final { return this->Clone(); }
};
...
GTEST_TEST(SapConstraint, TwoCliquesConstraintWrongArguments) {
    ...
    EXPECT_THROW(TestConstraint c(11, 7, J32, MatrixXd::Zero(1, 4)), std::exception);
}
