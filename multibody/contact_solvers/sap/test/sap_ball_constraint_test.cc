...
typename SapBallConstraint<T>::Kinematics MakeArbitraryKinematics(int num_cliques) {
    ...
    return typename SapBallConstraint<T>::Kinematics{objectA, p_WP, p_AP_W, objectB, p_WQ, p_BQ_W, J_PQ_W};
}

void ExpectEqual(const SapBallConstraint<double>& c1, const SapBallConstraint<double>& c2) {
...
GTEST_TEST(SapBallConstraint, SingleCliqueConstraint) {
    ...
    const SapBallConstraint<double>::Kinematics kinematics = MakeArbitraryKinematics(num_cliques);
...
void ValidateProjection(const Vector3d& vc) {
    ...
    const int num_cliques = 1;
    const SapBallConstraint<AutoDiffXd>::Kinematics kin_ad = MakeArbitraryKinematics<AutoDiffXd>(num_cliques);
...
GTEST_TEST(SapBallConstraint, SingleCliqueConstraintClone) {
    ...
    // Test ToDouble.
    SapBallConstraint<AutoDiffXd> c_ad(MakeArbitraryKinematics<AutoDiffXd>(num_cliques));
    auto clone_from_ad = dynamic_pointer_cast<SapBallConstraint<double>>(c_ad.ToDouble());
...
GTEST_TEST(SapBallConstraint, AccumulateSpatialImpulses) {
    ...
    const SpatialForce<double> F_Bo_W = (SpatialForce<double>(Vector3d::Zero(), gamma)).Shift(-kinematics.p_BQ_W());

    // Expected spatial impulse on A.
    const SpatialForce<double> F_Ao_W = (SpatialForce<double>(Vector3d::Zero(), -gamma)).Shift(-kinematics.p_AP_W());
