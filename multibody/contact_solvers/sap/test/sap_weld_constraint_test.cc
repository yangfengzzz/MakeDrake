...
typename SapWeldConstraint<T>::Kinematics MakeArbitraryKinematics(int num_cliques) {
    const int objectA = 12;
    RigidTransform<T> X_WP(RotationMatrix<T>::MakeZRotation(theta), Vector3d(1., 2., 3.));
    Vector3<T> p_AP_W(4., 5., 6.);
    const int objectB = 5;
    RigidTransform<T> X_WQ(RotationMatrix<T>::MakeZRotation(-theta), Vector3d(7., 8., 9.));
    Vector3<T> p_BQ_W(10., 11., 12.);
    const int clique0 = 3;
    const int clique1 = 12;
    auto J_AmBm_W = (num_cliques == 1) ? SapConstraintJacobian<T>(clique0, J62_W)
                                       : SapConstraintJacobian<T>(clique0, J62_W, clique1, J64_W);
    return typename SapWeldConstraint<T>::Kinematics{objectA, X_WP, p_AP_W, objectB, X_WQ, p_BQ_W, J_AmBm_W};
}

void ExpectEqual(const SapWeldConstraint<double>& c1, const SapWeldConstraint<double>& c2) {
...
GTEST_TEST(SapWeldConstraint, SingleCliqueConstraint) {
    ...
    const SapWeldConstraint<double>::Kinematics kinematics = MakeArbitraryKinematics(num_cliques);
    ...
    EXPECT_TRUE(CompareMatrices(c.first_clique_jacobian().MakeDenseMatrix(), J62_W, kEps, MatrixCompareType::relative));
...
GTEST_TEST(SapWeldConstraint, TwoCliquesConstraint) {
    ...
    EXPECT_TRUE(CompareMatrices(c.first_clique_jacobian().MakeDenseMatrix(), J62_W, kEps, MatrixCompareType::relative));
    EXPECT_TRUE(
            CompareMatrices(c.second_clique_jacobian().MakeDenseMatrix(), J64_W, kEps, MatrixCompareType::relative));
...
void ValidateProjection(const Vector6d& vc) {
    ...
    const int num_cliques = 1;
    const SapWeldConstraint<AutoDiffXd>::Kinematics kin_ad = MakeArbitraryKinematics<AutoDiffXd>(num_cliques);
...
GTEST_TEST(SapWeldConstraint, Gradients) {
    ...
    {
        const Vector6d vc = (Vector6d() << -1.2, 3.4, 5.6, -7.8, 9.1, -2.3).finished();
...
GTEST_TEST(SapWeldConstraint, TwoCliquesConstraintClone) {
    ...
    // Test ToDouble.
    SapWeldConstraint<AutoDiffXd> c_ad(MakeArbitraryKinematics<AutoDiffXd>(num_cliques));
    auto clone_from_ad = dynamic_pointer_cast<SapWeldConstraint<double>>(c_ad.ToDouble());
...
GTEST_TEST(SapWeldConstraint, AccumulateSpatialImpulses) {
    ...
    const Vector6d gamma = (Vector6d() << -1.2, 3.4, 5.6, -7.8, 9.1, -2.3).finished();

    // Expected spatial impulse on B.
    const SpatialForce<double> gamma_BBm_W(gamma.template head<3>(), gamma.template tail<3>());
    ...
    const SpatialForce<double> gamma_AAm_W(-gamma.template head<3>(), -gamma.template tail<3>());
    ...
    const SpatialForce<double> gamma0_BBo_W(Vector3d(1., 2., 3), Vector3d(4., 5., 6));
    SpatialForce<double> gamma_accumulated = gamma0_BBo_W;  // Initialize to non-zero value.
    ...
    EXPECT_TRUE(CompareMatrices(gamma_accumulated.get_coeffs(), gamma_BBo_W_expected.get_coeffs(),
                                std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));

    const SpatialForce<double> gamma0_AAo_W(Vector3d(1., 2., 3), Vector3d(4., 5., 6));
    ...
    EXPECT_TRUE(CompareMatrices(gamma_accumulated.get_coeffs(), gamma_AAo_W_expected.get_coeffs(),
                                std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
