...
typename SapCouplerConstraint<T>::Kinematics MakeArbitraryKinematics(int num_cliques) {
    const int clique0 = 4;
    ...
    return typename SapCouplerConstraint<T>::Kinematics{clique0,     clique_dof0, clique_nv0, q0,         clique1,
                                                        clique_dof1, clique_nv1,  q1,         gear_ratio, offset};
}

void ExpectEqual(const SapCouplerConstraint<double>& c1, const SapCouplerConstraint<double>& c2) {
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueConstraint) {
    ...
    const SapCouplerConstraint<double>::Kinematics kinematics = MakeArbitraryKinematics(num_cliques);
...
void ValidateProjection(const Vector1d& vc) {
    ...
    const int num_cliques = 1;
    const SapCouplerConstraint<AutoDiffXd>::Kinematics kin_ad = MakeArbitraryKinematics<AutoDiffXd>(num_cliques);
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueConstraintClone) {
    ...
    // Test ToDouble.
    SapCouplerConstraint<AutoDiffXd> c_ad(MakeArbitraryKinematics<AutoDiffXd>(num_cliques));
    auto clone_from_ad = dynamic_pointer_cast<SapCouplerConstraint<double>>(c_ad.ToDouble());
...
GTEST_TEST(SapCouplerConstraint, SingleCliqueAccumulateGeneralizedImpulses) {
    ...
    const VectorXd tau_0 = VectorXd::LinSpaced(kinematics.clique_nv0, 1, kinematics.clique_nv0);
    ...
    EXPECT_TRUE(CompareMatrices(tau_accumulated, tau_expected, std::numeric_limits<double>::epsilon(),
                                MatrixCompareType::relative));

    // Calling on clique one when num_cliques() == 1 should throw an exception.
    EXPECT_THROW(c.AccumulateGeneralizedImpulses(1, gamma, &tau_accumulated), std::exception);
}
...
GTEST_TEST(SapCouplerConstraint, TwoCliquesAccumulateGeneralizedImpulses) {
    ...
    const VectorXd tau_1 = VectorXd::LinSpaced(kinematics.clique_nv1, 1, kinematics.clique_nv1);
