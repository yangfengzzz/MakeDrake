...
#include "multibody/contact_solvers/sap/expect_equal.h"
#include "multibody/contact_solvers/sap/validate_constraint_gradients.h"
...
void ExpectEqual(const SapHolonomicConstraint<double>& c1, const SapHolonomicConstraint<double>& c2) {
...
public:
    void SetUp() override {
        SapHolonomicConstraint<double>::Parameters parameters = MakeArbitraryParameters();
        dut_ = std::make_unique<SapHolonomicConstraint<double>>(g_, SapConstraintJacobian<double>{clique1_, J_}, b_,
                                                                std::move(parameters));
    }

    template <typename T = double>
    static typename SapHolonomicConstraint<T>::Parameters MakeArbitraryParameters(double beta = 0) {
        ...
        return typename SapHolonomicConstraint<T>::Parameters(std::move(impulse_lower_limits),
                                                              std::move(impulse_upper_limits), std::move(stiffnesses),
                                                              std::move(relaxation_times), beta);
    }

    // Makes the same arbitrary set of parameters as MakeArbitraryParameters(),
    // but with an infinite stiffness.
    static SapHolonomicConstraint<double>::Parameters MakeArbitraryParametersWithInfiniteStiffness(double beta = 0) {
        const SapHolonomicConstraint<double>::Parameters params = MakeArbitraryParameters(beta);
        VectorXd stiffnesses = Vector3d::Constant(std::numeric_limits<double>::infinity());
        ...
        return SapHolonomicConstraint<double>::Parameters(params.impulse_lower_limits(), params.impulse_upper_limits(),
    ...
    static std::unique_ptr<SapHolonomicConstraint<AutoDiffXd>> ToAutoDiff(const SapHolonomicConstraint<double>& c) {
        const SapHolonomicConstraint<double>::Parameters& p = c.parameters();
        SapHolonomicConstraint<AutoDiffXd>::Parameters p_ad(p.impulse_lower_limits(), p.impulse_upper_limits(),
                                                            p.stiffnesses(), p.relaxation_times(), p.beta());
        return std::make_unique<SapHolonomicConstraint<AutoDiffXd>>(
                c.constraint_function(),
                SapConstraintJacobian<AutoDiffXd>{c.first_clique(), c.first_clique_jacobian().MakeDenseMatrix()},
                c.bias(), p_ad);
    }
...
TEST_F(SapHolonomicConstraintTests, SingleCliqueConstruction) {
    ...
    const SapHolonomicConstraint<double>::Parameters p = MakeArbitraryParameters();
    EXPECT_EQ(dut_->parameters().impulse_lower_limits(), p.impulse_lower_limits());
    EXPECT_EQ(dut_->parameters().impulse_upper_limits(), p.impulse_upper_limits());
...
TEST_F(SapHolonomicConstraintTests, TwoCliquesConstruction) {
    ...
    dut_ = std::make_unique<SapHolonomicConstraint<double>>(g, SapConstraintJacobian<double>{clique1, J1, clique2, J2},
                                                            p);
...
TEST_F(SapHolonomicConstraintTests, ValidateConstraintGradients) {
    std::unique_ptr<SapHolonomicConstraint<AutoDiffXd>> dut_ad = ToAutoDiff(*dut_);

    const int ne = dut_->num_constraint_equations();
    const double time_step = 0.02;
    VectorX<AutoDiffXd> delassus_estimation = VectorX<AutoDiffXd>::Constant(ne, 1.5);  // Arbitrary value.
    std::unique_ptr<AbstractValue> abstract_data = dut_ad->MakeData(time_step, delassus_estimation);
    const auto& data = abstract_data->get_value<SapHolonomicConstraintData<AutoDiffXd>>();
    ...
    // Impulses within limits.
    {
        ...
        EXPECT_TRUE((gl.array() < gamma.array()).all() && (gamma.array() < gu.array()).all());
    }
...
TEST_F(SapHolonomicConstraintTests, CalcDiagonalRegularization) {
    ...
    SapHolonomicConstraint<double>::Parameters parameters = MakeArbitraryParameters(beta);
    dut_ = std::make_unique<SapHolonomicConstraint<double>>(g_, SapConstraintJacobian<double>{clique1_, J_},
                                                            parameters);

    const double time_step = 0.01;
    const VectorXd delassus_approximation = VectorXd::Constant(dut_->num_constraint_equations(), 2.0);
    std::unique_ptr<AbstractValue> abstract_data = dut_->MakeData(time_step, delassus_approximation);
    const auto& data = abstract_data->get_value<SapHolonomicConstraintData<double>>();

    // Near-rigid regularization.
    const VectorXd R_near_rigid = beta * beta / (4.0 * M_PI * M_PI) * delassus_approximation;
    ...
    VectorXd R_expected = 1.0 / (time_step * (time_step + tau.array()) * k.array());

    // For this case we expect only the stiffer constraint, the third one, to be
    // in the rigid regime.
    R_expected(2) = R_near_rigid(2);

    EXPECT_TRUE(CompareMatrices(data.R(), R_expected, kEps, MatrixCompareType::relative));
}

TEST_F(SapHolonomicConstraintTests, RegularizationAndBiasFromInfiniteStiffness) {
    // We set a non-zero beta in order to test CalcDiagonalRegularization() for
    // parameters within the near-rigid regime.
    const double beta = 1.5;
    SapHolonomicConstraint<double>::Parameters parameters = MakeArbitraryParametersWithInfiniteStiffness(beta);
    dut_ = std::make_unique<SapHolonomicConstraint<double>>(g_, SapConstraintJacobian<double>{clique1_, J_},
                                                            parameters);
    ...
    EXPECT_TRUE(CompareMatrices(R, R_expected, kEps, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(vhat, vhat_expected, kEps, MatrixCompareType::relative));
}
...
TEST_F(SapHolonomicConstraintTests, Clone) {
    ...
    auto clone = dynamic_pointer_cast<SapHolonomicConstraint<double>>(dut_->Clone());
    ASSERT_NE(clone, nullptr);
    ExpectEqual(*dut_, *clone);

    // Test ToDouble.
    SapHolonomicConstraint<AutoDiffXd> c_ad(g_, SapConstraintJacobian<AutoDiffXd>(clique1_, J_), b_,
                                            MakeArbitraryParameters<AutoDiffXd>());
    auto clone_from_ad = dynamic_pointer_cast<SapHolonomicConstraint<double>>(c_ad.ToDouble());
