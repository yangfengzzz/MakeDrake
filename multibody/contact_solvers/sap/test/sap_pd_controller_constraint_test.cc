...
typename SapPdControllerConstraint<T>::Parameters MakeArbitraryParameters() {
    ...
    return typename SapPdControllerConstraint<T>::Parameters{Kp, Kd, effort_limit};
}

template <typename T = double>
typename SapPdControllerConstraint<T>::Configuration MakeArbitraryConfiguration() {
    const int clique = 4;
    ...
    const T vd = 0.3;
    const T u0 = 0.4;
    return typename SapPdControllerConstraint<T>::Configuration{clique, clique_dof, clique_nv, q0, qd, vd, u0};
}

void ExpectEqual(const SapPdControllerConstraint<double>& c1, const SapPdControllerConstraint<double>& c2) {
...
/* SapPdControllerConstraint models (without considering effort limits) the
 ...
 limits. */
double CalcVelocityFromActuationValue(const Configuration& k, const Parameters& p, double dt, double u) {
...
GTEST_TEST(SapPdControllerConstraint, Construction) {
    ...
    const MatrixX<double> J_expected = RowVectorX<double>::Unit(configuration.clique_nv, configuration.clique_dof);
...
GTEST_TEST(SapPdControllerConstraint, MakeDataOutsideTheNearRigidRegime) {
    ...
    const auto& data = abstract_data->get_value<SapPdControllerConstraintData<double>>();
...
void ValidateGradients(double v) {
    ...
    const SapPdControllerConstraint<AutoDiffXd>::Parameters p = MakeArbitraryParameters<AutoDiffXd>();
    const SapPdControllerConstraint<AutoDiffXd>::Configuration k = MakeArbitraryConfiguration<AutoDiffXd>();
...
GTEST_TEST(SapPdControllerConstraint, Clone) {
    ...
    auto clone = dynamic_pointer_cast<SapPdControllerConstraint<double>>(c.Clone());
    ASSERT_NE(clone, nullptr);
    ExpectEqual(c, *clone);

    // Test ToDouble.
    SapPdControllerConstraint<AutoDiffXd> c_ad(MakeArbitraryConfiguration<AutoDiffXd>(),
                                               MakeArbitraryParameters<AutoDiffXd>());
    auto clone_from_ad = dynamic_pointer_cast<SapPdControllerConstraint<double>>(c_ad.ToDouble());
...
GTEST_TEST(SapPdControllerConstraint, AccumulateGeneralizedImpulses) {
    ...
    const VectorXd tau_0 = VectorXd::LinSpaced(configuration.clique_nv, 1, configuration.clique_nv);
    ...
    EXPECT_TRUE(CompareMatrices(tau_accumulated, tau_expected, kEps, MatrixCompareType::relative));
}
