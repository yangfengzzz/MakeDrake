...
void ExpectEqual(const SapHuntCrossleyConstraint<double>& c1, const SapHuntCrossleyConstraint<double>& c2) {
...
typename SapHuntCrossleyConstraint<T>::Parameters MakeArbitraryParameters() {
    return typename SapHuntCrossleyConstraint<T>::Parameters{
            .friction = 0.5, .stiffness = 1.0e5, .dissipation = 0.01, .stiction_tolerance = 1.0e-4};
}
...
GTEST_TEST(SapHuntCrossleyConstraint, SingleCliqueConstraint) {
    const int clique = 12;
    const ContactConfiguration<double> configuration = MakeArbitraryConfiguration();
    SapConstraintJacobian<double> J(clique, J32);
    const SapHuntCrossleyConstraint<double>::Parameters parameters = MakeArbitraryParameters();
...
GTEST_TEST(SapHuntCrossleyConstraint, SapRegularization) {
    ...
    const ContactConfiguration<double> configuration = MakeArbitraryConfiguration();
    SapConstraintJacobian<double> J(clique, J32);
    SapHuntCrossleyConstraint<double>::Parameters parameters = MakeArbitraryParameters();

    const double time_step = 1e-2;
    const double w = 1.0;
    const Vector3d delassus_approximation = Vector3d::Constant(w);

    auto calc_invariant_data = [&](const SapHuntCrossleyConstraint<double>::Parameters& p) {
        SapHuntCrossleyConstraint<double> c(configuration, J, p);
        std::unique_ptr<AbstractValue> abstract_data = c.MakeData(time_step, delassus_approximation);
        const auto& data = abstract_data->get_value<SapHuntCrossleyConstraintData<double>>();
        const SapHuntCrossleyConstraintData<double>::InvariantData invariant_data = data.invariant_data;
    ...
    // Large regularization.
    {
        ...
        const SapHuntCrossleyConstraintData<double>::InvariantData invariant_data = calc_invariant_data(parameters);
...
class SapHuntCrossleyConstraintTest : public ::testing::TestWithParam<TestConfig> {
public:
    // This method validates analytical gradients implemented by
    // SapHuntCrossleyConstraint using automatic differentiation.
    void ValidateGradients(const SapHuntCrossleyConstraint<double>::Parameters& p, const Vector3d& vc) const {
        const SapHuntCrossleyApproximation approximation = this->GetParam().approximation;

        // Instantiate constraint on AutoDiffXd for automatic differentiation.
        SapHuntCrossleyConstraint<AutoDiffXd>::Parameters p_ad{approximation, p.friction,           p.stiffness,
        ...
        ContactConfiguration<AutoDiffXd> configuration = MakeArbitraryConfiguration<AutoDiffXd>();
        const SapHuntCrossleyConstraint<AutoDiffXd> c(std::move(configuration), std::move(J), std::move(p_ad));
    ...
    static std::array<double, kNumArbitraryValues> MakeArbitraryInContactVelocities() {
    ...
    static std::array<double, kNumArbitraryValues> MakeArbitraryNoContactVelocities() {
    ...
    // in stiction.
    static std::array<Vector2d, kNumArbitraryValues> MakeArbitraryStictionVelocities(double stiction_tolerance) {
        ...
        return {Vector2d(slip_within_stiction, 0.0), Vector2d(slip_within_stiction, -slip_within_stiction),
    ...
    // sliding.
    static std::array<Vector2d, kNumArbitraryValues> MakeArbitrarySlidingVelocities(double stiction_tolerance) {
        // Use a large enough velocity to ensure we are in the sliding regime.
        const double slip_velocity = 100 * stiction_tolerance;
        return {Vector2d(slip_velocity, 0.0), Vector2d(slip_velocity, -slip_velocity),
    ...
    // (vt[i], vn[i]).
    void CombineAndValidateGradients(const SapHuntCrossleyConstraint<double>::Parameters& p,
...
TEST_P(SapHuntCrossleyConstraintTest, ValidateGradientsForBreakingContact) {
    ...
    const auto vt_stiction = MakeArbitraryStictionVelocities(p.stiction_tolerance);
...
// stiction regime.
TEST_P(SapHuntCrossleyConstraintTest, InStictionWithZeroStiffnessAndZeroDissipation) {
    // An arbitrary set of parameters.
    SapHuntCrossleyConstraint<double>::Parameters p;
    p.friction = 0.5;
    p.stiffness = 0;
...
// potential is linear. Verify gradients when the constraint is working in the
// sliding regime.
TEST_P(SapHuntCrossleyConstraintTest, SlidingWithZeroStiffnessAndZeroDissipation) {
    // An arbitrary set of parameters.
    SapHuntCrossleyConstraint<double>::Parameters p;
    p.friction = 0.5;
    p.stiffness = 1.0e-14;
...
std::vector<TestConfig> MakeTestCases() {
    return std::vector<TestConfig>{
            {.description = "Similar", .approximation = SapHuntCrossleyApproximation::kSimilar},
            {.description = "Lagged", .approximation = SapHuntCrossleyApproximation::kLagged},
    };
}
...
GTEST_TEST(SapHuntCrossleyConstraint, SingleCliqueConstraintClone) {
    ...
    auto clone = dynamic_pointer_cast<SapHuntCrossleyConstraint<double>>(c.Clone());
    ASSERT_NE(clone, nullptr);
    ExpectEqual(c, *clone);

    // Test ToDouble.
    SapHuntCrossleyConstraint<AutoDiffXd> c_ad(MakeArbitraryConfiguration<AutoDiffXd>(),
                                               SapConstraintJacobian<AutoDiffXd>(clique, J32),
                                               MakeArbitraryParameters<AutoDiffXd>());
    auto clone_from_ad = dynamic_pointer_cast<SapHuntCrossleyConstraint<double>>(c_ad.ToDouble());
...
GTEST_TEST(SapHuntCrossleyConstraint, AccumulateSpatialImpulses) {
    ...
    EXPECT_TRUE(CompareMatrices(Faccumulated.get_coeffs(), F_Bq_W_expected.get_coeffs(),
    ...
    EXPECT_TRUE(CompareMatrices(Faccumulated.get_coeffs(), F_Ap_W_expected.get_coeffs(),
                                std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
