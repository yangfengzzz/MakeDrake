...
typename SapDistanceConstraint<T>::ComplianceParameters MakeArbitraryParameters() {
    const T stiffness = 1.0e5;
    const T damping = 1.0e3;
    return typename SapDistanceConstraint<T>::ComplianceParameters{stiffness, damping};
}

template <typename T = double>
typename SapDistanceConstraint<T>::Kinematics MakeArbitraryKinematics(int num_cliques) {
    ...
    const T length = 0.35;
    auto J_PQ_W = (num_cliques == 1) ? SapConstraintJacobian<T>(clique0, J32)
                                     : SapConstraintJacobian<T>(clique0, J32, clique1, J34);
    return typename SapDistanceConstraint<T>::Kinematics{objectA, p_WP, p_AP_W, objectB, p_WQ, p_BQ_W, length, J_PQ_W};
}

void ExpectEqual(const SapDistanceConstraint<double>& c1, const SapDistanceConstraint<double>& c2) {
...
GTEST_TEST(SapDistanceConstraint, SingleCliqueConstraint) {
    ...
    const SapDistanceConstraint<double>::ComplianceParameters parameters = MakeArbitraryParameters();
    const SapDistanceConstraint<double>::Kinematics kinematics = MakeArbitraryKinematics(num_cliques);
    ...
    EXPECT_EQ(c.first_clique_jacobian().MakeDenseMatrix(), p_hat.transpose() * J32);
...
GTEST_TEST(SapDistanceConstraint, TwoCliquesConstraint) {
    ...
    EXPECT_EQ(c.first_clique_jacobian().MakeDenseMatrix(), p_hat.transpose() * J32);
    EXPECT_EQ(c.second_clique_jacobian().MakeDenseMatrix(), p_hat.transpose() * J34);
    EXPECT_EQ(c.length(), kinematics.length());
    EXPECT_EQ(c.compliance_parameters().stiffness(), parameters.stiffness());
    EXPECT_EQ(c.compliance_parameters().damping(), parameters.damping());
}
...
void ValidateProjection(const SapDistanceConstraint<double>::ComplianceParameters& p, const Vector1d& vc) {
    SapDistanceConstraint<AutoDiffXd>::ComplianceParameters p_ad(p.stiffness(), p.damping());

    // Arbitrary kinematic values.
    const int num_cliques = 1;
    const SapDistanceConstraint<AutoDiffXd>::Kinematics kin_ad = MakeArbitraryKinematics<AutoDiffXd>(num_cliques);
...
GTEST_TEST(SapDistanceConstraint, SingleCliqueConstraintClone) {
    ...
    // Test ToDouble.
    SapDistanceConstraint<AutoDiffXd> c_ad(MakeArbitraryKinematics<AutoDiffXd>(num_cliques),
                                           MakeArbitraryParameters<AutoDiffXd>());
    auto clone_from_ad = dynamic_pointer_cast<SapDistanceConstraint<double>>(c_ad.ToDouble());
