...
#include "multibody/contact_solvers/newton_with_bisection.h"
...
void SapSolver<T>::PackSapSolverResults(const SapModel<T>& model,
    ...
    results->Resize(model.problem().num_velocities(), model.num_constraint_equations());
...
template <>
SapSolverStatus SapSolver<double>::SolveWithGuess(const SapContactProblem<double>& problem,
    ...
    auto model = std::make_unique<SapModel<double>>(&problem, parameters_.linear_solver_type);
...
//   ∂v/∂θ = −H⁻¹⋅∂m/∂θ.
template <>
SapSolverStatus SapSolver<AutoDiffXd>::SolveWithGuess(const SapContactProblem<AutoDiffXd>& problem_ad,
                                                      const VectorX<AutoDiffXd>& v_guess_ad,
                                                      SapSolverResults<AutoDiffXd>* results_ad) {
    if (problem_ad.num_constraints() == 0) {
        // In the absence of constraints the solution is trivially v = v*.
        results_ad->Resize(problem_ad.num_velocities(), problem_ad.num_constraint_equations());
    ...
    auto model = std::make_unique<SapModel<double>>(problem.get(), parameters_.linear_solver_type);
    auto context = model->MakeContext();
    ...
    // data in problem_ad.
    auto model_ad = std::make_unique<SapModel<AutoDiffXd>>(&problem_ad, parameters_.linear_solver_type);
    auto context_ad = model_ad->MakeContext();
    model_ad->GetMutableVelocities(context_ad.get()) = v_model;  // no gradients in v, only in θ.
    ...
    const HessianFactorizationCache& hessian_factorization = model->EvalHessianFactorizationCache(*context);
    ...
    const VectorX<AutoDiffXd> v_model_ad = drake::math::InitializeAutoDiff(v_model, dv_dtheta);
...
template <typename T>
SapSolverStatus SapSolver<T>::SolveWithGuessImpl(const SapModel<T>& model, Context<T>* context)
...
{  // NOLINT(whitespace/braces)
    ...
    for (;; ++k) {
        ...
        CalcStoppingCriteriaResidual(model, *context, &momentum_residual, &momentum_scale);
        stats_.optimality_criterion_reached =
                momentum_residual <= parameters_.abs_tolerance + parameters_.rel_tolerance * momentum_scale;
        ...
        if (stats_.optimality_criterion_reached || stats_.cost_criterion_reached) {
            ...
            if (ell > ell_previous + ell_slop) {
                DRAKE_LOGGER_DEBUG(
                        ...
                        "residual = {}\n",
                        k, std::abs(ell - ell_previous), alpha, momentum_residual / momentum_scale);
                if (parameters_.nonmonotonic_convergence_is_error) {
                    throw std::logic_error("SapSolver: Non-monotonic convergence detected.");
                }
            }
        }
        ...
        switch (parameters_.line_search_type) {
            case SapSolverParameters::LineSearchType::kBackTracking:
                std::tie(alpha, num_line_search_iters) =
                        PerformBackTrackingLineSearch(model, *context, search_direction_data, scratch.get());
                break;
            case SapSolverParameters::LineSearchType::kExact:
                std::tie(alpha, num_line_search_iters) =
                        PerformExactLineSearch(model, *context, search_direction_data, scratch.get());
                break;
        }
        ...
        stats_.cost_criterion_reached =
                ell_decrement < parameters_.cost_abs_tolerance + parameters_.cost_rel_tolerance * ell_scale &&
                alpha > 0.5;
    }
...
template <typename T>
T SapSolver<T>::CalcCostAlongLine(const SapModel<T>& model,
                                  const Context<T>& context,
    ...
    if (d2ell_dalpha2 != nullptr) {
        // N.B. This evaluation should be cheap given we called
        // EvalConstraintsHessian() at the very start of the scope of this function.
        const std::vector<MatrixX<T>>& G = model.EvalConstraintsHessian(context_alpha);
        ...
        if (isnan(d2ellR_dalpha2)) {
            throw std::logic_error(
                    fmt::format("The Hessian of the constraints cost along the search direction is "
                                "NaN. {}",
                                kNanValuesMessage));
        }
        if (d2ellR_dalpha2 < 0) {
...
template <typename T>
std::pair<T, int> SapSolver<T>::PerformBackTrackingLineSearch(const SapModel<T>& model,
                                                              const Context<T>& context,
                                                              const SearchDirectionData& search_direction_data,
                                                              Context<T>* scratch) const {
    DRAKE_DEMAND(parameters_.line_search_type == SapSolverParameters::LineSearchType::kBackTracking);
    DRAKE_DEMAND(scratch != nullptr);
    DRAKE_DEMAND(scratch != &context);
    using std::abs;
    ...
    const int max_iterations = parameters_.backtracking_line_search.max_iterations;
    ...
    T dell{NAN};
    T ell = CalcCostAlongLine(model, context, search_direction_data, alpha, scratch, &dell);
    ...
    const double ell_slop = parameters_.relative_slop / 10.0 * std::max(1.0, ell_scale);
    ...
    auto satisfies_armijo = [c, ell0, dell_dalpha0](const T& alpha_in, const T& ell_in) {
    ...
    for (; iteration < max_iterations; ++iteration) {
        alpha *= rho;
        ell = CalcCostAlongLine(model, context, search_direction_data, alpha, scratch);
        ...
        if (abs(dell_dalpha_approx) < ell_slop) return std::make_pair(alpha, iteration);
...
template <typename T>
std::pair<T, int> SapSolver<T>::PerformExactLineSearch(const SapModel<T>& model,
                                                       const Context<T>& context,
                                                       const SearchDirectionData& search_direction_data,
                                                       Context<T>* scratch) const
    requires std::is_same_v<T, double>
{  // NOLINT(whitespace/braces)
    DRAKE_DEMAND(parameters_.line_search_type == SapSolverParameters::LineSearchType::kExact);
    DRAKE_DEMAND(scratch != nullptr);
    DRAKE_DEMAND(scratch != &context);
    // dℓ/dα(α = 0) = ∇ᵥℓ(α = 0)⋅Δv.
    ...
    const double ell0 =
            CalcCostAlongLine(model, context, search_direction_data, alpha_max, scratch, &dell, &d2ell, &vec_scratch);
    ...
    if (-dell_dalpha0 < parameters_.cost_abs_tolerance + parameters_.cost_rel_tolerance * ell0)
    ...
    EvalData data{*this, model, context, search_direction_data, *scratch, dell_scale};

    // Cost and gradient of f(α) = −ℓ'(α)/ℓ'₀.
    auto cost_and_gradient = [&data](double x) {
        double dell_dalpha;
        double d2ell_dalpha2;
        data.solver.CalcCostAlongLine(data.model, data.context0, data.search_direction_data, x, &data.scratch,
                                      &dell_dalpha, &d2ell_dalpha2, &data.vec_scratch);
        return std::make_pair(dell_dalpha / data.dell_scale, d2ell_dalpha2 / data.dell_scale);
    };
    ...
    if (std::isnan(alpha_guess)) {
        throw std::logic_error(fmt::format("The initial guess for line search is NaN. {}", kNanValuesMessage));
    }

    // N.B. If we are here, then we already know that dell_dalpha0 < 0 and dell >
    // 0, and therefore [0, alpha_max] is a valid bracket.
    const Bracket bracket(0., dell_dalpha0 / dell_scale, alpha_max, dell / dell_scale);
    ...
    const auto [alpha, iters] =
            DoNewtonWithBisectionFallback(cost_and_gradient, bracket, alpha_guess, alpha_tolerance, f_tolerance,
...
template <typename T>
void SapSolver<T>::CalcSearchDirectionData(const SapModel<T>& model,
                                           const Context<T>& context,
...
{  // NOLINT(whitespace/braces)
    ...
    if (isnan(data->d2ellA_dalpha2)) {
        throw std::logic_error(
                fmt::format("The Hessian of the momentum cost along the search direction is NaN. "
                            "{}",
                            kNanValuesMessage));
    }
    if (data->d2ellA_dalpha2 <= 0) {
        throw std::logic_error(
                fmt::format("The Hessian of the momentum cost along the search direction is not "
