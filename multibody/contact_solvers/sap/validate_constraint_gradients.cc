...
void ValidateConstraintGradients(const SapConstraint<AutoDiffXd>& c, const AbstractValue& data) {
    ...
    const VectorXd minus_cost_gradient =
            (cost_ad.derivatives().size() == 0) ? VectorXd::Zero(nk) : VectorXd(-cost_ad.derivatives());
    ...
    EXPECT_TRUE(CompareMatrices(G, minus_gamma_ad_gradient, 32 * std::numeric_limits<double>::epsilon(),
                                MatrixCompareType::relative));
}

void ValidateConstraintGradients(const SapConstraint<AutoDiffXd>& c, const VectorXd& vc) {
    ...
    VectorX<AutoDiffXd> delassus_estimation = VectorX<AutoDiffXd>::Constant(nk, 1.0);
