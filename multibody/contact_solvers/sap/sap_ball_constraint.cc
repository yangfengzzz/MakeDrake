...
template <typename T>
typename SapHolonomicConstraint<T>::Parameters SapBallConstraint<T>::MakeSapHolonomicConstraintParameters() {
    ...
    const Vector3<T> kInf = std::numeric_limits<double>::infinity() * Vector3<T>::Ones();
    return typename SapHolonomicConstraint<T>::Parameters{-kInf, kInf, kInf, Vector3<T>::Zero(), kBeta};
}

template <typename T>
typename SapHolonomicConstraint<T>::Kinematics SapBallConstraint<T>::MakeSapHolonomicConstraintKinematics(
        const Kinematics& kinematics) {
    Vector3<T> g(kinematics.p_WQ() - kinematics.p_WP());  // Constraint function.
    Vector3<T> b = Vector3<T>::Zero();                    // Bias term.

    return typename SapHolonomicConstraint<T>::Kinematics(std::move(g), kinematics.jacobian(), std::move(b));
}

template <typename T>
void SapBallConstraint<T>::DoAccumulateSpatialImpulses(int i,
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapBallConstraint<T>::DoToDouble() const {
    SapConstraintJacobian<double> J = this->jacobian().ToDouble();
    SapBallConstraint<double>::Kinematics k(kinematics_.objectA(), math::DiscardGradient(kinematics_.p_WP()),
