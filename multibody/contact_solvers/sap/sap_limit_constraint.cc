...
template <typename T>
SapLimitConstraint<T>::Parameters::Parameters(
        const T& lower_limit, const T& upper_limit, const T& stiffness, const T& dissipation_time_scale, double beta)
...
template <typename T>
SapLimitConstraint<T>::SapLimitConstraint(int clique, int clique_dof, int clique_nv, const T& q0, Parameters parameters)
    : SapConstraint<T>(
              CalcConstraintJacobian(clique, clique_dof, clique_nv, parameters.lower_limit(), parameters.upper_limit()),
              {}),
      parameters_(std::move(parameters)),
      clique_dof_(clique_dof),
      q0_{q0},
      g_(CalcConstraintFunction(q0, parameters.lower_limit(), parameters.upper_limit())) {}

template <typename T>
VectorX<T> SapLimitConstraint<T>::CalcConstraintFunction(const T& q0, const T& ql, const T& qu) {
    constexpr double kInf = std::numeric_limits<double>::infinity();
...
std::unique_ptr<AbstractValue> SapLimitConstraint<T>::DoMakeData(
    ...
    VectorX<T> R = (beta_factor * delassus_estimation).cwiseMax(1.0 / (time_step * k * (time_step + taud)));
...
template <typename T>
void SapLimitConstraint<T>::DoCalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* abstract_data) const {
...
template <typename T>
void SapLimitConstraint<T>::DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const {
    const auto& data = abstract_data.get_value<SapLimitConstraintData<T>>();
    *gamma = data.gamma();
}

template <typename T>
void SapLimitConstraint<T>::DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const {
    const auto& data = abstract_data.get_value<SapLimitConstraintData<T>>();
    constexpr double kInf = std::numeric_limits<double>::infinity();
...
template <typename T>
void SapLimitConstraint<T>::DoAccumulateGeneralizedImpulses(int,
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapLimitConstraint<T>::DoToDouble() const {
    const typename SapLimitConstraint<T>::Parameters& p = parameters_;
    SapLimitConstraint<double>::Parameters p_to_double(
            ExtractDoubleOrThrow(p.lower_limit()), ExtractDoubleOrThrow(p.upper_limit()),
            ExtractDoubleOrThrow(p.stiffness()), ExtractDoubleOrThrow(p.dissipation_time_scale()), p.beta());
    ...
    return std::make_unique<SapLimitConstraint<double>>(clique, clique_dof(), clique_nv,
                                                        ExtractDoubleOrThrow(position()), std::move(p_to_double));
}
