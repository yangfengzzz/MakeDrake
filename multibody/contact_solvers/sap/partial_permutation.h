...
public:
    ...
    void Apply(const VectorType& x, PointerToPermutedVectorType x_permuted) const {
        ...
        DRAKE_THROW_UNLESS(static_cast<int>(x_permuted->size()) == permuted_domain_size());
    ...
    void ApplyInverse(const PermutedVectorType& x_permuted, PointerToVectorType x) const {
        DRAKE_THROW_UNLESS(static_cast<int>(x_permuted.size()) == permuted_domain_size());
