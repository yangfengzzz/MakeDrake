...
SapCouplerConstraint<T>::SapCouplerConstraint(Kinematics kinematics)
    : SapHolonomicConstraint<T>(
              MakeSapHolonomicConstraintKinematics(kinematics), MakeSapHolonomicConstraintParameters(), {}),
      kinematics_(std::move(kinematics)) {}

template <typename T>
typename SapHolonomicConstraint<T>::Parameters SapCouplerConstraint<T>::MakeSapHolonomicConstraintParameters() {
    ...
    return typename SapHolonomicConstraint<T>::Parameters{Vector1<T>(-kInf), Vector1<T>(kInf), Vector1<T>(kInf),
                                                          Vector1<T>(0.0), kBeta};
}

template <typename T>
typename SapHolonomicConstraint<T>::Kinematics SapCouplerConstraint<T>::MakeSapHolonomicConstraintKinematics(
        const Kinematics& kinematics) {
    Vector1<T> g(kinematics.q0 - kinematics.gear_ratio * kinematics.q1 - kinematics.offset);  // Constraint function.
    ...
    if (kinematics.clique0 == kinematics.clique1) {
        ...
        return typename SapHolonomicConstraint<T>::Kinematics(
                std::move(g), SapConstraintJacobian<T>(kinematics.clique0, std::move(J0)), std::move(b));
    } else {
        MatrixX<T> J0 = MatrixX<T>::Zero(1, kinematics.clique_nv0);
        ...
        return typename SapHolonomicConstraint<T>::Kinematics(
                std::move(g),
                SapConstraintJacobian<T>(kinematics.clique0, std::move(J0), kinematics.clique1, std::move(J1)),
                std::move(b));
    }
}

template <typename T>
void SapCouplerConstraint<T>::DoAccumulateGeneralizedImpulses(int c,
...
template <typename T>
std::unique_ptr<SapConstraint<double>> SapCouplerConstraint<T>::DoToDouble() const {
    SapCouplerConstraint<double>::Kinematics k{kinematics_.clique0,
