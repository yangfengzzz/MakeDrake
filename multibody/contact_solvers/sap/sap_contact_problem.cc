...
template <typename T>
SapContactProblem<T>::SapContactProblem(const T& time_step, std::vector<MatrixX<T>> A, VectorX<T> v_star)
    : time_step_(time_step), A_(std::move(A)), v_star_(std::move(v_star)), graph_(num_cliques()) {
    ...
    for (int i = 0; i < ssize(A_); ++i) {
        ...
        if (i > 0) velocities_start_[i] = velocities_start_[i - 1] + num_velocities(i - 1);
        nv_ += Ac.rows();
    }
...
template <typename T>
std::unique_ptr<SapContactProblem<double>> SapContactProblem<T>::ToDouble() const {
    ...
    auto clone = std::make_unique<SapContactProblem<double>>(time_step, std::move(A), std::move(v_star));
    clone->set_num_objects(num_objects());
    for (int i = 0; i < num_constraints(); ++i) {
        const SapConstraint<T>& c = get_constraint(i);
...
std::unique_ptr<SapContactProblem<T>> SapContactProblem<T>::MakeReduced(
    ...
    DRAKE_ASSERT_VOID(drake::multibody::internal::DemandIndicesValid(known_free_motion_dofs, num_velocities()));
    DRAKE_DEMAND(ssize(per_clique_known_free_motion_dofs) <= num_cliques());
    for (int i = 0; i < ssize(per_clique_known_free_motion_dofs); ++i) {
        DRAKE_ASSERT_VOID(drake::multibody::internal::DemandIndicesValid(per_clique_known_free_motion_dofs[i],
                                                                         num_velocities(i)));
    }
    ...
    mapping->constraint_equation_permutation = PartialPermutation(num_constraint_equations());
    ...
    VectorX<T> v_star_reduced = drake::multibody::internal::ExcludeRows(v_star_, known_free_motion_dofs);
    ...
    for (int i = 0; i < ssize(per_clique_known_free_motion_dofs); ++i) {
        ...
        if (ssize(per_clique_known_free_motion_dofs[i]) < num_velocities(i)) {
            A_reduced.push_back(
                    drake::multibody::internal::ExcludeRowsCols(A_[i], per_clique_known_free_motion_dofs[i]));
            mapping->clique_permutation.push(i);
        }
    }
    // Copy the rest of the cliques.
    for (int i = ssize(per_clique_known_free_motion_dofs); i < num_cliques(); ++i) {
    ...
    std::unique_ptr<SapContactProblem> problem =
            std::make_unique<SapContactProblem<T>>(time_step(), std::move(A_reduced), std::move(v_star_reduced));
    problem->set_num_objects(num_objects());

    // Make reduced constraints.
    int constraint_equation_index = 0;
    for (const auto& c : constraints_) {
        std::unique_ptr<SapConstraint<T>> c_reduced =
                c->MakeReduced(mapping->clique_permutation, per_clique_known_free_motion_dofs);

        if (c_reduced) {
            problem->AddConstraint(std::move(c_reduced));
            for (int j = 0; j < c->num_constraint_equations(); ++j) {
                mapping->constraint_equation_permutation.push(constraint_equation_index + j);
            }
        }
...
template <typename T>
void SapContactProblem<T>::ExpandContactSolverResults(const ReducedMapping& reduced_mapping,
                                                      const SapSolverResults<T>& reduced_results,
                                                      SapSolverResults<T>* results) const {
    DRAKE_DEMAND(reduced_mapping.velocity_permutation.domain_size() == num_velocities());
    DRAKE_DEMAND(reduced_mapping.clique_permutation.domain_size() == num_cliques());
    DRAKE_DEMAND(reduced_mapping.constraint_equation_permutation.domain_size() == num_constraint_equations());
    DRAKE_DEMAND(reduced_results.v.size() == reduced_mapping.velocity_permutation.permuted_domain_size());
    DRAKE_DEMAND(reduced_results.gamma.size() ==
                 reduced_mapping.constraint_equation_permutation.permuted_domain_size());
    DRAKE_DEMAND(reduced_results.vc.size() == reduced_mapping.constraint_equation_permutation.permuted_domain_size());
    DRAKE_DEMAND(reduced_results.j.size() == reduced_mapping.velocity_permutation.permuted_domain_size());
    ...
    for (int i = 0; i < num_constraints(); ++i) {
        ...
        Eigen::VectorBlock<VectorX<T>> vc_segment =
                results->vc.segment(constraint_equations_start(i), c.num_constraint_equations());

        c.first_clique_jacobian().MultiplyAndAddTo(
                results->v.segment(velocities_start(c.first_clique()), num_velocities(c.first_clique())), &vc_segment);

        if (c.num_cliques() > 1) {
            c.second_clique_jacobian().MultiplyAndAddTo(
                    results->v.segment(velocities_start(c.second_clique()), num_velocities(c.second_clique())),
                    &vc_segment);
        }
    }

    // Copy v and j for participating velocities.
    reduced_mapping.velocity_permutation.ApplyInverse(reduced_results.v, &results->v);
    reduced_mapping.velocity_permutation.ApplyInverse(reduced_results.j, &results->j);

    // Copy gamma and vc for participating constraints.
    reduced_mapping.constraint_equation_permutation.ApplyInverse(reduced_results.gamma, &results->gamma);
    reduced_mapping.constraint_equation_permutation.ApplyInverse(reduced_results.vc, &results->vc);
}

template <typename T>
int SapContactProblem<T>::AddConstraint(std::unique_ptr<SapConstraint<T>> c) {
    if (c->first_clique() >= num_cliques()) {
        throw std::runtime_error("First clique index must be strictly lower than num_cliques()");
    }
    if (c->num_cliques() == 2 && c->second_clique() >= num_cliques()) {
        throw std::runtime_error("Second clique index must be strictly lower than num_cliques()");
    }
    ...
    if (c->num_cliques() == 2 && c->second_clique_jacobian().cols() != num_velocities(c->second_clique())) {
        throw std::runtime_error(
                "The number of columns in the constraint's "
                "Jacobian does not match the number of velocities in this problem for "
                "the second clique.");
    }
    if (num_velocities(c->first_clique()) == 0 || (c->num_cliques() == 2 && num_velocities(c->second_clique()) == 0)) {
        throw std::runtime_error(
    ...
    for (int object_index : c->objects()) {
        if (object_index < 0 || object_index >= num_objects())
            throw std::runtime_error("Constraint object indices must be in the range [0, num_objects()).");
    }

    // Update graph.
    const int ni = c->num_constraint_equations();
    const int constraint_index = c->num_cliques() == 1
    ...
    constraint_equations_start_.push_back(constraint_equations_start_.back() + c->num_constraint_equations());
...
template <typename T>
void SapContactProblem<T>::CalcConstraintGeneralizedForces(const VectorX<T>& gamma,
                                                           int constraint_start,
                                                           int constraint_end,
                                                           VectorX<T>* generalized_forces) const {
    DRAKE_THROW_UNLESS(0 <= constraint_start && constraint_start < num_constraints());
    ...
    for (int i = constraint_start; i <= constraint_end; ++i) {
        ...
        for (int c = 0; c < constraint.num_cliques(); ++c) {
            ...
            auto clique_forces = generalized_forces->segment(velocities_start(clique), num_velocities(clique));
            constraint.AccumulateGeneralizedImpulses(c, constraint_gamma, &clique_forces);
        }
    }
...
template <typename T>
void SapContactProblem<T>::CalcConstraintMultibodyForces(const VectorX<T>& gamma,
