...
SapPdControllerConstraint<T>::Parameters::Parameters(T Kp, T Kd, T effort_limit)
    : Kp_(std::move(Kp)), Kd_(std::move(Kd)), effort_limit_(std::move(effort_limit)) {
...
template <typename T>
SapPdControllerConstraint<T>::SapPdControllerConstraint(Configuration configuration, Parameters parameters)
...
std::unique_ptr<AbstractValue> SapPdControllerConstraint<T>::DoMakeData(
        const T& dt, const Eigen::Ref<const VectorX<T>>& delassus_estimation) const {
...
template <typename T>
void SapPdControllerConstraint<T>::DoCalcData(const Eigen::Ref<const VectorX<T>>& vc,
                                              AbstractValue* abstract_data) const {
    auto& data = abstract_data->get_mutable_value<SapPdControllerConstraintData<T>>();
    // Const data.
...
template <typename T>
T SapPdControllerConstraint<T>::DoCalcCost(const AbstractValue& abstract_data) const {
    const auto& data = abstract_data.get_value<SapPdControllerConstraintData<T>>();
    return data.cost();
}

template <typename T>
void SapPdControllerConstraint<T>::DoCalcImpulse(const AbstractValue& abstract_data, EigenPtr<VectorX<T>> gamma) const {
    const auto& data = abstract_data.get_value<SapPdControllerConstraintData<T>>();
    *gamma = Vector1<T>::Constant(data.impulse());
}

template <typename T>
void SapPdControllerConstraint<T>::DoCalcCostHessian(const AbstractValue& abstract_data, MatrixX<T>* G) const {
    const auto& data = abstract_data.get_value<SapPdControllerConstraintData<T>>();
    (*G)(0, 0) = data.hessian();
}

template <typename T>
void SapPdControllerConstraint<T>::DoAccumulateGeneralizedImpulses(int,
...
template <typename T>
SapConstraintJacobian<T> SapPdControllerConstraint<T>::MakeConstraintJacobian(Configuration c) {
    MatrixX<T> J = RowVectorX<T>::Unit(c.clique_nv, c.clique_dof);
    return SapConstraintJacobian<T>(c.clique, std::move(J));
}

template <typename T>
std::unique_ptr<SapConstraint<double>> SapPdControllerConstraint<T>::DoToDouble() const {
    const typename SapPdControllerConstraint<T>::Parameters& p = parameters_;
    const typename SapPdControllerConstraint<T>::Configuration& c = configuration_;
    SapPdControllerConstraint<double>::Parameters p_to_double(
            ExtractDoubleOrThrow(p.Kp()), ExtractDoubleOrThrow(p.Kd()), ExtractDoubleOrThrow(p.effort_limit()));
    SapPdControllerConstraint<double>::Configuration c_to_double{c.clique,
                                                                 c.clique_dof,
                                                                 c.clique_nv,
    ...
    return std::make_unique<SapPdControllerConstraint<double>>(std::move(c_to_double), std::move(p_to_double));
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(
        class ::drake::multibody::contact_solvers::internal::SapPdControllerConstraint);
