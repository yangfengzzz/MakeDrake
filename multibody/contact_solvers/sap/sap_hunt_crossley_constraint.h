...
public:
    ...
    //@{
    SapHuntCrossleyConstraint& operator=(const SapHuntCrossleyConstraint&) = delete;
    ...
    SapHuntCrossleyConstraint(ContactConfiguration<T> configuration, SapConstraintJacobian<T> J, Parameters parameters);
...
private:
    ...
    void DoCalcData(const Eigen::Ref<const VectorX<T>>& vc, AbstractValue* data) const override;
    ...
    std::unique_ptr<SapConstraint<T>> DoClone() const final {
        return std::unique_ptr<SapHuntCrossleyConstraint<T>>(new SapHuntCrossleyConstraint<T>(*this));
    }
