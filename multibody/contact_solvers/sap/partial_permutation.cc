...
PartialPermutation::PartialPermutation(std::vector<int> permutation) : permutation_(std::move(permutation)) {
    const int domain_size = permutation_.size();
    // Determine size of the permuted domain.
    const int permuted_domain_size = *std::max_element(permutation_.begin(), permutation_.end()) + 1;

    if (permuted_domain_size > domain_size) {
        throw std::logic_error(
                fmt::format("The size of the permuted domain must be smaller or equal than that of "
...
int PartialPermutation::permuted_index(int i) const {
    DRAKE_THROW_UNLESS(0 <= i && i < domain_size());
    if (permutation_[i] < 0) {
        throw std::runtime_error(fmt::format("Index {} does not participate in this permutation.", i));
    }
