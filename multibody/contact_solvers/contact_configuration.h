...
struct ContactConfiguration {
    ...
    ContactConfiguration<double> ToDouble() const {
        return ContactConfiguration<double>{.objectA = objectA,
                                            ...
                                            .R_WC = math::RotationMatrix<double>(math::DiscardGradient(R_WC.matrix()))};
    }
...
template <typename T>
ContactConfiguration<T> MakeContactConfiguration(const multibody::internal::DiscreteContactPair<T>& input) {
