...
void RemoveValueFromSortedVector(int value, std::vector<int>* sorted_vector) {
    auto it = std::lower_bound(sorted_vector->begin(), sorted_vector->end(), value);
...
std::vector<int> ComputeMinimumDegreeOrdering(const BlockSparsityPattern& block_sparsity_pattern) {
...
std::vector<int> ComputeMinimumDegreeOrdering(const BlockSparsityPattern& block_sparsity_pattern,
    ...
    /* Keep the nodes sorted by degrees and break ties with indices. We use a
     ...
     (degree and index). */
    for (int n = 0; n < num_nodes; ++n) {
        SimplifiedNode node = {.degree = nodes[n].degree,
                               .index = nodes[n].index,
    ...
    for (int k = 0; k < num_nodes; ++k) {
        ...
        const SimplifiedNode min_node = sorted_nodes.extract(sorted_nodes.begin()).value();
        ...
        for (int i : node_p.L) {
            Node& node_i = nodes[i];
            const SimplifiedNode old_node = {.degree = node_i.degree,
                                             .index = node_i.index,
            ...
            SimplifiedNode new_node = {.degree = node_i.degree, .index = node_i.index, .priority = old_node.priority};
...
/* The complexity of this symbolic factorization is O(|L|) where L is the
 factorized matrix. */
BlockSparsityPattern SymbolicCholeskyFactor(const BlockSparsityPattern& block_sparsity) {
    ...
    for (int j = 0; j < N; ++j) {
        ...
        L_sparsity[j] =
                std::vector<int>(std::lower_bound(L_sparsity[j].begin(), L_sparsity[j].end(), j), L_sparsity[j].end());
...
std::vector<int> CalcAndConcatenateMdOrderingWithinGroup(const BlockSparsityPattern& global_pattern,
    ...
    const std::vector<int> v1_ordering =
            ComputeMinimumDegreeOrdering(BlockSparsityPattern(std::move(v1_block_sizes), std::move(G1)));
    const std::vector<int> v2_ordering =
            ComputeMinimumDegreeOrdering(BlockSparsityPattern(std::move(v2_block_sizes), std::move(G2)));
