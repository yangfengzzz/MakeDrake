...
public:
    ...
    BlockSparsityPattern(std::vector<int> block_sizes, std::vector<std::vector<int>> neighbors)
...
public:
    DRAKE_DEFAULT_COPY_AND_MOVE_AND_ASSIGN(BlockSparseLowerTriangularOrSymmetricMatrix);
    ...
    /* Constructs a BlockSparseLowerTriangularOrSymmetricMatrix with the given
     ...
     @param sparsity_pattern  The block sparsity pattern of lower triangular part
                              of the matrix. */
    explicit BlockSparseLowerTriangularOrSymmetricMatrix(BlockSparsityPattern sparsity_pattern);
    ...
    void SetBlockFlat(int flat, int j, MatrixType Aij) {
        ...
        DRAKE_ASSERT_VOID(AssertValid(block_row_indices(j)[flat], j, Aij, __func__));
        MatrixType& old_value = blocks_[j][flat];
        DRAKE_ASSERT(old_value.rows() == Aij.rows());
        DRAKE_ASSERT(old_value.cols() == Aij.cols());
    ...
    const BlockSparsityPattern& sparsity_pattern() const { return sparsity_pattern_; }
...
private:
    ...
    */
    void AssertValid(int i, int j, const std::optional<MatrixType>& Aij, const char* source) const;
...
using BlockSparseLowerTriangularMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<MatrixX<double>, false>;
using BlockSparseSymmetricMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<MatrixX<double>, true>;
using Block3x3SparseLowerTriangularMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<Matrix3<double>, false>;
using Block3x3SparseSymmetricMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<Matrix3<double>, true>;
