...
public:
    ...
    void Multiply(const Eigen::Ref<const VectorX<T>>& x, EigenPtr<VectorX<T>> y) const;
    ...
    void MultiplyByTranspose(const Eigen::Ref<const VectorX<T>>& x, EigenPtr<VectorX<T>> y) const;
...
public:
    ...
    BlockSparseMatrixBuilder(int block_rows, int block_cols, int nonzero_blocks_capacity)
