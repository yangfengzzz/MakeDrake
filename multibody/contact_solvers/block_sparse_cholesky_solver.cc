#include "multibody/contact_solvers/block_sparse_cholesky_solver.h"

#include <algorithm>
#include <utility>
#include <vector>

#include "multibody/contact_solvers/minimum_degree_ordering.h"
...
void BlockSparseCholeskySolver<BlockType>::SetMatrix(const SymmetricMatrix& A) {
    ...
    const std::vector<int> elimination_ordering = ComputeMinimumDegreeOrdering(A_block_pattern);
    BlockSparsityPattern L_block_pattern = SymbolicFactor(A, elimination_ordering);
    SetMatrixImpl(A, elimination_ordering, std::move(L_block_pattern));
}

template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::UpdateMatrix(const SymmetricMatrix& A) {
    PermuteAndCopyToL(A);
...
template <typename BlockType>
std::optional<MatrixX<double>> BlockSparseCholeskySolver<BlockType>::FactorAndCalcSchurComplement(
        const SymmetricMatrix& A, const std::unordered_set<int>& eliminated_blocks) {
    ...
    if (num_remaining_blocks == 0) {
        ...
        return success ? std::optional<MatrixX<double>>(MatrixX<double>::Zero(0, 0)) : std::nullopt;
    }
    ...
    /* The Schur complement of the `eliminated_blocks` appear on the bottom
     ...
     fill-in. */
    const std::vector<int> elimination_ordering = ComputeMinimumDegreeOrdering(A.sparsity_pattern(), eliminated_blocks);
    SetMatrixImpl(A, elimination_ordering, SymbolicFactor(A, elimination_ordering));
    ...
    const MatrixX<double> permuted_S =
            L_->MakeDenseBottomRightCorner(num_remaining_blocks).template selfadjointView<Eigen::Lower>();

    /* Finish the factorization. If the factorization fails, set the solver mode
     and exit. */
    bool success = CalcPartialFactorization(num_eliminated_blocks, L_->block_cols());
    solver_mode_ = success ? SolverMode::kFactored : SolverMode::kEmpty;
    if (!success) {
        return std::nullopt;
    }
    ...
    for (int i = 1; i < num_remaining_blocks; ++i) {
        remaining_starting_indices[i] = remaining_starting_indices[i - 1] + remaining_block_sizes[i - 1];
    }
    ...
    /* We start with permuted_i = ssize(eliminated_blocks) to skip all
     eliminated blocks. */
    for (int permuted_i = ssize(eliminated_blocks); permuted_i < ssize(elimination_ordering); ++permuted_i) {
    ...
    const int num_remaining_scalars = remaining_starting_indices.back() + remaining_block_sizes.back();
...
template <typename BlockType>
VectorX<double> BlockSparseCholeskySolver<BlockType>::Solve(const Eigen::Ref<const VectorX<double>>& b) const {
    VectorX<double> x(b);
    SolveInPlace(&x);
    return x;
}
...
template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::SolveInPlace(VectorX<double>* b) const {
    ...
    for (int j = 0; j < L_->block_cols(); ++j) {
        ...
        const VectorX<double> bj = L_diag_[j].matrixL().solve(permuted_b.segment(offset, block_size));
        ...
        for (int flat = 1; flat < ssize(blocks_in_col_j); ++flat) {
            ...
            permuted_b.segment(starting_cols[i], block_sizes[i]).noalias() -= L_->block_flat(flat, j) * bj;
        }
    }
    ...
    for (int j = L_->block_cols() - 1; j >= 0; --j) {
        ...
        for (int flat = 1; flat < ssize(blocks_in_col_j); ++flat) {
            ...
            permuted_z.segment(starting_cols[j], block_sizes[j]).noalias() -=
                    L_->block_flat(flat, j).transpose() * permuted_z.segment(starting_cols[i], block_sizes[i]);
        }
        /* Solve for the j-th block entry. */
        const VectorX<double> zj = L_diag_[j].matrixU().solve(permuted_z.segment(starting_cols[j], block_sizes[j]));
...
template <typename BlockType>
typename BlockSparseCholeskySolver<BlockType>::LowerTriangularMatrix BlockSparseCholeskySolver<BlockType>::L() const {
    DRAKE_THROW_UNLESS(solver_mode() == SolverMode::kFactored);
    return *L_;
}

template <typename BlockType>
Eigen::PermutationMatrix<Eigen::Dynamic> BlockSparseCholeskySolver<BlockType>::CalcPermutationMatrix() const {
    DRAKE_THROW_UNLESS(solver_mode() != SolverMode::kEmpty);
    const std::vector<int>& p = scalar_permutation_.permutation();
    return Eigen::PermutationMatrix<Eigen::Dynamic>(Eigen::Map<const VectorX<int>>(p.data(), p.size()));
}

template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::SetMatrixImpl(const SymmetricMatrix& A,
...
template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::SetScalarPermutation(const SymmetricMatrix& A,
                                                                const std::vector<int>& elimination_ordering) {
    ...
    std::vector<int> scalar_elimination_ordering(A.cols());
    {
        ...
        int i_permuted = 0;
        for (int block_permuted = 0; block_permuted < ssize(elimination_ordering); ++block_permuted) {
    ...
    for (int i_permuted = 0; i_permuted < ssize(scalar_permutation); ++i_permuted) {
...
BlockSparsityPattern BlockSparseCholeskySolver<BlockType>::SymbolicFactor(
    ...
    const BlockSparsityPattern& A_block_pattern = A.sparsity_pattern();
    const std::vector<int>& A_block_sizes = A_block_pattern.block_sizes();
    const std::vector<std::vector<int>>& sparsity_pattern = A_block_pattern.neighbors();
    std::vector<std::vector<int>> permuted_sparsity_pattern(sparsity_pattern.size());
    for (int i = 0; i < ssize(sparsity_pattern); ++i) {
        ...
        for (int j : sparsity_pattern[i]) {
            ...
            permuted_sparsity_pattern[std::min(pi, pj)].emplace_back(std::max(pi, pj));
        }
    }
...
template <typename BlockType>
bool BlockSparseCholeskySolver<BlockType>::CalcPartialFactorization(int starting_col_block, int ending_col_block) {
    DRAKE_THROW_UNLESS(solver_mode() == SolverMode::kAnalyzed);
    DRAKE_DEMAND(starting_col_block >= 0 && starting_col_block <= L_->block_cols());
...
template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::RightLookingSymmetricRank1Update(int j) {
...
template <typename BlockType>
void BlockSparseCholeskySolver<BlockType>::PermuteAndCopyToL(const SymmetricMatrix& A) {
