...
public:
    ...
    using SymmetricMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<BlockType, true>;
    using LowerTriangularMatrix = BlockSparseLowerTriangularOrSymmetricMatrix<BlockType, false>;
...
private:
    ...
    /* Sets `scalar_permutation_` given the matrix A and the prescribed
     ...
     @param[in] elimination_ordering  Elimination ordering of the blocks of A.
                                      Must be a permutation of {0, 1, ...,
                                      A.block_cols() - 1}. */
    void SetScalarPermutation(const SymmetricMatrix& A, const std::vector<int>& elimination_ordering);
    ...
    /* Returns the block sparsity pattern of the L matrix from a block sparse
     ...
     @param[in] elimination_ordering  Elimination ordering of the blocks of A.
                                      Must be a permutation of {0, 1, ...,
                                      A.block_cols() - 1}. */
    BlockSparsityPattern SymbolicFactor(const SymmetricMatrix& A, const std::vector<int>& elimination_ordering);
