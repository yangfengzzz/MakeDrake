...
BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::BlockSparseLowerTriangularOrSymmetricMatrix(
    ...
    for (int j = 1; j < block_cols_; ++j) {
        starting_cols_[j] = starting_cols_[j - 1] + sparsity_pattern_.block_sizes()[j - 1];
    }
    cols_ = block_cols_ == 0 ? 0 : starting_cols_.back() + sparsity_pattern_.block_sizes().back();
...
template <typename MatrixType, bool is_symmetric>
void BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::SetZero() {
...
template <typename MatrixType, bool is_symmetric>
MatrixX<double> BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::MakeDenseMatrix() const {
    return MakeDenseBottomRightCorner(block_cols());
}

template <typename MatrixType, bool is_symmetric>
MatrixX<double> BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::MakeDenseBottomRightCorner(
    ...
    MatrixX<double> result = MatrixX<double>::Zero(rows() - row_start, cols() - col_start);
    for (int j = block_col_start; j < block_cols(); ++j) {
        for (int flat = 0; flat < ssize(block_row_indices(j)); ++flat) {
            ...
            if (i != j && is_symmetric) {
                result.block(dest_col, dest_row, num_cols, num_rows) = blocks_[j][flat].transpose();
            }
        }
    }
    return result;
...
template <typename MatrixType, bool is_symmetric>
void BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::ZeroRowsAndColumns(
        const std::vector<int>& indices) {
    DRAKE_THROW_UNLESS(is_symmetric);
    for (int j : indices) {
        if (!(0 <= j && j < block_cols())) {
            throw std::logic_error(fmt::format("Input index out of range. Indices must lie in [0, {}); {} is given.",
                                               block_cols(), j));
        }
    }
...
template <typename MatrixType, bool is_symmetric>
void BlockSparseLowerTriangularOrSymmetricMatrix<MatrixType, is_symmetric>::AssertValid(
        int i, int j, const std::optional<MatrixType>& Aij, const char* source) const {
    if (!(0 <= j && j <= i && i <= block_rows())) {
        throw std::runtime_error(
                fmt::format("{}: block indices out of bound. It is required that 0 <= j && j <= i "
                            "&& i < block_rows(). Instead, i = {}, j = {}, block_rows() = {}.",
                            source, i, j, block_rows()));
    }
    if (!HasBlock(i, j)) {
        throw std::runtime_error(fmt::format("{}: The requested {},{}-th block doesn't exist.", source, i, j));
    }
...
template class BlockSparseLowerTriangularOrSymmetricMatrix<MatrixX<double>, true>;
template class BlockSparseLowerTriangularOrSymmetricMatrix<MatrixX<double>, false>;
template class BlockSparseLowerTriangularOrSymmetricMatrix<Matrix3<double>, true>;
template class BlockSparseLowerTriangularOrSymmetricMatrix<Matrix3<double>, false>;
