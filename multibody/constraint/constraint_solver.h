...
public:
    ...
    static void ConstructBaseDiscretizedTimeLcp(const ConstraintVelProblemData<T>& problem_data,
    ...
    static void UpdateDiscretizedTimeLcp(const ConstraintVelProblemData<T>& problem_data,
                                         double h,
                                         MlcpToLcpData* mlcp_to_lcp_data,
                                         VectorX<T>* a,
                                         MatrixX<T>* MM,
    ...
    /// @pre cf is non-null.
    static void PopulatePackedConstraintForcesFromLcpSolution(const ConstraintVelProblemData<T>& problem_data,
    ...
    static void ComputeGeneralizedForceFromConstraintForces(const ConstraintAccelProblemData<T>& problem_data,
                                                            const VectorX<T>& cf,
    ...
    /// @throws std::exception if `generalized_force` is null or `cf`
    ///         vector is incorrectly sized.
    static void ComputeGeneralizedForceFromConstraintForces(const ConstraintVelProblemData<T>& problem_data,
                                                            const VectorX<T>& cf,
                                                            VectorX<T>* generalized_force);
    ...
    static void ComputeGeneralizedAcceleration(const ConstraintAccelProblemData<T>& problem_data,
                                               const VectorX<T>& cf,
                                               VectorX<T>* generalized_acceleration) {
        ComputeGeneralizedAccelerationFromConstraintForces(problem_data, cf, generalized_acceleration);
    ...
    /// @pre `dt` is positive.
    static void ComputeGeneralizedAcceleration(const ConstraintVelProblemData<T>& problem_data,
                                               const VectorX<T>& v,
                                               const VectorX<T>& cf,
                                               double dt,
                                               VectorX<T>* generalized_acceleration);
    ...
    /// @throws std::exception if @p generalized_acceleration is null or
    ///         @p cf vector is incorrectly sized.
    static void ComputeGeneralizedAccelerationFromConstraintForces(const ConstraintAccelProblemData<T>& problem_data,
                                                                   const VectorX<T>& cf,
                                                                   VectorX<T>* generalized_acceleration);
    ...
    /// @throws std::exception if @p generalized_acceleration is null or
    ///         @p cf vector is incorrectly sized.
    static void ComputeGeneralizedAccelerationFromConstraintForces(const ConstraintVelProblemData<T>& problem_data,
                                                                   const VectorX<T>& cf,
                                                                   VectorX<T>* generalized_acceleration);
    ...
    ///         `cf` vector is incorrectly sized.
    static void ComputeGeneralizedVelocityChange(const ConstraintVelProblemData<T>& problem_data,
                                                 const VectorX<T>& cf,
    ...
    static void CalcContactForcesInContactFrames(const VectorX<T>& cf,
...
private:
    static void PopulatePackedConstraintForcesFromLcpSolution(const ConstraintVelProblemData<T>& problem_data,
                                                              ...
                                                              const VectorX<T>& a,
                                                              VectorX<T>* cf);
    static void ConstructLinearEquationSolversForMlcp(const ConstraintVelProblemData<T>& problem_data,
                                                      MlcpToLcpData* mlcp_to_lcp_data);
    void FormAndSolveConstraintLcp(const ConstraintAccelProblemData<T>& problem_data,
                                   const VectorX<T>& trunc_neg_invA_a,
                                   VectorX<T>* cf) const;
    void FormAndSolveConstraintLinearSystem(const ConstraintAccelProblemData<T>& problem_data,
                                            const VectorX<T>& trunc_neg_invA_a,
                                            VectorX<T>* cf) const;
    static void CheckAccelConstraintMatrix(const ConstraintAccelProblemData<T>& problem_data, const MatrixX<T>& MM);
    static void CheckVelConstraintMatrix(const ConstraintVelProblemData<T>& problem_data, const MatrixX<T>& MM);
    ...
    // not of size a × b.
    static void ComputeConstraintSpaceComplianceMatrix(std::function<VectorX<T>(const VectorX<T>&)> A_mult,
                                                       int a,
    ...
    static void ComputeInverseInertiaTimesGT(std::function<MatrixX<T>(const MatrixX<T>&)> M_inv_mult,
                                             std::function<VectorX<T>(const VectorX<T>&)> G_transpose_mult,
                                             int m,
                                             MatrixX<T>* iM_GT);

    static void FormImpactingConstraintLcp(const ConstraintVelProblemData<T>& problem_data,
                                           const VectorX<T>& invA_a,
                                           MatrixX<T>* MM,
                                           VectorX<T>* qq);
    static void FormSustainedConstraintLcp(const ConstraintAccelProblemData<T>& problem_data,
                                           const VectorX<T>& invA_a,
                                           MatrixX<T>* MM,
                                           VectorX<T>* qq);
    static void FormSustainedConstraintLinearSystem(const ConstraintAccelProblemData<T>& problem_data,
                                                    const VectorX<T>& invA_a,
                                                    MatrixX<T>* MM,
                                                    VectorX<T>* qq);
...
void ConstraintSolver<T>::DetermineNewPartialInertiaSolveOperator(
    ...
    *A_solve = [problem_data, delassus_QTZ, num_eq_constraints,
        ...
        for (int i = 0; i < X.cols(); ++i) G_iM_X.col(i) = problem_data->G_mult(iM_X.col(i));
        ...
        const MatrixX<T> iM_GT_Del_G_iM_X = problem_data->solve_inertia(GT_Del_G_iM_X);
...
void ConstraintSolver<T>::DetermineNewFullInertiaSolveOperator(
    ...
    *A_solve = [problem_data, delassus_QTZ, num_eq_constraints,
        ...
        for (int i = 0; i < Y.cols(); ++i) G_iM_Y.col(i) = problem_data->G_mult(iM_Y.col(i));
        ...
        const MatrixX<T> iM_GT_Del_G_iM_Y = problem_data->solve_inertia(GT_Del_G_iM_Y);
...
template <class T>
void ConstraintSolver<T>::FormAndSolveConstraintLinearSystem(const ConstraintAccelProblemData<T>& problem_data,
                                                             const VectorX<T>& trunc_neg_invA_a,
                                                             VectorX<T>* cf) const {
    using std::abs;
    using std::max;
    ...
    const std::vector<int>& non_sliding_contacts = problem_data.non_sliding_contacts;
    ...
    const int num_spanning_vectors = std::accumulate(problem_data.r.begin(), problem_data.r.end(), 0);
    const int num_limits = problem_data.kL.size();
    const int num_eq_constraints = problem_data.kG.size();

    // Initialize contact force vector.
    cf->resize(num_contacts + num_spanning_vectors + num_limits + num_eq_constraints);
...
template <typename T>
void ConstraintSolver<T>::FormAndSolveConstraintLcp(const ConstraintAccelProblemData<T>& problem_data,
                                                    const VectorX<T>& trunc_neg_invA_a,
                                                    VectorX<T>* cf) const {
    using std::abs;
    using std::max;
    ...
    const double max_dot = (zz.size() > 0) ? (zz.array() * ww.array()).abs().maxCoeff() : 0.0;
    ...
    if (!success ||
        (zz.size() > 0 &&
         (zz.minCoeff() < -num_vars * npivots * zero_tol || ww.minCoeff() < -num_vars * npivots * zero_tol ||
          max_dot > max(T(1), zz.maxCoeff()) * max(T(1), ww.maxCoeff()) * num_vars * npivots * zero_tol))) {
    ...
    const auto fD_minus = zz.segment(num_contacts + num_spanning_vectors, num_spanning_vectors);
...
template <typename T>
void ConstraintSolver<T>::SolveConstraintProblem(const ConstraintAccelProblemData<T>& problem_data,
                                                 VectorX<T>* cf) const {
    using std::abs;
    using std::max;
    ...
    const VectorX<T> candidate_accel = problem_data.solve_inertia(problem_data.tau);
    const VectorX<T> N_eval = problem_data.N_mult(candidate_accel) + problem_data.kN;
    const VectorX<T> L_eval = problem_data.L_mult(candidate_accel) + problem_data.kL;
    if ((num_contacts == 0 || N_eval.minCoeff() >= 0) && (num_limits == 0 || L_eval.minCoeff() >= 0) &&
    ...
    if (num_eq_constraints > 0) {
        ...
        ComputeInverseInertiaTimesGT(problem_data.solve_inertia, problem_data.G_transpose_mult, num_eq_constraints,
                                     &iM_GT);
        ComputeConstraintSpaceComplianceMatrix(problem_data.G_mult, num_eq_constraints, iM_GT, Del);
        ...
        DetermineNewFullInertiaSolveOperator(&problem_data, num_generalized_velocities, &mlcp_to_lcp_data.delassus_QTZ,
        ...
        DetermineNewPartialInertiaSolveOperator(&problem_data, num_generalized_velocities,
    ...
    ConstraintAccelProblemData<T> modified_problem_data(gv_dim + num_eq_constraints);
    ConstraintAccelProblemData<T>* data_ptr = &modified_problem_data;
    data_ptr = UpdateProblemDataForUnilateralConstraints(problem_data, mlcp_to_lcp_data.fast_A_solve, gv_dim, data_ptr);
    ...
    if (num_eq_constraints > 0) {
        ...
        //                                                        | kG |
        const VectorX<T> Xv = -data_ptr->N_minus_muQ_transpose_mult(fN) - data_ptr->F_transpose_mult(fF) -
                              data_ptr->L_transpose_mult(fL);
        VectorX<T> aug = a;
        aug.head(Xv.size()) += Xv;
        const VectorX<T> u = -mlcp_to_lcp_data.A_solve(aug);
        auto lambda = cf->segment(num_contacts + num_spanning_vectors + num_limits, num_eq_constraints);
        lambda = u.tail(num_eq_constraints);
    }
}

template <typename T>
void ConstraintSolver<T>::SolveImpactProblem(const ConstraintVelProblemData<T>& problem_data, VectorX<T>* cf) const {
    using std::abs;
    using std::max;
    ...
    ConstraintVelProblemData<T> modified_problem_data(gv_dim + num_eq_constraints);
    ...
    bool success = lcp_.SolveLcpLemke(MM, qq, &zz, -1, zero_tol);
    VectorX<T> ww = MM * zz + qq;
    const T max_dot = (zz.size() > 0) ? (zz.array() * ww.array()).abs().maxCoeff() : 0.0;
    ...
    if (!success ||
        ...
        DRAKE_LOGGER_DEBUG("zero tolerance for z/w: {}", num_vars * npivots * zero_tol);
        ...
        DRAKE_LOGGER_DEBUG("zero tolerance for <z,w>: {}",
                           max(T(1), zz.maxCoeff()) * max(T(1), ww.maxCoeff()) * num_vars * npivots * zero_tol);
        ...
        if (!lcp_.SolveLcpLemkeRegularized(MM, qq, &zz, min_exp, step_exp, max_exp, piv_tol, zero_tol)) {
    ...
    PopulatePackedConstraintForcesFromLcpSolution(problem_data, mlcp_to_lcp_data, zz, a, cf);
}

template <typename T>
void ConstraintSolver<T>::ConstructLinearEquationSolversForMlcp(const ConstraintVelProblemData<T>& problem_data,
                                                                MlcpToLcpData* mlcp_to_lcp_data) {
    // --------------------------------------------------------------------------
    // Using the LCP solution to solve the MLCP.
    // --------------------------------------------------------------------------
...
// @pre cf is non-null.
template <typename T>
void ConstraintSolver<T>::PopulatePackedConstraintForcesFromLcpSolution(const ConstraintVelProblemData<T>& problem_data,
                                                                        const MlcpToLcpData& mlcp_to_lcp_data,
                                                                        const VectorX<T>& zz,
                                                                        const VectorX<T>& a,
                                                                        VectorX<T>* cf) {
    PopulatePackedConstraintForcesFromLcpSolution(problem_data, mlcp_to_lcp_data, zz, a, 1.0, cf);
}
...
void ConstraintSolver<T>::PopulatePackedConstraintForcesFromLcpSolution(const ConstraintVelProblemData<T>& problem_data,
    ...
    if (zz.size() == 0) {
        ...
        if (num_eq_constraints > 0) {
            ...
            DRAKE_LOGGER_DEBUG("Bilateral constraint forces/impulses: {}", fmt_eigen(lambda.transpose()));
        }
    ...
    const auto fD_plus = zz.segment(num_contacts, num_spanning_vectors);
    const auto fD_minus = zz.segment(num_contacts + num_spanning_vectors, num_spanning_vectors);
    const auto fL = zz.segment(num_contacts * 2 + num_spanning_vectors * 2, num_limits);
    const auto fF = cf->segment(num_contacts, num_spanning_vectors);
    ...
    cf->segment(num_contacts, num_spanning_vectors) = fD_plus - fD_minus;
    cf->segment(num_contacts + num_spanning_vectors, num_limits) = fL;
    DRAKE_LOGGER_DEBUG("Normal contact forces/impulses: {}", fmt_eigen(fN.transpose()));
    DRAKE_LOGGER_DEBUG("Frictional contact forces/impulses: {}", fmt_eigen((fD_plus - fD_minus).transpose()));
    DRAKE_LOGGER_DEBUG(
    ...
    if (num_eq_constraints > 0) {
        ...
        const VectorX<T> Xv = (-problem_data.N_transpose_mult(fN) - problem_data.F_transpose_mult(fF) -
...
template <typename T>
void ConstraintSolver<T>::UpdateDiscretizedTimeLcp(const ConstraintVelProblemData<T>& problem_data,
                                                   double dt,
                                                   MlcpToLcpData* mlcp_to_lcp_data,
                                                   VectorX<T>* a,
                                                   MatrixX<T>* MM,
    ...
    MM->block(nc + nr, nc + nr, nr, nr) += Eigen::DiagonalMatrix<T, Eigen::Dynamic>(gammaF);
    MM->block(nc + nk, nc + nk, nc, nc) += Eigen::DiagonalMatrix<T, Eigen::Dynamic>(gammaE);
    MM->block(nc * 2 + nk, nc * 2 + nk, nl, nl) += Eigen::DiagonalMatrix<T, Eigen::Dynamic>(gammaL);
...
template <typename T>
void ConstraintSolver<T>::ConstructBaseDiscretizedTimeLcp(const ConstraintVelProblemData<T>& problem_data,
                                                          MlcpToLcpData* mlcp_to_lcp_data,
                                                          MatrixX<T>* MM,
                                                          VectorX<T>* qq) {
    DRAKE_DEMAND(MM != nullptr);
...
template <class T>
void ConstraintSolver<T>::ComputeConstraintSpaceComplianceMatrix(std::function<VectorX<T>(const VectorX<T>&)> A_mult,
                                                                 int a,
...
template <class T>
void ConstraintSolver<T>::ComputeInverseInertiaTimesGT(std::function<MatrixX<T>(const MatrixX<T>&)> M_inv_mult,
                                                       std::function<VectorX<T>(const VectorX<T>&)> G_transpose_mult,
                                                       int m,
                                                       MatrixX<T>* iM_GT) {
...
template <class T>
void ConstraintSolver<T>::CheckAccelConstraintMatrix(const ConstraintAccelProblemData<T>& problem_data,
                                                     const MatrixX<T>& MM) {
    ...
    Eigen::Ref<const MatrixX<T>> L_iM_FT = MM.block(num_contacts + nk + num_non_sliding, num_contacts, nl, nr);
    ...
    const double zero_tol = std::numeric_limits<double>::epsilon() * MM.norm() * MM.rows();
...
// constraint forces.
template <class T>
void ConstraintSolver<T>::FormSustainedConstraintLinearSystem(const ConstraintAccelProblemData<T>& problem_data,
                                                              const VectorX<T>& trunc_neg_invA_a,
                                                              MatrixX<T>* MM,
                                                              VectorX<T>* qq) {
    DRAKE_DEMAND(MM != nullptr);
    ...
    ComputeInverseInertiaTimesGT(iM, problem_data.N_minus_muQ_transpose_mult, nc, &iM_NT_minus_muQT);
    ComputeInverseInertiaTimesGT(iM, FT, nr, &iM_FT);
    ...
    // Compute the blocks.
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_NT_minus_muQT, N_iM_NT_minus_muQT);
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_FT, N_iM_FT);
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_LT, N_iM_LT);
    ComputeConstraintSpaceComplianceMatrix(F, nr, iM_NT_minus_muQT, F_iM_NT_minus_muQT);
    ComputeConstraintSpaceComplianceMatrix(F, nr, iM_FT, F_iM_FT);
    ComputeConstraintSpaceComplianceMatrix(F, nr, iM_LT, F_iM_LT);
    ComputeConstraintSpaceComplianceMatrix(L, nl, iM_NT_minus_muQT, L_iM_NT_minus_muQT);
...
template <class T>
void ConstraintSolver<T>::FormSustainedConstraintLcp(const ConstraintAccelProblemData<T>& problem_data,
                                                     const VectorX<T>& trunc_neg_invA_a,
                                                     MatrixX<T>* MM,
                                                     VectorX<T>* qq) {
    DRAKE_DEMAND(MM != nullptr);
    ...
    Eigen::Ref<MatrixX<T>> N_iM_NT_minus_muQT = MM->block(0, 0, nc, nc);
    Eigen::Ref<MatrixX<T>> N_iM_FT = MM->block(0, nc, nc, nr);
    Eigen::Ref<MatrixX<T>> N_iM_LT = MM->block(0, nc + nk + num_non_sliding, nc, nl);
    Eigen::Ref<MatrixX<T>> F_iM_NT_minus_muQT = MM->block(nc, 0, nr, nc);
    Eigen::Ref<MatrixX<T>> F_iM_FT = MM->block(nc, nc, nr, nr);
    Eigen::Ref<MatrixX<T>> F_iM_LT = MM->block(nc, nc + nk + num_non_sliding, nr, nl);
    Eigen::Ref<MatrixX<T>> L_iM_NT_minus_muQT = MM->block(nc + nk + num_non_sliding, 0, nl, nc);
    Eigen::Ref<MatrixX<T>> L_iM_LT = MM->block(nc + nk + num_non_sliding, nc + nk + num_non_sliding, nl, nl);
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_NT_minus_muQT, N_iM_NT_minus_muQT);
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_FT, N_iM_FT);
    ComputeConstraintSpaceComplianceMatrix(N, nc, iM_LT, N_iM_LT);
    ComputeConstraintSpaceComplianceMatrix(F, nr, iM_NT_minus_muQT, F_iM_NT_minus_muQT);
    ...
    MM->block(nc, nc + nr, num_spanning_vectors, nr) = -MM->block(nc, nc, nr, num_spanning_vectors);
    ...
    MM->block(nc + nk, nc + num_spanning_vectors, num_non_sliding, num_spanning_vectors) = -E.transpose();
    ...
    MM->block(nc + nk + num_non_sliding, 0, nl, nc + nk + num_non_sliding) =
            MM->block(0, nc + nk + num_non_sliding, nc + nk + num_non_sliding, nl).transpose().eval();
...
template <class T>
void ConstraintSolver<T>::CheckVelConstraintMatrix(const ConstraintVelProblemData<T>& problem_data,
                                                   const MatrixX<T>& MM) {
    // Get numbers of contacts.
    const int num_contacts = problem_data.mu.size();
    const int num_spanning_vectors = std::accumulate(problem_data.r.begin(), problem_data.r.end(), 0);
    ...
    Eigen::Ref<const MatrixX<T>> F_iM_NT = MM.block(num_contacts, 0, nr, num_contacts);
    Eigen::Ref<const MatrixX<T>> L_iM_NT = MM.block(num_contacts * 2 + nk, 0, nl, num_contacts);
    Eigen::Ref<const MatrixX<T>> L_iM_FT = MM.block(num_contacts * 2 + nk, num_contacts, nl, nr);
    ...
    DRAKE_ASSERT(F_iM_NT.rows() == 0 || F_iM_NT.cols() == 0 || (F_iM_NT - F_iM_NT_true).norm() < zero_tol);
    DRAKE_ASSERT(L_iM_NT.rows() == 0 || L_iM_NT.cols() == 0 || (L_iM_NT - L_iM_NT_true).norm() < zero_tol);
    DRAKE_ASSERT(L_iM_FT.rows() == 0 || L_iM_FT.cols() == 0 || (L_iM_FT - L_iM_FT_true).norm() < zero_tol);
}

// Forms the LCP matrix and vector, which is used to determine the collisional
// impulses.
template <class T>
void ConstraintSolver<T>::FormImpactingConstraintLcp(const ConstraintVelProblemData<T>& problem_data,
                                                     const VectorX<T>& trunc_neg_invA_a,
                                                     MatrixX<T>* MM,
                                                     VectorX<T>* qq) {
    DRAKE_DEMAND(MM != nullptr);
    ...
    MM->block(nc + nk, nc + num_spanning_vectors, nc, num_spanning_vectors) = -E.transpose();
    MM->block(nc + nk, nc + nk, nc, nc + nl).setZero();

    // Construct the last row block, which provides the generic unilateral
    // constraints.
    MM->block(nc * 2 + nk, 0, nl, nc * 2 + nk) = MM->block(0, nc * 2 + nk, nc * 2 + nk, nl).transpose().eval();
...
template <class T>
void ConstraintSolver<T>::ComputeGeneralizedForceFromConstraintForces(const ConstraintAccelProblemData<T>& problem_data,
                                                                      const VectorX<T>& cf,
                                                                      VectorX<T>* generalized_force) {
    if (!generalized_force) throw std::logic_error("generalized_force vector is null.");
    ...
    const int num_vars = num_contacts + num_spanning_vectors + num_limits + num_bilat_constraints;
    if (cf.size() != num_vars) {
        throw std::logic_error(
    ...
    const Eigen::Ref<const VectorX<T>> f_non_sliding_frictional = cf.segment(num_contacts, num_spanning_vectors);

    /// Get the limit forces.
    const Eigen::Ref<const VectorX<T>> f_limit = cf.segment(num_contacts + num_spanning_vectors, num_limits);

    // Get the bilateral constraint forces.
    const Eigen::Ref<const VectorX<T>> f_bilat =
            cf.segment(num_contacts + num_spanning_vectors + num_limits, num_bilat_constraints);
    ...
    *generalized_force = problem_data.N_minus_muQ_transpose_mult(f_normal) +
                         ...
                         problem_data.L_transpose_mult(f_limit) + problem_data.G_transpose_mult(f_bilat);
}

template <class T>
void ConstraintSolver<T>::ComputeGeneralizedForceFromConstraintForces(const ConstraintVelProblemData<T>& problem_data,
                                                                      const VectorX<T>& cf,
                                                                      VectorX<T>* generalized_force) {
    if (!generalized_force) throw std::logic_error("generalized_force vector is null.");
    ...
    const Eigen::Ref<const VectorX<T>> f_normal = cf.segment(0, num_contacts);
    const Eigen::Ref<const VectorX<T>> f_frictional = cf.segment(num_contacts, num_spanning_vectors);
    ...
    *generalized_force = problem_data.N_transpose_mult(f_normal) + problem_data.F_transpose_mult(f_frictional) +
                         problem_data.L_transpose_mult(f_limit) + problem_data.G_transpose_mult(f_bilat);
}

template <class T>
void ConstraintSolver<T>::ComputeGeneralizedAcceleration(const ConstraintVelProblemData<T>& problem_data,
                                                         const VectorX<T>& v,
                                                         const VectorX<T>& cf,
                                                         double dt,
                                                         VectorX<T>* generalized_acceleration) {
    ...
    ComputeGeneralizedForceFromConstraintForces(problem_data, cf, generalized_acceleration);
    ...
    const VectorX<T> vplus = problem_data.solve_inertia(problem_data.Mv + dt * (*generalized_acceleration));
    *generalized_acceleration = (vplus - v) / dt;
}

template <class T>
void ConstraintSolver<T>::ComputeGeneralizedAccelerationFromConstraintForces(
        const ConstraintAccelProblemData<T>& problem_data, const VectorX<T>& cf, VectorX<T>* generalized_acceleration) {
    if (!generalized_acceleration) throw std::logic_error("generalized_acceleration vector is null.");

    VectorX<T> generalized_force;
    ComputeGeneralizedForceFromConstraintForces(problem_data, cf, &generalized_force);
    *generalized_acceleration = problem_data.solve_inertia(generalized_force);
}

template <class T>
void ConstraintSolver<T>::ComputeGeneralizedAccelerationFromConstraintForces(
        const ConstraintVelProblemData<T>& problem_data, const VectorX<T>& cf, VectorX<T>* generalized_acceleration) {
    if (!generalized_acceleration) throw std::logic_error("generalized_acceleration vector is null.");
...
template <class T>
void ConstraintSolver<T>::ComputeGeneralizedVelocityChange(const ConstraintVelProblemData<T>& problem_data,
                                                           const VectorX<T>& cf,
                                                           VectorX<T>* generalized_delta_v) {
    if (!generalized_delta_v) throw std::logic_error("generalized_delta_v vector is null.");

    VectorX<T> generalized_impulse;
    ComputeGeneralizedForceFromConstraintForces(problem_data, cf, &generalized_impulse);
    *generalized_delta_v = problem_data.solve_inertia(generalized_impulse);
}

template <class T>
void ConstraintSolver<T>::CalcContactForcesInContactFrames(const VectorX<T>& cf,
                                                           const ConstraintAccelProblemData<T>& problem_data,
                                                           const std::vector<Matrix2<T>>& contact_frames,
                                                           std::vector<Vector2<T>>* contact_forces) {
    using std::abs;
    ...
    if (!contact_forces) throw std::logic_error("Vector of contact forces is null.");
    if (!contact_forces->empty()) throw std::logic_error("Vector of contact forces is not empty.");

    // Verify that cf is the correct size.
    const int num_non_sliding_contacts = problem_data.non_sliding_contacts.size();
    const int num_contacts = problem_data.sliding_contacts.size() + num_non_sliding_contacts;
    const int num_spanning_vectors = std::accumulate(problem_data.r.begin(), problem_data.r.end(), 0);
    const int num_limits = problem_data.kL.size();
    const int num_bilat_constraints = problem_data.kG.size();
    const int num_vars = num_contacts + num_spanning_vectors + num_limits + num_bilat_constraints;
    ...
    DRAKE_ASSERT(std::is_sorted(problem_data.sliding_contacts.begin(), problem_data.sliding_contacts.end()));

    // Resize the force vector.
    contact_forces->resize(contact_frames.size());

    // Set the forces.
    for (int i = 0, sliding_index = 0, non_sliding_index = 0; i < num_contacts; ++i) {
        // Alias the force.
        ...
        if (abs(contact_normal.dot(contact_tangent)) > loose_eps) {
            throw std::logic_error(
                    fmt::format("Contact normal ({}) and contact tangent ({}) insufficiently "
                                "orthogonal.",
                                fmt_eigen(contact_normal.transpose()), fmt_eigen(contact_tangent.transpose())));
        }
        ...
        const bool is_sliding =
                std::binary_search(problem_data.sliding_contacts.begin(), problem_data.sliding_contacts.end(), i);
...
void ConstraintSolver<T>::CalcContactForcesInContactFrames(const VectorX<T>& cf,
    ...
    for (int i = 0, tangent_index = 0; i < num_contacts; ++i) {
        ...
        Vector2<T> j0 = contact_normal * cf[i] + contact_tangent * cf[num_contacts + tangent_index++];
...
template <typename T>
using ConstraintSolver DRAKE_DEPRECATED("2024-09-01",
                                        "This class is being removed from Drake.") = internal::ConstraintSolver<T>;
