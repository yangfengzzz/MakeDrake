...
public:
    ...
    const SpatialAcceleration<T>& spatial_acceleration() const { return A_NBo_N_; }

    /// Returns the 3x3 rotation matrix relating Nx, Ny, Nz to Bx, By, Bz.
    /// @retval R_NB, the 3x3 rotation matrix relating frames N and B.
    const math::RotationMatrix<T>& rotation_matrix() const { return X_NB_.rotation(); }
    ...
    const Vector3<T>& translational_velocity() const { return V_NBo_N_.translational(); }

    /// Returns B's angular acceleration in N, expressed in N.
    /// @retval alpha_NB_N, B's angular acceleration in N, expressed in N.
    const Vector3<T>& angular_acceleration() const { return A_NBo_N_.rotational(); }

    /// Returns Bo's acceleration in N, expressed in N.
    /// @retval a_NBo_N, Bo's acceleration in N, expressed in N.
    const Vector3<T>& translational_acceleration() const { return A_NBo_N_.translational(); }
