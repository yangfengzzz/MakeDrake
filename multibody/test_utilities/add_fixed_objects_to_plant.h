...
void AddFixedObjectsToPlant(MultibodyPlant<T>* plant) {
    ...
    const std::string mug_sdf_path = FindResourceOrThrow("drake/examples/simple_gripper/simple_mug.sdf");

    // Load a model of a table for a robot.
    const ModelInstanceIndex robot_table_model = Parser(plant, "robot").AddModels(table_sdf_path).at(0);
    ...
    const ModelInstanceIndex objects_table_model = objects_parser.AddModels(table_sdf_path).at(0);
    ...
    plant->WeldFrames(objects_frame_O, mug.body_frame(), Translation3d(0.0, 0.0, 0.05));
}
