...
template <typename T>
RobotModel<T>::RobotModel(const RobotModelConfig& config, bool add_visualization)
...
{  // NOLINT(whitespace/braces)
    ...
    robot_model_instance_ =
            parser.AddModelsFromUrl("package://drake_models/iiwa_description/sdf/iiwa7_no_collision.sdf")[0];
    parser.AddModelsFromUrl("package://drake_models/dishes/plate_8in.sdf");

    // Weld the robot's base to the world.
    plant_->WeldFrames(plant_->world_frame(), plant_->GetBodyByName("iiwa_link_0").body_frame(),
                       math::RigidTransformd::Identity());

    // Ground geometry.
    geometry::ProximityProperties proximity_properties;
    geometry::AddContactMaterial(kHcDissipation, kStiffness, CoulombFriction<double>(kMu, kMu), &proximity_properties);
    proximity_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kRelaxationTime,
                                     kRelaxationTime / 2);
    plant_->RegisterCollisionGeometry(plant_->world_body(), math::RigidTransformd(Eigen::Vector3d(0.0, 0.0, -0.05)),
                                      geometry::Box(2.0, 2.0, 0.1), "ground_collision", proximity_properties);

    // Add simple contact geometry at the end effector.
    plant_->RegisterCollisionGeometry(plant_->GetBodyByName("iiwa_link_7"),
                                      math::RigidTransformd(Eigen::Vector3d(0.0, 0.0, 0.07)), geometry::Sphere(0.05),
    ...
    if (config.contact_configuration == RobotModelConfig::ContactConfig::kNoGeometry) {
        const auto& inspector = items.scene_graph.model_inspector();
        for (const auto id : inspector.GetAllGeometryIds(geometry::Role::kProximity)) {
            items.scene_graph.RemoveRole(*plant_->get_source_id(), id, geometry::Role::kProximity);
        }
    }
    ...
    if (plant_->get_discrete_contact_solver() == DiscreteContactSolver::kSap) {
        auto owned_contact_manager = std::make_unique<multibody::internal::CompliantContactManager<double>>();
        manager_ = owned_contact_manager.get();
        plant_->SetDiscreteUpdateManager(std::move(owned_contact_manager));
        driver_ = &multibody::internal::CompliantContactManagerTester::sap_driver(*manager_);
    }
    ...
    // Fix input ports.
    const VectorX<double> tau = VectorX<double>::Zero(plant_->num_actuated_dofs());
...
std::unique_ptr<RobotModel<U>> RobotModel<T>::ToScalarType() const {
    ...
    converted_model->diagram_ = dynamic_pointer_cast<systems::Diagram<U>>(diagram_->template ToScalarType<U>());
    converted_model->plant_ = const_cast<MultibodyPlant<U>*>(
            &converted_model->diagram_->template GetDowncastSubsystemByName<MultibodyPlant<U>>(plant_->get_name()));

    // Make and add a manager so that we have access to it and its driver.
    if constexpr (!std::is_same_v<U, symbolic::Expression>) {
        if (plant_->get_discrete_contact_solver() == DiscreteContactSolver::kSap) {
            auto owned_contact_manager_ad = std::make_unique<multibody::internal::CompliantContactManager<U>>();
            converted_model->manager_ = owned_contact_manager_ad.get();
            converted_model->plant_->SetDiscreteUpdateManager(std::move(owned_contact_manager_ad));
            converted_model->driver_ =
                    &multibody::internal::CompliantContactManagerTester::sap_driver(*converted_model->manager_);
        }
    }
    ...
    converted_model->plant_context_ = &converted_model->diagram_->GetMutableSubsystemContext(
            *converted_model->plant_, converted_model->context_.get());

    // Fix input ports.
    const VectorX<U> tau_ad = VectorX<U>::Zero(plant_->num_actuated_dofs());
    converted_model->plant_->get_actuation_input_port().FixValue(converted_model->plant_context_, tau_ad);
...
template <typename T>
const multibody::contact_solvers::internal::SapContactProblem<T>& RobotModel<T>::EvalContactProblem(
        const VectorX<T>& x0)
    requires(!std::is_same_v<T, symbolic::Expression>)
{  // NOLINT(whitespace/braces)
    DRAKE_DEMAND(plant_->get_discrete_contact_solver() == DiscreteContactSolver::kSap);
    SetState(x0);
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&RobotModel<T>::template ToScalarType<U>));

}  // namespace test
}  // namespace multibody
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::test::RobotModel);
