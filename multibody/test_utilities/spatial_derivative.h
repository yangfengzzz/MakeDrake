...
#include "multibody/math/spatial_acceleration.h"
#include "multibody/math/spatial_velocity.h"
...
SpatialAcceleration<double> CalcSpatialAccelerationViaAutomaticDifferentiation(const MultibodyPlant<double>& plant,
    ...
    std::unique_ptr<systems::Context<AutoDiffXd>> context_autodiff = plant_autodiff->CreateDefaultContext();
    ...
    const Frame<AutoDiffXd>& frame_A_autodiff = plant_autodiff->get_frame(frame_A.index());
    const Frame<AutoDiffXd>& frame_B_autodiff = plant_autodiff->get_frame(frame_B.index());
    const SpatialVelocity<AutoDiffXd> V_AB_A_autodiff =
            frame_B_autodiff.CalcSpatialVelocity(*context_autodiff, frame_A_autodiff, frame_A_autodiff);
    ...
    const Vector3<AutoDiffXd> p_BoBq_A_autodiff = R_AB_autodiff * p_BoBq_B_autodiff;
    const SpatialVelocity<AutoDiffXd> V_ABq_A_autodiff = V_AB_A_autodiff.Shift(p_BoBq_A_autodiff);

    // Form spatial acceleration via AutoDiffXd results.
    const SpatialAcceleration<double> A_ABq_A(math::ExtractGradient(V_ABq_A_autodiff.get_coeffs()));

    // Shortcut return if frame_A == frame_E.
    if (frame_E.index() == frame_A.index()) return A_ABq_A;

    // Otherwise, express the result in frame_E.
    const math::RotationMatrix<double> R_EA = frame_A.CalcRotationMatrix(context, frame_E);
...
SpatialAcceleration<double> CalcSpatialAccelerationViaAutomaticDifferentiation(const MultibodyPlant<double>& plant,
    ...
    const systems::ContinuousState<double>& derivs = plant.EvalTimeDerivatives(context);
    ...
    return CalcSpatialAccelerationViaAutomaticDifferentiation(plant, context, vdot, frame_B, p_BoBo_B, frame_A,
                                                              frame_E);
}
