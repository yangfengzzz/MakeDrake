...
public:
    void TestForUnityResultFromStartingOrder(const std::function<double(const Vector2d&)>& f, int starting_order) {
        ...
        for (int order = starting_order; order <= 5; ++order) {
            ...
            double result = TriangleQuadrature<double, double>::Integrate(f, rule, 0.5 /* triangle area */);
            EXPECT_NEAR(result, 1.0, 5 * num_weights * std::numeric_limits<double>::epsilon()) << order;
        }
    }
};
