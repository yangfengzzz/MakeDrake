...
#include "multibody/triangle_quadrature/triangle_quadrature_rule.h"
...
public:
    ...
    static NumericReturnType Integrate(const std::function<NumericReturnType(const Vector2<T>&)>& f,
    ...
    ///        (p[0], p[1], p[2]).
    /// @param area the area of the triangle.
    static NumericReturnType Integrate(const std::function<NumericReturnType(const Vector3<T>&)>& f,
                                       const TriangleQuadratureRule& rule,
                                       const T& area);
};
...
NumericReturnType TriangleQuadrature<NumericReturnType, T>::Integrate(
    ...
    const std::vector<Eigen::Vector2d>& barycentric_coordinates = rule.quadrature_points();
    ...
    for (int i = 1; i < static_cast<int>(weights.size()); ++i) integral += f(barycentric_coordinates[i]) * weights[i];
...
NumericReturnType TriangleQuadrature<NumericReturnType, T>::Integrate(
    ...
    std::function<NumericReturnType(const Vector2<T>&)> fprime = [&f](const Vector2<T>& barycentric_2D) {
        return f(Vector3<T>(barycentric_2D[0], barycentric_2D[1], T(1.0) - barycentric_2D[0] - barycentric_2D[1]));
    };
