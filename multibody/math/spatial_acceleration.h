...
#include "multibody/math/spatial_vector.h"
...
public:
    ...
    void ShiftInPlace(const Vector3<T>& offset, const Vector3<T>& angular_velocity_of_this_frame) {
        ...
        a_MCo_E += (alpha_MB_E.cross(p_BoCo_E) + w_MB_E.cross(w_MB_E.cross(p_BoCo_E)));
    }
    ...
    SpatialAcceleration<T> ShiftWithZeroAngularVelocity(const Vector3<T>& offset) const {
        ...
        return SpatialAcceleration<T>(alpha_MB_E, a_MBo_E + alpha_MB_E.cross(p_BoCo_E));
    }
...
template <typename T>
inline SpatialAcceleration<T> operator+(const SpatialAcceleration<T>& A1_E, const SpatialAcceleration<T>& A2_E) {
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::SpatialAcceleration);
