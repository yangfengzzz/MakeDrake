...
public:
    ...
    decltype(T() < T()) IsNearlyEqualWithinAbsoluteTolerance(const SpatialQuantity& other,
        ...
        std::tie(w_max_difference, v_max_difference) = GetMaximumAbsoluteDifferences(other);
        return w_max_difference <= rotational_tolerance && v_max_difference <= translational_tolerance;
    }
    ...
    void SetNaN() { V_.setConstant(std::numeric_limits<typename Eigen::NumTraits<T>::Literal>::quiet_NaN()); }
    ...
    friend SpatialQuantity operator+(const SpatialQuantity& V1_E, const SpatialQuantity& V2_E) {
    ...
    /// to documentation for the appropriate spatial quantity subclass (e.g.,
    /// SpatialVelocity, SpatialAcceleration, SpatialForce, or SpatialMomentum).
    friend SpatialQuantity operator-(const SpatialQuantity& V1, const SpatialQuantity& V2) {
        return SpatialQuantity(V1) -= V2;
    }

    /// Multiplication of a spatial vector V from the left by a scalar `s`.
    /// @relates SpatialVector.
    friend SpatialQuantity operator*(const T& s, const SpatialQuantity& V) { return SpatialQuantity(V) *= s; }
    ...
    /// </pre>
    friend SpatialQuantity operator*(const math::RotationMatrix<T>& R_FE, const SpatialQuantity& V_E) {
...
namespace fmt {
template <template <typename> class SpatialQuantity, typename T>
struct formatter<drake::multibody::SpatialVector<SpatialQuantity, T>> : drake::ostream_formatter {};
}  // namespace fmt
