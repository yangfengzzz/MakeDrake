...
public:
    ...
    SpatialForce(const Eigen::Ref<const Vector3<T>>& tau, const Eigen::Ref<const Vector3<T>>& f) : Base(tau, f) {}
...
template <typename T>
inline SpatialForce<T> operator+(const SpatialForce<T>& F1_E, const SpatialForce<T>& F2_E) {
