...
public:
    ...
    typedef typename spatial_vector_traits<SpatialQuantityType>::ScalarType ScalarType;
...
typedef ::testing::Types<SpatialVelocity<double>,
                         SpatialForce<double>,
...
TYPED_TEST(SpatialQuantityTest, IsNearlyEqualWithinAbsoluteTolerance) {
    ...
    EXPECT_TRUE(A.IsNearlyEqualWithinAbsoluteTolerance(B, 4 * w_delta, 4 * v_delta));
    EXPECT_FALSE(A.IsNearlyEqualWithinAbsoluteTolerance(B, 0.25 * w_delta, 0.25 * v_delta));
}
...
TYPED_TEST(SpatialQuantityTest, ReExpressInAnotherFrame) {
    ...
    const drake::math::RotationMatrix<T> R_EF(AngleAxis<T>(M_PI / 6, Vector3<T>::UnitX()) *
...
TYPED_TEST(SpatialVelocityTest, ShiftOperation) {
    ...
    EXPECT_TRUE(V_XY_A.ComposeWithMovingFrameVelocity(p_YZ_A, SpatialVelocity<T>::Zero()).IsApprox(V_XYz_A));

    // Unit test with the composition of the spatial velocity of a moving frame Z
    // in frame Y.
    const SpatialVelocity<T> V_YZ_A(Vector3<T>(1.0, 2.0, 3.0), Vector3<T>(4.0, 5.0, 6.0));
    const SpatialVelocity<T> V_XZ_A = V_XY_A.ComposeWithMovingFrameVelocity(p_YZ_A, V_YZ_A);
...
TYPED_TEST(SpatialVelocityTest, ComposeWithMovingFrameVelocity) {
    ...
    SpatialVelocity<T> V_MCo_E = V_MBo_E.ComposeWithMovingFrameVelocity(p_BoCo_E, SpatialVelocity<T>::Zero());
    ...
    V_MCo_E = V_MBo_E.ComposeWithMovingFrameVelocity(p_BoCo_E, SpatialVelocity<T>::Zero());
    EXPECT_TRUE(CompareMatrices(V_MCo_E.rotational(), w_MB_E));
    EXPECT_TRUE(CompareMatrices(V_MCo_E.translational(), v_MBo_E + w_MB_E.cross(p_BoCo_E)));
    ...
    EXPECT_TRUE(CompareMatrices(V_MCo_E.translational(), V_MBc_E.translational() + v_BCo_E));
    ...
    const SpatialVelocity<T> V_MM_E = V_MBo_E.ComposeWithMovingFrameVelocity(p_BoCo_E, V_BMo_E);
    EXPECT_TRUE(CompareMatrices(V_MM_E.get_coeffs(), SpatialVelocity<T>::Zero().get_coeffs()));
...
typedef ::testing::Types<SpatialForce<double>,
                         SpatialMomentum<double>,
                         SpatialForce<AutoDiffXd>,
                         SpatialMomentum<AutoDiffXd>,
                         SpatialForce<Expression>,
...
TYPED_TEST(ElementsInF6Test, ShiftOperation) {
    ...
    const Matrix6X<T> Fmatrix_Ao_E = F_Ao_E.get_coeffs().rowwise().replicate(num_forces);
...
TYPED_TEST(SpatialAccelerationTest, CentrifugalAcceleration) {
    ...
    A_AQ_expected.translational() = -w_AP_E.norm() * w_AP_E.norm() * p_PoQo_E.norm() * p_PoQo_E.normalized();
    ...
    const SpatialAcceleration<T> A_AQ_moving = A_AP.ComposeWithMovingFrameAcceleration(
            p_PoQo_E, w_AP_E, /* Same arguments as in the Shift() operator */
            SpatialVelocity<T>::Zero() /* V_PQ */, SpatialAcceleration<T>::Zero() /* A_PQ */);
...
TYPED_TEST(SpatialAccelerationTest, CoriolisAcceleration) {
    ...
    const SpatialVelocity<T> V_PQ(1.5 * Vector3<T>::UnitZ() /* w_PQ */, 2.0 * Vector3<T>::UnitX() /* v_PQ */);
    ...
    const SpatialAcceleration<T> A_AQ = A_AP.ComposeWithMovingFrameAcceleration(p_PoQo, w_AP, V_PQ, A_PQ);
...
TYPED_TEST(SpatialAccelerationTest, NoCentrifugalAcceleration) {
    ...
    A_AQ_expected.translational() = alpha_AP_E.norm() * p_PoQo_E.norm() * Vector3<T>::UnitX();
...
typedef ::testing::Types<SpatialVelocity<Expression>,
                         SpatialForce<Expression>,
                         SpatialAcceleration<Expression>,
                         SpatialMomentum<Expression>>
