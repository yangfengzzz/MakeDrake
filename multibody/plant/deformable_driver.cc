#include "multibody/plant/deformable_driver.h"
...
#include "multibody/contact_solvers/contact_configuration.h"
#include "multibody/fem/dirichlet_boundary_condition.h"
#include "multibody/fem/fem_model.h"
#include "multibody/fem/velocity_newmark_scheme.h"
#include "multibody/plant/contact_properties.h"
#include "multibody/plant/discrete_update_manager.h"
#include "multibody/plant/force_density_field.h"
...
template <typename T>
DeformableDriver<T>::DeformableDriver(const DeformableModel<T>* deformable_model,
    ...
    integrator_ = std::make_unique<fem::internal::VelocityNewmarkScheme<T>>(manager_->plant().time_step(), 1.0, 0.5);
}
...
template <typename T>
void DeformableDriver<T>::DeclareCacheEntries(DiscreteUpdateManager<T>* manager) {
    ...
    for (DeformableBodyIndex i(0); i < deformable_model_->num_bodies(); ++i) {
        ...
        const auto& constraint_participation_cache_entry = manager->DeclareCacheEntry(
                fmt::format("constraint participation of body {}", i),
                systems::ValueProducer(empty_contact_participation,
                                       std::function<void(const Context<T>&, ContactParticipation*)>{
                                               [this, i](const Context<T>& context, ContactParticipation* result) {
                                                   this->CalcConstraintParticipation(context, i, result);
                                               }}),
                {deformable_contact_ticket});
        cache_indexes_.constraint_participations.emplace_back(constraint_participation_cache_entry.cache_index());
        constraint_participation_tickets.emplace(constraint_participation_cache_entry.ticket());
        ...
        const auto& dof_permutation_cache_entry = manager->DeclareCacheEntry(
                fmt::format("partial permutation for dofs of body {} based on "
                            ...
                            i),
                systems::ValueProducer(std::function<void(const Context<T>&, PartialPermutation*)>{
                        [this, i](const Context<T>& context, PartialPermutation* result) {
                            this->CalcDofPermutation(context, i, result);
                        }}),
                {constraint_participation_cache_entry.ticket()});
        cache_indexes_.dof_permutations.emplace_back(dof_permutation_cache_entry.cache_index());

        /* Permutation for participating vertices for each body. */
        const GeometryId g_id = deformable_model_->GetGeometryId(deformable_model_->GetBodyId(i));
        const auto& vertex_permutation_cache_entry = manager->DeclareCacheEntry(
                fmt::format("partial permutation for vertices of body {} based on "
                            ...
                            i),
                systems::ValueProducer(std::function<void(const Context<T>&, PartialPermutation*)>{
                        [this, g_id](const Context<T>& context, PartialPermutation* result) {
                            this->CalcVertexPermutation(context, g_id, result);
                        }}),
                {constraint_participation_cache_entry.ticket()});
        cache_indexes_.vertex_permutations.emplace(g_id, vertex_permutation_cache_entry.cache_index());
        ...
        const auto& fem_solver_cache_entry = manager->DeclareCacheEntry(
                ...
                systems::ValueProducer(model_fem_solver,
                                       std::function<void(const systems::Context<T>&, FemSolver<T>*)>{
                                               [this, i](const systems::Context<T>& context, FemSolver<T>* fem_solver) {
                ...
                {fem_state_cache_entry.ticket(), vertex_permutation_cache_entry.ticket(),
                 systems::System<T>::all_input_ports_ticket()});
        cache_indexes_.fem_solvers.emplace_back(fem_solver_cache_entry.cache_index());
        ...
        const auto& next_fem_state_cache_entry = manager->DeclareCacheEntry(
                ...
                {systems::System<T>::xd_ticket(), systems::System<T>::all_parameters_ticket(),
                 systems::System<T>::all_input_ports_ticket(), systems::System<T>::time_ticket(),
                 systems::System<T>::accuracy_ticket()});
        cache_indexes_.next_fem_states.emplace_back(next_fem_state_cache_entry.cache_index());
    }

    const auto& participating_velocity_mux_cache_entry = manager->DeclareCacheEntry(
            "multiplexer for participating velocities",
            systems::ValueProducer(this, &DeformableDriver<T>::CalcParticipatingVelocityMultiplexer),
            constraint_participation_tickets);
    cache_indexes_.participating_velocity_mux = participating_velocity_mux_cache_entry.cache_index();

    auto constraint_participation_and_xd_tickets = constraint_participation_tickets;
    constraint_participation_and_xd_tickets.insert(systems::System<T>::xd_ticket());
    const auto& participating_velocities_cache_entry =
            manager->DeclareCacheEntry("participating velocities for all bodies",
                                       systems::ValueProducer(this, &DeformableDriver<T>::CalcParticipatingVelocities),
                                       constraint_participation_and_xd_tickets);
    cache_indexes_.participating_velocities = participating_velocities_cache_entry.cache_index();

    /* Input ports are needed for the calculation of the free motion velocities
     (via user defined external forces). */
    auto participarting_free_motion_velocities_tickets = constraint_participation_and_xd_tickets;
    participarting_free_motion_velocities_tickets.insert(systems::System<T>::all_input_ports_ticket());
    const auto& participating_free_motion_velocities_cache_entry = manager->DeclareCacheEntry(
            fmt::format("participating free motion velocities for all bodies"),
            systems::ValueProducer(this, &DeformableDriver<T>::CalcParticipatingFreeMotionVelocities),
            constraint_participation_and_xd_tickets);
...
template <typename T>
void DeformableDriver<T>::AppendLinearDynamicsMatrix(const systems::Context<T>& context,
    ...
    for (DeformableBodyIndex index(0); index < num_bodies; ++index) {
        const SchurComplement& schur_complement = EvalFreeMotionTangentMatrixSchurComplement(context, index);
        ...
        A->push_back(schur_complement.get_D_complement() * manager_->plant().time_step());
    }
}

template <typename T>
typename DeformableDriver<T>::ContactData DeformableDriver<T>::ComputeContactDataForDeformable(
        const systems::Context<T>& context,
        const geometry::internal::DeformableContactSurface<T>& surface,
        bool is_A) const {
    const geometry::GeometryId geometry_id = is_A ? surface.id_A() : surface.id_B();
    const DeformableBodyIndex body_index = deformable_model_->GetBodyIndex(deformable_model_->GetBodyId(geometry_id));
    const ContactParticipation& participation = EvalConstraintParticipation(context, body_index);
    const PartialPermutation& vertex_permutation = EvalVertexPermutation(context, geometry_id);
    const VectorX<T>& deformable_participating_v0 = EvalParticipatingVelocities(context);
    const Multiplexer<T>& mux = EvalParticipatingVelocityMultiplexer(context);
    const Eigen::Ref<const VectorX<T>> body_participating_v0 = mux.Demultiplex(deformable_participating_v0, body_index);
    ...
    const DirichletBoundaryCondition<T>& bc = fem_model.dirichlet_boundary_condition();
    ...
    for (int i = 0; i < surface.num_contact_points(); ++i) {
        ...
        const Vector4<int>& participating_vertices =
                is_A ? surface.contact_vertex_indexes_A()[i] : surface.contact_vertex_indexes_B()[i];
        const Vector4<T>& b = is_A ? surface.barycentric_coordinates_A()[i] : surface.barycentric_coordinates_B()[i];
        ...
        for (int v = 0; v < 4; ++v) {
            ...
            const int permuted_vertex = vertex_permutation.permuted_index(participating_vertices(v));
            if (!vertex_under_bc) {
                /* v_WAc = (b₀ * v₀ + b₁ * v₁ + b₂ * v₂ + b₃ * v₃) where v₀, v₁, v₂,
                 ...
                 barycentric weights. */
                triplets.emplace_back(0, permuted_vertex, scale * b(v) * surface.R_WCs()[i].matrix().transpose());
                v_WGc += b(v) * body_participating_v0.template segment<3>(3 * permuted_vertex);
            }
        ...
        const TreeIndex tree_index(manager_->internal_tree().get_topology().num_trees() + body_index);
        result.jacobian.emplace_back(tree_index, MatrixBlock<T>(std::move(scaled_Jv_v_WGc_C)));
    }
    return result;
}

template <typename T>
typename DeformableDriver<T>::ContactData DeformableDriver<T>::ComputeContactDataForRigid(
        const systems::Context<T>& context, const geometry::internal::DeformableContactSurface<T>& surface) const {
    ContactData result;
    ...
    const BodyIndex body_index = manager_->geometry_id_to_body_index().at(geometry_id);
    ...
    /* For rigid body, we use the origin of the body frame as the relative-to
     point. */
    result.p_WG = manager_->plant().EvalBodyPoseInWorld(context, manager_->plant().get_body(body_index)).translation();
    ...
    const Eigen::VectorBlock<const VectorX<T>> rigid_v0 = manager_->plant().GetVelocities(context);
    ...
    for (int i = 0; i < surface.num_contact_points(); ++i) {
        ...
        manager_->internal_tree().CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kV,
                                                                    rigid_body.body_frame(), frame_W, p_WC, frame_W,
                                                                    frame_W, &Jv_v_WGc_W);
        result.v_WGc.emplace_back(Jv_v_WGc_W * rigid_v0);
        Matrix3X<T> J = surface.R_WCs()[i].matrix().transpose() *
                        Jv_v_WGc_W.middleCols(tree_topology.tree_velocities_start_in_v(tree_index),
...
template <typename T>
void DeformableDriver<T>::AppendDiscreteContactPairs(const systems::Context<T>& context,
    ...
    const geometry::SceneGraphInspector<T>& inspector = manager_->plant().EvalSceneGraphInspector(context);
    const DeformableContact<T>& deformable_contact = EvalDeformableContact(context);
    const std::vector<DeformableContactSurface<T>>& contact_surfaces = deformable_contact.contact_surfaces();

    for (int surface_index = 0; surface_index < ssize(contact_surfaces); ++surface_index) {
        const DeformableContactSurface<T>& surface = contact_surfaces[surface_index];
        /* Write the contact jacobian and velocity for all contact points for body
         A. */
        ContactData contact_data_A = ComputeContactDataForDeformable(context, surface, true);
        const Vector3<T>& p_WA = contact_data_A.p_WG;

        ContactData contact_data_B = surface.is_B_deformable()
        ...
        const int body_index_A = deformable_model_->GetBodyIndex(deformable_model_->GetBodyId(id_A));
        /* Body B may be rigid or deformable. We first get its body index. */
        const int body_index_B =
                surface.is_B_deformable()
                        ? static_cast<int>(deformable_model_->GetBodyIndex(deformable_model_->GetBodyId(id_B)))
        ...
        /* By convention, the object index of a rigid body is just its body index.
         ...
         in the plant. */
        const int object_A = body_index_A + manager_->plant().num_bodies();  // Deformable body.
        const int object_B = surface.is_B_deformable()
        ...
        */
        ...
        for (int i = 0; i < surface.num_contact_points(); ++i) {
            ...
            const T kB = surface.is_B_deformable() ? kA : GetPointContactStiffness(id_B, default_rigid_k, inspector);
            ...
            const T d = GetCombinedHuntCrossleyDissipation(surface.id_A(), surface.id_B(), kA, kB,
            ...
            const T tau = GetCombinedDissipationTimeConstant(id_A, id_B, manager_->plant().time_step(),
                                                             contact_data_A.name, contact_data_B.name, inspector);
            const double mu = GetCombinedDynamicCoulombFriction(id_A, id_B, inspector);
...
void DeformableDriver<T>::AppendDeformableRigidFixedConstraintKinematics(
        const systems::Context<T>& context, std::vector<FixedConstraintKinematics<T>>* result) const {
    DRAKE_DEMAND(result != nullptr);
    ...
    const int nv = manager_->plant().num_velocities();
    const MultibodyTreeTopology& tree_topology = manager_->internal_tree().get_topology();
    const auto& configurations = manager_->plant()
    ...
    for (DeformableBodyIndex index(0); index < deformable_model_->num_bodies(); ++index) {
        ...
        auto is_under_bc = [&](int vertex_index) {
            return bc.index_to_boundary_state().count(multibody::fem::FemNodeIndex(vertex_index)) > 0;
        };

        const ContactParticipation& participation = EvalConstraintParticipation(context, index);
        ...
        for (const MultibodyConstraintId& constraint_id : deformable_model_->fixed_constraint_ids(body_id)) {
            const DeformableRigidFixedConstraintSpec& spec = deformable_model_->fixed_constraint_spec(constraint_id);
            const int num_vertices_in_constraint = ssize(spec.vertices);
            /* The Jacobian block for the deformable body A. */
            Block3x3SparseMatrix<T> negative_Jv_v_WAp(num_vertices_in_constraint,
                                                      participation.num_vertices_in_contact());
            ...
            const math::RigidTransform<T>& X_WB = manager_->plant().EvalBodyPoseInWorld(context, rigid_body);
            ...
            for (int v = 0; v < num_vertices_in_constraint; ++v) {
                ...
                if (!is_under_bc(spec.vertices[v])) {
                    const int permuted_vertex_index = vertex_permutation.permuted_index(spec.vertices[v]);
                    jacobian_triplets.emplace_back(v, permuted_vertex_index, -Matrix3<T>::Identity());
                }
            ...
            for (int v = 0; v < ssize(spec.vertices); ++v) {
                p_WPs.template segment<3>(3 * v) = p_WVs.template segment<3>(3 * spec.vertices[v]);
            }
            VectorX<T> p_PQs_W = p_WQs - p_WPs;
            /* By convention, deformable bodies are assigned object indexes after all
             rigid bodies. */
            const int object_A = index + manager_->plant().num_bodies();  // Deformable body.
            ...
            if (tree_index.is_valid()) {
                ...
                manager_->internal_tree().CalcJacobianTranslationalVelocity(
                        ...
                        Eigen::Map<const Matrix3X<T>>(p_WQs.data(), 3, p_WQs.size() / 3), frame_W, frame_W, &Jv_v_WBq);
                MatrixBlock<T> jacobian_block_B(
                        Jv_v_WBq.middleCols(tree_topology.tree_velocities_start_in_v(tree_index),
                                            tree_topology.num_tree_velocities(tree_index)));
                SapConstraintJacobian<T> J(clique_index_A, std::move(jacobian_block_A), clique_index_B,
                                           std::move(jacobian_block_B));
                result->emplace_back(object_A, std::move(p_WPs), object_B, std::move(p_BQs), std::move(p_PQs_W),
                                     std::move(J));
            } else {
                /* Rigid body is welded to world. */
                SapConstraintJacobian<T> J(clique_index_A, std::move(jacobian_block_A));
                result->emplace_back(object_A, std::move(p_WPs), std::move(p_PQs_W), std::move(J));
            }
        }
    }
}
...
template <typename T>
void DeformableDriver<T>::CalcDeformableContactInfo(const systems::Context<T>& context,
    ...
    const DiscreteContactData<DiscreteContactPair<T>>& contact_pairs = manager_->EvalDiscreteContactPairs(context);
    ...
    std::vector<SpatialForce<T>> F_Ao_W_per_surface(num_surfaces, SpatialForce<T>::Zero());
    std::vector<std::vector<DeformableContactPointData<T>>> contact_point_data(num_surfaces);
    for (int surface_index = 0; surface_index < num_surfaces; ++surface_index) {
        contact_point_data[surface_index].reserve(contact_surfaces[surface_index].num_contact_points());
    }

    for (int icontact = contact_pairs.deformable_contact_start();
         icontact < contact_pairs.deformable_contact_start() + contact_pairs.num_deformable_contacts(); ++icontact) {
        ...
        /* Contact forces applied on B at contact point point C expressed in the
         contact frame. */
        const Vector3<T> f_Bc_C(ft(2 * icontact), ft(2 * icontact + 1), fn(icontact));
        ...
        const SpatialForce<T> Fc_Ao_W = SpatialForce<T>(Vector3<T>::Zero(), f_Ac_W).Shift(p_CO_W);
        ...
        const Vector3<T> traction_Ac_W = f_Ac_W / s.contact_mesh_W().area(face_index);

        contact_point_data[surface_index].emplace_back(p_WC, face_index, vt_BcAc_W, traction_Ac_W);
    }
    for (int surface_index = 0; surface_index < num_surfaces; ++surface_index) {
        const DeformableContactSurface<T>& surface = contact_surfaces[surface_index];
        // TODO(xuchenhan-tri): consider avoid making a copy of the contact mesh
        // every time DeformableContactInfo is computed.
        contact_info->emplace_back(surface.id_A(), surface.id_B(), contact_surfaces[surface_index].contact_mesh_W(),
                                   F_Ao_W_per_surface[surface_index], std::move(contact_point_data[surface_index]));
    }
}

template <typename T>
void DeformableDriver<T>::CalcDiscreteStates(const systems::Context<T>& context,
    ...
    for (DeformableBodyIndex index(0); index < num_bodies; ++index) {
        ...
        next_states->set_value(deformable_model_->GetDiscreteStateIndex(id), discrete_value);
    }
}
...
template <typename T>
Eigen::Ref<const VectorX<T>> Multiplexer<T>::Demultiplex(const Eigen::Ref<const VectorX<T>>& input, int index) const {
...
template <typename T>
Eigen::Ref<VectorX<T>> Multiplexer<T>::Demultiplex(EigenPtr<VectorX<T>> input, int index) const {
    DRAKE_THROW_UNLESS(0 <= index && index < num_vectors());
    DRAKE_THROW_UNLESS(input->size() == num_entries_);
...
void DeformableDriver<T>::CalcFemState(const Context<T>& context,
    ...
    const systems::BasicVector<T>& discrete_state =
            context.get_discrete_state().get_vector(deformable_model_->GetDiscreteStateIndex(id));
...
template <typename T>
const FemState<T>& DeformableDriver<T>::EvalFemState(const Context<T>& context, DeformableBodyIndex index) const {
    return manager_->plant().get_cache_entry(cache_indexes_.fem_states.at(index)).template Eval<FemState<T>>(context);
}

template <typename T>
void DeformableDriver<T>::CalcFreeMotionFemSolver(const systems::Context<T>& context,
                                                  DeformableBodyIndex index,
    ...
    const PartialPermutation& permutation = EvalVertexPermutation(context, geometry_id);
    ...
    const fem::FemPlantData<T> plant_data{context, deformable_model_->GetExternalForces(body_id)};
    fem_solver->AdvanceOneTimeStep(fem_state, plant_data, nonparticipating_vertices);
}

template <typename T>
const FemSolver<T>& DeformableDriver<T>::EvalFreeMotionFemSolver(const systems::Context<T>& context,
                                                                 DeformableBodyIndex index) const {
    return manager_->plant().get_cache_entry(cache_indexes_.fem_solvers.at(index)).template Eval<FemSolver<T>>(context);
}

template <typename T>
const FemState<T>& DeformableDriver<T>::EvalFreeMotionFemState(const systems::Context<T>& context,
                                                               DeformableBodyIndex index) const {
...
template <typename T>
const SchurComplement& DeformableDriver<T>::EvalFreeMotionTangentMatrixSchurComplement(
...
void DeformableDriver<T>::CalcNextFemState(const systems::Context<T>& context,
    ...
    if (participation.num_vertices_in_contact() == 0) {
        ...
        const ContactSolverResults<T>& results = manager_->EvalContactSolverResults(context);
        ...
        const int num_participating_dofs = results.v_next.size() - manager_->plant().num_velocities();
        const auto participating_v_next = results.v_next.tail(num_participating_dofs);
        const VectorX<T>& participating_v_star = EvalParticipatingFreeMotionVelocities(context);
        /* The next time step velocities and free motion velocities for *this*
         body. */
        const Multiplexer<T>& mux = EvalParticipatingVelocityMultiplexer(context);
        const Eigen::Ref<const VectorX<T>> body_participating_v_star = mux.Demultiplex(participating_v_star, index);
        const Eigen::Ref<const VectorX<T>> body_participating_v_next = mux.Demultiplex(participating_v_next, index);
        const VectorX<T> body_participating_dv = body_participating_v_next - body_participating_v_star;
        /* Compute the value of the post-constraint non-participating
         velocities using Schur complement. */
        const SchurComplement& schur_complement = EvalFreeMotionTangentMatrixSchurComplement(context, index);
        const VectorX<T> body_nonparticipating_dv = schur_complement.SolveForX(body_participating_dv);
        /* Concatenate the participating and non-participating velocities and
         ...
         order. */
        const PartialPermutation& full_velocity_permutation = participation.CalcDofPermutation();
...
template <typename T>
const FemState<T>& DeformableDriver<T>::EvalNextFemState(const systems::Context<T>& context,
                                                         DeformableBodyIndex index) const {
...
template <typename T>
void DeformableDriver<T>::CalcDeformableContact(const geometry::QueryObject<T>& query_object,
    ...
    for (DeformableBodyIndex body_index(0); body_index < deformable_model_->num_bodies(); ++body_index) {
        ...
        if (deformable_model_->HasConstraint(body_id)) {
            ...
            if (!result->IsRegistered(geometry_id)) {
                result->RegisterDeformableGeometry(geometry_id, deformable_model_->GetFemModel(body_id).num_nodes());
            }
...
template <typename T>
const DeformableContact<T>& DeformableDriver<T>::EvalDeformableContact(const Context<T>& context) const {
...
template <typename T>
const ContactParticipation& DeformableDriver<T>::EvalConstraintParticipation(const Context<T>& context,
                                                                             DeformableBodyIndex index) const {
    return manager_->plant()
...
void DeformableDriver<T>::CalcDofPermutation(const Context<T>& context,
    ...
    *result = EvalConstraintParticipation(context, index).CalcDofPartialPermutation();
}

template <typename T>
const PartialPermutation& DeformableDriver<T>::EvalDofPermutation(const Context<T>& context,
                                                                  DeformableBodyIndex index) const {
    return manager_->plant()
...
template <typename T>
void DeformableDriver<T>::CalcVertexPermutation(const Context<T>& context,
                                                GeometryId id,
                                                PartialPermutation* result) const {
    const DeformableBodyIndex index = deformable_model_->GetBodyIndex(deformable_model_->GetBodyId(id));
    *result = EvalConstraintParticipation(context, index).CalcVertexPartialPermutation();
}

template <typename T>
const PartialPermutation& DeformableDriver<T>::EvalVertexPermutation(const Context<T>& context, GeometryId id) const {
    return manager_->plant()
...
template <typename T>
void DeformableDriver<T>::CalcParticipatingVelocityMultiplexer(const Context<T>& context,
                                                               Multiplexer<T>* result) const {
    const int num_bodies = deformable_model_->num_bodies();
    std::vector<int> num_participating_dofs(num_bodies);
    for (DeformableBodyIndex i(0); i < num_bodies; ++i) {
        num_participating_dofs[i] = EvalDofPermutation(context, i).permuted_domain_size();
    }
    *result = Multiplexer<T>(std::move(num_participating_dofs));
}

template <typename T>
const Multiplexer<T>& DeformableDriver<T>::EvalParticipatingVelocityMultiplexer(const Context<T>& context) const {
    return manager_->plant()
...
template <typename T>
void DeformableDriver<T>::CalcParticipatingVelocities(const Context<T>& context, VectorX<T>* result) const {
    const int num_bodies = deformable_model_->num_bodies();
    ...
    *result = EvalParticipatingVelocityMultiplexer(context).Multiplex(std::move(participating_velocities));
}

template <typename T>
const VectorX<T>& DeformableDriver<T>::EvalParticipatingVelocities(const Context<T>& context) const {
    return manager_->plant()
...
template <typename T>
void DeformableDriver<T>::CalcParticipatingFreeMotionVelocities(const Context<T>& context, VectorX<T>* result) const {
    DRAKE_DEMAND(result != nullptr);
    const int num_bodies = deformable_model_->num_bodies();
    std::vector<VectorX<T>> participating_v_star(num_bodies);
    for (DeformableBodyIndex i(0); i < num_bodies; ++i) {
        const PartialPermutation& permutation = EvalDofPermutation(context, i);
        const VectorX<T>& v_star = EvalFreeMotionFemState(context, i).GetVelocities();
    ...
    *result = EvalParticipatingVelocityMultiplexer(context).Multiplex(std::move(participating_v_star));
}

template <typename T>
const VectorX<T>& DeformableDriver<T>::EvalParticipatingFreeMotionVelocities(const Context<T>& context) const {
    return manager_->plant()
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::Multiplexer);
