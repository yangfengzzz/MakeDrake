...
#include "multibody/plant/contact_jacobians.h"
...
#include "multibody/plant/scalar_convertible_component.h"
...
public:
    ...
    void CalcContactSolverResults(const systems::Context<T>& context,
    ...
    /* MultibodyPlant invokes this method to perform the discrete variables
     update. */
    void CalcDiscreteValues(const systems::Context<T>& context, systems::DiscreteValues<T>* updates) const;
    ...
    const contact_solvers::internal::ContactSolverResults<T>& EvalContactSolverResults(
    ...
    const MultibodyForces<T>& EvalDiscreteUpdateMultibodyForces(const systems::Context<T>& context) const;
    ...
    const std::unordered_map<geometry::GeometryId, BodyIndex>& geometry_id_to_body_index() const;
    ...
    const DeformableDriver<double>* deformable_driver() const { return deformable_driver_.get(); }

    /* Private MultibodyPlant method, made public here. */
    const GeometryContactData<T>& EvalGeometryContactData(const systems::Context<T>& context) const;
...
protected:
    ...
    virtual std::unique_ptr<DiscreteUpdateManager<AutoDiffXd>> CloneToAutoDiffXd() const;
    ...
    virtual std::unique_ptr<DiscreteUpdateManager<symbolic::Expression>> CloneToSymbolic() const;
    ...
    systems::DiscreteStateIndex multibody_state_index() const { return multibody_state_index_; }
    ...
    const InputPortForces<T>& EvalInputPortForces(const drake::systems::Context<T>& context) const {
        return plant()
    ...
    void CalcForceElementsContribution(const drake::systems::Context<T>& context, MultibodyForces<T>* forces) const;
    ...
    const std::map<MultibodyConstraintId, internal::CouplerConstraintSpec>& coupler_constraints_specs() const;

    const std::map<MultibodyConstraintId, internal::DistanceConstraintSpec>& distance_constraints_specs() const;

    const std::map<MultibodyConstraintId, internal::BallConstraintSpec>& ball_constraints_specs() const;

    const std::map<MultibodyConstraintId, internal::WeldConstraintSpec>& weld_constraints_specs() const;
    ...
    virtual void DoCalcContactSolverResults(const systems::Context<T>& context,
                                            contact_solvers::internal::ContactSolverResults<T>* results) const = 0;

    virtual void DoCalcAccelerationKinematicsCache(const systems::Context<T>& context,
    ...
    virtual void DoCalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context,
    ...
    virtual void DoCalcActuation(const systems::Context<T>& context, VectorX<T>* actuation) const = 0;
    ...
    virtual void DoCalcDiscreteValues(const systems::Context<T>& context, systems::DiscreteValues<T>* updates) const;
...
private:
    ...
    [[nodiscard]] ScopeExit ThrowIfNonContactForceInProgress(const systems::Context<T>& context) const;
    ...
    void CalcInputPortForces(const systems::Context<T>& context, InputPortForces<T>* forces) const;

    /* NVI to DoDeclareCacheEntries(). */
    void DeclareCacheEntries();

    /* Calc version of EvalContactResults(). */
    void CalcContactResults(const systems::Context<T>& context, ContactResults<T>* contact_results) const;

    /* Calc version of EvalDiscreteUpdateMultibodyForces, NVI to
     DoCalcDiscreteUpdateMultibodyForces. */
    void CalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const;

    /* Calc version of EvalActuation, NVI to DoCalcActuation. */
    void CalcActuation(const systems::Context<T>& context, VectorX<T>* forces) const;

    /* Calc version of EvalDiscreteContactPairs(). */
    void CalcDiscreteContactPairs(const systems::Context<T>& context,
    ...
    void AppendDiscreteContactPairsForHydroelasticContact(
            const systems::Context<T>& context, DiscreteContactData<DiscreteContactPair<T>>* contact_kinematics) const
    ...
    void AppendContactResultsForPointContact(const systems::Context<T>& context,
    ...
    /* Helper method to fill in `contact_results` with hydroelastic contact
     ...
     @param[in,out] contact_results is appended to. */
    void AppendContactResultsForHydroelasticContact(const systems::Context<T>& context,
                                                    ContactResults<T>* contact_results) const;

    /* Helper method to fill in `contact_results` with deformable contact
     information for the given state stored in `context`.
     @param[in,out] contact_results is appended to. */
    void AppendContactResultsForDeformableContact(const systems::Context<T>& context,
                                                  ContactResults<T>* contact_results) const;
    ...
    void CalcHydroelasticContactInfo(const systems::Context<T>& context,
