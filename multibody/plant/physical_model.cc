...
#include "multibody/plant/multibody_plant_model_attorney.h"
...
template <typename T>
std::unique_ptr<PhysicalModel<double>> PhysicalModel<T>::CloneToDouble(MultibodyPlant<double>*) const {
    throw std::logic_error("Scalar conversion to double is not supported by this PhysicalModel.");
}

template <typename T>
std::unique_ptr<PhysicalModel<AutoDiffXd>> PhysicalModel<T>::CloneToAutoDiffXd(MultibodyPlant<AutoDiffXd>*) const {
...
template <typename T>
std::unique_ptr<PhysicalModel<symbolic::Expression>> PhysicalModel<T>::CloneToSymbolic(
        MultibodyPlant<symbolic::Expression>*) const {
    throw std::logic_error(
...
template <typename T>
void PhysicalModel<T>::ThrowIfSystemResourcesDeclared(const char* function_name) const {
...
template <typename T>
void PhysicalModel<T>::ThrowIfSystemResourcesNotDeclared(const char* function_name) const {
    if (owning_plant_ != nullptr) {
        throw std::logic_error(
...
geometry::SceneGraph<T>& PhysicalModel<T>::mutable_scene_graph() {
    ...
    return internal::MultibodyPlantModelAttorney<T>::mutable_scene_graph(owning_plant_);
}

template <typename T>
systems::DiscreteStateIndex PhysicalModel<T>::DeclareDiscreteState(const VectorX<T>& model_value) {
    DRAKE_THROW_UNLESS(owning_plant_ != nullptr);
    return internal::MultibodyPlantModelAttorney<T>::DeclareDiscreteState(owning_plant_, model_value);
}
...
systems::LeafOutputPort<T>& PhysicalModel<T>::DeclareAbstractOutputPort(
    ...
    return internal::MultibodyPlantModelAttorney<T>::DeclareAbstractOutputPort(
            owning_plant_, std::move(name), std::move(alloc_function), std::move(calc_function),
...
systems::LeafOutputPort<T>& PhysicalModel<T>::DeclareVectorOutputPort(
    ...
    return internal::MultibodyPlantModelAttorney<T>::DeclareVectorOutputPort(
            owning_plant_, std::move(name), model_vector, std::move(vector_calc_function),
            std::move(prerequisites_of_calc));
}

}  // namespace multibody
}  // namespace drake
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::PhysicalModel);
