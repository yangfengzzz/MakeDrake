...
T GetPointContactStiffness(geometry::GeometryId id,
    ...
    return prop->template GetPropertyOrDefault<double>(geometry::internal::kMaterialGroup,
                                                       geometry::internal::kPointStiffness, default_value);
}
...
T GetHydroelasticModulus(geometry::GeometryId id,
    ...
    if (prop->GetPropertyOrDefault(geometry::internal::kHydroGroup, geometry::internal::kComplianceType,
                                   geometry::internal::HydroelasticType::kUndefined) ==
        geometry::internal::HydroelasticType::kRigid) {
        return std::numeric_limits<double>::infinity();
    }
    return prop->template GetPropertyOrDefault<double>(geometry::internal::kHydroGroup, geometry::internal::kElastic,
                                                       default_value);
}
...
T GetHuntCrossleyDissipation(geometry::GeometryId id,
    ...
    DRAKE_DEMAND(prop != nullptr);
    return prop->template GetPropertyOrDefault<double>(geometry::internal::kMaterialGroup,
                                                       geometry::internal::kHcDissipation, default_value);
}
...
T GetDissipationTimeConstant(geometry::GeometryId id,
    ...
    auto provide_context_string = [&inspector, &body_name](geometry::GeometryId geometry_id) -> std::string {
        return fmt::format("For geometry {} on body {}.", inspector.GetName(geometry_id), body_name);
    };
    const T relaxation_time = prop->template GetPropertyOrDefault<double>(geometry::internal::kMaterialGroup,
                                                                          "relaxation_time", default_value);
...
T GetCombinedDissipationTimeConstant(geometry::GeometryId id_A,
                                     ...
                                     const geometry::SceneGraphInspector<T>& inspector) {
    return GetDissipationTimeConstant(id_A, default_value, inspector, body_A_name) +
           GetDissipationTimeConstant(id_B, default_value, inspector, body_B_name);
}
...
double GetCombinedDynamicCoulombFriction(geometry::GeometryId id_A,
    ...
    return CalcContactFrictionFromSurfaceProperties(mu_A, mu_B).dynamic_friction();
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((
        &GetPointContactStiffness<T>,
        &GetHydroelasticModulus<T>,
        &GetHuntCrossleyDissipation<T>,
        &GetCombinedHuntCrossleyDissipation<T>,
        &GetDissipationTimeConstant<T>,
        &GetCoulombFriction<T>,
        static_cast<T (*)(geometry::GeometryId, geometry::GeometryId, double, const geometry::SceneGraphInspector<T>&)>(
                &GetCombinedPointContactStiffness<T>),
        static_cast<T (*)(const T&, const T&)>(&GetCombinedPointContactStiffness<T>),
