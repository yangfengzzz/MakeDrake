...
private:
    ...
    // stored in `context`.
    internal::ContactJacobians<T> CalcContactJacobians(const systems::Context<T>& context) const;
    ...
    TamsiSolverResult SolveUsingSubStepping(TamsiSolver<T>* tamsi_solver,
                                            int num_substeps,
                                            const MatrixX<T>& M0,
                                            const MatrixX<T>& Jn,
                                            const MatrixX<T>& Jt,
    ...
    void CallTamsiSolver(TamsiSolver<T>* tamsi_solver,
                         const T& time0,
                         const VectorX<T>& v0,
                         const MatrixX<T>& M0,
                         const VectorX<T>& minus_tau,
    ...
    // contact results.
    void CalcAndAddSpatialContactForcesFromContactResults(const systems::Context<T>& context,
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::TamsiDriver);
