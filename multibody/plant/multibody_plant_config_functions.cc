...
AddResult AddMultibodyPlant(const MultibodyPlantConfig& config, systems::DiagramBuilder<double>* builder) {
    ...
    if (result.plant.is_discrete()) {
        DRAKE_DEMAND(result.plant.get_discrete_contact_approximation() == DiscreteContactApproximation::kTamsi);
        DRAKE_DEMAND(result.plant.get_discrete_contact_solver() == DiscreteContactSolver::kTamsi);
    }
...
AddMultibodyPlantSceneGraphResult<double> AddMultibodyPlant(const MultibodyPlantConfig& plant_config,
                                                            const geometry::SceneGraphConfig& scene_graph_config,
                                                            systems::DiagramBuilder<double>* builder) {
    AddResult result = AddMultibodyPlantSceneGraph(builder, plant_config.time_step);
...
void ApplyMultibodyPlantConfig(const MultibodyPlantConfig& config, MultibodyPlant<double>* plant) {
    ...
    plant->set_contact_model(internal::GetContactModelFromString(config.contact_model));
    // Only one of these can be non-empty at a time.
    if (!config.discrete_contact_solver.empty() && !config.discrete_contact_approximation.empty()) {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
            ...
            plant->set_discrete_contact_solver(
                    internal::GetDiscreteContactSolverFromString(config.discrete_contact_solver));
#pragma GCC diagnostic pop
            ...
            plant->set_discrete_contact_approximation(
                    internal::GetDiscreteContactApproximationFromString(config.discrete_contact_approximation));
        } else {
    ...
    plant->set_contact_surface_representation(
            internal::GetContactSurfaceRepresentationFromString(config.contact_surface_representation));
    plant->set_adjacent_bodies_collision_filters(config.adjacent_bodies_collision_filters);
}
...
struct NamedEnum {
    ...
    constexpr NamedEnum(Enum value_in) : value(value_in), name(EnumToChars(value_in)) {}
    const Enum value;
...
constexpr std::array<NamedEnum<DiscreteContactApproximation>, 4> kDiscreteContactApproximations{{
...
ContactModel GetContactModelFromString(std::string_view contact_model) {
    ...
    throw std::logic_error(fmt::format("Unknown contact_model: '{}'", contact_model));
}
...
DiscreteContactSolver GetDiscreteContactSolverFromString(std::string_view discrete_contact_solver) {
    for (const auto& [value, name] : kContactSolvers) {
        if (name == discrete_contact_solver) {
            return value;
        }
    }
    throw std::logic_error(fmt::format("Unknown discrete_contact_solver: '{}'", discrete_contact_solver));
}

std::string GetStringFromDiscreteContactSolver(DiscreteContactSolver contact_solver) {
    for (const auto& [value, name] : kContactSolvers) {
...
DiscreteContactApproximation GetDiscreteContactApproximationFromString(
    ...
    throw std::logic_error(fmt::format("Unknown discrete_contact_approximation: '{}'", discrete_contact_approximation));
}

std::string GetStringFromDiscreteContactApproximation(DiscreteContactApproximation contact_model) {
    for (const auto& [value, name] : kDiscreteContactApproximations) {
        if (value == contact_model) {
            return name;
        }
...
ContactRep GetContactSurfaceRepresentationFromString(std::string_view contact_representation) {
    for (const auto& [value, name] : kContactReps) {
        if (name == contact_representation) {
            return value;
        }
    }
    throw std::logic_error(fmt::format("Unknown hydroelastic contact representation: '{}'", contact_representation));
}

std::string GetStringFromContactSurfaceRepresentation(ContactRep contact_representation) {
    for (const auto& [value, name] : kContactReps) {
