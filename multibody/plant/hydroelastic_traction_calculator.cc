...
#include "multibody/triangle_quadrature/triangle_quadrature.h"
...
template <class T>
void HydroelasticTractionCalculator<T>::ComputeSpatialForcesAtCentroidFromHydroelasticModel(
        const Data& data,
        double dissipation,
        double mu_coulomb,
        std::vector<HydroelasticQuadraturePointData<T>>* traction_at_quadrature_points,
    ...
    for (int i = 0; i < data.surface.num_faces(); ++i) {
        ...
        if (data.surface.is_triangle()) {
            std::function<SpatialForce<T>(const Vector3<T>&)> traction_Ac_W = [this, &data, i, dissipation, mu_coulomb,
                                                                               traction_at_quadrature_points](
                                                                                      const Vector3<T>& Q_barycentric) {
                traction_at_quadrature_points->emplace_back(
                        CalcTractionAtPoint(data, i, Q_barycentric, dissipation, mu_coulomb));
                const HydroelasticQuadraturePointData<T>& traction_output = traction_at_quadrature_points->back();
                return ComputeSpatialTractionAtAcFromTractionAtAq(data, traction_output.p_WQ,
                                                                  traction_output.traction_Aq_W);
            };
            ...
            const SpatialForce<T> Fi_Ac_W =  // Force from triangle i.
                    TriangleQuadrature<SpatialForce<T>, T>::Integrate(traction_Ac_W, gaussian, data.surface.area(i));
            // Update the spatial force at the centroid.
            (*F_Ac_W) += Fi_Ac_W;
        } else {
            traction_at_quadrature_points->emplace_back(CalcTractionAtCentroid(data, i, dissipation, mu_coulomb));
            const HydroelasticQuadraturePointData<T>& traction_output = traction_at_quadrature_points->back();
            const SpatialForce<T> traction_Ac_W = ComputeSpatialTractionAtAcFromTractionAtAq(
...
template <class T>
void HydroelasticTractionCalculator<T>::ShiftSpatialForcesAtCentroidToBodyOrigins(const Data& data,
...
template <typename T>
SpatialForce<T> HydroelasticTractionCalculator<T>::ComputeSpatialTractionAtAcFromTractionAtAq(
        const Data& data, const Vector3<T>& p_WQ, const Vector3<T>& traction_Aq_W) const {
...
template <typename T>
HydroelasticQuadraturePointData<T> HydroelasticTractionCalculator<T>::CalcTractionAtPoint(
        const Data& data,
        int face_index,
        // NOLINTNEXTLINE(runtime/references): "template Bar..." confuses cpplint.
        const typename TriangleSurfaceMesh<T>::template Barycentric<T>& Q_barycentric,
        double dissipation,
        double mu_coulomb) const {
    // Compute the point of contact in the world frame.
    const Vector3<T> p_WQ = data.surface.tri_mesh_W().CalcCartesianFromBarycentric(face_index, Q_barycentric);
    ...
    return CalcTractionAtQHelper(data, face_index, e, nhat_W, dissipation, mu_coulomb, p_WQ);
}

template <typename T>
HydroelasticQuadraturePointData<T> HydroelasticTractionCalculator<T>::CalcTractionAtCentroid(const Data& data,
                                                                                             int face_index,
                                                                                             double dissipation,
                                                                                             double mu_coulomb) const {
    ...
    T e;
    if (data.surface.is_triangle()) {
        const typename TriangleSurfaceMesh<T>::template Barycentric<T> centroid_barycentric(1. / 3., 1. / 3., 1. / 3.);
    ...
    return CalcTractionAtQHelper(data, face_index, e, nhat_W, dissipation, mu_coulomb, p_WC);
}
 ...
 */
template <typename T>
HydroelasticQuadraturePointData<T> HydroelasticTractionCalculator<T>::CalcTractionAtQHelper(
        const Data& data,
        int face_index,
        const T& e,
    ...
    const T regularized_friction = (2.0 / M_PI) * mu_coulomb * normal_traction * CalcAtanXOverXFromXSquared(x_squared) /
