...
#include "multibody/plant/deformable_model.h"
#include "multibody/plant/multibody_plant.h"
#include "multibody/plant/multibody_plant_discrete_update_manager_attorney.h"
...
template <typename T>
void DiscreteUpdateManager<T>::CalcDiscreteValues(const systems::Context<T>& context,
...
void DiscreteUpdateManager<T>::DeclareCacheEntries() {
    const auto& query_object_input_ticket = plant().get_geometry_query_input_port().ticket();
    ...
    const auto& discrete_input_port_forces_cache_entry = DeclareCacheEntry(
            ...
            systems::ValueProducer(this, InputPortForces<T>(plant()), &DiscreteUpdateManager<T>::CalcInputPortForces),
            // The cache entry is manually managed to refresh at the beginning of a
            // discrete update.
            {systems::System<T>::nothing_ticket()});
    cache_indexes_.discrete_input_port_forces = discrete_input_port_forces_cache_entry.cache_index();

    const auto& contact_solver_results_cache_entry = DeclareCacheEntry(
            "Contact solver results", systems::ValueProducer(this, &DiscreteUpdateManager<T>::CalcContactSolverResults),
            ...
            {systems::System<T>::xd_ticket(), systems::System<T>::all_parameters_ticket(),
             ...
             discrete_input_port_forces_cache_entry.ticket(), query_object_input_ticket});
    cache_indexes_.contact_solver_results = contact_solver_results_cache_entry.cache_index();
    ...
    const auto& non_contact_forces_evaluation_in_progress = DeclareCacheEntry(
            ...
            systems::ValueProducer(false, &systems::ValueProducer::NoopCalc), {systems::System<T>::nothing_ticket()});
    cache_indexes_.non_contact_forces_evaluation_in_progress = non_contact_forces_evaluation_in_progress.cache_index();
    ...
    const auto& multibody_forces_cache_entry = DeclareCacheEntry(
            ...
            systems::ValueProducer(this, model_forces, &DiscreteUpdateManager<T>::CalcDiscreteUpdateMultibodyForces),
            // TODO(#20545): this should include all_input_ports_ticket, but we can't
            //  include it yet due to the cache entry hack introduced in #19225.
            {systems::System<T>::xd_ticket(), systems::System<T>::all_parameters_ticket()});
    cache_indexes_.discrete_update_multibody_forces = multibody_forces_cache_entry.cache_index();

    const auto& actuation_cache_entry =
            DeclareCacheEntry("Discrete update actuation",
    ...
    const auto& contact_results_cache_entry = DeclareCacheEntry(
            "Contact results (discrete)", systems::ValueProducer(this, &DiscreteUpdateManager<T>::CalcContactResults),
            // TODO(#20545): this should include all_input_ports_ticket, but we can't
            //  include it yet due to the cache entry hack introduced in #19225.
    ...
    const auto& discrete_contact_pairs_cache_entry = DeclareCacheEntry(
            ...
            systems::ValueProducer(this, &DiscreteUpdateManager<T>::CalcDiscreteContactPairs),
            {systems::System<T>::xd_ticket(), systems::System<T>::all_parameters_ticket(), query_object_input_ticket});
    cache_indexes_.discrete_contact_pairs = discrete_contact_pairs_cache_entry.cache_index();

    // Cache hydroelastic contact info.
    if constexpr (scalar_predicate<T>::is_bool) {
        const auto& hydroelastic_contact_info_cache_entry =
                DeclareCacheEntry("Hydroelastic contact info.",
                                  systems::ValueProducer(this, &DiscreteUpdateManager<T>::CalcHydroelasticContactInfo),
        ...
        cache_indexes_.hydroelastic_contact_info = hydroelastic_contact_info_cache_entry.cache_index();
    }
...
template <typename T>
systems::CacheEntry& DiscreteUpdateManager<T>::DeclareCacheEntry(std::string description,
    ...
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::DeclareCacheEntry(
            mutable_plant_, std::move(description), std::move(value_producer), std::move(prerequisites_of_calc));
}

template <typename T>
double DiscreteUpdateManager<T>::default_contact_stiffness() const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::default_contact_stiffness(plant());
}

template <typename T>
double DiscreteUpdateManager<T>::default_contact_dissipation() const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::default_contact_dissipation(plant());
}

template <typename T>
const std::unordered_map<geometry::GeometryId, BodyIndex>& DiscreteUpdateManager<T>::geometry_id_to_body_index() const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::geometry_id_to_body_index(*plant_);
}

template <typename T>
std::unique_ptr<DiscreteUpdateManager<double>> DiscreteUpdateManager<T>::CloneToDouble() const {
...
template <typename T>
std::unique_ptr<DiscreteUpdateManager<AutoDiffXd>> DiscreteUpdateManager<T>::CloneToAutoDiffXd() const {
    throw std::logic_error(
...
template <typename T>
std::unique_ptr<DiscreteUpdateManager<symbolic::Expression>> DiscreteUpdateManager<T>::CloneToSymbolic() const {
    throw std::logic_error(
...
template <typename T>
void DiscreteUpdateManager<T>::CalcNonContactForces(const drake::systems::Context<T>& context,
...
template <typename T>
const contact_solvers::internal::ContactSolverResults<T>& DiscreteUpdateManager<T>::EvalContactSolverResults(
        const systems::Context<T>& context) const {
    return plant()
            .get_cache_entry(cache_indexes_.contact_solver_results)
            .template Eval<contact_solvers::internal::ContactSolverResults<T>>(context);
}

template <typename T>
const GeometryContactData<T>& DiscreteUpdateManager<T>::EvalGeometryContactData(
        const systems::Context<T>& context) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::EvalGeometryContactData(plant(), context);
}

template <typename T>
void DiscreteUpdateManager<T>::AddJointLimitsPenaltyForces(const systems::Context<T>& context,
                                                           MultibodyForces<T>* forces) const {
    MultibodyPlantDiscreteUpdateManagerAttorney<T>::AddJointLimitsPenaltyForces(plant(), context, forces);
}

template <typename T>
void DiscreteUpdateManager<T>::CalcForceElementsContribution(const drake::systems::Context<T>& context,
                                                             MultibodyForces<T>* forces) const {
    MultibodyPlantDiscreteUpdateManagerAttorney<T>::CalcForceElementsContribution(plant(), context, forces);
}

template <typename T>
const internal::JointLockingCacheData<T>& DiscreteUpdateManager<T>::EvalJointLocking(
        const systems::Context<T>& context) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::EvalJointLocking(plant(), context);
}

template <typename T>
VectorX<T> DiscreteUpdateManager<T>::AssembleActuationInput(const systems::Context<T>& context) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::AssembleActuationInput(plant(), context);
}

template <typename T>
VectorX<T> DiscreteUpdateManager<T>::AssembleDesiredStateInput(const systems::Context<T>& context) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::AssembleDesiredStateInput(plant(), context);
}
...
DiscreteUpdateManager<T>::coupler_constraints_specs() const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::coupler_constraints_specs(*plant_);
}
...
DiscreteUpdateManager<T>::distance_constraints_specs() const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::distance_constraints_specs(*plant_);
}

template <typename T>
const std::map<MultibodyConstraintId, internal::BallConstraintSpec>& DiscreteUpdateManager<T>::ball_constraints_specs()
        const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::ball_constraints_specs(*plant_);
}

template <typename T>
const std::map<MultibodyConstraintId, internal::WeldConstraintSpec>& DiscreteUpdateManager<T>::weld_constraints_specs()
        const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::weld_constraints_specs(*plant_);
}

template <typename T>
const std::map<MultibodyConstraintId, bool>& DiscreteUpdateManager<T>::GetConstraintActiveStatus(
        const systems::Context<T>& context) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::GetConstraintActiveStatus(context, *plant_);
}

template <typename T>
BodyIndex DiscreteUpdateManager<T>::FindBodyByGeometryId(geometry::GeometryId geometry_id) const {
    return MultibodyPlantDiscreteUpdateManagerAttorney<T>::FindBodyByGeometryId(plant(), geometry_id);
}

template <typename T>
ScopeExit DiscreteUpdateManager<T>::ThrowIfNonContactForceInProgress(const systems::Context<T>& context) const {
    systems::CacheEntryValue& value = plant().get_cache_entry(cache_indexes_.non_contact_forces_evaluation_in_progress)
...
template <typename T>
void DiscreteUpdateManager<T>::SampleDiscreteInputPortForces(const drake::systems::Context<T>& context) const {
    const auto& discrete_input_forces_cache_entry = plant().get_cache_entry(cache_indexes_.discrete_input_port_forces);
    ...
    auto& cache_entry_value = discrete_input_forces_cache_entry.get_mutable_cache_entry_value(context);
    cache_entry_value.mark_out_of_date();
    InputPortForces<T>& forces = cache_entry_value.template GetMutableValueOrThrow<InputPortForces<T>>();
    CalcInputPortForces(context, &forces);
    cache_entry_value.mark_up_to_date();

    // Initiate a value modification event.
    const systems::DependencyTracker& tracker = context.get_tracker(discrete_input_forces_cache_entry.ticket());
    tracker.NoteValueChange(context.start_new_change_event());
}

template <typename T>
void DiscreteUpdateManager<T>::CalcJointActuationForces(const systems::Context<T>& context,
    ...
    if (plant().num_actuators() > 0) {
        ...
        for (JointActuatorIndex actuator_index : plant().GetJointActuatorIndices()) {
            ...
            VectorX<T>& actuation = actuator.has_controller() ? *actuation_w_pd : *actuation_wo_pd;
...
template <typename T>
void DiscreteUpdateManager<T>::CalcInputPortForces(const systems::Context<T>& context,
                                                   InputPortForces<T>* forces) const {
    forces->SetZero();
    MultibodyPlantDiscreteUpdateManagerAttorney<T>::AddAppliedExternalGeneralizedForces(
            plant(), context, &forces->externally_applied_forces);
    MultibodyPlantDiscreteUpdateManagerAttorney<T>::AddAppliedExternalSpatialForces(plant(), context,
                                                                                    &forces->externally_applied_forces);
    CalcJointActuationForces(context, &forces->actuation_w_pd, &forces->actuation_wo_pd);
}

template <typename T>
void DiscreteUpdateManager<T>::CalcContactResults(const systems::Context<T>& context,
...
template <typename T>
void DiscreteUpdateManager<T>::AppendContactResultsForPointContact(const drake::systems::Context<T>& context,
                                                                   ContactResults<T>* contact_results) const {
    DRAKE_DEMAND(contact_results != nullptr);

    const std::vector<PenetrationAsPointPair<T>>& point_pairs = EvalGeometryContactData(context).point_pairs;
    const DiscreteContactData<DiscreteContactPair<T>>& contact_pairs = EvalDiscreteContactPairs(context);
    const contact_solvers::internal::ContactSolverResults<T>& solver_results = EvalContactSolverResults(context);
    ...
    for (int icontact = 0; icontact < num_point_contacts; ++icontact) {
        ...
        contact_results->AddContactInfo(
                {bodyA_index, bodyB_index, f_Bc_W, pair.p_WC, separation_velocity, slip, point_pair});
    }
}

template <typename T>
void DiscreteUpdateManager<T>::AppendContactResultsForHydroelasticContact(const drake::systems::Context<T>& context,
                                                                          ContactResults<T>* contact_results) const {
    const std::vector<HydroelasticContactInfo<T>>& contact_info = EvalHydroelasticContactInfo(context);
...
template <typename T>
void DiscreteUpdateManager<T>::CalcHydroelasticContactInfo(const systems::Context<T>& context,
...
{  // NOLINT(whitespace/braces)
    ...
    std::vector<std::vector<HydroelasticQuadraturePointData<T>>> quadrature_data(num_surfaces);
    ...
    // points.
    for (int icontact = num_point_contacts; icontact < num_point_contacts + num_hydro_contacts; ++icontact) {
        const DiscreteContactPair<T>& pair = contact_pairs[icontact];
        // Quadrature point Q.
        ...
        // contact frame.
        const Vector3<T> f_Bq_C(ft(2 * icontact), ft(2 * icontact + 1), fn(icontact));
        ...
        const Vector3<T>& p_WO = s.is_triangle() ? s.tri_mesh_W().centroid() : s.poly_mesh_W().centroid();

        // Spatial force
        const Vector3<T> p_QO_W = p_WO - p_WQ;
        const SpatialForce<T> Fq_Ao_W = SpatialForce<T>(Vector3<T>::Zero(), f_Aq_W).Shift(p_QO_W);
    ...
    for (int surface_index = 0; surface_index < num_surfaces; ++surface_index) {
        contact_info->emplace_back(&all_surfaces[surface_index], F_Ao_W_per_surface[surface_index],
                                   std::move(quadrature_data[surface_index]));
    }
}

template <typename T>
const std::vector<HydroelasticContactInfo<T>>& DiscreteUpdateManager<T>::EvalHydroelasticContactInfo(
        const systems::Context<T>& context) const {
    return plant()
...
template <typename T>
void DiscreteUpdateManager<T>::AppendContactResultsForDeformableContact(const drake::systems::Context<T>& context,
                                                                        ContactResults<T>* contact_results) const {
    DRAKE_DEMAND(contact_results != nullptr);
    if constexpr (std::is_same_v<T, double>) {
        if (deformable_driver_ != nullptr) {
...
template <typename T>
void DiscreteUpdateManager<T>::CalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context,
                                                                 MultibodyForces<T>* forces) const {
    plant().ValidateContext(context);
    DRAKE_DEMAND(forces != nullptr);
    DRAKE_DEMAND(forces->CheckHasRightSizeForModel(plant()));
...
template <typename T>
void DiscreteUpdateManager<T>::CalcActuation(const systems::Context<T>& context, VectorX<T>* actuation) const {
    plant().ValidateContext(context);
...
template <typename T>
void DiscreteUpdateManager<T>::CalcDiscreteContactPairs(const systems::Context<T>& context,
...
void DiscreteUpdateManager<T>::AppendDiscreteContactPairsForPointContact(
        const systems::Context<T>& context, DiscreteContactData<DiscreteContactPair<T>>* contact_pairs) const {
    const std::vector<PenetrationAsPointPair<T>>& point_pairs = EvalGeometryContactData(context).point_pairs;
    ...
    const geometry::SceneGraphInspector<T>& inspector = plant().EvalSceneGraphInspector(context);
    ...
    for (int point_pair_index = 0; point_pair_index < num_point_contacts; ++point_pair_index) {
        ...
        const T kA = GetPointContactStiffness(pair.id_A, default_contact_stiffness(), inspector);
        const T kB = GetPointContactStiffness(pair.id_B, default_contact_stiffness(), inspector);
        ...
        internal_tree().CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kV, body_A.body_frame(),
                                                          frame_W, p_WC, frame_W, frame_W, &Jv_WAc_W);
        internal_tree().CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kV, body_B.body_frame(),
                                                          frame_W, p_WC, frame_W, frame_W, &Jv_WBc_W);
        ...
        math::RotationMatrix<T> R_WC = math::RotationMatrix<T>::MakeFromOneVector(nhat_AB_W, 2);
        ...
        if (treeA_has_dofs) {
            Matrix3X<T> J = R_WC.matrix().transpose() *
                            Jv_AcBc_W.middleCols(tree_topology().tree_velocities_start_in_v(treeA_index),
        ...
        if ((treeB_has_dofs && !treeA_has_dofs) || (treeB_has_dofs && treeB_index != treeA_index)) {
            Matrix3X<T> J = R_WC.matrix().transpose() *
                            Jv_AcBc_W.middleCols(tree_topology().tree_velocities_start_in_v(treeB_index),
                                                 tree_topology().num_tree_velocities(treeB_index));
            jacobian_blocks.emplace_back(treeB_index, MatrixBlock<T>(std::move(J)));
        }

        // Contact stiffness and damping
        const T k = GetCombinedPointContactStiffness(pair.id_A, pair.id_B, default_contact_stiffness(), inspector);
        ...
        // models.
        const T d = GetCombinedHuntCrossleyDissipation(pair.id_A, pair.id_B, kA, kB, default_contact_dissipation(),
                                                       inspector);
        ...
        const T tau = GetCombinedDissipationTimeConstant(pair.id_A, pair.id_B, default_dissipation_time_constant,
                                                         body_A.name(), body_B.name(), inspector);
        const T mu = GetCombinedDynamicCoulombFriction(pair.id_A, pair.id_B, inspector);

        const T phi0 = -pair.depth;
        const T fn0 = k * pair.depth;

        // Contact point position relative to each body.
        const RigidTransform<T>& X_WA = plant().EvalBodyPoseInWorld(context, body_A);
        const Vector3<T>& p_WA = X_WA.translation();
        const Vector3<T> p_AC_W = p_WC - p_WA;
        const RigidTransform<T>& X_WB = plant().EvalBodyPoseInWorld(context, body_B);
...
void DiscreteUpdateManager<T>::AppendDiscreteContactPairsForHydroelasticContact(
    ...
    requires scalar_predicate<T>::is_bool
{  // NOLINT(whitespace/braces)
    const std::vector<geometry::ContactSurface<T>>& surfaces = EvalGeometryContactData(context).surfaces;
    ...
    for (int surface_index = 0; surface_index < num_surfaces; ++surface_index) {
        ...
        const T hydro_modulus_M = GetHydroelasticModulus(s.id_M(), std::numeric_limits<double>::infinity(), inspector);
        const T hydro_modulus_N = GetHydroelasticModulus(s.id_N(), std::numeric_limits<double>::infinity(), inspector);
        // Hunt & Crossley dissipation. Used by the Tamsi, Lagged, and Similar
        // contact models. Ignored by Sap. See
        // multibody::DiscreteContactApproximation for details about these contact
        // models.
        const T d = GetCombinedHuntCrossleyDissipation(s.id_M(), s.id_N(), hydro_modulus_M, hydro_modulus_N,
        ...
        // models.
        const double default_dissipation_time_constant = 0.1;
        const T tau = GetCombinedDissipationTimeConstant(s.id_M(), s.id_N(), default_dissipation_time_constant,
                                                         body_A.name(), body_B.name(), inspector);
        // Combine friction coefficients.
        const T mu = GetCombinedDynamicCoulombFriction(s.id_M(), s.id_N(), inspector);
        ...
        for (int face = 0; face < s.num_faces(); ++face) {
            ...
            if (Ae > 1.0e-14) {
                ...
                const T gM = M_is_compliant ? s.EvaluateGradE_M_W(face).dot(nhat_BA_W)
                                            : T(std::numeric_limits<double>::infinity());
                const T gN = N_is_compliant ? -s.EvaluateGradE_N_W(face).dot(nhat_BA_W)
                                            : T(std::numeric_limits<double>::infinity());
                ...
                if (treeA_has_dofs) {
                    Matrix3X<T> J = R_WC.matrix().transpose() *
                                    Jv_AcBc_W.middleCols(tree_topology().tree_velocities_start_in_v(tree_A_index),
                ...
                // Tree B contribution to contact Jacobian Jv_W_AcBc_C.
                // This contribution must be added only if B is different from A.
                if ((treeB_has_dofs && !treeA_has_dofs) || (treeB_has_dofs && tree_B_index != tree_A_index)) {
                    Matrix3X<T> J = R_WC.matrix().transpose() *
                                    Jv_AcBc_W.middleCols(tree_topology().tree_velocities_start_in_v(tree_B_index),
                ...
                const T p0 = s.is_triangle() ? s.tri_e_MN().Evaluate(face, tri_centroid_barycentric)
...
template <typename T>
const MultibodyForces<T>& DiscreteUpdateManager<T>::EvalDiscreteUpdateMultibodyForces(
        const systems::Context<T>& context) const {
    return plant()
...
template <typename T>
const VectorX<T>& DiscreteUpdateManager<T>::EvalActuation(const systems::Context<T>& context) const {
    return plant().get_cache_entry(cache_indexes_.actuation).template Eval<VectorX<T>>(context);
}

template <typename T>
const ContactResults<T>& DiscreteUpdateManager<T>::EvalContactResults(const systems::Context<T>& context) const {
    return plant().get_cache_entry(cache_indexes_.contact_results).template Eval<ContactResults<T>>(context);
}

template <typename T>
const DiscreteContactData<DiscreteContactPair<T>>& DiscreteUpdateManager<T>::EvalDiscreteContactPairs(
        const systems::Context<T>& context) const {
    return plant()
...
template <typename T>
void DiscreteUpdateManager<T>::DoCalcDiscreteValues(const systems::Context<T>& context,
                                                    systems::DiscreteValues<T>* updates) const {
    const ContactSolverResults<T>& results = EvalContactSolverResults(context);

    // Previous time step positions.
    const int nq = plant().num_positions();
    const VectorX<T>& x0 = context.get_discrete_state(multibody_state_index()).value();
...
void DiscreteUpdateManager<T>::ExtractModelInfo() {
    ...
    const std::vector<const multibody::PhysicalModel<T>*> physical_models = plant().physical_models();
...
template <typename T>
void DiscreteUpdateManager<T>::ExtractConcreteModel(const DeformableModel<T>* model) {
    if constexpr (std::is_same_v<T, double>) {
        DRAKE_DEMAND(model != nullptr);
        DRAKE_DEMAND(deformable_driver_ == nullptr);
        if (model->num_bodies() > 0) {
            deformable_driver_ = std::make_unique<DeformableDriver<double>>(model, this);
        }
    } else {
        if (!model->is_empty()) {
            throw std::logic_error(
