...
public:
    ...
    explicit HydroelasticTractionCalculator(double vslip_regularizer = 1e-6) : vslip_regularizer_(vslip_regularizer) {}
...
private:
    ...
    HydroelasticQuadraturePointData<T> CalcTractionAtPoint(
            const Data& data,
            int face_index,
            const typename geometry::TriangleSurfaceMesh<T>::template Barycentric<T>& Q_barycentric,
            double dissipation,
            double mu_coulomb) const;

    HydroelasticQuadraturePointData<T> CalcTractionAtCentroid(const Data& data,
                                                              int face_index,
                                                              double dissipation,
                                                              double mu_coulomb) const;

    HydroelasticQuadraturePointData<T> CalcTractionAtQHelper(const Data& data,
                                                             int face_index,
                                                             const T& e,
    ...
    multibody::SpatialForce<T> ComputeSpatialTractionAtAcFromTractionAtAq(const Data& data,
