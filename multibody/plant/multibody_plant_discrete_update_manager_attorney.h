...
private:
    ...
    static const MultibodyTree<T>& internal_tree(const MultibodyPlant<T>& plant) { return plant.internal_tree(); }

    static systems::CacheEntry& DeclareCacheEntry(MultibodyPlant<T>* plant,
                                                  std::string description,
                                                  systems::ValueProducer value_producer,
                                                  std::set<systems::DependencyTicket> prerequisites_of_calc) {
        return plant->DeclareCacheEntry(std::move(description), std::move(value_producer),
                                        std::move(prerequisites_of_calc));
    }

    static const GeometryContactData<T>& EvalGeometryContactData(const MultibodyPlant<T>& plant,
    ...
    static void AddAppliedExternalGeneralizedForces(const MultibodyPlant<T>& plant,
                                                    const drake::systems::Context<T>& context,
                                                    MultibodyForces<T>* forces) {
        plant.AddAppliedExternalGeneralizedForces(context, forces);
    }

    static void AddAppliedExternalSpatialForces(const MultibodyPlant<T>& plant,
                                                const drake::systems::Context<T>& context,
                                                MultibodyForces<T>* forces) {
    ...
    static void CalcForceElementsContribution(const MultibodyPlant<T>& plant,
                                              const drake::systems::Context<T>& context,
                                              MultibodyForces<T>* forces) {
        return plant.CalcForceElementsContribution(context, forces);
    }

    static VectorX<T> AssembleActuationInput(const MultibodyPlant<T>& plant, const systems::Context<T>& context) {
        return plant.AssembleActuationInput(context);
    }

    static VectorX<T> AssembleDesiredStateInput(const MultibodyPlant<T>& plant, const systems::Context<T>& context) {
    ...
    /* Returns the per-body arrays of collision geometries indexed by BodyIndex
     for the given `plant`. */
    static const std::vector<std::vector<geometry::GeometryId>>& collision_geometries(const MultibodyPlant<T>& plant) {
    ...
    static const std::unordered_map<geometry::GeometryId, BodyIndex>& geometry_id_to_body_index(
            const MultibodyPlant<T>& plant) {
        return plant.geometry_id_to_body_index_;
    }

    static const internal::JointLockingCacheData<T>& EvalJointLocking(const MultibodyPlant<T>& plant,
                                                                      const systems::Context<T>& context) {
        return plant.EvalJointLocking(context);
    }

    static const std::map<MultibodyConstraintId, internal::CouplerConstraintSpec>& coupler_constraints_specs(
            const MultibodyPlant<T>& plant) {
        return plant.coupler_constraints_specs_;
    }

    static const std::map<MultibodyConstraintId, internal::DistanceConstraintSpec>& distance_constraints_specs(
            const MultibodyPlant<T>& plant) {
        return plant.distance_constraints_specs_;
    }

    static const std::map<MultibodyConstraintId, internal::BallConstraintSpec>& ball_constraints_specs(
            const MultibodyPlant<T>& plant) {
        return plant.ball_constraints_specs_;
    }

    static const std::map<MultibodyConstraintId, internal::WeldConstraintSpec>& weld_constraints_specs(
            const MultibodyPlant<T>& plant) {
        return plant.weld_constraints_specs_;
    }

    static const std::map<MultibodyConstraintId, bool>& GetConstraintActiveStatus(const systems::Context<T>& context,
                                                                                  const MultibodyPlant<T>& plant) {
        return plant.GetConstraintActiveStatus(context);
    }

    static BodyIndex FindBodyByGeometryId(const MultibodyPlant<T>& plant, geometry::GeometryId geometry_id) {
