...
public:
    ...
    */
    Wing(BodyIndex body_index,
         double surface_area,
         const math::RigidTransform<double>& X_BodyWing = math::RigidTransform<double>::Identity(),
         double fluid_density = kDefaultFluidDensity);

    /** Scalar-converting copy constructor.  See @ref system_scalar_conversion. */
    template <typename U>
    explicit Wing(const Wing<U>& other)
        : Wing<T>(other.body_index_, other.surface_area_, other.X_BodyWing_, other.default_fluid_density_) {}
    ...
    MultibodyPlant. */
    const systems::InputPort<T>& get_body_poses_input_port() const { return this->get_input_port(0); }
    ...
    const systems::InputPort<T>& get_body_spatial_velocities_input_port() const { return this->get_input_port(1); }
    ...
    const systems::InputPort<T>& get_wind_velocity_input_port() const { return this->get_input_port(2); }
    ...
    /** Returns a reference to the optional fluid_density input port, which
     ...
     constructor. */
    const systems::InputPort<T>& get_fluid_density_input_port() const { return this->get_input_port(3); }
    ...
    const systems::OutputPort<T>& get_spatial_force_output_port() const { return this->get_output_port(0); }
    ...
    /** Returns a 3-element position of the aerodynamic center of the wing in
     ...
     field:
           ...
           aerodynamic_center
     */
    const systems::OutputPort<T>& get_aerodynamic_center_output_port() const { return this->get_output_port(1); }
...
private:
    ...
    void CalcSpatialForce(const systems::Context<T>& context,
                          std::vector<ExternallyAppliedSpatialForce<T>>* spatial_force) const;

    // Calculates the aerodynamic center output port.
    void CalcAerodynamicCenter(const systems::Context<T>& context, systems::BasicVector<T>* aerodynamic_center) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::Wing);
