...
struct PropellerInfo {
    ...
    explicit PropellerInfo(const BodyIndex& body_index_,
        ...
        : body_index(body_index_), X_BP(X_BP_), thrust_ratio(thrust_ratio_), moment_ratio(moment_ratio_) {}
...
public:
    ...
    Propeller(const BodyIndex& body_index,
              const math::RigidTransform<double>& X_BP = math::RigidTransform<double>::Identity(),
              double thrust_ratio = 1.0,
              double moment_ratio = 0.0);
    ...
    explicit Propeller(const Propeller<U>& other)
        : Propeller<T>(std::vector<PropellerInfo>(other.info_.begin(), other.info_.end())) {}
    ...
    const systems::InputPort<T>& get_command_input_port() const { return this->get_input_port(0); }
    ...
    MultibodyPlant. */
    const systems::InputPort<T>& get_body_poses_input_port() const { return this->get_input_port(1); }
    ...
    const systems::OutputPort<T>& get_spatial_forces_output_port() const { return this->get_output_port(0); }

private:
    // Calculates the spatial forces in the world frame as expected by the
    // applied_spatial_force input port of MultibodyPlant.
    void CalcSpatialForces(const systems::Context<T>& context,
