...
public:
    ...
    void DeclareSystemResources(internal::MultibodyTreeSystem<T>* tree_system) {
        ...
        if (plant == nullptr) {
            throw std::logic_error("The given MultibodyTreeSystem does not belong to a MultibodyPlant.");
        }
...
protected:
    ...
    explicit ForceDensityField(ForceDensityType density_type = ForceDensityType::kPerCurrentVolume)
...
public:
    ...
    */
    GravityForceField(const Vector3<T>& gravity_vector, const T& mass_density)
        : ForceDensityField<T>(ForceDensityType::kPerReferenceVolume), force_density_(mass_density * gravity_vector) {}

    ~GravityForceField() override;

private:
    Vector3<T> DoEvaluateAt(const systems::Context<T>&, const Vector3<T>&) const final { return force_density_; };
