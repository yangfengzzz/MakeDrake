...
private:
    ...
    static systems::LeafOutputPort<T>& DeclareAbstractOutputPort(
        ...
        DRAKE_DEMAND(plant != nullptr);
        return plant->DeclareAbstractOutputPort(std::move(name), std::move(alloc_function), std::move(calc_function),
    ...
    static systems::LeafOutputPort<T>& DeclareVectorOutputPort(
            ...
            std::set<systems::DependencyTicket> prerequisites_of_calc = {systems::System<T>::all_sources_ticket()}) {
        DRAKE_DEMAND(plant != nullptr);
        return plant->DeclareVectorOutputPort(std::move(name), model_vector, std::move(vector_calc_function),
                                              std::move(prerequisites_of_calc));
    }
};
}  // namespace internal
