...
class RigidBodyOnCompliantGround : public ::testing::TestWithParam<ContactTestConfig> {
public:
    ...
    void SetUp() override {
        ...
        // work with.
        plant_->mutable_gravity_field().set_gravity_vector(Vector3d(0.0, 0.0, -kGravity_));

        // Arbitrary inertia values.
        const double radius = 0.1;
        const SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(kMass_, radius);
        ...
        const auto& intermediate_body = plant_->AddRigidBody(
                "intermediate_body", SpatialInertia<double>(0.0, Vector3d::Zero(), UnitInertia<double>(1.0, 1.0, 1.0)));

        z_axis_ = &plant_->AddJoint<PrismaticJoint>("z_axis", plant_->world_body(), {}, intermediate_body, {},
                                                    Vector3d::UnitZ());
        x_axis_ = &plant_->AddJoint<PrismaticJoint>("x_axis", intermediate_body, {}, *body_, {}, Vector3d::UnitX());
        ...
        geometry::AddContactMaterial(0.0, 1e40, CoulombFriction<double>(kMu_, kMu_), &body_props);

        if (config.point_contact) {
            plant_->RegisterCollisionGeometry(*body_, RigidTransformd::Identity(),
                                              geometry::Sphere(kPointContactSphereRadius_), "point_contact_geometry",
                                              body_props);
        } else {
            const std::string mesh_file =
                    FindResourceOrThrow("drake/multibody/plant/test_utilities/square_surface.obj");
            geometry::AddRigidHydroelasticProperties(&body_props);
            plant_->RegisterCollisionGeometry(*body_, RigidTransformd::Identity(), geometry::Mesh(mesh_file),
                                              "hydroelastic_contact_geometry", body_props);
        }

        // Ground geometry.
        geometry::ProximityProperties ground_props;
        geometry::AddContactMaterial(kHcDissipation_, kStiffness_, CoulombFriction<double>(kMu_, kMu_), &ground_props);
        geometry::AddCompliantHydroelasticPropertiesForHalfSpace(kGroundThickness_, kHydroelasticModulus_,
                                                                 &ground_props);
...
protected:
    ...
    double CalcBodyWeight() const { return kMass_ * plant_->gravity_field().gravity_vector().norm(); }
    ...
    double CalcEquilibriumZPosition() const {
        ...
        if (config.point_contact) {
            ...
            const double stiffness = kArea_ * kHydroelasticModulus_ / kGroundThickness_;
    ...
    void ApplyTangentialForceForBodyInStiction() const {
        ...
        forces[0].F_Bq_W = SpatialForce<double>(Vector3d::Zero(), kTangentialStictionForce_);
        plant_->get_applied_spatial_force_input_port().FixValue(plant_context_, forces);
    }
    ...
    void ApplyTangentialForceForBodyInSlip() const {
        ...
        forces[0].body_index = body_->index();
        forces[0].p_BoBq_B = body_->CalcCenterOfMassInBodyFrame(*plant_context_);
        forces[0].F_Bq_W = SpatialForce<double>(Vector3d::Zero(), kTangentialSlipForce_ + kTangentialExtraForce_);
        plant_->get_applied_spatial_force_input_port().FixValue(plant_context_, forces);
    }
    ...
    const Vector3d kTangentialStictionForce_{kScale_ * kMu_ * kMass_ * kGravity_, 0.0, 0.0};
