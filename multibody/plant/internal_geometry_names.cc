#include "multibody/plant/internal_geometry_names.h"
...
template <typename T>
void ResetHelper(const MultibodyPlant<T>& plant,
    ...
    for (BodyIndex i{0}; i < num_bodies; ++i) {
        ...
        prototype.model_instance_name = plant.GetModelInstanceName(body.model_instance());
        prototype.body_name = body.name();
        prototype.body_name_is_unique_within_plant = (plant.NumBodiesWithName(body.name()) == 1);
        ...
        for (const auto& geometry_id : geometry_ids) {
            ...
            if (geometry_name.has_value()) {
                geometry_name = UndoGetScopedName(plant, body.model_instance(), *geometry_name);
            }
...
template <typename T>
void GeometryNames::ResetFull(const MultibodyPlant<T>& plant, const SceneGraphInspector<T>& inspector) {
    DRAKE_THROW_UNLESS(plant.is_finalized());
    std::function lookup = [&inspector](GeometryId id) -> std::optional<std::string_view> {
...
std::string GeometryNames::GetFullName(GeometryId id, std::string_view sep) const {
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&GeometryNames::ResetFull<T>));

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&GeometryNames::ResetBasic<T>));
