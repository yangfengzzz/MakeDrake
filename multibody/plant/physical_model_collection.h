...
public:
    ...
    DeformableModel<T>& AddDeformableModel(std::unique_ptr<DeformableModel<T>> model);
    ...
    /* (For testing only) Adds a DummyPhysicalModel to the collection and returns
     ...
     @throws std::exception if model is nullptr.
     @throws std::exception if model->plant() != plant(). */
    DummyPhysicalModel<T>& AddDummyModel(std::unique_ptr<DummyPhysicalModel<T>> model);

    /* Returns a pointer to the stored DeformableModel if one exists and nullptr
     otherwise. */
    const DeformableModel<T>* deformable_model() const { return deformable_model_; }
    ...
    /* Returns a pointer to the stored DummyPhysicalModel if one exists and
     nullptr otherwise. */
    const internal::DummyPhysicalModel<T>* dummy_model() const { return dummy_model_; }
    internal::DummyPhysicalModel<T>* mutable_dummy_model() { return dummy_model_; }

    const std::vector<std::unique_ptr<PhysicalModel<T>>>& owned_models() const { return owned_models_; }
    ...
    std::unique_ptr<PhysicalModelCollection<ScalarType>> CloneToScalar(MultibodyPlant<ScalarType>* new_plant) const {
        ...
        if (deformable_model_) {
            auto deformable_model_clone =
                    std::unique_ptr<DeformableModel<ScalarType>>(static_cast<DeformableModel<ScalarType>*>(
                            deformable_model_->template CloneToScalar<ScalarType>(new_plant).release()));
            clone->AddDeformableModel(std::move(deformable_model_clone));
        }
        if (dummy_model_) {
            auto dummy_model_clone =
                    std::unique_ptr<DummyPhysicalModel<ScalarType>>(static_cast<DummyPhysicalModel<ScalarType>*>(
                            dummy_model_->template CloneToScalar<ScalarType>(new_plant).release()));
