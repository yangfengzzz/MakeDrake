...
struct ContactProblemCache {
    ...
    explicit ContactProblemCache(double time_step) {
        sap_problem = std::make_unique<contact_solvers::internal::SapContactProblem<T>>(
        ...
        sap_problem_locked = std::make_unique<contact_solvers::internal::SapContactProblem<T>>(
                time_step, std::vector<MatrixX<T>>(), VectorX<T>());
    }
    copyable_unique_ptr<contact_solvers::internal::SapContactProblem<T>> sap_problem;
    ...
    copyable_unique_ptr<contact_solvers::internal::SapContactProblem<T>> sap_problem_locked;
...
public:
    ...
    explicit SapDriver(const CompliantContactManager<T>* manager, double near_rigid_threshold = 1.0);
    ...
    void CalcActuation(const systems::Context<T>& context, VectorX<T>* actuation) const;

    // Evaluates a cache entry storing the SapContactProblem to be solved at the
    // state stored in `context`.
    const ContactProblemCache<T>& EvalContactProblemCache(const systems::Context<T>& context) const;
...
private:
    ...
    const MultibodyTreeTopology& tree_topology() const { return manager().tree_topology(); }
    ...
    void CalcLinearDynamicsMatrix(const systems::Context<T>& context, std::vector<MatrixX<T>>* A) const;

    // Given the previous state x0 stored in `context`, this method computes the
    // "free motion" velocities, denoted v*.
    void CalcFreeMotionVelocities(const systems::Context<T>& context, VectorX<T>* v_star) const;
    ...
    std::vector<math::RotationMatrix<T>> AddContactConstraints(
            const systems::Context<T>& context, contact_solvers::internal::SapContactProblem<T>* problem) const;
    ...
    // MultibodyPlant.
    void AddDistanceConstraints(const systems::Context<T>& context,
                                contact_solvers::internal::SapContactProblem<T>* problem) const;
    ...
    void AddBallConstraints(const systems::Context<T>& context,
                            contact_solvers::internal::SapContactProblem<T>* problem) const;
    ...
    void AddWeldConstraints(const systems::Context<T>& context,
                            contact_solvers::internal::SapContactProblem<T>* problem) const;

    void AddPdControllerConstraints(const systems::Context<T>& context,
                                    contact_solvers::internal::SapContactProblem<T>* problem) const;

    // Adds holonomic constraints to model fixed constraints for deformable bodies
    // specified in the DeformableModel.
    void AddFixedConstraints(const systems::Context<T>& context,
                             contact_solvers::internal::SapContactProblem<T>* problem) const;
    ...
    void PackContactSolverResults(const systems::Context<T>& context,
    ...
    // `clique`.
    void AddCliqueContribution(const systems::Context<T>& context,
                               int clique,
    ...
    void CalcContactProblemCache(const systems::Context<T>& context, ContactProblemCache<T>* cache) const;
    ...
    // `sap_results`.
    void CalcSapSolverResults(const systems::Context<T>& context,
