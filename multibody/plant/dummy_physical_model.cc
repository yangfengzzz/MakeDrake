...
void DummyPhysicalModel<T>::DoDeclareSystemResources() {
    ...
    vector_output_port_ =
            &this->DeclareVectorOutputPort("dummy_vector_output_port", systems::BasicVector<T>(num_dofs_),
                                           [this](const systems::Context<T>& context, systems::BasicVector<T>* output) {
...
void DummyPhysicalModel<T>::DoDeclareSceneGraphPorts() {
    scene_graph_output_port_ =
            &this->DeclareVectorOutputPort("dummy_scene_graph_port", systems::BasicVector<T>(1),
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::DummyPhysicalModel);
