#include "multibody/plant/calc_distance_and_time_derivative.h"
...
SignedDistanceWithTimeDerivative CalcDistanceAndTimeDerivative(const MultibodyPlant<double>& plant,
    ...
    const auto& query_object = query_port.Eval<geometry::QueryObject<double>>(context);
    ...
    const Frame<double>& frameA = plant.GetBodyFromFrameId(frame_A_id)->body_frame();
    const Frame<double>& frameB = plant.GetBodyFromFrameId(frame_B_id)->body_frame();
    ...
    const Eigen::Vector3d p_ACa = inspector.GetPoseInFrame(signed_distance_pair.id_A) * p_GaCa;
    Eigen::Matrix<double, 3, Eigen::Dynamic> Jv_v_BCa_W(3, plant.num_velocities());
    plant.CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kV, frameA, p_ACa, frameB,
                                            plant.world_frame(), &Jv_v_BCa_W);
    ret.distance_time_derivative = signed_distance_pair.nhat_BA_W.dot(Jv_v_BCa_W * plant.GetVelocities(context));
    return ret;
}
}  // namespace multibody
}  // namespace drake
