...
public:
    ...
    /* The total number of contact data, including all of point, hydroelastic, and
     deformable contacts. */
    int size() const { return num_point_contacts() + num_hydro_contacts() + num_deformable_contacts(); }
    ...
    const std::vector<Data>& deformable_contact_data() const { return deformable_; }
    ...
    void AppendPointData(Data&& data) { point_.push_back(std::forward<Data>(data)); }
    void AppendHydroData(Data&& data) { hydro_.push_back(std::forward<Data>(data)); }
    void AppendDeformableData(Data&& data) { deformable_.push_back(std::forward<Data>(data)); }
    ...
    int deformable_contact_start() const { return num_point_contacts() + num_hydro_contacts(); }
