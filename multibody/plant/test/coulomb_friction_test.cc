...
GTEST_TEST(CoulombFriction, ConstructionFromFrictionCoefficients) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(CoulombFriction<double>(-kStaticFriction, kDynamicFriction),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(CoulombFriction<double>(kDynamicFriction, kStaticFriction),
...
GTEST_TEST(CoulombFriction, EqualityOperator) {
    // They are equal.
    EXPECT_TRUE(CoulombFriction<double>(0.8, 0.5) == CoulombFriction<double>(0.8, 0.5));
    // Static friction coefficient differs.
    EXPECT_FALSE(CoulombFriction<double>(0.8, 0.5) == CoulombFriction<double>(1.2, 0.5));
    // Dynamic friction coefficient differs.
    EXPECT_FALSE(CoulombFriction<double>(0.8, 0.5) == CoulombFriction<double>(0.8, 0.3));
    // Both friction coefficients differ.
    EXPECT_FALSE(CoulombFriction<double>(0.8, 0.5) == CoulombFriction<double>(1.2, 0.6));
}
...
GTEST_TEST(CoulombFriction, CalcContactFrictionFromSurfaceProperties) {
    ...
    // Verify correctness.
    CoulombFriction<double> friction1_with_friction2 = CalcContactFrictionFromSurfaceProperties(friction1, friction2);
    EXPECT_EQ(friction1_with_friction2.static_friction(), combine_friction_coefficients(mu_s1, mu_s2));
    EXPECT_EQ(friction1_with_friction2.dynamic_friction(), combine_friction_coefficients(mu_d1, mu_d2));

    // Verify the operation is commutative.
    CoulombFriction<double> friction2_with_friction1 = CalcContactFrictionFromSurfaceProperties(friction2, friction1);
    ...
    CoulombFriction<double> friction1_with_frictionless =
            CalcContactFrictionFromSurfaceProperties(friction1, CoulombFriction<double>());
    ...
    CoulombFriction<double> frictionless_with_friction1 =
            CalcContactFrictionFromSurfaceProperties(CoulombFriction<double>(), friction1);
...
GTEST_TEST(CoulombFriction, SurfacesAreIdentical) {
    ...
    CoulombFriction<double> surface1_with_surface2 = CalcContactFrictionFromSurfaceProperties(surface1, surface2);
    EXPECT_NEAR(surface1.static_friction(), surface1_with_surface2.static_friction(),
                5 * std::numeric_limits<double>::epsilon());
    EXPECT_NEAR(surface1.dynamic_friction(), surface1_with_surface2.dynamic_friction(),
                5 * std::numeric_limits<double>::epsilon());
}
...
GTEST_TEST(CoulombFriction, BothSurfacesAreFrictionless) {
    ...
    CoulombFriction<double> frictionless_with_frictionless =
            CalcContactFrictionFromSurfaceProperties(frictionless_surface1, frictionless_surface2);
