...
TEST_F(KukaIiwaModelTests, ExternalBodyForces) {
    ...
    const SpatialForce<double> F_Ep_E(Vector3<double>(1.0, 2.0, 3.0), Vector3<double>(-5.0, -4.0, -2.0));
    ...
    end_effector_link_->AddInForce(*context_, p_EP, F_Ep_E, end_effector_link_->body_frame(), &forces);
    const VectorX<double> tau_id = plant_->CalcInverseDynamics(*context_, vdot, forces);
    ...
    plant_->CalcJacobianSpatialVelocity(*context_, multibody::JacobianWrtVariable::kV, end_effector_link_->body_frame(),
                                        p_EP, frame_W, frame_W, &Jv_WEp);

    // Compute the expected value of inverse dynamics when external forcing is
    // considered.
    const math::RotationMatrix<double>& R_WE = end_effector_link_->EvalPoseInWorld(*context_).rotation();
    const SpatialForce<double> F_Ep_W = R_WE * F_Ep_E;
    const VectorX<double> tau_id_expected = M * vdot + C - Jv_WEp.transpose() * F_Ep_W.get_coeffs();
    ...
    EXPECT_TRUE(CompareMatrices(tau_id, tau_id_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, BodyForceApi) {
    ...
    EXPECT_TRUE(CompareMatrices(end_effector_link_->GetForceInWorld(*context_, forces).get_coeffs(), F_expected));
    // Test frame-specfic, and ensure we accumulate.
    end_effector_link_->AddInForce(*context_, Vector3<double>::Zero(), F_Bo_W, plant_->world_frame(), &forces);
    EXPECT_TRUE(CompareMatrices(end_effector_link_->GetForceInWorld(*context_, forces).get_coeffs(), 2 * F_expected));
}
