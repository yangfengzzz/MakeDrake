...
protected:
    ...
    void SetUp() override {
        ...
        deformable_body_id_ = RegisterDeformableOctahedron(&deformable_model, X_WD, 0.1, "deformable");
        model_ = &deformable_model;

        rigid_body_index_ =
                plant_->AddRigidBody("rigid_box", SpatialInertia<double>::SolidCubeWithMass(1.0, 0.2)).index();
        ...
        plant_->RegisterVisualGeometry(box_body, RigidTransformd::Identity(), box, "box_visual", illustration_props);
        ...
        deformable_model.AddFixedConstraint(deformable_body_id_, box_body, X_RD, box, RigidTransformd::Identity());
        ...
        plant_->RegisterVisualGeometry(plant_->world_body(), X_WG, box, "static_box_visual", illustration_props);
        ...
        deformable_model.AddFixedConstraint(deformable_body_id_, plant_->world_body(), X_SD, box,
...
private:
    ...
    DeformableBodyId RegisterDeformableOctahedron(DeformableModel<double>* model,
                                                  ...
                                                  double radius,
                                                  std::string name) {
        auto geometry = make_unique<GeometryInstance>(X_WD, make_unique<Sphere>(radius), std::move(name));
...
TEST_F(DeformableFixedConstraintTest, SteadyState) {
    ...
    Context<double>& mutable_plant_context = plant_->GetMyMutableContextFromRoot(&mutable_diagram_context);
    /* Initialize the dynamic rigid box close to its equilibrium pose. */
    const RigidTransformd X_WR(Vector3d(0, 0, -0.4));
    plant_->SetFreeBodyPose(&mutable_plant_context, plant_->get_body(rigid_body_index_), X_WR);
    ...
    VectorXd discrete_state =
            plant_context.get_discrete_state(model_->GetDiscreteStateIndex(deformable_body_id_)).value();
