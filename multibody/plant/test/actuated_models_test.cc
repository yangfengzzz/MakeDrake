...
public:
    static VectorXd AssembleActuationInput(const MultibodyPlant<double>& plant, const Context<double>& context) {
        return plant.AssembleActuationInput(context);
    }

    static VectorXd AssembleDesiredStateInput(const MultibodyPlant<double>& plant, const Context<double>& context) {
...
public:
    ...
    void SetUpModel(ModelConfiguration model_config = ModelConfiguration::kArmIsNotControlled,
                    const MultibodyPlantConfig& config = MultibodyPlantConfig{
                            .time_step = 0.01, .discrete_contact_approximation = "sap"}) {
        const char kArmSdfUrl[] = "package://drake_models/iiwa_description/sdf/iiwa7_no_collision.sdf";
        ...
        acrobot_model_ = parser.AddModelsFromUrl("package://drake/multibody/benchmarks/acrobot/acrobot.sdf").at(0);

        // Add the gripper.
        gripper_model_ = parser.AddModelsFromUrl(kWsg50SdfUrl).at(0);

        // A model of a (non-actuated) plate.
        box_model_ = parser.AddModelsFromUrl("package://drake/multibody/models/box.urdf").at(0);
        ...
        if (model_config != ModelConfiguration::kNoPdControl) {
            ...
            for (JointActuatorIndex actuator_index : plant_->GetJointActuatorIndices()) {
                if (plant_->get_joint_actuator(actuator_index).model_instance() == arm_model_) {
            ...
            if (model_config == ModelConfiguration::kArmIsControlled) {
                ...
                for (JointActuatorIndex actuator_index : arm_actuators) {
                    JointActuator<double>& actuator = plant_->get_mutable_joint_actuator(actuator_index);
        ...
        const Joint<double>& acrobot_shoulder = plant_->GetJointByName("ShoulderJoint", acrobot_model_);
        ...
        if (test_remove_joint_actuators_) {
            plant_->RemoveJointActuator(plant_->GetJointActuatorByName("iiwa_joint_3"));
    ...
    // {arm, acrobot, gripper}.
    std::tuple<VectorXd, VectorXd, VectorXd> MakeActuationForEachModel(bool iiwa_within_limits,
                                                                       bool gripper_within_limits = true) {
        ...
        if (test_remove_joint_actuators_) {
            ...
            const VectorXd arm_u = iiwa_within_limits ? (VectorXd(6) << 50, 40, -35, -40, -35, -40).finished()
                                                      : (VectorXd(6) << 200, 200, -160, -120, -45, -45).finished();
            const VectorXd gripper_u =
                    gripper_within_limits ? VectorXd::LinSpaced(2, 1.0, 2.0) : Eigen::Vector2d(90, 90);
            // ElbowJoint is removed from the model.
            const Vector1d acrobot_u(3.0);
            return std::make_tuple(arm_u, acrobot_u, gripper_u);
        } else {
            const VectorXd arm_u = iiwa_within_limits ? (VectorXd(7) << 50, 40, 55, -35, -40, -35, -40).finished()
    ...
    VectorXd AssembleFullModelActuation(const VectorXd& arm_u, const VectorXd& acrobot_u, const VectorXd& gripper_u) {
    ...
    void VerifyActuationOutputFeedsThroughActuationInputs() {
        auto [arm_u, acrobot_u, gripper_u] = MakeActuationForEachModel(false /* iiwa outside limits */);

        // Set arbitrary actuation values.
        plant_->get_actuation_input_port(arm_model_).FixValue(context_.get(), arm_u);
        plant_->get_actuation_input_port(gripper_model_).FixValue(context_.get(), gripper_u);
        plant_->get_actuation_input_port(acrobot_model_).FixValue(context_.get(), acrobot_u);
    ...
    void VerifyNetActuationOutputPorts(const VectorXd& arm_u,
        ...
        EXPECT_TRUE(CompareMatrices(plant_->get_net_actuation_output_port(arm_model_).Eval(*context_), arm_u, tolerance,
                                    MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(plant_->get_net_actuation_output_port(gripper_model_).Eval(*context_), gripper_u,
                                    tolerance, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(plant_->get_net_actuation_output_port(acrobot_model_).Eval(*context_), acrobot_u,
                                    tolerance, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(plant_->get_net_actuation_output_port().Eval(*context_), u, tolerance,
                                    MatrixCompareType::relative));
    }
...
TEST_F(ActuatedIiwaArmTest, JointActuatorApis) {
    SetUpModel();
    for (JointActuatorIndex actuator_index : plant_->GetJointActuatorIndices(gripper_model_)) {
...
TEST_F(ActuatedIiwaArmTest, GetActuationInputPort) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->get_actuation_input_port(invalid_index),
...
TEST_F(ActuatedIiwaArmTest, GetDesiredStatePort) {
    ...
    // Invalid model index throws.
    const ModelInstanceIndex invalid_index(plant_->num_model_instances() + 10);
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->get_desired_state_input_port(invalid_index),
...
TEST_F(ActuatedIiwaArmTest, AssembleActuationInput) {
    ...
    plant_->get_desired_state_input_port(gripper_model_).FixValue(context_.get(), gripper_xd);

    // Fix input input ports to known values.
    // We leave the arm's port disconnected to verify it's value defaults to zero
    // actuation.
    auto [arm_u, acrobot_u, gripper_u] = MakeActuationForEachModel(false /* irrelevant for this test */);
    plant_->get_actuation_input_port(gripper_model_).FixValue(context_.get(), gripper_u);
    plant_->get_actuation_input_port(acrobot_model_).FixValue(context_.get(), acrobot_u);

    const VectorXd full_u = MultibodyPlantTester::AssembleActuationInput(*plant_, *context_);

    const int arm_nu = plant_->num_actuated_dofs(arm_model_);
    VectorXd expected_u = AssembleFullModelActuation(VectorXd::Zero(arm_nu), acrobot_u, gripper_u);
...
TEST_F(ActuatedIiwaArmTest, AssembleDesiredStateInput_ThrowsIfPartiallyPDControlled) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MultibodyPlantTester::AssembleDesiredStateInput(*plant_, *context_),
                                "Model iiwa7 is partially PD controlled. .*");
}

TEST_F(ActuatedIiwaArmTest, AssembleDesiredStateInput_ThrowsIfDesiredStateNotConnected) {
    SetUpModel();
...
// model instance.
TEST_F(ActuatedIiwaArmTest, AssembleDesiredStateInput_VerifyAssemblyWithOneModel) {
    SetUpModel();

    const VectorXd gripper_xd = (VectorXd(4) << 1.0, 2.0, 3.0, 4.0).finished();
    plant_->get_desired_state_input_port(gripper_model_).FixValue(context_.get(), gripper_xd);
    const VectorXd full_xd = MultibodyPlantTester::AssembleDesiredStateInput(*plant_, *context_);
...
// model instances.
TEST_F(ActuatedIiwaArmTest, AssembleDesiredStateInput_VerifyAssemblyWithTwoModels) {
    ...
    plant_->get_desired_state_input_port(arm_model_).FixValue(context_.get(), arm_xd);
...
TEST_F(ActuatedIiwaArmTest, PdControlledActuatorsOnlySupportedForDiscreteModels) {
    DRAKE_EXPECT_THROWS_MESSAGE(
            SetUpModel(ModelConfiguration::kArmIsControlled, MultibodyPlantConfig{.time_step = 0.0}),
...
TEST_F(ActuatedIiwaArmTest, WithinEffortLimitsActuationMatchesAppliedGeneralizedForces) {
    ...
    const VectorXd gripper_x0 = (VectorXd(4) << gripper_q0, gripper_v0).finished();
    ...
    plant_->get_desired_state_input_port(gripper_model_).FixValue(context_.get(), gripper_x0);
    plant_->get_desired_state_input_port(arm_model_).FixValue(context_.get(), arm_x0);

    // N.B. These values of actuation are well within effort limits, for both arm
    // and gripper.
    auto [arm_u, acrobot_u, gripper_u] = MakeActuationForEachModel(true /* iiwa inside limits */);
    const VectorXd expected_u = AssembleFullModelActuation(arm_u, acrobot_u, gripper_u);
    ...
    plant_->get_applied_generalized_force_input_port().FixValue(context_.get(), tau);
    ...
    plant_->get_applied_generalized_force_input_port().FixValue(context_.get(), VectorXd::Zero(17));
    plant_->get_actuation_input_port(arm_model_).FixValue(context_.get(), arm_u);
    plant_->get_actuation_input_port(acrobot_model_).FixValue(context_.get(), acrobot_u);
    plant_->get_actuation_input_port(gripper_model_).FixValue(context_.get(), gripper_u);
    plant_->CalcForcedDiscreteVariableUpdate(*context_, updates.get());
    ...
    EXPECT_TRUE(CompareMatrices(x_actuation, x_tau, kTolerance, MatrixCompareType::relative));

    // Verify the net actuation values reported by the plant.
    VerifyNetActuationOutputPorts(arm_u, acrobot_u, gripper_u, expected_u, kTolerance);
}

// We verify that the PD controlled actuators exert effort limits.
TEST_F(ActuatedIiwaArmTest, OutsideEffortLimitsActuationMatchesAppliedGeneralizedForces) {
    // We add PD controllers but set their gains to zero since for this test we
    // are only interested on verifying that the effect of input actuation in the
    // dynamics is handled properly.
    SetUpModel(ModelConfiguration::kModelWithZeroGains);
    ...
    const VectorXd limits = (VectorXd(7) << 176, 176, 110, 110, 110, 40, 40).finished();
    const VectorXd arm_u_clamped = arm_u.array().min(limits.array()).max(-limits.array());
    const VectorXd expected_u = AssembleFullModelActuation(arm_u_clamped, acrobot_u, gripper_u);
    ...
    VerifyNetActuationOutputPorts(arm_u_clamped, acrobot_u, gripper_u, expected_u, kTolerance);
    ...
    const VectorXd arm_u_when_joint4_is_locked = (VectorXd(7) << 176, 176, 55, -160, -110, -40, -40).finished();
    const VectorXd expected_u_when_joint4_is_locked =
            AssembleFullModelActuation(arm_u_when_joint4_is_locked, acrobot_u, gripper_u);
    VerifyNetActuationOutputPorts(arm_u_when_joint4_is_locked, acrobot_u, gripper_u, expected_u_when_joint4_is_locked,
                                  kTolerance);
}
...
TEST_F(ActuatedIiwaArmTest, RemovedActuatorNetActuationPDController) {
    ...
    plant_->get_desired_state_input_port(gripper_model_).FixValue(context_.get(), VectorXd::Zero(4));
    plant_->get_desired_state_input_port(arm_model_).FixValue(context_.get(), VectorXd::Zero(12));

    auto [arm_u, acrobot_u, gripper_u] =
            MakeActuationForEachModel(false /* iiwa outside limits */, false /* gripper outside limits */);
    ...
    // ports).
    const VectorXd arm_limits = (VectorXd(6) << 176, 176, 110, 110, 40, 40).finished();
    const VectorXd arm_u_clamped = arm_u.array().min(arm_limits.array()).max(-arm_limits.array());
    ...
    const VectorXd gripper_u_clamped = gripper_u.array().min(gripper_limits.array()).max(-gripper_limits.array());

    const VectorXd expected_u = AssembleFullModelActuation(arm_u_clamped, acrobot_u, gripper_u_clamped);

    VerifyNetActuationOutputPorts(arm_u_clamped, acrobot_u, gripper_u_clamped, expected_u);
}

// This test verifies that for continuous models the actuation output port
// simply feeds through the actuation inputs.
TEST_F(ActuatedIiwaArmTest, ActuationOutputForContinuousModelsFeedsThroughActuationInput) {
    SetUpModel(ModelConfiguration::kNoPdControl, MultibodyPlantConfig{.time_step = 0.0});
...
TEST_F(ActuatedIiwaArmTest, ActuationOutputForDiscreteNonSapModelsFeedsThroughActuationInput) {
    SetUpModel(ModelConfiguration::kNoPdControl,
               MultibodyPlantConfig{.time_step = 0.01, .discrete_contact_approximation = "tamsi"});
    VerifyActuationOutputFeedsThroughActuationInputs();
}

// This test verifies that SAP models without PD controllers also feed through
// the actuation input to the actuation output.
TEST_F(ActuatedIiwaArmTest, ActuationOutputForDiscreteSapModelsFeedsThroughActuationInput) {
    SetUpModel(ModelConfiguration::kNoPdControl,
               MultibodyPlantConfig{.time_step = 0.01, .discrete_contact_approximation = "sap"});
    VerifyActuationOutputFeedsThroughActuationInputs();
}
...
TEST_F(ActuatedIiwaArmTest, DontRemoveJointActuator) {
    ...
    const JointActuator<double>& iiwa_joint_3_actuator = plant_->GetJointActuatorByName("iiwa_joint_3");
    EXPECT_EQ(plant_->GetJointActuatorByName("iiwa_joint_3", arm_model_).index(), iiwa_joint_3_actuator.index());
    ...
    const JointActuator<double>& shoulder_joint_actuator = plant_->GetJointActuatorByName("ShoulderJoint");
    const JointActuator<double>& elbow_joint_actuator = plant_->GetJointActuatorByName("ElbowJoint");
    EXPECT_THAT(plant_->GetJointActuatorIndices(acrobot_model_),
                testing::ElementsAre(elbow_joint_actuator.index(), shoulder_joint_actuator.index()));
}
...
TEST_F(ActuatedIiwaArmTest, RemoveJointActuator) {
    ...
    const JointActuator<double>& shoulder_joint_actuator = plant_->GetJointActuatorByName("ShoulderJoint");
    EXPECT_THAT(plant_->GetJointActuatorIndices(acrobot_model_), testing::ElementsAre(shoulder_joint_actuator.index()));
}
