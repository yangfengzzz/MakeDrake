...
ExternallyAppliedSpatialForce<double> MakeDummyForce(BodyIndex body_index) {
    return {.body_index = body_index, .p_BoBq_B = Eigen::Vector3d::Zero(), .F_Bq_W = SpatialForce<double>::Zero()};
}

class ExternallyAppliedSpatialForceMultiplexerTest : public ::testing::Test {
protected:
    void SetUp() override {
        system_ = std::make_unique<ExternallyAppliedSpatialForceMultiplexer<double>>(2);
...
void AssertEqual(const ExternallyAppliedSpatialForce<double>& lhs, const ExternallyAppliedSpatialForce<double>& rhs) {
...
TEST_F(ExternallyAppliedSpatialForceMultiplexerTest, ConcatenateTwoVectors) {
    ...
    const ListType expected = {MakeDummyForce(BodyIndex{0}), MakeDummyForce(BodyIndex{1}), MakeDummyForce(BodyIndex{1}),
