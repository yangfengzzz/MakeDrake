...
public:
    ...
    void MakeModel(bool anchor_bodyA, bool use_hard_constraint_defaults) {
        ...
        if (use_hard_constraint_defaults) {
            ...
            plant_.AddDistanceConstraint(*bodyA_, p_AP_, *bodyB_, p_BQ_, kDistance_, kStiffness_, kDissipation_);
        }
...
TEST_P(TwoBodiesTest, ConfirmConstraintProperties) {
    ...
    plant_.SetFreeBodyPoseInWorldFrame(context_.get(), *bodyB_, RigidTransformd(Vector3d(kDistanceAoBo, 0.0, 0.0)));
    ...
    const auto* constraint = dynamic_cast<const SapHolonomicConstraint<double>*>(&problem.get_constraint(0));
...
std::vector<TestConfig> MakeTestCases() {
    return std::vector<TestConfig>{
            {.description = "SingleClique", .bodyA_anchored = true, .hard_constraint = false},
            {.description = "TwoCliques", .bodyA_anchored = false, .hard_constraint = false},
            {.description = "SingleCliqueWithDefaultValues", .bodyA_anchored = true, .hard_constraint = true},
            {.description = "TwoCliquesWithDefaultValues", .bodyA_anchored = false, .hard_constraint = true},
    };
}

INSTANTIATE_TEST_SUITE_P(SapDistanceConstraintTests,
                         TwoBodiesTest,
...
GTEST_TEST(DistanceConstraintsTests, VerifyIdMapping) {
    ...
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    const RigidBody<double>& bodyB = plant.AddRigidBody("B", SpatialInertia<double>::NaN());
    const Vector3d p_AP(1, 2, 3);
    const Vector3d p_BQ(4, 5, 6);
    const double distance = 1.2;
    MultibodyConstraintId distance_id = plant.AddDistanceConstraint(bodyA, p_AP, bodyB, p_BQ, distance);
    const DistanceConstraintSpec& distance_spec = plant.get_distance_constraint_specs(distance_id);
    ...
    const std::map<MultibodyConstraintId, DistanceConstraintSpec>& distance_specs =
            plant.get_distance_constraint_specs();
    ASSERT_EQ(ssize(distance_specs), 1);

    const MultibodyConstraintId distance_id_from_map = distance_specs.begin()->first;
    const DistanceConstraintSpec& distance_spec_from_map = distance_specs.begin()->second;
...
GTEST_TEST(DistanceConstraintTests, FailOnTAMSI) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant.AddDistanceConstraint(bodyA, Vector3d{0, 0, 0}, bodyB, Vector3d{0, 0, 0}, 1 /* distance */),
...
GTEST_TEST(DistanceConstraintTests, FailOnInvalidSpecs) {
    ...
    // Fail on same body.
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant.AddDistanceConstraint(bodyA, Vector3d{0, 0, 0}, bodyA, Vector3d{0, 0, 0}, 1 /* distance */),
            ".*Invalid set of parameters for constraint between bodies 'A' and "
            "'A'.*");
    // Fail on distance <= 0.
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant.AddDistanceConstraint(bodyA, Vector3d{0, 0, 0}, bodyB, Vector3d{0, 0, 0}, 0 /* distance */),
            ".*Invalid set of parameters for constraint between bodies 'A' and "
            "'B'.*");
    // Fail on negative stiffness.
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddDistanceConstraint(bodyA, Vector3d{0, 0, 0}, bodyB, Vector3d{0, 0, 0},
                                                            1 /* distance */, -1 /* stiffness */),
                                ".*Invalid set of parameters for constraint between bodies 'A' and "
                                "'B'.*");
    // Fail on negative damping.
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddDistanceConstraint(bodyA, Vector3d{0, 0, 0}, bodyB, Vector3d{0, 0, 0},
                                                            1 /* distance */, 0 /* stiffness */, -1 /* damping */),
                                ".*Invalid set of parameters for constraint between bodies 'A' and "
                                "'B'.*");
}
