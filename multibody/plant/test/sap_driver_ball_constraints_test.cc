...
TEST_P(TwoBodiesTest, ConfirmConstraintProperties) {
    ...
    EXPECT_EQ(p.impulse_lower_limits(), Vector3d(-kInfinity, -kInfinity, -kInfinity));
    EXPECT_EQ(p.impulse_upper_limits(), Vector3d(kInfinity, kInfinity, kInfinity));
...
INSTANTIATE_TEST_SUITE_P(SapBallConstraintTests,
                         TwoBodiesTest,
...
GTEST_TEST(BallConstraintsTests, VerifyIdMapping) {
    ...
    MultibodyConstraintId ball_id = plant.AddBallConstraint(bodyA, p_AP, bodyB, p_BQ);
    const BallConstraintSpec& ball_spec = plant.get_ball_constraint_specs(ball_id);
    ...
    const std::map<MultibodyConstraintId, BallConstraintSpec>& ball_specs = plant.get_ball_constraint_specs();
...
GTEST_TEST(BallConstraintTests, FailOnTAMSI) {
    ...
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    const RigidBody<double>& bodyB = plant.AddRigidBody("B", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddBallConstraint(bodyA, Vector3d{0, 0, 0}, bodyB, Vector3d{0, 0, 0}),
...
GTEST_TEST(BallConstraintTests, FailOnSameBody) {
    ...
    plant.set_discrete_contact_approximation(DiscreteContactApproximation::kSap);
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddBallConstraint(bodyA, Vector3d{0, 0, 0}, bodyA, Vector3d{0, 0, 0}),
