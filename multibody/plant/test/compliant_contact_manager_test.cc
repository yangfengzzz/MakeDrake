...
#include "geometry/proximity/volume_mesh_field.h"
...
#include "multibody/plant/multibody_plant_config_functions.h"
#include "multibody/plant/sap_driver.h"
#include "multibody/plant/test/compliant_contact_manager_tester.h"
#include "multibody/plant/test/spheres_stack.h"
#include "multibody/plant/test_utilities/rigid_body_on_compliant_ground.h"
...
class SapDriverTest {
public:
    static const ContactProblemCache<double>& EvalContactProblemCache(const SapDriver<double>& driver,
                                                                      const Context<double>& context) {
        return driver.EvalContactProblemCache(context);
    }

    static void PackContactSolverResults(const Context<double>& context,
        ...
        driver.PackContactSolverResults(context, problem, num_contacts, sap_results, contact_results);
    }
};
...
GTEST_TEST(CompliantContactManagerTest, ExtractModelInfo) {
    ...
    auto geometry_instance = std::make_unique<geometry::GeometryInstance>(math::RigidTransformd::Identity(),
                                                                          geometry::Sphere(1.0), "sphere");
    deformable_model.RegisterDeformableBody(std::move(geometry_instance), fem::DeformableBodyConfig<double>{}, 10.0);
    plant.Finalize();
    auto contact_manager = std::make_unique<CompliantContactManager<double>>();
    const CompliantContactManager<double>* contact_manager_ptr = contact_manager.get();
...
public:
    ...
    void VerifyDiscreteContactPairs(const ContactParameters& sphere1_point_params,
        ...
        const ContactParameters hard_hydro_contact{std::nullopt, std::numeric_limits<double>::infinity(), 0.0, 1.0};
        const SphereParameters sphere1_params{"Sphere1", 10.0 /* mass */, 0.2 /* size */, sphere1_contact_params};
        const SphereParameters sphere2_params{"Sphere2", 10.0 /* mass */, 0.2 /* size */, sphere2_contact_params};

        // Soft sphere/hard ground.
        MakeModel(hard_hydro_contact, sphere1_params, sphere2_params);

        const GeometryContactData<double>& geometry_contact_data = EvalGeometryContactData(*plant_context_);
        const std::vector<PenetrationAsPointPair<double>>& point_pairs = geometry_contact_data.point_pairs;
        const std::vector<geometry::ContactSurface<double>>& surfaces = geometry_contact_data.surfaces;
        ...
        if (!sphere1_point_params.relaxation_time.has_value() || !sphere2_point_params.relaxation_time.has_value()) {
            EXPECT_NO_THROW(contact_manager_->EvalDiscreteContactPairs(*plant_context_));
            return;
        }

        // Verify that the manager throws an exception if a negative relaxation
        // times is provided.
        if (*sphere1_point_params.relaxation_time < 0 || *sphere2_point_params.relaxation_time < 0) {
            DRAKE_EXPECT_THROWS_MESSAGE(contact_manager_->EvalDiscreteContactPairs(*plant_context_),
        ...
        const GeometryId sphere2_geometry = plant_->GetCollisionGeometriesForBody(*sphere2_)[0];
        ...
        for (int i = 0; i < static_cast<int>(pairs.size()); ++i) {
            ...
            if (i < num_point_pairs) {
                ...
                EXPECT_NEAR(point_pair.stiffness, stiffness_expected, kEps * stiffness_expected);

                // Verify contact location.
                const double pz_WS1 = plant_->GetFreeBodyPose(*plant_context_, *sphere1_).translation().z();
                const double pz_WC = -k2 / (k1 + k2) * penetration_distance_ + pz_WS1 + sphere1_params.radius;
            ...
            const double tau2 = i == 0 ? *sphere2_contact_params.relaxation_time : *hard_hydro_contact.relaxation_time;
            const double tau_expected = tau1 + tau2;
            EXPECT_NEAR(point_pair.dissipation_time_scale, tau_expected, kEps * tau_expected);

            // Verify friction.
            const double mu1 = sphere1_contact_params.friction_coefficient;
            const double mu2 =
                    i == 0 ? sphere2_contact_params.friction_coefficient : hard_hydro_contact.friction_coefficient;
            const double mu_expected = 2.0 * (mu1 * mu2) / (mu1 + mu2);
            EXPECT_NEAR(point_pair.friction_coefficient, mu_expected, kEps * mu_expected);
        }
    }
    ...
    MatrixXd CalcDenseJacobianMatrixInWorldFrame(
            const DiscreteContactData<DiscreteContactPair<double>>& contact_pairs) const {
        const int nc = contact_pairs.size();
        const MatrixXd J_AcBc_C =
                CompliantContactManagerTester::CalcDenseJacobianMatrixInContactFrame(*contact_manager_, contact_pairs);
        ...
        for (int i = 0; i < nc; ++i) {
            ...
            J_AcBc_W.middleRows<3>(3 * i) = contact_pair.R_WC.matrix() * J_AcBc_C.middleRows<3>(3 * i);
        }
        return J_AcBc_W;
    }
    ...
    const GeometryContactData<double>& EvalGeometryContactData(const Context<double>& context) const {
        return CompliantContactManagerTester::EvalGeometryContactData(*contact_manager_, context);
    }

    const SapContactProblem<double>& EvalSapContactProblem(const Context<double>& context) const {
        const auto& sap_driver = CompliantContactManagerTester::sap_driver(*contact_manager_);
        return *SapDriverTest::EvalContactProblemCache(sap_driver, *plant_context_).sap_problem;
    }
};
...
TEST_F(SpheresStackTest, CalcDiscreteContactPairs) {
    ...
    const GeometryId sphere1_geometry = plant_->GetCollisionGeometriesForBody(*sphere1_)[0];
    ...
    // between sphere 1 and sphere 2.
    {
        ...
        EXPECT_TRUE(CompareMatrices(v_S1cS2c_W, expected_v_S1cS2c_W, kEps, MatrixCompareType::relative));

        // Verify configuration.
        const BodyIndex body_A_index = contact_manager_->geometry_id_to_body_index().at(contact_pairs[0].id_A);
        const BodyIndex body_B_index = contact_manager_->geometry_id_to_body_index().at(contact_pairs[0].id_B);
        EXPECT_EQ(BodyIndex(contact_pairs[0].object_A), body_A_index);
        EXPECT_EQ(BodyIndex(contact_pairs[0].object_B), body_B_index);
        const Vector3d p_AoC_W = body_A_index == sphere1_->index() ? p_S1C_W : p_S2C_W;
        const Vector3d p_BoC_W = body_B_index == sphere1_->index() ? p_S1C_W : p_S2C_W;
        EXPECT_TRUE(CompareMatrices(contact_pairs[0].p_ApC_W, p_AoC_W, kEps, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(contact_pairs[0].p_BqC_W, p_BoC_W, kEps, MatrixCompareType::relative));
    }
    ...
    // We know hydroelastic pairs come after point pairs.
    {
        ...
        for (int q = 1; q < contact_pairs.size(); ++q) {
            ...
            const MatrixXd J_WS1c_W = sign * J_AcBc_W.block(3 * q, 0, 3, plant_->num_velocities());
            const Vector3d v_WS1c_W = J_WS1c_W * v;
            EXPECT_TRUE(CompareMatrices(v_WS1c_W, expected_v_WS1c, kEps, MatrixCompareType::relative));

            // Verify configuration.
            const BodyIndex body_A_index = contact_manager_->geometry_id_to_body_index().at(contact_pairs[q].id_A);
            const BodyIndex body_B_index = contact_manager_->geometry_id_to_body_index().at(contact_pairs[q].id_B);
            EXPECT_EQ(BodyIndex(contact_pairs[q].object_A), body_A_index);
            EXPECT_EQ(BodyIndex(contact_pairs[q].object_B), body_B_index);
            const Vector3d p_AoC_W = body_A_index == sphere1_->index() ? p_S1C_W : p_WC;
            const Vector3d p_BoC_W = body_B_index == sphere1_->index() ? p_S1C_W : p_WC;
            EXPECT_TRUE(CompareMatrices(contact_pairs[q].p_ApC_W, p_AoC_W, kEps, MatrixCompareType::relative));
            EXPECT_TRUE(CompareMatrices(contact_pairs[q].p_BqC_W, p_BoC_W, kEps, MatrixCompareType::relative));
        }
    }
}
...
TEST_F(SpheresStackTest, RelaxationTimeIsNotRequired) {
    ContactParameters soft_point_contact{1.0e3, std::nullopt,
...
TEST_F(SpheresStackTest, RelaxationTimeMustBePositive) {
    ContactParameters soft_point_contact{1.0e3, std::nullopt, -1.0 /* Negative dissipation timescale */, 1.0};
    ContactParameters hard_point_contact{1.0e40, std::nullopt, 0.0, 1.0};
...
TEST_F(SpheresStackTest, VerifyDiscreteContactPairsFromRigidCompliantHydroelasticContact) {
    SetupRigidGroundCompliantSphereAndNonHydroSphere();
    ...
    const std::vector<geometry::ContactSurface<double>>& surfaces = EvalGeometryContactData(*plant_context_).surfaces;
...
TEST_F(SpheresStackTest, DoCalcContactSolverResults) {
    ...
    const SapContactProblem<double>& sap_problem = EvalSapContactProblem(*plant_context_);
    ...
    const SapSolverStatus status =
            sap.SolveWithGuess(sap_problem, plant_->GetVelocities(*plant_context_), &sap_results);
    ASSERT_EQ(status, SapSolverStatus::kSuccess);

    ContactSolverResults<double> contact_results_expected;
    const auto& sap_driver = CompliantContactManagerTester::sap_driver(*contact_manager_);
    SapDriverTest::PackContactSolverResults(*plant_context_, sap_driver, sap_problem, num_contacts, sap_results,
                                            &contact_results_expected);

    // Verify the expected result.
    EXPECT_TRUE(CompareMatrices(contact_results.v_next, contact_results_expected.v_next, kEps,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(contact_results.fn, contact_results_expected.fn, kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(contact_results.ft, contact_results_expected.ft, kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(contact_results.vn, contact_results_expected.vn, kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(contact_results.vt, contact_results_expected.vt, kEps, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(contact_results.tau_contact, contact_results_expected.tau_contact, kEps,
                                MatrixCompareType::relative));
}
...
TEST_F(SpheresStackTest, DoCalcDiscreteValues) {
    ...
    const Vector3d& p_WS10 = plant_->EvalBodyPoseInWorld(*plant_context_, *sphere1_).translation();
    const Vector3d& p_WS20 = plant_->EvalBodyPoseInWorld(*plant_context_, *sphere2_).translation();
    ...
    plant_->SetFreeBodyPoseInWorldFrame(next_context.get(), *sphere1_, RigidTransformd(p_WS1));
    plant_->SetFreeBodyPoseInWorldFrame(next_context.get(), *sphere2_, RigidTransformd(p_WS2));
    ...
    std::unique_ptr<systems::DiscreteValues<double>> updates = diagram_->AllocateDiscreteVariables();
    ...
    // Verify the result.
    EXPECT_TRUE(CompareMatrices(x_next, x_next_expected, kEps, MatrixCompareType::relative));
}
...
TEST_P(RigidBodyOnCompliantGround, VerifyContactResultsEquilibriumPosition) {
    ...
    const ContactResults<double>& contact_results = manager_->EvalContactResults(*plant_context_);
    const ContactTestConfig& config = GetParam();

    // Body should be in equilibrium so we expect the contact force to oppose
    // gravity.
    const Vector3d expected_contact_force = -kMass_ * plant_->gravity_field().gravity_vector();
    ...
    if (config.point_contact) {
        ...
        const PointPairContactInfo<double>& contact_info = contact_results.point_pair_contact_info(0);
        ...
        const double scale = (contact_info.bodyB_index() == body_->index() ? 1 : -1);

        EXPECT_TRUE(CompareMatrices(scale * contact_info.contact_force(), expected_contact_force, kTolerance));
        EXPECT_NEAR(contact_info.contact_point().z(), CalcEquilibriumZPosition() - kPointContactSphereRadius_,
                    kTolerance);
        ...
        const HydroelasticContactInfo<double>& contact_info = contact_results.hydroelastic_contact_info(0);
        ...
        BodyIndex body_A_index =
                CompliantContactManagerTester::FindBodyByGeometryId(*manager_, contact_info.contact_surface().id_M());
        const double scale = (body_A_index == body_->index() ? 1 : -1);
        const SpatialForce<double> F_Ac_W = scale * contact_info.F_Ac_W();

        EXPECT_TRUE(F_Ac_W.IsApprox(SpatialForce<double>(Vector3d::Zero(), expected_contact_force), kTolerance));

        const Vector3d f_Ac_W = F_Ac_W.translational();

        const std::vector<HydroelasticQuadraturePointData<double>>& quadrature_point_data =
                contact_info.quadrature_point_data();

        // Sanity check the face indices.
        EXPECT_EQ(quadrature_point_data.size(), 2);
        EXPECT_NE(quadrature_point_data[0].face_index, quadrature_point_data[1].face_index);

        for (const HydroelasticQuadraturePointData<double>& data : quadrature_point_data) {
            // Sanity check the face indices.
            EXPECT_THAT(std::vector<int>{data.face_index}, testing::IsSubsetOf({0, 1}));
...
TEST_P(RigidBodyOnCompliantGround, VerifyContactResultsBodyInStiction) {
    ...
    const Vector3d expected_contact_force =
            -(kTangentialStictionForce_ + kMass_ * plant_->gravity_field().gravity_vector());
    ...
    if (config.point_contact) {
        ...
        EXPECT_TRUE(CompareMatrices(F_Ac_W.translational(), expected_contact_force, kTolerance));
        EXPECT_TRUE(CompareMatrices(F_Ac_W.rotational(), Vector3d::Zero(), kTolerance));
    }
}
...
TEST_P(RigidBodyOnCompliantGround, VerifyContactResultsBodyInSlip) {
    ...
    const double kRelativeSlipTolerance = config.contact_solver == DiscreteContactSolver::kTamsi ? 2.0e-6 : 0.0;
    ...
    if (config.point_contact) {
        ...
        // Verify that the contact force lies exactly on the boundary of the
        // friction cone. i.e. |fₜ| = μ|fₙ|
        const double tolerance = kRelativeSlipTolerance * kMu_ * F_Ac_W.translational().z();
        EXPECT_NEAR(-F_Ac_W.translational().x(), kMu_ * F_Ac_W.translational().z(), tolerance);

        for (const HydroelasticQuadraturePointData<double>& data : contact_info.quadrature_point_data()) {
