...
public:
    explicit AcrobotGravityCompensator(const MultibodyPlant<double>* plant) : plant_(plant) {
        this->DeclareAbstractOutputPort("spatial_forces",
                                        &AcrobotGravityCompensator::CalcSpatialForceGravityCompensation);
    }

private:
    void CalcSpatialForceGravityCompensation(const Context<double>& context,
        ...
        const RigidBody<double>& link1 = dynamic_cast<const RigidBody<double>&>(plant_->get_body(BodyIndex(1)));
        const RigidBody<double>& link2 = dynamic_cast<const RigidBody<double>&>(plant_->get_body(BodyIndex(2)));
...
protected:
    void MakePlantWithGravityCompensator(double time_step) {
        ...
        Parser(plant_).AddModelsFromUrl("package://drake/multibody/benchmarks/acrobot/acrobot.sdf");
        plant_->Finalize();

        // Add the system that applies inverse gravitational forces to the link
        // endpoints.
        acrobot_gravity_compensator_ = builder.AddSystem<AcrobotGravityCompensator>(plant_);
        ...
        auto zero_source = builder.AddSystem<ConstantVectorSource<double>>(Vector1<double>(0));
        builder.Connect(zero_source->get_output_port(), plant_->get_actuation_input_port());
        diagram_ = builder.Build();

        // Create a context.
        context_ = diagram_->CreateDefaultContext();
        auto& acrobot_context = diagram_->GetMutableSubsystemContext(*plant_, context_.get());
...
TEST_F(ExternallyAppliedForcesTest, ContinuousPlant) {
    ...
    EXPECT_TRUE(CompareMatrices(derivatives_vector, VectorXd::Zero(4), eps, MatrixCompareType::absolute));
}
...
TEST_F(ExternallyAppliedForcesTest, DiscretePlant) {
    ...
    const double eps = 100 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(delta_x, VectorXd::Zero(4), eps, MatrixCompareType::absolute));
}
...
TEST_F(ExternallyAppliedForcesTest, CalcGeneralizedForcesDueToMultibodyForces) {
    ...
    const Context<double>& compensator_context = acrobot_gravity_compensator_->GetMyContextFromRoot(*context_);
    const auto& externally_applied_spatial_forces =
            acrobot_gravity_compensator_->get_output_port(0).Eval<std::vector<ExternallyAppliedSpatialForce<double>>>(
                    compensator_context);

    MultibodyForces<double> forces(*plant_);
    for (const ExternallyAppliedSpatialForce<double>& a_force : externally_applied_spatial_forces) {
    ...
    const VectorX<double> tau_external = (VectorX<double>(2) << 1.4, -3.14).finished();
    ...
    EXPECT_TRUE(
            CompareMatrices(generalized_forces, expected_generalized_forces, tolerance, MatrixCompareType::relative));
}
