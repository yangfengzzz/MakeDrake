...
public:
    ...
    static const internal::MultibodyTree<double>& internal_tree(const MultibodyPlant<double>& plant) {
...
public:
    ...
    void LoadBothModelsSetStateAndFinalize(const VectorX<double>& rotor_inertias, const VectorX<double>& gear_ratios) {
    ...
    void SetZeroActuation(const MultibodyPlant<double>& plant, Context<double>* context) const {
        const VectorX<double> tau_arm = VectorX<double>::Zero(plant.num_actuated_dofs(arm_model));
        const VectorX<double> tau_gripper = VectorX<double>::Zero(plant.num_actuated_dofs(gripper_model));
    ...
    VectorX<double> CalcVdotIncludingAdditionalDiagonalInertias(const MultibodyPlant<double>& plant,
                                                                const VectorX<double>& additional_diagonal_inertias,
                                                                const Context<double>& context) const {
        const internal::MultibodyTree<double>& tree = MultibodyPlantTester::internal_tree(plant);

        internal::ArticulatedBodyInertiaCache<double> abic(tree.get_topology());
        tree.CalcArticulatedBodyInertiaCache(context, additional_diagonal_inertias, &abic);
        internal::ArticulatedBodyForceCache<double> aba_force_cache(tree.get_topology());
        ...
        tree.CalcArticulatedBodyForceCache(context, abic, Zb_Bo_W, forces, &aba_force_cache);
...
VectorX<double> MapVectorFromActuatorIndicesToGeneralizedVelocityIndices(const MultibodyPlant<double>& plant,
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcMassMatrix) {
    ...
    const VectorX<double> reflected_inertia_with_generalized_velocity_indices =
            MapVectorFromActuatorIndicesToGeneralizedVelocityIndices(plant_, reflected_inertia_with_actuator_indices);
    ...
    MatrixX<double> mass_matrix_ri(plant_ri_.num_velocities(), plant_ri_.num_velocities());
    ...
    const double kTolerance =
            16.0 * std::numeric_limits<double>::epsilon() * mass_matrix.norm() / plant_.num_velocities();
    ...
    const MatrixX<double> Mdiagonal = reflected_inertia_with_generalized_velocity_indices.asDiagonal();
    EXPECT_TRUE(CompareMatrices(Mdiff, Mdiagonal, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcKineticEnergyAndMomentum) {
    ...
    const SpatialMomentum<double> L_W = plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, p_WoWo_W);
    const SpatialMomentum<double> L_W_ri = plant_ri_.CalcSpatialMomentumInWorldAboutPoint(*context_ri_, p_WoWo_W);
    EXPECT_TRUE(CompareMatrices(L_W.get_coeffs(), L_W_ri.get_coeffs(), kTolerance));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcInverseDynamics) {
    ...
    VectorX<double> tau_ri = plant_ri_.CalcInverseDynamics(*context_ri_, vdot, forces_ri);

    const double kTolerance = 16.0 * std::numeric_limits<double>::epsilon();

    VectorX<double> tau_diff = tau_ri - tau;
    EXPECT_TRUE(CompareMatrices(tau_diff, reflected_inertia_with_generalized_velocity_indices, kTolerance,
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcForwardAndInverseDynamics) {
    ...
    const VectorX<double> tau_arm = VectorX<double>::Random(plant_ri_.num_actuated_dofs(arm_model));
    const VectorX<double> tau_gripper = VectorX<double>::Random(plant_ri_.num_actuated_dofs(gripper_model));
    plant_ri_.get_actuation_input_port(arm_model).FixValue(context_ri_.get(), tau_arm);
    plant_ri_.get_actuation_input_port(gripper_model).FixValue(context_ri_.get(), tau_gripper);
    ...
    tau_expected = MapVectorFromActuatorIndicesToGeneralizedVelocityIndices(plant_, tau_expected);
    ...
    const VectorX<double> tau_inverse_dynamics = plant_ri_.CalcInverseDynamics(*context_ri_, vdot, forces);
    ...
    EXPECT_TRUE(CompareMatrices(tau_inverse_dynamics, tau_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcForwardDynamicsWithExtraDiagonalTerms) {
    // Arbitrary reflected inertia values.
    // clang-format off
  ...
  const VectorX<double> gear_ratios = (VectorX<double>(kNumJoints) <<
    ...
    const VectorX<double> reflected_inertia_with_actuator_indices =
            gear_ratios.array() * gear_ratios.array() * rotor_inertias.array();
    const VectorX<double> reflected_inertia = MapVectorFromActuatorIndicesToGeneralizedVelocityIndices(
    ...
    const VectorX<double> vdot_expected = plant_ri_.get_generalized_acceleration_output_port()
                                                  .Eval<systems::BasicVector<double>>(*context_ri_)
                                                  .CopyToVector();
    ...
    // file.
    const VectorX<double> vdot = CalcVdotIncludingAdditionalDiagonalInertias(plant_, reflected_inertia, *context_);

    // We expect the results to match to machine precision.
    const double tolerance = std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(vdot, vdot_expected, tolerance, MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, CalcForwardDynamics) {
    ...
    const VectorX<double> tau_arm = VectorX<double>::Zero(plant_ri_.num_actuated_dofs(arm_model));
    const VectorX<double> tau_gripper = VectorX<double>::Zero(plant_ri_.num_actuated_dofs(gripper_model));
    plant_ri_.get_actuation_input_port(arm_model).FixValue(context_ri_.get(), tau_arm);
    plant_ri_.get_actuation_input_port(gripper_model).FixValue(context_ri_.get(), tau_gripper);

    VectorX<double> vdot_ABA = plant_ri_.get_generalized_acceleration_output_port()
                                       .Eval<systems::BasicVector<double>>(*context_ri_)
                                       .CopyToVector();
    ...
    const VectorX<double> tau_etc = plant_ri_.CalcInverseDynamics(*context_ri_, zero_vdot, forces);
    ...
    const double kTolerance = mass_matrix_condition_number * std::numeric_limits<double>::epsilon();

    EXPECT_TRUE(CompareMatrices(vdot_ABA, vdot, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, ScalarConversion) {
    ...
    std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_autodiff = systems::System<double>::ToAutoDiffXd(plant_ri_);
    std::unique_ptr<Context<AutoDiffXd>> context_autodiff = plant_autodiff->CreateDefaultContext();
    context_autodiff->SetTimeStateAndParametersFrom(*context_ri_);

    // Compute the mass matrix of the scalar converted system.
    MatrixX<AutoDiffXd> M_autodiff(plant_autodiff->num_velocities(), plant_autodiff->num_velocities());
    plant_autodiff->CalcMassMatrix(*context_autodiff, &M_autodiff);

    // Verify that the mass matrices are equal.
    const double kTolerance = 16.0 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(mass_matrix_ri, M_autodiff, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(MultibodyPlantReflectedInertiaTests, DefaultParameters) {
    ...
    for (JointActuatorIndex index : plant_ri_.GetJointActuatorIndices()) {
        JointActuator<double>& joint_actuator =
                dynamic_cast<JointActuator<double>&>(plant_ri_.get_mutable_joint_actuator(index));

        // Default gear ratio and rotor inertia end up in the context.
        EXPECT_EQ(joint_actuator.default_gear_ratio(), joint_actuator.gear_ratio(*context_ri_));
        EXPECT_EQ(joint_actuator.default_rotor_inertia(), joint_actuator.rotor_inertia(*context_ri_));
    }
    ...
    for (JointActuatorIndex index : plant_ri_.GetJointActuatorIndices()) {
        ...
        EXPECT_EQ(joint_actuator.default_gear_ratio(), joint_actuator.gear_ratio(*context));
        EXPECT_EQ(joint_actuator.default_rotor_inertia(), joint_actuator.rotor_inertia(*context));
    }
}
