...
GTEST_TEST(EmptyMultibodyPlantCenterOfMassTest, CalcCenterOfMassPosition) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.CalcCenterOfMassPositionInWorld(*context_),
                                "CalcCenterOfMassPositionInWorld\\(\\): This MultibodyPlant "
                                "only contains the world_body\\(\\) so its center of mass is undefined.");

    DRAKE_EXPECT_THROWS_MESSAGE(plant.CalcCenterOfMassTranslationalVelocityInWorld(*context_),
...
public:
    void SetUp() override {
        ...
        plant_.AddRigidBody("Sphere1", sphere_instance_,
        ...
        plant_.AddRigidBody("Triangle1", triangle_instance_,
                            multibody::SpatialInertia<double>::MakeFromCentralInertia(
                                    mass_T_, p_TTcm_T_,
                                    // Set random inertia tensor, which does not affect the Center of
                                    // Mass position.
    ...
    void CheckCmPosition(const math::RigidTransform<double>& X_WS, const math::RigidTransform<double>& X_WT) {
        ...
        const Eigen::Vector3d p_WCcm_expected =
                ((p_WSo_W + R_WS * p_SScm_S_) * mass_S_ + (p_WTo_W + R_WT * p_TTcm_T_) * mass_T_) / (mass_S_ + mass_T_);
    ...
    const RigidBody<double>& GetSphereBody() { return plant_.GetRigidBodyByName("Sphere1"); }

    const RigidBody<double>& GetTriangleBody() { return plant_.GetRigidBodyByName("Triangle1"); }
    ...
    void CheckCmTranslationalVelocity(const SpatialVelocity<double>& V_WS_W, const SpatialVelocity<double>& V_WT_W) {
        ...
        const Vector3<double> v_WScm_W = plant_.CalcCenterOfMassTranslationalVelocityInWorld(*context_);
        ...
        const Vector3<double> mv_sphere = mass_sphere * sphere.CalcCenterOfMassTranslationalVelocityInWorld(*context_);
        const Vector3<double> mv_triangle =
                mass_triangle * triangle.CalcCenterOfMassTranslationalVelocityInWorld(*context_);
        const Vector3<double> v_WScm_W_expected = (mv_sphere + mv_triangle) / (mass_sphere + mass_triangle);
...
TEST_F(MultibodyPlantCenterOfMassTest, CalcTotalMass) {
    ...
    const ModelInstanceIndex world_model_instance = multibody::world_model_instance();
    std::vector<ModelInstanceIndex> world_model_instance_array;
    world_model_instance_array.push_back(world_model_instance);
    double mass_zero = plant_.CalcTotalMass(*context_, world_model_instance_array);
...
TEST_F(MultibodyPlantCenterOfMassTest, CenterOfMassPosition) {
    ...
    Eigen::Vector3d result = (X_WS0 * p_SScm_S_ * mass_S_ + X_WT0 * p_TTcm_T_ * mass_T_) / (mass_S_ + mass_T_);
    // Allow for 3 bits (2^3 = 8) of error.
    const double kTolerance = 8 * std::numeric_limits<double>::epsilon();
    ...
    V1 = SpatialVelocity<double>(Vector3<double>(1, 2, 3), Vector3<double>(4, 5, 6));
    V2 = SpatialVelocity<double>(Vector3<double>(3, 5, 7), Vector3<double>(5, 3, 1));
    CheckCmTranslationalVelocity(V1, V2);

    // Ensure center of mass methods throw an exception for empty model_instances.
    std::vector<ModelInstanceIndex> model_instances;
    DRAKE_EXPECT_THROWS_MESSAGE(plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances),
                                "CalcCenterOfMassPositionInWorld\\(\\): There must be at "
                                "least one non-world body contained in model_instances.");

    DRAKE_EXPECT_THROWS_MESSAGE(plant_.CalcCenterOfMassTranslationalVelocityInWorld(*context_, model_instances),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_.CalcJacobianCenterOfMassTranslationalVelocity(*context_, model_instances, JacobianWrtVariable::kV,
    ...
    world_model_instance_array.push_back(world_model_instance);
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_.CalcCenterOfMassTranslationalVelocityInWorld(*context_, world_model_instance_array),
            "CalcCenterOfMassTranslationalVelocityInWorld\\(\\): There must be at "
            "least one non-world body contained in model_instances.");
    ...
    // in model_instances.
    model_instances.push_back(sphere_instance_);
    result = ((p_WSo_W + p_SScm_S_) * mass_S_ + (p_WTo_W + p_TTcm_T_) * mass_T_) / (mass_S_ + mass_T_);
    p_WCcm = plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances);
    EXPECT_TRUE(CompareMatrices(p_WCcm, result, kTolerance));
    ...
    const math::RigidTransformd X_WS2(math::RollPitchYawd(0.3, -1.5, 0.7), Eigen::Vector3d(5.2, -3.1, 10.9));
    const math::RigidTransformd X_WT2(math::RollPitchYawd(-2.3, -3.5, 1.2), Eigen::Vector3d(-70.2, 9.8, 843.1));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant_.CalcJacobianCenterOfMassTranslationalVelocity(*context_, JacobianWrtVariable::kV,
                                                                                     frame_W, frame_W, &Js_v_WCcm_W),
                                "CalcJacobianCenterOfMassTranslationalVelocity\\(\\): The "
                                "system's total mass must be greater than zero.");

    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_.CalcBiasCenterOfMassTranslationalAcceleration(*context_, JacobianWrtVariable::kV, frame_W, frame_W),
    ...
    ModelInstanceIndex error_index(10);
    model_instances.push_back(error_index);
    EXPECT_THROW(plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances), std::exception);
    EXPECT_THROW(plant_.CalcCenterOfMassTranslationalVelocityInWorld(*context_, model_instances), std::exception);
    EXPECT_THROW(plant_.CalcJacobianCenterOfMassTranslationalVelocity(
                         *context_, model_instances, JacobianWrtVariable::kV, frame_W, frame_W, &Js_v_WCcm_W),
                 std::exception);
}
