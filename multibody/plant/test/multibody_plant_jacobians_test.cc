...
void CalcJacobianViaPartialDerivativesOfPositionWithRespectToQ(const MultibodyPlant<AutoDiffXd>& plant,
...
TEST_F(KukaIiwaModelTests, CalcJacobianTranslationalVelocityNonUnitQuaternion) {
    ...
    plant_->CalcJacobianTranslationalVelocity(*context_, JacobianWrtVariable::kQDot, frame_E, p_EoEi_E, frame_W,
                                              frame_W, &Jq_v_WEi_W);

    // Create shortcuts to end-effector link frame E and world frame W.
    const RigidBody<AutoDiffXd>& end_effector_autodiff = plant_autodiff_->get_body(end_effector_link_->index());
    const Frame<AutoDiffXd>& frame_E_autodiff = end_effector_autodiff.body_frame();

    // Form the partial derivatives of p_WoEi_W with respect to q,
    // evaluated at q's values.
    const VectorX<double> q_double = plant_->GetPositions(*context_);
    MatrixX<double> p_WoEi_W_deriv_wrt_q(3 * kNumPoints, num_positions);
    CalcJacobianViaPartialDerivativesOfPositionWithRespectToQ(*plant_autodiff_, context_autodiff_.get(), q_double,
                                                              frame_E_autodiff, p_EoEi_E, &p_WoEi_W_deriv_wrt_q);
    ...
    EXPECT_TRUE(CompareMatrices(Jq_v_WEi_W, p_WoEi_W_deriv_wrt_q, kTolerance, MatrixCompareType::relative));
    ...
    plant_->CalcJacobianPositionVector(*context_, frame_E, p_EoEi_E, frame_W, frame_W, &Jq_p_WoEi_W);
    EXPECT_TRUE(CompareMatrices(Jq_p_WoEi_W, Jq_v_WEi_W, kTolerance, MatrixCompareType::relative));

    // Verify the Jacobian Jq_p_WoEi_W matches the one from auto-differentiation.
    EXPECT_TRUE(CompareMatrices(Jq_p_WoEi_W, p_WoEi_W_deriv_wrt_q, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, CalcJacobianSpatialVelocity) {
    ...
    plant_->CalcJacobianSpatialVelocity(*context_, JacobianWrtVariable::kQDot, end_effector_frame, p_EoEp_E,
    ...
    VectorX<AutoDiffXd> qdot_autodiff = math::InitializeAutoDiff(VectorX<double>::Zero(kNumPositions));
    ...
    plant_autodiff_->SetPositions(context_autodiff_.get(), q_double.cast<AutoDiffXd>());
    // Set the velocity values in the context using q̇.
    plant_autodiff_->MapQDotToVelocity(*context_autodiff_, qdot_autodiff, &v_autodiff);
    plant_autodiff_->SetVelocities(context_autodiff_.get(), v_autodiff);

    // Compute V_WEp.
    const RigidBody<AutoDiffXd>& end_effector_link_autodiff = plant_autodiff_->get_body(end_effector_link_->index());
    const RotationMatrix<AutoDiffXd>& R_WE_autodiff =
            plant_autodiff_->EvalBodyPoseInWorld(*context_autodiff_, end_effector_link_autodiff).rotation();
    const Vector3<AutoDiffXd> p_EoEp_W = R_WE_autodiff * p_EoEp_E.cast<AutoDiffXd>();
    const SpatialVelocity<AutoDiffXd> V_WEp_autodiff =
            plant_autodiff_->EvalBodySpatialVelocityInWorld(*context_autodiff_, end_effector_link_autodiff)
                    .Shift(p_EoEp_W);

    // Extract the spatial Jacobian generated by automatic differentiation.
    MatrixX<double> Jq_V_WEp_autodiff = math::ExtractGradient(V_WEp_autodiff.get_coeffs());
    ...
    EXPECT_TRUE(CompareMatrices(Jq_V_WEp, Jq_V_WEp_autodiff, kTolerance, MatrixCompareType::relative));
    ...
    plant_->CalcJacobianAngularVelocity(*context_, JacobianWrtVariable::kQDot, end_effector_frame, world_frame,
                                        world_frame, &Jq_w_WE);

    Matrix3X<double> top_three_rows(3, num_generalized_positions);
    top_three_rows = Jq_V_WEp.template topRows<3>();
    EXPECT_TRUE(CompareMatrices(Jq_w_WE, top_three_rows, kTolerance, MatrixCompareType::relative));
    ...
    plant_->CalcJacobianTranslationalVelocity(*context_, JacobianWrtVariable::kQDot, end_effector_frame, p_EoEp_E,
                                              world_frame, world_frame, &Jq_v_WEp);
    Matrix3X<double> bottom_three_rows(3, num_generalized_positions);
    bottom_three_rows = Jq_V_WEp.template bottomRows<3>();
    EXPECT_TRUE(CompareMatrices(Jq_v_WEp, bottom_three_rows, kTolerance, MatrixCompareType::relative));
    ...
    plant_->CalcJacobianPositionVector(*context_, end_effector_frame, p_EoEp_E, world_frame, world_frame, &Jq_p_WoEp_W);
    EXPECT_TRUE(CompareMatrices(Jq_p_WoEp_W, Jq_v_WEp, kTolerance, MatrixCompareType::relative));
    ...
    CalcJacobianViaPartialDerivativesOfPositionWithRespectToQ(*plant_autodiff_, context_autodiff_.get(), q_double,
                                                              frame_E_autodiff, p_EoEp_E, &p_WoEp_W_deriv_wrt_q);

    // Verify the Jacobian Jq_p_WoEi_W matches the one from auto-differentiation.
    EXPECT_TRUE(CompareMatrices(Jq_p_WoEp_W, p_WoEp_W_deriv_wrt_q, kTolerance, MatrixCompareType::relative));
}
...
public:
    ...
    void SetUp() override {
        ...
        const SpatialInertia<double> M_Bcm = SpatialInertia<double>::SolidBoxWithMass(mass_link_, link_length_, 1, 1);
        ...
        joint1_ = &plant_->AddJoint<RevoluteJoint>("PinJoint1", plant_->world_body(), std::nullopt, *bodyA_,
        ...
        joint2_ = &plant_->AddJoint<RevoluteJoint>("PinJoint2", *bodyA_, math::RigidTransformd(p_AoFo_A), *bodyB_,
...
TEST_F(TwoDOFPlanarPendulumTest, CalcBiasAccelerations) {
    ...
    const SpatialAcceleration<double> aBias_WAp_W = plant_->CalcBiasSpatialAcceleration(
            *context_, JacobianWrtVariable::kV, frame_A, p_AoAp_A, frame_W, frame_W);

    // Simple by-hand analysis gives aBias_WAp_W = -L wAz_² Wx.
    const double wA_squared = wAz_ * wAz_;
    const Vector3<double> aBias_WAp_W_expected = -link_length_ * wA_squared * Vector3d::UnitX();
    EXPECT_TRUE(CompareMatrices(aBias_WAp_W.translational(), aBias_WAp_W_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_WAp_W.rotational(), Vector3d::Zero(), kTolerance));
    ...
    const SpatialAcceleration<double> aBias_WBp_W = plant_->CalcBiasSpatialAcceleration(
            *context_, JacobianWrtVariable::kV, frame_B, p_BoBp_B, frame_W, frame_W);
    ...
    const Vector3<double> aBias_WBp_W_expected =
            (-link_length_ * wA_squared + -link_length_ * wAB_squared) * Vector3d::UnitX();
    EXPECT_TRUE(CompareMatrices(aBias_WBp_W.translational(), aBias_WBp_W_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_WBp_W.rotational(), Vector3d::Zero(), kTolerance));
    ...
    const SpatialAcceleration<double> aBias_AAp_A = plant_->CalcBiasSpatialAcceleration(
            *context_, JacobianWrtVariable::kV, frame_A, p_AoAp_A, frame_A, frame_A);
    EXPECT_TRUE(CompareMatrices(aBias_AAp_A.translational(), Vector3d::Zero(), kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_AAp_A.rotational(), Vector3d::Zero(), kTolerance));

    // For point Bp of B, calculate Bp's bias spatial acceleration in frame A.
    const SpatialAcceleration<double> aBias_ABp_A = plant_->CalcBiasSpatialAcceleration(
            *context_, JacobianWrtVariable::kV, frame_B, p_BoBp_B, frame_A, frame_A);

    // By-hand analysis gives aBias_ABp_A = -L wBz_² Ax.
    const Vector3<double> aBias_ABp_A_expected = -link_length_ * wBz_ * wBz_ * Vector3d::UnitX();
    EXPECT_TRUE(CompareMatrices(aBias_ABp_A.translational(), aBias_ABp_A_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_ABp_A.rotational(), Vector3d::Zero(), kTolerance));
    ...
    Matrix3X<double> aBias_ABi_B = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV,
                                                                             frame_B, p_BoBi_B, frame_A, frame_B);
    ...
    const Vector3<double> aBias_AB0_B_expected(-0.5 * (link_length_ + 2 * x) * wB_squared, -y * wB_squared, 0);
    const Vector3<double> aBias_AB1_B_expected(-0.5 * (link_length_ + 4 * x) * wB_squared, -3 * y * wB_squared, 0);
    EXPECT_TRUE(CompareMatrices(aBias_ABi_B.col(0), aBias_AB0_B_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_ABi_B.col(1), aBias_AB1_B_expected, kTolerance));
}

TEST_F(TwoDOFPlanarPendulumTest, CalcJacobianVelocityAndBiasAccelerationEtcOfSystemCenterOfMass) {
    Eigen::VectorXd state = Eigen::Vector4d(0.0, 0.0, wAz_, wBz_);
    joint1_->set_angle(context_.get(), state[0]);
    joint2_->set_angle(context_.get(), state[1]);
    joint1_->set_angular_rate(context_.get(), state[2]);
    ...
    const Vector3d v_WBcm_W_expected(0, 0.5 * link_length_ * wBz_ + 1.5 * link_length_ * wAz_, 0);
    const Vector3d v_WAcm_W = body_A.CalcCenterOfMassTranslationalVelocityInWorld(*context_);
    const Vector3d v_WBcm_W = body_B.CalcCenterOfMassTranslationalVelocityInWorld(*context_);
    ...
    const Vector3d v_WScm_W = plant_->CalcCenterOfMassTranslationalVelocityInWorld(*context_);
    const Vector3d v_WScm_W_expected = (wAz_ * link_length_ + wBz_ * 0.25 * link_length_) * Vector3d::UnitY();
    EXPECT_TRUE(CompareMatrices(v_WScm_W, v_WScm_W_expected, kTolerance));
    const Vector3d v_WScm_W_alternate =
            (mass_link_ * v_WAcm_W_expected + mass_link_ * v_WBcm_W_expected) / (2 * mass_link_);
    ...
    plant_->CalcJacobianCenterOfMassTranslationalVelocity(*context_, JacobianWrtVariable::kV, frame_W, frame_W,
                                                          &Js_v_WScm_W);
    ...
    EXPECT_TRUE(CompareMatrices(Js_v_WScm_W * state.tail(num_velocities), v_WScm_W_expected, kTolerance));

    // Test CalcJacobianCenterOfMassTranslationalVelocity() for 1 model instance.
    std::vector<ModelInstanceIndex> model_instances;
    const ModelInstanceIndex bodyA_instance_index = plant_->GetModelInstanceByName("bodyAInstanceName");
    model_instances.push_back(bodyA_instance_index);
    Eigen::MatrixXd Js_v_WAcm_W(3, num_velocities);
    plant_->CalcJacobianCenterOfMassTranslationalVelocity(*context_, model_instances, JacobianWrtVariable::kV, frame_W,
                                                          frame_W, &Js_v_WAcm_W);
    ...
    EXPECT_TRUE(CompareMatrices(Js_v_WAcm_W * state.tail(num_velocities), v_WAcm_W_expected, kTolerance));
    ...
    const ModelInstanceIndex bodyB_instance_index = plant_->GetModelInstanceByName("bodyBInstanceName");
    ...
    const Vector3<double>& abias_WScm_W = plant_->CalcBiasCenterOfMassTranslationalAcceleration(
            *context_, JacobianWrtVariable::kV, plant_->world_frame(), plant_->world_frame());
    ...
    Vector3d abias_WScm_W_expected =
            -link_length_ * (wAz_ * wAz_ + 0.5 * wAz_ * wBz_ + 0.25 * wBz_ * wBz_) * Vector3d::UnitX();
...
public:
    ...
    void SetUp() override {
        ...
        const SpatialInertia<double> M_Acm =
                SpatialInertia<double>::SolidCylinderWithMass(mA, rA, LA, Vector3<double>::UnitZ());
        ...
        joint1_ = &plant_->AddJoint<RevoluteJoint>("PinJoint1", plant_->world_body(), math::RigidTransformd(p_WoAo_W),
        ...
        joint2_ = &plant_->AddJoint<RevoluteJoint>("PinJoint2", *bodyA_, math::RigidTransformd(p_AoBo_A), *bodyB_,
...
TEST_F(SatelliteTrackerTest, CalcBiasAccelerations) {
    ...
    const Vector3d aBias_WQ_B_expected =
            LB_ * Vector3d(-wBB - cosqB * cosqB * wAA, sinqB * cosqB * wAA, -2 * sinqB * wAB);
    ...
    const SpatialAcceleration<double> aBias_WQ_B =
            plant_->CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_B, p_BoQ_B, frame_W, frame_B);
    EXPECT_TRUE(CompareMatrices(aBias_WQ_B.rotational(), alphaBias_WB_B_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_WQ_B.translational(), aBias_WQ_B_expected, kTolerance));

    // Calculate Q's bias spatial acceleration in frame A, expressed in frame B.
    const Frame<double>& frame_A = bodyA_->body_frame();
    const SpatialAcceleration<double> aBias_AQ_B =
            plant_->CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_B, p_BoQ_B, frame_A, frame_B);
    EXPECT_TRUE(CompareMatrices(aBias_AQ_B.rotational(), alphaBias_AB_B_expected, kTolerance));
    EXPECT_TRUE(CompareMatrices(aBias_AQ_B.translational(), aBias_AQ_B_expected, kTolerance));
    ...
    Vector3<double> aBias_ABp_B = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV, frame_B,
                                                                            p_BoBp_B, frame_A, frame_B);
...
TEST_F(KukaIiwaModelTests, CalcBiasSpatialAcceleration) {
    ...
    const VectorX<double> vdot = VectorX<double>::Constant(num_velocities, std::numeric_limits<double>::quiet_NaN());
    ...
    plant_autodiff_->SetPositionsAndVelocities(context_autodiff_.get(), x_autodiff);
    ...
    const Frame<AutoDiffXd>& frame_E_autodiff = plant_autodiff_->get_body(end_effector_link_->index()).body_frame();
    ...
    plant_autodiff_->CalcJacobianSpatialVelocity(*context_autodiff_, JacobianWrtVariable::kV, frame_E_autodiff,
                                                 p_EEp_autodiff, frame_W_autodiff, frame_W_autodiff,
                                                 &Jv_V_WEp_autodiff);
    ...
    const SpatialAcceleration<double> AvBias_WEp_W =
            plant_->CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_E, p_EEp, frame_W, frame_W);
    ...
    EXPECT_TRUE(
            CompareMatrices(AvBias_WEp_W.get_coeffs(), AvBias_WEp_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, CalcBiasTranslationalAcceleration) {
    ...
    plant_autodiff_->CalcJacobianTranslationalVelocity(*context_autodiff_, JacobianWrtVariable::kV, frame_E_autodiff,
                                                       p_EEi_autodiff, frame_W_autodiff, frame_W_autodiff,
                                                       &Jv_v_WEi_autodiff);
    ...
    const Matrix3X<double> avBias_WEi_W = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV,
                                                                                    frame_E, p_EEi, frame_W, frame_W);
    ...
    EXPECT_TRUE(CompareMatrices(avBias_WEi_W, avBias_WEi_W_expected, kTolerance, MatrixCompareType::relative));
    ...
    const Matrix3X<double> avBias_WEi_E = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV,
                                                                                    frame_E, p_EEi, frame_W, frame_E);

    EXPECT_TRUE(CompareMatrices(avBias_WEi_E, avBias_WEi_E_expected, kTolerance, MatrixCompareType::relative));
    ...
    const Vector3<double> avBias_WEp_E = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV,
                                                                                   frame_E, p_EEp, frame_W, frame_E);
    EXPECT_TRUE(CompareMatrices(avBias_WEp_E, avBias_WEi_E_expected.col(0), kTolerance, MatrixCompareType::relative));
    ...
    const MatrixX<double> avBias_WEj_E = plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kV,
                                                                                   frame_E, p_EEi, frame_W, frame_E);
    EXPECT_TRUE(CompareMatrices(avBias_WEj_E, avBias_WEi_E_expected, kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_THROW(plant_->CalcBiasTranslationalAcceleration(*context_, JacobianWrtVariable::kQDot, frame_E, p_EEi,
                                                           frame_W, frame_W),
                 std::exception);
}
