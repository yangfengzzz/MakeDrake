...
public:
    void SetUp() override {
        ...
        body_ = &plant_.AddRigidBody("Lumpy", SpatialInertia<double>(body_mass_, p_BBcm_, G_BBo_));

        spring_damper_ = &plant_.AddForceElement<LinearSpringDamper>(plant_.world_body(), p_WP_, *body_, p_BQ_,
                                                                     free_length_, stiffness_, damping_);
    ...
    void SetGeneralPose() {
        const RigidTransform<double> X_WB(RollPitchYaw<double>(.5, -.2, 1.2), Eigen::Vector3d(10., 20., -9.));
    ...
    void SetGeneralVelocity() {
        ...
        V_WB << 7., 8., -9.,    // ω
                10., 11., 12.;  // v
        plant_.SetFreeBodySpatialVelocity(context_.get(), *body_, SpatialVelocity<double>(V_WB));
    }

    const RigidTransform<double>& get_X_WB() const { return body_->EvalPoseInWorld(*context_); }

    const SpatialVelocity<double>& get_V_WB() const { return body_->EvalSpatialVelocityInWorld(*context_); }

    const SpatialInertia<double>& get_M_B_W() const {
        const std::vector<SpatialInertia<double>>& M_Bi_W = plant_.EvalSpatialInertiaInWorldCache(*context_);
    ...
    double absolute_tolerance(double quantity) const { return kAccuracy * std::max(1., std::abs(quantity)); }
...
TEST_F(EnergyAndPowerTester, KineticEnergy) {
    ...
    EXPECT_NEAR(plant_.EvalKineticEnergy(*context_), expected_KE_W, absolute_tolerance(expected_KE_W));
}
...
TEST_F(EnergyAndPowerTester, PotentialEnergy) {
    ...
    EXPECT_NEAR(plant_.EvalPotentialEnergy(*context_), energy, absolute_tolerance(energy));
}
...
TEST_F(EnergyAndPowerTester, NonConservativePower) {
    ...
    EXPECT_NEAR(plant_.EvalNonConservativePower(*context_), damping_power, absolute_tolerance(damping_power));
}
...
TEST_F(EnergyAndPowerTester, ConservativePower) {
    ...
    EXPECT_NEAR(plant_.EvalConservativePower(*context_), power, absolute_tolerance(power));
}
