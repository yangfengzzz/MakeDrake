...
const char kIiwaUrl[] = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
...
GTEST_TEST(JointLimitsTest, PrismaticJointConvergenceTest) {
    ...
    for (double time_step : {2.5e-4, 5.0e-4, 1.0e-3}) {
        ...
        const SpatialInertia<double> M_B = SpatialInertia<double>::SolidCubeWithMass(mass, box_size);
        const RigidBody<double>& body = plant.AddRigidBody("Body", M_B);
        const PrismaticJoint<double>& slider = plant.AddJoint<PrismaticJoint>(
                "Slider", plant.world_body(), std::nullopt, body, std::nullopt, Vector3<double>::UnitZ(),
        ...
        EXPECT_NEAR(slider.get_translation(context), slider.position_lower_limit(), expected_error);
        ...
        EXPECT_NEAR(slider.get_translation(context), slider.position_upper_limit(), expected_error);
        EXPECT_NEAR(slider.get_translation_rate(context), 0.0, kVelocityTolerance);
    }
}
...
GTEST_TEST(JointLimitsTest, RevoluteJoint) {
    ...
    for (double time_step : {2.5e-4, 5.0e-4, 1.0e-3}) {
        ...
        const auto M_B = SpatialInertia<double>::SolidCylinderWithMassAboutEnd(mass, rod_radius, rod_length,
                                                                               Vector3<double>::UnitX());
        const RigidBody<double>& body = plant.AddRigidBody("Body", M_B);
        const RevoluteJoint<double>& pin = plant.AddJoint<RevoluteJoint>(
                "Pin", plant.world_body(), std::nullopt, body, std::nullopt, Vector3<double>::UnitZ(),
                -M_PI / 5.0 /* lower limit */, M_PI / 3.0 /* upper limit */, 0.0 /* damping */);
        ...
        EXPECT_NEAR(pin.get_angle(context), pin.position_upper_limit(), expected_error);
        ...
        EXPECT_NEAR(pin.get_angle(context), pin.position_lower_limit(), expected_error);
        EXPECT_NEAR(pin.get_angular_rate(context), 0.0, kVelocityTolerance);
    }
}
...
VectorX<double> KukaPositionLowerLimits() {
    ...
    lower_limits << -2.96705972839, -2.09439510239, -2.96705972839, -2.09439510239, -2.96705972839, -2.09439510239,
            -3.05432619099;
    return lower_limits;
}
...
GTEST_TEST(JointLimitsTest, KukaArm) {
    ...
    EXPECT_TRUE(CompareMatrices(lower_limits_expected, plant.GetPositionLowerLimits()));
    EXPECT_TRUE(CompareMatrices(upper_limits_expected, plant.GetPositionUpperLimits()));
    ...
    for (int joint_number = 1; joint_number <= nq; ++joint_number) {
        ...
        EXPECT_NEAR(joint.position_lower_limit(), lower_limits_expected(joint_number - 1),
                    std::numeric_limits<double>::epsilon());
        EXPECT_NEAR(joint.position_upper_limit(), upper_limits_expected(joint_number - 1),
                    std::numeric_limits<double>::epsilon());
    }
    ...
    plant.get_actuation_input_port().FixValue(&context, VectorX<double>::Constant(nq, 0.4));
    simulator.Initialize();
    simulator.AdvanceTo(simulation_time);
    ...
    for (int joint_number = 1; joint_number <= nq; ++joint_number) {
        const std::string joint_name = "iiwa_joint_" + std::to_string(joint_number);
        const auto& joint = plant.GetJointByName<RevoluteJoint>(joint_name);
        EXPECT_LT(std::abs((joint.get_angle(context) - joint.position_upper_limit()) / joint.position_upper_limit()),
    ...
    for (int joint_number = 1; joint_number <= nq; ++joint_number) {
        const std::string joint_name = "iiwa_joint_" + std::to_string(joint_number);
        const auto& joint = plant.GetJointByName<RevoluteJoint>(joint_name);
        EXPECT_LT(std::abs((joint.get_angle(context) - joint.position_lower_limit()) / joint.position_lower_limit()),
                  kRelativePositionTolerance);
        EXPECT_NEAR(joint.get_angular_rate(context), 0.0, kVelocityTolerance);
    }
}
...
GTEST_TEST(JointLimitsTest, ContinuousLimitsDoNotFault) {
    ...
    plant.get_actuation_input_port().FixValue(context.get(), Eigen::VectorXd::Zero(7));
