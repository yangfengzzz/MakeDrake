...
#include "multibody/plant/discrete_contact_data.h"
...
#include "multibody/test_utilities/robot_model.h"
...
TYPED_TEST_P(MultibodyPlantDefaultScalarsTest, RevoluteJointAndSpring) {
    ...
    const RevoluteJoint<double>& pin = plant.AddJoint<RevoluteJoint>("Pin", plant.world_body(), std::nullopt, body,
    ...
    const auto& pin_u = plant_u->template GetJointByName<RevoluteJoint>(pin.name());
    const auto& spring_u = plant_u->template GetForceElement<RevoluteSpring>(spring.index());
...
void CompareMultibodyPlantPortIndices(const MultibodyPlant<T>& plant_t, const MultibodyPlant<U>& plant_u) {
    // Check input ports.
    ...
    EXPECT_EQ(plant_t.get_geometry_query_input_port().get_index(), plant_u.get_geometry_query_input_port().get_index());
    // Check output ports.
    EXPECT_EQ(plant_t.get_body_poses_output_port().get_index(), plant_u.get_body_poses_output_port().get_index());
    ...
    EXPECT_EQ(plant_t.get_state_output_port().get_index(), plant_u.get_state_output_port().get_index());
    ...
    EXPECT_EQ(plant_t.get_geometry_pose_output_port().get_index(), plant_u.get_geometry_pose_output_port().get_index());
    EXPECT_EQ(plant_t.get_deformable_body_configuration_output_port().get_index(),
              plant_u.get_deformable_body_configuration_output_port().get_index());
    EXPECT_EQ(plant_t.get_state_output_port(default_model_instance()).get_index(),
              plant_u.get_state_output_port(default_model_instance()).get_index());
    EXPECT_EQ(plant_t.get_generalized_acceleration_output_port(default_model_instance()).get_index(),
              plant_u.get_generalized_acceleration_output_port(default_model_instance()).get_index());
    EXPECT_EQ(plant_t.get_generalized_contact_forces_output_port(default_model_instance()).get_index(),
              plant_u.get_generalized_contact_forces_output_port(default_model_instance()).get_index());
}
...
TYPED_TEST_P(MultibodyPlantDefaultScalarsTest, PortIndexOrdering) {
    ...
    std::unique_ptr<Diagram<U>> diagram_u = System<double>::ToScalarType<U>(*diagram);
    const auto& plant_u = dynamic_cast<const MultibodyPlant<U>&>(*diagram_u->GetSystems().at(0));
...
REGISTER_TYPED_TEST_SUITE_P(MultibodyPlantDefaultScalarsTest, RevoluteJointAndSpring, PortIndexOrdering, DirectlyAdded);

using NonDoubleScalarTypes = ::testing::Types<AutoDiffXd, Expression>;
INSTANTIATE_TYPED_TEST_SUITE_P(My, MultibodyPlantDefaultScalarsTest, NonDoubleScalarTypes);

// A simple concrete DiscreteUpdateManager that does not support scalar
// conversion to either AutoDiffXd or Expression.
template <typename T>
class DoubleOnlyDiscreteUpdateManager final : public multibody::internal::DiscreteUpdateManager<T> {
public:
...
private:
    std::unique_ptr<multibody::internal::DiscreteUpdateManager<double>> CloneToDouble() const final {
    ...
    void DoCalcDiscreteValues(const systems::Context<T>&, systems::DiscreteValues<T>*) const final {}
...
GTEST_TEST(ScalarConversionTest, ExternalComponent) {
    ...
    EXPECT_NO_THROW(plant_autodiff_to_symbolic = System<AutoDiffXd>::ToSymbolic(*plant_autodiff));
    ...
    auto discrete_update_manager = std::make_unique<DoubleOnlyDiscreteUpdateManager<AutoDiffXd>>();
...
public:
    template <typename T>
    static std::map<MultibodyConstraintId, internal::CouplerConstraintSpec>& get_mutable_specs(
...
GTEST_TEST(ScalarConversionTest, CouplerConstraintSpec) {
    ...
    const internal::CouplerConstraintSpec reference_spec{j0, j1, kGearRatio, kOffset};

    // Directly add dummy constraint specs through the tester so that we don't
    // need to actually add any joints.
    MultibodyPlantTester::get_mutable_specs(&plant_double)[MultibodyConstraintId::get_new_id()] = reference_spec;
    plant_double.Finalize();
    // double -> AutoDiffXd.
    std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_double_to_autodiff = System<double>::ToAutoDiffXd(plant_double);
    EXPECT_EQ(plant_double_to_autodiff->num_constraints(), plant_double.num_constraints());
    // AutoDiffXd -> double.
    ...
    EXPECT_EQ(plant_autodiff_to_double->num_constraints(), plant_double.num_constraints());
}

template <typename T>
class DiscretePlantTest
    : public ::testing::TestWithParam<
              std::tuple<DiscreteContactApproximation, ContactModel, RobotModelConfig::ContactConfig>> {
public:
    void SetUp() override {
        const auto [contact_approximation, contact_model, contact_configuration] = GetParam();
        const RobotModelConfig robot_config{contact_configuration, contact_approximation, contact_model};
...
template <typename T>
std::string ParamInfoToString(const testing::TestParamInfo<typename DiscretePlantTest<T>::ParamType>& param_info) {
    const auto [contact_approximation, contact_model, contact_configuration] = param_info.param;
    const RobotModelConfig robot_config{contact_configuration, contact_approximation, contact_model};
    std::stringstream s;
    s << robot_config;
    return s.str();
...
auto MakeAllPermutations() {
    return ::testing::Combine(
            ::testing::Values(DiscreteContactApproximation::kSimilar, DiscreteContactApproximation::kTamsi),
            ::testing::Values(ContactModel::kPoint, ContactModel::kHydroelasticWithFallback),
...
INSTANTIATE_TEST_SUITE_P(SupportMatrixTests, DiscretePlantTestDouble, MakeAllPermutations(), ParamInfoToString<double>);
...
TEST_P(DiscretePlantTestDouble, ForcedUpdate) {
    ...
    EXPECT_NO_THROW(diagram.CalcForcedDiscreteVariableUpdate(model_->context(), updates.get()));
}

INSTANTIATE_TEST_SUITE_P(SupportMatrixTests,
                         DiscretePlantTestAutoDiff,
                         MakeAllPermutations(),
...
TEST_P(DiscretePlantTestExpression, ForcedUpdate) {
    ...
    // - not supported
    // - due to Expression
    DRAKE_EXPECT_THROWS_MESSAGE(diagram.CalcForcedDiscreteVariableUpdate(model_->context(), updates.get()),
