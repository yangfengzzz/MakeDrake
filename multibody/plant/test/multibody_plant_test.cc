...
#include "geometry/geometry_frame.h"
#include "geometry/geometry_roles.h"
#include "geometry/proximity_properties.h"
#include "geometry/query_object.h"
#include "geometry/scene_graph.h"
#include "geometry/test_utilities/dummy_render_engine.h"
#include "geometry/test_utilities/geometry_set_tester.h"
#include "math/autodiff_gradient.h"
#include "math/rigid_transform.h"
#include "math/roll_pitch_yaw.h"
#include "math/rotation_matrix.h"
#include "multibody/benchmarks/acrobot/acrobot.h"
...
#include "multibody/plant/discrete_contact_pair.h"
#include "multibody/plant/externally_applied_spatial_force.h"
#include "multibody/plant/test_utilities/multibody_plant_remodeling.h"
#include "multibody/test_utilities/add_fixed_objects_to_plant.h"
#include "multibody/tree/door_hinge.h"
#include "multibody/tree/planar_joint.h"
#include "multibody/tree/prismatic_joint.h"
#include "multibody/tree/prismatic_spring.h"
#include "multibody/tree/quaternion_floating_joint.h"
#include "multibody/tree/revolute_joint.h"
#include "multibody/tree/revolute_spring.h"
#include "multibody/tree/rigid_body.h"
...
#include "systems/primitives/constant_vector_source.h"
#include "systems/primitives/linear_system.h"
...
public:
    ...
    template <typename T>
    static BodyIndex FindBodyByGeometryId(const MultibodyPlant<T>& plant, GeometryId id) {
...
GTEST_TEST(MultibodyPlant, GetDefaultContactSurfaceRepresentation) {
    ...
    for (const auto& time_step : time_steps) {
        ...
        EXPECT_EQ(plant.get_contact_surface_representation(),
                  MultibodyPlant<double>::GetDefaultContactSurfaceRepresentation(time_step));
    }
}
...
GTEST_TEST(MultibodyPlant, SimpleModelCreation) {
    ...
    unique_ptr<MultibodyPlant<double>> plant = MakeAcrobotPlant(parameters, false /* Don't make a finalized plant. */);
    ...
    const ModelInstanceIndex pendulum_model_instance =
            Parser(plant.get()).AddModelsFromUrl("package://drake/multibody/plant/test/split_pendulum.sdf").at(0);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant->set_penetration_allowance(-1),
                                "set_penetration_allowance\\(\\): penetration_allowance must be strictly "
                                "positive.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->set_penetration_allowance(0),
                                "set_penetration_allowance\\(\\): penetration_allowance must be strictly "
                                "positive.");
    ...
    EXPECT_EQ(plant->get_actuation_input_port(default_model_instance()).size(), 1);
    ...
    EXPECT_EQ(plant->get_actuation_input_port(default_model_instance()).get_name(), "DefaultModelInstance_actuation");
    EXPECT_EQ(plant->get_state_output_port(default_model_instance()).get_name(), "DefaultModelInstance_state");
    EXPECT_EQ(plant->get_state_output_port(pendulum_model_instance).get_name(), "SplitPendulum_state");
    ...
    // Get links by name.
    const RigidBody<double>& link1 = plant->GetBodyByName(parameters.link1_name());
    EXPECT_EQ(link1.name(), parameters.link1_name());
    EXPECT_EQ(link1.model_instance(), default_model_instance());

    const RigidBody<double>& link2 = plant->GetBodyByName(parameters.link2_name());
    ...
    const std::vector<BodyIndex> acrobot_indices = plant->GetBodyIndices(default_model_instance());
    EXPECT_EQ(acrobot_indices.size(), 2);
    EXPECT_EQ(acrobot_indices[0], link1.index());
    EXPECT_EQ(acrobot_indices[1], link2.index());

    const std::vector<BodyIndex> pendulum_indices = plant->GetBodyIndices(pendulum_model_instance);
    EXPECT_EQ(pendulum_indices.size(), 2);
    EXPECT_EQ(pendulum_indices[0], upper.index());
    EXPECT_EQ(pendulum_indices[1], lower.index());

    // Get joints by name.
    const Joint<double>& shoulder_joint = plant->GetJointByName(parameters.shoulder_joint_name());
    ...
    Joint<double>& mutable_shoulder_joint = plant->GetMutableJointByName(parameters.shoulder_joint_name());
    EXPECT_EQ(&mutable_shoulder_joint, &shoulder_joint);
    const Joint<double>& elbow_joint = plant->GetJointByName(parameters.elbow_joint_name());
    ...
    EXPECT_EQ(&plant->gravity_field(), &plant->GetForceElement<UniformGravityFieldElement>(gravity_field_index));
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetForceElement<RevoluteSpring>(gravity_field_index),
    ...
    const std::vector<JointIndex> acrobot_joint_indices = plant->GetJointIndices(default_model_instance());
    ...
    const std::vector<JointIndex> pendulum_joint_indices = plant->GetJointIndices(pendulum_model_instance);
    ...
    EXPECT_EQ(plant->get_joint_actuator(acrobot_joint_actuator_indices[0]).joint().index(), elbow_joint.index());
    ...
    EXPECT_EQ(plant->get_joint_actuator(pendulum_joint_actuator_indices[0]).joint().index(), pin_joint.index());

    // Templatized version to obtain retrieve a particular known type of joint.
    const RevoluteJoint<double>& shoulder = plant->GetJointByName<RevoluteJoint>(parameters.shoulder_joint_name());
    EXPECT_EQ(shoulder.name(), parameters.shoulder_joint_name());
    const RevoluteJoint<double>& elbow = plant->GetJointByName<RevoluteJoint>(parameters.elbow_joint_name());
    EXPECT_EQ(elbow.name(), parameters.elbow_joint_name());
    const RevoluteJoint<double>& pin = plant->GetJointByName<RevoluteJoint>("pin");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->GetJointByName<PrismaticJoint>(parameters.shoulder_joint_name(), shoulder.model_instance()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->AddRigidBody("AnotherBody", default_model_instance(), SpatialInertia<double>::NaN()),
            "Post-finalize calls to '.*' are not allowed; "
            "calls to this method must happen before Finalize\\(\\).");
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->AddJoint<RevoluteJoint>("AnotherJoint", link1, std::nullopt, link2, std::nullopt, Vector3d::UnitZ()),
            "Post-finalize calls to '.*' are not allowed; "
            "calls to this method must happen before Finalize\\(\\).");
    // Test API for simplified `AddJoint` method.
    DRAKE_EXPECT_THROWS_MESSAGE(plant->AddJoint(std::make_unique<RevoluteJoint<double>>(
                                        "AnotherJoint", link1.body_frame(), link2.body_frame(), Vector3d::UnitZ())),
                                "Post-finalize calls to '.*' are not allowed; "
                                "calls to this method must happen before Finalize\\(\\).");
    ...
    const std::vector<FrameIndex> acrobot_frame_indices = plant->GetFrameIndices(default_model_instance());
    ...
    const std::vector<FrameIndex> pendulum_frame_indices = plant->GetFrameIndices(pendulum_model_instance);
...
GTEST_TEST(MultibodyPlantTest, AddJointActuator) {
    ...
    const Joint<double>& planar_joint = plant.AddJoint(std::make_unique<PlanarJoint<double>>(
            "planar", plant.world_body().body_frame(), body->body_frame(), Eigen::Vector3d{0, 0, 0.1}));

    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddJointActuator("planar_actuator", planar_joint), ".*3 degrees of freedom.*");
}
...
GTEST_TEST(MultibodyPlantTest, AddMultibodyPlantSceneGraph) {
    ...
    auto [first_element, second_element] = AddMultibodyPlantSceneGraph(&builder, 0.0);
    // Verify the expected types.
    EXPECT_EQ(drake::NiceTypeName::Get(first_element), drake::NiceTypeName::Get<MultibodyPlant<double>>());
    EXPECT_EQ(drake::NiceTypeName::Get(second_element), drake::NiceTypeName::Get<SceneGraph<double>>());
...
GTEST_TEST(MultibodyPlantTest, NoHeapAllocOnStringQueries) {
    ...
    const char kSdfUrl[] = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
    auto plant = std::make_unique<MultibodyPlant<double>>(0 /* plant type irrelevant */);
    Parser parser(plant.get());
    multibody::ModelInstanceIndex iiwa_instance = parser.AddModelsFromUrl(kSdfUrl).at(0);
    plant->Finalize();
...
GTEST_TEST(MultibodyPlantTest, EmptyWorldDiscrete) {
    ...
    DRAKE_EXPECT_NO_THROW(plant.CalcForcedDiscreteVariableUpdate(*context, new_discrete_state.get()));
}
...
GTEST_TEST(MultibodyPlantTest, EmptyWorldContinuous) {
    ...
    DRAKE_EXPECT_NO_THROW(plant.CalcTimeDerivatives(*context, new_derivatives.get()));
    VectorXd residual = plant.AllocateImplicitTimeDerivativesResidual();
    EXPECT_EQ(residual.size(), 0);
    DRAKE_EXPECT_NO_THROW(plant.CalcImplicitTimeDerivativesResidual(*context, *new_derivatives, &residual));
}

GTEST_TEST(MultibodyPlantTest, EmptyWorldContactErrors) {
    MultibodyPlant<double> plant(0.0);
    DRAKE_EXPECT_THROWS_MESSAGE(MultibodyPlantTester::FindBodyByGeometryId(plant, GeometryId{}),
                                ".* contact results for a null GeometryId.*");
    const GeometryId unknown_id = GeometryId::get_new_id();
    DRAKE_EXPECT_THROWS_MESSAGE(MultibodyPlantTester::FindBodyByGeometryId(plant, unknown_id),
...
GTEST_TEST(ActuationPortsTest, CheckActuation) {
    ...
    MultibodyPlant<double> plant(0.0);
    const std::string acrobot_url = "package://drake/multibody/benchmarks/acrobot/acrobot.sdf";
    ...
    EXPECT_TRUE(plant.GetBodyByName("uniformSolidCylinder").has_quaternion_dofs());
    ...
    std::unique_ptr<ContinuousState<double>> xdot_no_input = plant.AllocateTimeDerivatives();
    ...
    std::unique_ptr<ContinuousState<double>> xdot_zero_input = plant.AllocateTimeDerivatives();
    ...
    EXPECT_TRUE(CompareMatrices(xdot_no_input->CopyToVector(), xdot_zero_input->CopyToVector(), kEps,
                                MatrixCompareType::relative));

    // Verify that derivatives can be computed after fixing the cylinder actuation
    // input port with an empty vector.
    plant.get_actuation_input_port(cylinder_instance).FixValue(context.get(), VectorXd(0));
    std::unique_ptr<ContinuousState<double>> xdot = plant.AllocateTimeDerivatives();
    ...
    std::unique_ptr<ContinuousState<double>> xdot_sum = plant.AllocateTimeDerivatives();
    ...
    EXPECT_TRUE(CompareMatrices(xdot_sum->CopyToVector(), xdot->CopyToVector(), kEps, MatrixCompareType::relative));
}

GTEST_TEST(MultibodyPlant, UniformGravityFieldElementTest) {
    MultibodyPlant<double> plant(0.0);

    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddForceElement<UniformGravityFieldElement>(Vector3d(-1, 0, 0)),
...
public:
    ...
    void SetUp() override {
        ...
        // Finalize() conditions.
        const std::string url = "package://drake/multibody/benchmarks/acrobot/acrobot.sdf";
...
#pragma GCC diagnostic push
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(plant_->get_state_output_port(),
        ...
        const FrameId link1_frame_id = plant_->GetBodyFrameIdOrThrow(link1_->index());
        ...
        shoulder_ = &plant_->GetMutableJointByName<RevoluteJoint>(parameters_.shoulder_joint_name());
        elbow_ = &plant_->GetMutableJointByName<RevoluteJoint>(parameters_.elbow_joint_name());

        context_ = diagram_->CreateDefaultContext();
        derivatives_ = diagram_->AllocateTimeDerivatives();
        plant_context_ = &diagram_->GetMutableSubsystemContext(*plant_, context_.get());

        ASSERT_GT(plant_->num_actuators(), 0);
        input_port_ = &plant_->get_actuation_input_port().FixValue(plant_context_, 0.0);
    }
    ...
    void SetUpDiscreteAcrobotPlant(double time_step) {
        ...
        discrete_plant_->get_actuation_input_port().FixValue(discrete_context_.get(), 0.0);
    ...
    void VerifyCalcGravityGeneralizedForces(double theta1, double theta2) {
        ...
        const VectorX<double> tau_g = plant_->CalcGravityGeneralizedForces(*plant_context_);

        // Calculate a benchmark value.
        const Vector2d tau_g_expected = acrobot_benchmark_.CalcGravityVector(theta1, theta2);

        EXPECT_TRUE(CompareMatrices(tau_g, tau_g_expected, kTolerance, MatrixCompareType::relative));
        ...
        for (BodyIndex body_index(1); body_index < plant_->num_bodies(); ++body_index) {
            const RigidBody<double>& body = plant_->get_body(body_index);
            const SpatialForce<double>& F_Bo_W = body.GetForceInWorld(*plant_context_, forces);
            const double mass = body.default_mass();
            // TODO(amcastro-tri): provide RigidBody::EvalCOMInWorld().
            const Vector3<double> p_BoBcm_B = body.CalcCenterOfMassInBodyFrame(*plant_context_);
            const RigidTransform<double> X_WB = plant_->EvalBodyPoseInWorld(*plant_context_, body);
            ...
            EXPECT_TRUE(
                    CompareMatrices(F_Bo_W.translational(), f_Bo_W_expected, kTolerance, MatrixCompareType::relative));
            EXPECT_TRUE(CompareMatrices(F_Bo_W.rotational(), t_Bo_W_expected, kTolerance, MatrixCompareType::relative));
        }
        ...
        // Zero accelerations.
        const VectorX<double> zero_vdot = VectorX<double>::Zero(plant_->num_velocities());
        // Zero velocities.
        shoulder_->set_angular_rate(plant_context_, 0.0);
        elbow_->set_angular_rate(plant_context_, 0.0);
        const VectorX<double> tau_g_id = -plant_->CalcInverseDynamics(*plant_context_, zero_vdot, forces);
        EXPECT_TRUE(CompareMatrices(tau_g_id, tau_g_expected, kTolerance, MatrixCompareType::relative));
    }
    ...
    void VerifyCalcTimeDerivatives(
            double theta1, double theta2, double theta1dot, double theta2dot, double input_torque) {
        const double kTolerance = 5 * std::numeric_limits<double>::epsilon();
        ...
        const Vector2d C_expected = acrobot_benchmark_.CalcCoriolisVector(theta1, theta2, theta1dot, theta2dot);
        const Vector2d tau_g_expected = acrobot_benchmark_.CalcGravityVector(theta1, theta2);
        const Vector2d tau_damping(-parameters_.b1() * theta1dot, -parameters_.b2() * theta2dot);
        ...
        EXPECT_TRUE(CompareMatrices(forces.generalized_forces(), tau_damping, kTolerance, MatrixCompareType::relative));

        // Verify the computation of xdot.
        const Vector2d rhs = tau_g_expected + tau_damping - C_expected + Vector2d(0.0, input_torque);
        ...
        EXPECT_TRUE(CompareMatrices(xdot, xdot_expected, kTolerance, MatrixCompareType::relative));
        ...
        diagram_->CalcImplicitTimeDerivativesResidual(*context_, *derivatives_, &residual);
    ...
    void VerifyDoCalcDiscreteVariableUpdates(double theta1, double theta2, double theta1dot, double theta2dot) {
        ...
        const RevoluteJoint<double>& discrete_shoulder =
                discrete_plant_->GetJointByName<RevoluteJoint>(parameters_.shoulder_joint_name());
        const RevoluteJoint<double>& discrete_elbow =
                discrete_plant_->GetJointByName<RevoluteJoint>(parameters_.elbow_joint_name());
        ...
        discrete_plant_->CalcForcedDiscreteVariableUpdate(*discrete_context_, updates.get());
        ...
        xnext_expected.segment(nq, nv) = x0.segment(nq, nv) + time_step * xdot.segment(nq, nv);
        // We use the fact that nq = nv for this case.
        xnext_expected.segment(0, nq) = x0.segment(0, nq) + time_step * xnext_expected.segment(nq, nv);

        EXPECT_TRUE(CompareMatrices(xnext, xnext_expected, kTolerance, MatrixCompareType::relative));
    }
...
TEST_F(AcrobotPlantTests, VerifyCalcGravityGeneralizedForces) {
    ...
    VerifyCalcGravityGeneralizedForces(-M_PI / 5.0, M_PI / 2.0 /* joint's angles */);
    VerifyCalcGravityGeneralizedForces(M_PI / 3.0, -M_PI / 5.0 /* joint's angles */);
    VerifyCalcGravityGeneralizedForces(M_PI / 4.0, -M_PI / 3.0 /* joint's angles */);
...
TEST_F(AcrobotPlantTests, DoCalcDiscreteVariableUpdates) {
    ...
    const ModelInstanceIndex instance_index = discrete_plant_->GetModelInstanceByName("acrobot");
    ...
    EXPECT_EQ(discrete_plant_->get_generalized_contact_forces_output_port(instance_index).size(), 2);

    // Verify the implementation for a number of arbitrarily chosen states.
    VerifyDoCalcDiscreteVariableUpdates(-M_PI / 5.0, M_PI / 2.0, /* joint's angles */
                                        0.5, 1.0);               /* joint's angular rates */
    VerifyDoCalcDiscreteVariableUpdates(M_PI / 3.0, -M_PI / 5.0, /* joint's angles */
                                        0.7, -1.0);              /* joint's angular rates */
    VerifyDoCalcDiscreteVariableUpdates(M_PI / 4.0, -M_PI / 3.0, /* joint's angles */
                                        -0.5, 2.0);              /* joint's angular rates */
...
TEST_F(AcrobotPlantTests, VisualGeometryRegistration) {
    ...
    unique_ptr<AbstractValue> poses_value = plant_->get_geometry_pose_output_port().Allocate();
    DRAKE_EXPECT_NO_THROW(poses_value->get_value<FramePoseVector<double>>());
    const FramePoseVector<double>& poses = poses_value->get_value<FramePoseVector<double>>();
    ...
    const FrameId world_frame_id = plant_->GetBodyFrameIdOrThrow(plant_->world_body().index());
    ASSERT_TRUE(plant_->GetBodyFromFrameId(world_frame_id) != nullptr);
    EXPECT_EQ(plant_->GetBodyFromFrameId(world_frame_id)->index(), plant_->world_body().index());
    const double kTolerance = 5 * std::numeric_limits<double>::epsilon();
    for (BodyIndex body_index(1); body_index < plant_->num_bodies(); ++body_index) {
        const FrameId frame_id = plant_->GetBodyFrameIdOrThrow(body_index);
        // Also confirm the "maybe" variant works.
        const std::optional<FrameId> optional_id = plant_->GetBodyFrameIdIfExists(body_index);
        ...
        EXPECT_TRUE(CompareMatrices(X_WB.GetAsMatrix34(), X_WB_expected.GetAsMatrix34(), kTolerance,
                                    MatrixCompareType::relative));
    }
...
GTEST_TEST(MultibodyPlantTest, SetDefaultFreeBodyPose) {
    ...
    const auto& body = plant.AddRigidBody("body", SpatialInertia<double>::MakeUnitary());
    const auto& welded_body = plant.AddRigidBody("welded body", SpatialInertia<double>::MakeUnitary());
    ...
    EXPECT_TRUE(CompareMatrices(plant.GetDefaultFreeBodyPose(body).GetAsMatrix4(), X_WB.GetAsMatrix4(), kTolerance));
    EXPECT_TRUE(
            CompareMatrices(plant.GetDefaultFreeBodyPose(welded_body).GetAsMatrix4(), X_WB.GetAsMatrix4(), kTolerance));

    plant.Finalize();
    EXPECT_GT(plant.num_positions(), 0);
    auto context = plant.CreateDefaultContext();
    EXPECT_TRUE(CompareMatrices(body.EvalPoseInWorld(*context).GetAsMatrix4(), X_WB.GetAsMatrix4(), kTolerance));
    // The pose of a non-free body isn't affected by the call to
    // SetDefaultFreeBodyPose() even though it's allowed.
    EXPECT_TRUE(CompareMatrices(welded_body.EvalPoseInWorld(*context).GetAsMatrix4(),
...
TEST_F(AcrobotPlantTests, SetRandomState) {
    ...
    EXPECT_TRUE(CompareMatrices(context_->get_mutable_continuous_state_vector().CopyToVector(),
    ...
    plant_->SetRandomContext(random_context.get(), &generator);
    EXPECT_FALSE(CompareMatrices(context_->get_mutable_continuous_state_vector().CopyToVector(),
                                 random_context->get_mutable_continuous_state_vector().CopyToVector()));

    const Eigen::VectorXd first_random_state = random_context->get_mutable_continuous_state_vector().CopyToVector();
    // And every call should return something different.
    plant_->SetRandomContext(random_context.get(), &generator);
    EXPECT_FALSE(
            CompareMatrices(first_random_state, random_context->get_mutable_continuous_state_vector().CopyToVector()));
}
...
GTEST_TEST(MultibodyPlantTest, Graphviz) {
    ...
    const std::string dot = std::regex_replace(plant.GetTopologyGraphvizString(), std::regex("\\n"), "");
    ...
    EXPECT_NE(std::string::npos, dot.find("uniformSolidCylinder", pos + 1)) << dot;
    // Check for the second cylinder model instance.
    EXPECT_NE(std::string::npos, dot.find("cylinder2::uniformSolidCylinder")) << dot;
    ...
    plant.Finalize();
    const std::string dot_finalized = std::regex_replace(plant.GetTopologyGraphvizString(), std::regex("\\n"), "");
    ...
    EXPECT_THAT(dot_finalized.c_str(), testing::HasSubstr(dot.substr(0, dot.size() - 1).c_str()));

    // Check that the two floating joints created at Finalize() exist.
    const size_t pos_finalized = dot_finalized.find("uniformSolidCylinder [quaternion_floating]");
    EXPECT_NE(std::string::npos, pos_finalized) << dot_finalized;
    EXPECT_NE(std::string::npos, dot_finalized.find("uniformSolidCylinder [quaternion_floating]", pos_finalized + 1));
}
...
public:
    explicit SphereChainScenario(int sphere_count, std::vector<bool> weld_to_next = {}) : sphere_count_(sphere_count) {
        using std::to_string;
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder_, 0.0);
        ...
        auto make_sphere = [this](int i) {
            ...
            const RigidBody<double>& sphere =
                    plant_->AddRigidBody("Sphere" + to_string(i), SpatialInertia<double>::MakeUnitary());
            GeometryId sphere_id =
                    plant_->RegisterCollisionGeometry(sphere, RigidTransformd::Identity(), geometry::Sphere(radius),
            ...
            plant_->RegisterVisualGeometry(sphere, RigidTransformd::Identity(), geometry::Sphere(radius), "visual");
        ...
        for (int i = 0; i < sphere_count_ - 1; ++i) {
            if (weld_to_next[i]) {
                plant_->WeldFrames(spheres_[i]->body_frame(), spheres_[i + 1]->body_frame());
            } else {
                plant_->AddJoint<RevoluteJoint>("hinge" + to_string(i) + "_" + to_string(i + 1), *spheres_[i],
        ...
        // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
        no_geometry_body_ = &plant_->AddRigidBody("NothingRegistered", SpatialInertia<double>::MakeUnitary());
    }
    ...
    void Finalize() {
        ...
        for (int i = 0; i < sphere_count_; ++i) {
            ...
            for (int j = i + 2; j < sphere_count_; ++j) {
                unfiltered_collisions_.insert(std::make_pair(sphere_id(i), sphere_id(j)));
            }
        }
    }

    // Perform the pair-wise collision detection.
    std::vector<geometry::PenetrationAsPointPair<double>> ComputePointPairPenetration() const {
        // Grab query object to test for collisions.
        const auto& query_object =
                plant_->get_geometry_query_input_port().Eval<geometry::QueryObject<double>>(*plant_context_);
    ...
    const RigidBody<double>& no_geometry_body() const { return *no_geometry_body_; }
    ...
    // have been applied.
    const std::set<std::pair<GeometryId, GeometryId>>& unfiltered_collisions() const { return unfiltered_collisions_; }
...
GTEST_TEST(MultibodyPlantTest, AutoBodySceneGraphRegistration) {
    MultibodyPlant<double> plant(0.0);
    const RigidBody<double>& body1 = plant.AddRigidBody("body1", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetBodyFrameIdOrThrow(body1.index()),
    ...
    const RigidBody<double>& body2 = plant.AddRigidBody("body2", SpatialInertia<double>::NaN());
...
GTEST_TEST(MultibodyPlantTest, FilterAdjacentBodies) {
    for (bool do_filters : {false, true}) {
        ...
        scenario.Finalize();
        std::vector<geometry::PenetrationAsPointPair<double>> contacts = scenario.ComputePointPairPenetration();

        // The expected collisions.
        const std::set<std::pair<GeometryId, GeometryId>>& expected_pairs = scenario.unfiltered_collisions();
        if (do_filters) {
            ...
            auto expect_pair_in_set = [&expected_pairs](GeometryId id1, GeometryId id2) {
                ...
                if (!expected_pairs.contains(pair1) && !expected_pairs.contains(pair2)) {
                    GTEST_FAIL() << "The pair " << id1 << ", " << id2 << " is not in the expected set";
                }
            };
...
GTEST_TEST(MultibodyPlantTest, FilterWorldWelds) {
    ...
    scenario.mutable_plant()->WeldFrames(scenario.mutable_plant()->world_body().body_frame(),
...
GTEST_TEST(MultibodyPlantTest, FilterWeldedSubgraphs) {
    ...
    constexpr int filtered = filtered_in_first_subgraph + filtered_in_second_subgraph;
...
GTEST_TEST(MultibodyPlantTest, CollectRegisteredGeometriesErrors) {
    MultibodyPlant<double> plant(0.0);
    const RigidBody<double>& body = plant.AddRigidBody("body", SpatialInertia<double>::MakeUnitary());
...
GTEST_TEST(MultibodyPlantTest, CollectRegisteredGeometries) {
    ...
    auto check_geometries = [&]() {
        ...
        {
            GeometrySet set = plant.CollectRegisteredGeometries({&scenario.no_geometry_body()});
            GeometrySetTester tester(&set);
            EXPECT_EQ(tester.num_geometries(), 0);
            EXPECT_EQ(tester.num_frames(), 1);
        }
        ...
        {
            GeometrySet set = plant.CollectRegisteredGeometries({&scenario.mutable_plant()->world_body()});
            GeometrySetTester tester(&set);
            EXPECT_EQ(tester.num_frames(), 1);
            EXPECT_EQ(tester.num_geometries(), 0);
        ...
        {
            GeometrySet set = plant.CollectRegisteredGeometries(scenario.get_all_bodies());
            GeometrySetTester tester(&set);
...
GTEST_TEST(MultibodyPlantTest, GetBodiesWeldedTo) {
    ...
    const std::string sdf_url = "package://drake/multibody/plant/test/split_pendulum.sdf";
    MultibodyPlant<double> plant(0.0);
    ...
    const RigidBody<double>& extra =
            plant.AddRigidBody("extra", default_model_instance(), SpatialInertia<double>::NaN());
    plant.WeldFrames(plant.world_frame(), extra.body_frame());

    // Verify we can call GetBodiesWeldedTo() pre-finalize.
    EXPECT_THAT(plant.GetBodiesWeldedTo(plant.world_body()), UnorderedElementsAre(&plant.world_body(), &extra));
    EXPECT_THAT(plant.GetBodiesWeldedTo(lower), UnorderedElementsAre(&upper, &lower));
    ...
    std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_ad = systems::System<double>::ToAutoDiffXd(plant);
    const RigidBody<AutoDiffXd>& upper_ad = plant_ad->GetBodyByName("upper_section");
    const RigidBody<AutoDiffXd>& lower_ad = plant_ad->GetBodyByName("lower_section");
    ...
    EXPECT_THAT(plant_ad->GetBodiesWeldedTo(lower_ad), UnorderedElementsAre(&upper_ad, &lower_ad));
}
...
GTEST_TEST(MultibodyPlantTest, GetBodiesKinematicallyAffectedBy) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetBodiesKinematicallyAffectedBy(joints1),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetBodiesKinematicallyAffectedBy(joints2), ".*joint with index.*welded.");
...
GTEST_TEST(MultibodyPlantTest, ReversedWeldError) {
    ...
    // reflect that.
    DRAKE_EXPECT_THROWS_MESSAGE(plant.Finalize(), ".*already has a joint.*extra_welds_to_world.*joint.*not allowed.*");
}
...
bool VerifyFeedthroughPorts(const MultibodyPlant<double>& plant) {
    ...
    for (const auto& [name, is_feedthrough] : manifest) {
        std::set<std::string>& destination = is_feedthrough ? expected_feedthrough : expected_non_feedthrough;
        ...
        if (!needs_fmt) {
            ...
            for (ModelInstanceIndex i(0); i < plant.num_model_instances(); ++i) {
                ...
                destination.insert(fmt::format(fmt::runtime(name), fmt::arg("instance", instance)));
            }
        }
    }
    ...
    // Cross-check that every output port was listed in the manifest.
    {
        ...
        manifest_all.insert(expected_feedthrough.begin(), expected_feedthrough.end());
        manifest_all.insert(expected_non_feedthrough.begin(), expected_non_feedthrough.end());
    ...
    for (const OutputPortIndex& i : actual_feedthrough_indices) {
        actual_feedthrough.insert(plant.get_output_port(i, /*warn_deprecated=*/false).get_name());
    }
...
GTEST_TEST(MultibodyPlantTest, CollisionGeometryRegistration) {
    ...
    // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
    const RigidBody<double>& sphere1 = plant.AddRigidBody("Sphere1", SpatialInertia<double>::MakeUnitary());
    ...
    sphere1_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, sphere1_friction);
    sphere1_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kPointStiffness,
                                   sphere1_stiffness);
    sphere1_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kHcDissipation,
                                   sphere1_dissipation);
    GeometryId sphere1_id = plant.RegisterCollisionGeometry(
            sphere1, RigidTransformd::Identity(), geometry::Sphere(radius), "collision", std::move(sphere1_properties));
    ...
    sphere2_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, sphere2_friction);
    sphere2_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kPointStiffness,
                                   sphere2_stiffness);
    sphere2_properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kHcDissipation,
                                   sphere2_dissipation);
    const RigidBody<double>& sphere2 = plant.AddRigidBody("Sphere2", SpatialInertia<double>::MakeUnitary());
    GeometryId sphere2_id = plant.RegisterCollisionGeometry(
            sphere2, RigidTransformd::Identity(), geometry::Sphere(radius), "collision", std::move(sphere2_properties));

    // Confirm externally-defined proximity properties propagate through.
    {
        EXPECT_NE(scene_graph.model_inspector().GetProximityProperties(sphere2_id), nullptr);
        const geometry::ProximityProperties& props = *scene_graph.model_inspector().GetProximityProperties(sphere2_id);
    ...
    plant.SetFreeBodyPose(plant_context, sphere2, RigidTransformd(Vector3d(x_offset, radius, 0.0)));

    unique_ptr<AbstractValue> poses_value = plant.get_geometry_pose_output_port().Allocate();
    DRAKE_EXPECT_NO_THROW(poses_value->get_value<FramePoseVector<double>>());
    const FramePoseVector<double>& pose_data = poses_value->get_value<FramePoseVector<double>>();
    ...
    EXPECT_TRUE(ground_props.GetProperty<CoulombFriction<double>>(geometry::internal::kMaterialGroup,
                                                                  geometry::internal::kFriction) == ground_friction);
    EXPECT_TRUE(sphere1_props.GetProperty<CoulombFriction<double>>(geometry::internal::kMaterialGroup,
                                                                   geometry::internal::kFriction) == sphere1_friction);
    EXPECT_TRUE(sphere2_props.GetProperty<CoulombFriction<double>>(geometry::internal::kMaterialGroup,
                                                                   geometry::internal::kFriction) == sphere2_friction);

    EXPECT_TRUE(sphere1_props.GetProperty<double>(geometry::internal::kMaterialGroup,
                                                  geometry::internal::kPointStiffness) == sphere1_stiffness);
    EXPECT_TRUE(sphere2_props.GetProperty<double>(geometry::internal::kMaterialGroup,
    ...
    const geometry::ProximityProperties* context_ground_props = context_inspector.GetProximityProperties(ground_id);
    ASSERT_NE(context_ground_props, nullptr);
    EXPECT_TRUE(context_ground_props->GetProperty<CoulombFriction<double>>(
                        geometry::internal::kMaterialGroup, geometry::internal::kFriction) == ground_friction);
}
...
GTEST_TEST(MultibodyPlantTest, VisualGeometryRegistration) {
    ...
    GeometryId ground_id = plant.RegisterVisualGeometry(
            plant.world_body(),
            // A half-space passing through the origin in the x-z plane.
            geometry::HalfSpace::MakePose(Vector3d::UnitY(), Vector3d::Zero()), geometry::HalfSpace(), "ground");
    ...
    GeometryId sphere1_id = plant.RegisterVisualGeometry(sphere1, RigidTransformd::Identity(), geometry::Sphere(radius),
    ...
    sphere2_props.AddProperty("renderer", "accepting", std::set<std::string>{"not_dummy"});
    GeometryId sphere2_id = plant.RegisterVisualGeometry(sphere2, RigidTransformd::Identity(), geometry::Sphere(radius),
    ...
    unique_ptr<AbstractValue> state_value = scene_graph.get_query_output_port().Allocate();
    DRAKE_EXPECT_NO_THROW(state_value->get_value<QueryObject<double>>());
    const QueryObject<double>& query_object = state_value->get_value<QueryObject<double>>();
    scene_graph.get_query_output_port().Calc(*context, state_value.get());

    const SceneGraphInspector<double>& inspector = query_object.inspector();
    {
        const IllustrationProperties* material = inspector.GetIllustrationProperties(ground_id);
    ...
    auto get_diffuse_color = [&inspector](GeometryId id) -> Vector4<double> {
        const IllustrationProperties* material = inspector.GetIllustrationProperties(id);
        return material->GetProperty<Vector4<double>>("phong", "diffuse");
    };
    {
        const Vector4<double>& test_diffuse = get_diffuse_color(sphere1_id);
        EXPECT_TRUE(CompareMatrices(test_diffuse, sphere1_diffuse, 0.0, MatrixCompareType::absolute));
    }

    {
        const Vector4<double>& test_diffuse = get_diffuse_color(sphere2_id);
        EXPECT_TRUE(CompareMatrices(test_diffuse, sphere2_diffuse, 0.0, MatrixCompareType::absolute));
        const IllustrationProperties* material = inspector.GetIllustrationProperties(sphere2_id);
        ASSERT_TRUE(material->HasProperty("phong", "diffuse_map"));
        EXPECT_EQ(material->GetProperty<std::string>("phong", "diffuse_map"), "empty.png");
    }
}
...
GTEST_TEST(MultibodyPlantTest, AutoDiffCalcPointPairPenetrations) {
    ...
    auto autodiff_pendulum = drake::systems::System<double>::ToAutoDiffXd(*pendulum.get());
    ...
    DRAKE_EXPECT_NO_THROW(autodiff_pendulum->EvalPointPairPenetrations(*autodiff_context.get()));
}
...
GTEST_TEST(MultibodyPlantTest, LinearizePendulum) {
    ...
    PendulumParameters parameters;
    unique_ptr<MultibodyPlant<double>> pendulum = MakePendulumPlant(parameters);
    const auto& pin = pendulum->GetJointByName<RevoluteJoint>(parameters.pin_joint_name());
    unique_ptr<Context<double>> context = pendulum->CreateDefaultContext();
    pendulum->get_actuation_input_port().FixValue(context.get(), 0.0);
    pendulum->get_applied_generalized_force_input_port().FixValue(context.get(), 0.0);
    ...
    unique_ptr<LinearSystem<double>> linearized_pendulum =
            Linearize(*pendulum, *context, pendulum->get_actuation_input_port().get_index(),
                      systems::OutputPortSelection::kNoOutput);

    // Compute the expected solution by hand.
    Eigen::Matrix2d A;
    Eigen::Vector2d B;
    const double domegadot_domega = -parameters.damping() / (parameters.m() * parameters.l() * parameters.l());
    // clang-format off
  A << 0.0, 1.0,
    ...
    pin.set_angular_rate(context.get(), 0.0);
    linearized_pendulum = Linearize(*pendulum, *context, pendulum->get_actuation_input_port().get_index(),
                                    systems::OutputPortSelection::kNoOutput);
    // Compute the expected solution by hand.
    // clang-format off
  A << 0.0, 1.0,
...
TEST_F(AcrobotPlantTests, EvalStateAndAccelerationOutputPorts) {
    ...
    // for a MultibodyPlant sets all revolute joints to have zero angles and zero
    // angular velocity.
    unique_ptr<systems::Context<double>> diagram_context = diagram_->CreateDefaultContext();
    Context<double>& context = plant_->GetMyMutableContextFromRoot(diagram_context.get());
    ...
    unique_ptr<AbstractValue> state_value = plant_->get_state_output_port().Allocate();
    DRAKE_EXPECT_NO_THROW(state_value->get_value<BasicVector<double>>());
    const BasicVector<double>& state_out = state_value->get_value<BasicVector<double>>();
    ...
    const auto& accel = plant_->get_generalized_acceleration_output_port().Eval<BasicVector<double>>(context);
    ...
    const auto& accel_instance =
            plant_->get_generalized_acceleration_output_port(instance).Eval<BasicVector<double>>(context);
    ...
    // results.
    const auto& accel_default_instance = plant_->get_generalized_acceleration_output_port(default_model_instance())
                                                 .Eval<BasicVector<double>>(context);
    EXPECT_EQ(accel_default_instance.size(), 0);

    const auto& state_world_instance =
            plant_->get_state_output_port(world_model_instance()).Eval<BasicVector<double>>(context);
    EXPECT_EQ(state_world_instance.size(), 0);
}

// Helper function for the two v-to-qdot and qdot-to-v tests.
void InitializePlantAndContextForVelocityToQDotMapping(MultibodyPlant<double>* plant,
                                                       std::unique_ptr<Context<double>>* context) {
    // This is used in purely kinematic tests.
    // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
    const RigidBody<double>& body = plant->AddRigidBody("FreeBody", SpatialInertia<double>::MakeUnitary());
    plant->Finalize();
    ...
    *context = plant->CreateDefaultContext();
    ...
    const Vector3d axis_W =        // Orientation in world.
            (1.5 * Vector3d::UnitX() + 2.0 * Vector3d::UnitY() + 3.0 * Vector3d::UnitZ()).normalized();
    ...
    const SpatialVelocity<double> V_WB(Vector3d(1.0, 2.0, 3.0), Vector3d(-1.0, 4.0, -0.5));
...
GTEST_TEST(MultibodyPlantTest, MapVelocityToQDotAndBackContinuous) {
    ...
    const double kTolerance = 5 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(v_back.CopyToVector(), v.CopyToVector(), kTolerance));
...
public:
    void SetUp() override {
        ...
        Parser(&plant_).AddModelsFromUrl("package://drake/multibody/plant/test/split_pendulum.sdf");
        plant_.Finalize();
...
TEST_F(SplitPendulum, GetMultibodyPlantFromElement) {
    ...
    struct MyMBSystem : public internal::MultibodyTreeSystem<double> {
        MyMBSystem() {
            rigid_body = &mutable_tree().AddRigidBody("Body", SpatialInertia<double>::NaN());
            Finalize();
        }
        const RigidBody<double>* rigid_body{};
    } mb_system;

    DRAKE_EXPECT_THROWS_MESSAGE(mb_system.rigid_body->GetParentPlant(),
...
GTEST_TEST(MultibodyPlantTest, ScalarConversionConstructor) {
    ...
    const int link1_num_collisions = plant.GetCollisionGeometriesForBody(plant.GetBodyByName("link1")).size();
    const int link2_num_collisions = plant.GetCollisionGeometriesForBody(plant.GetBodyByName("link2")).size();
    const int link3_num_collisions = plant.GetCollisionGeometriesForBody(plant.GetBodyByName("link3")).size();
    ASSERT_EQ(link1_num_collisions, 2);
    ASSERT_EQ(link2_num_collisions, 0);
    ASSERT_EQ(link3_num_collisions, 1);

    const int link1_num_visuals = plant.GetVisualGeometriesForBody(plant.GetBodyByName("link1")).size();
    const int link2_num_visuals = plant.GetVisualGeometriesForBody(plant.GetBodyByName("link2")).size();
    const int link3_num_visuals = plant.GetVisualGeometriesForBody(plant.GetBodyByName("link3")).size();
    ...
    std::unique_ptr<MultibodyPlant<AutoDiffXd>> plant_autodiff = systems::System<double>::ToAutoDiffXd(plant);
    EXPECT_TRUE(plant_autodiff->geometry_source_is_registered());
    EXPECT_EQ(plant_autodiff->num_collision_geometries(), plant.num_collision_geometries());
    EXPECT_EQ(plant_autodiff->GetCollisionGeometriesForBody(plant_autodiff->GetBodyByName("link1")).size(),
              link1_num_collisions);
    EXPECT_EQ(plant_autodiff->GetCollisionGeometriesForBody(plant_autodiff->GetBodyByName("link2")).size(),
              link2_num_collisions);
    EXPECT_EQ(plant_autodiff->GetCollisionGeometriesForBody(plant_autodiff->GetBodyByName("link3")).size(),
              link3_num_collisions);
    EXPECT_EQ(plant_autodiff->GetVisualGeometriesForBody(plant_autodiff->GetBodyByName("link1")).size(),
              link1_num_visuals);
    EXPECT_EQ(plant_autodiff->GetVisualGeometriesForBody(plant_autodiff->GetBodyByName("link2")).size(),
              link2_num_visuals);
    EXPECT_EQ(plant_autodiff->GetVisualGeometriesForBody(plant_autodiff->GetBodyByName("link3")).size(),
              link3_num_visuals);
...
public:
    void SetUp() override {
        ...
        // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
        const RigidBody<double>& large_box = plant_.AddRigidBody("LargeBox", SpatialInertia<double>::MakeUnitary());
        large_box_id_ =
                plant_.RegisterCollisionGeometry(large_box, RigidTransformd::Identity(),
                                                 geometry::Box(large_box_size_, large_box_size_, large_box_size_),
                                                 "collision", CoulombFriction<double>());

        const RigidBody<double>& small_box = plant_.AddRigidBody("SmallBox", SpatialInertia<double>::MakeUnitary());
        small_box_id_ =
                plant_.RegisterCollisionGeometry(small_box, RigidTransformd::Identity(),
    ...
    void SetBoxesOnSlantedConfiguration(Context<double>* context) {
        ...
        const RigidTransform<double> X_WLb =
                // Pure rotation.
                RigidTransform<double>(RotationMatrix<double>::MakeZRotation(M_PI_4), Vector3<double>::Zero()) *
                // Pure translation.
    ...
    void SetPenetrationPairs(const Context<double>& context,
                             std::vector<PenetrationAsPointPair<double>>* penetrations) {
        const RigidBody<double>& large_box = plant_.GetBodyByName("LargeBox");
        const RigidBody<double>& small_box = plant_.GetBodyByName("SmallBox");

        // Pose of the boxes in the world frame.
        const RigidTransform<double>& X_WLb = plant_.EvalBodyPoseInWorld(context, large_box);
        const RigidTransform<double>& X_WSb = plant_.EvalBodyPoseInWorld(context, small_box);

        // Normal pointing outwards from the top surface of the large box.
        const Vector3<double> nhat_large_box_W = X_WLb.rotation() * Vector3<double>::UnitY();
    ...
    pair<unique_ptr<MultibodyPlant<AutoDiffXd>>, unique_ptr<Context<AutoDiffXd>>> ConvertPlantAndContextToAutoDiffXd() {
        // Scalar convert the plant and its context_.
        unique_ptr<MultibodyPlant<AutoDiffXd>> plant_autodiff = systems::System<double>::ToAutoDiffXd(plant_);
        unique_ptr<Context<AutoDiffXd>> context_autodiff = plant_autodiff->CreateDefaultContext();
        ...
        const VectorX<double> v = context_->get_continuous_state().get_generalized_velocity().CopyToVector();
        VectorX<AutoDiffXd> v_autodiff(plant_.num_velocities());
        math::InitializeAutoDiff(v, &v_autodiff);
        context_autodiff->get_mutable_continuous_state().get_mutable_generalized_velocity().SetFromVector(v_autodiff);
    ...
    // test.
    template <typename T>
    VectorX<T> CalcNormalVelocities(const MultibodyPlant<T>& plant_on_T,
        ...
        for (const auto& pair : pairs_set) {
            ...
            BodyIndex bodyA_index = MultibodyPlantTester::FindBodyByGeometryId(plant_on_T, pair.id_A);
            const RigidTransform<T>& X_WA =
                    plant_on_T.EvalBodyPoseInWorld(context_on_T, plant_on_T.get_body(bodyA_index));
            const SpatialVelocity<T> V_WA =
                    plant_on_T.EvalBodySpatialVelocityInWorld(context_on_T, plant_on_T.get_body(bodyA_index));

            BodyIndex bodyB_index = MultibodyPlantTester::FindBodyByGeometryId(plant_on_T, pair.id_B);
            const RigidTransform<T>& X_WB =
                    plant_on_T.EvalBodyPoseInWorld(context_on_T, plant_on_T.get_body(bodyB_index));
            const SpatialVelocity<T> V_WB =
                    plant_on_T.EvalBodySpatialVelocityInWorld(context_on_T, plant_on_T.get_body(bodyB_index));
    ...
    // test.
    template <typename T>
    VectorX<T> CalcTangentVelocities(const MultibodyPlant<T>& plant_on_T,
                                     const Context<T>& context_on_T,
...
GTEST_TEST(KukaModel, JointIndexes) {
    MultibodyPlant<double> plant(0.0);
    Parser(&plant).AddModelsFromUrl("package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf");
    const auto& base_link_frame = plant.GetFrameByName("iiwa_link_0");
    const Joint<double>& weld = plant.WeldFrames(plant.world_frame(), base_link_frame);
    plant.Finalize();
    ...
    const std::string weld_name = plant.world_frame().name() + "_welds_to_" + base_link_frame.name();
    ...
    for (int joint_number : {1, 2, 3, 4, 5, 6, 7}) {
        const auto& joint = plant.GetJointByName<RevoluteJoint>("iiwa_joint_" + std::to_string(joint_number));

        // We simply set each entry in the state with the value of its index.
        joint.set_angle(context.get(), joint.position_start());
        joint.set_angular_rate(context.get(), plant.num_positions() + joint.velocity_start());
    }

    // Verify that each entry has the value we expect it to have.
    const VectorX<double> xc = context->get_continuous_state_vector().CopyToVector();
    const VectorX<double> xc_expected =
            VectorX<double>::LinSpaced(plant.num_multibody_states() /* size */, 0 /* first index */,
...
GTEST_TEST(KukaModel, ActuationMatrix) {
    ...
    const Eigen::SparseMatrix<double> B_inv = plant.MakeActuationMatrixPseudoinverse();
...
TEST_F(MultibodyPlantRemodeling, MakeActuatorSelectorMatrix) {
    ...
    const std::vector<JointActuatorIndex> user_to_actuator_index_map{
            plant_->GetJointActuatorByName("actuator2").index(), plant_->GetJointActuatorByName("actuator0").index()};
  ...
  const Eigen::MatrixXd Su_expected =
    ...
    // using JointActuator::input_start().
    const Eigen::MatrixXd Su = plant_->MakeActuatorSelectorMatrix(user_to_actuator_index_map);
...
TEST_F(MultibodyPlantRemodeling, AddJointActuationForces) {
    ...
    // Actuator with index 1 has been removed.
    const systems::InputPort<double>& u_input = plant_->get_actuation_input_port();
    ...
    VectorXd forces(3);
    forces.setZero();
    MultibodyPlantTester::AddJointActuationForces(*plant_, *plant_context_, &forces);
...
TEST_F(MultibodyPlantRemodeling, RemoveJoint) {
    ...
    EXPECT_THAT(plant_->GetJointIndices(), testing::ElementsAre(JointIndex(0), JointIndex(2)));
    ...
    EXPECT_THAT(plant_->GetJointIndices(), testing::ElementsAre(JointIndex(0), JointIndex(2), JointIndex(3)));
    ...
    std::unique_ptr<MultibodyPlant<double>> clone = systems::System<double>::Clone(*plant_);
    ...
    EXPECT_THAT(clone->GetJointIndices(), testing::ElementsAre(JointIndex(0), JointIndex(2), JointIndex(3)));
...
TEST_F(MultibodyPlantRemodeling, RemoveAndReplaceJoint) {
    ...
    for (int i = 1; i < num_replacements; ++i) {
        const RevoluteJoint<double>& joint1 = plant_->AddJoint<RevoluteJoint>(
                "joint1", plant_->GetBodyByName("body0"), {}, plant_->GetBodyByName("body1"), {}, Vector3d::UnitZ());
    ...
    const WeldJoint<double>& joint1 =
            plant_->AddJoint<WeldJoint>("joint1", plant_->GetBodyByName("body0"), {}, plant_->GetBodyByName("body1"),
    ...
    EXPECT_THAT(plant_->GetJointIndices(),
                testing::ElementsAre(JointIndex(0), JointIndex(2), JointIndex(2 + num_replacements)));
    // Validate that ordinals are assigned and updated contiguously.
    const Joint<double>& joint0 = plant_->get_joint(JointIndex(0));
    const Joint<double>& joint2 = plant_->get_joint(JointIndex(2));
    EXPECT_EQ(joint0.ordinal(), 0);
...
TEST_F(MultibodyPlantRemodeling, RemoveJointWithActuator) {
    BuildModel();
    DRAKE_EXPECT_THROWS_MESSAGE(DoRemoval(false /* do not remove actuator */, true /* remove joint */),
...
TEST_F(MultibodyPlantRemodeling, RemoveJointWithCoupler) {
    ...
    plant_->AddCouplerConstraint(plant_->GetJointByName<RevoluteJoint>("joint0"),
                                 plant_->GetJointByName<RevoluteJoint>("joint1"), 2.0 /* gear ratio */,
                                 1.0 /* offset */);
    DRAKE_EXPECT_THROWS_MESSAGE(DoRemoval(true /* remove actuator */, true /* remove joint */),
...
TEST_F(MultibodyPlantRemodeling, RemoveJointWithDoorHinge) {
    ...
    plant_->AddForceElement<DoorHinge>(plant_->GetJointByName<RevoluteJoint>("joint1"), DoorHingeConfig());
    DRAKE_EXPECT_THROWS_MESSAGE(DoRemoval(true /* remove actuator */, true /* remove joint */),
...
TEST_F(MultibodyPlantRemodeling, RemoveJointWithRevoluteSpring) {
    ...
    plant_->AddForceElement<RevoluteSpring>(plant_->GetJointByName<RevoluteJoint>("joint1"), 0 /* nominal angle */,
                                            10 /* stiffness */);
    DRAKE_EXPECT_THROWS_MESSAGE(DoRemoval(true /* remove actuator */, true /* remove joint */),
                                "RemoveJoint: This plant has 1 user-added force elements. This plant "
                                "must have 0 user-added force elements in order to remove joint with "
...
TEST_F(MultibodyPlantRemodeling, RemoveJointWithPrismaticSpring) {
    ...
    plant_->AddForceElement<PrismaticSpring>(plant_->GetJointByName<PrismaticJoint>("joint1"), 0 /* nominal angle */,
                                             10 /* stiffness */);
    DRAKE_EXPECT_THROWS_MESSAGE(DoRemoval(true /* remove actuator */, true /* remove joint */),
                                "RemoveJoint: This plant has 1 user-added force elements. This plant "
                                "must have 0 user-added force elements in order to remove joint with "
...
protected:
    void SetUp() override {
        ...
        const Joint<double>& weld = plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0"));
        plant_->Finalize();
    ...
    void SetState(const VectorX<double>& xc) {
        ...
        for (JointIndex joint_index : plant_->GetJointIndices()) {
            ...
            const auto& joint = plant_->GetJointByName<RevoluteJoint>(plant_->get_joint(joint_index).name());
...
TEST_P(KukaArmTest, StateAccess) {
    ...
    VectorX<double> xc_expected =
            VectorX<double>::LinSpaced(plant_->num_multibody_states() /* size */, 1 /* first index */,
    ...
    // referenced by xc.
    Eigen::VectorBlock<const VectorX<double>> xc = plant_->GetPositionsAndVelocities(*context_);
    EXPECT_EQ(xc, xc_expected);
    ...
    plant_->SetPositions(context_.get(), xc_expected.head(plant_->num_positions()));
    EXPECT_EQ(plant_->GetPositions(*context_), xc_expected.head(plant_->num_positions()));

    // Modify velocities and change xc_expected to reflect changes to velocities.
    for (int i = 0; i < plant_->num_velocities(); ++i) xc_expected[i + plant_->num_positions()] *= -1;
    plant_->SetVelocities(context_.get(), xc_expected.tail(plant_->num_velocities()));
    EXPECT_EQ(plant_->GetVelocities(*context_), xc_expected.tail(plant_->num_velocities()));
    EXPECT_EQ(xc, xc_expected);
...
TEST_P(KukaArmTest, InstanceStateAccess) {
    ...
    multibody::ModelInstanceIndex arm1 = Parser(plant_.get(), "arm1").AddModelsFromUrl(kSdfUrl).at(0);
    multibody::ModelInstanceIndex arm2 = Parser(plant_.get(), "arm2").AddModelsFromUrl(kSdfUrl).at(0);
    plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", arm1));
    plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", arm2));
    plant_->Finalize();
    ...
    VectorX<double> q = VectorX<double>::LinSpaced(plant_->num_positions(arm2) /* size */, 1 /* first number */,
                                                   plant_->num_positions(arm2) /* last number */);
    VectorX<double> qd = VectorX<double>::LinSpaced(plant_->num_velocities(arm2) /* size */, 10 /* first number */,
    ...
    // altered.
    plant_->SetPositionsAndVelocities(context_.get(), VectorXd::Zero(plant_->num_multibody_states()));
    ...
    auto qv_block = x.head(q.size() + qd.size());
    {
        ...
        plant_->SetPositions(*context_, &context_->get_mutable_state(), arm2, q_block);
        plant_->SetVelocities(context_.get(), arm2, v_block);
        plant_->SetVelocities(*context_, &context_->get_mutable_state(), arm2, v_block);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->GetPositionsAndVelocities(*context_, arm2, &q_out),
                                "Output array is not properly sized.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->GetPositions(*context_, arm2, &qv_out), "Output array is not properly sized.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->GetVelocities(*context_, arm2, &qv_out), "Output array is not properly sized.");
...

    {
        // Ensure that getters accepting an output vector do not allocate heap.
        drake::test::LimitMalloc guard({.max_num_allocations = 0});
        plant_->GetPositionsFromArray(arm2, state_vector.head(plant_->num_positions()), &q_out_array);
        plant_->GetVelocitiesFromArray(arm2, state_vector.tail(plant_->num_velocities()), &v_out_array);
    }
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_->GetPositionsFromArray(arm2, state_vector.head(plant_->num_positions()), &q_out_array_err),
            "Output array is not properly sized.");
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_->GetVelocitiesFromArray(arm2, state_vector.tail(plant_->num_velocities()), &v_out_array_err),
            "Output array is not properly sized.");
}
...
INSTANTIATE_TEST_SUITE_P(Blank, KukaArmTest, testing::Values(0.0 /* continuous state */, 1e-3 /* discrete state */));

GTEST_TEST(StateSelection, JointHasNoActuator) {
    MultibodyPlant<double> plant(0.0);
    Parser(&plant).AddModelsFromUrl("package://drake/multibody/benchmarks/acrobot/acrobot.sdf");
    plant.Finalize();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.MakeActuatorSelectorMatrix(selected_joints),
                                "Joint 'ShoulderJoint' does not have an actuator.");
}

GTEST_TEST(StateSelection, KukaWithSimpleGripper) {
    const char kArmSdfUrl[] = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
    const char kWsg50SdfUrl[] = "package://drake_models/wsg_50_description/sdf/schunk_wsg_50.sdf";
    ...
    MultibodyPlant<double> plant(0.0);
    Parser parser(&plant);
    const ModelInstanceIndex arm_model = parser.AddModelsFromUrl(kArmSdfUrl).at(0);

    // Add the gripper.
    const ModelInstanceIndex gripper_model = parser.AddModelsFromUrl(kWsg50SdfUrl).at(0);
    ...
    const std::vector<std::string> arm_selected_joints_by_name = {"iiwa_joint_2", "iiwa_joint_7", "iiwa_joint_3"};
    ...
    const MatrixX<double> Sx_arm = plant.MakeStateSelectorMatrix(arm_selected_joints);

    // Actuation selector for the arm.
    const MatrixX<double> Su_arm = plant.MakeActuatorSelectorMatrix(arm_selected_joints);
    ...
    MatrixX<double> Sx_arm_expected = MatrixX<double>::Zero(num_selected_states, plant.num_multibody_states());
    ...
    // name.
    const MatrixX<double> Sx_arm_by_name = OldMakeStateSelectorMatrixFromJointNames(arm_selected_joints_by_name);
    ...
    const std::vector<std::string> repeated_joint_names = {"iiwa_joint_2", "iiwa_joint_3", "iiwa_joint_7",
                                                           "iiwa_joint_3"};
    DRAKE_EXPECT_THROWS_MESSAGE(OldMakeStateSelectorMatrixFromJointNames(repeated_joint_names),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.MakeStateSelectorMatrix(repeated_joint_indexes),
                                "Joint named 'iiwa_joint_3' is repeated multiple times.");

    // Verify the arm's actuation selector.
    MatrixX<double> Su_arm_expected = MatrixX<double>::Zero(plant.num_actuators(), 3);
    ...
    const auto& left_finger_sliding_joint = plant.GetJointByName("left_finger_sliding_joint");
    const auto& right_finger_sliding_joint = plant.GetJointByName("right_finger_sliding_joint");

    // We build a state selector for the gripper dofs.
    std::vector<JointIndex> gripper_selected_joints;
    gripper_selected_joints.push_back(left_finger_sliding_joint.index());   // user index = 0.
    gripper_selected_joints.push_back(right_finger_sliding_joint.index());  // user index = 1.

    // State selector for the griper.
    const MatrixX<double> Sx_gripper = plant.MakeStateSelectorMatrix(gripper_selected_joints);

    // Actuation selector for the gripper.
    const MatrixX<double> Su_gripper = plant.MakeActuatorSelectorMatrix(gripper_selected_joints);

    // Similarly we know the expected value for the gripper's selector matrix.
    MatrixX<double> Sx_gripper_expected = MatrixX<double>::Zero(4, plant.num_multibody_states());
    ...
    Sx_gripper_expected(2, plant.num_positions() + left_finger_sliding_joint.velocity_start()) = 1;
    // second joint velocity, right finger.
    Sx_gripper_expected(3, plant.num_positions() + right_finger_sliding_joint.velocity_start()) = 1;
    EXPECT_EQ(Sx_gripper, Sx_gripper_expected);

    // Verify the grippers's actuation selector.
    MatrixX<double> Su_gripper_expected = MatrixX<double>::Zero(plant.num_actuators(), 2);
    ...
    const MatrixX<double> Sx_from_empty_names = OldMakeStateSelectorMatrixFromJointNames(std::vector<std::string>());
    EXPECT_EQ(Sx_from_empty_names.rows(), 0);
    EXPECT_EQ(Sx_from_empty_names.cols(), plant.num_multibody_states());

    const MatrixX<double> Sx_from_empty_indexes = plant.MakeStateSelectorMatrix(std::vector<JointIndex>());
    EXPECT_EQ(Sx_from_empty_indexes.rows(), 0);
    EXPECT_EQ(Sx_from_empty_indexes.cols(), plant.num_multibody_states());

    const MatrixX<double> Su_from_empty_actuators = plant.MakeActuatorSelectorMatrix(std::vector<JointActuatorIndex>());
    EXPECT_EQ(Su_from_empty_actuators.rows(), plant.num_actuators());
    EXPECT_EQ(Su_from_empty_actuators.cols(), 0);

    const MatrixX<double> Su_from_empty_joints = plant.MakeActuatorSelectorMatrix(std::vector<JointIndex>());
    EXPECT_EQ(Su_from_empty_actuators.rows(), plant.num_actuators());
    EXPECT_EQ(Su_from_empty_actuators.cols(), 0);
    ...
    ASSERT_EQ(B.cols(), nu);
    // B is not invertible.
    const Eigen::SparseMatrix<double> Bplus = plant.MakeActuationMatrixPseudoinverse();
    ...
    auto B_iiwa = B_expected.block(6 /* skip floating base */, 0, 7 /* iiwa joints */, 7 /* iiwa actuators */);
    B_iiwa.setIdentity();

    // Fill in the block for the gripper's actuators.
    const auto& left_finger_actuator = plant.GetJointActuatorByName("left_finger_sliding_joint");
    const auto& right_finger_actuator = plant.GetJointActuatorByName("right_finger_sliding_joint");
    B_expected(left_finger_actuator.joint().velocity_start(), int{left_finger_actuator.index()}) = 1;
    B_expected(right_finger_actuator.joint().velocity_start(), int{right_finger_actuator.index()}) = 1;
...
GTEST_TEST(StateSelection, FloatingBodies) {
    ...
    const std::string mug_sdf_url = "package://drake/examples/simple_gripper/simple_mug.sdf";

    MultibodyPlant<double> plant(0.0);

    // Load a model of a table for the robot.
    Parser robot_parser(&plant, "robot");
    const ModelInstanceIndex robot_table_model = robot_parser.AddModelsFromUrl(table_sdf_url).at(0);
    plant.WeldFrames(plant.world_frame(), plant.GetFrameByName("link", robot_table_model));

    // Load the robot and weld it on top of the robot table.
    const ModelInstanceIndex arm_model = robot_parser.AddModelsFromUrl(iiwa_sdf_url).at(0);
    ...
    plant.WeldFrames(plant.world_frame(), plant.GetFrameByName("iiwa_link_0", arm_model), X_WLink0);

    // Load a second table for objects.
    Parser objects_parser(&plant, "objects");
    const ModelInstanceIndex objects_table_model = objects_parser.AddModelsFromUrl(table_sdf_url).at(0);
    const RigidTransformd X_WT(Vector3d(0.8, 0.0, 0.0));
    plant.WeldFrames(plant.world_frame(), plant.GetFrameByName("link", objects_table_model), X_WT);

    // Define a fixed frame on the -x, -y corner of the objects table.
    const RigidTransformd X_TO(RotationMatrixd::MakeXRotation(-M_PI_2), Vector3d(-0.3, -0.3, table_top_z_in_world));
    const auto& objects_frame_O = plant.AddFrame(std::make_unique<FixedOffsetFrame<double>>(
            "objects_frame", plant.GetFrameByName("link", objects_table_model), X_TO));
    ...
    plant.SetFreeBodyPoseInAnchoredFrame(context.get(), objects_frame_O, mug, X_OM);
    ...
    const double kTolerance = 5 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(X_WM.GetAsMatrix4(), X_WM_expected.GetAsMatrix4(), kTolerance,
                                MatrixCompareType::relative));

    // SetFreeBodyPoseInAnchoredFrame() should throw if the reference frame F is
    // not anchored to the world.
    const Frame<double>& end_effector_frame = plant.GetFrameByName("iiwa_link_7", arm_model);

    DRAKE_EXPECT_THROWS_MESSAGE(plant.SetFreeBodyPoseInAnchoredFrame(context.get(), end_effector_frame, mug, X_OM),
    ...
    const Eigen::SparseMatrix<double> Nplus = plant.MakeQDotToVelocityMap(*context);
...
GTEST_TEST(SetRandomTest, FloatingBodies) {
    ...
    // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
    const RigidBody<double>& body = plant.AddRigidBody("LoneBody", SpatialInertia<double>::MakeUnitary());
    plant.Finalize();

    RandomGenerator generator;

    std::uniform_real_distribution<symbolic::Expression> uniform;
    Vector3<symbolic::Expression> xyz(1.0 + uniform(generator), 2.0 + uniform(generator), 3.0 + uniform(generator));
...
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    ...
    const math::RigidTransform<double> X_WB_default = plant.GetFreeBodyPose(*context, body);
    ...
    // returned).
    EXPECT_FALSE(CompareMatrices(X_WB_default.rotation().matrix(), X_WB.rotation().matrix()));
    ...
    plant.SetFreeBodyRandomRotationDistribution(body, X_WB_new.cast<symbolic::Expression>().ToQuaternion());

    plant.SetRandomContext(context.get(), &generator);
    X_WB = plant.GetFreeBodyPose(*context, body);

    const double kTolerance = 5 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(X_WB_new.matrix(), X_WB.rotation().matrix(), kTolerance, MatrixCompareType::relative));
}
...
GTEST_TEST(SetRandomTest, SetDefaultWhenNoDistributionSpecified) {
    ...
    const RigidBody<double>& body0 = plant.AddRigidBody("free body 0", SpatialInertia<double>::MakeUnitary());
    // Add a rigid body that is "explicitly floating" (i.e. floating joint
    // explicitly added).
    const RigidBody<double>& body1 = plant.AddRigidBody("free body 1", SpatialInertia<double>::MakeUnitary());
    plant.AddJoint<QuaternionFloatingJoint>("" + body1.name(), plant.world_body(), {}, body1, {});
    const std::string acrobot_url = "package://drake/multibody/benchmarks/acrobot/acrobot.sdf";
    const ModelInstanceIndex acrobot = Parser(&plant).AddModelsFromUrl(acrobot_url)[0];
    plant.Finalize();

    // Set default positions through two different code paths.
    const Eigen::VectorXd acrobot_default_positions = Eigen::VectorXd::LinSpaced(2, 1.0, 2.0);  // 2 joints.
    plant.SetDefaultPositions(acrobot, acrobot_default_positions);
    const RigidTransformd X_WB0(RollPitchYawd(0.3, 0.4, 0.5), Vector3d(1.0, 2.0, 3.0));
    const RigidTransformd X_WB1(RollPitchYawd(0.1, 0.2, 0.3), Vector3d(2.0, 4.0, 6.0));
...
GTEST_TEST(MultibodyPlantTest, RigidBodyParameters) {
    ...
    const UnitInertia<double> sphere_unit_inertia = UnitInertia<double>::SolidSphere(sphere_radius);
    const RigidBody<double>& sphere =
            plant.AddRigidBody("sphere", SpatialInertia<double>(sphere_mass, sphere_com, sphere_unit_inertia));

    const double cube_length = 2.0;
    const double cube_mass = 5.0;
    const Vector3d cube_com(0, 0, 0);
    const UnitInertia<double> cube_unit_inertia = UnitInertia<double>::SolidCube(cube_length);
    const RigidBody<double>& cube =
            plant.AddRigidBody("cube", SpatialInertia<double>(cube_mass, cube_com, cube_unit_inertia));
    ...
    const RigidBody<double>& simple_2 = plant.AddRigidBody("simple_2", default_model_instance());
    ...
    const Vector3<double> sphere_com_in_context = sphere.CalcCenterOfMassInBodyFrame(*context);
    const SpatialInertia<double> sphere_inertia_in_context = sphere.CalcSpatialInertiaInBodyFrame(*context);

    const double cube_mass_in_context = cube.get_mass(*context);
    const Vector3<double> cube_com_in_context = cube.CalcCenterOfMassInBodyFrame(*context);
    const SpatialInertia<double> cube_inertia_in_context = cube.CalcSpatialInertiaInBodyFrame(*context);
    ...
    EXPECT_TRUE(CompareMatrices(sphere_inertia_in_context.get_unit_inertia().get_moments(),
                                sphere_unit_inertia.get_moments()));
    EXPECT_TRUE(CompareMatrices(sphere_inertia_in_context.get_unit_inertia().get_products(),
    ...
    EXPECT_TRUE(
            CompareMatrices(cube_inertia_in_context.get_unit_inertia().get_moments(), cube_unit_inertia.get_moments()));
    EXPECT_TRUE(CompareMatrices(cube_inertia_in_context.get_unit_inertia().get_products(),
    ...
    for (const auto* simple : {&simple_1, &simple_2}) {
        ...
        EXPECT_TRUE(simple->CalcSpatialInertiaInBodyFrame(*context).IsPhysicallyValid());
    }
    ...
    const UnitInertia<double> new_sphere_unit_inertia = UnitInertia<double>::SolidSphere(new_sphere_radius);

    const double new_cube_length = 1.3;
    const double new_cube_mass = 3.0;
    const Vector3d new_cube_com(0, 0, 0);
    const UnitInertia<double> new_cube_unit_inertia = UnitInertia<double>::SolidCube(new_cube_length);

    SpatialInertia<double> new_sphere_params(new_sphere_mass, new_sphere_com, new_sphere_unit_inertia);

    SpatialInertia<double> new_cube_params(new_cube_mass, new_cube_com, new_cube_unit_inertia);
    ...
    const Vector3<double> new_sphere_com_in_context = sphere.CalcCenterOfMassInBodyFrame(*context);
    const SpatialInertia<double> new_sphere_inertia_in_context = sphere.CalcSpatialInertiaInBodyFrame(*context);

    const double new_cube_mass_in_context = cube.get_mass(*context);
    const Vector3<double> new_cube_com_in_context = cube.CalcCenterOfMassInBodyFrame(*context);
    const SpatialInertia<double> new_cube_inertia_in_context = cube.CalcSpatialInertiaInBodyFrame(*context);
    ...
    EXPECT_TRUE(CompareMatrices(new_sphere_inertia_in_context.get_com(), new_sphere_com));
    EXPECT_TRUE(CompareMatrices(new_sphere_inertia_in_context.get_unit_inertia().get_moments(),
                                new_sphere_unit_inertia.get_moments()));
    EXPECT_TRUE(CompareMatrices(new_sphere_inertia_in_context.get_unit_inertia().get_products(),
    ...
    EXPECT_TRUE(CompareMatrices(new_cube_inertia_in_context.get_com(), new_cube_com));
    EXPECT_TRUE(CompareMatrices(new_cube_inertia_in_context.get_unit_inertia().get_moments(),
                                new_cube_unit_inertia.get_moments()));
    EXPECT_TRUE(CompareMatrices(new_cube_inertia_in_context.get_unit_inertia().get_products(),
...
GTEST_TEST(MultibodyPlantTest, AutoDiffAcrobotParameters) {
    ...
    const AcrobotParameters params(m1, m2, l1, l2, lc1, lc2, Ic1, Ic2, 0.0, 0.0, 9.81);
    unique_ptr<MultibodyPlant<double>> plant = MakeAcrobotPlant(params, true);

    // Create a default context and set state.
    unique_ptr<Context<double>> context = plant->CreateDefaultContext();

    const RevoluteJoint<double>& shoulder_joint = plant->GetJointByName<RevoluteJoint>(params.shoulder_joint_name());
    const RevoluteJoint<double>& elbow_joint = plant->GetJointByName<RevoluteJoint>(params.elbow_joint_name());
    shoulder_joint.set_angle(context.get(), 0.0);
    elbow_joint.set_angle(context.get(), 0.0);

    // Scalar convert the plant to AutoDiffXd.
    unique_ptr<MultibodyPlant<AutoDiffXd>> plant_autodiff = systems::System<double>::ToAutoDiffXd(*plant);
    unique_ptr<Context<AutoDiffXd>> context_autodiff = plant_autodiff->CreateDefaultContext();
    ...
    UnitInertia<AutoDiffXd> Gc1_Bcm = UnitInertia<AutoDiffXd>::StraightLine(Gc1_ad, Vector3d::UnitZ());
    SpatialInertia<AutoDiffXd> M1_L1o =
            SpatialInertia<AutoDiffXd>::MakeFromCentralInertia(m1_ad, p_L1L1cm, Gc1_Bcm * m1_ad);

    UnitInertia<AutoDiffXd> Gc2_Bcm = UnitInertia<AutoDiffXd>::StraightLine(Gc2_ad, Vector3d::UnitZ());
    SpatialInertia<AutoDiffXd> M2_L2o =
            SpatialInertia<AutoDiffXd>::MakeFromCentralInertia(m2_ad, p_L2L2cm, Gc2_Bcm * m2_ad);
    ...
    const FixedOffsetFrame<AutoDiffXd>& elbow_joint_parent_frame =
            dynamic_cast<const FixedOffsetFrame<AutoDiffXd>&>(elbow_joint_ad.frame_on_parent());

    const RigidTransform<AutoDiffXd> X_link1_Ei(-l1_ad * Vector3d::UnitZ());

    elbow_joint_parent_frame.SetPoseInParentFrame(context_autodiff.get(), X_link1_Ei);
  ...
  analytic_mass_matrix << I1 + I2 + m2*l1*l1 + 2*m2*l1*lc2, I2 + m2*l1*lc2,
                                            I2 + m2*l1*lc2,             I2;
    // clang-format on
    EXPECT_TRUE(CompareMatrices(mass_matrix, analytic_mass_matrix, kTolerance, MatrixCompareType::relative));
  ...
  analytic_mass_matrix_partial_m1 << (1.0 / 3.0) * l1 * l1, 0.0,
                                                       0.0, 0.0;
    // clang-format on
    EXPECT_TRUE(CompareMatrices(mass_matrix_grad.col(0), analytic_mass_matrix_partial_m1, kTolerance,
                                MatrixCompareType::relative));
  ...
  analytic_mass_matrix_partial_m2
    ...
    // clang-format on
    EXPECT_TRUE(CompareMatrices(mass_matrix_grad.col(1), analytic_mass_matrix_partial_m2, kTolerance,
                                MatrixCompareType::relative));
  ...
  analytic_mass_matrix_partial_l1
      << (2.0/3.0)*m1*l1 + 2*m2*l1 + 2*m2*lc2, m2*lc2,
                                       m2*lc2,      0;
    // clang-format on
    EXPECT_TRUE(CompareMatrices(mass_matrix_grad.col(2), analytic_mass_matrix_partial_l1, kTolerance,
                                MatrixCompareType::relative));
  ...
  analytic_mass_matrix_partial_l2 <<
    ...
    // clang-format on
    EXPECT_TRUE(CompareMatrices(mass_matrix_grad.col(3), analytic_mass_matrix_partial_l2, kTolerance,
                                MatrixCompareType::relative));
}
...
GTEST_TEST(MultibodyPlantTests, GetConstraintIds) {
    ...
    const RigidBody<double>& body_A = plant.AddRigidBody("body_A", SpatialInertia<double>::NaN());
    const RigidBody<double>& body_B = plant.AddRigidBody("body_B", SpatialInertia<double>::NaN());
    const RevoluteJoint<double>& world_A = plant.AddJoint<RevoluteJoint>("world_A", plant.world_body(), std::nullopt,
                                                                         body_A, std::nullopt, Vector3d::UnitZ());
    const RevoluteJoint<double>& A_B =
            plant.AddJoint<RevoluteJoint>("A_B", body_A, std::nullopt, body_B, std::nullopt, Vector3d::UnitZ());
    MultibodyConstraintId coupler_id = plant.AddCouplerConstraint(world_A, A_B, 2.3);
    MultibodyConstraintId distance_id =
            plant.AddDistanceConstraint(body_A, Vector3d(1.0, 2.0, 3.0), body_B, Vector3d(4.0, 5.0, 6.0), 2.0);
    MultibodyConstraintId ball_id =
            plant.AddBallConstraint(body_A, Vector3d(-1.0, -2.0, -3.0), body_B, Vector3d(-4.0, -5.0, -6.0));
    MultibodyConstraintId weld_id = plant.AddWeldConstraint(body_A, RigidTransformd(), body_B, RigidTransformd());

    std::vector<MultibodyConstraintId> ids = plant.GetConstraintIds();
    // The order of the constraints is not guaranteed.
    EXPECT_THAT(ids, testing::UnorderedElementsAre(coupler_id, distance_id, ball_id, weld_id));
...
GTEST_TEST(MultibodyPlantTests, ConstraintActiveStatus) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.set_discrete_contact_approximation(DiscreteContactApproximation::kTamsi),
                                ".*TAMSI does not support constraints.*");
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    DRAKE_EXPECT_THROWS_MESSAGE(plant.set_discrete_contact_solver(DiscreteContactSolver::kTamsi),
                                ".*TAMSI does not support constraints.*");
#pragma GCC diagnostic pop
...
GTEST_TEST(MultibodyPlantTests, RemoveConstraint) {
    ...
    // arbitrarily choose one model approximation that uses the SAP solver.
    plant.set_discrete_contact_approximation(DiscreteContactApproximation::kSap);
    const Body<double>& body_A = plant.AddRigidBody("body_A", SpatialInertia<double>::NaN());
    const Body<double>& body_B = plant.AddRigidBody("body_B", SpatialInertia<double>::NaN());
    const RevoluteJoint<double>& world_A = plant.AddJoint<RevoluteJoint>("world_A", plant.world_body(), std::nullopt,
                                                                         body_A, std::nullopt, Vector3d::UnitZ());
    const RevoluteJoint<double>& A_B =
            plant.AddJoint<RevoluteJoint>("A_B", body_A, std::nullopt, body_B, std::nullopt, Vector3d::UnitZ());
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.RemoveConstraint(coupler_id), ".*does not match any constraint.*");

    // Add a new coupler constraint
    MultibodyConstraintId coupler_id2 = plant.AddCouplerConstraint(world_A, A_B, 2.3);
    EXPECT_EQ(plant.num_coupler_constraints(), 1);

    plant.Finalize();

    DRAKE_EXPECT_THROWS_MESSAGE(plant.RemoveConstraint(coupler_id2),
...
GTEST_TEST(MultibodyPlantTests, FixedOffsetFrameFunctions) {
    ...
    const RigidTransformd X_WWp(RotationMatrixd::MakeZRotation(M_PI / 6), Vector3d(2, 2, 0));
    const RigidTransformd X_WpP(RotationMatrixd::MakeZRotation(M_PI / 12), Vector3d(0.1, 0.2, 0));
    RigidTransformd X_BP(RotationMatrixd::MakeXRotation(M_PI / 4), Vector3d(0, 0.3, 0.4));
    const Joint<double>& weld_joint =
            plant.AddJoint<WeldJoint>("weld_WB", plant.world_body(), X_WWp, body_B, X_BP, X_WpP);

    // Get a reference to the fixed offset frame P on body B.
    const FixedOffsetFrame<double>& frame_P =
            dynamic_cast<const FixedOffsetFrame<double>&>(weld_joint.frame_on_child());

    // Create a fixed offset frame F on body B whose parent frame is P.
    RigidTransformd X_PF(RotationMatrixd::MakeZRotation(M_PI / 3), Vector3d(0.9, 0.7, 0.3));
    const FixedOffsetFrame<double>& frame_F =
            plant.AddFrame(std::make_unique<FixedOffsetFrame<double>>("fixed_offset_frame_F", frame_P, X_PF));
    ...
    X_BP = RigidTransformd(RotationMatrixd::MakeXRotation(3 * M_PI / 4), Vector3d(0, 0.5, 0.8));
    ...
    X_PF = RigidTransformd(RotationMatrixd::MakeZRotation(2 * M_PI / 3), Vector3d(2, 2, 2));
...
GTEST_TEST(MultibodyPlant, FixInputPortsFrom) {
    ...
    Parser(&plant).AddModelsFromUrl("package://drake/multibody/plant/test/split_pendulum.sdf");
    plant.Finalize();
    auto diagram = builder.Build();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(autodiff_plant->FixInputPortsFrom(plant, plant_context, autodiff_context.get()),
...
GTEST_TEST(MultibodyPlantTests, ActuationPorts) {
    ...
    plant->get_actuation_input_port(default_model_instance()).FixValue(context1.get(), 1.0);
    plant->get_actuation_input_port(pendulum_model_instance).FixValue(context1.get(), 2.0);
    const auto& reaction_forces =
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context1);

    // Connect the actuation port for all instances.
    auto context2 = plant->CreateDefaultContext();
    plant->get_actuation_input_port().FixValue(context2.get(), Vector2d(1.0, 2.0));
    const auto& expected_reaction_forces =
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context2);
    ...
    for (int i = 0; i < static_cast<int>(reaction_forces.size()); ++i) {
        EXPECT_TRUE(CompareMatrices(reaction_forces[i].translational(), expected_reaction_forces[i].translational()));
        EXPECT_TRUE(CompareMatrices(reaction_forces[i].rotational(), expected_reaction_forces[i].rotational()));
        all_reaction_forces_zero &= (reaction_forces[i].rotational().norm() < kTolerance);
    }
...
GTEST_TEST(MultibodyPlantTests, AlgebraicLoopDetection) {
    ...
    for (double dt : time_steps) {
        systems::DiagramBuilder<double> builder;
        MultibodyPlant<double>* plant = builder.AddSystem<MultibodyPlant<double>>(dt);
        const char kSdfUrl[] = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
        Parser parser(plant);
        auto iiwa_instance = parser.AddModelsFromUrl(kSdfUrl).at(0);
        plant->Finalize();
        auto feedback = builder.AddSystem<systems::PassThrough<double>>(plant->num_velocities());
        builder.Connect(plant->get_generalized_contact_forces_output_port(iiwa_instance), feedback->get_input_port());
        builder.Connect(feedback->get_output_port(), plant->get_applied_generalized_force_input_port());
        std::unique_ptr<systems::Diagram<double>> diagram = builder.Build();
        std::unique_ptr<systems::Context<double>> diagram_context = diagram->CreateDefaultContext();
        const systems::Context<double>& plant_context = plant->GetMyContextFromRoot(*diagram_context);
        if (dt == 0.0) {
            EXPECT_NO_THROW(
                    plant->get_generalized_contact_forces_output_port(default_model_instance()).Eval(plant_context));
        } else {
            DRAKE_EXPECT_THROWS_MESSAGE(
                    plant->get_generalized_contact_forces_output_port(default_model_instance()).Eval(plant_context),
...
GTEST_TEST(MultibodyPlantTest, ThrowIfActuationPortContainsNaN) {
    const AcrobotParameters parameters;
    std::unique_ptr<MultibodyPlant<double>> plant = MakeAcrobotPlant(parameters, true /* Return a finalized plant */);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context),
...
GTEST_TEST(MultibodyPlantTest, ThrowIfModelInstanceActuationPortContainsNaN) {
    ...
    nan_vector.setConstant(std::numeric_limits<double>::quiet_NaN());
    auto context = plant->CreateDefaultContext();
    plant->get_actuation_input_port(default_model_instance()).FixValue(context.get(), nan_vector);
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context),
...
GTEST_TEST(MultibodyPlantTest, ThrowIfGeneralizedForcePortContainsNaN) {
    ...
    nan_vector.setConstant(std::numeric_limits<double>::quiet_NaN());
    auto context = plant->CreateDefaultContext();
    plant->get_applied_generalized_force_input_port().FixValue(context.get(), nan_vector);
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context),
...
GTEST_TEST(MultibodyPlantTest, ThrowIfSpatialForcePortContainsNaN) {
    ...
    nan_spatial_force[0].body_index = plant->GetBodyByName(parameters.link1_name()).index();
    auto context = plant->CreateDefaultContext();
    plant->get_applied_spatial_force_input_port().FixValue(context.get(), nan_spatial_force);
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context),
...
GTEST_TEST(MultibodyPlantTest, SetDefaultPositions) {
    ...
    const char kSdfUrl[] = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
    auto plant = std::make_unique<MultibodyPlant<double>>(0 /* plant type irrelevant */);
    multibody::ModelInstanceIndex iiwa0_instance = Parser(plant.get(), "iiwa0").AddModelsFromUrl(kSdfUrl).at(0);
    multibody::ModelInstanceIndex iiwa1_instance = Parser(plant.get(), "iiwa1").AddModelsFromUrl(kSdfUrl).at(0);
    // Weld iiwa0 to the world, leave iiwa1 to be floating.
    plant->WeldFrames(plant->world_frame(), plant->GetFrameByName("iiwa_link_0", iiwa0_instance));

    Eigen::VectorXd q = Eigen::VectorXd::LinSpaced(7 + 7 + 7, 1.0, 2.0);  // 7 joints each + 7 floating base positions.
    q.segment(7, 4).normalize();                                          // normalize the quaternion indices.

    // Throws if called pre-finalize.
    DRAKE_EXPECT_THROWS_MESSAGE(plant->SetDefaultPositions(q), ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->SetDefaultPositions(iiwa0_instance, q.head<7>()),
                                ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetDefaultPositions(), ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetDefaultPositions(iiwa0_instance), ".*you must call Finalize.* first.");
    plant->Finalize();

    // Throws if the q is the wrong size.
    DRAKE_EXPECT_THROWS_MESSAGE(plant->SetDefaultPositions(q.head<3>()), ".*num_positions.*");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->SetDefaultPositions(iiwa1_instance, q.head<3>()),
                                ".*q_instance.size.* == num_positions.*");

    const double kTol = 1e-15;

    EXPECT_FALSE(CompareMatrices(plant->GetDefaultPositions(), q, kTol));
    EXPECT_TRUE(CompareMatrices(plant->GetDefaultPositions(), plant->GetPositions(*plant->CreateDefaultContext())));
    ...
    EXPECT_TRUE(CompareMatrices(plant->GetPositions(*context, iiwa0_instance), q.head<7>()));
    EXPECT_TRUE(CompareMatrices(plant->GetPositions(*context, iiwa1_instance), q.tail<14>(), kTol));

    // Change q, and now verify the model_instance variant.
    q = Eigen::VectorXd::LinSpaced(7 + 7 + 7, 3.0, 4.0);  // 7 joints each + 7 floating base positions.
    q.segment(7, 4).normalize();                          // normalize the quaternion indices.

    EXPECT_FALSE(CompareMatrices(plant->GetDefaultPositions(iiwa0_instance), q.head<7>(), kTol));
    plant->SetDefaultPositions(iiwa0_instance, q.head<7>());
    EXPECT_TRUE(CompareMatrices(plant->GetDefaultPositions(iiwa0_instance), q.head<7>(), kTol));
    EXPECT_TRUE(CompareMatrices(plant->GetDefaultPositions(), plant->GetPositions(*plant->CreateDefaultContext())));

    EXPECT_FALSE(CompareMatrices(plant->GetDefaultPositions(iiwa1_instance), q.tail<14>(), kTol));
    plant->SetDefaultPositions(iiwa1_instance, q.tail<14>());
    EXPECT_TRUE(CompareMatrices(plant->GetDefaultPositions(iiwa1_instance), q.tail<14>(), kTol));
    EXPECT_TRUE(CompareMatrices(plant->GetDefaultPositions(), plant->GetPositions(*plant->CreateDefaultContext())));
...
GTEST_TEST(MultibodyPlantTest, GetNames) {
    ...
    // Throws if called pre-finalize.
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetPositionNames(), ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetVelocityNames(), ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetStateNames(), ".*you must call Finalize.* first.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant->GetActuatorNames(), ".*you must call Finalize.* first.");
    plant->Finalize();
    ...
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_joint_1_q", "iiwa_joint_2_q", "iiwa_joint_3_q", "iiwa_joint_4_q",
                                           "iiwa_joint_5_q", "iiwa_joint_6_q", "iiwa_joint_7_q"}));

    names = plant->GetPositionNames(iiwa0_instance, true /* add_model_instance_prefix */, true /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa0::iiwa14_iiwa_joint_1_q", "iiwa0::iiwa14_iiwa_joint_2_q",
    ...
    names = plant->GetPositionNames(iiwa0_instance, true /* add_model_instance_prefix */,
                                    false /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa0::iiwa14_iiwa_joint_1", "iiwa0::iiwa14_iiwa_joint_2",
    ...
    names = plant->GetPositionNames(iiwa0_instance, false /* add_model_instance_prefix */,
    ...
    names = plant->GetPositionNames(iiwa0_instance, false /* add_model_instance_prefix */,
                                    false /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa_joint_1", "iiwa_joint_2", "iiwa_joint_3", "iiwa_joint_4",
                                                  "iiwa_joint_5", "iiwa_joint_6", "iiwa_joint_7"}));

    names = plant->GetPositionNames(iiwa1_instance);
    EXPECT_THAT(names, testing::ElementsAreArray(
                               {"iiwa_link_0_qw", "iiwa_link_0_qx", "iiwa_link_0_qy", "iiwa_link_0_qz", "iiwa_link_0_x",
                                "iiwa_link_0_y", "iiwa_link_0_z", "iiwa_joint_1_q", "iiwa_joint_2_q", "iiwa_joint_3_q",
                                "iiwa_joint_4_q", "iiwa_joint_5_q", "iiwa_joint_6_q", "iiwa_joint_7_q"}));

    names = plant->GetPositionNames();
    EXPECT_THAT(
            names,
            testing::ElementsAreArray(
                    {"iiwa0::iiwa14_iiwa_joint_1_q", "iiwa0::iiwa14_iiwa_joint_2_q", "iiwa0::iiwa14_iiwa_joint_3_q",
                     "iiwa0::iiwa14_iiwa_joint_4_q", "iiwa0::iiwa14_iiwa_joint_5_q", "iiwa0::iiwa14_iiwa_joint_6_q",
                     "iiwa0::iiwa14_iiwa_joint_7_q", "iiwa1::iiwa14_iiwa_link_0_qw", "iiwa1::iiwa14_iiwa_link_0_qx",
                     "iiwa1::iiwa14_iiwa_link_0_qy", "iiwa1::iiwa14_iiwa_link_0_qz", "iiwa1::iiwa14_iiwa_link_0_x",
                     "iiwa1::iiwa14_iiwa_link_0_y",  "iiwa1::iiwa14_iiwa_link_0_z",  "iiwa1::iiwa14_iiwa_joint_1_q",
                     "iiwa1::iiwa14_iiwa_joint_2_q", "iiwa1::iiwa14_iiwa_joint_3_q", "iiwa1::iiwa14_iiwa_joint_4_q",
                     "iiwa1::iiwa14_iiwa_joint_5_q", "iiwa1::iiwa14_iiwa_joint_6_q", "iiwa1::iiwa14_iiwa_joint_7_q"}));

    names = plant->GetPositionNames(false /* add_model_instance_prefix */, false /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray(
                               {"iiwa_joint_1",   "iiwa_joint_2",  "iiwa_joint_3",   "iiwa_joint_4",   "iiwa_joint_5",
                                "iiwa_joint_6",   "iiwa_joint_7",  "iiwa_link_0_qw", "iiwa_link_0_qx", "iiwa_link_0_qy",
                                "iiwa_link_0_qz", "iiwa_link_0_x", "iiwa_link_0_y",  "iiwa_link_0_z",  "iiwa_joint_1",
                                "iiwa_joint_2",   "iiwa_joint_3",  "iiwa_joint_4",   "iiwa_joint_5",   "iiwa_joint_6",
                                "iiwa_joint_7"}));

    names = plant->GetVelocityNames(iiwa0_instance);
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_joint_1_w", "iiwa_joint_2_w", "iiwa_joint_3_w", "iiwa_joint_4_w",
                                           "iiwa_joint_5_w", "iiwa_joint_6_w", "iiwa_joint_7_w"}));

    names = plant->GetVelocityNames(iiwa0_instance, true /* add_model_instance_prefix */, true /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa0::iiwa14_iiwa_joint_1_w", "iiwa0::iiwa14_iiwa_joint_2_w",
    ...
    names = plant->GetVelocityNames(iiwa0_instance, true /* add_model_instance_prefix */,
                                    false /* always_add_suffix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa0::iiwa14_iiwa_joint_1", "iiwa0::iiwa14_iiwa_joint_2",
                                                  "iiwa0::iiwa14_iiwa_joint_3", "iiwa0::iiwa14_iiwa_joint_4",
                                                  "iiwa0::iiwa14_iiwa_joint_5", "iiwa0::iiwa14_iiwa_joint_6",
    ...
    names = plant->GetVelocityNames(iiwa0_instance, false /* add_model_instance_prefix */,
                                    true /* always_add_suffix */);
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_joint_1_w", "iiwa_joint_2_w", "iiwa_joint_3_w", "iiwa_joint_4_w",
                                           "iiwa_joint_5_w", "iiwa_joint_6_w", "iiwa_joint_7_w"}));
    ...
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_link_0_wx", "iiwa_link_0_wy", "iiwa_link_0_wz", "iiwa_link_0_vx",
                                           "iiwa_link_0_vy", "iiwa_link_0_vz", "iiwa_joint_1_w", "iiwa_joint_2_w",
                                           "iiwa_joint_3_w", "iiwa_joint_4_w", "iiwa_joint_5_w", "iiwa_joint_6_w",
                                           "iiwa_joint_7_w"}));

    names = plant->GetVelocityNames();
    EXPECT_THAT(names,
                testing::ElementsAreArray(
                        {"iiwa0::iiwa14_iiwa_joint_1_w", "iiwa0::iiwa14_iiwa_joint_2_w", "iiwa0::iiwa14_iiwa_joint_3_w",
                         "iiwa0::iiwa14_iiwa_joint_4_w", "iiwa0::iiwa14_iiwa_joint_5_w", "iiwa0::iiwa14_iiwa_joint_6_w",
                         "iiwa0::iiwa14_iiwa_joint_7_w", "iiwa1::iiwa14_iiwa_link_0_wx", "iiwa1::iiwa14_iiwa_link_0_wy",
                         "iiwa1::iiwa14_iiwa_link_0_wz", "iiwa1::iiwa14_iiwa_link_0_vx", "iiwa1::iiwa14_iiwa_link_0_vy",
                         "iiwa1::iiwa14_iiwa_link_0_vz", "iiwa1::iiwa14_iiwa_joint_1_w", "iiwa1::iiwa14_iiwa_joint_2_w",
                         "iiwa1::iiwa14_iiwa_joint_3_w", "iiwa1::iiwa14_iiwa_joint_4_w", "iiwa1::iiwa14_iiwa_joint_5_w",
                         "iiwa1::iiwa14_iiwa_joint_6_w", "iiwa1::iiwa14_iiwa_joint_7_w"}));

    names = plant->GetVelocityNames(false /* add_model_instance_prefix */, false /* always_add_suffix */);
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_joint_1",   "iiwa_joint_2",   "iiwa_joint_3",   "iiwa_joint_4",
                                           "iiwa_joint_5",   "iiwa_joint_6",   "iiwa_joint_7",   "iiwa_link_0_wx",
                                           "iiwa_link_0_wy", "iiwa_link_0_wz", "iiwa_link_0_vx", "iiwa_link_0_vy",
                                           "iiwa_link_0_vz", "iiwa_joint_1",   "iiwa_joint_2",   "iiwa_joint_3",
                                           "iiwa_joint_4",   "iiwa_joint_5",   "iiwa_joint_6",   "iiwa_joint_7"}));

    names = plant->GetStateNames(iiwa0_instance);
    EXPECT_THAT(names,
                testing::ElementsAreArray({"iiwa_joint_1_q", "iiwa_joint_2_q", "iiwa_joint_3_q", "iiwa_joint_4_q",
                                           "iiwa_joint_5_q", "iiwa_joint_6_q", "iiwa_joint_7_q", "iiwa_joint_1_w",
                                           "iiwa_joint_2_w", "iiwa_joint_3_w", "iiwa_joint_4_w", "iiwa_joint_5_w",
                                           "iiwa_joint_6_w", "iiwa_joint_7_w"}));

    names = plant->GetStateNames(iiwa0_instance, true /* add_model_instance_prefix */);
    EXPECT_THAT(names,
                testing::ElementsAreArray(
                        {"iiwa0::iiwa14_iiwa_joint_1_q", "iiwa0::iiwa14_iiwa_joint_2_q", "iiwa0::iiwa14_iiwa_joint_3_q",
                         "iiwa0::iiwa14_iiwa_joint_4_q", "iiwa0::iiwa14_iiwa_joint_5_q", "iiwa0::iiwa14_iiwa_joint_6_q",
                         "iiwa0::iiwa14_iiwa_joint_7_q", "iiwa0::iiwa14_iiwa_joint_1_w", "iiwa0::iiwa14_iiwa_joint_2_w",
                         "iiwa0::iiwa14_iiwa_joint_3_w", "iiwa0::iiwa14_iiwa_joint_4_w", "iiwa0::iiwa14_iiwa_joint_5_w",
    ...
    EXPECT_THAT(names,
                testing::ElementsAreArray(
                        {"iiwa_joint_1_q", "iiwa_joint_2_q", "iiwa_joint_3_q", "iiwa_joint_4_q", "iiwa_joint_5_q",
                         "iiwa_joint_6_q", "iiwa_joint_7_q", "iiwa_link_0_qw", "iiwa_link_0_qx", "iiwa_link_0_qy",
                         "iiwa_link_0_qz", "iiwa_link_0_x",  "iiwa_link_0_y",  "iiwa_link_0_z",  "iiwa_joint_1_q",
                         "iiwa_joint_2_q", "iiwa_joint_3_q", "iiwa_joint_4_q", "iiwa_joint_5_q", "iiwa_joint_6_q",
                         "iiwa_joint_7_q", "iiwa_joint_1_w", "iiwa_joint_2_w", "iiwa_joint_3_w", "iiwa_joint_4_w",
                         "iiwa_joint_5_w", "iiwa_joint_6_w", "iiwa_joint_7_w", "iiwa_link_0_wx", "iiwa_link_0_wy",
                         "iiwa_link_0_wz", "iiwa_link_0_vx", "iiwa_link_0_vy", "iiwa_link_0_vz", "iiwa_joint_1_w",
                         "iiwa_joint_2_w", "iiwa_joint_3_w", "iiwa_joint_4_w", "iiwa_joint_5_w", "iiwa_joint_6_w",
                         "iiwa_joint_7_w"}));
    ...
    names = plant->GetActuatorNames(iiwa0_instance, true /* add_model_instance_prefix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa0::iiwa14_iiwa_joint_1", "iiwa0::iiwa14_iiwa_joint_2",
                                                  "iiwa0::iiwa14_iiwa_joint_3", "iiwa0::iiwa14_iiwa_joint_4",
                                                  "iiwa0::iiwa14_iiwa_joint_5", "iiwa0::iiwa14_iiwa_joint_6",
                                                  "iiwa0::iiwa14_iiwa_joint_7"}));
    ...
    EXPECT_THAT(names,
                testing::ElementsAreArray(
                        {"iiwa0::iiwa14_iiwa_joint_1", "iiwa0::iiwa14_iiwa_joint_2", "iiwa0::iiwa14_iiwa_joint_3",
                         "iiwa0::iiwa14_iiwa_joint_4", "iiwa0::iiwa14_iiwa_joint_5", "iiwa0::iiwa14_iiwa_joint_6",
                         "iiwa0::iiwa14_iiwa_joint_7", "iiwa1::iiwa14_iiwa_joint_1", "iiwa1::iiwa14_iiwa_joint_2",
                         "iiwa1::iiwa14_iiwa_joint_3", "iiwa1::iiwa14_iiwa_joint_4", "iiwa1::iiwa14_iiwa_joint_5",
                         "iiwa1::iiwa14_iiwa_joint_6", "iiwa1::iiwa14_iiwa_joint_7"}));

    names = plant->GetActuatorNames(false /* add_model_instance_prefix */);
    EXPECT_THAT(names, testing::ElementsAreArray({"iiwa_joint_1", "iiwa_joint_2", "iiwa_joint_3", "iiwa_joint_4",
                                                  "iiwa_joint_5", "iiwa_joint_6", "iiwa_joint_7", "iiwa_joint_1",
                                                  "iiwa_joint_2", "iiwa_joint_3", "iiwa_joint_4", "iiwa_joint_5",
...
GTEST_TEST(MultibodyPlantTest, FloatingJointNames) {
    {
        MultibodyPlant<double> plant(0.0);
        plant.AddRigidBody("free_body", default_model_instance(), SpatialInertia<double>::MakeUnitary());
        plant.Finalize();
    ...
    // until the floating joint name is unique.
    {
        ...
        const RigidBody<double>& base_body =
                plant.AddRigidBody("base_body", default_model_instance(), SpatialInertia<double>::MakeUnitary());
        const RigidBody<double>& body2 =
                plant.AddRigidBody("body2", default_model_instance(), SpatialInertia<double>::MakeUnitary());
        const RigidBody<double>& body3 =
                plant.AddRigidBody("body3", default_model_instance(), SpatialInertia<double>::MakeUnitary());

        // Add joints with very unfortunate names.
        plant.AddJoint<RevoluteJoint>("base_body", base_body, {}, body2, {}, Vector3<double>(0, 0, 1));
        plant.AddJoint<RevoluteJoint>("_base_body", base_body, {}, body3, {}, Vector3<double>(0, 0, 1));
        plant.Finalize();
...
GTEST_TEST(MultibodyPlantTest, GetMutableSceneGraphPreFinalize) {
    MultibodyPlant<double> plant(0.0);
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetMutableSceneGraphPreFinalize(), ".*geometry_source_is_registered.*");
    ...
    plant.Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetMutableSceneGraphPreFinalize(), ".*!is_finalized.*");
}
...
GTEST_TEST(MultibodyPlantTest, RenameModelInstance) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.RenameModelInstance(ModelInstanceIndex(99), "invalid"),
...
<robot name="a">
  <link name="b">
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.RenameModelInstance(models[0], "a"), ".*must be unique.*");
    ...
    plant.Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(plant.RenameModelInstance(models[0], "too_late"), ".*finalized.*");
}
...
GTEST_TEST(MultibodyPlantTests, DiscreteContactApproximation) {
    ...
    auto set_solver_and_check_approximation = [&plant](DiscreteContactSolver solver) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
...
#pragma GCC diagnostic pop
        ...
        if (solver == DiscreteContactSolver::kTamsi) {
            ...
            EXPECT_EQ(plant.get_discrete_contact_approximation(), DiscreteContactApproximation::kTamsi);
        } else {
            // SAP can solve all approximations other than TAMSI.
            EXPECT_EQ(plant.get_discrete_contact_approximation(), DiscreteContactApproximation::kSap);
        }
    };

    auto set_approximation_and_check_solver = [&plant](DiscreteContactApproximation approximation) {
        ...
        if (approximation == DiscreteContactApproximation::kTamsi) {
            ...
            EXPECT_EQ(plant.get_discrete_contact_solver(), DiscreteContactSolver::kTamsi);
        } else {
