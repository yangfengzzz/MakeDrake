...
GTEST_TEST(PhysicalModelCollectionTest, EmptyCollection) {
    ...
    EXPECT_NO_THROW(model_collection.CloneToScalar<symbolic::Expression>(&symbolic_plant));
}
...
GTEST_TEST(PhysicalModelCollectionTest, NonEmptyDeformableModel) {
    ...
    DeformableModel<double>& deformable_model =
            model_collection.AddDeformableModel(std::make_unique<DeformableModel<double>>(&plant));

    /* Add a deformable body to the DeformableModel. */
    auto geometry = std::make_unique<geometry::GeometryInstance>(math::RigidTransformd{},
                                                                 std::make_unique<geometry::Sphere>(1.0), "sphere");
    deformable_model.RegisterDeformableBody(std::move(geometry), fem::DeformableBodyConfig<double>{}, 1.0);
...
GTEST_TEST(PhysicalModelCollectionTest, IncompatibleModel) {
    ...
    auto model_with_wrong_plant = std::make_unique<DummyPhysicalModel<double>>(&another_plant);
    ...
    EXPECT_THROW(model_collection.AddDummyModel(std::move(model_with_wrong_plant)), std::exception);
    DRAKE_EXPECT_THROWS_MESSAGE(model_collection.AddDummyModel(std::move(finalized_model)),
...
GTEST_TEST(PhysicalModelCollectionTest, DeclareSceneGraphPorts) {
    ...
    model_collection.AddDummyModel(std::make_unique<DummyPhysicalModel<double>>(&plant));
    const DummyPhysicalModel<double>& dummy_model = *model_collection.dummy_model();
    DRAKE_ASSERT_THROWS_MESSAGE(dummy_model.GetSceneGraphPortOrThrow(), ".*SceneGraph.*port.*not.*declared.*");
