...
GTEST_TEST(SlicingAndIndexing, ExcludeCols) {
    ...
    // Test dense MatrixBlock variant.
    {
        ...
        const contact_solvers::internal::MatrixBlock<double> S = ExcludeCols(M_block, indices);
        // clang-format off
    const MatrixXd S_expected = (MatrixXd(6, 2) <<
      1, 13,
      2, 14,
...
GTEST_TEST(SlicingAndIndexing, ExpandRows) {
    ...
    const VectorXd S_expected = (VectorXd(expanded_size) << 0, 1, 0, 2, 3, 0, 0, 0).finished();
    EXPECT_EQ(S, S_expected);
}
