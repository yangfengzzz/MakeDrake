...
public:
    ...
    void SetUp() override {
        ...
        frame_H_ = &plant_->AddFrame(std::make_unique<FixedOffsetFrame<double>>("H", *end_effector_link_, X_EH_));
        plant_->Finalize();
    ...
    void SetState(const VectorX<double>& x_joints) {
        ...
        for (JointIndex joint_index : plant_->GetJointIndices()) {
            ...
            const RevoluteJoint<double>& joint =
                    dynamic_cast<const RevoluteJoint<double>&>(plant_->get_joint(joint_index));
            joint.set_angle(context_.get(), x_joints[joint.ordinal()]);
            joint.set_angular_rate(context_.get(), x_joints[kNumRevoluteJoints + joint.ordinal()]);
        }

        // Set an arbitrary (though non-identity) pose of the floating base link.
        const auto& base_body = plant_->GetBodyByName("iiwa_link_0");
        const RigidTransform<double> X_WB(RollPitchYaw<double>(M_PI / 3, -M_PI / 2, M_PI / 8),
                                          Vector3<double>(0.05, -0.2, 0.05));
        plant_->SetFreeBodyPoseInAnchoredFrame(context_.get(), plant_->world_frame(), base_body, X_WB);
...
protected:
    ...
    const RigidTransform<double> X_EH_{RollPitchYaw<double>{M_PI_2, 0, M_PI_2}.ToRotationMatrix(),
