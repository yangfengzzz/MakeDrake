...
public:
    ...
    static VectorX<double> CalcGeneralizedAccelerations(const MultibodyPlant<double>& plant,
...
protected:
    ...
    void CalcForwardDynamicsViaArticulatedBodyAlgorithm(const Eigen::Ref<const VectorX<double>>& q,
        ...
        x << q, v;
        SetState(x);
        *vdot = MultibodyPlantTester::CalcGeneralizedAccelerations(*plant_, *context_);
    }
    ...
    // @param[in] v robot's joint velocities (generalized velocities).
    // @param[out] vdot generalized accelerations (1st derivative of v).
    void CalcForwardDynamicsViaMassMatrixSolve(const Eigen::Ref<const VectorX<double>>& q,
                                               const Eigen::Ref<const VectorX<double>>& v,
                                               EigenPtr<VectorX<double>> vdot) {
        DRAKE_DEMAND(vdot != nullptr);
        // Update joint positions and velocities.
        ...
        const VectorX<double> tau_id = plant_->CalcInverseDynamics(*context_, zero_vdot, forces);
    ...
    void CompareForwardDynamics(const Eigen::Ref<const VectorX<double>>& q,
        ...
        EXPECT_TRUE(CompareMatrices(vdot, vdot_expected, kRelativeTolerance, MatrixCompareType::relative));
    }
};
...
GTEST_TEST(MultibodyPlantForwardDynamics, AtlasRobot) {
    ...
    auto atlas_instance = Parser(&plant).AddModelsFromUrl("package://drake_models/atlas/atlas_convex_hull.urdf").at(0);
    plant.Finalize();
    ...
    plant.get_actuation_input_port(atlas_instance).FixValue(context.get(), VectorX<double>::Zero(num_actuators));
    ...
    EXPECT_TRUE(CompareMatrices(residual, Eigen::VectorXd::Zero(plant.num_multibody_states()), 6e-13));
}
...
GTEST_TEST(WeldedBoxesTest, ForwardDynamicsViaArticulatedBodyAlgorithm) {
    ...
    const VectorXd vdot = MultibodyPlantTester::CalcGeneralizedAccelerations(plant, *context);
    EXPECT_EQ(vdot.size(), 0);
}

std::unique_ptr<systems::LinearSystem<double>> MakeLinearizedCartPole(double time_step) {
    MultibodyPlant<double> plant(time_step);
    Parser(&plant).AddModelsFromUrl("package://drake/examples/multibody/cart_pole/cart_pole.sdf");
    plant.Finalize();

    auto context = plant.CreateDefaultContext();
    plant.get_actuation_input_port().FixValue(context.get(), 0.);
    plant.SetPositionsAndVelocities(context.get(), Eigen::Vector4d{0, M_PI, 0, 0});

    return systems::Linearize(plant, *context, plant.get_actuation_input_port().get_index(),
...
GTEST_TEST(MultibodyPlantTest, CartPoleLinearization) {
    ...
    A_expected.bottomRows<2>() += kTimeStep * ct_linearization->A().bottomRows<2>();
    A_expected.topRows<2>() += kTimeStep * A_expected.bottomRows<2>();
    Eigen::Vector4d B_expected;
    B_expected.bottomRows<2>() = kTimeStep * ct_linearization->B().bottomRows<2>();
...
const RigidBody<double>& AddBoxLink(MultibodyPlant<double>* plant,
    ...
    const UnitInertia<double> G_BBcm_B = UnitInertia<double>::SolidBox(Lx, Ly, Lz);
...
const RigidBody<double>& AddCubicalLink(MultibodyPlant<double>* plant,
                                        const std::string& body_name,
                                        const double mass,
                                        const double length,
                                        const bool skip_validity_check = false) {
...
protected:
    ...
    const RevoluteJoint<double>* AddJointToTestPlant(const std::string& joint_type_name,
                                                     const RigidBody<double>& bodyA,
                                                     const RigidTransform<double>& X_AF,
                                                     const RigidBody<double>& bodyB) {
        const RevoluteJoint<double>* joint{nullptr};
        const std::string joint_name = bodyA.name() + "_" + bodyB.name() + "_" + joint_type_name;
        const RigidTransform<double> X_BM;  // Identity rigid transform.
        if (joint_type_name == PrismaticJoint<double>::kTypeName) {
            plant_.AddJoint<PrismaticJoint>(joint_name, bodyA, X_AF, bodyB, X_BM, Vector3<double>::UnitX());
        } else if (joint_type_name == RevoluteJoint<double>::kTypeName) {
            joint = &plant_.AddJoint<RevoluteJoint>(joint_name, bodyA, X_AF, bodyB, X_BM, Vector3<double>::UnitZ());
...
GTEST_TEST(JointLocking, PendulumAccelerationTest) {
    ...
    const RigidBody<double>& bodyA = AddBoxLink(&plant, "bodyA", mA, lAx, lAy, lAz);
    const RigidBody<double>& bodyB = AddBoxLink(&plant, "bodyB", mB, lBx, lBy, lBz);
    const RigidBody<double>& bodyC = AddBoxLink(&plant, "bodyC", mC, lCx, lCy, lCz);

    // Joints for the locked double pendulum.
    const RigidTransform<double> X_WF;
    const RigidTransform<double> X_AM;
    const RevoluteJoint<double>& shoulder_AB = plant.AddJoint<RevoluteJoint>("shoulder_AB", plant.world_body(), X_WF,
    ...
    const RevoluteJoint<double>& elbow_AB =
            plant.AddJoint<RevoluteJoint>("elbow_AB", bodyA, X_AF, bodyB, X_BM, Vector3<double>::UnitY());

    // Joint for the single pendulum.
    const RigidTransform<double> X_CM;
    const RevoluteJoint<double>& shoulder_C = plant.AddJoint<RevoluteJoint>("shoulder_C", plant.world_body(), X_WF,
    ...
    {
        const VectorX<double> v_dot = MultibodyPlantTester::CalcGeneralizedAccelerations(plant, *context);
        ...
        // other.
        EXPECT_NEAR(v_dot[shoulder_AB.velocity_start()], v_dot[shoulder_C.velocity_start()], kEpsilon);
    }
...
GTEST_TEST(JointLocking, FreebodyAccelerationTest) {
    ...
    const SpatialInertia<double> M_BBo_B = SpatialInertia<double>::SolidSphereWithMass(mB, rB);
    ...
    // Calculate unlocked then locked, same context.
    {
        ...
        const SpatialAcceleration<double> A_WB = plant.EvalBodySpatialAccelerationInWorld(*context, bodyB);
