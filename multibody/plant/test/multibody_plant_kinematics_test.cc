...
public:
    ...
    void SetUp() override {
        ...
        const SpatialInertia<double> link_central_inertia =
                SpatialInertia<double>::ThinRodWithMass(link_mass, link_length_, Vector3<double>::UnitX());
        ...
        joint2_ = &plant_.AddJoint<RevoluteJoint>("PinJoint2", *bodyA_, math::RigidTransformd(p_AoAf_A_), *bodyB_,
    ...
    SpatialAcceleration<double> CalcApSpatialAccelerationInWExpressedInA(const double wAzdot,
                                                                         const Vector3<double>& p_AoAp_A) {
        ...
        const Vector3<double> a_WAp_A = alpha_WA_A.cross(p_AmAp_A) + w_WA_A.cross(w_WA_A.cross(p_AmAp_A));
    ...
    SpatialAcceleration<double> CalcBpSpatialAccelerationInAExpressedInA(const double wBzdot,
                                                                         const Vector3<double>& p_BoBp_B) {
        ...
        const math::RotationMatrix<double> R_AB = math::RotationMatrix<double>::MakeZRotation(qB_);
        const Vector3<double> p_BmBp_A = R_AB * p_BmBp_B;

        // a_ABp_A = alpha_AB_A x p_BmBp_A + w_AB_A x (w_AB_A x p_BmBp_A).
        const Vector3<double> a_ABp_A = alpha_AB_A.cross(p_BmBp_A) + w_AB_A.cross(w_AB_A.cross(p_BmBp_A));
    ...
    // param[in] p_BoBp_B point Bp's position from point Bo (link B's centroid).
    SpatialAcceleration<double> CalcBpSpatialAccelerationInWExpressedInA(const VectorX<double>& vdot,
                                                                         const Vector3<double>& p_BoBp_B) {
        const SpatialAcceleration<double> A_WAf_A = CalcApSpatialAccelerationInWExpressedInA(vdot[0], p_AoAf_A_);
        ...
        const Vector3<double> a_WBp_A = a_WAf_A + alpha_WB_A.cross(p_BmBp_A) + w_WB_A.cross(w_WB_A.cross(p_BmBp_A));
...
TEST_F(TwoDOFPlanarPendulumTest, CalcBiasSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> ABias_WAp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
    ...
    const SpatialAcceleration<double> ABias_WAp_A_expected = CalcApSpatialAccelerationInWExpressedInA(0.0, p_AoAp_A);
    EXPECT_TRUE(CompareMatrices(ABias_WAp_A_expected.get_coeffs(), ABias_WAp_A.get_coeffs(), kTolerance));
    ...
    const SpatialAcceleration<double> ABias_WAp_A_alternate =
            plant_.CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_A, p_AoAp_A, frame_W, frame_A);
    EXPECT_TRUE(CompareMatrices(ABias_WAp_A_alternate.get_coeffs(), ABias_WAp_A.get_coeffs(), kTolerance));
    ...
    const SpatialAcceleration<double> ABias_WBp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
    ...
    const SpatialAcceleration<double> ABias_WBp_A_expected = CalcBpSpatialAccelerationInWExpressedInA(vdot, p_BoBp_B);
    EXPECT_TRUE(CompareMatrices(ABias_WBp_A_expected.get_coeffs(), ABias_WBp_A.get_coeffs(), kTolerance));

    // Verify MultibodyPlant::CalcBiasSpatialAcceleration() results match those
    // of test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation().
    const SpatialAcceleration<double> ABias_WBp_A_alternate =
            plant_.CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_B, p_BoBp_B, frame_W, frame_A);
    EXPECT_TRUE(CompareMatrices(ABias_WBp_A_alternate.get_coeffs(), ABias_WBp_A.get_coeffs(), kTolerance));

    // Point Ap is an arbitrary point of link A.
    // Calculate Ap's bias spatial acceleration in frame A and ensure it is zero.
    const SpatialAcceleration<double> ABias_AAp_W = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_A, p_AoAp_A, frame_A, frame_W);
    EXPECT_TRUE(CompareMatrices(ABias_AAp_W.get_coeffs(), Vector6d::Zero(), kTolerance));

    // Verify MultibodyPlant::CalcBiasSpatialAcceleration() results match those
    // of test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation().
    const SpatialAcceleration<double> ABias_AAp_W_alternate =
            plant_.CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_A, p_AoAp_A, frame_A, frame_W);
    EXPECT_TRUE(CompareMatrices(ABias_AAp_W_alternate.get_coeffs(), Vector6d::Zero(), kTolerance));

    // Point Bp is an arbitrary point of link B.
    // Calculate Bp's bias spatial acceleration in link A, expressed in frame A.
    const SpatialAcceleration<double> ABias_ABp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
    ...
    const SpatialAcceleration<double> ABias_ABp_A_expected = CalcBpSpatialAccelerationInAExpressedInA(0.0, p_BoBp_B);
    EXPECT_TRUE(CompareMatrices(ABias_ABp_A_expected.get_coeffs(), ABias_ABp_A.get_coeffs(), kTolerance));

    // Verify MultibodyPlant::CalcBiasSpatialAcceleration() results match those
    // of test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation().
    const SpatialAcceleration<double> ABias_ABp_A_alternate =
            plant_.CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_B, p_BoBp_B, frame_A, frame_A);
    EXPECT_TRUE(CompareMatrices(ABias_ABp_A_alternate.get_coeffs(), ABias_ABp_A.get_coeffs(), kTolerance));

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcBpSpatialAccelerationInAExpressedInA().
    const SpatialAcceleration<double> ABias_ABp_B = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_B, p_BoBp_B, frame_A, frame_B);
    const math::RotationMatrix<double> R_BA = math::RotationMatrix<double>::MakeZRotation(-qB_);
    const SpatialAcceleration<double> ABias_ABp_B_expected = R_BA * ABias_ABp_A_expected;
    EXPECT_TRUE(CompareMatrices(ABias_ABp_B_expected.get_coeffs(), ABias_ABp_B.get_coeffs(), kTolerance));

    // Verify MultibodyPlant::CalcBiasSpatialAcceleration() results match those
    // of test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation().
    const SpatialAcceleration<double> ABias_ABp_B_alternate =
            plant_.CalcBiasSpatialAcceleration(*context_, JacobianWrtVariable::kV, frame_B, p_BoBp_B, frame_A, frame_B);
    EXPECT_TRUE(CompareMatrices(ABias_ABp_B_alternate.get_coeffs(), ABias_ABp_B.get_coeffs(), kTolerance));
}
...
TEST_F(TwoDOFPlanarPendulumTest, CalcSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_AAo_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_A, p_AoAo_A, frame_A, frame_A);
    EXPECT_TRUE(CompareMatrices(A_AAo_A.get_coeffs(), Vector6<double>::Zero(), kTolerance));
    ...
    const SpatialAcceleration<double> A_WAo_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_A, p_AoAo_A, frame_W, frame_A);

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcApSpatialAccelerationInWExpressedInA().
    const SpatialAcceleration<double> A_WAo_A_expected = CalcApSpatialAccelerationInWExpressedInA(wAzdot, p_AoAo_A);
    EXPECT_TRUE(CompareMatrices(A_WAo_A.get_coeffs(), A_WAo_A_expected.get_coeffs(), kTolerance));
    ...
    const math::RotationMatrix<double> R_WA = frame_A.CalcRotationMatrixInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(A_WAo_W.get_coeffs(), (R_WA * A_WAo_A).get_coeffs(), kTolerance));
    ...
    const SpatialAcceleration<double> A_WBo_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_B, p_BoBo_B, frame_W, frame_A);

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcBpSpatialAccelerationInWExpressedInA().
    const SpatialAcceleration<double> A_WBo_A_expected = CalcBpSpatialAccelerationInWExpressedInA(vdot, p_BoBo_B);
    EXPECT_TRUE(CompareMatrices(A_WBo_A.get_coeffs(), A_WBo_A_expected.get_coeffs(), kTolerance));

    // Verify MultibodyPlant::CalcSpatialAccelerationsFromVdot() results match
    // with test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation().
    EXPECT_TRUE(CompareMatrices(A_WBo_W.get_coeffs(), (R_WA * A_WBo_A).get_coeffs(), kTolerance));
    ...
    const SpatialAcceleration<double> A_WAp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_Af, p_AfAp_A, frame_W, frame_A);

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcApSpatialAccelerationInWExpressedInA().
    const SpatialAcceleration<double> A_WAp_A_expected = CalcApSpatialAccelerationInWExpressedInA(wAzdot, p_AoAp_A);
    EXPECT_TRUE(CompareMatrices(A_WAp_A.get_coeffs(), A_WAp_A_expected.get_coeffs(), kTolerance));

    // Point Bp is an arbitrary point of link B.
    // Calculate Bp's spatial acceleration in world W, expressed in frame A.
    const Vector3<double> p_BoBp_B(0.9, -0.8, 0.7);
    const SpatialAcceleration<double> A_WBp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_B, p_BoBp_B, frame_W, frame_A);

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcBpSpatialAccelerationInWExpressedInA().
    const SpatialAcceleration<double> A_WBp_A_expected = CalcBpSpatialAccelerationInWExpressedInA(vdot, p_BoBp_B);
    EXPECT_TRUE(CompareMatrices(A_WBp_A.get_coeffs(), A_WBp_A_expected.get_coeffs(), kTolerance));

    // Calculate frame_Bp's spatial acceleration in frame A, expressed in A.
    const SpatialAcceleration<double> A_ABp_A = test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(
            plant_, *context_, vdot, frame_B, p_BoBp_B, frame_A, frame_A);

    // Verify test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation()
    // results match by-hand result in CalcBpSpatialAccelerationInAExpressedInA().
    const SpatialAcceleration<double> A_ABp_A_expected = CalcBpSpatialAccelerationInAExpressedInA(wBzdot, p_BoBp_B);
    EXPECT_TRUE(CompareMatrices(A_ABp_A.get_coeffs(), A_ABp_A_expected.get_coeffs(), kTolerance));
}
