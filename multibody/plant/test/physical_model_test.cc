...
protected:
    ...
    static VectorXd dummy_state1() { return VectorXd::Ones(kState1Dofs) * kState1Value; }

    static VectorXd dummy_state2() { return VectorXd::Ones(kState2Dofs) * kState2Value; }

    MultibodyPlant<double> plant_{kDt};                 // A discrete MbP.
    DummyPhysicalModel<double>* dummy_model_{nullptr};  // The PhysicalModel under test.
};
...
TEST_F(PhysicalModelTest, DiscreteStateAndOutputPorts) {
    ...
    const VectorXd additional_state = dummy_model_->get_vector_output_port().Eval(*context);
    ...
    const VectorXd state_through_abstract_port = dummy_model_->get_abstract_output_port().Eval<VectorXd>(*context);
    EXPECT_TRUE(CompareMatrices(additional_state, state_through_abstract_port));
}

TEST_F(PhysicalModelTest, DeclareSceneGraphPorts) {
    DRAKE_EXPECT_THROWS_MESSAGE(dummy_model_->GetSceneGraphPortOrThrow(), ".*SceneGraph.*port.*not.*declared.*");
...
TEST_F(PhysicalModelTest, PostFinalizeStateAdditionNotAllowed) {
    plant_.Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(dummy_model_->AppendDiscreteState(dummy_state1()),
...
TEST_F(PhysicalModelTest, PostFinalizePortDeclarationNotAllowed) {
    plant_.Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(dummy_model_->DeclareSceneGraphPorts(),
