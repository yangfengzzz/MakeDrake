...
public:
    ...
    void RunSimulationToSteadyStateAndVerifyContactResults(double discrete_period) {
        auto diagram = MakeBoxDiagram(discrete_period);
        const auto& plant = dynamic_cast<const MultibodyPlant<double>&>(diagram->GetSubsystemByName("plant"));
        ...
        auto VerifyContactResults = [&](const MultibodyPlant<double>& the_plant, const Context<double>& the_context) {
            // We evaluate the contact results output port in order to verify the
            // simulation results.
            const ContactResults<double>& contact_results =
                    the_plant.get_contact_results_output_port().Eval<ContactResults<double>>(the_context);
            ...
            double direction = point_pair_contact_info.bodyB_index() == box.index() ? 1.0 : -1.0;

            // The expected value of the contact force applied on the box at the
            // contact point C.
            const Vector3<double> f_Bc_W(-applied_force_, 0.0, mass_ * g_);
            EXPECT_TRUE(CompareMatrices(point_pair_contact_info.contact_force(), f_Bc_W * direction, kTolerance,
            ...
            EXPECT_NEAR(point_pair_contact_info.point_pair().depth, penetration_allowance_, 1.0e-3);

            // Whether the normal points up or down depends on the order in which
            // scene graph orders bodies in the contact pair.
            const Vector3<double> expected_normal = -Vector3<double>::UnitZ() * direction;
            EXPECT_TRUE(CompareMatrices(point_pair_contact_info.point_pair().nhat_BA_W, expected_normal, kTolerance,
                                        MatrixCompareType::relative));
        ...
        const auto& plant2 = dynamic_cast<const MultibodyPlant<double>&>(diagram2->GetSubsystemByName("plant"));
        std::unique_ptr<Context<double>> diagram_context2 = diagram2->CreateDefaultContext();
        Context<double>& plant_context2 = diagram2->GetMutableSubsystemContext(plant2, diagram_context2.get());
        plant2.get_actuation_input_port().FixValue(&plant_context2, applied_force_);
        // Set the state from the computed solution.
        plant2.SetPositionsAndVelocities(&plant_context2, plant.GetPositionsAndVelocities(plant_context));
    ...
    std::unique_ptr<Diagram<double>> MakeBoxDiagram(double time_step) {
        ...
        MultibodyPlant<double>& plant =
                AddMultibodyPlantSceneGraph(&builder, std::make_unique<MultibodyPlant<double>>(time_step));
        plant.set_name("plant");
        Parser(&plant).AddModelsFromUrl("package://drake/multibody/plant/test/box.sdf");

        // Add gravity to the model.
        plant.mutable_gravity_field().set_gravity_vector(-g_ * Vector3<double>::UnitZ());
