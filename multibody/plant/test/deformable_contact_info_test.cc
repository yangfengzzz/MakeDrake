...
GTEST_TEST(DeformableContactInfo, ConstructAndAccess) {
    ...
    const PolygonSurfaceMesh<double> contact_mesh(std::move(face_data), std::move(vertices));
    ...
    contact_point_data.emplace_back(Vector3d(11, 22, 33), 42, Vector3d(44, 55, 66), Vector3d(77, 88, 99));
    const DeformableContactInfo<double> dut(id_A, id_B, contact_mesh, F_Ac_W, contact_point_data);
