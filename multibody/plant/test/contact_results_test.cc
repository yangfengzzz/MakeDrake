...
public:
    void SetUp() override {
        ...
        auto field = std::make_unique<PolygonSurfaceMeshFieldLinear<double, double>>(
                std::vector<double>{}, surface_mesh.get(), geometry::MeshGradientMode::kNone);
        contact_surface_ = std::make_unique<ContactSurface<double>>(GeometryId::get_new_id(), GeometryId::get_new_id(),
                                                                    std::move(surface_mesh), std::move(field));
        my_hydroelastic_contact_info_ = std::make_unique<HydroelasticContactInfo<double>>(
                contact_surface_.get(), SpatialForce<double>{}, std::vector<HydroelasticQuadraturePointData<double>>{});
        id_A_ = GeometryId::get_new_id();
        id_B_ = GeometryId::get_new_id();
        std::vector<DeformableContactPointData<double>> deformable_contact_point_data;
        deformable_contact_point_data.emplace_back(Vector3d(11, 22, 33), 42, Vector3d(44, 55, 66),
                                                   Vector3d(77, 88, 99));
        deformable_contact_info_ = std::make_unique<DeformableContactInfo<double>>(
                id_A_, id_B_, PolygonSurfaceMesh<double>(), SpatialForce<double>(Vector3d(1, 2, 3), Vector3d(4, 5, 6)),
...
protected:
    ...
    const PointPairContactInfo<double> point_pair_info_{
            BodyIndex{0}, BodyIndex{1}, Vector3d::Zero(), Vector3d::Zero(), 0, 0, PenetrationAsPointPair<double>{}};
    GeometryId id_A_;
    GeometryId id_B_;
    ...
    std::unique_ptr<HydroelasticContactInfo<double>> my_hydroelastic_contact_info_;
};
...
TEST_F(ContactResultsTest, SetAccessClear) {
    ...
    EXPECT_EQ(contact_results.point_pair_contact_info(0).bodyA_index(), point_pair_info_.bodyA_index());
    EXPECT_EQ(&contact_results.hydroelastic_contact_info(0), my_hydroelastic_contact_info_.get());
    ...
    EXPECT_NE(&copy.hydroelastic_contact_info(0), my_hydroelastic_contact_info_.get());
...
TEST_F(ContactResultsTest, AssignHydroelasticContactInfo) {
    ...
    // LHS will create unique pointers of copies of hydroelastic contacts.
    {
        ...
        EXPECT_NE(&lhs.hydroelastic_contact_info(0), &rhs_non_empty.hydroelastic_contact_info(0));
    }
}
...
TEST_F(ContactResultsTest, SelectHydroelastic) {
    ...
    const ContactResults<double> no_hydro_contacts =
            contact_results.SelectHydroelastic([](const HydroelasticContactInfo<double>&) -> bool {
                return false;
            });
    ...
    EXPECT_EQ(one_hydro_contact.hydroelastic_contact_info(0).contact_surface().id_M(),
              my_hydroelastic_contact_info_->contact_surface().id_M());
    // Verify the deep copy by checking for different memory address.
    EXPECT_NE(&one_hydro_contact.hydroelastic_contact_info(0), my_hydroelastic_contact_info_.get());
}
