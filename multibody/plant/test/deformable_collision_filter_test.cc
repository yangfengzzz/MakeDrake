...
protected:
    ...
    void SetUp() override {
        ...
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, 0.01);
        DeformableModel<double>& deformable_model = plant_->mutable_deformable_model();
        DeformableBodyId deformable_body_id = RegisterDeformableOctahedron(&deformable_model, "deformable");
        deformable_geometry_id_ = deformable_model.GetGeometryId(deformable_body_id);
        ...
        geometry::AddContactMaterial({}, {}, CoulombFriction{0.4, 0.4}, &proximity_prop);
        ...
        const SpatialInertia<double> M_Bcm = SpatialInertia<double>::SolidCubeWithMass(1.0, 1.0);
        const RigidBody<double>& welded_body = plant_->AddRigidBody("welded body", M_Bcm);
        plant_->AddJoint<WeldJoint>("weld joint", plant_->world_body(), std::nullopt, welded_body, std::nullopt,
                                    RigidTransformd::Identity());
        const Box box(1, 1, 1);
        welded_geometry_id_ = plant_->RegisterCollisionGeometry(welded_body, RigidTransformd::Identity(), box, "welded",
                                                                proximity_prop);

        const RigidBody<double>& free_body = plant_->AddRigidBody("free body", M_Bcm);
        free_geometry_id_ =
                plant_->RegisterCollisionGeometry(free_body, RigidTransformd::Identity(), box, "free", proximity_prop);
        ...
        GeometrySet welded_and_deformable{deformable_geometry_id_, welded_geometry_id_};
        GeometrySet free_and_deformable{deformable_geometry_id_, free_geometry_id_};
        std::pair<std::string, GeometrySet> collision_filter_group_a = {"a", welded_and_deformable};
        std::pair<std::string, GeometrySet> collision_filter_group_b = {"b", free_and_deformable};
        plant_->ExcludeCollisionGeometriesWithCollisionFilterGroupPair(collision_filter_group_a,
                                                                       collision_filter_group_a);
        plant_->ExcludeCollisionGeometriesWithCollisionFilterGroupPair(collision_filter_group_a,
                                                                       collision_filter_group_b);
        DRAKE_DEMAND(!scene_graph_->model_inspector().CollisionFiltered(deformable_geometry_id_, welded_geometry_id_));
        DRAKE_DEMAND(!scene_graph_->model_inspector().CollisionFiltered(deformable_geometry_id_, free_geometry_id_));
    ...
    /* Registers a deformable octahedron with the given `name` in the world to the
     given `model`. The octahedron looks like this in its geometry frame, F.
       ...
       -Fy---v4------v0+------v2---+ Fy
                 ...
                 +Fx   |
                      -Fz     */
    DeformableBodyId RegisterDeformableOctahedron(DeformableModel<double>* model, std::string name) {
        auto geometry =
                std::make_unique<GeometryInstance>(RigidTransformd(), std::make_unique<Sphere>(2.0), std::move(name));
        ProximityProperties props;
        geometry::AddContactMaterial({}, {}, CoulombFriction<double>(0.3, 0.3), &props);
        ...
        DeformableBodyId id = model->RegisterDeformableBody(std::move(geometry), body_config, kRezHint);
...
TEST_F(DeformableCollisionFilterTest, DefaultMbPFilterIgnoresDeformable) {
    ...
    const auto& scene_graph_context = scene_graph_->GetMyContextFromRoot(*diagram_context);
    const QueryObject<double>& query_object =
            scene_graph_->get_query_output_port().Eval<QueryObject<double>>(scene_graph_context);
...
TEST_F(DeformableCollisionFilterTest, ExplicitlyFilterDeformable) {
    scene_graph_->collision_filter_manager().Apply(
            CollisionFilterDeclaration(CollisionFilterScope::kAll)
                    .ExcludeBetween(GeometrySet(deformable_geometry_id_), GeometrySet(welded_geometry_id_)));
    auto diagram_context = diagram_->CreateDefaultContext();
    const auto& scene_graph_context = scene_graph_->GetMyContextFromRoot(*diagram_context);
    const QueryObject<double>& query_object =
            scene_graph_->get_query_output_port().Eval<QueryObject<double>>(scene_graph_context);
    ...
    EXPECT_EQ(deformable_contact.contact_surfaces()[0].id_A(), deformable_geometry_id_);
