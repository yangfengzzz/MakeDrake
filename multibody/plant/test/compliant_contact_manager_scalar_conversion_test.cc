...
TYPED_TEST_SUITE(CompliantContactManagerScalarConversionTest, NonSymbolicScalars);
...
TYPED_TEST(CompliantContactManagerScalarConversionTest, ToDouble) {
    ...
    std::unique_ptr<DiscreteUpdateManager<double>> clone = source.template CloneToScalar<double>();
...
TYPED_TEST(CompliantContactManagerScalarConversionTest, ToAutoDiffXd) {
    ...
    std::unique_ptr<DiscreteUpdateManager<AutoDiffXd>> clone = source.template CloneToScalar<AutoDiffXd>();
    ASSERT_NE(clone, nullptr);
}
...
// pipelines.
template <typename T>
std::unique_ptr<MultibodyPlant<T>> MakePlant(DiscreteContactSolver solver_type) {
...
void TestPlantConversionAndSimulate(DiscreteContactSolver solver_type) {
    ...
    const VectorX<T> initial_state = source_plant->GetPositionsAndVelocities(*source_context);
    auto source_simulator = std::make_unique<Simulator<T>>(*source_plant, std::move(source_context));
    source_simulator->AdvanceTo(10 * kTimeStep);
    const VectorX<T> source_final_state = source_plant->GetPositionsAndVelocities(source_simulator->get_context());
    ...
    auto dest_simulator = std::make_unique<Simulator<U>>(*dest_plant, std::move(dest_context));
    dest_simulator->AdvanceTo(10 * kTimeStep);
    const VectorX<U> dest_final_state = dest_plant->GetPositionsAndVelocities(dest_simulator->get_context());
...
GTEST_TEST(ScalarConvertAndSimulateTest, PlantWithSap) {
    TestPlantConversionAndSimulate<double, AutoDiffXd>(DiscreteContactSolver::kSap);
    TestPlantConversionAndSimulate<AutoDiffXd, double>(DiscreteContactSolver::kSap);
}

GTEST_TEST(ScalarConvertAndSimulateTest, PlantWithTamsi) {
    TestPlantConversionAndSimulate<double, AutoDiffXd>(DiscreteContactSolver::kTamsi);
    TestPlantConversionAndSimulate<AutoDiffXd, double>(DiscreteContactSolver::kTamsi);
}
...
GTEST_TEST(ScalarConvertTest, ConversionToAndFromSymbolic) {
    ...
    TestPlantConversion<double, symbolic::Expression>(DiscreteContactSolver::kTamsi);
    TestPlantConversion<double, symbolic::Expression>(DiscreteContactSolver::kSap);

    // Conversion from symbolic to double.
    TestPlantConversion<symbolic::Expression, double>(DiscreteContactSolver::kTamsi);
    TestPlantConversion<symbolic::Expression, double>(DiscreteContactSolver::kSap);
}
