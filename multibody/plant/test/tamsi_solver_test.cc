...
#include "multibody/plant/test/tamsi_solver_test_util.h"
...
public:
    static MatrixX<double> CalcJacobian(const TamsiSolver<double>& solver,
        ...
        std::vector<Matrix2<double>>& dft_dvt = solver.variable_size_workspace_.mutable_dft_dvt();
    ...
    static int get_capacity(const TamsiSolver<double>& solver) { return solver.variable_size_workspace_.capacity(); }
};
namespace {
...
protected:
    ...
    static Matrix2<double> Rotation(double theta) { return Eigen::Rotation2D<double>(theta).toRotationMatrix(); }
...
TEST_F(DirectionLimiter, ZeroVandZeroDv) {
    ...
    const double alpha = internal::TalsLimiter<double>::CalcAlpha(vt, dvt, cos_min, v_stiction, tolerance).value();
...
TEST_F(DirectionLimiter, ZeroVtoSlidingRegion) {
    ...
    EXPECT_TRUE(CompareMatrices(vt_alpha, vt_alpha_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(DirectionLimiter, SlidingRegionToStictionRegion) {
    ...
    const Vector2<double> vt_alpha_expected = Vector2<double>(-0.3, 0.45) * v_stiction;
...
TEST_F(DirectionLimiter, StictionToSliding) {
    const Vector2<double> vt = Vector2<double>(-0.5, 0.3) * v_stiction * tolerance;
...
TEST_F(DirectionLimiter, VerySmallDeltaV) {
    ...
    const Vector2<double> dvt = Vector2<double>(-0.5, 0.3) * v_stiction * tolerance;
    const double alpha = internal::TalsLimiter<double>::CalcAlpha(vt, dvt, cos_min, v_stiction, tolerance).value();
    EXPECT_NEAR(alpha, 1.0, kTolerance);
}
...
TEST_F(DirectionLimiter, CrossStictionRegionFromTheOutside) {
    ...
    const Vector2<double> vt_alpha_expected = Vector2<double>(0.3, 0.2) * v_stiction;

    // A unit vector normal to vt_alpha_expected.
    const Vector2<double> vt_normal = Vector2<double>(vt_alpha_expected(1), -vt_alpha_expected(0)).normalized();
...
TEST_F(DirectionLimiter, ChangesWithinTheSlidingRegion_QuadraticNonPositive1) {
    ...
    const std::optional<double> alpha =
            internal::TalsLimiter<double>::CalcAlpha(vt, dvt, cos_min, v_stiction, tolerance);
...
public:
    ...
    void SetProblem(const Vector3<double>& v0, const Vector3<double>& tau, double mu, double theta, double dt) {
    ...
    void SetNoContactProblem(const Vector3<double>& v0, const Vector3<double>& tau, double dt) {
        // Next time step generalized momentum if there are no friction forces.
        p_star_ = M_ * v0 + dt * tau;
...
TEST_F(PizzaSaver, SmallAppliedMoment) {
    ...
    const double vt_tolerance =
            ...
            solver_.get_solver_parameters().relative_tolerance * solver_.get_solver_parameters().stiction_tolerance;
    ...
    MatrixX<double> J_expected =
            test::CalcOneWayCoupledJacobianWithAutoDiff(M_, Jn_, Jt_, p_star_, mu_, fn_, dt, v_stiction, epsilon_v, v);

    // We use a tolerance scaled by the norm and size of the matrix.
    const double J_tolerance = J_expected.rows() * J_expected.norm() * std::numeric_limits<double>::epsilon();

    // Verify the result.
    EXPECT_TRUE(CompareMatrices(J, J_expected, J_tolerance, MatrixCompareType::absolute));
}
...
public:
    ...
    void SetImpactProblem(const Vector3<double>& v0,
        ...
        solver_.SetTwoWayCoupledProblemData(&M_, &Jn_, &Jt_, &p_star_, &fn0_, &stiffness_, &dissipation_, &mu_vector_);
    }
...
TEST_F(RollingCylinder, StictionAfterImpact) {
    ...
    for (int num_contacts = 1; num_contacts <= 256; num_contacts *= 4) {
        ...
        MatrixX<double> J_expected = test::CalcTwoWayCoupledJacobianWithAutoDiff(
                M_, Jn_, Jt_, p_star_, x0_, mu_vector_, stiffness_, dissipation_, dt, v_stiction, epsilon_v, v);

        // Verify the result.
        const double J_tolerance = 40 * std::numeric_limits<double>::epsilon();
        EXPECT_TRUE(CompareMatrices(J, J_expected, J_tolerance, MatrixCompareType::relative));
    }
}
...
GTEST_TEST(EmptyWorld, Solve) {
    ...
    DRAKE_EXPECT_NO_THROW(
            solver.SetTwoWayCoupledProblemData(&M, &Jn, &Jt, &p_star, &fn0, &stiffness, &dissipation, &mu_vector));
}
