...
#include "multibody/test_utilities/floating_body_plant.h"
...
public:
    ...
    static const QuaternionFloatingMobilizer<double>& get_floating_mobilizer(const MultibodyTree<double>& model,
...
GTEST_TEST(QuaternionFloatingMobilizer, Simulation) {
    ...
    FreeBody benchmark_(Quaterniond::Identity(), Vector3d::Zero(), p0_WBcm, v0_WBcm, gravity_W);

    // Instantiate the model for the free body in space.
    AxiallySymmetricFreeBodyPlant<double> free_body_plant(kMass, benchmark_.get_I(), benchmark_.get_J(), kGravity);
    ...
    const Vector3d w0_WB_expected = free_body_plant.get_default_initial_angular_velocity();
    const Vector3d v0_WB_expected = free_body_plant.get_default_initial_translational_velocity();

    const internal::QuaternionFloatingMobilizer<double>& mobilizer = MultibodyTreeTester::get_floating_mobilizer(
    ...
    EXPECT_TRUE(CompareMatrices(mobilizer.get_angular_velocity(context), 2.0 * w0_WB_expected, kEpsilon,
                                MatrixCompareType::relative));
    mobilizer.SetTranslationalVelocity(&context, -3.5 * v0_WB_expected);
    EXPECT_TRUE(CompareMatrices(mobilizer.get_translational_velocity(context), -3.5 * v0_WB_expected, kEpsilon,
                                MatrixCompareType::relative));

    // Unit test QuaternionFloatingMobilizer::SetOrientation().
    const Vector3d axis = (1.5 * Vector3d::UnitX() + 2.0 * Vector3d::UnitY() + 3.0 * Vector3d::UnitZ()).normalized();
    ...
    EXPECT_TRUE(CompareMatrices(q_WB_test.coeffs(), q_WB_test_expected.coeffs(), 5 * kEpsilon,
                                MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(mobilizer.get_quaternion(context).coeffs(), q_WB_test2.coeffs(), kEpsilon,
                                MatrixCompareType::relative));
    const Vector3d p_WB_test(1, 2, 3);
    mobilizer.SetTranslation(&context, p_WB_test);
    EXPECT_TRUE(CompareMatrices(mobilizer.get_translation(context), p_WB_test, kEpsilon, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(mobilizer.get_angular_velocity(context), w0_WB_expected, kEpsilon,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(mobilizer.get_translational_velocity(context), v0_WB_expected, kEpsilon,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(mobilizer.get_quaternion(context).coeffs(), Quaterniond::Identity().coeffs(), kEpsilon,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(mobilizer.get_translation(context), Vector3d::Zero(), kEpsilon,
                                MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(w0_WB, w0_WB_expected, kEpsilon, MatrixCompareType::relative));
    ...
    RungeKutta3Integrator<double>& integrator = simulator.reset_integrator<RungeKutta3Integrator<double>>();
    ...
    // Get solution:
    const math::RigidTransformd X_WB = free_body_plant.CalcPoseInWorldFrame(context);
    const math::RotationMatrixd R_WB = X_WB.rotation();
    const Vector3d p_WBcm = X_WB.translation();
    const SpatialVelocity<double> V_WB = free_body_plant.CalcSpatialVelocityInWorldFrame(context);
    ...
    std::tie(p_WBcm_exact, v_WBcm_exact, a_WBcm_exact) = benchmark_.CalculateExactTranslationalSolution(kEndTime);

    // Compare computed solution against benchmark:
    EXPECT_TRUE(CompareMatrices(R_WB.matrix(), R_WB_exact.matrix(), kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(p_WBcm, p_WBcm_exact, kTolerance, MatrixCompareType::relative));

    EXPECT_TRUE(CompareMatrices(w_WB, w_WB_exact, kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(v_WB, v_WBcm_exact, kTolerance, MatrixCompareType::relative));

    // Verify MultibodyTree::MapVelocityToQDot() to compute the quaternion time
    // derivative.
    const internal::MultibodyTree<double>& model = internal::GetInternalTree(free_body_plant);
    VectorX<double> qdot_from_v(model.num_positions());
    // The generalized velocity computed last at time = kEndTime.
    const VectorX<double> v = context.get_continuous_state().get_generalized_velocity().CopyToVector();
    ...
    // quaternion.
    EXPECT_TRUE(CompareMatrices(DtW_quat, quatDt_WB_exact, kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(DtW_p_WBcm, v_WB, kEpsilon, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(v_back, v, kTolerance, MatrixCompareType::relative));
...
GTEST_TEST(QuaternionFloatingMobilizer, MapVelocityToQDotAndBack) {
    ...
    // Instantiate the model for the free body in space.
    AxiallySymmetricFreeBodyPlant<double> free_body_plant(kMass, kInertia, kInertia, kGravity);
    const internal::MultibodyTree<double>& model = internal::GetInternalTree(free_body_plant);

    std::unique_ptr<Context<double>> context = free_body_plant.CreateDefaultContext();
    ...
    DRAKE_EXPECT_NO_THROW(model.SetFreeBodyPoseOrThrow(free_body_plant.body(), X_WB, context.get()));

    // Set velocities.
    const Vector3d w_WB(1.0, 2.0, 3.0);
    const Vector3d v_WB(-1.0, 4.0, -0.5);
    DRAKE_EXPECT_NO_THROW(model.SetFreeBodySpatialVelocityOrThrow(free_body_plant.body(), {w_WB, v_WB}, context.get()));
    ...
    VectorX<double> v_back(model.num_velocities());
    model.MapQDotToVelocity(*context, qdot_from_v, &v_back);
    EXPECT_TRUE(CompareMatrices(v_back, v, 10 * kEpsilon, MatrixCompareType::relative));
}
...
GTEST_TEST(QuaternionFloatingMobilizer, InboardJointLocking) {
    ...
    // Instantiate the model for the free body in space.
    AxiallySymmetricFreeBodyPlant<double> free_body_plant(kMass, kInertia, kInertia, kGravity, 0.001 /* time_step */);
    const internal::MultibodyTree<double>& model = internal::GetInternalTree(free_body_plant);
    ...
    DRAKE_EXPECT_NO_THROW(model.SetFreeBodySpatialVelocityOrThrow(free_body, {w_WB, v_WB}, &context));
...
GTEST_TEST(QuaternionFloatingMobilizer, ExceptionMessageForInvalidQuaternion) {
    ...
    std::unique_ptr<systems::ContinuousState<double>> stateDt = free_body_plant.AllocateTimeDerivatives();
    ...
    state_initial << bad_quat.w(), bad_quat.x(), bad_quat.y(), bad_quat.z(), p0_WBcm_W, w0_WB_W, v0_WBcm_W;
    context.SetContinuousState(state_initial);

    // A zero quaternion should throw an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(free_body_plant.CalcTimeDerivatives(context, stateDt_ptr),
