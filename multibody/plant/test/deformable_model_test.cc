...
protected:
    ...
    DeformableBodyId RegisterSphere(double resolution_hint, RigidTransformd X_WS = RigidTransformd()) {
    ...
    DeformableBodyId RegisterSphere(DeformableModel<T>* model,
        ...
        auto geometry = make_unique<GeometryInstance>(X_WS, make_unique<Sphere>(1), "sphere");
        DeformableBodyId body_id =
                model->RegisterDeformableBody(std::move(geometry), fem::DeformableBodyConfig<T>{}, resolution_hint);
        return body_id;
    }
};
...
TEST_F(DeformableModelTest, RegisterDeformableBody) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            deformable_model_ptr_->RegisterDeformableBody(
                    make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(1), "sphere"),
...
TEST_F(DeformableModelTest, SetWallBoundaryCondition) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->SetWallBoundaryCondition(fake_body_id, p_WQ2, n_W),
                                fmt::format(".*No.*id.*{}.*registered.*", fake_body_id));

    /* Setting boundary condition must be done pre-finalize. */
    plant_->Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->SetWallBoundaryCondition(body_id, p_WQ2, n_W),
...
TEST_F(DeformableModelTest, DiscreteStateIndexAndReferencePositions) {
    constexpr double kRezHint = 0.5;
    Sphere sphere(1.0);
    auto geometry = make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(sphere), "sphere");
    const DeformableBodyId body_id =
            deformable_model_ptr_->RegisterDeformableBody(std::move(geometry), default_body_config_, kRezHint);

    /* Getting state index before Finalize() is prohibited. */
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetDiscreteStateIndex(body_id),
    ...
    /* Verify that the position values in  default discrete state is given by the
     reference positions. */
    plant_->Finalize();
    systems::DiscreteStateIndex state_index = deformable_model_ptr_->GetDiscreteStateIndex(body_id);
    auto context = plant_->CreateDefaultContext();
    const VectorX<double>& discrete_state = context->get_discrete_state(state_index).value();
    const int num_dofs = deformable_model_ptr_->GetFemModel(body_id).num_dofs();
    EXPECT_EQ(discrete_state.head(num_dofs), deformable_model_ptr_->GetReferencePositions(body_id));
    /* Verify that the velocity and acceleration values in default discrete state
     is given by the reference positions. */
    EXPECT_EQ(discrete_state.tail(2 * num_dofs), VectorX<double>::Zero(2 * num_dofs));
}
...
TEST_F(DeformableModelTest, InvalidBodyId) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetReferencePositions(fake_id),
                                "GetReferencePositions.*No deformable body with id.*");

    plant_->Finalize();
    /* Post-finalize calls. */
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetDiscreteStateIndex(fake_id),
...
TEST_F(DeformableModelTest, GetBodyIdFromBodyIndex) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetBodyId(DeformableBodyIndex(0)),
    ...
    EXPECT_THROW(deformable_model_ptr_->GetBodyId(DeformableBodyIndex(1)), std::exception);
    EXPECT_THROW(deformable_model_ptr_->GetBodyId(DeformableBodyIndex()), std::exception);
}
...
TEST_F(DeformableModelTest, GetBodyIndex) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetBodyIndex(body_id), ".*before system resources.*declared.*");
    plant_->Finalize();
    EXPECT_EQ(deformable_model_ptr_->GetBodyIndex(body_id), DeformableBodyIndex(0));
    /* Throws for unregistered body id. */
    const DeformableBodyId fake_body_id = DeformableBodyId::get_new_id();
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetBodyIndex(fake_body_id),
                                fmt::format(".*No.*id.*{}.*registered.*", fake_body_id));
}
...
TEST_F(DeformableModelTest, GetGeometryId) {
    constexpr double kRezHint = 0.5;
    const DeformableBodyId body_id = RegisterSphere(kRezHint);
    geometry::GeometryId geometry_id = deformable_model_ptr_->GetGeometryId(body_id);
...
TEST_F(DeformableModelTest, GetBodyIdFromGeometryId) {
    ...
    const geometry::GeometryId fake_geometry_id = geometry::GeometryId::get_new_id();
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetBodyId(fake_geometry_id), ".*GeometryId.*not.*registered.*");
}

TEST_F(DeformableModelTest, ToPhysicalModelPointerVariant) {
    PhysicalModelPointerVariant<double> variant = deformable_model_ptr_->ToPhysicalModelPointerVariant();
...
TEST_F(DeformableModelTest, VertexPositionsOutputPort) {
    ...
    std::unique_ptr<systems::Context<double>> context = plant_->CreateDefaultContext();
    std::unique_ptr<AbstractValue> output_value = plant_->get_deformable_body_configuration_output_port().Allocate();
    /* Compute the configuration for each geometry in the model. */
    plant_->get_deformable_body_configuration_output_port().Calc(*context, output_value.get());
    ...
    const geometry::GeometryId geometry_id = deformable_model_ptr_->GetGeometryId(body_id);
    ...
    EXPECT_EQ(configurations.value(geometry_id), deformable_model_ptr_->GetReferencePositions(body_id));
}

TEST_F(DeformableModelTest, AddFixedConstraint) {
    RigidTransformd X_WA(math::RollPitchYawd(1, 2, 3), Vector3d(1.23, 4.56, 7.89));
    ...
    */
    ...
    const RigidBody<double>& rigid_body = plant_->AddRigidBody("box", SpatialInertia<double>::NaN());
    ...
    const MultibodyConstraintId constraint_id =
            deformable_model_ptr_->AddFixedConstraint(deformable_id, rigid_body, X_BA, box, X_BG);

    EXPECT_TRUE(deformable_model_ptr_->HasConstraint(deformable_id));
    EXPECT_EQ(deformable_model_ptr_->fixed_constraint_ids(deformable_id).size(), 1);
    const DeformableRigidFixedConstraintSpec& spec = deformable_model_ptr_->fixed_constraint_spec(constraint_id);
    ...
    const Vector3d p_WPi = deformable_model_ptr_->GetReferencePositions(deformable_id).segment<3>(3 * vertex_index);
    ...
    EXPECT_THROW(deformable_model_ptr_->AddFixedConstraint(fake_deformable_id, rigid_body, X_BA, box, X_BG),
                 std::exception);
    /* Non-existant rigid body (registered with a different MbP). */
    MultibodyPlant<double> other_plant(0.0);
    const RigidBody<double>& wrong_rigid_body = other_plant.AddRigidBody("wrong body", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(
            deformable_model_ptr_->AddFixedConstraint(deformable_id, wrong_rigid_body, X_BA, box, X_BG),
            ".*rigid body.*not registered.*");
    /* Unsupported shape. */
    const geometry::Mesh mesh("fake_mesh.vtk");
    EXPECT_THROW(deformable_model_ptr_->AddFixedConstraint(deformable_id, rigid_body, X_BA, mesh, X_BG),
                 std::exception);
    /* No constraint is added . */
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->AddFixedConstraint(deformable_id, rigid_body, X_BA, box,
                                                                          RigidTransformd(Vector3d(100, 100, 100))),
                                "No constraint has been added.*box.*");
    /* Adding constraint after finalize. */
    plant_->Finalize();
    EXPECT_THROW(deformable_model_ptr_->AddFixedConstraint(deformable_id, rigid_body, X_BA, box, X_BG), std::exception);
}
...
TEST_F(DeformableModelTest, ExternalForces) {
    ...
    public:
        ...
        explicit ConstantForceDensityField(std::function<Vector3<double>(const Vector3<double>&)> field)
    ...
    private:
        Vector3<double> DoEvaluateAt(const systems::Context<double>& context, const Vector3<double>& p_WQ) const final {
        ...
        void DoDeclareInputPorts(multibody::MultibodyPlant<double>* plant) final {
            scale_port_index_ =
                    this->DeclareVectorInputPort(plant, "on/off signal for the force field", BasicVector<double>(1.0))
                            .get_index();
        }
    ...
    auto constant_force = std::make_unique<ConstantForceDensityField>(force_field);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            deformable_model_ptr_->AddExternalForce(std::make_unique<ConstantForceDensityField>(force_field)),
            ".*AddExternalForce.*after system resources have been declared.*");

    DeformableBodyId fake_id = DeformableBodyId::get_new_id();
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->GetExternalForces(fake_id),
    ...
    for (const auto* force : external_forces) {
        ...
        if (g != nullptr) {
            ...
            EXPECT_EQ(force->EvaluateAt(*plant_context, p_WQ), gravity_vector * default_body_config_.mass_density());
        } else {
            EXPECT_EQ(f->density_type(), ForceDensityType::kPerCurrentVolume);
            const double scale = 2.71;
            constant_force_ptr->get_input_port().FixValue(plant_context.get(), Vector1d(scale));
...
TEST_F(DeformableModelTest, CloneBeforeFinalizeThrows) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(deformable_model_ptr_->CloneToScalar<double>(&double_plant),
...
TEST_F(DeformableModelTest, EmptyClone) {
    ...
    EXPECT_NO_THROW(deformable_model_ptr_->CloneToScalar<double>(&double_plant));
    /* double -> autodiff */
    EXPECT_NO_THROW(deformable_model_ptr_->CloneToScalar<AutoDiffXd>(&autodiff_plant));
    /* double -> symbolic */
    EXPECT_NO_THROW(deformable_model_ptr_->CloneToScalar<symbolic::Expression>(&symbolic_plant));
}
...
TEST_F(DeformableModelTest, NonEmptyClone) {
    ...
    const RigidTransformd X_BG(Vector3d(-1, 0, 0));
    const MultibodyConstraintId constraint_id =
            deformable_model_ptr_->AddFixedConstraint(body_id, rigid_body, X_BA, box, X_BG);
    ...
    /* double -> double */
    EXPECT_NO_THROW(double_clone = deformable_model_ptr_->CloneToScalar<double>(&double_plant));

    /* Now verify the double clone is indeed a copy of the original. */
    const DeformableModel<double>* double_clone_ptr = dynamic_cast<const DeformableModel<double>*>(double_clone.get());
    ASSERT_NE(double_clone_ptr, nullptr);
    EXPECT_EQ(double_clone_ptr->num_bodies(), deformable_model_ptr_->num_bodies());
    ...
    EXPECT_EQ(double_clone_ptr->GetReferencePositions(body_id), deformable_model_ptr_->GetReferencePositions(body_id));
    ...
    const geometry::GeometryId geometry_id = deformable_model_ptr_->GetGeometryId(body_id);
    EXPECT_EQ(double_clone_ptr->GetGeometryId(body_id), deformable_model_ptr_->GetGeometryId(body_id));
    EXPECT_EQ(double_clone_ptr->GetBodyId(geometry_id), deformable_model_ptr_->GetBodyId(geometry_id));
    EXPECT_EQ(double_clone_ptr->HasConstraint(body_id), deformable_model_ptr_->HasConstraint(body_id));
    EXPECT_EQ(double_clone_ptr->fixed_constraint_spec(constraint_id),
              deformable_model_ptr_->fixed_constraint_spec(constraint_id));
    EXPECT_EQ(double_clone_ptr->fixed_constraint_ids(body_id), deformable_model_ptr_->fixed_constraint_ids(body_id));
}
...
TEST_F(DeformableModelTest, EmptyDeformableModelWorksWithTamsi) {
    plant_->set_discrete_contact_approximation(multibody::DiscreteContactApproximation::kTamsi);
    EXPECT_TRUE(deformable_model_ptr_->is_empty());
...
TEST_F(DeformableModelTest, NonEmptyDeformableModelOnlyWorksWithSap) {
    ...
    RegisterSphere(0.5);
    EXPECT_FALSE(deformable_model_ptr_->is_empty());
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->Finalize(), ".*DeformableModel is only supported by.*SAP.*");
}
...
TEST_F(DeformableModelTest, RegistrationNotAllowedForNonDoubleModel) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RegisterSphere(&autodiff_deformable_model, 0.5),
                                ".*RegisterDeformableBody.*T != double.*not allowed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(autodiff_deformable_model.AddExternalForce(
                                        std::make_unique<GravityForceField<AutoDiffXd>>(Vector3d(0, 0, -10), 1.0)),
