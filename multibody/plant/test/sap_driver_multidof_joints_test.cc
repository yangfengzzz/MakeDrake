...
public:
    ...
    MultiDofJointWithLimits(const Frame<T>& frame_on_parent,
        ...
        : Joint<T>("MultiDofJointWithLimits",
                   frame_on_parent,
                   frame_on_child,
                   ...
                   VectorX<double>::Constant(kNumDofs, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(kNumDofs, std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(kNumDofs, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(kNumDofs, std::numeric_limits<double>::infinity())) {
...
private:
    ...
    std::unique_ptr<typename Joint<T>::BluePrint> MakeImplementationBlueprint() const override {
        ...
        auto revolute_mobilizer =
                std::make_unique<internal::RpyBallMobilizer<T>>(this->frame_on_parent(), this->frame_on_child());
    ...
    void DoAddInOneForce(const Context<T>&, int, const T&, MultibodyForces<T>*) const override { DRAKE_UNREACHABLE(); }
    void DoAddInDamping(const Context<T>&, MultibodyForces<T>*) const override { DRAKE_UNREACHABLE(); }
    std::string do_get_position_suffix(int) const override { DRAKE_UNREACHABLE(); }
    std::string do_get_velocity_suffix(int) const override { DRAKE_UNREACHABLE(); }
    void do_set_default_positions(const VectorX<double>&) override { DRAKE_UNREACHABLE(); }
    const T& DoGetOnePosition(const Context<T>&) const override { DRAKE_UNREACHABLE(); }
    const T& DoGetOneVelocity(const Context<T>&) const override { DRAKE_UNREACHABLE(); }
...
GTEST_TEST(MultiDofJointWithLimitsTest, ThrowForUnsupportedJoints) {
    ...
    const RigidBody<double>& body = plant.AddRigidBody("DummyBody", SpatialInertia<double>::MakeUnitary());
    plant.AddJoint(
            std::make_unique<MultiDofJointWithLimits<double>>(plant.world_frame(), body.body_frame(), -1.0, 2.0));
    plant.Finalize();
    auto owned_contact_manager = std::make_unique<CompliantContactManager<double>>();
    CompliantContactManager<double>* contact_manager = owned_contact_manager.get();
    ...
    const SapDriver<double>& driver = CompliantContactManagerTester::sap_driver(*contact_manager);

    // We verify the driver throws for the right reasons.
    DRAKE_EXPECT_THROWS_MESSAGE(SapDriverTest::AddLimitConstraints(driver, *context, v_star, &problem),
...
GTEST_TEST(MultiDofJointWithLimitsTest, VerifyMultiDofJointsWithoutLimitsAreSupported) {
    MultibodyPlant<double> plant(1.0e-3);
    // N.B. Currently only SAP goes through the manager.
    plant.set_discrete_contact_approximation(DiscreteContactApproximation::kSap);
    // To avoid unnecessary warnings/errors, use a non-zero spatial inertia.
    ...
    plant.AddJoint(
            std::make_unique<MultiDofJointWithLimits<double>>(plant.world_frame(), body.body_frame(), -kInf, kInf));
    plant.Finalize();
    auto owned_contact_manager = std::make_unique<CompliantContactManager<double>>();
    CompliantContactManager<double>* contact_manager = owned_contact_manager.get();
    plant.SetDiscreteUpdateManager(std::move(owned_contact_manager));
    ...
    EXPECT_NO_THROW(SapDriverTest::AddLimitConstraints(driver, *context, v_star, &problem));
