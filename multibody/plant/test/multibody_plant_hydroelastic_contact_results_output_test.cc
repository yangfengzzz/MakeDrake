...
#include "examples/multibody/rolling_sphere/populate_ball_plant.h"
...
protected:
    void SetUp() {
        ...
        examples::multibody::bouncing_ball::PopulateBallPlant(radius, mass, hydroelastic_modulus, dissipation, friction,
                                                              gravity_W, false /* rigid_sphere */,
        ...
        plant_->SetFreeBodyPose(plant_context_, plant_->GetBodyByName("Ball"), X_WB);
        ...
        plant_->SetFreeBodySpatialVelocity(plant_context_, plant_->GetBodyByName("Ball"), V_WB);
    }

    const HydroelasticContactInfo<double>& contact_results() const {
        // Get the contact results from the plant.
        const ContactResults<double>& contact_results =
                plant_->get_contact_results_output_port().Eval<ContactResults<double>>(*plant_context_);
...
TEST_F(HydroelasticContactResultsOutputTester, ContactSurfaceEquivalent) {
    ...
    const auto& query_object =
            plant_->get_geometry_query_input_port().template Eval<geometry::QueryObject<double>>(*plant_context_);
    ...
    std::vector<geometry::ContactSurface<double>> contact_surfaces =
            query_object.ComputeContactSurfaces(geometry::HydroelasticContactRepresentation::kTriangle);

    // Check that the two contact surfaces are equivalent.
    ASSERT_EQ(contact_surfaces.size(), 1);
    EXPECT_TRUE(contact_results().contact_surface().Equal(contact_surfaces.front()));
}
...
TEST_F(HydroelasticContactResultsOutputTester, SpatialForceAtCentroid) {
    ...
    ASSERT_EQ(results.contact_surface().num_faces() * 3, results.quadrature_point_data().size());
    ...
    const bool body_A_is_ball = (std::find(ball_collision_geometries.begin(), ball_collision_geometries.end(),
    ...
    for (const HydroelasticQuadraturePointData<double>& datum : results.quadrature_point_data()) {
        F_Ac_W_expected.translational() +=
                results.contact_surface().area(datum.face_index) * 1.0 / 3 * datum.traction_Aq_W;
    }

    // Use a relative tolerance since the magnitude of the spatial force will be
    // on the order of 10^4.
    const double tol = 10 * F_Ac_W.translational().norm() * std::numeric_limits<double>::epsilon();
    EXPECT_NEAR((F_Ac_W_expected.translational() - F_Ac_W.translational()).norm(), 0, tol);
    EXPECT_NEAR((F_Ac_W_expected.rotational() - F_Ac_W.rotational()).norm(), 0, tol);
}
...
TEST_F(HydroelasticContactResultsOutputTester, SlipVelocity) {
    const HydroelasticContactInfo<double>& results = contact_results();
    const std::vector<HydroelasticQuadraturePointData<double>>& quadrature_point_data = results.quadrature_point_data();
...
TEST_F(HydroelasticContactResultsOutputTester, Traction) {
    ...
    for (const auto& quadrature_point_datum : quadrature_point_data) {
        ...
        const Vector3d p_barycentric = results.contact_surface().tri_mesh_W().CalcBarycentric(
                quadrature_point_datum.p_WQ, quadrature_point_datum.face_index);

        const double pressure =
                results.contact_surface().tri_e_MN().Evaluate(quadrature_point_datum.face_index, p_barycentric);
        ...
        EXPECT_NEAR((quadrature_point_datum.traction_Aq_W - expected_traction).norm(), 0, tol);
    }
}
...
TEST_F(HydroelasticContactResultsOutputTester, AutoDiffXdSupport) {
    std::unique_ptr<systems::System<AutoDiffXd>> system_ad = diagram_->ToAutoDiffXd();
    systems::Diagram<AutoDiffXd>* diagram_ad = dynamic_cast<systems::Diagram<AutoDiffXd>*>(system_ad.get());
    auto context_ad = diagram_ad->CreateDefaultContext();
    const auto& plant_ad =
            dynamic_cast<const MultibodyPlant<AutoDiffXd>&>(diagram_ad->GetSubsystemByName(plant_->get_name()));
    systems::Context<AutoDiffXd>& plant_context_ad = diagram_ad->GetMutableSubsystemContext(plant_ad, context_ad.get());
    ...
    plant_ad.SetFreeBodyPose(&plant_context_ad, plant_ad.GetBodyByName("Ball"), X_WB);

    const auto& query_object =
            plant_ad.get_geometry_query_input_port().template Eval<geometry::QueryObject<AutoDiffXd>>(plant_context_ad);
    ...
    const geometry::FrameId frame_for_N = query_object.inspector().GetFrameId(contact_surfaces[0].id_N());
    ...
    const ContactResults<AutoDiffXd>& contact_results =
            plant_ad.get_contact_results_output_port().Eval<ContactResults<AutoDiffXd>>(plant_context_ad);
    DRAKE_DEMAND(contact_results.num_hydroelastic_contacts() == 1);
    ...
    EXPECT_TRUE(CompareMatrices(f_Ac_W.x().derivatives(), Vector3d{0, 0, 0}, kEps));
    EXPECT_TRUE(CompareMatrices(f_Ac_W.y().derivatives(), Vector3d{0, 0, 0}, kEps));
