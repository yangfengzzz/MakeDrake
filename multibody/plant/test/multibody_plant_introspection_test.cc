...
GTEST_TEST(MultibodyPlantIntrospection, FloatingBodies) {
    const std::string atlas_url = "package://drake_models/atlas/atlas_convex_hull.urdf";

    const std::string table_url =
            "package://drake/examples/kuka_iiwa_arm/models/table/"
            "extra_heavy_duty_table_surface_only_collision.sdf";

    const std::string mug_url = "package://drake/examples/simple_gripper/simple_mug.sdf";

    MultibodyPlant<double> plant(0.0);

    // Load a model of a table for the environment around the robot.
    Parser parser(&plant);
    const ModelInstanceIndex robot_table_model = parser.AddModelsFromUrl(table_url).at(0);
    plant.WeldFrames(plant.world_frame(), plant.GetFrameByName("link", robot_table_model));

    // Load two Atlas robots.
    const ModelInstanceIndex atlas_model1 = Parser(&plant, "atlas1").AddModelsFromUrl(atlas_url).at(0);
    const ModelInstanceIndex atlas_model2 = Parser(&plant, "atlas2").AddModelsFromUrl(atlas_url).at(0);
    const RigidBody<double>& pelvis1 = plant.GetBodyByName("pelvis", atlas_model1);
    const RigidBody<double>& pelvis2 = plant.GetBodyByName("pelvis", atlas_model2);
    ...
    // we call Finalize().
    DRAKE_EXPECT_THROWS_MESSAGE(mug.is_floating(), ".*The model to which this rigid body belongs must be finalized.*");
    DRAKE_EXPECT_THROWS_MESSAGE(mug.has_quaternion_dofs(),
                                ".*The model to which this rigid body belongs must be finalized.*");
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetFloatingBaseBodies(),
                                "Pre-finalize calls to 'GetFloatingBaseBodies\\(\\)' are not allowed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(plant.GetUniqueFreeBaseBodyOrThrow(robot_table_model),
    ...
    EXPECT_EQ(plant.GetUniqueFreeBaseBodyOrThrow(atlas_model1).index(), pelvis1.index());
    EXPECT_EQ(plant.GetUniqueFreeBaseBodyOrThrow(atlas_model2).index(), pelvis2.index());
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            ...
            "Model " + plant.GetModelInstanceName(robot_table_model) + " has a unique base body, but it is not free.");

    // Retrieve floating bodies.
    std::unordered_set<BodyIndex> expected_floating_bodies({pelvis1.index(), pelvis2.index(), mug.index()});
    ...
    const std::unordered_set<int> expected_floating_positions_start({0, atlas_nq, 2 * atlas_nq});
    const std::unordered_set<int> floating_positions_start(
            {pelvis1.floating_positions_start(), pelvis2.floating_positions_start(), mug.floating_positions_start()});
    EXPECT_EQ(floating_positions_start, expected_floating_positions_start);

    const int atlas_nv = plant.num_velocities(atlas_model1);
    const std::unordered_set<int> expected_floating_velocities_start_in_v({0, atlas_nv, 2 * atlas_nv});
    const std::unordered_set<int> floating_velocities_start_in_v({pelvis1.floating_velocities_start_in_v(),
                                                                  pelvis2.floating_velocities_start_in_v(),
                                                                  mug.floating_velocities_start_in_v()});
    EXPECT_EQ(floating_velocities_start_in_v, expected_floating_velocities_start_in_v);
}
...
GTEST_TEST(MultibodyPlantIntrospection, NonUniqueBaseBody) {
    ...
    const RigidBody<double>& fixed_body =
            plant.AddRigidBody("fixed_body", default_model_instance(), SpatialInertia<double>::NaN());
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            ...
            "Model " + plant.GetModelInstanceName(default_model_instance()) + " does not have a unique base body.");
}
}  // namespace
}  // namespace multibody
}  // namespace drake
