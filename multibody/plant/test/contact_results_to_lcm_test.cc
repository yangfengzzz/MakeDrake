...
#include "systems/lcm/lcm_publisher_system.h"
...
public:
    ...
    template <typename T>
    static unique_ptr<ContactResultsToLcmSystem<T>> Make(const MultibodyPlant<T>& plant,
                                                         const function<string(GeometryId)>& namer = nullptr) {
        /* We want to make sure we explicitly exercise both constructors. */
        if (namer == nullptr) {
            return unique_ptr<ContactResultsToLcmSystem<T>>(new ContactResultsToLcmSystem<T>(plant));
        } else {
            return unique_ptr<ContactResultsToLcmSystem<T>>(new ContactResultsToLcmSystem<T>(plant, namer));
        }
    }

    template <typename T>
    static unordered_map<GeometryId, FullBodyName>& get_geometry_id_to_body_map(ContactResultsToLcmSystem<T>* system) {
        return system->geometry_id_to_body_name_map_;
    }

    template <typename T>
    static void AddToGeometryBodyMap(ContactResultsToLcmSystem<T>* system,
                                     std::initializer_list<unordered_map<GeometryId, FullBodyName>::value_type> items) {
    ...
    template <typename T>
    static void AddToBodyNames(ContactResultsToLcmSystem<T>* system, std::initializer_list<string> items) {
        system->body_names_.insert(system->body_names_.end(), items);
    }

    template <typename T, typename U>
    static bool Equals(const ContactResultsToLcmSystem<T>& system_T, const ContactResultsToLcmSystem<U>& system_U) {
...
public:
    /* Adds the given geometry `id` to the given `body` as one of its collision
     geometries. */
    template <typename T>
    static void AddCollisionGeometryToBody(GeometryId id, const RigidBody<T>& body, MultibodyPlant<T>* plant) {
...
std::ostream& operator<<(std::ostream& out, const FullBodyName& name) {
    out << "Model: '" << name.model << "', Body = '" << name.body << "', Geo: '" << name.geometry
        << "', is_unique: " << name.body_name_is_unique << ", geometry count: " << name.geometry_count;
    return out;
}
...
/* Creates an arbitrary contact surface representing contact between the
 ...
 meshes. */
template <typename T>
ContactSurface<T> MakeContactSurface(GeometryId id_M, GeometryId id_N, const Vector3<T>& offset) {
    ...
    auto mesh = make_unique<TriangleSurfaceMesh<T>>(std::move(faces), std::move(vertices));
    ...
    return ContactSurface<T>(id_M, id_N, std::move(mesh),
                             make_unique<MeshFieldLinear<T, TriangleSurfaceMesh<T>>>(std::move(e_MN), mesh_pointer));
}

/* Makes quadrature data sufficient for the ContactSurface generated by
 MakeContactSurface(). */
template <typename T>
vector<HydroelasticQuadraturePointData<T>> MakeQuadratureData(const Vector3<T>& offset) {
    static_assert(!std::is_same_v<T, Expression>);
    ...
    for (int i = 1; i < kNumPoints; ++i) {
        ...
        quadrature_point_data[i].vt_BqAq_W = quadrature_point_data[i - 1].vt_BqAq_W + offset;
        quadrature_point_data[i].traction_Aq_W = quadrature_point_data[i - 1].traction_Aq_W + offset;
...
protected:
    ...
    static void AddFakePointPairContact(ContactResultsToLcmSystem<U>* lcm_system, ContactResults<U>* results) {
        if (lcm_system != nullptr) {
            ASSERT_EQ(ContactResultsToLcmTester::get_body_names(lcm_system).size(), 1);

            ContactResultsToLcmTester::AddToBodyNames(lcm_system, {"Body1", "Body2", "Body3", "Unreferenced"});
        }
        ...
        static const never_destroyed<PointPairContactInfo<U>> pair1(
                ...
                PenetrationAsPointPair<U>{GeometryId::get_new_id(), GeometryId::get_new_id(),
    ...
    template <typename U = T>
    static void AddFakeHydroContact(ContactResultsToLcmSystem<U>* lcm_system, ContactResults<U>* results) {
        ...
        if (lcm_system != nullptr) {
            ContactResultsToLcmTester::AddToGeometryBodyMap(lcm_system, {{ids[0], {"Model0", "Body0", "Geo0"}},
        ...
        static const never_destroyed<HydroelasticContactInfo<U>> pair1(
                &surface1.access(), SpatialForce<U>(Vector3<U>(1.1, 2.2, 3.3), Vector3<U>(4.4, 5.5, 6.6)),
        ...
        static const never_destroyed<HydroelasticContactInfo<U>> pair2(
                &surface2.access(), SpatialForce<U>(Vector3<U>(1.2, 2.3, 3.4), Vector3<U>(4.5, 5.6, 6.7)),
...
TYPED_TEST(ContactResultsToLcmTest, Constructor) {
    ...
    for (bool use_custom_names : {true, false}) {
        ...
        this->AddBody("dupe_name", model12, namer, &plant, &expected_body_names, &expected_geo_body_map, ref_name);

        ref_name.body_name_is_unique = true;
        ref_name.geometry_count = 1;
        this->AddBody("body2", model12, namer, &plant, &expected_body_names, &expected_geo_body_map, ref_name);

        const ModelInstanceIndex model3 = plant.AddModelInstance("JustForBody3");
        ref_name.body_name_is_unique = false;
        this->AddBody("dupe_name", model3, namer, &plant, &expected_body_names, &expected_geo_body_map, ref_name);

        plant.Finalize();

        SCOPED_TRACE(use_custom_names ? "Using custom names" : "Using default names");
        ...
        const auto& body_names = ContactResultsToLcmTester::get_body_names(lcm.get());
        const auto& id_to_body_map = ContactResultsToLcmTester::get_geometry_id_to_body_map(lcm.get());
...
TYPED_TEST(ContactResultsToLcmTest, EmptyContactResults) {
    ...
    lcm.get_contact_result_input_port().FixValue(context.get(), ContactResults<T>{});
    ...
    auto confirm_empty = [&lcm, &context = *context, kTime]() {
        const auto& message = lcm.get_lcm_message_output_port().template Eval<lcmt_contact_results_for_viz>(context);
    ...
    /* Add some content to the tables and try again. The actual content is
     arbitrary garbage. */
    {
        ContactResultsToLcmTester::AddToBodyNames(&lcm, {"A body", "Another body", "and more"});
...
TYPED_TEST(ContactResultsToLcmTest, PointPairContactOnly) {
    ...
    auto confirm_message = [kTime, &body_names](const lcmt_contact_results_for_viz& message_in,
        ...
        EXPECT_TRUE(CompareMatrices(  // BR
                EigenMapView(pair_message.contact_point), ExtractDoubleOrThrow(pair_data.contact_point())));
        EXPECT_TRUE(CompareMatrices(  // BR
                EigenMapView(pair_message.contact_force), ExtractDoubleOrThrow(pair_data.contact_force())));
        EXPECT_TRUE(CompareMatrices(EigenMapView(pair_message.normal),
...
TYPED_TEST(NonsymbolicContactResultsToLcmTest, HydroContactOnly) {
    ...
    const auto& geo_to_body_map = ContactResultsToLcmTester::get_geometry_id_to_body_map(&lcm);
    auto confirm_message = [&geo_to_body_map](const lcmt_contact_results_for_viz& message_in,
                                              const ContactResults<T>& contacts_in, int i) {
        ...
        EXPECT_TRUE(CompareMatrices(  // BR
                EigenMapView(pair_message.centroid_W), ExtractDoubleOrThrow(mesh.centroid())));
        EXPECT_TRUE(CompareMatrices(EigenMapView(pair_message.force_C_W),
                                    ExtractDoubleOrThrow(pair_data.F_Ac_W().translational())));
        EXPECT_TRUE(CompareMatrices(  // BR
                EigenMapView(pair_message.moment_C_W), ExtractDoubleOrThrow(pair_data.F_Ac_W().rotational())));

        /* Compare meshes and pressure fields. */

        /* Confirm vertices (count and values) and per-vertex pressure values. */
        EXPECT_EQ(static_cast<int>(pair_message.pressure.size()), mesh.num_vertices());
        ...
        EXPECT_EQ(pair_message.num_quadrature_points, mesh.num_triangles() * kNumPointPerTri);
        for (int q = 0; q < static_cast<int>(data_quads.size()); ++q) {
            EXPECT_TRUE(CompareMatrices(  // BR
                    EigenMapView(message_quads[q].p_WQ), ExtractDoubleOrThrow(data_quads[q].p_WQ)));
            EXPECT_TRUE(CompareMatrices(  // BR
                    EigenMapView(message_quads[q].vt_BqAq_W), ExtractDoubleOrThrow(data_quads[q].vt_BqAq_W)));
            EXPECT_TRUE(CompareMatrices(  // BR
                    EigenMapView(message_quads[q].traction_Aq_W), ExtractDoubleOrThrow(data_quads[q].traction_Aq_W)));
        }
    };
...
TYPED_TEST(ContactResultsToLcmTest, Transmogrifcation) {
    ...
    ContactResultsToLcmSystem<T>* lcm_T = dynamic_cast<ContactResultsToLcmSystem<T>*>(system_T.get());
    ...
    const auto& message = lcm_T->get_lcm_message_output_port().template Eval<lcmt_contact_results_for_viz>(*context);
...
protected:
    void AddPlantAndSceneGraphAndOneLink(DiagramBuilder<double>* builder) {
        ...
        plant_->RegisterCollisionGeometry(body, {}, Sphere(1.0), kGeoName, CoulombFriction<double>{});
        plant_->Finalize();
    }
    ...
    void ConfigureDiagram(bool is_nested) {
        if (is_nested) {
            ...
            inner_builder.ExportOutput(plant_->get_contact_results_output_port(), "contact_results");
    ...
    */
    void ExpectValidPublisher(systems::lcm::LcmPublisherSystem* publisher, double expected_publish_period = 1.0 / 64) {
    ...
    void ExpectGeometryNameSemantics(bool expect_default_names) {
        ...
        for (auto* system : builder_.GetMutableSystems()) {
            ...
            if (lcm != nullptr) {
                const auto& id_to_body_map = ContactResultsToLcmTester::get_geometry_id_to_body_map(lcm);
...
TEST_F(ConnectVisualizerTest, ConnectToPlantSceneGraphNames) {
    ...
    auto* publisher = ConnectContactResultsToDrakeVisualizer(&builder_, *plant_, *scene_graph_);
...
TEST_F(ConnectVisualizerTest, ConnectToPortSceneGraphNames) {
    ...
    auto* publisher = ConnectContactResultsToDrakeVisualizer(&builder_, *plant_, *scene_graph_, *contact_results_port_);
    ExpectValidPublisher(publisher);
    ExpectGeometryNameSemantics(false /* expect_default_names */);
}
...
TEST_F(ConnectVisualizerTest, ConnectToPlantSceneGraphNamesWithPeriod) {
    ConfigureDiagram(false /* is_nested */);
    auto* publisher = ConnectContactResultsToDrakeVisualizer(&builder_, *plant_, *scene_graph_, nullptr, 0.5);
