...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, AddMultibodyBasicTest) {
    ...
    EXPECT_EQ(result.plant.get_contact_surface_representation(), geometry::HydroelasticContactRepresentation::kPolygon);
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, AddMultibodySceneGraphConfigTest) {
    ...
    const geometry::SceneGraphConfig scene_graph_config{.default_proximity_properties = {.compliance_type = "compliant",
    ...
    const auto& got_properties = result.scene_graph.get_config().default_proximity_properties;
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, ApplyMultibodyPlantConfigBasicTest) {
    MultibodyPlantConfig config;
    config.time_step = 0.002;
    config.penetration_allowance = 0.003;
    config.stiction_tolerance = 0.004;
    ...
    EXPECT_EQ(plant.get_contact_surface_representation(), geometry::HydroelasticContactRepresentation::kPolygon);
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, YamlTest) {
    ...
    EXPECT_EQ(result.plant.get_discrete_contact_solver(), DiscreteContactSolver::kSap);
    EXPECT_EQ(result.plant.get_discrete_contact_approximation(), DiscreteContactApproximation::kLagged);
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, ContactModelTest) {
    std::vector<std::pair<const char*, ContactModel>> known_values{
            ...
            std::pair("hydroelastic_with_fallback", ContactModel::kHydroelasticWithFallback),
    };
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GetContactModelFromString("foobar"), ".*Unknown.*foobar.*");
}
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, ContactRepresentationTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GetContactSurfaceRepresentationFromString("nonsense"), ".*Unknown.*nonsense.*");
}

GTEST_TEST(MultibodyPlantConfigFunctionsTest, DiscreteContactApproximationTest) {
    std::vector<std::pair<const char*, DiscreteContactApproximation>> known_values{
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GetDiscreteContactApproximationFromString("foobar"), ".*Unknown.*foobar.*");
}
...
GTEST_TEST(MultibodyPlantConfigFunctionsTest, DiscreteContactApproximationAndDiscreteContactSolver) {
    MultibodyPlantConfig config;
    ...
    config.discrete_contact_solver = "";
    {
        MultibodyPlant<double> plant(config.time_step);
        ApplyMultibodyPlantConfig(config, &plant);
        EXPECT_EQ(plant.get_discrete_contact_approximation(), DiscreteContactApproximation::kLagged);
