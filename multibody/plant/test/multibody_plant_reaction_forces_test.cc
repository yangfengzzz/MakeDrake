...
public:
    ...
    static CompliantContactManager<double>& manager(const MultibodyPlant<double>& plant) {
        auto* manager = dynamic_cast<CompliantContactManager<double>*>(plant.discrete_update_manager_.get());
...
protected:
    void BuildLadderModel() {
        ...
        std::tie(plant_, scene_graph_) =
                AddMultibodyPlantSceneGraph(&builder, std::make_unique<MultibodyPlant<double>>(config.time_step));

        AddWall(config.hydro_geometry);
        AddPinnedLadder(config);
        plant_->mutable_gravity_field().set_gravity_vector(Vector3d(0.0, 0.0, -kGravity));
        ...
        if (plant_->is_discrete() && config.contact_solver == DiscreteContactSolver::kSap) {
        ...
        ConnectContactResultsToDrakeVisualizer(&builder, *plant_, *scene_graph_, &lcm_);
    ...
    void AddWall(bool hydro_geometry = false) {
        ...
        const RigidTransformd X_WB = Eigen::Translation3d(kDistanceToWall + kWallWidth / 2.0, 0.0, kWallHeight / 2.0);
        const Vector4<double> green(0.5, 1.0, 0.5, 1.0);
        const auto shape = geometry::Box(size(0), size(1), size(2));
        plant_->RegisterVisualGeometry(plant_->world_body(), X_WB, shape, "wall_visual", green);

        geometry::ProximityProperties properties;
        properties.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction,
                               CoulombFriction<double>(kFrictionCoefficient, kFrictionCoefficient));

        if (hydro_geometry) {
            properties.AddProperty(kHydroGroup, kComplianceType, HydroelasticType::kRigid);
        ...
        plant_->RegisterCollisionGeometry(plant_->world_body(), X_WB, shape, "wall_collision", properties);
    }
    ...
    void AddPinnedLadder(const LadderTestConfig& config) {
        ...
        const SpatialInertia<double> M_BBo_B = SpatialInertia<double>::ThinRodWithMassAboutEnd(
        ...
        const RigidTransformd X_BV(Vector3d(-kLadderWidth / 2.0, 0.0, kLadderLength / 4.0));
        const Vector4<double> light_blue(0.5, 0.8, 1.0, 1.0);
        const Vector4<double> dark_blue(0.0, 0.0, 0.8, 1.0);
        plant_->RegisterVisualGeometry(*ladder_lower_, X_BV, shape, "LadderLowerVisualGeometry", light_blue);
        plant_->RegisterVisualGeometry(*ladder_upper_, X_BV, shape, "LadderUpperVisualGeometry", dark_blue);
        ...
        // case.
        const RigidTransformd X_BC(Vector3d(0.0, kProblemWidth / 4.0, kLadderLength / 2.0));
        ...
        if (config.hydro_geometry) {
            properties.AddProperty(kHydroGroup, kComplianceType, HydroelasticType::kSoft);
        ...
        ladder_upper_geometry_id_ = plant_->RegisterCollisionGeometry(*ladder_upper_, X_BC, Sphere(kPointContactRadius),
                                                                      "LadderUpperCollisionGeometry", properties);
        plant_->RegisterVisualGeometry(*ladder_upper_, X_BC, Sphere(kPointContactRadius),
                                       "LadderUpperCollisionVisualGeometry", Vector4<double>(0.0, 0.0, 0.8, 0.3));

        // Pin to the floor with a revolute joint.
        pin_ = &plant_->AddJoint<RevoluteJoint>("PinToGround", plant_->world_body(), {}, *ladder_lower_, {},
        ...
        switch (config.weld_method) {
            case LadderTestConfig::WeldMethod::kWeldJoint: {
                joint_ = &plant_->WeldFrames(ladder_lower_->body_frame(), ladder_upper_->body_frame(), X_BlBu);
                break;
            }
            case LadderTestConfig::WeldMethod::kRevoluteJointWithLimits: {
                const double kInf = std::numeric_limits<double>::infinity();
                joint_ = &plant_->AddJoint<RevoluteJoint>("MiddleJoint", *ladder_lower_, X_BlBu, *ladder_upper_, {},
    ...
    std::unique_ptr<Simulator<double>> Simulate(std::unique_ptr<Context<double>> diagram_context) {
        Context<double>* plant_context = &diagram_->GetMutableSubsystemContext(*plant_, diagram_context.get());
        ...
        const double norm_EU = (kLadderLength * kPointContactRadius / (kDistanceToWall - kPointContactRadius));
        ...
        const LadderTestConfig& config = GetParam();
        auto simulator = std::make_unique<Simulator<double>>(*diagram_, std::move(diagram_context));
    ...
    void VerifyJointReactionForces(Context<double>* diagram_context, bool hydro_geometry = false) {
        Context<double>* plant_context = &diagram_->GetMutableSubsystemContext(*plant_, diagram_context);
        ...
        const auto& reaction_forces =
                plant_->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*plant_context);
        ASSERT_EQ(reaction_forces.size(), 2u);
        const SpatialForce<double>& F_Bl_Bl = reaction_forces[pin_->ordinal()];
        const RigidTransformd X_WBl = ladder_lower_->EvalPoseInWorld(*plant_context);
        ...
        if (hydro_geometry) {
            ...
            const double direction =
                    hydroelastic_contact_info.contact_surface().id_M() == ladder_upper_geometry_id_ ? 1.0 : -1.0;
            ...
            const Vector3d p_CP_W(0.0, 0.0, F_Bc_W.rotational().y() / F_Bc_W.translational().x());
            ...
            const PointPairContactInfo<double>& point_pair_contact_info = contact_results.point_pair_contact_info(0);

            const double direction = point_pair_contact_info.bodyB_index() == ladder_upper_->index() ? 1.0 : -1.0;
        ...
        const Vector3d p_WBcm = plant_->CalcCenterOfMassPositionInWorld(*plant_context);
        ...
        EXPECT_TRUE(CompareMatrices(F_Bl_W.translational(), f_Bl_W_expected, kTolerance, MatrixCompareType::relative));

        // Expected contact force.
        const Vector3d f_C_W_expected(-fc_x, 0.0, 0.0);
        EXPECT_TRUE(CompareMatrices(f_Bp_W, f_C_W_expected, kTolerance, MatrixCompareType::relative));
        ...
        EXPECT_TRUE(CompareMatrices(F_Bl_W.rotational(), t_Bl_W_expected, kTolerance, MatrixCompareType::relative));
        ...
        // Upper half's origin.
        const RigidTransformd X_WBu = ladder_upper_->EvalPoseInWorld(*plant_context);
        const Vector3d p_WBu = X_WBu.translation();
        // Upper half's COM in W.
        const Vector3d p_BuBucm = ladder_upper_->CalcCenterOfMassInBodyFrame(*plant_context);
        const Vector3d p_WBucm = X_WBu * p_BuBucm;
        // Reaction forces at X_WBu in W.
        const SpatialForce<double>& F_Bu_W = X_WBu.rotation() * reaction_forces[joint_->ordinal()];
        ...
        const double t_Bu_y = -(weight / 2.0) * (p_WBucm.x() - p_WBu.x()) + fc_x * (p_WP.z() - p_WBu.z());
        ...
        EXPECT_TRUE(CompareMatrices(F_Bu_W.rotational(), t_Bu_expected, kTolerance, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(F_Bu_W.translational(), f_Bu_expected, kTolerance, MatrixCompareType::relative));
    }
    ...
    void TestWithThreads() {
        ...
        VerifyJointReactionForces(&simulator_prototype->get_mutable_context(), config.hydro_geometry);
        ...
        // is fixed.
        const Context<double>& sg_context =
                diagram_->GetSubsystemContext(*scene_graph_, simulator_prototype->get_context());
        const auto& query_object =
                scene_graph_->get_query_output_port().Eval<geometry::QueryObject<double>>(sg_context);
...
std::vector<LadderTestConfig> MakeTestCases() {
    return std::vector<LadderTestConfig>{
            ...
            {.description = "ContinuousPoint", .time_step = 0.0, .hydro_geometry = false},
            {.description = "ContinuousHydroelastic", .time_step = 0.0, .hydro_geometry = true},
...
protected:
    void BuildModel(double discrete_update_period) {
        ...
        const SpatialInertia<double> M_BBo_B =
                SpatialInertia<double>::ThinRodWithMass(kMass, kLength, Vector3d::UnitZ());
        ...
        // axis.
        const RigidTransformd X_BJb(RollPitchYawd(M_PI_2, 0.0, 0.0), Vector3d(0.0, 0.0, -kLength / 2.0));
        pin_ = &plant_->AddJoint<RevoluteJoint>("pin", plant_->world_body(), {}, *rod_, X_BJb, Vector3d::UnitZ());
    ...
    void VerifyJointReactionForces() {
        ...
        const Vector3d Bz_W = rod_->EvalPoseInWorld(*context_).rotation().matrix().col(2);
        ...
        const auto& reaction_forces =
                plant_->get_reaction_forces_output_port().Eval<std::vector<SpatialForce<double>>>(*context_);
        ...
        // axis).
        const Vector3d f_BJb_Jb_expected(0.0, -kMass * kLength / 2.0 * kOmega * kOmega, kMass * kGravity);
        EXPECT_TRUE(CompareMatrices(F_BJb_Jb.translational(), f_BJb_Jb_expected, kTolerance));

        // The reaction torque must counteract gravity.
        const Vector3d t_BJb_Jb_expected(kMass * kGravity * kLength / 2.0, 0.0, 0.0);
        EXPECT_TRUE(CompareMatrices(F_BJb_Jb.rotational(), t_BJb_Jb_expected, kTolerance));
    }
...
protected:
    ...
    void AddBoxes() {
        const SpatialInertia<double> M_BBo_B = SpatialInertia<double>::SolidCubeWithMass(kBoxMass, kCubeSize);
    ...
    void VerifyBodyReactionForces() {
        ...
        const Vector3d t_Acm_W_expected = -kCubeSize * box_weight * Vector3d::UnitY();
...
public:
    void SetUp() {
        ...
        const RigidBody<double>& sphere0 =
                plant_->AddRigidBody("sphere0", SpatialInertia<double>::SolidSphereWithMass(kBodyMasses[0], 1.0));
        const RigidBody<double>& sphere1 =
                plant_->AddRigidBody("sphere1", SpatialInertia<double>::SolidSphereWithMass(kBodyMasses[1], 1.0));
        const RigidBody<double>& sphere2 =
                plant_->AddRigidBody("sphere2", SpatialInertia<double>::SolidSphereWithMass(kBodyMasses[2], 1.0));
        const RigidBody<double>& sphere3 =
                plant_->AddRigidBody("sphere3", SpatialInertia<double>::SolidSphereWithMass(kBodyMasses[3], 1.0));

        // Make sphere0 and sphere1 floating and weld sphere2 and sphere3 to world.
        floating0_ = &plant_->AddJoint<QuaternionFloatingJoint>("floating0", plant_->world_body(), {}, sphere0, {});
        floating1_ = &plant_->AddJoint<QuaternionFloatingJoint>("floating1", plant_->world_body(), {}, sphere1, {});
        weld2_ = &plant_->AddJoint<WeldJoint>("weld2", plant_->world_body(), {}, sphere2, {},
                                              RigidTransformd::Identity());
        weld3_ = &plant_->AddJoint<WeldJoint>("weld3", plant_->world_body(), {}, sphere3, {},
                                              RigidTransformd::Identity());
        ...
        if (replace_joints) {
            ...
            floating0_ = nullptr;
            floating1_ = nullptr;
            weld0_ = &plant_->AddJoint<WeldJoint>("weld0", plant_->world_body(), {}, sphere0, {},
                                                  RigidTransformd::Identity());
            weld1_ = &plant_->AddJoint<WeldJoint>("weld1", plant_->world_body(), {}, sphere1, {},
                                                  RigidTransformd::Identity());
        }
        plant_->mutable_gravity_field().set_gravity_vector(Vector3d(0.0, 0.0, -kGravity));
        plant_->Finalize();
...
TEST_P(WeldedAndFloatingTest, ReactionForcesOrdinalIndexing) {
    ...
    if (replace_joints) {
        ...
        const std::vector<const Joint<double>*> joints{weld0_, weld1_, weld2_, weld3_};
        const std::vector<JointIndex> expected_indices{JointIndex(4), JointIndex(5), JointIndex(2), JointIndex(3)};
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            const SpatialForce<double>& F_Bcm_W = reaction_forces[joints[i]->ordinal()];
            EXPECT_EQ(F_Bcm_W.translational(), kBodyMasses[i] * kGravity * Vector3d::UnitZ());
        ...
        const std::vector<const Joint<double>*> joints{floating0_, floating1_, weld2_, weld3_};
        const std::vector<JointIndex> expected_indices{JointIndex(0), JointIndex(1), JointIndex(2), JointIndex(3)};
...
INSTANTIATE_TEST_SUITE_P(ReactionForcesTests, WeldedAndFloatingTest, testing::ValuesIn({false, true}));
