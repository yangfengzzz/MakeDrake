...
public:
    ...
    void MakeModel() {
        ...
        plant_ = &AddMultibodyPlantSceneGraph(&builder, 0.01 /* Discrete model */).plant;
        plant_->set_discrete_contact_approximation(GetParam().contact_approximation);

        // Arbitrary inertia values only used by the driver to build a valid contact
        // problem.
        const double mass = 1.5;
        const double radius = 0.1;
        const SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(mass, radius);
        ...
        plant_->RegisterCollisionGeometry(*bodyA_, RigidTransformd(), shape, "bodyA_collision", properties);
        bodyB_ = &plant_->AddRigidBody("B", M_BBcm);
        plant_->RegisterCollisionGeometry(*bodyB_, RigidTransformd(), shape, "bodyB_collision", properties);
        ...
        const RigidTransformd X_WB(Vector3d(2.0 * kRadius_ - kPenetration_, 0.0, 0.0));
...
protected:
    ProximityProperties MakeProximityProperties() const {
        ...
        geometry::AddContactMaterial(kHuntCrossleyDissipation_, kStiffness_,
...
TEST_P(TwoBodiesTest, ConfirmContactConstraintProperties) {
    ...
    EXPECT_EQ(constraint.second_clique_jacobian().MakeDenseMatrix(), discrete_pair.jacobian[1].J.MakeDenseMatrix());

    const ContactConfiguration<double> expected_configuration =
            multibody::contact_solvers::internal::MakeContactConfiguration(discrete_pair);
    ...
    if (GetParam().contact_approximation == DiscreteContactApproximation::kSap) {
        // Constraint type.
        const auto* contact_constraint = dynamic_cast<const SapFrictionConeConstraint<double>*>(&constraint);
        ASSERT_NE(contact_constraint, nullptr);
        // Configuration.
        EXPECT_EQ(contact_constraint->configuration(), expected_configuration);
        // Parameters.
        EXPECT_EQ(contact_constraint->parameters().mu, discrete_pair.friction_coefficient);
        EXPECT_EQ(contact_constraint->parameters().stiffness, discrete_pair.stiffness);
        EXPECT_EQ(contact_constraint->parameters().dissipation_time_scale, discrete_pair.dissipation_time_scale);
        EXPECT_EQ(contact_constraint->parameters().beta, plant_->get_sap_near_rigid_threshold());
        ...
        // Constraint type.
        const auto* contact_constraint = dynamic_cast<const SapHuntCrossleyConstraint<double>*>(&constraint);
        ASSERT_NE(contact_constraint, nullptr);
        // Configuration.
        EXPECT_EQ(contact_constraint->configuration(), expected_configuration);
        // Parameters.
        const SapHuntCrossleyApproximation expected_model =
                GetParam().contact_approximation == DiscreteContactApproximation::kLagged
        ...
        EXPECT_EQ(contact_constraint->parameters().friction, discrete_pair.friction_coefficient);
        EXPECT_EQ(contact_constraint->parameters().stiffness, discrete_pair.stiffness);
        EXPECT_EQ(contact_constraint->parameters().dissipation, discrete_pair.damping);
        EXPECT_EQ(contact_constraint->parameters().stiction_tolerance, plant_->stiction_tolerance());
...
std::vector<TestConfig> MakeTestCases() {
    return std::vector<TestConfig>{
            {.description = "SAP", .contact_approximation = DiscreteContactApproximation::kSap},
            {.description = "Lagged", .contact_approximation = DiscreteContactApproximation::kLagged},
            {.description = "Similar", .contact_approximation = DiscreteContactApproximation::kSimilar},
    };
}

INSTANTIATE_TEST_SUITE_P(SapDriverContactConstraintsTests,
                         TwoBodiesTest,
