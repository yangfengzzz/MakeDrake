...
DeformableBodyId RegisterDeformableOctahedron(DeformableModel<double>* model,
    ...
    auto geometry = make_unique<GeometryInstance>(X_WF, make_unique<Sphere>(1.0), std::move(name));
    geometry::ProximityProperties props;
    geometry::AddContactMaterial({}, {}, CoulombFriction<double>(kMu, kMu), &props);
...
class DeformableDriverContactKinematicsTest : public ::testing::TestWithParam<bool> {
protected:
    void SetUp() {}
    ...
    void MakeDeformableRigidScene(bool dynamic_rigid_body) {
        ...
        deformable_body_id_ = RegisterDeformableOctahedron(&deformable_model, "deformable", X_WF_);
        ...
        geometry::AddContactMaterial({}, {}, CoulombFriction<double>(1.0, 1.0), &rigid_proximity_props);
        // TODO(xuchenhan-tri): Modify this when resolution hint is no longer used
        //  as the trigger for contact with deformable bodies.
        rigid_proximity_props.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kRezHint, 1.0);
        ...
        if (dynamic_rigid_body) {
            ...
            const RigidBody<double>& rigid_body = plant_->AddRigidBody("rigid_body", SpatialInertia<double>::NaN());
            rigid_geometry_id_ = plant_->RegisterCollisionGeometry(rigid_body, X_BR, geometry::Box(10, 10, 1),
            ...
            rigid_geometry_id_ = plant_->RegisterCollisionGeometry(plant_->world_body(), X_WR, geometry::Box(10, 10, 1),
        ...
        if (dynamic_rigid_body) {
            ...
            deformable_model.AddFixedConstraint(deformable_body_id_, plant_->get_body(rigid_body_index_), X_BF,
                                                geometry::Box(10, 10, 1), X_BR);
        } else {
            deformable_model.AddFixedConstraint(deformable_body_id_, plant_->world_body(), X_WF_,
                                                geometry::Box(10, 10, 1), X_BR);
        }
        ...
        if (dynamic_rigid_body) {
            ...
            Context<double>& mutable_plant_context = plant_->GetMyMutableContextFromRoot(context_.get());
            /* Only the deformable geometry can move; it gets the full desired
             relative velocity. */
            const Vector3d v_WR_F = Vector3d(0, 0, 0.5);
            const SpatialVelocity<double> V_WR(Vector3d::Zero(), X_WF_.rotation() * v_WR_F);
            plant_->SetFreeBodySpatialVelocity(&mutable_plant_context, plant_->get_body(rigid_body_index_), V_WR);
        } else {
    ...
    void MakeDeformableDeformableScene() {
        ...
        MultibodyPlantConfig config{.time_step = kDt, .discrete_contact_approximation = "sap"};
        ...
        deformable_body_id2_ = RegisterDeformableOctahedron(&deformable_model, "deformable2",
    ...
    void ValidateDeformableDeformableContactPairs() {
        ...
        const math::RotationMatrixd expected_R_WC = math::RotationMatrixd::MakeFromOneUnitVector(nhat_AB_W, kZAxis);
        ...
        for (int i = 0; i < num_contact_points; ++i) {
            ...
            const DeformableBodyIndex body_index = model_->GetBodyIndex(deformable_body_id_);
            const int object_A = body_index + plant_->num_bodies();
            EXPECT_EQ(contact_pair.object_A, object_A);
            const DeformableBodyIndex body_index2 = model_->GetBodyIndex(deformable_body_id2_);
            ...
            EXPECT_TRUE(CompareMatrices(contact_pair.nhat_BA_W, -nhat_AB_W, kTolerance));
            ...
            const Vector3d expected_p_ApC_W = expected_p_WC - contact_surface.contact_mesh_W().centroid();
            ...
            const double expected_k =
                    1e8 * contact_surface.contact_mesh_W().area(i) / 2.0;  // see implementation notes for why this
    ...
    void ValidateDeformableRigidContactKinematics(bool dynamic_rigid_body) {
        ...
        constexpr int kZAxis = 2;
        const math::RotationMatrixd expected_R_WC = math::RotationMatrixd::MakeFromOneUnitVector(nhat_W, kZAxis);
        /* Velocities of the participating deformable dofs. */
        ...
        for (int i = 0; i < num_contact_points; ++i) {
            ...
            if (dynamic_rigid_body) {
                ...
                EXPECT_TRUE(CompareMatrices(J0 * v0 + J1 * v1, expected_v_DpRp_C, kTolerance));
    ...
    void ValidateFixedConstraintKinematics(bool dynamic_rigid_body) {
        ...
        driver_->AppendDeformableRigidFixedConstraintKinematics(plant_context, &constraint_kinematics);
        ...
        const FixedConstraintKinematics<double>& constraint_kinematic = constraint_kinematics[0];
        if (dynamic_rigid_body) {
            ASSERT_EQ(constraint_kinematic.J.num_cliques(), 2);
            const Matrix3X<double> J0 = constraint_kinematic.J.clique_jacobian(0).MakeDenseMatrix();
            ASSERT_EQ(v0.size(), J0.cols());
            const Matrix3X<double> J1 = constraint_kinematic.J.clique_jacobian(1).MakeDenseMatrix();
            const Vector3d v_WR_F(0, 0, 0.5);
            const Vector3d v_WR = X_WF_.rotation() * v_WR_F;
            const Vector3d w_WR(0, 0, 0);
            Vector6<double> v1;
    ...
    void ValidateConstraintParticipation() {
        ...
        const ContactParticipation& participation =
                driver_->EvalConstraintParticipation(plant_->GetMyContextFromRoot(*context_), body_index);
        ...
        /*
         ...
         |        6         |        5         |       yes         |
        */
        EXPECT_THAT(participation.CalcVertexPermutation().permutation(), testing::ElementsAre(0, 1, 2, 3, 4, 6, 5));
        if (deformable_body_id2_.is_valid()) {
            ...
            const ContactParticipation& participation2 =
                    driver_->EvalConstraintParticipation(plant_->GetMyContextFromRoot(*context_), body_index2);
            EXPECT_EQ(participation.num_vertices_in_contact(), 6);
            /*
             |   Original       |   Permuted       |   Participating   |
             |   vertex index   |   vertex index   |   in contact      |
...
private:
    ...
    int GetNumContactPoints(const Context<double>& plant_context) const {
        const DeformableContact<double>& contact_data = driver_->EvalDeformableContact(plant_context);
...
TEST_P(DeformableDriverContactKinematicsTest, DeformableRigidConstraintKinematics) {
...
INSTANTIATE_TEST_SUITE_P(All, DeformableDriverContactKinematicsTest, ::testing::Values(true, false));

/* Tests deformable vs. deformable contact. */
TEST_F(DeformableDriverContactKinematicsTest, DeformableDeformableContactKinematics) {
...
public:
    static const PartialPermutation& EvalVertexPermutation(const DeformableDriver<double>& driver,
...
GTEST_TEST(DeformableDriverContactKinematicsWithBcTest, ContactJacobianWithBoundaryCondition) {
    systems::DiagramBuilder<double> builder;
    constexpr double kDt = 0.01;
    auto [plant, scene_graph] = AddMultibodyPlantSceneGraph(&builder, kDt);
    DeformableModel<double>& deformable_model = plant.mutable_deformable_model();
    DeformableBodyId body_id =
            RegisterDeformableOctahedron(&deformable_model, "deformable", RigidTransformd::Identity());
    /* Put the bottom vertex under bc. */
    deformable_model.SetWallBoundaryCondition(body_id, Vector3d(0, 0, -0.5), Vector3d(0, 0, 1));
    ...
    /* Register a static rigid geometry in contact with the bottom half of the
     octahedron. */
    plant.RegisterCollisionGeometry(plant.world_body(), X_WR, geometry::Box(10, 10, 1), "static_collision_geometry",
    ...
    const PartialPermutation& vertex_permutation = DeformableDriverTest::EvalVertexPermutation(
    ...
    for (int i = 0; i < static_cast<int>(contact_pairs.size()); ++i) {
        ...
        EXPECT_TRUE(CompareMatrices(J.middleCols<3>(3 * bottom_vertex_permuted_index), Matrix3d::Zero()));
    }
}
...
GTEST_TEST(DeformableDriverConstraintParticipation, ConstraintWithoutContact) {
    ...
    deformable_model.AddFixedConstraint(body_id, plant.world_body(), RigidTransformd::Identity(),
    ...
    const Context<double>& plant_context = plant.GetMyContextFromRoot(*context);
    const ContactParticipation& participation =
            driver->EvalConstraintParticipation(plant_context, DeformableBodyIndex(0));
