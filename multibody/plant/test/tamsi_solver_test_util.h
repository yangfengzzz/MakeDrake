...
VectorX<U> CalcNormalForces(const VectorX<U>& x,
    ...
    const VectorX<U> k_vn = stiffness.array() * (VectorX<U>::Ones(nc).array() - dissipation.array() * vn.array());
...
VectorX<U> CalcFrictionForces(
        double v_stiction, double epsilon_v, const VectorX<double> mu, const VectorX<U>& vt, const VectorX<U>& fn) {
    using std::sqrt;
...
template <typename U>
VectorX<U> CalcResidual(const MatrixX<double>& M,
    ...
    VectorX<U> residual = M * v - p_star - dt * Jt.transpose() * ft - dt * Jn.transpose() * fn;
...
MatrixX<double> CalcTwoWayCoupledJacobianWithAutoDiff(const MatrixX<double>& M,
                                                      const MatrixX<double>& Jn,
                                                      const MatrixX<double>& Jt,
                                                      const VectorX<double>& p_star,
                                                      const VectorX<double>& x0,
    ...
    VectorX<AutoDiffXd> residual = CalcResidual(M, Jn, Jt, p_star, x0, mu, not_used, stiffness, dissipation, dt,
                                                v_stiction, epsilon_v, true, v_autodiff);
    return math::ExtractGradient(residual);
}

// Computes the Jacobian J = ∇ᵥR of the residual for the one-way coupled scheme
// of TamsiSolver using automatic differentiation.
MatrixX<double> CalcOneWayCoupledJacobianWithAutoDiff(const MatrixX<double>& M,
                                                      const MatrixX<double>& Jn,
                                                      const MatrixX<double>& Jt,
                                                      const VectorX<double>& p_star,
                                                      const VectorX<double>& mu,
    ...
    const VectorX<double> not_used;
    VectorX<AutoDiffXd> residual = CalcResidual(M, Jn, Jt, p_star, not_used, mu, fn, not_used, not_used, dt, v_stiction,
                                                epsilon_v, false, v_autodiff);
    return math::ExtractGradient(residual);
}
