...
public:
    ...
    //   - Velocities are zero.
    void SetupRigidGroundCompliantSphereAndNonHydroSphere(bool sphere1_on_prismatic_joint = false) {
        const ContactParameters compliant_contact{1.0e5, 1.0e5, 0.01, 1.0};
        const ContactParameters non_hydro_contact{1.0e5, {}, 0.01, 1.0};
        const ContactParameters rigid_hydro_contact{std::nullopt, std::numeric_limits<double>::infinity(), 0.0, 1.0};
        const SphereParameters sphere1_params{"Sphere1", 10.0 /* mass */, 0.2 /* size */, compliant_contact};
        const SphereParameters sphere2_params{"Sphere2", 10.0 /* mass */, 0.2 /* size */, non_hydro_contact};
        MakeModel(rigid_hydro_contact, sphere1_params, sphere2_params, sphere1_on_prismatic_joint);
    }

    void SetupFreeFloatingSpheresWithNoContact() {
        const bool sphere1_on_prismatic_joint = false;
        const SphereParameters sphere1_params{"Sphere1", 10.0 /* mass */, 0.2 /* size */,
                                              std::nullopt /* no contact */};
        const SphereParameters sphere2_params{"Sphere2", 10.0 /* mass */, 0.2 /* size */,
                                              std::nullopt /* no contact */};
        MakeModel(std::nullopt /* no ground */, sphere1_params, sphere2_params, sphere1_on_prismatic_joint);
    }
    ...
    void MakeModel(const std::optional<ContactParameters>& ground_params,
        ...
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, time_step_);
        plant_->set_discrete_contact_approximation(DiscreteContactApproximation::kSap);

        // Add model of the ground.
        if (ground_params) {
            const ProximityProperties ground_properties = MakeProximityProperties(*ground_params);
            plant_->RegisterCollisionGeometry(plant_->world_body(), RigidTransformd(), geometry::HalfSpace(),
                                              "ground_collision", ground_properties);
            const Vector4<double> green(0.5, 1.0, 0.5, 1.0);
            plant_->RegisterVisualGeometry(plant_->world_body(), RigidTransformd(), geometry::HalfSpace(),
                                           "ground_visual", green);
        }
        ...
        if (sphere1_on_prismatic_joint) {
            slider1_ = &plant_->AddJoint<PrismaticJoint>("Sphere1Slider", plant_->world_body(), std::nullopt, *sphere1_,
                                                         std::nullopt, Vector3<double>::UnitZ(),
                                                         sphere1_params.radius /* lower limit */);
        }

        plant_->mutable_gravity_field().set_gravity_vector(-gravity_ * Vector3d::UnitZ());

        plant_->set_contact_model(drake::multibody::ContactModel::kHydroelasticWithFallback);
    ...
    void SetContactState(const SphereParameters& sphere1_params, const SphereParameters& sphere2_params) const {
        ...
        const double sphere2_com_z = 2.0 * sphere1_params.radius + sphere2_params.radius - 2.0 * penetration_distance_;
        const RigidTransformd X_WB2(Vector3d(0, 0, sphere2_com_z));
        plant_->SetFreeBodyPose(plant_context_, *sphere2_, X_WB2);
    }

    const MultibodyPlant<double>& plant() const { return *plant_; }
    const CompliantContactManager<double>& manager() const { return *contact_manager_; }
...
private:
    ...
    const RigidBody<double>& AddSphere(const SphereParameters& params) {
        // Add rigid body.
        const SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(params.mass, params.radius);
        ...
        if (params.contact_parameters) {
            ...
            const ProximityProperties properties = MakeProximityProperties(*params.contact_parameters);
            plant_->RegisterCollisionGeometry(body, RigidTransformd(), shape, params.name + "_collision", properties);
        }
    ...
    static ProximityProperties MakeProximityProperties(const ContactParameters& params) {
        ...
        if (params.hydro_modulus) {
            if (params.hydro_modulus == std::numeric_limits<double>::infinity()) {
                geometry::AddRigidHydroelasticProperties(/* resolution_hint */ 1.0, &properties);
            } else {
            ...
            properties.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kSlabThickness, 1.0);
        }
        geometry::AddContactMaterial(
                /* "hunt_crossley_dissipation" */ {}, params.point_stiffness,
                CoulombFriction<double>(params.friction_coefficient, params.friction_coefficient), &properties);

        if (params.relaxation_time.has_value()) {
            properties.AddProperty(geometry::internal::kMaterialGroup, "relaxation_time", *params.relaxation_time);
        }
        return properties;
    }
};
