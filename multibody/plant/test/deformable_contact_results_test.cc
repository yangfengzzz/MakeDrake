...
GTEST_TEST(CompliantContactManagerTest, ContactResultsWithDeformable) {
    ...
    geometry::AddContactMaterial({}, {}, CoulombFriction<double>(1.0, 1.0), &hydro_proximity_properties);
    plant.RegisterCollisionGeometry(plant.world_body(), RigidTransformd(), geometry::Box(10, 10, 10),
                                    "ground_collision", hydro_proximity_properties);
    ...
    plant.RegisterCollisionGeometry(body1, RigidTransformd(Vector3d(5, 5, 5)), geometry::Box(1, 1, 1), "hydro box",
    ...
    geometry::AddContactMaterial(1e6, 1.0, CoulombFriction<double>(1.0, 1.0), &point_proximity_properties);
    plant.RegisterCollisionGeometry(body2, RigidTransformd(Vector3d(-5, -5, 5)), geometry::Box(1, 1, 1),
                                    "point contact box", point_proximity_properties);
    DeformableModel<double>& deformable_model = plant.mutable_deformable_model();
    /* Add a deformable sphere that collides with the ground but not with any
     other rigid bodies. */
    auto deformable_geometry = std::make_unique<GeometryInstance>(RigidTransformd(Vector3d(0, 0, 5)),
                                                                  std::make_unique<Sphere>(1.0), "sphere");
    ProximityProperties deformable_proximity_props;
    geometry::AddContactMaterial({}, {}, CoulombFriction<double>(1.0, 1.0), &deformable_proximity_props);
    deformable_geometry->set_proximity_properties(std::move(deformable_proximity_props));
    multibody::fem::DeformableBodyConfig<double> body_config;
    constexpr double kRezHint = 10.0;
    deformable_model.RegisterDeformableBody(std::move(deformable_geometry), body_config, kRezHint);
    plant.Finalize();
    ...
    const auto& contact_results = plant.get_contact_results_output_port().Eval<ContactResults<double>>(plant_context);
