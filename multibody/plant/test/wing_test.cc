#include "multibody/plant/wing.h"
...
GTEST_TEST(WingTest, FallingFlatPlate) {
    ...
    const RigidBody<double>& body = plant->GetBodyByName("box");
    Wing<double>* wing = Wing<double>::AddToBuilder(&builder, plant, body.index(), kSurfaceArea,
    ...
    const Vector6<double> vdot_gravity_only = plant->CalcGravityGeneralizedForces(plant_context);

    systems::Context<double>& wing_context = wing->GetMyMutableContextFromRoot(context.get());

    EXPECT_EQ(wing->get_aerodynamic_center_output_port().Eval(wing_context), Vector3d::Zero());
    ...
    {  // Zero velocity.
        ...
        EXPECT_TRUE(CompareMatrices(plant->EvalTimeDerivatives(plant_context).get_generalized_velocity().CopyToVector(),
                                    vdot_expected, 1e-14));
    }
    ...
    {  // Falling straight down, with orientation.
        const math::RotationMatrix<double> R_WB(math::RollPitchYaw<double>(0.1, 0.2, 0.3));
        plant->SetFreeBodyPose(&plant_context, body, math::RigidTransform<double>(R_WB));
        const double zdot = -1.23;
        const SpatialVelocity<double> V_WB(Vector3d::Zero(), Vector3d{0, 0, zdot});
        plant->SetFreeBodySpatialVelocity(&plant_context, body, V_WB);
        ...
        const double longitudinal_velocity_norm = Eigen::Vector2d(v_WindBody_Wing[0], v_WindBody_Wing[2]).norm();
        vdot_expected.tail<3>() +=
                R_WB * Vector3d{0, 0, -kRho * kSurfaceArea * v_WindBody_Wing[2] * longitudinal_velocity_norm};
        EXPECT_TRUE(CompareMatrices(plant->EvalTimeDerivatives(plant_context).get_generalized_velocity().CopyToVector(),
                                    vdot_expected, 1e-14));
    }

    {  // Zero ground velocity, but with wind.
        const math::RotationMatrix<double> R_WWind(math::RollPitchYaw<double>(0.4, 0.5, 0.6));
        ...
        plant->SetFreeBodyPose(&plant_context, body, math::RigidTransform<double>());
        const SpatialVelocity<double> V_WB(Vector3d::Zero(), Vector3d::Zero());
        plant->SetFreeBodySpatialVelocity(&plant_context, body, V_WB);

        Vector6<double> vdot_expected = vdot_gravity_only;
        // force in z = ρ S (normal⋅v) |v|, where the normal here is (0, 0, 1).
        const double longitudinal_velocity_norm = Eigen::Vector2d(v_wind[0], v_wind[2]).norm();
        vdot_expected.tail<3>() += Vector3d{0, 0, kRho * kSurfaceArea * v_wind[2] * longitudinal_velocity_norm};
        EXPECT_TRUE(CompareMatrices(plant->EvalTimeDerivatives(plant_context).get_generalized_velocity().CopyToVector(),
                                    vdot_expected, 1e-14));
    }
}
...
GTEST_TEST(WingTest, DerivativesAtZeroVelocity) {
    ...
    const RigidBody<double>& body = plant->GetBodyByName("box");
    Wing<double>::AddToBuilder(&builder, plant, body.index(), kSurfaceArea, math::RigidTransform<double>::Identity());
    ...
    const MultibodyPlant<AutoDiffXd>* plant_ad =
            dynamic_cast<const MultibodyPlant<AutoDiffXd>*>(&diagram_ad->GetSubsystemByName("plant"));
    EXPECT_TRUE(plant_ad != nullptr);
    systems::Context<AutoDiffXd>& plant_context_ad = plant_ad->GetMyMutableContextFromRoot(context_ad.get());
    const RigidBody<AutoDiffXd>& body_ad = plant_ad->GetBodyByName("box");
    const SpatialVelocity<AutoDiffXd> V_WB(math::InitializeAutoDiff(Vector6d::Zero()));
    plant_ad->SetFreeBodySpatialVelocity(&plant_context_ad, body_ad, V_WB);

    const Vector6<AutoDiffXd> vdot_gravity_only = plant_ad->CalcGravityGeneralizedForces(plant_context_ad);
    const Vector6<AutoDiffXd> vdot =
            plant_ad->EvalTimeDerivatives(plant_context_ad).get_generalized_velocity().CopyToVector();
