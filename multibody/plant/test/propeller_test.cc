#include "multibody/plant/propeller.h"
...
GTEST_TEST(PropellerTest, SinglePropTest) {
    ...
    const auto M_B =
            SpatialInertia<double>(mass, Vector3<double>::Zero(), UnitInertia<double>::TriaxiallySymmetric(1.0));
    ...
    auto prop = builder.AddSystem<Propeller<double>>(
            body.index(),
            math::RigidTransform<double>(math::RotationMatrix<double>::MakeXRotation(M_PI / 2.0),
                                         Eigen::Vector3d(0, 1.0, 0.0)),
            thrust_ratio, moment_ratio);
    EXPECT_EQ(prop->num_propellers(), 1);

    builder.Connect(prop->get_spatial_forces_output_port(), plant->get_applied_spatial_force_input_port());
    builder.Connect(plant->get_body_poses_output_port(), prop->get_body_poses_input_port());
    ...
    for (const double command : {0.0, -1.4, 2.3}) {
        ...
        const Eigen::VectorXd acceleration =
                diagram->EvalTimeDerivatives(*context).get_generalized_velocity().CopyToVector();
        EXPECT_TRUE(CompareMatrices(acceleration.head<3>(), Eigen::Vector3d(0, -command * moment_ratio, 0), 1e-14));
        EXPECT_TRUE(CompareMatrices(
                acceleration.tail<3>(),
                Eigen::Vector3d(0, -command * thrust_ratio, 0) + plant->gravity_field().gravity_vector(), 1e-14));
    }

    // Repeat the check, but with the body rotated and translated.
    // Propeller is now at (1,2,1), pointing along the positive x-axis.
    plant->SetFreeBodyPose(&plant->GetMyMutableContextFromRoot(context.get()), body,
                           math::RigidTransform<double>(math::RotationMatrix<double>::MakeZRotation(M_PI / 2.0),
                                                        Eigen::Vector3d::Ones()));
    ...
    for (const double command : {0.0, -1.4, 2.3}) {
        ...
        const Eigen::VectorXd acceleration =
                diagram->EvalTimeDerivatives(*context).get_generalized_velocity().CopyToVector();
        EXPECT_TRUE(CompareMatrices(acceleration.head<3>(), Eigen::Vector3d(command * moment_ratio, 0, 0), 1e-14));
        EXPECT_TRUE(CompareMatrices(
                acceleration.tail<3>(),
                Eigen::Vector3d(command * thrust_ratio, 0, 0) + plant->gravity_field().gravity_vector(), 1e-14));
    }
...
GTEST_TEST(PropellerTest, BiRotorTest) {
    ...
    auto props = builder.AddSystem<Propeller<double>>(std::vector<PropellerInfo>({prop1, prop2}));
    EXPECT_EQ(props->num_propellers(), 2);

    builder.Connect(props->get_spatial_forces_output_port(), plant->get_applied_spatial_force_input_port());
    builder.Connect(plant->get_body_poses_output_port(), props->get_body_poses_input_port());
    ...
    EXPECT_TRUE(CompareMatrices(
            acceleration.tail<3>(),
            Eigen::Vector3d(0, 0, (command[0] + command[1]) * thrust_ratio) + plant->gravity_field().gravity_vector(),
            1e-14));
    ...
    diagram->CalcImplicitTimeDerivativesResidual(*context, *derivatives, &residual);
