...
public:
    ...
    void MakeSpringMassSystem(double mass, double period, double damping_ratio) {
        ...
        const double stiffness = mass * undamped_angular_frequency * undamped_angular_frequency;
        ...
        const auto M_B = SpatialInertia<double>::MakeFromCentralInertia(
                mass, Vector3<double>::Zero(), mass * UnitInertia<double>::TriaxiallySymmetric(1.0));
        const RigidBody<double>& body = plant_.AddRigidBody("mass", M_B);
        slider_ = &plant_.AddJoint<PrismaticJoint>("Slider", plant_.world_body(), std::nullopt, body, std::nullopt,
                                                   Vector3<double>::UnitX());
        plant_.AddForceElement<LinearSpringDamper>(plant_.world_body(), Vector3<double>::Zero(), body,
    ...
    double CalcAnalyticSolution(double period, double damping_ratio, double x0, double v0, double time) {
        ...
        const complex<double> w = w0 * sqrt(complex<double>{1.0 - damping_ratio * damping_ratio});
        ...
        const complex<double> x = constants(0) * exp(lambda1 * time) + constants(1) * exp(lambda2 * time);
...
TEST_F(SpringMassSystemTest, UnDampedCase) {
    ...
    const double x_analytic = CalcAnalyticSolution(period, damping_ratio, amplitude, 0.0, simulation_time);

    EXPECT_NEAR(slider_->get_translation(context), x_analytic, integration_accuracy);
}
