...
public:
    ...
    static void PackContactSolverResults(const SapDriver<double>& driver,
                                         const Context<double>& context,
...
public:
    const SapDriver<double>& sap_driver() const { return CompliantContactManagerTester::sap_driver(*contact_manager_); }

    // The functions below provide access to private SapDriver functions for unit
    // testing.

    const ContactProblemCache<double>& EvalContactProblemCache(const Context<double>& context) const {
    ...
    std::vector<MatrixXd> CalcLinearDynamicsMatrix(const Context<double>& context) const {
        return SapDriverTest::CalcLinearDynamicsMatrix(sap_driver(), context);
    }

    void PackContactSolverResults(const Context<double>& context,
                                  const contact_solvers::internal::SapContactProblem<double>& problem,
                                  int num_contacts,
                                  const contact_solvers::internal::SapSolverResults<double>& sap_results,
                                  contact_solvers::internal::ContactSolverResults<double>* contact_results) const {
        SapDriverTest::PackContactSolverResults(sap_driver(), context, problem, num_contacts, sap_results,
                                                contact_results);
    }
...
TEST_F(SpheresStackTest, EvalContactProblemCache) {
    ...
    const ContactProblemCache<double>& problem_cache = EvalContactProblemCache(*plant_context_);
    const SapContactProblem<double>& problem = *problem_cache.sap_problem;
    const std::vector<drake::math::RotationMatrix<double>>& R_WC = problem_cache.R_WC;
    ...
    for (int i = 0; i < contact_pairs.size(); ++i) {
        ...
        const auto* constraint = dynamic_cast<const SapFrictionConeConstraint<double>*>(&problem.get_constraint(i));
        ...
        EXPECT_EQ(constraint->first_clique_jacobian().MakeDenseMatrix(), discrete_pair.jacobian[0].J.MakeDenseMatrix());
        ...
        EXPECT_EQ(constraint->parameters().dissipation_time_scale, discrete_pair.dissipation_time_scale);
        EXPECT_EQ(constraint->parameters().beta, plant_->get_sap_near_rigid_threshold());
...
TEST_F(SpheresStackTest, CalcFreeMotionVelocitiesWithExternalForces) {
    ...
    plant_->get_applied_generalized_force_input_port().FixValue(plant_context_, tau);
    ...
    EXPECT_TRUE(CompareMatrices(v_star, v_expected, kEps, MatrixCompareType::relative));
}
...
TEST_F(SpheresStackTest, PackContactSolverResults) {
    ...
    const SapContactProblem<double>& sap_problem = *EvalContactProblemCache(*plant_context_).sap_problem;
    ContactSolverResults<double> contact_results;
    PackContactSolverResults(*plant_context_, sap_problem, num_contacts, sap_results, &contact_results);
    ...
    EXPECT_TRUE(CompareMatrices(gamma, sap_results.gamma, kEps, MatrixCompareType::relative));
    VectorXd vc(3 * num_contacts);
    MergeNormalAndTangent(contact_results.vn, contact_results.vt, &vc);
    EXPECT_TRUE(CompareMatrices(vc, sap_results.vc, kEps, MatrixCompareType::relative));
    ...
    const VectorXd tau_expected = J_AcBc_C.transpose() * sap_results.gamma / plant_->time_step();
    EXPECT_TRUE(CompareMatrices(contact_results.tau_contact, tau_expected, 2.0 * kEps, MatrixCompareType::relative));
}
...
TEST_F(SpheresStackTest, SapFailureException) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(contact_manager_->CalcContactSolverResults(*plant_context_, &contact_results),
...
TEST_F(SpheresStackTest, EvalContactSolverResults) {
    ...
    const ContactSolverResults<double> contact_results_without_cache =
            contact_manager_->EvalContactSolverResults(*plant_context_);
    EXPECT_EQ(contact_results_with_cache.v_next, contact_results_without_cache.v_next);
}
...
GTEST_TEST(SapDriverTest, ConstraintActiveStatus) {
    ...
    const RigidBody<double>& body_A = plant.AddRigidBody("body_A", SpatialInertia<double>::MakeUnitary());
    const RigidBody<double>& body_B = plant.AddRigidBody("body_B", SpatialInertia<double>::MakeUnitary());
    const RevoluteJoint<double>& world_A =
            plant.AddJoint<RevoluteJoint>("world_A", plant.world_body(), RigidTransformd::Identity(), body_A,
                                          RigidTransformd::Identity(), Vector3d::UnitZ());
    const RevoluteJoint<double>& A_B = plant.AddJoint<RevoluteJoint>("A_B", body_A, RigidTransformd::Identity(), body_B,
                                                                     RigidTransformd::Identity(), Vector3d::UnitZ());

    // Add one coupler constraint, one distance constraint, one ball
    // constraint and one weld constraint.
    MultibodyConstraintId coupler_constraint_id = plant.AddCouplerConstraint(world_A, A_B, 1.2);
    MultibodyConstraintId distance_constraint_id =
            plant.AddDistanceConstraint(body_A, Vector3d::Zero(), body_B, Vector3d(1.0, 2.0, 3.0), 1.0);
    MultibodyConstraintId ball_constraint_id =
            plant.AddBallConstraint(body_A, Vector3d::Zero(), body_B, Vector3d::Zero());
    MultibodyConstraintId weld_constraint_id =
            plant.AddWeldConstraint(body_A, RigidTransformd(), body_B, RigidTransformd());

    // Finalize and set the manager/driver.
    plant.Finalize();

    auto owned_contact_manager = std::make_unique<CompliantContactManager<double>>();
    const CompliantContactManager<double>* contact_manager = owned_contact_manager.get();
    plant.SetDiscreteUpdateManager(std::move(owned_contact_manager));
    const SapDriver<double>& sap_driver = CompliantContactManagerTester::sap_driver(*contact_manager);

    std::unique_ptr<Context<double>> context = plant.CreateDefaultContext();

    // All constraints active.
    {
        const ContactProblemCache<double>& problem_cache = SapDriverTest::EvalContactProblemCache(sap_driver, *context);
        const SapContactProblem<double>& problem = *problem_cache.sap_problem;
        ...
        EXPECT_EQ(problem.num_constraint_equations(), 1 /* coupler constraint */ + 1 /* distance constraint */ +
    ...
    {
        plant.SetConstraintActiveStatus(context.get(), distance_constraint_id, false);
        ...
        EXPECT_EQ(problem.num_constraint_equations(), 1 /* coupler constraint */ + 6 /* weld constraint */);
        EXPECT_EQ(problem.get_constraint(0).num_constraint_equations(), 1);
    ...
    // has 0 constraints.
    {
        plant.SetConstraintActiveStatus(context.get(), coupler_constraint_id, false);
    ...
    {
        plant.SetConstraintActiveStatus(context.get(), distance_constraint_id, true);
...
TEST_F(MultibodyPlantRemodeling, AddPdControllerConstraints) {
    ...
    const systems::InputPort<double>& x_desired_input = plant_->get_desired_state_input_port(default_model_instance());
    ...
    const SapDriver<double>& sap_driver =
            CompliantContactManagerTester::sap_driver(MultibodyPlantTester::manager(*plant_));
    ...
    for (int i = 0; i < problem_cache.num_pd_controller_constraints; ++i) {
        ...
        const SapPdControllerConstraint<double>& pd_constraint =
                dynamic_cast<const SapPdControllerConstraint<double>&>(problem.get_constraint(constraint_id));
        const auto configuration = pd_constraint.configuration();
        const int actuator_input_start = dof_to_actuator_input_start[configuration.clique_dof];
        EXPECT_EQ(configuration.qd, x_desired(actuator_input_start));
        EXPECT_EQ(configuration.vd, x_desired(actuator_input_start + plant_->num_actuated_dofs()));
