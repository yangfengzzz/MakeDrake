...
public:
    ...
    template <typename T>
    static internal::DiscreteUpdateManager<T>& discrete_update_manager(const MultibodyPlant<T>& plant) {
...
public:
    ...
    int num_calls_to_calc_contact_solver_results() const { return num_calls_to_calc_contact_solver_results_; }

    const VectorX<T>& EvalCacheEntry(const systems::Context<T>& context) const {
        const auto& data = this->plant().get_cache_entry(cache_index_).template Eval<VectorX<T>>(context);
        return data;
    }
...
private:
    ...
    std::unique_ptr<DiscreteUpdateManager<double>> CloneToDouble() const final { return CloneToScalar<double>(); }
    ...
    void DoDeclareCacheEntries() final {
        cache_index_ = this->DeclareCacheEntry("Twice the additional_state value",
                                               systems::ValueProducer(this, VectorX<T>(kNumAdditionalDofs),
    ...
    void CalcTwiceState(const Context<T>& context, VectorX<T>* data) const {
        *data = 2.0 * context.get_discrete_state(additional_state_index_).get_value();
    }

    /* Increments the number of times CalcContactSolverResults() is called for
     testing. */
    void DoCalcContactSolverResults(const Context<T>&, ContactSolverResults<T>* results) const final {
    ...
    void DoCalcAccelerationKinematicsCache(const Context<T>& context,
    ...
    /* Increments the discrete rigid dofs by 1 and additional discrete state by 2
     if there is any. */
    void DoCalcDiscreteValues(const Context<T>& context, DiscreteValues<T>* updates) const final {
        auto multibody_data = updates->get_mutable_value(this->multibody_state_index());
        multibody_data += VectorX<T>::Ones(multibody_data.size());
        if (additional_state_index_.is_valid()) {
            auto additional_data = updates->get_mutable_value(additional_state_index_);
            additional_data += 2.0 * VectorX<T>::Ones(additional_data.size());
        }
    }

    // Not used in these tests.
    void DoCalcDiscreteUpdateMultibodyForces(const systems::Context<T>&, MultibodyForces<T>*) const final {
...
protected:
    ...
    static VectorXd dummy_discrete_state() { return VectorXd::Ones(kNumAdditionalDofs) * kDummyStateValue; }
...
TEST_F(DiscreteUpdateManagerTest, CalcDiscreteState) {
    ...
    const VectorXd final_additional_state = dummy_model_->get_vector_output_port().Eval(simulator.get_context());
    EXPECT_EQ(final_additional_state.size(), kNumAdditionalDofs);
    EXPECT_TRUE(CompareMatrices(final_additional_state,
                                dummy_discrete_state() + 2.0 * VectorXd::Ones(kNumAdditionalDofs) * time_steps,
                                std::numeric_limits<double>::epsilon()));
    /* Verifies that the cache value is twice the addition state value. */
    const VectorXd& cache_value = dummy_manager_->EvalCacheEntry(simulator.get_context());
    EXPECT_TRUE(CompareMatrices(2.0 * final_additional_state, cache_value, std::numeric_limits<double>::epsilon()));
}
...
TEST_F(DiscreteUpdateManagerTest, CalcContactSolverResults) {
    ...
    const auto& port = plant_.get_generalized_contact_forces_output_port(default_model_instance());
    port.Eval(*context);
...
TEST_F(DiscreteUpdateManagerTest, CalcAccelerationKinematicsCache) {
    auto context = plant_.CreateDefaultContext();
    const auto generalized_acceleration = plant_.get_generalized_acceleration_output_port().Eval(*context);
    EXPECT_TRUE(CompareMatrices(generalized_acceleration, VectorXd::Ones(kNumRigidDofs) * kDummyVdot));
}
...
TEST_F(DiscreteUpdateManagerTest, ScalarConversion) {
    ...
    auto simulator = systems::Simulator<AutoDiffXd>(*autodiff_plant, std::move(context));
    ...
    const VectorXd final_additional_state = math::ExtractValue(final_additional_state_autodiff);
    EXPECT_EQ(final_additional_state.size(), kNumAdditionalDofs);
    EXPECT_TRUE(CompareMatrices(final_additional_state,
                                dummy_discrete_state() + 2.0 * VectorXd::Ones(kNumAdditionalDofs) * time_steps,
                                std::numeric_limits<double>::epsilon()));
...
class AlgebraicLoopDetection : public ::testing::TestWithParam<std::tuple<bool, std::string_view>> {
public:
    ...
    void MakeDiagram(bool with_algebraic_loop, std::string_view contact_approximation) {
        systems::DiagramBuilder<double> builder;

        MultibodyPlantConfig plant_config;
        plant_config.time_step = 1.0e-3;
        plant_config.discrete_contact_approximation = contact_approximation;
        std::tie(plant_, scene_graph_) = multibody::AddMultibodyPlant(plant_config, &builder);
        plant_->Finalize();
        ...
        if (with_algebraic_loop) {
            ...
            feedback = builder.AddSystem<systems::PassThrough>(plant_->num_velocities());
        } else {
            ...
            // point.
            feedback = builder.AddSystem<systems::ZeroOrderHold>(2.0e-4, plant_->num_velocities());
        }
        builder.Connect(plant_->get_generalized_contact_forces_output_port(default_model_instance()),
                        feedback->get_input_port(0));
        builder.Connect(feedback->get_output_port(0), plant_->get_applied_generalized_force_input_port());
    ...
    void VerifyLoopIsDetected() const {
        DRAKE_EXPECT_THROWS_MESSAGE(
                plant_->get_generalized_contact_forces_output_port(default_model_instance()).Eval(*plant_context_),
...
INSTANTIATE_TEST_SUITE_P(AlgebraicLoopTests,
                         AlgebraicLoopDetection,
                         ::testing::Combine(::testing::Bool(), ::testing::Values("tamsi", "sap")));
...
<sdf version="1.7">
  <model name="object">
    ...
    const std::vector<ModelInstanceIndex> models = Parser(&plant).AddModelsFromString(sdf_model, "sdf");
    ...
    const systems::InputPort<double>& u_input = plant.get_actuation_input_port(only_model);
    ...
    plant.get_generalized_contact_forces_output_port(only_model).Eval(plant_context);
...
TEST_F(MultibodyPlantRemodeling, RemoveJointActuator) {
    ...
    DiscreteUpdateManager<double>& manager = MultibodyPlantTester::discrete_update_manager(*plant_);
    ...
    manager.CalcNonContactForces(*plant_context_, false /* no joint limit penalty forces */,
                                 false /* no pd controlled actuator forces */, &forces);

    const Vector3d expected_actuation_wo_pd(1.0, 0.0, 3.0);
    EXPECT_TRUE(CompareMatrices(forces.generalized_forces(), expected_actuation_wo_pd));
}
