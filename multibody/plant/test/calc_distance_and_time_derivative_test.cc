...
namespace multibody {
namespace {
void CheckDistanceAndTimeDerivative(const multibody::MultibodyPlant<double>& plant,
    ...
    EXPECT_NEAR(result.distance_time_derivative, distance_time_derivative_expected, 1e-12);
}
...
TEST_F(BoxSphereTest, CalcDistanceAndTimeDerivative) {
    ...
    CheckDistanceAndTimeDerivative(*plant_double_, sphere_frame_index_, box_frame_index_,
                                   {box_geometry_id_, sphere_geometry_id_}, p_WB, quat_WB, p_WS, quat_WS, v_WB,
                                   omega_WB, v_WS, omega_WS, 14, 0, plant_context_double_);
    ...
    CheckDistanceAndTimeDerivative(*plant_double_, sphere_frame_index_, box_frame_index_,
                                   {box_geometry_id_, sphere_geometry_id_}, p_WB, quat_WB, p_WS, quat_WS, v_WB,
                                   omega_WB, v_WS, omega_WS, 14, v_WS(2), plant_context_double_);
    ...
    CheckDistanceAndTimeDerivative(*plant_double_, sphere_frame_index_, box_frame_index_,
                                   {box_geometry_id_, sphere_geometry_id_}, p_WB, quat_WB, p_WS, quat_WS, v_WB,
                                   omega_WB, v_WS, omega_WS, 14, v_WS(2) - v_WB(2), plant_context_double_);
    ...
    CheckDistanceAndTimeDerivative(*plant_double_, sphere_frame_index_, box_frame_index_,
                                   {box_geometry_id_, sphere_geometry_id_}, p_WB, quat_WB, p_WS, quat_WS, v_WB,
                                   omega_WB, v_WS, omega_WS, 20 * std::cos(theta) - 6, -20 * std::sin(theta) * thetadot,
