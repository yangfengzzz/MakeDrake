...
public:
    void SetUp() override {
        ...
        const FrameId f_id = scene_graph_.RegisterFrame(s_id, GeometryFrame("frame"));
        unique_ptr<GeometryInstance> geometry_A =
                MakeGeometryInstance("A", kMuA, kKA, kTauA, kHuntCrossleyDissipationA, kHydroModulusA);
        unique_ptr<GeometryInstance> geometry_B =
                MakeGeometryInstance("B", kMuB, std::nullopt, kTauB, std::nullopt, kHydroModulusB,
                                     geometry::internal::HydroelasticType::kRigid);
        unique_ptr<GeometryInstance> geometry_C =
                MakeGeometryInstance("C", kMuC, kKC, std::nullopt, std::nullopt, kHydroModulusC,
                                     geometry::internal::HydroelasticType::kSoft);
        unique_ptr<GeometryInstance> geometry_D =
                MakeGeometryInstance("D", kMuD, std::nullopt, kNegativeTauD, std::nullopt);
        ...
        scene_graph_ad_ = dynamic_pointer_cast<SceneGraph<AutoDiffXd>>(scene_graph_.ToAutoDiffXd());
    }
    ...
    const SceneGraphInspector<double>& inspector_d() const { return scene_graph_.model_inspector(); }

    const SceneGraphInspector<AutoDiffXd>& inspector_ad() const { return scene_graph_ad_->model_inspector(); }

    unique_ptr<GeometryInstance> MakeGeometryInstance(
            const std::string& name,
            const CoulombFriction<double>& mu,
            const std::optional<double>& k,
            const std::optional<double>& tau,
            const std::optional<double>& hunt_crossley_dissipation,
            const std::optional<double>& hydro_modulus = std::nullopt,
            const std::optional<geometry::internal::HydroelasticType>& compliance_type = std::nullopt) {
        auto result = make_unique<GeometryInstance>(RigidTransformd(), Sphere(1.0), name);
        geometry::ProximityProperties props;
        geometry::AddContactMaterial(hunt_crossley_dissipation, k, mu, &props);
        if (tau.has_value()) {
            props.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kRelaxationTime, *tau);
        }

        // Hydroelastic contact requires the resolution hint, even if not used for
        // these tests.
        constexpr double kRezHint = 1.0;
        props.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kRezHint, kRezHint);
        if (hydro_modulus.has_value()) {
            props.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kElastic, *hydro_modulus);
        }
        if (compliance_type.has_value()) {
            props.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kComplianceType, *compliance_type);
        }
...
TEST_F(ContactPropertiesTest, GetHydroelasticModulus) {
    EXPECT_EQ(GetHydroelasticModulus(g_A_, kHydroModulusDefault, inspector_d()), kHydroModulusA);
    EXPECT_EQ(GetHydroelasticModulus(g_B_, kHydroModulusDefault, inspector_d()), kInfinity);
    EXPECT_EQ(GetHydroelasticModulus(g_C_, kHydroModulusDefault, inspector_d()), kHydroModulusC);
    EXPECT_EQ(GetHydroelasticModulus(g_D_, kHydroModulusDefault, inspector_d()), kHydroModulusDefault);
...
TEST_F(ContactPropertiesTest, GetHuntCrossleyDissipation) {
    EXPECT_EQ(GetHuntCrossleyDissipation(g_A_, kHuntCrossleyDissipationDefault, inspector_d()),
              kHuntCrossleyDissipationA);
    EXPECT_EQ(GetHuntCrossleyDissipation(g_B_, kHuntCrossleyDissipationDefault, inspector_d()),
              kHuntCrossleyDissipationDefault);
    EXPECT_EQ(GetHuntCrossleyDissipation(g_C_, kHuntCrossleyDissipationDefault, inspector_d()),
              kHuntCrossleyDissipationDefault);

    EXPECT_EQ(GetHuntCrossleyDissipation(g_A_, kHuntCrossleyDissipationDefault, inspector_d()),
              GetHuntCrossleyDissipation(g_A_, kHuntCrossleyDissipationDefault, inspector_ad()));
}

TEST_F(ContactPropertiesTest, GetCombinedHuntCrossleyDissipation) {
    EXPECT_EQ(GetCombinedHuntCrossleyDissipation(g_A_, g_B_, kHydroModulusA, kInfinity, kHuntCrossleyDissipationDefault,
                                                 inspector_d()),
              kHuntCrossleyDissipationA);
    EXPECT_EQ(GetCombinedHuntCrossleyDissipation(g_B_, g_A_, kInfinity, kHydroModulusA, kHuntCrossleyDissipationDefault,
                                                 inspector_d()),
              kHuntCrossleyDissipationA);
    const double dAC = (kHydroModulusC / (kHydroModulusA + kHydroModulusC)) * kHuntCrossleyDissipationA +
                       (kHydroModulusA / (kHydroModulusA + kHydroModulusC)) * kHuntCrossleyDissipationDefault;
    EXPECT_EQ(GetCombinedHuntCrossleyDissipation(g_A_, g_C_, kHydroModulusA, kHydroModulusDefault,
                                                 kHuntCrossleyDissipationDefault, inspector_d()),
              dAC);

    EXPECT_EQ(GetCombinedHuntCrossleyDissipation(g_A_, g_B_, kHydroModulusA, kInfinity, kHuntCrossleyDissipationDefault,
                                                 inspector_d()),
              GetCombinedHuntCrossleyDissipation<AutoDiffXd>(g_A_, g_B_, kHydroModulusA, kInfinity,
                                                             kHuntCrossleyDissipationDefault, inspector_ad()));
}

TEST_F(ContactPropertiesTest, GetDissipationTimeConstant) {
    EXPECT_EQ(GetDissipationTimeConstant(g_A_, kTauDefault, inspector_d(), "A"), kTauA);
    EXPECT_EQ(GetDissipationTimeConstant(g_C_, kTauDefault, inspector_d(), "C"), kTauDefault);
    DRAKE_EXPECT_THROWS_MESSAGE(GetDissipationTimeConstant(g_D_, kTauDefault, inspector_d(), "D"),
...
TEST_F(ContactPropertiesTest, GetCoulombFriction) {
    ...
    EXPECT_EQ(GetCoulombFriction(g_A_, inspector_d()), GetCoulombFriction(g_A_, inspector_ad()));
}
...
TEST_F(ContactPropertiesTest, GetCombinedPointContactStiffness) {
    ...
    EXPECT_EQ(GetCombinedPointContactStiffness(g_A_, g_C_, kKDefault, inspector_d()), kKA * kKC / (kKA + kKC));
    EXPECT_EQ(GetCombinedPointContactStiffness(g_A_, g_B_, kKDefault, inspector_d()),
              kKA * kKDefault / (kKA + kKDefault));

    // Test the signature that performs the underlying computation.
    EXPECT_EQ(GetCombinedPointContactStiffness(kKA, kKC), kKA * kKC / (kKA + kKC));

    // Test the case where one geometry is infinitely stiff.
    EXPECT_EQ(GetCombinedPointContactStiffness(kKDefault, std::numeric_limits<double>::infinity()), kKDefault);
    EXPECT_EQ(GetCombinedPointContactStiffness(std::numeric_limits<double>::infinity(), kKDefault), kKDefault);

    EXPECT_EQ(GetCombinedPointContactStiffness(g_A_, g_C_, kKDefault, inspector_d()),
...
TEST_F(ContactPropertiesTest, GetCombinedDissipationTimeConstant) {
    EXPECT_EQ(GetCombinedDissipationTimeConstant(g_A_, g_B_, kTauDefault, "A", "B", inspector_d()), kTauA + kTauB);
    EXPECT_EQ(GetCombinedDissipationTimeConstant(g_A_, g_C_, kTauDefault, "A", "C", inspector_d()),
              kTauA + kTauDefault);
    DRAKE_EXPECT_THROWS_MESSAGE(GetCombinedDissipationTimeConstant(g_A_, g_D_, kTauDefault, "A", "D", inspector_d()),
                                ".*relaxation_time = -0.2.*D.*");

    EXPECT_EQ(GetCombinedDissipationTimeConstant(g_A_, g_B_, kTauDefault, "A", "B", inspector_d()),
              GetCombinedDissipationTimeConstant(g_A_, g_B_, kTauDefault, "A", "B", inspector_ad()));
}
