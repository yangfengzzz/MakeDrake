...
public:
    void SetUp() override {
        ...
        plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", iiwa1_));
        plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", iiwa2_));
    }
...
TEST_F(MultibodyPlantGravityForceTest, UniformGravityFieldElementApisToDisableGravity) {
...
TEST_F(MultibodyPlantGravityForceTest, InvalidModelInstancesThrow) {
    UniformGravityFieldElement<double>& gravity = plant_->mutable_gravity_field();
    const ModelInstanceIndex invalid_model_instance(plant_->num_model_instances());
    DRAKE_EXPECT_THROWS_MESSAGE(gravity.is_enabled(invalid_model_instance), "Model instance index is invalid.");
    DRAKE_EXPECT_THROWS_MESSAGE(gravity.set_enabled(invalid_model_instance, true), "Model instance index is invalid.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->is_gravity_enabled(invalid_model_instance), "Model instance index is invalid.");
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->set_gravity_enabled(invalid_model_instance, true),
                                "Model instance index is invalid.");
}

TEST_F(MultibodyPlantGravityForceTest, UniformGravityFieldElementSetEnabledThrowsIfCalledPostFinalize) {
    UniformGravityFieldElement<double>& gravity = plant_->mutable_gravity_field();
    plant_->Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(gravity.set_enabled(iiwa1_, true), "Gravity can only be enabled pre-finalize.");
}

TEST_F(MultibodyPlantGravityForceTest, MultibodyPlantSetEnabledThrowsIfCalledPostFinalize) {
    ...
    plant_->Finalize();
    DRAKE_EXPECT_THROWS_MESSAGE(plant_->set_gravity_enabled(iiwa1_, true),
