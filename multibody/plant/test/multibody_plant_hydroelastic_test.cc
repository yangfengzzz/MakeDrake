...
#include "multibody/plant/contact_results_to_lcm.h"
...
public:
    ...
    static const std::vector<SpatialForce<double>>& EvalHydroelasticContactForcesContinuous(
            const MultibodyPlant<double>& plant, const systems::Context<double>& context) {
        return plant.EvalHydroelasticContactForcesContinuous(context).F_BBo_W_array;
    }

    static BodyIndex FindBodyByGeometryId(const MultibodyPlant<double>& plant, GeometryId id) {
        return plant.FindBodyByGeometryId(id);
    }

    static const std::vector<SpatialForce<double>>& EvalSpatialContactForcesContinuous(
            const MultibodyPlant<double>& plant, const systems::Context<double>& context) {
...
protected:
    ...
    void SetUpModel(double mbp_dt, BoxType box_type, std::optional<double> compliant_box_hydroelastic_modulus) {
        systems::DiagramBuilder<double> builder;
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, mbp_dt);

        AddGroundBox(kFrictionCoefficient, plant_, box_type, compliant_box_hydroelastic_modulus);
        body_ = &AddObject(plant_, kSphereRadius, kElasticModulus, kDissipation, kFrictionCoefficient);

        // The default contact model today is hydroelastic with fallback.
        EXPECT_EQ(plant_->get_contact_model(), ContactModel::kHydroelasticWithFallback);
        ...
        scene_graph_context_ = &diagram_->GetMutableSubsystemContext(*scene_graph_, diagram_context_.get());
    }

    // @param compliant_hydroelastic_modulus is required for the compliant box
    //                                       but not the rigid box.
    void AddGroundBox(double friction_coefficient,
        ...
        plant->RegisterVisualGeometry(plant->world_body(), X_WG, ground, "GroundVisualGeometry", green);
        ...
        if (box_type == BoxType::kRigid) {
            ...
            geometry::AddCompliantHydroelasticProperties(kSize, compliant_hydroelastic_modulus.value(), &props);
        }
        geometry::AddContactMaterial({}, {}, CoulombFriction<double>(friction_coefficient, friction_coefficient),
                                     &props);
        plant->RegisterCollisionGeometry(plant->world_body(), X_WG, ground, "GroundCollisionGeometry",
                                         std::move(props));
    }
    ...
    const RigidBody<double>& AddObject(MultibodyPlant<double>* plant,
        ...
        const SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(kMass, radius);
        ...
        plant->RegisterVisualGeometry(body, X_BG, shape, "BodyVisualGeometry", lightBlue);

        geometry::ProximityProperties props;
        // This should produce a level-2 refinement (two steps beyond octahedron).
        geometry::AddCompliantHydroelasticProperties(radius / 2, hydroelastic_modulus, &props);
        geometry::AddContactMaterial(dissipation, {},
                                     CoulombFriction<double>(friction_coefficient, friction_coefficient), &props);
        plant->RegisterCollisionGeometry(body, X_BG, shape, "BodyCollisionGeometry", std::move(props));
        return body;
    }
    ...
    double CalcAnalyticalHydroelasticsForce(double d) {
        ...
        const double normal_force = M_PI / 3.0 * kElasticModulus * d * d * (3 - 2 * d / kSphereRadius);
        return normal_force;
    }
    ...
    void RunDiscreteTamsiSolver(SpatialForce<double>* F_BBo_W, Vector3<double>* p_WB_W) {
        ...
        plant_->SetFreeBodyPose(&plant_context, *body_, RigidTransformd(Vector3d(0.0, 0.0, kSphereRadius)));
        ...
        const RigidTransformd& X_WB = plant_->GetFreeBodyPose(plant_context, *body_);
        *p_WB_W = X_WB.translation();
        ...
        const ContactResults<double>& results =
                plant_->get_contact_results_output_port().Eval<ContactResults<double>>(plant_context);

        ASSERT_EQ(results.num_point_pair_contacts(), 0);

        for (int i = 0; i < results.num_hydroelastic_contacts(); ++i) {
            const HydroelasticContactInfo<double>& info = results.hydroelastic_contact_info(i);
            ...
            const BodyIndex bodyA_index = MultibodyPlantTester::FindBodyByGeometryId(*plant_, geometryM_id);
            const BodyIndex bodyB_index = MultibodyPlantTester::FindBodyByGeometryId(*plant_, geometryN_id);
...
TEST_F(HydroelasticModelTests, ContactForce) {
    ...
    auto calc_force = [this](double penetration) -> double {
        ...
        const auto& F_BBo_W_array =
                MultibodyPlantTester::EvalHydroelasticContactForcesContinuous(*plant_, *plant_context_);
    ...
    auto calc_observed_percentile_error = [R = kSphereRadius](double penetration) {
        return 27.5 - 10 / 0.6 * penetration / R;
    };

    for (const double extent : {0.2, 0.4, 0.6, 0.8}) {
        const double penetration = extent * kSphereRadius;
        const double analytical_force = CalcAnalyticalHydroelasticsForce(penetration);
        const double numerical_force = calc_force(penetration);
        const double percentile_error = (analytical_force - numerical_force) / analytical_force;
        const double observed_percentile_error = calc_observed_percentile_error(penetration);
...
TEST_F(HydroelasticModelTests, ContactDynamics) {
    ...
    SetPose(penetration);
    const auto& F_BBo_W_array = MultibodyPlantTester::EvalHydroelasticContactForcesContinuous(*plant_, *plant_context_);
    const SpatialForce<double>& F_BBo_W = F_BBo_W_array[body_->mobod_index()];
    ...
    const VectorX<double> vdot = derivatives->get_generalized_velocity().CopyToVector();
    ...
    const Vector3<double> a_WBo_expected = fhydro_BBo_W / kMass + plant_->gravity_field().gravity_vector();
    EXPECT_TRUE(CompareMatrices(a_WBo_expected, a_WBo, 40 * std::numeric_limits<double>::epsilon()));
}
...
TEST_F(HydroelasticModelTests, Parameters) {
    ...
    const std::vector<geometry::GeometryId>& g_ids = plant_->GetCollisionGeometriesForBody(*body_);
    ASSERT_EQ(g_ids.size(), 1);
    GeometryId gid = g_ids[0];
    const ProximityProperties* old_props = scene_graph_->model_inspector().GetProximityProperties(gid);

    ASSERT_TRUE(old_props != nullptr);

    const ContactResults<double> old_contact_results =
            plant_->get_contact_results_output_port().template Eval<ContactResults<double>>(*plant_context_);

    // Change in friction should affect only the tangential component of the
    // traction at each quadrature point.
    const CoulombFriction<double> mu_box(kFrictionCoefficient, kFrictionCoefficient);
    const CoulombFriction<double> mu_sphere(5.0, 5.0);
    const CoulombFriction<double> mu_combined = CalcContactFrictionFromSurfaceProperties(mu_box, mu_sphere);
    ProximityProperties new_props(*old_props);
    new_props.UpdateProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, mu_sphere);
    scene_graph_->AssignRole(scene_graph_context_, plant_->get_source_id().value(), gid, new_props,
                             geometry::RoleAssign::kReplace);
    const ContactResults<double>& new_contact_results =
            plant_->get_contact_results_output_port().template Eval<ContactResults<double>>(*plant_context_);

    ASSERT_EQ(old_contact_results.num_hydroelastic_contacts(), new_contact_results.num_hydroelastic_contacts());
    for (int i = 0; i < new_contact_results.num_hydroelastic_contacts(); ++i) {
        const HydroelasticContactInfo<double>& old_contact_info = old_contact_results.hydroelastic_contact_info(i);
        const HydroelasticContactInfo<double>& new_contact_info = new_contact_results.hydroelastic_contact_info(i);
        ASSERT_EQ(old_contact_info.quadrature_point_data().size(), new_contact_info.quadrature_point_data().size());
        ...
        for (int j = 0; j < ssize(new_contact_info.quadrature_point_data()); ++j) {
            const HydroelasticQuadraturePointData<double>& old_data = old_contact_info.quadrature_point_data()[j];
            const HydroelasticQuadraturePointData<double>& new_data = new_contact_info.quadrature_point_data()[j];
            ASSERT_TRUE(CompareMatrices(old_data.p_WQ, new_data.p_WQ));
            const Vector3d& n_hat_old = old_contact_info.contact_surface().face_normal(old_data.face_index);
            const Vector3d& n_hat_new = new_contact_info.contact_surface().face_normal(new_data.face_index);
            Vector3d ft_old = old_data.traction_Aq_W - n_hat_old.dot(old_data.traction_Aq_W) * n_hat_old;
            Vector3d ft_new = new_data.traction_Aq_W - n_hat_new.dot(new_data.traction_Aq_W) * n_hat_new;
            ...
            EXPECT_NEAR(ft_old.norm() / ft_new.norm(), kFrictionCoefficient / mu_combined.dynamic_friction(),
...
TEST_F(HydroelasticModelTests, DiscreteTamsiSolverRigidCompliant) {
    ...
    const Vector3<double> f_BBo_W_expected = -plant_->gravity_field().gravity_vector() * kMass;
...
TEST_F(HydroelasticModelTests, DiscreteTamsiSolverCompliantCompliantConsistentWithRigidCompliant) {
    ...
    Vector3<double> compliant_compliant_p_WB_W;
    {
        ...
        RunDiscreteTamsiSolver(&compliant_compliant_F_BBo_W, &compliant_compliant_p_WB_W);
    }

    EXPECT_TRUE(CompareMatrices(compliant_compliant_p_WB_W, rigid_compliant_p_WB_W, 1e-8));
}
...
protected:
    ...
    void Configure(ContactModel model,
                   bool connect_scene_graph = true,
                   double time_step = 0.0,
                   bool are_rigid_spheres_in_contact = true) {
        systems::DiagramBuilder<double> builder;
        if (connect_scene_graph) {
            std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, time_step);
        } else {
            ...
            plant_ = builder.AddSystem(std::make_unique<MultibodyPlant<double>>(time_step));
            scene_graph_ = builder.AddSystem(std::make_unique<SceneGraph<double>>());
            plant_->RegisterAsSourceForSceneGraph(scene_graph_);
        }

        geometry::ProximityProperties props;
        geometry::AddContactMaterial(kDissipation, {},
                                     CoulombFriction<double>(kFrictionCoefficient, kFrictionCoefficient), &props);
        ...
        if (forced_hydroelastic_contact_representation_.has_value()) {
            plant_->set_contact_surface_representation(*forced_hydroelastic_contact_representation_);
        }
    ...
    void AddGround(geometry::ProximityProperties contact_material, MultibodyPlant<double>* plant) {
        const double kSize = 10;
        const RigidTransformd X_WG{Vector3d{0, 0, -kSize / 2}};
        geometry::Box ground = geometry::Box::MakeCube(kSize);
        geometry::AddCompliantHydroelasticProperties(kSize, kElasticModulus, &contact_material);
        plant->RegisterCollisionGeometry(plant->world_body(), X_WG, ground, "GroundCollisionGeometry",
                                         std::move(contact_material));
    }

    const RigidBody<double>& AddSphere(const std::string& name,
                                       double radius,
        ...
        plant->RegisterCollisionGeometry(body, X_BG, shape, "collision", std::move(contact_material));
        return body;
    }
    ...
    std::vector<SpatialForce<double>> SpatialForceFromContactResults(const ContactResults<double>& contacts) {
        std::vector<SpatialForce<double>> F_BBo_W_array(plant_->num_bodies(),
        ...
        for (int i = 0; i < contacts.num_point_pair_contacts(); ++i) {
            ...
            const SpatialForce<double> F_Bc_W{Vector3d::Zero(), contact_info.contact_force()};
            const Vector3d& p_WC = contact_info.contact_point();
            const auto& bodyA = plant_->get_body(contact_info.bodyA_index());
            const Vector3d& p_WAo = bodyA.EvalPoseInWorld(*plant_context_).translation();
            const Vector3d& p_CAo_W = p_WAo - p_WC;
            const auto& bodyB = plant_->get_body(contact_info.bodyB_index());
            const Vector3d& p_WBo = bodyB.EvalPoseInWorld(*plant_context_).translation();
        ...
        for (int i = 0; i < contacts.num_hydroelastic_contacts(); ++i) {
            ...
            const Vector3d& p_WAo = body_A.EvalPoseInWorld(*plant_context_).translation();
            const Vector3d p_CAo_W = p_WAo - p_WC;
            const Vector3d& p_WBo = body_B.EvalPoseInWorld(*plant_context_).translation();
            const Vector3d p_CBo_W = p_WBo - p_WC;
    ...
    const ContactResults<double>& GetContactResults() const {
        return plant_->get_contact_results_output_port().Eval<ContactResults<double>>(*plant_context_);
    }
    ...
    std::optional<geometry::HydroelasticContactRepresentation> forced_hydroelastic_contact_representation_;
...
TEST_F(ContactModelTest, PointPairContact) {
    ...
    std::vector<SpatialForce<double>> F_BBo_W_array_expected = this->SpatialForceFromContactResults(contact_results);

    const std::vector<SpatialForce<double>>& F_BBo_W_array =
            MultibodyPlantTester::EvalSpatialContactForcesContinuous(*plant_, *plant_context_);
    ...
    for (int b = 1; b < plant_->num_bodies(); ++b) {
        ...
        EXPECT_TRUE(CompareMatrices(F_BBo_W_array[b].get_coeffs(), F_BBo_W_array_expected[b].get_coeffs()));
    }
}
...
TEST_F(ContactModelTest, HydroelasticOnly) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(GetContactResults(), "Requested contact between two rigid objects .+");
}
...
TEST_F(ContactModelTest, RejectPostFinalizeChange) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            plant_->set_contact_surface_representation(geometry::HydroelasticContactRepresentation::kPolygon),
...
protected:
    ...
    void Configure(double time_step) {
        ...
        ContactModelTest::Configure(ContactModel::kHydroelastic, connect_scene_graph, time_step,
    ...
    void RunTest(geometry::HydroelasticContactRepresentation expected_rep) {
        ...
        EXPECT_EQ(contact_results.num_hydroelastic_contacts(), 1);
        EXPECT_TRUE(contact_results.hydroelastic_contact_info(0).contact_surface().Equal(
...
TEST_F(CalcContactSurfacesTest, ContinuousSystem_Polygons) {
    forced_hydroelastic_contact_representation_ = geometry::HydroelasticContactRepresentation::kPolygon;
...
TEST_F(CalcContactSurfacesTest, DiscreteSystem_Triangles) {
    forced_hydroelastic_contact_representation_ = geometry::HydroelasticContactRepresentation::kTriangle;
...
protected:
    ...
    void Configure(double time_step) {
        ...
        // sphere-box contact.
        bool are_rigid_spheres_in_contact = true;
        ContactModelTest::Configure(ContactModel::kHydroelasticWithFallback, connect_scene_graph, time_step,
                                    are_rigid_spheres_in_contact);
    }
    ...
    void RunTest(geometry::HydroelasticContactRepresentation expected_rep) {
        ...
        plant_->get_geometry_query_input_port()
                .template Eval<geometry::QueryObject<double>>(*plant_context_)
                .ComputeContactSurfacesWithFallback(expected_rep, &expected_surfaces, &expected_point_pairs);

        // We only check the penetration depth as an evidence that the tested
        // result is what expected.
        EXPECT_EQ(contact_results.num_point_pair_contacts(), 1);
        EXPECT_EQ(contact_results.point_pair_contact_info(0).point_pair().depth, expected_point_pairs.at(0).depth);

        EXPECT_EQ(contact_results.num_hydroelastic_contacts(), 1);
        EXPECT_TRUE(contact_results.hydroelastic_contact_info(0).contact_surface().Equal(expected_surfaces.at(0)));
    }
};
