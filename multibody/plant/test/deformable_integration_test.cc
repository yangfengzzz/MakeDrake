...
protected:
    ...
    void SetUp() override {
        ...
        const RigidTransformd X_WG(RollPitchYawd(kSlopeAngle, 0, 0), Vector3d(0, 0, -0.7));
        const Box box(10, 10, 1);
        ground_collision_id_ =
                plant_->RegisterCollisionGeometry(plant_->world_body(), X_WG, box, "ground_collision", proximity_prop);
        IllustrationProperties illustration_props;
        illustration_props.AddProperty("phong", "diffuse", Vector4d(0.1, 0.8, 0.1, 0.8));
        plant_->RegisterVisualGeometry(plant_->world_body(), X_WG, box, "ground_visual", illustration_props);
        plant_->Finalize();
    ...
    const FemState<double>& EvalFemState(const Context<double>& context, DeformableBodyIndex index) const {
...
private:
    DeformableBodyId RegisterDeformableOctahedron(DeformableModel<double>* model, std::string name) {
        auto geometry = make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(0.1), std::move(name));
...
TEST_F(DeformableIntegrationTest, SteadyState) {
    ...
    const Context<double>& plant_context = plant_->GetMyContextFromRoot(diagram_context);
    const FemState<double>& fem_state = EvalFemState(plant_context, DeformableBodyIndex(0));
    ...
    EXPECT_TRUE(CompareMatrices(a, VectorXd::Zero(a.size()), kAccelerationThreshold));

    /* Computes the total contact force on the body. */
    const ContactSolverResults<double>& contact_solver_results = manager_->EvalContactSolverResults(plant_context);
    ...
    const Vector3d expected_contact_force = volume * kMassDensity * (-plant_->gravity_field().gravity_vector());
    ...
    const ContactResults<double>& contact_results = manager_->EvalContactResults(plant_context);
    ASSERT_EQ(contact_results.num_deformable_contacts(), 1);
    const DeformableContactInfo<double>& contact_info = contact_results.deformable_contact_info(0);
    ...
    const VectorXd& vertex_positions =
            plant_->get_deformable_body_configuration_output_port()
                    .template Eval<geometry::GeometryConfigurationVector<double>>(plant_context)
    ...
    for (int i = 0; i < num_vertices; ++i) {
        ...
        F_Ac_W_expected += SpatialForce<double>(Vector3d::Zero(), f_C_W.segment<3>(3 * i)).Shift(p_VC_W);
    }
    const double kTol = 16.0 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(contact_info.F_Ac_W().translational(), F_Ac_W_expected.translational(), kTol));
    EXPECT_TRUE(CompareMatrices(contact_info.F_Ac_W().rotational(), F_Ac_W_expected.rotational(), kTol));
}
