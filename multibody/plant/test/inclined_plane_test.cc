...
TEST_P(InclinedPlaneTest, RollingSphereTest) {
    ...
    const CoulombFriction<double> coefficient_friction_inclined_plane(muS_inclined_plane, muK_inclined_plane);
    const CoulombFriction<double> coefficient_friction_sphere(muS_sphere, muK_sphere);

    MultibodyPlant<double>& plant =
            AddMultibodyPlantSceneGraph(&builder, std::make_unique<MultibodyPlant<double>>(time_step_));
    benchmarks::inclined_plane::AddInclinedPlaneWithSphereToPlant(gravity, inclined_plane_angle, std::nullopt,
                                                                  coefficient_friction_inclined_plane,
                                                                  coefficient_friction_sphere, mass, radius, &plant);
    ...
    const SpatialVelocity<double>& V_WB = plant.EvalBodySpatialVelocityInWorld(plant_context, ball);
    const SpatialInertia<double> M_BBo_B = ball.default_spatial_inertia();
    const RigidTransform<double>& X_WB(plant.EvalBodyPoseInWorld(plant_context, ball));
    ...
    const double v_expected = std::sqrt(2 * gravity * h / (1.0 + G_Bcm / (radius * radius)));
    ...
    const auto& contact_forces_port = plant.get_generalized_contact_forces_output_port(default_model_instance());
    ...
    const double ft_expected = mass * gravity * (1.0 - radius * radius / G_Bc) * std::sin(inclined_plane_angle);
    ...
    EXPECT_LT(std::abs(friction_moment_expected - tau_contact(1)) / friction_moment_expected, relative_tolerance_);
}

// Instantiate the tests.
INSTANTIATE_TEST_SUITE_P(ContinuousAndTimeSteppingTest, InclinedPlaneTest, ::testing::Bool());
