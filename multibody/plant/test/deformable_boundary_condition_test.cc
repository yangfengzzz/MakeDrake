...
protected:
    ...
    void SetUp() override {
        ...
        plant_->RegisterVisualGeometry(plant_->world_body(), X_WG, box, "wall_visual", illustration_props);
        plant_->Finalize();
...
private:
    DeformableBodyId RegisterDeformableBall(DeformableModel<double>* model, std::string name) {
        auto geometry = make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(kRadius), std::move(name));
