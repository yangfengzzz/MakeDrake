...
public:
    ...
    static VectorXd CalcFreeMotionVelocities(const SapDriver<double>& driver, const Context<double>& context) {
        VectorXd v_star(driver.plant().num_velocities());
        driver.CalcFreeMotionVelocities(context, &v_star);
        return v_star;
    }

    static std::vector<MatrixXd> CalcLinearDynamicsMatrix(const SapDriver<double>& driver,
                                                          const Context<double>& context) {
...
public:
    const SapDriver<double>& sap_driver() const { return CompliantContactManagerTester::sap_driver(*manager_); }
    ...
    void SetSingleRobotModel() {
        ...
        auto owned_contact_manager = std::make_unique<CompliantContactManager<double>>();
    ...
    std::vector<ModelInstanceIndex> SetUpArmModel(MultibodyPlant<double>* plant) const {
        ...
        const Joint<double>& left_finger_slider = plant->GetJointByName("left_finger_sliding_joint", gripper_model);
        const Joint<double>& right_finger_slider = plant->GetJointByName("right_finger_sliding_joint", gripper_model);
        ...
        // numbers.
        plant->AddCouplerConstraint(left_finger_slider, right_finger_slider, kCouplerGearRatio, kCouplerOffset);
    ...
    MatrixXd CalcLinearDynamicsMatrixIncludingImplicitDampingContribution() const {
        const int nv = plant_.num_velocities();
        MatrixXd A(nv, nv);
        plant_.CalcMassMatrix(*context_, &A);
        A.diagonal() += plant_.time_step() * plant_.EvalJointDampingCache(*context_);
        return A;
    }
    ...
    void SetArbitraryStateWithLimitsSpecification(const MultibodyPlant<double>& plant,
    ...
    void SetArbitraryNonZeroActuation(const MultibodyPlant<double>& plant,
        ...
        const VectorX<double> tau_arm = VectorX<double>::LinSpaced(plant.num_actuated_dofs(arm_model), 10.0, 1000.0);
        const VectorX<double> tau_gripper =
                VectorX<double>::LinSpaced(plant.num_actuated_dofs(gripper_model), 10.0, 1000.0);
        plant.get_actuation_input_port(arm_model).FixValue(context, tau_arm);
        plant.get_actuation_input_port(gripper_model).FixValue(context, tau_gripper);
    }

private:
    std::vector<ModelInstanceIndex> LoadIiwaWithGripper(MultibodyPlant<double>* plant) const {
        DRAKE_DEMAND(plant != nullptr);
        const char kArmUrl[] = "package://drake_models/iiwa_description/urdf/iiwa14_no_collision.urdf";
        const char kWsg50Url[] = "package://drake_models/wsg_50_description/sdf/schunk_wsg_50.sdf";
    ...
    void AddInReflectedInertia(MultibodyPlant<double>* plant,
        ...
        for (JointActuatorIndex index : plant->GetJointActuatorIndices()) {
            JointActuator<double>& joint_actuator = plant->get_mutable_joint_actuator(index);
            if (std::count(models.begin(), models.end(), joint_actuator.model_instance()) > 0) {
                joint_actuator.set_default_rotor_inertia(rotor_inertias(local_joint_index));
    ...
    void SetArbitraryState(const MultibodyPlant<double>& plant, Context<double>* context) {
        for (JointIndex joint_index : plant.GetJointIndices()) {
            const Joint<double>& joint = plant.get_joint(joint_index);
            ...
            if (joint.type_name() == "revolute") {
                ...
                const double w = 1. * revolute_joint.velocity_start() / kNumJoints;  // Number in [0,1).
                ...
                // Set damping.
                revolute_joint.SetDamping(context, kJointDamping(revolute_joint.velocity_start()));
            } else if (joint.type_name() == "prismatic") {
                const PrismaticJoint<double>& prismatic_joint = dynamic_cast<const PrismaticJoint<double>&>(joint);
                // Arbitrary position within position limits.
                const double ql = prismatic_joint.position_lower_limit();
                const double qu = prismatic_joint.position_upper_limit();
                const double w = 1. * prismatic_joint.velocity_start() / kNumJoints;  // Number in [0,1).
                const double q = w * ql + (1.0 - w) * qu;                             // q in (ql, qu)
                prismatic_joint.set_translation(context, q);
                // Arbitrary velocity.
                prismatic_joint.set_translation_rate(context, 0.5 * joint_index);
                // Set damping.
                prismatic_joint.SetDamping(context, kJointDamping(prismatic_joint.velocity_start()));
            }
        }
    }
...
TEST_F(KukaIiwaArmTests, CalcLinearDynamicsMatrix) {
    SetSingleRobotModel();
    const std::vector<MatrixXd> A = SapDriverTest::CalcLinearDynamicsMatrix(sap_driver(), *context_);
    const int nv = plant_.num_velocities();
    MatrixXd Adense = MatrixXd::Zero(nv, nv);
    const MultibodyTreeTopology& topology = CompliantContactManagerTester::topology(*manager_);
    ...
    const MatrixXd Aexpected = CalcLinearDynamicsMatrixIncludingImplicitDampingContribution();
    EXPECT_TRUE(CompareMatrices(Adense, Aexpected, kEps, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaArmTests, CalcFreeMotionVelocities) {
    SetSingleRobotModel();
    const VectorXd v_star = SapDriverTest::CalcFreeMotionVelocities(sap_driver(), *context_);

    MultibodyForces<double> forces(plant_);
    CompliantContactManagerTester::CalcNonContactForces(*manager_, *context_, false, &forces);
    ...
    // A = M + dt*D
    const MatrixXd A = CalcLinearDynamicsMatrixIncludingImplicitDampingContribution();
    ...
    EXPECT_TRUE(CompareMatrices(v_star, v_star_expected, 5.0 * kEps, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaArmTests, CalcAccelerationKinematicsCache) {
    ...
    const VectorXd a_expected = (contact_results.v_next - v0) / plant_.time_step();
    std::vector<SpatialAcceleration<double>> A_WB_expected(plant_.num_bodies());
    plant_.CalcSpatialAccelerationsFromVdot(*context_, a_expected, &A_WB_expected);

    // Verify CompliantContactManager loads the acceleration kinematics with the
    // proper results.
    AccelerationKinematicsCache<double> ac(CompliantContactManagerTester::topology(*manager_));
...
TEST_F(KukaIiwaArmTests, LimitConstraints) {
    ...
    std::vector<InitializePositionAt> limits_specification(kNumJoints, InitializePositionAt::WellWithinLimits);
    limits_specification[0] = InitializePositionAt::BelowLowerLimit;
    limits_specification[1] = InitializePositionAt::AboveUpperLimit;
    limits_specification[2] = InitializePositionAt::AboveLowerLimitThoughPredictionBelow;
    limits_specification[3] = InitializePositionAt::BelowUpperLimitThoughPredictionAbove;
    ...
    SetArbitraryStateWithLimitsSpecification(plant_, limits_specification, context_.get());
    ...
    const ContactProblemCache<double>& problem_cache = SapDriverTest::EvalContactProblemCache(sap_driver(), *context_);
        ...
        if (joint.num_velocities() == 1) {
            ...
            if (limit_spec != InitializePositionAt::WellWithinLimits) {
                ...
                const auto* constraint =
                        dynamic_cast<const SapLimitConstraint<double>*>(&problem.get_constraint(num_constraints++));
                ...
                // sync.
                const SapLimitConstraint<double>::Parameters& params = constraint->parameters();
                ...
                const bool lower_limit_expected =
                        ...
                        limit_spec == InitializePositionAt::AboveLowerLimitThoughPredictionBelow;
                const bool upper_limit_expected =
                        limit_spec == InitializePositionAt::AboveUpperLimit ||
                        limit_spec == InitializePositionAt::BelowUpperLimitThoughPredictionAbove;

                const int expected_num_equations = lower_limit_expected && upper_limit_expected ? 2 : 1;
                EXPECT_EQ(constraint->num_constraint_equations(), expected_num_equations);
                EXPECT_EQ(constraint->first_clique_jacobian().rows(), expected_num_equations);

                const double kInf = std::numeric_limits<double>::infinity();
                const double ql_expected = lower_limit_expected ? joint.position_lower_limits()[0] : -kInf;
                const double qu_expected = upper_limit_expected ? joint.position_upper_limits()[0] : kInf;
...
TEST_F(KukaIiwaArmTests, CouplerConstraints) {
    ...
    // different arms.
    const Joint<double>& arm1_joint3 = plant_.GetJointByName("iiwa_joint_3", arm_gripper1[0]);
    const Joint<double>& arm2_joint6 = plant_.GetJointByName("iiwa_joint_6", arm_gripper2[0]);
    plant_.AddCouplerConstraint(arm1_joint3, arm2_joint6, kCouplerGearRatio, kCouplerOffset);
    ...
    context_ = plant_.CreateDefaultContext();
    SetArbitraryNonZeroActuation(plant_, arm_gripper1[0], arm_gripper1[1], context_.get());
    SetArbitraryNonZeroActuation(plant_, arm_gripper2[0], arm_gripper2[1], context_.get());

    // Specify a state in which all kNumJoints are within limits so that we know
    // the contact problem has no limit constraints.
    std::vector<InitializePositionAt> limits_specification(2 * kNumJoints, InitializePositionAt::WellWithinLimits);
    SetArbitraryStateWithLimitsSpecification(plant_, limits_specification, context_.get());
    ...
    coupler_joints.push_back({
            plant_.GetJointByName("left_finger_sliding_joint", arm_gripper1[1]).index(),
            plant_.GetJointByName("right_finger_sliding_joint", arm_gripper1[1]).index(),
    });

    // Coupler on second robot's gripper.
    coupler_joints.push_back({
            plant_.GetJointByName("left_finger_sliding_joint", arm_gripper2[1]).index(),
            plant_.GetJointByName("right_finger_sliding_joint", arm_gripper2[1]).index(),
    });
    ...
    for (int i = 0; i < 3; ++i) {
        const auto* constraint = dynamic_cast<const SapHolonomicConstraint<double>*>(&problem.get_constraint(i));
        ...
        if (i < 2) {
            ...
            const MatrixXd J_expected = (VectorXd::Unit(kNumJoints, left_index) -
            ...
            const MatrixXd J0_expected = VectorXd::Unit(kNumJoints, 2 /* third joint. */).transpose();
            const MatrixXd J1_expected =
                    -kCouplerGearRatio * VectorXd::Unit(kNumJoints, 5 /* sixth joint. */).transpose();
        ...
        const Vector1d kInfinity = Vector1d::Constant(std::numeric_limits<double>::infinity());
        const SapHolonomicConstraint<double>::Parameters& params = constraint->parameters();
