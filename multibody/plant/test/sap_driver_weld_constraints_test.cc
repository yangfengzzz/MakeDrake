...
TEST_P(TwoBodiesTest, ConfirmConstraintProperties) {
    ...
    // function.
    plant_.SetFreeBodyPoseInWorldFrame(context_.get(), *bodyB_, RigidTransformd(kOffset_));
    ...
    const auto* constraint = dynamic_cast<const SapWeldConstraint<double>*>(&problem.get_constraint(0));
    ...
    const RotationMatrixd R_PQ = X_WP.rotation().InvertAndCompose(X_WQ.rotation());
    ...
    //
    //        = J_B ⋅ V_WB - J_A ⋅ V_WA
    const Vector3d p_AAm_W = constraint->kinematics().p_AP_W() + 0.5 * constraint->kinematics().p_PoQo_W();
    const Vector3d p_BBm_W = constraint->kinematics().p_BQ_W() - 0.5 * constraint->kinematics().p_PoQo_W();
  ...
  const Matrix3d p_BBm_Wx =
    ...
    if (expected_num_cliques == 1) {
      ...
      const MatrixXd J_expected =
        ...
        // clang-format on
        EXPECT_TRUE(CompareMatrices(J, J_expected, kEps, MatrixCompareType::relative));
    } else {
    ...
    const MatrixXd Ja_expected =
      -(MatrixXd(6, 6) <<         I3, Matrix3d::Zero(),
        ...
        // clang-format on
        EXPECT_TRUE(CompareMatrices(Ja, Ja_expected, kEps, MatrixCompareType::relative));
      ...
      const MatrixXd Jb_expected =
        (MatrixXd(6, 6) <<        I3, Matrix3d::Zero(),
                           -p_BBm_Wx,               I3).finished();
        // clang-format on
        EXPECT_TRUE(CompareMatrices(Jb, Jb_expected, kEps, MatrixCompareType::relative));
    }
}
...
INSTANTIATE_TEST_SUITE_P(SapWeldConstraintTests,
                         TwoBodiesTest,
...
GTEST_TEST(WeldConstraintsTests, VerifyIdMapping) {
    ...
    MultibodyConstraintId weld_id = plant.AddWeldConstraint(bodyA, X_AP, bodyB, X_BQ);
    const WeldConstraintSpec& weld_spec = plant.get_weld_constraint_specs(weld_id);
    ...
    const std::map<MultibodyConstraintId, WeldConstraintSpec>& weld_specs = plant.get_weld_constraint_specs();
...
GTEST_TEST(BallConstraintTests, FailOnTAMSI) {
    ...
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    const RigidBody<double>& bodyB = plant.AddRigidBody("B", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddWeldConstraint(bodyA, RigidTransformd(), bodyB, RigidTransformd()),
...
GTEST_TEST(WeldConstraintTests, FailOnContinuous) {
    ...
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    const RigidBody<double>& bodyB = plant.AddRigidBody("B", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddWeldConstraint(bodyA, RigidTransformd{Vector3d{0, 0, 0}}, bodyB,
...
GTEST_TEST(WeldConstraintTests, FailOnSameBody) {
    ...
    plant.set_discrete_contact_approximation(DiscreteContactApproximation::kSap);
    const RigidBody<double>& bodyA = plant.AddRigidBody("A", SpatialInertia<double>::NaN());
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddWeldConstraint(bodyA, RigidTransformd{Vector3d{0, 0, 0}}, bodyA,
                                                        RigidTransformd{Vector3d{0, 0, 0}}),
