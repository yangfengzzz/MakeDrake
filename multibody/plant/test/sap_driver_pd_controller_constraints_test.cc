...
TEST_F(ActuatedIiiwaArmTest, VerifyConstraints) {
    ...
    const VectorXd arm_u = VectorXd::LinSpaced(kKukaNumPositions_, 1.0, kKukaNumPositions_);
    ...
    plant_->get_actuation_input_port(gripper_model_).FixValue(context_.get(), gripper_ff);
    ...
    auto make_finger_config = [&](const std::string& name) {
        ...
        SapPdControllerConstraint<double>::Configuration config{clique, clique_dof, clique_nv, q0, qd, vd, u_ff};
        return config;
    };
    ...
    for (int k = 0; k < problem.num_constraints(); ++k) {
        const auto* constraint = dynamic_cast<const SapPdControllerConstraint<double>*>(&problem.get_constraint(k));
