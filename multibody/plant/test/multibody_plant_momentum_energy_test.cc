...
GTEST_TEST(EmptyMultibodyPlantMomentumTest, CalcSpatialMomentumEmptyPlant) {
    ...
    const SpatialMomentum<double> spatial_momentum = plant.CalcSpatialMomentumInWorldAboutPoint(*context, p_WoP_W);
...
public:
    ...
    void SetUp() override {
        ...
        const SpatialInertia<double> M_Bcm =
                SpatialInertia<double>::MakeFromCentralInertia(mass_link_, p_BoBcm_B, I_BBcm);
        ...
        const Vector3<double> bodyA_translational_momentum_expected = mass_link_ * vA * Vector3<double>::UnitY();
        const Vector3<double> bodyB_translational_momentum_expected = mass_link_ * vB * Vector3<double>::UnitY();
        ...
        const Vector3<double> bodyB_angular_momentum_about_Wo =
                (Izz_ * (wAz_ + wBz_) + 2.25 * mLL * wAz_ + 0.75 * mLL * wBz_) * Vector3<double>::UnitZ();

        // Consolidate translational and angular momentum into spatial momentum.
        L_WAWo_W_expected_ =
                SpatialMomentum<double>(bodyA_angular_momentum_about_Wo, bodyA_translational_momentum_expected);
        L_WBWo_W_expected_ =
                SpatialMomentum<double>(bodyB_angular_momentum_about_Wo, bodyB_translational_momentum_expected);
    }
...
TEST_F(TwoDofPlanarPendulumTest, CalcSpatialMomentumEmptySet) {
    ...
    const SpatialMomentum<double> L_WEP_W =
            plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoP_W);
...
TEST_F(TwoDofPlanarPendulumTest, CalcSpatialMomentumNonsenseSet) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoWo_W),
...
TEST_F(TwoDofPlanarPendulumTest, CalcBodyASpatialMomentumInWorldAboutPoint) {
    ...
    const SpatialMomentum<double> L_WAWo_W =
            plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoWo_W);
    EXPECT_TRUE(CompareMatrices(L_WAWo_W.get_coeffs(), L_WAWo_W_expected_.get_coeffs(), kTolerance_));

    // Calculate body A's spatial momentum in world W, about Acm, expressed in W.
    // Compare to expected results.
    const Vector3<double> p_WoAcm_W = plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances);
    const SpatialMomentum<double> L_WAcm_W =
            plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoAcm_W);
    const SpatialMomentum<double> L_WAcm_W_expected = L_WAWo_W_expected_.Shift(p_WoAcm_W);
    EXPECT_TRUE(CompareMatrices(L_WAcm_W.get_coeffs(), L_WAcm_W_expected.get_coeffs(), kTolerance_));
}
...
TEST_F(TwoDofPlanarPendulumTest, CalcBodyBSpatialMomentumInWorldAboutPoint) {
    ...
    const Vector3<double> p_WoBcm_W = plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances);
    const SpatialMomentum<double> L_WBcm_W =
            plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoBcm_W);
    const SpatialMomentum<double> L_WBcm_W_expected = L_WBWo_W_expected_.Shift(p_WoBcm_W);
    EXPECT_TRUE(CompareMatrices(L_WBcm_W.get_coeffs(), L_WBcm_W_expected.get_coeffs(), kTolerance_));
}
...
TEST_F(TwoDofPlanarPendulumTest, CalcSystemSpatialMomentumInWorldAboutWo) {
    ...
    const ModelInstanceIndex bodyA_model_instance = plant_.GetModelInstanceByName("bodyA_model_instance");
    const ModelInstanceIndex bodyB_model_instance = plant_.GetBodyByName("BodyB").model_instance();
    const std::vector<ModelInstanceIndex> model_instances{bodyA_model_instance, bodyB_model_instance};
    ...
    const SpatialMomentum<double> L_WSWo_W_expected = L_WAWo_W_expected_ + L_WBWo_W_expected_;
    EXPECT_TRUE(CompareMatrices(L_WSWo_W.get_coeffs(), L_WSWo_W_expected.get_coeffs(), kTolerance_));
...
TEST_F(TwoDofPlanarPendulumTest, CalcSystemSpatialMomentumInWorldAboutPoint) {
    ...
    model_instances.push_back(bodyB_model_instance_);
    model_instances.push_back(bodyA_model_instance_);
    const Vector3<double> p_WoScm_W = plant_.CalcCenterOfMassPositionInWorld(*context_, model_instances);
    SpatialMomentum<double> L_WScm_W =
            plant_.CalcSpatialMomentumInWorldAboutPoint(*context_, model_instances, p_WoScm_W);
    const SpatialMomentum<double> L_WSWo_W_expected = L_WAWo_W_expected_ + L_WBWo_W_expected_;
    const SpatialMomentum<double> L_WScm_W_expected = L_WSWo_W_expected.Shift(p_WoScm_W);
    EXPECT_TRUE(CompareMatrices(L_WScm_W.get_coeffs(), L_WScm_W_expected.get_coeffs(), kTolerance_));
...
TEST_F(TwoDofPlanarPendulumTest, CalcSpatialInertia) {
    ...
    const SpatialInertia<double> M_SWo_W = plant_.CalcSpatialInertia(*context_, frame_W, body_indexes);
    Matrix6<double> M6_expected = Matrix6<double>::Zero();
    EXPECT_TRUE(CompareMatrices(M_SWo_W.CopyToFullMatrix6(), M6_expected, kEpsilon));
    ...
    const SpatialInertia<double> M_SAo_A = plant_.CalcSpatialInertia(*context_, frame_A, body_indexes);
    const SpatialInertia<double> M_AAo_A = body_A.CalcSpatialInertiaInBodyFrame(*context_);
    M6_expected = M_AAo_A.CopyToFullMatrix6();
    EXPECT_TRUE(CompareMatrices(M_SAo_A.CopyToFullMatrix6(), M6_expected, 16 * kEpsilon));
    ...
    SpatialInertia<double> M_SBo_B = plant_.CalcSpatialInertia(*context_, frame_B, body_indexes);
    const SpatialInertia<double> M_BBo_B = body_B.CalcSpatialInertiaInBodyFrame(*context_);
    ...
    EXPECT_TRUE(CompareMatrices(M_SBo_B.CopyToFullMatrix6(), M6_expected, 512 * kEpsilon));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant_.CalcSpatialInertia(*context_, frame_A, body_indexes),
