...
public:
    ...
    static const internal::JointLockingCacheData<double>& EvalJointLocking(const MultibodyPlant<double>& plant,
                                                                           const Context<double>& context) {
        return plant.EvalJointLocking(context);
    }

    template <typename T>
    static BodyIndex FindBodyByGeometryId(const MultibodyPlant<T>& plant, geometry::GeometryId id) {
...
private:
    void AddFloatingPendulum() {
        ...
        std::unique_ptr<RevoluteJoint<double>> body1_body2 = std::make_unique<RevoluteJoint<double>>(
                "body1_body2", plant_->GetRigidBodyByName("body1").body_frame(),
                plant_->GetRigidBodyByName("body2").body_frame(), Vector3d::UnitZ());
    ...
    void AddDoublePendulum() {
        ...
        std::unique_ptr<RevoluteJoint<double>> world_body3 = std::make_unique<RevoluteJoint<double>>(
                "world_body3", plant_->world_frame(), plant_->GetRigidBodyByName("body3").body_frame(),
                Vector3d::UnitZ());
        std::unique_ptr<RevoluteJoint<double>> body3_body4 = std::make_unique<RevoluteJoint<double>>(
                "body3_body4", plant_->GetRigidBodyByName("body3").body_frame(),
                plant_->GetRigidBodyByName("body4").body_frame(), Vector3d::UnitZ());
...
TEST_P(JointLockingTest, JointLockingIndicesTest) {
    ...
    const RevoluteJoint<double>& body1_body2 = plant_->GetJointByName<RevoluteJoint>("body1_body2");
    const RevoluteJoint<double>& world_body3 = plant_->GetJointByName<RevoluteJoint>("world_body3");
    RevoluteJoint<double>& body3_body4 = plant_->GetMutableJointByName<RevoluteJoint>("body3_body4");
    ...
    // No joints/bodies are locked, all joint/body velocity indices should exist.
    {
        ...
        const std::vector<int>& unlocked_velocity_indices = cache.unlocked_velocity_indices;
        const std::vector<int>& locked_velocity_indices = cache.locked_velocity_indices;
        const std::vector<std::vector<int>>& unlocked_velocity_indices_per_tree =
                cache.unlocked_velocity_indices_per_tree;
        const std::vector<std::vector<int>>& locked_velocity_indices_per_tree = cache.locked_velocity_indices_per_tree;
        ...
        const std::vector<int> expected_unlocked_velocity_indices = {
                body1_velocity_start,         body1_velocity_start + 1,     body1_velocity_start + 2,
                body1_velocity_start + 3,     body1_velocity_start + 4,     body1_velocity_start + 5,
                body1_body2.velocity_start(), world_body3.velocity_start(), body3_body4.velocity_start()};
        const std::vector<int> expected_locked_velocity_indices = {};

        EXPECT_THAT(unlocked_velocity_indices, testing::UnorderedElementsAreArray(expected_unlocked_velocity_indices));
        EXPECT_THAT(locked_velocity_indices, testing::UnorderedElementsAreArray(expected_locked_velocity_indices));

        const std::vector<std::vector<int>> expected_unlocked_velocity_indices_per_tree = {
                {0, 1},
                {0, 1, 2, 3, 4, 5, 6},
        };
        const std::vector<std::vector<int>> expected_locked_velocity_indices_per_tree = {{}, {}};

        for (int i = 0; i < 2; ++i) {
            EXPECT_THAT(unlocked_velocity_indices_per_tree[i],
                        testing::UnorderedElementsAreArray(expected_unlocked_velocity_indices_per_tree[i]));
            EXPECT_THAT(locked_velocity_indices_per_tree[i],
                        testing::UnorderedElementsAreArray(expected_locked_velocity_indices_per_tree[i]));
        }
    }
    ...
    // Lock body3_body4 and re-evaluate joint locking indices
    {
        ...
        const std::vector<int>& expected_locked_velocity_indices = {body3_body4.velocity_start()};

        EXPECT_THAT(unlocked_velocity_indices, testing::UnorderedElementsAreArray(expected_unlocked_velocity_indices));
        EXPECT_THAT(locked_velocity_indices, testing::UnorderedElementsAreArray(expected_locked_velocity_indices));

        const std::vector<std::vector<int>> expected_unlocked_velocity_indices_per_tree = {{0}, {0, 1, 2, 3, 4, 5, 6}};
        const std::vector<std::vector<int>> expected_locked_velocity_indices_per_tree = {{1}, {}};
    ...
    // Unlock body3_body4 and lock body1 and re-evaluate joint locking indices.
    {
        ...
        const std::vector<int>& expected_unlocked_velocity_indices = {
                body1_body2.velocity_start(), world_body3.velocity_start(), body3_body4.velocity_start()};
        const std::vector<int>& expected_locked_velocity_indices = {body1_velocity_start,     body1_velocity_start + 1,
        ...
        const std::vector<std::vector<int>> expected_unlocked_velocity_indices_per_tree = {{0, 1}, {6}};
        const std::vector<std::vector<int>> expected_locked_velocity_indices_per_tree = {{}, {0, 1, 2, 3, 4, 5}};
...
INSTANTIATE_TEST_SUITE_P(IndexPermutations, JointLockingTest, ::testing::Values(0, 1));
...
public:
    ...
    // two joints.
    std::unique_ptr<MultibodyPlant<double>> MakeDoublePendulumPlant(bool weld_elbow, DiscreteContactSolver solver) {
        ...
        const RigidBody<double>& body1 = plant->AddRigidBody("upper_arm", SpatialInertia<double>::MakeUnitary());
        const RigidBody<double>& body2 = plant->AddRigidBody("lower_arm", SpatialInertia<double>::MakeUnitary());

        plant->AddJoint<RevoluteJoint>("shoulder", plant->world_body(), {}, body1,
                                       RigidTransformd(Vector3d(kArmLength, 0, 0)), Vector3d::UnitY());
        ...
        if (weld_elbow) {
            ...
            plant->AddJoint<RevoluteJoint>("elbow", body1, {}, body2, RigidTransformd(Vector3d(kArmLength, 0, 0.0)),
                                           Vector3d::UnitY());
        }
        plant->Finalize();
        return plant;
...
TEST_P(TrajectoryTest, CompareWeldAndLocked) {
    ...
    std::unique_ptr<systems::Context<double>> context_welded = plant_welded_->CreateDefaultContext();
    std::unique_ptr<systems::Context<double>> context_locked = plant_locked_->CreateDefaultContext();

    // Lock the elbow in the unwelded plant.
    const RevoluteJoint<double>& elbow = plant_locked_->GetJointByName<RevoluteJoint>("elbow");
    ...
    auto simulator_welded = std::make_unique<Simulator<double>>(*plant_welded_, std::move(context_welded));
    auto simulator_locked = std::make_unique<Simulator<double>>(*plant_locked_, std::move(context_locked));
    ...
    for (int i = 1; i <= kNumTimesteps; ++i) {
        ...
        const auto& welded_context = plant_welded_->GetMyContextFromRoot(simulator_welded->get_context());
        const auto& locked_context = plant_locked_->GetMyContextFromRoot(simulator_locked->get_context());

        int shoulder_index_welded = plant_welded_->GetJointByName<RevoluteJoint>("shoulder").velocity_start();
        int shoulder_index_locked = plant_locked_->GetJointByName<RevoluteJoint>("shoulder").velocity_start();
        ...
        const auto welded_accelerations =
                plant_welded_->get_generalized_acceleration_output_port().Eval(welded_context);
        const auto locked_accelerations =
                plant_locked_->get_generalized_acceleration_output_port().Eval(locked_context);
        EXPECT_NEAR(welded_accelerations[shoulder_index_welded], locked_accelerations[shoulder_index_locked], kEps);
        // The locked elbow dof should have 0 acceleration.
        int elbow_index_locked = plant_locked_->GetJointByName<RevoluteJoint>("elbow").velocity_start();
        ...
        EXPECT_NEAR(welded_positions_and_velocities[plant_welded_->num_positions() + shoulder_index_welded],
                    locked_positions_and_velocities[plant_locked_->num_positions() + shoulder_index_locked], kEps);

        // Position of the locked joint should be identically kElbowPosition.
        // Velocity of the locked joint should be identically 0.
        EXPECT_EQ(locked_positions_and_velocities[elbow_index_locked], kElbowPosition);
        EXPECT_EQ(locked_positions_and_velocities[plant_locked_->num_positions() + elbow_index_locked], 0);
    }
}
...
INSTANTIATE_TEST_SUITE_P(JointLockingTests,
                         TrajectoryTest,
...
std::ostream& operator<<(std::ostream& out, const FilteredContactResultsConfig& c) {
    return out << internal::GetStringFromContactModel(c.contact_model) << "_"
               << (c.solver.has_value() ? internal::GetStringFromDiscreteContactSolver(*c.solver)
...
class FilteredContactResultsTest : public ::testing::TestWithParam<FilteredContactResultsConfig> {
public:
    void SetUp() {
    ...
    const RigidBody<double>& AddBall(const std::string& name) {
        SpatialInertia<double> M_BBcm = SpatialInertia<double>::SolidSphereWithMass(mass_, radius_);
        ...
        geometry::AddContactMaterial(1e-4 /* dissipation */, {} /* point stiffness */,
                                     CoulombFriction<double>{1.0, 1.0} /* friction */, &ball_props);

        // N.B. these properties go unused if the contact model is kPoint.
        geometry::AddCompliantHydroelasticProperties(0.1 * radius_, hydroelastic_modulus_, &ball_props);

        plant_->RegisterCollisionGeometry(ball, X_BS, geometry::Sphere(radius_), "collision", std::move(ball_props));

        // Add visual for the ball.
        const Vector4<double> red(1.0, 0.0, 0.0, 1.0);
        plant_->RegisterVisualGeometry(ball, X_BS, geometry::Sphere(radius_), "visual", red);
        return ball;
    }
...
TEST_P(FilteredContactResultsTest, VerifyLockedResults) {
    ...
    std::unique_ptr<systems::Context<double>> diagram_context = diagram_->CreateDefaultContext();
    systems::Context<double>& context = diagram_->GetMutableSubsystemContext(*plant_, diagram_context.get());
    ...
    // All spheres unlocked, expect two contact results.
    {
        const ContactResults<double>& results =
                plant_->get_contact_results_output_port().Eval<ContactResults<double>>(context);
        ...
        if (config.contact_model == ContactModel::kPoint) {
            ...
            for (int i = 0; i < results.num_point_pair_contacts(); ++i) {
                const PointPairContactInfo<double>& info = results.point_pair_contact_info(i);
                const BodyIndex body_A = MultibodyPlantTester::FindBodyByGeometryId(*plant_, info.point_pair().id_A);
                const BodyIndex body_B = MultibodyPlantTester::FindBodyByGeometryId(*plant_, info.point_pair().id_B);
...
std::vector<FilteredContactResultsConfig> MakeFilteredContactResultsTestCases() {
    return std::vector<FilteredContactResultsConfig>{
            ...
            {.contact_model = ContactModel::kPoint, .solver = DiscreteContactSolver::kTamsi},
            {.contact_model = ContactModel::kHydroelastic, .solver = DiscreteContactSolver::kTamsi},
            {.contact_model = ContactModel::kPoint, .solver = DiscreteContactSolver::kSap},
            {.contact_model = ContactModel::kHydroelastic, .solver = DiscreteContactSolver::kSap},
    };
}

INSTANTIATE_TEST_SUITE_P(JointLockingTests,
                         FilteredContactResultsTest,
