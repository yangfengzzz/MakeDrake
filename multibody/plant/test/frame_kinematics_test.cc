...
#include "multibody/plant/test/kuka_iiwa_model_tests.h"
#include "multibody/test_utilities/add_fixed_objects_to_plant.h"
#include "multibody/test_utilities/spatial_derivative.h"
...
TEST_F(KukaIiwaModelTests, FramesKinematics) {
    ...
    const RigidTransformd X_HH = plant_->CalcRelativeTransform(*context_, *frame_H_, *frame_H_);
    EXPECT_TRUE(CompareMatrices(X_HH.rotation().matrix(), Matrix3<double>::Identity(), kTolerance,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(X_HH.translation(), Vector3<double>::Zero(), kTolerance, MatrixCompareType::relative));

    // Test for a simple identity case of CalcRelativeRotationMatrix().
    const RotationMatrixd R_HH = plant_->CalcRelativeRotationMatrix(*context_, *frame_H_, *frame_H_);
    EXPECT_TRUE(CompareMatrices(R_HH.matrix(), Matrix3<double>::Identity(), kTolerance, MatrixCompareType::relative));

    const RigidTransform<double>& X_WE = end_effector_link_->EvalPoseInWorld(*context_);
    const RigidTransform<double> X_WH = frame_H_->CalcPoseInWorld(*context_);
    const RigidTransform<double> X_WH_expected = X_WE * X_EH_;
    EXPECT_TRUE(CompareMatrices(X_WH.GetAsMatrix34(), X_WH_expected.GetAsMatrix34(), kTolerance,
                                MatrixCompareType::relative));

    // Alternatively, we can get the pose X_WE using the plant's output port for
    // poses.
    const auto& X_WB_all = plant_->get_body_poses_output_port().Eval<std::vector<RigidTransform<double>>>(*context_);
    ASSERT_EQ(X_WB_all.size(), plant_->num_bodies());
    const RigidTransform<double>& X_WE_from_port = X_WB_all[end_effector_link_->index()];
    EXPECT_TRUE(CompareMatrices(X_WE.GetAsMatrix34(), X_WE_from_port.GetAsMatrix34(), kTolerance,
                                MatrixCompareType::relative));

    // Verify the invariance X_WB_all[0] = RigidTransform<double>::Identity().
    EXPECT_TRUE(CompareMatrices(X_WB_all[0].GetAsMatrix34(), RigidTransform<double>::Identity().GetAsMatrix34(),
                                kTolerance, MatrixCompareType::relative));

    const RotationMatrix<double> R_WH = frame_H_->CalcRotationMatrixInWorld(*context_);
    const RotationMatrix<double>& R_WH_expected = X_WH_expected.rotation();
    EXPECT_TRUE(CompareMatrices(R_WH.matrix(), R_WH_expected.matrix(), kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(X_HL3.GetAsMatrix34(), X_HL3_expected.GetAsMatrix34(), kTolerance,
                                MatrixCompareType::relative));

    const RotationMatrix<double> R_HL3 = frame_L3.CalcRotationMatrix(*context_, *frame_H_);
    const RotationMatrix<double> R_WL3 = frame_L3.CalcRotationMatrixInWorld(*context_);
    const RotationMatrix<double> R_HL3_expected = R_WH.InvertAndCompose(R_WL3);
    EXPECT_TRUE(CompareMatrices(R_HL3.matrix(), R_HL3_expected.matrix(), kTolerance, MatrixCompareType::relative));

    const SpatialVelocity<double> V_WE = end_effector_link_->EvalSpatialVelocityInWorld(*context_);
    const SpatialVelocity<double> V_WH = frame_H_->CalcSpatialVelocityInWorld(*context_);
    const Vector3<double> p_EH = frame_H_->GetFixedPoseInBodyFrame().translation();
    ...
    EXPECT_TRUE(
            CompareMatrices(V_WH.get_coeffs(), V_WH_expected.get_coeffs(), kTolerance, MatrixCompareType::relative));

    // Alternatively, we can get the spatial velocity V_WE using the plant's
    // output port for spatial velocities.
    const auto& V_WB_all =
            plant_->get_body_spatial_velocities_output_port().Eval<std::vector<SpatialVelocity<double>>>(*context_);
    ASSERT_EQ(V_WB_all.size(), plant_->num_bodies());
    const SpatialVelocity<double>& V_WE_from_port = V_WB_all[end_effector_link_->index()];
    ...
    // Also sanity check the deprecated port name (2024-10-01).
    {
        const auto& V_WB_all_deprecated =
                plant_->GetOutputPort("spatial_velocities").Eval<std::vector<SpatialVelocity<double>>>(*context_);
    ...
    const SpatialVelocity<double> V_HL3_E = frame_L3.CalcSpatialVelocity(*context_, *frame_H_, frame_E);
    // Compute V_HL3_E_expected.
    const RotationMatrix<double> R_EW = R_WE.inverse();
    const SpatialVelocity<double> V_WH_E = R_EW * V_WH;
    const math::RotationMatrix<double> R_EH = frame_H_->GetFixedPoseInBodyFrame().rotation();
    const Vector3<double> p_HL3_E = R_EH * X_HL3.translation();
    const SpatialVelocity<double> V_WL3_E = R_EW * link3.EvalSpatialVelocityInWorld(*context_);
    // V_WL3_E = V_WH_E.Shift(p_HL3_E) + V_HL3_E
    const SpatialVelocity<double> V_HL3_E_expected = V_WL3_E - V_WH_E.Shift(p_HL3_E);
    EXPECT_TRUE(CompareMatrices(V_HL3_E.get_coeffs(), V_HL3_E_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_EH_W = frame_H_->CalcSpatialVelocity(*context_, frame_E, frame_W);
    EXPECT_TRUE(CompareMatrices(V_EH_W.get_coeffs(), Vector6<double>::Zero(), kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, FrameAngularVelocity) {
    ...
    const Frame<double>& frame_E = end_effector_link_->body_frame();
    const Vector3<double> w_EH_E = frame_H.CalcAngularVelocity(*context_, frame_E, frame_E);
    EXPECT_TRUE(CompareMatrices(w_EH_E, Vector3<double>::Zero(), kTolerance, MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_WH_W = frame_H.CalcSpatialVelocityInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(w_WH_W, V_WH_W.rotational(), kTolerance, MatrixCompareType::relative));

    // Verify CalcAngularVelocity() against EvalAngularVelocityInWorld().
    const RigidBody<double>& link3 = plant_->GetBodyByName("iiwa_link_3");
    const Frame<double>& frame_L3 = link3.body_frame();
    const Frame<double>& frame_W = plant_->world_frame();
    const Vector3<double> w_WL3_W = frame_L3.CalcAngularVelocity(*context_, frame_W, frame_W);
    const Vector3<double> w_WL3_W_expected = frame_L3.EvalAngularVelocityInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(w_WL3_W, w_WL3_W_expected, kTolerance, MatrixCompareType::relative));
    ...
    const Vector3<double> w_L3E_H = frame_E.CalcAngularVelocity(*context_, frame_L3, frame_H);
    const Vector3<double> w_EL3_H = frame_L3.CalcAngularVelocity(*context_, frame_E, frame_H);
    EXPECT_NE(w_L3E_H, Vector3<double>::Zero());
    EXPECT_TRUE(CompareMatrices(w_L3E_H, -w_EL3_H, kTolerance, MatrixCompareType::relative));

    // Verify CalcAngularVelocity() produces the same results as
    // CalcSpatialVelocity().rotational() and handles the expressed-in frame.
    const Vector3<double> w_L3W_E_expected = frame_W.CalcSpatialVelocity(*context_, frame_L3, frame_E).rotational();
    const Vector3<double> w_L3W_E = frame_W.CalcAngularVelocity(*context_, frame_L3, frame_E);
    EXPECT_TRUE(CompareMatrices(w_L3W_E_expected, w_L3W_E, kTolerance, MatrixCompareType::relative));

    // Verify CalcAngularVelocity() produces the same results as
    // CalcRelativeSpatialVelocity().rotational().
    const Vector3<double> w_L3E_E_expected =
            frame_E.CalcRelativeSpatialVelocity(*context_, frame_L3, frame_H, frame_E).rotational();
    const Vector3<double> w_L3E_E = frame_E.CalcAngularVelocity(*context_, frame_L3, frame_E);
    EXPECT_TRUE(CompareMatrices(w_L3E_E_expected, w_L3E_E, kTolerance, MatrixCompareType::relative));
    ...
    const Vector3<double> w_WE_W_expected = frame_E.CalcSpatialVelocityInWorld(*context_).rotational();
    const Vector3<double> w_HL3_W = frame_L3.CalcAngularVelocity(*context_, frame_H, frame_W);
    const Vector3<double> w_L3E_W = frame_E.CalcAngularVelocity(*context_, frame_L3, frame_W);
    const Vector3<double> w_WE_W = w_WH_W + w_HL3_W + w_L3E_W;
    EXPECT_TRUE(CompareMatrices(w_WE_W_expected, w_WE_W, kTolerance, MatrixCompareType::relative));

    // Verify CalcAngularVelocity() handles the expressed-in frame.
    const RotationMatrix<double> R_WE = frame_E.CalcRotationMatrixInWorld(*context_);
    const RotationMatrix<double> R_EW = R_WE.inverse();
    const Vector3<double> w_WE_E_expected = R_EW * w_WE_W_expected;
    const Vector3<double> w_WE_E = frame_E.CalcAngularVelocity(*context_, frame_W, frame_E);
    EXPECT_TRUE(CompareMatrices(w_WE_E_expected, w_WE_E, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, FramesCalcRelativeSpatialVelocity) {
    ...
    const RigidBody<double>& link3 = plant_->GetBodyByName("iiwa_link_3");
    const Frame<double>& frame_L3 = link3.body_frame();
    const SpatialVelocity<double> V_W_L3H_W = frame_H.CalcRelativeSpatialVelocityInWorld(*context_, frame_L3);
    const SpatialVelocity<double> V_WL3_W = frame_L3.CalcSpatialVelocityInWorld(*context_);
    const SpatialVelocity<double> V_WH_W = frame_H.CalcSpatialVelocityInWorld(*context_);
    const SpatialVelocity<double> V_W_L3H_W_expected = V_WH_W - V_WL3_W;
    EXPECT_TRUE(CompareMatrices(V_W_L3H_W.get_coeffs(), V_W_L3H_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_W_HL3_W = frame_L3.CalcRelativeSpatialVelocityInWorld(*context_, frame_H);
    EXPECT_TRUE(
            CompareMatrices(V_W_L3H_W.get_coeffs(), -V_W_HL3_W.get_coeffs(), kTolerance, MatrixCompareType::relative));

    // Also verify that the rotational part of V_W_L3H_W is simply ω_L3H_W.
    const Frame<double>& frame_W = plant_->world_frame();
    const Vector3<double>& w_L3H_W = V_W_L3H_W.rotational();
    const Vector3<double> w_L3H_W_expected = frame_H.CalcSpatialVelocity(*context_, frame_L3, frame_W).rotational();
    EXPECT_TRUE(CompareMatrices(w_L3H_W, w_L3H_W_expected, kTolerance, MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_W_WH_W = frame_H.CalcRelativeSpatialVelocityInWorld(*context_, frame_W);
    const SpatialVelocity<double> V_W_WH_W_expected = frame_H.CalcSpatialVelocityInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(V_W_WH_W.get_coeffs(), V_W_WH_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));

    const SpatialVelocity<double> V_L3_L3H_W =
            frame_H.CalcRelativeSpatialVelocity(*context_, frame_L3, frame_L3, frame_W);
    const SpatialVelocity<double> V_L3_L3H_W_expected = frame_H.CalcSpatialVelocity(*context_, frame_L3, frame_W);
    EXPECT_TRUE(CompareMatrices(V_L3_L3H_W.get_coeffs(), V_L3_L3H_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const Frame<double>& frame_E = end_effector_link_->body_frame();
    const SpatialVelocity<double> V_W_EH_W = frame_H.CalcRelativeSpatialVelocityInWorld(*context_, frame_E);
    const Vector3<double>& w_EH_W = V_W_EH_W.rotational();
    EXPECT_TRUE(CompareMatrices(w_EH_W, Vector3<double>::Zero(), kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(V_W_EH_W.translational(), w_cross_p, kTolerance, MatrixCompareType::relative));
    ...
    const Vector3<double> p_L3oHo_L3 = frame_H.CalcPose(*context_, frame_L3).translation();
    const RotationMatrix<double> R_WL3 = frame_L3.CalcRotationMatrixInWorld(*context_);
...
TEST_F(KukaIiwaModelTests, CalcSpatialAcceleration) {
    ...
    const Frame<double>& frame_E = end_effector_link_->body_frame();
    const SpatialAcceleration<double> A_WE_W = frame_E.CalcSpatialAccelerationInWorld(*context_);
    ...
    const SpatialAcceleration<double> A_WE_W_expected2 = end_effector_link_->EvalSpatialAccelerationInWorld(*context_);
    ...
    const auto& A_WB_all =
            plant_->get_body_spatial_accelerations_output_port().Eval<std::vector<SpatialAcceleration<double>>>(
                    *context_);
    ASSERT_EQ(A_WB_all.size(), plant_->num_bodies());
    const SpatialAcceleration<double>& A_WE_W_expected3 = A_WB_all[end_effector_link_->index()];
    EXPECT_EQ(A_WE_W.get_coeffs(), A_WE_W_expected3.get_coeffs());

    // Also sanity check the deprecated port name (2024-10-01).
    {
        const auto& A_WB_all_deprecated = plant_->GetOutputPort("spatial_accelerations")
    ...
    const SpatialAcceleration<double> A_WH_W = frame_H_->CalcSpatialAccelerationInWorld(*context_);
    const Frame<double>& frame_W = plant_->world_frame();
    const SpatialAcceleration<double> A_WH_W_expected =
            test_utilities::CalcSpatialAccelerationViaAutomaticDifferentiation(*plant_, *context_, *frame_H_, frame_W,
                                                                               frame_W);
    EXPECT_TRUE(CompareMatrices(A_WH_W.get_coeffs(), A_WH_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_EH_W = frame_H_->CalcSpatialAcceleration(*context_, frame_E, frame_W);
    EXPECT_TRUE(CompareMatrices(A_EH_W.get_coeffs(), Vector6<double>::Zero(), kTolerance, MatrixCompareType::relative));
    ...
    const RigidBody<double>& link3 = plant_->GetBodyByName("iiwa_link_3");
    const Frame<double>& frame_L3 = link3.body_frame();
    const SpatialAcceleration<double> A_WL3_W = frame_L3.CalcSpatialAcceleration(*context_, frame_W, frame_W);
    const SpatialAcceleration<double> A_WL3_W_expected = frame_L3.CalcSpatialAccelerationInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(A_WL3_W.get_coeffs(), A_WL3_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_WL3_E = frame_L3.CalcSpatialAcceleration(*context_, frame_W, frame_E);
    const RotationMatrix<double> R_WE = frame_E.CalcRotationMatrixInWorld(*context_);
    const RotationMatrix<double> R_EW = R_WE.inverse();
    const SpatialAcceleration<double> A_WL3_E_expected = R_EW * A_WL3_W_expected;
    EXPECT_TRUE(CompareMatrices(A_WL3_E.get_coeffs(), A_WL3_E_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_HW_E = frame_W.CalcSpatialVelocity(*context_, *frame_H_, frame_E);

    // Verify the rotational part of V_HW_E via the familiar formula ω_WH = -ω_HW.
    const SpatialVelocity<double> V_WH_W = frame_H_->CalcSpatialVelocityInWorld(*context_);
    const Vector3<double>& w_WH_W = V_WH_W.rotational();
    EXPECT_TRUE(CompareMatrices(V_HW_E.rotational(), R_EW * (-w_WH_W), kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(
            CompareMatrices(V_HW_E.translational(), R_EW * v_HWo_W_expected, kTolerance, MatrixCompareType::relative));
    ...
    // spatial acceleration in frame H, expressed in the end-effector frame E.
    const SpatialAcceleration<double> A_HW_E = frame_W.CalcSpatialAcceleration(*context_, *frame_H_, frame_E);

    // Verify the rotational part of A_HW_E via the relationship α_HW = -α_WH.
    const Vector3<double>& alpha_WH_W = A_WH_W.rotational();
    EXPECT_TRUE(CompareMatrices(A_HW_E.rotational(), R_EW * (-alpha_WH_W), kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(
            CompareMatrices(A_HW_E.translational(), R_EW * a_HWo_W_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(KukaIiwaModelTests, FramesCalcRelativeSpatialAcceleration) {
    ...
    // Note: Frame H is a fixed offset frame, fixed to end-effector E.
    const FixedOffsetFrame<double>& frame_H = *frame_H_;
    const SpatialAcceleration<double> A_W_HH_W = frame_H.CalcRelativeSpatialAccelerationInWorld(*context_, frame_H);
    EXPECT_TRUE(
            CompareMatrices(A_W_HH_W.get_coeffs(), Vector6<double>::Zero(), kTolerance, MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_L3_HH_W =
            frame_H.CalcRelativeSpatialAcceleration(*context_, frame_H, frame_L3, frame_W);
    EXPECT_TRUE(
            CompareMatrices(A_L3_HH_W.get_coeffs(), Vector6<double>::Zero(), kTolerance, MatrixCompareType::relative));
    ...
    const SpatialVelocity<double> V_L3H_W = frame_H.CalcSpatialVelocity(*context_, frame_L3, frame_W);
    const SpatialAcceleration<double> A_L3H_W = frame_H.CalcSpatialAcceleration(*context_, frame_L3, frame_W);
    EXPECT_FALSE(
            CompareMatrices(V_L3H_W.rotational(), Vector3<double>::Zero(), kTolerance, MatrixCompareType::absolute));
    EXPECT_FALSE(
            CompareMatrices(V_L3H_W.translational(), Vector3<double>::Zero(), kTolerance, MatrixCompareType::absolute));
    EXPECT_FALSE(
            CompareMatrices(A_L3H_W.rotational(), Vector3<double>::Zero(), kTolerance, MatrixCompareType::absolute));
    EXPECT_FALSE(
            CompareMatrices(A_L3H_W.translational(), Vector3<double>::Zero(), kTolerance, MatrixCompareType::absolute));
    ...
    // First perform this test when the "measured-in" frame is the world frame W.
    // Next, perform this test when the "measured-in" frame is frame_L3.
    const SpatialAcceleration<double> A_W_WH_W = frame_H.CalcRelativeSpatialAccelerationInWorld(*context_, frame_W);
    const SpatialAcceleration<double> A_WH_W = frame_H.CalcSpatialAccelerationInWorld(*context_);
    EXPECT_TRUE(CompareMatrices(A_W_WH_W.get_coeffs(), A_WH_W.get_coeffs(), kTolerance, MatrixCompareType::relative));

    const SpatialAcceleration<double> A_L3_L3H_W =
            frame_H.CalcRelativeSpatialAcceleration(*context_, frame_L3, frame_L3, frame_W);
    EXPECT_TRUE(
            CompareMatrices(A_L3_L3H_W.get_coeffs(), A_L3H_W.get_coeffs(), kTolerance, MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_W_L3H_W = frame_H.CalcRelativeSpatialAccelerationInWorld(*context_, frame_L3);
    const SpatialAcceleration<double> A_WL3_W = frame_L3.CalcSpatialAccelerationInWorld(*context_);
    const SpatialAcceleration<double> A_W_L3H_W_expected = A_WH_W - A_WL3_W;
    EXPECT_TRUE(CompareMatrices(A_W_L3H_W.get_coeffs(), A_W_L3H_W_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_W_L3H_E =
            frame_H.CalcRelativeSpatialAcceleration(*context_, frame_L3, frame_W, frame_E);
    const RotationMatrix<double> R_EW = frame_W.CalcRotationMatrix(*context_, frame_E);
    const SpatialAcceleration<double> A_W_L3H_E_expected = R_EW * A_W_L3H_W_expected;
    EXPECT_TRUE(CompareMatrices(A_W_L3H_E.get_coeffs(), A_W_L3H_E_expected.get_coeffs(), kTolerance,
                                MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_W_HL3_W = frame_L3.CalcRelativeSpatialAccelerationInWorld(*context_, frame_H);
    EXPECT_TRUE(
            CompareMatrices(A_W_L3H_W.get_coeffs(), -A_W_HL3_W.get_coeffs(), kTolerance, MatrixCompareType::relative));

    // With the measured-in-frame being the end-effector frame E, verify that
    // frame H's spatial acceleration relative to frame L3 is the negative of
    // frame L3's spatial acceleration relative to frame H.
    const SpatialAcceleration<double> A_E_HL3_E =
            frame_L3.CalcRelativeSpatialAcceleration(*context_, frame_H, frame_E, frame_E);
    const SpatialAcceleration<double> A_E_L3H_E =
            frame_H.CalcRelativeSpatialAcceleration(*context_, frame_L3, frame_E, frame_E);
    EXPECT_TRUE(
            CompareMatrices(A_E_L3H_E.get_coeffs(), -A_E_HL3_E.get_coeffs(), kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_FALSE(CompareMatrices(DtW_w_L3H_W, alpha_L3H_W, kTolerance, MatrixCompareType::relative));

    // Verify that the rotational part of A_W_L3H_W is:
    // DtW(ω_L3H)_W = α_L3H_W + ω_L3H_W x ω_L3H_W.
    const Vector3<double> w_L3H_W = frame_H.CalcAngularVelocity(*context_, frame_L3, frame_W);
    const Vector3<double> w_WL3_W = frame_L3.EvalAngularVelocityInWorld(*context_);
    const Vector3<double> cross_product_term = w_WL3_W.cross(w_L3H_W);
    EXPECT_TRUE(
            CompareMatrices(DtW_w_L3H_W, alpha_L3H_W + cross_product_term, kTolerance, MatrixCompareType::relative));
    ...
    const SpatialAcceleration<double> A_W_EH_W = frame_H.CalcRelativeSpatialAccelerationInWorld(*context_, frame_E);
    ...
    const Vector3<double> alpha_WE_W = frame_E.CalcSpatialAccelerationInWorld(*context_).rotational();
    const Vector3<double> alphar = alpha_WE_W.cross(p_EoHo_W);
    EXPECT_TRUE(CompareMatrices(a_W_EoHo_W, alphar + wwr, kTolerance, MatrixCompareType::relative));
    ...
    const Vector3<double> v_W_L3H_W = frame_H.CalcRelativeSpatialVelocityInWorld(*context_, frame_L3).translational();
    ...
    const double separation_acceleration1 =
            a_W_L3H_W.dot(u_L3oHo_W) + 1.0 / distance * (v_W_L3H_W_magSquared - separation_speed_squared);
    ...
    const Vector3<double> v_L3_L3H_W =
            frame_H.CalcRelativeSpatialVelocity(*context_, frame_L3, frame_L3, frame_W).translational();
    const double v_L3_L3H_W_magSquared = v_L3_L3H_W.dot(v_L3_L3H_W);
    const double separation_acceleration2 =
            a_L3_L3H_W.dot(u_L3oHo_W) + 1.0 / distance * (v_L3_L3H_W_magSquared - separation_speed_squared);
...
GTEST_TEST(MultibodyPlantTest, FixedWorldKinematics) {
    ...
    const RigidTransformd& X_WM_expected = objects_frame.CalcPoseInWorld(*context) * X_OM;

    // We verify the results.
    EXPECT_TRUE(CompareMatrices(X_WM.GetAsMatrix34(), X_WM_expected.GetAsMatrix34(), kTolerance));

    // Now we evaluate some velocity kinematics.
    const SpatialVelocity<double>& V_WM = mug.EvalSpatialVelocityInWorld(*context);
