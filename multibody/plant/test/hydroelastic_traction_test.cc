...
#include "geometry/proximity/polygon_surface_mesh.h"
#include "geometry/proximity/triangle_surface_mesh.h"
...
const HydroelasticContactRepresentation kTriangle = HydroelasticContactRepresentation::kTriangle;

namespace std {
std::ostream& operator<<(std::ostream& out, drake::geometry::HydroelasticContactRepresentation r) {
...
std::unique_ptr<TriangleSurfaceMesh<double>> CreateTriangleMesh() {
    ...
    auto mesh = std::make_unique<TriangleSurfaceMesh<double>>(std::move(faces), std::move(vertices));

    for (int f = 0; f < mesh->num_triangles(); ++f) {
        // Can't use an ASSERT_TRUE here because it interferes with the return
        // value.
        if (!CompareMatrices(mesh->face_normal(f), -Vector3<double>::UnitZ(), std::numeric_limits<double>::epsilon())) {
...
std::unique_ptr<PolygonSurfaceMesh<double>> CreatePolygonMesh() {
    ...
    auto mesh = std::make_unique<PolygonSurfaceMesh<double>>(std::move(faces), std::move(vertices));
...
GeometryId FindGeometry(const MultibodyPlant<double>& plant, const std::string body_name) {
    const auto& geometries = plant.GetCollisionGeometriesForBody(plant.GetBodyByName(body_name));
...
std::unique_ptr<ContactSurface<double>> CreateContactSurface(
    ...
    if (representation == geometry::HydroelasticContactRepresentation::kTriangle) {
        ...
        for (int i = 0; i < mesh->num_vertices(); ++i) e_MN[i] = -mesh->vertex(i).z();
        ...
        return std::make_unique<ContactSurface<double>>(
                ...
                std::make_unique<MeshFieldLinear<double, TriangleSurfaceMesh<double>>>(std::move(e_MN), mesh_pointer));
    } else {
...
class MultibodyPlantHydroelasticTractionTests
    : public ::testing::TestWithParam<std::tuple<RigidTransform<double>, geometry::HydroelasticContactRepresentation>> {
public:
    const HydroelasticTractionCalculator<double>& traction_calculator() const { return traction_calculator_; }

    const HydroelasticTractionCalculator<double>::Data& calculator_data() { return *calculator_data_; }
    ...
    void set_calculator_data(const RigidTransform<double>& X_WA,
        ...
        calculator_data_ = std::make_unique<HydroelasticTractionCalculator<double>::Data>(X_WA, X_WB, V_WA, V_WB,
                                                                                          &contact_surface());
    }

    const ContactSurface<double>& contact_surface() const { return *contact_surface_; }

    const RigidTransform<double>& GetPose() const { return std::get<0>(GetParam()); }

    geometry::HydroelasticContactRepresentation GetRepresentation() const { return std::get<1>(GetParam()); }
    ...
    // tractions.
    void ComputeSpatialTractionsAtBodyOriginsFromHydroelasticModel(double dissipation,
                                                                   double mu_coulomb,
                                                                   SpatialForce<double>* Ft_Ao_W,
                                                                   SpatialForce<double>* Ft_Bo_W) {
        UpdateCalculatorData();

        // First compute the traction applied to Body A at point Q, expressed in the
        // world frame.
        HydroelasticQuadraturePointData<double> output = traction_calculator().CalcTractionAtCentroid(
        ...
        const geometry::HydroelasticContactRepresentation representation = GetRepresentation();
        ...
        const Vector3<double> p_YQ =
                representation == kTriangle ? Vector3d(-1. / 6.0, 1. / 6.0, -0.5) : Vector3d(0., 0., -0.5);
        const Vector3<double> p_WQ_expected = X_WY * p_YQ;

        // Verify the point of contact.
        for (int i = 0; i < 3; ++i) ASSERT_NEAR(output.p_WQ[i], p_WQ_expected[i], tol()) << i;

        const SpatialForce<double> Ft_Ac_W = traction_calculator().ComputeSpatialTractionAtAcFromTractionAtAq(
    ...
    void SetBoxTranslationalVelocity(const Vector3<double>& v) {
        plant_->SetFreeBodySpatialVelocity(plant_context_, plant_->GetBodyByName("box"),
    ...
    void UpdateCalculatorData() {
        ...
        const geometry::FrameId frameM_id = inspector.GetFrameId(contact_surface_->id_M());
        const geometry::FrameId frameN_id = inspector.GetFrameId(contact_surface_->id_N());
        ...
        const math::RigidTransform<double> X_WA = plant_->EvalBodyPoseInWorld(*plant_context_, bodyA);
        const math::RigidTransform<double> X_WB = plant_->EvalBodyPoseInWorld(*plant_context_, bodyB);

        // Get the spatial velocities for the two bodies (at the body frames).
        const SpatialVelocity<double> V_WA = plant_->EvalBodySpatialVelocityInWorld(*plant_context_, bodyA);
        const SpatialVelocity<double> V_WB = plant_->EvalBodySpatialVelocityInWorld(*plant_context_, bodyB);

        // (Re)-initialize the traction calculator data.
        set_calculator_data(X_WA, X_WB, V_WA, V_WB);
    }

    void ComputeSpatialForcesAtBodyOriginsFromHydroelasticModel(double dissipation,
                                                                double mu_coulomb,
        ...
        traction_calculator().ComputeSpatialForcesAtCentroidFromHydroelasticModel(
                calculator_data(), dissipation, mu_coulomb, &quadrature_point_data, &F_Ac_W);

        traction_calculator().ShiftSpatialForcesAtCentroidToBodyOrigins(calculator_data(), F_Ac_W, F_Ao_W, F_Bo_W);
    }
...
private:
    void SetUp() override {
        ...
        Parser(&plant, scene_graph).AddModelsFromUrl("package://drake/multibody/plant/test/block_on_halfspace.sdf");
        ...
        plant_context_ = &diagram_->GetMutableSubsystemContext(plant, context_.get());
        ...
        contact_surface_ = CreateContactSurface(halfspace_id, block_id, X_WH, GetRepresentation());
    }
    ...
    std::unique_ptr<HydroelasticTractionCalculator<double>::Data> calculator_data_;
};
...
TEST_P(MultibodyPlantHydroelasticTractionTests, VanillaTraction) {
    ...
    ComputeSpatialTractionsAtBodyOriginsFromHydroelasticModel(dissipation, mu_coulomb, &Ft_Ao_W, &Ft_Bo_W);
    ...
    const Vector3d p_BoCo_Y =
            GetRepresentation() == kTriangle ? Vector3d(-1. / 6., 1. / 6., -.5) : Vector3d(0., 0., -0.5);
    ...
    // opposite.
    EXPECT_NEAR((Ft_Bo_W.translational() + Ft_Ao_W.translational()).norm(), 0, tol());
}
...
TEST_P(MultibodyPlantHydroelasticTractionTests, TractionWithFriction) {
    ...
    const double regularization_scalar = traction_calculator().regularization_scalar();
...
TEST_P(MultibodyPlantHydroelasticTractionTests, TractionWithDissipation) {
    ...
    SetBoxTranslationalVelocity(R_WY * Vector3<double>(0, 0, separating_velocity));
    ...
    const double normal_traction_magnitude = field_value + damping_traction_magnitude;
...
TEST_P(MultibodyPlantHydroelasticTractionTests, VanillaTractionOverPatch) {
    ...
    ComputeSpatialForcesAtBodyOriginsFromHydroelasticModel(dissipation, mu_coulomb, &F_Ao_W, &F_Bo_W);
...
public:
    ...
    static HydroelasticQuadraturePointData<T> CalcTractionAtQHelper(
        ...
        return calculator.CalcTractionAtQHelper(data, face_index, e, nhat_W, dissipation, mu_coulomb, p_WQ);
    }

    template <typename T>
    static T CalcAtanXOverXFromXSquared(const T& x_squared) {
        return HydroelasticTractionCalculator<T>::CalcAtanXOverXFromXSquared(x_squared);
    }
};

GTEST_TEST(HydroelasticTractionCalculatorTest, CalcAtanXOverXFromXSquared_AutoDiffXd) {
    using std::atan;
    ...
    // Drake issue #15029.
    const std::vector<double> x_samples = {0.0, 1e-6, 1e-4, 0.119999, 0.12, 0.120001, 1.0};
    ...
    std::vector<double> dfdx_expected_precision(x_samples.size(), std::numeric_limits<double>::epsilon());
    ...
    for (size_t i = 0; i < x_samples.size(); ++i) {
        ...
        const AutoDiffXd fx = HydroelasticTractionCalculatorTester::CalcAtanXOverXFromXSquared<AutoDiffXd>(x2);
...
GTEST_TEST(HydroelasticTractionCalculatorTest, CalcTractionAtQHelperDerivatives) {
    HydroelasticTractionCalculator<AutoDiffXd> calculator;

    RigidTransform<AutoDiffXd> X_WA(math::InitializeAutoDiff(Vector3<double>(0, 0, 1)));
    ...
    const Vector3<AutoDiffXd> p_WC = (X_WA.translation() + X_WB.translation()) / 2;
    ...
    auto mesh_W = std::make_unique<geometry::TriangleSurfaceMesh<AutoDiffXd>>(std::move(faces), std::move(vertices));
    ...
    auto field = std::make_unique<geometry::TriangleSurfaceMeshFieldLinear<AutoDiffXd, AutoDiffXd>>(
    ...
    geometry::ContactSurface<AutoDiffXd> surface(idA, idB, std::move(mesh_W), std::move(field));
    ...
    // tricky case.
    SpatialVelocity<AutoDiffXd> V_WA{Vector3<AutoDiffXd>::Zero(), Vector3<AutoDiffXd>::Zero()};
    SpatialVelocity<AutoDiffXd> V_WB(V_WA);
    HydroelasticTractionCalculator<AutoDiffXd>::Data data(X_WA, X_WB, V_WA, V_WB, &surface);

    auto point_data = HydroelasticTractionCalculatorTester::CalcTractionAtQHelper<AutoDiffXd>(
    ...
    const Matrix3<double> grad_traction_Aq_W = math::ExtractGradient(point_data.traction_Aq_W);
...
class HydroelasticReportingTests : public ::testing::TestWithParam<RigidTransform<double>> {
public:
    const ContactSurface<double>& contact_surface() const { return *contact_surface_; }
    ...
    // origin.
    double pressure(const Vector3<double>& P_YQ) const { return pressure_field_normal().dot(P_YQ); }

    // Gets the normal to the pressure field in Frame Y.
    Vector3<double> pressure_field_normal() const { return Vector3<double>(-1, -1, -1).normalized(); }
...
private:
    void SetUp() override {
        ...
        std::vector<double> e_MN(mesh->num_vertices());
        for (int i = 0; i < mesh->num_vertices(); ++i) e_MN[i] = pressure(mesh->vertex(i));
        ...
        const SpatialVelocity<double> V_YA(Vector3<double>(0, 0, spin_rate), Vector3<double>::Zero());
        const SpatialVelocity<double> V_YB(Vector3<double>::Zero(), Vector3<double>::Zero());
...
const RigidTransform<double> poses[] = {
        ...
        RigidTransform<double>(math::RotationMatrix<double>::MakeYRotation(M_PI_4), drake::Vector3<double>(1, 2, 3))};

const geometry::HydroelasticContactRepresentation representations[] = {
        geometry::HydroelasticContactRepresentation::kTriangle, geometry::HydroelasticContactRepresentation::kPolygon};

INSTANTIATE_TEST_SUITE_P(TractionTestsInstantiations,
                         MultibodyPlantHydroelasticTractionTests,
                         ::testing::Combine(::testing::ValuesIn(poses), ::testing::ValuesIn(representations)));
...
SpatialForce<double> MakeSpatialForce() {
    return SpatialForce<double>(Vector3<double>(1, 2, 3), Vector3<double>(4, 5, 6));
}
...
HydroelasticContactInfo<double> CreateContactInfo(std::unique_ptr<ContactSurface<double>>* contact_surface,
    ...
    *contact_surface = CreateContactSurface(arbitrary_id, arbitrary_id, RigidTransform<double>::Identity());

    // Create the HydroelasticContactInfo using particular spatial force and
    // quadrature point data.
    std::vector<HydroelasticQuadraturePointData<double>> quadrature_point_data = GetQuadraturePointData();
    return HydroelasticContactInfo<double>(contact_surface->get(), MakeSpatialForce(),
...
GTEST_TEST(HydroelasticContactInfo, CopyConstruction) {
    ...
    std::unique_ptr<HydroelasticContactInfo<double>> contact_info;
    HydroelasticContactInfo<double> copy = CreateContactInfo(&contact_surface, &contact_info);
...
GTEST_TEST(HydroelasticContactInfo, MoveConstruction) {
    ...
    // Verify that the spatial force was copied.
    EXPECT_EQ(moved_copy.F_Ac_W().translational(), MakeSpatialForce().translational());
