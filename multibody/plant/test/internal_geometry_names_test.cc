...
public:
    void SetUp() override {
        ...
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder, 0.001);
        ...
        const std::string bin_url = "package://drake_models/manipulation_station/bin.sdf";
        parser.AddModelsFromUrl(bin_url);

        // Two identical models (each one has a single body, single geometry).
        const std::string sphere = "package://drake_models/manipulation_station/sphere.sdf";
    ...
    const std::vector<GeometryId>& GetGeometryIds(std::string_view body_name,
    ...
    GeometryId GetSoleGeometryId(std::string_view body_name, std::optional<std::string_view> model_name = {}) const {
