...
MultibodyPlant<double>& PopulateTestDiagram(DiagramBuilder<double>* builder,
    ...
    if (is_connected) {
        auto [plant_ref, scene_graph] = AddMultibodyPlantSceneGraph(builder, time_step);
        plant = &plant_ref;
    } else {
...
void EvalContactResults(const MultibodyPlant<double>& plant, const Context<double>& context) {
    plant.get_contact_results_output_port().Eval<ContactResults<double>>(context);
}

void EvalTimeDerivatives(const MultibodyPlant<double>& plant, const Context<double>& context) {
    auto derivatives = plant.AllocateTimeDerivatives();
    plant.CalcTimeDerivatives(context, derivatives.get());
}

void EvalTimeDerivativesResidual(const MultibodyPlant<double>& plant, const Context<double>& context) {
    auto derivatives = plant.AllocateTimeDerivatives();
...
void EvalForwardDynamics(const MultibodyPlant<double>& plant, const Context<double>& context) {
    plant.EvalForwardDynamics(context);
}

void EvalGeneralizedContactForces(const MultibodyPlant<double>& plant, const Context<double>& context) {
    plant.get_generalized_contact_forces_output_port(ModelInstanceIndex(1)).Eval(context);
}

void EvalReactionForces(const MultibodyPlant<double>& plant, const Context<double>& context) {
...
struct TestConfiguration {
    ...
    std::function<void(const MultibodyPlant<double>& plant, const Context<double>& context)> eval;
    double time_step{};
    bool has_collision_geometry{};
    bool connected{};
...
class MultibodySceneGraphConnectionTest : public testing::TestWithParam<TestConfiguration> {};
...
TEST_P(MultibodySceneGraphConnectionTest, ConnectionError) {
    ...
    const MultibodyPlant<double>& plant = PopulateTestDiagram(&builder, time_step, has_collision_geometry, connected);
    ...
    if (expect_throw) {
        DRAKE_EXPECT_THROWS_MESSAGE(
                ...
                fmt::format(".*{}[^]+#mbp-unconnected-query-object-port for help.", config.key_phrase));
    } else {
...
std::vector<TestConfiguration> MakeTestConfigurations() {
    std::vector<TestConfiguration> ref_configurations{
            ...
            {.key_phrase = "'DefaultModelInstance_generalized_contact_forces'", .eval = &EvalGeneralizedContactForces},
            {.key_phrase = "'reaction_forces'", .eval = &EvalReactionForces},
            // Time derivatives are only meaningful in continuous mode.
            {.key_phrase = "time derivatives", .eval = &EvalTimeDerivatives, .modes = PlantMode::kContinuous},
            ...
            {.key_phrase = "time derivatives", .eval = &EvalForwardDynamics, .modes = PlantMode::kContinuous},
            {.key_phrase = "discrete forward dynamics", .eval = &EvalForwardDynamics, .modes = PlantMode::kDiscrete},
    };
        ...
        for (double time_step : {1e-3, 0.0}) {
            ...
            for (bool has_collision_geometry : {true, false}) {
                for (bool connected : {true, false}) {
                    configurations.push_back({.key_phrase = ref_config.key_phrase,
...
INSTANTIATE_TEST_SUITE_P(Suite, MultibodySceneGraphConnectionTest, testing::ValuesIn(MakeTestConfigurations()));
