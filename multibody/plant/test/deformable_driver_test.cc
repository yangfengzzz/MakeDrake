...
protected:
    ...
    void SetUp() override {
        ...
        const RigidBody<double>& body =
                plant_->AddRigidBody("rigid_body", SpatialInertia<double>::SolidSphereWithMass(1.0, 1.0));
    ...
    /* Forwarding calls to private member functions in DeformableDriver with the
     same name.
     @{ */
    const FemState<double>& EvalFemState(const systems::Context<double>& context, DeformableBodyIndex index) const {
...
private:
    DeformableBodyId RegisterSphere(DeformableModel<double>* model, double resolution_hint) {
        auto geometry = make_unique<GeometryInstance>(RigidTransformd(), make_unique<Sphere>(1), "sphere");
        geometry::ProximityProperties props;
        geometry::AddContactMaterial({}, {}, CoulombFriction<double>(1.0, 1.0), &props);
        ...
        DeformableBodyId body_id = model->RegisterDeformableBody(std::move(geometry), body_config, resolution_hint);
        return body_id;
    }
};
...
TEST_F(DeformableDriverTest, FemState) {
    ...
    state_value << q, v, a;
    const systems::DiscreteStateIndex state_index = model_->GetDiscreteStateIndex(body_id_);
    plant_context_->SetDiscreteState(state_index, state_value);
    const FemState<double>& fem_state = EvalFemState(*plant_context_, DeformableBodyIndex(0));
...
TEST_F(DeformableDriverTest, FreeMotionFemState) {
    ...
    const FemState<double>& free_motion_fem_state = EvalFreeMotionFemState(*plant_context_, DeformableBodyIndex(0));
    ...
    EXPECT_TRUE(CompareMatrices(free_motion_fem_state.GetPositions(), next_q, kTol * kDt));
    EXPECT_TRUE(CompareMatrices(free_motion_fem_state.GetVelocities(), next_v, kTol));
    EXPECT_TRUE(CompareMatrices(free_motion_fem_state.GetAccelerations(), next_a, kTol / kDt));
}

TEST_F(DeformableDriverTest, NextFemState) {
    const FemState<double>& free_motion_fem_state = EvalFreeMotionFemState(*plant_context_, DeformableBodyIndex(0));
    const FemState<double>& next_fem_state = EvalNextFemState(*plant_context_, DeformableBodyIndex(0));
    /* The next state should be the same as the free motion state in the absence
     of contact or constraints. */
    EXPECT_EQ(free_motion_fem_state.GetPositions(), next_fem_state.GetPositions());
    EXPECT_EQ(free_motion_fem_state.GetVelocities(), next_fem_state.GetVelocities());
    EXPECT_EQ(free_motion_fem_state.GetAccelerations(), next_fem_state.GetAccelerations());
}
...
TEST_F(DeformableDriverTest, CalcDiscreteStates) {
    ...
    const systems::DiscreteStateIndex state_index = model_->GetDiscreteStateIndex(body_id_);
    const VectorX<double>& discrete_state = plant_context_->get_discrete_state(state_index).value();
    const int num_dofs = model_->GetFemModel(body_id_).num_dofs();
    const FemState<double>& fem_state = EvalFemState(*plant_context_, DeformableBodyIndex(0));
    EXPECT_EQ(discrete_state.head(num_dofs), fem_state.GetPositions());
    EXPECT_EQ(discrete_state.segment(num_dofs, num_dofs), fem_state.GetVelocities());
