...
public:
    template <typename T>
    static const internal::MultibodyTreeTopology& topology(const CompliantContactManager<T>& manager) {
        return manager.tree_topology();
    }

    template <typename T>
    static BodyIndex FindBodyByGeometryId(const CompliantContactManager<T>& manager, geometry::GeometryId id) {
        return manager.FindBodyByGeometryId(id);
    }

    template <typename T>
    static const GeometryContactData<T>& EvalGeometryContactData(const CompliantContactManager<T>& manager,
    ...
    static void CalcNonContactForces(const CompliantContactManager<T>& manager,
        ...
        manager.CalcNonContactForces(context, include_joint_limit_penalty_forces, include_pd_controlled_input, forces);
    }

    template <typename T>
    static const SapDriver<T>& sap_driver(const CompliantContactManager<T>& manager) {
        DRAKE_DEMAND(manager.sap_driver_ != nullptr);
        return *manager.sap_driver_;
    }

    template <typename T>
    static const TamsiDriver<T>& tamsi_driver(const CompliantContactManager<T>& manager) {
    ...
    static Eigen::MatrixX<T> CalcDenseJacobianMatrixInContactFrame(
        ...
        for (int i = 0; i < nc; ++i) {
            ...
            for (const typename DiscreteContactPair<T>::JacobianTreeBlock& tree_jacobian : contact_pair.jacobian) {
                ...
                J_AcBc_C.block(row_offset, col_offset, 3, tree_nv) = tree_jacobian.J.MakeDenseMatrix();
            }
        }
        return J_AcBc_C;
    }
