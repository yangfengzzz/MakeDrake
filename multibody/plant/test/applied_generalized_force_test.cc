...
class MultibodyPlantGeneralizedAppliedForceTest : public ::testing::TestWithParam<double> {
public:
    void SetUp() override {
        ...
        plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", iiwa1));
        plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", iiwa2));
        plant_->Finalize();
        ...
        builder.Connect(id_controller->get_output_port_control(), plant_->get_applied_generalized_force_input_port());
        builder.Connect(plant_->get_state_output_port(), id_controller->get_input_port_estimated_state());
        builder.Connect(plant_->get_state_output_port(), id_controller->get_input_port_desired_state());

        // Plug the actuator inputs.
        auto u1_source =
                builder.AddSystem<ConstantVectorSource<double>>(VectorX<double>::Zero(plant_->num_velocities(iiwa1)));
        auto u2_source =
                builder.AddSystem<ConstantVectorSource<double>>(VectorX<double>::Zero(plant_->num_velocities(iiwa2)));
        builder.Connect(u1_source->get_output_port(), plant_->get_actuation_input_port(iiwa1));
        builder.Connect(u2_source->get_output_port(), plant_->get_actuation_input_port(iiwa2));
...
TEST_P(MultibodyPlantGeneralizedAppliedForceTest, CheckGeneralizedAppliedForceInput) {
    ...
    VectorBase<double>& plant_state = (continuous_system) ? plant_context.get_mutable_continuous_state_vector()
                                                          : plant_context.get_mutable_discrete_state_vector();
    for (int i = 0; i < plant().num_positions(); ++i) plant_state[0] = 0.1 * (i + 1);
    ...
    if (continuous_system) {
        ...
        const VectorBase<double>& new_discrete_state_vector = new_discrete_state->get_vector();
        diagram().CalcForcedDiscreteVariableUpdate(context(), new_discrete_state.get());

        // Velocities start immediately after positions in the discrete state
        // vector.
        for (int i = plant().num_positions(); i < new_discrete_state_vector.size(); ++i)
            EXPECT_NEAR(new_discrete_state_vector[i] - discrete_state_vector[i], 0.0, eps);
    }
}

INSTANTIATE_TEST_SUITE_P(GravityCompensationTest,
                         MultibodyPlantGeneralizedAppliedForceTest,
                         ::testing::Values(0.0 /* continuous-time MBP */, 1e-3 /* time stepping MBP */));
