...
public:
    ...
    void VerifyMassMatrixComputation(const Context<double>& context) {
        ...
        // matrix.
        const double kTolerance = 10.0 * std::numeric_limits<double>::epsilon() * Mcba.norm() / plant_.num_velocities();
        EXPECT_TRUE(CompareMatrices(Mcba, Mid, kTolerance, MatrixCompareType::relative));
    }
...
TEST_F(MultibodyPlantMassMatrixTests, IiwaRobot) {
    LoadUrl("package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf");
    ...
    const VectorX<double> q0 = VectorX<double>::LinSpaced(plant_.num_positions(), 1, plant_.num_positions());
