#include "multibody/plant/sap_driver.h"
...
#include "multibody/contact_solvers/contact_solver_utils.h"
#include "multibody/contact_solvers/sap/sap_ball_constraint.h"
#include "multibody/contact_solvers/sap/sap_contact_problem.h"
#include "multibody/contact_solvers/sap/sap_coupler_constraint.h"
#include "multibody/contact_solvers/sap/sap_distance_constraint.h"
#include "multibody/contact_solvers/sap/sap_fixed_constraint.h"
#include "multibody/contact_solvers/sap/sap_friction_cone_constraint.h"
#include "multibody/contact_solvers/sap/sap_holonomic_constraint.h"
#include "multibody/contact_solvers/sap/sap_hunt_crossley_constraint.h"
#include "multibody/contact_solvers/sap/sap_limit_constraint.h"
#include "multibody/contact_solvers/sap/sap_pd_controller_constraint.h"
#include "multibody/contact_solvers/sap/sap_solver.h"
#include "multibody/contact_solvers/sap/sap_solver_results.h"
#include "multibody/contact_solvers/sap/sap_weld_constraint.h"
#include "multibody/plant/compliant_contact_manager.h"
...
template <typename T>
SapDriver<T>::SapDriver(const CompliantContactManager<T>* manager, double near_rigid_threshold)
...
template <typename T>
void SapDriver<T>::set_sap_solver_parameters(const contact_solvers::internal::SapSolverParameters& parameters) {
    sap_parameters_ = parameters;
}

template <typename T>
void SapDriver<T>::DeclareCacheEntries(CompliantContactManager<T>* mutable_manager) {
    ...
    const auto& contact_problem_cache_entry = mutable_manager->DeclareCacheEntry(
            ...
            // choose to depend on.
            {xd_ticket, inputs_ticket, parameters_ticket, systems::System<T>::time_ticket(),
             systems::System<T>::accuracy_ticket()});
    contact_problem_ = contact_problem_cache_entry.cache_index();

    const auto& sap_solver_results_cache_entry = mutable_manager->DeclareCacheEntry(
            "SAP solver results", systems::ValueProducer(this, &SapDriver<T>::CalcSapSolverResults),
...
template <typename T>
const SapSolverResults<T>& SapDriver<T>::EvalSapSolverResults(const systems::Context<T>& context) const {
    return plant().get_cache_entry(sap_results_).template Eval<SapSolverResults<T>>(context);
}

template <typename T>
const ContactProblemCache<T>& SapDriver<T>::EvalContactProblemCache(const systems::Context<T>& context) const {
    return plant().get_cache_entry(contact_problem_).template Eval<ContactProblemCache<T>>(context);
}

template <typename T>
void SapDriver<T>::CalcLinearDynamicsMatrix(const systems::Context<T>& context, std::vector<MatrixX<T>>* A) const {
...
template <typename T>
void SapDriver<T>::CalcFreeMotionVelocities(const systems::Context<T>& context, VectorX<T>* v_star) const {
    ...
    const VectorX<T>& vdot0 = manager().EvalAccelerationsDueToNonConstraintForcesCache(context).get_vdot();
    const double dt = this->plant().time_step();
    const VectorX<T>& x0 = context.get_discrete_state(manager().multibody_state_index()).value();
    const auto v0 = x0.bottomRows(this->plant().num_velocities());
    if constexpr (std::is_same_v<T, double>) {
        if (manager().deformable_driver() != nullptr) {
            const VectorX<T>& deformable_v_star =
                    manager().deformable_driver()->EvalParticipatingFreeMotionVelocities(context);
...
template <typename T>
std::vector<RotationMatrix<T>> SapDriver<T>::AddContactConstraints(const systems::Context<T>& context,
                                                                   SapContactProblem<T>* problem) const {
    DRAKE_DEMAND(problem != nullptr);
    DRAKE_DEMAND(plant().get_discrete_contact_approximation() != DiscreteContactApproximation::kTamsi);
    ...
    const DiscreteContactData<DiscreteContactPair<T>>& contact_pairs = manager().EvalDiscreteContactPairs(context);
    ...
    for (int icontact = 0; icontact < num_contacts; ++icontact) {
        ...
        auto make_sap_parameters = [&]() {
            return typename SapFrictionConeConstraint<T>::Parameters{friction, stiffness, dissipation_time_scale, beta,
                                                                     sigma};
        };
        ...
        auto make_hunt_crossley_parameters = [&]() {
            ...
            return typename SapHuntCrossleyConstraint<T>::Parameters{model, friction, stiffness, damping, vs, sigma};
        };

        R_WC.push_back(pair.R_WC);

        if (jacobian_blocks.size() == 1) {
            SapConstraintJacobian<T> J(jacobian_blocks[0].tree, std::move(jacobian_blocks[0].J));
            if (plant().get_discrete_contact_approximation() == DiscreteContactApproximation::kSap) {
                problem->AddConstraint(std::make_unique<SapFrictionConeConstraint<T>>(
                        MakeContactConfiguration(pair), std::move(J), make_sap_parameters()));
            } else {
                problem->AddConstraint(std::make_unique<SapHuntCrossleyConstraint<T>>(
                        MakeContactConfiguration(pair), std::move(J), make_hunt_crossley_parameters()));
            }
        } else {
            SapConstraintJacobian<T> J(jacobian_blocks[0].tree, std::move(jacobian_blocks[0].J),
...
void SapDriver<T>::AddLimitConstraints(const systems::Context<T>& context,
    ...
    for (JointIndex joint_index : plant().GetJointIndices()) {
        ...
        if (joint.num_positions() == 1 && joint.num_velocities() == 1) {
            ...
            const TreeIndex tree_index = tree_topology().velocity_to_tree_index(velocity_start);
            const int tree_nv = tree_topology().num_tree_velocities(tree_index);
            const int tree_velocity_start = tree_topology().tree_velocities_start_in_v(tree_index);
            ...
            if (!std::isinf(ql) || !std::isinf(qu)) {
                ...
                typename SapLimitConstraint<T>::Parameters parameters{ql, qu, stiffness, dissipation_time_scale, kBeta};
                problem->AddConstraint(std::make_unique<SapLimitConstraint<T>>(tree_index, tree_dof, tree_nv, q0,
                                                                               std::move(parameters)));
            }
        } else {
            ...
            if ((lower_limits.array() != -kInf).any() || (upper_limits.array() != kInf).any()) {
...
template <typename T>
void SapDriver<T>::AddCouplerConstraints(const systems::Context<T>& context, SapContactProblem<T>* problem) const {
    DRAKE_DEMAND(problem != nullptr);
    ...
    for (const auto& [id, info] : manager().coupler_constraints_specs()) {
        ...
        const int tree_dof0 = dof0 - tree_topology().tree_velocities_start_in_v(tree0);
        const int tree_dof1 = dof1 - tree_topology().tree_velocities_start_in_v(tree1);
        ...
        problem->AddConstraint(std::make_unique<SapCouplerConstraint<T>>(std::move(kinematics)));
    }
}

template <typename T>
void SapDriver<T>::AddDistanceConstraints(const systems::Context<T>& context, SapContactProblem<T>* problem) const {
    DRAKE_DEMAND(problem != nullptr);
    ...
    for (const auto& [id, spec] : manager().distance_constraints_specs()) {
        ...
        const math::RigidTransform<T>& X_WA = plant().EvalBodyPoseInWorld(context, body_A);
        const math::RigidTransform<T>& X_WB = plant().EvalBodyPoseInWorld(context, body_B);
        ...
        manager().internal_tree().CalcJacobianTranslationalVelocity(
                context, JacobianWrtVariable::kV, body_A.body_frame(), frame_W, p_WP, frame_W, frame_W, &Jv_WAp_W);
        manager().internal_tree().CalcJacobianTranslationalVelocity(
                context, JacobianWrtVariable::kV, body_B.body_frame(), frame_W, p_WQ, frame_W, frame_W, &Jv_WBq_W);
        Jv_ApBq_W = Jv_WBq_W - Jv_WAp_W;

        // Jacobian for the relative velocity v_PQ_W, as required by
        // SapDistanceConstraint.
        auto make_constraint_jacobian = [this, &Jv_ApBq_W](BodyIndex bodyA, BodyIndex bodyB) {
            ...
            // world.
            const bool single_tree = !treeA_has_dofs || !treeB_has_dofs || treeA_index == treeB_index;

            if (single_tree) {
                const TreeIndex tree_index = treeA_has_dofs ? treeA_index : treeB_index;
                MatrixX<T> Jtree = Jv_ApBq_W.middleCols(tree_topology().tree_velocities_start_in_v(tree_index),
                                                        tree_topology().num_tree_velocities(tree_index));
                return SapConstraintJacobian<T>(tree_index, std::move(Jtree));
            } else {
                MatrixX<T> JA = Jv_ApBq_W.middleCols(tree_topology().tree_velocities_start_in_v(treeA_index),
                                                     tree_topology().num_tree_velocities(treeA_index));
                MatrixX<T> JB = Jv_ApBq_W.middleCols(tree_topology().tree_velocities_start_in_v(treeB_index),
                                                     tree_topology().num_tree_velocities(treeB_index));
                return SapConstraintJacobian<T>(treeA_index, std::move(JA), treeB_index, std::move(JB));
            }
        };
        ...
        const typename SapDistanceConstraint<T>::ComplianceParameters parameters(spec.stiffness, spec.damping);

        problem->AddConstraint(
                std::make_unique<SapDistanceConstraint<T>>(std::move(kinematics), std::move(parameters)));
    }
}

template <typename T>
void SapDriver<T>::AddBallConstraints(const systems::Context<T>& context,
    ...
    for (const auto& [id, spec] : manager().ball_constraints_specs()) {
        ...
        auto make_constraint_jacobian = [this, &Jv_ApBq_W, &body_A, &body_B]() {
            const TreeIndex treeA_index = tree_topology().body_to_tree_index(body_A.index());
            const TreeIndex treeB_index = tree_topology().body_to_tree_index(body_B.index());
            const bool treeA_has_dofs = tree_topology().tree_has_dofs(treeA_index);
            const bool treeB_has_dofs = tree_topology().tree_has_dofs(treeB_index);
        ...
        const typename SapBallConstraint<T>::Kinematics kinematics(spec.body_A, p_WP, p_AP_W, spec.body_B, p_WQ, p_BQ_W,
                                                                   make_constraint_jacobian());

        problem->AddConstraint(std::make_unique<SapBallConstraint<T>>(std::move(kinematics)));
    }
}
...
void SapDriver<T>::AddWeldConstraints(const systems::Context<T>& context,
    ...
    for (const auto& [id, spec] : manager().weld_constraints_specs()) {
        ...
        const Vector3<T> p_AP_W = X_WA.rotation() * spec.X_AP.translation().template cast<T>();
        const Vector3<T> p_BQ_W = X_WB.rotation() * spec.X_BQ.translation().template cast<T>();
        ...
        // Dense Jacobian.
        manager().internal_tree().CalcJacobianSpatialVelocity(context, JacobianWrtVariable::kV, body_A.body_frame(),
                                                              p_AM, frame_W, frame_W, &J_WAm);
        manager().internal_tree().CalcJacobianSpatialVelocity(context, JacobianWrtVariable::kV, body_B.body_frame(),
                                                              p_BM, frame_W, frame_W, &J_WBm);
        ...
        auto make_constraint_jacobian = [this, &J_W_AmBm](const RigidBody<T>& bodyA, const RigidBody<T>& bodyB) {
            const TreeIndex treeA_index = tree_topology().body_to_tree_index(bodyA.index());
            const TreeIndex treeB_index = tree_topology().body_to_tree_index(bodyB.index());
            const bool treeA_has_dofs = tree_topology().tree_has_dofs(treeA_index);
            const bool treeB_has_dofs = tree_topology().tree_has_dofs(treeB_index);
            ...
            if (single_tree) {
                const TreeIndex tree_index = treeA_has_dofs ? treeA_index : treeB_index;
                MatrixX<T> Jtree_W = J_W_AmBm.middleCols(tree_topology().tree_velocities_start_in_v(tree_index),
                                                         tree_topology().num_tree_velocities(tree_index));
                return SapConstraintJacobian<T>(tree_index, std::move(Jtree_W));
            } else {
                MatrixX<T> JA_W = J_W_AmBm.middleCols(tree_topology().tree_velocities_start_in_v(treeA_index),
                                                      tree_topology().num_tree_velocities(treeA_index));
                MatrixX<T> JB_W = J_W_AmBm.middleCols(tree_topology().tree_velocities_start_in_v(treeB_index),
                                                      tree_topology().num_tree_velocities(treeB_index));
                return SapConstraintJacobian<T>(treeA_index, std::move(JA_W), treeB_index, std::move(JB_W));
            }
        };

        const typename SapWeldConstraint<T>::Kinematics kinematics(spec.body_A, X_WP, p_AP_W, spec.body_B, X_WQ, p_BQ_W,
                                                                   make_constraint_jacobian(body_A, body_B));

        problem->AddConstraint(std::make_unique<SapWeldConstraint<T>>(std::move(kinematics)));
    }
}

template <typename T>
void SapDriver<T>::AddPdControllerConstraints(const systems::Context<T>& context, SapContactProblem<T>* problem) const {
    DRAKE_DEMAND(problem != nullptr);
    ...
    const VectorX<T> feed_forward_actuation = manager_->AssembleActuationInput(context);
    ...
    for (JointActuatorIndex actuator_index : plant().GetJointActuatorIndices()) {
        ...
        if (actuator.has_controller()) {
            ...
            if (!joint.is_locked(context)) {
                ...
                typename SapPdControllerConstraint<T>::Parameters parameters{Kp, Kd, effort_limit};
                typename SapPdControllerConstraint<T>::Configuration configuration{tree, tree_dof, tree_nv, q0,
                                                                                   qd,   vd,       u0};

                problem->AddConstraint(std::make_unique<SapPdControllerConstraint<T>>(std::move(configuration),
                                                                                      std::move(parameters)));
            }
        }
    }
...
template <typename T>
void SapDriver<T>::AddFixedConstraints(const systems::Context<T>& context,
                                       contact_solvers::internal::SapContactProblem<T>* problem) const {
    DRAKE_DEMAND(problem != nullptr);
    if constexpr (std::is_same_v<T, double>) {
        if (manager().deformable_driver() != nullptr) {
            std::vector<FixedConstraintKinematics<T>> kinematics;
            manager().deformable_driver()->AppendDeformableRigidFixedConstraintKinematics(context, &kinematics);
            for (FixedConstraintKinematics<T>& k : kinematics) {
                problem->AddConstraint(std::make_unique<SapFixedConstraint<T>>(std::move(k)));
            }
        }
    } else {
...
template <typename T>
void SapDriver<T>::CalcContactProblemCache(const systems::Context<T>& context, ContactProblemCache<T>* cache) const {
    ...
    const int num_deformable_bodies =
            (manager().deformable_driver() == nullptr) ? 0 : manager().deformable_driver()->num_deformable_bodies();
    const int num_objects = num_rigid_bodies + num_deformable_bodies;
    cache->sap_problem = std::make_unique<SapContactProblem<T>>(plant().time_step(), std::move(A), std::move(v_star));
    ...
    cache->num_pd_controller_constraints = problem.num_constraints() - cache->pd_controller_constraints_start;
    ...
    // data.
    const internal::JointLockingCacheData<T>& joint_locking_data = manager().EvalJointLocking(context);
    const std::vector<int>& locked_indices = joint_locking_data.locked_velocity_indices;
    const std::vector<std::vector<int>>& locked_indices_per_tree = joint_locking_data.locked_velocity_indices_per_tree;
    ...
    bool has_locked_dofs =
            std::any_of(locked_indices_per_tree.begin(), locked_indices_per_tree.end(), [](const std::vector<int>& v) {
                return v.size() > 0;
            });

    if (has_locked_dofs) {
        cache->sap_problem_locked =
                std::move(problem.MakeReduced(locked_indices, locked_indices_per_tree, &cache->mapping));
    } else {
...
template <typename T>
void SapDriver<T>::PackContactSolverResults(const systems::Context<T>& context,
    ...
    const Eigen::VectorBlock<const VectorX<T>> contact_impulses = sap_results.gamma.head(3 * num_contacts);
    const Eigen::VectorBlock<const VectorX<T>> contact_velocities = sap_results.vc.head(3 * num_contacts);
    ...
    for (int i = 0; i < num_contacts; ++i) {
        const SapConstraint<T>& c = problem.get_constraint(i);
        {
            ...
            AddCliqueContribution(context, c.first_clique(), tau_clique, &tau_contact);
        }
        ...
        if (c.num_cliques() == 2) {
            ...
            VectorX<T> tau_clique = VectorX<T>::Zero(Jic.cols());
            Jic.TransposeAndMultiplyAndAddTo(impulse, &tau_clique);
            AddCliqueContribution(context, c.second_clique(), tau_clique, &tau_contact);
        }
    }
...
template <typename T>
void SapDriver<T>::AddCliqueContribution(const systems::Context<T>& context,
                                         int clique,
                                         const Eigen::Ref<const VectorX<T>>& clique_values,
                                         EigenPtr<VectorX<T>> values) const {
    if (clique >= tree_topology().num_trees()) {
        const DeformableDriver<double>* deformable_driver = manager().deformable_driver();
        DRAKE_THROW_UNLESS(deformable_driver != nullptr);
        if constexpr (std::is_same_v<T, double>) {
            const int num_deformable_dofs = values->size() - plant().num_velocities();
            Eigen::Ref<VectorX<T>> deformable_values = values->tail(num_deformable_dofs);
            DeformableBodyIndex body_index(clique - tree_topology().num_trees());
            deformable_driver->EvalParticipatingVelocityMultiplexer(context).Demultiplex(&deformable_values,
                                                                                         body_index) += clique_values;
        } else {
...
template <typename T>
void SapDriver<T>::CalcSapSolverResults(const systems::Context<T>& context, SapSolverResults<T>* sap_results) const {
    const ContactProblemCache<T>& contact_problem_cache = EvalContactProblemCache(context);
    ...
    if (has_locked_dofs) {
        const auto& unlocked_indices = manager().EvalJointLocking(context).unlocked_velocity_indices;
        v0 = SelectRows(v0, unlocked_indices);
    }

    if constexpr (std::is_same_v<T, double>) {
        if (manager().deformable_driver() != nullptr) {
            const VectorX<double> deformable_v0 = manager().deformable_driver()->EvalParticipatingVelocities(context);
            const int rigid_dofs = v0.size();
    ...
    if (has_locked_dofs) {
        ...
        status = sap.SolveWithGuess(*contact_problem_cache.sap_problem_locked, v0, &locked_sap_results);
        if (status == SapSolverStatus::kSuccess) {
            sap_problem.ExpandContactSolverResults(contact_problem_cache.mapping, locked_sap_results, sap_results);
        }
    } else {
...
template <typename T>
void SapDriver<T>::CalcContactSolverResults(const systems::Context<T>& context,
                                            contact_solvers::internal::ContactSolverResults<T>* results) const {
    const SapSolverResults<T>& sap_results = EvalSapSolverResults(context);
    const DiscreteContactData<DiscreteContactPair<T>>& contact_pairs = manager().EvalDiscreteContactPairs(context);
    const int num_contacts = contact_pairs.size();
    const ContactProblemCache<T>& contact_problem_cache = EvalContactProblemCache(context);
    PackContactSolverResults(context, *contact_problem_cache.sap_problem, num_contacts, sap_results, results);
}

template <typename T>
void SapDriver<T>::CalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context,
    ...
    manager().CalcNonContactForces(context,
                                   ...
                                   /* include_pd_controlled_input */ false, forces);
    ...
    const ContactProblemCache<T>& contact_problem_cache = EvalContactProblemCache(context);
    contact_problem_cache.sap_problem->CalcConstraintMultibodyForces(gamma, &constraints_generalized_forces,
...
template <typename T>
void SapDriver<T>::CalcActuation(const systems::Context<T>& context, VectorX<T>* actuation) const {
    ...
    *actuation = manager().AssembleActuationInput(context);
    ...
    const int num_pd_constraints = contact_problem_cache.num_pd_controller_constraints;
