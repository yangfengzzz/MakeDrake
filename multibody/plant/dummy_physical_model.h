...
public:
    ...
    explicit DummyPhysicalModel(MultibodyPlant<T>* plant) : PhysicalModel<T>(plant) {}
    ...
    const systems::OutputPort<T>& GetSceneGraphPortOrThrow() const {
        if (scene_graph_output_port_ == nullptr) {
            throw std::runtime_error("The SceneGraph output port has not been declared.");
        }
...
private:
    ...
    PhysicalModelPointerVariant<T> DoToPhysicalModelPointerVariant() const final { return std::monostate(); }
    ...
    std::unique_ptr<PhysicalModel<ScalarType>> CloneImpl(MultibodyPlant<ScalarType>* plant) const {
        ...
        for (size_t i = 0; i < discrete_states_.size(); ++i) {
            clone->discrete_states_[i] =
                    this->discrete_states_[i].unaryExpr(systems::scalar_conversion::ValueConverter<ScalarType, T>{});
        }
        return clone;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::DummyPhysicalModel);
