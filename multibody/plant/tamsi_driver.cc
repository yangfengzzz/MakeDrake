...
#include "multibody/plant/tamsi_solver.h"
...
template <typename T>
TamsiDriver<T>::TamsiDriver(const CompliantContactManager<T>* manager) : manager_(manager) {
...
template <typename T>
internal::ContactJacobians<T> TamsiDriver<T>::CalcContactJacobians(const systems::Context<T>& context) const {
    ...
    for (int i = 0; i < nc; ++i) {
        ...
        for (const typename DiscreteContactPair<T>::JacobianTreeBlock& tree_jacobian : contact_pair.jacobian) {
            const int col_offset = topology.tree_velocities_start_in_v(tree_jacobian.tree);
            const int tree_nv = topology.num_tree_velocities(tree_jacobian.tree);
            contact_jacobians.Jc.block(row_offset, col_offset, 3, tree_nv) = tree_jacobian.J.MakeDenseMatrix();
        }
        contact_jacobians.Jt.middleRows(2 * i, 2) = contact_jacobians.Jc.middleRows(3 * i, 2);
...
template <typename T>
void TamsiDriver<T>::CalcContactSolverResults(const systems::Context<T>& context,
    ...
    manager().CalcNonContactForces(context, /* include_joint_limit_penalty_forces */ true,
    ...
    manager().internal_tree().CalcInverseDynamics(context, vdot, F_BBo_W_array, minus_tau, &A_WB_array,
    ...
    const internal::ContactJacobians<T> contact_jacobians = CalcContactJacobians(context);
    ...
    const auto& indices = manager().EvalJointLocking(context).unlocked_velocity_indices;
    if (indices.empty()) {
    ...
    if (tamsi_solver.get_solver_parameters().stiction_tolerance != plant().stiction_tolerance()) {
    ...
    CallTamsiSolver(&tamsi_solver, context.get_time(), v0_unlocked, M0_unlocked, minus_tau_unlocked, fn0, Jn_unlocked,
                    Jt_unlocked, stiffness, damping, mu, &results_unlocked);

    // Joint locking: expand reduced outputs.
    results->v_next = ExpandRows(results_unlocked.v_next, plant().num_velocities(), indices);
...
template <typename T>
TamsiSolverResult TamsiDriver<T>::SolveUsingSubStepping(TamsiSolver<T>* tamsi_solver,
                                                        int num_substeps,
                                                        const MatrixX<T>& M0,
                                                        const MatrixX<T>& Jn,
                                                        const MatrixX<T>& Jt,
                                                        const VectorX<T>& minus_tau,
                                                        const VectorX<T>& stiffness,
    ...
    for (int substep = 0; substep < num_substeps; ++substep) {
        ...
        // Update the data.
        tamsi_solver->SetTwoWayCoupledProblemData(&M0, &Jn, &Jt, &p_star_substep, &fn0_substep, &stiffness, &damping,
                                                  &mu);
        ...
        fn0_substep = fn0_substep.array() - dt_substep * stiffness.array() * vn_substep.array();
...
template <typename T>
void TamsiDriver<T>::CallTamsiSolver(TamsiSolver<T>* tamsi_solver,
                                     const T& time0,
                                     const VectorX<T>& v0,
                                     const MatrixX<T>& M0,
                                     const VectorX<T>& minus_tau,
    ...
    do {
        ++num_substeps;
        info = SolveUsingSubStepping(tamsi_solver, num_substeps, M0, Jn, Jt, minus_tau, stiffness, damping, mu, v0,
                                     fn0);
    } while (info != TamsiSolverResult::kSuccess && num_substeps < kNumMaxSubTimeSteps);
...
void TamsiDriver<T>::CalcAndAddSpatialContactForcesFromContactResults(
    ...
    for (int i = 0; i < contact_results.num_point_pair_contacts(); ++i) {
        const PointPairContactInfo<T>& pair = contact_results.point_pair_contact_info(i);
    ...
    if constexpr (scalar_predicate<T>::is_bool) {
        for (int i = 0; i < contact_results.num_hydroelastic_contacts(); ++i) {
            const HydroelasticContactInfo<T>& info = contact_results.hydroelastic_contact_info(i);
...
template <typename T>
void TamsiDriver<T>::CalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context,
                                                       MultibodyForces<T>* forces) const {
    manager().CalcNonContactForces(context,
                                   /* include_joint_limit_penalty_forces */ true,
                                   /* include_pd_controlled_input */ true, forces);
    const ContactResults<T>& contact_results = manager().EvalContactResults(context);
    auto& Fapplied_Bo_W_array = forces->mutable_body_forces();
    CalcAndAddSpatialContactForcesFromContactResults(context, contact_results, &Fapplied_Bo_W_array);
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::TamsiDriver);
