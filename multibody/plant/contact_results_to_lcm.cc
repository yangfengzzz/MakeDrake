...
bool operator==(const FullBodyName& n1, const FullBodyName& n2) {
    return n1.model == n2.model && n1.body == n2.body && n1.geometry == n2.geometry &&
           n1.body_name_is_unique == n2.body_name_is_unique && n1.geometry_count == n2.geometry_count;
}
...
/* Wrapper for plant.GetCollisionGeometriesForBody() that can provide a
 ...
 generator. */
template <typename T>
const std::vector<GeometryId>& GetCollisionGeometriesForBody(const MultibodyPlant<T>& plant,
                                                             const RigidBody<T>& body,
                                                             bool warn_for_multi_geometry_body) {
    const std::vector<GeometryId>& geometries = plant.GetCollisionGeometriesForBody(body);
...
template <typename T>
ContactResultsToLcmSystem<T>::ContactResultsToLcmSystem(const MultibodyPlant<T>& plant)
...
template <typename T>
const systems::InputPort<T>& ContactResultsToLcmSystem<T>::get_contact_result_input_port() const {
    return this->get_input_port(contact_result_input_port_index_);
}

template <typename T>
const systems::OutputPort<T>& ContactResultsToLcmSystem<T>::get_lcm_message_output_port() const {
    return this->get_output_port(message_output_port_index_);
}

template <typename T>
ContactResultsToLcmSystem<T>::ContactResultsToLcmSystem(
        const MultibodyPlant<T>& plant, const std::function<std::string(GeometryId)>& geometry_name_lookup)
    ...
    const std::function<std::string(GeometryId)>& namer = use_default_namer ? &id_as_label : geometry_name_lookup;
    for (BodyIndex i{0}; i < body_count; ++i) {
        ...
        for (auto geometry_id : GetCollisionGeometriesForBody(plant, body, use_default_namer)) {
            const std::string& model_name = plant.GetModelInstanceName(body.model_instance());
            ...
            const int collision_count = static_cast<int>(plant.GetCollisionGeometriesForBody(body).size());
            geometry_id_to_body_name_map_[geometry_id] = {model_name, body.name(), namer(geometry_id),
...
ContactResultsToLcmSystem<T>::ContactResultsToLcmSystem(bool dummy)
    : systems::LeafSystem<T>(systems::SystemTypeTag<ContactResultsToLcmSystem>{}) {
    ...
    contact_result_input_port_index_ =
            this->DeclareAbstractInputPort(systems::kUseDefaultName, Value<ContactResults<T>>()).get_index();
    message_output_port_index_ =
            this->DeclareAbstractOutputPort(systems::kUseDefaultName, &ContactResultsToLcmSystem::CalcLcmContactOutput)
                    .get_index();
}
...
template <typename T>
void ContactResultsToLcmSystem<T>::CalcLcmContactOutput(const Context<T>& context,
                                                        lcmt_contact_results_for_viz* output) const {
    // Get input / output.
    const auto& contact_results = get_contact_result_input_port().template Eval<ContactResults<T>>(context);
    auto& message = *output;

    // Time in microseconds.
    message.timestamp = static_cast<int64_t>(ExtractDoubleOrThrow(context.get_time()) * 1e6);
    ...
    for (int i = 0; i < contact_results.num_point_pair_contacts(); ++i) {
        lcmt_point_pair_contact_info_for_viz& info_msg = message.point_pair_contact_info[i];
        info_msg.timestamp = message.timestamp;

        const PointPairContactInfo<T>& contact_info = contact_results.point_pair_contact_info(i);
    ...
    message.num_hydroelastic_contacts = contact_results.num_hydroelastic_contacts();
    ...
    if constexpr (scalar_predicate<T>::is_bool) {
        ...
        for (int i = 0; i < contact_results.num_hydroelastic_contacts(); ++i) {
            const HydroelasticContactInfo<T>& hydroelastic_contact_info = contact_results.hydroelastic_contact_info(i);
            const geometry::ContactSurface<T>& contact_surface = hydroelastic_contact_info.contact_surface();

            lcmt_hydroelastic_contact_surface_for_viz& surface_message = message.hydroelastic_contacts[i];

            // Get the two body names.
            const FullBodyName& name1 = geometry_id_to_body_name_map_.at(contact_surface.id_M());
            ...
            const FullBodyName& name2 = geometry_id_to_body_name_map_.at(contact_surface.id_N());
            ...
            assign_double(surface_message.force_C_W, hydroelastic_contact_info.F_Ac_W().translational());
            assign_double(surface_message.moment_C_W, hydroelastic_contact_info.F_Ac_W().rotational());
            ...
            surface_message.quadrature_point_data.resize(surface_message.num_quadrature_points);
            ...
            for (int j = 0; j < surface_message.num_quadrature_points; ++j) {
                ...
                assign_double(quad_data_message.vt_BqAq_W, quadrature_point_data[j].vt_BqAq_W);
                assign_double(quad_data_message.traction_Aq_W, quadrature_point_data[j].traction_Aq_W);
            }
...
systems::lcm::LcmPublisherSystem* ConnectWithNameLookup(systems::DiagramBuilder<double>* builder,
    ...
    const std::function<std::string(GeometryId)> name_lookup = [&inspector](GeometryId id) {
    ...
    auto contact_results_publisher =
            builder->AddSystem(systems::lcm::LcmPublisherSystem::Make<lcmt_contact_results_for_viz>(
                    "CONTACT_RESULTS", lcm, publish_period.value_or(default_publish_period)));
    contact_results_publisher->set_name("contact_results_publisher");

    builder->Connect(contact_results_port, contact_to_lcm->get_contact_result_input_port());
...
systems::lcm::LcmPublisherSystem* ConnectContactResultsToDrakeVisualizer(
        ...
        lcm::DrakeLcmInterface* lcm,
        std::optional<double> publish_period) {
    return ConnectWithNameLookup(builder, multibody_plant, multibody_plant.get_contact_results_output_port(),
...
systems::lcm::LcmPublisherSystem* ConnectContactResultsToDrakeVisualizer(
    ...
    return ConnectWithNameLookup(builder, multibody_plant, contact_results_port, scene_graph, lcm, publish_period);
}

}  // namespace multibody
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::ContactResultsToLcmSystem);
