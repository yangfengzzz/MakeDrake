...
#include "multibody/contact_solvers/contact_solver_results.h"
#include "multibody/plant/constraint_specs.h"
#include "multibody/plant/contact_results.h"
#include "multibody/plant/coulomb_friction.h"
#include "multibody/plant/dummy_physical_model.h"
#include "multibody/plant/multibody_plant_config.h"
#include "multibody/plant/physical_model_collection.h"
#include "multibody/tree/force_element.h"
#include "multibody/tree/frame.h"
#include "multibody/tree/joint.h"
#include "multibody/tree/joint_actuator.h"
#include "multibody/tree/multibody_forces.h"
#include "multibody/tree/multibody_tree-inl.h"
#include "multibody/tree/multibody_tree_system.h"
...
struct HydroelasticContactForcesContinuousCacheData {
    explicit HydroelasticContactForcesContinuousCacheData(int num_bodies) { F_BBo_W_array.resize(num_bodies); }
...
public:
    ...
    const systems::OutputPort<T>& get_geometry_poses_output_port() const { return get_geometry_pose_output_port(); }
    ...
    const RigidBody<T>& AddRigidBody(const std::string& name,
        ...
        const RigidBody<T>& body = this->mutable_tree().AddRigidBody(name, model_instance, M_BBo_B);
    ...
    const JointType<T>& AddJoint(std::unique_ptr<JointType<T>> joint) {
        static_assert(std::is_convertible_v<JointType<T>*, Joint<T>*>, "JointType must be a sub-class of Joint<T>.");
        DRAKE_MBP_THROW_IF_FINALIZED();
        const JointType<T>& result = this->mutable_tree().AddJoint(std::move(joint));
        return result;
    }
    ...
    const JointType<T>& AddJoint(const std::string& name,
        ...
        static_assert(std::is_base_of_v<Joint<T>, JointType<T>>, "JointType<T> must be a sub-class of Joint<T>.");
        DRAKE_MBP_THROW_IF_FINALIZED();
        const JointType<T>& result = this->mutable_tree().template AddJoint<JointType>(name, parent, X_PF, child, X_BM,
    ...
    const ForceElementType<T>& AddForceElement(Args&&... args) {
        DRAKE_MBP_THROW_IF_FINALIZED();
        return this->mutable_tree().template AddForceElement<ForceElementType>(std::forward<Args>(args)...);
    }
    ...
    ModelInstanceIndex AddModelInstance(const std::string& name) { return this->mutable_tree().AddModelInstance(name); }
    ...
    int num_constraints() const {
        return num_coupler_constraints() + num_distance_constraints() + num_ball_constraints() + num_weld_constraints();
    }
    ...
    int num_coupler_constraints() const { return ssize(coupler_constraints_specs_); }

    /// Returns the total number of distance constraints specified by the user.
    int num_distance_constraints() const { return ssize(distance_constraints_specs_); }
    ...
    geometry::FrameId GetBodyFrameIdOrThrow(BodyIndex body_index) const {
        const auto it = body_index_to_frame_id_.find(body_index);
        if (it == body_index_to_frame_id_.end()) {
            throw std::logic_error("Body '" + internal_tree().get_body(body_index).name() +
    ...
    bool get_adjacent_bodies_collision_filters() const { return adjacent_bodies_collision_filters_; }
    ...
    // already added.
    internal::DummyPhysicalModel<T>& AddDummyModel(std::unique_ptr<internal::DummyPhysicalModel<T>> model);
    ...
    double stiction_tolerance() const { return friction_model_.stiction_tolerance(); }
    ...
    void SetPositionsAndVelocities(systems::Context<T>* context,
        ...
        this->ValidateContext(context);
        DRAKE_THROW_UNLESS(q_v.size() == (num_positions(model_instance) + num_velocities(model_instance)));
    ...
    VectorX<T> GetPositions(const systems::Context<T>& context, ModelInstanceIndex model_instance) const {
        this->ValidateContext(context);
        return internal_tree().GetPositionsFromArray(model_instance, internal_tree().get_positions(context));
    }
    ...
    void GetPositions(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        internal_tree().GetPositionsFromArray(model_instance, internal_tree().get_positions(context), q_out);
    }
    ...
    void SetPositions(systems::Context<T>* context,
        ...
        Eigen::VectorBlock<VectorX<T>> q = internal_tree().GetMutablePositions(context);
    ...
    void SetPositions(const systems::Context<T>& context,
        ...
        DRAKE_THROW_UNLESS(q_instance.size() == num_positions(model_instance));
        Eigen::VectorBlock<VectorX<T>> q = internal_tree().get_mutable_positions(state);
        internal_tree().SetPositionsInArray(model_instance, q_instance, &q);
    }
    ...
    VectorX<T> GetVelocities(const systems::Context<T>& context, ModelInstanceIndex model_instance) const {
        this->ValidateContext(context);
        return internal_tree().GetVelocitiesFromArray(model_instance, internal_tree().get_velocities(context));
    }
    ...
    void GetVelocities(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        internal_tree().GetVelocitiesFromArray(model_instance, internal_tree().get_velocities(context), v_out);
    }
    ...
    void SetVelocities(systems::Context<T>* context,
        ...
        Eigen::VectorBlock<VectorX<T>> v = internal_tree().GetMutableVelocities(context);
    ...
    void SetVelocities(const systems::Context<T>& context,
        ...
        this->ValidateCreatedForThisSystem(state);
        DRAKE_THROW_UNLESS(v_instance.size() == num_velocities(model_instance));
        Eigen::VectorBlock<VectorX<T>> v = internal_tree().get_mutable_velocities(state);
        internal_tree().SetVelocitiesInArray(model_instance, v_instance, &v);
    }
    ...
    void SetDefaultFreeBodyPose(const RigidBody<T>& body, const math::RigidTransform<double>& X_WB) {
    ...
    void SetFreeBodyRandomTranslationDistribution(const RigidBody<T>& body,
        ...
        this->mutable_tree().SetFreeBodyRandomTranslationDistributionOrThrow(body, translation);
    }

    DRAKE_DEPRECATED("2024-08-01", "Use MultibodyPlant::SetFreeBodyRandomTranslationDistribution()")
    void SetFreeBodyRandomPositionDistribution(const RigidBody<T>& body,
    ...
    void SetFreeBodyRandomRotationDistribution(const RigidBody<T>& body,
        ...
        this->mutable_tree().SetFreeBodyRandomRotationDistributionOrThrow(body, rotation);
    }
    ...
    math::RotationMatrix<T> CalcRelativeRotationMatrix(const systems::Context<T>& context,
                                                       ...
                                                       const Frame<T>& frame_B) const {
        this->ValidateContext(context);
        return internal_tree().CalcRelativeRotationMatrix(context, frame_A, frame_B);
    }
    ...
    Vector3<T> CalcCenterOfMassPositionInWorld(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        return internal_tree().CalcCenterOfMassPositionInWorld(context, model_instances);
    }
    ...
    Vector3<T> CalcCenterOfMassTranslationalVelocityInWorld(const systems::Context<T>& context) const {
        return internal_tree().CalcCenterOfMassTranslationalVelocityInWorld(context);
    }
    ...
    Vector3<T> CalcCenterOfMassTranslationalVelocityInWorld(
        ...
        return internal_tree().CalcCenterOfMassTranslationalVelocityInWorld(context, model_instances);
    }
    ...
    SpatialMomentum<T> CalcSpatialMomentumInWorldAboutPoint(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        return internal_tree().CalcSpatialMomentumInWorldAboutPoint(context, p_WoP_W);
    }
    ...
    SpatialMomentum<T> CalcSpatialMomentumInWorldAboutPoint(const systems::Context<T>& context,
                                                            ...
                                                            const Vector3<T>& p_WoP_W) const {
        this->ValidateContext(context);
        return internal_tree().CalcSpatialMomentumInWorldAboutPoint(context, model_instances, p_WoP_W);
    }
    ...
    Matrix3X<T> CalcBiasTranslationalAcceleration(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        return internal_tree().CalcBiasTranslationalAcceleration(context, with_respect_to, frame_B, p_BoBi_B, frame_A,
                                                                 frame_E);
    }
    ...
    SpatialAcceleration<T> CalcBiasSpatialAcceleration(const systems::Context<T>& context,
        ...
        // TODO(Mitiguy) Allow with_respect_to to be JacobianWrtVariable::kQDot.
        this->ValidateContext(context);
        return internal_tree().CalcBiasSpatialAcceleration(context, with_respect_to, frame_B, p_BoBp_B, frame_A,
                                                           frame_E);
    }
    ...
    void CalcJacobianSpatialVelocity(const systems::Context<T>& context,
        ...
        internal_tree().CalcJacobianSpatialVelocity(context, with_respect_to, frame_B, p_BoBp_B, frame_A, frame_E,
                                                    Js_V_ABp_E);
    }
    ...
    void CalcJacobianAngularVelocity(const systems::Context<T>& context,
        ...
        return internal_tree().CalcJacobianAngularVelocity(context, with_respect_to, frame_B, frame_A, frame_E,
                                                           Js_w_AB_E);
    }
    ...
    void CalcJacobianTranslationalVelocity(const systems::Context<T>& context,
        ...
        internal_tree().CalcJacobianTranslationalVelocity(context, with_respect_to, frame_B, frame_B, p_BoBi_B, frame_A,
                                                          frame_E, Js_v_ABi_E);
    }
    ...
    void CalcJacobianPositionVector(const systems::Context<T>& context,
        ...
        internal_tree().CalcJacobianTranslationalVelocity(context, JacobianWrtVariable::kQDot, frame_B, frame_B,
                                                          p_BoBi_B, frame_A, frame_E, Jq_p_AoBi_E);
    }
    ...
    void CalcJacobianCenterOfMassTranslationalVelocity(const systems::Context<T>& context,
        ...
        internal_tree().CalcJacobianCenterOfMassTranslationalVelocity(context, with_respect_to, frame_A, frame_E,
                                                                      Js_v_ACcm_E);
    }
    ...
    void CalcJacobianCenterOfMassTranslationalVelocity(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        DRAKE_DEMAND(Js_v_ACcm_E != nullptr);
        internal_tree().CalcJacobianCenterOfMassTranslationalVelocity(context, model_instances, with_respect_to,
                                                                      frame_A, frame_E, Js_v_ACcm_E);
    }
    ...
    Vector3<T> CalcBiasCenterOfMassTranslationalAcceleration(const systems::Context<T>& context,
        ...
        this->ValidateContext(context);
        return internal_tree().CalcBiasCenterOfMassTranslationalAcceleration(context, with_respect_to, frame_A,
                                                                             frame_E);
    }
    ...
    const RigidBody<T>& world_body() const { return internal_tree().world_body(); }

    /// Returns a constant reference to the *world* frame.
    const RigidBodyFrame<T>& world_frame() const { return internal_tree().world_frame(); }
    ...
    /// this model.
    const RigidBody<T>& get_body(BodyIndex body_index) const { return internal_tree().get_body(body_index); }
    ...
    /// instances.
    bool HasBodyNamed(std::string_view name) const { return internal_tree().HasBodyNamed(name); }

    /// @returns The total number of bodies (across all model instances) with the
    /// given name.
    int NumBodiesWithName(std::string_view name) const { return internal_tree().NumBodiesWithName(name); }
    ...
    const RigidBody<T>& GetBodyByName(std::string_view name) const { return internal_tree().GetRigidBodyByName(name); }
    ...
    bool has_joint(JointIndex joint_index) const { return internal_tree().has_joint(joint_index); }
    ...
    /// joint in this model.
    const Joint<T>& get_joint(JointIndex joint_index) const { return internal_tree().get_joint(joint_index); }
    ...
    /// instances.
    bool HasJointNamed(std::string_view name) const { return internal_tree().HasJointNamed(name); }
    ...
    /// @throws std::exception when `joint_index` does not correspond to a
    /// joint in this model.
    Joint<T>& get_mutable_joint(JointIndex joint_index) { return this->mutable_tree().get_mutable_joint(joint_index); }
    ...
    const std::vector<JointIndex>& GetJointIndices() const { return internal_tree().GetJointIndices(); }
    ...
    JointType<T>& GetMutableJointByName(std::string_view name,
        ...
        return this->mutable_tree().template GetMutableJointByName<JointType>(name, model_instance);
    }
    ...
    /// this plant.
    const Frame<T>& get_frame(FrameIndex frame_index) const { return internal_tree().get_frame(frame_index); }

    /// @returns `true` if a frame named `name` was added to the model.
    /// @see AddFrame().
    /// @throws std::exception if the frame name occurs in multiple model
    /// instances.
    bool HasFrameNamed(std::string_view name) const { return internal_tree().HasFrameNamed(name); }
    ...
    const Frame<T>& GetFrameByName(std::string_view name) const { return internal_tree().GetFrameByName(name); }
    ...
    /// @throws std::exception if called pre-finalize.
    int num_actuators(ModelInstanceIndex model_instance) const { return internal_tree().num_actuators(model_instance); }
    ...
    /// instances.
    bool HasJointActuatorNamed(std::string_view name) const { return internal_tree().HasJointActuatorNamed(name); }
    ...
    /// @see AddForceElement().
    int num_force_elements() const { return internal_tree().num_force_elements(); }
    ...
    const ForceElementType<T>& GetForceElement(ForceElementIndex force_element_index) const {
        return internal_tree().template GetForceElement<ForceElementType>(force_element_index);
    }
    ...
    const UniformGravityFieldElement<T>& gravity_field() const { return internal_tree().gravity_field(); }

    // TODO(amastro-tri): mutation of the model should only be allowed
    // pre-finalize.
    /// A mutable accessor to the current gravity field.
    UniformGravityFieldElement<T>& mutable_gravity_field() { return this->mutable_tree().mutable_gravity_field(); }

    /// Returns the number of model instances in the model.
    /// @see AddModelInstance().
    int num_model_instances() const { return internal_tree().num_model_instances(); }
    ...
    /// @see AddModelInstance().
    bool HasModelInstanceNamed(std::string_view name) const { return internal_tree().HasModelInstanceNamed(name); }
    ...
    /// instance i.
    /// @throws std::exception if called pre-finalize.
    int num_positions(ModelInstanceIndex model_instance) const { return internal_tree().num_positions(model_instance); }
    ...
    /// -infinity.
    /// @throws std::exception if called pre-finalize.
    VectorX<double> GetPositionLowerLimits() const { return internal_tree().GetPositionLowerLimits(); }
    ...
    VectorX<double> GetPositionUpperLimits() const { return internal_tree().GetPositionUpperLimits(); }
    ...
    /// -infinity.
    /// @throws std::exception if called pre-finalize.
    VectorX<double> GetVelocityLowerLimits() const { return internal_tree().GetVelocityLowerLimits(); }

    /// Upper limit analog of GetVelocitysLowerLimits(), where any unbounded or
    /// unspecified limits will be +infinity.
    /// @see GetVelocityLowerLimits() for more information.
    VectorX<double> GetVelocityUpperLimits() const { return internal_tree().GetVelocityUpperLimits(); }
    ...
    /// @throws std::exception if called pre-finalize.
    VectorX<double> GetAccelerationLowerLimits() const { return internal_tree().GetAccelerationLowerLimits(); }
    ...
    VectorX<double> GetAccelerationUpperLimits() const { return internal_tree().GetAccelerationUpperLimits(); }
...
private:
    ...
    explicit MultibodyPlant(std::unique_ptr<internal::MultibodyTree<T>> tree_in, double time_step = 0);
    ...
    const geometry::QueryObject<T>& EvalGeometryQueryInput(const systems::Context<T>& context,
    ...
    void ValidateGeometryInput(const systems::Context<T>& context, std::string_view explanation) const;

    // A validation overload that automatically constructs an explanation when
    // the reason is due to evaluating an output port.
    void ValidateGeometryInput(const systems::Context<T>& context, const systems::OutputPort<T>& output_port) const;
    ...
    std::pair<T, T> GetPointContactParameters(geometry::GeometryId id,
    ...
    void CalcNetActuationOutput(const systems::Context<T>& context, systems::BasicVector<T>* output) const;
    ...
    void CalcNonContactForcesContinuous(const drake::systems::Context<T>& context, MultibodyForces<T>* forces) const;
    ...
    // @pre The plant is continuous.
    void AddInForcesContinuous(const systems::Context<T>& context, MultibodyForces<T>* forces) const override;
    ...
    void DoCalcForwardDynamicsDiscrete(const drake::systems::Context<T>& context,
    ...
    systems::EventStatus CalcDiscreteStep(const systems::Context<T>& context0,
                                          systems::DiscreteValues<T>* updates) const;

    // Data will be resized on output according to the documentation for
    // JointLockingCacheData.
    void CalcJointLocking(const systems::Context<T>& context, internal::JointLockingCacheData<T>* data) const;

    // Eval version of the method CalcJointLocking().
    const internal::JointLockingCacheData<T>& EvalJointLocking(const systems::Context<T>& context) const {
    ...
    void CalcGeometryContactData(const systems::Context<T>& context, internal::GeometryContactData<T>* result) const;

    // Eval function for the like-named cache entry (geometry_contact_data).
    const internal::GeometryContactData<T>& EvalGeometryContactData(const systems::Context<T>& context) const;
    ...
    void CalcContactResultsContinuous(const systems::Context<T>& context, ContactResults<T>* contact_results) const;
    ...
    void AppendContactResultsPointPairContinuous(const systems::Context<T>& context,
                                                 ContactResults<T>* contact_results) const;

    // Helper method to fill in `contact_results` with hydroelastic forces as a
    // function of the state stored in `context`.
    // @param[in,out] contact_results is appended to
    void AppendContactResultsHydroelasticContinuous(const systems::Context<T>& context,
                                                    ContactResults<T>* contact_results) const;

    // Evaluate contact results.
    const ContactResults<T>& EvalContactResults(const systems::Context<T>& context) const;

    // Calc method for the "reaction_forces" output port.
    void CalcReactionForcesOutput(const systems::Context<T>& context, std::vector<SpatialForce<T>>* output) const;

    // Collect joint actuator forces and externally provided spatial and
    // generalized forces.
    void AddInForcesFromInputPorts(const drake::systems::Context<T>& context, MultibodyForces<T>* forces) const;

    // Add contribution of generalized forces passed in through our
    // applied_generalized_force input port.
    void AddAppliedExternalGeneralizedForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const;

    // Add contribution of body spatial forces passed in through our
    // applied_spatial_force input port.
    void AddAppliedExternalSpatialForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const;
    ...
    void AddJointActuationForces(const systems::Context<T>& context, VectorX<T>* forces) const;
    ...
    geometry::GeometryId RegisterGeometry(const RigidBody<T>& body,
                                          const math::RigidTransform<double>& X_BG,
                                          const geometry::Shape& shape,
                                          const std::string& name);
    ...
    bool body_has_registered_frame(const RigidBody<T>& body) const {
        return body_index_to_frame_id_.find(body.index()) != body_index_to_frame_id_.end();
    }
    ...
    void CalcStateOutput(const systems::Context<T>& context, systems::BasicVector<T>* output) const;
    ...
    // ports.
    void CalcInstanceGeneralizedContactForcesOutput(ModelInstanceIndex model_instance,
                                                    const systems::Context<T>& context,
                                                    systems::BasicVector<T>* output) const;

    // Calc method for the "body_poses" output port.
    void CalcBodyPosesOutput(const systems::Context<T>& context, std::vector<math::RigidTransform<T>>* output) const;

    // Calc method for the "body_spatial_velocities" output port.
    void CalcBodySpatialVelocitiesOutput(const systems::Context<T>& context,
                                         std::vector<SpatialVelocity<T>>* output) const;

    // Calc method for the "body_spatial_accelerations" output port.
    void CalcBodySpatialAccelerationsOutput(const systems::Context<T>& context,
                                            std::vector<SpatialAcceleration<T>>* output) const;

    // Calc method for the "generalized_acceleration" output port.
    void CalcGeneralizedAccelerationOutput(const systems::Context<T>& context, systems::BasicVector<T>* output) const;

    // Calc method for the "{model_instance_name}_generalized_acceleration"
    // output ports.
    void CalcInstanceGeneralizedAccelerationOutput(ModelInstanceIndex model_instance,
                                                   const systems::Context<T>& context,
                                                   systems::BasicVector<T>* output) const;
    ...
    void CalcSpatialContactForcesContinuous(const drake::systems::Context<T>& context,
                                            std::vector<SpatialForce<T>>* F_BBo_W_array) const;

    // Eval version of CalcSpatialContactForcesContinuous().
    const std::vector<SpatialForce<T>>& EvalSpatialContactForcesContinuous(const systems::Context<T>& context) const;
    ...
    void CalcAndAddSpatialContactForcesContinuous(const drake::systems::Context<T>& context,
                                                  std::vector<SpatialForce<T>>* F_BBo_W_array) const;

    // Method to compute generalized contact forces for continuous plants.
    void CalcGeneralizedContactForcesContinuous(const drake::systems::Context<T>& context,
                                                VectorX<T>* tau_contact) const;

    // Eval version of CalcGeneralizedContactForcesContinuous().
    const VectorX<T>& EvalGeneralizedContactForcesContinuous(const systems::Context<T>& context) const;
    ...
    void CalcGeometryPoseOutput(const systems::Context<T>& context, geometry::FramePoseVector<T>* output) const;

    // Calc method for the "contact_results" output port.
    void CalcContactResultsOutput(const systems::Context<T>& context, ContactResults<T>* output) const;

    // (Advanced) Helper method to compute contact forces in the normal direction
    // using a penalty method.
    void CalcAndAddPointContactForcesContinuous(const systems::Context<T>& context,
                                                std::vector<SpatialForce<T>>* F_BBo_W_array) const;
    ...
    void CalcHydroelasticContactForcesContinuous(
            const systems::Context<T>& context, internal::HydroelasticContactForcesContinuousCacheData<T>* output) const
        requires scalar_predicate<T>::is_bool;

    // Eval version of CalcHydroelasticContactForces().
    const internal::HydroelasticContactForcesContinuousCacheData<T>& EvalHydroelasticContactForcesContinuous(
            const systems::Context<T>& context) const;
    ...
    void AddJointLimitsPenaltyForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const;
    ...
    const std::map<MultibodyConstraintId, bool>& GetConstraintActiveStatus(const systems::Context<T>& context) const {
    ...
    std::map<MultibodyConstraintId, bool>& GetMutableConstraintActiveStatus(systems::Context<T>* context) const {
        return context->get_mutable_parameters()
                .template get_mutable_abstract_parameter<internal::ConstraintActiveStatusMap>(
                        parameter_indices_.constraint_active_status)
                .map;
    }
    ...
    void RemoveUnsupportedScalars(const internal::ScalarConvertibleComponent<T>& component);
    ...
    public:
        ...
        T ComputeFrictionCoefficient(const T& speed_BcAc, const CoulombFriction<double>& friction) const;
    ...
    DiscreteContactApproximation discrete_contact_approximation_{DiscreteContactApproximation::kTamsi};

    // Near rigid regime parameter from [Castro et al., 2021]. Refer to
    // set_near_rigid_threshold() for details.
    double sap_near_rigid_threshold_{MultibodyPlantConfig{}.sap_near_rigid_threshold};
    ...
    std::map<MultibodyConstraintId, internal::CouplerConstraintSpec> coupler_constraints_specs_;

    // Map of distance constraints specifications.
    std::map<MultibodyConstraintId, internal::DistanceConstraintSpec> distance_constraints_specs_;

    // Map of ball constraint specifications.
    std::map<MultibodyConstraintId, internal::BallConstraintSpec> ball_constraints_specs_;

    // Map of weld constraint specifications.
    std::map<MultibodyConstraintId, internal::WeldConstraintSpec> weld_constraints_specs_;

    // Whether to apply collsion filters to adjacent bodies at Finalize().
    bool adjacent_bodies_collision_filters_{MultibodyPlantConfig{}.adjacent_bodies_collision_filters};
...
struct AddMultibodyPlantSceneGraphResult final {
    ...
    AddMultibodyPlantSceneGraphResult(AddMultibodyPlantSceneGraphResult&&) = default;
    AddMultibodyPlantSceneGraphResult(const AddMultibodyPlantSceneGraphResult&) = delete;
...
private:
    ...
    friend AddMultibodyPlantSceneGraphResult AddMultibodyPlantSceneGraph<T>(systems::DiagramBuilder<T>*,
                                                                            std::unique_ptr<MultibodyPlant<T>>,
                                                                            std::unique_ptr<geometry::SceneGraph<T>>);

    AddMultibodyPlantSceneGraphResult(MultibodyPlant<T>* plant_in, geometry::SceneGraph<T>* scene_graph_in)
        : plant(*plant_in), scene_graph(*scene_graph_in), plant_ptr(plant_in), scene_graph_ptr(scene_graph_in) {}
...
// @tparam_default_scalar
template <typename T>
std::pair<T, T> CombinePointContactParameters(const T& k1, const T& k2, const T& d1, const T& d2) {
    ...
    return std::pair(safe_divide(k1 * k2, k1 + k2),                                   // k
...
#endif
template <typename T>
struct tuple_size<drake::multibody::AddMultibodyPlantSceneGraphResult<T>> : std::integral_constant<std::size_t, 2> {};

template <typename T>
struct tuple_element<0, drake::multibody::AddMultibodyPlantSceneGraphResult<T>> {
    using type = drake::multibody::MultibodyPlant<T>;
};

template <typename T>
struct tuple_element<1, drake::multibody::AddMultibodyPlantSceneGraphResult<T>> {
