...
Propeller<T>::Propeller(const std::vector<PropellerInfo>& propeller_info)
    : systems::LeafSystem<T>(systems::SystemTypeTag<Propeller>{}), info_(propeller_info) {
    this->DeclareInputPort("command", systems::kVectorValued, num_propellers());

    this->DeclareAbstractInputPort("body_poses", Value<std::vector<math::RigidTransform<T>>>());

    this->DeclareAbstractOutputPort("spatial_forces", std::vector<ExternallyAppliedSpatialForce<T>>(num_propellers()),
...
template <typename T>
void Propeller<T>::CalcSpatialForces(const systems::Context<T>& context,
    ...
    const auto& poses = get_body_poses_input_port().template Eval<std::vector<math::RigidTransform<T>>>(context);
    ...
    for (int i = 0; i < num_propellers(); i++) {
        ...
        const SpatialForce<T> F_BPo_P(Vector3<T>(0, 0, command[i] * prop.moment_ratio),
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::Propeller);
