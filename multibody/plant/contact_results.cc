...
template <typename T>
ContactResults<T>& ContactResults<T>::operator=(const ContactResults<T>& contact_results) {
    ...
    if (contact_results.num_hydroelastic_contacts() == 0) {
        ...
        if (hydroelastic_contact_vector_ownership_mode() == kAliasedPointers) {
            hydroelastic_contact_info_ = std::vector<std::unique_ptr<HydroelasticContactInfo<T>>>();
        }

        // Copy the HydroelasticContactInfo data.
        std::vector<std::unique_ptr<HydroelasticContactInfo<T>>>& hydroelastic_contact_vector =
                hydroelastic_contact_vector_of_unique_ptrs();
        hydroelastic_contact_vector.resize(contact_results.num_hydroelastic_contacts());
        for (int i = 0; i < contact_results.num_hydroelastic_contacts(); ++i) {
            const HydroelasticContactInfo<T>& contact_info = contact_results.hydroelastic_contact_info(i);
            hydroelastic_contact_vector[i] = std::make_unique<HydroelasticContactInfo<T>>(contact_info);
        }
    }
...
template <typename T>
const PointPairContactInfo<T>& ContactResults<T>::point_pair_contact_info(int i) const {
    DRAKE_THROW_UNLESS(i >= 0 && i < num_point_pair_contacts());
    return point_pairs_info_[i];
}

template <typename T>
const HydroelasticContactInfo<T>& ContactResults<T>::hydroelastic_contact_info(int i) const {
...
template <typename T>
const DeformableContactInfo<T>& ContactResults<T>::deformable_contact_info(int i) const {
    DRAKE_THROW_UNLESS(i >= 0 && i < num_deformable_contacts());
    return deformable_contact_info_[i];
}

template <typename T>
void ContactResults<T>::AddContactInfo(const HydroelasticContactInfo<T>* hydroelastic_contact_info) {
    DRAKE_DEMAND(hydroelastic_contact_vector_ownership_mode() == kAliasedPointers);
    hydroelastic_contact_vector_of_pointers().push_back(hydroelastic_contact_info);
}
...
int ContactResults<T>::num_hydroelastic_contacts() const {
    if (hydroelastic_contact_vector_ownership_mode() == kAliasedPointers) {
        ...
        return static_cast<int>(hydroelastic_contact_vector_of_unique_ptrs().size());
    }
}
...
ContactResults<T> ContactResults<T>::SelectHydroelastic(
    ...
    for (int i = 0; i < num_hydroelastic_contacts; ++i) {
        const HydroelasticContactInfo<T>& contact_info = this->hydroelastic_contact_info(i);
    ...
    output_unique_pointers.deformable_contact_info_ = this->deformable_contact_info_;
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::ContactResults);
