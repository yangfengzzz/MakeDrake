...
public:
    ...
    */
    ...
    template <typename T>
    void ResetFull(const MultibodyPlant<T>& plant, const geometry::SceneGraphInspector<T>& inspector);
    ...
    The return value is no longer valid after any call to a reset function. */
    const std::unordered_map<geometry::GeometryId, Entry>& entries() const { return entries_; }
