...
#include "geometry/query_results/contact_surface.h"
#include "geometry/render/render_label.h"
#include "math/random_rotation.h"
#include "math/rotation_matrix.h"
#include "multibody/hydroelastics/hydroelastic_engine.h"
#include "multibody/plant/discrete_update_manager.h"
#include "multibody/plant/externally_applied_spatial_force.h"
#include "multibody/plant/geometry_contact_data.h"
#include "multibody/plant/hydroelastic_traction_calculator.h"
#include "multibody/plant/make_discrete_update_manager.h"
#include "multibody/plant/slicing_and_indexing.h"
...
struct JointLimitsPenaltyParametersEstimator {
    ...
    static std::pair<double, double> CalcCriticallyDampedHarmonicOscillatorParameters(double period, double inertia) {
    ...
    static std::pair<double, double> PickLessStiffPenaltyParameters(const std::pair<double, double>& params1,
    ...
    static std::pair<double, double> CalcPrismaticJointPenaltyParameters(const PrismaticJoint<T>& joint,
                                                                         double numerical_time_scale) {
        ...
        const auto parent_params = CalcCriticallyDampedHarmonicOscillatorParameters(numerical_time_scale, parent_mass);
        // Penalty parameters for the child body (parent fixed).
        const double child_mass = joint.child_body().index() == world_index() ? std::numeric_limits<double>::infinity()
                                                                              : joint.child_body().default_mass();
        const auto child_params = CalcCriticallyDampedHarmonicOscillatorParameters(numerical_time_scale, child_mass);
    ...
    // Then the penalty parameters estimated for each body are combined with
    // PickLessStiffPenaltyParameters() leading to a single set of parameters.
    static std::pair<double, double> CalcRevoluteJointPenaltyParameters(const RevoluteJoint<T>& joint,
                                                                        double numerical_time_scale) {
        ...
        auto CalcRotationalInertiaAboutAxis = [&joint](const Frame<T>& frame) {
            const RigidBody<T>* body = dynamic_cast<const RigidBody<T>*>(&frame.body());
            ...
            const SpatialInertia<T>& M_PPo_P = body->default_spatial_inertia().template cast<T>();
        ...
        const double I_Pa = joint.parent_body().index() == world_index()
                                    ? std::numeric_limits<double>::infinity()
                                    : CalcRotationalInertiaAboutAxis(joint.frame_on_parent());
        auto parent_params = CalcCriticallyDampedHarmonicOscillatorParameters(numerical_time_scale, I_Pa);

        // Rotational inertia about the joint's axis for the child body.
        const double I_Ca = joint.child_body().index() == world_index()
                                    ? std::numeric_limits<double>::infinity()
                                    : CalcRotationalInertiaAboutAxis(joint.frame_on_child());
        auto child_params = CalcCriticallyDampedHarmonicOscillatorParameters(numerical_time_scale, I_Ca);
...
// use any scoping.
template <typename T>
std::string GetScopedName(const MultibodyPlant<T>& plant, ModelInstanceIndex model_instance, const std::string& name) {
    if (model_instance != world_model_instance() && model_instance != default_model_instance()) {
...
// instance.
template <typename T>
int NumOfPdControlledActuators(const MultibodyPlant<T>& plant, ModelInstanceIndex model_instance) {
    int num_actuators = 0;
    for (JointActuatorIndex a : plant.GetJointActuatorIndices()) {
        const JointActuator<T>& actuator = plant.get_joint_actuator(a);
        if (actuator.model_instance() == model_instance && actuator.has_controller()) {
            ++num_actuators;
        }
    }
...
template <typename T>
MultibodyPlant<T>::MultibodyPlant(double time_step) : MultibodyPlant(nullptr, time_step) {
    ...
    DRAKE_DEMAND(MultibodyPlantConfig{}.contact_model == "hydroelastic_with_fallback");
    ...
    DRAKE_DEMAND(discrete_contact_approximation_ == DiscreteContactApproximation::kTamsi);
    DRAKE_DEMAND(MultibodyPlantConfig{}.discrete_contact_solver == "");
    DRAKE_DEMAND(MultibodyPlantConfig{}.discrete_contact_approximation == "");
}

template <typename T>
MultibodyPlant<T>::MultibodyPlant(std::unique_ptr<internal::MultibodyTree<T>> tree_in, double time_step)
    : internal::MultibodyTreeSystem<T>(systems::SystemTypeTag<MultibodyPlant>{}, std::move(tree_in), time_step > 0),
      contact_surface_representation_(GetDefaultContactSurfaceRepresentation(time_step)),
...
MultibodyPlant<T>::MultibodyPlant(const MultibodyPlant<U>& other)
    ...
    // (3) Place a disclaimer comment why that field does not need to be copied
    {
        ...
        penalty_method_contact_parameters_ = other.penalty_method_contact_parameters_;
        ...
        if (other.friction_model_.stiction_tolerance() > 0) {
            friction_model_.set_stiction_tolerance(other.friction_model_.stiction_tolerance());
        }
        ...
        adjacent_bodies_collision_filters_ = other.adjacent_bodies_collision_filters_;
    ...
    if (is_discrete()) {
        ...
        SetDiscreteUpdateManager(other.discrete_update_manager_->template CloneToScalar<T>());
    }
}
...
template <typename T>
bool MultibodyPlant<T>::GetConstraintActiveStatus(const systems::Context<T>& context, MultibodyConstraintId id) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
    const std::map<MultibodyConstraintId, bool>& constraint_active_status = this->GetConstraintActiveStatus(context);
...
void MultibodyPlant<T>::SetConstraintActiveStatus(systems::Context<T>* context,
    ...
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
    std::map<MultibodyConstraintId, bool>& constraint_active_status = this->GetMutableConstraintActiveStatus(context);
    DRAKE_THROW_UNLESS(constraint_active_status.contains(id));
    constraint_active_status[id] = status;
}

template <typename T>
MultibodyConstraintId MultibodyPlant<T>::AddCouplerConstraint(const Joint<T>& joint0,
    ...
    if (joint0.num_velocities() != 1 || joint1.num_velocities() != 1) {
        const std::string message = fmt::format(
                ...
                "DOFs.",
                joint0.name(), joint0.num_velocities(), joint1.name(), joint1.num_velocities());
        throw std::runtime_error(message);
    }

    const MultibodyConstraintId constraint_id = MultibodyConstraintId::get_new_id();

    coupler_constraints_specs_[constraint_id] =
            internal::CouplerConstraintSpec{joint0.index(), joint1.index(), gear_ratio, offset, constraint_id};

    return constraint_id;
}

template <typename T>
MultibodyConstraintId MultibodyPlant<T>::AddDistanceConstraint(const RigidBody<T>& body_A,
    ...
    internal::DistanceConstraintSpec spec{body_A.index(), p_AP,      body_B.index(), p_BQ,
...
template <typename T>
MultibodyConstraintId MultibodyPlant<T>::AddBallConstraint(const RigidBody<T>& body_A,
                                                           const Vector3<double>& p_AP,
                                                           const RigidBody<T>& body_B,
                                                           const Vector3<double>& p_BQ) {
    ...
    internal::BallConstraintSpec spec{body_A.index(), p_AP, body_B.index(), p_BQ, constraint_id};
    if (!spec.IsValid()) {
        const std::string msg = fmt::format(
                "Invalid set of parameters for constraint between bodies '{}' and "
...
template <typename T>
MultibodyConstraintId MultibodyPlant<T>::AddWeldConstraint(const RigidBody<T>& body_A,
    ...
    internal::WeldConstraintSpec spec{body_A.index(), X_AP, body_B.index(), X_BQ, constraint_id};
    if (!spec.IsValid()) {
        const std::string msg = fmt::format(
...
std::string MultibodyPlant<T>::GetTopologyGraphvizString() const {
    ...
    // have the box drawn.
    for (ModelInstanceIndex model_instance_index(0); model_instance_index < num_model_instances();
        ...
        for (const BodyIndex& body_index : GetBodyIndices(model_instance_index)) {
            ...
            graphviz += fmt::format(" body{} [label=\"{}\"];\n", body.index(), body.name());
        }
        graphviz += "}\n";
    }
    ...
    for (JointIndex joint_index : GetJointIndices()) {
        ...
        graphviz += fmt::format("body{} -> body{} [label=\"{} [{}]\"];\n", joint.child_body().index(),
...
template <typename T>
void MultibodyPlant<T>::set_discrete_contact_solver(DiscreteContactSolver contact_solver) {
    DRAKE_MBP_THROW_IF_FINALIZED();
    switch (contact_solver) {
        case DiscreteContactSolver::kTamsi:
            if (num_constraints() > 0) {
                throw std::runtime_error(
                        fmt::format("You selected TAMSI as the solver, but you have constraints "
...
DiscreteContactSolver MultibodyPlant<T>::get_discrete_contact_solver() const {
    ...
    if (discrete_contact_approximation_ == DiscreteContactApproximation::kTamsi) return DiscreteContactSolver::kTamsi;
    // All other approximations use the SAP solver.
    return DiscreteContactSolver::kSap;
}

template <typename T>
void MultibodyPlant<T>::set_discrete_contact_approximation(DiscreteContactApproximation approximation) {
    DRAKE_MBP_THROW_IF_FINALIZED();
    DRAKE_THROW_UNLESS(is_discrete());

    if (approximation == DiscreteContactApproximation::kTamsi && num_constraints() > 0) {
        throw std::runtime_error(
                fmt::format("You selected TAMSI as the contact approximation, but you have "
...
template <typename T>
DiscreteContactApproximation MultibodyPlant<T>::get_discrete_contact_approximation() const {
    return discrete_contact_approximation_;
}

template <typename T>
void MultibodyPlant<T>::set_sap_near_rigid_threshold(double near_rigid_threshold) {
    DRAKE_MBP_THROW_IF_FINALIZED();
...
int MultibodyPlant<T>::num_collision_geometries() const {
    ...
    if constexpr (std::is_same_v<T, double>) {
        const DeformableModel<T>* deformable_model = physical_models_->deformable_model();
...
template <typename T>
void MultibodyPlant<T>::SetFreeBodyRandomRotationDistributionToUniform(const RigidBody<T>& body) {
...
void MultibodyPlant<T>::RemoveJoint(const Joint<T>& joint) {
    ...
    if (num_force_elements() > 1) {
        throw std::logic_error(
                fmt::format("{}: This plant has {} user-added force elements. This plant must have "
    ...
    if (num_constraints() > 0) {
        throw std::logic_error(
                fmt::format("{}: This plant has {} user-added constraints. This plant must have "
    ...
    for (JointActuatorIndex index : GetJointActuatorIndices()) {
        ...
        if (actuator.joint().index() == joint.index()) {
            dependent_elements.push_back(
                    fmt::format("JointActuator(name: {}, index: {})", actuator.name(), actuator.index()));
        }
    }

    if (dependent_elements.size() > 0) {
        throw std::logic_error(
                fmt::format("{}: joint with index {} has the following dependent model elements "
...
template <typename T>
const WeldJoint<T>& MultibodyPlant<T>::WeldFrames(const Frame<T>& frame_on_parent_F,
                                                  const Frame<T>& frame_on_child_M,
                                                  const math::RigidTransform<double>& X_FM) {
    const std::string joint_name = frame_on_parent_F.name() + "_welds_to_" + frame_on_child_M.name();
    return AddJoint(std::make_unique<WeldJoint<T>>(joint_name, frame_on_parent_F, frame_on_child_M, X_FM));
}

template <typename T>
const JointActuator<T>& MultibodyPlant<T>::AddJointActuator(const std::string& name,
                                                            const Joint<T>& joint,
                                                            double effort_limit) {
    if (joint.num_velocities() != 1) {
        throw std::logic_error(
                fmt::format("Calling AddJointActuator with joint {} failed -- this joint has "
...
template <typename T>
geometry::SourceId MultibodyPlant<T>::RegisterAsSourceForSceneGraph(SceneGraph<T>* scene_graph) {
...
template <typename T>
geometry::GeometryId MultibodyPlant<T>::RegisterVisualGeometry(const RigidBody<T>& body,
                                                               const math::RigidTransform<double>& X_BG,
                                                               const geometry::Shape& shape,
                                                               const std::string& name) {
    return RegisterVisualGeometry(body, X_BG, shape, name, geometry::IllustrationProperties());
}
...
geometry::GeometryId MultibodyPlant<T>::RegisterVisualGeometry(const RigidBody<T>& body,
    ...
    return RegisterVisualGeometry(body, X_BG, shape, name, geometry::MakePhongIllustrationProperties(diffuse_color));
}
...
geometry::GeometryId MultibodyPlant<T>::RegisterVisualGeometry(const RigidBody<T>& body,
    ...
    GeometryId id = RegisterGeometry(body, X_BG, shape, GetScopedName(*this, body.model_instance(), name));
    ...
    if (properties.HasProperty("phong", "diffuse")) {
        perception_props.AddProperty("phong", "diffuse", properties.GetProperty<geometry::Rgba>("phong", "diffuse"));
    }
    if (properties.HasProperty("phong", "diffuse_map")) {
        perception_props.AddProperty("phong", "diffuse_map",
...
template <typename T>
const std::vector<geometry::GeometryId>& MultibodyPlant<T>::GetVisualGeometriesForBody(const RigidBody<T>& body) const {
    return visual_geometries_[body.index()];
}

template <typename T>
geometry::GeometryId MultibodyPlant<T>::RegisterCollisionGeometry(const RigidBody<T>& body,
                                                                  const math::RigidTransform<double>& X_BG,
                                                                  const geometry::Shape& shape,
                                                                  const std::string& name,
    ...
    DRAKE_MBP_THROW_IF_FINALIZED();
    DRAKE_THROW_UNLESS(geometry_source_is_registered());
    DRAKE_THROW_UNLESS(properties.HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction));
...
geometry::GeometryId MultibodyPlant<T>::RegisterCollisionGeometry(const RigidBody<T>& body,
    ...
    props.AddProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction, coulomb_friction);
    return RegisterCollisionGeometry(body, X_BG, shape, name, std::move(props));
}

template <typename T>
const std::vector<geometry::GeometryId>& MultibodyPlant<T>::GetCollisionGeometriesForBody(
...
template <typename T>
const SceneGraphInspector<T>& MultibodyPlant<T>::EvalSceneGraphInspector(const systems::Context<T>& context) const {
...
template <typename T>
std::vector<const RigidBody<T>*> MultibodyPlant<T>::GetBodiesWeldedTo(const RigidBody<T>& body) const {
    const std::set<BodyIndex> island = internal_tree().multibody_graph().FindBodiesWeldedTo(body.index());
...
std::vector<BodyIndex> MultibodyPlant<T>::GetBodiesKinematicallyAffectedBy(
    ...
    for (const JointIndex& joint : joint_indexes) {
        ...
        if (get_joint(joint).num_velocities() == 0) {
            throw std::logic_error(fmt::format("{}: joint with index {} is welded.", __func__, joint));
        }
    }
...
template <typename T>
geometry::GeometryId MultibodyPlant<T>::RegisterGeometry(const RigidBody<T>& body,
                                                         const math::RigidTransform<double>& X_BG,
                                                         const geometry::Shape& shape,
                                                         const std::string& name) {
    ...
    GeometryId geometry_id = scene_graph_->RegisterGeometry(source_id_.value(), body_index_to_frame_id_[body.index()],
...
template <typename T>
void MultibodyPlant<T>::RegisterRigidBodyWithSceneGraph(const RigidBody<T>& body) {
    if (geometry_source_is_registered()) {
        ...
        if (!body_has_registered_frame(body)) {
            FrameId frame_id = scene_graph_->RegisterFrame(
                    source_id_.value(), GeometryFrame(GetScopedName(*this, body.model_instance(), body.name()),
...
template <typename T>
void MultibodyPlant<T>::SetFreeBodyPoseInWorldFrame(systems::Context<T>* context,
...
template <typename T>
void MultibodyPlant<T>::SetFreeBodyPoseInAnchoredFrame(systems::Context<T>* context,
    ...
    if (!internal_tree().get_topology().IsBodyAnchored(frame_F.body().index())) {
        throw std::logic_error("Frame '" + frame_F.name() + "' must be anchored to the world frame.");
    }
...
template <typename T>
void MultibodyPlant<T>::CalcSpatialAccelerationsFromVdot(const systems::Context<T>& context,
                                                         const VectorX<T>& known_vdot,
                                                         std::vector<SpatialAcceleration<T>>* A_WB_array) const {
    this->ValidateContext(context);
    DRAKE_THROW_UNLESS(A_WB_array != nullptr);
    DRAKE_THROW_UNLESS(ssize(*A_WB_array) == num_bodies());
    internal_tree().CalcSpatialAccelerationsFromVdot(context, internal_tree().EvalPositionKinematics(context),
                                                     internal_tree().EvalVelocityKinematics(context), known_vdot,
                                                     A_WB_array);
    ...
    for (internal::MobodIndex mobod_index(1); mobod_index < topology.num_mobods(); ++mobod_index) {
...
template <typename T>
void MultibodyPlant<T>::CalcForceElementsContribution(const systems::Context<T>& context,
                                                      MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
    DRAKE_THROW_UNLESS(forces != nullptr);
    DRAKE_THROW_UNLESS(forces->CheckHasRightSizeForModel(internal_tree()));
    internal_tree().CalcForceElementsContribution(context, EvalPositionKinematics(context),
                                                  EvalVelocityKinematics(context), forces);
}

template <typename T>
void MultibodyPlant<T>::RenameModelInstance(ModelInstanceIndex model_instance, const std::string& name) {
...
void MultibodyPlant<T>::SetUpJointLimitsParameters() {
    for (JointIndex joint_index : GetJointIndices()) {
        ...
        if (revolute_joint) {
            ...
            if (!std::isinf(lower_limit) || !std::isinf(upper_limit)) {
                joint_limits_parameters_.joints_with_limits.push_back(revolute_joint->index());
                ...
                auto penalty_parameters =
                        internal::JointLimitsPenaltyParametersEstimator<T>::CalcRevoluteJointPenaltyParameters(
        ...
        if (prismatic_joint) {
            ...
            // We only compute parameters if joints do have upper/lower bounds.
            if (!std::isinf(lower_limit) || !std::isinf(upper_limit)) {
                joint_limits_parameters_.joints_with_limits.push_back(prismatic_joint->index());
                ...
                // Estimate penalty parameters.
                auto penalty_parameters =
                        internal::JointLimitsPenaltyParametersEstimator<T>::CalcPrismaticJointPenaltyParameters(
    ...
    if (!is_discrete() && !joint_limits_parameters_.joints_with_limits.empty()) {
        ...
        for (auto joint_index : joint_limits_parameters_.joints_with_limits) {
            joint_names_with_limits += fmt::format(", '{}'", get_joint(joint_index).name());
        }
...
void MultibodyPlant<T>::FinalizePlantOnly() {
    ...
    if (num_collision_geometries() > 0 && penalty_method_contact_parameters_.time_scale < 0)
        EstimatePointContactParameters(penetration_allowance_);
    if (num_collision_geometries() > 0 && friction_model_.stiction_tolerance() < 0) set_stiction_tolerance();
...
template <typename T>
Eigen::SparseMatrix<double> MultibodyPlant<T>::MakeActuationMatrixPseudoinverse() const {
    ...
    for (JointActuatorIndex actuator_index : GetJointActuatorIndices()) {
        ...
        // condition.
        DRAKE_DEMAND(actuator.joint().num_velocities() == 1);
        triplets.push_back(Eigen::Triplet<double>(actuator.input_start(), actuator.joint().velocity_start(), 1.0));
    }
...
template <typename T>
const geometry::QueryObject<T>& MultibodyPlant<T>::EvalGeometryQueryInput(const systems::Context<T>& context,
    ...
    return get_geometry_query_input_port().template Eval<geometry::QueryObject<T>>(context);
}

template <typename T>
void MultibodyPlant<T>::ValidateGeometryInput(const systems::Context<T>& context, std::string_view explanation) const {
...
template <typename T>
void MultibodyPlant<T>::ValidateGeometryInput(const systems::Context<T>& context,
                                              const systems::OutputPort<T>& output_port) const {
    if (!IsValidGeometryInput(context)) {
        ThrowForDisconnectedGeometryPort(
                fmt::format("You've tried evaluating MultibodyPlant's '{}' output port.", output_port.get_name()));
    }
}

template <typename T>
bool MultibodyPlant<T>::IsValidGeometryInput(const systems::Context<T>& context) const {
    return num_collision_geometries() == 0 || get_geometry_query_input_port().HasValue(context);
}

template <typename T>
std::pair<T, T> MultibodyPlant<T>::GetPointContactParameters(geometry::GeometryId id,
                                                             const SceneGraphInspector<T>& inspector) const {
    const geometry::ProximityProperties* prop = inspector.GetProximityProperties(id);
    DRAKE_DEMAND(prop != nullptr);
    return std::pair(prop->template GetPropertyOrDefault<T>(geometry::internal::kMaterialGroup,
                                                            geometry::internal::kPointStiffness,
                                                            penalty_method_contact_parameters_.geometry_stiffness),
                     prop->template GetPropertyOrDefault<T>(geometry::internal::kMaterialGroup,
                                                            geometry::internal::kHcDissipation,
                                                            penalty_method_contact_parameters_.dissipation));
}

template <typename T>
const CoulombFriction<double>& MultibodyPlant<T>::GetCoulombFriction(geometry::GeometryId id,
                                                                     const SceneGraphInspector<T>& inspector) const {
    const geometry::ProximityProperties* prop = inspector.GetProximityProperties(id);
    DRAKE_DEMAND(prop != nullptr);
    DRAKE_THROW_UNLESS(prop->HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction));
    return prop->GetProperty<CoulombFriction<double>>(geometry::internal::kMaterialGroup,
...
void MultibodyPlant<T>::ApplyDefaultCollisionFilters() {
            ...
            if (child_id && parent_id) {
                scene_graph_->collision_filter_manager().Apply(
                        CollisionFilterDeclaration(CollisionFilterScope::kOmitDeformable)
                                .ExcludeBetween(geometry::GeometrySet(*child_id), geometry::GeometrySet(*parent_id)));
            }
        }
    }
    // We explicitly exclude collisions within welded subgraphs.
    std::vector<std::set<BodyIndex>> subgraphs = internal_tree().multibody_graph().FindSubgraphsOfWeldedBodies();
    for (const auto& subgraph : subgraphs) {
        ...
        scene_graph_->collision_filter_manager().Apply(
                CollisionFilterDeclaration(CollisionFilterScope::kOmitDeformable).ExcludeWithin(geometries));
    }
}

template <typename T>
void MultibodyPlant<T>::ExcludeCollisionGeometriesWithCollisionFilterGroupPair(
        const std::pair<std::string, geometry::GeometrySet>& collision_filter_group_a,
    ...
    if (collision_filter_group_a.first == collision_filter_group_b.first) {
        scene_graph_->collision_filter_manager().Apply(CollisionFilterDeclaration(CollisionFilterScope::kOmitDeformable)
                                                               .ExcludeWithin(collision_filter_group_a.second));
    } else {
        scene_graph_->collision_filter_manager().Apply(
                CollisionFilterDeclaration(CollisionFilterScope::kOmitDeformable)
                        .ExcludeBetween(collision_filter_group_a.second, collision_filter_group_b.second));
    }
}

template <typename T>
BodyIndex MultibodyPlant<T>::FindBodyByGeometryId(GeometryId geometry_id) const {
    if (!geometry_id.is_valid()) {
        throw std::logic_error("MultibodyPlant received contact results for a null GeometryId");
    }
    ...
    throw std::logic_error(
            fmt::format("MultibodyPlant received contact results for GeometryId {}, but that"
                        " ID is not known to this plant",
                        geometry_id));
}

template <typename T>
void MultibodyPlant<T>::SetDiscreteUpdateManager(std::unique_ptr<internal::DiscreteUpdateManager<T>> manager) {
...
template <typename T>
std::vector<const PhysicalModel<T>*> MultibodyPlant<T>::physical_models() const {
...
template <typename T>
void MultibodyPlant<T>::set_penetration_allowance(double penetration_allowance) {
    ...
    // post-finalize.
    if (this->is_finalized()) EstimatePointContactParameters(penetration_allowance);
}
...
VectorX<T> MultibodyPlant<T>::GetDefaultPositions() const {
    ...
    for (JointIndex i : GetJointIndices()) {
        ...
        q.segment(joint.position_start(), joint.num_positions()) = joint.default_positions();
    }
    return q;
}

template <typename T>
VectorX<T> MultibodyPlant<T>::GetDefaultPositions(ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    const VectorX<T> q = GetDefaultPositions();
    const VectorX<T> q_instance = internal_tree().GetPositionsFromArray(model_instance, q);
    return q_instance;
}

template <typename T>
void MultibodyPlant<T>::SetDefaultPositions(const Eigen::Ref<const Eigen::VectorXd>& q) {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(q.size() == num_positions());
    for (JointIndex i : GetJointIndices()) {
        Joint<T>& joint = get_mutable_joint(i);
        joint.set_default_positions(q.segment(joint.position_start(), joint.num_positions()));
    }
}
...
void MultibodyPlant<T>::SetDefaultPositions(ModelInstanceIndex model_instance,
    ...
    internal_tree().SetPositionsInArray(model_instance, q_instance.cast<T>(), &q_T);
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetPositionNames(bool add_model_instance_prefix,
    ...
    for (JointIndex joint_index : GetJointIndices()) {
        ...
        const std::string prefix =
                add_model_instance_prefix ? fmt::format("{}_", GetModelInstanceName(joint.model_instance())) : "";
        for (int i = 0; i < joint.num_positions(); ++i) {
            const std::string suffix =
                    always_add_suffix || joint.num_positions() > 1 ? fmt::format("_{}", joint.position_suffix(i)) : "";
            names[joint.position_start() + i] = fmt::format("{}{}{}", prefix, joint.name(), suffix);
        }
    }
    return names;
}
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetPositionNames(ModelInstanceIndex model_instance,
                                                             bool add_model_instance_prefix,
                                                             bool always_add_suffix) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    ...
    for (const auto& joint_index : joint_indices) {
        position_offset = std::min(position_offset, get_joint(joint_index).position_start());
    }
    ...
    for (const auto& joint_index : joint_indices) {
        ...
        DRAKE_DEMAND(joint.position_start() + joint.num_positions() - position_offset <= ssize(names));

        const std::string prefix =
                add_model_instance_prefix ? fmt::format("{}_", GetModelInstanceName(model_instance)) : "";
        for (int i = 0; i < joint.num_positions(); ++i) {
            const std::string suffix =
                    always_add_suffix || joint.num_positions() > 1 ? fmt::format("_{}", joint.position_suffix(i)) : "";
            names[joint.position_start() + i - position_offset] = fmt::format("{}{}{}", prefix, joint.name(), suffix);
        }
    }
    return names;
}
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetVelocityNames(bool add_model_instance_prefix,
                                                             bool always_add_suffix) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    ...
    for (JointIndex joint_index : GetJointIndices()) {
        ...
        for (int i = 0; i < joint.num_velocities(); ++i) {
            const std::string suffix =
                    always_add_suffix || joint.num_velocities() > 1 ? fmt::format("_{}", joint.velocity_suffix(i)) : "";
            names[joint.velocity_start() + i] = fmt::format("{}{}{}", prefix, joint.name(), suffix);
        }
    }
    return names;
}
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetVelocityNames(ModelInstanceIndex model_instance,
                                                             bool add_model_instance_prefix,
                                                             bool always_add_suffix) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    ...
    for (const auto& joint_index : joint_indices) {
        velocity_offset = std::min(velocity_offset, get_joint(joint_index).velocity_start());
    }
    ...
    for (const auto& joint_index : joint_indices) {
        ...
        DRAKE_DEMAND(joint.velocity_start() + joint.num_velocities() - velocity_offset <= ssize(names));
        ...
        for (int i = 0; i < joint.num_velocities(); ++i) {
            const std::string suffix =
                    always_add_suffix || joint.num_velocities() > 1 ? fmt::format("_{}", joint.velocity_suffix(i)) : "";
            names[joint.velocity_start() + i - velocity_offset] = fmt::format("{}{}{}", prefix, joint.name(), suffix);
        }
    }
    return names;
}
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetStateNames(bool add_model_instance_prefix) const {
    std::vector<std::string> names = GetPositionNames(add_model_instance_prefix, true /* always_add_suffix */);
    std::vector<std::string> velocity_names = GetVelocityNames(add_model_instance_prefix, true /* always_add_suffix */);
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetStateNames(ModelInstanceIndex model_instance,
                                                          bool add_model_instance_prefix) const {
    std::vector<std::string> names =
            GetPositionNames(model_instance, add_model_instance_prefix, true /* always_add_suffix */);
    std::vector<std::string> velocity_names =
            GetVelocityNames(model_instance, add_model_instance_prefix, true /* always_add_suffix */);
    names.insert(names.end(), std::make_move_iterator(velocity_names.begin()),
                 std::make_move_iterator(velocity_names.end()));
    return names;
}
...
template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetActuatorNames(bool add_model_instance_prefix) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    ...
    for (JointActuatorIndex actuator_index : GetJointActuatorIndices()) {
        const JointActuator<T>& actuator = get_joint_actuator(actuator_index);
        const std::string prefix =
                add_model_instance_prefix ? fmt::format("{}_", GetModelInstanceName(actuator.model_instance())) : "";
        ...
        names[actuator.input_start()] = fmt::format("{}{}", prefix, actuator.name());
    }
    return names;
}

template <typename T>
std::vector<std::string> MultibodyPlant<T>::GetActuatorNames(ModelInstanceIndex model_instance,
                                                             bool add_model_instance_prefix) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    std::vector<std::string> names(num_actuators(model_instance));
    std::vector<JointActuatorIndex> actuator_indices = GetJointActuatorIndices(model_instance);
    ...
    for (const auto& actuator_index : actuator_indices) {
        ...
        // multiple inputs.
        DRAKE_DEMAND(actuator.num_inputs() == 1);
        names[actuator.input_start() - offset] = fmt::format("{}{}", prefix, actuator.name());
    }
    return names;
}

template <typename T>
void MultibodyPlant<T>::EstimatePointContactParameters(double penetration_allowance) {
    // Default to Earth's gravity for this estimation.
    const UniformGravityFieldElement<T>& gravity = gravity_field();
    const double g = (!gravity.gravity_vector().isZero()) ? gravity.gravity_vector().norm()
    ...
    penalty_method_contact_parameters_.geometry_stiffness = 2 * combined_stiffness;
...
template <typename T>
void MultibodyPlant<T>::CalcContactResultsOutput(const systems::Context<T>& context, ContactResults<T>* output) const {
    this->ValidateContext(context);
...
template <typename T>
void MultibodyPlant<T>::CalcContactResultsContinuous(const systems::Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::AppendContactResultsHydroelasticContinuous(const systems::Context<T>& context,
                                                                   ContactResults<T>* contact_results) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(contact_results != nullptr);
...
template <typename T>
void MultibodyPlant<T>::AppendContactResultsPointPairContinuous(const systems::Context<T>& context,
                                                                ContactResults<T>* contact_results) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(contact_results != nullptr);
    DRAKE_DEMAND(contact_results->plant() == this);
    ...
    const std::vector<PenetrationAsPointPair<T>>& point_pairs = EvalGeometryContactData(context).point_pairs;

    const internal::PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const internal::VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
    ...
    for (size_t icontact = 0; icontact < point_pairs.size(); ++icontact) {
        ...
        internal::MobodIndex bodyA_mobod_index = get_body(bodyA_index).mobod_index();
        internal::MobodIndex bodyB_mobod_index = get_body(bodyB_index).mobod_index();
        ...
        const Vector3<T> v_WAc = vc.get_V_WB(bodyA_mobod_index).Shift(-p_CoAo_W).translational();
        const Vector3<T> v_WBc = vc.get_V_WB(bodyB_mobod_index).Shift(-p_CoBo_W).translational();
        ...
        const CoulombFriction<double>& geometryA_friction = GetCoulombFriction(geometryA_id, inspector);
        const CoulombFriction<double>& geometryB_friction = GetCoulombFriction(geometryB_id, inspector);
        const CoulombFriction<double> combined_friction =
                CalcContactFrictionFromSurfaceProperties(geometryA_friction, geometryB_friction);
        ...
        if (fn_AC > 0) {
            ...
            if (vt_squared > kNonZeroSqd) {
                ...
                const T mu_stribeck = friction_model_.ComputeFrictionCoefficient(slip_velocity, combined_friction);
            ...
            contact_results->AddContactInfo({bodyA_index, bodyB_index, f_Bc_W, p_WC, vn, slip_velocity, pair});
        }
    }
}

template <typename T>
void MultibodyPlant<T>::CalcAndAddPointContactForcesContinuous(const systems::Context<T>& context,
    ...
    for (int pair_index = 0; pair_index < contact_results.num_point_pair_contacts(); ++pair_index) {
        const PointPairContactInfo<T>& contact_info = contact_results.point_pair_contact_info(pair_index);
        ...
        const internal::MobodIndex bodyA_mobod_index = get_body(bodyA_index).mobod_index();
        const internal::MobodIndex bodyB_mobod_index = get_body(bodyB_index).mobod_index();
...
{  // NOLINT(whitespace/braces)
    ...
    const std::vector<ContactSurface<T>>& all_surfaces = EvalGeometryContactData(context).surfaces;
    ...
    internal::HydroelasticTractionCalculator<T> traction_calculator(friction_model_.stiction_tolerance());
    ...
    for (const ContactSurface<T>& surface : all_surfaces) {
        ...
        const ProximityProperties* propM = inspector.GetProximityProperties(geometryM_id);
        const ProximityProperties* propN = inspector.GetProximityProperties(geometryN_id);
        DRAKE_DEMAND(propM != nullptr);
        DRAKE_DEMAND(propN != nullptr);
        DRAKE_THROW_UNLESS(propM->HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction));
        DRAKE_THROW_UNLESS(propN->HasProperty(geometry::internal::kMaterialGroup, geometry::internal::kFriction));

        const CoulombFriction<double>& geometryM_friction = propM->GetProperty<CoulombFriction<double>>(
                geometry::internal::kMaterialGroup, geometry::internal::kFriction);
        const CoulombFriction<double>& geometryN_friction = propN->GetProperty<CoulombFriction<double>>(
                geometry::internal::kMaterialGroup, geometry::internal::kFriction);
        ...
        typename internal::HydroelasticTractionCalculator<T>::Data data(X_WA, X_WB, V_WA, V_WB, &surface);

        // Combined Hunt & Crossley dissipation.
        const hydroelastics::internal::HydroelasticEngine<T> hydroelastics_engine;
        const double dissipation = hydroelastics_engine.CalcCombinedDissipation(geometryM_id, geometryN_id, inspector);
        ...
        traction_calculator.ComputeSpatialForcesAtCentroidFromHydroelasticModel(data, dissipation, dynamic_friction,
                                                                                &traction_output, &F_Ac_W);

        // Shift the traction at the centroid to tractions at the body origins.
        SpatialForce<T> F_Ao_W, F_Bo_W;
        traction_calculator.ShiftSpatialForcesAtCentroidToBodyOrigins(data, F_Ac_W, &F_Ao_W, &F_Bo_W);
...
MultibodyPlant<T>::EvalHydroelasticContactForcesContinuous(const systems::Context<T>& context) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(!is_discrete());
    return this->get_cache_entry(cache_indices_.hydroelastic_contact_forces_continuous)
            .template Eval<internal::HydroelasticContactForcesContinuousCacheData<T>>(context);
}

template <typename T>
const ContactResults<T>& MultibodyPlant<T>::EvalContactResults(const systems::Context<T>& context) const {
    this->ValidateContext(context);
    if (this->is_discrete()) {
...
template <typename T>
void MultibodyPlant<T>::AddInForcesFromInputPorts(const drake::systems::Context<T>& context,
                                                  MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
...
template <typename T>
void MultibodyPlant<T>::AddAppliedExternalGeneralizedForces(const systems::Context<T>& context,
                                                            MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
    ...
    if (applied_generalized_force_input.HasValue(context)) {
        const VectorX<T>& applied_generalized_force = applied_generalized_force_input.Eval(context);
        if (applied_generalized_force.hasNaN()) {
            throw std::runtime_error("Detected NaN in applied generalized force input port.");
        }
...
template <typename T>
void MultibodyPlant<T>::CalcGeneralizedForces(const systems::Context<T>& context,
    ...
    internal_tree().CalcInverseDynamics(context, zero_vdot, forces.body_forces(), forces.generalized_forces(),
                                        zero_velocities, &A_scratch, &F_scratch, generalized_forces);
    *generalized_forces = -*generalized_forces;
}

template <typename T>
void MultibodyPlant<T>::AddAppliedExternalSpatialForces(const systems::Context<T>& context,
                                                        MultibodyForces<T>* forces) const {
    ...
    const auto* applied_input = this->template EvalInputValue<std::vector<ExternallyAppliedSpatialForce<T>>>(
    ...
    auto throw_if_contains_nan = [this](const ExternallyAppliedSpatialForce<T>& external_spatial_force) {
        const SpatialForce<T>& spatial_force = external_spatial_force.F_Bq_W;
        if (external_spatial_force.p_BoBq_B.hasNaN() || spatial_force.rotational().hasNaN() ||
            spatial_force.translational().hasNaN()) {
            throw std::runtime_error(fmt::format("Spatial force applied on body {} contains NaN.",
...
template <typename T>
void MultibodyPlant<T>::AddJointActuationForces(const systems::Context<T>& context, VectorX<T>* forces) const {
    this->ValidateContext(context);
...
template <typename T>
void MultibodyPlant<T>::AddJointLimitsPenaltyForces(const systems::Context<T>& context,
                                                    MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
    DRAKE_THROW_UNLESS(is_discrete());
    DRAKE_DEMAND(forces != nullptr);

    auto CalcPenaltyForce = [](double lower_limit, double upper_limit, double stiffness, double damping, const T& q,
                               const T& v) {
    ...
    for (size_t index = 0; index < joint_limits_parameters_.joints_with_limits.size(); ++index) {
        const JointIndex joint_index = joint_limits_parameters_.joints_with_limits[index];
        ...
        const T penalty_force = CalcPenaltyForce(lower_limit, upper_limit, stiffness, damping, q, v);
...
template <typename T>
VectorX<T> MultibodyPlant<T>::AssembleActuationInput(const systems::Context<T>& context) const {
    this->ValidateContext(context);
    ...
    for (ModelInstanceIndex model_instance_index(0); model_instance_index < num_model_instances();
        ...
        const auto& input_port = this->get_input_port(input_port_indices_.instance[model_instance_index].actuation);

        if (input_port.HasValue(context)) {
            const auto& u_instance = input_port.Eval(context);
            if (u_instance.hasNaN()) {
                throw std::runtime_error(fmt::format("Actuation input port for model instance {} contains NaN.",
                                                     GetModelInstanceName(model_instance_index)));
            }
    ...
    const auto& actuation_port = this->get_input_port(input_port_indices_.actuation);
    if (actuation_port.HasValue(context)) {
        const auto& u = actuation_port.Eval(context);
        if (u.hasNaN()) {
            throw std::runtime_error("Detected NaN in the actuation input port for all instances.");
        }
...
template <typename T>
void MultibodyPlant<T>::CalcNetActuationOutput(const systems::Context<T>& context, BasicVector<T>* output) const {
    DRAKE_DEMAND(output != nullptr);
...
template <typename T>
void MultibodyPlant<T>::CalcInstanceNetActuationOutput(ModelInstanceIndex model_instance,
                                                       const systems::Context<T>& context,
                                                       systems::BasicVector<T>* output) const {
    // The per-instance calc delegates to the full-model calc and then slices it.
    const VectorX<T>& net_actuation = get_net_actuation_output_port().Eval(context);
    output->SetFromVector(this->GetActuationFromArray(model_instance, net_actuation));
}

template <typename T>
VectorX<T> MultibodyPlant<T>::AssembleDesiredStateInput(const systems::Context<T>& context) const {
    this->ValidateContext(context);
    ...
    for (ModelInstanceIndex model_instance_index(0); model_instance_index < num_model_instances();
        ...
        const auto& xd_input_port = this->get_desired_state_input_port(model_instance_index);

        const int num_pd_controlled_actuators = NumOfPdControlledActuators(*this, model_instance_index);
        ...
        } else if (0 < num_pd_controlled_actuators && num_pd_controlled_actuators < instance_num_u) {
            // Partially controlled model. Not supported.
            throw std::runtime_error(
                    fmt::format("Model {} is partially PD controlled. For PD controlling a model "
...
template <typename T>
void MultibodyPlant<T>::CalcGeometryContactData(const drake::systems::Context<T>& context,
    ...
    switch (contact_model_) {
        ...
        case ContactModel::kHydroelastic: {
            if constexpr (scalar_predicate<T>::is_bool) {
                result->surfaces = query_object.ComputeContactSurfaces(get_contact_surface_representation());
                break;
            } else {
        ...
        case ContactModel::kHydroelasticWithFallback: {
            if constexpr (scalar_predicate<T>::is_bool) {
                query_object.ComputeContactSurfacesWithFallback(get_contact_surface_representation(), &result->surfaces,
                                                                &result->point_pairs);
                break;
            } else {
                // TODO(SeanCurtis-TRI): Special case the QueryObject scalar support
    ...
    if constexpr (std::is_same_v<T, double>) {
        if (is_discrete()) {
            const auto* deformable_driver = discrete_update_manager_->deformable_driver();
            if (deformable_driver != nullptr) {
                deformable_driver->CalcDeformableContact(query_object, &result->deformable);
            }
        }
    }
    ...
    const internal::JointLockingCacheData<T>& joint_locking = EvalJointLocking(context);
    if (joint_locking.locked_velocity_indices.empty()) {
        return;
    }
    const auto& geometry_id_to_body_index = geometry_id_to_body_index_;
    const internal::MultibodyTreeTopology& topology = internal_tree().get_topology();
    const std::vector<std::vector<int>>& per_tree_unlocked_indices = joint_locking.unlocked_velocity_indices_per_tree;
    const auto is_irrelevant_geometry = [&geometry_id_to_body_index, &topology,
        ...
        const internal::TreeIndex tree_index = topology.body_to_tree_index(body_index);
        return !topology.tree_has_dofs(tree_index) || per_tree_unlocked_indices[tree_index].size() == 0;
    };
    const auto is_irrelevant_point_pair = [&is_irrelevant_geometry](const PenetrationAsPointPair<T>& point_pair) {
        // Checks whether `point_pair` refers to only zero-dof trees.
        return is_irrelevant_geometry(point_pair.id_A) && is_irrelevant_geometry(point_pair.id_B);
    };
    std::erase_if(result->point_pairs, is_irrelevant_point_pair);
    if constexpr (scalar_predicate<T>::is_bool) {
        const auto is_irrelevant_surface = [&is_irrelevant_geometry](const ContactSurface<T>& surface) {
            // Checks whether `surface` refers to only zero-dof trees.
            return is_irrelevant_geometry(surface.id_M()) && is_irrelevant_geometry(surface.id_N());
        };
...
template <typename T>
const GeometryContactData<T>& MultibodyPlant<T>::EvalGeometryContactData(const Context<T>& context) const {
    return this->get_cache_entry(cache_indices_.geometry_contact_data).template Eval<GeometryContactData<T>>(context);
}

template <typename T>
void MultibodyPlant<T>::CalcJointLocking(const systems::Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::CalcGeneralizedContactForcesContinuous(const Context<T>& context,
                                                               VectorX<T>* tau_contact) const {
    this->ValidateContext(context);
    ...
    const std::vector<SpatialForce<T>>& Fcontact_BBo_W_array = EvalSpatialContactForcesContinuous(context);
    ...
    internal_tree().CalcInverseDynamics(context, zero_vdot, Fcontact_BBo_W_array, tau_array,
                                        true /* Do not compute velocity-dependent terms */, &A_WB_array, &F_BMo_W_array,
                                        tau_contact);
...
template <typename T>
const VectorX<T>& MultibodyPlant<T>::EvalGeneralizedContactForcesContinuous(const systems::Context<T>& context) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(!is_discrete());
    return this->get_cache_entry(cache_indices_.generalized_contact_forces_continuous)
            .template Eval<VectorX<T>>(context);
}

template <typename T>
void MultibodyPlant<T>::CalcSpatialContactForcesContinuous(const drake::systems::Context<T>& context,
                                                           std::vector<SpatialForce<T>>* F_BBo_W_array) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(F_BBo_W_array != nullptr);
    DRAKE_DEMAND(ssize(*F_BBo_W_array) == num_bodies());
    ...
    std::fill(F_BBo_W_array->begin(), F_BBo_W_array->end(), SpatialForce<T>::Zero());

    CalcAndAddSpatialContactForcesContinuous(context, F_BBo_W_array);
}

template <typename T>
const std::vector<SpatialForce<T>>& MultibodyPlant<T>::EvalSpatialContactForcesContinuous(
...
template <typename T>
void MultibodyPlant<T>::CalcAndAddSpatialContactForcesContinuous(const drake::systems::Context<T>& context,
                                                                 std::vector<SpatialForce<T>>* F_BBo_W_array) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(F_BBo_W_array != nullptr);
    DRAKE_DEMAND(ssize(*F_BBo_W_array) == num_bodies());
    ...
    switch (contact_model_) {
        ...
        case ContactModel::kHydroelastic:
            *F_BBo_W_array = EvalHydroelasticContactForcesContinuous(context).F_BBo_W_array;
            break;
...
template <typename T>
void MultibodyPlant<T>::CalcNonContactForcesContinuous(const drake::systems::Context<T>& context,
                                                       MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(forces != nullptr);
...
template <typename T>
void MultibodyPlant<T>::AddInForcesContinuous(const systems::Context<T>& context, MultibodyForces<T>* forces) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(forces != nullptr);
    DRAKE_DEMAND(!is_discrete());

    // Guard against failure to acquire the geometry input deep in the call graph.
    ValidateGeometryInput(context, "You've tried evaluating time derivatives or their residuals.");
    ...
    const std::vector<SpatialForce<T>>& Fcontact_BBo_W_array = EvalSpatialContactForcesContinuous(context);
    for (int i = 0; i < ssize(Fapp_BBo_W_array); ++i) Fapp_BBo_W_array[i] += Fcontact_BBo_W_array[i];
}

template <typename T>
void MultibodyPlant<T>::DoCalcForwardDynamicsDiscrete(const drake::systems::Context<T>& context0,
    ...
    // Guard against failure to acquire the geometry input deep in the call graph.
    ValidateGeometryInput(context0, "You've tried evaluating discrete forward dynamics.");
...
template <typename T>
systems::EventStatus MultibodyPlant<T>::CalcDiscreteStep(const systems::Context<T>& context0,
...
void MultibodyPlant<T>::DeclareInputPorts() {
    // Input "actuation".
    input_port_indices_.actuation = this->DeclareVectorInputPort("actuation", num_actuated_dofs()).get_index();

    // Input "applied_generalized_force".
    input_port_indices_.applied_generalized_force =
            this->DeclareVectorInputPort("applied_generalized_force", num_velocities()).get_index();

    // Input "applied_spatial_force".
    input_port_indices_.applied_spatial_force =
            this->DeclareAbstractInputPort("applied_spatial_force",
    ...
    for (ModelInstanceIndex i(0); i < num_model_instances(); ++i) {
        ...
        input_port_indices_.instance[i].actuation =
                this->DeclareVectorInputPort(fmt::format("{}_actuation", model_instance_name), num_actuated_dofs(i))
                        .get_index();
        ...
        input_port_indices_.instance[i].desired_state =
                this->DeclareVectorInputPort(fmt::format("{}_desired_state", model_instance_name), instance_num_xd)
                        .get_index();
    }
}
...
void MultibodyPlant<T>::DeclareStateUpdate() {
    if (is_discrete()) {
        ...
        // a Forced update.
        this->DeclarePeriodicDiscreteUpdateEvent(time_step_, 0.0, &MultibodyPlant<T>::CalcDiscreteStep);
        this->DeclareForcedDiscreteUpdateEvent(&MultibodyPlant<T>::CalcDiscreteStep);
    }
}

template <typename T>
void MultibodyPlant<T>::DeclareOutputPorts() {
    const DependencyTicket state_ticket = is_discrete() ? this->xd_ticket() : this->kinematics_ticket();
    const DependencyTicket position_ticket = is_discrete() ? this->xd_ticket() : this->q_ticket();

    // Output "state".
    output_port_indices_.state = this->DeclareVectorOutputPort("state", num_multibody_states(),
                                                               &MultibodyPlant::CalcStateOutput, {state_ticket})
                                         .get_index();

    // Output "body_poses".
    output_port_indices_.body_poses =
            this->DeclareAbstractOutputPort("body_poses", std::vector<math::RigidTransform<T>>(num_bodies()),
    ...
    output_port_indices_.body_spatial_velocities =
            this->DeclareAbstractOutputPort("body_spatial_velocities", std::vector<SpatialVelocity<T>>(num_bodies()),
    ...
    // Output "spatial_velocities" (deprecated).
    {
        this->DeprecateOutputPort(
                this->DeclareAbstractOutputPort("spatial_velocities", std::vector<SpatialVelocity<T>>(num_bodies()),
                                                &MultibodyPlant<T>::CalcBodySpatialVelocitiesOutput,
                                                {state_ticket, this->all_parameters_ticket()}),
    ...
    output_port_indices_.body_spatial_accelerations =
            this->DeclareAbstractOutputPort("body_spatial_accelerations",
    ...
    {
        this->DeprecateOutputPort(this->DeclareAbstractOutputPort(
                                          "spatial_accelerations", std::vector<SpatialAcceleration<T>>(num_bodies()),
                                          &MultibodyPlant<T>::CalcBodySpatialAccelerationsOutput,
                                          {this->acceleration_kinematics_cache_entry().ticket()}),
    ...
    output_port_indices_.generalized_acceleration =
            this->DeclareVectorOutputPort("generalized_acceleration", num_velocities(),
    ...
    output_port_indices_.net_actuation =
            this->DeclareVectorOutputPort("net_actuation", num_actuated_dofs(), &MultibodyPlant::CalcNetActuationOutput,
                                          {state_ticket, this->all_input_ports_ticket(), this->all_parameters_ticket()})
                    .get_index();
    ...
    output_port_indices_.reaction_forces =
            this->DeclareAbstractOutputPort("reaction_forces", std::vector<SpatialForce<T>>(num_joints()),
    ...
    output_port_indices_.contact_results =
            this->DeclareAbstractOutputPort("contact_results", ContactResults<T>(),
    ...
    for (ModelInstanceIndex i(0); i < num_model_instances(); ++i) {
        ...
        output_port_indices_.instance[i].state =
                this->DeclareVectorOutputPort(fmt::format("{}_state", model_instance_name), num_multibody_states(i),
        ...
        output_port_indices_.instance[i].generalized_acceleration =
                this->DeclareVectorOutputPort(fmt::format("{}_generalized_acceleration", model_instance_name),
                                              num_velocities(i),
                                              [this, i](const Context<T>& context, BasicVector<T>* output) {
                                                  this->CalcInstanceGeneralizedAccelerationOutput(i, context, output);
                                              },
                                              {this->acceleration_kinematics_cache_entry().ticket()})
                        .get_index();
        ...
        if (is_discrete()) {
            ...
            prerequisites_of_generalized_contact_forces.insert(this->all_parameters_ticket());
        } else {
            prerequisites_of_generalized_contact_forces.insert(
                    this->get_cache_entry(cache_indices_.generalized_contact_forces_continuous).ticket());
        }
        output_port_indices_.instance[i].generalized_contact_forces =
                this->DeclareVectorOutputPort(
                            fmt::format("{}_generalized_contact_forces", model_instance_name), num_velocities(i),
                            [this, i](const systems::Context<T>& context, systems::BasicVector<T>* output) {
                                this->CalcInstanceGeneralizedContactForcesOutput(i, context, output);
                            },
        ...
        output_port_indices_.instance[i].net_actuation =
                this->DeclareVectorOutputPort(
                            fmt::format("{}_net_actuation", model_instance_name), num_actuated_dofs(i),
                            [this, i](const systems::Context<T>& context, systems::BasicVector<T>* output) {
...
void MultibodyPlant<T>::DeclareCacheEntries() {
    ...
    auto& geometry_contact_data_cache_entry = this->DeclareCacheEntry(
            ...
            {position_ticket, this->all_parameters_ticket(), get_geometry_query_input_port().ticket()});
    cache_indices_.geometry_contact_data = geometry_contact_data_cache_entry.cache_index();

    // Cache entry for HydroelasticContactForcesContinuous.
    const bool use_hydroelastic =
            contact_model_ == ContactModel::kHydroelastic || contact_model_ == ContactModel::kHydroelasticWithFallback;
    if constexpr (!std::is_same_v<T, symbolic::Expression>) {
        if (!is_discrete() && use_hydroelastic) {
            auto& hydroelastic_contact_forces_continuous_cache_entry = this->DeclareCacheEntry(
                    std::string("HydroelasticContactForcesContinuous"),
                    internal::HydroelasticContactForcesContinuousCacheData<T>(this->num_bodies()),
                    ...
                    {state_ticket, this->all_parameters_ticket(), get_geometry_query_input_port().ticket()});
    ...
    if (!is_discrete()) {
        auto& contact_results_continuous_cache_entry = this->DeclareCacheEntry(
                std::string("ContactResultsContinuous"), &MultibodyPlant<T>::CalcContactResultsContinuous,
                {state_ticket, this->all_parameters_ticket(), get_geometry_query_input_port().ticket()});
        cache_indices_.contact_results_continuous = contact_results_continuous_cache_entry.cache_index();
    }

    // Cache entry for SpatialContactForcesContinuous.
    if (!is_discrete()) {
        auto& spatial_contact_forces_continuous_cache_entry = this->DeclareCacheEntry(
                "SpatialContactForcesContinuous", std::vector<SpatialForce<T>>(num_bodies()),
                &MultibodyPlant::CalcSpatialContactForcesContinuous,
                {state_ticket, this->all_parameters_ticket(), get_geometry_query_input_port().ticket()});
        cache_indices_.spatial_contact_forces_continuous = spatial_contact_forces_continuous_cache_entry.cache_index();
    }

    // Cache entry for GeneralizedContactForcesContinuous.
    if (!is_discrete()) {
        auto& generalized_contact_forces_continuous_cache_entry =
                this->DeclareCacheEntry("GeneralizedContactForcesContinuous", VectorX<T>(num_velocities()),
                                        &MultibodyPlant::CalcGeneralizedContactForcesContinuous,
                                        {this->cache_entry_ticket(cache_indices_.spatial_contact_forces_continuous),
    ...
    const auto& joint_locking_cache_entry =
            this->DeclareCacheEntry("JointLocking", internal::JointLockingCacheData<T>{},
...
void MultibodyPlant<T>::DeclareParameters() {
    ...
    parameter_indices_.constraint_active_status =
            systems::AbstractParameterIndex{this->DeclareAbstractParameter(drake::Value(map_wrapper))};
}

template <typename T>
void MultibodyPlant<T>::CalcStateOutput(const Context<T>& context, BasicVector<T>* output) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
...
template <typename T>
void MultibodyPlant<T>::CalcInstanceStateOutput(ModelInstanceIndex model_instance,
                                                const Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::CalcInstanceGeneralizedContactForcesOutput(ModelInstanceIndex model_instance,
                                                                   const Context<T>& context,
                                                                   BasicVector<T>* output) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
    ValidateGeometryInput(context, get_generalized_contact_forces_output_port(model_instance));
    // Vector of generalized contact forces for the entire multibody system.
    const VectorX<T>& tau_contact = is_discrete()
                                            ? discrete_update_manager_->EvalContactSolverResults(context).tau_contact
...
template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_applied_generalized_force_input_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    return this->get_input_port(input_port_indices_.applied_generalized_force);
}

template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_actuation_input_port(ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_input_port(input_port_indices_.instance.at(model_instance).actuation);
}

template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_actuation_input_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    return this->get_input_port(input_port_indices_.actuation);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_net_actuation_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
...
const systems::OutputPort<T>& MultibodyPlant<T>::get_net_actuation_output_port(
    ...
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_output_port(output_port_indices_.instance.at(model_instance).net_actuation);
}

template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_desired_state_input_port(ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_input_port(input_port_indices_.instance.at(model_instance).desired_state);
}

template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_applied_spatial_force_input_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
...
template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_state_output_port(ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_output_port(output_port_indices_.instance.at(model_instance).state);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_generalized_acceleration_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    return this->get_output_port(output_port_indices_.generalized_acceleration);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_generalized_acceleration_output_port(
        ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_output_port(output_port_indices_.instance.at(model_instance).generalized_acceleration);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_generalized_contact_forces_output_port(
        ModelInstanceIndex model_instance) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    DRAKE_THROW_UNLESS(model_instance.is_valid());
    DRAKE_THROW_UNLESS(model_instance < num_model_instances());
    return this->get_output_port(output_port_indices_.instance.at(model_instance).generalized_contact_forces);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_contact_results_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    return this->get_output_port(output_port_indices_.contact_results);
}

template <typename T>
const systems::OutputPort<T>& MultibodyPlant<T>::get_reaction_forces_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
...
void MultibodyPlant<T>::DeclareSceneGraphPorts() {
    input_port_indices_.geometry_query =
            this->DeclareAbstractInputPort("geometry_query", Value<geometry::QueryObject<T>>{}).get_index();
    output_port_indices_.geometry_pose =
            this->DeclareAbstractOutputPort("geometry_pose", &MultibodyPlant<T>::CalcGeometryPoseOutput,
...
template <typename T>
void MultibodyPlant<T>::CalcBodyPosesOutput(const Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::CalcBodySpatialVelocitiesOutput(const Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::CalcBodySpatialAccelerationsOutput(const Context<T>& context,
...
template <typename T>
void MultibodyPlant<T>::CalcGeneralizedAccelerationOutput(const Context<T>& context, BasicVector<T>* output) const {
    output->SetFromVector(this->EvalForwardDynamics(context).get_vdot());
}

template <typename T>
void MultibodyPlant<T>::CalcInstanceGeneralizedAccelerationOutput(ModelInstanceIndex model_instance,
                                                                  const Context<T>& context,
                                                                  BasicVector<T>* output) const {
    // The per-instance calc delegates to the full-model calc and then slices it.
    const VectorX<T>& generalized_acceleration = get_generalized_acceleration_output_port().Eval(context);
    output->SetFromVector(this->GetVelocitiesFromArray(model_instance, generalized_acceleration));
}

template <typename T>
const SpatialAcceleration<T>& MultibodyPlant<T>::EvalBodySpatialAccelerationInWorld(const Context<T>& context,
...
template <typename T>
const std::vector<geometry::PenetrationAsPointPair<T>>& MultibodyPlant<T>::EvalPointPairPenetrations(
        const systems::Context<T>& context) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
    switch (contact_model_) {
...
template <typename T>
void MultibodyPlant<T>::CalcGeometryPoseOutput(const Context<T>& context, FramePoseVector<T>* output) const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    this->ValidateContext(context);
    const internal::PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    ...
    for (const auto& it : body_index_to_frame_id_) {
        ...
        output->set_value(body_index_to_frame_id_.at(body_index), pc.get_X_WB(body.mobod_index()));
    }
}

template <typename T>
void MultibodyPlant<T>::CalcReactionForcesOutput(const systems::Context<T>& context,
    ...
    if (is_discrete()) {
        applied_forces = discrete_update_manager_->EvalDiscreteUpdateMultibodyForces(context);
    } else {
    ...
    internal_tree().CalcInverseDynamics(context, vdot, Fapplied_Bo_W_array, tau_applied, &A_WB_vector, &F_BMo_W_vector,
                                        &tau_id);
    ...
    for (JointIndex joint_index : GetJointIndices()) {
        const Joint<T>& joint = get_joint(joint_index);
        const internal::MobilizerIndex mobilizer_index = internal_tree().get_joint_mobilizer(joint_index);
        const internal::Mobilizer<T>& mobilizer = internal_tree().get_mobilizer(mobilizer_index);
        const internal::MobodIndex mobod_index = mobilizer.get_topology().mobod_index;
        ...
        // We verify this:
        DRAKE_DEMAND((Jp_index == F_index && Jc_index == M_index) || (Jp_index == M_index && Jc_index == F_index));

        SpatialForce<T> F_CJc_W;
        if (Jc_index == M_index) {
            // Given we now Mo == Jc and B == C.
            F_CJc_W = F_BMo_W;
        } else if (joint.frame_on_child().index() == mobilizer.inboard_frame().index()) {
            // Given we now Mo == Jc and B == C.
            ...
            const RotationMatrix<T> R_WJp = frame_Jp.CalcRotationMatrixInWorld(context);
...
template <typename T>
const OutputPort<T>& MultibodyPlant<T>::get_body_spatial_velocities_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
    return this->get_output_port(output_port_indices_.body_spatial_velocities);
}

template <typename T>
const OutputPort<T>& MultibodyPlant<T>::get_body_spatial_accelerations_output_port() const {
    DRAKE_MBP_THROW_IF_NOT_FINALIZED();
...
const OutputPort<T>& MultibodyPlant<T>::get_geometry_pose_output_port() const {
    return systems::System<T>::get_output_port(output_port_indices_.geometry_pose);
}

template <typename T>
const systems::InputPort<T>& MultibodyPlant<T>::get_geometry_query_input_port() const {
    return this->get_input_port(input_port_indices_.geometry_query);
}

template <typename T>
const OutputPort<T>& MultibodyPlant<T>::get_deformable_body_configuration_output_port() const {
    const DeformableModel<T>* deformable_model = physical_models_->deformable_model();
    DRAKE_DEMAND(deformable_model != nullptr);
    return systems::System<T>::get_output_port(deformable_model->configuration_output_port_index());
}
...
template <typename T>
void MultibodyPlant<T>::RemoveUnsupportedScalars(const internal::ScalarConvertibleComponent<T>& component) {
    systems::SystemScalarConverter& scalar_converter = this->get_mutable_system_scalar_converter();
...
template <typename T>
std::vector<std::set<BodyIndex>> MultibodyPlant<T>::FindSubgraphsOfWeldedBodies() const {
    return internal_tree().multibody_graph().FindSubgraphsOfWeldedBodies();
}

template <typename T>
bool MultibodyPlant<T>::is_gravity_enabled(ModelInstanceIndex model_instance) const {
...
template <typename T>
void MultibodyPlant<T>::set_gravity_enabled(ModelInstanceIndex model_instance, bool is_enabled) {
    DRAKE_MBP_THROW_IF_FINALIZED();
    if (model_instance >= num_model_instances()) {
        throw std::logic_error("Model instance index is invalid.");
    }
...
DeformableModel<T>& MultibodyPlant<T>::AddDeformableModel() {
    ...
    return physical_models_->AddDeformableModel(std::make_unique<DeformableModel<T>>(this));
}

template <typename T>
T MultibodyPlant<T>::StribeckModel::ComputeFrictionCoefficient(const T& speed_BcAc,
...
AddMultibodyPlantSceneGraphResult<T> AddMultibodyPlantSceneGraph(systems::DiagramBuilder<T>* builder,
    ...
    builder->Connect(plant_ptr->get_geometry_pose_output_port(),
                     scene_graph_ptr->get_source_pose_port(plant_ptr->get_source_id().value()));
    builder->Connect(scene_graph_ptr->get_query_output_port(), plant_ptr->get_geometry_query_input_port());
    builder->Connect(plant_ptr->get_deformable_body_configuration_output_port(),
                     scene_graph_ptr->get_source_configuration_port(plant_ptr->get_source_id().value()));
...
AddMultibodyPlantSceneGraphResult<T> AddMultibodyPlantSceneGraph(systems::DiagramBuilder<T>* builder,
    ...
    plant->set_name("plant");
    return AddMultibodyPlantSceneGraph(builder, std::move(plant), std::move(scene_graph));
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((
        /* Use static_cast to disambiguate the two different overloads. */
        static_cast<AddMultibodyPlantSceneGraphResult<T> (*)(
                systems::DiagramBuilder<T>*, double, std::unique_ptr<geometry::SceneGraph<T>>)>(
                &AddMultibodyPlantSceneGraph),
        /* Use static_cast to disambiguate the two different overloads. */
        static_cast<AddMultibodyPlantSceneGraphResult<T> (*)(systems::DiagramBuilder<T>*,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::MultibodyPlant);
