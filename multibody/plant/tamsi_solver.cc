...
namespace internal {
template <typename T>
std::optional<T> TalsLimiter<T>::CalcAlpha(const Eigen::Ref<const Vector2<T>>& v,
    ...
    if (x < 1.0) {
        ...
        if (CrossesTheStictionRegion(v, dv, v_dot_dv, dv_norm, dv_norm2, epsilon_v, v_stiction, &alpha)) {
            return alpha;
        }
        ...
        if (cos1 > cos_theta_max) {
            ...
            std::optional<T> maybe_alpha = SolveQuadraticForTheSmallestPositiveRoot(a, b, c);
...
template <typename T>
bool TalsLimiter<T>::CrossesTheStictionRegion(const Eigen::Ref<const Vector2<T>>& v,
                                              const Eigen::Ref<const Vector2<T>>& dv,
                                              const T& v_dot_dv,
                                              const T& dv_norm,
                                              const T& dv_norm2,
...
template <typename T>
std::optional<T> TalsLimiter<T>::SolveQuadraticForTheSmallestPositiveRoot(const T& a, const T& b, const T& c) {
    using std::abs;
    using std::max;
    using std::min;
    using std::sqrt;
...
template <typename T>
void TamsiSolver<T>::SetOneWayCoupledProblemData(EigenPtr<const MatrixX<T>> M,
...
template <typename T>
void TamsiSolver<T>::SetTwoWayCoupledProblemData(EigenPtr<const MatrixX<T>> M,
                                                 EigenPtr<const MatrixX<T>> Jn,
                                                 EigenPtr<const MatrixX<T>> Jt,
                                                 EigenPtr<const VectorX<T>> p_star,
                                                 ...
                                                 EigenPtr<const VectorX<T>> mu) {
    DRAKE_DEMAND(M && Jn && Jt && p_star && fn0 && stiffness && dissipation && mu);
    nc_ = fn0->size();
    DRAKE_THROW_UNLESS(p_star->size() == nv_);
    DRAKE_THROW_UNLESS(M->rows() == nv_ && M->cols() == nv_);
    DRAKE_THROW_UNLESS(Jn->rows() == nc_ && Jn->cols() == nv_);
    ...
    // Keep references to the problem data.
    problem_data_aliases_.SetTwoWayCoupledData(M, Jn, Jt, p_star, fn0, stiffness, dissipation, mu);
    variable_size_workspace_.ResizeIfNeeded(nc_, nv_);
}
...
template <typename T>
void TamsiSolver<T>::CalcFrictionForcesGradient(const Eigen::Ref<const VectorX<T>>& fn,
...
template <typename T>
void TamsiSolver<T>::CalcNormalForces(const Eigen::Ref<const VectorX<T>>& vn,
    ...
    const VectorX<T> dfn_dvn = -(dissipation.array() * H_damping_factor.array() * undamped_fn.array() +
...
void TamsiSolver<T>::CalcJacobian(const Eigen::Ref<const MatrixX<T>>& M,
    ...
    for (int ic = 0; ic < nc; ++ic) {  // Index ic scans contact points.
        ...
        if (has_two_way_coupling()) {
            ...
            Gt.block(ik, 0, 1, nv) -= mu_vt(ic) * t_hat_ic(0) * Gn.block(ic, 0, 1, nv);
            Gt.block(ik + 1, 0, 1, nv) -= mu_vt(ic) * t_hat_ic(1) * Gn.block(ic, 0, 1, nv);
        }
    }
...
template <typename T>
std::optional<T> TamsiSolver<T>::CalcAlpha(const Eigen::Ref<const VectorX<T>>& vt,
    ...
    for (int ic = 0; ic < nc_; ++ic) {  // Index ic scans contact points.
        ...
        const std::optional<T> tals_alpha = internal::TalsLimiter<T>::CalcAlpha(
                vt_ic, dvt_ic, cos_theta_max_, v_stiction, parameters_.relative_tolerance);
...
template <typename T>
TamsiSolverResult TamsiSolver<T>::SolveWithGuess(double dt, const VectorX<T>& v_guess) const {
    ...
    const double v_contact_tolerance = parameters_.relative_tolerance * parameters_.stiction_tolerance;
    ...
    for (int iter = 0; iter < max_iterations; ++iter) {
        ...
        residual = M * v - p_star - dt * Jn.transpose() * fn - dt * Jt.transpose() * ft;
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(struct ::drake::multibody::internal::TalsLimiter);

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::TamsiSolver);
