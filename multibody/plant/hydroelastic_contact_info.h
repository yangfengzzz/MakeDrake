...
#include "multibody/plant/hydroelastic_quadrature_point_data.h"
...
public:
    ...
    HydroelasticContactInfo(const geometry::ContactSurface<T>* contact_surface,
        ...
        : contact_surface_(contact_surface), F_Ac_W_(F_Ac_W), quadrature_point_data_(std::move(quadrature_point_data)) {
    ...
    HydroelasticContactInfo(std::unique_ptr<geometry::ContactSurface<T>> contact_surface,
        ...
        DRAKE_DEMAND(std::get<std::unique_ptr<geometry::ContactSurface<T>>>(contact_surface_) != nullptr);
    }
    // @}
    ...
    HydroelasticContactInfo& operator=(const HydroelasticContactInfo& info) {
        contact_surface_ = std::make_unique<geometry::ContactSurface<T>>(info.contact_surface());
    ...
    const geometry::ContactSurface<T>& contact_surface() const {
        if (std::holds_alternative<const geometry::ContactSurface<T>*>(contact_surface_)) {
            return *std::get<const geometry::ContactSurface<T>*>(contact_surface_);
        } else {
            return *std::get<std::unique_ptr<geometry::ContactSurface<T>>>(contact_surface_);
        }
    }
...
private:
    ...
    std::variant<const geometry::ContactSurface<T>*, std::unique_ptr<geometry::ContactSurface<T>>> contact_surface_;
