...
#include "multibody/fem/deformable_body_config.h"
#include "multibody/fem/fem_model.h"
#include "multibody/plant/constraint_specs.h"
#include "multibody/plant/deformable_ids.h"
#include "multibody/plant/force_density_field.h"
#include "multibody/plant/physical_model.h"
...
public:
    ...
    bool HasConstraint(DeformableBodyId id) const { return body_id_to_constraint_ids_.contains(id); }
    ...
    */
    ...
    bool is_empty() const { return body_ids_.empty() && force_densities_.empty(); }
...
private:
    ...
    std::unique_ptr<PhysicalModel<double>> CloneToDouble(MultibodyPlant<double>* plant) const final;

    /* Since %DeformableModel is only cloneable to AutoDiffXd if the model is
     empty, the clone simply returns an empty model. */
    std::unique_ptr<PhysicalModel<AutoDiffXd>> CloneToAutoDiffXd(MultibodyPlant<AutoDiffXd>* plant) const final;
    ...
    typename std::enable_if_t<std::is_same_v<T1, double>, void> BuildLinearVolumetricModel(
    ...
    typename std::enable_if_t<std::is_same_v<T1, double>, void> BuildLinearVolumetricModelHelper(
            DeformableBodyId id, const geometry::VolumeMesh<double>& mesh, const fem::DeformableBodyConfig<T>& config);
    ...
    void CopyVertexPositions(const systems::Context<T>& context, AbstractValue* output) const;

    /* Helper to throw a useful message if a deformable body with the given `id`
     doesn't exist. */
    void ThrowUnlessRegistered(const char* function_name, DeformableBodyId id) const;
    ...
    std::unordered_map<DeformableBodyId, systems::DiscreteStateIndex> discrete_state_indexes_;
    std::unordered_map<DeformableBodyId, geometry::GeometryId> body_id_to_geometry_id_;
    std::unordered_map<geometry::GeometryId, DeformableBodyId> geometry_id_to_body_id_;
    std::unordered_map<DeformableBodyId, std::unique_ptr<fem::FemModel<T>>> fem_models_;
    ...
    std::vector<std::vector<const ForceDensityField<T>*>> body_index_to_force_densities_;
    std::unordered_map<DeformableBodyId, std::vector<MultibodyConstraintId>> body_id_to_constraint_ids_;
    ...
    std::map<MultibodyConstraintId, internal::DeformableRigidFixedConstraintSpec> fixed_constraint_specs_;
