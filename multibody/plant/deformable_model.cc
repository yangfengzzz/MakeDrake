...
#include "multibody/fem/corotated_model.h"
#include "multibody/fem/fem_state.h"
#include "multibody/fem/linear_constitutive_model.h"
#include "multibody/fem/linear_corotated_model.h"
#include "multibody/fem/linear_simplex_element.h"
#include "multibody/fem/simplex_gaussian_quadrature.h"
#include "multibody/fem/volumetric_model.h"
...
template <typename T>
DeformableModel<T>::DeformableModel(MultibodyPlant<T>* plant) : PhysicalModel<T>(plant) {}
...
DeformableBodyId DeformableModel<T>::RegisterDeformableBody(
    ...
    if constexpr (std::is_same_v<T, double>) {
        ...
        if (geometry_instance->illustration_properties() == nullptr) {
            geometry_instance->set_illustration_properties(geometry::IllustrationProperties{});
        }
        GeometryId geometry_id = scene_graph.RegisterDeformableGeometry(source_id, world_frame_id,
                                                                        std::move(geometry_instance), resolution_hint);
        /* Record the reference positions. */
        const geometry::SceneGraphInspector<T>& inspector = scene_graph.model_inspector();
        const geometry::VolumeMesh<double>* mesh_G = inspector.GetReferenceMesh(geometry_id);
        DRAKE_DEMAND(mesh_G != nullptr);
        const math::RigidTransform<double>& X_WG = inspector.GetPoseInFrame(geometry_id);
...
template <typename T>
void DeformableModel<T>::SetWallBoundaryCondition(DeformableBodyId id, const Vector3<T>& p_WQ, const Vector3<T>& n_W) {
    this->ThrowIfSystemResourcesDeclared(__func__);
    ...
    for (int n = 0; n < num_nodes; ++n) {
        ...
        if (is_inside_wall(p_WV)) {
            ...
            bc.AddBoundaryCondition(fem::FemNodeIndex(n), {p_WV, Vector3<T>::Zero(), Vector3<T>::Zero()});
        }
    }
...
template <typename T>
MultibodyConstraintId DeformableModel<T>::AddFixedConstraint(DeformableBodyId body_A_id,
    ...
    internal::DeformableRigidFixedConstraintSpec spec{body_A_id, body_B.index(), {}, {}, constraint_id};
    ...
    auto instance = std::make_unique<GeometryInstance>(X_AG, shape.Clone(), "rigid shape");
    geometry::GeometryId geometry_id = scene_graph.RegisterAnchoredGeometry(source_id, std::move(instance));
    scene_graph.AssignRole(source_id, geometry_id, geometry::ProximityProperties());
    auto context = scene_graph.CreateDefaultContext();
    auto query = scene_graph.get_query_output_port().Eval<geometry::QueryObject<double>>(*context);
    /* The deformable mesh in its geometry frame. */
    const geometry::VolumeMesh<double>* mesh_A =
            this->mutable_scene_graph().model_inspector().GetReferenceMesh(GetGeometryId(body_A_id));
    int vertex_index = 0;
    for (const Vector3<double>& p_APi : mesh_A->vertices()) {
        /* Note that `shape` is also registered in the A frame in the throw-away
         scene graph. */
        const std::vector<geometry::SignedDistanceToPoint<double>> signed_distances =
...
template <typename T>
systems::DiscreteStateIndex DeformableModel<T>::GetDiscreteStateIndex(DeformableBodyId id) const {
...
template <typename T>
void DeformableModel<T>::AddExternalForce(std::unique_ptr<ForceDensityField<T>> force_density) {
    this->ThrowIfSystemResourcesDeclared(__func__);
    ThrowIfNotDouble(__func__);
...
template <typename T>
const std::vector<const ForceDensityField<T>*>& DeformableModel<T>::GetExternalForces(DeformableBodyId id) const {
    this->ThrowIfSystemResourcesNotDeclared(__func__);
    ThrowUnlessRegistered(__func__, id);
...
template <typename T>
const fem::FemModel<T>& DeformableModel<T>::GetFemModel(DeformableBodyId id) const {
    ThrowUnlessRegistered(__func__, id);
    return *fem_models_.at(id);
}

template <typename T>
const VectorX<T>& DeformableModel<T>::GetReferencePositions(DeformableBodyId id) const {
    ThrowUnlessRegistered(__func__, id);
    return reference_positions_.at(id);
}

template <typename T>
DeformableBodyId DeformableModel<T>::GetBodyId(DeformableBodyIndex index) const {
    this->ThrowIfSystemResourcesNotDeclared(__func__);
...
template <typename T>
DeformableBodyIndex DeformableModel<T>::GetBodyIndex(DeformableBodyId id) const {
    this->ThrowIfSystemResourcesNotDeclared(__func__);
    ThrowUnlessRegistered(__func__, id);
...
template <typename T>
DeformableBodyId DeformableModel<T>::GetBodyId(geometry::GeometryId geometry_id) const {
...
template <typename T>
std::unique_ptr<PhysicalModel<double>> DeformableModel<T>::CloneToDouble(MultibodyPlant<double>* plant) const {
...
template <typename T>
std::unique_ptr<PhysicalModel<AutoDiffXd>> DeformableModel<T>::CloneToAutoDiffXd(
        MultibodyPlant<AutoDiffXd>* plant) const {
    return std::make_unique<DeformableModel<AutoDiffXd>>(plant);
}

template <typename T>
std::unique_ptr<PhysicalModel<symbolic::Expression>> DeformableModel<T>::CloneToSymbolic(
...
typename std::enable_if_t<std::is_same_v<T1, double>, void> DeformableModel<T>::BuildLinearVolumetricModel(
        DeformableBodyId id, const geometry::VolumeMesh<double>& mesh, const fem::DeformableBodyConfig<T>& config) {
    if (fem_models_.find(id) != fem_models_.end()) {
        throw std::logic_error("An FEM model with id: " + to_string(id) + " already exists.");
    }
    switch (config.material_model()) {
        case MaterialModel::kLinear:
            BuildLinearVolumetricModelHelper<fem::internal::LinearConstitutiveModel>(id, mesh, config);
            break;
        case MaterialModel::kCorotated:
            BuildLinearVolumetricModelHelper<fem::internal::CorotatedModel>(id, mesh, config);
            break;
        case MaterialModel::kLinearCorotated:
            BuildLinearVolumetricModelHelper<fem::internal::LinearCorotatedModel>(id, mesh, config);
            break;
    }
}
...
typename std::enable_if_t<std::is_same_v<T1, double>, void> DeformableModel<T>::BuildLinearVolumetricModelHelper(
        DeformableBodyId id, const geometry::VolumeMesh<double>& mesh, const fem::DeformableBodyConfig<T>& config) {
    constexpr int kNaturalDimension = 3;
    constexpr int kSpatialDimension = 3;
    constexpr int kQuadratureOrder = 1;
    using QuadratureType = fem::internal::SimplexGaussianQuadrature<kNaturalDimension, kQuadratureOrder>;
    constexpr int kNumQuads = QuadratureType::num_quadrature_points;
    using IsoparametricElementType =
            fem::internal::LinearSimplexElement<T, kNaturalDimension, kSpatialDimension, kNumQuads>;
    using ConstitutiveModelType = Model<T, kNumQuads>;
    static_assert(
            std::is_base_of_v<
                    fem::internal::ConstitutiveModel<ConstitutiveModelType, typename ConstitutiveModelType::Traits>,
                    ConstitutiveModelType>,
            "The template parameter 'Model' must be derived from "
            "ConstitutiveModel.");
    using FemElementType =
            fem::internal::VolumetricElement<IsoparametricElementType, QuadratureType, ConstitutiveModelType>;
    using FemModelType = fem::internal::VolumetricModel<FemElementType>;

    const fem::DampingModel<T> damping_model(config.mass_damping_coefficient(), config.stiffness_damping_coefficient());

    auto fem_model = std::make_unique<FemModelType>();
    ConstitutiveModelType constitutive_model(config.youngs_modulus(), config.poissons_ratio());
    typename FemModelType::VolumetricBuilder builder(fem_model.get());
    builder.AddLinearTetrahedralElements(mesh, constitutive_model, config.mass_density(), damping_model);
    builder.Build();
...
void DeformableModel<T>::DoDeclareSystemResources() {
    if (!is_empty()) {
        if (this->plant()->get_discrete_contact_solver() != DiscreteContactSolver::kSap) {
            throw std::runtime_error(
    ...
    for (const auto& [deformable_id, fem_model] : fem_models_) {
        std::unique_ptr<fem::FemState<T>> default_fem_state = fem_model->MakeFemState();
        ...
        model_state.segment(num_dofs, num_dofs) = default_fem_state->GetVelocities();
        model_state.tail(num_dofs) = default_fem_state->GetAccelerations();
        discrete_state_indexes_.emplace(deformable_id, this->DeclareDiscreteState(model_state));
    }
    ...
    for (const auto& [deformable_id, fem_model] : fem_models_) {
        ...
        auto gravity_force = std::make_unique<GravityForceField<T>>(gravity, density);
    ...
    /* Declare cache entries and input ports for force density fields that need
     them. */
    for (std::unique_ptr<ForceDensityField<T>>& force_density : force_densities_) {
...
void DeformableModel<T>::DoDeclareSceneGraphPorts() {
    ...
    configuration_output_port_index_ =
            this->DeclareAbstractOutputPort(
                        ...
                        []() {
                            return AbstractValue::Make<geometry::GeometryConfigurationVector<T>>();
                        },
...
template <typename T>
void DeformableModel<T>::CopyVertexPositions(const systems::Context<T>& context, AbstractValue* output) const {
    auto& output_value = output->get_mutable_value<geometry::GeometryConfigurationVector<T>>();
    output_value.clear();
    for (const auto& [body_id, geometry_id] : body_id_to_geometry_id_) {
        ...
        VectorX<T> vertex_positions = context.get_discrete_state(discrete_state_index).value().head(num_dofs);
        output_value.set_value(geometry_id, std::move(vertex_positions));
    }
}

template <typename T>
void DeformableModel<T>::ThrowUnlessRegistered(const char* function_name, DeformableBodyId id) const {
    if (fem_models_.find(id) == fem_models_.end()) {
        throw std::logic_error(
                fmt::format("{}(): No deformable body with id {} has been registered.", function_name, id));
    }
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::DeformableModel);
