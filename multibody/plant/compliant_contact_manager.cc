...
#include "multibody/plant/tamsi_driver.h"
#include "multibody/triangle_quadrature/gaussian_triangle_quadrature_rule.h"
...
template <typename T>
AccelerationsDueNonConstraintForcesCache<T>::AccelerationsDueNonConstraintForcesCache(
...
void CompliantContactManager<T>::DoDeclareCacheEntries() {
    ...
    AccelerationsDueNonConstraintForcesCache<T> non_constraint_forces_accelerations(
    ...
    const auto& non_constraint_forces_accelerations_cache_entry = this->DeclareCacheEntry(
            "Non-constraint forces and induced accelerations.",
            systems::ValueProducer(this, non_constraint_forces_accelerations,
                                   &CompliantContactManager<T>::CalcAccelerationsDueToNonConstraintForcesCache),
    ...
    cache_indexes_.non_constraint_forces_accelerations = non_constraint_forces_accelerations_cache_entry.cache_index();
...
template <typename T>
VectorX<T> CompliantContactManager<T>::CalcEffectiveDamping(const systems::Context<T>& context) const {
    const VectorX<T> diagonal_inertia =
            plant().EvalReflectedInertiaCache(context) + plant().EvalJointDampingCache(context) * plant().time_step();
    return diagonal_inertia;
}

template <typename T>
void CompliantContactManager<T>::CalcAccelerationsDueToNonConstraintForcesCache(
        const systems::Context<T>& context, AccelerationsDueNonConstraintForcesCache<T>* forward_dynamics_cache) const {
    ...
    this->CalcNonContactForces(context, /* include_joint_limit_penalty_forces */ false,
    ...
    // damping.
    this->internal_tree().CalcArticulatedBodyInertiaCache(context, diagonal_inertia, &forward_dynamics_cache->abic);
    this->internal_tree().CalcArticulatedBodyForceBias(context, forward_dynamics_cache->abic,
                                                       &forward_dynamics_cache->Zb_Bo_W);
    this->internal_tree().CalcArticulatedBodyForceCache(context, forward_dynamics_cache->abic,
                                                        forward_dynamics_cache->Zb_Bo_W, forward_dynamics_cache->forces,
                                                        &forward_dynamics_cache->aba_forces);
    this->internal_tree().CalcArticulatedBodyAccelerations(
            context, forward_dynamics_cache->abic, forward_dynamics_cache->aba_forces, &forward_dynamics_cache->ac);
}

template <typename T>
const multibody::internal::AccelerationKinematicsCache<T>&
CompliantContactManager<T>::EvalAccelerationsDueToNonConstraintForcesCache(const systems::Context<T>& context) const {
    return plant()
...
template <typename T>
void CompliantContactManager<T>::DoCalcContactSolverResults(const systems::Context<T>& context,
...
template <typename T>
std::unique_ptr<DiscreteUpdateManager<double>> CompliantContactManager<T>::CloneToDouble() const {
...
template <typename T>
std::unique_ptr<DiscreteUpdateManager<AutoDiffXd>> CompliantContactManager<T>::CloneToAutoDiffXd() const {
    // Create a manager with default SAP parameters.
...
template <typename T>
std::unique_ptr<DiscreteUpdateManager<symbolic::Expression>> CompliantContactManager<T>::CloneToSymbolic() const {
    // Create a manager with default SAP parameters.
    auto clone = std::make_unique<CompliantContactManager<symbolic::Expression>>();
    // N.B. we should copy/clone all members except for those overwritten in
    // ExtractModelInfo and DeclareCacheEntries.
    // E.g. SapParameters for SapDriver won't be the same after the clone.
    return clone;
...
void CompliantContactManager<T>::DoCalcAccelerationKinematicsCache(
        const systems::Context<T>& context0, multibody::internal::AccelerationKinematicsCache<T>* ac) const {
    // Current state.
    const VectorX<T>& x0 = context0.get_discrete_state(this->multibody_state_index()).value();
    const auto v0 = x0.bottomRows(plant().num_velocities());

    // Next state.
    const ContactSolverResults<T>& results = this->EvalContactSolverResults(context0);
    const VectorX<T>& v_next = results.v_next;

    ac->get_mutable_vdot() = (v_next - v0) / plant().time_step();

    this->internal_tree().CalcSpatialAccelerationsFromVdot(context0, plant().EvalPositionKinematics(context0),
                                                           plant().EvalVelocityKinematics(context0), ac->get_vdot(),
                                                           &ac->get_mutable_A_WB_pool());
}

template <typename T>
void CompliantContactManager<T>::DoCalcDiscreteUpdateMultibodyForces(const systems::Context<T>& context,
                                                                     MultibodyForces<T>* forces) const {
    // Thus far only TAMSI and SAP are supported. Verify this is true.
    DRAKE_DEMAND(plant().get_discrete_contact_solver() == DiscreteContactSolver::kSap ||
...
template <typename T>
void CompliantContactManager<T>::DoCalcActuation(const systems::Context<T>& context, VectorX<T>* actuation) const {
    // Thus far only TAMSI and SAP are supported. Verify this is true.
    DRAKE_DEMAND(plant().get_discrete_contact_solver() == DiscreteContactSolver::kSap ||
                 plant().get_discrete_contact_solver() == DiscreteContactSolver::kTamsi);
