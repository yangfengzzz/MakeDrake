...
CorotatedModel<T, num_locations>::CorotatedModel(const T& youngs_modulus, const T& poissons_ratio)
...
template <typename T, int num_locations>
void CorotatedModel<T, num_locations>::CalcElasticEnergyDensityImpl(const Data& data,
...
template <typename T, int num_locations>
void CorotatedModel<T, num_locations>::CalcFirstPiolaStressImpl(const Data& data,
...
void CorotatedModel<T, num_locations>::CalcFirstPiolaStressDerivativeImpl(
    ...
    for (int i = 0; i < num_locations; ++i) {
        ...
        const Matrix3<T>& JFinvT = data.JFinvT()[i];
        const Vector<T, 3 * 3>& flat_JFinvT = Eigen::Map<const Vector<T, 3 * 3>>(JFinvT.data(), 3 * 3);
