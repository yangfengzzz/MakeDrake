...
#include "multibody/fem/fem_element.h"
#include "multibody/fem/isoparametric_element.h"
#include "multibody/fem/quadrature.h"
...
template <typename T>
void PerformDoubleTensorContraction(const Eigen::Ref<const Eigen::Matrix<T, 9, 9>>& A,
                                    const Eigen::Ref<const Vector3<T>>& u,
                                    const Eigen::Ref<const Vector3<T>>& v,
                                    EigenPtr<Matrix3<T>> B) {
    B->setZero();
    for (int l = 0; l < 3; ++l) {
        for (int j = 0; j < 3; ++j) {
...
template <typename ConstitutiveModelType, int num_dofs, int num_quadrature_points>
...
template <class IsoparametricElementType, class QuadratureType, class ConstitutiveModelType>
class VolumetricElement;

/* The traits class for volumetric elasticity FEM element. */
template <class IsoparametricElementType, class QuadratureType, class ConstitutiveModelType>
struct FemElementTraits<VolumetricElement<IsoparametricElementType, QuadratureType, ConstitutiveModelType>> {
    ...
    static_assert(is_constitutive_model<ConstitutiveModelType>::value,
    ...
    static_assert(std::is_same_v<typename IsoparametricElementType::T, typename ConstitutiveModelType::T>,
    ...
    static_assert(QuadratureType::num_quadrature_points == IsoparametricElementType::num_sample_locations,
    ...
    static_assert(QuadratureType::num_quadrature_points == ConstitutiveModelType::num_locations,
    ...
    static_assert(IsoparametricElementType::natural_dimension == QuadratureType::natural_dimension,
    ...
    static constexpr int num_quadrature_points = QuadratureType::num_quadrature_points;
    ...
    using Data = VolumetricElementData<ConstitutiveModelType, num_dofs, num_quadrature_points>;
};
...
template <class IsoparametricElementType, class QuadratureType, class ConstitutiveModelType>
class VolumetricElement
    : public FemElement<VolumetricElement<IsoparametricElementType, QuadratureType, ConstitutiveModelType>> {
public:
    ...
    VolumetricElement(const std::array<FemNodeIndex, num_nodes>& node_indices,
                      ...
                      const Eigen::Ref<const Eigen::Matrix<T, 3, num_nodes>>& reference_positions,
                      T density,
                      DampingModel<T> damping_model)
        : FemElement<ElementType>(node_indices, std::move(constitutive_model), std::move(damping_model)),
          density_(std::move(density)) {
        DRAKE_DEMAND(density_ > 0);
        /* Computes the Jacobian of the change of variable function X(ξ). */
        const std::array<Eigen::Matrix<T, 3, natural_dimension>, num_quadrature_points> dXdxi =
                isoparametric_element_.CalcJacobian(reference_positions);
        ...
        const auto dSdX = isoparametric_element_.CalcGradientInSpatialCoordinates(reference_positions);
        for (int q = 0; q < num_quadrature_points; ++q) {
...
private:
    ...
    void AddNegativeElasticForce(const Data& data, EigenPtr<Vector<T, num_dofs>> neg_force) const {
        DRAKE_ASSERT(neg_force != nullptr);
        auto neg_force_matrix = Eigen::Map<Eigen::Matrix<T, 3, num_nodes>>(neg_force->data(), 3, num_nodes);
        for (int q = 0; q < num_quadrature_points; ++q) {
    ...
    /* Adds the negative damping force on the nodes of this element into the given
     ...
     @param[in, out] neg_force  The negative force vector to be added to.
     @pre neg_force != nullptr. */
    void AddNegativeDampingForce(const Data& data, EigenPtr<Vector<T, num_dofs>> neg_force) const {
        DRAKE_ASSERT(neg_force != nullptr);
        Eigen::Matrix<T, num_dofs, num_dofs> damping_matrix = Eigen::Matrix<T, num_dofs, num_dofs>::Zero();
    ...
    /* Adds a scaled derivative of the elastic force on the nodes of this
     ...
     @pre K != nullptr. */
    void AddScaledElasticForceDerivative(const Data& data,
                                         const T& scale,
            ...
            for (int a = 0; a < num_nodes; ++a) {
                for (int b = 0; b < num_nodes; ++b) {
                    /* Note that the scale is negated here because the tensor contraction
                     ...
                     force derivative. */
                    PerformDoubleTensorContraction<T>(data.dPdF[q], dSdX_transpose_[q].col(a),
    ...
    void DoCalcInverseDynamics(const Data& data, EigenPtr<Vector<T, num_dofs>> residual) const {
    ...
    void DoAddScaledStiffnessMatrix(const Data& data,
                                    const T& scale,
                                    EigenPtr<Eigen::Matrix<T, num_dofs, num_dofs>> K) const {
    ...
    void DoAddScaledMassMatrix(const Data&, const T& scale, EigenPtr<Eigen::Matrix<T, num_dofs, num_dofs>> M) const {
    ...
    Data DoComputeData(const FemState<T>& state) const {
        ...
        data.element_q0 = this->ExtractElementDofs(state.GetPreviousStepPositions());
        ...
        const auto& element_q_reshaped =
                Eigen::Map<const Eigen::Matrix<T, 3, num_nodes>>(data.element_q.data(), 3, num_nodes);
        data.quadrature_positions = isoparametric_element_.template InterpolateNodalValues<3>(element_q_reshaped);

        data.deformation_gradient_data.UpdateData(CalcDeformationGradient(data.element_q),
                                                  CalcDeformationGradient(data.element_q0));
        this->constitutive_model().CalcElasticEnergyDensity(data.deformation_gradient_data, &data.Psi);
        this->constitutive_model().CalcFirstPiolaStress(data.deformation_gradient_data, &data.P);
        this->constitutive_model().CalcFirstPiolaStressDerivative(data.deformation_gradient_data, &data.dPdF);
        return data;
    }
    ...
    void DoAddScaledExternalForces(const Data& data,
        ...
        const std::array<Vector<T, 3>, num_quadrature_points>& quadrature_positions = data.quadrature_positions;
        const std::array<Vector<T, num_nodes>, num_quadrature_points>& S = isoparametric_element_.GetShapeFunctions();
        ...
        for (int q = 0; q < num_quadrature_points; ++q) {
            ...
            for (const multibody::ForceDensityField<T>* force_density : plant_data.force_density_fields) {
                DRAKE_ASSERT(force_density != nullptr);
                const T change_of_volume =
                        force_density->density_type() == multibody::ForceDensityType::kPerReferenceVolume
                                ? 1.0
                                : deformation_gradients[q].determinant();
                scaled_force += scale * force_density->EvaluateAt(plant_data.plant_context, quadrature_positions[q]) *
    ...
    std::array<Matrix3<T>, num_quadrature_points> CalcDeformationGradient(
        ...
        const auto& element_q_reshaped =
                Eigen::Map<const Eigen::Matrix<T, 3, num_nodes>>(element_q.data(), 3, num_nodes);
        const std::array<typename IsoparametricElementType::JacobianMatrix, num_quadrature_points> dxdxi =
                isoparametric_element_.CalcJacobian(element_q_reshaped);
    ...
    Eigen::Matrix<T, num_dofs, num_dofs> PrecomputeMassMatrix() const {
        Eigen::Matrix<T, num_dofs, num_dofs> mass_matrix = Eigen::Matrix<T, num_dofs, num_dofs>::Zero();
        const std::array<Vector<T, num_nodes>, num_quadrature_points>& S = isoparametric_element_.GetShapeFunctions();
        ...
        /* weighted_SST = weighted_S * Sᵀ. The ij-th entry approximates the integral
         ∫SᵢSⱼ dX */
        const Eigen::Matrix<T, num_nodes, num_nodes> weighted_SST = weighted_S * S_mat.transpose();
        constexpr int kDim = 3;
        for (int a = 0; a < num_nodes; ++a) {
            for (int b = 0; b < num_nodes; ++b) {
                mass_matrix.template block<kDim, kDim>(kDim * a, kDim * b) =
                        Eigen::Matrix<T, kDim, kDim>::Identity() * weighted_SST(a, b) * density_;
            }
        }
        return mass_matrix;
    }
    ...
    std::array<Eigen::Matrix<T, natural_dimension, 3>, num_quadrature_points> dxidX_;
    ...
    std::array<Eigen::Matrix<T, 3, num_nodes>, num_quadrature_points> dSdX_transpose_;
