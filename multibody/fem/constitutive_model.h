...
public:
    ...
    /* "Calc" Methods
     ...
     @pre `Psi != nullptr`. */
    void CalcElasticEnergyDensity(const Data& data, std::array<T, num_locations>* Psi) const {
    ...
    /* Calculates the First Piola stress in unit of Pa, given the deformation
     ...
     @pre `P != nullptr`. */
    void CalcFirstPiolaStress(const Data& data, std::array<Matrix3<T>, num_locations>* P) const {
...
private:
    const DerivedConstitutiveModel& derived() const { return *static_cast<const DerivedConstitutiveModel*>(this); }
};

template <class Model>
struct is_constitutive_model {
    static constexpr bool value = std::is_base_of_v<ConstitutiveModel<Model, typename Model::Traits>, Model>;
};
