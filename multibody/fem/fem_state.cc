...
template <typename T>
FemState<T>::FemState(const internal::FemStateSystem<T>* system) : system_(system) {
    DRAKE_DEMAND(system != nullptr);
    owned_context_ = system_->CreateDefaultContext();
}

template <typename T>
FemState<T>::FemState(const internal::FemStateSystem<T>* system, const systems::Context<T>* context)
...
const VectorX<T>& FemState<T>::GetPositions() const {
    return get_context().get_discrete_state(system_->fem_position_index()).value();
}

template <typename T>
const VectorX<T>& FemState<T>::GetPreviousStepPositions() const {
    return get_context().get_discrete_state(system_->fem_previous_step_position_index()).value();
}

template <typename T>
const VectorX<T>& FemState<T>::GetVelocities() const {
    return get_context().get_discrete_state(system_->fem_velocity_index()).value();
}

template <typename T>
const VectorX<T>& FemState<T>::GetAccelerations() const {
    return get_context().get_discrete_state(system_->fem_acceleration_index()).value();
}
...
void FemState<T>::SetTimeStepPositions(const Eigen::Ref<const VectorX<T>>& q0) {
    get_mutable_context().SetDiscreteState(system_->fem_previous_step_position_index(), q0);
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::fem::FemState);
