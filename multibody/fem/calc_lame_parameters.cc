...
LameParameters<T> CalcLameParameters(const T& youngs_modulus, const T& poissons_ratio) {
    ...
    const T lambda = youngs_modulus * poissons_ratio / ((1.0 + poissons_ratio) * (1.0 - 2.0 * poissons_ratio));
    return {lambda, mu};
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&CalcLameParameters<T>));
