...
template <typename T>
void DirichletBoundaryCondition<T>::AddBoundaryCondition(FemNodeIndex index, const NodeState<T>& boundary_state) {
    node_to_boundary_state_[index] = boundary_state;
    node_indices_.emplace_back(index);
}

template <typename T>
void DirichletBoundaryCondition<T>::ApplyBoundaryConditionToState(FemState<T>* fem_state) const {
...
template <typename T>
void DirichletBoundaryCondition<T>::ApplyHomogeneousBoundaryCondition(EigenPtr<VectorX<T>> v) const {
