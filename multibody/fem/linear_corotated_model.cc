...
#include "multibody/fem/calc_lame_parameters.h"
...
LinearCorotatedModel<T, num_locations>::LinearCorotatedModel(const T& youngs_modulus, const T& poissons_ratio)
...
template <typename T, int num_locations>
void LinearCorotatedModel<T, num_locations>::CalcElasticEnergyDensityImpl(const Data& data,
                                                                          std::array<T, num_locations>* Psi) const {
    for (int i = 0; i < num_locations; ++i) {
        const Matrix3<T>& strain = data.strain()[i];
        const T& trace_strain = data.trace_strain()[i];
        (*Psi)[i] = mu_ * strain.squaredNorm() + 0.5 * lambda_ * trace_strain * trace_strain;
    }
}
...
/* First Piola stress P is the derivative of energy density
 ...
 which simplifies to
     2μRε + λtr(ε)R. */
template <typename T, int num_locations>
void LinearCorotatedModel<T, num_locations>::CalcFirstPiolaStressImpl(const Data& data,
...
void LinearCorotatedModel<T, num_locations>::CalcFirstPiolaStressDerivativeImpl(
        const Data& data, std::array<Eigen::Matrix<T, 9, 9>, num_locations>* dPdF) const {
                ...
                for (int alpha = 0; alpha < 3; ++alpha) {
                    for (int beta = 0; beta < 3; ++beta) {
                        ...
                        local_dPdF(3 * j + i, 3 * beta + alpha) +=
                                mu_ * R0(i, beta) * R0(alpha, j) + lambda_ * R0(alpha, beta) * R0(i, j);
                    }
                }
            }
        }
