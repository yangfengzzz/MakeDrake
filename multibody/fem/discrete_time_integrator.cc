...
template <typename T>
const VectorX<T>& DiscreteTimeIntegrator<T>::GetUnknowns(const FemState<T>& state) const {
    return DoGetUnknowns(state);
}

template <typename T>
void DiscreteTimeIntegrator<T>::UpdateStateFromChangeInUnknowns(const VectorX<T>& dz, FemState<T>* state) const {
...
template <typename T>
void DiscreteTimeIntegrator<T>::AdvanceOneTimeStep(const FemState<T>& prev_state,
                                                   const VectorX<T>& z,
