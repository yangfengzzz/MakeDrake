...
public:
    ...
    const std::vector<Data>& EvalElementData(const systems::CacheIndex cache_index) const {
        ...
        return system_->get_cache_entry(cache_index).template Eval<std::vector<Data>>(context);
    }
    ...
    int num_dofs() const { return get_context().get_discrete_state(system_->fem_position_index()).size(); }
    ...
    bool is_created_from_system(const internal::FemStateSystem<T>& system) const { return &system == system_; }
...
private:
    ...
    systems::Context<T>& get_mutable_context() {
        if (owned_context_ == nullptr) throw std::runtime_error("Trying to mutate a shared FemState.");
