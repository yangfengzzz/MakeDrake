...
public:
    ...
    static_assert(std::is_same_v<VolumetricElement<typename Element::IsoparametricElement,
                                                   typename Element::Quadrature,
                                                   ConstitutiveModel>,
                                 Element>,
    ...
    public:
        ...
        explicit VolumetricBuilder(VolumetricModel<Element>* model) : FemModel<T>::Builder(model), model_(model) {
        ...
        void AddLinearTetrahedralElements(const geometry::VolumeMesh<double>& mesh,
            ...
            if constexpr (Traits::num_nodes != 4 || Traits::natural_dimension != 3) {
                ...
                for (int e = 0; e < mesh.num_elements(); ++e) {
                    ...
                    const Vector<T, Traits::num_dofs> element_reference_positions =
                            Element::ExtractElementDofs(element_node_indices, new_reference_positions);
                    const auto& element_reference_positions_reshaped =
                            Eigen::Map<const Eigen::Matrix<T, kSpatialDimension, Traits::num_nodes>>(
                                    element_reference_positions.data());

                    Element element(element_node_indices, constitutive_model, element_reference_positions_reshaped,
                                    density, damping_model);
        ...
        */
        VectorX<double> ExtractMeshPositions(const geometry::VolumeMesh<double>& mesh) {
            ...
            for (int v = 0; v < num_new_vertices; ++v) {
                q.template segment<kSpatialDimension>(kSpatialDimension * v) = mesh.vertex(v);
            }
            return q;
        }
        ...
        void DoBuild() final {
            ...
            DRAKE_DEMAND(num_new_meshes == static_cast<int>(reference_positions_.size()));
            ...
            const int num_new_dofs = dof_offsets.back() + reference_positions_.back().size();
            ...
            for (int m = 0; m < num_new_meshes; ++m) {
                for (auto& e : elements_[m]) {
                    e.OffsetNodeIndex(FemNodeIndex(num_nodes_in_model + node_index_offsets[m]));
                }
                model_->AddElements(&(elements_[m]));
                /* Clear the moved-from elements. */
                elements_[m].clear();
                model_->reference_positions_.segment(num_old_dofs + dof_offsets[m], reference_positions_[m].size()) =
    ...
    T CalcElasticEnergy(const FemState<T>& fem_state) const {
        ...
        const std::vector<typename Element::Data>& element_data =
                fem_state.template EvalElementData<typename Element::Data>(this->element_data_index());
...
private:
    ...
    VectorX<T> MakeReferencePositions() const final { return reference_positions_; }
