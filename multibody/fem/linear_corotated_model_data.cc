...
template <typename T, int num_locations>
void LinearCorotatedModelData<T, num_locations>::UpdateFromDeformationGradient() {
    const std::array<Matrix3<T>, num_locations>& F = this->deformation_gradient();
    const std::array<Matrix3<T>, num_locations>& F0 = this->previous_step_deformation_gradient();
    for (int i = 0; i < num_locations; ++i) {
        ...
        local_strain = 0.5 * (corotated_F + corotated_F.transpose()) - Matrix3<T>::Identity();
