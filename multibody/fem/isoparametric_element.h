...
public:
    /* The number of locations to evaluate various quantities in the
     element. */
    static constexpr int num_sample_locations = DerivedTraits::num_sample_locations;
    ...
    /* Fixed size matrix type to store gradient in parent coordinates at a sample
     locations. */
    using GradientInParentCoordinates = Eigen::Matrix<T, num_nodes, natural_dimension>;

    /* Fixed size matrix type to store gradient in spatial coordinates at a sample
     location. */
    using GradientInSpatialCoordinates = Eigen::Matrix<T, num_nodes, spatial_dimension>;
    ...
    /* Fixed size matrix type to store the pseudoinverse Jacobian matrix at a
     sample location. */
    using PseudoinverseJacobianMatrix = Eigen::Matrix<T, natural_dimension, spatial_dimension>;
    ...
    /* Returns the std::array of sample locations in the parent domain at which
     ...
     construction. */
    const ArrayNumSamples<Vector<double, natural_dimension>>& locations() const { return locations_; }
    ...
    const ArrayNumSamples<VectorNumNodes>& GetShapeFunctions() const { return S_; }
    ...
    /* Obtains the gradient of the shape functions in parent coordinates, dS/dξ,
     ...
     node `a`. */
    const ArrayNumSamples<GradientInParentCoordinates>& GetGradientInParentCoordinates() const { return dSdxi_; }
    ...
    ArrayNumSamples<GradientInSpatialCoordinates> CalcGradientInSpatialCoordinates(
            const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>& xa) const {
...
protected:
    ...
    IsoparametricElement(ArrayNumSamples<Vector<double, natural_dimension>> locations,
                         ArrayNumSamples<Vector<T, num_nodes>> S,
                         ArrayNumSamples<GradientInParentCoordinates> dSdxi)
        : locations_(std::move(locations)), S_(std::move(S)), dSdxi_(std::move(dSdxi)) {}
    ...
    /* Default implementations of "Calc" methods. Derived concrete elements should
     ...
     @{ */
    ArrayNumSamples<GradientInSpatialCoordinates> DefaultCalcGradientInSpatialCoordinates(
            const Eigen::Ref<const Eigen::Matrix<T, spatial_dimension, num_nodes>>& xa) const {
    ...
    ArrayNumSamples<JacobianMatrix> DefaultCalcJacobian(
        ...
        const ArrayNumSamples<Eigen::Matrix<T, num_nodes, natural_dimension>>& dSdxi = GetGradientInParentCoordinates();
        for (int q = 0; q < num_sample_locations; ++q) {
            dxdxi[q] = xa * dSdxi[q];
        }
        return dxdxi;
    ...
    ArrayNumSamples<PseudoinverseJacobianMatrix> DefaultCalcJacobianPseudoinverse(
        ...
        for (int q = 0; q < num_sample_locations; ++q) {
            ...
            /* Thin unitaries are enough but Eigen does not allow them for fixed size
             matrix. */
            Eigen::JacobiSVD<JacobianMatrix> svd(jacobian[q], Eigen::ComputeFullU | Eigen::ComputeFullV);
            ...
            /* Use SVD to solve for the least square system which gives the
             pseudoinverse. */
            dxidx[q] = svd.solve(Eigen::Matrix<T, spatial_dimension, spatial_dimension>::Identity());
        }
        return dxidx;
    }
    /* @} */
...
private:
    const DerivedElement& derived() const { return static_cast<const DerivedElement&>(*this); }
...
struct is_isoparametric_element {
    static constexpr bool value = std::is_base_of_v<IsoparametricElement<Element, typename Element::Traits>, Element>;
};
}  // namespace internal
}  // namespace fem
}  // namespace multibody
