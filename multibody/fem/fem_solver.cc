...
using LinearSolver = contact_solvers::internal::BlockSparseCholeskySolver<Matrix3<double>>;

template <typename T>
FemSolver<T>::FemStateAndSchurComplement::FemStateAndSchurComplement(const FemModel<T>& model)
    : state(model.MakeFemState()) {}

template <typename T>
FemSolver<T>::FemStateAndSchurComplement::~FemStateAndSchurComplement() = default;
...
template <typename T>
FemSolver<T>::FemSolver(const FemModel<T>* model, const DiscreteTimeIntegrator<T>* integrator)
    : model_(model), integrator_(integrator), next_state_and_schur_complement_(*model), scratch_(*model) {
...
int FemSolver<T>::AdvanceOneTimeStep(const FemState<T>& prev_state,
    ...
    FemState<T>* next_state = next_state_and_schur_complement_.state.get_mutable();
    ...
    const int iterations = SolveNonlinearModel(plant_data, nonparticipating_vertices);
    if (iterations == -1) {
...
template <typename T>
bool FemSolver<T>::solver_converged(const T& residual_norm, const T& initial_residual_norm) const {
    return residual_norm < std::max(relative_tolerance_ * initial_residual_norm, absolute_tolerance_);
}

template <typename T>
int FemSolver<T>::SolveLinearModel(const FemPlantData<T>& plant_data,
                                   const std::unordered_set<int>& nonparticipating_vertices) {
    ...
    next_state_and_schur_complement_.schur_complement =
            contact_solvers::internal::SchurComplement(tangent_matrix, nonparticipating_vertices);
...
template <typename T>
int FemSolver<T>::SolveNonlinearModel(const FemPlantData<T>& plant_data,
                                      const std::unordered_set<int>& nonparticipating_vertices) {
    DRAKE_DEMAND(!model_->is_linear());
    VectorX<T>& b = scratch_.b;
    VectorX<T>& dz = scratch_.dz;
