...
public:
    ...
    protected:
        ...
        explicit Builder(FemModel<T>* model) : model_{model} { DRAKE_DEMAND(model_ != nullptr); }
    ...
    const internal::DirichletBoundaryCondition<T>& dirichlet_boundary_condition() const { return dirichlet_bc_; }
     ...
     */
    bool is_compatible_with(const FemState<T>& state) const { return state.is_created_from_system(*fem_state_system_); }
...
protected:
    ...
    virtual void DoCalcTangentMatrix(
            ...
            const Vector3<T>& weights,
            contact_solvers::internal::Block3x3SparseSymmetricMatrix* tangent_matrix) const = 0;
    ...
    /** Returns the FemStateSystem that manages the states and cache entries in
     this %FemModel. */
    const internal::FemStateSystem<T>& fem_state_system() const { return *fem_state_system_; }
