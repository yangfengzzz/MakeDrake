...
template <int natural_dimension_at_compile_time, int num_quadrature_points_at_compile_time>
class Quadrature {
public:
    ...
    static constexpr int num_quadrature_points = num_quadrature_points_at_compile_time;

    static_assert(1 <= natural_dimension && natural_dimension <= 3,
                  "Only 1, 2 and 3 dimensional shapes are supported.");
    static_assert(1 <= num_quadrature_points, "There has to be at least one quadrature point.");
...
protected:
    ...
    explicit Quadrature(std::pair<LocationsType, WeightsType>&& points_and_weights)
        : points_(std::move(points_and_weights.first)), weights_(std::move(points_and_weights.second)) {}
...
struct is_quadrature {
    static constexpr bool value =
            std::is_base_of_v<Quadrature<ObjectType::natural_dimension, ObjectType::num_quadrature_points>, ObjectType>;
};
