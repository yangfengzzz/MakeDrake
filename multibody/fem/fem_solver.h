...
public:
    ...
    FemSolver(const FemModel<T>* model, const DiscreteTimeIntegrator<T>* integrator);
    ...
    const FemState<T>& next_fem_state() const { return *next_state_and_schur_complement_.state; }
    ...
    const contact_solvers::internal::SchurComplement& next_schur_complement() const {
    ...
    void set_relative_tolerance(double tolerance) { relative_tolerance_ = tolerance; }
    ...
    void set_absolute_tolerance(double tolerance) { absolute_tolerance_ = tolerance; }
    ...
    bool solver_converged(const T& residual_norm, const T& initial_residual_norm) const;
...
private:
    ...
    struct Scratch {
        ...
        copyable_unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> tangent_matrix;
        contact_solvers::internal::BlockSparseCholeskySolver<Matrix3<T>> linear_solver;
        VectorX<T> b;
        VectorX<T> dz;
    };
    ...
    int SolveNonlinearModel(const FemPlantData<T>& plant_data,
                            const std::unordered_set<int>& nonparticipating_vertices);
    ...
    int SolveLinearModel(const FemPlantData<T>& plant_data, const std::unordered_set<int>& nonparticipating_vertices);
