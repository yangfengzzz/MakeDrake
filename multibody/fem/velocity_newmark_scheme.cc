...
template <typename T>
void VelocityNewmarkScheme<T>::DoUpdateStateFromChangeInUnknowns(const VectorX<T>& dz, FemState<T>* state) const {
...
template <typename T>
void VelocityNewmarkScheme<T>::DoAdvanceOneTimeStep(const FemState<T>& prev_state,
                                                    const VectorX<T>& z,
    ...
    /* Note that the partials of the next time step's (q, v, a) w.r.t. z are
     ...
     DoGetWeights(). */
    state->SetPositions(qn + dt() * (beta_over_gamma_ * v + (1.0 - beta_over_gamma_) * vn) +
                        dt() * dt() * (0.5 - beta_over_gamma_) * an);
    state->SetAccelerations(one_over_dt_gamma_ * (v - vn) - (1.0 - gamma_) / gamma_ * an);
