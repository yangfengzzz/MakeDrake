...
#include "multibody/contact_solvers/block_sparse_lower_triangular_or_symmetric_matrix.h"
...
public:
    ...
    static_assert(std::is_base_of_v<FemElement<Element>, Element>,
...
protected:
    ...
    /* Moves the input `element` into the vector of elements held by this
     FemModelImpl. */
    void AddElement(Element&& element) { elements_.emplace_back(std::move(element)); }
    ...
    void AddElements(std::vector<Element>* elements) {
        ...
        elements_.insert(elements_.end(), std::make_move_iterator(elements->begin()),
...
protected:
    ...
    void SetFrom(const FemModelImpl<Element>& other) { elements_ = other.elements_; }
...
private:
    void DoCalcResidual(const FemState<T>& fem_state,
        ...
        const std::vector<Data>& element_data = fem_state.template EvalElementData<Data>(element_data_index_);
        for (int e = 0; e < num_elements(); ++e) {
            /* residual = Ma-fₑ(x)-fᵥ(x, v)-fₑₓₜ. */
            /* The Ma-fₑ(x)-fᵥ(x, v) term. */
            elements_[e].CalcInverseDynamics(element_data[e], &element_residual);
            /* The -fₑₓₜ term. */
            elements_[e].AddScaledExternalForces(element_data[e], plant_data, -1.0, &element_residual);
            const std::array<FemNodeIndex, Element::num_nodes>& element_node_indices = elements_[e].node_indices();
    ...
    void DoCalcTangentMatrix(const FemState<T>& fem_state,
                             const Vector3<T>& weights,
                             contact_solvers::internal::Block3x3SparseSymmetricMatrix* tangent_matrix) const final {
        ...
        if constexpr (std::is_same_v<T, double>) {
            ...
            /* Scratch space to store the contribution to the tangent matrix from each
             element. */
            Eigen::Matrix<T, Element::num_dofs, Element::num_dofs> element_tangent_matrix;
            for (int e = 0; e < num_elements(); ++e) {
                elements_[e].CalcTangentMatrix(element_data[e], weights, &element_tangent_matrix);
                const std::array<FemNodeIndex, Element::num_nodes>& element_node_indices = elements_[e].node_indices();
                for (int a = 0; a < Element::num_nodes; ++a) {
                    ...
                    for (int b = 0; b <= a; ++b) {
                        ...
                        if (i >= j) {
                            tangent_matrix->AddToBlock(i, j, element_tangent_matrix.template block<3, 3>(3 * a, 3 * b));
                        } else {
                            tangent_matrix->AddToBlock(j, i, element_tangent_matrix.template block<3, 3>(3 * b, 3 * a));
                        }
                    }
                }
            }
    ...
    std::unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> DoMakeTangentMatrix() const final {
        ...
        if constexpr (std::is_same_v<T, double>) {
            ...
            for (int e = 0; e < num_elements(); ++e) {
                ...
                for (int a = 0; a < Element::num_nodes; ++a) {
                    for (int b = a; b < Element::num_nodes; ++b) {
                        ...
                        const int j = std::min(element_node_indices[a], element_node_indices[b]);
                        const int i = std::max(element_node_indices[a], element_node_indices[b]);
            ...
            for (int j = 0; j < this->num_nodes(); ++j) {
                sparsity_pattern.emplace_back(neighbor_nodes[j].begin(), neighbor_nodes[j].end());
            }
            contact_solvers::internal::BlockSparsityPattern block_pattern(std::vector<int>(this->num_nodes(), 3),
                                                                          std::move(sparsity_pattern));
            return std::make_unique<contact_solvers::internal::Block3x3SparseSymmetricMatrix>(std::move(block_pattern));
        } else {
            DRAKE_UNREACHABLE();
        }
    }

    void DeclareCacheEntries(internal::FemStateSystem<T>* fem_state_system) final {
        element_data_index_ =
                fem_state_system
                        ->DeclareCacheEntry("element data",
                                            systems::ValueProducer(this, &FemModelImpl<Element>::CalcElementData))
                        .cache_index();
    }
    ...
    void CalcElementData(const systems::Context<T>& context, std::vector<Data>* data) const {
