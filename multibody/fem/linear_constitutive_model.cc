...
LinearConstitutiveModel<T, num_locations>::LinearConstitutiveModel(const T& youngs_modulus, const T& poissons_ratio)
...
template <typename T, int num_locations>
void LinearConstitutiveModel<T, num_locations>::CalcElasticEnergyDensityImpl(const Data& data,
...
template <typename T, int num_locations>
void LinearConstitutiveModel<T, num_locations>::CalcFirstPiolaStressDerivativeImpl(
        const Data&, std::array<Eigen::Matrix<T, 9, 9>, num_locations>* dPdF) const {
    dPdF->fill(dPdF_);
}
