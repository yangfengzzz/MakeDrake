...
#include "multibody/fem/constitutive_model.h"
#include "multibody/fem/damping_model.h"
#include "multibody/fem/fem_indexes.h"
...
public:
    ...
    const std::array<FemNodeIndex, num_nodes>& node_indices() const { return node_indices_; }
    ...
    /* Calculates the tangent matrix for the element by combining the stiffness
     ...
     respectively. */
    void CalcTangentMatrix(const Data& data,
                           const Vector3<T>& weights,
                           EigenPtr<Eigen::Matrix<T, num_dofs, num_dofs>> tangent_matrix) const {
        DRAKE_DEMAND(tangent_matrix != nullptr);
        tangent_matrix->setZero();
        AddScaledStiffnessMatrix(data, weights(0) + weights(1) * damping_model_.stiffness_coeff_beta(), tangent_matrix);
        AddScaledMassMatrix(data, weights(2) + weights(1) * damping_model_.mass_coeff_alpha(), tangent_matrix);
    }
    ...
    void CalcInverseDynamics(const Data& data, EigenPtr<Vector<T, num_dofs>> external_force) const {
        DRAKE_ASSERT(external_force != nullptr);
        external_force->setZero();
        static_cast<const DerivedElement*>(this)->DoCalcInverseDynamics(data, external_force);
    }
    ...
    /* Accumulates the stiffness matrix (the derivative, or an approximation
     ...
     @param[in, out] K  The matrix to which the scaled stiffness matrix will be
                        added.
     @pre K != nullptr */
    void AddScaledStiffnessMatrix(const Data& data,
                                  const T& scale,
                                  EigenPtr<Eigen::Matrix<T, num_dofs, num_dofs>> K) const {
        DRAKE_ASSERT(K != nullptr);
        static_cast<const DerivedElement*>(this)->DoAddScaledStiffnessMatrix(data, scale, K);
    }
    ...
    /* Accumulates the damping matrix (the derivative of the residual with
     ...
     be expensive. */
    void AddScaledDampingMatrix(const Data& data,
                                const T& scale,
    ...
    /* Accumulates the mass matrix (the derivative of the residual with respect
     ...
     @pre M != nullptr */
    void AddScaledMassMatrix(const Data& data, const T& scale, EigenPtr<Eigen::Matrix<T, num_dofs, num_dofs>> M) const {
        DRAKE_ASSERT(M != nullptr);
        static_cast<const DerivedElement*>(this)->DoAddScaledMassMatrix(data, scale, M);
    }
    ...
    /* Accumulates external forces for this element given the `data` and the
     ...
     will be added.
     @pre external_force != nullptr */
    void AddScaledExternalForces(const Data& data,
                                 const FemPlantData<T>& plant_data,
                                 const T& scale,
                                 EigenPtr<Vector<T, num_dofs>> external_force) const {
        DRAKE_ASSERT(external_force != nullptr);
        static_cast<const DerivedElement*>(this)->DoAddScaledExternalForces(data, plant_data, scale, external_force);
    }

    /* Extracts the dofs corresponding to the nodes given by `node_indices` from
     the given `state_dofs`. */
    static Vector<T, 3 * num_nodes> ExtractElementDofs(const std::array<FemNodeIndex, num_nodes>& node_indices,
        ...
        for (int i = 0; i < num_nodes; ++i) {
            ...
            element_dofs.template segment<kDim>(i * kDim) = state_dofs.template segment<kDim>(node_indices[i] * kDim);
        }
        return element_dofs;
    }
    ...
    /* Extracts the dofs corresponding to the nodes in this element from the given
     `state_dofs`. */
    Vector<T, 3 * num_nodes> ExtractElementDofs(const VectorX<T>& state_dofs) const {
...
protected:
    ...
    Data DoComputeData(const FemState<T>& state) const { ThrowIfNotImplemented(__func__); }
    ...
    void DoCalcInverseDynamics(const Data& data, EigenPtr<Vector<T, num_dofs>> external_force) const {
    ...
    void DoAddScaledMassMatrix(const Data& data,
                               const T& scale,
    ...
    void DoAddScaledExternalForces(const Data&, const FemPlantData<T>&, const T&, EigenPtr<Vector<T, num_dofs>>) const {
        ThrowIfNotImplemented(__func__);
    }

    const ConstitutiveModel& constitutive_model() const { return constitutive_model_; }
...
private:
    ...
    void ThrowIfNotImplemented(const char* source_method) const {
        throw std::runtime_error("The DerivedElement from " + NiceTypeName::Get(*this) +
                                 " must provide an implementation for " + std::string(source_method) + "().");
    }
