...
#include "multibody/fem/test/dummy_model.h"
...
GTEST_TEST(FemModelTest, CalcResidual) {
    ...
    expected_residual.head<LinearDummyElement::kNumDofs>() += LinearDummyElement::inverse_dynamics_force();
    expected_residual.tail<LinearDummyElement::kNumDofs>() += LinearDummyElement::inverse_dynamics_force();
...
GTEST_TEST(FemModelTest, CalcResidualWithContextDependentExternalForce) {
    ...
    public:
        ...
        /* Constructs a force field that implements the test force f = time *
         ...
         context  */
        explicit TimeScaledForceDensityField(const Vector3d unit_vector) : unit_vector_(unit_vector) {}

    private:
        Vector3<double> DoEvaluateAt(const systems::Context<double>& context, const Vector3<double>&) const final {
    ...
    EXPECT_TRUE(CompareMatrices(2.0 * external_force, external_force2, 4.0 * std::numeric_limits<double>::epsilon()));
}
...
GTEST_TEST(FemModelTest, CalcTangentMatrix) {
    ...
    builder.Build();
    unique_ptr<FemState<double>> fem_state = model.MakeFemState();
    unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> tangent_matrix = model.MakeTangentMatrix();
    ...
    MatrixXd expected_mass_matrix = MatrixXd::Zero(model.num_dofs(), model.num_dofs());
    expected_mass_matrix.topLeftCorner(LinearDummyElement::kNumDofs, LinearDummyElement::kNumDofs) +=
            LinearDummyElement::mass_matrix();
    expected_mass_matrix.bottomRightCorner(LinearDummyElement::kNumDofs, LinearDummyElement::kNumDofs) +=
            LinearDummyElement::mass_matrix();
    MatrixXd expected_stiffness_matrix = MatrixXd::Zero(model.num_dofs(), model.num_dofs());
    expected_stiffness_matrix.topLeftCorner(LinearDummyElement::kNumDofs, LinearDummyElement::kNumDofs) +=
            LinearDummyElement::stiffness_matrix();
    expected_stiffness_matrix.bottomRightCorner(LinearDummyElement::kNumDofs, LinearDummyElement::kNumDofs) +=
            LinearDummyElement::stiffness_matrix();

    const MatrixXd expected_damping_matrix = LinearDummyModel::kMassDamping * expected_mass_matrix +
                                             LinearDummyModel::kStiffnessDamping * expected_stiffness_matrix;
    const MatrixXd expected_tangent_matrix = weights(0) * expected_stiffness_matrix +
                                             weights(1) * expected_damping_matrix + weights(2) * expected_mass_matrix;
    EXPECT_TRUE(CompareMatrices(tangent_matrix->MakeDenseMatrix(), expected_tangent_matrix,
                                4.0 * std::numeric_limits<double>::epsilon(), MatrixCompareType::relative));
}
...
GTEST_TEST(FemModelTest, CalcTangentMatrixNoAutoDiff) {
    ...
    using ConstitutiveModelType = fem::internal::LinearCorotatedModel<T, kNumQuads>;
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(fem_model->MakeTangentMatrix(), ".*only.*double.*");
    unique_ptr<FemState<T>> fem_state = fem_model->MakeFemState();
    contact_solvers::internal::BlockSparsityPattern empty_pattern({}, {});
    contact_solvers::internal::Block3x3SparseSymmetricMatrix tangent_matrix(empty_pattern);
    DRAKE_EXPECT_THROWS_MESSAGE(fem_model->CalcTangentMatrix(*fem_state, Vector3<T>(0.1, 0.2, 0.3), &tangent_matrix),
                                ".*only.*double.*");
}
...
GTEST_TEST(FemModelTest, IncompatibleModelState) {
    ...
    const systems::LeafContext<double> dummy_context;
    const FemPlantData<double> dummy_data{dummy_context, {}};
    DRAKE_EXPECT_THROWS_MESSAGE(model.CalcResidual(*fem_state, dummy_data, &residual),
    ...
    ASSERT_EQ(tangent_matrix->cols(), model.num_dofs());
    const Vector3d weights(0.1, 0.2, 0.3);
    DRAKE_EXPECT_THROWS_MESSAGE(model.CalcTangentMatrix(*fem_state, weights, tangent_matrix.get()),
...
GTEST_TEST(FemModelTest, MultipleBuilders) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder0.AddElementWithDistinctNodes(), "Build.* has been called.*");
}
...
GTEST_TEST(FemModelTest, DirichletBoundaryCondition) {
    ...
    auto are_same_states = [](const FemState<T>& a, const FemState<T>& b) {
        return a.GetAccelerations() == b.GetAccelerations() && a.GetVelocities() == b.GetVelocities() &&
    ...
    bc.AddBoundaryCondition(FemNodeIndex(0),
                            {Vector3<double>(1, 1, 1), Vector3<double>(2, 2, 2), Vector3<double>(3, 3, 3)});
     ...
     whether BC is applied from the model or directly from the BC.*/
    {
        ...
        unique_ptr<FemState<T>> state0 = model.MakeFemState();
        unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> tangent_matrix0 =
                model.MakeTangentMatrix();
        ...
        unique_ptr<FemState<T>> state1 = model_without_bc.MakeFemState();
        unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> tangent_matrix1 =
                model.MakeTangentMatrix();
...
GTEST_TEST(FemModelTest, Clone) {
    ...
    EXPECT_EQ(state->GetPreviousStepPositions(), clone_state->GetPreviousStepPositions());
    ...
    EXPECT_EQ(tangent_matrix->MakeDenseMatrix(), clone_tangent_matrix->MakeDenseMatrix());
