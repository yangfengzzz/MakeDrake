...
#include "multibody/fem/fem_state_system.h"
...
protected:
    void SetUp() {
        fem_state_system_ = std::make_unique<internal::FemStateSystem<T>>(q<T>(), v<T>(), a<T>());

        std::function<void(const systems::Context<T>&, std::vector<Data<T>>*)> calc_element_data =
                [this](const systems::Context<T>& context, std::vector<Data<T>>* element_data) {
        ...
        cache_index_ =
                fem_state_system_
                        ->DeclareCacheEntry(
                                "dummy_data", systems::ValueProducer(calc_element_data),
                                {fem_state_system_->discrete_state_ticket(fem_state_system_->fem_position_index())})
                        .cache_index();
    }
    ...
    void VerifyIsClone(const FemState<T>& state, const FemState<T>& clone) {
        ...
        const std::vector<Data<T>>& element_data = state.template EvalElementData<Data<T>>(cache_index_);
        const std::vector<Data<T>>& cloned_data = clone.template EvalElementData<Data<T>>(cache_index_);
...
TYPED_TEST(FemStateTest, SharedFemState) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(state.SetPositions(-1.23 * q<T>()), "Trying to mutate a shared FemState.");
    DRAKE_EXPECT_THROWS_MESSAGE(state.SetVelocities(3.14 * v<T>()), "Trying to mutate a shared FemState.");
    DRAKE_EXPECT_THROWS_MESSAGE(state.SetAccelerations(-1.29 * a<T>()), "Trying to mutate a shared FemState.");
}
...
TYPED_TEST(FemStateTest, SetStates) {
    ...
    EXPECT_THROW(state.SetPositions(VectorX<T>::Constant(1, 1.0)), std::exception);
    EXPECT_THROW(state.SetVelocities(VectorX<T>::Constant(1, 1.0)), std::exception);
    EXPECT_THROW(state.SetAccelerations(VectorX<T>::Constant(1, 1.0)), std::exception);
}

TYPED_TEST(FemStateTest, ElementData) {
    using T = TypeParam;
    const FemState<T> state(this->fem_state_system_.get());
    const std::vector<Data<T>>& element_data = state.template EvalElementData<Data<T>>(this->cache_index_);
     ...
     */
    EXPECT_THROW(state.template EvalElementData<double>(this->cache_index_), std::exception);
}
