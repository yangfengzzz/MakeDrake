...
protected:
    void set_max_newton_iterations(int max_iterations) { solver_.max_iterations_ = max_iterations; }
...
TYPED_TEST_P(FemSolverTest, AdvanceOneTimeStep) {
    ...
    const int num_iterations = this->solver_.AdvanceOneTimeStep(*state0, dummy_data, nonparticipating_vertices);
    EXPECT_EQ(num_iterations, 1);

    /* Compute the expected result from AdvanceOneTimeStep(). */
    std::unique_ptr<FemState<double>> expected_state = this->model_.MakeFemState();
    auto tangent_matrix0 = this->model_.MakeTangentMatrix();
    this->model_.CalcTangentMatrix(*state0, this->integrator_.GetWeights(), tangent_matrix0.get());
    ...
    EXPECT_TRUE(CompareMatrices(expected_state->GetPositions(), computed_state.GetPositions(), kTolerance));
    EXPECT_TRUE(CompareMatrices(expected_state->GetAccelerations(), computed_state.GetAccelerations(), kTolerance));
    EXPECT_TRUE(CompareMatrices(expected_state->GetVelocities(), computed_state.GetVelocities(), kTolerance));

    /* Check the Schur complement is as expected. */
    auto tangent_matrix = this->model_.MakeTangentMatrix();
    this->model_.CalcTangentMatrix(computed_state, this->integrator_.GetWeights(), tangent_matrix.get());
    contact_solvers::internal::SchurComplement force_balance_tangent_matrix_schur_complement(*tangent_matrix,
                                                                                             nonparticipating_vertices);
    const MatrixXd expected_schur_complement = force_balance_tangent_matrix_schur_complement.get_D_complement();
    const contact_solvers::internal::SchurComplement& computed_schur_complement = this->solver_.next_schur_complement();
    EXPECT_TRUE(CompareMatrices(expected_schur_complement, computed_schur_complement.get_D_complement(), kTolerance,
...
TYPED_TEST_P(FemSolverTest, Nonconvergence) {
    ...
    if (!is_linear) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(this->solver_.AdvanceOneTimeStep(*state0, dummy_data, nonparticipating_vertices),
                                    ".*failed.*");
    }
}
...
TYPED_TEST_P(FemSolverTest, DefaultStateAndSchurComplement) {
    ...
    EXPECT_TRUE(CompareMatrices(default_state->GetPositions(), next_state.GetPositions()));
    EXPECT_TRUE(CompareMatrices(default_state->GetAccelerations(), next_state.GetAccelerations()));
    EXPECT_TRUE(CompareMatrices(default_state->GetVelocities(), next_state.GetVelocities()));
    const contact_solvers::internal::SchurComplement next_schur_complement = this->solver_.next_schur_complement();
...
REGISTER_TYPED_TEST_SUITE_P(
        FemSolverTest, Tolerance, AdvanceOneTimeStep, Nonconvergence, DefaultStateAndSchurComplement);
