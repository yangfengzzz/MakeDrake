#include "multibody/fem/deformation_gradient_data.h"
...
class DummyData : public DeformationGradientData<DummyData<T, num_locations_at_compile_time>> {
public:
    ...
    std::array<Matrix3<T>, num_locations_at_compile_time> double_deformation_gradient() const {
...
private:
    ...
    void UpdateFromDeformationGradient() {
        const std::array<Matrix3<T>, num_locations_at_compile_time>& F = this->deformation_gradient();
        for (int i = 0; i < num_locations_at_compile_time; ++i) double_deformation_gradient_[i] = 2.0 * F[i];
    }

    std::array<Matrix3<T>, num_locations_at_compile_time> double_deformation_gradient_;
};
...
GTEST_TEST(DeformationGradientDataTest, UpdateData) {
    ...
    ASSERT_TRUE(CompareMatrices(dummy_data.deformation_gradient()[0], Matrix3d::Identity()));
    ASSERT_TRUE(CompareMatrices(dummy_data.double_deformation_gradient()[0], 2.0 * Matrix3d::Identity()));
  ...
  F << 4, 9, 2,
    ...
    EXPECT_TRUE(CompareMatrices(dummy_data.previous_step_deformation_gradient()[0], F0));
    EXPECT_TRUE(CompareMatrices(dummy_data.double_deformation_gradient()[0], 2.0 * F));
}
...
/* An invalid DeformationGradientData that is missing the
 ...
 method. */
template <typename T, int num_locations_at_compile_time>
class InvalidDummyData : public DeformationGradientData<InvalidDummyData<T, num_locations_at_compile_time>> {};
...
GTEST_TEST(DeformationGradientDataTest, UnshadowedUpdateFromDeformationGradient) {
    InvalidDummyData<double, kNumLocations> invalid_dummy_data;
    const Matrix3d F = 2.8 * Matrix3d::Identity();
    DRAKE_EXPECT_THROWS_MESSAGE(invalid_dummy_data.UpdateData({F}, {F}),
