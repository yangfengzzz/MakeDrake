...
const std::array<Vector2<double>, kNumQuads> tri_locations{{{0.0, 0.0}, {1.0 / 3.0, 1.0 / 3.0}}};
const std::array<Vector3<double>, kNumQuads> tet_locations{{{0.0, 0.0, 0.0}, {0.25, 0.25, 0.25}}};
...
TEST_F(LinearSimplexElementTest, ShapeFunctionDerivative2D) {
  ...
  xa << 0, 1, 0,
    ...
    for (int q = 0; q < kNumQuads; ++q) {
        EXPECT_TRUE(CompareMatrices(dSdX[q], dSdxi[q], std::numeric_limits<double>::epsilon()));
    }
}
