...
protected:
    void SetUp() { fem_state_system_ = std::make_unique<FemStateSystem<double>>(q(), v(), a()); }
...
TEST_F(VelocityNewmarkSchemeTest, Weights) {
    EXPECT_TRUE(
            CompareMatrices(scheme_.GetWeights(), Vector3<double>(kBeta / kGamma * kDt, 1.0, 1.0 / (kGamma * kDt))));
}
...
TEST_F(VelocityNewmarkSchemeTest, UpdateStateFromChangeInUnknowns) {
    ...
    EXPECT_TRUE(CompareMatrices(state.GetPositions() - state0.GetPositions(), weights(0) * dz, kTolerance));
    EXPECT_TRUE(CompareMatrices(state.GetVelocities() - state0.GetVelocities(), weights(1) * dz, kTolerance));
    EXPECT_TRUE(CompareMatrices(state.GetAccelerations() - state0.GetAccelerations(), weights(2) * dz, kTolerance));
}
...
TEST_F(VelocityNewmarkSchemeTest, AdvanceOneTimeStep) {
    ...
    EXPECT_TRUE(CompareMatrices(state_n.GetAccelerations(), state_0.GetAccelerations(), kTolerance * kTimeSteps / kDt));
    EXPECT_TRUE(CompareMatrices(state_n.GetVelocities(),
                                state_0.GetVelocities() + total_time * state_0.GetAccelerations(),
                                kTimeSteps * kTolerance));
    EXPECT_TRUE(CompareMatrices(state_n.GetPositions(),
...
TEST_F(VelocityNewmarkSchemeTest, EquivalenceWithAccelerationNewmark) {
    ...
    acceleration_scheme.AdvanceOneTimeStep(state0, state_v.GetAccelerations(), &state_a);
    EXPECT_TRUE(CompareMatrices(state_v.GetAccelerations(), state_a.GetAccelerations(), kTolerance));
    EXPECT_TRUE(CompareMatrices(state_v.GetVelocities(), state_a.GetVelocities(), kTolerance));
    EXPECT_TRUE(CompareMatrices(state_v.GetPositions(), state_a.GetPositions(), kTolerance));
}
