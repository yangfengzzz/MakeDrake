#include "multibody/fem/linear_corotated_model_data.h"
...
GTEST_TEST(LinearCorotatedModelDataTest, Initialization) {
    const LinearCorotatedModelData<double, kNumLocations> linear_corotated_model_data;
    VerifySizes(linear_corotated_model_data);
    EXPECT_TRUE(CompareMatrices(linear_corotated_model_data.deformation_gradient()[0], Matrix3<double>::Identity()));
    EXPECT_EQ(linear_corotated_model_data.previous_step_deformation_gradient()[0], Matrix3<double>::Identity());
    EXPECT_TRUE(CompareMatrices(linear_corotated_model_data.R0()[0], Matrix3<double>::Identity()));
    EXPECT_TRUE(CompareMatrices(linear_corotated_model_data.strain()[0], Matrix3<double>::Zero()));
...
GTEST_TEST(LinearCorotatedModelDataTest, UpdateData) {
    ...
    const Matrix3<double> R0 = math::RotationMatrix<double>(math::RollPitchYaw<double>(1.0, 2.0, 3.0)).matrix();
    // clang-format off
  ...
  .finished();
    ...
    EXPECT_TRUE(CompareMatrices(linear_corotated_model_data.deformation_gradient()[0], F));
    EXPECT_EQ(linear_corotated_model_data.previous_step_deformation_gradient()[0], F0);
    EXPECT_TRUE(CompareMatrices(linear_corotated_model_data.R0()[0], R0, kTol));
    const Matrix3<double> expected_strain =
            0.5 * (R0.transpose() * F + F.transpose() * R0) - Matrix3<double>::Identity();
    EXPECT_NEAR(linear_corotated_model_data.trace_strain()[0], expected_strain.trace(), kTol);
}
