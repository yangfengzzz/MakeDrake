...
struct FemElementTraits<DummyElement<is_linear>> {
    ...
    using ConstitutiveModel = std::conditional_t<is_linear,
...
public:
    ...
    DummyElement(const std::array<FemNodeIndex, Traits::num_nodes>& node_indices,
        ...
        : Base(node_indices, std::move(constitutive_model), std::move(damping_model)) {}

    /* Provides a fixed return value for CalcInverseDynamics(). */
    static Vector<T, kNumDofs> inverse_dynamics_force() { return Vector<T, kNumDofs>::Constant(1.23456); }
    ...
    static Eigen::Matrix<T, kNumDofs, kNumDofs> MakeSpdMatrix() {
        ...
        return (A * A.transpose()) + Eigen::Matrix<T, kNumDofs, kNumDofs>::Identity();
    }

    /* Provides a fixed return value for CalcStiffnessMatrix(). */
    static Eigen::Matrix<T, kNumDofs, kNumDofs> stiffness_matrix() { return 1.23 * MakeSpdMatrix(); }

    /* Provides a fixed return value for CalcMassMatrix(). */
    static Eigen::Matrix<T, kNumDofs, kNumDofs> mass_matrix() { return 7.89 * MakeSpdMatrix(); }
...
private:
    ...
    void DoCalcInverseDynamics(const Data& data, EigenPtr<Vector<T, kNumDofs>> external_force) const {
    ...
    void DoAddScaledStiffnessMatrix(const Data&,
                                    const T& scale,
    ...
    void DoAddScaledMassMatrix(const Data&, const T& scale, EigenPtr<Eigen::Matrix<T, kNumDofs, kNumDofs>> M) const {
        *M += scale * mass_matrix();
    }
