#include "multibody/fem/linear_constitutive_model_data.h"
...
void VerifySizes(const LinearConstitutiveModelData<double, kNumLocations>& data) {
...
GTEST_TEST(LinearConstitutiveModelDataTest, Initialization) {
    ...
    EXPECT_TRUE(CompareMatrices(linear_elasticity_data.deformation_gradient()[0], Matrix3<double>::Identity()));
    EXPECT_TRUE(CompareMatrices(linear_elasticity_data.strain()[0], Matrix3<double>::Zero()));
...
GTEST_TEST(LinearConstitutiveModelDataTest, UpdateData) {
  ...
  expected_strain << 3, 6, 5,
    ...
    EXPECT_TRUE(CompareMatrices(linear_elasticity_data.deformation_gradient()[0], F));
    EXPECT_TRUE(CompareMatrices(linear_elasticity_data.strain()[0], expected_strain,
                                std::numeric_limits<double>::epsilon()));
    EXPECT_DOUBLE_EQ(linear_elasticity_data.trace_strain()[0], expected_trace_strain);
}
