...
GTEST_TEST(MatrixUtilitiesTest, CalcConditionNumber) {
    ...
    EXPECT_THROW(CalcConditionNumberOfInvertibleMatrix<double>(B), std::exception);
}
...
GTEST_TEST(MatrixUtilitiesTest, AddScaledRotationalDerivative) {
    ...
    Eigen::Matrix<AutoDiffXd, 9, 9> scaled_dRdF = Eigen::Matrix<AutoDiffXd, 9, 9>::Zero();
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            EXPECT_TRUE(CompareMatrices(scale * Eigen::Map<const Matrix3<double>>(R(i, j).derivatives().data(), 3, 3),
...
GTEST_TEST(MatrixUtilitiesTest, CalcCofactorMatrix) {
    ...
    EXPECT_TRUE(CompareMatrices(A.inverse(), 1.0 / A.determinant() * C.transpose(), CalcTolerance(A)));
}
...
GTEST_TEST(MatrixUtilitiesTest, AddScaledCofactorMatrixDerivative) {
    ...
    Eigen::Matrix<AutoDiffXd, 9, 9> scaled_dCdA = Eigen::Matrix<AutoDiffXd, 9, 9>::Zero();
    AutoDiffXd scale = 1.23;
    ...
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            EXPECT_TRUE(CompareMatrices(scale * Eigen::Map<const Matrix3<double>>(C(i, j).derivatives().data(), 3, 3),
...
GTEST_TEST(MatrixUtilitiesTest, PermuteBlockVector) {
    ...
    const VectorX<double> permuted_v = PermuteBlockVector<double>(v, block_permutation);
