#include "multibody/fem/corotated_model_data.h"
...
GTEST_TEST(CorotatedModelDataTest, Initialization) {
    ...
    EXPECT_TRUE(CompareMatrices(corotated_model_data.deformation_gradient()[0], Matrix3<double>::Identity()));
    EXPECT_TRUE(CompareMatrices(corotated_model_data.R()[0], Matrix3<double>::Identity()));
    EXPECT_TRUE(CompareMatrices(corotated_model_data.S()[0], Matrix3<double>::Identity()));
    EXPECT_EQ(corotated_model_data.Jm1()[0], 0.0);
    EXPECT_TRUE(CompareMatrices(corotated_model_data.JFinvT()[0], Matrix3<double>::Identity()));
}
...
GTEST_TEST(CorotatedModelDataTest, UpdateData) {
    ...
    const Matrix3<double> R = math::RotationMatrix<double>(math::RollPitchYaw<double>(1.0, 2.0, 3.0)).matrix();
    // clang-format off
  ...
  .finished();
    ...
    EXPECT_TRUE(CompareMatrices(corotated_model_data.deformation_gradient()[0], F));
    ...
    EXPECT_TRUE(CompareMatrices(corotated_model_data.JFinvT()[0], F.determinant() * F.inverse().transpose(), kTol));
}
