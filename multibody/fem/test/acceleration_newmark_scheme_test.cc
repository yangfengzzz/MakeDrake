...
TEST_F(AccelerationNewmarkSchemeTest, Weights) {
    EXPECT_TRUE(CompareMatrices(scheme_.GetWeights(), Vector3<double>(kBeta * kDt * kDt, kGamma * kDt, 1.0), 0));
}
...
TEST_F(AccelerationNewmarkSchemeTest, AdvanceOneTimeStep) {
    ...
    EXPECT_TRUE(CompareMatrices(state_n.GetAccelerations(), state_0.GetAccelerations()));
...
TEST_F(AccelerationNewmarkSchemeTest, EquivalenceWithVelocityNewmark) {
    ...
    EXPECT_TRUE(CompareMatrices(state_v.GetAccelerations(), state_a.GetAccelerations(), kTolerance / kDt));
