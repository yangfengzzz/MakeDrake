...
private:
    ...
    const VectorXd& DoGetUnknowns(const FemState<double>& state) const final { return state.GetPositions(); }

    // Dummy implementation to set the position to `dz`.
    void DoUpdateStateFromChangeInUnknowns(const VectorXd& dz, FemState<double>* state) const final {
...
protected:
    ...
    void SetUp() {
        fem_state_system_ = std::make_unique<internal::FemStateSystem<double>>(q(), VectorXd::Zero(kNumDofs),
                                                                               VectorXd::Zero(kNumDofs));
    }
