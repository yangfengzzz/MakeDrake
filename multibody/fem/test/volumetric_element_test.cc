#include "multibody/fem/volumetric_element.h"
...
using AD = AutoDiffXd;
using QuadratureType = internal::SimplexGaussianQuadrature<kNaturalDimension, kQuadratureOrder>;
static constexpr int kNumQuads = QuadratureType::num_quadrature_points;
using IsoparametricElementType = internal::LinearSimplexElement<AD, kNaturalDimension, kSpatialDimension, kNumQuads>;
...
protected:
    using ElementType = VolumetricElement<IsoparametricElementType, QuadratureType, ConstitutiveModelType>;
    ...
    void SetUp() override {
        ...
        fem_state_system_ = make_unique<internal::FemStateSystem<AD>>(
                VectorX<AD>::Zero(kNumDofs), VectorX<AD>::Zero(kNumDofs), VectorX<AD>::Zero(kNumDofs));
        /* Set up element data. */
        std::function<void(const systems::Context<AD>&, std::vector<Data>*)> calc_element_data =
                [this](const systems::Context<AD>& context, std::vector<Data>* element_data) {
        ...
        cache_index_ = fem_state_system_->DeclareCacheEntry("Element data", systems::ValueProducer(calc_element_data))
                               .cache_index();
    }
    ...
    void SetupElement() {
        ...
        elements_.emplace_back(kNodeIndices, std::move(constitutive_model), X, kDensity, std::move(damping_model));
    }

    /* Makes an FemState to be consumed by the unit tests with the given q, v, and
     a as the state values. */
    unique_ptr<FemState<AD>> MakeFemState(const VectorX<AD>& q, const VectorX<AD>& v, const VectorX<AD>& a) {
    ...
    unique_ptr<FemState<AD>> MakeDeformedState() {
        ...
        perturbation << 0.18, 0.63, 0.54, 0.13, 0.92, 0.17, 0.03, 0.86, 0.85, 0.25, 0.53, 0.67;
        Eigen::Matrix<AD, kSpatialDimension, kNumNodes> X = reference_positions();
        Vector<double, kNumDofs> x =
                Eigen::Map<Vector<double, kNumDofs>>(math::DiscardGradient(X).data(), reference_positions().size()) +
                perturbation;
    ...
    Eigen::Matrix<AD, kSpatialDimension, kNumNodes> reference_positions() const {
        Eigen::Matrix<AD, kSpatialDimension, kNumNodes> X(kSpatialDimension, kNumNodes);
        // clang-format off
    ...
    const Data& EvalElementData(const FemState<AD>& state) const {
        const std::vector<Data>& all_data = state.EvalElementData<Data>(cache_index_);
    ...
    Vector<AD, kNumDofs> CalcNegativeElasticForce(const FemState<AD>& fem_state) const {
    ...
    Eigen::Matrix<AD, kNumDofs, kNumDofs> CalcNegativeElasticForceDerivative(const FemState<AD>& fem_state) const {
        Eigen::Matrix<AD, kNumDofs, kNumDofs> neg_force_derivative = Eigen::Matrix<AD, kNumDofs, kNumDofs>::Zero();
        element().AddScaledElasticForceDerivative(EvalElementData(fem_state), -1, &neg_force_derivative);
    ...
    DeformationGradientDataType CalcDeformationGradientData(const VectorX<AD>& q, const VectorX<AD>& q0) const {
        const std::array<Matrix3<AD>, kNumQuads> F = element().CalcDeformationGradient(q);
        const std::array<Matrix3<AD>, kNumQuads> F0 = element().CalcDeformationGradient(q0);
    ...
    void VerifyEnergyAndForceAreZero(const FemState<AD>& fem_state) const {
        ...
        EXPECT_TRUE(CompareMatrices(Vector<AD, kNumDofs>::Zero(), neg_elastic_force, kEpsilon));
    }

    /* Returns the constitutive model of the only element. */
    const ConstitutiveModelType& constitutive_model() const { return element().constitutive_model(); }
    ...
    const std::array<AD, kNumQuads>& reference_volume() const { return element().reference_volume_; }

    /* Calculates the mass matrix of the only element. */
    Eigen::Matrix<AD, kNumDofs, kNumDofs> CalcMassMatrix(const FemState<AD>& fem_state) const {
        Eigen::Matrix<AD, kNumDofs, kNumDofs> mass_matrix = Eigen::Matrix<AD, kNumDofs, kNumDofs>::Zero();
...
TEST_F(VolumetricElementTest, UndeformedState) {
    ...
    math::RigidTransform<AD> transform(math::RollPitchYaw<AD>(1, 2, 3), Vector3<AD>(0.1, 0.2, 0.3));
    Eigen::Matrix<AD, kSpatialDimension, kNumNodes> X = reference_positions();
    Eigen::Matrix<AD, kSpatialDimension, kNumNodes> rigid_transformed_X;
    for (int i = 0; i < kNumNodes; ++i) {
        rigid_transformed_X.col(i) = transform * X.col(i);
    }
    fem_state->SetPositions(Eigen::Map<Vector<AD, kNumDofs>>(rigid_transformed_X.data(), rigid_transformed_X.size()));
    VerifyEnergyAndForceAreZero(*fem_state);
}
...
TEST_F(VolumetricElementTest, DeformedState) {
    ...
    const auto deformation_gradient_data =
            CalcDeformationGradientData(fem_state->GetPositions(), fem_state->GetPreviousStepPositions());
    std::array<AD, kNumQuads> energy_density_array;
    constitutive_model().CalcElasticEnergyDensity(deformation_gradient_data, &energy_density_array);
    ...
    const double reference_volume = 1.0 / 6.0 * std::abs(matrix_for_volume_calculation.determinant());
    ...
    EXPECT_NEAR(element().CalcElasticEnergy(EvalElementData(*fem_state)).value(), analytical_energy, kEpsilon);

    const auto neg_elastic_force_autodiff = CalcNegativeElasticForce(*fem_state);
    Vector<double, kNumDofs> neg_elastic_force = math::DiscardGradient(neg_elastic_force_autodiff);
    /* Force on node 0. */
    const Vector3<double> force0 = -neg_elastic_force.head<3>();
    /* The first piola stress. */
    std::array<Matrix3<AD>, kNumQuads> P_array;
    constitutive_model().CalcFirstPiolaStress(deformation_gradient_data, &P_array);
    ...
    /* The directional face area of each face in the reference configuration. The
     ...
     face normal. */
    const Vector3<double> face012 = 0.5 * (X_double.col(1) - X_double.col(0)).cross(X_double.col(2) - X_double.col(0));
    const Vector3<double> face013 = 0.5 * (X_double.col(3) - X_double.col(0)).cross(X_double.col(1) - X_double.col(0));
    const Vector3<double> face023 = 0.5 * (X_double.col(2) - X_double.col(0)).cross(X_double.col(3) - X_double.col(0));
    /* The analytic force exerted on node 0 is the average of the total force
     ...
     element is used. */
    const Vector3<double> force0_expected = P * (face012 + face013 + face023) / 3.0;
...
TEST_F(VolumetricElementTest, NegativeElasticForceIsEnergyDerivative) {
    ...
    AD energy = element().CalcElasticEnergy(EvalElementData(*fem_state));
    Vector<AD, kNumDofs> neg_elastic_force = CalcNegativeElasticForce(*fem_state);
    EXPECT_TRUE(CompareMatrices(energy.derivatives(), neg_elastic_force, kEpsilon));
}
...
TEST_F(VolumetricElementTest, ElasticForceCompatibleWithItsDerivative) {
    unique_ptr<FemState<AD>> fem_state = MakeDeformedState();
    Vector<AD, kNumDofs> neg_elastic_force = CalcNegativeElasticForce(*fem_state);
    Eigen::Matrix<AD, kNumDofs, kNumDofs> neg_elastic_force_derivative = CalcNegativeElasticForceDerivative(*fem_state);
    for (int i = 0; i < kNumDofs; ++i) {
        EXPECT_TRUE(CompareMatrices(neg_elastic_force(i).derivatives().transpose(), neg_elastic_force_derivative.row(i),
                                    kEpsilon));
    }
}
...
TEST_F(VolumetricElementTest, MassMatrixSumUpToTotalMass) {
    ...
    const Eigen::Matrix<AD, kNumDofs, kNumDofs>& mass_matrix = CalcMassMatrix(*deformed_fem_state);
...
TEST_F(VolumetricElementTest, PerReferenceVolumeExternalForce) {
    ...
    const Vector3<AD> expected_gravity_force = reference_volume()[0] * gravity_vector * mass_density;
...
TEST_F(VolumetricElementTest, PerCurrentVolumeExternalForce) {
    ...
    const Eigen::Matrix<AD, kSpatialDimension, kNumNodes> X_matrix = reference_positions();
    const Eigen::Matrix<AD, kSpatialDimension, kNumNodes> x_matrix = scale * X_matrix;
    Vector<double, kNumDofs> x_double(
            Eigen::Map<Vector<double, kNumDofs>>(math::DiscardGradient(x_matrix).data(), x_matrix.size()));
    ...
    private:
        Vector3<AD> DoEvaluateAt(const systems::Context<AD>& context, const Vector3<AD>& p_WQ) const final {
            return f_;
        };
