...
protected:
    ...
    void SetUp() override {
        fem_state_system_ = std::make_unique<internal::FemStateSystem<T>>(q(), v(), a());
        /* Set up element data. */
        std::function<void(const systems::Context<T>&, std::vector<Data>*)> calc_element_data =
                [this](const systems::Context<T>& context, std::vector<Data>* element_data) {
        ...
        cache_index_ =
                fem_state_system_->DeclareCacheEntry("dummy element data", systems::ValueProducer(calc_element_data))
                        .cache_index();
    ...
    const Data& EvalElementData() const {
        const std::vector<Data>& element_data = fem_state_->EvalElementData<Data>(cache_index_);
...
TEST_F(FemElementTest, ElementData) {
    ...
    EXPECT_EQ(data.value, q()(kNumDofs - 1) + v()(kNumDofs - 1) + a()(kNumDofs - 1));
}
...
TEST_F(FemElementTest, ExtractElementDofs) {
    ...
    EXPECT_EQ(DummyElement<true>::ExtractElementDofs(kNodeIndices, q()), expected_element_q);
}
...
TEST_F(FemElementTest, StiffnessMatrix) {
    Eigen::Matrix<T, DummyElementTraits::num_dofs, DummyElementTraits::num_dofs> K;
    K.setZero();
    const T scale = 3.14;
...
TEST_F(FemElementTest, DampingMatrix) {
    Eigen::Matrix<T, DummyElementTraits::num_dofs, DummyElementTraits::num_dofs> D;
    D.setZero();
    const T scale = 3.14;
    element_.AddScaledDampingMatrix(EvalElementData(), scale, &D);

    Eigen::Matrix<T, DummyElementTraits::num_dofs, DummyElementTraits::num_dofs> expected_D;
    expected_D.setZero();
    element_.AddScaledMassMatrix(EvalElementData(), scale * kDampingModel.mass_coeff_alpha(), &expected_D);
    element_.AddScaledStiffnessMatrix(EvalElementData(), scale * kDampingModel.stiffness_coeff_beta(), &expected_D);
    EXPECT_EQ(D, expected_D);
}

TEST_F(FemElementTest, MassMatrix) {
    Eigen::Matrix<T, DummyElementTraits::num_dofs, DummyElementTraits::num_dofs> M;
    M.setZero();
    const T scale = 3.14;
...
TEST_F(FemElementTest, ExternalForce) {
    ...
    Vector<T, kNumDofs> external_force = VectorX<T>::LinSpaced(kNumDofs, 0.0, 1.0);
    ...
    for (int i = 0; i < kNumNodes; ++i) {
        EXPECT_EQ(external_force.segment<3>(3 * i), old_external_force.segment<3>(3 * i) + scale * f);
    }
}
