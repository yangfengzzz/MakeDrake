#include "multibody/fem/test/constitutive_model_test_utilities.h"
...
std::array<Matrix3<AutoDiffXd>, num_locations> MakeDeformationGradientsWithDerivatives() {
  ...
  F << 0.18, 0.63, 0.54,
    ...
    const Eigen::Matrix<double, 9, Eigen::Dynamic> derivatives(Eigen::Matrix<double, 9, 9>::Identity());
    for (int i = 0; i < num_locations; ++i) {
        const auto F_autodiff_flat = math::InitializeAutoDiff(
                Eigen::Map<const Eigen::Matrix<double, 9, 1>>(deformation_gradients[i].data(), 9), derivatives);
        deformation_gradients_autodiff[i] = Eigen::Map<const Matrix3<AutoDiffXd>>(F_autodiff_flat.data(), 3, 3);
    }
    return deformation_gradients_autodiff;
}

template <int num_locations>
std::array<Matrix3<AutoDiffXd>, num_locations> MakeDeformationGradientsWithoutDerivatives() {
    /* Create an arbitrary AutoDiffXd deformation. */
...
void TestParameters() {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE((Model(100.0, 0.5)), "Poisson's ratio must be in .*");

    DRAKE_EXPECT_THROWS_MESSAGE((Model(100.0, 0.6)), "Poisson's ratio must be in .*");

    DRAKE_EXPECT_THROWS_MESSAGE((Model(100.0, -1.0)), "Poisson's ratio must be in .*");

    DRAKE_EXPECT_THROWS_MESSAGE((Model(100.0, -1.1)), "Poisson's ratio must be in .*");
}
...
void TestPIsDerivativeOfPsi() {
    ...
    const std::array<Matrix3<AutoDiffXd>, num_locations> previous_step_deformation_gradients =
    ...
    for (int i = 0; i < num_locations; ++i) {
        EXPECT_TRUE(CompareMatrices(Eigen::Map<const Matrix3d>(energy[i].derivatives().data(), 3, 3), P[i],
    ...
    for (int q = 0; q < num_locations; ++q) {
        for (int i = 0; i < kSpaceDimension; ++i) {
            for (int j = 0; j < kSpaceDimension; ++j) {
                ...
                EXPECT_TRUE(CompareMatrices(Eigen::Map<const Matrix3d>(P[q](i, j).derivatives().data(), 3, 3), dPijdF,
                                            CalcConditionNumberOfInvertibleMatrix<AutoDiffXd>(P[q]) * kTolerance));
            }
        }
    }
}
