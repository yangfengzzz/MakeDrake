...
GTEST_TEST(SchurComplementTest, AnalyticTest) {
  ...
  D << 4, 1,
    ...
    calculation. */
    const SchurComplement<double> s(A.sparseView(), B.transpose().sparseView(), D.sparseView());
    Matrix2d expected_D_complement;
    // clang-format off
  expected_D_complement << 13./5., 1./5.,
                           1./5., 26./15.;
    // clang-format on
    EXPECT_TRUE(CompareMatrices(s.get_D_complement(), expected_D_complement, std::numeric_limits<double>::epsilon()));

    /* Verify that y = -D⁻¹Bᵀx. */
    const Vector2d x(1, 2);
    const Vector2d expected_y(1. / 3., -1. / 3.);
    EXPECT_TRUE(CompareMatrices(s.SolveForY(x), expected_y, std::numeric_limits<double>::epsilon()));
}
