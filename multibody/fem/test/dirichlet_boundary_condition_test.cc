...
protected:
    ...
    static unique_ptr<Block3x3SparseSymmetricMatrix> MakeTangentMatrix() {
        const std::vector<std::vector<int>> sparsity = {std::vector<int>({0}), std::vector<int>({1})};
        contact_solvers::internal::BlockSparsityPattern block_pattern(std::vector<int>(2, 3), sparsity);
...
TEST_F(DirichletBoundaryConditionTest, ApplyBoundaryConditionResidualAndTangentMatrix) {
...
TEST_F(DirichletBoundaryConditionTest, OutOfBound) {
    ...
    bc_.AddBoundaryCondition(FemNodeIndex(100),
                             {Vector3<double>::Zero(), Vector3<double>::Zero(), Vector3<double>::Zero()});
    DRAKE_EXPECT_THROWS_MESSAGE(bc_.ApplyBoundaryConditionToState(fem_state_.get()),
                                "An index of the Dirichlet boundary condition is out of range.");
    VectorXd b = MakeResidual();
    DRAKE_EXPECT_THROWS_MESSAGE(bc_.ApplyHomogeneousBoundaryCondition(&b),
                                "An index of the Dirichlet boundary condition is out of range.");
    auto A = MakeTangentMatrix();
    DRAKE_EXPECT_THROWS_MESSAGE(bc_.ApplyBoundaryConditionToTangentMatrix(A.get()),
                                "An index of the Dirichlet boundary condition is out of range.");
}
