...
struct QuadraticTestFunction2D {
    ...
    static double Eval(const Vector2d& x) {
        return a0 + a1 * x(0) + a2 * x(1) + a3 * x(0) * x(1) + a4 * x(0) * x(0) + a5 * x(1) * x(1);
    }
    ...
    static double EvalAnalyticalIntegralOverUnitTriangle() {
        ...
        return 0.5 * a0 + 1.0 / 6.0 * a1 + 1.0 / 6.0 * a2 + 1.0 / 24.0 * a3 + 1.0 / 12.0 * a4 + 1.0 / 12.0 * a5;
    }
};
...
struct CubicTestFunction2D {
    ...
    static double Eval(const Vector2d& x) {
        ...
        return a0 + a1 * x0 + a2 * x1 + a3 * x0 * x1 + a4 * x0_sqr + a5 * x1_sqr + a6 * x0_cube + a7 * x0_sqr * x1 +
               a8 * x1_sqr * x0 + a9 * x1_cube;
    }
    ...
    static double EvalAnalyticalIntegralOverUnitTriangle() {
        ...
        return 0.5 * a0 + 1.0 / 6.0 * a1 + 1.0 / 6.0 * a2 + 1.0 / 24.0 * a3 + 1.0 / 12.0 * a4 + 1.0 / 12.0 * a5 +
               1.0 / 20.0 * a6 + 1.0 / 60.0 * a7 + 1.0 / 60 * a8 + 1.0 / 20.0 * a9;
    }
};
...
struct LinearTestFunction3D {
    ...
    // evaluated at input x.
    static double Eval(const Vector3d& x) { return a0 + a1 * x(0) + a2 * x(1) + a3 * x(2); }
...
struct QuadraticTestFunction3D {
    ...
    static double Eval(const Vector3d& x) {
        return a0 + a1 * x(0) + a2 * x(1) + a3 * x(2) + a4 * x(0) * x(1) + a5 * x(0) * x(2) + a6 * x(1) * x(2) +
               a7 * x(0) * x(0) + a8 * x(1) * x(1) + a9 * x(2) * x(2);
    }
    ...
    static double EvalAnalyticalIntegralOverUnitTet() {
        ...
        return 1.0 / 6.0 * a0 + 1.0 / 24.0 * a1 + 1.0 / 24.0 * a2 + 1.0 / 24.0 * a3 + 1.0 / 120.0 * a4 +
               1.0 / 120.0 * a5 + 1.0 / 120.0 * a6 + 1.0 / 60.0 * a7 + 1.0 / 60.0 * a8 + 1.0 / 60.0 * a9;
    }
};
...
struct CubicTestFunction3D {
    ...
    static double Eval(const Vector3d& x) {
        ...
        return a0 + a1 * x0 + a2 * x1 + a3 * x2 + a4 * x0 * x1 + a5 * x0 * x2 + a6 * x1 * x2 + a7 * x0_sqr +
               a8 * x1_sqr + a9 * x2_sqr + a10 * x0_cube + a11 * x1_cube + a12 * x2_cube + a13 * x0_sqr * x1 +
               a14 * x0 * x1_sqr + a15 * x0_sqr * x2 + a16 * x0 * x2_sqr + a17 * x1_sqr * x2 + a18 * x1 * x2_sqr +
               a19 * x0 * x1 * x2;
    }
    ...
    static double EvalAnalyticalIntegralOverUnitTet() {
        ...
        //     a₁₄ + a₁₅ + a₁₆ + a₁₇ + a₁₈) + 1/720 * a₁₉.
        return 1.0 / 6.0 * a0 + 1.0 / 24.0 * (a1 + a2 + a3) + 1.0 / 120.0 * (a4 + a5 + a6) +
               1.0 / 60.0 * (a7 + a8 + a9) + 1.0 / 120.0 * (a10 + a11 + a12) +
               1.0 / 360.0 * (a13 + a14 + a15 + a16 + a17 + a18) + 1.0 / 720.0 * a19;
    }
};
...
protected:
    ...
    double Integrate(const Quadrature<natural_dim, num_quadrature_locations>& quadrature,
...
TEST_F(SimplexGaussianQuadratureTest, Linear2D) {
    ...
    const double numerical_integral = Integrate(linear_2d_quadrature_, LinearTestFunction2D::Eval);
    EXPECT_DOUBLE_EQ(LinearTestFunction2D::EvalAnalyticalIntegralOverUnitTriangle(), numerical_integral);
}
...
TEST_F(SimplexGaussianQuadratureTest, Linear3D) {
    ...
    // Numerical integral of f = ∑ᵢ wᵢ f(xᵢ) where wᵢ is the weight of the i-th
    // quadrature point and xᵢ is the location of the i-th quadrature point.
    const double numerical_integral = Integrate(linear_3d_quadrature_, LinearTestFunction3D::Eval);
    EXPECT_DOUBLE_EQ(LinearTestFunction3D::EvalAnalyticalIntegralOverUnitTet(), numerical_integral);
}
...
TEST_F(SimplexGaussianQuadratureTest, Quadratic2D) {
    ...
    // Numerical integral of f = ∑ᵢ wᵢ f(xᵢ) where wᵢ is the weight of the i-th
    // quadrature point and xᵢ is the location of the i-th quadrature point.
    const double numerical_integral = Integrate(quadratic_2d_quadrature_, QuadraticTestFunction2D::Eval);
    EXPECT_DOUBLE_EQ(QuadraticTestFunction2D::EvalAnalyticalIntegralOverUnitTriangle(), numerical_integral);
}
...
TEST_F(SimplexGaussianQuadratureTest, Quadratic3D) {
    ...
    // Numerical integral of f = ∑ᵢ wᵢ f(xᵢ) where wᵢ is the weight of the i-th
    // quadrature point and xᵢ is the location of the i-th quadrature point.
    const double numerical_integral = Integrate(quadratic_3d_quadrature_, QuadraticTestFunction3D::Eval);
    EXPECT_DOUBLE_EQ(QuadraticTestFunction3D::EvalAnalyticalIntegralOverUnitTet(), numerical_integral);
}
...
TEST_F(SimplexGaussianQuadratureTest, Cubic2D) {
    ...
    // Numerical integral of f = ∑ᵢ wᵢ f(xᵢ) where wᵢ is the weight of the i-th
    // quadrature point and xᵢ is the location of the i-th quadrature point.
    const double numerical_integral = Integrate(cubic_2d_quadrature_, CubicTestFunction2D::Eval);
    EXPECT_DOUBLE_EQ(CubicTestFunction2D::EvalAnalyticalIntegralOverUnitTriangle(), numerical_integral);
}
...
TEST_F(SimplexGaussianQuadratureTest, Cubic3D) {
    ...
    // Numerical integral of f = ∑ᵢ wᵢ f(xᵢ) where wᵢ is the weight of the i-th
    // quadrature point and xᵢ is the location of the i-th quadrature point.
    const double numerical_integral = Integrate(cubic_3d_quadrature_, CubicTestFunction3D::Eval);
    EXPECT_DOUBLE_EQ(CubicTestFunction3D::EvalAnalyticalIntegralOverUnitTet(), numerical_integral);
}
