...
#include "multibody/fem/acceleration_newmark_scheme.h"
...
using AutoDiffIsoparametricElement =
        internal::LinearSimplexElement<AutoDiffXd, kNaturalDimension, kSpatialDimension, kNumQuads>;
using AutoDiffConstitutiveModel = internal::LinearConstitutiveModel<AutoDiffXd, kNumQuads>;
using AutoDiffElement = VolumetricElement<AutoDiffIsoparametricElement, QuadratureType, AutoDiffConstitutiveModel>;

using DoubleIsoparametricElement =
        internal::LinearSimplexElement<double, kNaturalDimension, kSpatialDimension, kNumQuads>;
using DoubleConstitutiveModel = internal::LinearConstitutiveModel<double, kNumQuads>;
using DoubleElement = VolumetricElement<DoubleIsoparametricElement, QuadratureType, DoubleConstitutiveModel>;
...
protected:
    ...
    geometry::VolumeMesh<double> MakeBoxTetMesh() {
        ...
        geometry::VolumeMesh<double> mesh = geometry::internal::MakeBoxVolumeMesh<double>(box, kBoxLength);
    ...
    void AddBoxToModel(FemModelType* fem_model) {
        ...
        const typename FemModelType::ConstitutiveModel constitutive_model(kYoungsModulus, kPoissonRatio);
        ...
        builder.AddLinearTetrahedralElements(mesh, constitutive_model, kDensity, damping_model);
        builder.Build();
    }
    ...
    /* Returns an arbitrary FEM data whose generalized positions are different
     ...
     is AutoDiffXd. */
    template <typename FemModelType>
    unique_ptr<FemState<typename FemModelType::T>> MakeDeformedFemState(const FemModelType& fem_model) {
        using T = typename FemModelType::T;
        ...
        if constexpr (std::is_same_v<T, AutoDiffXd>) {
            ...
            const VectorX<double> perturbed_a =
                    math::ExtractValue(reference_fem_state->GetAccelerations()) + perturbation(num_dofs);
            ...
            autodiff_integrator_.AdvanceOneTimeStep(*reference_fem_state, perturbed_a_autodiff,
            ...
            unique_ptr<FemState<T>> deformed_fem_state = fem_model.MakeFemState();
            /* Perturb a. */
            const VectorX<double> perturbed_a = reference_fem_state->GetAccelerations() + perturbation(num_dofs);
            double_integrator_.AdvanceOneTimeStep(*reference_fem_state, perturbed_a, deformed_fem_state.get());
            return deformed_fem_state;
        }
        DRAKE_UNREACHABLE();
    }
    ...
    AccelerationNewmarkScheme<AutoDiffXd> autodiff_integrator_{kDt, kGamma, kBeta};
...
TEST_F(VolumetricModelTest, TangentMatrixIsResidualDerivative) {
    unique_ptr<FemState<AutoDiffXd>> autodiff_state = MakeDeformedFemState(model_);
    ...
    unique_ptr<FemState<double>> double_state = MakeDeformedFemState(double_model);
    auto tangent_matrix = double_model.MakeTangentMatrix();
    double_model.CalcTangentMatrix(*double_state, double_integrator_.GetWeights(), tangent_matrix.get());
    const MatrixXd tangent_matrix_dense = tangent_matrix->MakeDenseMatrix();
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(residual), tangent_matrix_dense,
...
TEST_F(VolumetricModelTest, MultipleMesh) {
    ...
    geometry::VolumeMesh<double> mesh = MakeBoxTetMesh();
    const AutoDiffConstitutiveModel constitutive_model(kYoungsModulus, kPoissonRatio);
    ...
    EXPECT_TRUE(CompareMatrices(q.head<kNumDofs>(), q.segment<kNumDofs>(kNumDofs)));
    EXPECT_TRUE(CompareMatrices(q.head<kNumDofs>(), q.segment<kNumDofs>(2 * kNumDofs)));
    EXPECT_TRUE(CompareMatrices(q.head<kNumDofs>(), q.segment<kNumDofs>(3 * kNumDofs)));
}
...
TEST_F(VolumetricModelTest, ElasticEnergy) {
    ...
    const DoubleModel::ConstitutiveModel constitutive_model(kYoungsModulus, kPoissonRatio);
    const double mu = constitutive_model.shear_modulus();
    const double lambda = constitutive_model.lame_first_parameter();
    const double energy_density = mu * strain.squaredNorm() + 0.5 * lambda * strain.trace() * strain.trace();
...
TEST_F(VolumetricModelTest, Clone) {
    ...
    const DoubleModel* double_clone = dynamic_cast<const DoubleModel*>(clone.get());
    ASSERT_NE(double_clone, nullptr);

    std::unique_ptr<FemState<double>> state = MakeDeformedFemState(double_model);
    std::unique_ptr<FemState<double>> clone_state = MakeDeformedFemState(*double_clone);
