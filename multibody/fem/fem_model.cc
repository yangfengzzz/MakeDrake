...
template <typename T>
void FemModel<T>::CalcTangentMatrix(const FemState<T>& fem_state,
                                    const Vector3<T>& weights,
                                    contact_solvers::internal::Block3x3SparseSymmetricMatrix* tangent_matrix) const {
    if constexpr (std::is_same_v<T, double>) {
        ...
        throw std::logic_error("FemModel::CalcTangentMatrix() only supports double at the moment.");
    }
}

template <typename T>
std::unique_ptr<contact_solvers::internal::Block3x3SparseSymmetricMatrix> FemModel<T>::MakeTangentMatrix() const {
    if constexpr (std::is_same_v<T, double>) {
        return DoMakeTangentMatrix();
    } else {
        throw std::logic_error("FemModel::MakeTangentMatrix() only supports double at the moment.");
    }
}
...
template <typename T>
FemModel<T>::FemModel()
    : fem_state_system_(std::make_unique<internal::FemStateSystem<T>>(VectorX<T>(0), VectorX<T>(0), VectorX<T>(0))) {}

template <typename T>
void FemModel<T>::ThrowIfModelStateIncompatible(const char* func, const FemState<T>& fem_state) const {
    if (!is_compatible_with(fem_state)) {
        throw std::logic_error(std::string(func) + "(): The FEM model and state are not compatible.");
    }
}
...
void FemModel<T>::UpdateFemStateSystem() {
    ...
    fem_state_system_ =
            std::make_unique<internal::FemStateSystem<T>>(model_positions, model_velocities, model_accelerations);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::fem::FemModel);
