#include "multibody/fem/schur_complement.h"
...
template <typename T>
SchurComplement<T>::SchurComplement(const Eigen::Ref<const Eigen::SparseMatrix<T>>& A,
...
template <typename T>
SchurComplement<T>::SchurComplement(MatrixX<T>&& D_complement, MatrixX<T>&& neg_Dinv_B_transpose)
    : D_complement_(std::move(D_complement)), neg_Dinv_B_transpose_(std::move(neg_Dinv_B_transpose)) {
...
template <typename T>
VectorX<T> SchurComplement<T>::SolveForY(const Eigen::Ref<const VectorX<T>>& x) const {
