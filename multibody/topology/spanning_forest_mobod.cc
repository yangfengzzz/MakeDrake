...
SpanningForest::Mobod::Mobod(MobodIndex mobod_index, BodyIndex world_link)
    : level_(0), index_(mobod_index), q_start_(0), nq_(0), v_start_(0), nv_(0), nq_inboard_(0), nv_inboard_(0) {
...
SpanningForest::Mobod::Mobod(
        MobodIndex mobod_index, BodyIndex link_index, JointIndex joint_index, int level, bool is_reversed)
    : joint_index_(joint_index), use_reverse_mobilizer_(is_reversed), level_(level), index_(mobod_index) {
    DRAKE_DEMAND(mobod_index.is_valid() && link_index.is_valid() && joint_index.is_valid());
    DRAKE_DEMAND(mobod_index != 0 && link_index != 0 && level > 0);
    follower_links_.push_back(link_index);
}

void SpanningForest::Mobod::FixupAfterReordering(const std::vector<MobodIndex>& old_to_new) {
...
void SpanningForest::Mobod::RenumberMobodIndexVector(const std::vector<MobodIndex>& old_to_new,
