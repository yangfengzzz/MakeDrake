...
public:
    static LinkJointGraph::Link MakeLink(BodyIndex index,
                                         std::string name,
    ...
    static LinkJointGraph::Joint MakeJoint(JointIndex index,
                                           std::string name,
                                           ModelInstanceIndex model_instance,
                                           ...
                                           JointFlags flags) {
        return LinkJointGraph::Joint(index, std::move(name), model_instance, joint_traits_index, parent_link_index,
                                     child_link_index, flags);
    }

    static LinkFlags set_link_flags(LinkFlags to_set, LinkJointGraph::Link* link) { return link->set_flags(to_set); }

    static JointFlags set_joint_flags(JointFlags to_set, LinkJointGraph::Joint* joint) {
        return joint->set_flags(to_set);
    }

    static LoopConstraintIndex AddLoopClosingWeldConstraint(BodyIndex primary_link_index,
                                                            BodyIndex shadow_link_index,
                                                            LinkJointGraph* graph) {
        return graph->AddLoopClosingWeldConstraint(primary_link_index, shadow_link_index);
    }

    static std::vector<BodyIndex> static_links(const LinkJointGraph& graph) { return graph.static_links(); }

    static std::vector<BodyIndex> non_static_must_be_base_body_links(const LinkJointGraph& graph) {
...
GTEST_TEST(LinkJointGraph, FlagsAndOptions) {
    ...
    static_assert(std::is_same_v<decltype(forest_building_options), ForestBuildingOptions>);
    EXPECT_EQ(forest_building_options & use_fixed_base, ForestBuildingOptions::kUseFixedBase);
    EXPECT_EQ(forest_building_options & combine_links, ForestBuildingOptions::kCombineLinkComposites);
    EXPECT_FALSE(static_cast<bool>(forest_building_options & ForestBuildingOptions::kUseRpyFloatingJoints));
    EXPECT_EQ(forest_building_options & ForestBuildingOptions::kUseRpyFloatingJoints, ForestBuildingOptions::kDefault);
    ...
    EXPECT_EQ(joint_flags & JointFlags::kMustBeModeled, JointFlags::kMustBeModeled);
    EXPECT_EQ(joint_flags | JointFlags::kMustBeModeled, JointFlags::kMustBeModeled);
}
...
GTEST_TEST(LinkJointGraph, SpecifyForestBuildingOptions) {
    ...
    const ForestBuildingOptions two_options =
            ForestBuildingOptions::kCombineLinkComposites | ForestBuildingOptions::kUseRpyFloatingJoints;
    ...
    EXPECT_EQ(graph.get_forest_building_options_in_use(default_model_instance()), default_options);
    EXPECT_EQ(graph.get_forest_building_options_in_use(ModelInstanceIndex(73)), default_options);
    ...
    graph.SetForestBuildingOptions(ModelInstanceIndex(9), ForestBuildingOptions::kStatic);
    ...
    EXPECT_EQ(graph.get_global_forest_building_options(), default_options);
    EXPECT_EQ(graph.get_forest_building_options_in_use(ModelInstanceIndex(9)), default_options);
}
...
GTEST_TEST(LinkJointGraph, WorldOnlyTest) {
    ...
    const LinkJointGraph::JointTraits& weld_type = graph.joint_traits(LinkJointGraph::weld_joint_traits_index());
    ...
    const LinkJointGraph::JointTraits& quaternion_floating_type =
            graph.joint_traits(LinkJointGraph::quaternion_floating_joint_traits_index());
    ...
    const BodyIndex dummy_index = graph.AddLink("dummy", default_model_instance());
    graph.AddJoint("joint0", default_model_instance(), "revolute", world_index(), dummy_index);
...
GTEST_TEST(LinkJointGraph, AddLinkErrors) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddLink("not_world", world_model_instance()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddLink("link1", ModelInstanceIndex(3)),
    ...
    const BodyIndex link2_index =
            graph.AddLink("link2", ModelInstanceIndex(3), LinkFlags::kTreatAsMassless | LinkFlags::kMustBeBaseBody);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddLink("link3", ModelInstanceIndex(3), LinkFlags::kShadow),
...
GTEST_TEST(LinkJointGraph, InternalListsAreBuiltCorrectly) {
    ...
    const BodyIndex link1_index = graph.AddLink("link1", default_model_instance(), LinkFlags::kStatic);
    const BodyIndex link2_index = graph.AddLink("link2", default_model_instance(), LinkFlags::kMustBeBaseBody);
    const BodyIndex link3_index =
            graph.AddLink("link3", default_model_instance(), LinkFlags::kStatic | LinkFlags::kMustBeBaseBody);

    // Links 1 and 3 are static.
    EXPECT_EQ(LinkJointGraphTester::static_links(graph), (std::vector<BodyIndex>{link1_index, link3_index}));
    // But only link 2 should be on the non-static must be base body list.
    EXPECT_EQ(LinkJointGraphTester::non_static_must_be_base_body_links(graph), std::vector<BodyIndex>{link2_index});
}

// Check operation of the public members of the Link subclass.
GTEST_TEST(LinkJoinGraph, LinkAPITest) {
    LinkJointGraph::Link link5 =
            LinkJointGraphTester::MakeLink(BodyIndex(5), "link5", ModelInstanceIndex(7), LinkFlags::kMustBeBaseBody);
...
GTEST_TEST(LinkJointGraph, JointAPITest) {
    ...
    LinkJointGraph::Joint joint3 = LinkJointGraphTester::MakeJoint(
            JointIndex(3), "joint3", ModelInstanceIndex(9), LinkJointGraph::rpy_floating_joint_traits_index(),
            parent_index, child_index, JointFlags::kMustBeModeled);
    ...
    EXPECT_EQ(joint3.traits_index(), LinkJointGraph::rpy_floating_joint_traits_index());
...
GTEST_TEST(LinkJointGraph, SerialChainAndMore) {
    ...
    const JointTraitsIndex revolute_index = graph.RegisterJointType("revolute", 1, 1);
    ...
    const LinkJointGraph::JointTraits& revolute_joint_traits = graph.joint_traits(revolute_index);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            ...
            fmt::format("AddLink\\(\\): Model instance index {} is reserved.*", world_model_instance()));
    ...
    EXPECT_EQ(link4.joints(), (std::vector<JointIndex>{JointIndex(3), JointIndex(4)}));
    ...
    const LoopConstraintIndex constraint0_index =
            LinkJointGraphTester::AddLoopClosingWeldConstraint(link4.index(),  // primary link
                                                               link5.index(),  // shadow link
                                                               &graph);
    EXPECT_EQ(constraint0_index, LoopConstraintIndex(0));
    EXPECT_EQ(ssize(graph.loop_constraints()), 1);
    const LinkJointGraph::LoopConstraint& constraint0 = graph.loop_constraints(constraint0_index);
    ...
    EXPECT_EQ(link4.loop_constraints(), std::vector<LoopConstraintIndex>{constraint0_index});
    EXPECT_EQ(link5.loop_constraints(), std::vector<LoopConstraintIndex>{constraint0_index});
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("pin3", model_instance, "revolute", BodyIndex(1), BodyIndex(2)),
                                "AddJoint.*already a joint named.*pin3.*model instance.*5.*");

    // We cannot add a redundant joint.
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("other", model_instance, "revolute", BodyIndex(1), BodyIndex(2)),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("reverse", model_instance, "revolute", BodyIndex(2), BodyIndex(1)),
                                "AddJoint\\(\\):.*already has joint.*pin2.*instance 5.*"
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("screw1", model_instance, "screw", BodyIndex(1), BodyIndex(2)),
                                "AddJoint\\(\\): Unrecognized type.*");

    // Invalid parent/child Link throws.
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("another_pin", model_instance, "revolute", BodyIndex(1), BodyIndex(9)),
                                "AddJoint\\(\\): child link index 9 for joint '.*' is out of range.");
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("another_pin", model_instance, "revolute", BodyIndex(9), BodyIndex(1)),
                                "AddJoint\\(\\): parent link index 9 for joint '.*' is out of range.");

    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("joint_to_self", model_instance, "revolute", BodyIndex(5), BodyIndex(5)),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            graph.AddJoint("static8_pin", model_instance, "revolute", graph.world_link().index(), static8_index),
    ...
    const JointIndex joint_10_11_index = graph.AddJoint("weld", model_instance, "weld", link10_index, base11_index);

    EXPECT_EQ(graph.MaybeGetJointBetween(link10_index, base11_index), joint_10_11_index);
    EXPECT_EQ(graph.MaybeGetJointBetween(base11_index, link10_index), joint_10_11_index);
    EXPECT_FALSE(graph.MaybeGetJointBetween(world_index(), link10_index).has_value());
}
