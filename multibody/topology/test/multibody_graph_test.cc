...
GTEST_TEST(MultibodyGraph, SerialChain) {
    ...
    // to AddRigidBody().
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddRigidBody("InvalidBody", world_model_instance()),
                                fmt::format("AddRigidBody\\(\\): Model instance index = {}.*", world_model_instance()));
    ...
    for (int i = 2; i <= 5; ++i) {
        BodyIndex child = graph.AddRigidBody("body" + std::to_string(i), model_instance);
        graph.AddJoint("pin" + std::to_string(i), model_instance, kRevoluteType, parent, child);
        parent = child;
    }
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("pin3", model_instance, kRevoluteType, BodyIndex(1), BodyIndex(2)),
                                "AddJoint\\(\\): Duplicate joint name.*");

    // We cannot add a redundant joint.
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("other", model_instance, kRevoluteType, BodyIndex(1), BodyIndex(2)),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.AddJoint("reverse", model_instance, kRevoluteType, BodyIndex(2), BodyIndex(1)),
                                "This MultibodyGraph already has a joint 'pin2' connecting 'body1'"
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(
            graph.AddJoint("another_pin", model_instance, kRevoluteType, BodyIndex(1), BodyIndex(9)),
            "AddJoint\\(\\): child body index for joint '.*' is invalid.");
    DRAKE_EXPECT_THROWS_MESSAGE(
            graph.AddJoint("another_pin", model_instance, kRevoluteType, BodyIndex(9), BodyIndex(1)),
    ...
    EXPECT_EQ(graph.get_body(BodyIndex(0)).model_instance(), world_model_instance());
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(graph.get_body(BodyIndex(9)), ".*index < num_bodies\\(\\).*");
    DRAKE_EXPECT_THROWS_MESSAGE(graph.get_joint(JointIndex(9)), ".*condition 'has_joint\\(index\\)' failed.*");
...
GTEST_TEST(MultibodyGraph, Weldedsubgraphs) {
    ...
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(1), BodyIndex(13));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(4), BodyIndex(1));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(13), BodyIndex(4));

    // Body 3 connects to subgraph A via a revolute joint.
    graph.AddJoint("joint" + std::to_string(j++), model_instance, kRevoluteType, BodyIndex(3), BodyIndex(13));

    // subgraph B: formed by bodies 8, 6, 10.
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(10), BodyIndex(6));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(10), BodyIndex(8));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(6), BodyIndex(8));

    // Body 1 (in subgraph A) and body 10 (in subgraph B) connect through a
    // prismatic joint.
    graph.AddJoint("joint" + std::to_string(j++), model_instance, kPrismaticType, BodyIndex(1), BodyIndex(10));

    // Closed kinematic loop of non-weld joints.
    graph.AddJoint("joint" + std::to_string(j++), model_instance, kPrismaticType, BodyIndex(2), BodyIndex(11));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, kPrismaticType, BodyIndex(7), BodyIndex(2));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, kRevoluteType, BodyIndex(7), BodyIndex(11));

    // subgraph C: formed by bodies 5, 7, 12.
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(5), BodyIndex(7));
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(5), world_index());
    graph.AddJoint("joint" + std::to_string(j++), model_instance, graph.weld_type_name(), BodyIndex(12), BodyIndex(5));

    EXPECT_EQ(graph.num_bodies(), 14);  // this includes the world body.

    const std::vector<std::set<BodyIndex>> welded_subgraphs = graph.FindSubgraphsOfWeldedBodies();
    ...
    const std::set<BodyIndex>& expected_world_subgraph =
            *expected_subgraphs.insert({BodyIndex(0), BodyIndex(5), BodyIndex(7), BodyIndex(12)}).first;
    const std::set<BodyIndex>& expected_subgraphA =
            *expected_subgraphs.insert({BodyIndex(1), BodyIndex(4), BodyIndex(13)}).first;
    const std::set<BodyIndex>& expected_subgraphB =
            *expected_subgraphs.insert({BodyIndex(6), BodyIndex(8), BodyIndex(10)}).first;
    ...
    const std::set<std::set<BodyIndex>> welded_subgraphs_set(welded_subgraphs.begin(), welded_subgraphs.end());
...
GTEST_TEST(MultibodyGraph, RemoveJoint) {
    ...
    JointIndex joint5 = graph.AddJoint("joint", model_instance5, graph.weld_type_name(), BodyIndex(0), body0);
    JointIndex joint9 = graph.AddJoint("joint", model_instance9, graph.weld_type_name(), BodyIndex(0), body1);
