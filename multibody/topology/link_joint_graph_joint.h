...
public:
    ...
    the specified `link`. */
    bool connects(BodyIndex link) const { return link == parent_link() || link == child_link(); }
    ...
    bool connects(BodyIndex link1, BodyIndex link2) const {
        return (parent_link() == link1 && child_link() == link2) || (parent_link() == link2 && child_link() == link1);
    }
    ...
    bool must_be_modeled() const { return static_cast<bool>(flags_ & JointFlags::kMustBeModeled); }
    ...
    bool has_been_processed() const { return !std::holds_alternative<std::monostate>(how_modeled_); }
...
private:
    ...
    JointFlags set_flags(JointFlags to_set) {
        return flags_ = (to_set == JointFlags::kDefault ? JointFlags::kDefault : flags_ | to_set);
    }
