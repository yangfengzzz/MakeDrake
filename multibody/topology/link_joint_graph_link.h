...
public:
    ...
    const std::vector<JointIndex>& joints_as_parent() const { return joints_as_parent_; }

    /** Returns indexes of all the joints that connect to this %Link in which
    this is the child %Link. */
    const std::vector<JointIndex>& joints_as_child() const { return joints_as_child_; }

    /** Returns indexes of all the LoopConstraints that connect to this %Link. */
    const std::vector<LoopConstraintIndex>& loop_constraints() const { return loop_constraints_; }
    ...
    World). */
    bool is_anchored() const { return is_world() || is_static() || (composite() == LinkCompositeIndex(0)); }

    /** Returns `true` if this %Link was added with LinkFlags::kStatic. */
    bool is_static() const { return static_cast<bool>(flags_ & LinkFlags::kStatic); }

    /** Returns `true` if this %Link was added with LinkFlags::kMustBeBaseBody. */
    bool must_be_base_body() const { return static_cast<bool>(flags_ & LinkFlags::kMustBeBaseBody); }

    /** Returns `true` if this %Link was added with
    LinkFlags::kTreatAsMassless. */
    bool treat_as_massless() const { return static_cast<bool>(flags_ & LinkFlags::kTreatAsMassless); }

    /** Returns `true` if this is a shadow Link added by BuildForest(). */
    bool is_shadow() const { return static_cast<bool>(flags_ & LinkFlags::kShadow); }
    ...
    std::optional<LinkCompositeIndex> composite() const { return link_composite_index_; }

private:
    friend class LinkJointGraph;
    friend class LinkJointGraphTester;

    Link(BodyIndex index, std::string name, ModelInstanceIndex model_instance, LinkFlags flags);
    ...
    LinkFlags set_flags(LinkFlags to_set) {
        return flags_ = (to_set == LinkFlags::kDefault ? LinkFlags::kDefault : flags_ | to_set);
    }
    ...
    void add_loop_constraint(LoopConstraintIndex constraint) { loop_constraints_.push_back(constraint); }
