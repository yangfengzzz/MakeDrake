...
public:
    ...
    const SpanningForest::Mobod* begin() const { return &forest_->mobods()[base_mobod_]; }

    /** An iterator pointing one entry past the last_mobod(), suitable for range
    iteration. Don't dereference this! */
    const SpanningForest::Mobod* end() const { return &forest_->mobods()[last_mobod_] + 1; }

    /** The Mobod whose index is returned by base_mobod(). */
    const SpanningForest::Mobod& front() const { return forest_->mobods()[base_mobod_]; }

    /** The Mobod whose index is returned by last_mobod(). */
    const SpanningForest::Mobod& back() const { return forest_->mobods()[last_mobod_]; }
