...
#include "multibody/topology/forest.h"
...
LinkJointGraph::LinkJointGraph(const LinkJointGraph& source) : data_(source.data_) {
    data_.forest->SetNewOwner(this);
}

LinkJointGraph::LinkJointGraph(LinkJointGraph&& source) : data_(std::move(source.data_)) {
    data_.forest->SetNewOwner(this);
    source.Clear();
}
...
void LinkJointGraph::Clear() {
    ...
    DRAKE_DEMAND(RegisterJointType("quaternion_floating", 7, 6, true) == quaternion_floating_joint_traits_index());
    DRAKE_DEMAND(RegisterJointType("rpy_floating", 6, 6) == rpy_floating_joint_traits_index());
...
void LinkJointGraph::SetGlobalForestBuildingOptions(ForestBuildingOptions global_options) {
    InvalidateForest();
...
// building.
void LinkJointGraph::SetForestBuildingOptions(ModelInstanceIndex instance_index, ForestBuildingOptions options) {
    InvalidateForest();
    if (instance_index >= ssize(data_.model_instance_forest_building_options)) {
        data_.model_instance_forest_building_options.resize(instance_index + 1, std::nullopt);
    }
...
void LinkJointGraph::InvalidateForest() {
    ...
    data_.links.erase(data_.links.begin() + data_.num_user_links, data_.links.end());
    data_.joints.erase(data_.joints.begin() + data_.num_user_joints, data_.joints.end());
...
BodyIndex LinkJointGraph::AddLink(const std::string& link_name, ModelInstanceIndex model_instance, LinkFlags flags) {
    ...
    if (ssize(links()) > 0 && model_instance == world_model_instance()) {
        throw std::logic_error(
                fmt::format("{}(): Model instance index {} is reserved for the World link. "
...
bool LinkJointGraph::HasLinkNamed(std::string_view name, ModelInstanceIndex model_instance_index) const {
...
bool LinkJointGraph::HasJointNamed(std::string_view name, ModelInstanceIndex model_instance_index) const {
    DRAKE_DEMAND(model_instance_index.is_valid());
    ...
    const auto model_range = data_.ephemeral_joint_name_to_index.equal_range(name);
    for (auto it = model_range.first; it != model_range.second; ++it) {
        if (joints(it->second).model_instance() == model_instance_index) {
            return true;
        }
...
std::optional<JointIndex> LinkJointGraph::MaybeGetJointBetween(BodyIndex link1_index, BodyIndex link2_index) const {
    ...
    const auto [joint_list_ptr, link_to_look_for] = ssize(link1.joints()) <= ssize(link2.joints())
...
JointIndex LinkJointGraph::AddJoint(const std::string& name,
    ...
    if (parent_link_index >= ssize(links())) {
        throw std::range_error(fmt::format("{}(): parent link index {} for joint '{}' is out of range.", __func__,
                                           parent_link_index, name));
    }
    if (child_link_index >= ssize(links())) {
        throw std::range_error(fmt::format("{}(): child link index {} for joint '{}' is out of range.", __func__,
                                           child_link_index, name));
    }

    if (parent_link_index == child_link_index) {
        throw std::logic_error(
                fmt::format("{}(): Joint '{}' (model instance {}) would connect link '{}' "
                            "to itself.",
    ...
    if (!type_index.has_value()) {
        throw std::logic_error(fmt::format("{}(): Unrecognized type '{}' for joint '{}' (model instance {}).", __func__,
                                           type, name, model_instance_index));
    }
    ...
    if (is_static && type_index != weld_joint_traits_index()) {
        const std::string static_link_name = new_parent.is_world() ? new_child.name() : new_parent.name();
        throw std::logic_error(
    ...
    if (std::optional<JointIndex> existing_joint_index = MaybeGetJointBetween(parent_link_index, child_link_index)) {
        ...
        throw std::logic_error(
                fmt::format("{}(): This LinkJointGraph already has joint '{}' (model instance {}) "
                            "connecting link '{}' to link '{}'. Therefore adding joint '{}' "
                            "(model instance {}) connecting link '{}' to link '{}' is not allowed.",
                            __func__, existing_joint.name(), existing_joint.model_instance(), existing_parent.name(),
                            existing_child.name(), name, model_instance_index, new_parent.name(), new_child.name()));
    }
    ...
    data_.joints.emplace_back(
            Joint(joint_index, name, model_instance_index, *type_index, parent_link_index, child_link_index, flags));
...
JointTraitsIndex LinkJointGraph::RegisterJointType(const std::string& joint_type_name,
                                                   int nq,
                                                   int nv,
                                                   bool has_quaternion) {
    ...
    if (it != data_.joint_type_name_to_index.end()) {
        throw std::logic_error(fmt::format("{}(): Duplicate joint type: '{}'.", __func__, joint_type_name));
    }
    ...
    data_.joint_traits.push_back({.name = joint_type_name, .nq = nq, .nv = nv, .has_quaternion = has_quaternion});
    data_.joint_type_name_to_index[joint_type_name] = joint_traits_index;
    DRAKE_DEMAND(data_.joint_type_name_to_index.size() == data_.joint_traits.size());
    return joint_traits_index;
}

bool LinkJointGraph::IsJointTypeRegistered(const std::string& joint_type_name) const {
    const auto it = data_.joint_type_name_to_index.find(joint_type_name);
...
LoopConstraintIndex LinkJointGraph::AddLoopClosingWeldConstraint(BodyIndex primary_link_index,
    ...
    data_.loop_constraints.emplace_back(index, shadow_link.name(), shadow_link.model_instance(),
...
std::optional<JointTraitsIndex> LinkJointGraph::GetJointTraitsIndex(const std::string& joint_type_name) const {
    const auto it = data_.joint_type_name_to_index.find(joint_type_name);
...
void LinkJointGraph::ChangeJointFlags(JointIndex joint_index, JointFlags flags) {
    InvalidateForest();
    mutable_joint(joint_index).set_flags(flags);
}

void LinkJointGraph::ChangeJointType(JointIndex existing_joint_index, const std::string& name_of_new_type) {
    DRAKE_DEMAND(existing_joint_index.is_valid() && existing_joint_index < ssize(joints()));
    const std::optional<JointTraitsIndex> new_traits_index = GetJointTraitsIndex(name_of_new_type);
    ...
    const bool is_static = (parent_link.is_world() && link_is_static(child_link)) ||
                           (child_link.is_world() && link_is_static(parent_link));
    if (is_static && new_traits_index != weld_joint_traits_index()) {
        const std::string static_link_name = parent_link.is_world() ? child_link.name() : parent_link.name();
        throw std::logic_error(
                fmt::format("{}(): can't change type of joint {} (in model instance {}) from {} to "
                            "{} because it connects static link {} to World; only a weld is "
                            "permitted for a static link.",
                            __func__, joint.name(), joint.model_instance(), joint_traits(joint.traits_index()).name,
                            name_of_new_type, static_link_name));
    }
...
JointIndex LinkJointGraph::AddEphemeralJointToWorld(JointTraitsIndex type_index, BodyIndex child_link_index) {
    ...
    while (HasJointNamed(joint_name, model_instance)) joint_name = "_" + joint_name;
    ...
    data_.joints.emplace_back(Joint(new_joint_index, joint_name, model_instance, type_index, BodyIndex(0),
                                    child_link_index, JointFlags::kDefault));
    // Links need to know their joints.
...
LinkCompositeIndex LinkJointGraph::AddToLinkComposite(BodyIndex maybe_composite_link_index, BodyIndex new_link_index) {
    DRAKE_ASSERT(maybe_composite_link_index.is_valid() && new_link_index.is_valid());
    ...
    std::optional<LinkCompositeIndex> existing_composite = maybe_composite_link.link_composite_index_;
    if (!existing_composite.has_value()) {
        ...
        data_.link_composites.emplace_back(std::vector<BodyIndex>{maybe_composite_link_index});
    }
...
BodyIndex LinkJointGraph::AddShadowLink(BodyIndex primary_link_index,
    ...
    std::string shadow_link_name = fmt::format("{}${}", primary_link.name(), shadow_num);
    while (HasLinkNamed(shadow_link_name, primary_link.model_instance())) shadow_link_name = "_" + shadow_link_name;
    const BodyIndex shadow_link_index(ssize(links()));
    DRAKE_DEMAND(shadow_link_index >= num_user_links());  // A sanity check.
    data_.ephemeral_link_name_to_index.insert({shadow_link_name, shadow_link_index});
    data_.links.emplace_back(
            Link(shadow_link_index, shadow_link_name, primary_link.model_instance(), LinkFlags::kShadow));
...
void LinkJointGraph::RenumberMobodIndexes(const std::vector<MobodIndex>& old_to_new) {
...
std::tuple<BodyIndex, BodyIndex, bool> LinkJointGraph::FindInboardOutboardLinks(MobodIndex inboard_mobod_index,
                                                                                JointIndex joint_index) const {
    const Joint& joint = joints(joint_index);
    const Link& parent_link = links(joint.parent_link());
    if (parent_link.mobod_index().is_valid() && parent_link.mobod_index() == inboard_mobod_index) {
        return std::make_tuple(joint.parent_link(), joint.child_link(), false);
    }
    const Link& child_link = links(joint.child_link());
    DRAKE_DEMAND(child_link.mobod_index().is_valid() && child_link.mobod_index() == inboard_mobod_index);
...
bool LinkJointGraph::link_is_static(const Link& link) const {
    ...
    return static_cast<bool>(get_forest_building_options_in_use(link.model_instance()) &
...
LinkJointGraph::Link::Link(BodyIndex index, std::string name, ModelInstanceIndex model_instance, LinkFlags flags)
    : index_(index), name_(std::move(name)), model_instance_(model_instance), flags_(flags) {
    DRAKE_DEMAND(index_.is_valid() && !name_.empty() && model_instance_.is_valid());
...
void LinkJointGraph::Link::ClearModel(int num_user_joints) {
    ...
    auto remove_ephemeral_joints = [num_user_joints](std::vector<JointIndex>& joints) {
        while (!joints.empty() && joints.back() >= num_user_joints) joints.pop_back();
    };
...
LinkJointGraph::Joint::Joint(JointIndex index,
                             std::string name,
    ...
    DRAKE_DEMAND(index_.is_valid() && !name_.empty() && model_instance_.is_valid());
    DRAKE_DEMAND(traits_index_.is_valid() && parent_link_index_.is_valid() && child_link_index_.is_valid());
