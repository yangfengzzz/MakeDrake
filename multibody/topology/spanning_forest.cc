...
SpanningForest::SpanningForest(const SpanningForest& source) : data_(source.data_) {
    FixInternalPointers();
}

SpanningForest::SpanningForest(SpanningForest&& source) : data_(std::move(source.data_)) {
    FixInternalPointers();
}
...
bool SpanningForest::BuildForest() {
    ...
    mutable_graph().set_primary_mobod_for_link(BodyIndex(0), MobodIndex(0), JointIndex{});
...
void SpanningForest::ChooseForestTopology() {
    ...
    ConnectLinksToWorld(graph().non_static_must_be_base_body_links(), false /* use floating joint */);
...
std::vector<MobodIndex> SpanningForest::CreateDepthFirstReordering() const {
    ...
    while (!to_process.empty()) {
        ...
        const std::vector<MobodIndex>& outboard_mobods = mobods(top_index).outboard_mobods_;
        for (auto outboard = outboard_mobods.rbegin(); outboard != outboard_mobods.rend(); ++outboard) {
...
void SpanningForest::FixupForestToUseNewNumbering(const std::vector<MobodIndex>& old_to_new) {
    ...
    for (auto& constraint : data_.loop_constraints) constraint.FixupAfterReordering(old_to_new);
    for (auto& tree : data_.trees) tree.FixupAfterReordering(old_to_new);
    for (auto& welded_mobods : data_.welded_mobods) Mobod::RenumberMobodIndexVector(old_to_new, &welded_mobods);
...
void SpanningForest::AssignCoordinates() {
    ...
    for (auto& mobod : data_.mobods) {
        ...
        const JointTraitsIndex joint_traits_index = joints(mobod.joint()).traits_index();
        const LinkJointGraph::JointTraits& joint_traits = graph().joint_traits()[joint_traits_index];
        ...
        DRAKE_DEMAND(ssize(data_.q_to_mobod) == next_q && ssize(data_.v_to_mobod) == next_v);
        for (int i = 0; i < mobod.nq_; ++i) data_.q_to_mobod.push_back(mobod.index());
        for (int i = 0; i < mobod.nv_; ++i) data_.v_to_mobod.push_back(mobod.index());
    ...
    for (auto m = data_.mobods.rbegin(); m != data_.mobods.rend(); ++m) {
        ...
        for (MobodIndex child_index : m->outboard_mobods_) {
            ...
            DRAKE_DEMAND(child.nq_outboard_ >= 0 && child.nv_outboard_ >= 0 && child.num_subtree_mobods_ >= 1);
...
void SpanningForest::ChooseBaseBodiesAndAddTrees(int* num_unprocessed_links) {
    ...
    std::priority_queue<BodyIndex, std::vector<BodyIndex>, decltype(data_.base_body_policy)> eligible_bases(
            data_.base_body_policy, std::move(jointed_links));
    ...
    while (*num_unprocessed_links > ssize(unjointed_links)) {
        ...
        const ModelInstanceIndex model_instance_index = links(next_base_link).model_instance();
        const JointIndex next_joint_index =
                mutable_graph().AddEphemeralJointToWorld(base_joint_traits_index(model_instance_index), next_base_link);
    ...
    for (BodyIndex unjointed_link : unjointed_links) {
        const ModelInstanceIndex model_instance_index = links(unjointed_link).model_instance();
        const JointTraitsIndex joint_type_to_use = base_joint_traits_index(model_instance_index);
        const JointIndex next_joint_index = mutable_graph().AddEphemeralJointToWorld(joint_type_to_use, unjointed_link);
...
void SpanningForest::ExtendTrees(const std::vector<JointIndex>& joints_to_model, int* num_unprocessed_links) {
...
void SpanningForest::ExtendTreesOneLevel(const std::vector<JointIndex>& joints_to_model,
    ...
    for (JointIndex joint_index : joints_to_model) {
        ...
        if (joint.has_been_processed()) continue;  // Already did this one from the other side.

        /* At least one of the Joint's Links must have already been modeled.
        (Could be both in the case of a loop.) */
        const bool parent_is_modeled = link_is_already_in_forest(joint.parent_link());
        const bool child_is_modeled = link_is_already_in_forest(joint.child_link());
        DRAKE_DEMAND(parent_is_modeled || child_is_modeled);

        const BodyIndex inboard_link_index = parent_is_modeled ? joint.parent_link() : joint.child_link();

        /* Don't keep references to Mobods since we're growing the vector below. */
        const MobodIndex inboard_mobod_index = graph().link_to_mobod(inboard_link_index);
        ...
        const auto [modeled_inboard_link_index, modeled_outboard_link_index, is_reversed] =
                graph().FindInboardOutboardLinks(inboard_mobod_index, modeled_joint_index);
        ...
        AddNewMobod(modeled_outboard_link_index, modeled_joint_index, inboard_mobod_index,
        ...
        if (!modeled_joint.is_weld() && modeled_outboard_link.treat_as_massless()) {
            ...
            if (ssize(modeled_outboard_link.joints()) == 1 && data_.dynamics_ok) {
                ...
                data_.why_no_dynamics = fmt::format(
                        ...
                        modeled_outboard_link.name(), modeled_joint_type.name, modeled_joint.name());
            }
        ...
        for (JointIndex next_joint_index : modeled_outboard_link.joints()) {
            if (!joints(next_joint_index).has_been_processed()) joints_to_model_next->push_back(next_joint_index);
        }
    }
}

/* See documentation in header before trying to decipher this. */
const SpanningForest::Mobod& SpanningForest::AddNewMobod(BodyIndex outboard_link_index,
    ...
    Mobod& new_mobod = data_.mobods.emplace_back(new_mobod_index, outboard_link_index, joint_index, inboard_level + 1,
                                                 is_reversed);
    ...
    mutable_graph().set_primary_mobod_for_link(outboard_link_index, new_mobod_index, joint_index);
    ...
    if (joint.traits_index() == LinkJointGraph::weld_joint_traits_index()) {
        if (!inboard_mobod.welded_mobods_index_.has_value()) {
            inboard_mobod.welded_mobods_index_ = WeldedMobodsIndex(ssize(welded_mobods()));
        ...
        data_.welded_mobods[*inboard_mobod.welded_mobods_index_].push_back(new_mobod_index);

        mutable_graph().AddToLinkComposite(inboard_mobod.link(), outboard_link_index);
    }

    return new_mobod;
}

void SpanningForest::ConnectLinksToWorld(const std::vector<BodyIndex>& links_to_connect, bool use_weld) {
    for (const auto& link_index : links_to_connect) {
        ...
        if (!found_joint_to_world) {
            const JointTraitsIndex joint_traits_index = use_weld ? LinkJointGraph::weld_joint_traits_index()
...
void SpanningForest::SetBaseBodyChoicePolicy() {
    ...
    data_.base_body_policy = [this](const BodyIndex& left, const BodyIndex& right) -> bool {
...
void SpanningForest::HandleLoopClosure(JointIndex loop_joint_index) {
    ...
    DRAKE_DEMAND(link_is_already_in_forest(joint.parent_link()) && link_is_already_in_forest(joint.child_link()));
    ...
    const bool parent_is_massless = graph().must_treat_as_massless(joint.parent_link());
    const bool child_is_massless = graph().must_treat_as_massless(joint.child_link());
    ...
    if (!(child_is_massless || parent_is_massless)) {
        const int child_level = mobods(graph().link_to_mobod(joint.child_link())).level();
        const int parent_level = mobods(graph().link_to_mobod(joint.parent_link())).level();
        if (parent_level > child_level) split_parent = true;
    } else if (child_is_massless) {
        split_parent = true;
    }

    AddShadowMobod(split_parent ? joint.parent_link() : joint.child_link(), joint.index());
}
...
const SpanningForest::Mobod& SpanningForest::AddShadowMobod(BodyIndex primary_link_index,
    ...
    const BodyIndex inboard_link_index = shadow_joint.other_link_index(primary_link_index);
    ...
    const BodyIndex shadow_link_index =
            mutable_graph().AddShadowLink(primary_link_index, shadow_joint_index, is_reversed);

    const LoopConstraintIndex loop_constraint_index =
            mutable_graph().AddLoopClosingWeldConstraint(primary_link_index, shadow_link_index);

    const MobodIndex inboard_mobod_index = links(inboard_link_index).mobod_index();
    const Mobod& shadow_mobod = AddNewMobod(shadow_link_index, shadow_joint_index, inboard_mobod_index, is_reversed);

    const MobodIndex primary_mobod_index = links(primary_link_index).mobod_index();
    data_.loop_constraints.emplace_back(loop_constraint_index, primary_mobod_index, shadow_mobod.index());
...
void SpanningForest::AddStubTreeAndLoopConstraint() {
    ...
    auto& mobod1 = data_.mobods.emplace_back(MobodIndex(1), BodyIndex(1), JointIndex(1), 1 /* level */,
                                             false /* is_reversed */);
    auto& mobod2 = data_.mobods.emplace_back(MobodIndex(2), BodyIndex(2), JointIndex(2), 2 /* level */,
                                             false /* is_reversed */);
    auto& mobod3 = data_.mobods.emplace_back(MobodIndex(3), BodyIndex(3), JointIndex(3), 1 /* level */,
                                             false /* is_reversed */);
    ...
    data_.loop_constraints.emplace_back(LoopConstraintIndex(0), MobodIndex(3), MobodIndex(2));
}
