...
MultibodyGraph::MultibodyGraph() {
    ...
    DRAKE_DEMAND(joint_type_name_to_index_[weld_type_name()] == JointTypeIndex(0));
}

BodyIndex MultibodyGraph::AddRigidBody(const std::string& body_name, ModelInstanceIndex model_instance) {
    ...
    if (HasBodyNamed(body_name, model_instance)) {
        throw std::runtime_error("AddRigidBody(): Duplicate body name '" + body_name + "'");
    }
    // next available
...
bool MultibodyGraph::HasBodyNamed(const std::string& name, ModelInstanceIndex model_instance) const {
    DRAKE_DEMAND(model_instance.is_valid());
...
bool MultibodyGraph::HasJointNamed(const std::string& name, ModelInstanceIndex model_instance) const {
    DRAKE_DEMAND(model_instance.is_valid());
...
JointIndex MultibodyGraph::AddJoint(const std::string& name,
    ...
    if (!type_index.is_valid()) {
        throw std::runtime_error("AddJoint(): Unrecognized type '" + type + "' for joint '" + name + "'.");
    }

    // Verify we are connecting bodies within the graph.
    if (!(parent_body_index.is_valid() && parent_body_index < num_bodies())) {
        throw std::runtime_error("AddJoint(): parent body index for joint '" + name + "' is invalid.");
    }
    if (!(child_body_index.is_valid() && child_body_index < num_bodies())) {
        throw std::runtime_error("AddJoint(): child body index for joint '" + name + "' is invalid.");
    }

    // next available index.
    const JointIndex joint_index(ssize(joints_));
    auto [map_iter, inserted] = bodies_to_joint_.insert({{parent_body_index, child_body_index}, joint_index});
    if (!inserted) {
        ...
        throw std::runtime_error("This MultibodyGraph already has a joint '" + existing_joint.name() +
                                 "' connecting '" + existing_parent.name() + "' to '" + existing_child.name() +
                                 "'. Therefore adding joint '" + name + "' connecting '" + new_parent.name() +
                                 "' to '" + new_child.name() + "' is not allowed.");
    }
    ...
    // Can't use emplace_back below because the constructor is private.
    joints_.push_back(Joint(name, model_instance, type_index, parent_body_index, child_body_index));
...
const MultibodyGraph::RigidBody& MultibodyGraph::get_body(BodyIndex index) const {
    DRAKE_THROW_UNLESS(index < num_bodies());
    return bodies_[index];
}

MultibodyGraph::RigidBody& MultibodyGraph::get_mutable_body(BodyIndex body_index) {
...
JointTypeIndex MultibodyGraph::RegisterJointType(const std::string& joint_type_name) {
    ...
    if (it != joint_type_name_to_index_.end())
        throw std::runtime_error(fmt::format("RegisterJointType(): Duplicate joint type: '{}'.", joint_type_name));
...
bool MultibodyGraph::IsJointTypeRegistered(const std::string& joint_type_name) const {
    const auto it = joint_type_name_to_index_.find(joint_type_name);
    return it != joint_type_name_to_index_.end();
}

JointTypeIndex MultibodyGraph::GetJointTypeIndex(const std::string& joint_type_name) const {
    const auto it = joint_type_name_to_index_.find(joint_type_name);
    return it == joint_type_name_to_index_.end() ? JointTypeIndex() : it->second;
}

std::vector<std::set<BodyIndex>> MultibodyGraph::FindSubgraphsOfWeldedBodies() const {
    ...
    for (const auto& body : bodies_) {
        if (!visited[body.index()]) {
            ...
            // connected to `body`.
            FindSubgraphsOfWeldedBodiesRecurse(body, &body_subgraph, &subgraphs, &visited);
        }
    }
    return subgraphs;
}

void MultibodyGraph::FindSubgraphsOfWeldedBodiesRecurse(const RigidBody& parent_body,
    ...
    for (JointIndex joint_index : parent_body.joints()) {
        const Joint& joint = get_joint(joint_index);
        const BodyIndex sibling_index =
                joint.parent_body() == parent_body.index() ? joint.child_body() : joint.parent_body();
        ...
        if (joint.type_index() == weld_type_index()) {
            ...
            FindSubgraphsOfWeldedBodiesRecurse(sibling, parent_subgraph, subgraphs, visited);
        } else {
            ...
            FindSubgraphsOfWeldedBodiesRecurse(sibling, &sibling_subgraph, subgraphs, visited);
        }
    }
}

std::set<BodyIndex> MultibodyGraph::FindBodiesWeldedTo(BodyIndex body_index) const {
    ...
    const std::vector<std::set<BodyIndex>> subgraphs = FindSubgraphsOfWeldedBodies();
    ...
    auto subgraph_iter = std::find_if(subgraphs.begin(), subgraphs.end(), predicate);
