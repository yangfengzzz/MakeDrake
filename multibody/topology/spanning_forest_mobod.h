...
public:
    ...
    Mobod(MobodIndex mobod_index, BodyIndex link_index, JointIndex joint_index, int level, bool is_reversed);
    ...
    bool is_anchored() const { return welded_mobods_index_ == WeldedMobodsIndex(0); }
    ...
    std::optional<WeldedMobodsIndex> welded_mobods_group() const { return welded_mobods_index_; }
    ...
    std::pair<int, int> subtree_velocities() const { return std::make_pair(v_start(), nv() + nv_outboard()); }
    ...
    std::pair<int, int> outboard_velocities() const { return std::make_pair(v_start() + nv(), nv_outboard()); }
...
private:
    ...
    static void RenumberMobodIndexVector(const std::vector<MobodIndex>& old_to_new,
