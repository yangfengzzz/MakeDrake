...
#include "multibody/topology/link_joint_graph_defs.h"
...
public:
    ...
    ForestBuildingOptions get_global_forest_building_options() const { return data_.global_forest_building_options; }
    ...
    void SetForestBuildingOptions(ModelInstanceIndex model_instance, ForestBuildingOptions options);
    ...
    ForestBuildingOptions get_forest_building_options_in_use(ModelInstanceIndex instance_index) const {
        return get_model_instance_options(instance_index).value_or(get_global_forest_building_options());
    }
    ...
    BodyIndex AddLink(const std::string& name,
    ...
    "rpy_floating". */
    JointTraitsIndex RegisterJointType(const std::string& joint_type_name, int nq, int nv, bool has_quaternion = false);
    ...
    Joint types. */
    const std::vector<JointTraits>& joint_traits() const { return data_.joint_traits; }
    ...
    const JointTraits& joint_traits(JointTraitsIndex index) const { return joint_traits().at(index); }
    ...
    inline const LoopConstraint& loop_constraints(LoopConstraintIndex constraint_index) const;
    ...
    const std::vector<std::vector<BodyIndex>>& link_composites() const { return data_.link_composites; }

    /** Returns a reference to a particular LinkComposite. Requires a
    LinkCompositeIndex, not a plain integer.*/
    const std::vector<BodyIndex>& link_composites(LinkCompositeIndex composite_link_index) const {
    ...
    bool HasLinkNamed(std::string_view name, ModelInstanceIndex model_instance_index) const;

    /** @returns `true` if a Joint named `name` was added to `model_instance`.
    @see AddJoint().
    @throws std::exception if `model_instance_index` is not a valid index. */
    bool HasJointNamed(std::string_view name, ModelInstanceIndex model_instance_index) const;
    ...
    std::optional<JointIndex> MaybeGetJointBetween(BodyIndex link1_index, BodyIndex link2_index) const;
    ...
    void ChangeJointType(JointIndex existing_joint_index, const std::string& name_of_new_type);

    // Forest building requires these joint types so they are predefined.

    /** The predefined index for the "weld" joint type's traits. */
    static JointTraitsIndex weld_joint_traits_index() { return JointTraitsIndex(0); }

    /** The predefined index for the "quaternion_floating" joint type's traits. */
    static JointTraitsIndex quaternion_floating_joint_traits_index() { return JointTraitsIndex(1); }

    /** The predefined index for the "rpy_floating" joint type's traits. */
    static JointTraitsIndex rpy_floating_joint_traits_index() { return JointTraitsIndex(2); }
...
private:
    ...
    LoopConstraintIndex AddLoopClosingWeldConstraint(BodyIndex primary_link_index, BodyIndex shadow_link_index);
    ...
    std::tuple<BodyIndex, BodyIndex, bool> FindInboardOutboardLinks(MobodIndex inboard_mobod_index,
    ...
    JointIndex AddEphemeralJointToWorld(JointTraitsIndex type_index, BodyIndex child_link_index);
    ...
    LinkCompositeIndex AddToLinkComposite(BodyIndex maybe_composite_link_index, BodyIndex new_link_index);
    ...
    BodyIndex AddShadowLink(BodyIndex primary_link_index, JointIndex shadow_joint_index, bool shadow_is_parent);
    ...
    std::optional<JointTraitsIndex> GetJointTraitsIndex(const std::string& joint_type_name) const;

    // Links that were explicitly flagged LinkFlags::kStatic in AddLink().
    const std::vector<BodyIndex>& static_links() const { return data_.static_links; }
    ...
    std::optional<ForestBuildingOptions> get_model_instance_options(ModelInstanceIndex instance_index) const {
    ...
    struct Data {
        ...
        std::map<ModelInstanceIndex, std::vector<BodyIndex>> model_instance_to_links;
        ...
        ForestBuildingOptions global_forest_building_options{ForestBuildingOptions::kDefault};

        // Indexed by ModelInstanceIndex.
        std::vector<std::optional<ForestBuildingOptions>> model_instance_forest_building_options;
