...
public:
    ...
    const RevoluteMobilizer<T>& SetAngle(systems::Context<T>* context, const T& angle) const;
    ...
    // @returns a constant reference to `this` mobilizer.
    const RevoluteMobilizer<T>& SetAngularRate(systems::Context<T>* context, const T& theta_dot) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::RevoluteMobilizer);
