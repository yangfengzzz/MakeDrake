#include "multibody/tree/multibody_tree.h"
...
#include "multibody/tree/body_node_world.h"
#include "multibody/tree/multibody_tree-inl.h"
#include "multibody/tree/quaternion_floating_joint.h"
#include "multibody/tree/quaternion_floating_mobilizer.h"
#include "multibody/tree/rigid_body.h"
#include "multibody/tree/spatial_inertia.h"
...
public:
    ...
    static Mobilizer<T>* Build(Joint<T>* joint, MultibodyTree<T>* tree) {
        ...
        std::unique_ptr<JointBluePrint> blue_print = joint->MakeImplementationBlueprint();
...
MultibodyTree<T>::MultibodyTree() {
    ...
    world_rigid_body_ = &AddRigidBody("world", world_model_instance(), SpatialInertia<double>::NaN());

    // `default_model_instance()` hardcodes the returned index.  Make sure it's
    // correct.
    ModelInstanceIndex default_instance = AddModelInstance("DefaultModelInstance");
    DRAKE_DEMAND(default_instance == default_model_instance());

    const ForceElement<T>& new_field = AddForceElement<UniformGravityFieldElement>();
...
template <typename T>
const RigidBody<T>& MultibodyTree<T>::AddRigidBody(const std::string& name,
    ...
    if (HasBodyNamed(name, model_instance)) {
        throw std::logic_error(
                fmt::format("Model instance '{}' already contains a body named '{}'. Body names "
...
template <typename T>
const RigidBody<T>& MultibodyTree<T>::AddRigidBody(const std::string& name, const SpatialInertia<double>& M_BBo_B) {
...
template <typename T>
const std::string& MultibodyTree<T>::GetModelInstanceName(ModelInstanceIndex model_instance) const {
    if (!model_instances_.has_element(model_instance)) {
        throw std::logic_error(
                fmt::format("There is no model instance id {} in the model.", std::to_string(model_instance)));
    }
    return model_instances_.get_element_unchecked(model_instance).name();
}

template <typename T>
bool MultibodyTree<T>::HasUniqueFreeBaseBodyImpl(ModelInstanceIndex model_instance) const {
    std::optional<BodyIndex> base_body_index = MaybeGetUniqueBaseBodyIndex(model_instance);
    return base_body_index.has_value() && rigid_bodies_.get_element(base_body_index.value()).is_floating();
}

template <typename T>
const RigidBody<T>& MultibodyTree<T>::GetUniqueFreeBaseBodyOrThrowImpl(ModelInstanceIndex model_instance) const {
    std::optional<BodyIndex> base_body_index = MaybeGetUniqueBaseBodyIndex(model_instance);
    if (!base_body_index.has_value()) {
        throw std::logic_error(fmt::format("Model {} does not have a unique base body.",
                                           model_instances_.get_element(model_instance).name()));
    }
    const RigidBody<T>& result = rigid_bodies_.get_element(base_body_index.value());
    if (!result.is_floating()) {
        throw std::logic_error(fmt::format("Model {} has a unique base body, but it is not free.",
                                           model_instances_.get_element(model_instance).name()));
    }
    return result;
}
...
const auto& GetElementByIndex(const MultibodyTree<T>& tree, const ElementIndex index) {
    if constexpr (std::is_same_v<ElementIndex, BodyIndex>) {
...
template <typename T, typename ElementIndex>
std::string GetElementModelInstancesByName(const MultibodyTree<T>& tree,
...
template <typename T, typename ElementIndex>
bool HasElementNamed(const MultibodyTree<T>& tree,
                     std::string_view name,
    ...
    if (std::next(lower) != upper) {
        const std::string_view element_classname = GetElementClassname<ElementIndex>();
        const std::string known_instances = GetElementModelInstancesByName(tree, name, name_to_index);
        throw std::logic_error(
                fmt::format("Has{}Named(): A {} named '{}' appears in multiple model instances"
...
const auto& GetElementByName(const MultibodyTree<T>& tree,
    ...
    const std::string& model_instance_name = model_instance ? tree.GetModelInstanceName(*model_instance) : empty_name;
    const std::string_view element_classname = GetElementClassname<ElementIndex>();
    ...
    if (lower == upper) {
        std::string message = fmt::format("Get{}ByName(): There is no {} named '{}' anywhere in the model ",
        ...
        // the map.
        std::map<ModelInstanceIndex, std::vector<std::string_view>> names_by_model_instance;
        ...
        if (names_by_model_instance.empty()) {
            message = fmt::format("Get{}ByName(): There are no {}s defined in the model", element_classname,
                                  element_classname);
        } else {
            std::vector<std::string> instance_messages;
            instance_messages.reserve(names_by_model_instance.size());
            for (auto& [model_instance_index, element_names] : names_by_model_instance) {
                std::sort(element_names.begin(), element_names.end());
                instance_messages.push_back(fmt::format("valid names in model instance '{}' are: {}",
    ...
    if (model_instance) {
        ...
        const std::string known_instances = GetElementModelInstancesByName(tree, name, name_to_index);
        throw std::logic_error(
                fmt::format("Get{}ByName(): There is no {} named '{}' in the model instance named"
                            " '{}', but one does exist in other model instances ({}).",
                            element_classname, element_classname, name, model_instance_name, known_instances));
    }
    ...
    if (std::next(lower) != upper) {
        const std::string known_instances = GetElementModelInstancesByName(tree, name, name_to_index);
        throw std::logic_error(
                fmt::format("Get{}ByName(): A {} named '{}' appears in multiple model instances"
                            " ({}); you must provide a model_instance argument to disambiguate.",
                            element_classname, element_classname, name, known_instances));
    }
...
template <typename T>
bool MultibodyTree<T>::HasBodyNamed(std::string_view name, ModelInstanceIndex model_instance) const {
    return HasElementNamed(*this, name, model_instance, rigid_bodies_.names_map());
}
...
template <typename T>
bool MultibodyTree<T>::HasFrameNamed(std::string_view name, ModelInstanceIndex model_instance) const {
...
template <typename T>
bool MultibodyTree<T>::HasJointNamed(std::string_view name, ModelInstanceIndex model_instance) const {
...
template <typename T>
bool MultibodyTree<T>::HasJointActuatorNamed(std::string_view name, ModelInstanceIndex model_instance) const {
...
template <typename T>
std::vector<BodyIndex> MultibodyTree<T>::GetBodyIndices(ModelInstanceIndex model_instance) const {
...
template <typename T>
std::vector<JointIndex> MultibodyTree<T>::GetJointIndices(ModelInstanceIndex model_instance) const {
    DRAKE_THROW_UNLESS(model_instances_.has_element(model_instance));
...
template <typename T>
std::vector<JointActuatorIndex> MultibodyTree<T>::GetJointActuatorIndices(ModelInstanceIndex model_instance) const {
    DRAKE_THROW_UNLESS(model_instances_.has_element(model_instance));
    return model_instances_.get_element(model_instance).GetJointActuatorIndices();
}

template <typename T>
std::vector<JointIndex> MultibodyTree<T>::GetActuatedJointIndices(ModelInstanceIndex model_instance) const {
    DRAKE_THROW_UNLESS(model_instances_.has_element(model_instance));
    return model_instances_.get_element(model_instance).GetActuatedJointIndices();
}

template <typename T>
std::vector<FrameIndex> MultibodyTree<T>::GetFrameIndices(ModelInstanceIndex model_instance) const {
    DRAKE_THROW_UNLESS(model_instances_.has_element(model_instance));
...
template <typename T>
const Frame<T>& MultibodyTree<T>::GetFrameByName(std::string_view name, ModelInstanceIndex model_instance) const {
    return GetElementByName(*this, name, model_instance, frames_.names_map());
}

template <typename T>
const RigidBody<T>& MultibodyTree<T>::GetRigidBodyByName(std::string_view name) const {
    return GetElementByName(*this, name, std::nullopt, rigid_bodies_.names_map());
}

template <typename T>
const RigidBody<T>& MultibodyTree<T>::GetRigidBodyByName(std::string_view name,
                                                         ModelInstanceIndex model_instance) const {
    return GetElementByName(*this, name, model_instance, rigid_bodies_.names_map());
}

template <typename T>
const RigidBody<T>& MultibodyTree<T>::AddRigidBodyImpl(std::unique_ptr<RigidBody<T>> body) {
    ...
    // RigidBodyAttorney.
    // - Register body frame.
    Frame<T>* body_frame = &internal::RigidBodyAttorney<T>::get_mutable_body_frame(body.get());
...
template <typename T>
const Joint<T>& MultibodyTree<T>::GetJointByNameImpl(std::string_view name,
                                                     std::optional<ModelInstanceIndex> model_instance) const {
    return GetElementByName(*this, name, model_instance, joints_.names_map());
}

template <typename T>
void MultibodyTree<T>::ThrowJointSubtypeMismatch(const Joint<T>& joint, std::string_view desired_type) const {
    throw std::logic_error(
            fmt::format("GetJointByName(): Joint '{}' in model instance '{}' is not of type {} "
                        "but of type {}.",
                        joint.name(), model_instances_.get_element(joint.model_instance()).name(), desired_type,
                        NiceTypeName::Get(joint)));
}

template <typename T>
const JointActuator<T>& MultibodyTree<T>::GetJointActuatorByName(std::string_view name) const {
    return GetElementByName(*this, name, std::nullopt, actuators_.names_map());
}

template <typename T>
const JointActuator<T>& MultibodyTree<T>::GetJointActuatorByName(std::string_view name,
                                                                 ModelInstanceIndex model_instance) const {
    return GetElementByName(*this, name, model_instance, actuators_.names_map());
}

template <typename T>
ModelInstanceIndex MultibodyTree<T>::GetModelInstanceByName(std::string_view name) const {
    ...
    if (it == names.end()) {
        ...
        throw std::logic_error(
                fmt::format("GetModelInstanceByName(): There is no model instance named '{}'. The "
...
template <typename T>
VectorX<T> MultibodyTree<T>::GetActuationFromArray(ModelInstanceIndex model_instance,
                                                   const Eigen::Ref<const VectorX<T>>& u) const {
    return model_instances_.get_element(model_instance).GetActuationFromArray(u);
}

template <typename T>
void MultibodyTree<T>::SetActuationInArray(ModelInstanceIndex model_instance,
                                           const Eigen::Ref<const VectorX<T>>& u_instance,
                                           EigenPtr<VectorX<T>> u) const {
    model_instances_.get_element(model_instance).SetActuationInArray(u_instance, u);
}

template <typename T>
VectorX<T> MultibodyTree<T>::GetPositionsFromArray(ModelInstanceIndex model_instance,
                                                   const Eigen::Ref<const VectorX<T>>& q) const {
    return model_instances_.get_element(model_instance).GetPositionsFromArray(q);
}

template <typename T>
void MultibodyTree<T>::GetPositionsFromArray(ModelInstanceIndex model_instance,
...
template <class T>
void MultibodyTree<T>::SetPositionsInArray(ModelInstanceIndex model_instance,
                                           const Eigen::Ref<const VectorX<T>>& q_instance,
                                           EigenPtr<VectorX<T>> q) const {
    model_instances_.get_element(model_instance).SetPositionsInArray(q_instance, q);
}

template <typename T>
VectorX<T> MultibodyTree<T>::GetVelocitiesFromArray(ModelInstanceIndex model_instance,
                                                    const Eigen::Ref<const VectorX<T>>& v) const {
    return model_instances_.get_element(model_instance).GetVelocitiesFromArray(v);
}

template <typename T>
void MultibodyTree<T>::GetVelocitiesFromArray(ModelInstanceIndex model_instance,
...
template <class T>
void MultibodyTree<T>::SetVelocitiesInArray(ModelInstanceIndex model_instance,
                                            const Eigen::Ref<const VectorX<T>>& v_instance,
                                            EigenPtr<VectorX<T>> v) const {
    model_instances_.get_element(model_instance).SetVelocitiesInArray(v_instance, v);
}
...
void MultibodyTree<T>::CreateJointImplementations() {
    ...
    for (JointIndex i : GetJointIndices()) {
        ...
        Mobilizer<T>* mobilizer = internal::JointImplementationBuilder<T>::Build(&joint, this);
    ...
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        ...
        const RigidBodyTopology& body_topology = get_topology().get_rigid_body(body.index());
        ...
        const Joint<T>& joint =
                this->AddJoint<QuaternionFloatingJoint>(floating_joint_name, world_body(), {}, body, {});
        Joint<T>& mutable_joint = joints_.get_mutable_element(joint.index());
        Mobilizer<T>* mobilizer = internal::JointImplementationBuilder<T>::Build(&mutable_joint, this);
        mobilizer->set_model_instance(joint.model_instance());
        // Record the joint to mobilizer map.
        joint_to_mobilizer_[joint.index()] = mobilizer->index();
    }
...
template <typename T>
const QuaternionFloatingMobilizer<T>& MultibodyTree<T>::GetFreeBodyMobilizerOrThrow(const RigidBody<T>& body) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    DRAKE_DEMAND(body.index() != world_index());
    const RigidBodyTopology& body_topology = get_topology().get_rigid_body(body.index());
    const QuaternionFloatingMobilizer<T>* mobilizer =
            dynamic_cast<const QuaternionFloatingMobilizer<T>*>(&get_mobilizer(body_topology.inboard_mobilizer));
    if (mobilizer == nullptr) {
        throw std::logic_error("Body '" + body.name() + "' is not a free floating body.");
    }
    return *mobilizer;
}

template <typename T>
const Frame<T>& MultibodyTree<T>::AddOrGetJointFrame(const RigidBody<T>& body,
                                                     const std::optional<math::RigidTransform<double>>& X_BF,
                                                     ModelInstanceIndex joint_instance,
                                                     std::string_view joint_name,
                                                     std::string_view frame_suffix) {
    if (X_BF.has_value()) {
        return this->AddFrame<FixedOffsetFrame>(fmt::format("{}_{}", joint_name, frame_suffix), body.body_frame(),
                                                *X_BF, joint_instance);
    } else {
...
void MultibodyTree<T>::FinalizeInternals() {
    ...
    for (MobodIndex mobod_index(1); mobod_index < topology_.num_mobods(); ++mobod_index) {
    ...
    for (MobodIndex mobod_index(0); mobod_index < topology_.num_mobods(); ++mobod_index) {
    ...
    for (JointIndex i : GetJointIndices()) {
        ...
        if (body.is_floating()) {
            const auto [quaternion, translation] = GetDefaultFreeBodyPoseAsQuaternionVec3Pair(body);
    ...
    for (JointIndex i : GetJointIndices()) {
        if (!multibody_graph_.has_joint(i)) {
            ...
            DRAKE_DEMAND(get_joint(i).parent_body().index().is_valid() &&
                         multibody_graph_.get_joint(i).parent_body() == get_joint(i).parent_body().index());
            DRAKE_DEMAND(get_joint(i).child_body().index().is_valid() &&
                         multibody_graph_.get_joint(i).child_body() == get_joint(i).child_body().index());
        }
    }
}
...
void MultibodyTree<T>::CreateBodyNode(MobodIndex mobod_index) {
    ...
    const RigidBody<T>& body = rigid_bodies_.get_element(node_topology.rigid_body);
    ...
    if (body_index == world_index()) {
        ...
        const Mobilizer<T>* mobilizer = owned_mobilizers_[node_topology.mobilizer].get();

        BodyNode<T>* parent_node = body_nodes_[node_topology.parent_body_node].get();
...
void MultibodyTree<T>::FinalizeModelInstances() {
    ...
    for (const auto& body_node : body_nodes_) {
        if (body_node->get_num_mobilizer_positions() > 0 || body_node->get_num_mobilizer_velocities() > 0) {
    ...
    for (const JointActuator<T>* actuator : actuators_.elements()) {
        model_instances_.get_mutable_element(actuator->model_instance()).add_joint_actuator(actuator);
    }
}

template <typename T>
std::unique_ptr<systems::LeafContext<T>> MultibodyTree<T>::CreateDefaultContext() const {
    ...
    return dynamic_pointer_cast<systems::LeafContext<T>>(tree_system_->CreateDefaultContext());
}

template <typename T>
void MultibodyTree<T>::SetDefaultState(const systems::Context<T>& context, systems::State<T>* state) const {
    for (const auto& mobilizer : owned_mobilizers_) {
...
template <typename T>
VectorX<T> MultibodyTree<T>::GetPositionsAndVelocities(const systems::Context<T>& context,
                                                       ModelInstanceIndex model_instance) const {
...
template <typename T>
void MultibodyTree<T>::GetPositionsAndVelocities(const systems::Context<T>& context,
                                                 ModelInstanceIndex model_instance,
                                                 EigenPtr<VectorX<T>> qv_out) const {
    DRAKE_DEMAND(qv_out != nullptr);

    Eigen::VectorBlock<const VectorX<T>> state_vector = get_positions_and_velocities(context);

    if (qv_out->size() != num_positions(model_instance) + num_velocities(model_instance))
    ...
    GetPositionsFromArray(model_instance, state_vector.head(num_positions()), &qv_out_head);
    GetVelocitiesFromArray(model_instance, state_vector.tail(num_velocities()), &qv_out_tail);
}

template <typename T>
void MultibodyTree<T>::SetPositionsAndVelocities(ModelInstanceIndex model_instance,
                                                 const Eigen::Ref<const VectorX<T>>& instance_state,
                                                 systems::Context<T>* context) const {
    Eigen::VectorBlock<VectorX<T>> state_vector = GetMutablePositionsAndVelocities(context);
    Eigen::VectorBlock<VectorX<T>> q = make_mutable_block_segment(&state_vector, 0, num_positions());
    Eigen::VectorBlock<VectorX<T>> v = make_mutable_block_segment(&state_vector, num_positions(), num_velocities());
    SetPositionsInArray(model_instance, instance_state.head(num_positions(model_instance)), &q);
    SetVelocitiesInArray(model_instance, instance_state.tail(num_velocities(model_instance)), &v);
}

template <typename T>
RigidTransform<T> MultibodyTree<T>::GetFreeBodyPoseOrThrow(const systems::Context<T>& context,
                                                           const RigidBody<T>& body) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    const QuaternionFloatingMobilizer<T>& mobilizer = GetFreeBodyMobilizerOrThrow(body);
    return RigidTransform<T>(mobilizer.get_quaternion(context), mobilizer.get_translation(context));
}

template <typename T>
void MultibodyTree<T>::SetDefaultFreeBodyPose(const RigidBody<T>& body, const RigidTransform<double>& X_WB) {
    if (!default_body_poses_.contains(body.index()) ||
        std::holds_alternative<std::pair<Eigen::Quaternion<double>, Vector3<double>>>(
                default_body_poses_.at(body.index()))) {
        default_body_poses_[body.index()] = std::make_pair(X_WB.rotation().ToQuaternion(), X_WB.translation());
        return;
    }
    auto& joint = joints_.get_mutable_element(std::get<JointIndex>(default_body_poses_.at(body.index())));
    auto* quaternion_floating_joint = dynamic_cast<QuaternionFloatingJoint<T>*>(&joint);
    DRAKE_DEMAND(quaternion_floating_joint != nullptr);
    quaternion_floating_joint->set_default_quaternion(X_WB.rotation().ToQuaternion());
    quaternion_floating_joint->set_default_translation(X_WB.translation());
}

template <typename T>
RigidTransform<double> MultibodyTree<T>::GetDefaultFreeBodyPose(const RigidBody<T>& body) const {
    const std::pair<Eigen::Quaternion<double>, Vector3<double>> pose = GetDefaultFreeBodyPoseAsQuaternionVec3Pair(body);
    return RigidTransform<double>(pose.first, pose.second);
}

template <typename T>
std::pair<Eigen::Quaternion<double>, Vector3<double>> MultibodyTree<T>::GetDefaultFreeBodyPoseAsQuaternionVec3Pair(
        const RigidBody<T>& body) const {
    if (!default_body_poses_.contains(body.index())) {
        return std::make_pair(Eigen::Quaternion<double>::Identity(), Vector3<double>::Zero());
    }
    const auto& default_body_pose = default_body_poses_.at(body.index());
    if (std::holds_alternative<JointIndex>(default_body_pose)) {
        const auto& joint = joints_.get_element(std::get<JointIndex>(default_body_pose));
        return joint.GetDefaultPosePair();
    }
    return std::get<std::pair<Eigen::Quaternion<double>, Vector3<double>>>(default_body_pose);
}

template <typename T>
void MultibodyTree<T>::SetFreeBodyPoseOrThrow(const RigidBody<T>& body,
...
template <typename T>
void MultibodyTree<T>::SetFreeBodySpatialVelocityOrThrow(const RigidBody<T>& body,
                                                         const SpatialVelocity<T>& V_WB,
                                                         systems::Context<T>* context) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    SetFreeBodySpatialVelocityOrThrow(body, V_WB, *context, &context->get_mutable_state());
}
...
void MultibodyTree<T>::SetFreeBodyRandomTranslationDistributionOrThrow(
        const RigidBody<T>& body, const Vector3<symbolic::Expression>& translation) {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    QuaternionFloatingMobilizer<T>& mobilizer = get_mutable_variant(GetFreeBodyMobilizerOrThrow(body));
...
void MultibodyTree<T>::SetFreeBodyRandomRotationDistributionOrThrow(
        const RigidBody<T>& body, const Eigen::Quaternion<symbolic::Expression>& rotation) {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    QuaternionFloatingMobilizer<T>& mobilizer = get_mutable_variant(GetFreeBodyMobilizerOrThrow(body));
...
template <typename T>
void MultibodyTree<T>::CalcAllBodyPosesInWorld(const systems::Context<T>& context,
...
// Note that the result is indexed by BodyIndex, not MobodIndex.
template <typename T>
void MultibodyTree<T>::CalcAllBodySpatialVelocitiesInWorld(const systems::Context<T>& context,
...
template <typename T>
void MultibodyTree<T>::CalcPositionKinematicsCache(const systems::Context<T>& context,
...
template <typename T>
void MultibodyTree<T>::CalcVelocityKinematicsCache(const systems::Context<T>& context,
    ...
    const std::vector<Vector6<T>>& H_PB_W_cache = EvalAcrossNodeJacobianWrtVExpressedInWorld(context);
    ...
    for (int level = 1; level < tree_height(); ++level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            Eigen::Map<const MatrixUpTo6<T>> H_PB_W = node.GetJacobianFromArray(H_PB_W_cache);
...
template <typename T>
void MultibodyTree<T>::CalcSpatialInertiasInWorld(const systems::Context<T>& context,
                                                  std::vector<SpatialInertia<T>>* M_B_W_all) const {
    DRAKE_THROW_UNLESS(M_B_W_all != nullptr);
    DRAKE_THROW_UNLESS(static_cast<int>(M_B_W_all->size()) == topology_.num_mobods());
...
template <typename T>
void MultibodyTree<T>::CalcReflectedInertia(const systems::Context<T>& context, VectorX<T>* reflected_inertia) const {
    DRAKE_THROW_UNLESS(reflected_inertia != nullptr);
    DRAKE_THROW_UNLESS(static_cast<int>(reflected_inertia->size()) == num_velocities());

    // See JointActuator::reflected_inertia().
    reflected_inertia->setZero();

    for (const JointActuator<T>* actuator : actuators_.elements()) {
        const int joint_velocity_index = actuator->joint().velocity_start();  // within v
        (*reflected_inertia)(joint_velocity_index) = actuator->calc_reflected_inertia(context);
    }
}

template <typename T>
void MultibodyTree<T>::CalcJointDamping(const systems::Context<T>& context, VectorX<T>* joint_damping) const {
    DRAKE_THROW_UNLESS(joint_damping != nullptr);
    DRAKE_THROW_UNLESS(ssize(*joint_damping) == num_velocities());

    for (const Joint<T>* joint : joints_.elements()) {
        joint_damping->segment(joint->velocity_start(), joint->num_velocities()) = joint->GetDampingVector(context);
    }
}

template <typename T>
void MultibodyTree<T>::CalcCompositeBodyInertiasInWorld(const systems::Context<T>& context,
                                                        std::vector<SpatialInertia<T>>* Mc_B_W_all) const {
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const std::vector<SpatialInertia<T>>& M_B_W_all = EvalSpatialInertiaInWorldCache(context);
    ...
    for (int level = tree_height() - 1; level > 0; --level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            composite_node.CalcCompositeBodyInertia_TipToBase(M_C_W, pc, *Mc_B_W_all, &Mc_C_W);
        }
    }
}

template <typename T>
void MultibodyTree<T>::CalcSpatialAccelerationBias(const systems::Context<T>& context,
...
template <typename T>
void MultibodyTree<T>::CalcArticulatedBodyForceBias(const systems::Context<T>& context,
    ...
    DRAKE_THROW_UNLESS(static_cast<int>(Zb_Bo_W_all->size()) == topology_.num_mobods());
    const std::vector<SpatialAcceleration<T>>& Ab_WB_cache = EvalSpatialAccelerationBiasCache(context);
    ...
    for (MobodIndex mobod_index(1); mobod_index < topology_.num_mobods(); ++mobod_index) {
        const ArticulatedBodyInertia<T>& Pplus_PB_W = abic.get_Pplus_PB_W(mobod_index);
...
void MultibodyTree<T>::CalcArticulatedBodyForceBias(const systems::Context<T>& context,
    ...
    DRAKE_THROW_UNLESS(Zb_Bo_W_all != nullptr);
    DRAKE_THROW_UNLESS(static_cast<int>(Zb_Bo_W_all->size()) == topology_.num_mobods());
    const ArticulatedBodyInertiaCache<T>& abic = EvalArticulatedBodyInertiaCache(context);
    CalcArticulatedBodyForceBias(context, abic, Zb_Bo_W_all);
}

// Result is indexed by MobodIndex.
template <typename T>
void MultibodyTree<T>::CalcDynamicBiasForces(const systems::Context<T>& context,
                                             std::vector<SpatialForce<T>>* Fb_Bo_W_all) const {
    DRAKE_THROW_UNLESS(Fb_Bo_W_all != nullptr);
    DRAKE_THROW_UNLESS(static_cast<int>(Fb_Bo_W_all->size()) == topology_.num_mobods());

    const std::vector<SpatialInertia<T>>& spatial_inertia_in_world_cache = EvalSpatialInertiaInWorldCache(context);
    ...
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        ...
        const SpatialInertia<T>& M_B_W = spatial_inertia_in_world_cache[body.mobod_index()];
        ...
        Fb_Bo_W = mass * SpatialForce<T>(w_WB.cross(G_B_W * w_WB), /* rotational */
                                         w_WB.cross(w_WB.cross(p_BoBcm_W)) /* translational */);
    }
}

template <typename T>
void MultibodyTree<T>::CalcSpatialAccelerationsFromVdot(const systems::Context<T>& context,
                                                        const PositionKinematicsCache<T>&,
                                                        const VelocityKinematicsCache<T>&,
                                                        const VectorX<T>& known_vdot,
                                                        std::vector<SpatialAcceleration<T>>* A_WB_array) const {
    const bool ignore_velocities = false;
    CalcSpatialAccelerationsFromVdot(context, known_vdot, ignore_velocities, A_WB_array);
}
...
void MultibodyTree<T>::CalcSpatialAccelerationsFromVdot(const systems::Context<T>& context,
    ...
    const VelocityKinematicsCache<T>* vc = ignore_velocities ? nullptr : &EvalVelocityKinematics(context);
    ...
    for (int level = 1; level < tree_height(); ++level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            // Update per-node kinematics.
            node.CalcSpatialAcceleration_BaseToTip(context, pc, vc, known_vdot, A_WB_array);
        }
    }
}

template <typename T>
void MultibodyTree<T>::CalcAccelerationKinematicsCache(const systems::Context<T>& context,
                                                       const PositionKinematicsCache<T>& pc,
...
template <typename T>
VectorX<T> MultibodyTree<T>::CalcInverseDynamics(const systems::Context<T>& context,
                                                 const VectorX<T>& known_vdot,
    ...
    CalcInverseDynamics(context, known_vdot, external_forces.body_forces(), external_forces.generalized_forces(), &A_WB,
                        &F_BMo_W, &tau);
    return tau;
}
...
template <typename T>
void MultibodyTree<T>::CalcInverseDynamics(const systems::Context<T>& context,
                                           const VectorX<T>& known_vdot,
    ...
    const bool ignore_velocities = false;
    CalcInverseDynamics(context, known_vdot, Fapplied_Bo_W_array, tau_applied_array, ignore_velocities, A_WB_array,
...
void MultibodyTree<T>::CalcInverseDynamics(const systems::Context<T>& context,
    ...
    DRAKE_DEMAND(static_cast<int>(F_BMo_W_array->size()) == topology_.num_mobods());
    ...
    for (int level = tree_height() - 1; level >= 0; --level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            if (tau_applied_size != 0) {
                tau_applied_mobilizer = node.get_mobilizer().get_generalized_forces_from_array(tau_applied_array);
            }
            if (Fapplied_size != 0) {
            ...
            node.CalcInverseDynamics_TipToBase(context, pc, spatial_inertia_in_world_cache, dynamic_bias_cache,
...
template <typename T>
void MultibodyTree<T>::CalcForceElementsContribution(const systems::Context<T>& context,
                                                     const PositionKinematicsCache<T>& pc,
                                                     const VelocityKinematicsCache<T>& vc,
...
template <typename T>
void MultibodyTree<T>::AddJointDampingForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const {
    DRAKE_DEMAND(forces != nullptr);
    for (const Joint<T>* joint : joints_.elements()) {
...
template <typename T>
void MultibodyTree<T>::MapQDotToVelocity(const systems::Context<T>& context,
...
template <typename T>
Eigen::SparseMatrix<T> MultibodyTree<T>::MakeVelocityToQDotMap(const systems::Context<T>& context) const {
        ...
        for (int i = 0; i < mobilizer->num_positions(); ++i) {
            for (int j = 0; j < mobilizer->num_velocities(); ++j) {
                if (N_mobilizer(i, j) != 0) {
                    triplet_list.push_back(Eigen::Triplet<T>(mobilizer->position_start_in_q() + i,
...
template <typename T>
Eigen::SparseMatrix<T> MultibodyTree<T>::MakeQDotToVelocityMap(const systems::Context<T>& context) const {
    ...
    for (const auto& mobilizer : owned_mobilizers_) {
        Nplus_mobilizer.resize(mobilizer->num_velocities(), mobilizer->num_positions());
        ...
        for (int i = 0; i < mobilizer->num_velocities(); ++i) {
            for (int j = 0; j < mobilizer->num_positions(); ++j) {
                if (Nplus_mobilizer(i, j) != 0) {
                    triplet_list.push_back(Eigen::Triplet<T>(mobilizer->velocity_start_in_v() + i,
                                                             mobilizer->position_start_in_q() + j,
                                                             Nplus_mobilizer(i, j)));
                }
            }
        }
...
template <typename T>
void MultibodyTree<T>::CalcMassMatrixViaInverseDynamics(const systems::Context<T>& context,
    ...
    for (int j = 0; j < nv; ++j) {
        ...
        tau.setZero();
        CalcInverseDynamics(context, vdot, {}, VectorX<T>(), ignore_velocities, &A_WB_array, &F_BMo_W_array, &tau);
        M->col(j) = tau;
    }
}

template <typename T>
void MultibodyTree<T>::CalcMassMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> M) const {
    DRAKE_DEMAND(M != nullptr);
    DRAKE_DEMAND(M->rows() == num_velocities());
    DRAKE_DEMAND(M->cols() == num_velocities());
    ...
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const std::vector<SpatialInertia<T>>& Mc_B_W_cache = EvalCompositeBodyInertiaInWorldCache(context);
    const std::vector<Vector6<T>>& H_PB_W_cache = EvalAcrossNodeJacobianWrtVExpressedInWorld(context);
    const VectorX<T>& reflected_inertia = EvalReflectedInertiaCache(context);
    ...
    for (int level = tree_height() - 1; level > 0; --level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            Eigen::Map<const MatrixUpTo6<T>> H_CpC_W = composite_node.GetJacobianFromArray(H_PB_W_cache);
            ...
            M->block(composite_start_in_v, composite_start_in_v, cnv, cnv) += H_CpC_W.transpose() * Fm_CCo_W;
            ...
            const BodyNode<T>* child_node = &composite_node;  // Child starts at frame C.
            ...
            while (body_node) {
                ...
                if (bnv > 0) {
                    ...
                    // its parent P.
                    const Eigen::Map<const MatrixUpTo6<T>> H_PB_W = body_node->GetJacobianFromArray(H_PB_W_cache);
                    ...
                    M->block(composite_start_in_v, body_start_in_v, cnv, bnv) += HtFm.transpose();
                }
...
template <typename T>
void MultibodyTree<T>::CalcBiasTerm(const systems::Context<T>& context, EigenPtr<VectorX<T>> Cv) const {
    ...
    CalcInverseDynamics(context, vdot, {}, VectorX<T>(), &A_WB_array, &F_BMo_W_array, Cv);
}

template <typename T>
VectorX<T> MultibodyTree<T>::CalcGravityGeneralizedForces(const systems::Context<T>& context) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    if (gravity_field_) {
...
template <typename T>
RigidTransform<T> MultibodyTree<T>::CalcRelativeTransform(const systems::Context<T>& context,
...
template <typename T>
RotationMatrix<T> MultibodyTree<T>::CalcRelativeRotationMatrix(const systems::Context<T>& context,
                                                               const Frame<T>& frame_F,
                                                               const Frame<T>& frame_G) const {
...
template <typename T>
void MultibodyTree<T>::CalcPointsPositions(const systems::Context<T>& context,
    ...
    const RigidTransform<T> X_AB = CalcRelativeTransform(context, frame_A, frame_B);
...
template <typename T>
T MultibodyTree<T>::CalcTotalMass(const systems::Context<T>& context,
                                  const std::vector<ModelInstanceIndex>& model_instances) const {
    T total_mass = 0;
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        const RigidBody<T>& body = get_body(body_index);
        if (std::find(model_instances.begin(), model_instances.end(), body.model_instance()) != model_instances.end()) {
            const T& body_mass = body.get_mass(context);
            total_mass += body_mass;
        }
    }
...
template <typename T>
Vector3<T> MultibodyTree<T>::CalcCenterOfMassPositionInWorld(const systems::Context<T>& context) const {
    if (num_bodies() <= 1) {
...
Vector3<T> MultibodyTree<T>::CalcCenterOfMassPositionInWorld(
        const systems::Context<T>& context, const std::vector<ModelInstanceIndex>& model_instances) const {
...
template <typename T>
SpatialInertia<T> MultibodyTree<T>::CalcSpatialInertia(const systems::Context<T>& context,
                                                       const Frame<T>& frame_F,
    ...
    const std::set<BodyIndex> without_duplicate_bodies(body_indexes.begin(), body_indexes.end());
    if (body_indexes.size() != without_duplicate_bodies.size()) {
        throw std::logic_error("CalcSpatialInertia(): contains a repeated BodyIndex.");
    }
    ...
    const std::vector<SpatialInertia<T>>& M_Bi_W = EvalSpatialInertiaInWorldCache(context);
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    ...
    SpatialInertia<T> M_SWo_W(0., Vector3<T>::Zero(), UnitInertia<T>::TriaxiallySymmetric(0));
    ...
    for (BodyIndex body_index : body_indexes) {
        ...
        if (body_index >= num_bodies()) {
            throw std::logic_error("CalcSpatialInertia(): contains an invalid BodyIndex.");
        }
...
template <typename T>
Vector3<T> MultibodyTree<T>::CalcCenterOfMassTranslationalVelocityInWorld(const systems::Context<T>& context) const {
    if (num_bodies() <= 1) {
        std::string message = fmt::format(
                "{}(): This MultibodyPlant only contains "
                "the world_body() so its center of mass is undefined.",
    ...
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        ...
        const Vector3<T> vi_WBcm_W = body.CalcCenterOfMassTranslationalVelocityInWorld(context);
...
template <typename T>
SpatialMomentum<T> MultibodyTree<T>::CalcSpatialMomentumInWorldAboutPoint(const systems::Context<T>& context,
    ...
    std::vector<ModelInstanceIndex> model_instances;
    for (ModelInstanceIndex model_instance_index(1); model_instance_index < num_model_instances();
         ++model_instance_index)
        model_instances.push_back(model_instance_index);

    return CalcSpatialMomentumInWorldAboutPoint(context, model_instances, p_WoP_W);
}
...
SpatialMomentum<T> MultibodyTree<T>::CalcSpatialMomentumInWorldAboutPoint(
    ...
    for (auto model_instance : model_instances) {
        ...
        const std::vector<BodyIndex> body_index_in_instance = GetBodyIndices(model_instance);
        for (BodyIndex body_index : body_index_in_instance) body_indexes.push_back(body_index);
    }

    // Form spatial momentum about Wo (origin of world frame W), expressed in W.
    const SpatialMomentum<T> L_WS_W = CalcBodiesSpatialMomentumInWorldAboutWo(context, body_indexes);
...
SpatialMomentum<T> MultibodyTree<T>::CalcBodiesSpatialMomentumInWorldAboutWo(
        const systems::Context<T>& context, const std::vector<BodyIndex>& body_indexes) const {
...
template <typename T>
const RigidTransform<T>& MultibodyTree<T>::EvalBodyPoseInWorld(const systems::Context<T>& context,
...
template <typename T>
const SpatialVelocity<T>& MultibodyTree<T>::EvalBodySpatialVelocityInWorld(const systems::Context<T>& context,
                                                                           const RigidBody<T>& body_B) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    body_B.HasThisParentTreeOrThrow(this);
...
template <typename T>
const SpatialAcceleration<T>& MultibodyTree<T>::EvalBodySpatialAccelerationInWorld(const systems::Context<T>& context,
                                                                                   const RigidBody<T>& body_B) const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    body_B.HasThisParentTreeOrThrow(this);
...
template <typename T>
void MultibodyTree<T>::CalcAcrossNodeJacobianWrtVExpressedInWorld(const systems::Context<T>& context,
                                                                  const PositionKinematicsCache<T>& pc,
    ...
    for (MobodIndex mobod_index(1); mobod_index < topology_.num_mobods(); ++mobod_index) {
        ...
        Eigen::Map<MatrixUpTo6<T>> H_PB_W = node.GetMutableJacobianFromArray(H_PB_W_cache);
...
template <typename T>
SpatialAcceleration<T> MultibodyTree<T>::CalcBiasSpatialAcceleration(const systems::Context<T>& context,
                                                                     JacobianWrtVariable with_respect_to,
                                                                     const Frame<T>& frame_B,
    ...
    CalcAllBodyBiasSpatialAccelerationsInWorld(context, with_respect_to, &AsBias_WB_all);
    ...
    const SpatialAcceleration<T> AsBias_WBodyB_W = AsBias_WB_all[body_B.mobod_index()];
    ...
    const SpatialAcceleration<T> AsBias_WBodyA_W = AsBias_WB_all[body_A.mobod_index()];

    // Calculate Bp's spatial acceleration bias in body_A, expressed in frame_E.
    return CalcSpatialAccelerationHelper(context, frame_B, p_BoBp_B, body_A, frame_E, AsBias_WBodyB_W, AsBias_WBodyA_W);
}

template <typename T>
SpatialAcceleration<T> MultibodyTree<T>::CalcSpatialAccelerationHelper(const systems::Context<T>& context,
                                                                       const Frame<T>& frame_F,
    ...
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
    const SpatialAcceleration<T> A_WFp_W = ShiftSpatialAccelerationInWorld(frame_F, p_FoFp_F, A_WB_W, pc, vc);
    ...
    if (frame_A.is_world_frame()) {
        ...
        const SpatialAcceleration<T> A_WAp_W = ShiftSpatialAccelerationInWorld(frame_A, p_AoAp_A, A_WA_W, pc, vc);
        ...
        const Vector3<T> w_WA_W = body_A.EvalSpatialVelocityInWorld(context).rotational();
        SpatialVelocity<T> V_AF_W = frame_F.CalcSpatialVelocity(context, frame_A, frame_W);
    ...
    const RotationMatrix<T> R_EW = frame_E.CalcRotationMatrixInWorld(context).inverse();
    const SpatialAcceleration<T> A_AFp_E = R_EW * A_AFp_W;
    return A_AFp_E;
}

template <typename T>
SpatialAcceleration<T> MultibodyTree<T>::ShiftSpatialAccelerationInWorld(const Frame<T>& frame_B,
                                                                         const Eigen::Ref<const Vector3<T>>& p_BoBp_B,
...
template <typename T>
Matrix3X<T> MultibodyTree<T>::CalcBiasTranslationalAcceleration(const systems::Context<T>& context,
                                                                JacobianWrtVariable with_respect_to,
                                                                const Frame<T>& frame_B,
    ...
    const SpatialAcceleration<T> AsBias_ABo_E =
            CalcBiasSpatialAcceleration(context, with_respect_to, frame_B, Vector3<T>::Zero(), frame_A, frame_E);

    // Get R_EB (rotation matrix relating frame_E to frame_B).
    const RotationMatrix<T> R_EB = CalcRelativeRotationMatrix(context, frame_E, frame_B);

    // Form w_AB_E (B's angular velocity in frame A, measured in frame_E).
    const Vector3<T> w_AB_E = frame_B.CalcSpatialVelocity(context, frame_A, frame_E).rotational();
    ...
    for (int ipoint = 0; ipoint < num_points; ++ipoint) {
        ...
        const SpatialAcceleration<T> AsBias_ABp_E = AsBias_ABo_E.Shift(p_BoBp_E, w_AB_E);
...
template <typename T>
void MultibodyTree<T>::CalcJacobianSpatialVelocity(const systems::Context<T>& context,
    ...
    const int num_columns = (with_respect_to == JacobianWrtVariable::kQDot) ? num_positions() : num_velocities();
    ...
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_A, p_WP, &Js_w_WAp, &Js_v_WAp);
    ...
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_B, p_WP, &Js_w_WBp, &Js_v_WBp);
    ...
    if (frame_E.index() != world_frame().index()) {
        const RotationMatrix<T> R_EW = CalcRelativeRotationMatrix(context, frame_E, world_frame());
        Js_V_ABp_E->template topRows<3>() = R_EW * Js_V_ABp_E->template topRows<3>();
        Js_V_ABp_E->template bottomRows<3>() = R_EW * Js_V_ABp_E->template bottomRows<3>();
    }
}

template <typename T>
void MultibodyTree<T>::CalcJacobianAngularVelocity(const systems::Context<T>& context,
                                                   const JacobianWrtVariable with_respect_to,
                                                   const Frame<T>& frame_B,
                                                   const Frame<T>& frame_A,
                                                   const Frame<T>& frame_E,
    ...
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_A, empty_position_list,
                                                         &Js_w_WA_W, nullptr);

    Matrix3X<T> Js_w_WB_W(3, num_columns);
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_B, empty_position_list,
                                                         &Js_w_WB_W, nullptr);
    ...
    if (frame_E.index() == frame_W.index()) {
        ...
        *Js_w_AB_E = Js_w_WB_W - Js_w_WA_W;  // This calculates Js_w_AB_W.
        ...
        const RotationMatrix<T> R_EW = CalcRelativeRotationMatrix(context, frame_E, frame_W);
        *Js_w_AB_E = R_EW * (Js_w_WB_W - Js_w_WA_W);
    }
}

template <typename T>
void MultibodyTree<T>::CalcJacobianTranslationalVelocityHelper(const systems::Context<T>& context,
                                                               const JacobianWrtVariable with_respect_to,
                                                               const Frame<T>& frame_B,
    ...
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_B, p_WoBi_W, nullptr,
                                                         Js_v_ABi_W);
    ...
    CalcJacobianAngularAndOrTranslationalVelocityInWorld(context, with_respect_to, frame_A, p_WoBi_W, nullptr,
                                                         &Js_v_WAi_W);
...
template <typename T>
void MultibodyTree<T>::CalcJacobianTranslationalVelocity(const systems::Context<T>& context,
                                                         const JacobianWrtVariable with_respect_to,
                                                         const Frame<T>& frame_B,
                                                         const Frame<T>& frame_F,
    ...
    const Frame<T>& frame_W = world_frame();
    if (&frame_F == &frame_W) {
        CalcJacobianTranslationalVelocityHelper(context, with_respect_to, frame_B, p_FoBi_F, frame_A, Js_v_ABi_E);
    } else {
        ...
        CalcJacobianTranslationalVelocityHelper(context, with_respect_to, frame_B, p_WoBi_W, frame_A, Js_v_ABi_E);
    }
    ...
    if (&frame_E != &frame_W) {
        ...
        for (int i = 0; i < num_points; ++i) {
            Js_v_ABi_E->template block<3, Eigen::Dynamic>(3 * i, 0, 3, num_columns) =
                    R_EW * Js_v_ABi_E->template block<3, Eigen::Dynamic>(3 * i, 0, 3, num_columns);
        }
    }
}
...
void MultibodyTree<T>::CalcJacobianAngularAndOrTranslationalVelocityInWorld(
    ...
    for (size_t level = 1; level < path_to_world.size(); ++level) {
        ...
        const int mobilizer_jacobian_ncols = is_wrt_qdot ? mobilizer_num_positions : mobilizer_num_velocities;
        ...
        if (Js_w_WF_W) {
            ...
            auto Js_w_PB_W = Js_w_WF_W->block(0, start_index, 3, mobilizer_jacobian_ncols);
            if (is_wrt_qdot) {
        ...
        if (Js_v_WFpi_W) {
            ...
            auto Js_v_PFpi_W = Js_v_WFpi_W->block(0, start_index, 3 * num_points, mobilizer_jacobian_ncols);
            ...
            for (int ipoint = 0; ipoint < num_points; ++ipoint) {
                ...
                auto Hv_PFpi_W = Js_v_PFpi_W.block(ipoint_row, 0, 3, mobilizer_jacobian_ncols);
...
template <typename T>
void MultibodyTree<T>::CalcJacobianCenterOfMassTranslationalVelocity(const systems::Context<T>& context,
                                                                     JacobianWrtVariable with_respect_to,
                                                                     const Frame<T>& frame_A,
                                                                     const Frame<T>& frame_E,
    ...
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        ...
        CalcJacobianTranslationalVelocity(context, with_respect_to, body.body_frame(), body.body_frame(), pi_BoBcm,
                                          frame_A, frame_E, &Jsi_v_ABcm_E);
        const T& body_mass = body.get_mass(context);
...
template <typename T>
Vector3<T> MultibodyTree<T>::CalcBiasCenterOfMassTranslationalAcceleration(const systems::Context<T>& context,
                                                                           JacobianWrtVariable with_respect_to,
                                                                           const Frame<T>& frame_A,
                                                                           const Frame<T>& frame_E) const {
    ...
    for (BodyIndex body_index(1); body_index < num_bodies(); ++body_index) {
        ...
        const SpatialAcceleration<T> AsBiasi_ACcm_E =
                CalcBiasSpatialAcceleration(context, with_respect_to, body.body_frame(), pi_BoBcm, frame_A, frame_E);
        const T& body_mass = body.get_mass(context);
...
template <typename T>
T MultibodyTree<T>::CalcPotentialEnergy(const systems::Context<T>& context) const {
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
...
template <typename T>
T MultibodyTree<T>::CalcKineticEnergy(const systems::Context<T>& context) const {
    const std::vector<SpatialInertia<T>>& M_Bi_W = EvalSpatialInertiaInWorldCache(context);
    const VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
    const VectorX<T>& reflected_inertia = EvalReflectedInertiaCache(context);
    ...
    twice_kinetic_energy_W += (v.array() * reflected_inertia.array() * v.array()).sum();

    return twice_kinetic_energy_W / 2.;
}

template <typename T>
T MultibodyTree<T>::CalcConservativePower(const systems::Context<T>& context) const {
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
...
template <typename T>
T MultibodyTree<T>::CalcNonConservativePower(const systems::Context<T>& context) const {
    const PositionKinematicsCache<T>& pc = EvalPositionKinematics(context);
    const VelocityKinematicsCache<T>& vc = EvalVelocityKinematics(context);
    ...
    // Add contributions from force elements.
    for (const auto& force_element : owned_force_elements_) {
        non_conservative_power += force_element->CalcNonConservativePower(context, pc, vc);
    }
...
void MultibodyTree<T>::ThrowDefaultMassInertiaError() const {
    ...
    std::vector<std::set<BodyIndex>> welded_bodies_list = topology.CreateListOfWeldedBodies();
    ...
    for (size_t i = 1; i < number_of_sets; ++i) {
        ...
        const RigidBodyTopology& parent_body_topology = tree_topology.get_rigid_body(parent_body_index);
        const MobilizerIndex& parent_mobilizer_index = parent_body_topology.inboard_mobilizer;
        const Mobilizer<T>& parent_mobilizer = get_mobilizer(parent_mobilizer_index);
        ...
        const BodyNodeTopology& parent_body_node_topology = tree_topology.get_body_node(parent_mobod_index);
        const bool is_composite_body_distal_leaf_in_tree =
                tree_topology.CalcNumberOfOutboardVelocitiesExcludingBase(parent_body_node_topology) == 0;
...
template <typename T>
double MultibodyTree<T>::CalcTotalDefaultMass(const std::set<BodyIndex>& body_indexes) const {
    double total_mass = 0;
    for (BodyIndex body_index : body_indexes) {
...
template <typename T>
bool MultibodyTree<T>::IsAnyDefaultRotationalInertiaNaN(const std::set<BodyIndex>& body_indexes) const {
    for (BodyIndex body_index : body_indexes) {
        const RigidBody<T>& body_B = get_body(body_index);
        const RotationalInertia<double> I_BBo_B = body_B.default_rotational_inertia();
...
template <typename T>
bool MultibodyTree<T>::AreAllDefaultRotationalInertiaZero(const std::set<BodyIndex>& body_indexes) const {
    for (BodyIndex body_index : body_indexes) {
        const RigidBody<T>& body_B = get_body(body_index);
        const RotationalInertia<double> I_BBo_B = body_B.default_rotational_inertia();
...
template <typename T>
void MultibodyTree<T>::CalcArticulatedBodyInertiaCache(const systems::Context<T>& context,
...
void MultibodyTree<T>::CalcArticulatedBodyInertiaCache(const systems::Context<T>& context,
    ...
    for (int depth = tree_height() - 1; depth > 0; --depth) {
        for (MobodIndex mobod_index : body_node_levels_[depth]) {
            ...
            Eigen::Map<const MatrixUpTo6<T>> H_PB_W = node.GetJacobianFromArray(H_PB_W_cache);
            const SpatialInertia<T>& M_B_W = spatial_inertia_in_world_cache[mobod_index];

            node.CalcArticulatedBodyInertiaCache_TipToBase(context, pc, H_PB_W, M_B_W, diagonal_inertias, abic);
        }
    }
}

template <typename T>
void MultibodyTree<T>::CalcArticulatedBodyForceCache(const systems::Context<T>& context,
                                                     const ArticulatedBodyInertiaCache<T>& abic,
    ...
    const std::vector<SpatialForce<T>>& dynamic_bias_cache = EvalDynamicBiasCache(context);
    ...
    for (int depth = tree_height() - 1; depth > 0; --depth) {
        for (MobodIndex mobod_index : body_node_levels_[depth]) {
            ...
            Eigen::Ref<const VectorX<T>> tau_applied =
                    node.get_mobilizer().get_generalized_forces_from_array(generalized_forces);
            ...
            node.CalcArticulatedBodyForceCache_TipToBase(context, pc, &vc, Fb_B_W, abic, Zb_Bo_W, Fapplied_Bo_W,
                                                         tau_applied, H_PB_W, aba_force_cache);
        }
    }
}
...
void MultibodyTree<T>::CalcArticulatedBodyForceCache(const systems::Context<T>& context,
    ...
    const std::vector<SpatialForce<T>>& Zb_Bo_W_cache = EvalArticulatedBodyForceBiasCache(context);
    CalcArticulatedBodyForceCache(context, abic, Zb_Bo_W_cache, forces, aba_force_cache);
}

template <typename T>
void MultibodyTree<T>::CalcArticulatedBodyAccelerations(const systems::Context<T>& context,
                                                        const ArticulatedBodyInertiaCache<T>& abic,
    ...
    for (int level = 1; level < tree_height(); ++level) {
        for (MobodIndex mobod_index : body_node_levels_[level]) {
            ...
            node.CalcArticulatedBodyAccelerations_BaseToTip(context, pc, abic, aba_force_cache, H_PB_W, Ab_WB, ac);
        }
    }
}
...
MatrixX<double> MultibodyTree<T>::MakeStateSelectorMatrix(
    ...
    for (const auto& joint_index : user_to_joint_index_map) {
        ...
        if (!inserted) {
            throw std::logic_error("Joint named '" + get_joint(joint_index).name() + "' is repeated multiple times.");
        }
    }
    ...
    const int num_selected_states = num_selected_positions + num_selected_velocities;
    ...
    for (JointIndex joint_index : user_to_joint_index_map) {
        ...
        Sx.block(selected_positions_index, pos_start, num_pos, num_pos) = MatrixX<double>::Identity(num_pos, num_pos);
...
MatrixX<double> MultibodyTree<T>::MakeActuatorSelectorMatrix(
    ...
    MatrixX<double> Su = MatrixX<double>::Zero(num_actuated_dofs(), num_selected_actuators);
    int user_index = 0;
...
MatrixX<double> MultibodyTree<T>::MakeActuatorSelectorMatrix(
    ...
    std::vector<std::optional<JointActuatorIndex>> joint_to_actuator_index(num_joints());
    ...
    for (JointIndex joint_index : user_to_joint_index_map) {
        ...
        if (!joint_to_actuator_index.at(joint.ordinal()).has_value()) {
            throw std::logic_error("Joint '" + joint.name() + "' does not have an actuator.");
        }

        user_to_actuator_index_map.push_back(joint_to_actuator_index.at(joint.ordinal()).value());
    }
...
VectorX<double> MultibodyTree<T>::GetPositionLowerLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd q_lower = Eigen::VectorXd::Constant(num_positions(), -std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        q_lower.segment(joint.position_start(), joint.num_positions()) = joint.position_lower_limits();
    }
    return q_lower;
}
...
VectorX<double> MultibodyTree<T>::GetPositionUpperLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd q_upper = Eigen::VectorXd::Constant(num_positions(), std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        q_upper.segment(joint.position_start(), joint.num_positions()) = joint.position_upper_limits();
    }
    return q_upper;
}
...
VectorX<double> MultibodyTree<T>::GetVelocityLowerLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd v_lower = Eigen::VectorXd::Constant(num_velocities(), -std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        v_lower.segment(joint.velocity_start(), joint.num_velocities()) = joint.velocity_lower_limits();
    }
    return v_lower;
}
...
VectorX<double> MultibodyTree<T>::GetVelocityUpperLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd v_upper = Eigen::VectorXd::Constant(num_velocities(), std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        v_upper.segment(joint.velocity_start(), joint.num_velocities()) = joint.velocity_upper_limits();
    }
    return v_upper;
}
...
VectorX<double> MultibodyTree<T>::GetAccelerationLowerLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd vd_lower = Eigen::VectorXd::Constant(num_velocities(), -std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        vd_lower.segment(joint.velocity_start(), joint.num_velocities()) = joint.acceleration_lower_limits();
    }
    return vd_lower;
}
...
VectorX<double> MultibodyTree<T>::GetAccelerationUpperLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd vd_upper = Eigen::VectorXd::Constant(num_velocities(), std::numeric_limits<double>::infinity());
    for (JointIndex i : GetJointIndices()) {
        const auto& joint = get_joint(i);
        vd_upper.segment(joint.velocity_start(), joint.num_velocities()) = joint.acceleration_upper_limits();
    }
    return vd_upper;
}
...
VectorX<double> MultibodyTree<T>::GetEffortLowerLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd lower = Eigen::VectorXd::Constant(num_actuated_dofs(), -std::numeric_limits<double>::infinity());
    for (JointActuatorIndex i : GetJointActuatorIndices()) {
        const auto& actuator = get_joint_actuator(i);
        for (int j = actuator.input_start(); j < actuator.input_start() + actuator.num_inputs(); ++j) {
...
VectorX<double> MultibodyTree<T>::GetEffortUpperLimits() const {
    DRAKE_MBT_THROW_IF_NOT_FINALIZED();
    Eigen::VectorXd upper = Eigen::VectorXd::Constant(num_actuated_dofs(), std::numeric_limits<double>::infinity());
    for (JointActuatorIndex i : GetJointActuatorIndices()) {
        const auto& actuator = get_joint_actuator(i);
        for (int j = actuator.input_start(); j < actuator.input_start() + actuator.num_inputs(); ++j) {
            DRAKE_ASSERT(j < num_actuated_dofs());
...
template <typename T>
std::optional<BodyIndex> MultibodyTree<T>::MaybeGetUniqueBaseBodyIndex(ModelInstanceIndex model_instance) const {
    DRAKE_THROW_UNLESS(model_instances_.has_element(model_instance));
    ...
    for (const RigidBody<T>* body : rigid_bodies_.elements()) {
        if (body->model_instance() == model_instance &&
            (topology_.get_rigid_body(body->index()).parent_body == world_index())) {
            if (base_body_index.has_value()) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::MultibodyTree);
