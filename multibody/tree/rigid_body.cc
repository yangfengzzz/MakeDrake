...
std::unique_ptr<Frame<ToScalar>> RigidBodyFrame<T>::TemplatedDoCloneToScalar(
    ...
    const RigidBody<ToScalar>& body_clone = tree_clone.get_body(this->body().index());
    // RigidBodyFrame's constructor cannot be called from std::make_unique since
    // it is private and therefore we use "new".
    return std::unique_ptr<RigidBodyFrame<ToScalar>>(new RigidBodyFrame<ToScalar>(body_clone));
}
...
ScopedName RigidBody<T>::scoped_name() const {
    return ScopedName(this->get_parent_tree().GetModelInstanceName(this->model_instance()), name_);
}

template <typename T>
RigidBody<T>::RigidBody(const std::string& body_name, const SpatialInertia<double>& M)
...
template <typename T>
void RigidBody<T>::SetCenterOfMassInBodyFrameNoModifyInertia(systems::Context<T>* context,
    ...
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_com_x, x);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_com_y, y);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_com_z, z);
}

template <typename T>
void RigidBody<T>::SetUnitInertiaAboutBodyOrigin(systems::Context<T>* context, const UnitInertia<T>& G_BBo_B) const {
    DRAKE_THROW_UNLESS(context != nullptr);
    ...
    systems::BasicVector<T>& spatial_inertia_parameter =
            context->get_mutable_numeric_parameter(spatial_inertia_parameter_index_);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gxx, Gxx);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gyy, Gyy);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gzz, Gzz);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gxy, Gxy);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gxz, Gxz);
    spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_Gyz, Gyz);
}

template <typename T>
void RigidBody<T>::SetCenterOfMassInBodyFrameAndPreserveCentralInertia(
        systems::Context<T>* context, const Vector3<T>& center_of_mass_position) const {
    DRAKE_THROW_UNLESS(context != nullptr);
...
void RigidBody<T>::AddInForce(const systems::Context<T>& context,
    ...
    const math::RotationMatrix<T> R_WE = frame_E.CalcRotationMatrixInWorld(context);
...
template <typename T>
Vector3<T> RigidBody<T>::CalcCenterOfMassTranslationalVelocityInWorld(const systems::Context<T>& context) const {
    ...
    const SpatialVelocity<T>& V_WBo_W = body_B.EvalSpatialVelocityInWorld(context);
    ...
    const math::RotationMatrix<T> R_WB = frame_B.CalcRotationMatrixInWorld(context);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::RigidBodyFrame);

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RigidBody);
