...
template <typename T>
MultibodyTreeSystem<T>::MultibodyTreeSystem(std::unique_ptr<MultibodyTree<T>> tree, bool is_discrete)
...
template <typename T>
MultibodyTreeSystem<T>::MultibodyTreeSystem(systems::SystemScalarConverter converter,
...
MultibodyTreeSystem<T>::MultibodyTreeSystem(systems::SystemScalarConverter converter,
    ...
    if (tree == nullptr) {
        if (!null_tree_is_ok) {
            throw std::logic_error("MultibodyTreeSystem(): the supplied MultibodyTree was null.");
        }
...
template <typename T>
void MultibodyTreeSystem<T>::SetDefaultParameters(const Context<T>& context, Parameters<T>* parameters) const {
    LeafSystem<T>::SetDefaultParameters(context, parameters);

    // Mobilizers.
    for (MobilizerIndex mobilizer_index(0); mobilizer_index < tree_->num_mobilizers(); ++mobilizer_index) {
        internal_tree().get_mobilizer(mobilizer_index).SetDefaultParameters(parameters);
    }
    // Joints.
    ...
    // JointActuators.
    for (JointActuatorIndex joint_actuator_index : tree_->GetJointActuatorIndices()) {
        internal_tree().get_joint_actuator(joint_actuator_index).SetDefaultParameters(parameters);
    }
    // Bodies.
    for (BodyIndex body_index(0); body_index < tree_->num_bodies(); ++body_index) {
        internal_tree().get_body(body_index).SetDefaultParameters(parameters);
    }
    // Frames.
    for (FrameIndex frame_index(0); frame_index < tree_->num_frames(); ++frame_index) {
        internal_tree().get_frame(frame_index).SetDefaultParameters(parameters);
    }
    // Force Elements.
    for (ForceElementIndex force_element_index(0); force_element_index < tree_->num_force_elements();
         ++force_element_index) {
        internal_tree().get_force_element(force_element_index).SetDefaultParameters(parameters);
    }
}

template <typename T>
void MultibodyTreeSystem<T>::SetDefaultState(const Context<T>& context, State<T>* state) const {
...
void MultibodyTreeSystem<T>::DeclareMultibodyElementParameters() {
    // Mobilizers.
    for (MobilizerIndex mobilizer_index(0); mobilizer_index < tree_->num_mobilizers(); ++mobilizer_index) {
        mutable_tree().get_mutable_mobilizer(mobilizer_index).DeclareParameters(this);
    }
    // Joints.
    for (JointIndex joint_index : tree_->GetJointIndices()) {
    ...
    // JointActuators.
    for (JointActuatorIndex joint_actuator_index : tree_->GetJointActuatorIndices()) {
        mutable_tree().get_mutable_joint_actuator(joint_actuator_index).DeclareParameters(this);
    }
    // Bodies.
    for (BodyIndex body_index(0); body_index < tree_->num_bodies(); ++body_index) {
    ...
    for (ForceElementIndex force_element_index(0); force_element_index < tree_->num_force_elements();
         ++force_element_index) {
        mutable_tree().get_mutable_force_element(force_element_index).DeclareParameters(this);
    }
}
...
void MultibodyTreeSystem<T>::Finalize() {
    if (already_finalized_) {
        throw std::logic_error("MultibodyTreeSystem::Finalize(): repeated calls not allowed.");
    }
    ...
    // Declare state.
    if (is_discrete_) {
        tree_->set_discrete_state_index(this->DeclareDiscreteState(tree_->num_states()));
    } else {
        this->DeclareContinuousState(BasicVector<T>(tree_->num_states()), tree_->num_positions(),
    ...
    const DependencyTicket position_ticket = is_discrete_ ? this->xd_ticket() : this->q_ticket();
    const DependencyTicket velocity_ticket = is_discrete_ ? this->xd_ticket() : this->v_ticket();

    // Allocate position cache.
    cache_indexes_.position_kinematics =
            this->DeclareCacheEntry(std::string("position kinematics"),
    ...
    cache_indexes_.spatial_inertia_in_world =
            this->DeclareCacheEntry(
                        ...
                        std::vector<SpatialInertia<T>>(internal_tree().num_bodies(), SpatialInertia<T>::NaN()),
                        &MultibodyTreeSystem<T>::CalcSpatialInertiasInWorld,
                        {position_kinematics_cache_entry().ticket()})
                    .cache_index();

    cache_indexes_.reflected_inertia =
            this->DeclareCacheEntry(std::string("reflected inertia"), VectorX<T>(internal_tree().num_velocities()),
                                    &MultibodyTreeSystem<T>::CalcReflectedInertia, {this->all_parameters_ticket()})
                    .cache_index();

    // Allocate cache entry for joint damping.
    cache_indexes_.joint_damping =
            this->DeclareCacheEntry(std::string("joint damping"), VectorX<T>(internal_tree().num_velocities()),
                                    &MultibodyTreeSystem<T>::CalcJointDamping, {this->all_parameters_ticket()})
                    .cache_index();
    ...
    cache_indexes_.velocity_kinematics =
            this->DeclareCacheEntry(std::string("velocity kinematics"),
    ...
    cache_indexes_.dynamic_bias =
            this->DeclareCacheEntry(std::string("dynamic bias (Fb_Bo_W)"),
    ...
    cache_indexes_.across_node_jacobians =
            this->DeclareCacheEntry(std::string("H_PB_W(q)"), std::vector<Vector6<T>>(internal_tree().num_velocities()),
    ...
    cache_indexes_.abi_cache_index =
            this->DeclareCacheEntry(std::string("Articulated Body Inertia"),
    ...
    cache_indexes_.spatial_acceleration_bias =
            this->DeclareCacheEntry(std::string("spatial acceleration bias (Ab_WB)"),
    ...
    cache_indexes_.articulated_body_force_bias =
            this->DeclareCacheEntry(std::string("ABI force bias cache (Zb_Bo_W)"),
                                    std::vector<SpatialForce<T>>(internal_tree().num_bodies()),
    ...
    const std::set<DependencyTicket> force_and_acceleration_prereqs = {
            position_ticket,     velocity_ticket,         this->all_parameters_ticket(),
            this->time_ticket(), this->accuracy_ticket(), this->all_input_ports_ticket()};

    // Articulated Body Algorithm (ABA) force cache.
    const auto& articulated_body_forces_cache_entry = this->DeclareCacheEntry(
            std::string("ABA force cache"), ArticulatedBodyForceCache<T>(internal_tree().get_topology()),
            &MultibodyTreeSystem<T>::CalcArticulatedBodyForceCache, force_and_acceleration_prereqs);
    cache_indexes_.articulated_body_forces = articulated_body_forces_cache_entry.cache_index();
    ...
    cache_indexes_.acceleration_kinematics =
            this->DeclareCacheEntry(std::string("Accelerations"),
                                    AccelerationKinematicsCache<T>(internal_tree().get_topology()),
                                    &MultibodyTreeSystem<T>::CalcForwardDynamics, force_and_acceleration_prereqs)
                    .cache_index();
...
template <typename T>
void MultibodyTreeSystem<T>::DoCalcTimeDerivatives(const systems::Context<T>& context,
    ...
    const VectorX<T>& x = dynamic_cast<const systems::BasicVector<T>&>(context.get_continuous_state_vector()).value();
...
template <typename T>
void MultibodyTreeSystem<T>::DoMapQDotToVelocity(const systems::Context<T>& context,
...
template <typename T>
void MultibodyTreeSystem<T>::DoMapVelocityToQDot(const systems::Context<T>& context,
...
void MultibodyTreeSystem<T>::DoCalcImplicitTimeDerivativesResidual(
    ...
    DRAKE_DEMAND(residual->size() == this->implicit_time_derivatives_residual_size());
    ...
    const VectorX<T>& qvdot_proposed =
            dynamic_cast<const systems::BasicVector<T>&>(proposed_derivatives.get_vector()).value();
    DRAKE_ASSERT(qvdot_proposed.size() == nq + nv);

    auto qdot_residual = residual->head(nq);
    // N(q)⋅v
    internal_tree().MapVelocityToQDot(context, internal_tree().get_velocities(context), &qdot_residual);
    // q̇_proposed - N(q)⋅v
    qdot_residual = qvdot_proposed.head(nq) - qdot_residual;
    // InverseDynamics(context, v_proposed)
    residual->tail(nv) = internal_tree().CalcInverseDynamics(context, qvdot_proposed.tail(nv), forces);
}

template <typename T>
void MultibodyTreeSystem<T>::CalcArticulatedBodyForceCache(const systems::Context<T>& context,
    ...
    internal_tree().CalcArticulatedBodyForceCache(context, forces, aba_force_cache);
}

template <typename T>
void MultibodyTreeSystem<T>::CalcForwardDynamicsContinuous(const systems::Context<T>& context,
                                                           AccelerationKinematicsCache<T>* ac) const {
    DRAKE_DEMAND(ac != nullptr);

    // Collect forces from all sources and propagate tip-to-base.
    const ArticulatedBodyForceCache<T>& aba_force_cache = EvalArticulatedBodyForceCache(context);

    // Perform the last base-to-tip pass to compute accelerations using the O(n)
    // ABA.
    internal_tree().CalcArticulatedBodyAccelerations(context, aba_force_cache, ac);
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::internal::MultibodyTreeSystem);
