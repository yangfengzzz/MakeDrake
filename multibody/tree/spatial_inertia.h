...
public:
    ...
    static SpatialInertia Zero() { return SpatialInertia(0, Vector3<T>::Zero(), UnitInertia<T>(0, 0, 0)); }
...
#pragma GCC diagnostic pop
    ...
    SpatialInertia<Scalar> cast() const {
        return SpatialInertia<Scalar>(get_mass(), get_com().template cast<Scalar>(),
    ...
    boolean<T> IsNaN() const {
        using std::isnan;
        return isnan(mass_) || G_SP_E_.IsNaN() || any_of(p_PScm_E_, [](const auto& x) {
                   return isnan(x);
               });
    }
    ...
    boolean<T> IsZero() const {
        return (mass_ == 0.0) && G_SP_E_.IsZero() && all_of(p_PScm_E_, [](const auto& x) {
                   return (x == 0.0);
               });
    }
    ...
    /// be converted to a real finite double. For example, an exception is thrown
    /// if `this` contains an erroneous NaN or if scalar type T is symbolic.
    std::pair<Vector3<double>, drake::math::RigidTransform<double>> CalcPrincipalSemiDiametersAndPoseForSolidEllipsoid()
            const {
        constexpr double inertia_shape_factor = 1.0 / 5.0;
        return CalcPrincipalHalfLengthsAndPoseForEquivalentShape(inertia_shape_factor);
    }
    ...
    Eigen::Matrix<T, 6, Derived::ColsAtCompileTime, 0, 6, Derived::MaxColsAtCompileTime> operator*(
        ...
        Eigen::Matrix<T, 6, Derived::ColsAtCompileTime, 0, 6, Derived::MaxColsAtCompileTime> F_Bo_E(6, Mmatrix.cols());

        // Rotational component.
        F_Bo_E.template topRows<3>() = I_SP_E * Vrotational - Vtranslational.colwise().cross(mp_BoBcm_E);

        // Translational component.
        F_Bo_E.template bottomRows<3>() = Vrotational.colwise().cross(mp_BoBcm_E) + get_mass() * Vtranslational;
        return F_Bo_E;
    }

private:
    // Helper method for NaN initialization.
    static constexpr T nan() { return std::numeric_limits<typename Eigen::NumTraits<T>::Literal>::quiet_NaN(); }
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool> CheckInvariants() const {
    ...
    // @pre On entry, the about-point for `this` SpatialInertia is Scm. Hence, on
    // entry the position vector p_PScm underlying `this` is the zero vector.
    [[nodiscard]] SpatialInertia<T> ShiftFromCenterOfMass(const Vector3<T>& p_ScmP_E) const;
    ...
    std::pair<Vector3<double>, drake::math::RigidTransform<double>> CalcPrincipalHalfLengthsAndPoseForEquivalentShape(
...
namespace fmt {
template <typename T>
struct formatter<drake::multibody::SpatialInertia<T>> : drake::ostream_formatter {};
}  // namespace fmt
