...
public:
    ...
    bool is_body_frame() const { return this->index() == body_.body_frame().index(); }
    ...
    virtual math::RigidTransform<T> GetFixedPoseInBodyFrame() const {
        throw std::logic_error("Attempting to retrieve a fixed pose from a frame of type '" +
                               drake::NiceTypeName::Get(*this) + "', which does not support this operation.");
    }
    ...
    virtual math::RotationMatrix<T> GetFixedRotationMatrixInBodyFrame() const {
        throw std::logic_error("Unable to retrieve a fixed rotation matrix from a frame of type '" +
                               drake::NiceTypeName::Get(*this) + "', which does not support this method.");
    }
    ...
    math::RigidTransform<T> CalcPoseInWorld(const systems::Context<T>& context) const {
        return this->get_parent_tree().CalcRelativeTransform(context, this->get_parent_tree().world_frame(), *this);
    }
    ...
    math::RigidTransform<T> CalcPose(const systems::Context<T>& context, const Frame<T>& frame_M) const {
        return this->get_parent_tree().CalcRelativeTransform(context, frame_M, *this);
    }
    ...
    math::RotationMatrix<T> CalcRotationMatrix(const systems::Context<T>& context, const Frame<T>& frame_M) const {
        return this->get_parent_tree().CalcRelativeRotationMatrix(context, frame_M, *this);
    }
    ...
    math::RotationMatrix<T> CalcRotationMatrixInWorld(const systems::Context<T>& context) const {
        return this->get_parent_tree().CalcRelativeRotationMatrix(context, this->get_parent_tree().world_frame(),
                                                                  *this);
    }
    ...
    SpatialVelocity<T> CalcRelativeSpatialVelocityInWorld(const systems::Context<T>& context,
        ...
        const SpatialVelocity<T> V_WB_W = frame_B.CalcSpatialVelocityInWorld(context);
    ...
    SpatialVelocity<T> CalcRelativeSpatialVelocity(const systems::Context<T>& context,
        ...
        const SpatialVelocity<T> V_MB_E = frame_B.CalcSpatialVelocity(context, frame_M, frame_E);
        const SpatialVelocity<T> V_MC_E = CalcSpatialVelocity(context, frame_M, frame_E);
    ...
    SpatialAcceleration<T> CalcRelativeSpatialAccelerationInWorld(const systems::Context<T>& context,
                                                                  const Frame<T>& other_frame) const {
        const Frame<T>& frame_B = other_frame;
        const SpatialAcceleration<T> A_WB_W = frame_B.CalcSpatialAccelerationInWorld(context);
        const SpatialAcceleration<T> A_WC_W = CalcSpatialAccelerationInWorld(context);
    ...
    SpatialAcceleration<T> CalcRelativeSpatialAcceleration(const systems::Context<T>& context,
        ...
        const Frame<T>& frame_M = measured_in_frame;
        const Frame<T>& frame_E = expressed_in_frame;
        const SpatialAcceleration<T> A_MB_E = frame_B.CalcSpatialAcceleration(context, frame_M, frame_E);
        const SpatialAcceleration<T> A_MC_E = CalcSpatialAcceleration(context, frame_M, frame_E);
