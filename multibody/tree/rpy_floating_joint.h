...
public:
    ...
    @retval angles The angle coordinates of this joint stored in the `context`
      ordered as θr, θp, θy. */
    Vector3<T> get_angles(const Context<T>& context) const { return get_mobilizer().get_angles(context); }
    ...
    DRAKE_DEPRECATED("2024-08-01", "Use RpyFloatingJoint::SetTranslation()")
    ...
    math::RigidTransform<T> GetPose(const systems::Context<T>& context) const {
        return math::RigidTransform<T>(math::RollPitchYaw<T>(get_angles(context)), get_translation(context));
    }
    ...
    Vector3<double> get_default_angles() const { return this->default_positions().template head<3>(); }
...
private:
    ...
    std::string do_get_velocity_suffix(int index) const final { return get_mobilizer().velocity_suffix(index); }
    ...
    const internal::RpyFloatingMobilizer<T>& get_mobilizer() const {
        ...
        const auto* mobilizer =
                dynamic_cast<const internal::RpyFloatingMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::RpyFloatingMobilizer<T>& get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::RpyFloatingMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return *mobilizer;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RpyFloatingJoint);
