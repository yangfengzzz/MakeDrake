...
public:
    ...
    const T& get_angle(const Context<T>& context) const { return get_mobilizer()->get_angle(context); }
    ...
    void set_random_angle_distribution(const symbolic::Expression& angle) {
        get_mutable_mobilizer()->set_random_position_distribution(Vector1<symbolic::Expression>{angle});
    }
    ...
    /// `context`.
    const T& get_angular_rate(const Context<T>& context) const { return get_mobilizer()->get_angular_rate(context); }
    ...
    void set_default_angle(double angle) { this->set_default_positions(Vector1d{angle}); }
...
protected:
    ...
    void DoAddInDamping(const systems::Context<T>& context, MultibodyForces<T>* forces) const override {
        const T damping_torque = -this->GetDamping(context) * get_angular_rate(context);
...
private:
    ...
    const T& DoGetOnePosition(const systems::Context<T>& context) const override { return get_angle(context); }

    const T& DoGetOneVelocity(const systems::Context<T>& context) const override { return get_angular_rate(context); }
    ...
    const internal::RevoluteMobilizer<T>* get_mobilizer() const {
        ...
        const internal::RevoluteMobilizer<T>* mobilizer =
                dynamic_cast<const internal::RevoluteMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::RevoluteMobilizer<T>* get_mutable_mobilizer() {
        // This implementation should always use a mobilizer.
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::RevoluteMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RevoluteJoint);
