...
struct RigidBodyTopology {
    ...
    RigidBodyTopology(BodyIndex body_index, FrameIndex frame_index) : index(body_index), body_frame(frame_index) {}
...
struct FrameTopology {
    ...
    FrameTopology(FrameIndex frame_index, BodyIndex body_index) : index(frame_index), rigid_body(body_index) {}
...
struct MobilizerTopology {
    ...
    bool connects_rigid_bodies(BodyIndex body1, BodyIndex body2) const {
        return (inboard_body == body1 && outboard_body == body2) || (inboard_body == body2 && outboard_body == body1);
    }
...
struct BodyNodeTopology {
    ...
    BodyNodeTopology(MobodIndex index_in,
                     int level_in,
                     MobodIndex parent_node_in,
                     BodyIndex rigid_body_in,
                     BodyIndex parent_rigid_body_in,
...
public:
    ...
    void GetKinematicPathToWorld(MobodIndex from, std::vector<MobodIndex>* path_to_world) const;
    ...
    int CalcNumberOfOutboardVelocitiesExcludingBase(const BodyNodeTopology& base) const {
    ...
    std::vector<BodyIndex> GetTransitiveOutboardBodies(std::vector<BodyIndex> body_indexes) const;

private:
    // Returns `true` if there is _any_ mobilizer in the multibody tree
    // connecting the frames with indexes `frame` and `frame2`.
    bool IsThereAMobilizerBetweenFrames(FrameIndex frame1, FrameIndex frame2) const;

    // Returns `true` if there is _any_ mobilizer in the multibody tree
    // connecting the bodies with indexes `body2` and `body2`.
    bool IsThereAMobilizerBetweenRigidBodies(BodyIndex body1, BodyIndex body2) const;
    ...
    void CreateListOfWeldedBodiesRecurse(BodyIndex parent_index,
    ...
    // depth first order.
    void TraverseOutboardNodes(const BodyNodeTopology& base,
