...
public:
    ...
    RotationalInertia(const T& Ixx, const T& Iyy, const T& Izz) : RotationalInertia(Ixx, Iyy, Izz, 0.0, 0.0, 0.0) {}
    ...
    /// @throws std::exception for Debug builds if not CouldBePhysicallyValid().
    RotationalInertia(const T& mass, const Vector3<T>& p_PQ_E) : RotationalInertia(mass * p_PQ_E, p_PQ_E) {}
    ...
    RotationalInertia<T>& operator*=(const T& nonnegative_scalar) {
        DRAKE_ASSERT_VOID(this->ThrowIfMultiplyByNegativeScalar(nonnegative_scalar));
    ...
    friend RotationalInertia<T> operator*(const T& nonnegative_scalar, const RotationalInertia<T>& I_BP_E) {
    ...
    // TODO(Mitiguy) Issue #6145, add direct unit test for this method.
    Vector3<T> operator*(const Vector3<T>& w_E) const { return Vector3<T>(get_symmetric_matrix_view() * w_E); }
    ...
    void SetToNaN() { I_SP_E_.setConstant(std::numeric_limits<typename Eigen::NumTraits<T>::Literal>::quiet_NaN()); }
    ...
    boolean<T> IsNaN() const {
        ...
        return isnan(I_SP_E_(0, 0)) || isnan(I_SP_E_(1, 0)) || isnan(I_SP_E_(1, 1)) || isnan(I_SP_E_(2, 0)) ||
    ...
    boolean<T> IsZero() const {
        ...
        static_assert(is_lower_triangular_order(2, 1), "Invalid indices");
        static_assert(is_lower_triangular_order(2, 2), "Invalid indices");
        return I_SP_E_(0, 0) == 0.0 && I_SP_E_(1, 0) == 0.0 && I_SP_E_(1, 1) == 0.0 && I_SP_E_(2, 0) == 0.0 &&
    ...
    Vector3<double> CalcPrincipalMomentsOfInertia() const { return CalcPrincipalMomentsAndMaybeAxesOfInertia(nullptr); }
    ...
    boolean<T> CouldBePhysicallyValid() const {
        ...
        return !IsNaN() && AreMomentsOfInertiaNearPositiveAndSatisfyTriangleInequality(p(0), p(1), p(2), epsilon);
    }
    ...
    [[nodiscard]] RotationalInertia<T> ReExpress(const math::RotationMatrix<T>& R_AE) const {
    ...
    /// is shifted to about-point Q violates CouldBePhysicallyValid().
    /// @remark Negating the position vector p_BcmQ_E has no affect on the result.
    [[nodiscard]] RotationalInertia<T> ShiftFromCenterOfMass(const T& mass, const Vector3<T>& p_BcmQ_E) const {
        RotationalInertia result(*this);
    ...
    /// @remark Negating the position vector `p_QBcm_E` has no affect on the
    /// result.
    [[nodiscard]] RotationalInertia<T> ShiftToCenterOfMass(const T& mass, const Vector3<T>& p_QBcm_E) const {
        RotationalInertia result(*this);
    ...
    void ShiftToThenAwayFromCenterOfMassInPlace(const T& mass, const Vector3<T>& p_PBcm_E, const Vector3<T>& p_QBcm_E) {
        *this += mass * ShiftUnitMassBodyToThenAwayFromCenterOfMass(p_PBcm_E, p_QBcm_E);
    }
    ...
    /// @remark Negating either (or both) position vectors p_PBcm_E and p_QBcm_E
    ///         has no affect on the result.
    [[nodiscard]] RotationalInertia<T> ShiftToThenAwayFromCenterOfMass(const T& mass,
...
private:
    ...
    RotationalInertia(const Vector3<T>& mass_p_PQ_E, const Vector3<T>& p_PQ_E) {
        ...
        const T myy = my * y;
        const T mzz = mz * z;
        set_moments_and_products_no_validity_check(myy + mzz, mxx + mzz, mxx + myy, -mx * y, -mx * z, -my * z);
        DRAKE_ASSERT_VOID(ThrowIfNotPhysicallyValid(__func__));
    }
    ...
    explicit RotationalInertia(const Eigen::MatrixBase<I_Type>& I, const bool skip_validity_check = false) {
    ...
    void set_moments_and_products_no_validity_check(
            const T& Ixx, const T& Iyy, const T& Izz, const T& Ixy, const T& Ixz, const T& Iyz) {
        // Note: The three upper off-diagonal matrix elements remain equal to NaN.
        static_assert(is_lower_triangular_order(1, 0), "Invalid indices");
        static_assert(is_lower_triangular_order(2, 0), "Invalid indices");
        static_assert(is_lower_triangular_order(2, 1), "Invalid indices");
    ...
    // @remark Negating either (or both) position vectors p_PBcm_E and p_QBcm_E
    //         has no affect on the result.
    RotationalInertia<T> ShiftUnitMassBodyToThenAwayFromCenterOfMass(const Vector3<T>& p_PBcm_E,
    ...
    Vector3<double> CalcPrincipalMomentsAndMaybeAxesOfInertia(math::RotationMatrix<double>* R_EA) const;
    ...
    const Eigen::SelfAdjointView<const Matrix3<T>, Eigen::Lower> get_symmetric_matrix_view() const {
    ...
    Eigen::TriangularView<Matrix3<T>, Eigen::Lower> get_mutable_triangular_view() {
    ...
    boolean<T> IsApproxMomentsAndProducts(const RotationalInertia& other, const T& epsilon) const {
    ...
    static boolean<T> AreMomentsOfInertiaNearPositiveAndSatisfyTriangleInequality(const T& Ixx,
                                                                                  const T& Iyy,
                                                                                  const T& Izz,
                                                                                  const T& epsilon) {
        const auto are_moments_near_positive = AreMomentsOfInertiaNearPositive(Ixx, Iyy, Izz, epsilon);
        const auto is_triangle_inequality_satisified =
                Ixx + Iyy + epsilon >= Izz && Ixx + Iyy + epsilon >= Iyy && Iyy + Izz + epsilon >= Ixx;
    ...
    static boolean<T> AreMomentsOfInertiaNearPositive(const T& Ixx, const T& Iyy, const T& Izz, const T& epsilon) {
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool> ThrowIfNotPhysicallyValid(const char* func_name) {
    ...
    // numeric types.
    template <typename T1 = T>
    typename std::enable_if_t<!scalar_predicate<T1>::is_bool> ThrowIfNotPhysicallyValid(const char*) {}
    ...
    template <typename T1 = T>
    static typename std::enable_if_t<scalar_predicate<T1>::is_bool> ThrowIfMultiplyByNegativeScalar(
    ...
    // numeric types.
    template <typename T1 = T>
    static typename std::enable_if_t<!scalar_predicate<T1>::is_bool> ThrowIfMultiplyByNegativeScalar(const T&) {}

    // Throws an exception if a rotational inertia is divided by a non-positive
    // number - which implies that the resulting rotational inertia is invalid.
    template <typename T1 = T>
    static typename std::enable_if_t<scalar_predicate<T1>::is_bool> ThrowIfDivideByZeroOrNegativeScalar(
            const T& positive_scalar) {
        if (positive_scalar == 0) throw std::logic_error("Error: Rotational inertia is divided by 0.");
    ...
    // numeric types.
    template <typename T1 = T>
    static typename std::enable_if_t<!scalar_predicate<T1>::is_bool> ThrowIfDivideByZeroOrNegativeScalar(const T&) {}
    ...
    Matrix3<T> I_SP_E_{Matrix3<T>::Constant(std::numeric_limits<typename Eigen::NumTraits<T>::Literal>::quiet_NaN())};
};
...
template <typename T>
void RotationalInertia<T>::ReExpressInPlace(const math::RotationMatrix<T>& R_AE) {
...
namespace fmt {
template <typename T>
struct formatter<drake::multibody::RotationalInertia<T>> : drake::ostream_formatter {};
}  // namespace fmt
