#include "multibody/tree/rpy_floating_mobilizer.h"
...
template <typename T>
std::string RpyFloatingMobilizer<T>::position_suffix(int position_index_in_mobilizer) const {
...
template <typename T>
std::string RpyFloatingMobilizer<T>::velocity_suffix(int velocity_index_in_mobilizer) const {
...
template <typename T>
Vector6<T> RpyFloatingMobilizer<T>::get_generalized_positions(const systems::Context<T>& context) const {
    return this->get_positions(context);
}

template <typename T>
Vector6<T> RpyFloatingMobilizer<T>::get_generalized_velocities(const systems::Context<T>& context) const {
    return this->get_velocities(context);
}

template <typename T>
Vector3<T> RpyFloatingMobilizer<T>::get_angles(const systems::Context<T>& context) const {
    return this->get_positions(context).template head<3>();
}

template <typename T>
Vector3<T> RpyFloatingMobilizer<T>::get_translation(const systems::Context<T>& context) const {
    return this->get_positions(context).template tail<3>();
}

template <typename T>
Vector3<T> RpyFloatingMobilizer<T>::get_angular_velocity(const systems::Context<T>& context) const {
    return this->get_velocities(context).template head<3>();
}

template <typename T>
Vector3<T> RpyFloatingMobilizer<T>::get_translational_velocity(const systems::Context<T>& context) const {
    return this->get_velocities(context).template tail<3>();
}

template <typename T>
const RpyFloatingMobilizer<T>& RpyFloatingMobilizer<T>::SetAngles(systems::Context<T>* context,
...
template <typename T>
const RpyFloatingMobilizer<T>& RpyFloatingMobilizer<T>::SetTranslation(systems::Context<T>* context,
...
template <typename T>
const RpyFloatingMobilizer<T>& RpyFloatingMobilizer<T>::SetAngularVelocity(systems::Context<T>* context,
...
template <typename T>
const RpyFloatingMobilizer<T>& RpyFloatingMobilizer<T>::SetTranslationalVelocity(systems::Context<T>* context,
...
template <typename T>
void RpyFloatingMobilizer<T>::set_random_angles_distribution(const Vector3<symbolic::Expression>& angles) {
...
template <typename T>
void RpyFloatingMobilizer<T>::set_random_translation_distribution(const Vector3<symbolic::Expression>& p_FM) {
    Vector<symbolic::Expression, 6> q;
    if (this->get_random_state_distribution()) {
        q = this->get_random_state_distribution()->template head<6>();
    } else {
...
template <typename T>
SpatialAcceleration<T> RpyFloatingMobilizer<T>::CalcAcrossMobilizerSpatialAcceleration(
...
template <typename T>
void RpyFloatingMobilizer<T>::ProjectSpatialForce(const systems::Context<T>&,
...
template <typename T>
void RpyFloatingMobilizer<T>::DoCalcNMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> N) const {
    using std::abs;
    using std::cos;
    using std::sin;
    ...
    if (abs(cp) < 1.0e-3) {
        throw std::runtime_error(
                fmt::format("The RpyFloatingMobilizer (implementing an RpyFloatingJoint) between "
...
template <typename T>
void RpyFloatingMobilizer<T>::DoCalcNplusMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> Nplus) const {
...
template <typename T>
void RpyFloatingMobilizer<T>::MapVelocityToQDot(const systems::Context<T>& context,
...
template <typename T>
void RpyFloatingMobilizer<T>::MapQDotToVelocity(const systems::Context<T>& context,
...
std::unique_ptr<Mobilizer<ToScalar>> RpyFloatingMobilizer<T>::TemplatedDoCloneToScalar(
    ...
    return std::make_unique<RpyFloatingMobilizer<ToScalar>>(inboard_frame_clone, outboard_frame_clone);
}
...
template <typename T>
std::unique_ptr<Mobilizer<symbolic::Expression>> RpyFloatingMobilizer<T>::DoCloneToScalar(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::RpyFloatingMobilizer);
