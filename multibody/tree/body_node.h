...
#include "multibody/math/spatial_algebra.h"
...
public:
    ...
    BodyNode(const BodyNode<T>* parent_node, const RigidBody<T>* body, const Mobilizer<T>* mobilizer)
        : MultibodyElement<T>(body->model_instance()), parent_node_(parent_node), body_(body), mobilizer_(mobilizer) {
    ...
    // node.
    int get_num_mobilizer_positions() const { return topology_.num_mobilizer_positions; }

    // Returns the number of generalized velocities for the Mobilizer in `this`
    // node.
    int get_num_mobilizer_velocities() const { return topology_.num_mobilizer_velocities; }
    ...
    // system.
    int velocity_start_in_v() const { return topology_.mobilizer_velocities_start_in_v; }
    //@}
    ...
    // in the tree.)
    void CalcPositionKinematicsCache_BaseToTip(const systems::Context<T>& context,
    ...
    void CalcVelocityKinematicsCache_BaseToTip(const systems::Context<T>& context,
    ...
    void CalcSpatialAcceleration_BaseToTip(const systems::Context<T>& context,
                                           const PositionKinematicsCache<T>& pc,
        ...
        //  kinematics.
        const math::RotationMatrix<T> R_PF = frame_F.CalcRotationMatrixInBodyFrame(context);
        ...
        SpatialAcceleration<T> A_FM = get_mobilizer().CalcAcrossMobilizerSpatialAcceleration(context, vmdot);
        ...
        if (vc != nullptr) {
            ...
            const SpatialAcceleration<T> A_PB_W = R_WF * A_FM.Shift(p_MB_F, V_FM.rotational());  // Eq. (4)

            // Velocities are non-zero.
            get_mutable_A_WB_from_array(&A_WB_array) =
                    A_WP.ComposeWithMovingFrameAcceleration(p_PB_W, V_WP.rotational(), V_PB_W, A_PB_W);
        } else {
            ...
            get_mutable_A_WB_from_array(&A_WB_array) = A_WP.ShiftWithZeroAngularVelocity(p_PB_W) + A_PB_W;
        }
    }
    ...
    void CalcInverseDynamics_TipToBase(const systems::Context<T>& context,
                                       const PositionKinematicsCache<T>& pc,
        ...
        DRAKE_DEMAND(tau_applied.size() == get_num_mobilizer_velocities() || tau_applied.size() == 0);
        ...
        CalcBodySpatialForceGivenItsSpatialAcceleration(M_B_W_cache, Fb_Bo_W_cache, A_WB, &Ftot_BBo_W);
        ...
        const math::RotationMatrix<T> R_PF = inboard_frame().CalcRotationMatrixInBodyFrame(context);
        const math::RotationMatrix<T>& R_WP = get_R_WP(pc);
    ...
    void CalcAcrossNodeJacobianWrtVExpressedInWorld(const systems::Context<T>& context,
        ...
        for (int imob = 0; imob < get_num_mobilizer_velocities(); ++imob) {
            ...
            const SpatialVelocity<T> Himob_FM = get_mobilizer().CalcAcrossMobilizerSpatialVelocity(context, v);
            v(imob) = 0.0;
    ...
    Eigen::Map<const MatrixUpTo6<T>> GetJacobianFromArray(const std::vector<Vector6<T>>& H_array) const {
        DRAKE_DEMAND(static_cast<int>(H_array.size()) == this->get_parent_tree().num_velocities());
        ...
        DRAKE_DEMAND(num_velocities == 0 || start_index_in_v < this->get_parent_tree().num_velocities());
        // The first column of this node's hinge matrix H_PB_W:
        const T* H_col0 = num_velocities == 0 ? nullptr : H_array[start_index_in_v].data();
    ...
    Eigen::Map<MatrixUpTo6<T>> GetMutableJacobianFromArray(std::vector<Vector6<T>>* H_array) const {
        ...
        DRAKE_DEMAND(num_velocities == 0 || start_index_in_v < this->get_parent_tree().num_velocities());
        // The first column of this node's hinge matrix H_PB_W:
        T* H_col0 = num_velocities == 0 ? nullptr : (*H_array)[start_index_in_v].data();
        // Create an Eigen map to the full H_PB_W for this node:
    ...
    void CalcArticulatedBodyInertiaCache_TipToBase(const systems::Context<T>& context,
                                                   const PositionKinematicsCache<T>& pc,
                                                   const Eigen::Ref<const MatrixUpTo6<T>>& H_PB_W,
        ...
        DRAKE_THROW_UNLESS(diagonal_inertias.size() == this->get_parent_tree().num_velocities());
        ...
        for (const BodyNode<T>* child : children_) {
            ...
            const ArticulatedBodyInertia<T>& Pplus_BC_W = child->get_Pplus_PB_W(*abic);
        ...
        if (nv != 0 && !this->mobilizer_->is_locked(context)) {
            ...
            D_B.diagonal() += diagonal_inertias.segment(this->velocity_start_in_v(), nv);

            // Compute the LLT factorization of D_B as llt_D_B.
            math::LinearSolver<Eigen::LLT, MatrixUpTo6<T>>& llt_D_B = get_mutable_llt_D_B(abic);
    ...
    // nullptr.
    void CalcArticulatedBodyForceCache_TipToBase(const systems::Context<T>& context,
                                                 const PositionKinematicsCache<T>& pc,
        ...
        for (const BodyNode<T>* child : children_) {
            ...
            const SpatialForce<T>& Zplus_BC_W = child->get_Zplus_PB_W(*aba_force_cache);
    ...
    void CalcArticulatedBodyAccelerations_BaseToTip(const systems::Context<T>& context,
                                                    const PositionKinematicsCache<T>& pc,
                                                    const ArticulatedBodyInertiaCache<T>& abic,
        ...
        const SpatialAcceleration<T> Aplus_WB =
                SpatialAcceleration<T>(A_WP.rotational(), A_WP.translational() + A_WP.rotational().cross(p_PoBo_W));
        ...
        if (this->mobilizer_->is_locked(context)) {
            ...
            // acceleration.
            const VectorUpTo6<T> nu_B = get_llt_D_B(abic).Solve(get_e_B(aba_force_cache));
    ...
    // in the tree.)
    void CalcCompositeBodyInertia_TipToBase(const SpatialInertia<T>& M_B_W,
                                            const PositionKinematicsCache<T>& pc,
    ...
    void CalcSpatialAccelerationBias(const systems::Context<T>& context,
        ...
        const VectorUpTo6<T> vmdot_zero = VectorUpTo6<T>::Zero(get_num_mobilizer_velocities());
        const SpatialAcceleration<T> Ab_FM =
                get_mobilizer().CalcAcrossMobilizerSpatialAcceleration(context, vmdot_zero);
        ...
        *Ab_WB = SpatialAcceleration<T>(w_WB.cross(w_PB_W) + Ab_PB_W.rotational(),
...
protected:
    ...
    // the _world_ body.
    const Frame<T>& inboard_frame() const { return get_mobilizer().inboard_frame(); }

    // Returns the outboard frame M of this node's mobilizer.
    // @throws std::exception if called on the root node corresponding to
    // the _world_ body.
    const Frame<T>& outboard_frame() const { return get_mobilizer().outboard_frame(); }
...
private:
    ...
    Eigen::VectorBlock<const VectorX<T>> get_mobilizer_velocities(const systems::Context<T>& context) const {
        return this->get_parent_tree().get_state_segment(context, topology_.mobilizer_velocities_start_in_state,
    ...
    // to the operator.
    Eigen::VectorBlock<const VectorX<T>> get_mobilizer_velocities(const VectorX<T>& v) const {
        return v.segment(topology_.mobilizer_velocities_start_in_v, topology_.num_mobilizer_velocities);
    }
    ...
    const math::RotationMatrix<T>& get_R_WP(const PositionKinematicsCache<T>& pc) const {
    ...
    const SpatialVelocity<T>& get_V_WB(const VelocityKinematicsCache<T>& vc) const {
    ...
    const SpatialVelocity<T>& get_V_WP(const VelocityKinematicsCache<T>& vc) const {
    ...
    const SpatialVelocity<T>& get_V_FM(const VelocityKinematicsCache<T>& vc) const {
    ...
    const SpatialVelocity<T>& get_V_PB_W(const VelocityKinematicsCache<T>& vc) const {
    ...
    const SpatialAcceleration<T>& get_A_WB(const AccelerationKinematicsCache<T>& ac) const {
        return ac.get_A_WB(topology_.index);
    }

    // Mutable version of get_A_WB().
    SpatialAcceleration<T>& get_mutable_A_WB(AccelerationKinematicsCache<T>* ac) const {
    ...
    const SpatialAcceleration<T>& get_A_WP(const AccelerationKinematicsCache<T>& ac) const {
    ...
    Eigen::Ref<VectorX<T>> get_mutable_accelerations(AccelerationKinematicsCache<T>* ac) const {
    ...
    const ArticulatedBodyInertia<T>& get_P_B_W(const ArticulatedBodyInertiaCache<T>& abic) const {
        return abic.get_P_B_W(topology_.index);
    }

    // Mutable version of get_P_B_W().
    ArticulatedBodyInertia<T>& get_mutable_P_B_W(ArticulatedBodyInertiaCache<T>* abic) const {
    ...
    const ArticulatedBodyInertia<T>& get_Pplus_PB_W(const ArticulatedBodyInertiaCache<T>& abic) const {
        return abic.get_Pplus_PB_W(topology_.index);
    }

    // Mutable version of get_Pplus_PB_W().
    ArticulatedBodyInertia<T>& get_mutable_Pplus_PB_W(ArticulatedBodyInertiaCache<T>* abic) const {
    ...
    math::LinearSolver<Eigen::LLT, MatrixUpTo6<T>>& get_mutable_llt_D_B(ArticulatedBodyInertiaCache<T>* abic) const {
    ...
    void CalcArticulatedBodyHingeInertiaMatrixFactorization(
            const MatrixUpTo6<T>& D_B, math::LinearSolver<Eigen::LLT, MatrixUpTo6<T>>* llt_D_B) const;

    // Returns a const reference to the Kalman gain `g_PB_W` of the body.
    const Matrix6xUpTo6<T>& get_g_PB_W(const ArticulatedBodyInertiaCache<T>& abic) const {
        return abic.get_g_PB_W(topology_.index);
    }

    // Mutable version of get_g_PB_W().
    Matrix6xUpTo6<T>& get_mutable_g_PB_W(ArticulatedBodyInertiaCache<T>* abic) const {
    ...
    // frame P.
    const SpatialForce<T>& get_Zplus_PB_W(const ArticulatedBodyForceCache<T>& aba_force_cache) const {
        return aba_force_cache.get_Zplus_PB_W(topology_.index);
    }

    // Mutable version of get_Zplus_PB_W().
    SpatialForce<T>& get_mutable_Zplus_PB_W(ArticulatedBodyForceCache<T>* aba_force_cache) const {
    ...
    const VectorUpTo6<T>& get_e_B(const ArticulatedBodyForceCache<T>& aba_force_cache) const {
        return aba_force_cache.get_e_B(topology_.index);
    }

    // Mutable version of get_e_B().
    VectorUpTo6<T>& get_mutable_e_B(ArticulatedBodyForceCache<T>* aba_force_cache) const {
    ...
    const SpatialAcceleration<T>& get_A_WB_from_array(const std::vector<SpatialAcceleration<T>>& A_WB_array) const {
        return A_WB_array[topology_.index];
    }

    // Mutable version of get_A_WB_from_array().
    SpatialAcceleration<T>& get_mutable_A_WB_from_array(std::vector<SpatialAcceleration<T>>* A_WB_array) const {
    ...
    // frame P in the parent node as measured and expressed in the world frame,
    // given an array of spatial accelerations for the entire MultibodyTree model.
    const SpatialAcceleration<T>& get_A_WP_from_array(const std::vector<SpatialAcceleration<T>>& A_WB_array) const {
        return A_WB_array[topology_.parent_body_node];
    }

    // Mutable version of get_A_WP_from_array().
    SpatialAcceleration<T>& get_mutable_A_WP_from_array(std::vector<SpatialAcceleration<T>>* A_WB_array) const {
        DRAKE_ASSERT(A_WB_array != nullptr);
        return (*A_WB_array)[topology_.parent_body_node];
    }
    ...
    // velocity (or time derivatives of the generalized velocities) is an argument
    // to the operator.
    Eigen::Ref<VectorX<T>> get_mutable_velocities_from_array(EigenPtr<VectorX<T>> v) const {
    ...
    // tree.
    Eigen::Ref<VectorX<T>> get_mutable_generalized_forces_from_array(EigenPtr<VectorX<T>> tau) const {
    ...
    void CalcAcrossMobilizerBodyPoses_BaseToTip(const systems::Context<T>& context,
                                                PositionKinematicsCache<T>* pc) const {
    ...
    void CalcAcrossMobilizerPositionKinematicsCache(const systems::Context<T>& context,
                                                    PositionKinematicsCache<T>* pc) const {
        DRAKE_ASSERT(pc != nullptr);
    ...
    //      computed.
    void CalcBodySpatialForceGivenItsSpatialAcceleration(const std::vector<SpatialInertia<T>>& M_B_W_cache,
                                                         const std::vector<SpatialForce<T>>* Fb_Bo_W_cache,
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::BodyNode);
