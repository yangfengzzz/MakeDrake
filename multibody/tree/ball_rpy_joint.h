...
public:
    ...
    void set_random_angles_distribution(const Vector3<symbolic::Expression>& angles) {
        get_mutable_mobilizer()->set_random_position_distribution(Vector3<symbolic::Expression>{angles});
    }
    ...
    Vector3<double> get_default_angles() const { return this->default_positions(); }

    /// Sets the default angles of this joint.
    /// @param[in] angles
    ///   The desired default angles of the joint
    void set_default_angles(const Vector3<double>& angles) { this->set_default_positions(angles); }
...
private:
    ...
    const internal::RpyBallMobilizer<T>* get_mobilizer() const {
        ...
        const internal::RpyBallMobilizer<T>* mobilizer =
                dynamic_cast<const internal::RpyBallMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::RpyBallMobilizer<T>* get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::RpyBallMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
