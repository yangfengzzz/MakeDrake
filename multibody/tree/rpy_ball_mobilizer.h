...
public:
    ...
    RpyBallMobilizer(const Frame<T>& inboard_frame_F, const Frame<T>& outboard_frame_M)
    ...
    const RpyBallMobilizer<T>& SetAngles(systems::Context<T>* context, const Vector3<T>& angles) const;
    ...
    // @returns a constant reference to this mobilizer.
    const RpyBallMobilizer<T>& SetFromRotationMatrix(systems::Context<T>* context,
    ...
    // @returns a constant reference to this mobilizer.
    const RpyBallMobilizer<T>& SetAngularVelocity(systems::Context<T>* context, const Vector3<T>& w_FM) const;
    ...
    //   in the inboard frame F, expressed in F.
    // @returns a constant reference to this mobilizer.
    const RpyBallMobilizer<T>& SetAngularVelocity(const systems::Context<T>& context,
    ...
    math::RigidTransform<T> CalcAcrossMobilizerTransform(const systems::Context<T>& context) const override;
    ...
    SpatialAcceleration<T> CalcAcrossMobilizerSpatialAcceleration(
            const systems::Context<T>& context, const Eigen::Ref<const VectorX<T>>& vdot) const override;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::RpyBallMobilizer);
