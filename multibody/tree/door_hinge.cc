...
template <typename T>
DoorHinge<T>::DoorHinge(const RevoluteJoint<T>& joint, const DoorHingeConfig& config)
    : DoorHinge(joint.model_instance(), joint.index(), config) {}

template <typename T>
DoorHinge<T>::DoorHinge(ModelInstanceIndex model_instance, JointIndex joint_index, const DoorHingeConfig& config)
    : ForceElement<T>(model_instance), joint_index_(joint_index), config_(config) {
...
const RevoluteJoint<T>& DoorHinge<T>::joint() const {
    const RevoluteJoint<T>* joint =
            dynamic_cast<const RevoluteJoint<T>*>(&this->get_parent_tree().get_joint(joint_index_));
    DRAKE_DEMAND(joint != nullptr);
    return *joint;
}

template <typename T>
T DoorHinge<T>::CalcPotentialEnergy(const Context<T>& context, const PositionKinematicsCache<T>&) const {
...
template <typename T>
T DoorHinge<T>::CalcNonConservativePower(const Context<T>& context,
                                         const PositionKinematicsCache<T>&,
                                         const VelocityKinematicsCache<T>&) const {
    const T& angular_rate = joint().get_angular_rate(context);
...
template <typename T>
void DoorHinge<T>::DoCalcAndAddForceContribution(const Context<T>& context,
                                                 const PositionKinematicsCache<T>&,
...
T DoorHinge<T>::CalcHingeFrictionalTorque(const T& angular_rate) const {
    if (config_.motion_threshold == 0) {
        ...
        const auto [s, singlet, doublet] = CalcApproximationCurves(T(config_.motion_threshold), angular_rate);
        drake::unused(singlet);
        return -(config_.dynamic_friction_torque * s + config_.static_friction_torque * doublet +
...
T DoorHinge<T>::CalcHingeSpringTorque(const T& angle) const {
    if (config_.catch_width == 0) {
        ...
        const auto [s, singlet, doublet] = CalcApproximationCurves(catch_center, angle - catch_center);
        drake::unused(s);
        drake::unused(singlet);
        return config_.catch_torque * doublet - config_.spring_constant * (angle - config_.spring_zero_angle_rad);
    }
}
...
template <typename T>
T DoorHinge<T>::CalcHingeConservativePower(const T& angle, const T& angular_rate) const {
...
T DoorHinge<T>::CalcHingeStoredEnergy(const T& angle) const {
    ...
    energy += 0.5 * config_.spring_constant * (spring_offset * spring_offset - spring_offset_init * spring_offset_init);

    // Compute catch torque energy.
    if (config_.catch_width != 0) {
        const T catch_center = config_.catch_width / 2;
        const auto [s_q, singlet_q, doublet_q] = CalcApproximationCurves(catch_center, angle - catch_center);
        drake::unused(s_q);
        drake::unused(doublet_q);

        const auto [s_0, singlet_0, doublet_0] = CalcApproximationCurves(catch_center, -catch_center);
        drake::unused(s_0);
...
std::unique_ptr<ForceElement<ToScalar>> DoorHinge<T>::TemplatedClone(const MultibodyTree<ToScalar>&) const {
...
template <typename T>
std::unique_ptr<ForceElement<double>> DoorHinge<T>::DoCloneToScalar(const MultibodyTree<double>& tree_clone) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::DoorHinge);
