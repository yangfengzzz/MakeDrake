...
std::unique_ptr<Joint<ToScalar>> RevoluteJoint<T>::TemplatedDoCloneToScalar(
    ...
    auto joint_clone = std::make_unique<RevoluteJoint<ToScalar>>(
            this->name(), frame_on_parent_body_clone, frame_on_child_body_clone, this->revolute_axis(),
            this->position_lower_limits()[0], this->position_upper_limit(), this->default_damping());
...
// in the header file.
template <typename T>
std::unique_ptr<typename Joint<T>::BluePrint> RevoluteJoint<T>::MakeImplementationBlueprint() const {
    auto blue_print = std::make_unique<typename Joint<T>::BluePrint>();
    auto revolute_mobilizer =
            std::make_unique<internal::RevoluteMobilizer<T>>(this->frame_on_parent(), this->frame_on_child(), axis_);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RevoluteJoint);
