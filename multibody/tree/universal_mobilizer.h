...
public:
    ...
    UniversalMobilizer(const Frame<T>& inboard_frame_F, const Frame<T>& outboard_frame_M)
    ...
    const UniversalMobilizer<T>& SetAngles(systems::Context<T>* context, const Vector2<T>& angles) const;
    ...
    //                       vector [ω₁, ω₂].
    // @returns a constant reference to `this` mobilizer.
    const UniversalMobilizer<T>& SetAngularRates(systems::Context<T>* context, const Vector2<T>& angles_dot) const;
...
protected:
    ...
    Eigen::Matrix<T, 3, 2> CalcHwMatrix(const systems::Context<T>& context, Vector3<T>* Hw_dot = nullptr) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::UniversalMobilizer);
