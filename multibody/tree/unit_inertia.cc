...
template <typename T>
UnitInertia<T>& UnitInertia<T>::SetFromRotationalInertia(const RotationalInertia<T>& I, const T& mass) {
...
template <typename T>
UnitInertia<T> UnitInertia<T>::SolidEllipsoid(const T& a, const T& b, const T& c) {
    const T a2 = a * a;
    const T b2 = b * b;
    const T c2 = c * c;
...
template <typename T>
UnitInertia<T> UnitInertia<T>::SolidCylinder(const T& radius, const T& length, const Vector3<T>& unit_vector) {
...
template <typename T>
UnitInertia<T> UnitInertia<T>::SolidCylinderAboutEnd(const T& radius, const T& length, const Vector3<T>& unit_vector) {
    DRAKE_THROW_UNLESS(radius >= 0);
    DRAKE_THROW_UNLESS(length >= 0);
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
    const T rsq = radius * radius;
...
UnitInertia<T> UnitInertia<T>::AxiallySymmetric(const T& moment_parallel,
    ...
    constexpr double two_plus_tiny = 2.0 + 32 * std::numeric_limits<double>::epsilon();
    ...
    const Matrix3<T> G_BBp_E = K * Matrix3<T>::Identity() + (J - K) * unit_vector * unit_vector.transpose();
    return UnitInertia<T>(G_BBp_E(0, 0), G_BBp_E(1, 1), G_BBp_E(2, 2), G_BBp_E(0, 1), G_BBp_E(0, 2), G_BBp_E(1, 2));
}

template <typename T>
UnitInertia<T> UnitInertia<T>::StraightLine(const T& moment_perpendicular, const Vector3<T>& unit_vector) {
...
template <typename T>
UnitInertia<T> UnitInertia<T>::ThinRod(const T& length, const Vector3<T>& unit_vector) {
...
UnitInertia<T> UnitInertia<T>::SolidBox(const T& Lx, const T& Ly, const T& Lz) {
    ...
    return UnitInertia(one_twelfth * (Ly2 + Lz2), one_twelfth * (Lx2 + Lz2), one_twelfth * (Lx2 + Ly2));
}

template <typename T>
UnitInertia<T> UnitInertia<T>::SolidCapsule(const T& radius, const T& length, const Vector3<T>& unit_vector) {
    DRAKE_THROW_UNLESS(radius >= 0);
    DRAKE_THROW_UNLESS(length >= 0);
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
...
template <typename T>
Matrix3<T> UpperTriangularOuterProduct(const Eigen::Ref<const Vector3<T>>& a, const Eigen::Ref<const Vector3<T>>& b) {
    Matrix3<T> M;
    M(0, 0) = a(0) * b(0);
    M(0, 1) = a(0) * b(1);
    M(0, 2) = a(0) * b(2);
...
template <typename T>
UnitInertia<T> UnitInertia<T>::SolidTetrahedronAboutPoint(const Vector3<T>& p0,
                                                          const Vector3<T>& p1,
                                                          const Vector3<T>& p2,
                                                          const Vector3<T>& p3) {
    ...
    const UnitInertia<T> G_BB0 = UnitInertia<T>::SolidTetrahedronAboutVertex(p_B0B1, p_B0B2, p_B0B3);
...
template <typename T>
UnitInertia<T> UnitInertia<T>::SolidTetrahedronAboutVertex(const Vector3<T>& p1,
                                                           const Vector3<T>& p2,
                                                           const Vector3<T>& p3) {
...
UnitInertia<T>::CalcPrincipalHalfLengthsAndAxesForEquivalentShape(double inertia_shape_factor) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::UnitInertia);
