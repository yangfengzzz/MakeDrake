...
template <typename T>
PrismaticSpring<T>::PrismaticSpring(const PrismaticJoint<T>& joint, double nominal_position, double stiffness)
    : PrismaticSpring(joint.model_instance(), joint.index(), nominal_position, stiffness) {}
...
const PrismaticJoint<T>& PrismaticSpring<T>::joint() const {
    const PrismaticJoint<T>* joint =
            dynamic_cast<const PrismaticJoint<T>*>(&this->get_parent_tree().get_joint(joint_index_));
    DRAKE_DEMAND(joint != nullptr);
    return *joint;
}

template <typename T>
void PrismaticSpring<T>::DoCalcAndAddForceContribution(const systems::Context<T>& context,
...
template <typename T>
T PrismaticSpring<T>::CalcPotentialEnergy(const systems::Context<T>& context,
...
template <typename T>
T PrismaticSpring<T>::CalcConservativePower(const systems::Context<T>& context,
                                            const internal::PositionKinematicsCache<T>&,
...
template <typename T>
T PrismaticSpring<T>::CalcNonConservativePower(const systems::Context<T>&,
                                               const internal::PositionKinematicsCache<T>&,
                                               const internal::VelocityKinematicsCache<T>&) const {
...
std::unique_ptr<ForceElement<ToScalar>> PrismaticSpring<T>::TemplatedDoCloneToScalar(
    ...
    std::unique_ptr<PrismaticSpring<ToScalar>> spring_clone(
            new PrismaticSpring<ToScalar>(this->model_instance(), joint_index_, nominal_position(), stiffness()));
    return spring_clone;
}
...
template <typename T>
std::unique_ptr<ForceElement<symbolic::Expression>> PrismaticSpring<T>::DoCloneToScalar(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::PrismaticSpring);
