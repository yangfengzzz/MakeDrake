...
#include "multibody/tree/parameter_conversion.h"
...
public:
    ...
    math::RigidTransform<T> CalcOffsetPoseInBody(const systems::Context<T>&,
                                                 const math::RigidTransform<T>& X_FQ) const override {
        return X_FQ;
    }

    math::RotationMatrix<T> CalcOffsetRotationMatrixInBody(const systems::Context<T>&,
                                                           const math::RotationMatrix<T>& R_FQ) const override {
        return R_FQ;
    }

    math::RigidTransform<T> GetFixedPoseInBodyFrame() const override { return math::RigidTransform<T>::Identity(); }
...
private:
    ...
    explicit RigidBodyFrame(const RigidBody<T>& body) : Frame<T>(body.name(), body) {}
...
private:
    ...
    // namespace.
    static RigidBodyFrame<T>& get_mutable_body_frame(RigidBody<T>* body) { return body->get_mutable_body_frame(); }
...
public:
    ...
    void Lock(systems::Context<T>* context) const {
        ...
        //  non-floating bodies.
        if (!is_floating()) {
            throw std::logic_error(fmt::format("Attempted to call Lock() on non-floating rigid body {}", name()));
        }
        this->get_parent_tree().get_mobilizer(topology_.inboard_mobilizer).Lock(context);
    }
    ...
    void Unlock(systems::Context<T>* context) const {
        ...
        //  non-floating bodies.
        if (!is_floating()) {
            throw std::logic_error(fmt::format("Attempted to call Unlock() on non-floating rigid body {}", name()));
        }
        this->get_parent_tree().get_mobilizer(topology_.inboard_mobilizer).Unlock(context);
    }
    ...
    bool is_locked(const systems::Context<T>& context) const {
        return this->get_parent_tree().get_mobilizer(topology_.inboard_mobilizer).is_locked(context);
    }
    ...
    const Vector3<double>& default_com() const { return default_spatial_inertia_.get_com(); }
    ...
    const UnitInertia<double>& default_unit_inertia() const { return default_spatial_inertia_.get_unit_inertia(); }
    ...
    const SpatialInertia<double>& default_spatial_inertia() const { return default_spatial_inertia_; }
    ...
    const SpatialVelocity<T>& EvalSpatialVelocityInWorld(const systems::Context<T>& context) const {
        return this->get_parent_tree().EvalBodySpatialVelocityInWorld(context, *this);
    }
    ...
    const SpatialAcceleration<T>& EvalSpatialAccelerationInWorld(const systems::Context<T>& context) const {
        return this->get_parent_tree().EvalBodySpatialAccelerationInWorld(context, *this);
    }
    ...
    const SpatialForce<T>& GetForceInWorld(const systems::Context<T>&, const MultibodyForces<T>& forces) const {
        DRAKE_THROW_UNLESS(forces.CheckHasRightSizeForModel(this->get_parent_tree()));
    ...
    const Vector3<T> CalcCenterOfMassInBodyFrame(const systems::Context<T>& context) const {
        const systems::BasicVector<T>& spatial_inertia_parameter =
                context.get_numeric_parameter(spatial_inertia_parameter_index_);
        return internal::parameter_conversion::GetCenterOfMass(spatial_inertia_parameter);
    }
    ...
    SpatialInertia<T> CalcSpatialInertiaInBodyFrame(const systems::Context<T>& context) const {
        ...
        const systems::BasicVector<T>& spatial_inertia_parameter =
                context.get_numeric_parameter(spatial_inertia_parameter_index_);
        return internal::parameter_conversion::ToSpatialInertia(spatial_inertia_parameter);
    }
    ...
    void SetMass(systems::Context<T>* context, const T& mass) const {
        ...
        spatial_inertia_parameter.SetAtIndex(internal::parameter_conversion::SpatialInertiaIndex::k_mass, mass);
    }
    ...
    void SetSpatialInertiaInBodyFrame(systems::Context<T>* context, const SpatialInertia<T>& M_Bo_B) const {
        ...
        systems::BasicVector<T>& spatial_inertia_parameter =
                context->get_mutable_numeric_parameter(spatial_inertia_parameter_index_);
        spatial_inertia_parameter.SetFrom(internal::parameter_conversion::ToBasicVector(M_Bo_B));
    }
...
private:
    ...
    void DoSetTopology(const internal::MultibodyTreeTopology& tree_topology) final {
    ...
    void DoDeclareParameters(internal::MultibodyTreeSystem<T>* tree_system) final {
        ...
        spatial_inertia_parameter_index_ = this->DeclareNumericParameter(tree_system, systems::BasicVector<T>(10));
    }
    ...
    void DoSetDefaultParameters(systems::Parameters<T>* parameters) const final {
        ...
        systems::BasicVector<T>& spatial_inertia_parameter =
                parameters->get_mutable_numeric_parameter(spatial_inertia_parameter_index_);
        spatial_inertia_parameter.SetFrom(
                internal::parameter_conversion::ToBasicVector<T>(default_spatial_inertia_.template cast<T>()));
    }
    ...
    void ThrowIfNotFinalized(const char* source_method) const {
        if (!this->get_parent_tree().topology_is_valid()) {
            throw std::runtime_error("From '" + std::string(source_method) +
    ...
    // @pre the context makes sense for use by this RigidBody.
    // @throws std::exception if context is null.
    void SetCenterOfMassInBodyFrameNoModifyInertia(systems::Context<T>* context,
                                                   const Vector3<T>& center_of_mass_position) const;
    ...
    // @pre the context makes sense for use by this RigidBody.
    // @throws std::exception if context is null.
    void SetUnitInertiaAboutBodyOrigin(systems::Context<T>* context, const UnitInertia<T>& G_BBo_B) const;
    ...
    std::unique_ptr<RigidBody<ToScalar>> TemplatedDoCloneToScalar(
            const internal::MultibodyTree<ToScalar>& tree_clone) const {
        unused(tree_clone);
        return std::make_unique<RigidBody<ToScalar>>(this->name(), default_spatial_inertia_);
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RigidBodyFrame);

DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RigidBody);
