...
template <typename T>
void ThrowUnlessValueIsPositiveFinite(const T& value, std::string_view value_name, std::string_view function_name) {
    if (!is_positive_finite(value)) {
        DRAKE_DEMAND(!value_name.empty());
        DRAKE_DEMAND(!function_name.empty());
        const std::string error_message =
                fmt::format("{}(): {} is not positive and finite: {}.", function_name, value_name, value);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::PointMass(const T& mass, const Vector3<T>& position) {
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidBoxWithDensity(const T& density, const T& lx, const T& ly, const T& lz) {
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidBoxWithMass(const T& mass, const T& lx, const T& ly, const T& lz) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(lx, "x-length", __func__);
    ThrowUnlessValueIsPositiveFinite(ly, "y-length", __func__);
    ThrowUnlessValueIsPositiveFinite(lz, "z-length", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCubeWithDensity(const T& density, const T& length) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCubeWithMass(const T& mass, const T& length) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(length, "length", __func__);
    const Vector3<T> p_BoBcm_B = Vector3<T>::Zero();
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCapsuleWithMass(const T& mass,
                                                          const T& radius,
                                                          const T& length,
                                                          const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ...
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
    const Vector3<T> p_BoBcm_B = Vector3<T>::Zero();
    const UnitInertia<T> G_BBo_B = UnitInertia<T>::SolidCapsule(radius, length, unit_vector);
    return SpatialInertia<T>(mass, p_BoBcm_B, G_BBo_B);
}
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCylinderWithMass(const T& mass,
                                                           const T& radius,
                                                           const T& length,
                                                           const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ...
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
    const Vector3<T> p_BoBcm_B = Vector3<T>::Zero();
    const UnitInertia<T> G_BBo_B = UnitInertia<T>::SolidCylinder(radius, length, unit_vector);
    return SpatialInertia<T>(mass, p_BoBcm_B, G_BBo_B);
}

template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCylinderWithDensityAboutEnd(const T& density,
                                                                      const T& radius,
                                                                      const T& length,
                                                                      const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidCylinderWithMassAboutEnd(const T& mass,
                                                                   const T& radius,
                                                                   const T& length,
                                                                   const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ...
    const UnitInertia<T> G_BBp_B = UnitInertia<T>::SolidCylinderAboutEnd(radius, length, unit_vector);
    return SpatialInertia(mass, p_BpBcm_B, G_BBp_B);  // Returns M_BBp_B.
}

template <typename T>
SpatialInertia<T> SpatialInertia<T>::ThinRodWithMass(const T& mass, const T& length, const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(length, "length", __func__);
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::ThinRodWithMassAboutEnd(const T& mass,
                                                             const T& length,
                                                             const Vector3<T>& unit_vector) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(length, "length", __func__);
    math::internal::ThrowIfNotUnitVector(unit_vector, __func__);
    SpatialInertia<T> M_BBcm_B = SpatialInertia<T>::ThinRodWithMass(mass, length, unit_vector);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidEllipsoidWithDensity(const T& density, const T& a, const T& b, const T& c) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidEllipsoidWithMass(const T& mass, const T& a, const T& b, const T& c) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(a, "semi-axis a", __func__);
    ThrowUnlessValueIsPositiveFinite(b, "semi-axis b", __func__);
    ThrowUnlessValueIsPositiveFinite(c, "semi-axis c", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidSphereWithDensity(const T& density, const T& radius) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
    ThrowUnlessValueIsPositiveFinite(radius, "radius", __func__);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidSphereWithMass(const T& mass, const T& radius) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(radius, "radius", __func__);
    const Vector3<T> p_BoBcm_B = Vector3<T>::Zero();
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::HollowSphereWithDensity(const T& area_density, const T& radius) {
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::HollowSphereWithMass(const T& mass, const T& radius) {
    ThrowUnlessValueIsPositiveFinite(mass, "mass", __func__);
    ThrowUnlessValueIsPositiveFinite(radius, "radius", __func__);
    const Vector3<T> p_BoBcm_B = Vector3<T>::Zero();
...
SpatialInertia<T> SpatialInertia<T>::SolidTetrahedronAboutPointWithDensity(
        const T& density, const Vector3<T>& p0, const Vector3<T>& p1, const Vector3<T>& p2, const Vector3<T>& p3) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
    ...
    SpatialInertia<T> M_BB0 =
            SpatialInertia<T>::SolidTetrahedronAboutVertexWithDensity(density, p_B0B1, p_B0B2, p_B0B3);
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::SolidTetrahedronAboutVertexWithDensity(const T& density,
                                                                            const Vector3<T>& p1,
                                                                            const Vector3<T>& p2,
                                                                            const Vector3<T>& p3) {
    ThrowUnlessValueIsPositiveFinite(density, "density", __func__);
    ...
    const UnitInertia<T> G_BBo = UnitInertia<T>::SolidTetrahedronAboutVertex(p1, p2, p3);
...
boolean<T> SpatialInertia<T>::IsPhysicallyValid() const {
    ...
    if (ret_value) {
        ...
        const RotationalInertia<T> I_SScm_E = G_SScm_E.MultiplyByScalarSkipValidityCheck(mass_);
...
void SpatialInertia<T>::ThrowNotPhysicallyValid() const {
    std::string error_message = fmt::format("Spatial inertia fails SpatialInertia::IsPhysicallyValid().");
...
template <typename T>
SpatialInertia<T>& SpatialInertia<T>::operator+=(const SpatialInertia<T>& M_BP_E) {
    const T total_mass = get_mass() + M_BP_E.get_mass();
    if (total_mass != 0) {
        p_PScm_E_ = (CalcComMoment() + M_BP_E.CalcComMoment()) / total_mass;
        G_SP_E_.SetFromRotationalInertia(CalcRotationalInertia() + M_BP_E.CalcRotationalInertia(), total_mass);
    } else {
        ...
        G_SP_E_.SetFromRotationalInertia(get_unit_inertia() + M_BP_E.get_unit_inertia(), 2.0);
    }
    mass_ = total_mass;
    return *this;
}
...
template <typename T>
void SpatialInertia<T>::ShiftFromCenterOfMassInPlace(const Vector3<T>& p_ScmP_E) {
...
template <typename T>
SpatialInertia<T> SpatialInertia<T>::ShiftFromCenterOfMass(const Vector3<T>& p_ScmP_E) const {
...
template <typename T>
SpatialForce<T> SpatialInertia<T>::operator*(const SpatialAcceleration<T>& A_WB_E) const {
...
template <typename T>
SpatialMomentum<T> SpatialInertia<T>::operator*(const SpatialVelocity<T>& V_WBp_E) const {
...
SpatialInertia<T>::CalcPrincipalHalfLengthsAndPoseForEquivalentShape(double inertia_shape_factor) const {
    ...
    const UnitInertia<T>& G_SScm_E = M_SScm_E.get_unit_inertia();
    const auto [abc, R_EA] = G_SScm_E.CalcPrincipalHalfLengthsAndAxesForEquivalentShape(inertia_shape_factor);
...
template <typename T>
void SpatialInertia<T>::WriteExtraCentralInertiaProperties(std::string* message) const {
    ...
    const RotationalInertia<T> I_BBcm = G_BBcm.MultiplyByScalarSkipValidityCheck(get_mass());
    ...
    if (!is_position_zero) {
        *message += fmt::format(" Inertia about center of mass, I_BBcm =\n{}", I_BBcm);
    }
...
std::ostream& operator<<(std::ostream& out, const SpatialInertia<T>& M) {
    ...
    const RotationalInertia<T> I_BP = G_BP.MultiplyByScalarSkipValidityCheck(mass);
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
        (static_cast<std::ostream& (*)(std::ostream&, const SpatialInertia<T>&)>(&operator<<)));

}  // namespace multibody
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::SpatialInertia);
