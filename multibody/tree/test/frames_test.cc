...
public:
    void SetUp() override {
        ...
        X_BP_ = RigidTransformd(AngleAxisd(M_PI / 6.0, Vector3d::UnitZ()) * AngleAxisd(M_PI / 5.0, Vector3d::UnitX()) *
                                Translation3d(0.0, -1.0, 0.0));
        // Frame P is rigidly attached to B with pose X_BP.
        frameP_ = &model->AddFrame<FixedOffsetFrame>("P", bodyB_->body_frame(), X_BP_);

        // Some arbitrary pose of frame Q in frame P.
        X_PQ_ = RigidTransformd(AngleAxisd(-M_PI / 3.0, Vector3d::UnitZ()) * AngleAxisd(M_PI / 7.0, Vector3d::UnitX()) *
        ...
        frameR_ = &model->AddFrame<FixedOffsetFrame>("R", *frameP_, math::RigidTransformd::Identity());

        // Frame S is arbitrary, but named and with a specific model instance.
        extra_instance_ = model->AddModelInstance("extra_instance");
        frameS_ = &model->AddFrame<FixedOffsetFrame>("S", model->world_frame(), math::RigidTransformd::Identity(),
                                                     extra_instance_);
        EXPECT_EQ(frameS_->scoped_name().get_full(), "extra_instance::S");
        // Ensure that the model instance propagates implicitly.
        frameSChild_ = &model->AddFrame<FixedOffsetFrame>("SChild", *frameS_, math::RigidTransformd::Identity());
        EXPECT_EQ(frameSChild_->scoped_name().get_full(), "extra_instance::SChild");

        // We are done adding modeling elements. Transfer tree to system and get
        // a Context.
        system_ = std::make_unique<internal::MultibodyTreeSystem<double>>(std::move(model));
        context_ = system_->CreateDefaultContext();

        // An arbitrary pose of an arbitrary frame G in an arbitrary frame F.
        X_FG_ = RigidTransformd(AngleAxisd(M_PI / 6.0, Vector3d::UnitY()) * AngleAxisd(M_PI / 8.0, Vector3d::UnitZ()) *
                                AngleAxisd(M_PI / 5.0, Vector3d::UnitX()) * Translation3d(2.0, -1.0, 0.5));
...
TEST_F(FrameTests, BodyFrameCalcPoseMethods) {
    ...
    EXPECT_TRUE(frameB_->CalcRotationMatrixInBodyFrame(*context_).IsExactlyIdentity());
    ...
    EXPECT_TRUE(frameB_->CalcOffsetPoseInBody(*context_, X_FG_).IsNearlyEqualTo(X_FG_, kEpsilon));
    const math::RotationMatrix<double>& R_FG = X_FG_.rotation();
    EXPECT_TRUE(frameB_->CalcOffsetRotationMatrixInBody(*context_, R_FG).IsNearlyEqualTo(R_FG, kEpsilon));
    ...
    EXPECT_TRUE(frameB_->GetFixedOffsetPoseInBody(X_FG_).IsNearlyEqualTo(X_FG_, kEpsilon));
    EXPECT_TRUE(frameB_->GetFixedRotationMatrixInBody(R_FG).IsNearlyEqualTo(R_FG, kEpsilon));
}
...
TEST_F(FrameTests, FixedOffsetFrameCalcPoseMethods) {
    ...
    EXPECT_TRUE(frameP_->CalcPoseInBodyFrame(*context_).IsNearlyEqualTo(X_BP_, kEpsilon));
    EXPECT_TRUE(frameP_->CalcRotationMatrixInBodyFrame(*context_).IsNearlyEqualTo(R_BP, kEpsilon));

    // Now verify the fixed pose and rotation matrix versions of those methods.
    EXPECT_TRUE(frameP_->GetFixedPoseInBodyFrame().IsNearlyEqualTo(X_BP_, kEpsilon));
    EXPECT_TRUE(frameP_->GetFixedRotationMatrixInBodyFrame().IsNearlyEqualTo(R_BP, kEpsilon));
    ...
    EXPECT_TRUE(frameP_->CalcOffsetPoseInBody(*context_, X_PQ_).IsNearlyEqualTo(X_BQ, kEpsilon));
    const math::RotationMatrix<double>& R_PQ = X_PQ_.rotation();
    const math::RotationMatrix<double>& R_BQ = X_BQ.rotation();
    EXPECT_TRUE(frameP_->CalcOffsetRotationMatrixInBody(*context_, R_PQ).IsNearlyEqualTo(R_BQ, kEpsilon));

    // Now verify the fixed pose and rotation matrix versions of those methods.
    EXPECT_TRUE(frameP_->GetFixedOffsetPoseInBody(X_PQ_).IsNearlyEqualTo(X_BQ, kEpsilon));
    EXPECT_TRUE(frameP_->GetFixedRotationMatrixInBody(R_PQ).IsNearlyEqualTo(R_BQ, kEpsilon));
}
...
TEST_F(FrameTests, ChainedFixedOffsetFrames) {
    ...
    const math::RigidTransform<double> X_BQ = X_BP_ * X_PQ_;
    const math::RotationMatrix<double> R_BQ = X_BQ.rotation();
    EXPECT_TRUE(frameQ_->CalcPoseInBodyFrame(*context_).IsNearlyEqualTo(X_BQ, kEpsilon));
    EXPECT_TRUE(frameQ_->CalcRotationMatrixInBodyFrame(*context_).IsNearlyEqualTo(R_BQ, kEpsilon));

    // Now verify the fixed pose and rotation matrix versions of those methods.
    EXPECT_TRUE(frameQ_->GetFixedPoseInBodyFrame().IsNearlyEqualTo(X_BQ, kEpsilon));
    EXPECT_TRUE(frameQ_->GetFixedRotationMatrixInBodyFrame().IsNearlyEqualTo(R_BQ, kEpsilon));
    ...
    EXPECT_TRUE(frameQ_->CalcOffsetPoseInBody(*context_, X_QG_).IsNearlyEqualTo(X_BG, kEpsilon));
    const math::RotationMatrix<double>& R_QG = X_QG_.rotation();
    const math::RotationMatrix<double>& R_BG = X_BG.rotation();
    EXPECT_TRUE(frameQ_->CalcOffsetRotationMatrixInBody(*context_, R_QG).IsNearlyEqualTo(R_BG, kEpsilon));

    // Now verify the fixed pose and rotation matrix versions of those methods.
    EXPECT_TRUE(frameQ_->GetFixedOffsetPoseInBody(X_QG_).IsNearlyEqualTo(X_BG, kEpsilon));
    EXPECT_TRUE(frameQ_->GetFixedRotationMatrixInBody(R_QG).IsNearlyEqualTo(R_BG, kEpsilon));
}
...
TEST_F(FrameTests, HasFrameNamed) {
    for (FrameIndex i{0}; i < tree().num_frames(); ++i) {
        ...
        EXPECT_TRUE(tree().HasFrameNamed(frame.name(), frame.model_instance())) << frame.name();
    }
}
