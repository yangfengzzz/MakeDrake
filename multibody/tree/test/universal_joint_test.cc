...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<UniversalJoint>("Joint", model->world_body(), std::nullopt, *body_, std::nullopt,
                                                  kDamping);
        mutable_joint_ = dynamic_cast<UniversalJoint<double>*>(&model->get_mutable_joint(joint_->index()));
        DRAKE_DEMAND(mutable_joint_ != nullptr);
        mutable_joint_->set_position_limits(Vector2d::Constant(kPositionLowerLimit),
                                            Vector2d::Constant(kPositionUpperLimit));
        mutable_joint_->set_velocity_limits(Vector2d::Constant(kVelocityLowerLimit),
                                            Vector2d::Constant(kVelocityUpperLimit));
        mutable_joint_->set_acceleration_limits(Vector2d::Constant(kAccelerationLowerLimit),
...
TEST_F(UniversalJointTest, GetJointLimits) {
    ...
    EXPECT_EQ(joint_->position_lower_limits(), Vector2d::Constant(kPositionLowerLimit));
    EXPECT_EQ(joint_->position_upper_limits(), Vector2d::Constant(kPositionUpperLimit));
    EXPECT_EQ(joint_->velocity_lower_limits(), Vector2d::Constant(kVelocityLowerLimit));
    EXPECT_EQ(joint_->velocity_upper_limits(), Vector2d::Constant(kVelocityUpperLimit));
    EXPECT_EQ(joint_->acceleration_lower_limits(), Vector2d::Constant(kAccelerationLowerLimit));
    EXPECT_EQ(joint_->acceleration_upper_limits(), Vector2d::Constant(kAccelerationUpperLimit));
}
...
TEST_F(UniversalJointTest, ContextDependentAccess) {
    ...
    EXPECT_THROW(joint_->SetDampingVector(context_.get(), Vector2d::Constant(-1)), std::exception);
}
...
TEST_F(UniversalJointTest, AddInOneForce) {
    ...
    for (auto& F1 : forces1.body_forces()) EXPECT_TRUE(F1.IsApprox(*F2++, kEpsilon));
}
...
TEST_F(UniversalJointTest, SetVelocityAndAccelerationLimits) {
    ...
    mutable_joint_->set_velocity_limits(Vector2d::Constant(new_lower), Vector2d::Constant(new_upper));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_velocity_limits(VectorX<double>(2), VectorX<double>()),
                                ".* 'lower_limits.size\\(\\) == upper_limits.size\\(\\)' failed.");
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_velocity_limits(VectorX<double>(), VectorX<double>(2)),
                                ".* 'lower_limits.size\\(\\) == upper_limits.size\\(\\)' failed.");
    // Lower limit is larger than upper limit.
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_velocity_limits(Vector2d::Constant(2), Vector2d::Constant(0)),
                                ".* '\\(lower_limits.array\\(\\) <= "
                                "upper_limits.array\\(\\)\\).all\\(\\)' failed.");

    // Check for acceleration limits.
    mutable_joint_->set_acceleration_limits(Vector2d::Constant(new_lower), Vector2d::Constant(new_upper));
    ...
    // Does not match num_velocities().
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_acceleration_limits(VectorX<double>(2), VectorX<double>()),
                                ".* 'lower_limits.size\\(\\) == upper_limits.size\\(\\)' failed.");
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_acceleration_limits(VectorX<double>(), VectorX<double>(2)),
                                ".* 'lower_limits.size\\(\\) == upper_limits.size\\(\\)' failed.");
    // Lower limit is larger than upper limit.
    DRAKE_EXPECT_THROWS_MESSAGE(mutable_joint_->set_acceleration_limits(Vector2d::Constant(2), Vector2d::Constant(0)),
                                ".* '\\(lower_limits.array\\(\\) <= "
                                "upper_limits.array\\(\\)\\).all\\(\\)' failed.");
}
...
TEST_F(UniversalJointTest, DefaultAngles) {
    ...
    const Vector2d new_default_angles = Vector2d::Constant(kPositionNonZeroDefault);

    const Vector2d out_of_bounds_low_angles = lower_limit_angles - Vector2d::Constant(1);
    const Vector2d out_of_bounds_high_angles = upper_limit_angles + Vector2d::Constant(1);
