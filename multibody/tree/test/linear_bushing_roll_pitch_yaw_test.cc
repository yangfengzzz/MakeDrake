...
public:
    ...
    static void VerifyHalfAngleAxisAlgorithm(double angle, const Vector3<double>& axis) {
        ...
        DRAKE_EXPECT_NO_THROW(LinearBushingRollPitchYaw<double>::ThrowIfInvalidHalfAngleAxis(R_AC, R_AB_expected));
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(LinearBushingRollPitchYaw<double>::ThrowIfInvalidHalfAngleAxis(R_AC, R_AB_bad),
...
public:
    void SetUp() override {
        ...
        const RotationalInertia<double> I_CCcm_C(Ixx_, Iyy_, Izz_, Ixy_, Ixz_, Iyz_);
        const SpatialInertia<double> M_CCo_C = SpatialInertia<double>::MakeFromCentralInertia(mC_, p_CoCcm_, I_CCcm_C);
        ...
        joint_ = &model->AddJoint(std::make_unique<QuaternionFloatingJoint<double>>("joint0", frameA, frameC));
        ...
        CalcReasonableStiffnessAndDampingConstants(&torque_stiffness_constants, &torque_damping_constants,
        ...
        bushing_ = &(model->AddForceElement<LinearBushingRollPitchYaw>(
                frameA, frameC, torque_stiffness_constants, torque_damping_constants, force_stiffness_constants,
        ...
        // TODO(Mitiguy) Per issue #12739, consider using MultibodyPlant not MBT.
        model->Finalize();
        mbtree_system_ = std::make_unique<MultibodyTreeSystem<double>>(std::move(model));
    ...
    void CompareToMotionGenesisResult(const math::RollPitchYaw<double>& rpy,
        ...
        const Vector3<double> qDt = rpy.CalcRpyDtFromAngularVelocityInParent(w_AC_A);
        ...
        const Vector3<double> tau(-(k0 * q0 + d0 * qDt(0)), -(k1 * q1 + d1 * qDt(1)), -(k2 * q2 + d2 * qDt(2)));
        ...
        const Matrix3<double> N = rpy.CalcMatrixRelatingRpyDtToAngularVelocityInParent();
        ...
        const Vector3<double> t_Ao_A_expected = -txyz + R_AB * p_AoBo_B.cross(f_A_B_expected);

        // Calculate the moment on frame C about Co, expressed in A.
        const Vector3<double> p_CoBo_B = -p_AoBo_B;
        const Vector3<double> t_Co_C_expected = R_CA * txyz + R_CB * p_CoBo_B.cross(f_C_B_expected);
        ...
        const SpatialForce<double> F_A_A = bushing_->CalcBushingSpatialForceOnFrameA(context);
        EXPECT_TRUE(CompareMatrices(F_A_A.rotational(), t_Ao_A_expected, torque_epsilon, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(F_A_A.translational(), f_A_A_expected, force_epsilon, MatrixCompareType::relative));

        // Verify the bushing's spatial force (torque and force) calculation at Cₒ.
        const SpatialForce<double> F_C_C = bushing_->CalcBushingSpatialForceOnFrameC(context);
        const Vector3<double>& t_Co_C = F_C_C.rotational();
        const Vector3<double>& f_C_C = F_C_C.translational();
        EXPECT_TRUE(CompareMatrices(t_Co_C, t_Co_C_expected, torque_epsilon, MatrixCompareType::relative));
        EXPECT_TRUE(CompareMatrices(f_C_C, f_C_C_expected, force_epsilon, MatrixCompareType::relative));

        // If available, also verify the previous results with by-hand calculations.
        if (t_Co_C_expected_simple != nullptr) {
            EXPECT_TRUE(CompareMatrices(t_Co_C, *t_Co_C_expected_simple, torque_epsilon, MatrixCompareType::relative));
        }
        if (f_C_C_expected_simple != nullptr) {
            EXPECT_TRUE(CompareMatrices(f_C_C, *f_C_C_expected_simple, force_epsilon, MatrixCompareType::relative));
        }

        // Verify potential energy and power methods throw exceptions.
        const MultibodyTree<double>& mbt = GetInternalTree(*mbtree_system_);
        const PositionKinematicsCache<double>& pc = mbt.EvalPositionKinematics(context);
        const VelocityKinematicsCache<double>& vc = mbt.EvalVelocityKinematics(context);

        // Verify CalcPotentialEnergy() throws an exception (see issue #12982).
        const ForceElement<double>* bushing_force_element = bushing_;
        DRAKE_EXPECT_THROWS_MESSAGE(bushing_force_element->CalcPotentialEnergy(context, pc),
                                    "Error: LinearBushingRollPitchYaw::CalcPotentialEnergy().*");

        // Verify CalcConservativePower() throws an exception (see issue #12982).
        DRAKE_EXPECT_THROWS_MESSAGE(bushing_force_element->CalcConservativePower(context, pc, vc),
                                    "Error: LinearBushingRollPitchYaw::CalcConservativePower().*");

        // Verify CalcConservativePower() throws an exception (see issue #12982).
        DRAKE_EXPECT_THROWS_MESSAGE(bushing_force_element->CalcNonConservativePower(context, pc, vc),
    ...
    void CalcReasonableStiffnessAndDampingConstants(Vector3<double>* torque_stiffness_constants, /* [k₀, k₁, k₂] */
...
protected:
    ...
    const Vector3<double>& K012() const { return bushing_->torque_stiffness_constants(); }
    const Vector3<double>& D012() const { return bushing_->torque_damping_constants(); }
    const Vector3<double>& Kxyz() const { return bushing_->force_stiffness_constants(); }
    const Vector3<double>& Dxyz() const { return bushing_->force_damping_constants(); }
};
...
TEST_F(LinearBushingRollPitchYawTester, ConstructionAndAccessors) {
    ...
    EXPECT_TRUE(CompareMatrices(torque_stiffness_constants, bushing_->torque_stiffness_constants(), tolerance,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(torque_damping_constants, bushing_->torque_damping_constants(), tolerance,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(force_stiffness_constants, bushing_->force_stiffness_constants(), tolerance,
                                MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(force_damping_constants, bushing_->force_damping_constants(), tolerance,
                                MatrixCompareType::relative));
}
...
TEST_F(LinearBushingRollPitchYawTester, VariousOrientationsAtRest) {
    ...
    CompareToMotionGenesisResult(rpy, p_zero, w_zero, v_zero, &f_C_C_expected, &t_Co_C_expected);
...
TEST_F(LinearBushingRollPitchYawTester, VariousPositionsAtRest) {
    ...
    Vector3<double> f_C_C_expected = Vector3<double>(0, 0, 0);
    Vector3<double> t_Co_C_expected = Vector3<double>(0, 0, 0);
    CompareToMotionGenesisResult(rpy_zero, p_AoCo_A, w_zero, v_zero, &f_C_C_expected, &t_Co_C_expected);
...
TEST_F(LinearBushingRollPitchYawTester, IdentityPoseVariousRotationRates) {
    ...
    Vector3<double> f_C_C_expected = Vector3<double>(0, 0, 0);
    Vector3<double> t_Co_C_expected = Vector3<double>(0, 0, 0);
    CompareToMotionGenesisResult(rpy_zero, p_zero, w_AC_A, v_zero, &f_C_C_expected, &t_Co_C_expected);
...
TEST_F(LinearBushingRollPitchYawTester, IdentityPoseWithTranslationalRates) {
    ...
    CompareToMotionGenesisResult(rpy_zero, p_zero, w_zero, v_ACo_A, &f_C_C_expected, &t_Co_C_expected);
...
TEST_F(LinearBushingRollPitchYawTester, TestGimbalLock) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(bushing_->CalcBushingSpatialForceOnFrameA(context), expected_message);

    // Check that CalcBushingSpatialForceOnFrameC() throws near gimbal lock.
    DRAKE_EXPECT_THROWS_MESSAGE(bushing_->CalcBushingSpatialForceOnFrameC(context), expected_message);
}
...
GTEST_TEST(LinearBushingRollPitchYawTest, BushingParameters) {
    ...
    const RigidBody<double>& sphere1 =
            plant.AddRigidBody("sphere1", SpatialInertia<double>::SolidSphereWithMass(sphere_mass, sphere_radius));

    const RigidBody<double>& sphere2 =
            plant.AddRigidBody("sphere2", SpatialInertia<double>::SolidSphereWithMass(sphere_mass, sphere_radius));
    ...
    const LinearBushingRollPitchYaw<double>& bushing = plant.AddForceElement<LinearBushingRollPitchYaw>(
            sphere1.body_frame(), sphere2.body_frame(), torque_stiffness, torque_damping, force_stiffness,
            force_damping);
    ...
    const Vector3<double> default_torque_stiffness = bushing.GetTorqueStiffnessConstants(*context);
    const Vector3<double> default_torque_damping = bushing.GetTorqueDampingConstants(*context);
    const Vector3<double> default_force_stiffness = bushing.GetForceStiffnessConstants(*context);
    const Vector3<double> default_force_damping = bushing.GetForceDampingConstants(*context);
    ...
    const Vector3<double> new_default_torque_stiffness = bushing.GetTorqueStiffnessConstants(*context);
    const Vector3<double> new_default_torque_damping = bushing.GetTorqueDampingConstants(*context);
    const Vector3<double> new_default_force_stiffness = bushing.GetForceStiffnessConstants(*context);
    const Vector3<double> new_default_force_damping = bushing.GetForceDampingConstants(*context);

    EXPECT_TRUE(CompareMatrices(new_torque_stiffness, new_default_torque_stiffness));
    EXPECT_TRUE(CompareMatrices(new_torque_damping, new_default_torque_damping));
    EXPECT_TRUE(CompareMatrices(new_force_stiffness, new_default_force_stiffness));
