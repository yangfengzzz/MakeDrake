...
public:
    ...
    void SetUp() override {
        ...
        model_.AddJoint<RevoluteJoint>("Joint1", model_.world_body(), std::nullopt, body1, std::nullopt,
                                       Vector3d::UnitZ());
        model_.AddJoint<RevoluteJoint>("Joint2", body1, std::nullopt, body2, std::nullopt, Vector3d::UnitZ());
        model_.Finalize();
    }
...
TEST_F(MultibodyForcesTests, NonZeroForces) {
    ...
    forces1.mutable_body_forces()[1] = SpatialForce<double>(Vector3d(0, 1, 2), Vector3d(3, 4, 5));
    forces1.mutable_body_forces()[2] = SpatialForce<double>(Vector3d(0, 1, 2), Vector3d(3, 4, 5));
    ...
    forces2.mutable_body_forces()[0] = SpatialForce<double>(Vector3d(6, 7, 8), Vector3d(9, 10, 11));
    forces2.mutable_body_forces()[2] = SpatialForce<double>(Vector3d(6, 7, 8), Vector3d(9, 10, 11));
    ...
    const std::vector<SpatialForce<double>>& spatial_forces = forces1.body_forces();

    // Check the results:
    EXPECT_EQ(generalized_forces, Vector2d(4, 6));
    EXPECT_TRUE(CompareMatrices(spatial_forces[0].get_coeffs(), (Vector6<double>() << 6, 7, 8, 9, 10, 11).finished(),
                                kEpsilon, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(spatial_forces[1].get_coeffs(), (Vector6<double>() << 0, 1, 2, 3, 4, 5).finished(),
                                kEpsilon, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(spatial_forces[2].get_coeffs(), (Vector6<double>() << 6, 8, 10, 12, 14, 16).finished(),
                                kEpsilon, MatrixCompareType::absolute));
