...
GTEST_TEST(ArticulatedBodyInertiaAlgorithm, FeatherstoneExample) {
    ...
    const SpatialInertia<double> M_Bcm = SpatialInertia<double>::SolidBoxWithMass(mass_box, Lx, Ly, Lz);
    ...
    const SpatialInertia<double> M_Ccm =
            SpatialInertia<double>::SolidCylinderWithMass(mass_cylinder, r, L, Vector3d::UnitX());
    ...
    const RigidBody<double>& massless_link = tree.AddRigidBody("massless", SpatialInertia<double>::Zero());
    tree.AddJoint<RevoluteJoint>("revolute", box_link, {}, massless_link, {}, Vector3d(1, 0, 0));

    // Add cylinder body and let it translate along y.
    const RigidBody<double>& cylinder_link = tree.AddRigidBody("cylinder", M_Ccm);
    tree.AddJoint<PrismaticJoint>("prismatic", massless_link, {}, cylinder_link, {}, Vector3d(0, 1, 0));
    ...
    // Compare results.
    const ArticulatedBodyInertia<double>& Pplus_C_W_actual = abc.get_Pplus_PB_W(cylinder_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_C_W_expected_mat, Pplus_C_W_actual.CopyToFullMatrix6(), kEpsilon));

    Matrix6<double> Pplus_M_W_expected_mat(Pplus_C_W_expected_mat);
    Pplus_M_W_expected_mat(0, 0) = 0.0;  // x inertia projected out

    const ArticulatedBodyInertia<double>& Pplus_M_W_actual = abc.get_Pplus_PB_W(massless_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_M_W_expected_mat, Pplus_M_W_actual.CopyToFullMatrix6(), kEpsilon));
    ...
    // Compare results.
    const ArticulatedBodyInertia<double>& P_B_W_actual = abc.get_Pplus_PB_W(box_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_B_W_expected_mat, P_B_W_actual.CopyToFullMatrix6(), kEpsilon));
}
...
template <int dofs>
Matrix6<double> Project(const Matrix6<double>& P, const Eigen::Matrix<double, 6, dofs>& H) {
...
GTEST_TEST(ArticulatedBodyInertiaAlgorithm, ModifiedFeatherstoneExample) {
    ...
    // Add box body and gimbal (BallRpy) joint.
    const RigidBody<double>& box_link = tree.AddRigidBody("box", M_Bcm);
    const auto& WB_joint = tree.AddJoint<BallRpyJoint>("ball", tree.world_body(), {}, box_link, {});

    // Add a massless body that can rotate about x.
    const RigidBody<double>& massless_link = tree.AddRigidBody("massless", SpatialInertia<double>::Zero());
    const auto& BM_joint = tree.AddJoint<RevoluteJoint>("revolute", box_link, {}, massless_link, {}, Vector3d(1, 0, 0));
    ...
    Matrix6<double> Pplus_C_W_expected_mat = R6_WC * M_Ccm_proj * R6_WC.transpose();  // Re-express in World.

    // Compare results for the cylinder.
    const ArticulatedBodyInertia<double>& Pplus_C_W_expected = ArticulatedBodyInertia<double>(Pplus_C_W_expected_mat);
    const ArticulatedBodyInertia<double>& Pplus_C_W_actual = abc.get_Pplus_PB_W(cylinder_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_C_W_expected_mat, Pplus_C_W_actual.CopyToFullMatrix6(), kEpsilon));
    ...
    Matrix6<double> P_M_W_expected_mat(Pplus_C_W_expected.Shift(R_WC * Vector3d(0.0, -0.2, 0.0)).CopyToFullMatrix6());

    // Verify that we get the right P_M.
    const ArticulatedBodyInertia<double>& P_M_W_actual = abc.get_P_B_W(massless_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(P_M_W_expected_mat, P_M_W_actual.CopyToFullMatrix6(), kEpsilon));
    ...
    const ArticulatedBodyInertia<double> Pplus_M_W_expected(Pplus_M_W_expected_mat);
    const ArticulatedBodyInertia<double>& Pplus_M_W_actual = abc.get_Pplus_PB_W(massless_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_M_W_expected_mat, Pplus_M_W_actual.CopyToFullMatrix6(), kEpsilon));
    ...
    const Matrix6<double> P_B_W_expected_mat = Pplus_M_W_expected_mat + M_Bcm.ReExpress(R_WB).CopyToFullMatrix6();
    ...
    // Compare results.
    const ArticulatedBodyInertia<double>& Pplus_B_W_actual = abc.get_Pplus_PB_W(box_link.mobod_index());
    EXPECT_TRUE(CompareMatrices(Pplus_B_W_expected_mat, Pplus_B_W_actual.CopyToFullMatrix6(), kEpsilon));
}
