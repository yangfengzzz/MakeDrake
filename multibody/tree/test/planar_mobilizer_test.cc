...
public:
    ...
    void SetUp() override {
        mobilizer_ = &AddJointAndFinalize<PlanarJoint, PlanarMobilizer>(std::make_unique<PlanarJoint<double>>(
                "joint0", tree().world_body().body_frame(), body_->body_frame(), Vector3d::Zero()));
    }
...
TEST_F(PlanarMobilizerTest, DefaultPosition) {
    PlanarMobilizer<double>* mutable_mobilizer = &mutable_tree().get_mutable_variant(*mobilizer_);
...
TEST_F(PlanarMobilizerTest, RandomState) {
    ...
    mutable_mobilizer->set_random_position_distribution(Vector3<symbolic::Expression>(
            uniform(generator) + 1.0, uniform(generator) + 2.0, uniform(generator) + 3.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    ...
    mutable_mobilizer->set_random_velocity_distribution(Vector3<symbolic::Expression>(
            uniform(generator) - 2.0, uniform(generator) - 3.0, uniform(generator) - 4.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_translations(*context_)[0], 1.0);
    EXPECT_LE(mobilizer_->get_translations(*context_)[0], 2.0);
    EXPECT_GE(mobilizer_->get_translations(*context_)[1], 2.0);
    ...
    const Vector2d last_translational_rates = mobilizer_->get_translation_rates(*context_);
    ...
    EXPECT_NE(mobilizer_->get_translation_rates(*context_), last_translational_rates);
...
TEST_F(PlanarMobilizerTest, CalcAcrossMobilizerTransform) {
    ...
    const RigidTransformd X_FM_expected(RotationMatrixd::MakeZRotation(angle), X_FM_translation);
...
TEST_F(PlanarMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    mobilizer_->set_translations(context_.get(), translations);
    mobilizer_->SetAngle(context_.get(), angle);
    const SpatialVelocity<double> V_FM = mobilizer_->CalcAcrossMobilizerSpatialVelocity(*context_, velocity);
...
TEST_F(PlanarMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            mobilizer_->CalcAcrossMobilizerSpatialAcceleration(*context_, acceleration);

    VectorXd a_expected(6);
    a_expected << 0.0, 0.0, acceleration[2], acceleration[0], acceleration[1], 0.0;
