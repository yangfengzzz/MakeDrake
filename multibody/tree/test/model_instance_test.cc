...
GTEST_TEST(ModelInstance, MissingName) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(internal::ModelInstance<double>(ModelInstanceIndex{1}, empty_name), ".*name.*empty.*");
}
...
GTEST_TEST(ModelInstance, ModelInstanceTest) {
    ...
    const RigidBody<double>& body1 = tree.AddRigidBody("Body1", instance1, SpatialInertia<double>::NaN());
    const RigidBody<double>& body2 = tree.AddRigidBody("Body2", instance1, SpatialInertia<double>::NaN());
    const RigidBody<double>& body3 = tree.AddRigidBody("Body3", instance1, SpatialInertia<double>::NaN());

    const auto& weld1 = tree.AddJoint<WeldJoint>("weld1", tree.world_body(), math::RigidTransformd::Identity(), body1,
    ...
    const Joint<double>& body1_body2 = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism1", body1.body_frame(), body2.body_frame(), Eigen::Vector3d(0, 0, 1)));
    tree.AddJointActuator("act1", body1_body2);

    const Joint<double>& body2_body3 =
            tree.AddJoint<PrismaticJoint>("prism2", body2, math::RigidTransformd::Identity(), body3,
    ...
    const RigidBody<double>& body4 = tree.AddRigidBody("Body4", instance2, SpatialInertia<double>::NaN());
    const RigidBody<double>& body5 = tree.AddRigidBody("Body5", instance2, SpatialInertia<double>::NaN());

    const Joint<double>& body4_body5 =
            tree.AddJoint<PrismaticJoint>("prism3", body4, math::RigidTransformd::Identity(), body5,
                                          math::RigidTransformd::Identity(), Eigen::Vector3d(0, 0, 1));
    ...
    EXPECT_TRUE(CompareMatrices(tree.GetActuationFromArray(instance1, act_vector), Eigen::Vector2d(1, 2)));
    EXPECT_TRUE(CompareMatrices(tree.GetActuationFromArray(instance2, act_vector), Vector1d(3)));
    ...
    Eigen::VectorXd instance1_pos = tree.GetPositionsFromArray(instance1, pos_vector);
    ...
    Eigen::VectorXd instance2_pos = tree.GetPositionsFromArray(instance2, pos_vector);
    ...
    Eigen::VectorXd instance1_vel = tree.GetVelocitiesFromArray(instance1, vel_vector);
    ...
    Eigen::VectorXd instance2_vel = tree.GetVelocitiesFromArray(instance2, vel_vector);
    ...
    std::unique_ptr<systems::Context<double>> context = mb_system.CreateDefaultContext();
    ...
    Eigen::VectorXd instance1_x(tree.num_positions(instance1) + tree.num_velocities(instance1));
    ...
    const Eigen::VectorXd instance1_pos_from_array = tree.GetPositionsFromArray(instance1, pos_vector);
    const Eigen::VectorXd instance1_vel_from_array = tree.GetVelocitiesFromArray(instance1, vel_vector);
    ...
    std::unique_ptr<internal::MultibodyTree<AutoDiffXd>> tree_ad = tree.CloneToScalar<AutoDiffXd>();
...
GTEST_TEST(ModelInstance, ModelInstanceRenameTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(tree.RenameModelInstance(ModelInstanceIndex(99), "invalid"),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(tree.RenameModelInstance(model1, "after"), ".*names must be unique.*");

    tree.Finalize();

    DRAKE_EXPECT_THROWS_MESSAGE(tree.RenameModelInstance(model0, "too_late"), ".*is finalized already.*");
}
