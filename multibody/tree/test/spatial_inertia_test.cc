...
#include "multibody/tree/rotational_inertia.h"
#include "multibody/tree/unit_inertia.h"
...
SpatialInertia<double> MakeArbitrarySpatialInertia() {
    ...
    const RotationalInertia<double> I_Bcm_W = mass * UnitInertia<double>::SolidCylinder(radius, length, axis_W);

    // Create the spatial inertia of body B about an arbitrary point P.
    const Vector3<double> p_BcmP_W(1, -2, 5);
    return SpatialInertia<double>::MakeFromCentralInertia(mass, -p_BcmP_W, I_Bcm_W);
}
...
GTEST_TEST(SpatialInertia, PointMass) {
    ...
    const SpatialInertia<double> M_BBp_B = SpatialInertia<double>::PointMass(mass, p_BpBcm_B);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_BBp_B.CopyToFullMatrix6()));
    ...
    const SpatialInertia<double> M_BBcm_B = SpatialInertia<double>::PointMass(mass, Vector3<double>::Zero());
    EXPECT_TRUE(CompareMatrices(M_BBcm_B_expected.CopyToFullMatrix6(), M_BBcm_B.CopyToFullMatrix6()));
...
GTEST_TEST(SpatialInertia, SolidBoxWithDensityOrMass) {
    ...
    const SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidBoxWithDensity(density, lx, ly, lz);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidBoxWithDensity() matches SolidBoxWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidBoxWithMass(mass, lx, ly, lz);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithMass(-0.1, lx, ly, lz),
                                "[^]* mass is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithDensity(-9.3, lx, ly, lz),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithDensity(density, 0, ly, lz),
                                "[^]* x-length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithMass(mass, 0, ly, lz),
                                "[^]* x-length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithDensity(density, lx, -0.1, lz),
                                "[^]* y-length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithMass(mass, lx, -0.1, lz),
                                "[^]* y-length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithDensity(density, lx, ly, -1E-15),
                                "[^]* z-length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidBoxWithMass(mass, lx, ly, -1E-15),
                                "[^]* z-length is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidCubeWithDensity) {
    ...
    const SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    const SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidCubeWithDensity(density, length);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Also test against a solid box with length = width = height.
    const SpatialInertia<double> Mbox = SpatialInertia<double>::SolidBoxWithDensity(density, length, length, length);
    EXPECT_TRUE(CompareMatrices(Mbox.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidCubeWithDensity() matches SolidCubeWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidCubeWithMass(mass, length);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithDensity(-9.3, length),
                                "[^]* density is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithMass(-0.1, length),
                                "[^]* mass is not positive and finite: .*.");

    // Ensure a negative or zero length throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithDensity(density, 0),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithMass(mass, 0),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithDensity(density, -1),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCubeWithMass(mass, -1),
                                "[^]* length is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidCapsuleWithDensityOrMass) {
    ...
    UnitInertia<double> G_BBo_B = UnitInertia<double>::SolidCapsule(r, l, unit_vec);
    SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidCapsuleWithDensity(density, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));
    ...
    M_with_density = SpatialInertia<double>::SolidCapsuleWithDensity(density, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidCapsuleWithDensity() matches SolidCapsuleWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidCapsuleWithMass(mass, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithDensity(-9.3, r, l, unit_vec),
                                "[^]* density is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithMass(-9.3, r, l, unit_vec),
                                "[^]* mass is not positive and finite: .*.");
    ...
    // Instead, each parameter is tested with a single bad value and we assume a
    // single value sufficiently tests the full domain of invalid values.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithDensity(density, 0, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithMass(mass, 0, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithDensity(density, r, -2, unit_vec),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCapsuleWithMass(mass, r, -2, unit_vec),
                                "[^]* length is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidCylinderWithDensityOrMass) {
    ...
    UnitInertia<double> G_BBo_B = UnitInertia<double>::SolidCylinder(r, l, unit_vec);
    SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidCylinderWithDensity(density, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidCylinderWithMass() matches SolidCylinderWithDensity().
    SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidCylinderWithMass(mass, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));
    ...
    const UnitInertia<double> G_BBp_B = UnitInertia<double>::SolidCylinderAboutEnd(r, l, unit_vec);
    M_expected = SpatialInertia<double>(mass, p_BpBcm_B, G_BBp_B);
    M_with_density = SpatialInertia<double>::SolidCylinderWithDensityAboutEnd(density, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Match SolidCylinderWithMassAboutEnd() & SolidCylinderWithDensityAboutEnd().
    M_with_mass = SpatialInertia<double>::SolidCylinderWithMassAboutEnd(mass, r, l, unit_vec);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Also test a solid cylinder B about Bp using Shift() function.
    const SpatialInertia<double> M_BBcm_B = SpatialInertia<double>::SolidCylinderWithDensity(density, r, l, unit_vec);
    ...
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6(), kTolerance,
                                MatrixCompareType::relative));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensity(-9.3, r, l, unit_vec),
                                "[^]* density is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMass(-9.3, r, l, unit_vec),
                                "[^]* mass is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensityAboutEnd(-9.3, r, l, unit_vec),
                                "[^]* density is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMassAboutEnd(-9.3, r, l, unit_vec),
                                "[^]* mass is not positive and finite: .*.");

    // Ensure a negative or zero radius throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensity(density, 0, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMass(mass, 0, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensityAboutEnd(density, -0.1, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMassAboutEnd(mass, -0.1, l, unit_vec),
                                "[^]* radius is not positive and finite: .*.");

    // Ensure a negative or zero length throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensity(density, r, 0, unit_vec),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMass(mass, r, 0, unit_vec),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensityAboutEnd(density, r, -0.1, unit_vec),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMassAboutEnd(density, r, -0.1, unit_vec),
                                "[^]* length is not positive and finite: .*.");

    // Ensure a bad unit vector throws an exception.
    const Vector3<double> bad_vec(1, 0.1, 0);
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensity(density, r, l, bad_vec),
                                "[^]* The unit_vector argument .* is not a unit vector.[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMass(mass, r, l, bad_vec),
                                "[^]* The unit_vector argument .* is not a unit vector.[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithDensityAboutEnd(density, r, l, bad_vec),
                                "[^]* The unit_vector argument .* is not a unit vector.[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidCylinderWithMassAboutEnd(mass, r, l, bad_vec),
                                "[^]* The unit_vector argument .* is not a unit vector.[^]*");
}
...
GTEST_TEST(SpatialInertia, ThinRodWithMass) {
    ...
    SpatialInertia<double> M = SpatialInertia<double>::ThinRodWithMass(mass, length, unit_vec);
    // Use an empirical tolerance of two bits = 2^2 times machine epsilon.
    const double kTolerance = 4 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M.CopyToFullMatrix6(), kTolerance));
    ...
    const SpatialInertia<double> M_BBp_B =
            SpatialInertia<double>::MakeFromCentralInertia(mass, p_BpBcm_B, mass * G_BBcm_B);
    EXPECT_TRUE(CompareMatrices(M_BBp_B.CopyToFullMatrix6(), M.CopyToFullMatrix6(), kTolerance));

    // Ensure a negative or zero mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::ThinRodWithMass(-1.23, length, unit_vec),
                                "[^]* mass is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::ThinRodWithMass(0, length, unit_vec),
                                "[^]* mass is not positive and finite: .*.");

    // Ensure a negative or zero length throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::ThinRodWithMass(mass, -4.56, unit_vec),
                                "[^]* length is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::ThinRodWithMass(mass, 0, unit_vec),
                                "[^]* length is not positive and finite: .*.");

    // Ensure a bad unit vector throws an exception.
    const Vector3<double> bad_vec(1, 0.1, 0);
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::ThinRodWithMass(mass, length, bad_vec),
                                "[^]* The unit_vector argument .* is not a unit vector.[^]*");
}
...
GTEST_TEST(SpatialInertia, SolidEllipsoidWithDensityOrMass) {
    ...
    SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidEllipsoidWithDensity(density, a, b, c);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidEllipsoidWithDensity() matches SolidEllipsoidWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidEllipsoidWithMass(mass, a, b, c);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithDensity(-9.3, a, b, c),
                                "[^]* density is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithMass(-9.3, a, b, c),
                                "[^]* mass is not positive and finite: .*.");
    ...
    // Instead, each parameter is tested with a single bad value and we assume a
    // single value sufficiently tests the full domain of invalid values.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithDensity(density, 0, b, c),
                                "[^]* semi-axis a is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithMass(mass, 0, b, c),
                                "[^]* semi-axis a is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithDensity(density, a, -2, c),
                                "[^]* semi-axis b is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithMass(mass, a, -2, c),
                                "[^]* semi-axis b is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithDensity(density, a, b, -0.01),
                                "[^]* semi-axis c is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidEllipsoidWithMass(mass, a, b, -0.01),
                                "[^]* semi-axis c is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidSphereWithDensityOrMass) {
    ...
    const SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    const SpatialInertia<double> M_with_density = SpatialInertia<double>::SolidSphereWithDensity(density, radius);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure SolidSphereWithDensity() matches SolidSphereWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::SolidSphereWithMass(mass, radius);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative mass or density throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithMass(-9.3, radius),
                                "[^]* mass is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithDensity(-9.3, radius),
                                "[^]* density is not positive and finite: .*.");

    // Ensure a negative or zero radius throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithMass(mass, 0),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithMass(mass, -0.2),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithDensity(density, 0),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidSphereWithDensity(density, -0.2),
                                "[^]* radius is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, HollowSphereWithDensityOrMass) {
    ...
    const SpatialInertia<double> M_expected(mass, p_BoBcm_B, G_BBo_B);
    const SpatialInertia<double> M_with_density = SpatialInertia<double>::HollowSphereWithDensity(area_density, radius);
    EXPECT_TRUE(CompareMatrices(M_expected.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure HollowSphereWithDensity() matches HollowSphereWithMass().
    const SpatialInertia<double> M_with_mass = SpatialInertia<double>::HollowSphereWithMass(mass, radius);
    EXPECT_TRUE(CompareMatrices(M_with_mass.CopyToFullMatrix6(), M_with_density.CopyToFullMatrix6()));

    // Ensure a negative density or mass throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithMass(-9.3, radius),
                                "[^]* mass is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithDensity(-9.3, radius),
                                "[^]* area_density is not positive and finite: .*.");

    // Ensure a negative or zero radius throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithMass(mass, 0),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithMass(mass, -0.2),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithDensity(area_density, 0),
                                "[^]* radius is not positive and finite: .*.");
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::HollowSphereWithDensity(area_density, -0.2),
                                "[^]* radius is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidTetrahedronAboutVertex) {
    ...
    const UnitInertia<double> G_BB0 = UnitInertia<double>::SolidTetrahedronAboutVertex(p1, p2, p3);
    const SpatialInertia<double> M_BB0_expected(mass, p_B0Bcm, G_BB0);
    SpatialInertia<double> M_BB0 = SpatialInertia<double>::SolidTetrahedronAboutVertexWithDensity(density, p1, p2, p3);

    // An empirical tolerance: two bits = 2^2 times machine epsilon.
    const double kTolerance = 4 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(M_BB0_expected.CopyToFullMatrix6(), M_BB0.CopyToFullMatrix6(), kTolerance));

    // Ensure nothing changes if two arguments are switched (e.g., p1 and p2).
    M_BB0 = SpatialInertia<double>::SolidTetrahedronAboutVertexWithDensity(density, p2, p1, p3);
    EXPECT_TRUE(CompareMatrices(M_BB0_expected.CopyToFullMatrix6(), M_BB0.CopyToFullMatrix6(), kTolerance));

    // Ensure a negative density throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::SolidTetrahedronAboutVertexWithDensity(-9.3, p1, p2, p3),
                                "[^]* density is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, SolidTetrahedronAboutPoint) {
    ...
    SpatialInertia<double> M_BA_expected =
            SpatialInertia<double>::SolidTetrahedronAboutVertexWithDensity(density, p_AB1, p_AB2, p_AB3);
    SpatialInertia<double> M_BA =
            SpatialInertia<double>::SolidTetrahedronAboutPointWithDensity(density, p_AB0, p_AB1, p_AB2, p_AB3);

    // An empirical tolerance: two bits = 2^2 times machine epsilon.
    const double kTolerance = 4 * std::numeric_limits<double>::epsilon();
    EXPECT_TRUE(CompareMatrices(M_BA_expected.CopyToFullMatrix6(), M_BA.CopyToFullMatrix6(), kTolerance));
    ...
    M_BA = SpatialInertia<double>::SolidTetrahedronAboutPointWithDensity(density, p_AB0, p_AB1, p_AB2, p_AB3);
    EXPECT_TRUE(CompareMatrices(M_BA_expected.CopyToFullMatrix6(), M_BA.CopyToFullMatrix6(), kTolerance));

    // Ensure nothing changes if two arguments are switched (e.g., p_AB2, p_AB3).
    M_BA = SpatialInertia<double>::SolidTetrahedronAboutPointWithDensity(density, p_AB0, p_AB1, p_AB3, p_AB2);
    EXPECT_TRUE(CompareMatrices(M_BA_expected.CopyToFullMatrix6(), M_BA.CopyToFullMatrix6(), kTolerance));

    // Ensure a negative density throws an exception.
    DRAKE_EXPECT_THROWS_MESSAGE(
            SpatialInertia<double>::SolidTetrahedronAboutPointWithDensity(-9.3, p_AB0, p_AB1, p_AB3, p_AB2),
            "[^]* density is not positive and finite: .*.");
}
...
GTEST_TEST(SpatialInertia, ConstructionFromMassCmAndUnitInertia) {
    ...
    ASSERT_TRUE(M.get_unit_inertia().CopyToFullMatrix3().isApprox(G.CopyToFullMatrix3(), kEpsilon));
...
GTEST_TEST(SpatialInertia, CastToAutoDiff) {
    ...
    const UnitInertia<double> G_double(m_double(0), m_double(1), m_double(2),  /* moments of inertia */
...
GTEST_TEST(SpatialInertia, PlusEqualOperator) {
    ...
    SpatialInertia<double> MRightBox_Wo_W(mass_right, Vector3d::Zero(), UnitInertia<double>::SolidCube(L));
    ...
    // So far the "about point" is the cube's centroid.
    // We'll shift the about point below.
    SpatialInertia<double> MLeftBox_Wo_W(mass_left, Vector3d::Zero(), UnitInertia<double>::SolidCube(L));
    ...
    const Vector3d com((mass_left * MLeftBox_Wo_W.get_com() + mass_right * MRightBox_Wo_W.get_com()) / mass);
    SpatialInertia<double> MExpected_Wo_W(mass, com, UnitInertia<double>::SolidBox(2 * L, L, L));
    EXPECT_TRUE(MBox_Wo_W.CopyToFullMatrix6().isApprox(MExpected_Wo_W.CopyToFullMatrix6(), kEpsilon));
}
...
GTEST_TEST(SpatialInertia, Shift) {
    ...
    SpatialInertia<double> M_BBcm_W(mass, Vector3d::Zero(),
    ...
    const double I_end = mass * (3 * radius * radius + length * length) / 12 /*About centroid.*/
    ...
    EXPECT_TRUE(M_BBcm_W_back.CopyToFullMatrix6().isApprox(M_BBcm_W.CopyToFullMatrix6(), kEpsilon));
}
...
GTEST_TEST(SpatialInertia, IsPhysicallyValidWithZeroMass) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::MakeFromCentralInertia(mass_zero, p_PBcm, I), expected_message);
...
GTEST_TEST(SpatialInertia, IsPhysicallyValidWithBadInertia) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::MakeFromCentralInertia(1.0, p_PBcm, Ibad), expected_message);
}
...
GTEST_TEST(SpatialInertia, IsPhysicallyValidWithCOMTooFarOut) {
    const std::string expected_message =
            ...
            "\\[-3.6  -3.6  0.4\\]\n";
    DRAKE_EXPECT_THROWS_MESSAGE(
            SpatialInertia<double>(1.0, Vector3d(2.0, 0.0, 0.0), UnitInertia<double>::SolidSphere(1.0)),
            expected_message);
}
...
GTEST_TEST(SpatialInertia, IsPhysicallyValidThrowsNiceExceptionMessage) {
    ...
    const RotationalInertia<double> I_BBcm = RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialInertia<double>::MakeFromCentralInertia(mass, p_PBcm, I_BBcm), expected_message);
}
...
GTEST_TEST(SpatialInertia, SkipValidityCheck) {
    DRAKE_EXPECT_NO_THROW(SpatialInertia<double>(-1.0, Vector3d::Zero(), UnitInertia<double>::SolidSphere(1.0), true));
    DRAKE_EXPECT_NO_THROW(
            SpatialInertia<double>(1.0, Vector3d(2.0, 0.0, 0.0), UnitInertia<double>::SolidSphere(1.0), true));
}
...
GTEST_TEST(SpatialInertia, MakeFromCentralInertia) {
    ...
    const SpatialInertia<double> M_BBo_B = SpatialInertia<double>::MakeFromCentralInertia(mass, p_BoBcm_B, I_BBcm_B);
...
GTEST_TEST(SpatialInertia, IsMakeFromCentralInertiaTestValidityImmediately) {
    ...
    const SpatialInertia<double> M_BBcm_B = SpatialInertia<double>::ThinRodWithMass(mass, length, Vector3d(0, 0, 1));
...
GTEST_TEST(SpatialInertia, KineticEnergy) {
    ...
    // Create the spatial inertia of body B about an arbitrary point P.
    const Vector3<double> p_BcmP_W(1, -2, 5);
    const SpatialInertia<double> M_BP_W = SpatialInertia<double>::MakeFromCentralInertia(mass, -p_BcmP_W, I_Bcm_W);

    // Say body B moves with spatial velocity V_WBcm.
    const SpatialVelocity<double> V_WBcm(Vector3<double>(1, 2, 3), Vector3<double>(4, 5, 6));
    ...
    const double ke_WB_expected = 0.5 * w_WB.dot(I_Bcm_W * w_WB) + 0.5 * mass * v_WBcm.squaredNorm();
...
GTEST_TEST(SpatialInertia, SymbolicNan) {
    ...
    const UnitInertia<T> G_SP_E{Variable{"Ixx"}, Variable{"Iyy"}, Variable{"Izz"},
                                Variable{"Ixy"}, Variable{"Ixz"}, Variable{"Iyz"}};
    const SpatialInertia<T> I{mass, p_PScm_E, G_SP_E};
    ASSERT_EQ(I.IsNaN().to_string(),
...
GTEST_TEST(SpatialInertia, PlusEqualOperatorForTwoMasslessBodies) {
    ...
    const UnitInertia<double> G_B1Bcm_E = UnitInertia<double>::TriaxiallySymmetric(1.0);
    const UnitInertia<double> G_B1P_E = G_B1Bcm_E.ShiftFromCenterOfMass(-p_PB1cm_E);
    const SpatialInertia<double> M_B1P_E(0.0, p_PB1cm_E, G_B1P_E);

    // Massless body 2.
    const Vector3<double> p_PB2cm_E(3.0, 2.0, 1.0);
    const UnitInertia<double> G_B2Bcm_E = UnitInertia<double>::TriaxiallySymmetric(3.0);
    const UnitInertia<double> G_B2P_E = G_B2Bcm_E.ShiftFromCenterOfMass(-p_PB2cm_E);
    const SpatialInertia<double> M_B2P_E(0.0, p_PB2cm_E, G_B2P_E);

    // Compose the spatial inertias of the two massless bodies.
    const SpatialInertia<double> M_BcP_E = SpatialInertia<double>(M_B1P_E) += M_B2P_E;
    ...
    const Vector3<double> p_PCcm_E = 0.5 * (M_B1P_E.get_com() + M_B2P_E.get_com());
    const Matrix3<double> G_CP_E =
            0.5 * (M_B1P_E.get_unit_inertia().CopyToFullMatrix3() + M_B2P_E.get_unit_inertia().CopyToFullMatrix3());
...
GTEST_TEST(SpatialInertia, CalcPrincipalHalfLengthsAndPoseForEquivalentShape) {
    ...
    const drake::math::RotationMatrix R_identity = drake::math::RotationMatrix<double>::Identity();
    ...
    SpatialInertia<double> M_BBcm_B = SpatialInertia<double>::SolidEllipsoidWithDensity(density, a, b, c);
    auto [abc, X_BA] = M_BBcm_B.CalcPrincipalSemiDiametersAndPoseForSolidEllipsoid();
    ...
    // Verify principal directions Ax, Ay, Az (R_BA is an identity matrix).
    // Verify p_BcmAo_B is zero (since Ao should be located at Bcm).
    M_BBcm_B = SpatialInertia<double>::SolidBoxWithDensity(density, 2 * a, 2 * b, 2 * c);
    ...
    // Verify principal directions Ax, Ay, Az (R_BA is an identity matrix).
    // Verify p_BcmAo_B is zero (since Ao should be located at Bcm).
    std::tie(abc, X_BA) = M_BBcm_B.CalcPrincipalHalfLengthsAndPoseForMinimumBoundingBox();
    ...
    std::tie(abc, X_BA) = M_BBo_B.CalcPrincipalHalfLengthsAndPoseForMinimumBoundingBox();
    EXPECT_TRUE(CompareMatrices(min_abc, abc, kTolerance));
    EXPECT_TRUE(X_BA.rotation().IsExactlyEqualTo(R_identity));
    EXPECT_TRUE(X_BA.translation() == p_BoBcm_B);
    ...
    const drake::math::RotationMatrix<double> R_BE = drake::math::RotationMatrix<double>::MakeZRotation(M_PI / 6.0);
    ...
    // Repeat the previous test for the minimum bounding box B.
    std::tie(abc, X_BA) = M_BBo_E.CalcPrincipalHalfLengthsAndPoseForMinimumBoundingBox();
    EXPECT_TRUE(CompareMatrices(min_abc, abc, kTolerance));
    R_BA = X_BA.rotation();
    Ax_B = R_BA.col(0);
    Ay_B = R_BA.col(1);
...
GTEST_TEST(SpatialInertia, VerifyMinimumBoundingBoxLengths) {
    ...
    M_BBcm_B += SpatialInertia<double>::PointMass(mass, Vector3d(-a, 0, 0));
    auto [abc, X_BA] = M_BBcm_B.CalcPrincipalHalfLengthsAndPoseForMinimumBoundingBox();
    EXPECT_TRUE(CompareMatrices(Vector3<double>(a, b, c), abc, kTolerance));
    const drake::math::RotationMatrix R_identity = drake::math::RotationMatrix<double>::Identity();
    EXPECT_TRUE(X_BA.rotation().IsExactlyEqualTo(R_identity));
    EXPECT_TRUE(X_BA.translation() == Vector3<double>::Zero());
