...
#include "multibody/tree/revolute_mobilizer.h"
...
public:
    ...
    // RevoluteMobilizer.
    static const internal::RevoluteMobilizer<double>* get_mobilizer(const RevoluteJoint<double>& joint) {
...
public:
    ...
    void CreatePendulumModel() {
        ...
        UnitInertia<double> G_U = UnitInertia<double>::StraightLine(link1_Ic_, Vector3d::UnitY());
        ...
        UnitInertia<double> G_Lcm = UnitInertia<double>::StraightLine(link2_Ic_, Vector3d::UnitY());
        ...
        // upper_link.
        shoulder_outboard_frame_ = &model_->AddFrame<FixedOffsetFrame>("So", upper_link_->body_frame(), X_USo_);
        ...
        shoulder_joint_ = &model_->AddJoint(make_unique<RevoluteJoint<double>>(
                "shoulder", *shoulder_inboard_frame_, *shoulder_outboard_frame_, Vector3d::UnitZ() /*revolute axis*/));
        ...
        elbow_joint_ = &model_->AddJoint<RevoluteJoint>("ElbowJoint", *upper_link_, X_UEi_, /* Pose of Ei in U. */
        ...
        // frame.
        ASSERT_EQ(elbow_outboard_frame_->index(), lower_link_->body_frame().index());

        // Add force element for a constant gravity pointing downwards, that is, in
        // the minus y-axis direction.
        model_->mutable_gravity_field().set_gravity_vector(Vector3d(0.0, -acceleration_of_gravity_, 0.0));
    }

    // Helper method to extract a pose from the position kinematics.
    template <typename T>
    static const RigidTransform<T> get_body_pose_in_world(const MultibodyTree<T>& tree,
        ...
        const MobodIndex mobod_index = topology.get_rigid_body(body.index()).mobod_index;
        return RigidTransform<T>(pc.get_X_WB(mobod_index));
    }

    // Helper method to extract spatial velocity from the velocity kinematics
    // cache.
    static const SpatialVelocity<double>& get_body_spatial_velocity_in_world(const MultibodyTree<double>& tree,
    ...
    // kinematics cache.
    static const SpatialAcceleration<double>& get_body_spatial_acceleration_in_world(
...
protected:
    ...
    // cache.
    void SetPendulumPoses(PositionKinematicsCache<double>* pc) { pc->get_mutable_X_WB(MobodIndex(1)) = X_WL_; }
...
TEST_F(PendulumTests, CreateModelBasics) {
    ...
    EXPECT_EQ(shoulder_mobilizer_->inboard_frame().index(), world_body_->body_frame().index());
    EXPECT_EQ(shoulder_mobilizer_->outboard_frame().index(), shoulder_outboard_frame_->index());
    ...
    // Checks that mobilizers connect the right frames.
    EXPECT_EQ(elbow_mobilizer_->inboard_frame().index(), elbow_inboard_frame_->index());
    EXPECT_EQ(elbow_mobilizer_->outboard_frame().index(), elbow_outboard_frame_->index());
    // Checks that mobilizers connect the right bodies.
...
TEST_F(PendulumTests, Finalize) {
    ...
    EXPECT_THROW(model_->AddFrame<FixedOffsetFrame>("F", *lower_link_, X_LEo_), std::logic_error);
    EXPECT_THROW(model_->AddMobilizer<RevoluteMobilizer>(*shoulder_inboard_frame_, *shoulder_outboard_frame_,
                                                         Vector3d::UnitZ()),
                 std::logic_error);
...
public:
    ...
    void VerifyMassMatrixViaInverseDynamics(double shoulder_angle, double elbow_angle) {
    ...
    void VerifyCoriolisTermViaInverseDynamics(double shoulder_angle, double elbow_angle) {
        ...
        C_expected = acrobot_benchmark_.CalcCoriolisVector(shoulder_angle, elbow_angle, shoulder_rate, elbow_rate);
        EXPECT_TRUE(CompareMatrices(C, C_expected, kTolerance, MatrixCompareType::relative));
    ...
    Vector2d VerifyGravityTerm(const Eigen::Ref<const VectorXd>& q) const {
        ...
        const Vector2d tau_g_expected = acrobot_benchmark_.CalcGravityVector(shoulder_angle, elbow_angle);
        ...
        std::vector<SpatialForce<double>>& Fapplied_Bo_W_array = forcing.mutable_body_forces();
        ...
        tau_applied.setZero();
        tree().CalcInverseDynamics(*context_, vdot, Fapplied_Bo_W_array, tau_applied, &A_WB_array, &F_BMo_W_array,
                                   &tau);
        ...
        tree().CalcInverseDynamics(*context_, vdot, Fapplied_Bo_W_array, tau_applied, &A_WB_array, &Fapplied_Bo_W_array,
                                   &tau_applied);
        // The result from inverse dynamics must be tau = -tau_g(q).
        EXPECT_TRUE(tau.isApprox(-tau_g_expected, kTolerance));

        // Compute the system's potential energy:
        const double V_expected = acrobot_benchmark_.CalcPotentialEnergy(shoulder_angle, elbow_angle);
    ...
    static SpatialVelocity<double> ComputeSpatialVelocityFromXdot(const Matrix4d& X_AB, const Matrix4d& X_AB_dot) {
...
private:
    ...
    Vector2d VerifyInverseDynamics(const Eigen::Ref<const VectorXd>& q,
        ...
        vector<SpatialAcceleration<double>> A_WB_array(tree().num_bodies());
        vector<SpatialForce<double>> F_BMo_W_array(tree().num_bodies());
        tree().CalcInverseDynamics(*context_, vdot, {}, VectorXd(), &A_WB_array, &F_BMo_W_array, &tau);
        ...
        const SpatialAcceleration<double>& A_WUcm_ac = get_body_spatial_acceleration_in_world(tree(), ac, *upper_link_);
        const SpatialAcceleration<double>& A_WL_ac = get_body_spatial_acceleration_in_world(tree(), ac, *lower_link_);
        // From inverse dynamics.
        const SpatialAcceleration<double>& A_WUcm_id = A_WB_array[upper_link_->mobod_index()];
        const SpatialAcceleration<double>& A_WL_id = A_WB_array[lower_link_->mobod_index()];
        EXPECT_TRUE(A_WUcm_id.IsApprox(A_WUcm_ac, kTolerance));
        EXPECT_TRUE(A_WL_id.IsApprox(A_WL_ac, kTolerance));

        // ======================================================================
        // Compute expected values using the acrobot benchmark.
        const Vector2d C_expected = acrobot_benchmark_.CalcCoriolisVector(shoulder_angle, elbow_angle,
        ...
        EXPECT_TRUE(CompareMatrices(tau, tau_expected, kTolerance, MatrixCompareType::relative));
        return tau;
    }
};
...
TEST_F(PendulumKinematicTests, CalcPositionKinematics) {
    ...
    for (double ishoulder = 0; ishoulder < num_angles; ++ishoulder) {
        ...
        for (double ielbow = 0; ielbow < num_angles; ++ielbow) {
            ...
            const MobodIndex shoulder_node = shoulder_mobilizer_->get_topology().mobod_index;
            const MobodIndex elbow_node = elbow_mobilizer_->get_topology().mobod_index;
            ...
            EXPECT_TRUE(pc.get_X_FM(shoulder_node).IsNearlyEqualTo(X_SiSo, kTolerance));
            ...
            EXPECT_EQ(&pc.get_X_FM(shoulder_node), &pc.get_mutable_X_FM(shoulder_node));
            EXPECT_EQ(&pc.get_X_FM(elbow_node), &pc.get_mutable_X_FM(elbow_node));

            // Retrieve body poses from position kinematics cache.
            const RigidTransformd X_WW = get_body_pose_in_world(tree(), pc, *world_body_);
            const RigidTransformd X_WU = get_body_pose_in_world(tree(), pc, *upper_link_);
            const RigidTransformd X_WL = get_body_pose_in_world(tree(), pc, *lower_link_);

            const RigidTransformd X_WU_expected = acrobot_benchmark_.CalcLink1PoseInWorldFrame(shoulder_angle);

            const RigidTransformd X_WL_expected =
                    acrobot_benchmark_.CalcElbowOutboardFramePoseInWorldFrame(shoulder_angle, elbow_angle);

            // Asserts that the retrieved poses match with the ones specified by the
            // unit test method SetPendulumPoses().
            EXPECT_TRUE(CompareMatrices(X_WW.GetAsMatrix4(), Matrix4d::Identity(), kTolerance,
                                        MatrixCompareType::relative));
            EXPECT_TRUE(CompareMatrices(X_WU.GetAsMatrix34(), X_WU_expected.GetAsMatrix34(), kTolerance,
                                        MatrixCompareType::relative));
            EXPECT_TRUE(CompareMatrices(X_WL.GetAsMatrix34(), X_WL_expected.GetAsMatrix34(), kTolerance,
                                        MatrixCompareType::relative));
        }
    }
}
...
TEST_F(PendulumKinematicTests, CalcVelocityAndAccelerationKinematics) {
    ...
    for (double ishoulder = 0; ishoulder < num_angles; ++ishoulder) {
        ...
        for (double ielbow = 0; ielbow < num_angles; ++ielbow) {
            ...
            shoulder_mobilizer_->SetAngularRate(context_.get(), shoulder_angle_rate);
            EXPECT_EQ(shoulder_mobilizer_->get_angular_rate(*context_), shoulder_angle_rate);
            ...
            const SpatialVelocity<double>& V_WUcm = get_body_spatial_velocity_in_world(tree(), vc, *upper_link_);
            const SpatialVelocity<double>& V_WL = get_body_spatial_velocity_in_world(tree(), vc, *lower_link_);
            ...
            const SpatialVelocity<double> V_WUcm_expected(
                    acrobot_benchmark_.CalcLink1SpatialVelocityInWorldFrame(shoulder_angle, shoulder_angle_rate));
            const SpatialVelocity<double> V_WLcm_expected(acrobot_benchmark_.CalcLink2SpatialVelocityInWorldFrame(
            ...
            SpatialAcceleration<double> A_WUcm = get_body_spatial_acceleration_in_world(tree(), ac, *upper_link_);
            SpatialAcceleration<double> A_WL = get_body_spatial_acceleration_in_world(tree(), ac, *lower_link_);
            ...
            SpatialAcceleration<double> A_WUcm_expected(acrobot_benchmark_.CalcLink1SpatialAccelerationInWorldFrame(
                    shoulder_angle, shoulder_angle_rate, vdot(0)));

            SpatialAcceleration<double> A_WLcm_expected(acrobot_benchmark_.CalcLink2SpatialAccelerationInWorldFrame(
                    shoulder_angle, elbow_angle, shoulder_angle_rate, elbow_angle_rate, vdot(0), vdot(1)));
            ...
            EXPECT_EQ(shoulder_mobilizer_->get_accelerations_from_array(vdot).size(), 1);
            EXPECT_EQ(shoulder_mobilizer_->get_accelerations_from_array(vdot)(0), -1.0);
            ...
            A_WUcm_expected = SpatialAcceleration<double>(acrobot_benchmark_.CalcLink1SpatialAccelerationInWorldFrame(
                    shoulder_angle, shoulder_angle_rate, vdot(0)));

            A_WLcm_expected = SpatialAcceleration<double>(acrobot_benchmark_.CalcLink2SpatialAccelerationInWorldFrame(
                    shoulder_angle, elbow_angle, shoulder_angle_rate, elbow_angle_rate, vdot(0), vdot(1)));
...
TEST_F(PendulumKinematicTests, CalcVelocityKinematicsWithAutoDiffXd) {
    ...
    std::unique_ptr<MultibodyTree<AutoDiffXd>> model_autodiff = tree().ToAutoDiffXd();
    ...
    const RevoluteJoint<AutoDiffXd>& elbow_joint_autodiff = model_autodiff->get_variant(*elbow_joint_);

    const RigidBody<AutoDiffXd>& upper_link_autodiff = model_autodiff->get_variant(*upper_link_);
    const RigidBody<AutoDiffXd>& lower_link_autodiff = model_autodiff->get_variant(*lower_link_);

    MultibodyTreeSystem<AutoDiffXd> tree_system_autodiff(std::move(model_autodiff));
    std::unique_ptr<Context<AutoDiffXd>> context_autodiff = tree_system_autodiff.CreateDefaultContext();
    ...
    for (int iw_shoulder = 0; iw_shoulder < num_velocities; ++iw_shoulder) {
        ...
        for (int iw_elbow = 0; iw_elbow < num_velocities; ++iw_elbow) {
            ...
            for (double iq_shoulder = 0; iq_shoulder < num_angles; ++iq_shoulder) {
                const AutoDiffXd shoulder_angle(-M_PI + iq_shoulder * kDeltaAngle, /* angle value */
                                                Vector1<double>::Constant(w_WU) /* angular velocity */);
                for (double iq_elbow = 0; iq_elbow < num_angles; ++iq_elbow) {
                    const AutoDiffXd elbow_angle(-M_PI + iq_elbow * kDeltaAngle, /* angle value */
                                                 Vector1<double>::Constant(w_UL) /* angular velocity */);

                    // Update position kinematics.
                    shoulder_mobilizer_autodiff.SetAngle(context_autodiff.get(), shoulder_angle);
                    ...
                    const RigidTransformd X_WL_expected = acrobot_benchmark_.CalcElbowOutboardFramePoseInWorldFrame(
                    ...
                    EXPECT_TRUE(X_WU_value.isApprox(X_WU_expected.GetAsMatrix4(), kTolerance));
                    EXPECT_TRUE(X_WL_value.isApprox(X_WL_expected.GetAsMatrix4(), kTolerance));
                    ...
                    SpatialVelocity<double> V_WUcm = ComputeSpatialVelocityFromXdot(X_WU_value, X_WU_dot);
                    SpatialVelocity<double> V_WL = ComputeSpatialVelocityFromXdot(X_WL_value, X_WL_dot);
                    // Obtain the lower link's center of mass frame spatial velocity by
                    // shifting V_WL:
                    const SpatialVelocity<double> V_WLcm = V_WL.Shift(p_LoLcm_W);

                    const SpatialVelocity<double> V_WUcm_expected(
                            acrobot_benchmark_.CalcLink1SpatialVelocityInWorldFrame(shoulder_angle.value(), w_WU));
                    const SpatialVelocity<double> V_WLcm_expected(
                            acrobot_benchmark_.CalcLink2SpatialVelocityInWorldFrame(shoulder_angle.value(),
                                                                                    elbow_angle.value(), w_WU, w_UL));

                    EXPECT_TRUE(V_WUcm.IsApprox(V_WUcm_expected, kTolerance));
                    EXPECT_TRUE(V_WLcm.IsApprox(V_WLcm_expected, kTolerance));

                    // Compute potential energy, and its time derivative.
                    const AutoDiffXd V = tree_autodiff.CalcPotentialEnergy(*context_autodiff);
                    const double V_value = V.value();
                    const double V_expected =
                            acrobot_benchmark_.CalcPotentialEnergy(shoulder_angle.value(), elbow_angle.value());
                    ...
                    shoulder_mobilizer_->SetAngularRate(context_.get(), shoulder_angle.derivatives()[0]);
                    elbow_mobilizer_->SetAngle(context_.get(), elbow_angle.value());
                    elbow_mobilizer_->SetAngularRate(context_.get(), elbow_angle.derivatives()[0]);
...
TEST_F(PendulumKinematicTests, PointsPositionsAndRelativeTransform) {
    ...
    tree().CalcPointsPositions(*context_, lower_link_->body_frame(), p_LQi_set, tree().world_frame(), &p_WQi_set);
    ...
    EXPECT_TRUE(CompareMatrices(p_WQi_set, p_WQi_set_expected, kTolerance, MatrixCompareType::relative));
    ...
    tree().CalcPointsPositions(*context_, upper_link_->body_frame(), p_UPi_set, tree().world_frame(), &p_WPi_set);
    ...
    EXPECT_TRUE(CompareMatrices(p_WPi_set, p_WPi_set_expected, kTolerance, MatrixCompareType::relative));
    ...
    const RigidTransformd X_UL(
            tree().CalcRelativeTransform(*context_, upper_link_->body_frame(), lower_link_->body_frame()));
    EXPECT_TRUE(CompareMatrices(X_UL.rotation().matrix(), R_UL_expected, kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(X_UL.translation(), p_UL_expected, kTolerance, MatrixCompareType::relative));

    // Verify the RotationMatrix returned by CalcRelativeRotationMatrix().
    const RotationMatrixd R_UL =
            tree().CalcRelativeRotationMatrix(*context_, upper_link_->body_frame(), lower_link_->body_frame());
    EXPECT_TRUE(CompareMatrices(R_UL.matrix(), R_UL_expected, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(PendulumKinematicTests, PointsHaveTheWrongSize) {
    ...
    // World positions of the set of points Qi:
    Matrix3X<double> p_WQi_set(3, 3);
    EXPECT_THROW(tree().CalcPointsPositions(*context_, lower_link_->body_frame(), p_LQi_set, tree().world_frame(),
                                            &p_WQi_set),
                 std::runtime_error);
}
