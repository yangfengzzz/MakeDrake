...
GTEST_TEST(RigidBody, RigidBodyConstructor2) {
    ...
    EXPECT_EQ(B.default_spatial_inertia().get_unit_inertia().get_moments(), M_Bo_B.get_unit_inertia().get_moments());
    EXPECT_EQ(B.default_spatial_inertia().get_unit_inertia().get_products(), M_Bo_B.get_unit_inertia().get_products());
...
public:
    void SetUp() override {
        ...
        model->Finalize();
        mbtree_system_ = std::make_unique<internal::MultibodyTreeSystem<double>>(std::move(model));
...
TEST_F(RigidBodyTest, SetCenterOfMassInBodyFrame) {
    ...
    const SpatialInertia<double> M_BBo_B_test = rigid_body_->CalcSpatialInertiaInBodyFrame(*context_);
    EXPECT_TRUE(CompareMatrices(M_BBo_B_test.CopyToFullMatrix6(), M_BBo_B.CopyToFullMatrix6(), kTolerance));
    ...
    EXPECT_TRUE(CompareMatrices(I_BBo_B.CopyToFullMatrix3(), I_BBo_B_expected.CopyToFullMatrix3(), kTolerance));
    ...
    EXPECT_TRUE(CompareMatrices(M_BBo_B.CopyToFullMatrix6(), M_BBo_B_expected.CopyToFullMatrix6(), kTolerance));
    ...
    const Vector3d p_BoBcm_B_calculated = rigid_body_->CalcCenterOfMassInBodyFrame(*context_);
    ...
    EXPECT_TRUE(CompareMatrices(G_BBo_B_test.CopyToFullMatrix3(), G_BBo_B.CopyToFullMatrix3(),
    ...
    EXPECT_TRUE(CompareMatrices(G_BBcm_B.CopyToFullMatrix3(), G_BBcm_B_expected.CopyToFullMatrix3(), kTolerance));
...
TEST_F(RigidBodyTest, SetCenterOfMassInBodyFrameAndPreserveCentralInertia) {
    ...
    p_BoBcm_B = Vector3d(L / 2, 0, 0);  // Now p_BoBcm_B = (1, 0, 0).
    rigid_body_->SetCenterOfMassInBodyFrameAndPreserveCentralInertia(context_ptr, p_BoBcm_B);
    const Vector3d p_BoBcm_B_calculated = rigid_body_->CalcCenterOfMassInBodyFrame(*context_);
