...
GTEST_TEST(RotationalInertia, DiagonalInertiaConstructor) {
    ...
    RotationalInertia<double> I = RotationalInertia<double>::TriaxiallySymmetric(I_diagonal);
...
GTEST_TEST(RotationalInertia, MakeFromMomentsAndProductsOfInertia) {
    ...
    RotationalInertia<double> I = RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(
    ...
    EXPECT_THROW(RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(2 * Ixx, Iyy, Izz, Ixy, Ixz, Iyz,
    ...
    EXPECT_NO_THROW(
            I = RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(2 * Ixx, Iyy, Izz, Ixy, Ixz, Iyz,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(RotationalInertia<double>::MakeFromMomentsAndProductsOfInertia(
...
GTEST_TEST(RotationalInertia, GeneralConstructor) {
    ...
    RotationalInertia<double> I(moments(0), moments(1), moments(2), product(0), product(1), product(2));
    EXPECT_EQ(I.get_moments(), moments);
...
GTEST_TEST(RotationalInertia, TraceIsSumOfDiagonalElements) {
    const Vector3d moments(2.0, 2.3, 2.4);
    const Vector3d product(0.1, -0.1, 0.2);
    const RotationalInertia<double> I(moments(0), moments(1), moments(2), product(0), product(1), product(2));
...
GTEST_TEST(RotationalInertia, IsNearlyEqualTo) {
    ...
    const RotationalInertia<double> I2(Ixx, Iyy, Izz, Ixy, Ixz, Iyz + 3 * epsilonI);
    const RotationalInertia<double> I3(Ixx, Iyy, Izz, Ixy, Ixz, Iyz + 8 * epsilonI);
...
GTEST_TEST(RotationalInertia, ReExpressInAnotherFrameB) {
    ...
    const RotationMatrixd R_BA = RotationMatrixd::MakeFromOrthonormalRows(
            Vector3d(R_BAxx, R_BAxy, R_BAxz), Vector3d(R_BAyx, R_BAyy, R_BAyz), Vector3d(R_BAzx, R_BAzy, R_BAzz));
    ...
    const RotationalInertia<double> I_BBo_A(I_BBo_Axx, I_BBo_Ayy, I_BBo_Azz, I_BBo_Axy, I_BBo_Axz, I_BBo_Ayz);
    ...
    const RotationalInertia<double> expected_I_BBo_B(I_BBo_Bxx, I_BBo_Byy, I_BBo_Bzz, I_BBo_Bxy, I_BBo_Bxz, I_BBo_Byz);
...
GTEST_TEST(RotationalInertia, ShiftFromCenterOfMass) {
    ...
    const RotationalInertia<double> I_BBcm_B(I_BBcm_Bxx, I_BBcm_Byy, I_BBcm_Bzz, I_BBcm_Bxy, I_BBcm_Bxz, I_BBcm_Byz);
    const double mass = 1.1, xQ = 2.234, yQ = 3.14, zQ = 0.56;
    const Vector3d p_BcmQ_B(xQ, yQ, zQ);
    const RotationalInertia<double> I_BQ_B = I_BBcm_B.ShiftFromCenterOfMass(mass, p_BcmQ_B);
...
GTEST_TEST(RotationalInertia, ShiftToCenterOfMass) {
    ...
    const RotationalInertia<double> I_BP_B(I_BP_Bxx, I_BP_Byy, I_BP_Bzz, I_BP_Bxy, I_BP_Bxz, I_BP_Byz);
    const double mass = 1.1, xBcm = 2.234, yBcm = 3.14, zBcm = 0.56;
    const Vector3d p_PBcm_B(xBcm, yBcm, zBcm);
    const RotationalInertia<double> I_BBcm_B = I_BP_B.ShiftToCenterOfMass(mass, p_PBcm_B);
    ...
    const RotationalInertia<double> expected_I_BBcm_B(Ixx, Iyy, Izz, Ixy, Ixz, Iyz);
...
GTEST_TEST(RotationalInertia, ShiftToThenAwayFromCenterOfMass) {
    ...
    const RotationalInertia<double> I_BP_B(I_BP_xx, I_BP_yy, I_BP_zz, I_BP_xy, I_BP_xz, I_BP_yz);
    const double mass = 1.1, xBcm = 2.234, yBcm = 3.14, zBcm = 0.56;
    ...
    const RotationalInertia<double> I_BBcm_B = I_BP_B.ShiftToCenterOfMass(mass, p_PBcm);
    const RotationalInertia<double> expected_I_BQ_B = I_BBcm_B.ShiftFromCenterOfMass(mass, p_QBcm);

    // Calculate with single method that does it slightly more efficiently.
    const RotationalInertia<double> I_BQ_B = I_BP_B.ShiftToThenAwayFromCenterOfMass(mass, p_PBcm, p_QBcm);
    EXPECT_TRUE(I_BQ_B.IsNearlyEqualTo(expected_I_BQ_B, 2 * kEpsilon));

    // Test that negating position vectors have no affect on results.
    EXPECT_TRUE(I_BBcm_B.IsNearlyEqualTo(I_BP_B.ShiftToCenterOfMass(mass, -p_PBcm), 2 * kEpsilon));
    EXPECT_TRUE(I_BQ_B.IsNearlyEqualTo(I_BBcm_B.ShiftFromCenterOfMass(mass, -p_QBcm), 2 * kEpsilon));
    EXPECT_TRUE(I_BQ_B.IsNearlyEqualTo(I_BP_B.ShiftToThenAwayFromCenterOfMass(mass, -p_PBcm, -p_QBcm), 2 * kEpsilon));
}
...
GTEST_TEST(RotationalInertia, CouldBePhysicallyValidA) {
    ...
    const RotationalInertia<double> I_BP(I_BP_xx, I_BP_yy, I_BP_zz, I_BP_xy, I_BP_xz, I_BP_yz);
    const double mass = 1.1, xBcm = 2.234, yBcm = 3.14, zBcm = 0.56;
    const Vector3d p_PBcm(xBcm, yBcm, zBcm);
    ...
    EXPECT_THROW_IF_ARMED(I_BBcm = I_BP.ShiftToCenterOfMass(mass, p_PBcm), std::logic_error);
}
...
GTEST_TEST(RotationalInertia, CouldBePhysicallyValidC) {
    EXPECT_THROW_IF_ARMED(RotationalInertia<double> bad_inertia(10, 10, 30), std::logic_error);
}
...
GTEST_TEST(RotationalInertia, CouldBePhysicallyValidD) {
    EXPECT_THROW_IF_ARMED(RotationalInertia<double> bad_I(4, 4, 4, 2.1, 0, 0), std::logic_error);
}
...
GTEST_TEST(RotationalInertia, CouldBePhysicallyValidE) {
    EXPECT_THROW_IF_ARMED(RotationalInertia<double> bad_inertia(2, 2, 2, -0.8, 0, -0.8), std::logic_error);
}
...
GTEST_TEST(RotationalInertia, PrincipalMomentsOfInertiaEtc) {
    ...
    const drake::math::RollPitchYaw<double> rpy(20 * deg_to_rad, 25 * deg_to_rad, 30 * deg_to_rad);
    ...
    std::sort(expected_principal_moments.data(), expected_principal_moments.data() + expected_principal_moments.size());

    // Verify principal moments against their expected value.
    const double inertia_tolerance =
            4 * std::numeric_limits<double>::epsilon() * I_BBcm_W.CalcMaximumPossibleMomentOfInertia();
    EXPECT_TRUE(CompareMatrices(expected_principal_moments, principal_moments, inertia_tolerance,
    ...
    EXPECT_TRUE(CompareMatrices(products_of_inertia, Vector3<double>::Zero(), inertia_tolerance,
                                MatrixCompareType::absolute));
    ...
    EXPECT_TRUE(CompareMatrices(R_QP.matrix(), m_expected, 0.0, MatrixCompareType::absolute));
    ...
    I_BBcm_P = I_BBcm_Q.CalcPrincipalMomentsAndAxesOfInertia();
    principal_moments = I_BBcm_P.first;
    EXPECT_TRUE(CompareMatrices(Vector3<double>(Iyy, Ixx, Izz), principal_moments, inertia_tolerance,
                                MatrixCompareType::absolute));
    R_QP = I_BBcm_P.second;
...
GTEST_TEST(RotationalInertia, CalcPrincipalMomentsAndAxesOfInertia) {
    ...
    RotationalInertia<double> I_BBcm_B = RotationalInertia<double>(Imed, Imed, Imed);
    std::pair<Vector3<double>, drake::math::RotationMatrix<double>> I_BBcm_P =
            I_BBcm_B.CalcPrincipalMomentsAndAxesOfInertia();
    EXPECT_TRUE(CompareMatrices(Vector3<double>(Imed, Imed, Imed), I_BBcm_P.first, kTolerance));
    drake::math::RotationMatrix<double> R_BP = I_BBcm_P.second;
    drake::math::RotationMatrix<double> R_BP_expected = drake::math::RotationMatrix<double>::Identity();
    ...
    EXPECT_TRUE(CompareMatrices(Vector3<double>(Imin, Imed, Imax), I_BBcm_P.first, kTolerance));
    ...
    R_BP_expected = drake::math::RotationMatrix<double>::MakeFromOrthonormalColumns(col_min, col_med, -col_max);
...
GTEST_TEST(RotationalInertia, PrincipalMomentsOfInertiaLaplacianTest) {
    ...
    EXPECT_THROW_IF_ARMED(RotationalInertia<double> I(Idiag, Idiag, Idiag, Ioff, 0.0, Ioff), std::logic_error);
}
...
GTEST_TEST(RotationalInertia, AutoDiff) {
    ...
    const drake::math::RotationMatrix<AutoDiffXd> R_BW = drake::math::RotationMatrix<AutoDiffXd>::MakeZRotation(-angle);
...
GTEST_TEST(RotationalInertia, CompatibleWithSymbolicExpression) {
    ...
    const RotationalInertia<Expression> I_BQ_E = I_Bcm_E.ShiftFromCenterOfMass(mass, p_BcmQ_E);
