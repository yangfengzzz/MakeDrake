...
public:
    ...
    std::unique_ptr<internal::MultibodyTree<double>> MakeModel() {
        ...
        joint1_ =
                &model->AddJoint<RevoluteJoint>("Joint1", model->world_body(), std::nullopt, *body1_, std::nullopt,
                                                Vector3d::UnitZ(), kPositionLowerLimit, kPositionUpperLimit, kDamping);
        mutable_joint1_ = dynamic_cast<RevoluteJoint<double>*>(&model->get_mutable_joint(joint1_->index()));
        DRAKE_DEMAND(mutable_joint1_ != nullptr);
        mutable_joint1_->set_velocity_limits(Vector1<double>::Constant(kVelocityLowerLimit),
                                             Vector1<double>::Constant(kVelocityUpperLimit));
        mutable_joint1_->set_acceleration_limits(Vector1<double>::Constant(kAccelerationLowerLimit),
...
TEST_F(RevoluteJointTest, ContextDependentAccess) {
    ...
    EXPECT_NO_THROW(joint1_->SetDampingVector(context_.get(), Vector1d(some_value)));
    ...
    EXPECT_THROW(joint1_->SetDampingVector(context_.get(), Vector1d(-1)), std::exception);
}
...
TEST_F(RevoluteJointTest, Clone) {
    ...
    EXPECT_EQ(joint1_clone.frame_on_parent().index(), joint1_->frame_on_parent().index());
    EXPECT_EQ(joint1_clone.frame_on_child().index(), joint1_->frame_on_child().index());
    EXPECT_EQ(joint1_clone.revolute_axis(), joint1_->revolute_axis());
    EXPECT_EQ(joint1_clone.position_lower_limits(), joint1_->position_lower_limits());
    EXPECT_EQ(joint1_clone.position_upper_limits(), joint1_->position_upper_limits());
    EXPECT_EQ(joint1_clone.velocity_lower_limits(), joint1_->velocity_lower_limits());
    EXPECT_EQ(joint1_clone.velocity_upper_limits(), joint1_->velocity_upper_limits());
    EXPECT_EQ(joint1_clone.acceleration_lower_limits(), joint1_->acceleration_lower_limits());
    EXPECT_EQ(joint1_clone.acceleration_upper_limits(), joint1_->acceleration_upper_limits());
...
TEST_F(RevoluteJointTest, SetVelocityAndAccelerationLimits) {
    ...
    mutable_joint1_->set_velocity_limits(Vector1<double>(new_lower), Vector1<double>(new_upper));
    ...
    EXPECT_THROW(mutable_joint1_->set_velocity_limits(VectorX<double>(1), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint1_->set_velocity_limits(VectorX<double>(), VectorX<double>(1)), std::exception);
    // Lower limit is larger than upper limit.
    EXPECT_THROW(mutable_joint1_->set_velocity_limits(Vector1<double>(2), Vector1<double>(0)), std::exception);

    // Check for acceleration limits.
    mutable_joint1_->set_acceleration_limits(Vector1<double>(new_lower), Vector1<double>(new_upper));
    EXPECT_EQ(joint1_->acceleration_lower_limit(), new_lower);
    EXPECT_EQ(joint1_->acceleration_upper_limit(), new_upper);
    // Does not match num_velocities().
    EXPECT_THROW(mutable_joint1_->set_acceleration_limits(VectorX<double>(1), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint1_->set_acceleration_limits(VectorX<double>(), VectorX<double>(1)), std::exception);
    // Lower limit is larger than upper limit.
    EXPECT_THROW(mutable_joint1_->set_acceleration_limits(Vector1<double>(2), Vector1<double>(0)), std::exception);
}
...
TEST_F(RevoluteJointTest, DefaultAngle) {
    ...
    const double new_default_angle = 0.5 * kPositionLowerLimit + 0.5 * kPositionUpperLimit;
