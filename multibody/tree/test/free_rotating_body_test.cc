...
GTEST_TEST(RollPitchYawTest, TimeDerivatives) {
    ...
    Vector3d w0_WB_expected = free_body_plant.get_default_initial_angular_velocity();
    ...
    SpatialVelocity<double> V_WB = free_body_plant.CalcSpatialVelocityInWorldFrame(context);
    ...
    EXPECT_TRUE(CompareMatrices(p_WBcm, Vector3d::Zero(), kTolerance, MatrixCompareType::relative));

    EXPECT_TRUE(CompareMatrices(w_WB, w_WB_exact, kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(v_WB, Vector3d::Zero(), kTolerance, MatrixCompareType::relative));
