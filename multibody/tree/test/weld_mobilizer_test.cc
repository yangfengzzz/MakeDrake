...
public:
    void SetUp() override {
        weld_body_to_world_ = &AddJointAndFinalize<WeldJoint, WeldMobilizer>(std::make_unique<WeldJoint<double>>(
                "joint0", tree().world_body().body_frame(), body_->body_frame(), X_WB_));
    }
...
TEST_F(WeldMobilizerTest, CalcAcrossMobilizerTransform) {
    const math::RigidTransformd X_FM(weld_body_to_world_->CalcAcrossMobilizerTransform(*context_));
    EXPECT_TRUE(CompareMatrices(X_FM.GetAsMatrix34(), X_WB_.GetAsMatrix34(), kTolerance, MatrixCompareType::relative));
}
...
TEST_F(WeldMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    const SpatialVelocity<double> V_FM =
            weld_body_to_world_->CalcAcrossMobilizerSpatialVelocity(*context_, zero_sized_vector);
...
TEST_F(WeldMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            weld_body_to_world_->CalcAcrossMobilizerSpatialAcceleration(*context_, zero_sized_vector);
...
TEST_F(WeldMobilizerTest, ProjectSpatialForce) {
    ...
    weld_body_to_world_->ProjectSpatialForce(*context_, F_Mo_F, zero_sized_vector);
}
...
TEST_F(WeldMobilizerTest, MapVelocityToQDotAndBack) {
    ...
    // vectors.
    weld_body_to_world_->MapVelocityToQDot(*context_, zero_sized_vector, &zero_sized_vector);
    weld_body_to_world_->MapQDotToVelocity(*context_, zero_sized_vector, &zero_sized_vector);
}
