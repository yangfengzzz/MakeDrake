...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<BallRpyJoint>("Joint", model->world_body(), std::nullopt, *body_, std::nullopt,
                                                kDamping);
        mutable_joint_ = dynamic_cast<BallRpyJoint<double>*>(&model->get_mutable_joint(joint_->index()));
        DRAKE_DEMAND(mutable_joint_ != nullptr);
        mutable_joint_->set_position_limits(Vector3d::Constant(kPositionLowerLimit),
                                            Vector3d::Constant(kPositionUpperLimit));
        mutable_joint_->set_velocity_limits(Vector3d::Constant(kVelocityLowerLimit),
                                            Vector3d::Constant(kVelocityUpperLimit));
        mutable_joint_->set_acceleration_limits(Vector3d::Constant(kAccelerationLowerLimit),
...
TEST_F(BallRpyJointTest, GetJointLimits) {
    ...
    EXPECT_EQ(joint_->position_lower_limits(), Vector3d::Constant(kPositionLowerLimit));
    EXPECT_EQ(joint_->position_upper_limits(), Vector3d::Constant(kPositionUpperLimit));
    EXPECT_EQ(joint_->velocity_lower_limits(), Vector3d::Constant(kVelocityLowerLimit));
    EXPECT_EQ(joint_->velocity_upper_limits(), Vector3d::Constant(kVelocityUpperLimit));
    EXPECT_EQ(joint_->acceleration_lower_limits(), Vector3d::Constant(kAccelerationLowerLimit));
    EXPECT_EQ(joint_->acceleration_upper_limits(), Vector3d::Constant(kAccelerationUpperLimit));
}
...
TEST_F(BallRpyJointTest, ContextDependentAccess) {
    ...
    EXPECT_THROW(joint_->SetDampingVector(context_.get(), Vector3d::Constant(-1)), std::exception);
}
...
TEST_F(BallRpyJointTest, AddInOneForce) {
    ...
    EXPECT_THROW(joint_->AddInOneForce(*context_, 0, some_value, &forces), std::logic_error);
}
...
TEST_F(BallRpyJointTest, SetVelocityAndAccelerationLimits) {
    ...
    mutable_joint_->set_velocity_limits(Vector3d::Constant(new_lower), Vector3d::Constant(new_upper));
    ...
    EXPECT_THROW(mutable_joint_->set_velocity_limits(Vector3d::Constant(2), Vector3d::Constant(0)), std::exception);

    // Check for acceleration limits.
    mutable_joint_->set_acceleration_limits(Vector3d::Constant(new_lower), Vector3d::Constant(new_upper));
    ...
    // Lower limit is larger than upper limit.
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(Vector3d::Constant(2), Vector3d::Constant(0)), std::exception);
}
