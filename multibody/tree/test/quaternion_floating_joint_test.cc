...
constexpr double kPositionNonZeroDefault = (kPositionLowerLimit + kPositionUpperLimit) / 2;
...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<QuaternionFloatingJoint>("Joint", model->world_body(), std::nullopt, *body_,
                                                           std::nullopt, kAngularDamping, kTranslationalDamping);
        mutable_joint_ = dynamic_cast<QuaternionFloatingJoint<double>*>(&model->get_mutable_joint(joint_->index()));
        DRAKE_DEMAND(mutable_joint_ != nullptr);
        mutable_joint_->set_position_limits(Vector7d::Constant(kPositionLowerLimit),
                                            Vector7d::Constant(kPositionUpperLimit));
        mutable_joint_->set_velocity_limits(Vector6d::Constant(kVelocityLowerLimit),
                                            Vector6d::Constant(kVelocityUpperLimit));
        mutable_joint_->set_acceleration_limits(Vector6d::Constant(kAccelerationLowerLimit),
                                                Vector6d::Constant(kAccelerationUpperLimit));

        // We are done adding modeling elements. Transfer tree to system and get
        // a Context.
        system_ = std::make_unique<internal::MultibodyTreeSystem<double>>(std::move(model), true /* is_discrete */);
        context_ = system_->CreateDefaultContext();
    }

    const internal::MultibodyTree<double>& tree() const { return internal::GetInternalTree(*system_); }
...
TEST_F(QuaternionFloatingJointTest, GetJointLimits) {
    ...
    EXPECT_EQ(joint_->position_lower_limits(), (Vector7d::Constant(kPositionLowerLimit)));
    EXPECT_EQ(joint_->position_upper_limits(), (Vector7d::Constant(kPositionUpperLimit)));
    EXPECT_EQ(joint_->velocity_lower_limits(), Vector6d::Constant(kVelocityLowerLimit));
    EXPECT_EQ(joint_->velocity_upper_limits(), Vector6d::Constant(kVelocityUpperLimit));
    EXPECT_EQ(joint_->acceleration_lower_limits(), Vector6d::Constant(kAccelerationLowerLimit));
    EXPECT_EQ(joint_->acceleration_upper_limits(), Vector6d::Constant(kAccelerationUpperLimit));
}
...
TEST_F(QuaternionFloatingJointTest, Damping) {
    ...
    EXPECT_EQ(joint_->default_damping_vector(), (Vector6d() << kAngularDamping, kAngularDamping, kAngularDamping,
...
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    ...
    EXPECT_TRUE(math::AreQuaternionsEqualForOrientation(joint_->get_quaternion(*context_), quaternion_B, kTolerance));

#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    const RotationMatrixd rotation_matrix_A(quaternion_A);
    joint_->SetFromRotationMatrix(context_.get(), rotation_matrix_A);
    EXPECT_TRUE(math::AreQuaternionsEqualForOrientation(joint_->get_quaternion(*context_), quaternion_A, kTolerance));
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    ...
    EXPECT_TRUE(joint_->get_pose(*context_).IsNearlyEqualTo(transform_A, kTolerance));
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    ...
    EXPECT_TRUE(joint_->GetPose(*context_).IsNearlyEqualTo(transform_A, kTolerance));
    ...
    EXPECT_EQ(joint_->get_translational_velocity(*context_), translational_velocity);
    ...
    // Damping.
    const Vector6d damping = (Vector6d() << kAngularDamping, kAngularDamping, kAngularDamping, kTranslationalDamping,
                              kTranslationalDamping, kTranslationalDamping)
                                     .finished();
    const Vector6d different_damping = (Vector6d() << 2.3, 2.3, 2.3, 4.5, 4.5, 4.5).finished();
    ...
    EXPECT_THROW(joint_->SetDampingVector(context_.get(), Vector6d::Constant(-1)), std::exception);
}
...
TEST_F(QuaternionFloatingJointTest, AddInOneForce) {
    ...
    EXPECT_THROW(joint_->AddInOneForce(*context_, 0, some_value, &forces), std::exception);
}
...
TEST_F(QuaternionFloatingJointTest, AddInDampingForces) {
    ...
    const Vector6d damping_forces_expected =
            (Vector6d() << -angular_damping * angular_velocity, -translational_damping * translational_veloctiy)
                    .finished();

    joint_->set_angular_velocity(context_.get(), angular_velocity);
    joint_->set_translational_velocity(context_.get(), translational_veloctiy);
    joint_->SetDampingVector(context_.get(), (Vector6d() << angular_damping, angular_damping, angular_damping,
...
TEST_F(QuaternionFloatingJointTest, Clone) {
    ...
    EXPECT_EQ(joint_clone.frame_on_parent().index(), joint_->frame_on_parent().index());
    EXPECT_EQ(joint_clone.frame_on_child().index(), joint_->frame_on_child().index());
    EXPECT_EQ(joint_clone.position_lower_limits(), joint_->position_lower_limits());
    EXPECT_EQ(joint_clone.position_upper_limits(), joint_->position_upper_limits());
    EXPECT_EQ(joint_clone.velocity_lower_limits(), joint_->velocity_lower_limits());
    EXPECT_EQ(joint_clone.velocity_upper_limits(), joint_->velocity_upper_limits());
    EXPECT_EQ(joint_clone.acceleration_lower_limits(), joint_->acceleration_lower_limits());
    EXPECT_EQ(joint_clone.acceleration_upper_limits(), joint_->acceleration_upper_limits());
    EXPECT_EQ(joint_clone.default_angular_damping(), joint_->default_angular_damping());
    EXPECT_EQ(joint_clone.default_translational_damping(), joint_->default_translational_damping());
    EXPECT_EQ(joint_clone.get_default_quaternion().coeffs(), joint_->get_default_quaternion().coeffs());
    EXPECT_EQ(joint_clone.get_default_translation(), joint_->get_default_translation());
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
...
TEST_F(QuaternionFloatingJointTest, SetVelocityAndAccelerationLimits) {
    ...
    mutable_joint_->set_velocity_limits(Vector6d::Constant(new_lower), Vector6d::Constant(new_upper));
    ...
    EXPECT_THROW(mutable_joint_->set_velocity_limits(VectorX<double>(3), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint_->set_velocity_limits(VectorX<double>(), VectorX<double>(3)), std::exception);
    // Lower limit is larger than upper limit.
    EXPECT_THROW(mutable_joint_->set_velocity_limits(Vector6d::Constant(2), Vector6d::Constant(0)), std::exception);

    // Check for acceleration limits.
    mutable_joint_->set_acceleration_limits(Vector6d::Constant(new_lower), Vector6d::Constant(new_upper));
    ...
    // Does not match num_velocities().
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(3), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(), VectorX<double>(3)), std::exception);
    // Lower limit is larger than upper limit.
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(Vector6d::Constant(2), Vector6d::Constant(0)), std::exception);
}
...
TEST_F(QuaternionFloatingJointTest, DefaultAngles) {
    ...
    const Vector7d new_default_angles = Vector7d::Constant(kPositionNonZeroDefault);

    const Vector7d out_of_bounds_low_angles = lower_limit_angles - Vector7d::Constant(1);
    const Vector7d out_of_bounds_high_angles = upper_limit_angles + Vector7d::Constant(1);
    ...
    EXPECT_NO_THROW(mutable_joint_->set_default_positions(out_of_bounds_low_angles));
    EXPECT_NO_THROW(mutable_joint_->set_default_positions(out_of_bounds_high_angles));
}
...
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    ...
    mutable_joint_->set_random_quaternion_distribution(Eigen::Quaternion<symbolic::Expression>::Identity());
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
    mutable_joint_->set_random_position_distribution(Eigen::Matrix<symbolic::Expression, 3, 1>::Zero());
#pragma GCC diagnostic pop  // pop -Wdeprecated-declarations
    mutable_joint_->set_random_translation_distribution(Eigen::Matrix<symbolic::Expression, 3, 1>::Zero());
    tree().SetRandomState(*context_, &context_->get_mutable_state(), &generator);
