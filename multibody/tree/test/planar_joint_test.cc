...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<PlanarJoint>("Joint", model->world_body(), std::nullopt, *body_, std::nullopt,
                                               Vector3d::Constant(kDamping));
        mutable_joint_ = dynamic_cast<PlanarJoint<double>*>(&model->get_mutable_joint(joint_->index()));
...
TEST_F(PlanarJointTest, AddInDampingForces) {
    ...
    const Vector3d damping_forces_expected =
            (Vector3d() << -translational_damping * translational_velocity, -angular_damping * angular_velocity)
                    .finished();
    ...
    joint_->SetDampingVector(context_.get(), Vector3d(translational_damping, translational_damping, angular_damping));
...
TEST_F(PlanarJointTest, DefaultState) {
    const Vector2d new_default_translation = Vector2d::Constant(kPositionNonZeroDefault);
    const Vector2d out_of_bounds_low_translation = Vector2d::Constant(kPositionLowerLimit - 1);
    const Vector2d out_of_bounds_high_translation = Vector2d::Constant(kPositionUpperLimit + 1);
    ...
    EXPECT_NO_THROW(mutable_joint_->set_default_translation(out_of_bounds_low_translation));
    EXPECT_NO_THROW(mutable_joint_->set_default_translation(out_of_bounds_high_translation));
    EXPECT_NO_THROW(mutable_joint_->set_default_rotation(out_of_bounds_low_rotation));
    EXPECT_NO_THROW(mutable_joint_->set_default_rotation(out_of_bounds_high_rotation));

    // Setting a new default pose should propagate so that
    // `get_default_translation()` and `get_default_rotation()` remain correct.
    mutable_joint_->set_default_pose(new_default_translation, new_default_rotation);
    EXPECT_EQ(joint_->get_default_translation(), new_default_translation);
    EXPECT_EQ(joint_->get_default_rotation(), new_default_rotation);
}
...
TEST_F(PlanarJointTest, RandomState) {
    ...
    mutable_joint_->set_random_pose_distribution(
            Vector2<symbolic::Expression>(uniform(generator) + 1.0, uniform(generator) + 2.0),
