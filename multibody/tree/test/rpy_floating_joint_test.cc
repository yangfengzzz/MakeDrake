...
#include "multibody/tree/rpy_floating_joint.h"
...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<RpyFloatingJoint>("Joint", model->world_body(), std::nullopt, *body_, std::nullopt,
                                                    kAngularDamping, kTranslationalDamping);
        mutable_joint_ = dynamic_cast<RpyFloatingJoint<double>*>(&model->get_mutable_joint(joint_->index()));
        DRAKE_DEMAND(mutable_joint_ != nullptr);
        mutable_joint_->set_position_limits(Vector6d::Constant(kPositionLowerLimit),
...
TEST_F(RpyFloatingJointTest, GetJointLimits) {
    ...
    EXPECT_EQ(joint_->position_lower_limits(), Vector6d::Constant(kPositionLowerLimit));
    EXPECT_EQ(joint_->position_upper_limits(), Vector6d::Constant(kPositionUpperLimit));
...
TEST_F(RpyFloatingJointTest, ContextDependentAccess) {
    ...
    const RotationMatrixd rotation_matrix_A = RotationMatrixd(RollPitchYawd(angles_A));
    const RotationMatrixd rotation_matrix_B = RotationMatrixd(RollPitchYawd(angles_B));
    ...
    EXPECT_TRUE(CompareMatrices(joint_->get_angles(*context_), angles_B, kTolerance));
...
TEST_F(RpyFloatingJointTest, AddInDampingForces) {
    ...
    joint_->SetDampingVector(
            context_.get(),
            (Vector6d() << Vector3d::Constant(angular_damping), Vector3d::Constant(translational_damping)).finished());
...
TEST_F(RpyFloatingJointTest, SetVelocityAndAccelerationLimits) {
    ...
    EXPECT_THROW(mutable_joint_->set_velocity_limits(VectorX<double>(6), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint_->set_velocity_limits(VectorX<double>(), VectorX<double>(6)), std::exception);
    ...
    // Does not match num_velocities().
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(6), VectorX<double>()), std::exception);
    EXPECT_THROW(mutable_joint_->set_acceleration_limits(VectorX<double>(), VectorX<double>(6)), std::exception);
    // Lower limit is larger than upper limit.
...
TEST_F(RpyFloatingJointTest, DefaultAnglesAndTranslation) {
    ...
    const Vector3d new_default_angles = Vector3d::Constant(kPositionNonZeroDefault);

    const Vector3d out_of_bounds_low_angles = lower_limit_angles - Vector3d::Constant(1);
    const Vector3d out_of_bounds_high_angles = upper_limit_angles + Vector3d::Constant(1);
    ...
    EXPECT_NO_THROW(mutable_joint_->set_default_angles(out_of_bounds_high_angles));
    ...
    EXPECT_TRUE(joint_->GetDefaultPose().IsNearlyEqualTo(
            RigidTransformd(RollPitchYawd(joint_->get_default_angles()), joint_->get_default_translation()),
            kTolerance));
}
