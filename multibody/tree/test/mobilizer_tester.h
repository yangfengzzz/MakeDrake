...
public:
    ...
    template <template <typename> class JointType, template <typename> class MobilizerType>
    const MobilizerType<double>& AddJointAndFinalize(std::unique_ptr<JointType<double>> joint) {
        // Add a joint between the world and the body:
        const JointType<double>& joint_ref = owned_tree_->AddJoint(std::move(joint));

        // We are done adding modeling elements. Transfer tree to system,
        // finalize, and get a Context.
        system_ = std::make_unique<MultibodyTreeSystem<double>>(std::move(owned_tree_));
        context_ = system_->CreateDefaultContext();

        const Mobilizer<double>& mobilizer = joint_ref.GetMobilizerInUse();
        const auto* typed_mobilizer = dynamic_cast<const MobilizerType<double>*>(&mobilizer);
    ...
    const MobilizerType<double>& AddMobilizerAndFinalize(std::unique_ptr<MobilizerType<double>> mobilizer) {
        // Add a mobilizer between the world and the body:
        const MobilizerType<double>& mobilizer_ref = owned_tree_->AddMobilizer(std::move(mobilizer));
