...
public:
    ...
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<ScrewJoint>("Joint", model->world_body(), std::nullopt, *body_, std::nullopt,
                                              kScrewPitch, kDamping);
        mutable_joint_ = dynamic_cast<ScrewJoint<double>*>(&model->get_mutable_joint(joint_->index()));
        DRAKE_DEMAND(mutable_joint_ != nullptr);
        mutable_joint_->set_position_limits(Vector1d::Constant(kPositionLowerLimit),
                                            Vector1d::Constant(kPositionUpperLimit));
        mutable_joint_->set_velocity_limits(Vector1d::Constant(kVelocityLowerLimit),
                                            Vector1d::Constant(kVelocityUpperLimit));
        mutable_joint_->set_acceleration_limits(Vector1d::Constant(kAccelerationLowerLimit),
...
TEST_F(ScrewJointTest, GetJointLimits) {
    ...
    EXPECT_EQ(joint_->position_lower_limits(), Vector1d::Constant(kPositionLowerLimit));
    EXPECT_EQ(joint_->position_upper_limits(), Vector1d::Constant(kPositionUpperLimit));
    EXPECT_EQ(joint_->velocity_lower_limits(), Vector1d::Constant(kVelocityLowerLimit));
    EXPECT_EQ(joint_->velocity_upper_limits(), Vector1d::Constant(kVelocityUpperLimit));
    EXPECT_EQ(joint_->acceleration_lower_limits(), Vector1d::Constant(kAccelerationLowerLimit));
    EXPECT_EQ(joint_->acceleration_upper_limits(), Vector1d::Constant(kAccelerationUpperLimit));
}
...
TEST_F(ScrewJointTest, ContextDependentAccess) {
    ...
    EXPECT_NO_THROW(joint_->SetDampingVector(context_.get(), Vector1d(different_damping)));
    ...
    EXPECT_THROW(joint_->SetDampingVector(context_.get(), Vector1d(-1)), std::exception);
}
...
TEST_F(ScrewJointTest, RandomState) {
    ...
    mutable_joint_->set_random_pose_distribution(Vector1<symbolic::Expression>(uniform(generator) + 1.0));
    tree().SetRandomState(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(joint_->get_rotation(*context_), 1.0);
    EXPECT_LE(joint_->get_rotation(*context_), 2.0);

    EXPECT_GE(joint_->get_translation(*context_), 1.0 / (2. * M_PI) * kScrewPitch);
    EXPECT_LE(joint_->get_translation(*context_), 2.0 / (2. * M_PI) * kScrewPitch);
