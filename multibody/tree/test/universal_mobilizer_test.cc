...
public:
    ...
    void SetUp() override {
        mobilizer_ = &AddJointAndFinalize<UniversalJoint, UniversalMobilizer>(std::make_unique<UniversalJoint<double>>(
...
TEST_F(UniversalMobilizerTest, DefaultPosition) {
    UniversalMobilizer<double>* mutable_mobilizer = &mutable_tree().get_mutable_variant(*mobilizer_);
...
TEST_F(UniversalMobilizerTest, RandomState) {
    ...
    mutable_mobilizer->set_random_position_distribution(
            Vector2<symbolic::Expression>(uniform(generator) + 2.0, uniform(generator) - 2.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    ...
    mutable_mobilizer->set_random_velocity_distribution(
            Vector2<symbolic::Expression>(uniform(generator) - 3.0, uniform(generator) + 3.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_angles(*context_)[0], 2.0);
    EXPECT_LE(mobilizer_->get_angles(*context_)[0], 3.0);
    EXPECT_GE(mobilizer_->get_angles(*context_)[1], -2.0);
...
TEST_F(UniversalMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    mobilizer_->SetAngles(context_.get(), angles);
    const SpatialVelocity<double> V_FM = mobilizer_->CalcAcrossMobilizerSpatialVelocity(*context_, angular_rates);
...
TEST_F(UniversalMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            mobilizer_->CalcAcrossMobilizerSpatialAcceleration(*context_, angular_acceleration);
    ...
    const SpatialAcceleration<double> A_FM_expected(H_expected * angular_acceleration + H_dot_expected * angular_rates);
