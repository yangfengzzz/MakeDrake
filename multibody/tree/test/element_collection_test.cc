...
GTEST_TEST(ElementCollectionTest, AddDense) {
    ...
    EXPECT_EQ(dut.get_element_unchecked(Index{1}).name(), "one");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_element(Index{}), ".*default-constructed ModelInstanceIndex.*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_mutable_element(Index{}), ".*default-constructed ModelInstanceIndex.*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_element(Index{22}), ".*ModelInstanceIndex.*22.*bounds.*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_mutable_element(Index{22}), ".*ModelInstanceIndex.*22.*bounds.*");
...
GTEST_TEST(ElementCollectionTest, Removals) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_element(Index{2}), ".*ModelInstanceIndex.*2.*removed.*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_mutable_element(Index{2}), ".*ModelInstanceIndex.*2.*removed.*");
    ...
    EXPECT_THAT(dut.indices(), testing::ElementsAre(Index{0}, Index{1}, Index{3}));
    ...
    EXPECT_THAT(dut.indices(), testing::ElementsAre(Index{3}, Index{4}, Index{5}));
...
GTEST_TEST(ElementCollectionTest, AllocateThenAdd) {
    ...
    const std::vector<std::string> expected_names{std::string{"zero"}, std::string{"one"}, std::string{"two"},
