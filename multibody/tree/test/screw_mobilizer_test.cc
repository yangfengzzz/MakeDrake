#include "multibody/tree/screw_mobilizer.h"
...
#include "multibody/tree/test/mobilizer_tester.h"
...
public:
    ...
    void SetUp() override {
        mobilizer_ = &AddJointAndFinalize<ScrewJoint, ScrewMobilizer>(std::make_unique<ScrewJoint<double>>(
                "joint0", tree().world_body().body_frame(), body_->body_frame(), kScrewAxis, kScrewPitch, 0.0));
    }
...
TEST_F(ScrewMobilizerTest, ExceptionRaisingWhenZeroPitch) {
    ...
    ScrewMobilizer<double> zero_pitch_screw_mobilizer(tree().world_body().body_frame(), body_->body_frame(),
                                                      Vector3<double>::UnitZ(), zero_screw_pitch);

    const double translation_z{1.0};
    const double velocity_z{2.0};
    EXPECT_THROW(zero_pitch_screw_mobilizer.SetTranslation(context_.get(), translation_z), std::exception);
    EXPECT_THROW(zero_pitch_screw_mobilizer.SetTranslationRate(context_.get(), velocity_z), std::exception);
}
...
TEST_F(ScrewMobilizerTest, StateAccess) {
    ...
    const double angular_velocity_z_second{velocity_z_second * 2 * M_PI / kScrewPitch};
...
TEST_F(ScrewMobilizerTest, ZeroState) {
    ...
    EXPECT_LE(std::fabs(mobilizer_->get_translation(*context_) - translation_z), kTolerance);
    EXPECT_LE(std::fabs(mobilizer_->get_translation_rate(*context_) - velocity_z), kTolerance);
...
TEST_F(ScrewMobilizerTest, DefaultPosition) {
    ScrewMobilizer<double>* mutable_mobilizer = &mutable_tree().get_mutable_variant(*mobilizer_);
    ...
    EXPECT_EQ(mobilizer_->get_translation(*context_), new_default(0) * kScrewPitch / (2.0 * M_PI));
}
...
TEST_F(ScrewMobilizerTest, RandomState) {
    ...
    mobilizer_->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_EQ(mobilizer_->get_angle(*context_), 0.0);
    EXPECT_EQ(mobilizer_->get_angular_rate(*context_), 0.0);

    // Set position to be random, but not velocity (yet).
    mutable_mobilizer->set_random_position_distribution(Vector1<symbolic::Expression>(uniform(generator) + 1.0));
    mobilizer_->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_angle(*context_), 1.0);
    EXPECT_EQ(mobilizer_->get_angular_rate(*context_), 0.0);

    // Set the velocity distribution.  Now both should be random.
    mutable_mobilizer->set_random_velocity_distribution(Vector1<symbolic::Expression>(uniform(generator) - 1.0));
    mobilizer_->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_angle(*context_), 1.0);
    ...
    const double last_translational_rate = mobilizer_->get_translation_rate(*context_);
    ...
    EXPECT_NE(mobilizer_->get_translation_rate(*context_), last_translational_rate);
...
TEST_F(ScrewMobilizerTest, CalcAcrossMobilizerTransform) {
    ...
    const RigidTransformd X_FM(mobilizer_->CalcAcrossMobilizerTransform(*context_));

    Vector3d X_FM_translation;
    X_FM_translation << kScrewAxis * angle / (2 * M_PI) * kScrewPitch;
    const RigidTransformd X_FM_expected(Eigen::AngleAxis<double>(angle, kScrewAxis), X_FM_translation);

    EXPECT_TRUE(CompareMatrices(X_FM.GetAsMatrix34(), X_FM_expected.GetAsMatrix34(), kTolerance,
...
TEST_F(ScrewMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    mobilizer_->SetAngle(context_.get(), angle);
    const SpatialVelocity<double> V_FM = mobilizer_->CalcAcrossMobilizerSpatialVelocity(*context_, angular_velocity);

    VectorXd v_expected(6);
    v_expected << kScrewAxis * angular_velocity[0], kScrewAxis * angular_velocity[0] / (2 * M_PI) * kScrewPitch;
...
TEST_F(ScrewMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            mobilizer_->CalcAcrossMobilizerSpatialAcceleration(*context_, angle_acceleration);

    VectorXd a_expected(6);
    a_expected << kScrewAxis * angle_acceleration[0], kScrewAxis * angle_acceleration[0] / (2 * M_PI) * kScrewPitch;
...
TEST_F(ScrewMobilizerTest, ProjectSpatialForce) {
    ...
    const Vector1d tau_expected(torque_Mo_F.dot(kScrewAxis) + force_Mo_F.dot(kScrewAxis) / (2 * M_PI) * kScrewPitch);
...
TEST_F(ScrewMobilizerTest, MapVelocityToQDotAndBack) {
    ...
    EXPECT_TRUE(CompareMatrices(qdot, v, kTolerance, MatrixCompareType::relative));

    qdot(0) = -std::sqrt(2);
    mobilizer_->MapQDotToVelocity(*context_, qdot, &v);
    EXPECT_TRUE(CompareMatrices(v, qdot, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(ScrewMobilizerTest, KinematicMapping) {
    ...
    tree().GetMutablePositionsAndVelocities(context_.get()).setConstant(std::numeric_limits<double>::quiet_NaN());
