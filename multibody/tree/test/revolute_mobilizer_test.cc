...
public:
    void SetUp() override {
        mobilizer_ = &AddJointAndFinalize<RevoluteJoint, RevoluteMobilizer>(std::make_unique<RevoluteJoint<double>>(
                "joint0", tree().world_body().body_frame(), body_->body_frame(), axis_F_));
    }
...
TEST_F(RevoluteMobilizerTest, AxisIsNormalizedAtConstruction) {
    EXPECT_TRUE(CompareMatrices(mobilizer_->revolute_axis(), axis_F_.normalized(), kTolerance,
...
TEST_F(RevoluteMobilizerTest, DefaultPosition) {
    RevoluteMobilizer<double>* mutable_mobilizer = &mutable_tree().get_mutable_variant(*mobilizer_);
...
TEST_F(RevoluteMobilizerTest, RandomState) {
    ...
    mutable_mobilizer->set_random_position_distribution(Vector1<symbolic::Expression>(uniform(generator) + 2.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_angle(*context_), 2.0);
    EXPECT_EQ(mobilizer_->get_angular_rate(*context_), 0);

    // Set the velocity distribution.  Now both should be random.
    mutable_mobilizer->set_random_velocity_distribution(Vector1<symbolic::Expression>(uniform(generator) - 2.0));
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_GE(mobilizer_->get_angle(*context_), 2.0);
...
TEST_F(RevoluteMobilizerTest, CalcAcrossMobilizerTransform) {
    ...
    const RigidTransformd X_FM_expected(RotationMatrixd(AngleAxisd(angle, axis_F_.normalized())));
...
TEST_F(RevoluteMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    const SpatialVelocity<double> V_FM =
            mobilizer_->CalcAcrossMobilizerSpatialVelocity(*context_, Vector1d(angular_rate));

    const SpatialVelocity<double> V_FM_expected(axis_F_.normalized() * angular_rate, Vector3d::Zero());
...
TEST_F(RevoluteMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            mobilizer_->CalcAcrossMobilizerSpatialAcceleration(*context_, Vector1d(angular_acceleration));

    const SpatialAcceleration<double> A_FM_expected(axis_F_.normalized() * angular_acceleration, Vector3d::Zero());
