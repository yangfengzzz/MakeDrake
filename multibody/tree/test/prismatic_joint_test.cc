...
public:
    ...
    std::unique_ptr<internal::MultibodyTree<double>> MakeModel() {
        ...
        joint1_ =
                &model->AddJoint<PrismaticJoint>("Joint1", model->world_body(), std::nullopt, *body1_, std::nullopt,
...
TEST_F(PrismaticJointTest, Clone) {
    ...
    EXPECT_EQ(joint1_clone.get_default_translation(), joint1_->get_default_translation());
}
...
TEST_F(PrismaticJointTest, RandomTranslationTest) {
    ...
    std::uniform_real_distribution<symbolic::Expression> uniform(1.0, kPositionUpperLimit);
...
TEST_F(PrismaticJointTest, DefaultTranslation) {
    ...
    const double new_default_translation = 0.5 * kPositionLowerLimit + 0.5 * kPositionUpperLimit;
    ...
    EXPECT_NO_THROW(mutable_joint1_->set_default_translation(out_of_bounds_low_translation));
    EXPECT_NO_THROW(mutable_joint1_->set_default_translation(out_of_bounds_high_translation));
}
