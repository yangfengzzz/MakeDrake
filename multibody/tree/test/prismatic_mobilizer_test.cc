...
public:
    void SetUp() override {
        slider_ = &AddJointAndFinalize<PrismaticJoint, PrismaticMobilizer>(std::make_unique<PrismaticJoint<double>>(
...
TEST_F(PrismaticMobilizerTest, AxisIsNormalizedAtConstruction) {
    EXPECT_TRUE(CompareMatrices(slider_->translation_axis(), axis_F_.normalized(), kTolerance,
...
TEST_F(PrismaticMobilizerTest, CalcAcrossMobilizerTransform) {
    ...
    const math::RigidTransformd X_FM(slider_->CalcAcrossMobilizerTransform(*context_));

    const math::RigidTransformd X_FM_expected(Vector3d(axis_F_.normalized() * translation));
...
TEST_F(PrismaticMobilizerTest, CalcAcrossMobilizerSpatialVeloctiy) {
    ...
    const SpatialVelocity<double> V_FM =
            slider_->CalcAcrossMobilizerSpatialVelocity(*context_, Vector1d(translation_rate));

    const SpatialVelocity<double> V_FM_expected(Vector3d::Zero(), axis_F_.normalized() * translation_rate);
...
TEST_F(PrismaticMobilizerTest, CalcAcrossMobilizerSpatialAcceleration) {
    ...
    const SpatialAcceleration<double> A_FM =
            slider_->CalcAcrossMobilizerSpatialAcceleration(*context_, Vector1d(translational_acceleration));

    const SpatialAcceleration<double> A_FM_expected(Vector3d::Zero(),
                                                    axis_F_.normalized() * translational_acceleration);
