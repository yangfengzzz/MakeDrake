#include "multibody/tree/linear_spring_damper.h"
...
public:
    void SetUp() override {
        ...
        const SpatialInertia<double> point_mass(1., Eigen::Vector3d::Zero(), UnitInertia<double>(0., 0., 0.));
        bodyA_ = &plant_.AddRigidBody("BodyA", point_mass);
        bodyB_ = &plant_.AddRigidBody("BodyB", point_mass);

        plant_.AddJoint<WeldJoint>("WeldBodyAToWorld", plant_.world_body(), std::nullopt, *bodyA_, std::nullopt,
                                   math::RigidTransform<double>::Identity());

        // Allow body B to slide along the x axis.
        slider_ = &plant_.AddJoint<PrismaticJoint>("Slider", plant_.world_body(), std::nullopt, *bodyB_, std::nullopt,
                                                   Vector3<double>::UnitX());

        spring_damper_ = &plant_.AddForceElement<LinearSpringDamper>(*bodyA_, p_AP_, *bodyB_, p_BQ_, free_length_,
                                                                     stiffness_, damping_);
    ...
    void CalcSpringDamperForces() const {
        forces_->SetZero();
        spring_damper_->CalcAndAddForceContribution(*context_, plant_.EvalPositionKinematics(*context_),
...
GTEST_TEST(LinearSpringDamper, BadParameters) {
    ...
    const auto& bodyA = plant.AddRigidBody("BodyA", SpatialInertia<double>::NaN());
    const auto& bodyB = plant.AddRigidBody("BodyB", SpatialInertia<double>::NaN());
    ...
    EXPECT_NO_THROW(
            plant.AddForceElement<LinearSpringDamper>(bodyA, p_AP, bodyB, p_BQ, free_length, stiffness, damping));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddForceElement<LinearSpringDamper>(
                                        bodyA, p_AP, bodyB, p_BQ, -1.0 /* negative rest length */, stiffness, damping),
                                ".*condition 'free_length > 0' failed.*");

    DRAKE_EXPECT_THROWS_MESSAGE(plant.AddForceElement<LinearSpringDamper>(bodyA, p_AP, bodyB, p_BQ, free_length,
...
TEST_F(SpringDamperTester, RestLength) {
    ...
    const double potential_energy =
            spring_damper_->CalcPotentialEnergy(*context_, plant_.EvalPositionKinematics(*context_));
...
TEST_F(SpringDamperTester, LengthLargerThanRestLength) {
    ...
    const double expected_torque_magnitude = expected_force_magnitude * torque_arm_length_;
    const SpatialForce<double> F_A_W_expected(Vector3<double>(0, 0, -expected_torque_magnitude),
                                              Vector3<double>(expected_force_magnitude, 0, 0));
    EXPECT_TRUE(
            CompareMatrices(F_A_W.get_coeffs(), F_A_W_expected.get_coeffs(), kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(
            CompareMatrices(F_B_W.get_coeffs(), -F_A_W_expected.get_coeffs(), kTolerance, MatrixCompareType::relative));

    // Verify the value of the potential energy.
    const double potential_energy_expected = 0.5 * stiffness_ * (length - free_length_) * (length - free_length_);
    const double potential_energy =
            spring_damper_->CalcPotentialEnergy(*context_, plant_.EvalPositionKinematics(*context_));
    ...
    const double conservative_power = spring_damper_->CalcConservativePower(
            *context_, plant_.EvalPositionKinematics(*context_), plant_.EvalVelocityKinematics(*context_));
    EXPECT_EQ(conservative_power, 0.0);

    const double non_conservative_power = spring_damper_->CalcNonConservativePower(
            *context_, plant_.EvalPositionKinematics(*context_), plant_.EvalVelocityKinematics(*context_));
...
TEST_F(SpringDamperTester, Power) {
    ...
    const double conservative_power = spring_damper_->CalcConservativePower(
            *context_, plant_.EvalPositionKinematics(*context_), plant_.EvalVelocityKinematics(*context_));
    const double conservative_power_expected = -stiffness_ * (length - free_length_) * length_dot;
    EXPECT_NEAR(conservative_power, conservative_power_expected, kTolerance);

    const double non_conservative_power = spring_damper_->CalcNonConservativePower(
            *context_, plant_.EvalPositionKinematics(*context_), plant_.EvalVelocityKinematics(*context_));
    const double non_conservative_power_expected = -damping_ * length_dot * length_dot;
    // It should always be non-positive.
    EXPECT_LT(non_conservative_power, 0.0);
    EXPECT_NEAR(non_conservative_power, non_conservative_power_expected, kTolerance);
