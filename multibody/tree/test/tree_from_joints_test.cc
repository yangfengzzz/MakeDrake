...
public:
    ...
    DoublePendulumModel() {
        ...
        UnitInertia<double> G1_L1 = UnitInertia<double>::StraightLine(Ic1_, Vector3d::UnitY());
        ...
        UnitInertia<double> G2_L2cm = UnitInertia<double>::StraightLine(Ic2_, Vector3d::UnitY());
        ...
        shoulder_ = &model->template AddJoint<RevoluteJoint>(
                "ShoulderJoint", *world_body_, {}, /* Default to Identity; frame Si IS the world frame W. */
        ...
        elbow_ = &model->template AddJoint<RevoluteJoint>("ElbowJoint", *link1_, X_L1Ei, /* Pose of Ei in L1. */
        ...
        gravity_element_->set_gravity_vector(Vector3d(0.0, -acceleration_of_gravity_, 0.0));
    ...
    const UniformGravityFieldElement<T>& gravity_element() const { return *gravity_element_; }

    const internal::MultibodyTree<double>& tree() const { return internal::GetInternalTree(*system_); }

    std::unique_ptr<Context<double>> CreateDefaultContext() const { return system_->CreateDefaultContext(); }
...
public:
    ...
    void VerifyCalcMassMatrixViaInverseDynamics(double theta1, double theta2) {
        ...
        EXPECT_TRUE(CompareMatrices(H, H_expected, kTolerance, MatrixCompareType::relative));
    }
    ...
    void VerifyGravityGeneralizedForces(double theta1, double theta2) const {
        ...
        const UniformGravityFieldElement<double>& gravity_element = pendulum_.gravity_element();

        Vector2d tau_g = gravity_element.CalcGravityGeneralizedForces(*context_);

        Vector2d tau_g_expected = acrobot_benchmark_.CalcGravityVector(theta1, theta2);
    ...
    void VerifyInverseDynamicsWithAppliedForces(
            double theta1, double theta2, double theta1dot, double theta2dot, double tau1, double tau2) {
        const double kTolerance = 10 * kEpsilon;
        ...
        tree().CalcInverseDynamics(*context_, vdot, F_BBo_W_array, tau_array, &A_WB_array, &F_BMo_W,
                                   &tau /* Output forces */);

        // Now compute inverse dynamics using our benchmark:
        Vector2d tau_expected(tau1, tau2);
        Vector2d C_expected = acrobot_benchmark_.CalcCoriolisVector(theta1, theta2, theta1dot, theta2dot);
        Vector2d tau_g_expected = acrobot_benchmark_.CalcGravityVector(theta1, theta2);
        Vector2d rhs = C_expected - tau_g_expected - tau_expected;

        EXPECT_TRUE(CompareMatrices(tau, rhs, kTolerance, MatrixCompareType::relative));

        // Verify alternative APIs for inverse dynamics.
        const VectorX<double> tau_id2 = tree().CalcInverseDynamics(*context_, vdot, forces);
        EXPECT_TRUE(CompareMatrices(tau_id2, rhs, kTolerance, MatrixCompareType::relative));
    }

    const internal::MultibodyTree<double>& tree() const { return pendulum_.tree(); }
...
TEST_F(PendulumTests, CalcForwardDynamicsViaExplicitMassMatrixSolve) {
    ...
    VerifyInverseDynamicsWithAppliedForces(M_PI / 3.0, 0.0,        /* joint's angles */
                                           0.0, 0.0,               /* joint's angular rates */
                                           0.0, 0.0);              /* joint's torques */
    VerifyInverseDynamicsWithAppliedForces(M_PI / 3.0, M_PI / 2.0, /* joint's angles */
                                           0.0, 0.0,               /* joint's angular rates */
                                           0.0, 0.0);              /* joint's torques */
    VerifyInverseDynamicsWithAppliedForces(M_PI / 3.0, M_PI / 3.0, /* joint's angles */
                                           0.0, 0.0,               /* joint's angular rates */
                                           0.0, 0.0);              /* joint's torques */
    VerifyInverseDynamicsWithAppliedForces(M_PI / 3.0, M_PI / 4.0, /* joint's angles */
                                           0.0, 0.0,               /* joint's angular rates */
                                           0.0, 0.0);              /* joint's torques */

    // With non-zero velocities and zero torques:
    VerifyInverseDynamicsWithAppliedForces(-M_PI / 5.0, M_PI / 2.0, /* joint's angles */
                                           0.5, 1.0,                /* joint's angular rates */
                                           0.0, 0.0);               /* joint's torques */
    VerifyInverseDynamicsWithAppliedForces(-M_PI / 5.0, M_PI / 3.0, /* joint's angles */
                                           0.5, 1.0,                /* joint's angular rates */
                                           0.0, 0.0);               /* joint's torques */
    VerifyInverseDynamicsWithAppliedForces(-M_PI / 5.0, M_PI / 2.0, /* joint's angles */
