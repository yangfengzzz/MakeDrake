...
public:
    ...
    void SetUp() override {
        mobilizer_ = &AddJointAndFinalize<BallRpyJoint, RpyBallMobilizer>(std::make_unique<BallRpyJoint<double>>(
...
TEST_F(RpyBallMobilizerTest, StateAccess) {
    ...
    EXPECT_TRUE(
            CompareMatrices(mobilizer_->get_angles(*context_), rpy.vector(), kTolerance, MatrixCompareType::relative));
}
...
TEST_F(RpyBallMobilizerTest, KinematicMapping) {
    ...
    EXPECT_TRUE(CompareMatrices(N_x_Nplus, Matrix3d::Identity(), kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(Nplus_x_N, Matrix3d::Identity(), kTolerance, MatrixCompareType::relative));
}
