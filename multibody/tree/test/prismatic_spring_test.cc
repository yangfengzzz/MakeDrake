...
public:
    void SetUp() override {
        ...
        model->AddJoint<WeldJoint>("WeldBodyAToWorld", model->world_body(), {}, *bodyA_, {},
                                   math::RigidTransform<double>::Identity());

        // Allow body B to translate along the z axis.
        joint_ = &model->AddJoint<PrismaticJoint>("joint_AB", *bodyA_, std::nullopt, *bodyB_, std::nullopt,
                                                  Vector3<double>::UnitZ());

        // Add spring
        spring_ = &model->AddForceElement<PrismaticSpring>(*joint_, nominal_position_, stiffness_);
    ...
    void CalcSpringForces() const {
        forces_->SetZero();
        spring_->CalcAndAddForceContribution(*context_, tree().EvalPositionKinematics(*context_),
                                             tree().EvalVelocityKinematics(*context_), forces_.get());
    }

    const MultibodyTree<double>& tree() const { return GetInternalTree(*system_); }
...
TEST_F(SpringTester, NominalPosition) {
    ...
    EXPECT_TRUE(CompareMatrices(generalized_forces, VectorX<double>::Zero(1), kTolerance, MatrixCompareType::relative));

    // Verify the potential energy is zero.
    const double potential_energy = spring_->CalcPotentialEnergy(*context_, tree().EvalPositionKinematics(*context_));
...
TEST_F(SpringTester, DeltaPosition) {
    ...
    const double expected_force_magnitude = stiffness_ * (nominal_position_ - position);
    VectorX<double> expected_generalized_forces(1);
    expected_generalized_forces << expected_force_magnitude;
    EXPECT_TRUE(
            CompareMatrices(generalized_forces, expected_generalized_forces, kTolerance, MatrixCompareType::relative));

    // Verify the value of the potential energy.
    const double potential_energy_expected =
            0.5 * stiffness_ * (position - nominal_position_) * (position - nominal_position_);
    const double potential_energy = spring_->CalcPotentialEnergy(*context_, tree().EvalPositionKinematics(*context_));
    ...
    const double conservative_power = spring_->CalcConservativePower(
            *context_, tree().EvalPositionKinematics(*context_), tree().EvalVelocityKinematics(*context_));
...
TEST_F(SpringTester, Power) {
    ...
    const double conservative_power = spring_->CalcConservativePower(
            *context_, tree().EvalPositionKinematics(*context_), tree().EvalVelocityKinematics(*context_));
    const double conservative_power_expected = -stiffness_ * (position - nominal_position_) * position_dot;
...
TEST_F(SpringTester, Clone) {
    ...
    const auto& force_element_clone = model_clone->get_force_element(ForceElementIndex(1));
