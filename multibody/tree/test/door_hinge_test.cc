...
#include "systems/analysis/initial_value_problem.h"
...
protected:
    ...
    const DoorHinge<double>& BuildDoorHinge(const DoorHingeConfig& config) {
        ...
        revolute_joint_ = &plant_->AddJoint<RevoluteJoint>(kRevoluteJointName, plant_->world_body(), {}, body1, {},
    ...
    double CalcHingePotentialEnergy() const {
        return door_hinge_->CalcPotentialEnergy(*plant_context_, plant_->EvalPositionKinematics(*plant_context_));
    }

    double CalcHingeConservativePower() const {
        return door_hinge_->CalcConservativePower(*plant_context_, plant_->EvalPositionKinematics(*plant_context_),
                                                  plant_->EvalVelocityKinematics(*plant_context_));
    }

    double CalcHingeNonConservativePower() const {
        return door_hinge_->CalcNonConservativePower(*plant_context_, plant_->EvalPositionKinematics(*plant_context_),
                                                     plant_->EvalVelocityKinematics(*plant_context_));
    }
    ...
    void TestPotentialEnergyCalculation() {
        ...
        auto energy_ode = [&door_hinge_temp](const double& t, const VectorX<double>& x,
        ...
        const double target_angle = plant_->GetJointByName(kRevoluteJointName).GetOnePosition(*plant_context_);
        const double angular_rate = plant_->GetJointByName(kRevoluteJointName).GetOneVelocity(*plant_context_);
        ...
        const VectorX<double> init_state = (VectorX<double>(3) << 0.0, 0.0, angular_rate).finished();

        systems::InitialValueProblem<double> ivp(energy_ode, init_state, kDefaultParameters);
    ...
    const systems::Context<double>& plant_context() const { return *plant_context_; }
...
TEST_F(DoorHingeTest, CloneTest) {
    ...
    const DoorHinge<AutoDiffXd>& door_hinge_ad = plant_ad->GetForceElement<DoorHinge>(ForceElementIndex(1));

    EXPECT_EQ(door_hinge_ad.config().spring_zero_angle_rad, config.spring_zero_angle_rad);
    EXPECT_EQ(door_hinge_ad.config().spring_constant, config.spring_constant);
    EXPECT_EQ(door_hinge_ad.config().dynamic_friction_torque, config.dynamic_friction_torque);
    EXPECT_EQ(door_hinge_ad.config().static_friction_torque, config.static_friction_torque);
...
TEST_F(DoorHingeTest, SpringTest) {
    ...
    // be zero.
    EXPECT_EQ(CalcHingeConservativePower(), dut.CalcHingeSpringTorque(kAngle) * kAngularRate);
...
TEST_F(DoorHingeTest, StaticFrictionTest) {
    ...
    SetHingeJointState(kAngle, kAngularRate);
    EXPECT_EQ(CalcHingeNonConservativePower(), dut.CalcHingeFrictionalTorque(kAngularRate) * kAngularRate);
...
double CalcSystemTotalEnergy(const systems::Simulator<double>& simulator, const MultibodyPlant<double>& plant) {
...
void TestEnergyConservation(const MultibodyPlant<double>& plant, const Eigen::Vector2d& init_state) {
    systems::Simulator<double> simulator(plant);
    simulator.Initialize();

    double non_conserv_work = 0.0;
    double prev_time = 0.0;
    simulator.set_monitor([&plant, &prev_time, &non_conserv_work](const systems::Context<double>& root_context) {
        ...
        const double non_conserv_power = plant.EvalNonConservativePower(root_context);
    ...
    init_plant_context.get_mutable_discrete_state_vector().SetFromVector(init_state);
    ...
    DRAKE_DEMAND(velocity_change > kMinVelocityChangeRatio * std::abs(init_state[1]));
