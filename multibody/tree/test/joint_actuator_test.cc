...
GTEST_TEST(JointActuatorTest, JointActuatorLimitTest) {
    ...
    const Joint<double>& body1_world = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism1", tree.world_body().body_frame(), body1->body_frame(), Eigen::Vector3d(0, 0, 1)));
    ...
    JointActuator<double>& mutable_actuator1 = tree.get_mutable_joint_actuator(actuator1.index());
    ...
    const Joint<double>& body2_body1 = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism2", body1->body_frame(), body2->body_frame(), Eigen::Vector3d(0, 0, 1)));
    DRAKE_EXPECT_THROWS_MESSAGE(tree.AddJointActuator("act2", body2_body1, kNegativeEffortLimit),
                                "Effort limit must be strictly positive!");

    // Throw if the effort limit is set to be negative.
    const Joint<double>& body3_body2 = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism3", body2->body_frame(), body3->body_frame(), Eigen::Vector3d(0, 0, 1)));
    DRAKE_EXPECT_THROWS_MESSAGE(tree.AddJointActuator("act3", body3_body2, kZeroEffortLimit),
                                "Effort limit must be strictly positive!");

    DRAKE_EXPECT_THROWS_MESSAGE(actuator1.input_start(), ".*after the MultibodyPlant is finalized.");
    DRAKE_EXPECT_THROWS_MESSAGE(actuator1.num_inputs(), ".*after the MultibodyPlant is finalized.");

    const auto body4 = &tree.AddRigidBody("body4", M_B);
    const Joint<double>& body4_world = tree.AddJoint(std::make_unique<PlanarJoint<double>>(
            "planar4", tree.world_body().body_frame(), body4->body_frame(), Eigen::Vector3d{0, 0, 0.1}));

    const auto& actuator4 = tree.AddJointActuator("act4", body4_world, kPositiveEffortLimit);
    ...
    EXPECT_THAT(tree.GetJointActuatorIndices(), testing::ElementsAre(actuator1.index(), actuator4.index()));
}
...
GTEST_TEST(JointActuatorTest, RemoveJointActuatorTest) {
    ...
    const Joint<double>& body1_world = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism1", tree.world_body().body_frame(), body1->body_frame(), Eigen::Vector3d(0, 0, 1)));
    const JointActuator<double>& actuator1 = tree.AddJointActuator("act1", body1_world, kPositiveEffortLimit);

    const Joint<double>& body2_body1 = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism2", body1->body_frame(), body2->body_frame(), Eigen::Vector3d(0, 0, 1)));
    const JointActuator<double>& actuator2 = tree.AddJointActuator("act2", body2_body1, kPositiveEffortLimit);

    const Joint<double>& body3_body2 = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism3", body2->body_frame(), body3->body_frame(), Eigen::Vector3d(0, 0, 1)));
    const JointActuator<double>& actuator3 = tree.AddJointActuator("act3", body3_body2, kPositiveEffortLimit);

    const Joint<double>& body4_world = tree.AddJoint(std::make_unique<PrismaticJoint<double>>(
            "prism4", tree.world_body().body_frame(), body4->body_frame(), Eigen::Vector3d(0, 0, 1)));
    const JointActuator<double>& actuator4 = tree.AddJointActuator("act4", body4_world, kPositiveEffortLimit);
    ...
    EXPECT_THAT(tree.GetJointActuatorIndices(),
                testing::ElementsAre(actuator1_index, actuator2_index, actuator3_index, actuator4_index));
    ...
    EXPECT_THAT(tree.GetJointActuatorIndices(), testing::ElementsAre(actuator2_index, actuator4_index));
    ...
    const JointActuator<double>& new_actuator3 = tree.AddJointActuator("act3", body3_body2, 2 * kPositiveEffortLimit);
    JointActuatorIndex new_actuator3_index = new_actuator3.index();
    EXPECT_EQ(tree.num_actuators(), 3);
    EXPECT_EQ(tree.num_actuated_dofs(), 3);
    EXPECT_TRUE(tree.has_joint_actuator(new_actuator3_index));
    EXPECT_THAT(tree.GetJointActuatorIndices(),
                testing::ElementsAre(actuator2_index, actuator4_index, new_actuator3_index));
    ...
    EXPECT_EQ(tree.GetJointActuatorByName("act3", model_instance1).index(), new_actuator3.index());
    EXPECT_EQ(tree.GetJointActuatorByName("act3").effort_limit(), 2 * kPositiveEffortLimit);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(tree.RemoveJointActuator(actuator2),
                                "Post-finalize calls to 'RemoveJointActuator.*' are not allowed.*");

    // Confirm that removed actuator logic is preserved after cloning.
    std::unique_ptr<internal::MultibodyTree<double>> clone = tree.CloneToScalar<double>();
    ...
    EXPECT_EQ(clone->GetJointActuatorByName("act3").index(), new_actuator3.index());
    EXPECT_EQ(clone->GetJointActuatorByName("act3", model_instance1).index(), new_actuator3.index());
    EXPECT_EQ(actuator2.input_start(), 0);
    EXPECT_EQ(actuator4.input_start(), 1);
    EXPECT_EQ(new_actuator3.input_start(), 2);
    EXPECT_EQ(actuator2.num_inputs(), 1);
