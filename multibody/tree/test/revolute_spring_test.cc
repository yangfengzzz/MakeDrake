...
public:
    void SetUp() override {
        ...
        joint_ = &model->AddJoint<RevoluteJoint>("joint_AB", *bodyA_, std::nullopt, *bodyB_, std::nullopt,
                                                 Vector3<double>::UnitZ());

        model->AddJoint<RevoluteJoint>("joint_BC", *bodyB_, std::nullopt, *bodyC_, std::nullopt,
                                       Vector3<double>::UnitX());

        // Add spring
        spring_ = &model->AddForceElement<RevoluteSpring>(*joint_, nominal_angle_, stiffness_);
...
TEST_F(SpringTester, DeltaAngle) {
    ...
    const double expected_torque_magnitude = stiffness_ * (nominal_angle_ - angle);
    ...
    const double potential_energy_expected = 0.5 * stiffness_ * (angle - nominal_angle_) * (angle - nominal_angle_);
    const double potential_energy = spring_->CalcPotentialEnergy(*context_, tree().EvalPositionKinematics(*context_));
...
TEST_F(SpringTester, Power) {
    ...
    const double conservative_power = spring_->CalcConservativePower(
            *context_, tree().EvalPositionKinematics(*context_), tree().EvalVelocityKinematics(*context_));
    const double conservative_power_expected = -stiffness_ * (angle - nominal_angle_) * angle_dot;
