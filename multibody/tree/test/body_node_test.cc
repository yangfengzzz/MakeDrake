...
public:
    ...
    static void CallLltFactorization(const BodyNode<double>& node, const MatrixUpTo6<double>& D_B) {
...
public:
    DummyBody(std::string name, BodyIndex index)
        : RigidBody<double>(std::move(name), ModelInstanceIndex(0), SpatialInertia<double>::NaN()) {
...
GTEST_TEST(BodyNodeTest, FactorArticulatedBodyHingeInertiaMatrixErrorMessages) {
    ...
    {
        // Rotation only.
        const RevoluteMobilizer<double> mobilizer(parent.body_frame(), child.body_frame(), Vector3d{0, 0, 1});
        const BodyNode<double> body_node(&parent_node, &child, &mobilizer);
        DRAKE_EXPECT_THROWS_MESSAGE(BodyNodeTester::CallLltFactorization(body_node, one_by_one),
    ...
    {
        // Translation only.
        const PrismaticMobilizer<double> mobilizer(parent.body_frame(), child.body_frame(), Vector3d{0, 0, 1});
        const BodyNode<double> body_node(&parent_node, &child, &mobilizer);
        DRAKE_EXPECT_THROWS_MESSAGE(BodyNodeTester::CallLltFactorization(body_node, one_by_one),
                                    "An internal mass matrix associated with the joint that connects body "
                                    "parent to body child is not positive-definite. Since the joint allows "
...

    {
        ...
        // space is irrelevant.
        const PlanarMobilizer<double> mobilizer(parent.body_frame(), child.body_frame());
        const BodyNode<double> body_node(&parent_node, &child, &mobilizer);
...
GTEST_TEST(BodyNodeTest, FactorHingeMatrixThrows) {
    ...
    const RevoluteMobilizer<double> mobilizer(world.body_frame(), body.body_frame(), Vector3d{0, 0, 1});
    ...
    for (const double value : {1.0e-120, 1.5, 1.0e120}) {
        ...
        EXPECT_NO_THROW(Tester::CallLltFactorization(body_node, one_by_one)) << "For expected good value: " << value;
    }

    // Values that should throw. Any non-positive value should throw.
    for (const double value : {0.0, -1.0e-15, -1.5, -1.0e10}) {
        one_by_one(0, 0) = value;
        EXPECT_THROW(Tester::CallLltFactorization(body_node, one_by_one), std::exception)
    ...
    const RotationMatrix<double> R = RotationMatrix<double>(AngleAxisd(M_PI / 7, Vector3d{1, 2, 3}.normalized()));
    ...
    for (const auto& K_eigen_values : {// A very small matrix.
        ...
        EXPECT_NO_THROW(Tester::CallLltFactorization(body_node, six_by_six))
                << fmt::format("For expected bad eigenvalues: {}", fmt_eigen(K_eigen_values.transpose()));
    }
    ...
    for (const auto& K_eigen_values : {// Negative eigenvalue.
                                       Vector3d{1.1e-12, -2e-12, 3e-12},
