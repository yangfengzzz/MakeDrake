...
public:
    ...
    void SetUp() override {
        mobilizer_ =
                &AddJointAndFinalize<RpyFloatingJoint, RpyFloatingMobilizer>(std::make_unique<RpyFloatingJoint<double>>(
    ...
    RollPitchYawd arbitrary_rpy() const { return RollPitchYawd(M_PI / 3, -M_PI / 3, M_PI / 5); }
...
TEST_F(RpyFloatingMobilizerTest, SetFromRigidTransform) {
    ...
    EXPECT_TRUE(CompareMatrices(mobilizer_->get_angles(*context_), arbitrary_rpy().vector(), kTolerance,
...
TEST_F(RpyFloatingMobilizerTest, ZeroState) {
    ...
    EXPECT_FALSE(mobilizer_->CalcAcrossMobilizerTransform(*context_).IsExactlyIdentity());
    ...
    EXPECT_TRUE(mobilizer_->CalcAcrossMobilizerTransform(*context_).IsExactlyIdentity());
}
...
TEST_F(RpyFloatingMobilizerTest, RandomState) {
    ...
    RpyFloatingMobilizer<double>* mutable_mobilizer = &mutable_tree().get_mutable_variant(*mobilizer_);

    // Default behavior is to set to zero.
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_TRUE(RigidTransformd(mobilizer_->CalcAcrossMobilizerTransform(*context_)).IsExactlyIdentity());
    ...
    mutable_mobilizer->set_random_state(*context_, &context_->get_mutable_state(), &generator);
    EXPECT_FALSE(RigidTransformd(mobilizer_->CalcAcrossMobilizerTransform(*context_)).IsExactlyIdentity());
...
TEST_F(RpyFloatingMobilizerTest, CalcAcrossMobilizer) {
    ...
    Vector6<double> v;
    v << 1, 2, 3, 4, 5, 6;
    const SpatialVelocity<double> V_FM = mobilizer_->CalcAcrossMobilizerSpatialVelocity(*context_, v);
    EXPECT_TRUE(CompareMatrices(V_FM.get_coeffs(), v, kTolerance, MatrixCompareType::relative));

    // Verify across mobilizer spatial velocity for an arbitrary non-zero state
    // and generalized accelrations.
    const SpatialAcceleration<double> A_FM = mobilizer_->CalcAcrossMobilizerSpatialAcceleration(*context_, v);
    EXPECT_TRUE(CompareMatrices(A_FM.get_coeffs(), v, kTolerance, MatrixCompareType::relative));
    ...
    EXPECT_TRUE(CompareMatrices(F.get_coeffs(), tau, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(RpyFloatingMobilizerTest, MapVelocityToQdotAndBack) {
    ...
    EXPECT_TRUE(CompareMatrices(v, v_back, kTolerance, MatrixCompareType::relative));
}
...
TEST_F(RpyFloatingMobilizerTest, KinematicMapping) {
    ...
    EXPECT_TRUE(CompareMatrices(N_x_Nplus, Matrix6<double>::Identity(), kTolerance, MatrixCompareType::relative));
    EXPECT_TRUE(CompareMatrices(Nplus_x_N, Matrix6<double>::Identity(), kTolerance, MatrixCompareType::relative));
}
...
TEST_F(RpyFloatingMobilizerTest, SingularityError) {
    ...
    const Vector6<double> v = (Vector6<double>() << 1, 2, 3, 4, 5, 6).finished();
    Vector6<double> qdot;
    DRAKE_EXPECT_THROWS_MESSAGE(mobilizer_->MapVelocityToQDot(*context_, v, &qdot), ".*singularity.*");

    // Compute N.
    MatrixX<double> N(6, 6);
    DRAKE_EXPECT_THROWS_MESSAGE(mobilizer_->CalcNMatrix(*context_, &N), ".*singularity.*");
}
