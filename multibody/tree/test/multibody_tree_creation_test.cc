...
#include "multibody/tree/prismatic_mobilizer.h"
...
public:
    ...
    template <typename T>
    static const internal::MultibodyTree<T>& get_parent_tree(const MultibodyElement<T>& element) {
...
GTEST_TEST(MultibodyTree, BasicAPIToAddBodiesAndJoints) {
    ...
    EXPECT_EQ(pendulum.scoped_name().get_full(), "DefaultModelInstance::pendulum");
    EXPECT_EQ(pendulum.body_frame().scoped_name().get_full(), "DefaultModelInstance::pendulum");

    // Adds a revolute joint.
    DRAKE_EXPECT_NO_THROW((model->AddJoint<RevoluteJoint>("joint0", world_body, {}, pendulum, {}, Vector3d::UnitZ())));

    // We cannot add another joint between the same two bodies.
    EXPECT_THROW((model->AddJoint<RevoluteJoint>("joint1", world_body, {}, pendulum, {}, Vector3d::UnitZ())),
                 std::exception);

    // Even if connected in the opposite order.
    EXPECT_THROW((model->AddJoint<RevoluteJoint>("joint1", pendulum, {}, world_body, {}, Vector3d::UnitZ())),
                 std::exception);

    // Verify we cannot add a joint between a body and itself.
    DRAKE_EXPECT_THROWS_MESSAGE(
            (model->AddJoint<RevoluteJoint>("joint1", pendulum, {}, pendulum, {}, Vector3d::UnitZ())),
    ...
    model->AddJoint<RevoluteJoint>("joint1", world_body, {}, pendulum2, {}, Vector3d::UnitZ());
...
GTEST_TEST(MultibodyTree, TopologicalLoopDisallowed) {
    ...
    const auto M_Bo_B = SpatialInertia<double>::NaN();
    const RigidBody<double>& pendulum = model->AddRigidBody("pendulum", M_Bo_B);
    model->AddJoint<RevoluteJoint>("joint0", world_body, {}, pendulum, {}, Vector3d::UnitZ());
    const RigidBody<double>& pendulum2 = model->AddRigidBody("pendulum2", M_Bo_B);
    model->AddJoint<RevoluteJoint>("joint1", world_body, {}, pendulum2, {}, Vector3d::UnitZ());
    ...
    model->AddJoint<RevoluteJoint>("joint2", pendulum, {}, pendulum2, {}, Vector3d::UnitZ());
    ...
    EXPECT_FALSE(model->topology_is_valid());
    DRAKE_EXPECT_THROWS_MESSAGE(model->Finalize(), ".*kinematic loop using joints.*");
    EXPECT_FALSE(model->topology_is_valid());
}
...
GTEST_TEST(MultibodyTree, MultibodyElementChecks) {
    ...
    const RevoluteJoint<double>& pin1 =
            model1->AddJoint<RevoluteJoint>("pin1", model1->world_body(), std::nullopt /*inboard frame*/, body1,
    ...
    DRAKE_EXPECT_THROWS_MESSAGE((model1->AddJoint<RevoluteJoint>("nogood", model1->world_body(), std::nullopt,
                                                                 body2 /*body2 belongs to model2, not model1!!!*/,
                                                                 std::nullopt, Vector3d::UnitZ() /*axis of rotation*/)),
    ...
    EXPECT_EQ(&MultibodyElementTester::get_parent_tree(model1->world_body()), &body1_tree);
    EXPECT_EQ(&MultibodyElementTester::get_parent_tree(model2->world_body()), &body2_tree);
...
public:
    ...
    const RigidBody<double>* AddTestBody(int i) {
        ...
        const auto M_Bo_B = SpatialInertia<double>::NaN();
        const RigidBody<double>* body = &model_->AddRigidBody(fmt::format("TestBody_{}", i), M_Bo_B);
        bodies_.push_back(body);
        return body;
    }

    void ConnectBodies(const RigidBody<double>& inboard, const RigidBody<double>& outboard) {
    ...
    void WeldBodies(const RigidBody<double>& inboard, const RigidBody<double>& outboard) {
        const Joint<double>* joint = &model_->AddJoint<WeldJoint>(fmt::format("WeldJoint{}", joint_counter_++), inboard,
                                                                  {}, outboard, {}, math::RigidTransformd::Identity());
        joints_.push_back(joint);
    }
    ...
    void FinalizeModel() {
        ...
        for (const Joint<double>* joint : joints_) {
            const RevoluteJoint<double>* revolute = dynamic_cast<const RevoluteJoint<double>*>(joint);
            if (revolute != nullptr) {
    ...
    static void TestBodyNode(const MultibodyTreeTopology& topology, BodyIndex body) {
        ...
        // correctly.
        EXPECT_EQ(topology.get_rigid_body(body).mobod_index, topology.get_body_node(node).index);
        EXPECT_EQ(topology.get_body_node(node).rigid_body, topology.get_rigid_body(body).index);

        // They should belong to the same level.
        EXPECT_EQ(topology.get_rigid_body(body).level, topology.get_body_node(node).level);

        const MobodIndex parent_node = topology.get_body_node(node).parent_body_node;
        ...
        if (body != world_index()) {
            ...
            const BodyIndex parent_body = topology.get_body_node(parent_node).rigid_body;
            EXPECT_TRUE(parent_body.is_valid());
            EXPECT_EQ(parent_body, topology.get_rigid_body(body).parent_body);
            EXPECT_EQ(topology.get_body_node(parent_node).index, topology.get_rigid_body(parent_body).mobod_index);
    ...
    static const BodyNodeTopology& node_topology_from_body_index(const MultibodyTreeTopology& topology,
                                                                 int body_index) {
        return topology.get_body_node(topology.get_rigid_body(BodyIndex(body_index)).mobod_index);
    }
    ...
    static void VerifyTopology(const MultibodyTreeTopology& topology) {
        ...
        set<BodyIndex> expected_level1 = {BodyIndex(4), BodyIndex(7), BodyIndex(5), BodyIndex(9)};
        set<BodyIndex> expected_level2 = {BodyIndex(2), BodyIndex(1), BodyIndex(3), BodyIndex(8)};
...
TEST_F(TreeTopologyTests, SizesAndIndexing) {
    ...
    for (MobodIndex mobod_index(1); /* Skips the world mobilized body. */
        ...
        const MobilizerTopology& mobilizer_topology = topology.get_mobilizer(mobilizer_index);
...
TEST_F(TreeTopologyTests, ToSymbolic) {
    ...
    const MultibodyTreeTopology& symbolic_topology = symbolic_model->get_topology();
...
TEST_F(TreeTopologyTests, KinematicPathToWorld) {
    ...
    const MobodIndex body6_mobod_index = topology.get_rigid_body(body6_index).mobod_index;
    const BodyNodeTopology& body6_node = topology.get_body_node(body6_mobod_index);
    ...
    const std::vector<BodyIndex> expected_bodies_path = {BodyIndex(0), BodyIndex(4), BodyIndex(1), BodyIndex(6)};
    ...
    for (BodyIndex body_index : expected_bodies_path) {
        ...
        const MobodIndex expected_mobod_index = topology.get_rigid_body(body_index).mobod_index;
...
TEST_F(TreeTopologyTests, GetTransitiveOutboardBodies) {
    ...
    std::vector<BodyIndex> expected_outboard_bodies{body1_index, body2_index, body4_index, body6_index};
    EXPECT_EQ(topology.GetTransitiveOutboardBodies(body4), expected_outboard_bodies);

    const std::vector<BodyIndex> body14{body1_index, body4_index};
    EXPECT_EQ(topology.GetTransitiveOutboardBodies(body14), expected_outboard_bodies);
    ...
    EXPECT_EQ(topology.GetTransitiveOutboardBodies(body94), expected_outboard_bodies);
    ...
    EXPECT_EQ(topology.GetTransitiveOutboardBodies(body6), expected_outboard_bodies);
}
...
GTEST_TEST(WeldedBodies, CreateListOfWeldedBodies) {
    ...
    auto AddRigidBody = [&model](const std::string& name) -> const RigidBody<double>& {
    ...
    auto AddJoint = [&model](const std::string& name, const RigidBody<double>& parent, const RigidBody<double>& child) {
        model.AddJoint<RevoluteJoint>(name, parent, {}, child, {}, Vector3<double>::UnitX());
    };

    // Helper method to add a WeldJoint between two bodies.
    auto AddWeldJoint = [&model](const std::string& name, const RigidBody<double>& parent,
                                 const RigidBody<double>& child) {
        model.AddJoint<WeldJoint>(name, parent, std::nullopt, child, std::nullopt, math::RigidTransformd::Identity());
    };
    ...
    std::vector<std::set<BodyIndex>> welded_bodies = topology.CreateListOfWeldedBodies();
    ...
    expected_welded_bodies.insert({body_d.index(), body_e.index(), body_f.index(), body_g.index()});
    ...
    expected_welded_bodies.insert({world_index(), body_l.index(), body_m.index(), body_n.index()});
    ...
    std::set<std::set<BodyIndex>> welded_bodies_set(welded_bodies.begin(), welded_bodies.end());
    ...
    for (size_t welded_body_index = 0; welded_body_index < welded_bodies.size(); ++welded_body_index) {
        const std::set<BodyIndex>& welded_body = welded_bodies[welded_body_index];
        // All bodies in welded_bodies[0] are, by definition, anchored to the world.
        // We verify this with IsBodyAnchored().
        for (BodyIndex body_index : welded_body) {
            EXPECT_EQ(topology.IsBodyAnchored(body_index), welded_body_index == 0 /* 'true' for anchored bodies. */);
        }
    }
}
...
const RigidBody<double>& AddCubicalLink(MultibodyTree<double>* model,
    ...
    const SpatialInertia<double> M_BBo_B(mass, p_BoBcm_B, G_BBo_B, skip_validity_check);
...
void AddPrismaticJointX(MultibodyTree<double>* model,
                        const std::string& name,
                        const RigidBody<double>& parent,
                        const RigidBody<double>& child) {
    DRAKE_DEMAND(model != nullptr);
    model->AddJoint<PrismaticJoint>(name, parent, {}, child, {}, Vector3<double>::UnitX());
}

// Helper function to add a z-axis revolute joint between two bodies.
void AddRevoluteJointZ(MultibodyTree<double>* model,
                       const std::string& name,
                       const RigidBody<double>& parent,
                       const RigidBody<double>& child) {
    DRAKE_DEMAND(model != nullptr);
    model->AddJoint<RevoluteJoint>(name, parent, {}, child, {}, Vector3<double>::UnitZ());
}

// Helper function to add a weld joint between two bodies.
void AddWeldJoint(MultibodyTree<double>* model,
                  const std::string& name,
                  const RigidBody<double>& parent,
                  const RigidBody<double>& child) {
    DRAKE_DEMAND(model != nullptr);
...
GTEST_TEST(WeldedBodies, ThrowErrorForDistalCompositeBodyWithZeroMass) {
    ...
    const std::string expected_message =
            "It seems that body bodyA is massless, "
            "yet it is attached by a joint that has a translational degree of freedom.";
    DRAKE_EXPECT_THROWS_MESSAGE(model.ThrowDefaultMassInertiaError(), expected_message);
}
...
GTEST_TEST(WeldedBodies, ThrowErrorForDistalCompositeBodyWithZeroInertia) {
    ...
    // The next function is usually called from MultibodyPlant::Finalize().
    const std::string expected_message =
            "Body bodyA has a zero rotational "
...
GTEST_TEST(WeldedBodies, ThrowErrorForDistalCompositeBodyWithNaNInertia) {
    ...
    const RigidBody<double>& body_A = AddCubicalLink(&model, "bodyA", mass, length, skip_validity_check);
    const RigidBody<double>& body_B = AddCubicalLink(&model, "bodyB", mass, length, skip_validity_check);
    ...
    // The next function is usually called from MultibodyPlant::Finalize().
    const std::string expected_message =
            "Body bodyA has a NaN rotational "
            "inertia, yet it is attached by a joint that "
            "has a rotational degree of freedom.";
    DRAKE_EXPECT_THROWS_MESSAGE(model.ThrowDefaultMassInertiaError(), expected_message);
}
...
GTEST_TEST(TestDistalBody, NoThrowErrorIfZeroMassBodyIsNotDistal) {
    ...
    model.AddJoint<PrismaticJoint>("AB_prismatic_jointY", body_A, {}, body_B, {}, Vector3<double>::UnitY());

    // Add bodyB to bodyC Z-prismatic joint (bodyC has zero mass and inertia).
    model.AddJoint<PrismaticJoint>("BC_prismatic_jointZ", body_B, {}, body_C, {}, Vector3<double>::UnitZ());
