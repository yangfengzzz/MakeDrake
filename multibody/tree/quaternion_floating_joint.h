...
public:
    ...
    QuaternionFloatingJoint(const std::string& name,
        ...
        : Joint<T>(name,
                   frame_on_parent,
                   frame_on_child,
                   (Vector6<double>() << angular_damping,
                    angular_damping,
                    angular_damping,
                    ...
                    translational_damping)
                           .finished(),
                   Vector<double, 7>::Constant(-std::numeric_limits<double>::infinity()),
                   Vector<double, 7>::Constant(std::numeric_limits<double>::infinity()),
    ...
    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::get_translation()")
    Vector3<T> get_position(const systems::Context<T>& context) const { return get_translation(context); }
    ...
    math::RigidTransform<T> GetPose(const systems::Context<T>& context) const {
        return math::RigidTransform<T>(get_quaternion(context), get_translation(context));
    }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::GetPose()")
    math::RigidTransform<T> get_pose(const systems::Context<T>& context) const { return GetPose(context); }
    ...
    const QuaternionFloatingJoint<T>& SetQuaternion(systems::Context<T>* context, const Quaternion<T>& q_FM) const {
        get_mobilizer().SetQuaternion(context, q_FM);
        return *this;
    }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::SetQuaternion()")
    ...
    /// @returns a constant reference to `this` joint.
    const QuaternionFloatingJoint<T>& SetOrientation(systems::Context<T>* context,
                                                     const math::RotationMatrix<T>& R_FM) const {
        get_mobilizer().SetOrientation(context, R_FM);
        return *this;
    }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::SetOrientation()")
    ...
    /// @returns a constant reference to `this` joint.
    const QuaternionFloatingJoint<T>& SetTranslation(systems::Context<T>* context, const Vector3<T>& p_FM) const {
        get_mobilizer().SetTranslation(context, p_FM);
        return *this;
    }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::SetTranslation()")
    ...
    /// @returns a constant reference to `this` joint.
    const QuaternionFloatingJoint<T>& SetPose(systems::Context<T>* context, const math::RigidTransform<T>& X_FM) const {
        SetTranslation(context, X_FM.translation());
        return SetOrientation(context, X_FM.rotation());
    }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::SetPose()")
    ...
    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::set_random_translation_distribution()")
    ...
    Vector3<double> get_default_translation() const { return this->default_positions().template tail<3>(); }

    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::get_default_translation()")
    Vector3<double> get_default_position() const { return get_default_translation(); }
    ...
    math::RigidTransform<double> get_default_pose() const {
        return math::RigidTransform(get_default_quaternion(), get_default_translation());
    }
    ...
    DRAKE_DEPRECATED("2024-08-01", "Use QuaternionFloatingJoint::set_default_translation()")
    void set_default_position(const Vector3<double>& p_FM) { set_default_translation(p_FM); }
    /// @}
...
private:
    int do_get_velocity_start() const override { return get_mobilizer().velocity_start_in_v(); }

    int do_get_num_velocities() const override { return 6; }

    int do_get_position_start() const override { return get_mobilizer().position_start_in_q(); }

    int do_get_num_positions() const override { return 7; }

    std::string do_get_position_suffix(int index) const override { return get_mobilizer().position_suffix(index); }

    std::string do_get_velocity_suffix(int index) const override { return get_mobilizer().velocity_suffix(index); }

    void do_set_default_positions(const VectorX<double>& default_positions) override {
        if (this->has_implementation()) {
            get_mutable_mobilizer()->set_default_position(default_positions);
        }
    }

    void DoSetDefaultPosePair(const Quaternion<double>& q_FM, const Vector3<double>& p_FM) final {
        VectorX<double> q(7);
        q << q_FM.w(), q_FM.vec(), p_FM;
        this->set_default_positions(q);
    }

    std::pair<Eigen::Quaternion<double>, Vector3<double>> DoGetDefaultPosePair() const final {
        const VectorX<double>& q = this->default_positions();
        return std::make_pair(Eigen::Quaternion<double>(q[0], q[1], q[2], q[3]), q.tail<3>());
    }

    // Joint<T> overrides:
    std::unique_ptr<typename Joint<T>::BluePrint> MakeImplementationBlueprint() const override;

    std::unique_ptr<Joint<double>> DoCloneToScalar(const internal::MultibodyTree<double>& tree_clone) const override;
    ...
    const internal::QuaternionFloatingMobilizer<T>& get_mobilizer() const {
        ...
        const internal::QuaternionFloatingMobilizer<T>* mobilizer =
                dynamic_cast<const internal::QuaternionFloatingMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::QuaternionFloatingMobilizer<T>* get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::QuaternionFloatingMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
