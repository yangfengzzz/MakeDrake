...
template <typename T>
RevoluteSpring<T>::RevoluteSpring(const RevoluteJoint<T>& joint, double nominal_angle, double stiffness)
    : RevoluteSpring(joint.model_instance(), joint.index(), nominal_angle, stiffness) {}
...
RevoluteSpring<T>::RevoluteSpring(ModelInstanceIndex model_instance,
                                  ...
                                  double stiffness)
    : ForceElement<T>(model_instance), joint_index_(joint_index), nominal_angle_(nominal_angle), stiffness_(stiffness) {
...
template <typename T>
void RevoluteSpring<T>::DoCalcAndAddForceContribution(const systems::Context<T>& context,
...
std::unique_ptr<ForceElement<ToScalar>> RevoluteSpring<T>::TemplatedDoCloneToScalar(
    ...
    std::unique_ptr<RevoluteSpring<ToScalar>> spring_clone(
            new RevoluteSpring<ToScalar>(this->model_instance(), joint_index_, nominal_angle(), stiffness()));
    return spring_clone;
}
...
template <typename T>
std::unique_ptr<ForceElement<symbolic::Expression>> RevoluteSpring<T>::DoCloneToScalar(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::RevoluteSpring);
