...
std::unique_ptr<Joint<ToScalar>> ScrewJoint<T>::TemplatedDoCloneToScalar(
    ...
    auto joint_clone =
            std::make_unique<ScrewJoint<ToScalar>>(this->name(), frame_on_parent_body_clone, frame_on_child_body_clone,
...
template <typename T>
std::unique_ptr<Joint<double>> ScrewJoint<T>::DoCloneToScalar(const internal::MultibodyTree<double>& tree_clone) const {
...
// in the header file.
template <typename T>
std::unique_ptr<typename Joint<T>::BluePrint> ScrewJoint<T>::MakeImplementationBlueprint() const {
    auto blue_print = std::make_unique<typename Joint<T>::BluePrint>();
    auto screw_mobilizer = std::make_unique<internal::ScrewMobilizer<T>>(
            this->frame_on_parent(), this->frame_on_child(), this->screw_axis(), screw_pitch_);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::ScrewJoint);
