...
template <typename T, template <typename> class Element, typename Index>
Element<T>& ElementCollection<T, Element, Index>::Add(std::unique_ptr<Element<T>> element) {
    return AddImpl(std::move(element));
}

template <typename T, template <typename> class Element, typename Index>
Element<T>& ElementCollection<T, Element, Index>::AddBorrowed(Element<T>* element) {
...
template <typename T, template <typename> class Element, typename Index>
Element<T>& ElementCollection<T, Element, Index>::AddImpl(std::shared_ptr<Element<T>> maybe_owned) {
    ...
    if (index == ssize(elements_by_index_)) {
        ...
        const size_t packed_offset = std::distance(
                indices_packed_.begin(), std::lower_bound(indices_packed_.begin(), indices_packed_.end(), index));
...
template <typename T, template <typename> class Element, typename Index>
typename string_unordered_multimap<Index>::const_iterator ElementCollection<T, Element, Index>::FindNamesIterator(
...
void ElementCollection<T, Element, Index>::Remove(Index index) {
    ...
    const size_t packed_offset = std::distance(indices_packed_.begin(),
...
template <typename T, template <typename> class Element, typename Index>
void ElementCollection<T, Element, Index>::Rename(Index index, std::string name) {
    // Currently, ModelInstance is the only Element that has `set_name`.
    if constexpr (std::is_same_v<Element<T>, ModelInstance<T>>) {
        const auto old_name_iter = FindNamesIterator(get_element(index).name(), index);
...
template <typename T, template <typename> class Element, typename Index>
void ElementCollection<T, Element, Index>::ThrowInvalidIndexException(Index index) const {
    const std::string nice_type = RemoveTemplates(NiceTypeName::RemoveNamespaces(NiceTypeName::Get<Element<T>>()));
    if (!index.is_valid()) {
        throw std::logic_error(
                fmt::format("The given default-constructed {}Index() cannot be used. You must "
                            "pass a valid integer as the index.",
                            nice_type));
    }
    if (index >= next_index()) {
        throw std::logic_error(fmt::format("The given {}Index({}) is out of bounds (must be less than {})", nice_type,
                                           index, next_index()));
    }
    throw std::logic_error(fmt::format("The {}Index({}) has been removed", nice_type, index));
}
