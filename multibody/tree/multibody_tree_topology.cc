...
bool MultibodyTreeTopology::operator==(const MultibodyTreeTopology& other) const {
    ...
    if (tree_velocities_start_in_v_ != other.tree_velocities_start_in_v_) return false;
    ...
    return std::equal(rigid_body_to_tree_index_.begin() + 1, rigid_body_to_tree_index_.end(),
                      other.rigid_body_to_tree_index_.begin() + 1, other.rigid_body_to_tree_index_.end());
}

bool RigidBodyTopology::operator==(const RigidBodyTopology& other) const {
    if (index != other.index) return false;
    if (inboard_mobilizer.is_valid() != other.inboard_mobilizer.is_valid()) return false;
    if (inboard_mobilizer.is_valid() && inboard_mobilizer != other.inboard_mobilizer) return false;
    ...
    if (floating_velocities_start_in_v != other.floating_velocities_start_in_v) return false;
    return true;
}
...
bool BodyNodeTopology::operator==(const BodyNodeTopology& other) const {
    ...
    if (parent_body_node.is_valid() != other.parent_body_node.is_valid()) return false;
    if (parent_body_node.is_valid() && parent_body_node != other.parent_body_node) return false;

    if (rigid_body != other.rigid_body) return false;

    if (parent_rigid_body.is_valid() != other.parent_rigid_body.is_valid()) return false;
    if (parent_rigid_body.is_valid() && parent_rigid_body != other.parent_rigid_body) return false;
    ...
    if (mobilizer_positions_start != other.mobilizer_positions_start) return false;
    if (num_mobilizer_velocities != other.num_mobilizer_velocities) return false;
    if (mobilizer_velocities_start_in_state != other.mobilizer_velocities_start_in_state) return false;
    if (mobilizer_velocities_start_in_v != other.mobilizer_velocities_start_in_v) return false;
...
const JointActuatorTopology& MultibodyTreeTopology::get_joint_actuator(JointActuatorIndex index) const {
...
MobilizerIndex MultibodyTreeTopology::add_mobilizer(FrameIndex in_frame,
    ...
    if (in_frame == out_frame) {
        throw std::runtime_error("Attempting to add a mobilizer between a frame and itself");
    }
    if (IsThereAMobilizerBetweenFrames(in_frame, out_frame)) {
        throw std::runtime_error(
                fmt::format("This multibody tree already has a mobilizer connecting "
    ...
    mobilizers_.emplace_back(mobilizer_index, in_frame, out_frame, inboard_body, outboard_body, num_positions,
                             num_velocities);
...
JointActuatorIndex MultibodyTreeTopology::add_joint_actuator(int num_dofs) {
    ...
    joint_actuators_.push_back(JointActuatorTopology{
            .index = actuator_index, .actuator_index_start = actuator_index_start, .num_dofs = num_dofs});
    num_actuated_dofs_ += num_dofs;
    return actuator_index;
}

void MultibodyTreeTopology::RemoveJointActuator(JointActuatorIndex actuator_index) {
...
void MultibodyTreeTopology::Finalize() {
    ...
    while (!stack.empty()) {
        ...
        body_nodes_.emplace_back(node, level /* node index and level */, parent_node /* This node's parent */,
        ...
        for (auto it = rigid_bodies_[current].child_bodies.rbegin(); it != rigid_bodies_[current].child_bodies.rend();
             ++it) {
            stack.push(*it);
        }
    }
    ...
    for (BodyIndex body(0); body < num_rigid_bodies(); ++body) {
        if (rigid_bodies_[body].level < 0) {
            throw std::runtime_error("Body with index " + std::to_string(body) + " was not assigned a mobilizer");
        }
    }
    ...
    for (MobodIndex node_index(1); node_index < num_mobods(); ++node_index) {
        ...
        node.mobilizer_velocities_start_in_state = mobilizer.velocities_start_in_state;
    ...
    for (RigidBodyTopology& body : rigid_bodies_) {
        if (body.is_floating) {
            ...
            const MobilizerTopology& mobilizer = get_mobilizer(body.inboard_mobilizer);
...
void MultibodyTreeTopology::GetKinematicPathToWorld(MobodIndex from, std::vector<MobodIndex>* path_to_world) const {
    ...
    for (MobodIndex node = from; node > world_mobod_index(); node = get_body_node(node).parent_body_node) {
...
std::vector<std::set<BodyIndex>> MultibodyTreeTopology::CreateListOfWeldedBodies() const {
    ...
    CreateListOfWeldedBodiesRecurse(world_index(), &bodies_welded_to_world, &welded_bodies_list);
    return welded_bodies_list;
}

std::vector<BodyIndex> MultibodyTreeTopology::GetTransitiveOutboardBodies(std::vector<BodyIndex> body_indexes) const {
    DRAKE_DEMAND(is_valid());
    ...
    for (const BodyIndex& body_index : body_indexes) {
        ...
        if (!outboard_bodies.contains(body_index)) {
            const BodyNodeTopology& root = get_body_node(get_rigid_body(body_index).mobod_index);
            TraverseOutboardNodes(root, collect_body);
        }
    }
    std::vector<BodyIndex> results(outboard_bodies.begin(), outboard_bodies.end());
    std::sort(results.begin(), results.end());
    return results;
}

bool MultibodyTreeTopology::IsThereAMobilizerBetweenFrames(FrameIndex frame1, FrameIndex frame2) const {
...
bool MultibodyTreeTopology::IsThereAMobilizerBetweenRigidBodies(BodyIndex body1, BodyIndex body2) const {
    for (const auto& mobilizer_topology : mobilizers_) {
...
void MultibodyTreeTopology::CreateListOfWeldedBodiesRecurse(BodyIndex parent_index,
                                                            std::set<BodyIndex>* parent_welded_body,
    ...
    for (BodyIndex child_index : parent.child_bodies) {
        ...
        const MobilizerTopology& child_mobilizer = get_mobilizer(child.inboard_mobilizer);
        if (child_mobilizer.is_weld_mobilizer()) {
            ...
            CreateListOfWeldedBodiesRecurse(child_index, parent_welded_body, welded_bodies);
        } else {
...
void MultibodyTreeTopology::TraverseOutboardNodes(const BodyNodeTopology& base,
...
int MultibodyTreeTopology::CalcNumberOfOutboardVelocities(const BodyNodeTopology& base) const {
    DRAKE_DEMAND(num_mobods() != 0);
    int nv = 0;
...
void MultibodyTreeTopology::ExtractForestInfo() {
    ...
    for (int t = 1; t < num_trees(); ++t) {
        tree_velocities_start_in_v_[t] = tree_velocities_start_in_v_[t - 1] + num_tree_velocities_[t - 1];
    }
}
