...
template <typename T>
std::string QuaternionFloatingMobilizer<T>::position_suffix(int position_index_in_mobilizer) const {
...
template <typename T>
std::string QuaternionFloatingMobilizer<T>::velocity_suffix(int velocity_index_in_mobilizer) const {
    ...
    throw std::runtime_error("QuaternionFloatingMobilizer has only 6 velocities.");
}

template <typename T>
Quaternion<T> QuaternionFloatingMobilizer<T>::get_quaternion(const systems::Context<T>& context) const {
...
template <typename T>
Vector3<T> QuaternionFloatingMobilizer<T>::get_translation(const systems::Context<T>& context) const {
    const auto q = this->get_positions(context);
    DRAKE_ASSERT(q.size() == kNq);
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetQuaternion(systems::Context<T>* context,
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetQuaternion(const systems::Context<T>&,
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetTranslation(systems::Context<T>* context,
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetTranslation(const systems::Context<T>&,
...
template <typename T>
void QuaternionFloatingMobilizer<T>::set_random_translation_distribution(const Vector3<symbolic::Expression>& p_FM) {
...
template <typename T>
Vector3<T> QuaternionFloatingMobilizer<T>::get_angular_velocity(const systems::Context<T>& context) const {
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetAngularVelocity(systems::Context<T>* context,
                                                                                         const Vector3<T>& w_FM) const {
    return SetAngularVelocity(*context, w_FM, &context->get_mutable_state());
}

template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetAngularVelocity(
        const systems::Context<T>&, const Vector3<T>& w_FM, systems::State<T>* state) const {
...
template <typename T>
Vector3<T> QuaternionFloatingMobilizer<T>::get_translational_velocity(const systems::Context<T>& context) const {
    // Note: we store the components of the angular velocity w_FM first, followed
    // by the components of the position vector v_FM.
...
template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetTranslationalVelocity(
        systems::Context<T>* context, const Vector3<T>& v_FM) const {
    return SetTranslationalVelocity(*context, v_FM, &context->get_mutable_state());
}

template <typename T>
const QuaternionFloatingMobilizer<T>& QuaternionFloatingMobilizer<T>::SetTranslationalVelocity(
        const systems::Context<T>&, const Vector3<T>& v_FM, systems::State<T>* state) const {
    auto v = this->get_mutable_velocities(state);
    DRAKE_ASSERT(v.size() == kNv);
...
template <typename T>
SpatialAcceleration<T> QuaternionFloatingMobilizer<T>::CalcAcrossMobilizerSpatialAcceleration(
        const systems::Context<T>&, const Eigen::Ref<const VectorX<T>>& vdot) const {
...
template <typename T>
void QuaternionFloatingMobilizer<T>::ProjectSpatialForce(const systems::Context<T>&,
...
template <typename T>
Eigen::Matrix<T, 4, 3> QuaternionFloatingMobilizer<T>::CalcLMatrix(const Quaternion<T>& q_FM) {
    ...
    return (Eigen::Matrix<T, 4, 3>() << mqv.transpose(), qs, qv.z(), mqv.y(), mqv.z(), qs, qv.x(), qv.y(), mqv.x(), qs)
            .finished();
}

template <typename T>
Eigen::Matrix<T, 4, 3> QuaternionFloatingMobilizer<T>::AngularVelocityToQuaternionRateMatrix(
        const Quaternion<T>& q_FM) {
    // With L given by CalcLMatrix we have:
    // N(q) = L(q_FM/2)
    return CalcLMatrix({q_FM.w() / 2.0, q_FM.x() / 2.0, q_FM.y() / 2.0, q_FM.z() / 2.0});
}

template <typename T>
Eigen::Matrix<T, 3, 4> QuaternionFloatingMobilizer<T>::QuaternionRateToAngularVelocityMatrix(
        const Quaternion<T>& q_FM) {
    ...
    const Vector4<T> q_FM_tilde = Vector4<T>(q_FM.w(), q_FM.x(), q_FM.y(), q_FM.z()) / q_norm;

    // Gradient of the normalized quaternion with respect to the unnormalized
    // generalized coordinates:
    const Matrix4<T> dqnorm_dq = (Matrix4<T>::Identity() - q_FM_tilde * q_FM_tilde.transpose()) / q_norm;

    // With L given by CalcLMatrix we have:
    // N⁺(q_tilde) = L(2 q_FM_tilde)ᵀ
    return CalcLMatrix({2.0 * q_FM_tilde[0], 2.0 * q_FM_tilde[1], 2.0 * q_FM_tilde[2], 2.0 * q_FM_tilde[3]})
                   .transpose() *
           dqnorm_dq;
}

template <typename T>
void QuaternionFloatingMobilizer<T>::DoCalcNMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> N) const {
    // Upper-left block
    N->template block<4, 3>(0, 0) = AngularVelocityToQuaternionRateMatrix(get_quaternion(context));
    // Upper-right block
...
template <typename T>
void QuaternionFloatingMobilizer<T>::DoCalcNplusMatrix(const systems::Context<T>& context,
                                                       EigenPtr<MatrixX<T>> Nplus) const {
    // Upper-left block
    Nplus->template block<3, 4>(0, 0) = QuaternionRateToAngularVelocityMatrix(get_quaternion(context));
    // Upper-right block
...
template <typename T>
void QuaternionFloatingMobilizer<T>::MapVelocityToQDot(const systems::Context<T>& context,
    ...
    qdot->template head<4>() = AngularVelocityToQuaternionRateMatrix(q_FM) * v.template head<3>();
    // Translational component, ṗ_WB = v_WB:
    qdot->template tail<3>() = v.template tail<3>();
}

template <typename T>
void QuaternionFloatingMobilizer<T>::MapQDotToVelocity(const systems::Context<T>& context,
    ...
    v->template head<3>() = QuaternionRateToAngularVelocityMatrix(q_FM) * qdot.template head<4>();
...
std::unique_ptr<Mobilizer<ToScalar>> QuaternionFloatingMobilizer<T>::TemplatedDoCloneToScalar(
        const MultibodyTree<ToScalar>& tree_clone) const {
    const Frame<ToScalar>& inboard_frame_clone = tree_clone.get_variant(this->inboard_frame());
    const Frame<ToScalar>& outboard_frame_clone = tree_clone.get_variant(this->outboard_frame());
    return std::make_unique<QuaternionFloatingMobilizer<ToScalar>>(inboard_frame_clone, outboard_frame_clone);
}
...
template <typename T>
std::unique_ptr<Mobilizer<symbolic::Expression>> QuaternionFloatingMobilizer<T>::DoCloneToScalar(
