...
public:
    ...
    void SetZeroState(const systems::Context<T>&, systems::State<T>* state) const final {
    ...
    void set_default_state(const systems::Context<T>&, systems::State<T>* state) const final {
    ...
    void set_default_position(const Eigen::Ref<const Vector<double, compile_time_num_positions>>& position) {
    ...
    void set_random_state(const systems::Context<T>& context,
        ...
        if (random_state_distribution_) {
            const Vector<double, kNx> sample =
                    Evaluate(*random_state_distribution_, symbolic::Environment{}, generator);
    ...
    void set_random_position_distribution(
            const Eigen::Ref<const Vector<symbolic::Expression, compile_time_num_positions>>& position) {
        if (!random_state_distribution_) {
            random_state_distribution_.emplace(Vector<symbolic::Expression, kNx>::Zero());
    ...
    void set_random_velocity_distribution(
            const Eigen::Ref<const Vector<symbolic::Expression, compile_time_num_velocities>>& velocity) {
        if (!random_state_distribution_) {
...
protected:
    ...
    virtual Vector<double, kNq> get_zero_position() const { return Vector<double, kNq>::Zero(); }
    ...
    const std::optional<Vector<symbolic::Expression, kNx>>& get_random_state_distribution() const {
    ...
    Eigen::VectorBlock<const VectorX<T>, kNq> get_positions(const systems::Context<T>& context) const {
        return this->get_parent_tree().template get_state_segment<kNq>(context, this->get_positions_start());
    }
    ...
    // @pre `context` is a valid multibody system Context.
    Eigen::VectorBlock<VectorX<T>, kNq> GetMutablePositions(systems::Context<T>* context) const {
        return this->get_parent_tree().template GetMutableStateSegment<kNq>(context, this->get_positions_start());
    }
    ...
    Eigen::VectorBlock<VectorX<T>, kNq> get_mutable_positions(systems::State<T>* state) const {
        return this->get_parent_tree().template get_mutable_state_segment<kNq>(state, this->get_positions_start());
    }

    // Helper to return a const fixed-size Eigen::VectorBlock referencing the
    // segment in the state vector corresponding to `this` mobilizer's state.
    // @pre `context` is a valid multibody system Context.
    Eigen::VectorBlock<const VectorX<T>, kNv> get_velocities(const systems::Context<T>& context) const {
        return this->get_parent_tree().template get_state_segment<kNv>(context, this->get_velocities_start_in_state());
    }
    ...
    // @pre `context` is a valid multibody system Context.
    Eigen::VectorBlock<VectorX<T>, kNv> GetMutableVelocities(systems::Context<T>* context) const {
        return this->get_parent_tree().template GetMutableStateSegment<kNv>(context,
    ...
    // @pre `state` is a valid multibody system State.
    Eigen::VectorBlock<VectorX<T>, kNv> get_mutable_velocities(systems::State<T>* state) const {
        return this->get_parent_tree().template get_mutable_state_segment<kNv>(state,
                                                                               this->get_velocities_start_in_state());
    }
    //@}
...
private:
    ...
    int get_positions_start() const { return this->get_topology().positions_start; }

    // Returns the index in the global array of generalized coordinates and
    // velocities [q v] in the MultibodyTree model to the first component of the
    // generalized velocities vector that corresponds to this mobilizer.
    int get_velocities_start_in_state() const { return this->get_topology().velocities_start_in_state; }
