...
public:
    ...
    UnitInertia(const T& Ixx, const T& Iyy, const T& Izz) : RotationalInertia<T>(Ixx, Iyy, Izz) {}
    ...
    explicit UnitInertia(const RotationalInertia<T>& I) : RotationalInertia<T>(I) {}
    ...
    void ReExpressInPlace(const math::RotationMatrix<T>& R_AE) { RotationalInertia<T>::ReExpressInPlace(R_AE); }
    ...
    ///                the centroid `Bcm`.
    [[nodiscard]] UnitInertia<T> ShiftFromCenterOfMass(const Vector3<T>& p_BcmQ_E) const {
    ...
    [[nodiscard]] UnitInertia<T> ShiftToCenterOfMass(const Vector3<T>& p_QBcm_E) const {
        UnitInertia<T> result(*this);
    ...
    static UnitInertia<T> SolidSphere(const T& r) { return UnitInertia<T>::TriaxiallySymmetric(0.4 * r * r); }
    ...
    static UnitInertia<T> HollowSphere(const T& r) { return UnitInertia<T>::TriaxiallySymmetric(2.0 / 3.0 * r * r); }
    ...
    static UnitInertia<T> TriaxiallySymmetric(const T& I_triaxial) {
        return UnitInertia<T>(RotationalInertia<T>::TriaxiallySymmetric(I_triaxial));
    }
    ...
    std::pair<Vector3<double>, math::RotationMatrix<double>> CalcPrincipalHalfLengthsAndAxesForEquivalentShape(
