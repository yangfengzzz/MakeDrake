...
public:
    ...
    ScrewJoint(const std::string& name,
               ...
               double screw_pitch,
               double damping)
        : ScrewJoint<T>(name, frame_on_parent, frame_on_child, Vector3<double>::UnitZ(), screw_pitch, damping) {}
    ...
    T get_translation(const Context<T>& context) const { return get_mobilizer()->get_translation(context); }
    ...
    const T& get_rotation(const systems::Context<T>& context) const { return get_mobilizer()->get_angle(context); }
    ...
    double get_default_translation() const {
        return internal::get_screw_translation_from_rotation(this->default_positions()[0], screw_pitch());
    }
    ...
    void set_default_translation(const double& z) {
        Vector1<double> state(internal::get_screw_rotation_from_translation(z, screw_pitch()));
...
private:
    ...
    int do_get_position_start() const final { return get_mobilizer()->position_start_in_q(); }
    ...
    void do_set_default_positions(const VectorX<double>& default_positions) final {
        if (this->has_implementation()) {
            get_mutable_mobilizer()->set_default_position(default_positions);
        }
    }

    const T& DoGetOnePosition(const systems::Context<T>& context) const override { return get_rotation(context); }
    ...
    const internal::ScrewMobilizer<T>* get_mobilizer() const {
        ...
        const internal::ScrewMobilizer<T>* mobilizer =
                dynamic_cast<const internal::ScrewMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::ScrewMobilizer<T>* get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::ScrewMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::ScrewJoint);
