...
public:
    ...
    PlanarMobilizer(const Frame<T>& inboard_frame_F, const Frame<T>& outboard_frame_M)
    ...
    const PlanarMobilizer<T>& set_translations(systems::Context<T>* context,
    ...
    /* Sets in `context` the orientation for `this` mobilizer to the angle θ
     ...
     @returns a constant reference to `this` mobilizer. */
    const PlanarMobilizer<T>& SetAngle(systems::Context<T>* context, const T& angle) const;
    ...
    /* Sets in `context` the rate of change, in meters per second, of `this`
     ...
     @returns A constant reference to `this` mobilizer. */
    const PlanarMobilizer<T>& SetTranslationRates(systems::Context<T>* context,
    ...
    /* Sets in `context` the rate of change, in radians per second, of `this`
     ...
     @returns A constant reference to `this` mobilizer. */
    const PlanarMobilizer<T>& SetAngularRate(systems::Context<T>* context, const T& theta_dot) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::PlanarMobilizer);
