...
systems::BasicVector<T> ToBasicVector(const SpatialInertia<T>& spatial_inertia) {
    ...
    return systems::BasicVector<T>({// mass
                                    ...
                                    com(0), com(1), com(2),
                                    // unit inertia
                                    unit_inertia(0, 0), unit_inertia(1, 1), unit_inertia(2, 2), unit_inertia(0, 1),
...
const T& GetMass(const systems::BasicVector<T>& spatial_inertia_vector) {
    DRAKE_DEMAND(spatial_inertia_vector.size() == SpatialInertiaIndex::k_num_coordinates);
...
// SpatialInertia<T>.
template <typename T>
Vector3<T> GetCenterOfMass(const systems::BasicVector<T>& spatial_inertia_vector) {
    DRAKE_DEMAND(spatial_inertia_vector.size() == SpatialInertiaIndex::k_num_coordinates);
...
template <typename T>
SpatialInertia<T> ToSpatialInertia(const systems::BasicVector<T>& spatial_inertia_basic_vector) {
    DRAKE_DEMAND(spatial_inertia_basic_vector.size() == SpatialInertiaIndex::k_num_coordinates);
    const auto& spatial_inertia_vector = spatial_inertia_basic_vector.get_value();
    return SpatialInertia<T>(spatial_inertia_vector[SpatialInertiaIndex::k_mass],
