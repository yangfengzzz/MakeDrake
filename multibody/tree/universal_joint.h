...
public:
    ...
    UniversalJoint(const std::string& name,
        ...
        : Joint<T>(name,
                   ...
                   VectorX<double>::Constant(2, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(2, std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(2, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(2, std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(2, -std::numeric_limits<double>::infinity()),
    ...
    ///          ordered as (θ₁, θ₂).
    Vector2<T> get_angles(const Context<T>& context) const { return get_mobilizer()->get_angles(context); }
    ...
    Vector2<double> get_default_angles() const { return this->default_positions(); }

    /// Sets the default angles of this joint.
    /// @param[in] angles The desired default angles of the joint
    void set_default_angles(const Vector2<double>& angles) { this->set_default_positions(angles); }
    ...
    void set_random_angles_distribution(const Vector2<symbolic::Expression>& angles) {
        get_mutable_mobilizer()->set_random_position_distribution(Vector2<symbolic::Expression>{angles});
    }
...
private:
    ...
    const internal::UniversalMobilizer<T>* get_mobilizer() const {
        ...
        const internal::UniversalMobilizer<T>* mobilizer =
                dynamic_cast<const internal::UniversalMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::UniversalMobilizer<T>* get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::UniversalMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
