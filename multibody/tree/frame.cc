...
std::string DeprecateWhenEmptyName(std::string name, std::string_view type) {
    if (name.empty()) {
        throw std::runtime_error(fmt::format("The name parameter to the {} constructor is required.", type));
    }
    return name;
}
...
template <typename T>
Vector3<T> Frame<T>::CalcAngularVelocity(const systems::Context<T>& context,
...
//  rigid body (not a soft body). Modify if soft bodies are possible.
template <typename T>
SpatialVelocity<T> Frame<T>::CalcSpatialVelocityInWorld(const systems::Context<T>& context) const {
    const math::RotationMatrix<T>& R_WB = body().EvalPoseInWorld(context).rotation();
...
template <typename T>
SpatialVelocity<T> Frame<T>::CalcSpatialVelocity(const systems::Context<T>& context,
                                                 const Frame<T>& frame_M,
                                                 const Frame<T>& frame_E) const {
    const math::RotationMatrix<T> R_WM = frame_M.CalcRotationMatrixInWorld(context);
...
//  rigid body (not a soft body). Modify if soft bodies are possible.
template <typename T>
SpatialAcceleration<T> Frame<T>::CalcSpatialAccelerationInWorld(const systems::Context<T>& context) const {
    // `this` frame_F is fixed to a body B.  Calculate A_WB_W, body B's spatial
    // acceleration in the world frame W, expressed in W.
    const SpatialAcceleration<T>& A_WB_W = body().EvalSpatialAccelerationInWorld(context);
...
template <typename T>
SpatialAcceleration<T> Frame<T>::CalcSpatialAcceleration(const systems::Context<T>& context,
                                                         const Frame<T>& measured_in_frame,
                                                         const Frame<T>& expressed_in_frame) const {
    const Frame<T>& frame_M = measured_in_frame;
    const Frame<T>& frame_E = expressed_in_frame;
    ...
    const SpatialAcceleration<T> A_WF_W = this->CalcSpatialAccelerationInWorld(context);
    ...
    auto calc_A_MF_W = [this, &context, &frame_M, &A_WF_W]() {
        ...
        const SpatialAcceleration<T> A_WM_W = frame_M.CalcSpatialAccelerationInWorld(context);
        const Vector3<T>& alpha_WM_W = A_WM_W.rotational();
        const Vector3<T>& w_WM_W = frame_M.EvalAngularVelocityInWorld(context);
        const Frame<T>& frame_W = this->get_parent_tree().world_frame();
        const SpatialVelocity<T> V_MF_W = CalcSpatialVelocity(context, frame_M, frame_W);
        ...
        const Vector3<T> a_WcoincidentPoint_W = A_WM_W.Shift(p_MoFo_W, w_WM_W).translational();
    ...
    const SpatialAcceleration<T> A_MF_W = frame_M.is_world_frame() ? A_WF_W : calc_A_MF_W();
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::Frame);
