#include "multibody/tree/rpy_ball_mobilizer.h"
...
template <typename T>
std::string RpyBallMobilizer<T>::position_suffix(int position_index_in_mobilizer) const {
...
template <typename T>
std::string RpyBallMobilizer<T>::velocity_suffix(int velocity_index_in_mobilizer) const {
...
template <typename T>
Vector3<T> RpyBallMobilizer<T>::get_angles(const systems::Context<T>& context) const {
    return this->get_positions(context);
}

template <typename T>
const RpyBallMobilizer<T>& RpyBallMobilizer<T>::SetAngles(systems::Context<T>* context,
...
template <typename T>
const RpyBallMobilizer<T>& RpyBallMobilizer<T>::SetFromRotationMatrix(systems::Context<T>* context,
...
template <typename T>
Vector3<T> RpyBallMobilizer<T>::get_angular_velocity(const systems::Context<T>& context) const {
    return this->get_velocities(context);
}

template <typename T>
const RpyBallMobilizer<T>& RpyBallMobilizer<T>::SetAngularVelocity(systems::Context<T>* context,
                                                                   const Vector3<T>& w_FM) const {
    return SetAngularVelocity(*context, w_FM, &context->get_mutable_state());
}

template <typename T>
const RpyBallMobilizer<T>& RpyBallMobilizer<T>::SetAngularVelocity(const systems::Context<T>&,
...
template <typename T>
math::RigidTransform<T> RpyBallMobilizer<T>::CalcAcrossMobilizerTransform(const systems::Context<T>& context) const {
...
template <typename T>
SpatialAcceleration<T> RpyBallMobilizer<T>::CalcAcrossMobilizerSpatialAcceleration(
...
template <typename T>
void RpyBallMobilizer<T>::ProjectSpatialForce(const systems::Context<T>&,
...
template <typename T>
void RpyBallMobilizer<T>::DoCalcNMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> N) const {
    using std::abs;
    using std::cos;
    using std::sin;
    ...
    if (abs(cp) < 1.0e-3) {
        throw std::runtime_error(
                fmt::format("The RpyBallMobilizer (implementing a BallRpyJoint) between "
...
template <typename T>
void RpyBallMobilizer<T>::DoCalcNplusMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> Nplus) const {
...
template <typename T>
void RpyBallMobilizer<T>::MapVelocityToQDot(const systems::Context<T>& context,
...
template <typename T>
void RpyBallMobilizer<T>::MapQDotToVelocity(const systems::Context<T>& context,
...
std::unique_ptr<Mobilizer<ToScalar>> RpyBallMobilizer<T>::TemplatedDoCloneToScalar(
    ...
    return std::make_unique<RpyBallMobilizer<ToScalar>>(inboard_frame_clone, outboard_frame_clone);
}

template <typename T>
std::unique_ptr<Mobilizer<double>> RpyBallMobilizer<T>::DoCloneToScalar(const MultibodyTree<double>& tree_clone) const {
...
template <typename T>
std::unique_ptr<Mobilizer<symbolic::Expression>> RpyBallMobilizer<T>::DoCloneToScalar(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::RpyBallMobilizer);
