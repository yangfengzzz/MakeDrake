...
public:
    ...
    std::string position_suffix(int position_index_in_joint) const {
        DRAKE_DEMAND(0 <= position_index_in_joint && position_index_in_joint < num_positions());
    ...
    std::string velocity_suffix(int velocity_index_in_joint) const {
        DRAKE_DEMAND(0 <= velocity_index_in_joint && velocity_index_in_joint < num_velocities());
        DRAKE_DEMAND(has_implementation());
    ...
    const VectorX<double>& position_lower_limits() const { return pos_lower_limits_; }

    /// Returns the position upper limits.
    const VectorX<double>& position_upper_limits() const { return pos_upper_limits_; }

    /// Returns the velocity lower limits.
    const VectorX<double>& velocity_lower_limits() const { return vel_lower_limits_; }

    /// Returns the velocity upper limits.
    const VectorX<double>& velocity_upper_limits() const { return vel_upper_limits_; }

    /// Returns the acceleration lower limits.
    const VectorX<double>& acceleration_lower_limits() const { return acc_lower_limits_; }

    /// Returns the acceleration upper limits.
    const VectorX<double>& acceleration_upper_limits() const { return acc_upper_limits_; }

    /// Returns the default positions.
    const VectorX<double>& default_positions() const { return default_positions_; }
    ...
    std::pair<Eigen::Quaternion<double>, Vector3<double>> GetDefaultPosePair() const { return DoGetDefaultPosePair(); }
    ...
    void SetDampingVector(systems::Context<T>* context, const VectorX<T>& damping) const {
        DRAKE_THROW_UNLESS(damping.size() == num_velocities());
        DRAKE_THROW_UNLESS((damping.array() >= 0).all());
        context->get_mutable_numeric_parameter(damping_parameter_index_).set_value(damping);
    }
    ...
    std::unique_ptr<Joint<ToScalar>> CloneToScalar(internal::MultibodyTree<ToScalar>* tree_clone) const {
        std::unique_ptr<Joint<ToScalar>> joint_clone = DoCloneToScalar(*tree_clone);

        std::unique_ptr<typename Joint<ToScalar>::JointImplementation> implementation_clone =
...
protected:
    ...
    struct JointImplementation {
        ...
        template <typename ToScalar>
        std::unique_ptr<typename Joint<ToScalar>::JointImplementation> CloneToScalar(
                internal::MultibodyTree<ToScalar>* tree_clone) const {
            auto implementation_clone = std::make_unique<typename Joint<ToScalar>::JointImplementation>();
            internal::Mobilizer<ToScalar>* mobilizer_clone = &tree_clone->get_mutable_variant(*mobilizer);
    ...
    virtual void DoSetDefaultPosePair(const Quaternion<double>& q_FM, const Vector3<double>& p_FM) {
        unused(q_FM, p_FM);
        throw std::logic_error(fmt::format("SetDefaultPose(): not implemented for joint type {}.", type_name()));
    }
    ...
    virtual std::pair<Eigen::Quaternion<double>, Vector3<double>> DoGetDefaultPosePair() const {
        throw std::logic_error(fmt::format("GetDefaultPose(): not implemented for joint type {}.", type_name()));
    }
    ...
    virtual const T& DoGetOnePosition(const systems::Context<T>&) const {
        throw std::domain_error("GetOnePosition can only be called on single-dof joints.");
    }
    ...
    virtual const T& DoGetOneVelocity(const systems::Context<T>&) const {
        throw std::domain_error("GetOneVelocity can only be called on single-dof joints.");
    }
...
private:
    ...
    void DoDeclareParameters(internal::MultibodyTreeSystem<T>* tree_system) final {
        ...
        damping_parameter_index_ = this->DeclareNumericParameter(tree_system, systems::BasicVector<T>(damping_.size()));
    }
