...
public:
    ...
    double position_lower_limit() const { return this->position_lower_limits()[0]; }

    /// Returns the position upper limit for `this` joint in meters.
    double position_upper_limit() const { return this->position_upper_limits()[0]; }

    /// Returns the velocity lower limit for `this` joint in meters per second.
    double velocity_lower_limit() const { return this->velocity_lower_limits()[0]; }

    /// Returns the velocity upper limit for `this` joint in meters per second.
    double velocity_upper_limit() const { return this->velocity_upper_limits()[0]; }

    /// Returns the acceleration lower limit for `this` joint in meters per second
    /// squared.
    double acceleration_lower_limit() const { return this->acceleration_lower_limits()[0]; }

    /// Returns the acceleration upper limit for `this` joint in meters per second
    /// squared.
    double acceleration_upper_limit() const { return this->acceleration_upper_limits()[0]; }
    ...
    const T& get_translation(const Context<T>& context) const { return get_mobilizer()->get_translation(context); }
    ...
    const T& GetDamping(const Context<T>& context) const { return this->GetDampingVector(context)[0]; }
    ...
    double get_default_translation() const { return this->default_positions()[0]; }

    /// Sets the `default_positions` of this joint (in this case a single
    /// translation)
    /// @param[in] translation
    ///   The desired default translation of the joint
    void set_default_translation(double translation) { this->set_default_positions(Vector1d{translation}); }

    void set_random_translation_distribution(const symbolic::Expression& translation) {
        get_mutable_mobilizer()->set_random_position_distribution(Vector1<symbolic::Expression>{translation});
    }
    ...
    void AddInForce(const systems::Context<T>& context, const T& force, MultibodyForces<T>* multibody_forces) const {
        ...
        DRAKE_DEMAND(multibody_forces->CheckHasRightSizeForModel(this->get_parent_tree()));
...
protected:
    ...
    void DoAddInOneForce(const systems::Context<T>&,
        ...
        Eigen::Ref<VectorX<T>> tau_mob =
                get_mobilizer()->get_mutable_generalized_forces_from_array(&forces->mutable_generalized_forces());
    ...
    void DoAddInDamping(const systems::Context<T>& context, MultibodyForces<T>* forces) const override {
        const T damping_force = -this->GetDamping(context) * get_translation_rate(context);
...
private:
    ...
    const T& DoGetOnePosition(const systems::Context<T>& context) const override { return get_translation(context); }
    ...
    // Joint<T> finals:
    std::unique_ptr<typename Joint<T>::BluePrint> MakeImplementationBlueprint() const final {
        auto blue_print = std::make_unique<typename Joint<T>::BluePrint>();
        auto prismatic_mobilizer = std::make_unique<internal::PrismaticMobilizer<T>>(this->frame_on_parent(),
                                                                                     this->frame_on_child(), axis_);
        prismatic_mobilizer->set_default_position(this->default_positions());
        blue_print->mobilizer = std::move(prismatic_mobilizer);
        return blue_print;
    }

    std::unique_ptr<Joint<double>> DoCloneToScalar(const internal::MultibodyTree<double>& tree_clone) const final;
    ...
    const internal::PrismaticMobilizer<T>* get_mobilizer() const {
        ...
        const internal::PrismaticMobilizer<T>* mobilizer =
                dynamic_cast<const internal::PrismaticMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::PrismaticMobilizer<T>* get_mutable_mobilizer() {
        // This implementation should always use a mobilizer.
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::PrismaticMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::PrismaticJoint);
