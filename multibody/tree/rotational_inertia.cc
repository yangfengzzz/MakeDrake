...
math::RotationMatrix<double> CalcCanonicalPrincipalDirections(const Vector3<double>& principal_moments_inertia,
...
Vector3<double> RotationalInertia<T>::CalcPrincipalMomentsAndMaybeAxesOfInertia(
    ...
    const double inertia_tolerance =
            4 * std::numeric_limits<double>::epsilon() * ExtractDoubleOrThrow(CalcMaximumPossibleMomentOfInertia());
    const bool is_diagonal =
            (std::abs(I_double(1, 0)) <= inertia_tolerance && std::abs(I_double(2, 0)) <= inertia_tolerance &&
    ...
    if (is_diagonal) {
        ...
        std::array I{Pair{I_double(0, 0), 0}, Pair{I_double(1, 1), 1}, Pair{I_double(2, 2), 2}};
        ...
        if (principal_directions != nullptr) {
            math::RotationMatrix<double> identity_matrix = math::RotationMatrix<double>::Identity();
            ...
            *principal_directions = math::RotationMatrix<double>::MakeFromOrthonormalColumns(
        ...
        const int compute_eigenvectors =
                principal_directions != nullptr ? Eigen::ComputeEigenvectors : Eigen::EigenvaluesOnly;
        ...
        if (principal_directions != nullptr) {
            ...
            *principal_directions = math::RotationMatrix<double>::MakeFromOrthonormalColumns(Bx_E, By_E, Bz_E);
        }
    ...
    if (principal_directions != nullptr) {
        *principal_directions =
                CalcCanonicalPrincipalDirections(principal_moments, *principal_directions, inertia_tolerance);
    }
    return principal_moments;
}

template <typename T>
void RotationalInertia<T>::ThrowNotPhysicallyValid(const char* func_name) const {
    ...
    if constexpr (scalar_predicate<T>::is_bool) {
        if (!IsNaN()) {
            ...
            if (!AreMomentsOfInertiaNearPositiveAndSatisfyTriangleInequality(p(0), p(1), p(2), /* epsilon = */ 0.0)) {
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(
        (static_cast<std::ostream& (*)(std::ostream&, const RotationalInertia<T>&)>(&operator<<)));

}  // namespace multibody
}  // namespace drake

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::RotationalInertia);
