...
template <typename T>
void LinearSpringDamper<T>::DoCalcAndAddForceContribution(const systems::Context<T>&,
    ...
    F_Bo_W_array[bodyA().mobod_index()] += SpatialForce<T>(Vector3<T>::Zero(), f_AP_W).Shift(p_PAo_W);

    // p_QBo = p_WBo - p_WQ
    const Vector3<T> p_QBo_W = X_WB.translation() - p_WQ;
    F_Bo_W_array[bodyB().mobod_index()] += SpatialForce<T>(Vector3<T>::Zero(), -f_AP_W).Shift(p_QBo_W);
}

template <typename T>
T LinearSpringDamper<T>::CalcPotentialEnergy(const systems::Context<T>&,
...
template <typename T>
T LinearSpringDamper<T>::CalcConservativePower(const systems::Context<T>&,
                                               const internal::PositionKinematicsCache<T>& pc,
...
template <typename T>
T LinearSpringDamper<T>::CalcNonConservativePower(const systems::Context<T>&,
                                                  const internal::PositionKinematicsCache<T>& pc,
                                                  const internal::VelocityKinematicsCache<T>& vc) const {
    // The rate at which the length of the spring changes.
    const T length_dot = CalcLengthTimeDerivative(pc, vc);
...
std::unique_ptr<ForceElement<ToScalar>> LinearSpringDamper<T>::TemplatedDoCloneToScalar(
    ...
    auto spring_damper_clone = std::make_unique<LinearSpringDamper<ToScalar>>(bodyA_clone, p_AP(), bodyB_clone, p_BQ(),
...
template <typename T>
std::unique_ptr<ForceElement<AutoDiffXd>> LinearSpringDamper<T>::DoCloneToScalar(
        const internal::MultibodyTree<AutoDiffXd>& tree_clone) const {
    return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<ForceElement<symbolic::Expression>> LinearSpringDamper<T>::DoCloneToScalar(
...
T LinearSpringDamper<T>::SafeSoftNorm(const Vector3<T>& x) const {
    using std::sqrt;
    const double epsilon_length = std::numeric_limits<double>::epsilon() * free_length();
...
template <typename T>
T LinearSpringDamper<T>::CalcLengthTimeDerivative(const internal::PositionKinematicsCache<T>& pc,
                                                  const internal::VelocityKinematicsCache<T>& vc) const {
    const math::RigidTransform<T>& X_WA = pc.get_X_WB(bodyA().mobod_index());
    const math::RigidTransform<T>& X_WB = pc.get_X_WB(bodyB().mobod_index());
    ...
    // p_PAo = p_WAo - p_WP
    const Vector3<T> p_PAo_W = X_WA.translation() - p_WP;
    const SpatialVelocity<T>& V_WP = vc.get_V_WB(bodyA().mobod_index()).Shift(-p_PAo_W);

    // p_QBo = p_WBo - p_WQ
    const Vector3<T> p_QBo_W = X_WB.translation() - p_WQ;
    const SpatialVelocity<T>& V_WQ = vc.get_V_WB(bodyB().mobod_index()).Shift(-p_QBo_W);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::LinearSpringDamper);
