...
public:
    ...
    const std::vector<const Element<T>*>& elements() const { return elements_packed_; }
    ...
    bool has_element(Index index) const {
        return index.is_valid() && (index < ssize(elements_by_index_)) && (elements_by_index_[index] != nullptr);
    }
    ...
    guaranteed to remain valid until the next call to any non-const member
    function. */
    const string_unordered_multimap<Index>& names_map() const { return names_map_; }
...
private:
    ...
    typename string_unordered_multimap<Index>::const_iterator FindNamesIterator(std::string_view name,
                                                                                Index index) const;
