...
#include "multibody/tree/model_instance.h"
...
const FrameType<T>& MultibodyTree<T>::AddFrame(std::unique_ptr<FrameType<T>> frame) {
    static_assert(std::is_convertible_v<FrameType<T>*, Frame<T>*>, "FrameType must be a sub-class of Frame<T>.");
    ...
    if (HasFrameNamed(frame->name(), frame->model_instance())) {
        throw std::logic_error(
                fmt::format("Model instance '{}' already contains a frame named '{}'. "
                            "Frame names must be unique within a given model.",
                            model_instances_.get_element(frame->model_instance()).name(), frame->name()));
    }
...
const MobilizerType<T>& MultibodyTree<T>::AddMobilizer(std::unique_ptr<MobilizerType<T>> mobilizer) {
    ...
    MobilizerIndex mobilizer_index = topology_.add_mobilizer(
            mobilizer->inboard_frame().index(), mobilizer->outboard_frame().index(), num_positions, num_velocities);
    ...
    bool is_body_floating =
            mobilizer->is_floating() && mobilizer->inboard_frame().body().index() == world_body().index();

    topology_.get_mutable_rigid_body(outboard_body_index).is_floating = is_body_floating;
    topology_.get_mutable_rigid_body(outboard_body_index).has_quaternion_dofs = mobilizer->has_quaternion_dofs();
...
const MobilizerType<T>& MultibodyTree<T>::AddMobilizer(Args&&... args) {
    static_assert(std::is_base_of_v<Mobilizer<T>, MobilizerType<T>>,
                  "MobilizerType must be a sub-class of Mobilizer<T>.");
    return AddMobilizer(std::make_unique<MobilizerType<T>>(std::forward<Args>(args)...));
}

template <typename T>
template <template <typename Scalar> class ForceElementType>
const ForceElementType<T>& MultibodyTree<T>::AddForceElement(std::unique_ptr<ForceElementType<T>> force_element) {
    ...
    auto gravity_element = dynamic_cast<UniformGravityFieldElement<T>*>(force_element.get());
    if (gravity_element) {
        if (gravity_field_) {
...
const ForceElementType<T>& MultibodyTree<T>::AddForceElement(Args&&... args) {
    static_assert(std::is_base_of_v<ForceElement<T>, ForceElementType<T>>,
                  ...
                  "ForceElement<T>.");
    return AddForceElement(std::make_unique<ForceElementType<T>>(std::forward<Args>(args)...));
}

template <typename T>
template <template <typename Scalar> class JointType>
const JointType<T>& MultibodyTree<T>::AddJoint(std::unique_ptr<JointType<T>> joint) {
    ...
    if (HasJointNamed(joint->name(), joint->model_instance())) {
        throw std::logic_error(
                fmt::format("Model instance '{}' already contains a joint named '{}'. Joint names "
                            "must be unique within a given model.",
                            model_instances_.get_element(joint->model_instance()).name(), joint->name()));
    }
    ...
    if (&joint->parent_body() == &joint->child_body()) {
        throw std::logic_error(fmt::format("AddJoint(): joint {} would connect body {} to itself.", joint->name(),
                                           joint->parent_body().name()));
    }

    if (&joint->parent_body().get_parent_tree() != &joint->child_body().get_parent_tree()) {
        throw std::logic_error(
                fmt::format("AddJoint(): can't add joint {} because bodies {} "
                            "and {} are from different MultibodyPlants.",
                            joint->name(), joint->parent_body().name(), joint->child_body().name()));
    }
...
const JointType<T>& MultibodyTree<T>::AddJoint(const std::string& name,
    ...
    const Frame<T>& frame_on_parent = this->AddOrGetJointFrame(parent, X_PF, joint_instance, name, "parent");
    const Frame<T>& frame_on_child = this->AddOrGetJointFrame(child, X_BM, joint_instance, name, "child");
    const JointType<T>& result = AddJoint(
            std::make_unique<JointType<T>>(name, frame_on_parent, frame_on_child, std::forward<Args>(args)...));
    DRAKE_DEMAND(result.model_instance() == joint_instance);
    return result;
}

template <typename T>
const JointActuator<T>& MultibodyTree<T>::AddJointActuator(const std::string& name,
                                                           const Joint<T>& joint,
                                                           double effort_limit) {
    if (HasJointActuatorNamed(name, joint.model_instance())) {
        throw std::logic_error(
                fmt::format("Model instance '{}' already contains a joint actuator named '{}'. "
    ...
    const JointActuatorIndex actuator_index = topology_.add_joint_actuator(joint.num_velocities());
...
ModelInstanceIndex MultibodyTree<T>::AddModelInstance(const std::string& name) {
    if (HasModelInstanceNamed(name)) {
        throw std::logic_error("This model already contains a model instance named '" + name +
...
template <typename T>
void MultibodyTree<T>::RenameModelInstance(ModelInstanceIndex model_instance, const std::string& name) {
...
template <typename T>
template <typename FromScalar>
RigidBody<T>* MultibodyTree<T>::CloneBodyAndAdd(const RigidBody<FromScalar>& body) {
    ...
    // RigidBodyAttorney.
    Frame<T>* body_frame_clone = &internal::RigidBodyAttorney<T>::get_mutable_body_frame(body_clone.get());
...
template <typename T>
template <typename FromScalar>
Mobilizer<T>* MultibodyTree<T>::CloneMobilizerAndAdd(const Mobilizer<FromScalar>& mobilizer) {
...
template <typename T>
template <typename FromScalar>
void MultibodyTree<T>::CloneForceElementAndAdd(const ForceElement<FromScalar>& force_element) {
...
template <typename T>
template <typename FromScalar>
void MultibodyTree<T>::CloneActuatorAndAdd(const JointActuator<FromScalar>& actuator) {
    std::unique_ptr<JointActuator<T>> actuator_clone = actuator.CloneToScalar(*this);
...
template <typename T>
Eigen::VectorBlock<const VectorX<T>> MultibodyTree<T>::get_positions_and_velocities(
    ...
    const systems::VectorBase<T>& continuous_qvz_base = context.get_continuous_state().get_vector();
    return extract_qv_from_continuous(continuous_qvz_base);
}

template <typename T>
Eigen::VectorBlock<const VectorX<T>> MultibodyTree<T>::get_positions(const systems::Context<T>& context) const {
    Eigen::VectorBlock<const VectorX<T>> qv = get_positions_and_velocities(context);
    return make_block_segment(qv, 0, num_positions());
}

template <typename T>
Eigen::VectorBlock<const VectorX<T>> MultibodyTree<T>::get_velocities(const systems::Context<T>& context) const {
    Eigen::VectorBlock<const VectorX<T>> qv = get_positions_and_velocities(context);
    return make_block_segment(qv, num_positions(), num_velocities());
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::GetMutablePositionsAndVelocities(systems::Context<T>* context) const {
    ...
    systems::VectorBase<T>& continuous_qvz_base = context->get_mutable_continuous_state().get_mutable_vector();
    return extract_mutable_qv_from_continuous(&continuous_qvz_base);
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::get_mutable_positions_and_velocities(systems::State<T>* state) const {
    ...
    systems::VectorBase<T>& continuous_qvz_base = state->get_mutable_continuous_state().get_mutable_vector();
    return extract_mutable_qv_from_continuous(&continuous_qvz_base);
}
...
template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::GetMutablePositions(systems::Context<T>* context) const {
...
template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::GetMutableVelocities(systems::Context<T>* context) const {
    Eigen::VectorBlock<VectorX<T>> qv = GetMutablePositionsAndVelocities(context);
    return make_mutable_block_segment(&qv, num_positions(), num_velocities());
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::get_mutable_positions(systems::State<T>* state) const {
    Eigen::VectorBlock<VectorX<T>> qv = get_mutable_positions_and_velocities(state);
    return make_mutable_block_segment(&qv, 0, num_positions());
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::get_mutable_velocities(systems::State<T>* state) const {
    Eigen::VectorBlock<VectorX<T>> qv = get_mutable_positions_and_velocities(state);
    return make_mutable_block_segment(&qv, num_positions(), num_velocities());
}
...
// anyway.)
template <typename T>
Eigen::VectorBlock<const VectorX<T>> MultibodyTree<T>::get_discrete_state_vector(
        const systems::Context<T>& context) const {
    DRAKE_ASSERT(is_state_discrete());
    DRAKE_ASSERT(discrete_state_index_.is_valid());
    // Only q and v.
    const systems::BasicVector<T>& discrete_state_vector = context.get_discrete_state(discrete_state_index_);
    DRAKE_ASSERT(discrete_state_vector.size() == num_positions() + num_velocities());
    return discrete_state_vector.value().head(discrete_state_vector.size());
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::get_mutable_discrete_state_vector(systems::Context<T>* context) const {
    DRAKE_ASSERT(context != nullptr);
    DRAKE_ASSERT(is_state_discrete());
    DRAKE_ASSERT(discrete_state_index_.is_valid());
    // Only q and v.
    systems::BasicVector<T>& discrete_state_vector = context->get_mutable_discrete_state(discrete_state_index_);
    DRAKE_ASSERT(discrete_state_vector.size() == num_positions() + num_velocities());
    return discrete_state_vector.get_mutable_value();
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::get_mutable_discrete_state_vector(systems::State<T>* state) const {
    DRAKE_ASSERT(state != nullptr);
    DRAKE_ASSERT(is_state_discrete());
    DRAKE_ASSERT(discrete_state_index_.is_valid());
    // Only q and v.
    systems::BasicVector<T>& discrete_state_vector = state->get_mutable_discrete_state(discrete_state_index_);
    DRAKE_ASSERT(discrete_state_vector.size() == num_positions() + num_velocities());
    return discrete_state_vector.get_mutable_value();
}
...
template <typename T>
Eigen::VectorBlock<const VectorX<T>> MultibodyTree<T>::extract_qv_from_continuous(
        const systems::VectorBase<T>& continuous_qvz_base) const {
    DRAKE_ASSERT(!is_state_discrete());
    DRAKE_ASSERT(dynamic_cast<const systems::BasicVector<T>*>(&continuous_qvz_base) != nullptr);
    const int num_qv = num_positions() + num_velocities();

    const systems::BasicVector<T>& continuous_qvz = static_cast<const systems::BasicVector<T>&>(continuous_qvz_base);
...
template <typename T>
Eigen::VectorBlock<VectorX<T>> MultibodyTree<T>::extract_mutable_qv_from_continuous(
        systems::VectorBase<T>* continuous_qvz_base) const {
    DRAKE_ASSERT(!is_state_discrete());
    DRAKE_ASSERT(continuous_qvz_base != nullptr);
    DRAKE_ASSERT(dynamic_cast<systems::BasicVector<T>*>(continuous_qvz_base) != nullptr);
    const int num_qv = num_positions() + num_velocities();

    systems::BasicVector<T>& continuous_qvz = static_cast<systems::BasicVector<T>&>(*continuous_qvz_base);
    DRAKE_ASSERT(continuous_qvz.size() >= num_qv);
