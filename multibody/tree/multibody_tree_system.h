...
public:
    ...
    explicit MultibodyTreeSystem(std::unique_ptr<MultibodyTree<T>> tree, bool is_discrete = false);
    ...
    const PositionKinematicsCache<T>& EvalPositionKinematics(const systems::Context<T>& context) const {
        ...
        return position_kinematics_cache_entry().template Eval<PositionKinematicsCache<T>>(context);
    }
    ...
    const VelocityKinematicsCache<T>& EvalVelocityKinematics(const systems::Context<T>& context) const {
        this->ValidateContext(context);
        return velocity_kinematics_cache_entry().template Eval<VelocityKinematicsCache<T>>(context);
    }
    ...
    const internal::AccelerationKinematicsCache<T>& EvalForwardDynamics(const systems::Context<T>& context) const {
        this->ValidateContext(context);
        return acceleration_kinematics_cache_entry().template Eval<AccelerationKinematicsCache<T>>(context);
    }
    ...
    const VectorX<T>& EvalReflectedInertiaCache(const systems::Context<T>& context) const {
        this->ValidateContext(context);
        return this->get_cache_entry(cache_indexes_.reflected_inertia).template Eval<VectorX<T>>(context);
    }
    ...
    const VectorX<T>& EvalJointDampingCache(const systems::Context<T>& context) const {
        this->ValidateContext(context);
        return this->get_cache_entry(cache_indexes_.joint_damping).template Eval<VectorX<T>>(context);
    }
    ...
    const std::vector<SpatialForce<T>>& EvalDynamicBiasCache(const systems::Context<T>& context) const {
        this->ValidateContext(context);
        return this->get_cache_entry(cache_indexes_.dynamic_bias).template Eval<std::vector<SpatialForce<T>>>(context);
    }
    ...
    const ArticulatedBodyForceCache<T>& EvalArticulatedBodyForceCache(const systems::Context<T>& context) const {
        this->ValidateContext(context);
...
protected:
    ...
    virtual void AddInForcesContinuous(const systems::Context<T>& context, MultibodyForces<T>* forces) const {
    ...
    virtual void DoCalcForwardDynamicsDiscrete(const systems::Context<T>& context,
                                               AccelerationKinematicsCache<T>* ac) const {
        unused(context, ac);
        throw std::logic_error("DoCalcForwardDynamicsDiscrete(): invoked but not implemented.");
    }
    //@}
    ...
    void SetDefaultParameters(const systems::Context<T>& context, systems::Parameters<T>* parameters) const final;
...
private:
    ...
    void DoCalcImplicitTimeDerivativesResidual(const systems::Context<T>& context,
    ...
    void CalcCompositeBodyInertiasInWorld(const systems::Context<T>& context,
        ...
        internal_tree().CalcCompositeBodyInertiasInWorld(context, composite_body_inertias);
    }

    void CalcAcrossNodeJacobianWrtVExpressedInWorld(const systems::Context<T>& context,
                                                    std::vector<Vector6<T>>* H_PB_W_all) const {
        internal_tree().CalcAcrossNodeJacobianWrtVExpressedInWorld(context, EvalPositionKinematics(context),
                                                                   H_PB_W_all);
    }
    ...
    void CalcVelocityKinematicsCache(const systems::Context<T>& context,
        ...
        internal_tree().CalcVelocityKinematicsCache(context, EvalPositionKinematics(context), velocity_cache);
    }
    ...
    void CalcForwardDynamics(const systems::Context<T>& context, AccelerationKinematicsCache<T>* ac) const {
        if (is_discrete())
    ...
    void CalcForwardDynamicsContinuous(const systems::Context<T>& context,
    ...
    void CalcForwardDynamicsDiscrete(const systems::Context<T>& context, AccelerationKinematicsCache<T>* ac) const {
...
private:
    ...
    static systems::NumericParameterIndex DeclareNumericParameter(MultibodyTreeSystem<T>* tree_system,
                                                                  const systems::BasicVector<T>& model_vector) {
        return systems::NumericParameterIndex{tree_system->DeclareNumericParameter(model_vector)};
    }

    static systems::AbstractParameterIndex DeclareAbstractParameter(MultibodyTreeSystem<T>* tree_system,
                                                                    const AbstractValue& model_value) {
        return systems::AbstractParameterIndex{tree_system->DeclareAbstractParameter(model_value)};
    }

    static systems::CacheEntry& DeclareCacheEntry(MultibodyTreeSystem<T>* tree_system,
                                                  std::string description,
                                                  systems::ValueProducer value_producer,
                                                  std::set<systems::DependencyTicket> prerequisites_of_calc) {
        DRAKE_DEMAND(tree_system != nullptr);
        return tree_system->DeclareCacheEntry(std::move(description), std::move(value_producer),
                                              std::move(prerequisites_of_calc));
    }

    static systems::InputPort<T>& DeclareAbstractInputPort(MultibodyTreeSystem<T>* tree_system,
                                                           std::string name,
                                                           const AbstractValue& model_value) {
        DRAKE_DEMAND(tree_system != nullptr);
    ...
    static systems::InputPort<T>& DeclareVectorInputPort(MultibodyTreeSystem<T>* tree_system,
                                                         std::string name,
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::multibody::internal::MultibodyTreeSystem);
