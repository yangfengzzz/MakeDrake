...
#include "multibody/tree/mobilizer_impl.h"
...
public:
    ...
    const QuaternionFloatingMobilizer<T>& SetQuaternion(systems::Context<T>* context, const Quaternion<T>& q_FM) const;
    ...
    const QuaternionFloatingMobilizer<T>& SetQuaternion(const systems::Context<T>& context,
    ...
    // @returns a constant reference to `this` mobilizer.
    const QuaternionFloatingMobilizer<T>& SetTranslation(systems::Context<T>* context, const Vector3<T>& p_FM) const;

    // Alternative signature to SetTranslation(context, p_FM) to set `state` to
    // store the position `p_FM` of M in F.
    const QuaternionFloatingMobilizer<T>& SetTranslation(const systems::Context<T>& context,
                                                         const Vector3<T>& p_FM,
                                                         systems::State<T>* state) const;

    // Sets the distribution governing the random samples of the position
    // component of the mobilizer state.
    void set_random_translation_distribution(const Vector3<symbolic::Expression>& position);
    ...
    // @returns a constant reference to `this` mobilizer.
    const QuaternionFloatingMobilizer<T>& SetOrientation(systems::Context<T>* context,
    ...
    // @returns a constant reference to `this` mobilizer.
    const QuaternionFloatingMobilizer<T>& SetAngularVelocity(systems::Context<T>* context,
    ...
    const QuaternionFloatingMobilizer<T>& SetAngularVelocity(const systems::Context<T>&,
    ...
    // @returns a constant reference to `this` mobilizer.
    const QuaternionFloatingMobilizer<T>& SetTranslationalVelocity(systems::Context<T>* context,
    ...
    const QuaternionFloatingMobilizer<T>& SetTranslationalVelocity(const systems::Context<T>&,
    ...
    // @{
    math::RigidTransform<T> CalcAcrossMobilizerTransform(const systems::Context<T>& context) const final;

    SpatialVelocity<T> CalcAcrossMobilizerSpatialVelocity(const systems::Context<T>& context,
                                                          const Eigen::Ref<const VectorX<T>>& v) const final;

    SpatialAcceleration<T> CalcAcrossMobilizerSpatialAcceleration(const systems::Context<T>& context,
...
protected:
    ...
    void DoCalcNMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> N) const final;

    void DoCalcNplusMatrix(const systems::Context<T>& context, EigenPtr<MatrixX<T>> Nplus) const final;
...
private:
    ...
    // N(q) = L(q_FM/2)
    static Eigen::Matrix<T, 4, 3> AngularVelocityToQuaternionRateMatrix(const Quaternion<T>& q);
    ...
    // With L given by CalcLMatrix we have:
    // N⁺(q) = L(2 q_FM)ᵀ
    static Eigen::Matrix<T, 3, 4> QuaternionRateToAngularVelocityMatrix(const Quaternion<T>& q);

    // Helper method to make a clone templated on ToScalar.
    template <typename ToScalar>
    std::unique_ptr<Mobilizer<ToScalar>> TemplatedDoCloneToScalar(const MultibodyTree<ToScalar>& tree_clone) const;
};
