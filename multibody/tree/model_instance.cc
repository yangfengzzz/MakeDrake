...
template <typename T>
std::vector<JointActuatorIndex> ModelInstance<T>::GetJointActuatorIndices() const {
...
template <typename T>
VectorX<T> ModelInstance<T>::GetActuationFromArray(const Eigen::Ref<const VectorX<T>>& u) const {
    ...
    for (const JointActuator<T>* actuator : joint_actuators_) {
        ...
        u_instance.segment(u_instance_offset, num_dofs) = actuator->get_actuation_vector(u);
...
template <typename T>
void ModelInstance<T>::SetActuationInArray(const Eigen::Ref<const VectorX<T>>& u_instance,
                                           EigenPtr<VectorX<T>> u) const {
    DRAKE_DEMAND(u != nullptr);
    if (u->size() != this->get_parent_tree().num_actuated_dofs() || u_instance.size() != num_actuated_dofs_) {
    ...
    for (const JointActuator<T>* actuator : joint_actuators_) {
        const int num_dofs = actuator->joint().num_velocities();
        actuator->set_actuation_vector(u_instance.segment(u_instance_offset, num_dofs), u);
        u_instance_offset += num_dofs;
        DRAKE_DEMAND(u_instance_offset <= u->size());
    }
}
...
template <typename T>
VectorX<T> ModelInstance<T>::GetPositionsFromArray(const Eigen::Ref<const VectorX<T>>& q_array) const {
...
template <typename T>
void ModelInstance<T>::GetPositionsFromArray(const Eigen::Ref<const VectorX<T>>& q, EigenPtr<VectorX<T>> q_out) const {
    DRAKE_DEMAND(q_out != nullptr);
    if (q.size() != this->get_parent_tree().num_positions())
        throw std::logic_error("Passed in array is not properly sized.");
    if (q_out->size() != num_positions_) throw std::logic_error("Output array is not properly sized.");
    int position_offset = 0;
    for (const Mobilizer<T>* mobilizer : mobilizers_) {
        const int mobilizer_positions = mobilizer->num_positions();
        q_out->segment(position_offset, mobilizer_positions) = mobilizer->get_positions_from_array(q);
...
template <class T>
void ModelInstance<T>::SetPositionsInArray(const Eigen::Ref<const VectorX<T>>& model_q,
                                           EigenPtr<VectorX<T>> q_array) const {
    DRAKE_DEMAND(q_array != nullptr);
    if (q_array->size() != this->get_parent_tree().num_positions() || model_q.size() != num_positions()) {
        throw std::logic_error("Passed in array(s) is not properly sized.");
    }
    int position_offset = 0;
    for (const Mobilizer<T>* mobilizer : mobilizers_) {
...
template <typename T>
VectorX<T> ModelInstance<T>::GetVelocitiesFromArray(const Eigen::Ref<const VectorX<T>>& v_array) const {
...
template <typename T>
void ModelInstance<T>::GetVelocitiesFromArray(const Eigen::Ref<const VectorX<T>>& v, EigenPtr<VectorX<T>> v_out) const {
    DRAKE_DEMAND(v_out != nullptr);
    if (v.size() != this->get_parent_tree().num_velocities())
        throw std::logic_error("Passed in array is not properly sized.");
    if (v_out->size() != num_velocities_) throw std::logic_error("Output array is not properly sized.");
    int velocity_offset = 0;
    for (const Mobilizer<T>* mobilizer : mobilizers_) {
        const int mobilizer_velocities = mobilizer->num_velocities();
        v_out->segment(velocity_offset, mobilizer_velocities) = mobilizer->get_velocities_from_array(v);
...
template <class T>
void ModelInstance<T>::SetVelocitiesInArray(const Eigen::Ref<const VectorX<T>>& model_v,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::ModelInstance);
