...
public:
    ...
    JointActuatorIndex index() const { return this->template index_impl<JointActuatorIndex>(); }
    ...
    /// See @ref reflected_inertia.
    void set_default_rotor_inertia(double rotor_inertia) { default_rotor_inertia_ = rotor_inertia; }

    /// Sets the default value for this actuator's gear ratio.
    /// See @ref reflected_inertia.
    void set_default_gear_ratio(double gear_ratio) { default_gear_ratio_ = gear_ratio; }
    ...
    void SetRotorInertia(systems::Context<T>* context, const T& rotor_inertia) const {
        context->get_mutable_numeric_parameter(rotor_inertia_parameter_index_)[0] = rotor_inertia;
    }
    ...
    void SetGearRatio(systems::Context<T>* context, const T& gear_ratio) const {
        context->get_mutable_numeric_parameter(gear_ratio_parameter_index_)[0] = gear_ratio;
    }
    ...
    std::unique_ptr<JointActuator<ToScalar>> CloneToScalar(const internal::MultibodyTree<ToScalar>& cloned_tree) const {
...
private:
    ...
    // `double`.
    std::unique_ptr<JointActuator<double>> DoCloneToScalar(const internal::MultibodyTree<double>& tree_clone) const;
    ...
    void DoDeclareParameters(internal::MultibodyTreeSystem<T>* tree_system) final {
        ...
        rotor_inertia_parameter_index_ = this->DeclareNumericParameter(tree_system, systems::BasicVector<T>(1));
        gear_ratio_parameter_index_ = this->DeclareNumericParameter(tree_system, systems::BasicVector<T>(1));
    }
    ...
    void DoSetDefaultParameters(systems::Parameters<T>* parameters) const final {
        ...
        systems::BasicVector<T>& rotor_inertia_parameter =
                parameters->get_mutable_numeric_parameter(rotor_inertia_parameter_index_);
