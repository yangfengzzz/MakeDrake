...
public:
    ...
    explicit ArticulatedBodyInertiaCache(const MultibodyTreeTopology& topology) : num_mobods_(topology.num_mobods()) {
        Allocate();
    }
    ...
    // articulated body inertia of parent body P as though it were inertialess,
    // but taken about Bo and expressed in W.
    const ArticulatedBodyInertia<T>& get_Pplus_PB_W(MobodIndex mobod_index) const {
        DRAKE_ASSERT(0 <= mobod_index && mobod_index < num_mobods_);
    ...
    // LLT factorization `llt_D_B` of the articulated body hinge inertia.
    const math::LinearSolver<Eigen::LLT, MatrixUpTo6<T>>& get_llt_D_B(MobodIndex mobod_index) const {
        DRAKE_ASSERT(0 <= mobod_index && mobod_index < num_mobods_);
        return llt_D_B_[mobod_index];
    }

    // Mutable version of get_llt_D_B().
    math::LinearSolver<Eigen::LLT, MatrixUpTo6<T>>& get_mutable_llt_D_B(MobodIndex mobod_index) {
        DRAKE_ASSERT(0 <= mobod_index && mobod_index < num_mobods_);
        return llt_D_B_[mobod_index];
    }
