...
public:
    ...
    explicit ArticulatedBodyInertia(const SpatialInertia<T>& M_SQ_E) {
        matrix_.template triangularView<Eigen::Lower>() = M_SQ_E.CopyToFullMatrix6();
    }
    ...
    ArticulatedBodyInertia<Scalar> cast() const {
        ...
        P.matrix_.template triangularView<Eigen::Lower>() = matrix_.template cast<Scalar>();
        return P;
    }
    ...
    typename std::enable_if_t<scalar_predicate<T1>::is_bool, bool> IsPhysicallyValid() const {
        ...
        const auto eigvals = matrix_.template selfadjointView<Eigen::Lower>().eigenvalues();
        ...
        const T tolerance = -eigvals.norm() * std::numeric_limits<double>::epsilon() * kEta;
    ...
    void ShiftInPlace(const Vector3<T>& p_QR_E) {
        ...
        Eigen::TriangularView<Eigen::Block<Matrix6<T>, 3, 3>, Eigen::Lower> J =
                matrix_.template block<3, 3>(0, 0).template triangularView<Eigen::Lower>();
        Eigen::Transpose<Eigen::Block<Matrix6<T>, 3, 3>> F = matrix_.template block<3, 3>(3, 0).transpose();
        Eigen::Block<Matrix6<T>, 3, 3> Fp = matrix_.template block<3, 3>(0, 3);
        Matrix3<T> M = matrix_.template block<3, 3>(3, 3).template selfadjointView<Eigen::Lower>();
    ...
    const Eigen::Product<Eigen::SelfAdjointView<const Matrix6<T>, Eigen::Lower>, OtherDerived> operator*(
            const Eigen::MatrixBase<OtherDerived>& rhs) const {
    ...
    /// during compile time.
    template <typename OtherDerived>
    friend const Eigen::Product<OtherDerived, Eigen::SelfAdjointView<const Matrix6<T>, Eigen::Lower>> operator*(
            const Eigen::MatrixBase<OtherDerived>& lhs, const ArticulatedBodyInertia& rhs) {
