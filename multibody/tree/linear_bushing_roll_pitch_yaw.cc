...
template <typename T>
LinearBushingRollPitchYaw<T>::LinearBushingRollPitchYaw(const Frame<T>& frameA,
...
template <typename T>
LinearBushingRollPitchYaw<T>::LinearBushingRollPitchYaw(ModelInstanceIndex model_instance,
...
template <typename T>
void LinearBushingRollPitchYaw<T>::DoCalcAndAddForceContribution(const systems::Context<T>& context,
    ...
    const Vector3<T> p_L0Ao_L0 = frameA().CalcPoseInBodyFrame(context).translation();
    ...
    std::vector<SpatialForce<T>>& F_BodyOrigin_W_array = forces->mutable_body_forces();
...
template <typename T>
math::RotationMatrix<T> LinearBushingRollPitchYaw<T>::CalcR_AB(math::RotationMatrix<T> R_AC) {
...
template <typename T>
void LinearBushingRollPitchYaw<T>::ThrowIfInvalidHalfAngleAxis(const math::RotationMatrix<T>& R_AC,
...
template <typename T>
Vector3<T> LinearBushingRollPitchYaw<T>::CalcBushing_xyzDt(const systems::Context<T>& context) const {
    // Calculate V_AC_A, frame C's spatial velocity in frame A, expressed in A.
    const SpatialVelocity<T> V_AC_A = frameC().CalcSpatialVelocity(context, frameA(), frameA());
...
template <typename T>
Vector3<T> LinearBushingRollPitchYaw<T>::CalcBushingTorqueOnCExpressedInA(const systems::Context<T>& context) const {
...
template <typename T>
void LinearBushingRollPitchYaw<T>::ThrowPitchAngleViolatesGimbalLockTolerance(const T& pitch_angle,
                                                                              const char* function_name) {
    const double pitch_radians = ExtractDoubleOrThrow(pitch_angle);
    const double pitch_tolerance = math::RollPitchYaw<double>::GimbalLockPitchAngleTolerance();
...
template <typename T>
T LinearBushingRollPitchYaw<T>::CalcPotentialEnergy(const systems::Context<T>&,
...
template <typename T>
T LinearBushingRollPitchYaw<T>::CalcConservativePower(const systems::Context<T>&,
                                                      const internal::PositionKinematicsCache<T>& /* pc */,
...
template <typename T>
T LinearBushingRollPitchYaw<T>::CalcNonConservativePower(const systems::Context<T>&,
                                                         const internal::PositionKinematicsCache<T>& /* pc */,
                                                         const internal::VelocityKinematicsCache<T>& /* vc */) const {
    // TODO(Mitiguy) Per issues #12982 and #12752, implement the following method.
    //  Currently this method has not been implemented and throws an exception.
...
std::unique_ptr<ForceElement<ToScalar>> LinearBushingRollPitchYaw<T>::TemplatedDoCloneToScalar(
    ...
    std::unique_ptr<LinearBushingRollPitchYaw<ToScalar>> bushing_clone(new LinearBushingRollPitchYaw<ToScalar>(
            this->model_instance(), frameA_index_, frameC_index_, k012, d012, kxyz, dxyz));

    return bushing_clone;
}

template <typename T>
std::unique_ptr<ForceElement<double>> LinearBushingRollPitchYaw<T>::DoCloneToScalar(
        const internal::MultibodyTree<double>& tree_clone) const {
    return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<ForceElement<AutoDiffXd>> LinearBushingRollPitchYaw<T>::DoCloneToScalar(
        const internal::MultibodyTree<AutoDiffXd>& tree_clone) const {
    return TemplatedDoCloneToScalar(tree_clone);
}

template <typename T>
std::unique_ptr<ForceElement<symbolic::Expression>> LinearBushingRollPitchYaw<T>::DoCloneToScalar(
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::LinearBushingRollPitchYaw);
