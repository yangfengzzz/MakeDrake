...
#include "multibody/topology/multibody_graph.h"
#include "multibody/tree/acceleration_kinematics_cache.h"
#include "multibody/tree/articulated_body_force_cache.h"
#include "multibody/tree/articulated_body_inertia_cache.h"
#include "multibody/tree/element_collection.h"
...
#include "multibody/tree/position_kinematics_cache.h"
#include "multibody/tree/spatial_inertia.h"
#include "multibody/tree/velocity_kinematics_cache.h"
...
public:
    ...
    int tree_height() const { return topology_.forest_height(); }
    ...
    // See MultibodyPlant method.
    const RigidBody<T>& get_body(BodyIndex body_index) const { return rigid_bodies_.get_element(body_index); }

    RigidBody<T>& get_mutable_body(BodyIndex body_index) { return rigid_bodies_.get_mutable_element(body_index); }

    // See MultibodyPlant method.
    bool has_joint(JointIndex joint_index) const { return joints_.has_element(joint_index); }

    // See MultibodyPlant method.
    const Joint<T>& get_joint(JointIndex joint_index) const { return joints_.get_element(joint_index); }

    // See MultibodyPlant method.
    Joint<T>& get_mutable_joint(JointIndex joint_index) { return joints_.get_mutable_element(joint_index); }

    // See MultibodyPlant method.
    bool has_joint_actuator(JointActuatorIndex actuator_index) const { return actuators_.has_element(actuator_index); }
    ...
    // See MultibodyPlant method.
    const Frame<T>& get_frame(FrameIndex frame_index) const { return frames_.get_element(frame_index); }

    Frame<T>& get_mutable_frame(FrameIndex frame_index) { return frames_.get_mutable_element(frame_index); }
    ...
    const ForceElementType<T>& GetForceElement(ForceElementIndex force_element_index) const {
        ...
        const ForceElementType<T>* typed_force_element = dynamic_cast<const ForceElementType<T>*>(force_element);
        if (typed_force_element == nullptr) {
            throw std::logic_error(fmt::format("ForceElement is not of type '{}' but of type '{}'.",
    ...
    ForceElement<T>& get_mutable_force_element(ForceElementIndex force_element_index) {
        DRAKE_THROW_UNLESS(force_element_index < num_force_elements());
        return *owned_force_elements_[force_element_index];
    }
    ...
    const RigidBody<T>& GetUniqueFreeBaseBodyOrThrowImpl(ModelInstanceIndex model_instance) const;
    ...
    // See MultibodyPlant method.
    const std::vector<JointIndex>& GetJointIndices() const { return joints_.indices(); }
    ...
    // See MultibodyPlant method.
    const std::vector<JointActuatorIndex>& GetJointActuatorIndices() const { return actuators_.indices(); }
    ...
    const JointType<T>& GetJointByName(std::string_view name,
        ...
        const JointType<T>* const typed_joint = dynamic_cast<const JointType<T>*>(&joint);
    ...
    JointType<T>& GetMutableJointByName(std::string_view name,
        ...
        const JointType<T>& const_joint = GetJointByName<JointType>(name, model_instance);
    ...
    Eigen::VectorBlock<const VectorX<T>> get_positions_and_velocities(const systems::Context<T>& context) const;
    ...
    Eigen::VectorBlock<VectorX<T>> get_mutable_positions_and_velocities(systems::State<T>* state) const;
    ...
    math::RigidTransform<T> GetFreeBodyPoseOrThrow(const systems::Context<T>& context, const RigidBody<T>& body) const;
    ...
    void SetFreeBodyRandomTranslationDistributionOrThrow(const RigidBody<T>& body,
                                                         const Vector3<symbolic::Expression>& translation);

    // See MultibodyPlant::SetFreeBodyRandomRotationDistribution.
    void SetFreeBodyRandomRotationDistributionOrThrow(const RigidBody<T>& body,
    ...
    // See MultibodyPlant method.
    void CalcAllBodyPosesInWorld(const systems::Context<T>& context, std::vector<math::RigidTransform<T>>* X_WB) const;
    ...
    // num_bodies().
    void CalcSpatialInertiasInWorld(const systems::Context<T>& context,
    ...
    // not num_velocities().
    void CalcReflectedInertia(const systems::Context<T>& context, VectorX<T>* reflected_inertia) const;
    ...
    // num_velocities().
    void CalcJointDamping(const systems::Context<T>& context, VectorX<T>* joint_damping) const;
    ...
    // num_bodies().
    void CalcCompositeBodyInertiasInWorld(const systems::Context<T>& context,
    ...
    // num_bodies().
    void CalcDynamicBiasForces(const systems::Context<T>& context, std::vector<SpatialForce<T>>* Fb_Bo_W_all) const;
    ...
    Quantity                            |    Featherstone 2008 †    |              Jain 2010 ††              | Drake
    monogram †††
    ------------------------------------|:-------------------------:|:--------------------------------------:|:-------------------
    ...
    Outboard body spatial acceleration  | 𝒂ᵢ = 𝒂ₗ + 𝒄ᵢ + 𝑺ᵢq̈ᵢ (7.31) | α(k) = α⁺(k) + 𝔞(k) + H*(k)v(k) (5.21) | A_WB =
    Aplus_WB + Ab_WB + H_PB_W * vdot_B Articulated Body Inertia (ABI)      |  𝑰ᴬᵢ                (7.2)  | P(k)   (6.6)
    | P_B_W ABI across the mobilizer            |  𝑰ᵃᵢ                (7.23) | P⁺(k)  (6.24)                         |
    Pplus_B_W ABA force bias                      |  𝒑ᴬᵢ                (7.2)  | 𝔷(k)   (6.6) | Z_B_W ABA force bias
    across the mobilizer |  𝒑ᵃᵢ                (7.24) | 𝔷⁺(k)  (6.33)                          | Zplus_B_W
    ...
    */
    ...
    // for further details.
    void CalcArticulatedBodyForceCache(const systems::Context<T>& context,
    ...
    // further details.
    void CalcArticulatedBodyAccelerations(const systems::Context<T>& context,
    ...
    void CalcArticulatedBodyForceBias(const systems::Context<T>& context,
    ...
    std::enable_if_t<std::is_base_of_v<Frame<T>, MultibodyElement<T>>, const MultibodyElement<T>&> get_variant(
    ...
    template <template <typename> class MultibodyElement, typename Scalar>
    std::enable_if_t<std::is_base_of_v<RigidBody<T>, MultibodyElement<T>>, const MultibodyElement<T>&> get_variant(
            const MultibodyElement<Scalar>& element) const {
    ...
    template <template <typename> class MultibodyElement, typename Scalar>
    std::enable_if_t<std::is_base_of_v<Mobilizer<T>, MultibodyElement<T>>, const MultibodyElement<T>&> get_variant(
            const MultibodyElement<Scalar>& element) const {
    ...
    // SFINAE overload for Mobilizer<T> elements.
    template <template <typename> class MultibodyElement, typename Scalar>
    std::enable_if_t<std::is_base_of_v<Mobilizer<T>, MultibodyElement<T>>, MultibodyElement<T>&> get_mutable_variant(
    ...
    template <template <typename> class MultibodyElement, typename Scalar>
    std::enable_if_t<std::is_base_of_v<Joint<T>, MultibodyElement<T>>, const MultibodyElement<T>&> get_variant(
            const MultibodyElement<Scalar>& element) const {
    ...
    std::unique_ptr<MultibodyTree<AutoDiffXd>> ToAutoDiffXd() const { return CloneToScalar<AutoDiffXd>(); }
    ...
    std::unique_ptr<MultibodyTree<ToScalar>> CloneToScalar() const {
        ...
        for (BodyIndex index(1); index < num_bodies(); ++index) {
            ...
            tree_clone->multibody_graph_.AddRigidBody(body.name(), body.model_instance());
        }
        ...
        tree_clone->gravity_field_ =
                dynamic_cast<UniformGravityFieldElement<ToScalar>*>(tree_clone->owned_force_elements_[0].get());
    ...
    const AccelerationKinematicsCache<T>& EvalAccelerationKinematics(const systems::Context<T>& context) const {
        DRAKE_ASSERT(tree_system_ != nullptr);
        return tree_system_->EvalForwardDynamics(context);
    }

    // Evaluate the cache entry storing articulated body inertias in `context`.
    const ArticulatedBodyInertiaCache<T>& EvalArticulatedBodyInertiaCache(const systems::Context<T>& context) const {
    ...
    // @pre `context` is a valid multibody system Context.
    Eigen::VectorBlock<const VectorX<T>> get_positions(const systems::Context<T>& context) const;
    ...
    // @pre `state` is a valid multibody system State.
    Eigen::VectorBlock<VectorX<T>> get_mutable_positions(systems::State<T>* state) const;

    // Returns a const reference to the velocity state vector v stored in the
    // given Context as an `Eigen::VectorBlock<const VectorX<T>>`.
    // @pre `context` is a valid multibody system Context.
    Eigen::VectorBlock<const VectorX<T>> get_velocities(const systems::Context<T>& context) const;
    ...
    // @note This does not cause cache invalidation so it is very fast.
    // @pre `state` is a valid multibody system State.
    Eigen::VectorBlock<VectorX<T>> get_mutable_velocities(systems::State<T>* state) const;
    ...
    template <int length>
    Eigen::VectorBlock<const VectorX<T>, length> get_state_segment(const systems::Context<T>& context,
                                                                   int start) const {
        Eigen::VectorBlock<const VectorX<T>> qv = get_positions_and_velocities(context);
    ...
    // @pre start, length >= 0 and start+length is in-bounds for qv state.
    template <int length>
    Eigen::VectorBlock<VectorX<T>, length> GetMutableStateSegment(systems::Context<T>* context, int start) const {
    ...
    // @pre start, length >= 0 and start+length is in-bounds for qv state.
    template <int length>
    Eigen::VectorBlock<VectorX<T>, length> get_mutable_state_segment(systems::State<T>* state, int start) const {
        Eigen::VectorBlock<VectorX<T>> qv = get_mutable_positions_and_velocities(state);
        return make_mutable_block_segment<length>(&qv, start);
    }
    ...
    // @pre `context` is a valid multibody system Context.
    // @pre start, length >= 0 and start+length is in-bounds for qv state.
    Eigen::VectorBlock<const VectorX<T>> get_state_segment(const systems::Context<T>& context,
                                                           int start,
                                                           int length) const {
        Eigen::VectorBlock<const VectorX<T>> qv = get_positions_and_velocities(context);
    ...
    // @pre `context` is a valid multibody system Context.
    // @pre start, length >= 0 and start+length is in-bounds for qv state.
    Eigen::VectorBlock<VectorX<T>> GetMutableStateSegment(systems::Context<T>* context, int start, int length) const {
        Eigen::VectorBlock<VectorX<T>> qv = GetMutablePositionsAndVelocities(context);
    ...
    // @pre `state` is a valid multibody system State.
    // @pre start, length >= 0 and start+length is in-bounds for qv state.
    Eigen::VectorBlock<VectorX<T>> get_mutable_state_segment(systems::State<T>* state, int start, int length) const {
        Eigen::VectorBlock<VectorX<T>> qv = get_mutable_positions_and_velocities(state);
        return make_mutable_block_segment(&qv, start, length);
    }
    //@}
    ...
    void CalcAcrossNodeJacobianWrtVExpressedInWorld(const systems::Context<T>& context,
                                                    const PositionKinematicsCache<T>& pc,
    ...
    // @param[in] body_indexes A set of BodyIndex.
    bool IsAnyDefaultRotationalInertiaNaN(const std::set<BodyIndex>& body_indexes) const;

    // In the set of bodies associated with BodyIndex, returns true if all the
    // bodies have a zero default rotational inertia.
    // @param[in] body_indexes A set of BodyIndex.
    bool AreAllDefaultRotationalInertiaZero(const std::set<BodyIndex>& body_indexes) const;
    ...
    const internal::MultibodyGraph& multibody_graph() const { return multibody_graph_; }
...
private:
    ...
    // discrete state.
    Eigen::VectorBlock<const VectorX<T>> get_discrete_state_vector(const systems::Context<T>& context) const;

    // Invalidates q- or v-dependent cache entries.
    Eigen::VectorBlock<VectorX<T>> get_mutable_discrete_state_vector(systems::Context<T>* context) const;

    // Does no invalidation.
    Eigen::VectorBlock<VectorX<T>> get_mutable_discrete_state_vector(systems::State<T>* state) const;

    // @pre the VectorBase is a BasicVector.
    Eigen::VectorBlock<const VectorX<T>> extract_qv_from_continuous(const systems::VectorBase<T>& continuous_qvz) const;

    // @pre the VectorBase is non-null and is a BasicVector.
    Eigen::VectorBlock<VectorX<T>> extract_mutable_qv_from_continuous(systems::VectorBase<T>* continuous_qvz) const;
    ...
    static Eigen::VectorBlock<const VectorX<T>> make_block_segment(const Eigen::VectorBlock<const VectorX<T>>& block,
                                                                   int start,
                                                                   int length) {
    ...
    static Eigen::VectorBlock<VectorX<T>> make_mutable_block_segment(Eigen::VectorBlock<VectorX<T>>* block,
                                                                     int start,
                                                                     int length) {
    ...
    static Eigen::VectorBlock<const VectorX<T>, length> make_block_segment(
        ...
        static_assert(length >= 0, "make_block_segment(): length must be non-negative");
        DRAKE_ASSERT(start >= 0);
        DRAKE_ASSERT(start + length <= block.rows());
        return block.nestedExpression().template segment<length>(block.startRow() + start);
    }

    template <int length>
    static Eigen::VectorBlock<VectorX<T>, length> make_mutable_block_segment(Eigen::VectorBlock<VectorX<T>>* block,
                                                                             int start) {
        static_assert(length >= 0, "make_mutable_block_segment(): length must be non-negative");
        DRAKE_ASSERT(block != nullptr);
        DRAKE_ASSERT(start >= 0);
        DRAKE_ASSERT(start + length <= block->rows());
        return block->nestedExpression().template segment<length>(block->startRow() + start);
    ...
    const Joint<T>& GetJointByNameImpl(std::string_view, std::optional<ModelInstanceIndex>) const;

    [[noreturn]] void ThrowJointSubtypeMismatch(const Joint<T>&, std::string_view) const;
    ...
    // and returns it.
    const Frame<T>& AddOrGetJointFrame(const RigidBody<T>& body,
    ...
    SpatialAcceleration<T> CalcSpatialAccelerationHelper(const systems::Context<T>& context,
                                                         const Frame<T>& frame_F,
    ...
    SpatialAcceleration<T> ShiftSpatialAccelerationInWorld(const Frame<T>& frame_B,
                                                           const Eigen::Ref<const Vector3<T>>& p_BoBp_B,
    ...
    void CalcAllBodyBiasSpatialAccelerationsInWorld(const systems::Context<T>& context,
                                                    JacobianWrtVariable with_respect_to,
    ...
    SpatialMomentum<T> CalcBodiesSpatialMomentumInWorldAboutWo(const systems::Context<T>& context,
                                                               const std::vector<BodyIndex>& body_indexes) const;
    ...
    const QuaternionFloatingMobilizer<T>& GetFreeBodyMobilizerOrThrow(const RigidBody<T>& body) const;
    ...
    const VectorX<T>& EvalReflectedInertiaCache(const systems::Context<T>& context) const {
        DRAKE_ASSERT(tree_system_ != nullptr);
    ...
    const VectorX<T>& EvalJointDampingCache(const systems::Context<T>& context) const {
        DRAKE_ASSERT(tree_system_ != nullptr);
    ...
    const std::vector<SpatialForce<T>>& EvalArticulatedBodyForceBiasCache(const systems::Context<T>& context) const {
        DRAKE_ASSERT(tree_system_ != nullptr);
    ...
    void CalcJacobianAngularAndOrTranslationalVelocityInWorld(const systems::Context<T>& context,
                                                              JacobianWrtVariable with_respect_to,
                                                              const Frame<T>& frame_F,
    ...
    void CalcJacobianTranslationalVelocityHelper(const systems::Context<T>& context,
                                                 JacobianWrtVariable with_respect_to,
                                                 const Frame<T>& frame_B,
    ...
    void AddJointDampingForces(const systems::Context<T>& context, MultibodyForces<T>* forces) const;
    ...
    const FrameType<T>& get_frame_variant(const FrameType<Scalar>& frame) const {
        static_assert(std::is_base_of_v<Frame<T>, FrameType<T>>, "FrameType<T> must be a sub-class of Frame<T>.");
        // TODO(amcastro-tri):
        ...
        const FrameType<T>* frame_variant = dynamic_cast<const FrameType<T>*>(&my_frame);
    ...
    const BodyType<T>& get_body_variant(const BodyType<Scalar>& body) const {
        static_assert(std::is_base_of_v<RigidBody<T>, BodyType<T>>, "BodyType<T> must be a sub-class of RigidBody<T>.");
        // TODO(amcastro-tri):
        //   DRAKE_DEMAND the parent tree of the variant is indeed a variant of this
        //   MultibodyTree. That will require the tree to have some sort of id.
        const BodyIndex body_index = body.index();
        const RigidBody<T>& my_body = rigid_bodies_.get_element(body_index);
        const BodyType<T>* body_variant = dynamic_cast<const BodyType<T>*>(&my_body);
    ...
    const MobilizerType<T>& get_mobilizer_variant(const MobilizerType<Scalar>& mobilizer) const {
        ...
        const MobilizerType<T>* mobilizer_variant =
                dynamic_cast<const MobilizerType<T>*>(owned_mobilizers_[mobilizer_index].get());
    ...
    template <template <typename> class MobilizerType, typename Scalar>
    MobilizerType<T>& get_mutable_mobilizer_variant(const MobilizerType<Scalar>& mobilizer) {
        static_assert(std::is_base_of_v<Mobilizer<T>, MobilizerType<T>>,
                      "MobilizerType<T> must be a sub-class of Mobilizer<T>.");
        // TODO(amcastro-tri):
        //   DRAKE_DEMAND the parent tree of the variant is indeed a variant of this
        ...
        MobilizerIndex mobilizer_index = mobilizer.index();
        DRAKE_DEMAND(mobilizer_index < num_mobilizers());
        MobilizerType<T>* mobilizer_variant = dynamic_cast<MobilizerType<T>*>(owned_mobilizers_[mobilizer_index].get());
        DRAKE_DEMAND(mobilizer_variant != nullptr);
        return *mobilizer_variant;
    }
    ...
    const JointType<T>& get_joint_variant(const JointType<Scalar>& joint) const {
        ...
        const JointType<T>* joint_variant = dynamic_cast<const JointType<T>*>(&my_joint);
    ...
    void RegisterJointInGraph(const Joint<T>& joint) {
        ...
        multibody_graph_.AddJoint(joint.name(), joint.model_instance(), type_name, joint.parent_body().index(),
    ...
    std::optional<BodyIndex> MaybeGetUniqueBaseBodyIndex(ModelInstanceIndex model_instance) const;

    // Helper function for GetDefaultFreeBodyPose().
    std::pair<Eigen::Quaternion<double>, Vector3<double>> GetDefaultFreeBodyPoseAsQuaternionVec3Pair(
    ...
    ElementCollection<T, internal::ModelInstance, ModelInstanceIndex> model_instances_;
    ...
    std::unordered_map<BodyIndex, std::variant<JointIndex, std::pair<Eigen::Quaternion<double>, Vector3<double>>>>
            default_body_poses_;
