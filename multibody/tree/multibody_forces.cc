...
template <typename T>
MultibodyForces<T>::MultibodyForces(const internal::MultibodyTreeSystem<T>& plant)
...
template <typename T>
bool MultibodyForces<T>::CheckHasRightSizeForModel(const internal::MultibodyTreeSystem<T>& plant) const {
    return CheckHasRightSizeForModel(internal::GetInternalTree(plant));
}

template <typename T>
bool MultibodyForces<T>::CheckHasRightSizeForModel(const internal::MultibodyTree<T>& model) const {
    return model.num_velocities() == num_velocities() && model.num_bodies() == num_bodies();
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::MultibodyForces);
