...
public:
    ...
    const std::vector<SpatialAcceleration<T>>& get_A_WB_pool() const { return A_WB_pool_; }

    // Mutable version of get_A_WB_pool().
    std::vector<SpatialAcceleration<T>>& get_mutable_A_WB_pool() { return A_WB_pool_; }
...
private:
    ...
    void InitializeToNaN() {
        for (MobodIndex mobod_index(0); mobod_index < get_num_mobods(); ++mobod_index) {
