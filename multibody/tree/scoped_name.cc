...
ScopedName::ScopedName(std::string_view namespace_name, std::string_view element_name) {
    std::optional<ScopedName> result = Make(namespace_name, element_name);
    if (!result) {
        throw std::logic_error(fmt::format("Cannot create a ScopedName('{}', '{}') with mis-placed delimiters",
                                           namespace_name, element_name));
    }
    *this = std::move(*result);
}

std::optional<ScopedName> ScopedName::Make(std::string_view namespace_name, std::string_view element_name) {
    if (namespace_name.starts_with(kDelim) || namespace_name.ends_with(kDelim) || element_name.empty() ||
        HasScopeDelimiter(element_name)) {
        return std::nullopt;
    }
    ScopedName result;
