...
public:
    ...
    ScrewMobilizer(const Frame<T>& inboard_frame_F,
                   ...
                   double screw_pitch)
        : MobilizerBase(inboard_frame_F, outboard_frame_M), screw_pitch_(screw_pitch) {
    ...
    const ScrewMobilizer<T>& SetTranslation(systems::Context<T>* context, const T& translation) const;
    ...
    /* Sets in `context` the orientation for `this` mobilizer to the angle θ
     ...
     @returns a constant reference to `this` mobilizer. */
    const ScrewMobilizer<T>& SetAngle(systems::Context<T>* context, const T& angle) const;
    ...
    /* Sets in `context` the rate of change, in meters per second, of `this`
     ...
     @throws std::exception if the screw_pitch is very near zero and
             |vz| > kEpsilon. */
    const ScrewMobilizer<T>& SetTranslationRate(systems::Context<T>* context, const T& vz) const;
    ...
    /* Sets in `context` the rate of change, in radians per second, of `this`
     ...
     @returns A constant reference to `this` mobilizer. */
    const ScrewMobilizer<T>& SetAngularRate(systems::Context<T>* context, const T& theta_dot) const;
...
template <typename T>
inline T get_screw_translation_from_rotation(const T& theta, double screw_pitch) {
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::internal::ScrewMobilizer);
