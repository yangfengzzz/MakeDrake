...
public:
    ...
    PlanarJoint(const std::string& name,
        ...
        : Joint<T>(name,
                   ...
                   frame_on_child,
                   std::move(damping),
                   VectorX<double>::Constant(3, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(3, std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(3, -std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(3, std::numeric_limits<double>::infinity()),
                   VectorX<double>::Constant(3, -std::numeric_limits<double>::infinity()),
    ...
    Vector3<double> default_damping() const { return this->default_damping_vector(); }
    ...
    Vector2<T> get_translation(const Context<T>& context) const { return get_mobilizer()->get_translations(context); }
    ...
    Vector2<double> get_default_translation() const { return this->default_positions().head(2); }
    ...
    void set_default_translation(const Vector2<double>& p_FoMo_F) {
        Vector3<double> state(p_FoMo_F[0], p_FoMo_F[1], this->default_positions()[2]);
...
private:
    ...
    const internal::PlanarMobilizer<T>* get_mobilizer() const {
        ...
        const internal::PlanarMobilizer<T>* mobilizer =
                dynamic_cast<const internal::PlanarMobilizer<T>*>(this->get_implementation().mobilizer);
    ...
    internal::PlanarMobilizer<T>* get_mutable_mobilizer() {
        DRAKE_DEMAND(this->get_implementation().has_mobilizer());
        auto* mobilizer = dynamic_cast<internal::PlanarMobilizer<T>*>(this->get_implementation().mobilizer);
        DRAKE_DEMAND(mobilizer != nullptr);
        return mobilizer;
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::multibody::PlanarJoint);
