...
SpatialInertia<double> CalcMeshSpatialInertia(const MeshType& mesh, double density) {
    const auto& extension = mesh.extension();
    if (extension == ".obj") {
        return CalcSpatialInertia(geometry::ReadObjToTriangleSurfaceMesh(mesh.filename(), mesh.scale()), density);
    }
    if (extension == ".vtk") {
        return CalcSpatialInertia(
                geometry::ConvertVolumeToSurfaceMesh(geometry::internal::MakeVolumeMeshFromVtk<double>(mesh)), density);
    }
    throw std::runtime_error(
            fmt::format("CalcSpatialInertia currently only supports .obj or tetrahedral-mesh"
...
SpatialInertia<double> CalcSpatialInertia(const geometry::Shape& shape, double density) {
    return shape.Visit<SpatialInertia<double>>(overloaded{
            [density](const geometry::Box& box) {
                return SpatialInertia<double>::SolidBoxWithDensity(density, box.width(), box.depth(), box.height());
            },
            [density](const geometry::Capsule& capsule) {
                return SpatialInertia<double>::SolidCapsuleWithDensity(density, capsule.radius(), capsule.length(),
                                                                       Vector3d::UnitZ());
            },
            ...
            [density](const geometry::Cylinder& cylinder) {
                return SpatialInertia<double>::SolidCylinderWithDensity(density, cylinder.radius(), cylinder.length(),
                                                                        Vector3d::UnitZ());
            },
            [density](const geometry::Ellipsoid& ellipsoid) {
                return SpatialInertia<double>::SolidEllipsoidWithDensity(density, ellipsoid.a(), ellipsoid.b(),
                                                                         ellipsoid.c());
            },
            [](const geometry::HalfSpace&) -> SpatialInertia<double> {
                throw std::logic_error("CalcSpatialInertia: Cannot compute mass of a HalfSpace");
            },
            ...
            [](const geometry::MeshcatCone&) -> SpatialInertia<double> {
                throw std::logic_error("CalcSpatialInertia: MeshcatCone is not yet supported");
            },
            [density](const geometry::Sphere& sphere) {
                return SpatialInertia<double>::SolidSphereWithDensity(density, sphere.radius());
            }});
}
...
SpatialInertia<double> CalcSpatialInertia(const geometry::TriangleSurfaceMesh<double>& mesh, double density) {
  ...
  C_canonical << 1 / 60.0,  1 / 120.0, 1 / 120.0,
    ...
    const UnitInertia G_GGo_G(trace_C - C(0, 0), trace_C - C(1, 1), trace_C - C(2, 2), -C(1, 0), -C(2, 0), -C(2, 1));
