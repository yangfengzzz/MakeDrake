...
public:
    ...
    typename std::enable_if_t<std::is_same_v<T, typename Derived::Scalar>, VectorX<T>> ComposeVariableValues(
            const systems::Context<T>& context, const Derived& lambda_value) const {
        DRAKE_ASSERT(lambda_value.rows() == num_lambda_ && lambda_value.cols() == 1);
    ...
    typename std::enable_if_t<std::is_same_v<typename DerivedQ::Scalar, typename DerivedLambda::Scalar>,
                              VectorX<typename DerivedQ::Scalar>>
    ComposeVariableValues(const Eigen::MatrixBase<DerivedQ>& q_value,
                          const Eigen::MatrixBase<DerivedLambda>& lambda_value) const {
        DRAKE_ASSERT(q_value.rows() == plant_->num_positions() && q_value.cols() == 1);
        DRAKE_ASSERT(lambda_value.rows() == num_lambda_ && lambda_value.cols() == 1);
     ...
     */
    const SortedPair<geometry::GeometryId>& geometry_id_pair() const { return geometry_id_pair_; }
...
protected:
    ...
    ContactWrenchEvaluator(const MultibodyPlant<AutoDiffXd>* plant,
        ...
        : solvers::EvaluatorBase(6, plant->num_positions() + num_lambda, description),
          plant_(plant),
          context_(context),
 ...
 */
class ContactWrenchFromForceInWorldFrameEvaluator final : public ContactWrenchEvaluator {
public:
     ...
     */
    ContactWrenchFromForceInWorldFrameEvaluator(const MultibodyPlant<AutoDiffXd>* plant,
                                                systems::Context<AutoDiffXd>* context,
                                                const SortedPair<geometry::GeometryId>& geometry_id_pair)
        : ContactWrenchEvaluator(plant, context, 3, geometry_id_pair, "contact_wrench_from_pure_force_in_world_frame") {
    }
...
private:
    ...
    void DoEvalGeneric(const Eigen::Ref<const VectorX<T>>& x, VectorX<U>* y) const;
};
...
struct GeometryPairContactWrenchEvaluatorBinding {
    GeometryPairContactWrenchEvaluatorBinding(std::vector<int> lambda_indices_in_all_lambda_in,
                                              std::shared_ptr<ContactWrenchEvaluator> contact_wrench_evaluator_in)
        : lambda_indices_in_all_lambda{std::move(lambda_indices_in_all_lambda_in)},
          contact_wrench_evaluator{std::move(contact_wrench_evaluator_in)} {
        DRAKE_DEMAND(static_cast<int>(lambda_indices_in_all_lambda.size()) == contact_wrench_evaluator->num_lambda());
    }
