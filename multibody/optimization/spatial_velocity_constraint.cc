...
VectorXd MakeLowerBounds(const Eigen::Ref<const Eigen::Vector3d>& v_AC_lower,
                         const std::optional<SpatialVelocityConstraint::AngularVelocityBounds>& w_AC_bounds) {
    if (w_AC_bounds) {
        VectorXd lower(5);
...
VectorXd MakeUpperBounds(const Eigen::Ref<const Eigen::Vector3d>& v_AC_upper,
                         const std::optional<SpatialVelocityConstraint::AngularVelocityBounds>& w_AC_bounds) {
    if (w_AC_bounds) {
        VectorXd upper(5);
        upper.head<3>() = v_AC_upper;
        DRAKE_THROW_UNLESS(w_AC_bounds->magnitude_lower <= w_AC_bounds->magnitude_upper);
...
SpatialVelocityConstraint::SpatialVelocityConstraint(const MultibodyPlant<AutoDiffXd>* const plant,
...
void SpatialVelocityConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    AutoDiffVecXd y_t;
    Eval(x.cast<AutoDiffXd>(), &y_t);
    *y = math::ExtractValue(y_t);
}

void SpatialVelocityConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    ...
    const SpatialVelocity<AutoDiffXd> V_AB_A = frameB.CalcSpatialVelocity(*context_, frameA, frameA);
    // v_BC_A = 0, because C is fixed in B.
    // p_BC_A = R_AB * p_BC.
    const math::RotationMatrix<AutoDiffXd> R_AB = frameB.CalcRotationMatrix(*context_, frameA);
    ...
    if (w_AC_reference_direction_) {
        ...
        // cos(θ)
        (*y)[4] = w_AC.dot(*w_AC_reference_direction_) / sqrt(squared_norm + 100 * kEps * kEps);
    }
}
