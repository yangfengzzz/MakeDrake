#include "multibody/optimization/sliding_friction_complementarity_constraint.h"

#include <cmath>
#include <limits>
#include <memory>

#include "multibody/inverse_kinematics/kinematic_evaluator_utilities.h"
...
namespace internal {
namespace {
Eigen::Matrix<double, 11, 1> GetConstraintLowerBound(double complementarity_tolerance) {
    Eigen::Matrix<double, 11, 1> lower_bound = Eigen::Matrix<double, 11, 1>::Zero();
...
Eigen::Matrix<double, 11, 1> GetConstraintUpperBound(double complementarity_tolerance) {
    Eigen::Matrix<double, 11, 1> upper_bound = Eigen::Matrix<double, 11, 1>::Zero();
...
SlidingFrictionComplementarityNonlinearConstraint::SlidingFrictionComplementarityNonlinearConstraint(
        const ContactWrenchEvaluator* const contact_wrench_evaluator, double complementarity_tolerance)
    : solvers::Constraint(11,
...
std::vector<std::pair<int, int>> SlidingFrictionComplementarityNonlinearConstraint::GetConstraintSparsityPattern()
        const {
    ...
    gradient_sparsity_pattern.reserve(11 * plant.num_positions() + 6 * plant.num_velocities() +
                                      3 * contact_wrench_evaluator_->num_lambda() + 24);
    Eigen::VectorXi x_indices = Eigen::VectorXi::LinSpaced(num_vars(), 0, num_vars() - 1);

    Eigen::VectorXi q_indices, v_indices, lambda_indices;
    Eigen::Vector3i f_static_indices, f_sliding_indices;
    int c_index;
    DecomposeX<int>(x_indices, &q_indices, &v_indices, &lambda_indices, &f_static_indices, &f_sliding_indices,
                    &c_index);
    ...
    auto set_gradient_dependent_variables = [&gradient_sparsity_pattern](
                                                    int constraint_row_index,
                                                    const Eigen::Ref<const Eigen::VectorXi>& var_indices) {
        for (int i = 0; i < var_indices.rows(); ++i) {
            gradient_sparsity_pattern.emplace_back(constraint_row_index, var_indices(i));
        }
    };
...
void SlidingFrictionComplementarityNonlinearConstraint::UpdateComplementarityTolerance(
...
void SlidingFrictionComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x,
...
void SlidingFrictionComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x,
    ...
    systems::Context<AutoDiffXd>& context =
            const_cast<systems::Context<AutoDiffXd>&>(contact_wrench_evaluator_->context());
    ...
    contact_wrench_evaluator_->Eval(contact_wrench_evaluator_->ComposeVariableValues(context, lambda), &F_AB_W);
    ...
    const auto& query_object = query_port.Eval<geometry::QueryObject<AutoDiffXd>>(context);
    ...
    for (const auto& signed_distance_pair : signed_distance_pairs) {
        if (signed_distance_pair.id_A == contact_wrench_evaluator_->geometry_id_pair().first() &&
            signed_distance_pair.id_B == contact_wrench_evaluator_->geometry_id_pair().second()) {
            ...
            const CoulombFriction<double>& geometryA_friction =
                    geometryA_props.GetProperty<CoulombFriction<double>>("material", "coulomb_friction");
            const CoulombFriction<double>& geometryB_friction =
                    geometryB_props.GetProperty<CoulombFriction<double>>("material", "coulomb_friction");
            ...
            const Frame<AutoDiffXd>& frameA = plant.GetBodyFromFrameId(frame_A_id)->body_frame();
            const Frame<AutoDiffXd>& frameB = plant.GetBodyFromFrameId(frame_B_id)->body_frame();

            const SpatialVelocity<AutoDiffXd> V_AB_W = frameB.CalcSpatialVelocity(context, frameA, plant.world_frame());
            ...
            const Vector3<AutoDiffXd> p_BbCb_W =
                    plant.CalcRelativeTransform(context, plant.world_frame(), frameB).rotation() * p_BbCb;

            // v_ACb_W is the sliding velocity of witness point Cb measured from the
            // geometry A, expressed in the world frame.
            const Vector3<AutoDiffXd> v_ACb_W = V_AB_W.Shift(p_BbCb_W).translational();
            ...
            const Vector3<AutoDiffXd> v_sliding_ACb_W =
                    (Eigen::Matrix3d::Identity() - nhat_BA_W * nhat_BA_W.transpose()) * v_ACb_W;
            ...
            (*y)(7) = f_sliding.dot((Eigen::Matrix3d::Identity() - (pow(combined_friction.dynamic_friction(), 2) + 1) *
            ...
            const Vector3<AutoDiffXd> f_sliding_tangential =
                    (Eigen::Matrix3d::Identity() - nhat_BA_W * nhat_BA_W.transpose()) * f_sliding;
...
void SlidingFrictionComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
...
AddSlidingFrictionComplementarityConstraint(const ContactWrenchEvaluator* contact_wrench_evaluator,
                                            double complementarity_tolerance,
    ...
    auto constraint = std::make_shared<internal::SlidingFrictionComplementarityNonlinearConstraint>(
    ...
    const auto c_var = prog->NewContinuousVariables<1>("sliding_friction_complementarity_slack_c")(0);
    prog->AddBoundingBoxConstraint(0, kInf, c_var);
    VectorX<symbolic::Variable> bound_vars(constraint->num_vars());
    constraint->ComposeX<symbolic::Variable>(q_vars, v_vars, lambda_vars, f_static, f_sliding, c_var, &bound_vars);
    prog->AddConstraint(constraint, bound_vars);
    return solvers::Binding<internal::SlidingFrictionComplementarityNonlinearConstraint>(constraint, bound_vars);
}
}  // namespace internal
...
AddSlidingFrictionComplementarityExplicitContactConstraint(
        ...
        const Eigen::Ref<const VectorX<symbolic::Variable>>& lambda_vars,
        solvers::MathematicalProgram* prog) {
    const auto sliding_friction_complementarity_constraint = internal::AddSlidingFrictionComplementarityConstraint(
            contact_wrench_evaluator, complementarity_tolerance, q_vars, v_vars, lambda_vars, prog);
    solvers::Binding<StaticFrictionConeConstraint> static_friction_cone_constraint(
            std::make_shared<StaticFrictionConeConstraint>(contact_wrench_evaluator), {q_vars, lambda_vars});
    prog->AddConstraint(static_friction_cone_constraint);
    return std::make_pair(sliding_friction_complementarity_constraint, static_friction_cone_constraint);
}
...
AddSlidingFrictionComplementarityImplicitContactConstraint(
    ...
    const auto sliding_friction_complementarity_constraint = internal::AddSlidingFrictionComplementarityConstraint(
            contact_wrench_evaluator, complementarity_tolerance, q_vars, v_vars, lambda_vars, prog);
    const auto static_friction_complementarity_constraint = AddStaticFrictionConeComplementarityConstraint(
            contact_wrench_evaluator, complementarity_tolerance, q_vars, lambda_vars, prog);
    return std::make_pair(sliding_friction_complementarity_constraint, static_friction_complementarity_constraint);
}
}  // namespace multibody
}  // namespace drake
