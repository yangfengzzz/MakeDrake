...
Vector1<T> EvalQuaternionIntegration(const Eigen::Quaternion<T>& quat1,
    ...
    if (scalar_predicate<T>::is_bool && ExtractDoubleOrThrow(angular_vel_norm) == 0) {
        delta_z_vec = angular_vel * h / 2;
    } else {
        delta_z_vec = angular_vel * sin(angular_vel_norm * h / 2) / angular_vel_norm;
    }
    const T delta_z_w = cos(angular_vel_norm * h / 2);
    const Eigen::Quaternion<T> delta_z(delta_z_w, delta_z_vec(0), delta_z_vec(1), delta_z_vec(2));
...
template <typename T>
void EulerIntegrationEvalGeneric(bool allow_quaternion_negation, const Eigen::Ref<const VectorX<T>>& x, VectorX<T>* y) {
    ...
    *y = EvalQuaternionIntegration<T>(quat1, quat2, angular_vel, x(11), allow_quaternion_negation);
}
}  // namespace

QuaternionEulerIntegrationConstraint::QuaternionEulerIntegrationConstraint(bool allow_quaternion_negation)
...
void QuaternionEulerIntegrationConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x,
                                                  Eigen::VectorXd* y) const {
    EulerIntegrationEvalGeneric<double>(allow_quaternion_negation_, x, y);
}

void QuaternionEulerIntegrationConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    EulerIntegrationEvalGeneric<AutoDiffXd>(allow_quaternion_negation_, x, y);
}

void QuaternionEulerIntegrationConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>& x,
                                                  VectorX<symbolic::Expression>* y) const {
    EulerIntegrationEvalGeneric<symbolic::Expression>(allow_quaternion_negation_, x.cast<symbolic::Expression>(), y);
}
}  // namespace multibody
}  // namespace drake
