...
namespace multibody {
namespace {
int GetLambdaSize(const std::map<SortedPair<geometry::GeometryId>, GeometryPairContactWrenchEvaluatorBinding>&
...
ManipulatorEquationConstraint::ManipulatorEquationConstraint(
        ...
        const std::map<SortedPair<geometry::GeometryId>, GeometryPairContactWrenchEvaluatorBinding>&
                contact_pair_to_wrench_evaluator)
    : solvers::Constraint(plant->num_velocities(),
                          plant->num_velocities() + plant->num_positions() + plant->num_velocities() +
                                  plant->num_actuated_dofs() + GetLambdaSize(contact_pair_to_wrench_evaluator) +
                                  1 /* for dt */,
...
void ManipulatorEquationConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
...
void ManipulatorEquationConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    ...
    const auto& qv_next = x.segment(num_velocities, num_positions + num_velocities);
    const auto& v_next = x.segment(num_velocities + num_positions, num_velocities);
    const auto& u_next = x.segment(num_velocities + num_positions + num_velocities, plant_->num_actuated_dofs());
    ...
    *y += plant_->CalcGravityGeneralizedForces(*context_);  // g(q[n+1])
    ...
    Eigen::Matrix<AutoDiffXd, Eigen::Dynamic, 1> C_bias(plant_->num_velocities(), 1);
    ...
    *y -= C_bias;
    ...
    const auto& query_object = query_port.Eval<geometry::QueryObject<AutoDiffXd>>(*context_);
    ...
    const int lambda_start_index_in_x = num_velocities + num_positions + num_velocities + plant_->num_actuated_dofs();
    for (const auto& signed_distance_pair : signed_distance_pairs) {
        ...
        const Frame<AutoDiffXd>& frameA = plant_->GetBodyFromFrameId(frame_A_id)->body_frame();
        const Frame<AutoDiffXd>& frameB = plant_->GetBodyFromFrameId(frame_B_id)->body_frame();
        ...
        Eigen::Matrix<AutoDiffXd, 6, Eigen::Dynamic> Jv_V_WCa(6, plant_->num_velocities());
        Eigen::Matrix<AutoDiffXd, 6, Eigen::Dynamic> Jv_V_WCb(6, plant_->num_velocities());
        plant_->CalcJacobianSpatialVelocity(*context_, JacobianWrtVariable::kV, frameA, p_ACa, plant_->world_frame(),
                                            plant_->world_frame(), &Jv_V_WCa);
        plant_->CalcJacobianSpatialVelocity(*context_, JacobianWrtVariable::kV, frameB, p_BCb, plant_->world_frame(),
                                            plant_->world_frame(), &Jv_V_WCb);

        const SortedPair<geometry::GeometryId> contact_pair(signed_distance_pair.id_A, signed_distance_pair.id_B);
        ...
        VectorX<AutoDiffXd> lambda(it->second.contact_wrench_evaluator->num_lambda());

        for (int i = 0; i < lambda.rows(); ++i) {
            lambda(i) = x(lambda_start_index_in_x + it->second.lambda_indices_in_all_lambda[i]);
        }

        AutoDiffVecXd F_AB_W;
        it->second.contact_wrench_evaluator->Eval(
                it->second.contact_wrench_evaluator->ComposeVariableValues(*context_, lambda), &F_AB_W);
    ...
    Eigen::Matrix<AutoDiffXd, Eigen::Dynamic, Eigen::Dynamic> M_mass(plant_->num_velocities(),
                                                                     plant_->num_velocities());
    plant_->CalcMassMatrixViaInverseDynamics(*context_, &M_mass);

    *y = *y * time_step - M_mass * (v_next - v);
}
void ManipulatorEquationConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
...
// TODO(rcory) Combine duplicate code between ManipulatorEquationConstraint and
//  StaticEquilibriumConstraint.
solvers::Binding<ManipulatorEquationConstraint> ManipulatorEquationConstraint::MakeBinding(
        const MultibodyPlant<AutoDiffXd>* plant,
        systems::Context<AutoDiffXd>* context,
        const std::vector<std::pair<std::shared_ptr<ContactWrenchEvaluator>, VectorX<symbolic::Variable>>>&
    ...
    std::map<SortedPair<geometry::GeometryId>, GeometryPairContactWrenchEvaluatorBinding>
            contact_pair_to_wrench_evaluator;
    ...
    int lambda_count = 0;
    for (const auto& contact_wrench_evaluator_and_lambda : contact_wrench_evaluators_and_lambda) {
        const auto& contact_wrench_evaluator = contact_wrench_evaluator_and_lambda.first;
        ...
        for (int i = 0; i < contact_wrench_evaluator_and_lambda.second.rows(); ++i) {
        ...
        contact_pair_to_wrench_evaluator.emplace(
                ...
                GeometryPairContactWrenchEvaluatorBinding{lambda_indices_in_all_lambda, contact_wrench_evaluator});
    }
    ...
    for (const auto& contact_wrench_evaluator_and_lambda : contact_wrench_evaluators_and_lambda) {
        ...
        for (int j = 0; j < lambda_i.rows(); ++j) {
            all_lambda_next(map_lambda_id_to_index.at(lambda_i[j].get_id())) = lambda_i(j);
        }
    }
    ...
    VectorX<symbolic::Variable> bound_x(plant->num_velocities() + plant->num_positions() + plant->num_velocities() +
                                        plant->num_actuated_dofs() + num_lambda + 1);
    bound_x << v_vars, q_next_vars, v_next_vars, u_next_vars, all_lambda_next, dt_var;
    auto manipulator_equation_constraint =
            ...
            std::shared_ptr<ManipulatorEquationConstraint>(
                    new ManipulatorEquationConstraint(plant, context, contact_pair_to_wrench_evaluator));
    return solvers::Binding<ManipulatorEquationConstraint>(manipulator_equation_constraint, bound_x);
}
}  // namespace multibody
}  // namespace drake
