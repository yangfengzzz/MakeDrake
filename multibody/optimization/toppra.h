...
public:
     ...
     *         respectively.
     */
    std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> AddJointAccelerationLimit(
     ...
     */
    std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> AddJointTorqueLimit(
            const Eigen::Ref<const Eigen::VectorXd>& lower_limit,
            const Eigen::Ref<const Eigen::VectorXd>& upper_limit,
            ToppraDiscretization discretization = ToppraDiscretization::kInterpolation);
     ...
     * @pydrake_mkdoc_identifier{const}
     */
    std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> AddFrameAccelerationLimit(
...
private:
     ...
     */
    std::optional<Eigen::Matrix2Xd> ComputeBackwardPass(double s_dot_0, double s_dot_N, const SolverInterface& solver);
     ...
     */
    void CalcInterpolationConstraint(Eigen::MatrixXd* A, Eigen::MatrixXd* lower_bound, Eigen::MatrixXd* upper_bound);
    ...
    struct ToppraBoundingBoxConstraint {
        ToppraBoundingBoxConstraint(const Eigen::VectorXd& lower_limit, const Eigen::VectorXd& upper_limit)
    ...
    // x_bounds_.
    std::unordered_map<Binding<BoundingBoxConstraint>, ToppraBoundingBoxConstraint> x_bounds_;
    ...
    std::unordered_map<Binding<LinearConstraint>, ToppraLinearConstraint> backward_lin_constraint_;
    ...
    std::unordered_map<Binding<LinearConstraint>, ToppraLinearConstraint> forward_lin_constraint_;
};
}  // namespace multibody
}  // namespace drake
