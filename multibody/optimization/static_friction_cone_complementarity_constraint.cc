...
StaticFrictionConeComplementarityNonlinearConstraint::StaticFrictionConeComplementarityNonlinearConstraint(
        const ContactWrenchEvaluator* contact_wrench_evaluator, double complementarity_tolerance)
    : solvers::Constraint(
              4,
              contact_wrench_evaluator->plant().num_positions() + contact_wrench_evaluator->num_lambda() + 2,
              Eigen::Vector4d::Zero(),
...
void StaticFrictionConeComplementarityNonlinearConstraint::UpdateComplementarityTolerance(
...
StaticFrictionConeComplementarityNonlinearConstraint::MakeBinding(
    ...
    auto constraint = std::make_shared<StaticFrictionConeComplementarityNonlinearConstraint>(contact_wrench_evaluator,
                                                                                             complementarity_tolerance);
    VectorX<symbolic::Variable> bound_vars(constraint->num_vars());
    bound_vars << q_vars, lambda_vars, constraint->alpha_var(), constraint->beta_var();
    return solvers::Binding<StaticFrictionConeComplementarityNonlinearConstraint>(constraint, bound_vars);
}

void StaticFrictionConeComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x,
...
void StaticFrictionConeComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x,
    ...
    const geometry::SignedDistancePair<AutoDiffXd> signed_distance_pair =
            query_object.ComputeSignedDistancePairClosestPoints(contact_wrench_evaluator_->geometry_id_pair().first(),
                                                                contact_wrench_evaluator_->geometry_id_pair().second());
    // Compute the friction.
    const geometry::ProximityProperties& geometryA_props =
            *query_object.inspector().GetProximityProperties(signed_distance_pair.id_A);
    const geometry::ProximityProperties& geometryB_props =
            *query_object.inspector().GetProximityProperties(signed_distance_pair.id_B);
    ...
    (*y)(0) = f_AB_W.dot(((std::pow(combined_friction.static_friction(), 2) + 1) * nhat_BA_W * nhat_BA_W.transpose() -
...
void StaticFrictionConeComplementarityNonlinearConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
...
AddStaticFrictionConeComplementarityConstraint(const ContactWrenchEvaluator* contact_wrench_evaluator,
    ...
    auto nonlinear_binding = internal::StaticFrictionConeComplementarityNonlinearConstraint::MakeBinding(
            contact_wrench_evaluator, complementarity_tolerance, q_vars, lambda_vars);
    // Add the variable α,β to the program.
    const Vector2<symbolic::Variable> alpha_beta_var(nonlinear_binding.evaluator()->alpha_var(),
    ...
    prog->AddBoundingBoxConstraint(Eigen::Vector2d::Zero(), Eigen::Vector2d::Constant(kInf), alpha_beta_var);
