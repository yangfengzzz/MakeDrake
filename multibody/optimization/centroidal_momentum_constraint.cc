...
CentroidalMomentumConstraint::CentroidalMomentumConstraint(
    ...
    : solvers::Constraint(momentum_dim(angular_only),
                          RefFromPtrOrThrow(plant).num_positions() + RefFromPtrOrThrow(plant).num_velocities() +
...
void DoEvalGeneric(const MultibodyPlant<T>& plant,
    ...
    UpdateContextPositionsAndVelocities(context, plant, x.head(plant.num_positions() + plant.num_velocities()));
    Vector3<T> p_WC;
    ...
    if (model_instances.has_value()) {
        ...
        h_WC_eval = plant.CalcSpatialMomentumInWorldAboutPoint(*context, model_instances.value(), p_WC);
    } else {
...
void CentroidalMomentumConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    if (use_autodiff()) {
        AutoDiffVecXd y_t;
...
void CentroidalMomentumConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    if (use_autodiff()) {
        DoEvalGeneric(*plant_autodiff_, context_autodiff_, model_instances_, angular_only_, x, y);
    } else {
        // Implement this part once we support computing the Jacobian of centroidal
        // momentum with MBP<double>
        throw std::runtime_error("Not implemented yet");
