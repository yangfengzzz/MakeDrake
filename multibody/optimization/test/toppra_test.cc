...
public:
    ...
    IiwaToppraTest() : iiwa_plant_(std::make_unique<MultibodyPlant<double>>(0)) {
        ...
        iiwa_plant_->WeldFrames(iiwa_plant_->world_frame(), iiwa_plant_->GetFrameByName("iiwa_link_0"));
        ...
        path_ = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(
                Eigen::VectorXd::LinSpaced(2, 0, 1), knots, Eigen::VectorXd::Zero(7), Eigen::VectorXd::Zero(7));
    ...
    SpatialVelocity<double> CalcSpatialVelocity(double t,
    ...
    Vector6d CalcSpatialAcceleration(double t, const PiecewisePolynomial<double>& s_path, const Frame<double>& frame) {
        const auto s_dot = s_path.EvalDerivative(t, 1);
        ...
        iiwa_plant_->CalcJacobianSpatialVelocity(*plant_context_, JacobianWrtVariable::kQDot, frame,
                                                 Eigen::Vector3d::Zero(), iiwa_plant_->world_frame(),
                                                 iiwa_plant_->world_frame(), &J);
        Eigen::VectorXd J_dot_v(6);
        J_dot_v = iiwa_plant_
                          ->CalcBiasSpatialAcceleration(*plant_context_, JacobianWrtVariable::kV, frame,
                                                        Eigen::Vector3d::Zero(), iiwa_plant_->world_frame(),
...
TEST_F(IiwaToppraTest, JointVelocityLimit) {
    ...
    auto velocity_constraint = toppra_->AddJointVelocityLimit(lower_bound, upper_bound);
    toppra_->AddJointAccelerationLimit(Eigen::VectorXd::Constant(7, -10), Eigen::VectorXd::Constant(7, 10));
...
TEST_F(IiwaToppraTest, JointAccelerationLimit) {
    ...
    auto acceleration_constraint = toppra_->AddJointAccelerationLimit(lower_bound, upper_bound);
...
TEST_F(IiwaToppraTest, JointTorqueLimit) {
    ...
    auto torque_constraint = toppra_->AddJointTorqueLimit(lower_bound, upper_bound);
...
TEST_F(IiwaToppraTest, FrameVelocityLimit) {
    ...
    for (int ii = 0; ii < s_path.get_number_of_segments(); ii++) {
        ...
        iiwa_plant_->SetPositions(plant_context_.get(), position);
        iiwa_plant_->SetVelocities(plant_context_.get(), velocity);
        frame_velocity = frame.CalcSpatialVelocityInWorld(*plant_context_).get_coeffs();
...
TEST_F(IiwaToppraTest, FrameTranslationalSpeedLimit) {
    ...
    for (bool use_trajectory : {true, false}) {
        SCOPED_TRACE(fmt::format("Speed limit with trajectory: {}", use_trajectory));
        if (use_trajectory) {
            ...
            const Eigen::MatrixXd upper_bound_mat = Vector1<double>::Constant(upper_bound);
            const std::vector samples{upper_bound_mat, upper_bound_mat};
            const auto upper_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
        ...
        const double tol = 1e-14;
        for (int ii = 0; ii < s_path.get_number_of_segments(); ii++) {
            const auto frame_velocity = CalcSpatialVelocity(s_path.start_time(ii), s_path, frame);
            const double speed = frame_velocity.translational().norm();

            EXPECT_LT(speed, upper_bound + tol);
        }

        const auto frame_velocity = CalcSpatialVelocity(s_path.end_time(), s_path, frame);
        const double speed = frame_velocity.translational().norm();
...
TEST_F(IiwaToppraTest, FrameTranslationalSpeedLimitTrajectoryOverlap) {
    ...
    // Limit domain lies "above" path domain.
    {
        ...
        const auto upper_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
        DRAKE_EXPECT_THROWS_MESSAGE(toppra_->AddFrameTranslationalSpeedLimit(frame, upper_bound_traj),
    ...
    {
        const std::vector<double> breaks{path_start - 1.1, (path_start + path_end) / 2};
        const auto upper_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
        EXPECT_NO_THROW(toppra_->AddFrameTranslationalSpeedLimit(frame, upper_bound_traj));
    }

    // Overlap at the upper range of the path domain is valid.
    {
        const std::vector<double> breaks{(path_start + path_end) / 2, path_end + 1.1};
        const auto upper_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, samples);
        EXPECT_NO_THROW(toppra_->AddFrameTranslationalSpeedLimit(frame, upper_bound_traj));
    }
}
...
TEST_F(IiwaToppraTest, FrameTranslationalSpeedLimitVarying) {
    ...
    const auto bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(Eigen::VectorXd::LinSpaced(3, s0, s2), samples);
    ...
    auto calc_speed_and_limit = [&s_path, &frame, &bound_traj, this](double t) -> std::pair<double, double> {
        const double s = s_path.scalarValue(t);
        const SpatialVelocity<double> frame_velocity = CalcSpatialVelocity(t, s_path, frame);
        const double speed = frame_velocity.translational().norm();
...
TEST_F(IiwaToppraTest, FrameAccelerationLimit) {
    ...
    for (bool use_trajectory : {true, false}) {
        SCOPED_TRACE(fmt::format("Frame acceleration limit with trajectory: {}", use_trajectory));
        if (use_trajectory) {
            const std::vector<double> breaks{path_.start_time(), path_.end_time()};
            const std::vector<MatrixX<double>> lower_samples{lower_bound, lower_bound};
            const auto lower_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, lower_samples);

            const std::vector<MatrixX<double>> upper_samples{upper_bound, upper_bound};
            const auto upper_bound_traj = PiecewisePolynomial<double>::ZeroOrderHold(breaks, upper_samples);
        ...
        const double tol = 1e-14;
        for (int ii = 0; ii < s_path.get_number_of_segments(); ii++) {
            Vector6d frame_acceleration = CalcSpatialAcceleration(s_path.start_time(ii), s_path, frame);
        ...
        Vector6d frame_acceleration = CalcSpatialAcceleration(s_path.end_time(), s_path, frame);
...
TEST_F(IiwaToppraTest, FrameAccelerationLimitTrajectoryOverlap) {
    ...
    const auto lower_bigger = PiecewisePolynomial<double>::ZeroOrderHold(big_breaks, lower_samples);
    const auto upper_bigger = PiecewisePolynomial<double>::ZeroOrderHold(big_breaks, upper_samples);

    {
        // Limit domain lies "above" path domain.
        const std::vector<double> bad_breaks{path_end + 0.1, path_end + 1.1};
        const auto lower_bad = PiecewisePolynomial<double>::ZeroOrderHold(bad_breaks, lower_samples);
        const auto upper_bad = PiecewisePolynomial<double>::ZeroOrderHold(bad_breaks, upper_samples);
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(
                ...
                fmt::format(".* can't add a trajectory frame acceleration .* lower "
                            ...
                            "\\[{}, {}\\] .* path domain \\[{}, {}\\].",
                            bad_breaks[0], bad_breaks[1], big_breaks[0], big_breaks[1], path_start, path_end));

        // Just for upper.
        DRAKE_EXPECT_THROWS_MESSAGE(toppra_->AddFrameAccelerationLimit(frame, lower_bigger, upper_bad),
                                    ".* can't add a trajectory frame acceleration .*");

        // Both.
        DRAKE_EXPECT_THROWS_MESSAGE(toppra_->AddFrameAccelerationLimit(frame, lower_bad, upper_bad),
                                    ".* can't add a trajectory frame acceleration .*");
    }

    {
        ...
        // Just for lower.
        DRAKE_EXPECT_THROWS_MESSAGE(toppra_->AddFrameAccelerationLimit(frame, lower_bad, upper_bigger),
                                    ".* can't add a trajectory frame acceleration .*");
...

    {
        ...
        const std::vector<double> low_breaks{path_start - 1, (path_start + path_end) / 2};
        const std::vector<double> high_breaks{(path_start + path_end) / 2, path_end + 1};
        const auto lower_traj = PiecewisePolynomial<double>::ZeroOrderHold(high_breaks, lower_samples);
        const auto upper_traj = PiecewisePolynomial<double>::ZeroOrderHold(low_breaks, upper_samples);
        EXPECT_NO_THROW(toppra_->AddFrameAccelerationLimit(frame, lower_traj, upper_traj));
    }
}
...
TEST_F(IiwaToppraTest, FrameAccelerationLimitVarying) {
    ...
    const auto lower_traj =
            PiecewisePolynomial<double>::ZeroOrderHold(Eigen::VectorXd::LinSpaced(3, s0, s2), lower_samples);
    Eigen::MatrixXd upper_samples(6, 3);
    upper_samples << upper0, upper1, upper1;
    const auto upper_traj =
            PiecewisePolynomial<double>::ZeroOrderHold(Eigen::VectorXd::LinSpaced(3, s0, s2), upper_samples);
    ...
    auto calc_accel_lower_and_upper_limit = [&s_path, &frame, &lower_traj, &upper_traj,
    ...
    for (int ii = 0; ii < s_path.get_number_of_segments(); ii++) {
        ...
        if (s_path.scalarValue(t) >= s1) {
            second_half_increased_accleration =
                    second_half_increased_accleration || (accel.array().abs() > upper1.array()).any();
        }
    }
    const double t = s_path.end_time();
    auto [accel, lower, upper] = calc_accel_lower_and_upper_limit(t);
...
GTEST_TEST(ToppraTest, GridpointsTest) {
    ...
    auto path = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(
            Eigen::VectorXd::LinSpaced(2, 0, 1), knots, Eigen::VectorXd::Zero(3), Eigen::VectorXd::Zero(3));
...
GTEST_TEST(ToppraTest, TimeOptimalTest) {
    ...
    plant->AddJoint<PrismaticJoint>("joint", plant->world_body(), std::nullopt, body, std::nullopt,
                                    Eigen::Vector3d::UnitX());
    plant->Finalize();

    auto path = PiecewisePolynomial<double>::CubicWithContinuousSecondDerivatives(
            Eigen::Vector2d(0, 1), Eigen::RowVector2d(0, 1), Vector1d(0), Vector1d(0));
    ...
    auto acceleration_constraint = toppra->AddJointAccelerationLimit(Vector1d(-1), Vector1d(1));
...
GTEST_TEST(ToppraTest, ZeroVelocityTest) {
    ...
    auto path = PiecewisePolynomial<double>::FirstOrderHold(Eigen::Vector3d(0, 0.9, 2), Eigen::RowVector3d(0, 0, 1));
    ...
    auto velocity_constraint = toppra->AddJointVelocityLimit(Vector1d(-1), Vector1d(1));
    auto acceleration_constraint = toppra->AddJointAccelerationLimit(Vector1d(-1), Vector1d(1));
