...
template <>
void AddDrakeVisualizer<double>(systems::DiagramBuilder<double>* builder,
...
template <typename T>
FreeSpheresAndBoxes<T>::FreeSpheresAndBoxes(std::vector<SphereSpecification> spheres,
                                            std::vector<BoxSpecification> boxes,
                                            CoulombFriction<double> ground_friction)
    : spheres_{std::move(spheres)}, boxes_{std::move(boxes)}, ground_friction_{std::move(ground_friction)} {
    ...
    for (int i = 0; i < num_spheres; ++i) {
        const auto& sphere = plant_->AddRigidBody("sphere" + std::to_string(i), spheres_[i].inertia);
        sphere_body_indices_.push_back(sphere.index());
        sphere_geometry_ids_.push_back(plant_->RegisterCollisionGeometry(
                sphere, math::RigidTransformd::Identity(), geometry::Sphere(spheres_[i].radius),
                "sphere" + std::to_string(i), spheres_[i].friction));
        plant_->RegisterVisualGeometry(sphere, math::RigidTransformd::Identity(), geometry::Sphere(spheres_[i].radius),
                                       "sphere" + std::to_string(i), geometry::IllustrationProperties());
    }
    ...
    for (int i = 0; i < num_boxes; ++i) {
        ...
        if (boxes_[i].X_WB.has_value()) {
            ...
            box_body = &(plant_->AddRigidBody("box" + std::to_string(i), boxes_[i].inertia));
        ...
        box_geometry_ids_.push_back(plant_->RegisterCollisionGeometry(
                *box_body, X_BBox, geometry::Box(boxes_[i].size(0), boxes_[i].size(1), boxes_[i].size(2)),
                "box" + std::to_string(i), boxes_[i].friction));
        plant_->RegisterVisualGeometry(*box_body, X_BBox,
    ...
    ground_geometry_id_ = plant_->RegisterCollisionGeometry(
            plant_->world_body(), X_WG, geometry::Box(ground_box_size(0), ground_box_size(1), ground_box_size(2)),
