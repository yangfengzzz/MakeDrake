#include "multibody/optimization/static_equilibrium_constraint.h"
...
public:
    ...
    void CheckStaticEquilibriumConstraintEval(
            const solvers::Binding<StaticEquilibriumConstraint>& static_equilibrium_binding,
        ...
        // B * u.
        spheres_->plant().SetPositions(spheres_->get_mutable_plant_context(), q_autodiff);
        ...
        std::function<void(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd*)> eval_fun =
                [&static_equilibrium_binding](const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) {
...
TEST_F(TwoFreeSpheresTest, Construction) {
    ...
    const auto& plant = spheres_->plant();
    const auto static_equilibrium_binding = StaticEquilibriumConstraint::MakeBinding(
            &plant, spheres_->get_mutable_plant_context(), contact_wrench_evaluators_and_lambda_, q_vars_, u_vars_);
    // Test the size of StaticEquilibriumConstraint
    EXPECT_EQ(static_equilibrium_binding.evaluator()->num_vars(), 23 /* 14 for position, 9 for lambda */);
    EXPECT_EQ(static_equilibrium_binding.evaluator()->num_constraints(), plant.num_velocities());
    EXPECT_TRUE(CompareMatrices(static_equilibrium_binding.evaluator()->lower_bound(), Eigen::VectorXd::Zero(12)));
    EXPECT_TRUE(CompareMatrices(static_equilibrium_binding.evaluator()->upper_bound(), Eigen::VectorXd::Zero(12)));
    ...
    for (const auto& contact_wrench_evaluator_and_lambda : contact_wrench_evaluators_and_lambda_) {
        const auto& lambda_indices = static_equilibrium_binding.evaluator()
                                             ->contact_pair_to_wrench_evaluator()
        ...
        for (int i = 0; i < static_cast<int>(lambda_indices.size()); ++i) {
            EXPECT_EQ(static_equilibrium_binding.variables()(q_vars_.rows() + u_vars_.rows() + lambda_indices[i]),
...
TEST_F(TwoFreeSpheresTest, Eval) {
    ...
    CheckStaticEquilibriumConstraintEval(static_equilibrium_binding, x_autodiff.head<14>(), x_autodiff.tail<9>());
    // Test lambda != 0
    ...
    // equilibrium.
    spheres_double_->plant().SetPositions(spheres_double_->get_mutable_plant_context(), result.GetSolution(q_vars_));
    ...
    EXPECT_TRUE(CompareMatrices(sphere0_total_wrench, Vector6<double>::Zero(), tol));
    EXPECT_TRUE(CompareMatrices(sphere1_total_wrench, Vector6<double>::Zero(), tol));
}
}  // namespace
}  // namespace multibody
}  // namespace drake
