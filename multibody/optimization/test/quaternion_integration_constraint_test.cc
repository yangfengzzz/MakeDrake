#include "multibody/optimization/quaternion_integration_constraint.h"
...
Vector1<T> EvalQuaternionIntegration(const Eigen::Quaternion<T>& quat1,
  ...
  A << 0, -angular_vel(0), -angular_vel(1), -angular_vel(2),
    ...
    if (ExtractDoubleOrThrow(angular_vel_norm) == 0) {
        exp_half_Ah = cos(angular_vel_norm * h / 2) * Eigen::Matrix4d::Identity() + h / 2 * A;
    } else {
        exp_half_Ah = cos(angular_vel_norm * h / 2) * Eigen::Matrix4d::Identity() +
                      sin(angular_vel_norm * h / 2) / angular_vel_norm * A;
    }
    const Vector4<T> delta_z_times_quat1_vec4 = exp_half_Ah * Vector4<T>(quat1.w(), quat1.x(), quat1.y(), quat1.z());
    const Eigen::Quaternion<T> delta_z_times_quat1(delta_z_times_quat1_vec4(0), delta_z_times_quat1_vec4(1),
...
void TestEval(const QuaternionEulerIntegrationConstraint& dut,
    ...
    Eigen::VectorXd y;
    dut.Eval(x, &y);
    const auto y_expected =
            EvalQuaternionIntegration<double>(Eigen::Quaterniond(quat1(0), quat1(1), quat1(2), quat1(3)),
    ...
    const AutoDiffVecXd x_ad = math::InitializeAutoDiff(Eigen::VectorXd(x), x_grad);
    AutoDiffVecXd y_ad;
    dut.Eval(x_ad, &y_ad);
    const auto y_ad_expected =
            EvalQuaternionIntegration<AutoDiffXd>(Eigen::Quaternion<AutoDiffXd>(x_ad(0), x_ad(1), x_ad(2), x_ad(3)),
                                                  Eigen::Quaternion<AutoDiffXd>(x_ad(4), x_ad(5), x_ad(6), x_ad(7)),
                                                  x_ad.segment<3>(8), x_ad(11), dut.allow_quaternion_negation());
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_ad), math::ExtractValue(y_ad_expected), 10 * kEps));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_ad), math::ExtractGradient(y_ad_expected), 10 * kEps));
}
...
GTEST_TEST(QuaternionEulerIntegrationConstraintTest, TestEval) {
    for (bool allow_quaternion_negation : {true, false}) {
        ...
        const Eigen::Vector4d quat1 = Eigen::Vector4d(0.3, 0.5, 0.2, 0.1).normalized();
        const Eigen::Vector4d quat2 = Eigen::Vector4d(1.3, -.5, -0.7, 0.6).normalized();
...
GTEST_TEST(QuaternionEulerIntegrationConstraintTest, TestEvalSymbolic) {
    for (bool allow_quaternion_negation : {true, false}) {
        ...
        EXPECT_NO_THROW(dut.Eval(
                dut.ComposeVariable<symbolic::Variable>(Vector4<symbolic::Variable>(z1_w, z1_x, z1_y, z1_z),
...
Eigen::Quaterniond ComputeNextQuaternion(const Eigen::Quaterniond& quat_curr,
    ...
    if (angular_vel.norm() == 0) {
        ...
        return Eigen::AngleAxisd(angular_vel.norm() * h, angular_vel.normalized()) * quat_curr;
    }
}

void ExpectTwoQuaternionsClose(const Eigen::Quaterniond& z1,
                               const Eigen::Quaterniond& z2,
                               double tol,
                               bool allow_quaternion_negation) {
...
void TestSolveNextQuaternion(bool allow_quaternion_negation) {
    ...
    auto dut = std::make_shared<QuaternionEulerIntegrationConstraint>(allow_quaternion_negation);
    auto integration_cnstr =
            prog.AddConstraint(dut, dut->ComposeVariable<symbolic::Variable>(quat1, quat2, angular_vel, h));
    // Add the unit length constraint
    prog.AddConstraint(std::make_shared<UnitQuaternionConstraint>(), quat2);

    auto angular_vel_cnstr =
            prog.AddBoundingBoxConstraint(Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), angular_vel);
    auto quat1_cnstr = prog.AddBoundingBoxConstraint(Eigen::Vector4d::Zero(), Eigen::Vector4d::Zero(), quat1);
    ...
    for (const auto& quat1_val : quat1_vals) {
        ...
        for (const auto& angular_vel_val : angular_vel_vals) {
            angular_vel_cnstr.evaluator()->set_bounds(angular_vel_val, angular_vel_val);
            for (const auto& h_val : h_vals) {
                ...
                const Eigen::Quaterniond quat2_expected = ComputeNextQuaternion(
                        Eigen::Quaterniond(quat1_val(0), quat1_val(1), quat1_val(2), quat1_val(3)), angular_vel_val,
                        h_val);
                ExpectTwoQuaternionsClose(Eigen::Quaterniond(quat2_sol(0), quat2_sol(1), quat2_sol(2), quat2_sol(3)),
...
void TestSolveAngularVel(bool allow_quaternion_negation) {
    ...
    auto quat1_cnstr = prog.AddBoundingBoxConstraint(Eigen::Vector4d::Zero(), Eigen::Vector4d::Zero(), quat1);
    auto quat2_cnstr = prog.AddBoundingBoxConstraint(Eigen::Vector4d::Zero(), Eigen::Vector4d::Zero(), quat2);
    auto h_cnstr = prog.AddBoundingBoxConstraint(0., 0., h);
    std::vector<Eigen::Vector4d> quat1_vals;
    quat1_vals.push_back(Eigen::Vector4d(1, 0, 0, 0).normalized());
    quat1_vals.push_back(Eigen::Vector4d(0.5, -0.2, 1., 0.3).normalized());
    ...
    for (const auto& quat1_val : quat1_vals) {
        ...
        for (const auto& quat2_val : quat2_vals) {
            ...
            for (const auto& h_val : h_vals) {
                ...
                // the solution from the optimization.
                const Eigen::Quaterniond quat2_expected = ComputeNextQuaternion(
                        Eigen::Quaterniond(quat1_val(0), quat1_val(1), quat1_val(2), quat1_val(3)),
                        result.GetSolution(angular_vel), h_val);
                ExpectTwoQuaternionsClose(Eigen::Quaterniond(quat2_val(0), quat2_val(1), quat2_val(2), quat2_val(3)),
                                          quat2_expected, 1E-5, allow_quaternion_negation);
            }
        }
    }
    ...
    for (const auto& quat1_val : quat1_vals) {
        ...
        for (const auto h_val : h_vals) {
            ...
            const Eigen::Quaterniond quat2_expected =
                    ComputeNextQuaternion(Eigen::Quaterniond(quat1_val(0), quat1_val(1), quat1_val(2), quat1_val(3)),
                                          result.GetSolution(angular_vel), h_val);
            ExpectTwoQuaternionsClose(Eigen::Quaterniond(-quat1_val(0), -quat1_val(1), -quat1_val(2), -quat1_val(3)),
                                      quat2_expected, 1E-5, allow_quaternion_negation);
        }
    }
}
