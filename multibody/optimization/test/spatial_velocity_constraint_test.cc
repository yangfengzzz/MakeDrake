#include "multibody/optimization/spatial_velocity_constraint.h"
...
TEST_F(IiwaKinematicConstraintTest, SpatialVelocityConstraint) {
    ...
    SpatialVelocityConstraint constraint_translation_only(plant_autodiff_.get(), frameA, v_AC_lower, v_AC_upper, frameB,
                                                          p_BC, plant_context_autodiff_.get());

    EXPECT_TRUE(CompareMatrices(constraint_translation_only.lower_bound(), v_AC_lower));
    EXPECT_TRUE(CompareMatrices(constraint_translation_only.upper_bound(), v_AC_upper));

    const SpatialVelocityConstraint::AngularVelocityBounds w_AC_bounds{
            .magnitude_lower = 0.1, .magnitude_upper = 2.0, .reference_direction = {0.1, 0.2, 0.3}, .theta_bound = 0.5};

    SpatialVelocityConstraint constraint(plant_autodiff_.get(), frameA, v_AC_lower, v_AC_upper, frameB, p_BC,
                                         plant_context_autodiff_.get(), w_AC_bounds);

    VectorXd expected_lower(5), expected_upper(5);
    expected_lower << v_AC_lower, std::pow(w_AC_bounds.magnitude_lower, 2), std::cos(w_AC_bounds.theta_bound);
    ...
    VectorXd x(14);
    x << 0.1, 0.2, 0.25, -0.6, 0.26, 0.12, 0.15, 0.36, 0.73, 0.5, -0.62, 0.243, 0.245, 0.55;
    ...
    const auto V_AW_W =
            plant_->world_frame().CalcSpatialVelocity(*plant_context_, bodyA.body_frame(), plant_->world_frame());
    ...
    Vector3d p_WC_W;
    plant_->CalcPointsPositions(*plant_context_, bodyB.body_frame(), p_BC, plant_->world_frame(), &p_WC_W);
    ...
    EXPECT_NEAR(y[4], V_AC_A.rotational().normalized().dot(w_AC_bounds.reference_direction.normalized()), 1e-14);
}
...
TEST_F(IiwaKinematicConstraintTest, InvalidInputs) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialVelocityConstraint(nullptr, frameA, v_AC_lower, v_AC_upper, frameB, p_BC,
                                                          plant_context_autodiff_.get(), w_AC_bounds),
                                "plant is nullptr.");

    // @throws std::exception if `plant_context` is nullptr.
    DRAKE_EXPECT_THROWS_MESSAGE(SpatialVelocityConstraint(plant_autodiff_.get(), frameA, v_AC_lower, v_AC_upper, frameB,
                                                          p_BC, nullptr, w_AC_bounds),
