#include "multibody/optimization/contact_wrench_evaluator.h"
...
public:
    ContactWrenchEvaluatorTest() {
        ...
        free_spheres_ = std::make_unique<test::FreeSpheresAndBoxes<AutoDiffXd>>(spheres, boxes, ground_friction);
    }
...
TEST_F(ContactWrenchEvaluatorTest, ContactWrenchFromForceInWorldFrameEvaluator) {
    // Test the constructor, `ComposeVariableValues` and `Eval` functions.
    systems::Context<AutoDiffXd>* plant_context = free_spheres_->get_mutable_plant_context();
    ContactWrenchFromForceInWorldFrameEvaluator evaluator(
            &(free_spheres_->plant()), plant_context,
            SortedPair<geometry::GeometryId>(free_spheres_->sphere_geometry_ids()[0],
    ...
    const Vector3<AutoDiffXd> lambda_val = math::InitializeAutoDiff(Eigen::Vector3d(1, 2, 3));
    const VectorX<AutoDiffXd> x_value = evaluator.ComposeVariableValues(*plant_context, lambda_val);
    AutoDiffVecXd y;
    evaluator.Eval(x_value, &y);
    EXPECT_EQ(math::ExtractValue(y.head<3>()), Eigen::Vector3d::Zero());
    EXPECT_EQ(y.tail<3>(), lambda_val);
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y.tail<3>()), math::ExtractGradient(lambda_val)));
}
}  // namespace
}  // namespace multibody
}  // namespace drake
