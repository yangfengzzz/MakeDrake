...
void CompareAutoDiff(const Eigen::Ref<const AutoDiffVecXd>& x1, const Eigen::Ref<const AutoDiffVecXd>& x2, double tol) {
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(x1), math::ExtractValue(x2), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(x1), math::ExtractGradient(x2), tol));
}
...
void ComputeRelativeMotion(test::FreeSpheresAndBoxes<T>* spheres,
    ...
    const math::RigidTransform<T> X_WSa =
            plant.CalcRelativeTransform(spheres->plant_context(), plant.world_frame(),
                                        plant.get_body(spheres->sphere_body_indices()[0]).body_frame());
    const math::RigidTransform<AutoDiffXd> X_WSb =
            plant.CalcRelativeTransform(spheres->plant_context(), plant.world_frame(),
    ...
    *nhat_SaSb_W = (X_WSb.translation() - X_WSa.translation()).normalized();
    ...
    const SpatialVelocity<T> V_SaSb_W =
            plant.get_body(spheres->sphere_body_indices()[1])
                    .body_frame()
                    .CalcSpatialVelocity(spheres->plant_context(),
                                         plant.get_body(spheres->sphere_body_indices()[0]).body_frame(),
                                         plant.world_frame());
...
GTEST_TEST(SlidingFrictionComplementarityNonlinearConstraintTest, Constructor) {
    ...
    test::FreeSpheresAndBoxes<AutoDiffXd> spheres({sphere1_spec, sphere2_spec}, {} /* no box. */, ground_friction);

    ContactWrenchFromForceInWorldFrameEvaluator contact_wrench_evaluator(
            &(spheres.plant()), spheres.get_mutable_plant_context(),
            SortedPair<geometry::GeometryId>(spheres.sphere_geometry_ids()[0], spheres.sphere_geometry_ids()[1]));

    double complementarity_tolerance = 1E-3;
    SlidingFrictionComplementarityNonlinearConstraint constraint(&contact_wrench_evaluator, complementarity_tolerance);

    EXPECT_EQ(constraint.num_constraints(), 11);
    EXPECT_EQ(constraint.num_vars(), spheres.plant().num_positions() + spheres.plant().num_velocities() +
    ...
    x_val.head<14>() << 0.2, 1.4, 0.5, 0.3, 1.2, 0.7, -0.3, 0.4, -0.5, 0.2, 1.5, 1.4, 0.8, -0.2;  // q
    x_val.segment<12>(14) << -0.3, 0.2, -1.5, 0.3, 0.2, 2.5, 0.2, -0.4, 0.3, 1.5, 0.6, -2.3;      // v
    ...
    AutoDiffXd c;
    constraint.DecomposeX<AutoDiffXd>(x_autodiff, &q, &v, &lambda, &f_static, &f_sliding, &c);
    ...
    ComputeRelativeMotion<AutoDiffXd>(&spheres, q, v, sphere2_spec.radius, &n_SaSb_W, &v_SaCb_W);
    const Vector3<AutoDiffXd> v_tangential_SaCb_W =
            (Eigen::Matrix3d::Identity() - n_SaSb_W * n_SaSb_W.transpose()) * v_SaCb_W;
    const AutoDiffXd f_static_normal_magnitude = f_static.dot(n_SaSb_W);

    CompareAutoDiff(y_autodiff.segment<3>(3), v_tangential_SaCb_W * (f_static.dot(-n_SaSb_W)), tol);

    // Evaluate constraint (3)
    const CoulombFriction<double> combined_friction =
            CalcContactFrictionFromSurfaceProperties(sphere1_friction, sphere2_friction);
    CompareAutoDiff(y_autodiff(6), f_sliding.dot(n_SaSb_W), tol);

    const Vector3<AutoDiffXd> f_sliding_tangential =
            (Eigen::Matrix3d::Identity() - n_SaSb_W * n_SaSb_W.transpose()) * f_sliding;
    CompareAutoDiff(y_autodiff(7),
                    f_sliding_tangential.squaredNorm() -
                            pow((combined_friction.dynamic_friction() * f_sliding.dot(n_SaSb_W)), 2),
                    tol);

    // Evaluate constraint (4)
    CompareAutoDiff(y_autodiff.tail<3>(), f_sliding_tangential + c * v_tangential_SaCb_W, tol);
    ...
    y_grad.setZero();
    const auto gradient_sparsity_pattern = constraint.GetConstraintSparsityPattern();
    for (const auto& row_col_pair : gradient_sparsity_pattern) {
        y_grad(row_col_pair.first, row_col_pair.second) = y_grad_expected(row_col_pair.first, row_col_pair.second);
    }
...
GTEST_TEST(SlidingFrictionComplementarityConstraintTest, AddConstraint) {
    ...
    internal::ComputeRelativeMotion<AutoDiffXd>(&spheres, q_val.cast<AutoDiffXd>(), v_val.cast<AutoDiffXd>(),
    ...
    Eigen::Vector3d v_tangential_SaCb_W =
            (Eigen::Matrix3d::Identity() - nhat_SaSb_W * nhat_SaSb_W.transpose()) * v_SaCb_W;
    double c_val1 = 0.1;
    Eigen::Vector3d f_sliding_tangential = -c_val1 * v_tangential_SaCb_W;
    const CoulombFriction<double> combined_friction =
            CalcContactFrictionFromSurfaceProperties(sphere1_friction, sphere2_friction);
    Eigen::Vector3d f_sliding_normal = nhat_SaSb_W * f_sliding_tangential.norm() / combined_friction.dynamic_friction();
    ...
    bindings.first.evaluator()->ComposeX<double>(q_val, v_val, lambda, f_static, f_sliding, c_val1, &x_satisfied);
