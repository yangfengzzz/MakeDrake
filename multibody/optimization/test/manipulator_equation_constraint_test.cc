#include "multibody/optimization/manipulator_equation_constraint.h"
...
#include "multibody/optimization/test/optimization_with_contact_utilities.h"
...
public:
    TwoFreeSpheresTest() {
        ...
        spheres_ = std::make_unique<test::FreeSpheresAndBoxes<AutoDiffXd>>(
                spheres, std::vector<test::BoxSpecification>() /* no boxes */, CoulombFriction<double>(1, 0.8));
        spheres_double_ = std::make_unique<test::FreeSpheresAndBoxes<double>>(spheres_->spheres(), spheres_->boxes(),
                                                                              spheres_->ground_friction());
        const auto& query_port = spheres_->plant().get_geometry_query_input_port();
        const auto& query_object = query_port.Eval<geometry::QueryObject<AutoDiffXd>>(spheres_->plant_context());
        const std::set<std::pair<geometry::GeometryId, geometry::GeometryId>> collision_candidate_pairs =
        ...
        for (const auto& geometry_pair : collision_candidate_pairs) {
            auto wrench_evaluator = std::make_shared<ContactWrenchFromForceInWorldFrameEvaluator>(
                    &plant, spheres_->get_mutable_plant_context(),
                    SortedPair<geometry::GeometryId>(geometry_pair.first, geometry_pair.second));
            auto lambda_i = prog_.NewContinuousVariables(wrench_evaluator->num_lambda(),
                                                         "lambda" + std::to_string(geometry_pair_count));
            contact_wrench_evaluators_and_lambda_.push_back(std::make_pair(wrench_evaluator, lambda_i));
            geometry_pair_count++;
        }
    ...
    void CheckManipulatorEquationsConstraintEval(
            const solvers::Binding<ManipulatorEquationConstraint>& manipulator_equation_binding,
        ...
        spheres_->plant().SetPositions(spheres_->get_mutable_plant_context(), q_next_autodiff);
        spheres_->plant().SetVelocities(spheres_->get_mutable_plant_context(), v_next_autodiff);
        const AutoDiffVecXd g_autodiff =
                spheres_->plant().CalcGravityGeneralizedForces(*(spheres_->get_mutable_plant_context()));
        Eigen::Matrix<AutoDiffXd, Eigen::Dynamic, 1> C_bias_autodiff(spheres_->plant().num_velocities(), 1);
        spheres_->plant().CalcBiasTerm(*spheres_->get_mutable_plant_context(), &C_bias_autodiff);

        AutoDiffVecXd y_autodiff_expected = g_autodiff - C_bias_autodiff;

        const auto& query_port = spheres_->plant().get_geometry_query_input_port();
        const auto& query_object = query_port.Eval<geometry::QueryObject<AutoDiffXd>>(spheres_->plant_context());
        const std::vector<geometry::SignedDistancePair<AutoDiffXd>> signed_distance_pairs =
                query_object.ComputeSignedDistancePairwiseClosestPoints();
        EXPECT_EQ(signed_distance_pairs.size(), 3);
        const geometry::SceneGraphInspector<AutoDiffXd>& inspector = query_object.inspector();
        for (const auto& signed_distance_pair : signed_distance_pairs) {
            const geometry::FrameId frame_A_id = inspector.GetFrameId(signed_distance_pair.id_A);
            const geometry::FrameId frame_B_id = inspector.GetFrameId(signed_distance_pair.id_B);
            const Frame<AutoDiffXd>& frameA = spheres_->plant().GetBodyFromFrameId(frame_A_id)->body_frame();
            const Frame<AutoDiffXd>& frameB = spheres_->plant().GetBodyFromFrameId(frame_B_id)->body_frame();
            Eigen::Matrix<AutoDiffXd, 6, Eigen::Dynamic> Jv_V_WCa(6, 12);
            Eigen::Matrix<AutoDiffXd, 6, Eigen::Dynamic> Jv_V_WCb(6, 12);
            spheres_->plant().CalcJacobianSpatialVelocity(spheres_->plant_context(), JacobianWrtVariable::kV, frameA,
                                                          signed_distance_pair.p_ACa, spheres_->plant().world_frame(),
                                                          spheres_->plant().world_frame(), &Jv_V_WCa);
            spheres_->plant().CalcJacobianSpatialVelocity(spheres_->plant_context(), JacobianWrtVariable::kV, frameB,
            ...
            auto lambda_indices_in_all_lambda =
                    manipulator_equation_binding.evaluator()
                            ...
                            .at(SortedPair<geometry::GeometryId>(signed_distance_pair.id_A, signed_distance_pair.id_B))
            ...
            y_autodiff_expected += Jv_V_WCa.transpose() * -F_AB_W + Jv_V_WCb.transpose() * F_AB_W;
        }

        Eigen::Matrix<AutoDiffXd, Eigen::Dynamic, Eigen::Dynamic> M_mass_autodiff(spheres_->plant().num_velocities(),
                                                                                  spheres_->plant().num_velocities());
        spheres_->plant().CalcMassMatrixViaInverseDynamics(*spheres_->get_mutable_plant_context(), &M_mass_autodiff);

        y_autodiff_expected = y_autodiff_expected * dt_autodiff - M_mass_autodiff * (v_next_autodiff - v_autodiff);

        // 48 = 24 for velocities, 14 for positions, 9 for lambda, 1 for dt
        AutoDiffVecXd x_autodiff(48);
        x_autodiff << v_autodiff, q_next_autodiff, v_next_autodiff, lambda_autodiff, dt_autodiff;
        AutoDiffVecXd y_autodiff;
        manipulator_equation_binding.evaluator()->Eval(x_autodiff, &y_autodiff);
        EXPECT_TRUE(
                CompareMatrices(math::ExtractValue(y_autodiff), math::ExtractValue(y_autodiff_expected), 100 * kEps));
        EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_autodiff), math::ExtractGradient(y_autodiff_expected),
                                    100 * kEps));
        ...
        std::function<void(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd*)> eval_fun =
                [&manipulator_equation_binding](const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) {
                    manipulator_equation_binding.evaluator()->Eval(x, y);
                };

        const auto J = math::ComputeNumericalGradient(eval_fun, math::ExtractValue(x_autodiff));
...
protected:
    ...
    std::vector<std::pair<std::shared_ptr<ContactWrenchEvaluator>, VectorX<symbolic::Variable>>>
...
TEST_F(TwoFreeSpheresTest, Construction) {
    ...
    const auto& plant = spheres_->plant();
    const auto manipulator_equation_binding = ManipulatorEquationConstraint::MakeBinding(
            &plant, spheres_->get_mutable_plant_context(), contact_wrench_evaluators_and_lambda_, v_vars_, q_next_vars_,
            v_next_vars_, u_next_vars_, dt_var_);
    // Test the size of ManipulatorEquationConstraint
    EXPECT_EQ(manipulator_equation_binding.evaluator()->num_vars(),
              48 /* 24 for velocities, 14 for positions, 9 for lambda, 1 for dt */);
    EXPECT_EQ(manipulator_equation_binding.evaluator()->num_constraints(), plant.num_velocities());
    EXPECT_TRUE(CompareMatrices(manipulator_equation_binding.evaluator()->lower_bound(), Eigen::VectorXd::Zero(12)));
    EXPECT_TRUE(CompareMatrices(manipulator_equation_binding.evaluator()->upper_bound(), Eigen::VectorXd::Zero(12)));
    // Now check if each contact wrench evaluator is bound to the correct lambda.
    for (const auto& contact_wrench_evaluator_and_lambda : contact_wrench_evaluators_and_lambda_) {
        const auto& lambda_indices = manipulator_equation_binding.evaluator()
                                             ->contact_pair_to_wrench_evaluator()
                                             .at(contact_wrench_evaluator_and_lambda.first->geometry_id_pair())
                                             .lambda_indices_in_all_lambda;
        // Check if lambda_indices_in_all_lambda is correct.
        EXPECT_EQ(lambda_indices.size(), contact_wrench_evaluator_and_lambda.second.rows());
        for (int i = 0; i < static_cast<int>(lambda_indices.size()); ++i) {
            EXPECT_EQ(manipulator_equation_binding.variables()(v_vars_.rows() + q_next_vars_.rows() +
                                                               v_next_vars_.rows() + u_next_vars_.rows() +
                                                               lambda_indices[i]),
...
TEST_F(TwoFreeSpheresTest, Eval) {
    ...
    X_WS0.set(math::RotationMatrix<double>(Eigen::AngleAxisd(M_PI_4, Eigen::Vector3d(1.0 / 3, 2.0 / 3, 2.0 / 3))),
              Eigen::Vector3d(0.1, 0.2, 0.3));
    X_WS1.set(math::RotationMatrix<double>(Eigen::AngleAxisd(-0.1, Eigen::Vector3d(0.1, 0.2, 0.3).normalized())),
    ...
    CheckManipulatorEquationsConstraintEval(manipulator_equation_binding, x_autodiff.head<12>(),
    ...
    X_WS1.set_translation(Eigen::Vector3d(std::sqrt(4 * spheres_->spheres()[0].radius * spheres_->spheres()[1].radius),
    ...
    prog_.SetDecisionVariableValueInVector(contact_wrench_evaluators_and_lambda_[1].second,
                                           Eigen::Vector3d(0, 0, -spheres_->spheres()[0].inertia.get_mass() * 9.81),
                                           &x_init);
    prog_.SetDecisionVariableValueInVector(contact_wrench_evaluators_and_lambda_[2].second,
    ...
    spheres_double_->plant().SetPositions(spheres_double_->get_mutable_plant_context(),
                                          result.GetSolution(q_next_vars_));
    spheres_double_->plant().SetVelocities(spheres_double_->get_mutable_plant_context(),
                                           result.GetSolution(v_next_vars_));
    X_WS0 = spheres_double_->plant().GetFreeBodyPose(spheres_double_->plant_context(),
                                                     spheres_double_->plant().GetBodyByName("sphere0"));
    X_WS1 = spheres_double_->plant().GetFreeBodyPose(spheres_double_->plant_context(),
    ...
    sphere0_total_wrench.tail<3>() << 0, 0, -spheres_double_->spheres()[0].inertia.get_mass() * gravity;
    sphere0_total_wrench.head<3>() = X_WS0.translation().cross(sphere0_total_wrench.tail<3>());
    sphere1_total_wrench.tail<3>() << 0, 0, -spheres_double_->spheres()[1].inertia.get_mass() * gravity;
    sphere1_total_wrench.head<3>() = X_WS1.translation().cross(sphere1_total_wrench.tail<3>());
    for (const auto& contact_wrench_evaluator_and_lambda : contact_wrench_evaluators_and_lambda_) {
        const Eigen::Vector3d lambda_sol = result.GetSolution(contact_wrench_evaluator_and_lambda.second);
        if (contact_wrench_evaluator_and_lambda.first->geometry_id_pair() ==
            SortedPair<geometry::GeometryId>(spheres_->sphere_geometry_ids()[0], spheres_->sphere_geometry_ids()[1])) {
            // Contact between two spheres.
            // Compute the witness points on sphere 0 and sphere 1.
            const Eigen::Vector3d p_WC0 =
                    X_WS0.translation() +
                    (X_WS1.translation() - X_WS0.translation()).normalized() * spheres_double_->spheres()[0].radius;
            const Eigen::Vector3d p_WC1 =
                    X_WS1.translation() +
                    (X_WS0.translation() - X_WS1.translation()).normalized() * spheres_double_->spheres()[1].radius;
            ...
            const Eigen::Vector3d p_WC0 =
                    X_WS0.translation() - Eigen::Vector3d(0, 0, spheres_double_->spheres()[0].radius);
            sphere0_total_wrench.head<3>() += p_WC0.cross(-lambda_sol);
            sphere0_total_wrench.tail<3>() += -lambda_sol;
        } else if (contact_wrench_evaluator_and_lambda.first->geometry_id_pair() ==
                   SortedPair<geometry::GeometryId>(spheres_->sphere_geometry_ids()[1],
                                                    spheres_->ground_geometry_id())) {
            // contact between the ground and sphere 1.
            // Compute the witness point on sphere 1
            const Eigen::Vector3d p_WC1 =
                    X_WS1.translation() - Eigen::Vector3d(0, 0, spheres_double_->spheres()[1].radius);
            sphere1_total_wrench.head<3>() += p_WC1.cross(-lambda_sol);
            sphere1_total_wrench.tail<3>() += -lambda_sol;
        } else {
    ...
    Eigen::Matrix<double, Eigen::Dynamic, 1> C_bias(spheres_double_->plant().num_velocities(), 1);
    spheres_double_->plant().CalcBiasTerm(*spheres_double_->get_mutable_plant_context(), &C_bias);
    EXPECT_TRUE(CompareMatrices(C_bias, Eigen::VectorXd(12).setZero(), 1e-12));

    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> M_mass(spheres_double_->plant().num_velocities(),
                                                                 spheres_double_->plant().num_velocities());
    spheres_double_->plant().CalcMassMatrixViaInverseDynamics(*spheres_double_->get_mutable_plant_context(), &M_mass);
    ...
    lhs.segment<3>(6) = lhs.segment<3>(6) + X_WS1.translation().cross(lhs.tail<3>());
