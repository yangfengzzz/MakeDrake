...
GTEST_TEST(StaticFrictionConeComplementarityNonlinearConstraint, TestEval) {
    const CoulombFriction<double> ground_friction(1.5 /* static friction */, 0.8 /* dynamic friction */);

    const double radius = 0.1;
    const CoulombFriction<double> sphere_friction = ground_friction;

    test::FreeSpheresAndBoxes<AutoDiffXd> dut({test::SphereSpecification(radius, 1E3, sphere_friction)},
    ...
    ContactWrenchFromForceInWorldFrameEvaluator contact_wrench_evaluator(
            ...
            SortedPair<geometry::GeometryId>(dut.sphere_geometry_ids()[0], dut.ground_geometry_id()));

    double complementarity_tolerance{1E-3};
    internal::StaticFrictionConeComplementarityNonlinearConstraint constraint(&contact_wrench_evaluator,
    ...
    EXPECT_TRUE(CompareMatrices(constraint.lower_bound(), Eigen::Vector4d::Zero()));
    EXPECT_TRUE(CompareMatrices(constraint.upper_bound(), Eigen::Vector4d(kInf, 0, 0, complementarity_tolerance)));
    ...
    constraint.DecomposeX<double>(x_val, &q_val, &lambda_val, &alpha_val, &beta_val);
    ...
    y_val_expected(0) = lambda_val.dot(((std::pow(ground_friction.static_friction(), 2) + 1) * n_W * n_W.transpose() -
    ...
    std::function<void(const Eigen::Ref<const Eigen::VectorXd>&, Eigen::VectorXd*)> eval_fun =
            [&constraint](const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) {
                constraint.Eval(x, y);
            };
    auto dy_dx = math::ComputeNumericalGradient(eval_fun, x_val);
    const double gradient_tol = 1E-5;
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(y_ad), dy_dx * dx, gradient_tol));
