#include "multibody/optimization/centroidal_momentum_constraint.h"
...
namespace multibody {
template <typename T>
VectorX<T> EvalConstraint(const MultibodyPlant<T>* plant,
    ...
    if (model_instances.has_value()) {
        ...
        h_WC = plant->CalcSpatialMomentumInWorldAboutPoint(*plant_context, model_instances.value(), p_WC);
    } else {
...
template <typename T>
void CentroidalMomentumConstraintTester(const MultibodyPlant<T>* plant,
    ...
    CentroidalMomentumConstraint dut(plant, model_instances, plant_context, angular_only);
    const int momentum_dim = angular_only ? 3 : 6;
    DRAKE_DEMAND(momentum_val.rows() == momentum_dim);
    EXPECT_EQ(dut.num_vars(), plant->num_positions() + plant->num_velocities() + momentum_dim);
    EXPECT_EQ(dut.num_constraints(), momentum_dim);
    EXPECT_TRUE(CompareMatrices(dut.lower_bound(), VectorX<double>::Zero(momentum_dim)));
    EXPECT_TRUE(CompareMatrices(dut.upper_bound(), VectorX<double>::Zero(momentum_dim)));
    Eigen::VectorXd x;
    ...
    if constexpr (std::is_same_v<T, double>) {
        const VectorX<double> y_expected =
                EvalConstraint<double>(plant, model_instances, plant_context, angular_only, q, v, momentum_val);
        EXPECT_TRUE(CompareMatrices(y, y_expected));
    } else {
        const VectorX<AutoDiffXd> y_expected =
                EvalConstraint<AutoDiffXd>(plant, model_instances, plant_context, angular_only, q.cast<AutoDiffXd>(),
                                           v.cast<AutoDiffXd>(), momentum_val.cast<AutoDiffXd>());
    ...
    VectorX<AutoDiffXd> y_autodiff_expected = EvalConstraint<AutoDiffXd>(
            ...
            x_autodiff.segment(plant->num_positions(), plant->num_velocities()), x_autodiff.tail(momentum_dim));
...
GTEST_TEST(CentroidalMomentumConstraintTest, TestDualIiwas) {
    ...
    for (bool angular_only : {true, false}) {
        ...
        std::vector<ModelInstanceIndex> model_instances{{ModelInstanceIndex{plant->GetModelInstanceByName("iiwa14")}}};
        CentroidalMomentumConstraintTester(plant_autodiff.get(), plant_autodiff.get(), model_instances,
                                           plant_context_autodiff.get(), plant_context_autodiff.get(), angular_only, q,
                                           v, momentum_val);
    }
}
}  // namespace multibody
