...
struct SphereSpecification {
    SphereSpecification(double radius_in, double density, CoulombFriction<double> friction_in)
        : radius{radius_in},
          inertia{SpatialInertia<double>::SolidSphereWithDensity(density, radius_in)},
...
struct BoxSpecification {
    BoxSpecification(Eigen::Vector3d size_in,
                     double density,
                     CoulombFriction<double> friction_in,
                     std::optional<math::RigidTransformd> X_WB_in)
        : size(std::move(size_in)),
          inertia{SpatialInertia<double>::SolidBoxWithDensity(density, size[0], size[1], size[2])},
          friction{std::move(friction_in)},
...
public:
    ...
    const systems::Context<T>& diagram_context() const { return *diagram_context_; }

    const systems::Context<T>& plant_context() const { return *plant_context_; }

    systems::Context<T>* get_mutable_plant_context() const { return plant_context_; }

    const std::vector<geometry::GeometryId>& sphere_geometry_ids() const { return sphere_geometry_ids_; }

    const std::vector<geometry::GeometryId>& box_geometry_ids() const { return box_geometry_ids_; }

    geometry::GeometryId ground_geometry_id() const { return ground_geometry_id_; }
    ...
    const CoulombFriction<double>& ground_friction() const { return ground_friction_; }

    const std::vector<BodyIndex>& sphere_body_indices() const { return sphere_body_indices_; }

    const std::vector<BodyIndex>& box_body_indices() const { return box_body_indices_; }
