...
StaticEquilibriumConstraint::StaticEquilibriumConstraint(
    ...
    : solvers::Constraint(
              ...
              plant->num_positions() + plant->num_actuated_dofs() + GetLambdaSize(contact_pair_to_wrench_evaluator),
...
void StaticEquilibriumConstraint::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    AutoDiffVecXd y_autodiff(num_constraints());
    DoEval(x.cast<AutoDiffXd>(), &y_autodiff);
    *y = math::ExtractValue(y_autodiff);
}

void StaticEquilibriumConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    const auto& q = x.head(plant_->num_positions());
    const auto& u = x.segment(plant_->num_positions(), plant_->num_actuated_dofs());
    *y = B_actuation_ * u;
    ...
    *y += plant_->CalcGravityGeneralizedForces(*context_);
    ...
    const int lambda_start_index_in_x = plant_->num_positions() + plant_->num_actuated_dofs();
    for (const auto& [contact_pair, evaluator] : contact_pair_to_wrench_evaluator_) {
        const geometry::SignedDistancePair<AutoDiffXd> signed_distance_pair =
                query_object.ComputeSignedDistancePairClosestPoints(contact_pair.first(), contact_pair.second());
        ...
        VectorX<AutoDiffXd> lambda(evaluator.contact_wrench_evaluator->num_lambda());

        for (int i = 0; i < lambda.rows(); ++i) {
            lambda(i) = x(lambda_start_index_in_x + evaluator.lambda_indices_in_all_lambda[i]);
        }

        AutoDiffVecXd F_AB_W;
        evaluator.contact_wrench_evaluator->Eval(
                evaluator.contact_wrench_evaluator->ComposeVariableValues(*context_, lambda), &F_AB_W);
...
void StaticEquilibriumConstraint::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
...
solvers::Binding<StaticEquilibriumConstraint> StaticEquilibriumConstraint::MakeBinding(
        const MultibodyPlant<AutoDiffXd>* plant,
        systems::Context<AutoDiffXd>* context,
    ...
    VectorX<symbolic::Variable> q_u_lambda(plant->num_positions() + plant->num_actuated_dofs() + num_lambda);
    ...
    auto static_equilibrium_constraint =
            ...
            std::shared_ptr<StaticEquilibriumConstraint>(
                    new StaticEquilibriumConstraint(plant, context, contact_pair_to_wrench_evaluator));
    return solvers::Binding<StaticEquilibriumConstraint>(static_equilibrium_constraint, q_u_lambda);
}
}  // namespace multibody
}  // namespace drake
