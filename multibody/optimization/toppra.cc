#include "multibody/optimization/toppra.h"
...
Eigen::VectorXd Toppra::CalcGridPoints(const Trajectory<double>& path, const CalcGridPointsOptions& options) {
    ...
    for (int iter = 0; iter < options.max_iter; iter++) {
        ...
        for (auto point = gridpts.begin(); std::next(point, 1) != gridpts.end(); point++) {
            ...
            const double error = 0.5 * std::pow(dist, 2) * path.EvalDerivative(mid_pt, 2).cwiseAbs().maxCoeff();
    ...
    return Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result.data(), result.size());
}
...
Toppra::Toppra(const Trajectory<double>& path,
    ...
    : backward_prog_{new solvers::MathematicalProgram()},
      ...
      backward_cost_{backward_prog_->AddLinearCost(Eigen::Vector2d::Zero(), 0, {backward_x_, backward_u_})},
      backward_continuity_con_{backward_prog_->AddLinearConstraint(
              Eigen::RowVector2d::Zero(), Vector1d(0), Vector1d(0), {backward_x_, backward_u_})},
      ...
      forward_continuity_con_{forward_prog_->AddLinearConstraint(Vector1d(0), Vector1d(0), Vector1d(0), forward_u_)},
      path_{path},
      plant_{plant},
...
Binding<BoundingBoxConstraint> Toppra::AddJointVelocityLimit(const Eigen::Ref<const Eigen::VectorXd>& lower_limit,
...
std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> Toppra::AddJointAccelerationLimit(
    ...
    DRAKE_DEMAND(lower_limit.size() == n_dof);
    DRAKE_DEMAND(upper_limit.size() == n_dof);
    const int n_con = discretization == ToppraDiscretization::kInterpolation ? 2 * n_dof : n_dof;
    ...
    auto backward_con =
            backward_prog_->AddLinearConstraint(Eigen::MatrixXd::Zero(n_con, 2), Eigen::VectorXd::Zero(n_con),
                                                Eigen::VectorXd::Zero(n_con), {backward_x_, backward_u_});
    auto forward_con = forward_prog_->AddLinearConstraint(Eigen::VectorXd::Zero(n_con), Eigen::VectorXd::Zero(n_con),
...
std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> Toppra::AddJointTorqueLimit(
        const Eigen::Ref<const Eigen::VectorXd>& lower_limit,
        const Eigen::Ref<const Eigen::VectorXd>& upper_limit,
        ToppraDiscretization discretization) {
    const int N = gridpoints_.size() - 1;
    ...
    auto backward_con =
            backward_prog_->AddLinearConstraint(Eigen::MatrixXd::Zero(n_con, 2), Eigen::VectorXd::Zero(n_con),
                                                Eigen::VectorXd::Zero(n_con), {backward_x_, backward_u_});
    auto forward_con = forward_prog_->AddLinearConstraint(Eigen::MatrixXd::Zero(n_con, 1), Eigen::VectorXd::Zero(n_con),
                                                          Eigen::VectorXd::Zero(n_con), forward_u_);
    auto coefficients = ToppraLinearConstraint(con_A, con_lb, con_ub);
    backward_lin_constraint_.emplace(backward_con, coefficients);
    forward_lin_constraint_.emplace(forward_con, coefficients);
...
Binding<BoundingBoxConstraint> Toppra::AddFrameVelocityLimit(const Frame<double>& constraint_frame,
    ...
    for (int knot = 0; knot < N; knot++) {
        ...
        plant_.SetPositions(plant_context_.get(), qs);
        plant_.SetVelocities(plant_context_.get(), qs_dot);
        velocity = constraint_frame.CalcSpatialVelocityInWorld(*plant_context_).get_coeffs();
...
// as path.
PiecewisePolynomial<double> MakeLimitTrajectory(const Trajectory<double>& path, const Eigen::MatrixXd& value) {
...
Eigen::MatrixXd SampleOrDefault(double t, const Trajectory<double>& path, const Eigen::MatrixXd& or_default) {
...
Binding<BoundingBoxConstraint> Toppra::AddFrameTranslationalSpeedLimit(const Frame<double>& constraint_frame,
                                                                       const double& upper_limit) {
    const auto upper_bound_traj = MakeLimitTrajectory(path_, Vector1<double>::Constant(upper_limit));
...
Binding<BoundingBoxConstraint> Toppra::AddFrameTranslationalSpeedLimit(const Frame<double>& constraint_frame,
    ...
    if (upper_limit.start_time() > path_.end_time() || upper_limit.end_time() < path_.start_time()) {
        throw std::runtime_error(
                fmt::format("Toppra can't add a trajectory translational speed "
                            "limit. The upper limit domain [{}, {}] must overlap the "
                            "path domain [{}, {}].",
                            upper_limit.start_time(), upper_limit.end_time(), path_.start_time(), path_.end_time()));
    }

    const MatrixX<double> kInf_mat = Vector1<double>::Constant(std::numeric_limits<double>::infinity());
    ...
    for (int knot = 0; knot < N; knot++) {
        ...
        plant_.SetPositions(plant_context_.get(), qs);
        plant_.SetVelocities(plant_context_.get(), qs_dot);
        const SpatialVelocity<double> velocity = constraint_frame.CalcSpatialVelocityInWorld(*plant_context_);
        const double speed_squared = velocity.translational().squaredNorm();

        const double sampled_limit = SampleOrDefault(t, upper_limit, kInf_mat)(0, 0);
...
std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> Toppra::AddFrameAccelerationLimit(
    ...
    return AddFrameAccelerationLimit(constraint_frame, lower_traj, upper_traj, discretization);
}
...
std::pair<Binding<LinearConstraint>, Binding<LinearConstraint>> Toppra::AddFrameAccelerationLimit(
    ...
    if (upper_limit.start_time() > path_.end_time() || upper_limit.end_time() < path_.start_time() ||
        lower_limit.start_time() > path_.end_time() || lower_limit.end_time() < path_.start_time()) {
        throw std::runtime_error(
                fmt::format("Toppra can't add a trajectory frame acceleration "
                            ...
                            "[{}, {}].",
                            lower_limit.start_time(), lower_limit.end_time(), upper_limit.start_time(),
                            upper_limit.end_time(), path_.start_time(), path_.end_time()));
    }
    const Vector6d kInf_mat = Vector6d::Constant(std::numeric_limits<double>::infinity());

    const int N = gridpoints_.size() - 1;
    const int n_dof = path_.rows();
    const int n_con = discretization == ToppraDiscretization::kInterpolation ? 12 : 6;
    Eigen::MatrixXd con_A(n_con, 2 * N);
    Eigen::MatrixXd con_lb(n_con, N);
    Eigen::MatrixXd con_ub(n_con, N);
    ...
    for (int knot = 0; knot < N; knot++) {
        ...
        plant_.SetPositions(plant_context_.get(), qs);
        plant_.SetVelocities(plant_context_.get(), qs_dot);
        plant_.CalcJacobianSpatialVelocity(*plant_context_, JacobianWrtVariable::kQDot, constraint_frame,
                                           Eigen::Vector3d::Zero(), plant_.world_frame(), plant_.world_frame(), &J);
        dJds_times_v =
                plant_.CalcBiasSpatialAcceleration(*plant_context_, JacobianWrtVariable::kV, constraint_frame,
...
void Toppra::CalcInterpolationConstraint(Eigen::MatrixXd* A,
    ...
    for (int knot = 1; knot < N; knot++) {
        ...
        A->block(n_con, 2 * (knot - 1), n_con, 1) << A->block(0, 2 * knot, n_con, 1);
        A->block(n_con, 2 * (knot - 1) + 1, n_con, 1)
                << A->block(0, 2 * knot + 1, n_con, 1) + 2 * delta * A->block(0, 2 * knot, n_con, 1);
        lower_bound->block(n_con, knot - 1, n_con, 1) << lower_bound->block(0, knot, n_con, 1);
        upper_bound->block(n_con, knot - 1, n_con, 1) << upper_bound->block(0, knot, n_con, 1);
    }
    A->block(n_con, 2 * (N - 1), n_con, 1) << A->block(0, 2 * (N - 1), n_con, 1);
    A->block(n_con, 2 * (N - 1) + 1, n_con, 1) << A->block(0, 2 * (N - 1) + 1, n_con, 1);
    lower_bound->block(n_con, N - 1, n_con, 1) << lower_bound->block(0, N - 1, n_con, 1);
    upper_bound->block(n_con, N - 1, n_con, 1) << upper_bound->block(0, N - 1, n_con, 1);
}
...
std::optional<Eigen::Matrix2Xd> Toppra::ComputeBackwardPass(double s_dot_0,
    ...
    for (int knot = N - 1; knot > -1; knot--) {
        ...
        backward_continuity_con_.evaluator()->UpdateCoefficients(Eigen::Vector2d(1, 2 * delta).transpose(),
                                                                 Vector1d(K(0, knot + 1)), Vector1d(K(1, knot + 1)));
        for (auto& [constraint, bounds] : x_bounds_) {
            constraint.evaluator()->set_bounds(Vector1d(bounds.lb(knot)), Vector1d(bounds.ub(knot)));
        }
        for (auto& [constraint, coefficients] : backward_lin_constraint_) {
            constraint.evaluator()->UpdateCoefficients(coefficients.coeffs.middleCols<2>(2 * knot),
...
std::optional<std::pair<Eigen::VectorXd, Eigen::VectorXd>> Toppra::ComputeForwardPass(
        double s_dot_0, const Eigen::Ref<const Eigen::Matrix2Xd>& K, const SolverInterface& solver) {
    const int N = gridpoints_.size() - 1;
    DRAKE_DEMAND(s_dot_0 >= 0);
    ...
    for (int knot = 0; knot < N; knot++) {
        ...
        forward_continuity_con_.evaluator()->UpdateCoefficients(
                Vector1d(2 * delta), Vector1d(K(0, knot + 1) - xstar(knot)), Vector1d(K(1, knot + 1) - xstar(knot)));
        for (auto& [constraint, coefficients] : forward_lin_constraint_) {
            constraint.evaluator()->UpdateCoefficients(
                    ...
                    coefficients.lb.col(knot) - xstar(knot) * coefficients.coeffs.col(2 * knot),
                    coefficients.ub.col(knot) - xstar(knot) * coefficients.coeffs.col(2 * knot));
        }
        solvers::MathematicalProgramResult result;
        solver.Solve(*forward_prog_, {}, {}, &result);
        if (!result.is_success()) {
            drake::log()->error("Toppra failed to find the maximum path acceleration at knot {}/{}.", knot, N);
            return std::nullopt;
        } else {
...
std::optional<PiecewisePolynomial<double>> Toppra::SolvePathParameterization() {
    ...
    auto solver = solvers::MakeFirstAvailableSolver({solvers::ClpSolver::id(), solvers::MosekSolver::id()});

    const std::optional<Eigen::Matrix2Xd> K = ComputeBackwardPass(s_dot_0, s_dot_N, *solver);
    ...
    for (size_t knot = 0; knot < t_knots.size() - 1; knot++) {
        ...
        const Eigen::Vector3d coeffs(gridpoints_(knot), sd_knots(knot), 0.5 * u_star(knot));
