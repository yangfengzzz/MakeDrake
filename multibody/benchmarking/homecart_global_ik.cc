...
BENCHMARK_F(HomecartGlobalIkBenchmark, PostureCost)(benchmark::State& state) {  // NOLINT
    ...
    parser.AddModelsFromUrl("package://drake_models/tri_homecart/homecart_no_grippers.dmd.yaml");
    plant.Finalize();

    Eigen::VectorXd q0(plant.num_positions());
    // A somewhat arbitrary configuration with the arms posed comfortably inside
    // the workspace.
    q0 << 1.75, -1.04, 1.27, -1.79, -2.75, 0.25, -1.45, -2.5, -1.04, -1.32, 3.11, 0;

    GlobalInverseKinematics global_ik(plant);
    global_ik.AddPostureCost(q0, Eigen::VectorXd::Constant(plant.num_bodies(), 1.0),
