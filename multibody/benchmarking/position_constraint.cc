...
public:
    void SetUp(::benchmark::State&) override {
        ...
        for (int i = 0; i < kNumIiwas; ++i) {
            const ModelInstanceIndex model_instance = parser.AddModelsFromUrl(iiwa_url).at(0);
            plant_->WeldFrames(plant_->world_frame(), plant_->GetFrameByName("iiwa_link_0", model_instance));
        }
        plant_->Finalize();
        ...
        const ModelInstanceIndex first_iiwa = plant_->GetModelInstanceByName("iiwa14");
        constraint_ = std::make_unique<PositionConstraint>(
                plant_.get(), plant_->GetFrameByName("iiwa_link_7", first_iiwa), p_AQ_lower, p_AQ_upper,
                plant_->GetFrameByName("iiwa_link_3", first_iiwa), p_BQ, context_.get());
        ...
        const ModelInstanceIndex ad_first_iiwa = ad_plant_->GetModelInstanceByName("iiwa14");
        ad_constraint_ = std::make_unique<PositionConstraint>(
                ad_plant_.get(), ad_plant_->GetFrameByName("iiwa_link_7", ad_first_iiwa), p_AQ_lower, p_AQ_upper,
                ad_plant_->GetFrameByName("iiwa_link_3", ad_first_iiwa), p_BQ, ad_context_.get());
    }
