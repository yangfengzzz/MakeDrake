...
BENCHMARK_F(RelaxedPosIkBenchmark, Iiwa)(benchmark::State& state) {  // NOLINT
    ...
    const std::string iiwa_url = "package://drake_models/iiwa_description/sdf/iiwa7_no_collision.sdf";
    ...
    const multibody::RigidBody<double>& ee_body = plant.GetBodyByName(ee_link_name);
    ...
    for (int i = 0; i < kNumRandGoals; ++i) {
        ...
        context->get_mutable_continuous_state().get_mutable_generalized_position().SetFromVector(
    ...
    // does not violate any of the joint position limits.
    const Eigen::MatrixXd q0(Eigen::MatrixXd::Random(plant.num_positions(), kNumRandInitGuess));

    // Create the task in terms of position constraints on the end effector.
    auto pos_constraint = std::make_shared<PositionConstraint>(&plant, plant.world_frame(), -kPosTol, kPosTol, ee_frame,
