...
protected:
    ...
    // a bitmask for the case's "Arg" to denote which quantities are in scope as
    // either gradients (for T=AutoDiffXd) or variables (for T=Expression).
    static bool want_grad_q(const benchmark::State& state) { return state.range(0) & kWantGradQ; }
    static bool want_grad_v(const benchmark::State& state) { return state.range(0) & kWantGradV; }
    static bool want_grad_vdot(const benchmark::State& state) { return state.range(0) & kWantGradVdot; }
    static bool want_grad_u(const benchmark::State& state) { return state.range(0) & kWantGradU; }
    ...
    FixedInputPortValue& input_ = plant_->get_actuation_input_port().FixValue(context_.get(), VectorX<T>::Zero(nu_));
...
void Cassie<T>::SetUpNonZeroState() {
    ...
    context_->get_mutable_continuous_state_vector().SetFromVector(VectorX<T>::LinSpaced(nq_ + nv_, 0.1, 0.9));
    for (const BodyIndex& index : plant_->GetFloatingBaseBodies()) {
        const RigidBody<T>& body = plant_->get_body(index);
        const RigidTransform<T> pose(RollPitchYaw<T>(0.1, 0.2, 0.3), Vector3<T>(0.4, 0.5, 0.6));
    ...
    // Reset 'u'.
    input_.GetMutableVectorData<T>()->SetFromVector(VectorX<T>::Constant(nu_, 0.5));

    // Reset 'tau'.
    external_forces_.mutable_generalized_forces() = VectorX<T>::LinSpaced(nv_, 0.01, 0.09);
...
void Cassie<AutoDiffXd>::SetUpGradientsOrVariables(benchmark::State& state) {
    ...
    std::tie(q_grad, v_grad, vdot_grad, u_grad) = math::InitializeAutoDiffTuple(q, v, vdot, u);
...
BENCHMARK_REGISTER_F(CassieDouble, MassMatrix)->Unit(benchmark::kMicrosecond)->Arg(kWantNoGrad);

// NOLINTNEXTLINE(runtime/references)
BENCHMARK_DEFINE_F(CassieDouble, InverseDynamics)(benchmark::State& state) {
    DoInverseDynamics(state);
}
BENCHMARK_REGISTER_F(CassieDouble, InverseDynamics)->Unit(benchmark::kMicrosecond)->Arg(kWantNoGrad);

// NOLINTNEXTLINE(runtime/references)
BENCHMARK_DEFINE_F(CassieDouble, ForwardDynamics)(benchmark::State& state) {
    DoForwardDynamics(state);
}
BENCHMARK_REGISTER_F(CassieDouble, ForwardDynamics)->Unit(benchmark::kMicrosecond)->Arg(kWantNoGrad);
