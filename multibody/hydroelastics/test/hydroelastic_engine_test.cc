...
protected:
    ...
    GeometryId AddGeometry(const std::string& name) {
        FrameId frame_id = scene_graph_.RegisterFrame(source_id_, GeometryFrame(name + "_frame"));
        GeometryId id = scene_graph_.RegisterGeometry(
                source_id_, frame_id, make_unique<GeometryInstance>(RigidTransformd{}, make_unique<Sphere>(1.0), name));
        return id;
    }

    /** Adds a geometry with the given name and assigns compliant hydroelastic
     geometry properties.  */
    GeometryId AddCompliantGeometry(const std::string& name, double hydroelastic_modulus, double dissipation) {
        GeometryId id = AddGeometry(name);
        ProximityProperties props;
        geometry::AddCompliantHydroelasticProperties(/* resolution_hint */ 1.0, hydroelastic_modulus, &props);
    ...
    GeometryId AddRigidGeometryWithElasticModulus(const std::string& name, double hydroelastic_modulus) {
        ...
        ProximityProperties props;
        geometry::AddRigidHydroelasticProperties(/* resolution_hint */ 1.0, &props);
        props.AddProperty(geometry::internal::kHydroGroup, geometry::internal::kElastic, hydroelastic_modulus);
        scene_graph_.AssignRole(source_id_, id, props);
        return id;
    }

    /** Returns a model inspector for this class's scene graph.  */
    const SceneGraphInspector<double>& inspector() const { return scene_graph_.model_inspector(); }
...
TEST_F(HydroelasticEngineTest, CombineSoftAndSoftMaterialProperties) {
    ...
    // The combination must be symmetric.
    EXPECT_NEAR(engine.CalcCombinedElasticModulus(id_A, id_B, inspector()), Estar, kEps);
    EXPECT_NEAR(engine.CalcCombinedDissipation(id_A, id_B, inspector()), dstar, kEps);
    EXPECT_NEAR(engine.CalcCombinedElasticModulus(id_B, id_A, inspector()), Estar, kEps);
    EXPECT_NEAR(engine.CalcCombinedDissipation(id_B, id_A, inspector()), dstar, kEps);
}
...
TEST_F(HydroelasticEngineTest, MissingProximityPropertiesThrows) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(engine.CalcCombinedElasticModulus(id_A, id_B, inspector()),
                                "Unable to get the material properties .+ it has no proximity properties "
                                "assigned.*");

    // Case: First lacking properties.
    DRAKE_EXPECT_THROWS_MESSAGE(engine.CalcCombinedElasticModulus(id_A, id_C, inspector()),
                                "Unable to get the material properties .+ it has no proximity properties "
                                "assigned.*");

    // Case: Second lacking properties.
    DRAKE_EXPECT_THROWS_MESSAGE(engine.CalcCombinedElasticModulus(id_C, id_B, inspector()),
                                "Unable to get the material properties .+ it has no proximity properties "
                                "assigned.*");
}
