...
template <typename T>
MaterialProperties GetMaterials(GeometryId id, const SceneGraphInspector<T>& inspector) {
    const double kInf = std::numeric_limits<double>::infinity();
    MaterialProperties material;
    if (const ProximityProperties* properties = inspector.GetProximityProperties(id)) {
        // If the geometry is defined to be rigid, force an elastic modulus of
        // infinity.
        if (properties->GetPropertyOrDefault(geometry::internal::kHydroGroup, geometry::internal::kComplianceType,
                                             geometry::internal::HydroelasticType::kUndefined) ==
            geometry::internal::HydroelasticType::kRigid) {
            material.hydroelastic_modulus = kInf;
        } else {
            material.hydroelastic_modulus = properties->GetPropertyOrDefault(geometry::internal::kHydroGroup,
                                                                             geometry::internal::kElastic, kInf);
        }

        material.dissipation = properties->GetPropertyOrDefault(geometry::internal::kMaterialGroup,
                                                                geometry::internal::kHcDissipation, 0.0);

        DRAKE_DEMAND(material.hydroelastic_modulus > 0);
        DRAKE_DEMAND(material.dissipation >= 0);
    } else {
        throw std::runtime_error(
                fmt::format("Unable to get the material properties for geometry {}; it has no "
...
template <typename T>
double HydroelasticEngine<T>::CalcCombinedElasticModulus(GeometryId id_A,
                                                         GeometryId id_B,
...
// resolved.
template <typename T>
double HydroelasticEngine<T>::CalcCombinedDissipation(GeometryId id_A,
                                                      GeometryId id_B,
                                                      const SceneGraphInspector<T>& inspector) const {
    const double kInf = std::numeric_limits<double>::infinity();
    const MaterialProperties material_A = GetMaterials(id_A, inspector);
