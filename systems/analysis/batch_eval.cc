#include "systems/analysis/batch_eval.h"
...
MatrixX<T> BatchEvalUniquePeriodicDiscreteUpdate(const System<T>& system,
    ...
    DRAKE_THROW_UNLESS(states.rows() == context.get_discrete_state_vector().size());
    DRAKE_THROW_UNLESS(states.cols() == num_evals);
    const InputPort<T>* input_port = system.get_input_port_selection(input_port_index);
    if (input_port) {
        DRAKE_THROW_UNLESS(input_port->get_data_type() == PortDataType::kVectorValued);
    ...
    const auto calc_next_state = [&](const int thread_num, const int64_t i) {
        ...
        for (int step = 0; step < num_time_steps; ++step) {
            ...
            next_states.col(i) = system.EvalUniquePeriodicDiscreteUpdate(*context_pool[thread_num]).value();
        }
    };

    StaticParallelForIndexLoop(DegreeOfParallelism(num_threads_to_use), 0, num_evals, calc_next_state,
...
MatrixX<T> BatchEvalTimeDerivatives(const System<T>& system,
    ...
    const auto calc_derivatives = [&](const int thread_num, const int64_t i) {
        ...
        derivatives.col(i) = system.EvalTimeDerivatives(*context_pool[thread_num]).CopyToVector();
    };

    StaticParallelForIndexLoop(DegreeOfParallelism(num_threads_to_use), 0, num_evals, calc_derivatives,
                               ParallelForBackend::BEST_AVAILABLE);

    return derivatives;
}

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&BatchEvalUniquePeriodicDiscreteUpdate<T>,
                                                                  &BatchEvalTimeDerivatives<T>));
