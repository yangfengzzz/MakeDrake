...
#include "systems/analysis/dense_output.h"
#include "systems/analysis/scalar_dense_output.h"
    ...
    explicit ScalarViewDenseOutput(std::unique_ptr<DenseOutput<T>> base_output, int n)
        ...
        if (n < 0 || base_output_->size() <= n) {
            throw std::runtime_error(
                    fmt::format("Index {} out of base dense output [0, {}) range.", n, base_output_->size()));
        }
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::systems::ScalarViewDenseOutput);
