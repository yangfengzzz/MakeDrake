...
#include "systems/analysis/simulator_python_internal.h"

namespace drake {
namespace systems {

template <typename T>
Simulator<T>::Simulator(const System<T>& system, std::unique_ptr<Context<T>> context)
    : Simulator(&system, nullptr, std::move(context)) {}

template <typename T>
Simulator<T>::Simulator(std::unique_ptr<const System<T>> owned_system, std::unique_ptr<Context<T>> context)
...
Simulator<T>::Simulator(const System<T>* system,
    ...
    integrator_ = std::unique_ptr<IntegratorBase<T>>(new RungeKutta3Integrator<T>(system_, context_.get()));
    ...
    witness_functions_ = std::make_unique<std::vector<const WitnessFunction<T>*>>();
...
SimulatorStatus Simulator<T>::Initialize(const InitializeParams& params) {
    ...
    if (!context_) throw std::logic_error("Initialize(): Context has not been set.");
    ...
    EventStatus accumulated_event_status = HandleUnrestrictedUpdate(merged_events_->get_unrestricted_update_events());
    if (HasEventFailureOrMaybeThrow(accumulated_event_status, true /*throw on failure*/, &initialize_status)) {
        return initialize_status;
    }

    // Do restricted (discrete variable) updates next.
    accumulated_event_status.KeepMoreSevere(HandleDiscreteUpdate(merged_events_->get_discrete_update_events()));
    if (HasEventFailureOrMaybeThrow(accumulated_event_status, true /*throw on failure*/, &initialize_status)) {
        return initialize_status;
    }
    ...
    if (!accumulated_event_status.reached_termination()) {
        ...
        const T slightly_before_current_time = internal::GetPreviousNormalizedValue(current_time);
        context_->PerturbTime(slightly_before_current_time, current_time);
        const T time_of_next_timed_event = system_.CalcNextUpdateTime(*context_, timed_events_.get());
        context_->SetTime(current_time);  // Restore the current time.

        // Indicate a timed event is to be handled now, if appropriate.
        time_or_witness_triggered_ = (time_of_next_timed_event == current_time ? kTimeTriggered : kNothingTriggered);
        ...
        if (time_or_witness_triggered_ & kTimeTriggered) merged_events_->AddToEnd(*timed_events_);
        ...
        accumulated_event_status.KeepMoreSevere(HandlePublish(merged_events_->get_publish_events()));

        // If requested, do a force-publish before the simulation starts.
        if (publish_at_initialization_) {
            accumulated_event_status.KeepMoreSevere(HandlePublish(system_.get_forced_publish_events()));
        }

        // Invoke the monitor() if there is one. This is logically like a
        // Diagram-level Publish event so we handle it similarly.
        if (get_monitor()) accumulated_event_status.KeepMoreSevere(get_monitor()(*context_));
    ...
    if (accumulated_event_status.severity() == EventStatus::kReachedTermination) {
        initialize_status.SetReachedTermination(ExtractDoubleOrThrow(context_->get_time()),
...
template <typename T>
EventStatus Simulator<T>::HandleUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events) {
    ...
    const EventStatus status = system_.CalcUnrestrictedUpdate(*context_, events, unrestricted_updates_.get());
    if (status.did_nothing() || status.failed()) return status;

    // Now write the update back into the context.
    system_.ApplyUnrestrictedUpdate(events, unrestricted_updates_.get(), context_.get());
...
template <typename T>
EventStatus Simulator<T>::HandleDiscreteUpdate(const EventCollection<DiscreteUpdateEvent<T>>& events) {
    if (!events.HasEvents()) return EventStatus::DidNothing();

    // First, compute the discrete updates into a temporary buffer and return
    // if no update is warranted.
    const EventStatus status = system_.CalcDiscreteVariableUpdate(*context_, events, discrete_updates_.get());
    if (status.did_nothing() || status.failed()) return status;

    // Write updates back into the context.
    system_.ApplyDiscreteVariableUpdate(events, discrete_updates_.get(), context_.get());
...
template <typename T>
EventStatus Simulator<T>::HandlePublish(const EventCollection<PublishEvent<T>>& events) {
    if (!events.HasEvents()) return EventStatus::DidNothing();
...
template <typename T>
bool Simulator<T>::HasEventFailureOrMaybeThrow(const EventStatus& event_status,
                                               bool throw_on_failure,
                                               SimulatorStatus* simulator_status) {
    if (!event_status.failed()) return false;
    simulator_status->SetEventHandlerFailed(ExtractDoubleOrThrow(context_->get_time()), event_status.system(),
                                            event_status.message());
    if (throw_on_failure) throw std::runtime_error(simulator_status->FormatMessage());
...
SimulatorStatus Simulator<T>::AdvanceTo(const T& boundary_time) {
    if (!initialization_done_) {
        ...
        if (!initialize_status.succeeded()) return initialize_status;
    }
    ...
    if (time_or_witness_triggered_ & kTimeTriggered) merged_events_->AddToEnd(*timed_events_);
    if (time_or_witness_triggered_ & kWitnessTriggered) merged_events_->AddToEnd(*witnessed_events_);
    ...
    while (true) {
        ...
        // Do unrestricted updates first.
        EventStatus accumulated_event_status =
                HandleUnrestrictedUpdate(merged_events_->get_unrestricted_update_events());
        if (HasEventFailureOrMaybeThrow(accumulated_event_status, true /*throw on failure*/, &simulator_status)) {
        ...
        if (!accumulated_event_status.reached_termination()) {
            ...
            using std::isfinite;
            DRAKE_DEMAND(!isfinite(time_of_next_timed_event) || timed_events_->HasEvents());
            ...
            if (timed_events_->HasDiscreteUpdateEvents() || timed_events_->HasUnrestrictedUpdateEvents()) {
            ...
            time_or_witness_triggered_ = IntegrateContinuousState(next_publish_time, next_update_time, boundary_time,
        ...
        if (accumulated_event_status.reached_termination()) {
            simulator_status.SetReachedTermination(ExtractDoubleOrThrow(context_->get_time()),
        ...
        if (!simulator_status.succeeded() || context_->get_time() >= boundary_time) break;
    }
...
std::optional<T> Simulator<T>::GetCurrentWitnessTimeIsolation() const {
    ...
    if (integrator_->get_fixed_step_mode()) {
        ...
        if (accuracy) {
            return max(integrator_->get_working_minimum_step_size(),
                       T(iso_scale_factor * accuracy.value() * integrator_->get_maximum_step_size()));
        } else {
    ...
    return max(integrator_->get_working_minimum_step_size(), iso_scale_factor * accuracy.value() * characteristic_time);
}
...
template <class T>
void Simulator<T>::IsolateWitnessTriggers(const std::vector<const WitnessFunction<T>*>& witnesses,
                                          const VectorX<T>& w0,
                                          const T& t0,
                                          const VectorX<T>& x0,
                                          const T& tf,
    ...
    if (!witness_iso_len) return;

    // Mini function for integrating the system forward in time from t0.
    std::function<void(const T&)> integrate_forward = [&t0, &x0, &context, this](const T& t_des) {
        const T inf = std::numeric_limits<double>::infinity();
        context.SetTime(t0);
        context.SetContinuousState(x0);
        while (context.get_time() < t_des) integrator_->IntegrateNoFurtherThanTime(inf, inf, t_des);
    };
    ...
    DRAKE_LOGGER_DEBUG("Isolating witness functions using isolation window of {} over [{}, {}]",
    ...
    do {
        ...
        for (size_t i = 0; i < witnesses.size(); ++i) {
            ...
            if (witnesses[i]->should_trigger(w0[i], wc[i])) trigger = true;
        }
    ...
    triggered_witnesses->clear();
    for (size_t i = 0; i < witnesses.size(); ++i) {
        if (witnesses[i]->should_trigger(w0[i], wc[i])) triggered_witnesses->push_back(witnesses[i]);
    }
}

// Evaluates the given vector of witness functions.
template <class T>
VectorX<T> Simulator<T>::EvaluateWitnessFunctions(const std::vector<const WitnessFunction<T>*>& witness_functions,
...
template <class T>
bool Simulator<T>::DidWitnessTrigger(const std::vector<const WitnessFunction<T>*>& witness_functions,
                                     const VectorX<T>& w0,
                                     const VectorX<T>& wf,
                                     std::vector<const WitnessFunction<T>*>* triggered_witnesses) {
...
template <class T>
void Simulator<T>::PopulateEventDataForTriggeredWitness(const T& t0,
                                                        const T& tf,
                                                        const WitnessFunction<T>* witness,
                                                        Event<T>* event,
                                                        CompositeEventCollection<T>* events) const {
    // Populate the event data.
    WitnessTriggeredEventData<T>* event_data = event->template get_mutable_event_data<WitnessTriggeredEventData<T>>();
    ...
    get_system().AddTriggeredWitnessFunctionToCompositeEventCollection(event, events);
}
...
template <class T>
typename Simulator<T>::TimeOrWitnessTriggered Simulator<T>::IntegrateContinuousState(
    ...
    typename IntegratorBase<T>::StepResult result =
            integrator_->IntegrateNoFurtherThanTime(next_publish_time, next_update_time, boundary_time);
    ...
    if (DidWitnessTrigger(witness_functions, w0_, wf_, &triggered_witnesses_)) {
        ...
        IsolateWitnessTriggers(witness_functions, w0_, t0, x0, tf, &triggered_witnesses_);
        ...
        if (!triggered_witnesses_.empty()) event_handler_xc_->SetFromVector(x0);

        // Store witness function(s) that triggered.
        for (const WitnessFunction<T>* fn : triggered_witnesses_) {
            DRAKE_LOGGER_DEBUG("Witness function {} crossed zero at time {}", fn->description(), context.get_time());
            ...
            PopulateEventDataForTriggeredWitness(t0, tf, fn, event.get(), witnessed_events);
        }
...
void Simulator<T>::PauseIfTooFast() const {
    ...
    const TimePoint desired_realtime = initial_realtime_ + Duration(simtime_passed / target_realtime_rate_);
    // TODO(sherm1): Could add some slop to now() and not sleep if
    // we are already close enough. But what is a reasonable value?
    if (desired_realtime > Clock::now()) std::this_thread::sleep_until(desired_realtime);
}
...
namespace internal {
template <typename T>
void SimulatorPythonInternal<T>::set_python_monitor(Simulator<T>* simulator, void (*monitor)()) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class drake::systems::Simulator);
