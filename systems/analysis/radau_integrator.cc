#include "systems/analysis/radau_integrator.h"
...
template <typename T, int num_stages>
RadauIntegrator<T, num_stages>::RadauIntegrator(const System<T>& system, Context<T>* context)
...
void RadauIntegrator<T, num_stages>::DoInitialize() {
    ...
    const int state_dim = this->get_context().get_continuous_state_vector().size();
    ...
    // mode).
    bs3_ = std::make_unique<BogackiShampine3Integrator<T>>(this->get_system(), this->get_mutable_context());
...
template <typename T, int num_stages>
const VectorX<T>& RadauIntegrator<T, num_stages>::ComputeFofZ(const T& t0,
                                                              const T& h,
                                                              const VectorX<T>& xt0,
                                                              const VectorX<T>& Z) {
...
template <typename T, int num_stages>
void RadauIntegrator<T, num_stages>::ComputeSolutionFromIterate(const VectorX<T>& xt0,
                                                                const VectorX<T>& Z,
...
template <typename T, int num_stages>
bool RadauIntegrator<T, num_stages>::StepRadau(
        const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus, int trial) {
    using std::max;
    using std::min;
    ...
    DRAKE_LOGGER_DEBUG("StepRadau() entered for t={}, h={}, trial={}", t0, h, trial);
    ...
    *xtplus = xt0;
    ...
    if (!this->get_use_full_newton() &&
        !this->MaybeFreshenMatrices(t0, xt0, h, trial, construct_iteration_matrix, &iteration_matrix_radau_)) {
        return false;
    }
    ...
    for (int iter = 0; iter < this->max_newton_raphson_iterations(); ++iter) {
        ...
        this->FreshenMatricesIfFullNewton(tf, *xtplus, h, construct_iteration_matrix, &iteration_matrix_radau_);
        ...
        DRAKE_LOGGER_DEBUG("residual: {}", fmt_eigen((A_tp_eye_ * (h * F_of_Z) - Z_).transpose()));
        VectorX<T> dZ = iteration_matrix_radau_.Solve(A_tp_eye_ * (h * F_of_Z) - Z_);
...
bool RadauIntegrator<T, num_stages>::StepImplicitTrapezoid(const T& t0,
    ...
    std::function<VectorX<T>()> g = [&xt0, h, &dx0, context, this]() {
        return (context->get_continuous_state().CopyToVector() - xt0 -
                h / 2 * (dx0 + this->EvalTimeDerivatives(this->get_context()).CopyToVector()))
                .eval();
    };
    ...
    int stored_num_iter_factorizations = this->get_num_iteration_matrix_factorizations();
    int64_t stored_num_function_evaluations = this->get_num_derivative_evaluations();
    int64_t stored_num_jacobian_function_evaluations = this->get_num_derivative_evaluations_for_jacobian();
    int stored_num_nr_iterations = this->get_num_newton_raphson_iterations();

    // Step.
    bool success = StepImplicitTrapezoidDetail(t0, h, xt0, g, radau_xtplus, xtplus);

    // Move statistics to implicit trapezoid-specific.
    num_err_est_jacobian_reforms_ += this->get_num_jacobian_evaluations() - stored_num_jacobian_evaluations;
    num_err_est_iter_factorizations_ +=
            this->get_num_iteration_matrix_factorizations() - stored_num_iter_factorizations;
    num_err_est_function_evaluations_ += this->get_num_derivative_evaluations() - stored_num_function_evaluations;
    ...
    num_err_est_nr_iterations_ += this->get_num_newton_raphson_iterations() - stored_num_nr_iterations;
...
bool RadauIntegrator<T, num_stages>::StepImplicitTrapezoidDetail(const T& t0,
    ...
    // Verify xtplus.
    Context<T>* context = this->get_mutable_context();
    DRAKE_ASSERT(xtplus && xtplus->size() == context->get_continuous_state_vector().size());
    ...
    *xtplus = radau_xtplus;
    ...
    if (!this->get_use_full_newton() &&
        !this->MaybeFreshenMatrices(t0, xt0, h, trial, ComputeImplicitTrapezoidIterationMatrix,
    ...
    for (int iter = 0; iter < this->max_newton_raphson_iterations(); ++iter) {
        ...
        this->FreshenMatricesIfFullNewton(tf, *xtplus, h, ComputeImplicitTrapezoidIterationMatrix,
                                          &iteration_matrix_implicit_trapezoid_);
    ...
    // factorizations as helpful.
    return StepImplicitTrapezoidDetail(t0, h, xt0, g, radau_xtplus, xtplus, trial + 1);
}

template <typename T, int num_stages>
bool RadauIntegrator<T, num_stages>::AttemptStepPaired(
        const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus_radau, VectorX<T>* xtplus_itr) {
    using std::abs;
    ...
    *xtplus_radau = xt0;
    ...
    if (StepImplicitTrapezoid(t0, h, xt0, dx0, *xtplus_radau, xtplus_itr)) {
        ...
        this->get_mutable_context()->SetTimeAndContinuousState(t0 + h, *xtplus_radau);
        return true;
    } else {
        DRAKE_LOGGER_DEBUG(
...
template <typename T, int num_stages>
void RadauIntegrator<T, num_stages>::ComputeAndSetErrorEstimate(const VectorX<T>& xtplus_prop,
...
bool RadauIntegrator<T, num_stages>::DoImplicitIntegratorStep(const T& h) {
    ...
    if (h < this->get_working_minimum_step_size()) {
        ...
        bool success = AttemptStepPaired(t0, h, xt0_, &xtplus_prop_, &xtplus_embed_);
...
void RadauIntegrator<T, num_stages>::ComputeRadauIterationMatrix(
    ...
    // TODO(edrumwri) Investigate how to do the below operation with a move.
    // Computes I - h A ⊗ J.
    iteration_matrix->SetAndFactorIterationMatrix(CalcTensorProduct(A * -h, J) + MatrixX<T>::Identity(n, n));
}

template <typename T, int num_stages>
MatrixX<T> RadauIntegrator<T, num_stages>::CalcTensorProduct(const MatrixX<T>& A, const MatrixX<T>& B) {
