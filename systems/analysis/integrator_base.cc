...
T IntegratorBase<T>::CalcStateChangeNorm(const ContinuousState<T>& dx_state) const {
    ...
    T v_nrm = qbar_v_weight.cwiseProduct(unweighted_substate_change_).template lpNorm<Eigen::Infinity>() *
              characteristic_time;
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::systems::IntegratorBase);
