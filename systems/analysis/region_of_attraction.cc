#include "systems/analysis/region_of_attraction.h"

#include <algorithm>

#include "math/continuous_lyapunov_equation.h"
...
Expression FixedLyapunovConvex(const solvers::VectorXIndeterminate& x,
    ...
    const Eigen::MatrixXd P = symbolic::Evaluate(symbolic::Jacobian(Vdot.Jacobian(x), x), env);
    ...
    if (Vdot_is_locally_negative_definite) {
        ...
        const Eigen::MatrixXd S = symbolic::Evaluate(symbolic::Jacobian(V.Jacobian(x), x), env);
    ...
    prog.AddSosConstraint(((V_balanced - rho) * Polynomial(pow((x.transpose() * x)[0], d)) - lambda * Vdot_balanced));
...
Expression FixedLyapunovConvexImplicit(const solvers::VectorXIndeterminate& x,
                                       const solvers::VectorXIndeterminate& xdot,
                                       const Expression& V,
                                       const Expression& Vdot,
                                       const VectorX<Expression>& g) {
    ...
    y << x, xdot;
    const Eigen::MatrixXd P = symbolic::Evaluate(symbolic::Jacobian(Vdot.Jacobian(y), y), env);
    const Eigen::MatrixXd G = symbolic::Evaluate(symbolic::Jacobian(g, y), env);
    Eigen::FullPivLU<MatrixXd> lu(G);
    MatrixXd N = lu.kernel();
    const double tolerance = 1e-8;
    bool Vdot_is_locally_negative_definite = IsPositiveDefinite(-N.transpose() * P * N, tolerance);
    ...
    for (int i = 0; i < g.size(); ++i) {
        ...
        const int lambda_gi_degree = std::max(lambda_degree + Vdot_degree - Polynomial(g[0]).TotalDegree(), 0);
    ...
    prog.AddSosConstraint(
            ((V_poly - rho) * Polynomial(pow((x.transpose() * x)[0], d)) - lambda * Vdot_poly + lambda_g.dot(g_poly)));
...
Expression RegionOfAttraction(const System<double>& system,
    ...
    VectorX<double> xdot0 = system.EvalTimeDerivatives(context).get_vector().CopyToVector();
    ...
    Expression V;
    bool user_provided_lyapunov_candidate = !options.lyapunov_candidate.EqualTo(Expression::Zero());
    ...
    if (user_provided_lyapunov_candidate) {
        ...
        const auto linearized_system =
                Linearize(system, context, InputPortSelection::kNoInput, OutputPortSelection::kNoOutput);
        const Eigen::MatrixXd Q = Eigen::MatrixXd::Identity(num_states, num_states);
        const Eigen::MatrixXd P = math::RealContinuousLyapunovEquation(linearized_system->A(), Q);
        V = x_bar.dot(P * x_bar);
    }
    ...
    if (options.use_implicit_dynamics) {
        ...
        const solvers::VectorXIndeterminate xdot = prog.NewIndeterminates(derivatives->size(), "xdot");
        const Expression Vdot = V.Jacobian(x_bar).dot(xdot);
        derivatives->SetFromVector(xdot.cast<Expression>());
        VectorX<Expression> g(symbolic_system->implicit_time_derivatives_residual_size());
        symbolic_system->CalcImplicitTimeDerivativesResidual(*symbolic_context, *derivatives, &g);
        V = FixedLyapunovConvexImplicit(x_bar, xdot, V, Vdot, g);
    } else {
        const VectorX<Expression> f =
                symbolic_system->EvalTimeDerivatives(*symbolic_context).get_vector().CopyToVector();
        const Expression Vdot = V.Jacobian(x_bar).dot(f);
        V = FixedLyapunovConvex(x_bar, V, Vdot, options.solver_id, options.solver_options);
    }
