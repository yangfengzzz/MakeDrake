...
public:
    ...
    explicit ImplicitEulerIntegrator(const System<T>& system, Context<T>* context = nullptr)
     ...
     */
    void set_use_implicit_trapezoid_error_estimation(bool flag) { use_implicit_trapezoid_error_estimation_ = flag; }
     ...
     */
    bool get_use_implicit_trapezoid_error_estimation() { return use_implicit_trapezoid_error_estimation_; }
...
private:
    ...
    int64_t do_get_num_newton_raphson_iterations() const final { return num_nr_iterations_; }
    ...
    int64_t do_get_num_error_estimator_derivative_evaluations() const final {
        ...
        return use_implicit_trapezoid_error_estimation_
                       ...
                       : (this->get_num_derivative_evaluations() - hie_statistics_.num_function_evaluations);
    }

    int64_t do_get_num_error_estimator_derivative_evaluations_for_jacobian() const final {
        // See the above comments in
        // do_get_num_error_estimator_derivative_evaluations().
        return use_implicit_trapezoid_error_estimation_ ? itr_statistics_.num_jacobian_function_evaluations
    ...
    int64_t do_get_num_error_estimator_newton_raphson_iterations() const final {
        ...
        return use_implicit_trapezoid_error_estimation_
                       ...
                       : (this->get_num_newton_raphson_iterations() - hie_statistics_.num_nr_iterations);
    }
    ...
    int64_t do_get_num_error_estimator_jacobian_evaluations() const final {
        ...
        return use_implicit_trapezoid_error_estimation_
                       ...
                       : (this->get_num_jacobian_evaluations() - hie_statistics_.num_jacobian_reforms);
    }

    int64_t do_get_num_error_estimator_iteration_matrix_factorizations() const final {
        // See the above comments in
        // do_get_num_error_estimator_derivative_evaluations().
        return use_implicit_trapezoid_error_estimation_
                       ? itr_statistics_.num_iter_factorizations
                       : (this->get_num_iteration_matrix_factorizations() - hie_statistics_.num_iter_factorizations);
    }
    ...
    // @post The time and continuous state in the context are indeterminate upon
    //       exit.
    bool StepImplicitEuler(const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus);
    ...
    // @post The time and continuous state in the context are indeterminate upon
    //       exit.
    bool StepImplicitEulerWithGuess(
    ...
    // @post The time and continuous state in the context are indeterminate upon
    //       exit.
    bool StepHalfSizedImplicitEulers(
    ...
    // @post The time and continuous state in the context are indeterminate upon
    //       exit.
    bool StepImplicitTrapezoid(const T& t0,
                               const T& h,
                               const VectorX<T>& xt0,
                               const VectorX<T>& dx0,
