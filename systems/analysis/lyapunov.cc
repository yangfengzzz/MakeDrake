...
// true.
void AddLinearConstraintIfNonTrivial(const symbolic::Formula& f, solvers::MathematicalProgram* prog) {
...
Eigen::VectorXd SampleBasedLyapunovAnalysis(
    ...
    const solvers::VectorXDecisionVariable slack = prog.NewContinuousVariables(num_samples, "s");
    // Minimize ∑ sᵢ
    ...
    for (int si = 0; si < num_samples; si++) {
        ...
        const Eigen::VectorXd phidot = math::ExtractGradient(phi) * derivatives->CopyToVector();
