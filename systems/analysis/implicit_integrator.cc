...
void ImplicitIntegrator<T>::ComputeAutoDiffJacobian(
        const System<T>& system, const T& t, const VectorX<T>& xt, const Context<T>& context, MatrixX<T>* J) {
    ...
    std::unique_ptr<Context<AutoDiffXd>> adiff_context = adiff_system->AllocateContext();
    ...
    const VectorX<AutoDiffXd> result = this->EvalTimeDerivatives(*adiff_system, *adiff_context).CopyToVector();
...
void ImplicitIntegrator<T>::ComputeForwardDiffJacobian(
        const System<T>&, const T& t, const VectorX<T>& xt, Context<T>* context, MatrixX<T>* J) {
    using std::abs;
    ...
    DRAKE_LOGGER_DEBUG("  ImplicitIntegrator Compute Forwarddiff {}-Jacobian t={}", n, t);
...
void ImplicitIntegrator<T>::ComputeCentralDiffJacobian(
    ...
    DRAKE_LOGGER_DEBUG("  ImplicitIntegrator Compute Centraldiff {}-Jacobian t={}", n, t);
...
template <class T>
void ImplicitIntegrator<T>::IterationMatrix::SetAndFactorIterationMatrix(const MatrixX<T>& iteration_matrix) {
    LU_.compute(iteration_matrix);
    matrix_factored_ = true;
}

template <class T>
VectorX<T> ImplicitIntegrator<T>::IterationMatrix::Solve(const VectorX<T>& b) const {
    return LU_.solve(b);
}

template <typename T>
typename ImplicitIntegrator<T>::ConvergenceStatus ImplicitIntegrator<T>::CheckNewtonConvergence(
        int iteration, const VectorX<T>& xtplus, const VectorX<T>& dx, const T& dx_norm, const T& last_dx_norm) const {
    ...
    if (iteration > 1) {
        ...
        DRAKE_LOGGER_DEBUG("Newton-Raphson loop {} theta: {}, eta: {}", iteration, theta, eta);
...
template <class T>
const MatrixX<T>& ImplicitIntegrator<T>::CalcJacobian(const T& t, const VectorX<T>& x) {
    // We change the context but will change it back.
    Context<T>* context = this->get_mutable_context();

    // Get the current time and state.
    const T t_current = context->get_time();
    const VectorX<T> x_current = context->get_continuous_state_vector().CopyToVector();
    ...
    // evaluations.
    num_jacobian_function_evaluations_ += this->get_num_derivative_evaluations() - current_ODE_evals;
...
bool ImplicitIntegrator<T>::MaybeFreshenMatrices(
    ...
    switch (trial) {
        ...
        case 3: {
            ...
            if (jacobian_is_fresh_) return false;
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class drake::systems::ImplicitIntegrator);
