...
std::vector<double> ExtractDoublesOrThrow(const std::vector<S>& input_vector) {
    ...
    std::transform(input_vector.begin(), input_vector.end(), std::back_inserter(output_vector), [](const S& value) {
    ...
    explicit HermitianDenseOutput(const trajectories::PiecewisePolynomial<T>& trajectory)
        ...
        for (int i = 0; i < trajectory.get_number_of_segments(); i++) {
            ...
            continuous_trajectory_.ConcatenateInTime(PiecewisePolynomial<double>(
                    {polyd}, {ExtractDoubleOrThrow(breaks[i]), ExtractDoubleOrThrow(breaks[i + 1])}));
        }
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::systems::HermitianDenseOutput);
