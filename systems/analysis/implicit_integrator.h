...
public:
    ...
    explicit ImplicitIntegrator(const System<T>& system, Context<T>* context = nullptr)
     ...
     */
    int max_newton_raphson_iterations() const { return do_max_newton_raphson_iterations(); }
    ...
    JacobianComputationScheme get_jacobian_computation_scheme() const { return jacobian_scheme_; }
    /// @}
    ...
    int64_t get_num_derivative_evaluations_for_jacobian() const { return num_jacobian_function_evaluations_; }
    ...
    int64_t get_num_jacobian_evaluations() const { return num_jacobian_evaluations_; }
    ...
    int64_t get_num_newton_raphson_iterations() const { return do_get_num_newton_raphson_iterations(); }
    ...
    int64_t get_num_iteration_matrix_factorizations() const { return num_iter_factorizations_; }
...
protected:
    ...
    void ComputeForwardDiffJacobian(
            const System<T>& system, const T& t, const VectorX<T>& xt, Context<T>* context, MatrixX<T>* J);
    ...
    // @param[out] J the Jacobian matrix around time and state `(t, xt)`.
    // @post The continuous state will be indeterminate on return.
    void ComputeCentralDiffJacobian(
            const System<T>& system, const T& t, const VectorX<T>& xt, Context<T>* context, MatrixX<T>* J);
...
private:
    ...
    JacobianComputationScheme jacobian_scheme_{JacobianComputationScheme::kForwardDifference};
...
template <>
inline void ImplicitIntegrator<AutoDiffXd>::ComputeAutoDiffJacobian(const System<AutoDiffXd>&,
                                                                    const AutoDiffXd&,
...
// header file (to avoid multiple definition errors).
template <>
inline void ImplicitIntegrator<AutoDiffXd>::IterationMatrix::SetAndFactorIterationMatrix(
...
// header file (to avoid multiple definition errors).
template <>
inline VectorX<AutoDiffXd> ImplicitIntegrator<AutoDiffXd>::IterationMatrix::Solve(const VectorX<AutoDiffXd>& b) const {
    return QR_.solve(b);
}
