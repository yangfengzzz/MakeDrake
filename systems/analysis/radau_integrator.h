...
#include "systems/analysis/bogacki_shampine3_integrator.h"
...
class RadauIntegrator final : public ImplicitIntegrator<T> {
    static_assert(num_stages == 1 || num_stages == 2, "Only 1-stage and 2-stage Radau are supported.");

public:
    DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(RadauIntegrator);

    explicit RadauIntegrator(const System<T>& system, Context<T>* context = nullptr);
...
private:
    ...
    int64_t do_get_num_error_estimator_newton_raphson_iterations() const final { return num_err_est_nr_iterations_; }

    int64_t do_get_num_error_estimator_jacobian_evaluations() const final { return num_err_est_jacobian_reforms_; }
    ...
    // iterate (Z).
    void ComputeSolutionFromIterate(const VectorX<T>& xt0, const VectorX<T>& Z, VectorX<T>* xtplus) const;

    // Updates the error estimate from the propagated solution and the embedded
    // solution.
    void ComputeAndSetErrorEstimate(const VectorX<T>& xtplus_prop, const VectorX<T>& xtplus_embed);
    ...
    const VectorX<T>& ComputeFofZ(const T& t0, const T& h, const VectorX<T>& xt0, const VectorX<T>& Z);
    ...
    //           step of size `h`.
    bool StepRadau(const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus, int trial = 1);
    ...
    static void ComputeRadauIterationMatrix(const MatrixX<T>& J,
                                            const T& h,
                                            const MatrixX<double>& A,
                                            typename ImplicitIntegrator<T>::IterationMatrix* iteration_matrix);

    // Does all of the real work for the implicit trapezoid method.
    bool StepImplicitTrapezoidDetail(const T& t0,
                                     const T& h,
                                     const VectorX<T>& xt0,
    ...
    typename ImplicitIntegrator<T>::IterationMatrix iteration_matrix_implicit_trapezoid_;
