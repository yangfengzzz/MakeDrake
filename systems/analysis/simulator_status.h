...
public:
    ...
    void SetReachedTermination(double termination_time, const SystemBase* system, std::string message) {
        SetResult(termination_time, kReachedTerminationCondition, system, std::move(message));
    }
    ...
    bool IsIdenticalStatus(const SimulatorStatus& other) {
        return boundary_time() == other.boundary_time() && return_time() == other.return_time() &&
               reason() == other.reason() && system() == other.system() && message() == other.message();
    }
    ...
    explicit SimulatorStatus(double boundary_time) : boundary_time_(boundary_time) { SetReachedBoundaryTime(); }
#endif

private:
    void SetResult(double return_time, ReturnReason reason, const SystemBase* system, std::string message) {
