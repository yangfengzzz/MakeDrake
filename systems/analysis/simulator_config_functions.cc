...
template <typename T>
using ResetIntegratorFunc = function<IntegratorBase<T>*(Simulator<T>*, const T& /* max_step_size */)>;
...
NamedResetIntegratorFunc<T> MakeResetter() {
    constexpr bool is_fixed_step = std::is_constructible_v<Integrator<T>, const System<T>&, T, Context<T>*>;
    constexpr bool is_error_controlled = std::is_constructible_v<Integrator<T>, const System<T>&, Context<T>*>;
    static_assert(is_fixed_step ^ is_error_controlled);
    return NamedResetIntegratorFunc<T>(
            GetIntegratorName<Integrator>(), [](Simulator<T>* simulator, const T& max_step_size) {
                if constexpr (is_fixed_step) {
                    IntegratorBase<T>& result = simulator->template reset_integrator<Integrator<T>>(max_step_size);
                    return &result;
                } else {
                    IntegratorBase<T>& result = simulator->template reset_integrator<Integrator<T>>();
...
template <typename T>
IntegratorBase<T>& ResetIntegratorFromFlags(Simulator<T>* simulator, const string& scheme, const T& max_step_size) {
    ...
    throw std::runtime_error(fmt::format("Unknown integration scheme: {}", scheme));
}
...
template <typename T>
void ApplySimulatorConfig(const SimulatorConfig& config, Simulator<T>* simulator) {
    DRAKE_THROW_UNLESS(simulator != nullptr);
    IntegratorBase<T>& integrator =
            ResetIntegratorFromFlags(simulator, config.integration_scheme, T(config.max_step_size));
...
SimulatorConfig ExtractSimulatorConfig(const Simulator<T>& simulator) {
    ...
    result.max_step_size = ExtractDoubleOrThrow(integrator.get_maximum_step_size());
    if (integrator.supports_error_estimation()) {
        result.use_error_control = !integrator.get_fixed_step_mode();
        const double accuracy_in_use = ExtractDoubleOrThrow(integrator.get_accuracy_in_use());
    ...
    result.target_realtime_rate = ExtractDoubleOrThrow(simulator.get_target_realtime_rate());
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&ResetIntegratorFromFlags<T>,
