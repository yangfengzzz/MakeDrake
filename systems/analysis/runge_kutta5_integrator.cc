#include "systems/analysis/runge_kutta5_integrator.h"
...
bool RungeKutta5Integrator<T>::DoStep(const T& h) {
    ...
    /*
     ...
     8/9  (c5) | 19372/6561 (a51)   −25360/2187 (a52)   64448/6561 (a53)   −212/729 (a54) // NOLINT(*) 1    (c6) |
     9017/3168 (a61)       −355/33 (a62)   46732/5247 (a63)     49/176 (a64)    −5103/18656 (a65) // NOLINT(*) 1    (c7)
     |     35/384 (a71)             0 (a72)     500/1113 (a73)    125/192 (a74)     −2187/6784 (a75)     11/84 (a76) //
     NOLINT(*)
     ------------------------------------------------------------------------------------------------------------------------------------
     // NOLINT(*) 35/384  (b1)             0 (b2)      500/1113 (b3)     125/192 (b4)      −2187/6784 (b5)      11/84
     (b6)      0 (b7)  // NOLINT(*) 5179/57600  (d1)             0 (d2)    7571/16695 (d3)     393/640 (d4)
     −92097/339200 (d5)   187/2100 (d6)   1/40 (d7)  // NOLINT(*)
    */
    ...
    VectorBase<T>& xc = context.SetTimeAndGetMutableContinuousStateVector(t0 + c2 * h);
    xc.PlusEqScaled(a21 * h, k1);

    // Evaluate the derivative (denoted k2) at t₀ + c2 * h, xc₀ + a21 * h * k1.
    derivs2_->get_mutable_vector().SetFrom(this->EvalTimeDerivatives(context).get_vector());
    ...
    derivs3_->get_mutable_vector().SetFrom(this->EvalTimeDerivatives(context).get_vector());
    ...
    derivs4_->get_mutable_vector().SetFrom(this->EvalTimeDerivatives(context).get_vector());
    ...
    derivs5_->get_mutable_vector().SetFrom(this->EvalTimeDerivatives(context).get_vector());
    ...
    //       a65 * h * k5.
    xc.SetFromVector(save_xc0_);
    xc.PlusEqScaled({{a61 * h, k1}, {a62 * h, k2}, {a63 * h, k3}, {a64 * h, k4}, {a65 * h, k5}});
    derivs6_->get_mutable_vector().SetFrom(this->EvalTimeDerivatives(context).get_vector());
    ...
    //       a75 * h * k5 + a76 * h * k6.
    xc.SetFromVector(save_xc0_);
    xc.PlusEqScaled({{a71 * h, k1}, {a73 * h, k3}, {a74 * h, k4}, {a75 * h, k5}, {a76 * h, k6}});
    ...
    this->get_mutable_error_estimate()->SetFromVector(err_est_vec_->CopyToVector().cwiseAbs());
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class drake::systems::RungeKutta5Integrator);
