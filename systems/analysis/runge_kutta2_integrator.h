...
public:
     ...
     * @sa Initialize()
     */
    RungeKutta2Integrator(const System<T>& system, const T& max_step_size, Context<T>* context = nullptr)
...
bool RungeKutta2Integrator<T>::DoStep(const T& h) {
    ...
    // cache entry.
    VectorBase<T>& xc =
            context->SetTimeAndGetMutableContinuousStateVector(context->get_time() + h);  // t⁽ᵃ⁾ ← t₁ = t₀ + h
    xc.PlusEqScaled(h, xcdot0);  // xc⁽ᵃ⁾ ← xc₀ + h * xcdot₀

    // Evaluate derivative xcdot⁽ᵃ⁾ ← xcdot(t⁽ᵃ⁾, x⁽ᵃ⁾, u⁽ᵃ⁾).
    const VectorBase<T>& xcdot_a = this->EvalTimeDerivatives(*context).get_vector();
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::systems::RungeKutta2Integrator);
