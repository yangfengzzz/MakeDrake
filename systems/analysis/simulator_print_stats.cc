#include "systems/analysis/simulator_print_stats.h"
...
void PrintSimulatorStatistics(const Simulator<T>& simulator) {
    ...
    std::string integrator_scheme_name = NiceTypeName::RemoveNamespaces(NiceTypeName::Get(integrator));
    ...
    if constexpr (std::is_same_v<T, double>) {
        integrator_scheme_name = std::regex_replace(integrator_scheme_name, std::regex("<double>"), "");
    }
    ...
    if (!integrator.get_fixed_step_mode()) {
        ...
        fmt::print("Smallest adapted step size = {:10.6g} s\n",
                   ExtractDoubleOrThrow(integrator.get_smallest_adapted_step_size_taken()));
    ...
    const systems::ImplicitIntegrator<T>* implicit_integrator =
            dynamic_cast<const systems::ImplicitIntegrator<T>*>(&(simulator.get_integrator()));
    ...
    if (integrator_is_implicit) {
        ...
        if (integrator.supports_error_estimation()) {
            ...
            fmt::print(
                    ...
                    implicit_integrator->get_num_error_estimator_derivative_evaluations_for_jacobian(),
                    implicit_integrator->get_num_derivative_evaluations_for_jacobian());
            fmt::print("Number of Iteration Matrix Factorizations = {:d}, {:d}, {:d}\n",
            ...
            fmt::print("Number of Derivative Evaluations for Jacobians = {:d}\n",
                       implicit_integrator->get_num_derivative_evaluations_for_jacobian());
            fmt::print("Number of Iteration Matrix Factorizations = {:d}\n",
                       implicit_integrator->get_num_iteration_matrix_factorizations());
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS((&PrintSimulatorStatistics<T>));
}  // namespace systems
}  // namespace drake
