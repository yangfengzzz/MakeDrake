...
template <typename T>
ScalarInitialValueProblem<T>::ScalarInitialValueProblem(const ScalarOdeFunction& scalar_ode_function,
                                                        const T& x0,
    ...
    typename InitialValueProblem<T>::OdeFunction ode_function =
            [scalar_ode_function](const T& t, const VectorX<T>& x, const VectorX<T>& params) -> VectorX<T> {
        return VectorX<T>::Constant(1, scalar_ode_function(t, x[0], params));
    };
    // Instantiates the vector initial value problem.
    vector_ivp_ = std::make_unique<InitialValueProblem<T>>(ode_function, Vector1<T>{x0}, k);
}
...
template <typename T>
std::unique_ptr<ScalarDenseOutput<T>> ScalarInitialValueProblem<T>::DenseSolve(const T& t0, const T& tf) const {
    ...
    std::unique_ptr<DenseOutput<T>> vector_dense_output = this->vector_ivp_->DenseSolve(t0, tf);
