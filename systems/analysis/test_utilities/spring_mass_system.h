...
public:
    ...
    T get_position(const Context<T>& context) const { return get_state(context).get_position(); }

    /// Gets the current velocity of the mass in the given Context.
    T get_velocity(const Context<T>& context) const { return get_state(context).get_velocity(); }
    ...
    /// Context.
    T get_conservative_work(const Context<T>& context) const { return get_state(context).get_conservative_work(); }
    ...
    void GetClosedFormSolution(const T& x0, const T& v0, const T& tf, T* xf, T* vf) const {
        ...
        if (!xf || !vf) throw std::logic_error("Passed final position/velocity is null.");
        if (system_is_forced_)
            throw std::logic_error(
                    "Can only compute closed form solution on "
                    "unforced system");
...
private:
    ...
    void SetOutputValues(const Context<T>& context, SpringMassStateVector<T>* output) const;

    // TODO(david-german-tri): Add a cast that is dynamic_cast in Debug mode,
    // and static_cast in Release mode.

    static const SpringMassStateVector<T>& get_state(const ContinuousState<T>& cstate) {
        return dynamic_cast<const SpringMassStateVector<T>&>(cstate.get_vector());
    }

    static SpringMassStateVector<T>& get_mutable_state(ContinuousState<T>* cstate) {
        return dynamic_cast<SpringMassStateVector<T>&>(cstate->get_mutable_vector());
    }
