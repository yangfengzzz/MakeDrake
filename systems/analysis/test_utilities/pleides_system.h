...
public:
    ...
    void SetDefaultState(const Context<double>& context, State<double>* state) const override {
        ...
        q.head(kNumParticles)[0] = 3.0;
        q.tail(kNumParticles)[0] = 3.0;
        q.head(kNumParticles)[1] = 3.0;
        q.tail(kNumParticles)[1] = -3.0;
        q.head(kNumParticles)[2] = -1.0;
        q.tail(kNumParticles)[2] = 2.0;
        q.head(kNumParticles)[3] = -3.0;
        q.tail(kNumParticles)[3] = 0.0;
        q.head(kNumParticles)[4] = 2.0;
        q.tail(kNumParticles)[4] = 0.0;
        q.head(kNumParticles)[5] = -2.0;
        q.tail(kNumParticles)[5] = -4.0;
        q.head(kNumParticles)[6] = 2.0;
        q.tail(kNumParticles)[6] = 4.0;

        // Set the initial velocities. Uses the same representation layout as with
        // the positions.
        v.head(kNumParticles)[0] = 0.0;
        v.tail(kNumParticles)[0] = 0.0;
        v.head(kNumParticles)[1] = 0.0;
        v.tail(kNumParticles)[1] = 0.0;
        v.head(kNumParticles)[2] = 0.0;
        v.tail(kNumParticles)[2] = 0.0;
        v.head(kNumParticles)[3] = 0.0;
        v.tail(kNumParticles)[3] = -1.25;
        v.head(kNumParticles)[4] = 0.0;
        v.tail(kNumParticles)[4] = 1.0;
        v.head(kNumParticles)[5] = 1.75;
        v.tail(kNumParticles)[5] = 0.0;
        v.head(kNumParticles)[6] = -1.5;
        v.tail(kNumParticles)[6] = 0.0;

        state->get_mutable_continuous_state().get_mutable_generalized_position().SetFromVector(q);
        state->get_mutable_continuous_state().get_mutable_generalized_velocity().SetFromVector(v);
    }

    void DoCalcTimeDerivatives(const Context<double>& context, ContinuousState<double>* deriv) const override {
        const VectorBase<double>& q = context.get_continuous_state().get_generalized_position();
        ...
        for (int i = 0; i < kNumParticles; ++i) {
            ...
            for (int j = 0; j < kNumParticles; ++j) {
                ...
                Fi += G() * (mass_[i] * mass_[j]) * rij / (distance * distance * distance);
            }
