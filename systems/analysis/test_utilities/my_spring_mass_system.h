...
public:
    ...
    MySpringMassSystem(double stiffness, double mass, double update_rate)
        ...
        if (update_rate > 0.0) {
            this->DeclarePeriodicDiscreteUpdateEvent(1.0 / update_rate, 0.0, &MySpringMassSystem::CountDiscreteUpdates);
        }
    }
...
private:
    ...
    EventStatus CountDiscreteUpdates(const Context<T>&, DiscreteValues<T>*) const {
        ++update_count_;
        return EventStatus::Succeeded();
    }
