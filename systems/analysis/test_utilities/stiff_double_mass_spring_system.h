...
public:
    ...
    StiffDoubleMassSpringSystem() { this->DeclareContinuousState(2 /* num_q */, 2 /* num_v */, 0 /* num_z */); }
    ...
    Eigen::Vector2d get_spring_constants() const { return Eigen::Vector2d(750, 1e20); }

    /// Gets the two damping constants.
    Eigen::Vector2d get_damping_constants() const { return Eigen::Vector2d(0, 0); }
    ...
    void DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* deriv) const override {
        // Get velocity.
        const VectorBase<T>& xd = context.get_continuous_state().get_generalized_velocity();
    ...
    void SetDefaultState(const Context<T>&, State<T>* state) const override {
        ...
        state->get_mutable_continuous_state().get_mutable_generalized_position().SetFromVector(x);
        state->get_mutable_continuous_state().get_mutable_generalized_velocity().SetFromVector(xd);
    }
    ...
    void GetSolution(const Context<T>& context, const T& t, ContinuousState<T>* state) const {
        const Eigen::Vector2d b = get_damping_constants();
        ...
        const T offset =
                state->get_generalized_position().GetAtIndex(1) - state->get_generalized_position().GetAtIndex(0);

        // Omega will use the first body (the one connected to the "world" with the
        // non-stiff spring).
        const double omega = std::sqrt(get_spring_constants()(0) / (get_mass()(0) + get_mass()(1)));

        // Setup c1 and c2 for ODE constants.
        const double c1 = context.get_continuous_state().get_generalized_position().GetAtIndex(0);
        const double c2 = context.get_continuous_state().get_generalized_velocity().GetAtIndex(0) / omega;
        ...
        const double v1_final = c1 * -sin(omega * t) * omega + c2 * +cos(omega * t) * omega;
