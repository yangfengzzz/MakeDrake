...
PidControlledSpringMassSystem<T>::PidControlledSpringMassSystem(
        double spring_stiffness, double mass, double Kp, double Ki, double Kd, const T& target_position)
    : Diagram<T>() {
    ...
    plant_ = builder.template AddSystem<SpringMassSystem>(spring_stiffness, mass, true /* is forced */);
    plant_->set_name("plant");
    controller_ = builder.template AddSystem<controllers::PidController>(
            VectorX<double>::Constant(1, Kp), VectorX<double>::Constant(1, Ki), VectorX<double>::Constant(1, Kd));
    ...
    builder.Connect(mux->get_output_port(0), controller_->get_input_port_estimated_state());

    // Connects the desired state to PID.
    builder.Connect(target_->get_output_port(), controller_->get_input_port_desired_state());

    // Closes the feedback loop.
    builder.Connect(controller_->get_output_port_control(), plant_->get_force_port());
...
template <typename T>
T PidControlledSpringMassSystem<T>::get_position(const Context<T>& context) const {
    const Context<T>& plant_context = Diagram<T>::GetSubsystemContext(*plant_, context);
    return plant_->get_position(plant_context);
}

template <typename T>
T PidControlledSpringMassSystem<T>::get_velocity(const Context<T>& context) const {
    const Context<T>& plant_context = Diagram<T>::GetSubsystemContext(*plant_, context);
    return plant_->get_velocity(plant_context);
}

template <typename T>
T PidControlledSpringMassSystem<T>::get_conservative_work(const Context<T>& context) const {
    const Context<T>& plant_context = Diagram<T>::GetSubsystemContext(*plant_, context);
    return plant_->get_conservative_work(plant_context);
}

template <typename T>
void PidControlledSpringMassSystem<T>::set_position(Context<T>* context, const T& position) const {
    Context<T>& plant_context = Diagram<T>::GetMutableSubsystemContext(*plant_, context);
    plant_->set_position(&plant_context, position);
}

template <typename T>
void PidControlledSpringMassSystem<T>::set_velocity(Context<T>* context, const T& velocity) const {
    Context<T>& plant_context = Diagram<T>::GetMutableSubsystemContext(*plant_, context);
