...
template <typename T>
SpringMassStateVector<T>::SpringMassStateVector(const T& initial_position, const T& initial_velocity)
...
template <typename T>
SpringMassStateVector<T>::SpringMassStateVector() : SpringMassStateVector(0.0, 0.0) {}
...
SpringMassSystem<T>::SpringMassSystem(SystemScalarConverter converter,
    ...
    this->DeclareVectorOutputPort(kUseDefaultName, SpringMassStateVector<T>(), &SpringMassSystem::SetOutputValues);

    this->DeclareContinuousState(SpringMassStateVector<T>(), 1 /* num_q */, 1 /* num_v */, 1 /* num_z */);
}

template <typename T>
SpringMassSystem<T>::SpringMassSystem(double spring_constant_N_per_m, double mass_kg, bool system_is_forced)
    : SpringMassSystem(SystemTypeTag<SpringMassSystem>{}, spring_constant_N_per_m, mass_kg, system_is_forced) {}

template <typename T>
template <typename U>
SpringMassSystem<T>::SpringMassSystem(const SpringMassSystem<U>& other)
    : SpringMassSystem(other.get_spring_constant(), other.get_mass(), other.get_system_is_forced()) {}
...
template <typename T>
T SpringMassSystem<T>::DoCalcConservativePower(const Context<T>& context) const {
...
template <typename T>
void SpringMassSystem<T>::SetOutputValues(const Context<T>& context, SpringMassStateVector<T>* output_vector) const {
...
template <typename T>
void SpringMassSystem<T>::DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::SpringMassStateVector);

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::SpringMassSystem);
