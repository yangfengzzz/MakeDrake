...
public:
    ...
    SpringMassDamperSystem(double spring_constant_N_per_m, double damping_constant_Ns_per_m, double mass_kg)
    ...
    explicit SpringMassDamperSystem(const SpringMassDamperSystem<U>& other)
        : SpringMassDamperSystem(other.get_spring_constant(), other.get_damping_constant(), other.get_mass()) {}
    ...
    void GetClosedFormSolution(const T& x0, const T& v0, const T& tf, T* xf, T* vf) const {
        ...
        if (get_damping_constant() * get_damping_constant() < 4 * this->get_mass() * this->get_spring_constant()) {
            throw std::logic_error(
                    "Closed form solution not available for "
                    "underdamped system.");
        }
        ...
        T r, s;
        std::tie(r, s) =
                SolveRestrictedQuadratic(this->get_mass(), get_damping_constant(), this->get_spring_constant());
...
protected:
    ...
    SpringMassDamperSystem(SystemScalarConverter converter,
                           ...
                           double mass_kg)
        : SpringMassSystem<T>(std::move(converter), spring_constant_N_per_m, mass_kg, false /* unforced */),
...
private:
    ...
    static std::pair<T, T> SolveRestrictedQuadratic(const T& a, const T& b, const T& c) {
        using std::abs;
        using std::sqrt;
