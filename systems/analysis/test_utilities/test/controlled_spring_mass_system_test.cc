...
protected:
    void SetUp() override {
        model_ = make_unique<PidControlledSpringMassSystem<double>>(
                kSpring, kMass, kProportionalConstant, kIntegralConstant, kDerivativeConstant, kTargetPosition);

        model_context_ = model_->CreateDefaultContext();
        output_ = model_->AllocateOutput();

        // Gets the plant subcontext.
        plant_context_ = &model_->GetMutableSubsystemContext(model_->get_plant(), model_context_.get());
    }
...
TEST_F(SpringMassSystemTest, EvalTimeDerivatives) {
    ...
    std::unique_ptr<ContinuousState<double>> derivatives = model_->AllocateTimeDerivatives();
    ...
    const ContinuousState<double>& plant_xcdot = model_->GetSubsystemDerivatives(model_->get_plant(), *derivatives);
    ...
    const double pid_actuation = kProportionalConstant * error + kDerivativeConstant * error_rate;
    EXPECT_EQ((-kSpring * x0 - pid_actuation) / kMass, plant_xcdot.get_vector().GetAtIndex(1));

    // Power.
    EXPECT_EQ(model_->get_plant().EvalConservativePower(*plant_context_), plant_xcdot.get_vector().GetAtIndex(2));
}
