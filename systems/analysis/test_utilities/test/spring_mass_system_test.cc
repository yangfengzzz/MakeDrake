...
public:
    ...
    void Initialize(bool with_input_force = false) {
        ...
        system_ = std::make_unique<SpringMassSystem<double>>(kSpring, kMass, with_input_force);
        ...
        state_ = dynamic_cast<SpringMassStateVector<double>*>(&context_->get_mutable_continuous_state_vector());
        output_ = dynamic_cast<const SpringMassStateVector<double>*>(system_output_->get_vector_data(0));
        derivatives_ = dynamic_cast<SpringMassStateVector<double>*>(&system_derivatives_->get_mutable_vector());
    }
...
TEST_F(SpringMassSystemTest, CloneState) {
    ...
    SpringMassStateVector<double>* typed_clone = dynamic_cast<SpringMassStateVector<double>*>(clone.get());
...
TEST_F(SpringMassSystemTest, SecondOrderStructure) {
    ...
    ContinuousState<double>& continuous_state = context_->get_mutable_continuous_state();
    ...
    EXPECT_NEAR(1.2, continuous_state.get_generalized_position().GetAtIndex(0), 1e-8);
    EXPECT_NEAR(3.4, continuous_state.get_generalized_velocity().GetAtIndex(0), 1e-8);
}
...
TEST_F(SpringMassSystemTest, MapVelocityToConfigurationDerivative) {
    ...
    system_->MapVelocityToQDot(*context_, continuous_state.get_generalized_velocity(), &configuration_derivatives);
...
TEST_F(SpringMassSystemTest, DynamicsWithExternalForce) {
    ...
    ASSERT_EQ(3, derivatives_->size());
    // The derivative of position is velocity.
    EXPECT_NEAR(0.1, derivatives_->get_position(), Eigen::NumTraits<double>::epsilon());
    // The derivative of velocity is force over mass.
    EXPECT_NEAR((-kSpring * 0.1 + kExternalForce) / kMass, derivatives_->get_velocity(),
...
MatrixX<double> CalcDxdotDx(const System<double>& system, const Context<double>& context) {
...
/* Semi-explicit Euler (neutrally stable):
    ...
    v1 = v0 + h vdot(t0,x0)
    q1 = q0 + h qdot(q0,v1) */
void StepSemiExplicitEuler(double h,
...
void StepImplicitEuler(double h,
                       const System<double>& system,
                       // TODO(#2274) Fix NOLINTNEXTLINE(runtime/references).
                       ContinuousState<double>& derivs,  // in/out
                       // TODO(#2274) Fix NOLINTNEXTLINE(runtime/references).
...
double CalcEnergy(const SpringMassSystem<double>& system, const Context<double>& context) {
    return system.CalcPotentialEnergy(context) + system.CalcKineticEnergy(context);
}
...
TEST_F(SpringMassSystemTest, IntegrateConservativePower) {
    ...
    unique_ptr<ContinuousState<double>> derivs = system_->AllocateTimeDerivatives();
