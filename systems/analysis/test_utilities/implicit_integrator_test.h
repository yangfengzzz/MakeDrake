...
#include "systems/analysis/implicit_integrator.h"
#include "systems/analysis/test_utilities/discontinuous_spring_mass_damper_system.h"
#include "systems/analysis/test_utilities/linear_scalar_system.h"
#include "systems/analysis/test_utilities/robertson_system.h"
#include "systems/analysis/test_utilities/spring_mass_damper_system.h"
#include "systems/analysis/test_utilities/spring_mass_system.h"
#include "systems/analysis/test_utilities/stationary_system.h"
#include "systems/analysis/test_utilities/stiff_double_mass_spring_system.h"
...
public:
    ImplicitIntegratorTest() {
        ...
        spring_mass_ = std::make_unique<SpringMassSystem<double>>(spring_k_, mass_, false /* no forcing */);

        spring_mass_damper_ = std::make_unique<implicit_integrator_test::SpringMassDamperSystem<double>>(
                stiff_spring_k_, stiff_damping_b_, mass_);

        // The discontinuous spring-mass-damper is critically damped.
        mod_spring_mass_damper_ =
                std::make_unique<implicit_integrator_test::DiscontinuousSpringMassDamperSystem<double>>(
                        semistiff_spring_k_, std::sqrt(semistiff_spring_k_ / mass_), mass_, constant_force_mag_);

        stiff_double_system_ = std::make_unique<analysis::test::StiffDoubleMassSpringSystem<double>>();
        ...
        mod_spring_mass_damper_context_ = mod_spring_mass_damper_->CreateDefaultContext();
    ...
    void DoubleSpringMassDamperTest(ReuseType type) {
        ...
        stiff_double_system_->GetSolution(*dspring_context_, t_final, &state_copy->get_mutable_continuous_state());
        ...
        const VectorX<double> nsol = dspring_context_->get_continuous_state().get_generalized_position().CopyToVector();
        const VectorX<double> sol = state_copy->get_continuous_state().get_generalized_position().CopyToVector();

        for (int i = 0; i < nsol.size(); ++i) EXPECT_NEAR(sol(i), nsol(i), sol_tol_pos);

        // Check the velocity solution.
        const VectorX<double> nsolv =
                dspring_context_->get_continuous_state().get_generalized_velocity().CopyToVector();
        const VectorX<double> solv = state_copy->get_continuous_state().get_generalized_velocity().CopyToVector();
        for (int i = 0; i < nsolv.size(); ++i) EXPECT_NEAR(solv(i), nsolv(i), sol_tol_vel);
    ...
    void SpringMassDamperStiffTest(ReuseType type) {
        ...
        IntegratorType integrator(*spring_mass_damper_, spring_mass_damper_context_.get());
        ...
        spring_mass_damper_->set_position(spring_mass_damper_context_.get(), initial_position);
        spring_mass_damper_->set_velocity(spring_mass_damper_context_.get(), initial_velocity);
        ...
        const VectorBase<double>& xc_final = spring_mass_damper_context_->get_continuous_state().get_vector();
        ...
        spring_mass_damper_->GetClosedFormSolution(initial_position, initial_velocity, t_final, &x_final_true,
                                                   &v_final_true);
        ...
        integrator.set_jacobian_computation_scheme(IntegratorType::JacobianComputationScheme::kCentralDifference);
        ...
        integrator.set_jacobian_computation_scheme(IntegratorType::JacobianComputationScheme::kAutomatic);
    ...
    void DiscontinuousSpringMassDamperTest(ReuseType type) {
        // Create the integrator.
        IntegratorType integrator(*mod_spring_mass_damper_, mod_spring_mass_damper_context_.get());
        integrator.set_maximum_step_size(h_);
        integrator.set_throw_on_minimum_step_size_violation(false);
        if (integrator.supports_error_estimation()) {
            integrator.set_target_accuracy(1e-5);
        ...
        // Set initial condition.
        mod_spring_mass_damper_->set_position(mod_spring_mass_damper_context_.get(), initial_position);
        mod_spring_mass_damper_->set_velocity(mod_spring_mass_damper_context_.get(), initial_velocity);
        ...
        // Get the final position and velocity.
        double x_final = mod_spring_mass_damper_context_->get_continuous_state().get_vector().GetAtIndex(0);
        double xdot_final = mod_spring_mass_damper_context_->get_continuous_state().get_vector().GetAtIndex(1);
        ...
        // opposite).
        const double equilibrium_position = -constant_force_magnitude() / semistiff_spring_stiffness();
        ...
        x_final = mod_spring_mass_damper_context_->get_continuous_state().get_vector().GetAtIndex(0);
        xdot_final = mod_spring_mass_damper_context_->get_continuous_state().get_vector().GetAtIndex(1);
        EXPECT_NEAR(mod_spring_mass_damper_context_->get_time(), t_final, ttol);
        EXPECT_NEAR(equilibrium_position, x_final, sol_tol);
        EXPECT_NEAR(equilibrium_velocity, xdot_final, sol_tol);
        CheckGeneralStatsValidity(&integrator);
    ...
    void SpringMassStepTest(ReuseType type) {
        ...
        double x_final_true, v_final_true;
        spring_mass.GetClosedFormSolution(initial_position, initial_velocity, t_final, &x_final_true, &v_final_true);
    ...
    void ErrorEstimationTest(ReuseType type) {
        ...
        for (int j = 0; j < n_h; ++j) {
            for (int i = 0; i < n_initial_conditions; ++i) {
                ...
                ASSERT_TRUE(integrator.IntegrateWithSingleFixedStepToTime(context->get_time() + h[j]));

                // Check the time.
                EXPECT_NEAR(context->get_time(), h[j], ttol);

                // Get the error estimate.
                const double est_err = std::abs(integrator.get_error_estimate()->CopyToVector()[0]);

                // Get the final position of the spring.
                const double x_final = context->get_continuous_state().get_vector().GetAtIndex(0);

                // Get the true position.
                double x_final_true, v_final_true;
                spring_mass.GetClosedFormSolution(initial_position[i], initial_velocity[i], h[j], &x_final_true,
                                                  &v_final_true);
    ...
    void SpringMassStepAccuracyEffectsTest(ReuseType type) {
        ...
        // Take all the defaults.
        integrator.Initialize();
        EXPECT_NEAR(integrator.get_accuracy_in_use(), 1e-4, std::numeric_limits<double>::epsilon());

        // Get the actual solution.
        double x_final_true, v_final_true;
        spring_mass.GetClosedFormSolution(initial_position, initial_velocity, large_h_, &x_final_true, &v_final_true);

        // Integrate exactly one step.
        integrator.IntegrateWithMultipleStepsToTime(context->get_time() + large_h_);

        // Get the positional error.
        const double pos_err = std::abs(x_final_true - context->get_continuous_state_vector().GetAtIndex(0));
        ...
        EXPECT_NEAR(integrator.get_accuracy_in_use(), 100.0, std::numeric_limits<double>::epsilon());
        integrator.Initialize();
        context->SetTime(0);
        spring_mass.set_position(context.get(), initial_position);
        spring_mass.set_velocity(context.get(), initial_velocity);
        integrator.IntegrateWithMultipleStepsToTime(context->get_time() + large_h_);
        EXPECT_GT(std::abs(x_final_true - context->get_continuous_state_vector().GetAtIndex(0)), pos_err);
    }
    ...
    static void CheckGeneralStatsValidity(IntegratorType* integrator) {
        ...
        if (integrator->supports_error_estimation()) {
            EXPECT_GE(integrator->get_num_error_estimator_derivative_evaluations_for_jacobian(), 0);
        }
        ...
        if (integrator->supports_error_estimation()) {
            EXPECT_GE(integrator->get_num_error_estimator_iteration_matrix_factorizations(), 0);
        }
...
TYPED_TEST_P(ImplicitIntegratorTest, FullNewton) {
    ...
    EXPECT_EQ(integrator.get_num_iteration_matrix_factorizations(), integrator.get_num_newton_raphson_iterations());
    EXPECT_EQ(integrator.get_num_jacobian_evaluations(), integrator.get_num_newton_raphson_iterations());
...
TYPED_TEST_P(ImplicitIntegratorTest, Stationary) {
    ...
    VectorBase<double>& state = context->get_mutable_continuous_state().get_mutable_vector();
    ...
    // scheme.
    integrator.set_jacobian_computation_scheme(Integrator::JacobianComputationScheme::kAutomatic);

    // Reset the time, position, and velocity.
    context->SetTime(0.0);
    VectorBase<double>& new_state = context->get_mutable_continuous_state().get_mutable_vector();
    ...
    // Verify the solution.
    EXPECT_NEAR(new_state.GetAtIndex(0), 0, std::numeric_limits<double>::epsilon());
    EXPECT_NEAR(new_state.GetAtIndex(1), 0, std::numeric_limits<double>::epsilon());
    ImplicitIntegratorTest<Integrator>::CheckGeneralStatsValidity(&integrator);
}
...
TYPED_TEST_P(ImplicitIntegratorTest, Robertson) {
    ...
    // Verify the solution.
    const VectorBase<double>& state = context->get_continuous_state().get_vector();
...
TYPED_TEST_P(ImplicitIntegratorTest, FixedStepThrowsOnMultiStep) {
    ...
    // large step.
    integrator.Initialize();
    EXPECT_FALSE(integrator.IntegrateWithSingleFixedStepToTime(context->get_time() + h));
}
...
TYPED_TEST_P(ImplicitIntegratorTest, ContextAccess) {
    ...
    EXPECT_THROW(integrator.Initialize(), std::logic_error);
    EXPECT_THROW(integrator.IntegrateNoFurtherThanTime(t_final, t_final, t_final), std::logic_error);
}
...
TYPED_TEST_P(ImplicitIntegratorTest, LinearTest) {
    ...
    if (integrator1.supports_error_estimation()) {
        ...
        // Verify the solution.
        VectorX<double> state = linear_context->get_continuous_state().get_vector().CopyToVector();
        EXPECT_NEAR(state[0], linear.Evaluate(t_final), std::numeric_limits<double>::epsilon());
        ...
        const double updated_err_est = integrator2.get_error_estimate()->get_vector()[0];
        ...
        EXPECT_NEAR(scaled_linear_context->get_continuous_state().get_vector().CopyToVector()[0],
                    scaled_linear.Evaluate(updated_t_final), 10 * std::numeric_limits<double>::epsilon());
    }
}
...
REGISTER_TYPED_TEST_SUITE_P(ImplicitIntegratorTest,
                            Reuse,
                            FullNewton,
                            MiscAPINoReuse,
                            MiscAPIReuse,
