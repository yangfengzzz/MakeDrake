...
public:
    ...
    StatelessSystem(double offset, const WitnessFunctionDirection& dir_type)
        ...
        witness_ = this->MakeWitnessFunction("clock witness", dir_type, &StatelessSystem::CalcClockWitness,
    ...
    explicit StatelessSystem(const StatelessSystem<U>& other)
        : StatelessSystem<T>(other.get_trigger_time(), other.witness_->direction_type()) {}

    void set_publish_callback(std::function<void(const Context<T>&)> callback) { publish_callback_ = callback; }

    /// Gets the time that the witness function triggered.
    double get_trigger_time() const { return offset_; }

protected:
    void DoGetWitnessFunctions(const Context<T>&, std::vector<const WitnessFunction<T>*>* w) const override {
...
private:
    ...
    T CalcClockWitness(const Context<T>& context) const { return context.get_time() - offset_; }
