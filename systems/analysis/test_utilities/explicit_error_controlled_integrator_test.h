...
#include "systems/analysis/test_utilities/pleides_system.h"
...
public:
    ExplicitErrorControlledIntegratorTest() {
        ...
        spring_mass = std::make_unique<analysis_test::MySpringMassSystem<double>>(kSpringK, kMass, 0.);
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, ErrorEstSupport) {
    ...
    DRAKE_EXPECT_NO_THROW(this->integrator->request_initial_step_size_target(this->kDt));
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, MagDisparity) {
    ...
    DRAKE_EXPECT_NO_THROW(this->integrator->IntegrateWithMultipleStepsToTime(1e-40));
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, Scaling) {
    ...
    // Test scaling
    EXPECT_EQ(this->integrator->get_mutable_generalized_state_weight_vector().size(), 1);
    EXPECT_EQ(this->integrator->get_mutable_generalized_state_weight_vector().template lpNorm<Eigen::Infinity>(), 1);
    EXPECT_EQ(this->integrator->get_misc_state_weight_vector().size(), 1);
    EXPECT_EQ(this->integrator->get_mutable_misc_state_weight_vector().template lpNorm<Eigen::Infinity>(), 1);
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, BulletProofSetup) {
    ...
    this->spring_mass->set_position(this->integrator->get_mutable_context(), initial_position);
    this->spring_mass->set_velocity(this->integrator->get_mutable_context(), initial_velocity);
    ...
    this->integrator->Initialize();
    EXPECT_LE(this->integrator->get_accuracy_in_use(), this->integrator->get_target_accuracy());
    ...
    const double x_final = this->context->get_continuous_state().get_vector().GetAtIndex(0);

    // Check the solution. We're not really looking for accuracy here, just
    // want to make sure that the value is finite.
    EXPECT_NEAR(c1 * std::cos(omega * t_final) + c2 * std::sin(omega * t_final), x_final, 1e0);
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, ErrEstOrder) {
    ...
    EXPECT_NEAR(this->context->get_time(), h, std::numeric_limits<double>::epsilon());

    // Get the true solution.
    const double x_true = c1 * std::cos(omega * h) + c2 * std::sin(omega * h);

    // Get the integrator's solution.
    const double x_approx_h = this->context->get_continuous_state_vector().GetAtIndex(0);

    // Get the error estimate and the error in the error estimate.
    const double err_est_h = this->integrator->get_error_estimate()->get_vector().GetAtIndex(0);
    ...
    this->spring_mass->set_velocity(this->integrator->get_mutable_context(), initial_velocity);
    this->integrator->Initialize();
    ASSERT_TRUE(this->integrator->IntegrateWithSingleFixedStepToTime(t_final / 2.0));
    ASSERT_TRUE(this->integrator->IntegrateWithSingleFixedStepToTime(t_final));
    EXPECT_NEAR(this->context->get_time(), h, std::numeric_limits<double>::epsilon());
    const double x_approx_2h_h = this->context->get_continuous_state_vector().GetAtIndex(0);
    const double err_est_2h_h = this->integrator->get_error_estimate()->get_vector().GetAtIndex(0);
    const double err_est_2h_h_err = std::abs(err_est_2h_h - (x_true - x_approx_2h_h));
    ...
    EXPECT_GE(K * err_est_2h_h_err, err_est_h_err / std::pow(2.0, err_est_order + 1));
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, SpringMassStepEC) {
    ...
    // Step for 1 second.
    const double t_final = 1.0;
    for (double t = this->kDt; t <= t_final; t += this->kDt) this->integrator->IntegrateNoFurtherThanTime(t, t, t);
    ...
    // Check the solution.
    EXPECT_NEAR(c1 * std::cos(omega * t_final) + c2 * std::sin(omega * t_final), x_final, 1e-5);
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, StepToCurrentTimeNoOp) {
    ...
    const double t_final = 1.0;
    while (this->context->get_time() < t_final) this->integrator->IntegrateNoFurtherThanTime(t_final, t_final, t_final);
    ASSERT_EQ(this->context->get_time(), t_final);

    // Get the final state.
    const VectorX<double> x_final = this->context->get_continuous_state_vector().CopyToVector();
    ...
    for (int i = 0; i < x_final.size(); ++i) EXPECT_EQ(x_final[i], this->context->get_continuous_state_vector()[i]);
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, MaxStepSizeRespected) {
    ...
    do {
        ...
        this->integrator->IntegrateNoFurtherThanTime(inf, this->context->get_time() + max_step_size + eps, t_final);
    } while (this->context->get_time() < t_final);

    // Verify the statistics.
    EXPECT_LE(this->integrator->get_largest_step_size_taken(), max_step_size * this->integrator->get_stretch_factor());
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, IllegalFixedStep) {
    ...
    ASSERT_EQ(this->context->get_time(), 0.0);
    EXPECT_THROW(unused(this->integrator->IntegrateWithSingleFixedStepToTime(1e-8)), std::logic_error);
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, CheckStat) {
    ...
    EXPECT_LE(this->integrator->get_smallest_adapted_step_size_taken(), this->kDt);
}
...
TYPED_TEST_P(ExplicitErrorControlledIntegratorTest, DenseOutput) {
    ...
    const std::unique_ptr<trajectories::PiecewisePolynomial<double>> dense_output =
            this->integrator->StopDenseIntegration();

    // Check the dense output.
    EXPECT_EQ(dense_output->start_time(), 0.0);
    EXPECT_EQ(dense_output->end_time(), 1.0);
    EXPECT_EQ(dense_output->get_number_of_segments(), this->integrator->get_num_steps_taken());
}
...
TYPED_TEST_P(PleidesTest, Pleides) {
    ...
    // Check the result.
    const VectorX<double> q = this->context->get_continuous_state().get_generalized_position().CopyToVector();
    const VectorX<double> q_des = analysis::test::PleidesSystem::GetSolution(this->context->get_time());
    for (int i = 0; i < q.size(); ++i) EXPECT_NEAR(q[i], q_des[i], kTolerance) << i;
}
