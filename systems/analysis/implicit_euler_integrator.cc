...
void ImplicitEulerIntegrator<T>::DoInitialize() {
    ...
    rk2_ = std::make_unique<RungeKutta2Integrator<T>>(this->get_system(),
...
void ImplicitEulerIntegrator<T>::ComputeAndFactorImplicitEulerIterationMatrix(
    ...
    iteration_matrix->SetAndFactorIterationMatrix(J * -h + MatrixX<T>::Identity(n, n));
}

template <class T>
void ImplicitEulerIntegrator<T>::ComputeAndFactorImplicitTrapezoidIterationMatrix(
        const MatrixX<T>& J, const T& h, typename ImplicitIntegrator<T>::IterationMatrix* iteration_matrix) {
    const int n = J.rows();
    // TODO(edrumwri) Investigate using a move-type operation below.
    // We form the iteration matrix in this particular way to avoid an O(n^2)
    ...
    iteration_matrix->SetAndFactorIterationMatrix(J * (-h / 2.0) + MatrixX<T>::Identity(n, n));
}
...
bool ImplicitEulerIntegrator<T>::StepAbstract(
    ...
    DRAKE_LOGGER_DEBUG("StepAbstract() entered for t={}, h={}, trial={}", t0, h, trial);
    ...
    *xtplus = xtplus_guess;
    ...
    if (!this->get_use_full_newton() &&
        !this->MaybeFreshenMatrices(t0, xt0, h, trial, compute_and_factor_iteration_matrix, iteration_matrix)) {
        return false;
    }

    // Do the Newton-Raphson iterations.
    for (int i = 0; i < this->max_newton_raphson_iterations(); ++i) {
        this->FreshenMatricesIfFullNewton(tf, *xtplus, h, compute_and_factor_iteration_matrix, iteration_matrix);
    ...
    return StepAbstract(t0, h, xt0, g, compute_and_factor_iteration_matrix, xtplus_guess, iteration_matrix, xtplus,
                        trial + 1);
}

template <class T>
bool ImplicitEulerIntegrator<T>::StepImplicitEuler(const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus) {
...
bool ImplicitEulerIntegrator<T>::StepImplicitEulerWithGuess(
        const T& t0, const T& h, const VectorX<T>& xt0, const VectorX<T>& xtplus_guess, VectorX<T>* xtplus) {
    using std::abs;
    ...
    // Attempt the step.
    return StepAbstract(t0, h, xt0, g, ComputeAndFactorImplicitEulerIterationMatrix, xtplus_guess,
                        &ie_iteration_matrix_, &*xtplus);
}

template <class T>
bool ImplicitEulerIntegrator<T>::StepHalfSizedImplicitEulers(
        const T& t0, const T& h, const VectorX<T>& xt0, const VectorX<T>& xtplus_ie, VectorX<T>* xtplus) {
    using std::abs;
    ...
    const int stored_num_jacobian_evaluations = this->get_num_jacobian_evaluations();
    const int stored_num_iter_factorizations = this->get_num_iteration_matrix_factorizations();
    ...
    const int stored_num_nr_iterations = this->get_num_newton_raphson_iterations();
    ...
    if (!success) {
        ...
        success = StepImplicitEulerWithGuess(t0 + 0.5 * h, 0.5 * h, xthalf, xtplus_ie, xtplus);
        if (!success) {
    ...
    hie_statistics_.num_jacobian_reforms += this->get_num_jacobian_evaluations() - stored_num_jacobian_evaluations;
    ...
    hie_statistics_.num_nr_iterations += this->get_num_newton_raphson_iterations() - stored_num_nr_iterations;

    return success;
}

template <class T>
bool ImplicitEulerIntegrator<T>::StepImplicitTrapezoid(const T& t0,
                                                       const T& h,
                                                       const VectorX<T>& xt0,
                                                       const VectorX<T>& dx0,
    ...
    std::function<VectorX<T>()> g = [&xt0, h, &dx0, context, this]() {
        return (context->get_continuous_state().CopyToVector() - xt0 -
                h / 2 * (dx0 + this->EvalTimeDerivatives(*context).CopyToVector().eval()))
                .eval();
    };
    ...
    // Attempt to step.
    bool success = StepAbstract(t0, h, xt0, g, ComputeAndFactorImplicitTrapezoidIterationMatrix, xtplus_ie,
                                &itr_iteration_matrix_, xtplus);

    // Move statistics to implicit trapezoid-specific.
    // Notice that we log the statistics even if the step fails.
    itr_statistics_.num_jacobian_reforms += this->get_num_jacobian_evaluations() - stored_num_jacobian_evaluations;
    ...
    itr_statistics_.num_jacobian_function_evaluations +=
            this->get_num_derivative_evaluations_for_jacobian() - stored_num_jacobian_function_evaluations;
    itr_statistics_.num_nr_iterations += this->get_num_newton_raphson_iterations() - stored_num_nr_iterations;
...
bool ImplicitEulerIntegrator<T>::AttemptStepPaired(
        const T& t0, const T& h, const VectorX<T>& xt0, VectorX<T>* xtplus_ie, VectorX<T>* xtplus_hie) {
    using std::abs;
    ...
    const VectorX<T> dx0 = this->EvalTimeDerivatives(this->get_context()).CopyToVector();
...
bool ImplicitEulerIntegrator<T>::DoImplicitIntegratorStep(const T& h) {
    ...
    if (h < this->get_working_minimum_step_size()) {
        ...
        if (use_implicit_trapezoid_error_estimation_) {
            ...
            itr_statistics_.num_function_evaluations += (evals_after_rk2 - evals_before_rk2);
            ...
            xtplus_hie_ += 0.5 * h * this->EvalTimeDerivatives(*context).CopyToVector();
