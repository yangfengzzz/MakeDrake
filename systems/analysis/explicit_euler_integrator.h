...
public:
     ...
     * @sa Initialize()
     */
    ExplicitEulerIntegrator(const System<T>& system, const T& max_step_size, Context<T>* context = nullptr)
...
bool ExplicitEulerIntegrator<T>::DoStep(const T& h) {
    ...
    VectorBase<T>& xc = context.SetTimeAndGetMutableContinuousStateVector(context.get_time() + h);  // t ← t₀ + h
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class drake::systems::ExplicitEulerIntegrator);
