#include "systems/analysis/velocity_implicit_euler_integrator.h"
...
template <class T>
void VelocityImplicitEulerIntegrator<T>::ComputeAndFactorImplicitEulerIterationMatrix(
        const MatrixX<T>& J, const T& h, typename ImplicitIntegrator<T>::IterationMatrix* iteration_matrix) {
    const int n = J.rows();
    ...
    iteration_matrix->SetAndFactorIterationMatrix(-h * J + MatrixX<T>::Identity(n, n));
}

template <class T>
void VelocityImplicitEulerIntegrator<T>::CalcVelocityJacobian(
        const T& t, const T& h, const VectorX<T>& y, const VectorX<T>& qk, const VectorX<T>& qn, MatrixX<T>* Jy) {
    ...
    if (this->get_jacobian_computation_scheme() ==
        ...
        std::function<void(const VectorX<T>&, VectorX<T>*)> l_of_y = [&qk, &t, &qn, &h, this](const VectorX<T>& y_state,
                                                                                              VectorX<T>* l_result) {
            *l_result = this->ComputeLOfY(t, y_state, qk, qn, h, this->qdot_.get());
        };
    ...
    this->increment_jacobian_computation_derivative_evaluations(this->get_num_derivative_evaluations() -
                                                                existing_ODE_evals);
}
...
void VelocityImplicitEulerIntegrator<T>::ComputeAutoDiffVelocityJacobian(
    ...
    DRAKE_LOGGER_DEBUG(
            ...
            "{}-Jacobian t={}",
            y.size(), t);
    DRAKE_LOGGER_DEBUG("  computing from qk {}, y {}", fmt_eigen(qk.transpose()), fmt_eigen(y.transpose()));
    ...
    const VectorX<AutoDiffXd> result =
            this->ComputeLOfY(t, y_ad, qk, qn, h, this->qdot_ad_.get(), *(this->system_ad_), this->context_ad_.get());
...
bool VelocityImplicitEulerIntegrator<T>::MaybeFreshenVelocityMatrices(
        ...
        const T& h,
        int trial,
        const std::function<void(const MatrixX<T>&, const T&, typename ImplicitIntegrator<T>::IterationMatrix*)>&
...
template <class T>
VectorX<T> VelocityImplicitEulerIntegrator<T>::ComputeResidualR(const T& t,
                                                                const VectorX<T>& y,
                                                                const VectorX<T>& qk,
                                                                const VectorX<T>& qn,
                                                                const VectorX<T>& yn,
...
template <class T>
template <typename U>
VectorX<U> VelocityImplicitEulerIntegrator<T>::ComputeLOfY(const T& t,
                                                           const VectorX<U>& y,
                                                           const VectorX<T>& qk,
                                                           const VectorX<T>& qn,
                                                           const T& h,
    ...
    system.MapVelocityToQDot(*context, context->get_continuous_state().get_generalized_velocity(), &*qdot);
    ...
    const ContinuousState<U>& xc_deriv = this->EvalTimeDerivatives(system, *context);
...
bool VelocityImplicitEulerIntegrator<T>::StepVelocityImplicitEuler(
    ...
    DRAKE_LOGGER_DEBUG("VelocityImplicitEulerIntegrator::StepVelocityImplicitEuler(h={}, t={})", h, t0);

    const System<T>& system = this->get_system();
    // Verify xtplus. We also verify the size to make sure we're not making
    // unnecessary heap allocations.
    DRAKE_ASSERT(xtplus != nullptr && xtplus->size() == xn.size() && xtplus_guess.size() == xn.size());
    ...
    *xtplus = xtplus_guess;
    ...
    if (!this->get_use_full_newton() &&
        !this->MaybeFreshenVelocityMatrices(t0, yn, qn, qn, h, trial, ComputeAndFactorImplicitEulerIterationMatrix,
                                            iteration_matrix, Jy)) {
        return false;
    }
    ...
    for (int i = 0; i < this->max_newton_raphson_iterations(); ++i) {
        ...
        this->FreshenVelocityMatricesIfFullNewton(tf, ytplus, qtplus, qn, h,
        ...
        //     R(yₖ) = yₖ - yⁿ - h ℓ(yₖ).
        VectorX<T> residual = ComputeResidualR(tf, ytplus, qtplus, qn, yn, h, qdot_.get());
        ...
        // position.
        context->get_mutable_continuous_state().get_mutable_generalized_position().SetFromVector(qtplus);
        ...
        if (status == ImplicitIntegrator<T>::ConvergenceStatus::kConverged) return true;
        ...
        DRAKE_DEMAND(status == ImplicitIntegrator<T>::ConvergenceStatus::kNotConverged);
    ...
    return StepVelocityImplicitEuler(t0, h, xn, xtplus_guess, xtplus, iteration_matrix, Jy, trial + 1);
}
...
bool VelocityImplicitEulerIntegrator<T>::StepHalfVelocityImplicitEulers(
    ...
    const int64_t stored_num_jacobian_evaluations = this->get_num_jacobian_evaluations();
    const int64_t stored_num_iter_factorizations = this->get_num_iteration_matrix_factorizations();
    const int64_t stored_num_function_evaluations = this->get_num_derivative_evaluations();
    const int64_t stored_num_jacobian_function_evaluations = this->get_num_derivative_evaluations_for_jacobian();
    const int64_t stored_num_nr_iterations = this->get_num_newton_raphson_iterations();
    ...
    bool success = StepVelocityImplicitEuler(t0, 0.5 * h, xn, xthalf_guess, xtplus, iteration_matrix, Jy);
    if (!success) {
        ...
        success = StepVelocityImplicitEuler(t0 + 0.5 * h, 0.5 * h, xthalf, xtplus_guess, xtplus, iteration_matrix, Jy);
        if (!success) {
    ...
    num_half_vie_jacobian_reforms_ += this->get_num_jacobian_evaluations() - stored_num_jacobian_evaluations;
    num_half_vie_iter_factorizations_ +=
            this->get_num_iteration_matrix_factorizations() - stored_num_iter_factorizations;
    num_half_vie_function_evaluations_ += this->get_num_derivative_evaluations() - stored_num_function_evaluations;
    num_half_vie_jacobian_function_evaluations_ +=
            this->get_num_derivative_evaluations_for_jacobian() - stored_num_jacobian_function_evaluations;
    num_half_vie_nr_iterations_ += this->get_num_newton_raphson_iterations() - stored_num_nr_iterations;
...
bool VelocityImplicitEulerIntegrator<T>::AttemptStepPaired(
        const T& t0, const T& h, const VectorX<T>& xn, VectorX<T>* xtplus_vie, VectorX<T>* xtplus_hvie) {
    DRAKE_LOGGER_DEBUG(
    ...
    if (!StepVelocityImplicitEuler(t0, h, xn, xtplus_guess, xtplus_vie, &iteration_matrix_vie_, &Jy_vie_)) {
        DRAKE_LOGGER_DEBUG(
    ...
    if (!StepHalfVelocityImplicitEulers(t0, h, xn, *xtplus_vie, xtplus_hvie, &iteration_matrix_vie_, &Jy_vie_)) {
        DRAKE_LOGGER_DEBUG(
...
bool VelocityImplicitEulerIntegrator<T>::DoImplicitIntegratorStep(const T& h) {
    ...
    if (h < this->get_working_minimum_step_size()) {
        ...
        const bool success = AttemptStepPaired(t0, h, xn_, &xtplus_vie_, &xtplus_hvie_);
    ...
    this->get_mutable_error_estimate()->get_mutable_vector().SetFromVector(err_est_vec_);
