...
#include "systems/analysis/test_utilities/cubic_scalar_system.h"
#include "systems/analysis/test_utilities/implicit_integrator_test.h"
#include "systems/analysis/test_utilities/linear_scalar_system.h"
#include "systems/analysis/test_utilities/quadratic_scalar_system.h"
...
GTEST_TEST(RadauIntegratorTest, CubicSystem) {
    ...
    // Verify the solution.
    VectorX<double> state = context->get_continuous_state().get_vector().CopyToVector();
    EXPECT_NEAR(state[0], cubic.Evaluate(h), std::numeric_limits<double>::epsilon());
    ...
    EXPECT_GT(std::abs(state[0] - cubic.Evaluate(h)), 1e9 * std::numeric_limits<double>::epsilon());
}
...
GTEST_TEST(RadauIntegratorTest, QuadraticTest) {
    ...
    EXPECT_NEAR(quadratic_context->get_continuous_state().get_vector().CopyToVector()[0], quadratic.Evaluate(t_final),
    ...
    RadauIntegrator<double> scaled_radau(scaled_quadratic, scaled_quadratic_context.get());
    ...
    const double updated_err_est = scaled_radau.get_error_estimate()->get_vector()[0];
    ...
    // Verify the solution too.
    EXPECT_NEAR(scaled_quadratic_context->get_continuous_state().get_vector().CopyToVector()[0],
                scaled_quadratic.Evaluate(updated_t_final), 10 * std::numeric_limits<double>::epsilon());
}
...
GTEST_TEST(RadauIntegratorTest, Radau1MatchesImplicitEuler) {
    ...
    // damping.
    implicit_integrator_test::DiscontinuousSpringMassDamperSystem<double> spring_damper(
            semistiff_spring_k, std::sqrt(semistiff_spring_k / mass), mass, constant_force_mag);
    ...
    RadauIntegrator<double, num_stages> radau1(spring_damper, context_radau1.get());
    ...
    EXPECT_NEAR(context_radau1->get_continuous_state().get_vector().GetAtIndex(0),
                context_ie->get_continuous_state().get_vector().GetAtIndex(0), tol);
    EXPECT_NEAR(context_radau1->get_continuous_state().get_vector().GetAtIndex(1),
                context_ie->get_continuous_state().get_vector().GetAtIndex(1), tol);
    ...
    EXPECT_EQ(radau1.get_largest_step_size_taken(), ie.get_largest_step_size_taken());
    EXPECT_EQ(radau1.get_num_derivative_evaluations_for_jacobian(), ie.get_num_derivative_evaluations_for_jacobian());
    EXPECT_EQ(radau1.get_num_error_estimator_derivative_evaluations(),
              ie.get_num_error_estimator_derivative_evaluations());
    EXPECT_EQ(radau1.get_num_error_estimator_derivative_evaluations_for_jacobian(),
    ...
    EXPECT_EQ(radau1.get_num_error_estimator_jacobian_evaluations(), ie.get_num_error_estimator_jacobian_evaluations());
    EXPECT_EQ(radau1.get_num_error_estimator_newton_raphson_iterations(),
              ie.get_num_error_estimator_newton_raphson_iterations());
    EXPECT_EQ(radau1.get_num_iteration_matrix_factorizations(), ie.get_num_iteration_matrix_factorizations());
    EXPECT_EQ(radau1.get_num_jacobian_evaluations(), ie.get_num_jacobian_evaluations());
    EXPECT_EQ(radau1.get_num_step_shrinkages_from_error_control(), ie.get_num_step_shrinkages_from_error_control());
...
typedef ::testing::Types<RadauIntegrator<double, 1>, RadauIntegrator<double, 2>> MyTypes;
