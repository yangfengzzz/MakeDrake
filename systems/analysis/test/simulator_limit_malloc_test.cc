...
private:
    EventStatus Update(const Context<double>&) const { return EventStatus::Succeeded(); }
    EventStatus Update(const Context<double>&, DiscreteValues<double>*) const { return EventStatus::Succeeded(); }
    EventStatus Update(const Context<double>&, State<double>*) const { return EventStatus::Succeeded(); }
};
...
GTEST_TEST(SimulatorLimitMallocTest, NoHeapAllocsInSimulatorForSystemsWithoutContinuousState) {
