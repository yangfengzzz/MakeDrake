#include "systems/analysis/scalar_initial_value_problem.h"
...
GTEST_TEST(ScalarInitialValueProblemTest, UsingMultipleIntegrators) {
    ...
    EXPECT_NEAR(ivp.Solve(t0, t1), k1[0] + (x0 - k1[0]) * std::exp(-(t1 - t0)), kAccuracy);
    ...
    IntegratorBase<double>* configured_integrator = ivp.reset_integrator<RungeKutta2Integrator<double>>(kMaximumStep);
    ...
    EXPECT_NEAR(ivp2.Solve(t0, t2), k2[0] + (x0 - k2[0]) * std::exp(-(t2 - t0)), kAccuracy);
}

// Parameterized fixture for testing accuracy of scalar IVP solutions.
class ScalarInitialValueProblemAccuracyTest : public ::testing::TestWithParam<double> {
protected:
...
TEST_P(ScalarInitialValueProblemAccuracyTest, StoredCharge) {
    ...
    for (double Rs = kLowestResistance; Rs <= kHighestResistance; Rs += kResistanceStep) {
        for (double Cs = kLowestCapacitance; Cs <= kHighestCapacitance; Cs += kCapacitanceStep) {
            ...
            ScalarInitialValueProblem<double> stored_charge_ivp(
                    [](const double& t, const double& q, const VectorX<double>& k) -> double {
            ...
            IntegratorBase<double>& inner_integrator = stored_charge_ivp.get_mutable_integrator();
            ...
            for (double t = kInitialTime; t <= kTotalTime; t += kTimeStep) {
                ...
                const double solution = (tau_sq / (1. + tau_sq) * std::exp(-t / tau) +
                                         tau / std::sqrt(1. + tau_sq) * std::sin(t - std::atan(tau))) /
                                        Rs;
                EXPECT_NEAR(stored_charge_ivp.Solve(t0, t), solution, integration_accuracy_)
                        << "Failure solving dQ/dt = (sin(t) - Q / Cs) / Rs using Q(t₀ = " << t0
                        << "; [Rs, Cs]) = " << Q0 << " for t = " << t << ", Rs = " << Rs << " and Cs = " << Cs
                        << " to an accuracy of " << integration_accuracy_;

                EXPECT_NEAR(stored_charge_approx->EvaluateScalar(t), solution, integration_accuracy_)
                        << "Failure approximating the solution for"
                        << " dQ/dt = (sin(t) - Q / Cs) / Rs using Q(t₀ = " << t0 << "; [Rs, Cs]) = " << Q0
                        << " for t = " << t << ", Rs = " << Rs << " and Cs = " << Cs << " to an accuracy of "
...
TEST_P(ScalarInitialValueProblemAccuracyTest, PopulationGrowth) {
    ...
    const double t0 = kInitialTime;
    const double tf = kTotalTime;
    for (double r = kLowestMalthusParam; r <= kHighestMalthusParam; r += kMalthusParamStep) {
        ScalarInitialValueProblem<double> population_growth_ivp(
                [](const double& t, const double& n, const VectorX<double>& k) -> double {
                    unused(t);
                    return k[0] * n;
                },
                kInitialPopulation, Vector1<double>{r});

        IntegratorBase<double>& inner_integrator = population_growth_ivp.get_mutable_integrator();
        inner_integrator.set_target_accuracy(integration_accuracy_);
        ...
        for (double t = kInitialTime; t <= kTotalTime; t += kTimeStep) {
            ...
            EXPECT_NEAR(population_growth_ivp.Solve(t0, t), solution, integration_accuracy_)
                    << "Failure solving dN/dt = r * N using N(t₀ = " << t0 << "; r) = " << N0 << " for t = " << t
                    << " and r = " << r << " to an accuracy of " << integration_accuracy_;

            EXPECT_NEAR(population_growth_approx->EvaluateScalar(t), solution, integration_accuracy_)
                    << "Failure approximating the solution for dN/dt = r * N" << " using N(t₀ = " << t0
                    << "; r) = " << N0 << " for t = " << t << " and r = " << r << " to an accuracy of "
                    << integration_accuracy_ << " with solver's continuous extension.";
        }
    }
}
