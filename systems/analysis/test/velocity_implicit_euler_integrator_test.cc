...
// zero).
GTEST_TEST(VelocityImplicitEulerIntegratorTest, QuadraticSystemErrorEstimatorAccuracy) {
    ...
    VelocityImplicitEulerIntegrator<double> vie(quadratic, quadratic_context.get());
    ...
    const double err_est_2h_2 = vie.get_error_estimate()->get_vector().GetAtIndex(0);
    ...
    ImplicitIntegratorTest<VelocityImplicitEulerIntegrator<double>>::CheckGeneralStatsValidity(&vie);
}
