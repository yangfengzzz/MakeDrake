#include "systems/analysis/monte_carlo.h"
...
void CheckConsistentReplay(const SimulatorFactory& make_simulator,
    ...
    const double random_output = RandomSimulation(make_simulator, output, final_time, &generator);

    // Run a few more random simulators to mix-up the state.  Make sure they
    // give different outputs.
    for (int i = 0; i < 5; i++) {
        EXPECT_NE(RandomSimulation(make_simulator, output, final_time, &generator), random_output);
    }

    // Reset the generator to our snapshot and confirm that we got our
    // original result:
    generator = generator_snapshot;
    EXPECT_EQ(RandomSimulation(make_simulator, output, final_time, &generator), random_output);
}

double GetScalarOutput(const System<double>& system, const Context<double>& context) {
    return system.get_output_port(0).Eval<BasicVector<double>>(context).GetAtIndex(0);
}
...
GTEST_TEST(RandomSimulationTest, DeterministicSimulator) {
    ...
    EXPECT_EQ(RandomSimulation(make_simulator, &GetScalarOutput, final_time, &generator), value);
}
...
GTEST_TEST(RandomSimulationTest, WithRandomSimulator) {
    ...
    const SimulatorFactory make_simulator = [](RandomGenerator* generator) {
        ...
        auto system = std::make_unique<ConstantVectorSource<double>>(distribution(*generator));
        return std::make_unique<Simulator<double>>(std::move(system));
    };
    const double final_time = 0.1;
...
private:
    void SetRandomState(const Context<double>& context,
        ...
        std::normal_distribution<> distribution;
        state->get_mutable_discrete_state(0).SetAtIndex(0, distribution(*generator));
    }
...
GTEST_TEST(RandomSimulationTest, WithRandomInputs) {
    const SimulatorFactory make_simulator = [](RandomGenerator*) {
        ...
        auto random_source =
                builder.AddSystem<RandomSource<double>>(RandomDistribution::kUniform, kNumOutputs, sampling_interval);
        auto pass_through = builder.template AddSystem<PassThrough>(kNumOutputs);
        builder.Connect(random_source->get_output_port(0), pass_through->get_input_port());
...
GTEST_TEST(MonteCarloSimulationTest, BasicTest) {
    ...
    const auto serial_results = MonteCarloSimulation(make_simulator, &GetScalarOutput, final_time, num_samples,
                                                     &serial_generator, Parallelism::None());
    const auto parallel_results = MonteCarloSimulation(make_simulator, &GetScalarOutput, final_time, num_samples,
    ...
    for (int sample = 0; sample < num_samples; ++sample) {
        ...
        RandomGenerator serial_reproduction_generator(serial_result.generator_snapshot);
        RandomGenerator parallel_reproduction_generator(parallel_result.generator_snapshot);

        EXPECT_EQ(RandomSimulation(make_simulator, &GetScalarOutput, final_time, &serial_reproduction_generator),
                  serial_result.output);
        EXPECT_EQ(RandomSimulation(make_simulator, &GetScalarOutput, final_time, &parallel_reproduction_generator),
...
public:
    ThrowingRandomContextSystem() : VectorSystem(0, 1) { this->DeclareDiscreteState(1); }
...
GTEST_TEST(MonteCarloSimulationExceptionTest, BasicTest) {
    ...
    EXPECT_THROW(MonteCarloSimulation(make_simulator, &GetScalarOutput, final_time, num_samples, &serial_generator,
                                      Parallelism::None()),
                 std::exception);
    EXPECT_THROW(MonteCarloSimulation(make_simulator, &GetScalarOutput, final_time, num_samples, &parallel_generator,
                                      Parallelism::Max()),
                 std::exception);
}
