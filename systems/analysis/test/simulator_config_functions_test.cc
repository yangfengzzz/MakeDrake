...
TYPED_TEST(SimulatorConfigFunctionsTest, ResetIntegratorTest) {
    ...
    IntegratorBase<T>& result = ResetIntegratorFromFlags(&simulator, "runge_kutta2", T(0.001));
    EXPECT_NE(&simulator.get_integrator(), prior_integrator);
    EXPECT_EQ(&simulator.get_integrator(), &result);
    const std::string result_name_expected =
            fmt::format("drake::systems::RungeKutta2Integrator<{}>", NiceTypeName::Get<T>());
...
TYPED_TEST(SimulatorConfigFunctionsTest, GetSchemes) {
    ...
    for (const auto& one_scheme : schemes) {
        DRAKE_EXPECT_NO_THROW(ResetIntegratorFromFlags(&simulator, one_scheme, T(0.001)));
    }
}
...
TYPED_TEST(SimulatorConfigFunctionsTest, CongruenceTest) {
    ...
    EXPECT_EQ(sim_defaults.integration_scheme, config_defaults.integration_scheme);
    ...
    EXPECT_EQ(sim_defaults.target_realtime_rate, config_defaults.target_realtime_rate);
    EXPECT_EQ(sim_defaults.publish_every_time_step, config_defaults.publish_every_time_step);
}
