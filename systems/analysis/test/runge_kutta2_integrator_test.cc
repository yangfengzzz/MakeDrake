...
GTEST_TEST(IntegratorTest, MiscAPI) {
    ...
    EXPECT_THROW(integrator.request_initial_step_size_target(1.0), std::logic_error);
...
GTEST_TEST(IntegratorTest, SpringMassStep) {
    ...
    spring_mass.GetClosedFormSolution(initial_position, initial_velocity, t_final, &x_final_true, &unused_v_final_true);
    ...
    std::unique_ptr<trajectories::PiecewisePolynomial<double>> dense_output = integrator.StopDenseIntegration();

    // Verify that the built dense output is valid.
    for (double t = 0; t <= t_final; t += h / 2.) {
        double x_true, unused_v_true;
        spring_mass.GetClosedFormSolution(initial_position, initial_velocity, t, &x_true, &unused_v_true);
...
GTEST_TEST(RK3IntegratorErrorEstimatorTest, QuadraticTest) {
    ...
    RungeKutta2Integrator<double> rk2(quadratic, t_final, quadratic_context.get());
    ...
    EXPECT_NEAR(quadratic_context->get_continuous_state_vector()[0], expected_result,
...
GTEST_TEST(IntegratorTest, Symbolic) {
    ...
    auto context = spring_mass.CreateDefaultContext();
    // Create the integrator.
    RungeKutta2Integrator<Expression> integrator(spring_mass, max_h, context.get());
    integrator.Initialize();
