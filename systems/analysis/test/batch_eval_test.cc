...
GTEST_TEST(BatchEvalUniquePeriodicDiscreteUpdate, BasicTest) {
  ...
  inputs << -0.12, -0.34,  0.45,
    ...
    const Eigen::MatrixXd x1 = BatchEvalUniquePeriodicDiscreteUpdate<double>(
            system, *context, times, x0, inputs, num_time_steps, input_port_index, parallelize);
    ...
    const Eigen::MatrixXd x2 = BatchEvalUniquePeriodicDiscreteUpdate<double>(
            system, *context, times1, x1, inputs, num_time_steps, input_port_index, parallelize);
    ...
    EXPECT_TRUE(CompareMatrices(
            x2_expected,
            BatchEvalUniquePeriodicDiscreteUpdate<double>(system, *context, times, x0, inputs, 2 /* num_time_steps */,
...
GTEST_TEST(BatchEvalUniquePeriodicDiscreteUpdate, TimeVaryingTest) {
    ...
    const Eigen::MatrixXd x2 =
            BatchEvalUniquePeriodicDiscreteUpdate<double>(*system, *context, times, x0, inputs, num_time_steps);

    Eigen::MatrixXd x2_expected = x0 + 2 * times + Eigen::RowVector3d::Constant(time_period);
    EXPECT_TRUE(CompareMatrices(x2, x2_expected, 1e-14));
}
...
GTEST_TEST(BatchEvalTimeDerivatives, BasicTest) {
  ...
  inputs << -0.12, -0.34,  0.45,
    ...
    const Eigen::MatrixXd xdot =
            BatchEvalTimeDerivatives<double>(system, *context, times, states, inputs, input_port_index, parallelize);
