#include "systems/analysis/lyapunov.h"
...
public:
    CubicPolynomialSystem() : systems::VectorSystem<double>(0, 0) {  // Zero inputs, zero outputs.
        this->DeclareContinuousState(1);                             // One state variable.
    }

private:
    // ẋ = -x + x³
    virtual void DoCalcVectorTimeDerivatives(const systems::Context<double>&,
...
GTEST_TEST(LyapunovTest, CubicPolynomialTest) {
    ...
    Eigen::VectorXd params = SampleBasedLyapunovAnalysis(cubic, *context, &cubic_bases, x_samples, Vector1d{0.});
...
GTEST_TEST(LyapunovTest, PendulumSampleBasedLyapunov) {
    ...
    Eigen::VectorXd q_samples = Eigen::VectorXd::LinSpaced(31, -1.5 * M_PI, 1.5 * M_PI);
    ...
    Eigen::VectorXd params =
            SampleBasedLyapunovAnalysis(pendulum, *context, &pendulum_bases<AutoDiffXd>, x_samples, x_zero);
    ...
    drake::log()->info("E = " + energy_params.dot(pendulum_bases<Expression>(state)).to_string());
    drake::log()->info("V = " + params.dot(pendulum_bases<Expression>(state)).to_string());
}
