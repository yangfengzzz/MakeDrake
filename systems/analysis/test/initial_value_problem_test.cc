...
#include "systems/framework/parameters.h"
...
GTEST_TEST(InitialValueProblemTest, SolutionUsingMultipleIntegrators) {
    ...
    InitialValueProblem<double> ivp(
            [](const double& t, const VectorX<double>& x, const VectorX<double>& k) -> VectorX<double> {
                unused(t);
                return -x + k;
            },
    ...
    EXPECT_TRUE(CompareMatrices(ivp.Solve(t0, t1), k1 + (x0 - k1) * std::exp(-(t1 - t0)), kAccuracy));
    ...
    // Testing against closed form solution of above's IVP, which can be written
    // as 𝐱(t; 𝐤) = 𝐤 + (𝐱₀ - 𝐤) * e^(-(t - t₀)).
    EXPECT_TRUE(CompareMatrices(ivp.Solve(t0, t2), k1 + (x0 - k1) * std::exp(-(t2 - t0)), kAccuracy));
}

// Parameterized fixture for testing accuracy of IVP solutions.
class InitialValueProblemAccuracyTest : public ::testing::TestWithParam<double> {
protected:
...
TEST_P(InitialValueProblemAccuracyTest, ParticleInAGasMomentum) {
    ...
    const VectorX<double> kInitialParticleMomentum = (VectorX<double>(3) << -3.0, 1.0, 2.0).finished();
    ...
    for (double mu = kLowestGasViscosity; mu <= kHighestGasViscosity; mu += kGasViscosityStep) {
        for (double m = kLowestParticleMass; m <= kHighestParticleMass; m += kParticleMassStep) {
            ...
            InitialValueProblem<double> particle_momentum_ivp(
                    [](const double& t, const VectorX<double>& p, const VectorX<double>& k) -> VectorX<double> {
            ...
            IntegratorBase<double>& inner_integrator = particle_momentum_ivp.get_mutable_integrator();
            ...
            for (double t = kInitialTime; t <= kTotalTime; t += kTimeStep) {
                ...
                EXPECT_TRUE(CompareMatrices(particle_momentum_ivp.Solve(t0, t), solution, integration_accuracy_))
                        << fmt::format(
                                   "Failure solving d𝐩/dt = -μ * 𝐩/m using 𝐩({}; [μ, m]) = {}"
                                   " for t = {}, μ = {} and m = {} to an accuracy of {}",
                                   t0, fmt_eigen(p0), t, mu, m, integration_accuracy_);

                EXPECT_TRUE(CompareMatrices(particle_momentum_approx->Evaluate(t), solution, integration_accuracy_))
                        << fmt::format(
...
TEST_P(InitialValueProblemAccuracyTest, ParticleInAGasForcedVelocity) {
    ...
    for (double mu = kLowestGasViscosity; mu <= kHighestGasViscosity; mu += kGasViscosityStep) {
        for (double m = kLowestParticleMass; m <= kHighestParticleMass; m += kParticleMassStep) {
            ...
            IntegratorBase<double>& inner_integrator = particle_velocity_ivp.get_mutable_integrator();
            inner_integrator.set_target_accuracy(integration_accuracy_);
            ...
            for (double t = kInitialTime; t <= kTotalTime; t += kTimeStep) {
                ...
                const VectorX<double> solution =
                        v0 * std::exp(-mu * (t - t0) / m) + F / mu * (1. - std::exp(-mu * (t - t0) / m));
                EXPECT_TRUE(CompareMatrices(particle_velocity_ivp.Solve(t0, t), solution, integration_accuracy_))
                        << fmt::format(
                                   "Failure solving"
                                   " d𝐯/dt = (-μ * 𝐯 + 𝐅) / m using 𝐯({}; [μ, m]) = {}"
                                   " for t = {}, μ = {}, m = {} and 𝐅 = {}"
                                   " to an accuracy of {}",
                                   t0, fmt_eigen(v0), t, mu, m, fmt_eigen(F), integration_accuracy_);

                EXPECT_TRUE(CompareMatrices(particle_velocity_approx->Evaluate(t), solution, integration_accuracy_))
                        << fmt::format(
