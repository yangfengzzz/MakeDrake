...
public:
    DummyIntegrator(const System<T>& system, Context<T>* context) : IntegratorBase<T>(system, context) {}
    ...
    std::pair<bool, T> CalcAdjustedStepSize(const T& err, const T& step_taken, bool* at_minimum_step_size) const {
        return IntegratorBase<T>::CalcAdjustedStepSize(err, step_taken, at_minimum_step_size);
    }
...
GTEST_TEST(IntegratorBaseTest, FixedStepFailureIndicatesSubstepFailure) {
    ...
    integrator.IntegrateNoFurtherThanTime(arbitrary_time, arbitrary_time, arbitrary_time);
...
GTEST_TEST(IntegratorBaseTest, CalcAdjustedStepSizeShrinksOnNaNAndInf) {
    ...
    result = integrator.CalcAdjustedStepSize(zero_error, step_taken, &at_minimum_step_size_true_on_entry);
    EXPECT_EQ(result.first, true);
    EXPECT_GE(result.second, step_taken);
    result = integrator.CalcAdjustedStepSize(zero_error, step_taken, &at_minimum_step_size_false_on_entry);
    EXPECT_EQ(result.first, true);
    EXPECT_GE(result.second, step_taken);
    ...
    result = integrator.CalcAdjustedStepSize(nan_error, step_taken, &at_minimum_step_size_true_on_entry);
    EXPECT_EQ(result.first, false);
    EXPECT_LT(result.second, kShrink * step_taken);
    result = integrator.CalcAdjustedStepSize(nan_error, step_taken, &at_minimum_step_size_false_on_entry);
    EXPECT_EQ(result.first, false);
    EXPECT_LT(result.second, kShrink * step_taken);
    ...
    result = integrator.CalcAdjustedStepSize(inf_error, step_taken, &at_minimum_step_size_true_on_entry);
    EXPECT_EQ(result.first, false);
    EXPECT_LT(result.second, kShrink * step_taken);
    result = integrator.CalcAdjustedStepSize(inf_error, step_taken, &at_minimum_step_size_false_on_entry);
    EXPECT_EQ(result.first, false);
    EXPECT_LT(result.second, kShrink * step_taken);
...
GTEST_TEST(IntegratorBaseTest, DoubleStateChangeNormPropagatesNaN) {
    ...
    // CalcStateChangeNorm().
    using std::isnan;
    context->get_mutable_continuous_state()[0] = std::numeric_limits<double>::quiet_NaN();
    context->get_mutable_continuous_state()[1] = 0;
    context->get_mutable_continuous_state()[2] = 0;
    EXPECT_TRUE(isnan(integrator.CalcStateChangeNorm(context->get_continuous_state())));

    // Set v to NaN, and q = z = 0 and verify that NaN is returned from
    // CalcStateChangeNorm().
    context->get_mutable_continuous_state()[0] = 0;
    context->get_mutable_continuous_state()[1] = std::numeric_limits<double>::quiet_NaN();
    context->get_mutable_continuous_state()[2] = 0;
    EXPECT_TRUE(isnan(integrator.CalcStateChangeNorm(context->get_continuous_state())));
    ...
    context->get_mutable_continuous_state()[0] = 0;
    context->get_mutable_continuous_state()[1] = 0;
    context->get_mutable_continuous_state()[2] = std::numeric_limits<double>::quiet_NaN();
    EXPECT_TRUE(isnan(integrator.CalcStateChangeNorm(context->get_continuous_state())));
}
...
GTEST_TEST(IntegratorBaseTest, AutoDiffXdStateChangeNormPropagatesNaN) {
    ...
    std::unique_ptr<Context<AutoDiffXd>> context = spring_mass.CreateDefaultContext();
...
GTEST_TEST(IntegratorBaseTest, DenseOutputTest) {
    ...
    const trajectories::PiecewisePolynomial<double>* dense_output = integrator.get_dense_output();
    ...
    context->SetTime(0.2);
    DRAKE_EXPECT_THROWS_MESSAGE(static_cast<void>(integrator.IntegrateWithSingleFixedStepToTime(0.3)),
