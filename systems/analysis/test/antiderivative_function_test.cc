#include "systems/analysis/antiderivative_function.h"

#include <gtest/gtest.h>

#include "common/eigen_types.h"
#include "common/test_utilities/expect_throws_message.h"
#include "systems/analysis/integrator_base.h"
...
GTEST_TEST(AntiderivativeFunctionTest, UsingMultipleIntegrators) {
    ...
    EXPECT_NEAR(antiderivative_function.Evaluate(kDefaultLowerIntegrationBound, u1),
                k1[0] / 2 * std::pow(u1, 2.0) + k1[1] * u1, kAccuracy);

    // Replaces default integrator.
    const double kMaximumStep = 0.1;
    const IntegratorBase<double>& default_integrator = antiderivative_function.get_integrator();
    using RK2 = RungeKutta2Integrator<double>;
    IntegratorBase<double>* configured_integrator = antiderivative_function.reset_integrator<RK2>(kMaximumStep);
    ...
    // can be written as F(u; 𝐤) = k₀/2 * u^2 + k₁ * u for the specified
    // integration lower bound.
    EXPECT_NEAR(antiderivative_function.Evaluate(kDefaultLowerIntegrationBound, u2),
                k1[0] / 2 * std::pow(u2, 2.0) + k1[1] * u2, kAccuracy);
}

class AntiderivativeFunctionAccuracyTest : public ::testing::TestWithParam<double> {
protected:
...
TEST_P(AntiderivativeFunctionAccuracyTest, NthPowerMonomialTestCase) {
    ...
    for (int n = kLowestOrder; n <= kHighestOrder; ++n) {
        ...
        IntegratorBase<double>& inner_integrator = antiderivative_function.get_mutable_integrator();
        inner_integrator.set_target_accuracy(integration_accuracy_);

        const std::unique_ptr<ScalarDenseOutput<double>> antiderivative_function_approx =
                antiderivative_function.MakeDenseEvalFunction(kArgIntervalLBound, kArgIntervalUBound);

        for (double u = kArgIntervalLBound; u <= kArgIntervalUBound; u += kArgStep) {
            ...
            EXPECT_NEAR(antiderivative_function.Evaluate(kArgIntervalLBound, u), solution, integration_accuracy_)
                    << "Failure integrating ∫₀ᵘ xⁿ dx for u = " << u << " and n = " << n << " to an accuracy of "
                    << integration_accuracy_;

            EXPECT_NEAR(antiderivative_function_approx->EvaluateScalar(u), solution, integration_accuracy_)
                    << "Failure approximating ∫₀ᵘ xⁿ dx for u = " << u << " and n = " << n << " to an accuracy of "
...
TEST_P(AntiderivativeFunctionAccuracyTest, HyperbolicTangentTestCase) {
    ...
    for (double a = kParamIntervalLBound; a <= kParamIntervalUBound; a += kParamStep) {
        AntiderivativeFunction<double> antiderivative_function(
                [](const double& x, const VectorX<double>& k) -> double {
        ...
        for (double u = kArgIntervalLBound; u <= kArgIntervalUBound; u += kArgStep) {
            ...
            EXPECT_NEAR(antiderivative_function.Evaluate(kArgIntervalLBound, u), solution, integration_accuracy_)
                    << "Failure integrating ∫₀ᵘ tanh(a⋅x) dx for" << " u = " << u << " and a = " << a
                    << " to an accuracy of " << integration_accuracy_;

            EXPECT_NEAR(antiderivative_function_approx->EvaluateScalar(u), solution, integration_accuracy_)
                    << "Failure approximating ∫₀ᵘ tanh(a⋅x) dx for" << " u = " << u << " and a = " << a
                    << " to an accuracy of " << integration_accuracy_ << " with solver's continuous extension.";
        }
    }
}
...
TEST_P(AntiderivativeFunctionAccuracyTest, SecondOrderRationalFunctionTestCase) {
    ...
    for (double a = k1stPoleIntervalLBound; a <= k1stPoleIntervalUBound; a += k1stPoleStep) {
        for (double b = k2ndPoleIntervalLBound; b <= k2ndPoleIntervalUBound; b += k2ndPoleStep) {
            AntiderivativeFunction<double> antiderivative_function(
                    [](const double& x, const VectorX<double>& k) -> double {
                        const double a_ = k[0];
                        const double b_ = k[1];
            ...
            for (double u = kArgIntervalLBound; u <= kArgIntervalUBound; u += kArgStep) {
                ...
                const double solution = std::log((b / a) * ((u + a) / (u + b))) / (b - a);

                EXPECT_NEAR(antiderivative_function.Evaluate(kArgIntervalLBound, u), solution, integration_accuracy_)
                        << "Failure integrating ∫₀ᵘ [(x + a)⋅(x + b)]⁻¹ dx for" << " u = " << u << ", a = " << a
                        << "and b = " << b << " to an accuracy of " << integration_accuracy_;

                EXPECT_NEAR(antiderivative_function_approx->EvaluateScalar(u), solution, integration_accuracy_)
                        << "Failure approximating ∫₀ᵘ [(x + a)⋅(x + b)]⁻¹ dx for" << " u = " << u << ", a = " << a
                        << "and b = " << b << " to an accuracy of " << integration_accuracy_
...
TEST_P(AntiderivativeFunctionAccuracyTest, ExponentialFunctionTestCase) {
    ...
    for (double n = kParamIntervalLBound; n <= kParamIntervalUBound; n += kParamStep) {
        AntiderivativeFunction<double> antiderivative_function(
                [](const double& x, const VectorX<double>& k) -> double {
        ...
        for (double u = kArgIntervalLBound; u <= kArgIntervalUBound; u += kArgStep) {
            ...
            const double solution = (u / n - 1. / (n * n)) * std::exp(n * u) + 1. / (n * n);

            EXPECT_NEAR(antiderivative_function.Evaluate(kArgIntervalLBound, u), solution, integration_accuracy_)
                    << "Failure integrating ∫₀ᵘ x eⁿˣ dx for" << " u = " << u << " and n = " << n
                    << " to an accuracy of " << integration_accuracy_;

            EXPECT_NEAR(antiderivative_function_approx->EvaluateScalar(u), solution, integration_accuracy_)
                    << "Failure approximating ∫₀ᵘ x eⁿˣ dx for" << " u = " << u << " and n = " << n
                    << " to an accuracy of " << integration_accuracy_ << " with solver's continuous extension.";
        }
    }
}
...
TEST_P(AntiderivativeFunctionAccuracyTest, TrigonometricFunctionTestCase) {
    ...
    for (double a = kParamIntervalLBound; a <= kParamIntervalUBound; a += kParamStep) {
        ...
        for (double u = kArgIntervalLBound; u <= kArgIntervalUBound; u += kArgStep) {
            ...
            const double solution = -u * std::cos(a * u) / a + std::sin(a * u) / (a * a);

            EXPECT_NEAR(antiderivative_function.Evaluate(kArgIntervalLBound, u), solution, integration_accuracy_)
                    << "Failure integrating ∫₀ᵘ x⋅sin(a⋅x) dx for" << " u = " << u << " and a = " << a
                    << " to an accuracy of " << integration_accuracy_;

            EXPECT_NEAR(antiderivative_function_approx->EvaluateScalar(u), solution, integration_accuracy_)
                    << "Failure approximating ∫₀ᵘ x⋅sin(a⋅x) dx for" << " u = " << u << " and a = " << a
                    << " to an accuracy of " << integration_accuracy_ << "with solver's continuous extension.";
            ;
        }
    }
