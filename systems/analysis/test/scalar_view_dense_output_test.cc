#include "systems/analysis/scalar_view_dense_output.h"
...
protected:
    std::unique_ptr<DenseOutput<T>> CreateDummyDenseOutput() {
        ...
        typename HermitianDenseOutput<T>::IntegrationStep step(this->kInitialTime, this->kInitialState,
    ...
    const MatrixX<T> kFinalState{(MatrixX<double>(3, 1) << 1., 10., 100.).finished()};
...
TYPED_TEST(ScalarViewDenseOutputTest, ExtensionConsistency) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ScalarViewDenseOutput<TypeParam>(std::unique_ptr<HermitianDenseOutput<TypeParam>>(),
                                                                 this->kValidElementIndex),
                                ".*dense output.*is null.*");

    // Verifies that views to invalid elements result in an error.
    DRAKE_EXPECT_THROWS_MESSAGE(
            ScalarViewDenseOutput<TypeParam>(this->CreateDummyDenseOutput(), this->kInvalidElementIndex),
            ".*out of.*dense output.*range.*");

    // Instantiates scalar continuous extension properly.
    ScalarViewDenseOutput<TypeParam> dense_output(this->CreateDummyDenseOutput(), this->kValidElementIndex);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dense_output.Evaluate(this->kInvalidTime), ".*[Tt]ime.*out of.*dense output.*domain.*");

    // Compares evaluations for consistency.
    EXPECT_EQ(dense_output.EvaluateScalar(this->kInitialTime),
    ...
    EXPECT_EQ(dense_output.EvaluateScalar(this->kFinalTime),
