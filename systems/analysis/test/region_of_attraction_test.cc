...
GTEST_TEST(RegionOfAttractionTest, CubicPolynomialTest) {
    Variable x("x");
    const auto system = SymbolicVectorSystemBuilder().state(x).dynamics(-x + pow(x, 3)).Build();
...
GTEST_TEST(RegionOfAttractionTest, ShiftedCubicPolynomialTest) {
    Variable x("x");
    const double x0 = 2;
    const auto system = SymbolicVectorSystemBuilder().state(x).dynamics(-(x - x0) + pow(x - x0, 3)).Build();
...
GTEST_TEST(RegionOfAttractionTest, NonConvexROA) {
    ...
    const auto system = SymbolicVectorSystemBuilder().state(x).dynamics((U - 1) * dUdx.transpose()).Build();
    const auto context = system->CreateDefaultContext();
    ...
    options.solver_options.emplace().SetOption(solvers::CsdpSolver::id(), "objtol", 1e-9);
    const Expression V = RegionOfAttraction(*system, *context, options);
...
GTEST_TEST(RegionOfAttractionTest, FixedInput) {
    Variable x("x");
    Variable u("u");
    const auto system = SymbolicVectorSystemBuilder().state(x).input(u).dynamics(u - x + pow(x, 3)).Build();
    auto context = system->CreateDefaultContext();
...
GTEST_TEST(RegionOfAttractionTest, SubSystem) {
    ...
    Variable y{"y"};
    const auto& system =
            *builder.AddSystem(SymbolicVectorSystemBuilder().state(x).dynamics(-x + pow(x, 3) + y).input(y).Build());
    const auto& value_system = *builder.AddSystem<ConstantVectorSource<double>>(0);
    builder.Connect(value_system, system);
    auto diagram = builder.Build();
    auto diagram_context = diagram->CreateDefaultContext();
    const auto& system_context = diagram->GetMutableSubsystemContext(system, diagram_context.get());
...
public:
    ...
    RationalPolynomialSystem() : LeafSystem<T>(SystemTypeTag<RationalPolynomialSystem>{}) {
        this->DeclareContinuousState(1);
    }

    // Scalar-converting copy constructor.  See @ref system_scalar_conversion.
    template <typename U>
    explicit RationalPolynomialSystem(const RationalPolynomialSystem<U>& other) : RationalPolynomialSystem() {}
