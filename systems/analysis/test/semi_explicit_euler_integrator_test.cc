...
GTEST_TEST(IntegratorTest, ContextAccess) {
    ...
    SemiExplicitEulerIntegrator<double> integrator(spring_mass, h, context.get());  // Use default Context.
    ...
    EXPECT_THROW(integrator.IntegrateNoFurtherThanTime(target_time, target_time, target_time), std::logic_error);
}
...
GTEST_TEST(IntegratorTest, RigidBody) {
    ...
    multibody::SpatialInertia<double> M_BBcm = multibody::SpatialInertia<double>::SolidSphereWithMass(mass, radius);
    ...
    // Get the final state.
    VectorX<double> x_final_ee = context->get_continuous_state_vector().CopyToVector();
    ...
    VectorX<double> x_final_see = context->get_continuous_state_vector().CopyToVector();
    const double tol = 5e-3;
    for (int i = 0; i < x_final_see.size(); ++i) EXPECT_NEAR(x_final_ee[i], x_final_see[i], tol);
}
...
GTEST_TEST(IntegratorTest, SpringMassStep) {
    ...
    double t;
    for (t = 0.0; std::abs(t - kTFinal) > h; t += h) integrator.IntegrateNoFurtherThanTime(inf, inf, kTFinal);
    ...
    // Check the solution.
    EXPECT_NEAR(c1 * std::cos(kOmega * t) + c2 * std::sin(kOmega * t), x_final, 5e-3);
