#include "systems/analysis/simulator.h"
...
#include "systems/analysis/explicit_euler_integrator.h"
#include "systems/analysis/implicit_euler_integrator.h"
#include "systems/analysis/runge_kutta2_integrator.h"
#include "systems/analysis/runge_kutta3_integrator.h"
#include "systems/analysis/test_utilities/controlled_spring_mass_system.h"
#include "systems/analysis/test_utilities/logistic_system.h"
#include "systems/analysis/test_utilities/my_spring_mass_system.h"
#include "systems/analysis/test_utilities/spring_mass_system.h"
...
#include "systems/primitives/vector_log_sink.h"
...
public:
    ...
    bool was_do_calc_time_derivatives_called() const { return do_calc_time_derivatives_called_; }
...
public:
    ...
    explicit StatelessDiagram(double offset) {
        ...
        stateless_ = builder.AddSystem<StatelessSystem>(offset, WitnessFunctionDirection::kCrossesZero);
        stateless_->set_name("stateless_diagram");
        builder.BuildInto(this);
    }

    void set_publish_callback(std::function<void(const Context<double>&)> callback) {
...
public:
    ...
    CompositeSystem(double k, double alpha, double nu, double trigger_time)
        ...
        logistic_witness_ =
                this->MakeWitnessFunction("logistic witness", WitnessFunctionDirection::kCrossesZero,
                                          &CompositeSystem::GetStateValue, &CompositeSystem::CallLogisticsCallback);
        clock_witness_ =
                this->MakeWitnessFunction("clock witness", WitnessFunctionDirection::kCrossesZero,
                                          &CompositeSystem::CalcClockWitness, &CompositeSystem::CallLogisticsCallback);
    }

    const WitnessFunction<double>* get_logistic_witness() const { return logistic_witness_.get(); }

    const WitnessFunction<double>* get_clock_witness() const { return clock_witness_.get(); }

protected:
    void DoGetWitnessFunctions(const Context<double>&, std::vector<const WitnessFunction<double>*>* w) const override {
        w->push_back(clock_witness_.get());
        w->push_back(logistic_witness_.get());
    }

private:
    double GetStateValue(const Context<double>& context) const { return context.get_continuous_state()[0]; }

    // The witness function is the time value itself plus the offset value.
    double CalcClockWitness(const Context<double>& context) const { return context.get_time() - trigger_time_; }

    void CallLogisticsCallback(const Context<double>& context, const PublishEvent<double>& event) const {
...
public:
    ...
    explicit TwoWitnessStatelessSystem(double off1, double off2) : offset1_(off1), offset2_(off2) {
        PublishEvent<double> event([](const System<double>& system, const Context<double>& callback_context,
                                      const PublishEvent<double>& callback_event) {
            dynamic_cast<const TwoWitnessStatelessSystem&>(system).PublishOnWitness(callback_context, callback_event);
            return EventStatus::Succeeded();
        });
        witness1_ = this->MakeWitnessFunction("clock witness1", WitnessFunctionDirection::kCrossesZero,
                                              &TwoWitnessStatelessSystem::CalcClockWitness1, event);
        witness2_ = this->MakeWitnessFunction("clock witness2", WitnessFunctionDirection::kCrossesZero,
...
private:
    ...
    void PublishOnWitness(const Context<double>& context, const PublishEvent<double>&) const {
    ...
    double CalcClockWitness1(const Context<double>& context) const { return context.get_time() - offset1_; }

    // The witness function is the time value itself minus the offset value.
    double CalcClockWitness2(const Context<double>& context) const { return context.get_time() - offset2_; }
...
GTEST_TEST(SimulatorTest, MultipleWitnessesIdentical) {
    ...
    system.set_publish_callback([&](const Context<double>& context) {
        ...
        std::optional<double> iso_time = simulator->GetCurrentWitnessTimeIsolation();
...
GTEST_TEST(SimulatorTest, MultipleWitnessesStaggered) {
    ...
    const std::optional<double> iso_tol = simulator.GetCurrentWitnessTimeIsolation();
    EXPECT_TRUE(iso_tol);
...
GTEST_TEST(SimulatorTest, WitnessTestCountSimplePositiveToNegative) {
    // Set empty system to trigger when time is +1.
    StatelessSystem system(+1, WitnessFunctionDirection::kPositiveThenNonPositive);
    int num_publishes = 0;
    system.set_publish_callback([&](const Context<double>& context) {
        num_publishes++;
    });
...
public:
    explicit PeriodicPublishWithTimedWitnessSystem(double witness_trigger_time) {
        ...
        this->DeclarePeriodicPublishEvent(1.0, 0.5, &PeriodicPublishWithTimedWitnessSystem::PublishPeriodic);

        // Declare the publish event for the witness trigger.
        PublishEvent<double> event([](const System<double>& system, const Context<double>& callback_context,
                                      const PublishEvent<double>& callback_event) {
            dynamic_cast<const PeriodicPublishWithTimedWitnessSystem&>(system).PublishWitness(callback_context,
                                                                                              callback_event);
            return EventStatus::Succeeded();
        });
...
private:
    ...
    void DoGetWitnessFunctions(const Context<double>&, std::vector<const WitnessFunction<double>*>* w) const {
        *w = {witness_.get()};
    }
    ...
    void PublishWitness(const Context<double>& context, const PublishEvent<double>&) const {
...
GTEST_TEST(SimulatorTest, ContextAccess) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(simulator.Initialize(), ".*Initialize.*Context.*not.*set.*");
...
GTEST_TEST(SimulatorTest, SpringMass) {
    ...
    auto& integrator = simulator.reset_integrator<ExplicitEulerIntegrator<double>>(h);
...
public:
    ...
    ExampleDiscreteSystem() {
        ...
        DeclarePeriodicDiscreteUpdateEvent(kPeriod, kOffset, &ExampleDiscreteSystem::Update);
...
private:
    void Update(const systems::Context<double>& context, systems::DiscreteValues<double>* xd) const {
        const double x_n = context.get_discrete_state()[0];
        (*xd)[0] = x_n + 1.;
    }

    void Output(const systems::Context<double>& context, systems::BasicVector<double>* result) const {
        const double x_n = context.get_discrete_state()[0];
...
GTEST_TEST(SimulatorTest, ExampleDiscreteSystem) {
    ...
    auto logger = LogVectorOutput(example->GetOutputPort("Sn"), &builder, ExampleDiscreteSystem::kPeriod);
...
public:
    ...
    SinusoidalDelayHybridSystem() {
        this->DeclarePeriodicDiscreteUpdateEvent(kUpdatePeriod, 0.0, &SinusoidalDelayHybridSystem::Update);
        this->DeclareDiscreteState(1 /* single state variable */);
        this->DeclareVectorOutputPort("y", 1, &SinusoidalDelayHybridSystem::CalcOutput);
    }
...
private:
    void Update(const Context<double>& context, DiscreteValues<double>* x_next) const {
...
GTEST_TEST(SimulatorTest, SinusoidalHybridSystem) {
    ...
    simulator.get_mutable_context().get_mutable_discrete_state()[0] = initial_value;
...
public:
    ...
    ShiftedTimeOutputter() { this->DeclareVectorOutputPort("time", 1, &ShiftedTimeOutputter::OutputTime); }

private:
    void OutputTime(const Context<double>& context, BasicVector<double>* output) const {
...
public:
    ...
    explicit SimpleHybridSystem(double offset) {
        this->DeclarePeriodicDiscreteUpdateEvent(kPeriod, offset, &SimpleHybridSystem::Update);
        this->DeclareDiscreteState(1 /* single state variable */);
...
GTEST_TEST(SimulatorTest, SimpleHybridSystemTestOffsetZero) {
    ...
    auto hybrid_system = builder.AddSystem<SimpleHybridSystem>(updating_offset_time);
    ...
    simulator.get_mutable_context().get_mutable_discrete_state()[0] = initial_condition;
    ...
    // u(t) = 1.
    const double u0 = 1;
    EXPECT_EQ(simulator.get_context().get_discrete_state()[0], initial_condition + u0);
...
private:
    void Output(const Context<double>& context, BasicVector<double>* output) const {
...
public:
    ...
    DiscreteInputAccumulator() {
        ...
        DeclareInitializationDiscreteUpdateEvent(&DiscreteInputAccumulator::OnInitializeUpdate);
        DeclarePeriodicPublishEvent(kPeriod, kPublishOffset, &DiscreteInputAccumulator::OnPeriodicPublish);
        DeclarePeriodicDiscreteUpdateEvent(kPeriod, kPublishOffset, &DiscreteInputAccumulator::OnPeriodicUpdate);
    }
...
private:
    ...
    EventStatus OnInitializeUpdate(const Context<double>&, DiscreteValues<double>* x_0) const {
        (*x_0)[0] = 0.0;
        result_.clear();
        return EventStatus::Succeeded();
    }
    ...
    // at the beginning of step n+1).
    EventStatus OnPeriodicUpdate(const Context<double>& context, DiscreteValues<double>* x_np1) const {
    ...
    double get_x(const Context<double>& context) const { return context.get_discrete_state()[0]; }
...
public:
        ...
        if (unrestricted_update_callback_ != nullptr) {
            UnrestrictedUpdateEvent<double> event(
                    ...
                    [callback = unrestricted_update_callback_](
                            ...
                            const UnrestrictedUpdateEvent<double>&, State<double>* event_state) {
    ...
    void set_unrestricted_update_callback(std::function<void(const Context<double>&, State<double>*)> callback) {
        unrestricted_update_callback_ = callback;
    }

    void set_derivatives_callback(std::function<void(const Context<double>&)> callback) {
        derivatives_callback_ = callback;
    }

private:
    const double t_upd_{0.0};
    std::function<void(const Context<double>&, State<double>*)> unrestricted_update_callback_{nullptr};
...
GTEST_TEST(SimulatorTest, ExactUpdateTime) {
    ...
    unrest_upd.set_unrestricted_update_callback([&updates](const Context<double>& context, State<double>* state) {
...
GTEST_TEST(SimulatorTest, ControlledSpringMass) {
    ...
    PidControlledSpringMassSystem<double> spring_mass(kSpring, kMass, kp, ki, kd, x_target);
    Simulator<double> simulator(spring_mass);  // Use default Context.
...
// Roots should be the complex conjugate of each other.
    ...
    double x_final{}, v_final{};
    {
        ...
        auto x = exp(-zeta * w0 * time) * (C1 * cos(wd * time) + C2 * sin(wd * time));
        x_final = x.value();
...
public:
    ...
    MixedContinuousDiscreteSystem() {
        ...
        DeclarePeriodicDiscreteUpdateEvent(kUpdatePeriod, offset,
                                           &MixedContinuousDiscreteSystem::HandleDiscreteVariableUpdates);
        DeclarePeriodicPublishEvent(kPublishPeriod, 0.0, &MixedContinuousDiscreteSystem::HandlePublish);
    ...
    void HandleDiscreteVariableUpdates(const Context<double>& context, DiscreteValues<double>*) const {
    ...
    void set_update_callback(std::function<void(const Context<double>&)> callback) { update_callback_ = callback; }
...
GTEST_TEST(SimulatorTest, UpdateThenPublishThenIntegrate) {
    ...
    system.set_publish_callback([&events, &simulator](const Context<double>& context) {
        events[simulator.get_num_steps_taken()].push_back(kPublish);
    });
    system.set_update_callback([&events, &simulator](const Context<double>& context) {
        events[simulator.get_num_steps_taken()].push_back(kUpdate);
    });
    system.set_derivatives_callback([&events, &simulator](const Context<double>& context) {
...
GTEST_TEST(SimulatorTest, StretchedStep) {
    ...
    // 1.0s.
    analysis_test::MySpringMassSystem<double> spring_mass(1., 1., 1. /* update rate */);
...
GTEST_TEST(SimulatorTest, Issue10443) {
    ...
    auto& periodic_logger = *builder.AddSystem<VectorLogSink<double>>(kSize, 1.0 / kFrequency);
    builder.Connect(integrator.get_output_port(), periodic_logger.get_input_port());
...
GTEST_TEST(SimulatorTest, ArtificalLimitingStep) {
    ...
    integrator.IntegrateNoFurtherThanTime(inf, context.get_time() + 1.0, context.get_time() + 1.0);
    ...
    integrator.IntegrateNoFurtherThanTime(inf, context.get_time() + desired_h, inf);
...
GTEST_TEST(SimulatorTest, PerStepAction) {
    ...
    public:
        ...
        void AddPerStepPublishEvent() { DeclarePerStepPublishEvent(&PerStepActionTestSystem::HandlePublish); }

        void AddPerStepDiscreteUpdateEvent() {
            DeclarePerStepDiscreteUpdateEvent(&PerStepActionTestSystem::HandleDiscrete);
        }

        void AddPerStepUnrestrictedUpdateEvent() {
            DeclarePerStepUnrestrictedUpdateEvent(&PerStepActionTestSystem::HandleUnrestricted);
        }

        const std::vector<double>& get_publish_times() const { return publish_times_; }

        const std::vector<double>& get_discrete_update_times() const { return discrete_update_times_; }

        const std::vector<double>& get_unrestricted_update_times() const { return unrestricted_update_times_; }
    ...
    private:
        ...
        EventStatus HandleDiscrete(const Context<double>& context, DiscreteValues<double>*) const {
            discrete_update_times_.push_back(context.get_time());
            return EventStatus::Succeeded();
        }

        EventStatus HandleUnrestricted(const Context<double>& context, State<double>*) const {
...
GTEST_TEST(SimulatorTest, Initialization) {
    ...
    public:
        InitializationTestSystem() {
            PublishEvent<double> pub_event([](const System<double>& system, const Context<double>& callback_context,
                                              const PublishEvent<double>& callback_event) {
                dynamic_cast<const InitializationTestSystem&>(system).InitPublish(callback_context, callback_event);
                return EventStatus::Succeeded();
            });
            DeclareInitializationEvent(pub_event);

            DeclareInitializationDiscreteUpdateEvent(&InitializationTestSystem::InitializationDiscreteUpdateHandler);
            DeclareInitializationUnrestrictedUpdateEvent(
                    &InitializationTestSystem::InitializationUnrestrictedUpdateHandler);

            DeclarePeriodicDiscreteUpdateEvent(0.1, 0.0, &InitializationTestSystem::PeriodicDiscreteUpdateHandler);
            DeclarePerStepUnrestrictedUpdateEvent(&InitializationTestSystem::PerStepUnrestrictedUpdateHandler);
        }
    ...
    private:
        void InitPublish(const Context<double>& context, const PublishEvent<double>& event) const {
            EXPECT_EQ(context.get_time(), 0);
            EXPECT_EQ(event.get_trigger_type(), TriggerType::kInitialization);
            pub_init_ = true;
        }

        EventStatus InitializationDiscreteUpdateHandler(const Context<double>& context, DiscreteValues<double>*) const {
            EXPECT_EQ(context.get_time(), 0);
            dis_update_init_ = true;
            return EventStatus::Succeeded();
        }

        EventStatus PeriodicDiscreteUpdateHandler(const Context<double>&, DiscreteValues<double>*) const {
            return EventStatus::DidNothing();
        }

        EventStatus InitializationUnrestrictedUpdateHandler(const Context<double>& context, State<double>*) const {
            EXPECT_EQ(context.get_time(), 0);
            unres_update_init_ = true;
            return EventStatus::Succeeded();
        }

        EventStatus PerStepUnrestrictedUpdateHandler(const Context<double>& context, State<double>*) const {
            return EventStatus::DidNothing();
        }
...
GTEST_TEST(SimulatorTest, OwnedSystemTest) {
    ...
    Simulator<double> simulator_w_system(std::make_unique<ExampleDiagram>(offset));

    // Check that my System reference is still valid.
    EXPECT_NE(dynamic_cast<const ExampleDiagram*>(&simulator_w_system.get_system()), nullptr);
}
...
public:
    ...
    WastefulIntegrator(const System<double>& system, double max_step_size, Context<double>* context = nullptr)
...
private:
    bool DoStep(const double& h) final {
        ...
        const ContinuousState<double>& xc_deriv = this->EvalTimeDerivatives(context);
        const VectorBase<double>& xcdot0 = xc_deriv.get_vector();
        VectorBase<double>& xc = context.SetTimeAndGetMutableContinuousStateVector(context.get_time() + h);
...
GTEST_TEST(SimulatorTest, MonitorFunctionAndStatusReturn) {
    ...
    const auto monitor = [&states](const Context<double>& root_context) {
        ...
        vector << root_context.get_time(), root_context.get_continuous_state_vector().CopyToVector();
    ...
    EXPECT_THAT(status.FormatMessage(),
                ::testing::MatchesRegex("Simulator successfully reached the boundary time.*1.*"));
    ...
    const auto good_monitor = [](const Context<double>& root_context) {
        ...
        return time < 3.49 ? EventStatus::Succeeded() : EventStatus::ReachedTermination(nullptr, "All done.");
    };
    ...
    EXPECT_THAT(status.FormatMessage(),
                ::testing::MatchesRegex("Simulator returned early.*3\\.5.*because.*requested termination.*"
    ...
    const auto bad_monitor = [&spring_mass](const Context<double>& root_context) {
        ...
        return time < 5.99 ? EventStatus::Succeeded()
                           : EventStatus::Failed(&spring_mass, "Something terrible happened.");
    };
    simulator.set_monitor(bad_monitor);
    DRAKE_EXPECT_THROWS_MESSAGE(simulator.AdvanceTo(10.),
...
public:
    ...
    template <TriggerType trigger_type, UpdateType update_type, int which, typename... Args>
    ...
    void SetSeverity(TriggerType trigger_type, UpdateType update_type, int which, EventStatus::Severity severity) {
...
private:
    ...
    EventStatus MakeStatus(TriggerType trigger_type, UpdateType update_type, int which, double context_time) const {
        DRAKE_DEMAND(which == 0 || which == 1);
        const std::string description = fmt::format("{} {} event {} at {}",
        ...
        switch (event_severities_.at(Summary{trigger_type, update_type, which})) {
            ...
            case EventStatus::kReachedTermination:
                return EventStatus::ReachedTermination(this, fmt::format("{} terminated", description));
    ...
    void MakeOneOfEach() {
        DeclareInitializationUnrestrictedUpdateEvent(
                &EventStatusTestSystem::template GenericHandler<TriggerType::kInitialization, UpdateType::kUnrestricted,
                                                                which, State<double>*>);
        DeclareInitializationDiscreteUpdateEvent(
                &EventStatusTestSystem::template GenericHandler<TriggerType::kInitialization, UpdateType::kDiscrete,
                                                                which, DiscreteValues<double>*>);
        DeclareInitializationPublishEvent(&EventStatusTestSystem::template GenericHandler<TriggerType::kInitialization,
                                                                                          UpdateType::kPublish, which>);

        DeclarePerStepPublishEvent(
                &EventStatusTestSystem::template GenericHandler<TriggerType::kPerStep, UpdateType::kPublish, which>);

        const double period = 0.5;
        const double offset = 0.0;
        DeclarePeriodicUnrestrictedUpdateEvent(
                period, offset,
                &EventStatusTestSystem::template GenericHandler<TriggerType::kPeriodic, UpdateType::kUnrestricted,
                                                                which, State<double>*>);
        DeclarePeriodicDiscreteUpdateEvent(
                period, offset,
                &EventStatusTestSystem::template GenericHandler<TriggerType::kPeriodic, UpdateType::kDiscrete, which,
                                                                DiscreteValues<double>*>);
        DeclarePeriodicPublishEvent(
                period, offset,
                &EventStatusTestSystem::template GenericHandler<TriggerType::kPeriodic, UpdateType::kPublish, which>);

        // Set return status to "succeeded" initially.
        for (auto trigger_type : {TriggerType::kInitialization, TriggerType::kPerStep, TriggerType::kPeriodic}) {
            for (auto update_type : {UpdateType::kUnrestricted, UpdateType::kDiscrete, UpdateType::kPublish}) {
                if (trigger_type == TriggerType::kPerStep && update_type != UpdateType::kPublish) {
                    continue;
                }
                event_severities_[Summary{trigger_type, update_type, which}] = EventStatus::kSucceeded;
            }
        }
    }
...
GTEST_TEST(SimulatorTest, EventStatusReturnHandlingForInitialize) {
    ...
    const std::string all_events[] = {"initialization unrestricted update event 0 at 0.125",
    ...
    // failed.
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kUnrestricted, 1, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(sim.Initialize(), fmt::format("Simulator stopped.*"
                                                              "unrestricted update event 1.*failed.*"));
    EXPECT_THAT(system.take_static_events(), ElementsAre("initialization unrestricted update event 0 at 0.125",
                                                         "initialization unrestricted update event 1 at 0.125"));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 0);
    EXPECT_EQ(sim.get_num_discrete_updates(), 0);
    EXPECT_EQ(sim.get_num_publishes(), 0);
    ...
    // events.
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kUnrestricted, 1, EventStatus::kReachedTermination);
    status = sim.Initialize();
    EXPECT_THAT(system.take_static_events(), ElementsAre("initialization unrestricted update event 0 at 0.125",
                                                         "initialization unrestricted update event 1 at 0.125",
                                                         "initialization discrete update event 0 at 0.125",
    ...
    EXPECT_THAT(status.FormatMessage(),
                ::testing::MatchesRegex(fmt::format("Simulator returned early.*"
    ...
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kUnrestricted, 1, EventStatus::kReachedTermination);
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kDiscrete, 0, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(sim.Initialize(), fmt::format("Simulator stopped.*"
    ...
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kPublish, 0, EventStatus::kReachedTermination);
    status = sim.Initialize();
    EXPECT_THAT(system.take_static_events(), ElementsAreArray(all_events));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 1);
    EXPECT_EQ(sim.get_num_discrete_updates(), 1);
    ...
    EXPECT_EQ(status.reason(), SimulatorStatus::kReachedTerminationCondition);
    EXPECT_EQ(status.system(), &system);
    EXPECT_THAT(status.FormatMessage(), ::testing::MatchesRegex(fmt::format("Simulator returned early.*"
    ...
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kPublish, 0, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(sim.Initialize(), fmt::format("Simulator stopped.*"
    ...
    reset();
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kPublish, 0, EventStatus::kFailed);
    system.SetSeverity(TriggerType::kInitialization, UpdateType::kPublish, 1, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(sim.Initialize(), fmt::format("Simulator stopped.*"
                                                              "publish event 0.*failed.*"));
    EXPECT_THAT(system.take_static_events(), ElementsAreArray(all_events));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 1);
...
GTEST_TEST(SimulatorTest, EventStatusReturnHandlingForAdvanceTo) {
    ...
    const std::string all_events[] = {"per-step publish event 0 at 0.5",
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kUnrestricted, 1, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(status = sim.AdvanceTo(boundary_time),
                                fmt::format("Simulator stopped.*"
                                            "periodic unrestricted update event 1.*failed.*"));
    EXPECT_THAT(
            system.take_static_events(),
            ElementsAre("per-step publish event 0 at 0.5", "per-step publish event 1 at 0.5",
                        "periodic publish event 0 at 0.5", "periodic publish event 1 at 0.5",
                        "periodic unrestricted update event 0 at 0.5", "periodic unrestricted update event 1 at 0.5"));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 0);
    EXPECT_EQ(sim.get_num_discrete_updates(), 0);
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kPublish, 0, EventStatus::kReachedTermination);
    status = sim.AdvanceTo(boundary_time);
    ...
    EXPECT_THAT(
            system.take_static_events(),
            ElementsAre("periodic unrestricted update event 0 at 0.5", "periodic unrestricted update event 1 at 0.5",
                        "periodic discrete update event 0 at 0.5", "periodic discrete update event 1 at 0.5",
                        "per-step publish event 0 at 0.5", "per-step publish event 1 at 0.5"));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 1);
    EXPECT_EQ(sim.get_num_discrete_updates(), 1);
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kUnrestricted, 1, EventStatus::kReachedTermination);
    status = sim.AdvanceTo(boundary_time);
    EXPECT_THAT(
            system.take_static_events(),
            ElementsAre("per-step publish event 0 at 0.5", "per-step publish event 1 at 0.5",
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kUnrestricted, 1, EventStatus::kReachedTermination);
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kDiscrete, 0, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(status = sim.AdvanceTo(boundary_time),
                                fmt::format("Simulator stopped.*"
    ...
    EXPECT_THAT(system.take_static_events(),
                ElementsAre("per-step publish event 0 at 0.5", "per-step publish event 1 at 0.5",
                            "periodic publish event 0 at 0.5", "periodic publish event 1 at 0.5",
                            "periodic unrestricted update event 0 at 0.5",
                            "periodic unrestricted update event 1 at 0.5", "periodic discrete update event 0 at 0.5"));
    EXPECT_EQ(sim.get_num_unrestricted_updates(), 1);
    EXPECT_EQ(sim.get_num_discrete_updates(), 0);
    EXPECT_EQ(sim.get_num_publishes(), 1);  // the one at 0.5 (end of 1st step)
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kPublish, 0, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(status = sim.AdvanceTo(boundary_time),
                                fmt::format("Simulator stopped.*"
    ...
    reset();
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kPublish, 0, EventStatus::kFailed);
    system.SetSeverity(TriggerType::kPeriodic, UpdateType::kPublish, 1, EventStatus::kFailed);
    DRAKE_EXPECT_THROWS_MESSAGE(status = sim.AdvanceTo(boundary_time),
                                fmt::format("Simulator stopped.*"
                                            "periodic publish event 0.*failed.*"));
    EXPECT_THAT(system.take_static_events(),
...
GTEST_TEST(SimulatorTest, MissedPublishEventIssue13296) {
    ...
    public:
        ...
        void set_message_is_waiting(bool message_is_waiting) { message_is_waiting_ = message_is_waiting; }
    ...
    private:
        void DoCalcNextUpdateTime(const Context<double>& context,
            ...
            *next_update_time = message_is_waiting_ ? context.get_time() : inf;
            PublishEvent<double> event([](const System<double>& system, const Context<double>& callback_context,
                                          const PublishEvent<double>& callback_event) {
                dynamic_cast<const RightNowEventSystem&>(system).MyPublishHandler(callback_context, callback_event);
                return EventStatus::Succeeded();
            });
            event.AddToComposite(TriggerType::kTimed, event_info);
        }

        void MyPublishHandler(const Context<double>& context, const PublishEvent<double>&) const { ++publish_counter_; }
    ...
    public:
        PeriodicEventSystem() { this->DeclarePeriodicPublishEvent(0.25, 0., &PeriodicEventSystem::MakeItCount); }
    ...
    double next_update_time = simulator.get_system().CalcNextUpdateTime(simulator.get_context(), events.get());
    ...
    next_update_time = simulator.get_system().CalcNextUpdateTime(simulator.get_context(), events.get());
