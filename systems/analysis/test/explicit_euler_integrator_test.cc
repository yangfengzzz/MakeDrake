...
GTEST_TEST(IntegratorTest, MiscAPI) {
    ...
    ExplicitEulerIntegrator<double> int_dbl(spring_mass_dbl, h, context_dbl.get());
    ExplicitEulerIntegrator<AutoDiffXd> int_ad(spring_mass_ad, h, context_ad.get());
...
GTEST_TEST(IntegratorTest, ContextAccess) {
    ...
    ExplicitEulerIntegrator<double> integrator(spring_mass, h, context.get());  // Use default Context.
...
GTEST_TEST(IntegratorTest, InvalidDts) {
    ...
    const double t_final = context->get_time() + h;
    DRAKE_EXPECT_NO_THROW(integrator.IntegrateNoFurtherThanTime(t_final, t_final, t_final));
    EXPECT_THROW(integrator.IntegrateNoFurtherThanTime(t_final, -1, t_final), std::logic_error);
    EXPECT_THROW(integrator.IntegrateNoFurtherThanTime(-1, t_final, t_final), std::logic_error);
    EXPECT_THROW(integrator.IntegrateNoFurtherThanTime(t_final, t_final, -1), std::logic_error);
}
...
GTEST_TEST(IntegratorTest, AccuracyEstAndErrorControl) {
    ...
    EXPECT_THROW(integrator.request_initial_step_size_target(h), std::logic_error);
}
...
GTEST_TEST(IntegratorTest, MagDisparity) {
    ...
    ASSERT_TRUE(integrator.IntegrateWithSingleFixedStepToTime(context->get_time() + h));
}
...
GTEST_TEST(IntegratorTest, SpringMassStep) {
    ...
    double t;
    for (t = 0.0; std::abs(t - t_final) > h; t += h) integrator.IntegrateNoFurtherThanTime(inf, inf, t_final);
    ...
    // Check the solution.
    EXPECT_NEAR(c1 * std::cos(omega * t) + c2 * std::sin(omega * t), x_final, 5e-3);
...
GTEST_TEST(IntegratorTest, StepSize) {
    ...
    // The step ends on the next publish time.
    {
        ...
        typename IntegratorBase<double>::StepResult result =
                integrator.IntegrateNoFurtherThanTime(publish_time, update_time, infinity);
    ...
    // The step ends on the simulation end time because it's shortest.
    {
        ...
        const double boundary_time = context->get_time() + boundary_dt;
        typename IntegratorBase<double>::StepResult result =
                integrator.IntegrateNoFurtherThanTime(publish_time, update_time, boundary_time);
...
GTEST_TEST(IntegratorTest, Symbolic) {
    ...
    auto context = spring_mass.CreateDefaultContext();
    // Create the integrator.
    ExplicitEulerIntegrator<Expression> integrator(spring_mass, max_h, context.get());
    integrator.Initialize();
