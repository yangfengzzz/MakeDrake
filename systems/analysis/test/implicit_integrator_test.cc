...
public:
    ...
    DummyImplicitIntegrator(const System<double>& system, Context<double>* context)
...
private:
    ...
    int64_t do_get_num_error_estimator_jacobian_evaluations() const final { return 0; }
    int64_t do_get_num_error_estimator_derivative_evaluations_for_jacobian() const final { return 0; }
    int64_t do_get_num_error_estimator_newton_raphson_iterations() const final { return 0; }
    int64_t do_get_num_error_estimator_iteration_matrix_factorizations() const final { return 0; }

    void DoResetCachedJacobianRelatedMatrices() final { has_reset_cached_matrices_ = true; }
...
GTEST_TEST(ImplicitIntegratorTest, IsUpdateZero) {
    ...
    std::unique_ptr<Context<double>> context = dummy_system.CreateDefaultContext();
...
GTEST_TEST(ImplicitIntegratorTest, SetComputationSchemeResetsCachedMatrices) {
    ...
    EXPECT_EQ(dummy_integrator.get_jacobian_computation_scheme(),
              ImplicitIntegrator<double>::JacobianComputationScheme::kForwardDifference);
    ...
    dummy_integrator.set_jacobian_computation_scheme(ImplicitIntegrator<double>::JacobianComputationScheme::kAutomatic);
