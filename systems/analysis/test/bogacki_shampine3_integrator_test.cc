...
#include "systems/analysis/test_utilities/explicit_error_controlled_integrator_test.h"
#include "systems/analysis/test_utilities/generic_integrator_test.h"
...
GTEST_TEST(BS3IntegratorErrorEstimatorTest, CubicTest) {
    ...
    const double allowable_3rd_order_error = expected_answer * std::numeric_limits<double>::epsilon();
    ...
    ASSERT_TRUE(bs3.IntegrateWithSingleFixedStepToTime(t_final));
    const double err_est_2h_2 = bs3.get_error_estimate()->get_vector().GetAtIndex(0);

    EXPECT_NEAR(err_est_2h_2, 1.0 / 8 * err_est_h, 10 * std::numeric_limits<double>::epsilon());
}
