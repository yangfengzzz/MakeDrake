...
public:
    ...
    void SetUp(benchmark::State&) override { builder_ = std::make_unique<DiagramBuilder<double>>(); }
...
std::unique_ptr<DiagramBuilder<double>> MakeDiagramBuilder(int num_systems, int depth) {
    ...
    for (int i = 0; i < num_systems; ++i) {
        if (depth == 0) {
            ...
            children.push_back(builder->AddSystem(MakeDiagramBuilder(num_systems, depth - 1)->Build()));
        }
...
BENCHMARK(DiagramBuild)->Unit(benchmark::kMillisecond)->Args({3, 0})->Args({30, 0})->Args({3, 1})->Args({3, 2});
