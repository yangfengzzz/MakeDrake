...
public:
    ...
    std::shared_ptr<SystemConstraintWrapper> Create(SystemConstraintIndex index,
        ...
        return std::make_shared<SystemConstraintWrapper>(
                ...
                std::forward<UpdateContextFromDecisionVariablesFunction<double>>(updater),
                std::forward<UpdateContextFromDecisionVariablesFunction<AutoDiffXd>>(updater), x_size);
    }
     ...
     */
    std::optional<std::vector<solvers::Binding<solvers::Constraint>>> MaybeCreateConstraintSymbolically(
     ...
     * parameters.
     */
    std::optional<solvers::Binding<solvers::Constraint>> MaybeCreateGenericConstraintSymbolically(
            SystemConstraintIndex index, const Context<symbolic::Expression>& context) const;

    /**
     * Getters for the system instantiated with AutoDiffXd.
     */
    const System<AutoDiffXd>& system_autodiff() const { return *system_autodiff_; }
