#include "systems/optimization/system_constraint_wrapper.h"
...
template <typename T>
void Updater1(const System<T>&, const Eigen::Ref<const VectorX<T>>& val, Context<T>* context) {
...
void TestDummySystemConstraint(const SystemConstraintWrapper& constraint,
    ...
    EXPECT_TRUE(CompareMatrices(constraint.upper_bound(), Eigen::Vector2d(kInf, kInf)));
    ...
    Updater1<AutoDiffXd>(constraint.system_autodiff(), val_autodiff, context_autodiff.get());
    AutoDiffVecXd y_autodiff_expected;
    DummySystemConstraintCalc<AutoDiffXd>(*context_autodiff, &y_autodiff_expected);
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff), math::ExtractValue(y_autodiff_expected), tol));
...
GTEST_TEST(SystemConstraintWrapperTest, BasicTest) {
    ...
    SystemConstraintWrapper constraint1(&system_double, system_autodiff.get(), system_double.constraint_index(),
    ...
    SystemConstraintWrapper constraint2(&system_double, nullptr, system_double.constraint_index(), *context_double,
                                        Updater1<double>, Updater1<AutoDiffXd>, 3);
    TestDummySystemConstraint(constraint2, system_double, context_double.get());
}

// Assumes x only contains the position q.
template <typename T>
void Updater2(const System<T>& plant, const Eigen::Ref<const VectorX<T>>& x, Context<T>* context) {
    dynamic_cast<const multibody::MultibodyPlant<T>&>(plant).SetPositions(context, x);
}

void TestFreeBodyPlantConstraint(const SystemConstraintWrapper& constraint, Context<double>* context_double) {
    ...
    Updater2<AutoDiffXd>(constraint.system_autodiff(), x_autodiff, context_autodiff.get());

    AutoDiffVecXd y_autodiff_expected;
    auto plant_autodiff = dynamic_cast<const multibody::MultibodyPlant<AutoDiffXd>*>(&(constraint.system_autodiff()));
    plant_autodiff->get_constraint(constraint.constraint_index()).Calc(*context_autodiff, &y_autodiff_expected);

    EXPECT_TRUE(CompareMatrices(math::ExtractValue(y_autodiff), math::ExtractValue(y_autodiff_expected), 3 * kEps));
    EXPECT_TRUE(
            CompareMatrices(math::ExtractGradient(y_autodiff), math::ExtractGradient(y_autodiff_expected), 3 * kEps));
}
...
TEST_F(FreeBodyPlantTest, FreeBodyPlantTest) {
    ...
    const Eigen::Quaterniond quat0(Eigen::AngleAxisd(0.2 * M_PI, Eigen::Vector3d(1, 2, 3).normalized()));
    ...
    SystemConstraintWrapper constraint1(plant_double_.get(), plant_autodiff_.get(), unit_quaternion_constraint_index_,
    ...
    SystemConstraintWrapper constraint2(plant_double_.get(), nullptr, unit_quaternion_constraint_index_,
                                        *context_double, Updater2<double>, Updater2<AutoDiffXd>, 7);
