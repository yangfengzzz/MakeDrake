...
public:
    DummySystem(bool with_abstract_state = false, bool with_abstract_parameter = false)
        ...
        this->constraint_index_ = this->DeclareInequalityConstraint(
                DummySystemConstraintCalc<T>, {Eigen::Vector2d(2, 0), std::nullopt}, "dummy_system_constraint");
    ...
    explicit DummySystem(const DummySystem<U>& other)
        : DummySystem(other.with_abstract_state(), other.with_abstract_parameter()) {}
...
template <typename T>
void CalcUnitQuaternion(const System<T>& system, const Context<T>& context, VectorX<T>* value) {
...
public:
    FreeBodyPlantTest() : plant_double_{std::make_unique<multibody::MultibodyPlant<double>>(0)} {
        const double mass{1};
        ...
        const multibody::SpatialInertia<double> M_BBo_B =
                multibody::SpatialInertia<double>::MakeFromCentralInertia(mass, p_BoBcm_B, I_BBcm_B);
        plant_double_->AddRigidBody("body", M_BBo_B);

        auto unit_quaternion_constraint = ExternalSystemConstraint::MakeForNonsymbolicScalars(
        ...
        this->unit_quaternion_constraint_index_ = plant_double_->AddExternalConstraint(unit_quaternion_constraint);

        plant_double_->Finalize();
        plant_autodiff_ = dynamic_pointer_cast<multibody::MultibodyPlant<AutoDiffXd>>(plant_double_->ToAutoDiffXd());
    }

    SystemConstraintIndex unit_quaternion_constraint_index() const { return unit_quaternion_constraint_index_; }
