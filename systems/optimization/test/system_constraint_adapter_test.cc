#include "systems/optimization/system_constraint_adapter.h"
...
#include "systems/optimization/test/system_optimization_test_util.h"
...
struct DummySystemUpdater1 {
    template <typename T>
    void operator()(const System<T>&, const Eigen::Ref<const VectorX<T>>& vars, Context<T>* context) {
...
GTEST_TEST(SystemConstraintAdapterTest, CreateSystemConstraintWrapper) {
    ...
    auto constraint1 = adapter.Create(system.constraint_index(), *context, DummySystemUpdater1{}, 4);
    EXPECT_TRUE(CompareMatrices(constraint1->lower_bound(), Eigen::Vector2d(2, 0)));
    EXPECT_TRUE(CompareMatrices(constraint1->upper_bound(), Eigen::Vector2d(kInf, kInf)));
    ...
    auto constraint2 = adapter.Create(system.constraint_index(), *context, selector2, 3);
...
GTEST_TEST(SystemConstraintAdapterTest, SolveDummySystemConstraint) {
    ...
    auto constraint = adapter.Create(system.constraint_index(), *context, DummySystemUpdater1{}, 4);
    ...
    init_val << 2, 3, 4, 35;
    solvers::MathematicalProgramResult result = solvers::Solve(prog, init_val, {});
    EXPECT_EQ(result.get_solution_result(), solvers::SolutionResult::kSolutionFound);
    const auto p_val = result.GetSolution(p);
    const auto x_val = result.GetSolution(x);
    const double tol = 1E-6;
    EXPECT_GE(p_val(0) * x_val(0) + x_val(1), 2 - tol);
    EXPECT_GE(x_val(2) - x_val(1) * x_val(1) - p_val(0) * x_val(0) * x_val(0), -tol);
}

GTEST_TEST(SystemConstraintAdapterTest, MaybeCreateGenericConstraintSymbolically) {
    DummySystem<double> system;
    ...
    std::optional<solvers::Binding<solvers::Constraint>> binding =
            adapter.MaybeCreateGenericConstraintSymbolically(system.constraint_index(), *context_symbolic);
    ...
    EXPECT_EQ(symbolic::Variables(binding->variables()), symbolic::Variables({a, b, c, t}));
    double a_val = 2;
    double b_val = 3;
    double c_val = 4;
    double t_val = 5;
    ...
    AutoDiffVecXd abct_autodiff =
            math::InitializeAutoDiff(Eigen::Vector4d(a_val, b_val, c_val, t_val), Eigen::Matrix4Xd::Identity(4, 4));

    // Implicitly relying on auto to all resolve to the same type.
    auto set_bound_variable_value = [&a, &b, &c, &t](const VectorX<symbolic::Variable>& bound_variables, auto a_value,
        ...
        for (int i = 0; i < bound_variables.rows(); ++i) {
            if (bound_variables(i).get_id() == a.get_id()) {
                ...
                throw std::runtime_error("The bound_variables should only include a, b and t.");
            }
        }
    ...
    context_double->get_mutable_continuous_state_vector().SetFromVector(Vector3<double>(a_val, x1_val, x2_val));
    ...
    context_autodiff->get_mutable_numeric_parameter(0).GetAtIndex(0) = abct_autodiff(1);
    context_double->get_mutable_discrete_state_vector().SetAtIndex(0, c_val);
    context_autodiff->get_mutable_discrete_state_vector().GetAtIndex(0) = abct_autodiff(2);
    ...
    binding->evaluator()->Eval(set_bound_variable_value(binding->variables(), a_val, b_val, c_val, t_val),
                               &constraint_val);
    ...
    binding->evaluator()->Eval(set_bound_variable_value(binding->variables(), abct_autodiff(0), abct_autodiff(1),
                                                        abct_autodiff(2), abct_autodiff(3)),
                               &constraint_autodiff);
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(constraint_autodiff),
                                math::ExtractValue(constraint_autodiff_expected), tol));
    EXPECT_TRUE(CompareMatrices(math::ExtractGradient(constraint_autodiff),
    ...
    EXPECT_FALSE(
            adapter.MaybeCreateGenericConstraintSymbolically(system.constraint_index(), *context_symbolic).has_value());
}

GTEST_TEST(SystemConstraintAdapterTest, MaybeCreateGenericConstraintSymbolicallyFailure) {
    auto check_failure = [](const DummySystem<double>& system, const std::string& failure_message) {
        SystemConstraintAdapter adapter(&system);
        auto context = adapter.system_symbolic().CreateDefaultContext();
        DRAKE_EXPECT_THROWS_MESSAGE(
                adapter.MaybeCreateGenericConstraintSymbolically(system.constraint_index(), *context), failure_message);
    };

    // With abstract state, no abstract parameters.
    DummySystem<double> system1(true, false);
    check_failure(system1, "SystemConstraintAdapter: cannot handle system with abstract state.*");
...
GTEST_TEST(SystemConstraintAdapterTest, ExternalSystemConstraint) {
    Eigen::Matrix2d A;
    A << 0, 1, 0, 0;
    LinearSystem<double> double_integrator(A, Eigen::Vector2d(0, 1), Eigen::Matrix2d::Identity(),
                                           Eigen::Vector2d::Zero());

    // Now add an external system constraint that we don't want the velocity to be
    // to high.
    ExternalSystemConstraint velocity_bound = ExternalSystemConstraint::MakeForAllScalars(
            "|velocity| < 5", SystemConstraintBounds(Vector1d(-5), Vector1d(5)),
            [](const auto& system, const auto& context, auto* value) {
                *value = context.get_continuous_state_vector().CopyToVector().template tail<1>();
            });
    const SystemConstraintIndex velocity_bound_index = double_integrator.AddExternalConstraint(velocity_bound);
...
void CheckBoundingBoxConstraint(const solvers::Binding<solvers::Constraint>& constraint,
                                const symbolic::Variable& var,
                                double lower,
                                double upper,
                                double tol = 1E-14) {
    const auto bounding_box = solvers::internal::BindingDynamicCast<solvers::BoundingBoxConstraint>(constraint);
...
void CheckLinearConstraint(const solvers::Binding<solvers::Constraint>& constraint,
                           const Eigen::Ref<const VectorX<symbolic::Variable>>& vars,
                           const Eigen::Ref<const Eigen::MatrixXd>& A,
                           const Eigen::Ref<const Eigen::VectorXd>& lower,
                           const Eigen::Ref<const Eigen::VectorXd>& upper,
                           double tol = 1E-14) {
    const auto linear_constraint = solvers::internal::BindingDynamicCast<solvers::LinearConstraint>(constraint);
    EXPECT_EQ(linear_constraint.variables(), vars);
    EXPECT_TRUE(CompareMatrices(linear_constraint.evaluator()->GetDenseA(), A, tol));
    EXPECT_TRUE(CompareMatrices(linear_constraint.evaluator()->lower_bound(), lower, tol));
    EXPECT_TRUE(CompareMatrices(linear_constraint.evaluator()->upper_bound(), upper, tol));
}

void CheckLinearEqualityConstraint(const solvers::Binding<solvers::Constraint>& constraint,
                                   const Eigen::Ref<const VectorX<symbolic::Variable>>& vars,
                                   const Eigen::Ref<const Eigen::MatrixXd>& A,
                                   const Eigen::Ref<const Eigen::VectorXd>& rhs,
                                   double tol = 1E-14) {
    const auto linear_eq_constraint =
            solvers::internal::BindingDynamicCast<solvers::LinearEqualityConstraint>(constraint);
    EXPECT_EQ(linear_eq_constraint.variables(), vars);
    EXPECT_TRUE(CompareMatrices(linear_eq_constraint.evaluator()->GetDenseA(), A, tol));
    EXPECT_TRUE(CompareMatrices(linear_eq_constraint.evaluator()->lower_bound(), rhs, tol));
}
...
GTEST_TEST(SystemConstraintAdapterTest, MaybeCreateConstraintSymbolically1) {
    ...
    // 1 <= 2 * x(0) + x(1) <= 4
    // x(0) + 2 * x(1) = 3
    ExternalSystemConstraint system_constraint = ExternalSystemConstraint::MakeForAllScalars(
    ...
    const SystemConstraintIndex system_constraint_index = double_integrator.AddExternalConstraint(system_constraint);
    ...
    context_symbolic->get_mutable_continuous_state_vector().SetFromVector(Vector2<symbolic::Expression>(a, b));

    auto constraints = adapter.MaybeCreateConstraintSymbolically(system_constraint_index, *context_symbolic);
    EXPECT_TRUE(constraints.has_value());
    ASSERT_EQ(constraints->size(), 3);
    CheckBoundingBoxConstraint(constraints.value()[0], a, -1, 4);
    CheckLinearConstraint(constraints.value()[1], Vector2<symbolic::Variable>(a, b), Eigen::RowVector2d(2, 1),
                          Vector1d(1), Vector1d(4));
    CheckLinearEqualityConstraint(constraints.value()[2], Vector2<symbolic::Variable>(a, b), Eigen::RowVector2d(1, 2),
                                  Vector1d(3));
    ...
    // 0 <= 2 * a + 2 * b <= 3
    // a + b = 1
    constraints = adapter.MaybeCreateConstraintSymbolically(system_constraint_index, *context_symbolic);
    EXPECT_TRUE(constraints.has_value());
    EXPECT_EQ(constraints->size(), 3);
    CheckLinearConstraint(constraints.value()[0], Vector2<symbolic::Variable>(a, b), Eigen::RowVector2d(1, 1),
                          Vector1d(-1), Vector1d(4));
    CheckLinearConstraint(constraints.value()[1], Vector2<symbolic::Variable>(a, b), Eigen::RowVector2d(2, 2),
                          Vector1d(0), Vector1d(3));
    CheckLinearEqualityConstraint(constraints.value()[2], Vector2<symbolic::Variable>(a, b), Eigen::RowVector2d(1, 1),
                                  Vector1d(1));
