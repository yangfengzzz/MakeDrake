...
namespace drake {
namespace systems {
SystemConstraintAdapter::SystemConstraintAdapter(const System<double>* const system)
...
SystemConstraintAdapter::MaybeCreateConstraintSymbolically(SystemConstraintIndex index,
                                                           const Context<symbolic::Expression>& context) const {
    if (!system_symbolic_) {
        return {};
    }
    const SystemConstraint<symbolic::Expression>& system_constraint = system_symbolic_->get_constraint(index);
    ...
    for (int i = 0; i < constraint_val.size(); ++i) {
        std::unique_ptr<solvers::Binding<solvers::Constraint>> linear_constraint =
                solvers::internal::MaybeParseLinearConstraint(constraint_val(i), system_constraint.bounds().lower()(i),
...
struct ContextIndex {
    ContextIndex(ContextElementKind kind_in, int group_index_in, int element_offset_in, int var_index_in)
        : kind{kind_in}, group_index{group_index_in}, element_offset{element_offset_in}, var_index{var_index_in} {}
...
public:
    UpdateContextForSymbolicSystemConstraint(std::vector<ContextIndex> context_indices,
                                             const std::optional<int>& time_var_index)
        : context_indices_(std::move(context_indices)), time_var_index_{time_var_index} {}

    template <typename T>
    void operator()(const System<T>&, const Eigen::Ref<const VectorX<T>>& x, Context<T>* context) const {
        // Time.
        ...
        for (const auto& item : context_indices_) {
            switch (item.kind) {
                case ContextElementKind::kContinuous: {
                    // Continuous state.
                    context->get_mutable_continuous_state_vector().GetAtIndex(item.element_offset) = x(item.var_index);
                    break;
                }
                case ContextElementKind::kDiscrete: {
                    // Discrete state.
                    context->get_mutable_discrete_state(item.group_index).GetAtIndex(item.element_offset) =
                            x(item.var_index);
                    break;
                }
                case ContextElementKind::kParam: {
                    // Numeric parameter.
                    context->get_mutable_numeric_parameter(item.group_index).GetAtIndex(item.element_offset) =
                            x(item.var_index);
                    break;
                }
            }
...
// false.
bool ParseSymbolicVariableOrConstant(const symbolic::Expression& expr,
...
std::optional<solvers::Binding<solvers::Constraint>> SystemConstraintAdapter::MaybeCreateGenericConstraintSymbolically(
        SystemConstraintIndex index, const Context<symbolic::Expression>& context) const {
    ...
    std::optional<int> variable_index{};
    // Time
    bool success = ParseSymbolicVariableOrConstant(context.get_time(), &map_var_to_index, &bound_variables,
                                                   &variable_index, &constant_val);
    if (!success) {
        return {};
    }
    ...
    for (int i = 0; i < context.get_continuous_state_vector().size(); ++i) {
        success = ParseSymbolicVariableOrConstant(context.get_continuous_state_vector().GetAtIndex(i),
                                                  &map_var_to_index, &bound_variables, &variable_index, &constant_val);
        if (!success) {
            return {};
        }
        if (variable_index.has_value()) {
            context_indices.emplace_back(ContextElementKind::kContinuous, 0, i, *variable_index);
        } else {
            DRAKE_ASSERT(constant_val.has_value());
            context_fixed->get_mutable_continuous_state_vector().GetAtIndex(i) = constant_val.value();
        }
    }
    // Discrete state.
    for (int i = 0; i < context.num_discrete_state_groups(); ++i) {
        for (int j = 0; j < context.get_discrete_state(i).size(); ++j) {
            success = ParseSymbolicVariableOrConstant(context.get_discrete_state(i).GetAtIndex(j), &map_var_to_index,
                                                      &bound_variables, &variable_index, &constant_val);
            if (!success) {
                return {};
            }
            if (variable_index.has_value()) {
                context_indices.emplace_back(ContextElementKind::kDiscrete, i, j, *variable_index);
            } else {
                DRAKE_ASSERT(constant_val.has_value());
                context_fixed->get_mutable_discrete_state(i).GetAtIndex(j) = constant_val.value();
            }
        }
    }
    // numeric parameters.
    for (int i = 0; i < context.num_numeric_parameter_groups(); ++i) {
        for (int j = 0; j < context.get_numeric_parameter(i).size(); ++j) {
            success = ParseSymbolicVariableOrConstant(context.get_numeric_parameter(i).GetAtIndex(j), &map_var_to_index,
                                                      &bound_variables, &variable_index, &constant_val);
            if (!success) {
                return {};
            }
            if (variable_index.has_value()) {
                context_indices.emplace_back(ContextElementKind::kParam, i, j, *variable_index);
            } else {
                DRAKE_ASSERT(constant_val.has_value());
                context_fixed->get_mutable_numeric_parameter(i).GetAtIndex(j) = constant_val.value();
            }
        }
    }
    ...
    UpdateContextForSymbolicSystemConstraint updater(context_indices, time_var_index);

    return solvers::Binding<solvers::Constraint>(this->Create(index, *context_fixed, updater, map_var_to_index.size()),
                                                 bound_variables);
}

const System<symbolic::Expression>& SystemConstraintAdapter::system_symbolic() const {
    if (system_symbolic_) {
