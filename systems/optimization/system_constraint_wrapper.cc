...
SystemConstraintWrapper::SystemConstraintWrapper(
        ...
        int x_size)
    : solvers::Constraint(system_double->get_constraint(index).size(),
                          x_size,
      ...
      owned_system_autodiff_{system_autodiff == nullptr ? system_double_->ToAutoDiffXd() : nullptr},
      system_autodiff_{system_autodiff == nullptr ? owned_system_autodiff_.get() : system_autodiff},
      index_{index},
...
void SystemConstraintWrapper::DoEval(const Eigen::Ref<const Eigen::VectorXd>& x, Eigen::VectorXd* y) const {
    updater_double_(*system_double_, x, context_double_.get());
    system_double_->get_constraint(index_).Calc(*context_double_, y);
}

void SystemConstraintWrapper::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
    updater_autodiff_(system_autodiff(), x, context_autodiff_.get());
    system_autodiff().get_constraint(index_).Calc(*context_autodiff_, y);
}

void SystemConstraintWrapper::DoEval(const Eigen::Ref<const VectorX<symbolic::Variable>>&,
                                     VectorX<symbolic::Expression>*) const {
    throw std::runtime_error("SystemConstraintWrapper::DoEval does not support symbolic computation.");
}
}  // namespace systems
}  // namespace drake
