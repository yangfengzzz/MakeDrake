...
using UpdateContextFromDecisionVariablesFunction =
        std::function<void(const System<T>&, const Eigen::Ref<const VectorX<T>>&, Context<T>*)>;
...
private:
    ...
    const UpdateContextFromDecisionVariablesFunction<AutoDiffXd> updater_autodiff_;
};
}  // namespace systems
}  // namespace drake
