#include "systems/lcm/lcm_scope_system.h"
...
void Convert(const double time, const VectorX<double>& input, lcmt_scope* output) {
...
LcmScopeSystem::LcmScopeSystem(int size) {
    const InputPort<double>& input_port = this->DeclareVectorInputPort("input", size);
...
std::tuple<LcmScopeSystem*, LcmPublisherSystem*> LcmScopeSystem::AddToBuilder(systems::DiagramBuilder<double>* builder,
    ...
    auto* publisher = builder->AddSystem(LcmPublisherSystem::Make<lcmt_scope>(channel, lcm, publish_period));
