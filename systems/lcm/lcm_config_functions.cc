...
#include "systems/lcm/lcm_interface_system.h"
...
public:
    ...
    std::shared_ptr<DrakeSubscriptionInterface> Subscribe(const std::string&, HandlerFunction) final {
    ...
    std::shared_ptr<DrakeSubscriptionInterface> SubscribeAllChannels(MultichannelHandlerFunction) final {
        return MakeSubscription();
    }
...
LcmBuses ApplyLcmBusConfig(const std::map<std::string, DrakeLcmParams>& lcm_buses, DiagramBuilder<double>* builder) {
    DRAKE_THROW_UNLESS(builder != nullptr);
    LcmBuses result;
    for (const auto& [bus_name, lcm_params] : lcm_buses) {
        // Create the DrakeLcm.
        std::shared_ptr<DrakeLcmInterface> drake_lcm = std::make_shared<DrakeLcm>(lcm_params);
        ...
        auto* owner_system = builder->AddSystem<SharedPointerSystem<double>>(drake_lcm);
        ...
        auto* pumper_system = builder->AddSystem<LcmInterfaceSystem>(drake_lcm.get());
        pumper_system->set_name(fmt::format("LcmInterfaceSystem(bus_name={})", bus_name));

        // Display an update; provide the interface pointer to our caller.
        drake::log()->info("LCM bus '{}' created for URL {}", bus_name, canonical_url);
        result.Add(bus_name, pumper_system);
    }
    return result;
}

LcmBuses ApplyLcmBusConfig(const std::map<std::string, std::optional<DrakeLcmParams>>& lcm_buses,
    ...
    for (const auto& [bus_name, lcm_params] : lcm_buses) {
        if (lcm_params.has_value()) {
            ...
            new_map.emplace(bus_name, DrakeLcmParams{.lcm_url = LcmBuses::kLcmUrlMemqNull});
        }
    }
    ...
    if (lcm_buses == nullptr) {
        if (bus_name != "default") {
            throw std::runtime_error(
                    fmt::format("{} requested a non-default LCM bus '{}' but did not provide an"
                                " LcmBuses object to locate it",
                                description_of_caller, bus_name));
        }
        auto* owner_system = builder->AddSystem<SharedPointerSystem<double>>(std::make_shared<DrakeLcm>());
