    ...
    : lcm_interface_node_id_{get_node_id(lcm)},
      ...
      type_line_{message_type != nullptr
                         ? std::optional<std::string>{fmt::format(
                                   "type={}", NiceTypeName::RemoveNamespaces(NiceTypeName::Get(*message_type)))}
                         : std::nullopt},
      publish_{publish},
...
Result LcmSystemGraphviz::DecorateResult(Result&& result) {
    ...
    if (publish_) {
        new_result.fragments.push_back(fmt::format("{}:e -> {}in [style=\"dashed\", color=\"{}\"];\n", node_id_,
                                                   lcm_interface_node_id_, get_color()));
    }
    if (subscribe_) {
        new_result.fragments.push_back(fmt::format("{}out -> {}:w [style=\"dashed\", color=\"{}\"];\n",
                                                   lcm_interface_node_id_, node_id_, get_color()));
    }
    return new_result;
}

std::string LcmSystemGraphviz::get_node_id(const drake::lcm::DrakeLcmInterface& lcm) {
