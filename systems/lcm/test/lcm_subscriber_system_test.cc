    ...
    if (event_info->HasEvents()) {
        ...
        if (event_info->HasDiscreteUpdateEvents()) {
            const EventStatus status = sub.CalcDiscreteVariableUpdate(
                    *context, event_info->get_discrete_update_events(), &tmp_state->get_mutable_discrete_state());
            EXPECT_TRUE(status.succeeded());
        } else if (event_info->HasUnrestrictedUpdateEvents()) {
            const EventStatus status =
                    sub.CalcUnrestrictedUpdate(*context, event_info->get_unrestricted_update_events(), tmp_state.get());
            EXPECT_TRUE(status.succeeded());
        } else {
            DRAKE_DEMAND(false);
        }
...
struct SampleData {
    ...
    void PublishAndHandle(drake::lcm::DrakeLcmInterface* lcm, const std::string& channel_name) const {
...
GTEST_TEST(LcmSubscriberSystemTest, WaitTest) {
    ...
    wait();
    sample_data.PublishAndHandle(&lcm, channel_name);
    EXPECT_TRUE(CompareLcmtDrakeSignalMessages(future_message.get(), sample_data.value));
...
GTEST_TEST(LcmSubscriberSystemTest, Graphviz) {
    ...
    EXPECT_THAT(dut->GetGraphvizString(), testing::ContainsRegex("drakelcm[a-z0-9]* -> "));
