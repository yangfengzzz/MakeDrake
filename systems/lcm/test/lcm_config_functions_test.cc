...
#include "lcm/drake_lcm_params.h"
...
GTEST_TEST(LcmConfigFunctionsFindOrCreateTest, FindBus) {
    ...
    DrakeLcmInterface* result = FindOrCreateLcmBus(forced_result, &lcm_buses, &builder, description, bus_name);
    EXPECT_EQ(result, &special);

    // It's an error to use a non-existent bus_name.
    bus_name = "missing";
    DRAKE_EXPECT_THROWS_MESSAGE(FindOrCreateLcmBus(forced_result, &lcm_buses, &builder, description, bus_name),
...
GTEST_TEST(LcmConfigFunctionsFindOrCreateTest, IgnoredLcmBuses) {
    ...
    DrakeLcmInterface* result =
            FindOrCreateLcmBus(&forced_result, nullptr /* lcm_buses */, &builder, description, bus_name);
    ...
    result = FindOrCreateLcmBus(&forced_result, &empty_lcm_buses, &builder, description, bus_name);
    EXPECT_EQ(result, &forced_result);
    EXPECT_EQ(builder.GetSystems().size(), 0);
}
