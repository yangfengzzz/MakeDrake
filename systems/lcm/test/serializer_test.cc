...
GTEST_TEST(SerializerTest, BasicTest) {
  ...
  const lcmt_drake_signal sample_data{
    ...
    dut->Deserialize(message_bytes.data(), message_bytes.size(), abstract_value.get());
    EXPECT_TRUE(CompareLcmtDrakeSignalMessages(abstract_value->get_value<lcmt_drake_signal>(), sample_data));
}
