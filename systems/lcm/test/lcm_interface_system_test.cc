...
TEST_P(LcmInterfaceSystemTest, AcceptanceTest) {
    ...
    auto subscriber = builder.AddSystem(LcmSubscriberSystem::Make<lcmt_drake_signal>(channel, dut));
    ...
    auto thread = std::make_unique<std::thread>([&next_tx_timestamp, &publisher_lcm, channel]() {
...
INSTANTIATE_TEST_SUITE_P(test, LcmInterfaceSystemTest, ::testing::Values(0, 1, 2, 3));
