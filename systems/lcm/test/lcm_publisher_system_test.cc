...
#include "lcm/lcmt_drake_signal_utils.h"
#include "lcmt_drake_signal.hpp"
...
GTEST_TEST(LcmPublisherSystemTest, DefaultLcmTest) {
    ...
    DrakeLcm interface;
    auto dut1 = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface);
    EXPECT_EQ(&(dut1->lcm()), &interface);

    // Now leave out the LCM interface and check that a DrakeLcm gets allocated.
    auto dut2 = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, nullptr);
...
GTEST_TEST(LcmPublisherSystemTest, TestInitializationEvent) {
    ...
    dut1->AddInitializationMessage([&interface, &init_was_called](const Context<double>&, DrakeLcmInterface* lcm) {
    ...
    const EventStatus status = dut1->Publish(*context, init_events->get_publish_events());
...
GTEST_TEST(LcmPublisherSystemTest, SerializerTest) {
    ...
    auto dut = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface);
...
GTEST_TEST(LcmPublisherSystemTest, TestPerStepPublishTrigger) {
    ...
    auto dut = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface, {TriggerType::kPerStep});
...
GTEST_TEST(LcmPublisherSystemTest, TestForcedPublishTrigger) {
    ...
    auto dut = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface, {TriggerType::kForced});
...
public:
    TimeMessageSystem() { this->DeclareAbstractOutputPort("output", &TimeMessageSystem::CalcOutput); }

private:
    void CalcOutput(const Context<double>& context, lcmt_drake_signal* output) const {
        *output = {};
...
GTEST_TEST(LcmPublisherSystemTest, TestPublishPeriod) {
    ...
    auto dut = builder.AddSystem(
            LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface, kPublishPeriod, kPublishOffset));
    ...
    for (double time = 0; time < 4; time += 0.01) {
        ...
        const double expected_publish_time = std::floor(time / kPublishPeriod) * kPublishPeriod;
        EXPECT_EQ(sub.message().timestamp, expected_publish_time * 1000) << fmt::format("time = {}", time);
    }
...
GTEST_TEST(LcmPublisherSystemTest, TestPublishPeriodTrigger) {
    ...
    auto dut = LcmPublisherSystem::Make<lcmt_drake_signal>(channel_name, &interface, {TriggerType::kPeriodic},
...
GTEST_TEST(LcmPublisherSystemTest, TestPublishOffset) {
    ...
    for (double time = 0; time < 4; time += 0.01) {
        ...
        const double expected_publish_time =
                (time < kPublishOffset)
                        ? 0.0
                        : kPublishOffset + std::floor((time - kPublishOffset) / kPublishPeriod) * kPublishPeriod;
        EXPECT_EQ(sub.message().timestamp, expected_publish_time * 1000) << fmt::format("time = {}", time);
    }
