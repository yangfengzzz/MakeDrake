...
GTEST_TEST(LcmSystemGraphvizTest, Publish) {
    ...
    const Params params_orig{.node_id = "foo", .header_lines = {"existing header"}};
    const Params params = dut.DecorateParams(params_orig);
    EXPECT_EQ(params.node_id, "foo");
    EXPECT_THAT(params.header_lines, testing::ElementsAre(
                                             // Prior item.
                                             "existing header",
                                             // Newly-added items.
                                             "channel=SIGNAL", "type=lcmt_drake_signal"));

    const Result result_orig{.input_ports = {"foo:u0"}, .output_ports = {"foo:y0"}, .fragments = {"existing fragment"}};
    ...
    EXPECT_THAT(result.fragments.back(), testing::Not(testing::HasSubstr("out ->")));
}
...
GTEST_TEST(LcmSystemGraphvizTest, Subscribe) {
    ...
    EXPECT_THAT(result.fragments.back(), testing::Not(testing::HasSubstr("-> drakelcm")));
}
...
GTEST_TEST(LcmSystemGraphvizTest, TwoDifferentPublish) {
    ...
    std::vector<std::string> bravo_graphviz;
    {
        ...
        alpha_graphviz = absl::StrSplit(fmt::format("{}", fmt::join(result.fragments, "")), " ");
    }
    {
        LcmSystemGraphviz dut(bravo, "CHANNEL", nullptr, true, false);
        dut.DecorateParams(Params{.node_id = "bravo"});
        const Result result = dut.DecorateResult(Result{});
        bravo_graphviz = absl::StrSplit(fmt::format("{}", fmt::join(result.fragments, "")), " ");
    }
