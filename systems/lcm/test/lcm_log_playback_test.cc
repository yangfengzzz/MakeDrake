...
public:
    ...
    DummySys() {
        ...
        DeclarePeriodicPublishEvent(1.0 / publish_freq_, 0.0 /* no time offset */, &DummySys::SaveMessage);
    }

    const std::vector<lcmt_drake_signal>& get_received_msgs() const { return received_msgs_; }
    ...
    const std::vector<double> get_received_times() const {
        return std::vector<double>(received_time_.begin() + 1, received_time_.end());
    }

private:
    EventStatus SaveMessage(const Context<double>& context) const {
        const lcmt_drake_signal& msg = this->get_input_port(0).Eval<lcmt_drake_signal>(context);
...
void CheckLog() {
    ...
    auto sub0 = builder.AddSystem(LcmSubscriberSystem::Make<lcmt_drake_signal>("Ch0", &r_log));
    sub0->set_name("sub0");
    auto sub1 = builder.AddSystem(LcmSubscriberSystem::Make<lcmt_drake_signal>("Ch0", &r_log));
    sub1->set_name("sub1");
    auto sub2 = builder.AddSystem(LcmSubscriberSystem::Make<lcmt_drake_signal>("Ch1", &r_log));
    sub2->set_name("sub2");
    ...
    CheckLog({0.1, 0.3}, {1, 5}, "Ch0", printer0->get_received_msgs(), printer0->get_received_times());

    // printer1 should have the exact same msg as print0.
    CheckLog({0.1, 0.3}, {1, 5}, "Ch0", printer1->get_received_msgs(), printer1->get_received_times());

    // printer2 should have msg at t = [0.1, 0.3], with val = [2, 6, 7].
    CheckLog({0.22, 0.3, 0.4}, {2, 6, 7}, "Ch1", printer2->get_received_msgs(), printer2->get_received_times());
}
