#include "systems/lcm/lcm_subscriber_system.h"
...
LcmSubscriberSystem::LcmSubscriberSystem(const std::string& channel,
    ...
    : channel_(channel),
      ...
      wait_for_message_on_initialization_timeout_{wait_for_message_on_initialization_timeout},
      ...
      graphviz_{std::make_unique<internal::LcmSystemGraphviz>(*lcm,
                                                              channel_,
    ...
    subscription_ = lcm->Subscribe(channel_, [this](const void* buffer, int size) {
    ...
    auto message_state_index = this->DeclareAbstractState(*serializer_->CreateDefaultValue());
    ...
    // remove this feature.
    this->DeclareForcedUnrestrictedUpdateEvent(&LcmSubscriberSystem::ProcessMessageAndStoreToAbstractState);

    // On initialization, we process any existing received messages and maybe
    // wait for new messages.
    this->DeclareInitializationUnrestrictedUpdateEvent(&LcmSubscriberSystem::Initialize);
...
EventStatus LcmSubscriberSystem::ProcessMessageAndStoreToAbstractState(const Context<double>& context,
    ...
    serializer_->Deserialize(received_message_.data(), received_message_.size(),
                             &state->get_mutable_abstract_state().get_mutable_value(kStateIndexMessage));
    state->get_mutable_abstract_state<int>(kStateIndexMessageCount) = received_message_count_;
...
void LcmSubscriberSystem::DoCalcNextUpdateTime(const Context<double>& context,
    ...
    // function.
    auto callback = [](const System<double>& system, const Context<double>& callback_context,
                       const systems::UnrestrictedUpdateEvent<double>&, State<double>* callback_state) {
        const auto& self = dynamic_cast<const LcmSubscriberSystem&>(system);
        return self.ProcessMessageAndStoreToAbstractState(callback_context, callback_state);
    };

    // Schedule an update event at the current time.
    *time = context.get_time();
    EventCollection<UnrestrictedUpdateEvent<double>>& uu_events = events->get_mutable_unrestricted_update_events();
    uu_events.AddEvent(systems::UnrestrictedUpdateEvent<double>(TriggerType::kTimed, callback));
}
...
int LcmSubscriberSystem::WaitForMessage(int old_message_count, AbstractValue* message, double timeout) const {
    ...
    if (timeout <= 0) {
        ...
        const Duration requested = std::chrono::duration_cast<Duration>(std::chrono::duration<double>(timeout));
        ...
        const Duration duration = requested < kMaxDuration ? requested : kMaxDuration;
        DRAKE_ASSERT(TimePoint::max() - duration > Clock::now());
        if (!received_message_condition_variable_.wait_for(lock, duration, message_received)) {
            return received_message_count_;
        }
    }

    if (message) {
        serializer_->Deserialize(received_message_.data(), received_message_.size(), message);
    }
...
EventStatus LcmSubscriberSystem::Initialize(const Context<double>& context, State<double>* state) const {
    ...
    while (Duration(Clock::now() - start_time).count() < wait_for_message_on_initialization_timeout_) {
    ...
    // We ran out of time.
    result = EventStatus::Failed(this, fmt::format("Timed out without receiving any message on channel {} at url {}",
...
LeafSystem<double>::GraphvizFragment LcmSubscriberSystem::DoGetGraphvizFragment(
    ...
    return graphviz_->DecorateResult(LeafSystem<double>::DoGetGraphvizFragment(graphviz_->DecorateParams(params)));
}
