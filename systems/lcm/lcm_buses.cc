...
DrakeLcmInterface* LcmBuses::Find(std::string_view description_of_caller, const std::string& bus_name) const {
    ...
    throw std::runtime_error(
            fmt::format("{} requested an LCM bus '{}' that does not exist", description_of_caller, bus_name));
}
...
void LcmBuses::Add(std::string bus_name, DrakeLcmInterface* bus) {
    ...
    if (!inserted) {
        throw std::runtime_error(fmt::format("An LCM bus with name '{}' has already been defined", bus_name));
    }
}
