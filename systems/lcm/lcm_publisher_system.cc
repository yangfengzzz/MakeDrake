...
#include "lcm/drake_lcm_interface.h"
...
LcmPublisherSystem::LcmPublisherSystem(const std::string& channel,
    ...
    if (publish_triggers.find(TriggerType::kForced) != publish_triggers.end()) {
        this->DeclareForcedPublishEvent(&LcmPublisherSystem::PublishInputAsLcmMessage);
    }
    ...
    if (publish_triggers.find(TriggerType::kPeriodic) != publish_triggers.end()) {
        DRAKE_THROW_UNLESS(publish_period > 0.0);
        this->DeclarePeriodicPublishEvent(publish_period, publish_offset,
    ...
    if (publish_triggers.find(TriggerType::kPerStep) != publish_triggers.end()) {
        this->DeclarePerStepPublishEvent(&LcmPublisherSystem::PublishInputAsLcmMessage);
    }
}
...
void LcmPublisherSystem::AddInitializationMessage(InitializationPublisher initialization_publisher) {
...
EventStatus LcmPublisherSystem::Initialize(const Context<double>& context) const {
...
EventStatus LcmPublisherSystem::PublishInputAsLcmMessage(const Context<double>& context) const {
    ...
    lcm_->Publish(channel_, message_bytes.data(), message_bytes.size(), context.get_time());
...
LeafSystem<double>::GraphvizFragment LcmPublisherSystem::DoGetGraphvizFragment(
    ...
    internal::LcmSystemGraphviz lcm_system_graphviz(*lcm_, channel_,
                                                    &serializer_->CreateDefaultValue()->static_type_info(),
                                                    /* publish = */ true,
                                                    /* subscribe = */ false);
    return lcm_system_graphviz.DecorateResult(
            LeafSystem<double>::DoGetGraphvizFragment(lcm_system_graphviz.DecorateParams(params)));
}
