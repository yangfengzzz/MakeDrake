...
LcmInterfaceSystem::LcmInterfaceSystem(std::unique_ptr<DrakeLcmInterface> owned) : LcmInterfaceSystem(owned.get()) {
...
void LcmInterfaceSystem::Publish(const std::string& channel,
                                 const void* data,
                                 int data_size,
...
std::shared_ptr<DrakeSubscriptionInterface> LcmInterfaceSystem::Subscribe(const std::string& channel,
                                                                          HandlerFunction handler) {
    return lcm_->Subscribe(channel, std::move(handler));
}

std::shared_ptr<DrakeSubscriptionInterface> LcmInterfaceSystem::SubscribeMultichannel(
        std::string_view regex, MultichannelHandlerFunction handler) {
    return lcm_->SubscribeMultichannel(regex, std::move(handler));
}

std::shared_ptr<DrakeSubscriptionInterface> LcmInterfaceSystem::SubscribeAllChannels(
...
void LcmInterfaceSystem::OnHandleSubscriptionsError(const std::string& error_message) {
...
void LcmInterfaceSystem::DoCalcNextUpdateTime(const Context<double>& context,
...
LeafSystem<double>::GraphvizFragment LcmInterfaceSystem::DoGetGraphvizFragment(
    ...
    GraphvizFragment result = LeafSystem<double>::DoGetGraphvizFragment(new_params);
    result.fragments.push_back(fmt::format("{}in [color={}];", node_id, internal::LcmSystemGraphviz::get_color()));
    result.fragments.push_back(fmt::format("{}out [color={}];", node_id, internal::LcmSystemGraphviz::get_color()));
    return result;
}
