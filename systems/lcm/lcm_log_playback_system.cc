...
void LcmLogPlaybackSystem::DoCalcNextUpdateTime(const Context<double>& context,
    ...
    auto callback = [log = log_](const System<double>&, const Context<double>& callback_context,
    ...
    *time = next_message_time;
    events->get_mutable_publish_events().AddEvent(systems::PublishEvent<double>(TriggerType::kTimed, callback));
}
