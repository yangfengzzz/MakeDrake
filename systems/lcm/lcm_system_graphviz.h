...
public:
    ...
    SystemBase::GraphvizFragmentParams DecorateParams(const SystemBase::GraphvizFragmentParams& params);

    /* Rewrites the `result` to customize for LCM.
    @pre DecorateParams has already been called. */
    SystemBase::GraphvizFragment DecorateResult(SystemBase::GraphvizFragment&& result);
