...
std::vector<PerceptronActivationType> MakeDefaultActivations(int num, PerceptronActivationType activation_type) {
...
struct BackPropData {
    explicit BackPropData(int n)
        : Wx(n), Wx_plus_b(n), Xn(n), dXn_dWx_plus_b(n), dloss_dXn(n), dloss_dWx_plus_b(n), dloss_dW(n), dloss_db(n) {}
...
void Activation(PerceptronActivationType type,
...
template <typename T>
MultilayerPerceptron<T>::MultilayerPerceptron(const std::vector<int>& layers, PerceptronActivationType activation_type)
    : MultilayerPerceptron<T>(layers, MakeDefaultActivations(RejectEmpty(layers).size() - 1, activation_type)) {}

template <typename T>
MultilayerPerceptron<T>::MultilayerPerceptron(const std::vector<int>& layers,
                                              const std::vector<PerceptronActivationType>& activation_types)
    : MultilayerPerceptron<T>(std::vector<bool>(RejectEmpty(layers)[0], false),
                              std::vector<int>(RejectEmpty(layers).begin() + 1, layers.end()),
                              activation_types) {}

template <typename T>
MultilayerPerceptron<T>::MultilayerPerceptron(const std::vector<bool>& use_sin_cos_for_input,
    ...
    this->DeclareVectorOutputPort("y", layers_[num_weights_], &MultilayerPerceptron<T>::CalcOutput);
    ...
    this->DeclareNumericParameter(BasicVector<T>(VectorX<T>::Zero(num_parameters_)));
    ...
    calc_layers_cache_ =
            &this->DeclareCacheEntry("calc_layers", calc_layers_data, &MultilayerPerceptron<T>::CalcLayers);

    // Declare cache entry for Backpropagation:
    BackPropData<T> backprop_data(num_weights_);
    backprop_cache_ = &this->DeclareCacheEntry("backprop", ValueProducer(backprop_data, &ValueProducer::NoopCalc));
}

template <typename T>
template <typename U>
MultilayerPerceptron<T>::MultilayerPerceptron(const MultilayerPerceptron<U>& other)
    : MultilayerPerceptron<T>(other.use_sin_cos_for_input_,
                              std::vector<int>(other.layers().begin() + 1, other.layers().end()),
                              other.activation_types_) {}

template <typename T>
const VectorX<T>& MultilayerPerceptron<T>::GetParameters(const Context<T>& context) const {
    this->ValidateContext(context);
    return context.get_numeric_parameter(0).value();
}

template <typename T>
Eigen::VectorBlock<VectorX<T>> MultilayerPerceptron<T>::GetMutableParameters(Context<T>* context) const {
    this->ValidateContext(context);
    return context->get_mutable_numeric_parameter(0).get_mutable_value();
}

template <typename T>
void MultilayerPerceptron<T>::SetRandomParameters(const Context<T>&,
    ...
    for (int i = 0; i < num_weights_; ++i) {
        ...
        for (int n = weight_indices_[i]; n < weight_indices_[i] + layers_[i + 1] * layers_[i]; ++n) {
...
template <typename T>
void MultilayerPerceptron<T>::SetParameters(Context<T>* context, const Eigen::Ref<const VectorX<T>>& params) const {
...
template <typename T>
Eigen::Map<const MatrixX<T>> MultilayerPerceptron<T>::GetWeights(const Context<T>& context, int layer) const {
    this->ValidateContext(context);
    return GetWeights(context.get_numeric_parameter(0).value(), layer);
}

template <typename T>
Eigen::Map<const VectorX<T>> MultilayerPerceptron<T>::GetBiases(const Context<T>& context, int layer) const {
    this->ValidateContext(context);
    return GetBiases(context.get_numeric_parameter(0).value(), layer);
}

template <typename T>
void MultilayerPerceptron<T>::SetWeights(Context<T>* context, int layer, const Eigen::Ref<const MatrixX<T>>& W) const {
    ...
    Eigen::Map<MatrixX<T>>(params.get_mutable_value().data() + weight_indices_[layer], layers_[layer + 1],
                           layers_[layer]) = W;
}

template <typename T>
void MultilayerPerceptron<T>::SetBiases(Context<T>* context, int layer, const Eigen::Ref<const VectorX<T>>& b) const {
    DRAKE_DEMAND(layer >= 0 && layer < num_weights_);
    DRAKE_DEMAND(b.rows() == layers_[layer + 1]);
    this->ValidateContext(context);
    context->get_mutable_numeric_parameter(0).get_mutable_value().segment(bias_indices_[layer], layers_[layer + 1]) = b;
}

template <typename T>
Eigen::Map<const MatrixX<T>> MultilayerPerceptron<T>::GetWeights(const Eigen::Ref<const VectorX<T>>& params,
                                                                 int layer) const {
    DRAKE_DEMAND(layer >= 0 && layer < num_weights_);
    DRAKE_DEMAND(params.rows() == num_parameters_);
    return Eigen::Map<const MatrixX<T>>(params.data() + weight_indices_[layer], layers_[layer + 1], layers_[layer]);
}

template <typename T>
Eigen::Map<const VectorX<T>> MultilayerPerceptron<T>::GetBiases(const Eigen::Ref<const VectorX<T>>& params,
                                                                int layer) const {
    DRAKE_DEMAND(layer >= 0 && layer < num_weights_);
    DRAKE_DEMAND(params.rows() == num_parameters_);
    return Eigen::Map<const VectorX<T>>(params.data() + bias_indices_[layer], layers_[layer + 1]);
}

template <typename T>
void MultilayerPerceptron<T>::SetWeights(EigenPtr<VectorX<T>> params,
                                         int layer,
    ...
    DRAKE_DEMAND(W.rows() == layers_[layer + 1]);
    DRAKE_DEMAND(W.cols() == layers_[layer]);
    Eigen::Map<MatrixX<T>>(params->data() + weight_indices_[layer], layers_[layer + 1], layers_[layer]) = W;
}

template <typename T>
void MultilayerPerceptron<T>::SetBiases(EigenPtr<VectorX<T>> params,
                                        int layer,
...
T MultilayerPerceptron<T>::Backpropagation(
        ...
        const std::function<T(const Eigen::Ref<const MatrixX<T>>& Y, EigenPtr<MatrixX<T>> dloss_dY)>& loss,
    ...
    BackPropData<T>& data =
            backprop_cache_->get_mutable_cache_entry_value(context).template GetMutableValueOrThrow<BackPropData<T>>();
    // Forward pass:
    ...
    Activation<T, Eigen::Dynamic>(activation_types_[0], data.Wx_plus_b[0], &data.Xn[0], &data.dXn_dWx_plus_b[0]);
    for (int i = 1; i < num_weights_; ++i) {
        ...
        Activation<T, Eigen::Dynamic>(activation_types_[i], data.Wx_plus_b[i], &data.Xn[i], &data.dXn_dWx_plus_b[i]);
    }
    ...
    const T l = loss(data.Xn[num_weights_ - 1], &data.dloss_dXn[num_weights_ - 1]);
    // Backward pass:
    for (int i = num_weights_ - 1; i >= 0; --i) {
        data.dloss_dWx_plus_b[i] = (data.dloss_dXn[i].array() * data.dXn_dWx_plus_b[i].array()).matrix();
        data.dloss_dW[i].resize(layers_[i + 1], layers_[i]);
        data.dloss_dW[i].setZero();
        for (int j = 0; j < X.cols(); ++j) {
            if (i > 0) {
                data.dloss_dW[i].noalias() += data.dloss_dWx_plus_b[i].col(j) * data.Xn[i - 1].col(j).transpose();
            } else if (has_input_features_) {
                data.dloss_dW[i].noalias() += data.dloss_dWx_plus_b[i].col(j) * data.input_features.col(j).transpose();
            } else {
                data.dloss_dW[i].noalias() += data.dloss_dWx_plus_b[i].col(j) * X.col(j).transpose();
            }
        }
        ...
        if (i > 0) {
            data.dloss_dXn[i - 1].noalias() = GetWeights(context, i).transpose() * data.dloss_dWx_plus_b[i];
        }
    }
    return l;
}
...
template <typename T>
T MultilayerPerceptron<T>::BackpropagationMeanSquaredError(const Context<T>& context,
                                                           const Eigen::Ref<const MatrixX<T>>& X,
    ...
    DRAKE_DEMAND(Y_desired.rows() == layers_[num_weights_]);
    DRAKE_DEMAND(Y_desired.cols() == X.cols());
    auto MSE_loss = [&Y_desired](const Eigen::Ref<const MatrixX<T>>& Y, EigenPtr<MatrixX<T>> dloss_dY) {
...
void MultilayerPerceptron<T>::BatchOutput(const Context<T>& context,
    ...
    data.Wx_plus_b[0].noalias() = data.Wx[0].colwise() + GetBiases(context, 0);
    Activation<T, Eigen::Dynamic>(activation_types_[0], data.Wx_plus_b[0], &data.Xn[0],
                                  gradients ? &data.dXn_dWx_plus_b[0] : nullptr);
    for (int i = 1; i < num_weights_; ++i) {
        data.Wx[i].noalias() = GetWeights(context, i) * data.Xn[i - 1];
        data.Wx_plus_b[i].noalias() = data.Wx[i].colwise() + GetBiases(context, i);
        Activation<T, Eigen::Dynamic>(activation_types_[i], data.Wx_plus_b[i], &data.Xn[i],
    ...
    if (gradients) {
        ...
        for (int i = num_weights_ - 1; i >= 0; --i) {
            ...
            if (i > 0) {
                ...
                data.dloss_dinput_features.noalias() = GetWeights(context, 0).transpose() * data.dloss_dWx_plus_b[0];
                int feature_row = 0, input_row = 0;
                for (bool use_sin_cos : use_sin_cos_for_input_) {
                    if (use_sin_cos) {
                        dYdX->row(input_row) =
                                data.dloss_dinput_features.row(feature_row).array() * X.row(input_row).array().cos() -
                                data.dloss_dinput_features.row(feature_row + 1).array() *
                                        X.row(input_row).array().sin();
                        feature_row += 2;
                        ++input_row;
                    } else {
                        dYdX->row(input_row++) = data.dloss_dinput_features.row(feature_row++);
                    }
                }
            } else {
                dYdX->noalias() = GetWeights(context, 0).transpose() * data.dloss_dWx_plus_b[i];
            }
        }
    }
}
...
template <typename T>
void MultilayerPerceptron<T>::CalcOutput(const Context<T>& context, BasicVector<T>* y) const {
    this->ValidateContext(context);
    y->get_mutable_value() = calc_layers_cache_->Eval<internal::CalcLayersData<T>>(context).Xn[num_weights_ - 1];
}

template <typename T>
void MultilayerPerceptron<T>::CalcLayers(const Context<T>& context, internal::CalcLayersData<T>* data) const {
    if (has_input_features_) {
        CalcInputFeatures(this->get_input_port().Eval(context), &data->input_features);
        data->Wx[0].noalias() = GetWeights(context, 0) * data->input_features;
    } else {
        data->Wx[0].noalias() = GetWeights(context, 0) * this->get_input_port().Eval(context);
    }
    data->Wx_plus_b[0].noalias() = data->Wx[0].colwise() + GetBiases(context, 0);
...
template <typename T>
void MultilayerPerceptron<T>::CalcInputFeatures(const Eigen::Ref<const MatrixX<T>>& X,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::MultilayerPerceptron);
