...
template <typename T>
Adder<T>::Adder(int num_inputs, int size) : LeafSystem<T>(SystemTypeTag<Adder>{}) {
...
template <typename T>
template <typename U>
Adder<T>::Adder(const Adder<U>& other) : Adder<T>(other.num_input_ports(), other.get_input_port(0).size()) {}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Adder);
