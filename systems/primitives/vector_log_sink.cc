...
VectorLogSink<T>::VectorLogSink(int input_size, double publish_period)
    : VectorLogSink<T>(input_size,
                       (publish_period > 0.0) ? TriggerTypeSet({TriggerType::kForced, TriggerType::kPeriodic})
                                              : TriggerTypeSet({TriggerType::kForced, TriggerType::kPerStep}),
                       publish_period) {}

template <typename T>
VectorLogSink<T>::VectorLogSink(int input_size, const TriggerTypeSet& publish_triggers, double publish_period)
    ...
    log_cache_index_ = this->DeclareCacheEntry("log", ValueProducer(VectorLog<T>(input_size), &ValueProducer::NoopCalc),
                                               {this->nothing_ticket()})
                               .cache_index();
    ...
    if (publish_triggers.find(TriggerType::kPeriodic) != publish_triggers.end()) {
        ...
        this->DeclarePeriodicPublishEvent(publish_period, offset, &VectorLogSink<T>::WriteToLog);
    } else {
...
VectorLogSink<T>::VectorLogSink(const VectorLogSink<U>& other)
    : VectorLogSink<T>(other.get_input_port().size(), other.publish_triggers_, other.publish_period_) {}
...
template <typename T>
const VectorLog<T>& VectorLogSink<T>::FindLog(const Context<T>& root_context) const {
...
template <typename T>
VectorLog<T>& VectorLogSink<T>::GetLogFromCache(const Context<T>& context) const {
    this->ValidateContext(context);
    CacheEntryValue& value = this->get_cache_entry(log_cache_index_).get_mutable_cache_entry_value(context);
    return value.GetMutableValueOrThrow<VectorLog<T>>();
}

template <typename T>
EventStatus VectorLogSink<T>::WriteToLog(const Context<T>& context) const {
    GetLogFromCache(context).AddData(context.get_time(), this->get_input_port().Eval(context));
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::VectorLogSink);
