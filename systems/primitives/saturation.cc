...
Saturation<T>::Saturation(int input_size)
    : min_max_ports_enabled_(true),
      ...
      max_value_(VectorX<T>::Constant(input_size, std::numeric_limits<double>::infinity())),
      min_value_(VectorX<T>::Constant(input_size, -std::numeric_limits<double>::infinity())) {
    // Checks if input size is a positive integer.
    DRAKE_THROW_UNLESS(input_size_ > 0);

    // Input and outputs are of same dimension.
    input_port_index_ = this->DeclareInputPort(kUseDefaultName, kVectorValued, input_size_).get_index();
    max_value_port_index_ = this->DeclareInputPort(kUseDefaultName, kVectorValued, input_size_).get_index();
    min_value_port_index_ = this->DeclareInputPort(kUseDefaultName, kVectorValued, input_size_).get_index();
    this->DeclareVectorOutputPort(kUseDefaultName, input_size_, &Saturation::CalcSaturatedOutput).get_index();
}

template <typename T>
Saturation<T>::Saturation(const VectorX<T>& min_value, const VectorX<T>& max_value)
    : min_max_ports_enabled_(false), input_size_(min_value.size()), max_value_(max_value), min_value_(min_value) {
    // Checks if input size is a positive integer.
    DRAKE_THROW_UNLESS(input_size_ > 0);
...
template <typename T>
void Saturation<T>::CalcSaturatedOutput(const Context<T>& context, BasicVector<T>* output_vector) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Saturation);
