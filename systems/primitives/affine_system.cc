...
TimeVaryingAffineSystem<T>::TimeVaryingAffineSystem(
    ...
    if (num_states > 0) {
        if (time_period_ == 0.0) {
            ...
            this->DeclareForcedDiscreteUpdateEvent(&TimeVaryingAffineSystem<T>::CalcDiscreteUpdate);
        }
    }

    if (num_inputs_ > 0) this->DeclareInputPort(kUseDefaultName, kVectorValued, num_inputs_);
    if (num_outputs_ > 0) {
        ...
        // state, accuracy, etc.).
        this->DeclareVectorOutputPort(kUseDefaultName, num_outputs_, &TimeVaryingAffineSystem::CalcOutputY);
    }
}
...
template <typename T>
void TimeVaryingAffineSystem<T>::configure_default_state(const Eigen::Ref<const VectorX<T>>& x0) {
...
template <typename T>
void TimeVaryingAffineSystem<T>::configure_random_state(const Eigen::Ref<const Eigen::MatrixXd>& covariance) {
    DRAKE_DEMAND(covariance.rows() == num_states_);
    DRAKE_DEMAND(covariance.cols() == num_states_);
    if (num_states_ == 0) return;
    Sqrt_Sigma_x0_ = Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd>(covariance).operatorSqrt();
}

template <typename T>
template <typename U>
void TimeVaryingAffineSystem<T>::ConfigureDefaultAndRandomStateFrom(const TimeVaryingAffineSystem<U>& other) {
...
template <typename T>
void TimeVaryingAffineSystem<T>::CalcOutputY(const Context<T>& context, BasicVector<T>* output_vector) const {
...
template <typename T>
void TimeVaryingAffineSystem<T>::DoCalcTimeDerivatives(const Context<T>& context,
    ...
    const auto& x = dynamic_cast<const BasicVector<T>&>(context.get_continuous_state_vector()).get_value();
...
template <typename T>
EventStatus TimeVaryingAffineSystem<T>::CalcDiscreteUpdate(const Context<T>& context,
...
template <typename T>
void TimeVaryingAffineSystem<T>::SetDefaultState(const Context<T>& context, State<T>* state) const {
    unused(context);
    if (num_states_ == 0) return;
...
template <typename T>
void TimeVaryingAffineSystem<T>::SetRandomState(const Context<T>& context,
                                                State<T>* state,
...
AffineSystem<T>::AffineSystem(const Eigen::Ref<const Eigen::MatrixXd>& A,
                              ...
                              double time_period)
    : AffineSystem<T>(SystemTypeTag<AffineSystem>{}, A, B, f0, C, D, y0, time_period) {}
...
// correct size.
int CalcNumInputs(const Eigen::Ref<const Eigen::MatrixXd>& B, const Eigen::Ref<const Eigen::MatrixXd>& D) {
    int num_inputs = 0;
    if (B.size() > 0) {
        num_inputs = B.cols();
    }
...
void CompareMatrixSize(const Eigen::Ref<const Eigen::MatrixXd>& M1,
    ...
    if (M1.rows() != M2.rows() || M1.cols() != M2.cols()) {
        throw std::runtime_error(fmt::format("New and current {} have different sizes.", matrix_name));
    }
}
...
void CheckOutputConsistency(const bool is_meaningful,
    ...
    if (!is_meaningful && is_meaningful_new) {
        throw std::runtime_error(
                fmt::format("{} makes the output dependent on {}, when it was "
...
AffineSystem<T>::AffineSystem(SystemScalarConverter converter,
                              ...
                              const Eigen::Ref<const Eigen::VectorXd>& y0,
                              double time_period)
    : TimeVaryingAffineSystem<T>(std::move(converter),
                                 CalcNumStates(A, B, f0, C),
                                 CalcNumInputs(B, D),
                                 CalcNumOutputs(C, D, y0),
                                 time_period),
      A_(A.size() ? A : Eigen::MatrixXd::Zero(this->num_states(), this->num_states())),
      B_(B.size() ? B : Eigen::MatrixXd::Zero(this->num_states(), this->num_inputs())),
      f0_(f0.size() ? f0 : Eigen::VectorXd::Zero(this->num_states())),
      C_(C.size() ? C : Eigen::MatrixXd::Zero(this->num_outputs(), this->num_states())),
      D_(D.size() ? D : Eigen::MatrixXd::Zero(this->num_outputs(), this->num_inputs())),
...
AffineSystem<T>::AffineSystem(const AffineSystem<U>& other)
    : AffineSystem(other.A(), other.B(), other.f0(), other.C(), other.D(), other.y0(), other.time_period()) {
...
template <typename T>
void AffineSystem<T>::CalcOutputY(const Context<T>& context, BasicVector<T>* output_vector) const {
...
template <typename T>
void AffineSystem<T>::DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const {
...
template <typename T>
EventStatus AffineSystem<T>::CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* updates) const {
    if (this->num_states() == 0 || this->time_period() == 0.0) return EventStatus::DidNothing();
...
template <typename T>
void AffineSystem<T>::UpdateCoefficients(const Eigen::Ref<const Eigen::MatrixXd>& new_A,
    ...
    CheckOutputConsistency(has_meaningful_C_, is_new_C_meaningful, "new_C", "state");
    CheckOutputConsistency(has_meaningful_D_, is_new_D_meaningful, "new_D", "input");
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::TimeVaryingAffineSystem);

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::AffineSystem);
