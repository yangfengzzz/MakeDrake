...
protected:
    ...
    ContinuousState<double>& continuous_state() { return context_->get_mutable_continuous_state(); }
...
TEST_F(IntegratorTest, Output) {
    ...
    integrator_->get_input_port(0).FixValue(context_.get(), Eigen::Vector3d{1.0, 2.0, 3.0});
...
protected:
    void SetUp() override {
        ...
        symbolic_integrator_->get_input_port(0).FixValue(
                ...
                Vector3<symbolic::Expression>{symbolic::Variable("u0"), symbolic::Variable("u1"),
...
TEST_F(SymbolicIntegratorTest, Output) {
    const auto& out = symbolic_integrator_->get_output_port(0).Eval(*symbolic_context_);
...
TEST_F(SymbolicIntegratorTest, Derivatives) {
    symbolic_integrator_->CalcTimeDerivatives(*symbolic_context_, symbolic_derivatives_.get());
