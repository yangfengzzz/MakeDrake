...
protected:
    void SetUpSingleTimeConstantFilter() {
        filter_ = std::make_unique<FirstOrderLowPassFilter<double>>(kTimeConstant, kSignalSize);
        context_ = filter_->CreateDefaultContext();
        derivatives_ = filter_->AllocateTimeDerivatives();

        // Sets the state to zero initially.
        filter_->set_initial_output_value(context_.get(), Eigen::VectorXd::Zero(kSignalSize));
    }
...
TEST_F(FirstOrderLowPassFilterTest, Output) {
    ...
    filter_->get_input_port().FixValue(context_.get(), Vector3<double>(1.0, 2.0, 3.0));
...
TEST_F(FirstOrderLowPassFilterTest, ToAutoDiff) {
    ...
    EXPECT_TRUE(is_autodiffxd_convertible(*filter_, [&](const auto& converted) {
        ...
        EXPECT_EQ(Vector3<double>(4.0, 3.5, 3.0), converted.get_time_constants_vector());
    }));
}
