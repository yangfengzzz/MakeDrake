...
protected:
    ...
    void SetUp() override {
        ...
        if (!is_abstract_) {
            ...
            // Initialize to zero.
            hold_ = std::make_unique<ZeroOrderHold<double>>(kPeriod,
        ...
        if (!is_abstract_) {
            ...
            hold_->get_input_port().FixValue(&*context_, SimpleAbstractType(input_value_));
        }
    ...
    void CheckForUpdateAction() const {
        if (!is_abstract_) {
            CheckForSinglePeriodicEvent(leaf_info_->get_discrete_update_events().get_events());
        } else {
            CheckForSinglePeriodicEvent(leaf_info_->get_unrestricted_update_events().get_events());
        }
    }
...
GTEST_TEST(SimpleZeroOrderHoldTest, VectorPeriodAndOffset) {
    ...
    const std::optional<PeriodicEventData> data = dut.GetUniquePeriodicDiscreteUpdateAttribute();
...
GTEST_TEST(SimpleZeroOrderHoldTest, AbstractPeriodAndOffset) {
    ...
    const std::vector<const Event<double>*>& events = events_by_period_map.begin()->second;
...
TEST_P(ZeroOrderHoldTest, InitialState) {
    ...
    if (!is_abstract_) {
        ...
        const SimpleAbstractType& state_value = context_->get_abstract_state<SimpleAbstractType>(0);
...
TEST_P(ZeroOrderHoldTest, Output) {
    ...
    if (!is_abstract_) {
        ...
        SimpleAbstractType& state_value = context_->get_mutable_abstract_state<SimpleAbstractType>(0);
        state_value = SimpleAbstractType(output_expected);
        output = hold_->get_output_port().template Eval<SimpleAbstractType>(*context_).value();
    }
...
TEST_P(ZeroOrderHoldTest, Update) {
    ...
    if (!is_abstract_) {
        ...
        value = hold_->get_output_port().Eval<SimpleAbstractType>(*context_).value();
    }
...
INSTANTIATE_TEST_SUITE_P(test, ZeroOrderHoldTest, ::testing::Values(false, true));
...
GTEST_TEST(ZeroOrderHoldTest, UseInDiagram) {
    ...
    auto sine = builder.AddSystem<Sine<double>>(amplitude, frequency, phase, size);
    sine->set_name("sine");
...
protected:
    void SetUp() override {
        ...
        const int size = 1;
        hold_ = std::make_unique<ZeroOrderHold<symbolic::Expression>>(period_sec, size);

        // Initialize the context with symbolic variables.
        context_ = hold_->CreateDefaultContext();
        hold_->get_input_port().FixValue(context_.get(), symbolic::Expression(symbolic::Variable("u0")));
