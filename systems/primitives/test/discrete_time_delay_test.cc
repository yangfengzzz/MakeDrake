#include "systems/primitives/discrete_time_delay.h"
...
#include "systems/primitives/sine.h"
...
Eigen::VectorXd ConcatenateAbstractBufferToVector(Context<double>* context) {
    ...
    Eigen::VectorXd value = Eigen::VectorXd::Constant(kLength * (kBuffer + 1), -1);
    const int& oldest_index = context->template get_abstract_state<int>(kBuffer + 1);
    for (int ii = 0; ii < kBuffer + 1; ii++) {
        const SimpleAbstractType& next_value =
                context->get_abstract_state<SimpleAbstractType>((oldest_index + ii) % (kBuffer + 1));
...
protected:
    ...
    void SetUp() override {
        ...
        if (!is_abstract_) {
            delay_ = std::make_unique<DiscreteTimeDelay<double>>(kUpdatePeriod, kBuffer, kLength);
        } else {
            delay_ = std::make_unique<DiscreteTimeDelay<double>>(kUpdatePeriod, kBuffer,
        ...
        if (!is_abstract_) {
            ...
            delay_->get_input_port().FixValue(context_.get(), SimpleAbstractType(input_value_));
        }
    }
...
TEST_P(DiscreteTimeDelayTest, InitialState) {
    const Eigen::VectorXd value_expected = Eigen::VectorXd::Zero(kLength * (kBuffer + 1));
...
TEST_P(DiscreteTimeDelayTest, Output) {
    ...
    if (!is_abstract_) {
        ...
        const int& oldest_index = context_->template get_abstract_state<int>(kBuffer + 1);
        SimpleAbstractType& next_value = context_->get_mutable_abstract_state<SimpleAbstractType>(oldest_index);
        next_value = SimpleAbstractType(state_value_override_);
        output = delay_->get_output_port().template Eval<SimpleAbstractType>(*context_).value();
    }
...
TEST_P(DiscreteTimeDelayTest, Update) {
    ...
    Eigen::VectorXd value_expected = Eigen::VectorXd::Zero(kLength * (kBuffer + 1));
...
INSTANTIATE_TEST_SUITE_P(test, DiscreteTimeDelayTest, ::testing::Values(false, true));
...
GTEST_TEST(DiscreteTimeDelayTest, DiscreteSimulation) {
    ...
    auto counter = builder.AddSystem<AffineSystem<double>>(Vector1d::Constant(1.0),      // A.
    ...
    auto delay = builder.AddSystem<DiscreteTimeDelay<double>>(kUpdatePeriod, kBuffer, 1);
...
GTEST_TEST(DiscreteTimeDelayTest, ContinuousSimulation) {
    ...
    DiagramBuilder<double> builder;
    auto sine = builder.AddSystem<Sine<double>>(sine_amplitude, sine_frequency, sine_phase, size);
    sine->set_name("sine");

    auto delay = builder.AddSystem<DiscreteTimeDelay<double>>(kUpdatePeriod, kBuffer, size);
    delay->set_name("delay");
...
protected:
    void SetUp() override {
        const int size = 1;
        delay_ = std::make_unique<DiscreteTimeDelay<symbolic::Expression>>(kUpdatePeriod, kBuffer, size);

        // Initialize the context with symbolic variables.
        context_ = delay_->CreateDefaultContext();
        delay_->get_input_port().FixValue(context_.get(), symbolic::Expression(symbolic::Variable("u0")));
