...
class TrajectoryAffineSystemTest : public ::testing::TestWithParam<ConstructorType> {
public:
    void SetUp() override {
        if (this->GetParam() == ConstructorType::FromContinuous) {
            dut_ = make_unique<TrajectoryAffineSystem<double>>(data_.A, data_.B, data_.f0, data_.C, data_.D, data_.y0);
...
TEST_P(TrajectoryAffineSystemTest, DiscreteUpdates) {
    ...
    for (const double t : data_.times) {
        ...
        if (time_period_ == 0.) {
            ...
            EXPECT_TRUE(CompareMatrices(A * x + B * u + f0, derivatives_->get_vector().CopyToVector(), tol));
        } else {
            updates_->SetFrom(dut_->EvalUniquePeriodicDiscreteUpdate(*context_));
            EXPECT_TRUE(CompareMatrices(A * x + B * u + f0, updates_->get_vector(0).CopyToVector(), tol));
        }
    }
}
...
TEST_P(TrajectoryAffineSystemTest, Output) {
    ...
    for (const double t : data_.times) {
        ...
        EXPECT_TRUE(CompareMatrices(C * x + D * u + y0, dut_->get_output_port().Eval(*context_), tol));
    }
}
