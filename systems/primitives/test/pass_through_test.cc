...
class PassThroughTest : public ::testing::TestWithParam<std::tuple<bool, bool>> {
protected:
    PassThroughTest() : is_abstract_(std::get<0>(GetParam())), use_default_value_(std::get<1>(GetParam())) {}
    ...
    void SetUp() override {
        ...
        if (!is_abstract_) {
            ...
            if (use_default_value_) {
                pass_through_ = make_unique<PassThrough<double>>(Value<SimpleAbstractType>(default_value_));
            } else {
                pass_through_ = make_unique<PassThrough<double>>(Value<SimpleAbstractType>(size));
            }
        }
...
TEST_P(PassThroughTest, VectorThroughPassThroughSystem) {
    ...
    if (!is_abstract_) {
        ...
        output = pass_through_->get_output_port(0).Eval<SimpleAbstractType>(*context_).value();
    }
    if (use_default_value_) {
    ...
    if (!is_abstract_) {
        ...
        pass_through_->get_input_port(0).FixValue(context_.get(), SimpleAbstractType(input_value_));
    }
...
GTEST_TEST(PassThroughTest, AutoDiffFromDouble) {
    ...
    EXPECT_EQ(value, math::DiscardZeroGradient(pass.get_output_port().Eval(*context)));
}
