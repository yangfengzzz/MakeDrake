...
GTEST_TEST(TestVectorLogSink, LogFinders) {
    ...
    auto& logger_context = dut->GetMyMutableContextFromRoot(diagram_context.get());

    EXPECT_EQ(&(dut->GetLog(logger_context)), &(dut->FindLog(*diagram_context)));
    EXPECT_EQ(&(dut->FindLog(*diagram_context)), &(dut->FindMutableLog(diagram_context.get())));

    DRAKE_EXPECT_THROWS_MESSAGE(dut->GetLog(*diagram_context), ".*was passed the root Diagram's Context[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut->FindLog(logger_context), ".*given context must be a root context.*");
}
...
GTEST_TEST(TestVectorLogSink, LogFindersWrongContext) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut->GetLog(*wrong_diagram_context), ".*was passed the Context of .+ instead[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(dut->FindLog(*wrong_diagram_context), ".*was passed the Context of .+ instead[^]*");
}
...
GTEST_TEST(TestVectorLogSink, LinearSystemTest) {
    ...
    auto plant = builder.AddSystem<LinearSystem<double>>(Vector1d::Constant(-1.0),      // A.
...
GTEST_TEST(TestVectorLogSink, ForcedPublishOnly) {
    ...
    DiagramBuilder<double> builder;
    auto system = builder.AddSystem<ConstantVectorSource<double>>(2.0);
    auto logger = LogVectorOutput(system->get_output_port(), &builder, {TriggerType::kForced});
    auto diagram = builder.Build();
...
GTEST_TEST(TestVectorLogSink, NoForcedPublish) {
    ...
    DiagramBuilder<double> builder;
    auto system = builder.AddSystem<ConstantVectorSource<double>>(2.0);
    auto logger = LogVectorOutput(system->get_output_port(), &builder, {TriggerType::kPeriodic}, 0.125);
    auto diagram = builder.Build();
...
GTEST_TEST(TestVectorLogSink, ScalarConversion) {
    ...
    for (const auto* dut : {&dut_per_step_publish, &dut_forced_publish, &dut_periodic_publish}) {
