#include "systems/primitives/trajectory_source.h"
...
protected:
    ...
    void BuildSource(PiecewisePolynomial<double> pp) {
        ...
        source_ = make_unique<TrajectorySource<double>>(*kppTraj_, kDerivativeOrder, true);
...
TEST_F(TrajectorySourceTest, OutputTest) {
    ...
    for (size_t i = 1; i <= kDerivativeOrder; ++i) {
        EXPECT_TRUE(CompareMatrices(kppTraj_->derivative(i).value(kTestTime), output.segment(len * i, len), 1e-10,
    ...
    samples << 0, 1;
    PiecewisePolynomial<double> pp = PiecewisePolynomial<double>::FirstOrderHold(Eigen::Vector2d(0, 2), samples);
    ...
    for (size_t i = 1; i <= kDerivativeOrder; ++i) {
        EXPECT_TRUE(CompareMatrices(pp.derivative(i).value(kTestTime), output2.segment(len * i, len), 1e-10,
                                    MatrixCompareType::absolute));
    }
}
...
void TestScalar(bool zero_derivatives_beyond_limits) {
    auto pp = PiecewisePolynomial<T>::ZeroOrderHold(Vector3<T>{0, 1, 2}, RowVector3<T>{1.2, 3, 1.5});
    TrajectorySource<T> source(pp, 1, zero_derivatives_beyond_limits);
    auto context = source.CreateDefaultContext();
    context->SetTime(0.5);
    EXPECT_NEAR(ExtractDoubleOrThrow(source.get_output_port().Eval(*context))[0], 1.2, 1e-14);
}
...
GTEST_TEST(AdditionalTrajectorySourceTests, ScalarConversion) {
    auto pp = PiecewisePolynomial<double>::ZeroOrderHold(Eigen::Vector3d{0, 1, 2}, Eigen::RowVector3d{1.2, 3, 1.5});
    for (bool zero_derivatives_beyond_limits : {true, false}) {
        // AutoDiffXd.
        ...
        EXPECT_NEAR(ExtractDoubleOrThrow(source_ad->get_output_port().Eval(*context_ad))[0], 1.2, 1e-14);

        // Taking the gradient w.r.t to time throws.
        context_ad->SetTime(AutoDiffXd(0.5, Eigen::Vector2d(1, 2)));
        DRAKE_EXPECT_THROWS_MESSAGE(source_ad->get_output_port().Eval(*context_ad), ".*UpdateTrajectory.*");

        // UpdateTrajectory can resolve it.
        auto pp_ad = PiecewisePolynomial<AutoDiffXd>::ZeroOrderHold(Vector3<AutoDiffXd>{0, 1, 2},
                                                                    RowVector3<AutoDiffXd>{1.2, 3, 1.5});
        ...
        EXPECT_NEAR(ExtractDoubleOrThrow(source_sym->get_output_port().Eval(*context_sym))[0], 1.2, 1e-14);

        // When time is symbolic, the output port eval throws.
        symbolic::Variable t("t");
        context_sym->SetTime(t);
        DRAKE_EXPECT_THROWS_MESSAGE(source_sym->get_output_port().Eval(*context_sym), ".*symbolic time.*");
        ...
        // time).
        trajectories::BezierCurve<Expression> curve_sym(0.0, 2.0, RowVector2<Expression>{1.2, 3});
        source_sym->UpdateTrajectory(curve_sym);
        Expression expected = (1.2 * (1.0 - (min(2.0, max(0.0, t)) / 2.0)) + 3.0 * (min(2.0, max(0.0, t)) / 2.0));
        EXPECT_TRUE(source_sym->get_output_port().Eval(*context_sym)[0].EqualTo(expected));
    }
}
