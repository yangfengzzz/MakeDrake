...
TEST_F(ConstantVectorSourceTest, EigenModel) {
    ...
    EXPECT_TRUE(kConstantVectorSource.isApprox(source_->get_output_port().Eval(*context_),
                                               Eigen::NumTraits<double>::epsilon()));

    // Tests that the output reflects changes to the parameter value in the
    // context.
    BasicVector<double>& source_value = source_->get_mutable_source_value(context_.get());
    source_value.SetFromVector(2.0 * source_value.get_value());

    EXPECT_TRUE(kConstantVectorSource.isApprox(0.5 * source_->get_output_port().Eval(*context_),
                                               Eigen::NumTraits<double>::epsilon()));
}
...
TEST_F(ConstantVectorSourceTest, BasicVectorModel) {
    ...
    BasicVector<double>& source_value =
            static_cast<ConstantVectorSource<double>*>(source_.get())->get_mutable_source_value(context_.get());
    source_value.SetFromVector(2.0 * source_value.get_value());
    EXPECT_EQ(43.0, 0.5 * source_->get_output_port().Eval<MyVector3d>(*context_)[1]);
}
...
TEST_F(ConstantVectorSourceTest, ToAutoDiffPass) {
    ...
    EXPECT_TRUE(is_autodiffxd_convertible(*source_, [&](const auto& converted) {
        ...
        const BasicVector<AutoDiffXd>& new_default = converted.get_source_value(*new_context);
...
TEST_F(ConstantVectorSourceTest, ToSymbolic) {
    ...
    EXPECT_TRUE(is_symbolic_convertible(*source_, [&](const auto& converted) {
        auto new_context = converted.CreateDefaultContext();
        const BasicVector<symbolic::Expression>& new_default = converted.get_source_value(*new_context);
