...
GTEST_TEST(SharedPointerSystemTest, BuilderFromUnique) {
    ...
    std::string* gotten = SharedPointerSystem<double>::AddToBuilder(builder.get(), std::move(held));
...
GTEST_TEST(SharedPointerSystemTest, ScalarConversion) {
    auto dut1 = std::make_unique<SharedPointerSystem<double>>(std::make_unique<std::string>("held"));
...
GTEST_TEST(SharedPointerSystemTest, Null) {
    auto builder = std::make_unique<DiagramBuilder<double>>();
    std::string* nothing = SharedPointerSystem<double>::AddToBuilder(builder.get(), std::shared_ptr<std::string>{});
