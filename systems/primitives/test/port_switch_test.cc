...
GTEST_TEST(PortSwitchTest, VectorTest) {
    ...
    system.get_port_selector_input_port().FixValue(context.get(), port_A.get_index());
    EXPECT_TRUE(CompareMatrices(system.get_output_port().Eval(*context), a));

    system.get_port_selector_input_port().FixValue(context.get(), port_B.get_index());
...
GTEST_TEST(PortSwitchTest, TemplatedAbstractValue) {
    ...
    autodiff_system->GetInputPort("port_selector").FixValue(autodiff_context.get(), port_A.get_index());
    const MySignal<AutoDiffXd>& autodiff_output =
            autodiff_system->get_output_port(0).template Eval<MySignal<AutoDiffXd>>(*autodiff_context);
    EXPECT_EQ(autodiff_output.value.value(), 2.0);

    auto symbolic_system = system.ToSymbolic();
    EXPECT_EQ(symbolic_system->num_input_ports(), 3);
    EXPECT_EQ(symbolic_system->GetInputPort("A").size(), 0);
    const InputPort<symbolic::Expression>& port_B = symbolic_system->GetInputPort("B");
    EXPECT_EQ(port_B.size(), 0);
    ...
    symbolic_system->GetInputPort("port_selector").FixValue(symbolic_context.get(), port_B.get_index());
    const MySignal<symbolic::Expression>& symbolic_output =
            symbolic_system->get_output_port(0).template Eval<MySignal<symbolic::Expression>>(*symbolic_context);
