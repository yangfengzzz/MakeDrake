...
// [y₁, y₂] = [x₁, x₂] + u*[1, 1].
class SimpleTimeVaryingLinearSystem final : public TimeVaryingLinearSystem<double> {
public:
...
protected:
    void SetUp() {
        ...
        xstar_discrete_ = (Eigen::Matrix3d::Identity() - A_).colPivHouseholderQr().solve(B_ * u0_ + f0_);

        continuous_system_.reset(new AffineSystem<double>(A_, B_, f0_, C_, D_, y0_));
        discrete_system_.reset(new AffineSystem<double>(A_, B_, f0_, C_, D_, y0_, time_period_));
    }
...
TEST_F(TestLinearizeFromAffine, ContinuousAtEquilibrium) {
    ...
    context->get_mutable_continuous_state_vector().SetFromVector(xstar_continuous_);

    auto linearized_system = Linearize(*continuous_system_, *context);

    double tol = 1e-10;
    EXPECT_TRUE(CompareMatrices(A_, linearized_system->A(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(B_, linearized_system->B(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(C_, linearized_system->C(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(D_, linearized_system->D(), tol, MatrixCompareType::absolute));

    std::unique_ptr<AffineSystem<double>> affine_system = FirstOrderTaylorApproximation(*continuous_system_, *context);
    EXPECT_TRUE(CompareMatrices(f0_, affine_system->f0(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(y0_, affine_system->y0(), tol, MatrixCompareType::absolute));
}
...
TEST_F(TestLinearizeFromAffine, ContinuousAtNonEquilibrium) {
    ...
    EXPECT_TRUE(CompareMatrices(A_, affine_system->A(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(B_, affine_system->B(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(C_, affine_system->C(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(D_, affine_system->D(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(f0_, affine_system->f0(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(y0_, affine_system->y0(), tol, MatrixCompareType::absolute));
}
...
TEST_F(TestLinearizeFromAffine, DiscreteAtEquilibrium) {
    ...
    // Set x0 to the actual equilibrium point.
    systems::BasicVector<double>& xd = context->get_mutable_discrete_state().get_mutable_vector();
    ...
    std::unique_ptr<AffineSystem<double>> affine_system = FirstOrderTaylorApproximation(*discrete_system_, *context);
    EXPECT_TRUE(CompareMatrices(f0_, affine_system->f0(), tol, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(y0_, affine_system->y0(), tol, MatrixCompareType::absolute));
}
...
public:
    ...
    EmptyStateSystemWithAbstractInput() : LeafSystem<T>(SystemTypeTag<EmptyStateSystemWithAbstractInput>{}) {
        this->DeclareAbstractInputPort("dummy", Value<std::vector<double>>() /* Arbitrary data type */);
    }

    // Scalar-converting copy constructor. See @ref system_scalar_conversion.
    template <typename U>
    explicit EmptyStateSystemWithAbstractInput(const EmptyStateSystemWithAbstractInput<U>&)
...
public:
    ...
    EmptyStateSystemWithMixedInputs() : LeafSystem<T>(SystemTypeTag<EmptyStateSystemWithMixedInputs>{}) {
    ...
    /// Scalar-converting copy constructor.  See @ref system_scalar_conversion.
    template <typename U>
    explicit EmptyStateSystemWithMixedInputs(const EmptyStateSystemWithMixedInputs<U>&)
...
GTEST_TEST(TestSystem, IsSimpleEquationSystem) {
    ...
    EXPECT_FALSE(continuous_time_system.IsDifferenceEquationSystem(&returned_time_period));
    ...
    EXPECT_TRUE(discrete_time_system.IsDifferenceEquationSystem(&returned_time_period));
    ...
    EXPECT_FALSE(non_periodic_system.IsDifferenceEquationSystem(&returned_time_period));
...
GTEST_TEST(TestLinearize, LinearizingOnAbstractPortThrows) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Linearize(system, *context), ".*only supports vector-valued.*");
}
...
GTEST_TEST(TestLinearize, LinearizingWithMixedInputs) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(Linearize(system, *context), "InputPort.*is not connected");
...
GTEST_TEST(TestLinearize, Controllability) {
    ...
    EXPECT_TRUE(CompareMatrices(ControllabilityMatrix(sys1), ControllabilityMatrix(sys2)));
...
TEST_F(LinearSystemSymbolicTest, MakeLinearSystem) {
    ...
    // system correctly.
    const auto dut = LinearSystem<double>::MakeLinearSystem(A_ * x_ + B_ * u_, C_ * x_ + D_ * u_, x_, u_, 10.0);
    EXPECT_EQ(dut->A(), A_);
    EXPECT_EQ(dut->B(), B_);
    EXPECT_EQ(dut->C(), C_);
    EXPECT_EQ(dut->D(), D_);
...
TEST_F(LinearSystemSymbolicTest, MakeLinearSystemException1) {
  ...
  extra_terms << x0_ * x0_,
    ...
    // clang-format on
    EXPECT_THROW(
            LinearSystem<double>::MakeLinearSystem(extra_terms + A_ * x_ + B_ * u_, C_ * x_ + D_ * u_, x_, u_, 10.0),
            std::runtime_error);
}
...
TEST_F(LinearSystemTest, LinearizeSystemWithParameters) {
    ...
    examples::pendulum::PendulumState<double>& state =
            dynamic_cast<examples::pendulum::PendulumState<double>&>(context->get_mutable_continuous_state_vector());
    state.set_theta(0.0);
    state.set_thetadot(0.0);

    std::unique_ptr<LinearSystem<double>> linearized_pendulum = Linearize(pendulum, *context);

    const auto params =
            dynamic_cast<const examples::pendulum::PendulumParams<double>*>(&context->get_numeric_parameter(0));
    EXPECT_TRUE(params);
    ...
    Eigen::Matrix2d A;
    A << 0, 1.0, -params->gravity() / params->length(), -params->damping() / inertia;
...
public:
    ...
    explicit MimoSystem(bool is_discrete) : LeafSystem<T>(SystemTypeTag<MimoSystem>{}), is_discrete_(is_discrete) {
        ...
        if (is_discrete) {
            ...
            this->DeclarePeriodicDiscreteUpdateEvent(0.1, 0.0, &MimoSystem::CalcDiscreteUpdate);
        } else {
    ...
    template <typename U>
    explicit MimoSystem(const MimoSystem<U>& other) : MimoSystem<T>(other.is_discrete_) {}
    ...
    void CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
        Vector1<T> u0 = this->get_input_port(0).Eval(context);
        Vector3<T> u1 = this->get_input_port(1).Eval(context);
        Vector2<T> x = get_state_vector(context);
...
GTEST_TEST(LinearSystemBespokeTest, InfiniteRecursionDuringCalc) {
    ...
    auto* plant = builder.AddSystem(std::make_unique<LinearSystem<double>>(Ap, Bp, Cp, Dp));
    ...
    MatrixXd Dc(1, 2);
    Dc << -1, -1;
    auto* controller = builder.AddSystem(std::make_unique<LinearSystem<double>>(Ac, Bc, Cc, Dc));
    ...
    const VectorXd y = plant->get_output_port().Eval(plant->GetMyContextFromRoot(*context));
