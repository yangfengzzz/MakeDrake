...
void TestInputAndOutput(const Saturation<T>& saturation_system,
    ...
    EXPECT_EQ(saturation_system.get_output_port().Eval(*context), expected_output);
}
...
void TestConstantSaturation(const Saturation<T>& saturation_system,
    ...
    TestInputAndOutput<T>(saturation_system, std::move(context), input_vector, expected_output);
}
...
void TestVariableSaturation(const Saturation<T>& saturation_system,
    ...
    if (min_value_vector.size() > 0) {
        ...
        saturation_system.get_min_value_port().FixValue(context.get(), min_value_vector);
    }

    if (max_value_vector.size()) {
        // Hook max value of the expected size.
        saturation_system.get_max_value_port().FixValue(context.get(), max_value_vector);
    }
...
void SaturationTest(bool run_constant_saturation_test) {
    ...
    EXPECT_ANY_THROW(
            std::make_unique<Saturation<T>>(Vector3<T>(1.0, -4.5, -2.5) /* u_min */, Vector2<T>(3.0, 5.0) /* u_max */));

    // Tests for error thrown due to incorrectly initialized Saturation. (u_min
    // >= u_max along some or all dimensions).
    EXPECT_ANY_THROW(std::make_unique<Saturation<T>>(Vector3<T>(1.0, -4.5, -2.5) /* u_min */,
    ...
    Eigen::MatrixXd input_vector_range =
            Eigen::VectorXd::LinSpaced(kNumPoints, -5.0 /* lower limit */, 5.0 /* upper limit */)
    ...
    for (int i = 0; i < kNumPoints; ++i) {
        ...
        if (run_constant_saturation_test) {
            TestConstantSaturation<T>(*saturation_system, input_vector_range.col(i), expected);
        } else {
            // Tests Saturation with variable max and min values.
            TestVariableSaturation<T>(*saturation_system, kUMin, kUMax, input_vector_range.col(i), expected);
            ...
            EXPECT_ANY_THROW(TestVariableSaturation<T>(*saturation_system, dummy_vector, dummy_vector,
