#include "systems/primitives/linear_transform_density.h"
...
void TestConstructor() {
    LinearTransformDensity<T> dut(RandomDistribution::kUniform, 2 /* input_size */, 3 /* output_size */);
    ...
    EXPECT_EQ(dut.get_input_port_w_in().get_random_type(), RandomDistribution::kUniform);
...
GTEST_TEST(LinearTransformDensityTest, ToAutoDiff) {
    auto dut = std::make_unique<LinearTransformDensity<double>>(RandomDistribution::kUniform, 2, 3);
    EXPECT_TRUE(is_autodiffxd_convertible(*dut, [&](const auto& converted) {
        EXPECT_EQ(converted.get_input_port_w_in().size(), dut->get_input_port_w_in().size());
        EXPECT_EQ(converted.get_input_port_A().size(), dut->get_input_port_A().size());
        EXPECT_EQ(converted.get_input_port_b().size(), dut->get_input_port_b().size());
        EXPECT_EQ(converted.get_output_port_w_out().size(), dut->get_output_port_w_out().size());
        EXPECT_EQ(converted.get_output_port_w_out_density().size(), dut->get_output_port_w_out_density().size());
    }));
}
...
void TestCalcOutput() {
  ...
  A << 1., 2.,
    ...
    const auto& port_A_vector = port_A_val.template get_vector_value<T>().get_value();
...
void TestCalcOutputDensity() {
  ...
  A << 1., 2.,
    ...
    // Test when port b is not connected.
    const auto w_out_density_no_b = dut.get_output_port_w_out_density().Eval(*context);
    const T w_out_density_no_b_expected = dut.CalcDensity(*context);
    EXPECT_TRUE(CompareMatrices(w_out_density_no_b, Vector1<T>(w_out_density_no_b_expected)));
    ...
    EXPECT_TRUE(CompareMatrices(w_out_density, Vector1<T>(w_out_density_expected)));
}
...
void CheckDensity(RandomDistribution distribution,
    ...
    for (const auto& w_in_double : nonzero_prob_inputs) {
        ...
        EXPECT_NEAR(ExtractDoubleOrThrow(density), ExtractDoubleOrThrow(density_expected), 10 * kEps);
...
GTEST_TEST(LinearTransformDensityTest, NoninvertibleTransform) {
    ...
    for (RandomDistribution distribution :
         {RandomDistribution::kUniform, RandomDistribution::kGaussian, RandomDistribution::kExponential}) {
        ...
        dut1.get_input_port_w_in().FixValue(context1.get(), Eigen::Vector2d(0.5, 0.6));
        DRAKE_EXPECT_THROWS_MESSAGE(dut1.CalcDensity(*context1), ".* to compute the density.*");
        ...
        dut2.get_input_port_w_in().FixValue(context2.get(), Eigen::Vector2d(0.5, 0.6));
        DRAKE_EXPECT_THROWS_MESSAGE(dut2.CalcDensity(*context2), ".* to compute the density.*");
    }
}
...
void CheckGaussianDensity() {
    CheckDensity<T>(RandomDistribution::kGaussian,
                    {Eigen::Vector2d(0, 0), Eigen::Vector2d(1, 5), Eigen::Vector2d(-0.5, 1.8)}, {});
    ...
    double density_expected = std::exp(ExtractDoubleOrThrow(-0.5 * (w_out - b).dot(ldlt_solver.solve(w_out - b)))) /
                              std::sqrt(std::pow(2 * M_PI, 2) * ExtractDoubleOrThrow(cov.determinant()));
...
GTEST_TEST(LinearTransformDensityTest, CalcDensityUniform) {
    const std::vector<Eigen::Vector2d> nonzero_prob_inputs{
            {Eigen::Vector2d(0.5, 0.2), Eigen::Vector2d(10 * kEps, 0.9), Eigen::Vector2d(0.3, 1. - 10 * kEps)}};
    const std::vector<Eigen::Vector2d> zero_prob_inputs{
            {Eigen::Vector2d(-0.1, 0.5), Eigen::Vector2d(0.2, 1.4), Eigen::Vector2d(-0.5, 1.5)}};
    CheckDensity<double>(RandomDistribution::kUniform, nonzero_prob_inputs, zero_prob_inputs);
    CheckDensity<AutoDiffXd>(RandomDistribution::kUniform, nonzero_prob_inputs, zero_prob_inputs);
}

GTEST_TEST(LinearTransformDensityTest, CalcDensityExponential) {
    const std::vector<Eigen::Vector2d> nonzero_prob_inputs{
            {Eigen::Vector2d(0.5, 1.2), Eigen::Vector2d(10 * kEps, 0.9), Eigen::Vector2d(0.3, 1.6)}};
    const std::vector<Eigen::Vector2d> zero_prob_inputs{
            {Eigen::Vector2d(-0.1, 0.5), Eigen::Vector2d(0.5, -1.4), Eigen::Vector2d(-0.5, 1.5)}};
    CheckDensity<double>(RandomDistribution::kExponential, nonzero_prob_inputs, zero_prob_inputs);
    CheckDensity<AutoDiffXd>(RandomDistribution::kExponential, nonzero_prob_inputs, zero_prob_inputs);
}
...
Eigen::VectorXd CalcRandomSourceDensityGradient(RandomDistribution distribution,
...
void CheckDensityGradient(RandomDistribution distribution, const Eigen::Vector2d w_in_val) {
    ...
    Matrix2<AutoDiffXd> A = Eigen::Vector2d(2, -3).cast<AutoDiffXd>().asDiagonal();
    ...
    Eigen::Vector2d ddensity_db_expected =
            (1 / std::abs(A_val.determinant()) * CalcRandomSourceDensityGradient(distribution, w_in_val).transpose() *
             -A_val.inverse())
                    .transpose();
    Eigen::VectorXd density_grad =
            density.derivatives().rows() == 0 ? Eigen::VectorXd::Zero(2).eval() : density.derivatives();
    ...
    Eigen::Vector2d w_out_val = math::ExtractValue(dut.get_output_port_w_out().Eval(*context));
    Eigen::Vector2d b_val = math::ExtractValue(b);
    const double A_det_abs = std::abs(A_val.determinant());
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < 2; ++j) {
            const double dp_dAij = CalcRandomSourceDensityGradient(distribution, w_in_val)
                                                   .dot(-A_inv * dA_dAij[i][j] * A_inv * (w_out_val - b_val)) /
                                           A_det_abs -
                                   CalcProbabilityDensity<double>(distribution, w_in_val) * std::pow(A_det_abs, -2) *
                                           dA_abs_det_dAij[i][j];
            const double density_derivative_actual =
                    density.derivatives().rows() == 0 ? 0.0 : density.derivatives()(i + 2 * j);
...
GTEST_TEST(LinearTransformDensityTest, CalcDensityGradient) {
    ...
    CheckDensityGradient(RandomDistribution::kGaussian, Eigen::Vector2d(0.2, 0.5));
    CheckDensityGradient(RandomDistribution::kGaussian, Eigen::Vector2d(-0.2, 0.5));

    // Exponential distribution
    // w_in has zero probability.
    CheckDensityGradient(RandomDistribution::kExponential, Eigen::Vector2d(-0.2, 0.5));
