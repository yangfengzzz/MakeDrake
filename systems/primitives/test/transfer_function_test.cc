...
GTEST_TEST(TransferFunctionTest, BadInputsToConstructor) {
    ...
    // it here.
    RationalFunction my_s(Monomial(Variable("s")));  // Not TransferFunction::s_var().
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(1.0 / (my_s + 1.0)), ".*H must only be a function of.*");
    RationalFunction z(Monomial(Variable("z")));  // Not TransferFunction::z_var().
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(1.0 / (z - 1.0), 0.1), ".*H must only be a function of.*");
    my_s = TransferFunction::s();
    z = TransferFunction::z();
    // Using s with a time_step > 0 will throw.
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(1.0 / (my_s + 1.0), 0.1), ".*H must only be a function of.*");
    // Using z with a time_step == 0 will throw.
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(1.0 / (z - 1.0), 0.0), ".*H must only be a function of.*");
    // time_step < 0 will throw.
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(1.0 / (z - 1.0), -0.1), ".*time_step >= 0.*");
    ...
    H << z, 1 / z, other, z;
    DRAKE_EXPECT_THROWS_MESSAGE(TransferFunction(H, 0.1), ".*H must only be a function of.*");
}
