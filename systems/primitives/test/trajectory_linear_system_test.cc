...
#include "systems/primitives/test/trajectory_linear_affine_test.h"
...
class TrajectoryLinearSystemTest : public ::testing::TestWithParam<ConstructorType> {
public:
    void SetUp() override {
        if (this->GetParam() == ConstructorType::FromContinuous) {
            dut_ = make_unique<TrajectoryLinearSystem<double>>(data_.A, data_.B, data_.C, data_.D);
...
TEST_P(TrajectoryLinearSystemTest, Derivatives) {
    ...
    std::vector<double> times{0., 1e-5 * data_.kDiscreteTimeStep, 0.25 * data_.kDiscreteTimeStep,
    ...
    for (const double t : times) {
        ...
        if (time_period_ == 0.) {
            ...
            EXPECT_TRUE(CompareMatrices(A * x + B * u, derivatives_->get_vector().CopyToVector(), tol));
        } else {
            updates_->SetFrom(dut_->EvalUniquePeriodicDiscreteUpdate(*context_));
            EXPECT_TRUE(CompareMatrices(A * x + B * u, updates_->get_vector(0).CopyToVector(), tol));
        }
    }
}
...
TEST_P(TrajectoryLinearSystemTest, Output) {
    ...
    for (const double t : times) {
        ...
        EXPECT_TRUE(CompareMatrices(C * x + D * u, dut_->get_output_port().Eval(*context_), tol));
    }
}
...
INSTANTIATE_TEST_SUITE_P(Constructor,
                         TrajectoryLinearSystemTest,
                         testing::Values(ConstructorType::FromContinuous, ConstructorType::FromDiscrete));
