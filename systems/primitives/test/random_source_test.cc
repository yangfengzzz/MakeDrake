#include "systems/primitives/random_source.h"
...
void CheckStatistics(const std::function<double(double)>& cumulative_distribution,
                     double min_value,
                     double max_value,
                     double h,
                     double fudge_factor,
    ...
    BasicVector<double>& state = simulator.get_mutable_context().get_mutable_discrete_state(0);
    ...
    for (double a = min_value; a <= max_value - h; a += h) {
        ...
        const double count = (x.array() >= a && x.array() <= a + h).template cast<double>().matrix().sum();
        ...
        const double EY = cumulative_distribution(a + h) - cumulative_distribution(a);
...
GTEST_TEST(RandomSourceTest, UniformWhiteNoise) {
    auto random_source = std::make_unique<RandomSource<double>>(RandomDistribution::kUniform, 2, 0.0025);
    ...
    CheckStatistics(Phi, min_value, max_value, h, fudge_factor, std::move(random_source));
}
...
GTEST_TEST(RandomSourceTest, UniformWhiteNoiseAutoDiff) {
    auto random_source = std::make_unique<RandomSource<AutoDiffXd>>(RandomDistribution::kUniform, 2, 0.0025);
    EXPECT_EQ(random_source->get_distribution(), RandomDistribution::kUniform);
    EXPECT_EQ(random_source->get_fixed_seed(), std::nullopt);
    ...
    static_assert(std::is_same_v<decltype(sample)::Scalar, AutoDiffXd>, "The sample scalar type should be AutoDiffXd");
...
GTEST_TEST(RandomSourceTest, GaussianWhiteNoise) {
    auto random_source = std::make_unique<RandomSource<double>>(RandomDistribution::kGaussian, 2, 0.0025);
...
GTEST_TEST(RandomSourceTest, ExponentialWhiteNoise) {
    auto random_source = std::make_unique<RandomSource<double>>(RandomDistribution::kExponential, 2, 0.0025);
    EXPECT_EQ(random_source->get_distribution(), RandomDistribution::kExponential);
    EXPECT_EQ(random_source->get_fixed_seed(), std::nullopt);
...
void AddToDiagramBuilderTest() {
    ...
    sys1->DeclareInputPort("uniform", kVectorValued, 3, RandomDistribution::kUniform);
    sys1->DeclareInputPort("exponential", kVectorValued, 2, RandomDistribution::kExponential);

    auto* sys2 = builder.template AddSystem<TestSystem<T>>();
    sys2->DeclareInputPort("gaussian", kVectorValued, 5, RandomDistribution::kGaussian);
    sys2->DeclareInputPort("exponential", kVectorValued, 2, RandomDistribution::kExponential);
    sys2->DeclareInputPort("scalar_gaussian", kVectorValued, 1, RandomDistribution::kGaussian);

    // Export input 1.
    builder.ExportInput(sys2->get_input_port(1));

    // Connect input 2 to a different block.
    const auto* constant_input = builder.template AddSystem<ConstantVectorSource<T>>(T(14.0));
    ...
    EXPECT_TRUE(sys1->get_input_port(0).HasValue(diagram->GetSubsystemContext(*sys1, *context)));

    // Check that the exponential input port is connected.
    EXPECT_TRUE(sys1->get_input_port(1).HasValue(diagram->GetSubsystemContext(*sys1, *context)));

    // Check that the Gaussian input port is connected.
    EXPECT_TRUE(sys2->get_input_port(0).HasValue(diagram->GetSubsystemContext(*sys2, *context)));
    ...
    EXPECT_EQ(sys2->get_input_port(2).Eval(diagram->GetSubsystemContext(*sys2, *context))[0], T(14.0));
}
...
GTEST_TEST(RandomSourceTest, CorrelationTest) {
    ...
    const auto* random1 = builder.AddSystem<RandomSource<double>>(RandomDistribution::kGaussian, kSize, kSampleTime);
    const auto* log1 = LogVectorOutput(random1->get_output_port(0), &builder);

    const auto* random2 = builder.AddSystem<RandomSource<double>>(RandomDistribution::kGaussian, kSize, kSampleTime);
    ...
    for (int i = 0; i < N; i++) {
        ...
        const double xcorr = (x1.middleCols(0, N).array() * x2.middleCols(i, N).array()).sum();
...
GTEST_TEST(RandomSourceTest, SeedTest) {
    ...
    auto make_new_seed_and_output = [&dut, &new_seed_generator]() {
        ...
        return SeedAndOutput{dut.get_seed(*random_context), dut.get_output_port(0).Eval(*random_context)[0]};
    };
    ...
    const double other_default_output = other_port.Eval(*other.CreateDefaultContext())[0];
    ...
    EXPECT_EQ(other_port.Eval(*other.CreateDefaultContext())[0], other_default_output);
}
...
GTEST_TEST(RandomSourceTest, SetRandomContextTest) {
    ...
    auto context = random_source.CreateDefaultContext();
    const Eigen::Vector2d default_values = random_source.get_output_port(0).Eval(*context);

    RandomGenerator generator;
    random_source.SetRandomContext(context.get(), &generator);
    const Eigen::Vector2d novel_values = random_source.get_output_port(0).Eval(*context);
    ...
    const Eigen::Vector2d default_values_again = random_source.get_output_port(0).Eval(*context);
