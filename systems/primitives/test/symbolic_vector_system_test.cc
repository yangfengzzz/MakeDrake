#include "systems/primitives/symbolic_vector_system.h"
...
TEST_F(SymbolicVectorSystemTest, BuilderBasicPattern) {
    ...
    EXPECT_TRUE(SymbolicVectorSystemBuilder().state(x_[1]).state()[0].equal_to(x_[1]));

    EXPECT_TRUE(SymbolicVectorSystemBuilder().state(x_).state()[1].equal_to(x_[1]));

    EXPECT_EQ(SymbolicVectorSystemBuilder().state({x_[0], x_[1]}).state(), x_);

    EXPECT_TRUE(SymbolicVectorSystemBuilder().input(u_[1]).input()[0].equal_to(u_[1]));

    EXPECT_TRUE(SymbolicVectorSystemBuilder().input(u_).input()[1].equal_to(u_[1]));

    EXPECT_EQ(SymbolicVectorSystemBuilder().input({u_[0], u_[1]}).input(), u_);

    EXPECT_TRUE(SymbolicVectorSystemBuilder().parameter(p_[1]).parameter()[0].equal_to(p_[1]));

    EXPECT_TRUE(SymbolicVectorSystemBuilder().parameter(p_).parameter()[1].equal_to(p_[1]));

    EXPECT_EQ(SymbolicVectorSystemBuilder().parameter({p_[0], p_[1]}).parameter(), p_);

    EXPECT_TRUE(
            SymbolicVectorSystemBuilder().dynamics(x_[0] + u_[0] + p_[0]).dynamics()[0].EqualTo(x_[0] + u_[0] + p_[0]));
    ...
    EXPECT_TRUE(SymbolicVectorSystemBuilder().output(x_[0] + u_[0] + p_[0]).output()[0].EqualTo(x_[0] + u_[0] + p_[0]));
    ...
    EXPECT_TRUE(SymbolicVectorSystemBuilder().output({x_[0] + u_[0] + p_[0], x_[1]}).output()[1].EqualTo(x_[1]));

    EXPECT_EQ(SymbolicVectorSystemBuilder().time_period(3.2).time_period(), 3.2);

    EXPECT_TRUE(SymbolicVectorSystemBuilder().state(x_[0]).dynamics(-x_[0] + pow(x_[0], 3)).state()[0].equal_to(x_[0]));
...
TEST_F(SymbolicVectorSystemTest, CubicPolyViaBuilder) {
    ...
    auto system =
            SymbolicVectorSystemBuilder().state(x).parameter(p).dynamics(-x + pow(x, 3) + p).output(x * p).Build();
    ...
    EXPECT_TRUE(CompareMatrices(xdotval.CopyToVector(), Vector1d{-xval + xval * xval * xval + pval}, 1e-14));
    EXPECT_TRUE(CompareMatrices(system->get_output_port().template Eval<BasicVector<double>>(*context).CopyToVector(),
                                Vector1d{xval * pval}));
}
...
TEST_F(SymbolicVectorSystemTest, ScalarPassThrough) {
    ...
    SymbolicVectorSystem<double> system({}, Vector0<Variable>{}, Vector1<Variable>{u}, Vector0<Expression>{},
...
public:
    ...
    explicit CalcRecorder(int output_size = 1) {
        this->DeclareVectorOutputPort(kUseDefaultName, output_size, &CalcRecorder::CalcOutput);
    }
...
TYPED_TEST(InputEvaluationTest, EvaluateOnlyWhenNecessary) {
    ...
    Variable u0("u0");
    Variable x0("x0");
    auto input_evaluated_for_output = [u0, x0](const Vector1<Expression>& output) {
        DiagramBuilder<T> builder;
        auto& recorder = *builder.template AddSystem<CalcRecorder<T>>();
        Vector1<Expression> dynamics{x0};
        auto& dut = *builder.template AddSystem<SymbolicVectorSystem<T>>(std::nullopt, Vector1<Variable>{x0},
                                                                         Vector1<Variable>{u0}, dynamics, output);
...
TEST_F(SymbolicVectorSystemTest, VectorPassThrough) {
    // y = u.
    SymbolicVectorSystem<double> system({}, Vector0<Variable>{}, u_, Vector0<Expression>{}, u_.cast<Expression>());
...
TEST_F(SymbolicVectorSystemTest, OutputScaledTime) {
    SymbolicVectorSystem<double> system(t_, Vector0<Variable>{}, Vector0<Variable>{}, Vector0<Expression>{},
...
TEST_F(SymbolicVectorSystemTest, ContinuousStateOnly) {
    // xdot = -x + x^3
    const Variable& x{x_[0]};
    SymbolicVectorSystem<double> system({}, Vector1<Variable>{x}, Vector0<Variable>{},
    ...
    context->SetContinuousState(Vector1d{xval});
    const auto& xdotval = system.EvalTimeDerivatives(*context);
    EXPECT_TRUE(CompareMatrices(xdotval.CopyToVector(), Vector1d{-xval + xval * xval * xval}));
}
...
TEST_F(SymbolicVectorSystemTest, DiscreteStateOnly) {
    ...
    EXPECT_TRUE(CompareMatrices(discrete_variables->get_vector().get_value(), Vector1d{-xval + xval * xval * xval}));
}
...
TEST_F(SymbolicVectorSystemTest, ContinuousTimeSymbolic) {
    ...
    EXPECT_PRED2(ExprEqual, system->dynamics_for_variable(x_[1]), x_[1] + u_[1] + p_[1]);
    ...
    const auto& y = system->get_output_port().template Eval<BasicVector<Expression>>(*context).get_value();
...
TEST_F(SymbolicVectorSystemTest, LinearizeDynamics) {
    ...
    // non-linear dynamics
    const Vector2<Expression> dynamics{theta_dot, (tau - m * g * l * sin(theta) - b * theta_dot) / (m * l * l)};
    ...
    EXPECT_PRED2(ExprEqual, system_builder.dynamics_for_variable(theta), theta_dot);
    ...
    EXPECT_EQ(new_parameters.size(), p.size() + 3 /* dim of the linearization point */);
    ...
    const Polynomial linearized_dynamics_for_theta_dot{
            system_builder.dynamics_for_variable(theta_dot), {theta, theta_dot, tau} /* indeterminates */
    };
    ...
    {
        const Environment env1{{theta_lp, -0.1}, {theta_dot_lp, 0.2}, {tau_lp, 0.3}};
        const Environment env2{{theta, env1[theta_lp]}, {theta_dot, env1[theta_dot_lp]}, {tau, env1[tau_lp]}};

        const Polynomial p1{linearized_dynamics_for_theta_dot.EvaluatePartial(env1)};
    ...
    {
        const Environment env1{{theta_lp, 0.3}, {theta_dot_lp, -0.1}, {tau_lp, 0.4}};
        const Environment env2{{theta, env1[theta_lp]}, {theta_dot, env1[theta_dot_lp]}, {tau, env1[tau_lp]}};
...
TEST_F(SymbolicVectorSystemTest, TestScalarConversion) {
    const Variable& x{x_[0]};
    const Variable& p{p_[0]};
    auto system = SymbolicVectorSystemBuilder().state(x).parameter(p).dynamics(-x + p).Build();
...
protected:
    ...
    void SetContext(const VectorX<AutoDiffXd>& txup) {
        ...
        autodiff_system_->get_input_port(0).FixValue(context_.get(), txup.segment<2>(2));
...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdFullGradient) {
    ...
    const auto xdotval = autodiff_system_->EvalTimeDerivatives(*context_).CopyToVector();
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(xdotval), Vector1d{-xval_ * pval_[0] + xval_ * xval_ * xval_}));
    EXPECT_TRUE(CompareMatrices(xdotval[0].derivatives(), expected_xdotval0_deriv_, 1.0e-15));

    const auto& yval =
            autodiff_system_->get_output_port(0).template Eval<BasicVector<AutoDiffXd>>(*context_).get_value();
    EXPECT_TRUE(CompareMatrices(math::ExtractValue(yval), Vector1d{tval_ + 2 * uval_[0] + 3 * uval_[1] + pval_[1]}));
...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdGradientRelativeToInput) {
    ...
    gradient(2, 0) = 1.0;  // u₀
    gradient(3, 1) = 1.0;  // u₁
    const VectorX<AutoDiffXd> txup = math::InitializeAutoDiff(Eigen::MatrixXd(txupval_), gradient);
    SetContext(txup);

    const auto xdotval = autodiff_system_->EvalTimeDerivatives(*context_).CopyToVector();
    EXPECT_TRUE(CompareMatrices(xdotval[0].derivatives(), expected_xdotval0_deriv_.segment<2>(2)));

    const auto& yval =
            autodiff_system_->get_output_port(0).template Eval<BasicVector<AutoDiffXd>>(*context_).get_value();
    EXPECT_TRUE(CompareMatrices(yval[0].derivatives(), expected_yval0_deriv_.segment<2>(2)));
}
...
TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdGradientRelativeToState) {
    ...
    const auto xdotval = autodiff_system_->EvalTimeDerivatives(*context_).CopyToVector();
    EXPECT_TRUE(CompareMatrices(xdotval[0].derivatives(), expected_xdotval0_deriv_.head<2>(), 1.0e-15));

    const auto& yval =
            autodiff_system_->get_output_port(0).template Eval<BasicVector<AutoDiffXd>>(*context_).get_value();
    EXPECT_TRUE(CompareMatrices(yval[0].derivatives(), expected_yval0_deriv_.head<2>()));
}

TEST_F(SymbolicVectorSystemAutoDiffXdTest, AutodiffXdGradientRelativeToParameter) {
    Eigen::MatrixXd gradient{Eigen::MatrixXd::Zero(6, 2)};
    gradient(4, 0) = 1.0;  // p₀
    gradient(5, 1) = 1.0;  // p₁
    const VectorX<AutoDiffXd> txup = math::InitializeAutoDiff(Eigen::MatrixXd(txupval_), gradient);
    ...
    const auto xdotval = autodiff_system_->EvalTimeDerivatives(*context_).CopyToVector();
    EXPECT_TRUE(CompareMatrices(xdotval[0].derivatives(), expected_xdotval0_deriv_.segment<2>(4)));

    const auto& yval =
            autodiff_system_->get_output_port(0).template Eval<BasicVector<AutoDiffXd>>(*context_).get_value();
    EXPECT_TRUE(CompareMatrices(yval[0].derivatives(), expected_yval0_deriv_.segment<2>(4)));
}
