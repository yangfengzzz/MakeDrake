...
GTEST_TEST(DiscreteTimeIntegratorTest, BasicTest) {
    ...
    std::optional<PeriodicEventData> data = integrator.GetUniquePeriodicDiscreteUpdateAttribute();
    EXPECT_TRUE(data.has_value());
    EXPECT_EQ(data->period_sec(), kTimeStep);

    auto context = integrator.CreateDefaultContext();
    EXPECT_TRUE(CompareMatrices(integrator.get_output_port().Eval(*context), Eigen::Vector3d::Zero(), 1e-14));
    ...
    EXPECT_TRUE(CompareMatrices(integrator.get_output_port().Eval(*context), x, 1e-14));

    const Eigen::Vector3d u{4.0, 5.0, 6.0};
    integrator.get_input_port().FixValue(context.get(), u);
    EXPECT_TRUE(
            CompareMatrices(x + kTimeStep * u, integrator.EvalUniquePeriodicDiscreteUpdate(*context).value(), 1e-14));
}
...
GTEST_TEST(DiscreteTimeIntegratorTest, Simulation) {
    ...
    EXPECT_TRUE(CompareMatrices(integrator.get_output_port().Eval(context), Eigen::Vector2d::Zero(), 1e-14));

    // From the docs: the output at time `t` is `xₙ` where `n = ceil(t/h)`.
    // Test the output at t = 0.5 * h.
    simulator.AdvanceTo(kTimeStep / 2);
    EXPECT_TRUE(CompareMatrices(integrator.get_output_port().Eval(context), kTimeStep * u, 1e-14));
