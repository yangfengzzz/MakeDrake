...
void RunFirstOrderHold(const bool suppress_initial_transient) {
  ...
  knots << 4, 4+2,
           5, 5+3;
    // clang-format on
    const auto pp = trajectories::PiecewisePolynomial<double>::FirstOrderHold(Vector2d(0., kDuration), knots);
    auto ppsource = builder.AddSystem<TrajectorySource<double>>(pp);

    const int kNumInputs = 2;
    const double time_step = 0.1;
    auto deriv = builder.AddSystem<DiscreteDerivative<double>>(kNumInputs, time_step, suppress_initial_transient);
    ...
    auto logger = LogVectorOutput(deriv->get_output_port(), &builder, time_step / 3.267);
    ...
    for (int i = 0; i < log.sample_times().size(); i++) {
        if (log.sample_times()(i) == 0.0) {
            ...
            if (!suppress_initial_transient) {
                ...
                EXPECT_TRUE(CompareMatrices(log.data().col(i), Vector2d(4.0 / time_step, 5.0 / time_step), 1e-12));
            } else {
...
void RunSetState(const bool suppress_initial_transient) {
    const int kNumInputs = 2;
    const double time_step = 0.1;
    DiscreteDerivative<double> deriv(kNumInputs, time_step, suppress_initial_transient);
    ...
    EXPECT_TRUE(
            CompareMatrices(deriv.get_output_port().Eval<BasicVector<double>>(*context).get_value(), Vector2d::Zero()));
    ...
    deriv.set_input_history(context.get(), Eigen::Vector2d(2.0, 20.0), Eigen::Vector2d(1.0, 10.0));
    EXPECT_TRUE(CompareMatrices(deriv.get_output_port().Eval<BasicVector<double>>(*context).get_value(),
                                Vector2d(10.0, 100.0)));
}
...
GTEST_TEST(StateInterpolatorWithDiscreteDerivativeTest, BasicTest) {
    ...
    const double time_step = 0.1;
    StateInterpolatorWithDiscreteDerivative<double> position_to_state(kNumPositions, time_step);

    EXPECT_EQ(&position_to_state.GetInputPort("position"), &position_to_state.get_input_port());
    EXPECT_EQ(&position_to_state.GetOutputPort("state"), &position_to_state.get_output_port());
    ...
    EXPECT_TRUE(CompareMatrices(position_to_state.get_output_port().Eval<BasicVector<double>>(*context).get_value(),
                                expected_state));
