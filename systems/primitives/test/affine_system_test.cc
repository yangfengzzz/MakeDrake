#include "systems/primitives/affine_system.h"
...
#include "systems/primitives/test/affine_linear_test.h"
...
TEST_F(AffineSystemTest, Output) {
    ...
    EXPECT_TRUE(CompareMatrices(expected_output, dut_->get_output_port().Eval(*context_), 1e-10));
}
...
TEST_F(AffineSystemTest, UpdateCoefficients) {
    ...
    const std::string error_msg_regex = ".*This would change the dependencies of the output port.*";
    DRAKE_EXPECT_THROWS_MESSAGE(dut_zero_C.UpdateCoefficients(new_A, new_B, new_f0, new_C, new_D, new_y0),
                                error_msg_regex);

    auto dut_zero_D = AffineSystem<double>(A_, B_, f0_, C_, D_zero, y0_);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_zero_D.UpdateCoefficients(new_A, new_B, new_f0, new_C, new_D, new_y0),
                                error_msg_regex);
}
...
TEST_F(AffineSystemTest, UpdateCoefficientsButWrongSize) {
    ...
    const std::string error_msg_regrex = "^New and current .* have different sizes\\.$";

    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(new_A, B_, f0_, C_, D_, y0_), error_msg_regrex);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(A_, new_B, f0_, C_, D_, y0_), error_msg_regrex);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(A_, B_, new_f0, C_, D_, y0_), error_msg_regrex);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(A_, B_, f0_, new_C, D_, y0_), error_msg_regrex);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(A_, B_, f0_, C_, new_D, y0_), error_msg_regrex);
    DRAKE_EXPECT_THROWS_MESSAGE(dut_->UpdateCoefficients(A_, B_, f0_, C_, D_, new_y0), error_msg_regrex);
}

TEST_F(AffineSystemTest, DefaultAndRandomState) {
    EXPECT_TRUE(CompareMatrices(dut_->get_default_state(), x0_, 0.0));
    EXPECT_TRUE(CompareMatrices(context_->get_continuous_state_vector().CopyToVector(), x0_, 0.0));
    EXPECT_TRUE(CompareMatrices(dut_->get_random_state_covariance(), Sigma_x0_, 1e-16));
    ...
    for (int i = 0; i < kNumSamples; i++) {
        ...
        samples.col(i) = context_->get_continuous_state_vector().CopyToVector() - x0_;
    }
    const Eigen::Matrix2d sample_cov = (samples * samples.transpose()) / (kNumSamples - 1);
...
TEST_F(AffineSystemTest, ConvertScalarType) {
    EXPECT_TRUE(is_autodiffxd_convertible(*dut_, [&](const auto& converted) {
        ...
        EXPECT_TRUE(CompareMatrices(math::ExtractValue(converted.get_default_state()), x0_, 0.0));
        EXPECT_TRUE(CompareMatrices(converted.get_random_state_covariance(), Sigma_x0_, 1e-16));
    }));
    EXPECT_TRUE(is_symbolic_convertible(*dut_, [&](const auto& converted) {
        ...
        EXPECT_EQ(converted.D(), D_);
        EXPECT_EQ(converted.y0(), y0_);
        EXPECT_TRUE(CompareMatrices(symbolic::Evaluate(converted.get_default_state()), x0_, 0.0));
        EXPECT_TRUE(CompareMatrices(converted.get_random_state_covariance(), Sigma_x0_, 1e-16));
    }));
}
...
public:
    void SetDCornerElement(double d_1_1_element) { d_1_1_element_ = d_1_1_element; }

    void InitialiseSystem() {
        // Setup an arbitrary AffineSystem which is feedthrough if and only if
        // the elements of D matrix are exactly 0.
        Eigen::MatrixXd A_(AffineLinearSystemTest::make_2x2_matrix(1.5, 2.7, 3.5, -4.9));
        Eigen::MatrixXd B_(AffineLinearSystemTest::make_2x2_matrix(4.9, -5.1, 6.8, 7.2));
        Eigen::VectorXd f0_(AffineLinearSystemTest::make_2x1_vector(0, 0));
        Eigen::MatrixXd C_(AffineLinearSystemTest::make_2x2_matrix(1.1, 2.5, -3.8, 4.6));
        Eigen::MatrixXd D_(AffineLinearSystemTest::make_2x2_matrix(d_1_1_element_, 0, 0, 0));
...
GTEST_TEST(DiscreteAffineSystemTest, DiscreteTime) {
    ...
    EXPECT_TRUE(CompareMatrices(context->get_discrete_state_vector().CopyToVector(), Eigen::Vector3d::Zero(), 1e-16));
    ...
    EXPECT_TRUE(CompareMatrices(update.get_vector(0).CopyToVector(), A * x0 + B * u0 + f0));
    ...
    EXPECT_TRUE(CompareMatrices(system.get_output_port().Eval(*context), C * x0 + D * u0 + y0));
    ...
    EXPECT_TRUE(CompareMatrices(context->get_discrete_state_vector().CopyToVector(), x0, 1e-16));
    system.SetRandomContext(context.get(), &generator);
    EXPECT_TRUE(CompareMatrices(context->get_discrete_state_vector().CopyToVector(), x0, 1e-16));
}
...
GTEST_TEST(StatelessAffineSystemTest, StatelessTest) {
    ...
    EXPECT_TRUE(CompareMatrices(ct_system.get_output_port().Eval(*context), 2 * Eigen::Vector2d::Ones(), 1e-16));
    ...
    EXPECT_TRUE(CompareMatrices(dt_system.get_output_port().Eval(*context), 3 * Eigen::Vector2d::Ones(), 1e-16));
}
...
public:
    ...
    explicit SimpleTimeVaryingAffineSystem(double time_period)
        : TimeVaryingAffineSystem(SystemScalarConverter{}, kNumStates, kNumInputs, kNumOutputs, time_period) {}
    ...
    Eigen::MatrixXd B(const double& t) const override { return Eigen::Matrix<double, kNumOutputs, kNumInputs>::Ones(); }
    Eigen::VectorXd f0(const double& t) const override { return Eigen::Matrix<double, kNumOutputs, 1>::Zero(); }
    Eigen::MatrixXd C(const double& t) const override { return Eigen::Matrix2d::Identity(); }
    Eigen::MatrixXd D(const double& t) const override { return Eigen::Matrix<double, kNumOutputs, kNumInputs>::Ones(); }
    Eigen::VectorXd y0(const double& t) const override { return Eigen::Matrix<double, kNumOutputs, 1>::Ones(); }
};
...
GTEST_TEST(SimpleTimeVaryingAffineSystemTest, EvalTest) {
    ...
    EXPECT_TRUE(CompareMatrices(sys.A(t) * x + 42.0 * sys.B(t), derivs->CopyToVector()));

    EXPECT_TRUE(CompareMatrices(x + sys.y0(t) + 42.0 * sys.D(t), sys.get_output_port().Eval(*context)));
}

GTEST_TEST(SimpleTimeVaryingAffineSystemTest, ContinuousDiscreteVariableUpdatesTest) {
    ...
    EXPECT_NO_THROW(sys.CalcForcedDiscreteVariableUpdate(*context, updates.get()));
}
...
GTEST_TEST(SimpleTimeVaryingAffineSystemTest, DiscreteEvalTest) {
    ...
    EXPECT_TRUE(CompareMatrices(sys.A(t) * x + 42.0 * sys.B(t), updates.get_vector().CopyToVector()));
...
GTEST_TEST(IllegalTimeVaryingAffineSystemTest, BadSizeTest) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(sys.CalcTimeDerivatives(*context, derivatives.get()), ".*rows.*");
}
...
TEST_F(AffineSystemSymbolicTest, MakeAffineSystem) {
    ...
    // system correctly.
    const auto dut =
            AffineSystem<double>::MakeAffineSystem(A_ * x_ + B_ * u_ + f0_, C_ * x_ + D_ * u_ + y0_, x_, u_, 10.0);
    EXPECT_EQ(dut->A(), A_);
    EXPECT_EQ(dut->B(), B_);
    EXPECT_EQ(dut->C(), C_);
    EXPECT_EQ(dut->D(), D_);
...
TEST_F(AffineSystemSymbolicTest, MakeAffineSystemException1) {
  ...
  extra_terms << x0_ * x0_,
                 ...
                 x2_ * x2_;
    // clang-format on
    EXPECT_THROW(AffineSystem<double>::MakeAffineSystem(extra_terms + A_ * x_ + B_ * u_ + f0_, C_ * x_ + D_ * u_ + y0_,
                                                        x_, u_, 10.0),
                 std::runtime_error);
}
...
void CheckSizes(const AffineSystem<double>& sys, int num_states, int num_inputs, int num_outputs) {
