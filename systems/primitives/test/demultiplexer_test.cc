...
GTEST_TEST(OutputSize, SizeDifferentFromOne) {
    ...
    auto demux = make_unique<Demultiplexer<double>>(kInputSize /* size */, kOutputSize /* output_ports_size */);
    ...
    Eigen::VectorXd input_vector = Eigen::VectorXd::LinSpaced(kInputSize, 1.0, 6.0);
    ...
    for (int output_index = 0; output_index < kNumOutputs; ++output_index) {
        const auto& output_vector = demux->get_output_port(output_index).Eval(*context);
        ASSERT_EQ(kOutputSize, output_vector.size());
        ASSERT_EQ(input_vector.segment<kOutputSize>(output_index * kOutputSize), output_vector);
    }
}
...
GTEST_TEST(VectorizedOutputSize, SizeDifferentFromOne) {
    ...
    const int kInputSize = kOutputPortOneSize + kOutputPortTwoSize + kOutputPortThreeSize;
    const std::vector<int> kOutputPortsSizes{kOutputPortOneSize, kOutputPortTwoSize, kOutputPortThreeSize};
    ...
    // vector.
    auto demux = make_unique<Demultiplexer<double>>(kOutputPortsSizes /* output_ports_sizes */);
    auto context = demux->CreateDefaultContext();
    ...
    for (int output_index = 0; output_index < num_output_ports; ++output_index) {
        ...
        ASSERT_EQ(input_vector.segment(output_port_start, output_size), output_vector);
