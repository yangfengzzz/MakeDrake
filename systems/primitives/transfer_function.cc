...
TransferFunction::TransferFunction() : TransferFunction(MatrixX<RationalFunction>(0, 0), 0.0) {}

TransferFunction::TransferFunction(MatrixX<RationalFunction> H, double time_step) : H_{H}, time_step_{time_step} {
        ...
        for (int j = 0; j < H_.cols(); ++j) {
            if (!H(i, j).numerator().indeterminates().IsSubsetOf(s_or_z) ||
                ...
                throw std::runtime_error(
                        fmt::format("H must only be a function of {}, because time-step = {}. H({},{}) "
                                    "= {}). Note that you must use the static methods of this class to "
                                    "obtain {}; variables with  the same name will not be recognized.",
                                    time_step > 0.0 ? "z_var()" : "s_var()", time_step, i, j, H(i, j).ToExpression(),
                                    time_step > 0.0 ? "z_var()" : "s_var()"));
            }
        }
    }
}
