...
template <typename T>
LinearTransformDensity<T>::LinearTransformDensity(RandomDistribution distribution, int input_size, int output_size)
    : LeafSystem<T>(SystemTypeTag<LinearTransformDensity>{}),
      distribution_{distribution},
      input_size_{input_size},
      output_size_{output_size} {
    w_in_port_id_ = this->DeclareInputPort("w_in", kVectorValued, input_size_, distribution).get_index();
    A_port_id_ = this->DeclareInputPort("A", kVectorValued, input_size_ * output_size_).get_index();
    b_port_id_ = this->DeclareInputPort("b", kVectorValued, output_size_).get_index();

    w_out_port_id_ =
            this->DeclareVectorOutputPort("w_out", output_size_, &LinearTransformDensity<T>::CalcOutput).get_index();
    w_out_density_port_id_ =
            this->DeclareVectorOutputPort("w_out_density", 1, &LinearTransformDensity<T>::CalcOutputDensity)
                    .get_index();
}

template <typename T>
template <typename U>
LinearTransformDensity<T>::LinearTransformDensity(const LinearTransformDensity<U>& other)
    : LinearTransformDensity<T>(
              other.get_distribution(), other.get_input_port_w_in().size(), other.get_input_port_b().size()) {}

template <typename T>
void LinearTransformDensity<T>::CalcOutput(const Context<T>& context, BasicVector<T>* w_out) const {
...
template <typename T>
void LinearTransformDensity<T>::CalcOutputDensity(const Context<T>& context, BasicVector<T>* w_out_density) const {
    w_out_density->get_mutable_value()(0) = CalcDensity(context);
}

template <typename T>
Eigen::Map<const MatrixX<T>> LinearTransformDensity<T>::GetA(const Context<T>& context) const {
...
template <typename T>
FixedInputPortValue& LinearTransformDensity<T>::FixConstantA(Context<T>* context,
                                                             const Eigen::Ref<const MatrixX<T>>& A) const {
    this->ValidateContext(context);
    DRAKE_DEMAND(A.rows() == output_size_ && A.cols() == input_size_);
    return this->get_input_port_A().FixValue(context,
                                             Eigen::Map<const VectorX<T>>(A.data(), input_size_ * output_size_));
}

template <typename T>
FixedInputPortValue& LinearTransformDensity<T>::FixConstantB(Context<T>* context,
