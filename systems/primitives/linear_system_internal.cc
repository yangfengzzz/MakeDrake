...
MatrixXd ControllabilityMatrix(const Ref<const MatrixXd>& A, const Ref<const MatrixXd>& B) {
    ...
    for (int i = 1; i < num_states; ++i) {
        R.middleCols(num_inputs * i, num_inputs) = A * R.middleCols(num_inputs * (i - 1), num_inputs);
    }
    return R;
}

bool IsControllable(const Ref<const MatrixXd>& A, const Ref<const MatrixXd>& B, std::optional<double> threshold) {
...
MatrixXd ObservabilityMatrix(const Ref<const MatrixXd>& A, const Ref<const MatrixXd>& C) {
    ...
    for (int i = 1; i < num_states; ++i) {
        O.middleRows(num_outputs * i, num_outputs) = O.middleRows(num_outputs * (i - 1), num_outputs) * A;
    }
    return O;
}

bool IsObservable(const Ref<const MatrixXd>& A, const Ref<const MatrixXd>& C, std::optional<double> threshold) {
...
bool IsStabilizable(const Ref<const MatrixXd>& A,
    ...
    for (int i = 0; i < es.eigenvalues().size(); ++i) {
        ...
        if (!stable_mode) {
            ...
            mat.leftCols(A.cols()) = es.eigenvalues()(i) * MatrixXd::Identity(A.rows(), A.cols()) - A;
...
bool IsDetectable(const Ref<const MatrixXd>& A,
                  const Ref<const MatrixXd>& C,
                  bool continuous_time,
                  std::optional<double> threshold) {
    // The system (C, A) is detectable iff (A', C') is stabilizable.
    return IsStabilizable(A.transpose(), C.transpose(), continuous_time, threshold);
}
