...
Demultiplexer<T>::Demultiplexer(const std::vector<int>& output_ports_sizes)
    ...
    const int size = std::accumulate(output_ports_sizes.begin(), output_ports_sizes.end(), 0);
    ...
    for (int i = 0; i < num_output_ports; ++i) {
        ...
        this->DeclareVectorOutputPort(systems::kUseDefaultName, output_port_size,
...
template <typename T>
template <typename U>
Demultiplexer<T>::Demultiplexer(const Demultiplexer<U>& other) : Demultiplexer<T>(other.get_output_ports_sizes()) {}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Demultiplexer);
