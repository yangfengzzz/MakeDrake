...
template <typename T>
ConstantVectorSource<T>::ConstantVectorSource(const Eigen::Ref<const VectorX<T>>& source_value)
    : ConstantVectorSource(SystemTypeTag<ConstantVectorSource>{}, BasicVector<T>(source_value)) {}
...
template <typename T>
ConstantVectorSource<T>::ConstantVectorSource(const BasicVector<T>& source_value)
    : ConstantVectorSource<T>({}, source_value) {}

template <typename T>
template <typename U>
ConstantVectorSource<T>::ConstantVectorSource(const ConstantVectorSource<U>& other)
    : ConstantVectorSource<T>(ConvertDefaultValue<T, U>(other)) {}

// All other constructors delegate here.
template <typename T>
ConstantVectorSource<T>::ConstantVectorSource(SystemScalarConverter converter, const BasicVector<T>& source_value)
    ...
    DRAKE_DEMAND(this->get_system_scalar_converter().empty() || typeid(source_value) == typeid(BasicVector<T>));
}
...
template <typename T>
void ConstantVectorSource<T>::DoCalcVectorOutput(const Context<T>& context,
                                                 Eigen::VectorBlock<VectorX<T>>* output) const {
    *output = get_source_value(context).get_value();
}

template <typename T>
const BasicVector<T>& ConstantVectorSource<T>::get_source_value(const Context<T>& context) const {
    return this->GetNumericParameter(context, source_value_index_);
}

template <typename T>
BasicVector<T>& ConstantVectorSource<T>::get_mutable_source_value(Context<T>* context) {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::ConstantVectorSource);
