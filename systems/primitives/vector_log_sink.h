...
#include "systems/primitives/vector_log.h"
...
public:
    ...
    /// @see LogVectorOutput() helper function for a convenient way to add
    /// %logging.
    VectorLogSink(int input_size, const TriggerTypeSet& publish_triggers, double publish_period = 0.0);
...
VectorLogSink<T>* LogVectorOutput(const OutputPort<T>& src, DiagramBuilder<T>* builder, double publish_period = 0.0) {
    VectorLogSink<T>* sink = builder->template AddSystem<VectorLogSink<T>>(src.size(), publish_period);
...
VectorLogSink<T>* LogVectorOutput(const OutputPort<T>& src,
    ...
    VectorLogSink<T>* sink =
            builder->template AddSystem<VectorLogSink<T>>(src.size(), publish_triggers, publish_period);
    builder->Connect(src, sink->get_input_port());
    return sink;
}
