...
ZeroOrderHold<T>::ZeroOrderHold(double period_sec,
    ...
    if (!is_abstract()) {
        ...
        this->DeclarePeriodicDiscreteUpdateEvent(period_sec_, offset_sec_, &ZeroOrderHold::LatchInputVectorToState);
...
template <typename T>
void ZeroOrderHold<T>::LatchInputVectorToState(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
...
template <typename T>
void ZeroOrderHold<T>::LatchInputAbstractValueToState(const Context<T>& context, State<T>* state) const {
    DRAKE_ASSERT(is_abstract());
    const auto& input = this->get_input_port().template Eval<AbstractValue>(context);
    AbstractValue& state_value = state->get_mutable_abstract_state().get_mutable_value(0);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::ZeroOrderHold);
