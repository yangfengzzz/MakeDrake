...
public:
    ...
    const Eigen::MatrixXd get_random_state_covariance() const { return Sqrt_Sigma_x0_ * Sqrt_Sigma_x0_; }
...
protected:
    ...
    virtual void CalcOutputY(const Context<T>& context, BasicVector<T>* output_vector) const;
    ...
    /// classes may override this for performance reasons.
    virtual EventStatus CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* updates) const;
...
public:
    ...
    explicit AffineSystem(const Eigen::Ref<const Eigen::MatrixXd>& A = Eigen::MatrixXd(),
...
private:
    void CalcOutputY(const Context<T>& context, BasicVector<T>* output_vector) const final;
    ...
    EventStatus CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* updates) const final;
