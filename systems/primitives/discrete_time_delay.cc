...
template <typename T>
DiscreteTimeDelay<T>::DiscreteTimeDelay(double update_sec,
    ...
    if (!is_abstract()) {
        ...
        this->DeclareVectorOutputPort("delayed_u", model_value, &DiscreteTimeDelay::CopyDelayedVector,
                                      {this->xd_ticket()});
        this->DeclareDiscreteState(vector_size_ * delay_buffer_size_);
        this->DeclarePeriodicDiscreteUpdateEvent(update_sec_, 0., &DiscreteTimeDelay::SaveInputVectorToBuffer);
    } else {
        DRAKE_DEMAND(vector_size < 0);
        ...
        this->DeclarePeriodicUnrestrictedUpdateEvent(update_sec_, 0.,
                                                     &DiscreteTimeDelay::SaveInputAbstractValueToBuffer);
    }
}
...
DiscreteTimeDelay<T>::DiscreteTimeDelay(const DiscreteTimeDelay<U>& other)
    : DiscreteTimeDelay(other.update_sec_,
                        other.delay_buffer_size_ - 1,
                        other.vector_size_,
                        other.is_abstract() ? other.abstract_model_value_->Clone() : nullptr) {}

template <typename T>
void DiscreteTimeDelay<T>::CopyDelayedVector(const Context<T>& context, BasicVector<T>* output) const {
...
template <typename T>
void DiscreteTimeDelay<T>::SaveInputVectorToBuffer(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
    DRAKE_ASSERT(!is_abstract());
    ...
    Eigen::VectorBlock<VectorX<T>> updated_state_value = discrete_state->get_mutable_value(0);
...
template <typename T>
void DiscreteTimeDelay<T>::CopyDelayedAbstractValue(const Context<T>& context, AbstractValue* output) const {
    DRAKE_ASSERT(is_abstract());
    const int& oldest_index = context.template get_abstract_state<int>(delay_buffer_size_);
    const AbstractValue& state_value = context.get_abstract_state().get_value(oldest_index);
    output->SetFrom(state_value);
}

template <typename T>
void DiscreteTimeDelay<T>::SaveInputAbstractValueToBuffer(const Context<T>& context, State<T>* state) const {
    DRAKE_ASSERT(is_abstract());
    const auto& input = this->get_input_port().template Eval<AbstractValue>(context);
    int& oldest_index = state->template get_mutable_abstract_state<int>(delay_buffer_size_);
    AbstractValue& state_value = state->get_mutable_abstract_state().get_mutable_value(oldest_index);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::DiscreteTimeDelay);
