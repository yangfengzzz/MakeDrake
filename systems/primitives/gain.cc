...
template <typename T>
Gain<T>::Gain(const Eigen::VectorXd& k) : VectorSystem<T>(SystemTypeTag<Gain>{}, k.size(), k.size()), k_(k) {}
...
double Gain<T>::get_gain() const {
    if (!k_.isConstant(k_[0])) {
        throw std::runtime_error(
                fmt::format("The gain vector [{}] cannot be represented as a scalar value. "
...
template <typename T>
void Gain<T>::DoCalcVectorOutput(const Context<T>&,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Gain);
