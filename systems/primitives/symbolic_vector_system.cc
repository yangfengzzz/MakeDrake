...
bool Includes(const Ref<const VectorX<Variable>>& variables, const Variable& v) {
...
SymbolicVectorSystemBuilder SymbolicVectorSystemBuilder::LinearizeDynamics(const Ref<const VectorX<Expression>>& x0,
...
template <typename T>
SymbolicVectorSystem<T>::SymbolicVectorSystem(const std::optional<Variable>& time,
    ...
    VectorX<Variable> vars_vec(state_vars_.size() + input_vars_.size() + parameter_vars_.size() + (time_var_ ? 1 : 0));
    if (time_var_) {
    ...
    if (state_vars_.size() > 0) {
        ...
        if (time_period_ == 0.0) {
            ...
            this->DeclarePeriodicDiscreteUpdateEvent(time_period_, 0.0, &SymbolicVectorSystem<T>::CalcDiscreteUpdate);
            this->DeclareForcedDiscreteUpdateEvent(&SymbolicVectorSystem<T>::CalcDiscreteUpdate);
        }
    }
    ...
    if (output_.size() > 0) {
        ...
        this->DeclareVectorOutputPort(kUseDefaultName, output_.size(), &SymbolicVectorSystem<T>::CalcOutput);
    }
...
template <typename T>
bool SymbolicVectorSystem<T>::DependsOnInputs(const VectorX<Expression>& expr) const {
...
void SymbolicVectorSystem<T>::PopulateFromContext(const Context<T>& context, bool needs_inputs, Container* penv) const {
    Container& env = *penv;
    if (time_var_) {
        env[*time_var_] = context.get_time();
    }
    if (state_vars_.size() > 0) {
        const VectorBase<T>& state =
                (time_period_ > 0.0) ? context.get_discrete_state_vector() : context.get_continuous_state_vector();
        for (int i = 0; i < state_vars_.size(); i++) {
...
template <>
void SymbolicVectorSystem<double>::EvaluateWithContext(const Context<double>& context,
...
template <>
void SymbolicVectorSystem<AutoDiffXd>::EvaluateWithContext(const Context<AutoDiffXd>& context,
                                                           const VectorX<Expression>& expr,
                                                           const MatrixX<symbolic::Expression>& jacobian,
                                                           bool needs_inputs,
    ...
    const VectorBase<AutoDiffXd>& state = (state_vars_.size() > 0)
    ...
    const BasicVector<AutoDiffXd>& input =
            needs_inputs ? get_input_port().Eval<BasicVector<AutoDiffXd>>(context) : empty;

    const BasicVector<AutoDiffXd>& parameter = (parameter_vars_.size() > 0) ? context.get_numeric_parameter(0) : empty;
    ...
    auto set_num_gradients = [&num_gradients](const VectorBase<AutoDiffXd>& vars) {
        for (int i = 0; i < vars.size(); i++) {
            if (vars[i].derivatives().size()) {
                if (num_gradients == 0) {
                    num_gradients = static_cast<int>(vars[i].derivatives().size());
                } else {
                    DRAKE_DEMAND(static_cast<int>(vars[i].derivatives().size()) == num_gradients);
                }
            }
        }
    };
...
template <>
void SymbolicVectorSystem<Expression>::EvaluateWithContext(const Context<Expression>& context,
                                                           const VectorX<Expression>& expr,
                                                           const MatrixX<symbolic::Expression>& jacobian,
                                                           bool needs_inputs,
...
template <typename T>
void SymbolicVectorSystem<T>::CalcOutput(const Context<T>& context, BasicVector<T>* output_vector) const {
    DRAKE_DEMAND(output_.size() > 0);
    EvaluateWithContext(context, output_, output_jacobian_, output_needs_inputs_, output_vector);
}

template <typename T>
void SymbolicVectorSystem<T>::DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const {
    DRAKE_DEMAND(time_period_ == 0.0);
    DRAKE_DEMAND(dynamics_.size() > 0);
    EvaluateWithContext(context, dynamics_, dynamics_jacobian_, dynamics_needs_inputs_,
                        &derivatives->get_mutable_vector());
}

template <typename T>
EventStatus SymbolicVectorSystem<T>::CalcDiscreteUpdate(const drake::systems::Context<T>& context,
                                                        drake::systems::DiscreteValues<T>* updates) const {
    DRAKE_DEMAND(time_period_ > 0.0);
    DRAKE_DEMAND(dynamics_.size() > 0);
    EvaluateWithContext(context, dynamics_, dynamics_jacobian_, dynamics_needs_inputs_, &updates->get_mutable_vector());
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::SymbolicVectorSystem);
