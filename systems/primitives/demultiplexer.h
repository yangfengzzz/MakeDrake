...
public:
    ...
    const std::vector<int>& get_output_ports_sizes() const { return output_ports_sizes_; }

private:
    // Sets the port_index-th output port value.
    void CopyToOutput(const Context<T>& context, OutputPortIndex port_index, BasicVector<T>* output) const;

    // Return a std vector with size of 'size / output_ports_size' and all values
    // as @p output_ports_sizes.
    static const std::vector<int> CalcOutputPortsSizes(int size, int output_ports_size) {
    ...
    // input port vector.
    static const std::vector<int> CalcOutputPortsStart(const std::vector<int>& output_ports_sizes) {
        ...
        for (int i = 1; i < num_output_ports; i++) {
            output_ports_start[i] = output_ports_start[i - 1] + output_ports_sizes[i - 1];
        }
