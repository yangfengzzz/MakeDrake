...
public:
    ...
    explicit SharedPointerSystem(std::shared_ptr<Held> value_to_hold)
        : SharedPointerSystem(std::move(value_to_hold), std::type_index(typeid(Held))) {}
    ...
    template <typename Held>
    static Held* AddToBuilder(DiagramBuilder<T>* builder, std::shared_ptr<Held> value_to_hold) {
        DRAKE_THROW_UNLESS(builder != nullptr);
        auto holder = std::make_unique<SharedPointerSystem<T>>(std::move(value_to_hold));
    ...
    @exclude_from_pydrake_mkdoc{A unique_ptr input is unworkable in Python} */
    template <typename Held>
    static Held* AddToBuilder(DiagramBuilder<T>* builder, std::unique_ptr<Held> value_to_hold) {
        return SharedPointerSystem::AddToBuilder(builder, std::shared_ptr<Held>(std::move(value_to_hold)));
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::SharedPointerSystem);
