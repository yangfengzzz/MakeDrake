...
public:
    ...
    SymbolicVectorSystem(
            ...
            const Eigen::Ref<const VectorX<symbolic::Expression>>& output = Vector0<symbolic::Expression>{},
    ...
    SymbolicVectorSystem(
            ...
            const Eigen::Ref<const VectorX<symbolic::Expression>>& output = Vector0<symbolic::Expression>{},
            double time_period = 0.0)
        : SymbolicVectorSystem<T>(time, state, input, Vector0<symbolic::Variable>{}, dynamics, output, time_period) {}

    /// Scalar-converting copy constructor.  See @ref system_scalar_conversion.
    template <typename U>
    explicit SymbolicVectorSystem(const SymbolicVectorSystem<U>& other)
        : SymbolicVectorSystem<T>(other.time_var_,
                                  other.state_vars_,
                                  other.input_vars_,
                                  other.parameter_vars_,
                                  other.dynamics_,
                                  other.output_,
                                  other.time_period_) {}
    ...
    /// @{
    const std::optional<symbolic::Variable>& time_var() const { return time_var_; }
    ...
    const VectorX<symbolic::Variable>& parameter_vars() const { return parameter_vars_; }
    ...
    /// the variable @p var.
    const symbolic::Expression& dynamics_for_variable(const symbolic::Variable& var) const {
...
private:
    ...
    void PopulateFromContext(const Context<T>& context, bool needs_inputs, Container* penv) const;
    ...
    EventStatus CalcDiscreteUpdate(const drake::systems::Context<T>& context,
...
public:
    ...
    SymbolicVectorSystemBuilder state(const symbolic::Variable& v) { return state(Vector1<symbolic::Variable>{v}); }
    ...
    SymbolicVectorSystemBuilder state(const std::vector<symbolic::Variable>& vars) {
        return state(Eigen::Map<const VectorX<symbolic::Variable>>(vars.data(), vars.size()));
    }
    ...
    SymbolicVectorSystemBuilder input(const std::vector<symbolic::Variable>& vars) {
        return input(Eigen::Map<const VectorX<symbolic::Variable>>(vars.data(), vars.size()));
    }
    ...
    SymbolicVectorSystemBuilder parameter(const std::vector<symbolic::Variable>& vars) {
        return parameter(Eigen::Map<const VectorX<symbolic::Variable>>(vars.data(), vars.size()));
    }
    ...
    SymbolicVectorSystemBuilder dynamics(const std::vector<symbolic::Expression>& e) {
        return dynamics(Eigen::Map<const VectorX<symbolic::Expression>>(e.data(), e.size()));
    }
    ...
    SymbolicVectorSystemBuilder output(const std::vector<symbolic::Expression>& e) {
        return output(Eigen::Map<const VectorX<symbolic::Expression>>(e.data(), e.size()));
    }
    ...
    std::unique_ptr<SymbolicVectorSystem<T>> Build() {
        return std::make_unique<SymbolicVectorSystem<T>>(time_var_, state_vars_, input_vars_, parameter_vars_,
                                                         dynamics_, output_, time_period_);
    }
    ...
    const VectorX<symbolic::Variable>& parameter() const { return parameter_vars_; }
    ...
    const symbolic::Expression& dynamics_for_variable(const symbolic::Variable& var) const {
        ...
        if (it != state_var_to_index_.end()) {
            ...
            throw std::out_of_range{
                    fmt::format("This SymbolicVectorSystemBuilder does not have a dynamics for "
                                "the given variable {}",
                                var)};
        }
    }
...
template <>
void SymbolicVectorSystem<symbolic::Expression>::EvaluateWithContext(const Context<symbolic::Expression>& context,
                                                                     const VectorX<symbolic::Expression>& expr,
                                                                     const MatrixX<symbolic::Expression>& jacobian,
                                                                     bool needs_inputs,
