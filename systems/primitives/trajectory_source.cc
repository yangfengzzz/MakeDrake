...
TrajectorySource<T>::TrajectorySource(const Trajectory<T>& trajectory,
    ...
    : SingleOutputVectorSource<T>(SystemTypeTag<TrajectorySource>{}, trajectory.rows() * (1 + output_derivative_order)),
...
TrajectorySource<T>::TrajectorySource(const TrajectorySource<U>& other)
    : SingleOutputVectorSource<T>(SystemTypeTag<TrajectorySource>{}, other.get_output_port().size()),
    ...
    if constexpr (std::is_same_v<U, double>) {
        ...
        if (other.failsafe_trajectory_ == nullptr) {
            throw std::logic_error(fmt::format("System {} of type {} does not support scalar conversion to type {}",
...
template <typename T>
void TrajectorySource<T>::UpdateTrajectory(const trajectories::Trajectory<T>& trajectory) {
    CheckInvariants();
    const int rows = (trajectory_ != nullptr) ? trajectory_->rows() : failsafe_trajectory_->rows();
...
void CalcVectorOutputImpl(const T1& time,
                          const std::unique_ptr<Trajectory<T1>>& trajectory,
    ...
    boolean<T1> want_derivatives =
            !boolean<T1>(clamp_derivatives) || (time >= trajectory->start_time() && time <= trajectory->end_time());
    if constexpr (scalar_predicate<T1>::is_bool) {
        ...
        for (int i = 0; i < ssize(derivatives); ++i) {
            ...
            for (int j = 0; j < len; ++j) {
                (*output)[len * (i + 1) + j] = if_then_else(want_derivatives, value[j], 0.0);
            }
        }
    }
}
...
template <typename T>
void TrajectorySource<T>::DoCalcVectorOutput(const Context<T>& context, Eigen::VectorBlock<VectorX<T>>* output) const {
    CheckInvariants();
    const T time = context.get_time();
    if (failsafe_trajectory_ != nullptr) {
        const double failsafe_time = ExtractFailsafeTime(time);
        CalcVectorOutputImpl(failsafe_time, failsafe_trajectory_, clamp_derivatives_, failsafe_derivatives_, output);
    } else {
        CalcVectorOutputImpl(time, trajectory_, clamp_derivatives_, derivatives_, output);
    }
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::TrajectorySource);
