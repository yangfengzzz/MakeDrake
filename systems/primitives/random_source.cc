...
public:
    ...
    SampleGenerator(Seed seed, RandomDistribution which)
        : seed_(seed), generator_(seed), distribution_(MakeDistributionVariant(which)) {}
...
template <typename T>
RandomSource<T>::RandomSource(RandomDistribution distribution, int num_outputs, double sampling_interval_sec)
    ...
    this->DeclarePeriodicUnrestrictedUpdateEvent(sampling_interval_sec, 0., &RandomSource<T>::UpdateSamples);
...
RandomSource<T>::RandomSource(const RandomSource<U>& other)
    : RandomSource<T>(other.get_distribution(), other.get_output_port(0).size(), other.sampling_interval_sec_) {}
...
template <typename T>
void RandomSource<T>::SetDefaultState(const Context<T>& context, State<T>* state) const {
...
template <typename T>
void RandomSource<T>::SetSeed(Seed seed, const Context<T>& context, State<T>* state) const {
    state->template get_mutable_abstract_state<SampleGenerator>(0) = SampleGenerator(seed, distribution_);
...
int AddRandomInputs(double sampling_interval_sec, DiagramBuilder<T>* builder) {
    ...
    for (const auto* system : builder->GetMutableSystems()) {
        for (int i = 0; i < system->num_input_ports(); i++) {
            ...
            const auto* const source = builder->template AddSystem<RandomSource<T>>(port.get_random_type().value(),
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ::drake::systems::RandomSource);
