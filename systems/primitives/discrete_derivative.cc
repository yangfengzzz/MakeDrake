#include "systems/primitives/discrete_derivative.h"
...
#include "systems/primitives/multiplexer.h"
...
template <typename T>
DiscreteDerivative<T>::DiscreteDerivative(int num_inputs, double time_step, bool suppress_initial_transient)
    ...
    this->DeclareVectorOutputPort("dudt", n_, &DiscreteDerivative<T>::CalcOutput, {this->xd_ticket()});
    ...
    this->DeclarePeriodicDiscreteUpdateEvent(time_step_, 0.0, &DiscreteDerivative<T>::CalcDiscreteUpdate);
    this->DeclareForcedDiscreteUpdateEvent(&DiscreteDerivative<T>::CalcDiscreteUpdate);
}
...
template <typename T>
void DiscreteDerivative<T>::set_input_history(drake::systems::State<T>* state,
...
template <typename T>
EventStatus DiscreteDerivative<T>::CalcDiscreteUpdate(const drake::systems::Context<T>& context,
...
template <typename T>
void DiscreteDerivative<T>::CalcOutput(const drake::systems::Context<T>& context,
    ...
    if (!suppress_initial_transient_) {
        ...
        output_vector->SetFromVector(if_then_else(is_active, derivative, VectorX<T>::Zero(n_).eval()));
    }
}

template <typename T>
StateInterpolatorWithDiscreteDerivative<T>::StateInterpolatorWithDiscreteDerivative(int num_positions,
                                                                                    double time_step,
                                                                                    bool suppress_initial_transient) {
    DiagramBuilder<T> builder;

    derivative_ = builder.template AddSystem<DiscreteDerivative>(num_positions, time_step, suppress_initial_transient);
    auto mux = builder.template AddSystem<Multiplexer>(std::vector<int>{num_positions, num_positions});

    const auto port_index = builder.ExportInput(derivative_->get_input_port(), "position");
...
template <typename T>
bool StateInterpolatorWithDiscreteDerivative<T>::suppress_initial_transient() const {
    return derivative_->suppress_initial_transient();
}

template <typename T>
void StateInterpolatorWithDiscreteDerivative<T>::set_initial_position(
        systems::State<T>* state, const Eigen::Ref<const VectorX<T>>& position) const {
    derivative_->set_input_history(&this->GetMutableSubsystemState(*derivative_, state), position, position);
}

template <typename T>
void StateInterpolatorWithDiscreteDerivative<T>::set_initial_state(systems::State<T>* state,
    ...
    derivative_->set_input_history(&this->GetMutableSubsystemState(*derivative_, state), position,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::DiscreteDerivative);
