...
template <typename T>
Sine<T>::Sine(double amplitude, double frequency, double phase, int size, bool is_time_based)
    : Sine(Eigen::VectorXd::Ones(size) * amplitude,
           Eigen::VectorXd::Ones(size) * frequency,
           Eigen::VectorXd::Ones(size) * phase,
           is_time_based) {}
...
Sine<T>::Sine(const Eigen::VectorXd& amplitudes,
    ...
    value_output_port_index_ =
            this->DeclareVectorOutputPort(kUseDefaultName, amplitudes.size(), &Sine::CalcValueOutput).get_index();
    first_derivative_output_port_index_ =
            this->DeclareVectorOutputPort(kUseDefaultName, amplitudes.size(), &Sine::CalcFirstDerivativeOutput)
                    .get_index();
    second_derivative_output_port_index_ =
            this->DeclareVectorOutputPort(kUseDefaultName, amplitudes.size(), &Sine::CalcSecondDerivativeOutput)
                    .get_index();
}
...
Sine<T>::Sine(const Sine<U>& other)
    : Sine<T>(other.amplitude_vector(), other.frequency_vector(), other.phase_vector(), other.is_time_based()) {}

template <typename T>
double Sine<T>::amplitude() const {
    if (!is_const_amplitude_) {
        throw std::logic_error(
                fmt::format("The amplitude vector, [{}], cannot be represented as a scalar value. "
...
double Sine<T>::frequency() const {
    if (!is_const_frequency_) {
        throw std::logic_error(
                fmt::format("The frequency vector, [{}], cannot be represented as a scalar value. "
...
double Sine<T>::phase() const {
    if (!is_const_phase_) {
        throw std::logic_error(
                fmt::format("The phase vector, [{}], cannot be represented as a scalar value. "
...
template <typename T>
void Sine<T>::CalcValueOutput(const Context<T>& context, BasicVector<T>* output) const {
    VectorX<T> sine_arg;
...
template <typename T>
void Sine<T>::CalcFirstDerivativeOutput(const Context<T>& context, BasicVector<T>* output) const {
    VectorX<T> cos_arg;
    Sine::CalcArg(context, &cos_arg);

    Eigen::VectorBlock<VectorX<T>> output_block = output->get_mutable_value();
    output_block = amplitude_.array() * frequency_.array() * cos_arg.array().cos();
}

template <typename T>
void Sine<T>::CalcSecondDerivativeOutput(const Context<T>& context, BasicVector<T>* output) const {
    VectorX<T> sine_arg;
    Sine::CalcArg(context, &sine_arg);

    Eigen::VectorBlock<VectorX<T>> output_block = output->get_mutable_value();
    output_block = -amplitude_.array() * frequency_.array().pow(2) * sine_arg.array().sin();
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Sine);
