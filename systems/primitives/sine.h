...
public:
    ...
    Sine(double amplitude, double frequency, double phase, int size, bool is_time_based = true);
...
private:
    ...
    void CalcFirstDerivativeOutput(const Context<T>& context, BasicVector<T>* output) const;
    void CalcSecondDerivativeOutput(const Context<T>& context, BasicVector<T>* output) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Sine);
