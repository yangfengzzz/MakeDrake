...
#include "systems/primitives/linear_system_internal.h"
...
LinearSystem<T>::LinearSystem(const LinearSystem<U>& other)
    : LinearSystem<T>(other.A(), other.B(), other.C(), other.D(), other.time_period()) {}
...
LinearSystem<T>::LinearSystem(SystemScalarConverter converter,
                              ...
                              const Eigen::Ref<const Eigen::MatrixXd>& D,
                              double time_period)
    : AffineSystem<T>(std::move(converter), A, B, Eigen::VectorXd(), C, D, Eigen::VectorXd(), time_period) {}
...
std::unique_ptr<AffineSystem<double>> DoFirstOrderTaylorApproximation(
    ...
    const bool is_discrete_system = system.IsDifferenceEquationSystem(&time_period);
    DRAKE_THROW_UNLESS(context.is_stateless() || context.has_only_continuous_state() || is_discrete_system);

    // Create an autodiff version of the system.
    std::unique_ptr<System<AutoDiffXd>> autodiff_system = drake::systems::System<double>::ToAutoDiffXd(system);
    ...
    const InputPort<AutoDiffXd>* input_port = autodiff_system->get_input_port_selection(input_port_index);
    const OutputPort<AutoDiffXd>* output_port = autodiff_system->get_output_port_selection(output_port_index);

    // Verify that the input port is not abstract valued.
    if (input_port && input_port->get_data_type() == PortDataType::kAbstractValued) {
    ...
    const Eigen::VectorXd x0 = context.is_stateless() ? Eigen::VectorXd::Zero(0)
    ...
    if (num_states > 0) {
        if (autodiff_context->has_only_continuous_state()) {
            ...
            if (equilibrium_check_tolerance && !xdot0.isZero(*equilibrium_check_tolerance)) {
            ...
            auto& autodiff_x0 = autodiff_context->get_mutable_discrete_state().get_mutable_vector();
            autodiff_x0.SetFromVector(std::get<0>(autodiff_args));
            std::unique_ptr<DiscreteValues<AutoDiffXd>> autodiff_x1 = autodiff_system->AllocateDiscreteVariables();
            autodiff_x1->SetFrom(autodiff_system->EvalUniquePeriodicDiscreteUpdate(*autodiff_context));
            ...
            if (equilibrium_check_tolerance && !(x1 - x0).isZero(*equilibrium_check_tolerance)) {
                throw std::runtime_error(
                        "The nominal operating point (x0,u0) is not an equilibrium point "
                        "of the system.  Without additional information, a time-invariant "
                        "linearization of this system is not well defined.");
    ...
    return std::make_unique<AffineSystem<double>>(A, B, f0, C, D, y0, time_period);
}
...
std::unique_ptr<LinearSystem<double>> Linearize(const System<double>& system,
    ...
    std::unique_ptr<AffineSystem<double>> affine = DoFirstOrderTaylorApproximation(
            system, context, std::move(input_port_index), std::move(output_port_index), equilibrium_check_tolerance);

    return std::make_unique<LinearSystem<double>>(affine->A(), affine->B(), affine->C(), affine->D(),
                                                  affine->time_period());
}
...
std::unique_ptr<AffineSystem<double>> FirstOrderTaylorApproximation(
        ...
        std::variant<InputPortSelection, InputPortIndex> input_port_index,
        std::variant<OutputPortSelection, OutputPortIndex> output_port_index) {
    return DoFirstOrderTaylorApproximation(system, context, std::move(input_port_index), std::move(output_port_index));
}
...
bool IsControllable(const LinearSystem<double>& sys, std::optional<double> threshold) {
...
bool IsObservable(const LinearSystem<double>& sys, std::optional<double> threshold) {
    return internal::IsObservable(sys.A(), sys.C(), threshold);
}

bool IsStabilizable(const LinearSystem<double>& sys, std::optional<double> threshold) {
    const bool continuous_time = sys.time_period() <= 0;
    return internal::IsStabilizable(sys.A(), sys.B(), continuous_time, threshold);
}

bool IsDetectable(const LinearSystem<double>& sys, std::optional<double> threshold) {
    const bool continuous_time = sys.time_period() <= 0;
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::LinearSystem);

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::TimeVaryingLinearSystem);
