...
public:
    ...
    MultilayerPerceptron(const std::vector<int>& layers, PerceptronActivationType activation_type = kTanh);
    ...
    MultilayerPerceptron(const std::vector<int>& layers, const std::vector<PerceptronActivationType>& activation_types);
    ...
    MultilayerPerceptron(const std::vector<bool>& use_sin_cos_for_input,
                         const std::vector<int>& remaining_layers,
                         const std::vector<PerceptronActivationType>& activation_types);
    ...
    void SetRandomParameters(const Context<T>& context,
    ...
    PerceptronActivationType activation_type(int layer) const {
        DRAKE_DEMAND(layer >= 0 && layer < static_cast<int>(activation_types_.size()));
    ...
    /** Returns a mutable reference to all of the parameters (weights and biases)
     as a single vector. */
    Eigen::VectorBlock<VectorX<T>> GetMutableParameters(Context<T>* context) const;
    ...
    void SetParameters(Context<T>* context, const Eigen::Ref<const VectorX<T>>& params) const;

    /** Returns the weights used in the mapping from `layer` to `layer+1`.
     @pydrake_mkdoc_identifier{context} */
    Eigen::Map<const MatrixX<T>> GetWeights(const Context<T>& context, int layer) const;

    /** Returns the biases used in the mapping from `layer` to `layer+1`.
     @pydrake_mkdoc_identifier{context} */
    Eigen::Map<const VectorX<T>> GetBiases(const Context<T>& context, int layer) const;

    /** Sets the weights in the `context` used in the mapping from `layer` to
     `layer+1`.
      @pydrake_mkdoc_identifier{context} */
    void SetWeights(Context<T>* context, int layer, const Eigen::Ref<const MatrixX<T>>& W) const;

    /** Sets the biases in the `context` used in the mapping from `layer` to
     `layer+1`.
      @pydrake_mkdoc_identifier{context} */
    void SetBiases(Context<T>* context, int layer, const Eigen::Ref<const VectorX<T>>& b) const;

    /** Returns the weights in `params` used in the mapping from `layer` to
     `layer+1`.
     @pydrake_mkdoc_identifier{vector} */
    Eigen::Map<const MatrixX<T>> GetWeights(const Eigen::Ref<const VectorX<T>>& params, int layer) const;

    /** Returns the biases in `params` used in the mapping from `layer` to
     `layer+1`.
     @pydrake_mkdoc_identifier{vector} */
    Eigen::Map<const VectorX<T>> GetBiases(const Eigen::Ref<const VectorX<T>>& params, int layer) const;

    /** Sets the weights in `params` used in the mapping from `layer` to
     `layer+1`.
     @pydrake_mkdoc_identifier{vector} */
    void SetWeights(EigenPtr<VectorX<T>> params, int layer, const Eigen::Ref<const MatrixX<T>>& W) const;

    /** Sets the biases in `params` used in the mapping from `layer` to `layer+1`.
     @pydrake_mkdoc_identifier{vector} */
    void SetBiases(EigenPtr<VectorX<T>> params, int layer, const Eigen::Ref<const VectorX<T>>& b) const;
    ...
    T BackpropagationMeanSquaredError(const Context<T>& context,
                                      const Eigen::Ref<const MatrixX<T>>& X,
...
private:
    ...
    void CalcLayers(const Context<T>& context, internal::CalcLayersData<T>* data) const;

    // Calculates the (potentially batch) feature vector values.  When `X` is
    // size `num_inputs`-by-`N`, then `Features` is set to size
    // `layers()[0]`-by-`N`.
    void CalcInputFeatures(const Eigen::Ref<const MatrixX<T>>& X, MatrixX<T>* input_features) const;
