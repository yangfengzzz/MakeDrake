...
public:
    ...
    explicit PortSwitch(const OutputType& model_value)
        : PortSwitch(-1, AbstractValue::Make<OutputType>(model_value), nullptr, nullptr) {}
    ...
    explicit PortSwitch(const OutputType<T>& dummy_value)
        : PortSwitch(-1,
                     std::unique_ptr<AbstractValue>(new Value<OutputType<double>>()),
                     std::unique_ptr<AbstractValue>(new Value<OutputType<AutoDiffXd>>()),
                     std::unique_ptr<AbstractValue>(new Value<OutputType<symbolic::Expression>>())) {
        unused(dummy_value);
    }
    ...
    type InputPortIndex. */
    const InputPort<T>& get_port_selector_input_port() const { return this->get_input_port(0); }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::PortSwitch);
