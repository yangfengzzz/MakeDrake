...
template <typename T>
FirstOrderLowPassFilter<T>::FirstOrderLowPassFilter(double time_constant, int size)
    : FirstOrderLowPassFilter(VectorX<double>::Ones(size) * time_constant) {}

template <typename T>
FirstOrderLowPassFilter<T>::FirstOrderLowPassFilter(const VectorX<double>& time_constants)
    : VectorSystem<T>(SystemTypeTag<FirstOrderLowPassFilter>{}, time_constants.size(), time_constants.size()),
...
template <typename T>
template <typename U>
FirstOrderLowPassFilter<T>::FirstOrderLowPassFilter(const FirstOrderLowPassFilter<U>& other)
...
double FirstOrderLowPassFilter<T>::get_time_constant() const {
    if (!time_constants_.isConstant(time_constants_[0])) {
        throw std::domain_error(
                fmt::format("The time constants vector, [{}], cannot be represented as a scalar "
...
template <typename T>
const VectorX<double>& FirstOrderLowPassFilter<T>::get_time_constants_vector() const {
    return time_constants_;
}

template <typename T>
void FirstOrderLowPassFilter<T>::set_initial_output_value(Context<T>* context,
...
template <typename T>
void FirstOrderLowPassFilter<T>::DoCalcVectorTimeDerivatives(const Context<T>&,
...
template <typename T>
void FirstOrderLowPassFilter<T>::DoCalcVectorOutput(const Context<T>&,
                                                    const Eigen::VectorBlock<const VectorX<T>>& input,
                                                    const Eigen::VectorBlock<const VectorX<T>>& state,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::FirstOrderLowPassFilter);
