...
template <typename T>
PortSwitch<T>::PortSwitch(int vector_size) : PortSwitch(vector_size, nullptr, nullptr, nullptr) {
...
PortSwitch<T>::PortSwitch(const PortSwitch<U>& other)
    : PortSwitch<T>(
              other.vector_size_, other.model_value_double_, other.model_value_autodiff_, other.model_value_symbolic_) {
...
PortSwitch<T>::PortSwitch(int vector_size,
    ...
    : LeafSystem<T>(SystemTypeTag<PortSwitch>()),
      ...
      model_value_autodiff_(model_value_autodiff ? model_value_autodiff : model_value_double),
      model_value_symbolic_(model_value_symbolic ? model_value_symbolic : model_value_double) {
    ...
    if (vector_size_ > 0) {
        ...
        this->DeclareVectorOutputPort("value", vector_size_, &PortSwitch<T>::CopyVectorOut);
    } else {
        ...
        this->DeclareAbstractOutputPort("value", abstract_value_allocator,
...
const InputPort<T>& PortSwitch<T>::DeclareInputPort(std::string name) {
    if (vector_size_ >= 0) {
        return LeafSystem<T>::DeclareInputPort(std::move(name), kVectorValued, vector_size_);
    }
    return this->DeclareAbstractInputPort(std::move(name), get_model_value());
}

template <typename T>
void PortSwitch<T>::CopyVectorOut(const Context<T>& context, BasicVector<T>* vector) const {
    const InputPortIndex selector = get_port_selector_input_port().template Eval<InputPortIndex>(context);
    DRAKE_DEMAND(selector >= 0 && selector < this->num_input_ports());
    vector->set_value(this->get_input_port(selector).Eval(context));
}

template <typename T>
void PortSwitch<T>::CopyValueOut(const Context<T>& context, AbstractValue* value) const {
    const InputPortIndex selector = get_port_selector_input_port().template Eval<InputPortIndex>(context);
    DRAKE_DEMAND(selector >= 0 && selector < this->num_input_ports());
    value->SetFrom(this->get_input_port(selector).template Eval<AbstractValue>(context));
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::PortSwitch);
