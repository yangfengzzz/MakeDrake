...
WrapToSystem<T>::WrapToSystem(int input_size) : input_size_(input_size) {
    ...
    this->DeclareVectorOutputPort(kUseDefaultName, input_size_, &WrapToSystem::CalcWrappedOutput);
}
...
template <typename T>
void WrapToSystem<T>::CalcWrappedOutput(const Context<T>& context, BasicVector<T>* output) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::WrapToSystem);
