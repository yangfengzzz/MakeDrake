#include "systems/sensors/lcm_image_array_to_images.h"
...
#include "systems/sensors/vtk_image_reader_writer.h"
...
bool DecompressVtk(ImageFileFormat format, const lcmt_image* lcm_image, Image<kPixelType>* image) {
    vtkSmartPointer<vtkImageReader2> reader =
            internal::MakeReader(format, lcm_image->data.data(), lcm_image->data.size());
    reader->Update();
    ...
    if (exporter->GetDataMemorySize() != image->width() * image->height() * image->kPixelSize) {
        drake::log()->error("Malformed output decoding incoming LCM {} image", format);
        return false;
    }
...
bool DecompressZlib(const lcmt_image* lcm_image, Image<kPixelType>* image) {
    ...
    const int status =
            uncompress(reinterpret_cast<Bytef*>(image->at(0, 0)), &dest_len, lcm_image->data.data(), lcm_image->size);
    if (status != Z_OK) {
...
bool UnpackLcmImage(const lcmt_image* lcm_image, Image<kPixelType>* image) {
    DRAKE_DEMAND(lcm_image->pixel_format == LcmPixelTraits<Image<kPixelType>::kPixelFormat>::kPixelFormat);
    DRAKE_DEMAND(lcm_image->channel_type == LcmImageTraits<kPixelType>::kChannelType);
    ...
    drake::log()->error("Unsupported LCM compression method: {}", lcm_image->compression_method);
    *image = Image<kPixelType>();
    return false;
}
...
LcmImageArrayToImages::LcmImageArrayToImages()
    : image_array_t_input_port_index_(
              this->DeclareAbstractInputPort("image_array_t", Value<lcmt_image_array>()).get_index()),
      color_image_output_port_index_(
              this->DeclareAbstractOutputPort("color_image", &LcmImageArrayToImages::CalcColorImage).get_index()),
      depth_image_output_port_index_(
              this->DeclareAbstractOutputPort("depth_image", &LcmImageArrayToImages::CalcDepthImage).get_index()),
      label_image_output_port_index_(
              this->DeclareAbstractOutputPort("label_image", &LcmImageArrayToImages::CalcLabelImage).get_index()) {
    // TODO(sammy-tri) Calculating our output ports can be kinda expensive.  We
    // should cache the images.
}

void LcmImageArrayToImages::CalcColorImage(const Context<double>& context, ImageRgba8U* color_image) const {
    const auto& images = image_array_t_input_port().Eval<lcmt_image_array>(context);
...
void LcmImageArrayToImages::CalcDepthImage(const Context<double>& context, ImageDepth32F* depth_image) const {
    const auto& images = image_array_t_input_port().Eval<lcmt_image_array>(context);
    ...
    switch (lcm_image->channel_type) {
        ...
        default: {
            drake::log()->error("Unsupported depth image channel type: {}", lcm_image->channel_type);
            *depth_image = ImageDepth32F();
            return;
        }
    }
...
void LcmImageArrayToImages::CalcLabelImage(const Context<double>& context, ImageLabel16I* label_image) const {
    const auto& images = image_array_t_input_port().Eval<lcmt_image_array>(context);
    ...
    if (lcm_image->channel_type != lcmt_image::CHANNEL_TYPE_INT16) {
        drake::log()->error("Unsupported label image channel type: {}", lcm_image->channel_type);
        *label_image = {};
        return;
    }
