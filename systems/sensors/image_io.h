...
#include "systems/sensors/image_file_format.h"
...
public:
    ...
    std::optional<Metadata> LoadMetadata(const std::filesystem::path& path) const { return LoadMetadataImpl(&path); }

    /** Returns the metadata of the given image buffer, or nullopt if the metadata
    cannot be determined or is unsupported. */
    std::optional<Metadata> LoadMetadata(ByteSpan buffer) const { return LoadMetadataImpl(buffer); }
...
private:
    ...
    using OutputAny = std::variant<const std::filesystem::path*, std::vector<uint8_t>*>;

    // Implementation functions for the public API.
    std::optional<Metadata> LoadMetadataImpl(InputAny input_any) const;
    ImageAny LoadImpl(InputAny input_any, std::optional<ImageFileFormat> format) const;
    void LoadImpl(InputAny input_any, std::optional<ImageFileFormat> format, ImageAnyMutablePtr image_any) const;
    void SaveImpl(ImageAnyConstPtr image_any, std::optional<ImageFileFormat> format, OutputAny output_any) const;

    // Helpers.
    struct LoaderTools;
    LoaderTools MakeLoaderTools(InputAny input_any, std::optional<ImageFileFormat> format) const;
