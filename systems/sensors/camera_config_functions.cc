...
#include "systems/lcm/lcm_config_functions.h"
...
const std::string& LookupEngineType(const std::string& class_name) {
    ...
    static const never_destroyed<Dict> type_lookup(std::initializer_list<Dict::value_type>{
            {"RenderEngineVtk", "drake::geometry::render_vtk::internal::RenderEngineVtk"},
            {"RenderEngineGl", "drake::geometry::render_gl::internal::RenderEngineGl"},
...
const char* GetEngineName(const ParamsType&) {
    ...
    } else if constexpr (std::is_same_v<ParamsType, RenderEngineGltfClientParams>) {
        return "RenderEngineGltfClient";
    }
}

void MakeEngineByClassName(const std::string& class_name, const CameraConfig& config, SceneGraph<double>* scene_graph) {
    if (class_name == "RenderEngineGl") {
        ...
        scene_graph->AddRenderer(config.renderer_name, MakeRenderEngineGltfClient({}));
        return;
    }
...
void ValidateEngineAndMaybeAdd(const CameraConfig& config, SceneGraph<double>* scene_graph) {
    ...
    const std::string type_name = scene_graph->GetRendererTypeName(config.renderer_name);

    // Non-empty type name says that it already exists.
    bool already_exists = !type_name.empty();

    if (already_exists) {
        std::visit<void>(overloaded{[&type_name, &config](const std::string& class_name) {
                                        if (!class_name.empty() && LookupEngineType(class_name) != type_name) {
                                            throw std::logic_error(
                                                    fmt::format("Invalid camera configuration; requested renderer_name "
    ...
    std::visit<void>(overloaded{[&config, scene_graph](const std::string& class_name) {
                                ...
                                [&config, scene_graph](const RenderEngineGltfClientParams& params) {
                                    scene_graph->AddRenderer(config.renderer_name, MakeRenderEngineGltfClient(params));
                                }},
                     config.renderer_class);
}
...
void ApplyCameraConfig(const CameraConfig& config,
    ...
    if (scene_graph == nullptr) {
        scene_graph = &builder->GetMutableDowncastSubsystemByName<SceneGraph>("scene_graph");
    }
    ...
    const Frame<double>& base_frame =
            config.X_PB.base_frame ? GetScopedFrameByName(*plant, *config.X_PB.base_frame) : plant->world_frame();
    ...
    if (config.output_delay == 0.0) {
        const SimRgbdSensor sim_camera(config.name, base_frame, config.fps, X_PB, color_camera, depth_camera);
        ...
        camera_sys = builder->AddSystem<RgbdSensorAsync>(
                scene_graph, frame_A, X_AB, config.fps, config.capture_offset, config.output_delay,
                (config.rgb || config.label) ? std::optional<ColorRenderCamera>{color_camera} : std::nullopt,
                config.depth ? std::optional<DepthRenderCamera>{depth_camera} : std::nullopt, config.label);
    ...
    // Find the LCM bus.
    lcm = FindOrCreateLcmBus(lcm, lcm_buses, builder, "ApplyCameraConfig", config.lcm_bus);
    ...
    const double lcm_publisher_offset = config.capture_offset + (1.001 * config.output_delay);

    // Connect the sensor to the lcm system.
    AddSimRgbdSensorLcmPublisher(config.name, config.fps, lcm_publisher_offset,
                                 config.rgb ? &camera_sys->GetOutputPort("color_image") : nullptr,
                                 config.depth ? &camera_sys->GetOutputPort("depth_image_16u") : nullptr,
                                 config.label ? &camera_sys->GetOutputPort("label_image") : nullptr, config.do_compress,
                                 builder, lcm);
}
