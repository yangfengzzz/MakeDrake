...
vtkSmartPointer<vtkImageReader2> MakeReader(ImageFileFormat format, const fs::path& filename) {
...
public:
    ...
    void set_progress_callback(std::function<void(double)> callback) { callback_ = std::move(callback); }
...
vtkSmartPointer<vtkImageWriter> MakeWriter(ImageFileFormat format, const fs::path& filename) {
