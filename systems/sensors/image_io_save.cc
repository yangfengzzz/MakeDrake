...
void CopyImage(const Image<kPixelType>& source_image, vtkImageData* dest_image) {
    ...
    dest_image->AllocateScalars(internal::GetVtkScalarType<kPixelType>(), num_channels);
...
void ImageIo::SaveImpl(ImageAnyConstPtr image_any, std::optional<ImageFileFormat> format, OutputAny output_any) const {
    ...
    const ImageFileFormat chosen_format = [format, output_any]() {
        if (format.has_value()) {
            ...
            if (std::optional<ImageFileFormat> from_ext = internal::FileFormatFromExtension(path)) {
                return *from_ext;
            } else {
                throw std::logic_error(
                        fmt::format("ImageIo::Save(path='{}') requires SetFileFormat() to be called "
