#include "systems/sensors/rotary_encoders.h"
...
RotaryEncoders<T>::RotaryEncoders(const std::vector<int>& ticks_per_revolution)
    : RotaryEncoders(ticks_per_revolution.size() /* input_port_size */,
                     vector_iota(ticks_per_revolution.size()) /* input_vector_indices */,
                     ticks_per_revolution) {}

template <typename T>
RotaryEncoders<T>::RotaryEncoders(int input_port_size, const std::vector<int>& input_vector_indices)
    : RotaryEncoders(input_port_size, input_vector_indices, std::vector<int>() /* ticks_per_revolution */) {}
...
RotaryEncoders<T>::RotaryEncoders(int input_port_size,
    ...
    : VectorSystem<T>(
              SystemTypeTag<RotaryEncoders>{}, input_port_size, input_vector_indices.size() /* output_port_size */),
    ...
    DRAKE_ASSERT(*std::max_element(indices_.begin(), indices_.end()) < input_port_size);
    DRAKE_DEMAND(ticks_per_revolution_.empty() || indices_.size() == ticks_per_revolution_.size());
    DRAKE_ASSERT(ticks_per_revolution_.empty() ||
                 *std::min_element(ticks_per_revolution_.begin(), ticks_per_revolution_.end()) >= 0);

    // This vector is numeric parameter 0.
    this->DeclareNumericParameter(BasicVector<T>(VectorX<T>::Zero(num_encoders_)));
}

template <typename T>
template <typename U>
RotaryEncoders<T>::RotaryEncoders(const RotaryEncoders<U>& other)
    : RotaryEncoders(other.get_input_port().size(), other.indices_, other.ticks_per_revolution_) {}

template <typename T>
void RotaryEncoders<T>::DoCalcVectorOutput(const Context<T>& context,
                                           const Eigen::VectorBlock<const VectorX<T>>& input,
                                           const Eigen::VectorBlock<const VectorX<T>>& state,
                                           Eigen::VectorBlock<VectorX<T>>* output) const {
    unused(state);

    const VectorX<T>& calibration_offsets = context.get_numeric_parameter(0).value();
...
template <typename T>
void RotaryEncoders<T>::set_calibration_offsets(Context<T>* context,
...
template <typename T>
const VectorX<T>& RotaryEncoders<T>::get_calibration_offsets(const Context<T>& context) const {
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::sensors::RotaryEncoders);
