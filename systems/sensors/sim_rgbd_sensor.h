...
public:
    ...
    SimRgbdSensor(const std::string& serial,
                  const multibody::Frame<double>& frame_P,
                  double rate_hz,
    ...
    const geometry::render::ColorRenderCamera& color_properties() const { return color_properties_; }

    const geometry::render::DepthRenderCamera& depth_properties() const { return depth_properties_; }
...
std::pair<geometry::FrameId, math::RigidTransformd> GetGeometryFrame(const multibody::Frame<double>& plant_frame,
                                                                     const math::RigidTransformd& X_PB);
