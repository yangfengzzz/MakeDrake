...
const InputPort<double>& ImageWriter::DeclareImageInputPort(std::string port_name,
                                                            std::string file_name_format,
                                                            double publish_period,
                                                            double start_time) {
    // Test to confirm valid pixel type.
    static_assert(kPixelType == PixelType::kRgba8U || kPixelType == PixelType::kDepth32F ||
                          kPixelType == PixelType::kDepth16U || kPixelType == PixelType::kLabel16I ||
    ...
    const std::string test_dir = DirectoryFromFormat(file_name_format, port_name, kPixelType);
    ...
    if (file_name_format.substr(file_name_format.size() - extension.size()) != extension) {
    ...
    const auto& port = DeclareAbstractInputPort(port_name, Value<Image<kPixelType>>());

    // There is no DeclarePeriodicPublishEvent that accepts a lambda, so we must
    // use the advanced API to add our event.
    PublishEvent<double> event(TriggerType::kPeriodic, [port_index = port.get_index()](const System<double>& system,
...
const InputPort<double>& ImageWriter::DeclareImageInputPort(PixelType pixel_type,
    ...
    switch (pixel_type) {
        ...
        case PixelType::kRgba8U: {
            return this->template DeclareImageInputPort<PixelType::kRgba8U>(
                    std::move(port_name), std::move(file_name_format), publish_period, start_time);
        }
    ...
    throw std::logic_error(fmt::format("ImageWriter::DeclareImageInputPort does not support pixel_type={}",
...
void ImageWriter::WriteImage(const Context<double>& context, int index) const {
    ...
    ImageIo{}.Save(image,
                   MakeFileName(data.format, data.pixel_type, context.get_time(), port.get_name(), data.count++));
}
...
std::string ImageWriter::MakeFileName(
        const std::string& format, PixelType pixel_type, double time, const std::string& port_name, int count) const {
    ...
    return fmt::format(fmt_runtime(format), fmt::arg("port_name", port_name),
                       fmt::arg("image_type", labels_.at(pixel_type)), fmt::arg("time_double", time),
                       fmt::arg("time_usec", u_time), fmt::arg("time_msec", m_time), fmt::arg("count", count));
}
...
std::string ImageWriter::DirectoryFromFormat(const std::string& format,
    ...
    if (format.back() == '/') {
        throw std::logic_error(fmt::format("ImageWriter: The file_name_format '{}' cannot end with a '/'", format));
    }
    ...
    if (!match.empty()) {
        throw std::logic_error("ImageWriter: The directory path cannot include time or image count");
    }
    return MakeFileName(dir_format, pixel_type, 0, port_name, 0);
}

ImageWriter::FolderState ImageWriter::ValidateDirectory(const std::string& file_path_str) {
...
template const InputPort<double>& ImageWriter::DeclareImageInputPort<PixelType::kRgba8U>(std::string port_name,
                                                                                         std::string file_name_format,
                                                                                         double publish_period,
                                                                                         double start_time);
template const InputPort<double>& ImageWriter::DeclareImageInputPort<PixelType::kDepth32F>(std::string port_name,
                                                                                           std::string file_name_format,
                                                                                           double publish_period,
                                                                                           double start_time);
template const InputPort<double>& ImageWriter::DeclareImageInputPort<PixelType::kLabel16I>(std::string port_name,
                                                                                           std::string file_name_format,
                                                                                           double publish_period,
                                                                                           double start_time);
template const InputPort<double>& ImageWriter::DeclareImageInputPort<PixelType::kDepth16U>(std::string port_name,
                                                                                           std::string file_name_format,
                                                                                           double publish_period,
                                                                                           double start_time);
template const InputPort<double>& ImageWriter::DeclareImageInputPort<PixelType::kGrey8U>(std::string port_name,
                                                                                         std::string file_name_format,
                                                                                         double publish_period,
                                                                                         double start_time);
...
template void ImageWriter::WriteImage<PixelType::kDepth16U>(const Context<double>& context, int index) const;
template void ImageWriter::WriteImage<PixelType::kGrey8U>(const Context<double>& context, int index) const;
