...
#include "systems/sensors/image_io_internal.h"
...
struct DRAKE_NO_EXPORT ImageIo::LoaderTools {
    ...
    std::unique_ptr<std::vector<DiagnosticDetail>> errors = std::make_unique<std::vector<DiagnosticDetail>>();
    std::unique_ptr<DiagnosticPolicy> diagnostic = std::make_unique<DiagnosticPolicy>();
...
ImageIo::LoaderTools ImageIo::MakeLoaderTools(InputAny input_any, std::optional<ImageFileFormat> format) const {
    ...
    const std::string diagnostic_filename = (input_any.index() == 0) ? std::get<0>(input_any)->string() : "ImageIo";
    tools.diagnostic->SetActionForErrors(
            [diagnostic_filename, errors = tools.errors.get()](const DiagnosticDetail& detail) {
    ...
    if (format.has_value()) {
        tools.format = *format;
    } else if (std::optional<ImageFileFormat> guess = internal::GuessFileFormat(input_any)) {
        tools.format = *guess;
    } else {
    ...
    if (input_any.index() == 0) {
        ...
        tools.reader = internal::MakeReader(tools.format, std::get<1>(input_any).data, std::get<1>(input_any).size);
    }
    ...
    if (std::optional<PixelScalar> pixel_scalar = internal::GetDrakeScalarType(vtk_scalar)) {
        tools.metadata.scalar = *pixel_scalar;
    } else {
        tools.diagnostic->Error(fmt::format("The image uses an unsupported scalar type (VTK type {})", vtk_scalar));
...
void CopyVtkToDrakeImage(const LoaderTools& tools, Image<kPixelType>* image) {
    ...
    if (metadata.scalar != expected_scalar) {
        ...
        const bool allow_hack = ((metadata.scalar == PixelScalar::k16U) && (expected_scalar == PixelScalar::k16I));
        if (!allow_hack) {
            tools.diagnostic->Error(
                    fmt::format("Can't load image with scalar={} into scalar={}.", metadata.scalar, expected_scalar));
            return;
        }
    }
    ...
    if (!add_alpha && !drop_alpha && (metadata.channels != num_channels)) {
        tools.diagnostic->Error(fmt::format("Can't load image with channels={} into object with channels={}.",
    ...
    const T* const source = reinterpret_cast<T*>(tools.loader->GetPointerToData());
    ...
    if constexpr (sizeof(T) > 1) {
        ...
        internal::CopyAndFlipRaw<T, num_channels, swizzle>(source, dest, width, height);
    } else {
        if (add_alpha) {
...
ImageAny ImageIo::LoadImpl(InputAny input_any, std::optional<ImageFileFormat> format) const {
    // Parse the image.
    ...
    if (match) {
        ...
        tools.diagnostic->Error(
                fmt::format("Can't load image (depth={}, channels={}, scalar={}) into any known "
...
void ImageIo::LoadImpl(InputAny input_any, std::optional<ImageFileFormat> format, ImageAnyMutablePtr image_any) const {
    // Parse the image.
    LoaderTools tools = MakeLoaderTools(input_any, format);
    FlushDiagnostics(tools);
