...
std::optional<ImageFileFormat> FileFormatFromExtension(const std::filesystem::path& filename);
...
std::optional<ImageFileFormat> GuessFileFormat(std::variant<const std::filesystem::path*, ImageIo::ByteSpan> input_any);
...
template <typename T, int source_channels, bool swizzle, int dest_channels = source_channels, uint8_t dest_pad = 0xff>
