...
std::pair<geometry::FrameId, RigidTransformd> GetGeometryFrame(const multibody::Frame<double>& sensor_frame,
    ...
    const std::optional<geometry::FrameId> body_A_id = plant.GetBodyFrameIdIfExists(body_A.index());
...
RgbdSensor* AddSimRgbdSensor(const SceneGraph<double>& scene_graph,
    ...
    const auto [frame_A, X_AB] = GetGeometryFrame(sim_rgbd_sensor.frame(), sim_rgbd_sensor.X_PB());
    auto* rgbd_sensor_sys = builder->AddSystem<RgbdSensor>(frame_A, X_AB, sim_rgbd_sensor.color_properties(),
    ...
    builder->Connect(scene_graph.get_query_output_port(), rgbd_sensor_sys->query_object_input_port());
    return rgbd_sensor_sys;
}

void AddSimRgbdSensorLcmPublisher(std::string_view serial,
                                  double fps,
    ...
    auto image_to_lcm_image_array = builder->AddSystem<ImageToLcmImageArrayT>(do_compress);
    image_to_lcm_image_array->set_name(fmt::format("image_to_lcm_{}", serial));
    if (depth_16u_port) {
        const auto& lcm_depth_port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kDepth16U>("depth");
        builder->Connect(*depth_16u_port, lcm_depth_port);
    }
    if (rgb_port) {
        const auto& lcm_rgb_port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kRgba8U>("rgb");
        builder->Connect(*rgb_port, lcm_rgb_port);
    }
    if (label_port) {
        const auto& lcm_label_port = image_to_lcm_image_array->DeclareImageInputPort<PixelType::kLabel16I>("label");
        builder->Connect(*label_port, lcm_label_port);
    }
    auto image_array_lcm_publisher = builder->AddSystem(lcm::LcmPublisherSystem::Make<lcmt_image_array>(
            fmt::format("DRAKE_RGBD_CAMERA_IMAGES_{}", serial), lcm, 1.0 / fps, publish_offset));
...
void AddSimRgbdSensorLcmPublisher(const SimRgbdSensor& sim_camera,
    ...
    AddSimRgbdSensorLcmPublisher(sim_camera.serial(), sim_camera.rate_hz(), publish_offset, rgb_port, depth_16u_port,
