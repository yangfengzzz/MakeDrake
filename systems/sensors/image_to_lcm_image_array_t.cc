...
#include "lcmt_image_array.hpp"
#include "systems/sensors/lcm_image_traits.h"
...
void Compress(const Image<kPixelType>& image, lcmt_image* msg) {
    ...
    auto compress_status = compress2(dest.data(), &dest_size, reinterpret_cast<const Bytef*>(image.at(0, 0)),
...
template <PixelType kPixelType>
void PackImageToLcmImageT(const Image<kPixelType>& image, lcmt_image* msg, bool do_compress) {
    msg->width = image.width();
    msg->height = image.height();
    msg->row_stride = image.kPixelSize * msg->width;
    msg->bigendian = false;
    msg->pixel_format = LcmPixelTraits<ImageTraits<kPixelType>::kPixelFormat>::kPixelFormat;
...
// Overwrites everything in msg except its header.
void PackImageToLcmImageT(const AbstractValue& untyped_image, PixelType pixel_type, lcmt_image* msg, bool do_compress) {
    switch (pixel_type) {
        case PixelType::kRgb8U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kRgb8U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kBgr8U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kBgr8U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kRgba8U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kRgba8U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kBgra8U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kBgra8U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kGrey8U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kGrey8U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kDepth16U: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kDepth16U>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kDepth32F: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kDepth32F>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
        case PixelType::kLabel16I: {
            const auto& image_value = untyped_image.get_value<Image<PixelType::kLabel16I>>();
            PackImageToLcmImageT(image_value, msg, do_compress);
            break;
        }
    }
...
ImageToLcmImageArrayT::ImageToLcmImageArrayT(bool do_compress) : do_compress_(do_compress) {
    image_array_t_msg_output_port_index_ =
            DeclareAbstractOutputPort(kUseDefaultName, &ImageToLcmImageArrayT::CalcImageArray).get_index();
}
...
ImageToLcmImageArrayT::ImageToLcmImageArrayT(const string& color_frame_name,
    ...
    color_image_input_port_index_ = DeclareImageInputPort<PixelType::kRgba8U>(color_frame_name).get_index();
    depth_image_input_port_index_ = DeclareImageInputPort<PixelType::kDepth32F>(depth_frame_name).get_index();
    label_image_input_port_index_ = DeclareImageInputPort<PixelType::kLabel16I>(label_frame_name).get_index();
...
void ImageToLcmImageArrayT::CalcImageArray(const systems::Context<double>& context, lcmt_image_array* msg) const {
    ...
    // them.)
    const int64_t utime = static_cast<int64_t>(context.get_time() * kSecToMillisec);
    msg->header = {};
    ...
    for (int i = 0; i < num_inputs; i++) {
        ...
        const auto& value = this->get_input_port(i).template Eval<AbstractValue>(context);
