...
std::optional<ImageFileFormat> FileFormatFromExtension(const fs::path& filename) {
...
template <size_t N>
std::array<uint8_t, N> ReadHeader(std::variant<const fs::path*, ImageIo::ByteSpan> input_any) {
...
std::optional<ImageFileFormat> GuessFileFormat(std::variant<const fs::path*, ImageIo::ByteSpan> input_any) {
