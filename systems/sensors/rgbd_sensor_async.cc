...
public:
    ...
    explicit QueryObjectChef(const SceneGraph<double>* scene_graph) : scene_graph_(scene_graph) {
        ...
        scratch_index_ = this->DeclareCacheEntry("scratch", ValueProducer(scratch, &ValueProducer::NoopCalc),
                                                 {this->nothing_ticket()})
                                 .cache_index();
...
public:
    ...
    SnapshotSensor(const SceneGraph<double>* scene_graph,
                   FrameId parent_id,
        ...
        auto* rgbd = builder.AddNamedSystem<RgbdSensor>("camera", parent_id, X_PB, color_camera, depth_camera);
...
public:
    ...
    Worker(std::shared_ptr<const SnapshotSensor> sensor, bool color, bool depth, bool label)
        : sensor_{std::move(sensor)}, color_{color}, depth_{depth}, label_{label} {
...
RgbdSensorAsync::RgbdSensorAsync(const SceneGraph<double>* scene_graph,
    ...
    DeclarePeriodicUnrestrictedUpdateEvent(1 / fps, capture_offset, &Self::CalcTick);
    DeclarePeriodicUnrestrictedUpdateEvent(1 / fps, capture_offset + output_delay, &Self::CalcTock);
...
const OutputPort<double>& RgbdSensorAsync::body_pose_in_world_output_port() const {
...
const RgbdSensorAsync::TickTockState& RgbdSensorAsync::get_state(const Context<double>& context) const {
    return context.template get_abstract_state<TickTockState>(0);
}

RgbdSensorAsync::TickTockState& RgbdSensorAsync::get_mutable_state(State<double>* state) const {
    DRAKE_DEMAND(state != nullptr);
    return state->template get_mutable_abstract_state<TickTockState>(0);
}

EventStatus RgbdSensorAsync::Initialize(const Context<double>& context, State<double>* state) const {
    ...
    // output.
    auto sensor = std::make_shared<const SnapshotSensor>(scene_graph_, parent_id_, X_PB_, std::move(*color),
                                                         std::move(*depth));
    next_state.worker = std::make_shared<Worker>(std::move(sensor), color_camera_.has_value(),
                                                 depth_camera_.has_value(), render_label_image_);
    next_state.output = {};
    return EventStatus::Succeeded();
}

void RgbdSensorAsync::CalcTick(const Context<double>& context, State<double>* state) const {
...
void RgbdSensorAsync::CalcTock(const Context<double>& context, State<double>* state) const {
    // Grab the downcast references from our arguments.
    const TickTockState& prior_state = get_state(context);
    TickTockState& next_state = get_mutable_state(state);
...
void RgbdSensorAsync::CalcColor(const Context<double>& context, ImageRgba8U* output) const {
    DRAKE_DEMAND(color_camera_.has_value());
    CopyImage(get_state(context).output.color.get(), output);
}

void RgbdSensorAsync::CalcLabel(const Context<double>& context, ImageLabel16I* output) const {
    DRAKE_DEMAND(color_camera_.has_value());
    CopyImage(get_state(context).output.label.get(), output);
}

void RgbdSensorAsync::CalcDepth32F(const Context<double>& context, ImageDepth32F* output) const {
    DRAKE_DEMAND(depth_camera_.has_value());
    CopyImage(get_state(context).output.depth.get(), output);
}

void RgbdSensorAsync::CalcDepth16U(const Context<double>& context, ImageDepth16U* output) const {
    DRAKE_DEMAND(depth_camera_.has_value());
    CopyImage(get_state(context).output.depth.get(), output);
}

void RgbdSensorAsync::CalcX_WB(const Context<double>& context, RigidTransformd* output) const {
    *output = get_state(context).output.X_WB;
}

void RgbdSensorAsync::CalcImageTime(const Context<double>& context, BasicVector<double>* output) const {
...
void Worker::Start(double context_time, const QueryObject<double>& query) {
    ...
    for (bool first = true; const auto& source_id : inspector.GetAllSourceIds()) {
        if (first) {
            ...
            DRAKE_DEMAND(inspector.BelongsToSource(inspector.world_frame_id(), source_id));
            continue;
        }
    ...
    auto task = [this, context_time, poses = std::move(poses)]() -> RenderedImages {
        ...
        if (color_) {
            result.color = std::make_shared<const ImageRgba8U>(
                    sensor_->GetOutputPort("color_image").template Eval<ImageRgba8U>(*sensor_context_));
        }
        if (depth_) {
            result.depth = std::make_shared<const ImageDepth32F>(
                    sensor_->GetOutputPort("depth_image_32f").template Eval<ImageDepth32F>(*sensor_context_));
        }
        if (label_) {
            result.label = std::make_shared<const ImageLabel16I>(
                    sensor_->GetOutputPort("label_image").template Eval<ImageLabel16I>(*sensor_context_));
        }
        result.X_WB = sensor_->GetOutputPort("body_pose_in_world").template Eval<RigidTransformd>(*sensor_context_);
