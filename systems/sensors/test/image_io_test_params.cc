...
void ImageIoTestParams::CompareImageBuffer(const void* original, const void* readback) const {
    ...
    if (format == ImageFileFormat::kJpeg) {
        ...
        const Eigen::Array<int, 1, Eigen::Dynamic> increments = tail.cast<int>().array() - head.cast<int>().array();
        const int max_increment = 2;
        EXPECT_LE(increments.abs().maxCoeff(), max_increment) << fmt::to_string(fmt_eigen(increments.matrix()));
        return;
    }
