...
const TargetSystem* GetSystem(const DiagramBuilder<double>& builder, const std::string& name) {
...
std::pair<ColorRenderCamera, DepthRenderCamera> MakeCameras() {
    const RenderCameraCore core("dummy_renderer", CameraInfo(320, 240, 0.75), ClippingRange{10, 20}, {});
    return {ColorRenderCamera(core, true), DepthRenderCamera(core, DepthRange{11.5, 13.5})};
}

class SimRgbdSensorTest : public ::testing::Test {
protected:
    void SetUp() override {
        std::tie(plant_, scene_graph_) = AddMultibodyPlantSceneGraph(&builder_, 1e-2);
        bodyA_ = &plant_->AddRigidBody("bodyA");
        bodyB_ = &plant_->AddRigidBody("bodyB");

        X_AF_ = RigidTransformd(Vector3d(-0.5, 0.5, 0.75));
        frame_F_ = &plant_->AddFrame(std::make_unique<FixedOffsetFrame<double>>("test_frame", *bodyA_, X_AF_));
    ...
    SimRgbdSensor MakeSensorSpec(const Frame<double>* frame = nullptr) {
        ...
        return SimRgbdSensor("test1", frame ? *frame : bodyA_->body_frame(), 32.0, X_AB, color, depth);
    }
    ...
    const std::pair<SimRgbdSensor, const RgbdSensor*> MakeSensorOrThrow(const Frame<double>* frame = nullptr) {
        ...
        const auto* rgbd = GetSystem<RgbdSensor>(builder_, "rgbd_sensor_" + sensor.serial());
        if (rgbd == nullptr) {
    ...
    void AssertPublishedPorts(bool rgb, bool depth, bool label) {
        ...
        AddSimRgbdSensorLcmPublisher(sensor, rgb ? &rgbd->color_image_output_port() : nullptr,
                                     depth ? &rgbd->depth_image_16U_output_port() : nullptr,
                                     label ? &rgbd->label_image_output_port() : nullptr, false, &builder_, &lcm_);
        EXPECT_LT(old_system_count, builder_.GetSystems().size());

        const auto* images = GetSystem<ImageToLcmImageArrayT>(builder_, "image_to_lcm_" + sensor.serial());
...
TEST_F(SimRgbdSensorTest, SimRgbdSensorValues) {
    ...
    const RenderCameraCore core("dummy_renderer", CameraInfo(320, 240, 0.75), ClippingRange{10, 20}, {});
    {
        ...
        EXPECT_TRUE(drake::CompareMatrices(X_PB.GetAsMatrix34(), sensor.X_PB().GetAsMatrix34()));
...
TEST_F(SimRgbdSensorTest, AddSensorWithBodyFrameSpecification) {
    ...
    const auto& X_WB = rgbd->body_pose_in_world_output_port().Eval<RigidTransformd>(rgbd_context);

    const RigidTransformd& X_AB = sensor.X_PB();
    EXPECT_TRUE(drake::CompareMatrices(X_WB.GetAsMatrix34(), (X_WA * X_AB).GetAsMatrix34()));
}
...
TEST_F(SimRgbdSensorTest, AddSensorWithBodyOffsetFrameSpecification) {
    ...
    EXPECT_TRUE(drake::CompareMatrices(X_WB.GetAsMatrix34(), (X_WA * X_AF_ * X_FB).GetAsMatrix34()));
}
...
TEST_F(SimRgbdSensorTest, AddSensorForRgbdSensorConfiguration) {
    ...
    // got copied.
    EXPECT_EQ(rgbd->color_render_camera().core().renderer_name(), sensor.color_properties().core().renderer_name());
    EXPECT_EQ(rgbd->depth_render_camera().core().renderer_name(), sensor.depth_properties().core().renderer_name());
}
...
TEST_F(SimRgbdSensorTest, AddPublisherNoPortIsNoOp) {
    ...
    AddSimRgbdSensorLcmPublisher(sensor, nullptr, nullptr, nullptr, false, &builder_, &lcm_);
...
TEST_F(SimRgbdSensorTest, AddPublisherTestProperties) {
    ...
    const size_t old_system_count = builder_.GetSystems().size();
    AddSimRgbdSensorLcmPublisher(sensor, &rgbd->color_image_output_port(), &rgbd->depth_image_16U_output_port(),
                                 &rgbd->label_image_output_port(), false, &builder_, &lcm_);
    EXPECT_LT(old_system_count, builder_.GetSystems().size());

    const auto* publisher = GetSystem<LcmPublisherSystem>(
            builder_, fmt::format("LcmPublisherSystem(DRAKE_RGBD_CAMERA_IMAGES_{})", sensor.serial()));
    ASSERT_NE(publisher, nullptr);
    EXPECT_DOUBLE_EQ(publisher->get_publish_period(), 1.0 / sensor.rate_hz());
    EXPECT_THAT(publisher->get_channel_name(), ::testing::HasSubstr(sensor.serial()));
}
