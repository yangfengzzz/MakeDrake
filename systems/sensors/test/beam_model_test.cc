#include "systems/sensors/beam_model.h"
...
#include "systems/sensors/beam_model_params.h"
...
GTEST_TEST(BeamModelTest, TestInputPorts) {
    ...
    EXPECT_EQ(model.get_event_random_input_port().get_random_type().value(), RandomDistribution::kUniform);

    EXPECT_EQ(model.get_hit_random_input_port().size(), kNumReadings);
    EXPECT_EQ(model.get_hit_random_input_port().get_random_type().value(), RandomDistribution::kGaussian);

    EXPECT_EQ(model.get_short_random_input_port().size(), kNumReadings);
    EXPECT_EQ(model.get_short_random_input_port().get_random_type().value(), RandomDistribution::kExponential);

    EXPECT_EQ(model.get_uniform_random_input_port().size(), kNumReadings);
    EXPECT_EQ(model.get_uniform_random_input_port().get_random_type().value(), RandomDistribution::kUniform);
}
...
GTEST_TEST(BeamModelTest, TestProbabilityDensity) {
    ...
    auto constant_depth = builder.AddSystem<ConstantVectorSource>(Vector1d(kDepthInput));
    builder.Connect(constant_depth->get_output_port(), beam_model->get_depth_input_port());

    auto w_event = builder.AddSystem<RandomSource<double>>(RandomDistribution::kUniform, 1, 0.0025);
    builder.Connect(w_event->get_output_port(0), beam_model->get_event_random_input_port());

    auto w_hit = builder.AddSystem<RandomSource<double>>(RandomDistribution::kGaussian, 1, 0.0025);
    builder.Connect(w_hit->get_output_port(0), beam_model->get_hit_random_input_port());

    auto w_short = builder.AddSystem<RandomSource<double>>(RandomDistribution::kExponential, 1, 0.0025);
    builder.Connect(w_short->get_output_port(0), beam_model->get_short_random_input_port());

    auto w_uniform = builder.AddSystem<RandomSource<double>>(RandomDistribution::kUniform, 1, 0.0025);
    builder.Connect(w_uniform->get_output_port(0), beam_model->get_uniform_random_input_port());
    ...
    for (int i = 0; i < simulator.get_context().num_discrete_state_groups(); i++) {
    ...
    auto& params = beam_model->get_mutable_parameters(
            &diagram->GetMutableSubsystemContext(*beam_model, &simulator.get_mutable_context()));
    ...
    double probability_hit =
            1.0 - params.probability_uniform() - params.probability_miss() - params.probability_short();
    ...
    auto probability_density_function = [&](double z) {
        DRAKE_DEMAND(z >= 0.0 && z < kMaxRange);  // Doesn't capture the delta
                                                  ...
                                                  // at kMaxRange.
        const double p_short = (z <= kDepthInput) ? params.lambda_short() * std::exp(-params.lambda_short() * z) : 0.0;

        const double sigma_sq = params.sigma_hit() * params.sigma_hit();
        return params.probability_uniform() / kMaxRange + params.probability_short() * p_short +
               probability_hit * std::exp(-0.5 * (z - kDepthInput) * (z - kDepthInput) / sigma_sq) /
    ...
    const Eigen::VectorXd depth = Eigen::VectorXd::LinSpaced(1000, 0.0, kMaxRange - 1e-6);
    ...
    for (double a = 0.0; a < kMaxRange; a += h) {
        ...
        const double count = (x.array() >= a && x.array() < a + h - 1e-8).template cast<double>().matrix().sum();
    ...
    const double p_max = params.probability_miss() +
                         probability_hit * Phi(-kDepthInput / params.sigma_hit())  // "hit" would have returned < 0.0.
                         + probability_hit * Phi((kDepthInput - kMaxRange) /
                                                 params.sigma_hit());  // "hit" would have returned > kMaxRange.
    EXPECT_NEAR((x.array() == kMaxRange).template cast<double>().matrix().sum() / N, p_max, 3e-3);
}
