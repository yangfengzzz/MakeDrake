#include "systems/sensors/camera_config.h"
...
#include "geometry/render_gl/render_engine_gl_params.h"
#include "geometry/render_gltf_client/render_engine_gltf_client_params.h"
#include "geometry/render_vtk/render_engine_vtk_params.h"
...
bool operator==(const RenderEngineVtkParams& p1, const RenderEngineVtkParams& p2) {
    return yaml::SaveYamlString(p1) == yaml::SaveYamlString(p2);
}

bool operator==(const RenderEngineGlParams& p1, const RenderEngineGlParams& p2) {
    return yaml::SaveYamlString(p1) == yaml::SaveYamlString(p2);
}

bool operator==(const RenderEngineGltfClientParams& p1, const RenderEngineGltfClientParams& p2) {
    return yaml::SaveYamlString(p1) == yaml::SaveYamlString(p2);
}
...
GTEST_TEST(CameraConfigTest, CameraConfigFocalLength) {
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(null_focal.focal_x(), ".*you must define .* for FocalLength.");
        DRAKE_EXPECT_THROWS_MESSAGE(null_focal.focal_y(), ".*you must define .* for FocalLength.");
    }
}
...
GTEST_TEST(CameraConfigTest, CameraConfigFovDegrees) {
    ...
    constexpr double kFocalX = 250;
    constexpr double kFocalY = 250;
    const CameraInfo intrinsics(kWidth, kHeight, kFocalX, kFocalY, kWidth * 0.5, kHeight * 0.5);
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(null_fov.focal_x(kWidth, kHeight), ".*you must define .* for FovDegrees.");
        DRAKE_EXPECT_THROWS_MESSAGE(null_fov.focal_y(kWidth, kHeight), ".*you must define .* for FovDegrees.");
    }
}
...
GTEST_TEST(CameraConfigTest, FocalLength) {
...

    {
        ...
        const CameraInfo intrinsics(c.width, c.height, target_focal_x, target_focal_y, 0.5 * c.width, 0.5 * c.height);
        const double fov_x_rad = intrinsics.fov_x();
        const double fov_y_rad = intrinsics.fov_y();

        c.focal = CameraConfig::FovDegrees{fov_x_rad * 180 / M_PI, fov_y_rad * 180 / M_PI};
...
GTEST_TEST(CameraConfigTest, SerializeBackgroundRgba) {
    ...
    const std::string yaml = SaveYamlString<CameraConfig>(config, {}, CameraConfig());
...
GTEST_TEST(CameraConfigTest, DeserializingRendererClass) {
    ...
    EXPECT_THAT(config_1.renderer_class, testing::VariantWith<std::string>("RenderEngineVtk"));

    const CameraConfig config_2 = parse("renderer_class: !RenderEngineVtkParams {}");
    EXPECT_THAT(config_2.renderer_class, testing::VariantWith<RenderEngineVtkParams>(RenderEngineVtkParams{}));
   ...
   renderer_class: !RenderEngineVtkParams
    ...
    EXPECT_THAT(config_3.renderer_class,
                testing::VariantWith<RenderEngineVtkParams>(RenderEngineVtkParams{.default_clear_color = {0, 0, 0}}));
   ...
   renderer_class: !RenderEngineGlParams
    ...
    EXPECT_THAT(config_4.renderer_class,
                testing::VariantWith<RenderEngineGlParams>(RenderEngineGlParams{.default_clear_color = Rgba(0, 0, 0)}));
   ...
   renderer_class: !RenderEngineGltfClientParams
     ...
     cleanup: false)""");
    EXPECT_THAT(
            config_5.renderer_class,
            testing::VariantWith<RenderEngineGltfClientParams>(RenderEngineGltfClientParams{
                    .base_url = "http://10.10.10.1", .render_endpoint = "server", .verbose = true, .cleanup = false}));
...
void CoreIsValid(const RenderCameraCore& core, const CameraConfig& config, const RigidTransformd& X_BS) {
...
GTEST_TEST(CameraConfigTest, Validation) {
...

    {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(config.ValidateOrThrow(), ".*X_BD must not specify a base frame. 'error' found.");
    }

    {
        CameraConfig config;
        config.X_BC.base_frame = "error";
        DRAKE_EXPECT_THROWS_MESSAGE(config.ValidateOrThrow(), ".*X_BC must not specify a base frame. 'error' found.");
    }

    // We require a non-empty name.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.name = ""}.ValidateOrThrow(), ".* name cannot be empty.");
    // We require a non-empty renderer-name.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.renderer_name = ""}.ValidateOrThrow(),
                                ".*renderer_name cannot be empty.");

    // Good renderer_class strings don't throw (we already know that default
    // doesn't throw).
    EXPECT_NO_THROW(CameraConfig{.renderer_class = "RenderEngineVtk"}.ValidateOrThrow());
    EXPECT_NO_THROW(CameraConfig{.renderer_class = "RenderEngineGl"}.ValidateOrThrow());
    // Bad renderer_class strings throw -- proof the field is being validated.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.renderer_class = "BadName"}.ValidateOrThrow(),
                                ".*the given renderer_class value.*");

    // Non-positive, non-finite values all throw.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.fps = 0}.ValidateOrThrow(), ".*FPS.*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.fps = -11}.ValidateOrThrow(), ".*FPS.*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.fps = std::numeric_limits<double>::infinity()}.ValidateOrThrow(),
                                ".*FPS.*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraConfig{.fps = std::numeric_limits<double>::quiet_NaN()}.ValidateOrThrow(),
                                ".*FPS.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(SaveYamlString(CameraConfig{.name = ""}), ".*name cannot be empty.");
    // An invalid sub-struct should throw when serializing.
    DRAKE_EXPECT_THROWS_MESSAGE(SaveYamlString(CameraConfig{.focal = CameraConfig::FovDegrees{}}),
