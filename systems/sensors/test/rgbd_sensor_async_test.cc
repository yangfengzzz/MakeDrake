...
public:
    ...
    static constexpr double kClearDepth32 = ImageTraits<PixelType::kDepth32F>::kTooFar;
    static constexpr uint16_t kClearDepth16 = ImageTraits<PixelType::kDepth16U>::kTooFar;
    static const RenderLabel& kClearLabel;

    SimpleRenderEngine() { this->set_force_accept(true); }

private:
    std::unique_ptr<RenderEngine> DoClone() const final { return std::make_unique<SimpleRenderEngine>(*this); }

    void DoRenderColorImage(const ColorRenderCamera& camera, ImageRgba8U* output) const final {
        DummyRenderEngine::DoRenderColorImage(camera, output);
        *output = ImageRgba8U(camera.core().intrinsics().width(), camera.core().intrinsics().height(), kClearColor);
    }

    void DoRenderDepthImage(const DepthRenderCamera& camera, ImageDepth32F* output) const final {
        DummyRenderEngine::DoRenderDepthImage(camera, output);
        *output = ImageDepth32F(camera.core().intrinsics().width(), camera.core().intrinsics().height(), kClearDepth32);
    }

    void DoRenderLabelImage(const ColorRenderCamera& camera, ImageLabel16I* output) const final {
        DummyRenderEngine::DoRenderLabelImage(camera, output);
        *output = ImageLabel16I(camera.core().intrinsics().width(), camera.core().intrinsics().height(), kClearLabel);
    }
};
...
public:
    RgbdSensorAsyncTest()
        : ::testing::Test(),
          ...
          // depth.
          color_camera_({kRendererName, {64, 48, M_PI / 4}, {0.1, 10.0}, {}}, false),
          depth_camera_({kRendererName, {32, 24, M_PI / 6}, {0.1, 10.0}, {}}, {0.1, 10}) {}
    ...
    void ExpectDefaultImages(const System<double>& system, const Context<double>& context) {
        SCOPED_TRACE(fmt::format("... at context.time = {}", context.get_time()));
        const auto& color_image = system.GetOutputPort("color_image").Eval<ImageRgba8U>(context);
        const auto& label_image = system.GetOutputPort("label_image").Eval<ImageLabel16I>(context);
        const auto& depth32_image = system.GetOutputPort("depth_image_32f").Eval<ImageDepth32F>(context);
        const auto& depth16_image = system.GetOutputPort("depth_image_16u").Eval<ImageDepth16U>(context);
        const Eigen::VectorXd& image_time = system.GetOutputPort("image_time").Eval(context);
    ...
    void ExpectClearImages(const System<double>& system, const Context<double>& context, double expected_time) {
        SCOPED_TRACE(fmt::format("... at context.time = {}", context.get_time()));
        ...
        ASSERT_THAT(to_vector(color_image), ::testing::Each(SimpleRenderEngine::kClearColor));
        ASSERT_THAT(to_vector(label_image), ::testing::Each(SimpleRenderEngine::kClearLabel));
        ASSERT_THAT(to_vector(depth32_image), ::testing::Each(SimpleRenderEngine::kClearDepth32));
        ASSERT_THAT(to_vector(depth16_image), ::testing::Each(SimpleRenderEngine::kClearDepth16));
...
TEST_F(RgbdSensorAsyncTest, ConstructorAndSimpleAccessors) {
    ...
    const RgbdSensorAsync dut(&scene_graph, parent_id, X_PB, fps, capture_offset, output_delay, color_camera_,
                              depth_camera_, render_label_image);
    ...
    EXPECT_EQ(dut.body_pose_in_world_output_port().get_name(), "body_pose_in_world");
...
TEST_F(RgbdSensorAsyncTest, GeometryVersionFailFast) {
    ...
    plant.Finalize();
    scene_graph.AddRenderer(kRendererName, std::make_unique<SimpleRenderEngine>());
    const FrameId world = SceneGraph<double>::world_frame_id();
    const RigidTransform<double> X;
    const double fps = 4;
    const double capture_offset = 0.001;
    const double output_delay = 0.200;
    const auto* dut = builder.AddSystem<RgbdSensorAsync>(&scene_graph, world, X, fps, capture_offset, output_delay,
                                                         color_camera_, std::nullopt);
    ...
    Context<double>* scene_graph_context = &scene_graph.GetMyMutableContextFromRoot(&simulator.get_mutable_context());
    const GeometryId geometry_id =
            scene_graph.RegisterGeometry(scene_graph_context, plant.get_source_id().value(), world,
                                         std::make_unique<GeometryInstance>(X, Sphere(0.1), "sphere"));
    PerceptionProperties properties;
    properties.UpdateProperty("label", "id", RenderLabel(1));
    scene_graph.AssignRole(scene_graph_context, plant.get_source_id().value(), geometry_id, properties);

    // Simulating again is an error (for now). Ideally, we'd adapt to the new
    // geometry but for now at least we fail-fast.
    DRAKE_EXPECT_THROWS_MESSAGE(simulator.AdvanceTo(0.002), ".*must manually call.*Initialize.*");
}
...
TEST_F(RgbdSensorAsyncTest, ConstructorColorOnly) {
    ...
    const RgbdSensorAsync dut(&scene_graph, parent_id, X_PB, fps, capture_offset, output_delay, color_camera_,
                              std::nullopt, render_label_image);
...
TEST_F(RgbdSensorAsyncTest, ConstructorDepthOnly) {
    ...
    const double output_delay = 0.200;
    const bool render_label_image = false;
    const RgbdSensorAsync dut(&scene_graph, parent_id, X_PB, fps, capture_offset, output_delay, std::nullopt,
                              depth_camera_, render_label_image);
...
TEST_F(RgbdSensorAsyncTest, RenderBackgroundColor) {
    ...
    const bool render_label_image = true;
    const auto* dut =
            builder.AddSystem<RgbdSensorAsync>(&scene_graph, parent_id, X_PB, fps, capture_offset, output_delay,
    ...
    double next_time = simulator.get_system().CalcNextUpdateTime(simulator.get_context(), events.get());
    EXPECT_EQ(next_time, 1.501);
    const EventCollection<UnrestrictedUpdateEvent<double>>& update_events = events->get_unrestricted_update_events();
    EXPECT_TRUE(update_events.HasEvents());
    auto next_state_out = simulator.get_context().CloneState();
    EXPECT_TRUE(simulator.get_system()
                        .CalcUnrestrictedUpdate(simulator.get_context(), update_events, next_state_out.get())
                        .succeeded());
}
