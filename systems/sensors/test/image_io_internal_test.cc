...
GTEST_TEST(ImageIoInternalTest, GuessFileFormatFromFile) {
    const fs::path jpg_file = FindResourceOrThrow("drake/systems/sensors/test/jpeg_test.jpg");
    const fs::path png_file = FindResourceOrThrow("drake/systems/sensors/test/png_color_test.png");
    const fs::path tif_file = FindResourceOrThrow("drake/systems/sensors/test/tiff_32f_test.tif");
...
GTEST_TEST(ImageIoInternalTest, GetDrakeScalarType) {
    ...
    // actually `constexpr` with a spot-check. We rely on code review to ensure
    // the switch cases are correct.
    constexpr std::optional<PixelScalar> result = GetDrakeScalarType(VTK_TYPE_UINT8);
...
GTEST_TEST(ImageIoInternalTest, CopyAndFlipRaw) {
  ...
  const std::array<uint8_t, source_size> source = {
    ...
    CopyAndFlipRaw<uint8_t, source_channels, /* swizzle = */ false, dest_channels, dest_pad>(source.data(), dest.data(),
                                                                                             width, height);
    // clang-format off
  EXPECT_THAT(dest, testing::ElementsAre(
    ...
    CopyAndFlipRaw<uint8_t, source_channels, /* swizzle = */ true, dest_channels, dest_pad>(source.data(), dest.data(),
                                                                                            width, height);
    // clang-format off
  EXPECT_THAT(dest, testing::ElementsAre(
    ...
    CopyAndFlipRaw<uint8_t, /* source_channels = */ dest_channels,
                   /* swizzle = */ true, /* dest_channels = */ source_channels>(dest.data(), source_again.data(), width,
                                                                                height);
    // clang-format off
