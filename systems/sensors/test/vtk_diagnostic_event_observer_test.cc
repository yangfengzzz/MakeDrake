...
GTEST_TEST(VtkDiagnosticEventObserverTest, General) {
    ...
    const char* const warning_message =
            "text for warning\n"
            "next line\n\n";
    dummy->InvokeEvent(vtkCommand::ErrorEvent, const_cast<void*>(static_cast<const void*>(error_message)));
    dummy->InvokeEvent(vtkCommand::WarningEvent, const_cast<void*>(static_cast<const void*>(warning_message)));

    // Check for what came out.
    EXPECT_THAT(messages,
                testing::ElementsAre("error: text for error: next line", "warning: text for warning: next line"));
}
