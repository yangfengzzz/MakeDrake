...
protected:
    void SetUp() override {
        ...
        const math::RotationMatrix<double> R_BS(drake::math::RollPitchYaw<double>(M_PI / 2, 0, 0));
        const math::RigidTransform<double> X_BS(R_BS, Eigen::Vector3d::Zero());

        gyroscope_ = &Gyroscope<double>::AddToDiagram(arm_body, X_BS, *plant_, &builder);
...
TEST_F(GyroscopeTest, Rotated) {
    ...
    auto& gyro_context = diagram_->GetMutableSubsystemContext(*gyroscope_, diagram_context.get());
    ...
    const auto& result = gyroscope_->get_measurement_output_port().Eval<BasicVector<double>>(gyro_context);
