#include "systems/sensors/accelerometer.h"
...
protected:
    void SetUp() override {
        ...
        const std::string urdf_name = FindResourceOrThrow("drake/examples/pendulum/Pendulum.urdf");
        ...
        builder.Connect(plant_->get_body_poses_output_port(), accel_default_->get_body_poses_input_port());
        ...
        const math::RigidTransform<double> X_BS_rotated(math::RotationMatrix<double>::MakeYRotation(M_PI / 2),
                                                        Eigen::Vector3d(0, 0, -r_BS_));
        accel_rotated_ = &Accelerometer<double>::AddToDiagram(arm_body, X_BS_rotated, gravity_, *plant_, &builder);
...
TEST_F(AccelerometerTest, DefaultRotation) {
    ...
    auto& plant_context = diagram_->GetMutableSubsystemContext(*plant_, diagram_context.get());
    auto& accel_context = diagram_->GetMutableSubsystemContext(*accel_default_, diagram_context.get());
    ...
    const auto& result = accel_default_->get_measurement_output_port().Eval<BasicVector<double>>(accel_context);

    // Compute expected result:
    // g/L sin(theta)
    double angular_acceleration = -gravity_.norm() / .5 * sin(angle);
    Eigen::Vector3d expected_result(-angular_acceleration * r_BS_, 0, 0);
    Eigen::Vector3d g_S(cos(angle) * gravity_(0) - sin(angle) * gravity_(2), gravity_(1),
    ...
    double m = 1;
    double b = .1;
    angular_acceleration = -g / L * sin(angle) - b * angular_velocity / (m * L * L);

    plant_->SetVelocities(&plant_context, Vector1d(angular_velocity));
    const auto& result_with_velocity = accel_default_->get_output_port(0).Eval<BasicVector<double>>(accel_context);
    Eigen::Vector3d expected_result_with_velocity(-angular_acceleration * r_BS_, 0,
                                                  angular_velocity * angular_velocity * r_BS_);
    expected_result_with_velocity -= g_S;
    EXPECT_TRUE(CompareMatrices(result_with_velocity.get_value(), expected_result_with_velocity, tol));
}
...
TEST_F(AccelerometerTest, Rotated) {
    ...
    auto diagram_context = diagram_->CreateDefaultContext();
    auto& plant_context = diagram_->GetMutableSubsystemContext(*plant_, diagram_context.get());
    auto& accel_context = diagram_->GetMutableSubsystemContext(*accel_rotated_, diagram_context.get());
    ...
    const auto& result = accel_rotated_->get_output_port(0).Eval<BasicVector<double>>(accel_context);

    // Compute expected result:
    // g/L sin(theta)
    double angular_acceleration = -gravity_.norm() / .5 * sin(angle);
    Eigen::Vector3d expected_result(0, 0, -angular_acceleration * r_BS_);
    Eigen::Vector3d g_S(-cos(angle) * gravity_(2) - sin(angle) * gravity_(0), gravity_(1),
    ...
    plant_->SetVelocities(&plant_context, Vector1d(angular_velocity));
    const auto& result_with_velocity = accel_rotated_->get_output_port(0).Eval<BasicVector<double>>(accel_context);
    Eigen::Vector3d expected_result_with_velocity(-angular_velocity * angular_velocity * r_BS_, 0,
