...
GTEST_TEST(TestCameraInfo, ConstructionTest) {
    const Matrix3d expected((Matrix3d() << kFx, 0., kCx, 0., kFy, kCy, 0., 0., 1.).finished());
...
GTEST_TEST(TestCameraInfo, ConstructionWithFovTest) {
    const Matrix3d expected((Matrix3d() << kFy, 0., kCx, 0., kFy, kCy, 0., 0., 1.).finished());
...
GTEST_TEST(TestCameraInfo, BadConstructionThrows) {
    ...
    // Bad image size.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(-13, kH, kFx, kFy, kCx, kCy), "[^]*Width.+-13[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, -17, kFx, kFy, kCx, kCy), "[^]*Height.+-17[^]*");
    // Bad focal length.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, 0, kFy, kCx, kCy), "[^]*Focal X.+0[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, -10, kFy, kCx, kCy), "[^]*Focal X.+-10[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kInf, kFy, kCx, kCy), "[^]*Focal X.+inf[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kNaN, kFy, kCx, kCy), "[^]*Focal X.+nan[^]*");

    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, 0, kCx, kCy), "[^]*Focal Y.+0[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, -10, kCx, kCy), "[^]*Focal Y.+-10[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kInf, kCx, kCy), "[^]*Focal Y.+inf[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kNaN, kCx, kCy), "[^]*Focal Y.+nan[^]*");

    // Bad principal point.
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, 0, kCy), "[^]*Center X.+0[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, -10, kCy), "[^]*Center X.+-10[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kW, kCy), "[^]*Center X.+\\d+[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kW + 1, kCy), "[^]*Center X.+\\d+[^]*");

    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kCx, 0), "[^]*Center Y.+0[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kCx, -10), "[^]*Center Y.+-10[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kCx, kH), "[^]*Center Y.+\\d+[^]*");
    DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, kFx, kFy, kCx, kH + 1), "[^]*Center Y.+\\d+[^]*");
    ...
    const Matrix3d K_valid((Matrix3d() << kFy, 0., kCx, 0., kFy, kCy, 0., 0., 1.).finished());
    EXPECT_NO_THROW(CameraInfo(kW, kH, K_valid));

    const std::vector<std::pair<int, int>> indices{{0, 1}, {1, 0}, {2, 0}, {2, 1}, {2, 2}};
    for (auto [i, j] : indices) {
        Matrix3d K = K_valid;
        K(i, j) = -1;
        DRAKE_EXPECT_THROWS_MESSAGE(CameraInfo(kW, kH, K), "[^]*intrinsic matrix is malformed[^]*");
    }
}
