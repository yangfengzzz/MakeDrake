...
struct ImageIoTestParams {
    ...
    int vtk_scalar() const {
        return force_16u ? VTK_TYPE_UINT16 : (format == ImageFileFormat::kTiff) ? VTK_TYPE_FLOAT32 : VTK_TYPE_UINT8;
    }
    ...
    int total_storage() const { return width() * height() * depth() * channels(); }
    ...
    friend std::ostream& operator<<(std::ostream& os, const ImageIoTestParams& self) {
        os << fmt::format("[format = {}; write_to_file = {}; force_16u = {}]", self.format, self.write_to_file,
                          self.force_16u);
        return os;
    }
};
...
inline auto GetAllImageIoTestParams() {
    ...
    return testing::Values(
            ...
            Params{.format = kJpeg, .write_to_file = true}, Params{.format = kJpeg, .write_to_file = false},
            // VTK PNG supports grayscale, an alpha channel, and 16-bit channels.
            Params{.format = kPng, .write_to_file = true}, Params{.format = kPng, .write_to_file = false},
