...
GTEST_TEST(TestEncoders, QuantizeOnly) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        EXPECT_TRUE(
                CompareMatrices(desired_measurement, measurement->CopyToVector(), tol, MatrixCompareType::absolute));
    }
}
...
GTEST_TEST(TestEncoders, QuantizationAndSelector) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        using std::floor;
        for (int j = 0; j < 2; j++) {
            desired_measurement(j) = floor(angle(indices[j]) * ticks_per_radian(j)) / ticks_per_radian(j);
        }
...
GTEST_TEST(TestEncoders, ScalarConversion) {
    ...
    EXPECT_TRUE(is_symbolic_convertible(encoders, [&](const systems::sensors::RotaryEncoders<Expression>& dut) {
        auto context = dut.CreateDefaultContext();

        // Set input to be symbolic variables.
        const Vector4<Expression> input{symbolic::Variable("u0"), symbolic::Variable("u1"), symbolic::Variable("u2"),
        ...
        const systems::BasicVector<Expression>& output = *(outputs->get_vector_data(0));
