...
GTEST_TEST(RgbdSensorDiscrete, Construction) {
    const DepthRenderCamera depth_camera({"render", {640, 480, M_PI / 4}, {0.1, 10.0}, {}}, {0.1, 10});
    ...
    RgbdSensorDiscrete sensor(
            make_unique<RgbdSensor>(SceneGraph<double>::world_frame_id(), RigidTransformd::Identity(), depth_camera),
...
GTEST_TEST(RgbdSensorDiscrete, ImageHold) {
    ...
    const double kPeriod = 0.1;
    const bool include_render_port = true;
    RgbdSensorDiscrete discrete_sensor(std::move(sensor), kPeriod, include_render_port);
    ...
    auto confirm_hold = [&sensor_raw, &sub_systems, &kPeriod, &discrete_sensor](int port_index) {
        const auto* zoh = dynamic_cast<const ZeroOrderHold<double>*>(sub_systems.at(1 + port_index));
        ASSERT_NE(zoh, nullptr);
        EXPECT_EQ(zoh->period(), kPeriod);
        EXPECT_TRUE(discrete_sensor.AreConnected(sensor_raw->get_output_port(port_index), zoh->get_input_port()));
    };
