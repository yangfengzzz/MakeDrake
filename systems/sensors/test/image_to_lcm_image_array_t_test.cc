...
lcmt_image_array SetUpInputAndOutput(ImageToLcmImageArrayT* dut,
    ...
    const InputPort<double>& color_image_input_port = dut->color_image_input_port();
    const InputPort<double>& depth_image_input_port = dut->depth_image_input_port();
    const InputPort<double>& label_image_input_port = dut->label_image_input_port();
...
GTEST_TEST(ImageToLcmImageArrayT, ValidTest) {
    ...
    auto Verify = [color_image, depth_image, label_image](const ImageToLcmImageArrayT& dut,
        ...
        for (auto const& image : output_image_array.images) {
            ...
            if (image.pixel_format == lcmt_image::PIXEL_FORMAT_RGBA) {
                ...
                row_stride = color_image.width() * color_image.kNumChannels * sizeof(*color_image.at(0, 0));
                ...
                row_stride = depth_image.width() * depth_image.kNumChannels * sizeof(*depth_image.at(0, 0));
                ...
                row_stride = label_image.width() * label_image.kNumChannels * sizeof(*label_image.at(0, 0));
    ...
    ImageToLcmImageArrayT dut_compressed(kColorFrameName, kDepthFrameName, kLabelFrameName, true);
    auto image_array_t_compressed = SetUpInputAndOutput(&dut_compressed, color_image, depth_image, label_image);
    Verify(dut_compressed, image_array_t_compressed, lcmt_image::COMPRESSION_METHOD_ZLIB);

    ImageToLcmImageArrayT dut_uncompressed(kColorFrameName, kDepthFrameName, kLabelFrameName, false);
    auto image_array_t_uncompressed = SetUpInputAndOutput(&dut_uncompressed, color_image, depth_image, label_image);
    Verify(dut_uncompressed, image_array_t_uncompressed, lcmt_image::COMPRESSION_METHOD_NOT_COMPRESSED);
}
