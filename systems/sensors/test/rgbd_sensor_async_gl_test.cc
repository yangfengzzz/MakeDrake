...
#include "common/trajectories/piecewise_pose.h"
...
#include "systems/sensors/rgbd_sensor_discrete.h"
...
public:
    ...
    explicit AbstractLogSink(const AbstractValue& model_value) {
        ...
        this->DeclarePeriodicPublishEvent(period, offset, &AbstractLogSink<T>::WriteToLog);
    }

    void WriteToLog(const Context<T>& context) const {
        const AbstractValue& value = this->get_input_port().template Eval<AbstractValue>(context);
        values_.emplace_back(value.Clone());
    }

    const std::vector<copyable_unique_ptr<AbstractValue>>& values() const { return values_; }
...
public:
    FramePoseTrajectorySource(FrameId frame_id, const PiecewisePose<double>& traj) : frame_id_(frame_id), traj_(traj) {
        this->DeclareAbstractOutputPort("frame_pose_vector", &FramePoseTrajectorySource::CalcOutput);
    }

private:
    void CalcOutput(const Context<double>& context, FramePoseVector<double>* output) const {
...
GTEST_TEST(RgbdSensorAsyncGlTest, CompareAsyncToDiscrete) {
    ...
    const RigidTransform<double> X_WC_start(RollPitchYaw<double>{0.0, 0.0, 0.0}, Vector3d{-1.5, 0.0, 0.3});
    const RigidTransform<double> X_WC_end(RollPitchYaw<double>{0.0, M_PI_2, 0.0}, Vector3d{0.0, 0.0, 1.5});
    const PiecewisePose<double> camera_traj =
            PiecewisePose<double>::MakeLinear({start_time, end_time}, {X_WC_start, X_WC_end});
    const SourceId camera_kinematics_source = scene_graph.RegisterSource("camera_kinematics");
    const FrameId camera_kinematics_frame =
            scene_graph.RegisterFrame(camera_kinematics_source, GeometryFrame{"camera_frame"});
    const auto* camera_kinematics =
            builder.AddNamedSystem<FramePoseTrajectorySource>("camera_pose", camera_kinematics_frame, camera_traj);
    builder.Connect(camera_kinematics->get_output_port(), scene_graph.get_source_pose_port(camera_kinematics_source));
    ...
    const RigidTransform<double> X_PB(RollPitchYaw<double>{-M_PI_2, 0, -M_PI_2}, Vector3d{0.001, 0.002, 0.003});
    const ColorRenderCamera color_camera({kRendererName, {320, 240, M_PI / 4}, {0.1, 10.0}, {}});
    const DepthRenderCamera depth_camera({kRendererName, {160, 120, M_PI / 6}, {0.1, 10.0}, {}}, {0.1, 10});
    ...
    const auto* sensor_async =
            builder.AddSystem<RgbdSensorAsync>(&scene_graph, parent_id, X_PB, fps, async_capture_offset,
                                               async_output_delay, color_camera, depth_camera, render_label_image);
    builder.Connect(scene_graph.get_query_output_port(), sensor_async->get_input_port());
    ...
    const auto* sensor_discrete = builder.AddSystem<RgbdSensorDiscrete>(
            std::make_unique<RgbdSensor>(parent_id, X_PB, color_camera, depth_camera), discrete_period,
            render_label_image);
    builder.Connect(scene_graph.get_query_output_port(), sensor_discrete->query_object_input_port());
    ...
    const std::vector<std::pair<std::string, PixelType>> ports{{"color_image", PixelType::kRgba8U},
    ...
    for (const auto& [port_name, pixel_type] : ports) {
        ...
        const auto* logger_async = builder.AddNamedSystem<AbstractLogSink<double>>(async_name, *image);
        const auto* logger_discrete = builder.AddNamedSystem<AbstractLogSink<double>>(discrete_name, *image);
        builder.Connect(sensor_async->GetOutputPort(port_name), logger_async->get_input_port());
        builder.Connect(sensor_discrete->GetOutputPort(port_name), logger_discrete->get_input_port());

        // Log the images to disk for offline inspection by humans.
        const auto& writer_port_async = image_writer->DeclareImageInputPort(pixel_type, async_name, file_name_format,
                                                                            1.0 / fps, image_writer_offset);
        const auto& writer_port_discrete = image_writer->DeclareImageInputPort(
                pixel_type, discrete_name, file_name_format, 1.0 / fps, image_writer_offset);
        builder.Connect(sensor_async->GetOutputPort(port_name), writer_port_async);
        builder.Connect(sensor_discrete->GetOutputPort(port_name), writer_port_discrete);
    }
    ...
    log()->info("Simulation rate was {:.3}x realtime", simulator.get_actual_realtime_rate());

    // Check the image outputs.
    auto image_eq = [](PixelType pixel_type, const AbstractValue& actual, const AbstractValue& expected) -> bool {
        switch (pixel_type) {
            case PixelType::kRgba8U:
    ...
    for (const auto& [port_name, pixel_type] : ports) {
        const std::vector<copyable_unique_ptr<AbstractValue>>& async_values =
                diagram->template GetDowncastSubsystemByName<AbstractLogSink>(fmt::format("{}_async", port_name))
                        .values();
        const std::vector<copyable_unique_ptr<AbstractValue>>& discrete_values =
                diagram->template GetDowncastSubsystemByName<AbstractLogSink>(fmt::format("{}_discrete", port_name))
                        .values();
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            SCOPED_TRACE(fmt::format("pixel_type = {}, i = {}", static_cast<int>(pixel_type), i));
            ...
            if (i > 0) {
                ASSERT_FALSE(image_eq(pixel_type, *discrete_values[i], *discrete_values[i - 1]));
            }
        }
    }
