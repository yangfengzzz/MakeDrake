#include "systems/sensors/rgbd_sensor.h"
...
template <typename T>
const DummyRenderEngine* GetDummyRenderEngine(const systems::Context<T>& context, const std::string& name) {
    // Technically brittle, but relatively safe assumption that GeometryState
    // is abstract Parameter value 0.
    auto& geo_state = context.get_parameters().template get_abstract_parameter<geometry::GeometryState<T>>(0);
    const DummyRenderEngine* engine = dynamic_cast<const DummyRenderEngine*>(geo_state.GetRenderEngineByName(name));
    DRAKE_DEMAND(engine != nullptr);
    return engine;
}

::testing::AssertionResult CompareCameraInfo(const CameraInfo& test, const CameraInfo& expected) {
    if (test.width() != expected.width() || test.height() != expected.height() ||
        test.focal_x() != expected.focal_x() || test.focal_y() != expected.focal_y() ||
        test.center_x() != expected.center_x() || test.center_y() != expected.center_y()) {
...
::testing::AssertionResult CompareClipping(const ClippingRange& test, const ClippingRange& expected) {
...
::testing::AssertionResult CompareDepthRange(const DepthRange& test, const DepthRange& expected) {
...
::testing::AssertionResult CompareCameraCore(const RenderCameraCore& test, const RenderCameraCore& expected) {
    ...
    if (test.renderer_name() != expected.renderer_name()) {
        return ::testing::AssertionFailure() << "Renderer name doesn't match.\n Expected " << expected.renderer_name()
...
::testing::AssertionResult Compare(const ColorRenderCamera& test, const ColorRenderCamera& expected) {
    if (test.show_window() != expected.show_window()) {
        return ::testing::AssertionFailure() << "'show_window' doesn't match.\n Expected " << expected.show_window()
                                             << "\n got " << test.show_window();
    }
    return CompareCameraCore(test.core(), expected.core());
}

::testing::AssertionResult Compare(const DepthRenderCamera& test, const DepthRenderCamera& expected) {
...
public:
    RgbdSensorTest()
        : ::testing::Test(),
          ...
          // of it.
          color_camera_({kRendererName, {640, 480, M_PI / 4}, {0.1, 10.0}, RigidTransformd{Vector3d{1, 0, 0}}}, false),
          depth_camera_({kRendererName, {320, 240, M_PI / 6}, {0.1, 10.0}, RigidTransformd{Vector3d{-1, 0, 0}}},
                        {0.1, 10}) {}
...
protected:
    ...
    void MakeCameraDiagram(std::function<unique_ptr<RgbdSensor>(SceneGraph<double>*)> make_sensor) {
        ASSERT_EQ(scene_graph_, nullptr) << "Only call MakeCameraDiagram() once per test";
        ...
        builder.Connect(scene_graph_->get_query_output_port(), sensor_->query_object_input_port());
        diagram_ = builder.Build();
        context_ = diagram_->CreateDefaultContext();
        context_->SetTime(22.2);  // An arbitrary non-zero value.
        context_->DisableCaching();
        scene_graph_context_ = &diagram_->GetMutableSubsystemContext(*scene_graph_, context_.get());
        sensor_context_ = &diagram_->GetMutableSubsystemContext(*sensor_, context_.get());
    ...
    ::testing::AssertionResult ValidateConstruction(FrameId parent_id,
                                                    const RigidTransformd& X_WC_expected,
                                                    std::function<void()> pre_render_callback = {}) const {
        if (sensor_->parent_frame_id() != parent_id) {
            return ::testing::AssertionFailure() << "The sensor's parent id (" << sensor_->parent_frame_id()
                                                 << ") does not match the expected id (" << parent_id << ")";
        }
        ::testing::AssertionResult result = ::testing::AssertionSuccess();
        result = CompareCameraInfo(sensor_->color_camera_info(), color_camera_.core().intrinsics());
        if (!result) return result;

        result = Compare(sensor_->color_render_camera(), color_camera_);
        if (!result) return result;

        result = CompareCameraInfo(sensor_->depth_camera_info(), depth_camera_.core().intrinsics());
        if (!result) return result;
        ...
        EXPECT_TRUE(CompareMatrices(render_engine_->last_updated_X_WC().GetAsMatrix4(), X_WC_expected.GetAsMatrix4(),
                                    1e-15));
...
TEST_F(RgbdSensorTest, PortNames) {
    RgbdSensor sensor(SceneGraph<double>::world_frame_id(), RigidTransformd::Identity(), depth_camera_);
    ...
    EXPECT_EQ(sensor.body_pose_in_world_output_port().get_name(), "body_pose_in_world");
...
TEST_F(RgbdSensorTest, ConstructAnchoredCamera) {
    ...
    auto make_sensor = [this, &X_WB](SceneGraph<double>*) {
        return make_unique<RgbdSensor>(SceneGraph<double>::world_frame_id(), X_WB, color_camera_, depth_camera_);
    };
    MakeCameraDiagram(make_sensor);

    const RigidTransformd& X_BC = sensor_->X_BC();
    const RigidTransformd X_WC_expected = X_WB * X_BC;
    EXPECT_TRUE(ValidateConstruction(scene_graph_->world_frame_id(), X_WC_expected));
    EXPECT_EQ(sensor_->image_time_output_port().Eval(*sensor_context_), Vector1d{22.2});
}
...
TEST_F(RgbdSensorTest, ConstructFrameFixedCamera) {
    ...
    const RigidTransformd X_PB(AngleAxisd(M_PI / 6, Vector3d(1, 1, 1)), Vector3d(1, 2, 3));
    const RigidTransformd X_WP(AngleAxisd(M_PI / 7, Vector3d(-1, 0, 1)), Vector3d(-2, -1, -3));
    const FramePoseVector<double> X_WPs{{frame.id(), X_WP}};

    // The sensor requires a frame to be registered in order to attach to the
    // frame.
    auto make_sensor = [this, &source_id, &frame, &X_PB](SceneGraph<double>* graph) {
        source_id = graph->RegisterSource("source");
        graph->RegisterFrame(source_id, frame);
        return make_unique<RgbdSensor>(frame.id(), X_PB, color_camera_, depth_camera_);
    };
    MakeCameraDiagram(make_sensor);
    ...
    auto pre_render_callback = [this, &X_WPs, source_id]() {
        scene_graph_->get_source_pose_port(source_id).FixValue(scene_graph_context_, X_WPs);
    };
    EXPECT_TRUE(ValidateConstruction(frame.id(), X_WC_expected, pre_render_callback));
}

TEST_F(RgbdSensorTest, ConstructCameraWithNonTrivialOffsets) {
    const RigidTransformd X_BC{math::RotationMatrixd::MakeFromOrthonormalRows(
    ...
    const ColorRenderCamera color_camera{{color_camera_.core().renderer_name(), color_camera_.core().intrinsics(),
                                          color_camera_.core().clipping(), X_BC},
                                         color_camera_.show_window()};
    const DepthRenderCamera depth_camera{{depth_camera_.core().renderer_name(), depth_camera_.core().intrinsics(),
                                          depth_camera_.core().clipping(), X_BD},
                                         depth_camera_.depth_range()};
    const RigidTransformd X_WB;
    const RgbdSensor sensor(scene_graph_->world_frame_id(), X_WB, color_camera, depth_camera);
    EXPECT_TRUE(CompareMatrices(sensor.X_BC().GetAsMatrix4(), X_BC.GetAsMatrix4()));
    EXPECT_TRUE(CompareMatrices(sensor.X_BD().GetAsMatrix4(), X_BD.GetAsMatrix4()));
}
...
TEST_F(RgbdSensorTest, ConstructCameraWithNonTrivialOffsetsDeprecated) {
    ...
    const ColorRenderCamera color_camera(
            {color_camera_.core().renderer_name(),
             {color_camera_.core().intrinsics().width(), color_camera_.core().intrinsics().height(),
    ...
    const DepthRenderCamera depth_camera(
            {depth_camera_.core().renderer_name(),
             {depth_camera_.core().intrinsics().width(), depth_camera_.core().intrinsics().height(),
