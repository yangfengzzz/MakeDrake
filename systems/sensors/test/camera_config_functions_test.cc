#include "systems/sensors/camera_config_functions.h"
...
#include "geometry/render_gl/factory.h"
#include "geometry/render_gltf_client/factory.h"
#include "geometry/render_vtk/factory.h"
#include "lcm/drake_lcm.h"
#include "systems/lcm/lcm_publisher_system.h"
#include "systems/sensors/image_to_lcm_image_array_t.h"
#include "systems/sensors/rgbd_sensor_async.h"
#include "systems/sensors/sim_rgbd_sensor.h"
...
GTEST_TEST(CameraConfigTest, NonDefault) {
    ...
    const std::string differential_data = SaveYamlString<CameraConfig>(MakeConfig(), {}, CameraConfig{});
...
protected:
    void SetUp() override {
        ...
        plant_->AddFrame(std::make_unique<FixedOffsetFrame<double>>("test_frame", body, RigidTransformd()));
    }
...
TEST_F(CameraConfigFunctionsTest, ParentBaseFrameDefaultToWorld) {
    ...
    const auto& sensor = builder_.GetDowncastSubsystemByName<RgbdSensor>("rgbd_sensor_preview_camera");
...
TEST_F(CameraConfigFunctionsTest, InvalidParentBaseFrame) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ApplyCameraConfig(config, &builder_), ".*invalid_frame.*");
}
...
TEST_F(CameraConfigFunctionsTest, RendererClassBasic) {
    ...
    const auto& sensor1 = builder_.GetDowncastSubsystemByName<RgbdSensor>("rgbd_sensor_preview_camera");
    EXPECT_EQ(sensor1.color_render_camera().core().renderer_name(), config.renderer_name);
    EXPECT_EQ(sensor1.depth_render_camera().core().renderer_name(), config.renderer_name);
    ...
    const auto& sensor2 = builder_.GetDowncastSubsystemByName<RgbdSensor>("rgbd_sensor_preview_camera_the_other_one");
    EXPECT_EQ(sensor2.color_render_camera().core().renderer_name(), config.renderer_name);
    EXPECT_EQ(sensor2.depth_render_camera().core().renderer_name(), config.renderer_name);
    ...
    // New RgbdSensor added.
    EXPECT_GT(builder_.GetSystems().size(), previous_system_count);
    const auto& sensor3 = builder_.GetDowncastSubsystemByName<RgbdSensor>("rgbd_sensor_just_for_test");
    EXPECT_EQ(sensor3.color_render_camera().core().renderer_name(), config.renderer_name);
    EXPECT_EQ(sensor3.depth_render_camera().core().renderer_name(), config.renderer_name);
}
...
TEST_F(CameraConfigFunctionsTest, RendererClassVariant) {
    ...
    // Add VTK by name.
    {
        const CameraConfig config{.renderer_name = "vtk_name", .renderer_class = "RenderEngineVtk"};
        ApplyCameraConfig(config, &builder_);
        EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
        EXPECT_THAT(scene_graph_->GetRendererTypeName(config.renderer_name), testing::EndsWith("RenderEngineVtk"));
    }
    // Add VTK by parameters.
    {
        const CameraConfig config{.renderer_name = "vtk_params", .renderer_class = RenderEngineVtkParams{}};
        ApplyCameraConfig(config, &builder_);
        EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
        EXPECT_THAT(scene_graph_->GetRendererTypeName(config.renderer_name), testing::EndsWith("RenderEngineVtk"));
    }
    ...
    {
        const CameraConfig config{.renderer_name = "gltf_client_name", .renderer_class = "RenderEngineGltfClient"};
        ApplyCameraConfig(config, &builder_);
        EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
    ...
    if (geometry::kHasRenderEngineGl) {
        // Add GL by name.
        {
            const CameraConfig config{.renderer_name = "gl_name", .renderer_class = "RenderEngineGl"};
            ApplyCameraConfig(config, &builder_);
            EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
            EXPECT_THAT(scene_graph_->GetRendererTypeName(config.renderer_name), testing::EndsWith("RenderEngineGl"));
        }
        // Add GL by parameters.
        {
            const CameraConfig config{.renderer_name = "gl_params", .renderer_class = RenderEngineGlParams{}};
            ApplyCameraConfig(config, &builder_);
            EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
            EXPECT_THAT(scene_graph_->GetRendererTypeName(config.renderer_name), testing::EndsWith("RenderEngineGl"));
        }
...
TEST_F(CameraConfigFunctionsTest, RendererNameReuse) {
    ...
    auto perform_test = [this, &renderer_count](const auto& parameters, const std::string& name) {
        CameraConfig config{.renderer_name = name + "_renderer",
                            .renderer_class = parameters,
                            .name = name + "_initial_params_succeeds"};
        ApplyCameraConfig(config, &builder_);
        EXPECT_EQ(scene_graph_->RendererCount(), ++renderer_count);
        EXPECT_THAT(scene_graph_->GetRendererTypeName(config.renderer_name), testing::EndsWith(name));
        ...
        config.renderer_class = name == "RenderEngineVtk" ? "RenderEngineGltfClient" : "RenderEngineVtk";
        DRAKE_EXPECT_THROWS_MESSAGE(ApplyCameraConfig(config, &builder_),
...
TEST_F(CameraConfigFunctionsTest, AllParametersCount) {
    ...
    const auto& sensor = builder_.GetDowncastSubsystemByName<RgbdSensor>("rgbd_sensor_test_camera");
    ...
    // Publishing rate.
    const auto& publisher = builder_.GetDowncastSubsystemByName<LcmPublisherSystem>(
    ...
    const DrakeLcmInterface& actual_lcm = const_cast<LcmPublisherSystem&>(publisher).lcm();
...
TEST_F(CameraConfigFunctionsTest, AsyncCamera) {
    ...
    const auto& sensor = builder_.GetDowncastSubsystemByName<RgbdSensorAsync>("rgbd_sensor_test_camera");
    ...
    EXPECT_NEAR(publisher.get_publish_offset(), config.capture_offset + config.output_delay,
...
TEST_F(CameraConfigFunctionsTest, SubsystemPointers) {
    ...
    EXPECT_NO_THROW(ApplyCameraConfig(CameraConfig{}, &builder_, {}, plant_, scene_graph_));
}
...
TEST_F(CameraConfigFunctionsTest, PublishingRgb) {
    ...
    // Check image ports.
    const auto& images = builder_.GetDowncastSubsystemByName<ImageToLcmImageArrayT>("image_to_lcm_test_camera");
...
TEST_F(CameraConfigFunctionsTest, BadLcmBus) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ApplyCameraConfig(config, &builder_), ".*non-default.*special_request.*");
}
...
TEST_F(CameraConfigFunctionsTest, RenderEngineRequest) {
    ...
    ASSERT_EQ(NiceTypeName::RemoveNamespaces(scene_graph_->GetRendererTypeName(default_config.renderer_name)),
              "RenderEngineVtk");

    // Explicitly specifying *new* VTK class produces VTK render engine.
    const CameraConfig vtk_config{.renderer_name = "vtk_renderer", .renderer_class = "RenderEngineVtk"};
    ASSERT_FALSE(scene_graph_->HasRenderer(vtk_config.renderer_name));
    ApplyCameraConfig(vtk_config, &builder_);
    ASSERT_EQ(NiceTypeName::RemoveNamespaces(scene_graph_->GetRendererTypeName(vtk_config.renderer_name)),
              "RenderEngineVtk");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ApplyCameraConfig(CameraConfig{.renderer_name = default_config.renderer_name,
    ...
    const CameraConfig gl_config{.renderer_name = "gl_renderer", .renderer_class = "RenderEngineGl"};
    if (geometry::kHasRenderEngineGl) {
        ASSERT_FALSE(scene_graph_->HasRenderer(gl_config.renderer_name));
        ApplyCameraConfig(gl_config, &builder_);
        ASSERT_EQ(NiceTypeName::RemoveNamespaces(scene_graph_->GetRendererTypeName(gl_config.renderer_name)),
                  "RenderEngineGl");
    } else {
        DRAKE_EXPECT_THROWS_MESSAGE(ApplyCameraConfig(gl_config, &builder_), ".*'RenderEngineGl' is not supported.*");
    }
}
