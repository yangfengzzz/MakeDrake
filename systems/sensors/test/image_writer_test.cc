#include "systems/sensors/image_writer.h"
...
#include "systems/framework/event_collection.h"
...
public:
    ...
    static bool IsDirectoryValid(const std::string& file_path) {
        return ImageWriter::ValidateDirectory(file_path) == ImageWriter::FolderState::kValid;
    }

    static bool DirectoryIsMissing(const std::string& file_path) {
        return ImageWriter::ValidateDirectory(file_path) == ImageWriter::FolderState::kMissing;
    }

    static bool DirectoryIsFile(const std::string& file_path) {
        return ImageWriter::ValidateDirectory(file_path) == ImageWriter::FolderState::kIsFile;
    }

    static bool DirectoryIsUnwritable(const std::string& file_path) {
        return ImageWriter::ValidateDirectory(file_path) == ImageWriter::FolderState::kUnwritable;
    }
    ...
    const std::string& port_format(int port_index) const { return writer_.port_info_[port_index].format; }

    int port_count(int port_index) const { return writer_.port_info_[port_index].count; }

    const std::string& label(PixelType pixel_type) const { return writer_.labels_.at(pixel_type); }

    const std::string& extension(PixelType pixel_type) const { return writer_.extensions_.at(pixel_type); }
...
Image<PixelType::kRgba8U> test_image() {
    ...
    auto set_color = [&color_image](int x, int y, uint8_t r, uint8_t g, uint8_t b) {
...
public:
    static void SetUpTestCase() { ASSERT_TRUE(ImageWriterTester::IsDirectoryValid(temp_dir())); }
    ...
    static void TearDownTestCase() {
        for (const auto& file_name : files_) {
            if (fs::exists({file_name})) {
                ...
                EXPECT_FALSE(fs::exists({file_name})) << "Failed to delete temporary test file: " << file_name;
            }
        }
    }
    ...
    static std::string temp_name() {
        ...
        do {
            temp_path = temp_dir();
            temp_path.append("image_writer_test_" + std::to_string(++img_count_) + ".png");
    ...
    // test suite.
    static void add_file_for_cleanup(const std::string& file_name) { files_.insert(file_name); }
    ...
    static void TestWritingImageOnPort() {
        ...
        const auto& port = writer.DeclareImageInputPort<kPixelType>(port_name, path.string(), period, start_time);
        ...
        const std::string expected_name =
                tester.MakeFileName(tester.port_format(port.get_index()), kPixelType, context->get_time(), port_name,
                                    tester.port_count(port.get_index()));
        fs::path expected_file(expected_name);
        EXPECT_FALSE(fs::exists(expected_file));
        const EventStatus status = writer.Publish(*context, events->get_publish_events());
...
TEST_F(ImageWriterTest, DirectoryFromFormat) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("", "port_name", PixelType::kRgba8U), ".*empty.*");
    EXPECT_EQ("", tester.DirectoryFromFormat("/root", "port_name", PixelType::kRgba8U));
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("/root/", "port_name", PixelType::kRgba8U),
                                ".*cannot end with a '/'");
    EXPECT_EQ("/root", tester.DirectoryFromFormat("/root/file", "port_name", PixelType::kRgba8U));
    ...
    EXPECT_EQ("/root/color", tester.DirectoryFromFormat("/root/{image_type}/file", "port_name", PixelType::kRgba8U));
    EXPECT_EQ("/root/my_port", tester.DirectoryFromFormat("/root/{port_name}/file", "my_port", PixelType::kRgba8U));

    // Test against invalid formatting arguments.
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("/root/{count}/file", "port", PixelType::kRgba8U),
                                ".*The directory path cannot include time or image count");
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("/root/{time_double}/file", "port", PixelType::kRgba8U),
                                ".*The directory path cannot include time or image count");
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("/root/{time_usec}/file", "port", PixelType::kRgba8U),
                                ".*The directory path cannot include time or image count");
    DRAKE_EXPECT_THROWS_MESSAGE(tester.DirectoryFromFormat("/root/{time_msec}/file", "port", PixelType::kRgba8U),
                                ".*The directory path cannot include time or image count");

    // Make sure it's not fooled by strings that are *almost* format arguments.
    EXPECT_EQ("/root/time_double", tester.DirectoryFromFormat("/root/time_double/file", "my_port", PixelType::kRgba8U));
    EXPECT_EQ("/root/time_usec", tester.DirectoryFromFormat("/root/time_usec/file", "my_port", PixelType::kRgba8U));
    EXPECT_EQ("/root/time_msec", tester.DirectoryFromFormat("/root/time_msec/file", "my_port", PixelType::kRgba8U));
    EXPECT_EQ("/root/count", tester.DirectoryFromFormat("/root/count/file", "my_port", PixelType::kRgba8U));
}

// Tests the logic for formatting images.
TEST_F(ImageWriterTest, FileNameFormatting) {
    auto test_file_name = [](const ImageWriter& writer, const std::string& format, PixelType pixel_type, double time,
                             const std::string& port_name, int count, const std::string expected) {
        const std::string path = ImageWriterTester(writer).MakeFileName(format, pixel_type, time, port_name, count);
    ...
    test_file_name(writer, "/hard/coded/file.png", PixelType::kRgba8U, 0, "port", 0, "/hard/coded/file.png");

    // Use the port name.
    test_file_name(writer, "/hard/{port_name}/file.png", PixelType::kRgba8U, 0, "port", 0, "/hard/port/file.png");

    // Use the image type.
    test_file_name(writer, "/hard/{image_type}/file.png", PixelType::kRgba8U, 0, "port", 0, "/hard/color/file.png");
    test_file_name(writer, "/hard/{image_type}/file.png", PixelType::kDepth32F, 0, "port", 0, "/hard/depth/file.png");
    test_file_name(writer, "/hard/{image_type}/file.png", PixelType::kLabel16I, 0, "port", 0, "/hard/label/file.png");

    // Use the time values.
    test_file_name(writer, "/hard/{time_double:.2f}.png", PixelType::kRgba8U, 0, "port", 0, "/hard/0.00.png");
    test_file_name(writer, "/hard/{time_double:.2f}.png", PixelType::kRgba8U, 1.111, "port", 0, "/hard/1.11.png");
    test_file_name(writer, "/hard/{time_double:.2f}.png", PixelType::kRgba8U, 1.116, "port", 0, "/hard/1.12.png");

    test_file_name(writer, "/hard/{time_usec:03}.png", PixelType::kRgba8U, 0, "port", 0, "/hard/000.png");
    test_file_name(writer, "/hard/{time_usec:03}.png", PixelType::kRgba8U, 1.111, "port", 0, "/hard/1111000.png");

    test_file_name(writer, "/hard/{time_msec:03}.png", PixelType::kRgba8U, 0, "port", 0, "/hard/000.png");
    test_file_name(writer, "/hard/{time_msec:03}.png", PixelType::kRgba8U, 1.111, "port", 0, "/hard/1111.png");

    // Use the count value.
    test_file_name(writer, "/hard/{count:03}.png", PixelType::kRgba8U, 1.111, "port", 0, "/hard/000.png");
    test_file_name(writer, "/hard/{count:03}.png", PixelType::kRgba8U, 1.111, "port", 12, "/hard/012.png");

    // Bad place holders.
    EXPECT_THROW(
            test_file_name(writer, "/hard/{port}/file.png", PixelType::kRgba8U, 0, "port", 0, "/hard/{port}/file.png"),
            fmt::format_error);
}
...
TEST_F(ImageWriterTest, ValidateDirectory) {
    ...
    EXPECT_TRUE(ImageWriterTester::DirectoryIsMissing("this/path/does/not_exist"));
...
TEST_F(ImageWriterTest, ConfigureInputPortErrors) {
    ...
    // Bad publish period.
    DRAKE_EXPECT_THROWS_MESSAGE(writer.DeclareImageInputPort<PixelType::kRgba8U>("port", "format", -0.1, 0),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(writer.DeclareImageInputPort<PixelType::kRgba8U>("port", "/root/name", 0.1, 0),
    ...
    const auto& port = writer.DeclareImageInputPort<PixelType::kRgba8U>("port", path.string(), 0.1, 0);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(writer.DeclareImageInputPort<PixelType::kRgba8U>("port", path2.string(), 0.1, 0),
...
void TestRuntimePixelType() {
    ImageWriter writer;
    const auto& port = writer.DeclareImageInputPort(kPixelType, "in", "/tmp/{time_usec}", 1, 1);
...
template <PixelType kPixelType>
void TestPixelExtension(const std::string& folder, ImageWriter* writer, int* count) {
    using std::to_string;
    ...
    format.append("file");
    const auto& port = writer->DeclareImageInputPort<kPixelType>("port" + to_string(++(*count)), format.string(), 1, 1);
...
TEST_F(ImageWriterTest, FileExtension) {
    ...
    // Case: Format string with correct extension remains unchanged.
    {
        ...
        const auto& port =
                writer.DeclareImageInputPort<PixelType::kRgba8U>("port" + to_string(++count), format.string(), 1, 1);
        const std::string& final_format = tester.port_format(port.get_index());
...
TEST_F(ImageWriterTest, SingleConfiguredPort) {
    ...
    const auto& port = writer.DeclareImageInputPort<PixelType::kRgba8U>(port_name, path.string(), period, start_time);
    ...
    // configuration parameters above are called out below in commented lines.
    {
...

        {
            const auto& publish_events =
                    dynamic_cast<const LeafEventCollection<PublishEvent<double>>&>(events->get_publish_events())
                            .get_events();
            ...
            const auto& forced_publish_events =
                    dynamic_cast<const LeafEventCollection<PublishEvent<double>>&>(*forced_events).get_events();
            ...
            // in an error.
            DRAKE_EXPECT_THROWS_MESSAGE(writer.Publish(*context, events->get_publish_events()),
            ...
            std::string expected_name =
                    tester.MakeFileName(tester.port_format(port.get_index()), pixel_type, context->get_time(),
            ...
            expected_name = tester.MakeFileName(tester.port_format(port.get_index()), pixel_type, context->get_time(),
                                                port_name, tester.port_count(port.get_index()));
