...
class VtkImageReaderWriterTest : public testing::TestWithParam<ImageIoTestParams> {
public:
...
TEST_P(VtkImageReaderWriterTest, RoundTrip) {
    ...
    if (write_to_file) {
        filename = fs::path(temp_directory()) / fmt::format("RoundTrip_{}.image", format);
...
INSTANTIATE_TEST_SUITE_P(All, VtkImageReaderWriterTest, GetAllImageIoTestParams());
...
GTEST_TEST(UnparameterizedVtkImageReaderWriterTest, TiffWriteToMemoryThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(MakeWriter(ImageFileFormat::kTiff, &output), ".*TIFF.*memory buffer.*");
}
