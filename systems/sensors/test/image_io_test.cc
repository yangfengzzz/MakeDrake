...
#include "systems/sensors/test/image_io_test_params.h"
...
class ImageIoParameterizedTest : public testing::TestWithParam<ImageIoTestParams> {
public:
...
TEST_P(ImageIoParameterizedTest, RoundTripPreAllocated) {
    ...
    // Call Save().
    const fs::path path = GetTempPath(fmt::format("RoundTripPreAllocated_{}.image", format));
    std::visit(
            [&](const auto& image) {
                ImageIo{}.Save(image, path, format);
            },
...
INSTANTIATE_TEST_SUITE_P(All, ImageIoParameterizedTest, GetAllImageIoTestParams());
...
GTEST_TEST(ImageIoTest, SaveToFileNoExensionNoFormatError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Save(image, path), ".*path does not imply.*");
}
...
GTEST_TEST(ImageIoTest, LoadNoSuchFileError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(path), ".*LoadNoSuchFileError.*open.*");
}
...
GTEST_TEST(ImageIoTest, LoadNotAnImageError) {
    ...
    EXPECT_EQ(ImageIo{}.LoadMetadata(path), std::nullopt);
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(path), ".*LoadNotAnImageError.*header.*");
}

// Loading into the wrong pre-allocated Image scalar type fails.
GTEST_TEST(ImageIoTest, LoadWrongPrealloatedScalarError) {
    const std::vector<uint8_t> saved = ImageIo{}.Save(ImageDepth16U(1, 1), ImageFileFormat::kPng);
    const ImageIo::ByteSpan buffer{saved.data(), saved.size()};
    ImageRgba8U color;
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(buffer, &color), ".*scalar=16U into scalar=8U.*");
}
...
GTEST_TEST(ImageIoTest, LoadWrongPrealloatedNumChannelsError) {
    const std::vector<uint8_t> saved = ImageIo{}.Save(ImageGrey8U(1, 1), ImageFileFormat::kPng);
    const ImageIo::ByteSpan buffer{saved.data(), saved.size()};
    ImageRgb8U rgb;
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(buffer, &rgb), ".*channels=1.*channels=3.*");
}
...
GTEST_TEST(ImageIoTest, LoadUnsupportedChannelScalarError) {
    const fs::path path{FindResourceOrThrow("drake/systems/sensors/test/png_color16_test.png")};
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(path, ImageFileFormat::kPng), ".*template instantiation.*");
}

GTEST_TEST(ImageIoTest, LoadUnsupportedColorTiffError) {
    const fs::path path{FindResourceOrThrow("drake/systems/sensors/test/tiff_rgb32f_test.tif")};
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(path, ImageFileFormat::kTiff), ".*template instantiation.*");
    ImageRgba8U color;
    DRAKE_EXPECT_THROWS_MESSAGE(ImageIo{}.Load(path, &color), ".*scalar=32F into scalar=8U.*");
}
