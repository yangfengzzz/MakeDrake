...
BeamModel<T>::BeamModel(int num_depth_readings, double max_range)
    ...
    this->DeclareInputPort("event", kVectorValued, num_depth_readings, RandomDistribution::kUniform);
    // Declare "hit" random input port.
    this->DeclareInputPort("hit", kVectorValued, num_depth_readings, RandomDistribution::kGaussian);
    // Declare "short" random input port.
    this->DeclareInputPort("short", kVectorValued, num_depth_readings, RandomDistribution::kExponential);
    // Declare "uniform" random input port.
    this->DeclareInputPort("uniform", kVectorValued, num_depth_readings, RandomDistribution::kUniform);
    // Declare measurement output port.
    this->DeclareVectorOutputPort("depth", num_depth_readings, &BeamModel<T>::CalcOutput);
    ...
    ContextConstraintCalc<T> calc_event_probabilities_constraint = [](const Context<T>& context, VectorX<T>* value) {
        const auto* params = dynamic_cast<const BeamModelParams<T>*>(&context.get_numeric_parameter(0));
        DRAKE_DEMAND(params != nullptr);
        (*value)[0] = 1.0 - params->probability_short() - params->probability_miss() - params->probability_uniform();
    };
    this->DeclareInequalityConstraint(calc_event_probabilities_constraint,
...
template <typename T>
BeamModelParams<T>& BeamModel<T>::get_mutable_parameters(Context<T>* context) const {
    return this->template GetMutableNumericParameter<BeamModelParams>(context, 0);
}

template <typename T>
void BeamModel<T>::CalcOutput(const systems::Context<T>& context, BasicVector<T>* output) const {
    const auto params = dynamic_cast<const BeamModelParams<T>*>(&context.get_numeric_parameter(0));
    DRAKE_DEMAND(params != nullptr);
    ...
    for (int i = 0; i < static_cast<int>(depth.size()); i++) {
        ...
        } else if (w_event[i] <= params->probability_uniform() + params->probability_miss()) {
            // Then "miss".
            measurement[i] = max_range_;
        } else if (w_event[i] <=
                           params->probability_uniform() + params->probability_miss() + params->probability_short() &&
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_NONSYMBOLIC_SCALARS(class ::drake::systems::sensors::BeamModel);
