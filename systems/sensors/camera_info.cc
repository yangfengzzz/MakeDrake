...
void CheckBetweenZeroAndMax(const char* name, double value, int max, std::stringstream* s) {
...
CameraInfo::CameraInfo(int width, int height, double focal_x, double focal_y, double center_x, double center_y)
    : CameraInfo(width,
                 height,
                 // clang-format off
                 (Eigen::Matrix3d() <<
...
CameraInfo::CameraInfo(int width, int height, const Eigen::Matrix3d& intrinsic_matrix)
    ...
    if (K(0, 1) != 0 || K(1, 0) != 0 || K(2, 0) != 0 || K(2, 1) != 0 || K(2, 2) != 1) {
