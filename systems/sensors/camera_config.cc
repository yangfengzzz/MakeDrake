...
std::pair<ColorRenderCamera, DepthRenderCamera> CameraConfig::MakeCameras() const {
    const Vector2<double> center = principal_point();
    CameraInfo intrinsics(width, height, focal_x(), focal_y(), center.x(), center.y());
    ClippingRange clipping_range(clipping_near, clipping_far);
    RenderCameraCore color_core(renderer_name, intrinsics, clipping_range, X_BC.GetDeterministicValue());
    RenderCameraCore depth_core(renderer_name, intrinsics, clipping_range, X_BD.GetDeterministicValue());
    DepthRange depth_range(z_near, z_far);
    return {ColorRenderCamera(color_core, show_rgb), DepthRenderCamera(depth_core, depth_range)};
}
...
void CameraConfig::ValidateOrThrow() const {
    ...
    if (std::holds_alternative<std::string>(renderer_class)) {
        ...
        if (!class_name.empty() && !(class_name == "RenderEngineVtk" || class_name == "RenderEngineGl" ||
                                     class_name == "RenderEngineGltfClient")) {
            throw std::logic_error(
                    fmt::format("Invalid camera configuration; the given renderer_class value '{}' "
    ...
    if (name.empty()) {
        throw std::logic_error("Invalid camera configuration; name cannot be empty.");
    }

    if (renderer_name.empty()) {
        throw std::logic_error("Invalid camera configuration; renderer_name cannot be empty.");
    }
    ...
    if (capture_offset < 0 || !std::isfinite(capture_offset)) {
        throw std::logic_error(
                fmt::format("Invalid camera configuration; capture_offset ({}) must be a finite, "
                            "non-negative value.",
                            capture_offset));
    }
