...
public:
    ...
    // system.
    const InputPort<T>& get_observed_system_input_input_port() const { return this->get_input_port(1); }

    // Returns the input port that expects the outputs of the observed system.
    const InputPort<T>& get_observed_system_output_input_port() const { return this->get_input_port(0); }

    // Returns the output port that provides the estimated state.
    const OutputPort<T>& get_estimated_state_output_port() const { return this->get_output_port(0); }
...
private:
    ...
    void CalcEstimatedState(const Context<T>& context, BasicVector<T>* output) const;

    void UpdateObservedSystemContext(const Context<T>& context, Context<T>* observed_system_context) const;
