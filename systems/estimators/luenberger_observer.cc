#include "systems/estimators/luenberger_observer.h"
...
template <typename T>
LuenbergerObserver<T>::LuenbergerObserver(const System<T>* system,
    ...
    const InputPort<T>* const observed_system_input =
            (observed_system_->num_input_ports() > 0) ? &observed_system_->get_input_port() : nullptr;
    const OutputPort<T>& observed_system_output = observed_system_->get_output_port();
    ...
    this->DeclareVectorOutputPort("estimated_state", xc.size(), &LuenbergerObserver::CalcEstimatedState,
                                  {this->xc_ticket()});
    ...
    const auto& y_input_port = this->DeclareVectorInputPort(
            ...
            *observed_system_->AllocateOutput()->get_vector_data(observed_system_output.get_index()));
    ...
    if (observed_system_input != nullptr) {
        ...
        auto input_vec = observed_system_->AllocateInputVector(*observed_system_input);
        const auto& u_input_port = this->DeclareVectorInputPort("observed_system_input", *input_vec);
    ...
    observed_system_context_cache_entry_ =
            &this->DeclareCacheEntry("observed system context", observed_system_context,
                                     &LuenbergerObserver::UpdateObservedSystemContext, input_port_tickets);
}

template <typename T>
LuenbergerObserver<T>::LuenbergerObserver(const System<T>& observed_system,
                                          const Context<T>& observed_system_context,
                                          const Eigen::Ref<const Eigen::MatrixXd>& observer_gain)
    : LuenbergerObserver(&observed_system, nullptr, observed_system_context, observer_gain) {}

template <typename T>
LuenbergerObserver<T>::LuenbergerObserver(std::unique_ptr<System<T>> observed_system,
                                          const Context<T>& observed_system_context,
                                          const Eigen::Ref<const Eigen::MatrixXd>& observer_gain)
    : LuenbergerObserver(nullptr, std::move(observed_system), observed_system_context, observer_gain) {}

template <typename T>
void LuenbergerObserver<T>::CalcEstimatedState(const Context<T>& context, BasicVector<T>* output) const {
    output->set_value(context.get_continuous_state_vector().CopyToVector());
}

template <typename T>
void LuenbergerObserver<T>::UpdateObservedSystemContext(const Context<T>& context,
    ...
    if (observed_system_->num_input_ports() > 0) {
        ...
        observed_system_->get_input_port().FixValue(observed_system_context,
                                                    this->get_observed_system_input_input_port().Eval(context));
    }
    // Set observed system state.
    observed_system_context->get_mutable_continuous_state_vector().SetFrom(context.get_continuous_state_vector());
}

template <typename T>
void LuenbergerObserver<T>::DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const {
    const Context<T>& observed_system_context = observed_system_context_cache_entry_->Eval<Context<T>>(context);

    // Evaluate the observed system.
    const VectorX<T>& yhat = observed_system_->get_output_port().Eval(observed_system_context);
    VectorX<T> xdothat = observed_system_->EvalTimeDerivatives(observed_system_context).CopyToVector();

    // Get the measurements.
    const VectorX<T>& y = this->get_observed_system_output_input_port().Eval(context);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::estimators::LuenbergerObserver);
