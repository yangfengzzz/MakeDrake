...
GTEST_TEST(LuenbergerObserverTest, ErrorDynamics) {
  ...
  L << 24., 25., 26.,
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        std::unique_ptr<systems::estimators::LuenbergerObserver<double>> observer{nullptr};
        switch (i) {
            case 0:  // pass system and context by reference.
                observer = std::make_unique<systems::estimators::LuenbergerObserver<double>>(*plant, *plant_context, L);
                break;
            case 1:  // owned system version.
                observer = std::make_unique<systems::estimators::LuenbergerObserver<double>>(std::move(plant),
                                                                                             *plant_context, L);
                break;
        }
        ...
        EXPECT_TRUE(CompareMatrices(xhat, output->GetMutableVectorData(0)->CopyToVector(), tol));
...
GTEST_TEST(LuenbergerObserverTest, DerivedTypes) {
    ...
    auto observer =
            std::make_unique<systems::estimators::LuenbergerObserver<double>>(std::move(plant), *plant_context, L);
    auto context = observer->CreateDefaultContext();
    ...
    EXPECT_TRUE(is_dynamic_castable<examples::pendulum::PendulumState<double>>(input0.get()));

    // The second input is the input of the plant.
    auto input1 = observer->AllocateInputVector(observer->get_input_port(1));
    EXPECT_TRUE(is_dynamic_castable<examples::pendulum::PendulumInput<double>>(input1.get()));
