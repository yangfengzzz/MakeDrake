...
GTEST_TEST(TestKalman, DoubleIntegrator) {
    ...
    auto linear_filter = SteadyStateKalmanFilter(std::make_unique<LinearSystem<double>>(A, B, C, D), W, V);
    ...
    auto filter = SteadyStateKalmanFilter(std::move(sys), std::move(context), W, V);
