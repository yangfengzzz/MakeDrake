#include "systems/rendering/multibody_position_to_geometry_pose.h"
...
GTEST_TEST(MultibodyPositionToGeometryPoseTest, BadConstruction) {
...

    {
        ...
        Parser(&mbp).AddModelsFromUrl("package://drake_models/iiwa_description/sdf/iiwa7_no_collision.sdf");
        DRAKE_EXPECT_THROWS_MESSAGE(MultibodyPositionToGeometryPose<double>{mbp},
...
GTEST_TEST(MultibodyPositionToGeometryPoseTest, Ownership) {
    ...
    SceneGraph<double> scene_graph;
    mbp->RegisterAsSourceForSceneGraph(&scene_graph);
    Parser(mbp.get()).AddModelsFromUrl("package://drake_models/iiwa_description/sdf/iiwa7_no_collision.sdf");
    mbp->Finalize();
    ...
    const Eigen::VectorXd position = Eigen::VectorXd::LinSpaced(raw_ptr->num_positions(), 0.123, 0.456);
    dut.get_input_port().FixValue(context.get(), position);

    const auto& output = dut.get_output_port().Eval<geometry::FramePoseVector<double>>(*context);
    for (BodyIndex i(0); i < raw_ptr->num_bodies(); ++i) {
        if (i == raw_ptr->world_body().index()) {
            // The world geometry will not appear in the poses.
            continue;
        }
        const std::optional<geometry::FrameId> id = raw_ptr->GetBodyFrameIdIfExists(i);
...
GTEST_TEST(MultibodyPositionToGeometryPoseTest, InputOutput) {
    ...
    const Eigen::VectorXd position = Eigen::VectorXd::LinSpaced(mbp.num_positions(), 0.123, 0.456);
    dut.get_input_port().FixValue(context.get(), position);
...
GTEST_TEST(MultibodyPositionToGeometryPoseTest, FullStateInput) {
    ...
    const Eigen::VectorXd state = Eigen::VectorXd::LinSpaced(mbp->num_multibody_states(), 0.123, 0.456);
    ...
    position_sys.get_input_port().FixValue(position_context.get(), state.head(mbp->num_positions()));
    const auto& position_output =
            position_sys.get_output_port().Eval<geometry::FramePoseVector<double>>(*position_context);
    ...
    const auto& state_output = state_sys.get_output_port().Eval<geometry::FramePoseVector<double>>(*state_context);

    EXPECT_EQ(position_output.size(), state_output.size());
    for (const auto& id : position_output.ids()) {
        EXPECT_TRUE(position_output.value(id).IsExactlyEqualTo(state_output.value(id)));
    }
    ...
    EXPECT_EQ(owned_sys.get_input_port().size(), owned_sys.multibody_plant().num_multibody_states());
}
