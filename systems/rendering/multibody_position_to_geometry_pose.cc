...
template <typename T>
MultibodyPositionToGeometryPose<T>::MultibodyPositionToGeometryPose(const multibody::MultibodyPlant<T>& plant,
...
MultibodyPositionToGeometryPose<T>::MultibodyPositionToGeometryPose(
        std::unique_ptr<multibody::MultibodyPlant<T>> owned_plant, bool input_multibody_state)
...
void MultibodyPositionToGeometryPose<T>::Configure(bool input_multibody_state) {
    ...
    this->DeclareInputPort("position", kVectorValued,
                           input_multibody_state ? plant_.num_multibody_states() : plant_.num_positions());
...
template <typename T>
void MultibodyPositionToGeometryPose<T>::CalcGeometryPose(const Context<T>& context, AbstractValue* output) const {
    ...
    plant_.SetPositions(plant_context_.get(), this->get_input_port().Eval(context).head(plant_.num_positions()));
