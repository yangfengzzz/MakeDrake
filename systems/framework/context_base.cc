...
std::unique_ptr<ContextBase> ContextBase::Clone() const {
    if (!is_root_context()) {
        throw std::logic_error(
                fmt::format("Context::Clone(): Cannot clone a non-root Context; "
...
void ContextBase::AddOutputPort(OutputPortIndex expected_index,
                                DependencyTicket ticket,
...
void ContextBase::SetFixedInputPortValue(InputPortIndex index, std::unique_ptr<FixedInputPortValue> port_value) {
    ...
    if (old_value != nullptr) {
        ...
        DRAKE_ASSERT(port_tracker.HasPrerequisite(graph_.get_tracker(ticket_to_use)));
    } else {
...
void ContextBase::CreateBuiltInTrackers() {
    ...
    auto& xd_unique_periodic_update_tracker = graph.CreateNewDependencyTracker(
            DependencyTicket(internal::kXdUniquePeriodicUpdateTicket), "xd_unique_periodic_update");
    unused(xd_unique_periodic_update_tracker);
}

void ContextBase::BuildTrackerPointerMap(const ContextBase& source,
...
void ContextBase::FixContextPointers(const ContextBase& source,
