...
protected:
    ...
    [[noreturn]] void ThrowOutOfRange(int index) const {
        throw std::out_of_range(fmt::format("Index {} is not within [0, {}) while accessing {}.", index, size(),
                                            NiceTypeName::Get(*this)));
    }

    [[noreturn]] void ThrowMismatchedSize(int other_size) const {
        throw std::out_of_range(fmt::format("Operand vector size {} does not match this {} size {}", other_size,
                                            NiceTypeName::Get(*this), size()));
    }
};
