...
public:
    ...
    void set_value(const Eigen::Ref<const VectorX<T>>& value) { get_mutable_vector().set_value(value); }
    ...
    Eigen::VectorBlock<VectorX<T>> get_mutable_value() { return get_mutable_vector().get_mutable_value(); }
    ...
    /// direct access to the underlying VectorX rather than wrapping it in a
    /// VectorBlock.
    Eigen::VectorBlock<const VectorX<T>> get_value(int index) const { return get_vector(index).get_value(); }
    //@}
