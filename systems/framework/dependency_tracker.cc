...
void DependencyTracker::NoteValueChange(int64_t change_event) const {
    ...
    if (last_change_event_ == change_event || suppress_notifications_) {
        ...
        DRAKE_LOGGER_DEBUG("... ignoring repeated or suppressed value change notification.");
        return;
    }
...
void DependencyTracker::NotePrerequisiteChange(int64_t change_event,
    ...
    if (last_change_event_ == change_event || suppress_notifications_) {
        ++num_ignored_notifications_;
        DRAKE_LOGGER_DEBUG("{}... ignoring repeated or suppressed prereq change notification.", Indent(depth));
        return;
    }
    last_change_event_ = change_event;
...
void DependencyTracker::RepairTrackerPointers(const DependencyTracker& source,
    ...
    DRAKE_DEMAND(has_associated_cache_entry_ == source.has_associated_cache_entry_);
