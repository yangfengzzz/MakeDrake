...
bool SystemScalarConverter::IsConvertible(const std::type_info& t_info, const std::type_info& u_info) const {
...
void ThrowConversionMismatch(const type_info& s_t_info, const type_info& s_u_info, const type_info& other_info) {
    throw std::runtime_error(
            fmt::format("SystemScalarConverter was configured to convert a {} into a {}"
                        " but was called with a {} at runtime",
                        NiceTypeName::Get(s_u_info), NiceTypeName::Get(s_t_info), NiceTypeName::Get(other_info)));
}

template <typename T, typename U>
void AddPydrakeConverterFunction(SystemScalarConverter* converter,
...
DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&AddPydrakeConverterFunction<T, U>));

}  // namespace system_scalar_converter_internal

DRAKE_DEFINE_FUNCTION_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS((&SystemScalarConverter::IsConvertible<T, U>,
