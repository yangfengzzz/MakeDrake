...
std::string SystemBase::GetMemoryObjectName() const {
    ...
    const std::string default_name =
            std::regex_replace(type_name_without_templates, std::regex(":+"), std::string("/"));
...
std::string SystemBase::GetGraphvizString(std::optional<int> max_depth,
...
SystemBase::GraphvizFragment SystemBase::GetGraphvizFragment(std::optional<int> max_depth,
                                                             const std::map<std::string, std::string>& options) const {
    ...
    const std::string system_type = std::regex_replace(NiceTypeName::RemoveNamespaces(NiceTypeName::Get(*this)),
                                                       std::regex("<.*>$"), std::string());
    params.header_lines.push_back(fmt::format("<B>{}</B>", HtmlEscape(system_type)));
    ...
    if (!system_name.empty() && system_name != this->GetMemoryObjectName()) {
        params.header_lines.push_back(fmt::format("name={}", HtmlEscape(system_name)));
    }

    return DoGetGraphvizFragment(params);
}

SystemBase::GraphvizFragment SystemBase::DoGetGraphvizFragment(const GraphvizFragmentParams& params) const {
    ...
    const std::string node_id_input = split_in_twain ? params.node_id + "in" : params.node_id;
    const std::string node_id_output = split_in_twain ? params.node_id + "out" : params.node_id;
    ...
    {
        std::vector<std::string> labels_input = GetGraphvizPortLabels(/* input= */ true);
        std::vector<std::string> labels_output = GetGraphvizPortLabels(/* input= */ false);
    ...
    if (num_inputs != num_outputs) {
        ...
        if (min_num_inputs_outputs == 1) {
            ...
            // the entire column.
            port_styles(0, shorter_col) = fmt::format("ROWSPAN=\"{}\"", max_num_inputs_outputs);
        } else {
            ...
            port_styles(filler_row, shorter_col) = fmt::format("COLOR=\"grey\" ROWSPAN=\"{}\" SIDES=\"{}\"", remainder,
                                                               shorter_col == 0 ? "BL" : "BR");
        }
    }
...
std::vector<std::string> SystemBase::GetGraphvizPortLabels(bool input) const {
    ...
    for (int i = 0; i < num_ports; ++i) {
        ...
        if (port.get_data_type() == PortDataType::kVectorValued && port.size() == 0) {
...
CacheEntry& SystemBase::DeclareCacheEntryWithKnownTicket(DependencyTicket known_ticket,
    ...
    cache_entries_.emplace_back(std::make_unique<CacheEntry>(this, index, known_ticket, std::move(description),
...
void SystemBase::InitializeContextBase(ContextBase* context_ptr) const {
    ...
    DRAKE_DEMAND(!internal::SystemBaseContextBaseAttorney::is_context_base_initialized(context));
    ...
    for (CacheIndex index(0); index < num_cache_entries(); ++index) {
        ...
        CacheEntryValue& cache_value = cache.CreateNewCacheEntryValue(
                entry.cache_index(), entry.ticket(), entry.description(), entry.prerequisites(), &graph);
...
void SystemBase::CreateSourceTrackers(ContextBase* context_ptr) const {
    ...
    make_trackers(xd_ticket(), discrete_state_tickets_,
    ...
    make_trackers(xa_ticket(), abstract_state_tickets_,
...
// that system.
const AbstractValue* SystemBase::EvalAbstractInputImpl(const char* func,
                                                       const ContextBase& context,
                                                       InputPortIndex port_index) const {
    if (port_index >= num_input_ports()) ThrowInputPortIndexOutOfRange(func, port_index);

    if (input_ports_[port_index]->get_deprecation().has_value()) WarnPortDeprecation(/* is_input = */ true, port_index);
...
void SystemBase::ThrowNegativePortIndex(const char* func, int port_index) const {
    ...
    throw std::out_of_range(fmt::format("{}: negative port index {} is illegal. (System {})", FmtFunc(func), port_index,
                                        GetSystemPathname()));
}
...
void SystemBase::ThrowNotAVectorInputPort(const char* func, InputPortIndex port) const {
    throw std::logic_error(
            fmt::format("{}: vector port required, but input port '{}' (index {}) was declared "
                        "abstract. Even if the actual value is a vector, use "
                        "EvalInputValue<V> instead for an abstract port containing a vector "
                        "of type V. (System {})",
                        FmtFunc(func), get_input_port_base(port).get_name(), port, GetSystemPathname()));
}
...
void SystemBase::ThrowInputPortHasWrongType(const char* func,
    ...
    throw std::logic_error(
            fmt::format("{}: expected value of type {} for input port '{}' (index {}) "
                        "but the actual type was {}. (System {})",
                        FmtFunc(func), expected_type, port_name, port, actual_type, system_pathname));
}
...
void SystemBase::ThrowValidateContextMismatch(const ContextBase& context) const {
    ...
    if (get_parent_service() != nullptr) {
        ...
        const internal::SystemId root_id = get_parent_service()->GetRootSystemBase().get_system_id();
        if (context.get_system_id() == root_id) {
            throw std::logic_error(
                    fmt::format("A function call on a {} system named '{}' was passed the root "
    ...
    const ContextBase& root_context = [&context]() -> const ContextBase& {
        ...
        while (true) {
            const ContextBase* parent = internal::SystemBaseContextBaseAttorney::get_parent_base(*iterator);
            if (parent == nullptr) {
                return *iterator;
            }
            iterator = parent;
    ...
    if (root_context.get_system_id() == get_system_id()) {
        throw std::logic_error(
                fmt::format("A function call on the root Diagram was passed a subcontext "
    ...
    throw std::logic_error(
            fmt::format("A function call on a {} system named '{}' was passed the Context of "
                        "a system named '{}' instead of the appropriate subsystem Context.\n{}",
                        this->GetSystemType(), this->GetSystemPathname(), context.GetSystemPathname(), info_link));
}

void SystemBase::ThrowNotCreatedForThisSystemImpl(const std::string& nice_type_name, internal::SystemId id) const {
    if (!id.is_valid()) {
        throw std::logic_error(
                fmt::format("{} was not associated with any System but should have been "
                            "created for {} System {}",
                            nice_type_name, GetSystemType(), GetSystemPathname()));
    } else {
        throw std::logic_error(fmt::format("{} was not created for {} System {}", nice_type_name, GetSystemType(),
                                           GetSystemPathname()));
    }
}
...
void SystemBase::WarnPortDeprecation(bool is_input, int port_index) const {
    ...
    std::atomic<bool>* const deprecation_already_warned = internal::PortBaseAttorney::deprecation_already_warned(port);
    ...
    const char* const message = deprecation.size() ? deprecation.c_str() : "no deprecation details were provided";
    drake::log()->warn("{} is deprecated: {}", description, message);
}

std::string SystemBase::GetUnsupportedScalarConversionMessage(const std::type_info& source_type,
                                                              const std::type_info& destination_type) const {
    unused(source_type);
    return fmt::format("System {} of type {} does not support scalar conversion to type {}", GetSystemPathname(),
                       GetSystemType(), NiceTypeName::Get(destination_type));
}

namespace internal {

std::string DiagramSystemBaseAttorney::GetUnsupportedScalarConversionMessage(const SystemBase& system,
                                                                             const std::type_info& source_type,
                                                                             const std::type_info& destination_type) {
    return system.GetUnsupportedScalarConversionMessage(source_type, destination_type);
}

std::vector<std::string> DiagramSystemBaseAttorney::GetGraphvizPortLabels(const SystemBase& system, bool input) {
