...
#include "systems/framework/framework_common.h"
#include "systems/framework/output_port_base.h"
...
public:
    ...
    */
    ...
    const VectorX<T>& Eval(const Context<T>& context) const { return Eval<BasicVector<T>>(context).value(); }
    ...
    template <typename ValueType,
              typename = std::enable_if_t<!std::is_same_v<AbstractValue, ValueType> &&
    ...
    template <typename ValueType,
              typename = std::enable_if_t<std::is_base_of_v<BasicVector<T>, ValueType> &&
                                          !std::is_same_v<BasicVector<T>, ValueType>>>
    ...
    std::unique_ptr<AbstractValue> Allocate() const {
        ...
        if (value == nullptr) {
            throw std::logic_error(
                    fmt::format("OutputPort::Allocate(): allocator returned a nullptr for {}.", GetFullDescription()));
        }
...
protected:
    ...
    OutputPort(const System<T>* system,
               ...
               int size)
        : OutputPortBase(system_interface, system_id, std::move(name), index, ticket, data_type, size),
          system_{*system} {
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::OutputPort);
