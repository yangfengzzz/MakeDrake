...
public:
    ...
    [[nodiscard]] EventStatus Publish(const Context<T>& context, const EventCollection<PublishEvent<T>>& events) const;
    ...
    const Vec<T>* EvalVectorInput(const Context<T>& context, int port_index) const {
        ...
        if (value == nullptr) {
            ThrowInputPortHasWrongType(__func__, iport_index, NiceTypeName::Get<Vec<T>>(),
    ...
    [[nodiscard]] EventStatus CalcDiscreteVariableUpdate(const Context<T>& context,
    ...
    [[nodiscard]] EventStatus CalcUnrestrictedUpdate(const Context<T>& context,
    ...
    void ApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                 State<T>* state,
                                 Context<T>* context) const;
    ...
    */
    ...
    @param context Optional Context to pass on to Event selection functions;
        not commonly needed. */
    std::map<PeriodicEventData, std::vector<const Event<T>*>, PeriodicEventDataComparator> MapPeriodicEventsByTiming(
    ...
    const InputPort<T>& get_input_port(int port_index, bool warn_deprecated = true) const {
        ...
        return static_cast<const InputPort<T>&>(this->GetInputPortBaseOrThrow(__func__, port_index, warn_deprecated));
    }
    ...
    const OutputPort<T>& get_output_port(int port_index, bool warn_deprecated = true) const {
        ...
        // has a check that port.get_system_interface() matches `this` which is a
        // System<T>, so we are safe.
        return static_cast<const OutputPort<T>&>(this->GetOutputPortBaseOrThrow(__func__, port_index, warn_deprecated));
    }
    ...
    static std::unique_ptr<S<U>> ToScalarType(const S<T>& from) {
        ...
        if (!base_result) {
            ...
            throw std::logic_error(upcast_from.GetUnsupportedScalarConversionMessage(typeid(T), typeid(U)));
        }
...
protected:
    ...
    static void FindUniquePeriodicDiscreteUpdatesOrThrow(const char* api_name,
        ...
        system.DoFindUniquePeriodicDiscreteUpdatesOrThrow(api_name, context, timing, events);
    }
    ...
    [[nodiscard]] virtual EventStatus DispatchPublishHandler(const Context<T>& context,
    ...
    the given `events`. */
    virtual void DoApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                           State<T>* state,
                                           Context<T>* context) const = 0;
    //@}
    ...
    InputPort<T>& DeclareInputPort(std::variant<std::string, UseDefaultName> name,
                                   PortDataType type,
                                   int size,
    ...
    @see MapPeriodicEventsByTiming() for a detailed description of the returned
         variable. */
    virtual std::map<PeriodicEventData, std::vector<const Event<T>*>, PeriodicEventDataComparator>
...
private:
    ...
    virtual void DoFindUniquePeriodicDiscreteUpdatesOrThrow(const char* api_name,
                                                            const Context<T>& context,
                                                            std::optional<PeriodicEventData>* timing,
    ...
    const BasicVector<T>* EvalBasicVectorInputImpl(const char* func,
                                                   const Context<T>& context,
    ...
    void CalcUniquePeriodicDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* updated) const;
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::System);
