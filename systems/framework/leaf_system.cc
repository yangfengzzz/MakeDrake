...
#include "systems/framework/system_symbolic_inspector.h"
#include "systems/framework/value_checker.h"
...
void LeafSystem<T>::SetDefaultState(const Context<T>& context, State<T>* state) const {
    ...
    DRAKE_DEMAND(model_discrete_state_.num_groups() == 0 || model_discrete_state_.num_groups() == xd.num_groups());
...
std::multimap<int, int> LeafSystem<T>::GetDirectFeedthroughs() const {
    ...
    for (const auto& input_output : orig_unknown) {
        ...
        const OutputPortBase& output =
                this->GetOutputPortBaseOrThrow(__func__, input_output.second, /* warn_deprecated = */ false);
        ...
        const InputPortBase& input =
                this->GetInputPortBaseOrThrow(__func__, input_output.first, /* warn_deprecated = */ false);
    ...
    for (const auto& input_output : unknown) {
        if (!inspector || inspector->IsConnectedInputToOutput(input_output.first, input_output.second)) {
...
void LeafSystem<T>::DoCalcNextUpdateTime(const Context<T>& context,
    ...
    using NextEventsVector =
            absl::InlinedVector<const Event<T>*, LeafEventCollection<PublishEvent<T>>::kDefaultCapacity>;
    ...
    auto scan_events = [&context, &min_time](const auto& typed_events, NextEventsVector* event_list) {
        for (const auto* event : typed_events.get_events()) {
            const PeriodicEventData* event_data = event->template get_event_data<PeriodicEventData>();
...
std::unique_ptr<ContinuousState<T>> LeafSystem<T>::AllocateContinuousState() const {
    DRAKE_DEMAND(model_continuous_state_vector_->size() == this->num_continuous_states());
    const SystemBase::ContextSizes& sizes = this->get_context_sizes();
    auto result = std::make_unique<ContinuousState<T>>(
            model_continuous_state_vector_->Clone(), sizes.num_generalized_positions, sizes.num_generalized_velocities,
...
template <typename T>
AbstractStateIndex LeafSystem<T>::DeclareAbstractState(const AbstractValue& model_value) {
...
template <typename T>
void LeafSystem<T>::DeprecateInputPort(const InputPort<T>& port, std::string message) {
    InputPort<T>& mutable_port = const_cast<InputPort<T>&>(this->get_input_port(port.get_index()));
...
LeafOutputPort<T>& LeafSystem<T>::DeclareAbstractOutputPort(std::variant<std::string, UseDefaultName> name,
                                                            ...
                                                            std::set<DependencyTicket> prerequisites_of_calc) {
    auto calc_function = [captured_calc = std::move(calc)](const ContextBase& context_base, AbstractValue* result) {
...
LeafOutputPort<T>& LeafSystem<T>::DeclareStateOutputPort(std::variant<std::string, UseDefaultName> name,
    ...
    return DeclareVectorOutputPort(std::move(name), *model_continuous_state_vector_,
...
template <typename T>
void LeafSystem<T>::DeprecateOutputPort(const OutputPort<T>& port, std::string message) {
    OutputPort<T>& mutable_port = const_cast<OutputPort<T>&>(this->get_output_port(port.get_index()));
    DRAKE_THROW_UNLESS(&mutable_port == &port);
    DRAKE_THROW_UNLESS(mutable_port.get_deprecation() == std::nullopt);
    mutable_port.set_deprecation({std::move(message)});
}
...
template <typename T>
SystemConstraintIndex LeafSystem<T>::DeclareEqualityConstraint(ContextConstraintCalc<T> calc,
                                                               int count,
                                                               std::string description) {
    return DeclareInequalityConstraint(std::move(calc), SystemConstraintBounds::Equality(count),
...
std::unique_ptr<AbstractValue> LeafSystem<T>::DoAllocateInput(const InputPort<T>& input_port) const {
    ...
    throw std::logic_error(
            fmt::format("System::AllocateInputAbstract(): a System with abstract input ports "
                        "must pass a model_value to DeclareAbstractInputPort; the port[{}] "
                        "named '{}' did not do so (System {})",
                        input_port.get_index(), input_port.get_name(), this->GetSystemPathname()));
}
...
template <typename T>
EventStatus LeafSystem<T>::DispatchPublishHandler(const Context<T>& context,
...
template <typename T>
EventStatus LeafSystem<T>::DispatchDiscreteVariableUpdateHandler(const Context<T>& context,
    ...
    for (const DiscreteUpdateEvent<T>* event : leaf_events.get_events()) {
        const EventStatus per_event_status = event->handle(*this, context, discrete_state);
        overall_status.KeepMoreSevere(per_event_status);
...
void LeafSystem<T>::DoApplyDiscreteVariableUpdate(const EventCollection<DiscreteUpdateEvent<T>>& events,
    ...
    DRAKE_ASSERT(dynamic_cast<const LeafEventCollection<DiscreteUpdateEvent<T>>*>(&events) != nullptr);
...
template <typename T>
EventStatus LeafSystem<T>::DispatchUnrestrictedUpdateHandler(const Context<T>& context,
...
void LeafSystem<T>::DoApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                              State<T>* state,
                                              Context<T>* context) const {
    DRAKE_ASSERT(dynamic_cast<const LeafEventCollection<UnrestrictedUpdateEvent<T>>*>(&events) != nullptr);
    DRAKE_DEMAND(events.HasEvents());
    // TODO(sherm1) Should swap rather than copy.
...
// recursion.
template <typename T>
void LeafSystem<T>::DoFindUniquePeriodicDiscreteUpdatesOrThrow(const char* api_name,
                                                               const Context<T>& context,
                                                               std::optional<PeriodicEventData>* timing,
                                                               EventCollection<DiscreteUpdateEvent<T>>* events) const {
    unused(context);
    auto& leaf_collection = dynamic_cast<LeafEventCollection<DiscreteUpdateEvent<T>>&>(*events);

    for (const DiscreteUpdateEvent<T>* event : periodic_events_.get_discrete_update_events().get_events()) {
        DRAKE_DEMAND(event->get_trigger_type() == TriggerType::kPeriodic);
        const PeriodicEventData* const event_timing = event->template get_event_data<PeriodicEventData>();
        DRAKE_DEMAND(event_timing != nullptr);
        if (!timing->has_value())  // First find sets the required timing.
            *timing = *event_timing;
        if (!(*event_timing == *(*timing))) {
            throw std::logic_error(
                    fmt::format("{}(): found more than one "
                                "periodic timing that triggers discrete update events. "
                                "Timings were (offset,period)=({},{}) and ({},{}).",
                                api_name, (*timing)->offset_sec(), (*timing)->period_sec(), event_timing->offset_sec(),
                                event_timing->period_sec()));
        }
...
template <typename T>
void LeafSystem<T>::DoGetPeriodicEvents(const Context<T>&, CompositeEventCollection<T>* events) const {
...
LeafOutputPort<T>& LeafSystem<T>::CreateVectorLeafOutputPort(
    ...
    auto cache_calc_function = [vector_calculator](const ContextBase& context_base, AbstractValue* abstract) {
        ...
        if (value == nullptr) {
            throw std::logic_error(
                    fmt::format("An output port calculation required a {} object for its result "
                                "but got a {} object instead.",
                                NiceTypeName::Get<Value<BasicVector<T>>>(), abstract->GetNiceTypeName()));
        }
...
template <typename T>
LeafOutputPort<T>& LeafSystem<T>::CreateAbstractLeafOutputPort(std::string name,
...
template <typename T>
LeafOutputPort<T>& LeafSystem<T>::CreateCachedLeafOutputPort(std::string name,
                                                             const std::optional<int>& fixed_size,
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::LeafSystem);
