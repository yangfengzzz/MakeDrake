...
::testing::AssertionResult is_autodiffxd_convertible(const S<double>& dut, Callback callback) {
    ...
    std::unique_ptr<System<AutoDiffXd>> converted_67273 = dut.ToAutoDiffXdMaybe();
    ::testing::AssertionResult result_67273 = is_dynamic_castable<S<AutoDiffXd>>(converted_67273);
    if (!result_67273) {
        return result_67273;
    }

    // Allow calling code to specify additional tests on the converted System.
    const S<AutoDiffXd>& downcast_67273 = dynamic_cast<const S<AutoDiffXd>&>(*converted_67273);
...
::testing::AssertionResult is_symbolic_convertible(const S<double>& dut, Callback callback) {
    ...
    // Check if a proper type came out; return early if not.
    std::unique_ptr<System<symbolic::Expression>> converted_67273 = dut.ToSymbolicMaybe();
    ::testing::AssertionResult result_67273 = is_dynamic_castable<S<symbolic::Expression>>(converted_67273);
    if (!result_67273) {
        return result_67273;
    }

    // Allow calling code to specify additional tests on the converted System.
    const S<symbolic::Expression>& downcast_67273 = dynamic_cast<const S<symbolic::Expression>&>(*converted_67273);
    callback(downcast_67273);
