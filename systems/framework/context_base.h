...
protected:
    ...
    bool owns_any_variables_or_parameters() const {
        return !(discrete_state_tickets_.empty() && abstract_state_tickets_.empty() &&
                 numeric_parameter_tickets_.empty() && abstract_parameter_tickets_.empty());
    }
...
private:
    ...
    CacheEntryValue& dummy_cache_entry_value() const final { return get_mutable_cache().dummy_cache_entry_value(); }
...
private:
    ...
    static void AddInputPort(ContextBase* context,
                             InputPortIndex expected_index,
                             DependencyTicket ticket,
    ...
    static void AddOutputPort(ContextBase* context,
                              OutputPortIndex expected_index,
                              DependencyTicket ticket,
                              const internal::OutputPortPrerequisite& prerequisite) {
        DRAKE_DEMAND(context != nullptr);
