...
public:
    ...
    */
    ...
    DependencyTicket input_port_ticket(InputPortIndex index) const {
        ...
        if (input_ports_[index]->get_deprecation().has_value()) WarnPortDeprecation(/* is_input = */ true, index);
...
protected:
    ...
    void AddInputPort(std::unique_ptr<InputPortBase> port) {
        ...
        for (InputPortIndex i{0}; i < num_input_ports(); i++) {
            if (port->get_name() == input_ports_[i]->get_name()) {
                throw std::logic_error("System " + GetSystemName() + " already has an input port named " +
                                       port->get_name());
            }
        }
    ...
    void AddOutputPort(std::unique_ptr<OutputPortBase> port) {
        ...
        for (OutputPortIndex i{0}; i < num_output_ports(); i++) {
            if (port->get_name() == output_ports_[i]->get_name()) {
                throw std::logic_error("System " + GetSystemName() + " already has an output port named " +
                                       port->get_name());
            }
        }
    ...
    static void set_parent_service(SystemBase* child, const internal::SystemParentServiceInterface* parent_service) {
        ...
        DRAKE_DEMAND(child->parent_service_ == nullptr || child->parent_service_ == parent_service);
    ...
    [[noreturn]] void ThrowInputPortHasWrongType(const char* func,
    ...
    /** Throws std::exception because someone called API method `func` claiming
    the input port had some value type that was wrong. */
    [[noreturn]] static void ThrowInputPortHasWrongType(const char* func,
...
private:
    ...
    static std::string GetUnsupportedScalarConversionMessage(const SystemBase&,
                                                             const std::type_info&,
                                                             const std::type_info&);

    static std::vector<std::string> GetGraphvizPortLabels(const SystemBase&, bool input);
};
}  // namespace internal
#endif
