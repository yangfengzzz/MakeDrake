...
#include "systems/framework/system_visitor.h"
...
std::unique_ptr<System<T>> System<T>::Clone() const {
    ...
    if (result == nullptr) {
        throw std::logic_error(fmt::format("System::Clone(): {} system '{}' does not support Cloning",
...
std::unique_ptr<BasicVector<T>> System<T>::AllocateInputVector(const InputPort<T>& input_port) const {
    ...
    const InputPortBase& self_input_port_base =
            this->GetInputPortBaseOrThrow(__func__, input_port.get_index(), /* warn_deprecated = */ false);
...
std::unique_ptr<SystemOutput<T>> System<T>::AllocateOutput() const {
    ...
    for (int i = 0; i < this->num_output_ports(); ++i) {
        const auto& output_port =
                dynamic_cast<const OutputPort<T>&>(this->GetOutputPortBaseOrThrow(__func__, i,
...
void System<T>::AllocateFixedInputs(Context<T>* context) const {
    ...
    for (InputPortIndex i(0); i < num_input_ports(); ++i) {
        const InputPortBase& input_port_base =
                this->GetInputPortBaseOrThrow(__func__, i, /* warn_deprecated = */ false);
...
template <typename T>
EventStatus System<T>::Publish(const Context<T>& context, const EventCollection<PublishEvent<T>>& events) const {
    ValidateContext(context);
...
void System<T>::ForcedPublish(const Context<T>& context) const {
    const EventStatus status = Publish(context, this->get_forced_publish_events());
...
SystemConstraintIndex System<T>::AddExternalConstraint(ExternalSystemConstraint constraint) {
    ...
    if (calc) {
        ...
        constraints_.emplace_back(std::make_unique<SystemConstraint<T>>(
                this, fmt::format("{} (disabled for this scalar type)", constraint.description())));
    }
...
void System<T>::CalcImplicitTimeDerivativesResidual(const Context<T>& context,
    ...
    if (residual->size() != this->implicit_time_derivatives_residual_size()) {
        throw std::logic_error(
                fmt::format("CalcImplicitTimeDerivativesResidual(): expected "
...
template <typename T>
EventStatus System<T>::CalcDiscreteVariableUpdate(const Context<T>& context,
...
template <typename T>
void System<T>::CalcForcedDiscreteVariableUpdate(const Context<T>& context, DiscreteValues<T>* discrete_state) const {
    const EventStatus status =
            CalcDiscreteVariableUpdate(context, this->get_forced_discrete_update_events(), discrete_state);
    status.ThrowOnFailure(__func__);
}

template <typename T>
EventStatus System<T>::CalcUnrestrictedUpdate(const Context<T>& context,
    ...
    const EventStatus status = DispatchUnrestrictedUpdateHandler(context, events, state);
...
template <typename T>
void System<T>::ApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                        State<T>* state,
                                        Context<T>* context) const {
    ValidateContext(context);
    ValidateCreatedForThisSystem(state);
...
template <typename T>
void System<T>::CalcForcedUnrestrictedUpdate(const Context<T>& context, State<T>* state) const {
    const EventStatus status = CalcUnrestrictedUpdate(context, this->get_forced_unrestricted_update_events(), state);
    status.ThrowOnFailure(__func__);
}
...
T System<T>::CalcNextUpdateTime(const Context<T>& context, CompositeEventCollection<T>* events) const {
    ...
    if (isnan(time)) {
        throw std::logic_error(fmt::format(
                ...
                this->GetSystemType(), this->GetSystemPathname(), ExtractDoubleOrThrow(context.get_time())));
    }
...
template <typename T>
const DiscreteValues<T>& System<T>::EvalUniquePeriodicDiscreteUpdate(const Context<T>& context) const {
    const CacheEntry& entry = this->get_cache_entry(unique_periodic_discrete_update_cache_index_);
...
template <typename T>
void System<T>::CalcUniquePeriodicDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* discrete_values) const {
    ValidateContext(context);
    ...
    FindUniquePeriodicDiscreteUpdatesOrThrow(__func__, *this, context, &timing,
    ...
    const EventStatus status =
            this->CalcDiscreteVariableUpdate(context, collection->get_discrete_update_events(), discrete_values);
    status.ThrowOnFailure(__func__);
}

template <typename T>
void System<T>::GetPeriodicEvents(const Context<T>& context, CompositeEventCollection<T>* events) const {
    ValidateContext(context);
    ValidateCreatedForThisSystem(events);
    events->Clear();
...
void System<T>::ExecuteInitializationEvents(Context<T>* context) const {
    ...
    if (init_events->get_unrestricted_update_events().HasEvents()) {
        ...
        const EventStatus status =
                CalcUnrestrictedUpdate(*context, init_events->get_unrestricted_update_events(), state.get());
        status.ThrowOnFailure(__func__);
        ApplyUnrestrictedUpdate(init_events->get_unrestricted_update_events(), state.get(), context);
    }
    ...
    if (init_events->get_discrete_update_events().HasEvents()) {
        ...
        const EventStatus status =
                CalcDiscreteVariableUpdate(*context, init_events->get_discrete_update_events(), discrete_updates.get());
        status.ThrowOnFailure(__func__);
        ApplyDiscreteVariableUpdate(init_events->get_discrete_update_events(), discrete_updates.get(), context);
    }
    // Do any publishes last.
    if (init_events->get_publish_events().HasEvents()) {
        const EventStatus status = Publish(*context, init_events->get_publish_events());
        status.ThrowOnFailure(__func__);
    }
}
...
void System<T>::ExecuteForcedEvents(Context<T>* context, bool publish) const {
    ...
    if (get_forced_unrestricted_update_events().HasEvents()) {
        auto state = context->CloneState();
        const EventStatus status =
                CalcUnrestrictedUpdate(*context, get_forced_unrestricted_update_events(), state.get());
        status.ThrowOnFailure(__func__);
        ApplyUnrestrictedUpdate(get_forced_unrestricted_update_events(), state.get(), context);
    }
    // Do restricted (discrete variable) updates next.
    if (get_forced_discrete_update_events().HasEvents()) {
        auto discrete_updates = AllocateDiscreteVariables();
        const EventStatus status =
                CalcDiscreteVariableUpdate(*context, get_forced_discrete_update_events(), discrete_updates.get());
        status.ThrowOnFailure(__func__);
        ApplyDiscreteVariableUpdate(get_forced_discrete_update_events(), discrete_updates.get(), context);
    }
    // Do any publishes last.
...
const Context<T>& System<T>::GetSubsystemContext(const System<T>& subsystem, const Context<T>& context) const {
    ...
    throw std::logic_error(
            fmt::format("GetSubsystemContext(): {} subsystem '{}' is not "
                        "contained in {} System '{}'.",
                        subsystem.GetSystemType(), subsystem.GetSystemPathname(), this->GetSystemType(),
...
const InputPort<T>& System<T>::GetSoleInputPort() const {
    ...
    if (num_input_ports() == 0) {
        throw std::logic_error(fmt::format("System::get_input_port(): {} system '{}' does not have any inputs",
                                           this->GetSystemType(), this->GetSystemPathname()));
    }
    ...
    for (InputPortIndex i{0}; i < num_input_ports(); i++) {
        const InputPortBase& port_base = this->GetInputPortBaseOrThrow(__func__, i, /* warn_deprecated = */ false);
    ...
    // Too many inputs.
    throw std::logic_error(
            fmt::format("System::get_input_port(): {} system '{}' has {} inputs, so this "
...
const InputPort<T>& System<T>::GetInputPort(const std::string& port_name) const {
    ...
    throw std::logic_error(
            fmt::format("System {} does not have an input port named {} "
                        "(valid port names: {})",
...
const OutputPort<T>& System<T>::GetSoleOutputPort() const {
    ...
    if (num_output_ports() == 0) {
        throw std::logic_error(fmt::format("System::get_output_port(): {} system '{}' does not have any outputs",
                                           this->GetSystemType(), this->GetSystemPathname()));
    }
    ...
    for (OutputPortIndex i{0}; i < num_output_ports(); i++) {
        const OutputPortBase& port_base = this->GetOutputPortBaseOrThrow(__func__, i, /* warn_deprecated = */ false);
        if (port_base.get_deprecation() == std::nullopt) {
            ++num_non_deprecated;
            non_deprecated_index = i;
        }
    ...
    // Too many outputs.
    throw std::logic_error(
            fmt::format("System::get_output_port(): {} system '{}' has {} outputs, so this "
                        "convenience function cannot be used; instead, use another overload "
...
const OutputPort<T>& System<T>::GetOutputPort(const std::string& port_name) const {
    ...
    throw std::logic_error(
            fmt::format("System {} does not have an output port named {} "
                        "(valid port names: {})",
                        GetSystemName(), port_name, fmt::join(port_names, ", ")));
}
...
const SystemConstraint<T>& System<T>::get_constraint(SystemConstraintIndex constraint_index) const {
    if (constraint_index < 0 || constraint_index >= num_constraints()) {
        throw std::out_of_range("System " + get_name() + ": Constraint index " + std::to_string(constraint_index) +
                                " is out of range. There are only " + std::to_string(num_constraints()) +
                                " constraints.");
    }
...
void System<T>::FixInputPortsFrom(const System<double>& other_system,
    ...
    for (int i = 0; i < num_input_ports(); ++i) {
        const InputPortBase& input_port_base =
                this->GetInputPortBaseOrThrow(__func__, i, /* warn_deprecated = */ false);
        const InputPortBase& other_port_base =
                other_system.GetInputPortBaseOrThrow(__func__, i, /* warn_deprecated = */ false);
        const auto& input_port = dynamic_cast<const InputPort<T>&>(input_port_base);
        const auto& other_port = dynamic_cast<const InputPort<double>&>(other_port_base);
...
System<T>::System(SystemScalarConverter converter) : system_scalar_converter_(std::move(converter)) {
    ...
    kinetic_energy_cache_index_ =
            DeclareCacheEntry("kinetic energy", ValueProducer(this, &System<T>::CalcKineticEnergy),
    ...
    unique_periodic_discrete_update_cache_index_ =
            this->DeclareCacheEntryWithKnownTicket(xd_unique_periodic_update_ticket(),
...
template <typename T>
InputPort<T>& System<T>::DeclareInputPort(std::variant<std::string, UseDefaultName> name,
                                          PortDataType type,
                                          int size,
    ...
    auto port = internal::FrameworkFactory::Make<InputPort<T>>(
            this, this, get_system_id(), NextInputPortName(std::move(name)), port_index, port_ticket, type, size,
            random_type, std::move(eval), std::move(alloc));
...
SystemConstraintIndex System<T>::AddConstraint(std::unique_ptr<SystemConstraint<T>> constraint) {
    ...
    if (!external_constraints_.empty()) {
        throw std::logic_error(
                fmt::format("System {} cannot add an internal constraint (named {}) "
                            "after an external constraint (named {}) has already been added",
                            GetSystemName(), constraint->description(), external_constraints_.front().description()));
    }
...
void System<T>::DoCalcImplicitTimeDerivativesResidual(const Context<T>& context,
    ...
    if (residual->size() != proposed_derivatives.size()) {
        throw std::logic_error(
                fmt::format("System::DoCalcImplicitTimeDerivativesResidual(): "
...
template <typename T>
void System<T>::DoGetPeriodicEvents(const Context<T>& context, CompositeEventCollection<T>* events) const {
    unused(context, events);
}
...
template <typename T>
void System<T>::DoMapVelocityToQDot(const Context<T>& context,
                                    const Eigen::Ref<const VectorX<T>>& generalized_velocity,
                                    VectorBase<T>* qdot) const {
    unused(context);
    // In the particular case where generalized velocity and generalized
...
std::function<void(const AbstractValue&)> System<T>::MakeFixInputPortTypeChecker(InputPortIndex port_index) const {
    const InputPortBase& port_base = this->GetInputPortBaseOrThrow(__func__, port_index, /* warn_deprecated = */ false);
            ...
            return [expected_size, port_index, path_name, port_name](const AbstractValue& actual) {
                ...
                if (actual_vector->size() != expected_size) {
                    SystemBase::ThrowInputPortHasWrongType(
                            ...
                            fmt::format("{} with size={}", NiceTypeName::Get<BasicVector<T>>(), expected_size),
                            fmt::format("{} with size={}", NiceTypeName::Get(*actual_vector), actual_vector->size()));
                }
            };
        }
    }
...
template <typename T>
const BasicVector<T>* System<T>::EvalBasicVectorInputImpl(const char* func,
                                                          const Context<T>& context,
                                                          InputPortIndex port_index) const {
    // Make sure this is the right kind of port before worrying about whether
    // it is connected up properly.
    const InputPortBase& port = GetInputPortBaseOrThrow(func, port_index, /* warn_deprecated = */ true);
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::System);
