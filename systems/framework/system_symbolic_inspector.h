...
public:
    ...
    /// state.
    const VectorX<symbolic::Variable>& continuous_state() const { return continuous_state_variables_; }
    ...
    const VectorX<symbolic::Variable>& discrete_state(int i) const {
        DRAKE_DEMAND(i >= 0 && i < static_cast<int>(discrete_state_variables_.size()));
    ...
    /// dynamics.
    VectorX<symbolic::Expression> derivatives() const { return derivatives_->CopyToVector(); }
    ...
    const std::set<symbolic::Formula>& constraints() const { return constraints_; }
    /// @}
