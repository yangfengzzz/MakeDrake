...
#include "systems/framework/abstract_values.h"
...
#include "systems/framework/system_scalar_converter.h"
#include "systems/framework/value_producer.h"
...
protected:
    ...
    void DeclarePerStepPublishEvent(EventStatus (MySystem::*publish)(const Context<T>&) const) {
        ...
        DeclarePerStepEvent<PublishEvent<T>>(
    ...
    @see DeclarePerStepEvent() */
    template <class MySystem>
    void DeclarePerStepDiscreteUpdateEvent(EventStatus (MySystem::*update)(const Context<T>&, DiscreteValues<T>*)
                                                   const) {
        static_assert(std::is_base_of_v<LeafSystem<T>, MySystem>,
                      "Expected to be invoked from a LeafSystem-derived System.");
        DRAKE_DEMAND(update != nullptr);

        DeclarePerStepEvent(DiscreteUpdateEvent<T>(TriggerType::kPerStep,
                                                   [update](const System<T>& system, const Context<T>& context,
                                                            const DiscreteUpdateEvent<T>&, DiscreteValues<T>* xd) {
                                                       const auto& sys = dynamic_cast<const MySystem&>(system);
                                                       return (sys.*update)(context, &*xd);
    ...
    void DeclarePerStepUnrestrictedUpdateEvent(EventStatus (MySystem::*update)(const Context<T>&, State<T>*) const) {
        ...
        DeclarePerStepEvent(UnrestrictedUpdateEvent<T>(TriggerType::kPerStep,
                                                       [update](const System<T>& system, const Context<T>& context,
                                                                const UnrestrictedUpdateEvent<T>&, State<T>* x) {
                                                           const auto& sys = dynamic_cast<const MySystem&>(system);
                                                           return (sys.*update)(context, &*x);
    ...
    void DeclareInitializationPublishEvent(EventStatus (MySystem::*publish)(const Context<T>&) const) {
        ...
        DeclareInitializationEvent<PublishEvent<T>>(
    ...
    void DeclareInitializationDiscreteUpdateEvent(EventStatus (MySystem::*update)(const Context<T>&, DiscreteValues<T>*)
        ...
        DeclareInitializationEvent(DiscreteUpdateEvent<T>(
                TriggerType::kInitialization, [update](const System<T>& system, const Context<T>& context,
                                                       const DiscreteUpdateEvent<T>&, DiscreteValues<T>* xd) {
                    const auto& sys = dynamic_cast<const MySystem&>(system);
                    return (sys.*update)(context, &*xd);
                }));
    ...
    @pre `update` must not be null. */
    template <class MySystem>
    void DeclareForcedDiscreteUpdateEvent(EventStatus (MySystem::*update)(const Context<T>&, DiscreteValues<T>*)
                                                  const) {
        static_assert(std::is_base_of_v<LeafSystem<T>, MySystem>,
                      "Expected to be invoked from a LeafSystem-derived System.");
        auto this_ptr = dynamic_cast<const MySystem*>(this);
        ...
        DiscreteUpdateEvent<T> forced(TriggerType::kForced,
                                      [update](const System<T>& system, const Context<T>& context,
                                               const DiscreteUpdateEvent<T>&, DiscreteValues<T>* discrete_state) {
                                          const auto& sys = dynamic_cast<const MySystem&>(system);
    ...
    void DeclareForcedUnrestrictedUpdateEvent(EventStatus (MySystem::*update)(const Context<T>&, State<T>*) const) {
        ...
        // Instantiate the event.
        UnrestrictedUpdateEvent<T> forced(TriggerType::kForced,
                                          [update](const System<T>& system, const Context<T>& context,
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessFunction(const std::string& description,
        ...
        static_assert(std::is_base_of_v<LeafSystem<T>, MySystem>,
                      "Expected to be invoked from a LeafSystem-derived system.");
        PublishEvent<T> event(TriggerType::kWitness,
        ...
        return std::make_unique<WitnessFunction<T>>(this, this, description, direction_type, calc, event.Clone());
    }
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessFunction(const std::string& description,
        ...
        DiscreteUpdateEvent<T> event(
                ...
                [du_callback](const System<T>& system, const Context<T>& context,
                              const DiscreteUpdateEvent<T>& callback_event, DiscreteValues<T>* values) {
                    const auto& sys = dynamic_cast<const MySystem&>(system);
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessFunction(
            ...
            const WitnessFunctionDirection& direction_type,
            T (MySystem::*calc)(const Context<T>&) const,
            void (MySystem::*uu_callback)(const Context<T>&, const UnrestrictedUpdateEvent<T>&, State<T>*)
                    const) const {
        static_assert(std::is_base_of_v<LeafSystem<T>, MySystem>,
                      "Expected to be invoked from a LeafSystem-derived system.");
        UnrestrictedUpdateEvent<T> event(
                ...
                [uu_callback](const System<T>& system, const Context<T>& context,
                              const UnrestrictedUpdateEvent<T>& callback_event, State<T>* state) {
                    const auto& sys = dynamic_cast<const MySystem&>(system);
...
private:
    ...
    [[nodiscard]] EventStatus DispatchPublishHandler(const Context<T>& context,
    ...
    void DoGetPeriodicEvents(const Context<T>& context, CompositeEventCollection<T>* events) const final;

    void DoGetPerStepEvents(const Context<T>& context, CompositeEventCollection<T>* events) const final;

    void DoGetInitializationEvents(const Context<T>& context, CompositeEventCollection<T>* events) const final;

    // Creates a new cached, vector-valued LeafOutputPort in this LeafSystem and
    // returns a reference to it.
    LeafOutputPort<T>& CreateVectorLeafOutputPort(std::string name,
                                                  int fixed_size,
    ...
    // returns a reference to it.
    LeafOutputPort<T>& CreateAbstractLeafOutputPort(std::string name,
    ...
    LeafOutputPort<T>& CreateCachedLeafOutputPort(std::string name,
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::LeafSystem);
