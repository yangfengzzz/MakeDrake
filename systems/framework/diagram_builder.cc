...
void DiagramBuilder<T>::RemoveSystem(const System<T>& system) {
    ...
    if (!systems_.contains(&system)) {
        throw std::logic_error(
                fmt::format("Cannot RemoveSystem on {} because it has not been added to this "
                            "DiagramBuilder",
                            system.GetSystemPathname()));
    }
    const size_t system_index = std::distance(registered_systems_.begin(),
    ...
    for (size_t i = 0; i < input_port_ids_.size();) {
        ...
        if (locator.first == &system) {
            ...
            disconnected_diagram_input_port_names.insert(std::move(input_port_names_[i]));
    ...
    for (const auto& name : disconnected_diagram_input_port_names) {
        const bool num_connections = std::count(input_port_names_.begin(), input_port_names_.end(), name);
...
template <typename T>
const System<T>& DiagramBuilder<T>::GetSubsystemByName(std::string_view name) const {
    ThrowIfAlreadyBuilt();
    ...
    for (const auto& child : registered_systems_) {
        if (child->get_name() == name) {
            if (result != nullptr) {
                throw std::logic_error(
                        fmt::format("DiagramBuilder contains multiple subsystems named {} so cannot "
    ...
    throw std::logic_error(fmt::format("DiagramBuilder does not contain a subsystem named {}", name));
}
...
template <typename T>
const std::map<typename DiagramBuilder<T>::InputPortLocator, typename DiagramBuilder<T>::OutputPortLocator>&
...
void DiagramBuilder<T>::Connect(const OutputPort<T>& src, const InputPort<T>& dest) {
    ...
    if (src.get_data_type() != dest.get_data_type()) {
        throw std::logic_error(fmt::format(
                ...
                src.get_name(), src.get_system().get_name(), dest.get_name(), dest.get_system().get_name()));
    }
    if ((src.get_data_type() != kAbstractValued) && (src.size() != dest.size())) {
        throw std::logic_error(
                fmt::format("DiagramBuilder::Connect: Mismatched vector sizes while connecting "
                            "output port {} of System {} (size {}) to "
                            "input port {} of System {} (size {})",
                            src.get_name(), src.get_system().get_name(), src.size(), dest.get_name(),
                            dest.get_system().get_name(), dest.size()));
    }
    if (src.get_data_type() == kAbstractValued) {
        ...
        if (output_type != input_type) {
            throw std::logic_error(
                    fmt::format("DiagramBuilder::Connect: Mismatched value types while connecting "
                                "output port {} of System {} (type {}) to "
                                "input port {} of System {} (type {})",
                                src.get_name(), src.get_system().get_name(), NiceTypeName::Get(output_type),
                                dest.get_name(), dest.get_system().get_name(), NiceTypeName::Get(input_type)));
        }
    }
...
InputPortIndex DiagramBuilder<T>::DeclareInput(const InputPort<T>& input,
    ...
    if (diagram_input_indices_.contains(port_name)) {
        throw std::logic_error(fmt::format("Diagram already has an input port named {}", port_name));
    }
...
template <typename T>
void DiagramBuilder<T>::ConnectInput(std::string_view diagram_port_name, const InputPort<T>& input) {
    ThrowIfAlreadyBuilt();
    DRAKE_THROW_UNLESS(diagram_input_indices_.count(diagram_port_name));
    const InputPortIndex diagram_port_index = diagram_input_indices_.find(diagram_port_name)->second;
    ConnectInput(diagram_port_index, input);
}
...
void DiagramBuilder<T>::ConnectInput(InputPortIndex diagram_port_index, const InputPort<T>& input) {
    ...
    ThrowIfSystemNotRegistered(&input.get_system());
    DRAKE_THROW_UNLESS(diagram_port_index < InputPortIndex(diagram_input_data_.size()));
    ...
    if (model.get_data_type() != input.get_data_type()) {
        throw std::logic_error(
                fmt::format("DiagramBuilder::ConnectInput: Cannot mix vector-valued and abstract-"
    ...
    if ((model.get_data_type() != kAbstractValued) && (model.size() != input.size())) {
        throw std::logic_error(
                fmt::format("DiagramBuilder::ConnectInput: Mismatched vector sizes while "
                            "connecting input port {} of System {} (size {}) to "
                            "input port {} of Diagram (size {})",
                            input.get_name(), input.get_system().get_name(), input.size(), port_name, model.size()));
    }
    if (model.get_data_type() == kAbstractValued) {
        ...
        if (model_type != input_type) {
            throw std::logic_error(
                    fmt::format("DiagramBuilder::ConnectInput: Mismatched value types while "
                                "connecting input port {} of System {} (type {}) to "
                                "input port {} of Diagram (type {})",
                                input.get_name(), input.get_system().get_name(), NiceTypeName::Get(input_type),
...
template <typename T>
bool DiagramBuilder<T>::ConnectToSame(const InputPort<T>& exemplar, const InputPort<T>& dest) {
    ThrowIfAlreadyBuilt();
    ...
    if (iter != connection_map_.end()) {
        ...
        const OutputPort<T>& exemplar_port = exemplar_loc.first->get_output_port(exemplar_loc.second);
...
bool DiagramBuilder<T>::IsConnectedOrExported(const InputPort<T>& port) const {
    ...
    if (this->connection_map_.contains(id) || this->diagram_input_set_.contains(id)) {
        return true;
    }
    return false;
}
...
void DiagramBuilder<T>::ThrowIfInputAlreadyWired(const InputPortLocator& id) const {
    if (connection_map_.find(id) != connection_map_.end() || diagram_input_set_.find(id) != diagram_input_set_.end()) {
        ...
        throw std::logic_error(fmt::format("Input port {} is already connected.", input_port_names_[index]));
    }
}
...
void DiagramBuilder<T>::ThrowIfSystemNotRegistered(const System<T>* system) const {
    ...
    if (!systems_.contains(system)) {
        ...
        throw std::logic_error(
                fmt::format("DiagramBuilder: System '{}' has not been registered to this "
...
bool HasCycleRecurse(const PortIdentifier& n,
    ...
    if (edge_iter != edges.end()) {
        ...
        for (const auto& target : edge_iter->second) {
            if (!visited->contains(target) && HasCycleRecurse(target, edges, visited, stack)) {
                return true;
...
void DiagramBuilder<T>::ThrowIfAlgebraicLoopsExist() const {
    ...
    static constexpr char kAdvice[] =
            ...
            "classdrake_1_1systems_1_1_leaf_system.html#DeclareLeafOutputPort_feedthrough";
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::DiagramBuilder);
