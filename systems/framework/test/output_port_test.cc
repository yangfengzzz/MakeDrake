...
public:
    MyOutputPort(DummySystem* dummy, OutputPortIndex index, DependencyTicket ticket)
        : OutputPort<double>(dummy, dummy, dummy->get_system_id(), "my_output", index, ticket, kVectorValued, 2) {}

    std::unique_ptr<AbstractValue> DoAllocate() const override {
        return AbstractValue::Make<BasicVector<double>>(MyVector2d(Vector2d(1., 2.)));
    }

    void DoCalc(const Context<double>&, AbstractValue* value) const override {
        EXPECT_NE(value, nullptr);
        DRAKE_EXPECT_NO_THROW(value->set_value<BasicVector<double>>(MyVector2d(Vector2d(3., 4.))));
    }
    ...
    void ThrowIfInvalidPortValueType(const Context<double>&, const AbstractValue& proposed_value) const final {
        ...
        if (proposed_value.type_info() != good_value->type_info()) {
            throw std::logic_error(fmt::format(
                    ...
                    "but got {} for {}.",
                    good_value->GetNiceTypeName(), proposed_value.GetNiceTypeName(), PortBase::GetFullDescription()));
        }
    }
};
...
public:
    ...
    std::unique_ptr<AbstractValue> DoAllocate() const override { return AbstractValue::Make<std::string>("hello"); }
};
...
public:
    ...
    std::unique_ptr<AbstractValue> DoAllocate() const override {
        return AbstractValue::Make<BasicVector<double>>(MyVector3d(Vector3d(1., 2., 3.)));
    }
};
...
GTEST_TEST(TestBaseClass, BadAllocators) {
    DummySystem dummy;
    MyStringAllocatorPort string_allocator{&dummy, OutputPortIndex(0), dummy.assign_next_dependency_ticket()};
    MyBadSizeAllocatorPort bad_size_allocator{&dummy, OutputPortIndex(1), dummy.assign_next_dependency_ticket()};
    MyNullAllocatorPort null_allocator{&dummy, OutputPortIndex(2), dummy.assign_next_dependency_ticket()};

    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(string_allocator.Allocate(),
                                         "OutputPort::Allocate().*expected BasicVector.*but got.*std::string"
                                         ".*OutputPort\\[0\\].*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(bad_size_allocator.Allocate(),
                                         "OutputPort::Allocate().*expected vector.*size 2.*but got.*size 3"
                                         ".*OutputPort\\[1\\].*");

    // Nullptr check is unconditional.
    DRAKE_EXPECT_THROWS_MESSAGE(null_allocator.Allocate(), "OutputPort::Allocate().*nullptr.*OutputPort\\[2\\].*");
}

// This message can be caused by user action.
GTEST_TEST(TestBaseClass, BadOutputType) {
    DummySystem dummy;
    MyOutputPort port{&dummy, OutputPortIndex(0), dummy.assign_next_dependency_ticket()};
    ...
    if (kDrakeAssertIsArmed) {
        DRAKE_EXPECT_THROWS_MESSAGE(port.Calc(*context, bad_port_value.get()),
...
protected:
    ...
    std::unique_ptr<LeafOutputPort<double>> absport_general_ptr_ =
            internal::FrameworkFactory::Make<LeafOutputPort<double>>(
                    ...
                    kAbstractValued,
                    0 /* size */,
                    &dummy_.DeclareCacheEntry("absport", ValueProducer(alloc_string, calc_string)));
    ...
    std::unique_ptr<LeafOutputPort<double>> vecport_general_ptr_ =
            internal::FrameworkFactory::Make<LeafOutputPort<double>>(
                    ...
                    kVectorValued,
                    3 /* size */,
                    &dummy_.DeclareCacheEntry("vecport", ValueProducer(alloc_myvector3, calc_vector3)));
...
void AbstractPortCheck(const Context<double>& context, const LeafOutputPort<double>& port, string alloc_string) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(port.Eval<int>(context),
...
TEST_F(LeafOutputPortTest, DisableByDefaultWorks) {
    ...
    const CacheEntryValue& abs_value = my_context->get_cache().get_cache_entry_value(abs_entry.cache_index());
    const CacheEntryValue& vec_value = my_context->get_cache().get_cache_entry_value(vec_entry.cache_index());
...
void VectorPortCheck(const Context<double>& context, const LeafOutputPort<double>& port, Vector3d alloc_value) {
    ...
    const BasicVector<double>& eval_basic = port.Eval<BasicVector<double>>(context);
    ...
    EXPECT_EQ(eval_abs.get_value<BasicVector<double>>().CopyToVector(), new_value);
}
...
TEST_F(LeafOutputPortTest, ThrowIfNullAlloc) {
    ...
    auto null_port = internal::FrameworkFactory::Make<LeafOutputPort<double>>(
            &dummy_,  // implicit_cast<const System<T>*>(&dummy_)
            &dummy_,  // implicit_cast<SystemBase*>(&dummy_),
            dummy_.get_system_id(), "null_port", OutputPortIndex(dummy_.num_output_ports()),
            dummy_.assign_next_dependency_ticket(), kAbstractValued, 0 /* size */,
            &dummy_.DeclareCacheEntry("null", ValueProducer(alloc_null, calc_string)));
...
TEST_F(LeafOutputPortTest, ThrowIfBadCalcOutput) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(absport_general_.Calc(*context_, bad_out.get()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(vecport_general_.Calc(*context_, bad_vec.get()),
...
public:
    explicit SystemWithNStates(int num_states) {
        ...
        DeclareAbstractOutputPort(kUseDefaultName, &SystemWithNStates::ReturnNumContinuous);
    }
...
GTEST_TEST(DiagramOutputPortTest, OneLevel) {
    ...
    const auto& sys1_context = diagram_context.GetSubsystemContext(SubsystemIndex{0});
    DRAKE_EXPECT_THROWS_MESSAGE(out0.Eval<int>(sys1_context), ".*Context.*was not created for this OutputPort.*");
}
