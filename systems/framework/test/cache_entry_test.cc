#include "systems/framework/cache_entry.h"
...
private:
    std::unique_ptr<ContextBase> DoCloneWithoutPointers() const final { return std::make_unique<MyContextBase>(*this); }
};
...
public:
    ...
    MySystemBase() {
        ...
        entry1_ = &DeclareCacheEntry("entry1", ValueProducer(Alloc1, Calc99), {entry0_->ticket()});

        // Use the overload that takes a model value and member calculator.
        entry2_ = &DeclareCacheEntry("entry2", 2, &MySystemBase::CalcInt98, {entry0_->ticket(), entry1_->ticket()});

        // Use the overload that takes just a member calculator. The model value
        // will be value-initialized (using an `int{}` for this one).
        entry3_ = &DeclareCacheEntry("entry3", &MySystemBase::CalcInt98, {entry0_->ticket(), entry1_->ticket()});

        // Ditto but using a string{} defaulted model value.
        string_entry_ = &DeclareCacheEntry("string thing", &MySystemBase::CalcString, {time_ticket()});

        // Ditto but using a MyVector3d{} defaulted model value.
        vector_entry_ = &DeclareCacheEntry("vector thing", MyVector3d(Vector3d(1., 2., 3.)),
    ...
    void CalcString(const MyContextBase& my_context, string* out) const { *out = "calculated_result"; }
    void CalcMyVector3(const MyContextBase& my_context, MyVector3d* out) const { out->set_value(Vector3d(3., 2., 1.)); }
...
private:
    ...
    std::function<void(const AbstractValue&)> MakeFixInputPortTypeChecker(InputPortIndex /* unused */) const final {
        return {};
    }
...
GTEST_TEST(CacheEntryAllocTest, BadAllocGetsCaught) {
    MySystemBase system;
    system.DeclareCacheEntry("bad alloc entry", ValueProducer(AllocNull, Calc99), {system.nothing_ticket()});
...
GTEST_TEST(CacheEntryAllocTest, EmptyPrerequisiteListForbidden) {
    ...
    DRAKE_EXPECT_NO_THROW(system.DeclareCacheEntry("default prerequisites", alloc3_calc99));
    DRAKE_EXPECT_NO_THROW(system.DeclareCacheEntry("no prerequisites", alloc3_calc99, {system.nothing_ticket()}));
    DRAKE_EXPECT_THROWS_MESSAGE(system.DeclareCacheEntry("empty prerequisites", alloc3_calc99, {}),
...
GTEST_TEST(CacheEntryAllocTest, DetectsDefaultPrerequisites) {
    MySystemBase system;
    const ValueProducer alloc3_calc99(Alloc3, Calc99);
    const CacheEntry& default_prereqs = system.DeclareCacheEntry("default prerequisites", alloc3_calc99);
    ...
    const CacheEntry& explicit_default_prereqs =
            system.DeclareCacheEntry("explicit default prerequisites", alloc3_calc99, {system.all_sources_ticket()});
    EXPECT_TRUE(explicit_default_prereqs.has_default_prerequisites());  // Not good.

    // This specifies exactly the same dependencies as all_sources_ticket() but
    // in a way that is clearly non-default.
    const CacheEntry& long_form_all_sources_prereqs =
            system.DeclareCacheEntry("long form all sources prerequisites", alloc3_calc99,
                                     {system.all_sources_except_input_ports_ticket(), system.all_input_ports_ticket()});
    EXPECT_FALSE(long_form_all_sources_prereqs.has_default_prerequisites());  // Good!

    const CacheEntry& no_prereqs =
            system.DeclareCacheEntry("no prerequisites", alloc3_calc99, {system.nothing_ticket()});
    EXPECT_FALSE(no_prereqs.has_default_prerequisites());

    const CacheEntry& time_only_prereq =
            system.DeclareCacheEntry("time only prerequisite", alloc3_calc99, {system.time_ticket()});
...
protected:
    void SetUp() override {
        ...
        EXPECT_EQ(entry0_tracker.prerequisites()[0], &context_.get_tracker(system_.all_sources_ticket()));
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(cache_value(string_index_).SetValueOrThrow<string>("initial"),
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(vector_entry().GetKnownUpToDate<MyVector3d>(context_),
                                    ".*GetKnownUpToDate().*value out of date.*");
        EXPECT_EQ(cache_value(vector_index_).PeekValueOrThrow<MyVector3d>().get_value(), Vector3d(1., 2., 3.));
    ...
    const DependencyTracker& tracker(CacheIndex index) { return tracker(cache_value(index).ticket()); }
    ...
    const DependencyGraph& graph() { return context_.get_mutable_dependency_graph(); }
    const DependencyTracker& tracker(DependencyTicket dticket) { return graph().get_tracker(dticket); }
...
TEST_F(CacheEntryTest, ValueMethodsWork) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(entry0().GetKnownUpToDate<int>(context_), ".*GetKnownUpToDate().*value out of date.*");
    ...
    const AbstractValue& abstract_value_get = entry0().GetKnownUpToDateAbstract(context_);
    EXPECT_EQ(&abstract_value_get, &abstract_value_eval);
    EXPECT_EQ(value0.serial_number(), expected_serial_num);  // No change.

    CacheEntryValue& string_value = string_entry().get_mutable_cache_entry_value(context_);
    ...
    if (kDrakeAssertIsArmed) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(string_entry().Calc(context_, &*bad_out),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(string_entry().GetKnownUpToDateAbstract(context_),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(string_entry().GetKnownUpToDate<int>(context_),
...
TEST_F(CacheEntryTest, ModifyPrerequisites) {
    const std::set<DependencyTicket>& string_prereqs = string_entry().prerequisites();
    EXPECT_EQ(string_prereqs.size(), 1);  // Just time_ticket.
    EXPECT_FALSE(string_entry().is_out_of_date(context_));
    const DependencyTracker& entry1_tracker = context_.get_tracker(entry1().ticket());
    const DependencyTracker& entry2_tracker = context_.get_tracker(entry2().ticket());
    ...
    CacheEntry& mutable_string_entry = system_.get_mutable_cache_entry(string_entry().cache_index());
    ...
    string_entry().get_mutable_cache_entry_value(*new_context).SetValueOrThrow<std::string>("something");
    EXPECT_FALSE(string_entry().is_out_of_date(*new_context));

    const DependencyTracker& new_entry1_tracker = new_context->get_tracker(entry1().ticket());
    const DependencyTracker& new_entry2_tracker = new_context->get_tracker(entry2().ticket());
    ...
    string_entry().get_mutable_cache_entry_value(*new_context).SetValueOrThrow<std::string>("something else");
    const DependencyTracker& new_time_tracker = new_context->get_tracker(system_.time_ticket());
    EXPECT_FALSE(string_entry().is_out_of_date(*new_context));
...
TEST_F(CacheEntryTest, DisableCacheWorks) {
    ...
    CacheEntryValue& str_val = string_entry().get_mutable_cache_entry_value(context_);
    CacheEntryValue& vec_val = vector_entry().get_mutable_cache_entry_value(context_);
...
TEST_F(CacheEntryTest, CanSwapValue) {
    ...
    if (kDrakeAssertIsArmed) {
        ...
        DRAKE_EXPECT_THROWS_MESSAGE(entry_value.swap_value(&empty_ptr), ".*swap_value().*value.*empty.*");
        auto bad_value = AbstractValue::Make<int>(29);
        DRAKE_EXPECT_THROWS_MESSAGE(entry_value.swap_value(&bad_value),
...
TEST_F(CacheEntryTest, Copy) {
    ...
    MyContextBase& clone_context = dynamic_cast<MyContextBase&>(*clone_context_ptr);

    // The copy should have the same values.
    EXPECT_EQ(entry0().GetKnownUpToDate<int>(context_), entry0().GetKnownUpToDate<int>(clone_context));
    EXPECT_EQ(entry1().GetKnownUpToDate<int>(context_), entry1().GetKnownUpToDate<int>(clone_context));
    EXPECT_EQ(entry2().GetKnownUpToDate<int>(context_), entry2().GetKnownUpToDate<int>(clone_context));
    EXPECT_EQ(entry3().GetKnownUpToDate<int>(context_), entry3().GetKnownUpToDate<int>(clone_context));
    EXPECT_EQ(string_entry().GetKnownUpToDate<string>(context_),
              string_entry().GetKnownUpToDate<string>(clone_context));
    EXPECT_EQ(vector_entry().GetKnownUpToDate<MyVector3d>(context_).get_value(),
