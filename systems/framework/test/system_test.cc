...
public:
    ...
    explicit TestSystemBase(int num_numeric_params_groups = 0, int num_continuous_states = 0)
    ...
    void AddTriggeredWitnessFunctionToCompositeEventCollection(Event<T>*, CompositeEventCollection<T>*) const final {
    ...
    std::unique_ptr<EventCollection<DiscreteUpdateEvent<T>>> AllocateForcedDiscreteUpdateEventCollection() const final {
        return LeafEventCollection<DiscreteUpdateEvent<T>>::MakeForcedEventCollection();
    }

    std::unique_ptr<EventCollection<UnrestrictedUpdateEvent<T>>> AllocateForcedUnrestrictedUpdateEventCollection()
            const final {
        return LeafEventCollection<UnrestrictedUpdateEvent<T>>::MakeForcedEventCollection();
    }
...
private:
    std::unique_ptr<ContextBase> DoAllocateContext() const final {
        ...
        if (num_numeric_param_groups_ > 0) {
            ...
            for (int g = 0; g < num_numeric_param_groups_; ++g) {
                numeric_params.emplace_back(std::make_unique<BasicVector<T>>(std::initializer_list<T>{0.0}));
            }
            auto parameters = std::make_unique<Parameters<T>>(std::move(numeric_params));
            parameters->set_system_id(this->get_system_id());
            context->init_parameters(std::move(parameters));
            DRAKE_DEMAND(context->get_parameters().num_numeric_parameter_groups() == num_numeric_param_groups_);
        }

        // Allocate requested continuous state variables.
        auto state = std::make_unique<ContinuousState<T>>(std::make_unique<BasicVector<T>>(num_continuous_states_));
    ...
    T DoCalcWitnessValue(const Context<T>&, const WitnessFunction<T>&) const final {
        ADD_FAILURE() << "A test called a method that was expected to be unused.";
        return {};
    }
    ...
    EventStatus DispatchUnrestrictedUpdateHandler(const Context<T>&,
    ...
    void DoApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                   State<T>* state,
                                   Context<T>* context) const final {
        ADD_FAILURE() << "A test called a method that was expected to be unused.";
    }

    std::map<PeriodicEventData, std::vector<const Event<T>*>, PeriodicEventDataComparator> DoMapPeriodicEventsByTiming(
            const Context<T>&) const final {
        ADD_FAILURE() << "A test called a method that was expected to be unused.";
        return {};
    }

    std::unique_ptr<AbstractValue> DoAllocateInput(const InputPort<T>&) const override {
        ADD_FAILURE() << "A test called a method that was expected to be unused.";
        return {};
    }
    ...
    void DoFindUniquePeriodicDiscreteUpdatesOrThrow(const char* api_name,
    ...
    EventStatus DispatchPublishHandler(const Context<T>& context,
                                       const EventCollection<PublishEvent<T>>& event_info) const override {
        ADD_FAILURE() << "A test called a method that was expected to be unused.";
        return EventStatus::DidNothing();
    }

    EventStatus DispatchDiscreteVariableUpdateHandler(const Context<T>& context,
...
public:
    TestSystem() {
        this->set_forced_publish_events(this->AllocateForcedPublishEventCollection());
        this->set_forced_discrete_update_events(this->AllocateForcedDiscreteUpdateEventCollection());
        this->set_forced_unrestricted_update_events(this->AllocateForcedUnrestrictedUpdateEventCollection());
    ...
    LeafOutputPort<double>& AddAbstractOutputPort() {
        ...
        CacheEntry& cache_entry =
                this->DeclareCacheEntry("null output port", ValueProducer(0, &ValueProducer::NoopCalc));
        ...
        auto port = internal::FrameworkFactory::Make<LeafOutputPort<double>>(
                ...
                OutputPortIndex(this->num_output_ports()), assign_next_dependency_ticket(), kAbstractValued, 0,
                &cache_entry);
    ...
    const std::vector<int>& get_published_numbers() const { return published_numbers_; }
    const std::vector<int>& get_updated_numbers() const { return updated_numbers_; }

    // The default publish function.
    void MyPublish(const Context<double>& context, const std::vector<const PublishEvent<double>*>& events) const {
        ++publish_count_;
    }

protected:
    EventStatus DispatchPublishHandler(const Context<double>& context,
    ...
    EventStatus DispatchDiscreteVariableUpdateHandler(const Context<double>& context,
        ...
        const LeafEventCollection<DiscreteUpdateEvent<double>>& leaf_events =
                dynamic_cast<const LeafEventCollection<DiscreteUpdateEvent<double>>&>(events);
        if (leaf_events.HasEvents()) {
            this->MyCalcDiscreteVariableUpdates(context, leaf_events.get_events(), discrete_state);
            return EventStatus::Succeeded();
        }
        return EventStatus::DidNothing();
    }
    ...
    void MyCalcDiscreteVariableUpdates(const Context<double>& context,
...
TEST_F(SystemTest, ContextBelongsWithSystem) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(system2.ForcedPublish(*context_), "[^]*#framework-context-system-mismatch.*");
    DRAKE_EXPECT_THROWS_MESSAGE(system2.SetDefaultContext(context_.get()), "[^]*#framework-context-system-mismatch.*");
}
...
TEST_F(SystemTest, ConfigurationDerivativeVelocitySizeMismatch) {
    ...
    EXPECT_THROW(system_.MapQDotToVelocity(*context_, *state_vec1, &state_vec2), std::runtime_error);
}
...
TEST_F(SystemTest, VelocityConfigurationDerivativeSizeMismatch) {
    ...
    EXPECT_THROW(system_.MapVelocityToQDot(*context_, *state_vec1, &state_vec2), std::runtime_error);
}
...
TEST_F(SystemTest, DiscretePublish) {
    ...
    const auto& events = dynamic_cast<const LeafCompositeEventCollection<double>*>(event_info.get())
                                 ->get_publish_events()
                                 .get_events();
    EXPECT_EQ(events.size(), 1);
    EXPECT_EQ(events.front()->get_trigger_type(), TriggerType::kPeriodic);

    const EventStatus status = system_.Publish(*context_, event_info->get_publish_events());
...
TEST_F(SystemTest, DiscreteUpdate) {
    ...
    std::unique_ptr<DiscreteValues<double>> update = system_.AllocateDiscreteVariables();
    const EventStatus status =
            system_.CalcDiscreteVariableUpdate(*context_, event_info->get_discrete_update_events(), update.get());
    EXPECT_TRUE(status.succeeded());
...
TEST_F(SystemTest, ExactlyOneInputPortConvenience) {
    // No ports: fail.
    DRAKE_EXPECT_THROWS_MESSAGE(system_.get_input_port(), ".*does not have any inputs");
    ...
    const_cast<InputPort<double>&>(system_.get_input_port(0)).set_deprecation("deprecated");
    EXPECT_EQ(&system_.get_input_port(), &system_.get_input_port(0));
    ...
    const_cast<InputPort<double>&>(system_.get_input_port(1)).set_deprecation("deprecated");
    EXPECT_EQ(&system_.get_input_port(), &system_.get_input_port(2));

    // Three deprecated ports: fail
    const_cast<InputPort<double>&>(system_.get_input_port(2)).set_deprecation("deprecated");
    DRAKE_EXPECT_THROWS_MESSAGE(system_.get_input_port(), ".*has 3 inputs.*");
}
...
TEST_F(SystemTest, ExactlyOneOutputPortConvenience) {
    // No ports: fail.
    DRAKE_EXPECT_THROWS_MESSAGE(system_.get_output_port(), ".*does not have any outputs");
    ...
    // One deprecated port: pass.
    const_cast<OutputPort<double>&>(system_.get_output_port(0)).set_deprecation("deprecated");
    EXPECT_EQ(&system_.get_output_port(), &system_.get_output_port(0));
    ...
    // Three ports (one non-deprecated): pass.
    const_cast<OutputPort<double>&>(system_.get_output_port(1)).set_deprecation("deprecated");
    EXPECT_EQ(&system_.get_output_port(), &system_.get_output_port(2));

    // Three deprecated ports: fail.
    const_cast<OutputPort<double>&>(system_.get_output_port(2)).set_deprecation("deprecated");
    DRAKE_EXPECT_THROWS_MESSAGE(system_.get_output_port(), ".*has 3 outputs.*");
}

TEST_F(SystemTest, PortNameTest) {
    const auto& unnamed_input = system_.DeclareInputPort(kUseDefaultName, kVectorValued, 2);
    const auto& named_input = system_.DeclareInputPort("my_input", kVectorValued, 3);
    const auto& named_abstract_input = system_.DeclareInputPort("abstract", kAbstractValued, 0);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(system_.DeclareInputPort("my_input", kAbstractValued, 0),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetInputPort("not_my_input"), ".*does not have an input port named.*");
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetOutputPort("not_my_output"), ".*does not have an output port named.*");

    // Error messages should mention valid ports.
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetInputPort("not_my_input"), ".*valid port names: .*u0.*");
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetOutputPort("not_my_output"), ".*valid port names: .*y0.*");
}

TEST_F(SystemTest, PortNameNoPortsTest) {
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetInputPort("not_my_input"), ".* it has no input ports.*");
    DRAKE_EXPECT_THROWS_MESSAGE(system_.GetOutputPort("not_my_output"), ".* it has no output ports.*");
}

TEST_F(SystemTest, PortSelectionTest) {
    // Input ports.
    EXPECT_EQ(system_.get_input_port_selection(InputPortSelection::kNoInput), nullptr);
    EXPECT_EQ(system_.get_input_port_selection(InputPortSelection::kUseFirstInputIfItExists), nullptr);

    const auto& input_port = system_.DeclareInputPort("my_input", kVectorValued, 3);
    EXPECT_EQ(system_.get_input_port_selection(InputPortSelection::kUseFirstInputIfItExists), &input_port);

    EXPECT_EQ(system_.get_input_port_selection(InputPortIndex{0}), &system_.get_input_port(0));

    // Output ports.
    EXPECT_EQ(system_.get_output_port_selection(OutputPortSelection::kNoOutput), nullptr);
    EXPECT_EQ(system_.get_output_port_selection(OutputPortSelection::kUseFirstOutputIfItExists), nullptr);

    const auto& output_port = system_.AddAbstractOutputPort();
    EXPECT_EQ(system_.get_output_port_selection(OutputPortSelection::kUseFirstOutputIfItExists), &output_port);

    EXPECT_EQ(system_.get_output_port_selection(OutputPortIndex{0}), &system_.get_output_port(0));
}
...
TEST_F(SystemTest, SystemConstraintTest) {
    ...
    EXPECT_THROW(system_.get_constraint(SystemConstraintIndex(0)), std::out_of_range);

    ContextConstraintCalc<double> calc = [](const Context<double>& context, Eigen::VectorXd* value) {
        unused(context);
        (*value)[0] = 1.0;
    };
    const double kInf = std::numeric_limits<double>::infinity();
    SystemConstraintIndex test_constraint = system_.AddConstraint(std::make_unique<SystemConstraint<double>>(
            &system_, calc, SystemConstraintBounds(Vector1d(0), std::nullopt), "test"));
    ...
    ContextConstraintCalc<double> calc_false = [](const Context<double>& context, Eigen::VectorXd* value) {
        unused(context);
        (*value)[0] = -1.0;
    };
    system_.AddConstraint(std::make_unique<SystemConstraint<double>>(
            &system_, calc_false, SystemConstraintBounds(Vector1d(0), Vector1d(kInf)), "bad constraint"));
...
TEST_F(SystemTest, TransmogrifyNotSupported) {
    ...
    EXPECT_THROW((System<double>::ToScalarType<AutoDiffXd, TestSystemBase>(system_)), std::exception);
    EXPECT_THROW((System<double>::ToScalarType<symbolic::Expression, TestSystemBase>(system_)), std::exception);
    ...
    EXPECT_FALSE((system_.get_system_scalar_converter().IsConvertible<double, double>()));
}
...
public:
    ...
    void SetDefaultParameters(const Context<double>& context, Parameters<double>* parameters) const final {
    ...
    // magic number.
    void SetDefaultState(const Context<double>& context, State<double>* state) const final {
        const BasicVector<double>& param = context.get_parameters().get_numeric_parameter(0);
...
GTEST_TEST(SystemDefaultTest, SetDefaultInvocationContract) {
    ...
    EXPECT_EQ(context->get_state().get_continuous_state()[0], TestDefaultSystem::kMagicValue);
}
...
public:
    ...
    ValueIOTestSystem() {
        ...
        this->AddOutputPort(internal::FrameworkFactory::Make<LeafOutputPort<T>>(
                this,  // implicit_cast<const System<T>*>(this)
                this,  // implicit_cast<const SystemBase*>(this)
                this->get_system_id(), "absport", OutputPortIndex(this->num_output_ports()),
                this->assign_next_dependency_ticket(), kAbstractValued, 0 /* size */,
                &this->DeclareCacheEntry("absport", &ValueIOTestSystem::CalcStringOutput)));
        ...
        this->AddOutputPort(internal::FrameworkFactory::Make<LeafOutputPort<T>>(
                this,  // implicit_cast<const System<T>*>(this)
                this,  // implicit_cast<const SystemBase*>(this)
                this->get_system_id(), "vecport", OutputPortIndex(this->num_output_ports()),
                this->assign_next_dependency_ticket(), kVectorValued, 1 /* size */,
                &this->DeclareCacheEntry("vecport", BasicVector<T>(1), &ValueIOTestSystem::CalcVectorOutput)));
    ...
    const InputPort<T>& AddAbstractInputPort() { return this->DeclareInputPort(kUseDefaultName, kAbstractValued, 0); }

    std::unique_ptr<AbstractValue> DoAllocateInput(const InputPort<T>& input_port) const override {
    ...
    void CalcStringOutput(const ContextBase& context, std::string* output) const {
        const std::string* str_in = this->template EvalInputValue<std::string>(context, 0);
        *output = *str_in + "output";
    }

    // Sets output(1) = 2 * input(1).
    void CalcVectorOutput(const ContextBase& context_base, BasicVector<T>* output) const {
...
TEST_F(SystemInputErrorTest, CheckMessages) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.get_input_port(-1), ".*get_input_port.*negative.*-1.*illegal.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalVectorInput(*context_, -1),
                                         ".*EvalVectorInput.*negative.*-1.*illegal.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalAbstractInput(*context_, -2),
                                         ".*EvalAbstractInput.*negative.*-2.*illegal.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalInputValue<int>(*context_, -3),
                                         ".*EvalInputValue.*negative.*-3.*illegal.*");

    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.get_input_port(9), ".*get_input_port.*no input port.*9.*only.*4.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalVectorInput(*context_, 9),
                                         ".*EvalVectorInput.*no input port.*9.*only.*4.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalAbstractInput(*context_, 10),
                                         ".*EvalAbstractInput.*no input port.*10.*only.*4.*");
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalInputValue<int>(*context_, 11),
    ...
    DRAKE_EXPECT_NO_THROW(system_.EvalVectorInput(*context_, 2));  // BasicVector OK.
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalVectorInput<WrongVector>(*context_, 2),
                                         ".*EvalVectorInput.*expected.*WrongVector"
                                         ".*input port.*2.*actual.*MyVector.*");

    DRAKE_EXPECT_NO_THROW(system_.EvalInputValue<BasicVector<double>>(*context_, 1));
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalInputValue<int>(*context_, 1),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalVectorInput(*context_, 0),
                                         ".*EvalVectorInput.*vector port required.*input port.*0.*"
                                         "was declared abstract.*");

    DRAKE_EXPECT_THROWS_MESSAGE_IF_ARMED(system_.EvalInputValue<double>(*context_, 0),
...
TEST_F(SystemIOTest, SystemValueIOTest) {
    ...
    EXPECT_EQ(output_->get_data(0)->get_value<std::string>(), std::string("inputoutput"));
    ...
    EXPECT_EQ(dynamic_cast<const TestTypedVector<double>*>(test_sys_.EvalVectorInput(*context_, 1)), nullptr);
    // Now allocate.
    ...
    EXPECT_NE(dynamic_cast<const TestTypedVector<double>*>(test_sys_.EvalVectorInput(*context_, 1)), nullptr);
}
...
TEST_F(SystemIOTest, RandomInputPortTest) {
    ...
    EXPECT_EQ(test_sys_.get_input_port(2).get_random_type(), RandomDistribution::kUniform);
    EXPECT_EQ(test_sys_.get_input_port(3).get_random_type(), RandomDistribution::kGaussian);
}
...
TEST_F(SystemIOTest, TransmogrifyAndFix) {
    ...
    EXPECT_EQ(dest_system.EvalAbstractInput(*dest_context, 0)->get_value<std::string>(), "input");

    const TestTypedVector<AutoDiffXd>* fixed_vec = dest_system.EvalVectorInput<TestTypedVector>(*dest_context, 1);
...
TEST_F(SystemIOTest, FixFromTypeDependentAbstractInput) {
    ...
    std::unique_ptr<AbstractValue> input_value = test_sys_.AllocateInputAbstract(typed_input);
    DRAKE_EXPECT_NO_THROW(input_value->get_value<BasicVector<double>>());

    const auto context = test_sys_.CreateDefaultContext();
    typed_input.FixValue(context.get(), BasicVector<double>(Eigen::VectorXd::Zero(1)));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(autodiff_system.FixInputPortsFrom(test_sys_, *context, autodiff_context.get()),
...
public:
    ...
    // Just u0.
    std::unique_ptr<AbstractValue> DoAllocateInput(const InputPort<double>& input_port) const final {
    ...
    std::unique_ptr<ContinuousState<double>> AllocateTimeDerivatives() const final {
        auto result = std::make_unique<ContinuousState<double>>(std::make_unique<BasicVector<double>>(3), 1, 1,
                                                                1);  // q, v, z
        result->set_system_id(this->get_system_id());
        return result;
    }
...
private:
    ...
    double DoCalcNonConservativePower(const Context<double>& context) const final {
        ++pnc_count_;
...
TEST_F(ComputationTest, Eval) {
    ...
    const int64_t serial_number = entry.get_cache_entry_value(*context_).serial_number();
    test_sys_.EvalTimeDerivatives(*context_);
    EXPECT_NE(entry.get_cache_entry_value(*context_).serial_number(), serial_number);
}
