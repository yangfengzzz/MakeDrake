...
GTEST_TEST(EventsTest, EventDataAccess) {
    ...
    EXPECT_EQ(event.get_mutable_event_data<WitnessTriggeredEventData<double>>(), nullptr);
    ...
    const PeriodicEventData* stored_data = event.get_event_data<PeriodicEventData>();
