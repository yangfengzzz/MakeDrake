...
#include "common/trajectories/exponential_plus_piecewise_polynomial.h"
...
#include "systems/primitives/demultiplexer.h"
...
GTEST_TEST(DiagramBuilderTest, Remove) {
    ...
    diagram->GetInputPort("u0").FixValue(context.get(), Eigen::VectorXd::Constant(1, 22.0));
    diagram->GetInputPort("u2").FixValue(context.get(), Eigen::VectorXd::Constant(1, 44.0));
...
GTEST_TEST(DiagramBuilderTest, RemoveError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.RemoveSystem(dummy), ".*RemoveSystem.*dummy.*not.*added.*");
}
...
GTEST_TEST(DiagramBuilderTest, AlreadyBuilt) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.AddSystem<Adder>(2, 2), ".*DiagramBuilder may no longer be used.*");
}
...
public:
    ...
    const systems::InputPort<T>& get_vec_input_port() const { return this->get_input_port(0); }

    const systems::OutputPort<T>& get_echo_output_port() const { return systems::System<T>::get_output_port(0); }

    const systems::OutputPort<T>& get_const_output_port() const { return systems::System<T>::get_output_port(1); }

    void CalcConstant(const Context<T>& context, BasicVector<T>* const_value) const {
...
GTEST_TEST(DiagramBuilderTest, AlgebraicLoop) {
    DiagramBuilder<double> builder;
    auto adder = builder.AddNamedSystem<Adder>("adder", 1 /* inputs */, 1 /* size */);
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Build(), fmt::format("Reported algebraic loop detected in DiagramBuilder:\n"
...
GTEST_TEST(DiagramBuilderTest, CycleButNoAlgebraicLoopSystemLevel) {
    ...
    //        |->| 1       |                 |---> output
    //        |------------------------------|
    auto adder = builder.AddNamedSystem<Adder>("adder", 2 /* inputs */, 1 /* size */);
    auto integrator = builder.AddNamedSystem<Integrator>("integrator", 1 /* size */);
...
GTEST_TEST(DiagramBuilderTest, CascadedNonDirectFeedthrough) {
    ...
    auto integrator1 = builder.AddNamedSystem<Integrator>("integrator1", 1 /* size */);
    auto integrator2 = builder.AddNamedSystem<Integrator>("integrator2, ", 1 /* size */);

    builder.Connect(integrator1->get_output_port(), integrator2->get_input_port());
...
GTEST_TEST(DiagramBuilderTest, SystemsThatAreNotAddedThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(adder, adder),
                                "DiagramBuilder: System 'adder' has not been registered to this "
                                "DiagramBuilder using AddSystem nor AddNamedSystem.\n\n.*'integrator1', "
                                "'integrator2'.*\n\n.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ExportInput(adder.get_input_port(0)),
                                "DiagramBuilder: System 'adder' has not been registered to this "
                                "DiagramBuilder using AddSystem nor AddNamedSystem.\n\n.*'integrator1', "
                                "'integrator2'.*\n\n.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ExportOutput(adder.get_output_port()),
                                "DiagramBuilder: System 'adder' has not been registered to this "
                                "DiagramBuilder using AddSystem nor AddNamedSystem.\n\n.*'integrator1', "
                                "'integrator2'.*\n\n.*");
}
...
GTEST_TEST(DiagramBuilderTest, ConnectVectorToAbstractThrow) {
    DiagramBuilder<double> builder;
    auto vector_system = builder.AddNamedSystem<PassThrough<double>>("vector_system", 1 /* size */);
    auto abstract_system = builder.AddNamedSystem<PassThrough<double>>("abstract_system", Value<int>{});
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(vector_system->get_output_port(), abstract_system->get_input_port()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(abstract_system->get_output_port(), vector_system->get_input_port()),
                                "DiagramBuilder::Connect: "
                                "Cannot mix vector-valued and abstract-valued ports while connecting "
...
GTEST_TEST(DiagramBuilderTest, ExportInputVectorToAbstractThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ConnectInput(port_index, abstract_system->get_input_port()),
...
GTEST_TEST(DiagramBuilderTest, ExportInputAbstractToVectorThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ConnectInput(port_index, vector_system->get_input_port()),
                                "DiagramBuilder::ConnectInput: "
                                "Cannot mix vector-valued and abstract-valued ports while connecting "
...
GTEST_TEST(DiagramBuilderTest, ConnectVectorSizeMismatchThrow) {
    DiagramBuilder<double> builder;
    auto size1_system = builder.AddNamedSystem<PassThrough<double>>("size1_system", 1 /* size */);
    auto size2_system = builder.AddNamedSystem<PassThrough<double>>("size2_system", 2 /* size */);
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(size1_system->get_output_port(), size2_system->get_input_port()),
                                "DiagramBuilder::Connect: "
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(size2_system->get_output_port(), size1_system->get_input_port()),
                                "DiagramBuilder::Connect: "
                                "Mismatched vector sizes while connecting "
...
GTEST_TEST(DiagramBuilderTest, ExportInputVectorSizeMismatchThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ConnectInput(port_index, size2_system->get_input_port()),
                                "DiagramBuilder::ConnectInput: "
                                "Mismatched vector sizes while connecting "
...
GTEST_TEST(DiagramBuilderTest, ConnectAbstractTypeMismatchThrow) {
    DiagramBuilder<double> builder;
    auto int_system = builder.AddNamedSystem<PassThrough<double>>("int_system", Value<int>{});
    auto char_system = builder.AddNamedSystem<PassThrough<double>>("char_system", Value<char>{});
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(int_system->get_output_port(), char_system->get_input_port()),
                                "DiagramBuilder::Connect: "
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Connect(char_system->get_output_port(), int_system->get_input_port()),
                                "DiagramBuilder::Connect: "
                                "Mismatched value types while connecting "
...
GTEST_TEST(DiagramBuilderTest, ExportInputAbstractTypeMismatchThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ConnectInput(port_index, char_system->get_input_port()),
                                "DiagramBuilder::ConnectInput: "
                                "Mismatched value types while connecting "
...
GTEST_TEST(DiagramBuilderTest, ConnectAbstractSubtypes) {
    ...
    using ExponentialPlusPiecewisePolynomiald = trajectories::ExponentialPlusPiecewisePolynomial<double>;

    DiagramBuilder<double> builder;
    auto sys1 = builder.AddSystem<PassThrough<double>>(Value<Trajectoryd>(PiecewisePolynomiald{}));
    auto sys2 = builder.AddSystem<PassThrough<double>>(Value<Trajectoryd>(ExponentialPlusPiecewisePolynomiald{}));
    ...
    // We can feed PiecewisePolynomial through the Diagram.
    {
        ...
        diagram->get_input_port(0).FixValue(context.get(), Value<Trajectoryd>(input));
        const auto& output =
                dynamic_cast<const PiecewisePolynomiald&>(diagram->get_output_port(0).Eval<Trajectoryd>(*context));
    ...
    // We can feed ExponentialPlusPiecewisePolynomial through the Diagram.
    {
        ...
        const ExponentialPlusPiecewisePolynomiald input(
                PiecewisePolynomiald::ZeroOrderHold(Eigen::Vector2d(0., 1.), Eigen::RowVector2d(22.0, 22.0)));
        diagram->get_input_port(0).FixValue(context.get(), Value<Trajectoryd>(input));
        const auto& output = dynamic_cast<const ExponentialPlusPiecewisePolynomiald&>(
...
GTEST_TEST(DiagramBuilderTest, SetPortNamesTest) {
    ...
    const auto source1_out = builder.ExportOutput(source1->get_output_port(0), "source1");
    const auto source2_out = builder.ExportOutput(source2->get_output_port(0), "source2");
...
GTEST_TEST(DiagramBuilderTest, DuplicateInputPortNamesThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ExportInput(sink2->get_input_port(0), "sink"),
...
GTEST_TEST(DiagramBuilderTest, ThrowIfInputAlreadyWired) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.ExportInput(sink->get_input_port(0), "sink2"),
...
GTEST_TEST(DiagramBuilderTest, InputPortNamesFanout) {
    ...
    EXPECT_TRUE(builder.ConnectToSame(sink1->get_input_port(0), sink5->get_input_port(0)));
...
GTEST_TEST(DiagramBuilderTest, DuplicateOutputPortNamesThrow) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(builder.Build(), ".*already has an output port named.*");
}
...
protected:
    void SetUp() override {
        out1_ = builder_.AddNamedSystem<ConstantVectorSource>("constant", Vector1d::Ones());
        in1_ = builder_.AddNamedSystem<Sink>("sink");
        in1out1_ = builder_.AddNamedSystem<Gain>("gain", 1.0 /* gain */, 1 /* size */);
        in2out1_ = builder_.AddNamedSystem<Adder>("adder", 2 /* inputs */, 1 /* size */);
...
TEST_F(DiagramBuilderSolePortsTest, SourceGainSink2) {
    builder_.Connect(*out1_, *in1out1_);
    EXPECT_TRUE(builder_.ConnectToSame(in1out1_->get_input_port(), in1_->get_input_port()));
...
TEST_F(DiagramBuilderSolePortsTest, ConnectToSameNothing) {
    EXPECT_FALSE(builder_.ConnectToSame(in1out1_->get_input_port(), in1_->get_input_port()));
    auto diagram = builder_.Build();
...
TEST_F(DiagramBuilderSolePortsTest, TooManyDestInputs) {
    ...
    const_cast<InputPort<double>&>(in2out1_->get_input_port(1)).set_deprecation("deprecated");
...
TEST_F(DiagramBuilderSolePortsTest, TooManySrcInputs) {
    ...
    const_cast<OutputPort<double>&>(in1out2_->get_output_port(1)).set_deprecation("deprecated");
...
GTEST_TEST(DiagramBuilderTest, GetMutableSystems) {
    DiagramBuilder<double> builder;
    auto adder1 = builder.AddNamedSystem<Adder>("adder1", 1 /* inputs */, 1 /* size */);
    auto adder2 = builder.AddNamedSystem<Adder>("adder2", 1 /* inputs */, 1 /* size */);
    EXPECT_EQ((std::vector<const System<double>*>{adder1, adder2}), builder.GetSystems());
    EXPECT_EQ((std::vector<System<double>*>{adder1, adder2}), builder.GetMutableSystems());
}
...
GTEST_TEST(DiagramBuilderTest, GetByName) {
    ...
    // Downcasting by name.
    const Adder<double>& adder2 = builder.GetDowncastSubsystemByName<Adder>("adder");
    const PassThrough<double>& pass2 = builder.GetDowncastSubsystemByName<PassThrough>("pass");
    const UntemplatedSystem& untemplated2 = builder.GetDowncastSubsystemByName<UntemplatedSystem>("untemplated");
    EXPECT_EQ(&adder2, adder);
    EXPECT_EQ(&pass2, pass);
    EXPECT_EQ(&untemplated2, untemplated);

    // Mutable downcasting by name.
    Adder<double>& adder3 = builder.GetMutableDowncastSubsystemByName<Adder>("adder");
    PassThrough<double>& pass3 = builder.GetMutableDowncastSubsystemByName<PassThrough>("pass");
    UntemplatedSystem& untemplated3 = builder.GetMutableDowncastSubsystemByName<UntemplatedSystem>("untemplated");
    EXPECT_EQ(&adder3, adder);
    EXPECT_EQ(&pass3, pass);
    EXPECT_EQ(&untemplated3, untemplated);

    // Error: no such name.
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetSubsystemByName("not_a_subsystem"), ".*not_a_subsystem.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetMutableSubsystemByName("not_a_subsystem"), ".*not_a_subsystem.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetDowncastSubsystemByName<Adder>("not_a_subsystem"), ".*not_a_subsystem.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetMutableDowncastSubsystemByName<Adder>("not_a_subsystem"),
                                ".*not_a_subsystem.*");

    // Error: wrong type.
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetDowncastSubsystemByName<Gain>("adder"), ".*cast.*Adder.*Gain.*");
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetMutableDowncastSubsystemByName<Gain>("adder"), ".*cast.*Adder.*Gain.*");
    ...
    EXPECT_EQ(&builder.GetSubsystemByName("adder"), adder);
    EXPECT_TRUE(builder.HasSubsystemNamed("pass"));
    DRAKE_EXPECT_THROWS_MESSAGE(builder.GetMutableSubsystemByName("pass"), ".*multiple subsystems.*pass.*unique.*");
...
GTEST_TEST(DiagramBuilderTest, ExportInputOutputIndex) {
    DiagramBuilder<double> builder;
    auto adder1 = builder.AddNamedSystem<Adder>("adder1", 3 /* inputs */, 1 /* size */);
    auto adder2 = builder.AddNamedSystem<Adder>("adder2", 1 /* inputs */, 1 /* size */);

    EXPECT_EQ(builder.ExportInput(adder1->get_input_port(0)), 0 /* exported input port id */);
    EXPECT_EQ(builder.ExportInput(adder1->get_input_port(1)), 1 /* exported input port id */);

    EXPECT_EQ(builder.num_input_ports(), 2);
    EXPECT_EQ(builder.num_output_ports(), 0);

    EXPECT_EQ(builder.ExportOutput(adder1->get_output_port()), 0 /* exported output port id */);
    EXPECT_EQ(builder.ExportOutput(adder2->get_output_port()), 1 /* exported output port id */);
