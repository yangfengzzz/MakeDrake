...
GTEST_TEST(SystemScalarConversionDoxygen, PendulumPlantAutodiff) {
    ...
    auto* state = dynamic_cast<PendulumState<double>*>(&context->get_mutable_continuous_state_vector());
    state->set_theta(0.1);
    ...
    xc[PendulumStateIndices::kTheta].derivatives() = MatrixXd::Identity(kNumDerivatives, kNumDerivatives).col(0);
    ...
    AutoDiffXd autodiff_energy = autodiff_plant->CalcTotalEnergy(*autodiff_context);
