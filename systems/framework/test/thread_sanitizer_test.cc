...
GTEST_TEST(ThreadSanitizerTest, PerThreadContextTest) {
    ...
    const auto context_rw_operation = [](Context<double>* system_context) {
        Eigen::VectorXd state_vector = system_context->get_discrete_state_vector().get_value();
    ...
    std::future<void> context_A_rw_operation = std::async(std::launch::async, context_rw_operation, context_A.get());
    std::future<void> context_B_rw_operation = std::async(std::launch::async, context_rw_operation, context_B.get());
...
GTEST_TEST(ThreadSanitizerTest, SharedFrozenContextTest) {
    ...
    const auto context_ro_operation = [](Context<double>* system_context) {
        const Eigen::VectorXd state_vector = system_context->get_discrete_state_vector().get_value();
        return state_vector;
    };
