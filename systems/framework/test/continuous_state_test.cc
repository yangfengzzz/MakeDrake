...
protected:
    ...
    std::unique_ptr<ContinuousState<T>> MakeNanState() {
        auto result = std::make_unique<ContinuousState<T>>(std::make_unique<BasicVector<T>>(kLength), kPositionLength,
...
TEST_F(ContinuousStateTest, Access) {
    ...
    EXPECT_EQ(kPositionLength, continuous_state_->get_generalized_position().size());
    EXPECT_EQ(1, continuous_state_->get_generalized_position()[0]);
    EXPECT_EQ(2, continuous_state_->get_generalized_position()[1]);

    EXPECT_EQ(kVelocityLength, continuous_state_->get_generalized_velocity().size());
...
TEST_F(ContinuousStateTest, OutOfBoundsAccess) {
    EXPECT_THROW(continuous_state_->get_generalized_position().GetAtIndex(2), std::exception);
    EXPECT_THROW(continuous_state_->get_mutable_generalized_velocity().SetAtIndex(1, 42), std::exception);
}
...
TEST_F(ContinuousStateTest, OutOfBoundsConstruction) {
    EXPECT_THROW(continuous_state_.reset(new ContinuousState<double>(MakeSomeVector<double>(), kPositionLength,
...
TEST_F(ContinuousStateTest, MoreVelocityThanPositionVariables) {
    EXPECT_THROW(continuous_state_.reset(new ContinuousState<double>(MakeSomeVector<double>(), 1 /* num_q */,
                                                                     2 /* num_v */, kMiscLength + 1)),
                 std::out_of_range);
}
...
TEST_F(ContinuousStateTest, SetFrom) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(actual_double->SetFrom(*unbound_symbolic), ".*variable q.*\n*");
...
TEST_F(ContinuousStateTest, SetFromException) {
    ...
    const auto wrong = std::make_unique<ContinuousState<double>>(MakeSomeVector<double>(), kPositionLength - 1,
                                                                 kVelocityLength, kMiscLength + 1);
...
TEST_F(ContinuousStateTest, StringStream) {
    ...
    const std::string expected =
            fmt::format("hello {} world", fmt_eigen(continuous_state_->CopyToVector().transpose()));
...
protected:
    void SetUp() override {
        state0_.reset(new ContinuousState<double>(MyVector3d::Make(1, 2, 3), 1, 1, 1));
        state1_.reset(new ContinuousState<double>(BasicVector<double>::Make(4, 5, 6, 7, 8), 2, 1, 2));
        state2_.reset(new ContinuousState<double>(BasicVector<double>::Make(10, 11), 0, 0, 2));
        state3_.reset(new ContinuousState<double>(BasicVector<double>::Make(-1, -2, -3, -4), 2, 1, 1));
        ...
        //   state2 2z                            10,11
        unowned_.reset(new DiagramContinuousState<double>({&*state0_, &*state1_, &*state2_}));
        ...
        //     state1
        //     state2
        root_unowned_.reset(new DiagramContinuousState<double>({&*state3_, &*unowned_}));
...
TEST_F(DiagramContinuousStateTest, Values) {
    ...
    const VectorXd unowned_q = unowned_->get_generalized_position().CopyToVector();
    const VectorXd unowned_v = unowned_->get_generalized_velocity().CopyToVector();
    const VectorXd unowned_z = unowned_->get_misc_continuous_state().CopyToVector();
    ...
    const VectorXd root_unowned_q = root_unowned_->get_generalized_position().CopyToVector();
    const VectorXd root_unowned_v = root_unowned_->get_generalized_velocity().CopyToVector();
    const VectorXd root_unowned_z = root_unowned_->get_misc_continuous_state().CopyToVector();
    ...
    const VectorXd root_owned_q = root_owned_->get_generalized_position().CopyToVector();
    const VectorXd root_owned_v = root_owned_->get_generalized_velocity().CopyToVector();
    const VectorXd root_owned_z = root_owned_->get_misc_continuous_state().CopyToVector();
...
TEST_F(DiagramContinuousStateTest, Clone) {
    ...
    for (int i = 0; i < unowned_->size(); ++i) EXPECT_EQ((*clone_of_unowned)[i], (*unowned_)[i]);
    for (int i = 0; i < root_unowned_->size(); ++i) EXPECT_EQ((*clone_of_root_unowned)[i], (*root_unowned_)[i]);
...
TEST_F(DiagramContinuousStateTest, CloneSystemId) {
    ...
    for (int i = 0; i < unowned_->num_substates(); ++i) {
        EXPECT_EQ(clone_of_unowned->get_substate(i).get_system_id(), unowned_->get_substate(i).get_system_id());
    }

    EXPECT_EQ(clone_of_root_unowned->get_system_id(), root_unowned_->get_system_id());
