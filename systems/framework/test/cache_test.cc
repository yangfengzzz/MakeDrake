...
#include "systems/framework/context_base.h"
...
protected:
    void SetUp() override {
        ...
        cache().CreateNewCacheEntryValue(index0_, next_ticket_++, "entry0", {all_sources_ticket_}, &graph());
        cache_value(index0_).SetInitialValue(PackValue(0));

        index1_ = next_cache_index_++;
        cache().CreateNewCacheEntryValue(index1_, next_ticket_++, "entry1", {cache_value(index0_).ticket()}, &graph());
        cache_value(index1_).SetInitialValue(PackValue(1));

        index2_ = next_cache_index_++;
        cache().CreateNewCacheEntryValue(index2_, next_ticket_++, "entry2",
                                         {cache_value(index0_).ticket(), cache_value(index1_).ticket()}, &graph());
        ...
        cache().CreateNewCacheEntryValue(string_index_, next_ticket_++, "string thing", {time_ticket_}, &graph());
        cache_value(string_index_).SetInitialValue(AbstractValue::Make<string>("initial"));
        ...
        cache().CreateNewCacheEntryValue(vector_index_, next_ticket_++, "vector thing",
                                         {xc_ticket_, cache_value(string_index_).ticket()}, &graph());
        const MyVector3d my_vec(Vector3d(0, 0, 0));
        cache_value(vector_index_).SetInitialValue(AbstractValue::Make<MyVector3d>(my_vec));
    ...
    DependencyTracker& tracker(CacheIndex index, ContextBase* context_ptr = nullptr) {
        if (!context_ptr) context_ptr = &context_;
        return tracker(cache_value(index, &context_ptr->get_mutable_cache()).ticket(), &*context_ptr);
    }
    ...
    DependencyTracker& tracker(DependencyTicket ticket, ContextBase* context_ptr = nullptr) {
        if (!context_ptr) context_ptr = &context_;
        return graph(&*context_ptr).get_mutable_tracker(ticket);
    }

    std::unique_ptr<MyContextBase> context_ptr_ = std::make_unique<MyContextBase>();
...
TEST_F(CacheTest, CanAssociateExistingTrackerWithNewCacheEntry) {
    ...
    CacheEntryValue& normal_value =
            cache().CreateNewCacheEntryValue(index, ticket, "normal cache entry", {nothing_ticket_}, &graph());
    ...
    CacheEntryValue& xcdot_value =
            cache().CreateNewCacheEntryValue(index, xcdot_ticket_, "xcdot cache entry", {nothing_ticket_}, &graph());
    // No new tracker should have been created.
    EXPECT_EQ(graph().trackers_size(), expected_num_trackers);
    EXPECT_EQ(graph().get_tracker(xcdot_ticket_).cache_entry_value(), &xcdot_value);
}
...
TEST_F(CacheTest, SetInitialValueWorks) {
    ...
    EXPECT_THROW(cache_value(index2_).SetInitialValue(PackValue(5)), std::logic_error);
    ...
    CacheEntryValue& value =
            cache().CreateNewCacheEntryValue(index, next_ticket_++, "null value", {nothing_ticket_}, &graph());
    EXPECT_FALSE(value.has_value());
    EXPECT_THROW(cache_value(index).SetInitialValue(std::unique_ptr<AbstractValue>()), std::logic_error);
...
TEST_F(CacheTest, FreezeUnfreezeWork) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(str_val.SetValueOrThrow<std::string>("three"),
                                ".*string thing.*SetValueOrThrow.*cache is frozen.*");
    // (Despite the snake_case name, this still checks for frozen cache.)
    DRAKE_EXPECT_THROWS_MESSAGE(str_val.set_value<std::string>("four"), ".*string thing.*set_value.*cache is frozen.*");
    DRAKE_EXPECT_THROWS_MESSAGE(str_val.GetMutableAbstractValueOrThrow(),
                                ".*string thing.*GetMutableAbstractValueOrThrow.*cache is frozen.*");
    DRAKE_EXPECT_THROWS_MESSAGE(str_val.GetMutableValueOrThrow<std::string>(),
                                ".*string thing.*GetMutableValueOrThrow.*cache is frozen.*");
    DRAKE_EXPECT_THROWS_MESSAGE(str_val.swap_value(&swapper), ".*string thing.*swap_value.*cache is frozen.*");
}
...
TEST_F(CacheTest, Clone) {
    ...
    cache().CreateNewCacheEntryValue(last_index, next_ticket_++, "last entry", {nothing_ticket_}, &graph());
    ...
    for (CacheIndex index(0); index < cache().cache_size(); ++index) {
        EXPECT_EQ(cache().has_cache_entry_value(index), clone_cache.has_cache_entry_value(index));
        if (!cache().has_cache_entry_value(index)) continue;

        const CacheEntryValue& value = cache().get_cache_entry_value(index);
        const CacheEntryValue& clone_value = clone_cache.get_cache_entry_value(index);
        ...
        EXPECT_THROW(clone_value.ThrowIfBadCacheEntryValue(&context_), std::logic_error);
        ...
        const DependencyTracker& clone_value_tracker = tracker(value.ticket(), &clone_context);
        DRAKE_EXPECT_NO_THROW(value_tracker.ThrowIfBadDependencyTracker(&context_, &value));
        DRAKE_EXPECT_NO_THROW(clone_value_tracker.ThrowIfBadDependencyTracker(&clone_context, &clone_value));
    ...
    EXPECT_EQ(cache_value(index0_, &clone_cache).GetValueOrThrow<int>(), cache_value(index0_).GetValueOrThrow<int>());
    EXPECT_EQ(cache_value(index1_, &clone_cache).GetValueOrThrow<int>(), cache_value(index1_).GetValueOrThrow<int>());
    EXPECT_EQ(cache_value(index2_, &clone_cache).GetValueOrThrow<int>(), cache_value(index2_).GetValueOrThrow<int>());
    EXPECT_EQ(cache_value(string_index_, &clone_cache).GetValueOrThrow<string>(),
              cache_value(string_index_).GetValueOrThrow<string>());
    EXPECT_EQ(cache_value(vector_index_, &clone_cache).GetValueOrThrow<MyVector3d>().get_value(),
...
TEST_F(CacheTest, ValueMethodsWork) {
    CacheIndex index(next_cache_index_++);
    cache().CreateNewCacheEntryValue(index, next_ticket_++, "get test", {nothing_ticket_}, &graph());
    ...
    const AbstractValue& abstract_value = cache().get_cache_entry_value(index).GetAbstractValueOrThrow();
