...
protected:
    void SetUp() override {
        ...
        internal::SystemBaseContextBaseAttorney::set_system_id(&context_, internal::SystemId::get_new_id());
        ...
        context_.init_continuous_state(std::make_unique<ContinuousState<double>>(
                BasicVector<double>::Make({1.0, 2.0, 3.0, 5.0, 8.0}), kGeneralizedPositionSize,
        ...
        context_.init_discrete_state(std::make_unique<DiscreteValues<double>>(std::move(xd_single)));
        ...
        context_.init_discrete_state(std::make_unique<DiscreteValues<double>>(std::move(xd)));
        ...
        graph.get_mutable_tracker(DependencyTicket(internal::kXdTicket)).SubscribeToPrerequisite(&xd0_tracker);
        ...
        context_.init_abstract_state(std::make_unique<AbstractValues>(std::move(xa)));
        ...
        graph.get_mutable_tracker(DependencyTicket(internal::kXaTicket)).SubscribeToPrerequisite(&xa0_tracker);
        ...
        graph.get_mutable_tracker(DependencyTicket(internal::kPnTicket)).SubscribeToPrerequisite(&pn0_tracker);
        graph.get_mutable_tracker(DependencyTicket(internal::kPnTicket)).SubscribeToPrerequisite(&pn1_tracker);
        ...
        graph.get_mutable_tracker(DependencyTicket(internal::kPaTicket)).SubscribeToPrerequisite(&pa0_tracker);

        context_.init_parameters(
                std::make_unique<Parameters<double>>(std::move(vector_params), std::move(abstract_params)));
    }
    ...
    // Reads a FixedInputPortValue connected to @p context at @p index.
    const std::string* ReadStringInputPort(const Context<double>& context, int index) {
        const FixedInputPortValue* free_value = context.MaybeGetFixedInputPortValue(InputPortIndex(index));
        return free_value ? &free_value->get_value().get_value<std::string>() : nullptr;
    }

    // Reads a FixedInputPortValue connected to @p context at @p index.
    const AbstractValue* ReadAbstractInputPort(const Context<double>& context, int index) {
        const FixedInputPortValue* free_value = context.MaybeGetFixedInputPortValue(InputPortIndex(index));
    ...
    template <typename T>
    void AddInputPort(InputPortIndex i,
    ...
    void AddOutputPorts(int n, LeafContext<T>* context) {
        ...
        context->AddOutputPort(OutputPortIndex(0), next_ticket_++, {SubsystemIndex(1), DependencyTicket(0)});

        for (OutputPortIndex i(1); i < n; ++i) {
            output_port_tickets_.push_back(next_ticket_);
            context->AddOutputPort(i, next_ticket_++, {std::nullopt, DependencyTicket(internal::kAllSourcesTicket)});
        }
    }
...
void VerifyClonedState(const State<double>& clone) {
    ...
    const BasicVector<double>* xc_data = dynamic_cast<const BasicVector<double>*>(&xc.get_vector());
...
TEST_F(LeafContextTest, CheckPorts) {
    ...
    // input value.
    auto& u_tracker = context_.get_tracker(DependencyTicket(internal::kAllInputPortsTicket));
    ...
    // entry tracker).
    auto& all_sources_tracker = context_.get_tracker(DependencyTicket(internal::kAllSourcesTicket));
...
TEST_F(LeafContextTest, GetNumStates) {
    ...
    context.init_continuous_state(
            std::make_unique<ContinuousState<double>>(BasicVector<double>::Make({1.0, 2.0, 3.0, 5.0, 8.0})));
    ...
    xd.push_back(BasicVector<double>::Make({128.0}));
    xd.push_back(BasicVector<double>::Make({256.0, 512.0}));
    context.init_discrete_state(std::make_unique<DiscreteValues<double>>(std::move(xd)));
...
TEST_F(LeafContextTest, GetVectorInput) {
    ...
    context.FixInputPort(0, Value<BasicVector<double>>(Eigen::Vector2d(5.0, 6.0)));
...
TEST_F(LeafContextTest, SetAndGetCache) {
    CacheIndex index = context_.get_mutable_cache()
                               .CreateNewCacheEntryValue(CacheIndex(0), ++next_ticket_, "entry",
                                                         {DependencyTicket(internal::kNothingTicket)},
                                                         &context_.get_mutable_dependency_graph())
                               .cache_index();
    CacheEntryValue& entry_value = context_.get_mutable_cache().get_mutable_cache_entry_value(index);
...
TEST_F(LeafContextTest, Clone) {
    ...
    for (int i = 0; i < kNumInputPorts; ++i) {
        ...
        EXPECT_TRUE(
                CompareMatrices(context_port->get_value(), clone_port->get_value(), 1e-8, MatrixCompareType::absolute));
    }
    ...
    LeafContext<double>* leaf_clone = dynamic_cast<LeafContext<double>*>(clone.get());
...
TEST_F(LeafContextTest, BadClone) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(bad_context.Clone(), ".*typeid.source. == typeid.clone.*");
}
...
TEST_F(LeafContextTest, SetTimeStateAndParametersFrom) {
    ...
    target.init_continuous_state(std::make_unique<ContinuousState<AutoDiffXd>>(
            std::make_unique<BasicVector<AutoDiffXd>>(5), kGeneralizedPositionSize, kGeneralizedVelocitySize,
    ...
    target.init_discrete_state(std::make_unique<DiscreteValues<AutoDiffXd>>(std::move(xd)));
...
void MarkAllCacheValuesUpToDate(Cache* cache) {
    for (CacheIndex i(0); i < cache->cache_size(); ++i) {
        if (cache->has_cache_entry_value(i)) cache->get_mutable_cache_entry_value(i).mark_up_to_date();
    }
}

void CheckAllCacheValuesUpToDateExcept(const Cache& cache, const std::set<CacheIndex>& should_be_out_of_date) {
    for (CacheIndex i(0); i < cache.cache_size(); ++i) {
...
TEST_F(LeafContextTest, Invalidation) {
    ...
    for (int ticket = internal::kNothingTicket; ticket <= internal::kLastSourceTicket; ++ticket) {
        CacheEntryValue& entry =
                cache.CreateNewCacheEntryValue(index, next_ticket_++, "entry" + std::to_string(index),
    ...
    CheckAllCacheValuesUpToDateExcept(
            cache, {depends[internal::kTimeTicket], depends[internal::kAllSourcesExceptInputPortsTicket],
    ...
    CheckAllCacheValuesUpToDateExcept(
            cache, {depends[internal::kAccuracyTicket], depends[internal::kConfigurationTicket],
                    depends[internal::kKinematicsTicket], depends[internal::kAllSourcesExceptInputPortsTicket],
                    depends[internal::kAllSourcesTicket]});
    ...
    const std::set<CacheIndex> vz_dependent{depends[internal::kVTicket],
    ...
    MarkAllCacheValuesUpToDate(&cache);
    context_.SetContinuousState(context_.get_continuous_state_vector().CopyToVector());
    CheckAllCacheValuesUpToDateExcept(cache, xc_dependent);
    ...
    MarkAllCacheValuesUpToDate(&cache);
    context_.SetTimeAndContinuousState(context_.get_time() + 1., context_.get_continuous_state_vector().CopyToVector());
    CheckAllCacheValuesUpToDateExcept(cache, t_and_xc_dependent);

    context_.SetTime(1.);
    MarkAllCacheValuesUpToDate(&cache);
    VectorBase<double>& xc1 = context_.SetTimeAndGetMutableContinuousStateVector(2.);
    CheckAllCacheValuesUpToDateExcept(cache, t_and_xc_dependent);
    ...
    const std::set<CacheIndex> xd_dependent{depends[internal::kXdTicket],
                                            depends[internal::kXTicket],
                                            depends[internal::kConfigurationTicket],
                                            depends[internal::kKinematicsTicket],
                                            depends[internal::kAllSourcesExceptInputPortsTicket],
    ...
    const std::set<CacheIndex> xa_dependent{depends[internal::kXaTicket],
                                            depends[internal::kXTicket],
                                            depends[internal::kConfigurationTicket],
                                            depends[internal::kKinematicsTicket],
                                            depends[internal::kAllSourcesExceptInputPortsTicket],
    ...
    // Modify parameters.
    const std::set<CacheIndex> pn_dependent{depends[internal::kPnTicket],
                                            depends[internal::kConfigurationTicket],
                                            depends[internal::kKinematicsTicket],
    ...
    const std::set<CacheIndex> pa_dependent{depends[internal::kPaTicket],
                                            depends[internal::kConfigurationTicket],
                                            depends[internal::kKinematicsTicket],
                                            depends[internal::kAllParametersTicket],
                                            depends[internal::kAllSourcesExceptInputPortsTicket],
    ...
    FixedInputPortValue* port_value = context_.MaybeGetMutableFixedInputPortValue(InputPortIndex(0));
    ASSERT_NE(port_value, nullptr);
    MarkAllCacheValuesUpToDate(&cache);
    port_value->GetMutableData();
    CheckAllCacheValuesUpToDateExcept(cache,
                                      {depends[internal::kAllInputPortsTicket], depends[internal::kAllSourcesTicket]});
}
...
TEST_F(LeafContextTest, TestStateSettingSugar) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(context_.SetDiscreteState(xd0_new),
    ...
    std::unique_ptr<DiscreteValues<double>> xd_clone = context_.get_discrete_state().Clone();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(context_.SetAbstractState(0, std::string("hello")),
