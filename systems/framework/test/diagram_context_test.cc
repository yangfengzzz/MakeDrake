...
public:
    SystemWithNumericParameters() { DeclareNumericParameter(BasicVector<double>(2)); }
...
protected:
    void SetUp() override {
        ...
        system_with_numeric_parameters_ = std::make_unique<SystemWithNumericParameters>();
        system_with_abstract_parameters_ = std::make_unique<SystemWithAbstractParameters>();
        ...
        system_with_abstract_parameters_->set_name("system_with_abstract_parameters");
        ...
        internal::SystemBaseContextBaseAttorney::set_system_id(context_.get(), internal::SystemId::get_new_id());
    ...
    void AttachInputPorts() {
        context_->FixInputPort(0, Value<BasicVector<double>>(Vector1<double>(128.0)));
        context_->FixInputPort(1, Value<BasicVector<double>>(Vector1<double>(256.0)));
    }

    // Reads a FixedInputPortValue connected to @p context at @p index.
    // Returns nullptr if the port is not connected.
    const BasicVector<double>* ReadVectorInputPort(const Context<double>& context, int index) {
        const FixedInputPortValue* free_value = context.MaybeGetFixedInputPortValue(InputPortIndex(index));
    ...
    void VerifyContinuousStateValue(const Eigen::Vector2d& expected) {
        ...
        EXPECT_EQ(context_->GetSubsystemContext(SubsystemIndex(2)).get_continuous_state_vector()[0], expected[0]);
        EXPECT_EQ(context_->GetSubsystemContext(SubsystemIndex(3)).get_continuous_state_vector()[0], expected[1]);
    }
    ...
    std::vector<int64_t> SaveNotifications(DependencyTicket ticket) {
        ...
        for (SubsystemIndex i(0); i < kNumSystems; ++i) {
            notifications.push_back(NumNotifications(context_->GetSubsystemContext(i), ticket));
        }
    ...
    static int64_t NumNotifications(const ContextBase& context, DependencyTicket ticket) {
        const auto& tracker = context.get_tracker(ticket);
        return tracker.num_notifications_received() - tracker.num_ignored_notifications();
    }
    ...
    std::unique_ptr<SystemWithAbstractParameters> system_with_abstract_parameters_;
};
...
TEST_F(DiagramContextTest, RetrieveConstituents) {
    ...
    for (SubsystemIndex i(0); i < kNumSystems; ++i) {
        auto context = dynamic_cast<const LeafContext<double>*>(&context_->GetSubsystemContext(i));
...
TEST_F(DiagramContextTest, MutableStateNotifications) {
    ...
    VerifyNotifications("get_mutable_state: x", has_state(), SystemBase::all_state_ticket(), &x_before);
    VerifyNotifications("get_mutable_state: xc", has_continuous_state(), SystemBase::xc_ticket(), &xc_before);
    VerifyNotifications("get_mutable_state: xd", has_discrete_state(), SystemBase::xd_ticket(), &xd_before);
    VerifyNotifications("get_mutable_state: xa", has_abstract_state(), SystemBase::xa_ticket(), &xa_before);

    // Changing continuous state should affect only x and xc.
    context_->get_mutable_continuous_state();  // Return value ignored.
    VerifyNotifications("get_mutable_continuous_state: x", has_continuous_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_continuous_state: xc", has_continuous_state(), SystemBase::xc_ticket(),
                        &xc_before);
    ...
    VerifyNotifications("get_mutable_continuous_state_vector: x", has_continuous_state(),
                        SystemBase::all_state_ticket(), &x_before);
    VerifyNotifications("get_mutable_continuous_state_vector: xc", has_continuous_state(), SystemBase::xc_ticket(),
                        &xc_before);
    ...
    VerifyNotifications("SetContinuousState: x", has_continuous_state(), SystemBase::all_state_ticket(), &x_before);
    VerifyNotifications("SetContinuousState: xc", has_continuous_state(), SystemBase::xc_ticket(), &xc_before);
    ...
    VerifyNotifications("SetTimeAndContinuousState: t", SystemBase::time_ticket(), &t_before);
    VerifyNotifications("SetTimeAndContinuousState: x", has_continuous_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("SetTimeAndContinuousState: xc", has_continuous_state(), SystemBase::xc_ticket(), &xc_before);
    ...
    VerifyNotifications("get_mutable_discrete_state: x", has_discrete_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_discrete_state: xd", has_discrete_state(), SystemBase::xd_ticket(), &xd_before);
    ...
    VerifyNotifications("get_mutable_discrete_state_vector: x", has_discrete_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_discrete_state_vector: xd", has_discrete_state(), SystemBase::xd_ticket(),
                        &xd_before);
    ...
    VerifyNotifications("get_mutable_discrete_state(0): x", has_discrete_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_discrete_state(0): xd", has_discrete_state(), SystemBase::xd_ticket(), &xd_before);
    ...
    VerifyNotifications("get_mutable_abstract_state: x", has_abstract_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_abstract_state: xa", has_abstract_state(), SystemBase::xa_ticket(), &xa_before);
    ...
    VerifyNotifications("get_mutable_abstract_state(0): x", has_abstract_state(), SystemBase::all_state_ticket(),
                        &x_before);
    VerifyNotifications("get_mutable_abstract_state(0): xa", has_abstract_state(), SystemBase::xa_ticket(), &xa_before);
...
TEST_F(DiagramContextTest, MutableParameterNotifications) {
    ...
    VerifyNotifications("get_mutable_parameters: p", has_parameter(), SystemBase::all_parameters_ticket(), &p_before);
    VerifyNotifications("get_mutable_parameters: pn", has_numeric_parameter(), SystemBase::pn_ticket(), &pn_before);
    VerifyNotifications("get_mutable_parameters: pa", has_abstract_parameter(), SystemBase::pa_ticket(), &pa_before);
    ...
    VerifyNotifications("get_mutable_numeric_parameter(0): p", has_numeric_parameter(),
                        SystemBase::all_parameters_ticket(), &p_before);
    VerifyNotifications("get_mutable_numeric_parameter(0): pn", has_numeric_parameter(), SystemBase::pn_ticket(),
                        &pn_before);
    ...
    VerifyNotifications("get_mutable_abstract_parameter(0): p", has_abstract_parameter(),
                        SystemBase::all_parameters_ticket(), &p_before);
    VerifyNotifications("get_mutable_abstract_parameter(0): pa", has_abstract_parameter(), SystemBase::pa_ticket(),
                        &pa_before);
...
TEST_F(DiagramContextTest, MutableEverythingNotifications) {
    ...
    VerifyNotifications("SetTimeStateAndParametersFrom: t", SystemBase::time_ticket(), &t_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: a", SystemBase::accuracy_ticket(), &a_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: p", has_parameter(), SystemBase::all_parameters_ticket(),
                        &p_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: pn", has_numeric_parameter(), SystemBase::pn_ticket(),
                        &pn_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: pa", has_abstract_parameter(), SystemBase::pa_ticket(),
                        &pa_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: x", has_state(), SystemBase::all_state_ticket(), &x_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: xc", has_continuous_state(), SystemBase::xc_ticket(),
                        &xc_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: xd", has_discrete_state(), SystemBase::xd_ticket(), &xd_before);
    VerifyNotifications("SetTimeStateAndParametersFrom: xa", has_abstract_state(), SystemBase::xa_ticket(), &xa_before);
}
...
TEST_F(DiagramContextTest, State) {
    ...
    ContinuousState<double>& integrator0_xc =
            context_->GetMutableSubsystemContext(SubsystemIndex(2)).get_mutable_continuous_state();
    ContinuousState<double>& integrator1_xc =
            context_->GetMutableSubsystemContext(SubsystemIndex(3)).get_mutable_continuous_state();
    EXPECT_EQ(42.0, integrator0_xc.get_vector()[0]);
    EXPECT_EQ(43.0, integrator1_xc.get_vector()[0]);
    // - Discrete
    DiscreteValues<double>& discrete_xd =
            context_->GetMutableSubsystemContext(SubsystemIndex(4)).get_mutable_discrete_state();
...
TEST_F(DiagramContextTest, DiagramState) {
    auto diagram_state = dynamic_cast<DiagramState<double>*>(&context_->get_mutable_state());
...
TEST_F(DiagramContextTest, ConnectValid) {
    DRAKE_EXPECT_NO_THROW(
            context_->SubscribeInputPortToOutputPort({SubsystemIndex(0) /* adder0_ */, OutputPortIndex(0)},
...
TEST_F(DiagramContextTest, NextChangeEventNumber) {
    ...
    EXPECT_EQ(const_cast<const DiagramContext<double>*>(context_.get())->start_new_change_event(), first + 2);

    // Obtain a subcontext and verify we still count up.
    Context<double>& discrete_context = context_->GetMutableSubsystemContext(SubsystemIndex(4));
    ...
    Context<double>& discrete_clone = clone->GetMutableSubsystemContext(SubsystemIndex(4));
...
TEST_F(DiagramContextTest, Clone) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        EXPECT_TRUE(
                CompareMatrices(orig_port->get_value(), clone_port->get_value(), 1e-8, MatrixCompareType::absolute));
    }
}
...
TEST_F(DiagramContextTest, SubcontextCloneIsError) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(subcontext.Clone(),
...
TEST_F(DiagramContextTest, SubcontextSetTimeStateAndParametersFromIsError) {
    auto clone = dynamic_pointer_cast<DiagramContext<double>>(context_->Clone());
    ASSERT_TRUE(clone != nullptr);
    const auto& source_subcontext = context_->GetSubsystemContext(SubsystemIndex{0});
    auto& dest_subcontext = clone->GetMutableSubsystemContext(SubsystemIndex{0});
    DRAKE_EXPECT_THROWS_MESSAGE(dest_subcontext.SetTimeStateAndParametersFrom(source_subcontext),
...
TEST_F(DiagramContextTest, SubcontextSetStateAndParametersFrom) {
    ...
    for (int i = 0; i < kNumSystems; ++i) {
        const auto& source_subcontext = context_->GetSubsystemContext(SubsystemIndex{i});
        auto& dest_subcontext = clone->GetMutableSubsystemContext(SubsystemIndex{i});
