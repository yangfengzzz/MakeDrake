#include "systems/framework/event_status.h"
...
bool equal_status(const EventStatus& one, const EventStatus& two) {
    return one.severity() == two.severity() && one.system() == two.system() && one.message() == two.message();
}
...
GTEST_TEST(EventStatusTest, ConstructionAndRetrieval) {
    ...
    const EventStatus terminated = EventStatus::ReachedTermination(dummy, "reached termination");
...
GTEST_TEST(EventStatusTest, ThrowOnFailure) {
    ...
    const EventStatus did_nothing = EventStatus::DidNothing();
    const EventStatus terminated = EventStatus::ReachedTermination(&test_system, "this shouldn't throw");
    const EventStatus failed_no_system = EventStatus::Failed(nullptr, "error from somewhere");
    const EventStatus failed_with_system = EventStatus::Failed(&test_system, "error from test system");
