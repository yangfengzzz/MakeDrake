...
public:
    ...
    template <typename U>
    explicit AnyToAnySystem(const AnyToAnySystem<U>& other) : AnyToAnySystem(other.magic()) {}
...
public:
    ...
    explicit NonSymbolicSystem(int magic) : magic_(magic) {
        ...
        this->DeclareVectorOutputPort(kUseDefaultName, 1, [this](const Context<T>& context, BasicVector<T>* output) {
            const auto& input = this->get_input_port(0).Eval(context);
            (*output)[0] = test::copysign_int_to_non_symbolic_scalar(this->magic(), input[0]);
        });
    }

    // Copy constructor that converts to a different scalar type.
    template <typename U>
    explicit NonSymbolicSystem(const NonSymbolicSystem<U>& other) : NonSymbolicSystem(other.magic()) {}
...
public:
    ...
    // copy constructor.
    explicit FromDoubleSystem(const FromDoubleSystem<double>& other, int dummy = 0) : FromDoubleSystem(other.magic()) {}
...
public:
    ...
    // Copy constructor that converts to a different scalar type.
    template <typename U>
    explicit SubclassOfAnyToAnySystem(const SubclassOfAnyToAnySystem<U>&) : SubclassOfAnyToAnySystem() {}
};
...
GTEST_TEST(SystemScalarConverterTest, SubclassMismatch) {
    // When correctly configured, converting the subclass type is successful.
    {
        ...
        const std::unique_ptr<System<AutoDiffXd>> converted = dut.Convert<AutoDiffXd, double>(original);
                ...
                ({
                    try {
                        ...
                        EXPECT_THAT(std::string(e.what()),
                                    testing::MatchesRegex("SystemScalarConverter was configured to convert a "
    ...
    {
        auto dut = SystemScalarConverter::MakeWithoutSubtypeChecking<AnyToAnySystem>();
        const SubclassOfAnyToAnySystem<double> original;
        EXPECT_TRUE(is_dynamic_castable<AnyToAnySystem<AutoDiffXd>>(dut.Convert<AutoDiffXd, double>(original)));
    }
}
...
GTEST_TEST(SystemScalarConverterTest, RemoveUnlessAlsoSupportedBy) {
    ...
    EXPECT_TRUE(is_dynamic_castable<AnyToAnySystem<AutoDiffXd>>(dut.Convert<AutoDiffXd, double>(system)));
}
