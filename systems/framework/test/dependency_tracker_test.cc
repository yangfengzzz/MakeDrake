#include "systems/framework/dependency_tracker.h"
...
void ExpectStatsMatch(const DependencyTracker* tracker, const Stats& expected) {
    EXPECT_EQ(tracker->num_ignored_notifications(), expected.ignored) << tracker->description();
    EXPECT_EQ(tracker->num_notifications_sent(), expected.sent) << tracker->description();
    EXPECT_EQ(tracker->num_value_change_events(), expected.value_change) << tracker->description();
    EXPECT_EQ(tracker->num_prerequisite_change_events(), expected.prereq_change) << tracker->description();
    EXPECT_EQ(tracker->num_notifications_received(),
              tracker->num_value_change_events() + tracker->num_prerequisite_change_events())
            << tracker->description();
}
...
GTEST_TEST(DependencyTracker, BuiltInTrackers) {
    ...
    for (int ticket_int = 0; ticket_int < internal::kNextAvailableTicket; ++ticket_int) {
        ...
        const auto& context_interface = static_cast<const internal::ContextMessageInterface&>(context);
        DRAKE_EXPECT_NO_THROW(
                tracker.ThrowIfBadDependencyTracker(&context, &context_interface.dummy_cache_entry_value()));
        EXPECT_THROW(tracker.ThrowIfBadDependencyTracker(&context2), std::logic_error);
    }
    ...
    auto& all_non_u_sources = context.get_tracker(DT(internal::kAllSourcesExceptInputPortsTicket));
...
protected:
    void SetUp() override {
        ...
        downstream1_ = &graph.CreateNewDependencyTracker(next_ticket++, "downstream1");
        downstream2_ = &graph.CreateNewDependencyTracker(next_ticket++, "downstream2");
        ...
        entry0_ = &cache.CreateNewCacheEntryValue(index, next_ticket++, "entry0",
        ...
        DRAKE_EXPECT_NO_THROW(entry0_tracker_ = &graph.get_mutable_tracker(entry0_->ticket()));
    ...
    Stats tt_stats_, up1_stats_, up2_stats_, mid1_stats_, down1_stats_, down2_stats_, entry0_stats_;
};
...
TEST_F(HandBuiltDependencies, Construction) {
    ...
    auto& tracker100 = graph.CreateNewDependencyTracker(DependencyTicket(100), "tracker100");
    ...
    const CacheEntryValue& xcdot_value =
            cache.CreateNewCacheEntryValue(index, xcdot_ticket, "xcdot cache value", {time_ticket_}, &graph);
...
TEST_F(HandBuiltDependencies, GetPathname) {
    ...
    EXPECT_EQ(middle1_->GetPathDescription(), system_path + ":" + mid1_description);
}
...
TEST_F(HandBuiltDependencies, Clone) {
    ...
    const DependencyTracker& after_gap = graph.CreateNewDependencyTracker(after_gap_ticket, "after_gap");
    ...
    const auto& context_interface = static_cast<const internal::ContextMessageInterface&>(context_);
    const auto& clone_context_interface = static_cast<const internal::ContextMessageInterface&>(*clone_context);
    ASSERT_NE(&context_interface.dummy_cache_entry_value(), &clone_context_interface.dummy_cache_entry_value());

    // Now verify that the trackers are associated with the right one.
    const DependencyTracker& original_time_tracker = context_.get_tracker(time_ticket_);
    const DependencyTracker& clone_time_tracker = clone_context->get_tracker(time_ticket_);
    DRAKE_EXPECT_NO_THROW(
            original_time_tracker.ThrowIfBadDependencyTracker(&context_, &context_interface.dummy_cache_entry_value()));
    ...
    CacheEntryValue& clone_entry0 = clone_cache.get_mutable_cache_entry_value(entry0_->cache_index());

    // Expected statistics for the cloned trackers; initially zero.
    Stats tt_stats, up1_stats, up2_stats, mid1_stats, down1_stats, down2_stats, entry0_stats;
...
TEST_F(HandBuiltDependencies, SuppressNotifications) {
    ...
    auto clone_context = context_.Clone();
    const DependencyTracker& clone_middle1_ = clone_context->get_dependency_graph().get_tracker(middle1_->ticket());
    EXPECT_TRUE(clone_middle1_.notifications_are_suppressed());
    // Check a random one to make sure they didn't all get suppressed!
    const DependencyTracker& clone_upstream1_ = clone_context->get_dependency_graph().get_tracker(upstream1_->ticket());
