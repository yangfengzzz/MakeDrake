...
GTEST_TEST(SystemConstraintBoundsTest, BadSizes) {
    ...
    EXPECT_THROW(SystemConstraintBounds(Vector1d::Constant(1.0), Vector2d::Constant(2.0)), std::exception);
}
...
GTEST_TEST(SystemConstraintTest, Basic) {
    ...
    ContextConstraintCalc<double> calc2 = [](const Context<double>& context, Eigen::VectorXd* value) {
        *value = Eigen::Vector2d(context.get_continuous_state_vector().CopyToVector());
    };

    Eigen::VectorXd value;

    // Make a (linear) system just to make a valid context.
    LinearSystem<double> system(Eigen::Matrix2d::Identity(), Eigen::Vector2d::Zero(), Eigen::MatrixXd(0, 2),
                                Eigen::MatrixXd(0, 1));
    ...
    SystemConstraint<double> equality_constraint(&system, calc, SystemConstraintBounds::Equality(1),
                                                 "equality constraint");
    ...
    SystemConstraint<double> inequality_constraint(&system, calc2, {Eigen::Vector2d::Ones(), Eigen::Vector2d(4, 6)},
                                                   "inequality constraint");
    EXPECT_TRUE(CompareMatrices(inequality_constraint.lower_bound(), Eigen::Vector2d::Ones()));
    EXPECT_TRUE(CompareMatrices(inequality_constraint.upper_bound(), Eigen::Vector2d(4, 6)));
...
GTEST_TEST(SystemConstraintTest, WrongOrder) {
    DRAKE_EXPECT_THROWS_MESSAGE(WrongOrderSystem{},
...
// quite commonly used.
ExternalSystemConstraint ZeroStateDerivativeConstraint(const System<double>& shape) {
...
ExternalSystemConstraint ZeroVelocityConstraint(const System<double>& shape) {
    ...
    return ExternalSystemConstraint::MakeForAllScalars(
            ...
            [](const auto& system, const auto& context, auto* value) {
                *value = context.get_continuous_state().get_generalized_velocity().CopyToVector();
            });
}
...
protected:
    ...
    LinearSystem<double> system_{(Matrix2d{} << 0.0, 1.0, 0.0, 0.0).finished(), Vector2d(0.0, 1.0), MatrixXd(0, 2),
                                 MatrixXd(0, 1)};
};
...
TEST_F(ExternalSystemConstraintTest, FixedPoint) {
    ...
    EXPECT_EQ(make_conjunction(inspector.constraints()).to_string(), "((u0_0 == 0) and (xc1 == 0))");
...
TEST_F(ExternalSystemConstraintTest, DoubleOnly) {
    ...
    const ExternalSystemConstraint dut(
            ...
            [&](const System<double>& system, const Context<double>& context, VectorXd* value) {
...
TEST_F(ExternalSystemConstraintTest, NonSymbolic) {
    const auto& dut = ExternalSystemConstraint::MakeForNonsymbolicScalars(
            "xcdot = 0", SystemConstraintBounds::Equality(2), [](const auto& system, const auto& context, auto* value) {
                const auto& xc = system.EvalTimeDerivatives(context).get_vector();
                *value = xc.CopyToVector();
            });
    ...
    system_autodiff->get_input_port().FixValue(context_autodiff.get(), VectorX<T>::Zero(1));
    context_autodiff->SetContinuousState((VectorX<T>(2) << T{0.0}, T{22.0}).finished());
