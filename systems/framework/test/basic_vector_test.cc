...
GTEST_TEST(BasicVectorTest, DoubleInitiallyNaN) {
    ...
    EXPECT_TRUE(
            CompareMatrices(expected, vec.value(), Eigen::NumTraits<double>::epsilon(), MatrixCompareType::absolute));
}
...
GTEST_TEST(BasicVectorTest, MakeSymbolic) {
    auto vec = BasicVector<symbolic::Expression>::Make(symbolic::Variable("x"), 2.0, symbolic::Variable("y") + 2.0);
...
using DefaultScalars = ::testing::Types<double, AutoDiffXd, symbolic::Expression>;
...
TYPED_TEST(TypedBasicVectorTest, StringStream) {
    ...
    std::stringstream s;
    s << "hello " << vec << " world";
    const std::string expected = fmt::format("hello {} world", fmt_eigen(vec.value().transpose()));
...
TYPED_TEST(TypedBasicVectorTest, ZeroLengthStringStream) {
    ...
    std::stringstream s;
    s << "foo [" << vec << "] bar";
    const std::string expected = fmt::format("foo [{}] bar", fmt_eigen(vec.value().transpose()));
    EXPECT_EQ(s.str(), expected);
}
...
GTEST_TEST(BasicVectorTest, ExceptionMessages) {
    BasicVector<double> pair{1.0, 2.0};
    DRAKE_EXPECT_THROWS_MESSAGE(pair.GetAtIndex(-1),
                                "Index -1 is not within \\[0, 2\\) while accessing .*BasicVector.*");
    DRAKE_EXPECT_THROWS_MESSAGE(pair.GetAtIndex(10),
                                "Index 10 is not within \\[0, 2\\) while accessing .*BasicVector.*");
    DRAKE_EXPECT_THROWS_MESSAGE(pair.SetFromVector(Eigen::Vector3d::Zero()),
