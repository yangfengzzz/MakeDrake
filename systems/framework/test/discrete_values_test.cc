...
protected:
    const VectorXd v00_ = Vector2d{1., 1.}, v01_ = Vector2d{2., 3.}, v10_ = Vector3d{9., 10., 11.},
                   v11_ = Vector3d{-1.25, -2.5, -3.75}, v12_ = Vector2d{5., 6.};
...
TEST_F(DiscreteValuesTest, UnownedState) {
    DiscreteValues<double> xd(std::vector<BasicVector<double>*>{data_[0].get(), data_[1].get()});
    EXPECT_EQ(xd.get_vector(0).value(), v00_);
    EXPECT_EQ(xd.get_vector(1).value(), v01_);
...
TEST_F(DiscreteValuesTest, NoNullsAllowed) {
    // Unowned.
    EXPECT_THROW(DiscreteValues<double>(std::vector<BasicVector<double>*>{nullptr, data_[1].get()}), std::logic_error);
    // Owned.
...
TEST_F(DiscreteValuesTest, SetFrom) {
    ...
    DiscreteValues<AutoDiffXd> dut_autodiff(BasicVector<AutoDiffXd>::Make({3.0, 4.0}));
    DiscreteValues<symbolic::Expression> dut_symbolic(BasicVector<symbolic::Expression>::Make({5.0, 6.0}));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut_double.SetFrom(dut_symbolic), ".*variable x.*\n*");
}
...
TEST_F(DiscreteValuesTest, DiagramDiscreteValues) {
    ...
    EXPECT_FALSE(is_dynamic_castable<MyVector3d>(&root2.get_vector(0)));  // dv3[0]
    ...
    const DiscreteValues<double>& root2_ddv1 = root2.get_subdiscrete(SubsystemIndex(1));
