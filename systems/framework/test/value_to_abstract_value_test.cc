#include "systems/framework/value_to_abstract_value.h"
...
public:
    ...
    CopyableAndCloneable(const CopyableAndCloneable& src) : value_(src.value()), was_copy_constructed_(true) {}
...
private:
    virtual std::unique_ptr<JustCloneable> DoClone() const { return std::make_unique<JustCloneable>(value_); }
    double value_{};
};
...
public:
    ...
    BaseClassCloneable(std::string name, double value) : JustCloneable(value), name_(std::move(name)) {}
...
public:
    ...
    DerivedClassCloneable(std::string name, double value) : JustCloneable(value), name_(std::move(name)) {}
...
GTEST_TEST(ValueToAbstractValue, AbstractPolicy) {
    ...
    const auto value_str_copy = AbstractPolicy::ToAbstract("MyApi", value_str);          // (1, with conversion)
    const auto absval_double_copy = AbstractPolicy::ToAbstract("MyApi", absval_double);  // (1)
    ...
    const auto str_val = AbstractPolicy::ToAbstract("MyApi", std::string("string"));  // (4)
    ...
    const auto copyable_val = AbstractPolicy::ToAbstract("MyApi", CopyConstructible(1.25));  // (4, copy)
    EXPECT_EQ(copyable_val->get_value<CopyConstructible>().value(), 1.25);

    const auto cloneable_val = AbstractPolicy::ToAbstract("MyApi", JustCloneable(0.75));  // (4, clone)
    ...
    const auto base_cloneable_val =
            AbstractPolicy::ToAbstract("MyApi", BaseClassCloneable("base_cloneable", 1.5));  // (4, clone-to-base)
    EXPECT_EQ(base_cloneable_val->get_value<JustCloneable>().value(), 1.5);
    EXPECT_EQ(dynamic_cast<const BaseClassCloneable&>(base_cloneable_val->get_value<JustCloneable>()).name(),
              "base_cloneable");
    ...
    const auto derived_cloneable_val =
            AbstractPolicy::ToAbstract("MyApi", DerivedClassCloneable("derived_cloneable", 0.25));  // (4, clone)
    EXPECT_EQ(derived_cloneable_val->get_value<DerivedClassCloneable>().value(), 0.25);
    EXPECT_EQ(derived_cloneable_val->get_value<DerivedClassCloneable>().name(), "derived_cloneable");
    ...
    const auto cc_val_abstract_copy = AbstractPolicy::ToAbstract("MyApi", cc_value);  // (4, prefer copy)
    const auto& cc_val_copy = cc_val_abstract_copy->get_value<CopyableAndCloneable>();
...
GTEST_TEST(ValueToAbstractValue, AbstractPolicyOnVectors) {
    ...
    const auto scalar_val = AbstractPolicy::ToAbstract("MyApi", expected_scalar);  // (4)
    EXPECT_EQ(scalar_val->get_value<double>(), expected_scalar);

    // An Eigen vector object can't be supplied directly.
    DRAKE_EXPECT_THROWS_MESSAGE(AbstractPolicy::ToAbstract("MyApi", expected_vec),  // (3)
                                ".*MyApi.*Eigen.*cannot automatically be stored.*Drake abstract "
                                "quantity.*");

    // But providing the type explicitly works.
    const auto vec3_val = AbstractPolicy::ToAbstract("MyApi", Value<Eigen::Vector3d>(expected_vec));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(AbstractPolicy::ToAbstract("MyApi", long_vec.tail(3)),  // (3)
                                ".*MyApi.*Eigen.*cannot automatically be stored.*Drake abstract "
                                "quantity.*");
    ...
    const auto basic_vector_val = AbstractPolicy::ToAbstract("MyApi", basic_vector3);  // (4)
    EXPECT_EQ(basic_vector_val->get_value<BasicVector<double>>().get_value(), Eigen::Vector3d(7., 8., 9.));
    ...
    const MyVector3d& my_vector3_from_abstract = my_vector3_val->get_value<MyVector3d>();
...
GTEST_TEST(ValueToVectorValue, GenericObjects) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(VectorPolicy::ToAbstract("MyApi", bad_val),
                                ".*MyApi.*AbstractValue.*type int is not.*vector quantity.*");

    const auto good_val_copy = VectorPolicy::ToAbstract("MyApi", good_val);
    EXPECT_EQ(good_val_copy->get_value<BasicVector<double>>().get_value(), Eigen::Vector3d(1., 2., 3.));

    // Non-vector objects should throw via signature (5).
    DRAKE_EXPECT_THROWS_MESSAGE(VectorPolicy::ToAbstract("MyApi", 5), ".*MyApi.*type int is not.*vector quantity.*");
    DRAKE_EXPECT_THROWS_MESSAGE(VectorPolicy::ToAbstract("MyApi", std::string("string")),
                                ".*MyApi.*type std::string is not.*vector quantity.*");

    DRAKE_EXPECT_THROWS_MESSAGE(VectorPolicy::ToAbstract("MyApi", CopyConstructible(1.25)),
                                ".*MyApi.*CopyConstructible is not.*vector quantity.*");

    DRAKE_EXPECT_THROWS_MESSAGE(VectorPolicy::ToAbstract("MyApi", JustCloneable(0.75)),
...
GTEST_TEST(ValueToVectorValue, VectorPolicy) {
    ...
    EXPECT_EQ(vec3_val->get_value<BasicVector<double>>().get_value(), expected_vec);

    // Pass a more complicated Eigen object; should still work.
    const Eigen::Vector4d long_vec(.25, .5, .75, 1.);
    auto tail3_val = VectorPolicy::ToAbstract("MyApi", long_vec.tail(3));  // (1)
    EXPECT_EQ(tail3_val->get_value<BasicVector<double>>().get_value(), Eigen::Vector3d(.5, .75, 1.));

    auto segment2_val = VectorPolicy::ToAbstract("MyApi", long_vec.segment<2>(1));  // (1)
    EXPECT_EQ(segment2_val->get_value<BasicVector<double>>().get_value(), Eigen::Vector2d(.5, .75));

    // A plain BasicVector is stored as a Value<BasicVector>. It is handled
    // by the BasicVector overload (3).
    const BasicVector<double> basic_vector3({7., 8., 9.});
    const auto basic_vector_val = VectorPolicy::ToAbstract(__func__, basic_vector3);  // (3)
    EXPECT_EQ(basic_vector_val->get_value<BasicVector<double>>().get_value(), Eigen::Vector3d(7., 8., 9.));
    ...
    const BasicVector<double>& basic_vector = my_vector3_val->get_value<BasicVector<double>>();
    EXPECT_EQ(basic_vector.get_value(), expected_vec);

    // Check that the concrete type was preserved.
    EXPECT_EQ(dynamic_cast<const MyVector3d&>(basic_vector).get_value(), expected_vec);
}
