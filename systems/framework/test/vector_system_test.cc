...
public:
    ...
    // N.B. This method signature might be used by many downstream projects.
    // Change it only with good reason and with a deprecation period first.
    void DoCalcVectorTimeDerivatives(const Context<double>& context,
                                     const Eigen::VectorBlock<const VectorXd>& input,
                                     const Eigen::VectorBlock<const VectorXd>& state,
    ...
    // N.B. This method signature might be used by many downstream projects.
    // Change it only with good reason and with a deprecation period first.
    void DoCalcVectorDiscreteVariableUpdates(const Context<double>& context,
                                             const Eigen::VectorBlock<const VectorXd>& input,
                                             const Eigen::VectorBlock<const VectorXd>& state,
    ...
    int get_discrete_variable_updates_count() const { return discrete_variable_updates_count_; }
...
TEST_F(VectorSystemTest, OutputContinuous) {
    ...
    auto& output_port = dut.get_output_port();
    dut.get_input_port(0).FixValue(context.get(), Eigen::Vector2d(1.0, 2.0));
    context->get_mutable_continuous_state_vector().SetFromVector(Eigen::Vector2d::Ones());
    const auto& output = output_port.Eval(*context);
    EXPECT_EQ(dut.get_output_count(), 1);
    EXPECT_EQ(dut.get_last_context(), context.get());
...
TEST_F(VectorSystemTest, TimeDerivatives) {
    TestVectorSystem dut;
    auto context = dut.CreateDefaultContext();
    std::unique_ptr<ContinuousState<double>> derivatives = dut.AllocateTimeDerivatives();
...
public:
    NoFeedthroughContinuousTimeSystem() : VectorSystem<double>(1, 1, false) { this->DeclareContinuousState(1); }
...
public:
    NoInputContinuousTimeSystem() : VectorSystem<double>(0, 1) { this->DeclareContinuousState(1); }

    // Let test code abuse this by making it public.
    using VectorSystem<double>::EvalVectorInput;

private:
    virtual void DoCalcVectorTimeDerivatives(const drake::systems::Context<double>& context,
                                             const Eigen::VectorBlock<const Eigen::VectorXd>& input,
                                             const Eigen::VectorBlock<const Eigen::VectorXd>& state,
    ...
    virtual void DoCalcVectorOutput(const drake::systems::Context<double>& context,
                                    const Eigen::VectorBlock<const Eigen::VectorXd>& input,
                                    const Eigen::VectorBlock<const Eigen::VectorXd>& state,
...
TEST_F(VectorSystemTest, ImplicitlyNoFeedthroughTest) {
    static_assert(std::is_base_of_v<VectorSystem<double>, Integrator<double>>,
...
private:
    // x[n+1] = x[n]^3
    virtual void DoCalcVectorDiscreteVariableUpdates(const drake::systems::Context<double>& context,
                                                     const Eigen::VectorBlock<const Eigen::VectorXd>& input,
                                                     const Eigen::VectorBlock<const Eigen::VectorXd>& state,
...
TEST_F(VectorSystemTest, NoInputNoOutputDiscreteTimeSystemTest) {
    ...
    auto context = dut.CreateDefaultContext();
    context->get_mutable_discrete_state().get_mutable_vector().SetFromVector(Vector1d::Constant(2.0));
...
public:
    ...
    explicit OpenScalarTypeSystem(int some_number)
        : VectorSystem<T>(SystemTypeTag<OpenScalarTypeSystem>{}, 1, 1), some_number_(some_number) {}

    // Scalar-converting copy constructor.
    template <typename U>
    explicit OpenScalarTypeSystem(const OpenScalarTypeSystem<U>& other) : OpenScalarTypeSystem<T>(other.some_number_) {}
...
public:
    ...
    DirectScalarTypeConversionSystem() : VectorSystem<T>(SystemTypeTag<DirectScalarTypeConversionSystem>(), 0, 0) {
    ...
    explicit DirectScalarTypeConversionSystem(const DirectScalarTypeConversionSystem<double>&, int dummy = 0)
...
struct Traits<DirectScalarTypeConversionSystem> {
    ...
    using supported =
            typename std::bool_constant<std::is_same_v<U, double> && !std::is_same_v<T, symbolic::Expression>>;
};
}  // namespace scalar_conversion
...
public:
    MissingMethodsContinuousTimeSystem() : VectorSystem<double>(0, 1) { this->DeclareContinuousState(1); }
};
...
TEST_F(VectorSystemTest, MissingMethodsContinuousTimeSystemTest) {
    ...
    std::unique_ptr<ContinuousState<double>> derivatives = dut.AllocateTimeDerivatives();
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcTimeDerivatives(*context, derivatives.get()),
                                ".*TimeDerivatives.*derivatives->size.. == 0.*failed.*");

    const auto& output = dut.get_output_port();
    DRAKE_EXPECT_THROWS_MESSAGE(output.Eval(*context), ".*Output.*'output->size.. == 0.*failed.*");
}
...
TEST_F(VectorSystemTest, MissingMethodsDiscreteTimeSystemTest) {
    ...
    context->get_mutable_discrete_state().get_mutable_vector().SetFromVector(Vector1d::Constant(2.0));
    auto discrete_updates = dut.AllocateDiscreteVariables();
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcForcedDiscreteVariableUpdate(*context, discrete_updates.get()),
