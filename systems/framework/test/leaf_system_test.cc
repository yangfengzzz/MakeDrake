...
#include "systems/framework/system.h"
...
public:
    ...
    void AddPeriodicUpdate() {
        ...
        this->DeclarePeriodicDiscreteUpdateEvent(period, offset, &TestSystem<T>::NoopDiscreteUpdate);
        std::optional<PeriodicEventData> periodic_attr = this->GetUniquePeriodicDiscreteUpdateAttribute();
    ...
    EventStatus NoopDiscreteUpdate(const Context<T>&, DiscreteValues<T>*) const { return EventStatus::DidNothing(); }

    void AddPeriodicUnrestrictedUpdate(double period, double offset) {
        this->DeclarePeriodicUnrestrictedUpdateEvent(period, offset, &TestSystem<T>::NoopUnrestritedUpdate);
    }

    EventStatus NoopUnrestritedUpdate(const Context<T>&, State<T>*) const { return EventStatus::DidNothing(); }

    void AddPublish(double period) {
        const double offset = 0.0;
        this->DeclarePeriodicPublishEvent(period, offset, &TestSystem<T>::NoopPublish);
    }

    EventStatus NoopPublish(const Context<T>&) const { return EventStatus::DidNothing(); }
    ...
    const BasicVector<T>& GetVanillaNumericParameters(const Context<T>& context) const {
        return this->GetNumericParameter(context, 0 /* index */);
    }

    BasicVector<T>& GetVanillaMutableNumericParameters(Context<T>* context) const {
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessWithoutEvent() const {
        return this->MakeWitnessFunction("dummy1", WitnessFunctionDirection::kCrossesZero,
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessWithEvent() const {
        return this->MakeWitnessFunction("dummy2", WitnessFunctionDirection::kNone,
                                         &TestSystem<double>::DummyWitnessFunction, PublishEvent<double>());
    }
    ...
    std::unique_ptr<WitnessFunction<T>> MakeWitnessWithDiscreteUpdate() const {
        return this->MakeWitnessFunction("dummy4", WitnessFunctionDirection::kNone,
                                         &TestSystem<double>::DummyWitnessFunction,
                                         &TestSystem<double>::DiscreteUpdateCallback);
    }

    // Fifth testing type: unrestricted update callback specified.
    std::unique_ptr<WitnessFunction<T>> MakeWitnessWithUnrestrictedUpdate() const {
        return this->MakeWitnessFunction("dummy5", WitnessFunctionDirection::kNone,
                                         &TestSystem<double>::DummyWitnessFunction,
    ...
    std::unique_ptr<WitnessFunction<T>> DeclareLambdaWitnessWithoutEvent() const {
        return this->MakeWitnessFunction("dummy6", WitnessFunctionDirection::kCrossesZero,
    ...
    std::unique_ptr<WitnessFunction<T>> DeclareLambdaWitnessWithUnrestrictedUpdate() const {
    ...
    bool discrete_update_callback_called() const { return discrete_update_callback_called_; }
    bool unrestricted_update_callback_called() const { return unrestricted_update_callback_called_; }

    // Verifies that the forced publish events collection has been allocated.
    bool forced_publish_events_collection_allocated() const { return this->forced_publish_events_exist(); }
    ...
    const EventCollection<PublishEvent<double>>& get_forced_publish_events_collection() const {
        return this->get_forced_publish_events();
    }

    // Gets the forced discrete update events collection.
    const EventCollection<DiscreteUpdateEvent<double>>& get_forced_discrete_update_events_collection() const {
        return this->get_forced_discrete_update_events();
    }

    // Gets the forced unrestricted update events collection.
    const EventCollection<UnrestrictedUpdateEvent<double>>& get_forced_unrestricted_update_events_collection() const {
        return this->get_forced_unrestricted_update_events();
    }

    // Sets the forced publish events collection.
    void set_forced_publish_events_collection(std::unique_ptr<EventCollection<PublishEvent<double>>> publish_events) {
    ...
    void set_forced_discrete_update_events_collection(
            std::unique_ptr<EventCollection<DiscreteUpdateEvent<double>>> discrete_update_events) {
    ...
    void set_forced_unrestricted_update_events_collection(
            std::unique_ptr<EventCollection<UnrestrictedUpdateEvent<double>>> unrestricted_update_events) {
        this->set_forced_unrestricted_update_events(std::move(unrestricted_update_events));
    }

    // Gets the mutable version of the forced publish events collection.
    EventCollection<PublishEvent<double>>& get_mutable_forced_publish_events_collection() {
...
private:
    ...
    // MakeWitnessFunction() interface works as promised.
    void PublishCallback(const Context<T>&, const PublishEvent<T>&) const { publish_callback_called_ = true; }
    ...
    void DiscreteUpdateCallback(const Context<T>&, const DiscreteUpdateEvent<T>&, DiscreteValues<T>*) const {
        discrete_update_callback_called_ = true;
    }

    // Unrestricted update callback function, which serves to test whether the
    // appropriate MakeWitnessFunction() interface works as promised.
    void UnrestrictedUpdateCallback(const Context<T>&, const UnrestrictedUpdateEvent<T>&, State<T>*) const {
...
protected:
    void SetUp() override {
        ...
        leaf_info_ = dynamic_cast<const LeafCompositeEventCollection<double>*>(event_info_.get());
...
TEST_F(LeafSystemTest, ForcedEventCollectionsTest) {
    ...
    auto forced_publishes = std::make_unique<LeafEventCollection<PublishEvent<double>>>();
    auto* forced_publishes_pointer = forced_publishes.get();
    system_.set_forced_publish_events_collection(std::move(forced_publishes));
    EXPECT_EQ(&system_.get_forced_publish_events_collection(), forced_publishes_pointer);
    EXPECT_EQ(&system_.get_mutable_forced_publish_events_collection(), forced_publishes_pointer);

    // Verify that we can set the discrete update collection.
    auto forced_discrete_updates = std::make_unique<LeafEventCollection<DiscreteUpdateEvent<double>>>();
    auto* forced_discrete_updates_pointer = forced_discrete_updates.get();
    system_.set_forced_discrete_update_events_collection(std::move(forced_discrete_updates));
    EXPECT_EQ(&system_.get_forced_discrete_update_events_collection(), forced_discrete_updates_pointer);

    // Verify that we can set the forced unrestricted update collection.
    auto forced_unrestricted_updates = std::make_unique<LeafEventCollection<UnrestrictedUpdateEvent<double>>>();
    auto* forced_unrestricted_updates_pointer = forced_unrestricted_updates.get();
    system_.set_forced_unrestricted_update_events_collection(std::move(forced_unrestricted_updates));
    EXPECT_EQ(&system_.get_forced_unrestricted_update_events_collection(), forced_unrestricted_updates_pointer);
}

TEST_F(LeafSystemTest, DefaultPortNameTest) {
    EXPECT_EQ(system_.DeclareVectorInputPort(kUseDefaultName, BasicVector<double>(2)).get_name(), "u0");
    EXPECT_EQ(system_.DeclareAbstractInputPort(kUseDefaultName, Value<int>(1)).get_name(), "u1");

    EXPECT_EQ(system_.DeclareVectorOutputPort(kUseDefaultName, &TestSystem<double>::CalcOutput).get_name(), "y0");
    EXPECT_EQ(
            system_.DeclareAbstractOutputPort(kUseDefaultName, BasicVector<double>(2), &TestSystem<double>::CalcOutput)
                    .get_name(),
            "y1");
}

TEST_F(LeafSystemTest, DeclareVectorPortsBySizeTest) {
    const InputPort<double>& input = system_.DeclareVectorInputPort("my_input", 2);
    EXPECT_EQ(input.get_name(), "my_input");
    EXPECT_EQ(input.size(), 2);
    EXPECT_FALSE(input.is_random());
    EXPECT_TRUE(system_.DeclareVectorInputPort(kUseDefaultName, 3, RandomDistribution::kUniform).is_random());

    const OutputPort<double>& output = system_.DeclareVectorOutputPort("my_output", 3, &TestSystem<double>::CalcOutput);
    EXPECT_EQ(output.get_name(), "my_output");
    EXPECT_EQ(output.size(), 3);

    const OutputPort<double>& output2 =
            system_.DeclareVectorOutputPort("my_output2", 2, [](const Context<double>&, BasicVector<double>*) {});
...
TEST_F(LeafSystemTest, WitnessDeclarations) {
    ...
    auto de = dynamic_cast<const DiscreteUpdateEvent<double>*>(witness4->get_event());
    ASSERT_TRUE(de);
    ...
    auto ue = dynamic_cast<const UnrestrictedUpdateEvent<double>*>(witness5->get_event());
    ASSERT_TRUE(ue);
    ...
    EXPECT_EQ(witness7->direction_type(), WitnessFunctionDirection::kPositiveThenNonPositive);
    ...
    ue = dynamic_cast<const UnrestrictedUpdateEvent<double>*>(witness7->get_event());
    ASSERT_TRUE(ue);
}
...
TEST_F(LeafSystemTest, DiscreteAndAbstractStateTrackers) {
    ...
    const DependencyTracker& xd_tracker = context->get_tracker(system_.xd_ticket());
    for (DiscreteStateIndex i(0); i < system_.num_discrete_state_groups(); ++i) {
        const DependencyTracker& tracker = context->get_tracker(system_.discrete_state_ticket(i));
    ...
    const DependencyTracker& xa_tracker = context->get_tracker(system_.xa_ticket());
    for (AbstractStateIndex i(0); i < system_.num_abstract_states(); ++i) {
        const DependencyTracker& tracker = context->get_tracker(system_.abstract_state_ticket(i));
...
TEST_F(LeafSystemTest, NumericParameters) {
    std::unique_ptr<Context<double>> context = system_.CreateDefaultContext();
    const BasicVector<double>& vec = system_.GetVanillaNumericParameters(*context);
    EXPECT_EQ(13.0, vec[0]);
    EXPECT_EQ(7.0, vec[1]);
    BasicVector<double>& mutable_vec = system_.GetVanillaMutableNumericParameters(context.get());
    mutable_vec[1] = 42.0;
    EXPECT_EQ(42.0, vec[1]);

    EXPECT_EQ(system_.num_numeric_parameter_groups(), 1);
    const DependencyTracker& pn_tracker = context->get_tracker(system_.pn_ticket());
    for (NumericParameterIndex i(0); i < system_.num_numeric_parameter_groups(); ++i) {
        const DependencyTracker& tracker = context->get_tracker(system_.numeric_parameter_ticket(i));
...
TEST_F(LeafSystemTest, AbstractParameters) {
    std::unique_ptr<Context<double>> context = system_.CreateDefaultContext();
    const std::string& param = context->get_abstract_parameter(0 /*index*/).get_value<std::string>();
    EXPECT_EQ(param, "parameter value");
    std::string& mutable_param = context->get_mutable_abstract_parameter(0 /*index*/).get_mutable_value<std::string>();
    ...
    const DependencyTracker& pa_tracker = context->get_tracker(system_.pa_ticket());
    for (AbstractParameterIndex i(0); i < system_.num_abstract_parameters(); ++i) {
        const DependencyTracker& tracker = context->get_tracker(system_.abstract_parameter_ticket(i));
...
TEST_F(LeafSystemTest, DeclareTypedMiscContinuousState) {
    ...
    EXPECT_NE(nullptr, dynamic_cast<MyVector2d*>(&context->get_mutable_continuous_state_vector()));
    EXPECT_EQ(2, xc.size());
    EXPECT_EQ(0, xc.get_generalized_position().size());
    EXPECT_EQ(0, xc.get_generalized_velocity().size());
    EXPECT_EQ(2, xc.get_misc_continuous_state().size());
...
TEST_F(LeafSystemTest, DeclareTypedContinuousState) {
    ...
    const ContinuousState<double>& xc = context->get_continuous_state();
    // Check that type was preserved.
    EXPECT_NE(nullptr, dynamic_cast<MyVector9d*>(&context->get_mutable_continuous_state_vector()));
    ...
    const auto& state_output_port = system_.DeclareStateOutputPort("state", state_index);
...
TEST_F(LeafSystemTest, ContinuousStateBelongsWithSystem) {
    ...
    std::unique_ptr<ContinuousState<double>> derivatives = system_.AllocateTimeDerivatives();
    EXPECT_EQ(derivatives->get_system_id(), context_.get_system_id());
    DRAKE_EXPECT_NO_THROW(system_.CalcTimeDerivatives(context_, derivatives.get()));
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(other_system.CalcTimeDerivatives(*other_context, derivatives.get()),
...
public:
    ...
    DeclaredModelPortsSystem() {
        ...
        unused(in_port);
        in_port = &DeclareVectorInputPort("uniform", MyVector2d(), RandomDistribution::kUniform);
        unused(in_port);
        in_port = &DeclareVectorInputPort("gaussian", MyVector2d(), RandomDistribution::kGaussian);
        unused(in_port);

        // Output port 0 uses a BasicVector base class model.
        out_port = &DeclareVectorOutputPort("basic_vector", BasicVector<double>(3),
                                            &DeclaredModelPortsSystem::CalcBasicVector3);
        unused(out_port);
        // Output port 1 uses a class derived from BasicVector.
        out_port = &DeclareVectorOutputPort("my_vector", MyVector4d(), &DeclaredModelPortsSystem::CalcMyVector4d);
        unused(out_port);

        // Output port 2 uses a concrete string model.
        out_port = &DeclareAbstractOutputPort("string", std::string("45"), &DeclaredModelPortsSystem::CalcString);
        unused(out_port);

        // Output port 3 uses the "Advanced" methods that take a model
        // and a general calc function rather than a calc method.
        out_port = &DeclareVectorOutputPort("advanced", BasicVector<double>(2),
...
private:
    void CalcBasicVector3(const Context<double>&, BasicVector<double>* out) const {
        ASSERT_NE(out, nullptr);
    ...
    std::unique_ptr<MyVector4d> expected_myvector_{MyVector4d::Make(4., 3., 2., 1.)};
};
...
GTEST_TEST(ModelLeafSystemTest, ModelPortsTopology) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_input_port_base(InputPortIndex(10)),
    ...
    const DependencyTracker& all_inputs_tracker = context->get_tracker(dut.all_input_ports_ticket());
    for (InputPortIndex i(0); i < dut.num_input_ports(); ++i) {
        const DependencyTracker& tracker = context->get_tracker(dut.input_port_ticket(i));
        EXPECT_TRUE(all_inputs_tracker.HasPrerequisite(tracker));
        EXPECT_TRUE(tracker.HasSubscriber(all_inputs_tracker));
    }

    const OutputPortBase& out2_base = dut.get_output_port_base(OutputPortIndex(2));
    EXPECT_EQ(out2_base.get_index(), 2);
    const DependencyTicket out2_ticket = dut.output_port_ticket(OutputPortIndex(2));
    const DependencyTicket out3_ticket = dut.output_port_ticket(OutputPortIndex(3));
    EXPECT_TRUE(out2_ticket.is_valid() && out3_ticket.is_valid());
    EXPECT_NE(out2_ticket, out3_ticket);  // We don't know the actual values.
    DRAKE_EXPECT_THROWS_MESSAGE(dut.get_output_port_base(OutputPortIndex(7)),
...
public:
    MissingModelAbstractInputSystem() { this->DeclareInputPort("no_model_input", kAbstractValued, 0); }
};

GTEST_TEST(ModelLeafSystemTest, MissingModelAbstractInput) {
    MissingModelAbstractInputSystem dut;
    dut.set_name("dut");
    DRAKE_EXPECT_THROWS_MESSAGE(dut.AllocateInputAbstract(dut.get_input_port(0)),
...
GTEST_TEST(ModelLeafSystemTest, ModelInputGovernsFixedInput) {
    ...
    context->FixInputPort(0, Value<BasicVector<double>>(VectorXd::Constant(1, 0.0)));
    DRAKE_EXPECT_THROWS_MESSAGE(context->FixInputPort(0, Value<BasicVector<double>>(VectorXd::Constant(2, 0.0))),
                                ...
                                "\\(System ::dut\\)");
    DRAKE_EXPECT_THROWS_MESSAGE(context->FixInputPort(0, Value<std::string>()),
                                "System::FixInputPortTypeCheck\\(\\): expected value of type "
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(context->FixInputPort(2, Value<std::string>()),
                                "System::FixInputPortTypeCheck\\(\\): expected value of type "
                                "int "
...
GTEST_TEST(ModelLeafSystemTest, ModelPortsCalcOutput) {
    ...
    const auto& cache2 = dynamic_cast<const LeafOutputPort<double>&>(port2).cache_entry();
    ...
    EXPECT_THROW(cache2.GetKnownUpToDate<std::string>(*context), std::logic_error);
    ...
    DRAKE_EXPECT_NO_THROW(vec1 = dynamic_cast<const MyVector4d*>(&values[1]->get_value<BasicVector<double>>()));
...
GTEST_TEST(ModelLeafSystemTest, ModelDiscreteState) {
    ...
    for (int i = 0; i < static_cast<int>(dut.indexes_.size()); ++i) EXPECT_TRUE(dut.indexes_[i] == i);
    ...
    EXPECT_EQ(dut.get_output_port().Eval<MyVector2d>(*context).get_value(), Vector2d(1., 2.));
    ...
    EXPECT_EQ(dut.get_output_port().Eval<MyVector2d>(*context).get_value(), Vector2d(9., 10.));
...
public:
    ...
    DeclaredNonModelOutputSystem() {
        ...
        // its allocator.
        port = &DeclareVectorOutputPort(kUseDefaultName, &DeclaredNonModelOutputSystem::CalcDummyVec2);
        unused(port);
        // Output port 1 default-constructs a string as its allocator.
        port = &DeclareAbstractOutputPort(kUseDefaultName, &DeclaredNonModelOutputSystem::CalcString);
        unused(port);
        ...
        // value-initialized.
        port = &DeclareAbstractOutputPort(kUseDefaultName, &DeclaredNonModelOutputSystem::CalcPOD);
        unused(port);
    }
...
GTEST_TEST(NonModelLeafSystemTest, NonModelPortsOutput) {
    ...
    for (OutputPortIndex i(0); i < dut.num_output_ports(); ++i) {
        internal::OutputPortPrerequisite prereq = dut.get_output_port(i).GetPrerequisite();
    ...
    EXPECT_EQ(out0.Eval<BasicVector<double>>(*context).get_value(), out0_dummy->get_value());
...
GTEST_TEST(ZeroSizeSystemTest, AcceptanceTest) {
    ...
    // Input.
    auto& in0 = dut.DeclareVectorInputPort(kUseDefaultName, BasicVector<double>(0));
    EXPECT_EQ(in0.get_data_type(), kVectorValued);
    EXPECT_EQ(in0.size(), 0);

    // Output.
    auto& out0 = dut.DeclareVectorOutputPort(kUseDefaultName, BasicVector<double>(0),
...
TEST_F(LeafSystemTest, CallbackAndInvalidUpdates) {
    ...
    auto context = dynamic_pointer_cast_or_throw<LeafContext<double>>(system_.CreateDefaultContext());

    context->init_continuous_state(
            std::make_unique<ContinuousState<double>>(std::make_unique<BasicVector<double>>(9), 3, 3, 3));
    context->init_discrete_state(std::make_unique<DiscreteValues<double>>(std::make_unique<BasicVector<double>>(1)));
    ...
    context->init_abstract_state(std::make_unique<AbstractValues>(std::move(abstract_data)));
    ...
    LeafCompositeEventCollection<double> leaf_events;
    {
        UnrestrictedUpdateEvent<double> event(
                ...
                [](const System<double>&, const Context<double>& c, const Event<double>&, State<double>* s) {
    ...
    const EventStatus status =
            system_.CalcUnrestrictedUpdate(*context, leaf_events.get_unrestricted_update_events(), x.get());
    ...
    leaf_events.Clear();
    {
        UnrestrictedUpdateEvent<double> event(
                ...
                [](const System<double>&, const Context<double>& c, const Event<double>&, State<double>* s) {
                    ...
                    disc_data.push_back(std::make_unique<BasicVector<double>>(1));
                    s->set_discrete_state(std::make_unique<DiscreteValues<double>>(std::move(disc_data)));
                    return EventStatus::Succeeded();
                });
        event.AddToComposite(&leaf_events);
    }
    ...
    x->set_discrete_state(std::make_unique<DiscreteValues<double>>(std::make_unique<BasicVector<double>>(1)));
...
public:
    ...
    InputPortIndex AddAbstractInputPort(Name name = kUseDefaultName) {
        return this->DeclareAbstractInputPort(name, Value<std::string>{}).get_index();
    }

    OutputPortIndex AddAbstractOutputPort(std::optional<std::set<DependencyTicket>> prerequisites_of_calc = {}) {
        // Dummies.
        auto alloc = []() {
        ...
        if (prerequisites_of_calc) {
            return this->DeclareAbstractOutputPort(kUseDefaultName, alloc, calc, *prerequisites_of_calc).get_index();
        } else {
            // The DeclareAbstractOutputPort API's default value for the
            // prerequisites_of_calc is everything (i.e., "all_sources_ticket()").
            return this->DeclareAbstractOutputPort(kUseDefaultName, alloc, calc).get_index();
        }
    }
...
GTEST_TEST(FeedthroughTest, DefaultWithPrerequisites) {
    ...
    const std::vector<DependencyTicket> non_feedthrough_tickets{
            system.nothing_ticket(),    system.time_ticket(),           system.accuracy_ticket(),
            system.all_state_ticket(),  system.all_parameters_ticket(), system.configuration_ticket(),
...
public:
    explicit SymbolicSparsitySystem(bool use_default_prereqs = true)
        : SymbolicSparsitySystem(SystemTypeTag<SymbolicSparsitySystem>{}, use_default_prereqs) {}
...
protected:
    explicit SymbolicSparsitySystem(SystemScalarConverter converter, bool use_default_prereqs = true)
        : LeafSystem<T>(std::move(converter)), use_default_prereqs_(use_default_prereqs) {
        const int kSize = 1;
        ...
        if (is_using_default_prereqs()) {
            ...
            this->DeclareVectorOutputPort(kUseDefaultName, BasicVector<T>(kSize), &SymbolicSparsitySystem::CalcY0);
            this->DeclareVectorOutputPort(kUseDefaultName, BasicVector<T>(kSize), &SymbolicSparsitySystem::CalcY1);
        } else {
...
using FeedthroughTestScalars = ::testing::Types<double, AutoDiffXd, symbolic::Expression>;
...
// scalar conversion.
class NoScalarConversionSymbolicSparsitySystem : public SymbolicSparsitySystem<symbolic::Expression> {
public:
...
GTEST_TEST(LeafSystemScalarConverterTest, AutoDiffYes) {
    ...
    std::unique_ptr<SymbolicSparsitySystem<AutoDiffXd>> clone = System<double>::ToAutoDiffXd(dut);
    ...
    EXPECT_TRUE((dut.get_system_scalar_converter().IsConvertible<AutoDiffXd, double>()));
    EXPECT_FALSE((dut.get_system_scalar_converter().IsConvertible<double, double>()));
}
...
GTEST_TEST(LeafSystemScalarConverterTest, SymbolicYes) {
    ...
    // Static method automatically downcasts.
    std::unique_ptr<SymbolicSparsitySystem<symbolic::Expression>> clone = System<double>::ToSymbolic(dut);
    ASSERT_NE(clone, nullptr);
    EXPECT_EQ(clone->get_name(), "special_name");
...
GTEST_TEST(LeafSystemScalarConverterTest, SymbolicNo) {
    ...
    EXPECT_THROW(System<double>::ToScalarType<symbolic::Expression>(dut), std::exception);
...
GTEST_TEST(LeafSystemCloneTest, Supported) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(copy_double->ValidateContext(*context_double), "[^]*Context-System mismatch[^]*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(copy_autodiff->ValidateContext(*context_autodiff), "[^]*Context-System mismatch[^]*");

    auto dut_symbolic = std::make_unique<SymbolicSparsitySystem<symbolic::Expression>>();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(copy_symbolic->ValidateContext(*context_symbolic), "[^]*Context-System mismatch[^]*");
}

// Check that the static Clone(foo) preserves the subtype.
GTEST_TEST(LeafSystemCloneTest, SupportedStatic) {
    SymbolicSparsitySystem<double> dut;
    std::unique_ptr<SymbolicSparsitySystem<double>> copy = System<double>::Clone(dut);
...
public:
    ...
    explicit FromDoubleSystem(const FromDoubleSystem<double>& other, int dummy = 0) : FromDoubleSystem() {}
};
...
GTEST_TEST(LeafSystemCloneTest, Unsupported) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(TestSystem<double>{}.Clone(), ".*does not support Clon.*");

    // Systems that allow double -> autodiff but not vice versa cannot be cloned.
    DRAKE_EXPECT_THROWS_MESSAGE(FromDoubleSystem<double>{}.Clone(), ".*does not support Clon.*");
}
...
protected:
    std::unique_ptr<LeafContext<T>> DoMakeLeafContext() const override { return std::make_unique<CustomContext<T>>(); }
};
...
public:
    ...
    void CalcState0Constraint(const Context<double>& context, VectorXd* value) const {
        *value = Vector1d(context.get_continuous_state_vector()[0]);
    }
    void CalcStateConstraint(const Context<double>& context, VectorXd* value) const {
        *value = context.get_continuous_state_vector().CopyToVector();
    }

    void CalcOutput(const Context<double>& context, ConstraintBasicVector<double, 44>* output) const {
...
private:
    void DoCalcTimeDerivatives(const Context<double>& context, ContinuousState<double>* derivatives) const override {
        ...
        derivatives->SetFromVector(
                -dynamic_cast<const BasicVector<double>&>(context.get_continuous_state_vector()).get_value());
    }
};
...
GTEST_TEST(SystemConstraintTest, ClassMethodTest) {
    ...
    EXPECT_EQ(dut.DeclareEqualityConstraint(&ConstraintTestSystem::CalcState0Constraint, 1, "x0"), 0);
    EXPECT_EQ(dut.num_constraints(), 1);

    EXPECT_EQ(dut.DeclareInequalityConstraint(&ConstraintTestSystem::CalcStateConstraint,
                                              {Vector2d::Zero(), std::nullopt}, "x"),
              1);
    EXPECT_EQ(dut.num_constraints(), 2);

    auto context = dut.CreateDefaultContext();
    context->get_mutable_continuous_state_vector().SetFromVector(Vector2d(5.0, 7.0));
    ...
    EXPECT_TRUE(dut.get_constraint(SystemConstraintIndex(0)).is_equality_constraint());
    EXPECT_EQ(dut.get_constraint(SystemConstraintIndex(0)).description(), "x0");

    EXPECT_FALSE(dut.get_constraint(SystemConstraintIndex(1)).is_equality_constraint());
...
GTEST_TEST(SystemConstraintTest, FunctionHandleTest) {
    ...
    ContextConstraintCalc<double> calc0 = [](const Context<double>& context, VectorXd* value) {
        *value = Vector1d(context.get_continuous_state_vector()[1]);
    };
    EXPECT_EQ(dut.DeclareInequalityConstraint(calc0, {Vector1d::Zero(), std::nullopt}, "x1_lower"), 0);
    EXPECT_EQ(dut.num_constraints(), 1);

    ContextConstraintCalc<double> calc1 = [](const Context<double>& context, VectorXd* value) {
        *value = Vector2d(context.get_continuous_state_vector()[1], context.get_continuous_state_vector()[0]);
    };
    EXPECT_EQ(dut.DeclareInequalityConstraint(calc1, {std::nullopt, Vector2d(2, 3)}, "x_upper"), 1);

    auto context = dut.CreateDefaultContext();
    context->get_mutable_continuous_state_vector().SetFromVector(Vector2d(5.0, 7.0));

    VectorXd value;
    const SystemConstraint<double>& inequality_constraint0 = dut.get_constraint(SystemConstraintIndex(0));
    ...
    const SystemConstraint<double>& inequality_constraint1 = dut.get_constraint(SystemConstraintIndex(1));
    ...
    const SystemConstraint<double>& equality_constraint = dut.get_constraint(SystemConstraintIndex(2));
...
GTEST_TEST(SystemConstraintTest, ModelVectorTest) {
    ...
    EXPECT_THAT(constraint0.description(),
                ::testing::ContainsRegex("^parameter 0 of type .*ConstraintBasicVector<double,11>$"));
    ...
    EXPECT_THAT(constraint1.description(),
                ::testing::ContainsRegex("^continuous state of type .*ConstraintBasicVector<double,22>$"));
    ...
    EXPECT_THAT(constraint2.description(),
                ::testing::ContainsRegex("^input 0 of type .*ConstraintBasicVector<double,33>$"));

    // Declaring a constrained model vector output should add constraints.
    // We want `vec[0] >= 44` on the output vector.
    dut.DeclareVectorOutputPort(kUseDefaultName, &ConstraintTestSystem::CalcOutput);
    ...
    EXPECT_THAT(constraint3.description(),
                ::testing::ContainsRegex("^output 0 of type .*ConstraintBasicVector<double,44>$"));
    ...
    EXPECT_THAT(constraint4.description(),
                ::testing::ContainsRegex("^discrete state of type .*ConstraintBasicVector<double,55>$"));
...
GTEST_TEST(RandomContextTest, SetRandomTest) {
    ...
    EXPECT_TRUE((params.array() == context->get_numeric_parameter(0).get_value().array()).all());
...
GTEST_TEST(InitializationTest, ManualEventProcessing) {
    ...
    EXPECT_TRUE(status.succeeded());
    status =
            dut.CalcDiscreteVariableUpdate(*context, init_events->get_discrete_update_events(), discrete_updates.get());
    EXPECT_TRUE(status.succeeded());
    status = dut.CalcUnrestrictedUpdate(*context, init_events->get_unrestricted_update_events(), state.get());
    EXPECT_TRUE(status.succeeded());
...
public:
    explicit EventSugarTestSystem(EventStatus::Severity desired_status = EventStatus::kSucceeded)
        : desired_status_(desired_status) {
        DeclareInitializationPublishEvent(&EventSugarTestSystem::MyPublishHandler);
        DeclareInitializationDiscreteUpdateEvent(&EventSugarTestSystem::MyDiscreteUpdateHandler);
        DeclareInitializationUnrestrictedUpdateEvent(&EventSugarTestSystem::MyUnrestrictedUpdateHandler);

        DeclarePerStepPublishEvent(&EventSugarTestSystem::MyPublishHandler);
        DeclarePerStepDiscreteUpdateEvent(&EventSugarTestSystem::MyDiscreteUpdateHandler);
        DeclarePerStepUnrestrictedUpdateEvent(&EventSugarTestSystem::MyUnrestrictedUpdateHandler);

        DeclarePeriodicPublishEvent(kPeriod, kOffset, &EventSugarTestSystem::MyPublishHandler);
        DeclarePeriodicDiscreteUpdateEvent(kPeriod, kOffset, &EventSugarTestSystem::MyDiscreteUpdateHandler);
        DeclarePeriodicUnrestrictedUpdateEvent(kPeriod, kOffset, &EventSugarTestSystem::MyUnrestrictedUpdateHandler);
        ...
        // additive).
        DeclareForcedDiscreteUpdateEvent(&EventSugarTestSystem::MyDiscreteUpdateHandler);
        DeclareForcedDiscreteUpdateEvent(&EventSugarTestSystem::MySecondDiscreteUpdateHandler);

        // Two forced unrestricted update callbacks (to ensure that they are
        // additive).
        DeclareForcedUnrestrictedUpdateEvent(&EventSugarTestSystem::MyUnrestrictedUpdateHandler);
        DeclareForcedUnrestrictedUpdateEvent(&EventSugarTestSystem::MySecondUnrestrictedUpdateHandler);

        // These variants don't require an EventStatus return.
        DeclarePeriodicPublishEvent(kPeriod, kOffset, &EventSugarTestSystem::MySuccessfulPublishHandler);
        DeclarePeriodicDiscreteUpdateEvent(kPeriod, kOffset, &EventSugarTestSystem::MySuccessfulDiscreteUpdateHandler);
    ...
    int num_second_publish_handler_publishes() const { return num_second_publish_handler_publishes_; }
    ...
    int num_second_unrestricted_update() const { return num_second_unrestricted_update_; }
...
private:
    ...
    EventStatus MyDiscreteUpdateHandler(const Context<double>& context, DiscreteValues<double>* discrete_state) const {
        MySuccessfulDiscreteUpdateHandler(context, &*discrete_state);
        return MakeStatus();
    }

    EventStatus MySecondDiscreteUpdateHandler(const Context<double>& context,
                                              DiscreteValues<double>* discrete_state) const {
        ++num_second_discrete_update_;
        return MakeStatus();
    }

    EventStatus MyUnrestrictedUpdateHandler(const Context<double>& context, State<double>* state) const {
        MySuccessfulUnrestrictedUpdateHandler(context, &*state);
        return MakeStatus();
    }

    EventStatus MySecondUnrestrictedUpdateHandler(const Context<double>& context, State<double>* state) const {
        ++num_second_unrestricted_update_;
        return MakeStatus();
    }

    void MySuccessfulPublishHandler(const Context<double>&) const { ++num_publish_; }

    void MySuccessfulDiscreteUpdateHandler(const Context<double>&, DiscreteValues<double>*) const {
        ++num_discrete_update_;
    }

    void MySuccessfulUnrestrictedUpdateHandler(const Context<double>&, State<double>*) const {
...
GTEST_TEST(EventSugarTest, HandlersGetCalled) {
    ...
    EventStatus status = dut.CalcUnrestrictedUpdate(*context, all_events->get_unrestricted_update_events(), &*state);
    EXPECT_TRUE(status.succeeded());
    dut.CalcForcedUnrestrictedUpdate(*context, &*state);
    status = dut.CalcDiscreteVariableUpdate(*context, all_events->get_discrete_update_events(), &*discrete_state);
    EXPECT_TRUE(status.succeeded());
    ...
    EXPECT_EQ(dut.GetCounts(), (string_map<int>{{"u1", 5}, {"u2", 1}, {"d1", 5}, {"d2", 1}, {"p1", 5}, {"p2", 1}}));
}
...
GTEST_TEST(EventSugarTest, ForcedEventsThrowOnFailure) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.ForcedPublish(*context),
                                "ForcedPublish.*event handler.*"
                                "EventSugarTestSystem.*dut.*failed.*Something bad happened.*");

    auto discrete_state = dut.AllocateDiscreteVariables();
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcForcedDiscreteVariableUpdate(*context, &*discrete_state),
                                "CalcForcedDiscreteVariableUpdate.*event handler.*"
                                "EventSugarTestSystem.*dut.*failed.*Something bad happened.*");

    auto state = context->CloneState();
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcForcedUnrestrictedUpdate(*context, &*state),
                                "CalcForcedUnrestrictedUpdate.*event handler.*"
                                "EventSugarTestSystem.*dut.*failed.*Something bad happened.*");

    DRAKE_EXPECT_THROWS_MESSAGE(dut.ExecuteInitializationEvents(&*context),
...
GTEST_TEST(EventSugarTest, ForcedEventsDontThrowWhenNoFailure) {
    ...
    EXPECT_NO_THROW(successful.CalcForcedDiscreteVariableUpdate(*successful_context, &*successful_discrete_state));
    EXPECT_NO_THROW(terminating.CalcForcedDiscreteVariableUpdate(*terminating_context, &*terminating_discrete_state));

    auto successful_state = successful_context->CloneState();
    auto terminating_state = terminating_context->CloneState();
    EXPECT_NO_THROW(successful.CalcForcedUnrestrictedUpdate(*successful_context, &*successful_state));
    EXPECT_NO_THROW(terminating.CalcForcedUnrestrictedUpdate(*terminating_context, &*terminating_state));

    EXPECT_NO_THROW(successful.ExecuteInitializationEvents(&*successful_context));
    EXPECT_NO_THROW(terminating.ExecuteInitializationEvents(&*terminating_context));
}
...
GTEST_TEST(EventSugarTest, ForceEventsCanBeTriggered) {
    ...
    EXPECT_EQ(successful.GetCounts(),
              (string_map<int>{{"u1", 1}, {"u2", 1}, {"d1", 1}, {"d2", 1}, {"p1", 1}, {"p2", 1}}));

    // Do that again but without the publish events.
    successful.ExecuteForcedEvents(&*successful_context, /*publish=*/false);
    EXPECT_EQ(successful.GetCounts(),
              (string_map<int>{{"u1", 2}, {"u2", 2}, {"d1", 2}, {"d2", 2}, {"p1", 1}, {"p2", 1}}));

    // A "reached termination" return should be ignored.
    terminating.ExecuteForcedEvents(&*terminating_context);
    EXPECT_EQ(terminating.GetCounts(),
              (string_map<int>{{"u1", 1}, {"u2", 1}, {"d1", 1}, {"d2", 1}, {"p1", 1}, {"p2", 1}}));

    DRAKE_EXPECT_THROWS_MESSAGE(failing.ExecuteForcedEvents(&*failing_context),
...
public:
    ...
    void RedeclareResidualSize(int n) { DeclareImplicitTimeDerivativesResidualSize(n); }
...
GTEST_TEST(ImplicitTimeDerivatives, DefaultImplementation) {
    ...
    Vector4d residual4;
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcImplicitTimeDerivativesResidual(*context, *xdot, &residual4),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcImplicitTimeDerivativesResidual(*context, *xdot, &residual3),
...
GTEST_TEST(ImplicitTimeDerivatives, OverrideImplementation) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(dut.CalcImplicitTimeDerivativesResidual(*context, *xdot, &bad_residual),
                                ".*CalcImplicitTimeDerivativesResidual.*"
...
GTEST_TEST(SystemTest, MissedEventIssue12620) {
    ...
    public:
        explicit TriggerTimeButNoEventSystem(double trigger_time) : trigger_time_(trigger_time) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(nan_system.CalcNextUpdateTime(*nan_context, events.get()),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(trigger_system.CalcNextUpdateTime(*trigger_context, events.get()),
                                ".*CalcNextUpdateTime.*TriggerTimeButNoEventSystem.*MyTriggerSystem.*"
...
GTEST_TEST(SystemTest, ForgotToSetTheUpdateTime) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(forgot_system.CalcNextUpdateTime(*forgot_context, events.get()),
