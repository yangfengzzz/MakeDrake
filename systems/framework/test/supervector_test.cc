#include "systems/framework/supervector.h"
...
protected:
    void SetUp() override {
        ...
        supervector_ = std::make_unique<Supervector<double>>(
                std::vector<VectorBase<double>*>{vec1_.get(), vec2_.get(), vec3_.get(), vec4_.get()});
    }
...
TEST_F(SupervectorTest, SetFromVector) {
    auto next_value = BasicVector<double>::Make({10, 11, 12, 13, 14, 15, 16, 17, 18});
    ...
    EXPECT_THROW(supervector_->SetFromVector(Eigen::Vector3d::Zero()), std::exception);
}
