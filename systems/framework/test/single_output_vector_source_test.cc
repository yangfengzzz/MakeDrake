...
public:
    ...
    void DoCalcVectorOutput(const Context<double>& context, Eigen::VectorBlock<Eigen::VectorXd>* output) const final {
...
TEST_F(SingleOutputVectorSourceTest, OutputTest) {
    ...
    EXPECT_EQ(source_->get_output_port(0).Eval(*context_), Eigen::Vector3d::Ones());
}
...
public:
    ...
    template <typename U>
    explicit Convertable(const Convertable<U>&) : Convertable<T>(UseTransmogrify{}) {}

private:
    auto MakeVec() const { return std::make_unique<BasicVector<T>>(VectorX<T>::Constant(kSize, 22.0)); }

    void DoCalcVectorOutput(const Context<T>& context, Eigen::VectorBlock<VectorX<T>>* output) const final {
        *output = Eigen::Vector3d::Ones();
    }
};
