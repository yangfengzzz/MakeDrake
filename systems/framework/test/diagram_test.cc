#include "systems/framework/diagram.h"
...
#include "examples/pendulum/pendulum_plant.h"
...
#include "systems/framework/fixed_input_port_value.h"
#include "systems/framework/leaf_system.h"
#include "systems/framework/output_port.h"
#include "systems/framework/test_utilities/initialization_test_system.h"
#include "systems/framework/test_utilities/pack_value.h"
#include "systems/framework/test_utilities/scalar_conversion.h"
#include "systems/primitives/adder.h"
#include "systems/primitives/constant_value_source.h"
#include "systems/primitives/constant_vector_source.h"
#include "systems/primitives/gain.h"
#include "systems/primitives/integrator.h"
#include "systems/primitives/zero_order_hold.h"
...
public:
    ...
    void AddPeriodicDiscreteUpdate() {
        ...
        this->DeclarePeriodicDiscreteUpdateEvent(default_period, default_offset, &EmptySystem::Noop);
    }

    // Adds a specific periodic discrete update.
    void AddPeriodicDiscreteUpdate(double period, double offset) {
        this->DeclarePeriodicDiscreteUpdateEvent(period, offset, &EmptySystem::Noop);
    }

    EventStatus Noop(const Context<T>&, DiscreteValues<T>*) const { return EventStatus::DidNothing(); }
};
...
public:
    ...
    EmptySystemDiagram(UpdateType num_periodic_discrete_updates, int recursion_depth, bool unique_updates) {
        ...
        switch (num_periodic_discrete_updates) {
            case kTwoUpdatesPerLevel:
                if (unique_updates) {
                    ...
                    sys1->AddPeriodicDiscreteUpdate(recursion_depth + 1, recursion_depth * 3);
                    sys2->AddPeriodicDiscreteUpdate(recursion_depth + 3, recursion_depth * 5);
                }
                break;
            ...
            case kOneUpdatePerLevelSys2:
                if (unique_updates) {
                    ...
                    sys2->AddPeriodicDiscreteUpdate(recursion_depth, recursion_depth * 11);
                }
                break;
        ...
        if (recursion_depth > 0) {
            builder.AddSystem<EmptySystemDiagram>(num_periodic_discrete_updates, recursion_depth - 1, unique_updates);
        }
...
GTEST_TEST(EmptySystemDiagramTest, CheckPeriodicTriggerDiscreteUpdateUnique) {
    ...
    EmptySystemDiagram d_sys2upd_zero(EmptySystemDiagram::kOneUpdatePerLevelSys1, 0, true);
    EmptySystemDiagram d_sys1upd_zero(EmptySystemDiagram::kOneUpdatePerLevelSys2, 0, true);
    EmptySystemDiagram d_bothupd_zero(EmptySystemDiagram::kTwoUpdatesPerLevel, 0, true);
    ASSERT_TRUE(periodic_data = d_sys2upd_zero.GetUniquePeriodicDiscreteUpdateAttribute());
    CheckPeriodAndOffset(periodic_data.value());
    ASSERT_TRUE(periodic_data = d_sys1upd_zero.GetUniquePeriodicDiscreteUpdateAttribute());
    CheckPeriodAndOffset(periodic_data.value());
    ASSERT_TRUE(periodic_data = d_bothupd_zero.GetUniquePeriodicDiscreteUpdateAttribute());
    CheckPeriodAndOffset(periodic_data.value());

    // Check systems with up to three levels of recursion.
    for (int i = 1; i <= 3; ++i) {
        // Create the systems.
        EmptySystemDiagram d_sys1upd(EmptySystemDiagram::kOneUpdatePerLevelSys1, i, true);
        EmptySystemDiagram d_sys2upd(EmptySystemDiagram::kOneUpdatePerLevelSys2, i, true);
        EmptySystemDiagram d_bothupd(EmptySystemDiagram::kTwoUpdatesPerLevel, i, true);
        EmptySystemDiagram d_sys1_last(EmptySystemDiagram::kOneUpdateAtLastLevelSys1, i, true);
        EmptySystemDiagram d_sys2_last(EmptySystemDiagram::kOneUpdateAtLastLevelSys2, i, true);
        EmptySystemDiagram d_both_last(EmptySystemDiagram::kTwoUpdatesAtLastLevel, i, true);

        // All of these should return "true". Check them.
        ASSERT_TRUE(periodic_data = d_sys1upd.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
        ASSERT_TRUE(periodic_data = d_sys2upd.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
        ASSERT_TRUE(periodic_data = d_bothupd.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
        ASSERT_TRUE(periodic_data = d_both_last.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
        ASSERT_TRUE(periodic_data = d_sys1_last.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
        ASSERT_TRUE(periodic_data = d_sys2_last.GetUniquePeriodicDiscreteUpdateAttribute());
        CheckPeriodAndOffset(periodic_data.value());
    }
}
...
GTEST_TEST(EmptySystemDiagramTest, CheckPeriodicTriggerDiscreteUpdate) {
    ...
    EmptySystemDiagram d_sys2upd_zero(EmptySystemDiagram::kOneUpdatePerLevelSys1, 0, false);
    EmptySystemDiagram d_sys1upd_zero(EmptySystemDiagram::kOneUpdatePerLevelSys2, 0, false);
    EmptySystemDiagram d_bothupd_zero(EmptySystemDiagram::kTwoUpdatesPerLevel, 0, false);
    ...
    for (int i = 1; i <= 3; ++i) {
        // Create the systems.
        EmptySystemDiagram d_sys1upd(EmptySystemDiagram::kOneUpdatePerLevelSys1, i, false);
        EmptySystemDiagram d_sys2upd(EmptySystemDiagram::kOneUpdatePerLevelSys2, i, false);
        EmptySystemDiagram d_bothupd(EmptySystemDiagram::kTwoUpdatesPerLevel, i, false);
        EmptySystemDiagram d_sys1_last(EmptySystemDiagram::kOneUpdateAtLastLevelSys1, i, false);
        EmptySystemDiagram d_sys2_last(EmptySystemDiagram::kOneUpdateAtLastLevelSys2, i, false);
        EmptySystemDiagram d_both_last(EmptySystemDiagram::kTwoUpdatesAtLastLevel, i, false);
...
GTEST_TEST(BadDiagramTest, UnconnectedInsideInputPort) {
    ...
    const Context<double>& inside_context = diagram.GetSubsystemContext(diagram.inside(), *context);
...
public:
    ...
    KitchenSinkStateAndParameters() : LeafSystem<T>(systems::SystemTypeTag<KitchenSinkStateAndParameters>{}) {
    ...
    template <typename U>
    explicit KitchenSinkStateAndParameters(const KitchenSinkStateAndParameters<U>&)
...
public:
    explicit ExampleDiagram(int size, bool use_abstract = false, bool use_double_only = false) {
        DiagramBuilder<double> builder;
        ...
        stateless_ = builder.AddSystem<StatelessSystem<double>>(1.0 /* trigger time */,
        ...
        builder.Connect(integrator0_->get_output_port(), integrator1_->get_input_port());
    ...
    KitchenSinkStateAndParameters<double>* kitchen_sink() { return kitchen_sink_; }
...
protected:
    ...
    ContinuousState<double>& GetMutableContinuousState(const System<double>* system) {
        return diagram_->GetMutableSubsystemState(*system, context_.get()).get_mutable_continuous_state();
    }

    // Asserts that output_ is what it should be for the default values
    // of input0_, input1_, and input2_.
    void ExpectDefaultOutputs() {
        Vector3d expected_output0(1 + 8 + 64, 2 + 16 + 128,
                                  4 + 32 + 256);  // B
...
TEST_F(DiagramTest, AreConnected) {
    ...
    EXPECT_TRUE(diagram_->AreConnected(diagram_->adder0()->get_output_port(), diagram_->adder1()->get_input_port(0)));
    EXPECT_TRUE(diagram_->AreConnected(diagram_->adder1()->get_output_port(), diagram_->adder2()->get_input_port(1)));
    EXPECT_TRUE(diagram_->AreConnected(diagram_->adder0()->get_output_port(), diagram_->adder2()->get_input_port(0)));

    // A couple unconnected ports -- but they all belong to the diagram.
    EXPECT_FALSE(diagram_->AreConnected(diagram_->adder0()->get_output_port(), diagram_->adder1()->get_input_port(1)));
    EXPECT_FALSE(diagram_->AreConnected(diagram_->adder1()->get_output_port(), diagram_->adder2()->get_input_port(0)));

    // Test against a port that does *not* belong to the diagram.
    Adder<double> temp_adder(2, kSize);
    EXPECT_FALSE(diagram_->AreConnected(temp_adder.get_output_port(), diagram_->adder1()->get_input_port(1)));
    EXPECT_FALSE(diagram_->AreConnected(diagram_->adder1()->get_output_port(), temp_adder.get_input_port(1)));
}
...
TEST_F(DiagramTest, CheckPortSubscriptions) {
    const Context<double>& adder1_subcontext = diagram_->GetSubsystemContext(*diagram_->adder1(), *context_);
    const OutputPortBase& adder1_oport0 = diagram_->adder1()->get_output_port_base(OutputPortIndex(0));
    const DependencyTracker& adder1_oport0_tracker = adder1_subcontext.get_tracker(adder1_oport0.ticket());
    const Context<double>& adder2_subcontext = diagram_->GetSubsystemContext(*diagram_->adder2(), *context_);
    const InputPortBase& adder2_iport1 = diagram_->adder2()->get_input_port_base(InputPortIndex(1));
    const DependencyTracker& adder2_iport1_tracker = adder2_subcontext.get_tracker(adder2_iport1.ticket());
    ...
    const OutputPortBase& diagram_oport1 = diagram_->get_output_port_base(OutputPortIndex(1));
    const DependencyTracker& diagram_oport1_tracker = context_->get_tracker(diagram_oport1.ticket());
    const OutputPortBase& adder2_oport0 = diagram_->adder2()->get_output_port_base(OutputPortIndex(0));
    const DependencyTracker& adder2_oport0_tracker = adder2_subcontext.get_tracker(adder2_oport0.ticket());
    ...
    const InputPortBase& diagram_iport2 = diagram_->get_input_port_base(InputPortIndex(2));
    const DependencyTracker& diagram_iport2_tracker = context_->get_tracker(diagram_iport2.ticket());
    const InputPortBase& adder1_iport1 = diagram_->adder1()->get_input_port_base(InputPortIndex(1));
    const DependencyTracker& adder1_iport1_tracker = adder1_subcontext.get_tracker(adder1_iport1.ticket());
    ...
    for (auto subsystem : systems) {
        const auto& subcontext = diagram_->GetSubsystemContext(*subsystem, *context_);
...
TEST_F(DiagramTest, Path) {
    ...
    // Just the root.
    EXPECT_EQ("::Unicode Snowman's Favorite Diagram!!1!☃!", diagram_->GetSystemPathname());
}
...
TEST_F(DiagramTest, ValidateContext) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(adder0()->ValidateContext(*context_),
    ...
    // another leaf system.
    const auto& integrator0_context = integrator0()->GetMyContextFromRoot(*context_);
    DRAKE_EXPECT_THROWS_MESSAGE(integrator1()->ValidateContext(integrator0_context),
...
TEST_F(DiagramTest, Graphviz) {
    ...
    EXPECT_THAT(dot, testing::HasSubstr("COLOR=\"blue\" CELLSPACING=\"3\" STYLE=\"rounded\""));
    EXPECT_THAT(dot, testing::HasSubstr("COLOR=\"green\" CELLSPACING=\"3\" STYLE=\"rounded\""));
    // Check that subsystems appear.
    EXPECT_THAT(dot, testing::HasSubstr("\nname=adder1\n"));
    // Check that internal edges appear.
    const SystemBase::GraphvizFragment adder1_frag = diagram_->adder1()->GetGraphvizFragment();
    const SystemBase::GraphvizFragment adder2_frag = diagram_->adder2()->GetGraphvizFragment();
    // [Adder 1, output 0] -> [Adder 2, input 1]
    std::string edge = fmt::format("{}:e -> {}:w", adder1_frag.output_ports.at(0), adder2_frag.input_ports.at(1));
    ...
    edge = fmt::format("{}:e -> {}:w [color=blue]", fragment.input_ports.at(2), adder1_frag.input_ports.at(1));
    EXPECT_THAT(dot, testing::HasSubstr(edge));
    // [Diagram Input 2] -> [Sink, input]
    const SystemBase::GraphvizFragment sink_frag = diagram_->sink()->GetGraphvizFragment();
    edge = fmt::format("{}:e -> {}:w [color=blue]", fragment.input_ports.at(2), sink_frag.input_ports.at(0));
    EXPECT_THAT(dot, testing::HasSubstr(edge));
    // [Adder 2, output 0] -> [Diagram Output 1]
    edge = fmt::format("{}:e -> {}:w [color=green]", adder2_frag.output_ports.at(0), fragment.output_ports.at(1));
    EXPECT_THAT(dot, testing::HasSubstr(edge));
}
...
TEST_F(DiagramTest, GetMutableSubsystemState) {
    State<double>& state_from_context = diagram_->GetMutableSubsystemState(*diagram_->integrator0(), context_.get());
    State<double>& state_from_state =
            diagram_->GetMutableSubsystemState(*diagram_->integrator0(), &context_->get_mutable_state());
...
TEST_F(DiagramTest, EvalOutput) {
    ...
    for (OutputPortIndex i(0); i < diagram_->num_output_ports(); ++i) {
        internal::OutputPortPrerequisite prereq = diagram_->get_output_port(i).GetPrerequisite();
...
TEST_F(DiagramTest, CalcTimeDerivatives) {
    AttachInputs();
    std::unique_ptr<ContinuousState<double>> derivatives = diagram_->AllocateTimeDerivatives();
    ...
    const ContinuousState<double>& integrator0_xcdot = diagram_->GetSubsystemDerivatives(*integrator0(), *derivatives);
    ...
    const ContinuousState<double>& integrator1_xcdot = diagram_->GetSubsystemDerivatives(*integrator1(), *derivatives);
...
TEST_F(DiagramTest, ContinuousStateBelongsWithSystem) {
    ...
    std::unique_ptr<ContinuousState<double>> derivatives = diagram_->AllocateTimeDerivatives();
    DRAKE_EXPECT_NO_THROW(diagram_->CalcTimeDerivatives(*context_, derivatives.get()));

    // Successfully calc using storage that was indirectly created by the system.
    auto temp_context = diagram_->AllocateContext();
    ContinuousState<double>& temp_xc = temp_context->get_mutable_continuous_state();
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(other_diagram.CalcTimeDerivatives(*other_context, derivatives.get()),
...
TEST_F(DiagramTest, GetSubsystemByName) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(diagram_->GetSubsystemByName("not_a_subsystem"),
...
TEST_F(DiagramTest, GetDowncastSubsystemByName) {
    const StatelessSystem<double>& stateless = diagram_->GetDowncastSubsystemByName<StatelessSystem>("stateless");
    EXPECT_EQ(stateless.get_name(), "stateless");

    DRAKE_EXPECT_THROWS_MESSAGE(diagram_->GetDowncastSubsystemByName<EmptySystem>("stateless"),
                                ".*cast.*StatelessSystem.*EmptySystem.*");

    DRAKE_EXPECT_THROWS_MESSAGE(diagram_->GetDowncastSubsystemByName<StatelessSystem>("not_a_subsystem"),
                                "System .* does not have a subsystem named not_a_subsystem. The existing "
                                "subsystems are named \\{adder0, adder1, adder2, stateless, integrator0, "
                                "integrator1, sink, kitchen_sink\\}.");
    ...
    const StatelessSystem<double>& also_stateless =
            diagram_->GetDowncastSubsystemByName<StatelessSystem<double>>("stateless");
...
TEST_F(DiagramTest, CachingChangePropagation) {
    ...
    const Context<double>& adder1_subcontext = diagram_->GetSubsystemContext(*diagram_->adder1(), *context_);
    const OutputPort<double>& adder1_output = diagram_->adder1()->get_output_port();
    auto& adder1_leaf_output = dynamic_cast<const LeafOutputPort<double>&>(adder1_output);
...
TEST_F(DiagramTest, ToAutoDiffXd) {
    ...
    std::unique_ptr<Context<AutoDiffXd>> context = ad_diagram->CreateDefaultContext();
    std::unique_ptr<SystemOutput<AutoDiffXd>> output = ad_diagram->AllocateOutput();
    ...
    for (InputPortIndex i{0}; i < diagram_->num_input_ports(); i++) {
        EXPECT_EQ(diagram_->get_input_port(i).get_name(), ad_diagram->get_input_port(i).get_name());
    }

    // Make sure that the output port names survive type conversion.
    for (OutputPortIndex i{0}; i < diagram_->num_output_ports(); i++) {
        EXPECT_EQ(diagram_->get_output_port(i).get_name(), ad_diagram->get_output_port(i).get_name());
    }
    ...
    auto diagram_with_double_only = std::make_unique<ExampleDiagram>(kSize, use_abstract, use_double_only);
    DRAKE_EXPECT_THROWS_MESSAGE(diagram_with_double_only->ToAutoDiffXd(),
...
TEST_F(DiagramTest, Clone) {
    ...
    EXPECT_TRUE(is_dynamic_castable<DiagramContinuousState<double>>(diagram_->AllocateTimeDerivatives()));
    EXPECT_TRUE(is_dynamic_castable<DiagramDiscreteValues<double>>(diagram_->AllocateDiscreteVariables()));
    const State<double>& state = context_->get_state();
    EXPECT_TRUE(is_dynamic_castable<DiagramState<double>>(&state));
    EXPECT_TRUE(is_dynamic_castable<DiagramContinuousState<double>>(&state.get_continuous_state()));
    EXPECT_TRUE(is_dynamic_castable<DiagramDiscreteValues<double>>(&state.get_discrete_state()));
    ...
    EXPECT_TRUE(is_dynamic_castable<DiagramContinuousState<double>>(&clone_state.get_continuous_state()));
    EXPECT_TRUE(is_dynamic_castable<DiagramDiscreteValues<double>>(&clone_state.get_discrete_state()));
    ...
    Vector3d expected_output0(3 + 8 + 64, 6 + 16 + 128,
                              9 + 32 + 256);  // B
    const BasicVector<double>* output0 = output_->get_vector_data(0);
    ASSERT_TRUE(output0 != nullptr);
    EXPECT_EQ(expected_output0[0], output0->get_value()[0]);
    ...
    Vector3d expected_output1(3 + 8, 6 + 16,
                              9 + 32);     // A
    expected_output1 += expected_output0;  // A + B
    const BasicVector<double>* output1 = output_->get_vector_data(1);
    ASSERT_TRUE(output1 != nullptr);
...
TEST_F(DiagramTest, DerivativesOfStatelessSystemAreEmpty) {
    std::unique_ptr<ContinuousState<double>> derivatives = diagram_->AllocateTimeDerivatives();
    EXPECT_EQ(0, diagram_->GetSubsystemDerivatives(*adder0(), *derivatives).size());
}
...
TEST_F(DiagramTest, DiscreteValuesOfStatelessSystemAreEmpty) {
    std::unique_ptr<DiscreteValues<double>> updates = diagram_->AllocateDiscreteVariables();
    EXPECT_EQ(0, diagram_->GetSubsystemDiscreteValues(*adder0(), *updates).num_groups());
}
...
protected:
    void SetUp() override {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            builder.Connect(subdiagram0_->get_output_port(i), subdiagram1_->get_input_port(i));
        ...
        // Initialize the integrator states.
        Context<double>& d0_context = diagram_->GetMutableSubsystemContext(*subdiagram0_, context_.get());
        Context<double>& d1_context = diagram_->GetMutableSubsystemContext(*subdiagram1_, context_.get());

        State<double>& integrator0_x =
                subdiagram0_->GetMutableSubsystemState(*subdiagram0_->integrator0(), &d0_context);
        integrator0_x.get_mutable_continuous_state().get_mutable_vector()[0] = 3;

        State<double>& integrator1_x =
                subdiagram0_->GetMutableSubsystemState(*subdiagram0_->integrator1(), &d0_context);
        integrator1_x.get_mutable_continuous_state().get_mutable_vector()[0] = 9;

        State<double>& integrator2_x =
                subdiagram1_->GetMutableSubsystemState(*subdiagram1_->integrator0(), &d1_context);
        integrator2_x.get_mutable_continuous_state().get_mutable_vector()[0] = 27;

        State<double>& integrator3_x =
                subdiagram1_->GetMutableSubsystemState(*subdiagram1_->integrator1(), &d1_context);
...
TEST_F(DiagramOfDiagramsTest, DeclaredContextSizes) {
    EXPECT_EQ(diagram_->num_continuous_states(), context_->num_continuous_states());
    EXPECT_EQ(diagram_->num_discrete_state_groups(), context_->num_discrete_state_groups());
    EXPECT_EQ(diagram_->num_abstract_states(), context_->num_abstract_states());
    EXPECT_EQ(diagram_->num_numeric_parameter_groups(), context_->num_numeric_parameter_groups());
    EXPECT_EQ(diagram_->num_abstract_parameters(), context_->num_abstract_parameters());
}
...
TEST_F(DiagramOfDiagramsTest, ScalarConvertAndCheckContextSizes) {
    ...
    EXPECT_EQ(diagram_ad.num_continuous_states(), context_->num_continuous_states());
    EXPECT_EQ(diagram_ad.num_discrete_state_groups(), context_->num_discrete_state_groups());
    EXPECT_EQ(diagram_ad.num_abstract_states(), context_->num_abstract_states());
    EXPECT_EQ(diagram_ad.num_numeric_parameter_groups(), context_->num_numeric_parameter_groups());
    EXPECT_EQ(diagram_ad.num_abstract_parameters(), context_->num_abstract_parameters());
}
...
TEST_F(DiagramOfDiagramsTest, Graphviz) {
    ...
    const SystemBase::GraphvizFragment sub0_frag = subdiagram0_->GetGraphvizFragment();
    const SystemBase::GraphvizFragment sub1_frag = subdiagram1_->GetGraphvizFragment();
...
TEST_F(DiagramOfDiagramsTest, EvalOutput) {
    ...
    //   output2 = 81 (state of integrator1_)
    FixedInputPortValue& port_value = diagram_->get_input_port(0).FixValue(context_.get(), 10.0);
    EXPECT_EQ(1255, diagram_->get_output_port(0).Eval<BasicVector<double>>(*context_)[0]);
    EXPECT_EQ(2501, diagram_->get_output_port(1).Eval<BasicVector<double>>(*context_)[0]);
    EXPECT_EQ(81, diagram_->get_output_port(2).Eval<BasicVector<double>>(*context_)[0]);
    ...
    EXPECT_EQ(1249, diagram_->get_output_port(0).Eval<BasicVector<double>>(*context_)[0]);
    EXPECT_EQ(2489, diagram_->get_output_port(1).Eval<BasicVector<double>>(*context_)[0]);
    EXPECT_EQ(81, diagram_->get_output_port(2).Eval<BasicVector<double>>(*context_)[0]);
}
...
public:
    explicit PublishingSystem(std::function<void(double)> callback) : callback_(callback) {
...
public:
    explicit PublishNumberDiagram(double constant) : Diagram<double>() {
        ...
        constant_ = builder.AddSystem<ConstantVectorSource<double>>(Vector1d{constant});
        constant_->set_name("constant");
        ...
        builder.Connect(constant_->get_output_port(), publisher_->get_input_port(0));
        builder.BuildInto(this);
    }
...
public:
    FeedbackDiagram()
        ...
        Integrator<T>* const integrator = integrator_builder.template AddSystem<Integrator>(kSize);
        integrator->set_name("integrator");
        integrator_builder.ExportInput(integrator->get_input_port());
        integrator_builder.ExportOutput(integrator->get_output_port());
        Diagram<T>* const integrator_diagram = builder.AddSystem(integrator_builder.Build());
        integrator_diagram->set_name("integrator_diagram");

        DiagramBuilder<T> gain_builder;
        Gain<T>* const gain = gain_builder.template AddSystem<Gain>(1.0 /* gain */, kSize);
        gain->set_name("gain");
    ...
    template <typename U>
    explicit FeedbackDiagram(const FeedbackDiagram<U>& other) : Diagram<T>(SystemTypeTag<FeedbackDiagram>{}, other) {}
};
    ...
    EXPECT_THROW(({
                     try {
                         ...
                         EXPECT_THAT(std::string(e.what()),
                                     testing::MatchesRegex(".*convert a .*::FeedbackDiagram<double>.* called with a"
...
public:
    Reduce() {
        const auto& input0 = this->DeclareInputPort(kUseDefaultName, kVectorValued, 1);
        feedthrough_input_ = input0.get_index();
        sink_input_ = this->DeclareInputPort(kUseDefaultName, kVectorValued, 1).get_index();
        this->DeclareVectorOutputPort(kUseDefaultName, 1, &Reduce::CalcFeedthrough, {input0.ticket()});
    }

    const systems::InputPort<double>& get_sink_input() const { return this->get_input_port(sink_input_); }

    const systems::InputPort<double>& get_feedthrough_input() const { return this->get_input_port(feedthrough_input_); }

    void CalcFeedthrough(const Context<double>& context, BasicVector<double>* output) const {
...
public:
    RandomInputSystem() {
        ...
        this->DeclareInputPort("uniform", kVectorValued, 1, RandomDistribution::kUniform);
        this->DeclareInputPort("gaussian", kVectorValued, 1, RandomDistribution::kGaussian);
    }
};
...
GTEST_TEST(RandomInputSystemTest, RandomInputTest) {
    ...
    EXPECT_EQ(diagram->get_input_port(1).get_random_type(), RandomDistribution::kUniform);
    EXPECT_EQ(diagram->get_input_port(2).get_random_type(), RandomDistribution::kGaussian);
}
...
protected:
    [[nodiscard]] SecondOrderStateVector* DoClone() const override { return new SecondOrderStateVector; }
};
...
public:
    SecondOrderStateSystem() {
        ...
        DeclareContinuousState(SecondOrderStateVector{}, 1 /* num_q */, 1 /* num_v */, 0 /* num_z */);
    }

    SecondOrderStateVector* x(Context<double>* context) const {
        return dynamic_cast<SecondOrderStateVector*>(&context->get_mutable_continuous_state_vector());
    }

protected:
    // qdot = 2 * v.
    void DoMapVelocityToQDot(const Context<double>& context,
    ...
    // v = 1/2 * qdot.
    void DoMapQDotToVelocity(const Context<double>& context,
...
public:
    ...
    SecondOrderStateVector* x(Context<double>* context, const SecondOrderStateSystem* subsystem) {
        Context<double>& subsystem_context = GetMutableSubsystemContext(*subsystem, context);
...
GTEST_TEST(SecondOrderStateTest, MapVelocityToQDot) {
    ...
    const VectorBase<double>& v = context->get_continuous_state().get_generalized_velocity();
...
GTEST_TEST(GetSystemsTest, GetSystems) {
    ...
    EXPECT_EQ((std::vector<const System<double>*>{diagram->adder0(), diagram->adder1(), diagram->adder2(),
...
public:
    TestPublishingSystem() {
        this->DeclarePeriodicPublishEvent(kTestPublishPeriod, 0.0, &TestPublishingSystem::HandlePeriodPublish);
...
public:
    ForcedPublishingSystem() { this->DeclareForcedPublishEvent(&ForcedPublishingSystem::PublishHandler); }
    bool published() const { return published_; }
...
public:
    ForcedPublishingSystemDiagram() : Diagram<double>() {
        DiagramBuilder<double> builder;
        publishing_system_one_ = builder.template AddSystem<ForcedPublishingSystem>();
        publishing_system_two_ = builder.template AddSystem<ForcedPublishingSystem>();
        builder.BuildInto(this);
    }
    ForcedPublishingSystem* publishing_system_one() const { return publishing_system_one_; }
    ForcedPublishingSystem* publishing_system_two() const { return publishing_system_two_; }
...
TEST_F(DiscreteStateTest, CalcNextUpdateTimeHold1) {
    ...
    EXPECT_EQ(2.0, time);
    const auto& subevent_collection = diagram_.GetSubsystemCompositeEventCollection(*diagram_.hold1(), *events);
...
TEST_F(DiscreteStateTest, CalcNextUpdateTimeHold2) {
    ...
    EXPECT_EQ(6.0, time);
    {
        const auto& subevent_collection = diagram_.GetSubsystemCompositeEventCollection(*diagram_.hold2(), *events);
        EXPECT_TRUE(subevent_collection.get_discrete_update_events().HasEvents());
    }
...
TEST_F(DiscreteStateTest, UpdateDiscreteVariables) {
    ...
    Context<double>& ctx1 = diagram_.GetMutableSubsystemContext(*diagram_.hold1(), context_.get());
    ctx1.get_mutable_discrete_state(0)[0] = 1001.0;
    Context<double>& ctx2 = diagram_.GetMutableSubsystemContext(*diagram_.hold2(), context_.get());
    ...
    const DiscreteValues<double>& updates1 = diagram_.GetSubsystemDiscreteValues(*diagram_.hold1(), *updates);
    const DiscreteValues<double>& updates2 = diagram_.GetSubsystemDiscreteValues(*diagram_.hold2(), *updates);
    ...
    EventStatus status =
            diagram_.CalcDiscreteVariableUpdate(*context_, events->get_discrete_update_events(), updates.get());
    ...
    diagram_.ApplyDiscreteVariableUpdate(events->get_discrete_update_events(), updates.get(), context_.get());
    ...
    status = diagram_.CalcDiscreteVariableUpdate(*context_, events->get_discrete_update_events(), updates.get());
    EXPECT_TRUE(status.succeeded());
...
TEST_F(DiscreteStateTest, DiscreteUpdateNotificationsAreLocalized) {
    ...
    auto num_notifications = [](const Context<double>& context) {
        return context.get_tracker(SystemBase::xd_ticket()).num_notifications_received();
    };
    ...
    const EventStatus status = diagram_.CalcDiscreteVariableUpdate(*context_, discrete_events, updates.get());
    EXPECT_TRUE(status.succeeded());
    ...
    diagram_.ApplyDiscreteVariableUpdate(discrete_events, updates.get(), context_.get());
...
public:
    SystemWithDiscreteState(int id, double update_period) : id_(id) {
        ...
        if (update_period > 0.0) {
            DeclarePeriodicDiscreteUpdateEvent(update_period, 0, &SystemWithDiscreteState::AddTimeToDiscreteVariable);
        }

        DeclarePerStepDiscreteUpdateEvent(&SystemWithDiscreteState::IncrementCounter);
    }
...
private:
    ...
    void AddTimeToDiscreteVariable(const Context<double>& context, DiscreteValues<double>* discrete_state) const {
        ASSERT_EQ((*discrete_state)[0], context.get_discrete_state(0)[0]);
        (*discrete_state)[0] += context.get_time();
    }

    EventStatus IncrementCounter(const Context<double>&, DiscreteValues<double>*) const {
        ++counter_;
        return EventStatus::Succeeded();
    }
...
GTEST_TEST(DiscreteStateDiagramTest, NumDiscreteStateGroups) {
    ...
    EXPECT_EQ(diagram->num_discrete_state_groups(), context->num_discrete_state_groups());
}
...
GTEST_TEST(DiscreteStateDiagramTest, CalcDiscreteVariableUpdate) {
    ...
    EXPECT_EQ(diagram.CalcNextUpdateTime(*context, events.get()), 2.);
    {
        const auto& subevent_collection =
                diagram.GetSubsystemCompositeEventCollection(diagram.get_sys(kSys1Id), *events);
        EXPECT_TRUE(subevent_collection.get_discrete_update_events().HasEvents());
    }
    {
        const auto& subevent_collection =
                diagram.GetSubsystemCompositeEventCollection(diagram.get_sys(kSys2Id), *events);
    ...
    std::unique_ptr<DiscreteValues<double>> x_buf = diagram.AllocateDiscreteVariables();
    ...
    EventStatus status =
            diagram.CalcDiscreteVariableUpdate(*context, events->get_discrete_update_events(), x_buf.get());
    EXPECT_TRUE(status.succeeded());
    ...
    diagram.ApplyDiscreteVariableUpdate(events->get_discrete_update_events(), x_buf.get(), context.get());
    ...
    for (int i : {kSys1Id, kSys2Id}) {
        const auto& subevent_collection = diagram.GetSubsystemCompositeEventCollection(diagram.get_sys(i), *events);
        EXPECT_TRUE(subevent_collection.get_discrete_update_events().HasEvents());
    }
    ...
    time = 6.0;
    context->SetTime(time);
    status = diagram.CalcDiscreteVariableUpdate(*context, events->get_discrete_update_events(), x_buf.get());
    EXPECT_TRUE(status.succeeded());
...
GTEST_TEST(DiscreteStateDiagramTest, EvalUniquePeriodicDiscreteUpdateErrors) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(no_discrete_updates.EvalUniquePeriodicDiscreteUpdate(*no_discrete_updates_context),
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(two_period_diagram->EvalUniquePeriodicDiscreteUpdate(*two_period_context),
...
GTEST_TEST(DiscreteStateDiagramTest, EvalUniquePeriodicDiscreteUpdate) {
    ...
    context->SetTime(4.);
    const DiscreteValues<double>& result = two_discretes->EvalUniquePeriodicDiscreteUpdate(*context);
...
TEST_F(DiscreteStateTest, Publish) {
    ...
    const EventStatus status = diagram_.Publish(*context_, events->get_publish_events());
    EXPECT_TRUE(status.succeeded());
...
GTEST_TEST(ContinuousStateDiagramTest, IsDifferentialEquationSystem) {
    ...
    EXPECT_FALSE(mixed_discrete_continuous_diagram->IsDifferentialEquationSystem());
}
...
public:
    SystemWithAbstractState(int id, double update_period) : id_(id) {
        DeclarePeriodicUnrestrictedUpdateEvent(update_period, 0, &SystemWithAbstractState::CalcUnrestrictedUpdate);
...
private:
    ...
    void CalcUnrestrictedUpdate(const Context<double>& context, State<double>* state) const {
        double& state_num = state->get_mutable_abstract_state().get_mutable_value(0).get_mutable_value<double>();
...
protected:
    ...
    double get_sys1_abstract_data_as_double() {
        const Context<double>& sys_context = diagram_.GetSubsystemContext(*diagram_.get_mutable_sys1(), *context_);
        return sys_context.get_abstract_state<double>(0);
    }

    double get_sys2_abstract_data_as_double() {
        const Context<double>& sys_context = diagram_.GetSubsystemContext(*diagram_.get_mutable_sys2(), *context_);
        return sys_context.get_abstract_state<double>(0);
    }
...
TEST_F(AbstractStateDiagramTest, CalcUnrestrictedUpdate) {
    ...
    {
        const auto& subevent_collection =
                diagram_.GetSubsystemCompositeEventCollection(diagram_.get_sys(kSys1Id), *events);
        EXPECT_TRUE(subevent_collection.get_unrestricted_update_events().HasEvents());
    }
    {
        const auto& subevent_collection =
                diagram_.GetSubsystemCompositeEventCollection(diagram_.get_sys(kSys2Id), *events);
        EXPECT_FALSE(subevent_collection.get_unrestricted_update_events().HasEvents());
    }
    ...
    diagram_.ApplyUnrestrictedUpdate(events->get_unrestricted_update_events(), x_buf.get(), context_.get());
    ...
    for (int i : {kSys1Id, kSys2Id}) {
        const auto& subevent_collection = diagram_.GetSubsystemCompositeEventCollection(diagram_.get_sys(i), *events);
        EXPECT_TRUE(subevent_collection.get_unrestricted_update_events().HasEvents());
    }

    time = 6.0;
    context_->SetTime(time);
    status = diagram_.CalcUnrestrictedUpdate(*context_, events->get_unrestricted_update_events(), x_buf.get());
    EXPECT_TRUE(status.succeeded());
...
TEST_F(AbstractStateDiagramTest, UnrestrictedUpdateNotificationsAreLocalized) {
    ...
    Context<double>& ctx1 = diagram_.GetMutableSubsystemContext(diagram_.get_sys(kSys1Id), context_.get());
    Context<double>& ctx2 = diagram_.GetMutableSubsystemContext(diagram_.get_sys(kSys2Id), context_.get());
    ...
    auto num_notifications = [](const Context<double>& context) {
        return context.get_tracker(SystemBase::xa_ticket()).num_notifications_received();
    };
    ...
    const EventStatus status = diagram_.CalcUnrestrictedUpdate(*context_, unrestricted_events, updates.get());
    EXPECT_TRUE(status.succeeded());
    ...
    // Selectively apply the update; only hold1 should get notified.
    diagram_.ApplyUnrestrictedUpdate(unrestricted_events, updates.get(), context_.get());
...
protected:
    void SetUp() override {
        ...
        big_diagram_builder.Connect(src->get_output_port(), integrator2_->get_input_port());
        big_diagram_builder.Connect(src->get_output_port(), diagram0_->get_input_port(0));
        big_diagram_builder.Connect(src->get_output_port(), diagram0_->get_input_port(1));

        big_diagram_builder.Connect(src->get_output_port(), diagram1_->get_input_port(0));
...
TEST_F(NestedDiagramContextTest, GetSubsystemContext) {
    ...
    // Checks states.
    EXPECT_EQ(big_diagram_->GetSubsystemContext(*integrator0_, *big_context_).get_continuous_state_vector()[0], 1);
    EXPECT_EQ(big_diagram_->GetSubsystemContext(*integrator1_, *big_context_).get_continuous_state_vector()[0], 2);
    EXPECT_EQ(big_diagram_->GetSubsystemContext(*integrator2_, *big_context_).get_continuous_state_vector()[0], 3);
    EXPECT_EQ(big_diagram_->GetSubsystemContext(*integrator3_, *big_context_).get_continuous_state_vector()[0], 4);
    ...
    EXPECT_EQ(integrator1_->GetMyContextFromRoot(*big_context_).get_continuous_state_vector()[0], 2);
    // Integrator 2 is a direct child.
    EXPECT_EQ(integrator2_->GetMyContextFromRoot(*big_context_).get_continuous_state_vector()[0], 3);
    // Extract a sub-diagram context.
    Context<double>& diagram0_context = diagram0_->GetMyMutableContextFromRoot(&*big_context_);
    // This should fail because this context is not a root context.
    DRAKE_EXPECT_THROWS_MESSAGE(integrator1_->GetMyContextFromRoot(diagram0_context),
                                ".*context must be a root context.*");

    // Should fail because integrator3 is not in diagram0.
    DRAKE_EXPECT_THROWS_MESSAGE(diagram0_->GetSubsystemContext(*integrator3_, diagram0_context),
                                ".*Integrator.*int3.*not contained in.*Diagram.*diagram0.*");

    // Modify through the sub-Diagram context, then read back from root.
    Context<double>& integrator1_context = diagram0_->GetMutableSubsystemContext(*integrator1_, &diagram0_context);
    integrator1_context.get_mutable_continuous_state_vector()[0] = 17.;
    EXPECT_EQ(integrator1_->GetMyContextFromRoot(*big_context_).get_continuous_state_vector()[0], 17.);
}
...
TEST_F(NestedDiagramContextTest, GetSubsystemState) {
    ...
    // Checks state.
    EXPECT_EQ(big_diagram_->GetSubsystemState(*integrator0_, big_state).get_continuous_state().get_vector()[0], 1);
    EXPECT_EQ(big_diagram_->GetSubsystemState(*integrator1_, big_state).get_continuous_state().get_vector()[0], 2);
    EXPECT_EQ(big_diagram_->GetSubsystemState(*integrator2_, big_state).get_continuous_state().get_vector()[0], 3);
    EXPECT_EQ(big_diagram_->GetSubsystemState(*integrator3_, big_state).get_continuous_state().get_vector()[0], 4);
...
TEST_F(NestedDiagramContextTest, CachingChangePropagation) {
    const Context<double>& diagram1_subcontext = big_diagram_->GetSubsystemContext(*diagram1_, *big_context_);
    const Context<double>& integrator3_subcontext = diagram1_->GetSubsystemContext(*integrator3_, diagram1_subcontext);
    const OutputPort<double>& integrator3_output = integrator3_->get_output_port();
    auto& integrator3_leaf_output = dynamic_cast<const LeafOutputPort<double>&>(integrator3_output);
...
GTEST_TEST(MutateSubcontextTest, DiagramRecalculatesOnSubcontextChange) {
    ...
    auto& derivative_tracker = diagram_context->get_tracker(diagram->xcdot_ticket());
    ASSERT_NE(derivative_tracker.cache_entry_value(), nullptr);
    const CacheEntryValue& derivative_cache = *derivative_tracker.cache_entry_value();
    ...
    EXPECT_EQ(diagram->get_output_port(0).Eval<BasicVector<double>>(*diagram_context)[0], 6.);
    ...
    Context<double>& context2 = diagram->GetMutableSubsystemContext(*integ2, &*diagram_context);
    FixedInputPortValue& u2_value = integ2->get_input_port(0).FixValue(&context2, 0.75);
    ...
    Context<double>& context0 = diagram->GetMutableSubsystemContext(*integ0, &*diagram_context);
    Context<double>& context1 = diagram->GetMutableSubsystemContext(*integ1, &*diagram_context);
    ...
    EXPECT_EQ(diagram->get_output_port(0).Eval<BasicVector<double>>(*diagram_context)[0], 17.);
    ...
    EXPECT_EQ(diagram->get_output_port(0).Eval<BasicVector<double>>(*diagram_context)[0],
              17.);  // No change.
    EXPECT_TRUE(derivative_cache.is_out_of_date());
    ...
    DRAKE_EXPECT_NO_THROW(diagram_context->SetTimeStateAndParametersFrom(*diagram_context_clone));
    DRAKE_EXPECT_NO_THROW(diagram_context->SetAccuracy(1e-6));

    // Time & accuracy changes NOT allowed at child (leaf) level.
    DRAKE_EXPECT_THROWS_MESSAGE(context0.SetTime(3.), ".*SetTime.*Time change allowed only.*root.*");
    DRAKE_EXPECT_THROWS_MESSAGE(context0.SetTimeAndContinuousState(4., new_x0),
                                ".*SetTimeAndContinuousState.*Time change allowed only.*root.*");
    DRAKE_EXPECT_THROWS_MESSAGE(context0.SetTimeStateAndParametersFrom(context1),
                                ".*SetTimeStateAndParametersFrom.*Time change allowed only.*root.*");
    DRAKE_EXPECT_THROWS_MESSAGE(context0.SetAccuracy(1e-7), ".*SetAccuracy.*Accuracy change allowed only.*root.*");
}
...
public:
    ...
    void AddPublishEvent() { DeclarePerStepPublishEvent(&PerStepActionTestSystem::PublishHandler); }

    void AddDiscreteUpdateEvent() { DeclarePerStepDiscreteUpdateEvent(&PerStepActionTestSystem::DiscreteHandler); }

    void AddUnrestrictedUpdateEvent() {
        DeclarePerStepUnrestrictedUpdateEvent(&PerStepActionTestSystem::UnrestrictedHandler);
    }
...
private:
    ...
    EventStatus DiscreteHandler(const Context<double>& context, DiscreteValues<double>* discrete_state) const {
        (*discrete_state)[0] = context.get_discrete_state(0)[0] + 1;
        return EventStatus::Succeeded();
    }

    EventStatus UnrestrictedHandler(const Context<double>& context, State<double>* state) const {
        // Depends on discrete state (therefore order of event evaluation).
        int int_num = static_cast<int>(context.get_discrete_state(0)[0]);
        state->get_mutable_abstract_state<std::string>(0) = "wow" + std::to_string(int_num);
        return EventStatus::Succeeded();
    }
...
GTEST_TEST(DiagramEventEvaluation, Propagation) {
    ...
    EventStatus status =
            diagram->CalcUnrestrictedUpdate(*context, events->get_unrestricted_update_events(), tmp_state.get());
    EXPECT_TRUE(status.succeeded());
    context->get_mutable_state().SetFrom(*tmp_state);

    // Does discrete updates second.
    status = diagram->CalcDiscreteVariableUpdate(*context, events->get_discrete_update_events(),
...
public:
    EventStatusTestSystem() {
        DeclareForcedUnrestrictedUpdateEvent(&EventStatusTestSystem::UnrestrictedHandler);
    ...
    void set_unrestricted_severity(EventStatus::Severity severity) { unrestricted_severity_ = severity; }
    void set_discrete_severity(EventStatus::Severity severity) { discrete_severity_ = severity; }
    void set_publish_severity(EventStatus::Severity severity) { publish_severity_ = severity; }
...
private:
    static std::vector<std::string>& events_singleton() {
        static never_destroyed<std::vector<std::string>> global(std::vector<std::string>{});
    ...
    EventStatus MakeStatus(std::string id, EventStatus::Severity severity) const {
        ...
        switch (severity) {
            ...
            case EventStatus::kReachedTermination:
                return EventStatus::ReachedTermination(this, fmt::format("{} terminated", id));
    ...
    EventStatus UnrestrictedHandler(const Context<double>&, State<double>*) const {
        return MakeStatus("unrestricted", unrestricted_severity_);
    }
    EventStatus DiscreteHandler(const Context<double>&, DiscreteValues<double>*) const {
        return MakeStatus("discrete", discrete_severity_);
    }
    EventStatus PublishHandler(const Context<double>&) const { return MakeStatus("publish", publish_severity_); }
...
GTEST_TEST(DiagramEventEvaluation, EventStatusHandling) {
    ...
    auto calc_unrestricted_update = [&context, &state](const auto& dut) {
        return dut->CalcUnrestrictedUpdate(*context, *dut->AllocateForcedUnrestrictedUpdateEventCollection(), &state);
    };
    auto calc_discrete_variable_update = [&context, &discrete_state](const auto& dut) {
        return dut->CalcDiscreteVariableUpdate(*context, *dut->AllocateForcedDiscreteUpdateEventCollection(),
                                               &discrete_state);
    };
    ...
    const std::string all_unrestricted[] = {"sys0 unrestricted", "sys1 unrestricted", "sys2 unrestricted",
                                            "sys3 unrestricted", "sys4 unrestricted"};
    const std::string all_discrete[] = {"sys0 discrete", "sys1 discrete", "sys2 discrete", "sys3 discrete",
                                        "sys4 discrete"};
    const std::string all_publish[] = {"sys0 publish", "sys1 publish", "sys2 publish", "sys3 publish", "sys4 publish"};
    ...
    EXPECT_THAT(take_static_events(), ElementsAreArray(all_unrestricted, /* count = */ 2));
    ...
    EXPECT_THAT(take_static_events(), ElementsAreArray(all_discrete, /* count = */ 3));
    ...
    EXPECT_EQ(discrete_status.message(), "discrete failed");
    EXPECT_THAT(take_static_events(), ElementsAreArray(all_discrete, /* count = */ 4));
}
...
GTEST_TEST(MyEventTest, MyEventTestLeaf) {
    ...
    for (double period : {0.2, 0.0}) {
        ...
        const EventStatus status = dut.Publish(*context, events->get_publish_events());
        EXPECT_TRUE(status.succeeded());
...
public:
    ConstraintTestSystem() : LeafSystem<T>(systems::SystemTypeTag<ConstraintTestSystem>{}) {
        this->DeclareContinuousState(2);
        this->DeclareEqualityConstraint(&ConstraintTestSystem::CalcState0Constraint, 1, "x0");
        this->DeclareInequalityConstraint(&ConstraintTestSystem::CalcStateConstraint, {Vector2d::Zero(), std::nullopt},
                                          "x");
    }

    // Scalar-converting copy constructor.
    template <typename U>
    explicit ConstraintTestSystem(const ConstraintTestSystem<U>& system) : ConstraintTestSystem() {}
    ...
    void CalcState0Constraint(const Context<T>& context, VectorX<T>* value) const {
...
private:
    void DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const override {
        ...
        derivatives->SetFromVector(
                -dynamic_cast<const BasicVector<T>&>(context.get_continuous_state_vector()).get_value());
    }
};
...
GTEST_TEST(DiagramConstraintTest, SystemConstraintsTest) {
    ...
    const SystemConstraint<double>& constraint0 = diagram->get_constraint(SystemConstraintIndex(0));
    ...
    const SystemConstraint<double>& constraint1 = diagram->get_constraint(SystemConstraintIndex(1));
    ...
    const SystemConstraint<double>& constraint2 = diagram->get_constraint(SystemConstraintIndex(2));
    ...
    const SystemConstraint<double>& constraint3 = diagram->get_constraint(SystemConstraintIndex(3));
...
GTEST_TEST(DiagramParametersTest, ParameterTest) {
    ...
    systems::DiagramBuilder<double> builder;
    auto pendulum1 = builder.AddSystem<examples::pendulum::PendulumPlant<double>>();
    auto pendulum2 = builder.AddSystem<examples::pendulum::PendulumPlant<double>>();
    auto constant_torque = builder.AddSystem<ConstantVectorSource<double>>(Vector1d(1.0));
    ...
    // parameters.
    EXPECT_EQ(diagram->num_numeric_parameter_groups(), context->num_numeric_parameter_groups());

    // Get pointers to the parameters.
    auto params1 = dynamic_cast<examples::pendulum::PendulumParams<double>*>(
            &diagram->GetMutableSubsystemContext(*pendulum1, context.get()).get_mutable_numeric_parameter(0));
    auto params2 = dynamic_cast<examples::pendulum::PendulumParams<double>*>(
            &diagram->GetMutableSubsystemContext(*pendulum2, context.get()).get_mutable_numeric_parameter(0));
...
public:
    ...
    void SetRandomState(const Context<double>& context,
                        State<double>* state,
                        RandomGenerator* generator) const override {
        std::normal_distribution<double> normal;
        for (int i = 0; i < context.get_continuous_state_vector().size(); i++) {
            state->get_mutable_continuous_state().get_mutable_vector().SetAtIndex(i, normal(*generator));
        }
    }
    void SetRandomParameters(const Context<double>& context,
        ...
        for (int i = 0; i < context.get_numeric_parameter(0).size(); i++) {
            params->get_mutable_numeric_parameter(0).SetAtIndex(i, uniform(*generator));
        }
    }
};
...
GTEST_TEST(RandomContextTest, SetRandomTest) {
    ...
    diagram->SetDefaultContext(context.get());
    EXPECT_TRUE((state.array() == context->get_continuous_state_vector().CopyToVector().array()).all());
    EXPECT_TRUE((params0.array() == context->get_numeric_parameter(0).get_value().array()).all());
    EXPECT_TRUE((params1.array() == context->get_numeric_parameter(1).get_value().array()).all());
...
private:
    void DoCalcTimeDerivatives(const Context<double>& context, ContinuousState<double>* derivatives) const final {
...
private:
    void DoCalcImplicitTimeDerivativesResidual(const systems::Context<double>& context,
