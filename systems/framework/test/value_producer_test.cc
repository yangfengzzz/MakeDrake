...
public:
    ...
    std::unique_ptr<std::string> AllocateCharlie() const { return std::make_unique<std::string>("charlie"); }
};
...
void (MyClass::*const kBadCalcOne)(const Context<double>&, std::string*) const = nullptr;

// kBadCalcTwo is an null argument for series (2).
std::string (MyClass::*const kBadCalcTwo)(const Context<double>&) const = nullptr;
...
std::unique_ptr<std::string> (MyClass::*const kBadAllocateCharlie)() const = nullptr;
...
TEST_F(ValueProducerTest, DefaultCtor) {
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(empty.Calc(context_, storage.get()), ".* null Calc.*");
    ...
    DRAKE_EXPECT_THROWS_MESSAGE(empty_copy.Calc(context_, storage.get()), ".* null Calc.*");
}
...
TEST_F(ValueProducerTest, CharlieOne) {
    const ValueProducer dut(&my_class_, &MyClass::AllocateCharlie, &MyClass::CalcOne);
    CheckValues(dut, "charlie", "one");
    DRAKE_CHECK_CTOR_NULL((kBadInstance, &MyClass::AllocateCharlie, &MyClass::CalcOne));
...
TEST_F(ValueProducerTest, CharlieTwo) {
    const ValueProducer dut(&my_class_, &MyClass::AllocateCharlie, &MyClass::CalcTwo);
    CheckValues(dut, "charlie", "two");
    DRAKE_CHECK_CTOR_NULL((kBadInstance, &MyClass::AllocateCharlie, &MyClass::CalcTwo));
...
TEST_F(ValueProducerTest, CharlieThree) {
    const ValueProducer dut(&my_class_, &MyClass::AllocateCharlie, std::function(CalcThree));
    CheckValues(dut, "charlie", "three");
    DRAKE_CHECK_CTOR_NULL((kBadInstance, &MyClass::AllocateCharlie, std::function(CalcThree)));
    DRAKE_CHECK_CTOR_NULL((&my_class_, kBadAllocateCharlie, std::function(CalcThree)));
    DRAKE_CHECK_CTOR_NULL((&my_class_, &MyClass::AllocateCharlie, std::function(kBadCalcThree)));
}

TEST_F(ValueProducerTest, CharlieFour) {
    const ValueProducer dut(&my_class_, &MyClass::AllocateCharlie, std::function(CalcFour));
    CheckValues(dut, "charlie", "four");
    DRAKE_CHECK_CTOR_NULL((kBadInstance, &MyClass::AllocateCharlie, std::function(CalcFour)));
    DRAKE_CHECK_CTOR_NULL((&my_class_, kBadAllocateCharlie, std::function(CalcFour)));
    DRAKE_CHECK_CTOR_NULL((&my_class_, &MyClass::AllocateCharlie, std::function(kBadCalcFour)));
}
...
TEST_F(ValueProducerTest, DocumentationExample3) {
    ...
    class MyClazz {
    public:  // This line is omitted from the documentation, for brevity.
        double MyCalc(const Context<double>& context) const { return context.get_time(); }
    };
    MyClazz my_class;
    ValueProducer foo = ValueProducer(&my_class, &MyClazz::MyCalc);
}
