...
public:
    SparseSystem() {
        ...
        this->DeclareVectorOutputPort(kUseDefaultName, kSize, &SparseSystem::CalcY0);
        this->DeclareVectorOutputPort(kUseDefaultName, kSize, &SparseSystem::CalcY1);
        ...
        this->DeclareEqualityConstraint(&SparseSystem::CalcConstraint, kSize, "equality constraint");
        this->DeclareInequalityConstraint(&SparseSystem::CalcConstraint, {Eigen::VectorXd::Zero(kSize), std::nullopt},
                                          "inequality constraint");
    }

    void AddAbstractInputPort() { this->DeclareAbstractInputPort(kUseDefaultName, Value<std::string>{}); }

    void set_discrete_update_is_affine(bool value) { discrete_update_is_affine_ = value; }

private:
    // Calculation function for output port 0.
    void CalcY0(const Context<symbolic::Expression>& context, BasicVector<symbolic::Expression>* y0) const {
    ...
    void CalcY1(const Context<symbolic::Expression>& context, BasicVector<symbolic::Expression>* y1) const {
        const auto& u0 = this->get_input_port(0).Eval(context);
        const auto& u1 = this->get_input_port(1).Eval(context);
    ...
    void CalcConstraint(const Context<symbolic::Expression>& context, VectorX<symbolic::Expression>* value) const {
        *value = context.get_continuous_state_vector().CopyToVector();
    }

    // Implements a time-varying affine dynamics.
    void DoCalcTimeDerivatives(const Context<symbolic::Expression>& context,
                               ContinuousState<symbolic::Expression>* derivatives) const override {
        const auto& u0 = this->get_input_port(0).Eval(context);
        const auto& u1 = this->get_input_port(1).Eval(context);
        const auto& t = context.get_time();
        const Vector2<symbolic::Expression> x = context.get_continuous_state_vector().CopyToVector();
        ...
        const Vector2<symbolic::Expression> xdot = A * t * x + B1 * u0 + B2 * u1 + f0;
        derivatives->SetFromVector(xdot);
    }

    EventStatus CalcDiscreteUpdate(const systems::Context<symbolic::Expression>& context,
                                   systems::DiscreteValues<symbolic::Expression>* discrete_state) const {
        const auto& u0 = this->get_input_port(0).Eval(context);
        const auto& u1 = this->get_input_port(1).Eval(context);
        const Vector2<symbolic::Expression> xd = context.get_discrete_state(0).get_value();
...
protected:
    void SetUp() override { inspector_ = std::make_unique<SystemSymbolicInspector>(system_); }
...
TEST_F(SystemSymbolicInspectorTest, ConstraintTest) {
    ...
    std::transform(constraints.begin(), constraints.end(), std::back_inserter(actual), [](const auto& item) {
