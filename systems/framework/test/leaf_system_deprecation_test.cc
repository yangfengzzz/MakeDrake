...
protected:
    ...
    OutputPort<double>& DeclareDeprecatedOutput(const std::string& message = "") {
        auto& result =
                dut_.DeclareVectorOutputPort(kUseDefaultName, 1, noop_calc_, {SystemBase::all_input_ports_ticket()});
    ...
    const LeafOutputPort<double>::CalcVectorCallback noop_calc_ = [](const Context<double>&, BasicVector<double>*) {};
};
...
TEST_F(LeafSystemDeprecationTest, FixedFromNoSpurious) {
    ...
    other.get_input_port().FixValue(other_context.get(), Eigen::VectorXd::Constant(1, 22.0));
...
public:
    SillyPassThrough() {
        ...
        output_new_ = &this->DeclareVectorOutputPort("output_new", 1, &SillyPassThrough::CalcOutput);
        ...
        output_old_ = &this->DeclareVectorOutputPort("output", 1, &SillyPassThrough::CalcOutput);
    ...
    void CalcOutput(const Context<double>& context, BasicVector<double>* output) const {
        ...
        const Eigen::VectorXd& input = (has_old ? *input_old_ : *input_new_).Eval(context);
