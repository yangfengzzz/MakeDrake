...
protected:
    void SetUp() override {
        port0_value_ = &context_.FixInputPort(InputPortIndex(0), Value<BasicVector<double>>(Eigen::Vector2d(5.0, 6.0)));

        port1_value_ = &context_.FixInputPort(InputPortIndex(1), Value<std::string>("foo"));
...
TEST_F(FixedInputPortTest, RepeatedFixInputPortUsesSameTracker) {
    ...
    const DependencyTicket old_port1_ticket = context_.input_port_ticket(InputPortIndex(1));
    const DependencyTicket old_port1_value_ticket = port1_value_->ticket();

    // Replace the value object for output port 1.
    const FixedInputPortValue& new_port1_value = context_.FixInputPort(InputPortIndex(1), Value<std::string>("bar"));
    ...
    const DependencyTicket new_port1_ticket = context_.input_port_ticket(InputPortIndex(1));
...
TEST_F(FixedInputPortTest, Clone) {
    ...
    EXPECT_EQ(free0->get_vector_value<double>().get_value(), port0_value_->get_vector_value<double>().get_value());
    EXPECT_EQ(free1->get_value().get_value<std::string>(), port1_value_->get_value().get_value<std::string>());
...
TEST_F(FixedInputPortTest, Access) {
    EXPECT_EQ(Vector2<double>(5, 6), port0_value_->get_vector_value<double>().get_value());
...
TEST_F(FixedInputPortTest, Mutation) {
    ...
    EXPECT_EQ(Vector2<double>(7, 8), port0_value_->get_vector_value<double>().get_value());
