...
InputPortBase::InputPortBase(internal::SystemMessageInterface* owning_system,
...
std::unique_ptr<AbstractValue> InputPortBase::Allocate() const {
    ...
    if (value == nullptr) {
        throw std::logic_error(
                fmt::format("InputPort::Allocate(): allocator returned a nullptr for {}.", GetFullDescription()));
    }
    return value;
}

void InputPortBase::ThrowRequiredMissing() const {
    throw std::logic_error(fmt::format("InputPort::Eval(): required {} is not connected", GetFullDescription()));
}
