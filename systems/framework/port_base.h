...
public:
    ...
    void set_deprecation(std::optional<std::string> deprecation) { deprecation_ = std::move(deprecation); }
...
private:
    ...
    // diagram port this will be the diagram, not the leaf system whose port was
    // exported.
    static const SystemMessageInterface& get_system_interface(const PortBase& port) {
    ...
    static std::atomic<bool>* deprecation_already_warned(PortBase* port) { return &port->deprecation_already_warned_; }
};
}  // namespace internal
#endif
