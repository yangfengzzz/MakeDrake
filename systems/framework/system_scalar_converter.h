...
void AddPydrakeConverterFunction(SystemScalarConverter*, const std::function<System<T>*(const System<U>&)>&);
}  // namespace system_scalar_converter_internal
#endif
    ...
    if constexpr (Traits::template supported<T, U>::value) {
        ...
        auto func = [](const void* const other_system_u) -> void* {
            ...
            std::unique_ptr<System<T>> result =
                    system_scalar_converter_internal::Make<subtype_preservation, S, T, U>(other);
