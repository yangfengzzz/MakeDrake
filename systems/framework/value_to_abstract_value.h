...
public:
    ...
    static std::unique_ptr<AbstractValue> ToAbstract(const char* api_name, const ValueType& eigen_value, ...) {
        ...
        throw std::logic_error(
                fmt::format("{}(): Eigen objects and expressions cannot automatically be stored "
    ...
    template <typename ValueType,
              typename =
                      std::enable_if_t<!(std::is_base_of_v<AbstractValue, ValueType> || is_eigen_refable<ValueType>())>>
