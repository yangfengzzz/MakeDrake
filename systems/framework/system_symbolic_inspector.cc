...
SystemSymbolicInspector::SystemSymbolicInspector(const System<symbolic::Expression>& system)
    ...
    for (int i = 0; i < system.num_constraints(); i++) {
        const SystemConstraint<Expression>& constraint = system.get_constraint(SystemConstraintIndex(i));
        const double tol = 0.0;
        constraints_.emplace(constraint.CheckSatisfied(*context_, tol));
    }
}

void SystemSymbolicInspector::InitializeVectorInputs(const System<symbolic::Expression>& system) {
...
void SystemSymbolicInspector::InitializeContinuousState() {
    ...
    VectorBase<symbolic::Expression>& xc = context_->get_mutable_continuous_state_vector();
...
bool SystemSymbolicInspector::IsAbstract(const System<symbolic::Expression>& system,
...
bool SystemSymbolicInspector::IsConnectedInputToOutput(int input_port_index, int output_port_index) const {
    DRAKE_ASSERT(input_port_index >= 0 && input_port_index < static_cast<int>(input_variables_.size()));
    DRAKE_ASSERT(output_port_index >= 0 && output_port_index < static_cast<int>(output_port_types_.size()));
    ...
    const BasicVector<symbolic::Expression>* output_exprs = output_->get_vector_data(output_port_index);
...
bool is_time_invariant(const VectorX<symbolic::Expression>& expressions, const symbolic::Variable& t) {
...
bool SystemSymbolicInspector::IsTimeInvariant() const {
    ...
    for (int i = 0; i < discrete_updates_->num_groups(); ++i) {
        if (!is_time_invariant(discrete_updates_->get_vector(i).get_value(), time_)) {
            return false;
        }
    }
