...
public:
    ...
    void SetDiscreteState(const Eigen::Ref<const VectorX<T>>& xd) {
        if (num_discrete_state_groups() != 1) {
            throw std::logic_error(
                    fmt::format("Context::SetDiscreteState(): expected exactly 1 discrete state "
    ...
    void SetDiscreteState(const DiscreteValues<T>& xd) { get_mutable_discrete_state().SetFrom(xd); }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Context);
