...
#include "systems/framework/system_constraint.h"
...
const typename Diagram<T>::OutputPortLocator& Diagram<T>::get_output_port_locator(OutputPortIndex port_index) const {
    DRAKE_DEMAND(port_index >= 0 && port_index < static_cast<int>(output_port_ids_.size()));
...
const System<T>& Diagram<T>::GetSubsystemByName(std::string_view name) const {
    ...
    throw std::logic_error(
            fmt::format("System {} does not have a subsystem named {}. The existing subsystems "
                        "are named {{{}}}.",
...
std::map<std::string, std::string> FilterGraphvizOptions(const std::map<std::string, std::string>& diagram_options,
...
template <typename T>
typename Diagram<T>::GraphvizFragment Diagram<T>::DoGetGraphvizFragment(const GraphvizFragmentParams& params) const {
    ...
    for (const auto& subsystem : registered_systems_) {
        subsystem_fragments.emplace(subsystem.get(),
                                    subsystem->GetGraphvizFragment(params.max_depth - 1,
...
{}
    ...
    auto print_ports = [](std::vector<std::string>* fragments, std::vector<std::string>* port_ids,
                          const std::string& kind /* "input" or "output" */, const std::string& ports_node_id,
                          const std::string& color, const std::vector<std::string>& labels) {
...
<TABLE BORDER="0" COLOR="{}" CELLSPACING="3" STYLE="rounded">
        ...
        for (int i = 0; i < ssize(labels); ++i) {
            fragments->push_back(fmt::format(R"""(<TR><TD BORDER="1" PORT="{}">{}</TD></TR>
)""",
                                             i, labels[i]));
...
>];
    ...
    if (this->num_input_ports() > 0) {
        ...
        print_ports(&result.fragments, &result.input_ports, "input", inputs_node_id, "blue",
                    GetGraphvizPortLabels(/* input= */ true));
    }
    if (this->num_output_ports() > 0) {
        std::string outputs_node_id = fmt::format("{}out", params.node_id);
        print_ports(&result.fragments, &result.output_ports, "output", outputs_node_id, "green",
...
label=""
    ...
    for (const auto& subsystem : registered_systems_) {
        ...
        std::move(sub_frag.fragments.begin(), sub_frag.fragments.end(), std::back_inserter(result.fragments));
    }
    ...
    for (const auto& edge : connection_map_) {
        ...
        result.fragments.push_back(fmt::format("{}:e -> {}:w\n",
    ...
    for (int i = 0; i < this->num_input_ports(); ++i) {
        for (const auto& port_id : GetInputPortLocators(InputPortIndex(i))) {
            ...
            const std::string& to = subsystem_fragments.at(sub_sys).input_ports.at(sub_index);
            result.fragments.push_back(fmt::format("{}:e -> {}:w [color=blue];\n", from, to));
        }
    }
    for (int i = 0; i < this->num_output_ports(); ++i) {
        ...
        const std::string& from = subsystem_fragments.at(sub_sys).output_ports.at(sub_index);
        const std::string& to = result.output_ports.at(i);
        result.fragments.push_back(fmt::format("{}:e -> {}:w [color=green];\n", from, to));
    }
...
std::vector<std::string> Diagram<T>::GetGraphvizPortLabels(bool input) const {
    return internal::DiagramSystemBaseAttorney::GetGraphvizPortLabels(*this, input);
}
...
void Diagram<T>::AddTriggeredWitnessFunctionToCompositeEventCollection(Event<T>* event,
    ...
    WitnessTriggeredEventData<T>* data = event->template get_mutable_event_data<WitnessTriggeredEventData<T>>();
...
template <typename T>
void Diagram<T>::DoMapQDotToVelocity(const Context<T>& context,
...
template <typename T>
std::string Diagram<T>::GetUnsupportedScalarConversionMessage(const std::type_info& source_type,
                                                              const std::type_info& destination_type) const {
    // Start with the default message for this system.
    std::stringstream result;
    result << SystemBase::GetUnsupportedScalarConversionMessage(source_type, destination_type);
    ...
    for (const auto& system : registered_systems_) {
        ...
        causes.push_back(internal::DiagramSystemBaseAttorney::GetUnsupportedScalarConversionMessage(
...
std::unique_ptr<ContextBase> Diagram<T>::DoAllocateContext() const {
    ...
    for (const auto& connection : connection_map_) {
        ...
        context->SubscribeInputPortToOutputPort(ConvertToContextPortIdentifier(src),
...
const AbstractValue* Diagram<T>::EvalConnectedSubsystemInputPort(const ContextBase& context_base,
    ...
    auto& system = static_cast<const System<T>&>(internal::PortBaseAttorney::get_system_interface(input_port_base));
...
bool Diagram<T>::DiagramHasDirectFeedthrough(
        int input_port, int output_port, std::unordered_map<const System<T>*, std::multimap<int, int>>* memoize) const {
    ...
    while (!active_set.empty()) {
        ...
        if (memoized_feedthroughs == memoize->end()) {
            bool inserted{};
            std::tie(memoized_feedthroughs, inserted) = memoize->emplace(sys, sys->GetDirectFeedthroughs());
            DRAKE_ASSERT(inserted);
        }
        const std::multimap<int, int>& sys_feedthroughs = memoized_feedthroughs->second;
...
template <typename T>
EventStatus Diagram<T>::DispatchPublishHandler(const Context<T>& context,
    ...
    for (SubsystemIndex i(0); i < num_subsystems(); ++i) {
        ...
        if (subinfo.HasEvents()) {
            const Context<T>& subcontext = diagram_context->GetSubsystemContext(i);
            const EventStatus per_subsystem_status = registered_systems_[i]->Publish(subcontext, subinfo);
...
template <typename T>
EventStatus Diagram<T>::DispatchDiscreteVariableUpdateHandler(const Context<T>& context,
    ...
    for (SubsystemIndex i(0); i < num_subsystems(); ++i) {
        ...
        if (subevents.HasEvents()) {
            ...
            const EventStatus per_subsystem_status =
                    registered_systems_[i]->CalcDiscreteVariableUpdate(subcontext, subevents, &subdiscrete);
            overall_status.KeepMoreSevere(per_subsystem_status);
...
template <typename T>
EventStatus Diagram<T>::DispatchUnrestrictedUpdateHandler(const Context<T>& context,
...
template <typename T>
void Diagram<T>::DoApplyUnrestrictedUpdate(const EventCollection<UnrestrictedUpdateEvent<T>>& events,
                                           State<T>* state,
                                           Context<T>* context) const {
    // If this method is called, these are all Diagram objects.
...
BaseStuff* Diagram<T>::GetSubsystemStuff(
    ...
    static_assert(std::is_same_v<BaseStuff, typename std::remove_pointer_t<BaseStuff>>,
...
template <typename T>
void Diagram<T>::DoFindUniquePeriodicDiscreteUpdatesOrThrow(const char* api_name,
                                                            const Context<T>& context,
                                                            std::optional<PeriodicEventData>* timing,
                                                            EventCollection<DiscreteUpdateEvent<T>>* events) const {
    auto& diagram_collection = dynamic_cast<DiagramEventCollection<DiscreteUpdateEvent<T>>&>(*events);

    for (int i = 0; i < num_subsystems(); ++i) {
        const System<T>& sub_system = *registered_systems_[i];
        const Context<T>& sub_context = GetSubsystemContext(sub_system, context);
        EventCollection<DiscreteUpdateEvent<T>>& sub_collection = diagram_collection.get_mutable_subevent_collection(i);
        System<T>::FindUniquePeriodicDiscreteUpdatesOrThrow(api_name, sub_system, sub_context, &*timing,
                                                            &sub_collection);
    }
}

template <typename T>
void Diagram<T>::DoGetPeriodicEvents(const Context<T>& context, CompositeEventCollection<T>* event_info) const {
    auto diagram_context = dynamic_cast<const DiagramContext<T>*>(&context);
    auto info = dynamic_cast<DiagramCompositeEventCollection<T>*>(event_info);
    DRAKE_DEMAND(diagram_context != nullptr);
    DRAKE_DEMAND(info != nullptr);
...
void Diagram<T>::Initialize(std::unique_ptr<Blueprint> blueprint) {
    ...
    DRAKE_DEMAND(blueprint->input_port_ids.size() == blueprint->input_port_names.size());
    ...
    this->set_forced_discrete_update_events(AllocateForcedEventCollection<DiscreteUpdateEvent<T>>(
            &System<T>::AllocateForcedDiscreteUpdateEventCollection));
...
void Diagram<T>::ExportOrConnectInput(const InputPortLocator& port, std::string name) {
    ...
    if (this->HasInputPort(name)) {
        ...
        const InputPort<T>& new_port =
                this->DeclareInputPort(std::move(name), subsystem_input_port.get_data_type(),
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Diagram);
