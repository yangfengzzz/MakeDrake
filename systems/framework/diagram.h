...
#include "systems/framework/diagram_discrete_values.h"
#include "systems/framework/diagram_output_port.h"
...
public:
    ...
    const MyUntemplatizedSystem& GetDowncastSubsystemByName(std::string_view name) const {
        ...
        return *dynamic_pointer_cast_or_throw<const MyUntemplatizedSystem>(&subsystem);
    }
...
protected:
    ...
    /// related to scalar-type conversion support.
    template <typename U>
    Diagram(SystemScalarConverter converter, const Diagram<U>& other) : Diagram(std::move(converter)) {
    ...
    /// in its context.
    void DoMapVelocityToQDot(const Context<T>& context,
    ...
    std::string GetUnsupportedScalarConversionMessage(const std::type_info& source_type,
...
private:
    ...
    bool DiagramHasDirectFeedthrough(int input_port,
                                     int output_port,
                                     std::unordered_map<const System<T>*, std::multimap<int, int>>* memoize) const;
    ...
    void DoGetPerStepEvents(const Context<T>& context, CompositeEventCollection<T>* event_info) const final;

    void DoGetInitializationEvents(const Context<T>& context, CompositeEventCollection<T>* event_info) const final;
    ...
    typename DiagramContext<T>::InputPortIdentifier ConvertToContextPortIdentifier(
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Diagram);
