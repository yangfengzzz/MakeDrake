...
template <typename T>
void Context<T>::PropagateTimeChange(Context<T>* context,
                                     const T& time,
...
void Context<T>::init_continuous_state(std::unique_ptr<ContinuousState<T>> xc) {
    ...
    if (xc->num_q() == 0) {
        get_mutable_tracker(DependencyTicket(internal::kQTicket)).suppress_notifications();
    }
    if (xc->num_v() == 0) {
        get_mutable_tracker(DependencyTicket(internal::kVTicket)).suppress_notifications();
    }
    if (xc->num_z() == 0) {
        get_mutable_tracker(DependencyTicket(internal::kZTicket)).suppress_notifications();
    }
    if (xc->size() == 0) {  // i.e. nq+nv+nz
        get_mutable_tracker(DependencyTicket(internal::kXcTicket)).suppress_notifications();
    }
...
void Context<T>::init_discrete_state(std::unique_ptr<DiscreteValues<T>> xd) {
    // Don't propagate notifications through variables we don't have.
    if (xd->num_groups() == 0) {
        get_mutable_tracker(DependencyTicket(internal::kXdTicket)).suppress_notifications();
    }
...
void Context<T>::init_abstract_state(std::unique_ptr<AbstractValues> xa) {
    // Don't propagate notifications through variables we don't have.
    if (xa->size() == 0) {
        get_mutable_tracker(DependencyTicket(internal::kXaTicket)).suppress_notifications();
    }
...
void Context<T>::ThrowIfNotRootContext(const char* func_name, const char* quantity) const {
    if (!is_root_context()) {
        throw std::logic_error(fmt::format("{}(): {} change allowed only in the root Context.", func_name, quantity));
    }
}
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::Context);
