...
ContinuousState<T>::ContinuousState(std::unique_ptr<VectorBase<T>> state, int num_q, int num_v, int num_z) {
    ...
    if (state_->size() != num_q + num_v + num_z) {
        throw std::out_of_range("Continuous state of size " + std::to_string(state_->size()) +
...
DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::ContinuousState);
