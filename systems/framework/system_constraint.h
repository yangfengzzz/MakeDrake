...
private:
    ...
    void MaybeValidateSystemIdsMatch(const Context<T>& context) const {
        DRAKE_DEMAND(!system_id_.has_value() || *system_id_ == context.get_system_id());
    }
...
template <>
inline const SystemConstraintCalc<AutoDiffXd>& ExternalSystemConstraint::do_get_calc() const {
    return calc_autodiffxd_;
}

template <>
inline const SystemConstraintCalc<symbolic::Expression>& ExternalSystemConstraint::do_get_calc() const {
