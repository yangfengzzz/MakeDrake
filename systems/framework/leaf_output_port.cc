    ...
    if (proposed.type_info() != value.type_info()) {
        throw std::logic_error(
                fmt::format("OutputPort::Calc(): expected output type {} "
                            "but got {} for {}.",
                            value.GetNiceTypeName(), proposed.GetNiceTypeName(), PortBase::GetFullDescription()));
    }
}

}  // namespace drake::systems

DRAKE_DEFINE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::LeafOutputPort);
