...
protected:
    ...
    /// "Example using drake::systems::VectorSystem as the base class".
    VectorSystem(SystemScalarConverter converter,
                 int input_size,
                 int output_size,
        ...
        this->DeclareForcedDiscreteUpdateEvent(&VectorSystem<T>::CalcDiscreteUpdate);
    }
    ...
    void DoCalcTimeDerivatives(const Context<T>& context, ContinuousState<T>* derivatives) const final {
        ...
        const VectorX<T>& state_vector = dynamic_cast<const BasicVector<T>&>(state_base).value();
        const Eigen::VectorBlock<const VectorX<T>> state_block = state_vector.head(state_vector.rows());
    ...
    void CalcVectorOutput(const Context<T>& context, BasicVector<T>* output) const {
        ...
        static const never_destroyed<VectorX<T>> empty_vector(0);
        const VectorX<T>& input_vector = should_eval_input ? EvalVectorInput(context) : empty_vector.access();
        const Eigen::VectorBlock<const VectorX<T>> input_block = input_vector.head(input_vector.rows());
    ...
    virtual void DoCalcVectorOutput(const Context<T>& context,
    ...
    /// and throws an exception otherwise.
    virtual void DoCalcVectorTimeDerivatives(const Context<T>& context,
                                             const Eigen::VectorBlock<const VectorX<T>>& input,
                                             const Eigen::VectorBlock<const VectorX<T>>& state,
    ...
    void DeclarePeriodicDiscreteUpdate(double period_sec, double offset_sec) {
        this->DeclarePeriodicDiscreteUpdateEvent(period_sec, offset_sec, &VectorSystem<T>::CalcDiscreteUpdate);
    }
...
DRAKE_DECLARE_CLASS_TEMPLATE_INSTANTIATIONS_ON_DEFAULT_SCALARS(class ::drake::systems::VectorSystem);
