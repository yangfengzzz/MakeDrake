...
#include "math/continuous_algebraic_riccati_equation.h"
...
LinearQuadraticRegulatorResult LinearQuadraticRegulator(const Eigen::Ref<const Eigen::MatrixXd>& A,
    ...
    if (N.rows() != 0) {
        ...
        if (F.rows() == 0) {
            ...
            const Eigen::MatrixXd Sy = math::ContinuousAlgebraicRiccatiEquation((*P) * A1 * P->transpose(), (*P) * B,
                                                                                (*P) * Q1 * P->transpose(), R_cholesky);
            ...
            ret.K = R_cholesky.solve(B.transpose() * ret.S + N.transpose() * P->transpose() * (*P));
        }
    } else {
        if (F.rows() == 0) {
            ...
            ret.S = P->transpose() *
                    math::ContinuousAlgebraicRiccatiEquation((*P) * A * P->transpose(), (*P) * B,
...
std::unique_ptr<systems::LinearSystem<double>> LinearQuadraticRegulator(const LinearSystem<double>& system,
                                                                        const Eigen::Ref<const Eigen::MatrixXd>& Q,
                                                                        const Eigen::Ref<const Eigen::MatrixXd>& R,
                                                                        const Eigen::Ref<const Eigen::MatrixXd>& N) {
    ...
    LinearQuadraticRegulatorResult lqr_result =
            (system.time_period() == 0.0) ? LinearQuadraticRegulator(system.A(), system.B(), Q, R, N)
                                          : DiscreteTimeLinearQuadraticRegulator(system.A(), system.B(), Q, R);

    // Return the controller: u = -Kx.
    return std::make_unique<systems::LinearSystem<double>>(Eigen::Matrix<double, 0, 0>::Zero(),   // A
...
std::unique_ptr<systems::AffineSystem<double>> LinearQuadraticRegulator(const System<double>& system,
    ...
    // LQR design).
    auto linear_system = Linearize(system, context, InputPortIndex{input_port_index}, OutputPortSelection::kNoOutput);
    ...
    LinearQuadraticRegulatorResult lqr_result =
            (linear_system->time_period() == 0.0)
                    ...
                    : DiscreteTimeLinearQuadraticRegulator(linear_system->A(), linear_system->B(), Q, R);

    const Eigen::VectorXd& x0 = (linear_system->time_period() == 0.0)
    ...
    return std::make_unique<systems::AffineSystem<double>>(Eigen::Matrix<double, 0, 0>::Zero(),   // A
                                                           Eigen::MatrixXd::Zero(0, num_states),  // B
