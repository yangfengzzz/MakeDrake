#include "systems/controllers/joint_stiffness_controller.h"
...
template <typename T>
JointStiffnessController<T>::JointStiffnessController(std::unique_ptr<multibody::MultibodyPlant<T>> owned_plant,
    ...
    input_port_index_estimated_state_ =
            this->DeclareInputPort("estimated_state", kVectorValued, num_states).get_index();

    input_port_index_desired_state_ = this->DeclareInputPort("desired_state", kVectorValued, num_states).get_index();
    ...
    output_port_index_force_ =
            this->DeclareVectorOutputPort("generalized_force", num_q, &JointStiffnessController<T>::CalcOutputForce,
    ...
    plant_context_cache_index_ =
            this->DeclareCacheEntry("plant_context_cache", *plant_context,
                                    ...
                                    {this->input_port_ticket(get_input_port_estimated_state().get_index())})
                    .cache_index();

    // Declare external forces cache entry
    applied_forces_cache_index_ = this->DeclareCacheEntry("applied_forces_cache", MultibodyForces<T>(*plant_),
...
template <typename T>
JointStiffnessController<T>::JointStiffnessController(const MultibodyPlant<T>& plant,
                                                      const Eigen::Ref<const Eigen::VectorXd>& kp,
                                                      const Eigen::Ref<const Eigen::VectorXd>& kd)
    : JointStiffnessController(nullptr, &plant, kp, kd) {}

template <typename T>
JointStiffnessController<T>::JointStiffnessController(std::unique_ptr<multibody::MultibodyPlant<T>> plant,
                                                      const Eigen::Ref<const Eigen::VectorXd>& kp,
                                                      const Eigen::Ref<const Eigen::VectorXd>& kd)
...
template <typename T>
template <typename U>
JointStiffnessController<T>::JointStiffnessController(const JointStiffnessController<U>& other)
    : JointStiffnessController(systems::System<U>::template ToScalarType<T>(*other.plant_), other.kp_, other.kd_) {}

template <typename T>
JointStiffnessController<T>::~JointStiffnessController() = default;

template <typename T>
void JointStiffnessController<T>::SetMultibodyContext(const Context<T>& context, Context<T>* plant_context) const {
    const VectorX<T>& x = get_input_port_estimated_state().Eval(context);
    plant_->SetPositionsAndVelocities(plant_context, x);
}

template <typename T>
void JointStiffnessController<T>::CalcMultibodyForces(const Context<T>& context,
                                                      MultibodyForces<T>* cache_value) const {
...
template <typename T>
void JointStiffnessController<T>::CalcOutputForce(const Context<T>& context, BasicVector<T>* output) const {
    const int num_q = plant_->num_positions();
    ...
    const auto& applied_forces =
            this->get_cache_entry(applied_forces_cache_index_).template Eval<MultibodyForces<T>>(context);

    // Compute inverse dynamics with zero generalized accelerations.
    // ID(q, v, v̇)  = M(q)v̇ + C(q, v)v - tau_app
    // So with v̇ = 0 we get:
    // ID(q, v, 0) = C(q,v)v - tau_app(q).
    VectorX<T> tau = plant_->CalcInverseDynamics(plant_context, VectorX<T>::Zero(num_q), /* vdot = 0 */
                                                 applied_forces);
