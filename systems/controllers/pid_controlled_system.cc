...
PidControlledSystem<T>::PidControlledSystem(std::unique_ptr<System<T>> plant,
    ...
    : state_output_port_index_(state_output_port_index), plant_input_port_index_{plant_input_port_index} {
    ...
    const MatrixX<double> selector =
            MatrixX<double>::Identity(plant->get_input_port(plant_input_port_index_).size() * 2,
...
PidControlledSystem<T>::PidControlledSystem(std::unique_ptr<System<T>> plant,
    ...
    : PidControlledSystem(std::move(plant),
                          MatrixX<double>::Identity(2 * Kp.size(), 2 * Kp.size()),
                          Kp,
                          Ki,
                          Kd,
...
template <typename T>
void PidControlledSystem<T>::Initialize(std::unique_ptr<System<T>> plant,
                                        const MatrixX<double>& feedback_selector,
                                        const Eigen::VectorXd& Kp,
                                        const Eigen::VectorXd& Ki,
                                        const Eigen::VectorXd& Kd) {
    ...
    auto input_ports = ConnectController(plant_->get_input_port(plant_input_port_index_),
                                         plant_->get_output_port(state_output_port_index_), feedback_selector, Kp, Ki,
                                         Kd, &builder);
...
template <typename T>
typename PidControlledSystem<T>::ConnectResult PidControlledSystem<T>::ConnectController(
        ...
        const Eigen::VectorXd& Kd,
        DiagramBuilder<T>* builder) {
    auto controller = builder->template AddSystem<PidController<T>>(feedback_selector, Kp, Ki, Kd);

    auto plant_input_adder = builder->template AddSystem<Adder<T>>(2, plant_input.size());

    builder->Connect(plant_output, controller->get_input_port_estimated_state());
    builder->Connect(controller->get_output_port_control(), plant_input_adder->get_input_port(0));
    builder->Connect(plant_input_adder->get_output_port(), plant_input);

    return ConnectResult{plant_input_adder->get_input_port(1), controller->get_input_port_desired_state()};
}
...
typename PidControlledSystem<T>::ConnectResult PidControlledSystem<T>::ConnectController(
    ...
    return ConnectController(plant_input, plant_output,
                             MatrixX<double>::Identity(plant_output.size(), plant_output.size()), Kp, Ki, Kd, builder);
}

template <typename T>
typename PidControlledSystem<T>::ConnectResult PidControlledSystem<T>::ConnectControllerWithInputSaturation(
        const InputPort<T>& plant_input,
        const OutputPort<T>& plant_output,
        const MatrixX<double>& feedback_selector,
        const Eigen::VectorXd& Kp,
        ...
        DiagramBuilder<T>* builder) {
    auto saturation = builder->template AddSystem<Saturation<T>>(min_plant_input, max_plant_input);
    builder->Connect(saturation->get_output_port(), plant_input);

    return PidControlledSystem<T>::ConnectController(saturation->get_input_port(), plant_output, feedback_selector, Kp,
                                                     Ki, Kd, builder);
}
