...
LinearModelPredictiveController<T>::LinearModelPredictiveController(
        ...
        double time_period,
        double time_horizon)
    : state_input_index_(this->DeclareVectorInputPort(kUseDefaultName, Q.cols()).get_index()),
      control_output_index_(
              this->DeclareVectorOutputPort(kUseDefaultName, R.cols(), &LinearModelPredictiveController<T>::CalcControl)
                      .get_index()),
...
template <typename T>
void LinearModelPredictiveController<T>::CalcControl(const Context<T>& context, BasicVector<T>* control) const {
    const VectorX<T>& current_state = get_state_port().Eval(context);

    const Eigen::VectorXd current_input = SetupAndSolveQp(*base_context_, current_state);
...
template <typename T>
EventStatus LinearModelPredictiveController<T>::DoNothingButPretendItWasSomething(const Context<T>&,
                                                                                  DiscreteValues<T>*) const {
    return EventStatus::Succeeded();
}

template <typename T>
VectorX<T> LinearModelPredictiveController<T>::SetupAndSolveQp(const Context<T>& base_context,
                                                               const VectorX<T>& current_state) const {
    DRAKE_DEMAND(linear_model_ != nullptr);

    const int kNumSampleTimes = static_cast<int>(time_horizon_ / time_period_ + 0.5);

    DirectTranscription dirtran(linear_model_.get(), *base_context_, kNumSampleTimes);
    auto& prog = dirtran.prog();

    const auto state_error = dirtran.state();
    const auto input_error = dirtran.input();

    dirtran.AddRunningCost(state_error.transpose() * Q_ * state_error + input_error.transpose() * R_ * input_error);

    const VectorX<T> state_ref = base_context.get_discrete_state().get_vector().CopyToVector();
