#include "systems/controllers/inverse_dynamics_controller.h"
...
#include "systems/controllers/test_utilities/compute_torque.h"
...
protected:
    void SetPidGains(VectorX<double>* kp, VectorX<double>* ki, VectorX<double>* kd) {
        *kp << 1, 2, 3, 4, 5, 6, 7;
    ...
    void ConfigTestAndCheck(InverseDynamicsController<double>* test_sys,
        ...
        const MultibodyPlant<double>& robot_plant = *test_sys->get_multibody_plant_for_control();
        ...
        // Connects inputs.
        VectorX<double> state_input(robot_plant.num_positions() + robot_plant.num_velocities());
        state_input << q, v;

        VectorX<double> reference_state_input(robot_plant.num_positions() + robot_plant.num_velocities());
        ...
        test_sys->get_input_port_estimated_state().FixValue(inverse_dynamics_context.get(), state_input);
        test_sys->get_input_port_desired_state().FixValue(inverse_dynamics_context.get(), reference_state_input);
        test_sys->get_input_port_desired_acceleration().FixValue(inverse_dynamics_context.get(),
                                                                 reference_acceleration_input);
        ...
        VectorX<double> vd_d = (kp.array() * (q_r - q).array()).matrix() + (kd.array() * (v_r - v).array()).matrix() +
                               (ki.array() * q_int.array()).matrix() + vd_r;

        std::unique_ptr<Context<double>> workspace_context =
                robot_context == nullptr ? robot_plant.CreateDefaultContext() : robot_context->Clone();
        VectorX<double> expected_torque =
                controllers_test::ComputeTorque(robot_plant, q, v, vd_d, workspace_context.get());

        // Checks the expected and computed gravity torque.
        const BasicVector<double>* output_vector = output->get_vector_data(0);
        EXPECT_TRUE(CompareMatrices(expected_torque, output_vector->get_value(), 1e-10, MatrixCompareType::absolute));
    }
};
...
TEST_F(InverseDynamicsControllerTest, TestTorqueWithReferencedPlant) {
    ...
    auto dut = std::make_unique<InverseDynamicsController<double>>(*robot, kp, ki, kd,
    ...
    dut = std::make_unique<InverseDynamicsController<double>>(*robot, kp, ki, kd, true, custom_context.get());
...
TEST_F(InverseDynamicsControllerTest, TestTorqueWithOwnedPlant) {
    ...
    auto dut = std::make_unique<InverseDynamicsController<double>>(std::move(robot), kp, ki, kd,
                                                                   true /* expose reference acceleration port */);
...
TEST_F(InverseDynamicsControllerTest, TestTorqueWithOwnedPlantAndCustomContext) {
    auto robot = std::make_unique<MultibodyPlant<double>>(0.0);
    multibody::Parser(robot.get())
            .AddModelsFromUrl(
                    "package://drake_models/iiwa_description/sdf/"
    ...
    auto dut = std::make_unique<InverseDynamicsController<double>>(std::move(robot), kp, ki, kd, true,
                                                                   custom_context.get());
...
GTEST_TEST(AdditionalInverseDynamicsTest, ScalarConversion) {
    ...
    VectorXd kp = VectorXd::Constant(dim, 0.12), ki = VectorXd::Constant(dim, 0.34), kd = VectorXd::Constant(dim, 0.56);
    ...
    // Check the PID gains.
    const auto* pid_ad = dynamic_cast<const PidController<AutoDiffXd>*>(&idc_ad->GetSubsystemByName("pid"));
    ...
    InverseDynamicsController<double> idc_with_ownership(std::move(mbp), kp, ki, kd, true);
    ...
    EXPECT_EQ(idc_ad->get_input_port(0).size(), num_states);
    // Check the PID gains.
    pid_ad = dynamic_cast<const PidController<AutoDiffXd>*>(&idc_ad->GetSubsystemByName("pid"));
    ASSERT_NE(pid_ad, nullptr);
    EXPECT_TRUE(CompareMatrices(pid_ad->get_Kp_vector(), kp));
    EXPECT_TRUE(CompareMatrices(pid_ad->get_Ki_vector(), ki));
    EXPECT_TRUE(CompareMatrices(pid_ad->get_Kd_vector(), kd));
