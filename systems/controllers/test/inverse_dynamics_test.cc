...
protected:
    void Init(std::unique_ptr<MultibodyPlant<double>> plant,
        ...
        multibody_context_ =
                (plant_context == nullptr) ? multibody_plant_->CreateDefaultContext() : std::move(plant_context);
        inverse_dynamics_ =
                make_unique<InverseDynamics<double>>(multibody_plant_.get(), mode, multibody_context_.get());
        FinishInit(mode);
    }
    ...
    void CheckGravityTorque(const Eigen::VectorXd& position) {
        CheckTorque(position, VectorXd::Zero(num_velocities()), VectorXd::Zero(num_velocities()));
    }
    ...
    void CheckTorque(const Eigen::VectorXd& position,
        ...
        inverse_dynamics_->get_input_port_estimated_state().FixValue(inverse_dynamics_context_.get(), state_input);

        if (!inverse_dynamics_->is_pure_gravity_compensation()) {
            inverse_dynamics_->get_input_port_desired_acceleration().FixValue(inverse_dynamics_context_.get(), vd_d);
        }
        ...
        expected_torque =
                controllers_test::ComputeTorque(*multibody_plant_, position, velocity, vd_d, multibody_context_.get());
    ...
    bool GravityModeled(const VectorXd& q) const {
        ...
        const VectorXd tau_g = multibody_plant_->CalcGravityGeneralizedForces(*multibody_context_);
...
TEST_F(InverseDynamicsTest, InverseDynamicsTest) {
    ...
    mbp->mutable_gravity_field().set_gravity_vector(-9.8 * Vector3<double>::UnitZ());
    mbp->Finalize();
    Init(std::move(mbp), InverseDynamics<double>::InverseDynamicsMode::kInverseDynamics);
...
TEST_F(InverseDynamicsTest, InverseDynamicsWithCustomContextTest) {
    ...
    // Transfer ownership.
    Init(std::move(mbp), InverseDynamics<double>::InverseDynamicsMode::kInverseDynamics, std::move(custom_context));
...
TEST_F(InverseDynamicsTest, GravityCompensationTest) {
    auto mbp = std::make_unique<MultibodyPlant<double>>(0.0);
    const std::string url = "package://drake_models/iiwa_description/sdf/iiwa14_no_collision.sdf";
    ...
    mbp->Finalize();
    Init(std::move(mbp), InverseDynamics<double>::InverseDynamicsMode::kGravityCompensation);
...
GTEST_TEST(AdditionalInverseDynamicsTest, ScalarConversion) {
    ...
    InverseDynamics<double> id_with_modified_mass(std::move(mbp), InverseDynamics<double>::kGravityCompensation,
                                                  custom_context.get());
    ...
    id_double->get_input_port_estimated_state().FixValue(id_double_context.get(), state_input);
    auto output = id_double->AllocateOutput();
    id_double->CalcOutput(*id_double_context, output.get());
    VectorXd expected_torque;
    expected_torque = controllers_test::ComputeTorque(*mbp_copy, robot_position, Eigen::VectorXd::Zero(7),
                                                      Eigen::VectorXd::Zero(7), custom_context.get());
    auto output_vector = output->get_vector_data(0);
    EXPECT_TRUE(CompareMatrices(expected_torque, output_vector->get_value(), 1e-14, MatrixCompareType::absolute));
}
