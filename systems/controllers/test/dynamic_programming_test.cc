#include "systems/controllers/dynamic_programming.h"
...
GTEST_TEST(FittedValueIterationTest, SingleIntegrator) {
    ...
    const math::BarycentricMesh<double>::MeshGrid state_grid({{-4., -3., -2., -1., 0., 1., 2., 3., 4.}});
    ...
    std::tie(policy, cost_to_go_values) =
            FittedValueIteration(&simulator, cost_function, state_grid, input_grid, time_step);
...
GTEST_TEST(FittedValueIterationTest, PeriodicBoundary) {
    ...
    options.periodic_boundary_conditions.push_back(DynamicProgrammingOptions::PeriodicBoundaryCondition(0, -4., 4.));

    std::unique_ptr<BarycentricMeshSystem<double>> policy;
    Eigen::RowVectorXd cost_to_go_values;
    std::tie(policy, cost_to_go_values) =
            FittedValueIteration(&simulator, cost_function, state_grid, input_grid, time_step, options);
...
void VisualizationCallback(int iteration,
    ...
    Eigen::Map<const Eigen::MatrixXd> J(cost_to_go.data(), Qbins.size(), Qdotbins.size());
...
GTEST_TEST(FittedValueIteration, MultibodyPlant) {
    ...
    std::tie(mbp, scene_graph) = multibody::AddMultibodyPlantSceneGraph(&builder, 0.0);

    multibody::Parser(mbp, scene_graph).AddModels(FindResourceOrThrow("drake/examples/pendulum/Pendulum.urdf"));
    mbp->Finalize();

    // Export an input that we don't need, and export it first, just to test the
    // input_port_index option.
    builder.ExportInput(mbp->get_applied_generalized_force_input_port(), "extra_input");
    ...
    options.periodic_boundary_conditions = {DynamicProgrammingOptions::PeriodicBoundaryCondition(0, 0, 2 * M_PI)};
...
GTEST_TEST(LinearProgrammingTest, SingleIntegrator) {
    ...
    const auto cost_to_go_function = [](const Eigen::Ref<const Eigen::VectorXd>& state,
    ...
    Eigen::VectorXd J = LinearProgrammingApproximateDynamicProgramming(&simulator, cost_function, cost_to_go_function,
                                                                       kNumParameters, state_samples, input_samples,
                                                                       time_step, options);
