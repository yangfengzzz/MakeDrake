...
GTEST_TEST(testQPInverseDynamicsController, testPoseSetpoint) {
    ...
    // Desired orientation
    const math::RigidTransform<double> pose_d(AngleAxis<double>(ang_d, vec_d), Vector3<double>::Zero());

    // Set Kp to 1, and everything else to zeros, so the computed acceleration
    // is the rotation difference.
    CartesianSetpoint<double> setpoint(pose_d.GetAsIsometry3(), Vector6<double>::Zero(), Vector6<double>::Zero(),
    ...
    for (double ang = ang_d; ang < ang_d + 2 * M_PI + 0.1; ang += 0.1) {
        ...
        acc = setpoint.ComputeTargetAcceleration(pose.GetAsIsometry3(), Vector6<double>::Zero());
        ...
        EXPECT_TRUE(CompareMatrices(acc, expected, 1e-8, MatrixCompareType::absolute));
    }
}
