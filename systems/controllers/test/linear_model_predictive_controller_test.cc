#include "systems/controllers/linear_model_predictive_controller.h"

#include <gtest/gtest.h>

#include "common/test_utilities/eigen_matrix_compare.h"
#include "math/discrete_algebraic_riccati_equation.h"
...
protected:
    void SetUp() override {
        ...
        std::unique_ptr<LinearSystem<double>> system = std::make_unique<LinearSystem<double>>(A, B, C, D, kTimeStep);

        // Nominal, fixed reference condition.
        const Eigen::Vector2d x0 = Eigen::Vector2d::Zero();
        const Vector1d u0 = Vector1d::Zero();

        std::unique_ptr<Context<double>> system_context = system->CreateDefaultContext();
        system->get_input_port().FixValue(system_context.get(), u0);
        system_context->SetDiscreteState(0, x0);

        dut_.reset(new LinearModelPredictiveController<double>(std::move(system), std::move(system_context), Q_, R_,
...
TEST_F(TestMpcWithDoubleIntegrator, TestAgainstInfiniteHorizonSolution) {
    ...
    const Eigen::Matrix<double, 1, 2> K = -(R_ + B.transpose() * S * B).inverse() * (B.transpose() * S * A);
    ...
    EXPECT_TRUE(CompareMatrices(K * x0, output->get_vector_data(0)->get_value(), kTolerance));
}
...
public:
    explicit CubicPolynomialSystem(double time_step)
        : LeafSystem<T>(SystemTypeTag<CubicPolynomialSystem>{}), time_step_(time_step) {
        this->DeclareInputPort(kUseDefaultName, kVectorValued, 1);
        this->DeclareVectorOutputPort(kUseDefaultName, 2, &CubicPolynomialSystem::OutputState,
    ...
    template <typename U>
    CubicPolynomialSystem(const CubicPolynomialSystem<U>& other) : CubicPolynomialSystem(other.time_step_) {}
...
private:
    ...
    // x1(k+1) = u(k)
    // x2(k+1) = -x1³(k)
    void CalcDiscreteUpdate(const Context<T>& context, DiscreteValues<T>* next_state) const {
        using std::pow;
    ...
    void OutputState(const systems::Context<T>& context, BasicVector<T>* output) const {
...
protected:
    const System<double>& GetSystemByName(std::string name, const Diagram<double>& diagram) {
    ...
    void MakeTimeInvariantMpcController() {
        ...
        BasicVector<double>& x = context->get_mutable_discrete_state().get_mutable_vector();
        x.SetFromVector(Eigen::Vector2d::Zero());  // Fixed point is zero.

        dut_.reset(new LinearModelPredictiveController<double>(std::move(system_), std::move(context), Q_, R_,
    ...
    void MakeControlledSystem(bool is_time_varying) {
        ...
        builder.Connect(cubic_system->get_output_port(0), controller->get_state_port());
        builder.Connect(controller->get_control_port(), cubic_system->get_input_port(0));
    ...
    void Simulate() {
        ...
        Context<double>& cubic_system_context =
                diagram_->GetMutableSubsystemContext(cubic_system, &simulator_->get_mutable_context());
        BasicVector<double>& x0 = cubic_system_context.get_mutable_discrete_state().get_mutable_vector();
...
TEST_F(TestMpcWithCubicSystem, TimeInvariantMpc) {
    ...
    // one step.
    Eigen::Vector2d result = simulator_->get_mutable_context().get_discrete_state(0).get_value();
...
GTEST_TEST(TestMpcConstructor, ThrowIfRNotStrictlyPositiveDefinite) {
    ...
    std::unique_ptr<LinearSystem<double>> system = std::make_unique<LinearSystem<double>>(A, B, C, D, 1.);
    ...
    EXPECT_THROW(LinearModelPredictiveController<double>(std::move(system), std::move(context), Q, R, 1., 1.),
                 std::runtime_error);
}
