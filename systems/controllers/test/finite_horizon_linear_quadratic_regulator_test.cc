...
GTEST_TEST(FiniteHorizonLQRTest, InfiniteHorizonTest) {
    ...
    LinearQuadraticRegulatorResult lqr_result = LinearQuadraticRegulator(A, B, Q, R, N);
    ...
    FiniteHorizonLinearQuadraticRegulatorResult result =
            FiniteHorizonLinearQuadraticRegulator(sys, *context, t0, tf, Q, R, options);
    ...
    const std::unique_ptr<System<double>> regulator =
            MakeFiniteHorizonLinearQuadraticRegulator(sys, *context, t0, tf, Q, R, options);
    ...
    EXPECT_TRUE(CompareMatrices(regulator->get_output_port(0).Eval(*regulator_context), -lqr_result.K * x, 1e-5));

    // Test that it stays at the fixed-point if initialized at the fixed point.
    options.Qf = lqr_result.S;
    result = FiniteHorizonLinearQuadraticRegulator(sys, *context, t0, t0 + 2.0, Q, R, options);
...
GTEST_TEST(FiniteHorizonLQRTest, NominalTrajectoryTest) {
    ...
    const auto system = SymbolicVectorSystemBuilder().state(x).input(u).dynamics(-x + pow(x, 3) + u).Build();
    ...
    for (const double x0 : std::vector<double>({-2, -1., 0., 1, 2})) {
        ...
        LinearQuadraticRegulatorResult lqr_result =
                LinearQuadraticRegulator(linear_sys->A(), linear_sys->B(), Q, R, Eigen::Matrix<double, 0, 0>());
        ...
        EXPECT_TRUE(CompareMatrices(result.S->value(t0), result.S->value(tf), 1e-12));
        EXPECT_TRUE(CompareMatrices(result.S->value(t0), lqr_result.S, 1e-12));
        EXPECT_TRUE(CompareMatrices(result.K->value(t0), lqr_result.K, 1e-12));
        ...
        // Test that the System version also works.
        const std::unique_ptr<System<double>> regulator =
                MakeFiniteHorizonLinearQuadraticRegulator(*system, *context, t0, tf, Q, R, options);
        auto regulator_context = regulator->CreateDefaultContext();
        const Vector1d xs(-.3);
        regulator->get_input_port(0).FixValue(regulator_context.get(), xs);
        EXPECT_EQ(regulator->get_input_port(0).size(), 1);
        EXPECT_EQ(regulator->get_output_port(0).size(), 1);
        EXPECT_TRUE(CompareMatrices(regulator->get_output_port(0).Eval(*regulator_context),
...
GTEST_TEST(FiniteHorizonLQRTest, DoubleIntegratorWithNonZeroGoal) {
    ...
    LinearQuadraticRegulatorResult lqr_result = LinearQuadraticRegulator(A, B, Q, R);
    ...
    EXPECT_TRUE(CompareMatrices(result.s0->value(t0), xdv.transpose() * lqr_result.S * xdv, 1e-5));
    ...
    regulator->get_input_port(0).FixValue(regulator_context.get(), x);
    EXPECT_TRUE(
            CompareMatrices(regulator->get_output_port(0).Eval(*regulator_context), -lqr_result.K * (x - xdv), 1e-5));
}
...
GTEST_TEST(FiniteHorizonLQRTest, AffineSystemTest) {
    ...
    AffineSystem<double> sys(A, B, c, Eigen::Matrix<double, 0, 2>(), Eigen::Matrix<double, 0, 2>(),
    ...
    const Eigen::Vector2d x(.1, -.3);
    regulator->get_input_port(0).FixValue(regulator_context.get(), x);
    EXPECT_TRUE(CompareMatrices(regulator->get_output_port(0).Eval(*regulator_context), udv - lqr_result.K * x, 4e-5));
...
GTEST_TEST(FiniteHorizonLQRTest, ResultSystemIsScalarConvertible) {
    ...
    const std::unique_ptr<System<double>> regulator =
            MakeFiniteHorizonLinearQuadraticRegulator(sys, *context, t0, t0 + 0.1, Q, R);
...
GTEST_TEST(FiniteHorizonLQRTest, SimulatorConfig) {
    ...
    const trajectories::PiecewisePolynomial<double>* S =
            dynamic_cast<const trajectories::PiecewisePolynomial<double>*>(result.S.get());
    ASSERT_NE(S, nullptr);
    EXPECT_EQ(S->get_number_of_segments(), 30);

    options.simulator_config.max_step_size = 0.2;
    result = FiniteHorizonLinearQuadraticRegulator(sys, *context, t0, tf, Q, R, options);
    S = dynamic_cast<const trajectories::PiecewisePolynomial<double>*>(result.S.get());
    ASSERT_NE(S, nullptr);
