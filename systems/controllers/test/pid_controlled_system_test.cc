...
#include "systems/primitives/saturation.h"
...
public:
    ...
    double GetInputValue(const Context<double>& context) { return get_input_port(0).Eval(context)[0]; }
};
...
public:
    TestPlantWithMinOutputs() {
        ...
        DeclareVectorOutputPort(kUseDefaultName, 2, &TestPlantWithMinOutputs::CalcOutputVector,
                                {this->nothing_ticket()});
    }

    void CalcOutputVector(const Context<double>&, BasicVector<double>* output) const {
...
public:
    TestPlantWithMoreOutputs() {
        DeclareVectorInputPort(kUseDefaultName, 1);
        DeclareVectorOutputPort(kUseDefaultName, 6, &TestPlantWithMoreOutputs::CalcOutputVector,
                                {this->nothing_ticket()});
    }
...
public:
    TestPlantWithMoreOutputPorts() {
        ...
        DeclareVectorOutputPort(kUseDefaultName, 3, &TestPlantWithMoreOutputPorts::CalcNonStateOutputVector,
                                {this->nothing_ticket()});
        DeclareVectorOutputPort(kUseDefaultName, 2, &TestPlantWithMoreOutputPorts::CalcStateOutputVector,
                                {this->nothing_ticket()});
    }

    void CalcNonStateOutputVector(const Context<double>&, BasicVector<double>* output) const {
        BasicVector<double>& output_vector = *output;
        output_vector[0] = 2.;
        output_vector[1] = 0.2;
    ...
    void CalcStateOutputVector(const Context<double>&, BasicVector<double>* output) const {
        BasicVector<double>& output_vector = *output;
        output_vector[0] = 43.;
...
protected:
    ...
    void DoPidControlledSystemTest(std::unique_ptr<TestPlant> plant, const MatrixX<double>& feedback_selector) {
        ...
        auto controller = builder.AddSystem<PidControlledSystem>(std::move(plant), feedback_selector, Kp_, Ki_, Kd_);

        builder.Connect(input_source->get_output_port(), controller->get_input_port(0));
        builder.Connect(state_source->get_output_port(), controller->get_input_port(1));
        ...
        const systems::Context<double>& plant_context = diagram_->GetSubsystemContext(*controller->plant(), *context);

        diagram_->CalcOutput(*context, output.get());
        const BasicVector<double>* output_vec = output->get_vector_data(0);
        const double pid_input = dynamic_cast<TestPlant*>(controller->plant())->GetInputValue(plant_context);
        EXPECT_EQ(pid_input, input[0] + (state[0] - output_vec->get_value()[0]) * Kp_(0) +
...
TEST_F(PidControlledSystemTest, ExistingNamesRespected) {
    ...
    PidControlledSystem<double> controller(std::move(plant), MatrixX<double>::Identity(state_size, state_size), Kp_,
                                           Ki_, Kd_);
...
TEST_F(PidControlledSystemTest, SimplePidControlledSystem) {
    ...
    auto plant = std::make_unique<TestPlantWithMinOutputs>();
    const int state_size = plant->get_output_port(0).size();
    DoPidControlledSystemTest(std::move(plant), MatrixX<double>::Identity(state_size, state_size));
}
...
TEST_F(PidControlledSystemTest, PlantWithMoreOutputs) {
    ...
    MatrixX<double> feedback_selector_matrix(controller_feedback_size, plant_output_size);
...
TEST_F(PidControlledSystemTest, PlantWithMoreOutputPorts) {
    ...
    PidControlledSystem<double> system(std::move(plant), Kp_, Ki_, Kd_, state_output_port_index,
...
protected:
    void SetUp() override {
        ...
        feedback_selector_ =
                MatrixX<double>::Identity(plant_ptr->get_output_port(0).size(), plant_ptr->get_output_port(0).size());
    ...
    void ConnectPidPorts(PidControlledSystem<double>::ConnectResult plant_pid_ports) {
        builder_.Connect(input_source_->get_output_port(), plant_pid_ports.control_input_port);
        builder_.Connect(state_source_->get_output_port(), plant_pid_ports.state_input_port);
    }
    ...
    double ComputePidInput() {
        ...
        auto plant_output = standard_diagram->GetSubsystemByName(plant_->get_name()).AllocateOutput();
        auto& plant_context = standard_diagram->GetSubsystemContext(*plant_, *context);

        plant_->CalcOutput(plant_context, plant_output.get());
        const BasicVector<double>* output_vec = plant_output->get_vector_data(0);
        const double pid_input = dynamic_cast<TestPlant*>(plant_)->GetInputValue(plant_context);
...
TEST_F(ConnectControllerTest, NonSaturatingController) {
    auto plant_pid_ports = PidControlledSystem<double>::ConnectController(
            plant_->get_input_port(0), plant_->get_output_port(0), feedback_selector_, Kp, Ki, Kd, &builder_);

    ConnectPidPorts(plant_pid_ports);

    const double pid_input = ComputePidInput();
    double calculated_input = plant_input_[0] + (desired_state_[0] - output_position_) * Kp(0) +
...
TEST_F(ConnectControllerTest, SaturatingController) {
    ...
    auto plant_pid_ports = PidControlledSystem<double>::ConnectControllerWithInputSaturation(
            plant_->get_input_port(0), plant_->get_output_port(0), feedback_selector_, Kp, Ki, Kd,
            Vector1d(0.0) /* u_min */, Vector1d(saturation_max), &builder_);
