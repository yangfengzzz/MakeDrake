...
GTEST_TEST(JointStiffnessControllerTest, SimpleDoublePendulum) {
    ...
    multibody::Parser(plant).AddModelsFromUrl("package://drake/multibody/benchmarks/acrobot/double_pendulum.urdf");
    ...
    builder.ExportInput(controller->get_input_port_desired_state(), "desired_state");
    ...
    VectorXd tau_expected =
            -plant->CalcGravityGeneralizedForces(plant_context) + kDamping * x.tail<2>() +
            (kp.array() * (x_d.head<2>() - x.head<2>()).array() + kd.array() * (x_d.tail<2>() - x.tail<2>()).array())
                    .matrix();
...
GTEST_TEST(JointStiffnessControllerTest, ScalarConversion) {
    ...
    VectorXd kp = VectorXd::Constant(num_q, 0.12), kd = VectorXd::Constant(num_q, 0.34);
    ...
    EXPECT_EQ(controller_sym->get_input_port_estimated_state().size(), num_states);

    JointStiffnessController<double> controller_with_ownership(std::move(mbp), kp, kd);
    // Test AutoDiffXd.
    controller_ad = systems::System<double>::ToAutoDiffXd(controller_with_ownership);
    // Check the multibody plant.
    EXPECT_EQ(controller_ad->get_input_port_estimated_state().size(), num_states);

    // Test Expression.
    controller_sym = systems::System<double>::ToSymbolic(controller_with_ownership);
    EXPECT_EQ(controller_sym->get_input_port_estimated_state().size(), num_states);
}
