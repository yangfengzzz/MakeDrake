...
TEST_F(PidControllerTest, PortNames) {
    ...
    EXPECT_EQ(controller_.GetOutputPort("control").get_index(), controller_.get_output_port_control().get_index());
}
...
TEST_F(PidControllerTest, CalcOutput) {
    ...
    EXPECT_EQ((kp_.array() * error_signal_.array() + kd_.array() * error_rate_signal_.array()).matrix(),
    ...
    controller_.CalcOutput(*context_, output_.get());
    EXPECT_EQ((kp_.array() * error_signal_.array() + ki_.array() * integral_value.array() +
...
TEST_F(PidControllerTest, ToAutoDiff) {
    ...
    const auto* const downcast = dynamic_cast<PidController<AutoDiffXd>*>(clone.get());
...
GTEST_TEST(PidConstructorTest, TestThrow) {
    // Gains size mismatch.
    EXPECT_THROW(PidController<double>(VectorX<double>(2), VectorX<double>(3), VectorX<double>(3)), std::logic_error);
    // State projection row != 2 * |kp|.
    EXPECT_THROW(
            PidController<double>(MatrixX<double>(5, 2), VectorX<double>(3), VectorX<double>(3), VectorX<double>(3)),
            std::logic_error);
    // Output projection col != |kp|
    EXPECT_THROW(PidController<double>(MatrixX<double>(6, 2), MatrixX<double>(3, 2), VectorX<double>(3),
...
GTEST_TEST(PidIOProjectionTest, Test) {
    ...
    MatrixX<double> P_input = MatrixX<double>::Random(2 * num_controlled_q, 2 * num_full_q);
    MatrixX<double> P_output = MatrixX<double>::Random(num_control, num_controlled_q);
    ...
    VectorX<double> output_expected = (kp.array() * q_err.array()).matrix() + (kd.array() * v_err.array()).matrix() +
                                      (ki.array() * integral.array()).matrix();
    output_expected = P_output * output_expected;

    EXPECT_TRUE(CompareMatrices(output_expected, output->get_vector_data(0)->get_value(), 1e-12,
