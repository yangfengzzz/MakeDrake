#include "systems/controllers/linear_quadratic_regulator.h"
...
GTEST_TEST(TestLqr, TestException) {
    ...
    EXPECT_THROW(LinearQuadraticRegulator(A, B, Q, Eigen::Matrix<double, 1, 1>::Zero()), std::runtime_error);
    EXPECT_THROW(LinearQuadraticRegulator(A, B, Q, Eigen::Matrix<double, 1, 1>::Zero(), N), std::runtime_error);
}

void TestLqrAgainstKnownSolution(double tolerance,
    ...
    LinearQuadraticRegulatorResult result = LinearQuadraticRegulator(A, B, Q, R, N);
    EXPECT_TRUE(CompareMatrices(K_known, result.K, tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(S_known, result.S, tolerance, MatrixCompareType::absolute));
}
...
void TestLqrLinearSystemAgainstKnownSolution(
        ...
        const Eigen::Ref<const Eigen::MatrixXd>& R,
        const Eigen::Ref<const Eigen::MatrixXd>& N = Eigen::Matrix<double, 0, 0>::Zero()) {
    std::unique_ptr<LinearSystem<double>> linear_lqr = LinearQuadraticRegulator(sys, Q, R, N);

    int n = sys.A().rows();
    int m = sys.B().cols();
    EXPECT_TRUE(CompareMatrices(linear_lqr->A(), Eigen::Matrix<double, 0, 0>::Zero(), tolerance,
                                MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(linear_lqr->B(), Eigen::MatrixXd::Zero(0, n), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(linear_lqr->C(), Eigen::MatrixXd::Zero(m, 0), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(linear_lqr->D(), -K_known, tolerance, MatrixCompareType::absolute));
...
void TestLqrAffineSystemAgainstKnownSolution(
    ...
    std::unique_ptr<AffineSystem<double>> lqr = LinearQuadraticRegulator(sys, *context, Q, R, N);

    EXPECT_TRUE(CompareMatrices(lqr->A(), Eigen::Matrix<double, 0, 0>::Zero(), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(lqr->B(), Eigen::MatrixXd::Zero(0, n), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(
            CompareMatrices(lqr->f0(), Eigen::Matrix<double, 0, 1>::Zero(), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(lqr->C(), Eigen::MatrixXd::Zero(m, 0), tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(lqr->D(), -K_known, tolerance, MatrixCompareType::absolute));
    EXPECT_TRUE(CompareMatrices(lqr->y0(), u0 + K_known * x0, tolerance, MatrixCompareType::absolute));
...
void TestLqrWithHjb(const Eigen::Ref<const Eigen::MatrixXd>& A,
                    const Eigen::Ref<const Eigen::MatrixXd>& B,
                    const Eigen::Ref<const Eigen::MatrixXd>& Q,
                    const Eigen::Ref<const Eigen::MatrixXd>& R,
    ...
    EXPECT_TRUE(CompareMatrices(Ry * lqr_result.K * Pt, By.transpose() * Sy + Ny.transpose(), tol));
    // Now make sure that by plugging in u = -Kx = -KPᵀy, the left hand side of
    // HJB is zero.
    const Eigen::MatrixXd Ky = lqr_result.K * Pt;
    EXPECT_TRUE(CompareMatrices(Qy + Ky.transpose() * Ry * Ky - Ny * Ky - Ky.transpose() * Ny.transpose() + Sy * Ay +
                                        Ay.transpose() * Sy - Sy * By * Ky - Ky.transpose() * By.transpose() * Sy,
...
GTEST_TEST(TestLqr, DoubleIntegrator) {
    ...
    A << 0, 1, 0, 0;
    B << 0, 1;
    LinearSystem<double> sys(A, B, Eigen::Matrix<double, 0, 2>::Zero(), Eigen::Matrix<double, 0, 1>::Zero());
...
GTEST_TEST(TestLqr, DiscreteDoubleIntegrator) {
  ...
  S << 2.947122966707012, 2.369205407092467,
    ...
    LinearQuadraticRegulatorResult result = DiscreteTimeLinearQuadraticRegulator(A, B, Q, R);

    const double tol = 1e-10;
    EXPECT_TRUE(CompareMatrices(result.K, K, tol));
    EXPECT_TRUE(CompareMatrices(result.S, S, tol));

    LinearSystem<double> sys(A, B, Eigen::Matrix<double, 0, 2>::Zero(), Eigen::Matrix<double, 0, 1>::Zero(), 0.1);
...
GTEST_TEST(TestLqr, AcrobotTest) {
    ...
    examples::acrobot::AcrobotState<double>& state = plant.get_mutable_state(context.get());
    ...
    const auto lqr_result = LinearQuadraticRegulator(linear_system->A(), linear_system->B(), Q, R);
    ...
    multibody::Parser(&mbp).AddModels(FindResourceOrThrow("drake/examples/acrobot/Acrobot.urdf"));
    mbp.Finalize();
    ...
    const auto mbp_controller = LinearQuadraticRegulator(mbp, *mbp_context, Q, R, Eigen::Matrix<double, 4, 1>::Zero(),
