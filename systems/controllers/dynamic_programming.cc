...
DynamicProgrammingOptions::PeriodicBoundaryCondition::PeriodicBoundaryCondition(int state_index_in,
                                                                                double low_in,
                                                                                double high_in)
...
std::pair<std::unique_ptr<BarycentricMeshSystem<double>>, Eigen::RowVectorXd> FittedValueIteration(
    ...
    DRAKE_DEMAND(context.has_only_continuous_state() || options.assume_non_continuous_states_are_fixed);
    DRAKE_DEMAND(context.num_continuous_states() == state_size);

    const InputPort<double>* input_port = system.get_input_port_selection(options.input_port_index);
    ...
    for (int input = 0; input < num_inputs; input++) {
        ...
        for (int state = 0; state < num_states; state++) {
            ...
            for (const auto& b : options.periodic_boundary_conditions) {
                state_vec[b.state_index] = math::wrap_to(state_vec[b.state_index], b.low, b.high);
            }
        ...
        for (int state = 0; state < num_states; state++) {
            ...
            for (int input = 0; input < num_inputs; input++) {
                ...
                for (int index = 0; index < num_state_indices; index++) {
                    Q += options.discount_factor * T[input](index, state) * J(Tind[input](index, state));
                }
...
Eigen::VectorXd LinearProgrammingApproximateDynamicProgramming(
    ...
    drake::log()->info("Computing one-step dynamics and setting up the linear program.");
    solvers::MathematicalProgram prog;

    const solvers::VectorXDecisionVariable params = prog.NewContinuousVariables(num_parameters, "a");
    ...
    for (int input = 0; input < num_inputs; input++) {
        ...
        for (int state = 0; state < num_states; state++) {
            ...
            for (const auto& b : options.periodic_boundary_conditions) {
                next_state_vec[b.state_index] = math::wrap_to(next_state_vec[b.state_index], b.low, b.high);
            }

            const symbolic::Formula f =
                    (J(state_vec, params) <= cost + options.discount_factor * J(next_state_vec, params));
    ...
    if (!result.is_success()) {
        drake::log()->error("No solution found.  SolutionResult = " + to_string(result.get_solution_result()));
    }
