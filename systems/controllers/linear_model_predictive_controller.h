...
public:
    ...
    /// Diagram.
    LinearModelPredictiveController(std::unique_ptr<systems::System<double>> model,
    ...
    const InputPort<T>& get_state_port() const { return this->get_input_port(state_input_index_); }
    const OutputPort<T>& get_control_port() const { return this->get_output_port(control_output_index_); }

private:
    void CalcControl(const Context<T>& context, BasicVector<T>* control) const;

    EventStatus DoNothingButPretendItWasSomething(const Context<T>&, DiscreteValues<T>*) const;

    // Sets up a DirectTranscription problem and solves for the current control
    // input.
    VectorX<T> SetupAndSolveQp(const Context<T>& base_context, const VectorX<T>& current_state) const;
